...
# Modify.........: No.FUN-CB0002 12/11/05 By lujh CR轉XtraGrid
# Modify.........: No.FUN-D40128 13/04/28 By wangrr 增加料件規格和工作站名稱欄位
# Modify.........: No:FUN-D40129 13/05/08 By yangtt pid01,pie02新增開窗
...
DEFINE rpt_type    LIKE type_file.chr1   #FUN-CB0002  add
...
#070402 MOD-720046 By TSD.Jin--start
   ...
   LET g_sql = "sw.type_file.chr1,",  
               ...
               "ecd02.ecd_file.ecd02,",   #作業說明#FUN-D40128
               ...
               "ima021.ima_file.ima021,", #規格 #FUN-D40128
               ...
               "Workstation.type_file.chr1000"     #FUN-CB0002  add
   LET l_table = cl_prt_temptable('aimx853',g_sql) CLIPPED   # 產生Temp Table
   IF l_table = -1 THEN EXIT PROGRAM END IF                  # Temp Table產生
  
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ",
               "        ?,?,?,?,?, ?,?,?,?,?, ",
               "        ?,?,?,?,?, ?,?) " #FUN-770071 add ?  #NO.FUN-A60080 add ?  #FUN-CB0002  add  ? #FUN-D40128 add 2?
...
#070402 MOD-720046 By TSD.Jin--end
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'	# If background job sw is off
      THEN CALL x853_tm(0,0)		# Input print condition
      ELSE CALL x853()		# Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
END MAIN
 
FUNCTION x853_tm(p_row,p_col)
...
DEFINE p_row,p_col	LIKE type_file.num5,    #No.FUN-690026 SMALLINT
   ...
   OPEN WINDOW x853_w AT p_row,p_col
        WITH FORM "aim/42f/aimx853"
...
WHILE TRUE
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW x853_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimx853',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW x853_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
#       l_time      LIKE type_file.chr8,    # Used time for running the job  #No.FUN-690026 VARCHAR(8) #No.FUN-6A0074
	...
	sr3         RECORD
                    ...
                    ima021   LIKE ima_file.ima021,   #規格   #FUN-D40128
...
#070402 MOD-720046 By TSD.Jin--start
   DEFINE l_ima02  LIKE ima_file.ima02,     #品名規格
          l_ima021 LIKE ima_file.ima021,    #規格  #FUN-D40128
          l_ima08  LIKE ima_file.ima08,     #來源碼
          l_ima25  LIKE ima_file.ima25      #庫存單位
   DEFINE l_ecd02  LIKE ecd_file.ecd02      #FUN-D40128
 
   ## *** 與 Crystal Reports 串聯段 - <<<< 清除暫存資料 >>>> *** ##
   CALL cl_del_data(l_table)
 
   DELETE FROM aim_x853
#070402 MOD-720046 By TSD.Jin--end
 
     SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aimx853'
...
#NO.CHI-6A0004--END
    ...
    #LET g_len = 155       #No.FUN-550029   #FUN-5A0199 mark
     ...
     PREPARE x853_prepare FROM l_sql
     ...
     END IF
     DECLARE x853_cs  CURSOR FOR x853_prepare
     LET l_cnt = 0
     FOREACH x853_cs  INTO sr2.*
       IF SQLCA.sqlcode != 0 THEN
...
#MOD-C70065---add-----END
       ...
       END IF
       INSERT INTO aim_x853 VALUES(sr2.pid02,sr2.pid03,
                                   sr2.pid01,sr2.pie02,
                                   sr2.pie03,sr2.pie04,
                                   sr2.pie07,sr2.pie05,
                                   l_uninv,  l_count,
       ...
       IF SQLCA.sqlcode THEN
          ...
          CALL cl_err3("ins","aim_x853",sr2.pid02,"",SQLCA.sqlcode,"",
     ...
     PREPARE x853_prepare2 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
        EXIT PROGRAM
     ...
     END IF
     DECLARE x853_cs2  CURSOR FOR x853_prepare2
 
     #---->讀取料件排行(明細表)
     LET l_sql = " SELECT '',aim_x853.*,0,ima02,ima021,ima25 ", #FUN-D40128 add ima021
                 " FROM aim_x853 ,OUTER ima_file",
                 " WHERE aim_x853.pie02 = ima_file.ima01 AND  pie02 = ? "
 
     PREPARE x853_prepare3 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
        EXIT PROGRAM
     ...
     END IF
     DECLARE x853_cs3  CURSOR FOR x853_prepare3
     ...
     PREPARE x853_ppart2  FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
        EXIT PROGRAM
     ...
     END IF
     DECLARE x853_cpart2  CURSOR FOR x853_ppart2
     ...
     PREPARE x853_ppart1  FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
        EXIT PROGRAM
     ...
     END IF
     DECLARE x853_cpart1  CURSOR FOR x853_ppart1
 
     #---->讀取相關資料
     LET l_sql = "SELECT unique ima02,ima021,ima25,ima_file.ima08,cost ", #FUN-D40128 add ima021
                 "  FROM ima_file,aim_x853",
                 " WHERE ima01 = ?  ",
                 "   AND ima01 = pie02 "
     PREPARE x853_p5  FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
        EXIT PROGRAM
     ...
     END IF
     DECLARE x853_cs5  CURSOR FOR x853_p5
 
    #070402 MOD-720046 By TSD.Jin--start mark
    #CALL cl_outnam('aimx853') RETURNING l_name
    #LET l_name2=l_name
    #LET l_name2[11,11]='x'
    ...
    #CASE tm.c
    #   WHEN '1'                                  #總表
    #      START REPORT x853_rep  TO l_name
    #   WHEN '2'                                  #明細表
    #      START REPORT x853_rep2 TO l_name2
    #   WHEN '3'                                  #全部
    #      START REPORT x853_rep TO l_name
    #      START REPORT x853_rep2 TO l_name2
    #   OTHERWISE EXIT CASE
    ...
    #070402 MOD-720046 By TSD.Jin--end mark
     ...
     IF tm.c ='1' OR tm.c ='3' THEN
        FOREACH x853_cs2  INTO sr4.pie02, sr4.uninv, sr4.diff,
                               sr4.count, sr4.tot
           IF SQLCA.sqlcode != 0 THEN
              CALL cl_err('foreach x853_cs2:',SQLCA.sqlcode,1)
              EXIT FOREACH
           END IF
           IF tm.d = 'N' THEN
           ...
           END IF
        #070402 MOD-720046 By TSD.Jin--start
        #  OUTPUT TO REPORT x853_rep(sr4.*)
           LET l_ima02 = NULL
           LET l_ima021= NULL #FUN-D40128
           LET l_ima08 = NULL
           LET l_ima25 = NULL
           LET l_cost  = 0
           OPEN x853_cs5 USING sr4.pie02
           FETCH x853_cs5 INTO l_ima02,l_ima021,l_ima25,l_ima08,l_cost #FUN-D40128 add l_ima021
           CLOSE x853_cs5 
           
           ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> *** ##
           EXECUTE insert_prep USING
              sr4.sw,'','','','',
              sr4.pie02,'','','','','',  #FUN-D40128 add ''
              sr4.uninv,sr4.count,sr4.diff,l_cost,sr4.tot,  
              l_ima02,l_ima021,l_ima08,l_ima25,g_azi03,g_azi04, #FUN-D40128 add l_ima021
              g_azi05,'1'   #FUN-770071 add '1'   #1.總表
              ,sr4.pie012,sr4.pie013,''  #No.FUN-A60080   #FUN-CB0002  add ''
           #FUN-CB0002--add--str--
           LET rpt_type = '1'    #FUN-CB0002  add
           IF sr4.sw = '2' THEN 
              LET g_xgrid.headerinfo1 = cl_getmsg('aim-202',g_lang)
           ELSE
              LET g_xgrid.headerinfo1 = cl_getmsg('aim-203',g_lang)
           END IF 
           #FUN-CB0002--add--end--
        #070402 MOD-720046 By TSD.Jin--end  
        END FOREACH
    ...
    IF tm.c = '2' OR tm.c ='3' THEN
       LET l_seq = 0
       #--->負值排列
       FOREACH x853_cpart1  INTO l_part
        IF SQLCA.sqlcode != 0 THEN
           CALL cl_err('foreach x853_cpart1 error:',SQLCA.sqlcode,1)
           EXIT FOREACH
        END IF
           LET l_seq = l_seq + 1
           FOREACH x853_cs3
           ...
           INTO sr3.*
             IF SQLCA.sqlcode != 0 THEN
                CALL cl_err('foreach x853_cs3 error:',SQLCA.sqlcode,1)
                EXIT FOREACH
             END IF
                LET l_sw = '1'
                LET sr3.seq = l_seq
             ...
             #070402 MOD-720046 By TSD.Jin--start
             #  OUTPUT TO REPORT x853_rep2(l_sw,sr3.*)    #明細
                ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> *** ##
             LET l_ecd02=NULL  #FUN-D40128
             SELECT ecd02 INTO l_ecd02 FROM ecd_file WHERE ecd01=sr3.pie03  #FUN-D40128

                EXECUTE insert_prep USING
                   l_sw,sr3.seq,sr3.pid02,sr3.pid03,sr3.pie01,
                   sr3.pie02,sr3.pie03,l_ecd02,sr3.pie04,sr3.pie07,sr3.pie05, #FUN-D40128 add l_ecd02
                   sr3.uninv,sr3.cnt,sr3.diff,sr3.cost,sr3.tot,        
                   sr3.ima02,sr3.ima021,sr3.ima08,sr3.ima25,g_azi03,g_azi04,#FUN-D40128 add ima021
                   g_azi05,'2'   #FUN-770071 add '2'   #2.明細表
                   ,sr3.pie012,sr3.pie013,'' #No.FUN-A60080  #FUN-CB0002  add ''
                #FUN-CB0002--add--str--
                LET rpt_type = '2'   
                IF l_sw = '2' THEN 
                   LET g_xgrid.headerinfo1 = cl_getmsg('aim-202',g_lang)
       ...
       END FOREACH
       #--->正值排列
       FOREACH x853_cpart2  INTO l_part
        IF SQLCA.sqlcode != 0 THEN
           CALL cl_err('foreach x853_cpart2 error:',SQLCA.sqlcode,1)
           EXIT FOREACH
        END IF
           LET l_seq = l_seq + 1
           FOREACH x853_cs3
           ...
           INTO sr3.*
             ...
             END IF
                ...
                LET sr3.seq = l_seq
                LET sr3.tot = sr3.diff * sr3.cost
                LET sr3.tot = sr3.tot * -1     #FUN-CB0002  add
             #070402 MOD-720046 By TSD.Jin--start
             #  OUTPUT TO REPORT x853_rep2(l_sw,sr3.*) #明細
                ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> *** ##
             LET l_ecd02=NULL  #FUN-D40128
   ...
   #070402 MOD-720046 By TSD.Jin--start
   #CASE
   #  WHEN tm.c = '1' FINISH REPORT x853_rep
   #                  DELETE FROM aim_x853
   #  WHEN tm.c = '2' FINISH REPORT x853_rep2
   #                 #No.+366 010705 plum
   #                  LET l_cmd = "chmod 777 ", l_name2
   #                  RUN l_cmd
   #                 #No.+366..end
   #                  LET l_name = l_name2
   #  WHEN tm.c = '3' FINISH REPORT x853_rep
   #                  FINISH REPORT x853_rep2
   #                 #No.+366 010705 plum
   #                  LET l_cmd = "chmod 777 ", l_name2
   #                  RUN l_cmd
   #                 #No.+366..end
   ...
   # CALL cl_prt(l_name,g_prtway,g_copies,g_len)
    ...
    #END IF
    #FUN-CB0002--end--str--
###XtraGrid###    LET g_str = g_str,";",g_costdesc
 
###XtraGrid###    LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
    #No.FUN-A60080--begin
    #CASE tm.c WHEN '1' CALL cl_prt_cs3('aimx853','aimx853_1',l_sql,g_str)
    #          WHEN '2' CALL cl_prt_cs3('aimx853','aimx853_2',l_sql,g_str)
    #          WHEN '3' CALL cl_prt_cs3('aimx853','aimx853_3',l_sql,g_str)   #FUN-770071 add
    #END CASE
    CASE tm.c WHEN '1' 
              IF g_sma.sma541='N' THEN 
###XtraGrid###                 CALL cl_prt_cs3('aimx853','aimx853_1',l_sql,g_str)
                 LET g_xgrid.template = 'aimx853_1'
                 ###XtraGrid###
              ELSE
###XtraGrid###                 CALL cl_prt_cs3('aimx853','aimx853_4',l_sql,g_str)   
                 LET g_xgrid.template = 'aimx853_4'              
              END IF            
              WHEN '2' 
              IF g_sma.sma541='N' THEN 
###XtraGrid###                 CALL cl_prt_cs3('aimx853','aimx853_2',l_sql,g_str)
                 LET g_xgrid.template = 'aimx853_2'
              ELSE
###XtraGrid###                 CALL cl_prt_cs3('aimx853','aimx853_5',l_sql,g_str)  
                 LET g_xgrid.template = 'aimx853_5'               
              END IF            
              WHEN '3'
              IF g_sma.sma541='N' THEN 
###XtraGrid###                 CALL cl_prt_cs3('aimx853','aimx853_3',l_sql,g_str)
                 IF rpt_type = '1' THEN 
                    LET g_xgrid.template = 'aimx853_1'
                 ELSE
                    LET g_xgrid.template = 'aimx853_2'
                 END IF 
              ELSE
###XtraGrid###                 CALL cl_prt_cs3('aimx853','aimx853_6',l_sql,g_str) 
                 IF rpt_type = '1' THEN 
                    LET g_xgrid.template = 'aimx853_4'
                 ELSE
                    LET g_xgrid.template = 'aimx853_5'
    ...
    LET g_xgrid.order_field = "pid03,pie02,pid02"
    LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'cost',g_costdesc,'')
    IF g_zz05 = 'Y' THEN
       CALL cl_wcchp(tm.wc,'pid03,pie02,pid01,pid02')
       RETURNING tm.wc
    END IF
...
#   ON EVERY ROW
#      OPEN x853_cs5 USING sr4.pie02
#      FETCH x853_cs5 INTO l_ima02,l_ima25,l_ima08,l_cost
...
FUNCTION x853_temp()
  ...
  CREATE TEMP TABLE aim_x853(
      ...
      pie012  LIKE pie_file.pie012,   #No.FUN-A60080
      pie013  LIKE pie_file.pie013);  #No.FUN-A60080
    CREATE  index s_1  on aim_x853 (pie02);
END FUNCTION
