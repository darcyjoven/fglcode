...
# Modify.........: No:FUN-CB0005 12/11/01 By chenying CR轉XtraGrid
# Modify.........: No:FUN-D30070 13/03/21 By yangtt 去掉頁脚排序顯示 
# Modify.........: No:FUN-D40129 13/05/08 By yangtt ima09,ima10,ima11新增開窗
...
#FUN-CB0005--add--str--
                " l_total.type_file.num15_3,",
                " l_diff.ima_file.ima27,",
                " l_diff1.ima_file.ima38,",
                " l_num.type_file.num5 "  
#FUN-CB0005--add--end--
                                                                                                                                    
    LET l_table = cl_prt_temptable('aimx503',g_sql) CLIPPED   # 產生Temp Table                                                      
...
#No.FUN-750097---------------end------------
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL x503_tm()	        	# Input print condition
      ELSE CALL aimx503()			# Read data and create out-file
   END IF
...
DEFINE l_cmd	     LIKE type_file.chr1000, #No.FUN-690026 VARCHAR(1000)
   ...
   OPEN WINDOW x503_w AT p_row,p_col
        WITH FORM "aim/42f/aimx503"
################################################################################
...
#No.FUN-570240 --end
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW x503_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
#        LET tm.t = tm2.t1,tm2.t2,tm2.t3
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimx503',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW x503_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
#                              diff2  LIKE ima_file.ima26, #No.FUN-670066     #FUN-A20044
     ...
     DEFINE l_str             STRING                       #FUN-CB0005
     DEFINE l_total           LIKE type_file.num15_3       #FUN-CB0005
     DEFINE l_diff            LIKE ima_file.ima27          #FUN-CB0005
     DEFINE l_diff1           LIKE ima_file.ima38          #FUN-CB0005
     DEFINE l_num             LIKE type_file.num10         #FUN-CB0005
...
#No.FUN-670066-Begin
#   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aimx503'    
...
#No.FUN-670066-End
     ...
     IF tm.a = '1' THEN
        LET l_sql = "SELECT '','', ",
                 " ima01, ima02,ima021, ima05, ima06,  ",   #FUN-CB0005 add ima021
...
#No.FUN-A20044 ---mark---end 
     ELSE
        #--->再補貨點
        LET l_sql = "SELECT '','', ",
                 " ima01, ima02,ima021, ima05, ima06, ",     #FUN-CB0005
		 "  ima08, ima07, ima37, ima25,",
...
#No.FUN-A20044 ---mark---end 
     ...
     END IF
     PREPARE x503_prepare1 FROM l_sql
     ...
     END IF
     DECLARE x503_curs1 CURSOR FOR x503_prepare1
 
#    CALL cl_outnam('aimx503') RETURNING l_name         #No.FUN-750097
#    START REPORT x503_rep TO l_name           #No.FUN-750097
     LET g_pageno = 0
#No.FUN-670066-Begin
#No.FUN-750097-Begin
    IF tm.c='Y' THEN
       LET l_name1 = 'aimx503_1'
    ELSE 
       LET l_name1 = 'aimx503'
    END IF
...
#No.FUN-750097-------------start------------------
   ...
   #    END IF
#No.FUN-670066-End
#      OUTPUT TO REPORT x503_rep(sr.*)
#FUN-CB0005---add--str--
       LET l_diff = 0
       LET l_diff1 = 0
       IF tm.c = 'Y' THEN 
          LET l_total = sr.avl_stk + sr.unavl_stk
       ELSE
          LET l_total = sr.avl_stk 
       END IF
       IF tm.a = '1' THEN 
       ...
       IF tm.a = '1' THEN 
          LET l_diff1 = 0 
       ELSE
          LET l_diff1 = l_total - sr.ima38
       END IF  
       LET l_num = 2
#FUN-CB0005---add--end--
       EXECUTE insert_prep USING
               sr.ima01,sr.ima02,sr.ima021,sr.ima05,sr.ima06,sr.ima08,sr.ima07,sr.ima37,        #FUN-CB0005
#              sr.ima25,sr.ima26,sr.ima261,sr.ima262,sr.ima38,sr.ima27,sr.ima99,                #FUN-A20044
               ...
               sr.ima09,sr.ima10,sr.ima11,sr.ima12,l_total,l_diff,l_diff1,l_num    #FUN-CB0005 add l_total,l_diff,l_diff1,l_num
       LET g_flag = 'Y'              #No.FUN-990081
     END FOREACH
 
#    FINISH REPORT x503_rep  
###XtraGrid###     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                       
     ...
     IF g_zz05 = 'Y' THEN                                                                                                            
        ...
        LET g_str = tm.wc                                                                                                            
     END IF                                                                                                                          
###XtraGrid###     LET g_str = g_str,";",tm.s[1,1],";",tm.s[2,2],";",tm.t,";",tm.c,";",tm.a,";",tm.y 
###XtraGrid###     CALL cl_prt_cs3('aimx503',l_name1,l_sql,g_str)
...
#FUN-CB0005--add--str--    
    ...
    IF tm.c='Y' THEN
       IF tm.a = '1' THEN 
          LET g_xgrid.template = 'aimx503_3'
       ELSE
          LET g_xgrid.template = 'aimx503_4'
       END IF  
    ELSE
       IF tm.a = '1' THEN 
          LET g_xgrid.template = 'aimx503'
       ELSE
          LET g_xgrid.template = 'aimx503_1'
       END IF  
    END IF
    CASE tm.y
       WHEN '0'
          LET g_xgrid.order_field = cl_get_order_field(tm.s,"ima01,ima06")
          LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"ima01,ima06")
         #LET l_str = cl_wcchp(g_xgrid.order_field,"ima01,ima06")  #FUN-D30070  mark
       WHEN '1'
          LET g_xgrid.order_field= cl_get_order_field(tm.s,"ima01,ima09")
          LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"ima01,ima09")
         #LET l_str = cl_wcchp(g_xgrid.order_field,"ima01,ima09")  #FUN-D30070  mark
       WHEN '2'
          LET g_xgrid.order_field= cl_get_order_field(tm.s,"ima01,ima10")
          LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"ima01,ima10")
         #LET l_str = cl_wcchp(g_xgrid.order_field,"ima01,ima10")  #FUN-D30070  mark
       WHEN '3'
          LET g_xgrid.order_field= cl_get_order_field(tm.s,"ima01,ima11")
          LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"ima01,ima11")
         #LET l_str = cl_wcchp(g_xgrid.order_field,"ima01,ima11")  #FUN-D30070  mark
       WHEN '4'
          LET g_xgrid.order_field= cl_get_order_field(tm.s,"ima01,ima12")
          LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"ima01,ima12")
         #LET l_str = cl_wcchp(g_xgrid.order_field,"ima01,ima12")  #FUN-D30070  mark
    END CASE
...
#      ima262          LIKE ima_file.ima262,     #FUN-A20044
   ...
   PREPARE x503_p1 FROM l_sql
   DECLARE x503_cs1 CURSOR FOR x503_p1
 
   BEGIN WORK
      LET g_success = 'Y'
      FOREACH x503_cs1 INTO sr.*
        IF SQLCA.sqlcode != 0 THEN
           CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
        END IF
        IF tm.c='Y' THEN  #含不可用數量
...
#          LET l_total=sr.ima262                     #FUN-A20044
        ...
        IF l_diff<0 THEN
           IF l_count=0 THEN
              OPEN WINDOW x5031 WITH FORM "aim/42f/aimx5031"
                  ATTRIBUTE (STYLE = g_win_style CLIPPED)
              CALL cl_ui_locale("aimx5031")
