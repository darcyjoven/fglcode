...
#No.FUN-A40023 -----------start-----------------
    ...
    LET l_table = cl_prt_temptable('aimr560',g_sql) CLIPPED   # 產生Temp Table
...
#No.FUN-A40023---------------end------------
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL r560_tm()	        	# Input print condition
      ELSE CALL aimr560()			# Read data and create out-file
   END IF
...
#TQC-A70003 ---start
   CONSTRUCT BY NAME tm.wc1 ON azp01
         ...
         AFTER FIELD azp01 
            ...
            END IF 
            IF g_chk_azp01 THEN
               DECLARE r560_zxy_cs1 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH r560_zxy_cs1 INTO l_zxy03 
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimr560',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW r560_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
#MOD-B90166 -- end --
     ...
     PREPARE r560_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
        EXIT PROGRAM
     END IF
     DECLARE r560_curs1 CURSOR FOR r560_prepare1
#No.FUN-A40023 ---start---
     FOREACH r560_curs1 INTO sr.*
        IF SQLCA.sqlcode != 0 THEN
...
#TQC-A70003 add ---------------------------------start----------------------------------------
        ...
        IF sr.rty13 IS NULL THEN
          ...
          EXECUTE insert_prep USING sr.ima01,sr.ima02,sr.ima021,sr.ima25,sr.avl_stk_mpsmrp,
                                      sr.ima27,sr.ima271,sr.ima902,sr.imgplant
        ELSE
          EXECUTE insert_prep USING sr.ima01,sr.ima02,sr.ima021,sr.ima25,sr.avl_stk_mpsmrp,
                                      sr.ima27,sr.rty13,sr.ima902,sr.imgplant
        END IF
#TQC-A70003 add  -----------------------------------end----------------------------------------
     END FOREACH 
END FOREACH
...
#No FUN-750093 --start--
#     CALL cl_outnam('aimr560') RETURNING l_name 
#     START REPORT r560_rep TO l_name
...
#          CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
#       END IF
#       OUTPUT TO REPORT r560_rep(sr.*)  #FUN-750093
#     END FOREACH
#     FINISH REPORT r560_rep   #FUN-750093
#No FUN-750093 --end--
...
#    CALL cl_prt_cs1('aimr560','aimr560',l_sql,g_str)   #FUN-A40023
     CALL cl_prt_cs3('aimr560','aimr560',g_sql,g_str)   #FUN-A40023
END FUNCTION
#FUN-750093 --start--
{
REPORT r560_rep(sr)
