...
# Modify.........: No:TQC-AC0218 10/12/20 By jan 背景執行時,重撈g_imm.*的值
# Modify.........: No:FUN-AC0074 11/05/09 By jan 取消確認時增加備置處理
...
# Modify.........: No:FUN-BC0062 13/03/01 By lixh1 當成本計算方式ccz28選擇'6'，過帳還原時控制不可過帳還原
# Modify.........: No:FUN-D30024 13/03/12 By qiull 負庫存依據imd23判斷
# Modify.........: No:FUN-D50079 13/05/22 By fengmy 增加過帳日欄位,FUN-D40053補充
...
#        CALL cl_err('sel imm:',STATUS,0) NEXT FIELD imm01 END IF
        ...
        END IF
        #No:9377(end)
       #IF g_sma.sma53 IS NOT NULL AND g_imm.imm02 <= g_sma.sma53 THEN   #FUN-D50079 mark 
        IF g_sma.sma53 IS NOT NULL AND g_imm.imm17 <= g_sma.sma53 THEN   #FUN-D50079
...
#   IF cl_sure(0,0) THEN 
#      CALL p378_p2() 
#      CALL cl_end(0,0)    #010409 by plum
#      IF g_success='Y' THEN
#         COMMIT WORK
#         CALL cl_end2(1) RETURNING l_flag        #批次作業正確結束
#      ELSE
