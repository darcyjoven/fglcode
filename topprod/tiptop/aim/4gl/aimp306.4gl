...
#
# Pattern name...: aimp306.4gl
# Descriptions...: 同業間借料結案作業
# Input parameter: 
# Return code....: 
# Date & Author..: 03/02/26 By Mandy
# Modify.........: No.MOD-490371 04/09/22 By Yuna Controlp 未加display
# Modify.........: No.MOD-4A0049 04/10/07 By Mandy 底下會show 一行按 [return]鍵執行或取消==>請拿掉該行說明
...
DEFINE
    tm         RECORD
               ...
               a        LIKE type_file.chr1,    #是否整張借料單全部設定結案  #No.FUN-690026 VARCHAR(1)
               y        LIKE type_file.chr1     #已結案資料是否顯示  #No.FUN-690026 VARCHAR(1)
               END RECORD,
...
FUNCTION p306_tm(p_row,p_col)
   ...
   IF p_row = 0 THEN LET p_row = 3 LET p_col = 2 END IF
 
 
   OPEN WINDOW p306_w AT p_row,p_col
        WITH FORM "aim/42f/aimp306"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
         ...
         AFTER FIELD imo01            #請購單號
            ...
            ELSE 
               ...
               IF NOT cl_null(g_errno) THEN
                  ...
                  NEXT FIELD imo01
               END IF
               #檢查此借料單是否有未過帳(只抓未過帳的,作廢的不用管)還料單
               #檢查aimt308,aimt309的單頭和單身),若有,
               #出訊息『此借料單尚有未過帳的還料資料!』
               #只出訊息提醒,不需要拒絕
               SELECT COUNT(*) INTO g_cnt 
                 FROM imr_file,imq_file
                WHERE imr01   = imq01
                  AND (imr05  = tm.imo01 OR imq03 = tm.imo01)
                  AND imrpost = 'N'
              IF g_cnt > 0 THEN
                  #此借料單尚有未過帳的還料資料! 
                  CALL cl_err(tm.imo01,'aim-112',0)
              END IF
            END IF
...
#                 CALL q_imo(7,3,tm.imo01) RETURNING tm.imo01
...
FUNCTION p306_imo01()
   ...
   CASE 
      ...
      WHEN l_imoconf = 'X'     LET g_errno = 'aim-111' #借料單已作廢不可結案!
...
#  l_time        LIKE type_file.chr8            #No.FUN-6A0074
   ...
   LET l_sql = " SELECT 'N',imp02,imp03,imp07,(imp04-imp08)",
               "   FROM imp_file",
               "  WHERE imp01 = '",tm.imo01,"'"
               
   IF tm.y = 'N' THEN #已結案資料不顯示
       LET l_sql = l_sql CLIPPED,
                  "  AND imp07 != 'Y' ",
                  "  ORDER BY imp02 "
   END IF
   PREPARE p306_prepare FROM l_sql
   DECLARE p306_cur CURSOR FOR p306_prepare
   LET l_ac = 1
#  LET g_arrcnt = 200
   LET l_exit_sw = 'n'
   CALL s_showmsg_init()     #No.FUN-710025	
   FOREACH p306_cur INTO g_imp[l_ac].sure,
                         g_imp[l_ac].imp02,
                         g_imp[l_ac].imp03,
                         l_imp07,
                         g_imp[l_ac].qty
      ...
      #No.FUN-710025--Begin--
      #   CALL cl_err('foreach p306_cur',SQLCA.sqlcode,1)
          CALL s_errmsg('','','foreach p306_cur',SQLCA.sqlcode,1)
          LET g_success = 'N'
          EXIT FOREACH
      #No.FUN-710025--End--
      END IF
   ...
   WHILE TRUE
       ...
       #No.FUN-710025--Begin--
       #   CALL cl_err(tm.imo01,'mfg9171',1) 
           CALL s_errmsg('','','','mfg9171',1)
           LET l_exit_sw = 'y'
           EXIT WHILE
       #No.FUN-710025--End--
       END IF
#      IF l_ac > g_arrcnt THEN 
#          IF NOT cl_arful(0,0,g_arrcnt) THEN EXIT WHILE END IF
#      END IF
       CALL SET_COUNT(l_ac - 1)
       IF tm.a = 'N' THEN   #是否整張借料單全部設定結案
           CALL  p306_sure()         #確定否
...
#                                        g_imp[l_no].imp02,SQLCA.sqlcode,"",
#                                        "upd imp07",1)  #No.FUN-660156
...
#No.FUN-710025--End--
                   ...
                   END IF
               END FOR
               CALL p306_up_imo07() #update 單頭結案碼
           END IF
       ELSE #整張借料單全部設定結案
           UPDATE imp_file 
              SET imp07 = 'Y'
...
#No.FUN-710025--Begin--
#              CALL cl_err3("upd","imp_file",tm.imo01,"",SQLCA.sqlcode,"",
#                           "upd imp07",1)  #No.FUN-660156
...
FUNCTION p306_up_imo07() #update 單頭結案碼
...
#No.FUN-710025--Begin--
#      CALL cl_err3("upd","imo_file",tm.imo01,"",SQLCA.sqlcode,"",
#                   "upd imo07",1)  #No.FUN-660156
