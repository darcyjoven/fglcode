...
# Modify.........: No.MOD-A60093 10/06/14 By Sarah r521_2()段,imf_file是OUTER table,應將imf04條件移到FOREACH裡再做判斷
# Modify.........: No.FUN-C30190 12/03/29 By xumeimei 原报表改为CR报表
# Modify.........: No.TQC-CC0085 12/12/17 By qirl 增加開窗
# Modify.........: No:TQC-CC0086 12/12/17 By qirl 增加欄位
   ...
   IF g_sma.sma12 = 'N' THEN
      LET g_sql = "order1.ima_file.ima01,",
             ...
             #----TQC-CC0086--add--end--
                  ...
                  "diff.img_file.img10,",
                  "sta.type_file.chr1"
      LET l_table = cl_prt_temptable('aimr521',g_sql) CLIPPED
      IF l_table = -1 THEN EXIT PROGRAM END IF
      LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
                  " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?      ,?,?,?)"     #---TQC-CC0086--add--3?
   ...
   ELSE
      LET g_sql = "order1.ima_file.ima01,",
             ...
             #----TQC-CC0086--add--end--
                  ...
                  "diff.img_file.img10,",
                  "sta.type_file.chr1"
      LET l_table1 = cl_prt_temptable('aimr521',g_sql) CLIPPED
...
#FUN-C30190 --------------END 
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      THEN CALL r521_tm(0,0)		    # Input print condition
   ELSE
...
#---TQC-CC0085--add-end---
 ...
 END CONSTRUCT
   ...
   END IF
   IF INT_FLAG THEN LET INT_FLAG = 0 CLOSE WINDOW r521_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM 
   END IF
   IF tm.wc = " 1=1" THEN
...
################################################################################
   ...
   END INPUT
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW r521_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimr521',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW r521_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
#		     diff     LIKE  ima_file.ima261,#差異    #FUN-A20044
     ...
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aimr521'
    #LET g_len = 85    #TQC-5B0019
     ...
     LET l_sql = " SELECT UNIQUE ' ',' ',' ',ima01,ima02,ima021,'','',ima05,ima06,",   #-TQC-CC0086
...
#No.FUN-A20044 ---mark---end 
     ...
     IF SQLCA.sqlcode != 0 THEN
         ...
         EXIT PROGRAM 
         END IF
     DECLARE r521_c1 CURSOR FOR r521_prepare1
 
     #CALL cl_outnam('aimr521') RETURNING l_name              #FUN-C30190 mark
     #START REPORT r521_rep1 TO l_name                        #FUN-C30190 mark
#FUN-680035--begin
     LET g_pageno = 0
...
#No.FUN-A20044 ----end---       
       ...
       ELSE
	      ...
	      END IF
       END IF
       #OUTPUT TO REPORT r521_rep1(sr.*)              #FUN-C30190 mark
       EXECUTE insert_prep USING sr.*                    #FUN-C30190 add
     END FOREACH
 
     #FINISH REPORT r521_rep1                         #FUN-C30190 mark
  
     #CALL cl_prt(l_name,g_prtway,g_copies,g_len)     #FUN-C30190 mark
     #FUN-C30190-----add---str----
     LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED," ORDER BY order1,order2,order3,diff"
     CALL cl_wcchp(tm.wc,'ima01')
          RETURNING tm.wc
     LET g_str = tm.wc
     CALL cl_prt_cs3("aimr521","aimr521_1",l_sql,g_str)
     #FUN-C30190-----add---end----
END FUNCTION
...
# 
#   ON EVERY ROW
#	  CALL r521_sta(sr.sta) RETURNING l_sta
##FUN-680025--begin
##    PRINT sr.ima01,      
...
#       l_time          LIKE type_file.chr8         #No.FUN-6A0074
     ...
     SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
     DECLARE r521_zc2 CURSOR FOR
                  SELECT za02,za05
                    FROM za_file
                    WHERE za01 = "aimr521" AND za03 = g_lang
     FOREACH r521_zc2 INTO g_i,l_za05
        LET g_x[g_i] = l_za05
     END FOREACH
     ...
     LET l_sql = " SELECT UNIQUE '','','',img01,img02,img03,",
                #"               img04,img09,img10,imf04,ima02,ima021,'','',",   #MOD-A60093 mark   #----TQC-CC0086--add
                 "               img04,img09,img10,0,ima02,ima021,'','',",       #MOD-A60093
                 "               ima05,ima06,ima09,ima10,ima11,ima12, ",
                #"               ima07,ima08,(img10-imf04),''",     #MOD-A60093 mark
                 "               ima07,ima08,0,''",                 #MOD-A60093
    ...
    #END IF
    #end MOD-A60093 mark
     PREPARE r521_prepare2 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
        EXIT PROGRAM
     END IF
     DECLARE r521_c2 CURSOR FOR r521_prepare2
 
     #CALL cl_outnam('aimr521') RETURNING l_name                #FUN-C30190 mark
     #START REPORT r521_rep2 TO l_name                          #FUN-C30190 mark
...
#----TQC-CC0086---add---end---
       ...
       #str MOD-A60093 add
        ...
        LET sr.order2 = l_order[2]
        LET sr.order3 = l_order[3]
        #OUTPUT TO REPORT r521_rep2(sr.*)              #FUN-C30190 mark
        EXECUTE insert_prep1 USING sr.*              #FUN-C30190 add
     END FOREACH
 
     #FINISH REPORT r521_rep2                          #FUN-C30190 mark
     #CALL cl_prt(l_name,g_prtway,g_copies,g_len)      #FUN-C30190 mark
     #FUN-C30190-----add---str----
     ...
     CALL cl_wcchp(tm.wc,'ima01')
          RETURNING tm.wc
     LET g_str = tm.wc
     CALL cl_prt_cs3("aimr521","aimr521_2",l_sql,g_str)
     #FUN-C30190-----add---end----
END FUNCTION
