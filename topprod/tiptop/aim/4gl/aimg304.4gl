...
# Modify.........: No.FUN-B40097 11/05/17 By chenying 憑證類CR報表轉成GR
# Modify.........: No.FUN-C20052 12/02/10 By chenying GR調整
...
#               "l_str2.type_file.chr1000"     #No.FUN-860026    
   ...
   LET l_table = cl_prt_temptable('aimg304',g_sql) CLIPPED
   IF l_table = -1 THEN 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1)     #FUN-B40097
      EXIT PROGRAM 
   END IF
#No.FUN-860026---begin                                                                                                              
   ...
   LET l_table1 = cl_prt_temptable('aimg3041',g_sql) CLIPPED                                                                        
   IF  l_table1 = -1 THEN 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time    
       CALL cl_gre_drop_temptable(l_table||"|"||l_table1)     #FUN-B40097
       EXIT PROGRAM 
...
#FUN-710084.....end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'	# If background job sw is off
      ...
      ELSE CALL aimg304()		# Read data and create out-file
   END IF
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimg304',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW r304_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1) 
    ...
    #LET l_sql = "SELECT imm02,imm01,imn02,imn03,ima02,ima05,ima08, ",          #FUN-5C0002 mark
     LET l_sql = "SELECT imm02,imm01,imn02,imn03,ima02,ima021,ima05,ima08, ",   #FUN-5C0002
               ...
               # " FROM imn_file,imm_file,OUTER ima_file ", 
                 " FROM imm_file,imn_file LEFT OUTER JOIN ima_file ON imn03=ima01 ",  
                 " WHERE imm01 = imn01 ",
...
#FUN-710084.....begin mark
  #   CALL cl_outnam('aimg304') RETURNING l_name
  #   #No.FUN-580005 --start--
...
#No.FUN-860026---begin                         
   ...
   #FOREACH  r920_d INTO l_rvbs.*
    #FUN-C50007--MARK--end      
    FOREACH  r920_d  USING sr.imm01,sr.imn02 INTO l_rvbs.*  #FUN-C50007--add                                                                                             
         LET flag = 1
...
#       EXECUTE insert_prep USING sr.*,l_str1,l_str2
       EXECUTE insert_prep USING sr.*,l_str1,l_str2,flag, 
...
###GENGRE###                  "SELECT * FROM ",g_cr_db_str CLIPPED, l_table1 CLIPPED    #No.FUN-860026  
      SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog
           ...
           RETURNING tm.wc
###GENGRE###      LET g_str = tm.wc,";",g_zz05,";",tm.a   #No.FUN-860026
    # CALL cl_prt_cs3('aimg304',l_sql,g_str)   #TQC-730088
###GENGRE###      CALL cl_prt_cs3('aimg304','aimg304',l_sql,g_str)
    LET g_cr_table = l_table                   #主報表的temp table名稱          #FUN-C40019 add
    LET g_cr_apr_key_f = "imm01"               #報表主鍵欄位名稱，用"|"隔開     #FUN-C40019 add
    CALL aimg304_grdata()    ###GENGRE###
#FUN-710084.....end
END FUNCTION
...
FUNCTION aimg304_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aimg304")
        IF handler IS NOT NULL THEN
            START REPORT aimg304_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY imm01,imn02"
            DECLARE aimg304_datacur1 CURSOR FROM l_sql
            FOREACH aimg304_datacur1 INTO sr1.*
                OUTPUT TO REPORT aimg304_rep(sr1.*)
            END FOREACH
            FINISH REPORT aimg304_rep
        END IF
...
REPORT aimg304_rep(sr1)
    ...
    ORDER EXTERNAL BY sr1.imm01,sr1.imn02
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.imm01
            LET l_lineno = 0
        BEFORE GROUP OF sr1.imn02
        ...
        AFTER GROUP OF sr1.imm01
           ...
           LET l_sql=" SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                    #" WHERE rvbs01 = '",sr1.imm01 CLIPPED,"' AND rvbs09 != -1 ",  #FUN-C20052 mark
                     " WHERE rvbs01 = '",sr1.imm01 CLIPPED,"' AND rvbs09 = -1 ",   #FUN-C20052 add
                     " ORDER BY rvbs02"
           START REPORT aimg304_subrep01
           DECLARE aimg304_repcur1 CURSOR FROM l_sql
           FOREACH aimg304_repcur1 INTO sr2.*
               OUTPUT TO REPORT aimg304_subrep01(sr2.*)
           END FOREACH
           FINISH REPORT aimg304_subrep01
           #FUN-B40097----add-----end-------------------
       
        AFTER GROUP OF sr1.imn02
...
FORMAT
    ...
    AFTER GROUP OF sr2.rvbs02
       LET l_sum_rvbs06 = GROUP SUM(sr2.rvbs06)
       PRINTX l_sum_rvbs06
END REPORT
#FUN-B40097---add---end------------
