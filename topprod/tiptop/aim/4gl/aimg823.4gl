...
# Modify.........: No.FUN-D30025 13/03/12 By yangtt CR轉GRW                        
...
###GENGRE###START
TYPE sr1_t RECORD
    order1 LIKE pia_file.pia02,  #FUN-D30025
    order2 LIKE pia_file.pia02,  #FUN-D30025
    order3 LIKE pia_file.pia02,  #FUN-D30025
...
TYPE sr3_t RECORD
    ...
    l_count1 LIKE pias_file.pias09
END RECORD
...
TYPE sr4_t RECORD
    ...
    l_count1 LIKE piad_file.piad09
END RECORD
###GENGRE###END
...
#No.FUN-7C0007---Begin                                                          
   LET g_sql = 
               "order1.pia_file.pia02,",  #FUN-D30025
               "order2.pia_file.pia02,",  #FUN-D30025
               "order3.pia_file.pia02,",  #FUN-D30025
   ...
   LET l_table = cl_prt_temptable('aimg823',g_sql) CLIPPED                      
   ...
   LET g_sql = "pia01.pia_file.pia01,", 
              ...
              #CHI-B50008 mod --end--
   LET l_table1 = cl_prt_temptable('aimg8231',g_sql) CLIPPED                      
...
#No.FUN-7C0007---End
   ...
   LET l_table2 = cl_prt_temptable('aimg8232',g_sql) CLIPPED                      
   ...
   #LET l_table3 = cl_prt_temptable('aimg8232',g_sql) CLIPPED      #TQC-C40207 mark                
   LET l_table3 = cl_prt_temptable('aimg8233',g_sql) CLIPPED       #TQC-C40207 add                
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' # If background job sw is off
      THEN CALL g823_tm(0,0)         # Input print condition
      ELSE CALL g823()               # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
END MAIN
 
FUNCTION g823_tm(p_row,p_col)
   ...
   OPEN WINDOW g823_w AT p_row,p_col
        WITH FORM "aim/42f/aimg823" 
################################################################################
...
#No.FUN-570240 --end  
 ...
 LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW g823_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimg823',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW g823_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
#                         " AND (pia30 IS NOT NULL OR pia50 IS NOT NULL) "
           ...
           END CASE
     END IF
     PREPARE g823_prepare1 FROM l_sql
     ...
     END IF
     DECLARE g823_curs1 CURSOR FOR g823_prepare1
 
#     LET l_name = 'aimg823.out'
#     CALL cl_outnam('aimg823') RETURNING l_name
    #START REPORT g823_rep TO l_name   #TQC-740117 
...
#     LET g_pageno = 0
     IF g_sma.sma115 = "Y" THEN
         LET l_name = 'aimg823'
     ELSE
         LET l_name = 'aimg823_1'
     END IF
     #LET l_name = 'aimg823'     ##MOD-860014 mark
#No.FUN-7C0007---End 
 
     FOREACH g823_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN 
...
#       LET sr.order3 = l_order[3]
      IF g_sma.sma115='Y' THEN
            ...
            PREPARE g823_piaa_p FROM l_sql
            IF SQLCA.sqlcode != 0 THEN 
                CALL cl_err('prepare:',SQLCA.sqlcode,1) 
                CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
                EXIT PROGRAM 
            ...
            END IF
            DECLARE g823_piaa_c CURSOR FOR g823_piaa_p
            FOREACH g823_piaa_c INTO l_piaa09,l_piaa30,l_piaa40,l_piaa50,l_piaa60 
            IF SQLCA.sqlcode != 0 THEN 
                CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH 
            END IF
            LET l_cnt = l_cnt + 1
      ...
      IF tm.lot='Y' THEN
            ...
            PREPARE g823_pias_p FROM l_sql
            IF SQLCA.sqlcode != 0 THEN 
                CALL cl_err('prepare:',SQLCA.sqlcode,1) 
                CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
                EXIT PROGRAM 
            ...
            END IF
            DECLARE g823_pias_c CURSOR FOR g823_pias_p
            FOREACH g823_pias_c INTO l_pias06,l_pias07,
      ...
      IF s_industry('icd') THEN
         IF tm.dot='Y' THEN
            ...
            PREPARE g823_piad_p FROM l_sql
            IF SQLCA.sqlcode != 0 THEN 
                CALL cl_err('prepare:',SQLCA.sqlcode,1) 
                CALL cl_used(g_prog,g_time,2) RETURNING g_time 
                EXIT PROGRAM
            END IF
            DECLARE g823_piad_c CURSOR FOR g823_piad_p
            FOREACH g823_piad_c INTO l_piad06,l_piad07,
      ...
      END FOR
      #FUN-D30025----add---end---
      EXECUTE insert_prep USING l_order[1],l_order[2],l_order[3],     #FUN-D30025 add l_order[1],l_order[2],l_order[3]
                                sr.pia01,sr.pia02,sr.pia03,sr.pia04,sr.pia05,sr.pia09,sr.pia10,sr.count,
                                l_piaa09,l_count1,sr.ima02,sr.ima021,sr.ima25,sr.flag
                 
#       OUTPUT TO REPORT g823_rep(sr.*)
#No.FUN-7C0007---End
     END FOREACH
#No.FUN-7C0007---Begin
#     FINISH REPORT g823_rep
      IF g_zz05 = 'Y' THEN                                                      
         CALL cl_wcchp(tm.wc,'pia02,pia03,pia04,pia05,pia01,pia931')                 #FUN-930122 Add pia931                        
              RETURNING tm.wc                                                   
      END IF                                                                    
###GENGRE###      LET g_str=tm.wc,";",tm.data,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",
###GENGRE###                      tm.t[1,1],";",tm.t[2,2],";",tm.t[3,3],";",
###GENGRE###                      tm.tot,";",g_q_point,";",l_cnt
                                                                          
###GENGRE###   LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED, "|",
###GENGRE###               "SELECT * FROM ", g_cr_db_str CLIPPED, l_table1 CLIPPED,"|",  
###GENGRE###               "SELECT * FROM ", g_cr_db_str CLIPPED, l_table2 CLIPPED,"|",  #FUN-860001
###GENGRE###               "SELECT * FROM ", g_cr_db_str CLIPPED, l_table3 CLIPPED       #FUN-B70032
   
###GENGRE###   CALL cl_prt_cs3('aimg823',l_name,l_sql,g_str) 
    LET g_template = l_name  #FUN-D30025
    CALL aimg823_grdata()    ###GENGRE###
...
FUNCTION aimg823_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aimg823")
        IF handler IS NOT NULL THEN
            START REPORT aimg823_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY lower(order1),order2,order3,pia02"   #FUN-D30025
          
            DECLARE aimg823_datacur1 CURSOR FROM l_sql
            FOREACH aimg823_datacur1 INTO sr1.*
                OUTPUT TO REPORT aimg823_rep(sr1.*)
            END FOREACH
            FINISH REPORT aimg823_rep
        END IF
...
REPORT aimg823_rep(sr1)
   ...
   #DEFINE l_flag   LIKE zaa_file.zaa08  #FUN-D30025 
    DEFINE l_flag   STRING               #FUN-D30025 
    DEFINE l_count_pia10   LIKE pia_file.pia10 #FUN-D30025 
    DEFINE l_order1 STRING   #FUN-D30025
    DEFINE l_order2 STRING   #FUN-D30025
    DEFINE l_order3 STRING   #FUN-D30025
    DEFINE l_skip1  STRING   #FUN-D30025
    DEFINE l_skip2  STRING   #FUN-D30025
    DEFINE l_skip3  STRING   #FUN-D30025
    DEFINE l_n1     LIKE type_file.num5  #FUN-D30025
    DEFINE l_n2     LIKE type_file.num5  #FUN-D30025

    ORDER EXTERNAL By sr1.order1,sr1.order2,sr1.order3,sr1.pia02     #FUN-D30025 add
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
            #FUN-D30025----add---str---
            LET l_order1 = cl_gr_getmsg("gre-340",g_lang,tm.s[1,1])
            LET l_order2 = cl_gr_getmsg("gre-340",g_lang,tm.s[2,2])
            LET l_order2 = cl_gr_getmsg("gre-340",g_lang,tm.s[3,3])
            PRINTX l_order1,l_order2,l_order3
        ...
        ON EVERY ROW
            ...
            #FUN-D30025----add---str---
            IF sr1.flag = 'N' THEN
               LET l_flag = g_q_point 
            ELSE
               LET l_flag = sr1.count USING '--,---,---.&&&'
            END IF 
            LET l_flag = l_flag.trim()
            PRINTX l_flag
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE pia01 = '",sr1.pia01 CLIPPED,"'",
                        "  ORDER BY pia01 "
            START REPORT aimg823_subrep01
            DECLARE aimg823_repcur1 CURSOR FROM l_sql
            FOREACH aimg823_repcur1 INTO sr2.*
                OUTPUT TO REPORT aimg823_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT aimg823_subrep01

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        " WHERE piad01 = '",sr1.pia01 CLIPPED,"'",
                        "  ORDER BY piad06 "
            START REPORT aimg823_subrep03
            DECLARE aimg823_repcur3 CURSOR FROM l_sql
            FOREACH aimg823_repcur3 INTO sr4.*
                OUTPUT TO REPORT aimg823_subrep03(sr4.*)
            END FOREACH
            FINISH REPORT aimg823_subrep03

            LET l_sql = "SELECT COUNT(*) FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        " WHERE piad01 = '",sr1.pia01 CLIPPED,"'"
            DECLARE aimg823_repcur3_1 CURSOR FROM l_sql
            FOREACH aimg823_repcur3_1 INTO l_n1
            END FOREACH
 
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,
                        " WHERE pias01 = '",sr1.pia01 CLIPPED,"'",
                        "  ORDER BY pias06 "
            START REPORT aimg823_subrep02
            DECLARE aimg823_repcur2 CURSOR FROM l_sql
            FOREACH aimg823_repcur2 INTO sr3.*
                OUTPUT TO REPORT aimg823_subrep02(sr3.*)
            END FOREACH
            FINISH REPORT aimg823_subrep02

            LET l_sql = "SELECT COUNT(*) FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,
                        " WHERE pias01 = '",sr1.pia01 CLIPPED,"'"
            DECLARE aimg823_repcur2_1 CURSOR FROM l_sql
            FOREACH aimg823_repcur2_1 INTO l_n2
            END FOREACH
        ...
        AFTER GROUP OF sr1.pia02  #FUN-D30025
            #FUN-D30025----add---str---
            LET l_count_pia10 = GROUP SUM(sr1.count * sr1.pia10)
            PRINTX l_count_pia10 
            #FUN-D30025----add---end---
