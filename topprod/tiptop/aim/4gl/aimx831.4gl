...
# Modify.........: No.FUN-CB0001 12/11/05 By chenying CR轉XtraGrid
...
# Modify.........: No:FUN-D40129 13/05/08 By yangtt pia931新增開窗 
...
#              "l_diff.ima_file.ima26,",    #FUN-A20044
            ...
            #FUN-CB0001--add--end--
   LET l_table = cl_prt_temptable('aimx831',g_sql) CLIPPED
   
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,?)"   #FUN-CB0001 add 3?
   ...
   #No.FUN-570264 ---end---
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN CALL x831_tm(0,0)    	
   ELSE
#No.FUN-590110 --start--
       CALL x831()
#No.FUN-590110 --end--
...
DEFINE p_row,p_col	LIKE type_file.num5,    #No.FUN-690026 SMALLINT
   ...
   OPEN WINDOW x831_w AT p_row,p_col
        WITH FORM "aim/42f/aimx831"
################################################################################
...
#No.FUN-570240 --start
      ...
      ON ACTION controlp
    #       IF INFIELD(pia02) THEN      #FUN-CB0001 mark
         CASE     #FUN-CB0001 
            WHEN INFIELD(pia02) #FUN-CB0001
         ...
         #FUN-CB0001-----add---str--
            ...
            WHEN INFIELD(pia51)
               ...
               LET g_qryparam.state = 'c'
               CALL cl_create_qry() RETURNING g_qryparam.multiret
               DISPLAY g_qryparam.multiret TO pia51
               NEXT FIELD pia51
            WHEN INFIELD(pia61)
               CALL cl_init_qry_var()
               LET g_qryparam.form = "q_gen02"
               LET g_qryparam.state = 'c'
               CALL cl_create_qry() RETURNING g_qryparam.multiret
               DISPLAY g_qryparam.multiret TO pia61
               NEXT FIELD pia61
           #FUN-D40129----add---str--
            WHEN INFIELD(pia931)
               CALL cl_init_qry_var()
               LET g_qryparam.form = "q_pia931_1"
               LET g_qryparam.state = 'c'
               CALL cl_create_qry() RETURNING g_qryparam.multiret
...
#No.FUN-570240 --end
 ...
 LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW x831_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimx831',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW x831_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8         #No.FUN-6A0074
     ...
     DEFINE l_print     STRING  #FUN-CB0001 
     DEFINE l_type      STRING  #FUN-CB0001 
     DEFINE l_num       LIKE type_file.num5 #FUN-CB0001
     DEFINE l_imd02    LIKE imd_file.imd02  #FUN-CB0001
     DEFINE l_ime03    LIKE ime_file.ime03  #FUN-CB0001
                     
     CALL cl_del_data(l_table)    #No.FUN-840055
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aimx831'   #No.FUN-840055
     SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
     DECLARE x831_za1 CURSOR FOR
             SELECT za02,za05 FROM za_file
              WHERE za01 = "aimx831" AND za03 = g_lang
     FOREACH x831_za1 INTO g_i,l_za05 LET g_x[g_i] = l_za05 END FOREACH
#No.FUN-590110 --start--
...
#No.FUN-590110 --end--
     ...
     PREPARE x831_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
        EXIT PROGRAM
     END IF
     DECLARE x831_c1 CURSOR FOR x831_prepare1
     #CALL cl_outnam('aimx831') RETURNING l_name   #No.FUN-840055
#No.FUN-840055----START--     
#No.FUN-590110 --start--
     ...
     IF g_sma.sma12='N' THEN
        IF tm.diff = 'N' THEN
           LET l_name = 'aimx831_3'
        ELSE
           LET l_name = 'aimx831_2'
        END IF
     ELSE
        IF tm.diff = 'N' THEN
           LET l_name = 'aimx831_1'
        ELSE
           LET l_name = 'aimx831'
        END IF
     END IF     
...
#No.FUN-590110 --end--
     ...
     #START REPORT x831_rep TO l_name
#No.FUN-840055----end
 
     LET g_pageno = 0
     FOREACH x831_c1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
    ...
    #   OUTPUT TO REPORT x831_rep(sr.*)
       #---->多倉差異量
      LET l_diff = sr.count - sr.pia08
      LET l_num = 3  #FUN-CB0001
       SELECT imd02 INTO l_imd02 FROM imd_file WHERE imd01 = sr.pia03   #FUN-CB0001
      #SELECT ime03 INTO l_ime03 FROM ime_file WHERE ime01 = sr.pia04   #FUN-CB0001 #FUN-D40128 mark
      ...
      EXECUTE insert_prep USING
            ...
            sr.ima021,sr.pia51,sr.pia61,l_num,l_imd02,l_ime03  #FUN-CB0001 
    #No.FUN-840055-----end   
     END FOREACH
...
###XtraGrid###                 tm.t[1,1],";",tm.t[2,2],";",tm.t[3,3],";",tm.diff,";",
###XtraGrid###                 tm.count,";",tm.type
                 
###XtraGrid###     CALL cl_prt_cs3('aimx831',l_name,g_sql,g_str)
    ...
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"pia01,pia03,pia04,pia05,pia02,ima08,pia51,pia61")
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"pia01,pia03,pia04,pia05,pia02,ima08,pia51,pia61") #FUN-D30053
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"pia01,pia03,pia04,pia05,pia02,ima08,pia51,pia61")
    IF tm.diff = 'Y' THEN
       IF tm.count = 'Y' THEN
          LET l_print = cl_getmsg('aim-053',g_lang)
       ELSE
          LET l_print = cl_getmsg('aim-054',g_lang)
       END IF
    ELSE
       IF tm.count = 'Y' THEN
          LET l_print = cl_getmsg('aim-055',g_lang)
       ELSE
          LET l_print = " " 
       END IF
    END IF   
    IF tm.type = '1' THEN
       LET l_type = cl_getmsg('aim-056',g_lang)
    ELSE
       LET l_type = cl_getmsg('aim-057',g_lang)
    END IF
    LET g_xgrid.footerinfo1 = cl_getmsg('aim-051',g_lang),':',l_print,'|',cl_getmsg('aim-052',g_lang),':',l_type
   #LET l_str = cl_wcchp(g_xgrid.order_field,"pia01,pia03,pia04,pia05,pia02,ima08,pia51,pia61") #FUN-D30070 mark
   #LET l_str = cl_replace_str(l_str,',','-') #FUN-D30070 mark 
   #LET g_xgrid.footerinfo2 = cl_getmsg('aim-058',g_lang),l_str #FUN-D30070 mark
    LET g_xgrid.condition = cl_getmsg('lib-160',g_lang),tm.wc
    #FUN-CB0001-----add---end--
    CALL cl_xg_view()    ###XtraGrid###
     #FINISH REPORT x831_rep   
