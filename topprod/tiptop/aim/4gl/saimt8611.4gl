...
#
# Pattern name...: aimt8611.4gl
# Descriptions...: 複盤維護作業－在製工單
...
# Modify.........: No.CHI-C80012 12/08/06 By Elise pie16欄位改為checkbox，可CONSTRUCT，不可修改 
# Modify.........: No.CHI-C80028 12/08/23 By Elise 開放查詢:作業編號、部門廠商
...
DEFINE
    ...
    g_pie_t         RECORD                 #程式變數 (舊值)
        ...
        azf03       LIKE azf_file.azf03    #理由碼名稱  #FUN-CB0087
                    END RECORD,
    g_tagdate,g_tagdate2 LIKE pie_file.pie35,
...
FUNCTION aimt8611(p_argv1)
    ...
    OPEN WINDOW t8611_w WITH FORM "aim/42f/aimt8611"
...
DEFINE  lc_qbe_sn       LIKE    gbm_file.gbm01    #No.FUN-580031  HCN
   ...
   CALL g_pie.clear()
    ...
    CONSTRUCT BY NAME g_wc ON                     # 螢幕上取單頭條件
                        pid01,pid02,pid03
                       ,pid012,pid021,pid022,pid023     #FUN-A60092 add  #CHI-C80028 add pid022,pid023
...
#FUN-A60092 --end--
    ...
    CONSTRUCT g_wc2 ON pie07,pie02,pie04,pie16                 #CHI-C80012 add pie16
            FROM s_pie[1].pie07,
                 s_pie[1].pie02,s_pie[1].pie04,s_pie[1].pie16  #CHI-C80012 add pie16
		#No.FUN-580031 --start--     HCN
		BEFORE CONSTRUCT
		   CALL cl_qbe_display_condition(lc_qbe_sn)
		#No.FUN-580031 --end--       HCN
...
#FUN-AA0059 --End--
                ...
                WHEN INFIELD(qty)
                      ...
                      END IF
                      CALL t8611_upd_peo()    #TQC-740117 add
                OTHERWISE
                    EXIT CASE
            END CASE
       ON IDLE g_idle_seconds
    ...
    PREPARE t8611_prepare FROM g_sql
    DECLARE t8611_cs                         #SCROLL CURSOR
        SCROLL CURSOR WITH HOLD FOR t8611_prepare
 
    LET g_forupd_sql = "SELECT * FROM pid_file ",
                       " WHERE pid01 = ? FOR UPDATE "
    LET g_forupd_sql=cl_forupd_sql(g_forupd_sql)

    DECLARE t8611_cl CURSOR FROM g_forupd_sql
    ...
    END IF
    PREPARE t8611_precount FROM g_sql
    DECLARE t8611_count CURSOR FOR t8611_precount
END FUNCTION
   ...
   #No.FUN-550029 --start--
       LET g_pid.pid01 = g_pid_t.pid01[1,g_doc_len],'-',
                        #g_pid_t.pid01[g_no_sp,g_no_ep] + 1 using'&&&&&&'
                         g_pid_t.pid01[5,10]+l_i using'&&&&&&' #FUN-570033
...
#              CALL cl_err(g_pid.pid01,SQLCA.sqlcode,0) #No.FUN-660156
        ...
        CALL g_pie.clear()
        CALL t8611_b_fill('1=1')                 #單身
        CALL t8611_b()                   #輸入單身
        INITIALIZE g_pid.* TO NULL
        FOR g_cnt = 1 TO g_pie.getLength()
            INITIALIZE g_pie[g_cnt].* TO NULL
        END FOR
        CLEAR FORM
   CALL g_pie.clear()
        CONTINUE WHILE
...
FUNCTION t8611_u()
   ...
   CALL g_pie.clear()
    ...
    OPEN t8611_cl USING g_pid.pid01
    IF STATUS THEN
       CALL cl_err("OPEN t8611_cl:", STATUS, 1)
       CLOSE t8611_cl
       ROLLBACK WORK
       RETURN
    END IF
    FETCH t8611_cl INTO g_pid.*            # 鎖住將被更改或取消的資料
    IF SQLCA.sqlcode THEN
...
DEFINE
   ...
   IF g_argv1<>'1' THEN LET g_pid.pid043=g_pid.pid044 END IF
   CALL cl_set_head_visible("","YES")   #No.FUN-6B0030
    INPUT g_pid.pid01,g_pid.pid02,g_pid.pid03,g_pid.pid022,g_pid.pid023,g_pid.pid043
          WITHOUT DEFAULTS
      FROM pid01,pid02,pid03,pid022,pid023,pid043
     ...
     #    IF p_cmd='u'AND g_pid.pid06='Y' THEN NEXT FIELD pid02 END IF
        ...
        BEFORE INPUT
          ...
          CALL t8611_set_entry(p_cmd)
          CALL t8611_set_no_entry(p_cmd)
        ...
        AFTER FIELD pid02      #工單編號
         ...
         AFTER FIELD pid022   #作業編號
            IF NOT cl_null(g_pid.pid022) THEN
        ...
        AFTER INPUT
          ...
          IF l_flag='Y' THEN
             ...
             NEXT FIELD pid02
          END IF
        IF g_argv1='1' THEN UPDATE pid_file SET pid043=g_pid.pid043
                             WHERE pid01=g_pid.pid01
                         ELSE UPDATE pid_file SET pid043=g_pid.pid044
                               WHERE pid01=g_pid.pid01
          END IF
...
FUNCTION t8611_q()
   ...
   CALL g_pie.clear()
    ...
    END IF
    MESSAGE " SEARCHING ! "
    OPEN t8611_cs                            # 從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
       CALL cl_err('',SQLCA.sqlcode,0)
       INITIALIZE g_pid.* TO NULL
    ELSE
       OPEN t8611_count
       FETCH t8611_count INTO g_row_count
       DISPLAY g_row_count TO FORMONLY.cnt
       CALL t8611_fetch('F')                  # 讀出TEMP第一筆並顯示
    END IF
    MESSAGE ""
END FUNCTION
 
#處理資料的讀取
FUNCTION t8611_fetch(p_flag)
DEFINE
    p_flag          LIKE type_file.chr1                  #處理方式  #No.FUN-690026 VARCHAR(1)
 
    CASE p_flag
        WHEN 'N' FETCH NEXT     t8611_cs INTO g_pid.pid01
        WHEN 'P' FETCH PREVIOUS t8611_cs INTO g_pid.pid01
        WHEN 'F' FETCH FIRST    t8611_cs INTO g_pid.pid01
        WHEN 'L' FETCH LAST     t8611_cs INTO g_pid.pid01
        WHEN '/'
...
#                       CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
                ...
                END IF
            END IF
            FETCH ABSOLUTE g_jump t8611_cs INTO g_pid.pid01
            LET mi_no_ask = FALSE
    END CASE
...
FUNCTION t8611_show()
    ...
    DISPLAY BY NAME                              # 顯示單頭值
       ...
       ,g_pid.pid012,g_pid.pid021   #FUN-A60092  
        
    IF g_argv1<>'1' THEN
        LET g_pid.pid043=g_pid.pid044
    END IF
    DISPLAY BY NAME g_pid.pid043
 
    CALL t8611_pid01('d')
    CALL t8611_b_fill(g_wc2)                 #單身
...
DEFINE
    ...
    l_time          LIKE pie_file.pie63,    #No.FUN-690026 VARCHAR(10)
    ...
    l_pie900        LIKE pie_file.pie900,   #No.FUN-870051 
    ...
    LET g_forupd_sql = " SELECT pie07,pie02,' ',pie04,'',pie16,pie70,'',pie50,pie60,",   #FUN-CB0087 add pie70,''
                       "        pie54,pie55,pie65,pie65,pie900", #No.FUN-870051 add pie900
                       " FROM pie_file",
                       "  WHERE pie01=? ",
                       "   AND pie07=? ",
                       " FOR UPDATE "
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    DECLARE t8611_bcl CURSOR FROM g_forupd_sql      # LOCK CURSOR
        ...
        BEFORE ROW
            ...
            IF g_rec_b>=l_ac THEN
                ...
                BEGIN WORK
                OPEN t8611_bcl USING g_pid.pid01,g_pie_t.pie07
                IF STATUS THEN
                   CALL cl_err("OPEN t8611_bcl:", STATUS, 1)
                   LET l_lock_sw = "Y"
                ELSE
                   FETCH t8611_bcl INTO g_pie[l_ac].*,l_pie50,l_pie60,
                                       l_pie54,l_pie55,l_pie64,l_pie65
                                       ,l_pie900     #No.FUN-870051
                   IF SQLCA.sqlcode THEN
                   ...
                   ELSE
                       ...
                       END IF
                       CALL t8611_pie02(' ')           #for referenced field
                       CALL t8611_azf03()   #FUN-CB0087
                   END IF
                END IF
            ...
            IF g_pie[l_ac].pie07 IS NOT NULL AND g_pie[l_ac].pie02 IS NOT NULL THEN
               IF g_argv1 = '1' THEN
                  INSERT INTO pie_file(pie01,pie02,pie03,pie04,pie05,
                                       pie06,pie07,pie16,pie50,
                                       pie51,pie52,pie53,pie54,pie55,pie70,pieplant,pielegal,pie012,pie013) #FUN-980004 add pieplant,pielegal #FUN-CB0087 add pie70
                       ...
                       VALUES(g_pid.pid01,g_pie[l_ac].pie02,
                              ...
                              g_pie[l_ac].qty,g_user,g_today,
                              g_time,g_peo,g_tagdate,g_pie[l_ac].pie70,g_plant,g_legal,' ',0) #FUN-980004 add g_plant,g_legal #FUN-CB0087 add g_pie[l_ac].pie70
                                                      #FUN-A60092 add ' ',0
               ELSE
                  INSERT INTO pie_file(pie01,pie02,pie03,pie04,pie05,
                                       pie06,pie07,pie16,pie60,
                                       pie61,pie62,pie63,pie64,pie65,pie70,pieplant,pielegal,pie012,pie013) #FUN-980004 add pieplant,pielegal  #FUN-CB0087 add pie70
                                                             #FUN-A60092 add pie012,pie013
                       VALUES(g_pid.pid01,g_pie[l_ac].pie02,
                              g_pid.pid022,g_pie[l_ac].pie04,
                              g_pid.pid022,
...
#                 CALL cl_err(g_pie[l_ac].pie02,SQLCA.sqlcode,0) #No.FUN-660156
        ...
        BEFORE FIELD pie07                        #default 序號
            CALL t8611_set_entry_b(p_cmd)
            IF p_cmd="u" THEN
               CALL t8611_set_no_entry_b(p_cmd)
            ELSE
               ...
               DISPLAY BY NAME g_pie[l_ac].qty                                     #FUN-910088--add--
                 SELECT count(*) INTO l_cnt FROM sfa_file
                  WHERE sfa01 =g_pid.pid02
                    ...
                    AND sfa27 =g_pie[l_ac].pie02  #No.FUN-870051
                 IF l_cnt = 0 THEN
#                   CALL cl_prtmsg(0,0,'mfg3414',g_lang)
                    CALL cl_confirm('mfg3414')
                    RETURNING l_sw
#                   IF l_sw ='y' OR l_sw ='Y' THEN
                    IF l_sw THEN
                        LET l_cmd = "asfi301 ","'",g_pid.pid02,"'" clipped
            ...
            ELSE
                 IF g_argv1 = '1' THEN
                     UPDATE pie_file SET
                      ...
                      WHERE CURRENT OF t8611_bcl
                     IF SQLCA.sqlcode THEN
#                       CALL cl_err(g_pie[l_ac].pie07,SQLCA.sqlcode,0) #No.FUN-660156
                        CALL cl_err3("upd","pie_file","","",SQLCA.sqlcode,"",
                                     "",1)  #No.FUN-660156
                         LET g_pie[l_ac].* = g_pie_t.*
...
DEFINE
    ...
    IF INT_FLAG THEN
        ...
        RETURN
    END IF
    CALL t8611_b_fill(l_wc2)
END FUNCTION
 
FUNCTION t8611_b_fill(p_wc2)              #BODY FILL UP
DEFINE
    ...
    #FUN-CB0087---mark---str---
    #LET g_sql =
    #    "SELECT pie07,pie02,ima02,pie04,' ',pie16,",
    #    "pie50,pie60",
...
#genero
FUNCTION t8611_set_entry(p_cmd)
DEFINE   p_cmd     LIKE type_file.chr1    #No.FUN-690026 VARCHAR(1)
...
FUNCTION t8611_set_no_entry(p_cmd)
DEFINE   p_cmd     LIKE type_file.chr1    #No.FUN-690026 VARCHAR(1)
 
   IF NOT g_before_input_done THEN
      IF (p_cmd='u'AND g_chkey = 'N' AND NOT cl_null(g_pid.pid02) AND g_pid.pid06='N') THEN  #No.FUN-570110
...
FUNCTION t8611_set_entry_b(p_cmd)
DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-690026 VARCHAR(1)
...
FUNCTION t8611_set_no_entry_b(p_cmd)
DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-690026 VARCHAR(1)
