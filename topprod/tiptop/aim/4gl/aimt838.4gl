...
#
# Pattern name...: aimt838.4gl
# Descriptions...: 複盤調整作業－多單位現有庫存
# Date & Author..: 05/07/18 By Carrier
# Modify.........: No.FUN-5A0199 05/12/29 By Sarah 標籤別放大至5碼,單號放大至10碼
...
DEFINE g_piaa09_t            LIKE piaa_file.piaa09  #No.FUN-BB0086
DEFINE g_qty_t             LIKE piaa_file.piaa50   #No.FUN-BB0086
...
#     DEFINE    l_time LIKE type_file.chr8              #No.FUN-6A0074
    ...
    DECLARE t838_cl CURSOR FROM g_forupd_sql              # LOCK CURSOR
 
    LET p_row = 3 LET p_col = 27
 
    OPEN WINDOW t838_w AT p_row,p_col
         WITH FORM "aim/42f/aimt838"   ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
 ...
 DEFINE  l_code  LIKE type_file.num5,    #No.FUN-690026 SMALLINT
    ...
    ELSE
       ...
       CONSTRUCT BY NAME g_wc ON                    # 螢幕上取條件
           piaa01, piaa09, piaa02, piaa03, piaa04, piaa05,piaa931,piaa70       #FUN-930122 Add piaa931   #FUN-CB0087 add>piaa70
           ...
           ON ACTION controlp
              CASE                  
                WHEN INFIELD(piaa931)
                  ...
                  LET g_qryparam.default1 = g_piaa.piaa931
    ...
    LET g_sql="SELECT piaa01,piaa09 FROM piaa_file ", # 組合出 SQL 指令
              ...
              "   AND (piaa50 IS NOT NULL OR piaa60 IS NOT NULL ",
              "         )",
              " AND ",g_wc CLIPPED
    IF l_code THEN 
       LET g_sql = g_sql CLIPPED,
                  " AND (piaa50 != piaa60 OR piaa50 IS NULL OR piaa60 IS NULL",
                  "      ) ",
                  " ORDER BY piaa01 "
    ELSE  LET g_sql = g_sql clipped, " ORDER BY piaa01"
    END IF
    PREPARE t838_prepare FROM g_sql           # RUNTIME 編譯
    DECLARE t838_cs                         # SCROLL CURSOR
        SCROLL CURSOR WITH HOLD FOR t838_prepare
    LET g_sql=
        "SELECT COUNT(*) FROM piaa_file ",
        " WHERE (piaa02 IS NOT NULL AND piaa02 != ' ' ) ",
        " AND (piaa50 IS NOT NULL OR piaa60 IS NOT NULL ",
        "  )",
        " AND ",g_wc CLIPPED
    IF l_code THEN 
       LET g_sql = g_sql CLIPPED,
                  ...
                  " ) "
    END IF 
    PREPARE t838_precount FROM g_sql
    DECLARE t838_count CURSOR FOR t838_precount
END FUNCTION
...
#                CALL cl_err(g_piaa.piaa01,SQLCA.sqlcode,0)
              ...
              IF cl_null(g_argv1) THEN
                 LET l_sum1=0
                 LET l_sum2=0
                 SELECT SUM(piaa50*piaa10) INTO l_sum1 FROM piaa_file
                  WHERE piaa01=g_piaa.piaa01
                 ...
                 SELECT SUM(piaa60*piaa10) INTO l_sum2 FROM piaa_file
                  WHERE piaa01=g_piaa.piaa01
                    AND piaa02=g_piaa.piaa02
                    AND piaa03=g_piaa.piaa03
                    AND piaa04=g_piaa.piaa04
                 ...
                 #No.FUN-BB0086--add--start--
                 UPDATE pia_file SET pia50=l_sum1,pia60=l_sum2,
...
#                   CALL cl_err('upd_pia',SQLCA.sqlcode,1)
                    CALL cl_err3("upd","pia_file",g_piaa.piaa01,g_piaa.piaa02,
                                  SQLCA.sqlcode,"","",1)   #NO.FUN-640266 #No.FUN-660156
                 END IF 
              END IF
        ...
        ELSE
           ...
           INITIALIZE g_piaa.* LIKE piaa_file.*
           LET g_qty  = NULL
          #LET g_piaa.piaa01 = g_piaa_t.piaa01[1,3]           #FUN-5A0199 mark
           LET g_piaa.piaa01 = g_piaa_t.piaa01[1,g_doc_len]   #FUN-5A0199
           CALL t838_i("a")                            # 各欄位輸入
           IF INT_FLAG THEN                            # 若按了DEL鍵
               LET INT_FLAG = 0
               INITIALIZE g_piaa.* TO NULL
...
FUNCTION t838_i(p_cmd)
  ...
  DEFINE l_tf      LIKE type_file.chr1     #No.FUN-BB00086
  DEFINE l_case    STRING     #No.FUN-BB00086
  DEFINE l_sql        STRING                #FUN-CB0087
  DEFINE l_where      STRING                #FUN-CB0087
 
    INPUT g_piaa.piaa01,g_piaa.piaa09,g_piaa.piaa02,
          g_piaa.piaa03,g_piaa.piaa04,g_piaa.piaa05,
          g_piaa.piaa06,g_piaa.piaa07,g_piaa.piaa70,g_qty            #FUN-CB0087 add>piaa70
          WITHOUT DEFAULTS 
      FROM piaa01,piaa09,piaa02,piaa03,piaa04,
           piaa05,piaa06,piaa07,piaa70,qty                           #FUN-CB0087 add>piaa70
	   ...
	   IF NOT cl_null(g_piaa.piaa09) THEN
           ...
           #No.FUN-BB0086--add--start--
           LET l_tf = ""
        #No.FUN-C20068--mark--start--
        #   IF p_cmd = 'u' AND g_chkey matches'[Nn]' AND (NOT g_before_input_done) THEN
        #      LET g_qty = s_digqty(g_qty,g_piaa.piaa09)
        #      DISPLAY g_qty TO FORMONLY.qty
        #   ELSE 
        #      CALL t838_qty_check(p_cmd) RETURNING l_tf,l_case
        #   END IF 
        #No.FUN-C20068--mark---end---
        #No.FUN-C20068--add--start--
           IF NOT cl_null(g_qty) AND g_qty<>0 THEN
              CALL t838_qty_check(p_cmd) RETURNING l_tf,l_case
           END IF
        #No.FUN-C20068--add--start--
           LET g_piaa09_t = g_piaa.piaa09
...
FUNCTION t838_piaa01(p_cmd)  
    ...
    CASE WHEN SQLCA.SQLCODE = 100        LET g_errno = 'mfg0002'
              ...
              LET l_imaacti     = NULL
              LET g_piaa.piaa931 = NULL                                 #FUN-930122
    ...
    IF cl_null(g_errno) OR p_cmd = 'd' THEN
       DISPLAY BY NAME g_piaa.piaa02,g_piaa.piaa03,g_piaa.piaa04,
                       ...
                       g_piaa.piaa09,g_piaa.piaa50,g_piaa.piaa60,g_piaa.piaa931      #FUN-930122 Add piaa931
       DISPLAY l_ima02       TO FORMONLY.ima02
    END IF
END FUNCTION
   
FUNCTION t838_piaa02(p_cmd)  #料件編號
    DEFINE p_cmd	LIKE type_file.chr1,    #No.FUN-690026 VARCHAR(1)
           l_ima02      LIKE ima_file.ima02,
           l_ima021     LIKE ima_file.ima021,
           l_ima906     LIKE ima_file.ima906,
...
FUNCTION t838_q()
    ...
    END IF
    OPEN t838_count
    FETCH t838_count INTO g_row_count
    DISPLAY g_row_count TO FORMONLY.cnt  
    OPEN t838_cs                            # 從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
       CALL cl_err(g_piaa.piaa01,SQLCA.sqlcode,0)
       INITIALIZE g_piaa.* TO NULL
       LET g_qty = NULL
    ELSE
       CALL t838_fetch('F')                  # 讀出TEMP第一筆並顯示
    END IF
END FUNCTION
 
FUNCTION t838_fetch(p_flpiaa)
    DEFINE p_flpiaa          LIKE type_file.chr1    #No.FUN-690026 VARCHAR(1)
 
    CASE p_flpiaa
        WHEN 'N' FETCH NEXT     t838_cs INTO g_piaa.piaa01,g_piaa.piaa09
        WHEN 'P' FETCH PREVIOUS t838_cs INTO g_piaa.piaa01,g_piaa.piaa09
        WHEN 'F' FETCH FIRST    t838_cs INTO g_piaa.piaa01,g_piaa.piaa09
        WHEN 'L' FETCH LAST     t838_cs INTO g_piaa.piaa01,g_piaa.piaa09
        WHEN '/'
            IF (NOT mi_no_ask) THEN
                ...
                END IF
            END IF
            FETCH ABSOLUTE g_jump t838_cs INTO g_piaa.piaa01,g_piaa.piaa09
            LET mi_no_ask = FALSE
    END CASE
    IF SQLCA.sqlcode THEN
       CALL cl_err(g_piaa.piaa01,SQLCA.sqlcode,0)
...
FUNCTION t838_show()
    ...
    DISPLAY BY NAME 
        ...
        g_piaa.piaa07,g_piaa.piaa50,g_piaa.piaa60,g_piaa.piaa931,g_piaa.piaa70              #FUN-930122 Add piaa931  #FUN-CB0087 add>piaa70
    DISPLAY g_qty       TO FORMONLY.qty 
    CALL t838_piaa02('d')
    #FUN-CB0087---add---str---
    IF g_piaa.piaa70 IS NOT NULL THEN
...
FUNCTION t838_u()
 ...
 DEFINE  l_pia09         LIKE pia_file.pia09   #No.FUN-BB0086
    ...
    OPEN t838_cl USING g_piaa.piaa01,g_piaa.piaa09
    IF STATUS THEN
      CALL cl_err("OPEN t838_cl:", STATUS, 1)
      CLOSE t838_cl
      ROLLBACK WORK
      RETURN
    END IF
    FETCH t838_cl INTO g_piaa.*               # 對DB鎖定
    IF SQLCA.sqlcode THEN
       CALL cl_err(g_piaa.piaa01,SQLCA.sqlcode,0)
       RETURN
    END IF
    CALL t838_show()                          # 顯示最新資料
    WHILE TRUE
        CALL t838_i("u")                      # 欄位更改
        IF INT_FLAG THEN
           LET INT_FLAG = 0
