...
DEFINE tm  RECORD
           ...
           u            LIKE type_file.chr3,    #No.FUN-690026 VARCHAR(03)
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aimr610',g_sql) CLIPPED   # 產生Temp Table
  ...
  #TQC-610072-begin
   ...
   LET tm.a = ARG_VAL(16)   #CHI-C70012
   #No.FUN-570264 --start--
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimr610',g_time,l_cmd)
      END IF
      CLOSE WINDOW r610_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
#MOD-720046 By TSD.Jin--end
    ...
    #CALL cl_outnam('aimr610') RETURNING l_name
...
## --
	 ...
	 #    		 WHEN tm.choice ='4' LET l_order[g_i] = sr.ima12
         ...
         #MOD-720046 By TSD.Jin--start
         #OUTPUT TO REPORT r610_rep(sr.*)     #MOD-720046 By TSD.Jin mark
         ...
         EXECUTE insert_prep USING
            ...
            l_imk085_fac,l_imk09_fac,g_azi03,g_azi04,g_azi05 
         #MOD-720046 By TSD.Jin--end
     END FOREACH
    ...
    IF g_zz05 = 'Y' THEN
       ...
       LET g_str = tm.wc
    END IF
    LET g_str = g_str,";",tm.yy USING '####',";",tm.m1 USING '##',";",
                tm.m2 USING '##',";",tm.choice,";",tm.s,";",tm.t,";",tm.u,";",
                tm.detail_flag
 
    CALL cl_prt_cs3('aimr610','aimr610',l_sql,g_str)   #FUN-710080 modify
