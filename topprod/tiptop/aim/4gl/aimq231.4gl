...
#                 wc2    LIKE type_file.chr1000		# Body Where condition  #No.FUN-690026 VARCHAR(500)         #TQC-CC0058 mark
    ...
    g_img         RECORD
                  ...
                  imd02  LIKE imd_file.imd02,  #  TQC-CC0094--add-
                  img03  LIKE img_file.img03, # 存放位置
                  ime03  LIKE ime_file.ime03,  #  TQC-CC0094--add-
...
#     DEFINE   l_time LIKE type_file.chr8    #No.FUN-6A0074
 ...
 #  LET g_bdate=DATE(g_msg)  #FUN-570246
    ...
    OPEN WINDOW q231_w AT p_row,p_col
         WITH FORM "aim/42f/aimq231" 
...
#              "   AND ima01=img01 "                      #CHI-A80048 add
    ...
    LET g_sql= " SELECT img01,img02,img03,img04,ima02,ima021,ima08,img10,imk09 ",
               ...
               " INTO temp x "
    PREPARE q231_sel FROM g_sql
    EXECUTE q231_sel

    LET g_sql=" SELECT COUNT(*) FROM x "
    #TQC-C10121--end
    PREPARE q231_pp  FROM g_sql
    DECLARE q231_count   CURSOR FOR q231_pp
END FUNCTION
...
FUNCTION q231_q()
    ...
    ELSE
       OPEN q231_count
       FETCH q231_count INTO g_row_count
...
DEFINE
    ...
    CASE p_flag
        WHEN 'N' FETCH NEXT     q231_cs INTO g_img.img01,g_img.img02,g_img.img03,g_img.img04,g_ima02,  #No.TQC-9B0006
                                           # g_ima021,g_ima08,g_img.img10,g_imk09
                                             g_ima021,g_ima08,g_img.img10,g_imk09 #FUN-890125                                           
        WHEN 'P' FETCH PREVIOUS q231_cs INTO g_img.img01,g_img.img02,g_img.img03,g_img.img04,g_ima02,  #No.TQC-9B0006
                                           # g_ima021,g_ima08,g_img.img10,g_imk09
                                             g_ima021,g_ima08,g_img.img10,g_imk09 #FUN-890125                                           
        WHEN 'F' FETCH FIRST    q231_cs INTO g_img.img01,g_img.img02,g_img.img03,g_img.img04,g_ima02,  #No.TQC-9B0006
                                           # g_ima021,g_ima08,g_img.img10,g_imk09
                                             g_ima021,g_ima08,g_img.img10,g_imk09 #FUN-890125                                          
        WHEN 'L' FETCH LAST     q231_cs INTO g_img.img01,g_img.img02,g_img.img03,g_img.img04,g_ima02,  #No.TQC-9B0006
                                           # g_ima021,g_ima08,g_img.img10,g_imk09
                                             g_ima021,g_ima08,g_img.img10,g_imk09 #FUN-890125                                              
        WHEN '/'
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
               ...
               END IF
            END IF
            FETCH ABSOLUTE g_jump q231_cs INTO g_img.img01,g_img.img02,g_img.img03,g_img.img04,g_ima02,  #No.TQC-9B0006
                                          # g_ima021,g_ima08,g_img.img10,g_imk09
                                            g_ima021,g_ima08,g_img.img10,g_imk09 #FUN-890125                                             
            LET mi_no_ask = FALSE
    END CASE
	...
	SELECT img01,img02,'',img03,'',img04,img09,img10    #TQC-CC0094--add-''''
	  INTO g_img.* 
	  FROM img_file
...
FUNCTION q231_b_fill()              #BODY FILL UP
   ...
   LET l_sql =
        "SELECT tlf06,tlf07,tlf026,tlf036,tlf10,tlf11,tlf024,' ',tlf03,",
        "       tlf031,tlf032,tlf033,tlf034,tlf035,tlf13,tlf08,tlf12,tlf02 ",
#        " FROM  tlf_file,idd_file",
         " FROM  tlf_file",
    ...
    #           " AND tlf033 = '",g_img.img04,"' ) )", 
        "  AND ",tm.wc2 CLIPPED,
        " ORDER BY tlf06,tlf07,tlf08"      #MOD-8C0060 add tlf07
 ...
 #  END FOR
    LET g_rec_b=0
    LET m_cnt = 1
    FOREACH q231_bcs INTO g_tlff_1[m_cnt].*,g_tlf03,
#    FOREACH q231_bcs INTO g_tlff_1[m_cnt].tlf06,g_tlff_1[m_cnt].cond,g_tlff_1[m_cnt].tlf026,
#                          g_tlff_1[m_cnt].tlf036,g_tlff_1[m_cnt].tlf10,g_tlff_1[m_cnt].tlf11,
#                          g_tlff_1[m_cnt].tlf024,g_tlf03,
                          g_tlf031,g_tlf032,g_tlf033,g_tlf034,g_tlf035,g_tlf13,
                          g_tlf08, g_tlf12
                          ,g_tlf02    #No.TQC-A80095
...
#       END IF
 ...
 END FOREACH
    CALL g_tlff_1.deleteElement(m_cnt)    #TQC-6B0162 add
    LET g_rec_b = m_cnt - 1
...
FUNCTION q231_bp(p_ud)
   ...
   CALL cl_set_act_visible("accept,cancel", FALSE)
    DISPLAY ARRAY g_tlff_1 TO s_tlf.* ATTRIBUTE(COUNT=g_rec_b,UNBUFFERED)  #No.MOD-480045
