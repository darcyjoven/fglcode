...
# Modify ........: No:9132 04/07/08 By Mandy aimg300.4gl 第185 OUTER azf_file,where 條件azf01請改為azf_file.azf01,否則報表OUTER會有問題,實際是修正aimg300.ora
...
#                 "azf03.azf_file.azf03,l_str2.type_file.chr1000"     #No.FUN-860026 
                 ...
                 "inb16.inb_file.inb16,l_str3.type_file.chr1000,",    #No.FUN-870163 
    ...
    LET l_table = cl_prt_temptable('aimg300',g_sql) CLIPPED   # 產生Temp Table
    IF l_table = -1 THEN
...
#No.FUN-860026---begin
   ...
   LET l_table1 = cl_prt_temptable('aimg3001',g_sql) CLIPPED                                                                        
   IF  l_table1 = -1 THEN 
       CALL cl_used(g_prog, g_time,2) RETURNING g_time #FUN-B40092
       CALL cl_gre_drop_temptable(l_table||"|"||l_table1) #FUN-B40092
       EXIT PROGRAM 
  ...
  #TQC-610072-end
      THEN CALL aimg300_tm(0,0)             # Input print condition
     # ELSE LET tm.wc="ina01= '",tm.wc CLIPPED,"'"     #TQC-610072
    ELSE                                               #TQC-610072
           CALL aimg300()                   # Read data and create out-file
   END IF
...
FUNCTION aimg300_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,    #No.FUN-690026 SMALLINT
   ...
   OPEN WINDOW aimg300_w AT p_row,p_col
        WITH FORM "aim/42f/aimg300"
################################################################################
...
WHILE TRUE
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW aimg300_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1)
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimg300',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW aimg300_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1)
      EXIT PROGRAM
   END IF
...
#No.FUN-860026---begin
    LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
                ...
                "        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?,?)"  #No.FUN-860026  #No.FUN-870163 add  #FUN-C40019 add 4?                                                                      
...
#                "       ina06,ina07,inb03,inb04,ima02,inb05,inb06,",          #MOD-840423-modify
              ...
              #   "  FROM ina_file,inb_file,OUTER gem_file,",           #FUN-C50007--MARK             
              #   " OUTER azf_file,OUTER ima_file ",                    #FUN-C50007--MARK             
              #  " WHERE ",tm.wc CLIPPED,                               #FUN-C50007--MARK              
              #  "   AND ina01=inb01 ",                                 #FUN-C50007--MARK             
               # "   AND ima_file.ima01=inb_file.inb04 ",               #FUN-C50007--MARK
               # "   AND gem_file.gem01=ina_file.ina04 ",               #FUN-C50007--MARK
  ...
  #FUN-C50007--ADD--STR
    IF g_sma.sma79='Y' THEN 				
       LET l_sql = l_sql CLIPPED,											
                   " FROM inb_file LEFT OUTER JOIN ima_file ON inb04 = ima01 LEFT OUTER JOIN azf_file ON inb15 = azf01 AND azf02 = 'A', ",
                   " ina_file LEFT OUTER JOIN gem_file ON ina04 = gem01"
    ELSE LET l_sql = l_sql CLIPPED,											
                     " FROM inb_file LEFT OUTER JOIN ima_file ON inb04 = ima01 LEFT OUTER JOIN azf_file ON inb15 = azf01 AND azf02 = '2', ",
                     " ina_file LEFT OUTER JOIN gem_file ON ina04 = gem01"
    END IF 											
    ...
    #FUN-C50007 --ADD---END
     ...
     PREPARE aimg300_prepare1 FROM l_sql
     ...
     END IF
     DECLARE aimg300_curs1 CURSOR FOR aimg300_prepare1
 
   #FUN-710084  --begin
   # CALL cl_outnam('aimg300') RETURNING l_name
     #No.FUN-580005 --start--
   ...
   # START REPORT aimg300_rep TO l_name
   #FUN-710084  --end
...
#No.FUN-860026---begin       
    ...
    #FUN-C50007--MARK--STR
    LET m = 0 LET i=0 LET j=0    #FUN-C50007
    FOREACH  r920_d USING sr.ina01,sr.inb03 INTO l_rvbs.*        #FUN-C50007
      LET flag = 1                                                                                        
      LET m=m+1                                                                                                                     
      IF (m mod 2) = 1  THEN                                                                                                        
         LET i=i+1                                                                                                                  
...
#                                 sr.ina06,sr.ina07,sr.inb03,sr.inb04,sr.ima02,              #MOD-840423-modify
                                  ...
                                  sr.inb16,l_str3,    #No.FUN-870163  
                                  "",l_img_blob,"N",""    #FUN-C40019 add
      #OUTPUT TO REPORT aimg300_rep(sr.*)
     END FOREACH
 
    #FINISH REPORT aimg300_rep
...
#        LET g_str = g_str ,";",tm.wc    #No.FUN-860026 
         LET l_str1 = tm.wc                                      
     END IF
###GENGRE###     LET g_str = g_str,";",l_str1,";",tm.c   #No.FUN-860026
   # CALL cl_prt_cs3('aimg300',l_sql,g_str)   #TQC-730088
###GENGRE###     CALL cl_prt_cs3('aimg300','aimg300',l_sql,g_str) 
    LET g_cr_table = l_table                  #主報表的temp table名稱          #FUN-C40019 add
    LET g_cr_apr_key_f = "ina01"              #報表主鍵欄位名稱，用"|"隔開     #FUN-C40019 add
    CALL aimg300_grdata()    ###GENGRE###
### FUN-710084 End ###
END FUNCTION
### FUN-710084 Start ###
{
REPORT aimg300_rep(sr)
...
FUNCTION aimg300_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aimg300")
        IF handler IS NOT NULL THEN
            START REPORT aimg300_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY ina01,inb03"       
          
            DECLARE aimg300_datacur1 CURSOR FROM l_sql
            FOREACH aimg300_datacur1 INTO sr1.*
                OUTPUT TO REPORT aimg300_rep(sr1.*)
            END FOREACH
            FINISH REPORT aimg300_rep
        END IF
...
REPORT aimg300_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE sr2 sr2_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B40092------add------str
    DEFINE l_ina04_gem02       STRING
    DEFINE l_inb05_inb06_inb07 STRING
    DEFINE l_inb15_azf03       STRING
    DEFINE l_sql               STRING
    ...
    #FUN-B40092------add------end
    DEFINE l_display3          STRING   #FUN-C30085
    DEFINE l_display4          LIKE type_file.chr1 #DEV-D30030
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
            PRINTX g_sma115        #FUN-B40092
            IF sr1.ina00 = '1' OR sr1.ina00 = '2' OR sr1.ina00 = '3' OR sr1.ina00 = '4' OR sr1.ina00 = '5' OR sr1.ina00 = '6' THEN 
               LET l_title = cl_gr_getmsg("gre-110",g_lang,sr1.ina00)
            ELSE
               LET l_title = NULL
            END IF
            PRINTX l_title
            #DEV-D30030--begin
            IF g_aza.aza131 = 'Y' AND sr1.ina00 MATCHES "[13]" THEN
               LET l_display4 = 'Y'
            ELSE
               LET l_display4 = 'N'
            END IF
        ...
        BEFORE GROUP OF sr1.ina01
            LET l_ina04_gem02 = sr1.ina04,' ',sr1.gem02    #FUN-B40092  add
            LET l_lineno = 0
            PRINTX l_ina04_gem02

        BEFORE GROUP OF sr1.inb03

        
        ON EVERY ROW
            #FUN-B40092------add------str
            IF sr1.inb15 = " " AND sr1.l_str2 = " " THEN
               LET l_display = 'N'
            ELSE
               LET l_display = 'Y'
            END IF
            ...
            LET l_sql = "SELECT MAX(flag) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " WHERE ina01='",sr1.ina01 CLIPPED,"'"
            PREPARE l_prep1 FROM l_sql
            EXECUTE l_prep1 INTO l_flag
            PRINTX l_flag

           #IF cl_null(sr1.l_str3) OR g_sma115 = "N" THEN   #FUN-C30085
            IF cl_null(sr1.l_str3) THEN   #FUN-C30085
               LET l_display1 = "N"
            ELSE 
               LET l_display1 = "Y"
            END IF 
            ...
            PRINTX l_display3 
            #FUN-C30085--add--mod- 
            LET l_inb15_azf03 = sr1.inb15,' ',sr1.azf03
            IF cl_null(l_inb15_azf03) THEN
               LET l_display2 = "N"
            ELSE 
               LET l_display2 = "Y"
            END IF
            PRINTX l_display2
            IF cl_null(sr1.inb07) THEN
               LET l_inb05_inb06_inb07 = '(',sr1.inb05,'/',sr1.inb06,'/',')'
            ELSE
               LET l_inb05_inb06_inb07 = '(',sr1.inb05,'/',sr1.inb06,'/',sr1.inb07,')'
            END IF
            PRINTX l_inb05_inb06_inb07
            PRINTX l_inb15_azf03
            #FUN-B40092------add------end
        ...
        AFTER GROUP OF sr1.ina01
           #FUN-B40092------add------str
            LET l_sql=" SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                      " WHERE rvbs01 = '",sr1.ina01,"'", # AND rvbs02 = ",sr1.inb03  
                      " ORDER BY rvbs02"
           START REPORT aimg300_subrep01
           DECLARE aimg300_repcur1 CURSOR FROM l_sql
           FOREACH aimg300_repcur1 INTO sr2.*
               OUTPUT TO REPORT aimg300_subrep01(sr2.*)
           END FOREACH
           FINISH REPORT aimg300_subrep01
           #FUN-B40092------add------end
            
        AFTER GROUP OF sr1.inb03
...
END REPORT
 ...
 #FUN-B40092------add------str
REPORT aimg300_subrep01(sr2)
DEFINE sr2 sr2_t
DEFINE l_sum_rvbs06  LIKE rvbs_file.rvbs06
DEFINE l_sum_rvbs106 LIKE rvbs_file.rvbs06
DEFINE l_sum_rvbs206 LIKE rvbs_file.rvbs06
...
FORMAT
    ...
    AFTER GROUP OF sr2.rvbs02
       ...
       LET l_sum_rvbs106 = GROUP SUM(sr2.rvbs106)
       LET l_sum_rvbs206 = GROUP SUM(sr2.rvbs206)
       IF NOT cl_null(sr2.rvbs106) AND NOT cl_null(sr2.rvbs206) THEN
          LET l_sum_rvbs06 = l_sum_rvbs106 + l_sum_rvbs206
       ELSE
          IF cl_null(sr2.rvbs206) THEN
             LET l_sum_rvbs06 = l_sum_rvbs106
          END IF
       END IF
       PRINTX l_sum_rvbs06
END REPORT
