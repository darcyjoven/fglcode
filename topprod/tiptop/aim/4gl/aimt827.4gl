...
#
# Pattern name...: aimt827.4gl
# Descriptions...: 初盤adjust作業－在製工單
# Date & Author..: 93/06/14 By Apple
# Modify.........: No.FUN-550029 05/05/30 By vivien 單據編號格式放大
# Modify.........: No.FUN-570029 05/07/05 By kim 修改快速輸入的方式
# Modify.........: No.TQC-5C0003 05/12/01 By kim 修正 FUN-570029 (快速輸入會跳過第一筆)的 BUG
...
# Modify.........: No.TQC-B60199 11/06/21 By zhangweib 筆數查詢條件有錯需修改
# Modify.........: No.FUN-910088 11/11/30 By chenjing 增加數量欄位小數取位
# Modify.........: No.FUN-CB0087 12/12/07 By xianghui 倉庫單據理由碼改善
...
DEFINE
    ...
    g_pie01_t       LIKE pie_file.pie01,
    g_ima25         LIKE ima_file.ima25,
    g_wc,g_sql      string,                 #No.FUN-580092 HCN
    g_qty           LIKE pie_file.pie30
...
#     DEFINE    l_time LIKE type_file.chr8              #No.FUN-6A0074
   ...
   END IF
    ...
    DECLARE t827_cl CURSOR FROM g_forupd_sql              # LOCK CURSOR
 
       LET p_row = 3 LET p_col = 27
 
    OPEN WINDOW t827_w AT p_row,p_col
             WITH FORM "aim/42f/aimt827" 
...
FUNCTION t827_cs()
 ...
 DEFINE  l_flag  LIKE type_file.chr1,   #FUN-CB0087
    ...
    CLEAR FORM
    CALL cl_getmsg('mfg8888',g_lang) RETURNING l_str
    CALL cl_prompt(0,0,l_str) RETURNING l_code 
    INITIALIZE g_pie.* TO NULL    #FUN-640213 add
    CONSTRUCT BY NAME g_wc ON                    # 螢幕上取條件
        pie01, pie07, pid02, pid03, pie05, 
...
#FUN-A60092 --end--
    ...
    LET g_sql="SELECT pie_file.pie01,pie07 ",
             ...
             #"   AND (pie30 IS NOT NULL OR pie40 IS NOT NULL) ",   #TQC-B60199 Mark
              " AND  ",g_wc CLIPPED
 
    IF l_code THEN 
       LET g_sql = g_sql clipped,
                  " AND (pie30 IS NOT NULL OR pie40 IS NOT NULL) ",   #TQC-B60199 Add
                  " AND (pie30 != pie40 OR pie30 IS NULL OR pie40 IS NULL)",
                   " ORDER BY pie01,pie07 "
    ELSE 
       LET g_sql = g_sql clipped, " ORDER BY pie01,pie07"
    END IF
 
    PREPARE t827_prepare FROM g_sql           # RUNTIME 編譯
    DECLARE t827_cs                         # SCROLL CURSOR
        SCROLL CURSOR WITH HOLD FOR t827_prepare
  # IF g_wc = " 1=1" THEN
       LET g_sql=
           "SELECT COUNT(*) FROM pie_file,pid_file WHERE ",g_wc CLIPPED,   #TQC-B60199  Add pid_file
           "   AND pid01=pie01",   #TQC-B60199   Add
           "   AND (pie02 IS NOT NULL AND pie02 != ' ' ) "
         #,"   AND (pie30 IS NOT NULL OR pie40 IS NOT NULL) "   #TQC-B60178   Mark
  # ELSE
  #    LET g_sql=
  #       "SELECT COUNT(*) FROM pid_file,pie_file WHERE pid01=pie01 ",
  #       " AND ",g_wc CLIPPED
  # END IF
    IF l_code THEN 
       LET g_sql = g_sql clipped,
                  ...
                  " AND (pie30 != pie40 OR pie30 IS NULL OR pie40 IS NULL)"
    END IF
    PREPARE t827_precount FROM g_sql
    DECLARE t827_count CURSOR FOR t827_precount
END FUNCTION
...
#           EXIT MENU
        ...
        ON ACTION related_document             #相關文件"                        
         LET g_action_choice="related_document"           
            IF cl_chk_act_auth() THEN                     
               IF g_pie.pie01 IS NOT NULL THEN            
                  LET g_doc.column1 = "pie01"               
                  LET g_doc.column2 = "pie07"               
                  LET g_doc.value1 = g_pie.pie01            
                  LET g_doc.value2 = g_pie.pie07            
                  CALL cl_doc()                             
               END IF                                        
            END IF                                           
...
FUNCTION t827_a()
  ...
  DEFINE l_flag         LIKE type_file.num5    #No.FUN-690026 SMALLINT
    ...
    WHILE TRUE
       ...
       #LET g_pie.pie01 = g_pie_t.pie01
        ...
        IF INT_FLAG THEN                            # 若按了DEL鍵
            ...
            EXIT WHILE
        END IF
        IF g_pie.pie01 IS NULL OR g_qty IS NULL 
        THEN CONTINUE WHILE
        END IF
        ...
        LET g_pie.pie30 = g_qty
        LET g_pie.pie40 = g_qty
        UPDATE pie_file SET pie_file.* = g_pie.*    # 更新DB
            WHERE pie01 = g_pie.pie01 AND pie07 = g_pie.pie07               # COLAUTH?
...
FUNCTION t827_i(p_cmd)
    ...
    INPUT g_pie.pie01,g_pie.pie07,g_pid.pid03,    
          g_pid.pid02,g_pie.pie05,g_pie.pie03,g_pie.pie02,
          g_pie.pie40,g_qty,g_pie.pie70     #FUN-CB0087                
          WITHOUT DEFAULTS 
      FROM pie01,pie07,pid03,
           pid02,pie05,pie03,pie02,
           pie40,qty,pie70                 #FUN-CB0087
...
#           IF p_cmd = 'u' THEN NEXT FIELD qty  END IF
        ...
        AFTER FIELD pie01 
           IF g_pie.pie01 IS NOT NULL OR g_pie.pie01 != ' ' THEN
                CALL t827_pie01('d')
                IF NOT cl_null(g_errno)  THEN 
                   CALL cl_err(g_pie.pie01,'mfg0114',0)
                   NEXT FIELD pie01
                END IF
...
#FUN-AA0059 ---------------------end-------------------------------
            ...
            THEN CALL t827_part('a',g_pid.pid03,'1')
                 IF NOT cl_null(g_errno)  THEN 
...
#FUN-AA0059 ---------------------end-------------------------------
            ...
            THEN CALL t827_part('a',g_pie.pie02,'2')
                 IF NOT cl_null(g_errno)  THEN 
        ...
        AFTER FIELD qty
            LET g_qty = s_digqty(g_qty,g_pie.pie04)   #FUN-910088--add--
            DISPLAY g_qty TO FORMONLY.qty             #FUN-910088--add--
            IF g_qty < 0 THEN 
               NEXT FIELD qty
            END IF 
...
#FUN-AA0059 --End--
                  DISPLAY BY NAME g_pid.pid03                                   
                  CALL t827_part('d',g_pid.pid03,'1')                           
                  NEXT FIELD pid03                                              
...
#FUN-AA0059 --End-- 
                  DISPLAY BY NAME g_pie.pie02                                   
                  CALL t827_part('d',g_pie.pie02,'2')                           
                  NEXT FIELD pie02                                              
               #FUN-CB0087---add---str---
               WHEN INFIELD(pie70) #理由
...
DEFINE   p_cmd   LIKE type_file.chr1                                                            #No.FUN-690026 VARCHAR(1)
   ...
   IF p_cmd = 'a' AND (NOT g_before_input_done) THEN                            
      CALL cl_set_comp_entry("pie01",TRUE)                                      
   END IF                                                                       
...
DEFINE   p_cmd   LIKE type_file.chr1                                                            #No.FUN-690026 VARCHAR(1)
   ...
   IF p_cmd = 'u' AND g_chkey matches'[Nn]' AND (NOT g_before_input_done) THEN  
      CALL cl_set_comp_entry("pie01",FALSE)                                     
   END IF                                                                       
                                                                                
END FUNCTION  
   
FUNCTION t827_pie01(p_cmd)  
DEFINE p_cmd     LIKE type_file.chr1,    #No.FUN-690026 VARCHAR(1)
    ...
    LET g_errno = ' '
    SELECT pid_file.*, sfb02, ima02,ima021,imaacti,sfb98 #FUN-670093
      INTO g_pid.*,l_sfb02, l_ima02,l_ima021,l_imaacti,g_sfb98 #FUN-670093
      FROM pid_file, OUTER sfb_file, OUTER ima_file
      WHERE pid01 = g_pie.pie01 AND pid_file.pid03 = ima_file.ima01
         AND pid_file.pid02= sfb_file.sfb01
...
FUNCTION t827_pie07(p_cmd)  
    DEFINE p_cmd	LIKE type_file.chr1,    #No.FUN-690026 VARCHAR(1)
           l_ima02      LIKE ima_file.ima02,
           l_ima021     LIKE ima_file.ima021
 
    LET g_errno = ' '
    SELECT pie_file.*,ima02,ima021
      INTO g_pie.*,l_ima02,l_ima021
...
FUNCTION t827_part(p_cmd,p_part,p_dis)  #料件編號
    DEFINE p_cmd	LIKE type_file.chr1,    #No.FUN-690026 VARCHAR(1)
...
FUNCTION t827_q()
    ...
    END IF
    OPEN t827_count
    FETCH t827_count INTO g_row_count
    DISPLAY g_row_count TO FORMONLY.cnt  
    OPEN t827_cs                            # 從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
        CALL cl_err(g_pie.pie01,SQLCA.sqlcode,0)
        INITIALIZE g_pid.* TO NULL
        LET g_qty = NULL
    ELSE
        CALL t827_fetch('F')                  # 讀出TEMP第一筆並顯示
    END IF
END FUNCTION
 
FUNCTION t827_fetch(p_flpid)
    DEFINE
        p_flpid          LIKE type_file.chr1    #No.FUN-690026 VARCHAR(1)
 
    CASE p_flpid
        WHEN 'N' FETCH NEXT     t827_cs INTO g_pie.pie01,
                                                         g_pie.pie07
        WHEN 'P' FETCH PREVIOUS t827_cs INTO g_pie.pie01,
                                                         g_pie.pie07
        WHEN 'F' FETCH FIRST    t827_cs INTO g_pie.pie01,
                                                         g_pie.pie07
        WHEN 'L' FETCH LAST     t827_cs INTO g_pie.pie01,
                                                         g_pie.pie07
        WHEN '/'
...
#                      CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
                ...
                END IF
            END IF
            FETCH ABSOLUTE g_jump t827_cs INTO g_pie.pie01,
                                                           g_pie.pie07
            LET mi_no_ask = FALSE
    END CASE
    IF SQLCA.sqlcode THEN
        CALL cl_err(g_pie.pie01,SQLCA.sqlcode,0)
        INITIALIZE g_pie.* TO NULL  #TQC-6B0105
        RETURN
    ELSE
       CASE p_flpid
...
FUNCTION t827_show()
    ...
    DISPLAY BY NAME 
        g_pie.pie01,g_pid.pid02,g_pid.pid03,
        g_pie.pie30,g_pie.pie40,g_pie.pie07,
        g_pie.pie05,g_pie.pie03,g_pie.pie02
       ,g_pie.pie012,g_pie.pie013,   #FUN-A60092 
        g_pie.pie70                  #FUN-CB0087
        
    DISPLAY g_pie.pie04 TO FORMONLY.pie04_1 
    DISPLAY g_pie.pie04 TO FORMONLY.pie04_2 
    DISPLAY g_pie.pie04 TO FORMONLY.unit    
    CALL t827_azf03()                #FUN-CB0087
    CALL t827_pie01('d')  
    CALL t827_part('d',g_pid.pid03,'1')
    CALL t827_part('d',g_pie.pie02,'2')
    CALL cl_show_fld_cont()                   #No.FUN-550037 hmf
...
FUNCTION t827_u()
    IF s_shut(0) THEN RETURN END IF
    IF g_pie.pie01 IS NULL THEN CALL cl_err('',-400,0) RETURN END IF
    MESSAGE ""
    CALL cl_opmsg('u')
    LET g_pie01_t = g_pie.pie01
    LET g_pid_o.*=g_pid.*   
    LET g_pie_o.*=g_pie.*   
    BEGIN WORK
 
    OPEN t827_cl USING g_pie.pie01,g_pie.pie07
    IF STATUS THEN
       CALL cl_err("OPEN t827_cl:", STATUS, 1)
       CLOSE t827_cl
       ROLLBACK WORK
       RETURN
    END IF
    FETCH t827_cl INTO g_pie.*               # 對DB鎖定
    IF SQLCA.sqlcode THEN
        CALL cl_err(g_pie.pie01,SQLCA.sqlcode,0)
        RETURN
    END IF
    CALL t827_show()                          # 顯示最新資料
    WHILE TRUE
        CALL t827_i("u")                      # 欄位更改
        IF INT_FLAG THEN
            LET INT_FLAG = 0
        ...
        LET g_pie.pie40 = g_qty
        UPDATE pie_file SET pie_file.* = g_pie.*    # 更新DB
            WHERE pie01 = g_pie_o.pie01 AND pie07 = g_pie_o.pie07             # COLAUTH?
        IF SQLCA.sqlcode THEN
#          CALL cl_err(g_pie.pie01,SQLCA.sqlcode,0) #No.FUN-660156
           CALL cl_err3("upd","pie_file",g_pie_t.pie01,"",SQLCA.sqlcode,"","",1)  #No.FUN-660156
           CONTINUE WHILE
