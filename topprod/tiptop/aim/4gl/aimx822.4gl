...
# Modify.........: No:FUN-D40129 13/05/07 By yangtt 添加eca02,ecd02欄位顯示
...
MAIN
   ...
   LET g_sql = " order1.ima_file.ima01,",
               ...
               " ecd02.ecd_file.ecd02,",   #FUN-D40129
               " pie04.pie_file.pie04,",
               " pie05.pie_file.pie05,",
               " eca02.eca_file.eca02,",   #FUN-D40129
               ...
               "l_num.type_file.num5"         #FUN-CB0003  add
    LET l_table = cl_prt_temptable('aimx822',g_sql) CLIPPED  #產生Temp Table
   IF l_table = -1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?,?,?,?,?,?, ",
               "        ?,?,?,?,?,?,?,?,?,?, ",
               "        ?,?,?,?,?,?,?,?,?,  ?,?,?,?,?)   "   #FUN-A60092 add ? ? #FUN-CB0003  add ?   #FUN-D40129 add 2?
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL x822_tm(0,0)        # Input print condition
      ELSE CALL x822()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
END MAIN
 
FUNCTION x822_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,    #No.FUN-690026 SMALLINT
   ...
   OPEN WINDOW x822_w AT p_row,p_col
        WITH FORM "aim/42f/aimx822" 
################################################################################
...
#FUN-CB0003--add--str--
      ON ACTION CONTROLP
         CASE
            ...
            WHEN INFIELD (pie03)
               CALL cl_init_qry_var()
               LET g_qryparam.form = "q_pie03"
               LET g_qryparam.state = 'c'
               CALL cl_create_qry() RETURNING g_qryparam.multiret
               DISPLAY g_qryparam.multiret TO pie03
               NEXT FIELD pie03
            ...
            WHEN INFIELD (pie31)
               ...
               LET g_qryparam.state = 'c'
               CALL cl_create_qry() RETURNING g_qryparam.multiret
               DISPLAY g_qryparam.multiret TO pie31
               NEXT FIELD pie31
            ...
            WHEN INFIELD (pie41)
               ...
               LET g_qryparam.state = 'c'
               CALL cl_create_qry() RETURNING g_qryparam.multiret
               DISPLAY g_qryparam.multiret TO pie41
               NEXT FIELD pie41
...
#FUN-CB0003--add--end--
 ...
 LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW x822_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimx822',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW x822_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
DEFINE l_eca02 LIKE eca_file.eca02   #FUN-D40129
DEFINE l_ecd02 LIKE ecd_file.ecd02   #FUN-D40129
     ...
     IF tm.diff ='N' THEN 
        LET l_sql = l_sql clipped," AND (pie30 != pie40 OR ",
                              ...
                              " pie40 IS NULL  )"
     END IF
     PREPARE x822_prepare1 FROM l_sql
     ...
     END IF
     DECLARE x822_curs1 CURSOR FOR x822_prepare1
 
     LET l_name = 'aimx822.out'
    #CALL cl_outnam('aimx822') RETURNING l_name    #FUN-710080 mark
    #CALL x822_getzaa()                            #TQC-740065 mark #FUN-710080 add
     #START REPORT x822_rep TO l_name #Mod MOD-720046 By TSD.hoho CR11
     FOR g_i = 1 TO 80 LET g_q_point[g_i,g_i] = '?' END FOR
 
     LET g_pageno = 0
     FOREACH x822_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN 
       ...
       #OUTPUT TO REPORT x822_rep(sr.*) #Mod MOD-720046 By TSD.hoho CR11
...
#FUN-CB0003--add--str---
       LET l_num = 3
       IF sr.pie30 IS NULL THEN 
          LET sr.pie30 = '???????????????'
       ELSE
          LET sr.pie30 = sr.pie30
       END IF
       IF sr.pie40 IS NULL THEN 
          LET sr.pie40 = '???????????????'
       ELSE
          LET sr.pie40 = sr.pie40
       END IF
#FUN-CB0003--add---end---
       SELECT eca02 INTO l_eca02 FROM eca_file WHERE eca01 = sr.pie05  #FUN-D40129
       SELECT ecd02 INTO l_ecd02 FROM ecd_file WHERE ecd01 = sr.pie03  #FUN-D40129
       EXECUTE insert_prep USING
              sr.order1,sr.order2,sr.order3,sr.pid01, sr.pid02,
              sr.pid03, sr.pie02, sr.ima02, sr.ima021,sr.pie03,l_ecd02,  #FUN-D40129 add l_ecd02 
              sr.pie04, sr.pie05, l_eca02, sr.pie30, sr.pie31, sr.pie34,   #FUN-D40129 add l_eca02
             ...
             ,sr.pie012,sr.pie013,l_num           #FUN-A60092	 add  #FUN-CB0003 add l_num
...
###XtraGrid###     LET g_str = g_str,";",tm.t[1,1],";",tm.t[2,2],";",tm.t[3,3],";",tm.s        #TQC-740065 mod    

   IF g_sma.sma541 = 'N' THEN                           #FUN-A60092 add
###XtraGrid###     CALL cl_prt_cs3('aimx822','aimx822',l_sql,g_str)   #FUN-710080 modify     
    LET g_xgrid.table = l_table    ###XtraGrid###
  #FUN-CB0003--start---
    LET g_xgrid.template = 'aimx822'
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"pid01,pid02,pid03,pie02,pie05,pie03,pie31,pie41")
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"pid01,pid02,pid03,pie02,pie05,pie03,pie31,pie41")  #FUN-D30053
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"pid01,pid02,pid03,pie02,pie05,pie03,pie31,pie41")
   #LET l_str = cl_wcchp(g_xgrid.order_field,"pid01,pid02,pid03,pie02,pie05,pie03,pie31,pie41")  #FUN-D30070  mar:
  ...
  #FUN-CB0003---end---
   ...
   ELSE                                                 #FUN-A60092
###XtraGrid###     CALL cl_prt_cs3('aimx822','aimx822_1',l_sql,g_str) #FUN-A60092         
    LET g_xgrid.table = l_table    ###XtraGrid###
  #FUN-CB0003--start---
    LET g_xgrid.template = 'aimx822_1'
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"pid01,pid02,pid03,pie02,pie05,pie03,pie31,pie41")
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"pid01,pid02,pid03,pie02,pie05,pie03,pie31,pie41")
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"pid01,pid02,pid03,pie02,pie05,pie03,pie31,pie41")
    LET l_str = cl_wcchp(g_xgrid.order_field,"pid01,pid02,pid03,pie02,pie05,pie03,pie31,pie41")
  ...
  #FUN-CB0003---end---
   ...
   END IF                                               #FUN-A60092 
     ...
     #FINISH REPORT x822_rep #Mod MOD-720046 By TSD.hoho CR11
