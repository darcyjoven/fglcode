...
DEFINE g_ima940s    LIKE ima_file.ima940
DEFINE g_ima941s    LIKE ima_file.ima941
DEFINE g_ima_slk    DYNAMIC ARRAY OF RECORD
             agc01    LIKE agc_file.agc01,
             agc02    LIKE agc_file.agc02,
             agc07    LIKE agc_file.agc07,
             agd02    LIKE agd_file.agd02,
             agd03    LIKE agd_file.agd03
                    END RECORD
DEFINE g_ima_slk_t  RECORD
             agc01     LIKE agc_file.agc01,
             agc02     LIKE agc_file.agc02,
             agc07     LIKE agc_file.agc07,
             agd02     LIKE agd_file.agd02,
             agd03     LIKE agd_file.agd03
                     END RECORD
DEFINE g_n        LIKE type_file.num5        #No.FUN-B90101 標記母料件產生的子料件的筆數
#No.FUN-A50011 ------- end -------
#FUN-B90103---------------add-------------
DEFINE g_att                DYNAMIC ARRAY OF RECORD
                       agb03 LIKE agb_file.agb03
                     END RECORD
DEFINE      g_count   LIKE type_file.num5    #屬性群組對應的長度
DEFINE      g_sum     LIKE type_file.num5    #生成子料件成功的數量 
DEFINE      g_sum2    LIKE type_file.num5
DEFINE      g_xb      LIKE type_file.num5    #屬性群組對應的長度的下標 
#FUN-B90103---------------end-------------
MAIN
DEFINE l_sma120      LIKE sma_file.sma120    #No.FUN-810016
   ...
   LET g_prog = "aimi100_slk"   #No.FUN-810016
   UPDATE zz_file SET zz13='N' WHERE zz01=g_prog  #FUN-B90101
   ...
   IF g_sma.sma94 = 'Y' THEN  
      CALL cl_set_comp_visible("ima928,ima929,gb13",TRUE)
      CALL cl_set_act_visible("feature_maintain",TRUE)
   ELSE    
   #FUN-C60046---end
      CALL cl_set_comp_visible("ima928,ima929,gb13",FALSE) #TQC-C40248--ADD--
      CALL cl_set_act_visible("feature_maintain",FALSE)              #TQC-C40248--ADD--
   END IF #FUN-C60046
   ...
   END IF
    ...
    SELECT sma120 INTO l_sma120 FROM sma_file
          CALL cl_set_act_visible("style_informate",TRUE)
          CALL cl_set_act_visible("query_attributevalue,produce_sub_parts,maintain_color,maintain_size",TRUE)    #No.FUN-A50011
...
#No.FUN-A50011 ------begin------
            ...
            CALL cl_set_comp_visible("ima940,ima941,agc021,agc022",TRUE)    #No.FUN-A50011
            CALL cl_set_comp_entry("ima940,ima941",TRUE)     #No.FUN-A50011
         BEFORE FIELD ima151
...
#No.FUN-A50011 -------end------
	    ...
	    ELSE
               ...
               CALL cl_set_comp_visible("imaag",FALSE)  #FUN-CA0092--TRUE---FALSE
         #     CALL cl_set_comp_visible("ima940,ima941,agc021,agc022",FALSE)   #No.FUN-A50011
         #     CALL cl_set_comp_entry("ima940,ima941",FALSE)    #No.FUN-A50011
         #     LET g_ima.ima940 = NULL                          #No.FUN-A50011
         #     LET g_ima.ima941 = NULL                          #No.FUN-A50011
            END IF
#No.FUN-A50011 ----begin------
      {   AFTER FIELD ima940
            IF g_ima.ima940 IS NOT NULL THEN 
               SELECT count(*) INTO l_n FROM agc_file WHERE agc01 = g_ima.ima940 AND agc07 = '1'
	       IF l_n = 0 THEN
         ...
         AFTER FIELD ima941
            IF g_ima.ima941 IS NOT NULL THEN 
               SELECT count(*) INTO l_n FROM agc_file WHERE agc01 = g_ima.ima941 AND agc07 = '2'
	       IF l_n = 0 THEN
...
#&endif
#FUN-B30192---mark
      ON ACTION style_informate
         LET g_action_choice="style_informate"
         IF cl_chk_act_auth() THEN
            CALL i100_style_informate()
            CALL i100_show()
         END IF
#No.FUN-A50011 -----begin-----
      ON ACTION query_attributevalue
         LET g_action_choice = "query_attributevalue"
	 IF cl_chk_act_auth() THEN
	    CALL i100_query_attributevalue()
	 END IF
#FUN-C60021----ADD----STR-------
      ON ACTION add_multi_attr_sub
         IF cl_null(g_ima.ima01) THEN
...
#FUN-C60021----ADD----END-------
      ON ACTION produce_sub_parts
         ...
         ELSE                                 
             ...
             END IF
         END IF                                   
      ON ACTION maintain_color
         LET g_action_choice = "maintain_color"
         IF cl_chk_act_auth() THEN
            CALL i100_maintain_color()
         END IF
      ON ACTION maintain_size
         LET g_action_choice = "maintain_size"
         IF cl_chk_act_auth() THEN
            CALL i100_maintain_size()
         END IF
#No.FUN-A50011 -----end-----       
      ON IDLE g_idle_seconds
         CALL cl_on_idle()
...
FUNCTION i100_a()
   ...
   WHILE TRUE
      ...
      CALL cl_set_comp_visible("imaag",FALSE)    # No.FUN-810016
    # CALL cl_set_comp_visible("imaag",TRUE)     #FUN-CA0092--mark    
    # CALL cl_set_comp_visible("ima940,ima941,agc021,agc022",FALSE)   # No.FUN-A50011
...
#No.FUN-D40110 ---------- add ----------- end ----------------------
       ...
       #DEV-D40019 add end------
        ...
        ON CHANGE ima151
          ...
          END IF
          #TQC-C50222--add--end--
          IF  g_ima.ima151 = "N" THEN    #FUN-CA0092-add 
              CALL cl_set_comp_entry("ima940,ima941,imaag",FALSE)    #FUN-CA0092-add
              CALL cl_set_comp_visible("imaag",FALSE)  #FUN-CA0092-add
              CALL cl_set_comp_visible("ima940,ima941,agc021,agc022",FALSE)   #FUN-CA0092-add
          ELSE   #FUN-CA0092-add
              CALL cl_set_comp_visible("imaag",TRUE)
              CALL cl_set_comp_visible("ima940,ima941,agc021,agc022",TRUE) 
          END IF #FUN-CA0092-add
#MOD-C30694------ADD-------STR------
          IF g_ima.ima151="Y" THEN
              CALL cl_set_act_visible("style_informate",TRUE)
              CALL cl_set_comp_visible("ima940,ima941,agc021,agc022",TRUE) 
              IF p_cmd = 'a' OR (p_cmd = 'u' AND g_n =0 ) THEN              #  更改狀態下，若已產生子料件，則不開啟
                 CALL cl_set_comp_entry("ima940,ima941",TRUE)
                 #TQC-C50222--add--str--
                 IF NOT cl_null(g_ima_t.ima940) AND cl_null(g_ima.ima940) THEN
                    LET g_ima.ima940 = g_ima_t.ima940
                 END IF 
                 IF NOT cl_null(g_ima_t.ima941) AND cl_null(g_ima.ima941) THEN 
                    LET g_ima.ima941 = g_ima_t.ima941
                 END IF  
                 DISPLAY BY NAME g_ima.ima940,g_ima.ima941                 
                 #TQC-C50222--add--end--
              END IF
...
#              CALL cl_set_act_visible("style_informate",TRUE)
#              CALL cl_set_comp_visible("ima940,ima941,agc021,agc022",TRUE) #No.FUN-A50011
#              IF p_cmd = 'a' OR (p_cmd = 'u' AND g_n =0 ) THEN              #No.FUN-B90101  更改狀態下，若已產生子料件，則不開啟  
#                 CALL cl_set_comp_entry("ima940,ima941",TRUE)               #No.FUN-A50011
#              END IF                                                        #No.FUN-B90101 
#              CALL cl_set_comp_entry("imaag",FALSE)                         #No.FUN-A50011
...
#              CALL cl_set_act_visible("style_informate",FALSE)
##No.FUN-A50011  -----begin-----	      
#          #   CALL cl_set_comp_visible("ima940,ima941,agc021,agc022",FALSE)
#              CALL cl_set_comp_entry("ima940",FALSE)
#	      CALL cl_set_comp_entry("ima941",FALSE)
#	      LET g_ima.ima940 = NULL
#	      LET g_ima.ima941 = NULL
...
##            IF l_n >0 AND NOT cl_null(l_n) THEN
##               CALL cl_err('','mfg-999',1)
##            END IF
##TQC-C30171-----end------------------------
##          NEXT FIELD ima940                                                #No.FUN-A50011 #TQC-C30171--mark 
#MOD-C30694------MARK------END--------------------------------

#No.FUN-A50011 -----begin-----
	AFTER FIELD ima940
          IF g_azw.azw04 <> '1' THEN    #FUN-B90103--add--
           IF g_ima.ima940 IS NOT NULL THEN 
              IF p_cmd = "a" OR
                  (p_cmd = "u" AND (g_ima.ima940 != g_ima_t.ima940  OR g_ima_t.ima940 IS NULL)) THEN   #FUN-C20006 add 舊值為空的判斷
                  CALL i100_ima940('d')
	   ...
	   ELSE 
        ...
        AFTER FIELD ima941
          IF g_azw.azw04 <> '1' THEN    #FUN-B90103--add--
           IF g_ima.ima941 IS NOT NULL THEN 
              IF p_cmd = "a" OR
                  (p_cmd = "u" AND (g_ima.ima941 != g_ima_t.ima941 OR g_ima_t.ima941 IS NULL)) THEN    #FUN-C20006 add 舊值為空的判斷
                  CALL i100_ima941('d')
                  IF NOT cl_null(g_errno) THEN
...
#No.FUN-A50011  -----begin----
            IF g_azw.azw04 <> '1' THEN   #FUN-B90103--add---
               IF (NOT cl_null(g_ima.ima940)) AND (NOT cl_null(g_ima.ima941)) THEN   #TQC-C20117  add
                  IF g_sma.sma120 = 'Y' THEN                                         #TQC-C20117  add
                     LET g_ima.imaag = g_ima.ima940,g_sma.sma46,g_ima.ima941         #TQC-C20117  add
                    #LET g_ima.imaag = g_ima.ima940,"-",g_ima.ima941                    #TQC-C20117  mark
                     DISPLAY BY NAME g_ima.imaag
               ...
               END IF    #TQC-C20117  add
               IF g_ima.ima151 = 'Y' THEN
                  CALL cl_set_act_visible("query_attributevalue,produce_sub_parts,maintain_color,maintain_size",TRUE)
               ELSE
                  CALL cl_set_act_visible("query_attributevalue,produce_sub_parts,maintain_color,maintain_size",FALSE)
               END IF
             END IF    #FUN-B90103--add
#No.FUN-A50011 -----end----- 
...
#FUN-B90103-------modify--------
#       SELECT ima940,ima941 INTO g_ima940s,g_ima941s FROM  ima_file WHERE ima01 IN (SELECT imaag1 FROM ima_file WHERE ima01 = g_ima.ima01)      #No.FUN-A50011
        LET g_ima940s = ''  #MOD-CB0082 add
        LET g_ima941s = ''  #MOD-CB0082 add
        IF g_ima.ima151= 'N' THEN
          #SELECT ima940,ima941  g_ima940s,g_ima941s FROM imx_file,ima_file WHERE imx000 = g_ima.ima01 AND imx00=ima01
           SELECT ima940,ima941 INTO g_ima940s,g_ima941s FROM imx_file,ima_file WHERE imx000 = g_ima.ima01 AND imx00=ima01 #MOD-CB0082 add INTO
        ELSE
           SELECT ima940,ima941 INTO g_ima940s,g_ima941s FROM ima_file WHERE ima01 = g_ima.ima01 
        END IF
#FUN-B90103------end------------
        CALL i100_show()                      # 重新顯示
    END IF
...
#  LET g_d2=g_ima.ima262-g_ima.ima26      #FUN-A20044
   ...
   ELSE
     CALL cl_set_act_visible("style_informate",FALSE)
     CALL cl_set_act_visible("query_attributevalue,produce_sub_parts,maintain_color,maintain_size",FALSE) #No.FUN-A50011
     CALL cl_set_comp_visible("ima940,ima941,agc021,agc022",FALSE) #No.FUN-A50011  #FUN-CA0092--remark
     CALL cl_set_comp_visible("imaag",FALSE)
     CALL cl_set_comp_visible("imaag",FALSE)                 #No.FUN-A50011 #FUN-CA0092--TRUE-->FALSE
   END IF
...
#No.FUN-A50011 -----begin-----
   ...
   END IF
   IF g_ima.ima151 = 'N' THEN
      SELECT agd03 INTO l_agd031 FROM agd_file WHERE agd02 = g_ima.ima940 AND agd01 = g_ima940s
      DISPLAY l_agd031 TO FORMONLY.agc021
      SELECT agd03 INTO l_agd032 FROM agd_file WHERE agd02 = g_ima.ima941 AND agd01 = g_ima941s
      DISPLAY l_agd032 TO FORMONLY.agc022
   END IF
#No.FUN-A50011 -----end-----
...
#FUN-B90101--add--end--
    ...
    WHILE TRUE
       ...
       IF g_ima.ima151 = "N" THEN    #FUN-CA0092-add
          CALL cl_set_comp_visible("ima940,ima941,agc021,agc022,imaag",FALSE) #FUN-CA0092-add
       ELSE                                                                   #FUN-CA0092-add
          CALL cl_set_comp_visible("ima940,ima941,agc021,agc022,imaag",TRUE)   #No.FUN-A50011
       END IF            #FUN-CA0092-add
       CALL cl_set_comp_entry("imaag",FALSE)                                #FUN-B90103--add
...
FUNCTION i100_set_entry(p_cmd)
   ...
   IF p_cmd = 'a' AND ( NOT g_before_input_done ) THEN        #No.FUN-550021
      ...
      IF g_ima.ima151 = "N" THEN                 #FUN-CA0092--add   
         CALL cl_set_comp_visible("imaag",FALSE) #FUN-CA0092--add
      END IF    #FUN-CA0092--add
      CALL cl_set_comp_entry("ima940",TRUE)                   #No.FUN-A50011
      CALL cl_set_comp_entry("ima941",TRUE)                   #No.FUN-A50011
   ...
   IF g_azw.azw04 = '2' THEN
      IF p_cmd = 'u' AND ( NOT g_before_input_done )  THEN
         ...
         IF l_ima151 = 'N' AND l_imaag = '@CHILD' THEN
            CALL cl_set_comp_entry("ima02,ima151,ima940,ima941,ima25,ima35,ima36",FALSE)
         END IF
      END IF
   END IF    
   IF g_ima.ima151 = "N" THEN                 #FUN-CA0092--add
      CALL cl_set_comp_visible("imaag,ima940,ima941,agc021,agc022",FALSE) #FUN-CA0092--add
   END IF    #FUN-CA0092--add
#MOD-C30270--add--end--
  #FUN-C90075---begin
  SELECT gev04 INTO g_gev04 FROM gev_file
...
FUNCTION i100_style_informate()
  DEFINE i,j   LIKE type_file.num5,
         l_ac  LIKE type_file.num5,
         l_lock_sw  LIKE type_file.chr1
  DEFINE l_imt DYNAMIC ARRAY OF RECORD
               imt02 LIKE imt_file.imt02,
               imt03 LIKE imt_file.imt03
               END RECORD,
         l_imt_t RECORD
               imt02 LIKE imt_file.imt02,
               imt03 LIKE imt_file.imt03
  ...
  OPEN WINDOW i100_a_w AT 4,3 WITH FORM"aim/42f/aimi100a"
        ATTRIBUTE(STYLE=g_win_style CLIPPED)
   CALL cl_ui_locale("aimi100a")
  
    
   
   DECLARE i100_a_c CURSOR FOR
     SELECT imt02,imt03 FROM imt_file
       WHERE imt01=g_ima.ima01
 
   LET i=1
   FOREACH i100_a_c INTO l_imt[i].*
     LET i=i+1
   END FOREACH
   CALL l_imt.deleteElement(i)
   LET i = i-1 
   ...
   WHILE TRUE
   LET l_act =0
   DISPLAY ARRAY l_imt TO s_imt.* ATTRIBUTE (COUNT=g_i ,UNBUFFERED)
   ...
   END IF   
   IF l_act =2 THEN 
      CALL i100_style_informate_b(l_imt,l_ac) 
   END IF    
  END WHILE   
   CLOSE WINDOW i100_a_w

#FUN-B90101----add--begin---- 服飾行業，母料件更改，更新子料件資料
   IF g_ima.ima151='Y' THEN
      CALL i100_ins_imt()
   END IF
#FUN-B90101----add--end---
...
#FUN-B90101----add--begin---- 服飾行業，母料件更改后修改，更新子料件資料
FUNCTION i100_ins_imt()
 DEFINE l_imx000 LIKE imx_file.imx000
 DEFINE l_sql    STRING
 DEFINE l_imt    RECORD LIKE imt_file.*

   LET l_sql="SELECT * FROM imt_file WHERE imt01='",g_ima.ima01,"'"
   PREPARE imt_ins FROM l_sql
   DECLARE imt_ins_upd CURSOR FOR imt_ins

   FOREACH imt_ins_upd INTO l_imt.*
      DELETE FROM imt_file WHERE imt01 IN (SELECT imx000 FROM imx_file WHERE imx00=l_imt.imt01)
      IF SQLCA.sqlcode THEN
         CALL cl_err3("del","imt_file",l_imt.imt01,"",SQLCA.sqlcode,"","",1)
         CONTINUE FOREACH
      END IF

      LET l_sql="SELECT imx000 FROM imx_file WHERE imx00='",l_imt.imt01,"'"
      PREPARE imt_ins1 FROM l_sql
      DECLARE imt_ins1_upd CURSOR FOR imt_ins1

      FOREACH imt_ins1_upd INTO l_imx000
         LET l_imt.imt01=l_imx000
         INSERT INTO imt_file VALUES(l_imt.*)
         IF SQLCA.sqlcode THEN
            CALL cl_err3("ins","imt_file",l_imt.imt01,"",SQLCA.sqlcode,"","",1)
            CONTINUE FOREACH
         END IF
      END FOREACH
...
FUNCTION i100_style_informate_b(p_imt,p_ac)
 DEFINE i,j   LIKE type_file.num5,
         p_i   LIKE type_file.num5, 
         p_ac  LIKE type_file.num5,
         l_ac  LIKE type_file.num5,
         l_lock_sw  LIKE type_file.chr1
  DEFINE p_imt DYNAMIC ARRAY OF RECORD
               imt02 LIKE imt_file.imt02,
               imt03 LIKE imt_file.imt03
               END RECORD,
         p_imt_t RECORD
   ...
   LET l_forupd_sql = "SELECT imt02,imt03 FROM imt_file ",
                      " WHERE imt01 = ? AND imt02 = ?  FOR UPDATE "
   LET l_forupd_sql = cl_forupd_sql(l_forupd_sql)

   DECLARE i100_s_cl CURSOR FROM l_forupd_sql
 
   LET l_allow_insert=cl_detail_input_auth("insert")
   LET l_allow_delete=cl_detail_input_auth("delete")
 
   INPUT ARRAY p_imt WITHOUT DEFAULTS FROM s_imt.*
   ...
   BEFORE INPUT
     ...
     BEFORE ROW
       ...
       IF g_i>=l_ac THEN
        LET p_imt_t.* = p_imt[l_ac].*
        OPEN i100_s_cl USING g_ima.ima01,p_imt_t.imt02
        IF STATUS THEN
          CALL cl_err("OPEN:",STATUS,1)
          LET l_lock_sw ='Y'
        ELSE
    	    FETCH i100_s_cl INTO p_imt[l_ac].*
    	    IF SQLCA.sqlcode THEN
    	      CALL cl_err(p_imt_t.imt02,SQLCA.sqlcode,1)
    	      LET l_lock_sw ='Y'
    	    END IF
        END IF 	
       END IF
      ...
      AFTER INSERT
       IF INT_FLAG THEN
         ...
         cancel INSERT
       END IF
       INSERT INTO imt_file(imt01,imt02,imt03)
            VALUES (g_ima.ima01,p_imt[l_ac].imt02,p_imt[l_ac].imt03)
       IF SQLCA.sqlcode THEN
          CALL cl_err(p_imt[l_ac].imt02,SQLCA.sqlcode,0)
      ...
      BEFORE DELETE
       IF p_imt_t.imt02>0 AND p_imt_t.imt02 IS NOT NULL THEN
          ...
          IF l_lock_sw ='Y' THEN
            ...
            CANCEL DELETE
          END IF
          DELETE FROM imt_file
           WHERE imt01 = g_ima.ima01
             AND imt02 = p_imt_t.imt02
          IF SQLCA.sqlcode THEN
            CALL cl_err(p_imt_t.imt02,SQLCA.SQLCODE,0)
            ROLLBACK WORK
            CANCEL DELETE
          END IF
       ...
       ON ROW change
        ...
        ELSE
        	UPDATE imt_file
        	      SET imt02 = p_imt[l_ac].imt02,
        	          imt03 = p_imt[l_ac].imt03
        	    WHERE imt01 = g_ima.ima01
        	      AND imt02 = p_imt_t.imt02
...
#No.FUN-9C0072 精簡程式碼
#No.FUN-A50011 ------begin------
FUNCTION i100_query_attributevalue()
  DEFINE l_sql    STRING
  OPTIONS
     INPUT NO WRAP
  DEFER INTERRUPT
  IF(NOT cl_user()) THEN
     CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B90046 
     EXIT PROGRAM
  END IF
  WHENEVER ERROR CALL cl_err_msg_log
  IF(NOT cl_setup("AIM")) THEN
  ...
  OPEN WINDOW i100_query AT 3,4 WITH FORM "aim/42f/aimi100_slk_a" ATTRIBUTE (STYLE = g_win_style CLIPPED)
  CALL cl_ui_init()
  CALL cl_set_act_visible("accept,cancel",FALSE)
  LET l_sql = "SELECT agc01,agc02,agc07,agd02,agd03 FROM agc_file LEFT JOIN agd_file ON agc01 = agd01",
              " WHERE agc01 ='",g_ima.ima940,"' OR agc01='",g_ima.ima941,"'"
  DECLARE ima_query_cs CURSOR FROM l_sql
  CALL g_ima_slk.clear()
  LET g_cnt = 1
  FOREACH ima_query_cs INTO g_ima_slk[g_cnt].*
     IF STATUS THEN
	...
	EXIT FOREACH
     END IF
  END FOREACH
  CALL g_ima_slk.deleteElement(g_cnt)
  LET g_rec_b = g_cnt - 1
  DISPLAY g_rec_b TO FORMONLY.cnt
  LET g_cnt = 0
  DISPLAY ARRAY g_ima_slk TO s_imaslk.* ATTRIBUTE (COUNT = g_rec_b)  
     BEFORE ROW 
        LET l_ac3 = ARR_CURR()
        CALL cl_show_fld_cont()
     ON ACTION locale
        CALL cl_dynamic_locale()
        CALL cl_show_fld_cont()
     ON ACTION attribute_data
        LET g_action_choice = "attribute_data"
        IF cl_chk_act_auth() THEN
           CALL i100_attribute_data()
        END IF
     ON ACTION exit
        EXIT DISPLAY
     ON ACTION accept
       MESSAGE "THE DATA CANN'T GO IN THE FORM BODY"
  END DISPLAY
  CALL cl_set_act_visible("accept,cancel",TRUE)
  CLOSE WINDOW i100_query
  CALL cl_used(g_prog,g_time,2) RETURNING g_time
END FUNCTION
FUNCTION i100_attribute_data()
  DEFINE l_cmd     LIKE type_file.chr1000
  LET l_cmd = "aimi300 '",g_ima_slk[l_ac3].agc01,"'"
  CALL cl_cmdrun_wait(l_cmd)
END FUNCTION
FUNCTION i100_maintain_color()
  DEFINE l_cmd     LIKE type_file.chr1000
  LET l_cmd = "aimi300 '",g_ima.ima940,"' '",g_ima.ima01,"'"
  CALL cl_cmdrun_wait(l_cmd)
  SELECT * INTO g_ima.* FROM ima_file WHERE ima01 = g_ima.ima01
  CALL i100_show()
END FUNCTION
FUNCTION i100_maintain_size()
  DEFINE l_cmd     LIKE type_file.chr1000
  LET l_cmd = "aimi300 '",g_ima.ima941,"' '",g_ima.ima01,"'"
  CALL cl_cmdrun_wait(l_cmd)
  SELECT * INTO g_ima.* FROM ima_file WHERE ima01 = g_ima.ima01
  CALL i100_show()
END FUNCTION
FUNCTION i100_ima940(p_cmd)
   ...
   IF p_cmd = 'd' OR cl_null(g_errno) THEN
      DISPLAY l_agc02 TO FORMONLY.agc021
   END IF
END FUNCTION 
...
FUNCTION i100_ima941(p_cmd)
   ...
   END CASE
   IF p_cmd = 'd' OR cl_null(g_errno) THEN
      DISPLAY l_agc02 TO FORMONLY.agc022
   END IF
END FUNCTION 
...
FUNCTION i100_produce_sub_parts()
   ...
   DEFINE l_imaag1   LIKE ima_file.imaag1    #FUN-B90103--add
   DEFINE i          LIKE  type_file.num5    #FUN-B90103--add
#  DEFINE l_count    LIKE type_file.num5     #FUN-B90103--add
#FUN-C60021---ADD----STR----
   DEFINE l_imaslk  RECORD LIKE imaslk_file.*
   DEFINE l_n3      LIKE type_file.num5
 
#FUN-C60021---ADD----END----  
  
#  SELECT COUNT(*) INTO l_count FROM agb_file WHERE agb01=g_ima.imaag
#  IF (g_azw.azw04 <> 1) OR (g_azw.azw04 = 1 AND l_count = 2) THEN    #FUN-B90103--add
...
#FUN-B90101--add--end--
      ...
      LET g_ima940s = g_ima.ima940
      LET g_ima941s = g_ima.ima941
      LET l_ima01_t = g_ima.ima01   
      LET l_imaag1  = g_ima.imaag   #add  by FUN-B90103
...
#             LET g_ima.imaag1 = l_ima01_t     #mark by FUN-B90103
              LET g_ima.imaag1 = l_imaag1      #FUN-B90103---add 
              LET g_ima.ima151 = 'N'
              LET g_ima.ima120 = '1'                 #FUN-A90049 add   
              LET g_ima.ima571 = l_ima01_t   #FUN-B90103--add  #MOD-C30165 modify 
              LET g_ima.ima94 = ''           #FUN-B90103--add
              IF g_ima.ima151 = 'N' THEN
                 LET g_ima.ima940 = l_agd021 
#                DISPLAY l_agd031 TO FORMONLY.agc021  #FUN-B90103--mark 
                 LET g_ima.ima941 = l_agd022 
#                DISPLAY l_agd032 TO FORMONLY.agc022  #FUN-B90103--mark
              END IF
...
#                      VALUES(g_ima.ima01,g_ima.imaag1,g_ima.ima940,g_ima.ima941)   #FUN-B90103--mark
                       VALUES(g_ima.ima01,l_ima01_t,g_ima.ima940,g_ima.ima941)      #FUN-B90103--add
                    IF SQLCA.sqlcode THEN
                    ...
                    END IF
#FUN-C60021------ADD------STR-----
                    SELECT COUNT(*) INTO l_n3 FROM imaslk_file WHERE imaslk00=l_ima01_t
                    IF l_n1>0 THEN
                       SELECT * INTO l_imaslk.* FROM imaslk_file WHERE imaslk00=l_ima01_t
                       LET l_imaslk.imaslk00=g_ima.ima01
                       LET l_imaslk.imaslk11=l_agd021
...
#     SELECT * INTO g_ima.* FROM ima_file WHERE ima01 = g_ima.ima01
#     CALL g_att.clear()  
#     DECLARE i100_agb03 CURSOR FOR
#        SELECT agb03 FROM agb_file WHERE agb01=g_ima.imaag
#     LET i=1
#     FOREACH i100_agb03 INTO g_att[i].agb03
#        LET i = i+1 
#     END FOREACH
#     CALL g_att.deleteElement(i)  
#     LET g_count = g_att.getLength()
#     LET g_xb = 0
#     LET g_sum = 0
#     LET g_sum2 = 0
#     CALL i100_attributes(g_xb,g_ima.ima01,g_ima.ima02)   #下標值和母料件編號
#     CALL l_confirm2('aim1104','aim1105',g_sum,g_sum2)
#   END IF
#FUN-B90103---------end----------------------
...
#     ON ACTION confirm   #FUN-B90103--mark
      ON ACTION accept    #FUN-B90103--add
         EXIT MENU
      ON IDLE g_idle_seconds
         CALL cl_on_idle()
         CONTINUE MENU
...
END FUNCTION
{
FUNCTION i100_update_sub_parts()
   DEFINE l_sql1    STRING
   DEFINE l_sql2    STRING
   DEFINE l_msg     STRING
   DEFINE l_agd021  LIKE  agd_file.agd02
   ...
   ELSE
        WHILE(l_cmd1<=l_n1)
          IF l_cmd1 =1 THEN
             OPEN i100_upd_cs1
             FETCH ABSOLUTE 1 i100_upd_cs1 INTO l_agd021,l_agd031
          ELSE 
             FETCH NEXT i100_upd_cs1 INTO l_agd021,l_agd031
          END IF  
          WHILE(l_cmd2<=l_n2)
            IF l_cmd2 = 1 THEN
               OPEN i100_upd_cs2 
               FETCH ABSOLUTE 1 i100_upd_cs2 INTO l_agd022,l_agd032
            ELSE 
               FETCH NEXT i100_upd_cs2 INTO l_agd022,l_agd032
            END IF
            LET g_ima.ima01 = l_ima01_t,"-",l_agd021,"-",l_agd022
            LET g_ima.ima02 = l_ima02_t,"-",l_agd031,"-",l_agd032 
            LET g_ima.imaag = '@CHILD'
            LET g_ima.imaag1 = l_ima01_t
            LET g_ima.ima151 = 'N'
            IF g_ima.ima151 = 'N' THEN
               LET g_ima.ima940 = l_agd021 
               LET g_ima.ima941 = l_agd022 
...
#FUN-C60021-----ADD---END-----
     ...
     LET l_imx01=NULL
     LET l_imx02=NULL
     #LET l_ima571=NULL  #MOD-C30165 mark
     LET l_ima94=NULL
     LET l_imaag=NULL
     ...
     FOREACH i100_upd INTO l_imx000,l_imx01,l_imx02                    
        IF SQLCA.sqlcode  THEN
            EXIT FOREACH
        END IF
        #LET l_ima571= l_imx000   #MOD-C30165mark
        LET l_ima94 = ''
#FUN-C30158-----ADD----STR------
        IF g_ima.ima928='Y' THEN
           UPDATE ima_file SET ima928='N',ima929=g_ima.ima01,
                               imadate = g_today     #FUN-C30315 add
            WHERE ima01=l_imx000
        END IF
#FUN-C30158-----ADD----END------
        #UPDATE ima_file SET ima940=l_imx01,ima941=l_imx02,ima571=l_imx000,ima94='',imaag1=l_imaag #MOD-C30165 mark
        UPDATE ima_file SET ima940=l_imx01,ima941=l_imx02,ima94='',imaag1=l_imaag,                  #MOD-C30165 delete ima571
                            imadate = g_today     #FUN-C30315 add
          WHERE ima01=l_imx000 
        IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
           EXIT FOREACH
        END IF
#FUN-C60021-----ADD---STR---
        SELECT COUNT(*) INTO l_n1 FROM imaslk_file WHERE imaslk00=g_ima.ima01
        IF l_n1>0 THEN
           SELECT * INTO l_imaslk.* FROM imaslk_file WHERE imaslk00=g_ima.ima01
           LET l_imaslk.imaslk00=l_imx000
           LET l_imaslk.imaslk11=l_imx01
           INSERT INTO imaslk_file VALUES(l_imaslk.*)   
...
FUNCTION i100_imaag()
  SELECT agb03 INTO g_ima.ima940 FROM agb_file WHERE agb01=g_ima.imaag AND agb02 = 1
  SELECT agb03 INTO g_ima.ima941 FROM agb_file WHERE agb01=g_ima.imaag AND agb02 = 2 
END FUNCTION

#業態為製造業時處理子料件的方式
#FUNCTION i100_attributes(p_j,p_ima01,p_ima02)
# DEFINE l_sql    STRING
...
#   LET l_agd03 = NULL
#   SELECT sma46 INTO l_ps FROM sma_file 
#   LET l_sql = "SELECT agd02,agd03 FROM agd_file WHERE agd01 ='",g_att[p_j].agb03,"'"
#   DECLARE i100_attribute CURSOR  FROM l_sql
#   FOREACH i100_attribute INTO l_agd02,l_agd03
#      SELECT agd01 INTO l_agd01 FROM agd_file WHERE agd02 = l_agd02 
#      IF p_j=g_count AND l_agd01 = g_att[1].agb03 AND g_sum <> 0 THEN
#         LET p_j=1
#         LET p_ima01=g_ima.ima01
#         LET p_ima02=g_ima.ima02
#      END IF 

#      IF p_j<g_count THEN
#        LET p_ima01=p_ima01,l_ps,l_agd02
#        LET p_ima02=p_ima02,l_ps,l_agd03
#      END IF

#      IF p_j=g_count THEN
#         LET l_ima01=p_ima01,l_ps,l_agd02
#         LET l_ima02=p_ima02,l_ps,l_agd03

#         LET l_ima.*=g_ima.*
#         LET l_ima.ima01=l_ima01
#         LET l_ima.ima02=l_ima02
#         LET l_ima.imaag = '@CHILD'
#         LET l_ima.imaag1= g_ima.imaag
#         LET l_ima.ima151 = 'N'
#         LET l_ima.ima120 = '1'
...
#         LET l_ima.ima940=''
#         LET l_ima.ima941=''
#        SELECT count(*) INTO l_n FROM ima_file WHERE ima01 = l_ima.ima01
#        IF l_n = 0 THEN
#           IF cl_null(g_ima.ima156) THEN LET l_ima.ima156 ='N' END IF 
#           IF cl_null(g_ima.ima157) THEN LET l_ima.ima157 =' ' END IF
#           IF cl_null(g_ima.ima158) THEN LET l_ima.ima158 ='N' END IF 
#           LET l_ima.ima927='N' 
#           IF cl_null(g_ima.ima912) THEN LET l_ima.ima912 = 0 END IF  
#           IF cl_null(g_ima.ima159) THEN LET l_ima.ima159 = '3' END IF
#           
#              INSERT INTO ima_file VALUES(l_ima.*)
#              IF SQLCA.sqlcode THEN
...
#              ELSE
#                 INSERT INTO imx_file(imx000,imx00,imx01,imx02)
#                   VALUES(l_ima.ima01,g_ima.ima01,'','')
#                 IF SQLCA.sqlcode THEN
#                    CALL cl_err3("ins","imx_file",g_ima.ima01,"",SQLCA.sqlcode,"","",1)
...
#        CONTINUE FOREACH 
#      END IF
#      CALL i100_attributes(p_j,p_ima01,p_ima02)
#   END FOREACH
#END FUNCTION
#FUN-B90103------------end-------------------------
#FUN-B50046
