...
#TQC-AC0207 ------------mod end----------------
  #TQC-610072-end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'	# If background job sw is off
      ...
      ELSE CALL r801()		# Read data and create out-file
   END IF
...
DEFINE li_result       LIKE type_file.num5     #CHI-960065
   ...
   OPEN WINDOW r801_w AT p_row,p_col WITH FORM "aim/42f/aimr801"
...
##            PREPARE r801_pre FROM l_sql
##            DECLARE r801_cur1 CURSOR FOR r801_pre
##            OPEN r801_cur1
##            FETCH r801_cur1 INTO l_cnt
##           #end FUN-5A0199
##            IF l_cnt = 0 THEN
...
#TQC-AC0207 -------------------mark end-------------------------
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimr801',g_time,l_cmd)
      END IF
      CLOSE WINDOW r801_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8	    #No.FUN-6A0074
          ...
          l_order       ARRAY[5] OF LIKE pid_file.pid01, #No.FUN-690026 VARCHAR(20)
...
#No.TQC-C20047 ----- mark ----- end
     ...
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aimr801'
    ...
    #LET l_wipeno = tm.wip CLIPPED,'-',tm.eno CLIPPED    #TQC-AC0207 mark
     ...
     END IF
     PREPARE r801_prepare FROM l_sql
     ...
     END IF
     DECLARE r801_cs CURSOR FOR r801_prepare
#     CALL cl_outnam('aimr801') RETURNING l_name            #NO.FUN-870060
     LET l_sql = " UPDATE pid_file ",
                 " SET pid12=pid12+1,",
                 "     pid11='",g_today,"' " ,
                 " WHERE pid01=? "
     PREPARE r801_up FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
        EXIT PROGRAM
...
#        START REPORT r801_rep3 TO l_name                   #NO.FUN-870060
         LET l_name = 'aimr801_2'                           #NO.FUN-870060
     ELSE
         IF tm.t = 'Y' THEN
#            START REPORT r801_rep2 TO l_name                #NO.FUN-870060
             LET l_name = 'aimr801_1'                        #NO.FUN-870060
         ELSE
#            START REPORT r801_rep1 TO l_name                #NO.FUN-870060
             LET l_name = 'aimr801'                          #NO.FUN-870060
         END IF
     END IF
#     LET g_pageno = 0                                      #NO.FUN-870060
 
     FOREACH r801_cs INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
       END IF

       #更新列印次數及列印日期
       EXECUTE r801_up  USING sr.pid01
...
#       #無使用製程
#      IF g_sma.sma54='N' THEN
#           OUTPUT TO REPORT r801_rep3(sr.*)
#      ELSE IF tm.t = 'Y' THEN
#              OUTPUT TO REPORT r801_rep2(sr.*)
#           ELSE
#              OUTPUT TO REPORT r801_rep1(sr.*)
#           END IF
#      END IF
#NO.FUN-870060--end---mark----
        EXECUTE insert_prep USING
          sr.pid01,sr.pid02,sr.pid03,sr.ima02_d,sr.pie02,sr.pie03,
...
#NO.FUN-870060--start---mark----
#    IF g_sma.sma54='N' THEN
#         FINISH REPORT r801_rep3
#    ELSE IF tm.t = 'Y' THEN
#            FINISH REPORT r801_rep2
#         ELSE
#            FINISH REPORT r801_rep1
#         END IF
#    END IF
...
#NO.FUN-870060----start----
     ...
     LET g_str = tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",tm.size,";",
                 tm.code,";",tm.yearstr,";",tm.a,";",tm.b,";",tm.c
                ,";",g_sma.sma541        #FUN-A60092 add 

#No.TQC-C20047 ----- mark ----- begin
#  LET g_cr_table = l_table                 #主報表的temp table名稱        #No.TQC-C10034 add
#  LET g_cr_apr_key_f = "pid01|pie07"             #報表主鍵欄位名稱，用"|"隔開   #No.TQC-C10034 add
#No.TQC-C20047 ----- mark ----- end
     CALL cl_prt_cs3('aimr801',l_name,g_sql,g_str) 
#NO.FUN-870060--end---
END FUNCTION
