...
# Modify.........: No:FUN-D40128 13/05/07 By wangrr pid03,pie02,sfb82三欄位增加開窗
...
#No.FUN-7B0138--START--
   LET g_sql = "pid01.pid_file.pid01,",
    ...
    LET l_table = cl_prt_temptable('aimx835',g_sql) CLIPPED
   IF l_table =-1 THEN EXIT PROGRAM END IF 
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES (?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?, ?,?,?,?)"    #FUN-A60092 add ?,?   #MOD-AC0094 add ? #FUN-CB0003  add ?
...
#No.FUN-7B0138--END
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL x835_tm(0,0)        # Input print condition
      ELSE CALL x835()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
END MAIN
 
FUNCTION x835_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,    #No.FUN-690026 SMALLINT
   ...
   OPEN WINDOW x835_w AT p_row,p_col
        WITH FORM "aim/42f/aimx835" 
################################################################################
...
#FUN-CB0003--add--end--
 ...
 LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW x835_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimx835',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW x835_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0074
   ...
   DEFINE l_user LIKe type_file.chr100   #FUN-CB0003
     ...
     LET l_sql = "SELECT '','','', ",
                 ...
                 " ima02,ima021, ima08, ima25, 0, sfb82, gem02,pid012,pid021", #MOD-520029
     ...
     THEN  CASE tm.choice    
           ...
           END CASE
     END IF
     PREPARE x835_prepare1 FROM l_sql
     ...
     END IF
     DECLARE x835_curs1 CURSOR FOR x835_prepare1
 
#    LET l_name = 'aimx835.out'
#No.FUN-7B0138--START
#     CALL cl_outnam('aimx835') RETURNING l_name
#     START REPORT x835_rep TO l_name
#     IF tm.choice = '1' 
...
#       OUTPUT TO REPORT x835_rep(sr.*)
#FUN-CB0003--add--str--
        IF tm.user = '1' THEN 
           LET l_user = cl_getmsg("lib-065",g_lang)
        ELSE 
           LET l_user = cl_getmsg("lib-066",g_lang)
        END IF
        LET l_num = 3
#FUN-CB0003--add--end--
        EXECUTE insert_prep USING
             sr.pid01,sr.pid02,sr.pid022,sr.pid03,sr.pie02,sr.pie03,sr.pie04,sr.pie07,   #MOD-AC0094 add pid022
            #sr.pie30,sr.pie40,sr.pie50,sr.pie60,sr.count,sr.ima02,sr.ima02, #FUN-D40128 mark
             sr.pie30,sr.pie40,sr.pie50,sr.pie60,sr.count,sr.ima02,sr.ima021, #FUN-D40128 
             sr.ima08,sr.ima25,sr.sfa13,sr.sfb82,sr.gem02
            ,sr.pid012,sr.pid021,l_num     #FUN-A60092 add  #FUN-CB0003 add l_num
#No.FUN-7B0138--END
     END FOREACH
#No.FUN-7B0138--START
###XtraGrid###     LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     IF g_zz05 = "Y" THEN 
         ...
         LET l_str= tm.wc
     END IF
###XtraGrid###     LET l_str = l_str,";",tm.choice,";",tm.user,";",tm.s[1,1],";",tm.s[2,2],";",
###XtraGrid###                  tm.s[3,3],";",tm.t[1,1],";",tm.t[2,2],";",tm.t[3,3],";",tm.data
     IF g_sma.sma541 = 'N' THEN                           #FUN-A60092
###XtraGrid###        CALL cl_prt_cs3('aimx835','aimx835',g_sql,l_str)
    LET g_xgrid.template = 'aimx835'   #FUN-CB0003
#   LET g_xgrid.table = l_table    ###XtraGrid###
#   CALL cl_xg_view()    ###XtraGrid###
     ELSE                                                   #FUN-A60092
###XtraGrid###        CALL cl_prt_cs3('aimx835','aimx835_1',g_sql,l_str)  #FUN-A60092      
    LET g_xgrid.template = 'aimx835_1'  ##FUN-CB0003
#   LET g_xgrid.table = l_table    ###XtraGrid###
#   CALL cl_xg_view()    ###XtraGrid###
     END IF                                                 #FUN-A60092
#FUN-CB0003--add--start---
    LET g_xgrid.table = l_table    ###XtraGrid###
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"pid02,pid01,pid03,pie02,sfb82")
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"pid02,pid01,pid03,pie02,sfb82")  #FUN-D30053
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"pid02,pid01,pid03,pie02,sfb82")
    LET g_xgrid.footerinfo1 = cl_getmsg("lib-067",g_lang),tm.data,"|",l_user
   #LET l_str = cl_wcchp(g_xgrid.order_field,"pid02,pid01,pid03,pie02,sfb82")  #FUN-D30070  mark
   #LET l_str = cl_replace_str(l_str,',','-')  #FUN-D30070  mark
   #LET g_xgrid.footerinfo2 = cl_getmsg("lib-626",g_lang),l_str   #FUN-D30070  mark
    LET g_xgrid.condition = cl_getmsg('lib-160',g_lang),tm.wc
    CALL cl_xg_view()    ###XtraGrid###
#FUN-CB0003--add--end--
#    FINISH REPORT x835_rep
