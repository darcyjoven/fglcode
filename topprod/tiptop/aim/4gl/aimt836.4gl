...
#
# Pattern name...: aimt836.4gl
# Descriptions...: 複盤adjust作業－現有庫存
# Date & Author..: 93/06/11 By Apple
# NOTE...........: 本程式的設計必須考慮使用者的操作輸入為主
# Modify.........: No.FUN-570082 By Carrier 多單位修改
# Modify.........: No.FUN-570034 05/07/05 By kim 快速輸入第一筆取消+1
# Modify.........: No.FUN-5B0137 05/12/01 By kim 增加 參考單位 ima906='3' 時 同 ima906='2' 時 處理
...
# Modify.........: No.MOD-840340 08/04/21 By Carol 調整SQL條件
# Modify.........: No.TQC-860018 08/06/09 By Smapmin 增加on idle控管
# Modify.........: No.MOD-8A0090 08/10/09 By claire  pia60為數值不可以空字串判斷值
...
# Modify.........: No:TQC-C20365 12/02/29 By bart menu段少LET g_action_choice="lot_checking"
...
DEFINE g_unit_t            LIKE pia_file.pia09    #No.FUN-BB0086
...
#     DEFINE    l_time LIKE type_file.chr8              #No.FUN-6A0074
   ...
   END IF
    ...
    DECLARE t836_cl CURSOR FROM g_forupd_sql              # LOCK CURSOR
 
       LET p_row = 3 LET p_col = 27
 
    OPEN WINDOW t836_w AT p_row,p_col
             WITH FORM "aim/42f/aimt836" 
...
FUNCTION t836_cs()
 DEFINE  l_code  LIKE type_file.num5,    #No.FUN-690026 SMALLINT
    ...
    CONSTRUCT BY NAME g_wc ON                    # 螢幕上取條件
        pia01, pia02, pia03, pia04, pia05,pia931 #FUN-930122 Add pia931
        ,pia930,pia70 #FUN-670093                #FUN-CB0087 add>pia70
    ...
    LET g_sql="SELECT pia01 FROM pia_file ", # 組合出 SQL 指令
              ...
              "   AND (pia50 IS NOT NULL OR pia60 IS NOT NULL ",
#             "        OR pia50 != '' OR pia60 != ' ' )",     #MOD-840340-mark
              "         )",      #MOD-840340-modify
              " AND ",g_wc CLIPPED
    IF l_code THEN 
       LET g_sql = g_sql CLIPPED,
                  " AND (pia50 != pia60 OR pia50 IS NULL OR pia60 IS NULL",
#                 "      OR pia50 ='' OR pia60 =' ') ",       #MOD-840340-mark
                  "      ) ",        #MOD-840340-modify
                  " ORDER BY pia01 "
    ELSE  LET g_sql = g_sql clipped, " ORDER BY pia01"
    END IF
    PREPARE t836_prepare FROM g_sql           # RUNTIME 編譯
    DECLARE t836_cs                         # SCROLL CURSOR
        SCROLL CURSOR WITH HOLD FOR t836_prepare
    LET g_sql=
    ...
    IF l_code THEN 
       LET g_sql = g_sql CLIPPED,
                  ...
                  " ) "          #MOD-8A0090 modify 
    END IF 
    PREPARE t836_precount FROM g_sql
    DECLARE t836_count CURSOR FOR t836_precount
END FUNCTION
...
#No.FUN-8A0147--begin
        ON ACTION lot_checking
            LET g_action_choice="lot_checking"   #TQC-C20365
...
#No.FUN-8A0147--begin
        ...
        ON ACTION multi_unit_adjust
            LET g_sql = "aimt838"," '",g_pia.pia01 CLIPPED,"'"
            CALL cl_cmdrun_wait(g_sql)
            CALL t836_multi_unit()
        #No.FUN-570082  --end
        ON ACTION help 
            CALL cl_show_help()
        ON ACTION locale
...
FUNCTION t836_a()
 ...
 DEFINE  l_i             LIKE type_file.num5   #FUN-570034  #No.FUN-690026 SMALLINT
...
#No.FUN-8A0147--end
   ...
   END IF
    ...
    WHILE TRUE
       ...
       #start FUN-5A0199
       #LET g_pia.pia01 = g_pia_t.pia01[1,3],'-',
       #                  g_pia_t.pia01[5,10] +l_i using '&&&&&&'
#No.FUN-8A0147--begin
#       LET g_pia.pia01 = g_pia_t.pia01[1,g_doc_len],'-',
#                         g_pia_t.pia01[g_no_sp,g_no_ep] +l_i using '&&&&&&'
#----TQC-CB0097---MARK---
#       CASE l_no_ep
...
FUNCTION t836_i(p_cmd)
    ...
    INPUT g_pia.pia01,g_pia.pia02,g_pia.pia03,g_pia.pia04,
          ...
          g_qty,g_unit,g_pia.pia930,g_pia.pia70 #FUN-670093  #FUN-CB0087 add>pia70
          WITHOUT DEFAULTS 
      FROM pia01,pia02,pia03,pia04,
           pia05,pia06,pia09,pia07,
           qty,unit,pia930,pia70 #FUN-670093                 #FUN-CB0087 add>pia70
        ...
        AFTER FIELD pia01 
           ...
           IF g_pia.pia01 IS NOT NULL OR g_pia.pia01 != ' ' THEN
                ...
                IF NOT cl_null(g_errno)  THEN 
                   ...
                   NEXT FIELD pia01
                END IF
                IF g_sma.sma115='Y' AND g_ima906='2' THEN             #No.FUN-C20068 add
                   CALL t836_qty_check(l_cnt1,p_cmd) RETURNING l_tf   #No.FUN-BB0086
                END IF                                                #No.FUN-C20068 add
           END IF
#No.FUN-8A0147--begin
...
#No.FUN-8A0147--end
        ...
        AFTER FIELD qty 
           IF NOT t836_qty_check(l_cnt1,p_cmd) THEN NEXT FIELD qty END IF   #No.FUN-BB0086
	...
	IF g_unit IS NOT NULL OR g_unit != ' ' THEN
                 ...
                 IF g_sma.sma115='Y' AND g_ima906='2' THEN
                    ...
                    DISPLAY g_qty TO FORMONLY.qty
                 ELSE 
                    IF NOT t836_qty_check(l_cnt1,p_cmd) THEN 
                       LET g_unit_t = g_unit
...
FUNCTION t836_pia01(p_cmd)  
    ...
    CASE WHEN SQLCA.SQLCODE = 100 LET g_errno = 'mfg0002' 
              ...
              LET g_pia.pia931 = NULL                              #FUN-930122
   ...
   #FUN-690022------mod------- 
	...
	IF cl_null(g_errno) OR p_cmd = 'd' THEN
       DISPLAY BY NAME g_pia.pia02,g_pia.pia03,g_pia.pia04,
                       ...
                       g_pia.pia09,g_pia.pia50,g_pia.pia60,g_pia.pia931     #FUN-930122 Add pia931
...
FUNCTION t836_pia02(p_cmd)  #料件編號
    DEFINE p_cmd	LIKE type_file.chr1,    #No.FUN-690026 VARCHAR(1)
           l_ima02      LIKE ima_file.ima02,
           l_imaacti    LIKE ima_file.imaacti
    ...
    LET g_errno = ' '
	LET l_ima02=' '
    SELECT ima02,ima25,imaacti INTO l_ima02,g_ima25,l_imaacti
...
FUNCTION t836_q()
    ...
    END IF
    OPEN t836_count
    FETCH t836_count INTO g_row_count
    DISPLAY g_row_count TO FORMONLY.cnt  
    OPEN t836_cs                            # 從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
        CALL cl_err(g_pia.pia01,SQLCA.sqlcode,0)
        INITIALIZE g_pia.* TO NULL
        LET g_qty = NULL LET g_unit =NULL
    ELSE
        CALL t836_fetch('F')                  # 讀出TEMP第一筆並顯示
    END IF
END FUNCTION
 
FUNCTION t836_fetch(p_flpia)
    DEFINE
        p_flpia          LIKE type_file.chr1    #No.FUN-690026 VARCHAR(1)
 
    CASE p_flpia
        WHEN 'N' FETCH NEXT     t836_cs INTO g_pia.pia01
        WHEN 'P' FETCH PREVIOUS t836_cs INTO g_pia.pia01
        WHEN 'F' FETCH FIRST    t836_cs INTO g_pia.pia01
        WHEN 'L' FETCH LAST     t836_cs INTO g_pia.pia01
        WHEN '/'
...
#                      CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
                ...
                END IF
            END IF
            FETCH ABSOLUTE g_jump t836_cs INTO g_pia.pia01
            LET mi_no_ask = FALSE
    END CASE
    IF SQLCA.sqlcode THEN
        CALL cl_err(g_pia.pia01,SQLCA.sqlcode,0)
...
FUNCTION t836_show()
    ...
    DISPLAY BY NAME 
        ...
        g_pia.pia07,g_pia.pia50,g_pia.pia60,g_pia.pia930 #FUN-670093
        ,g_pia.pia931,g_pia.pia70                                    #FUN-930122 Add pia931   #FUN-CB0087 add>pia70
    DISPLAY g_qty       TO FORMONLY.qty 
    DISPLAY g_unit      TO FORMONLY.unit 
    DISPLAY g_pia.pia09 TO FORMONLY.pia09_1 
    DISPLAY g_pia.pia09 TO FORMONLY.pia09_2 
...
FUNCTION t836_u()
    ...
    OPEN t836_cl USING g_pia.pia01
    IF STATUS THEN
       CALL cl_err("OPEN t836_cl:", STATUS, 1)
       CLOSE t836_cl
       ROLLBACK WORK
       RETURN
    END IF
    FETCH t836_cl INTO g_pia.*               # 對DB鎖定
    IF SQLCA.sqlcode THEN
        CALL cl_err(g_pia.pia01,SQLCA.sqlcode,0)
        RETURN
    END IF
    CALL t836_show()                          # 顯示最新資料
    WHILE TRUE
        CALL t836_i("u")                      # 欄位更改
        IF INT_FLAG THEN
            LET INT_FLAG = 0
...
#         CALL cl_err('update pia',SQLCA.sqlcode,0)
       ...
       END IF
    END IF
    DISPLAY BY NAME g_pia.pia50
    DISPLAY BY NAME g_pia.pia60
    DISPLAY g_qty TO FORMONLY.qty
END FUNCTION
#No.FUN-570082  --end  

#No.FUN-BB0086--add--start--
FUNCTION t836_qty_check(l_cnt1,p_cmd)
