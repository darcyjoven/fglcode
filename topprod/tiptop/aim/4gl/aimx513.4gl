...
# Modify.........: No.FUN-D40129 13/05/08 By yangtt 添加撥出倉庫的倉庫名稱、儲位名稱以及撥入倉庫的倉庫名稱、儲位名稱
...
#No.FUN-7C0049--start--
   LET g_sql = "imm01.imm_file.imm01,",
               ...
               "l_num.type_file.num5,",  #FUN-CB0001
               "imd02_1.imd_file.imd02,",    #FUN-D40129
               "ime03_1.ime_file.ime03,",    #FUN-D40129
               "imd02_2.imd_file.imd02,",    #FUN-D40129
               "ime03_2.ime_file.ime03"      #FUN-D40129
   LET l_table = cl_prt_temptable('aimx513',g_sql) CLIPPED
   IF l_table = -1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
                " VALUES(?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?, ?,?,?,?)"   #FUN-CB0001 add 1?  #FUN-D40129 add 4?
...
#No.FUN-7C0049--end
  ...
  #TQC-610072-end
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      THEN CALL x513_tm(0,0)            # Input print condition
      ELSE CALL x513()          # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
END MAIN
 
FUNCTION x513_tm(p_row,p_col)
...
DEFINE p_row,p_col   LIKE type_file.num5,    #No.FUN-690026 SMALLINT
   ...
   OPEN WINDOW x513_w AT p_row,p_col
        WITH FORM "aim/42f/aimx513"
################################################################################
...
WHILE TRUE
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW x513_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimx513',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW x513_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
...
#No.FUN-7C0049--end
     ...
     DEFINE l_imd02_1     LIKE imd_file.imd02,    #FUN-D40129
            l_ime03_1     LIKE ime_file.ime03,    #FUN-D40129
            l_imd02_2     LIKE imd_file.imd02,    #FUN-D40129
            l_ime03_2     LIKE ime_file.ime03     #FUN-D40129

     CALL cl_del_data(l_table)              #No.FUN-7C0049
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aimx513'   #No.FUN-7C0049
    ...
    #END IF
     ...
     END IF
    #---------------No.MOD-810120 end
     PREPARE x513_prepare FROM l_sql
     ...
     END IF
     DECLARE x513_cs CURSOR FOR x513_prepare
#    CALL cl_outnam('aimx513') RETURNING l_name     #No.FUN-7C0049
     #No.FUN-580005 --start--
     ...
     IF g_sma115 = "Y" THEN
       ...
       LET l_name = 'aimx513'         #No.FUN-7C0049
     ELSE
       #LET g_zaa[47].zaa06 = "Y"     #No.FUN-7C0049
       #LET g_zaa[50].zaa06 = "Y"     #No.FUN-7C0049
       LET l_name = 'aimx513_1'       #No.FUN-7C0049
     END IF
     CALL cl_prt_pos_len()
     #No.FUN-580005 --end--
 
    #START REPORT x513_rep TO l_name      #No.FUN-7C0049
 
    #LET g_pageno = 0                     #No.FUN-7C0049
     FOREACH x513_cs INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#No.FUN-7C0049--start--  on every row 提上來
       ...
       #FUN-D40129---add--str--
       SELECT imd02 INTO l_imd02_1 FROM imd_file WHERE imd01 = sr.imn15   #撥入倉庫名稱
       SELECT ime03 INTO l_ime03_1 FROM ime_file WHERE ime02 = sr.imn16   #倉儲名稱
       SELECT imd02 INTO l_imd02_2 FROM imd_file WHERE imd01 = sr.imn04   #撥出倉庫名稱
       SELECT ime03 INTO l_ime03_2 FROM ime_file WHERE ime02 = sr.imn05   #倉儲名稱
       #FUN-D40129---add--end--
       EXECUTE insert_prep USING
          ...
          sr.imn06,sr.ima021,l_num  #FUN-CB0001 l_num             
          ,l_imd02_1,l_ime03_1,l_imd02_2,l_ime03_1  #FUN-D40129
       #OUTPUT TO REPORT x513_rep(sr.*)
#No.FUN-7C0049--end
     END FOREACH
 
#No.FUN-7C0049--start--
     #FINISH REPORT x513_rep
###XtraGrid###    LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED 
    LET g_xgrid.table = l_table    ###XtraGrid###
    SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog #FUN-CB0001 add
    IF g_zz05 = 'Y' THEN  
...
###XtraGrid###    CALL cl_prt_cs3('aimx513',l_name,g_sql,g_str)
    LET g_xgrid.template = l_name  #FUN-CB0001 add
