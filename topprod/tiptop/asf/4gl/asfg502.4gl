...
###GENGRE###START
TYPE sr1_t RECORD
    order1 LIKE type_file.chr1000, #FUN-B50018
    order2 LIKE type_file.chr1000, #FUN-B50018
#   order3 LIKE type_file.chr100,  #FUN-B50018 
#   order4 LIKE type_file.chr100,  #FUN-B50018 
...
#  LET tm.wc    = ARG_VAL(7)  #TQC-610080     #No.TQC-6C0143
   ...
   #No.FUN-710082--begin
   LET g_sql ="order1.type_file.chr1000,",  #FUN-B50018 
              "order2.type_file.chr1000,",  #FUN-B50018 
            # "order3.type_file.chr100,",   #FUN-B50018
            # "order4.type_file.chr100,",   #FUN-B50018
...
#              "i.type_file.num5"      #No.FUN-860026
   ...
   LET l_table = cl_prt_temptable('asfg502',g_sql) CLIPPED
   IF l_table = -1 THEN 
...
#No.FUN-860026---begin                                                                                                              
   ...
   LET l_table1 = cl_prt_temptable('asfg5021',g_sql) CLIPPED                                                                        
   IF  l_table1 = -1 THEN 
       #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add #FUN-BB0047 mark
       #CALL cl_gre_drop_temptable(l_table||"|"||l_table1)   #FUN-B50018  add#FUN-C10036 mark
       EXIT PROGRAM 
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   END IF
   OPEN WINDOW g502_w AT p_row,p_col
        WITH FORM "asf/42f/asfg502"
################################################################################
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('asfg502',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW g502_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1)
...
#     DEFINE   l_time LIKE type_file.chr8       #No.FUN-6A0090
 DEFINE   l_name   LIKE type_file.chr20,         #No.FUN-680121 VARCHAR(20)
          ...
          sr  RECORD
             ...
             #order3    LIKE sfs_file.sfs03,   #FUN-B50018 
             #order4    LIKE sfs_file.sfs04,   #FUN-B50018
              sfs02	LIKE sfs_file.sfs02,
              sfs03	LIKE sfs_file.sfs03,
              sfs04	LIKE sfs_file.sfs04,
              sfs05	LIKE sfs_file.sfs05,
...
#No.FUN-590110  --begin
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'asfg502'
...
#No.FUN-590110  --end
     ...
     PREPARE g502_pr1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
        CALL cl_gre_drop_temptable(l_table||"|"||l_table1)
        EXIT PROGRAM 
     END IF
     DECLARE g502_cs1 CURSOR FOR g502_pr1
 
     #No.FUN-710082--begin
#    CALL cl_outnam('asfg502') RETURNING l_name
 
#    LET g_pageno = 0
#    START REPORT g502_rep TO l_name
   ...
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               ...
               "        ?,?,?,?,?,  ",
#               "        ?,?,?) "        #No.FUN-860026 
               "        ?,?,?,?,?, ?,?,?,?,?,",  #No.FUN-860026     #FUN-A60027 add 2? #MOD-AB0043 add ? #FUN-B50018 2?
               "        ?,?,?) "         #FUN-C40019 add 4?
...
#                        "          sfs08,sfs09,sfs10,sfs012,sfs013,ima02,ima021,ima23,img103 ",       #FUN-A60027 add sfs012,sfs013
                         "          sfs08,sfs09,sfs10,sfs012,sfs013,ima02,ima021,ima23,img10 ",       #FUN-A60027 add sfs012,sfs013    #FUN-B50018 img103->img10
...
#                        " ORDER BY 2"
       END IF
       PREPARE g502_pr2 FROM l_sql
       IF STATUS THEN CALL cl_err('prepare2:',STATUS,1) 
          CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
          CALL cl_gre_drop_temptable(l_table||"|"||l_table1)
          EXIT PROGRAM 
       END IF
       DECLARE g502_cs2 CURSOR FOR g502_pr2
       LET i=1
       FOREACH g502_cs2 INTO sr.*
          IF tm.p='N' THEN LET sr.ima23=' ' END IF
 
#         CASE tm.q WHEN '1' LET sr.order=sr.sfs04,sr.sfs03
#                   WHEN '2' LET sr.order=sr.sfs03,sr.sfs04
#         END CASE
#        CASE tm.q WHEN '1' LET sr.order3 = sr.sfs04 LET sr.order4 = sr.sfs03  #FUN-B50018
#                  WHEN '2' LET sr.order3 = sr.sfs03 LET sr.order4 = sr.sfs04  #FUN-B50018
#        END CASE                                                              #FUN-B50018
     ...
     #FUN-C50003-----mark---end--
           LET j=1
           LET l_flag=0 
      FOREACH g502_cs3 USING l_sfp.sfp01 INTO             #FUN-C50003 add USING l_sfp.sfp01 
...
#No.FUN-860026---begin    
   ...
   #              ORDER BY  rvbs04                                                                                                  
   #FUN-C50003-----mark---end--
    FOREACH  r920_d USING l_sfp.sfp01,sr.sfs02 INTO l_rvbs.*         #FUN-C50003 add USING l_sfp.sfp01,sr.sfs02 
         LET flag = 1                                                                                          
...
#No.FUN-860026---end      
         ...
         #FUN-B50018------ add-----str-----
          IF tm.p = 'N' THEN   #不依倉管員跳頁
            ...
            LET l_order[2] = sr.ima23
         ELSE
            LET l_order[1] = sr.ima23
            LET l_order[2] = l_sfp.sfp01
         END IF
         #FUN-B50018------ add-----end-----
         #EXECUTE insert_prep USING l_order[1],l_order[2],sr.order3,sr.order4,sr.sfs02,sr.sfs03,sr.sfs04,sr.sfs05,sr.sfs06,
          EXECUTE insert_prep USING l_order[1],l_order[2],sr.sfs02,sr.sfs03,sr.sfs04,sr.sfs05,sr.sfs06,
...
#No.FUN-860026---begin                                                                                                              
   ...
   #              ORDER BY  rvbs04                                                                                                  
   #FUN-C50003-----mark---end--
    FOREACH  r920_c USING l_sfp.sfp01,sr.sfs02 INTO l_rvbs.*   #FUN-C50003 add USING l_sfp.sfp01,sr.sfs02
         LET flag = 1                                                                                                    
         EXECUTE insert_prep1 USING  l_sfp.sfp01,sr.sfs02,l_rvbs.rvbs03,                                                            
                                     l_rvbs.rvbs04,l_rvbs.rvbs06,l_rvbs.rvbs021,                                                    
...
#         OUTPUT TO REPORT g502_rep(l_sfp.*, sr.*)
       END FOREACH
     END FOREACH
   ...
   # LET l_sql = "SELECT * FROM ",l_table CLIPPED  #TQC-730088                    
     ...
     END CASE
###GENGRE###     LET l_sql =l_sql CLIPPED,"|",
###GENGRE###                "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED    #No.FUN-860026                                                          
     ...
     IF g_zz05 = 'Y' THEN                                                         
        ...
        RETURNING tm.wc                                                           
     END IF                      
###GENGRE###     LET l_str = tm.wc CLIPPED,";",g_zz05 CLIPPED,";",tm.q,";",tm.p,";",tm.c   #No.FUN-860026   add tm.c 
 
   # CALL cl_prt_cs3('asfg502',l_sql,l_str)   #TQC-730088
   # CALL cl_prt_cs3('asfg502','asfg502',l_sql,l_str)            #FUN-A60027
    LET g_cr_table = l_table                   #主報表的temp table名稱          #FUN-C40019 add
    LET g_cr_apr_key_f = "sfp01"               #報表主鍵欄位名稱，用"|"隔開     #FUN-C40019 add
   #FUN-A60027 --------------------start----------------------
     IF g_sma.sma541 = 'Y' THEN
###GENGRE###        CALL cl_prt_cs3('asfg502','asfg502_3',l_sql,l_str) 
    CALL asfg502_grdata()    ###GENGRE###
     ELSE
###GENGRE###        CALL cl_prt_cs3('asfg502','asfg502',l_sql,l_str)
    CALL asfg502_grdata()    ###GENGRE###
     END IF 
  #FUN-A60027 --------------------end------------------------   
#    FINISH REPORT g502_rep
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
     #No.FUN-710082--end  
END FUNCTION
 
#No.FUN-710082--begin
#REPORT g502_rep(l_sfp, sr)
#  DEFINE
...
#           COLUMN 27,g_x[16] CLIPPED,l_sfp.sfp06,' ',g502_sfp06(l_sfp.sfp06),
...
#     FOREACH g502_cs3 INTO l_sfq.*, l_sfb.*, l_ima02
#       IF SQLCA.SQLCODE THEN CONTINUE FOREACH END IF
#      #FUN-5A0140-begin
...
FUNCTION asfg502_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("asfg502")
        IF handler IS NOT NULL THEN
            START REPORT asfg502_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                 #      ," ORDER BY order1,order2"             #FUN-B50018
                        ," ORDER BY sfp01 "  
            DECLARE asfg502_datacur1 CURSOR FROM l_sql
            FOREACH asfg502_datacur1 INTO sr1.*
                OUTPUT TO REPORT asfg502_rep(sr1.*)
            END FOREACH
            FINISH REPORT asfg502_rep
        END IF
...
REPORT asfg502_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE sr2 sr2_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B50018----add-----str-----------------
    DEFINE l_flag               LIKE  type_file.num5
    DEFINE l_group1             LIKE  sfs_file.sfs04
    DEFINE l_ima23_gen02        STRING
    DEFINE l_sfp06_desc         STRING
    DEFINE l_sfp07_gem02        STRING
    DEFINE l_sfp07_sfs08_sfs09  STRING
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.order2
           ...
           #FUN-B50018----add-----str-----------------
           # IF NOT cl_null(sr1_t.sfp01) AND NOT cl_null(sr1_t.ima23) THEN
           #    IF sr1_t.sfp01 = sr1.sfp01 AND sr1_t.ima23 = sr1.ima23 THEN
           #       LET l_display1 = 'N '
           #    ELSE
           #       LET l_display1 = 'Y '
           #    END IF
           #END IF
           #PRINTX l_display1
             IF sr1_t.ima23 = sr1.ima23 AND sr1_t.sfp01 = sr1.sfp01 THEN
                LET l_display3 = 'N'
             ELSE
                LET l_display3 = 'Y'
             END IF
        ...
        ON EVERY ROW
            ...
            IF tm.q = '1' THEN
               LET l_group1 = sr1.sfs04
            ELSE
               LET l_group1 = sr1.sfs03
            END IF
            PRINTX l_group1
            ...
            LET l_sfp07_sfs08_sfs09 = sr1.sfs07,'/',sr1.sfs08,'/',sr1.sfs09
            PRINTX l_sfp07_sfs08_sfs09
        ...
        AFTER GROUP OF sr1.order2
           ...
           #FUN-B50018----add-----str-----------------
           LET l_sql = "SELECT sfp01,sfq02,sfq04,sfb05,sfb08,sfb081,sfb09,ima02a,sfq03,ima021a FROM ",
                       g_cr_db_str CLIPPED,l_table CLIPPED,
                       " WHERE sfp01 = '",sr1.sfp01 CLIPPED,"'",
                       " GROUP BY sfp01,sfq02,sfq04,sfb05,sfb08,sfb081,sfb09,ima02a,sfq03,ima021a"
           START REPORT asfg502_subrep02
           DECLARE asfg502_repcur2 CURSOR FROM l_sql
           FOREACH asfg502_repcur2 INTO sr1.sfp01,sr1.sfq02,sr1.sfq04,sr1.sfb05,sr1.sfb08,sr1.sfb081,sr1.sfb09,
                                        sr1.ima02a,sr1.sfq03,sr1.ima021a
               OUTPUT TO REPORT asfg502_subrep02(sr1.sfp01,sr1.sfq02,sr1.sfq04,sr1.sfb05,sr1.sfb08,sr1.sfb081,sr1.sfb09,
                                        sr1.ima02a,sr1.sfq03,sr1.ima021a)
           END FOREACH
           FINISH REPORT asfg502_subrep02

           IF tm.p = 'Y' THEN
              LET l_sql = "SELECT sfp01,sfs02,sfs03,sfs04,sfs10,sfs012,sfs013,sfs06,sfs05,ima02,sfs07,sfs08,sfs09,ima021,ima23,img10 FROM ",
                       g_cr_db_str CLIPPED,l_table CLIPPED,
                       " WHERE sfp01 = '",sr1.sfp01 CLIPPED,"' AND ima23 = '",sr1.ima23,"'",
                       " GROUP BY sfp01,sfs02,sfs03,sfs04,sfs10,sfs012,sfs013,sfs06,sfs05,ima02,sfs07,sfs08,sfs09,ima021,ima23,img10",   
                       " ORDER BY sfs02 "
           ELSE
              LET l_sql = "SELECT sfp01,sfs02,sfs03,sfs04,sfs10,sfs012,sfs013,sfs06,sfs05,ima02,sfs07,sfs08,sfs09,ima021,ima23,img10 FROM ",
                       g_cr_db_str CLIPPED,l_table CLIPPED,
                       ...
                       " ORDER BY sfs02 "
           END IF
           START REPORT asfg502_subrep03
           DECLARE asfg502_repcur3 CURSOR FROM l_sql
           FOREACH asfg502_repcur3 INTO sr1.sfp01,sr1.sfs02,sr1.sfs03,sr1.sfs04,sr1.sfs10,sr1.sfs012,sr1.sfs013,
                                        sr1.sfs06,sr1.sfs05,sr1.ima02,sr1.sfs07,sr1.sfs08,sr1.sfs09,sr1.ima021,sr1.ima23,sr1.img10
               OUTPUT TO REPORT asfg502_subrep03(sr1.sfp01,sr1.sfs02,sr1.sfs03,sr1.sfs04,sr1.sfs10,sr1.sfs012,sr1.sfs013,
                                        sr1.sfs06,sr1.sfs05,sr1.ima02,sr1.sfs07,sr1.sfs08,sr1.sfs09,sr1.ima021,sr1.ima23,sr1.img10)
           END FOREACH
           FINISH REPORT asfg502_subrep03

           LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                       " WHERE rvbs01 = '",sr1.sfp01 CLIPPED,"'"
           START REPORT asfg502_subrep01
           DECLARE asfg502_repcur1 CURSOR FROM l_sql
           FOREACH asfg502_repcur1 INTO sr2.*
               OUTPUT TO REPORT asfg502_subrep01(sr2.*)
           END FOREACH
           FINISH REPORT asfg502_subrep01
           #FUN-B50018----add-----end-----------------
...
REPORT asfg502_subrep02(sr1)
    ...
    ORDER EXTERNAL BY sr1.sfp01,sr1.sfq02,sr1.sfq03,sr1.sfq04
    FORMAT
        ON EVERY ROW
            PRINTX sr1.*
            PRINTX g_sma.sma541
...
REPORT asfg502_subrep03(sr1)
    DEFINE sr1   RECORD
                #order3 LIKE sfs_file.sfs03,    
                #order4 LIKE sfs_file.sfs04,
                 sfp01 LIKE sfp_file.sfp01,
                 sfs02 LIKE sfs_file.sfs02,
                 sfs03 LIKE sfs_file.sfs03,
                 sfs04 LIKE sfs_file.sfs04,
   ...
   #ORDER EXTERNAL BY sr1.order3,sr1.order4 
    ORDER EXTERNAL BY sr1.sfp01,sr1.sfs02,sr1.ima23 
    FORMAT
        ON EVERY ROW
            IF cl_null(sr1.sfs02) AND cl_null(sr1.sfs03) AND cl_null(sr1.sfs10) THEN
               LET l_display = 'N'
            ELSE
               LET l_display = 'Y'
            END IF
            ...
            PRINTX sr1.*
            IF g_sma.sma541 = 'N' THEN LET l_display1 = 'N'
            ELSE LET l_display1 = 'Y' END IF
            PRINTX l_display1
END REPORT
#FUN-B50018----add-----end-----------------
###GENGRE###END 
