...
#add by zhangsba201014--s
# Modify.........: No:CHI-E20031 14/03/06 By Alberti 1.當asfi514 or asfi519(倒扣料) 要還原時，皆須排除超領單的數量，超領的數量只能從 超領退 or 超領單來回寫
#                                                    2.並控卡 有超領量>0時，發料單還原會使已發數量(sfa06)<0，會無法過帳還原
# Modify.........: No:MOD-E70014 14/07/03 By Reanna 判斷已發數量時，若有替代需考慮被替代料與替代料的情況
# Modify.........: No:MOD-FA0136 15/10/29 By fionchen 修正MOD-E70014,錯誤時要加上RETURN,修正CHI-E20031,錯誤訊息asf1181與asf1180調整為cl_err 
# Modify.........: No:MOD-EB0129 14/11/27 By Alberti 修正 發料時才做取替代，如替代料與原料單位不同，會無法過帳
# Modify.........: No:MOD-F60094 15/06/17 By Alberti 修正 MOD-EB0129，需額外判斷是否為取替代料
#add by zhangsba201014--e
# Modify.........: No.2021113001 21/11/30 By jc 扣账自动同步SCM
...
DEFINE l_ima63     LIKE ima_file.ima63     #MOD-EB0129 add
#add by zhangzs 210113  ----s-----
DEFINE l_sfs03_1     LIKE sfs_file.sfs03
DEFINE l_tsc21     LIKE tsc_file.tsc21
DEFINE l_count_1   LIKE type_file.num5
DEFINE l_count_2   LIKE type_file.num5
...
#2021113001 add----end----
   ...
   #add zhangzs 210113  判斷是否有过账的atmt260单子
   #SELECT sfs03 INTO l_sfs03_1 FROM sfp_file,sfs_file WHERE sfp01 = sfs01 AND sfp01 = l_sfp.sfp01 #mark by liy211217
      SELECT sfs03 INTO l_sfs03_1 FROM sfp_file,sfs_file WHERE sfp01 = sfs01 AND sfp01 = p_sfp01 #add by liy211217
   SELECT count(*) INTO l_count_2 FROM tsc_file,shb_file WHERE tscud02 = shb01 AND shb05 = l_sfs03_1
   SELECT count(*) INTO l_count_1 FROM tsc_file WHERE tsc19 = l_sfs03_1
   IF l_count_1 > 0 OR l_count_2 > 0 THEN 
     IF l_count_1 > 0 THEN 
      SELECT tsc21 INTO l_tsc21 FROM tsc_file WHERE tsc19 = l_sfs03_1
      #LET l_sfp.sfp03 = l_tsc21   #add by wangxt170210  
     END IF 
     IF l_count_2 > 0 THEN 
      SELECT tsc21 INTO l_tsc21 FROM tsc_file,shb_file WHERE tscud02 = shb01 AND shb05 = l_sfs03_1
      #LET l_sfp.sfp03 = l_tsc21   #add by wangxt170210  
     END IF 
   ELSE 
   ...
   #add zhangzs 210113  判斷是否有过账的atmt260单子
   SELECT sfs03 INTO l_sfs03_1 FROM sfp_file,sfs_file WHERE sfp01 = sfs01 AND sfp01 = l_sfp.sfp01
   SELECT count(*) INTO l_count_2 FROM tsc_file,shb_file WHERE tscud02 = shb01 AND shb05 = l_sfs03_1
   SELECT count(*) INTO l_count_1 FROM tsc_file WHERE tsc19 = l_sfs03_1
   IF l_count_1 > 0 OR l_count_2 > 0 THEN 
     IF l_count_1 > 0 THEN 
...
#TQC-C40174 -----------------Begin-----------------
      ...
      #add by zhangzs 210113 ----s-----
      LET l_str2 = p_sfp01
      LET l_str1 = l_str2.subString(1,3)
     IF l_str1 = 'MRH' THEN
        LET g_rvbs00 = 'asfi528'
     END IF 
     IF l_str1 = '200' THEN
        LET g_rvbs00 = 'asfi529'
     END IF
     #add by zhangzs 210113 ----e-----
...
#TQC-C40174 -----------------End-------------------
     ...
     #FUN-C10035 add end -----
      ...
      #IF l_cnt > 0 THEN    #大於有效日期  #mark by darcy 220228
      IF l_cnt > 0 AND g_prog MATCHES 'asfi51*' THEN    #大於有效日期  #add by darcy 220228
     ...
     SELECT sfq03 INTO l_sfq03 FROM sfq_file                                                                                    
      WHERE sfq01 = l_sfp.sfp01                                                                                                 
        AND sfq02 = l_sfq02                                                                                                     
     IF l_sfq03 > l_qty  and g_user<>'tiptop' THEN                                                                                                    
...
#add by zhangsba201111--s
LET l_str = " asfi514 工单领料过账 账套  ",g_plant," 程序名 ",g_prog," sfp01 发料单号 ",l_sfp.sfp01," sfp06 ",l_sfp.sfp06,"\n"
RUN " echo '"||l_str||"' >> /u1/topprod/topcust/cws/4gl/asfi514_zhangsba_auto_post_i501sub_s2.log"
#add by zhangsba201111--e        
     ELSE
        CALL i501sub_s2_asr(p_argv1,l_sfp.*) #FUN-5C0114
#add by zhangsba201111--s
LET l_str = " asfi514 工单领料过账 账套  ",g_plant," 程序名 ",g_prog," sfp01 发料单号 ",l_sfp.sfp01," sfp06 ",l_sfp.sfp06,"\n"
RUN " echo '"||l_str||"' >> /u1/topprod/topcust/cws/4gl/asfi514_zhangsba_auto_post_i501sub_s2_asr.log"
#add by zhangsba201111--e        
     END IF
        END IF
     END IF
  ...
  #FUN-9A0095 add end-----
     ...
     COMMIT WORK
        ...
        #2021113001 add----begin----
        IF NOT cl_null(l_sfp.sfp01)  AND l_sfp.sfp06 MATCHES '[468]' AND cl_getscmparameter() AND ((l_sfp.sfpud06[1,3] <> 'MR1' AND l_sfp.sfpud06[1,3] <> 'OR1') OR l_sfp.sfpud06 IS NULL ) THEN
            INITIALIZE l_ret TO NULL
            IF l_sfp.sfp06 = '4' THEN 
              CALL cjc_zmx_json_task('DK1',l_sfp.sfp01) RETURNING l_ret.*
              ELSE 
              CALL cjc_zmx_json_task('OR1',l_sfp.sfp01) RETURNING l_ret.*
            END IF 
            ...
            ELSE
               IF cl_null(l_ret.msg) THEN
               	IF l_sfp.sfp06 = '4' THEN 
                   LET l_ret.msg = "工单倒扣发料(",l_sfp.sfp01 CLIPPED,")同步失败"
                   ELSE 
                   LET l_ret.msg = "工单退料(",l_sfp.sfp01 CLIPPED,")同步失败"
                 END IF 
               END IF
            END IF
  ...
  #     CALL i501sub_VMI_conf(l_sfp.sfp01)
        IF g_success = 'N' THEN
          ...
          IF NOT s_industry('icd') THEN #CHI-C90045
             ...
             #CALL s_padd_imgg_drop(l_imgg_table)  #FUN-CC0095 
          END IF  #CHI-C90045
          IF cl_null(g_aimp880) THEN    #CHI-E20031 add
             CALL s_showmsg()           #CHI-E20031 add
          END IF                        #CHI-E20031 add
          RETURN
        END IF
  #   END IF
...
#add by zhangsba201111--s
LET l_str = " asfi514 工单领料过账 账套  ",g_plant," 是否成功 ",g_success," 程序名 ",g_prog," sfp01 发料单号 ",l_sfp.sfp01," sfp06 ",l_sfp.sfp06,"\n"
RUN " echo '"||l_str||"' >> /u1/topprod/topcust/cws/4gl/asfi514_zhangsba_auto_post_i501sub_update.log"
#add by zhangsba201111--e              
        IF g_success='N' THEN 
           LET g_totsuccess='N'
           LET g_success="Y"
...
#add by zhangsba201111--s
LET l_str = " asfi514 工单领料过账 账套  ",g_plant," 是否成功 ",g_success," 程序名 ",g_prog," sfp01 发料单号 ",l_sfp.sfp01," sfp06 ",l_sfp.sfp06,"\n"
RUN " echo '"||l_str||"' >> /u1/topprod/topcust/cws/4gl/asfi514_zhangsba_auto_post_i501sub_u_sfa.log"
#add by zhangsba201111--e         
        IF g_success='N' THEN RETURN END IF
...
#MOD-B90008 --end--
  ...
  #        l_imaqty LIKE ima_file.ima262,
    ...
    DEFINE l_str  STRING
      ...
      END IF
      IF g_prog <> 'asfi528' AND g_prog <> 'asfi529' THEN    #add by sx210305 发料时才考虑img18，退料不考虑
         LET g_forupd_sql = "SELECT img10,img16,img23,img24,img09,img21,img18 FROM img_file",
             ...
             " FOR UPDATE "
      ELSE   
         LET g_forupd_sql = "SELECT img10,img16,img23,img24,img09,img21,img18 FROM img_file",   #add by sx210305 发料时才考虑img18，退料不考虑
     ...
     #add by lixwz210319 s---
     LET l_str = g_prog,  "工单领料过账 账套  ",g_plant," sfa01 工单编号 ",l_sfs.sfs03," sfa03 发料料号 ",l_sfs.sfs04, " p_factor ",p_factor," l_img.img10 ",l_img.img10
     RUN " echo '"||l_str||"' >> /u1/topprod/tiptop/asf/4gl/asfi514_lixwz_post.log"
     #add by lixwz210319 e---
      ...
      RETURN
         END IF
      END IF
      IF g_prog = 'atmt260' THEN   #add by zhangzs 210113
      CALL s_upimg(l_sfs.sfs04,p_ware,p_loca,p_lot,u_type,p_qty*p_factor,l_sfp.sfp03, #FUN-8C0084   #No.MOD-940320 add
       ...
       '','','','','','','','','','','','abcdefg') #add by zhangzs 210113
      ELSE  #add by zhangzs 210113
      CALL s_upimg(l_sfs.sfs04,p_ware,p_loca,p_lot,u_type,p_qty*p_factor,l_sfp.sfp03, #FUN-8C0084   #No.MOD-940320 add  #add by zhangzs 210113
       '','','','',l_sfs.sfs01,l_sfs.sfs02,   #No.CHI-860032  #add by zhangzs 210113
       '','','','','','','','','','','','')
      END IF #add by zhangzs 210113
      IF g_success='N' THEN
  ...
  FUNCTION i501sub_tlf(p_ware,p_loca,p_lot,p_unit,p_qty,p_img10,p_uom,p_factor,
     ...
     IF p_argv1='2' THEN
        ...
        #add by lixwz210319 s---
        SELECT img09 INTO g_tlf.tlf035 FROM img_file 
         where img01=l_sfs.sfs04
           AND img02=p_ware
           AND img03=p_loca
           AND img04=p_lot
        IF cl_null(g_tlf.tlf035) THEN
        #add by lixwz210319 s---
        LET g_tlf.tlf035=p_uom             #庫存單位(ima_file or img_file)
        END IF #add by lixwz210319
    ...
    WHEN l_sfp.sfp06='C' LET g_tlf.tlf13='asri230'       ##ASR領料
     ...
     END IF
     #add by lixwz210319 s---
     LET l_str = g_prog,  "工单领料过账 账套  ",g_plant," sfa01 工单编号 ",l_sfs.sfs03," sfa03 发料料号 ",l_sfs.sfs04, " 单位 ",g_tlf.tlf035," 数量 ",p_img10
     RUN " echo '"||l_str||"' >> /u1/topprod/tiptop/asf/4gl/asfi514_lixwz_post.log"
     #add by lixwz210319 e---
...
#FUN-A70125 --end--
  ...
  FUNCTION i501sub_u_sfa(p_code,p_argv1,l_sfp,l_sfs,l_sfb)
    ...
    DEFINE l_sfa062_s   LIKE sfa_file.sfa062     #CHI-E20031 add 
    DEFINE l_sfa06      LIKE sfa_file.sfa06      #MOD-E70014
    ...
    DECLARE i501_tlf_c CURSOR FOR
       SELECT tlf13,SUM(tlf10) FROM tlf_file
        WHERE tlf62 =l_sfs.sfs03 AND tlf01 =l_sfs.sfs04
          ...
          OR (tlf012 is null and tlf013 is null )) #add by lixwz200730
        GROUP BY tlf13
...
#MOD-D50031 add end------------------------------------
   ...
   #MOD-EB0129-Start-Add 
    SELECT ima63,ima63_fac INTO l_ima63,l_ima63_fac
      FROM ima_file
     WHERE ima01 = l_sfs.sfs04
   #MOD-EB0129-End-Add    
    
   #MOD-F60094-Start-Add
    IF l_sfs.sfs26 MATCHES '[SUBC]' THEN 
       LET l_sfs06 = l_ima63
    ELSE 
       LET l_sfs06 = l_sfs.sfs06   
    END IF 
   #MOD-F60094-End-Add
    ...
    SELECT * INTO l_sfa.* FROM sfa_file
        WHERE sfa01=l_sfs.sfs03 AND sfa03=l_sfs.sfs04
        #AND sfa12=l_sfs.sfs06  #MOD-EB0129 mark
         #AND sfa12=l_ima63      #MOD-EB0129 add   #MOD-F60094 mark
          AND sfa12=l_sfs06                        #MOD-F60094 add
          AND sfa08=l_sfs.sfs10
  ...
  CASE WHEN SQLCA.SQLCODE=0
        ...
        IF NOT s_industry('std') THEN  #FUN-9B0149 add
         ...
         WHERE sfai01=l_sfs.sfs03 AND sfai03=l_sfs.sfs04
             #AND sfai12=l_sfs.sfs06  #MOD-EB0129 mark
             #AND sfai12=l_ima63      #MOD-EB0129 add   #MOD-F60094 mark 
             AND sfai12=l_sfs06                        #MOD-F60094 add
...
#FUN-A70125 --end--
    ...
    LET l_sfa062_s = sfa062_t   #CHI-E20031 add #先把超領量額外存放，以免值被變動，此為超領退+超領單的量
...
#FUN-B80129--begin--add-----
    IF l_sfp.sfp06 MATCHES '[49]' THEN  
       IF l_used <= l_sfa.sfa05 THEN
          IF p_code = 'z' THEN
            #LET sfa06_t = l_used + l_sfa.sfa064              #CHI-E20031 mark
            #LET sfa062_t = 0                                 #CHI-E20031 mark
             LET sfa06_t = l_used + l_sfa.sfa064 - l_sfa062_s #CHI-E20031 add #再多扣除超領單(asfi512)的數量
             LET sfa062_t = l_sfa062_s                        #CHI-E20031 add
          ELSE
            #LET sfa06_t  = l_used                            #CHI-E20031 mark
            #LET sfa062_t = 0                                 #CHI-E20031 mark
             LET sfa06_t  = l_used - l_sfa062_s               #CHI-E20031 add
             LET sfa062_t = l_sfa062_s                        #CHI-E20031 add
          END IF
       ELSE
          LET sfa06_t = l_sfa.sfa05                           #CHI-E20031 mark
          LET sfa062_t= l_used - l_sfa.sfa05                  #CHI-E20031 mark
       END IF
    END IF
#FUN-B80129--end--add----
   
    IF sfa06_t > l_sfa.sfa05 AND g_user<>'tiptop' AND  #FUN-B50059
   ...
   #FUN-B50059 mark (E)
    ...
    #CHI-E20031-Start-Add
   #IF sfa062_t > 0 THEN #超領量 > 0                          #MOD-FA0123 mark
    IF sfa062_t > 0 AND g_b_confirm <> 'A' THEN #超領量 > 0   #MOD-FA0123 add aimp880會將g_b_confirm給'A',aimp880盤點過帳時不控卡此段,
       IF sfa06_t <= 0 THEN 
          IF l_sfp.sfp06 = '2' THEN 
             #MOD-E70014 add----------------------------------------------------
             LET l_sfa06 = sfa06_t
             IF l_sfa.sfa26 MATCHES '[34SU]' THEN
                SELECT SUM(sfa06) INTO l_sfa06 FROM sfa_file
                 WHERE sfa01 = l_sfs.sfs03
                   ...
                   AND sfa012= l_sfs.sfs012
                   AND sfa013= l_sfs.sfs013
               #LET sfa06_t = l_sfa06                     #MOD-F70131 mark
             END IF
             IF l_sfa06 <= 0 THEN
             #MOD-E70014 add end------------------------------------------------
               #CALL s_errmsg('sfs04',l_sfs.sfs04,'','asf1181',1)   #已發數量需>0，才可做超領     #MOD-FA0136 mark
                CALL cl_err(l_sfs.sfs04,'asf1181',1)                                              #MOD-FA0136 add
                LET g_success='N' #MOD-E70014
                RETURN            #MOD-FA0136 add
             END IF               #MOD-E70014
          ELSE    
            #CALL s_errmsg('sfs04',l_sfs.sfs04,'','asf1180',1)   #已發數量(sfa06)>0,才可做超領,因該料件已有超領單,過帳還原後會導致已發數量(sfa06)<=0,故不可過帳還原!  #MOD-FA0136 mark
             CALL cl_err(l_sfs.sfs04,'asf1180',1)                                                  #MOD-FA0136 add
             LET g_success='N' #MOD-E70014
             RETURN            #MOD-FA0136 add
          END IF
         #LET g_success='N'    #MOD-E70014 mark
       END IF 
    END IF 
    #CHI-E20031-End-Add
    ...
    IF STATUS OR SQLCA.sqlerrd[3]=0 THEN
       ...
       LET g_success='N' RETURN
    END IF
    IF g_prog = 'asfi527' THEN
    #add by lixwz200911 s---
      LET l_str = " asfi527 超领退料过账 sfa06 已发数量 ",sfa06_t," sfa062  超领数量 ",sfa062_t ,"\n"
      RUN " echo '"||l_str||"' >> /u1/topprod/tiptop/asf/4gl/asfi527_lixwz.log"
    #add by lixwz200911 e---
    END IF
    #add by zhangsba201014 s---
    IF g_prog = 'asfi514' THEN
      #mod by zhangsba201107 --s
      CASE 
        WHEN g_action_choice = "stock_post"     
          LET l_str = " asfi514 工单领料过账 账套  ",g_plant," sfa01 工单编号 ",l_sfs.sfs03," sfa03 发料料号 ",l_sfs.sfs04," sfa12 单位 ",l_sfs06," sfs06 单位 ",l_sfs.sfs06," sfa08 作业编号 ",l_sfs.sfs10," sfa27 被替代料号 ",l_sfs.sfs27," sfs05 发料量 ",l_sfs.sfs05," sfa06 已发数量 ",sfa06_t," sfa062  超领数量  ",sfa062_t ," sfa012 ",l_sfs.sfs012 ," sfa013 ",l_sfs.sfs013 ,"\n"
          RUN " echo '"||l_str||"' >> /u1/topprod/tiptop/asf/4gl/asfi514_zhangsba_post.log"
        WHEN g_action_choice = "undo_post"   
          LET l_str = " asfi514 工单领料过账还原 账套  ",g_plant," sfa01 工单编号 ",l_sfs.sfs03," sfa03 发料料号 ",l_sfs.sfs04," sfa12 单位 ",l_sfs06," sfs06 单位 ",l_sfs.sfs06," sfa08 作业编号 ",l_sfs.sfs10," sfa27 被替代料号 ",l_sfs.sfs27," sfs05 发料量 ",l_sfs.sfs05," sfa06 已发数量 ",sfa06_t," sfa062  超领数量  ",sfa062_t ," sfa012 ",l_sfs.sfs012 ," sfa013 ",l_sfs.sfs013 ,"\n"
          RUN " echo '"||l_str||"' >> /u1/topprod/tiptop/asf/4gl/asfi514_zhangsba_unpost.log"
      END CASE
      #mod by zhangsba201107 --e
    #add by zhangsba201014 e---
...
#  DEFINE l_imaicd13   LIKE imaicd_file.imaicd13  #FUN-BA0050
   ...
   DEFINE l_cnt1        LIKE type_file.num10  #add by zhangzs 210113
...
#        END IF #MOD-DB0006
     ...
     #------------No:CHI-B50041  add
         ...
         #取替代料控管
            IF l_sfs.sfs26 MATCHES '[US]' AND g_sma.sma107 = 'Y' THEN        
               ...
               IF l_sfa05_1 - l_sfa06_1 < l_sfs.sfs05/l_sfa28 AND g_user<>'tiptop' THEN
     ...
     IF g_prog='asfi511' OR g_prog='asfi513' OR g_prog='asfi526' THEN
        ...
        SELECT COUNT(*) INTO l_cnt1 FROM sfq_file,tsc_file              #add by zhangzs210113-S     atmt260产生的工单排除                                                         
        WHERE sfq02=tsc19
        IF cl_null(l_cnt1) THEN LET l_cnt1 = 0 END IF                   #add by zhangzs210113-E
        
        IF l_cnt > 0 AND l_cnt1 = 0 THEN            #add l_cnt1 by zhangzs210113-E
...
#          IF l_min_set < 0 THEN                        #FUN-960037 --- mark
         #IF (l_min_set - l_qty) < 0 THEN               #FUN-960037 --- add   #MOD-C80038 mark
          IF (l_min_set - l_qty + l_ecm315) < 0 AND g_user<>'tiptop' THEN                          #MOD-C80038 add 
             IF p_cmd = 's' THEN 
   ...
   #str-----mark by guanyao160819 #remark by maoyy20220127
...
#MOD-B90008--end--
        ...
        #str-----mark by guanyao160819  临时取消 #remark by maoyy20220127
         IF (l_min_set - l_qty) < 0 AND g_sfa11 <> 'E' AND g_sfa11 <> 'S' THEN   #MOD-CC0198 add
           IF p_cmd = 's' THEN 
              CALL cl_err(p_sfb.sfb01||"|"||p_sfs.sfs04,'asf-879',1)  #asf-879:該筆資料過帳後,發料套數會小於工單報工數量! #mod by lixwz220225
           ELSE 
              CALL cl_err(p_sfb.sfb01||"|"||p_sfs.sfs04,'asf-880',1)  #asf-880:該筆資料過帳還原後,發料套數會小於工單報工數量! 
           END IF
           #add by darcy 220225 s---
           IF g_user = '25181' OR g_user = 'tiptop'  THEN 
           ELSE 
            LET g_success="N"
            RETURN
           END IF 
           #add by darcy 220225 e---
         END IF
         #end----mark by guanyao160819
      END IF
      #FUN-960037 -------add end
...
#MOD-C80077 add end-------------------------------
   ...
   END IF
   #2021113001 add----begin----
   IF NOT cl_null(p_sfp01) AND l_sfp.sfp06 MATCHES '[468]' AND cl_getscmparameter() THEN
       INITIALIZE l_ret TO NULL
       IF l_sfp.sfp06 = '4' THEN
         CALL cjc_zmx_json_cancelWorktask(p_sfp01,'DK1') RETURNING l_ret.*
         ELSE 
         CALL cjc_zmx_json_cancelWorktask(p_sfp01,'OR1') RETURNING l_ret.*
       END IF 
       IF l_ret.success = 'Y' THEN
       ELSE
          IF cl_null(l_ret.msg) THEN
          	IF l_sfp.sfp06 = '4' THEN 
              LET l_ret.msg = "工单倒扣发料(",p_sfp01 CLIPPED,")撤销失败"
              ELSE 
              LET l_ret.msg = "工单退料(",p_sfp01 CLIPPED,")撤销失败"
            END IF 
          END IF
          CALL cl_err(l_ret.msg,'!',1)
          RETURN
...
#         l_imaqty LIKE ima_file.ima262,
   ...
   DEFINE l_d      DATE
    ...
    END IF
    IF g_prog = 'atmt260' THEN   #add by zhangzs 210113
    CALL s_upimg(l_sfe.sfe07,p_ware,p_loca,p_lot,u_type,p_qty*p_factor,g_today, #FUN-8C0084  #add by zhanzgs 210113
                 '','','','',l_sfe.sfe02,l_sfe.sfe28,   #No.CHI-860032  #add by zhanzgs 210113
                 '','','','','','','','','','','','abcdefg')  #add by zhanzgs 210113
    ELSE   #add by zhanzgs 210113
...
DEFINE p_sfp03    LIKE sfp_file.sfp03      #add by guanyao170110
   ...
   #BEGIN WORK 
   LET g_success = 'Y'
   SELECT smyslip INTO l_slip FROM smy_file WHERE smysys = 'aim' AND smykind ='4' AND smyacti='Y'
...
--
    ...
    --CREATE TEMP TABLE l_sfs_tmp(
       --sfs01_tmp      LIKE sfs_file.sfs01,
       --sfs02_tmp      LIKE sfs_file.sfs02,
       --sfs04_tmp      LIKE sfs_file.sfs04,
       --sfs05_tmp      LIKE sfs_file.sfs05,
       --sfsud07_tmp    LIKE sfs_file.sfsud07)    
--
