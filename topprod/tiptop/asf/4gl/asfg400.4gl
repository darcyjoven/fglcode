...
#No.FUN-750101 --begin --
   ...
   LET l_table=cl_prt_temptable('asfg400',g_sql)
...
#----------------------No.TQC-670055 end
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'   # If background job sw is off
      THEN CALL asfg400_tm()        # Input print condition
      ELSE CALL asfg400()              # Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW asfg400_w AT p_row,p_col WITH FORM "asf/42f/asfg400"
...
#------------No.TQC-670055 end
     ...
     IF g_bgjob = 'Y' THEN
        ...
        ELSE
           ...
           CALL cl_cmdat('asfg400',g_time,l_cmd)    # Execute cmd at later time
        END IF
        CLOSE WINDOW asfg400_w
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
...
#No.FUN-750101 --end--
    ...
    #str MOD-A70016 add
     ...
     PREPARE asfg400_pre_wip1 FROM g_sql
     LET g_sql = "SELECT SUM(l_qty) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                 " WHERE sfb01 = ? AND tlf01!=? AND sfa03=?"
     PREPARE asfg400_pre_wip2 FROM g_sql
    #end MOD-A70016 add
     ...
     PREPARE asfg400_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
         ...
         EXIT PROGRAM
     END IF
     DECLARE asfg400_curs1 CURSOR FOR asfg400_prepare1
 
     #CALL cl_outnam('asfg400') RETURNING l_name    #No.FUN-750101 
     #START REPORT asfg400_rep TO l_name            #No.FUN-750101 
 
     #LET g_pageno = 0                              #No.FUN-750101 
     FOREACH asfg400_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0  THEN
       ...
       #------No.TQC-5A0054 add
          #IF l_chr ='Y' THEN #MOD-670011 OR (l_chr ='N' AND sr.sfa27 !=sr.sfb05) THEN   #MOD-8B0140 mark
           IF l_chr ='Y' OR (l_chr ='N' AND sr.sfa27 !=sr.sfb05) THEN                    #MOD-8B0140
              #OUTPUT TO REPORT asfg400_rep(sr.*,sr1.*,l_qty,l_wip)                          #No.FUN-750101
...
#------------------------------No.TQC-670055 end----------------------------
         ...
         #當不同元件替代成同一個料時,在製量的計算有問題,需重算
          ...
          EXECUTE asfg400_pre_wip1 USING sr.sfb01,sr.sfb05,sr.sfa03 INTO l_wip_1
          EXECUTE asfg400_pre_wip2 USING sr.sfb01,sr.sfb05,sr.sfa03 INTO l_wip_2
...
###GENGRE###        LET g_str=tm.wc                                                          #No.FUN-750101
     END IF                                                                      #No.FUN-750101
###GENGRE###     LET l_sql="SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED              #No.FUN-750101
     IF g_sma.sma541 <> 'Y' THEN
###GENGRE###        CALL cl_prt_cs3("asfg400","asfg400",l_sql,g_str)                            #No.FUN-750101
        LET g_template = 'asfg400'                                               #FUN-CB0072 add
        CALL asfg400_grdata()    ###GENGRE###
     ELSE
###GENGRE###        CALL cl_prt_cs3("asfg400","asfg400_1",l_sql,g_str)                       #FUN-A60027 add
        LET g_template = 'asfg400_1'                                             #FUN-CB0072 add
        CALL asfg400_grdata()    ###GENGRE###
     END IF                                                                      #FUN-A60027 add
     #FINISH REPORT asfg400_rep                                                  #No.FUN-750101
...
REPORT asfg400_rep(sr,sr1,l_qty,l_wip)
...
FUNCTION asfg400_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("asfg400")
        IF handler IS NOT NULL THEN
            START REPORT asfg400_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                       ," ORDER BY sfb01,sfa03"              #FUN-CB0072 add
          
            DECLARE asfg400_datacur1 CURSOR FROM l_sql
            FOREACH asfg400_datacur1 INTO sr1.*
                OUTPUT TO REPORT asfg400_rep(sr1.*)
            END FOREACH
            FINISH REPORT asfg400_rep
        END IF
...
REPORT asfg400_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE sr1_o sr1_t       #FUN-CB0072 add
    DEFINE l_lineno LIKE type_file.num5
   #FUN-CB0072--add--str---
    DEFINE l_qty_1    STRING               
    DEFINE l_sfa064_1 STRING              
    DEFINE l_wip_1    STRING              
    DEFINE l_str_label  STRING            
    DEFINE l_wip_label  STRING            
    DEFINE l_sfa03_1  LIKE sfa_file.sfa03
    DEFINE l_ima02_1  LIKE ima_file.ima02
    DEFINE l_ima021_1 LIKE ima_file.ima021
    DEFINE l_sfa12  LIKE sfa_file.sfa12
   #FUN-CB0072--add--end---
    
    ORDER EXTERNAL BY sr1.sfb01,sr1.sfa03
    ...
    FORMAT
        ...
        ON EVERY ROW
           ...
           #FUN-CB0072--add--str---
            IF sr1.tlf13 = 'asft670' THEN 
               LET l_qty_1 = sr1.l_qty,"(-)"
               LET l_qty_1 = l_qty_1.trim()
            ELSE 
               IF sr1.tlf907 = 1 THEN
                  LET l_qty_1 = sr1.l_qty,"(-)"
                  LET l_qty_1 = l_qty_1.trim()
               ELSE
                  IF sr1.tlf907 = -1 THEN
            ...
            IF sr1_o.sfb01 = sr1.sfb01 THEN
               IF NOT cl_null(sr1_o.sfa03) THEN
                  IF sr1_o.sfa03 = sr1.sfa03 THEN
                     LET l_sfa03_1 = ' '
                  ELSE
                     LET l_sfa03_1 = sr1.sfa03
                  END IF
               ELSE
                  LET l_sfa03_1 = sr1.sfa03
               END IF
            ...
            IF sr1_o.sfa03 = sr1.sfa03 THEN
               IF NOT cl_null(sr1_o.l_ima02) THEN
                  IF sr1_o.l_ima02 = sr1.l_ima02 THEN
                     LET l_ima02_1 = ' '
                  ELSE 
                     LET l_ima02_1 = sr1.l_ima02
                  END IF
               ELSE
                  LET l_ima02_1 = sr1.l_ima02
               END IF
            ...
            IF sr1_o.sfa03 = sr1.sfa03 THEN
               IF NOT cl_null(sr1_o.l_ima021) THEN
                  IF sr1_o.l_ima021 = sr1.l_ima021 THEN
                     LET l_ima021_1 = ' '
                  ELSE
                     LET l_ima021_1 = sr1.l_ima021
                  END IF
               ELSE
                  LET l_ima021_1 = sr1.l_ima021
               END IF
            ...
            IF sr1_o.sfa03 = sr1.sfa03 THEN
               IF NOT cl_null(sr1_o.sfa12) THEN
                  IF sr1_o.sfa12 =  sr1.sfa12 THEN
                     LET l_sfa12 = ' '
                  ELSE
                     LET l_sfa12 = sr1.sfa12
                  END IF
               ELSE
                  LET l_sfa12 = sr1.sfa12
               END IF
            ...
            PRINTX l_sfa03_1,l_ima02_1,l_ima021_1,l_sfa12   
        ...
        AFTER GROUP OF sr1.sfa03
          #FUN-CB0072--add--str---
           LET l_str_label = cl_gr_getmsg('gre-331',g_lang,1)
           PRINTX l_str_label
           LET l_wip_label = sr1.sfa03,cl_gr_getmsg('gre-331',g_lang,2)
           PRINTX l_wip_label
           
           IF sr1.sfa064 < 0 THEN
              LET l_sfa064_1 = sr1.sfa064,"(-)"
           ELSE 
              IF sr1.sfa064 > 0 THEN
                 LET l_sfa064_1 = sr1.sfa064,"(","+",")"
              ELSE
           ...
           IF sr1.sfa064 != 0 THEN
              LET l_wip_1 = sr1.l_wip + sr1.sfa064
           ELSE
              LET l_wip_1 = sr1.l_wip
           END IF
           PRINTX l_wip_1
          #FUN-CB0072--add--end--- 
