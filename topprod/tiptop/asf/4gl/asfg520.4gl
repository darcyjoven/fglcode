...
# Modify.........: No:FUN-CB0073 12/12/24 By chenjing CR轉GR
...
TYPE sr1_t RECORD
    ...
    qty4 LIKE sfa_file.sfa04,
    unavl_stk LIKE type_file.num15_3,
    avl_stk LIKE type_file.num15_3,
    qty1 LIKE sfa_file.sfa04,
    qty2 LIKE sfa_file.sfa04,
    qty3 LIKE sfa_file.sfa04,
    sfb05 LIKE sfb_file.sfb05,
    l_sfb04 LIKE sfb_file.sfb04,
    l_sfb20 LIKE sfb_file.sfb20,
    l_sfb21 LIKE sfb_file.sfb21,
    l_sfb14 LIKE sfb_file.sfb14,
    l_sfb15 LIKE sfb_file.sfb15,
    l_sfb16 LIKE sfb_file.sfb16,
...
#              "qty3.sfa_file.sfa03,",           #No.MOD-840492
   ...
   LET l_table = cl_prt_temptable('asfg520',g_sql) CLIPPED   # 產生Temp Table
   ...
   ELSE
      CALL asfg520()                            # Read data and create out-file
   END IF
...
FUNCTION r520_tm()
   ...
   OPEN WINDOW r520_w AT p_row,p_col WITH FORM "asf/42f/asfg520"
################################################################################
...
#---TQC-CC0087--add--end---
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('asfg520',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW r520_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM
...
###GENGRE###   LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
   ...
   END IF
###GENGRE###   LET g_str = tm.wc CLIPPED,";",g_zz05 CLIPPED,";",tm.b   #No.MOD-7B0186 add
   
###GENGRE###   CALL cl_prt_cs3('asfg520','asfg520',l_sql,g_str)
    CALL asfg520_grdata()    ###GENGRE###
   #------------------------------ CR (4) ------------------------------#
   #end FUN-710080 add
...
FUNCTION asfg520_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("asfg520")
        IF handler IS NOT NULL THEN
            START REPORT asfg520_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY ",g_cr_db_str CLIPPED,l_table CLIPPED,".ttp02"   #FUN-CB0073
            DECLARE asfg520_datacur1 CURSOR FROM l_sql
            FOREACH asfg520_datacur1 INTO sr1.*
                OUTPUT TO REPORT asfg520_rep(sr1.*)
            END FOREACH
            FINISH REPORT asfg520_rep
        END IF
...
REPORT asfg520_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_str    STRING       #FUN-CB0073 
    DEFINE l_sfb04_desc  STRING  #FUN-CB0073
    DEFINE l_display     LIKE type_file.chr1 #FUN-CB0073
    DEFINE l_display1    LIKE type_file.chr1 #FUN-CB0073

    
    ORDER EXTERNAL BY sr1.ttp02
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.ttp02
     #FUN-CB0073--add--str--
           LET l_sfb04_desc = cl_gr_getmsg('gre-335',g_lang,sr1.l_sfb04)
           PRINTX l_sfb04_desc
           IF cl_null(sr1.l_ima05) AND cl_null(sr1.l_ima08) THEN
              LET l_str = sr1.sfb05
           ELSE
              IF cl_null(sr1.l_ima05) AND NOT cl_null(sr1.l_ima08) THEN
                 LET l_str = sr1.sfb05," ",sr1.l_ima08
              ELSE
                 IF NOT cl_null(sr1.l_ima05) AND cl_null(sr1.l_ima08) THEN
                    LET l_str = sr1.sfb05," ",sr1.l_ima05
                 ELSE
                    LET l_str = sr1.sfb05," ",sr1.l_ima05," ",sr1.l_ima08
                 END IF
              END IF
           END IF
           PRINTX l_str
     #FUN-CB0073--add--end--
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
      #FUN-CB0073--add--str--
            IF tm.b = "N" OR (tm.b = "Y" AND sr1.ttp07>0) THEN 
               LET l_display = "Y"
            ELSE 
               LET l_display = "N"
            END IF
            PRINTX l_display
            IF sr1.l_min>sr1.ttp04 THEN 
               LET l_display1 = "Y"
            ELSE
               LET l_display1 = "N"
            END IF
