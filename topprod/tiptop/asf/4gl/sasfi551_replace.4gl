...
# 
# Pattern name...: sasfi551_replace.4gl 
# Descriptions...: 配方替代 
...
FUNCTION i551_replace_item_no(p_sfs27,p_sfs03,p_sfs10,p_sfs05_sum,p_sfs06)
DEFINE   p_sfs27     LIKE  sfs_file.sfs27,    #被替代料號
   ...
   OPEN WINDOW i551_replace_w  WITH FORM "asf/42f/sasfi551_replace" 
     ATTRIBUTE( STYLE = g_win_style ) 
   CALL cl_ui_locale("sasfi551_replace")
   CALL cl_set_comp_visible("sfs02",FALSE)
   CALL g_new_sfs.clear()
   CALL i551_replace_show()
   CALL i551_replace_menu()
   CLOSE WINDOW i551_replace_w

END FUNCTION

FUNCTION i551_replace_menu() 
  
   WHILE TRUE 
      CALL i551_replace_bp("G") 
...
FUNCTION i551_replace_show()
   ...
   CALL i551_replace_b_fill()   
END FUNCTION 

FUNCTION i551_replace_b()
DEFINE
...
#           IF l_ima918='Y' THEN
#              CALL i551_replace_mod_lot(g_new_sfs[l_ac].ima01,g_old_sfs06,
         ...
         IF g_sfp.sfp06 MATCHES '[12346789]' THEN
            IF p_cmd = 'u' AND NOT cl_null(g_new_sfs[l_ac].sfs07) THEN
               IF g_new_sfs_t.sfs07 <> g_new_sfs[l_ac].sfs07 THEN
                  CALL i551_replace_ind_icd_chk_icd()
...
&endif
         ...
         IF NOT cl_null(g_new_sfs[l_ac].ima01) THEN
            SELECT ima159 INTO l_ima159 FROM ima_file
             WHERE ima01 = g_new_sfs[l_ac].ima01
            IF l_ima159 = '2' THEN
               CASE i551_replace_b_sfs09_inschk(p_cmd)
...
#  END IF
#TQC-C20508 ----- mark ----- end
   CALL i551_replace_write_back()
   COMMIT WORK
   
END FUNCTION 
#資料正確性的檢查
FUNCTION  i551_replace_chk()
DEFINE    l_sum_qty    LIKE   rvbs_file.rvbs06
...
#資料回寫發料單身
FUNCTION i551_replace_write_back()
...
DEFINE   l_rvbs       RECORD LIKE rvbs_file.*
   ...
   PREPARE i551_replace_pre  FROM g_sql
   DECLARE i551_replace_cs  CURSOR FOR  i551_replace_pre
   CALL g_new_sfs.clear()
   LET  g_cnt = 1
   FOREACH   i551_replace_cs INTO g_new_sfs[g_cnt].*
      IF SQLCA.sqlcode THEN 
         CALL cl_err('foreach',SQLCA.sqlcode,0)
         EXIT FOREACH
      END IF 
...
#批序號維護
FUNCTION i551_replace_mod_lot(p_item,p_sfs06,p_sqty,p_new_sfs07,p_new_sfs08,p_new_sfs09)
...
DEFINE   l_n      LIKE  type_file.num5    #TQC-C20568 add
   ...
   OPEN WINDOW i551_mod_lot_w  WITH FORM "asf/42f/sasfi551_mod_lot" 
     ATTRIBUTE( STYLE = g_win_style ) 
   CALL cl_ui_locale("sasfi551_mod_lot")
   CALL i551_mod_lot_refresh_detail()
   ...
   IF l_n = 0 THEN                                                  #TQC-C20568 add
      CALL i551_mod_lot_gen()  #TQC-C20509 add
   ELSE                                                             #TQC-C20568 add
      CALL i551_mod_lot_show()
   END IF                                                           #TQC-C20568 add
   CALL i551_mod_lot_menu()
   CLOSE WINDOW i551_mod_lot_w
   RETURN g_rvbs_srvbs06,g_rvbs_seqty   
END FUNCTION

FUNCTION i551_mod_lot_menu() 
  
   WHILE TRUE 
      CALL i551_mod_lot_bp("G") 
      CASE g_action_choice 
         WHEN "detail" 
            CALL i551_mod_lot_b() 
         WHEN "help" 
         ...
         WHEN "controlg" 
            CALL cl_cmdask() 
         WHEN "gen_data" 
            CALL i551_mod_lot_gen() 
      END CASE 
   END WHILE 
  
END FUNCTION  

FUNCTION i551_mod_lot_show()
...
FUNCTION i551_mod_lot_refresh_detail()
   ...
   INPUT ARRAY g_lot_rvbs WITHOUT DEFAULTS FROM s_lot_rvbs.*
      ...
      BEFORE ROW 
         ...
         IF g_rec_b2 >= l_ac2 THEN
            ...
            CALL i551_mod_lot_entry_b()
         END IF 

      ON CHANGE sel
         #重新計算實際發料量合計和折合量合計
         CALL i551_mod_lot_cal()
...
FUNCTION i551_replace_b_sfs09_inschk(p_cmd)
...
FUNCTION i551_replace_ind_icd_chk_icd()
    LET g_cnt = 0
    LET g_errno = ' '
