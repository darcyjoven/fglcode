...
# Modify.........: No.FUN-CB0001 12/11/09 By yangtt CR轉XtraGrid
# Modify.........: No.FUN-D40020 13/01/31 By chenying 取消GRUP_FIELD 
...
DEFINE l_table          STRING  #FUN-CB0001 add
DEFINE g_str            STRING  #FUN-CB0001 add
DEFINE g_sql            STRING  #FUN-CB0001 add
...
MAIN
   ...
   #FUN-CB0001 -----------------------------(S)
   LET g_sql = "order1.ima_file.ima34,",
               ...
               "flt.ima_file.ima59,",
               "ult.ima_file.ima59,",
               "alot.type_file.num10,",
               "l_num1.type_file.num5,",
               "l_num2.type_file.num5,",
               "l_num3.type_file.num5,",
               "l_str1.type_file.chr1,",
               "l_str2.type_file.chr10,",
               "l_str3.type_file.chr10,",
               "l_str4.type_file.chr10"
   LET l_table = cl_prt_temptable('asfr110',g_sql) CLIPPED
   IF l_table = -1 THEN EXIT PROGRAM END IF
   #FUN-CB0001 -----------------------------(E)

   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL x110_tm(0,0)        # Input print condition
      ELSE CALL x110()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
END MAIN
 
FUNCTION x110_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW x110_w AT p_row,p_col WITH FORM "asf/42f/asfx110"
   ...
   WHILE TRUE
     ...
     CONSTRUCT BY NAME tm.wc ON sfb02,sfb04,sfb01,sfb05,sfb81,sfb13 #FUN-970054 add sfb81 sfb13
       ...
       ON IDLE g_idle_seconds
        ...
        ON ACTION help          #FUN-CB0001
         CALL cl_show_help()    #FUN-CB0001

        ON ACTION controlg      #FUN-CB0001
           CALL cl_cmdask()     #FUN-CB0001
     ...
     IF g_bgjob = 'Y' THEN
        ...
        ELSE
           ...
           CALL cl_cmdat('asfx110',g_time,l_cmd)    # Execute cmd at later time
        END IF
        CLOSE WINDOW x110_w
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
        EXIT PROGRAM
...
#         l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT  #TQC-630166        #No.FUN-680121 VARCHAR(1100)
          ...
          sr  RECORD order1 LIKE ima_file.ima34,  #No.FUN-680121 VARCHAR(10)
                     ...
                     ima02 LIKE ima_file.ima02,       #FUN-CB0001
                     ima021 LIKE ima_file.ima021,     #FUN-CB0001
     ...
     DEFINE l_num1      LIKE type_file.num5           #FUN-CB0001 add
     DEFINE l_num2      LIKE type_file.num5           #FUN-CB0001 add
     DEFINE l_num3      LIKE type_file.num5           #FUN-CB0001 add
     DEFINE l_str1      LIKE type_file.chr1           #FUN-CB0001 add
     DEFINE l_str2      LIKE type_file.chr10          #FUN-CB0001 add
     DEFINE l_str3      LIKE type_file.chr10          #FUN-CB0001 add
     DEFINE l_str4      LIKE type_file.chr10          #FUN-CB0001 add
     ...
     LET l_sql = "SELECT UNIQUE '',",
                 " sfb01, sfb02, '', sfb04, '', sfb05, '', '', sfb23, sfb40,",     #FUN-CB0001 4 ''
     ...
     END IF
     DECLARE x110_c1 CURSOR FOR x110_p1
 
#    LET l_name = 'asfx110.out'
#No.FUN-590110 --start--
#    CALL cl_outnam('asfx110') RETURNING l_name  #FUN-CB0001 mark
     IF tm.d='N' THEN
    ...
    END IF
    CALL cl_prt_pos_len()
#   START REPORT x110_rep TO l_name   #FUN-CB0001
#No.FUN-590110 --end--
     #今天 - 30 再將工作日置入 array 中以備 s_ofday(起日,止日)算工作日
     CALL s_filldate()
     LET g_pageno = 0
     FOREACH x110_c1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
       ...
       #FUN-CB0001---add--str--
       CASE sr.sfb02
          WHEN '1' LET sr.sfb02_1 = cl_getmsg('asf-841',g_lang)
          WHEN '5' LET sr.sfb02_1 = cl_getmsg('asf-842',g_lang)
          WHEN '7' LET sr.sfb02_1 = cl_getmsg('asf-843',g_lang)
          WHEN '8' LET sr.sfb02_1 = cl_getmsg('asf-856',g_lang)
          WHEN '11' LET sr.sfb02_1 = cl_getmsg('asf-853',g_lang)
          WHEN '13' LET sr.sfb02_1 = cl_getmsg('asf-843',g_lang)
          WHEN '15' LET sr.sfb02_1 = cl_getmsg('asf-855',g_lang)
       END CASE
       CASE sr.sfb04
          WHEN '1' LET sr.sfb04_1 = cl_getmsg('asf-845',g_lang)
          WHEN '2' LET sr.sfb04_1 = cl_getmsg('asf-846',g_lang)
          WHEN '3' LET sr.sfb04_1 = cl_getmsg('asf-847',g_lang)
          WHEN '4' LET sr.sfb04_1 = cl_getmsg('asf-848',g_lang)
          WHEN '5' LET sr.sfb04_1 = cl_getmsg('asf-849',g_lang)
          WHEN '6' LET sr.sfb04_1 = cl_getmsg('asf-850',g_lang)
          WHEN '7' LET sr.sfb04_1 = cl_getmsg('asf-851',g_lang)
          WHEN '8' LET sr.sfb04_1 = cl_getmsg('asf-852',g_lang)
       END CASE
       ...
       IF tm.d='N' THEN
         # OUTPUT TO REPORT x110_rep(sr.*)    #FUN-CB0001 mark
           LET sr.sfb34 = sr.sfb34 USING '--,---,---,---.&&&&'    #FUN-CB0001 add
           LET l_num1 = 3    #FUN-CB0001
           LET l_num2 = 0    #FUN-CB0001
           LET l_num3 = 4    #FUN-CB0001
           LET l_str1 = '%'  #FUN-CB0001
           #FUN-CB0001----add--str
           IF sr.sfb23 = 'Y' OR sr.sfb23 = 'y' THEN
              LET l_str4 = cl_getmsg('asf-562',g_lang)
           ELSE
              LET l_str4 = cl_getmsg('asf-563',g_lang)
           END IF
           LET l_str2 = cl_getmsg('asf-560',g_lang)
           LET l_str3 = cl_getmsg('asf-561',g_lang)
           #FUN-CB0001----add--end
           EXECUTE insert_prep USING sr.*,l_num1,l_num2,l_num3,l_str1,l_str2,l_str3,l_str4  #FUN-CB0001 add
           CONTINUE FOREACH
       END IF
...
##
     ...
     #IF g_sma.sma54 = 'N'   THEN
     IF tm.d  ='N' THEN
       #FINISH REPORT x110_rep            #FUN-CB0001 mark 
        LET l_name = 'asfx110'            #FUN-CB0001
     ELSE
        CASE
                WHEN g_sma.sma26 = '1'
                #FINISH REPORT x110_rep     #FUN-CB0001 mark
                 LET l_name = 'asfx110_1'   #FUN-CB0001
            OTHERWISE
                #FINISH REPORT x110_rep     #FUN-CB0001 mark
                 LET l_name = 'asfx110_2'   #FUN-CB0001 
        END CASE
     END IF
    #CALL cl_prt(l_name,g_prtway,g_copies,g_len)   #FUN-CB0001 mark
     #FUN-CB0001 add str
     LET g_xgrid.table = l_table
     IF g_zz05 = 'Y' THEN   
         CALL cl_wcchp(tm.wc,'sfb02,sfb04,sfb01,sfb05,sfb15') 
              RETURNING tm.wc
     END IF
     LET g_xgrid.template = l_name
     LET g_xgrid.order_field = cl_get_order_field(tm.b,"sfb40,sfb13,sfb15,sfb34")
#FUN-D40020--mark--
#    IF tm.d  ='N' THEN
#       LET g_xgrid.grup_field = 'sfb01'
#    ELSE
#       LET g_xgrid.grup_field = 'sfb01,ecb03'
#    END IF
#FUN-D40020--mark--
     CALL cl_xg_view()
END FUNCTION
   ...
   ON EVERY ROW
    IF tm.d='N' THEN
      ...
      PRINTX name=D1
            ...
            COLUMN g_c[44],sr.sfb02 USING "####",
            COLUMN g_c[45],sr.sfb04 USING "####",
      ...
      IF l_sw = 'Y' THEN
        ...
        END IF
            ...
            PRINTX name=D1
                  ...
                  COLUMN g_c[57],sr.sfb02 USING "####",
                  COLUMN g_c[58],sr.sfb04 USING "####",
