  ...
  CASE WHEN g_wc2 = " 1=1" AND g_wc3 = " 1=1"
         ...
         WHEN g_wc2 = " 1=1" AND g_wc3 <> " 1=1"
              LET g_sql = "SELECT UNIQUE sfp01 ",                              #09/10/21 xiaofeizhu Add
                          ...
                          #--
              #FUN-B70061 --START--
              IF g_wc3.getindexof('sfsi',1)>0 THEN                 
                 CALL cl_replace_str(l_buf, "sfsiicd", "sfeiicd") 
                  RETURNING l_buf_rpl
                 LET g_sql = "SELECT UNIQUE sfp01 ",                             
                             "  FROM sfp_file, sfs_file, sfsi_file",
                             " WHERE sfp01 = sfs01",
                             "   AND sfs01 = sfsi01 AND sfs02 = sfsi02",
                             "   AND ", g_wc CLIPPED, " AND ",g_wc3 CLIPPED,
                             " UNION " ,
                             " SELECT UNIQUE sfp01 ",                             
                             " FROM sfp_file,sfe_file,sfei_file ",
                             " WHERE sfp01 = sfe02 ",
                             " AND sfe02 = sfei02 AND sfe28 = sfei28 ",
                             " AND sfp04 ='Y' ",
                             " AND ",g_wc CLIPPED, " AND ",l_buf_rpl CLIPPED,
                             " ORDER BY sfp01 "
              END IF 
              #FUN-B70061 --END--
         WHEN g_wc2 <> " 1=1" AND g_wc3 <> " 1=1"
              ...
              IF g_wc3.getindexof('sfsi',1)>0 THEN
                 CALL cl_replace_str(l_buf, "sfsiicd", "sfeiicd") 
                  RETURNING l_buf_rpl
                 LET g_sql = "SELECT UNIQUE sfp01 ",                              
                             "  FROM sfp_file, sfs_file, sfq_file, sfsi_file",
                             " WHERE sfp01 = sfs01 AND sfp01=sfq01",
                             "   AND sfs01 = sfsi01 AND sfs02 = sfsi02",
                             "   AND ", g_wc CLIPPED,
                             "   AND ", g_wc2 CLIPPED, " AND ",g_wc3 CLIPPED,
                             " UNION",
                             " SELECT UNIQUE sfp01 ",                              
                             " FROM sfp_file,sfe_file,sfq_file,sfei_file ",
                             " WHERE sfp01 = sfe02 AND sfp01 = sfq01 ",
                             " AND sfe02 = sfei02 AND sfe28 = sfei28 ",
                             " AND sfp04 ='Y' ",
                             " AND ",g_wc CLIPPED,
                             " AND ",g_wc2 CLIPPED," AND ",l_buf_rpl CLIPPED,
                             " ORDER BY sfp01 "
              END IF 
              #FUN-B70061 --END--
    END CASE
...
FUNCTION i501_menu()
   DEFINE li_cnt  LIKE type_file.num5          #記錄發/退料單身筆數#No.FUN-810016 add 
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         WHEN "detail"
            IF cl_chk_act_auth() THEN
#進入發/退料單身之前，當作業為asfi512/513/514/527/528/529,
#即g_argv2 in ['2','3','4','7','8','9']，并且單頭有輸入PBI
#單號且發料底稿單身(sfs*)為空,則自動調用aciton:依PBI發料/
#退料自動分配重新產生單身的功能，否則依據原來的處理不變
               LET li_cnt = 0
               SELECT COUNT(*) INTO li_cnt FROM sfs_file
                WHERE sfs01 = g_sfp.sfp01
               IF g_argv2 MATCHES '[234789]' AND
                  g_sfp.sfp08 IS NOT NULL AND
                  li_cnt = 0 THEN
                  CALL i501_auto_b()
               ELSE
                  CALL i501_b()
...
FUNCTION i501_a()
    ...
    DEFINE li_cnt      LIKE type_file.num5                     #No.FUN-810016
...
FUNCTION i501_chkkey()
    ...
    END IF
   #FUN-B70061 --START--   
   UPDATE sfsi_file SET sfsi01=g_sfp.sfp01 WHERE sfsi01=g_sfp_t.sfp01
   IF STATUS THEN
      CALL cl_err3("upd","sfsi_file",g_sfp_t.sfp01,"",STATUS,"","upd sfsi01",1) 
      LET g_sfp.*=g_sfp_t.* CALL i501_show() ROLLBACK WORK RETURN      
   END IF 
   #FUN-B70061 --END--
...
# 當有耗材產生時,應將asft620之耗材單號清為NULL
        ...
        END IF
        #FUN-B70061 --START--
        IF NOT s_del_sfsi(g_sfp.sfp01,'','') THEN
           #不作處理        
        END IF
        #FUN-B70061 --END--
...
FUNCTION i501_d_i()
   ...
   DEFINE li_cnt        LIKE type_file.num5             #No.FUN-810016 add
            ...
            IF g_sfp.sfp06 MATCHES '[1D]' THEN   #FUN-C70014 add 'D'
               ...
               ELSE
                 ...
                 #CALL i501_sfq03_chk1(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012) #只选过账的,用于预设值
                  CALL i501_sfq03_chk(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012,g_sfq[i].sfq014,'1')  #FUN-BC0060 mod   #FUN-C70014 add sfq014
                       RETURNING qty1,qty2
         ...
         IF g_sfp.sfp06 MATCHES '[1D]' THEN #FUN-C70014 add 'D'
            ...
            #將撈取qty1,qty2的部分整理到函數 i501_sfq03_chk1()處理
             # CALL i501_sfq03_chk1(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012)  #FUN-A70095  #只选过账的,用于预设值
               CALL i501_sfq03_chk(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012,g_sfq[i].sfq014,'1')  #FUN-A70095  #FUN-BC0060 mod  #FUN-C70014 add sfq014
...
FUNCTION i501_g_b()
 ...
 #DEFINE l_qty           LIKE sfq_file.sfq03    #NO.FUN-A20048  #FUN-AC0074 mark
  ...
  IF issue_type='0' THEN
     DELETE FROM sfs_file WHERE sfs01=g_sfp.sfp01
     IF NOT s_del_sfsi(g_sfp.sfp01,'','') THEN END IF   #FUN-B70061         
  ...
  END IF      #FUN-940039 add
   ...
   LET g_wc4 = ' 1=1'
   IF (issue_type MATCHES '[13456]') AND (NOT g_sfp.sfp06 MATCHES '[ABC]') THEN  #MOD-9C0195 add #FUN-D10073 add 6
  ...
  IF NOT (g_sfp.sfp06 MATCHES '[ABC]') THEN  #FUN-5C0114
      ...
      FOREACH i501_g_b_c INTO b_sfq.*
       ...
       #      AND sfq01=sfp01 AND sfp06='6' AND sfp04='Y'
       #END IF
        CALL i501_sfq03_chk(b_sfq.sfq02,b_sfq.sfq04,'',b_sfq.sfq014,'1')  #FUN-A70095  #FUN-BC0060 mod 只选过账的 #FUN-C70014 add sfq014
                    RETURNING qty1,qty2
       #Mod FUN-BC0060--end
...
FUNCTION i501_chk_img(l_mai_ware,l_mai_loc,l_wip_ware,l_wip_loc,l_lot_no,l_sie_flag)	# 依 issue_qty1 尋找 img_file可用資料  #FUN-AC0074     #FUN-B80086  main改成mai
    ...
    DEFINE l_sfcislk01  LIKE sfci_file.sfcislk01   # No.FUN-870117
    ...
    IF issue_type = '6' THEN
       LET l_sql=l_sql CLIPPED," ,img18 desc"
    END IF 
    #FUN-D10073---end
    END IF  #FUN-D10073
...
#NO.TQC-B90236 ------add----end
    ...
    #FUN-B70061 --START--
    LET g_forupd_sql = " SELECT * FROM sfsi_file ",
                          " WHERE sfsi01= ? AND sfsi02= ?  FOR UPDATE"
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    DECLARE i501_bcl_ind CURSOR FROM g_forupd_sql
    #FUN-B70061 --END--
...
#No.TQC-B90236-------add-----------end
               ...
               ELSE
                  ...
                  ELSE
                     #FUN-B70061 --START--
                     OPEN i501_bcl_ind USING g_sfp.sfp01,g_sfs_t.sfs02
                     IF STATUS THEN
                        CALL cl_err("OPEN i501_bcl_ind:", STATUS, 1)
                        LET l_lock_sw = "Y"
                     ELSE 
                        FETCH i501_bcl_ind INTO b_sfsi.*
                        IF SQLCA.sqlcode THEN
                           CALL cl_err('lock sfsi',SQLCA.sqlcode,1)
                           LET l_lock_sw = "Y"
                        END IF 
                     END IF
                     #FUN-B70061 --END--
                  ...
                  IF g_sma.sma899 = 'Y' THEN 
                     ...
                     ELSE
                       #LET l_qty = (((l_sfa06-g_sfs[l_ac].sfs05-l_sfs05)/l_sfa05) * l_sfb08 * (1+l_sfa100/100))             #MOD-B20062 mark
                        ...
                        LET l_qty = (((l_sfa06-(g_sfs[l_ac].sfs05/l_sfa28)-l_sfs05)/l_sfa05) * l_sfb08 * (100+l_sfa100/100)) #MOD-D50178
                        #已發料(扣除退料數) - 入庫數(含FQC)
...
#TQC-C10033 --unmark
                ...
                END IF
                #FUN-B70061 --START--
                IF NOT s_del_sfsi(g_sfp.sfp01,g_sfs_t.sfs02,'') THEN
                   ROLLBACK WORK
                   CANCEL DELETE
                END IF
                #FUN-B70061 --END--
            ...
            ELSE
             LET l_cnt=0
              ...
              END IF
               ...
               ELSE
               #FUN-B70061 --START--
                  UPDATE sfsi_file SET * = b_sfsi.*
                   WHERE sfsi01=g_sfp.sfp01 AND sfsi02=g_sfs_t.sfs02
               END IF
               IF SQLCA.sqlcode THEN
                  CALL cl_err3("upd","sfsi_file",g_sfp.sfp01,g_sfs_t.sfs02,SQLCA.sqlcode,"","upd sfsi",1)
                  LET g_sfs[l_ac].* = g_sfs_t.*
                  ROLLBACK WORK  
               ELSE    
               #FUN-B70061 --END--
...
#FUN-C70014 add end -----------------------------
        ...
        ON ACTION qry_issue_item
           ...
           LET g_qryparam.default3 = b_sfs.sfs28
           LET g_qryparam.arg1 = g_sfs[l_ac].sfs03
           CALL cl_create_qry() RETURNING g_sfs[l_ac].sfs04,g_sfs[l_ac].sfs27,b_sfs.sfs28  #FUN-940039 add
           NEXT FIELD sfs04
...
FUNCTION i501_b_fill(p_wc2)              #BODY FILL UP
    ...
    IF g_sfp.sfp04='N' THEN 
       LET g_sql =
        "SELECT sfs02,sfs26,sfs28,sfs014,sfs03,sfs27,sfs04,",  
        "       ima02,ima021,sfs012,'',sfs013,sfs06,sfs10,(sfa05-sfa065),sfa06,'',",    #CHI-BC0040 add sfs28   ,  sfs27與sfs04交換 #FUN-C70014 add sfs014
        "       sfs07,sfs08,sfs09,sfs05,sfs33,sfs34,sfs35,sfs30,sfs31,sfs32,",
        "       sfs21,img10,0,sfs930,'',sfs36,'',sfs37,azf03 ",        #FUN-CB0087 add sfs37,azf03
        "       ,sfsud01,sfsud02,sfsud03,sfsud04,sfsud05,",
        "       sfsud06,sfsud07,sfsud08,sfsud09,sfsud10,",
        "       sfsud11,sfsud12,sfsud13,sfsud14,sfsud15",
        "       ,sfsiicd028,sfsiicd029", 
        ",sfa07",  
        " FROM sfs_file LEFT OUTER JOIN sfa_file ON sfs03=sfa01 AND sfs04=sfa03 AND sfs06=sfa12 AND sfs10=sfa08 AND sfs27=sfa27",   
        "  AND sfs012 = sfa012 AND sfs013 = sfa013 ",
        " LEFT OUTER JOIN sfsi_file ON sfs01 = sfsi01 AND sfs02 = sfsi02 ", 
        " LEFT OUTER JOIN img_file ON sfs04=img01 AND sfs07=img02 AND sfs08=img03 AND sfs09=img04",                                 
        " LEFT OUTER JOIN ima_file ON sfs04=ima01 ",           
    ...
    ELSE
       LET g_sql =
        "SELECT sfe28,sfe26,sfa28,sfe014,sfe01,sfe27,sfe07,",
        "       ima02,ima021,sfe012,'',sfe013,sfe17,sfe14,(sfa05-sfa065),sfa06,'',",      #CHI-BC0040 add sfa28  ,  sfe27與sfe07交換  #FUN-C70014 add sfe014
        "       sfe08,sfe09,sfe10,sfe16,sfe33,sfe34,sfe35,sfe30,sfe31,sfe32,",
        "       sfe11,img10,'',sfe930,'',sfe36,'',sfe37,azf03 ",       #FUN-CB0087 #FUN-D50017 sfe37 
        #"       ,'','','','','',", #FUN-CB0043
        #"       '','','','','',",  #FUN-CB0043
        #"       '','','','','' ",  #FUN-CB0043
        ...
        "       ,sfeiicd028,sfeiicd029", 
        ",''",                                   
        " FROM sfe_file LEFT OUTER JOIN sfa_file ON sfe01=sfa01 AND sfe07=sfa03 AND sfe17=sfa12 AND sfe14=sfa08 AND sfe27=sfa27",   
        "  AND sfe012 = sfa012 AND sfe013 = sfa013 ",
        " LEFT OUTER JOIN sfei_file ON sfe02 = sfei02 AND sfe28 = sfei28 ", 
        " LEFT OUTER JOIN img_file ON sfe07=img01 AND sfe08=img02 AND sfe09=img03 AND sfe10=img04 ",                                
        " LEFT OUTER JOIN ima_file ON sfe07=ima01 ",  
...
FUNCTION i501_out()
    ...
    IF g_sfp.sfp06 MATCHES '[ABC]' THEN #FUN-630001 #TQC-650053
       IF g_argv1='1' THEN
         #LET l_prog='asrr210' #FUN-D60005 mark
          LET l_prog='asrg210' #FUN-D60005
       ELSE
         #LET l_prog='asrr220' #FUN-D60005 mark
          LET l_prog='asrg220' #FUN-D60005
       END IF
    ELSE
       IF g_argv1='1' THEN
...
DEFINE  l_cnt        LIKE type_file.num5    #No.FUN-680121 SMALLINT
DEFINE  l_imni       RECORD LIKE imni_file.*   #FUN-B70061 ch
         ...
         IF g_sfp.sfp06 = '8' THEN    #一般退料
            ...
            IF g_sma.sma899 = 'Y' THEN 
               ...
               ELSE
                  ...
                  LET l_qty = (((l_sfa06-g_sfs[l_ac].sfs05-l_sfs05)/l_sfa05) * l_sfb08 * (100+l_sfa100/100))  #MOD-D50178
                  #已發料(扣除退料數) - 入庫數(含FQC)
                  LET l_qty = l_qty - (l_sfb09+l_sfb11)
               END IF        #No:MOD-970298 add
               #MOD-BA0193 --begin
...
END FUNCTION
FUNCTION i501_auto_b()
    DEFINE l_ta           DYNAMIC ARRAY OF RECORD    #程式變數(Prinram Variables)
                          ta1 LIKE type_file.chr50,  #料號
                          ta2 LIKE type_file.chr1000,#品名
                          ta3 LIKE type_file.chr4,   #單位
                          ta4 LIKE type_file.chr6,   #作業編號
                          ta5 LIKE type_file.chr10,  #倉庫
                          ta6 LIKE type_file.chr10,  #儲位
                          ta7 LIKE type_file.chr30,  #批號
                          ta8 LIKE type_file.num20_6 #發/退料量
                          END RECORD
    DEFINE l_ta_t         RECORD
                          ta1 LIKE type_file.chr50,  #料號
                          ta2 LIKE type_file.chr1000,#品名
                          ta3 LIKE type_file.chr4,   #單位
                          ta4 LIKE type_file.chr6,   #作業編號
    ...
    DEFINE l_row          LIKE type_file.num5        #分段輸入之行，列數
    DEFINE l_col          LIKE type_file.num5        #分段輸入之行，列數
    DEFINE l_n            LIKE type_file.num5        #檢查重復用
    DEFINE l_cnt          LIKE type_file.num5        #檢查重復用
    DEFINE l_cn           LIKE type_file.num5        #檢查重復用
    DEFINE li_cnt         LIKE type_file.num5        #檢查重復用
    ...
    DEFINE t_ta8          LIKE type_file.num20_6
    DEFINE l_ima02        LIKE ima_file.ima02
    DEFINE l_ima63        LIKE ima_file.ima63
    DEFINE t_sfa          DYNAMIC ARRAY OF RECORD 
                          ...
                          END RECORD
    DEFINE t_qty          LIKE sfa_file.sfa06
    DEFINE l_sfa05_06     LIKE sfa_file.sfa05
    DEFINE l_sfa06        LIKE sfa_file.sfa06
    DEFINE l_sfa062       LIKE sfa_file.sfa062
    DEFINE t_sfb09        LIKE sfb_file.sfb09
    ...
    DEFINE l_sfs28        LIKE sfs_file.sfs28
    DEFINE l_str          STRING
    DEFINE l_sql1         STRING
    DEFINE l_sql2         STRING
    DEFINE l_sql3         STRING
    DEFINE l_sql4         STRING
    DEFINE l_rec_b_auto   LIKE type_file.num5            #auto_b單身筆數記錄
 
    LET g_action_choice = ""
    
    OPEN WINDOW i501_ta_w WITH FORM "asf/42f/asfi501ta"
         ATTRIBUTE (STYLE = g_win_style CLIPPED)
    CALL cl_ui_locale("asfi501ta")
    
    IF g_sfp.sfp01 IS NULL THEN 
       CLOSE WINDOW i501_ta_w
       RETURN
    END IF
    SELECT * INTO g_sfp.* FROM sfp_file 
     WHERE sfp01=g_sfp.sfp01
    ...
    CALL cl_opmsg('b')
    DROP TABLE ta_file
    CREATE TEMP TABLE ta_file(
     ta1 LIKE ima_file.ima01,
     ta2 LIKE ima_file.ima02,
     ta3 LIKE sfa_file.sfa12,
     ta4 LIKE sfa_file.sfa08,
     ta5 LIKE img_file.img02,
     ta6 LIKE img_file.img03,
     ta7 LIKE img_file.img04,
     ta8 LIKE sfs_file.sfs05);
    
    CREATE UNIQUE INDEX ta_1 on ta_file(ta1,ta3,ta4);
     
    LET g_forupd_sql = "SELECT * FROM ta_file ",
                       " WHERE ta1 = ? AND ta3 = ? AND ta4 = ? FOR UPDATE"
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    DECLARE i501_auto_bcl CURSOR FROM g_forupd_sql
 
    LET l_allow_insert = cl_detail_input_auth("insert")
    LET l_allow_delete = cl_detail_input_auth("delete")
 
    INPUT ARRAY l_ta WITHOUT DEFAULTS FROM s_ta.*
          ATTRIBUTE(COUNT=l_rec_b_auto,MAXCOUNT=g_max_rec,UNBUFFERED,
                    INSERT ROW=l_allow_insert,DELETE ROW=l_allow_delete,
                    APPEND ROW=l_allow_insert)
 
        BEFORE INPUT
            IF l_rec_b_auto!=0 THEN
               CALL fgl_set_arr_curr(l_ac)
            END IF
        ...
        BEFORE ROW
           ...
           BEGIN WORK
            ...
            IF l_rec_b_auto >= l_ac THEN
               LET p_cmd='u'
               LET l_ta_t.* = l_ta[l_ac].*
               OPEN i501_auto_bcl USING l_ta_t.ta1,l_ta_t.ta3,l_ta_t.ta4
               IF STATUS THEN
                  CALL cl_err("OPEN i501_auto_bcl:", STATUS, 1)
                  LET l_lock_sw = "Y"
               ELSE
                  FETCH i501_auto_bcl INTO l_ta[l_ac].*
                  IF SQLCA.sqlcode THEN
                     CALL cl_err('lock ta',SQLCA.sqlcode,1)
                     LET l_lock_sw = "Y"
                  END IF
               END IF
               CALL cl_show_fld_cont()
        ...
        BEFORE INSERT
            INITIALIZE l_ta_t.* TO NULL
            LET l_n = ARR_COUNT()
            LET p_cmd='a'
            INITIALIZE l_ta[l_ac].* TO NULL            
            CALL cl_show_fld_cont()
            NEXT FIELD ta1
        ...
        AFTER INSERT
            ...
            IF g_argv1 = '1' AND g_argv2 = '3' THEN
               LET l_n = 0 
               SELECT COUNT(*) INTO l_n 
                 FROM sfa_file,sfb_file 
                WHERE sfa_file.sfa01 = sfb_file.sfb01
                  AND sfb_file.sfb85 = g_sfp.sfp08
                  AND sfa_file.sfa03 = l_ta[l_ac].ta1 
                  AND sfa_file.sfa08 = l_ta[l_ac].ta4
                  AND sfa_file.sfa12 = l_ta[l_ac].ta3 
                  AND sfb_file.sfb02 NOT IN ('13','15') 
                  AND sfb_file.sfb04 NOT IN ('1','8')
                  AND sfb_file.sfb01 IN 
                      (SELECT sfq02 FROM sfq_file
                        WHERE sfq01 = g_sfp.sfp01)                       
                  AND sfa_file.sfa11='N'
                  AND sfa_file.sfa26 IN ('0','1','2','5','7')  #FUN-A20037 add '7'
                IF l_n = 0 THEN
                   LET l_str = g_sfp.sfp08," ",l_ta[l_ac].ta1," ",
                               l_ta[l_ac].ta3," ",l_ta[l_ac].ta4," "
                   CALL cl_err(l_str,"auto-01",1)
                   NEXT FIELD ta1
                ELSE
                   SELECT SUM(sfa_file.sfa05-sfa_file.sfa06)   #FUN-B50059
                     INTO l_sfa05_06 
                     FROM sfa_file,sfb_file 
                    WHERE sfa_file.sfa01 = sfb_file.sfb01
                      AND sfb_file.sfb85 = g_sfp.sfp08
                      ...
                      AND sfa_file.sfa11='N'
                      AND sfa_file.sfa26 IN ('0','1','2','5','7')  #FUN-A20037 add '7'
                   IF l_sfa05_06 < l_ta[l_ac].ta8 THEN
                      LET l_str = l_ta[l_ac].ta8,">",l_sfa05_06
                      CALL cl_err(l_str,"auto-02",1)
                   END IF
                END IF                   
            END IF                      
            
            IF g_argv1 = '1' AND g_argv2 = '2' THEN
               LET l_n = 0 
               SELECT COUNT(*) INTO l_n 
                 FROM sfa_file,sfb_file 
                WHERE sfa_file.sfa01 = sfb_file.sfb01
                  ...
                  AND sfb_file.sfb02 NOT IN ('13','15') 
                  AND sfb_file.sfb04 NOT IN ('1','8')
                  AND (sfa_file.sfa06-sfa_file.sfa05) >= 0  #FUN-B50059
                  AND sfa_file.sfa11='N'
                  AND sfa_file.sfa26 IN ('0','1','2','5','7') #FUN-A20037 ddd '7'
                IF l_n = 0 THEN
                   LET l_str = g_sfp.sfp08," ",l_ta[l_ac].ta1," ",
                               l_ta[l_ac].ta3," ",l_ta[l_ac].ta4," "
                   CALL cl_err(l_str,"auto-03",1)
                   NEXT FIELD ta1
                END IF
            END IF
            
            IF g_argv1 = '1' AND g_argv2 = '4' THEN
               LET l_n = 0 
               SELECT COUNT(*) INTO l_n 
                 FROM sfa_file,sfb_file 
                WHERE sfa_file.sfa01 = sfb_file.sfb01
                ...
                ELSE
                   ...
                   IF l_sfa05_06 < l_ta[l_ac].ta8 THEN
                      LET l_str = l_ta[l_ac].ta8,">",l_sfa05_06
                      CALL cl_err(l_str,"auto-05",1)
                      NEXT FIELD ta8
                   END IF      
                END IF
            END IF    
                
            IF g_argv1 = '2' AND g_argv2 = '7' THEN
               LET l_n = 0 
               SELECT COUNT(*) INTO l_n 
                 FROM sfa_file,sfb_file 
                WHERE sfa_file.sfa01 = sfb_file.sfb01
                ...
                IF l_n = 0 THEN
                   LET l_str = g_sfp.sfp08," ",l_ta[l_ac].ta1," ",
                               l_ta[l_ac].ta3," ",l_ta[l_ac].ta4," "
                   CALL cl_err(l_str,"auto-06",1)
                   NEXT FIELD ta1
                ELSE
                   SELECT SUM(sfa_file.sfa062) 
                     INTO l_sfa062 
                     FROM sfa_file,sfb_file 
                    WHERE sfa_file.sfa01 = sfb_file.sfb01
                      AND sfb_file.sfb85 = g_sfp.sfp08
                      ...
                      AND sfb_file.sfb04 NOT IN ('1','8')
                      AND sfa_file.sfa26 IN ('0','1','2','5','7')  #FUN-A20037 add '7'
                   IF l_sfa062 < l_ta[l_ac].ta8 THEN
                      LET l_str = l_ta[l_ac].ta8,">",l_sfa062
                      CALL cl_err(l_str,"auto-07",1)
                      NEXT FIELD ta8
                   END IF      
                END IF
            END IF
            
            IF g_argv1 = '2' AND g_argv2 = '8' THEN
               LET l_n = 0 
               SELECT COUNT(*) INTO l_n 
                 FROM sfa_file,sfb_file 
                WHERE sfa_file.sfa01 = sfb_file.sfb01
                ...
                IF l_n = 0 THEN
                   LET l_str = g_sfp.sfp08," ",l_ta[l_ac].ta1," ",
                               l_ta[l_ac].ta3," ",l_ta[l_ac].ta4," "
                   CALL cl_err(l_str,"auto-08",1)
                   NEXT FIELD ta1
                ELSE
                   SELECT SUM(sfa06-sfb09*sfa161)   #FUN-B50059
                     INTO t_qty
                     FROM sfa_file,sfb_file 
                    WHERE sfa_file.sfa01 = sfb_file.sfb01
                      AND sfb_file.sfb85 = g_sfp.sfp08
                      ...
                      AND sfa_file.sfa11='N'
                      AND sfa_file.sfa26 IN ('0','1','2','5','7')   #FUN-A20037 add '7'
                   IF t_qty < l_ta[l_ac].ta8 THEN
                      LET l_str = l_ta[l_ac].ta8,">",t_qty
                      CALL cl_err(l_str,"auto-09",1)
                      NEXT FIELD ta8
                   END IF      
                END IF
            END IF
            
            IF g_argv1 = '2' AND g_argv2 = '9' THEN
               LET l_n = 0 
               SELECT COUNT(*) INTO l_n 
                 FROM sfa_file,sfb_file 
                WHERE sfa_file.sfa01 = sfb_file.sfb01
                ...
                ELSE
                   ...
                   IF t_qty < l_ta[l_ac].ta8 THEN
                      LET l_str = l_ta[l_ac].ta8,">",t_qty
                      CALL cl_err(l_str,"auto-11",1)
                      NEXT FIELD ta8
                   END IF      
                END IF
            END IF
            
            IF g_argv1 = "1" AND g_argv2 MATCHES '[234]' THEN
            
               DECLARE i501_auto_ta CURSOR FOR
                SELECT ta2,ta8 FROM ta_file 
                 WHERE ta_file.ta1 = l_ta[l_ac].ta1
                   AND ta_file.ta5 = l_ta[l_ac].ta5 
                   AND ta_file.ta6 = l_ta[l_ac].ta6 
                   AND ta_file.ta7 = l_ta[l_ac].ta7
                   AND ta_file.ta3!= l_ta[l_ac].ta3 
                   AND ta_file.ta4!= l_ta[l_ac].ta4
                   
               LET li_cnt = 1    
               FOREACH i501_auto_ta INTO l_ta[l_ac].*
                  IF STATUS THEN 
                     CALL cl_err('foreach ta',STATUS,0) 
                     EXIT FOREACH 
                  END IF               
                  ...
                  SELECT img09,img10 INTO l_img09,l_img10
                   FROM img_file
                  WHERE img01 = l_ta[li_cnt].ta1
                    AND img02 = l_ta[li_cnt].ta5
                    AND img03 = l_ta[li_cnt].ta6 
                    AND img04 = l_ta[li_cnt].ta7
                    
                  CALL s_umfchk(l_ta[li_cnt].ta1,l_ta[li_cnt].ta3,l_img09)
                       RETURNING l_flag,l_factor
                       
                  LET  t_ta8 = t_ta8 + l_ta[li_cnt].ta8 * l_factor
                  IF t_ta8 + l_ta[li_cnt].ta8 > l_img10 THEN
                    #IF g_sma.sma894[3,3]='N' OR g_sma.sma894[3,3] IS NULL THEN                                #FUN-C80107 mark
                    #FUN-D30024--modify--str--
                    #INITIALIZE g_sma894 TO NULL                                                               #FUN-C80107   
                    #CALL s_inv_shrt_by_warehouse(g_sma.sma894[3,3],l_ta[li_cnt].ta5) RETURNING g_sma894       #FUN-C80107   
                    #IF g_sma894 = 'N' THEN                                                                    #FUN-C80107
                     INITIALIZE g_imd23 TO NULL
                     CALL s_inv_shrt_by_warehouse(g_sfs[l_ac].sfs07,g_plant) RETURNING g_imd23  #TQC-D40078 g_plant 
                     IF g_imd23 = 'N' THEN
                    #FUN-D30024--modify--end--
                        CALL cl_err(t_ta8+l_ta[li_cnt].ta8,'mfg1303',0)
                        NEXT FIELD ta8
                     END IF
                  END IF
                  LET li_cnt = li_cnt + 1      
            ...
            INSERT INTO ta_file VALUES(l_ta[l_ac].*)
            IF SQLCA.sqlcode THEN
               CALL cl_err3("ins","ta_file",l_ta[l_ac].ta1,l_ta[l_ac].ta3,SQLCA.sqlcode,"","ins ta",1)
               CANCEL INSERT
               SELECT ima918,ima921 INTO g_ima918,g_ima921 
                 FROM ima_file
                WHERE ima01 = g_sfs[l_ac].sfs04
...
#TQC-C10033 --unmark
            ELSE
               MESSAGE 'INSERT O.K'
               LET l_rec_b_auto = l_rec_b_auto + 1
               DISPLAY l_rec_b_auto TO FORMONLY.cn
            END IF
 
        AFTER FIELD ta1
            IF l_ta[l_ac].ta1 IS NOT NULL THEN
               LET l_n = 0 
               SELECT COUNT(*) INTO l_n
                 FROM ima_file
                WHERE ima01 = l_ta[l_ac].ta1
               IF l_n = 0 THEN
                  CALL cl_err(l_ta[l_ac].ta1,"auto-12",1)
                  NEXT FIELD ta1
               END IF
               SELECT ima02 INTO l_ima02
                 FROM ima_file
                WHERE ima01 = l_ta[l_ac].ta1
               LET l_ta[l_ac].ta2 = l_ima02
               DISPLAY l_ta[l_ac].ta2 TO ta2
               SELECT ima_file.ima63 INTO l_ima63
                 FROM ima_file 
                WHERE ima01 = l_ta[l_ac].ta1
               LET l_ta[l_ac].ta3 = l_ima63
               DISPLAY l_ta[l_ac].ta3 TO ta3 
            END IF
 
        AFTER FIELD ta3
            IF l_ta[l_ac].ta3 IS NOT NULL THEN
               LET l_n = 0 
               SELECT COUNT(*) INTO l_n
                 FROM gfe_file
                WHERE gfe01 = l_ta[l_ac].ta3
               IF l_n = 0 THEN
                  CALL cl_err(l_ta[l_ac].ta3,"auto-13",1)
                  NEXT FIELD ta3
               END IF  
            END IF
            
        AFTER FIELD ta4
            IF cl_null(l_ta[l_ac].ta4) THEN 
               LET l_ta[l_ac].ta4 = ' ' 
            ELSE 
               CALL i501_ta4(l_ta[l_ac].ta4)
                    RETURNING g_errno
               IF NOT cl_null(g_errno) THEN
                  CALL cl_err(l_ta[l_ac].ta4,g_errno,1)
                  NEXT FIELD ta4
               END IF 
            END IF    
        ...
        #Add No.FUN-AB0018
        AFTER FIELD ta5
            IF NOT cl_null(l_ta[l_ac].ta5) THEN
               IF NOT s_chk_ware(l_ta[l_ac].ta5) THEN  #检查仓库是否属于当前门店
                  NEXT FIELD ta5
               END IF
            END IF
        #End Add No.FUN-AB0018
        ...
        AFTER FIELD ta8
            IF l_ta[l_ac].ta8 IS NOT NULL THEN
               IF l_ta[l_ac].ta8 < 0 THEN
                  CALL cl_err(l_ta[l_ac].ta8,"auto-14",1)
                  NEXT FIELD ta8
               END IF
            END IF
        ...
        BEFORE DELETE                            #是否取消單身
            ...
            DELETE FROM ta_file
             WHERE ta1 = g_sfp.sfp01 
               AND ta3 = l_ta_t.ta3
               AND ta4 = l_ta_t.ta4
            IF SQLCA.sqlcode THEN
               CALL cl_err3("del","ta_file",g_sfp.sfp01,l_ta_t.ta3,SQLCA.sqlcode,"","",1)
               ROLLBACK WORK
               CANCEL DELETE
            END IF
            COMMIT WORK
        ...
        ON ROW CHANGE
            ...
            IF l_lock_sw = 'Y' THEN
               CALL cl_err(l_ta[l_ac].ta1,-263,1)
               LET l_ta[l_ac].* = l_ta_t.*
            ELSE
               IF g_argv1 = '1' AND g_argv2 = '3' THEN
                  LET l_n = 0 
               ...
               IF g_argv1 = "1" AND g_argv2 MATCHES '[234]' THEN
                  ...
                  DECLARE i501_auto_ta1 CURSOR FOR
                   SELECT ta2,ta8 FROM ta_file 
                    WHERE ta_file.ta1 = l_ta[l_ac].ta1
                      AND ta_file.ta5 = l_ta[l_ac].ta5 
                      AND ta_file.ta6 = l_ta[l_ac].ta6 
                  ...
                  LET li_cnt = 1    
                  FOREACH i501_auto_ta1 INTO l_ta[l_ac].*
                     IF STATUS THEN 
                        CALL cl_err('foreach ta',STATUS,0) 
                        EXIT FOREACH 
                     END IF               
               ...
               UPDATE ta_file SET ta1 = l_ta[l_ac].ta1,
                                  ta2 = l_ta[l_ac].ta2,
                                  ta3 = l_ta[l_ac].ta3,
                                  ta4 = l_ta[l_ac].ta4,
                                  ta5 = l_ta[l_ac].ta5,
                ...
                WHERE ta1 = g_sfp.sfp01 
                  ...
                  AND ta4 = l_ta_t.ta4
               IF SQLCA.sqlcode THEN
                  CALL cl_err3("upd","ta_file",g_sfp.sfp01,l_ta_t.ta3,SQLCA.sqlcode,"","upd ta",1)
                  LET l_ta[l_ac].* = l_ta_t.*
                  ROLLBACK WORK
               ELSE
                  MESSAGE 'UPDATE O.K'
        ...
        AFTER ROW
           ...
           #LET l_ac_t = l_ac      #FUN-D40030 Mark
            ...
            COMMIT WORK
            DISPLAY "commit work after row"
           #CALL l_ta.deleteElement(l_rec_b_auto+1)   #FUN-D40030 Mark
            
        AFTER INPUT 
           IF INT_FLAG THEN
              EXIT INPUT      
           END IF
           DECLARE i501_auto_b CURSOR FOR
            SELECT * FROM ta_file
            
           LET l_sql1= "SELECT sfa01,sfa05,sfa06,sfa062,sfa08,",  #FUN-B50059
                       "       sfa12,sfa161,sfa26,sfa27,sfa28 ",
                       "  FROM sfa_file,sfb_file",
                       " WHERE sfa01 = sfb01 ",
                       "   AND sfb85 = '",g_sfp.sfp08,"'",
                       "   AND sfa03 = ? ",
                       "   AND sfa08 = ? ",
                       "   AND sfa12 = ? ",
                       "   AND sfb02 NOT IN ('13','15') ",
                       "   AND sfb04 NOT IN ('1','8') ",
                       "   AND sfa11 = 'N' ",
                       "   AND sfa26 IN ('0','1','2','5','7')",  #FUN-A20037 add '7'
                       "   AND sfb01 IN ",
                       "       (SELECT sfq02 FROM sfq_file ",
                       "         WHERE sfq01 = '",g_sfp.sfp01,"') ",
                       "   AND (sfa05-sfa06) > 0 "  #FUN-B50059
           PREPARE i501_sfa_cl1_pre FROM l_sql1
           DECLARE i501_sfa_cl1 CURSOR FOR i501_sfa_cl1_pre
              
           LET l_sql2= "SELECT sfa01,sfa05,sfa06,sfa062,sfa08,",  #FUN-B50059
                       "       sfa12,sfa161,sfa26,sfa27,sfa28 ",
                       "  FROM sfa_file,sfb_file",
                       " WHERE sfa01 = sfb01 ",
                       "   AND sfb85 = '",g_sfp.sfp08,"'",
                       ...
                       "   AND sfa26 IN ('0','1','2','5','7')",  #FUN-A20037 add '7'
                       "   AND (sfa05-sfa06) > 0 "  #FUN-B50059
           PREPARE i501_sfa_cl2_pre FROM l_sql2
           DECLARE i501_sfa_cl2 CURSOR FOR i501_sfa_cl2_pre
              
           LET l_sql3= "SELECT sfa01,sfa05,sfa06,sfa062,sfa08,",  #FUN-B50059
                       "       sfa12,sfa161,sfa26,sfa27,sfa28 ",
                       "  FROM sfa_file,sfb_file",
                       " WHERE sfa01 = sfb01 ",
                       "   AND sfb85 = '",g_sfp.sfp08,"'",
                       ...
                       "   AND sfb04 NOT IN ('1','8') ",
                       "   AND sfa26 IN ('0','1','2','5','7')"   #FUN-A20037 add '7'
           PREPARE i501_sfa_cl3_pre FROM l_sql3
           DECLARE i501_sfa_cl3 CURSOR FOR i501_sfa_cl3_pre
              
           LET l_sql4= "SELECT sfb09,sfa01,sfa05,sfa06,sfa062,sfa08,",  #FUN-B50059
                       "       sfa12,sfa161,sfa26,sfa27,sfa28 ",
                       "  FROM sfa_file,sfb_file",
                       " WHERE sfa01 = sfb01 ",
                       "   AND sfb85 = '",g_sfp.sfp08,"'",
                       ...
                       "   AND sfa11 = 'N' ",
                       "   AND sfa26 IN ('0','1','2','5','7')"   #FUN-A20037 add '7'
           PREPARE i501_sfa_cl4_pre FROM l_sql4
           DECLARE i501_sfa_cl4 CURSOR FOR i501_sfa_cl4_pre
              
           LET l_cn = 1 
           FOREACH i501_auto_b INTO l_ta[l_cn].*
              IF STATUS THEN 
                 CALL cl_err('foreach ta',STATUS,0) 
                 EXIT FOREACH 
              END IF
              ...
              IF g_argv1 = '1' AND g_argv2 = '3' THEN
                 ...
                 SELECT SUM(sfa_file.sfa05-sfa_file.sfa06)   #FUN-B50059
                  ...
                  WHERE sfa_file.sfa01 = sfb_file.sfb01  
                    AND sfb_file.sfb85 = g_sfp.sfp08
                    AND sfa_file.sfa03 = l_ta[l_cn].ta1 
                    AND sfa_file.sfa08 = l_ta[l_cn].ta4
                    AND sfa_file.sfa12 = l_ta[l_cn].ta3 
                    AND sfb_file.sfb02 NOT IN ('13','15') 
                    AND sfb_file.sfb04 NOT IN ('1','8')
                    AND sfb_file.sfb01 IN 
                        (SELECT sfq02 FROM sfq_file
                    ...
                    LET l_cnt = 1
                    FOREACH i501_sfa_cl1 USING l_ta[l_cn].ta1,l_ta[l_cn].ta4,l_ta[l_cn].ta3
                                          INTO t_sfa[l_cnt].*
                       IF STATUS THEN 
                          CALL cl_err('foreach sfa',STATUS,0) 
                          EXIT FOREACH 
                       END IF
                       
                       LET l_sfs01 = g_sfp.sfp01
                       SELECT MAX(sfs02)+1 INTO g_sfs[l_cnt].sfs02
                         FROM sfs_file WHERE sfs01 = g_sfp.sfp01
                       IF g_sfs[l_cnt].sfs02 IS NULL THEN 
                          LET g_sfs[l_cnt].sfs02=1 
                       END IF
                       LET g_sfs[l_cnt].sfs03 = t_sfa[l_cnt].sfa01
                       LET g_sfs[l_cnt].sfs04 = l_ta[l_cn].ta1
                       LET g_sfs[l_cnt].sfs05 = l_ta[l_cn].ta8*(t_sfa[l_cnt].sfa05-t_sfa[l_cnt].sfa06)/l_sfa05_06
                       LET g_sfs[l_cnt].sfs06 = t_sfa[l_cnt].sfa12
                       LET g_sfs[l_cnt].sfs07 = l_ta[l_cn].ta5
                       LET g_sfs[l_cnt].sfs08 = l_ta[l_cn].ta6
                       LET g_sfs[l_cnt].sfs09 = l_ta[l_cn].ta7
                       LET g_sfs[l_cnt].sfs10 = t_sfa[l_cnt].sfa08
                       LET g_sfs[l_cnt].sfs26 = NULL
                       LET l_sfs27 = NULL
                       LET l_sfs28 = NULL
                       IF t_sfa[l_cnt].sfa26 MATCHES '[SUTZ]' THEN   #FUN-A20037 add 'Z'
                          LET g_sfs[l_cnt].sfs26 = t_sfa[l_cnt].sfa26
                          LET l_sfs27 = t_sfa[l_cnt].sfa27
                          LET l_sfs28 = t_sfa[l_cnt].sfa28
                       END IF
                       IF g_sma.sma115='Y' THEN
                          LET g_sfs[l_cnt].sfs30 = g_sfs[l_cnt].sfs06
                          CALL s_umfchk(g_sfs[l_cnt].sfs04,g_sfs[l_cnt].sfs06,t_sfa[l_cnt].sfa12) 
                               RETURNING g_errno,l_factor
                          LET g_sfs[l_cnt].sfs31 = l_factor
                          SELECT ima906,ima907 INTO l_ima906,l_ima907 
                            FROM ima_file 
                           WHERE ima_file.ima01 = g_sfs[l_cnt].sfs04
                          IF l_ima906 = '1' THEN
                             LET g_sfs[l_cnt].sfs33 = NULL
                             LET g_sfs[l_cnt].sfs34 = NULL
                             LET g_sfs[l_cnt].sfs35 = NULL
                          ELSE
                             LET g_sfs[l_cnt].sfs33 = l_ima907
                             CALL s_umfchk(g_sfs[l_cnt].sfs04,g_sfs[l_cnt].sfs33,t_sfa[l_cnt].sfa12) 
                                  RETURNING g_errno,l_factor
                             LET g_sfs[l_cnt].sfs34 = l_factor
                             LET g_sfs[l_cnt].sfs35 = 0        
                          END IF
                       END IF
                       IF cl_null(l_sfs27) THEN LET l_sfs27=g_sfs[l_cnt].sfs04 END IF  #No.MOD-8B0086 add
                       IF cl_null(l_sfs27) THEN
                          LET l_sfs27 = ' '
                       END IF
         #FUN-A60028 --begin--
                       IF cl_null(g_sfs[l_cnt].sfs012) THEN 
                          LET g_sfs[l_cnt].sfs012 = ' ' 
                       END IF 
                       IF cl_null(g_sfs[l_cnt].sfs013) THEN 
                          LET g_sfs[l_cnt].sfs013 = 0
                       END IF  
        #FUN-A60028 --end--                        
                       IF cl_null(g_sfs[l_cnt].sfs014) THEN LET g_sfs[l_cnt].sfs014=' ' END IF #FUN-C70014 add
                       INSERT INTO sfs_file(sfs01,sfs02,sfs03,sfs04,sfs05,
                                            sfs06,sfs07,sfs08,sfs09,sfs10,
                                            sfs26,sfs27,sfs28,sfs30,sfs31,
                                            sfs32,sfs33,sfs34,sfs35,
                                            sfsplant,sfslegal,sfs012,sfs013,sfs014)  #FUN-980008 add  #FUN-A60028 add sfs012,sfs013
                                      VALUES(l_sfs01,g_sfs[l_cnt].sfs02,
                                             g_sfs[l_cnt].sfs03,g_sfs[l_cnt].sfs04,
                                             g_sfs[l_cnt].sfs05,g_sfs[l_cnt].sfs06,
                                             g_sfs[l_cnt].sfs07,g_sfs[l_cnt].sfs08,
                                             g_sfs[l_cnt].sfs09,g_sfs[l_cnt].sfs10,
                                             g_sfs[l_cnt].sfs26,l_sfs27,l_sfs28,
                                             g_sfs[l_cnt].sfs30,g_sfs[l_cnt].sfs31,
                                             g_sfs[l_cnt].sfs32,g_sfs[l_cnt].sfs33,
                                             g_sfs[l_cnt].sfs34,g_sfs[l_cnt].sfs35,
                                             g_plant,g_legal,g_sfs[l_cnt].sfs012,g_sfs[l_cnt].sfs013,g_sfs[l_cnt].sfs014)   #FUN-980008 add
                                                                   #FUN-A60028 add sfs012,sfs013
                       #FUN-B70061 --START--
                       LET b_sfsi.sfsi01 = l_sfs01
                       LET b_sfsi.sfsi02 = g_sfs[l_cnt].sfs02
                       IF NOT s_ins_sfsi(b_sfsi.*,g_plant) THEN
              ...
              IF g_argv1 = '1' AND g_argv2 = '4' THEN
                 ...
                 SELECT SUM(sfa_file.sfa05-sfa_file.sfa06)  #FUN-B50059
                  ...
                  WHERE sfa_file.sfa01 = sfb_file.sfb01  
                    ...
                    FOREACH i501_sfa_cl2 USING l_ta[l_cn].ta1,l_ta[l_cn].ta4,l_ta[l_cn].ta3
                                          INTO t_sfa[l_cnt].*
                       IF STATUS THEN 
                          CALL cl_err('foreach sfa',STATUS,0) 
                          EXIT FOREACH 
        ...
        #FUN-A60028 --end--                            
              ...
              IF g_argv1 = '1' AND g_argv2 = '2' THEN
                 LET l_sfa06 = NULL
                 SELECT SUM(sfa_file.sfa06)   #FUN-B50059
                   INTO l_sfa06 
                   FROM sfa_file,sfb_file 
                  WHERE sfa_file.sfa01 = sfb_file.sfb01  
                    AND sfb_file.sfb85 = g_sfp.sfp08
                    ...
                    LET l_cnt = 1
                    FOREACH i501_sfa_cl3 USING l_ta[l_cn].ta1,l_ta[l_cn].ta4,l_ta[l_cn].ta3
                                          INTO t_sfa[l_cnt].*
                       IF STATUS THEN 
                          CALL cl_err('foreach sfa',STATUS,0) 
                          EXIT FOREACH 
                       ...
                       LET g_sfs[l_cnt].sfs03 = t_sfa[l_cnt].sfa01
                       LET g_sfs[l_cnt].sfs04 = l_ta[l_cn].ta1
                       LET g_sfs[l_cnt].sfs05 = l_ta[l_cn].ta8*t_sfa[l_cnt].sfa06/l_sfa06
                       LET g_sfs[l_cnt].sfs06 = t_sfa[l_cnt].sfa12
                       LET g_sfs[l_cnt].sfs07 = l_ta[l_cn].ta5
                       LET g_sfs[l_cnt].sfs08 = l_ta[l_cn].ta6
                       LET g_sfs[l_cnt].sfs09 = l_ta[l_cn].ta7
                       ...
                       IF g_sma.sma115='Y' THEN
                          ...
                          CALL s_umfchk(g_sfs[l_cnt].sfs04,g_sfs[l_cnt].sfs06,t_sfa[l_cnt].sfa12) 
                               RETURNING g_errno,l_factor
                          LET g_sfs[l_cnt].sfs31 = l_factor
                          LET g_sfs[l_cnt].sfs32 = g_sfs[l_cnt].sfs05/l_factor
                          SELECT ima906,ima907 INTO l_ima906,l_ima907 
                            FROM ima_file 
                           WHERE ima_file.ima01 = g_sfs[l_cnt].sfs04
                          IF l_ima906 = '1' THEN
              ...
              IF g_argv1 = '2' AND g_argv2 = '8' THEN
                 ...
                 SELECT SUM(sfa06-sfb09*sfa161)   #FUN-B50059
                  ...
                  WHERE sfa_file.sfa01 = sfb_file.sfb01  
                    ...
                    LET l_cnt = 1
                    FOREACH i501_sfa_cl4 USING l_ta[l_cn].ta1,l_ta[l_cn].ta4,l_ta[l_cn].ta3
                                          INTO t_sfb09,t_sfa[l_cnt].*
                       IF STATUS THEN 
                          CALL cl_err('foreach sfa',STATUS,0) 
                          EXIT FOREACH 
                       END IF
                       ...
                       LET g_sfs[l_cnt].sfs03 = t_sfa[l_cnt].sfa01
                       LET g_sfs[l_cnt].sfs04 = l_ta[l_cn].ta1
                       LET g_sfs[l_cnt].sfs05 = l_ta[l_cn].ta8*(t_sfa[l_cnt].sfa06-t_sfb09*t_sfa[l_cnt].sfa161)/t_qty
                       LET g_sfs[l_cnt].sfs06 = t_sfa[l_cnt].sfa12
                       LET g_sfs[l_cnt].sfs07 = l_ta[l_cn].ta5
                       LET g_sfs[l_cnt].sfs08 = l_ta[l_cn].ta6
                       LET g_sfs[l_cnt].sfs09 = l_ta[l_cn].ta7
                 ...
                 SELECT SUM(sfa062) 
                  ...
                  WHERE sfa_file.sfa01 = sfb_file.sfb01  
                    ...
                    FOREACH i501_sfa_cl3 USING l_ta[l_cn].ta1,l_ta[l_cn].ta4,l_ta[l_cn].ta3
                       ...
                       LET g_sfs[l_cnt].sfs03 = t_sfa[l_cnt].sfa01
                       LET g_sfs[l_cnt].sfs04 = l_ta[l_cn].ta1
                       LET g_sfs[l_cnt].sfs05 = l_ta[l_cn].ta8*t_sfa[l_cnt].sfa062/l_sfa062
                       LET g_sfs[l_cnt].sfs06 = t_sfa[l_cnt].sfa12
                       LET g_sfs[l_cnt].sfs07 = l_ta[l_cn].ta5
                       LET g_sfs[l_cnt].sfs08 = l_ta[l_cn].ta6
                       LET g_sfs[l_cnt].sfs09 = l_ta[l_cn].ta7
        ...
        ON ACTION CONTROLO                        #沿用所有欄位
           IF INFIELD(ta1) AND l_ac > 1 THEN
              LET l_ta[l_ac].ta2=l_ta[l_ac-1].ta2
              LET l_ta[l_ac].ta3=l_ta[l_ac-1].ta3
              LET l_ta[l_ac].ta4=l_ta[l_ac-1].ta4
              LET l_ta[l_ac].ta5=l_ta[l_ac-1].ta5
              LET l_ta[l_ac].ta6=l_ta[l_ac-1].ta6
              LET l_ta[l_ac].ta7=l_ta[l_ac-1].ta7
              LET l_ta[l_ac].ta8=l_ta[l_ac-1].ta8
              NEXT FIELD ta1
           END IF
...
#                    CALL cl_init_qry_var()
#                    LET g_qryparam.form ="q_ima"
#                    LET g_qryparam.default1 = l_ta[l_ac].ta1
#                    CALL cl_create_qry() RETURNING l_ta[l_ac].ta1
                     CALL q_sel_ima(FALSE, "q_ima","",l_ta[l_ac].ta1,"","","","","",'' ) 
                       RETURNING l_ta[l_ac].ta1  
#FUN-AA0059---------mod------------end-----------------
                     DISPLAY l_ta[l_ac].ta1 TO ta1
                     NEXT FIELD ta1
                WHEN INFIELD(ta3)
                     CALL cl_init_qry_var()
                     LET g_qryparam.form ="q_gfe"
                     LET g_qryparam.default1 = l_ta[l_ac].ta3
                     CALL cl_create_qry() RETURNING l_ta[l_ac].ta3
                     DISPLAY l_ta[l_ac].ta3 TO ta3
                     NEXT FIELD ta3
                WHEN INFIELD(ta4)
                     CALL cl_init_qry_var()
                     LET g_qryparam.form ="q_ecd3"
                     LET g_qryparam.default1 = l_ta[l_ac].ta4
                     CALL cl_create_qry() RETURNING l_ta[l_ac].ta4
                     DISPLAY l_ta[l_ac].ta4 TO ta4
                     NEXT FIELD ta4           
                WHEN INFIELD(ta5) OR INFIELD(ta6) OR INFIELD(ta7)
                   #FUN-C30300---begin
                   LET g_ima906 = NULL
                   SELECT ima906 INTO g_ima906 FROM ima_file
                    WHERE ima01 = l_ta[l_ac].ta1
                   #IF s_industry("icd") AND g_ima906='3' THEN  #TQC-C60028
                   IF s_industry("icd") THEN  #TQC-C60028
                      CALL q_idc(FALSE,TRUE,l_ta[l_ac].ta1,l_ta[l_ac].ta5,l_ta[l_ac].ta6,l_ta[l_ac].ta7)
                                  RETURNING l_ta[l_ac].ta5,l_ta[l_ac].ta6,l_ta[l_ac].ta7
                   ELSE
                   #FUN-C30300---end
                     CALL q_img4(FALSE,TRUE,l_ta[l_ac].ta1,l_ta[l_ac].ta5,l_ta[l_ac].ta6,l_ta[l_ac].ta7,'A')
                     RETURNING l_ta[l_ac].ta5,l_ta[l_ac].ta6,l_ta[l_ac].ta7
                   END IF #FUN-C30300
                     DISPLAY l_ta[l_ac].ta5 TO ta5
                     DISPLAY l_ta[l_ac].ta6 TO ta6
                     DISPLAY l_ta[l_ac].ta7 TO ta7
                     IF INFIELD(ta5) THEN NEXT FIELD ta5 END IF
                     IF INFIELD(ta6) THEN NEXT FIELD ta6 END IF
                     IF INFIELD(ta7) THEN NEXT FIELD ta7 END IF
           END CASE
...
FUNCTION i501_ta4(p_ta4)
   DEFINE p_ta4       LIKE type_file.chr6
   DEFINE l_ecdacti   LIKE ecd_file.ecdacti
   DEFINE l_errno     LIKE type_file.chr10
...
DEFINE l_shm08 	    LIKE shm_file.shm08
   ...
   #將撈取qty1,qty2的部分整理到函數 i501_sfq03_chk1()處理  
   CALL i501_sfq03_chk(p_sfq02,p_sfq04,'',p_sfq014,'1')  #FUN-C70014 add sfq014
           RETURNING qty1,qty2                   
   IF qty1 IS NULL THEN LET qty1=0 END IF
   IF qty2 IS NULL THEN LET qty2=0 END IF
