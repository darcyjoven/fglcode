...
# Modify.........: No.FUN-D40129 13/05/06 By yangtt 1、執行Ctrl+g功能; 2、4fd中flag應不能勾選到null值
#                                                   3、製造部門*，製造部門為空的資料不列印
...
#             wc       VARCHAR(600),      # Where condition   #TQC-630166
             ...
             #u        LIKE type_file.chr3,          #No.FUN-680121 VARCHAR(3)# Group total sw #FUN-D30070 mark
...
MAIN
  ...
  #LET tm.u  = ARG_VAL(10) #FUN-D30070 mark
   #No.FUN-570264 --start--
   LET g_rep_user = ARG_VAL(10) #FUN-D30070 mod 11->10
   LET g_rep_clas = ARG_VAL(11) #FUN-D30070 mod 12->11
   LET g_template = ARG_VAL(12) #FUN-D30070 mod 13->12
   LET tm.flag = ARG_VAL(13)  #TQC-960161 #FUN-D30070 mod 14->13 
   #No.FUN-570264 ---end---

   #FUN-CB0002--add--str--
   LET g_sql="order1.sfb_file.sfb05,order2.sfb_file.sfb05,",
             "order3.sfb_file.sfb05,sfb82.sfb_file.sfb82,",
             "gem02.gem_file.gem02,",
             "sfb01.sfb_file.sfb01,sfb25.sfb_file.sfb25,",
             "sfb13.sfb_file.sfb13,sfb15.sfb_file.sfb15,",
             "sfb05.sfb_file.sfb05,sfb02.sfb_file.sfb01,",
             "sfb08.sfb_file.sfb08,ima02.ima_file.ima02,",
             "ima021.ima_file.ima021,ima55.ima_file.ima55"
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW asfr130_w AT p_row,p_col
       #WITH FORM "asf/42f/asfr130" #FUN-D30070 mark
        WITH FORM "asf/42f/asfx130" #FUN-D30070
...
#         l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT   #TQC-630166        #No.FUN-680121 VARCHAR(1000)
          ...
          sr               RECORD order1 LIKE sfb_file.sfb05,        #No.FUN-680121 VARCHAR(40)#FUN-5B0105 20->40
                                  ...
                                  gem02 LIKE gem_file.gem02,    #FUN-CB0002  add
                                  sfb01 LIKE sfb_file.sfb01,
                                  sfb25 LIKE sfb_file.sfb25,
                                  sfb13 LIKE sfb_file.sfb13,
                                  sfb15 LIKE sfb_file.sfb15,
                                  sfb05 LIKE sfb_file.sfb05,
                                  #sfb02 LIKE sfb_file.sfb02,   #FUN-CB0002  mark
                                  sfb02 LIKE sfb_file.sfb01,    #FUN-CB0002  add
     ...
     #FUN-D40129---add---str-
     IF tm.wc MATCHES "sfb82 like '%'" THEN
        LET tm.wc =  cl_replace_str(tm.wc,"sfb82 like '%'","(sfb82 like '%' OR sfb82 IS NULL)")
     END IF
     #FUN-D40129---add---end-
 
 IF tm.flag = 'Y' then                                        #TQC-960161  
     LET l_sql = "SELECT '','','',",
                 "  sfb82, '',sfb01, sfb25, sfb13, sfb15, sfb05,",     #FUN-CB0002  add ''
...
#TQC-960161 --end--  
     ...
     #CALL cl_outnam('asfr130') RETURNING l_name   #FUN-CB0002  mark
     #START REPORT asfr130_rep TO l_name           #FUN-CB0002  mark
     ...
     FOREACH asfr130_curs1 INTO sr.*
       ...
       #OUTPUT TO REPORT asfr130_rep(sr.*)    #FUN-CB0002  add
       ...
       CASE sr.sfb02
         WHEN '1'   LET sr.sfb02 = cl_getmsg('asf-333',g_lang)
         WHEN '5'   LET sr.sfb02 = cl_getmsg('asf-334',g_lang)
         WHEN '7'   LET sr.sfb02 = cl_getmsg('asf-335',g_lang)
         WHEN '8'   LET sr.sfb02 = cl_getmsg('asf-336',g_lang)
         WHEN '11'  LET sr.sfb02 = cl_getmsg('asf-337',g_lang)
         WHEN '13'  LET sr.sfb02 = cl_getmsg('asf-338',g_lang)
         WHEN '15'  LET sr.sfb02 = cl_getmsg('asf-339',g_lang)
       END CASE
     ...
     #FINISH REPORT asfr130_rep    #FUN-CB0002  mark
 
     #CALL cl_prt(l_name,g_prtway,g_copies,g_len)   #FUN-CB0002  mark
     ...
     IF g_zz05 = 'Y' THEN
        ...
        RETURNING tm.wc
     END IF
     LET g_xgrid.order_field = cl_get_order_field(tm.s,"sfb82,sfb01,sfb25,sfb13,sfb15,sfb05")
     #LET g_xgrid.grup_field = cl_get_sum_field(tm.s,tm.u,"sfb82,sfb01,sfb25,sfb13,sfb15,sfb05")
     LET g_xgrid.grup_field = cl_get_order_field(tm.s,"sfb82,sfb01,sfb25,sfb13,sfb15,sfb05")
    #LET g_xgrid.grup_sum_field = cl_get_sum_field(tm.s,tm.u,"sfb82,sfb01,sfb25,sfb13,sfb15,sfb05") #FUN-D30070 mark
     LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"sfb82,sfb01,sfb25,sfb13,sfb15,sfb05")
    #LET l_str = cl_wcchp(g_xgrid.order_field,"sfb82,sfb01,sfb25,sfb13,sfb15,sfb05") #FUN-D30070 mark
...
REPORT asfr130_rep(sr)
  ...
  FORMAT
   ...
   AFTER GROUP OF sr.order3
      #IF tm.u[3,3] = 'Y' THEN #FUN-D30070 mark
         PRINT g_dash2
