...
# Modify.........: No.FUN-CB0073 12/12/12 By chenjing CR轉GR
# Modify.........: No.FUN-D10098 13/01/30 By chenjing 增加說明ACTION
...
TYPE sr1_t RECORD
    ...
    l_qty LIKE rpf_file.rpf04,
    l_qty1 LIKE bed_file.bed07,
    l_qty2 LIKE bed_file.bed07,
    l_sfb05 LIKE sfb_file.sfb05,
    l_ima02 LIKE ima_file.ima02,    #FUN-D10098
    l_ima021 LIKE ima_file.ima021   #FUN-D10098
END RECORD
###GENGRE###END
...
#No.FUN-7C0007---Begin                                                          
   LET g_sql = "out01.type_file.chr1000,",                                         
               ...
               "l_ima02.ima_file.ima02,",    #FUN-D10098
               "l_ima021.ima_file.ima021"    #FUN-D10098   
                                                        
   LET l_table = cl_prt_temptable('asfg500',g_sql) CLIPPED                      
   IF l_table = -1 THEN EXIT PROGRAM END IF 
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,              
               " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) "   #FUN-D10098 add 2?     
...
#No.FUN-7C0007---End
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL asfg500_tm(0,0)        # Input print condition
      ELSE CALL asfg500()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
   CALL cl_gre_drop_temptable(l_table)            #FUN-CB0073
END MAIN
 
FUNCTION asfg500_tm(p_row,p_col)
   DEFINE
   ...
   END IF
   OPEN WINDOW asfg500_w AT p_row,p_col
     WITH FORM "asf/42f/asfg500"
################################################################################
...
#       tmp161    LIKE ima_file.ima26,
  ...
  #No.FUN-680121-END     
       ...
       INPUT BY NAME tm.a, tm.b, tm.c  WITHOUT DEFAULTS
          ...
          ON ACTION help          #FUN-D10098
             CALL cl_show_help()  #FUN-D10098
       ...
       IF g_bgjob = 'Y' THEN
          ...
          ELSE
             ...
             CALL cl_cmdat('asfg500',g_time,l_cmd)
          END IF
          CLOSE WINDOW asfg500_w
          CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
          CALL cl_gre_drop_temptable(l_table)            #FUN-CB0073
...
#     l_qty1    LIKE ima_file.ima26,          #No.FUN-680121 DECIMAL(12,3)
      ...
      l1_ima02     LIKE ima_file.ima02,          #No.FUN-D10098 add
      l1_ima021    LIKE ima_file.ima021,         #No.FUN-D10098 add
      sr   RECORD
...
#No.FUN-590110  --end
   ...
   PREPARE asfg500_pre1 FROM l_sql
   DECLARE asfg500_curs1 CURSOR WITH HOLD FOR asfg500_pre1
   FOREACH asfg500_curs1 INTO l_sfb01,l_sfb02,
      l_sfb05,l_sfb23,l_sfb071,l_sfb06,l_sfb08 ,l_sfb04,l_sfb42
      IF SQLCA.SQLCODE THEN
         CALL cl_err('Foreach asfg500_curs1:',SQLCA.sqlcode,1)
      END IF
   ...
   PREPARE asfg500_pre3 FROM l_sql
   DECLARE asfg500_curs3 CURSOR WITH HOLD FOR asfg500_pre3
   #==>供給方面之工單尋找
   ...
   PREPARE asfg500_pre4 FROM l_sql
   DECLARE asfg500_curs4 CURSOR WITH HOLD FOR asfg500_pre4
   #==>供給方面之請購單尋找
   ...
   PREPARE asfg500_pre5 FROM l_sql
   DECLARE asfg500_curs5 CURSOR WITH HOLD FOR asfg500_pre5
   ...
   FOREACH r500_suply INTO l_sfa03
      ...
      END IF
      #取採購單供給資料
      FOREACH asfg500_curs3 USING l_sfa03 INTO tp1.*
         IF SQLCA.SQLCODE THEN
            CALL cl_err('foreach asfg500_curs3:',SQLCA.sqlcode,1)
         END IF
         #將供給資料存入輸出暫存檔
...
#           CALL cl_err('Insert fail:ckp#01',SQLCA.sqlcode,1)   #No.FUN-660128
      ...
      FOREACH asfg500_curs4 USING l_sfa03 INTO tp1.*
         IF SQLCA.SQLCODE THEN
            CALL cl_err('Foreach asfg500_curs4:',SQLCA.sqlcode,1)
         END IF
         #將供給資料存入輸出暫存檔
         INSERT INTO out_file VALUES(tp1.*,0,0,'2')
...
#           CALL cl_err('Insert fail:ckp#02',SQLCA.sqlcode,1)   #No.FUN-660128
      ...
      IF tm.b = "Y" AND g_sma.sma32 = "Y" THEN  #供給明細含請購單
         FOREACH asfg500_curs5 USING l_sfa03 INTO tp1.*,l_pmk25,l_pml21
            IF SQLCA.SQLCODE THEN
               CALL cl_err('Foreach asfg500_curs5:',SQLCA.sqlcode,1)
            END IF
...
#   LET l_name = 'asfg500.out'                    #No.FUN-7C0007
#   CALL cl_outnam('asfg500') RETURNING l_name    #No.FUN-7C0007
#   START REPORT asfg500_rep TO l_name            #No.FUN-7C0007
 
#   LET g_pageno = 0                              #No.FUN-7C0007
   IF tm.c = "Y" THEN      #列印供給不足之料件明細
      DECLARE asfg500_curs7 CURSOR FOR SELECT UNIQUE out01
       FROM out_file ORDER BY out01
      LET g_sql =  "SELECT * FROM out_file ",
                   " WHERE out01 = ?   ",
                   " ORDER BY out01,out03,out08,out02"
      PREPARE asfg500_pre8 FROM g_sql
      DECLARE asfg500_curs8 CURSOR FOR asfg500_pre8
      MESSAGE 'Data Ordering.....'
      CALL ui.Interface.refresh()
      FOREACH asfg500_curs7 INTO l_out01
         IF SQLCA.SQLCODE THEN
            CALL cl_err('Foreach asfg500_curs7:',SQLCA.sqlcode,1)
         END IF
...
#        LET l_sum3 = l_ima262
         ...
         IF l_sum3 - l_sum1 + l_sum2 < 0 THEN
            ...
            CALL ui.Interface.refresh()
            FOREACH asfg500_curs8 USING l_out01 INTO sr.*
               IF SQLCA.sqlcode != 0  THEN
                  CALL cl_err('Foreach asfg500_curs8:',SQLCA.sqlcode,1)
                  EXIT FOREACH
               END IF
#No.FUN-7C0007---Begin
...
#     LET l_qty = l_ima262          #NO.FUN-A20044
      ...
      IF sr.out08 = '1' THEN  
         ...
         SELECT ima02,ima021 INTO l1_ima02,l1_ima021 FROM ima_file WHERE ima01 = l_sfb05   #FUN-D10098
         EXECUTE insert_prep USING sr.out01,sr.out02,sr.out03,sr.out04,sr.out05,                                                    
                                   sr.out06,sr.out08,l_ima02,l_ima021,l_ima08,l_ima25,                                                       
                                   l_qty,l_qty1,l_qty2,l_sfb05,l1_ima02,l1_ima021     #FUN-D10098 add l1_ima02,l1_ima021  
      END IF
      IF sr.out08 = '2' THEN  
         LET l_qty = l_qty + sr.out04
      #END IF   #MOD-940340
         ...
         EXECUTE insert_prep USING sr.out01,sr.out02,sr.out03,sr.out04,sr.out05,                                                    
                                   ...
                                   l_qty,l_qty1,l_qty2,l_sfb05,l1_ima02,l1_ima021    #FUN-D10098 add l1_ima02,l1_ima021   
      END IF   #MOD-940340
#               OUTPUT TO REPORT asfg500_rep(sr.*)
...
#No.FUN-7C0007---End
   ...
   IF tm.c = "N" THEN   #列印供需所有之料件明細
      DECLARE asfg500_curs6 CURSOR FOR SELECT *
                          FROM out_file
                          ORDER BY out01,out03,out08,out02
      MESSAGE 'Process Done , Printing.....'
      CALL ui.Interface.refresh()
      FOREACH asfg500_curs6 INTO sr.*
         IF SQLCA.sqlcode != 0  THEN
            CALL cl_err('Foreach asfg500_curs6:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
#No.FUN-7C0007---Begin
      SELECT ima02,ima021,ima08,ima25,ima50,ima48,ima49,        
...
#     LET l_qty = l_ima262        #NO.FUN-A20044
      ...
      IF sr.out08 = '2' THEN 
         ...
         LET sr.out06=''
         LET l_sfb05=''
         LET l1_ima02 = ''   #FUN-D10098
         LET l1_ima021 = ''  #FUN-D10098
         EXECUTE insert_prep USING sr.out01,sr.out02,sr.out03,sr.out04,sr.out05,                                                    
                                   sr.out06,sr.out08,l_ima02,l_ima021,l_ima08,l_ima25,                                                       
                                   l_qty,l_qty1,l_qty2,l_sfb05,l1_ima02,l1_ima021    #FUN-D10098 add l1_ima02,l1_ima021       
      END IF
...
#No.FUN-7C0007---End
   ...
   END IF
#No.FUN-7C0007---Begin 
#   FINISH REPORT asfg500_rep
###GENGRE###    LET g_str=''
###GENGRE###    LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED
###GENGRE###    CALL cl_prt_cs3('asfg500','asfg500',l_sql,g_str) 
    CALL asfg500_grdata()    ###GENGRE###
#No.FUN-7C0007---End
   DROP TABLE tmp_file
   DROP TABLE out_file
   MESSAGE ' '
...
#  DEFINE p_wo         VARCHAR(10), #work order number
   DEFINE p_wo         LIKE oea_file.oea01,          #No.FUN-680121 VARCHAR(16)#No.FUN-550067
          ...
          p_btflg      LIKE type_file.chr1,          #No.FUN-680121 VARCHAR(01)#blow through flag
          ...
          p_mps        LIKE type_file.chr1,          #No.FUN-680121 VARCHAR(01)#if MPS phantom, blow through flag (Y/N)
          p_yld        LIKE type_file.chr1,          #No.FUN-680121 VARCHAR(01)#inflate yield factor (Y/N)
...
#    tmp07        LIKE ima_file.ima26,        #No.FUN-680121 DECIMAL(12,3)#缺料數量
     ...
     tmp11        LIKE type_file.chr1,        #No.FUN-680121 VARCHAR(01)#旗標
     tmp12        LIKE gfe_file.gfe01,        #No.FUN-680121 VARCHAR(04)#發料單位
...
FUNCTION asfg500_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("asfg500")
        IF handler IS NOT NULL THEN
            START REPORT asfg500_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY lower(out01),out03,out08,out02"    #FUN-CB0073
          
            DECLARE asfg500_datacur1 CURSOR FROM l_sql
            FOREACH asfg500_datacur1 INTO sr1.*
                OUTPUT TO REPORT asfg500_rep(sr1.*)
            END FOREACH
            FINISH REPORT asfg500_rep
        END IF
...
REPORT asfg500_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_i      LIKE type_file.chr1   #FUN-CB0073 add
    
    ORDER EXTERNAL BY sr1.out01
