...
# Modify.........: No.MOD-5A0198 05/10/17 By pengu 當工單型態是'重工委外工單'時，報表(asfx410)的在製量會算到2次
...
# Modify.........: No.FUN-CB0001 12/11/01 By yangtt CR转XtraGrid
...
#               "l_wip.ima_file.ima26"           #NO.FUN-A20044   
   ...
   LET l_table = cl_prt_temptable('asfx410',g_sql) CLIPPED   # 產生Temp Table
   IF l_table = -1 THEN EXIT PROGRAM END IF                  # Temp Table產生
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?)"    #FUN-A60027 add 2?   #FUN-CB0001 add 1?
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'   # If background job sw is off
      THEN CALL asfx410_tm()        # Input print condition
      ELSE CALL asfx410()              # Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW asfx410_w AT p_row,p_col WITH FORM "asf/42f/asfx410"
...
#No.FUN-570240 --end--
     ...
     IF g_bgjob = 'Y' THEN
        ...
        ELSE
           ...
           CALL cl_cmdat('asfx410',g_time,l_cmd)    # Execute cmd at later time
        END IF
        CLOSE WINDOW asfx410_w
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
        EXIT PROGRAM
     END IF
     CALL cl_wait()
...
#             l_wip  LIKE ima_file.ima26    #FUN-720005
   ...
   PREPARE asfx410_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1)
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
       EXIT PROGRAM
   END IF
   DECLARE asfx410_curs1 CURSOR FOR asfx410_prepare1
 
   LET g_pageno = 0
   FOREACH asfx410_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0  THEN
...
#MOD-970151 --end--               
      ...
      EXECUTE insert_prep USING 
         ...
         sr.sfa28,sr.sfa161,sr.l_wip,l_num           #FUN-720005, l_wip    #FUN-BC0001 l_num
      #------------------------------ CR (3) ------------------------------#
      #end FUN-720005 add
   END FOREACH
...
###XtraGrid###      LET g_str = tm.wc
   END IF
  #CALL cl_prt_cs3('asfx410','asfx410',l_sql,g_str)   #FUN-710080 modify
  #No.FUN-A60027-----------------start-------------------
   IF g_sma.sma541  = 'Y' THEN
###XtraGrid###      CALL cl_prt_cs3('asfx410','asfx410_1',l_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
    IF g_zz05 = 'Y' THEN
       CALL cl_wcchp(tm.wc,'sfb01,sfb05,sfa03')
           RETURNING tm.wc
    END IF
    LET g_xgrid.template = 'asfx410_1'   #FUN-CB0001 add
    LET g_xgrid.order_field = 'sfa03'   #FUN-CB0001 add
    LET g_xgrid.grup_field = 'sfa03'   #FUN-CB0001 add
    LET g_xgrid.condition = cl_getmsg('lib-160',g_lang),tm.wc   #FUN-CB0001 add
    CALL cl_xg_view()    ###XtraGrid###
   ELSE
###XtraGrid###      CALL cl_prt_cs3('asfx410','asfx410',l_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
    IF g_zz05 = 'Y' THEN
       CALL cl_wcchp(tm.wc,'sfb01,sfb05,sfa03')
           RETURNING tm.wc
    END IF
    LET g_xgrid.template = 'asfx410'   #FUN-CB0001 add
    LET g_xgrid.order_field = 'sfa03'   #FUN-CB0001 add
    LET g_xgrid.grup_field = 'sfa03'   #FUN-CB0001 add
    LET g_xgrid.condition = cl_getmsg('lib-160',g_lang),tm.wc   #FUN-CB0001 add
    CALL cl_xg_view()    ###XtraGrid###
