...
# Modify.........: No:FUN-D60054 13/06/14 By lixh1 倒扣料完工入庫時入庫數量給預設值
...
# Modify.........: No.TQC-D40089 13/04/26 By dongsz 錄入FQC單帶出入庫數量調整
   ...
   #               " AND sfu01[1,",g_doc_len,"] NOT IN (SELECT smy71 FROM smy_file WHERE smy71 IS NOT NULL) "  #CHI-9B0005 mod
    ...
    IF cl_null(g_argv2) THEN    #bugno:6963
       CONSTRUCT g_wc2 ON sfv03,sfv17,sfv11,
                          ...
                          ,sfviicd028,sfviicd029   #FUN-B70061
               FROM s_sfv[1].sfv03, s_sfv[1].sfv17, s_sfv[1].sfv11,  #FUN-550085
                    ...
                    ,s_sfv[1].sfviicd028,s_sfv[1].sfviicd029   #FUN-B70061
                BEFORE CONSTRUCT
                   CALL cl_qbe_display_condition(lc_qbe_sn)
...
#FUN-AA0059---------mod------------end------------------
               ...
               WHEN INFIELD(sfv930)
                ...
                #FUN-BC0104---add---end
                #FUN-B70061 --START--
                WHEN INFIELD(sfviicd029 )
                   CALL q_slot(TRUE,TRUE,g_sfv[1].sfviicd029,'',g_sfv[1].sfv11)                         
                       RETURNING g_qryparam.multiret
                   DISPLAY g_qryparam.multiret TO sfviicd029
                   NEXT FIELD sfviicd029
                #FUN-B70061 --END--  
           END CASE
...
#                              " SELECT tse19 FROM tse_file WHERE tse19 IS NOT NULL) ))"
    ...
    LET g_sql = " SELECT UNIQUE  sfu01 ",   #No.TQC-9A1020 mod
                ...
                "  ORDER BY sfu01 "
   #FUN-B70061 --START--
   IF g_wc2.getindexof('sfvi',1)>0 THEN
      LET g_sql = " SELECT UNIQUE  sfu01 ",   
                   " FROM sfu_file LEFT OUTER JOIN sfv_file ON sfu01 = sfv01",
                   " LEFT OUTER JOIN sfvi_file ON (sfv01 = sfvi01 AND sfv03 = sfvi03)",
                   " WHERE sfu00 = '",g_argv,"'",
                   " AND ",g_wc CLIPPED, " AND ",g_wc2 CLIPPED,
                   " ORDER BY sfu01 "
   END IF                    
...
DEFINE l_flag       LIKE type_file.chr1      #TQC-B60082 add
   DEFINE l_dies      LIKE sfv_file.sfv09
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         WHEN "aic_s_icdqry"
            IF cl_chk_act_auth() THEN
               IF NOT cl_null(g_sfu.sfu01) THEN
                  CALL s_icdqry(1,g_sfu.sfu01,'',g_sfu.sfupost,'')
               END IF
            END IF
       ...
       #@WHEN 刻號/BIN號入庫明細
         WHEN "aic_s_icdin"
            IF cl_chk_act_auth() THEN
               IF (NOT cl_null(g_sfu.sfu01)) AND (g_sfu.sfupost = 'N')
                 #FUN-A80128--mod---str---
                 #AND (l_ac <> 0) THEN
                  AND (l_ac <> 0) AND g_sfu.sfu15 NOT matches '[Ss]' THEN
                 #FUN-A80128--mod---end---
                  #避免畫面資料是舊的，所以重撈
                  ...
                  SELECT sfbiicd14 INTO l_sfbiicd14 FROM sfbi_file
                   WHERE sfbi01 = l_sfv.sfv11
                  CALL s_icdin(1,l_sfv.sfv04, l_sfv.sfv05,
                                 l_sfv.sfv06, l_sfv.sfv07,
                                 l_sfv.sfv08, l_sfv.sfv09,
                                 g_sfu.sfu01, l_sfv.sfv03,
                                 g_sfu.sfu02, '',
                                 l_sfbiicd14, l_sfv.sfv11, '0')
                     RETURNING l_dies,l_r,l_qty   #FUN-C30302
                     #FUN-C30302---begin
                     IF l_r = 'Y' THEN
                         LET l_qty = s_digqty(l_qty,l_sfv.sfv08) 
                         LET g_sfv[l_ac].sfv09 = l_qty
                         LET g_sfv[l_ac].sfv32 = l_qty
                         LET g_sfv[l_ac].sfv35 = l_dies
                         UPDATE sfv_file set sfv09 = g_sfv[l_ac].sfv09,
                         ...
                         ELSE
                            LET g_success  = 'Y'
                         END IF 
                         DISPLAY BY NAME g_sfv[l_ac].sfv09,g_sfv[l_ac].sfv32,
                                         g_sfv[l_ac].sfv35
                     #FUN-C30302---end
                         CALL t620_ind_icd_upd_dies()   #FUN-B70061   
                     END IF #FUN-C30302
               #FUN-A80128  add str ---              
               ELSE
...
FUNCTION t620_chkkey()
    ...
    ELSE 
       UPDATE sfvi_file SET sfvi01=g_sfu.sfu01
        WHERE sfvi01=g_sfu_t.sfu01


       IF SQLCA.SQLCODE OR SQLCA.SQLERRD[3]=0 THEN
          CALL cl_err3("upd","sfvi_file",g_sfu_t.sfu01,"",SQLCA.sqlcode,"",
                       "upd sfvi01",1)  
          LET g_sfu.*=g_sfu_t.*
          CALL t620_show()
          ROLLBACK WORK
...
FUNCTION t620_r()
    ...
    IF cl_delh(20,16) THEN
        ...
        #FUN-BC0104---add---end---
        #FUN-B70061 --START--
        IF NOT s_del_sfvi(g_sfu.sfu01,'','') THEN
           #不作處理        
        END IF
        #FUN-B70061 --END--
...
#FUN-B20089 -----------------------------Begin----------------------------------
 ...
 FUNCTION t620_ins_sfv()
  ...
  IF SQLCA.sqlcode THEN
     ...
     LET g_success = 'N'
  ELSE
     INITIALIZE b_sfvi.* TO NULL
     LET b_sfvi.sfvi01 = b_sfv.sfv01
     LET b_sfvi.sfvi03 = b_sfv.sfv03
     IF NOT s_ins_sfvi(b_sfvi.*,b_sfv.sfvplant) THEN
        LET g_success = 'N'
     END IF
    END IF
 END FUNCTION
...
DEFINE l_where       STRING                    #FUN-CB0087   
    ...
    #FUN-B70061 --START--
    LET g_forupd_sql = " SELECT * FROM sfvi_file ",
                        " WHERE sfvi01 = ? ",
                        " AND sfvi03 = ? ",
                        " FOR UPDATE "
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    DECLARE t620_bcl_ind CURSOR FROM g_forupd_sql
    #FUN-B70061 --END--
        ...
        BEFORE ROW
            ...
            IF g_rec_b >= l_ac THEN
               ...
               ELSE
                  ...
                  ELSE
                     #FUN-B70061 --START--
                     OPEN t620_bcl_ind USING g_sfu.sfu01,g_sfv_t.sfv03
                     IF STATUS THEN
                        CALL cl_err("OPEN t620_bcl_ind:", STATUS, 1)
                        CLOSE t620_bcl_ind
                        ROLLBACK WORK
                        RETURN
                     ELSE 
                        FETCH t620_bcl_ind INTO b_sfvi.*
                        IF SQLCA.sqlcode THEN
                           CALL cl_err('lock sfvi',SQLCA.sqlcode,1)
                           LET l_lock_sw = "Y"
                        END IF 
                     END IF
                     #FUN-B70061 --END--
...
#&ifdef ICD    #FUN-B50096 
               ...
               END IF
               #FUN-BC0104---add---end---
               CALL t620_set_no_required_sfviicd028()   #FUN-B70061
               CALL t620_set_required_sfviicd028(p_cmd) #FUN-B70061
               CALL t620_set_no_required_sfviicd029()   #FUN-B70061
               CALL t620_set_required_sfviicd029(p_cmd) #FUN-B70061
               #TQC-C60020---begin
               IF NOT cl_null(g_sfv[l_ac].sfv04) THEN 
                  SELECT imaicd09 INTO l_imaicd09 FROM imaicd_file
                   WHERE imaicd00 = g_sfv[l_ac].sfv04
                  IF l_imaicd09 = 'Y' THEN
                     CALL cl_set_comp_entry('sfviicd028',TRUE)
                  ELSE
                     CALL cl_set_comp_entry('sfviicd028',FALSE)
                  END IF 
               END IF 
               #TQC-C60020---end
...
#FUN-B50096 -------------------End---------------------
           ...
           END IF
           #FUN-A40022--end--add--------------
           CALL t620_def_sfviicd029()   #FUN-B70061
           CALL t620_set_no_entry_sfv07()   #FUN-B50096
           #MOD-C30084--begin
        ...
        ON ROW CHANGE
            ...
            ELSE
               ...
               #FUN-B70061 --START--
                  LET g_success = 'N'
               ELSE                  
                  UPDATE sfvi_file SET * = b_sfvi.*
                   WHERE sfvi01=g_sfu.sfu01 AND sfvi03=g_sfv_t.sfv03 
                  IF SQLCA.sqlcode THEN
                     CALL cl_err('upd sfvi',SQLCA.sqlcode,0)
                     LET g_sfv[l_ac].* = g_sfv_t.*
                     LET g_success = 'N'                     
                  END IF
               END IF
    ...
    #CHI-C30118---add---END
        ...
        ON ACTION controlp
           CASE WHEN INFIELD(sfv11)    #工單單號
                ...
                #FUN-B70061 --START--
                WHEN INFIELD(sfviicd029 )
                   CALL q_slot(FALSE,TRUE,g_sfv[l_ac].sfviicd029,g_sfv[l_ac].sfv07,g_sfv[l_ac].sfv11)                         
                       RETURNING g_sfv[l_ac].sfviicd029
                   DISPLAY BY NAME g_sfv[l_ac].sfviicd029
                   NEXT FIELD sfviicd029
                #FUN-B70061 --END--  
           END CASE
...
#須走FQC流程-合計FQC單總完工入庫量
    ...
    #批號(sfv07) = 由所輸入之工單單號(sfv11)串工單檔之回貨批號批號(sfbiicd13)
    IF cl_null(g_sfv_t.sfv11) OR g_sfv[p_ac].sfv11 ! = g_sfv_t.sfv11 THEN
       IF g_argv = '1' THEN
          SELECT sfbiicd13 INTO g_sfv[l_ac].sfv07
            FROM sfbi_file
           WHERE sfbi01=g_sfv[l_ac].sfv11
          DISPLAY BY NAME g_sfv[p_ac].sfv07
       END IF
    END IF
END FUNCTION
...
#FUN-AA0059---------mod------------end-----------------
   ...
   FOR k=1 TO l_sfb.getLength()
       ...
       IF g_success = 'Y' THEN
          ...
          LET l_sfvi.sfvilegal = l_sfv.sfvlegal
          LET l_sfvi.sfviplant = l_sfv.sfvplant
...
DEFINE p_wc2           LIKE type_file.chr1000       #No.FUN-680121 CHAR(200)
       ...
       LET g_sql = "SELECT sfv03,sfv17,sfv14,sfv11,sfv46,qcl02,sfv47,sfv04,ima02,ima021,", #FUN-550085 #FUN-5C0114 add sfv14  #FUN-BC0104 add sfv46,qcl02,sfv47
                   ...
                   " ORDER BY sfv03"
       #FUN-B70061 --START--
       LET g_sql = "SELECT sfv03,sfv17,sfv14,sfv11,sfv46,qcl02,sfv47,sfv04,ima02,ima021,",
                   "sfv08,sfv05,sfv06,sfv07,sfv09,sfv33,sfv34,",
                   "sfv35,sfv30,sfv31,sfv32,sfv41,sfv42,sfv43,sfv44,azf03,sfv12,sfv16,sfv930,'',  ", 
                   "sfvud01,sfvud02,sfvud03,sfvud04,sfvud05,",
                   "sfvud06,sfvud07,sfvud08,sfvud09,sfvud10,",
                   "sfvud11,sfvud12,sfvud13,sfvud14,sfvud15,",
                   "sfviicd028,sfviicd029 ", 
                   " FROM sfv_file LEFT OUTER JOIN ima_file ON sfv04 = ima01 ",  
                   "               LEFT OUTER JOIN azf_file ON sfv44 = azf01 AND azf02='2' AND azfacti='Y'",
                   "               LEFT OUTER JOIN qcl_file ON sfv46 = qcl01 ",   #FUN-BC0104
                   "               LEFT OUTER JOIN sfvi_file ON (sfv01 = sfvi01 AND sfv03 = sfvi03)",
                   " WHERE sfv01 ='",g_sfu.sfu01,"'",
                   " AND ",p_wc2 CLIPPED,  
...
FUNCTION t620_set_entry_b(p_cmd)
 DEFINE p_cmd   LIKE type_file.chr1          #No.FUN-680121 CHAR(1)
   ...
   CALL cl_set_comp_entry("sfviicd028,sfviicd029",TRUE)   #FUN-B70061
...
FUNCTION t620_set_no_entry_b(p_cmd)
 ...
 IF l_ac > 0 THEN
    ...
    #FUN-BA0051 --END mark--
    IF s_icdbin(g_sfv[l_ac].sfv04) THEN   #FUN-BA0051
       CALL cl_set_comp_entry("sfviicd028,sfviicd029",FALSE)
    END IF
    #TQC-C60020---begin
    IF NOT cl_null(g_sfv[l_ac].sfv04) THEN 
       SELECT imaicd09 INTO l_imaicd09 FROM imaicd_file
      ...
      IF g_cnt = 0 AND l_bmm03_cnt = 0 THEN
         ...
         IF g_argv = '1' THEN
            ...
            #CALL cl_getmsg('asf1037',g_lang) RETURNING g_x[1]  #MOD-D60129 mark
             CALL cl_getmsg('asf1037',g_lang) RETURNING g_x[1]  #MOD-D60129 add
...
FUNCTION t620_b_move_to()
   ...
   LET g_sfv[l_ac].sfviicd028 = b_sfvi.sfviicd028 #FUN-B70061
   LET g_sfv[l_ac].sfviicd029 = b_sfvi.sfviicd029 #FUN-B70061
END FUNCTION
...
FUNCTION t620_b_move_back()
   ...
   LET b_sfvi.sfviicd028 = g_sfv[l_ac].sfviicd028 #FUN-B70061
   LET b_sfvi.sfviicd029 = g_sfv[l_ac].sfviicd029 #FUN-B70061
...
#FUN-B70061 --START--
FUNCTION t620_set_required_sfviicd028(p_cmd)
   ...
   IF p_cmd='u' OR INFIELD(sfv04) THEN
      IF NOT cl_null(g_sfv[l_ac].sfv04) THEN
         SELECT imaicd09 INTO l_imaicd09 FROM imaicd_file
          WHERE imaicd00 = g_sfv[l_ac].sfv04
         IF l_imaicd09 = 'Y' THEN            
            CALL cl_set_comp_required("sfviicd028",TRUE)
         END IF
      END IF
   END IF
END FUNCTION

FUNCTION t620_set_no_required_sfviicd028()
     CALL cl_set_comp_required("sfviicd028",FALSE)
END FUNCTION

FUNCTION t620_set_required_sfviicd029(p_cmd)
   ...
   IF p_cmd='u' OR INFIELD(sfv04) THEN
      IF NOT cl_null(g_sfv[l_ac].sfv04) THEN
         SELECT imaicd13 INTO l_imaicd13 FROM imaicd_file
          WHERE imaicd00 = g_sfv[l_ac].sfv04
         IF l_imaicd13 = 'Y' THEN            
            CALL cl_set_comp_required("sfviicd029",TRUE)
         END IF
      END IF
   END IF
END FUNCTION

FUNCTION t620_set_no_required_sfviicd029()
     CALL cl_set_comp_required("sfviicd029",FALSE)
END FUNCTION
#FUN-B70061 --END--
...
DEFINE l_sql STRING 
   ...
   #IF NOT cl_null(l_imaicd08) AND l_imaicd08 <> 'Y' THEN 
   #FUN-BA0051 --END mark--
   IF NOT s_icdbin(g_sfv[l_ac].sfv04) THEN   #FUN-BA0051   
      LET l_sql = "SELECT sfe03 FROM sfe_file",
                   " WHERE sfe01 = '", g_sfv[l_ac].sfv11, "' "                                         
      PREPARE t620_def_p FROM l_sql
      DECLARE sfvi_icd_cs CURSOR FOR t620_def_p
      OPEN sfvi_icd_cs
      FETCH sfvi_icd_cs INTO l_sfe03
      CLOSE sfvi_icd_cs
      LET g_sfv[l_ac].sfviicd029 = l_sfe03   
      DISPLAY BY NAME g_sfv[l_ac].sfviicd029
   END IF 
END FUNCTION

#回寫母批/Datacode
FUNCTION t620_ind_icd_upd_dies()
    #DEFINE l_imaicd08  LIKE imaicd_file.imaicd08   #FUN-BA0051 mark       
    ...
    IF s_icdbin(g_sfv[l_ac].sfv04) THEN   #FUN-BA0051   
       LET g_sql = "SELECT ida15 FROM ida_file",   #FUN-BC0109 del ida16
                   " WHERE ida07 = '", g_sfu.sfu01, "'",
                   " AND ida08 =", g_sfv[l_ac].sfv03       
       DECLARE t620_upd_dia_c CURSOR FROM g_sql            
       OPEN t620_upd_dia_c                                       
       FETCH t620_upd_dia_c INTO g_sfv[l_ac].sfviicd029 #FUN-BC0109 del ,g_sfv[l_ac].sfviicd028
       #串接Date Code值
       CALL s_icdfun_datecode('2',g_sfu.sfu01,g_sfv[l_ac].sfv03) 
                                RETURNING g_sfv[l_ac].sfviicd028  #FUN-BC0109   
       UPDATE sfvi_file set sfviicd029 = g_sfv[l_ac].sfviicd029,
        sfviicd028 = g_sfv[l_ac].sfviicd028
        WHERE sfvi01=g_sfu.sfu01 AND sfvi03=g_sfv[l_ac].sfv03 
       IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
          LET g_sfv[l_ac].sfviicd029 = b_sfvi.sfviicd029
          LET g_sfv[l_ac].sfviicd028 = b_sfvi.sfviicd028
          LET g_success  = 'N'
       ELSE
          LET b_sfvi.sfviicd029 = g_sfv[l_ac].sfviicd029
          LET b_sfvi.sfviicd028 = g_sfv[l_ac].sfviicd028
          LET g_success  = 'Y'
       END IF 
       DISPLAY BY NAME g_sfv[l_ac].sfviicd029, g_sfv[l_ac].sfviicd028       
    END IF
