    ...
    WHILE TRUE
      ...
      CASE g_action_choice
         ...
         WHEN "auto_b"                                                    
           IF cl_chk_act_auth() THEN                                            
              CALL t705_auto_b('u') 
              CALL t705_b_fill(' 1=1')  #No.FUN-8A0151
   ...
   INPUT BY NAME g_sgk.sgk01,g_sgk.sgk02,g_sgk.sgk03,g_sgk.sgk04,
      ...
      AFTER FIELD sgk01
       ...
       AFTER FIELD  sgk03
         ...
         END IF
       AFTER FIELD sgk05 
         IF (p_cmd="u" AND g_sgk.sgk05='N') THEN 
             SELECT COUNT(*) INTO l_n FROM sgl_file 
               WHERE sgl01=g_sgk.sgk01 AND sgl03 IS NOT  NULL 
           IF l_n>0 THEN
             CALL cl_err('','asf-253',0)
             NEXT FIELD sgk05
           END IF 
         END IF
          
       AFTER FIELD sgk06 
         IF (p_cmd="u" AND g_sgk.sgk06='Y') THEN 
             SELECT COUNT(*) INTO l_n FROM sgl_file 
               WHERE sgl01=g_sgk.sgk01 AND sgl06 IS NOT NULL 
           IF l_n>0 THEN
             CALL cl_err('','asf-254',0)
             NEXT FIELD sgk06
           END IF
         END IF   
       AFTER INPUT                                                             
     ...
     BEFORE ROW
       ...
       AFTER INSERT
        ...
        AFTER FIELD sgl12
           ...
           END IF
        AFTER FIELD sgl03
          IF g_sgl[l_ac].sgl03 IS NOT NULL THEN
#FUN-AA0059 ---------------------start----------------------------
             IF NOT s_chk_item_no(g_sgl[l_ac].sgl03,"") THEN
                CALL cl_err('',g_errno,1)
                LET g_sgl[l_ac].sgl03= g_sgl_t.sgl03
                NEXT FIELD sgl03
             END IF
#FUN-AA0059 ---------------------end-------------------------------
             IF p_cmd="a" OR (p_cmd="u" AND g_sgl[l_ac].sgl03 !=g_sgl_t.sgl03) THEN
              SELECT COUNT(*) INTO l_m1 FROM skh_file 
                 WHERE skh01 = g_sgl[l_ac].sgl03
              IF  l_m1>0 THEN   
               IF g_sgk.sgk06 = 'Y'  THEN 
                SELECT COUNT(*) 
                    INTO l_m2
                     FROM skh_file
                     WHERE (( skh07 IS  NULL) OR (skh08 = ' ' AND skh08 IS  NULL) ) 
                     AND  skh_file.skh100 ='Y' AND  skh01 = g_sgl[l_ac].sgl03
                SELECT COUNT(*) 
                    INTO l_m
                     FROM skh_file
                     WHERE (( skh07 IS  NULL) OR (skh08 = ' ' AND skh08 IS  NULL) ) 
                     AND  (NOT exists (select * from sgl_file where skh_file.skh01 = sgl_file.sgl03 )or skh_file.skh01 = ''  )
                     AND  skh_file.skh100 ='Y' AND  skh01 = g_sgl[l_ac].sgl03     
                     IF l_m = 0 THEN
                       IF l_m2=0 THEN 
                         CALL cl_err(g_sgl[l_ac].sgl03,'asf-300',0)
                         NEXT FIELD sgl03
                       ELSE 
                       	  CALL cl_err(g_sgl[l_ac].sgl03,'asf-258',0)
                          NEXT FIELD sgl03
                       END IF     
                     END IF 
                ELSE 
                    ...
                    INTO l_m2
                     FROM skh_file
                     WHERE (( skh07 IS NOT NULL) OR (skh08 != ' ' AND skh08 IS NOT NULL) )  
                     AND  skh_file.skh100 ='Y' AND  skh01 = g_sgl[l_ac].sgl03
                	 SELECT COUNT(*) 
                    INTO l_m
                     FROM skh_file
                     ...
                     IF l_m = 0 THEN 
                       IF l_m2 =0 THEN 
                        CALL cl_err(g_sgl[l_ac].sgl03,'asf-259',0)
                        NEXT FIELD sgl03
                       ELSE 
                       	 CALL cl_err(g_sgl[l_ac].sgl03,'asf-258',0)
                         NEXT FIELD sgl03 
                ...
                END IF
               ELSE 
               	CALL cl_err(g_sgl[l_ac].sgl03,'ask-008',0)
                NEXT FIELD sgl03 
               END IF         
                SELECT skh06,skh07,skh08 
                   INTO l_skh06,l_skh07,l_skh08 
                   FROM skh_file
                   WHERE skh01 = g_sgl[l_ac].sgl03
                   AND skh100='Y'
                   AND skh_file.skh01 not IN
                   (select sgl03 from sgl_file 
                   where sgl_file.sgl03!=g_sgl_t.sgl03)
                IF SQLCA.sqlcode THEN
                   CALL cl_err('','asf-255',0)
                   NEXT FIELD sgl03
                END IF        
                SELECT ima02,ima55,sfb05 INTO l_ima02,l_ima55,l_sfb05 
                   FROM ima_file,sfb_file
                    WHERE sfb_file.sfb01=l_skh06
                ...
                IF g_sgk.sgk06='N' THEN
                  IF ((l_skh07 IS NULL OR l_skh07=0) OR (l_skh08 IS NULL OR l_skh08=' '))THEN
                     CALL cl_err('','asf-250',1)
                     NEXT FIELD sgl03
                  END IF
                ELSE
                	IF NOT((l_skh07 IS NULL OR l_skh07=0) AND (l_skh08 IS NULL OR l_skh08=' ')) THEN
                	   CALL cl_err('','asf-251',1)
                     NEXT FIELD sgl03
                  END IF
                END IF
#若sgk05='Y',則輸入飛票號碼后，自動從飛票檔skh_file
#帶出工單編號，工序，單元編號，產品料號，產品名稱,良品數量，報工單位字段
                IF g_sgk.sgk05='Y' THEN
                  LET g_sgl[l_ac].sgl08=l_skh12
                  LET g_sgl[l_ac].sgl04=l_skh06
                  LET g_sgl[l_ac].sgl06=l_skh07
                  LET g_sgl[l_ac].sgl07=l_skh08
                  ...
                  DISPLAY BY NAME g_sgl[l_ac].sgl08,g_sgl[l_ac].sgl09
                  LET g_sgl10_t = g_sgl[l_ac].sgl10
                  #No.FUN-BB0086--add--end--
                  LET g_sgl[l_ac].sgl09 = 0
                  LET g_sgl[l_ac].sgl11 = 0
                  CALL t705_sgl08 (g_sgl[l_ac].sgl03,g_sgl[l_ac].sgl04,g_sgl[l_ac].sgl07) RETURNING g_sgl[l_ac].sgl08
                END IF 
             NEXT FIELD sgl04
            ELSE
            LET g_sgl[l_ac].sgl08=g_sgl_t.sgl08
            LET g_sgl[l_ac].sgl04=g_sgl_t.sgl04
            LET g_sgl[l_ac].sgl06=g_sgl_t.sgl06
            LET g_sgl[l_ac].sgl07=g_sgl_t.sgl07
            LET g_sgl[l_ac].sgl05=g_sgl_t.sgl05
            LET g_sgl[l_ac].ima02=g_sgl_t.ima02
            LET g_sgl[l_ac].sgl10=g_sgl_t.sgl10
            LET g_sgl[l_ac].sgl09 = g_sgl_t.sgl09 
            LET g_sgl[l_ac].sgl11 = g_sgl_t.sgl11
            END IF 
          END IF 
        AFTER FIELD sgl04
        ...
        ON ACTION CONTROLP
          CASE
            ...
            WHEN INFIELD(sgl03)
                 CALL cl_init_qry_var()                                         
                 LET g_qryparam.form      = "q_sgl03" 
                 LET g_qryparam.arg1=g_sgl_t.sgl03 
                 IF g_sgk.sgk06 ='Y' THEN 
                   LET g_qryparam.where = "( skh07 IS NULL) AND (skh08 = ' ' OR skh08 IS NULL)"
                 ELSE 
                 	 LET g_qryparam.where = "( skh07 IS NOT NULL) OR (skh08 != ' ' AND skh08 IS NOT NULL)" 
                 END IF 	 
                 LET g_qryparam.default1  = g_sgl[l_ac].sgl03
                 CALL cl_create_qry() RETURNING g_sgl[l_ac].sgl03
                 CALL FGL_DIALOG_SETBUFFER(g_sgl[l_ac].sgl03)
                 NEXT FIELD sgl03
             WHEN INFIELD(sgl04)
                 CALL cl_init_qry_var()
                 LET g_qryparam.form      = "q_sgl04"
