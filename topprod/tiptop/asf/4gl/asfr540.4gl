...
# Modify.........: No.MOD-6A0125 06/10/27 By eric757 因發料asfi501  已將sfe06異動別改為1,2,3,4 需調整 asfr540,asfr550 倒扣料程式段
...
# Modify.........: No.TQC-D10046 13/01/09 By xuxz 添加sbf01，sfb85開窗
...
#No.FUN-750098-------Begin                                                                                                       
   ...
   LET l_table = cl_prt_temptable('asfr540',g_sql) CLIPPED                                                                          
   IF l_table = -1 THEN EXIT PROGRAM END IF                                                                                         
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,                                                                            
               " VALUES(?, ?, ?, ?, ?,  ?, ?, ?, ?, ?,",
               "        ?, ?, ?, ?, ?,  ?, ?, ?, ?, ?,",
               "        ?, ?)"   #FUN-A60027 add 2?  #CHI-C60014 add ?
...
#No.FUN-750098---------End  
  ...
  #LET tm.more  = ARG_VAL(8)  #TQC-610080 以下順序調整
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL asfr540_tm(0,0)        # Input print condition
      ELSE CALL asfr540()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
END MAIN
 
FUNCTION asfr540_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   END IF
   OPEN WINDOW asfr540_w AT p_row,p_col
        WITH FORM "asf/42f/asfr540"
################################################################################
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('asfr540',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW asfr540_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM
...
#         l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT  #TQC-630166         #No.FUN-680121 VARCHAR(1200)
   ...
   DEFINE  l_factor LIKE bmb_file.bmb10_fac,  #CHI-C60014
     ...
     PREPARE asfr540_pre1 FROM l_sql
     IF STATUS THEN CALL cl_err('',STATUS,0) RETURN END IF
     DECLARE asfr540_cur1 CURSOR FOR asfr540_pre1
     IF STATUS THEN CALL cl_err('',STATUS,0) RETURN END IF
...
#----------------No.MOD-590036 end
     ...
     PREPARE asfr540_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('prepare:',SQLCA.sqlcode,1)
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
         EXIT PROGRAM
     END IF
     DECLARE asfr540_curs1 CURSOR FOR asfr540_prepare1
#No.FUN-750098----Begin
    #CALL cl_outnam('asfr540') RETURNING l_name    
    #START REPORT asfr540_rep TO l_name
     LET g_pageno = 0
     FOREACH asfr540_curs1 INTO sr.*
#No.FUN-750098 ----End 
       IF SQLCA.sqlcode != 0  THEN
       ...
       #CALL r540_amt(sr.sfb01,sr.bmb03,sr.sfb39) RETURNING exhaust #No.MOD-590036 mark
       CALL r540_amt(sr.sfb01,sr.sfa03,sr.sfb39,sr.sfa26,sr.sfa08,sr.sfa012,sr.sfa013) RETURNING exhaust  #No.MOD-590036 add   #FUN-A60027 add sfa012,sfa013
       #差異量計算
   ...
   #---No.MOD-590036 add 若有取替代時將取替代料件之未耗量算在被替換皆料件上
       IF sr.sfa26 MATCHES '[348]' THEN      #FUN-A20037 add '8'
          ...
          PREPARE asfr540_pre3 FROM l_sql
          IF STATUS THEN CALL cl_err('',STATUS,0) RETURN END IF
          DECLARE asfr540_cur3 CURSOR FOR asfr540_pre3
          IF STATUS THEN CALL cl_err('',STATUS,0) RETURN END IF
          FOREACH asfr540_cur3 USING sr.sfb01,sr.sfa03,sr.sfa012,sr.sfa013      #FUN-A60027 add sfa012,sfa013
...
#      OUTPUT TO REPORT asfr540_rep(sr.*,exhaust)
    #CHI-C60014---S---
     LET sr.l_sfa12 = ''
     LET sr.l_sfa27 = ''
...
#    FINISH REPORT asfr540_rep
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                               
     #是否列印選擇條件                                                                                                              
     IF g_zz05 = 'Y' THEN                                                                                                           
        CALL cl_wcchp(tm.wc,'sfb01,sfb05,sfb85') RETURNING tm.wc                                                        
        LET g_str = tm.wc                                                                                                           
     END IF
    #CALL cl_prt_cs3('asfr540','asfr540',l_sql,g_str)   #FUN-A60027
    #FUN-A60027 ----------------start----------------------
    IF g_sma.sma541 = 'Y' THEN
       CALL cl_prt_cs3('asfr540','asfr540_1',l_sql,g_str)
    ELSE
       CALL cl_prt_cs3('asfr540','asfr540',l_sql,g_str)
    END IF 
    #FUN-A60027 -----------------end----------------------- 
#No.FUN-750098----End
END FUNCTION
...
#FUNCTION r540_amt(p_sfb01,p_bmb03,p_sfb39)                   #No.MOD-590036 mark
FUNCTION r540_amt(p_sfb01,p_sfa03,p_sfb39,p_sfa26,p_sfa08,p_sfa012,p_sfa013)    #No.MOD-590036 add  #FUN-A60027 add sfa012,sfa013
 ...
 LET exhaust = 0
 #FOREACH asfr540_cur1 USING p_sfb01,p_bmb03 INTO l_sfe06,l_sfe16  #No.MOD-590036 mark
 FOREACH asfr540_cur1 USING p_sfb01,p_sfa03,p_sfa08,p_sfa012,p_sfa013 INTO l_sfe06,l_sfe16 #No.MOD-590036 add   #FUN-A60027 add sfa012,sfa013
...
#-----No.MOD-590036 add 若為替代料件時將耗用量算在被替代料件上
 IF p_sfa26 MATCHES '[348]' THEN  #FUN-A20037 add '8'
    ...
    CALL r540_amt(p_sfb01,l_sfa03,p_sfb39,p_sfa26,p_sfa08,p_sfa012,p_sfa013) RETURNING exhaust1   #FUN-A60027 add sfa012,sfa013
