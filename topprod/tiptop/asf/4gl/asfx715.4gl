...
# Modify.........: No.FUN-D40129 13/05/06 By yangtt 【線/班別】欄位新增開窗；新增Ctrl+g功能
#                                                    添加對【工單編號】開窗的說明欄位
...
## *** 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>> 2007/02/13 TSD.Martin  *** ##
    ...
    LET l_table = cl_prt_temptable('asfx715',g_sql) CLIPPED   # 產生Temp Table
...
#----------------------------------------------------------CR (1) ------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      THEN CALL x715_tm(0,0)        # Input print condition
      ELSE CALL asfx715()        # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
END MAIN
 
FUNCTION x715_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   END IF
   OPEN WINDOW x715_w AT p_row,p_col
        WITH FORM "asf/42f/asfx715"
################################################################################
...
#No.FUN-570240 --start--
     ...
     ON ACTION controlp
            #FUN-CB0002--add--str--
            IF INFIELD(shb05) THEN
               CALL cl_init_qry_var()
              #LET g_qryparam.form = "q_shb05"     #FUN-D40129
               LET g_qryparam.form = "q_shb05_1"   #FUN-D40129
...
#No.FUN-570240 --end--
            #FUN-D40129---add---str--
            IF INFIELD(shb08) THEN
               CALL cl_init_qry_var()
               LET g_qryparam.form = "q_shb08"
               LET g_qryparam.state = "c"
               CALL cl_create_qry() RETURNING g_qryparam.multiret
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN LET INT_FLAG = 0 CLOSE WINDOW x715_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM 
   END IF
...
################################################################################
   ...
   END IF
   IF g_bgjob = 'Y' THEN
      SELECT zz08 INTO l_cmd FROM zz_file  WHERE zz01='asfx715'
      IF SQLCA.sqlcode OR l_cmd IS NULL THEN
         CALL cl_err('asfx715','9031',1)
      ELSE
         ...
         CALL cl_cmdat('asfx715',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW x715_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0090
    ...
    #End:FUN-980030
     ...
     PREPARE x715_prepare1 FROM l_sql
     ...
     END IF
     DECLARE x715_cs1 CURSOR FOR x715_prepare1
     LET g_tot2 = 0
     LET g_count=0
     FOREACH x715_cs1 INTO sr.*,sr1.*
...
###XtraGrid###        LET g_str = tm.wc
     #END IF
     #FUN-CB0002--mark--end--
    #CALL cl_prt_cs3('asfx715','asfx715',l_sql,g_str)   #FUN-710080 modify   #FUN-A60027 
    #FUN-A60027 -------------------start------------------------
     IF g_sma.sma541 = 'Y' THEN
###XtraGrid###        CALL cl_prt_cs3('asfx715','asfx715_1',l_sql,g_str)
        LET g_xgrid.table = l_table    ###XtraGrid###
        LET g_xgrid.template = 'asfx715_1'
        LET g_xgrid.order_field = "shb10"
        IF g_zz05 = 'Y' THEN
           CALL cl_wcchp(tm.wc,'shb05,shb10,shb08') 
                RETURNING tm.wc
        END IF
     ...
     ELSE
###XtraGrid###        CALL cl_prt_cs3('asfx715','asfx715',l_sql,g_str)
        LET g_xgrid.table = l_table    ###XtraGrid###
        LET g_xgrid.template = 'asfx715'
        LET g_xgrid.order_field = "shb10"
        IF g_zz05 = 'Y' THEN
           CALL cl_wcchp(tm.wc,'shb05,shb10,shb08') 
                RETURNING tm.wc
