...
# Modify.........: No:MOD-B50060 11/05/09 By sabrina r201()時要判斷g_wc2是否為null
...
#             "l_qty.ima_file.ima262,",         #NO.FUN-A20044
   ...
   LET l_table = cl_prt_temptable('asfr201',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN
       CALL r201_tm()                       # If background job sw is off
   ELSE
       CALL r201_gen1()  #TQC-730022
       CALL r201()                      # Read data and create out-file
   END IF
...
DEFINE   li_result   LIKE type_file.num5              #No.FUN-550067        #No.FUN-680121 SMALLINT
   ...
   OPEN WINDOW r201_w AT p_row,p_col WITH FORM "asf/42f/asfr201"
...
#          IF g_sfc01[5,10] <> ' ' THEN
              ...
              DECLARE r201_sfd_c CURSOR FOR
                SELECT * FROM sfd_file WHERE sfd01=g_sfc01 ORDER BY 1
              LET g_cnt=1
              FOREACH r201_sfd_c INTO l_sfd.*
...
#No.FUN-550067-end
     ...
     WHILE TRUE
        ...
        IF INT_FLAG THEN LET INT_FLAG=0 CLOSE WINDOW r201_w 
           CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
           EXIT PROGRAM 
        END IF
...
#No.FUN-550067-end
    ...
    #------------No.MOD-780063 end
     ...
     END INPUT
     IF INT_FLAG THEN
        LET INT_FLAG = 0 CLOSE WINDOW r201_w 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
        EXIT PROGRAM
    ...
    #----------------No.MOD-780063 end
     IF g_bgjob = 'Y' THEN
        ...
        ELSE
           ...
           CALL cl_cmdat('asfr201',g_time,l_cmd)    # Execute cmd at later time
        END IF
        CLOSE WINDOW r201_w
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
        EXIT PROGRAM
...
FUNCTION r201_gen1()
  #DEFINE l_sql         LIKE type_file.chr1000       #No.FUN-680121 VARCHAR(1100) #No.TQC-C80084 mark
   ...
   PREPARE r201_gen_p2 FROM l_sql
   DECLARE r201_gen_c2 CURSOR FOR r201_gen_p2
   FOREACH r201_gen_c2 INTO l_sfb01
      FOR i=1 TO 100
...
FUNCTION r201_gen()
   ...
   OPEN WINDOW r201_gen_w AT 10,25
   ...
   PREPARE r201_gen_p FROM l_sql
   DECLARE r201_gen_c CURSOR FOR r201_gen_p
   FOREACH r201_gen_c INTO l_sfb01
      FOR i=1 TO 100
         IF g_no[i]=l_sfb01 THEN CONTINUE FOREACH END IF
      END FOR
      FOR i=1 TO 100
...
#    CALL cl_outnam('asfr201') RETURNING l_name
#    START REPORT r201_rep TO l_name
    #-----------No.MOD-780063 modify
     IF cl_null(g_wc2) THEN LET g_wc2 = '1=1' END IF      #MOD-B50060 add
     FOR l_cnt=1 TO 100
        #DECLARE r201_sfa_c CURSOR FOR
        ...
        #           #AND ima23 BETWEEN b_keeper AND e_keeper
         ...
         PREPARE r201_p1 FROM l_sql
         DECLARE r201_sfa_c CURSOR FOR r201_p1
     #-----------No.MOD-780063 end
         FOREACH r201_sfa_c INTO sr.*
...
#           SELECT ima262 INTO l_ima262 FROM ima_file WHERE ima01=sr.sfa03  #NO.FUN-A20044
            ...
            #--------------------------------- 扣除前面已出現部份
            LET l_qty2=0
            SELECT qty INTO l_qty2 FROM r201_tmp WHERE partno=sr.sfa03
#           LET l_ima262=l_ima262-l_qty2                                    #NO.FUN-A20044
            LET l_avl_stk = l_avl_stk - l_qty2                              #NO.FUN-A20044
            UPDATE r201_tmp SET qty=qty+l_qty WHERE partno=sr.sfa03
            IF SQLCA.SQLERRD[3]=0 THEN
               INSERT INTO r201_tmp VALUES(sr.sfa03, l_qty)
            END IF
 
            IF tm.QVL_flag='Y' THEN
               DECLARE r201_QVL_c CURSOR FOR
                  SELECT * FROM bml_file
                         WHERE bml01 = sr.sfa03
                           AND (bml02='ALL' OR bml02=sr.sfb05)
                         ORDER BY bml03
                  LET l_qvl = ""
               FOREACH r201_QVL_c INTO l_bml.*
                  LET l_qvl = "    QVL:"
...
#                 SELECT bmd_file.*, ima02,ima021,ima262 FROM bmd_file, ima_file  #NO.FUN-A20044
                  SELECT bmd_file.*, ima02,ima021,0 FROM bmd_file, ima_file    #NO.FUN-A20044
                         ...
                         ORDER BY bmd03
               LET l_flag=0
#              FOREACH r201_SUB_c INTO l_bmd.*, l_ima02,l_ima021, l_ima262a     #NO.FUN-A20044
               FOREACH r201_SUB_c INTO l_bmd.*, l_ima02,l_ima021, l_avl_stka    #NO.FUN-A20044
                   LET l_flag=1
...
#           OUTPUT TO REPORT r201_rep(sr.*)
   ...
   # CALL cl_prt_cs3('asfr201',l_sql,l_str) 
     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED  
     CALL cl_prt_cs3('asfr201','asfr201',l_sql,l_str) 
#    FINISH REPORT r201_rep
...
#                  ORDER BY bmd03
#        LET g_cnt=0
#        FOREACH r201_SUB_c INTO l_bmd.*, l_ima02,l_ima021, l_ima262
