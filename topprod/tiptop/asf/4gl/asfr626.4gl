...
MAIN
   ...
   LET l_table = cl_prt_temptable('asfr626',g_sql) CLIPPED   
   ...
   IF cl_null(g_program) THEN LET g_program = 'asfr626' END IF
   IF cl_null(g_bgjob) OR g_bgjob = 'N'   # If background job sw is off
      THEN CALL r626_tm(0,0)        # Input print condition
      ELSE CALL r626()              # Read data and create out-file
   END IF
...
END CONSTRUCT
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('asfr626',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW r626_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM
...
#                "ksc06,ksc07,ksd03,ksd11,ksd04,ima02,ksd14,ksd15,ksd08,",         #No.FUN-710082
     ...
     #No.FUN-710082--begin
#    CALL cl_outnam('asfr626') RETURNING l_name
#    #No.FUN-580005 --start--
...
#    CALL cl_prt_pos_len()
#    #No.FUN-580005 --end--
#    START REPORT r626_rep TO l_name
#    LET g_pageno = 0
 
     CALL cl_del_data(l_table) 
 
     FOREACH r626_curs1 INTO sr.*
          IF SQLCA.sqlcode != 0 THEN
             CALL cl_err('foreach:',SQLCA.sqlcode,1)
             EXIT FOREACH
          END IF
#          OUTPUT TO REPORT r626_rep(sr.*)
         ...
         EXECUTE insert_prep USING sr.ksc00,sr.ksc01,sr.ksc02,sr.ksd17,sr.ksc04,   #CHI-950036 ksc03-->ksd17
                                   ...
                                   sr.ksd07,sr.ksd09,sr.ksd13,sr.ksd12,l_str2,
                                   "",l_img_blob,"N",""                             #TQC-C10039 ADD "",l_img_blob,"N",""
 
     END FOREACH
 
#    FINISH REPORT r626_rep
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
     ...
     IF g_zz05 = 'Y' THEN                                                         
        ...
        RETURNING tm.wc                                                           
     END IF                      
     LET l_str = tm.wc CLIPPED,";",g_zz05,";",g_sma115

#TQC-C10039--add--start---
     LET g_cr_table = l_table      #主報表的temp table名稱
     LET g_cr_apr_key_f = "ksc01"  #報表主鍵欄位名稱，用"|"隔開 
#TQC-C10039--add--end---
  #  LET l_sql = "SELECT * FROM ",l_table CLIPPED  #TQC-730088 
  #  CALL cl_prt_cs3('asfr626',l_sql,l_str) 
     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED  
     CALL cl_prt_cs3('asfr626','asfr626',l_sql,l_str) 
     #No.FUN-710082--end  
END FUNCTION
