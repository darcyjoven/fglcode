...
MAIN
   ...
   LET l_table = cl_prt_temptable('asfr732',g_sql) CLIPPED
   ...
   LET l_table1 = cl_prt_temptable('asfr7321',g_sql) CLIPPED
   IF l_table1 = -1 THEN EXIT PROGRAM END IF
   #No.FUN-710082--end  
 
   CALL cl_used(g_prog,g_time,1) RETURNING g_time #FUN-BB0047 add
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      THEN CALL r732_tm(0,0)        # Input print condition
      ELSE CALL asfr732()        # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
END MAIN
 
FUNCTION r732_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   END IF
   OPEN WINDOW r732_w AT p_row,p_col
        WITH FORM "asf/42f/asfr732"
################################################################################
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN LET INT_FLAG = 0 CLOSE WINDOW r732_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM 
   END IF
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      SELECT zz08 INTO l_cmd FROM zz_file  WHERE zz01='asfr732'
      IF SQLCA.sqlcode OR l_cmd IS NULL THEN
         CALL cl_err('asfr732','9031',1)
      ELSE
         ...
         CALL cl_cmdat('asfr732',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW r732_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM
...
#               "   shb115,shb16,ima02,gen02,ecm03,ecm04,ecm45,ecm06,ecm41 ",        #No.MOD-640281 add         #No.FUN-710082
   ...
   #            "   AND shc_file.shc01 = shb01 AND qce_file.qce01=shc_file.shc04",
     ...
     PREPARE r732_prepare1 FROM l_sql
     ...
     END IF
     DECLARE r732_cs1 CURSOR FOR r732_prepare1
...
#                " ORDER BY 1"      #No.FUN-710082
     PREPARE r732_p2 FROM l_sql
     IF STATUS THEN CALL cl_err('p2:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
        EXIT PROGRAM 
     END IF
     DECLARE r732_c2 CURSOR FOR r732_p2
 
     #No.FUN-710082--begin
#    CALL cl_outnam('asfr732') RETURNING l_name
#    START REPORT r732_rep TO l_name
     ...
     LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
                 " VALUES(?,?,?,?,?, ?,?,?,?,?,",
                 "        ?,?,?,?,?, ?,?,?,?,?,",
                 "        ?,?,?,?,?, ?,?,?,?,?,?) "      ##FUN-A60076 add ? #TQC-C10039 add 4?
     ...
     #----------------No.MOD-640281 add
       ...
       END IF
     #----------------No.MOD-640281 end
#      OUTPUT TO REPORT r732_rep(sr.*)
 
         FOREACH r732_c2 USING sr.shb01,sr.shb05 INTO sr1.*
            IF SQLCA.sqlcode != 0 THEN
               CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
            END IF
...
#TQC-C10039--add--start---
     ...
     LET g_cr_apr_key_f = "shb01"  #報表主鍵欄位名稱，用"|"隔開
#TQC-C10039--add--end---
   # CALL cl_prt_cs3('asfr732',l_sql,l_str)    #TQC-730088
   # CALL cl_prt_cs3('asfr732','asfr732',l_sql,l_str)  # FUN-A60027 mark by huangtao
     #No.FUN-710082--end  
      #FUN-A60027------------------start---------------
     IF g_sma.sma541 = 'Y' THEN
        CALL cl_prt_cs3('asfr732','asfr732_1',l_sql,l_str)
     ELSE
        CALL cl_prt_cs3('asfr732','asfr732',l_sql,l_str)
     END IF 
    #FUN-A60027 -----------------end----------------
