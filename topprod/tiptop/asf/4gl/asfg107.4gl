...
# Modify.........: No.FUN-CB0072 12/12/03 By dongsz CR轉GR
...
###GENGRE###START
TYPE sr1_t RECORD
    order1 LIKE type_file.chr1000,   #FUN-CB0072 mod chr1000 -> order1
    sfa03 LIKE sfa_file.sfa03,
    ima02 LIKE ima_file.ima02,
    ima021 LIKE ima_file.ima021,      #FUN-CB0072 add
    ima25 LIKE ima_file.ima25,
    avl_stk LIKE type_file.num15_3,
    num10 LIKE type_file.num10,
    sfb08 LIKE sfb_file.sfb08,
    sfb09 LIKE sfb_file.sfb09,
    sfb10 LIKE sfb_file.sfb10,
    sfb11 LIKE bed_file.bed07,
    sfb01 LIKE sfb_file.sfb01,
    sfb05 LIKE sfb_file.sfb05,
    ima02_1 LIKE ima_file.ima02,      #FUN-CB0072 add
    ima021_1 LIKE ima_file.ima021,    #FUN-CB0072 add
...
MAIN
 ...
 #No.FUN-750115---Begin                                                         
   LET g_sql = "order1.type_file.chr1000,",   #FUN-CB0072 mod chr1000 -> order1
...
#              "ima262.ima_file.ima262,",
               ...
               "ima02_1.ima_file.ima02,",     #FUN-CB0072 add
               "ima021_1.ima_file.ima021,",   #FUN-CB0072 add
               ...
               "num2.type_file.num5"                     #FUN-CB0072 add
                                                                                
   LET l_table = cl_prt_temptable('asfg107',g_sql) CLIPPED                      
   IF l_table = -1 THEN EXIT PROGRAM END IF                                     
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,                        
               " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) "             #FUN-A60027 add 2?  #FUN-CB0072 add 5?                       
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW r107_w AT p_row,p_col WITH FORM "asf/42f/asfg107"
     ...
     END IF
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('asfg107',g_time,l_cmd)
         END IF
         CLOSE WINDOW r107_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
         CALL cl_gre_drop_temptable(l_table)            #FUN-CB0072 add
...
#                   ima262   LIKE ima_file.ima262,
   ...
   DEFINE l_num2        LIKE type_file.num5              #FUN-CB0072 add
...
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'asfg107'        #No.FUN-530808
     #Begin:FUN-980030
...
#                "sfa03,ima02,ima25,ima262,0,0,0,0,0,", #NO.FUN-A20044
                 "sfa03,ima02,ima021,ima25,0,0,0,0,0,0,",      #NO.FUN-A20044   #FUN-CB0072 add ima021
...
#    CALL cl_outnam('asfg107') RETURNING l_name           #No.FUN-750115
#No.FUN-550067-begin
#No.FUN-530808--begin
#    LET g_len = 88
...
#         LET sr.tot = sr.ima262 + sr.sfl07 + sr.sqty + sr.poqty + sr.ppqty   #NO.FUN-A20044
         ...
         #FUN-CB0072 --add--str---
          SELECT ima02,ima02 INTO l_ima02,l_ima021 FROM ima_file
           WHERE ima01 = sr.sfb05
         #FUN-CB0072---add--end---
          LET l_num1 = 3           #FUN-CB0072 add
          LET l_num2 = 2           #FUN-CB0072 add
#No.FUN-750115---Begin
#         OUTPUT TO REPORT r107_rep(sr.*)
#         EXECUTE insert_prep USING s_order,sr.sfa03,sr.ima02,sr.ima25,      #No.FUN-760085
          EXECUTE insert_prep USING sr.order,sr.sfa03,sr.ima02,sr.ima021,sr.ima25,     #No.FUN-760085   #FUN-CB0072 add sr.ima021
#                                   sr.ima262,sr.sfl07,sr.sqty,sr.poqty,     #NO.FUN-A20044
                                    ...
                                    sr.sfb05,l_ima02,l_ima021,'','',sr.ndate,    #FUN-CB0072 add l_ima02,l_ima021
                                    sr.sfa12,sr.sfa05,sr.qty,sr.sfa07,sr.sfa012,sr.sfa013,l_num1,l_num2  #No.FUN-A60027 add sfa012,sfa013 #FUN-CB0072 add l_num1,l_num2
#No.FUN-750115---End
     END FOREACH
...
###GENGRE###     LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED 
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog         #FUN-CB0072 add
     IF g_zz05 = 'Y' THEN                                                       
        CALL cl_wcchp(tm.wc,'sfa03,sfb01,sfb82,sfb05,sfb85,sfb13')    #FUN-7B0052 add sfb13                 
        RETURNING tm.wc                                                         
        LET l_str = l_str CLIPPED,";",tm.wc                                     
     END IF         
###GENGRE###     LET l_str = tm.wc 
 
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
#    CALL cl_prt_cs3('asfg107','asfg107',l_sql,l_str)   #FUN-A60027
     #No.FUN-A60027 ------------------start----------------
     IF g_sma.sma541 = 'Y' THEN
###GENGRE###        CALL cl_prt_cs3('asfg107','asfg107_1',l_sql,l_str)
        LET g_template = 'asfg107_1'     #FUN-CB0072 add
        CALL asfg107_grdata()    ###GENGRE###
     ELSE
###GENGRE###        CALL cl_prt_cs3('asfg107','asfg107',l_sql,l_str)
        LET g_template = 'asfg107'       #FUN-CB0072 add
        CALL asfg107_grdata()    ###GENGRE###
     END IF 
     #No.FUN-A60027 ----------------end--------------------   
...
FUNCTION asfg107_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("asfg107")
        IF handler IS NOT NULL THEN
            START REPORT asfg107_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                       ," ORDER BY lower(sfa03),order1"           #FUN-CB0072 add
          
            DECLARE asfg107_datacur1 CURSOR FROM l_sql
            FOREACH asfg107_datacur1 INTO sr1.*
                OUTPUT TO REPORT asfg107_rep(sr1.*)
            END FOREACH
            FINISH REPORT asfg107_rep
        END IF
...
REPORT asfg107_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_sfa05_sum_1 LIKE sfa_file.sfa05
    DEFINE l_sft05_sum_1 LIKE sft_file.sft05
    DEFINE l_sfa07_sum_1 LIKE sfa_file.sfa07
    DEFINE l_sfa05_sum_2 LIKE sfa_file.sfa05
    DEFINE l_sft05_sum_2 LIKE sft_file.sft05
    DEFINE l_sfa07_sum_2 LIKE sfa_file.sfa07
    DEFINE l_sfb11 STRING                    #FUN-CB0072 add
    
    ORDER EXTERNAL BY sr1.sfa03,sr1.order1  #FUN-CB0072 mod chr1000 -> order1
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.sfa03
           #FUN-CB0072--add--str---
            IF sr1.sfb11 >= 0 THEN 
               LET l_sfb11 = cl_gr_getmsg('gre-325',g_lang,1)
            ELSE
               LET l_sfb11 = cl_gr_getmsg('gre-325',g_lang,2)
            END IF
            PRINTX l_sfb11 
        ...
        AFTER GROUP OF sr1.sfa03
            ...
            PRINTX l_sfa07_sum_1
            LET l_sft05_sum_1 = GROUP SUM(sr1.sft05)
            PRINTX l_sft05_sum_1
            LET l_sfa05_sum_1 = GROUP SUM(sr1.sfa05)
            PRINTX l_sfa05_sum_1
        AFTER GROUP OF sr1.order1  
            LET l_sfa07_sum_2 = GROUP SUM(sr1.sfa07)
            PRINTX l_sfa07_sum_2
            LET l_sft05_sum_2 = GROUP SUM(sr1.sft05)
            PRINTX l_sft05_sum_2
            LET l_sfa05_sum_2 = GROUP SUM(sr1.sfa05)
            PRINTX l_sfa05_sum_2
