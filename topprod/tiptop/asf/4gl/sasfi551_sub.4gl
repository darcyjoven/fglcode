...
#
# Program name...: sasfi551_sub.4gl
# Description....: 提供sasfi551.4gl使用的sub routine
# Date & Author..: 07/04/27 by kim (FUN-740187)
# Modify.........: No.MOD-740055 07/05/03 By pengu 備料有三筆資料狀況為 3, U, U時sasfi551 中計算欠料資料時會錯誤
...
# Modify.........: No:MOD-B10169 11/01/21 By lilingyu 調用i551sub_z()函數時,需要增加控管是否在事務裡面
...
# Modify.........: No:MOD-B30062 11/03/08 By sabrina RETURN l_rvb.rvb01,l_rvb.rvb02應在call i551_vmi_tlf()及i551_vmi_tlff()之後
...
# Modify.........: No.TQC-B90236 11/11/08 By zhuhao FUNCTION i551sub_u_o_sfa中，增加判斷
# Modify.........: No.CHI-BA0020 11/11/10 By jason 過帳時取替代料檢查已發數量不可大於應發數量時,不可將超領量納入
# Modify.........: No:MOD-BB0111 11/11/12 By johung 調整i551_tlff_c問題
...
# Modify.........: No:TQC-C60079 12/06/11 By fengrui 函數i551sub_y_chk(p_sfp01)添加參數
...
# Modify.........: No:TQC-C70085 12/07/13 By fengrui 修正i551sub_y_chk函數中的RETURN
...
FUNCTION i551sub_s(p_argv1,p_sfp01,p_inTransaction,p_ask_post)
...
DEFINE l_qty2      LIKE sfb_file.sfb08     #MOD-D30125
   ...
   IF l_sfp.sfp06='6' THEN
      CALL i551sub_disassemble(l_sfp.sfp01,'2')
      IF g_success = 'N' THEN 
         #IF NOT s_industry('icd') THEN #CHI-C90045 #FUN-CC0095
         #   CALL s_padd_img_drop(l_img_table)    #FUN-CC0095
         #   CALL s_padd_imgg_drop(l_imgg_table)  #FUN-CC0095
   ...
   IF p_ask_post='Y' THEN  #FUN-840012 外部呼叫的程式不可出現詢問視窗
      ...
      DECLARE i551sub_s_c CURSOR FOR SELECT * FROM sfs_file
             WHERE sfs01 = p_sfp01
      LET g_success = "Y"
      CALL s_showmsg_init()
 
      FOREACH i551sub_s_c INTO l_sfs.*
         IF STATUS THEN
            EXIT FOREACH
         END IF
...
#     AND sfs05 > 0    #MOD-D60189 mark
   ...
   #FUN-C70087---begin
   DECLARE i551_s1_c4 CURSOR FOR SELECT * FROM sfs_file
     WHERE sfs01 = l_sfp.sfp01 
   #CHI-C90045
   IF s_industry('icd') THEN  
      FOREACH i551_s1_c4 INTO l_sfs.*
   ...
   DECLARE i551_s1_c3 CURSOR FOR SELECT * FROM sfs_file
     WHERE sfs01 = l_sfp.sfp01
      LET g_success = "Y"
...
#TQC-C40174 -----------------End-------------------
     ...
     #與MES整合:工單型態(sfb02)= 1 or 13   
      IF g_success = 'Y' AND g_aza.aza90 MATCHES "[Yy]" THEN
         CALL i551_sub_chk_sfb01(l_sfs.sfs03,g_mescnt)  #傳入:工單單號,第幾筆資料
     ...
     #FUN-C10035 add end -----
      ...
      IF NOT s_industry('icd') THEN  #FUN-910077
         ...
         IF (l_sfb02 = '7' OR l_sfb02 = '8') AND l_sfb100 = '1' THEN
            DECLARE i551_s1_c9 CURSOR FOR 
            SELECT pmm25,pmm18  INTO  l_pmm25,l_pmm18
              FROM pmm_file,pmn_file
             WHERE pmm01=pmn01
               AND pmn41=l_sfs.sfs03
            FOREACH i551_s1_c9 INTO l_pmm25,l_pmm18
     ...
     WHERE sfq02 NOT IN (SELECT sfs03 FROM sfs_file WHERE sfs01=l_sfp.sfp01)
      ...
      #AND sfq03 > 0       #MOD-B60169 mark
       AND sfq03 >= 0      #MOD-B60169 add
         ...
         FOREACH i551_cs10 INTO l_sfq02
 ...
 #  IF g_success = 'Y' THEN  #FUN-B80093 mark                              #MOD-C10100    #TQC-C10033
     IF g_success = 'Y' AND  g_sma.sma93 = 'Y' THEN  #FUN-B80093 add      #MOD-C10100   因正式區sma32未增加好，暫MARK，待後續還原  #TQC-C10033
        DECLARE i551sub_sfs_cs CURSOR FOR SELECT * FROM sfs_file
          WHERE sfs01 = l_sfp.sfp01 ORDER BY sfs07,sfs08 
   
     LET g_cnt_vmi = 1
     FOREACH i551sub_sfs_cs INTO g_sfs_vmi[g_cnt_vmi].*
        IF STATUS THEN EXIT FOREACH END IF
        SELECT COUNT(*) INTO l_cnt
          FROM pmc_file
        ...
        IF l_cnt > 0 THEN 
           CALL i551sub_VMI_settlement(l_sfp.*,g_sfs_vmi[g_cnt_vmi].*) 
        END IF    
     ...
     IF g_success = 'Y' THEN
        IF l_sfp.sfp06 MATCHES '[13]' THEN  #FUN-AC0074
            DECLARE  i551sub_sif_cs CURSOR FOR SELECT * FROM sfs_file 
              WHERE sfs01 = l_sfp.sfp01 
            FOREACH i551sub_sif_cs INTO g_sfs_sif.*
              IF STATUS THEN EXIT FOREACH END IF
        ...
        END IF #FUN-AC0074
         ...
         #   END IF 
         # END IF 
         # CALL i551sub_ins_sif('1',g_sfs_sif.*) 
         #END IF 
         #END IF 
        #END FOREACH   
        #FUN-AC0074--end--mark------
     ...
     CALL i551sub_lock_cl()
   
     OPEN i551sub_cl USING l_sfp.sfp01
     IF STATUS THEN
        CALL cl_err("OPEN i551sub_cl:", STATUS, 1)
        CLOSE i551sub_cl
     ...
     FETCH i551sub_cl INTO l_sfp.*          # 鎖住將被更改或取消的資料
     ...
     ELSE
        ...
        CALL i551sub_s1(l_sfp.*) RETURNING l_sfp.*
        IF g_success = 'Y' THEN
     IF NOT l_sfp.sfp06 MATCHES '[ABC]' THEN #FUN-5C0114
        CALL i551sub_s2(p_argv1,l_sfp.*)
     ELSE
        CALL i551sub_s2_asr(p_argv1,l_sfp.*) #FUN-5C0114
     END IF
        END IF
     END IF
  #No.FUN-A10001  mark by dxfwo 
  #   IF g_success = 'Y' THEN
  #      CALL i551sub_VMI_settlement(l_sfp.*)
  #   END IF
  #No.FUN-A10001  mark by dxfwo  
   
     IF s_industry('icd') THEN
        CALL i551sub_ind_icd_upd_sfb08('Y',l_sfp.*)
     END IF
  ...
  #FUN-9A0095 add end-----
     ...
     DECLARE i551sub_sie_c CURSOR FOR
     ...
     #  FROM sfs_file 
     # WHERE sfs01=l_sfp.sfp01
     FOREACH i551sub_sie_c INTO l_sfe01,l_sfe07,l_sfe14,l_sfe17,l_sfe27,l_sfe012,l_sfe013
  ...
  #     CALL i551sub_VMI_conf(l_sfp.sfp01)
        IF g_success = 'N' THEN
          IF NOT p_inTransaction THEN           #MOD-B10169
             CALL i551sub_z('1',l_sfp.sfp01,'',TRUE)
          END IF                                #MOD-B10169
          IF NOT s_industry('icd') THEN #CHI-C90045
             #CALL s_padd_img_drop(l_img_table)   #FUN-CC0095 
             #CALL s_padd_imgg_drop(l_imgg_table)  #FUN-CC0095 
  ...
  #   END IF
     ...
     IF l_sfp.sfp04 = "N" THEN
        DECLARE i551_s1_c2 CURSOR FOR SELECT * FROM sfs_file
    WHERE sfs01 = l_sfp.sfp01
  ...
  #FUNCTION i551sub_ins_sif(p_sia04,l_sfs)
  ...
  FUNCTION i551sub_VMI_settlement(p_sfp,l_sfs)
   ...
   SELECT * INTO l_pmc.* FROM pmc_file
    WHERE pmc917 = l_sfs.sfs07
     AND pmc918 = l_sfs.sfs08
        ...
        ##MOD-C50190  add end-------------------
        #MOD-C50220 mark end--------------------
        CALL i551sub_VMI_insinainb(p_sfp.sfp06,l_sfs.*,l_pmc.*,p_sfp.*)
        IF g_success = 'N' THEN RETURN  END IF
        
       #CALL i551sub_VMI_insrvarvb(p_sfp.sfp06,l_sfs.*,l_pmc.*)  RETURNING l_rvb01,l_rvb02  #FUN-B40053 mark
       #FUN-B40053 add begin--------------------------------- 
        IF p_sfp.sfp06 MATCHES '[1234AC]' THEN
          #CALL i551sub_VMI_insrvarvb(p_sfp.sfp06,l_sfs.*,l_pmc.*)  RETURNING l_rvb01,l_rvb02   #TQC-C70080 mark
           CALL i551sub_VMI_insrvarvb(p_sfp.sfp06,l_sfs.*,l_pmc.*,p_sfp.sfp03)  RETURNING l_rvb01,l_rvb02   #TQC-C70080 add sfp03
        END IF
       #FUN-B40053 add -end----------------------------------
        IF g_success = 'N' THEN RETURN END IF
       ...
       #CALL i551sub_VMI_insrvurvv(l_rvb01,l_rvb02,p_sfp.sfp06,l_sfs.*,l_pmc.*)  #TQC-C70080 mark
        CALL i551sub_VMI_insrvurvv(l_rvb01,l_rvb02,p_sfp.sfp06,l_sfs.*,l_pmc.*,p_sfp.sfp03) #TQC-C70080 add sfp03
        IF g_success = 'N' THEN RETURN END IF
  ...
  FUNCTION i551sub_VMI_insinainb(p_sfp06,p_sfs,p_pmc,p_sfp)
  ...
  #     LET g_ina_vmi.inaconf  = 'N'   #No.FUN-A10001
    ...
    ##NO.FUN-A10001   add--begin
     ...
     IF g_ina_vmi.ina00 = '1' THEN 
       CALL i551_vmi_tlf(g_inb_vmi.inb04,g_inb_vmi.inb05,g_inb_vmi.inb06,g_inb_vmi.inb07,
     ...
     IF g_ina_vmi.ina00 = '1' THEN 
      CALL i551_vmi_tlff(g_inb_vmi.inb04,g_inb_vmi.inb05,g_inb_vmi.inb06,g_inb_vmi.inb07,
            g_inb_vmi.inb09*g_inb_vmi.inb08_fac,'2',
            g_inb_vmi.inb01,g_inb_vmi.inb03,g_inb_vmi.inb11,'',
           #g_inb_vmi.inb08,g_inb_vmi.inb08_fac,g_ina_vmi.ina04,'')   #TQC-C70080 mark
            g_inb_vmi.inb08,g_inb_vmi.inb08_fac,g_ina_vmi.ina04,'',g_ina_vmi.ina02)   #TQC-C70080 add g_ina_vmi.ina02
  ...
  END FUNCTION
    ...
    ##NO.FUN-A10001   add--begin
 #FUNCTION i551_vmi_tlf(p_part,p_ware,p_loca,p_lot,p_qty,p_sta,p_no,p_no1,p_no2,p_no3,p_unit,p_factor,p_gov) #FUN-740187
  FUNCTION i551_vmi_tlf(p_part,p_ware,p_loca,p_lot,p_qty,p_sta,p_no,p_no1,p_no2,p_no3,p_unit,p_factor,p_gov,p_sfp03) #TQC-C70080 add p_sfp03
     DEFINE
...
# FUNCTION i551_vmi_tlff(p_part,p_ware,p_loca,p_lot,p_qty,p_sta,p_no,p_no1,p_no2,p_no3,p_unit,p_fac,p_gov,p_tlff219)  #TQC-C70080 mark
  FUNCTION i551_vmi_tlff(p_part,p_ware,p_loca,p_lot,p_qty,p_sta,p_no,p_no1,p_no2,p_no3,p_unit,p_fac,p_gov,p_tlff219,p_sfp03)   #TQC-C70080 add p_sfp03
     DEFINE
        p_part  LIKE img_file.img01,   ##料件編號(p_part)
        p_ware  LIKE img_file.img02,   ##倉庫 #MOD-580001
        p_loca  LIKE img_file.img03,   ##儲位 #MOD-580001
...
#FUNCTION i551sub_VMI_insrvarvb(p_sfp06,p_sfs,p_pmc)
FUNCTION i551sub_VMI_insrvarvb(p_sfp06,p_sfs,p_pmc,p_sfp03)  #TQC-C70080 add p_sfp03
   DEFINE p_sfs      RECORD LIKE sfs_file.*
   DEFINE p_pmc      RECORD LIKE pmc_file.*
  ...
  #  LET l_rvb.rvb03 = ''         #No.FUN-A10001
     ...
     CALL i551sub_VMI_set_rvb87(l_rvb.*) RETURNING l_rvb.rvb87
    ...
    ##NO.FUN-A10001   add--begin
     CALL i551_vmi_tlf(l_rvb.rvb05,l_rvb.rvb36,l_rvb.rvb37,l_rvb.rvb38,
     ...
     CALL i551_vmi_tlff(l_rvb.rvb05,l_rvb.rvb36,l_rvb.rvb37,l_rvb.rvb38,
  ...
  FUNCTION i551sub_VMI_set_rvb87(p_rvb)
...
#TQC-C50247 add end     
 ...
 #FUNCTION i551sub_VMI_insrvurvv(p_rvb01,p_rvb02,p_sfp06,p_sfs,p_pmc)   #No.TQC-9C0163
  FUNCTION i551sub_VMI_insrvurvv(p_rvb01,p_rvb02,p_sfp06,p_sfs,p_pmc,p_sfp03)   #TQC-C70080 add p_sfp03
  ...
  #     LET l_rvu.rvuconf = 'N'
    ...
    AND img04=l_rvv.rvv34
        ...
        CALL s_upimg(l_rvv.rvv31,l_rvv.rvv32,l_rvv.rvv33,l_rvv.rvv34,+1,
         ...
         1,'','','','','','','')
    IF l_rvu.rvu00 = '1' THEN
      CALL i551_vmi_tlf(l_rvv.rvv31,l_rvv.rvv32,l_rvv.rvv33,l_rvv.rvv34,
    ...
    #多單位處理
      ...
      IF NOT cl_null(l_rvv.rvv83) THEN
         ...
         CALL s_upimgg(l_rvv.rvv31,l_rvv.rvv32,l_rvv.rvv33,l_rvv.rvv34,
           ...
           '','','',l_rvv.rvv84)
         IF l_rvu.rvu00 = '1' THEN
           CALL i551_vmi_tlff(l_rvv.rvv31,l_rvv.rvv32,l_rvv.rvv33,l_rvv.rvv34,
           l_rvv.rvv17*l_rvv.rvv35_fac,'4',l_rvv.rvv04,
           l_rvv.rvv05,l_rvv.rvv01,l_rvv.rvv02,l_rvv.rvv35,
          #l_rvv.rvv35_fac,l_rvv.rvv06,'')   #TQC-C70080 mark
           l_rvv.rvv35_fac,l_rvv.rvv06,'',l_rvv.rvv09)   #TQC-C70080 add l_rvv.rvv09
  ...
  #FUNCTION i551sub_VMI_conf(p_sfp01)
  ...
  # #####--執行收貨單(spmt110)確認功能 ------------#####
  #   IF g_success = 'Y' THEN
  #   DECLARE i551sub_VMI_rva CURSOR FOR 
  #   SELECT * INTO l_rva.* FROM rva_file WHERE rva117=p_sfp01
  #   FOREACH i551sub_VMI_rva INTO l_rva.*
  # 
  ...
  # 
  #   IF g_success = 'N' THEN
  #      DECLARE i551sub_VMI_rva1 CURSOR FOR
  #       SELECT * INTO l_rva.* FROM rva_file WHERE rva117=p_sfp01
  #      FOREACH i551sub_VMI_rva1 INTO l_rva.*
 ...
 #FUNCTION i551sub_VMI_z(p_sfp01)           #MOD-C20155 mark
  FUNCTION i551sub_VMI_z(p_sfp01,p_argv1)   #MOD-C20155 add
   DEFINE p_sfp01  LIKE sfp_file.sfp01
   DEFINE l_cnt    LIKE type_file.num5
   ...
   DEFINE p_argv1  LIKE type_file.chr1      #MOD-C20155 add
     ...
     RETURN
        ELSE
    #CALL i551sub_VMI_z1(p_sfp01)            #MOD-C20155 mark
     CALL i551sub_VMI_z1(p_sfp01,p_argv1)    #MOD-C20155 add
 ...
 #FUNCTION i551sub_VMI_z1(p_sfp01)          #MOD-C20155 mark
  FUNCTION i551sub_VMI_z1(p_sfp01,p_argv1)  #MOD-C20155 add
   DEFINE p_sfp01      LIKE sfp_file.sfp01
   DEFINE l_rva        RECORD LIKE rva_file.*
   DEFINE l_rvu        RECORD LIKE rvu_file.*
   ...
   #FUN-BC0104-add-end--
    ##NO.FUN-A10001   add--begin 
     DECLARE i551sub_VMI_rva2 CURSOR FOR                                           
     SELECT * INTO l_rva.* FROM rva_file WHERE rva117=p_sfp01                     
     DECLARE i551sub_VMI_rvu2 CURSOR FOR 
     SELECT * INTO l_rvu.* FROM rvu_file WHERE rvu117=p_sfp01
     DECLARE i551sub_VMI_ina2 CURSOR FOR                                           
     SELECT * INTO l_ina.* FROM ina_file WHERE ina103 = p_sfp01
    
  #   FOREACH i551sub_VMI_rva2 INTO l_rva.*
  ...
  #      CALL cl_cmdrun_wait(l_cmd)
     ...
     #TQC-D30044--mark--end--
  #還原入庫/退貨單
     FOREACH i551sub_VMI_rvu2 INTO l_rvu.*
        DECLARE i551sub_VMI_rvv2 CURSOR FOR 
        SELECT * INTO l_rvv.* FROM rvv_file WHERE rvv01=l_rvu.rvu01
        FOREACH i551sub_VMI_rvv2 INTO l_rvv.*
         IF l_rvu.rvu00 = '1' THEN 
  ...
  #      LET l_cmd = "apmt111 '",l_rva.rva01 CLIPPED,"' '' 'V' "
  #      CALL cl_cmdrun_wait(l_cmd)
        DECLARE i551sub_VMI_rvb2 CURSOR FOR 
        SELECT * INTO l_rvb.* FROM rvb_file WHERE rvb01=l_rva.rva01
        FOREACH i551sub_VMI_rvb2 INTO l_rvb.* 
     DELETE FROM tlf_file
...
##MOD-C30882 add end
  ...
  #  CALL cl_cmdrun_wait(l_cmd)
  #還原雜收/發單
     FOREACH i551sub_VMI_ina2 INTO l_ina.*
        DECLARE i551sub_VMI_inb2 CURSOR FOR 
        SELECT * INTO l_inb.* FROM inb_file WHERE inb01=l_ina.ina01   
        FOREACH i551sub_VMI_inb2 INTO l_inb.*
  ...
  FUNCTION i551sub_s1(l_sfp)
    ...
    DECLARE i551_s1_c CURSOR FOR
        SELECT * FROM sfq_file WHERE sfq01=l_sfp.sfp01
   
    FOREACH i551_s1_c INTO l_sfq.*
        IF STATUS THEN
        ...
        IF s_industry('slk') THEN 
           ...
           END IF 
           DECLARE i551_cs41 CURSOR FROM l_sql   
           FOREACH i551_cs41 INTO qty1
        ...
        ELSE 
          ...
          #END FOREACH
           ...
           FOREACH i551_cs4_1 INTO qty1_1
                   IF STATUS THEN LET qty1_1=0 END IF
                   EXIT FOREACH
           END FOREACH

           #未按作業編號已發套數
           DECLARE i551_cs4_2 CURSOR FOR
            SELECT SUM(sfq03) FROM sfp_file,sfq_file
            #WHERE sfp06='1'  
             WHERE sfp06 IN ('1','D')  #FUN-C70014 sfp06='1' --> sfp06 IN ('1','D')
               AND sfp01=sfq01
           ...
           FOREACH i551_cs4_2 INTO qty1_2
        ...
        IF s_industry('slk') THEN  
           ...
           ELSE 
              LET l_sql = "SELECT min(sum(sfq03)) from sfp_file ,sfq_file ",  #No.TQC-930166 add
                          ...
                          "   and sfp04 = 'Y'  group by sfq04"
           END IF 
           DECLARE i551_cs51 CURSOR FROM l_sql 
           FOREACH i551_cs51 INTO qty2
               IF STATUS THEN LET qty2=0 END IF
               EXIT FOREACH
           END FOREACH  
        ELSE                          
          #MOD-B50119  mod      
          #DECLARE i551_cs5 CURSOR FOR
          # SELECT SUM(sfq03) FROM sfp_file,sfq_file
          #  WHERE sfp06='6'
          #    AND sfp01=sfq01
          #    AND sfq02=l_sfq.sfq02
          ...
          #END FOREACH
           ...
           DECLARE i551_cs5_1 CURSOR FOR
            SELECT SUM(sfq03) FROM sfp_file,sfq_file
             ...
             GROUP BY sfq04
             ORDER BY 1 DESC
           FOREACH i551_cs5_1 INTO qty2_1
               IF STATUS THEN LET qty2_1=0 END IF
               EXIT FOREACH
           END FOREACH

           #未按作業編號已退套數
           DECLARE i551_cs5_2 CURSOR FOR
            SELECT SUM(sfq03) FROM sfp_file,sfq_file
             WHERE sfp06='6'
               AND sfp01=sfq01
               AND sfq02=l_sfq.sfq02
             ...
             GROUP BY sfq04
             ORDER BY 1 DESC
           FOREACH i551_cs5_2 INTO qty2_2
  ...
  FUNCTION i551sub_s2(p_argv1,l_sfp)
...
#   DEFINE l_sql  STRING #MOD-C60234   #MOD-D10094 mark
    ...
    FOREACH i551_s2_c INTO l_sfs.*, l_sfb.*
        ...
        IF s_industry('icd') THEN      
           CALL i551sub_ind_icd_s2(l_sfp.*,l_sfs.*,l_sfb.*)
        END IF
        ...
        IF g_sma.sma115 = 'Y' THEN
           IF l_sfs.sfs32 != 0 OR l_sfs.sfs35 != 0 THEN
              CALL i551sub_update_du('s',p_argv1,l_sfp.*,l_sfs.*,l_sfe.*)    #No.MOD-790112 add l_sfe
              IF g_success='N' THEN
                 LET g_totsuccess='N'
                 LET g_success="Y"
                 CONTINUE FOREACH   #No.FUN-6C0083
        ...
        CALL i551sub_update(l_sfs.sfs07,l_sfs.sfs08,l_sfs.sfs09,
        ...
        #message '_u_sfa'
        CALL i551sub_u_sfa('s',p_argv1,l_sfp.*,l_sfs.*,l_sfb.*)
        IF g_success='N' THEN RETURN END IF
...
#MOD-B90008 --end--
          ...
          #MOD-C10171--begin--
           SELECT sfa11 INTO g_sfa11 FROM sfa_file
            WHERE sfa01=l_sfs.sfs03 AND sfa03=l_sfs.sfs04
              ...
              AND sfa013=l_sfs.sfs013   
          #MOD-C10171--end--  
        #  CALL i551sub_qty('s',l_sfb.*)               #FUN-C70037  mark       
        #  CALL i551sub_qty('s',l_sfp.sfp03,l_sfb.*)   #FUN-C70037  #FUN-C90077 mark
           CALL i551sub_qty('s',l_sfs.*,l_sfp.sfp03,l_sfp.sfp06,l_sfb.*)       #FUN-C90077
           IF g_success='N' THEN RETURN END IF  
        END IF
    END FOREACH
       ...
       DECLARE i551_s3_c CURSOR FOR
         SELECT sfe01,sfe07,sfe14,sfe012,sfe013,sfb_file.* FROM sfe_file, sfb_file    #FUN-A60028 add sfe012,sfe013 by vealxu
          WHERE sfe02=l_sfp.sfp01 AND sfe01=sfb01
            AND sfb93='Y'
       FOREACH i551_s3_c INTO l_sfe01,l_sfe07,l_sfe14, l_sfe012, l_sfe013, l_sfb.*          #FUN-A60028 add l_sfe012,l_sfe013 by vealxu
         ...
         #      CALL i551sub_upd_ecm301('s',l_sfe01,l_sfe07,l_sfe14,l_sfb.*,l_sfe012,l_sfe013)
         #           RETURNING l_flag
         #      IF NOT l_flag THEN
         #         LET g_success = 'N'
         #      END IF
         ...
         ##MOD-C60234 add end----------------------
         #MOD-D10094 mark end---------------------------------
             CALL i551sub_upd_ecm301('s',l_sfe01,l_sfe07,l_sfe14,l_sfb.*,l_sfe012,l_sfe013)    #FUN-A60028 add l_sfe012,l_sfe013 by vealxu 
             RETURNING l_flag
             IF NOT l_flag THEN
                LET g_success = 'N'
             END IF
  ...
  FUNCTION i551sub_update(p_ware,p_loca,p_lot,p_qty,p_uom,p_factor,p_argv1,l_sfp,l_sfs)
  ...
  #        l_imaqty LIKE ima_file.ima262,
    ...
    DEFINE l_sfs RECORD LIKE sfs_file.* #FUN-740187
      ...
      LET g_forupd_sql = "SELECT img10,img16,img23,img24,img09,img21 FROM img_file",
      ...
      IF STATUS THEN 
         ...
         #CHI-BA0003 -- begin --
         LET g_msg = 'lock img fail:',l_sfs.sfs02,'/',l_sfs.sfs04,'/',p_ware,'/',p_loca,'/',p_lot
     ...
     WHEN p_argv1='2' LET u_type=+1
      ...
      #---insert tlf_file
      CALL i551sub_tlf(p_ware,p_loca,p_lot,l_ima25,p_qty,l_qty,p_uom,p_factor,u_type,p_argv1,l_sfp.*,l_sfs.*) #FUN-740187
   
      #---insert sfe_file
      CALL i551sub_sfe(l_sfp.*,l_sfs.*)
  ...
  FUNCTION i551sub_tlf(p_ware,p_loca,p_lot,p_unit,p_qty,p_img10,p_uom,p_factor,
  ...
  FUNCTION i551sub_sfe(l_sfp,l_sfs)
...
#FUN-A70125 --end--
  ...
  FUNCTION i551sub_u_sfa(p_code,p_argv1,l_sfp,l_sfs,l_sfb)
    ...
    #message 'foreach tlf'
    FOREACH i551_tlf_c INTO l_tlf13,l_tlf10
      IF STATUS THEN
...
#MOD-D50031 add begin----------------------------------
    IF p_argv1 = '2' THEN
       ...
       PREPARE i551_tlf_cc FROM l_sql
       EXECUTE i551_tlf_cc INTO l_tlf10
...
#MOD-D50031 add end------------------------------------
    ...
    IF s_industry('icd') THEN
       IF g_sma.sma115='Y' THEN
    DECLARE i551_tlff_c CURSOR FOR
    ...
    FOREACH i551_tlff_c INTO l_tlff13,l_tlff10
      IF STATUS THEN
  ...
  CASE WHEN SQLCA.SQLCODE=0
      ...
      WHEN SQLCA.SQLCODE=100 AND l_sfs.sfs26 MATCHES '[SUBC]'  #TQC-C30044 ADD 'BC'
        CALL i551sub_u_o_sfa(p_argv1,l_sfs.*,l_sfb.*) # 將原sfa項次應發減少,替代碼 1/2 -> 3/4
        IF g_success='N' THEN RETURN END IF
...
#MOD-B80175 --end--
  ...
  FUNCTION i551sub_upd_ecm301(p_code,l_sfs03,l_sfs04,l_sfs10,l_sfb,l_sfs012,l_sfs013)   #FUN-A60028 add l_sfs012,l_sfs013 by vealxu
     DEFINE p_code        LIKE type_file.chr1
     DEFINE l_sfb RECORD  LIKE sfb_file.*
...
#    DEFINE l_ecm57_gfe03 LIKE gfe_file.gfe03    #FUN-A60028
     ...
     IF g_sma.sma541 = 'Y' AND cl_null(l_sfs012) THEN
        ...
        ELSE
           ...
           DECLARE i551_upd_ecm_c1 CURSOR FOR
              SELECT ecm012 FROM ecm_file
               WHERE ecm01=l_sfb.sfb01
                 AND ecm04=l_sfs10
               ORDER BY ecm012
           FOREACH i551_upd_ecm_c1 INTO l_sfa012
              LET l_sfs012=l_sfa012
              LET l_find = 'Y'  #MOD-D10094
              EXIT FOREACH
           END FOREACH
...
#FUN-A60028 --end--
  ...
  FUNCTION i551sub_u_o_sfa(p_argv1,l_sfs,l_sfb) # 將原sfa項次應發減少,替代碼 1/2 -> 3/4
       DEFINE o_sfa RECORD LIKE sfa_file.*
...
#    END IF
#   
#    PREPARE i551_short_pre FROM l_sql
#    DECLARE i551_short_c CURSOR FOR i551_short_pre
#   
#    FOREACH i551_short_c INTO l_sfa.*
...
#        GROUP BY sfq04
#        ORDER BY 1 DESC
#      FOREACH i551_cs6 INTO qty1
#        IF STATUS THEN LET qty1=0 END IF
#        EXIT FOREACH
#      END FOREACH
#   
#  #計算單身備料成套退料總數量
#      DECLARE i551_cs7 CURSOR FOR
...
#         CONTINUE FOREACH
#      END IF
#      IF l_sfa.sfa26 MATCHES '[34SU8Z]' THEN CALL i551sub_short2(l_sfa.*,l_issue_set) END IF  #FUN-A20037 add '8,Z'
#    END FOREACH
#    IF SQLCA.sqlcode THEN CALL cl_err('f sfa',STATUS,1)LET g_success='N' RETURN END IF
#  END FUNCTION
#
#  # 當有替代狀況時, 須作以下處理
#  FUNCTION i551sub_short2(l_sfa,l_issue_set)
...
#  ##
#   
#      DECLARE i551_short_c2 CURSOR FOR
...
#      FOREACH i551_short_c2 INTO l_sfa2.*     #應發(含替代)料件(l_sfa2
...
#FUN-A60095 mark(E)
  ...
  FUNCTION i551sub_s2_asr(p_argv1,l_sfp)
    DEFINE l_ima108 LIKE ima_file.ima108
    DEFINE l_n      LIKE type_file.num5    #No.FUN-680121 SMALLINT
    DEFINE p_argv1  LIKE type_file.chr1    #FUN-740187
    DEFINE l_sfp    RECORD LIKE sfp_file.* #FUN-740187
    ...
    DECLARE i551_s2_c1 CURSOR FOR
      SELECT * FROM sfs_file
       WHERE sfs01=l_sfp.sfp01
    FOREACH i551_s2_c1 INTO l_sfs.*
        IF STATUS THEN
     CALL cl_err('for sfs:',STATUS,1) LET g_success='N' EXIT FOREACH   
        END IF
        IF cl_null(l_sfs.sfs04) THEN CONTINUE FOREACH END IF
  ...
  #p_type : s-過帳 ; w-取銷過帳
  FUNCTION i551sub_update_du(p_type,p_argv1,l_sfp,l_sfs,l_sfe)    #No.MOD-790112 add l_sfe
  DEFINE l_ima25   LIKE ima_file.ima25,
     ...
     IF NOT cl_null(l_sfs.sfs33) THEN
        CALL i551sub_upd_imgg('1',l_sfs.sfs04,l_sfs.sfs07,l_sfs.sfs08,
               l_sfs.sfs09,l_sfs.sfs33,l_sfs.sfs34,l_sfs.sfs35,u_type,'2',l_sfp.*)
        IF g_success='N' THEN RETURN END IF
   
        IF NOT cl_null(l_sfs.sfs35) AND l_sfs.sfs35 <> 0 THEN
           CALL i551sub_tlff(l_sfs.sfs07,l_sfs.sfs08,l_sfs.sfs09,l_ima25,
     ...
     IF NOT cl_null(l_sfe.sfe33) THEN
        CALL i551sub_upd_imgg('1',l_sfe.sfe07,l_sfe.sfe08,l_sfe.sfe09,
     ...
     END IF
        END IF
        IF p_type = 'w' THEN
     CALL i551sub_tlff_w(l_sfp.*,l_sfe.*)    #No.MOD-790112 add l_sfp
     IF g_success='N' THEN RETURN END IF
        END IF
     END IF
     IF l_ima906 = '3' THEN  #參考單位
        IF p_type = 's' THEN
     IF NOT cl_null(l_sfs.sfs33) THEN
        CALL i551sub_upd_imgg('2',l_sfs.sfs04,l_sfs.sfs07,l_sfs.sfs08,
               l_sfs.sfs09,l_sfs.sfs33,l_sfs.sfs34,l_sfs.sfs35,u_type,'2',l_sfp.*)
        IF g_success = 'N' THEN RETURN END IF
     ...
     END IF
        ELSE
     IF NOT cl_null(l_sfe.sfe33) THEN
        CALL i551sub_upd_imgg('2',l_sfe.sfe07,l_sfe.sfe08,l_sfe.sfe09,
               l_sfe.sfe10,l_sfe.sfe33,l_sfe.sfe34,l_sfe.sfe35,u_type,'2',l_sfp.*)
        IF g_success = 'N' THEN RETURN END IF
     END IF
        END IF
  ...
  FUNCTION i551sub_tlff_w(l_sfp,l_sfe)    #No.MOD-790112 add l_sfp
     DEFINE l_sfe  RECORD LIKE sfe_file.*
  ...
  FUNCTION i551sub_upd_imgg(p_imgg00,p_imgg01,p_imgg02,p_imgg03,p_imgg04,
  ...
  FUNCTION i551sub_tlff(p_ware,p_loca,p_lot,p_unit,p_qty,p_img10,p_uom,p_factor,
  ...
  FUNCTION i551sub_refresh(p_sfp01)
    DEFINE p_sfp01 LIKE sfp_file.sfp01
    DEFINE l_sfp RECORD LIKE sfp_file.*
  ...
  FUNCTION i551sub_ind_icd_s2(l_sfp,l_sfs,l_sfb)
    DEFINE l_sfp RECORD LIKE sfp_file.*
    DEFINE l_sfs RECORD LIKE sfs_file.*
    DEFINE l_sfb RECORD LIKE sfb_file.*
    DEFINE l_sfbi RECORD LIKE sfbi_file.*
   ...
   #CASE WHEN l_sfp.sfp06 = '1' #CHI-A70060 mark
    CASE WHEN l_sfp.sfp06 MATCHES '[1234]' #CHI-A70060 
       ...
       CALL s_icdpost(-1,l_sfs.sfs04,l_sfs.sfs07,l_sfs.sfs08,
        ...
        #複製生產說明(ico_file)
        CALL i551sub_ind_icd_ins_ico(l_sfs.*)
        IF g_success = 'N' THEN RETURN END IF
  ...
  FUNCTION i551sub_ind_icd_ins_ico(l_sfs)
  ...
  FUNCTION i551sub_ind_icd_upd_sfb08(p_post,l_sfp)
...
FUNCTION i551sub_y_chk(p_sfp01,p_action_choice) #TQC-C60079 add
...
#MOD-C10100 --begin--
  IF NOT l_sfp.sfp06 MATCHES '[ABC]' THEN
     ...
     #sfq_file的工單一定要存在sfs_file裡
      LET l_sfq02=NULL
      DECLARE i551_chk_sfq02 CURSOR FOR
      SELECT sfq02 FROM sfq_file
       WHERE sfq02 NOT IN (SELECT sfs03 FROM sfs_file WHERE sfs01=l_sfp.sfp01)
         AND sfq01=l_sfp.sfp01
         AND sfq03 >= 0

      FOREACH i551_chk_sfq02 INTO l_sfq02
        IF STATUS THEN
           LET l_sfq02=NULL
        END IF
        EXIT FOREACH
...
#MOD-C10100 --end--
     ...
     IF l_sfp.sfp06='3' OR l_sfp.sfp06='1' THEN 
        DECLARE i551_y_chk_sfs05 CURSOR FOR 
         SELECT sfs03,sfs04,sfs27,sfs06,sfs10,sfs012,sfs013 FROM sfs_file  #TQC-B30039
                WHERE sfs01=p_sfp01
        FOREACH i551_y_chk_sfs05 INTO l_sfs03a,l_sfs04a,l_sfs27a,
     ...
     DECLARE i551_y_chk_c CURSOR FOR SELECT * FROM sfs_file
                                      WHERE sfs01=p_sfp01
     FOREACH i551_y_chk_c INTO l_sfs.*

       #FUN-C10035 add str -----
       #當與MES整合,檢查此張發料單的工單是否有混雜:與MES整合工單及不與MES整合的工單
       #與MES整合:工單型態(sfb02)= 1 or 13
        IF g_aza.aza90 MATCHES "[Yy]" THEN
           CALL i551_sub_chk_sfb01(l_sfs.sfs03,g_mescnt)
           IF g_mesflag <> g_mesflag_t THEN          #本張工單的整合類型與上張工單不符
              CALL cl_err('','asf-146',1)            #與MES整合時,發料單內的工單型態需一致
              LET g_success = "N"
              EXIT FOREACH
...
FUNCTION i551sub_y_upd(p_sfp01,p_action_choice,p_inTransaction)  
   DEFINE l_forupd_sql    STRING
   DEFINE p_sfp01         LIKE sfp_file.sfp01
  ...
  #CHI-C30106---mark---E
   ...
   LET g_forupd_sql = "SELECT * FROM sfp_file WHERE sfp01 = ? FOR UPDATE"
   LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
   DECLARE i551sub_upd_cl CURSOR FROM g_forupd_sql
 
   OPEN i551sub_upd_cl USING p_sfp01
   IF STATUS THEN
      CALL cl_err("OPEN i551sub_upd_cl:", STATUS, 1)
      CLOSE i551sub_upd_cl
      IF NOT p_inTransaction THEN #FUN-840012
         ROLLBACK WORK
      END IF
      RETURN l_sfp.*
   ...
   FETCH i551sub_upd_cl INTO l_sfp.*     # 鎖住將被更改或取消的資料
   IF SQLCA.sqlcode THEN
       CALL cl_err(l_sfp.sfp01,SQLCA.sqlcode,0)     # 資料被他人LOCK
       CLOSE i551sub_upd_cl 
       ROLLBACK WORK 
...
#FUNCTION i551sub_qty(p_cmd,p_sfb)            #FUN-C70037  mark
#FUNCTION i551sub_qty(p_cmd,p_sfp03,p_sfb)    #FUN-C70037  #FUN-C90077 mark
FUNCTION i551sub_qty(p_cmd,p_sfs,p_sfp03,p_sfp06,p_sfb)    #FUN-C90077
...
#跨越補單考慮取替代
      ...
      ELSE
         ...
         END IF
      END IF
      PREPARE i551sub_sfe16_pre FROM l_sql
      DECLARE i551sub_sfe16_cur CURSOR FOR i551sub_sfe16_pre 
     ...
     IF (l_year*12+l_month) <> (g_sma.sma51*12+g_sma.sma52) THEN
        IF p_cmd = 's' THEN
           ...
           IF cl_null(l_sfe16_new) THEN
              LET l_sfe16_new = 0
           END IF
           CALL i551sub_totsfs05(p_sfp01,p_sfp03,p_sfp06,p_sfs.sfs02,p_sfs.sfs03,p_sfs.sfs04,p_sfs.sfs06,p_sfs.sfs10,p_sfs.sfs27)
                RETURNING l_totsfs05
           LET l_min_qty = l_sfe16 - l_sfe16_1 - l_totsfs05 + l_sfe16_new
           LET l_qty_chk = l_min_qty
           FOREACH i551sub_sfe16_cur USING p_sfs.sfs27,p_sfs.sfs07 INTO l_sfa.*,l_sfe.*
...
FUNCTION i551sub_totsfs05(p_sfp01,p_sfp03,p_sfp06,p_sfs02,p_sfs03,p_sfs04,p_sfs06,p_sfs10,p_sfs27)
...
FUNCTION i551sub_w(p_sfp01,p_action_choice,p_call_transaction)
  DEFINE p_sfp01         LIKE sfp_file.sfp01
...
FUNCTION i551sub_z(p_argv1,p_sfp01,p_action_choice,p_call_transaction)
...
#TQC-C90083--add--
   LET l_sfq02 = ' '
   DECLARE i551_z_check CURSOR FOR
   SELECT sfq02 FROM sfq_file WHERE sfq01=l_sfp.sfp01
   FOREACH i551_z_check INTO l_sfq02
      IF STATUS THEN
...
#TQC-C90083--add--
  #日期由扣帳日期做判斷,不用輸入日期
   ...
   IF l_sfp.sfp06='1' THEN
      LET l_sfq02 = ''
      DECLARE i551_z_c1 CURSOR FOR
         SELECT sfq02 FROM sfq_file WHERE sfq01=l_sfp.sfp01
      FOREACH i551_z_c1 INTO l_sfq02
         IF STATUS THEN
            CALL cl_err('FOREACH',STATUS,0) 
            LET g_success='N'
            EXIT FOREACH 
...
#MOD-B30434 --------------------------End-----------------------------------
   ...
   #拆件式工单若有入库的时候,要卡一定有发料,至于发多少,不管控
   IF l_sfp.sfp06='1' THEN
      CALL i551sub_disassemble(l_sfp.sfp01,'1')
...
#MOD-C80077 add begin-----------------------------
   #判斷發料單單身資料
   DECLARE i551_z_c4 CURSOR FOR SELECT * FROM sfe_file
     WHERE sfe02 = l_sfp.sfp01
      LET g_success = "Y"

   CALL s_showmsg_init()

   FOREACH i551_z_c4 INTO l_sfe.*
      IF STATUS THEN
         EXIT FOREACH
      END IF
...
#MOD-C80077 add end-------------------------------
  ...
  ##NO.FUN-A10001   add--begin   
   ...
   IF l_cnt > 0 THEN          
     #CALL i551sub_VMI_z(p_sfp01)          #MOD-C20155 mark
      CALL i551sub_VMI_z(p_sfp01,p_argv1)  #MOD-C20155 add
      IF g_success = 'N' THEN
         IF p_call_transaction THEN
            ROLLBACK WORK
         END IF
  ...
  ##NO.FUN-A10001   add--end
   ...
   IF NOT l_sfp.sfp06 MATCHES '[ABC]' THEN #FUN-5C0114
      CALL i551sub_z2(p_argv1,l_sfp.*)     

      LET g_mescnt = 1    #FUN-C10035 add
 
      DECLARE i551_z_c CURSOR FOR
        SELECT DISTINCT sfq02,sfb04,sfb02 FROM sfq_file,sfb_file  #MOD-540074 add sfb02
        WHERE sfq01=l_sfp.sfp01
          AND sfq02=sfb01
      FOREACH i551_z_c INTO l_sfq02,l_sfb04,l_sfb02  #MOD-540074 add l_sfb02
         IF STATUS THEN CALL cl_err('foreach z',STATUS,0) EXIT FOREACH END IF        
         CALL i551sub_chk_sfb04(l_sfq02,l_sfb04)   #bugno:6050  
            ...
            IF g_sfb02 = '1' OR g_sfb02 = '5' OR g_sfb02 = '13' THEN   #FUN-C10035 add   #FUN-CC0122 add g_sfb02 = '5'
              ...
              #         (3)Key
               IF l_sfp.sfp06 MATCHES "[1234]" THEN    #發料
                  CALL i551_mes('asfi511',l_sfp.sfp01)
               END IF
              ...
              #IF l_sfp.sfp06 MATCHES "[5678]" AND g_aza.aza96 MATCHES "[Nn]" THEN    #退料 #CHI-A70060 mark
               IF l_sfp.sfp06 MATCHES "[6789]" AND g_aza.aza96 MATCHES "[Nn]" THEN    #退料 #CHI-A70060
                  CALL i551_mes('asfi526',l_sfp.sfp01)
               END IF
            END IF                                     #FUN-C10035 add
         END IF
      END IF
   ...
   ELSE
      CALL i551sub_z2_asr(p_argv1,l_sfp.*) #FUN-5C0114
   END IF
 
  #FUN-A20048 --begin
      IF l_sfp.sfp06 MATCHES '[13D]' THEN #FUN-AC0074   #FUN-C70014 add 'D'
         DECLARE  i551sub_sif_cs1 CURSOR FOR SELECT * FROM sfs_file 
           WHERE sfs01 = l_sfp.sfp01 
         FOREACH i551sub_sif_cs1 INTO g_sfs_sif.*
           IF STATUS THEN EXIT FOREACH END IF
...
#FUN-A20048 --end 
   ...
   IF s_industry('icd') THEN
      CALL i551sub_ind_icd_upd_sfb08('N',l_sfp.*)  #FUN-810038
   END IF
...
FUNCTION i551sub_z2(p_argv1,l_sfp)
  DEFINE p_argv1 LIKE type_file.chr1
...
# DEFINE l_sql  STRING #MOD-C60234   #MOD-D10094 mark
  ...
  DECLARE i551_z2_c CURSOR FOR
          SELECT * FROM sfe_file, sfb_file
           WHERE sfe02=l_sfp.sfp01 AND sfe01=sfb01
  FOREACH i551_z2_c INTO l_sfe.*, l_sfb.*
      IF STATUS THEN
      ...
      IF g_sma.sma115 = 'Y' THEN
         CALL i551sub_update_du('w',p_argv1,l_sfp.*,l_sfs.*,l_sfe.*)      #No.MOD-790112 add l_sfe
         IF g_success='N' THEN 
            LET g_totsuccess='N'
            LET g_success="Y"
            CONTINUE FOREACH   #No.FUN-6C0083
         END IF
      END IF
      CALL i551sub_z_update(p_argv1,l_sfe.*,1)                            #FUN-920175
 
      CALL i551sub_sfs(l_sfp.sfp06,l_sfe.*) RETURNING l_sfs.*
      ...
      IF g_success='N' THEN 
         ...
         #RETURN 
      END IF
      CALL i551sub_u_sfa('z',p_argv1,l_sfp.*,l_sfs.*,l_sfb.*) #FUN-740187
      IF g_success='N' THEN RETURN END IF
      ...
      IF l_sfb.sfb02 != '11' THEN           #No:MOD-9B0079 add
         ...
         SELECT sfa11 INTO g_sfa11 FROM sfa_file
            WHERE sfa01=l_sfs.sfs03 AND sfa03=l_sfs.sfs04
              ...
              AND sfa013=l_sfs.sfs013  
         #MOD-C10171 ---end
      #  CALL i551sub_qty('z',l_sfb.*)         #CHI-870030  add  #FUN-C70037 mark
      #  CALL i551sub_qty('z',l_sfp.sfp03,l_sfb.*)    #FUN-C70037   #FUN-C90077 mark
      #  CALL i551sub_qty('z',l_sfp.sfp01,l_sfp.sfp03,l_sfp.sfp06,l_sfb.*)    #FUN-C90077
         CALL i551sub_qty('z',l_sfs.*,l_sfp.sfp03,l_sfp.sfp06,l_sfb.*)        #FUN-C90077 
     ...
     DECLARE i551_z3_c CURSOR FOR
             SELECT sfs03,sfs04,sfs10,sfs012,sfs013,sfb_file.* FROM sfs_file, sfb_file   #FUN-A60028 add sfs012,sfs013 by vealxu 
              WHERE sfs01=l_sfp.sfp01 AND sfs03=sfb01  #No.FUN-9B0140 
                AND sfb93='Y'
     FOREACH i551_z3_c INTO l_sfs03,l_sfs04,l_sfs10,l_sfs012,l_sfs013,l_sfb.*            #FUN-A60028 add l_sfs012.l_sfs013 by vealxu 
       #MOD-D10094 mark begin----------------------------------
       ##MOD-C60234 add begin--------------------
       ...
       #      CALL i551sub_upd_ecm301('z',l_sfs03,l_sfs04,l_sfs10,l_sfb.*,l_sfs012,l_sfs013)
       #          RETURNING l_flag
       #      IF NOT l_flag THEN
       #         LET g_success = 'N'
       #      END IF
       ...
       ##MOD-C60234 add end----------------------
       #MOD-D10094 mark end-----------------------------------
           CALL i551sub_upd_ecm301('z',l_sfs03,l_sfs04,l_sfs10,l_sfb.*,l_sfs012,l_sfs013)   #FUN-A60028 add l_sfs012.l_sfs013 by vealxu 
                RETURNING l_flag
           IF NOT l_flag THEN
              LET g_success = 'N'
           END IF
...
FUNCTION i551sub_z_update(p_argv1,l_sfe,p_factor)
   DEFINE p_argv1 LIKE type_file.chr1
...
#         l_imaqty LIKE ima_file.ima262,
  ...
  ##NO.FUN-8C0131   add--begin   
    ...
    DECLARE i551_u_tlf_c CURSOR FROM l_sql
    LET l_i = 0 
    CALL la_tlf.clear()
    FOREACH i551_u_tlf_c INTO g_tlf.*
       LET l_i = l_i + 1
...
FUNCTION i551sub_sfs(l_sfp06,l_sfe)
  DEFINE l_sfp06 LIKE sfp_file.sfp06
  DEFINE l_sfe RECORD LIKE sfe_file.*
  DEFINE l_sfs RECORD LIKE sfs_file.*
...
FUNCTION i551sub_z2_asr(p_argv1,l_sfp)
  DEFINE p_argv1 LIKE type_file.chr1
  DEFINE l_sfp   RECORD LIKE sfp_file.*
  DEFINE l_sfe RECORD LIKE sfe_file.*
  DEFINE l_sfs RECORD LIKE sfs_file.*
  ...
  DECLARE i551_z2_c1 CURSOR FOR
          SELECT * FROM sfe_file WHERE sfe02=l_sfp.sfp01
  FOREACH i551_z2_c1 INTO l_sfe.*
     IF STATUS THEN
        CALL cl_err('for sfe:',STATUS,1) LET g_success='N' EXIT FOREACH  
     END IF
 
     IF g_sma.sma115 = 'Y' THEN
        CALL i551sub_update_du('w',p_argv1,l_sfp.*,l_sfs.*,l_sfe.*)      #No.MOD-790112 add b_sfe
        IF g_success='N' THEN 
           LET g_totsuccess='N'
           LET g_success="Y"
           CONTINUE FOREACH   #No.FUN-6C0083
     ...
     CALL i551sub_z_update(p_argv1,l_sfe.*,1)
     IF g_success='N' THEN 
        LET g_totsuccess='N'
        LET g_success="Y"
        CONTINUE FOREACH   #No.FUN-6C0083
...
FUNCTION i551sub_chk_sfb04(p_sfb01,p_sfb04)    #bugno:6050 add
...
#FUNCTION i551sub_x(p_sfp01,p_action_choice,p_call_transaction)           #CHI-D20010
FUNCTION i551sub_x(p_sfp01,p_action_choice,p_call_transaction,p_type)     #CHI-D20010 
   DEFINE p_sfp01 LIKE sfp_file.sfp01
   DEFINE p_action_choice STRING
   DEFINE p_call_transaction LIKE type_file.num5 #WHEN TRUE -> CALL BEGIN/ROLLBACK/COMMIT WORK
   DEFINE l_sfp   RECORD LIKE sfp_file.*
...
#FUN-9A0095 add --------------------
FUNCTION i551_mes(p_key1,p_key2)
...
#No.TQC-B80182  --Begin
FUNCTION i551sub_disassemble(p_sfp01,p_type)
   DEFINE p_sfp01      LIKE sfp_file.sfp01
   ...
   #拆件式工单若有入库的时候,要卡一定有发料,至于发多少,不管控
   DECLARE i551_11_sfq02_cs CURSOR FOR
    SELECT sfq02,SUM(sfq03),sfb081 FROM sfq_file,sfb_file
    WHERE sfq02 = sfb01
      AND sfq01 = p_sfp01
      AND sfb02 = '11'
    GROUP BY sfq02,sfb081
   FOREACH i551_11_sfq02_cs INTO l_sfq02_11,l_sfq03_11,l_sfb081_11
      LET l_cnt = 0
...
#FUN-C10035 add str ------
FUNCTION i551_sub_chk_sfb01(p_sfb01,p_cnt)
   DEFINE p_sfb01   LIKE sfb_file.sfb01
