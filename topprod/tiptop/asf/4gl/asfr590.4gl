...
# Modify.........: No:TQC-D30076 13/03/29 By wujie 避免抓到其餘非指定成品的sfa_file資料
...
#              " use1.ima_file.ima26,",  #TQC-940142  
   ...
   LET l_table = cl_prt_temptable('asfr590',g_sql) CLIPPED
...
#No.FUN-760046 --end --CR  
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN  # If background job sw is off
      CALL asfr590_tm(0,0)                 # Input print condition
   ELSE
      CALL asfr590()                       # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
END MAIN
 
FUNCTION asfr590_tm(p_row,p_col)
   ...
   OPEN WINDOW asfr590_w AT p_row,p_col
        WITH FORM "asf/42f/asfr590"
...
#    unalc       LIKE ima_file.ima26           #No.FUN-680121 DECIMAL(12,3)
   ...
   #FUN-940008---End
   DEFINE l_sql2      STRING  #No.TQC-D30076 
#No.CUN-760046 --begin --CR
#  CALL cl_outnam('asfr590') RETURNING l_name
...
#No.FUN-760046 --begin --CR
#  START REPORT asfr590_rep TO l_name
      FOR l_i = 1 TO g_bma.getLength()
          IF NOT cl_null(g_bma[l_i].bma01) THEN
#            OUTPUT TO REPORT asfr590_rep(g_bma[l_i].*)
             EXECUTE insert_prep USING
                     '0',g_bma[l_i].cnt,g_bma[l_i].bma01,g_bma[l_i].bma06,
                     g_bma[l_i].qty,g_bma[l_i].num,g_bma[l_i].ima02,'','',
                     '','','','','','','','',''
#add --end --07/06/28             
          END IF
      END FOR
#  FINISH REPORT asfr590_rep
#No.FUN-760046 --end --CR
 
#No.FUN-760046 -begin --CR
#  LET l_name2 ='asfr590x'
   LET l_count = 0
#  LET g_len=100 #TQC-5B0023
#  FOR g_i = 1 TO g_len  LET g_dash[g_i,g_i] = '=' END FOR ##TQC-5B0023
#  START REPORT asfr590_rep2 TO l_name2
      #--->彙總相同成品編號生產總數
      FOR l_i = 1 TO g_bma.getLength()
...
#           AND pmn20 > (pmn50-pmn55) AND pmn16 <= '2'        #No.FUN-9A0068 mark
         ...
         IF sr.unalc IS NULL OR sr.unalc = ' ' THEN 
            LET sr.unalc = 0
         END IF
...
#        IF tm.c MATCHES '[yY]' THEN
         ...
         END IF
         END IF
#        OUTPUT TO REPORT asfr590_rep2(sr.*)
         LET l_count = l_count + 1    #add by hellen No.FUN-760046
         EXECUTE insert_prep USING 
                 '1','','','','','',sr.ima02,l_count,sr.bmb03,sr.xx05,sr.use,
                 sr.minus,sr.qty4,sr.qty1,sr.ima25,sr.unalc,sr.qty2,sr.mark
      END FOREACH
   
      LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
      CALL cl_prt_cs3('asfr590','asfr590',l_sql,'')
#  FINISH REPORT asfr590_rep2
        ...
        ON ACTION CONTROLP
            CASE
                WHEN INFIELD(bma01)
                    ...
                    LET g_qryparam.form = 'q_bma10'         #No:MOD-9B0175 modif
