...
#                                         -> i551sub_short() SQL 中 MATCHES '[xxxx]' 未轉換為 IN ('x','x'..)
#                                            modify sasfi551.ora
...
# Modify.........: No:9390 04/03/31 Melody 在工單委外代買發料時選1.依料件主要倉庫/儲位發退料時,會將委外代買料件一併發料造成負庫存及負發料數!!請將i551_g_b1()的bug no:2551之下,行數1652的mark
...
# Modify.........: No.MOD-560218 05/07/14 By pengu  i551_s2函式回應錯誤訊息CALLcl_err(b_sfs.sfs07,'asf-724',0)
                                          #         將其設為強制開窗，避免使用者看不到訊息
...
# Modify.........: No.MOD-590028 05/09/07 By kim i551()供外部呼叫,當asfp510呼叫時,不執行 501_q()
...
#                                                  但若直接用滑鼠點選確定時卻沒有進行控管
# Modify.........: No.FUN-5B0037 05/12/27 By Sarah 單頭沒有需填入工單資料,應該不用再SHOW出i551-g-b-w之畫面
...
# Modify.........: NO:EXT-610018 06/03/24 By PENGU 1.call i551()時,所用的單據後面多加'S',迼成在CALL i551()時單據傳入時
                                  #                  執行到i551sub_s()時讀取不到sfp_file的資料
# Modify.........: NO:EXT-610018 06/03/28 By PENGU call i551()時,所用的單據後面多加'S',迼成在CALL i551()時單據傳入時執行到
                                  #                  i551sub_s()時讀取不到sfp_file的資料
...
# Modify.........: No.TQC-740331 07/04/26 By Ray 畫面asfi551d的"班別",“生產料號”欄位開窗，挑選后，值帶不入欄位
...
# Modify.........: No.MOD-790175 07/10/17 By Pengu FUNCTION i551_totsfs05中的SQL應該加入單位與作業編號條件
...
# Modify.........: No.FUN-810016 08/01/08 By hongmei 服飾版相關開發修改 sasfi551.4gl->sasfi551.src.4gl
...
# Modify.........: No.FUN-830132 08/03/27 By hellen 修改CALL i551_ind_icd_chk_icdqty()的方式
# Modify.........: No.FUN-840042 08/04/15 by TSD.zeak 自訂欄位功能修改 
# Modify.........: No.MOD-840123 08/04/19 By Pengu i551_cs中有join sfs_file 但已過帳就無sfs了
...
# Modify.........: No.MOD-890268 08/09/26 By claire i551_t() 欄位定義應放大含小數位
...
# Modify.........: No.CHI-870030 08/10/02 By claire 於確認段時加入若為"退料單"時, 更新i551sub_u_sfa資料後, 再判斷是否符合完工套數的控管
...
# Modify.........: No:MOD-A30065 10/03/11 By Sarah 新增時,不在i551_d_i()詢問是否重新產生單身
...
# Modify.........: No:FUN-AC0074 11/04/12 By lixh1 移除發料單自動產生單身的開窗選項"備置發料選項"及超領時asfi551a_1開窗
...
#                                                     4.FUNCTION i551_ins_rvbs的rvbs09:IF sfp06='6,7,8,9,B' rvbs09 = 1 ELSE rvbs09 = -1 END IF
...
# Modify.........: No:TQC-C20098 12/02/09 by destiny 修改i551_ind_icd_chk_icdqty 检查方式
...
# Modify.........: No.TQC-C60079 12/06/11 By fengrui 函數i551sub_y_chk添加參數
...
# Modify.........: No:MOD-D10040 13/01/08 By bart 修改asfi551a視窗問題
...
FUNCTION i551(p_argv1, p_argv2, p_argv3, p_argv4)
...
#MOD-B30148 --------------------End----------------------------
   ...
   #CHI-CB0063---add---E
    ...
    DECLARE i551_cl CURSOR FROM g_forupd_sql
    ...
    IF NOT cl_null(g_argv3) THEN
       CASE g_argv4
          ...
          WHEN "efconfirm"
             ...
             CALL i551_q()
             CALL i551sub_y_chk(g_sfp.sfp01,g_action_choice)           #CALL 原確認的 check 段 #TQC-C60079
             IF g_success = 'Y' THEN
                 CALL i551sub_y_upd(g_sfp.sfp01,g_action_choice,FALSE) #CALL 原確認的 update 段
                      RETURNING g_sfp.*
             END IF
  ...
  CASE WHEN g_wc2 = " 1=1" AND g_wc3 = " 1=1"
         ...
         WHEN g_wc2 <> " 1=1" AND g_wc3 <> " 1=1"
              LET g_sql = "SELECT UNIQUE sfp01 ",                              #09/10/21 xiaofeizhu Add
                          ...
                          #--
    END CASE
    PREPARE i551_prepare FROM g_sql
    DECLARE i551_cs                         #SCROLL CURSOR
        SCROLL CURSOR WITH HOLD FOR i551_prepare
    DECLARE i551_fill_cs CURSOR FOR i551_prepare    #FUN-CB0014 add
    DISPLAY "g_sql",g_sql
    CASE WHEN g_wc2 = " 1=1" AND g_wc3 = " 1=1"
        ...
        EXIT CASE
    END CASE
    PREPARE i551_precount FROM g_sql
    DECLARE i551_count CURSOR FOR i551_precount
END FUNCTION
         ...
         WHEN "confirm"
            IF cl_chk_act_auth() THEN
               ...
               IF g_success = "Y" THEN
                  ...
                  CALL i551_pic() #圖形顯示
                  #FUN-AB0001---add----str--
                  CALL i551sub_refresh(g_sfp.sfp01) RETURNING g_sfp.*
                  CALL i551_show()
                  #FUN-AB0001---add----end--
               END IF
            END IF
       #@WHEN "取消確認"
         WHEN "undo_confirm"
            IF cl_chk_act_auth() THEN
               ...
               ELSE    
                  #TQC-C50227 add  end------            
                 #CALL i551_w() #FUN-920175
                  CALL i551sub_w(g_sfp.sfp01,g_action_choice,TRUE)   #FUN-920175
                  CALL i551sub_refresh(g_sfp.sfp01) RETURNING g_sfp.*  #FUN-920175
                  CALL i551_show()  #FUN-920175
               END IF ##TQC-C50227
            END IF
         WHEN "stock_post"
            IF cl_chk_act_auth() THEN
              ...
              #若料號設定使用條碼否(ima930)='Y'、不為包號管理(ima931)<>'Y'且無條碼資料,則控管不可過帳
               ...
               ELSE
              #DEV-D30026 add end--------------
                  CALL i551sub_s(g_argv1,g_sfp.sfp01,FALSE,'Y') #FUN-740187
                  IF g_success='Y' THEN
                     CALL i551sub_refresh(g_sfp.sfp01) RETURNING g_sfp.*
                     CALL i551_show()
                    #FUN-9A0095 mark str---------------------------------------- 
         ...
         WHEN "undo_post"
            IF cl_chk_act_auth() THEN
               ...
               #TQC-C50227 add  end------             
                  ...
                  END IF                                                        
                  #FUN-B40082 --END--
                  CALL i551sub_z(g_argv1,g_sfp.sfp01,g_action_choice,TRUE)   #FUN-920175
                  CALL i551sub_refresh(g_sfp.sfp01) RETURNING g_sfp.*  #FUN-920175
                  CALL i551_show()  #FUN-920175 mark
               END IF #TQC-C50227   
         ...
         WHEN "void"
            IF cl_chk_act_auth() THEN
              #CALL i551sub_x(g_sfp.sfp01,g_action_choice,TRUE)   #FUN-920175       #CHI-D20010
               CALL i551sub_x(g_sfp.sfp01,g_action_choice,TRUE,1)                   #CHI-D20010
               CALL i551sub_refresh(g_sfp.sfp01) RETURNING g_sfp.*  #FUN-920175
               CALL i551_show()  #FUN-920175
            END IF
         #CHI-D20010---add--str
         WHEN "undo_void"
            IF cl_chk_act_auth() THEN
              #CALL i551sub_x(g_sfp.sfp01,g_action_choice,TRUE)   #FUN-920175       #CHI-D20010
               CALL i551sub_x(g_sfp.sfp01,g_action_choice,TRUE,2)                   #CHI-D20010
               CALL i551sub_refresh(g_sfp.sfp01) RETURNING g_sfp.*  #FUN-920175
               CALL i551_show()  #FUN-920175
            END IF
         #CHI-D20010--add--end
...
#End Mark TQC-AC0125
    ...
    WHILE TRUE
      ...
      #FUN-AC0074 -----------End--------------
        ...
        IF g_sfp.sfp06 MATCHES '[136ABD]' AND l_cnt1 = 0 THEN CALL i551_g_b() END IF   #FUN-5B0037 #FUN-5C0114 add 'AB'  #FUN-C70014 add 'D'
        CALL i551_b_fill(" 1=1")
   #FUN-A20048 --begin #其他發料時，按以下處理
        IF g_sfp.sfp06 MATCHES '[24]' AND l_cnt1 = 0 THEN 
           CALL i551_g_b_1()
        ELSE 
           CALL i551_b()  #輸入單身-sfs
        END IF 
   #FUN-A20048  --end 
        IF g_cnt>0 AND g_smy.smyprint='Y' THEN CALL i551_out() END IF
        EXIT WHILE
    END WHILE
END FUNCTION
...
FUNCTION i551_u()
    ...
    OPEN i551_cl USING g_sfp.sfp01                 #09/10/21 xiaofeizhu Add
    IF STATUS THEN
       CALL cl_err("OPEN i551_cl:", STATUS, 1)
       CLOSE i551_cl
       ROLLBACK WORK
       RETURN
    END IF
    FETCH i551_cl INTO g_sfp.*          # 鎖住將被更改或取消的資料
    IF SQLCA.sqlcode THEN
        CALL cl_err(g_sfp.sfp01,SQLCA.sqlcode,0)     # 資料被他人LOCK
        CLOSE i551_cl ROLLBACK WORK RETURN
    END IF
    CALL i551_show()
    WHILE TRUE
        ...
        IF g_sfp.sfp01 != g_sfp_t.sfp01 THEN CALL i551_chkkey() END IF
        EXIT WHILE
    END WHILE
 
    CLOSE i551_cl
    COMMIT WORK
    CALL i551_show()                          #顯示最新資料   #FUN-AB0001 add
    CALL cl_flow_notify(g_sfp.sfp01,'U')
 
END FUNCTION
 
FUNCTION i551_chkkey()
    ...
    IF STATUS THEN
       ...
       LET g_sfp.*=g_sfp_t.* CALL i551_show() ROLLBACK WORK RETURN
    END IF
...
#End Mark No:TQC-AC0125
        ...
        BEFORE INPUT
            ...
            CALL i551_set_entry(p_cmd)
            CALL i551_set_no_entry(p_cmd)
...
FUNCTION i551_q()
  DEFINE  l_sfp01     LIKE sfp_file.sfp01
   ...
   #MESSAGE " SEARCHING ! "                 #FUN-AB0001 mark
    ...
    OPEN i551_cs                            #從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
        CALL cl_err('',SQLCA.sqlcode,0)
        INITIALIZE g_sfp.* TO NULL
    ELSE
        LET g_row_count=0
        FOREACH i551_count INTO l_sfp01
            LET g_row_count=g_row_count +1
        END FOREACH
        DISPLAY g_row_count TO FORMONLY.cnt  #ATTRIBUTE(MAGENTA)
 
        CALL i551_fetch('F')                 # 讀出TEMP第一筆並顯示
    END IF
   #MESSAGE " SEARCHING ! "                  #FUN-AB0001 mark
    CALL cl_msg(" SEARCHING ! ")             #FUN-AB0001 add
END FUNCTION
 
FUNCTION i551_fetch(p_flag)
DEFINE
    p_flag          LIKE type_file.chr1,                 #處理方式  #No.FUN-680121 VARCHAR(1)
    l_abso          LIKE type_file.num10                 #絕對的筆數  #No.FUN-680121 INTEGER
 
    CASE p_flag
        WHEN 'N' FETCH NEXT     i551_cs INTO g_sfp.sfp01                 #09/10/21 xiaofeizhu Add 
        WHEN 'P' FETCH PREVIOUS i551_cs INTO g_sfp.sfp01                 #09/10/21 xiaofeizhu Add
        WHEN 'F' FETCH FIRST    i551_cs INTO g_sfp.sfp01                 #09/10/21 xiaofeizhu Add
        WHEN 'L' FETCH LAST     i551_cs INTO g_sfp.sfp01                 #09/10/21 xiaofeizhu Add
        WHEN '/'
            IF (NOT mi_no_ask) THEN
                ...
                END IF
            END IF
            FETCH ABSOLUTE g_jump i551_cs INTO g_sfp.sfp01               #09/10/21 xiaofeizhu Add
            LET mi_no_ask = FALSE
    END CASE
...
FUNCTION i551_show()
    ...
    CALL i551_sfp16('d')                      #FUN-AB0001 add
    ...
    END IF
    CALL i551_pic() #圖形顯示
    CALL i551_d_fill(g_wc2)
 
   #str MOD-A50110 mod
   #CALL i551_b_fill(g_wc3)
    IF g_sfp.sfp04='N' THEN
       CALL i551_b_fill(g_wc3)
    ELSE
       CALL i551_b_fill(g_wc5)
    END IF
   #end MOD-A50110 mod
...
# 當有耗材產生時,應將asft620之耗材單號清為NULL
        ...
        IF (g_sfp.sfp06 MATCHES '[1234567]') AND (g_sfp.sfp06 <> '8') THEN         #No.MOD-570241 add sfp06的判斷  #FUN-5C0114
           DECLARE i551_upd_sfb CURSOR FOR
             SELECT * FROM sfs_file WHERE sfs01 = g_sfp.sfp01
           FOREACH i551_upd_sfb INTO l_sfs.*
...
#  DEFINE l_per         LIKE sfq_file.sfq03   #MOD-B70193         #MOD-AB0047
  ...
  #DEFINE a_qty2        LIKE sfq_file.sfq03 #TQC-BC0130   #MOD-C80256 mark
   ...
   IF NOT cl_null(g_sfp.sfp08) THEN
      IF i=0 THEN
         DECLARE i551_s_sfq_c CURSOR FOR         
         ...
         CALL g_sfq.clear()
         LET i=1
        FOREACH i551_s_sfq_c INTO g_sfq[i].sfq02,
                                  g_sfq[i].sfq04,
                                  g_sfq[i].sfb05,
                                  g_sfq[i].ima02_a,
                                  g_sfq[i].ima021_a,
   ...
   INPUT ARRAY g_sfq WITHOUT DEFAULTS FROM s_sfq.*
      ...
      BEFORE INPUT
         CALL i551_set_entry_d('a')
         CALL i551_set_no_entry_d('a')
      ...
      AFTER FIELD sfq014
         ...
         IF g_sfp.sfp06 = 'D' THEN #FUN-C70014 add
            IF NOT i551_chk_sfq(i,'1') THEN NEXT FIELD sfq014 END IF  #FUN-C70014 add
         END IF #FUN-C70014 add
      #FUN-C70014 add end --------------
      AFTER FIELD sfq02
...
#FUN-AA0059 ---------------------end-------------------------------
         ...
         IF (NOT cl_null(g_sfq[i].sfq02)) AND (NOT g_sfp.sfp06 MATCHES '[ABC]') THEN #FUN-5C0114
            ...
            IF g_sfp.sfp06 = '1' THEN   #成套發料
               ...
               FOREACH i551_cs0 INTO l_sfq03_r
         ...
         END IF
         IF g_sfp.sfp06 = 'D' THEN #FUN-C70014 add
            IF NOT i551_chk_sfq(i,'1') THEN NEXT FIELD sfq02 END IF  #FUN-C70014 add
         END IF 
...
#FUN-B20095 ---------------------Begin-------------------------
      AFTER FIELD sfq012 
           ...
           #IF g_sfp.sfp06 MATCHES '[1]' THEN
            IF g_sfp.sfp06 MATCHES '[1D]' THEN   #FUN-C70014 add 'D'
               ...
               ELSE
                  LET qty1 = 0
                  LET qty2 = 0
                 #CALL i551_sfq03_chk1(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012) #只选过账的,用于预设值
                  CALL i551_sfq03_chk(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012,g_sfq[i].sfq014,'1')  #FUN-BC0060 mod   #FUN-C70014 add sfq014
                       RETURNING qty1,qty2
               ...
               IF cl_null(g_sfq[i].sfq04) THEN
                  LET g_sfq[i].sfq04 = ' '
               END IF
           #將撈取l_qty1,l_qty2的部分整理到函數 i551_sfq03_chk()處理 
              #CALL i551_sfq03_chk(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012)  RETURNING l_qty1,l_qty2  #只要有效存在於發料當中的,用于控管
               CALL i551_sfq03_chk(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012,g_sfq[i].sfq014,'2')  RETURNING l_qty1,l_qty2  #FUN-BC0060 mod #FUN-C70014 add sfq014
         ...
         END IF   
         IF g_sfp.sfp06 = 'D' THEN #FUN-C70014 add
            IF NOT i551_chk_sfq(i,'1') THEN NEXT FIELD sfq012 END IF  #FUN-C70014 add 
         END IF 
#FUN-B20095 ---------------------End---------------------------         
      AFTER FIELD sfq04
         ...
         IF cl_null(g_sfq[i].sfq04) THEN
            LET g_sfq[i].sfq04 = ' '
         ELSE                                      
            CALL i551_sfq04(g_sfq[i].sfq04) RETURNING g_errno
            IF NOT cl_null(g_errno) THEN
         ...
         IF g_sfp.sfp06 MATCHES '[1D]' THEN #FUN-C70014 add 'D'
            ...
            #將撈取qty1,qty2的部分整理到函數 i551_sfq03_chk1()處理
             # CALL i551_sfq03_chk1(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012)  #FUN-A70095  #只选过账的,用于预设值
               CALL i551_sfq03_chk(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012,g_sfq[i].sfq014,'1')  #FUN-A70095  #FUN-BC0060 mod  #FUN-C70014 add sfq014
                    RETURNING qty1,qty2        #FUN-A70095
         #此段功能由i551_sfq03_chk1()代替
         #FUN-A70095 ---------------Begin----------------------
         ...
         #FUN-A70095 ---------------End-----------------------
            ...
            IF cl_null(g_sfq[i].sfq04) THEN
               LET g_sfq[i].sfq04 = ' '
            END IF
           #將撈取l_qty1,l_qty2的部分整理到函數 i551_sfq03_chk()處理        #FUN-B20095
              #CALL i551_sfq03_chk(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012)  RETURNING l_qty1,l_qty2    #FUN-B20095 #只要有效存在於發料當中的,用于控管
               CALL i551_sfq03_chk(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012,g_sfq[i].sfq014,'2')  RETURNING l_qty1,l_qty2    #FUN-B20095  #FUN-BC0060 mod #FUN-C70014 add sfq014
#FUN-B20095 ---------------------------Begin-----------------------
...
#FUN-B20095 ---------------------------End--------------------------
         ...
         IF g_sfp.sfp06='6' THEN
           ...
           #CALL i551_sfq03_chk(g_sfq[i].sfq02,g_sfq[i].sfq04,g_sfq[i].sfq012,g_sfq[i].sfq014,'2') RETURNING a_qty1,a_qty2 #TQC-BC0130  #FUN-C70014 add sfq014  #MOD-C80256 mark
            SELECT sfb081,sfb09 INTO qty1,qty2 FROM sfb_file
               WHERE sfb01 = g_sfq[i].sfq02
            IF SQLCA.sqlcode THEN
               LET g_sfq[i].sfq03 = 0
        ...
        #MOD-AA0123---add---end---
         IF g_sfp.sfp06 = 'D' THEN #FUN-C70014 add
            IF NOT i551_chk_sfq(i,'1') THEN NEXT FIELD sfq04 END IF  #FUN-C70014 add
         END IF 
      ...
      AFTER FIELD sfq05
         ...
         END IF
         IF g_sfp.sfp06 = 'D' THEN #FUN-C70014 add
            IF NOT i551_chk_sfq(i,'1') THEN NEXT FIELD sfq05 END IF  #FUN-C70014 add
         END IF 
...
#MOD-BA0017 -- end --
           ...
           #IF g_sfp.sfp06 = '1' THEN    #成套發料
            IF g_sfp.sfp06 MATCHES '[1D]' THEN   #FUN-C70014 add 'D'  新增Run Card成套發料
              ...
              #  ORDER BY sfq03 DESC
              # 
              #FOREACH i551_cs13 INTO l_sfq03_r
              #   IF STATUS THEN LET l_sfq03_r=0 END IF
              #   EXIT FOREACH
              #END FOREACH
              # 
      ...
      AFTER INPUT
         ...
         IF g_sfp.sfp06 = 'D' THEN 
            ...
            FOR l_n = 1 TO g_sfq.getLength()
               IF NOT i551_chk_sfq(l_n,'2') THEN 
                  LET g_success = 'N' 
               END IF   
            END FOR 
            IF g_success = 'N' THEN 
...
#FUN-AA0059---------mod------------end-----------------
   ...
   FOR i = 1 TO g_sfq.getLength()
       ...
       CALL i551_b_i_move_back(i) #FUN-730075  
...
#FUN-BC0060 mark
##FUN-B20095 -------------------Begin---------------------
#FUNCTION i551_sfq03_chk(p_sfq02,p_sfq04,p_sfq012)
...
#   PREPARE i551_sfq03_pre FROM l_sql1
#   EXECUTE i551_sfq03_pre INTO l_qty1
#
#   LET l_sql2 = l_sql CLIPPED," AND sfp06 = '6'"
#   PREPARE i551_sfq03_pre1 FROM l_sql2
#   EXECUTE i551_sfq03_pre1 INTO l_qty2
#
#   IF l_qty1 IS NULL THEN LET l_qty1=0 END IF
#   IF l_qty2 IS NULL THEN LET l_qty2=0 END IF
#
...
#END FUNCTION
#
#FUNCTION i551_sfq03_chk1(p_sfq02,p_sfq04,p_sfq012)
#DEFINE p_sfq02       LIKE sfq_file.sfq02
#DEFINE p_sfq04       LIKE sfq_file.sfq04
#DEFINE p_sfq012      LIKE sfq_file.sfq012
#DEFINE l_qty1        LIKE sfq_file.sfq03
...
#
#   LET l_sql1 = l_sql CLIPPED,"  AND sfp06 = '1' "
#   PREPARE i551_sfq03_pre2 FROM l_sql1
#   EXECUTE i551_sfq03_pre2 INTO l_qty1
#
#   LET l_sql2 = l_sql CLIPPED,"  AND sfp06 = '6' "
#   PREPARE i551_sfq03_pre3 FROM l_sql2
#   EXECUTE i551_sfq03_pre3 INTO l_qty2
#
#   IF l_qty1 IS NULL THEN LET l_qty1=0 END IF
#   IF l_qty2 IS NULL THEN LET l_qty2=0 END IF
#   RETURN l_qty1,l_qty2
...
#FUN-BC0060 add
#FUNCTION i551_sfq03_chk(p_sfq02,p_sfq04,p_sfq012,p_flag)
FUNCTION i551_sfq03_chk(p_sfq02,p_sfq04,p_sfq012,p_sfq014,p_flag)   #FUN-C70014 add p_sfq014
DEFINE p_sfq02       LIKE sfq_file.sfq02
DEFINE p_sfq04       LIKE sfq_file.sfq04
DEFINE p_sfq012      LIKE sfq_file.sfq012
...
DEFINE qty2_3,qty2_4  LIKE sfq_file.sfq03   #已退按作業編號的套數和已發不按作業編號的套數
   ...
   IF NOT cl_null(l_sql_1) THEN
     #LET l_sql1_1 = l_sql_1 CLIPPED," AND sfp06 = '1'"
      ...
      PREPARE i551_sfq03_pre1_1 FROM l_sql1_1
      DECLARE i551_sfq03_curs1_1 CURSOR FOR i551_sfq03_pre1_1
      FOREACH i551_sfq03_curs1_1 INTO qty1_1
     ...
     #LET l_sql1_2 = l_sql_2 CLIPPED," AND sfp06 = '1'"
      ...
      IF cl_null(p_sfq04) AND cl_null(p_sfq012) THEN
         LET l_sql1_2 = l_sql1_2 CLIPPED,
                        ...
                        " ORDER BY 1 DESC "
      END IF
      PREPARE i551_sfq03_pre1_2 FROM l_sql1_2
      DECLARE i551_sfq03_curs1_2 CURSOR FOR i551_sfq03_pre1_2
      FOREACH i551_sfq03_curs1_2 INTO qty1_2
   ...
   IF NOT cl_null(l_sql_3) THEN
     #LET l_sql1_3 = l_sql_3 CLIPPED," AND sfp06 = '1'"
      ...
      PREPARE i551_sfq03_pre1_3 FROM l_sql1_3
      DECLARE i551_sfq03_curs1_3 CURSOR FOR i551_sfq03_pre1_3
      FOREACH i551_sfq03_curs1_3 INTO qty1_3
   ...
   IF NOT cl_null(l_sql_4) THEN
     #LET l_sql1_4 = l_sql_4 CLIPPED," AND sfp06 = '1'"
      ...
      PREPARE i551_sfq03_pre1_4 FROM l_sql1_4
      DECLARE i551_sfq03_curs1_4 CURSOR FOR i551_sfq03_pre1_4
      FOREACH i551_sfq03_curs1_4 INTO qty1_4
   ...
   IF NOT cl_null(l_sql_1) THEN
      ...
      PREPARE i551_sfq03_pre2_1 FROM l_sql2_1
      DECLARE i551_sfq03_curs2_1 CURSOR FOR i551_sfq03_pre2_1
      FOREACH i551_sfq03_curs2_1 INTO qty2_1
   ...
   IF NOT cl_null(l_sql_2) THEN
      ...
      IF cl_null(p_sfq04) AND cl_null(p_sfq012) THEN
         LET l_sql2_2 = l_sql2_2 CLIPPED,
                        ...
                        " ORDER BY 1 DESC "
      END IF
      PREPARE i551_sfq03_pre2_2 FROM l_sql2_2
      DECLARE i551_sfq03_curs2_2 CURSOR FOR i551_sfq03_pre2_2
      FOREACH i551_sfq03_curs2_2 INTO qty2_2
   ...
   IF NOT cl_null(l_sql_3) THEN
      ...
      PREPARE i551_sfq03_pre2_3 FROM l_sql2_3
      DECLARE i551_sfq03_curs2_3 CURSOR FOR i551_sfq03_pre2_3
      FOREACH i551_sfq03_curs2_3 INTO qty2_3
   ...
   IF NOT cl_null(l_sql_4) THEN
      ...
      PREPARE i551_sfq03_pre2_4 FROM l_sql2_4
      DECLARE i551_sfq03_curs2_4 CURSOR FOR i551_sfq03_pre2_4
      FOREACH i551_sfq03_curs2_4 INTO qty2_4
...
FUNCTION i551_set_entry_d(p_cmd)
  DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-680121 VARCHAR(1)
...
FUNCTION i551_set_no_entry_d(p_cmd)
  DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-680121 VARCHAR(1)
...
#FUN-AC0074 -----------------Begin------------------------  
# OPEN WINDOW i551_g_b_j WITH FORM "asf/42f/asfi551a_1"
#         ATTRIBUTE (STYLE = g_win_style CLIPPED)
# CALL cl_ui_locale("asfi551a_1") 
...
#                  CALL  i551_g_b01(g_sib_1[i].sib02,'')
#               END IF 
#            END FOR 
#            EXIT INPUT
#  
...
#  IF INT_FLAG THEN
#     LET INT_FLAG=0
#     CLOSE WINDOW i551_g_b_q
#     CALL i551_b_fill(' 1=1')
#     RETURN
#  END IF    
#
#  CALL i551_sort_by_partno()
#  LET g_rec_b = g_i
# 
#  CLOSE WINDOW i551_g_b_q
...
FUNCTION i551_g_b()
 ...
 #DEFINE l_qty           LIKE sfq_file.sfq03    #NO.FUN-A20048  #FUN-AC0074 mark
  ...
  IF g_sfp.sfp06 MATCHES '[ABC]' THEN  #FUN-5C0114
     OPEN WINDOW i551_g_b_w WITH FORM "asr/42f/asri210a"
          ATTRIBUTE (STYLE = g_win_style CLIPPED)
     CALL cl_ui_locale("asri210a")
  ELSE
     OPEN WINDOW i551_g_b_w WITH FORM "asf/42f/asfi551a"
          ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
     CALL cl_ui_locale("asfi551a")
  END IF
  ...
  INPUT BY NAME b_part,e_part,part_type,issue_type,noqty,
     ...
     BEFORE FIELD issue_type
        ...
        IF part_type = 'Y' THEN
           ...
           DISPLAY BY NAME issue_type
        END IF
        CALL i551_set_no_entry_gb()  #MOD-D10040
  ...
  IF NOT (g_sfp.sfp06 MATCHES '[ABC]') THEN  #FUN-5C0114
      DECLARE i551_g_b_c CURSOR FOR
         SELECT * FROM sfq_file WHERE sfq01=g_sfp.sfp01
      FOREACH i551_g_b_c INTO b_sfq.*
       ...
       #      AND sfq01=sfp01 AND sfp06='6' AND sfp04='Y'
       #END IF
        CALL i551_sfq03_chk(b_sfq.sfq02,b_sfq.sfq04,'',b_sfq.sfq014,'1')  #FUN-A70095  #FUN-BC0060 mod 只选过账的 #FUN-C70014 add sfq014
                    RETURNING qty1,qty2
       #Mod FUN-BC0060--end
...
#FUN-A20048 --begin 
 ...
 #FUN-A20048 --end   	
  ...
  ELSE
      ...
      DECLARE i551_g_b_c1 CURSOR FOR
      ...
      FOREACH i551_g_b_c1 INTO b_sfq.sfq01,b_sfq.sfq02,b_sfq.sfq03
        ...
        CASE g_sfp.sfp06
           WHEN 'A'
              ...
              PREPARE i551_g_b_c2_pre FROM l_sql
              DECLARE i551_g_b_c2 CURSOR FOR i551_g_b_c2_pre
 
              #No.FUN-A70034  --Begin
              #FOREACH i551_g_b_c2 INTO l_bmb.*,g_ima108
              FOREACH i551_g_b_c2 INTO l_bmb.*,g_ima108,l_bmb081,l_bmb082
              #No.FUN-A70034  --End
                ...
                #FUN-AC0074 (E)
                CALL i551_chk_img(l_mai_ware,l_mai_loc,l_wip_ware,l_wip_loc,lot_no,FALSE)		# 依 issue_qty1 尋找 img_file可用資料           #FUN-AC0074 #FUN-B80086  main改成mai
           ...
           WHEN 'B'
              ...
              PREPARE i551_g_b_c3_pre FROM l_sql
              DECLARE i551_g_b_c3 CURSOR FOR i551_g_b_c3_pre
 
              FOREACH i551_g_b_c3 INTO l_bmb.*,g_ima108
                IF part_type = 'Y' AND (g_ima108= 'N' OR cl_null(g_ima108)) THEN
                   CONTINUE FOREACH
                END IF
                IF part_type = 'N' AND (g_ima108 = 'Y' OR cl_null(g_ima108)) THEN
...
#FUN-A60028 --end--                
                ...
                IF STATUS THEN 
                   ...
                   END IF
                CALL i551_ins_rvbs(b_sfs.sfs02,b_sfs.sfs03,b_sfs.sfs04)   #No.FUN-870106
...
#FUN-AC0074 mark(S)
#FUN-A20048 --begin
#FUNCTION i551_g_b01(l_sfq02,l_sfq04) 		# 依备置發料(When sfp06=1/6)
...
# 
#  PREPARE i551_g_b01_pre FROM l_sql
#  DECLARE i551_g_b01_c CURSOR FOR i551_g_b01_pre
# 
#  FOREACH i551_g_b01_c INTO g_sfa2.*,l_sie.*,g_ima108   #FUN-AC0074 
#                                             
#    IF part_type = 'Y' AND (g_ima108= 'N' OR cl_null(g_ima108)) THEN
#       CONTINUE FOREACH
#    END IF
...
# 
#          CALL i551_chk_img()	# 依 issue_qty1 尋找 img_file可用資料
...
#          CALL i551_set_du_by_origin()
#       END IF
#       LET b_sfs.sfs930=i551_get_sfs930(b_sfs.sfs03) #FUN-670103
#       IF cl_null(b_sfs.sfs27) THEN
#          LET b_sfs.sfs27=b_sfs.sfs04
#       END IF
#       IF cl_null(b_sfs.sfs27) THEN
...
FUNCTION i551_g_b0() 		# 全數發料 (除了消耗料件&代買料件)
...
#FUN-B20095 -----------------End----------------------
  ...
  PREPARE i551_g_b0_pre FROM l_sql
  DECLARE i551_g_b0_c CURSOR FOR i551_g_b0_pre
 
  FOREACH i551_g_b0_c INTO g_sfa2.*,g_ima108
    IF part_type = 'Y' AND (g_ima108= 'N' OR cl_null(g_ima108)) THEN
       CONTINUE FOREACH
    END IF
...
FUNCTION i551_g_b1() 		# 依套數發料/退料(When sfp06=1/6)
  DEFINE l_sql		LIKE type_file.chr1000 #No.FUN-680121 VARCHAR(1000)
...
#FUN-B20095 ---------------------END----------------------------
  ...
  PREPARE i551_g_b1_pre FROM l_sql
  DECLARE i551_g_b1_c CURSOR FOR i551_g_b1_pre
 
  FOREACH i551_g_b1_c INTO g_sfa.*,g_ima108	#原始料件(g_sfa)
    IF part_type = 'Y' AND (g_ima108= 'N' OR cl_null(g_ima108)) THEN
       CONTINUE FOREACH
    END IF
...
#TQC-C30067  ---begin---add
         IF g_sfa2.sfa26 MATCHES '[BC]' THEN
            ...
            CALL i500_chk_ima64(g_sfa2.sfa03, issue_qty1) RETURNING issue_qty1
            CALL i551_chk_img(l_mai_ware,l_mai_loc,l_wip_ware,l_wip_loc,lot_no,FALSE) 
         ELSE
#QC-C30067  ---END---         
...
FUNCTION i551_issue_sie()
  ...
  DEFINE l_sql,l_where         STRING
    ...
    END IF
    PREPARE i551_g_b0_pre1 FROM l_sql
    DECLARE i551_g_b0_c1 CURSOR FOR i551_g_b0_pre1
    FOREACH i551_g_b0_c1 INTO l_sie.*
      IF issue_qty1 <=0 THEN EXIT FOREACH END IF
       ...
       END IF 
       CALL i500_chk_ima64(g_sfa2.sfa03, issue_qty1) RETURNING issue_qty1
       CALL i551_chk_img(l_sie.sie02,l_sie.sie03,l_sie.sie02,l_sie.sie03,l_sie.sie04,TRUE)		# 依 issue_qty1 尋找 img_file可用資料           #FUN-AC0074 
...
FUNCTION i551_chk_img(l_mai_ware,l_mai_loc,l_wip_ware,l_wip_loc,l_lot_no,l_sie_flag)	# 依 issue_qty1 尋找 img_file可用資料  #FUN-AC0074     #FUN-B80086  main改成mai
    DEFINE l_sql		LIKE type_file.chr1000 #No.FUN-680121 VARCHAR(600)
...
FUNCTION i551_ins_sfs()	# 依 issue_qty2 Insert sfs_file
...
#FUN-B20095 -----------------End----------------------
  ...
  PREPARE i551_g_b3_pre FROM l_sql
  DECLARE i551_g_b3_c CURSOR FOR i551_g_b3_pre
 
  FOREACH i551_g_b3_c INTO g_sfa2.*,g_ima108
       IF part_type = 'Y' AND (g_ima108= 'N' OR cl_null(g_ima108)) THEN
          CONTINUE FOREACH
       END IF
       IF part_type = 'N' AND (g_ima108 = 'Y' OR cl_null(g_ima108)) THEN
...
#FUN-B20095 -----------------End----------------------
  ...
  PREPARE i551_g_b7_pre FROM l_sql
  DECLARE i551_g_b7_c CURSOR FOR i551_g_b7_pre
 
  FOREACH i551_g_b7_c INTO g_sfa2.*,g_ima108
    IF part_type = 'Y' AND (g_ima108= 'N' OR cl_null(g_ima108)) THEN
       CONTINUE FOREACH
    END IF
...
#FUN-B20095 -----------------End----------------------
  ...
  PREPARE i551_g_b5_pre FROM l_sql
  DECLARE i551_g_b5_c CURSOR FOR i551_g_b5_pre
 
  FOREACH i551_g_b5_c INTO g_sfa2.*,g_ima108
    IF part_type = 'Y' AND (g_ima108= 'N' OR cl_null(g_ima108)) THEN
       CONTINUE FOREACH
    END IF
    IF part_type = 'N' AND (g_ima108 = 'Y' OR cl_null(g_ima108)) THEN
...
FUNCTION i551_sort_by_partno() 		#
  ...
  DECLARE i551_sort_c CURSOR FOR
    SELECT * FROM sfs_file
     WHERE sfs01=g_sfp.sfp01
     ORDER BY sfs04,sfs03
 
  LET g_i=0
  FOREACH i551_sort_c INTO l_sfs.*
     LET g_i=g_i+1
...
#NO.TQC-B90236 ------add----end
    ...
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    DECLARE i551_bcl CURSOR FROM g_forupd_sql
        ...
        BEFORE INPUT
           ...
           IF g_wm = 'N'THEN    #TQC-AC0197
              IF g_rec_b!=0 THEN
                 ...
                 CALL i551_set_entry_b('u')
                 CALL i551_set_no_entry_b('u')
              ELSE
                 CALL i551_set_entry_b('a')
                 CALL i551_set_no_entry_b('a')
              END IF              
#TQC-AC0197 ---------------------------Begin--------------------------
           ELSE
...
#FUN-A60028 --end--            
	    ...
	    BEGIN WORK
         ...
         #FUN-BB0084 -----------End--------------
               ...
               DISPLAY BY NAME g_sfp.sfpdate         #NO:6908
#FUN-BA0050 -------------------Begin-----------------
               CALL i551_set_no_required_sfs09()
               CALL i551_set_required_sfs09()
               CALL i551_set_entry_sfs09()
               CALL i551_set_no_entry_sfs09()
#FUN-BA0050 -------------------End-------------------
#No.TQC-B90236-------add-----------begin
               IF g_wm = 'N' THEN
...
#No.TQC-B90236-------add-----------end
               ...
               OPEN i551_bcl USING g_sfp.sfp01,g_sfs_t.sfs02
               IF STATUS THEN
                  CALL cl_err("OPEN i551_bcl:", STATUS, 1)
                  LET l_lock_sw = "Y"
               ELSE
                  FETCH i551_bcl INTO b_sfs.*
                  IF SQLCA.sqlcode THEN
                     CALL cl_err('lock sfs',SQLCA.sqlcode,1)
                     LET l_lock_sw = "Y"
                  ELSE
                     CALL i551_b_move_to()
                     CALL i551_azf03_desc()  #TQC-D20042 add
                     IF g_wm != 'Y' THEN         #TQC-AC0197
                        CALL i551_set_entry_b(p_cmd)
                        CALL i551_set_no_entry_b(p_cmd)
                     END IF                      #TQC-AC0197
               ...
               IF g_sma.sma115 = 'Y' THEN
                  ...
                  IF g_wm != 'Y' THEN            #TQC-AC0197 
                     CALL i551_set_entry_b('u')
                     CALL i551_set_no_entry_b('u')
                     CALL i551_set_no_required()
                     CALL i551_set_required()    
        ...
        AFTER INSERT
            ...
            IF g_sma.sma115 = 'Y' THEN
               ...
               CALL i551_set_origin_field()
               IF NOT (g_sfp.sfp06 MATCHES '[ABC]') THEN #FUN-5C0114
                  #計算sfs05的值,檢查入庫數量的合理性
                  CALL i551_check_inventory_qty()
                      RETURNING g_flag
                  IF g_flag = '1' THEN
        ...
        AFTER FIELD sfs26
           IF NOT cl_null(g_sfs[l_ac].sfs26) THEN
             #IF g_sfs[l_ac].sfs26 NOT MATCHES '[SU]' THEN   #No.TQC-5C0135 mark
              ...
              END IF
           END IF
           CALL i551_chk_entry_sfs27()  #TQC-C70050 add
...
#-----CHI-BC0040 end add--------
        ...
        AFTER FIELD sfs03
           CALL i551_set_required_sfs09()    #FUN-BA0050
           CALL i551_set_no_entry_sfs09()    #FUN-BA0050
#FUN-AA0059 ---------------------start----------------------------
...
#FUN-AA0059 ---------------------end-------------------------------
           ...
           IF (NOT cl_null(g_sfs[l_ac].sfs03)) AND (NOT (g_sfp.sfp06 MATCHES '[ABC]')) THEN #FUN-5C0114
              CALL i551_sfb01(g_sfs[l_ac].sfs03)
              IF NOT cl_null(g_errno) THEN
           ...
           IF NOT cl_null(g_sfs[l_ac].sfs27) AND 
             ...
             #FUN-B80143 --END mark--
              ...
              END IF              
              PREPARE i551_b_p1 FROM g_sql
              DECLARE i551_b_c1 CURSOR FOR i551_b_p1
              OPEN i551_b_c1
              FETCH i551_b_c1 INTO l_n              
              IF SQLCA.SQLCODE THEN
                 CALL cl_err("", SQLCA.SQLCODE, 0)
                 CLOSE i551_b_c1
                 NEXT FIELD sfs03 
...
#FUN-AA0059 ---------------------end-------------------------------
              ...
              IF g_sfs[l_ac].sfs26 MATCHES '[SUZ]' AND             #FUN-A40058 add 'Z'
                 g_sfp.sfp06 MATCHES '[12346789]' THEN             #MOD-B20062 add 6789
                  IF g_sfs[l_ac].sfs04=g_sfs27 THEN        #MOD-9A0123
                    ...
                    NEXT FIELD sfs04
                 END IF
                #GP5.15發料改善修改:b_sfs.sfs27 原由i551_sfs27()開出的
              ...
              #    INTO l_n, l_b2, l_b3                 #FUN-B20079  mark
                 SELECT COUNT(*),MIN(sfa12),MIN(sfa08),MIN(sfa012),MIN(sfa013)  #FUN-B20079
                   ...
                   FROM sfa_file
                  WHERE sfa01=g_sfs[l_ac].sfs03
                    AND (sfa03=g_sfs[l_ac].sfs04 OR sfa03=b_sfs.sfs27)
                 IF l_n=0 THEN
           ...
           IF NOT cl_null(g_sfs[l_ac].sfs27) AND 
             ...
             #FUN-B80143 --END mark--
              ...
              IF g_sfp.sfp06 = '7' THEN
                 LET g_sql = g_sql, " AND sfa062 > 0 "
              END IF              
              PREPARE i551_b_p2 FROM g_sql
              DECLARE i551_b_c2 CURSOR FOR i551_b_p2
              OPEN i551_b_c2
              FETCH i551_b_c2 INTO l_n              
              IF SQLCA.SQLCODE THEN
                 CALL cl_err("", SQLCA.SQLCODE, 0)
                 CLOSE i551_b_c2
                 NEXT FIELD sfs04 
           ...
           IF NOT cl_null(g_sfs[l_ac].sfs27) THEN
             ...
             #FUN-B80143 --START--
              ...
              IF g_sfp.sfp06 = '7' THEN
                 LET g_sql = g_sql, " AND sfa062 > 0 "
              END IF              
              PREPARE i551_b_p3 FROM g_sql
              DECLARE i551_b_c3 CURSOR FOR i551_b_p3
              OPEN i551_b_c3
              FETCH i551_b_c3 INTO l_n              
              IF SQLCA.SQLCODE THEN
                 CALL cl_err("", SQLCA.SQLCODE, 0)
                 CLOSE i551_b_c3
                 NEXT FIELD sfs27
        ...
        AFTER FIELD sfs10
           IF cl_null(g_sfs[l_ac].sfs10) THEN 
              LET g_sfs[l_ac].sfs10=' ' 
           ELSE 
              CALL i551_sfs10(g_sfs[l_ac].sfs10) RETURNING g_errno
              IF NOT cl_null(g_errno) THEN
           ...
           IF g_sfp.sfp06 MATCHES '[6789]' THEN    #NO:7075
             ...
             #計算欠料量g_short_qty(原g_sfa07)
              ...
              CALL s_shortqty(g_sfs[l_ac].sfs03,g_sfs[l_ac].sfs04,g_sfs[l_ac].sfs10,
                   ...
                   RETURNING g_short_qty
              IF cl_null(g_short_qty) THEN LET g_short_qty = 0 END IF 
              CALL i551_ins_rvbs(g_sfs[l_ac].sfs02,g_sfs[l_ac].sfs03,g_sfs[l_ac].sfs04)   #No.FUN-870106
           END IF
...
#FUN-BA0050 ----------------Begin----------------
           IF NOT cl_null(g_sfs[l_ac].sfs04) THEN
              SELECT ima159 INTO l_ima159 FROM ima_file
               WHERE ima01 = g_sfs[l_ac].sfs04              
              IF l_ima159 = '2' THEN
                 CASE i551_b_sfs09_inschk(p_cmd)
...
#               CALL i551_du_default(p_cmd)
#            END IF
# &ifdef ICD
#            CALL i551_def_sfsiicd029() #FUN-B70061
# &endif
#FUN-BA0050 ------------------End-------------------
...
{
                   IF (g_sfs[l_ac].sfs05/l_sfa161) > l_sfq03 THEN 
             ...
             IF g_sfp.sfp06 MATCHES '[1234]' THEN
               ...
               #IF (g_sfs[l_ac].sfs05 * l_factor) > g_sfs[l_ac].img10 THEN     #MOD-C50190 mark
                IF (g_sfs[l_ac].sfs05 * l_factor) > g_sfs[l_ac].img10 AND NOT i551_isVMI(g_sfs[l_ac].sfs07,g_sfs[l_ac].sfs08) THEN #MOD-C50190 add i551_isVMI 
             ...
             IF g_sfp.sfp06 MATCHES '[6789]' THEN   #NO:7075                                                                                                
                ...
                IF (g_sfp.sfp06 = '7') AND (l_sfa11 <> 'S') THEN    #超領退料  #FUN-9C0040                                                                                              
                   ...
                   IF g_sfs[l_ac].sfa06 = 0 THEN
                      ...
                      NEXT FIELD sfs05 
                   ELSE
                      CALL i551_totsfs05(g_sfp.sfp01,g_sfp.sfp06,g_sfs[l_ac].sfs02,
             ...
             #MOD-BA0137 -- mark end --
              ...
              IF NOT cl_null(g_sfs[l_ac].sfs33) THEN
                 IF g_sfp.sfp06 MATCHES '[1234]' THEN
                    ...
                    IF g_sfs[l_ac].sfs35 > g_imgg10_2 AND NOT i551_isVMI(g_sfs[l_ac].sfs07,g_sfs[l_ac].sfs08) THEN #MOD-C50190 add i551_isVMI
                      #IF g_sma.sma894[3,3]='N' OR g_sma.sma894[3,3] IS NULL THEN                                #FUN-C80107 mark
                      #FUN-D30024--modify--str--
                      #INITIALIZE g_sma894 TO NULL                                                               #FUN-C80107
                      #CALL s_inv_shrt_by_warehouse(g_sma.sma894[3,3],g_sfs[l_ac].sfs07) RETURNING g_sma894      #FUN-C80107
              ...
              IF NOT cl_null(g_sfs[l_ac].sfs30) THEN
                 IF g_sfp.sfp06 MATCHES '[1234]' THEN
                    IF g_ima906 = '2' THEN
                       ...
                       IF g_sfs[l_ac].sfs32 > g_imgg10_1 AND NOT i551_isVMI(g_sfs[l_ac].sfs07,g_sfs[l_ac].sfs08) THEN #MOD-C50190 add i551_isVMI 
                         #IF g_sma.sma894[3,3]='N' OR g_sma.sma894[3,3] IS NULL THEN                                #FUN-C80107 mark
                         #FUN-D30024--modify--str--
                         #INITIALIZE g_sma894 TO NULL                                                               #FUN-C80107
                         #CALL s_inv_shrt_by_warehouse(g_sma.sma894[3,3],g_sfs[l_ac].sfs07) RETURNING g_sma894      #FUN-C80107
                    ...
                    ELSE
                      ...
                      #IF l_factor * g_sfs[l_ac].sfs32 > g_imgg10_1 THEN           #MOD-A50001 #MOD-C50190 mark
                       IF l_factor * g_sfs[l_ac].sfs32 > g_imgg10_1 AND NOT i551_isVMI(g_sfs[l_ac].sfs07,g_sfs[l_ac].sfs08) THEN #MOD-C50190 add i551_isVMI
                         #IF g_sma.sma894[3,3]='N' OR g_sma.sma894[3,3] IS NULL THEN                                #FUN-C80107 mark
                         #FUN-D30024--modify--str--
                         #INITIALIZE g_sma894 TO NULL                                                               #FUN-C80107
                         #CALL s_inv_shrt_by_warehouse(g_sma.sma894[3,3],g_sfs[l_ac].sfs07) RETURNING g_sma894      #FUN-C80107
            ...
            ELSE
             LET l_cnt=0
              ...
              IF g_sma.sma115 = 'Y' THEN
                 ...
                 END IF
              END IF
              IF NOT i551_sfs37_check() THEN NEXT FIELD sfs37 END IF  #FUN-CB0087 add
...
#TQC-C10033 --unmark
 #MOD-C10100 --begin--
              CALL i551_replace_item_no(g_sfs[l_ac].sfs27,g_sfs[l_ac].sfs03,
...
#No.TQC-B90236--------------add----end  配方替代
   ...
   #FUN-AB0001 add str-------------------
    IF g_cnt>0 AND g_smy.smydmy4='Y' AND g_smy.smyapr <> 'Y'THEN  #單據需自動確認且不需簽核
       LET g_action_choice = "insert"
   #FUN-AB0001 add end-------------------
       CALL i551sub_y_chk(g_sfp.sfp01,g_action_choice)  #TQC-C60079
       IF g_success = "Y" THEN
          CALL i551sub_y_upd(g_sfp.sfp01,g_action_choice,FALSE)   
            RETURNING g_sfp.*
          DISPLAY BY NAME g_sfp.sfpconf  
          IF g_sfp.sfpconf='X' THEN
             LET g_chr='Y' 
...
FUNCTION i551_set_entry_b(p_cmd)
   DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-680121 VARCHAR(1)
...
FUNCTION i551_set_no_entry_b(p_cmd)
   DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-680121 VARCHAR(1)
...
FUNCTION i551_b_askkey()
    ...
    CALL i551_b_fill(l_wc2)
 
END FUNCTION
 
FUNCTION i551_d_fill(p_wc2)              #BODY FILL UP
 ...
 DEFINE l_ima01 LIKE ima_file.ima01
    ...
    PREPARE i551_pd FROM g_sql
    DECLARE sfq_curs CURSOR FOR i551_pd
...
#FUN-A20048 --begin
FUNCTION i551_b_fill_1() 
    DEFINE p_wc2           LIKE type_file.chr1000 #No.FUN-680121 VARCHAR(800)
    DEFINE l_factor        LIKE ima_file.ima31_fac  #TQC-7B0065
    DEFINE l_cnt           LIKE type_file.num5  #TQC-7B0065 
    
   	  OPEN WINDOW i551_g_b_q WITH FORM "asf/42f/asfi551a_2"
   	            ATTRIBUTE(STYLE = g_win_style CLIPPED )
   	  CALL cl_ui_locale("asfi551a_2") 
  
    LET g_sql="SELECT UNIQUE 'N',sie05 FROM sie_file WHERE sie11 >0 " 
    PREPARE i551_pb_1 FROM g_sql
    DECLARE sib_curs_1 CURSOR FOR i551_pb_1 
    CALL g_sib_1.clear()
...
FUNCTION i551_b_fill(p_wc2)              #BODY FILL UP
    ...
    DECLARE sfs_curs CURSOR FOR i551_pb  
...
# 
#   IF g_errno='genb' THEN CALL i551_b_fill(' 1=1') LET g_errno='' END IF
# 
#   DISPLAY ARRAY g_sfs TO s_sfs.* ATTRIBUTE(COUNT=g_rec_b,UNBUFFERED)
# 
#      BEFORE DISPLAY
...
FUNCTION i551_t()
   ...
   OPEN WINDOW i551_t_w AT 4,3 WITH FORM "asf/42f/asfi551c"
         ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
 
   CALL cl_ui_locale("asfi551c")
   CALL cl_load_act_list(NULL)   #FUN-610042
 
   DECLARE i551_t_c CURSOR FOR
...
#FUN-D40103 ------Begin------
   ...
   ELSE
      ...
      END FOREACH
      CALL i551_b_fill(" 1=1")  #MOD-840550 add
...
FUNCTION i551_du_default(p_cmd)
...
#對原來數量/換算率/單位的賦值
FUNCTION i551_set_origin_field()
  DEFINE    l_ima906 LIKE ima_file.ima906,
            l_ima907 LIKE ima_file.ima907,
            l_img09  LIKE img_file.img09,     #img單位
...
FUNCTION i551_set_du_by_origin()
...
FUNCTION i551_du_data_to_correct()
...
#計算庫存總量是否滿足所輸入數量
FUNCTION i551_check_inventory_qty()
...
DEFINE l_eci06 LIKE eci_file.eci06,
   ...
   OPEN WINDOW i551_plan AT 2,2 WITH FORM "asf/42f/asfi551d"
         ATTRIBUTE (STYLE = g_win_style CLIPPED)
 
   CALL cl_ui_locale("asfi551d")
...
#FUN-AA0059---------mod------------end-----------------
   ...
   PREPARE i551_plan_c_pre FROM l_wc1
   DECLARE i551_plan_c CURSOR FOR i551_plan_c_pre
 
   LET g_success='N'
   BEGIN WORK
   FOREACH i551_plan_c INTO l_sre.*
      IF SQLCA.sqlcode THEN
...
FUNCTION i551_get_sfs930(p_sfs03)
...
FUNCTION i551_b_i_move_back(l_i)
...
FUNCTION i551_totsfs05(p_sfp01,p_sfp06,p_sfs02,p_sfs03,p_sfs04,p_sfs06,p_sfs10,p_sfs27)  #No.MOD-790175 modify#FUN-9B0149
...
FUNCTION i551_ins_rvbs(l_sfs02,l_sfs03,l_sfs04)
   ...
   IF g_ima918 = "Y" OR g_ima921 = "Y" THEN
      DECLARE i551_g_sfs CURSOR FOR SELECT sfe02 FROM sfe_file
                                     WHERE sfe01 = l_sfs03
                                       AND sfe07 = l_sfs04
      ...
      FOREACH i551_g_sfs INTO l_sfe02
         IF STATUS THEN
            CALL cl_err('sfs',STATUS,1)
         END IF
      
         DECLARE i551_g_rvbs CURSOR FOR SELECT * FROM rvbs_file
                                        WHERE rvbs01 = l_sfe02
                                          AND rvbs021 = l_sfs04
         
         FOREACH i551_g_rvbs INTO l_rvbs.*
            IF STATUS THEN
...
FUNCTION i551_y_upd()  
   ...
   CALL i551sub_y_upd(g_sfp.sfp01,g_action_choice,FALSE)  #FUN-840012  
     RETURNING g_sfp.*
...
FUNCTION i551_ef()
    ...
    CALL i551sub_y_chk(g_sfp.sfp01,NULL)     #CALL 原確認的 check 段 #TQC-C60079
    IF g_success = "N" THEN
       RETURN
    END IF
...
FUNCTION i551_sfp16(p_cmd)  #申請人編號
 DEFINE   p_cmd      LIKE type_file.chr1,
...
#FUN-BA0050 ------------------Begin---------------------
FUNCTION i551_b_sfs09_inschk(p_cmd)
   DEFINE p_cmd           LIKE type_file.chr1                
...
FUNCTION i551_set_required_sfs09()
DEFINE l_ima159   LIKE ima_file.ima159    
DEFINE p_cmd      LIKE type_file.chr1
...
FUNCTION i551_set_no_required_sfs09()
   CALL cl_set_comp_required("sfs09",FALSE)
END FUNCTION

FUNCTION i551_set_no_entry_sfs09()
DEFINE l_ima159   LIKE ima_file.ima159
   IF NOT cl_null(g_sfs[l_ac].sfs04) THEN
      SELECT ima159 INTO l_ima159 FROM ima_file
       WHERE ima01 = g_sfs[l_ac].sfs04
...
#判斷是否是VMI
FUNCTION i551_isVMI(p_sfs07,p_sfs08)
...
#MOD-C50190 add end ------------
#FUN-C70014 add begin--------------
FUNCTION i551_multi_sfq014(p_n)
...
DEFINE l_success    STRING 
   ...
   FOR l_i=1 TO g_multi_sfq014.getLength()
      ...
      IF g_sfp.sfp06 MATCHES '[D]' THEN
         ...
         END IF 
         #抓去發料套數自動帶出
         CALL i551_sfq03_519(g_sfq[l_n].sfq014,g_sfq[l_n].sfq02,g_sfq[l_n].sfq04)
...
#Run Card 成套發料套數計算函數
FUNCTION i551_sfq03_519(p_sfq014,p_sfq02,p_sfq04)
...
DEFINE l_shm08 	    LIKE shm_file.shm08
   ...
   #將撈取qty1,qty2的部分整理到函數 i551_sfq03_chk1()處理  
   CALL i551_sfq03_chk(p_sfq02,p_sfq04,'',p_sfq014,'1')  #FUN-C70014 add sfq014
           RETURNING qty1,qty2                   
   IF qty1 IS NULL THEN LET qty1=0 END IF
   IF qty2 IS NULL THEN LET qty2=0 END IF
...
#檢查runcard資料
FUNCTION i551_chk_runcard(p_shm01,p_shm012)
...
#檢查輸入套數單身資料是否重複
FUNCTION i551_chk_sfq(p_n,p_type)
...
#FUN-CB0014---add---str---
FUNCTION i551_list_fill()
  DEFINE l_sfp01         LIKE sfp_file.sfp01
  DEFINE l_i             LIKE type_file.num10

    CALL g_sfp_l.clear()
    LET l_i = 1
    FOREACH i551_fill_cs INTO l_sfp01
       IF SQLCA.sqlcode THEN
...
FUNCTION i551_sfs27_cho() 
    ...
    OPEN WINDOW i551_sfs27_w WITH FORM "asf/42f/asfi551f"
   	          ATTRIBUTE(STYLE = g_win_style CLIPPED )
    CALL cl_ui_locale("asfi551f") 
  
    LET g_sql="SELECT sfa26,sfa27,sfa03,sfa05,sfa06 FROM sfa_file WHERE sfa01= '",g_sfs[l_ac].sfs03,"'" 
    PREPARE i551_sfs27_pb FROM g_sql
    DECLARE sfs27_curs CURSOR FOR i551_sfs27_pb 
    CALL l_sfa.clear()
