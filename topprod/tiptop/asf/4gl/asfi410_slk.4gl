...
DEFINE
    ...
    g_sfci  RECORD LIKE sfci_file.*,      #No.FUN-840096
    g_sfci_t RECORD LIKE sfci_file.*,     #No.FUN-840096
...
DEFINE l_sfcislk01_skd03  LIKE skd_file.skd03          #FUN-840096
DEFINE l_a1               STRING                       #FUN-840096
DEFINE l_sfb04            LIKE sfb_file.sfb04          #FUN-840096
DEFINE l_prog             STRING                       #FIN-840096
...
MAIN
  ...
  #start TQC-630068
  ...
  #end TQC-630068
   LET g_prog = 'asfi410_slk'  #No.FUN-840096 
   IF (NOT cl_user()) THEN
      EXIT PROGRAM
   END IF
   ...
   END IF
    ...
    DECLARE i410_cl CURSOR FROM g_forupd_sql              # LOCK CURSOR
       ...
       LET g_sql = "SELECT * FROM sfci_file WHERE sfci01 = ? FOR UPDATE "
       LET g_sql = cl_forupd_sql(g_sql)
       DECLARE i410_cl_ind CURSOR FROM g_sql
 ...
 #start TQC-630068
  ...
  ELSE
   ...
   INITIALIZE g_sfc.* TO NULL    #No.FUN-750051
    CONSTRUCT BY NAME g_wc ON                    # 螢幕上取條件
        sfc01,sfc02,
        sfcislk01,sfcislk02,sfcislk03,sfcislk04,sfcislk05,  #No.FUN-840096
        ...
        ON ACTION controlp
           CASE
              ...
              WHEN INFIELD(sfcislk01)
                 CALL cl_init_qry_var()
                 LET g_qryparam.form="q_sfcislk01"
                 LET g_qryparam.state="c"
                 LET g_qryparam.default1=g_sfci.sfcislk01
                 CALL cl_create_qry() RETURNING g_sfci.sfcislk01
                 CALL FGL_DIALOG_SETBUFFER(g_sfci.sfcislk01)
                 DISPLAY BY NAME g_sfci.sfcislk01
                 CALL i410_sfcislk01('d') 
  ...
  END IF   #TQC-630068
    ...
    LET g_wc = g_wc CLIPPED,cl_get_extra_cond('sfcuser', 'sfcgrup')  #MOD-D10251
       ...
       LET g_sql="SELECT sfc01 FROM sfc_file,sfci_file ", # 組合出 SQL 指令
                 " WHERE sfc_file.sfc01 = sfci_file.sfci01 AND ",g_wc CLIPPED, " ORDER BY sfc01" 
    PREPARE i410_prepare FROM g_sql           # RUNTIME 編譯
    DECLARE i410_cs                         # SCROLL CURSOR
       SCROLL CURSOR WITH HOLD FOR i410_prepare
      LET g_sql="SELECT COUNT(*) FROM sfc_file,sfci_file", 
                " WHERE sfc_file.sfc01 = sfci_file.sfci01 AND ",g_wc CLIPPED
...
FUNCTION i410_a()
    ...
    LET g_sfc01_t = NULL
    LET g_sfc_t.*=g_sfc.* 
    INITIALIZE g_sfci.* LIKE sfci_file.*
    LET g_sfci_t.* = g_sfci.*                    #No.FUN-840096
    CALL cl_opmsg('a')
    WHILE TRUE
        ...
        LET g_sfc.sfcacti = 'Y'
        LET g_sfci.sfcislk04 = 'N'                  #No.FUN-840096
        LET g_sfci.sfcislk05 = 'N'                  #No.FUN-840096
        LET g_sfci.sfcislk02 = g_today              #No.FUN-840096
        LET g_sfci.sfcislk03 = g_today              #No.FUN-840096
        LET l_a1 = '0'                              #No.FUN-840096
        CALL i410_i("a")                         # 各欄位輸入
        IF INT_FLAG THEN                         # 若按了DEL鍵
            LET INT_FLAG = 0
            INITIALIZE g_sfc.* TO NULL
            INITIALIZE g_sfci.* TO NULL          #No.FUN-840096
            CALL cl_err('',9001,0)
            CLEAR FORM
            EXIT WHILE
        END IF
...
#            CALL cl_err(g_sfc.sfc01,SQLCA.sqlcode,0)   #No.FUN-660128
             ...
             ROLLBACK WORK   #No:7829
             CONTINUE WHILE
          #NO.FUN-7B0018 08/02/17 add --begin
          ELSE    #No.FUN-830132
             INITIALIZE l_sfci.* TO NULL
             LET l_sfci.sfci01 = g_sfc.sfc01
             #No.FUN-840096--begin--          
             LET l_sfci.sfcislk01 = g_sfci.sfcislk01
             LET l_sfci.sfcislk02 = g_sfci.sfcislk02
             LET l_sfci.sfcislk03 = g_sfci.sfcislk03
             LET l_sfci.sfcislk04 = g_sfci.sfcislk01
             LET l_sfci.sfcislk05 = g_sfci.sfcislk05
             #No.FUN-840096--end--
...
#          #No.FUN-840096 --begin--
#          IF  l_prog.getIndexOf('_',1) THEN
#           	LET g_sfci.sfci01 = g_sfc.sfc01     
#             INSERT INTO sfci_file VALUES(g_sfci.*)       # DISK WRITE
#             IF SQLCA.sqlcode THEN  
#                ROLLBACK WORK
#                CONTINUE WHILE
#             END IF     
#          END IF 
#          #No.FUN-840096 --end--
           #NO.FUN-7B0018 08/02/17 add --end
          END IF   
          COMMIT WORK     #No:7829
...
#       l_sfc01         VARCHAR(3),
    ...
    DISPLAY BY NAME
        ...
        ,g_sfci.sfcislk04,g_sfci.sfcislk05                           #No.FUN-840096
    INPUT BY NAME g_sfc.sfcoriu,g_sfc.sfcorig,
        g_sfc.sfc01,g_sfc.sfc02,
        g_sfci.sfcislk01,g_sfci.sfcislk02,g_sfci.sfcislk03,g_sfci.sfcislk04,g_sfci.sfcislk05,  #No.FUN-840096
        g_sfc.sfcuser,g_sfc.sfcgrup,g_sfc.sfcmodu,g_sfc.sfcdate,g_sfc.sfcacti
        WITHOUT DEFAULTS
...
#              DISPLAY l_desc TO FORMONLY.d1
         #No.FUN-550067 ---end---
            END IF
        #No.FUN-840096--begin--
           AFTER FIELD sfcislk02
             IF g_sfci.sfcislk02>g_sfci.sfcislk03 THEN 
                CALL cl_err('','asf-121',0)
                NEXT FIELD sfcislk02
             END IF
 
           AFTER FIELD sfcislk03
             IF g_sfci.sfcislk02>g_sfci.sfcislk03 THEN  
                CALL cl_err('','asf-121',0)
                NEXT FIELD sfcislk03 
             END IF
                                                                                                          
           AFTER FIELD sfcislk01
             IF NOT cl_null(g_sfci.sfcislk01) THEN 
              IF p_cmd='a' OR (p_cmd='u' AND g_sfci.sfcislk01!=g_sfci_t.sfcislk01) THEN
                 SELECT COUNT(*) INTO l_n FROM skd_file
                   WHERE skd01 = g_sfci.sfcislk01
                     AND skd_file.skd04 = 'Y'
                     AND skd_file.skdacti = 'Y'
                 IF l_n =0 THEN
                   CALL cl_err('','ask-008',0)
                   LET g_sfci.sfcislk01 = g_sfci_t.sfcislk01
                   NEXT FIELD sfcislk01
                 END IF      
              END IF 
             END IF  
...
#       CALL cl_err(g_sfc.sfc01,SQLCA.sqlcode,0)   #No.FUN-660128
    ...
    ELSE
          SELECT * INTO g_sfci.* FROM sfci_file           
            WHERE sfci01 = g_sfc.sfc01
...
#   LET l_sfc01 = g_sfc.sfc01[1,3]
    ...
    #No.FUN-840096--begin--
    DISPLAY BY NAME g_sfci.sfcislk01
    DISPLAY BY NAME g_sfci.sfcislk02
    DISPLAY BY NAME g_sfci.sfcislk03
    DISPLAY BY NAME g_sfci.sfcislk04
    DISPLAY BY NAME g_sfci.sfcislk05
    LET l_sfcislk01_skd03= ''       #No.FUN-870117
    SELECT skd03 INTO l_sfcislk01_skd03 FROM skd_file,sfci_file WHERE skd01=g_sfci.sfcislk01
       DISPLAY l_sfcislk01_skd03 TO FORMONLY.sfcislk01_skd03 
    SELECT sfb04 INTO l_sfb04 FROM sfb_file,sfc_file WHERE sfb01=g_sfc.sfc01
              IF g_sfci.sfcislk05="Y" THEN 
                  LET l_a1="1"
              ELSE
                CASE l_sfb04
                WHEN '4'
    ...
    CALL cl_show_fld_cont()                   #No.FUN-550037 hmf
    CALL cl_set_field_pic1(g_sfci.sfcislk05,""  ,""  ,""  ,""  ,"",""    ,"")  #No.FUN-870117
END FUNCTION
...
FUNCTION i410_u()
    ...
    #No.FUN-840096 --begin--
       ...
       OPEN i410_cl_ind USING g_sfc.sfc01
       IF STATUS THEN
          CALL cl_err('OPEN i410_cl_ind:',STATUS,1)
          CLOSE i410_cl_ind
          ROLLBACK WORK
          RETURN
       END IF
    FETCH i410_cl_ind INTO g_sfci.*
    #No.FUN-840096 --end --
 
    LET g_sfc.sfcmodu=g_user                     #修改者
    LET g_sfc.sfcdate = g_today                  #修改日期
    #No.FUN-840096 --begin--
#    LET g_sfci.sfcislk04 = 'N'               #NO.FUN-870117
#    LET g_sfci.sfcislk05 = 'N'               #NO.FUN-870117
    LET g_sfci.sfcislk02 = g_today
    LET g_sfci.sfcislk03 = g_today
    #No.FUN-840096  --end--
    ...
    WHILE TRUE
        ...
        IF INT_FLAG THEN
          if g_bb = 'Y' then
            ...
            LET l_flag = s_del_sfci(g_sfc01_t,'')
...
#           CALL cl_err(g_sfc.sfc01,SQLCA.sqlcode,0)   #No.FUN-660128
        ...
        END IF
        #No.FUN-840096 --begin--
          UPDATE sfci_file SET sfci_file.sfcislk01 = g_sfci.sfcislk01,    # 更新DB
                               sfci_file.sfcislk02 = g_sfci.sfcislk02,
                               sfci_file.sfcislk03 = g_sfci.sfcislk03,
                               sfci_file.sfcislk04 = g_sfci.sfcislk04,
                               sfci_file.sfcislk05 = g_sfci.sfcislk05
            WHERE sfci_file.sfci01 = g_sfc.sfc01             # COLAUTH?
          IF SQLCA.sqlcode THEN
            CALL cl_err3("upd","sfci_file",g_sfc01_t,"",SQLCA.sqlcode,"","",1)  
            CONTINUE WHILE
          END IF
        #No.FUN-840096 --end--
       EXIT WHILE
...
#TQC-970108 --end--
    ...
    IF cl_delete() THEN
     ...
     #------------No.MOD-780177 end
      ...
      #NO.FUN-7B0018 08/02/16 add --begin
      LET l_flag = s_del_sfci(g_sfc.sfc01,'')
      #TQC-BC0166 -----modify---放開該段mark的內容
#     #No.FUN-840096 --begin--
     IF l_prog.getIndexOf('_',1) THEN
        DELETE FROM sfci_file WHERE sfci01 = g_sfc.sfc01
     END IF   
#     #No.FUN-840096 --end--
      #TQC-BC0166 -----modify---end
      #NO.FUN-7B0018 08/02/16 add --end
      CLEAR FORM
      OPEN i410_count
...
#          CALL cl_err(l_newno,SQLCA.sqlcode,0)   #No.FUN-660128
       ...
       #NO.FUN-7B0018 08/02/17 add --begin
       ELSE
          #No.FUN-830132 080327 add --begin
          SELECT * INTO l_sfci.*
            FROM sfci_file
           WHERE sfci01 = g_sfc.sfc01
          #No.FUN-830132 080327 add --end
          LET l_sfci.sfci01 = l_newno
          #No.FUN-840096 --begin--
          LET l_sfci.sfcislk01 = g_sfci.sfcislk01
          LET l_sfci.sfcislk02 = g_sfci.sfcislk02
          LET l_sfci.sfcislk03 = g_sfci.sfcislk03
          LET l_sfci.sfcislk04 = g_sfci.sfcislk04
          LET l_sfci.sfcislk05 = 'N' 
          #No.FUN-840096 --end--
          IF NOT s_ins_sfci(l_sfci.*,'') THEN
             ROLLBACK WORK
             RETURN
...
#      #No.FUN-840096 --begin--
#      IF l_prog.getIndexOf('_',1) THEN
#        SELECT * FROM sfci_file
#          WHERE sfci01 = g_sfc.sfc01
#          INTO TEMP x2 
#        UPDATE x2 SET sfci01 = l_newno  
#        INSERT INTO sfci_file
#          SELECT * FROM x2
#        IF SQLCA.sqlcode THEN
#          CALL cl_err3("ins","sfc_file",l_newno,"",SQLCA.sqlcode,"","",1) 
#          ROLLBACK WORK  
#          RETURN
#        END IF
#      END IF   
...
#No.FUN-840096--begin--
FUNCTION i410_confirm()
   IF l_a1!="0" THEN
      CALL cl_err('','asf-122',0)
      RETURN
   END IF
   IF g_sfc.sfc01 IS NULL THEN
     CALL cl_err('',-400,0)
   ...
   END IF 
    ...
    ELSE
        IF cl_confirm('aap-222') THEN
            ...
            WHERE sfci01=g_sfc.sfc01
        IF SQLCA.sqlcode THEN
            CALL cl_err3("upd","sfci_file",g_sfci.sfci01,"",SQLCA.sqlcode,"","sfcislk05",1)
            ROLLBACK WORK
        ELSE
            COMMIT WORK 
            LET g_sfci.sfcislk05 = "Y"
        END IF
        END IF
     END IF
END FUNCTION
 
FUNCTION i410_notconfirm()
  DEFINE l_n    LIKE type_file.num5
  
  SELECT COUNT(*) INTO l_n FROM sfb_file
     WHERE sfb85 = g_sfc.sfc01
  IF l_n > 0 THEN 
     CALL cl_err('','asf-260',1)
     RETURN
  END IF       
  IF l_a1!="1" THEN 
      CALL cl_err('','asf-123',0) 
      RETURN
  END IF  
  IF g_sfc.sfc01 IS NULL THEN
     CALL cl_err('',-400,0)
     RETURN
  END IF
  IF g_sfci.sfcislk05="N" OR g_sfc.sfcacti="N" THEN
    ...
    ELSE
      ...
      ELSE 
         COMMIT WORK
         LET g_sfci.sfcislk05 = "N" 
      END IF
      END IF
    END IF
END FUNCTION 
 
FUNCTION i410_sfcislk01(p_cmd)
   DEFINE l_skd03   LIKE skd_file.skd03, 
          l_skdacti LIKE skd_file.skdacti, 
          p_cmd     like type_file.chr1                                         
                                                                                
   LET g_errno=''                                                               
   SELECT skd03 INTO l_skd03 FROM skd_file,sfci_file                                     
          WHERE skd01=g_sfci.sfcislk01                                         
                                                                                
   CASE WHEN SQLCA.sqlcode=100 LET g_errno='mfg3006'                            
                           LET l_skd03=NULL                                      
        WHEN l_skdacti='N' LET g_errno='9028'                                   
        OTHERWISE          LET g_errno=SQLCA.sqlcode USING '-------'           
   END CASE                     
   IF cl_null(g_errno) OR p_cmd='d' THEN                                        
   DISPLAY l_skd03 TO FORMONLY.sfcislk01_skd03
   END IF                                              
END FUNCTION
