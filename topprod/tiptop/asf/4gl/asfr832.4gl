...
MAIN
   ...
   LET l_table = cl_prt_temptable('asfr832',g_sql) CLIPPED
   ...
   LET l_table1 = cl_prt_temptable('asfr8321',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      ...
      ELSE CALL asfr832()        # Read data and create out-file 
   END IF
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN LET INT_FLAG = 0 CLOSE WINDOW r832_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM 
   END IF
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      SELECT zz08 INTO l_cmd FROM zz_file  WHERE zz01='asfr832'
      IF SQLCA.sqlcode OR l_cmd IS NULL THEN
         CALL cl_err('asfr832','9031',1)
      ELSE
         ...
         CALL cl_cmdat('asfr832',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW r832_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM
...
#               "   shb115,shb16 ,ima02,gen02,sgm03,sgm04,sgm45,sgm06,sgm41 ", #No.FUN-710082  
   ...
   #            "   AND shc_file.shc01 = shb01 AND qce_file.qce01=shc_file.shc04",
     ...
     END IF
     DECLARE r832_cs1 CURSOR FOR r832_prepare1
...
#                " ORDER BY 1"   #No.FUN-710082
     PREPARE r832_p2 FROM l_sql
     IF STATUS THEN CALL cl_err('p2:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
        EXIT PROGRAM 
     END IF
     DECLARE r832_c2 CURSOR FOR r832_p2
 
     #No.FUN-710082--begin
#    CALL cl_outnam('asfr832') RETURNING l_name
#    START REPORT r832_rep TO l_name
     ...
     LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
                 " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ",
                 "        ?,?,?,?,?, ?,?,?,?,?,  ",
                 "        ?,?,?,?,?, ?) " #NO.FUN-A60080 add ? #TQC-C10039 add 4?
     ...
     FOREACH r832_cs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
       END IF
       FOREACH r832_c2 USING sr.shb01,sr.shb16 INTO sr1.*
...
#TQC-C10039--add--end---
   # CALL cl_prt_cs3('asfr832',l_sql,l_str)  #TQC-730088
     #NO.FUN-A60080--begin
     ...
     ELSE 
     	  LET l_t='asfr832'
     END IF 
     CALL cl_prt_cs3('asfr832',l_t,l_sql,l_str)  
     #NO.FUN-A60080--end   
     #CALL cl_prt_cs3('asfr832','asfr832',l_sql,l_str) 
     #No.FUN-710082--end  
END FUNCTION
