...
# Modify.........: No.FUN-CB0072 12/12/04 By dongsz CR轉GR
# Modify.........: No:FUN-D10098 13/01/30 By chenjing 增加幫助ACTION
# Modify.........: No:FUN-CB0072 13/02/05 By chenying 增加料表批號開窗 
#                                                     追單MOD-D10038
...
TYPE sr1_t RECORD
    ...
    ima021 LIKE ima_file.ima021,   #FUN-D10098
...
MAIN
   ...
   LET g_sql = "sfa03.sfa_file.sfa03,",
               ...
               "ima021.ima_file.ima021,",   #FUN-D10098
   ...
   LET l_table = cl_prt_temptable('asfg108',g_sql) CLIPPED
   IF  l_table = -1 THEN EXIT PROGRAM END IF 
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES (?,?,?,?,?, ?,?,?,?,?, ?,?,?,?, ?,?,?)"         #FUN-A60027 add 2? #FUN-D10098 add 3? 
     ...
     INPUT BY NAME tm.bdate,tm.edate,tm.bdate1,tm.edate1,tm.c,tm.d,tm.more
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('asfg108',g_time,l_cmd)
         END IF
         CLOSE WINDOW r108_w
...
#         l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT    #TQC-630166       #No.FUN-680121 VARCHAR(1000)
   ...
   DEFINE l_ima02       LIKE ima_file.ima02       #FUN-D10098
   DEFINE l_ima021      LIKE ima_file.ima021       #FUN-D10098
     ...
     LET l_sql = "SELECT ",
                #"sfa03,rvb05,ima02,ima021,0,rvb12,sfb01,sfb05,",     #FUN-D10098  #FUN-CB0072 追單MOD-D10038
                 "sfa03,sfa03,ima02,ima021,0,'',sfb01,sfb05,",     #FUN-D10098     #FUN-CB0072 追單MOD-D10038
              #  "dateadd(dd,sfa09,sfb13),sfa12,sfa05,sfa05-sfa06,sfa07",    #FUN-940008 mark
                ...
                #"'',sfb13,sfa09,sfa12,sfa05,sfa012,sfa013,sfa05-sfa06,'','',''",  #FUN-CB0072 追單MOD-D10038
                 "sfb13+COALESCE(sfa09,0),sfb13,sfa09,sfa12,sfa05,sfa012,sfa013,sfa05-sfa06,'','',''",   #FUN-CB0072 追單MOD-D10038
                #" FROM sfb_file,sfa_file,rvb_file,ima_file", #FUN-CB0072 追單MOD-D10038
                 " FROM sfb_file,sfa_file,ima_file", #FUN-CB0072 追單MOD-D10038
                 " WHERE ", tm.wc CLIPPED,
                #"   AND sfb01 = rvb34",    #MOD-C30010 mark
                #"   AND sfa03 = rvb05",    #FUN-CB0072 追單MOD-D10038
                 # "   AND rvb12 BETWEEN  cast('",tm.bdate1,"' as datetime)  AND  cast('",tm.edate1,"' as datetime) ",
                #"   AND rvb12 BETWEEN  '",tm.bdate1 ,"' AND  '",tm.edate1 ,"' ",             #TQC-A70128 #FUN-CB0072 追單MOD-D10038
                 "   AND sfb13+COALESCE(sfa09,0) BETWEEN  '",tm.bdate1 ,"' AND  '",tm.edate1 ,"' ",       #FUN-CB0072 追單MOD-D10038
                #"   AND rvb05 = ima_file.ima01",  #FUN-CB0072 追單MOD-D10038
                 "   AND sfa03 = ima_file.ima01",  #FUN-CB0072 追單MOD-D10038
...
#    CALL cl_outnam('asfg108') RETURNING l_name        #NO.FUN-750093
#    START REPORT r108_rep TO l_name                   #NO.FUN-750093
     ...
     FOREACH r108_curs1 INTO sr.*
         ...
         ##FUN-CB0072 追單MOD-D10038--mark
          ...
          SELECT ima02,ima021 INTO l_ima02,l_ima021 FROM ima_file WHERE ima01 = sr.sfb05   #FUN-D10098
          # NO.FUN-750093     -------begin-------
          EXECUTE insert_prep USING sr.sfa03,sr.rvb05,
                                    sr.ima02,sr.ima021,sr.rvb07,    #FUN-D10098 add sr.ima021
                                    sr.rvb12,sr.sfb01,
                                    sr.sfb05,sr.ndate,
                                    sr.sfa12,sr.sfa05,
                                    sr.sfa012,sr.sfa013,              #FUN-A60027 add 
                                    sr.qty,sr.sfa07,
                                    l_ima02,l_ima021                  #FUN-D10098 add         
...
###GENGRE###     LET g_str = tm.wc                             #NO.FUN-750093
     END IF                                        #NO.FUN-750093
###GENGRE###     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED   #NO.FUN-750093
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)   #NO.FUN-750093
#    CALL cl_prt_cs3('asfg108','asfg108',l_sql,g_str) #NO.FUN-750093
     #No.FUN-A60027 --------------------start----------------------
     IF g_sma.sma541 = 'Y' THEN
###GENGRE###        CALL cl_prt_cs3('asfg108','asfg108_1',l_sql,g_str)
        LET g_template = 'asfg108_1'     #FUN-CB0072 add
        CALL asfg108_grdata()    ###GENGRE###
     ELSE
###GENGRE###        CALL cl_prt_cs3('asfg108','asfg108',l_sql,g_str)
        LET g_template = 'asfg108'       #FUN-CB0072 add
        CALL asfg108_grdata()    ###GENGRE###
     END IF 
     #No.FUN-A60027 ----------------------end---------------------- 
END FUNCTION
...
FUNCTION asfg108_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("asfg108")
        IF handler IS NOT NULL THEN
            START REPORT asfg108_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                       ," ORDER BY rvb05,sfb01"               #FUN-CB0072 add
          
            DECLARE asfg108_datacur1 CURSOR FROM l_sql
            FOREACH asfg108_datacur1 INTO sr1.*
                OUTPUT TO REPORT asfg108_rep(sr1.*)
            END FOREACH
            FINISH REPORT asfg108_rep
        END IF
...
REPORT asfg108_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_sfa05_1_sum LIKE sfa_file.sfa05
    DEFINE l_qty_1_sum LIKE sfa_file.sfa07
    DEFINE l_sfa07_1_sum LIKE sfa_file.sfa07
    DEFINE l_sfa05_2_sum LIKE sfa_file.sfa05
    DEFINE l_qty_2_sum LIKE sfa_file.sfa07
    DEFINE l_sfa07_2_sum LIKE sfa_file.sfa07

    
    ORDER EXTERNAL BY sr1.rvb05,sr1.sfb01
    ...
    FORMAT
        ...
        AFTER GROUP OF sr1.rvb05
            LET l_sfa07_1_sum = GROUP SUM(sr1.sfa07)
            PRINTX l_sfa07_1_sum
            LET l_qty_1_sum = GROUP SUM(sr1.qty)
            PRINTX l_qty_1_sum
            LET l_sfa05_1_sum = GROUP SUM(sr1.sfa05)
            PRINTX l_sfa05_1_sum
        AFTER GROUP OF sr1.sfb01
            LET l_sfa07_2_sum = GROUP SUM(sr1.sfa07)
            PRINTX l_sfa07_2_sum
            LET l_qty_2_sum = GROUP SUM(sr1.qty)
            PRINTX l_qty_2_sum
            LET l_sfa05_2_sum = GROUP SUM(sr1.sfa05)
            PRINTX l_sfa05_2_sum
