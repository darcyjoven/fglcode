...
# Modify.........: No.160704     16/07/04 By guanyao 若工单类型为试产，则必输，其他类型则不允许输入
# Modify.........: No.170731     17/07/31 By ly  若工单类型为返工，排版=1
# Modify.........: NO:MOD-DA0166 21/03/15 By hehw 當工單型態為5or8時，QPA及應發數量 有異常
# Modify.........: No:MOD-FC0046 21/03/15 By hehw   調整MOD-DA0166,若單身資料新舊值並無異動時,不需重新計算實際QPA與應發數量 
...
DEFINE
    ...
    g_tc_ecm RECORD LIKE tc_ecm_file.*,
    ...
    g_sfa     DYNAMIC ARRAY OF RECORD    #程式變數(Prinram Variables)
                 ...
                 sfaud08 LIKE sfa_file.sfaud08,#str---add by huanglf160808
...
#FUN-C30114---begin
               ...
               WHEN INFIELD(sfbud04) 
                    CALL cl_init_qry_var()
                    LET g_qryparam.state    = "c"
                    LET g_qryparam.form     = "cq_sfb02"    #TQC-7B0048
                    LET g_qryparam.default1 = g_sfb.sfbud04
                    CALL cl_create_qry() RETURNING g_qryparam.multiret
                    DISPLAY g_qryparam.multiret TO sfbud04
                    NEXT FIELD sfbud04 
...
#FUN-C30114---end               
               ...
               WHEN INFIELD(sfb27)
                    CALL cl_init_qry_var()
                    LET g_qryparam.state    = "c"
                    #LET g_qryparam.form     = "q_pja2"  #FUN-810045   #MOD-980224 q_pja2-->q_pja4 #MOD-9A0048 q_pja4 -> q_pja2 #mark by guanyao160704
                    LET g_qryparam.form     = "q_pja"  #add by guanyao160704
                    CALL cl_create_qry() RETURNING g_qryparam.multiret
      ...
      CONSTRUCT g_wc2 ON sfa012,sfa013,sfa27,sfa03,sfa12,   #FUN-A50066
                         ...
                         #sfa28,sfa161,sfa100,sfaud08,sfa05,#str----add by huanglf160808  #mark by guanyao160907
                         sfa28,sfa161,sfa100,sfa05,#str----add by huanglf160808           #add by guanyao160907
                         sfa06,
                         sfa062,
                         sfa065,
                         sfa063,sfa064,
           ...
           FROM s_sfa[1].sfa012,s_sfa[1].sfa013,     #FUN-A50066
                ...
                #s_sfa[1].sfa161,s_sfa[1].sfa100,s_sfa[1].sfaud08,s_sfa[1].sfa05, #str---add by huanglf160808  #mark by guanyao160907
                s_sfa[1].sfa161,s_sfa[1].sfa100,s_sfa[1].sfa05, #str---add by huanglf160808  #add by guanyao160907
                s_sfa[1].sfa06,
                s_sfa[1].sfa062,
                s_sfa[1].sfa065,
                s_sfa[1].sfa063,
...
#DEV-D30026 add end------------------ 
DEFINE l_cmd LIKE type_file.chr1000  #add by huanglf161012
#No.18010101--begin--
DEFINE l_ret        RECORD
...
#str----add by huanglf161012
        WHEN "update_no"
          IF cl_chk_act_auth() THEN 
              IF l_ac > 0 THEN           #No.MOD-A30038 add
               LET l_cmd = "ceci700 "," '",g_sfb.sfb01,"'",
                           " '",g_sfb.sfb05,"'"," '",g_sfa[l_ac].sfa08,"'"          #No.MOD-780015 add
...
#  並將所有合拼的工單及PBI資料做確認
                    UPDATE sfb_file SET sfbud10 = (select imaud10 from ima_file ,sfb_file  where ima01 = sfb05 and ima01 = g_sfb.sfb05 and sfb01 = g_sfb.sfb01) WHERE sfb05 = g_sfb.sfb05 and sfb01 = g_sfb.sfb01
                    IF g_sfb.sfb01[1,3] ='MSY' THEN
                     update sfb_file set sfbud08 = (select oeb12 from oeb_file where oeb01 = g_sfb.sfb22 and oeb03 = g_sfb.sfb221 ) where sfb01 = g_sfb.sfb01
                     update sfb_file set sfbud09 = (select oeb12/sfb08*100 from sfb_file ,(select oeb01,oeb03,oeb04,oeb12 from oeb_file where oeb01 = g_sfb.sfb22 and oeb03 = g_sfb.sfb221) where sfb22 = oeb01 and sfb221 = oeb03 and sfb05 = oeb04 and sfb01 = g_sfb.sfb01) where sfb01 = g_sfb.sfb01
                     END IF
                     ...
                     #begin--版本第一次做给提示
					 LET l_cnt1=0
					 SELECT COUNT(*) INTO l_cnt1 FROM sfb_file WHERE sfb05=g_sfb.sfb85 AND sfb06=g_sfb.sfb06 AND sfb87='Y'
					 IF l_cnt1=1 THEN 
					  CALL cl_err('','csf-602',1) 
					 END IF
					 #end 版本第一次做给提示
...
#FUN-A80054--end--add---------------------------------------------------------------------------
         ...
         WHEN "undo_confirm"
            IF cl_chk_act_auth() THEN
                #No.18010101--begin--
                IF NOT cl_null(g_sfb.sfb01) AND cl_getscmparameter() THEN
                    INITIALIZE l_ret TO NULL
                    CALL cjc_zmx_json_cancelWorktask(g_sfb.sfb01,'WT') RETURNING l_ret.*
                    IF l_ret.success = 'Y' THEN
                    ELSE
                       IF cl_null(l_ret.msg) THEN
                           LET l_ret.msg = "工单撤销(",g_sfb.sfb01 CLIPPED,")同步失败"
                       END IF
         ...
         WHEN "output"
            IF cl_chk_act_auth() THEN
              # CALL i301_out()  #mark by guanyao160621
               CALL i301_out1()  #add by guanyao160621
                SELECT sfb04 INTO g_sfb.sfb04 FROM sfb_file
                WHERE sfb01=g_sfb.sfb01
                DISPLAY BY NAME g_sfb.sfb04
            END IF
         ...
         #str---add by jixf 160810 批量刷新runcard单身工艺资料
         WHEN "upd_runcard"
            IF cl_chk_act_auth() THEN
               CALL i301_upd_runcard()
            END IF 
      ...
      #str---add by ly 180328    BLOCK
         WHEN "bolck"
           ...
           SELECT sfbud06  INTO g_sfb.sfbud06 FROM sfb_file  WHERE sfb01=g_sfb.sfb01   
        ...
        #DEV-D30026---add----end----------------
         ...
         WHEN "transf2scm"
            IF cl_chk_act_auth() THEN
                IF NOT cl_null(g_sfb.sfb01) AND cl_getscmparameter() THEN
                    INITIALIZE l_ret TO NULL
                    CALL cjc_zmx_json_sfb(g_sfb.sfb01) RETURNING l_ret.*
                    IF l_ret.success = 'Y' THEN

                    ELSE
                       IF cl_null(l_ret.msg) THEN
                           LET l_ret.msg = "工单(",g_sfb.sfb01 CLIPPED,")同步失败"
                       END IF
                    END IF
                    CALL cl_err(l_ret.msg,'!',1)
                END IF
...
FUNCTION i301_a()
   ...
   #DEFINE l_sfd        RECORD LIKE sfd_file.*   #FUN-A80054
    ...
    DEFINE l_oea00      LIKE oea_file.oea00   #add by guanyao160505
#str----add by guanyao160612
    ...
    DEFINE l_oeb12         LIKE oeb_file.oeb12 #add by darcy 2022年3月11日
...
#FUN-A20037  --end--      
        ...
        IF STATUS THEN
          ...
          #FUN-960025 ADD --END-------------------------------
        END IF
        #str----add by guanyao160505
        IF NOT cl_null(g_sfb.sfb22) AND NOT cl_null(g_sfb.sfb221)THEN
           LET l_oea00 = ''
           SELECT oea00 INTO l_oea00 FROM oea_file WHERE oea01 =  g_sfb.sfb22
           IF l_oea00 = '0' THEN 
              #str----add by guanyao160612
              LET l_ima562_1 = 0
              LET l_ima55_fac_1=0
              SELECT ima55_fac,((ima562+100)/100) INTO l_ima55_fac_1,l_ima562_1 FROM ima_file WHERE ima01 = g_sfb.sfb05
              ...
              IF cl_null(l_ima562_1) THEN 
                 LET l_ima562_1 = 1
              END IF 
              LET l_sfb08_1 = g_sfb.sfb08*l_ima55_fac_1/l_ima562_1
              #add by darcy2022-03-11 13:49:58 s---
              SELECT oeb12 INTO l_oeb12 FROM oeb_file WHERE oeb01 = g_sfb.sfb22 AND oeb03 = g_sfb.sfb221 
              LET l_sfb08_1 = IIF(l_sfb08_1 > l_oeb12,l_oeb12,l_sfb08_1)
              #add by darcy2022-03-11 13:49:58 e---
              #end----add by guanyao160612           
              UPDATE oeb_file SET oebud07 =nvl(oebud07+l_sfb08_1,l_sfb08_1),
                                  oebud09 = nvl(oebud09-l_sfb08_1)   #add by guanyao160614
               WHERE oeb01 = g_sfb.sfb22
                 AND oeb03 = g_sfb.sfb221
            END IF 
        END IF 
        #end----add by guanyao160505
        COMMIT WORK
...
#  DEFINE l_woqty         LIKE ima_file.ima26            #No.FUN-680121 DEC(15,3)
   ...
   DEFINE l_pja02_1         LIKE pja_file.pja02      #add by guanyao160704
   DEFINE l_no              LIKE type_file.num5      #add by chenkun160815
   DEFINE l_sum1            LIKE sfb_file.sfb08
   DEFINE l_sum2            LIKE sfb_file.sfb08
   DEFINE l_sum3            LIKE sfb_file.sfb08 
    LET g_bomdate=NULL
    ...
    CALL cl_set_head_visible("","YES")  #NO.FUN-6B0031
    INPUT BY NAME g_sfb.sfboriu,g_sfb.sfborig,g_sfb.sfb01,g_sfb.sfb101,g_sfb.sfb44,g_sfb.sfb81,g_sfb.sfb02,g_sfb.sfb39,g_sfb.sfbud04,   #FUN-8C0081 add sfb44    #add sfbud04 by chenkun160815
                 ...
                 #g_sfb.sfb13,g_sfb.sfb15,g_sfb.sfb25,g_sfb.sfb081,                         #FUN-9A0029 mark
                  ...
                  ,g_sfb.sfbud01,g_sfb.sfbud02,g_sfb.sfbud03,
              ...
              IF NOT cl_null(g_sfb.sfb01) THEN
               ...
               #Mark FUN-B50026--end
                ...
                END IF
                 ...
                 IF g_sfb.sfb02 ='7' THEN LET g_sfb.sfb93='N' END IF #add by liy210707
             ...
             #str MOD-A40133 add
              ...
              IF g_cnt > 0 THEN
                 ...
                 SELECT COUNT(*) INTO g_cnt
                  ...
                  WHERE oeb70 = 'N'
                    #AND oea00 <> '0' AND oea00 <> '4'  #mark by guanyao160505
                    AND oea11 <> '3' AND oea00 <> '4'   #add by guanyao160505
...
#FUN-AA0059 ---------------------end-------------------------------
            ...
            #END IF
             ...
             IF cl_null(g_ima.imaud10) OR g_ima.imaud10 = 0 THEN 
                LET g_ima.imaud10 = 1
             END IF 
           #add by 170731 ly
                 LET g_sfb.sfbud10 = g_ima.imaud10              
             DISPLAY BY NAME g_sfb.sfbud10
            ...
            IF NOT cl_null(g_sfb.sfb05) AND NOT cl_null(g_sfb.sfb08) THEN 
               CALL i301_sfbud07()
            END IF  
...
# No:2422  modify 1998/09/08 ---------(check 最小生產數量) ---
        ...
        IF g_sfb.sfbud04 IS NOT NULL THEN 
          SELECT SUM(sfb08) INTO l_sum1 FROM sfb_file WHERE sfbud04 = g_sfb.sfbud04
           IF cl_null(l_sum1) THEN LET l_sum1 = 0 END IF 
          SELECT sfb08 INTO l_sum2 FROM sfb_file WHERE sfb01 = g_sfb.sfbud04
           IF cl_null(l_sum2) THEN LET l_sum2 = 0 END IF 
          LET l_sum3 = l_sum2 - l_sum1
          IF l_sum3<g_sfb.sfb08 THEN 
             CALL cl_err('','csf-075',0) 
             LET g_sfb.sfb08 = l_sum3
             NEXT FIELD sfb08
          END IF 
        END IF 
...
#FUN-A50066--end--mark--------------------
       ...
       AFTER FIELD sfb27
          IF NOT cl_null(g_sfb.sfb27) AND g_aza.aza08='Y' THEN    #bugno:7255
             SELECT * FROM pja_file WHERE pja01=g_sfb.sfb27
                                      AND pjaclose='N'            #FUN-960038
                                      AND pjaconf = 'Y'           #add by guanyao160704
             IF STATUS THEN
       ...
       AFTER FIELD sfbud04
          ...
          IF NOT cl_null(g_sfb.sfbud04) THEN
              IF g_sfb.sfbud04 != g_sfb_t.sfbud04
                 OR g_sfb_t.sfbud04 IS NULL THEN
               LET l_n=0
               SELECT count(sfb01) INTO l_no FROM sfb_file WHERE sfb01=g_sfb.sfbud04
               IF l_no=0 THEN  
                  CALL cl_err('','sfb-001',1)
                  NEXT FIELD sfbud04
               END IF 
            END IF
          #str----add by huanglf160818
          SELECT sfb05 INTO g_sfb.sfb05 FROM sfb_file WHERE sfb01 = g_sfb.sfbud04
          DISPLAY BY NAME g_sfb.sfb05 
          SELECT SUM(sfb08) INTO l_sum1 FROM sfb_file WHERE sfbud04 = g_sfb.sfbud04
          IF cl_null(l_sum1) THEN LET l_sum1 = 0 END IF 
          SELECT sfb08 INTO l_sum2 FROM sfb_file WHERE sfb01 = g_sfb.sfbud04
          IF cl_null(l_sum2) THEN LET l_sum2 = 0 END IF 
           LET g_sfb.sfb08 = l_sum2 - l_sum1 
          DISPLAY BY NAME g_sfb.sfb08
          #tianry add 161121
          SELECT imaud10 INTO g_sfb.sfbud10 FROM ima_file WHERE ima01=g_sfb.sfb05
          DISPLAY BY NAME g_sfb.sfbud10
        ...
        ON ACTION controlp
           CASE
              WHEN INFIELD(sfb01) #order nubmer
               ...
               WHEN INFIELD(sfb22)
                    CALL cl_init_qry_var()
                    #LET g_qryparam.form     = "q_oea3"  #mark by guanyao160505
                    LET g_qryparam.form     = "cq_oea03" #add by guanyao160505
...
#                   CALL cl_init_qry_var()                              #FUN-AA0059 mark
                    IF g_sfb.sfb02 = '15' THEN 
                       ...
                       #LET g_qryparam.form     = "q_bmq1"              #mark by guanyao160704
                       LET g_qryparam.form     = "cq_bmo"              #add by guanyao160704
                       LET g_qryparam.default1 = g_sfb.sfb05
 ...
 DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-680121 VARCHAR(1)
    ...
    IF INFIELD(sfb02) OR ( NOT g_before_input_done ) THEN
       ...
       IF g_sfb.sfb02 ='15' THEN
          ...
          CALL cl_set_comp_entry("sfb27",TRUE)
          CALL cl_set_comp_required("sfb27",TRUE)
...
FUNCTION i301_set_no_entry(p_cmd)
  ...
  END IF 
   ...
   #TQC-940069 add ICD行業時不受此控管,及ICD委外工單也要可以輸入sfb06(for sfbiicd09 kind = '6:tyk')
    ...
    END IF
    IF g_sfb.sfb02 MATCHES '[78]' THEN LET g_sfb.sfb93='N' CALL cl_set_comp_entry("sfb93",FALSE) END IF #add by liy210707 
    ...
    END IF
     ...
     IF g_sfb.sfb02 !='15' THEN
        ...
        #str-----add by guanyao160704
        CALL cl_set_comp_entry("sfb27",FALSE)
        CALL cl_set_comp_required("sfb27",FALSE)
        #end-----add by guanyao160704
     END IF
...
FUNCTION i301_b(p_in)
   ...
   DEFINE l_ima928        LIKE ima_file.ima928      #FUN-C30047
   DEFINE l_sfaud02_t     LIKE sfa_file.sfaud02
   DEFINE l_num,l_num1      LIKE type_file.num5       #add by huanglf161012
   DEFINE l_sql   STRING 
    LET g_action_choice = ""
...
#str-----add by huanglf161012
        ...
        INITIALIZE g_tc_ecm.* TO NULL
                LET g_tc_ecm.tc_ecm00= g_sfb.sfb01
                LET g_tc_ecm.tc_ecm01 =g_sfb.sfb05
                LET g_tc_ecm.tc_ecm02 =g_sfa[l_ac].sfa08

      SELECT COUNT(*) INTO l_num1 FROM tc_ecm_file 
      WHERE tc_ecm00 = g_tc_ecm.tc_ecm00 AND tc_ecm01 = g_tc_ecm.tc_ecm01 AND tc_ecm02 = g_tc_ecm.tc_ecm02
         IF cl_null(l_num1) THEN 
            LET l_num1 = 0
         END IF
         
      SELECT COUNT(*) INTO l_num FROM tc_ecn_file 
      WHERE tc_ecn01 = g_tc_ecm.tc_ecm01 AND tc_ecn02 = g_tc_ecm.tc_ecm02

   IF l_num>0 AND l_num1 = 0 THEN
        LET l_sql = " SELECT '',tc_ecn_file.*",
                    " FROM tc_ecn_file ",
                    " WHERE tc_ecnacti = 'Y' AND tc_ecn01 = '",g_tc_ecm.tc_ecm01,"'  AND  tc_ecn02 = '",g_tc_ecm.tc_ecm02,"'",
                    " AND tc_ecn09 = (SELECT max(tc_ecn09) FROM tc_ecn_file",
                    " WHERE tc_ecnacti = 'Y' AND tc_ecn01 = '",g_tc_ecm.tc_ecm01,"'  AND  tc_ecn02 = '",g_tc_ecm.tc_ecm02,"')"
        PREPARE tc_ecm_pb FROM l_sql
        DECLARE tc_ecm_curs CURSOR FOR tc_ecm_pb
        INITIALIZE g_tc_ecm.* TO NULL

     
       #單身 ARRAY 填充
        FOREACH tc_ecm_curs INTO g_tc_ecm.*        
          LET g_tc_ecm.tc_ecm00 = g_sfb.sfb01
          INSERT INTO tc_ecm_file(tc_ecm00,tc_ecm01,tc_ecm02,tc_ecm03,tc_ecm04,tc_ecm05,tc_ecm06,
                              ...
                              VALUES(g_tc_ecm.tc_ecm00,g_tc_ecm.tc_ecm01,g_tc_ecm.tc_ecm02,g_tc_ecm.tc_ecm03,
                                     g_tc_ecm.tc_ecm04,g_tc_ecm.tc_ecm05,g_tc_ecm.tc_ecm06,
                                     g_tc_ecm.tc_ecm07,g_tc_ecm.tc_ecm08,g_tc_ecm.tc_ecmacti,g_tc_ecm.tc_ecm09)      #No.FUN-980030 10/01/04  insert columns oriu, orig
                IF SQLCA.sqlcode THEN
                   CALL cl_err3("ins","tc_ecm_file",g_tc_ecm.tc_ecm01,g_tc_ecm.tc_ecm09,
...
#FUN-AA0059 ---------------------end-------------------------------
              ...
              #-------------------No:MOD-A10017 end
               ...
               IF (g_sfa[l_ac].sfa26 IS NULL OR g_sfa[l_ac].sfa27 != g_sfa_t.sfa27 
                  ...
                  LET g_sfa[l_ac].sfa16 = b_sfa.sfa16    #add by hehw 210317
                  ...
                  DISPLAY BY NAME g_sfa[l_ac].sfa16    #add by hehw 210317
              ...
              #FUN-910088--add--end--
               END IF
                  IF (g_sfa_o.sfa27 IS NULL) OR (g_sfa_o.sfa27 != g_sfa[l_ac].sfa27)  THEN    #MOD-FC0046 add
                     ...
                     LET g_sfa[l_ac].sfa161 = g_sfa[l_ac].sfa05/g_sfb.sfb08   #add by hehw 210317
...
#FUN-AA0059 ---------------------end-------------------------------
    ...
    AFTER FIELD sfa100
       ...
       IF  g_sfa[l_ac].sfa100 >100 OR   g_sfa[l_ac].sfa100< -100 THEN 
    ...
    AFTER FIELD sfa065
     ...
     #FUN-910088--mark--end--
       ...
       AFTER FIELD sfaud08
        ...
        IF NOT cl_null(g_sfa[l_ac].sfaud08) THEN 
           IF p_cmd='a' OR (p_cmd = 'u' AND (g_sfa_t.sfaud08  != g_sfa[l_ac].sfaud08 OR g_sfa_t.sfaud08 IS NULL ) ) THEN 
              IF g_sfa[l_ac].sfaud08>0 AND g_sfa[l_ac].sfaud08<=1  THEN
                 LET g_sfa[l_ac].sfa05 = g_sfb.sfb08 * g_sfa[l_ac].sfa16/g_sfa[l_ac].sfaud08
                 LET g_sfa[l_ac].sfa05 = s_digqty(g_sfa[l_ac].sfa05,g_sfa[l_ac].sfa12) 
                 LET g_sfa[l_ac].sfa161 =  g_sfa[l_ac].sfa05/g_sfb.sfb08
              ELSE 
                 LET g_sfa[l_ac].sfaud08 = ''
                 NEXT FIELD sfaud08
              END IF 
              CALL s_digqty(g_sfa[l_ac].sfa05,g_sfa[l_ac].sfa12) RETURNING  g_sfa[l_ac].sfa05
              DISPLAY BY NAME g_sfa[l_ac].sfaud08
           ELSE 
             #LET g_sfa[l_ac].sfaud08 = ''
             #NEXT FIELD sfaud08
           END IF 
...
FUNCTION i301_set_no_entry_b(p_cmd)
  DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-680121 VARCHAR(1)
    ...
    END IF
    IF g_sfb.sfb02 MATCHES '[78]' THEN CALL cl_set_comp_entry("sfb93",FALSE) END IF #add by liy210707
...
DEFINE l_wc2           LIKE type_file.chr1000 #No.FUN-680121 VARCHAR(200)
    ...
    CONSTRUCT l_wc2 ON sfa012,sfa013,sfa27,sfa08,sfa26,sfa28,sfa03,  #FUN-A50066
                       sfa12,sfa161,sfaud08,sfa05,sfa065,sfa06,   #str----huanglf160808
                       sfa062,
                       sfa063,sfa064
            FROM s_sfa[1].sfa012,s_sfa[1].sfa013,    #FUN-A50066
                 s_sfa[1].sfa27, s_sfa[1].sfa08, s_sfa[1].sfa26,
                 s_sfa[1].sfa28, s_sfa[1].sfa03, s_sfa[1].sfa12,
                 s_sfa[1].sfa161, s_sfa[1].sfaud08,s_sfa[1].sfa05, s_sfa[1].sfa065,
...
#No.FUN-630087 維護雜發單
     ...
     #DEV-D30026--add--end-------------
      ...
      #str---add by jixf 160810 批量刷新runcard单身工艺资料
      ON ACTION upd_runcard
         LET g_action_choice = 'upd_runcard'
         EXIT DISPLAY 
      #end---add by jixf 160810
...
FUNCTION i301_out()
    ...
    #LET g_sql = "asfg102 ",  #FUN-C30085 add
                " '",g_today,"'",
                " '",g_user,"'",
...
#TQC-BA0162
            ...
            IF NOT i301_upd_sfbdate(g_sfb.sfb01) THEN
               LET g_success = 'N'
            END IF  
...
FUNCTION i301_s()
   ...
   DEFINE old_sfa08         LIKE sfa_file.sfa08
   DEFINE sfa26             LIKE sfa_file.sfa26  #add by huanglf160920
...
#         ima262            LIKE ima_file.ima262,  ###GP5.2  #NO.FUN-A20044
  ...
  #MOD-B10225---add---end---
   ...
   WHILE TRUE
      INPUT BY NAME old_part,old_sfa08,old_sfa012,old_sfa013,sfa26 WITHOUT DEFAULTS  #FUN-A50066
...
#str---add by huanglf160920
         ...
         #MOD-C80093 end add-----
          ...
          LET sfa26 = l_sfa.sfa26
          DISPLAY BY NAME sfa26,old_qty,l_sfa.sfa12
...
#FUN-A50066--end--add---------------------------
     ...
     #MOD-C80093 end add-----
      ...
      LET left_qty = l_sfa.sfa05
      LET old_qty = l_sfa.sfa05 - l_sfa.sfa06
      LET l_sfa.sfa26 = sfa26 #add by huanglf160920
...
#FUN-A50066--end--mark-------------------
   ...
   CASE
      ...
      OTHERWISE                 #一般工單展單階
          ...
          ELSE
             LET l_btflg = 'Y'
          END IF  #add by huanglf161012
          CALL cs_cralc(g_sfb.sfb01,g_sfb.sfb02,g_sfb.sfb05,l_btflg,
...
FUNCTION i301_tmp2()
  ...
  DROP TABLE i301_temp                                                                                                             
   ...
   CREATE TEMP TABLE i301_tmp2(                                                                                                      
               ...
               new_rate  DECIMAL(15,3),
               new_yes   VARCHAR(1),
               new_qty   DECIMAL(15,3),
               new_qty3  DECIMAL(15,3),
               avl_stk   DECIMAL(15,3),
               new_qty4  DECIMAL(15,3),
               new_qty5  DECIMAL(15,3),
               new_qty6  DECIMAL(15,3),
               new_qty7  DECIMAL(15,3),
               new_qty8  DECIMAL(15,3))     
...
DEFINE l_wc string
    ...
    MENU "" 
       ...
       ON ACTION print_aec
          LET l_wc = ' sfb01="',g_sfb.sfb01 CLIPPED,'"'
          LET g_msg = "csfr005", 
                      " '",g_today CLIPPED,"' ''",
                      " '",g_lang CLIPPED,"' 'Y' '' '1'",  
                      " '",l_wc CLIPPED,"' '2'"
...
DEFINE l_imaud10      LIKE ima_file.imaud10
      ...
      IF g_sfb.sfb02='5' THEN 
        LET l_imaud10=0
      ELSE 
         SELECT imaud10 INTO l_imaud10  FROM ima_file WHERE ima01 =g_sfb.sfb05 
      END IF 

     
      IF cl_null(l_imaud10) OR l_imaud10 = 0 THEN 
         LET g_sfb.sfbud07 = g_sfb.sfb08
      ELSE 
         LET g_sfb.sfbud07 = g_sfb.sfb08/l_imaud10
      END IF 
     
      DISPLAY BY NAME g_sfb.sfbud07
END FUNCTION 
#end------add by guanyao160715
...
FUNCTION i301_upd_runcard()
DEFINE l_sql       STRING 
DEFINE l_shm_1     DYNAMIC ARRAY OF RECORD
...
DEFINE l_ac        LIKE type_file.num5
   ...
   LET l_sql=" SELECT 'N',shm01,shm05,ima02,ima021,shm06,shm08,ta_shm01,shm09 ",
             " FROM shm_file LEFT JOIN ima_file ON shm05=ima01 ",
             " WHERE shm012='",g_sfb.sfb01,"'"
   PREPARE l_pre14 FROM l_sql
   DECLARE l_cur14 CURSOR WITH HOLD FOR l_pre14

   LET l_i=1
   LET l_rec_b=0
   FOREACH l_cur14 INTO l_shm_1[l_i].*
      IF SQLCA.sqlcode THEN
   ...
   LET l_rec_b=l_i
   CALL l_shm_1.deleteElement(l_i)

   OPEN WINDOW csfi301_1_w AT 4,3 WITH FORM "csf/42f/csfi301_1"
         ATTRIBUTE (STYLE = g_win_style CLIPPED)
 
   CALL cl_ui_locale("csfi301_1")

   INPUT ARRAY l_shm_1 WITHOUT DEFAULTS FROM s_shm_1.*
       ...
       ON ACTION select_all
          FOR l_ac=1 TO l_shm_1.getLength()
             LET l_shm_1[l_ac].check1='Y'
          END FOR 

       ON ACTION unselect_all
          FOR l_ac=1 TO l_shm_1.getLength()
             LET l_shm_1[l_ac].check1='N'
          END FOR
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0
      CLOSE WINDOW csfi301_1_w
      RETURN
   ELSE 
      CALL s_showmsg_init()
      CALL i301_upd_run(l_shm_1)
      CALL s_showmsg()
   END IF

   CALL cl_err('','csf-317',1)
   
   CLOSE WINDOW csfi301_1_w

END FUNCTION 

FUNCTION i301_upd_run(l_shm)
...
DEFINE l_count2     LIKE type_file.num5
   ...
   LET l_cnt=l_shm.getLength()
   FOR l_i=1 TO l_cnt
      IF l_shm[l_i].check1='Y' THEN 
         SELECT COUNT(*) INTO l_count1 FROM tc_shc_file WHERE tc_shc03=l_shm[l_i].shm01
         SELECT COUNT(*) INTO l_count2 FROM tc_shb_file WHERE tc_shb03=l_shm[l_i].shm01
         IF l_count1=0 AND l_count2=0 THEN  #没有报工资料，直接删除lot单身，重新生成
            CALL i301_ins_sgm(l_shm[l_i].shm01)
         ELSE 
            CALL i301_upd_sgm(l_shm[l_i].shm01)
         END IF 
      END IF 
   END FOR  
END FUNCTION 

FUNCTION i301_ins_sgm(l_shm01)
...
DEFINE l_i     LIKE type_file.num5
   ...
   SELECT * INTO l_shm.* FROM shm_file WHERE shm01=l_shm01

   LET l_count=0
   SELECT COUNT(*) INTO l_count FROM sgm_file WHERE sgm01=l_shm01
   IF l_count>0 THEN 
      DELETE FROM sgm_file WHERE sgm01=l_shm01
      IF STATUS THEN
         LET g_success='N'
         CALL s_errmsg('sgm01',l_sgm.sgm01,'del sgm',STATUS,1)
      END IF
   END IF 

   LET l_i=1
   LET l_sql=" SELECT * FROM ecm_file where ecm01='",l_shm.shm012,"' ORDER BY ecm03 ASC"
   ...
   FOREACH l_cur22 INTO l_ecm.*
      ...
      LET l_sgm.sgm01=l_shm.shm01
      LET l_sgm.sgm02=l_shm.shm012
      LET l_sgm.sgm03_par=l_shm.shm05
      ...
      IF l_i=1 THEN 
         LET l_sgm.sgm301=l_shm.shm08
      ELSE 
         LET l_sgm.sgm301=0
      END IF 
      LET l_sgm.sgm302=l_ecm.ecm302
      LET l_sgm.sgm303=l_ecm.ecm303
      LET l_sgm.sgm304=0
      LET l_sgm.sgm311=0
      LET l_sgm.sgm312=0
      LET l_sgm.sgm313=0
      ...
      #tianry add 170221
      SELECT ta_ecd05 INTO l_sgm.ta_sgm06 FROM ecd_file WHERE ecd01=l_sgm.sgm04
...
DEFINE l_sgm315   LIKE sgm_file.sgm315
   ...
   SELECT MAX(tc_shc06) INTO l_tc_shc06 FROM tc_shc_file WHERE tc_shc03=l_shm01
   SELECT MAX(tc_shb06) INTO l_tc_shb06 FROM tc_shb_file WHERE tc_shb03=l_shm01
   IF cl_null(l_tc_shc06) THEN LET l_tc_shc06 = 0 END IF 
   IF cl_null(l_tc_shb06) THEN LET l_tc_shb06 = 0 END IF 
   IF l_tc_shc06>l_tc_shb06 THEN 
      LET l_ecm03=l_tc_shc06
   ELSE 
      LET l_ecm03=l_tc_shb06
   END IF 
   ...
   LET l_count=0
   SELECT COUNT(*) INTO l_count FROM sgm_file WHERE sgm01=l_shm01 AND sgm03>l_ecm03
   IF l_count>0 THEN 
      DELETE FROM sgm_file WHERE sgm01=l_shm01 AND sgm03>l_ecm03
      IF STATUS THEN
         LET g_success='N'
         CALL s_errmsg('sgm01',l_sgm.sgm01,'del sgm',STATUS,1)
      END IF
   ...
   LET l_sgm311=0
   LET l_sgm315=0
   SELECT sgm311,sgm315 INTO l_sgm311,l_sgm315 FROM sgm_file WHERE sgm01=l_shm.shm01 AND sgm03=l_ecm03
   
   LET l_sql=" SELECT * FROM ecm_file where ecm01='",l_shm.shm012,"' AND ecm03>",l_ecm03," ORDER BY ecm03 ASC "
   PREPARE l_pre223 FROM l_sql
   DECLARE l_cur221 CURSOR FOR l_pre223
   FOREACH l_cur221 INTO l_ecm.*
      IF SQLCA.sqlcode THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
      ...
      LET l_sgm.sgm292=l_ecm.ecm292
      IF l_i=1 THEN 
         LET l_sgm.sgm301=l_sgm311+l_sgm315
      ELSE  
         LET l_sgm.sgm301=0
      END IF 
      LET l_sgm.sgm302=l_ecm.ecm302
...
FUNCTION i301_tc_data_pro()  
   DEFINE l_num       LIKE tc_imr_file.tc_imr06
   DEFINE l_sql       STRING
   DEFINE l_sfb       RECORD 
          sfb01     LIKE sfb_file.sfb01,
          sfb05     LIKE sfb_file.sfb05,
          ...
          sfa161    LIKE sfa_file.sfa161   
          END RECORD
   DEFINE l_tc_imo    RECORD LIKE tc_imo_file.*

   DELETE FROM tc_imo_file;
   DELETE FROM tc_imr_file;
   
   LET l_sql="select sfb01,sfb05,sfb08,sfb081,sfb09,sfa03,sfa08,sfa05,sfa06,tc_shb12,tc_shb12*sfa161 as sfa05_1 ,tc_shb12*sfa161-sfa06 as sfa05_2,sfa161 from sfb_file,sfa_file left outer join ",
             "(select tc_shb04,tc_shb08,sum(tc_shb12) as tc_shb12 from tc_shb_file,ecd_file where tc_shb01=1 and tc_shb08=ecd01 and ta_ecd04='Y' group by tc_shb04,tc_shb08 ",
             " union all ",
             " select tc_shc04,tc_shc08,sum(tc_shc12) from tc_shc_file,ecd_file where tc_shc01=1 and tc_shc08=ecd01 and ta_ecd04='N' group by tc_shc04,tc_shc08 )",
             " on sfa08=tc_shb08 and sfa01=tc_shb04 ",
             " where sfb01=sfa01 AND tc_shb12*sfa161-sfa06 > 0 AND sfb01='",g_sfb.sfb01 CLIPPED,"'"
   PREPARE l_data_pre FROM l_sql
   DECLARE l_data_cur CURSOR FOR l_data_pre           
   FOREACH l_data_cur INTO l_sfb.*
       LET l_tc_imo.tc_imo02=l_sfb.sfb01     #工单单号
       LET l_tc_imo.tc_imo03=l_sfb.sfa08     #作业编号
       LET l_tc_imo.tc_imo04=l_sfb.sfb08     #应发套数
       LET l_tc_imo.tc_imo05=l_sfb.sfb081    #已发套数
       LET l_tc_imo.tc_imo06=l_sfb.sfa03     #发料料号
       LET l_tc_imo.tc_imo07=l_sfb.sfa05     #应发数量
       LET l_tc_imo.tc_imo08=l_sfb.sfa06     #已发数量
       LET l_tc_imo.tc_imo09=l_sfb.sfa161    #实际QPA
       LET l_tc_imo.tc_imo10=l_sfb.tc_shb12  #完工量
       LET l_tc_imo.tc_imo11=l_sfb.sfa05_1   #完工对应应发量
       LET l_tc_imo.tc_imo12=l_sfb.sfa05_2   #本次需补数量

       IF l_tc_imo.tc_imo05 * l_tc_imo.tc_imo09 - l_tc_imo.tc_imo07 >= l_tc_imo.tc_imo12 THEN
          LET l_tc_imo.tc_imo13 = l_tc_imo.tc_imo12  #本次欠料补料量
          LET l_tc_imo.tc_imo14 = 0                  #本次成套发料量
          LET l_tc_imo.tc_imo15 = 0                  #本次成套发料量
       ELSE 
          LET l_tc_imo.tc_imo13 = l_tc_imo.tc_imo05 * l_tc_imo.tc_imo09 - l_tc_imo.tc_imo07
          LET l_tc_imo.tc_imo14 = l_tc_imo.tc_imo12 - l_tc_imo.tc_imo13
          LET l_tc_imo.tc_imo15 = l_tc_imo.tc_imo14/l_tc_imo.tc_imo09
       END IF
       INSERT INTO tc_imo_file VALUES(l_tc_imo.*)
   END FOREACH 
      
   #CALL i301_tc_old_pro()
   MESSAGE '执行完成!'
END FUNCTION


{
FUNCTION i301_tc_old_pro()
   DEFINE l_sfa01     LIKE sfa_file.sfa01  
   DEFINE l_rec       RECORD LIKE tc_imr_file.*
   DEFINE l_sfa03     LIKE sfa_file.sfa03
   DEFINE l_sfa161    LIKE sfa_file.sfa161
   DEFINE l_tc_imo    RECORD LIKE tc_imo_file.*
   DEFINE l_sql       STRING

   LET l_sql=" SELECT DISTINCT tc_imr02 FROM tc_imr_file"
   PREPARE l_old_pre2 FROM l_sql
   DECLARE l_old_cur2 CURSOR FOR l_old_pre2
   FOREACH l_old_cur2 INTO l_sfa01   
     LET l_sql=" SELECT * FROM tc_imr_file where tc_imr02='",l_sfa01,"'"
     PREPARE l_old_pre FROM l_sql
     DECLARE l_old_cur CURSOR FOR l_old_pre
     FOREACH l_old_cur INTO l_rec.*
        LET l_sql=" SELECT sfa03,sfa161 FROM sfa_file where sfa01='",l_rec.tc_imr02,"' AND sfa08='",l_rec.tc_imr04,"'"
        PREPARE l_old_pre1 FROM l_sql
        DECLARE l_old_cur1 CURSOR FOR l_old_pre1
        FOREACH l_old_cur1 INTO l_sfa03,l_sfa161
           LET l_tc_imo.tc_imo02 = l_rec.tc_imr02             #工单单号
           LET l_tc_imo.tc_imo03 = l_rec.tc_imr04             #作业编号
           LET l_tc_imo.tc_imo04 = l_rec.tc_imr06             #应发套数
           LET l_tc_imo.tc_imo06 = l_sfa03                    #应发料号
           LET l_tc_imo.tc_imo07 = l_rec.tc_imr06 * l_sfa161  #应发数量
           SELECT MAX(tc_imo01) INTO l_tc_imo.tc_imo01 FROM tc_imo_file
           IF cl_null(l_tc_imo.tc_imo01) OR l_tc_imo.tc_imo01 = 0 THEN 
              LET l_tc_imo.tc_imo01 = 0
           END IF
           LET l_tc_imo.tc_imo01 = l_tc_imo.tc_imo01 + 1  #序号
 
           SELECT sfb081 INTO l_tc_imo.tc_imo05 FROM sfb_file WHERE sfb01=l_rec.tc_imr02
           SELECT sfa06,sfa161 INTO l_tc_imo.tc_imo08,l_tc_imo.tc_imo09
           FROM sfa_file WHERE sfa01=l_rec.tc_imr02 AND sfa03=l_sfa03
           LET l_tc_imo.tc_imo10 = l_tc_imo.tc_imo07 - l_tc_imo.tc_imo08 - (l_tc_imo.tc_imo04- l_tc_imo.tc_imo05)*l_tc_imo.tc_imo09
...
FUNCTION i301_tc_old_ecd_sel(p_sfa01,p_eca01,p_num)
  DEFINE p_sfa01     LIKE sfa_file.sfa01
  DEFINE p_eca01     LIKE eca_file.eca01
  DEFINE p_num       LIKE tc_imr_file.tc_imr06
  DEFINE l_ecm03     LIKE ecm_file.ecm03
  DEFINE l_tc_imr    RECORD LIKE tc_imr_file.*
  DEFINE l_sql       STRING
  DEFINE l_cnt       LIKE type_file.num5

  LET p_sfa01 = p_sfa01[1,12] #传进来的是LOT，按工单发料，只要前面12位
  SELECT MAX(ecm03) INTO l_ecm03 FROM  ecm_file WHERE ecm01=p_sfa01 AND ecm04 IN 
  (SELECT ecd01 FROM ecd_file WHERE ecd07=p_eca01)

  IF cl_null(l_ecm03) THEN RETURN END IF

  LET l_sql=" SELECT ecm01,ecm03,ecm04 FROM ecm_file where ecm01='",p_sfa01,"' and ecm03<=",l_ecm03
  PREPARE l_ecd_pre FROM l_sql
  DECLARE l_ecd_cur CURSOR FOR l_ecd_pre
  FOREACH l_ecd_cur INTO l_tc_imr.tc_imr02,l_tc_imr.tc_imr03,l_tc_imr.tc_imr04  
     LET l_cnt = 0
     SELECT COUNT(*) INTO l_cnt FROM sfa_file WHERE sfa01=l_tc_imr.tc_imr02 AND sfa08=l_tc_imr.tc_imr04
     IF l_cnt = 0 THEN  CONTINUE FOREACH END IF  #工艺没有发料的不要
     
     LET l_cnt = 0
     SELECT COUNT(*) FROM tc_imr_file WHERE tc_imr02=l_tc_imr.tc_imr02 AND tc_imr03=l_tc_imr.tc_imr03
     IF l_cnt > 0 THEN
        SELECT * INTO l_tc_imr.* FROM tc_imr_file WHERE tc_imr02=l_tc_imr.tc_imr02 AND tc_imr03=l_tc_imr.tc_imr03
        UPDATE tc_imr_fle SET tc_imr06= tc_imr06 + p_num WHERE tc_imr01=l_tc_imr.tc_imr01
     ELSE
       SELECT MAX(tc_imr01) INTO l_tc_imr.tc_imr01 FROM tc_imr_file 
       IF cl_null(l_tc_imr.tc_imr01) OR l_tc_imr.tc_imr01 = 0 THEN 
          LET l_tc_imr.tc_imr01 = 0
       END IF
       LET l_tc_imr.tc_imr01 = l_tc_imr.tc_imr01 + 1     
       LET l_tc_imr.tc_imr05 = p_eca01
       LET l_tc_imr.tc_imr06 = p_num
       INSERT INTO tc_imr_file VALUES (l_tc_imr.*)    
     END IF      
  END FOREACH
END FUNCTION
}

FUNCTION i301_upd_sfbdate(p_sfb01)
   DEFINE    p_sfb01     LIKE sfb_file.sfb01

   UPDATE sfb_file SET sfbdate  = g_today
    WHERE sfb01 = p_sfb01
   
   IF STATUS THEN
      CALL cl_err("upd sfbdate","!",1)
      RETURN FALSE 
   END IF 
   RETURN TRUE
END FUNCTION
