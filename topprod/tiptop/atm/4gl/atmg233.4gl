...
TYPE sr1_t RECORD
    ...
    tqa02e LIKE tqa_file.tqa02,
    tqa02f LIKE tqa_file.tqa02,
    tqa02g LIKE tqa_file.tqa02,
    sum1 LIKE tsb_file.tsb06,
    #FUN-C40036---add---str
...
MAIN
   ...
   LET g_sql ="tqx01.tqx_file.tqx01,",
              ...
              "sum1.tsb_file.tsb06,",       #FUN-C50007 ADD,
              #FUN-C40036---add---str   
   ...
   LET l_table = cl_prt_temptable('atmg233',g_sql) CLIPPED
   IF l_table = -1 THEN 
   ...
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               ...
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?) "            #FUN-C40036 add 4?  #FUN-C50007 add ?
   ...
   IF cl_null(tm.wc) THEN
        CALL atmg233_tm(0,0)             # Input print condition
   ELSE LET tm.wc="tqx01= '",tm.wc CLIPPED,"'"
        CALL atmg233()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690124
   CALL cl_gre_drop_temptable(l_table)
END MAIN
 
FUNCTION atmg233_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW atmg233_w AT p_row,p_col WITH FORM "atm/42f/atmg233"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW atmg233_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690124
      CALL cl_gre_drop_temptable(l_table)
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,        
                         ...
                         " '",g_rep_clas CLIPPED,"'",           
                         " '",g_template CLIPPED,"'"            
         CALL cl_cmdat('atmg233',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW atmg233_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690124
      CALL cl_gre_drop_temptable(l_table)
      EXIT PROGRAM
   END IF
   ...
   DEFINE  l_img_blob     LIKE type_file.blob    #FUN-C40036 add
     ...
     #End:FUN-980030
      ...
      LET l_sql="SELECT UNIQUE tqx01,tqx02,tqx03,tqx04,tqx05,tqx06,tqx07,tqx08,",   
               ...
               "        a.tqa02,b.tqa02,c.tqa02,tqb02,gec02 ",                                  #FUN-C50007 add  
           #   "  FROM tqx_file,tqy_file,tqz_file,tsa_file,OUTER ima_file ",                    #FUN-C50007 MARK
               "  FROM tqx_file LEFT OUTER JOIN tqa_file a ON tqx03=a.tqa01 and a.tqa03='15' ", #FUN-C50007 add
               "  LEFT OUTER JOIN tqa_file b ON tqx04=b.tqa01 and b.tqa03='17' ",               #FUN-C50007 add
               "  LEFT OUTER JOIN tqa_file c ON tqx13=c.tqa01 and c.tqa03='20' ",               #FUN-C50007 add
               "  LEFT OUTER JOIN tqb_file ON tqb01=tqx12 LEFT OUTER JOIN gec_file ON gec01=tqx08 and gec011='2', ", #FUN-C50007 add
               "  tqy_file,tqz_file LEFT OUTER JOIN ima_file ON tqz03=ima01 ,tsa_file ",         #FUN-C50007 add
               " WHERE tqx01=tsa01 ",
               "   AND tqx01=tqy01 ",
               "   AND tqx01=tqz01 ",
               "   AND tqy02=tsa02 ",
               "   AND tqz02=tsa03 ",
           #   "   AND tqz_file.tqz03=ima_file.ima01 ",                    #FUN-C50007 MARK
               "   AND ",tm.wc CLIPPED,
               " ORDER BY tqx01,tsa02,tsa03 "
     PREPARE atmg233_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690124
        CALL cl_gre_drop_temptable(l_table)
        EXIT PROGRAM 
     END IF
     DECLARE atmg233_curs1 CURSOR FOR atmg233_prepare1
...
#    ELSE
#       CALL cl_outnam('atmg233') RETURNING l_name
#    END IF
 
#    START REPORT atmg233_rep TO l_name
#FUN-C50007--ADD---STR--------------------------
     LET l_sql="SELECT tsb03,tsb04,tsb05,tsb06,tsb08,tsb09,tsb10,tsb11,tsb12, ",
                 "  d.tqa02,oaj02,e.tqa02,f.tqa02,g.tqa02 ",
                 "  FROM tsb_file LEFT OUTER JOIN tqa_file d ON tsb04=d.tqa01 and d.tqa03='1'",
                 " LEFT OUTER JOIN oaj_file  ON tsb05=oaj01 ",
                 " LEFT OUTER JOIN tqa_file e ON tsb08=e.tqa01 and e.tqa03='8' ",
                 " LEFT OUTER JOIN tqa_file f ON tsb10=f.tqa01 and f.tqa03='6' ",
                 " LEFT OUTER JOIN tqa_file g ON tsb09=g.tqa01 and g.tqa03='7 '",
                 " WHERE tsb01=? AND tsb03=? "
       PREPARE atmg233_prepare2 FROM l_sql
       IF STATUS THEN CALL cl_err('prepare:',STATUS,1)
          CALL cl_used(g_prog,g_time,2) RETURNING g_time 
          CALL cl_gre_drop_temptable(l_table)
          EXIT PROGRAM
       END IF
       DECLARE atmg233_curs2 CURSOR FOR atmg233_prepare2 
#FUN-C50007--ADD--END---------------------------


#    LET g_pageno = 0
#    FOREACH atmg233_curs1 INTO sr.*                                             #FUN-C50007  MARK
     FOREACH atmg233_curs1 INTO sr.*,l_tqa02,l_tqa02a,l_tqa02b,l_tqb02,l_gec02   #FUN-C50007 add
      ...
      #FUN-C50007--MARK--END
       LET l_flag=0
      #FOREACH atmg233_curs2 INTO sr2.*                            #FUN-C50007 MARK
       FOREACH atmg233_curs2 USING sr.tqx01,sr.tsa02 INTO sr2.*,l_tqa02d,l_oaj02,l_tqa02e,l_tqa02f,l_tqa02g  #FUN-C50007 ADD
       IF STATUS THEN CALL cl_err('declare:',STATUS,1) RETURN END IF
       LET l_flag=1
      #FUN-C500007--MARK--STR
      ...
      #FUN-C50007---MAEK--END
#      OUTPUT TO REPORT atmg233_rep(sr.*)
...
###GENGRE###     CALL cl_prt_cs3(g_prog,'atmg233',l_sql,l_str) 

    LET g_cr_table = l_table                   #主報表的temp table名稱         #FUN-C40036 add
    LET g_cr_apr_key_f = "tqx01"       #報表主鍵欄位名稱，用"|"隔開  #FUN-C40036 add
    CALL atmg233_grdata()    ###GENGRE###
     #No.FUN-710082--end  
END FUNCTION
 
#No.FUN-710082--begin
#REPORT atmg233_rep(sr)
...
#        END IF
#        DECLARE atmg233_curs2 CURSOR FOR atmg233_prepare2
#        FOREACH atmg233_curs2 INTO sr2.*
#        IF STATUS THEN CALL cl_err('declare:',STATUS,1) RETURN END IF
#        SELECT tqa02 INTO l_tqa02d FROM tqa_file WHERE tqa01=sr2.tsb04 AND tqa03='1'
#        SELECT oaj02 INTO l_oaj02  FROM oaj_file WHERE oaj01=sr2.tsb05
#        SELECT tqa02 INTO l_tqa02e FROM tqa_file WHERE tqa01=sr2.tsb08 AND tqa03='8'
...
FUNCTION atmg233_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("atmg233")
        IF handler IS NOT NULL THEN
            START REPORT atmg233_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY tqx01,tsa02,tsa03"
          
            DECLARE atmg233_datacur1 CURSOR FROM l_sql
            FOREACH atmg233_datacur1 INTO sr1.*
                OUTPUT TO REPORT atmg233_rep(sr1.*)
            END FOREACH
            FINISH REPORT atmg233_rep
        END IF
...
REPORT atmg233_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B50018----add-----str-----------------
    DEFINE l_tqx03_tqa02   STRING
    DEFINE l_tqx04_tqa02a  STRING
    DEFINE l_tqx08_gec02   STRING
    DEFINE l_tqx12_tqb02   STRING
    DEFINE l_tqx13_tqa02b  STRING
    DEFINE l_tqy03_occ02   STRING
    DEFINE l_tqy05_too02   STRING
    DEFINE l_tqy06_top02   STRING
    DEFINE l_tqy36_azp02   STRING
    DEFINE l_tqz03_tqz18   STRING
    DEFINE l_tqz08_tsa04   STRING
    DEFINE l_tsa04         STRING
    DEFINE l_tqz10_tqa02c  STRING
    DEFINE l_sql           STRING
    DEFINE l_tsa05_sum     LIKE tsa_file.tsa05
    DEFINE l_tsa08_sum     LIKE tsa_file.tsa08
    DEFINE l_tsa05_01_sum  LIKE tsa_file.tsa05
    DEFINE l_tsa08_01_sum  LIKE tsa_file.tsa08
    #FUN-B50018----add-----end-----------------

    
    ORDER EXTERNAL BY sr1.tqx01,sr1.tsa02,sr1.tsa03
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.tqx01

            
            #FUN-B50018----add-----str-----------------
            LET  l_tqx03_tqa02 = sr1.tqx03,' ',sr1.tqa02
            PRINTX  l_tqx03_tqa02


            LET  l_tqx04_tqa02a = sr1.tqx04,' ',sr1.tqa02a
            PRINTX  l_tqx04_tqa02a

            LET  l_tqx08_gec02 = sr1.tqx08,' ',sr1.gec02
            PRINTX  l_tqx08_gec02


            LET  l_tqx12_tqb02 = sr1.tqx12,' ',sr1.tqb02
            PRINTX  l_tqx12_tqb02

            LET  l_tqx13_tqa02b = sr1.tqx13,' ',sr1.tqa02b
            PRINTX  l_tqx13_tqa02b



            LET  l_tsa04 = sr1.tsa04   USING '###,##&.&&&'

            #FUN-B50018----add-----end-----------------
            LET l_lineno = 0
        BEFORE GROUP OF sr1.tsa02 

            LET  l_tqy03_occ02 = sr1.tqy03,' ',sr1.occ02
            PRINTX  l_tqy03_occ02

            LET  l_tqy05_too02 = sr1.tqy05,' ',sr1.too02
            PRINTX  l_tqy05_too02

            LET  l_tqy06_top02 = sr1.tqy06,' ',sr1.top02
            PRINTX  l_tqy06_top02

            LET  l_tqy36_azp02 = sr1.tqy36,' ',sr1.azp02
            PRINTX  l_tqy36_azp02 
        BEFORE GROUP OF sr1.tsa03     
        ...
        ON EVERY ROW
            ...
            #FUN-B50018----add-----str-----------------
            LET  l_tqz03_tqz18 = sr1.tqz03,' ',sr1.tqz18
            PRINTX  l_tqz03_tqz18

            LET  l_tqz08_tsa04 = sr1.tqz08,'/',l_tsa04
            PRINTX  l_tqz08_tsa04

            LET  l_tqz10_tqa02c = sr1.tqz10,' ',sr1.tqa02c
            PRINTX  l_tqz10_tqa02c
            #FUN-B50018----add-----end-----------------

            PRINTX sr1.*

        AFTER GROUP OF sr1.tqx01

            #FUN-B50018----add-----str-----------------
            LET l_tsa05_sum = GROUP SUM(sr1.tsa05)
            LET l_tsa08_sum = GROUP SUM(sr1.tsa08)
            PRINTX l_tsa05_sum
            PRINTX l_tsa08_sum 
            #FUN-B50018----add-----end-----------------
        AFTER GROUP OF sr1.tsa02
            #FUN-B50018----add-----str-----------------
            LET l_tsa05_01_sum = GROUP SUM(sr1.tsa05)
            LET l_tsa08_01_sum = GROUP SUM(sr1.tsa08)
            PRINTX l_tsa05_01_sum
            PRINTX l_tsa08_01_sum

            
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " WHERE tqx01 = '",sr1.tqx01 CLIPPED,"' AND tsb03 = ",sr1.tsa02,
                        " AND tsb03 = tsa02 "
            START REPORT atmg233_subrep01
            DECLARE atmg233_subrep01 CURSOR FROM l_sql
            FOREACH atmg233_subrep01 INTO sr1.*
                OUTPUT TO REPORT atmg233_subrep01 (sr1.*)
            END FOREACH
            FINISH REPORT atmg233_subrep01
            #FUN-B50018----add-----end-----------------
        AFTER GROUP OF sr1.tsa03
        ON LAST ROW

END REPORT


REPORT atmg233_subrep01(sr1)
    DEFINE sr1 sr1_t
    DEFINE sr1_t sr1_t
    DEFINE l_tsb04_tqa02d  STRING
    DEFINE l_tsb05_oaj02   STRING
    DEFINE l_tsb08_tqa02e  STRING
    DEFINE l_tsb09_tqa02g  STRING
    DEFINE l_tsb10_tqa02f  STRING
    DEFINE l_tsb06_sum     LIKE tsb_file.tsb06 
    DEFINE l_display       LIKE type_file.chr1 

    ORDER EXTERNAL BY sr1.tqx01,sr1.tsa02,sr1.tsb04
    FORMAT

       BEFORE GROUP OF sr1.tqx01
       BEFORE GROUP OF sr1.tsa02
       BEFORE GROUP OF sr1.tsb04
           IF sr1_t.tsb03 = sr1.tsb03 THEN
              LET l_display = 'N'
           ELSE
              LET l_display = 'Y'
           END IF
       ...
       AFTER GROUP OF sr1.tqx01
            ...
            LET l_tsb06_sum = GROUP SUM(sr1.tsb06)
            PRINTX l_tsb06_sum
       AFTER GROUP OF sr1.tsa02
            
       AFTER GROUP OF sr1.tsb04
            LET  l_tsb04_tqa02d = sr1.tsb04,' ',sr1.tqa02d
            PRINTX  l_tsb04_tqa02d

            LET  l_tsb05_oaj02 = sr1.tsb05,' ',sr1.oaj02
            PRINTX  l_tsb05_oaj02

            LET  l_tsb08_tqa02e = sr1.tsb08,' ',sr1.tqa02e
            PRINTX  l_tsb08_tqa02e

            LET  l_tsb09_tqa02g = sr1.tsb09,' ',sr1.tqa02g
            PRINTX  l_tsb09_tqa02g

            LET  l_tsb10_tqa02f = sr1.tsb10,' ',sr1.tqa02f
            PRINTX  l_tsb10_tqa02f
END REPORT
###GENGRE###END
#FUN-B80061
