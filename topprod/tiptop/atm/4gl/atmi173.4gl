...
MAIN
    ...
    OPEN WINDOW i173_w WITH FORM "atm/42f/atmi173"
        ATTRIBUTE(STYLE = g_win_style CLIPPED)
    CALL cl_ui_init() 
 
    CALL g_x.clear()
    LET g_wc2 = '1=1' CALL i173_b_fill(g_wc2)
 
    CALL i173_menu()  
    CLOSE WINDOW i173_w                 #結束畫面
      ...
      CASE g_action_choice                                                      
         ...
         WHEN "output"                                                          
            IF cl_chk_act_auth() THEN
               ...
               LET l_cmd='p_query "atmi173" "',g_wc2 CLIPPED,'"'  #No.FUN-780056
               CALL cl_cmdrun(l_cmd)                              #No.FUN-780056                                           
               #CALL i173_out()                                   #No.FUN-780056                                        
            END IF                                                              
         WHEN "help"                                                            
            CALL cl_show_help()                                                    
         WHEN "exit"                                                            
...
DEFINE
    ...
    LET g_forupd_sql = "SELECT obq01,obq02,obqacti FROM obq_file ",
                       " WHERE obq01=? FOR UPDATE "
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    DECLARE i173_bcl CURSOR FROM g_forupd_sql     # LOCK CURSOR
...
#No.FUN-570109 --start                                                                                                              
           ...
           CALL i173_set_entry(p_cmd)                                                                                               
           CALL i173_set_no_entry(p_cmd)                                                                                            
           LET g_before_input_done = TRUE                                                                                           
#No.FUN-570109 --end     
           OPEN i173_bcl USING g_obq_t.obq01              #表示更改狀態
           IF STATUS THEN                
              CALL cl_err("OPEN i173_bcl:", STATUS, 1)         
              LET l_lock_sw = "Y"                      
           ELSE 
              FETCH i173_bcl INTO g_obq[l_ac].* 
...
#No.FUN-570109 --end     
    ...
    AFTER INSERT
           IF INT_FLAG THEN                       
              ...
              CANCEL INSERT
           END IF
           INSERT INTO obq_file(obq01,obq02,obqacti,obquser,obqdate,obqoriu,obqorig) 
           VALUES(g_obq[l_ac].obq01,g_obq[l_ac].obq02, 
                  g_obq[l_ac].obqacti,g_user,g_today, g_user, g_grup)        #No.FUN-980030 10/01/04  insert columns oriu, orig
           IF SQLCA.sqlcode THEN                                     
#             CALL cl_err(g_obq[l_ac].obq01,SQLCA.sqlcode,0)           #No.FUN-660104
              CALL cl_err3("ins","obq_file",g_obq[l_ac].obq01,"",
...
{ckp#1}        DELETE FROM obq_file WHERE obq01 = g_obq_t.obq01
               IF SQLCA.sqlcode THEN
#                  CALL cl_err(g_obq_t.obq01,SQLCA.sqlcode,0)  #No.FUN-660104
                   CALL cl_err3("del","obq_file",g_obq_t.obq01,"",
                                 SQLCA.sqlcode,"","",1)  #No.FUN-660104
                   EXIT INPUT
               END IF
               LET g_rec_b=g_rec_b-1
...
#               WHERE CURRENT OF i173_bcl
              IF SQLCA.sqlcode THEN
#                CALL cl_err(g_obq[l_ac].obq01,SQLCA.sqlcode,0)  #No.FUN-660104
                 CALL cl_err3("upd","obq_file",g_obq[l_ac].obq01,"",
                               SQLCA.sqlcode,"","",1)  #No.FUN-660104
                 LET g_obq[l_ac].* = g_obq_t.*
              ELSE
                 MESSAGE 'UPDATE O.K'
...
#    IF INT_FLAG THEN LET INT_FLAG = 0 RETURN END IF
   ...
   END IF
#No.TQC-710076 -- end --
    CALL i173_b_fill(g_wc2)
END FUNCTION
 
FUNCTION i173_b_fill(p_wc2)              #BODY FILL UP
DEFINE
...
LET g_sql = "SELECT obq01,obq02,obqacti,''",                                
                ...
                " WHERE ", p_wc2 CLIPPED,                     #             
                " ORDER BY obq01"
    PREPARE i173_pb FROM g_sql
    DECLARE obq_curs CURSOR FOR i173_pb
...
FUNCTION i173_out()
    ...
    LET g_sql="SELECT * FROM obq_file ",          # 組合出 SQL 指令
              " WHERE ",g_wc2 CLIPPED
    PREPARE i173_p1 FROM g_sql                # RUNTIME 編譯
    DECLARE i173_co                         # SCROLL CURSOR
         CURSOR FOR i173_p1
      
    CALL cl_outnam('atmi173') RETURNING l_name
    START REPORT i173_rep TO l_name
      
    FOREACH i173_co INTO sr.*
        IF SQLCA.sqlcode THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
        END IF
        OUTPUT TO REPORT i173_rep(sr.*)
    END FOREACH
      
    FINISH REPORT i173_rep
...
FUNCTION i173_set_no_entry(p_cmd)                                                                                                   
  DEFINE p_cmd   LIKE type_file.chr1             #No.FUN-680120 VARCHAR(1)
