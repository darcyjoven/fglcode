...
# Modify.........: No.FUN-6B0043 06/11/14 By jamie 1.FUNCTION _fetch() 一開始應清空key值
#                                                  2.新增action"相關文件"
...
DEFINE
       ...
       g_t1        LIKE oay_file.oayslip,           #No.FUN-550026        #No.FUN-680120 VARCHAR(5)
       g_sw        LIKE type_file.chr1,             #No.FUN-680120 VARCHAR(1)
       g_buf       LIKE bnb_file.bnb06,             #No.FUN-680120 VARCHAR(20)
...
DEFINE  p_row,p_col     LIKE type_file.num5     #No.FUN-680120 SMALLINT
    ...
    OPEN WINDOW t102_w AT p_row,p_col WITH FORM "atm/42f/atmt102"
...
#FUN-AA0059---------mod------------end-----------------
    ...
    LET g_sql = "SELECT adg01,adg02 ",
                ...
                "   AND adf01 = adg01 ",
                "  INTO TEMP x "
    PREPARE t102_precount_x FROM g_sql
    EXECUTE t102_precount_x
 
        LET g_sql="SELECT COUNT(*) FROM x "
 
    PREPARE t102_precount FROM g_sql
    DECLARE t102_count CURSOR FOR t102_precount
END FUNCTION
...
FUNCTION t102_q()
    ...
    IF INT_FLAG THEN 
       LET INT_FLAG = 0 
       INITIALIZE g_adg.* TO NULL RETURN 
    END IF
    MESSAGE " SEARCHING ! "
    ...
    ELSE
        ...
        CALL t102_fetch('F')                  # 讀出TEMP第一筆并顯示
    END IF
    MESSAGE ""
END FUNCTION
...
DEFINE
    ...
    CASE p_flag
        WHEN 'N' FETCH NEXT     t102_cs INTO g_adg.adg01,g_adg.adg02
        WHEN 'P' FETCH PREVIOUS t102_cs INTO g_adg.adg01,g_adg.adg02
        WHEN 'F' FETCH FIRST    t102_cs INTO g_adg.adg01,g_adg.adg02
        WHEN 'L' FETCH LAST     t102_cs INTO g_adg.adg01,g_adg.adg02
        WHEN '/'
...
#                      CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
            ...
            END IF
            LET mi_no_ask = FALSE
            FETCH ABSOLUTE g_jump t102_cs INTO g_adg.adg01,g_adg.adg02
    END CASE
 
    IF SQLCA.sqlcode THEN
        CALL cl_err(g_adg.adg01,SQLCA.sqlcode,0)
        INITIALIZE g_adg.* TO NULL              #No.FUN-6B0043
        RETURN
    ELSE
       CASE p_flag
    ...
    SELECT adf00,adg01,adg02,adg09,adg05,adf03,adf05 INTO g_adg.*
      FROM adf_file,adg_file
     WHERE adg_file.adg01 = g_adg.adg01 AND adg_file.adg02 = g_adg.adg02
       AND adf01 = adg01
    IF SQLCA.sqlcode THEN
    #   CALL cl_err(g_adg.adg01,SQLCA.sqlcode,0)      #No.FUN-660104
        CALL cl_err3("sel","adf_file,adg_file",g_adg.adg01,"",SQLCA.sqlcode,"","",1)      #No.FUN-660104
        INITIALIZE g_adg.* TO NULL
        RETURN
    END IF
...
DEFINE l_i      LIKE type_file.num10,            #No.FUN-680120 INTEGER
       ...
       FOREACH obo_cur INTO l_obo.*
          ...
          INSERT INTO oze_file(oze01,oze02,oze03,oze04,oze07,oze08,
                               ozeplant,ozelegal) #FUN-980009
          VALUES(g_adg.adg01,g_adg.adg02,l_i,
                 l_obo.obo04,l_obo.obo05,l_obo.obo06,
                 g_plant,g_legal) #FUN-980009
          IF SQLCA.sqlcode THEN
          #  CALL cl_err('insert oze_file',SQLCA.sqlcode,0)    #No.FUN-660104
             CALL cl_err3("ins","oze_file",g_adg.adg01,g_adg.adg02,SQLCA.sqlcode,"","insert oze_file",1)    #No.FUN-660104
             EXIT FOREACH
          END IF
          LET l_i = l_i + 1
       END FOREACH
...
FUNCTION t102_b_fill()              #BODY FILL UP
    ...
    DECLARE oze_curs CURSOR FOR t102_pb
...
DEFINE
    ...
    CALL t102_g_b()
    DECLARE t102_za_cur CURSOR FOR
            SELECT za02,za05 FROM za_file
             WHERE za01 = "atmt102" AND za03 = g_lang
    FOREACH t102_za_cur INTO g_i,l_za05
       LET g_x[g_i] = l_za05
    END FOREACH
    ...
    INPUT ARRAY g_oze WITHOUT DEFAULTS FROM s_oze.*
        ...
        BEFORE ROW
            ...
            IF g_rec_b >= l_ac THEN
                ...
                OPEN t102_bcl USING g_adg.adg01,g_adg.adg02,g_oze_t.oze03
                IF STATUS THEN
                   CALL cl_err("OPEN t102_bcl:", STATUS, 1)
                   LET l_lock_sw = "Y"
                ELSE
                   FETCH t102_bcl INTO g_oze[l_ac].*
                   IF SQLCA.sqlcode THEN
                      CALL cl_err("FETCH t102_bcl",SQLCA.sqlcode,1)
                      LET l_lock_sw = "Y"
                   END IF
                END IF
        ...
        AFTER INSERT
           ...
           INSERT INTO oze_file(oze01,oze02,oze03,oze04,oze05,oze06,       #TQC-5B0194
            ...
            VALUES(g_adg.adg01,g_adg.adg02,g_oze[l_ac].oze03,
                   ...
                   g_plant,g_legal) #FUN-980009
           IF SQLCA.sqlcode THEN
           #   CALL cl_err("INSERT oze_file",SQLCA.sqlcode,1)    #No.FUN-660104
               CALL cl_err3("ins","oze_file",g_oze[l_ac].oze03,"",SQLCA.sqlcode,"","INSERT oze_file",1)    #No.FUN-660104
               CANCEL INSERT
           ELSE
               MESSAGE 'INSERT O.K'
        ...
        BEFORE DELETE                            #是否取消單身
            IF g_oze_t.oze03 > 0 AND
               ...
               #  CALL cl_err("DELETE oze_file",SQLCA.sqlcode,0)   #No.FUN-660104
                  CALL cl_err3("del","oze_file",g_oze_t.oze03,"",SQLCA.sqlcode,"","",1)   #No.FUN-660104
                  ROLLBACK WORK
                  CANCEL DELETE
               END IF
               LET g_rec_b=g_rec_b-1
     ...
     ON ROW CHANGE
            ...
            ELSE
               ...
               #  CALL cl_err("UPDATE oze_file",SQLCA.sqlcode,1)   #No.FUN-660104
                  CALL cl_err3("upd","oze_file",g_oze[l_ac].oze03,"",SQLCA.sqlcode,"","",1)   #No.FUN-660104
                  LET g_oze[l_ac].* = g_oze_t.*
                  CLOSE t102_bcl
                  ROLLBACK WORK
               ELSE
