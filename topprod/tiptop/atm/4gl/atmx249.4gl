...
# Modify.........: No.FUN-D40129 13/05/22 By yangtt oea23,oeahold欄位新增開窗
...
DEFINE tm  RECORD
          ...
          #u       LIKE type_file.chr3,       #TQC-840066 #FUN-D30070 mark
...
#FUN-740081 add
   ...
   LET g_sql = "oea01.oea_file.oea01,",
               ...
               "l_str1.type_file.chr1,",    #FUN-CB0001
               "l_num1.type_file.num5"      #FUN-CB0001
 
   LET l_table = cl_prt_temptable('atmx249',g_sql) CLIPPED   # 產生Temp Table
...
#FUN-740081 end
  ...
  #LET tm.u  = ARG_VAL(10) #FUN-D30070 mark
   ...
   LET tm.b  = ARG_VAL(11) #FUN-D30070 mod 12->11
   ...
   LET g_rpt_name = ARG_VAL(15)  #No.FUN-7C0078 #FUN-D30070 mod 16->15
   IF cl_null(g_bgjob) OR g_bgjob = 'N' OR g_bgjob is NULL
      THEN CALL x249_tm(0,0)
      ELSE CALL x249()
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690124
END MAIN
 
FUNCTION x249_tm(p_row,p_col)
...
DEFINE l_cmd          LIKE type_file.chr1000       #No.FUN-680120 VARCHAR(1000)
   ...
   OPEN WINDOW x249_w AT p_row,p_col WITH FORM "atm/42f/atmx249"
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,
                        ...
                        #" '",tm.u CLIPPED,"'", #FUN-D30070 mark
                         ...
                         " '",g_rep_clas CLIPPED,"'",          
                         " '",g_template CLIPPED,"'"            
         CALL cl_cmdat('atmx249',g_time,l_cmd)
      END IF
      CLOSE WINDOW x249_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690124
      EXIT PROGRAM
...
DEFINE l_str2   LIKE type_file.chr1  #FUN-CB0001
...
#NO.CHI-6A0004 --END
     ...
     LET l_sql = "SELECT '','','',",
                 ...
                 "   AND ", tm.wc CLIPPED,                                      
                 " ORDER BY oea01"  
     PREPARE x249_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690124
        EXIT PROGRAM
     END IF
     DECLARE x249_curs1 CURSOR FOR x249_prepare1
   # CALL cl_outnam('atmx249') RETURNING l_name #FUN-740081 TSD.c123k mark
   # START REPORT x249_rep TO l_name            #FUN-740081 TSD.c123k mark
     LET g_pageno = 0
 
 
     FOREACH x249_curs1 INTO sr.*
          IF SQLCA.sqlcode != 0 THEN
	  ...
	  IF tm.b = '2' AND NOT cl_null(sr.oeahold) THEN CONTINUE FOREACH END IF
        ...
        # OUTPUT TO REPORT x249_rep(sr.*)  # FUN-740081 TSD.c123k mark
          ...
          EXECUTE insert_prep USING
             ...
             l_str2,     l_num1  #FUN-CB0001 l_num1
          #------------------------------ CR (3) ------------------------------#
          # FUN-740081 TSD.c123k end ---------------------------
...
###XtraGrid###    LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
    ...
    IF g_zz05 = 'Y' THEN
       ...
       LET g_str = tm.wc
    END IF
###XtraGrid###    LET g_str = g_str,";",tm.s,";",tm.t,";",tm.u,";",tm.a,";",tm.b
 
###XtraGrid###    CALL cl_prt_cs3('atmx249','atmx249',l_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
    #FUN-CB0001-----add---str---
   #FUN-D30053---mod---str--
   #LET g_xgrid.order_field = cl_get_order_field(tm.s,"oea01,oea02,oea03,oea04,oea05")
   #LET g_xgrid.grup_field = cl_get_order_field(tm.s,"oea01,oea02,oea03,oea04,oea05")
   #LET g_xgrid.grup_sum_field = cl_get_sum_field(tm.s,tm.u,"oea01,oea02,oea03,oea04,oea05")
   #LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"oea01,oea02,oea03,oea04,oea05")
   #LET l_str = cl_wcchp(g_xgrid.order_field,"oea01,oea02,oea03,oea04,oea05")
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"oea01,oea02,oea03,oea04,gen02,gem02,oea23,oea12,oeahold")
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"oea01,oea02,oea03,oea04,gen02,gem02,oea23,oea12,oeahold")
   #LET g_xgrid.grup_sum_field = cl_get_sum_field(tm.s,tm.u,"oea01,oea02,oea03,oea04,gen02,gem02,oea23,oea12,oeahold")#FUN-D30070 mark
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"oea01,oea02,oea03,oea04,gen02,gem02,oea23,oea12,oeahold")
   #LET l_str = cl_wcchp(g_xgrid.order_field,"oea01,oea02,oea03,oea04,gen02,gem02,oea23,oea12,oeahold") #FUN-D30070 mark
   #FUN-D30053---mod---end--
   #LET l_str = cl_replace_str(l_str,',','-') #FUN-D30070 mark
   #LET g_xgrid.footerinfo1 = cl_getmsg('lib-626',g_lang),l_str #FUN-D30070 mark
    CASE tm.a
       WHEN '1'  LET g_xgrid.footerinfo2 = cl_getmsg('atm-082',g_lang)
       WHEN '2'  LET g_xgrid.footerinfo2 = cl_getmsg('atm-083',g_lang)
       WHEN '3'  LET g_xgrid.footerinfo2 = cl_getmsg('atm-084',g_lang)
    END CASE
