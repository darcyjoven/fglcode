...
#
# Pattern name...: atmq301
# Descriptions...: 客戶商品明細數量查詢
# Date & Author..: 2006/03/23 By Sarah
# Modify.........: No.FUN-630027 06/03/23 By Sarah 新增"客戶商品明細數量查詢"
...
# Modify.........: No.TQC-790073 07/09/12 By lumxa 指定筆對話框沒有"確定","程序信息"按鈕
...
DEFINE tm                RECORD
       ...
       g_tup01           LIKE tup_file.tup01,          #客戶編號
       g_occ02           LIKE occ_file.occ02,          #客戶名稱
       g_tup12           LIKE tup_file.tup12,          #送貨客戶
       ...
       g_argv2           LIKE tup_file.tup01,             #INPUT ARGUMENT - 2
...
#     DEFINE   l_time LIKE type_file.chr8	    #No.FUN-6B0014
   ...
   CASE g_argv1
      WHEN "1" LET g_prog = 'atmq301'
      WHEN "2" LET g_prog = 'atmq311'
      OTHERWISE 
   ...
   OPEN WINDOW q301_w AT p_row,p_col
        WITH FORM "atm/42f/atmq301"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
FUNCTION q301_cs()
   ...
   ELSE 
      ...
      #螢幕上取單頭條件
   INITIALIZE g_tup01 TO NULL    #No.FUN-750051
   INITIALIZE g_tup11 TO NULL    #No.FUN-750051
   INITIALIZE g_tup12 TO NULL    #No.FUN-750051
      CONSTRUCT tm.wc ON tup01,tup12,tup02,tup03,tup04,tup05,tup06,tup07 
           FROM tup01,tup12,s_tup[1].tup02,s_tup[1].tup03,s_tup[1].tup04,
   ...
   LET g_sql="SELECT UNIQUE tup01,tup11,tup12 FROM tup_file ",
             " WHERE ",tm.wc CLIPPED,l_sql,
             " ORDER BY tup01 "
   PREPARE q301_prepare FROM g_sql
   DECLARE q301_cs                         #SCROLL CURSOR
           SCROLL CURSOR FOR q301_prepare
 
   #取合乎條件筆數
   #若使用組合鍵值, 則可以使用本方法去得到筆數值
   LET g_sql="SELECT COUNT(UNIQUE tup01) FROM tup_file ",
             " WHERE ",tm.wc CLIPPED,l_sql 
   PREPARE q301_pp FROM g_sql
   DECLARE q301_cnt CURSOR FOR q301_pp
   OPEN q301_cnt                                                            
   FETCH q301_cnt INTO g_row_count                                           
   CLOSE q301_cnt
END FUNCTION
...
DEFINE
    ...
    CASE p_flag
        WHEN 'N' FETCH NEXT     q301_cs INTO g_tup01,g_tup11,g_tup12
        WHEN 'P' FETCH PREVIOUS q301_cs INTO g_tup01,g_tup11,g_tup12
        WHEN 'F' FETCH FIRST    q301_cs INTO g_tup01,g_tup11,g_tup12
        WHEN 'L' FETCH LAST     q301_cs INTO g_tup01,g_tup11,g_tup12
        WHEN '/'
          ...
          END IF
          FETCH ABSOLUTE g_jump q301_cs INTO g_tup01,g_tup11,g_tup12
          LET mi_no_ask = FALSE  #No.FUN-6A0072
    END CASE
 
 
    IF SQLCA.sqlcode THEN
        CALL cl_err(g_tup01,SQLCA.sqlcode,0)
        INITIALIZE g_tup01 TO NULL  #TQC-6B0105
        INITIALIZE g_tup11 TO NULL  #TQC-6B0105
        INITIALIZE g_tup12 TO NULL  #TQC-6B0105
        RETURN
    ELSE
       CASE p_flag                                                              
...
FUNCTION q301_show()
   DISPLAY g_tup11,g_tup01,g_tup12 TO tup11,tup01,tup12
   SELECT occ02 INTO g_occ02 FROM occ_file WHERE occ01 = g_tup01 
   IF SQLCA.sqlcode THEN
#     CALL cl_err(g_tup01,SQLCA.sqlcode,0)  #No.FUN-660104 MARK
      CALL cl_err3("sel","occ_file",g_tup01,"",SQLCA.sqlcode,"","",0)  #No.FUN-660104
      RETURN
   END IF
   SELECT occ02 INTO g_occ02_2 FROM occ_file WHERE occ01 = g_tup12 
   IF SQLCA.sqlcode THEN
#     CALL cl_err(g_tup12,SQLCA.sqlcode,0)  #No.FUN-660104 MARK
      CALL cl_err3("sel","occ_file",g_tup12,"",SQLCA.sqlcode,"","",0)  #No.FUN-660104
      RETURN
   END IF
   DISPLAY g_occ02,g_occ02_2 TO occ02,occ02_2
   CALL q301_b_fill() #單身
   CALL cl_show_fld_cont()                   #No.FUN-550037 hmf
END FUNCTION
 
FUNCTION q301_b_fill()              #BODY FILL UP
   DEFINE l_sql     LIKE type_file.chr1000        #No.FUN-680120 VARCHAR(1000)
 
   LET l_sql =
        "SELECT tup02,ima02,tup03,tup04,tup05,tup06,tup07 ",
        "  FROM tup_file, OUTER ima_file",
        " WHERE tup01 = '",g_tup01,"'",
        "   AND tup11 = '",g_tup11,"'",
        "   AND tup12 = '",g_tup12,"'",
        "   AND tup_file.tup02 = ima_file.ima01 AND ", tm.wc CLIPPED,
        " ORDER BY tup02,tup03"
    PREPARE q301_pb FROM l_sql
    DECLARE q301_bcs CURSOR FOR q301_pb             #BODY CURSOR
 
    CALL g_tup.clear()  
    LET g_rec_b= 0
    LET g_cnt  = 1
    FOREACH q301_bcs INTO g_tup[g_cnt].*
       IF SQLCA.sqlcode THEN
          CALL cl_err('Foreach:',SQLCA.sqlcode,1)
          EXIT FOREACH
       END IF
       IF cl_null(g_tup[g_cnt].tup05) THEN LET g_tup[g_cnt].tup05 = 0 END IF
       DISPLAY g_tup[g_cnt].ima02 TO ima02
       LET g_cnt = g_cnt + 1
