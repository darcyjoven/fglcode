...
# Modify.........: No.FUN-D10098 13/02/01 By lujh 新增gxrg420
...
MAIN
   ...
   LET l_table = cl_prt_temptable('gxrg420',g_sql) CLIPPED
   IF l_table = -1 THEN 
  ...
  # LET l_table1 = cl_prt_temptable('gxrg4201',g_sql) CLIPPED                      
  # IF l_table1 = -1 THEN
  #    #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40092    #FUN-C10036  mark
  #    #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4) #FUN-B40092   #FUN-C10036  mark
  #    EXIT PROGRAM END IF
  ...
  #                                                                              
  # LET l_table2 = cl_prt_temptable('gxrg4202',g_sql) CLIPPED                    
  # IF l_table2 = -1 THEN 
  #    #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40092    #FUN-C10036  mark
  #    #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4) #FUN-B40092   #FUN-C10036  mark
  #    EXIT PROGRAM END IF      
  ...
  #                                                                              
  # LET l_table3 = cl_prt_temptable('gxrg4203',g_sql) CLIPPED                    
  # IF l_table3 = -1 THEN
  #    #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40092   #FUN-C10036  mark
  #    #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4) #FUN-B40092   #FUN-C10036  mark
  #    EXIT PROGRAM 
  ...
  #                                                                              
  # LET l_table4 = cl_prt_temptable('gxrg4204',g_sql) CLIPPED                    
  # IF l_table4 = -1 THEN 
  #    #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40092   #FUN-C10036  mark
  #    #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4) #FUN-B40092    #FUN-C10036  mark
  #    EXIT PROGRAM 
...
FUNCTION gxrg420_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01        
   ...
   OPEN WINDOW gxrg420_w AT p_row,p_col
        WITH FORM "gxr/42f/gxrg420"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW gxrg420_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_template CLIPPED,"'",           
                         " '",g_rpt_name CLIPPED,"'"            
         CALL cl_cmdat('gxrg420',g_time,l_cmd)    
      END IF
      CLOSE WINDOW gxrg420_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)      
      EXIT PROGRAM
   END IF
...
FUNCTION gxrg420()
   ...
   DEFINE l_ii           INTEGER
   #DEFINE l_sql_2       LIKE type_file.chr1000    #FUN-CC0093 mark
...
#TQC-B30012 --end--   
     ...
     PREPARE gxrg420_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE gxrg420_curs1 CURSOR FOR gxrg420_prepare1
 
     FOREACH gxrg420_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
###GENGRE###     CALL cl_prt_cs3('gxrg420','gxrg420',l_sql,g_str)
    CALL gxrg420_grdata()    ###GENGRE###
END FUNCTION

###GENGRE###START
FUNCTION gxrg420_grdata()
    DEFINE l_sql    STRING
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("gxrg420")
        IF handler IS NOT NULL THEN
            START REPORT gxrg420_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                       # " ORDER BY ooa01,ooc02" #FUN-CC0093 mark
                        " ORDER BY ooa01 "       #FUN-CC0093 add
            DECLARE gxrg420_datacur1 CURSOR FROM l_sql
            FOREACH gxrg420_datacur1 INTO sr1.*
                OUTPUT TO REPORT gxrg420_rep(sr1.*)
            END FOREACH
            FINISH REPORT gxrg420_rep
        END IF
...
REPORT gxrg420_rep(sr1)
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.ooa01
            ...
            #            " AND oao03 = 0 AND oao05='1'"
            #START REPORT gxrg420_subrep01
            #DECLARE gxrg420_repcur1 CURSOR FROM l_sql
            #FOREACH gxrg420_repcur1 INTO sr2.*
            #    OUTPUT TO REPORT gxrg420_subrep01(sr2.*)
            #END FOREACH
            #FINISH REPORT gxrg420_subrep01
            #FUN-CC0093--mark--end
            #FUN-B40092------add------end
            LET l_lineno = 0 
        ...
        ON EVERY ROW
            ...
            #START REPORT gxrg420_subrep02
            #DECLARE gxrg420_repcur2 CURSOR FROM l_sql
            #FOREACH gxrg420_repcur2 INTO sr3.*
            #    OUTPUT TO REPORT gxrg420_subrep02(sr3.*)
            #END FOREACH
            #FINISH REPORT gxrg420_subrep02
            ...
            #START REPORT gxrg420_subrep03
            #DECLARE gxrg420_repcur3 CURSOR FROM l_sql
            #FOREACH gxrg420_repcur3 INTO sr4.*
            #    OUTPUT TO REPORT gxrg420_subrep03(sr4.*)
            #END FOREACH
            #FINISH REPORT gxrg420_subrep03
            #FUN-CC0093--mark--end
        ...
        AFTER GROUP OF sr1.ooa01
           #FUN-CC0093--add--str--
            ...
            START REPORT gxrg420_subrep01
            DECLARE gxrg420_repcur1 CURSOR FROM l_sql
            FOREACH gxrg420_repcur1 INTO sr1.*
                OUTPUT TO REPORT gxrg420_subrep01(sr1.*)
            END FOREACH
            FINISH REPORT gxrg420_subrep01
            ...
            START REPORT gxrg420_subrep02               
            DECLARE gxrg420_repcur2 CURSOR FROM l_sql
            FOREACH gxrg420_repcur2 INTO sr1.*
                OUTPUT TO REPORT gxrg420_subrep02(sr1.*)
            END FOREACH
            FINISH REPORT gxrg420_subrep02  
            ...
            START REPORT gxrg420_subrep03
            DECLARE gxrg420_repcur3 CURSOR FROM l_sql
            FOREACH gxrg420_repcur3 INTO sr1.*
                OUTPUT TO REPORT gxrg420_subrep03(sr1.*)
            END FOREACH
            FINISH REPORT gxrg420_subrep03
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        ...
                        "   AND oob04 IN ('A','D') ",
                        "ORDER BY oob04"
            START REPORT gxrg420_subrep04
            DECLARE gxrg420_repcur4 CURSOR FROM l_sql
            FOREACH gxrg420_repcur4 INTO sr1.*
                OUTPUT TO REPORT gxrg420_subrep04(sr1.*)
            END FOREACH
            FINISH REPORT gxrg420_subrep04
          #FUN-CC0093--add--end
            ...
            #            " ORDER BY oob03"  
            #START REPORT gxrg420_subrep05
            #DECLARE gxrg420_repcur5 CURSOR FROM l_sql
            #FOREACH gxrg420_repcur5 INTO sr1.*
            #    OUTPUT TO REPORT gxrg420_subrep05(sr1.*)
            #END FOREACH
            #FINISH REPORT gxrg420_subrep05
            ...
            #START REPORT gxrg420_subrep04
            #DECLARE gxrg420_repcur4 CURSOR FROM l_sql
            #FOREACH gxrg420_repcur4 INTO sr5.*
            #    OUTPUT TO REPORT gxrg420_subrep04(sr5.*)
            #END FOREACH
            #FINISH REPORT gxrg420_subrep04
            #FUN-CC0093--mark--end
            #FUN-B40092------add------end 
