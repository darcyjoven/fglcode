...
#No.FUN-7B0026--start
   ...
   LET l_table = cl_prt_temptable('gxrr600',g_sql) CLIPPED
   IF l_table=-1 THEN EXIT PROGRAM END IF 
   LET g_sql = "oox01_1.oox_file.oox01,",
               "oox05.oox_file.oox05,",
               "oox10.oox_file.oox10,",
               "azi05.azi_file.azi05"
   LET l_table1 = cl_prt_temptable('gxrr6001',g_sql) CLIPPED
   ...
   LET g_sql = "oox01.oox_file.oox01,",
               ...
               "oox10.oox_file.oox10,",
               "azi05.azi_file.azi05"
   LET l_table2 = cl_prt_temptable('gxrr6002',g_sql) CLIPPED
...
FUNCTION gxrr600_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW gxrr600_w AT p_row,p_col WITH FORM "gxr/42f/gxrr600"
...
WHILE TRUE
  ...
  LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW gxrr600_w
       CALL cl_used(g_prog,g_time,2) RETURNING g_time    #FUN-B80036 ADD
      EXIT PROGRAM
   END IF
   IF tm.wc=" 1=1" THEN
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('gxrr600',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW gxrr600_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time    #FUN-B80036 ADD
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#No.FUN-7B0026--start--
     ...
     IF STATUS THEN 
         ...
         EXIT PROGRAM
     END IF                                      
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='gxrr600'
#No.FUN-7B0026--end        
     ...
     PREPARE gxrr600_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare1:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time    #FUN-B80036 ADD
        EXIT PROGRAM
     END IF
     DECLARE gxrr600_curs1 CURSOR FOR gxrr600_prepare1
     ...
     LET l_sql="SELECT oox05,SUM(oox10) FROM oox_file ",
               ...
               " GROUP BY oox05",
               " ORDER BY 1"
     PREPARE gxrr600_prepare3 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare3:',SQLCA.sqlcode,1)
         CALL cl_used(g_prog,g_time,2) RETURNING g_time    #FUN-B80036 ADD
        EXIT PROGRAM
     END IF
     DECLARE gxrr600_curs3 CURSOR FOR gxrr600_prepare3
     ...
     PREPARE gxrr600_prepare2 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare2:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time    #FUN-B80036 ADD
        EXIT PROGRAM
     END IF
     DECLARE gxrr600_curs2 CURSOR FOR gxrr600_prepare2
    ...
    #str MOD-8B0099 add
     ...
     PREPARE gxrr600_prepare4_1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare4_1:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time    #FUN-B80036 ADD
        EXIT PROGRAM
     END IF
     DECLARE gxrr600_curs4_1 CURSOR FOR gxrr600_prepare4_1
    #end MOD-8B0099 add
     ...
     LET l_sql="SELECT oox05,SUM(oox10) FROM oox_file ",
               ...
               " GROUP BY oox05",
               " ORDER BY 1"
     PREPARE gxrr600_prepare4 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare4:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time    #FUN-B80036 ADD
        EXIT PROGRAM
     END IF
     DECLARE gxrr600_curs4 CURSOR FOR gxrr600_prepare4
 
#     CALL cl_outnam('gxrr600') RETURNING l_name               #No.FUN-7B0026
#No.FUN-7B0026--start--
     IF tm.b='1' THEN
     ...
     IF tm.b = '1' THEN              #統計報表 #modify NO.A080 030616
        FOREACH gxrr600_curs1 INTO sr.*
           ...
           ELSE
              LET sr.oox03v = '2'
           END IF
#          OUTPUT TO REPORT gxrr600_rep(sr.*)                  #No.FUN-7B0026
#No.FUN-7B0026--start--
           ...
           FOREACH gxrr600_curs3 USING sr.oox01 INTO gum.*                                                                               
...
#No.FUN-7B0026--end 
        END FOREACH
#       FINISH REPORT gxrr600_rep                              #No.FUN-7B0026
     ELSE                          #明細報表
        FOREACH gxrr600_curs2 INTO sr1.*
           IF SQLCA.sqlcode != 0 THEN
              CALL cl_err('foreach:',SQLCA.sqlcode,1)
           EXIT FOREACH
           END IF
#          OUTPUT TO REPORT gxrr600_rep(sr1.*)                 #No.FUN-7B0026
#No.FUN-7B0026--start--                                                                                                             
           SELECT azi04,azi05,azi07 INTO l_azi04,l_azi05,l_azi07 FROM azi_file       #FUN-870151 Add azi07                                                                     
        ...
        FOREACH gxrr600_curs4_1 INTO sr1.oox01,sr1.oox02   #MOD-8B0099 add
           FOREACH gxrr600_curs4 USING sr1.oox01,sr1.oox02 INTO gum.*                                                                     
              SELECT azi05 INTO l_azi05 FROM azi_file                                                                                   
              WHERE azi01=gum.oox05                                                                                                    
              IF SQLCA.sqlcode THEN                                                                                                     
                 LET l_azi05 = g_azi05    #使用本國幣別
...
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
     ...
     CALL cl_prt_cs3('gxrr600',l_name,l_sql,g_str)
#No.FUN-7B0026--end       
     #No.FUN-BB0047--mark--Begin---
...
#       FOREACH gxrr600_curs4 USING sr.oox01,sr.oox02 INTO gum.*
#          SELECT azi05 INTO l_azi05 FROM azi_file
#           WHERE azi01=gum.oox05
#          IF SQLCA.sqlcode THEN
