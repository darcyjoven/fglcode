...
# Modify.........: No.FUN-B40092 11/06/09 By xujing 憑證報表轉GRW
...
TYPE sr1_t RECORD
    ...
    l_str1 LIKE type_file.num5,
    l_str2 LIKE rme_file.rme01,   #FUN-C40026 add,
#FUN-C40026----add---str---
...
MAIN
   ...
   IF (NOT cl_setup("ARM")) THEN
      EXIT PROGRAM
   END IF
   #CALL cl_used(g_prog,g_time,1) RETURNING g_time  #FUN-B80073    ADD   #FUN-C10036 MARK
...
#FUN-730019.....begin 
   LET g_sql = "rme01.rme_file.rme01,rme03.rme_file.rme03,",
    ...
    LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
                #" VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"     #TQC-760177
                " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"  #TQC-760177     #FUN-C40026 add 4?
...
#FUN-730019.....end
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL armg140_tm(0,0)        # Input print condition
      ELSE CALL armg140()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073    ADD
   CALL cl_gre_drop_temptable(l_table)              #FUN-B40092
END MAIN
 
FUNCTION armg140_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   END IF
   OPEN WINDOW armg140_w AT p_row,p_col
        WITH FORM "arm/42f/armg140"
################################################################################
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW armg140_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073    ADD
      CALL cl_gre_drop_temptable(l_table)              #FUN-B40092
      EXIT PROGRAM
   END IF
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('armg140',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW armg140_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073    ADD
      CALL cl_gre_drop_temptable(l_table)              #FUN-B40092
      EXIT PROGRAM
   END IF
...
#     CALL  cl_used(g_prog,g_time,1) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6A0085 #FUN-B40092 mark
    ...
    #  FROM yba_file                           #MOD-590504 add   #FUN-650081 mark
     ...
     LET l_sql="SELECT rme01,rme03,occ18,rme21,rme073,rme074,rme075,rme076,rme077, ",  #FUN-720014 add rme076/77   #MOD-890206最後少一個逗號
               ...
               "       rmc31,rmc09,rmc16 ",
              #"  FROM rme_file,rmc_file,OUTER occ_file ",  #FUN-C50008 mark--
              #"  WHERE rme01=rmc23 AND rme_file.rme03=occ_file.occ01 ", #FUN-C50008 mark--
               "  FROM rme_file LEFT OUTER JOIN occ_file ON rme03=occ01,rmc_file ", #FUN-C50008 add LEFT OUTER  
               "  WHERE rme01=rmc23  ",  #FUN-C50008 add
               "    AND rmeconf <> 'X' ",  #CHI-C80041
               "    AND rmevoid='Y' AND ",tm.wc CLIPPED
 
     PREPARE armg140_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE armg140_curs1 CURSOR FOR armg140_prepare1
 
#    CALL cl_outnam('armg140') RETURNING l_name  #FUN-730019 mark
#    START REPORT armg140_rep TO l_name          #FUN-730019 mark
 
     LET g_pageno = 0
     FOREACH armg140_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#FUN-730019.....begin
       ...
       EXECUTE insert_prep USING sr.rme01,sr.rme03,sr.rme073,sr.rme074,
                                 ...
                                 sr.occ18,l_str1,l_str2,                 #FUN-C40026 add ,
                                 "",l_img_blob,"N",""                    #FUN-C40026 add
#      OUTPUT TO REPORT armg140_rep(sr.*)
#FUN-730019.....end
     END FOREACH
     LET g_cr_table = l_table                 #FUN-C40026 add
     LET g_cr_apr_key_f = "rme01"             #FUN-C40026 add
...
###GENGRE###     CALL cl_prt_cs3('armg140','armg140',l_sql,g_str)   #FUN-710080 modify
    CALL armg140_grdata()    ###GENGRE###
#    FINISH REPORT armg140_rep
...
FUNCTION armg140_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("armg140")
        IF handler IS NOT NULL THEN
            START REPORT armg140_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY rme01,rmc02"
            DECLARE armg140_datacur1 CURSOR FROM l_sql
            FOREACH armg140_datacur1 INTO sr1.*
                OUTPUT TO REPORT armg140_rep(sr1.*)
            END FOREACH
            FINISH REPORT armg140_rep
        END IF
...
REPORT armg140_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    
    ORDER EXTERNAL BY sr1.rme01,sr1.rmc02
