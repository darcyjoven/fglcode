...
#FUN-730019.....begin
   ...
   LET l_table = cl_prt_temptable('armr180',g_sql) CLIPPED
...
#FUN-730019.....end
   ...
   IF cl_null(tm.wc)
      THEN CALL armr180_tm(0,0)        # Input print condition
      ELSE LET tm.wc = "rma01='",tm.wc CLIPPED,"'"
         CALL armr180()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073 ADD #FUN-BB0047 從IF中拉出
END MAIN
 
FUNCTION armr180_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   END IF
   OPEN WINDOW armr180_w AT p_row,p_col
        WITH FORM "arm/42f/armr180"
################################################################################
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW armr180_w
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('armr180',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW armr180_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073    ADD
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0085
    ...
    #  FROM yba_file                                           #FUN-650081 mark
     ...
     LET l_sql="SELECT rma01,rma03,rma04,rma13,rma14,rma20,rma21,rmb02,rmb03, ",
               ...
               "  FROM rma_file,rmb_file,OUTER gen_file,OUTER gem_file ",
               "  WHERE rma01=rmb01 AND rma_file.rma13=gen_file.gen01 AND rma_file.rma14=gem_file.gem01 ",
               "  AND   rma09 !='6' AND rmavoid='Y' ",
               "  AND ",tm.wc CLIPPED
 
     PREPARE armr180_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare1:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073    ADD
        EXIT PROGRAM
     END IF
     DECLARE armr180_curs1 CURSOR FOR armr180_prepare1
 
#    CALL cl_outnam('armr180') RETURNING l_name   #FUN-730019 mark
#    START REPORT armr180_rep TO l_name           #FUN-730019 mark
     SELECT zx02 INTO g_name FROM zx_file WHERE zx01=g_user
     LET g_pageno = 0
     FOREACH armr180_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,1)
          EXIT FOREACH
       END IF
       EXECUTE insert_prep USING sr.*,    #FUN-730019
                                 "",l_img_blob, "N",""  #TQC-C10039 ADD "",l_img_blob, "N",""
#      OUTPUT TO REPORT armr180_rep(sr.*)#FUN-730019 mark
     END FOREACH
#FUN-730019.....bgein
     ...
     CALL cl_prt_cs3('armr180','armr180',l_sql,g_str)   #FUN-710080 modify
#    FINISH REPORT armr180_rep         
