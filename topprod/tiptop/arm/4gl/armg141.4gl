...
# Modify.........: No.FUN-C60060 12/06/18 By lixiang CR報表轉換為GR報表
...
TYPE sr1_t RECORD
    ...
    rme01 LIKE rme_file.rme01,
    rme03 LIKE rme_file.rme03,
    occ18 LIKE occ_file.occ18,
    occ09 LIKE occ_file.occ09,
    l_occ09 LIKE occ_file.occ18,
    occ07 LIKE occ_file.occ07,
    l_occ07 LIKE occ_file.occ18,
    occ21 LIKE occ_file.occ21,
    l_geb02 LIKE geb_file.geb02,
    rme12 LIKE rme_file.rme12,
    gen02 LIKE gen_file.gen02,
    rme21 LIKE rme_file.rme21,
    rme073 LIKE rme_file.rme073,
    rme074 LIKE rme_file.rme074,
    rme075 LIKE rme_file.rme075,
    rme076 LIKE rme_file.rme076,
    rme077 LIKE rme_file.rme077,
    rme021 LIKE rme_file.rme021,
    ...
    sign_str LIKE type_file.chr1000  #FUN-C60060
END RECORD
###GENGRE###END
...
#FUN-C30190 add begin ---
   LET g_sql =  "no.type_file.num5,",
                ...
                "sign_str.type_file.chr1000 "     #FUN-C60060
   LET l_table = cl_prt_temptable('armg141',g_sql) CLIPPED
...
#FUN-C30190 add end ---
   ...
   IF cl_null(tm.wc)
      THEN CALL armg141_tm(0,0)        # Input print condition
      ELSE
        #LET tm.wc = "rme01='",tm.wc CLIPPED,"'"    #No.TQC-610087 mark
        CALL armg141()            # Read data and create out-file
   END IF
 { IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL armg141_tm(0,0)        # Input print condition
      ELSE CALL armg141()            # Read data and create out-file
   END IF  }
   CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073 ADD #FUN-BB0047 從IF中拉出
   CALL cl_gre_drop_temptable(l_table)   #FUN-C60060 
END MAIN
 
FUNCTION armg141_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   END IF
   OPEN WINDOW armg141_w AT p_row,p_col
        WITH FORM "arm/42f/armg141"
################################################################################
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW armg141_w
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('armg141',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW armg141_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073    ADD
      CALL cl_gre_drop_temptable(l_table)   #FUN-C60060
      EXIT PROGRAM
   END IF
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0085
          ...
          sr        RECORD
                   ...
                   ,sign_type LIKE type_file.chr1,      #FUN-C30190 add
                    ...
                    sign_str  LIKE type_file.chr1000    #FUN-C60060 add 
                    END RECORD
...
" rmc_file LEFT OUTER JOIN ima_file ON ima_file.ima01=rmc_file.rmc04 ",
     ...
     PREPARE armg141_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE armg141_curs1 CURSOR FOR armg141_prepare1
 
#    CALL cl_outnam('armg141') RETURNING l_name                   #FUN-C30190 mark
#    START REPORT armg141_rep TO l_name                           #FUN-C30190 mark
 
     SELECT zx02 INTO g_name FROM zx_file WHERE zx01=g_user
     LET g_pageno = 0
     INITIALIZE sr.* TO NULL                                      #FUN-C30190 add
     FOREACH armg141_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,1)
          EXIT FOREACH
       END IF
#      OUTPUT TO REPORT armg141_rep(sr.*)                         #FUN-C30190 mark
#FUN-C30190 add begin ----
...
#FUN-C30190 add begin ---
   LET l_sql="SELECT rme01 ",
             ...
             " WHERE ",tm.wc CLIPPED,
             "  GROUP BY rme01"
   PREPARE g141_pr4 FROM l_sql
   IF SQLCA.sqlcode THEN
      CALL cl_err('prepare g141_pr4:',SQLCA.sqlcode,0)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(l_table)   #FUN-C60060
      EXIT PROGRAM
   END IF
   DECLARE g141_cs4 CURSOR FOR g141_pr4
   LET g_cr_table = l_table
   LET g_cr_gcx01 = "armi001"
   LET g_cr_apr_key_f = "rme01"
   LET l_ii = 1
   CALL g_cr_apr_key.clear()
   FOREACH g141_cs4 INTO l_key
      LET g_cr_apr_key[l_ii].v1 = l_key
      LET l_ii = l_ii + 1
   END FOREACH
###GENGRE###   LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
###GENGRE###   LET g_str = tm.wc
###GENGRE###   CALL cl_prt_cs3('armg141','armg141',l_sql,g_str)
    CALL armg141_grdata()    ###GENGRE###
#FUN-C30190 add end ----
#    FINISH REPORT armg141_rep                                    #FUN-C30190 mark
...
FUNCTION armg141_grdata()
    ...
    LOCATE sr1.sign_img IN MEMORY   #FUN-C60060 add
    CALL cl_gre_init_apr()          #FUN-C60060 add

    WHILE TRUE
        CALL cl_gre_init_pageheader()            
        LET handler = cl_gre_outnam("armg141")
        IF handler IS NOT NULL THEN
            START REPORT armg141_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE armg141_datacur1 CURSOR FROM l_sql
            FOREACH armg141_datacur1 INTO sr1.*
                OUTPUT TO REPORT armg141_rep(sr1.*)
            END FOREACH
            FINISH REPORT armg141_rep
        END IF
...
REPORT armg141_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5

    
    ORDER EXTERNAL BY sr1.rme01
