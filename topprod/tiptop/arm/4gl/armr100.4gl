...
MAIN
   ...
   LET l_table = cl_prt_temptable('armr100',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N' THEN
           CALL armr100_tm(0,0)             # Input print condition
      ELSE
           CALL armr100()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073    ADD
END MAIN
 
FUNCTION armr100_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW armr100_w AT p_row,p_col
        WITH FORM "arm/42f/armr100"
################################################################################
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW armr100_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073    ADD
      EXIT PROGRAM
   END IF
   IF tm.wc=" 1=1" THEN
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('armr100',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW armr100_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073    ADD
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0085
     ...
     PREPARE armr100_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare1:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073    ADD
        EXIT PROGRAM
     END IF
     DECLARE armr100_curs1 CURSOR FOR armr100_prepare1
#    CALL cl_outnam('armr100') RETURNING l_name  #No.FUN-710085 mark
 
#    START REPORT armr100_rep TO l_name   #No.FUN-710085 mark
 
#    LET g_pageno = 0                     #No.FUN-710085 mark
     FOREACH armr100_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#      OUTPUT TO REPORT armr100_rep(sr.*) #No.FUN-710085 mark
     END FOREACH
 
#    FINISH REPORT armr100_rep            #No.FUN-710085 mark
    ...
    #str MOD-A30139 mod
     ...
     LET g_str = tm.wc CLIPPED,";",g_zz05 CLIPPED,";",
     ...
     CALL cl_prt_cs3('armr100','armr100',l_sql,g_str)   #FUN-710080 modify
     #No.FUN-710085 --end--
