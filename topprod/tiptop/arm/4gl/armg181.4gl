...
# Modify.........: No.FUN-C10036 12/01/11 By yangtt TQC-B70175追單
...
#FUN-730019.....begin
   LET g_sql = "rma01.rma_file.rma01,rma03.rma_file.rma03,",
               ...
               #FUN-C40026----add---end----
   LET l_table = cl_prt_temptable('armg181',g_sql) CLIPPED
   IF l_table = -1 THEN
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   END IF
   OPEN WINDOW armg181_w AT p_row,p_col
        WITH FORM "arm/42f/armg181"
################################################################################
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW armg181_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073    ADD
      CALL cl_gre_drop_temptable(l_table)    #FUN-B40097
      EXIT PROGRAM
   END IF
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('armg181',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW armg181_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80073    ADD
      CALL cl_gre_drop_temptable(l_table)    #FUN-B40097
      EXIT PROGRAM
   END IF
...
#     CALL  cl_used(g_prog,g_time,1) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6A0085 #FUN-B40097 mark
    ...
    #  FROM yba_file                                           #FUN-650081 mark
     ...
     LET l_sql="SELECT rma01,rma03,rma04,rma13,rma14,rma20,rma10,rma11,rma18,",
         ...
         #      " FROM rma_file,rmb_file,oay_file,OUTER gen_file,OUTER gem_file", #FUN-C50004 mark
         #      " WHERE rma01=rmb01 AND rma_file.rma13=gen_file.gen01 AND rma_file.rma14=gem_file.gem01 ", #FUN-C50004 mark
               " FROM rmb_file,oay_file,rma_file LEFT OUTER JOIN gen_file ON rma13=gen01 ",  #FUN-C50004 add
...
#              " AND rma01[1,3]=oayslip",
     ...
     PREPARE armg181_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE armg181_curs1 CURSOR FOR armg181_prepare1
#FUN-730019.....begin mark
#    CALL cl_outnam('armg181') RETURNING l_name
#    START REPORT armg181_rep TO l_name
#FUN-730019.....end mark
     LET g_pageno = 0
     LET g_cnt = 0
     FOREACH armg181_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
       ...
       EXECUTE insert_prep USING sr.rma01,sr.rma03,sr.rma04,sr.rma10,sr.rma11,
                                 ...
                                 #sr.gen02,sr.gem02         #FUN-730019   #FUN-C40026 mark
                                 sr.gen02,sr.gem02,"",l_img_blob,"N",""                     #FUN-C40026 add
#      OUTPUT TO REPORT armg181_rep(sr.*)     #FUN-730019 mark
     END FOREACH
...
###GENGRE###     CALL cl_prt_cs3('armg181','armg181',l_sql,g_str)   #FUN-710080 modify
    CALL armg181_grdata()    ###GENGRE###
#    FINISH REPORT armg181_rep
...
FUNCTION armg181_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("armg181")
        IF handler IS NOT NULL THEN
            START REPORT armg181_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                       #" ORDER BY rma01,rmb02"         #FUN-C20052 mark 
                        " ORDER BY lower(rma01),rmb02"  #FUN-C20052 add          
            DECLARE armg181_datacur1 CURSOR FROM l_sql
            FOREACH armg181_datacur1 INTO sr1.*
                OUTPUT TO REPORT armg181_rep(sr1.*)
            END FOREACH
            FINISH REPORT armg181_rep
        END IF
...
REPORT armg181_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_rmb12  LIKE rmb_file.rmb12     #FUN-B40097

    
    ORDER EXTERNAL BY sr1.rma01,sr1.rmb02
    ...
    FORMAT
        ...
        AFTER GROUP OF sr1.rma01
            #FUN-B40097----add----str----------------
            LET l_rmb12 = GROUP SUM(sr1.rmb12)
            PRINTX l_rmb12  
            #FUN-B40097----add----end----------------

        AFTER GROUP OF sr1.rmb02
