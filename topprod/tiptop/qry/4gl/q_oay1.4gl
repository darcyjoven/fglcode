...
# Modify.........: No.TQC-930148 09/03/25 By Carrier smyslip變成oayslip
# Modify.........: No.FUN-980030 09/08/14 By Hiko 加上資料權限控制
# Modify.........: No:CHI-9C0048 09/12/23 By tsai_yen 陣列索引ARR_CURR()為0的判斷
# Modify........: No:FUN-A70130 10/08/11 By huangtao 抓取oay_file資料時加入oaysys條件篩選
...
DEFINE   l_sys           LIKE zz_file.zz011    #No.FUN-A70130
 
FUNCTION q_oay1(pi_multi_sel,pi_need_cons,ps_default1,ps_sys)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_oay1" ATTRIBUTE(STYLE="create_qry") #No.FUN-660161
 
   CALL cl_ui_locale("q_oay1")
...
FUNCTION oay1_qry_sel()
   ...
   WHILE TRUE
      ...
      CALL oay1_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL oay1_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL oay1_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION oay1_qry_prep_result_set()
   ...
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_oay1', 'oay_file')
   ...
   IF (NOT mi_multi_sel) THEN
      LET ls_where = " AND oaytype MATCHES '3*'"
   END IF
   ...
   FOREACH lcurs_qry INTO lr_qry.*
      ...
      SELECT COUNT(*) INTO l_n FROM smu_file WHERE smu01=lr_qry.oayslip  AND smu03=p_sys #CHECK USER  #No.TQC-930148
      IF l_n>0 THEN                                                #USER權限存有資料,並g_user判斷是否存在
         SELECT COUNT(*) INTO l_n FROM smu_file WHERE smu01=lr_qry.oayslip AND smu02=g_user AND smu03=p_sys    #No.TQC-930148
         IF l_n=0 THEN
             IF g_gen03 IS NULL THEN                               #g_user沒有部門           
                 CONTINUE FOREACH
             ELSE                                                  #CHECK g_user部門是否存在                                                    
                 SELECT COUNT(*) INTO l_n FROM smv_file WHERE smv01=lr_qry.oayslip AND smv02=g_gen03 AND smv03=p_sys   #No.TQC-930148
                 IF l_n=0 THEN
                    CONTINUE FOREACH
                 END IF
             END IF
         END IF
      ELSE                                                          #CHECK Dept               
         SELECT COUNT(*) INTO l_n FROM smv_file WHERE smv01=lr_qry.oayslip AND smv03=p_sys  #No.TQC-930148
          IF l_n>0 THEN
             IF g_gen03 IS NULL THEN                                #g_user沒有部門     
                 CONTINUE FOREACH 
             ELSE                                                   #CHECK g_user部門是否存在
...
# Modify        :
##################################################
FUNCTION oay1_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION oay1_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_oay.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL oay1_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_oay.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL oay1_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL oay1_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION oay1_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION oay1_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION oay1_qry_accept(pi_sel_index)
