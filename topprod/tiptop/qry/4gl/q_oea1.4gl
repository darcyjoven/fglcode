...
#{
# Program name  : q_oea1.4gl
# Program ver.  : 7.0
# Description   : 訂單資料查詢
# Date & Author : 2006/01/13 by yoyo 
# Memo          : 
# Modify        :
# Modify        : No.FUN-650108 06/07/01 By yoyo將oeb04_2拿掉,將資料全部顯示在oeb04
...
# Modify........: No.TQC-760159 07/06/21 By Mandy 調整FUN-650108備註亂碼
...
DEFINE   ma_qry   DYNAMIC ARRAY OF RECORD
         check    LIKE type_file.chr1,  	#No.FUN-680131 VARCHAR(1) #TQC-760159
...
FUNCTION q_oea1(pi_multi_sel,pi_need_cons,ps_default1,p_cus_no,p_type)
...
FUNCTION oea1_qry_sel()
   ...
   WHILE TRUE
         ...
         IF (mi_need_cons) THEN
          IF NOT cl_null(g_cus_no) THEN
            CONSTRUCT ms_cons_where ON oea01, oea00, oea02, oea032, oea14
                                  FROM s_oea[1].oea01,s_oea[1].oea00,s_oea[1].oea02,s_oea[1].oea032,s_oea[1].oea14 
      ...
      CALL oea1_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL oea1_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL oea1_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION oea1_qry_prep_result_set()
...
#No.FUN-650108--end      
   ...
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_oea1', 'oea_file')
   IF NOT cl_null(l_filter_cond) THEN
      LET ms_cons_where = ms_cons_where,l_filter_cond
   END IF
   #End:FUN-980030
   LET ls_sql = "SELECT 'N',oea01,oea00,oea02,oea032,oea14,'',''", 
              #  " FROM oea_file",
              #  " WHERE ",ms_cons_where
              #  by yoyo
                 " FROM oea_file,oeb_file",
                 "WHERE oea01=oeb01 AND oeb1003='1'",
                 " AND ",ms_cons_where 
   IF NOT mi_multi_sel THEN
...
# Modify        :
##################################################
FUNCTION oea1_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION oea1_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_oea.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL oea1_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_oea.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL oea1_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL oea1_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION oea1_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION oea1_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION oea1_qry_accept(pi_sel_index)
