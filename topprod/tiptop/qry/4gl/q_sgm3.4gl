...
#
# Program name   : q_sgm3.4gl
# Program ver.   : 7.0
# Description    : Run Card資料查詢
# Date & Author  : 2006/08/28 by rainy
# Memo           : 傳回RUN CARD 及作業編號
# Modify.........: No.FUN-680131 06/09/04 By Carrier 欄位型態用LIKE定義
...
# Modify.........: No:TQC-AB0389 10/12/03 By jan 移除sgm59
...
FUNCTION q_sgm3(pi_multi_sel,pi_need_cons,p_key1,ps_default2)
   DEFINE   pi_multi_sel   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
            pi_need_cons   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
            ps_default1    LIKE sgm_file.sgm01, #預設回傳值(在取消時會回傳此類預設值).
            ps_default2    LIKE sgm_file.sgm01, #預設回傳值(在取消時會回傳此類預設值).
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_sgm3" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
 
   CALL cl_ui_locale("q_sgm3")
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            CONSTRUCT ms_cons_where ON sgm01,sgm04,sgm06
                       FROM s_sgm[1].sgm01,s_sgm[1].sgm04,s_sgm[1].sgm06
      ...
      CALL sgm3_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL sgm3_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL sgm3_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION sgm3_qry_prep_result_set()
   ...
   DEFINE   l_sgm54         LIKE sgm_file.sgm54,
           ...
           #l_sgm59         LIKE sgm_file.sgm59,  #TQC-AB0389
   ...
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_sgm3', 'sgm_file')
   IF NOT cl_null(l_filter_cond) THEN
      LET ms_cons_where = ms_cons_where,l_filter_cond
   END IF
   #End:FUN-980030
   LET ls_sql = "SELECT  'N',sgm01,sgm04,sgm45,'',sgm06,",
                "        sgm54,sgm291,sgm301,sgm302,sgm303,sgm304,",
                "        sgm311,sgm312,sgm313,", 
                "        sgm314,sgm316,sgm317,sgm321,sgm322 ",  #TQC-AB0389
                "  FROM sgm_file",
                " WHERE ",ms_cons_where CLIPPED
   LET ls_sql = ls_sql CLIPPED," ORDER BY sgm01,sgm04,sgm06"
  ...
  ##NO.FUN-980025 GP5.2 add--end
   ...
   FOREACH lcurs_qry INTO lr_qry.*,l_sgm54 ,l_sgm291,l_sgm301,l_sgm302,
                          ...
                          l_sgm314,l_sgm316,l_sgm317,l_sgm321,l_sgm322
                         #l_sgm59   #TQC-AB0389
      ...
      IF l_sgm54='Y' THEN          # Check in='Y'
         LET lr_qry.wipqty 
                             =  l_sgm291             #check in
                              - l_sgm311 #*l_sgm59     #良品轉出  #TQC-AB0389
                              - l_sgm312 #*l_sgm59     #重工轉出  #TQC-AB0389
                              - l_sgm313 #*l_sgm59     #當站報廢  #TQC-AB0389
                              - l_sgm314 #*l_sgm59     #當站下線  #TQC-AB0389
                              - l_sgm316 #*l_sgm59                #TQC-AB0389
                              - l_sgm317 #*l_sgm59                #TQC-AB0389
#                             - l_sgm321             #委外加工量
#                             + l_sgm322             #委外完工量
      ELSE
         LET lr_qry.wipqty 
...
#                             + l_sgm322             #委外完工量
      END IF
    #---------No.MOD-640338 modify 
      IF lr_qry.wipqty > 0 THEN
...
# Modify        :
##################################################
FUNCTION sgm3_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION sgm3_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_sgm.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL sgm3_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_sgm.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL sgm3_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL sgm3_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION sgm3_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION sgm3_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION sgm3_qry_accept(pi_sel_index)
      ...
      FOR li_i = 1 TO ma_qry.getLength()
         IF (ma_qry[li_i].check = 'Y') THEN
            IF (lsb_multi_sel.getLength() = 0) THEN
               CALL lsb_multi_sel.append(ma_qry[li_i].sgm01 CLIPPED)
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].sgm01 CLIPPED)
            END IF
         END IF    
   ...
   ELSE
      LET ms_ret2 = ma_qry[pi_sel_index].sgm01 CLIPPED
      LET ms_ret3 = ma_qry[pi_sel_index].sgm04 CLIPPED
