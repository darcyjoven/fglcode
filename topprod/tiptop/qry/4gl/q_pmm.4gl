...
# Modify........: No.TQC-7C0094 07/12/10 By chenl  增加distinct
...
DEFINE   ma_qry   DYNAMIC ARRAY OF RECORD
	 ...
	 pmn04    LIKE type_file.chr1000,       #No.FUN-680131 VARCHAR(60)
...
FUNCTION q_pmm(pi_multi_sel,pi_need_cons,ps_default1)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_pmm" ATTRIBUTE(STYLE="create_qry") #No.FUN-660161
...
FUNCTION pmm_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            ...
            IF g_sma.sma120 = 'Y' THEN
               CALL pmm_multi_cons()   #調用自定義的CONSTRUCT過程
      ...
      CALL pmm_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL pmm_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL pmm_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION pmm_qry_prep_result_set()
   ...
   IF ( g_sma.sma120 = 'N' )OR(( g_sma.sma120 = 'Y')AND( g_multi_where.getLength() = 0 ))THEN
      IF ms_cons_where2 = " 1=1" THEN    #No.TQC-7C0094 增加了一個空格。
 
         #Begin:FUN-980030
         LET l_filter_cond = cl_get_extra_cond_for_qry('q_pmm', 'pmm_file')
...
# Modify        :
##################################################
FUNCTION pmm_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION pmm_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_pmm.* 
      ...
      ON ACTION prevpage
         ...
         CALL pmm_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_pmm.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL pmm_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL pmm_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION pmm_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION pmm_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION pmm_qry_accept(pi_sel_index)
...
FUNCTION pmm_multi_cons()
  ...
  INPUT ARRAY ma_cons_lif WITHOUT DEFAULTS FROM s_pmm.* 
    ...
    AFTER FIELD pmm01,pmm02,pmm04,pmm07,pmm09,pmm12,pmn04
      CALL pmm_after_normal_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
    ...
    AFTER FIELD att01,att02,att03,att04,att05,att06,att07,att08,att09,att10,
      ...
      CALL pmm_after_detail_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
  ...
  ELSE 
     ...
     CALL pmm_make_sql_condition()   #這個市cl_create_qry中定義的標準函數
  END IF  
...
#明細屬性欄位以及相應的處理過程
#傳入pname為欄位的colname
FUNCTION pmm_after_normal_field(pname,pvalue,prow)
  DEFINE pname        STRING,
         pvalue       STRING,
...
#當前內容到對應的后台數組中
FUNCTION pmm_after_detail_field(pname,pvalue,pi)
  DEFINE pname         STRING
...
FUNCTION pmm_make_sql_condition()  
