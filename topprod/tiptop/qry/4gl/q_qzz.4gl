...
FUNCTION q_qzz(pi_multi_sel,pi_need_cons,ps_default1)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_qzz" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
 
   CALL cl_ui_locale("q_qzz")
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            IF g_lang = '0' THEN
               CONSTRUCT ms_cons_where ON zz01,zz02 
                                     FROM s_zz[1].*
               ...
               END CONSTRUCT
            ELSE
               CONSTRUCT ms_cons_where ON zz01,zz02e
                                     FROM s_zz[1].zz01,s_zz[1].zz02
                  ON IDLE g_idle_seconds
                     CALL cl_on_idle()
                     CONTINUE CONSTRUCT
...
FUNCTION qzz_qry_prep_result_set()
   ...
   IF g_lang = '0' THEN
      ...
      #Begin:FUN-980030
      LET l_filter_cond = cl_get_extra_cond_for_qry('q_qzz', 'zz_file')
   ...
   IF (NOT mi_multi_sel) THEN
      LET ls_where = " AND zz01 = zy02 AND zy01 = zx04 AND zx01 = '",g_user,"'"
   END IF
   LET ls_sql = ls_sql CLIPPED,ls_where CLIPPED," ORDER BY zz01"
...
FUNCTION qzz_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   #INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_zz.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE)            #FUN-880082 mark
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_zz.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      BEFORE INPUT
