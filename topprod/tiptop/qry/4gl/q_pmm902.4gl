...
#
# Program name  : q_pmm902.4gl
# Program ver.  : 7.0
# Description   : 採購單資料查詢
# Date & Author : 2008/05/05 by wujie  #No.TQC-850008
# Modify........: No.TQC-860010 08/06/05 By wujie  與p_qry中資料重名，故改名
# Modify........: No.FUN-880082 08/08/25 By tsai_yen 開窗全選功能
# Modify........: No.MOD-8A0051 08/10/06 By clover 只抓取採購單項次為"2.轉成採購單"的資料
# Modify........: No.MOD-8A0139 08/10/17 By Sarah 畫面改開啟q_pmm902,增加項次pmn02欄位,並回傳
# Modify.........: No.FUN-980030 09/08/14 By Hiko 加上資料權限控制
# Modify.........: No.FUN-980025 09/10/09 By dxfwo hardcode的部分修改
# Modify.........: No.FUN-9A0093 09/11/01 By lutingting 新增參數p_plant
...
DEFINE ma_qry         DYNAMIC ARRAY OF RECORD
       ...
       pmn02          LIKE pmn_file.pmn02,   #MOD-8A0139 add
       pmm02          LIKE pmm_file.pmm02,
       pmm04          LIKE pmm_file.pmm04,
       pmm07          LIKE pmm_file.pmm07, 
       pmm09          LIKE pmm_file.pmm09, 
       pmm12          LIKE pmm_file.pmm12,
       pmn04          LIKE pmn_file.pmn04,   #MOD-8A0139 mod #type_file.chr1000
       attdesc        LIKE type_file.chr1000,
...
DEFINE ms_cons_where2 STRING                      #暫存CONSTRUCT區塊的WHERE條件.
DEFINE mi_page_count  LIKE type_file.num10        #每頁顯現資料筆數.
DEFINE ms_default1    STRING     
DEFINE ms_default2    STRING                      #MOD-8A0139 add
DEFINE ms_ret1        STRING  
DEFINE ms_ret2        STRING                      #MOD-8A0139 add
       {%格式為ms_ret+流水號}
DEFINE ms_fac_type    STRING                      #No.MOD-840179
DEFINE ms_dbs         LIKE type_file.chr21             #FUN-9A0093 
DEFINE ms_plant       LIKE type_file.chr10             #FUN-9A0093
DEFINE g_apa01        LIKE apa_file.apa01         #MOD-C80172 add

#FUNCTION q_pmm9(pi_multi_sel,pi_need_cons,ps_default1,ps_fac_type)
#FUNCTION q_pmm902(pi_multi_sel,pi_need_cons,ps_default1,ps_fac_type)     #No.TQC-860010  #FUN-9A0093
FUNCTION q_pmm902(pi_multi_sel,pi_need_cons,ps_default1,ps_fac_type,p_plant,p_apa01)      #FUN-9A0093  #MOD-C80172 add p_apa01
   DEFINE pi_multi_sel LIKE type_file.num5,
          pi_need_cons LIKE type_file.num5,
          ps_default1  STRING    
          {%與回傳欄位的個數相對應,格式為ps_default+流水號}
   DEFINE ps_fac_type  STRING                     #No.MOD-840179
   DEFINE p_plant      LIKE type_file.chr10            #FUN-9A0093 
   DEFINE p_apa01      LIKE apa_file.apa01        #MOD-C80172 add

   LET ms_default1 = ps_default1
   LET ms_fac_type = ps_fac_type                  #No.MOD-840179
   ...
   #ELSE
     ...
     LET g_apa01 = p_apa01      #MOD-C80172 add
     #FUN-9C0041--mod--str--
     #CALL s_getdbs()
  ...
  #str MOD-8A0139 mod
  #OPEN WINDOW w_qry WITH FORM "qry/42f/q_pmm" ATTRIBUTE(STYLE="create_qry")
  #CALL cl_ui_locale("q_pmm")
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_pmm902" ATTRIBUTE(STYLE="create_qry")
   CALL cl_ui_locale("q_pmm902")
  #end MOD-8A0139 mod
   ...
   IF (mi_multi_sel) THEN
     #CALL cl_set_comp_font_color("pmm01", "red")         #MOD-8A0139 mark
      CALL cl_set_comp_font_color("pmm01,pmn02", "red")   #MOD-8A0139
   END IF
   ...
   IF (mi_multi_sel) THEN
      RETURN ms_ret1 
   ELSE
      RETURN ms_ret1,ms_ret2    #MOD-8A0139 add ms_ret2 
   END IF
END FUNCTION
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            #根據當前系統中是否進行料件多屬性管理來確定
            #CONSTRUCT的方式
            IF g_sma.sma120 = 'Y' THEN
               CALL pmm902_multi_cons()     #調用自定義的CONSTRUCT過程
            ...
            ELSE
             ...
             #str MOD-8A0139 mod
             #       ON pmm01,pmm02,pmm04,pmm07,pmm09,pmm12
             #     FROM s_pmm[1].pmm01,s_pmm[1].pmm02,s_pmm[1].pmm04,
             #          s_pmm[1].pmm07,s_pmm[1].pmm09,s_pmm[1].pmm12
                     ON pmm01,pmn02,pmm02,pmm04,pmm07,pmm09,pmm12,pmn04
                   FROM s_pmm[1].pmm01,s_pmm[1].pmn02,s_pmm[1].pmm02,
                        s_pmm[1].pmm04,s_pmm[1].pmm07,s_pmm[1].pmm09,
                        s_pmm[1].pmm12,s_pmm[1].pmn04
             #end MOD-8A0139 mod
              ...
              END CONSTRUCT
             #str MOD-8A0139 mark
             #CONSTRUCT ms_cons_where2 ON pmn04
             #     FROM s_pmm[1].pmn04
             #   ON IDLE g_idle_seconds
             #      CALL cl_on_idle()
             #      CONTINUE CONSTRUCT
         ...
         CALL pmm902_qry_prep_result_set() 
      ...
      CALL pmm902_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL pmm902_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL pmm902_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION pmm902_qry_prep_result_set()
   ...
   DEFINE   l_pmn87pmn31    LIKE apb_file.apb24   #MOD-C80172 add
   DEFINE   l_apb24         LIKE apb_file.apb24   #MOD-C80172 add
   DEFINE   l_pmc913        LIKE pmc_file.pmc913  #MOD-C80172 add
   
   #如果不進行料件多屬性管理，或進行了多屬性管理但是沒有輸入任何條件，則執行原有代碼
   IF ( g_sma.sma120 = 'N' )OR(( g_sma.sma120 = 'Y')AND( g_multi_where.getLength() = 0 ))THEN
      IF ms_cons_where2 = " 1=1" THEN               #增加了一個空格。
        #LET ls_sql = "SELECT DISTINCT 'N',pmm01,pmm02,pmm04,pmm07,pmm09,pmm12,'', ",            #MOD-8A0139 mark
 
         #Begin:FUN-980030
         LET l_filter_cond = cl_get_extra_cond_for_qry('q_pmm902', 'pmm_file')
         IF NOT cl_null(l_filter_cond) THEN
            LET ms_cons_where = ms_cons_where,l_filter_cond
         END IF
         #End:FUN-980030
         LET ls_sql = "SELECT DISTINCT 'N',pmm01,pmn02,pmm02,pmm04,pmm07,pmm09,pmm12,pmn04, ",   #MOD-8A0139
                      "'','','','','','','','','','','','','','','','',",
                      "'','','','',''",
                     #" FROM pmm_file ",            #MOD-8A0139 mark
                     #" FROM pmm_file,pmn_file ",   #MOD-8A0139   #FUN-9A0093 mark
                      #" FROM ",ms_dbs CLIPPED,"pmm_file,",ms_dbs CLIPPED,"pmn_file ",   #FUN-9A0093 add
                      " FROM ",cl_get_target_table(g_plant_new,'pmm_file'),",", #FUN-A50102
                               cl_get_target_table(g_plant_new,'pmn_file'),     #FUN-A50102
                      " WHERE ",ms_cons_where
         IF NOT mi_multi_sel THEN
            IF cl_null(ms_fac_type) THEN            #No.MOD-840179
               LET ls_where = " AND pmmacti='Y' AND pmm25 MATCHES '*' AND ",
                              " pmm18 <> 'X' ",
                              " AND pmm01 = pmn01"   #MOD-8A0139 add
            #No.MOD-840179 080423 add --begin                  
            ELSE
               LET ls_where = " AND pmmacti='Y' AND pmm25 MATCHES '*' AND ",
                              " pmm18 <> 'X' ",ms_fac_type,  
                              " AND pmm01 = pmn01"   #MOD-8A0139 add
            END IF
            #No.MOD-840179 080423 add --end                                    
         END IF
      ...
      ELSE
        #LET ls_sql = "SELECT DISTINCT 'N',pmm01,pmm02,pmm04,pmm07,pmm09,pmm12,'', ",            #MOD-8A0139 mark
         LET ls_sql = "SELECT DISTINCT 'N',pmm01,pmn02,pmm02,pmm04,pmm07,pmm09,pmm12,pmn04, ",   #MOD-8A0139
                      ...
                      " WHERE ",ms_cons_where," AND ",ms_cons_where2     
         IF NOT mi_multi_sel THEN
            IF cl_null(ms_fac_type) THEN            #No.MOD-840179
               LET ls_where = " AND pmmacti='Y' AND pmm25 MATCHES '*' AND ",
                              " pmm01 = pmn01 AND pmm18 <> 'X' "
            #No.MOD-840179 080423 add --begin                  
            ELSE
            	 LET ls_where = " AND pmmacti='Y' AND pmm25 MATCHES '*' AND ",
                              " pmm01 = pmn01 AND pmm18 <> 'X' ",ms_fac_type
            END IF
            #No.MOD-840179 080423 add --end                  
         END IF
         LET ls_sql = ls_sql CLIPPED," AND pmn16='2' "      #MOD-8A0051
      END IF 
   ELSE 
      #否則按照料件多屬性機制的進行合成條件
     #LET ls_sql = "SELECT DISTINCT 'N',pmm01,pmm02,pmm04,pmm07,pmm09,pmm12,'', ",            #MOD-8A0139 mark
      LET ls_sql = "SELECT DISTINCT 'N',pmm01,pmn02,pmm02,pmm04,pmm07,pmm09,pmm12,pmn04, ",   #MOD-8A0139
                   "'','','','','','','','','','','','','','','','',",
                   "'','','','',''",
                   " FROM pmm_file,pmn_file",g_multi_join,
                   " WHERE ",g_multi_where
      IF NOT mi_multi_sel THEN
         IF cl_null(ms_fac_type) THEN            #No.MOD-840179
            LET ls_where = " AND pmmacti='Y' AND pmm25 MATCHES '*' AND ",
  ...
  #LET ls_sql = ls_sql CLIPPED,ls_where CLIPPED," ORDER BY pmm01 "         #MOD-8A0139 mark
   LET ls_sql = ls_sql CLIPPED,ls_where CLIPPED," ORDER BY pmm01,pmn02 "   #MOD-8A0139
  ...
  ##NO.FUN-980025 GP5.2 add--end
   ...
   FOREACH lcurs_qry INTO lr_qry.*
     ...
     #str MOD-8A0139 mark
     #LET ls_sql = "SELECT pmn04 FROM pmn_file",
     #             " WHERE pmn01 = '",lr_qry.pmm01,"'",
     #             "   AND ",ms_cons_where2 CLIPPED
     #PREPARE q_pmn_prepare FROM ls_sql
     #DECLARE pmn_curs CURSOR FOR q_pmn_prepare
     #FOREACH pmn_curs INTO l_pmn04
     #   LET lr_qry.pmn04 = lr_qry.pmn04 CLIPPED," ",l_pmn04
     #END FOREACH
     #end MOD-8A0139 mark

     #MOD-C80172---add---S
      SELECT pmc913 INTO l_pmc913 
        FROM pmc_file
       WHERE pmc01 = lr_qry.pmm09

      IF g_apz.apz61 = '1' OR (g_apz.apz61 = '3' AND l_pmc913 ='Y') THEN
         LET l_pmn87pmn31=0
         LET l_apb24=0

         SELECT pmn87*pmn31 INTO l_pmn87pmn31 
           FROM pmn_file,pmm_file
          WHERE pmm01=pmn01
            AND pmn01=lr_qry.pmm01
            AND pmn02=lr_qry.pmn02

         SELECT SUM(apb24) INTO l_apb24 
           FROM apb_file,apa_file
          WHERE apa01=apb01
            AND apb01!=g_apa01 AND apa42!='X'
            AND apb06=lr_qry.pmm01 AND apb07=lr_qry.pmn02
            AND apa00 = '15'
      END IF

      IF cl_null(l_pmn87pmn31) THEN
         LET l_pmn87pmn31=0
      END IF
...
# Modify        :
##################################################
FUNCTION pmm902_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION pmm902_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_pmm.* 
      ...
      BEFORE INPUT
         ...
         END IF
      ON ACTION prevpage
         CALL GET_FLDBUF(s_pmm.check) RETURNING ma_qry_tmp[ARR_CURR()].check
         CALL pmm902_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_pmm.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL pmm902_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL pmm902_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
      ...
      ON ACTION cancel
         ...
         IF (NOT mi_multi_sel) THEN
            ...
            LET ms_ret2 = ms_default2   #MOD-8A0139 add
         END IF
...
# Modify   	    :
##################################################
FUNCTION pmm902_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10,
            pi_end_index     LIKE type_file.num10
   DEFINE   li_i             LIKE type_file.num10,
            li_j             LIKE type_file.num10
...
# Parameter   	: ps_hide_act      STRING    所要隱藏的Action Button
#               : pi_start_index   LIKE type_file.num10    所要顯現的查詢資料起始位置
#               : pi_end_index     LIKE type_file.num10    所要顯現的查詢資料結束位置R
# Return   	    : SMALLINT   是否繼續
#               : SMALLINT   是否重新查詢
#               : INTEGER    改變後的起始位置
# Memo        	:
# Modify   	    :
##################################################
FUNCTION pmm902_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10,
            pi_end_index     LIKE type_file.num10
   DEFINE   li_continue      LIKE type_file.num5,
   ...
   DISPLAY ARRAY ma_qry_tmp TO s_pmm.*
      BEFORE DISPLAY
         CALL cl_set_act_visible("prevpage,nextpage,reconstruct",TRUE)
         IF (ps_hide_act IS NOT NULL) THEN   
            CALL cl_set_act_visible(ps_hide_act, FALSE)
...
# Modify   	    :
##################################################
FUNCTION pmm902_qry_accept(pi_sel_index)
      ...
      FOR li_i = 1 TO ma_qry.getLength()
         IF (ma_qry[li_i].check = 'Y') THEN
            IF (lsb_multi_sel.getLength() = 0) THEN
               CALL lsb_multi_sel.append(ma_qry[li_i].pmm01 CLIPPED)
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].pmm01 CLIPPED)
            END IF
         END IF    
   ...
   ELSE
      LET ms_ret1 = ma_qry[pi_sel_index].pmm01 CLIPPED
      LET ms_ret2 = ma_qry[pi_sel_index].pmn02 CLIPPED   #MOD-8A0139 add
...
FUNCTION pmm902_fill_mutli_rec()
DEFINE
  li_i,li_j            LIKE type_file.num5,
  lc_j,lc_jc,lc_jb     LIKE type_file.chr2,
  la_field             DYNAMIC ARRAY OF STRING
 
  #填充字段名稱數組
  CALL la_field.clear()
  LET la_field[1] = 'pmm01'
  LET la_field[2] = 'pmn02'   #MOD-8A0139 add
  LET la_field[3] = 'pmm02'
  LET la_field[4] = 'pmm04'
  LET la_field[5] = 'pmm07'
  LET la_field[6] = 'pmm09'
  LET la_field[7] = 'pmm12'
  LET la_field[8] = 'pmn04'
  
  #清空該模塊全局數組
  CALL ma_multi_rec.clear()
  #逐項添加信息(7個標準欄位)
  FOR li_i = 1 TO 8   #MOD-8A0139 mod 7->8
  ...
  END FOR 
  #設置獨特的屬性
  LET ma_multi_rec[8].imafld = 'Y'   #MOD-8A0139 mod 7->8
  #逐項添加信息(21個輔助欄位)
  LET ma_multi_rec[li_i].index = li_i
...
FUNCTION pmm902_multi_cons()
  DEFINE l_ac        LIKE type_file.num5,
         lc_ze03     LIKE ze_file.ze03        
 
  #初始化中間數組
  CALL pmm902_fill_mutli_rec()
  ...
  CALL cl_set_comp_entry('pmm01,pmn02,pmm02,pmm04,pmm07,pmm09,pmm12,pmn04',TRUE)   #MOD-8A0139 add pmn02
  ...
  INPUT ARRAY ma_cons_lif WITHOUT DEFAULTS FROM s_pmm.* 
    ...
    #信息以及顯示可能會有的明細屬性列  
    AFTER FIELD pmm01,pmn02,pmm02,pmm04,pmm07,pmm09,pmm12,pmn04   #MOD-8A0139 add pmn02
      CALL pmm902_after_normal_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
    ...
    AFTER FIELD att01,att02,att03,att04,att05,att06,att07,att08,att09,att10,
      ...
      CALL pmm902_after_detail_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
  ...
  ELSE 
     ...
     CALL pmm902_make_sql_condition()   #這個市cl_create_qry中定義的標準函數
  END IF  
  ...
  CALL cl_set_comp_entry('pmm01,pmn02,pmm02,pmm04,pmm07,pmm09,pmm12,pmn04',FALSE)   #MOD-8A0139 add pmn02
...
#明細屬性欄位以及相應的處理過程
#傳入pname為欄位的colname
FUNCTION pmm902_after_normal_field(pname,pvalue,prow)
  DEFINE pname        STRING,
         pvalue       STRING,
         prow         LIKE type_file.num5,
         pvalue_tmp   LIKE type_file.chr1000,
         li_ia,l_i    LIKE type_file.num5,
         li_i         LIKE type_file.num5,
         lc_i         LIKE type_file.chr2,
         #這個數組用于接收各個屬性列的詳細信息
         ...
         li_edit      LIKE type_file.num5,
         li_comb      LIKE type_file.num5
...
#當前內容到對應的后台數組中
FUNCTION pmm902_after_detail_field(pname,pvalue,pi)
  DEFINE pname         STRING
  DEFINE pvalue        STRING
  DEFINE pi            LIKE type_file.num5
  DEFINE lc_string     STRING
  DEFINE lc_disp_field STRING
  DEFINE li_i,li_ia    LIKE type_file.num5
...
FUNCTION pmm902_make_sql_condition()  
  DEFINE li_rowcount    LIKE type_file.num5,
         li_i,li_ia     LIKE type_file.num5,
         li_n           LIKE type_file.num5,
         li_idx_a       LIKE type_file.num5,
         li_idx_b       LIKE type_file.num5,
         li_c           LIKE type_file.num5
  ...
  DEFINE li_alias      LIKE type_file.num5,
         lc_alias      STRING,
         lc_tablename  STRING
  ...
  FOR li_n = 1 to li_rowcount
      ...
      FOR li_i = 1 TO ma_multi_rec.getLength() 
          ...
          #如果當前欄位已經是'pmn04'(即最後一個標準欄位)則退出當前層次的循環
          IF ma_multi_rec[li_i].colname = 'pmn04' THEN 
             EXIT FOR
          END IF
