...
#
# Program name  : q_pmm9.4gl
# Program ver.  : 7.0
# Description   : 採購單資料查詢
# Date & Author : 2008/04/23 by hellen #No.MOD-840179
...
FUNCTION q_pmm9(pi_multi_sel,pi_need_cons,ps_default1,ps_fac_type)
   ...
   IF (mi_multi_sel) THEN
      CALL cl_set_comp_font_color("pmm01", "red")
   END IF
...
FUNCTION pmm9_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            ...
            IF g_sma.sma120 = 'Y' THEN
               CALL pmm9_multi_cons()     #調用自定義的CONSTRUCT過程
      ...
      CALL pmm9_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL pmm9_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL pmm9_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION pmm9_qry_prep_result_set()
   ...
   IF ( g_sma.sma120 = 'N' )OR(( g_sma.sma120 = 'Y')AND( g_multi_where.getLength() = 0 ))THEN
      IF ms_cons_where2 = " 1=1" THEN               #增加了一個空格。
         ...
         #Begin:FUN-980030
         LET l_filter_cond = cl_get_extra_cond_for_qry('q_pmm9', 'pmm_file')
         IF NOT cl_null(l_filter_cond) THEN
            LET ms_cons_where = ms_cons_where,l_filter_cond
         END IF
         #End:FUN-980030
         LET ls_sql = "SELECT DISTINCT 'N',pmm01,pmm02,pmm04,pmm07,pmm09,pmm12,'', ",
                      "'','','','','','','','','','','','','','','','',",
                      "'','','','',''",
                      " FROM pmm_file ",
                      " WHERE ",ms_cons_where
   ...
   LET ls_sql = ls_sql CLIPPED,ls_where CLIPPED," ORDER BY pmm01 "
...
# Modify        :
##################################################
FUNCTION pmm9_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION pmm9_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_pmm.* 
      ...
      ON ACTION prevpage
         ...
         CALL pmm9_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_pmm.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL pmm9_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL pmm9_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	    :
##################################################
FUNCTION pmm9_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10,
            pi_end_index     LIKE type_file.num10
   DEFINE   li_i             LIKE type_file.num10,
            li_j             LIKE type_file.num10
...
# Modify   	    :
##################################################
FUNCTION pmm9_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10,
            pi_end_index     LIKE type_file.num10
   DEFINE   li_continue      LIKE type_file.num5,
...
# Modify   	    :
##################################################
FUNCTION pmm9_qry_accept(pi_sel_index)
...
DEFINE
  ...
  LET la_field[2] = 'pmm02'
  LET la_field[3] = 'pmm04'
  LET la_field[4] = 'pmm07'
  LET la_field[5] = 'pmm09'
  LET la_field[6] = 'pmm12'
  LET la_field[7] = 'pmn04'
...
FUNCTION pmm9_multi_cons()
  DEFINE l_ac        LIKE type_file.num5,
         lc_ze03     LIKE ze_file.ze03        
 
  #初始化中間數組
  CALL pmm9_fill_mutli_rec()
  ...
  CALL cl_set_comp_entry('pmm01,pmm02,pmm04,pmm07,pmm09,pmm12,pmn04',TRUE)
  ...
  INPUT ARRAY ma_cons_lif WITHOUT DEFAULTS FROM s_pmm.* 
    ...
    #信息以及顯示可能會有的明細屬性列  
    AFTER FIELD pmm01,pmm02,pmm04,pmm07,pmm09,pmm12,pmn04
      CALL pmm9_after_normal_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
    ...
    AFTER FIELD att01,att02,att03,att04,att05,att06,att07,att08,att09,att10,
      ...
      CALL pmm9_after_detail_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
  ...
  ELSE 
     ...
     CALL pmm9_make_sql_condition()   #這個市cl_create_qry中定義的標準函數
  END IF  
  ...
  CALL cl_set_comp_entry('pmm01,pmm02,pmm04,pmm07,pmm09,pmm12,pmn04',FALSE)
...
#明細屬性欄位以及相應的處理過程
#傳入pname為欄位的colname
FUNCTION pmm9_after_normal_field(pname,pvalue,prow)
  DEFINE pname        STRING,
         pvalue       STRING,
...
#當前內容到對應的后台數組中
FUNCTION pmm9_after_detail_field(pname,pvalue,pi)
  DEFINE pname         STRING
...
FUNCTION pmm9_make_sql_condition()  
