...
#
# Program name  : q_zz01.4gl
# Program ver.  : 7.0
# Description   : 料件編號資料查詢
# Date & Author : No.FUN-C80092 2012/09/07 by lixh1
...
FUNCTION q_zz01(pi_multi_sel,pi_need_cons,ps_default1,ps_default2)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_zz01" ATTRIBUTE(STYLE="create_qry") 
 
   CALL cl_ui_locale("q_zz01")
...
FUNCTION zz01_qry_sel()
   ...
   WHILE TRUE
      ...
      CALL zz01_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL zz01_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL zz01_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
FUNCTION zz01_construct()
  CLEAR FORM
  CALL ma_qry_tmp.clear()
  CONSTRUCT ms_cons_where ON zz01 FROM s_zz[1].zz01            
     ON ACTION about         
        CALL cl_about()        
...
# Modify        :
##################################################
FUNCTION zz01_qry_prep_result_set()
   ...
   LET l_filter_cond1 = cl_get_extra_cond_for_qry('q_zz01', 'zz_file')
   LET l_filter_cond2 = cl_get_extra_cond_for_qry('q_zz01', 'gga_file')
   LET ms_cons_where1 = ms_cons_where
   LET l_where = cl_replace_str(ms_cons_where,'zz','gga')
   ...
   END IF
   LET ls_sql = "SELECT 'N',zz01,gaz03 FROM zz_file,gaz_file",
                " WHERE zz01 = gaz01",
                "   AND gaz02 = '",g_lang,"'",
                "   AND ",ms_cons_where1 CLIPPED,
                " UNION ",
                "SELECT 'N',gga01,gga02 FROM gga_file ",
                " WHERE  gga01 <> '",ms_default1,"'",
                "   AND  ggaacti = 'Y'",
...
# Modify        :
##################################################
FUNCTION zz01_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION zz01_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED)
      ...
      ON ACTION prevpage
         ...
         CALL zz01_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        
            CALL GET_FLDBUF(s_ecm.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL zz01_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL zz01_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        
            LET ms_ret1 = NULL       
         END IF                      
...
# Modify   	:
##################################################
FUNCTION zz01_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10,
            pi_end_index     LIKE type_file.num10
   DEFINE   li_i             LIKE type_file.num10,
            li_j             LIKE type_file.num10 
...
# Modify   	:
##################################################
FUNCTION zz01_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10,
            pi_end_index     LIKE type_file.num10
   DEFINE   li_continue      LIKE type_file.num5,
...
# Modify   	:
##################################################
FUNCTION zz01_qry_accept(pi_sel_index)
 ...
 #GDC 1.3版本後，若沒有資料，ARR_CURR()會是0
   ...
   ELSE
      ...
      LET ms_ret2 = ma_qry[pi_sel_index].gaz03 CLIPPED
   END IF
END FUNCTION
#FUN-C20047
#FUN-BC0104
