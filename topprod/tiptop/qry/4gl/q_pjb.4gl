...
#
# Program name   : q_pjb.4gl
# Program ver.   : 5.0
# Description    : WBS資料查詢(多工廠)
...
DEFINE   ms_omb41         LIKE omb_file.omb41
FUNCTION q_pjb(pi_multi_sel,pi_need_cons,ps_default1,ps_omb41,p_plant)
   DEFINE   pi_multi_sel   LIKE type_file.num5, 
            pi_need_cons   LIKE type_file.num5, 
            ps_default1    STRING  , #預設回傳值(在取消時會回傳此類預設值).
            ps_omb41       LIKE omb_file.omb41,
            p_plant        LIKE type_file.chr10

   
    LET ms_default1 = ps_default1 
    LET ms_omb41 = ps_omb41
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_pjb" ATTRIBUTE(STYLE="create_qry") 
 
   CALL cl_ui_locale("q_pjb")
   ...
   IF (mi_multi_sel) THEN #在複選狀態時,要將回傳欄位的字型顏色設為紅色,以作為標示.
      CALL cl_set_comp_font_color("pjb02", "red")
   END IF
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            ...
            CONSTRUCT ms_cons_where ON pjb02,pjb03,pja01
                                   FROM s_pjb[1].pjb02,s_pjb[1].pjb03,s_pjb[1].pja01
         ...
         CALL m_pjb_qry_prep_result_set() 
      ...
      CALL m_pjb_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL m_pjb_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL m_pjb_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION m_pjb_qry_prep_result_set()
   ...
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_pjb', 'pjb_file')
   IF NOT cl_null(l_filter_cond) THEN
      LET ms_cons_where = ms_cons_where,l_filter_cond
   END IF  
 
   LET ls_sql = "SELECT 'N',pjb02,pjb03,pja01 ",
                #"  FROM ",ms_dbs CLIPPED,"pjb_file, ",ms_dbs CLIPPED,"pja_file ",
                "  FROM ",cl_get_target_table(g_plant_new,'pjb_file'),",", #FUN-A50102
                          cl_get_target_table(g_plant_new,'pja_file'),     #FUN-A50102
                " WHERE pja01 = pjb01 AND pja01 = '",ms_omb41,"' ",
                "   AND pjaclose = 'N' AND pjbacti = 'Y' AND ",ms_cons_where
...
# Modify        :
##################################################
FUNCTION m_pjb_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION m_pjb_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_pjb.* 
   ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED)
      BEFORE INPUT
         ...
         END IF
      ON ACTION prevpage
         CALL GET_FLDBUF(s_pjb.check) RETURNING ma_qry_tmp[ARR_CURR()].check
         CALL m_pjb_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_pjb.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL m_pjb_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL m_pjb_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION m_pjb_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 
            pi_end_index     LIKE type_file.num10 
   DEFINE   li_i             LIKE type_file.num10,
            li_j             LIKE type_file.num10 
...
# Modify   	:
##################################################
FUNCTION m_pjb_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 
            pi_end_index     LIKE type_file.num10 
   DEFINE   li_continue      LIKE type_file.num5,
   ...
   DISPLAY ARRAY ma_qry_tmp TO s_pjb.*
      BEFORE DISPLAY
         CALL cl_set_act_visible("prevpage,nextpage,reconstruct",TRUE)
         IF (ps_hide_act IS NOT NULL) THEN   
            CALL cl_set_act_visible(ps_hide_act, FALSE)
...
# Modify   	:
##################################################
FUNCTION m_pjb_qry_accept(pi_sel_index)
      ...
      FOR li_i = 1 TO ma_qry.getLength()
         IF (ma_qry[li_i].check = 'Y') THEN
            IF (lsb_multi_sel.getLength() = 0) THEN
               CALL lsb_multi_sel.append(ma_qry[li_i].pjb02 CLIPPED) 
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].pjb02 CLIPPED)  
            END IF
         END IF    
   ...
   ELSE
      LET ms_ret1 = ma_qry[pi_sel_index].pjb02 CLIPPED
   END IF
END FUNCTION
#FUN-9A0093
