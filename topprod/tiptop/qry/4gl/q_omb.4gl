...
# Modify.........: No.TQC-810034 08/01/09 By chenl 增加限制條件，若oob_file中已存在的omb01,omb03，則不予顯示。
# Modify.........: No.FUN-840023 08/04/07 By saki 串查功能
# Modify.........: No.FUN-880082 08/09/01 By tsai_yen 開窗全選功能
# Modify.........: No.FUN-980030 09/08/14 By Hiko 加上資料權限控制
# Modify.........: No:CHI-9C0048 09/12/23 By tsai_yen 陣列索引ARR_CURR()為0的判斷
# Modify.........: No:MOD-A80249 10/09/01 By Dido 過濾條件增加 oob06 不為 NULL 
...
FUNCTION q_omb(pi_multi_sel,pi_need_cons,ps_default1,p_key1,p_key2,p_cus,p_type)
...
FUNCTION omb_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            ...
            IF g_sma.sma120 = 'Y' THEN
               CALL omb_multi_cons()   #調用自定義的CONSTRUCT過程
      ...
      CALL omb_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL omb_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL omb_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION omb_qry_prep_result_set()
...
# Modify        :
##################################################
FUNCTION omb_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION omb_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_omb.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL omb_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_omb.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL omb_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL omb_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION omb_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION omb_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION omb_qry_accept(pi_sel_index)
...
FUNCTION omb_multi_cons()
  ...
  INPUT ARRAY ma_cons_lif WITHOUT DEFAULTS FROM s_omb.* 
    ...
    AFTER FIELD omb01,omb03,omb16t,omb14t,omb04,oma23,oox10,omb35,omb34
      CALL omb_after_normal_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
    ...
    AFTER FIELD att01,att02,att03,att04,att05,att06,att07,att08,att09,att10,
      ...
      CALL omb_after_detail_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
  ...
  ELSE 
     ...
     CALL omb_make_sql_condition()   #這個市cl_create_qry中定義的標準函數
  END IF  
...
#明細屬性欄位以及相應的處理過程
#傳入pname為欄位的colname
FUNCTION omb_after_normal_field(pname,pvalue,prow)
  DEFINE pname        STRING,
         pvalue       STRING,
...
#當前內容到對應的后台數組中
FUNCTION omb_after_detail_field(pname,pvalue,pi)
  DEFINE pname         STRING
...
FUNCTION omb_make_sql_condition()  
