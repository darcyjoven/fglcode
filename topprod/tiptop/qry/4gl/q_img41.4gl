...
#                  p_dbs  :工廠別
# Date & Author : 2004/07/07 by Mandy Copy From q_img4
# Memo          : 
# Modify        :
...
# Modify.........: No.FUN-980094 09/08/24 By TSD.hoho GP5.2 跨資料庫語法修改
...
DEFINE   ms_dbs           LIKE azp_file.azp03 #工廠別
DEFINE   ms_dbs_tra       LIKE azp_file.azp03 #FUN-980094 PLANT的TRANS DB
DEFINE   g_reconstruct    LIKE type_file.num5      #No.MOD-660044 add  #No.FUN-680131 SMALLINT
 
#FUN-980094 傳入的db變數應該成傳入plant 變數---------------------(S)
#FUNCTION q_img41(pi_multi_sel,pi_need_cons,p_img01,ps_default1,ps_default2,ps_default3,p_kind,p_dbs)
FUNCTION q_img41(pi_multi_sel,pi_need_cons,p_img01,
                 ps_default1,ps_default2,ps_default3,p_kind,p_plant)
#FUN-980094 傳入的db變數應該成傳入plant 變數---------------------(E)
   ...
   DEFINE   p_dbs          LIKE azp_file.azp03 #工廠別
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_img41" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
 
   CALL cl_ui_locale("q_img41")
...
FUNCTION img41_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         CALL img41_qry_prep_result_set() 
      ...
      CALL img41_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL img41_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL img41_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION img41_qry_prep_result_set()
   ...
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_img41', 'img_file')
   IF NOT cl_null(l_filter_cond) THEN
      LET ms_cons_where = ms_cons_where,l_filter_cond
   END IF
   #End:FUN-980030
   LET ls_sql = "SELECT 'N',img02,img03,img04,img09,img10,img27", 
              # " FROM ",ms_dbs CLIPPED,".dbo.img_file",     #TQC-950048 MARK                                                           
               #FUN-980094 ----------(S)
               #" FROM ",s_dbstring(ms_dbs),"img_file",  #TQC-950048 ADD     
...
# Modify        :
##################################################
FUNCTION img41_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION img41_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_img.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL img41_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_img.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL img41_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL img41_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION img41_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION img41_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION img41_qry_accept(pi_sel_index)
