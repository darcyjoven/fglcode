...
#{
# Program name  : q_oay4.4gl
# Program ver.  : 7.0
# Description   : 單據性質查詢
# Date & Author : 06/02/27 by day
# Memo          : 
...
#FUNCTION q_oay4(pi_multi_sel,pi_need_cons,ps_default1,p_type,p_sys,p_dbs)   #No.FUN-980025
FUNCTION q_oay4(pi_multi_sel,pi_need_cons,ps_default1,p_type,p_sys,p_plant)  #No.FUN-980025
   DEFINE   pi_multi_sel   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
            pi_need_cons   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
            ps_default1    STRING,   
            p_type         LIKE oay_file.oaytype,   #單據性質     
   ...
   LET ms_sys      = p_sys
   #LET mc_dbs      = p_dbs
   LET l_sys       = DOWNSHFIT(ms_sys)                #No.FUN-A70130
 
   WHENEVER ERROR CALL cl_err_msg_log
 
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_oay4" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
 
   CALL cl_ui_locale("q_oay4")  
...
FUNCTION oay4_qry_sel()
   ...
   WHILE TRUE
      ...
      CALL oay4_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL oay4_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL oay4_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION oay4_qry_prep_result_set()
   ...
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_oay4', 'oay_file')
   ...
   IF (NOT mi_multi_sel) THEN
#      LET ls_where = " AND oaytype ='70' "
      IF ms_type != '*' THEN
         LET ls_where = ls_where CLIPPED," AND oaytype='",ms_type,"'"
      END IF
   END IF
...
# Modify        :
##################################################
FUNCTION oay4_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION oay4_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_oay.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL oay4_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_oay.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL oay4_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL oay4_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION oay4_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION oay4_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION oay4_qry_accept(pi_sel_index)
