...
# Modify........: No.FUN-740171 07/04/25 By kim 按取消回傳值傳錯
...
DEFINE   ms_oldvalue      STRING     #FUN-740171
DEFINE   ms_ret1          STRING,    #回傳欄位的變數
         ms_rvu01         LIKE rvu_file.rvu01,
         ms_no            LIKE rvb_file.rvb01
 
FUNCTION q_rvb7(pi_multi_sel,pi_need_cons,ps_default1,p_rvu01,p_no)
   DEFINE   pi_multi_sel   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
            pi_need_cons   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
            ps_default1    STRING  , #預設回傳值(在取消時會回傳此類預設值). => 錯誤,取消時應該回傳p_rvu01才對 #FUN-740171
            p_rvu01        LIKE rvu_file.rvu01,
            p_no           LIKE rvb_file.rvb01 
 
 
   LET ms_default1 = ps_default1
   LET ms_rvu01    = p_rvu01
   LET ms_no       = p_no
   LET ms_oldvalue = p_rvu01 #FUN-740171  在取消時會回傳此預設值
 
   WHENEVER ERROR CALL cl_err_msg_log
 
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_rvb7" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
 
   CALL cl_ui_locale("q_rvb7")
   ...
   IF (mi_multi_sel) THEN
      CALL cl_set_comp_font_color("rvb02", "red")
   END IF
...
FUNCTION rvb7_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            ...
            IF g_sma.sma120 = 'Y' THEN
               CALL rvb7_multi_cons()   #調用自定義的CONSTRUCT過程
            ...
            ELSE         
              ...
              #No.FUN-550131 Add End
               ...
               CONSTRUCT ms_cons_where ON rvb02,rvb04,rvb03,rvb05,rvb30
                                     FROM s_rvb[1].rvb02,s_rvb[1].rvb04,
                                          s_rvb[1].rvb03,s_rvb[1].rvb05,
                                          s_rvb[1].rvb30
      ...
      CALL rvb7_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL rvb7_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL rvb7_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION rvb7_qry_prep_result_set()
   ...
   IF ( g_sma.sma120 = 'N' )OR(( g_sma.sma120 = 'Y')AND( g_multi_where.getLength() = 0 ))THEN
      ...
      LET ls_sql=" SELECT 'N',rvb02,rvb04,rvb03,rvb05,",
                 "'','','','','','','','','','','','','','','','',",  #FUN-550131
                 "'','','','','',",                                   #FUN-550131 Add      
                 "rvb30",
                 "   FROM rvb_file",
   ...
   ELSE 
      LET ls_sql=" SELECT 'N',rvb02,rvb04,rvb03,rvb05,",
                 ...
                 "'','','','','',",                                   #FUN-550131 Add      
                 "rvb30",
                 "   FROM rvb_file",g_multi_join,
                 "  WHERE ",g_multi_where CLIPPED
   END IF 
   #FUN-550131 Modified End    
                            
   IF NOT mi_multi_sel THEN
      LET ls_where = " AND rvb01='",ms_default1,"'"
   END IF
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_rvb7', 'rvb_file')
   IF NOT cl_null(l_filter_cond) THEN
      LET ms_cons_where = ms_cons_where,l_filter_cond
   END IF
   #End:FUN-980030
   LET ls_sql = ls_sql CLIPPED,ls_where CLIPPED," ORDER BY rvb02"
  ...
  ##NO.FUN-980025 GP5.2 add--end
   ...
   FOREACH lcurs_qry INTO lr_qry.*
      ...
      SELECT SUM(rvv17) INTO l_rvv17 FROM rvv_file
       WHERE rvv04 = ms_no 
         AND rvv05 = lr_qry.rvb02 
         AND rvv03 = '3'  
        AND rvv01!= ms_rvu01      
      IF cl_null(l_rvv17) THEN
         LET l_rvv17 = 0
      END IF
      LET lr_qry.rvb30 = lr_qry.rvb30 - l_rvv17
      IF lr_qry.rvb30 < 0 THEN
         LET lr_qry.rvb30 = 0
      END IF   
...
# Modify        :
##################################################
FUNCTION rvb7_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION rvb7_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   #INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_rvb.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE)            #FUN-880082 mark
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_rvb.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      BEFORE INPUT
         ...
         END IF
      ON ACTION prevpage
         CALL GET_FLDBUF(s_rvb.check) RETURNING ma_qry_tmp[ARR_CURR()].check
         CALL rvb7_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_rvb.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL rvb7_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL rvb7_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
      ...
      ON ACTION cancel
         ...
         IF (NOT mi_multi_sel) THEN
           #LET ms_ret1 = ms_default1 #FUN-740171
            LET ms_ret1 = ms_oldvalue #FUN-740171
         END IF
...
# Modify   	:
##################################################
FUNCTION rvb7_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION rvb7_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
   ...
   DISPLAY ARRAY ma_qry_tmp TO s_rvb.*
      BEFORE DISPLAY
         CALL cl_set_act_visible("prevpage,nextpage,reconstruct",TRUE)
         IF (ps_hide_act IS NOT NULL) THEN   
            CALL cl_set_act_visible(ps_hide_act, FALSE)
...
# Modify   	:
##################################################
FUNCTION rvb7_qry_accept(pi_sel_index)
      ...
      FOR li_i = 1 TO ma_qry.getLength()
         IF (ma_qry[li_i].check = 'Y') THEN
            IF (lsb_multi_sel.getLength() = 0) THEN
               CALL lsb_multi_sel.append(ma_qry[li_i].rvb02 CLIPPED)
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].rvb02 CLIPPED)
            END IF
         END IF    
   ...
   ELSE
      LET ms_ret1 = ma_qry[pi_sel_index].rvb02 CLIPPED
...
DEFINE
  ...
  #填充字段名稱數組
  #rvb02,rvb04,rvb03,rvb05,rvb30
  CALL la_field.clear()
  LET la_field[1] = 'rvb02'
  LET la_field[2] = 'rvb04'
  LET la_field[3] = 'rvb03'
  LET la_field[4] = 'rvb05'
  LET la_field[27] = 'rvb30'
  ...
  END FOR 
  #設置獨特的屬性
  LET ma_multi_rec[4].imafld = 'Y'
  #rvb02,rvb03,rvb30是NUMBER型
  LET ma_multi_rec[1].dbtype  = 'NUMBER'
  LET ma_multi_rec[3].dbtype  = 'NUMBER'
  LET ma_multi_rec[27].dbtype = 'NUMBER'
  
  #這裡要手工制定li_i，因為輔助欄位是插在中間而非添加到最後
  LET li_i = 5  #規則:如果第n個欄位是料件編號欄位則li_i為n+1
  #逐項添加信息(21個輔助欄位)
  LET ma_multi_rec[li_i].index = li_i
...
FUNCTION rvb7_multi_cons()
  ...
  CALL cl_set_comp_entry('rvb02,rvb04,rvb03,rvb05',TRUE) #rvb30數據庫必要欄位，所以這裡不能開放其輸入權限
  ...
  INPUT ARRAY ma_cons_lif WITHOUT DEFAULTS FROM s_rvb.* 
    ...
    #信息以及顯示可能會有的明細屬性列  
    AFTER FIELD rvb02,rvb04,rvb03,rvb05,rvb30
      CALL rvb7_after_normal_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
    ...
    AFTER FIELD att01,att02,att03,att04,att05,att06,att07,att08,att09,att10,
      ...
      CALL rvb7_after_detail_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
  ...
  ELSE 
     ...
     CALL rvb7_make_sql_condition()   #這個市cl_create_qry中定義的標準函數
  END IF  
  ...
  CALL cl_set_comp_entry('rvb02,rvb04,rvb03,rvb05,rvb30',FALSE)
...
#明細屬性欄位以及相應的處理過程
#傳入pname為欄位的colname
FUNCTION rvb7_after_normal_field(pname,pvalue,prow)
  DEFINE pname        STRING,
         pvalue       STRING,
...
#當前內容到對應的后台數組中
FUNCTION rvb7_after_detail_field(pname,pvalue,pi)
  DEFINE pname         STRING
...
FUNCTION rvb7_make_sql_condition()  
  ...
  FOR li_n = 1 to li_rowcount
      ...
      FOR li_i = 1 TO ma_multi_rec.getLength() 
          ...
          #如果當前欄位已經是'rvb30'(即最後一個標準欄位)則退出當前層次的循環
          IF ma_multi_rec[li_i].colname = 'rvb30' THEN 
             EXIT FOR
          END IF
