...
#
# Program name  : q_m_aag4.4gl
# Description   : 多工廠會計科目編號查詢(顯示帳別)
# Date & Author : FUN-B60082 2011/06/20 BY belle
...
FUNCTION q_m_aag4(pi_multi_sel,pi_need_cons,p_plant,ps_default1,p_aag07,ps_bookno)  #No.FUN-730020   #FUN-990069
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_m_aag4" ATTRIBUTE(STYLE="create_qry") #No.FUN-660161
 
   CALL cl_ui_locale("q_m_aag4")
...
FUNCTION m_aag4_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         CALL m_aag4_qry_prep_result_set() 
      ...
      CALL m_aag4_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL m_aag4_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL m_aag4_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION m_aag4_qry_prep_result_set()
   ...
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_m_aag4', 'aag_file')
   ...
   LET ls_sql = "SELECT 'N',aag00,aag01,aag03,aag02,aag13,aag04,aag07",  #FUN-6C0012
                ...
                " WHERE ",ms_cons_where
   IF NOT mi_multi_sel THEN
      LET ls_where = " AND aag00 = '",ms_bookno,"'"," AND aag09 = 'Y'"   #FUN-B60082
...
# Modify        :
##################################################
FUNCTION m_aag4_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION m_aag4_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   #ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE)            #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL m_aag4_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_aag.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL m_aag4_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL m_aag4_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION m_aag4_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION m_aag4_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION m_aag4_qry_accept(pi_sel_index)
