...
#
# Program name   : q_qcf_2.4gl
# Program ver.   : 1.0 
# Description    : FQC單查詢,排除拆件式工單之FQC單據
# Date & Author  : 2012/11/09 By Elise
# Memo           : 
# Modify.........: No:CHI-CA0011 12/11/09 By Elise 排除拆件式工單之FQC單據
# Modify.........: No:MOD-D30249 13/03/28 By bart qcf_qry_accept改成qcf_qry_accept_2
DATABASE ds
...
FUNCTION q_qcf_2(pi_multi_sel,pi_need_cons,ps_default1,ps_type)
...
FUNCTION qcf_qry_sel_2()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         CALL qcf_qry_prep_result_set_2() 
      ...
      CALL qcf_qry_set_display_data_2(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL qcf_qry_input_array_2(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL qcf_qry_display_array_2(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION qcf_qry_prep_result_set_2()
   ...
   LET ls_sql = "SELECT 'N',qcf01,qcf02,qcf021,qcf03,qcf091,0,0,0 ", #FUN-C30095 ADD qcf021
               ...
               "   AND qcf02 NOT IN (SELECT sfb01 FROM sfb_file WHERE sfb02='11')"  #CHI-CA0011 add
   CASE ms_type 
...
# Modify        :
##################################################
FUNCTION qcf_qry_set_display_data_2(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION qcf_qry_input_array_2(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_qcf.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL qcf_qry_reset_multi_sel_2(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_qcf.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL qcf_qry_reset_multi_sel_2(pi_start_index, pi_end_index)
            CALL qcf_qry_accept_2(pi_start_index+ARR_CURR()-1)  #MOD-D30249
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION qcf_qry_reset_multi_sel_2(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION qcf_qry_display_array_2(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION qcf_qry_accept_2(pi_sel_index)
