...
#{
# Program name  : q_oea.4gl
# Program ver.  : 7.0
# Description   : 訂單資料查詢
# Date & Author : 2003/09/22 by jack
# Memo          : 
# Modify        : No.FUN-650038 06/05/24 By Sarah 將oeb04_2拿掉,將資料全部顯示在oeb04
...
# Modify.........: No:CHI-A40040 10/04/26 By Summer 加客戶訂單單號(oea10)供篩選
...
DEFINE   ma_qry   DYNAMIC ARRAY OF RECORD
         ...
         oea10    LIKE oea_file.oea10,          #No.CHI-A40040 add           
        #start FUN-650038 modify
        #oeb04_1  VARCHAR(27),                               
        #oeb04_2  VARCHAR(70) 
         oeb04    LIKE type_file.chr1000        #No.FUN-680131 VARCHAR(97)
        #end FUN-650038 modify
END RECORD
...
FUNCTION q_oea(pi_multi_sel,pi_need_cons,ps_default1,p_cus_no,p_type)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_oea" ATTRIBUTE(STYLE="create_qry") #No.FUN-660161
 
   CALL cl_ui_locale("q_oea")
...
FUNCTION oea_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
          IF NOT cl_null(g_cus_no) THEN
            CONSTRUCT ms_cons_where ON oea01, oea00, oea02, oea032, oea14, oea10  #No.CHI-A40040 add oea10
                                  FROM s_oea[1].oea01,s_oea[1].oea00,s_oea[1].oea02,s_oea[1].oea032,s_oea[1].oea14,s_oea[1].oea10 #No.CHI-A40040 add s_oea[1].oea10 
      ...
      CALL oea_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL oea_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL oea_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION oea_qry_prep_result_set()
   ...
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_oea', 'oea_file')
   IF NOT cl_null(l_filter_cond) THEN
      LET ms_cons_where = ms_cons_where,l_filter_cond
   END IF
   #End:FUN-980030
   LET ls_sql = "SELECT 'N',oea01,oea00,oea02,oea032,oea14,oea10,'',''", #No.CHI-A40040 add oea10 
                " FROM oea_file",
                " WHERE ",ms_cons_where
   IF NOT mi_multi_sel THEN
     #MOD-B40099 mod --start--
     #LET ls_where = "   AND oeaconf = 'Y' AND oea00!='0' "
      LET ls_where = "   AND oeaconf = 'Y' "
      IF g_type != '4' THEN                                  
         LET ls_where = ls_where ,"   AND oea00!='0' "
      END IF
     #MOD-B40099 mod --end--
   END IF
...
# Modify        :
##################################################
FUNCTION oea_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION oea_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_oea.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL oea_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_oea.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL oea_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL oea_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION oea_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION oea_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION oea_qry_accept(pi_sel_index)
