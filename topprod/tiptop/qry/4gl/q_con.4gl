...
#{
# Program name  : q_con.4gl
# Program ver.  : 7.0
# Description   : 出口異動資料查詢
...
DEFINE ma_multi_rec DYNAMIC ARRAY OF RECORD
       ...
       dbtype    LIKE type_file.chr1000,#該欄位在數據庫中的數據類型 #TQC-5A0134 VARCHAR->CHAR
...
DEFINE   mi_page_count    LIKE type_file.num10,    #每頁顯現資料筆數.	#No.FUN-680131 INTEGER
         ms_default1    LIKE con_file.con01,
         ms_default2    LIKE con_file.con02,
         ms_default3    LIKE con_file.con013
 DEFINE   ms_custom      LIKE con_file.con08   #No.MOD-490398
DEFINE   ms_ret1          STRING  
DEFINE   l_cob02          LIKE cob_file.cob02
         {%格式為ms_ret+流水號}
 
FUNCTION q_con(pi_multi_sel,pi_need_cons,ps_default1,
                                          ps_default2,
                                           ps_default3,ps_custom) #No.MOD-490398
   DEFINE   pi_multi_sel   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
            pi_need_cons   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
             ps_custom      LIKE con_file.con08,  #No.MOD-490398
            ps_default1    LIKE con_file.con01,
            ps_default2    LIKE con_file.con02,
            ps_default3    LIKE con_file.con013
 
   LET ms_default1 = ps_default1 
   LET ms_default2 = ps_default2 
   LET ms_default3 = ps_default3 
    LET ms_custom   = ps_custom     #No.MOD-490398 
 
   WHENEVER ERROR CALL cl_err_msg_log
 
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_con" ATTRIBUTE(STYLE="create_qry") #No.FUN-660161
 
   CALL cl_ui_locale("q_con")
   ...
   IF (mi_multi_sel) THEN
      CALL cl_set_comp_font_color("con02", "red")
   END IF
...
FUNCTION con_qry_sel()
   ...
   WHILE TRUE
      ...
      LET ls_hide_act = ""
       ...
       DISPLAY ms_default1,ms_default3,ms_custom TO com01,com02,com03  #No.MOD-490398
      SELECT cob02 INTO l_cob02 FROM cob_file
       WHERE cob01 = ms_default1
      IF STATUS THEN LET l_cob02 = '' END IF
      DISPLAY l_cob02 TO cob02
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            ...
            IF g_sma.sma120 = 'Y' THEN
               CALL con_multi_cons()   #調用自定義的CONSTRUCT過程
            ...
            ELSE
              ...
              #No.FUN-550131 Add End
               ...
               CONSTRUCT ms_cons_where 
                      ON con02, con03, con04, con05, con06
                    FROM s_con[1].con02,s_con[1].con03,s_con[1].con04,
                         s_con[1].con05,s_con[1].con06
      ...
      CALL con_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL con_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL con_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION con_qry_prep_result_set()
   ...
   IF ( g_sma.sma120 = 'N' )OR(( g_sma.sma120 = 'Y')AND( g_multi_where.getLength() = 0 ))THEN       
      ...
      #Begin:FUN-980030
      LET l_filter_cond = cl_get_extra_cond_for_qry('q_con', 'con_file')
      IF NOT cl_null(l_filter_cond) THEN
         LET ms_cons_where = ms_cons_where,l_filter_cond
      END IF
      #End:FUN-980030
      LET ls_sql = "SELECT 'N',con02,con03,",
                   "'','','','','','','','','','','','','','','','',",  #FUN-550131
                   "'','','','','',",                                   #FUN-550131 Add
                   "con04,con05,con06 ",
                   " FROM con_file ",
                   " WHERE ",ms_cons_where,
                    " AND con08 = '",ms_custom,"'",   #No.MOD-490398
                   " AND con01 = '",ms_default1,"'",
                   " AND con013= '",ms_default3,"'"
   ELSE
      LET ls_sql = "SELECT 'N',con02,con03,",
                   "'','','','','','','','','','','','','','','','',",  #FUN-550131
                   "'','','','','',",                                   #FUN-550131 Add
                   "con04,con05,con06 ",
                   " FROM con_file ",g_multi_join,
                   " WHERE ",g_multi_where,
                    " AND con08 = '",ms_custom,"'",   #No.MOD-490398
                   " AND con01 = '",ms_default1,"'",
                   " AND con013= '",ms_default3,"'"   	
   ...
   LET ls_sql = ls_sql CLIPPED,ls_where CLIPPED," ORDER BY con02"
...
# Modify        :
##################################################
FUNCTION con_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION con_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_con.* 
      ...
      BEFORE INPUT
         ...
         END IF
      ON ACTION prevpage
         CALL GET_FLDBUF(s_con.check) RETURNING ma_qry_tmp[ARR_CURR()].check
         CALL con_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_con.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL con_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL con_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
      ...
      ON ACTION cancel
         ...
         IF (NOT mi_multi_sel) THEN
             LET ms_ret1 = ms_default2   #No.MOD-490398
         END IF
...
# Modify   	:
##################################################
FUNCTION con_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION con_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
   ...
   DISPLAY ARRAY ma_qry_tmp TO s_con.*
      BEFORE DISPLAY
         CALL cl_set_act_visible("prevpage,nextpage,reconstruct",TRUE)
         IF (ps_hide_act IS NOT NULL) THEN   
            CALL cl_set_act_visible(ps_hide_act, FALSE)
...
# Modify   	:
##################################################
FUNCTION con_qry_accept(pi_sel_index)
      ...
      FOR li_i = 1 TO ma_qry.getLength()
         IF (ma_qry[li_i].check = 'Y') THEN
            IF (lsb_multi_sel.getLength() = 0) THEN
               CALL lsb_multi_sel.append(ma_qry[li_i].con02 CLIPPED)
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].con02 CLIPPED)
            END IF
         END IF    
   ...
   ELSE
      LET ms_ret1 = ma_qry[pi_sel_index].con02 CLIPPED
...
DEFINE
  ...
  CALL la_field.clear()
  LET la_field[1]  = 'con02'
  LET la_field[2]  = 'con03'
  LET la_field[24] = 'con04'
  LET la_field[25] = 'con05'
  LET la_field[26] = 'con06'
  ...
  END FOR 
  #設置獨特的屬性
  LET ma_multi_rec[2].imafld = 'Y'
  #con02,con05,con06是NUMBER型
  LET ma_multi_rec[1].dbtype  = 'NUMBER'
  LET ma_multi_rec[25].dbtype = 'NUMBER'
  LET ma_multi_rec[26].dbtype = 'NUMBER'
    
  #這裡要手工制定li_i，因為輔助欄位是插在中間而非添加到最後
  LET li_i = 3  #規則:如果第n個欄位是料件編號欄位則li_i為n+1
  #逐項添加信息(21個輔助欄位)
  LET ma_multi_rec[li_i].index = li_i
...
FUNCTION con_multi_cons()
  ...
  CALL cl_set_comp_entry('con02,con03,con04,con05,con06',TRUE)
  ...
  INPUT ARRAY ma_cons_lif WITHOUT DEFAULTS FROM s_con.* 
    ...
    #信息以及顯示可能會有的明細屬性列  
    AFTER FIELD con02,con03,con04,con05,con06
      CALL con_after_normal_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
    ...
    AFTER FIELD att01,att02,att03,att04,att05,att06,att07,att08,att09,att10,
      ...
      CALL con_after_detail_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
  ...
  ELSE 
     ...
     CALL con_make_sql_condition()   #這個市cl_create_qry中定義的標準函數
  END IF  
  ...
  CALL cl_set_comp_entry('con02,con03,con04,con05,con06',FALSE)
...
#明細屬性欄位以及相應的處理過程
#傳入pname為欄位的colname
FUNCTION con_after_normal_field(pname,pvalue,prow)
  DEFINE pname        STRING,
         pvalue       STRING,
...
#當前內容到對應的后台數組中
FUNCTION con_after_detail_field(pname,pvalue,pi)
  DEFINE pname         STRING
...
FUNCTION con_make_sql_condition()  
  ...
  FOR li_n = 1 to li_rowcount
      ...
      FOR li_i = 1 TO ma_multi_rec.getLength() 
          ...
          #如果當前欄位已經是'con06'(即最後一個標準欄位)則退出當前層次的循環
          IF ma_multi_rec[li_i].colname = 'con06' THEN 
             EXIT FOR
          END IF
