...
#
# Program name  : q_smy4.4gl
# Program ver.  : 7.0
# Description   : 單據性質查詢
# Date & Author : 2012/03/19 by Downheal
# Memo          : 以q_smy.4gl修改而成, 固定p_kind參數為1,2,3
# Modify        : NO.FUN-C20103 2012/03/19 By Downheal 配合p_cr_apr中aimr300單別開窗
...
FUNCTION q_smy4(pi_multi_sel,pi_need_cons,ps_default1,p_sys,p_kind)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_smy4" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
...
#TQC-970230 --end-- 
      ...
      CALL smy4_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL smy4_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL smy4_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION smy4_qry_prep_result_set()
   ...
   IF NOT mi_multi_sel THEN
      ...
      #FUN-C20103 傳入固定skykind參數
      LET ls_where = ls_where CLIPPED," AND (smykind ='1' OR smykind ='2' OR smykind ='3')" 
   END IF
...
# Modify        :
##################################################
FUNCTION smy4_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION smy4_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_smy.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED)   #No.FUN-870043
      ...
      ON ACTION prevpage
         ...
         CALL smy4_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_smy.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL smy4_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL smy4_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION smy4_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION smy4_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION smy4_qry_accept(pi_sel_index)
...
#No.FUN-950021  --Begin
FUNCTION smy4_qry_set_par_where(p_where)
