...
#
# Program name  : q_sgm2.4gl
# Program ver.  : 7.0
# Description   : Run Card資料查詢
# Date & Author : 2004/05/05 by Pengu
# Memo          : 
# Modify        :
# Modify.........: No.MOD-640338 06/04/010 By pengu Run Card欄位查詢時,不應出現無WIP量的Run Card.
...
FUNCTION q_sgm2(pi_multi_sel,pi_need_cons,p_key1,ps_default2)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_sgm2" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
 
   CALL cl_ui_locale("q_sgm2")
...
FUNCTION sgm2_qry_sel()
   ...
   WHILE TRUE
      ...
      CALL sgm2_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL sgm2_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL sgm2_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION sgm2_qry_prep_result_set()
   ...
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_sgm2', 'sgm_file')
   ...
   LET ls_sql = "SELECT  'N',sgm01,sgm04,sgm45,'',sgm06,",
                ...
                "        sgm314,sgm316,sgm317,sgm321,sgm322,sgm59 ",
                "  FROM sgm_file",
  ...
  ##NO.FUN-980025 GP5.2 add--end
   ...
   FOREACH lcurs_qry INTO lr_qry.*,l_sgm54 ,l_sgm291,l_sgm301,l_sgm302,
      ...
      IF l_sgm54='Y' THEN          # Check in='Y'
         LET lr_qry.wipqty 
                             =  l_sgm291             #check in
                              - l_sgm311*l_sgm59     #良品轉出
                              - l_sgm312*l_sgm59     #重工轉出
                              - l_sgm313*l_sgm59     #當站報廢
                              - l_sgm314*l_sgm59     #當站下線
                              - l_sgm316*l_sgm59
                              - l_sgm317*l_sgm59
#                             - l_sgm321             #委外加工量
#                             + l_sgm322             #委外完工量
...
# Modify        :
##################################################
FUNCTION sgm2_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION sgm2_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_sgm.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL sgm2_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_sgm.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL sgm2_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL sgm2_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION sgm2_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION sgm2_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION sgm2_qry_accept(pi_sel_index)
