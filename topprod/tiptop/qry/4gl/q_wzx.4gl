...
#
# Program name   : q_wzx.4gl
# Program ver.   : 3.0
# Description    : 
# Date & Author  : 2009/12/28 By hongmei(FUN-9A0065)
...
DEFINE   ma_qry   DYNAMIC ARRAY OF RECORD
         ...
         wzx01    LIKE zx_file.zx01,
         wzx02    LIKE zx_file.zx02
END RECORD
...
FUNCTION q_wzx(pi_multi_sel,pi_need_cons,p_plant)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_wzx" ATTRIBUTE(STYLE="create_qry") 
 
   CALL cl_ui_locale("q_wzx")
   ...
   IF (mi_multi_sel) THEN #在複選狀態時,要將回傳欄位的字型顏色設為紅色,以作為標示.
      CALL cl_set_comp_font_color("wzx01", "red")
   END IF
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            ...
            CONSTRUCT ms_cons_where ON wzx01,wzx02 
                                   FROM s_wzx[1].wzx01,s_wzx[1].wzx02
         ...
         CALL m_wzx_qry_prep_result_set() 
      ...
      CALL m_wzx_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL m_wzx_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL m_wzx_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION m_wzx_qry_prep_result_set()
   ...
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_wzx', 'wds.wzx_file')
   IF NOT cl_null(l_filter_cond) THEN
      LET ms_cons_where = ms_cons_where,l_filter_cond
   END IF  
 
   LET ls_sql = "SELECT 'N',wzx01,wzx02 ",
                "  FROM wds.wzx_file ",
                " WHERE wzxacti = 'Y' ",
                " ORDER BY wzx01 "   
...
# Modify        :
##################################################
FUNCTION m_wzx_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION m_wzx_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_wzx.* 
   ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED)
      BEFORE INPUT
         ...
         END IF
      ON ACTION prevpage
         CALL GET_FLDBUF(s_wzx.check) RETURNING ma_qry_tmp[ARR_CURR()].check
         CALL m_wzx_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN      
            CALL GET_FLDBUF(s_wzx.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL m_wzx_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL m_wzx_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        
            LET ms_ret1 = NULL     
         END IF                     
...
# Modify   	:
##################################################
FUNCTION m_wzx_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 
            pi_end_index     LIKE type_file.num10 
   DEFINE   li_i             LIKE type_file.num10,
            li_j             LIKE type_file.num10 
...
# Modify   	:
##################################################
FUNCTION m_wzx_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 
            pi_end_index     LIKE type_file.num10 
   DEFINE   li_continue      LIKE type_file.num5,
   ...
   DISPLAY ARRAY ma_qry_tmp TO s_wzx.*
      BEFORE DISPLAY
         CALL cl_set_act_visible("prevpage,nextpage,reconstruct",TRUE)
         IF (ps_hide_act IS NOT NULL) THEN   
            CALL cl_set_act_visible(ps_hide_act, FALSE)
...
# Modify   	:
##################################################
FUNCTION m_wzx_qry_accept(pi_sel_index)
      ...
      FOR li_i = 1 TO ma_qry.getLength()
         IF (ma_qry[li_i].check = 'Y') THEN
            IF (lsb_multi_sel.getLength() = 0) THEN
               CALL lsb_multi_sel.append(ma_qry[li_i].wzx01 CLIPPED) 
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].wzx01 CLIPPED)  
            END IF
         END IF    
   ...
   ELSE
      LET ms_ret1 = ma_qry[pi_sel_index].wzx01 CLIPPED
      LET ms_ret2 = ma_qry[pi_sel_index].wzx02 CLIPPED
   END IF
END FUNCTION
#FUN-9A0065---end
