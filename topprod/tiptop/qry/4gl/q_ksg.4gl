...
#{
# Program name  : q_ksg.4gl
# Program ver.  : 7.0
# Description   : 製造通知單查詢
# Date & Author : 2004/02/19 by saki
# Memo          : 
...
# Modify........: No.FUN-650117 06/05/24 By Sarah IF noqty<0 THEN noqty=0
# Modify........: No.FUN-660161 06/07/07 By cl  增加匯出Excel功能
# Modify........: No.FUN-660119 06/08/29 By Sarah 排除已結案(ksg09!='Y')的資料
# Modify........: No.FUN-680131 06/09/01 By Carrier 欄位型態用LIKE定義
# Modify........: No.CHI-690081 06/10/31 By Judy hardcode程序退出將INT_FLAG置為0
# Modify........: No.TQC-740137 07/04/19 By Carrier 過濾未確認的通知單
...
# Modify.........: No:TQC-AB0085 10/11/26 By destiny 已开工单数量计算有误，会重复计算
# Modify........: No:TQC-D10023 13/01/06 By qirl 增加規格
...
DEFINE   ma_qry   DYNAMIC ARRAY OF RECORD
         ...
         ksg01    LIKE ksg_file.ksg01,
         ksg02    LIKE ksg_file.ksg02,
         ksg03    LIKE ksg_file.ksg03,
	 #FUN-550131 Add Start
	 attdesc      LIKE type_file.chr1000,#TQC-5A0134 VARCHAR->CHAR	#No.FUN-680131 VARCHAR(500)
         ...
         ima021   LIKE ima_file.ima021,   #TQC-D10023--add
         ksg05    LIKE ksg_file.ksg05,
         sfb08    LIKE sfb_file.sfb08,
         noqty    LIKE ksg_file.ksg05
END RECORD
...
FUNCTION q_ksg(pi_multi_sel,pi_need_cons,ps_default1,ps_default2)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_ksg" ATTRIBUTE(STYLE="create_qry") #No.FUN-660161
 
   CALL cl_ui_locale("q_ksg")
   ...
   IF (mi_multi_sel) THEN
      CALL cl_set_comp_font_color("ksg01", "red")
   END IF
...
FUNCTION ksg_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            ...
            IF g_sma.sma120 = 'Y' THEN
               CALL ksg_multi_cons()   #調用自定義的CONSTRUCT過程
            ...
            ELSE
              ...
              #No.FUN-550131 Add End
               ...
               {CONSTRUCT ms_cons_where ON ksg01,ksg02,ksg03,ima02,ksg05,sfb08,noqty
                                     FROM s_ksg[1].ksg01,s_ksg[1].ksg02,
                                          s_ksg[1].ksg03,s_ksg[1].ima02,
                                          s_ksg[1].ksg05,s_ksg[1].sfb08, 
                                          s_ksg[1].noqty}
               #FUN-550131 上面一段代碼由問題,其中包含了ima02,sfb08和noqty三個FORMONLY的
               #欄位這些欄位雖然可以下條件，但是在組sql是並沒有指定相應的table，所以會導致
               #查詢出錯，不知道臺灣的工程師為什麼會這樣來寫，我只是出於我對程式的理解來改了
               CONSTRUCT ms_cons_where ON ksg01,ksg02,ksg03,ksg05
                                     FROM s_ksg[1].ksg01,s_ksg[1].ksg02,
                                          s_ksg[1].ksg03,s_ksg[1].ksg05
      ...
      CALL ksg_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL ksg_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL ksg_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION ksg_qry_prep_result_set()
   ...
   IF ( g_sma.sma120 = 'N' )OR(( g_sma.sma120 = 'Y')AND( g_multi_where.getLength() = 0 ))THEN
      ...
      #Begin:FUN-980030
      LET l_filter_cond = cl_get_extra_cond_for_qry('q_ksg', 'ksg_file')
      IF NOT cl_null(l_filter_cond) THEN
         LET ms_cons_where = ms_cons_where,l_filter_cond
      END IF
      #End:FUN-980030
      LET ls_sql = "SELECT 'N',ksg01,ksg02,ksg03,'',",
                   "'','','','','','','','','','','','','','','','',",  #FUN-550131
             #     "'','','','','',",                                   #FUN-550131 Add
                   "'','','','','',   '',",   #TQC-D10023--add''
                   "ksg05,0,0", 
                   " FROM ksg_file,ksf_file",      #No.TQC-740137
                   " WHERE ",ms_cons_where CLIPPED, 
                   " AND ksf01 = ksg01 ",          #No.TQC-740137
                   " AND ksfconf = 'Y' ",          #No.TQC-740137
                   " AND ksg09!='Y'",              #FUN-660119 add
                   " ORDER BY ksg01,ksg02,ksg03"   #MOD-490471 
   ELSE
      LET ls_sql = "SELECT 'N',ksg01,ksg02,ksg03,'',",
                   "'','','','','','','','','','','','','','','','',",  #FUN-550131
             #     "'','','','','',",                                   #FUN-550131 Add
                   "'','','','','',   '',",   #TQC-D10023--add''
                   "ksg05,0,0", 
                   " FROM ksg_file,ksf_file",g_multi_join,  #No.TQC-740137
                   " WHERE ",g_multi_where CLIPPED, 
                   " AND ksf01 = ksg01 ",          #No.TQC-740137
                   " AND ksfconf = 'Y' ",          #No.TQC-740137
                   " AND ksg09!='Y'",              #FUN-660119 add
                   " ORDER BY ksg01,ksg02,ksg03"   #MOD-490471    	
...
#TQC-D10023--add--ima021--star--
      SELECT ima02,ima021 INTO lr_qry.ima02,lr_qry.ima021 FROM ima_file
       WHERE ima01 = lr_qry.ksg03
      IF SQLCA.sqlcode THEN
         LET lr_qry.ima02 = ' '
         LET lr_qry.ima021 = ' '
      END IF
#TQC-D10023--add--ima021--end--
      #No.B377 010510 add by linda 讀取已開工單量
      #No.TQC-AB0085--begin
      #SELECT SUM(sfb08) INTO lr_qry.sfb08
      #  FROM sfb_file
      # WHERE sfb91 = lr_qry.ksg01 AND sfb92 = lr_qry.ksg02
      #   AND sfb91 IS NOT NULL AND sfb87! = 'X'
      LET l_sql=" SELECT SUM(sfb08) FROM sfb_file WHERE sfb91 = '",lr_qry.ksg01,"' AND sfb92 = '",lr_qry.ksg02,"' ",
                " AND sfb91 IS NOT NULL AND sfb87! = 'X' AND sfb05= '",lr_qry.ksg03,"' "    
      PREPARE ksg_qry_pre FROM l_sql
      EXECUTE ksg_qry_pre INTO lr_qry.sfb08
      #No.TQC-AB0085--end
      IF cl_null(lr_qry.sfb08) THEN 
         LET lr_qry.sfb08=0
      END IF
      LET lr_qry.noqty=lr_qry.ksg05 - lr_qry.sfb08
      IF lr_qry.noqty < 0 THEN LET lr_qry.noqty = 0 END IF   #FUN-650117 add
...
# Modify        :
##################################################
FUNCTION ksg_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION ksg_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   #INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_ksg.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE)            #FUN-880082 mark
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_ksg.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      BEFORE INPUT
         ...
         END IF
      ON ACTION prevpage
         CALL GET_FLDBUF(s_ksg.check) RETURNING ma_qry_tmp[ARR_CURR()].check
         CALL ksg_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_ksg.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL ksg_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL ksg_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION ksg_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION ksg_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
   ...
   DISPLAY ARRAY ma_qry_tmp TO s_ksg.*
      BEFORE DISPLAY
         CALL cl_set_act_visible("prevpage,nextpage,reconstruct",TRUE)
         IF (ps_hide_act IS NOT NULL) THEN   
            CALL cl_set_act_visible(ps_hide_act, FALSE)
...
# Modify   	:
##################################################
FUNCTION ksg_qry_accept(pi_sel_index)
      ...
      FOR li_i = 1 TO ma_qry.getLength()
         IF (ma_qry[li_i].check = 'Y') THEN
            IF (lsb_multi_sel.getLength() = 0) THEN
               CALL lsb_multi_sel.append(ma_qry[li_i].ksg01 CLIPPED)
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].ksg01 CLIPPED)
            END IF
         END IF    
   ...
   ELSE
      LET ms_ret1 = ma_qry[pi_sel_index].ksg01 CLIPPED
      LET ms_ret2 = ma_qry[pi_sel_index].ksg02 CLIPPED
   END IF
END FUNCTION
...
DEFINE
  ...
  #填充字段名稱數組
  #ksg01,ksg02,ksg03,ima02,ksg05,sfb08,noqty
  CALL la_field.clear()
  LET la_field[1] = 'ksg01'
  LET la_field[2] = 'ksg02'
  LET la_field[3] = 'ksg03'
  LET la_field[25] = 'ima02'
  LET la_field[26] = 'ksg05'
  LET la_field[27] = 'sfb08'
  LET la_field[28] = 'noqty'
  ...
  END FOR 
  #設置獨特的屬性
  LET ma_multi_rec[3].imafld = 'Y'
  #ksg02,ksg05,sfb08,noqty是NUMBER型
  LET ma_multi_rec[2].dbtype = 'NUMBER'
  LET ma_multi_rec[26].dbtype = 'NUMBER'
  LET ma_multi_rec[27].dbtype = 'NUMBER'
  LET ma_multi_rec[28].dbtype = 'NUMBER'
  
  #這裡要手工制定li_i，因為輔助欄位是插在中間而非添加到最後
  LET li_i = 4  #規則:如果第n個欄位是料件編號欄位則li_i為n+1  
  #逐項添加信息(21個輔助欄位)
  LET ma_multi_rec[li_i].index = li_i
...
FUNCTION ksg_multi_cons()
  ...
  CALL cl_set_comp_entry('ksg01,ksg02,ksg03,ksg05',TRUE) #不允許對ima02,sfb08和noqty三個FORMONLY欄位下條件
  ...
  INPUT ARRAY ma_cons_lif WITHOUT DEFAULTS FROM s_ksg.* 
    ...
    #信息以及顯示可能會有的明細屬性列  
    AFTER FIELD ksg01,ksg02,ksg03,ksg05 #不允許對ima02,sfb08和noqty三個FORMONLY欄位下條件
      CALL ksg_after_normal_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
    ...
    AFTER FIELD att01,att02,att03,att04,att05,att06,att07,att08,att09,att10,
      ...
      CALL ksg_after_detail_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
  ...
  ELSE 
     ...
     CALL ksg_make_sql_condition()   #這個市cl_create_qry中定義的標準函數
  END IF  
  ...
  CALL cl_set_comp_entry('ksg01,ksg02,ksg03,ksg05',FALSE) #不允許對ima02,sfb08和noqty三個FORMONLY欄位下條件
...
#明細屬性欄位以及相應的處理過程
#傳入pname為欄位的colname
FUNCTION ksg_after_normal_field(pname,pvalue,prow)
  DEFINE pname        STRING,
         pvalue       STRING,
         prow         LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
         pvalue_tmp   LIKE type_file.chr1000,#TQC-5A0134 VARCHAR->CHAR	#No.FUN-680131 VARCHAR(500)
...
#當前內容到對應的后台數組中
FUNCTION ksg_after_detail_field(pname,pvalue,pi)
  DEFINE pname         STRING
...
FUNCTION ksg_make_sql_condition()  
  ...
  FOR li_n = 1 to li_rowcount
      ...
      FOR li_i = 1 TO ma_multi_rec.getLength() 
          ...
          #如果當前欄位已經是'noqty'(即最後一個標準欄位)則退出當前層次的循環
          IF ma_multi_rec[li_i].colname = 'noqty' THEN 
             EXIT FOR
          END IF
