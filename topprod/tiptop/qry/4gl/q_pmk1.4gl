...
# Modify.........: No:TQC-BB0099 11/11/10 by destiny 过滤掉未审核资料
...
FUNCTION q_pmk1(pi_multi_sel,pi_need_cons,ps_default1,p_sta,p_type)
...
FUNCTION pmk1_qry_sel()
   ...
   WHILE TRUE
      ...
      CALL pmk1_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL pmk1_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL pmk1_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION pmk1_qry_prep_result_set()
   ...
   IF cl_null(ms_sta) THEN
      ...
      #Begin:FUN-980030
      LET l_filter_cond = cl_get_extra_cond_for_qry('q_pmk1', 'pmk_file')
      IF NOT cl_null(l_filter_cond) THEN
         LET ms_cons_where = ms_cons_where,l_filter_cond
      END IF
      #End:FUN-980030
      LET ls_sql = "SELECT 'N',pmk01,pmk02,pmk04,pmk09,pmk25,pmk18,pmkmksg ",
                   " FROM pmk_file",
                   " WHERE  ",ms_cons_where
     #LET ls_where="   AND pmk18!='X' ",    #TQC-BB0099 
      LET ls_where="   AND pmk18 ='Y' ",    #TQC-BB0099
                   " AND pmkacti IN ('Y','y') "
   ELSE
      LET ls_sql = " SELECT 'N',pmk01,pmk02,pmk04,pmk09,pmk25,pmk18,pmkmksg",
                   " FROM pmk_file",
                   " WHERE ", ms_cons_where
      LET ls_where=" AND pmk18!='X' ",
                   " AND pmkacti IN ('Y','y') ",
  ...
  ##NO.FUN-980025 GP5.2 add--end
   ...
   FOREACH lcurs_qry INTO lr_qry.*,l_pmk18,l_pmkmksg
      ...
      IF lr_qry.pmk25 = '0' AND ms_type = '1' THEN
         IF NOT q_pmk1_check(lr_qry.pmk01) THEN
            INITIALIZE lr_qry.pmk01 TO NULL
            CONTINUE FOREACH
         END IF
      END IF
...
# Modify        :
##################################################
FUNCTION pmk1_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION pmk1_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_pmk.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL pmk1_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_pmk.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL pmk1_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL pmk1_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION pmk1_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION pmk1_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION pmk1_qry_accept(pi_sel_index)
...
FUNCTION  q_pmk1_check(l_pmk01)
   ...
   #是否為'N',且己簽核順序等於應簽核順序方可進行合併
   # IF g_sma.sma46 = 'N' AND   #NO:4426sma46 已無用
