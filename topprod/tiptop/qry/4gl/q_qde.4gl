...
#{
# Program name  : q_qde.4gl
# Program ver.  : 7.0
# Description   : 聯產品資料查詢
# Date & Author : 2003/11/11 by saki
# Memo          : 
...
# Modify.........: No.MOD-750061 07/09/26 By pengu 無法查詢出資料
...
DEFINE   ms_ret1          STRING,    
         ...
         ms_ret4          STRING,
         ms_ret5          STRING
DEFINE   ms_key1          LIKE qde_file.qde01,#FQC單號
         ms_key2          LIKE qde_file.qde02 #工單單號
 
FUNCTION q_qde(pi_multi_sel,pi_need_cons,ps_key1,ps_key2)
   DEFINE   pi_multi_sel   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
            pi_need_cons   LIKE type_file.num5  	#No.FUN-680131 SMALLINT
   DEFINE   ps_key1        LIKE qde_file.qde01,
            ps_key2        LIKE qde_file.qde02
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_qde" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
 
   CALL cl_ui_locale("q_qde")
   ...
   IF (mi_multi_sel) THEN
      CALL cl_set_comp_font_color("qde05", "red")
   END IF
   ...
   IF (mi_multi_sel) THEN
      RETURN ms_ret1 
   ELSE
      RETURN ms_ret1,ms_ret2,ms_ret3,ms_ret4,ms_ret5 
   END IF
END FUNCTION
...
FUNCTION qde_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            ...
            IF g_sma.sma120 = 'Y' THEN
               CALL qde_multi_cons()   #調用自定義的CONSTRUCT過程
            ...
            ELSE        
              ...
              #No.FUN-550131 Add End
               ...
               CONSTRUCT ms_cons_where ON qde05,qde06,qde09,qde10,qde11,
                                          qde12,qde13
                                     FROM s_qde[1].qde05,s_qde[1].qde06,
                                          s_qde[1].qde09,s_qde[1].qde10,
                                          s_qde[1].qde11,s_qde[1].qde12,
                                          s_qde[1].qde13
      ...
      CALL qde_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL qde_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL qde_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION qde_qry_prep_result_set()
   ...
   IF ( g_sma.sma120 = 'N' )OR(( g_sma.sma120 = 'Y')AND( g_multi_where.getLength() = 0 ))THEN
      ...
      #Begin:FUN-980030
      LET l_filter_cond = cl_get_extra_cond_for_qry('q_qde', 'qde_file')
      IF NOT cl_null(l_filter_cond) THEN
         LET ms_cons_where = ms_cons_where,l_filter_cond
      END IF
      #End:FUN-980030
      LET ls_sql = "SELECT 'N',qde05,",
                   "'','','','','','','','','','','','','','','','',",  #FUN-550131
                   "'','','','','',",                                   #FUN-550131 Add      
                   "ima02,qde06,qde09,qde10,qde11,qde12,qde13",
                   "  FROM qde_file,OUTER ima_file",
                  #----------------No.MOD-750061 modify
                  #" WHERE qde05 = ima_file.ima01",ms_cons_where CLIPPED
                   " WHERE qde05 = ima_file.ima01 AND ",ms_cons_where CLIPPED
                  #----------------No.MOD-750061 end
   ELSE 
      LET ls_sql = "SELECT 'N',qde05,",
                   "'','','','','','','','','','','','','','','','',",  #FUN-550131
                   "'','','','','',",                                   #FUN-550131 Add      
                   "ima02,qde06,qde09,qde10,qde11,qde12,qde13",
                   "  FROM qde_file,OUTER ima_file",g_multi_join,
                   " WHERE qde05 = ima_file.ima01 AND ",g_multi_where CLIPPED
   END IF 
   #FUN-550131 Modified End   
   
   IF (NOT mi_multi_sel) THEN
      IF NOT cl_null(ms_key1) THEN
         LET ls_where = ls_where CLIPPED," AND qde01 = '",ms_key1,"'" #FQC單號
      END IF
      IF NOT cl_null(ms_key2) THEN
         LET ls_where = ls_where CLIPPED," AND qde02 = '",ms_key2,"'" #工單單號
      END IF
   END IF
   LET ls_sql = ls_sql CLIPPED,ls_where CLIPPED," ORDER BY qde05"
...
# Modify        :
##################################################
FUNCTION qde_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION qde_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   #INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_qde.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE)            #FUN-880082 mark
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_qde.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      BEFORE INPUT
         ...
         END IF
      ON ACTION prevpage
         CALL GET_FLDBUF(s_qde.check) RETURNING ma_qry_tmp[ARR_CURR()].check
         CALL qde_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_qde.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL qde_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL qde_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION qde_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION qde_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
   ...
   DISPLAY ARRAY ma_qry_tmp TO s_qde.*
      BEFORE DISPLAY
         CALL cl_set_act_visible("prevpage,nextpage,reconstruct",TRUE)
         IF (ps_hide_act IS NOT NULL) THEN   
            CALL cl_set_act_visible(ps_hide_act, FALSE)
...
# Modify   	:
##################################################
FUNCTION qde_qry_accept(pi_sel_index)
      ...
      FOR li_i = 1 TO ma_qry.getLength()
         IF (ma_qry[li_i].check = 'Y') THEN
            IF (lsb_multi_sel.getLength() = 0) THEN
               CALL lsb_multi_sel.append(ma_qry[li_i].qde05 CLIPPED)
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].qde05 CLIPPED)
            END IF
         END IF    
   ...
   ELSE
      LET ms_ret1 = ma_qry[pi_sel_index].qde05 CLIPPED
      LET ms_ret2 = ma_qry[pi_sel_index].qde12 CLIPPED
      LET ms_ret3 = ma_qry[pi_sel_index].qde09 CLIPPED
      LET ms_ret4 = ma_qry[pi_sel_index].qde10 CLIPPED
      LET ms_ret5 = ma_qry[pi_sel_index].qde11 CLIPPED
   END IF
END FUNCTION
...
DEFINE
  ...
  #填充字段名稱數組
  #qde05,ima02,qde06,qde09,qde10,qde11,qde12,qde13
  CALL la_field.clear()
  LET la_field[1] = 'qde05'
  LET la_field[23] = 'ima02'
  LET la_field[24] = 'qde06'
  LET la_field[25] = 'qde09'
  LET la_field[26] = 'qde10'
  LET la_field[27] = 'qde11'
  LET la_field[28] = 'qde12'
  LET la_field[29] = 'qde13'
  ...
  END FOR 
  #設置獨特的屬性
  LET ma_multi_rec[1].imafld = 'Y'  
  #qde06,qde13是NUMBER型
  LET ma_multi_rec[24].dbtype = 'NUMBER'
  LET ma_multi_rec[29].dbtype = 'NUMBER'
  
  #這裡要手工制定li_i，因為輔助欄位是插在中間而非添加到最後
  LET li_i = 2  #規則:如果第n個欄位是料件編號欄位則li_i為n+1
  #逐項添加信息(21個輔助欄位)
  LET ma_multi_rec[li_i].index = li_i
...
FUNCTION qde_multi_cons()
  ...
  CALL cl_set_comp_entry('qde05,qde06,qde09,qde10,qde11,qde12,qde13',TRUE)
  ...
  INPUT ARRAY ma_cons_lif WITHOUT DEFAULTS FROM s_qde.* 
    ...
    #信息以及顯示可能會有的明細屬性列  
    AFTER FIELD qde05,qde06,qde09,qde10,qde11,qde12,qde13
      CALL qde_after_normal_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
    ...
    AFTER FIELD att01,att02,att03,att04,att05,att06,att07,att08,att09,att10,
      ...
      CALL qde_after_detail_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
  ...
  ELSE 
     ...
     CALL qde_make_sql_condition()   #這個市cl_create_qry中定義的標準函數
  END IF  
  ...
  CALL cl_set_comp_entry('qde05,qde06,qde09,qde10,qde11,qde12,qde13',FALSE)
...
#明細屬性欄位以及相應的處理過程
#傳入pname為欄位的colname
FUNCTION qde_after_normal_field(pname,pvalue,prow)
  DEFINE pname        STRING,
         pvalue       STRING,
...
#當前內容到對應的后台數組中
FUNCTION qde_after_detail_field(pname,pvalue,pi)
  DEFINE pname         STRING
...
FUNCTION qde_make_sql_condition()  
  ...
  FOR li_n = 1 to li_rowcount
      ...
      FOR li_i = 1 TO ma_multi_rec.getLength() 
          ...
          #如果當前欄位已經是'qde13'(即最後一個標準欄位)則退出當前層次的循環
          IF ma_multi_rec[li_i].colname = 'qde13' THEN 
             EXIT FOR
          END IF
  ...
  IF g_multi_where = '(1 = 0)' THEN
     LET g_multi_where = ''
  ELSE
     LET g_multi_where ='(',g_multi_where,')'
  END IF
