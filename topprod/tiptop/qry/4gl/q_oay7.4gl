...
FUNCTION q_oay7(pi_multi_sel,pi_need_cons,ps_default1,ps_type,ps_sys)
   ...
   IF g_bgjob='Y' AND g_gui_type = 0 THEN
      ...
      CALL oay7_qry_prep_result_set()
      RETURN
   END IF
   #END FUN-720021
 
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_oay7" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
 
   CALL cl_ui_locale("q_oay7")
...
FUNCTION oay7_qry_sel()
   ...
   WHILE TRUE
      ...
      CALL oay7_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL oay7_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL oay7_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION oay7_qry_prep_result_set()
   ...
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_oay7', 'oay_file')
   ...
   IF (NOT mi_multi_sel) THEN
      	LET ls_where = " AND oaytype = '20' OR oaytype = '22' OR oaytype = '30'",
                        " OR oaytype = '32' OR oaytype = '33' OR oaytype = '34'" 
   END IF
...
# Modify        :
##################################################
FUNCTION oay7_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION oay7_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_oay.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL oay7_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_oay.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL oay7_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL oay7_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION oay7_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION oay7_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION oay7_qry_accept(pi_sel_index)
