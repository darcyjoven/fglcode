...
FUNCTION q_apa5(pi_multi_sel,pi_need_cons,ps_default1)
...
FUNCTION apa5_qry_sel()
   ...
   WHILE TRUE
      ...
      CALL apa5_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL apa5_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL apa5_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION apa5_qry_prep_result_set()
   ...
   IF g_apz.apz27='Y' THEN   #TQC-610006
      ...
      #Begin:FUN-980030
      LET l_filter_cond = cl_get_extra_cond_for_qry('q_apa5', 'apa_file')
      IF NOT cl_null(l_filter_cond) THEN
         LET ms_cons_where = ms_cons_where,l_filter_cond
      END IF
      #End:FUN-980030
      LET ls_sql = "SELECT 'N',apa01,apa06,apa07,apa73,apa20,apa14,apa25 ", 
                   " FROM apa_file",
                   " WHERE ",ms_cons_where
   #-----TQC-610006---------
   ELSE 
      LET ls_sql = "SELECT 'N',apa01,apa06,apa07,apa34-apa35,apa20,apa14,apa25 ", 
                   " FROM apa_file",
                   " WHERE ",ms_cons_where
   END IF
   #-----END TQC-610006-----   
   IF NOT mi_multi_sel THEN
      LET ls_where = "   AND  apa00 matches '2*' ", 
                     "   AND (apa73-apa20*apa14) > 0 ",    #bug no:A064              
                     "   AND  apa42 = 'N' "
   END IF
...
# Modify        :
##################################################
FUNCTION apa5_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION apa5_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_apa.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL apa5_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_apa.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL apa5_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL apa5_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION apa5_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION apa5_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION apa5_qry_accept(pi_sel_index)
