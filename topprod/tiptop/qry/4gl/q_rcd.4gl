...
#
# Program name  : q_rcd.4gl
# Program ver.  : 7.0
# Description   : 
# Date & Author : No.FUN-B50156 2011/04/23 by baogc
...
DEFINE   g_key1           LIKE rcd_file.rcd01 
DEFINE   g_key2           LIKE rcd_file.rcd02 
DEFINE   g_key3           LIKE rcd_file.rcdplant
DEFINE   g_org            LIKE rcd_file.rcdplant
        
DEFINE   l_chr            LIKE type_file.chr1 
DEFINE   m_rcd01          LIKE rcd_file.rcd01

FUNCTION q_rcd(pi_multi_sel,pi_need_cons,p_plant,ps_default1,ps_default2,ps_default3)
   DEFINE   pi_multi_sel   LIKE type_file.num5,  	
            pi_need_cons   LIKE type_file.num5,  
            p_plant        LIKE rcd_file.rcdplant,
            ps_default1    STRING,  #預設回傳值(在取消時會回傳此類預設值).
            ps_default2    STRING,  #預設回傳值(在取消時會回傳此類預設值).
            ps_default3    STRING   #預設回傳值(在取消時會回傳此類預設值). 

   LET m_rcd01 = ps_default1
   LET g_org = p_plant
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_rcd" ATTRIBUTE(STYLE="create_qry") 
 
   CALL cl_ui_locale("q_rcd")
   ...
   IF (mi_multi_sel) THEN
      CALL cl_set_comp_font_color("rcd01", "red")
   END IF   
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (li_continue) AND (li_reconstruct) AND (l_chr = 'Y') THEN 
            CALL rcd01_construct()   
         END IF  
         CALL rcd01_qry_prep_result_set() 
      ...
      CALL rcd01_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL rcd01_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL rcd01_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
FUNCTION rcd01_construct()
  CLEAR FORM
  CALL ma_qry_tmp.clear()
  CONSTRUCT ms_cons_where ON rcd01,rcd02,rcdplant
       FROM s_rcd[1].rcd01,s_rcd[1].rcd02,s_rcd[1].rcdplant
...
# Modify        :
##################################################
FUNCTION rcd01_qry_prep_result_set()
   ...
   DEFINE l_plant    LIKE azw_file.azw01
    LET l_filter_cond = cl_get_extra_cond_for_qry('q_rcd01', 'rcd_file')
    ...
    FOREACH ls_curs INTO l_plant
      ...
      LET ls_sql = "SELECT DISTINCT 'N',rcd01,rcd02,rcdplant ", 
                   "  FROM ",cl_get_target_table(l_plant,'rcd_file'),
                   " WHERE ",ms_cons_where
      IF NOT mi_multi_sel THEN
          LET ls_where = " AND rcd01='",g_key1,"'"
          IF NOT cl_null(g_key2) THEN
             LET ls_where = ls_where CLIPPED," AND rcd02='",g_key2,"'"
          END IF
      END IF
      LET ls_sql=ls_sql CLIPPED,ls_where CLIPPED," ORDER BY rcd01 "
...
# Modify        :
##################################################
FUNCTION rcd01_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION rcd01_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_rcd.* 
   ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED)
      ...
      BEFORE INPUT
         ...
         END IF
      ON ACTION prevpage
         CALL GET_FLDBUF(s_rcd.check) RETURNING ma_qry_tmp[ARR_CURR()].check
         CALL rcd01_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        
            CALL GET_FLDBUF(s_rcd.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL rcd01_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL rcd01_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        
            LET ms_ret1 = NULL       
         END IF                      
...
# Modify   	:
##################################################
FUNCTION rcd01_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10,
            pi_end_index     LIKE type_file.num10
   DEFINE   li_i             LIKE type_file.num10,
            li_j             LIKE type_file.num10 
...
# Modify   	:
##################################################
FUNCTION rcd01_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10,
            pi_end_index     LIKE type_file.num10
   DEFINE   li_continue      LIKE type_file.num5,
   ...
   DISPLAY ARRAY ma_qry_tmp TO s_rcd.*
      BEFORE DISPLAY
         CALL cl_set_act_visible("prevpage,nextpage,reconstruct",TRUE)
         IF (ps_hide_act IS NOT NULL) THEN   
            CALL cl_set_act_visible(ps_hide_act, FALSE)
...
# Modify   	:
##################################################
FUNCTION rcd01_qry_accept(pi_sel_index)
      ...
      FOR li_i = 1 TO ma_qry.getLength()
         IF (ma_qry[li_i].check = 'Y') THEN
            IF (lsb_multi_sel.getLength() = 0) THEN
               CALL lsb_multi_sel.append(ma_qry[li_i].rcd01 CLIPPED)
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].rcd01 CLIPPED)
            END IF
         END IF    
   ...
   ELSE
      LET ms_ret1 = ma_qry[pi_sel_index].rcd01 CLIPPED
      LET ms_ret2 = ma_qry[pi_sel_index].rcd02 CLIPPED
      LET ms_ret3 = ma_qry[pi_sel_index].rcdplant CLIPPED
   END IF
END FUNCTION
#FUN-B50156
