...
#{
# Program name  : q_m_aac1.4gl
# Program ver.  : 7.0
# Description   : 傳票單單據性質查詢(多工廠)
# Date & Author : 2008/04/20 by jamie   #MOD-840238
# Modify.........: No.FUN-840125 08/06/20 By sherry 系統自動拋轉傳票設定 
...
#FUNCTION q_m_aac1(pi_multi_sel,pi_need_cons,p_dbs,ps_default1,p_aac11,p_aac03,p_aac10,p_sys)  #No.FUN-980025 mark
FUNCTION q_m_aac1(pi_multi_sel,pi_need_cons,p_plant,ps_default1,p_aac11,p_aac03,p_aac10,p_sys) #No.FUN-980025   
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_m_aac1" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
 
   CALL cl_ui_locale("q_m_aac1")
...
FUNCTION m_aac1_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         CALL m_aac1_qry_prep_result_set()
      ...
      CALL m_aac1_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL m_aac1_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL m_aac1_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION m_aac1_qry_prep_result_set()
   ...
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_m_aac1', 'aac_file')
   ...
   LET ls_sql = "SELECT 'N',aac01,aac02,aac11,aacauno,aac03", 
                ...
                " WHERE ",ms_cons_where CLIPPED,    #MOD-840238 add 
              # "   AND aac11='1' " ,               #MOD-840238 add #FUN-970118 mark
                "   AND aac03 = '",mi_aac03,"'"     #No.FUN-840125 
   #FUN-970118---Begin
   #IF NOT mi_multi_sel THEN
...
# Modify        :
##################################################
FUNCTION m_aac1_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION m_aac1_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_aac.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL m_aac1_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_aac.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL m_aac1_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL m_aac1_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION m_aac1_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION m_aac1_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION m_aac1_qry_accept(pi_sel_index)
