...
FUNCTION q_oea4(pi_multi_sel,pi_need_cons,ps_default1,ps_cus_no)
...
FUNCTION oea4_qry_sel()
   ...
   WHILE TRUE
      ...
      CALL oea4_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL oea4_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL oea4_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION oea4_qry_prep_result_set()
  ...
  #IF (NOT mi_multi_sel) THEN            #MOD-A80023 mark
     ...
     #LET ls_where = " AND oea61>(oea62+oea63) AND oea00 = '1' AND oeaconf = 'Y'"
      LET ls_where = " AND oea00 = '1' AND oeaconf = 'Y'"  
     #-----------No:MOD-970045 end
...
#FUN-990069---end  
   ...
   FOREACH lcurs_qry INTO lr_qry.*
     ...
     #             "   AND oeb12 > (oeb24-oeb25) ",                         #MOD-A70171 mark
                   "   AND oeb12 > (oeb24-oeb25+oeb905) ",                  #MOD-A70171
...
# Modify        :
##################################################
FUNCTION oea4_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION oea4_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_oea.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL oea4_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_oea.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL oea4_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL oea4_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION oea4_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION oea4_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION oea4_qry_accept(pi_sel_index)
