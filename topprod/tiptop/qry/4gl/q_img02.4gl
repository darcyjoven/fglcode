...
#
# Program name  : q_img02.4gl copy from q_img42.4gl
# Program ver.  : 1.0
...
#                  p_type :經營方式
#                  p_plant:機構別
# Date & Author : NO.TQC-C20490  12/02/23 By lixiang 
# Memo          : 
DATABASE ds
...
FUNCTION q_img02(pi_multi_sel,pi_need_cons,p_img01,ps_default1,ps_default2,ps_default3,p_kind,p_type,p_plant)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_img02" ATTRIBUTE(STYLE="create_qry")  
   CALL cl_ui_locale("q_img02")  
...
FUNCTION img02_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         CALL img02_qry_prep_result_set() 
      ...
      CALL img02_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL img02_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL img02_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION img02_qry_prep_result_set()
   ...
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_img02', 'img_file')
   IF NOT cl_null(l_filter_cond) THEN
      LET ms_cons_where = ms_cons_where,l_filter_cond
   END IF
   #End:FUN-980030
   LET ls_sql = "SELECT DISTINCT 'N',img02,img03,img04,img09 ", 
                " FROM ", cl_get_target_table(ms_plant,'img_file') , 
                " ,",cl_get_target_table(ms_plant,'imx_file') ,
                " WHERE ",ms_cons_where 
   
   IF NOT mi_multi_sel THEN
      SELECT ima151 INTO l_ima151 FROM ima_file WHERE ima01 = ms_img01
      IF l_ima151 = 'Y' THEN
         LET ls_where = " AND img01 = imx000 AND imx00 ='", ms_img01,"' "
      ELSE
   ...
   END IF
      IF ms_type = '1' THEN
         LET ls_where = ls_where CLIPPED," AND img02 NOT IN (SELECT jce02 FROM ",
                                         cl_get_target_table(ms_plant,'jce_file'), ")"
      END IF
   ...
   LET ls_sql = ls_sql CLIPPED,ls_where CLIPPED, " ORDER BY img02,img03"
...
# Modify        :
##################################################
FUNCTION img02_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION img02_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_img.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE)
      ...
      ON ACTION prevpage
         ...
         CALL img02_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_img.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL img02_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL img02_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION img02_qry_reset_multi_sel(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION img02_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
...
# Modify   	:
##################################################
FUNCTION img02_qry_accept(pi_sel_index)
