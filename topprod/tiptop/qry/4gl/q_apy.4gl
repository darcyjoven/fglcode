...
# Modify.........: No.TQC-970258 09/07/24 By Carrier 顯示完資料后,把reconstruct的action顯示出來
# Modify.........: No.FUN-980030 09/08/14 By Hiko 加上資料權限控制
# Modify.........: No:CHI-9C0048 09/12/23 By tsai_yen 陣列索引ARR_CURR()為0的判斷
# Modify.........: No:FUN-A90007 10/09/02 By wujie    调帐应付单别控管
# Modify.........: No:TQC-BC0195 11/12/30 By wujie    调帐应付单别控管增加aapt335 
# Modify.........: No:MOD-C50168 12/05/25 By Polly 單別為12類，才做apz68條件判斷
# Modify.........: No:MOD-D40226 13/04/28 By wujie 增加mi_kind ='22'的判断
...
FUNCTION q_apy(pi_multi_sel,pi_need_cons,ps_default1,p_kind,p_sys)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_apy" ATTRIBUTE(STYLE="create_qry") #No.FUN-660161
 
   CALL cl_ui_locale("q_apy")
...
#              LET ls_hide_act = "prevpage,nextpage"              #No.TQC-970258
      ...
      CALL apy_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL apy_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL apy_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION apy_qry_prep_result_set()
   ...
   IF NOT mi_multi_sel THEN
      IF mi_kind != '*' THEN   #若指定單據性質
         LET ls_where = ls_where CLIPPED," AND apykind='",mi_kind,"'"
      END IF
   END IF
#No.FUN-A90007 --begin   
  #IF g_prog ='aaps100' OR g_prog ='aapt335' THEN   #No.TQC-BC0195        #MOD-C50168 mark
   IF (g_prog ='aaps100' OR g_prog ='aapt335') AND (mi_kind = '12' OR mi_kind = '22') THEN    #MOD-C50168 add  MOD-D40226 add '22'
...
# Modify        :
##################################################
FUNCTION apy_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION apy_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_apy.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL apy_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_apy.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL apy_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL apy_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION apy_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION apy_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION apy_qry_accept(pi_sel_index)
