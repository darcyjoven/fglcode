...
# Modify.........: No:TQC-AB0008 10/11/01 By yinhy 修改函数名稱
# Modify.........: No:TQC-AB0059 10/11/01 By yinhy 开窗报错
...
FUNCTION q_ime_2(pi_multi_sel,pi_need_cons,ps_default1,ps_default2,p_type,p_plant)
   ...
   IF (mi_multi_sel) THEN
      CALL cl_set_comp_font_color("ime01,ime02", "red")
   END IF
 
   CALL q_ime_2_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (li_continue) AND (li_reconstruct) AND (l_chr = 'Y') THEN 
           CALL q_ime_2_construct()   
         END IF
         
         CALL q_ime_2_qry_prep_result_set()   #TQC-AB0008 mod
      ...
      CALL q_ime_2_qry_set_display_data(li_start_index, li_end_index)
     
      LET li_curr_page = li_end_index / mi_page_count
      MESSAGE "Total count : " || ma_qry.getLength() || "  Page : " || li_curr_page
     
      IF (mi_multi_sel) THEN
         CALL q_ime_2_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL q_ime_2_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
FUNCTION q_ime_2_construct()
  CLEAR FORM
  CALL ma_qry_tmp.clear()
      		
  CONSTRUCT ms_cons_where ON ime01,imd02,ime02,ime03,ime04,ime05,ime06,ime07
       FROM s_ime_1[1].ime01,s_ime_1[1].imd02,s_ime_1[1].ime02,s_ime_1[1].ime03,  #TQC-AB0059 modify s_ime to s_ime_1
...
# Modify        :
##################################################
FUNCTION q_ime_2_qry_prep_result_set()
   ...
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_ime_2', 'ime_file')
...
# Modify        :
##################################################
FUNCTION q_ime_2_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION q_ime_2_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_ime_1.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE, APPEND ROW=FALSE, UNBUFFERED)
      BEFORE INPUT
      ...
      ON ACTION prevpage
         ...
         CALL q_ime_2_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_ime_1.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL q_ime_2_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL q_ime_2_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION q_ime_2_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION q_ime_2_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION q_ime_2_qry_accept(pi_sel_index)
   ...
   ELSE
      LET ms_ret1 = ma_qry[pi_sel_index].ime01 CLIPPED
      LET ms_ret2 = ma_qry[pi_sel_index].ime02 CLIPPED
