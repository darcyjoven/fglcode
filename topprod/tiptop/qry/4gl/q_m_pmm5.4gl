...
#
# Program name  : q_m_pmm5.4gl
# Program ver.  : 7.0
# Description   : 多工廠採購單資料查詢
...
DEFINE   ma_qry   DYNAMIC ARRAY OF RECORD
         ...
         pmn04    LIKE pmn_file.pmn04,          #No.FUN-680131 VARCHAR(60)
...
DEFINE   g_vendor         LIKE pmc_file.pmc04           #No.FUN-680131 VARCHAR(10)
 
#FUN-980094 傳入的db變數應該成傳入plant 變數---------------------(S)
#FUNCTION q_m_pmm5(pi_multi_sel,pi_need_cons,ps_default1,p_dbs,p_vendor)
 FUNCTION q_m_pmm5(pi_multi_sel,pi_need_cons,ps_default1,p_plant,p_vendor)
#FUN-980094 傳入的db變數應該成傳入plant 變數---------------------(E)
   DEFINE   pi_multi_sel   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
            ...
            p_vendor       LIKE pmc_file.pmc04          #No.FUN-680131 VARCHAR(10)
   ...
   LET g_dbs = p_dbs CLIPPED
   LET g_vendor = p_vendor
 
   WHENEVER ERROR CALL cl_err_msg_log
 
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_m_pmm5" ATTRIBUTE(STYLE="create_qry") #No.FUN-660161
 
   CALL cl_ui_locale("q_m_pmm5")
...
FUNCTION m_pmm5_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            ...
            IF g_sma.sma120 = 'Y' THEN
               CALL pmm5_multi_cons()   #調用自定義的CONSTRUCT過程
            ...
            ELSE
              ...
              #No.FUN-550131 Add End
               ...
               CONSTRUCT ms_cons_where ON pmm01,pmm02,pmm04,pmm07,pmm20,pmm12
                                     FROM s_pmm[1].pmm01,s_pmm[1].pmm02,
                                          s_pmm[1].pmm04,s_pmm[1].pmm07,
                                          s_pmm[1].pmm20,s_pmm[1].pmm12
               ...
               CONSTRUCT ms_cons_where2 ON pmn04 FROM s_pmm[1].pmn04
                  ON IDLE g_idle_seconds
                     CALL cl_on_idle()
                     CONTINUE CONSTRUCT
         ...
         CALL m_pmm5_qry_prep_result_set() 
      ...
      CALL m_pmm5_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL m_pmm5_qry_input_array(ls_hide_act,li_start_index,li_end_index) 
              RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL m_pmm5_qry_display_array(ls_hide_act,li_start_index,li_end_index)
              RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION m_pmm5_qry_prep_result_set()
   ...
   DEFINE   lr_qry   RECORD
         ...
         pmm20    LIKE pmm_file.pmm20, 
         pmm12    LIKE pmm_file.pmm12,
         pmn04    LIKE pmn_file.pmn04,          #No.FUN-680131 VARCHAR(40)
	 #FUN-550131 Add Start
	 attdesc  LIKE type_file.chr1000,       #No.FUN-680131 VARCHAR(500)
         att01    LIKE imx_file.imx01,
         att02    LIKE imx_file.imx01,
   ...
   IF ( g_sma.sma120 = 'N' )OR(( g_sma.sma120 = 'Y')AND( g_multi_where.getLength() = 0 ))THEN
      ...
      #Begin:FUN-980030
      LET l_filter_cond = cl_get_extra_cond_for_qry('q_m_pmm5', 'pmm_file')
      IF NOT cl_null(l_filter_cond) THEN
         LET ms_cons_where = ms_cons_where,l_filter_cond
      END IF
      #End:FUN-980030
      LET ls_sql =     "SELECT 'N',pmm01,pmm02,pmm04,pmm07,pmm20,",
                       "       pmm12, '',",
                       "'','','','','','','','','','','','','','','','',",  #FUN-550131
                       "'','','','',''",                                    #FUN-550131 Add
                     # " FROM ",g_dbs CLIPPED,"pmm_file,",
                     #          g_dbs CLIPPED,"pmn_file,",
                       #" FROM ",g_dbs_tra CLIPPED,"pmm_file,",    #FUN-980094 GP5.2 mod
                       #         g_dbs_tra CLIPPED,"pmn_file,",    #FUN-980094 GP5.2 mod
                       #         g_dbs CLIPPED,"pmc_file",
                       " FROM ",cl_get_target_table(g_plant_new,'pmm_file'),",", #FUN-A50102
                                cl_get_target_table(g_plant_new,'pmn_file'),",", #FUN-A50102
                                cl_get_target_table(g_plant_new,'pmc_file'),     #FUN-A50102
                       " WHERE pmmacti = 'Y' AND pmm18 != 'X' ",
                       "   AND pmm09   = pmc01 ",
                       "   AND pmm01   = pmn01 "
      IF NOT mi_multi_sel THEN
         LET ls_where = " AND pmc04 = '",g_vendor,"'"
      END IF
      IF NOT cl_null(ms_cons_where) THEN
         LET ls_sql = ls_sql CLIPPED,"   AND ",ms_cons_where
      END IF
      IF NOT cl_null(ms_cons_where2) THEN
         LET ls_sql = ls_sql CLIPPED,"   AND ",ms_cons_where2
      END IF
   ELSE 
      #否則按照料件多屬性機制的進行合成條件
      #FUN-550131 Add Start
      LET ls_sql =     "SELECT 'N',pmm01,pmm02,pmm04,pmm07,pmm20,",
                  ...
                  "'','','','','','','','','','','','','','','','',",  #FUN-550131
                  "'','','','',''",                                    #FUN-550131 Add
                 #" FROM ",g_dbs CLIPPED,"pmm_file,",g_dbs CLIPPED,"pmn_file,",g_dbs CLIPPED,"pmc_file",g_multi_join,
                  #" FROM ",g_dbs_tra CLIPPED,"pmm_file,",
                  #         g_dbs_tra CLIPPED,"pmn_file,",
                  #         g_dbs CLIPPED,"pmc_file",g_multi_join,
                  " FROM ",cl_get_target_table(g_plant_new,'pmm_file'),",", #FUN-A50102
                           cl_get_target_table(g_plant_new,'pmn_file'),",", #FUN-A50102
                           cl_get_target_table(g_plant_new,'pmc_file'),g_multi_join, #FUN-A50102                           
                       " WHERE pmmacti = 'Y' AND pmm18 != 'X' ",
                       "   AND pmm09   = pmc01 ",
                       "   AND pmm01   = pmn01 AND ",g_multi_where
   ...
   #FUN-550131 , 這裡如果用了ORDER則可能會報錯，不知道什，是不是排序緩衝區給小了 
   #LET ls_sql = ls_sql CLIPPED,ls_where CLIPPED," ORDER BY pmm01"
   LET ls_sql = ls_sql CLIPPED,ls_where
   
   DISPLAY "ls_sql=",ls_sql
 	 CALL cl_replace_sqldb(ls_sql) RETURNING ls_sql        #FUN-920032
   CALL cl_parse_qry_sql(ls_sql,g_plant_new) RETURNING ls_sql #FUN-980094
...
# Modify        :
##################################################
FUNCTION m_pmm5_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION m_pmm5_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   #ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE)            #FUN-880082 mark
      ...
      ON ACTION prevpage
         ...
         CALL m_pmm5_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_pmm.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL m_pmm5_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL m_pmm5_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION m_pmm5_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION m_pmm5_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION m_pmm5_qry_accept(pi_sel_index)
...
FUNCTION pmm5_multi_cons()
  ...
  CALL cl_set_comp_entry('pmm01,pmm02,pmm04,pmm07,pmm20,pmm12,pmn04',TRUE)
  ...
  INPUT ARRAY ma_cons_lif WITHOUT DEFAULTS FROM s_pmm.* 
    ...
    #信息以及顯示可能會有的明細屬性列  
    AFTER FIELD pmm01,pmm02,pmm04,pmm07,pmm20,pmm12,pmn04
      CALL pmm5_after_normal_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
    ...
    AFTER FIELD att01,att02,att03,att04,att05,att06,att07,att08,att09,att10,
      ...
      CALL pmm5_after_detail_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
  ...
  ELSE 
     ...
     CALL pmm5_make_sql_condition()   #這個市cl_create_qry中定義的標準函數
  END IF  
  ...
  CALL cl_set_comp_entry('pmm01,pmm02,pmm04,pmm07,pmm20,pmm12,pmn04',FALSE)
...
#明細屬性欄位以及相應的處理過程
#傳入pname為欄位的colname
FUNCTION pmm5_after_normal_field(pname,pvalue,prow)
  DEFINE pname        STRING,
         pvalue       STRING,
...
#當前內容到對應的后台數組中
FUNCTION pmm5_after_detail_field(pname,pvalue,pi)
  DEFINE pname         STRING
...
FUNCTION pmm5_make_sql_condition()  
