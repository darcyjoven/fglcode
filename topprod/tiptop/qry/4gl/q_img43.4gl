...
# Date & Author : #WEB-C30007 2012/01/05 by Mandy Copy from q_img4 差異是查出數量img10>0的資料
...
FUNCTION q_img43(pi_multi_sel,pi_need_cons,p_img01,ps_default1,ps_default2,ps_default3,p_kind)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_img43" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
 
   CALL cl_ui_locale("q_img43")
...
FUNCTION img43_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         CALL img43_qry_prep_result_set() 
      ...
      CALL img43_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL img43_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL img43_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION img43_qry_prep_result_set()
...
#No.CHI-960097 --End
   END RECORD
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_img43', 'img_file')
...
#No.CHI-960097 --Begin                                                                                                              
   #IF g_sma.sma115 = 'Y' THEN   #CHI-9A0007
   IF g_sma.sma115 = 'Y' AND g_ima906 <> '1' THEN   #CHI-9A0007
      DECLARE q_img43_imgg09 CURSOR FOR                                                                                              
       SELECT UNIQUE imgg09 FROM imgg_file                                                                                          
        WHERE imgg01 = ms_img01                                                                                                     
       LET li_i = 1                                                                                                                 
       FOREACH q_img43_imgg09 INTO l_imgg10[li_i]                                                                                    
...
#No.CHI-960097 --End
   ...
   #  LET ls_sql = "SELECT 'N',img02,img03,img04,img09,img10,img27",         #FUN-B50087
      LET ls_sql = "SELECT 'N',img02,img03,img04,img09,img10,img27,img18",   #FUN-B50087
                   " FROM img_file",
                   " WHERE ",ms_cons_where,
                   "   AND img10 > 0 "            
...
# Modify        :
##################################################
FUNCTION img43_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION img43_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_img.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL img43_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_img.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL img43_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL img43_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION img43_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION img43_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION img43_qry_accept(pi_sel_index)
