...
#{
# Program name  : q_omb2.4gl
# Program ver.  : 7.0
# Description   : A/R 細項資料查詢
# Date & Author : 2013/02/21 FUN-D20041 by lujh
...
DEFINE   ma_qry   DYNAMIC ARRAY OF RECORD
         check    LIKE type_file.chr1,  	#No.FUN-D20041  Add
         ...
         oox10  LIKE oox_file.oox10,    #bug no:A064
...
DEFINE   ma_cons_lif   DYNAMIC ARRAY OF RECORD
         ...
         omb34  LIKE omb_file.omb34	   
END RECORD
DEFINE   g_amt            LIKE omb_file.omb35   #FUN-D80086 add
DEFINE   g_amt1           LIKE omb_file.omb34   #FUN-D80086 add
...
DEFINE   ms_ret1          STRING, 
         ...
         ms_ret3          STRING,
         ms_ret4          STRING,
         g_cus            LIKE oma_file.oma03,             #帳款客戶	#No.FUN-680131 VARCHAR(20)
         g_key1           LIKE oob_file.oob02,             #沖帳單項次  #No.FUN-680131 SMALLINT
         g_key2           LIKE oob_file.oob01,  #沖帳單號
         g_type           LIKE oma_file.oma00   #帳款類別: '1*'  #No.FUN-680131 VARCHAR(2)
 
FUNCTION q_omb2(pi_multi_sel,pi_need_cons,ps_default1,p_key1,p_key2,p_cus,p_type)
   DEFINE   pi_multi_sel   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
            pi_need_cons   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
            ps_default1    STRING  , 
            p_key2         LIKE oob_file.oob01,  #沖帳單號
            p_key1         LIKE oob_file.oob02,  #沖帳單項次  #No.FUN-680131 SMALLINT
            p_cus          LIKE oma_file.oma03,             #帳款客戶	#No.FUN-680131 VARCHAR(20)
            p_type         LIKE oma_file.oma00   #帳款類別: '1*'   #No.FUN-680131 VARCHAR(2)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_omb" ATTRIBUTE(STYLE="create_qry") #No.FUN-660161
 
   CALL cl_ui_locale("q_omb")
   ...
   IF (mi_multi_sel) THEN
      CALL cl_set_comp_font_color("omb01", "red")
   END IF
...
FUNCTION omb2_qry_sel()
   ...
   WHILE TRUE
      ...
      #IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            ...
            IF g_sma.sma120 = 'Y' THEN
               call omb2_multi_cons()   #調用自定義的CONSTRUCT過程
            ...
            ELSE
              ...
              #No.FUN-550131 Add End
               ...
               CONSTRUCT ms_cons_where ON omb01 ,omb03,omb04,oma23,omb16t,omb14t,oox10,omb35,omb34
                                     FROM s_omb[1].omb01 ,s_omb[1].omb03 ,
                          s_omb[1].omb04 ,s_omb[1].oma23 ,
                          s_omb[1].omb16t,s_omb[1].omb14t,s_omb[1].oox10,  
                          s_omb[1].omb35 ,s_omb[1].omb34
      ...
      call omb2_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         call omb2_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         call omb2_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION omb2_qry_prep_result_set()
   ...
   IF ( g_sma.sma120 = 'N' )OR(( g_sma.sma120 = 'Y')AND( g_multi_where.getLength() = 0 ))THEN
      ...
      #Begin:FUN-980030
      LET l_filter_cond = cl_get_extra_cond_for_qry('q_omb', 'omb_file')
      IF NOT cl_null(l_filter_cond) THEN
         LET ms_cons_where = ms_cons_where,l_filter_cond
      END IF
      #End:FUN-980030
      LET ls_sql = "SELECT 'N',omb01,omb03,omb16t,omb14t,omb04,",
                   "'','','','','','','','','','','','','','','','',",  #FUN-550131
                   "'','','','','',",                                   #FUN-550131 Add      
                   "           oma23,oox10,omb35,omb34 ",
                   " FROM oma_file,omb_file,OUTER oox_file ",
                   " WHERE ",ms_cons_where
   ELSE 
      LET ls_sql = "SELECT 'N',omb01,omb03,omb16t,omb14t,omb04,",
                   "'','','','','','','','','','','','','','','','',",  #FUN-550131
                   "'','','','','',",                                   #FUN-550131 Add      
                   "           oma23,oox10,omb35,omb34 ",
                   " FROM oma_file,omb_file,OUTER oox_file ",g_multi_join,
                   " WHERE ",g_multi_where   	
   END IF 
   #FUN-550131 Modified End             
                
      LET ls_where = "   AND oma01=omb01 ",
                     "   AND omaconf='Y' AND omavoid='N' ",
                     "   AND oma03 ='",g_cus,"'",
                     "   AND omb14t > omb34 ",
                     "   AND oox_file.oox00='AR' AND omb01= oox_file.oox03 ",
                     "   AND omb03 = oox_file.oox04 "
      IF NOT cl_null(g_type) THEN
         LET ls_where = ls_where CLIPPED," AND oma00 LIKE '",g_type,"'"
     END IF
  #No.TQC-810034--begin--
  #LET ls_sql = ls_sql CLIPPED,ls_where CLIPPED," ORDER BY omb01,omb03 "  #No.TQC-810034 mark
   LET ls_sql = ls_sql CLIPPED,ls_where CLIPPED
  #LET ls_sql = ls_sql CLIPPED," AND (omb01,omb03) NOT IN (SELECT oob06,oob15 FROM oob_file WHERE oob01='",g_key2 CLIPPED,"') "                         #MOD-A80249 mark
   LET ls_sql = ls_sql CLIPPED," AND (omb01,omb03) NOT IN (SELECT oob06,oob15 FROM oob_file WHERE oob01='",g_key2 CLIPPED,"' AND oob06 IS NOT NULL) "   #MOD-A80249
   LET ls_sql = ls_sql CLIPPED," ORDER BY omb01,omb03 "
  #No.TQC-810034---end---
...
# Modify        :
##################################################
FUNCTION omb2_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION omb2_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   #INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_omb.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE)            #FUN-880082 mark
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_omb.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      BEFORE INPUT
      ...
      ON CHANGE CHECK
        IF ma_qry_tmp[ARR_CURR()].CHECK = 'Y' THEN
           LET g_amt  = g_amt  + ma_qry_tmp[ARR_CURR()].omb35
           LET g_amt1 = g_amt1 + ma_qry_tmp[ARR_CURR()].omb34   
        ELSE
           LET g_amt  = g_amt  - ma_qry_tmp[ARR_CURR()].omb35
           LET g_amt1 = g_amt1 - ma_qry_tmp[ARR_CURR()].omb34       
        END IF 
      ...
      ON ACTION prevpage
         ...
         call omb2_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_omb.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            call omb2_qry_reset_multi_sel(pi_start_index, pi_end_index)
            call omb2_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION omb2_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION omb2_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION omb2_qry_accept(pi_sel_index)
   ...
   IF cl_null(ma_qry[pi_sel_index].oox10) THEN 
      LET ma_qry[pi_sel_index].oox10 = 0
   END IF 
      ...
      FOR li_i = 1 TO ma_qry.getLength()
         IF (ma_qry[li_i].check = 'Y') THEN
            IF (lsb_multi_sel.getLength() = 0) THEN
               CALL lsb_multi_sel.append(ma_qry[li_i].omb01 CLIPPED || "," 
                                         || ma_qry[li_i].omb03 CLIPPED || "," 
                                         || ma_qry[pi_sel_index].omb14t - ma_qry[pi_sel_index].omb34 CLIPPED || "," 
                                         || ma_qry[pi_sel_index].omb16t - ma_qry[pi_sel_index].omb35 + ma_qry[pi_sel_index].oox10)
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].omb01 CLIPPED
                                             || "," || ma_qry[li_i].omb03 CLIPPED 
                                             || "," || ma_qry[pi_sel_index].omb14t - ma_qry[pi_sel_index].omb34 CLIPPED 
   ...
   ELSE
       LET ms_ret1 = ma_qry[pi_sel_index].omb01 CLIPPED
       LET ms_ret2 = ma_qry[pi_sel_index].omb03 CLIPPED
       LET ms_ret3 = ma_qry[pi_sel_index].omb14t - ma_qry[pi_sel_index].omb34
       LET ms_ret4 = ma_qry[pi_sel_index].omb16t - ma_qry[pi_sel_index].omb35 + ma_qry[pi_sel_index].oox10
   END IF
END FUNCTION
...
DEFINE
  ...
  #填充字段名稱數組
  #omb01,omb03,omb16t,omb14t,omb04,... oma23,oox10,omb35,omb34
  CALL la_field.clear()
  LET la_field[1] = 'omb01'
  LET la_field[2] = 'omb03'
  LET la_field[3] = 'omb16t'
  LET la_field[4] = 'omb14t'
  LET la_field[5] = 'omb04'
  LET la_field[27] = 'oma23'
  LET la_field[28] = 'oox10'
  LET la_field[29] = 'omb35'
  LET la_field[30] = 'omb34'
  ...
  #omb03,omb14t,omb16t,omb34,omb35,oox10是NUMBER型
  LET ma_multi_rec[2].dbtype  = 'NUMBER'
  LET ma_multi_rec[3].dbtype  = 'NUMBER'
  LET ma_multi_rec[4].dbtype  = 'NUMBER'
  LET ma_multi_rec[28].dbtype = 'NUMBER'
  LET ma_multi_rec[29].dbtype = 'NUMBER'
  LET ma_multi_rec[30].dbtype = 'NUMBER'  
...
FUNCTION omb2_multi_cons()
  ...
  #CALL cl_set_comp_entry('omb01,omb03,omb16t,omb14t,omb04,oma23,oox10,omb35,omb34',TRUE)
  CALL cl_set_comp_entry('omb01,omb03,omb04,oma23,oox10,omb35,omb34',TRUE)
  CALL cl_set_comp_entry('omb16t,omb14t',FALSE)
  ...
  INPUT ARRAY ma_cons_lif WITHOUT DEFAULTS FROM s_omb.* 
    ...
    #信息以及顯示可能會有的明細屬性列  
    AFTER FIELD omb01,omb03,omb16t,omb14t,omb04,oma23,oox10,omb35,omb34
      call omb2_after_normal_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
    ...
    AFTER FIELD att01,att02,att03,att04,att05,att06,att07,att08,att09,att10,
      ...
      call omb2_after_detail_field(FGL_DIALOG_GETFIELDNAME(),FGL_DIALOG_GETBUFFER(),l_ac)
  ...
  ELSE 
     ...
     call omb2_make_sql_condition()   #這個市cl_create_qry中定義的標準函數
  END IF  
  ...
  CALL cl_set_comp_entry('omb01,omb03,omb16t,omb14t,omb04,oma23,oox10,omb35,omb34',FALSE)
...
#明細屬性欄位以及相應的處理過程
#傳入pname為欄位的colname
FUNCTION omb2_after_normal_field(pname,pvalue,prow)
  DEFINE pname        STRING,
         pvalue       STRING,
         ...
         lc_i         LIKE type_file.chr2,      #No.FUN-680131 VARCHAR(2)
  #這個數組用于接收各個屬性列的詳細信息
...
#當前內容到對應的后台數組中
FUNCTION omb2_after_detail_field(pname,pvalue,pi)
  DEFINE pname         STRING
...
FUNCTION omb2_make_sql_condition()  
  ...
  FOR li_n = 1 to li_rowcount
      ...
      FOR li_i = 1 TO ma_multi_rec.getLength() 
          ...
          #如果當前欄位已經是'omb34'(即最後一個標準欄位)則退出當前層次的循環
          IF ma_multi_rec[li_i].colname = 'omb34' THEN 
             EXIT FOR
          END IF
  ...
  IF g_multi_where = '(1 = 0)' THEN
     LET g_multi_where = ''
  ELSE
     LET g_multi_where = '(',g_multi_where,')'  #如果不這樣做，凡是有OUTER的地方都會出錯
  END IF
