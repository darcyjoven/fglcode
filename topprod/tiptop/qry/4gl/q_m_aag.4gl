...
#
# Program name  : q_m_aag.4gl
# Program ver.  : 7.0
# Description   : 多工廠會計科目編號查詢
...
# Modify........: No.FUN-730070 07/03/15 By Carrier 會計科目加帳套 - CONSTRUCT時不納aag00的條件
...
# Modify.........: No.TQC-B90213 11/10/10 By Carrier 增加科目有效否选项
...
#
#FUNCTION q_m_aag(pi_multi_sel,pi_need_cons,p_dbs,ps_default1,p_aag07,ps_bookno)  #No.FUN-730020   #No.FUN-980025 mark 
FUNCTION q_m_aag(pi_multi_sel,pi_need_cons,p_plant,ps_default1,p_aag07,ps_bookno)  #No.FUN-730020  #No.FUN-980025      
   DEFINE   pi_multi_sel   LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
            ...
            p_plant        LIKE type_file.chr10, 	#No.FUN-980025 VARCHAR(10)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_m_aag" ATTRIBUTE(STYLE="create_qry") #No.FUN-660161
 
   CALL cl_ui_locale("q_m_aag")
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            CONSTRUCT ms_cons_where ON aag01,aag03,aag02,aag13,aag04,aag07 #FUN-6C0012
                                  FROM s_aag[1].aag01,s_aag[1].aag03,
         ...
         CALL m_aag_qry_prep_result_set() 
      ...
      CALL m_aag_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL m_aag_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL m_aag_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION m_aag_qry_prep_result_set()
   ...
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_m_aag', 'aag_file')
   IF NOT cl_null(l_filter_cond) THEN
      LET ms_cons_where = ms_cons_where,l_filter_cond
   END IF
   #End:FUN-980030
   #FUN-B20004--begin
   IF cl_null(ms_cons_where) AND mi_need_cons=0 AND NOT cl_null(ms_default1) THEN 
      LET ms_cons_where=" aag01 LIKE '",ms_default1,"%' "
   END IF  
   #FUN-B20004--end 
   LET ls_sql = "SELECT 'N',aag01,aag03,aag02,aag13,aag04,aag07",  #FUN-6C0012
   ...
   IF NOT mi_multi_sel THEN
      ...
      IF NOT cl_null(g_aag07) THEN
         LET ls_where = ls_where CLIPPED,
             " AND aag07 IN ",cl_parse(g_aag07 CLIPPED)
      END IF
   END IF
   #No.TQC-B90213  --Begin
   LET ls_sql = ls_sql CLIPPED," AND aagacti = 'Y'"
   #No.TQC-B90213  --End
...
# Modify        :
##################################################
FUNCTION m_aag_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION m_aag_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   #ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE)            #FUN-880082 mark
      ...
      ON ACTION prevpage
         ...
         CALL m_aag_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_aag.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL m_aag_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL m_aag_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION m_aag_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION m_aag_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION m_aag_qry_accept(pi_sel_index)
