...
# Modify.........: No.MOD-930052 09/03/05 By lilingyu 單身入庫單號查詢時,查不到任何資料,但如果手工KEY值后存檔,再做開窗就有資料
# Modify.........: No.FUN-980030 09/08/14 By Hiko 加上資料權限控制
# Modify.........: No.FUN-980025 09/10/09 By dxfwo hardcode的部分修改
# Modify.........: No.FUN-9A0093 09/11/02 By lutingting加傳參數營運中心 
...
#FUNCTION q_rvv(pi_multi_sel,pi_need_cons,p_vender,p_no1,p_no2)    #FUN-9A0093
FUNCTION q_rvv(pi_multi_sel,pi_need_cons,p_vender,p_no1,p_no2,p_plant)    #FUN-9A0093
...
FUNCTION rvv_qry_sel()
   ...
   WHILE TRUE
      ...
      CALL rvv_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL rvv_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL rvv_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION rvv_qry_prep_result_set()
...
#  LET ls_sql=" SELECT 'N',rvv01,rvv02,rvv03,'',rvv17-rvv23",  #TQC-830018
  #LET ls_sql=" SELECT 'N',rvv01,rvv02,rvv03,'',rvv87-rvv23",  #TQC-830018
...
#END TQC-5B0017
   END IF
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_rvv', 'rvv_file')
  ...
  ##NO.FUN-980025 GP5.2 add--end
   ...
   FOREACH lcurs_qry INTO lr_qry.*,l_rvv17,l_rvv23   #No.MOD-910062
      ...
      ELSE
       #  IF l_rvv17 > l_rvv23 THEN    #MOD-930052
          IF lr_qry.rvv17 <= 0 THEN    #MOD-930052
            CONTINUE FOREACH
         END IF
      END IF
      #-----No.MOD-910062 END-----
...
# Modify        :
##################################################
FUNCTION rvv_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION rvv_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_rvv.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL rvv_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_rvv.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL rvv_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL rvv_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION rvv_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION rvv_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION rvv_qry_accept(pi_sel_index)
 ...
 #              CALL lsb_multi_sel.append("|" || ma_qry[li_i].rvv02 CLIPPED)  #No.MOD-520035
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].rvv01 CLIPPED)
 #              CALL lsb_multi_sel.append("|" || ma_qry[li_i].rvv02 CLIPPED)  #No.MOD-520035
            END IF
