...
FUNCTION q_ima(pi_multi_sel,pi_need_cons,p_plant)
...
FUNCTION m_ima_qry_sel()
   ...
   WHILE TRUE
      ...
      ON ACTION qbe_save
         ...
         CALL m_ima_qry_prep_result_set() 
      ...
      CALL m_ima_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL m_ima_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL m_ima_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION m_ima_qry_prep_result_set()
...
#    IF g_azw.azw04 = '2' AND NOT cl_null(l_rtz04) THEN                 #FUN-A90048
 ...
 # No.FUN-A80121 .. end     
    ...
    IF g_prog = "axmt410" OR g_prog= "axmt400" THEN 
       LET ls_sql = ls_sql CLIPPED," AND ima06 IN ('G01','BCP') AND instr(ima01,'-') =0 "
    END IF 
...
#FUN-A90048 -------end
   ...
   FOREACH lcurs_qry INTO lr_qry.*
      ...
      IF g_prog = "axmt410" OR g_prog= "axmt400" THEN 
         ...
         END IF
         LET g_oeb04_1 = 'Y'
         CALL m_ima_qry_prep_result_check(lr_qry.ima01)
         IF g_oeb04_1 = 'N' THEN 
            CONTINUE FOREACH
         END IF 
      END IF 
...
# Modify        :
##################################################
FUNCTION m_ima_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION m_ima_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED)
      ...
      ON ACTION prevpage
         ...
         CALL m_ima_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_ima.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL m_ima_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL m_ima_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION m_ima_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 
            pi_end_index     LIKE type_file.num10 
   DEFINE   li_i             LIKE type_file.num10,
            li_j             LIKE type_file.num10 
...
# Modify   	:
##################################################
FUNCTION m_ima_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 
            pi_end_index     LIKE type_file.num10 
   DEFINE   li_continue      LIKE type_file.num5,
...
# Modify   	:
##################################################
FUNCTION m_ima_qry_accept(pi_sel_index)
...
#str----add by guanyao160729
FUNCTION m_ima_qry_prep_result_check(l_oeb04)
      ...
      FOR i=1  TO  l_ac
         ...
         IF l_num2 >0 THEN 
            ...
            IF l_bma10!=2  THEN 
               ...
               EXIT FOR 
            ELSE 
               CALL m_ima_qry_prep_result_check(l_oeb04)
            END IF   
         END IF 
      END FOR 
   END IF 
