...
# Modify.........: No:CHI-C10017 12/01/12 By bart 加入oay_file
...
FUNCTION q_smy3(pi_multi_sel,pi_need_cons,ps_default1,p_sys,p_kind)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_smy3" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
 
   CALL cl_ui_locale("q_smy3")
...
#TQC-970230 --end-- 
      ...
      CALL smy_qry_set_display_data3(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL smy_qry_input_array3(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL smy_qry_display_array3(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION smy_qry_prep_result_set3()
...
#CHI-C10017-----begin   
   DEFINE   l_ms_cons_where    STRING
   DEFINE   ls_where1          STRING
   LET l_ms_cons_where = ms_cons_where 

   LET l_ms_cons_where = cl_replace_str(l_ms_cons_where,'smyslip',' oayslip')
   LET l_ms_cons_where = cl_replace_str(l_ms_cons_where,'smydesc',' oaydesc')
   LET l_ms_cons_where = cl_replace_str(l_ms_cons_where,'smyauno',' oayauno')
   LET l_ms_cons_where = cl_replace_str(l_ms_cons_where,'smydmy5',' oaydmy6')
#CHI-C10017-----end
   ...
   IF NOT mi_multi_sel THEN
      ...
      LET ls_where1 = " AND oayacti='Y'"  #CHI-C10017
   ...
   #No.FUN-950021  --End  
   #Begin:FUN-980030
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_smy3', 'smy_file')
   IF NOT cl_null(l_filter_cond) THEN
      LET ms_cons_where = ms_cons_where,l_filter_cond
   END IF
   #End:FUN-980030
   LET ls_sql = ls_sql CLIPPED,ls_where CLIPPED,#" ORDER BY smyslip"  #CHI-C10017

#CHI-C10017-----begin
               " UNION ",
               " SELECT 'N',oayslip,oaydesc,oayauno,oaydmy6",
               " FROM oay_file ",
               " WHERE ",l_ms_cons_where CLIPPED

   LET ls_sql = ls_sql CLIPPED,ls_where1 CLIPPED," ORDER BY smyslip"
#CHI-C10017-----end
...
# Modify        :
##################################################
FUNCTION smy_qry_set_display_data3(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION smy_qry_input_array3(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_smy.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED)   #No.FUN-870043
      ...
      ON ACTION prevpage
         ...
         CALL smy_qry_reset_multi_sel3(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_smy.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL smy_qry_reset_multi_sel3(pi_start_index, pi_end_index)
            CALL smy_qry_accept3(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION smy_qry_reset_multi_sel3(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION smy_qry_display_array3(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify   	:
##################################################
FUNCTION smy_qry_accept3(pi_sel_index)
...
#No.FUN-950021  --Begin
FUNCTION smy_qry_set_par_where3(p_where)
