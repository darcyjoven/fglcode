...
#C2017030201chenyang    增加排除仓库ZTC的逻辑
...
FUNCTION q_imd_1(pi_multi_sel,pi_need_cons,ps_default1,p_type,p_plant,p_imd01_cons,p_where_sql)
   DEFINE   pi_multi_sel   LIKE type_file.num5,
            ...
            p_imd01_cons   LIKE type_file.chr1,
            p_where_sql    STRING
   ...
   END IF   
   LET ms_imd01_cons = p_imd01_cons
   LET ms_where_sql = p_where_sql
 
   WHENEVER ERROR CALL cl_err_msg_log
 
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_imd_1" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
 
   CALL cl_ui_locale("q_imd_1")  #No.FUN-AB0029
   ...
   IF (mi_multi_sel) THEN
      CALL cl_set_comp_font_color("imd01", "red")
   END IF
 
   CALL q_imd_1_qry_sel()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (li_continue) AND (li_reconstruct) AND (l_chr = 'Y') THEN 
           CALL q_imd_1_construct()   
         END IF
         
         CALL q_imd_1_qry_prep_result_set() 
      ...
      CALL q_imd_1_qry_set_display_data(li_start_index, li_end_index)
     
      LET li_curr_page = li_end_index / mi_page_count
      MESSAGE "Total count : " || ma_qry.getLength() || "  Page : " || li_curr_page
     
      IF (mi_multi_sel) THEN
         CALL q_imd_1_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL q_imd_1_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
FUNCTION q_imd_1_construct()
  CLEAR FORM
  CALL ma_qry_tmp.clear()

  CONSTRUCT ms_cons_where ON imd01,imd02,imd10,imd11,imd12,imd13,imd14,imd20
       FROM s_imd[1].imd01,s_imd[1].imd02,s_imd[1].imd10,s_imd[1].imd11,
            s_imd[1].imd12,s_imd[1].imd13,s_imd[1].imd14,s_imd[1].imd20
...
# Modify        :
##################################################
FUNCTION q_imd_1_qry_prep_result_set()
   ...
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_imd_1','imd_file')
   IF NOT cl_null(l_filter_cond) THEN
      LET ms_cons_where = ms_cons_where,l_filter_cond
   END IF
   
   LET ls_sql = "SELECT 'N',imd01,imd02,imd10,imd11,imd12,imd13,imd14,imd20",                         
                 " FROM ",cl_get_target_table(ms_plant,'imd_file'), 
                 " WHERE ",ms_cons_where,
                 " AND imdacti='Y'",  
				 " AND imd01<> 'ZTC' ",     #C2017030201chenyang  add
...
#   END IF   
   ...
   END IF   
#No.FUN-AA0084  --End    
   IF ms_imd01_cons ='N' THEN  #若指定是否納入計算成本庫 N不計算成本庫
      LET ls_where = ls_where CLIPPED," AND imd01 IN (SELECT jce02 FROM jce_file)"
   ELSE          
     IF ms_imd01_cons ='Y' THEN
            LET ls_where = ls_where CLIPPED," AND imd01 NOT IN (SELECT jce02 FROM jce_file)"
     END IF
   END IF
...
# Modify        :
##################################################
FUNCTION q_imd_1_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify         :
##################################################
FUNCTION q_imd_1_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_imd.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE, APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL q_imd_1_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_imd.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL q_imd_1_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL q_imd_1_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify         :
##################################################
FUNCTION q_imd_1_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10,       #No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10       #No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10,       #No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10       #No.FUN-680131 INTEGER
...
# Modify         :
##################################################
FUNCTION q_imd_1_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10,       #No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10       #No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,        #No.FUN-680131 SMALLINT
...
# Modify         :
##################################################
FUNCTION q_imd_1_qry_accept(pi_sel_index)
      ...
      FOR li_i = 1 TO ma_qry.getLength()
         IF (ma_qry[li_i].check = 'Y') THEN
            IF (lsb_multi_sel.getLength() = 0) THEN
               CALL lsb_multi_sel.append(ma_qry[li_i].imd01 CLIPPED)
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].imd01 CLIPPED)
            END IF
         END IF    
   ...
   ELSE
      LET ms_ret1 = ma_qry[pi_sel_index].imd01 CLIPPED
