...
#
# Program name  : q_smy5.4gl
# Program ver.  : 7.0
# Description   : 單據性質查詢
# Date & Author : 2013/01/10 by qiull  #FUN-CB0087
...
FUNCTION q_smy5(pi_multi_sel,pi_need_cons,ps_default1,ps_default2)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_smy5" ATTRIBUTE(STYLE="create_qry") 
 
   CALL cl_ui_locale("q_smy5")
...
# Date & Author : 2013/01/10 by qiull
# Parameter   	: none
# Return   	: void
# Memo        	:
# Modify   	:
...
FUNCTION smy5_qry_sel()
   ...
   WHILE TRUE
      ...
      CALL smy5_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL smy5_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL smy5_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION smy5_qry_prep_result_set()
   ...
   LET l_filter_cond1 = cl_get_extra_cond_for_qry('q_smy5', 'smy_file')
   LET l_filter_cond2 = cl_get_extra_cond_for_qry('q_smy5', 'oay_file')
   LET ms_cons_where1 = ms_cons_where
   LET l_where = cl_replace_str(ms_cons_where,'smy','oay')
   ...
   END IF
   LET ls_sql = "SELECT DISTINCT 'N',smyslip,smydesc,smyauno,smydmy5",
                "  FROM smy_file ",
                " WHERE ",ms_cons_where1 CLIPPED,
                " UNION ",
                "SELECT 'N',oayslip,oaydesc,oayauno,oaydmy6",
                "  FROM oay_file ",
                " WHERE ",ms_cons_where2 CLIPPED
                 
   LET ls_sql=ls_sql CLIPPED," ORDER BY smyslip"
...
# Modify        :
##################################################
FUNCTION smy5_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	    :
##################################################
FUNCTION smy5_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_smy.* 
   ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED)
      ...
      ON ACTION prevpage
         ...
         CALL smy5_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        
            CALL GET_FLDBUF(s_smy.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL smy5_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL smy5_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        
            LET ms_ret1 = NULL       
         END IF                      
...
# Modify   	    :
##################################################
FUNCTION smy5_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10,
            pi_end_index     LIKE type_file.num10
   DEFINE   li_i             LIKE type_file.num10,
            li_j             LIKE type_file.num10 
...
# Modify     	:
##################################################
FUNCTION smy5_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10,
            pi_end_index     LIKE type_file.num10
   DEFINE   li_continue      LIKE type_file.num5,
...
# Modify   	:
##################################################
FUNCTION smy5_qry_accept(pi_sel_index)
