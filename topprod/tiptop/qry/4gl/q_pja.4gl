...
#
# Program name   : q_pja.4gl
# Program ver.   : 4.0
# Description    : 項目基本資料查詢(多工廠)
# Date & Author  : 2009/10/30 By lutingting(FUN-9A0093)
# Memo           : 
...
DEFINE   ms_dbs           LIKE type_file.chr30 

FUNCTION q_pja(pi_multi_sel,pi_need_cons,ps_default1,p_plant)
   ...
   OPEN WINDOW w_qry WITH FORM "qry/42f/q_pja1" ATTRIBUTE(STYLE="create_qry") 
 
   CALL cl_ui_locale("q_pja1")
   ...
   IF (mi_multi_sel) THEN #在複選狀態時,要將回傳欄位的字型顏色設為紅色,以作為標示.
      CALL cl_set_comp_font_color("pja01", "red")
   END IF
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            ...
            CONSTRUCT ms_cons_where ON pja01,pja02,pja08,pja09,pja13
                                   FROM s_pja[1].pja01,s_pja[1].pja02,s_pja[1].pja08,
                                        s_pja[1].pja09,s_pja[1].pja13
         ...
         CALL m_pja_qry_prep_result_set() 
      ...
      CALL m_pja_qry_set_display_data(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL m_pja_qry_input_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL m_pja_qry_display_array(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION m_pja_qry_prep_result_set()
   ...
   LET l_filter_cond = cl_get_extra_cond_for_qry('q_pja', 'pja_file')
   IF NOT cl_null(l_filter_cond) THEN
      LET ms_cons_where = ms_cons_where,l_filter_cond
   END IF  
 
   LET ls_sql = "SELECT UNIQUE 'N',pja01,pja02,pja08,pja09,pja13 ",
                #"  FROM ",ms_dbs CLIPPED,"pja_file,",ms_dbs CLIPPED,"pjb_file ",
                "  FROM ",cl_get_target_table(g_plant_new,'pja_file'),",", #FUN-A50102
                          cl_get_target_table(g_plant_new,'pjb_file'),      #FUN-A50102
                " WHERE  pja01 = pjb01 AND pjaacti = 'Y'  ",
                "   AND pjaclose = 'N' AND ",ms_cons_where,
                " ORDER BY pja01 "   
...
# Modify        :
##################################################
FUNCTION m_pja_qry_set_display_data(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION m_pja_qry_input_array(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_pja.* 
   ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED)
      BEFORE INPUT
         ...
         END IF
      ON ACTION prevpage
         CALL GET_FLDBUF(s_pja.check) RETURNING ma_qry_tmp[ARR_CURR()].check
         CALL m_pja_qry_reset_multi_sel(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            CALL GET_FLDBUF(s_pja.check) RETURNING ma_qry_tmp[ARR_CURR()].check
            CALL m_pja_qry_reset_multi_sel(pi_start_index, pi_end_index)
            CALL m_pja_qry_accept(pi_start_index+ARR_CURR()-1)
         ELSE                        #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION m_pja_qry_reset_multi_sel(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 
            pi_end_index     LIKE type_file.num10 
   DEFINE   li_i             LIKE type_file.num10,
            li_j             LIKE type_file.num10 
...
# Modify   	:
##################################################
FUNCTION m_pja_qry_display_array(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 
            pi_end_index     LIKE type_file.num10 
   DEFINE   li_continue      LIKE type_file.num5,
   ...
   DISPLAY ARRAY ma_qry_tmp TO s_pja.*
      BEFORE DISPLAY
         CALL cl_set_act_visible("prevpage,nextpage,reconstruct",TRUE)
         IF (ps_hide_act IS NOT NULL) THEN   
            CALL cl_set_act_visible(ps_hide_act, FALSE)
...
# Modify   	:
##################################################
FUNCTION m_pja_qry_accept(pi_sel_index)
      ...
      FOR li_i = 1 TO ma_qry.getLength()
         IF (ma_qry[li_i].check = 'Y') THEN
            IF (lsb_multi_sel.getLength() = 0) THEN
               CALL lsb_multi_sel.append(ma_qry[li_i].pja01 CLIPPED) 
            ELSE
               CALL lsb_multi_sel.append("|" || ma_qry[li_i].pja01 CLIPPED)  
            END IF
         END IF    
   ...
   ELSE
      LET ms_ret1 = ma_qry[pi_sel_index].pja01 CLIPPED
      LET ms_ret2 = ma_qry[pi_sel_index].pja02 CLIPPED
   END IF
END FUNCTION
#FUN-9A0093
