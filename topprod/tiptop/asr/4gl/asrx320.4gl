...
# Modify.........: No.FUN-D40128 13/05/14 By wangrr 增加"ctrl+g"功能,更改"品名""規格"取值方式
...
MAIN
   ...
   LET g_sql = "sfu00.sfu_file.sfu00,",
               ...
               "num2.type_file.num5"        #FUN-CB0004 add
   
   LET l_table = cl_prt_temptable("asrx320",g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'   # If background job sw is off
      THEN CALL x320_tm(0,0)              # Input print condition
      ELSE CALL x320()                    # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time      # FUN-B80063--add--
END MAIN
 
FUNCTION x320_tm(p_row,p_col)
   ...
   OPEN WINDOW x320_w AT p_row,p_col WITH FORM "asr/42f/asrx320"
   ATTRIBUTE (STYLE = g_win_style CLIPPED)
 
   CALL cl_set_locale_frm_name("asrx320")
   CALL cl_ui_init()
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_rep_clas CLIPPED,"'",
                         " '",g_template CLIPPED,"'"
         CALL cl_cmdat('asrx320',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW x320_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time      # FUN-B80063--add--
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8        #No.FUN-6B0014
     ...
     DEFINE l_num2        LIKE type_file.num5        #FUN-CB0004 add
     ...
     LET l_sql = "SELECT sfu00,sfu01,sfu02,sfv17,sfu04,'',sfu05,'',",
                 ...
                 "sfv05,'',sfv06,'',sfv07,sfv09,sfv13,sfv12,",               #FUN-CB0004 add '',''
     ...
     PREPARE x320_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time      # FUN-B80063--add--
        EXIT PROGRAM
     END IF
     DECLARE x320_curs1 CURSOR FOR x320_prepare1
 
     #CALL cl_outnam("asrx320") RETURNING l_name       #No.FUN-7C0034
     
     SELECT sma115,sma116 INTO g_sma115,g_sma116 FROM sma_file
 
     #CALL cl_prt_pos_len()                            #No.FUN-7C0034
     #START REPORT x320_rep TO l_name                  #No.FUN-7C0034
     #LET g_pageno = 0                                 #No.FUN-7C0034
 
     FOREACH x320_curs1 INTO sr.*
          ...
          SELECT ima02,ima021 INTO sr.ima02,sr.ima021
             FROM ima_file
          # WHERE ima01 = sr.sfu04  #FUN-D40128 mark
            WHERE ima01 = sr.sfv04  #FUN-D40128 add
          IF SQLCA.sqlcode THEN
             LET sr.ima02 = ''
             LET sr.ima021 = ''
          END IF
         ...
         #FUN-CB0004--add--str---
          SELECT imd02 INTO sr.imd02 FROM imd_file
           WHERE imd01 = sr.sfv05
          SELECT ime03 INTO sr.ime03 FROM ime_file
           WHERE ime01 = sr.sfv05
             AND ime02 = sr.sfv06
          IF SQLCA.sqlcode THEN    
             LET sr.imd02 = ''
             LET sr.ime03 = ''  
         ...
         #FUN-CB0004--add--end---
          ...
          ELSE 
             LET l_str2 = NULL
          END IF
          LET l_num1 = 0    LET l_num2 = 3           #FUN-CB0004 add
          EXECUTE insert_prep USING sr.sfu00,sr.sfu01,sr.sfu02,sr.sfv17,sr.sfu04,
                                    ...
                                    sr.sfv14,sr.sfv15,sr.sfv08,sr.sfv05,sr.imd02,sr.sfv06,sr.ime03,      #FUN-CB0004 add sr.imd02,sr.ime03
                                    sr.sfv07,sr.sfv09,sr.sfv13,sr.sfv12,sr.sfv30,
                                    sr.sfv31,sr.sfv32,sr.sfv33,sr.sfv34,sr.sfv35,
                                    l_str2,l_num1,l_num2                              #FUN-CB0004 add l_num1,l_num2
          #No.FUN-7C0034 --end--
     END FOREACH
...
###XtraGrid###     CALL cl_prt_cs3('asrx320','asrx320',g_sql,g_str)
    #FUN-CB0004--add--str---
     LET g_xgrid.order_field = "sfu01,sfv03"
    #LET g_xgrid.grup_field = "sfu01"
     LET g_xgrid.skippage_field = "sfu01" 
     LET g_xgrid.condition = cl_getmsg('lib-160',g_lang),tm.wc
     CALL cl_xg_view()    ###XtraGrid###
    #FUN-CB0004--add--end---
     #FINISH REPORT x320_rep                         
