# Prog. Version..: '5.30.07-13.05.16(00005)'     #
#
# Pattern name...: asrr140.4gl
# Descriptions...: 生產指示單
...
# Modify.........: No.FUN-CB0004 12/11/13 By dongsz sql條件修改
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,
                         ...
                         " '",g_rlang CLIPPED,"'", #No.FUN-7C0078
                            ...
                            " '",g_rep_clas CLIPPED,"'",           
                            " '",g_template CLIPPED,"'"            
            CALL cl_cmdat('asrr140',g_time,l_cmd)
         END IF
         CLOSE WINDOW r140_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time      # FUN-B80063--add--
         EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8        #No.FUN-6B0014
     ...
     LET l_sql = "SELECT sre01,sre02,sre03,sre05,sre06,sre04,ima02,ima021,sra03,sre07",
                 " FROM sra_file,sre_file,ima_file ",
                 " WHERE sra01 = sre03 ",
                 "   AND sra02 = sre04 ",    
                 "   AND sre07 <> 0 ",    
                 "   AND ima01 = sre04 ",
     ...
     #CALL cl_outnam("asrr140") RETURNING l_name
     ...
     IF g_zz05 = 'Y' THEN                                                        
        CALL cl_wcchp(tm.wc,'sre03,sre04')                           
          RETURNING tm.wc                                                        
     END IF                                                                      
     LET g_str = tm.wc,";",tm.yy,";",tm.mm,";",tm.remk                                
     CALL cl_prt_cs1('asrr140','asrr140',l_sql,g_str)   
