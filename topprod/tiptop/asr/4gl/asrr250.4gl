...
#
# Pattern name...: asrr250.4gl
# Descriptions...: 機台(生產線)稼動率分析表(每日)
# Input parameter:
# Return code....:
...
# Modify.........: No.FUN-7B0103 07/11/21 BY xiaofeizhu報表改為Crystal Report或p_query
...
### *** FUN-7B0103 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>>--*** ##                                                     
   LET g_sql = "srf03.srf_file.srf03,",
               "srf05.srf_file.srf05,",
               "eci06.eci_file.eci06,",
               "time1.srg_file.srg10,",
               "time2.srg_file.srg10,",
               "time3.srg_file.srg10"
    LET l_table = cl_prt_temptable('asrr250',g_sql) CLIPPED   # 產生Temp Table                                                      
...
DEFINE lc_qbe_sn        LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW r250_w AT p_row,p_col
        WITH FORM "asr/42f/asrr250"
 ...
 WHILE TRUE
   ...
   LET tm.edate=r250_GETLASTDAY(tm.bdate)
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('asrr250',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW r250_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time      # FUN-B80063--add--
      EXIT PROGRAM
...
DEFINE    l_name   LIKE type_file.chr20,         #No.FUN-680130 VARCHAR(20)
     ...
     LET l_sql = " SELECT srf03,'',srf05,SUM(srg10),0,0,0 FROM srf_file,srg_file",
                 "  WHERE srf01=srg01 AND ",tm.wc CLIPPED,
                 "    AND srfconf='Y'"
     IF NOT cl_null(tm.bdate) THEN
        LET l_sql=l_sql," AND srf05>='",tm.bdate,"'"
     END IF
     IF NOT cl_null(tm.edate) THEN
        LET l_sql=l_sql," AND srf05<='",tm.edate,"'"
     END IF
     LET l_sql=l_sql, " GROUP BY srf03,srf05",
                      " ORDER BY srf05"
     PREPARE r250_pr1 FROM l_sql
     IF STATUS THEN 
        CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time      # FUN-B80063--add--
        EXIT PROGRAM 
     END IF
     DECLARE r250_cs1 CURSOR FOR r250_pr1
 
#    CALL cl_outnam('asrr250') RETURNING l_name                                   #No.FUN-7B0103
     ## *** 與 Crystal Reports 串聯段 - <<<< 清除暫存資料 >>>> FUN-7B0103 *** ##                                                    
     CALL cl_del_data(l_table)                                                                                                      
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog   ### FUN-7B0103 add ###                                              
     #------------------------------ CR (2) ------------------------------#
     LET g_pageno = 0
#    START REPORT r250_rep TO l_name                                              #No.FUN-7B0103
...
#      OUTPUT TO REPORT r250_rep(sr.*)                      #No.FUN-7B0103
        ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-7B0103 *** ##                                                   
           EXECUTE insert_prep USING                                                                                                
                   sr.srf03,sr.srf05,sr.eci06,sr.time1,sr.time2,sr.time3                                                
          #------------------------------ CR (3) ------------------------------# 
     END FOREACH
...
#No.FUN-7B0103--Begin-Add                                                                                                               
    ...
    CALL cl_prt_cs3('asrr250','asrr250',l_sql,g_str)                                                                                  
    #------------------------------ CR (4) ------------------------------#
#No.FUN-7B0103--End-Add
 
#    FINISH REPORT r250_rep                                                        #No.FUN-7B0103
...
#       LET l_str=g_x[11] CLIPPED,' ',tm.bdate,'~',tm.edate
...
#    ON EVERY ROW
#     PRINT COLUMN g_c[31],sr.srf05,
#           COLUMN g_c[32],sr.srf03,
#           COLUMN g_c[33],sr.eci06,
#           COLUMN g_c[34],cl_numfor(sr.time1,34,3),
#           COLUMN g_c[35],cl_numfor(sr.time2,35,3),
#           COLUMN g_c[36],cl_numfor(sr.time3,36,3)
...
FUNCTION r250_GETLASTDAY(p_date)
