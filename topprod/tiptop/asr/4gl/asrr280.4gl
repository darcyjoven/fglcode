...
### *** FUN-7B0103 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>>--*** ##                                                     
   LET g_sql = "srg03.srg_file.srg03,",                                                                                             
    ...
    LET l_table = cl_prt_temptable('asrr280',g_sql) CLIPPED   # 產生Temp Table                                                      
...
FUNCTION r280_tm(p_row,p_col)
   ...
   END IF
   OPEN WINDOW r280_w AT p_row,p_col
        WITH FORM "asr/42f/asrr280"
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,
                         ...
                         " '",g_rlang CLIPPED,"'", #No.FUN-7C0078
                            ...
                            " '",g_rep_clas CLIPPED,"'",           
                            " '",g_template CLIPPED,"'"            
            CALL cl_cmdat('asrr280',g_time,l_cmd)
         END IF
         CLOSE WINDOW r280_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time      # FUN-B80063--add--
         EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8        #No.FUN-6B0014
     ...
     PREPARE r280_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time      # FUN-B80063--add--
        EXIT PROGRAM
     END IF
     DECLARE r280_curs1 CURSOR FOR r280_prepare1
#    CALL cl_outnam("asrr280") RETURNING l_name            #No.FUN-7B0103
#    START REPORT r280_rep TO l_name                       #No.FUN-7B0103
     ## *** 與 Crystal Reports 串聯段 - <<<< 清除暫存資料 >>>> FUN-7B0103 *** ##                                                    
     CALL cl_del_data(l_table)                                                                                                      
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog   ### FUN-7B0103 add ###                                              
     #------------------------------ CR (2) ------------------------------#
     LET g_pageno = 0
     FOREACH r280_curs1 INTO sr.*
...
#       OUTPUT TO REPORT r280_rep(sr.*)                     #No.FUN-7B0103
...
#No.FUN-7B0103--Begin-Add                                                                                                           
    ...
    LET g_str = ''                                                                                                                  
    LET g_str = tm.wc,";",tm.bdate,";",tm.edate,";",g_azi03,";",g_azi04,";",g_azi05                                                                                     
    CALL cl_prt_cs3('asrr280','asrr280',l_sql,g_str)                                                                                
    #------------------------------ CR (4) ------------------------------#                                                          
#No.FUN-7B0103--End-Add
#    FINISH REPORT r280_rep                                                       #No.FUN-7B0103                             
