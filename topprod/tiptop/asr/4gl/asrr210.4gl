...
#
# Pattern name...: asrr210.4gl (copy from asfr501.4gl)
# Descriptions...: 工單發/領料單列印
# Input parameter:
# Return code....:
# Date & Author..: 06/03/01 By kim
...
# Modify.........: No.FUN-7C0034 07/12/15 By Sunyanchun老報表改成CR
...
DEFINE tm  RECORD		        # Print condition RECORD
        ...
        p       LIKE type_file.chr1,    #No.FUN-680130 VARCHAR(01)
        q       LIKE type_file.chr1,    #No.FUN-680130 VARCHAR(01)
        r       LIKE type_file.chr1,    #No.FUN-680130 VARCHAR(01)
...
MAIN
   ...
   LET l_table1 = cl_prt_temptable('asrr210_1',g_sql) CLIPPED
   ...
   LET l_table2 = cl_prt_temptable('asrr210_2',g_sql) CLIPPED
   ...
   LET l_table3 = cl_prt_temptable('asrr210_3',g_sql) CLIPPED                   
 ...
 WHILE TRUE
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('asrr210',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW r210_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time      # FUN-B80063--add--
      EXIT PROGRAM
...
DEFINE    l_name   LIKE type_file.chr20,         #No.FUN-680130 VARCHAR(20)
          ...
          l_gen02  LIKE gen_file.gen02,          #No.FUN-7C0034 
          l_ima02  LIKE ima_file.ima02,          #No.FUN-7C0034
          l_sfq		RECORD LIKE sfq_file.*,  #No.FUN-7C0034
     ...
     LET l_sql = " SELECT * FROM sfp_file",
                 "  WHERE ",tm.wc CLIPPED,
                 "    AND sfp06 IN ('A','C') AND sfpconf!='X' " #FUN-660106
     PREPARE r210_pr1 FROM l_sql
     IF STATUS THEN 
        CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time      # FUN-B80063--add--
        EXIT PROGRAM 
     END IF
     DECLARE r210_cs1 CURSOR FOR r210_pr1
 
#     CALL cl_outnam('asrr210') RETURNING l_name    #No.FUN-7C0034
 
#     LET g_pageno = 0
#     LET g_no = 0
#     START REPORT r210_rep TO l_name    #No.FUN-7C0034
     FOREACH r210_cs1 INTO l_sfp.*
       IF STATUS THEN 
       ...
       IF STATUS OR SQLCA.sqlerrd[3]=0 THEN
          CALL cl_err3("upd","sfp_file",l_sfp.sfp01,"",STATUS,"","upd sfp_file",1)
       END IF
 
       DECLARE r210_cs3 CURSOR FOR
                   SELECT * FROM sfq_file
                    WHERE sfq01=l_sfp.sfp01
       FOREACH r210_cs3 INTO l_sfq.*
          ...
          EXECUTE insert_prep2 USING 
            l_sfp.sfp01,l_sfq.sfq02,l_sfq.sfq04,l_sfq.sfq05,l_sfq.sfq03,l_ima02
       END FOREACH
       #No.FUN-7C0034-----end-----       
       ...
       END IF
       PREPARE r210_pr2 FROM l_sql
       IF STATUS THEN 
          CALL cl_err('prepare2:',STATUS,1) 
          CALL cl_used(g_prog,g_time,2) RETURNING g_time      # FUN-B80063--add--
          EXIT PROGRAM 
       END IF
       DECLARE r210_cs2 CURSOR FOR r210_pr2
       FOREACH r210_cs2 INTO sr.*
          ...
          IF SQLCA.sqlcode THEN
             LET l_gen02=''
          END IF
          LET l_sfp.sfp06 = l_sfp.sfp06,' ',r210_sfp06(l_sfp.sfp06)
          EXECUTE insert_prep1 USING l_sfp.sfp01,
             l_sfp.sfp02,l_sfp.sfp03,l_gen02,l_sfp.sfp07,l_sfp.sfp06,l_sfp.sfp04,
             sr.order1,sr.order2,sr.sfs02,sr.sfs05,sr.ima23
            
          EXECUTE insert_prep3 USING l_sfp.sfp01,                               
             sr.order1,sr.order2,sr.sfs02,sr.sfs03,sr.sfs04,sr.sfs05,           
             sr.sfs06,sr.sfs07,sr.sfs08,sr.sfs09,                      
             sr.ima02,sr.ima23,sr.img10
          #No.FUN-7C0034-----end-----           
#          OUTPUT TO REPORT r210_rep(l_sfp.*, sr.*)     #No.FUN-7C0034
       END FOREACH
     END FOREACH
#     FINISH REPORT r210_rep    #No.FUN-7C0034   
...
#No.FUN-7C0034-----start-----
     ...
     IF g_zz05 = 'Y' THEN                                                                                                          
       ...
       LET g_str = tm.wc
     END IF
     LET g_str = g_str,";",tm.p
     IF tm.p = 'N' THEN   #不依倉管員跳頁  
        CALL cl_prt_cs3('asrr210','asrr210',l_sql,g_str)
     ELSE
        CALL cl_prt_cs3('asrr210','asrr210_1',l_sql,g_str)
     END IF
     #No.FUN-7C0034-----end-----   
     CALL  cl_used(g_prog,g_time,2) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6B0014
END FUNCTION
 #No.FUN-7C0034-----start-----     
#REPORT r210_rep(l_sfp, sr)
#  DEFINE
...
#     PRINT
#     LET g_x[1]=r210_sfp06(l_sfp.sfp06)
...
#          COLUMN 27,g_x[16] CLIPPED,l_sfp.sfp06,' ',r210_sfp06(l_sfp.sfp06),
...
FUNCTION r210_sfp06(p_sfp06)
    DEFINE p_sfp06	LIKE sfp_file.sfp06                  #No.FUN-680130 VARCHAR(10)
    DEFINE l_str	LIKE type_file.chr8                  #No.FUN-680130 VARCHAR(8)
     CASE WHEN p_sfp06='A' LET l_str=g_x[64] CLIPPED
          WHEN p_sfp06='C' LET l_str=g_x[65] CLIPPED
     END CASE
    RETURN l_str
END FUNCTION
#Patch....NO.TQC-610037 <> #
