...
# Modify.........: No.FUN-B40092 11/06/13 By xujing 憑證類CR轉GR
...
# Modify.........: No:FUN-C50007 12/05/14 By minpp GR程式优化 
...
TYPE sr1_t RECORD
    ...
    l_aag02_l LIKE aag_file.aag01,
    l_aag02_r LIKE aag_file.aag01,
    l_aag13_l LIKE aag_file.aag01,
    l_aag13_r LIKE aag_file.aag01,
...
MAIN
   ...
   LET l_table = cl_prt_temptable('gglg308',g_sql) CLIPPED                                                                          
   IF l_table = -1 THEN
...
#  END IF
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL gglg308_tm()	        	# Input print condition
      ELSE CALL gglg308()			# Read data and create out-file
   END IF
...
#  CALL s_dsmark(g_bookno)   #No.TQC-740093
   ...
   OPEN WINDOW gglg308_w AT p_row,p_col
     WITH FORM "ggl/42f/gglg308"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW gglg308_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1) 
      EXIT PROGRAM
   END IF
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
		...
		#        " '",g_bookno CLIPPED,"'",       #No.TQC-740093
         ...
         CALL cl_cmdat('gglg308',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW gglg308_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1) 
      EXIT PROGRAM
   END IF
...
#No.TQC-740093 ---End
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'gglg308'
    #TQC-650055...............begin
...
#    SELECT aaa03 INTO g_aaa03 FROM aaa_file WHERE aaa01 = g_bookno          #No.TQC-740093
 ...
 #        CALL cl_err('',SQLCA.sqlcode,0)     #No.FUN-660124
  ...
  LET l_sql = "SELECT aba01,aba02,aba10,aba21,abb02,abb04,abb06,",
                 ...
                 "       azi07,aac02,0,'','','','',0,0",
               # "  FROM aba_file,abb_file,aac_file,aag_file,OUTER azi_file",       #FUN-C50007  MARK
                 "  FROM aba_file,abb_file LEFT OUTER JOIN azi_file ON abb24=azi01,aac_file,aag_file ",  #FUN-C50007 ADD
                 " WHERE aba00 = abb00",
                 "   AND aba01 = abb01",
...
#                "   AND aba01[1,3] = aac01",
               ...
               # "   AND azi_file.azi01 = abb_file.abb24 ",   #FUN-C50007  MARK
                 "   AND ",tm.wc CLIPPED
     ...
     PREPARE gglg308_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time
        CALL cl_gre_drop_temptable(l_table||"|"||l_table1)
        EXIT PROGRAM
      END IF
     DECLARE gglg308_curs1 CURSOR FOR gglg308_prepare1
#No.FUN-770061---Begin
#     CALL cl_outnam('gglg308') RETURNING l_name
#    #TQC-650055...............begin
#     IF g_len = 0 OR g_len IS NULL THEN
#       #IF tm.n = 'N' THEN LET g_len = 122 ELSE LET g_len = 140 END IF
...
#     FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR
#    #TQC-650055...............end
#     START REPORT gglg308_rep TO l_name
 
#     LET g_pageno = 0
#No.FUN-770061---End
     FOREACH gglg308_curs1 INTO sr.*
       IF sr.abb06 = '1' THEN
       ...
       END IF
#No.FUN-770061---Begin                                                                                                              
        EXECUTE insert_prep USING sr.aba01,sr.aba02,sr.aba21,sr.abb02,sr.abb04,sr.abb07f,                                                    
                                  sr.abb24,sr.abb25,sr.aac02,sr.l_aag01_l,sr.l_aag01_r,                                             
                                  sr.l_aag02_l,sr.l_aag02_r,sr.l_aag13_l,sr.l_aag13_r,                                              
                                  sr.l_abb07_l,sr.l_abb07_r,sr.l_pageno,sr.azi07,t_azi04,t_azi05 
#       OUTPUT TO REPORT gglg308_rep(sr.*)                                                                                          
#No.FUN-770061---End
     END FOREACH
#No.FUN-770061---Begin
#     FINISH REPORT gglg308_rep
         IF g_zz05 = 'Y' THEN                                                                                                       
...
###GENGRE###      LET g_str=tm.wc ,";",tm.n,";",tm.e,";",g_azi02,";",g_aaz.aaz64,";",l_str,";",l_str1                                           
                                                                                                                                    
###GENGRE###   LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED                                                               
###GENGRE###   CALL cl_prt_cs3('gglg308','gglg308',l_sql,g_str)  
    CALL gglg308_grdata()    ###GENGRE###
#     CALL cl_prt(l_name,g_prtway,g_copies,g_len)
#No.FUN-770061---End 
#       CALL  cl_used(g_prog,g_time,2) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6A0097 #FUN-B40092 mark
END FUNCTION
#No.FUN-770061---Begin    
#REPORT gglg308_rep(sr)
...
FUNCTION gglg308_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("gglg308")
        IF handler IS NOT NULL THEN
            START REPORT gglg308_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY aba01"          
            DECLARE gglg308_datacur1 CURSOR FROM l_sql
            FOREACH gglg308_datacur1 INTO sr1.*
                OUTPUT TO REPORT gglg308_rep(sr1.*)
            END FOREACH
            FINISH REPORT gglg308_rep
        END IF
...
REPORT gglg308_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B40092------add------str
    DEFINE l_p4           LIKE azi_file.azi02
    DEFINE l_p5           LIKE aaz_file.aaz64
    DEFINE l_p6           LIKE type_file.chr1000
    DEFINE l_p7           LIKE type_file.chr1000 
    DEFINE l_l            LIKE aag_file.aag01
    DEFINE l_r            LIKE aag_file.aag01
    DEFINE l_show_abb07_l LIKE abb_file.abb07
    DEFINE l_show_abb07_r LIKE abb_file.abb07
    DEFINE l_sum_abb07_l  STRING
    DEFINE l_pageno       LIKE type_file.num5
    DEFINE l_aba02        LIKE aba_file.aba02  
    DEFINE l_str          STRING
    DEFINE l_abb07_1_fmt  STRING
    DEFINE l_abb07_r_fmt  STRING
    DEFINE l_abb07f_fmt   STRING
    DEFINE l_abb25_fmt    STRING
    DEFINE l_show_abb07_1_fmt  STRING
    DEFINE l_show_abb07_r_fmt  STRING
    #FUN-B40092------add------end
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            #FUN-B40092------add------str
            SELECT azi02 INTO l_p4 FROM azi_file WHERE azi01 = g_aaa03
            PRINTX l_p4

            LET l_p5 = '[',g_aaz.aaz64,']'
            PRINTX l_p5

            LET l_p6 = sr1.aba01[g_no_sp,g_no_ep] CLIPPED,
                        '-',g_pageno USING "&&&&",'/',sr1.l_pageno USING "&&&&"
            PRINTX l_p6
            LET l_p7 = YEAR(sr1.aba02) USING "###&" ,'    ',
                         MONTH(sr1.aba02) USING "#&",'    ',DAY(sr1.aba02) USING "#&"
            PRINTX l_p7
            #FUN-B40092------add------end
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B40092------add------str
            LET l_abb07_1_fmt = cl_gr_numfmt('abb_file','abb07',t_azi04)
            PRINTX l_abb07_1_fmt
            LET l_abb07_r_fmt = cl_gr_numfmt('abb_file','abb07',t_azi04)
            PRINTX l_abb07_r_fmt
            LET l_abb07f_fmt = cl_gr_numfmt('abb_file','abb07f',t_azi04)
            PRINTX l_abb07f_fmt
            LET l_abb25_fmt = cl_gr_numfmt('abb_file','abb25',t_azi04)
            PRINTX l_abb25_fmt
            IF tm.e = 'Y' THEN
               LET l_l = sr1.l_aag13_l
            ELSE 
               LET l_l = sr1.l_aag02_l
            END IF
            PRINTX l_l

            IF tm.e = 'Y' THEN
               LET l_r = sr1.l_aag13_r
            ELSE
               LET l_r = sr1.l_aag02_r
            END IF
            PRINTX l_r
        ...
        AFTER GROUP OF sr1.aba01
            #FUN-B40092------add------str
            LET l_show_abb07_1_fmt = cl_gr_numfmt('abb_file','abb07',t_azi05)
            PRINTX l_show_abb07_1_fmt
            LET l_show_abb07_r_fmt = cl_gr_numfmt('abb_file','abb07',t_azi05)
            PRINTX l_show_abb07_r_fmt
            IF GROUP SUM(sr1.l_abb07_l) = 0 THEN
               LET l_show_abb07_l = sr1.l_abb07_l USING '--,---,---,---,---,--&.&&'
            ELSE
               LET l_show_abb07_l = GROUP SUM(sr1.l_abb07_l)  
            END IF
            PRINTX l_show_abb07_l

            IF GROUP SUM(sr1.l_abb07_r) = 0 THEN
               LET l_show_abb07_r = sr1.l_abb07_r USING '--,---,---,---,---,--&.&&'
            ELSE
               LET l_show_abb07_r = GROUP SUM(sr1.l_abb07_r)
            END IF
            PRINTX l_show_abb07_r

            LET l_sum_abb07_l = GROUP SUM (sr1.l_abb07_l)
            LET l_str = s_sayc2(l_sum_abb07_l,50) 
