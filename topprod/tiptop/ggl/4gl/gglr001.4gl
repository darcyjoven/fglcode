...
# Modify.........: No.MOD-890101 08/09/17 By Sarah r001_bom()段組SQL時不需過濾aag01!=p_key,這樣會將獨立帳戶科目都排除掉
...
DEFINE tm RECORD
          wc        STRING,    #NO FUN-690009   VARCHAR(300)  #MOD-BC0026 mod 1000 -> STRING
          bookno    LIKE aaa_file.aaa01,       #No.FUN-740032
          t     LIKE type_file.chr1,       # Prog. Version..: '5.30.06-13.03.12(01)       #No.TQC-650071 
...
#No.FUN-830053---Begin
   ...
   LET g_sql = "aag01.aag_file.aag01,",
               ...
               "aag08.aag_file.aag08,"
   #NO.FUN-B40024 --End   
   LET l_table = cl_prt_temptable('gglr001',g_sql) CLIPPED                                                                          
...
#   LET tm.bookno= ARG_VAL(9)   #No.TQC-7B0102 mark
    LET tm.t     = ARG_VAL(9)   #TQC-610056  #No.TQC-7B0102 10->9
    #No.FUN-740032  --End  
   #No.FUN-570264 --start--
...
#No.TQC-5B0097-begin
  ...
  ELSE
    IF cl_null(g_bgjob) OR g_bgjob = 'N'      # If background job sw is off
       THEN CALL r001_tm(0,0)                 # Input print condition
       ELSE CALL r001()                       # Read data and create out-file
    END IF
  END IF
...
FUNCTION r001_tm(p_row, p_col)
   ...
   OPEN WINDOW r001_w AT p_row, p_col
        WITH FORM "ggl/42f/gglr001"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
#No.FUN-B20010  --Mark End
    INPUT BY NAME tm.t,tm.h,tm.more ATTRIBUTE(WITHOUT DEFAULTS)     #FUN-B20010
          #輸入2.其他列印條件                                                                                                       
...
#                         " '",tm.bookno CLIPPED,"'" ,          #No.FUN-740032 #No.TQC-7B0102 mark
          ...
          CALL cl_cmdat('gglr001',g_time,l_cmd)    # Execute cmd at later time
       END IF
       CLOSE WINDOW gglr001_w
       CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
       EXIT PROGRAM
    END IF
...
#    CALL cl_outnam('gglr001') RETURNING l_name  #NO.TQC-650054 #FUN-830053 MARK BY COCKROACH
 
    SELECT zo02 INTO g_company FROM zo_file
      WHERE  zo01 = g_rlang
#TQC-940042  ---start                                                                                                               
   #SELECT zz17, zz05 INTO g_len, g_zz05                                                                                            
   #  FROM zz_file WHERE zz01= 'gglr001'                                                                                            
...
#No.TQC-5B0097-end
    ...
    PREPARE r001_prepare1 FROM l_sql
...
#No.TQC-5B0097-begin
       ...
       LET g_pageno = 0
#       OUTPUT TO REPORT r001_rep(p_level,sr.*)     #No.FUN-830053 
#       OUTPUT TO REPORT r001_rep(p_level+1,sr_null.*)  #No.FUN-830053 
       LET g_end = 0
       CALL r001_bom(0,sr.aag01)
       LET g_end = 1
#No.TQC-5B0097-end
    END FOREACH
#No.FUN-830053---Begin                                                                                                              
#    FINISH REPORT r001_rep                                                                                                         
      LET g_str=tm.t,";",g_towhom                                                                                                   
                                                                                                                                    
   LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED                                                               
   CALL cl_prt_cs3('gglr001','gglr001',l_sql,g_str)                                                                                 
...
FUNCTION r001()
   ...
   LET l_n = 1
   CALL sr.clear()
   FOREACH r001_curs1 INTO sr[l_n].*
   ...
   CALL cl_prt_cs3('gglr001','gglr001',g_sql,g_str)   
...
#          OUTPUT TO REPORT r001_rep(p_level,sr[i].*)    #No.FUN-830053 
...
#             OUTPUT TO REPORT r001_rep(p_level+1,sr[600].*)  #No.FUN-830053 
             CALL r001_bom(p_level,sr[i].aag01)
          END IF
        END FOR
END FUNCTION
#No.TQC-5B0097-end
