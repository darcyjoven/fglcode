...
#              "sign_show.type_file.chr1,sign_str.type_file.chr1000"    #是否顯示簽核資料(Y/N)  #TQC-C10039  #MOD-C20064 Mark TQC-C10039
   ...
   LET l_table = cl_prt_temptable('gglr903',g_sql) CLIPPED   # 產生Temp Table
...
#       CALL cl_err(g_aaa03,SQLCA.sqlcode,0)   # No.FUN-660124 
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'            # If background job sw is off
      THEN CALL gglr903_tm(0,0)         # Input print condition
      ELSE CALL gglr903()                       # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD

END MAIN
 
FUNCTION gglr903_tm(p_row,p_col)
...
#   CALL s_dsmark(g_bookno)   #No.FUN-740055
   ...
   END IF
   OPEN WINDOW gglr903_w AT p_row,p_col
        WITH FORM "ggl/42f/gglr903"
################################################################################
...
#                         " '",g_bookno CLIPPED,"'",    #No.FUN-740055
         ...
         CALL cl_cmdat('gglr903',g_time,l_cmd)  # Execute cmd at later time
      END IF
      CLOSE WINDOW gglr903_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
      EXIT PROGRAM
...
#     LOCATE l_img_blob IN MEMORY    #TQC-C10039  #MOD-C20064 Mark TQC-C10039
     ...
     SELECT aaf03 INTO g_company FROM aaf_file WHERE aaf01 =tm.aba00    #No.FUN-740055 
                        AND aaf02 = g_rlang
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'gglr903'
    #TQC-650055...............begin
    ...
    #TQC-650055...............end
     ...
     LET l_sql = "SELECT ' ',aba_file.*,abb_file.*,aag02,gem02 ",
...
#                 " WHERE aba00 = '",g_bookno,"' AND abb00 ='",g_bookno,"'",    #No.FUN-740055
                 ...
                 " AND abb00 = aag00 ",   #FUN-770041 add
                 " AND abb_file.abb03 = aag_file.aag01 ", 
                 " AND abb_file.abb05 = gem_file.gem01 ", 
     ...
     PREPARE gglr903_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare1:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
        EXIT PROGRAM
     END IF
     DECLARE gglr903_curs1 CURSOR FOR gglr903_prepare1
 
     #-->額外摘要
      LET l_sql = " SELECT abc03,abc04 FROM abc_file",
                  "  WHERE abc00 = ? AND abc01=?  AND abc02= ? ",
                  "   ORDER BY 1 "
     PREPARE gglr903_pre2 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare2:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
        EXIT PROGRAM
     END IF
     DECLARE r903_c2  CURSOR FOR gglr903_pre2
 
     IF NOT cl_null(g_rpt_name)   #已有外部指定報表名稱
        THEN
        LET l_name = g_rpt_name
    #ELSE                                             #FUN-770041 mark
    #   CALL cl_outnam('gglr903') RETURNING l_name    #FUN-770041 mark
     END IF
    #TQC-650055...............begin
    #IF g_len = 0 OR g_len IS NULL THEN LET g_len = 80 END IF
     ...
     FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR
    #TQC-650055...............end
    #START REPORT gglr903_rep TO l_name    #FUN-77041 mark
 
     LET g_pageno = 0
     FOREACH gglr903_curs1 INTO sr.*
        IF SQLCA.sqlcode != 0
        ...
        SELECT azi04 INTO sr.azi04 FROM azi_file WHERE azi01 = sr.abb.abb24   #MOD-640001   #No.CHI-6A0004
       #OUTPUT TO REPORT gglr903_rep(sr.*) #FUN-770041 mark
 
      #str FUN-770041 add
      IF tm.v='Y' THEN
         FOREACH r903_c2 USING sr.aba.aba00,sr.aba.aba01,sr.abb.abb02
                                 INTO g_cnt,g_msg
            IF SQLCA.sqlcode THEN
...
#   LET g_cr_apr_key_f = "aba01"             #報表主鍵欄位名稱  #TQC-C10039  #MOD-C20064 Mark TQC-C10039
   CALL cl_prt_cs3('gglr903','gglr903',g_sql,g_str)
   #------------------------------ CR (4) ------------------------------#
   #end FUN-770041 add
 
    #FINISH REPORT gglr903_rep                      #FUN-770041 mark
