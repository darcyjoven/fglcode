...
MAIN
   ...
   LET l_table = cl_prt_temptable('gglr400',g_sql) CLIPPED
...
#   LET tm.a = g_bookno   #No.FUN-740055
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL gglr400_tm(0,0)		        # Input print condition
      ELSE CALL gglr400()			# Read data and create out-file
   END IF
...
FUNCTION gglr400_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,    #NO FUN-690009   SMALLINT	
   ...
   OPEN WINDOW gglr400_w WITH FORM "ggl/42f/gglr400"
...
#   IF INT_FLAG THEN
#      LET INT_FLAG = 0 CLOSE WINDOW gglr400_w EXIT PROGRAM
#   END IF
...
#No.FUN-B20010  --Mark End
   ...
   #INPUT BY NAME yy,mm,tm.a,tm.rate_op,tm.t,tm.e,tm.h,tm.more WITHOUT DEFAULTS  #FUN-6C0012 #No.MOD-860252 add tm.h #B20010 mark
    INPUT BY NAME yy,mm,tm.rate_op,tm.t,tm.e,tm.h,tm.more ATTRIBUTE(WITHOUT DEFAULTS)  #FUN-B20010
         #No.FUN-580031 --start--
         BEFORE INPUT
             CALL cl_qbe_display_condition(lc_qbe_sn)
         #No.FUN-580031 ---end---    
...
#No.FUN-B20010  --Mark End 
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('gglr400',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW gglr400_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8      #No.FUN-6A0097
   ...
   DEFINE l_yy      LIKE type_file.chr6  #add by dengsy170304
   ...
   # LET l_sql = "SELECT UNIQUE aag_file.*,tah08 ",     #MOD-950238 MARK
    ...
    #            "   AND aag00 ='",tm.a,"' ",  #No.FUN-740055 
     ...
     PREPARE gglr400_prepare1 FROM l_sql
     ...
     DECLARE gglr400_curs1 CURSOR FOR gglr400_prepare1
 
     #No.FUN-780031  --Begin
     #CALL cl_outnam('gglr400') RETURNING l_name 
     #START REPORT gglr400_rep TO l_name
     #LET g_pageno = 0
     #No.FUN-780031  --End  
    ...
    #MOD-C70285------S------
     ...
     SELECT azi04,azi05 INTO g_azi04,g_azi05
      ...
      WHERE azi01 = l_aaa03
    #MOD-C70285------E------
    #FOREACH gglr400_curs1 INTO l_aag.*,l_tah08 #MOD-990164                                                                         
     FOREACH gglr400_curs1 INTO l_aag.* #MOD-990164      
        IF SQLCA.sqlcode != 0 THEN
       ...
       #DECLARE b_curs CURSOR FOR
        ...
        FOREACH b_curs INTO sr.tah01,sr.tah08,sr.tah09,sr.tah04
           ...
           #CALL s_curr3(sr.tah08,l_date,tm.rate_op) RETURNING sr.azj07    #按參數取得評價匯率  #MOD-C70285 mark     
            ...
            LET l_yy=yy USING '&&&&',mm USING '&&'
            SELECT azj07 INTO sr.azj07 FROM azj_file
            WHERE azj02=l_yy AND azj01=sr.tah08
            ...
            END IF
            #No.FUN-780031  --Begin
            #OUTPUT TO REPORT gglr400_rep(sr.*)
            IF tm.e = 'Y' THEN                                                        
     ...
     IF g_zz05 = 'Y' THEN
        CALL cl_wcchp(tm.wc,'tah01,tah08')
             RETURNING g_str
     END IF
     LET g_str = g_str,";",yymm,";",tm.t,";",g_azi04,";",g_azi05
     CALL cl_prt_cs3('gglr400','gglr400',g_sql,g_str)
     #No.FUN-780031  --End  
     #No.FUN-B80096--mark--Begin---    
