...
FUNCTION gglp102_tm(p_row,p_col)
   ...
   OPEN WINDOW gglp102_w AT p_row,p_col WITH FORM "ggl/42f/gglp102" 
   ...
   WHILE TRUE 
         ...
         IF NOT cl_null(tm.asj03) THEN
            ...
            ELSE
               IF tm.asj03 = g_year AND tm.asj04<=g_month THEN
                  CALL cl_err(tm.asj04,'atp-164',0)
                  NEXT FIELD asj04
               END IF
            END IF
         END IF
     ...
     #FUN-570145 --start--
      ...
      IF g_bgjob = 'Y' THEN
         SELECT zz08 INTO lc_cmd FROM zz_file WHERE zz01= 'gglp102'
         IF SQLCA.sqlcode OR lc_cmd IS NULL THEN
            CALL cl_err('gglp102','9031',1)   
         ELSE
            LET lc_cmd = lc_cmd CLIPPED,
                         ...
                         " '",tm.aac01 CLIPPED,"'",  
                         " '",g_bgjob CLIPPED,"'"
            CALL cl_cmdat('gglp102',g_time,lc_cmd CLIPPED)
         END IF
         CLOSE WINDOW gglp102_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
         EXIT PROGRAM
...
DEFINE l_cn       LIKE type_file.num5
   ######INSERT 前先删除
   ######指定[年度],[期别],[族群]只对应一笔asj08='1' ,asj081 = 'W'的调整分录
   ...
   SELECT COUNT(*) INTO l_cnt FROM asj_file
    WHERE asj00 = g_asz01 AND asj03 = tm.asj03 AND asj04 = tm.asj04
      AND asj05 = tm.asj05
      AND asj08  = '1' AND asj081 = 'W'
      AND asjconf = 'Y'    #FUN-B60134
   IF l_cnt > 0 THEN
      LET l_sql = "SELECT asj01 FROM asj_file WHERE asj00 = '",g_asz01,"'",
                  "   AND asj03 = '",tm.asj03,"' AND asj04 = '",tm.asj04,"'",
                  "   AND asjconf = 'Y' ",   #FUN-B60134
                  "   AND asj05 = '",tm.asj05,"' AND asj08 = '1' AND asj081 = 'W'"
      PREPARE sel_asj01 FROM l_sql
      DECLARE sel_asj01_cur CURSOR FOR sel_asj01
      LET l_cn = 1
      FOREACH sel_asj01_cur INTO l_asj01
      ...
      #   AND asj08 = '1' AND asj081 = 'W'
          ...
          DELETE FROM ask_file WHERE ask00 = g_asz01 AND ask01 = l_asj01
             AND askconf = 'Y'   #FUN-B60134
          LET l_cn = l_cn+1
      END FOREACH
   END IF
   
LET l_sql = "SELECT asa02 FROM asa_file WHERE asa01 = '",tm.asj05,"'"
...
FOREACH sel_asa02_cur INTO l_asa02
  ...
  #CALL s_auto_assign_no("GGL",tm.aac01,g_today,"","asj_file","asj01",g_plant,2,l_asj.asj00)  #NO.FUN-B40104  #carrier 20111024
   CALL s_auto_assign_no("AGL",tm.aac01,g_today,"","asj_file","asj01",g_plant,2,l_asj.asj00)  #NO.FUN-B40104  #carrier 20111024
...
#  LET l_asj.asj21 = '00'   #版本
   ...
   LET g_sql = "SELECT UNIQUE ask03,ask05 FROM asj_file,ask_file,aag_file ",
               ...
               "   AND asj04 = '",tm.asj04,"'",
               "   AND asj05 = '",tm.asj05,"'",
               "   AND asj06 = '",l_asa02,"'",    #luttb 101204
               "   AND asj08 = '1' AND asj081 IN ('U','V')",
               "   AND asjconf = 'Y'",   #FUN-B60134
   ...
   ELSE 
      ...
      IF cl_null(l_ask.ask02) THEN LET l_ask.ask02 = 0 END IF
      LET l_ask.ask02 = l_ask.ask02+1
      #LET l_ask.ask05 = ' '     #luttb 101213
      LET l_ask.ask05 = l_asa02  #luttb 101213
