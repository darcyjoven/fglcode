...
#    END INPUT
    ...
    END CONSTRUCT
      ...
      CONSTRUCT BY NAME tm.wc3 ON ted09
      ...
      ON ACTION CONTROLP
       CASE 
          WHEN INFIELD(ted09)
                  ...
                  CALL cl_create_qry() RETURNING g_qryparam.multiret
                  DISPLAY g_qryparam.multiret TO ted09
                  NEXT FIELD ted09            
       END CASE
       ON ACTION ACCEPT
         EXIT DIALOG        
...
#No.FUN-8B0106 add end          
     ...
     LET l_sql1 = "SELECT UNIQUE aag01,aag02,ted02,ted09 FROM aag_file,ted_file ", #TQC-D60028 ''->aag02
                  ...
                  "    AND ",tm.wc CLIPPED,
                  "    AND ",tm.wc3 CLIPPED,                #add by liyjf170330
...
FUNCTION gglq701_get_ahe02(p_aag01_str,p_aed02,p_gaq01) #TQC-930163
  ...
  DEFINE l_ahe03         LIKE ahe_file.ahe03  #No.TQC-C50211
     ...
     #  CALL cl_err('open gglq701_gaq01_cs',SQLCA.sqlcode,1)
        ...
        RETURN NULL
     END IF
     FETCH FIRST gglq701_gaq01_cs INTO l_ahe01
     IF SQLCA.sqlcode THEN
...
DEFINE l_i        LIKE type_file.num5
   ...
   #IF tm.c = 'N' THEN   #FUN-C80102  mark
   IF tm.e = 'N' THEN    #FUN-C80102  add
      LET g_sql = "SELECT aed01,aag02,aed04,'',SUM(df),SUM(cf),SUM(d),SUM(c) ",
                  "  FROM gglq701_tmp",
   ...
   ELSE 
      ...
      IF tm.f ='Y' THEN     #FUN-C80102  add
         LET g_sql = "SELECT aed01,aag02,aed04,ted09,SUM(df),SUM(cf),SUM(d),SUM(c) ",
                     "  FROM gglq701_tmp",
                     " GROUP BY aed01,aag02,aed04,ted09 ",
                     " ORDER BY aed01,aag02,aed04,ted09 "
      ELSE 
         LET g_sql = "SELECT aed01,aag02,aed04,'',0,0,SUM(d),SUM(c) ",
                     "  FROM gglq701_tmp",
                     " GROUP BY aed01,aag02,aed04 ",
                     " ORDER BY aed01,aag02,aed04 "
      END IF 
   ...
   FOREACH gglq701_sum_m INTO l_aed01,l_aag02,l_aed04,l_ted09,l_df,l_cf,l_d,l_c
      IF SQLCA.sqlcode THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
   ...
   FOR l_i = tm.m1 TO tm.m2      
       #IF tm.c = 'N' THEN     #FUN-C80102  mark
       IF tm.e = 'N' THEN      #FUN-C80102  add
          LET g_sql = "SELECT aed01,aag02,'','',SUM(df),SUM(cf),SUM(d),SUM(c) ",
                      "  FROM gglq701_tmp",
                      " WHERE aed04 <= '",l_i,"'",
                      "   AND type <> '3' ",
                      "   AND type <> '4' ",
       ...
       ELSE 
       	  #IF tm.d ='Y' THEN    #FUN-C80102  mark
          IF tm.f ='Y' THEN     #FUN-C80102  add
             LET g_sql = "SELECT aed01,aag02,'',ted09,SUM(df),SUM(cf),SUM(d),SUM(c) ",
                         "  FROM gglq701_tmp",
                         " WHERE aed04 <= '",l_i,"'",
                         "   AND type <> '3' ",
                         "   AND type <> '4' ",
          ...
          ELSE 
             LET g_sql = "SELECT aed01,aag02,'','',0,0,SUM(d),SUM(c) ",
                         "  FROM gglq701_tmp",
                         " WHERE aed04 <= '",l_i,"'",
                         "   AND type <> '3' ",
                         "   AND type <> '4' ",
       ...
       FOREACH gglq701_sum_y INTO l_aed01,l_aag02,l_aed04,l_ted09,l_df,l_cf,l_d,l_c
          IF SQLCA.sqlcode THEN
             CALL cl_err('foreach:',SQLCA.sqlcode,1)
             EXIT FOREACH
          END IF
