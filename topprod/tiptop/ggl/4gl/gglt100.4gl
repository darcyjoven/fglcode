...
# Modify.........: No.FUN-760044 07/06/15 By Sarah 版本(asj21)隱藏,程式寫死塞入00
# Modify.........: No.FUN-770086 07/07/24 By kim asj08 add item 3
# Modify.........: No.FUN-770069 07/08/03 By Sarah 調整期別(asj04)與版本(asj21)的查詢欄位輸入順序
...
#                                         版次不為00的單據在刪除時需檢查是否此單號有存在版次為00的單據的asj13欄位值中，如有，則不可進行刪除，只有版次00的單據可以做刪除動作
#                                         且版次00的單據在取消確認時，要把asj13單號相同and版次=期別的單據刪除 
# Modify.........: No.FUN-930090 09/05/21 By ve007 單頭asj06上層公司開放輸入，應依族群代碼asj05為key,需存在agli002單頭asa02設定，開窗資料/欄位輸入檢查邏輯相同 
...
# Modify.........: No:MOD-9C0458 09/12/31 By Sarah asj00值的應抓合併後帳別aaz641,而不是aaz64
...
# Modify.........: No.FUN-A40041 10/04/20 By wujie     g_sys->GGL
# Modify.........: NO.MOD-A40193 10/04/30 by yiting 1.asj21寫死為00
# Modify.........: NO.TQC-A50088 10/05/20 by sabrina 由於執行 aglp002 寫入 atcc07 為必要值,
#                                                    因此請將 ask05 增加判斷,若為 null 時給予 ' '  
# Modify.........: No.FUN-A50102 10/06/07 By lutingtingGP5.3集團架構優化:跨庫統一改為使用cl_get_target_table()實现
# Modify.........: No.FUN-AA0040 10/10/19 By Carrier gglq002 串查gglt100 & 修改串查条件
# Modify.........: NO.TQC-AB0046 10/11/12 BY Dido 進入單身時重新取 t_azi 
# Modify.........: No.FUN-A90030 11/01/24 By vealxu 增加asj081欄位  
# Modify.........: No.FUN-AA0005 11/01/30 By chenmoyan asj08為空白時,NEXT FIELD asj08
# Modify.........: No:FUN-B20004 11/02/09 By destiny 科目查询自动过滤
# Modify.........: No.FUN-B50062 11/05/24 By xianghui BUG修改，刪除時提取資料報400錯誤
# Modify.........: No.FUN-B70003 11/06/28 By lujh 當是大陸版時，只能查询
# Modify.........: No.FUN-B60143 11/07/06 By zhangweib  add itmes asj08_4  asj08_5         
...
# Modify.........: No.FUN-C70002 12/07/02 By lujh gglt100只能查詢,不應區分大陸還是台灣
...
DEFINE
    ...
    #No.FUN-AA0040   --Beatk                                                       
    #g_argv1         LIKE asj_file.asj01,         #传票编号    #No.FUN-560014  #N
    #g_argv2         LIKE aaa_file.aaa01,         #帐别   #No.FUN-670039         
    g_argv1          LIKE asj_file.asj00,         #帐套                          
    g_argv2          LIKE asj_file.asj03,         #年度                          
    ...
    g_asg06          LIKE asg_file.asg06,         #FUN-590062 新增記帳幣別 
...
DEFINE   g_dbs_asg03    LIKE type_file.chr21         #FUN-920070 add
DEFINE   g_plant_asg03  LIKE type_file.chr10         #FUN-980025 add
DEFINE   g_asg03        LIKE asg_file.asg03          #FUN-920070 add
DEFINE   g_asg04        LIKE asg_file.asg04          #FUN-960184 add
DEFINE   g_asa09        LIKE asa_file.asa09          #FUN-960184 add
DEFINE   g_newno        LIKE asj_file.asj01          #FUN-930144 add
...
DEFINE  lc_qbe_sn       LIKE    gbm_file.gbm01    #No.FUN-580031  HCN
    ...
    IF g_argv1<>' ' THEN
       #No.FUN-AA0040  --Beatk                                                  
       #LET g_wc=" asj00='",g_argv2,"'",                                        
       #         " AND asj01='",g_argv1,"'"                                     
       LET g_wc = " asj00 = '",g_argv1,"' AND asj03 = ",g_argv2,                
                  " AND asj04 = ",g_argv3," AND asj21 = '",g_argv4,"'"          
       #No.FUN-AA0040  --End       
       LET g_wc1=" 1=1"
    ELSE
       CALL cl_set_head_visible("","YES")    #No.FUN-6B0029
 
       INITIALIZE g_asj.* TO NULL    #No.FUN-750051
       CONSTRUCT BY NAME g_wc ON
             asj01,asj02,asj03,asj04,asj05,asj06,asj07,asj10,  #NO.FUN-950048
             asj08,asj081,asj09,asjconf,asj11,asj12,           #FUN-9A0030
             asjuser,asjgrup,asjmodu,asjdate
          BEFORE CONSTRUCT
             CALL cl_qbe_init()
          ...
          ON ACTION controlp
             CASE
                WHEN INFIELD(asj01) #單據性質
                   CALL q_aac(TRUE,TRUE,g_asj.asj01,'A','','','GGL') RETURNING g_asj.asj01     #TQC-670008
       ...
       CONSTRUCT g_wc1 ON ask02,ask03,ask04,ask05,ask06,ask07
          ...
          ON ACTION controlp
             CASE
                WHEN INFIELD(ask03)
                   CALL q_m_aag2(TRUE,TRUE,g_plant_asg03,g_ask[1].ask03,'23',g_aaz641)  #No.FUN-980025
    ...
    IF cl_null(g_wc1) OR g_wc1=" 1=1" THEN
       LET g_sql="SELECT asj01,asj00 FROM asj_file ", # 組合出 SQL 指令
           " WHERE ",g_wc CLIPPED, " ORDER BY asj01"
    ELSE
...
FUNCTION t001_q()
   ...
   INITIALIZE g_asj.* TO NULL              #No.FUN-6B0040
   MESSAGE ""
   CALL cl_opmsg('q')
...
FUNCTION t001_fetch(p_flasj)
   DEFINE p_flasj     LIKE type_file.chr1,          #No.FUN-680098  VARCHAR(1)
   ...
   IF SQLCA.sqlcode THEN
      CALL cl_err(g_asj.asj01,SQLCA.sqlcode,0)
      INITIALIZE g_asj.* TO NULL  #TQC-6B0105
      RETURN
   ELSE
      CASE p_flasj
   ...
   IF SQLCA.sqlcode THEN
      CALL cl_err3("sel","asj_file",g_asj.asj01,g_asj.asj00,SQLCA.sqlcode,"","",1)  #No.FUN-660123
   ELSE
      LET g_data_owner = g_asj.asjuser     #No.FUN-4C0048
      LET g_data_group = g_asj.asjgrup     #No.FUN-4C0048
      CALL t001_show()                      # 重新顯示
   END IF
END FUNCTION
...
FUNCTION t001_show()
   ...
   END IF
   #--FUN-AA0005 end-
   DISPLAY BY NAME g_asj.asjoriu,g_asj.asjorig,
      g_asj.asj01,g_asj.asj02,g_asj.asj03,g_asj.asj04,g_asj.asj05,  #NO.FUN-750076 #FUN-950048拿掉asj21
      g_asj.asj06,g_asj.asj07,g_asj.asj09,g_asj.asj08,g_asj.asj081, #No.FUN-A90030
...
#       WHERE asg01 = g_asj.asj06
#      
#       LET g_plant_new = g_asg03        #營運中心 #No.FUN-980025
#       LET g_plant_asg03 = g_asg03      #營運中心
#      CALL s_getdbs()
...
#--------------------------------No.TQC-B80009--------------------------start
   ...
   END IF   
#--------------------------------No.TQC-B80009----------------------------end
   LET g_t1=s_get_doc_no(g_asj.asj01)        #No.FUN-560014
...
DEFINE l_cmd       LIKE type_file.chr1000     #No.FUN-680098  VARCHAR(400)
   ...
   LET g_asj.asj09 = 'N'
   LET g_asj.asj08 = '1'
   LET g_asj.asj081 = '1'     #FUN-A90030
   LET g_asj.asjuser = g_user
   LET g_asj.asjoriu = g_user #FUN-980030
   LET g_asj.asjorig = g_grup #FUN-980030
   LET g_asj.asjgrup = g_grup               #使用者所屬群
   LET g_asj.asjdate = g_today
   LET g_asj.asj11=0
   LET g_asj.asj12=0
   LET g_asj.asjlegal= g_legal #FUN-980003 add g_legal
   CALL cl_opmsg('a')
   WHILE TRUE
      BEGIN WORK
      CALL t001_i('a')
      ...
      IF cl_null(g_asj.asj01) THEN                # KEY 不可空白
         CONTINUE WHILE
      END IF
     #CALL s_auto_assign_no("ggl",g_asj.asj01,g_asj.asj02,"","asj_file","asj01",g_plant,"2",g_bookno)           #No.FUN-560014 #FUN-980094   #carrier 20111024
      CALL s_auto_assign_no("AGL",g_asj.asj01,g_asj.asj02,"","asj_file","asj01",g_plant,"2",g_bookno)           #No.FUN-560014 #FUN-980094   #carrier 20111024
      ...
      IF SQLCA.sqlcode THEN
         CALL cl_err3("ins","asj_file",g_asj.asj01,g_asj.asj00,SQLCA.sqlcode,"","t001_ins_asj:",1)  #No.FUN-660123
...
FUNCTION t001_i(p_cmd)
    ...
    DEFINE l_asj02     STRING                  #MOD-930284 add
    DEFINE l_i         LIKE type_file.num5     #MOD-930284 add
 
    LET g_asj.asj21 = '00'     #版本,寫死塞入00   #FUN-760044 add #FUN-950048  #MOD-A40193 
    CALL cl_set_head_visible("","YES")          #No.FUN-6B0029 
 
    INPUT BY NAME g_asj.asjoriu,g_asj.asjorig,
             g_asj.asj01,g_asj.asj02,g_asj.asj03,g_asj.asj04,  #NO.FUN-750076 #FUN-950048 mod
             g_asj.asj05,g_asj.asj06,g_asj.asj07,g_asj.asj10,
             g_asj.asj08,g_asj.asj081,g_asj.asj09,g_asj.asj11,g_asj.asj12, #FUN-770086   #FUN-A90030
             g_asj.asjconf,g_asj.asjuser,g_asj.asjmodu,
             g_asj.asjgrup,g_asj.asjdate
...
#             CALL s_check_no(g_sys,g_asj.asj01,g_asj01_t,"*","asj_file","asj01","")
              CALL s_check_no("ggl",g_asj.asj01,g_asj01_t,"*","asj_file","asj01","")   #No.FUN-A40041
                   RETURNING li_result,g_asj.asj01
              DISPLAY BY NAME g_asj.asj01
              IF (NOT li_result) THEN
        ...
        AFTER FIELD asj05   #族群編號
           IF NOT cl_null(g_asj.asj05) THEN
              ...
              #FUN-B90057--add--end
           ELSE                                        #FUN-930090 add
              CALL cl_err(g_asj.asj05,'mfg0037',0)     #FUN-930090 add
              NEXT FIELD asj05                         #FUN-930090 add
           END IF
...
#                 WHERE asa01=g_asj.asj05
#                   AND asa02=g_asj.asj06
#                IF cl_null(g_asj.asj07) THEN 
#                   CALL cl_err(g_asj.asj07,'agl-223',0) NEXT FIELD asj06
#                ELSE 
#                   DISPLAY BY NAME g_asj.asj07
#                END IF
...
#                   LET g_dbs_asg03 = s_dbstring(g_dbs_new)       
#                   LET g_plant_new = g_plant  #FUN-A50102
#                ELSE    
#                   SELECT asa09 INTO g_asa09 FROM asa_file
#                    WHERE asa01 = g_asj.asj05 AND asa02 = g_asj.asj06
#                   IF g_asa09 = 'N' THEN 
#                     SELECT azp03 INTO g_dbs_new FROM azp_file 
#                      WHERE azp01 = g_plant
#                     LET g_plant_new = g_plant   #FUN-A50102
...
#                      WHERE azp01 = g_asg03
#                    #FUN-A10135--mod--end
#                     LET g_plant_new = g_asg03   #FUN-A50102
#                   END IF
...
#FUN-AA0005 --End mark
                 ...
                 #END IF
                 #--MOD-B70103 mark
                 CALL s_aaz641_asg(g_asj.asj05,g_asj.asj06) RETURNING g_dbs_asg03  #FUN-AA0005
                 CALL s_get_aaz641_asg(g_dbs_asg03) RETURNING g_aaz641                 #FUN-AA0005
                 #---MOD-B70103 start--
                 IF cl_null(g_aaz641) THEN
                    CALL cl_err(g_asg03,'agl-601',1)
                 END IF
                 #--MOD-B70103 end---
                 LET g_asj.asj00 = g_aaz641   #MOD-9C0458 add
                #FUN-B90057--mark--str--
                #SELECT asg06 INTO g_asg06
                #  FROM asg_file
                # WHERE asg01 = g_asj.asj06
        ...
        AFTER FIELD asj08
          IF cl_null(g_asj.asj08) THEN NEXT FIELD asj08 END IF  #FUN-AA0005
          IF NOT cl_null(g_asj.asj08) THEN
          #FUN-A90030--------------add start------------------
          #  IF p_cmd='a' THEN
          #     IF g_asj.asj08 = '2' THEN
          #        CALL cl_err('','agl-227',1) 
          #        NEXT FIELD asj08
          #     END IF
          #  END IF
          #FUN-A90030 ------------add end----------------------
            #IF g_asj.asj08 NOT MATCHES '[123]' THEN #FUN-770086    #FUN-B60143   Mark
             IF g_asj.asj08 NOT MATCHES '[123456]' THEN #FUN-770086 #FUN-B60143   Add
                NEXT FIELD asj08
             END IF
          END IF
          #FUN-A90030 -----------add start-------------------
        ...
        ON ACTION controlp
           CASE
              WHEN INFIELD(asj01) #單據性質
                 CALL q_aac(FALSE,TRUE,g_asj.asj01,'A','','','GGL') RETURNING g_asj.asj01  #TQC-670008
                 DISPLAY BY NAME g_asj.asj01
                 NEXT FIELD asj01
              WHEN INFIELD(asj05) #族群編號
                 CALL cl_init_qry_var()
...
FUNCTION t001_set_entry(p_cmd)
   ...
   END IF
   IF p_cmd='a' THEN
      CALL cl_set_comp_entry("asj08",TRUE)
   END IF
   IF g_asj.asj08 = '1' THEN
  ...
  #FUN-A90030 ----------add start---------
   IF g_asj.asj08 = '2' THEN
      CALL cl_set_comp_entry("asj081",TRUE)
   END IF 
  #FUN-A90030 ---------add end------------ 
END FUNCTION
...
FUNCTION t001_set_no_entry(p_cmd)
   ...
   END IF
   IF p_cmd='u' THEN
      CALL cl_set_comp_entry("asj08",FALSE)
   END IF
   IF g_asj.asj08 = '1' THEN
      LET l_asj02 = g_asj.asj02
      LET l_i = l_asj02.getLength()
  ...
  #FUN-A90030 -----------add start----------
   IF g_asj.asj08 <> '2' THEN
      CALL cl_set_comp_entry("asj081",FALSE)
   END IF
  #FUN-A90030 -----------add end-------------
END FUNCTION
...
DEFINE
    ...
    l_asa09         LIKE asa_file.asa09           #獨立會科合并   #FUN-950051 add
    ...
    DECLARE t001_bcl CURSOR FROM g_forupd_sql
      ...
      INPUT ARRAY g_ask WITHOUT DEFAULTS FROM s_ask.*
        ...
        BEFORE INSERT
            LET l_n = ARR_COUNT()
            LET p_cmd='a'
            INITIALIZE g_ask[l_ac].* TO NULL      #900423
        ...
        AFTER INSERT
           ...
           LET b_ask.asklegal=g_legal #FUN-980003 add g_legal
          #TQC-A50088---add---start---
           IF cl_null(b_ask.ask05) THEN
              LET b_ask.ask05 = ' '
           END IF
          #TQC-A50088---add---end---
           ...
           IF SQLCA.sqlcode THEN
              CALL cl_err3("ins","ask_file",b_ask.ask01,b_ask.ask02,SQLCA.sqlcode,"","ins ask",1)  #No.FUN-660123
              CANCEL INSERT
           ELSE
              CALL t001_bu()
        ...
        AFTER FIELD ask03   #科目編號
            IF NOT cl_null(g_ask[l_ac].ask03) THEN
               ...
               IF NOT cl_null(g_errno) THEN
                    ...
                    #FUN-B20004--beatk
                    CALL q_m_aag2(FALSE,FALSE,g_plant_asg03,g_ask[1].ask03,'23',g_aaz641)
                        RETURNING g_ask[l_ac].ask03 
                    #LET g_ask[l_ac].ask03=g_ask_t.ask03
                    #FUN-B20004--end
                     DISPLAY g_ask[l_ac].ask03 TO ask03               #No.MOD-490344
                    NEXT FIELD ask03
               END IF
            END IF
            ...
            IF NOT cl_null(g_ask[l_ac].ask04) THEN
               ...
               IF g_msg[1,1] = '.' THEN
                  ...
                  SELECT aad02 INTO g_ask[l_ac].ask04 FROM aad_file
                   ...
                   DISPLAY g_ask[l_ac].ask04 TO ask04      #No.MOD-490344
                  NEXT FIELD ask04
               END IF
            END IF
        ...
        AFTER FIELD ask07
          LET g_ask[l_ac].ask07 = cl_digcut(g_ask[l_ac].ask07,t_azi04)   #No.FUN-590062    #No.CHI-6A0004 g_azi-->t_azi 
          IF NOT cl_null(g_ask[l_ac].ask07) THEN
             IF g_ask[l_ac].ask07 <= 0 THEN
               CALL cl_err(g_ask[l_ac].ask07,'aap-022',0)
               NEXT FIELD ask07
             END IF
          END IF
        ...
        BEFORE DELETE                            #是否取消單身
            IF g_ask_t.ask02 IS NOT NULL THEN
                ...
                IF SQLCA.SQLERRD[3] = 0 THEN
                    CALL cl_err3("del","ask_file",g_asj.asj00,g_asj.asj01,SQLCA.sqlcode,"","",1)  #No.FUN-660123
                    ROLLBACK WORK
                    CANCEL DELETE
                END IF
                CALL t001_bu()
       ...
       ON ROW CHANGE
         ...
         ELSE
            UPDATE ask_file SET ask02 = b_ask.ask02,
                        ...
                        IF SQLCA.sqlcode THEN
                           CALL cl_err3("upd","ask_file",g_asj.asj00,g_asj.asj01,SQLCA.sqlcode,"","upd ask",1)  #No.FUN-660123
                           LET g_ask[l_ac].* = g_ask_t.*
                           LET g_success='N'
                        ELSE
                           CALL t001_bu()
        ...
        ON ACTION controlp
           CASE
              WHEN INFIELD(ask03)
                   CALL q_m_aag2(FALSE,TRUE,g_plant_asg03,g_ask[1].ask03,'23',g_aaz641)#No.FUN-980025
                        RETURNING g_ask[l_ac].ask03 
...
DEFINE
    ...
    END IF 
    #No.TQC-B70040  --End
   #LET g_sql = "SELECT aag02,aag03,aag07,aagacti FROM ",g_dbs_asg03,"aag_file", #FUN-A50102
    ...
    IF g_errno = ' ' OR p_cmd = 'd' THEN
       LET g_ask[l_ac].aag02 = l_aag02
        DISPLAY g_ask[l_ac].aag02 TO aag02            #No.MOD-490344
    END IF
END FUNCTION
...
FUNCTION t001_bu()
   ...
   LET g_asj.asj11 = cl_digcut(g_asj.asj11,t_azi04)   #No.FUN-590062   #No.CHI-6A0004 g_azi-->t_azi 
   LET g_asj.asj12 = cl_digcut(g_asj.asj12,t_azi04)   #No.FUN-590062   #No.CHI-6A0004 g_azi-->t_azi 
 
   UPDATE asj_file SET asj11=g_asj.asj11, asj12=g_asj.asj12
    WHERE asj01=g_asj.asj01 AND asj00=g_asj.asj00
   IF SQLCA.sqlcode THEN
      LET g_success = 'N'
      CALL cl_err3("upd","asj_file",g_asj.asj01,g_asj.asj00,STATUS,"","upd asj",1)  #No.FUN-660123
      RETURN
   END IF
...
#             CALL s_check_no(g_sys,l_newno,"","*","asj_file","asj01","")
              CALL s_check_no("ggl",l_newno,"","*","asj_file","asj01","")   #No.FUN-A40041
...
#                    WHERE asa01=l_newasj05
#                      AND asa02=l_newasj06
#                   IF cl_null(l_newasj07) THEN
#                      CALL cl_err(l_newasj07,'agl-223',0) NEXT FIELD asj06
#                   ELSE
#                      DISPLAY l_newasj07 TO asj07
#                   END IF
#FUN-AA0005 --End mark
...
#                ELSE
#                   SELECT asa09 INTO g_asa09 FROM asa_file
#                    WHERE asa01 = l_newasj05 AND asa02 = l_newasj06
#                   IF g_asa09 = 'N' THEN
#                     SELECT azp03 INTO g_dbs_new FROM azp_file
#                      WHERE azp01 = g_plant
#                     LET g_plant_new = g_plant   #FUN-A50102
...
#             END IF     #FUN-AA0005 mark
              BEGIN WORK
              #CALL s_auto_assign_no("ggl",l_newno,l_newdate,"","asj_file","asj01",g_dbs,"2",g_aaz641)   #FUN-A50102
             #CALL s_auto_assign_no("ggl",l_newno,l_newdate,"","asj_file","asj01",g_plant,"2",g_aaz641)    #FUN-A50102  #carrier 20111024
              CALL s_auto_assign_no("AGL",l_newno,l_newdate,"","asj_file","asj01",g_plant,"2",g_aaz641)    #FUN-A50102  #carrier 20111024
                        RETURNING li_result,l_newno
              IF (NOT li_result) THEN
                 NEXT FIELD asj01
                 END IF
   ...
   SELECT asj_file.* INTO g_asj.* FROM asj_file WHERE asj01 = l_newno AND asj00 = g_aaz641
   CALL t001_u()
   CALL t001_b()
...
FUNCTION t001_b_move_to()
   ...
   LET b_ask.ask07 = cl_digcut(b_ask.ask07,t_azi04)   #No.FUN-590062       #No.CHI-6A0004 g_azi-->t_azi 
    ...
    WHILE TRUE
        ...
        IF g_asj.asj01 != g_asj01_t THEN            # 更改單號
            UPDATE ask_file SET ask01 = g_asj.asj01
                WHERE ask00 = g_asj.asj00
                AND ask01 = g_asj01_t      #No.TQC-950063
            IF SQLCA.sqlcode THEN
                CALL cl_err3("upd","ask_file",g_asj.asj00,"",SQLCA.sqlcode,"","abb",1)  #No.FUN-660123
                CONTINUE WHILE
            END IF
            LET g_errno = TIME
            ...
            INSERT INTO azo_file (azo01,azo02,azo03,azo04,azo05,azo06,azoplant,azolegal) #FUN-980003 add azoplant,azolegal
               VALUES('gglt100',g_user,g_today,g_errno,g_asj01_t,g_msg,g_plant,g_legal)  #FUN-980003 add g_palnt,g_legal
        END IF
        UPDATE asj_file SET asj_file.* = g_asj.*
            WHERE asj00=g_asj.asj00 AND asj01 = g_asj01_t
        IF SQLCA.sqlcode THEN
            CALL cl_err3("upd","asj_file",g_asj_t.asj00,g_asj_t.asj01,SQLCA.sqlcode,"","",1)  #No.FUN-660123
            CONTINUE WHILE
        END IF
        EXIT WHILE
    END WHILE
...
FUNCTION t001_r()
    ...
    IF cl_delete() THEN
        ...
        LET g_doc.column1 = "asj00"         #No.FUN-9B0098 10/02/24
        LET g_doc.value1 = g_asj.asj00      #No.FUN-9B0098 10/02/24
        LET g_doc.column2 = "asj01"         #No.FUN-9B0098 10/02/24
        LET g_doc.value2 = g_asj.asj01      #No.FUN-9B0098 10/02/24
       ...
       IF k_n > 0 THEN
          ...
          IF SQLCA.sqlcode THEN
          CALL cl_err3("del","ask_file",g_asj.asj01,g_asj.asj00,SQLCA.sqlcode,"","(t001_r:delete ask)",1)  #No.FUN-660123
          LET g_success='N'  
          END IF
       END IF
       DELETE FROM asj_file WHERE asj01 = g_asj.asj01 AND asj00=g_asj.asj00
       IF SQLCA.sqlcode THEN
          CALL cl_err3("del","asj_file",g_asj.asj01,g_asj.asj00,SQLCA.sqlcode,"","(t001_r:delete asj)",1)  #No.FUN-660123
          LET g_success='N' 
       END IF
       INITIALIZE g_asj.* TO NULL
       IF g_success = 'Y' THEN
...
#  IF cl_confirm('atp-108') THEN #CHI-C30107 mark
      ...
      IF g_success = 'Y' THEN
         UPDATE asj_file SET asjconf='Y'
                              #asj13 = g_newno  #FUN-930144  #FUN-960003 mark
          WHERE asj00=g_asj.asj00 AND asj01=g_asj.asj01
             IF SQLCA.sqlcode OR SQLCA.SQLERRD[3]=0 THEN
                CALL cl_err3("upd","asj_file",g_asj.asj00,g_asj.asj01,SQLCA.sqlcode,"","upd asj_file",1)  #No.FUN-660123
                LET g_success='N'
             END IF
      END IF
...
FUNCTION t001_z()
   ...
   IF cl_confirm('aap-224') THEN
      ...
      IF SQLCA.sqlcode OR SQLCA.SQLERRD[3]=0 THEN
         LET g_success='N'
         CALL cl_err3("upd","asj_file",g_asj.asj00,g_asj.asj01,SQLCA.sqlcode,"","",1)  #No.FUN-660123
      END IF
      CLOSE t001_cl
      IF g_success='N' THEN
         ROLLBACK WORK RETURN
