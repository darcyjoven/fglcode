...
#               "sign_show.type_file.chr1,sign_str.type_file.chr1000"    #是否顯示簽核資料(Y/N)  #TQC-C10039  #MOD-C20064 Mark TQC-C10039
   ...
   LET l_table = cl_prt_temptable('gglr401',g_sql) CLIPPED
...
#               "        ?, ?, ?, ?, ?, ?, ?, ?,?,?,? )          " #TQC-C10039 add 4?  #MOD-C20064 Mark TQC-C10039
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL gglr401_tm()	        	# Input print condition
      ELSE CALL gglr401()			# Read data and create out-file
   END IF
...
FUNCTION gglr401_tm()
   ...
   OPEN WINDOW gglr401_w AT p_row,p_col
        WITH FORM "ggl/42f/gglr401"
   ...
   WHILE TRUE
    ...
    #No.FUN-B20010  --End
     ...
     #IF INT_FLAG THEN
     #   LET INT_FLAG = 0 CLOSE WINDOW gglr401_w EXIT PROGRAM
     #END IF
     #No.FUN-B20010  --Mark End
       ...
       ELSE
          ...
          LET l_cmd = l_cmd CLIPPED,
                    ...
                    " '",edate CLIPPED,"'",
                         ...
                         " '",g_rep_clas CLIPPED,"'",  
                         " '",g_template CLIPPED,"'"  
          CALL cl_cmdat('gglr401',g_time,l_cmd)	# Execute cmd at later time
       END IF
       CLOSE WINDOW gglr401_w
       CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
       EXIT PROGRAM
...
#     LOCATE l_img_blob IN MEMORY    #TQC-C10039  #MOD-C20064 Mark TQC-C10039
     ...
     PREPARE gglr401_prepare1 FROM l_sql1
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
        EXIT PROGRAM
     END IF
     DECLARE gglr401_curs1 CURSOR FOR gglr401_prepare1
     ...
     PREPARE gglr401_prepare2 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
        EXIT PROGRAM
     END IF
     DECLARE gglr401_curs2 CURSOR FOR gglr401_prepare2
     ...
     PREPARE gglr401_prepare3 FROM g_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
        EXIT PROGRAM
     END IF
     DECLARE qcye_curs1 CURSOR FOR gglr401_prepare3
     ...
     LET g_sql = "SELECT SUM(abb07),SUM(abb07f) FROM abb_file,aba_file",
              ...
              #  "   AND aba02 >= '",bdate,"' AND aba02 <= '",edate,"'",  #No.FUN-7C0064
                 "   AND aba02 >= ?           AND aba02 <= ?          ",  #No.FUN-7C0064
     ...
     PREPARE gglr401_prepare4 FROM g_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
        EXIT PROGRAM
     END IF
     DECLARE qjyd_curs1 CURSOR FOR gglr401_prepare4
     ...
     #CALL cl_outnam('gglr401') RETURNING l_name
 
     #IF tm.e = 'N' THEN
     #   START REPORT gglr401_rep1 TO l_name
     #ELSE
     #   START REPORT gglr401_rep2 TO l_name
     #END IF
     ...
     IF tm.e = 'N' THEN 
         LET l_name = 'gglr401'
     ELSE
         LET l_name = 'gglr401_1'
     END IF
     #No.FUN-7C0064  --End  
 
     FOREACH gglr401_curs1 INTO sr1.*
        IF SQLCA.sqlcode != 0 THEN
           CALL cl_err('foreach:',SQLCA.sqlcode,1)
           EXIT FOREACH
        END IF
        CALL gglr401_qcye(sr1.aag01)      #期初余額
             RETURNING l_bal,l_bal2
        IF tm.a = 'N' THEN
           CALL gglr401_qjyd(sr1.aag01)
                RETURNING l_flag
           IF tm.e = 'N' THEN
...
#FUN-6C0012.....end
        ...
        FOR l_i = mm1 TO nn1
            ...
            CALL r401_cpt(sr1.aag01,l_i) RETURNING l_md,l_mdf,l_mc,l_mcf
            CALL r401_cpt1(sr1.aag01,l_i) RETURNING l_yd,l_ydf,l_yc,l_ycf
            #No.FUN-7C0064  --End
            LET g_cnt = 0
            LET l_flag='N'
            FOREACH gglr401_curs2 USING sr1.aag01,l_i INTO sr.*
               IF SQLCA.sqlcode != 0 THEN
                  CALL cl_err('foreach:',SQLCA.sqlcode,1)
                  EXIT FOREACH
               END IF
               ...
               CALL r401_abb03(sr.aba01,sr.abb06) RETURNING l_abb03_s
               #IF tm.e = 'N' THEN
               #   OUTPUT TO REPORT gglr401_rep1(sr.*)
               #ELSE
               #   OUTPUT TO REPORT gglr401_rep2(sr.*)
               #END IF
               LET l_line = l_line + 1
...
#                       ,"",l_img_blob,"N",""    #TQC-C10039 add "",l_img_blob,"N",""  #MOD-C20064 Mark TQC-C10039
     ...
     #No.FUN-7C0064  --Begin
     #IF tm.e = 'N' THEN
     #   FINISH REPORT gglr401_rep1
     #ELSE
     #   FINISH REPORT gglr401_rep2
     #END IF
     ...
     IF g_zz05 = 'Y' THEN
        CALL cl_wcchp(tm.wc1,'aag01')
             RETURNING g_str
     END IF
     LET g_str = g_str,";",YEAR(bdate),";",g_azi04,";",l_type1
...
#     LET g_cr_apr_key_f = "aag01"             #報表主鍵欄位名稱  #TQC-C10039  #MOD-C20064 Mark TQC-C10039
     CALL cl_prt_cs3('gglr401',l_name,g_sql,g_str)
     #No.FUN-7C0064  --End  
     #No.FUN-B80096--mark--Begin---    
...
#FUNCTION gglr401_sjkm(l_aag01)   #追溯上級科目名稱且將其連成一體
...
#END FUNCTION
#FUN-6C0012.....end
FUNCTION gglr401_qcye(l_aag01)   #計算列印期間前的余額
   DEFINE l_aag01 LIKE aag_file.aag01
...
#yearly qjyd
FUNCTION r401_cpt1(p_aag01,p_i)
  DEFINE p_aag01   LIKE aag_file.aag01
  DEFINE p_i       LIKE type_file.num5
  DEFINE l_bdate   LIKE aba_file.aba02
  DEFINE l_edate   LIKE aba_file.aba02
...
#      END IF
#      IF sr.qcye > 0 THEN PRINT COLUMN 114,g_x[2],COLUMN 117,cl_numfor(gglr401_abs(sr.qcye),13,g_azi04) END IF
#      IF sr.qcye = 0 THEN PRINT COLUMN 114,g_x[3],COLUMN 117,cl_numfor(gglr401_abs(sr.qcye),13,g_azi04) END IF 
#      IF sr.qcye < 0 THEN PRINT COLUMN 114,g_x[4],COLUMN 117,cl_numfor(gglr401_abs(sr.qcye),13,g_azi04) END IF
#
#   ON EVERY ROW
...
#         IF LINENO =34 THEN
#            IF l_qcye > 0 THEN PRINT COLUMN 29,g_x[9],COLUMN 84,cl_numfor(l_abb07_dsum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_dsum,13,g_azi04),COLUMN 114,g_x[2],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            IF l_qcye = 0 THEN PRINT COLUMN 29,g_x[9],COLUMN 84,cl_numfor(l_abb07_dsum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_dsum,13,g_azi04),COLUMN 114,g_x[3],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            IF l_qcye < 0 THEN PRINT COLUMN 29,g_x[9],COLUMN 84,cl_numfor(l_abb07_dsum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_dsum,13,g_azi04),COLUMN 114,g_x[4],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            SKIP TO TOP OF PAGE
#            IF l_qcye > 0 THEN PRINT COLUMN 29,g_x[8],COLUMN 84,cl_numfor(l_abb07_dsum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_dsum,13,g_azi04),COLUMN 114,g_x[2],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF #過次頁
#            IF l_qcye = 0 THEN PRINT COLUMN 29,g_x[8],COLUMN 84,cl_numfor(l_abb07_dsum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_dsum,13,g_azi04),COLUMN 114,g_x[3],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF #過次頁
#            IF l_qcye < 0 THEN PRINT COLUMN 29,g_x[8],COLUMN 84,cl_numfor(l_abb07_dsum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_dsum,13,g_azi04),COLUMN 114,g_x[4],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF #過次頁
#         END IF
#         LET g_sql = "SELECT DISTINCT abb03 FROM abb_file ",
...
#            IF l_qcye > 0 THEN PRINT COLUMN 84,cl_numfor(sr.abb07,13,g_azi04),COLUMN 114,g_x[2],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            IF l_qcye = 0 THEN PRINT COLUMN 84,cl_numfor(sr.abb07,13,g_azi04),COLUMN 114,g_x[3],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            IF l_qcye < 0 THEN PRINT COLUMN 84,cl_numfor(sr.abb07,13,g_azi04),COLUMN 114,g_x[4],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#         ELSE
#            LET l_qcye = l_qcye-sr.abb07
#            IF l_qcye > 0 THEN PRINT COLUMN 99,cl_numfor(sr.abb07,13,g_azi04),COLUMN 114,g_x[2],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            IF l_qcye = 0 THEN PRINT COLUMN 99,cl_numfor(sr.abb07,13,g_azi04),COLUMN 114,g_x[3],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            IF l_qcye < 0 THEN PRINT COLUMN 99,cl_numfor(sr.abb07,13,g_azi04),COLUMN 114,g_x[4],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#         END IF
...
#                     PRINT
#                     PRINT
#                     IF l_qcye > 0 THEN PRINT COLUMN 29,g_x[9],COLUMN 84,cl_numfor(l_abb07_sum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_sum,13,g_azi04),COLUMN 114,g_x[2],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#                     IF l_qcye = 0 THEN PRINT COLUMN 29,g_x[9],COLUMN 84,cl_numfor(l_abb07_sum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_sum,13,g_azi04),COLUMN 114,g_x[3],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#                     IF l_qcye < 0 THEN PRINT COLUMN 29,g_x[9],COLUMN 84,cl_numfor(l_abb07_sum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_sum,13,g_azi04),COLUMN 114,g_x[4],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#                     SKIP TO TOP OF PAGE
#                     IF l_qcye > 0 THEN PRINT COLUMN 29,g_x[8],COLUMN 84,cl_numfor(l_abb07_sum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_sum,13,g_azi04),COLUMN 114,g_x[2],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF #過次頁
#                     IF l_qcye = 0 THEN PRINT COLUMN 29,g_x[8],COLUMN 84,cl_numfor(l_abb07_sum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_sum,13,g_azi04),COLUMN 114,g_x[3],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF #過次頁
#                     IF l_qcye < 0 THEN PRINT COLUMN 29,g_x[8],COLUMN 84,cl_numfor(l_abb07_sum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_sum,13,g_azi04),COLUMN 114,g_x[4],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF #過次頁
#                  END IF
#                  IF LINENO = 33 THEN
#                     PRINT
#                     IF l_qcye > 0 THEN PRINT COLUMN 29,g_x[9],COLUMN 84,cl_numfor(l_abb07_sum,13,g_azi04),COLUMN 99,cl_numfor(t_abb07_sum,13,g_azi04),COLUMN 114,g_x[2],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
...
#               IF l_qcye > 0 THEN 
#                  PRINT COLUMN 114,g_x[2],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) 
#                  PRINT COLUMN 15,sr.mm USING '<<',COLUMN 29,g_x[12],COLUMN 84,cl_numfor(l_sum1,13,g_azi04),COLUMN 99,cl_numfor(t_sum1,13,g_azi04),COLUMN 114,g_x[2],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04)
#               END IF
#               IF l_qcye = 0 THEN 
#                  PRINT COLUMN 114,g_x[3],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) 
#                  PRINT COLUMN 15,sr.mm USING '<<',COLUMN 29,g_x[12],COLUMN 84,cl_numfor(l_sum1,13,g_azi04),COLUMN 99,cl_numfor(t_sum1,13,g_azi04),COLUMN 114,g_x[3],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04)
#               END IF
#               IF l_qcye < 0 THEN 
#                  PRINT COLUMN 114,g_x[4],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) 
#                  PRINT COLUMN 15,sr.mm USING '<<',COLUMN 29,g_x[12],COLUMN 84,cl_numfor(l_sum1,13,g_azi04),COLUMN 99,cl_numfor(t_sum1,13,g_azi04),COLUMN 114,g_x[4],COLUMN 117,cl_numfor(gglr401_abs(l_qcye),13,g_azi04)
#               END IF
#               LET l_sum =0
#               LET t_sum =0
#               LET l_i = 0
...
#      END IF
#      IF sr.qcye > 0 THEN PRINT COLUMN 142,g_x[2],COLUMN 145,cl_numfor(gglr401_abs(sr.qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(sr.qcye),13,g_azi04) END IF
#      IF sr.qcye = 0 THEN PRINT COLUMN 142,g_x[3],COLUMN 145,cl_numfor(gglr401_abs(sr.qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(sr.qcye),13,g_azi04) END IF 
#      IF sr.qcye < 0 THEN PRINT COLUMN 142,g_x[4],COLUMN 145,cl_numfor(gglr401_abs(sr.qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(sr.qcye),13,g_azi04) END IF
#
#   ON EVERY ROW
#      IF NOT cl_null(sr.aba01) THEN
...
#         IF LINENO =34 THEN
#            IF l_qcye > 0 THEN PRINT COLUMn 27,g_x[9],COLUMN 82,cl_numfor(l_abb07f_dsum,13,g_azi04),COLUMN 97,cl_numfor(l_abb07_dsum,13,g_azi04),COLUMN 112,cl_numfor(t_abb07f_dsum,13,g_azi04),COLUMN 127,cl_numfor(t_abb07_dsum,13,g_azi04),COLUMN 142,g_x[2],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            IF l_qcye = 0 THEN PRINT COLUMn 27,g_x[9],COLUMN 82,cl_numfor(l_abb07f_dsum,13,g_azi04),COLUMN 97,cl_numfor(l_abb07_dsum,13,g_azi04),COLUMN 112,cl_numfor(t_abb07f_dsum,13,g_azi04),COLUMN 127,cl_numfor(t_abb07_dsum,13,g_azi04),COLUMN 142,g_x[3],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            IF l_qcye < 0 THEN PRINT COLUMn 27,g_x[9],COLUMN 82,cl_numfor(l_abb07f_dsum,13,g_azi04),COLUMN 97,cl_numfor(l_abb07_dsum,13,g_azi04),COLUMN 112,cl_numfor(t_abb07f_dsum,13,g_azi04),COLUMN 127,cl_numfor(t_abb07_dsum,13,g_azi04),COLUMN 142,g_x[4],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            SKIP TO TOP OF PAGE
...
#            IF l_qcye > 0 THEN PRINT COLUMN 73,cl_numfor(sr.abb25,6,g_azi04),COLUMN 82,cl_numfor(sr.abb07f,13,g_azi04),COLUMN 97,cl_numfor(sr.abb07,13,g_azi04),COLUMN 142,g_x[2],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            IF l_qcye = 0 THEN PRINT COLUMN 73,cl_numfor(sr.abb25,6,g_azi04),COLUMN 82,cl_numfor(sr.abb07f,13,g_azi04),COLUMN 97,cl_numfor(sr.abb07,13,g_azi04),COLUMN 142,g_x[3],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            IF l_qcye < 0 THEN PRINT COLUMN 73,cl_numfor(sr.abb25,6,g_azi04),COLUMN 82,cl_numfor(sr.abb07f,13,g_azi04),COLUMN 97,cl_numfor(sr.abb07,13,g_azi04),COLUMN 142,g_x[4],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#         ELSE
#            LET l_qcye = l_qcye-sr.abb07
#            LET l_qcyef = l_qcyef-sr.abb07f
#            IF l_qcye > 0 THEN PRINT COLUMN 73,cl_numfor(sr.abb25,6,g_azi04),COLUMN 112,cl_numfor(sr.abb07f,13,g_azi04),COLUMN 127,cl_numfor(sr.abb07,13,g_azi04),COLUMN 142,g_x[2],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            IF l_qcye = 0 THEN PRINT COLUMN 73,cl_numfor(sr.abb25,6,g_azi04),COLUMN 112,cl_numfor(sr.abb07f,13,g_azi04),COLUMN 127,cl_numfor(sr.abb07,13,g_azi04),COLUMN 142,g_x[3],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#            IF l_qcye < 0 THEN PRINT COLUMN 73,cl_numfor(sr.abb25,6,g_azi04),COLUMN 112,cl_numfor(sr.abb07f,13,g_azi04),COLUMN 127,cl_numfor(sr.abb07,13,g_azi04),COLUMN 142,g_x[4],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#         END IF
#         IF l_aba02 = sr.aba02  THEN
#            LET l_abb07_day = 0   #No.TQC-710095
#            LET t_abb07_day = 0   #No.TQC-710095
...
#                     IF l_qcye < 0 THEN PRINT COLUMn 27,g_x[9],COLUMN 82,cl_numfor(l_abb07f_sum,13,g_azi04),COLUMN 97,cl_numfor(l_abb07_sum,13,g_azi04),COLUMN 112,cl_numfor(t_abb07f_sum,13,g_azi04),COLUMN 127,cl_numfor(t_abb07_sum,13,g_azi04),COLUMN 142,g_x[4],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
#                     SKIP TO TOP OF PAGE
#                     IF l_qcye > 0 THEN PRINT COLUMn 27,g_x[8],COLUMN 82,cl_numfor(l_abb07f_sum,13,g_azi04),COLUMN 97,cl_numfor(l_abb07_sum,13,g_azi04),COLUMN 112,cl_numfor(t_abb07f_sum,13,g_azi04),COLUMN 127,cl_numfor(t_abb07_sum,13,g_azi04),COLUMN 142,g_x[2],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF #過次頁
#                     IF l_qcye = 0 THEN PRINT COLUMn 27,g_x[8],COLUMN 82,cl_numfor(l_abb07f_sum,13,g_azi04),COLUMN 97,cl_numfor(l_abb07_sum,13,g_azi04),COLUMN 112,cl_numfor(t_abb07f_sum,13,g_azi04),COLUMN 127,cl_numfor(t_abb07_sum,13,g_azi04),COLUMN 142,g_x[3],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF #過次頁
#                     IF l_qcye < 0 THEN PRINT COLUMn 27,g_x[8],COLUMN 82,cl_numfor(l_abb07f_sum,13,g_azi04),COLUMN 97,cl_numfor(l_abb07_sum,13,g_azi04),COLUMN 112,cl_numfor(t_abb07f_sum,13,g_azi04),COLUMN 127,cl_numfor(t_abb07_sum,13,g_azi04),COLUMN 142,g_x[4],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF #過次頁
#                  END IF
#                  IF LINENO = 33 THEN
#                     PRINT
#                     IF l_qcye > 0 THEN PRINT COLUMn 27,g_x[9],COLUMN 82,cl_numfor(l_abb07f_sum,13,g_azi04),COLUMN 97,cl_numfor(l_abb07_sum,13,g_azi04),COLUMN 112,cl_numfor(t_abb07f_sum,13,g_azi04),COLUMN 127,cl_numfor(t_abb07_sum,13,g_azi04),COLUMN 142,g_x[2],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) END IF
...
#               IF l_qcye > 0 THEN 
#                  PRINT COLUMN 142,g_x[2],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) 
#                  PRINT COLUMN 14,sr.mm USING '<<',COLUMn 27,g_x[12],COLUMN 82,cl_numfor(l_sumf1,13,g_azi04),COLUMN 97,cl_numfor(l_sum1,13,g_azi04),COLUMN 112,cl_numfor(t_sumf1,13,g_azi04),COLUMN 127,cl_numfor(t_sum1,13,g_azi04),COLUMN 142,g_x[2],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04)
#               END IF
#               IF l_qcye = 0 THEN 
#                  PRINT COLUMN 142,g_x[3],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) 
#                  PRINT COLUMN 14,sr.mm USING '<<',COLUMn 27,g_x[12],COLUMN 82,cl_numfor(l_sumf1,13,g_azi04),COLUMN 97,cl_numfor(l_sum1,13,g_azi04),COLUMN 112,cl_numfor(t_sumf1,13,g_azi04),COLUMN 127,cl_numfor(t_sum1,13,g_azi04),COLUMN 142,g_x[3],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04)
#               END IF
#               IF l_qcye < 0 THEN 
#                  PRINT COLUMN 142,g_x[4],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04) 
#                  PRINT COLUMN 14,sr.mm USING '<<',COLUMn 27,g_x[12],COLUMN 82,cl_numfor(l_sumf1,13,g_azi04),COLUMN 97,cl_numfor(l_sum1,13,g_azi04),COLUMN 112,cl_numfor(t_sumf1,13,g_azi04),COLUMN 127,cl_numfor(t_sum1,13,g_azi04),COLUMN 142,g_x[4],COLUMN 145,cl_numfor(gglr401_abs(l_qcyef),13,g_azi04),COLUMN 160,cl_numfor(gglr401_abs(l_qcye),13,g_azi04)
#               END IF
#               LET l_sum =0
#               LET t_sum =0
#               LET l_sumf =0
...
FUNCTION gglr401_abs(p_sqlerrd2)                                                                                                      
...
#No.FUN-7C0064  --Begin
FUNCTION r401_abb03(p_aba01,p_abb06)
