...
# Modify.........: No:FUN-C80102 12/09/18 By yangtt CR報表改善，將aglr105改成q類查詢作業gglq813 
...
DEFINE   g_pr_ar    DYNAMIC ARRAY OF RECORD         
                    ...
                    bal4_1 LIKE aah_file.aah04,  #期末借 
                    bal4_2 LIKE aah_file.aah04,  #期末貸 
                    line   LIKE type_file.num5,  #1:表示此筆為空行 2:表示此筆不為空行
...
#
#  LET l_table = cl_prt_temptable('gglq813',g_sql) CLIPPED   # 產生Temp Table
...
#FUN-C80102-----add----str----
   OPEN WINDOW q813_w AT 5,10 WITH FORM "ggl/42f/gglq813" 
       ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
    
    CALL cl_ui_init()
   DROP TABLE gglq813_tmp;
   CREATE TEMP TABLE gglq813_tmp(
   maj20  LIKE maj_file.maj20,
                    maj20e LIKE maj_file.maj20e,
                    maj02  LIKE maj_file.maj02,  #項次(排序要用的)
                    maj03  LIKE maj_file.maj03,  #列印碼
...
#FUN-C80102-----add----end----
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL q813_tm()                   # Input print condition
      ELSE CALL q813()                      # Read data and create out-file
   END IF
   CALL q813_menu()           #FUN-C80102 
   CLOSE WINDOW q813_w        #FUN-C80102 
...
#  OPEN WINDOW q813_w AT p_row,p_col WITH FORM "ggl/42f/gglq813" 
#      ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
    
#   CALL cl_ui_init()

   DELETE FROM gglq813_tmp;    #No.FUN-C80102   Add
...
#     CALL cl_err('sel azi:',SQLCA.sqlcode,0)  #No.FUN-660123
 ...
 # LET tm.b = g_bookno  #No.FUN-740020 
   ...
   LET g_bgjob = 'N'
   LET g_copies = '1'
 # LET tm.m = 'N'                   #FUN-8B0059   #FUN-C80102 mark
   LET tm.m = 'Y'                   #FUN-C80102
   LET tm.yy = YEAR(g_today)    #FUN-C80102
   LET tm.bm = MONTH(g_today)   #FUN-C80102
   LET tm.em = MONTH(g_today)   #FUN-C80102

   #FUN-C80102--add--str--
   SELECT COUNT(*) INTO l_n FROM mai_file WHERE mai00 = tm.b AND mai03 ='1' AND maiacti='Y' ORDER BY mai01
   IF l_n = '1' THEN
      SELECT mai01 INTO l_mai01 FROM mai_file WHERE mai00 = tm.b AND mai03 ='1' AND maiacti='Y' ORDER BY mai01
      LET tm.a = l_mai01
   ELSE 
      SELECT mai01 INTO tm.a FROM mai_file WHERE mai00 = tm.b AND mai03 ='1' AND maiacti='Y' ORDER BY mai01
   END IF
...
#      INPUT BY NAME tm.a,tm.b,tm.yy,tm.bm,tm.em,tm.dd,
      #INPUT BY NAME tm.b,tm.a,tm.yy,tm.bm,tm.em,tm.dd,            #No.FUN-740020 #FUN-C80102 mark
       INPUT BY NAME tm.b,tm.a,tm.d,tm.yy,tm.bm,tm.em,             #No.FUN-C80102 add
#                    tm.e,tm.f,tm.d,tm.c,tm.h,tm.m,tm.o,tm.r,      #No.FUN-8B0059 Add tm.m  #FUN-C80102 mark
                     tm.c,tm.h,tm.o,tm.r,                          #No.FUN-C80102 add
#                    tm.p,tm.q,tm.more WITHOUT DEFAULTS            #FUN-C80102 mark
                     tm.p,tm.q WITHOUT DEFAULTS                    #FUN-C80102 add 
         #No.FUN-580031 --start--
         BEFORE INPUT
             CALL cl_qbe_init()
         #No.FUN-580031 ---end---
...
#              CALL FGL_DIALOG_SETBUFFER( tm.a )
         ...
         #     LET g_qryparam.where = " mai00 = '",tm.b,"'"  #No.FUN-740020  #FUN-C80102 mark
               LET g_qryparam.where = " mai00 = '",tm.b,"' AND mai03 ='1' "  #FUN-C80102 add
...
#               CALL FGL_DIALOG_SETBUFFER( tm.p )
      ...
      ON ACTION help          #MOD-4C0121
       ...
       END INPUT
       IF INT_FLAG THEN
         #LET INT_FLAG = 0 CLOSE WINDOW q813_w    #FUN-C80102 mark
...
#           CALL cl_cmdat('gglq813',g_time,l_cmd)    # Execute cmd at later time
#        END IF
#        CLOSE WINDOW q813_w
#        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
#        EXIT PROGRAM
...
#-----END TQC-620081-----
   ...
   DEFINE l_sw      LIKE type_file.num5          #FUN-C80102  
   DEFINE l_i       LIKE type_file.num5          #FUN-C80102
...
#  CALL cl_del_data(l_table)    #FUN-C80102 mark
   #------------------------------ CR (2) ------------------------------#
   #end FUN-770044 add
   ...
   DELETE FROM aglq813_tmp;    #FUN-C80102
 
   LET l_sql = "SELECT * FROM maj_file",
               " WHERE maj01 = '",tm.a,"' ORDER BY maj02"
   PREPARE q813_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM 
   END IF
   DECLARE q813_c CURSOR FOR q813_p
   ...
   END FOR
  #CALL cl_outnam('gglq813') RETURNING l_name   #FUN-770044 mark
  #START REPORT q813_rep TO l_name              #FUN-770044 mark
   #-----MOD-850040---------
   LET l_maj02=''
   SELECT MAX(maj02) INTO l_maj02 FROM maj_file 
     WHERE maj01 = tm.a
   #-----END MOD-850040-----
   FOREACH q813_c INTO maj.*
      IF SQLCA.sqlcode THEN 
...
#-----END MOD-850040-----
      ...
      IF maj.maj04 = 0 THEN 
        ...
        #   '2',maj.maj21,maj.maj22                                            #FUN-8B0059 Add maj.maj21,maj.maj22           
         INSERT INTO gglq813_tmp VALUES(
            maj.maj20,maj.maj20e,maj.maj02,maj.maj03,
            sr.bal1_1,sr.bal1_2 ,sr.bal2_1,sr.bal2_2,
            sr.bal3_1,sr.bal3_2 ,sr.bal4_1,sr.bal4_2,
            '2',maj.maj21,maj.maj22)
        #FUN-C80102----mod---end----
      ELSE   
      	#FUN-C80102----mod---str----
        #EXECUTE insert_prep USING 
         ...
         FOR i = 1 TO maj.maj04
           ...
           #   '1',maj.maj21,maj.maj22                                         #FUN-8B0059 Add maj.maj21,maj.maj22
            INSERT INTO gglq813_tmp VALUES(
               ...
               '0','0','0','0',
               '0','0','0','0',
               '1',maj.maj21,maj.maj22)
           #FUN-C80102----mod---end----
         END FOR
      END IF
      #end FUN-770044 add
 
     #OUTPUT TO REPORT q813_rep(maj.*, sr.*)   #FUN-770044 mark
   END FOREACH
...
#              tm.em,";",tm.dd,";",tm.e,";",tm.m,";",tm.p                   #FUN-8B0059 Add tm.m,tm.p
#  CALL cl_prt_cs3('gglq813','gglq813',g_sql,g_str)
#  #------------------------------ CR (4) ------------------------------#
#  #end FUN-770044 add
#FUN-C80102-----mark-----end---
#FUN-C80102-----add------str---
   DECLARE gglq813_cur CURSOR FOR SELECT * FROM gglq813_tmp
                                     ORDER BY maj02,line
     CALL g_maj.clear()
     LET l_i = 1
     FOREACH gglq813_cur INTO g_pr.*
         IF (cl_null(g_pr.maj21) OR g_pr.maj21 = ' ') AND 
            (cl_null(g_pr.maj22) OR g_pr.maj22 = ' ') THEN 
            LET g_maj[l_i].maj20 = g_pr.maj20
         ELSE 
            IF tm.m = 'Y' THEN
               LET g_maj[l_i].maj20 = g_pr.maj21,'-',g_pr.maj22,' ',g_pr.maj20
         #  ELSE               #FUN-C80102 mark
         #     LET g_maj[l_i].maj20 = g_pr.maj20   #FUN-C80102 mark
            END IF
         END IF
         LET g_maj[l_i].bal1_1 = g_pr.bal1_1
         LET g_maj[l_i].bal1_2 = g_pr.bal1_2
         LET g_maj[l_i].bal2_1 = g_pr.bal2_1
         LET g_maj[l_i].bal2_2 = g_pr.bal2_2
         LET g_maj[l_i].bal3_1 = g_pr.bal3_1
         LET g_maj[l_i].bal3_2 = g_pr.bal3_2
         LET g_maj[l_i].bal4_1 = g_pr.bal4_1
         LET g_maj[l_i].bal4_2 = g_pr.bal4_2
         LET g_pr_ar[l_i].* = g_pr.*
         LET l_i = l_i + 1
     END FOREACH
     CALL g_maj.deleteElement(l_i)
     LET g_rec_b = l_i - 1
#FUN-C80102-----add------end---
 
  #FINISH REPORT q813_rep
...
FUNCTION q813_out()
   ...
   DEFINE l_msg   LIKE type_file.chr1000     #FUN-C80102 add
{   
   IF g_maj.getLength() = 0  THEN RETURN END IF
   ...
   IF tm.dd IS NULL THEN LET tm.dd=' ' END IF
   LET g_str = g_mai02,";",tm.a,";",tm.d,";",tm.yy,";",tm.bm,";",
               tm.em,";",tm.dd,";",tm.e,";",tm.m,";",tm.p                
   CALL cl_prt_cs3('aglr105','aglr105',g_sql,g_str)
}
   IF g_aaz.aaz77 = 'N' THEN LET g_mai02 = '' END IF
   IF tm.dd IS NULL THEN LET tm.dd=' ' END IF
   IF cl_null(tm.e) THEN LET tm.e = 0 END IF 
   LET l_msg = "aglr105 '",tm.b,"' '' '' '",g_lang,"' 'Y' '' '' '",tm.a,"' '",tm.b,"' '",tm.yy,"' '",tm.bm
               ,"' '",tm.em,"' '",tm.dd,"' '",tm.c,"' '",tm.d,"' '",tm.e,"' '",tm.f,"' '",tm.h,
               "' '",tm.o,"' '",tm.p,"' '",tm.q,"' '",tm.r,"' '' '' '' ''"
