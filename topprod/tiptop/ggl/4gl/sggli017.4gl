...
FUNCTION i017(p_argv1)
   ...
   OPEN WINDOW i017_w AT p_row,p_col WITH FORM "ggl/42f/ggli607"  
      ATTRIBUTE (STYLE = g_win_style CLIPPED)
   
   CALL cl_ui_init()
   CALL cl_set_comp_visible("aai04",FALSE)
   CALL cl_set_comp_entry("aag02,ats02,atn02",FALSE)
...
DEFINE
    ...
    LET g_forupd_sql = "SELECT aai01,'',aai02,'',aai03,'',aai04",
    ...
    BEFORE ROW
       ...
       IF g_rec_b>=l_ac THEN
          ...
          ELSE 
             ...
             SELECT ats02 INTO g_aai[l_ac].ats02 FROM ats_file WHERE ats01=g_aai[l_ac].aai02
             SELECT atn02 INTO g_aai[l_ac].atn02 FROM atn_file WHERE atn01=g_aai[l_ac].aai03
          END IF
          CALL cl_show_fld_cont()     
       END IF
    ...
    BEFORE INSERT
     ...
     AFTER INSERT
        ...
        IF (NOT cl_null(g_aai[l_ac].aai01)) AND (NOT cl_null(g_aai[l_ac].aai02))
           ...
           END IF
        END IF 
        INSERT INTO aai_file(aai00,aai01,aai02,aai03,aai04)
        VALUES(g_aai00,g_aai[l_ac].aai01,g_aai[l_ac].aai02,g_aai[l_ac].aai03,g_aai[l_ac].aai04)
        IF SQLCA.sqlcode THEN 
    ...
    AFTER FIELD aai02  
        IF NOT cl_null(g_aai[l_ac].aai02) THEN
           ...
           ELSE
              LET l_aai04='N'
           END IF
           SELECT COUNT(*) INTO l_cnt FROM ats_file WHERE ats01=g_aai[l_ac].aai02
                                                      AND ats07=l_aai04   
           IF l_cnt=0 THEN
              CALL cl_err('',100,0)
    ...
    AFTER FIELD aai03  
        IF NOT cl_null(g_aai[l_ac].aai03) THEN
           SELECT COUNT(*) INTO l_cnt FROM atn_file WHERE atn01=g_aai[l_ac].aai03
           IF l_cnt=0 THEN
              CALL cl_err('',100,0)
     ...
     ON ACTION CONTROLP
          CASE
             ...
             WHEN INFIELD(aai03)
                ...
                CALL cl_create_qry() RETURNING g_aai[l_ac].aai03
...
DEFINE
    ...
    LET g_sql =
     "SELECT aai01,'',aai02,'',aai03,'',aai04",
     " FROM aai_file",
     " WHERE ", p_wc CLIPPED, 
     " AND aai00 = '",g_aai00,"'",
     " AND aai04 = '",l_aai04,"'",
    ...
    FOREACH aai_curs INTO g_aai[g_cnt].*   #單身 ARRAY 填充
        ...
        SELECT ats02 INTO g_aai[g_cnt].ats02 FROM ats_file WHERE ats01=g_aai[g_cnt].aai02
        SELECT atn02 INTO g_aai[g_cnt].atn02 FROM atn_file WHERE atn01=g_aai[g_cnt].aai03
        LET g_cnt = g_cnt + 1
...
DEFINE l_cmd  LIKE type_file.chr1000
   ...
   IF g_argv1 = '1' THEN
      LET l_cmd = 'p_query "ggli607" "',g_wc CLIPPED,'"'
   ELSE
      LET l_cmd = 'p_query "ggli6071" "',g_wc CLIPPED,'"'
   END IF
