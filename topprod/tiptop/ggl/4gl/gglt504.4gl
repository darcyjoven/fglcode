...
# Modify.........: No.FUN-B80180 11/09/02 By xuxz 程序中涉及到atm_file的sql
#                                                  加上條件:AND atm04 = tm.y1 AND atm05 = tm.m2
#                                                  WHERE atj03 NOT IN ('4','5')---WHERE atj03 NOT IN ('4','5'，‘6’)
# Modify.........: No.TQC-B90078 11/09/014 By lujh 現金流量表改為多期報表
...
DEFINE tm  RECORD
              ...
              #m1      LIKE type_file.num5,   #Beatk 期別
              #y2      LIKE type_file.num5,   #輸入截止年度
              #m2      LIKE type_file.num5,   # 期別
              title1  LIKE type_file.chr30,   #輸入期別名稱     #TQC-B90078    add
              yy1     LIKE type_file.num5,    #輸入年度         #TQC-B90078   add
              bm1     LIKE type_file.num5,    ##Beatk 期別     #TQC-B90078    add
              em1     LIKE type_file.num5,    ## End  期別     #TQC-B90078    add
              title2  LIKE type_file.chr30,   ##輸入期別名稱     #TQC-B90078    add
              yy2     LIKE type_file.num5,    ##輸入年度        #TQC-B90078    add
              bm2     LIKE type_file.num5,    ##Beatk 期別     #TQC-B90078    add
              em2     LIKE type_file.num5,    ## End  期別     #TQC-B90078    add
...
MAIN
   ...
   LET tm.title1  = ARG_VAL(9)         #TQC-B90078    add
   LET tm.yy1 = ARG_VAL(10)            #TQC-B90078    add
   LET tm.bm1 = ARG_VAL(11)            #TQC-B90078    add
   LET tm.em1 = ARG_VAL(12)            #TQC-B90078    add
   LET tm.title2  = ARG_VAL(13)        #TQC-B90078    add
   LET tm.yy2 = ARG_VAL(14)            #TQC-B90078    add
   LET tm.bm2 = ARG_VAL(15)            #TQC-B90078    add
   LET tm.em2 = ARG_VAL(16)            #TQC-B90078    add
   LET tm.b = ARG_VAL(17)
   LET tm.c = ARG_VAL(18)
   LET tm.d = ARG_VAL(19)
   LET tm.o = ARG_VAL(20)
   ...
   LET tm.asa01 = ARG_VAL(28)
   #TQC-B90078--mod--end--

   IF cl_null(tm.b) THEN
      SELECT asz01 INTO tm.b FROM asz_file WHERE asz00 = '0'
   END IF

   LET g_rlang  = g_lang

   OPEN WINDOW t940_w AT 5,10
        WITH FORM "ggl/42f/gglt504" ATTRIBUTE(STYLE = g_win_style)              
                                                                                
   CALL cl_ui_init()                                                           
   CALL cl_set_comp_visible("tia08b",TRUE)     #TQC-B90078   mod
...
FUNCTION t940_tm()
   ...
   OPEN WINDOW t940_w1 AT p_row,p_col
     WITH FORM "ggl/42f/gglt504_1"  ATTRIBUTE (STYLE = g_win_style CLIPPED)        
 
   CALL cl_ui_locale("gglt504_1") 
   ...
   LET g_bgjob  = 'N'
   LET g_copies = '1'
   SELECT asz01 INTO g_asz01 FROM asz_file WHERE asz00 = '0'    #TQC-B90078    add
   LET tm.b = g_asz01      #TQC-B90078    add

   WHILE TRUE

      INPUT BY NAME tm.title,tm.asa01,
                    #tm.y1,tm.m1,tm.m2,                 #TQC-B90078   mark
                    tm.title1,tm.yy1,tm.bm1,tm.em1,     #TQC-B90078   add
                    tm.title2,tm.yy2,tm.bm2,tm.em2,     #TQC-B90078   add
                    tm.b,tm.d,tm.c,tm.o,
         ...
         AFTER FIELD asa01   #族群代號
            IF NOT cl_null(tm.asa01) THEN
               ...
               END IF
               #TQC-B90078--add--str--
               LET tm.title1 = cl_getmsg('cgl-015',g_lang)
               LET tm.title2 = cl_getmsg('cgl-016',g_lang)
               SELECT aaa04,aaa05
                 INTO tm.yy1,tm.bm1
                 FROM aaa_file
               ...
               LET tm.bm2 = 1
               LET tm.em2 = tm.em1
               DISPLAY BY NAME tm.title1,tm.title2,tm.yy1,tm.bm1,tm.em1,tm.yy2,tm.bm2,tm.em2
               #TQC-B90078--add--end--
            END IF
         ...
         AFTER FIELD yy2
            IF tm.yy2 IS NULL OR tm.yy2 = 0 THEN
               NEXT FIELD yy2
            END IF

         AFTER FIELD bm2
             IF NOT cl_null(tm.bm2) THEN
                SELECT azm02 INTO g_azm.azm02 FROM azm_file
                 WHERE azm01 = tm.yy2
                IF g_azm.azm02 = 1 THEN
                   IF tm.bm2 > 12 OR tm.bm2 < 1 THEN
                      CALL cl_err('','agl-020',0)
                      NEXT FIELD bm2
                   END IF
                ELSE
                   IF tm.bm2 > 13 OR tm.bm2 < 1 THEN
                      CALL cl_err('','agl-020',0)
                      NEXT FIELD bm2
                   END IF
                END IF
             END IF
             IF tm.bm2 IS NULL THEN NEXT FIELD bm2 END IF
         ...
         AFTER FIELD em2
             IF NOT cl_null(tm.em2) THEN
                ...
                END IF
             END IF
             IF tm.em2 IS NULL THEN 
                NEXT FIELD em2 
             END IF
             IF tm.bm2 > tm.em2 THEN 
                CALL cl_err('','9011',0) 
                NEXT FIELD bm2 
             END IF
        #lujh 110906--mod--end--
     ...
     IF g_bgjob = 'Y' THEN
        ...
        ELSE
           LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                        ...
                        " '",g_copies CLIPPED,"'",
                        " '",tm.title CLIPPED,"'" ,
                        #" '",tm.y1 CLIPPED,"'" ,    #TQC-B90078   mark
                        #" '",tm.y2 CLIPPED,"'" ,    #TQC-B90078   mark
                        #" '",tm.m1 CLIPPED,"'" ,    #TQC-B90078   mark 
                        #" '",tm.m2 CLIPPED,"'" ,    #TQC-B90078   mark
                        " '",tm.title1 CLIPPED,"'",  #TQC-B90078   add
                        " '",tm.yy1 CLIPPED,"'",     #TQC-B90078   add
                        " '",tm.bm1 CLIPPED,"'",     #TQC-B90078   add
                        " '",tm.em1 CLIPPED,"'",     #TQC-B90078   add
                        " '",tm.title2 CLIPPED,"'",  #TQC-B90078   add 
                        " '",tm.yy2 CLIPPED,"'",     #TQC-B90078   add
                        " '",tm.bm2 CLIPPED,"'",     #TQC-B90078   add
                        " '",tm.em2 CLIPPED,"'",     #TQC-B90078   add  
                        " '",tm.b CLIPPED,"'" ,
                       # " '",tm.ver CLIPPED,"'" ,
                        " '",tm.c CLIPPED,"'" ,
                        " '",tm.d CLIPPED,"'" ,
                        ...
                        " '",g_template CLIPPED,"'",
                        " '",tm.asa01 CLIPPED,"'"
            CALL cl_cmdat('gglt504',g_time,l_cmd)    # Execute cmd at later tim
        END IF
...
DEFINE l_atd03     LIKE atd_file.atd03
DEFINE l_atd04     LIKE atd_file.atd04
DEFINE l_atd05     LIKE atd_file.atd05
DEFINE l_ate07     LIKE ate_file.ate07
DEFINE l_ate08     LIKE ate_file.ate08
DEFINE l_ate10     LIKE ate_file.ate10
DEFINE l_ate06     LIKE ate_file.ate06
DEFINE l_ate09     LIKE ate_file.ate09
DEFINE l_atj01     LIKE atj_file.atj01
DEFINE l_atj02     LIKE atj_file.atj02
DEFINE l_amt_s     LIKE atc_file.atc08
DEFINE l_amt_s_b   LIKE atc_file.atc08      #TQC-B90078  add
DEFINE l_amt_s1    LIKE atc_file.atc08
DEFINE l_amt_s1_b  LIKE atc_file.atc08      #TQC-B90078  add
DEFINE l_amt_s2    LIKE atc_file.atc08
DEFINE l_amt_s2_b  LIKE atc_file.atc08      #TQC-B90078  add
DEFINE l_sub_amt   LIKE atc_file.atc08
DEFINE l_sub_amt_b LIKE atc_file.atc08      #TQC-B90078  add
DEFINE l_amt       LIKE atc_file.atc08
DEFINE l_last_y    LIKE type_file.num5
DEFINE l_last_m    LIKE type_file.num5 
DEFINE l_amt1      LIKE atc_file.atc08      #TQC-B90078  add
DEFINE l_last_y1   LIKE type_file.num5      #TQC-B90078  add
DEFINE l_last_m1   LIKE type_file.num5      #TQC-B90078  add
DEFINE l_atm     RECORD LIKE atm_file.*
DEFINE g_rec_b_t   LIKE type_file.num10     #TQC-B90078  add
DEFINE tmp RECORD
            atk02      LIKE atk_file.atk02, 
            atk03      LIKE atk_file.atk03,
            atk04      LIKE atk_file.atk04
            END RECORD
   #No.FUN-B80057--mark--Beatk--- 
   #CALL  cl_used(g_prog,g_time,1) RETURNING g_time 
   #No.FUN-B80057--mark--End-----
   LET g_prog = 'gglt504'
   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'gglt504'
...
#  SELECT * FROM atd_file
#   WHERE atd_file.atd03 = tm.y1
#     AND atd_file.atd04 BETWEEN tm.m1 AND tm.m2
#     AND atd_file.atd01 = tm.asa01   #族群
#     AND atd_file.atd12 NOT IN (SELECT asa02 FROM asa_file WHERE asa01= tm.asa01)
#     AND atd_file.atd12 NOT IN (SELECT asb04 FROM asb_file WHERE asb01= tm.asa01)
#    INTO TEMP y

    #TQC-B90078--mark--str--
    #SELECT * FROM atf_file
    #WHERE atf_file.atf03 = tm.y1
    #  AND atf_file.atf04 BETWEEN tm.m1 AND tm.m2
    #  AND atf_file.atf01 = tm.asa01   #族群
    # INTO TEMP y
    #TQC-B90078--mark--end--
#FU-B50001--mod--end
 ...
 #TQC-B90078--mod--str--
  ...
  SELECT atf05,SUM(atf06) as atf06 FROM atf_file
   WHERE atf_file.atf03 = tm.yy1
     AND atf_file.atf04 BETWEEN tm.bm1 AND tm.em1
     AND atf_file.atf01 = tm.asa01   #族群
   GROUP BY atf05
    INTO TEMP y

  SELECT atf05,SUM(atf06) as atf06 FROM atf_file
   WHERE atf_file.atf03 = tm.yy2
     AND atf_file.atf04 BETWEEN tm.bm2 AND tm.em2
     AND atf_file.atf01 = tm.asa01   #族群
   GROUP BY atf05
    INTO TEMP x
  
  LET l_sql = "SELECT nml01,nml02,nml03,nml05,y.atf06 tia08a, x.atf06 tia08b",
              "  FROM nml_file LEFT OUTER JOIN y",
              #" WHERE y.atd05(+) = nml01",   #luttb 110506
              "    ON y.atf05 = nml01",
              "  LEFT OUTER JOIN x ",
              "    ON x.atf05 = nml01"
  
   #LET l_sql = "SELECT nml01,nml02,nml03,nml05,SUM(atd06) tia08a", #FUN-B50001 luttb 
      #TQC-B90078--mark--str--
      #LET l_sql = "SELECT nml01,nml02,nml03,nml05,SUM(atf06) tia08a",
      #            "  FROM nml_file LEFT OUTER JOIN y",
      #            #"    ON y.atd05 = nml01 ",   #FUN-B50001 luttb
      #            "    ON y.atf05 = nml01 ",    #FUN-B50001 luttb
      #            " GROUP BY nml01,nml02,nml03,nml05"
      #TQC-B90078--mark--str--
   #TQC-B90078--mod--end--
   PREPARE gglt504_gentemp FROM l_sql
   IF STATUS THEN
      CALL cl_err('prepare',STATUS,1)
      RETURN
   END IF

   CALL cl_outnam('gglt504') RETURNING l_name
   ...
   CALL g_nml.clear()
   LET g_rec_b = 1
   DECLARE gglt504_curs CURSOR FOR gglt504_gentemp 
   FOREACH gglt504_curs INTO sr.nml01,sr.nml02,sr.nml03,sr.nml05,sr.tia08a,sr.tia08b    #TQC-B90078   mod
      ...
      IF sr.tia08b IS NULL THEN LET sr.tia08b = 0 END IF       #TQC-B90078   add
...
##################以下为现金流量表间接法
   ...
   #TQC-B90078--add--str--
   IF tm.bm1=1 THEN 
      LET l_last_y = tm.yy1 - 1
      LET l_last_m = 12
   ELSE
      LET l_last_y = tm.yy1
      LET l_last_m = tm.bm1 - 1
   END IF

   IF tm.bm2=1 THEN 
      LET l_last_y1 = tm.yy2 - 1
      LET l_last_m1 = 12
   ELSE
      LET l_last_y1 = tm.yy2
      LET l_last_m1 = tm.bm2 - 1
   END IF
   #TQC-B90078--add--end-- 
   ...
   ###经营活动
   LET l_sql="SELECT atj01,atj02 FROM atj_file ",
             #" WHERE atj03 NOT IN ('4','5') AND atj04='Y'" #No:FUN-B80180 mark
             " WHERE atj03 NOT IN ('4','5','6') AND atj04='Y'"  #No:FUN-B80180 add
   PREPARE t940_atj_p1 FROM l_sql
   DECLARE t940_atj_cs1 CURSOR FOR t940_atj_p1

   LET l_sql = "SELECT atk02,atk03,atk04",
               "  FROM atk_file,atj_file",
               " WHERE atk01 = atj01  AND atj04='Y' AND atk01 = ?"
   PREPARE t940_p31 FROM l_sql
   DECLARE t940_cu31 CURSOR FOR t940_p31
   LET l_sub_amt = 0
   LET l_sub_amt_b = 0    #TQC-B90078  add
   FOREACH t940_atj_cs1 INTO l_atj01,l_atj02
      LET l_amt_s = 0
      LET l_amt = 0
      LET l_amt_s_b = 0     #TQC-B90078   add
      LET l_amt1 = 0        #TQC-B90078   add
      FOREACH t940_cu31 USING l_atj01 INTO tmp.*
          #TQC-B90078--mark--str--
          #CASE tmp.atk04    
          #  WHEN '1'    #异动
          #    CALL t940_atc(tmp.atk02,tm.y1,tm.m2,l_last_y,l_last_m) 
          #       RETURNING l_amt
          #    IF cl_null(l_amt) THEN LET l_amt = 0 END IF 
          #  WHEN '2'    #期初
          #    CALL t940_atc2(tmp.atk02,l_last_y,l_last_m) RETURNING l_amt
          #    IF cl_null(l_amt) THEN LET l_amt = 0 END IF
          #  WHEN '3'    #期末
          #    CALL t940_atc2(tmp.atk02,tm.y1,tm.m2) RETURNING l_amt
          #    IF cl_null(l_amt) THEN LET l_amt = 0 END IF
          #  WHEN '4'    #人工录入
          #    SELECT SUM(atl05) INTO l_amt FROM atl_file
          #      WHERE atl01 = l_atj01 AND atl02 = tmp.atk02
          #        AND atl06 = tm.y1 AND atl07 = tm.m2
          #        AND atl08 = tm.asa01
          #        AND atl00 = tm.b
          #    IF cl_null(l_amt) THEN LET l_amt = 0 END IF
          #END CASE
          ...
          #TQC-B90078--add--str--
           ...
           CASE tmp.atk04
            WHEN '1'    #异动
              CALL t940_atc(tmp.atk02,tm.yy1,tm.em1,l_last_y,l_last_m) 
                 RETURNING l_amt
              IF cl_null(l_amt) THEN LET l_amt = 0 END IF 
              CALL t940_atc(tmp.atk02,tm.yy2,tm.em2,l_last_y1,l_last_m1) 
                 RETURNING l_amt1
              IF cl_null(l_amt1) THEN LET l_amt1 = 0 END IF 
            WHEN '2'    #期初
              CALL t940_atc2(tmp.atk02,l_last_y,l_last_m) RETURNING l_amt
              IF cl_null(l_amt) THEN LET l_amt = 0 END IF
              CALL t940_atc2(tmp.atk02,l_last_y1,l_last_m1) RETURNING l_amt1
              IF cl_null(l_amt1) THEN LET l_amt1 = 0 END IF
            WHEN '3'    #期末
              CALL t940_atc2(tmp.atk02,tm.yy1,tm.em1) RETURNING l_amt
              IF cl_null(l_amt) THEN LET l_amt = 0 END IF
              CALL t940_atc2(tmp.atk02,tm.yy2,tm.em2) RETURNING l_amt1
              IF cl_null(l_amt1) THEN LET l_amt1 = 0 END IF
            WHEN '4'    #人工录入
              SELECT SUM(atl05) INTO l_amt FROM atl_file
                WHERE atl01 = l_atj01 AND atl02 = tmp.atk02
                  AND atl06 = tm.yy1 AND atl07 = tm.em1
                  AND atl08 = tm.asa01
                  AND atl00 = tm.b
              IF cl_null(l_amt) THEN LET l_amt = 0 END IF
              SELECT SUM(atl05) INTO l_amt1 FROM atl_file
                WHERE atl01 = l_atj01 AND atl02 = tmp.atk02
                  AND atl06 = tm.yy2 AND atl07 = tm.em2
                  AND atl08 = tm.asa01
                  AND atl00 = tm.b
              IF cl_null(l_amt1) THEN LET l_amt1 = 0 END IF
            WHEN '5'    
              CALL t940_atc3(tmp.atk02,tm.yy1,tm.em1,l_last_y,l_last_m) 
                 RETURNING l_amt
              IF cl_null(l_amt) THEN LET l_amt = 0 END IF 
              CALL t940_atc3(tmp.atk02,tm.yy2,tm.em2,l_last_y1,l_last_m1) 
                 RETURNING l_amt1
              IF cl_null(l_amt1) THEN LET l_amt1 = 0 END IF 
            WHEN '6'    
              CALL t940_atc4(tmp.atk02,tm.yy1,tm.em1,l_last_y,l_last_m) 
                 RETURNING l_amt
              IF cl_null(l_amt) THEN LET l_amt = 0 END IF 
              CALL t940_atc4(tmp.atk02,tm.yy2,tm.em2,l_last_y1,l_last_m1) 
                 RETURNING l_amt1
              IF cl_null(l_amt1) THEN LET l_amt1 = 0 END IF 
          END CASE
          #TQC-B90078--add--end--
          ...
          IF tmp.atk03 = '-' THEN 
             LET l_amt  = l_amt * -1 
             LET l_amt1 = l_amt1* -1     #TQC-B90078   add
          END IF
          LET l_amt_s = l_amt_s + l_amt
          LET l_amt_s_b = l_amt_s_b + l_amt1     #TQC-B90078   add
          IF (l_amt_s = 0 OR l_amt_s_b = 0) AND tm.c = 'N' THEN    #TQC-B90078   mod
             CONTINUE FOREACH
          END IF
      END FOREACH
      LET g_rec_b = g_rec_b +1
      LET g_nml[g_rec_b].nml02 = l_atj02
      LET g_nml[g_rec_b].nml05 = g_rec_b
      LET g_nml[g_rec_b].tia08a = l_amt_s
      LET g_nml[g_rec_b].tia08b = l_amt_s_b     #TQC-B90078   add

      Let l_sub_amt = l_sub_amt + l_amt_s   #計算经营活动产生的现金流量净额
      LET l_amt_s = l_amt_s*tm.q/g_unit     #依匯率及單位換算
      Let l_sub_amt_b = l_sub_amt_b + l_amt_s_b   #TQC-B90078  add
      LET l_amt_s_b = l_amt_s_b*tm.q/g_unit       #TQC-B90078  add
   END FOREACH
   ...
   LET g_nml[g_rec_b].tia08b = l_sub_amt_b     #TQC-B90078  add
   ...
   LET g_nml[g_rec_b].tia08b = NULL       #TQC-B90078    add
   ...
   LET g_nml[g_rec_b].tia08b = 0         #TQC-B90078    add
   ...
   ###现金的期末余额
   SELECT atj01,atj02 INTO l_atj01,l_atj02
     FROM atj_file
    WHERE atj03 = '5' AND atj04 = 'Y'

   LET l_sql = "SELECT atk02,atk03,atk04",
               "  FROM atk_file,atj_file",
               #" WHERE atk01 = atj01  AND atj04='Y' AND atk01 = l_atj01"    #FUN-B60095
               " WHERE atk01 = atj01  AND atj04='Y' AND atk01 = '",l_atj01,"'" #FUN-B60095
   ...
   LET l_amt_s1 = 0
   LET l_amt1 = 0       #TQC-B90078   add
   LET l_amt_s1_b = 0   #TQC-B90078   add
   FOREACH t940_cu32 INTO tmp.* 
      ...
      LET l_amt_s1_b = l_amt_s1_b + l_amt1     #TQC-B90078   add
      IF (l_amt_s1 = 0 OR l_amt_s1_b = 0) AND tm.c = 'N' THEN     #TQC-B90078   mod
         CONTINUE FOREACH
      END IF
   END FOREACH
   LET g_rec_b = g_rec_b +1
   ...
   LET g_nml[g_rec_b].tia08b = l_amt_s1_b       #TQC-B90078   add

   ### 减：现金的期初余额
   SELECT atj01,atj02 INTO l_atj01,l_atj02
     FROM atj_file
    WHERE atj03 = '4' AND atj04 = 'Y'

   LET l_sql = "SELECT atk02,atk03,atk04",
               "  FROM atk_file",
               #" WHERE atk01 = l_atj01"  #FUN-B60095
               " WHERE atk01 = '",l_atj01,"'"   #FUN-B60095
   ...
   LET l_amt_s2 = 0
   LET l_amt1 = 0             #TQC-B90078   add
   LET l_amt_s2_b = 0         #TQC-B90078   add
   FOREACH t940_cu33 INTO tmp.*
      ...
      LET l_amt_s2_b = l_amt_s2_b + l_amt1    #TQC-B90078   add 
      IF (l_amt_s2 = 0 OR l_amt_s2_b = 0) AND tm.c = 'N' THEN     #TQC-B90078  mod
         CONTINUE FOREACH
      END IF
   END FOREACH
   LET g_rec_b = g_rec_b +1
   ...
   LET g_nml[g_rec_b].tia08b = l_amt_s2_b      #TQC-B90078   add
   ...
   LET g_nml[g_rec_b].tia08b = l_amt_s1_b-l_amt_s2_b      #TQC-B90078   add

#luttb 101227--add--str-- ###揭露事项(agli920)
   ###揭露事项
   LET g_rec_b = g_rec_b +1 
   LET g_rec_b_t=g_rec_b                        #TQC-B90078    add
   LET g_nml[g_rec_b].nml02 = g_x[42]
   LET g_nml[g_rec_b].nml05 = g_rec_b
   LET g_nml[g_rec_b].tia08a = NULL
   LET g_nml[g_rec_b].tia08b = NULL             #TQC-B90078    add

   DECLARE sel_atm_cur CURSOR FOR
    SELECT atm_file.* FROM atm_file
     WHERE atm00 = 'Y'
       AND atm04 = ?   #No:FUN-B80180 add        #TQC-B90078    mod
       AND atm05 = ?  #No:FUN-B80180 add         #TQC-B90078    mod
       #AND atm06 = tm.asa01 #No:FUN-B80180 add
   FOREACH sel_atm_cur USING tm.yy1,tm.em1  INTO l_atm.*    #TQC-B90078    mod
       LET g_rec_b = g_rec_b+1 
       LET g_nml[g_rec_b].nml02 = l_atm.atm02
       LET g_nml[g_rec_b].nml05 = g_rec_b
       LET g_nml[g_rec_b].tia08a = l_atm.atm03
   END FOREACH
   #TQC-B90078--add--str--
   LET g_rec_b=g_rec_b_t 
   FOREACH sel_atm_cur USING tm.yy2,tm.em2 INTO l_atm.*
      LET g_rec_b = g_rec_b+1 
      LET g_nml[g_rec_b].tia08b = l_atm.atm03
   END FOREACH
   #TQC-B90078--add--end--
...
DEFINE
   ...
   DISPLAY tm.yy2 TO FORMONLY.yy
   DISPLAY tm.bm2 TO FORMONLY.mm1
   DISPLAY tm.em2 TO FORMONLY.mm2
   ...
   IF NOT cl_null(g_msg) THEN
      CALL cl_set_comp_att_text("tia08a",g_msg CLIPPED)
   END IF

   LET g_msg = tm.title2
   IF NOT cl_null(g_msg) THEN
      CALL cl_set_comp_att_text("tia08b",g_msg CLIPPED)
   END IF
   #TQC-B90078--add--end--
...
FUNCTION t940_out()
  DEFINE l_i   LIKE type_file.num10
   ...
   LET l_table = cl_prt_temptable('gglt504',g_sql) CLIPPED
   IF l_table = -1 THEN
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80057--add--
      EXIT PROGRAM
   END IF
   ...
   #TQC-B90078--add--str--
   LET g_str = g_str,";",tm.title,";",tm.p,";",tm.d,";",
               t_azi04,";",t_azi05,";",g_zo12,";",tm.yy2,';',tm.bm2,';',
               tm.em2,';',g_zx02  #FUN-920034 #DFUN-960012
   #TQC-B90078--end--str--
   CALL cl_prt_cs3('gglt504','gglt504',g_sql,g_str)
   #No.FUN-780031  --End
END FUNCTION


FUNCTION t940_atc(p_atc05,p_atc06,p_atc07,p_last_y,p_last_m)
   DEFINE p_atc05     LIKE atc_file.atc05,
          p_atc06     LIKE atc_file.atc06,
          p_atc07     LIKE atc_file.atc07,
          p_last_y    LIKE atc_file.atc06,   
          p_last_m    LIKE atc_file.atc07,  
          l_value1    LIKE atc_file.atc08, 
          l_value2    LIKE atc_file.atc08,
          l_amt       LIKE atc_file.atc08,
   ...
   #本期餘額
   SELECT SUM(atc08-atc09) INTO l_value1 FROM atc_file
    WHERE atc00=tm.b                           #帳別
      AND atc05=p_atc05                        #科目
      AND atc06=p_atc06                        #年度
      AND atc07=p_atc07                        #期別     #TQC-B90078    mod
      AND atc01=tm.asa01                       #族群
      AND atc13 = '00'
   IF cl_null(l_value1) THEN LET l_value1 = 0 END IF 
   #上期餘額
   SELECT SUM(atc08-atc09) INTO l_value2 FROM atc_file
    WHERE atc00=tm.b                           #帳別
      AND atc05=p_atc05                        #科目
      AND atc06=p_last_y                       #年度
      AND atc07=p_last_m                       #期別   
      AND atc01=tm.asa01                       #族群
      AND atc13 = '00'              
...
FUNCTION t940_atc2(p_atc05,p_atc06,p_atc07)
   DEFINE p_atc05     LIKE atc_file.atc05,
          p_atc06     LIKE atc_file.atc06,
          p_atc07     LIKE atc_file.atc07,
          p_value     LIKE atc_file.atc08  
   ...
   SELECT SUM(atc08-atc09) INTO p_value FROM atc_file
    WHERE atc00=tm.b                   #帳別
      AND atc05=p_atc05                #科目
      AND atc06=p_atc06                #年度
      AND atc07=p_atc07                #期別
      AND atc01=tm.asa01               #族群
      AND atc13='00'                   #版本 
   IF l_type = '2' THEN   #貸餘
...
#TQC-B90078--add--str--
FUNCTION t940_atc3(p_atc05,p_atc06,p_atc07,p_last_y,p_last_m)
   DEFINE p_atc05     LIKE atc_file.atc05,
          p_atc06     LIKE atc_file.atc06,
          p_atc07     LIKE atc_file.atc07,
          p_last_y    LIKE atc_file.atc06,   
   ...
   #本期餘額
   SELECT SUM(atc08) INTO l_value1 FROM atc_file
    WHERE atc00=tm.b                           #帳別
      AND atc05=p_atc05                        #科目
      AND atc06=p_atc06                        #年度
      AND atc07=p_atc07                        #期別 
   ...
   IF cl_null(l_value1) THEN LET l_value1 = 0 END IF 
   #上期餘額
   SELECT SUM(atc08) INTO l_value2 FROM atc_file
    WHERE atc00=tm.b                           #帳別
      AND atc05=p_atc05                        #科目
      AND atc06=p_last_y                       #年度
      AND atc07=p_last_m                       #期別   
...
FUNCTION t940_atc4(p_atc05,p_atc06,p_atc07,p_last_y,p_last_m)
   DEFINE p_atc05     LIKE atc_file.atc05,
          p_atc06     LIKE atc_file.atc06,
          p_atc07     LIKE atc_file.atc07,
          p_last_y    LIKE atc_file.atc06,   
   ...
   #本期餘額
   SELECT SUM(atc09) INTO l_value1 FROM atc_file
    WHERE atc00=tm.b                           #帳別
      AND atc05=p_atc05                        #科目
      AND atc06=p_atc06                        #年度
      AND atc07=p_atc07                        #期別 
   ...
   IF cl_null(l_value1) THEN LET l_value1 = 0 END IF 
   #上期餘額
   SELECT SUM(atc09) INTO l_value2 FROM atc_file
    WHERE atc00=tm.b                           #帳別
      AND atc05=p_atc05                        #科目
      AND atc06=p_last_y                       #年度
      AND atc07=p_last_m                       #期別   
