...
# Descriptions...: 核算項科目明細帳
# Date & Author..: 10/11/23 By Elva
...
MAIN
   ...
   OPEN WINDOW q708_w AT 5,10
        WITH FORM "ggl/42f/gglq708_1" ATTRIBUTE(STYLE = g_win_style)
...
FUNCTION gglq708_tm()
  ...
  #LET tm.c    = 'N' #modify by mb090923
   ...
   WHILE TRUE
     ...
     IF INT_FLAG THEN
        LET INT_FLAG = 0 CLOSE WINDOW gglq708_w1
        CALL cl_used(g_prog,g_time,2) RETURNING g_time   #CHI-C30115 add
        EXIT PROGRAM
     END IF
     ...
     IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW gglq708_w1 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #CHI-C30115
         EXIT PROGRAM
     END IF

     DISPLAY BY NAME tm.a,tm.b,tm.c,tm.more
     INPUT BY NAME bookno,bdate,edate,tm.a,tm.b,tm.c,tm.more
           WITHOUT DEFAULTS
     ...
     IF g_bgjob = 'Y' THEN
        ...
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #CHI-C30115 #CHI-C30115 add
        EXIT PROGRAM
     END IF
     CALL cl_wait()
     CALL gglq708()
        ...
        FOREACH gglq708_ted02_cs1 USING l_aag01_str
          ...
          #CALL gglq708_get_ahe02(l_aag01_str,l_ted02)           #No.100113 mark
           CALL gglq708_get_ahe02(l_aag01_str,l_ted02,sr1.aag01) #No.100113 
                RETURNING l_ted02_d
           INSERT INTO gglq708_ted_tmp VALUES(sr1.aag01,sr1.aag02,l_ted02,l_ted02_d)
...
#          IF SQLCA.sqlcode AND SQLCA.sqlcode <> -239 THEN #CHI-C30115 mark
     ...
     DECLARE gglq708_cs1 CURSOR FOR
      SELECT UNIQUE aag01,aag02,ted02,ted02_d FROM gglq708_ted_tmp
       ORDER BY aag01,ted02

     FOREACH gglq708_cs1 INTO sr2.*
        IF SQLCA.sqlcode != 0 THEN
        ...
        IF tm.c MATCHES '[12]'  THEN #1 全部 2 已审核
           #1~mm-1
...
FUNCTION gglq708_b_fill()
  ...
  DEFINE  l_aag06    LIKE aag_file.aag06  
   ...
   LET g_sql = "SELECT aag01,aag02,yy,aba04,aba02,aba01,abb04,abb24,df,abb25_d,d,",    
               "       cf,abb25_c,c,dc,balf,abb25_bal,bal,",
               "       azi04,azi05,azi07,abb06,type ",
               "  FROM gglq708_tmp",
               " WHERE ted02 ='",g_ted02,"'",
               " ORDER BY aag01,type,aba04,aba02,aba01,order_no "
...
DEFINE    l_order_no                  INT 
  OUTPUT TOP MARGIN g_top_margin
    ...
    ORDER BY sr.ted02,sr.aag01,sr.yy,sr.aba04,sr.aba02,sr.aba01
...
FUNCTION q708_drill_down()  
   ...
   IF cl_null(g_ted02) THEN RETURN END IF
