...
#              "sign_show.type_file.chr1,sign_str.type_file.chr1000"    #是否顯示簽核資料(Y/N)  #TQC-C10039  #MOD-C20064 Mark TQC-C10039
   ...
   LET l_table = cl_prt_temptable('gglr403',g_sql) CLIPPED                      
...
#     LET g_aaa.aaa01 = g_aaz.aaz64
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL gglr403_tm()	        	# Input print condition
      ELSE CALL gglr403()			# Read data and create out-file
   END IF
...
FUNCTION gglr403_tm()
   ...
   OPEN WINDOW gglr403_w AT p_row,p_col
        WITH FORM "ggl/42f/gglr403"
...
#
#    IF INT_FLAG THEN
#       LET INT_FLAG = 0 CLOSE WINDOW gglr403_w EXIT PROGRAM
#    END IF
#FUN-B20054--mark--end
    ...
    IF g_bgjob = 'Y' THEN
       ...
       ELSE
          ...
          LET l_cmd = l_cmd CLIPPED,
                     ...
                     " '",g_rep_clas CLIPPED,"'",
                     " '",g_template CLIPPED,"'" 
          CALL cl_cmdat('gglr403',g_time,l_cmd)	# Execute cmd at later time
       END IF
       CLOSE WINDOW gglr403_w
       CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
       EXIT PROGRAM
...
#No.FUN-7C0066   ----- end
    ...
    #IF tm.c = 'Y' THEN LET l_sql1 = l_sql1 CLIPPED," AND aag24 = 99 " END IF   
     ...
     END IF 
     #No.MOD-860252---end---
     PREPARE gglr403_prepare2 FROM l_sql1
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
        EXIT PROGRAM
     END IF
     DECLARE gglr403_curs2 CURSOR FOR gglr403_prepare2
     ...
     PREPARE gglr403_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
        EXIT PROGRAM
     END IF
     DECLARE gglr403_curs1 CURSOR FOR gglr403_prepare1
...
#No.FUN-7C0066  -----begin
#    CALL cl_outnam('gglr403') RETURNING l_name
#    IF tm.e = 'N' THEN
#       START REPORT gglr403_rep1 TO l_name
#    ELSE
#       START REPORT gglr403_rep2 TO l_name
#    END IF
#    LET g_pageno = 0
#    CALL cl_prt_pos_len() 
#No.FUN-7C0066  ----end
     FOREACH gglr403_curs2 INTO sr1.*
        IF SQLCA.sqlcode != 0 THEN
           CALL cl_err('foreach:',SQLCA.sqlcode,1)
           EXIT FOREACH
        END IF
        CALL gglr403_qcye(sr1.aag01)      #期初余額
             RETURNING l_bal,l_bal2
        IF tm.a = 'N' THEN
           CALL gglr403_qjyd(sr1.aag01) RETURNING l_flag
           IF tm.e = 'N' THEN
        ...
        END IF
        #FUN-6C0012.....begin
#       CALL gglr403_sjkm(sr1.aag01) RETURNING p_aag02
        IF tm.g = '1' THEN                                                      
           LET p_aag02 = NULL                                                   
        END IF
        IF tm.g = '2' THEN                                                      
...
#              AND aag00 = sr1.aaa01   # No.FUN-740055 #FUN-7C0066                                       
        ...
        FOR l_i = mm1 TO nn1
            #No.FUN-7C0066   ------begin
            CALL r403_cpt(sr1.aag01,l_i) RETURNING l_md,l_mdf,l_mc,l_mcf
            CALL r403_cpt1(sr1.aag01,l_i) RETURNING l_yd,l_ydf,l_yc,l_ycf
            #No.FUN_7C0066   ------ end
            LET g_cnt = 0
            LET l_flag='N'
            FOREACH gglr403_curs1 USING sr1.aag01,l_i INTO sr.*
               IF SQLCA.sqlcode != 0 THEN
                  CALL cl_err('foreach:',SQLCA.sqlcode,1)
                  EXIT FOREACH
               END IF
                  ...
                  ELSE
                     UPDATE tpg_file SET tpg04 = g_pageno
                      WHERE tpg01 = YEAR(bdate)
                        ...
                        AND tpg05 = g_prog 
                  END IF
#                 OUTPUT TO REPORT gglr403_rep1(sr.*)
               LET l_name = 'gglr403'
                      ...
                      ELSE
                         UPDATE tpg_file SET tpg04 = g_pageno
                          WHERE tpg01 = YEAR(bdate)
                            ...
                            AND tpg05 = g_prog 
                      END IF
#                 OUTPUT TO REPORT gglr403_rep2(sr.*)
                  LET l_name = 'gglr403_1'
                  EXECUTE insert_prep USING  sr.aag01,sr.mm,g_pageno,sr.aag02,l_date,
                                             l_begin,sr.qcye,l_qcye,l_abb07_dsum,t_abb07_dsum,
                                             sr.aba02,sr.aba01,sr.abb04,t_abb03t,m_abb03t,
                                             sr.abb07,l_mm,l_abb07_sum,t_abb07_sum,l_abb07,
...
#No.FUN-7C0066     ------begin
#    IF tm.e = 'N' THEN
#       FINISH REPORT gglr403_rep1
#    ELSE
#       FINISH REPORT gglr403_rep2
#    END IF
     IF g_zz05 = 'Y' THEN                                                         
        CALL cl_wcchp(tm.wc1,'pmn04,pmm09,pmm01,pmm04,pmm22')                                   
        RETURNING g_str                                                         
     END IF              
     LET g_str = g_str,";",YEAR(bdate),";",bdate,";",g_azi04,";",tm.g 
...
#     LET g_cr_apr_key_f = "aag01"             #報表主鍵欄位名稱  #TQC-C10039	 #MOD-C20064 Mark TQC-C10039
     CALL cl_prt_cs3('gglr403',l_name,l_sql,g_str)
...
#yearly qjyd
FUNCTION r403_cpt1(p_aag01,p_i)
  DEFINE p_aag01   LIKE aag_file.aag01
  DEFINE p_i       LIKE type_file.num5
  DEFINE l_bdate   LIKE aba_file.aba02
  DEFINE l_edate   LIKE aba_file.aba02
...
#FUNCTION gglr403_sjkm(l_aag01)   #追溯上級科目名稱且將其連成一體
...
#END FUNCTION
#FUN-6C0012.....end
FUNCTION gglr403_qcye(l_aag01)   #計算列印期間前的余額
   DEFINE l_aag01 LIKE aag_file.aag01
...
FUNCTION gglr403_qjyd(l_aag01)   #計算列印期間前的余額
