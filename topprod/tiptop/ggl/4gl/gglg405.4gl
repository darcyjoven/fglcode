...
# Modify.........: No.MOD-8B0123 08/11/12 By Sarah 將g405_file裡的abb04欄位放大成CHAR(40)
...
# Modify.........: No:FUN-C50007 12/05/09 By minpp GR程序优化 
...
TYPE sr1_t RECORD
    ...
    head LIKE type_file.chr1000,
    l_i LIKE type_file.num5
END RECORD

#TYPE sr2_t RECORD     #FUN-B50018-----mark----
 
#END RECORD            #FUN-B50018-----mark----
###GENGRE###END
...
MAIN
   ...
   IF (NOT cl_setup("GGL")) THEN
      ...
      #CALL cl_gre_drop_temptable(g_table1||"|"||g_table2)              #FUN-B50018  add   #FUN-C10036   mark
      EXIT PROGRAM
   END IF
   ...
   LET g_table1 = cl_prt_temptable('gglg4051',g_sql) CLIPPED
   IF g_table1 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add     #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(g_table1||"|"||g_table2)              #FUN-B50018  add   #FUN-C10036   mark
      EXIT PROGRAM 
   ...
   LET g_table2 = cl_prt_temptable('gglg4052',g_sql) CLIPPED
   IF g_table2 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add    #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(g_table1||"|"||g_table2)              #FUN-B50018  add   #FUN-C10036   mark
      EXIT PROGRAM 
   ...
   DROP TABLE g405_file
   #cnt 第幾張憑証
   #i   套表上的第幾張格式
   #no  每張套表上的第幾列
   CREATE TEMP TABLE g405_file(
       cnt       LIKE type_file.num5,
       i         LIKE type_file.num5,
       no        LIKE type_file.num5,
       aba02     LIKE type_file.dat,
       aba01     LIKE type_file.chr12,
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL g405_tm()                           # Input print condition
   ELSE
      CALL g405()         
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time
   CALL cl_gre_drop_temptable(g_table1||"|"||g_table2)              #FUN-B50018  add
...
FUNCTION g405_tm()
   ...
   OPEN WINDOW g405_w AT p_row,p_col WITH FORM "ggl/42f/gglg405" 
...
#No.TQC-720032 -- end --
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW g405_w 
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_rep_clas CLIPPED,"'",
                         " '",g_template CLIPPED,"'" 
         CALL cl_cmdat('gglg405',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW g405_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(g_table1||"|"||g_table2)              #FUN-B50018  add
...
FUNCTION g405()
   ...
   SELECT aaf03 INTO g_company FROM aaf_file WHERE aaf01 = tm.b
      AND aaf02 = g_rlang
   #No.FUN-7C0064  --Begin
   #SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'gglg405'
   #IF g_len = 0 OR g_len IS NULL THEN LET g_len = 250 END IF 
   #FOR g_i = 1 TO g_len LET g_dash2[g_i,g_i] = '=' END FOR
 
   #CALL cl_outnam('gglg405') RETURNING l_name
   #CALL cl_outnam('gglg405_1') RETURNING l_name1       #No.TQC-710101
   #CALL cl_outnam('gglg405_2') RETURNING l_name2       #No.TQC-710101
   #No.FUN-7C0064  --End  
...
#將科目填入array------------------------------------
   ...
   DECLARE g405_bom1 CURSOR FOR
    SELECT maj21,maj20[1,20],maj20e[1,20] FROM maj_file WHERE maj01=tm.a  #FUN-6C0012
     ORDER BY 1


   FOREACH g405_bom1 INTO l_maj21,l_maj20,l_maj20e  #FUN-6C0012
   ...
   END FOREACH
#---------------------------------------------------
  DECLARE g405_aba_cur CURSOR FOR
   SELECT aba02,aba01,cnt FROM g405_file
    ORDER BY aba02,aba01
   DELETE FROM g405_file
   LET g_cnt = 1
...
#控制一次印十三個科目---------------------------------
   ...
   FOR l_i = 13 TO l_cnt STEP 13
       ...
       FOR l_cn = 1 TO l_k
           ...
           CALL g405_process(l_i,l_cn,l_maj21)
       END FOR
   END FOR
...
#       LET l_name1='g405_',l_i/13 USING '&&&','.out'
#       LET l_name2='g405_2_',l_i/13 USING '&&&','.out''
 
       #No.FUN-7C0064  --Begin
       #IF g_flag1 = 'n' THEN
       #   START REPORT g405_rep1 TO l_name1
       #ELSE
       #   START REPORT g405_rep2 TO l_name2
       #END IF
       #LET g_pageno = 0
       #No.FUN-7C0064  --End  
       ...
       #foreach 所有的憑証
       FOREACH g405_aba_cur INTO l_aba02,l_aba01,l_j  
         IF STATUS THEN 
            CALL cl_err('foreach g405_aba_cur',STATUS,1)
            EXIT FOREACH
         END IF
         ...
         IF g_flag1 = 'n' THEN
            ...
            #No.FUN-7C0064  --Begin
            #OUTPUT TO REPORT g405_rep1(sr.aba02,sr.aba01,sr.abb04,
         ...
         ELSE 
            LET g_flag2 ='1'                    #TQC-710101
            #No.FUN-7C0064  --Begin
            #OUTPUT TO REPORT g405_rep2(sr.aba02,sr.aba01,sr.abb04,
            #                           sr.abb07,sr.abb071,sr.abb06,l_str)
       ...
       #No.FUN-7C0064  --Begin
       #IF g_flag1 = 'n' THEN
       #   FINISH REPORT g405_rep1
       #ELSE
       #   FINISH REPORT g405_rep2
       #END IF
   ...
   #CALL cl_used(g_prog,g_time,2) RETURNING g_time
   #No.FUN-B80096--mark--End-----
   CALL cl_gre_drop_temptable(g_table1||"|"||g_table2)
END FUNCTION
 
FUNCTION g405_prt_v(p_name)
...
FUNCTION g405_process(l_i,l_cn,l_maj21)
  ...
  # CALL cl_used(g_prog,g_time,2) RETURNING g_time      #FUN-C10036  MARK
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      ...
      EXIT PROGRAM
   END IF
   DECLARE g405_c CURSOR FOR g405_p
   ...
   LET l_sql1 = "SELECT aba02,aba01,abb04,abb07,abb06 ",
         ...
         END IF
         PREPARE g405_sum FROM l_sql1
         DECLARE g405_sumc CURSOR FOR g405_sum
   #FUN-C50007--ADD--END

   FOREACH g405_c INTO maj.*
      ...
      IF NOT cl_null(maj.maj21) THEN
        ...
        #PREPARE g405_sum FROM l_sql1
        #DECLARE g405_sumc CURSOR FOR g405_sum
        #FOREACH g405_sumc INTO sr1.*
        #FUN-C50007--MARK--END
         FOREACH g405_sumc USING maj.maj21 INTO sr1.*      #FUN-C50007 add
            ...
            INSERT INTO g405_file VALUES(g_cnt,l_i,l_cn,sr1.aba02,sr1.aba01,
                                         sr1.abb04,sr1.abb07,sr1.abb06)
            IF STATUS OR SQLCA.SQLERRD[3] = 0 THEN
               CALL cl_err('ins g405_file',STATUS,1) EXIT FOREACH
            END IF
            LET g_cnt = g_cnt + 1
         END FOREACH
      END IF
...
#No.FUN-7B0064  --Begin
FUNCTION g405_r1()
###GENGRE###     LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,g_table1 CLIPPED
     LET g_str = ''
###GENGRE###     LET g_str = g_azi04,";",g_aaz.aaz77,";",g_mai02,";",
###GENGRE###                 tm.a,";",tm.yy
###GENGRE###     CALL cl_prt_cs3('gglg405','gglg405',g_sql,g_str)
    CALL gglg405_grdata()    ###GENGRE###
END FUNCTION
...
FUNCTION g405_r2()
     ...
     LET g_str = g_azi04,";",g_aaz.aaz77,";",g_mai02,";",
                 tm.a,";",tm.yy
###GENGRE###     CALL cl_prt_cs3('gglg405','gglg405_1',g_sql,g_str)
    CALL gglg405_grdata()    ###GENGRE###
END FUNCTION
#No.FUN-7B0064  --End  

###GENGRE###START
FUNCTION gglg405_grdata()
    DEFINE l_sql    STRING
    ...
    LET l_cnt = cl_gre_rowcnt(g_table1)
    IF l_cnt <= 0 THEN RETURN END IF
     
    WHILE TRUE
        CALL cl_gre_init_pageheader()            
        LET handler = cl_gre_outnam("gglg405")
        IF handler IS NOT NULL THEN
            START REPORT gglg405_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,g_table1 CLIPPED
          
            DECLARE gglg405_datacur1 CURSOR FROM l_sql
            FOREACH gglg405_datacur1 INTO sr1.*
                OUTPUT TO REPORT gglg405_rep(sr1.*)
            END FOREACH
            FINISH REPORT gglg405_rep
        END IF
...
REPORT gglg405_rep(sr1)
   ...
   #DEFINE sr2 sr2_t
    ...
    DEFINE l_dd         STRING
    DEFINE l_dd_d       LIKE  type_file.num5 
    DEFINE l_h1         STRING
    DEFINE l_dc           STRING 
    DEFINE l_mm         STRING
    DEFINE l_mm_m       LIKE  type_file.num5 
    DEFINE l_yy         STRING
    DEFINE l_yy_y       LIKE  type_file.num5 
    DEFINE l_abb071_sum LIKE  abb_file.abb07
    DEFINE l_abb07_1_fmt STRING
    DEFINE l_abb07_2_fmt STRING
    DEFINE l_abb071_sum_fmt  STRING 
    #FUN-B50018----add-----end----------------- 

    
    ORDER EXTERNAL BY sr1.l_i,sr1.aba01
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
            PRINTX g_time     #FUN-B50018 add
              
        BEFORE GROUP OF sr1.l_i
            LET l_lineno = 0
        ...
        ON EVERY ROW
            ...
            IF g_aaz.aaz77 = 'Y' THEN 
               LET l_h1 = g_mai02
            ELSE
               LET l_h1 = g_grPageHeader.title1
            END IF
            PRINTX l_h1

            IF sr1.abb071 > 0 THEN 
               LET l_dc = '1'
            ELSE
               LET l_dc = '2'
            END IF
        ...
        AFTER GROUP OF sr1.l_i
            LET l_abb071_sum = GROUP SUM(sr1.abb071)
            PRINTX l_abb071_sum
            LET l_abb071_sum_fmt = cl_gr_numfmt('abb_file','abb07',g_azi04)
            PRINTX l_abb071_sum_fmt
