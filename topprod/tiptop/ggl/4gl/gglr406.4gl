...
#              "sign_show.type_file.chr1,sign_str.type_file.chr1000"    #是否顯示簽核資料(Y/N)  #TQC-C10039 #MOD-C20064 Mark TQC-C10039
   ...
   LET l_table = cl_prt_temptable('gglr406',g_sql) CLIPPED
...
#               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?, ?,?,?,?)"   #TQC-C10039 add 4?  #MOD-C20064 Mark TQC-C10039
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'   # If background job sw is off
      THEN CALL r406_tm(0,0)        # Input print condition
      ELSE CALL r406()              # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
END MAIN
 
FUNCTION r406_tm(p_row,p_col)
  ...
  DEFINE li_chk_bookno LIKE type_file.num5    #FUN-B20054
   ...
   OPEN WINDOW r406_w AT p_row,p_col WITH FORM "ggl/42f/gglr406"
...
#FUN-B20054--mark--str--
#   IF INT_FLAG THEN
#      LET INT_FLAG = 0 CLOSE WINDOW r406_w
#      EXIT PROGRAM
#   END IF
#FUN-B20054--mark--end--
#TQC-730072....end 還原位置
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_rep_clas CLIPPED,"'",         
                         " '",g_template CLIPPED,"'"         
         CALL cl_cmdat('gglr406',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW r406_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
      EXIT PROGRAM
...
#     LOCATE l_img_blob IN MEMORY    #TQC-C10039  #MOD-C20064 Mark TQC-C10039
     ...
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'gglr406'
     ...
     #No.FUN-7C0064  --Begin
     #CALL cl_outnam('gglr406') RETURNING l_name
     #LET g_len=168      #TQC-5B0102
     #FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR
     #START REPORT r406_rep TO l_name
     #No.FUN-7C0064  --End  
     ...
     FOR g_idx = 1 TO 8
         ...
         END IF
         #No.MOD-860252---end---
         PREPARE r406_prepare1 FROM l_sql
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('prepare:',SQLCA.sqlcode,1)
            CALL cl_used(g_prog,g_time,2) RETURNING g_time           #FUN-B80096   ADD
            EXIT PROGRAM
         END IF
         DECLARE r406_curs1 CURSOR FOR r406_prepare1
         LET g_pageno = 0
         FOREACH r406_curs1 INTO sr.*
              IF SQLCA.sqlcode != 0 THEN
...
#                      "",l_img_blob,"N",""    #TQC-C10039 add "",l_img_blob,"N",""  #MOD-C20064 Mark TQC-C10039
     ...
     LET g_str = g_ary[1].aag02,';',g_ary[2].aag02,';',g_ary[3].aag02,';',
                 ...
                 g_bal[1],';', g_bal[2],';', g_bal[3],';', g_bal[4],';',
                 g_bal[5],';', g_bal[6],';', g_bal[7],';', g_bal[8],';',
                 g_azi04
...
#     LET g_cr_apr_key_f = "aag01"             #報表主鍵欄位名稱  #TQC-C10039  #MOD-C20064 Mark TQC-C10039
     CALL cl_prt_cs3('gglr406','gglr406',g_sql,g_str)
     #No.FUN-7C0064  --End
