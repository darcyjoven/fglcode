...
# Modify.........: No.FUN-D40129 13/05/24 By yangtt 添加【列印條件】 

DATABASE ds
 
GLOBALS "../../config/top.global"
 
   DEFINE yymm  LIKE aab_file.aab02             #No.FUN-680107   #No.TQC-D40018
...
#No.FUN-7A0036--start--
   LET g_sql = "nma10.nma_file.nma10,",
               ...
               "l_detail.type_file.chr1,",  #No.FUN-D40020   Add
               "g_azi04.azi_file.azi04,",   #No.FUN-D40020   Add
               "g_azi05.azi_file.azi05"     #No.FUN-D40020   Add
   LET l_table = cl_prt_temptable('anmx363',g_sql) CLIPPED
   IF l_table=-1 THEN EXIT PROGRAM END IF
   LET g_sql="INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
              " VALUES(?,?,?,?,?,?,?,?,?,?, ?,?,?,?,? ,?,?,?)"   #No.FUN-D40020   Add 3 ?
...
#No.FUN-7A0036--end-- 
   ...
   IF cl_null(tm.wc) THEN
      CALL anmx363_tm(0,0)             # Input print condition
   ELSE 
      CALL anmx363()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
END MAIN
 
FUNCTION anmx363_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,     #No.FUN-680107 SMALLINT
   ...
   OPEN WINDOW anmx363_w AT p_row,p_col
        WITH FORM "anm/42f/anmx363"
   ...
   LET g_copies = '1'
   LET tm.detail='Y'
   CALL cl_set_comp_visible('detail',FALSE)   #No.FUN-D40020
...
WHILE TRUE
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW anmx363_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmx363',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW anmx363_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0082
   ...
   DEFINE l_nme12   LIKE nme_file.nme12              #CHI-830003 add   
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='anmx363'   #No.FUN-7A0036
     ...
     PREPARE anmx363_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
        EXIT PROGRAM
     END IF
     DECLARE anmx363_curs1 CURSOR FOR anmx363_prepare1
#    CALL cl_outnam('anmx363') RETURNING l_name            #No.FUN-7A0036 Mark
#    START REPORT anmx363_rep TO l_name                    #No.FUN-7A0036 Mark
 
#    LET g_pageno = 0                                      #No.FUN-7A0036 Mark
     FOREACH anmx363_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#                             "   AND convert(decimal(8,2),oox02) <= '",l_oox02,"'",     #TQC-9B0018 Mark
                              ...
                              "   AND oox04 = '0'",
                              "   AND oox041 = '0'"                             
                PREPARE x363_prepare7 FROM l_sql_2
                DECLARE x363_oox7 CURSOR FOR x363_prepare7
                OPEN x363_oox7
                FETCH x363_oox7 INTO l_count
                CLOSE x363_oox7                       
                IF l_count = 0 THEN
             ...
             IF l_count <> 0 THEN        
                PREPARE x363_prepare07 FROM l_sql_1
                DECLARE x363_oox07 CURSOR FOR x363_prepare07
                OPEN x363_oox07
                FETCH x363_oox07 INTO l_nme07
                CLOSE x363_oox07
             END IF              
          END WHILE                       
       END IF
...
#END FUN-550119
      ...
      #LET amt1 = cl_digcut(amt1,g_azi04)               #MOD-940049 add  #MOD-C60043 mark
       ...
       EXECUTE insert_prep USING                                                 #No.FUN-7A0036
         ...
         ,tm.detail,g_azi04,g_azi05   #No.FUN-D40020   Add
#      OUTPUT TO REPORT anmx363_rep(sr.*)                        #No.FUN-7A0036 Mark  
     END FOREACH
#No.FUN-7A0036--start--
###XtraGrid###     LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     IF g_zz05='Y' THEN
        ...
        LET g_str = tm.wc
     END IF
###XtraGrid###     LET g_str = g_str,";",tm.mm,";",tm.yy,";",g_azi04,";",g_azi05,";",tm.detail
###XtraGrid###     CALL cl_prt_cs3('anmx363','anmx363',g_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
    LET g_xgrid.order_field = "nma10"
    LET g_xgrid.grup_field = "nma10"
#   LET g_xgrid.condition= tm.wc  #FUN-D40129 mark
    LET g_xgrid.footerinfo1=cl_getmsg('afa-199',g_lang),tm.yy," / ",tm.mm
    LET g_xgrid.condition=cl_getmsg('lib-160',g_lang),tm.wc   #FUN-D40129
    CALL cl_xg_view()    ###XtraGrid###
#No.FUN-7A0036--end--    
#    FINISH REPORT anmx363_rep                                #No.FUN-7A0036 Mark
