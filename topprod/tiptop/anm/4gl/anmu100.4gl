...
#
# Pattern name...: anmu100.4gl
# Descriptions...: 應付票據資料清除作業
...
DEFINE
    ...
    LET p_row = 4
    LET p_col = 30 
    OPEN WINDOW u100_w AT p_row,p_col
		WITH FORM "anm/42f/anmu100" 
...
#	LET tm.more='Y'   #MOD-940337
	...
	LET l_jmp_flag='N'
     ...
     WHILE TRUE
	CONSTRUCT BY NAME tm.wc ON nmd07,nmd05,nmd13,nmd12 
	...
	INPUT BY NAME tm.download,tm.file_name,tm.file_extension,
      ...
      ON ACTION help          #MOD-4C0121
		...
		#		" '",tm.backup_device CLIPPED,"'"
				CALL cl_cmdat('anmu100',g_time,l_cmd)
			END IF
			CLOSE WINDOW u100_w
			CALL cl_used(g_prog,g_time,2) RETURNING g_time      #FUN-B30211
                        EXIT PROGRAM
...
DEFINE
	...
	IF tm.cleanup='Y' THEN
		...
		LET l_sql= " DELETE FROM nmd_file WHERE ",tm.wc  CLIPPED
		PREPARE u100_del  FROM l_sql
