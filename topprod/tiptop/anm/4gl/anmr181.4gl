...
# Modify.........: No.TQC-C20056 12/02/09 By yuhuabao 套表不需要添加簽核內容
...
MAIN
  ...
  #LET g_sql = "nmd07.nmd_file.nmd07,nmd05.nmd_file.nmd05,",    #MOD-870280 mark
   LET g_sql = "nmd07.type_file.chr10,nmd05.type_file.chr10,",  #MOD-870280
               ...
               "nt_note.type_file.chr1000,azi04.azi_file.azi04"  #No.TQC-C10034 add , , #No.TQC-C20056 del ,,
#No.TQC-C20056 ----- mark ----- begin
...
#No.TQC-C20056 ----- mark ----- end 
   LET l_table = cl_prt_temptable('anmr181',g_sql) CLIPPED
   IF l_table = -1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ? )"     #No.TQC-C10034 add 4? #No.TQC-C20056 del 4?
   ...
   #IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN	# If background job sw is off
      IF cl_null(tm.wc) THEN   #No.MOD-4B0281
         CALL anmr181_tm()	        	# Input print condition
      ELSE
	     ...
	     LET l_nna03 = tm.nna03[l_count+1,g_nna04] + 1
             CALL r181_nna03(g_nna04,l_count,l_nna03,l_tmp) RETURNING
                  l_nna03,l_tmp    #調整位數
            LET tm.nna03[1,l_count] = l_head
            LET tm.nna03[l_count+1,g_nna04] = l_tmp
         END FOR
         #-----END MOD-7C0027-----
         CALL anmr181()                           # Read data and create out-file
      END IF
   #END IF
...
FUNCTION anmr181_tm()
   ...
   OPEN WINDOW anmr181_w AT p_row,p_col
        WITH FORM "anm/42f/anmr181"
...
WHILE TRUE
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW anmr181_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
...
#    CALL FGL_DIALOG_SETBUFFER( tm.nmd03 )
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmr181',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW anmr181_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#No.TQC-C20056 ----- mark ----- end 
     #no.4849 只回寫到截止票號
     DROP TABLE r181_temp;
#No.FUN-680107 --start 
#    CREATE TEMP TABLE r181_temp
#    (seq       smallint,       #序號
...
#    );
     ...
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'anmr181'
#NO.CHI-6A0004--BEGIN
...
#NO.CHI-6A0004--END
   ...
   LET l_sql="SELECT nmw04,nmw05  FROM nmw_file ",        #支票簿
            ...
            # "   nmwacti = 'Y')"
            #CHI-A60021 mark --end--
   PREPARE r181_nmw_prepare FROM l_sql       # RUNTIME 編譯
   DECLARE r181_nmw_cs                       # SCROLL CURSOR
           SCROLL CURSOR WITH HOLD FOR r181_nmw_prepare
 ...
 #                 " WHERE nmd30 <> 'X' AND ",tm.wc CLIPPED,   #MOD-570383
                 ...
                 " ORDER BY nmd01 "
     PREPARE anmr181_prepare1 FROM l_sql
     IF STATUS != 0 THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
        EXIT PROGRAM 
     END IF
     DECLARE anmr181_curs1 CURSOR FOR anmr181_prepare1
#    CALL cl_outnam('anmr181') RETURNING l_name  #No.FUN-710085 mark
#No.TQC-6A0110 -- begin --
#   #TQC-650057---start---
...
#    START REPORT anmr181_rep TO l_name  #No.FUN-710085 mark
     FOREACH anmr181_curs1 INTO sr.*
       IF STATUS != 0 THEN
	  ...
	  #  LET l_nna03 = g_nna03[l_count+1,g_nna04] + 1
       ...
       OPEN r181_nmw_cs
       FETCH r181_nmw_cs INTO l_nmw04,l_nmw05
...
#         ,"",  l_img_blob,   "N",""      #No.TQC-C10034 add #No.TQC-C20056 mark
       #No.FUN-710085
 
#      OUTPUT TO REPORT anmr181_rep(sr.*)  #No.FUN-710085 mark
 
       #no.4849 只回寫到截止票號
       INSERT INTO r181_temp VALUES(g_cnt,sr.nmd01,g_nna03_o,g_nna03)
       IF STATUS THEN
#         CALL cl_err('ins tmp:',STATUS,1)   #No.FUN-660148
          CALL cl_err3("ins","r181_temp","","",STATUS,"","ins tmp:",1) #No.FUN-660148
          EXIT FOREACH
       END IF
       #no.4849 (end)
       ...
       LET g_cnt = g_cnt + 1
     END FOREACH
#    FINISH REPORT anmr181_rep                     #No.FUN-710085 mark 
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)   #No.FUN-710085 mark
   ...
   # CALL cl_prt_cs3('anmr181',l_sql,g_str)
     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
#    LET g_cr_table = l_table                 #主報表的temp table名稱        #No.TQC-C10034 add #No.TQC-C20056 mark
#    LET g_cr_apr_key_f = "nmd07"             #報表主鍵欄位名稱，用"|"隔開   #No.TQC-C10034 add #No.TQC-C20056 mark
     CALL cl_prt_cs3('anmr181','anmr181',l_sql,g_str)
     #No.FUN-710085 --end--
...
#No.TQC-950165 add by liuxqa 
           CALL cl_load_act_sys('anmr181')
           CALL cl_load_act_list('anmr181')
#No.TQC-950165 add by liuxqa 
        ...
        SELECT chkno INTO chkno_e FROM r181_temp
         WHERE seq = (SELECT MAX(seq) FROM r181_temp)
        LET chkno_l = chkno_e
        display 'kammy test:'
...
#No.tqc-950165 add --end--
        ...
        INPUT BY NAME chkno_l WITHOUT DEFAULTS
            AFTER FIELD chkno_l
               ...
               END IF
               SELECT seq INTO l_seq FROM r181_temp
                WHERE chkno = chkno_l
               IF STATUS THEN
#                 CALL cl_err('sel seq:',STATUS,0)   #No.FUN-660148
                  CALL cl_err3("sel","r181_temp",chkno_l,"",STATUS,"","sel seq:",0) #No.FUN-660148
                  NEXT FIELD chkno_l
               END IF
     	   ON IDLE g_idle_seconds
     	      CALL cl_on_idle()
      ...
      ON ACTION controlg      #MOD-4C0121
        ...
        DECLARE tmp_cs CURSOR FOR
          SELECT * FROM r181_temp WHERE seq <= l_seq
...
FUNCTION r181_nna03(l_nna04,l_count,l_nna03,l_tmp)
