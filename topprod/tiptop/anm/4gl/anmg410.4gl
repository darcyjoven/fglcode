...
###GENGRE###START
TYPE sr1_t RECORD
    gxe01 LIKE gxe_file.gxe01,
    gxe011 LIKE gxe_file.gxe011,
    gxe02 LIKE gxe_file.gxe02,
    gxe03 LIKE gxe_file.gxe03,
    gxe04 LIKE gxe_file.gxe04,
    gxe05 LIKE gxe_file.gxe05,
    gxe06 LIKE gxe_file.gxe06,
    gxe07 LIKE gxe_file.gxe07,
    gxe071 LIKE gxe_file.gxe071,
    gxe08 LIKE gxe_file.gxe08,
    gxe09 LIKE gxe_file.gxe09,
    gxe10 LIKE gxe_file.gxe10,
    gxe11 LIKE gxe_file.gxe11,
    gxe12 LIKE gxe_file.gxe12,
    gxe13 LIKE gxe_file.gxe13,
    gxe14 LIKE gxe_file.gxe14,
    gxe141 LIKE gxe_file.gxe141,
    gxe15 LIKE gxe_file.gxe15,
    gxe16 LIKE gxe_file.gxe16,
    gxe17 LIKE gxe_file.gxe17,
    gxe18 LIKE gxe_file.gxe18,
    alg02 LIKE alg_file.alg02,
    desc1 LIKE zaa_file.zaa08,
    l_alg021 LIKE alg_file.alg021,
    azi03 LIKE azi_file.azi04,
...
#No.FUN-710085--begin 
   ...
   LET l_table = cl_prt_temptable('anmg410',g_sql) CLIPPED 
   IF l_table = -1 THEN 
...
#No.FUN-710085--end 
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL anmg410_tm()	        	# Input print condition
      ELSE CALL anmg410()			# Read data and create out-file
   END IF
...
DEFINE p_row,p_col   LIKE type_file.num5,   #No.FUN-680107 SMALLINT
   ...
   OPEN WINDOW anmg410_w AT p_row,p_col
        WITH FORM "anm/42f/anmg410"
...
WHILE TRUE
 ...
 LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW anmg410_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      CALL cl_gre_drop_temptable(l_table)
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmg410',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW anmg410_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      CALL cl_gre_drop_temptable(l_table)
      EXIT PROGRAM
   END IF
...
#       l_time          LIKE type_file.chr8	    #No.FUN-6A0082
          ...
          sr            RECORD
                        ...
                        alg021_1 LIKE alg_file.alg021  #銀行全名 #FUN-C50007 add
                        END RECORD
...
#No.FUN-710085--begin                                                                                                               
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'anmg410'                                                                    
#No.FUN-710085--end 
     SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'anmg410'
#     IF g_len = 0 OR g_len IS NULL THEN LET g_len = 80 END IF
     LET g_len = 90                           #No.FUN-550057
     FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR
     
   # LET l_sql = "SELECT gxe_file.*,alg021,' ' ",   #FUN-C50007 MARK
     LET l_sql = "SELECT gxe_file.*,a.alg021,' ',b.alg021 ", #FUN-C50007
                #FUN-C50007--MOD--STR
                #"  FROM gxe_file,OUTER alg_file ",
                #" WHERE alg_file.alg01 = gxe_file.gxe07 ",
                #"   AND gxe13 ='Y' ",
                 "  FROM gxe_file LEFT OUTER JOIN alg_file a ON gxe07=a.alg01 ",
                 "  LEFT OUTER JOIN alg_file b ON gxe071=b.alg01 ",
                 "  WHERE gxe13 ='Y' ",
                 #FUN-C50007--MOD--END
                 "   AND ",tm.wc CLIPPED
     PREPARE anmg410_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
        CALL cl_gre_drop_temptable(l_table)
        EXIT PROGRAM 
     END IF
     DECLARE anmg410_curs1 CURSOR FOR anmg410_prepare1
 
#No.FUN-710085--begin 
#    CALL cl_outnam('anmg410') RETURNING l_name
#    START REPORT anmg410_rep TO l_name
 
#    LET g_pageno = 0
#No.FUN-710085--end
     FOREACH anmg410_curs1 INTO sr.*
...
#      END IF
     ...
     # SELECT alg021 INTO l_alg021 FROM alg_file  #FUN-C50007-MARK
     #  WHERE alg01 = sr.gxe.gxe071               #FUN-C50007-MARK
       EXECUTE insert_prep USING  sr.gxe.gxe01,sr.gxe.gxe011,sr.gxe.gxe02,sr.gxe.gxe03,sr.gxe.gxe04,sr.gxe.gxe05,
                                ...
                                # sr.gxe.gxe16,sr.gxe.gxe17,sr.gxe.gxe18,sr.alg021,sr.desc,l_alg021_1,  #FUN-C50007-mark
                                  sr.gxe.gxe16,sr.gxe.gxe17,sr.gxe.gxe18,sr.alg021,sr.desc,sr.alg021_1,  #FUN-C50007 add
                                  t_azi03,t_azi04,t_azi05,t_azi07_1,t_azi07_2,g_azi07 ,"",  l_img_blob,"N",""  # No.FUN-C40020 add  
 
#      OUTPUT TO REPORT anmg410_rep(sr.*)
#No.FUN-710085--end
     END FOREACH
 
#No.FUN-710085--begin
#    FINISH REPORT anmg410_rep
...
###GENGRE###     LET g_str = tm.wc         
   # CALL cl_prt_cs3('anmg410',g_sql,g_str)         #TQC-730088
###GENGRE###     CALL cl_prt_cs3('anmg410','anmg410',g_sql,g_str)  
    LET g_cr_table = l_table                    # No.FUN-C40020 add
    LET g_cr_apr_key_f = "gxe01"                    # No.FUN-C40020 add
    CALL anmg410_grdata()    ###GENGRE###
#No.FUN-710085--end
END FUNCTION
 
#No.FUN-710085--begin
#REPORT anmg410_rep(sr)
...
FUNCTION anmg410_grdata()
    ...
    PREPARE anmg410_precnt FROM l_sql
    EXECUTE anmg410_precnt INTO l_cnt
    IF SQLCA.SQLCODE THEN
        CALL cl_err('anmg410_precnt:', SQLCA.SQLCODE, 1)
        RETURN
    ELSE
        IF l_cnt <= 0 THEN
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("anmg410")
        IF handler IS NOT NULL THEN
            START REPORT anmg410_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY gxe01" 
            DECLARE anmg410_datacur1 CURSOR FROM l_sql
            FOREACH anmg410_datacur1 INTO sr1.*
                OUTPUT TO REPORT anmg410_rep(sr1.*)
            END FOREACH
            FINISH REPORT anmg410_rep
        END IF
...
REPORT anmg410_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno       LIKE type_file.num5
    DEFINE l_desc         STRING               #FUN-B50018
    #FUN-B50018----add-----str-----------------
    DEFINE l_gxe08_fmt    STRING    
    DEFINE l_gxe09_fmt    STRING
    DEFINE l_gxe10_fmt    STRING
    DEFINE l_gxe11_fmt    STRING
    #FUN-B50018----add-----end-----------------

    ORDER EXTERNAL BY sr1.gxe01
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            #FUN-B50018----add-----str-----------------
            LET l_gxe08_fmt = cl_gr_numfmt('gxe_file','gxe08',sr1.azi04)
            PRINTX l_gxe08_fmt
            LET l_gxe09_fmt = cl_gr_numfmt('gxe_file','gxe08',sr1.azi07_2)
            PRINTX l_gxe09_fmt
            LET l_gxe10_fmt = cl_gr_numfmt('gxe_file','gxe10',sr1.azi07_1)
            PRINTX l_gxe10_fmt
            LET l_gxe11_fmt = cl_gr_numfmt('gxe_file','gxe11',g_azi07)
            PRINTX l_gxe11_fmt
            IF sr1.gxe02 = '1' THEN
               LET  l_desc = cl_gr_getmsg("gre-042",g_lang,'1')
            ELSE
               LET  l_desc = cl_gr_getmsg("gre-042",g_lang,'0')
            END IF 
            PRINTX l_desc
            #FUN-B50018----add-----end-----------------
