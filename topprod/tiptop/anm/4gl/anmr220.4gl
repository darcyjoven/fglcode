# Prog. Version..: '5.30.06-13.03.20(00007)'     #
#
# Pattern name...: anmr220.4gl
# Descriptions...: 票據託收明細表作業
# Input parameter:
# Return code....:
# Date & Author..: 93/04/10 By Falicity
# Modify : 94/01/17 By Fiona (修改:nmh31-->nmh01, nmh25-->nmh01)
#                : 96/06/13 By Lynn   託貼銀行(nmh21)取6碼
# Modify.........: No.FUN-4C0098 04/12/29 By pengu 報表轉XML
# Modify.........: No.MOD-580242 05/09/12 By Nicola PAGE LENGTH g_line 改為g_page_line
# Modify.........: No.MOD-5A0248 05/10/21 By Smapmin 付款銀行簡稱改抓nmt02
# Modify.........: No.FUN-680107 06/08/28 By Hellen 欄位類型修改
# Modify.........: No.FUN-690117 06/10/16 By cheunl cl_used位置調整及EXIT PROGRAM后加cl_used
# Modify.........: No.CHI-6A0004 06/10/24 By yjkhero g_azixx(本幣取位)與t_azixx(原幣取位)變數定義問題修改
# Modify.........: No.FUN-6A0082 06/11/06 By dxfwo l_time轉g_time
# Modify.........: No.FUN-770038 07/08/04 By Carrier 報表轉Crystal Report格式
# Modify.........: No.TQC-830031 08/04/03 By Carol lc_cmd 型態改為type_file.chr1000
# Modify.........: No.CHI-8A0001 08/11/04 By tsai_yen 寫ora取代"只能使用相同群的資料"的MATCHES字串
# Modify.........: No.FUN-980030 09/08/31 By Hiko 加上GP5.2的相關設定
# Modify.........: No.TQC-CB0054 12/11/19 By yuhuabao 報表顯示添加票別說明欄位nmo02
# Modify.........: No:TQC-D20012 13/02/18 By xuxz 添加篩選票況為2.托收
  ...
  DEFINE tm  RECORD
		wc  	STRING,                  #TQC-630166
   		...
   		more	LIKE type_file.chr1    #No.FUN-680107 VARCHAR(1)
             END RECORD,
  l_orderA  ARRAY[3] OF LIKE zaa_file.zaa08, #No.FUN-680107 ARRAY[3] OF VARCHAR(16)
  g_tot_bal LIKE type_file.num20_6,		       #No.FUN-680107 DECIMAL(13,2)
  l_dash	  LIKE type_file.chr1000           #No.FUN-680107 VARCHAR(150)
...
#No.FUN-680107 --end
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'
      THEN CALL anmr220_tm(0,0)	
      ELSE CALL anmr220()		
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
END MAIN
 
FUNCTION anmr220_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01     #No.FUN-580031
   ...
   OPEN WINDOW anmr220_w AT p_row,p_col
        WITH FORM "anm/42f/anmr220"
...
WHILE TRUE
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW anmr220_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmr220',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW anmr220_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8	    #No.FUN-6A0082
          ...
          sr               RECORD order1 LIKE nmh_file.nmh01, #No.FUN-680107 VARCHAR(10)
                                  ...
                                  order3 LIKE nmh_file.nmh01, #No.FUN-680107 VARCHAR(10)
                                  ...
                                  nmo02 LIKE nmo_file.nmo02,  #TQC-CB0054 add
     ...
     LET l_sql = "SELECT nmh04, nmh05, nmh08, nmh09, nmh31, nmh10,nmo02,",   #TQC-CB0054 add nmo02
                 "       nmh03, nmh02, nmh07, nmh20, nmh21, nmh11, ",
                 "       nmh30, nmh24, nmh01, nmh29, azi04, azi05, ",
                 "       nma03, nmh06, nmt02 ",
                 "   FROM nmh_file LEFT OUTER JOIN nma_file ",   
                 "   ON ((nmh_file.nmh21 = nma_file.nma01)) ",   
                 "   LEFT OUTER JOIN azi_file  ",   
                 "   ON ((nmh_file.nmh03 = azi_file.azi01)) ",   
                 "   LEFT OUTER JOIN nmt_file ON ",   
                 "   ((nmh_file.nmh06 = nmt_file.nmt01)) ",  
                 "   LEFT OUTER JOIN nmo_file ON ",                #TQC-CB0054 add
                 "   ((nmh_file.nmh10 = nmo_file.nmo01)) ",        #TQC-CB0054 add
                 "   WHERE nmh38 <> 'x' ",       
                #TQC-D20012--add--str
                #添加篩選條件票況為2.托收
                 "   AND nmh24 = '2' ",
                #TQC-D20012--add--end  
                 "   AND ",tm.wc
#     PREPARE anmr220_prepare1 FROM l_sql
#     IF SQLCA.sqlcode != 0 THEN
#        CALL cl_err('prepare:',SQLCA.sqlcode,1)
#        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
#        EXIT PROGRAM
#     END IF
#     DECLARE anmr220_curs1 CURSOR FOR anmr220_prepare1
#
##    LET l_name = 'anmr220.out'
#     CALL cl_outnam('anmr220') RETURNING l_name
#     START REPORT anmr220_rep TO l_name
#
#     LET g_pageno = 0
#     FOREACH anmr220_curs1 INTO sr.*
#       IF SQLCA.sqlcode != 0 THEN CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH END IF
## 處理排列順序於列印時所需控制
#       FOR g_i = 1 TO 3
#          CASE WHEN tm.s[g_i,g_i] = '1' LET l_order[g_i] = sr.nmh04
# 							USING 'yyyymmdd'
#                                        LET l_orderA[g_i] = g_x[33]
#               WHEN tm.s[g_i,g_i] = '2' LET l_order[g_i] = sr.nmh05
# 							USING 'yyyymmdd'
#                                        LET l_orderA[g_i] = g_x[34]
#               WHEN tm.s[g_i,g_i] = '3' LET l_order[g_i] = sr.nmh09
# 							USING 'yyyymmdd'
#                                        LET l_orderA[g_i] = g_x[35]
#               WHEN tm.s[g_i,g_i] = '4' LET l_order[g_i] = sr.nmh11
#                                        LET l_orderA[g_i] = g_x[42]
#               WHEN tm.s[g_i,g_i] = '5' LET l_order[g_i] = sr.nmh01 #no.6821
#                                        LET l_orderA[g_i] = g_x[36]
#               WHEN tm.s[g_i,g_i] = '6' LET l_order[g_i] = sr.nmh10
#                                        LET l_orderA[g_i] = g_x[16]
#               WHEN tm.s[g_i,g_i] = '7' LET l_order[g_i] = sr.nmh29
#                                        LET l_orderA[g_i] = g_x[17]
#               WHEN tm.s[g_i,g_i] = '8' LET l_order[g_i] = sr.nmh24
#                                        LET l_orderA[g_i] = g_x[18]
#               WHEN tm.s[g_i,g_i] = '9' LET l_order[g_i] = sr.nmh20
# 							USING 'yyyymmdd'
#                                        LET l_orderA[g_i] = g_x[19]
#               WHEN tm.s[g_i,g_i] = 'A' LET l_order[g_i] = sr.nmh21
#                                        LET l_orderA[g_i] = g_x[20]
...
#       OUTPUT TO REPORT anmr220_rep(sr.*)
#     END FOREACH
#
#     FINISH REPORT anmr220_rep
#
#     CALL cl_prt(l_name,g_prtway,g_copies,g_len)
     #是否列印選擇條件
     IF g_zz05 = 'Y' THEN
        CALL cl_wcchp(tm.wc,'nmh04,nmh05,nmh09,nmh11,nmh01,nmh10,nmh29,nmh24,nmh20,nmh21')
             RETURNING g_str
     END IF
     LET g_str = g_str,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",tm.t,";",tm.u
     CALL cl_prt_cs1('anmr220','anmr220',l_sql,g_str)
     #No.FUN-770038  --End  
END FUNCTION
 
#No.FUN-770038  --Begin
#REPORT anmr220_rep(sr)
...
#          l_cnt3        LIKE type_file.num5,      #合計票據張數 order3  #No.FUN-680107 SMALLINT
#          l_cnt_tot     LIKE type_file.num5,      #No.FUN-680107 SMALLINT
##         l_nma02       LIKE nma_file.nma02,      #付款銀行簡稱 #MOD-5A0248
#          l_nmt02       LIKE nmt_file.nmt02,      #付款銀行簡稱 #MOD-5A0248
...
#
#  ORDER BY sr.nmh21,sr.order1,sr.order2,sr.order3,sr.nmh04
#  FORMAT
#   PAGE HEADER
...
#      LET l_last_sw = 'n'
#
#   BEFORE GROUP OF sr.nmh21
#      PRINT COLUMN g_c[33],g_x[10] CLIPPED,sr.nmh21,COLUMN g_c[35],sr.nma03,
#            COLUMN g_c[38],g_x[11] CLIPPED,COLUMN g_c[39],sr.nmh20
...
#      LET l_cnt = 0
#      PRINT COLUMN g_c[33],g_x[10] CLIPPED,sr.nmh21[1,6],COLUMN g_c[35],sr.nma03,
#            COLUMN g_c[38],g_x[11] CLIPPED,COLUMN g_c[39],sr.nmh20
#
#      PRINT g_dash[1,g_len]
#      PRINT COLUMN g_c[31],g_x[31] CLIPPED,
#            COLUMN g_c[32],g_x[32] CLIPPED,
#            COLUMN g_c[33],g_x[33] CLIPPED,
#            COLUMN g_c[34],g_x[34] CLIPPED,
#            COLUMN g_c[35],g_x[35] CLIPPED,
#            COLUMN g_c[36],g_x[36] CLIPPED,
#            COLUMN g_c[37],g_x[37] CLIPPED,
#            COLUMN g_c[38],g_x[38] CLIPPED,
#            COLUMN g_c[39],g_x[39] CLIPPED,
#            COLUMN g_c[40],g_x[40] CLIPPED,
#            COLUMN g_c[41],g_x[41] CLIPPED,
#            COLUMN g_c[42],g_x[42] CLIPPED,
#            COLUMN g_c[43],g_x[43] CLIPPED,
#            COLUMN g_c[44],g_x[44] CLIPPED
#      PRINT g_dash1
#
#      END IF
#
...
#      END IF
#
#   AFTER GROUP OF sr.nmh21
#      PRINT ' '
#
#   ON EVERY ROW
##      SELECT nma02 INTO l_nma02 FROM nma_file WHERE nma01=sr.nmh06   #MOD-5A0248
#      LET l_nmt02 =''   #MOD-5A0248
#      SELECT nmt02 INTO l_nmt02 FROM nmt_file WHERE nmt01=sr.nmh06   #MOD-5A0248
#      LET l_nmh05=sr.nmh05,'(+',sr.nmh08 USING "&",')'
#
#      LET l_cnt = l_cnt + 1
#      PRINT COLUMN g_c[31],l_cnt USING '####',
#            COLUMN g_c[32],sr.nmh10,
#            COLUMN g_c[33],sr.nmh04,
#            COLUMN g_c[34],l_nmh05,
#            COLUMN g_c[35],sr.nmh09,
#            COLUMN g_c[36],sr.nmh31,
#            COLUMN g_c[37],sr.nmh03,
#            COLUMN g_c[38],cl_numfor(sr.nmh02,38,sr.azi04) CLIPPED,
#            COLUMN g_c[39],sr.nmh06,
##            COLUMN g_c[40],l_nma02,   #MOD-5A0248
#            COLUMN g_c[40],l_nmt02,   #MOD-5A0248
#            COLUMN g_c[41],sr.nmh07[1,15],
#            COLUMN g_c[42],sr.nmh11 CLIPPED,
#            COLUMN g_c[43],sr.nmh30,
#            COLUMN g_c[44],sr.nmh01
#      LET l_cnt1 = l_cnt1 + 1
#      LET l_cnt2 = l_cnt2 + 1
#      LET l_cnt3 = l_cnt3 + 1
...
#         PRINT " "
#{*}      PRINT COLUMN g_c[33],l_orderA[1],COLUMN g_c[34],g_x[12] CLIPPED,COLUMN g_c[35],l_cnt1,
...
#         LET l_cnt1 = 0
#      ELSE IF tm.t[1,1] = 'Y'
#              THEN  PRINT g_dash2   #l_dash[1,g_len]
#              PRINT COLUMN (g_len-29),l_orderA[1],"  ",
#                    sr.order1, g_x[16] CLIPPED
#           END IF
#      END IF
#
...
#         LET l_amt = GROUP SUM(sr.nmh02)
#         SKIP 1 LINE
#         PRINT COLUMN g_c[33],l_orderA[1],COLUMN g_c[34],g_x[12] CLIPPED,COLUMN g_c[35],l_cnt2,
#               COLUMN g_c[36],g_x[14] CLIPPED;
#
#         #no.5195
...
#         LET l_amt = GROUP SUM(sr.nmh02)
#         SKIP 1 LINE
#         PRINT COLUMN g_c[33],l_orderA[1],COLUMN g_c[34],g_x[12] CLIPPED,COLUMN g_c[35],l_cnt3,
#               COLUMN g_c[36],g_x[14] CLIPPED;
#
#         #no.5195
...
#      PRINT
#      PRINT COLUMN g_c[34],g_x[13],COLUMN g_c[35],l_cnt_tot,COLUMN g_c[36],g_x[14] CLIPPED ;
#        #    15 SPACES,
#        #    cl_numfor(l_amt,15,sr.azi05) CLIPPED
#      #no.5195
