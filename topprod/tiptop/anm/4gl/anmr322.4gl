...
# Modify.........: No:MOD-D40043 13/04/09 By Lori 1.tm.wc/l_sql 改為 STRING
#                                                 2.參考 bal01 抓取條件改用 count 方式計數,若有存在 npg_file 時,才將 l_d1/l_c1/l_d2/l_c2 歸零
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD
             #wc      LIKE type_file.chr1000,#No.FUN-680107 VARCHAR(600)    #MOD-D40043 mark
              wc      STRING,                #MOD-D40043 add 
...
#No.FUN-7A0036--start--
   ...
   LET l_table = cl_prt_temptable('anmr322',g_sql) CLIPPED
   IF l_table=-1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?,?,?,?,?)"#TQC-CB0094 add ?
...
#No.FUN-A30106  --end  
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW r322_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
...
#     ON ACTION CONTROLP CALL r322_wc()      # Input detail Where Condition
    AFTER INPUT
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmr322',g_time,l_cmd)
      END IF
      CLOSE WINDOW r322_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0082
         #l_sql     LIKE type_file.chr1000,# RDSQL STATEMENT #No.FUN-680107 VARCHAR(1200)    #MOD-D40043 mark
          l_sql     STRING,                     #MOD-D40043 add
     ...
     DEFINE l_cnt     LIKE type_file.num5              #MOD-D40043 add
 
     CALL cl_del_data(l_table)                            #No.FUN-7A0036
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'anmr322'  #No.FUN-7A0036
...
#    LET l_name = 'anmr322.out'                      #No.FUN-7A0036   
#    CALL cl_outnam('anmr322') RETURNING l_name      #No.FUN-7A0036 
#    START REPORT r322_rep TO l_name                 #No.FUN-7A0036
      ...
      IF g_nmz.nmz20 = 'Y' THEN
         ...
         IF tm.date_sw = '2' THEN
            LET l_sql_3 = "SELECT nme12 FROM nme_file",
                          ...
                          " ORDER BY nme16 DESC"
         END IF         
         PREPARE r322_prepare08 FROM l_sql_3
         IF SQLCA.sqlcode != 0 THEN
...
#                            "   AND convert(decimal(8,2),oox02) <= '",l_oox02,"'",     #TQC-9B0018 Mark
                             ...
                             "   AND oox04 = '0'",
                             "   AND oox041 = '0'"                             
               PREPARE r322_prepare7 FROM l_sql_2
               ...
               IF l_count <> 0 THEN        
                  PREPARE r322_prepare07 FROM l_sql_1
...
#      OUTPUT TO REPORT r322_rep(sr.*)                              #No.FUN-7A0036
      #TQC-CB0094 add--str
       LET l_nmb02 = ''
...
#No.FUN-7A0036--start--
     ...
     ELSE
         LET tm.wc=""
     END IF
     LET g_str = tm.wc,";",g_azi04,";",tm.edate,";",tm.date_sw
     CALL cl_prt_cs3('anmr322','anmr322',g_sql,g_str) 
#    FINISH REPORT r322_rep                                         #No.FUN-7A0036 
