# Prog. Version..: '5.30.06-13.04.16(00007)'     #
#
# Pattern name...: anmr603.4gl
...
# Modify.........: No.FUN-590111 05/10/04 By Nicola gsb07,gsb17取消
...
# Modify.........: No.FUN-830145 08/04/21 By lutingting報表轉為使用CR 
# Modify.........: No.FUN-980030 09/08/31 By Hiko 加上GP5.2的相關設定
# Modify.........: No.MOD-CA0078 12/10/12 By Polly 調整建立temptable的gsa02欄位屬性/調整l_sql形態
...
DEFINE   g_sql      STRING           #No.FUN-830145
...
MAIN
   ...
   LET g_sql = "gsb05.gsb_file.gsb05,",
               "gsa02.gsa_file.gsa02,",      #MOD-CA0078 mod gsa_fiel.gsa02
               "gsb03.gsb_file.gsb03,",
               "gsb01.gsb_file.gsb01,",
               "gsb06.gsb_file.gsb06,",
               "gsb12.gsb_file.gsb12,",
               "gsb121.gsb_file.gsb121,",
               "gsb09.gsb_file.gsb09,",
               "l_gsb05_t.gsb_file.gsb05"
   LET l_table = cl_prt_temptable('anmr603',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL anmr603_tm()	        	# Input print condition
      ELSE CALL anmr603()			# Read data and create out-file
   END IF
...
DEFINE p_row,p_col LIKE type_file.num5,   #No.FUN-680107 SMALLINT
   ...
   OPEN WINDOW anmr603_w AT p_row,p_col
        WITH FORM "anm/42f/anmr603"
...
WHILE TRUE
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW anmr603_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmr603',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW anmr603_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8	    #No.FUN-6A0082
         #l_sql         LIKE type_file.chr1000,         #RDSQL STATEMENT #No.FUN-680107 VARCHAR(1200) #MOD-CA0078 mark
          l_sql         STRING,                         #MOD-CA0078 add
          ...
          sr            RECORD
                          ...
                          #gsb07    LIKE gsb_file.gsb07,#No.FUN-590111 Mark
                           ...
                           gsb121   LIKE gsb_file.gsb121,
                           gsb09    LIKE gsb_file.gsb09
                          #gsb17    LIKE gsb_file.gsb17 #No.FUN-590111 Mark
                        END RECORD
   DEFINE   l_gsb05_t   LIKE gsb_file.gsb05   #No.FUN-830145  
     
     CALL cl_del_data(l_table)     #No.FUN-830145
     
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'anmr603' #No.FUN-830145
...
#NO.CHI-6A0004--END
   ...
   LET l_sql = "SELECT gsb05,gsa02,gsb03,gsb01,gsb06,gsb12,gsb121,gsb09 ",   #No:590111
     ...
     PREPARE anmr603_prepare1 FROM l_sql
     ...
     END IF
     DECLARE anmr603_curs1 CURSOR FOR anmr603_prepare1
     #CALL cl_outnam('anmr603') RETURNING l_name   #No.FUN-830145
     #START REPORT anmr603_rep TO l_name           #No.FUN-830145
 
     LET g_pageno = 0
     FOREACH anmr603_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
       END IF
          #OUTPUT TO REPORT anmr603_rep(sr.*)  #No.FUN-830145
          #No.FUN-830145--start--         
          EXECUTE insert_prep USING
             sr.gsb05,sr.gsa02,sr.gsb03,sr.gsb01,sr.gsb06,sr.gsb12,
             sr.gsb121,sr.gsb09,l_gsb05_t
     ...
     CALL cl_prt_cs3('anmr603','anmr603',g_sql,g_str)
     #No.FUN-830145--end
     #FINISH REPORT anmr603_rep  #No.FUN-830145
     #CALL cl_prt(l_name,g_prtway,g_copies,g_len)  #No.FUN-830145
END FUNCTION
 
#No.FUN-830145--start--
#REPORT anmr603_rep(sr)
...
#            COLUMN g_c[37],cl_numfor(sr.gsb12,37,3),
#            COLUMN g_c[38],cl_numfor(sr.gsb121,38,g_azi05),
#            COLUMN g_c[39],cl_numfor(sr.gsb09,39,g_azi05)
