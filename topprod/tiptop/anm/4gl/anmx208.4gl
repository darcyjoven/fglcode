...
# Modify.........: No.MOD-BA0176 11/10/26 By Polly PREPARE anmx208_prepare2 增加抓取 npn02 給予 nmh25 判斷
# Modify.........: No:MOD-C10189 12/02/06 By Polly 取消 nmh24=1 的設定
# Modify.........: No:MOD-C70306 12/08/01 By Elise FETCH FIRST anmx208_curs2前先清空變數
# Modify.........: No:FUN-D30060 13/03/25 By wangrr CR轉Xtragrid
# Modify.........: No:FUN-D40129 13/04/15 By yangtt nmh06,nmh01,nmh11,nmh31新增開窗
...
MAIN
   ...
   LET l_table = cl_prt_temptable('anmx208',g_sql) CLIPPED
   IF l_table = -1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?,  ?,?,?,?,?, ?,?,?,?,?)"#MOD-870166 add ?#FUN-8B0027 add ?#FUN-960097 add ??? #FUN-D30060 add 2?
   ...
   LET tm.s  = ARG_VAL(10)
   LET tm.t  = ARG_VAL(11)
  #LET tm.u  = ARG_VAL(12)  #FUN-D30060 mark
   #No.FUN-570264 --start--
   LET g_rep_user = ARG_VAL(12)  #FUN-D30060 mod 13->12
   LET g_rep_clas = ARG_VAL(13)  #FUN-D30060 mod 14->13
   LET g_template = ARG_VAL(14)  #FUN-D30060 mod 15->14
   LET g_rpt_name = ARG_VAL(15)  #No.FUN-7C0078 #FUN-D30060 mod 16->15
   #No.FUN-570264 ---end---
  #FUN-A10098 -BEGIN-----
  ##FUN-8B0027--BEGIN-- 
  #LET tm.b =  ARG_VAL(17)
  ...
  ##FUN-8B0027--END-- 
   LET tm.type = ARG_VAL(16)  #FUN-D30060 mod 17->16
  #FUN-A10098 -END-------
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL anmx208_tm()	        	# Input print condition
      ELSE CALL anmx208()			# Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn       LIKE gbm_file.gbm01           #No.FUN-580031
   ...
   OPEN WINDOW anmx208_w AT p_row,p_col
        WITH FORM "anm/42f/anmx208"
   ...
   LET tm.p1=g_plant
 #No.FUN-A10098 -BEGIN-------
 # CALL x208_set_entry_1()
 # CALL x208_set_no_entry_1()
   CALL x208_set_comb()
 #No.FUN-A10098 -END-------
   #FUN-8B0027-End--#
   ...
   LET tm2.t1   = tm.t[1,1]
   LET tm2.t2   = tm.t[2,2]
  #LET tm2.u1   = tm.u[1,1] #FUN-D30060 mark
  #LET tm2.u2   = tm.u[2,2] #FUN-D30060 mark
...
WHILE TRUE
   CONSTRUCT BY NAME tm.wc ON nmh11,nmh06,nmh05,nmh01,nmh31,nmh26,nmh10,nmh29 #FUN-960097 add nmh26
    ...
    ON IDLE g_idle_seconds
     ...
     #FUN-960097   ----end      
         ...
         END IF
         IF INFIELD(nmh06) THEN
            CALL cl_init_qry_var()
            LET g_qryparam.state = "c"
            LET g_qryparam.form ="q_nmh06"
            CALL cl_create_qry() RETURNING g_qryparam.multiret
            DISPLAY g_qryparam.multiret TO nmh06
         END IF
         IF INFIELD(nmh11) THEN
            CALL cl_init_qry_var()
            LET g_qryparam.state = "c"
            LET g_qryparam.form ="q_nmh11"
            CALL cl_create_qry() RETURNING g_qryparam.multiret
            DISPLAY g_qryparam.multiret TO nmh11
         END IF
         IF INFIELD(nmh31) THEN
            CALL cl_init_qry_var()
            LET g_qryparam.state = "c"
            LET g_qryparam.form ="q_nmh31"
            CALL cl_create_qry() RETURNING g_qryparam.multiret
            DISPLAY g_qryparam.multiret TO nmh31
         END IF
         #FUN-D40129----add--end---
 ...
 LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond('nmhuser', 'nmhgrup') #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW anmx208_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
  ...
  #              tm.more
  #              WITHOUT DEFAULTS
  #INPUT BY NAME tm.edate,tm.type,tm2.s1,tm2.s2,tm2.t1,tm2.t2,tm2.u1,tm2.u2,tm.more #FUN-D30060 mark
   INPUT BY NAME tm.edate,tm.type,tm2.s1,tm2.s2,tm2.t1,tm2.t2,tm.more #FUN-D30060
                 WITHOUT DEFAULTS
  #No.FUN-A10098 -END-------
         #No.FUN-580031 --start--
         BEFORE INPUT
...
#   #          LET g_qryparam.form ="q_azp"    #No.FUN-940102
    ...
    AFTER INPUT
     ...
     #LET tm.u = tm2.u1,tm2.u2  #FUN-D30060 mark
  ...
  END INPUT
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmx208',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW anmx208_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#              "   AND nmh22=pmc_file.pmc01  ",   #MOD-5A0212
   ...
   PREPARE anmx208_prepare1 FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM 
   END IF
   DECLARE anmx208_curs1 CURSOR FOR anmx208_prepare1
   #No.FUN-780011  --Begin
   #CALL cl_outnam('anmx208') RETURNING l_name
   #START REPORT anmx208_rep TO l_name
   #LET g_pageno = 0
   #No.FUN-780011  --End  
 
   LET g_cnt    = 1
   FOREACH anmx208_curs1 INTO sr.*
     ...
     #LET l_sql2 = "SELECT npn03 ",                   #MOD-BA0176 mark
      LET l_sql2 = "SELECT npn02,npn03 ",             #MOD-BA0176 add
                 ...
                 # "       npn02 = (SELECT MAX(npn02) FROM ",l_dbs CLIPPED,"npn_file,",l_dbs CLIPPED,"npo_file ",  #FUN-8B0027 #No.FUN-A10098
                   ...
                   " npnconf = 'Y'",
                   " ORDER BY npn03 DESC"
      PREPARE anmx208_prepare2 FROM l_sql2
      DECLARE anmx208_curs2 SCROLL CURSOR FOR anmx208_prepare2
      OPEN anmx208_curs2
     #FETCH FIRST anmx208_curs2 INTO l_npn03              #MOD-BA0176 mark
      FETCH FIRST anmx208_curs2 INTO l_npn02,l_npn03      #MOD-BA0176 add
 ...
 #    IF sr.g_nmh.nmh24 MATCHES '[56789]' AND sr.g_nmh.nmh25 <= tm.edate THEN      #No:7015   #MOD-580071
      ...
      #OUTPUT TO REPORT anmx208_rep(sr.*)
      LET l_nmd24=''
      ...
      #  sr.g_nmh.nmh26,m_dbs[l_i1]   #FUN-8B0027 #FUN-960097 add nmh26 #No.FUN-A10098
        ...
        ,g_azi04,g_azi05   #FUN-D30060 add
      #No.FUN-780011  --End  
...
###XtraGrid###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
   ...
   IF g_zz05 = 'Y' THEN
      CALL cl_wcchp(tm.wc,'nmh11,nmh06,nmh05,nmh01,nmh31,nmh26,nmh10,nmh29') #FUN-960097 add nmh26 
           RETURNING g_str
   END IF
###XtraGrid###   LET g_str = g_str,";",tm.s[1,1],";",tm.s[2,2],";",tm.t,";",tm.u,";",
###XtraGrid###   #           g_azi04,";",g_azi05,";",tm.b     #FUN-8B0027 #No.FUN-A10098
###XtraGrid###               g_azi04,";",g_azi05,";",'N'      #No.FUN-A10098
###XtraGrid###   CALL cl_prt_cs3('anmx208','anmx208',g_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
#FUN-D30060--add--str--
    LET g_xgrid.order_field=cl_get_order_field(tm.s,"nmh11,nmh06,nmh05,nmh01,nmh31,nmh10,nmh29,nmh26")
    LET g_xgrid.grup_field=cl_get_order_field(tm.s,"nmh11,nmh06,nmh05,nmh01,nmh31,nmh10,nmh29,nmh26")
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"nmh11,nmh06,nmh05,nmh01,nmh31,nmh10,nmh29,nmh26")
...
#No.FUN-A10098 -BEGIN----- 
##FUN-8B0027--Begin--#
#FUNCTION x208_set_entry_1()
#   CALL cl_set_comp_entry("p1,p2,p3,p4,p5,p6,p7,p8",TRUE)
#END FUNCTION
#FUNCTION x208_set_no_entry_1()
#   IF tm.b = 'N' THEN
