...
#
# Pattern name...: anmr332.4gl
# Descriptions...: 科目別現金變動年統計表列印作業
# Date & Author..: 93/05/03  By  Felicity  Tseng
...
MAIN
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'
      THEN CALL r332_tm(0,0)
      ELSE CALL r332()
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
END MAIN
 
FUNCTION r332_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01     #No.FUN-580031
DEFINE p_row,p_col    LIKE type_file.num5,    #No.FUN-680107 SMALLINT
       l_cmd          LIKE type_file.chr1000  #No.FUN-680107 VARCHAR(400)
 
   LET p_row = 4 LET p_col = 12
   OPEN WINDOW r332_w AT p_row,p_col
        WITH FORM "anm/42f/anmr332"
   ...
   LET g_copies = '1'
WHILE TRUE
   CONSTRUCT BY NAME tm.wc ON nml01
         #No.FUN-580031 --start--
         BEFORE CONSTRUCT
             CALL cl_qbe_init()
         #No.FUN-580031 ---end---
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW r332_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmr332',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW r332_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0082
          ...
          sr               RECORD
                                  ...
                                  month LIKE nmp_file.nmp03,  #變動月份
     ...
     LET l_sql = "SELECT ",
                 " nml01,nml02,nma10,nma01,nma02,nmc03,SUM(nme04),0,azi05 ",
                 ...
                 " AND ",tm.wc CLIPPED,
                 " GROUP BY nml01, nml02, nma10, nma01, nma02, nmc03, azi05 "
...
#    LET l_sql = l_sql CLIPPED," ORDER BY apa01"
     ...
     CALL cl_outnam('anmr332') RETURNING l_name
     START REPORT r332_rep TO l_name
     PREPARE r332_prepare1 FROM l_sql
       IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('prepare:',SQLCA.sqlcode,1)
          CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
          EXIT PROGRAM
       END IF
       DECLARE r332_curs1 CURSOR FOR r332_prepare1
     FOR l_idx = 1 TO 12
         IF l_idx != 1 THEN
         ...
         LET g_pageno = 0
         LET g_counter = 0
         FOREACH r332_curs1 USING l_bdate,l_edate INTO sr.*
              IF SQLCA.sqlcode != 0 THEN
              ...
              OUTPUT TO REPORT r332_rep(sr.*)
         END FOREACH
     END FOR
 
     FINISH REPORT r332_rep
...
REPORT r332_rep(sr)
   DEFINE l_last_sw    LIKE type_file.chr1,                   #No.FUN-680107 VARCHAR(1)
      ...
      l_bank       LIKE type_file.chr50,        #No.FUN-680107 VARCHAR(40)
      l_items      LIKE zaa_file.zaa08,         #No.FUN-680107 VARCHAR(40)
      l_curr       LIKE zaa_file.zaa08,         #No.FUN-680107 VARCHAR(18)
      l_ary        ARRAY [20] OF LIKE type_file.num20_6 #No.FUN-680107 ARRAY[20] OF DEC(20,6)
  ...
  ORDER BY sr.nml01, sr.nma10, sr.nma01,sr.month,sr.nmc03
  FORMAT
   PAGE HEADER
   ...
   BEFORE GROUP OF sr.nml01   #現金變動碼
      ...
      END IF
      LET l_items=g_x[25] CLIPPED,sr.nml01,'   ',sr.nml02
      PRINT COLUMN g_c[31],l_items
      SKIP 1 LINE
 
   BEFORE GROUP OF sr.nma10        #幣別
      FOR l_idx = 1 TO 12          #月合計
          LET l_ary[l_idx] = 0
      END FOR
 
   BEFORE GROUP OF sr.nma01        #銀行
      LET l_gsum = 0               #年合計
      #96-06-14 Modify By Lynn
      LET l_bank=sr.nma01[1,8],'-',sr.nma02
      PRINT COLUMN g_c[31],l_bank;
 
   BEFORE GROUP OF sr.month
      LET l_mony = 0
   ...
   AFTER GROUP OF sr.nma01         #銀行編號
      PRINT COLUMN g_c[44], cl_numfor(l_gsum,44,sr.azi05) CLIPPED  #Print 年合計
 
   AFTER GROUP OF sr.nma10         #幣別
      IF tm.choice = 'Y' THEN
         LET g_counter = 0
      END IF
      LET l_curr=sr.nma10 CLIPPED,'   ',g_x[26] CLIPPED
      PRINT COLUMN g_c[31],l_curr;
      FOR l_idx = 1 TO 12
{T}       IF l_ary[l_idx] = 0 THEN
             LET l_ary[l_idx] = NULL
          END IF
      END FOR
      FOR l_idx = 1 TO 12          #Print 月合計
