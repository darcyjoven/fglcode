...
#No.FUN-710085--begin  
   LET g_sql = "nnk01.nnk_file.nnk01,nnk02.nnk_file.nnk02,",
               ...
               "nmd02.nmd_file.nmd02,Date1.type_file.dat,",
               ...
               "azi07_g.azi_file.azi07,",   #No.TQC-C10034 add , ,
   ...
   LET l_table = cl_prt_temptable('anmr750',g_sql) CLIPPED 
   IF l_table = -1 THEN EXIT PROGRAM END IF        
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ? ,?,?,?,?)"   #MOD-870306 add ?  #No.TQC-C10034 add 4?
...
#No.FUN-710085--end  
   ...
   #No.FUN-570264 ---end---
   IF cl_null(tm.wc)
      THEN CALL anmr750_tm(0,0)             # Input print condition
      ELSE
           CALL anmr750()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
END MAIN
 
FUNCTION anmr750_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,  #No.FUN-680107 SMALLINT
   ...
   END IF
   OPEN WINDOW anmr750_w AT p_row,p_col
        WITH FORM "anm/42f/anmr750"
################################################################################
...
WHILE TRUE
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW anmr750_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmr750',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW anmr750_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#No.FUN-710085--begin
  ...
  LOCATE l_img_blob IN MEMORY   #blob初始化   #Genero Version 2.21.02之後要先初始化 #No.TQC-C10034 add
    ...
    #清除暫存檔的資料
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'anmr750'
#No.FUN-710085--end
     SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
#No.FUN-580010  --begin
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'anmr750'
...
#No.FUN-580010  --end
     ...
     LET l_sql="SELECT * FROM nnk_file,nnl_file",
               " WHERE nnk01=nnl01 ",
               "   AND ",tm.wc CLIPPED
     IF tm.n='1' THEN
     ...
     END IF
     PREPARE anmr750_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
        EXIT PROGRAM 
     END IF
     DECLARE anmr750_curs1 CURSOR FOR anmr750_prepare1
 
#No.FUN-710085--begin
#    CALL cl_outnam('anmr750') RETURNING l_name
#    START REPORT anmr750_rep TO l_name
 
#    LET g_pageno = 0
#No.FUN-710085--end
     FOREACH anmr750_curs1 INTO sr.*,sr1.*
...
#No.FUN-710085--begin
      ...
      #IF SQLCA.sqlcode THEN LET l_nma02 = ' ' END IF    #MOD-930156
       ...
       EXECUTE insert_prep USING
          ...
          sr1.nnl17, l_nma02,  l_nma02_2,l_alg02,  l_nml02,
          l_nmd02,   l_Date1,  l_Date2,  l_rate,   t_azi03,
          t_azi04,   t_azi05,  t_azi07,  g_azi04,  g_azi05,   #MOD-870306 add t_azi07
          g_azi07,"",  l_img_blob,   "N",""      #No.TQC-C10034 add ,"",  l_img_blob,   "N",""
 
#      OUTPUT TO REPORT anmr750_rep(sr.*,sr1.*)
#No.FUN-710085--end
     END FOREACH
 
#No.FUN-710085--begin                                                                                                               
#    FINISH REPORT anmr750_rep                                                                                                      
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)                                                                                    
   ...
   # LET g_sql = "SELECT * FROM ",l_table CLIPPED #TQC-730088
     ...
     LET g_cr_apr_key_f = "nnk01"             #報表主鍵欄位名稱，用"|"隔開   #No.TQC-C10034 add
   # CALL cl_prt_cs3('anmr750',g_sql,g_str)   #TQC-73088
     CALL cl_prt_cs3('anmr750','anmr750',g_sql,g_str) 
#No.FUN-710085--end 
END FUNCTION
 
#No.FUN-710085--begin
#REPORT anmr750_rep(sr,sr1)
