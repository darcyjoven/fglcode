...
ELSE
   ...
   PREPARE p409_2_p4 FROM g_sql
      ...
      EXECUTE p409_2_p4 USING g_nmz.nmz02b               #No.FUN-D60110 Add
   IF SQLCA.sqlcode THEN
     #str CHI-C20017 mod
...
FUNCTION p409_t_1()
   ...
   ELSE
     ...
     #EXECUTE p409_2_p42 USING g_existno2,g_nmz.nmz02c   #No.FUN-D60110   Mark
      EXECUTE p409_2_p42 USING g_nmz.nmz02c   #No.FUN-D60110   Add
      IF SQLCA.sqlcode THEN
        #str CHI-C20017 mod
         #CALL cl_err('(del aba)',SQLCA.sqlcode,1) 
         #LET g_success = 'N'
...
FUNCTION p409_existno_chk()  
   ...
   IF l_cnt18 > 0 AND g_nmz.nmz52 = 'Y' THEN     
      LET g_sql="SELECT aba00,aba02,aba03,aba04,abapost,aba19,abaacti,aba20,aba01,aba06 ",   #MOD-G30031 add aba06 
                "  FROM ",cl_get_target_table(g_plant_new,'aba_file'), 
               #" WHERE aba00 = ? AND aba06='AC' AND ",tm.wc1 CLIPPED   #MOD-G30031 mark
                " WHERE aba00 = ? AND ",tm.wc1 CLIPPED                  #MOD-G30031 add
   ELSE
      LET g_sql="SELECT aba00,aba02,aba03,aba04,abapost,aba19,abaacti,aba20,aba01,aba06 ",   #MOD-G30031 add aba06  
                "  FROM ",cl_get_target_table(g_plant_new,'aba_file'), 
               #" WHERE aba00 = ? AND aba06='NM' AND ",tm.wc1 CLIPPED   #MOD-G30031 mark
                " WHERE aba00 = ? AND ",tm.wc1 CLIPPED                  #MOD-G30031 add
   END IF
   CALL cl_replace_sqldb(g_sql) RETURNING g_sql     
   CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql
   ...
   FOREACH p409_t_c1 INTO l_aba00,gl_date,gl_yy,gl_mm,l_abapost,l_aba19,
                        l_abaacti,l_aba20,l_aba01,l_aba06   #MOD-G30031 add aba06 
      IF STATUS THEN
         CALL s_errmsg('',l_aba01,'',STATUS,1)
         LET g_success = 'N'
      END IF
