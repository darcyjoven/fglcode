...
FUNCTION t930_cs()
   ...
   INITIALIZE g_nnw.* TO NULL    #No.FUN-750051
      CONSTRUCT BY NAME g_wc ON nnw01,nnw02,nnw03,nnw04,nnw05,nnw20,
                                ...
                                nnwacti,nnwuser,nnwgrup,nnworiu,nnworig,nnwmodu,nnwdate       #TQC-C40089   add  nnworiu,nnworig
...
FUNCTION t930_menu()
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         WHEN "output"
            IF cl_chk_act_auth() THEN
               IF NOT cl_null(g_wc) THEN
                  IF g_nnw00 = "1" THEN
                   # LET g_msg = 'anmr930 "',g_today,'" "',g_user,   #FUN-C30085 mark
                     LET g_msg = 'anmg930 "',g_today,'" "',g_user,   #FUN-C30085 add
                                 '" "',g_lang,'" ',' "Y" " " "1" "',g_wc,'" "N"'   #No.MOD-740397
                  ELSE
                    #LET g_msg = 'anmr940  "',g_today,'" "',g_user,  #FUN-C30085 mark
                     LET g_msg = 'anmg940  "',g_today,'" "',g_user,  #FUN-C30085 add
                                 '" "',g_lang,'" ',' "Y" " " "1" "',g_wc,'" "N"'   #No.MOD-740397
                  END IF
         ...
         WHEN "void"
             IF cl_chk_act_auth() THEN
                 #CALL t930_x() #FUN-D20035 mark
                 CALL t930_x(1) #FUN-D20035 add 
...
DEFINE li_result   LIKE type_file.num5        #No.FUN-550057  #No.FUN-680107 SMALLINT
   ...
   WHILE TRUE
      ...
      IF SQLCA.sqlcode THEN 
         CALL cl_err3("ins","nnw_file",g_nnw.nnw01,"",SQLCA.sqlcode,"","",1)  #No.FUN-660148  #No.FUN-B80067---調整至回滾事務前---
         ROLLBACK WORK
         CONTINUE WHILE
      ELSE
         COMMIT WORK
...
#==>撥出**********************************************
        ...
        AFTER FIELD nnw12 #撥出科目
            IF NOT cl_null(g_nnw.nnw12) THEN
                CALL t930_aag(g_nnw.nnw12,g_plant_out,'1') RETURNING g_aag02_1   #FUN-980020  #FUN-C10024 add '1'
                IF NOT cl_null(g_errno) THEN
        ...
        AFTER FIELD nnw121
            IF NOT cl_null(g_nnw.nnw121) THEN
                CALL t930_aag(g_nnw.nnw121,g_plant_out,'2') RETURNING g_aag02_4   #FUN-980020  #FUN-C10024 add '2'
                IF NOT cl_null(g_errno) THEN
...
#==>手續費*********************************************
        ...
        AFTER FIELD nnw19 #手續費科目
            IF NOT cl_null(g_nnw.nnw19) THEN
                CALL t930_aag(g_nnw.nnw19,g_plant_out,'1') RETURNING g_aag02_3   #FUN-980020  #FUN-C10024 add '1'
                IF NOT cl_null(g_errno) THEN
        ...
        AFTER FIELD nnw191
            IF NOT cl_null(g_nnw.nnw191) THEN
                CALL t930_aag(g_nnw.nnw191,g_plant_out,'2') RETURNING g_aag02_6  #FUN-980020   #FUN-C10024 add '2'
                IF NOT cl_null(g_errno) THEN
...
#==>撥入**********************************************
        ...
        AFTER FIELD nnw27 #撥入科目
            IF NOT cl_null(g_nnw.nnw27) THEN
                CALL t930_aag(g_nnw.nnw27,g_plant_in,'1') RETURNING g_aag02_2    #FUN-980020  #FUN-C10024 add '1'
                IF NOT cl_null(g_errno) THEN
        ...
        AFTER FIELD nnw271
            IF NOT cl_null(g_nnw.nnw271) THEN
                CALL t930_aag(g_nnw.nnw271,g_plant_in,'2') RETURNING g_aag02_5   #FUN-980020   #FUN-C10024 add '2'
                IF NOT cl_null(g_errno) THEN
  ...
  DEFINE  li_result    LIKE type_file.num5    #No.FUN-560002  #No.FUN-680107 SMALLINT
    ...
    DISPLAY BY NAME g_nnw.nnworiu,g_nnw.nnworig,
       ...
       ,g_nnw.nnwud01,g_nnw.nnwud02,g_nnw.nnwud03,g_nnw.nnwud04,
        ...
        CALL t930_aag(g_nnw.nnw27,g_plant_in,'1') RETURNING g_aag02_2  #幣別  #FUN-980020  #FUN-C10024 add '1'`
        DISPLAY g_aag02_2  TO FORMONLY.aag02_2
        IF g_aza.aza63 = 'Y' THEN
           CALL t930_aag(g_nnw.nnw271,g_plant_in,'2') RETURNING g_aag02_5     #FUN-980020 #FUN-C10024 add '2'
           DISPLAY g_aag02_5  TO FORMONLY.aag02_5
...
DEFINE
    ...
    CLOSE t930_bcl
    COMMIT WORK
    CALL t930_delHeader()     #CHI-C30002 add
 
END FUNCTION
 
#CHI-C30002 -------- add -------- begin
FUNCTION t930_delHeader()
   ...
   IF g_rec_b = 0 THEN
      #CHI-C80041---begin
      CALL s_get_doc_no(g_nnw.nnw01) RETURNING l_slip
      LET l_sql = " SELECT COUNT(*) FROM nnw_file ",
                  "  WHERE nnw01 LIKE '",l_slip,"%' ",
                  "    AND nnw01 > '",g_nnw.nnw01,"'"
      PREPARE t930_pb1 FROM l_sql 
      EXECUTE t930_pb1 INTO l_cnt 
   ...
   CASE WHEN SQLCA.SQLCODE = 100
        ...
        WHEN l_nma28 = "1" 
           IF g_aza.aza26!='2' THEN             #FUN-C80018
              LET g_errno = "aap-804"  #No.MOD-640330 add#該銀行存款種類有誤
           END IF                               #FUN-C80018
        OTHERWISE          LET g_errno = SQLCA.SQLCODE USING '-------'
   END CASE
   IF cl_null(g_errno) THEN
...
FUNCTION t930_aag(p_key,p_plant,p_flag)             #FUN-980020  #FUN-C10024 add p_flag
...
DEFINE  l_bookno     LIKE aza_file.aza81     #FUN-C10024 add
DEFINE  p_flag         LIKE type_file.chr1   #FUN-C10024 add
...
#FUN-C10024--add--str--
   IF p_flag = '1' THEN
      LET l_bookno = l_bookno1
   ELSE
      LET l_bookno = l_bookno2
   END IF
#FUN-C10024--add--end--
   ...
   LET l_sql = "SELECT aag02,aagacti,aag07,aag03,aag09",
               ...
               " WHERE aag01 = '",p_key,"'",
               #"   AND aag00 = '",l_bookno1,"'"  #No.FUN-730032  #FUN-C10024 mark
               "   AND aag00 = '",l_bookno,"'"      #FUN-C10024 add
...
FUNCTION t930_z() #when g_nnw.nnwconf='Y' (Turn to 'N')
  ...
  DEFINE l_aba19    LIKE aba_file.aba19  #No.FUN-670060
   ...
   IF g_nmy.nmydmy3 = 'Y' AND g_nmy.nmyglcr = 'Y' THEN
      ...
      IF NOT cl_null(g_nnw.nnw28) AND NOT cl_null(g_nnw.nnw29) THEN
         CALL cl_err(g_nnw.nnw28,'aap-929',1)
         RETURN
      END IF
  ...
  #IF g_nmy.nmyglcr = 'N' THEN    #CHI-C90052 mark
...
#anmt940----------------------------------------------
   ...
   DEFINE p_yn      LIKE type_file.chr1      #No.FUN-680107 VARCHAR(01) #Y:確認  N:取消確認
...
#FUNCTION t930_x() #FUN-D20035 mark
FUNCTION t930_x(p_type) 
   DEFINE p_type    LIKE type_file.chr1  #FUN-D20035 add  
    ...
    #FUN-D20035---begin 
    IF p_type = 1 THEN 
       IF g_nnw.nnwconf='X' THEN RETURN END IF
    ELSE
       IF g_nnw.nnwconf<>'X' THEN RETURN END IF
    END IF 
    #FUN-D20035---end     
