...
# Modify.........: No.TQC-D40039 13/04/17 By Polly 當nmh11為MISC時，l_occ02改抓nmh30
...
#No.FUN-710085--begin
   ...
   LET l_table = cl_prt_temptable('anmg250',g_sql) CLIPPED  
   IF l_table = -1 THEN 
...
#No.FUN-710085--end
   ...
   #No.FUN-570264 ---end---
   IF cl_null(tm.wc)
      THEN CALL anmg250_tm(0,0)             # Input print condition
      ELSE 
           CALL anmg250()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
   CALL cl_gre_drop_temptable(l_table)
END MAIN
 
FUNCTION anmg250_tm(p_row,p_col)
...
DEFINE comb_item      LIKE type_file.chr1000 #FUN-CC0093 add 
   ...
   OPEN WINDOW anmg250_w AT p_row,p_col
        WITH FORM "anm/42f/anmg250"
...
WHILE TRUE
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW anmg250_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      CALL cl_gre_drop_temptable(l_table)
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmg250',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW anmg250_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      CALL cl_gre_drop_temptable(l_table)
      EXIT PROGRAM
   END IF
...
#No.FUN-710085--begin
   ...
   DEFINE l_aag02_1         LIKE aag_file.aag02   #FUN-C50007
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'anmg250'
#No.FUN-710085--end
     SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
#No.FUN-580010  --begin
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'anmg250'
...
#NO.CHI-6A0004--END
     ...
     PREPARE anmg250_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
        CALL cl_gre_drop_temptable(l_table)
        EXIT PROGRAM 
     END IF
     DECLARE anmg250_curs1 CURSOR FOR anmg250_prepare1
 
#No.FUN-710085--begin
#    CALL cl_outnam('anmg250') RETURNING l_name
#    START REPORT anmg250_rep TO l_name
 
#    LET g_pageno = 0
#No.FUN-710085--end
     FOREACH anmg250_curs1 INTO sr.*,
...
#No.FUN-710085--begin
    ...
    #         WHERE azi01=sr.npn04  #原幣金額的小數位數依本張單據之幣別                     #FUN-C50007 MARK
     ...
     # SELECT pmc03 INTO l_pmc03 FROM pmc_file WHERE pmc01=sr.npn14         #FUN-C50007--MARK
       IF g_nmh.nmh11 != 'MISC' THEN                                        #TQC-D40039 add
          SELECT occ02 INTO l_occ02 FROM occ_file where occ01=g_nmh.nmh11
       ELSE                                                                 #TQC-D40039 add
          LET l_occ02 = g_nmh.nmh30                                         #TQC-D40039 add
       END IF                                                               #TQC-D40039 add
...
#      OUTPUT TO REPORT anmg250_rep(sr.*)
#No.FUN-710085--end  
     END FOREACH
 
#No.FUN-710085--begin
#    FINISH REPORT anmg250_rep
...
###GENGRE###     LET g_str = tm.wc
   # CALL cl_prt_cs3('anmg250',g_sql,g_str)        #TQC-730088
###GENGRE###     CALL cl_prt_cs3('anmg250','anmg250',g_sql,g_str)
    LET g_cr_table = l_table                    # No.FUN-C40020 add
    LET g_cr_apr_key_f = "npn01"                    # No.FUN-C40020 add
    CALL anmg250_grdata()    ###GENGRE###
...
FUNCTION anmg250_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("anmg250")
        IF handler IS NOT NULL THEN
            START REPORT anmg250_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY npn01,npo02"
          
            DECLARE anmg250_datacur1 CURSOR FROM l_sql
            FOREACH anmg250_datacur1 INTO sr1.*
                OUTPUT TO REPORT anmg250_rep(sr1.*)
            END FOREACH
            FINISH REPORT anmg250_rep
        END IF
