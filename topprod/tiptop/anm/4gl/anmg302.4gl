...
MAIN
   ...
   LET l_table = cl_prt_temptable('anmg302',g_sql) CLIPPED
   IF l_table = -1 THEN
   ...
   IF cl_null(tm.wc) THEN
      CALL anmg302_tm(0,0)             # Input print condition
   ELSE
      CALL anmg302()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
   CALL cl_gre_drop_temptable(l_table)
END MAIN
 
FUNCTION anmg302_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
DEFINE p_row,p_col    LIKE type_file.num5,  #No.FUN-680107 SMALLINT
       l_cmd          LIKE type_file.chr1000#No.FUN-680107 VARCHAR(400)
 
   LET p_row = 5 LET p_col = 15
   OPEN WINDOW anmg302_w AT p_row,p_col
        WITH FORM "anm/42f/anmg302"
...
WHILE TRUE
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW anmg302_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      CALL cl_gre_drop_temptable(l_table)
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmg302',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW anmg302_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      CALL cl_gre_drop_temptable(l_table)
      EXIT PROGRAM
   END IF
...
#No.FUN-580010  --begin
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'anmg302'
...
#NO.CHI-6A0004--END
     ...
     END IF
     PREPARE anmg302_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
        CALL cl_gre_drop_temptable(l_table)
        EXIT PROGRAM 
     END IF
     DECLARE anmg302_curs1 CURSOR FOR anmg302_prepare1
 
#    CALL cl_outnam('anmg302') RETURNING l_name  #No.FUN-710085 mark
#    START REPORT anmg302_rep TO l_name          #No.FUN-710085 mark
 
     LET g_pageno = 0
     FOREACH anmg302_curs1 INTO sr.*
...
#      OUTPUT TO REPORT anmg302_rep(sr.*)        #No.FUN-710085 mark
     END FOREACH
 
#    FINISH REPORT anmg302_rep                   #No.FUN-710085 mark
...
###GENGRE###     LET g_str = tm.wc,";",g_zz05
   ...
   # CALL cl_prt_cs3('anmg302',l_sql,g_str)
###GENGRE###     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
###GENGRE###     CALL cl_prt_cs3('anmg302','anmg302',l_sql,g_str)
    LET g_cr_table = l_table                    # No.FUN-C40020 add
    LET g_cr_apr_key_f = "nmg00"                    # No.FUN-C40020 add
    CALL anmg302_grdata()    ###GENGRE###
     #No.FUN-710085 --end--
 
END FUNCTION
 
#No.FUN-710085 --start-- mark
{REPORT anmg302_rep(sr)
...
FUNCTION anmg302_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("anmg302")
        IF handler IS NOT NULL THEN
            START REPORT anmg302_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY nmg00,npk01"
          
            DECLARE anmg302_datacur1 CURSOR FROM l_sql
            FOREACH anmg302_datacur1 INTO sr1.*
                OUTPUT TO REPORT anmg302_rep(sr1.*)
            END FOREACH
            FINISH REPORT anmg302_rep
        END IF
