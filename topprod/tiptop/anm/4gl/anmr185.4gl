...
# Modify.........: No.MOD-BC0250 11/12/24 By Polly 受款人列印nmd09長度由24調整為60
# Modify.........: No.TQC-C10034 12/01/18 By yuhuabao GP 5.3 CR報表列印TIPTOP與EasyFlow簽核圖片
# Modify.........: No.MOD-C20019 12/02/06 By Polly 給予g_nna021值
# Modify.........: No.FUN-C40036 12/07/23 By xujing GP 5.3 CR報表列印TIPTOP與EasyFlow簽核圖片 還原
...
MAIN
  ...
  #LET g_sql = "nmd01.nmd_file.nmd01,nmd02.nmd_file.nmd02,str.type_file.chr1000"                          #MOD-A60143 mark
   LET g_sql = "nmd01.nmd_file.nmd01,nmd02.nmd_file.nmd02,npy02.npy_file.npy02,str.type_file.chr1000"   #,",     #MOD-A60143 #No.TQC-C10034 add , ,#FUN-C40036 mark,",
#              "sign_type.type_file.chr1,",   #簽核方式                   #No.TQC-C10034 add  #FUN-C40036 mark
#              "sign_img.type_file.blob,",    #簽核圖檔                   #No.TQC-C10034 add  #FUN-C40036 mark
#              "sign_show.type_file.chr1,",   #是否顯示簽核資料(Y/N)      #No.TQC-C10034 add  #FUN-C40036 mark
#              "sign_str.type_file.chr1000"   #簽核字串                   #No.TQC-C10034 add  #FUN-C40036 mark
   
   LET l_table = cl_prt_temptable('anmr185',g_sql) CLIPPED
   IF l_table = -1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
#              " VALUES(?,?,?,? ,?,?,?,?)"   #No.TQC-C10034 add 4?           #MOD-A60143 add ? #FUN-C40036 mark
               " VALUES(?,?,?,?)"            #FUN-C40036
  ...
  #LET tm.r     = ARG_VAL(18)    #FUN-830027 add 選擇報表格式  #MOD-A10047 mark
      ...
      IF cl_null(tm.wc) THEN   #No.MOD-4B0281
         CALL anmr185_tm()	        	# Input print condition
      ELSE
         SELECT nna03,nna04,nna05,nna021
          ...
          WHERE nna01 = nma01
            ...
            #                AND nna02 = tm.nmd31)
            #CHI-A60021 mark --end--
         CALL anmr185()                        # Read data and create out-file
      END IF
...
FUNCTION anmr185_tm()
   ...
   OPEN WINDOW anmr185_w AT p_row,p_col
        WITH FORM "anm/42f/anmr185"
...
WHILE TRUE
 ...
 LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond('nmduser', 'nmdgrup') #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW anmr185_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
     ...
     AFTER FIELD nmd03   #銀行
      ...
      AFTER FIELD nmd31
          ...
          ELSE
             ...
             ELSE                           #MOD-C20019 add
                LET g_nna021 = tm.nna021    #MOD-C20019 add
             END IF
             #CHI-A60021 add --end--
	    ...
	    #       LET l_nna03 = tm.nna03[l_count+1,g_nna04] + 1
            #       CALL r185_nna03(g_nna04,l_count,l_nna03,l_tmp) RETURNING
            #            l_nna03,l_tmp    #調整位數
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmr185',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW anmr185_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#  DEFINE l_img_blob     LIKE type_file.blob                                         #No.TQC-C10034 add #FUN-C40036 mark
#  LOCATE l_img_blob IN MEMORY   #blob初始化   #Genero Version 2.21.02之後要先初始化 #No.TQC-C10034 add #FUN-C40036 mark
   ...
   END IF
     #只回寫到截止票號
     DROP TABLE r185_temp;
     CREATE TEMP TABLE r185_temp(
   ...
   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'anmr185'
   ...
   LET l_sql="SELECT nmw04,nmw05  FROM nmw_file ",        #支票簿
            ...
            # "   nmwacti = 'Y')"
            #CHI-A60021 mark --end--
   PREPARE r185_nmw_prepare FROM l_sql       # RUNTIME 編譯
   DECLARE r185_nmw_cs                       # SCROLL CURSOR
           SCROLL CURSOR WITH HOLD FOR r185_nmw_prepare
     ...
     LET l_sql = "SELECT nmd01,nmd02,nmd04,nmd26,nmd05,nmd07,nmd08,nmd09,nmd10, ",
                ...
                #" AND nmd30 <> 'X' ",                          #MOD-A70055 mark
                 ...
                 " ORDER BY nmd01 "
     PREPARE anmr185_prepare1 FROM l_sql
     IF STATUS != 0 THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
        EXIT PROGRAM 
     END IF
     DECLARE anmr185_curs1 CURSOR FOR anmr185_prepare1
    IF tm.r = '1' THEN
        CALL cl_outnam('anmr185') RETURNING l_name
    END IF
     ...
     IF tm.r = '1' THEN
        START REPORT anmr185_rep TO l_name  #No.FUN-710085 mark   #MOD-830042 取消mark
     END IF
 
     FOREACH anmr185_curs1 INTO sr.*
       IF STATUS != 0 THEN
       ...
       OPEN r185_nmw_cs
       FETCH r185_nmw_cs INTO l_nmw04,l_nmw05
...
#                                            ,"",  l_img_blob,   "N",""      #No.TQC-C10034 add #FUN-C40036 mark
                   LET l_str  = NULL
                   LET l_str2 = NULL
                END IF
                FOR i = 1 TO l_npy.npy04 
                    LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
                                " VALUES('",sr.nmd01,"','",sr.nmd02,"',",l_npy.npy02,",'')"    #MOD-A60143 
             ...
             CASE l_npy.npy03
               ...
               WHEN 'C' # 受款人
                    IF tm.a = 'Y' THEN
                      ...
                      #LET l_leng = 24 - FGL_WIDTH(sr.nmd09) #MOD-970233 #MOD-BC0250 mark
                       LET l_leng = 60 - FGL_WIDTH(sr.nmd09) #MOD-BC0250 
                       ...
                       ELSE
                          IF l_leng < 0 THEN                     #當欄位大於 24 時,依此長度切割 
                              ...
                              FOR l_i = 1 TO l_leng2                                          
                                 ...
                                 #IF l_leng > = 24 THEN   #MOD-BC0250 mark
                                  IF l_leng > = 60 THEN   #MOD-BC0250 
                                     EXIT FOR                                              
                                  END IF                                                       
                              END FOR                                                          
...
#                                         ,"",  l_img_blob,   "N",""      #No.TQC-C10034 add #FUN-C40036 mark
      ...
      END IF         #FUN-830027 add
       ...
       IF tm.r = '1' THEN    #FUN-830027 add 
          OUTPUT TO REPORT anmr185_rep(sr.*)    #No.FUN-710085   #MOD-830042 取消mark
       END IF   #MOD-960278 add
 
          #只回寫到截止票號
          INSERT INTO r185_temp VALUES(g_cnt,sr.nmd01,g_nna03_o,g_nna03)
          IF STATUS THEN
             CALL cl_err3("ins","r185_temp","","",STATUS,"","ins tmp:",1) #No.FUN-660148
             EXIT FOREACH
          END IF
       LET g_cnt = g_cnt + 1
     END FOREACH
     #zaa流程 
     IF tm.r = '1' THEN    
        FINISH REPORT anmr185_rep                    #No.FUN-710085 mark   #MOD-830042 取消mark
...
#       LET g_cr_table = l_table                 #主報表的temp table名稱        #No.TQC-C10034 add   #FUN-C40036 mark
#       LET g_cr_apr_key_f = "nmd01"             #報表主鍵欄位名稱，用"|"隔開   #No.TQC-C10034 add   #FUN-C40036 mark
        CALL cl_prt_cs3('anmr185','anmr185',l_sql,'')   #FUN-740023 modify
     END IF
  ...
  WHILE TRUE
     IF g_cnt > 1 THEN
        ...
        IF g_bgjob = 'Y' THEN
           ...
           CALL cl_load_act_sys('anmr185')
           CALL cl_load_act_list('anmr185')
        ...
        SELECT chkno INTO chkno_e FROM r185_temp
         WHERE seq = (SELECT MAX(seq) FROM r185_temp)
        LET chkno_l = chkno_e
        ...
        INPUT BY NAME chkno_l WITHOUT DEFAULTS
            AFTER FIELD chkno_l
               ...
               END IF
               SELECT seq INTO l_seq FROM r185_temp
                WHERE chkno = chkno_l
               IF STATUS THEN
                  CALL cl_err3("sel","r185_temp","","",STATUS,"","sel seq:",0) #No.FUN-660148
                  NEXT FIELD chkno_l
               END IF
     	   ON IDLE g_idle_seconds
     	      CALL cl_on_idle()
      ...
      ON ACTION controlg      #MOD-4C0121
        ...
        DECLARE tmp_cs CURSOR FOR
          SELECT * FROM r185_temp WHERE seq <= l_seq
                  ...
                  #-MOD-B10053-add-
                   ...
                   ELSE
                      IF l_leng < 0 THEN                     #當欄位大於 24 時,依此長度切割 
                         ...
                         #LET l_leng = 24 - FGL_WIDTH(l_fillin)   #MOD-BC0250 mark
                          LET l_leng = 60 - FGL_WIDTH(l_fillin)   #MOD-BC0250 
                          PRINT l_fillin ;
                          PRINT l_leng SPACES;  
                      END IF
                   END IF
...
FUNCTION r185_nna03(l_nna04,l_count,l_nna03,l_tmp)
