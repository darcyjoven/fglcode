...
# Modify.........: No.FUN-D40128 13/05/16 By wangrr "幣種,客戶編號,發票單號"增加開窗
# Modify.........: No.FUN-D40128 13/05/22 By yangtt 1、匯率採用用平均時，無法抓到月底重評價匯率（azj06->azj07）
#                                                   2、匯率採用用平均時，匯率為空，報錯且跳出程序
#                                                   3、匯率採用用平均時，欄位【rate_op】隱藏

DATABASE ds
 
GLOBALS "../../config/top.global"
 
   DEFINE yymm   	LIKE azj_file.azj02         #No.FUN-680107 VARCHAR(6)   #No.TQC-D40018
...
MAIN
   ...
   LET g_sql = "nmh03.nmh_file.nmh03,",
               ...
               "g_azi04.azi_file.azi04,",   #No.FUN-CC0093   Add
               "g_azi05.azi_file.azi05"     #No.FUN-CC0093   Add
 
   LET l_table = cl_prt_temptable('anmx263',g_sql) CLIPPED
   ...
   IF cl_null(tm.wc) THEN
      CALL anmx263_tm(0,0)             # Input print condition
   ELSE
      CALL anmx263()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
END MAIN
 
FUNCTION anmx263_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01        #No.FUN-580031
DEFINE p_row,p_col    LIKE type_file.num5,       #No.FUN-680107 SMALLINT
       l_cmd          LIKE type_file.chr1000     #No.FUN-680107 VARCHAR(400)
 
   LET p_row = 4 LET p_col = 10
   OPEN WINDOW anmx263_w AT p_row,p_col
        WITH FORM "anm/42f/anmx263"
   ...
   IF g_aza.aza19 = '1' THEN
      CALL cl_set_comp_visible("rate_op",FALSE)
   END IF
   #FUN-D40128---add---end--
   LET tm.more = 'N'
   LET g_pdate = g_today
...
WHILE TRUE
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW anmx263_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('anmx263',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW anmx263_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
      EXIT PROGRAM
   END IF
   CALL cl_wait()
   ...
   DEFINE l_nmh28   LIKE nmh_file.nmh28              #CHI-830003 add                    
     ...
     IF g_aza.aza19 = '1' THEN
        LET l_sql="SELECT nmh03,nmh04,nmh21,nmh01,nmh31,nmh11,nmh30,",
                 #"       nmh28,azj06,nmh02,nmh32,0,0,0",    #FUN-D40128 mark
                  "       nmh28,azj07,nmh02,nmh32,0,0,0",    #FUN-D40128 add
     ...
     PREPARE anmx263_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690117
        EXIT PROGRAM
     END IF
     DECLARE anmx263_curs1 CURSOR FOR anmx263_prepare1
 
     #No.FUN-780011  --Begin
     #CALL cl_outnam('anmx263') RETURNING l_name
     #START REPORT anmx263_rep TO l_name
     #LET g_pageno = 0
     #No.FUN-780011  --End  
 
     FOREACH anmx263_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#                            "   AND convert(decimal(8,2),oox02) <= '",l_oox02,"'",     #TQC-9B0018 Mark
                             ...
                             "   AND oox04 = '0'",
                             "   AND oox041 = '0'"                             
               PREPARE x263_prepare7 FROM l_sql_2
               DECLARE x263_oox7 CURSOR FOR x263_prepare7
               OPEN x263_oox7
               FETCH x263_oox7 INTO l_count
               CLOSE x263_oox7                       
               IF l_count = 0 THEN
            ...
            IF l_count <> 0 THEN        
               PREPARE x263_prepare07 FROM l_sql_1
               DECLARE x263_oox07 CURSOR FOR x263_prepare07
               OPEN x263_oox07
               FETCH x263_oox07 INTO l_nmh28
               CLOSE x263_oox07
            END IF              
         END WHILE                       
      END IF
      #CHI-830003--Add--End--#       
       ...
       #FUN-D40128---add--str--
       ELSE
          IF cl_null(sr.new_ex) OR sr.new_ex = 0 THEN
             CALL cl_err('','aoo-056',1)
             CONTINUE FOREACH
          END IF
       #FUN-D40128---add--end--
       END IF
      ...
      #CHI-830003--End--#       
       ...
       #No.FUN-780011  --Begin
       #OUTPUT TO REPORT anmx263_rep(sr.*)
       SELECT azi04,azi05,azi07
         INTO t_azi04,t_azi05,t_azi07    #NO.CHI-6A0004
         FROM azi_file
        WHERE azi01=sr.nmh03
       EXECUTE insert_prep USING sr.*,t_azi04,t_azi05,t_azi07,g_azi04,g_azi05   #No.FUN-CC0093   Add g_azi04,g_azi05
       #No.FUN-780011  --End  
     END FOREACH
 
     #No.FUN-780011  --Begin
     #FINISH REPORT anmx263_rep
...
###XtraGrid###     LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     ...
     IF g_zz05 = 'Y' THEN
        CALL cl_wcchp(tm.wc,'nmh03,nmh11,nmh01')
             RETURNING g_str
     END IF
###XtraGrid###     LET g_str = g_str,";",tm.edate,";",tm.detail,";",
###XtraGrid###                 g_azi04,";",g_azi05
###XtraGrid###     CALL cl_prt_cs3('anmx263','anmx263',g_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
    LET g_xgrid.order_field = "nmh11,nmh30,nmh04"
    LET g_xgrid.grup_field = "nmh03"
    CALL cl_xg_view()    ###XtraGrid###
     #No.FUN-780011  --End  
END FUNCTION
 
#No.FUN-780011  --Begin
#REPORT anmx263_rep(sr)
