...
# Pattern name...: axdr207.4gl
# Descriptions...: 集團調撥撥出單明細列印
...
# Modify.........: No:TQC-6A0095 06/11/13 By xumin 項次、申請項次問題更改
DATABASE ds
...
DEFINE l_cmd          LIKE type_file.chr1000       #No.FUN-680108 VARCHAR(1000)
   ...
   OPEN WINDOW r207_w AT p_row,p_col
        WITH FORM "axd/42f/axdr207"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No:FUN-580092 HCN
   ...
   LET g_copies = '1'
WHILE TRUE
   CONSTRUCT BY NAME tm.wc ON adf01,adf02,adf03,adf04
              HELP 1
 ...
 END CONSTRUCT
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axdr207',g_time,l_cmd)
      END IF
      CLOSE WINDOW r207_w
      EXIT PROGRAM
   END IF
...
DEFINE sr        RECORD
                     adf      RECORD LIKE adf_file.*,
                     adg      RECORD LIKE adg_file.*,
     ...
     IF tm.a = '1' THEN LET tm.wc = tm.wc CLIPPED," AND adf00 = '1'" END IF
     IF tm.a = '2' THEN LET tm.wc = tm.wc CLIPPED," AND adf00 = '2'" END IF
     IF tm.a = '3' THEN LET tm.wc = tm.wc CLIPPED," AND adf00 = '3'" END IF
     CASE
        WHEN tm.b = '0' LET tm.wc = tm.wc CLIPPED," AND adf10 = '0'"
        WHEN tm.b = '1' LET tm.wc = tm.wc CLIPPED," AND adf10 = '1'"
        WHEN tm.b = '2' LET tm.wc = tm.wc CLIPPED," AND adf10 = 'S'"
        WHEN tm.b = '3' LET tm.wc = tm.wc CLIPPED," AND adf10 = 'R'"
        WHEN tm.b = '4' LET tm.wc = tm.wc CLIPPED," AND adf10 = 'W'"
     END CASE

     LET l_sql = " SELECT adf_file.*,adg_file.*,gen02,",
                 "        gem02,ima02,ima021",
                 "   FROM adf_file,adg_file,OUTER ima_file,",
                 "        OUTER gen_file,OUTER gem_file ",
                 " WHERE adf01 = adg01 AND adf03 = gen_file.gen01 ",
                 "   AND adf04 = gem_file.gem01 ",
                 "   AND adg05 = ima_file.ima01 ",
                 "   AND ", tm.wc CLIPPED,
                 " ORDER BY adf01,adg02"
     PREPARE r207_prepare1 FROM l_sql
     IF SQLCA.sqlcode !=0 THEN
         CALL cl_err('prepare:',SQLCA.sqlcode,1)
         EXIT PROGRAM
     END IF
     DECLARE r207_curs1 CURSOR FOR r207_prepare1

     CALL cl_outnam('axdr207') RETURNING l_name
     START REPORT r207_rep TO l_name
     LET g_pageno = 0
     FOREACH r207_curs1 INTO sr.*
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
         END IF
         IF cl_null(sr.adg.adg12) THEN LET sr.adg.adg12 = 0 END IF
         IF cl_null(sr.adg.adg17) THEN LET sr.adg.adg17 = 0 END IF
         OUTPUT TO REPORT r207_rep(sr.*)
     END FOREACH
     FINISH REPORT r207_rep
...
DEFINE sr           RECORD
  ...
  ORDER BY sr.adf.adf01,sr.adg.adg02
  FORMAT
   PAGE HEADER
            ...
            PRINT g_x[31],g_x[32],g_x[33],g_x[34],g_x[35],
                  ...
                  g_x[56],g_x[57],g_x[58],g_x[59]
            PRINT g_dash1

      LET l_last_sw = 'n'

   BEFORE GROUP OF sr.adf.adf01
      CASE sr.adf.adf00
   ...
   ON EVERY ROW
      PRINT COLUMN g_c[31],sr.adf.adf01 CLIPPED,   #TQC-6A0095
            COLUMN g_c[32],sr.adf.adf02 CLIPPED,   #TQC-6A0095
            COLUMN g_c[33],sr.adf.adf03 CLIPPED,   #TQC-6A0095
            COLUMN g_c[34],sr.gen02 CLIPPED,       #TQC-6A0095
            COLUMN g_c[35],sr.adf.adf07 CLIPPED,   #TQC-6A0095
            COLUMN g_c[36],sr.adf.adf06 CLIPPED,   #TQC-6A0095
            COLUMN g_c[37],sr.adf.adf00 CLIPPED,   #TQC-6A0095
            COLUMN g_c[38],l_desc CLIPPED,         #TQC-6A0095
            COLUMN g_c[39],sr.adf.adf05 CLIPPED,   #TQC-6A0095
            COLUMN g_c[40],sr.adf.adf10 CLIPPED,   #TQC-6A0095
            COLUMN g_c[41],l_str CLIPPED,   #TQC-6A0095
    #        COLUMN g_c[42],sr.adg.adg02 USING "###&", #FUN-590118
            COLUMN g_c[42],cl_numfor(sr.adg.adg02,4,0),   #TQC-6A0095
            COLUMN g_c[43],sr.adg.adg03 CLIPPED,   #TQC-6A0095
   #         COLUMN g_c[44],sr.adg.adg04 USING "#######&", #FUN-590118
            COLUMN g_c[44],cl_numfor(sr.adg.adg04,8,0),   #TQC-6A0095
            COLUMN g_c[45],sr.adg.adg05 CLIPPED,   #TQC-6A0095
            COLUMN g_c[46],sr.ima02 CLIPPED,  #MOD-4A0238  #TQC-6A0095
            COLUMN g_c[47],sr.ima021 CLIPPED,  #MOD-4A0238   #TQC-6A0095
            COLUMN g_c[48],sr.adg.adg06 CLIPPED,  #TQC-6A0095
            COLUMN g_c[49],sr.adg.adg07 CLIPPED,  #TQC-6A0095
            COLUMN g_c[50],sr.adg.adg08 CLIPPED,  #TQC-6A0095
            COLUMN g_c[51],sr.adg.adg09 CLIPPED,  #TQC-6A0095
            COLUMN g_c[52],sr.adg.adg10 CLIPPED,  #TQC-6A0095
            COLUMN g_c[53],sr.adg.adg11 CLIPPED,  #TQC-6A0095
            COLUMN g_c[54],sr.adg.adg12 USING "----------&.&&&",
            COLUMN g_c[55],sr.adg.adg17 USING "----------&.&&&",
            COLUMN g_c[56],sr.adg.adg18 CLIPPED;  #TQC-6A0095
      CASE sr.adg.adg18
      ...
      END CASE
      PRINT COLUMN g_c[57],l_desc1 CLIPPED,  #TQC-6A0095
            COLUMN g_c[58],sr.adg.adg19 CLIPPED,  #TQC-6A0095
            COLUMN g_c[59],sr.adg.adg20 USING '###&' #FUN-590118
      PRINT
