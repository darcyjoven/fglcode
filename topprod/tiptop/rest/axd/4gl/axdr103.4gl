...
# Pattern name...: axdr103.4gl
# Descriptions...: 料件庫存數量表
# Date & Author..: 04/01/12 By Carrier
# Modify.........: No.MOD-4A0238 04/10/18 By Smapmin 放寬ima02
# Modify.........: No.MOD-4B0067 04/11/17 BY DAY 將變數用Like方式定義,報表拉成一行
...
DEFINE l_cmd          LIKE type_file.chr1000       #No.FUN-680108 VARCHAR(1000)
   ...
   OPEN WINDOW r103_w AT p_row,p_col
        WITH FORM "axd/42f/axdr103"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No:FUN-580092 HCN
   ...
   LET g_copies = '1'
WHILE TRUE
   CONSTRUCT BY NAME tm.wc ON adp02,adp03,adp01
              HELP 1 
      #No:FUN-580031 --start--
      BEFORE CONSTRUCT
          CALL cl_qbe_init()
 ...
 END CONSTRUCT                                                                  
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axdr103',g_time,l_cmd) 
      END IF
      CLOSE WINDOW r103_w
      EXIT PROGRAM
   END IF
...
DEFINE sr        RECORD 
     ...
     LET l_sql = " SELECT adp02,ima02,ima021,adp03,adp01,occ02,adp04,",
                 "        adp05,adp06,adp07",
                 " FROM adp_file,OUTER ima_file,OUTER occ_file ",
                 " WHERE ima_file.ima01 = adp02 ",
                 "   AND occ_file.occ01 = adp01 ",
                 "   AND ", tm.wc CLIPPED,
                 " ORDER BY adp02,adp03,adp01,adp04"
     ...
     CALL cl_outnam('axdr103') RETURNING l_name
...
DEFINE sr           RECORD 
  ...
  ORDER BY sr.adp02,sr.adp03,sr.adp01,sr.adp04
  FORMAT
   PAGE HEADER
