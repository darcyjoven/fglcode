...
# Modify.........: No:MOD-B80132 11/08/12 By Polly 1.增加判斷，若maj06 = '3'或maj06 = '1'，時，則年合計抓取整年>
#                                                  2.修正當oaj06=1時，判斷期別應小於tm.bm
# Modify.........: No.FUN-B90140 11/10/11 By minpp 增加checkbox"列印會計科目"
# Modify.........: No.MOD-BC0231 11/12/22 bY Polly 將maj08=0時，將科目餘額歸零的判斷mark 
# Modify.........: No:MOD-C30617 12/03/13 By Polly 查詢條件後需再重新給予 INPUT 欄位變數
# Modify.........: No:MOD-C50003 12/05/02 By wujie 本年利润查不到 
# Modify.........: No.TQC-C50042 12/05/07 By zhangweib 修改開窗q_mai去除報表性質為5\6的資料
# Modify.........: No.MOD-C60128 12/06/15 By Polly 調整本期損並計算條件
# Modify.........: No.TQC-C70061 12/07/09 By Polly 調整減項計算
# Modify.........: No.MOD-C70093 12/07/09 By yinhy 調整數據顯示方式
# Modify.........: No:CHI-C70031 12/08/23 By chenying 去除CE、CA憑證資料，否則月末結轉損益後，查詢不到損益類科目
# Modify.........: No.FUN-D40020 12/11/06 By wangrr CR轉XtraGrid,年度期別給默認值
# Modify.........: No.FUN-D40128 13/05/15 By wangrr 報表中當tm.acc_code ='Y'時顯示"科目編號",當tm.rtype='2'時顯示"年合計"
...
MAIN
   ...
   #No.FUN-780058---Begin                                                       
   LET g_sql = "maj31.maj_file.maj31,",  #FUN-B90140 add maj31
               ...
               "e.type_file.num5,    ",    #FUN-D40020 #小數位數
               "flag1.type_file.chr1 "     #FUN-D40020 #小數位數
                                                                 
   LET l_table = cl_prt_temptable('aglx115',g_sql) CLIPPED   # 產生Temp Table   
   IF l_table = -1 THEN EXIT PROGRAM END IF                  # Temp Table產生   
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,#No.TQC-820008 
               " VALUES(?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ? )"   #FUN-B90140 add ? #FUN-D40020 add 2 '?'                                   
...
FUNCTION x115_tm()
   ...
   OPEN WINDOW x115_w AT p_row,p_col WITH FORM "agl/42f/aglx115"
...
#  LET tm.b = g_bookno      #No.FUN-740020
   ...
   LET tm.h = 'N'
   LET tm.o = 'N'
   CALL cl_set_comp_entry('p,q',FALSE)   #No.FUN-D40020   Add
   LET tm.p = g_aaa03
   LET tm.q = 1
   LET tm.r = g_aaa03
   ...
   LET g_bgjob = 'N'
   LET g_copies = '1'
   LET tm.yy = YEAR(g_today)  #FUN-D40020
   LET tm.em = MONTH(g_today) #FUN-D40020 
   WHILE TRUE
...
#No.TQC-720032 -- end --
         ...
         AFTER FIELD o
           ...
           #No.FUN-D40020 ---end  --- Mod
            ...
            ELSE                                     #No.FUN-D40020   Add
               CALL cl_set_comp_entry('p,q',TRUE)    #No.FUN-D40020   Add
            END IF
...
#              CALL cl_err(tm.p,'agl-109',0)    #No.FUN-660123
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglx115',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#No.FUN-A10098 ----mark end
   ...
   LET l_sql = "SELECT * FROM maj_file",
               ...
               "   AND ",g_msg CLIPPED,
               " ORDER BY maj02"
   PREPARE x115_p FROM l_sql
   IF STATUS THEN
      CALL cl_err('prepare:',STATUS,1)   
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE x115_c CURSOR FOR x115_p
   ...
   #CALL cl_outnam('aglx115') RETURNING l_name        #No.FUN-780058
   #START REPORT x115_rep TO l_name                   #No.FUN-780058
 
   FOREACH x115_c INTO maj.*
      IF STATUS THEN
                  ...
                  IF maj.maj24 IS NULL THEN                                     
                     SELECT SUM(aah04-aah05) INTO amt                           
                      ...
                      WHERE aah00 = tm.b                                        
                        ...
                        AND aah01 BETWEEN maj.maj21 AND maj.maj22               
                        AND aah02 = tm.yy                                       
                       #AND aah03 <= i                           #MOD-B80132 mark
                        AND aah03 < tm.bm                        #MOD-B80132 add
                        AND aah01 = aag01                                       
                       #AND aag07 MATCHES '[23]'                 #TQC-AB0066 mark  
                        AND aag07 IN ('2','3')                   #TQC-AB0066
                  ELSE                                                          
                     SELECT SUM(aao05-aao06) INTO amt                           
                      ...
                      WHERE aao00 = tm.b                                        
                        ...
                        AND aao02 BETWEEN maj.maj24 AND maj.maj25               
                        AND aao03 = tm.yy
                       #AND aao04 <= i                           #MOD-B80132 mark
                        AND aao04 < tm.bm                        #MOD-B80132 add
                        AND aao01 = aag01                                       
                       #AND aag07 MATCHES '[23]'                 #TQC-AB0066 mark  
                        AND aag07 IN ('2','3')                   #TQC-AB0066
                  END IF                                                        
...
#                          "   AND abb03 = aag01 ",
#                          "   AND aag03 <> '4' ",
#                          "   AND aag04 = '2' "                #MOD-C60128 add
#              PREPARE x115_prepare3 FROM l_sql
#              DECLARE x115_c3  CURSOR FOR x115_prepare3
#              OPEN x115_c3
#              FETCH x115_c3 INTO l_endy1
##No.FUN-A10098 ----mark start
##              LET l_sql = "SELECT SUM(abb07)",
##                          "  FROM ",l_dbs CLIPPED," abb_file,",
##                                    l_dbs CLIPPED," aba_file,",
...
#                          "   AND aag03 <> '4' ",
#                          "   AND aag04 = '2' "                #MOD-C60128 add
#              PREPARE x115_prepare4 FROM l_sql
#              DECLARE x115_c4  CURSOR FOR x115_prepare4
#              OPEN x115_c4
#              FETCH x115_c4 INTO l_endy2
...
#CHI-C70031--add--end--
           ...
           #end MOD-930148 add
         END IF
       #No.MOD-C60112  --remark Begin
       #----------------MOD-C60128--------------------------mark
       ##No.MOD-C50003 --begi
        IF NOT cl_null(maj.maj21) THEN
           IF maj.maj06 MATCHES '[123]' AND maj.maj07 = '2' THEN
              LET amt = amt * -1
              #LET l_endy1 = l_endy1 * -1    #MOD-C70093
              #LET l_endy2 = l_endy2 * -1    #MOD-C70093
           END IF
           ...
           IF maj.maj06 = '5' AND maj.maj07 = '1' THEN
              ...
              #LET l_endy2 = l_endy2 * -1    #MOD-C70093
           END IF
           #LET amt = amt - (l_endy1 - l_endy2)  #MOD-C70093
        END IF
       ##No.MOD-C50003 --end  
       #----------------MOD-C60128--------------------------mark
       #No.MOD-C60112  --remark End
     ...
     #IF (tm.c='N' OR maj.maj03='2') AND maj.maj03 MATCHES "[012]"    #MOD-880120 mark
      ...
      IF maj.maj06 NOT MATCHES "[13]" THEN                                           #MOD-B80132 add
         LET sr.bal13=bal[1]+bal[2]+bal[3]+bal[4]+bal[5]+bal[6]+   #MOD-B60201 mark  #MOD-B80132 remark
                      bal[7]+bal[8]+bal[9]+bal[10]+bal[11]+bal[12] #MOD-B60201 mark  #MOD-B80132 remark
      ELSE                                                         #MOD-B80132 add
         LET sr.bal13=bal[tm.em]    #MOD-B60201  #MOD-B70132 mod 12 -> tm.em
      END IF                                                       #MOD-B80132 add
    ...
    #----------------MOD-C60128--------------------------remark
     #No.MOD-C50003 --begin
     ##No.FUN-780058---Begin
       #IF maj.maj07='2' THEN                      #MOD-C60112 mark                            
       IF maj.maj07='2' AND maj.maj09='-' THEN     #MOD-C60112                                              
 ...
 #FUN-D40020--add--str--
      LET sr.bal01=cl_digcut(sr.bal01,tm.e)
      LET sr.bal02=cl_digcut(sr.bal02,tm.e)
      LET sr.bal03=cl_digcut(sr.bal03,tm.e)
      LET sr.bal04=cl_digcut(sr.bal04,tm.e)
      LET sr.bal05=cl_digcut(sr.bal05,tm.e)
      LET sr.bal06=cl_digcut(sr.bal06,tm.e)
      LET sr.bal07=cl_digcut(sr.bal07,tm.e)
      LET sr.bal08=cl_digcut(sr.bal08,tm.e)
      LET sr.bal09=cl_digcut(sr.bal09,tm.e)
      LET sr.bal10=cl_digcut(sr.bal10,tm.e)
      LET sr.bal11=cl_digcut(sr.bal11,tm.e)
      LET sr.bal12=cl_digcut(sr.bal12,tm.e)
      LET sr.bal13=cl_digcut(sr.bal13,tm.e) 
 #FUN-D40020--ad--end
      IF maj.maj04 = 0 THEN                                                     
         EXECUTE insert_prep USING                                              
            maj.maj31,  #FUN-B90140 add maj31
            ...
            sr.bal13,    
            '2',tm.e,tm.acc_code #FUN-D40020 add tm.e                                                                 
      ELSE                                                                      
         EXECUTE insert_prep USING                                              
            maj.maj31,  #FUN-B90140 add maj31
            maj.maj20,maj.maj20e,maj.maj02,maj.maj03,                           
         ...
         FOR i = 1 TO maj.maj04                                                 
            EXECUTE insert_prep USING                                           
               ...
               '0','0','0','0',                                                 
               '0',                                                 
               '1',tm.e,tm.acc_code #FUN-D40020 add tm.e                                                              
         END FOR                                                                
      END IF                                                       
      #OUTPUT TO REPORT x115_rep(maj.*, sr.*)
      #NO.FUN-780058---End
   END FOREACH
 
   #No.FUN-780058---Begin
   #FINISH REPORT x115_rep
 
   #CALL cl_prt(l_name,g_prtway,g_copies,g_len)
   IF tm.rtype = "1" THEN                                                       
      LET l_name = 'aglx115_1'                                                  
   ELSE                                                                         
      LET l_name = 'aglx115' 
   END IF 
###XtraGrid###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED             
###XtraGrid###   LET g_str = g_mai02,";",tm.a,";",tm.p,";",tm.d,";",tm.yy,";",
###XtraGrid###               tm.rtype,";",tm.e      
###XtraGrid###               ,";",tm.acc_code   #FUN-B90140   Add  
###XtraGrid### CALL cl_prt_cs3('aglx115',l_name,g_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
#FUN-D40020--add--str--
   LET g_xgrid.order_field = "maj02,line"
   LET g_xgrid.template = 'aglx115'
   IF NOT cl_null(g_mai02) THEN
      LET g_xgrid.title2=g_mai02
   END IF
   LET g_str=cl_getmsg('azz1150',g_lang),':',tm.p,'        '
   LET g_str=g_str,cl_getmsg('asm-011',g_lang),':',cl_gr_getmsg('gre-115',g_lang,tm.d)
   LET g_xgrid.headerinfo1=g_str
   LET g_xgrid.footerinfo1=cl_getmsg('agl-273',g_lang),tm.a
   LET g_xgrid.footerinfo2=cl_getmsg('agl-172',g_lang),':',tm.yy
   LET g_xgrid.footerinfo3=cl_getmsg('agl-276',g_lang)
#FUN-D40020--add--end
   #FUN-D40128--add--str--
   IF tm.acc_code='Y' THEN
      LET g_xgrid.visible_column = cl_xg_column_visible(g_xgrid.visible_column,"maj31","Y")
   ELSE
      LET g_xgrid.visible_column = cl_xg_column_visible(g_xgrid.visible_column,"maj31","N")
   END IF
   IF tm.rtype='1' THEN
      LET g_xgrid.visible_column = cl_xg_column_visible(g_xgrid.visible_column,"bal13","N")
   ELSE
      LET g_xgrid.visible_column = cl_xg_column_visible(g_xgrid.visible_column,"bal13","Y")
   END IF
   #FUN-D40128--add--end
