...
# Modify.........: No:FUN-D70095 13/08/15 BY fengmy 1.CE類憑證結轉科目部門統計檔，報表列印時應減回CE類憑證
...
#No.FUN-D70095  --Begin
DEFINE  g_bal_a     ARRAY[200,7] OF RECORD
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglr193',g_sql) CLIPPED
...
#           CALL cl_err(tm.p,'agl-109',0)
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglr193',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW r193_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
...
#  CALL cl_outnam('aglr193') RETURNING l_name        #No.FUN-780061
...
#  START REPORT r193_rep TO l_name                   #No.FUN-780061
...
#    FINISH REPORT r193_rep                          #No.FUN-780061
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)     #No.FUN-780061
     ...
     LET l_tit7 = "(",tm.yy7 USING '<<<<','/',tm.bm7 USING'&&',
                  '-',tm.em7 USING'&&',")"
     LET g_str = g_mai02,";",tm.a,";",tm.p,";",tm.d,";",tm.e,";",tm.title1,";",
                 tm.title2,";",tm.title3,";",tm.title4,";",tm.title5,";",
                 tm.title6,";",tm.title7,";",l_tit1,";",l_tit2,";",l_tit3,";",
                 l_tit4,";",l_tit5,";",l_tit6,";",l_tit7
     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     CALL cl_prt_cs3('aglr193','aglr193',l_sql,g_str)
     #No.FUN-780061 --end--
END FUNCTION 
...
FUNCTION r193_process(l_dept)
   ...
   DEFINE l_sql5          STRING         # FUN-D70095
   DEFINE l_chr     LIKE type_file.chr1                  #No.FUN-680098   VARCHAR(1) 
  ...
  DEFINE cr        RECORD     
                   ...
                   END RECORD
  #CHI-A40048 add --end--
  DEFINE l_sw      LIKE type_file.num5      #FUN-D70095 add
  DEFINE l_i       LIKE type_file.num5      #FUN-D70095 add     
  DEFINE l_maj08   LIKE maj_file.maj08      #FUN-D70095 add
  DEFINE l_CE_sum1    LIKE abb_file.abb07   #FUN-D70095 add
  DEFINE l_CE_sum2    LIKE abb_file.abb07   #FUN-D70095 add
  
   LET r_cnt = r_cnt + 1
   LET g_cnt=1  #FUN-D70095 add
 ...
 #                           ".aao_file,aag_file ",   #MOD-570228
              ...
              END IF
#FUN-D70095--------------------------begin
              LET l_sql5 = "SELECT SUM(abb07) FROM abb_file,aba_file ",
                           " WHERE aba00 = abb00 AND aba01 = abb01",
                           "   AND abb03 BETWEEN ? AND ?",
                           "   AND abb05 IN (",g_buf CLIPPED,")",
                           "   AND aba03 = ? ",
                           "   AND aba04 BETWEEN ? AND ? ",
                           "   AND aba06 = 'CE'  AND abb06 = ? ",
                           "   AND abb00 = ? ",
                           "   AND abapost = 'Y'"
              PREPARE r121_cesum FROM l_sql5
              ...
              OPEN r121_cesumc USING maj.maj21,maj.maj22,
               ...
               LET l_amt = l_amt - l_CE_sum1 + l_CE_sum2
#FUN-D70095--------------------------end
...
#FUN-D70095--------------------------begin
               IF maj.maj08 = '1' THEN
                  LET g_bal_a[g_cnt,g_i].maj02 = maj.maj02
                  LET g_bal_a[g_cnt,g_i].maj03 = maj.maj03
                  LET g_bal_a[g_cnt,g_i].bal1  = a_amt[g_i]                  
                  LET g_bal_a[g_cnt,g_i].maj08 = maj.maj08
                  LET g_bal_a[g_cnt,g_i].maj09 = maj.maj09
               ELSE
                  LET g_bal_a[g_cnt,g_i].maj02 = maj.maj02
                  LET g_bal_a[g_cnt,g_i].maj03 = maj.maj03
                  LET g_bal_a[g_cnt,g_i].maj08 = maj.maj08
                  ...
                  FOR l_i = g_cnt - 1 TO 1 STEP -1       
                      IF maj.maj08 <= g_bal_a[l_i,g_i].maj08 THEN
                         EXIT FOR
                      END IF
                      IF g_bal_a[l_i,g_i].maj03 NOT MATCHES "[012]" THEN
                         CONTINUE FOR
                      END IF                    
                      IF l_i = g_cnt - 1 THEN       
                         LET l_maj08 = g_bal_a[l_i,g_i].maj08
                      END IF
                      IF g_bal_a[l_i,g_i].maj09 = '+' THEN
                         LET l_sw = 1
                      ELSE
                         LET l_sw = -1
                      END IF
                      IF g_bal_a[l_i,g_i].maj08 >= l_maj08 THEN   
                         LET g_bal_a[g_cnt,g_i].bal1 = g_bal_a[g_cnt,g_i].bal1 + 
                             g_bal_a[l_i,g_i].bal1 * l_sw                         
                      END IF
                      IF g_bal_a[l_i,g_i].maj08 > l_maj08 THEN
                         LET l_maj08 = g_bal_a[l_i,g_i].maj08
                      END IF
          ...
          ELSE           
              LET g_bal_a[g_cnt,g_i].maj02 = maj.maj02
              LET g_bal_a[g_cnt,g_i].maj03 = maj.maj03
              LET g_bal_a[g_cnt,g_i].bal1  = 0             
              LET g_bal_a[g_cnt,g_i].maj08 = maj.maj08
              LET g_bal_a[g_cnt,g_i].maj09 = maj.maj09
          END IF
       END IF
       LET sr[g_i] = g_bal_a[g_cnt,g_i].bal1
       LET g_cnt = g_cnt + 1 
#FUN-D70095--------------------------end   
...
#FUN-D70095---------------mark-----------begin  
      ...
      END IF
#     OUTPUT TO REPORT r193_rep(l_dept,maj.*,sr[1],sr[2],sr[3], #No.FUN-780061
...
#130813
  ...
  IF (NOT cl_null(s_amt[1])) OR (NOT cl_null(s_amt[2])) OR (NOT cl_null(s_amt[3])) OR (NOT cl_null(s_amt[4])) 
     ...
     LET l_sql = "SELECT maj02,amt01,amt02,amt03,amt04,amt05,amt06,amt07,line",
                 ...
                 " WHERE gem01='",l_dept,"'"
     PREPARE r193_crtmp_p FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time
     END IF
     DECLARE r193_crtmp_c CURSOR FOR r193_crtmp_p
     FOREACH r193_crtmp_c INTO cr.*
        IF s_amt[1] <> 0 THEN 
...
{REPORT r193_rep(r_gem01,maj,sr,r_cnt)
