...
#
# Pattern name...: aglr808.4gl
# Descriptions...: 科目異動碼明細表(aglr808)
#                : By TSD.Ken 080714
#                  copy from aglr810
#
...
#   l_table用于匯集存儲初始數據,與RECORD sr對應
   ...
   LET l_table = cl_prt_temptable('aglr808',g_sql) CLIPPED
...
#  臨時表字段的順序依據原報表的畫面欄位呈現順序
   ...
   LET l_table1 = cl_prt_temptable('aglr8081',g_sql) CLIPPED
...
#No.FUN-7C0038-------------END-------------
   ...
   LET l_table3 = cl_prt_temptable('aglr8081xxx',g_sql) CLIPPED
...
#   SELECT azi04,azi05 INTO g_azi04,g_azi05 FROM azi_file WHERE azi01=g_aza.aza17                 #No.CHI-6A0004 mark
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'  THEN # If background job sw is off
      CALL aglr808_tm(0,0)                    # Input print condition
   ELSE
      CALL aglr808()                          # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
END MAIN
 
FUNCTION aglr808_tm(p_row,p_col)
...
DEFINE li_chk_bookno  LIKE type_file.num5        #No.FUN-670005  #No.FUN-680098  smallint     
   ...
   END IF
   OPEN WINDOW aglr808_w AT p_row,p_col WITH FORM "agl/42f/aglr808"  #FUN-8A0105
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW aglr808_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglr808',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW aglr808_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
#NO.FUN-7C0038----------------END-----------
   ...
   PREPARE aglr808_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM 
   END IF
   DECLARE aglr808_curs1 CURSOR FOR aglr808_prepare1
...
,
   ...
   LET g_str = tm.wc,";",g_zz05,";",g_azi03,";",
               g_azi04,";",g_azi05,";",tm.e,";",tm.c,";",tm.d,";",
               tm.edate
 ...
 {#TSD.Ken 080815 Mark
  ...
  #    LET g_sql = "SELECT DISTINCT abi05, abi11,abi10 FROM ",g_cr_db_str CLIPPED,
   ...
   CALL cl_prt_cs3('aglr808','aglr808',g_sql,g_str)
END FUNCTION
