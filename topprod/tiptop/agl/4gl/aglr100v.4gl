# Prog. Version..: '5.30.04-12.11.13(00010)'     #
#
...
DEFINE        g_bal_a     DYNAMIC ARRAY OF RECORD
               ...
               END RECORD 
DEFINE g_sum         DYNAMIC ARRAY OF  LIKE type_file.num20_6   
MAIN
   OPTIONS
       INPUT NO WRAP
...
FUNCTION r100_page()
   ...
   LET l_cnt=(10-(g_no MOD 10))+g_no
   LET l_i = 0
   SELECT COUNT(*) INTO maj02_max FROM maj_file    #MOD-640583
     WHERE maj01 = tm.a 
   FOR g_i = 1 TO maj02_max LET g_sum[g_i] = 0 END FOR   
   FOR l_i = 10 TO l_cnt STEP 10
      ...
      IF l_i <= g_no THEN
         LET l_no = l_i - 10
         LET g_sql ="INSERT INTO dept1_tmp  SELECT dept_m,gem02,dept_d FROM                                  ",
                     "                       ( SELECT A.*, ROWNUM RN FROM (SELECT * FROM dept_tmp) A ",
                     "                          WHERE ROWNUM <= ",l_i," ) WHERE RN >= ",l_no+1," "
         PREPARE dept1 FROM g_sql
         EXECUTE dept1
         DISPLAY "process1        begin:",TIME
         CALL r100_process1()
         DISPLAY "process1        end  :",TIME
      ...
      ELSE
      	 ...
      	 IF (g_no MOD 10) = 0 THEN
      	    #fill maj值                   
            DECLARE r100_fill_maj02_cs1 CURSOR FOR  
             SELECT UNIQUE maj02,maj20,maj20e,maj03 FROM r100_tmp
              ORDER BY maj02
            LET l_maj02_cnt = 1
            FOREACH r100_fill_maj02_cs1 INTO l_maj_arr[l_maj02_cnt].*
                LET l_maj02_cnt = l_maj02_cnt + 1
            END FOREACH
            LET l_maj02_cnt = l_maj02_cnt - 1 
            FOR i = 1 TO l_maj02_cnt
                IF l_maj_arr[i].maj03 = '0' THEN  #NOT PRINT
                   CONTINUE FOR
                END IF
                IF cl_null(g_sum[i]) THEN LET g_sum[i] = 0 END IF
                LET l_amt[1]=g_sum[i]
                LET m_dept[1] = '合  計'
                EXECUTE insert_prep USING
                    #l_maj_arr[i].*,l_i,
                     l_maj_arr[i].maj02,l_maj_arr[i].maj03,'','','',
                    l_maj_arr[i].maj20,l_maj_arr[i].maj20e,l_i,
      	 ...
      	 ELSE      	 	  
            ...
            LET g_sql ="INSERT INTO dept1_tmp  SELECT dept_m,gem02,dept_d FROM                                  ",
                     "                       ( SELECT A.*, ROWNUM RN FROM (SELECT * FROM dept_tmp) A ",
                     "                          WHERE ROWNUM <= ",g_no," ) WHERE RN >= ",l_no+1," "
            PREPARE dept2 FROM g_sql
            EXECUTE dept2
            DISPLAY "process1        begin:",TIME
            CALL r100_process1()
            DISPLAY "process1        end  :",TIME
...
FUNCTION r100_dept_fill()
   ...
   IF g_abe01 = ' ' THEN
      IF g_abd01 = ' ' THEN
         ...
         IF tm.s = 'Y' THEN
            CALL r100_dept(tm.abe01,g_gem02,tm.abe01,'Y')
         END IF 
      ELSE 
      	 LET g_no=0                                   #--- 部門層級
         ...
         FOREACH r192_bom1 INTO l_abd02,l_chr,g_gem02
            ...
            LET g_no = g_no + 1
            IF tm.s = 'Y' THEN
               CALL r100_dept(l_abd02,g_gem02,l_abd02,l_chr)
            END IF
         END FOREACH
      END IF
   ELSE 
      ...
      FOREACH r192_bom2 INTO l_abe03,l_chr,g_gem02
         ...
         LET g_no = g_no + 1
         IF tm.s = 'Y' THEN
            CALL r100_dept(l_abe03,g_gem02,l_abe03,l_chr)
         END IF 
      END FOREACH
   END IF
END FUNCTION
...
FUNCTION r100_dept(l_dept_s,l_gem02,l_dept_d,l_sw)
    ...
    FOR l_cnt2 = 1 TO l_cnt1 - 1
        IF l_arr[l_cnt2].gem01 IS NOT NULL THEN
           CALL r100_dept(l_dept_s,l_gem02,l_arr[l_cnt2].*)
        END IF
    END FOR
    IF l_sw = 'Y' THEN 
...
FUNCTION r100_process1()
    DEFINE l_factor    LIKE type_file.num20_6
    DELETE FROM r100_tmp;
    DELETE FROM abb_tmp;
    
    INSERT INTO r100_tmp SELECT maj02,dept_m,maj20,maj20e,maj03,maj08,0,0,0,0,0,0
                           FROM mai_file,maj_file,dept1_tmp
                          WHERE mai01 = maj01
                            AND mai01 = tm.a
                            AND mai00 = tm.b

    LET g_sql = "UPDATE r100_tmp a SET d = ( SELECT NVL(SUM(aao05),0) FROM aao_file,aag_file,mai_file,maj_file b",
    ...
    LET g_sql = "UPDATE r100_tmp a SET c = ( SELECT NVL(SUM(aao06),0) FROM aao_file,aag_file,mai_file,maj_file b",
                " WHERE mai01 = b.maj01  AND mai01 = '",tm.a,"' AND mai00 = '",tm.b,"' ",
                "   AND b.maj02 = a.maj02 ",
                "   AND aao00 = '",tm.b,"'  AND aao01 BETWEEN maj21 AND maj22 ",
                "   AND aao03 =  ",tm.yy,"  AND aao04 BETWEEN  ",tm.bm," AND ",tm.em,
    ...
    LET g_sql = " UPDATE r100_tmp a SET d_ce = (SELECT NVL(SUM(abb07),0) FROM abb_tmp,maj_file b,mai_file ",
                "                 WHERE abb06 = '1'  ",
                "                   AND mai01 = b.maj01 ",
                "                   AND mai01 = '",tm.a,"'",
                "                   AND mai00 = '",tm.b,"'",
                ...
                "              )       "
     PREPARE p1 FROM g_sql
     EXECUTE p1

    LET g_sql = " UPDATE r100_tmp a SET c_ce = (SELECT NVL(SUM(abb07),0) FROM abb_tmp,maj_file b,mai_file ",
                "                  WHERE abb06 = '2'   ",
                "                    AND mai01 = b.maj01",
                "                    AND mai01 = '",tm.a,"'",
                "                    AND mai00 = '",tm.b,"'",
    ...
    UPDATE r100_tmp   SET sum = d - c - d_ce + c_ce ,amt = d - c - d_ce + c_ce
    ...
    UPDATE r100_tmp   SET sum = d  - d_ce ,amt = d  - d_ce
     WHERE maj02 IN ( SELECT maj02 FROM mai_file,maj_file
                         WHERE maj01 = mai01
                           AND mai00 = tm.b
                           AND maj01 = tm.a
    ...
    UPDATE r100_tmp   SET sum = c - c_ce ,amt = c - c_ce
     WHERE maj02 IN ( SELECT maj02 FROM mai_file,maj_file
                         WHERE maj01 = mai01
                           AND mai00 = tm.b
                           AND maj01 = tm.a
   ...
   IF tm.d MATCHES '[23]' THEN
      ...
      END IF
      LET g_sql = " UPDATE r100_tmp SET d    = d    / ",l_factor,",",
                                    "   c    = c    / ",l_factor,",",
                                    "   d_ce = d_ce / ",l_factor,",",
                                    "   c_ce = c_ce / ",l_factor,",",
                                    "   sum  = sum  / ",l_factor,",",
                                    "   amt  = amt  / ",l_factor
      PREPARE pp9 FROM g_sql
      EXECUTE pp9
   END IF
...
FUNCTION r100_process2()
   ...
   DECLARE r100_maj_cs CURSOR FOR
   ...
   LET g_cnt = 1 
   LET l_max_maj08 = 0
   FOREACH r100_maj_cs INTO l_maj.* 
        ...
        IF l_maj.maj06 MATCHES '[579]' AND l_maj.maj07 = '1' THEN
           LET g_sql = " UPDATE r100_tmp SET sum = -1 * sum , amt = -1 * amt  WHERE maj02 = ",l_maj.maj02  #fengmy add 130828
           PREPARE pp1 FROM g_sql
           EXECUTE pp1
        END IF
         
        IF l_maj.maj06 MATCHES '[123468]' AND l_maj.maj07 = '2' THEN
           LET g_sql = " UPDATE r100_tmp SET sum = -1 * sum , amt = -1 * amt  WHERE maj02 = ",l_maj.maj02  #fengmy add 130828
           PREPARE pp2 FROM g_sql
           EXECUTE pp2
        END IF
   ...
   IF l_max_maj08 > 1 THEN 
      LET g_sql = " UPDATE r100_tmp a SET a.amt  = (SELECT SUM(b.amt * d.sw) FROM r100_tmp b ,speed_tmp c ,sw_tmp d",
...
#select a.maj02,a.dept_m,SUM(b.amt * d.sw ) FROM TT1824328_r100_tmp a,TT1824328_r100_tmp b ,TT1824328_speed_tmp c,TT1824328_sw_tmp d
...
#SELECT a.maj02,a.dept_m,SUM(b.amt * d.sw) FROM TT1824007_r100_tmp b ,TT1824007_speed_tmp c ,TT1824007_sw_tmp d ,TT1824007_r100_tmp a
#WHERE a.dept_m = b.dept_m AND a.maj02 = c.maj02_d AND b.maj02 = c.maj02_s AND b.maj02 = d.maj02
#  AND a.maj02 <> c.maj02_s     
#    and a.dept_m = 'BJHFIN' 
#group by a.maj02,a.dept_m

      PREPARE pp3 FROM g_sql 
...
FUNCTION r100_cre_temp_table()
    ...
    CREATE INDEX r100_tmp_01 ON r100_tmp(maj02);
...
FUNCTION r100_fill_arr(l_i) 
   ...
   DEFINE l_i            LIKE type_file.num5
   #fill 部门值
   DECLARE r100_fill_dept_cs CURSOR FOR
    SELECT UNIQUE dept_m,gem02 FROM dept1_tmp
     ORDER BY dept_m
   LET l_dept_cnt = 1  
   FOREACH r100_fill_dept_cs INTO l_dept_arr[l_dept_cnt].*
      LET l_dept_cnt = l_dept_cnt + 1
   END FOREACH    
   LET l_dept_cnt = l_dept_cnt - 1
   #fill maj值                   
   DECLARE r100_fill_maj02_cs CURSOR FOR  
    SELECT UNIQUE maj02,maj20,maj20e,maj03  FROM r100_tmp
     ORDER BY maj02
   LET l_maj02_cnt = 1
   FOREACH r100_fill_maj02_cs INTO l_maj_arr[l_maj02_cnt].*
       LET l_maj02_cnt = l_maj02_cnt + 1
   END FOREACH
   LET l_maj02_cnt = l_maj02_cnt - 1 
                                   
   LET g_sql = "SELECT amt FROM r100_tmp ",
               " WHERE maj02 = ? AND dept_m = ? "
   PREPARE r100_sel_amt_cs FROM g_sql   
   ...
   FOR l_k = 1 TO l_maj02_cnt
       ...
       FOR l_j = 1 TO l_dept_cnt
           LET l_amt1 = 0
           EXECUTE r100_sel_amt_cs INTO l_amt1 USING l_maj_arr[l_k].maj02,l_dept_arr[l_j].dept_m
       ...
       END FOR
       IF g_flag = 'n' THEN
          LET g_sum[l_k] =g_sum[l_k]+l_amt[1]+l_amt[2]+l_amt[3]+l_amt[4]+l_amt[5]
                     +l_amt[6]+l_amt[7]+l_amt[8]+l_amt[9]+l_amt[10]
       ELSE       	  
       	  FOR l_j =1 TO l_dept_cnt
       	      LET g_sum[l_k] = g_sum[l_k] + l_amt[l_j]
       	  END FOR   
       	  LET l_amt[l_dept_cnt +1] = g_sum[l_k]  
       	  LET m_dept[l_dept_cnt + 1] = '合  計'   	   
       END IF      

       IF cl_null(g_sum[l_k]) THEN LET g_sum[l_k] = 0 END IF
       #display "g_sum ",l_k," ",g_sum[l_k]
       EXECUTE insert_prep USING
         
          l_maj_arr[l_k].maj02,l_maj_arr[l_k].maj03,'','','',
          l_maj_arr[l_k].maj20,l_maj_arr[l_k].maj20e,l_i,
          m_dept[1],m_dept[2],m_dept[3],m_dept[4],m_dept[5],
          m_dept[6],m_dept[7],m_dept[8],m_dept[9],m_dept[10],
