...
# Modify.........: No.MOD-970245 09/07/27 By mike 將aglg102_file里的maj20e改為定義成LIKE maj_file.maj20e  
# Modify.........: No.MOD-980081 09/08/24 By mike g102_sum 的sql 條件少了 "   AND aah00 = aag00 ", 煩請協助,謝謝!
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglg102',g_sql) CLIPPED   # 產生Temp Table   
   IF l_table = -1 THEN 
   ...
   DROP TABLE aglg102_file
#FUN-680098-BEGIN
   CREATE TEMP TABLE aglg102_file(
...
#FUN-680098-END
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL g102_tm()                        # Input print condition
   ELSE
      CALL g102()         
   END IF
...
FUNCTION g102_tm()
   ...
   OPEN WINDOW g102_w AT p_row,p_col WITH FORM "agl/42f/aglg102" 
...
#            CALL FGL_DIALOG_SETBUFFER( tm.b )
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglg102',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW g102_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)               #FUN-B80158 add
...
#     DEFINE   l_time LIKE type_file.chr8        #No.FUN-6A0073
   ...
   LET l_sql = "SELECT * FROM maj_file",
               ...
               " ORDER BY maj02"
   PREPARE g102_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)               #FUN-B80158 add
      EXIT PROGRAM 
   END IF
   DECLARE g102_c CURSOR FOR g102_p
 
   #No.FUN-780058---Begin
   #CALL cl_outnam('aglg102') RETURNING l_name
...
#FUN-680098-BEGIN
   CREATE TEMP TABLE aglg102_file(
       ...
       aah04_05  LIKE type_file.num20_6)
#FUN-680098-END
   #START REPORT g102_rep TO l_name              #No.FUN-780058 
   ...
   DECLARE tmp_curs CURSOR FOR
       SELECT * FROM aglg102_file ORDER BY maj02
...
###GENGRE###   LET g_str = g_mai02,";",tm.a,";",tm.d,";",tm.yy,";",                
###GENGRE###               tm.bm,";",tm.em,";",tm.e,";",t_azi04                                                
###GENGRE###   CALL cl_prt_cs3('aglg102','aglg102',g_sql,g_str)                             
    CALL aglg102_grdata()    ###GENGRE###
   #No.FUN-780058---End     
END FUNCTION
 
FUNCTION g102_process()
...
# 
#    PREPARE g102_sum FROM l_sql
#    DECLARE g102_sumc CURSOR FOR g102_sum
##    IF STATUS THEN CALL cl_err('prepare:g102_sum',STATUS,1) 
#       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
#       EXIT PROGRAM 
#    END IF
...
#    PREPARE g102_sum1 FROM l_sql
#    DECLARE g102_sumc1 CURSOR FOR g102_sum1
#    IF STATUS THEN CALL cl_err('prepare:g102_sum1',STATUS,1) 
#       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
#       EXIT PROGRAM 
#    END IF
#end CHI-A40035 --mark
    ...
    #FUN-C50004 sta
     ...
     LET l_sql = "SELECT SUM(aah04-aah05) FROM aah_file,aag_file",
                 ...
                 "   AND aah00 = aag00 ",
                 "   AND aah01 = aag01 AND aag07 IN ('2','3')"
     PREPARE g102_sum2 FROM l_sql
     DECLARE g102_sumc2 CURSOR FOR g102_sum2                
       #3:科目之期末(借減貸)
     LET l_sql = "SELECT SUM(aah04-aah05) FROM aah_file,aag_file",
                 " WHERE aah00 = '",tm.b,"'",
                 "   AND aah01 BETWEEN ? AND ? ",
                 ...
                 "   AND aah00 = aag00 ",
                 "   AND aah01 = aag01 AND aag07 IN ('2','3')"
     PREPARE g102_sum3 FROM l_sql
     DECLARE g102_sumc3 CURSOR FOR g102_sum3                
       #4:科目之當期異動(借)
     LET l_sql = "SELECT SUM(aah04) FROM aah_file,aag_file",
                 ...
                 "   AND aah00 = aag00 ",
                 "   AND aah01 = aag01 AND aag07 IN ('2','3')"
     PREPARE g102_sum4 FROM l_sql
     DECLARE g102_sumc4 CURSOR FOR g102_sum4                
       #5:科目之當期異動(貸)
     LET l_sql = "SELECT SUM(aah05) FROM aah_file,aag_file",
                 ...
                 "   AND aah00 = aag00 ",
                 "   AND aah01 = aag01 AND aag07 IN ('2','3')"
     PREPARE g102_sum5 FROM l_sql
     DECLARE g102_sumc5 CURSOR FOR g102_sum5                
        #6:科目之期初(借)
     LET l_sql = "SELECT SUM(aah04) FROM aah_file,aag_file",
                 " WHERE aah00 = '",tm.b,"'",
                 "   AND aah01 BETWEEN ? AND ? ",
                 ...
                 "   AND aah00 = aag00 ",
                 "   AND aah01 = aag01 AND aag07 IN ('2','3')"
     PREPARE g102_sum6 FROM l_sql
     DECLARE g102_sumc6 CURSOR FOR g102_sum6                
        #7:科目之期初(貸)
     LET l_sql = "SELECT SUM(aah05) FROM aah_file,aag_file",
                 " WHERE aah00 = '",tm.b,"'",
                 "   AND aah01 BETWEEN ? AND ? ",
                 ...
                 "   AND aah00 = aag00 ",
                 "   AND aah01 = aag01 AND aag07 IN ('2','3')"
     PREPARE g102_sum7 FROM l_sql
     DECLARE g102_sumc7 CURSOR FOR g102_sum7                
        #8:科目之期末(借)
     LET l_sql = "SELECT SUM(aah04) FROM aah_file,aag_file",
                 " WHERE aah00 = '",tm.b,"'",
                 "   AND aah01 BETWEEN ? AND ? ",
                 ...
                 "   AND aah00 = aag00 ",
                 "   AND aah01 = aag01 AND aag07 IN ('2','3')"
     PREPARE g102_sum8 FROM l_sql
     DECLARE g102_sumc8 CURSOR FOR g102_sum8               
       #9:科目之期末(貸)
     LET l_sql = "SELECT SUM(aah05) FROM aah_file,aag_file",
                 " WHERE aah00 = '",tm.b,"'",
                 "   AND aah01 BETWEEN ? AND ? ",
                 ...
                 "   AND aah00 = aag00 ",
                 "   AND aah01 = aag01 AND aag07 IN ('2','3')"
     PREPARE g102_sum9 FROM l_sql
     DECLARE g102_sumc9 CURSOR FOR g102_sum9    
    #FUN-C50004 end 
    FOREACH g102_c INTO l_maj.*
       IF STATUS THEN CALL cl_err('foreach:g102_c',STATUS,1) EXIT FOREACH END IF
       
       LET l_aah04_05 = 0
       IF NOT cl_null(l_maj.maj21) THEN 
          #str CHI-A40035 mark
          #IF l_maj.maj06 = '2' THEN      #2:科目之當期異動
          #    OPEN g102_sumc USING tm.b,l_maj.maj21,l_maj.maj22,tm.yy,tm.bm,tm.em
          #    FETCH g102_sumc INTO l_aah04_05   
          #ELSE
          ...
          #        LET l_mm = tm.em 
          #    END IF
          #    OPEN g102_sumc1 USING tm.b,l_maj.maj21,l_maj.maj22,tm.yy,l_mm
          #    FETCH g102_sumc1 INTO l_aah04_05    
          #END IF
          #IF STATUS THEN CALL cl_err('fetch:g102_sumc',STATUS,1) EXIT FOREACH END IF
          #end CHI-A40035 mark
          #str CHI-A40035 add
          #FUN-C50004 sta
          ...
          CASE l_maj.maj06
             WHEN 1   #1:科目之期初(借減貸)
               ...
               #            "   AND aah01 = aag01 AND aag07 IN ('2','3')"
               #FUN-C50004 mark end 
               OPEN g102_sumc1 USING l_maj.maj21,l_maj.maj22 #FUN-C50004 add
               FETCH g102_sumc1 INTO l_aah04_05 #FUN-C50004 add
             WHEN 2   #2:科目之當期異動
               #FUN-C50004 mark sta
               #LET l_sql = "SELECT SUM(aah04-aah05) FROM aah_file,aag_file",
               #            " WHERE aah00 = '",tm.b,"'",
               ...
               #            "   AND aah01 = aag01 AND aag07 IN ('2','3')"
               #FUN-C50004 mark end 
               OPEN g102_sumc2 USING l_maj.maj21,l_maj.maj22 #FUN-C50004 add
               FETCH g102_sumc2 INTO l_aah04_05 #FUN-C50004 add      
             WHEN 3   #3:科目之期末(借減貸)
               #FUN-C50004 mark sta
               #LET l_sql = "SELECT SUM(aah04-aah05) FROM aah_file,aag_file",
               #            " WHERE aah00 = '",tm.b,"'",
               ...
               #            "   AND aah01 = aag01 AND aag07 IN ('2','3')"
               #FUN-C50004 mark end 
               OPEN g102_sumc3 USING l_maj.maj21,l_maj.maj22 #FUN-C50004 add
               FETCH g102_sumc3 INTO l_aah04_05 #FUN-C50004 add
             WHEN 4   #4:科目之當期異動(借)
               #FUN-C50004 mark sta
               #LET l_sql = "SELECT SUM(aah04) FROM aah_file,aag_file",
               #            " WHERE aah00 = '",tm.b,"'",
               ...
               #            "   AND aah01 = aag01 AND aag07 IN ('2','3')"
               #FUN-C50004 mark end   
               OPEN g102_sumc4 USING l_maj.maj21,l_maj.maj22 #FUN-C50004 add
               FETCH g102_sumc4 INTO l_aah04_05 #FUN-C50004 add      
             WHEN 5   #5:科目之當期異動(貸)
               #FUN-C50004 mark sta
               #LET l_sql = "SELECT SUM(aah05) FROM aah_file,aag_file",
               #            " WHERE aah00 = '",tm.b,"'",
               ...
               #           "   AND aah01 = aag01 AND aag07 IN ('2','3')"
               #FUN-C50004 mark end  
               OPEN g102_sumc5 USING l_maj.maj21,l_maj.maj22 #FUN-C50004 add
               FETCH g102_sumc5 INTO l_aah04_05 #FUN-C50004 add           
             WHEN 6   #6:科目之期初(借)
               #FUN-C50004 mark sta
               #LET l_sql = "SELECT SUM(aah04) FROM aah_file,aag_file",
               #            " WHERE aah00 = '",tm.b,"'",
               ...
               #            "   AND aah01 = aag01 AND aag07 IN ('2','3')"
               #FUN-C50004 mark end   
               OPEN g102_sumc6 USING l_maj.maj21,l_maj.maj22 #FUN-C50004 add
               FETCH g102_sumc6 INTO l_aah04_05 #FUN-C50004 add          
             WHEN 7   #7:科目之期初(貸)
               #FUN-C50004 mark sta
               #LET l_sql = "SELECT SUM(aah05) FROM aah_file,aag_file",
               #            " WHERE aah00 = '",tm.b,"'",
               ...
               #            "   AND aah01 = aag01 AND aag07 IN ('2','3')"
               #FUN-C50004 mark end   
               OPEN g102_sumc7 USING l_maj.maj21,l_maj.maj22 #FUN-C50004 add
               FETCH g102_sumc7 INTO l_aah04_05 #FUN-C50004 add          
             WHEN 8   #8:科目之期末(借)
               #FUN-C50004 mark sta
               #LET l_sql = "SELECT SUM(aah04) FROM aah_file,aag_file",
               #            " WHERE aah00 = '",tm.b,"'",
               ...
               #            "   AND aah01 = aag01 AND aag07 IN ('2','3')"
               #FUN-C50004 mark end  
               OPEN g102_sumc8 USING l_maj.maj21,l_maj.maj22 #FUN-C50004 add
               FETCH g102_sumc8 INTO l_aah04_05 #FUN-C50004 add           
             WHEN 9   #9:科目之期末(貸)
               #FUN-C50004 mark sta
               #LET l_sql = "SELECT SUM(aah05) FROM aah_file,aag_file",
               #            " WHERE aah00 = '",tm.b,"'",
               ...
               #            "   AND aah01 = aag01 AND aag07 IN ('2','3')"
               #FUN-C50004 mark end  
               OPEN g102_sumc9 USING l_maj.maj21,l_maj.maj22 #FUN-C50004 add
               FETCH g102_sumc9 INTO l_aah04_05 #FUN-C50004 add          
          END CASE
          #FUN-C50004 mark sta
          #IF cl_null(l_maj.maj06) THEN
          #   LET l_sql = ''
          ...
          #PREPARE g102_sum FROM l_sql
          #DECLARE g102_sumc CURSOR FOR g102_sum
          #OPEN g102_sumc USING l_maj.maj21,l_maj.maj22
          #FETCH g102_sumc INTO l_aah04_05
          #FUN-C50004 mark end
         #end CHI-A40035 add
       ...
       INSERT INTO aglg102_file VALUES(l_maj.maj02,l_maj.maj03,l_maj.maj04,
                                    l_maj.maj05,l_maj.maj07,l_maj.maj20,l_maj.maj20e,
                                    l_aah04_05)
       IF STATUS OR SQLCA.SQLERRD[3] = 0 THEN
#          CALL cl_err('ins aglg102_file',STATUS,1) #No.FUN-660123
           CALL cl_err3("ins","aglg102_file",l_maj.maj02,l_maj.maj03,STATUS,"","ins aglg102_file",1)   #No.FUN-660123
           EXIT FOREACH 
       END IF
    END FOREACH
END FUNCTION
...
FUNCTION aglg102_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg102")
        IF handler IS NOT NULL THEN
            START REPORT aglg102_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY maj02,line"     #FUN-B80158 add
          
            DECLARE aglg102_datacur1 CURSOR FROM l_sql
            FOREACH aglg102_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg102_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg102_rep
        END IF
...
REPORT aglg102_rep(sr1)
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B80158----add----str------
            LET l_aah04_05_fmt = cl_gr_numfmt("aah_file","aah04",t_azi04)
            PRINTX l_aah04_05_fmt
            IF tm.e = 'Y' THEN 
