...
# Modify.........: No:FUN-B80158 12/01/05 By qirl FUN-B80057,FUN-BB0047追单
...
DEFINE   g_sql           STRING
DEFINE   g_str           STRING
DEFINE   g_str1          STRING     #FUN-B80158 add
...
TYPE sr1_t RECORD
    ...
    #FUN-B80158------add----str---
    amt1  LIKE abb_file.abb07,
    amt2  LIKE abb_file.abb07
    #FUN-B80158------add----end---
END RECORD
...
#NO.FUN-830094------START---
   LET g_sql = "abb05.abb_file.abb05,",
               ...
               "amt2.abb_file.abb07"
               #FUN-B80158------add----end---
   LET l_table = cl_prt_temptable('aglg750',g_sql) CLIPPED
   IF l_table=-1 THEN  
   ...
   LET l_table1 = cl_prt_temptable('aglg7501',g_sql) CLIPPED
   IF  l_table1 = -1 THEN 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time         #FUN-B80158 add
       CALL cl_gre_drop_temptable(l_table||"|"||l_table1)     #FUN-B80158 add 
       EXIT PROGRAM 
...
#NO.FUN-830094-----END----
   ...
   #No.FUN-570264 ---end---
   LET tm.a     = ARG_VAL(14)  #FUN-6C0012
   LET tm.aag38 = ARG_VAL(15)   #TQC-6C0098 add
...
#    CALL cl_err(g_aaa03,SQLCA.sqlcode,0)  # NO.FUN-660123
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
        CALL aglg750_tm()                # Input print condition
   ELSE
        CALL aglg750()                   # Read data and create out-file
   END IF
...
DEFINE li_chk_bookno  LIKE type_file.num5            #FUN-B20054 
   ...
   END IF
   OPEN WINDOW aglg750_w AT p_row,p_col
        WITH FORM "agl/42f/aglg750"
################################################################################
...
# 
#   IF INT_FLAG THEN
#      LET INT_FLAG = 0 CLOSE WINDOW aglg750_w 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
#      EXIT PROGRAM
#         
#   END IF
...
#     #  ON ACTION CONTROLP CALL aglg750_wc    # Input detail where condiction
# 
#   AFTER INPUT
#      LET l_jmp_flag = 'N'
#      ON IDLE g_idle_seconds
 ...
 #FUN-B20054--add--end
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglg750',g_time,l_cmd)  # Execute cmd at later time
      END IF
      CLOSE WINDOW aglg750_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1)     #FUN-B80158 add 
      EXIT PROGRAM
   END IF
...
DEFINE        l_str        LIKE type_file.chr1000
   ...
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aglg750'
#NO.FUN-830094------END-----
    #SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
     ...
     LET l_sql = "SELECT abb05,gem03, abb00,abb02,abb03,abb11,abb12,abb13,",
                ...
                #"       abb14,aag02, aba02,aba01,abb04, abb06, abb07",
                 ...
                 "       aag02, aag13,aba02,aba01,abb04, abb06, abb07, 0, 0",   #FUN-6C0012   #FUN-B80158 add 2 0
                #->FUN-5C0015-----------------------------------------------(E)
     ...
     PREPARE aglg750_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN CALL cl_err('prepare1:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
        CALL cl_gre_drop_temptable(l_table||"|"||l_table1)     #FUN-B80158 add 
        EXIT PROGRAM END IF
     DECLARE aglg750_curs1 CURSOR FOR aglg750_prepare1
 
     LET l_sql="SELECT abc04 FROM abc_file ",
               " WHERE abc00=? AND abc01=? AND abc02=? "
 
     PREPARE aglg750_prepare2 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN CALL cl_err('prepare2:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
        CALL cl_gre_drop_temptable(l_table||"|"||l_table1)     #FUN-B80158 add 
        EXIT PROGRAM END IF
     DECLARE aglg750_curs2 CURSOR FOR aglg750_prepare2
 
#     CALL cl_outnam('aglg750') RETURNING l_name        #NO.FUN-830094
#NO.FUN-830094----START-----
     #FUN-6C0012.....begin
  #  IF tm.a = 'N' THEN              #FUN-B80158 mark
      #  LET g_zaa[34].zaa06 = 'N'                                               
      #  LET g_zaa[40].zaa06 = 'Y'                                               
      #  LET l_name = 'aglg750_1'    #FUN-B80158 mark
  #   ELSE                            #FUN-B80158 mark                                           
      #  LET g_zaa[34].zaa06 = 'Y'                                               
      #  LET g_zaa[40].zaa06 = 'N'                                               
      #  LET l_name = 'aglg750'       #FUN-B80158 mark
  #  END IF                           #FUN-B80158 mark
#NO.FUN-830094----END-----
#     CALL cl_prt_pos_len()                      #NO.FUN-830094
    #No.FUN-6C0012  --end                                                      
                          
#     START REPORT aglg750_rep TO l_name         #NO.FUN-830094
 
#     LET g_pageno = 0                           #NO.FUN-830094
     LET g_cnt    = 1
     FOREACH aglg750_curs1 INTO sr.*
       IF STATUS != 0 THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
       IF sr.abb05 IS NULL THEN LET sr.abb05=' ' END IF
#       OUTPUT TO REPORT aglg750_rep(sr.*)       #NO.FUN-830094
#NO.FUN-830094-----START---
      LET l_no=0
      ...
      FOREACH aglg750_curs2 USING sr.aba00,sr.aba01,sr.abb02 INTO l_abc04
         IF g_cnt=0 THEN
            CASE
                 WHEN l_no=0
     ...
     #FUN-B80158-------add-----str------
     IF sr.abb06 = '1' THEN
        LET sr.amt1 = sr.abb07
     ELSE
        LET sr.amt1 = 0
     END IF

     IF sr.abb06 = '2' THEN
        LET sr.amt2 = sr.abb07
     ELSE
        LET sr.amt2 = 0
     END IF
     #FUN-B80158-------add-----end------
     ...
     EXECUTE insert_prep USING
        ...
        sr.abb07,l_no,l_abc04,t_azi04,sr.amt1,sr.amt2                  #FUN-B80158 add sr.amt1,sr.amt2 
#NO.FUN-830094-----END---   
     END FOREACH
 
#     FINISH REPORT aglg750_rep                   #NO.FUN-830094
#     CALL cl_prt(l_name,g_prtway,g_copies,g_len) #NO.FUN-830094
#NO.FUN-830094----------START----------------------------------
###GENGRE###     LET g_sql = " SELECT DISTINCT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,"|",
###GENGRE###                 " SELECT DISTINCT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED
     ...
     ELSE 
         LET tm.wc = ""
     END IF
     IF tm.a = 'N' THEN                 #FUN-B80158 add 
        LET g_template = 'aglg750_1'    #FUN-B80158 add 
        CALL aglg750_grdata()           #FUN-B80158 add
     ELSE
        LET g_template = 'aglg750'      #FUN-B80158 add 
        CALL aglg750_grdata()           #FUN-B80158 add
     END IF 
         
       
###GENGRE###     LET g_str =tm.wc,";",tm.b_date,";",tm.e_date,";",tm.a
###GENGRE###     CALL cl_prt_cs3('aglg750',l_name,g_sql,g_str)
#   CALL aglg750_grdata()    ###GENGRE###   #FUN-B80158 mark
#NO.FUN-830094----------END------------------------------- 
END FUNCTION
 
#NO.FUN-830094---------start------mark--- 
#REPORT aglg750_rep(sr)
...
FUNCTION aglg750_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg750")
        IF handler IS NOT NULL THEN
            START REPORT aglg750_rep TO XML HANDLER handler
            IF g_template = 'aglg750_1' THEN
               LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                   #    ," ORDER BY abb05,abb03,aba02,aba01,abb02"     #FUN-B80158 add
                        ," ORDER BY abb05,abb03"     #FUN-B80158 add
            ELSE
               LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                          ," ORDER BY abb05,abb03"     #FUN-B80158 add
            END IF
          
            DECLARE aglg750_datacur1 CURSOR FROM l_sql
            FOREACH aglg750_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg750_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg750_rep
        END IF
...
REPORT aglg750_rep(sr1)
    ...
    DEFINE l_sql        STRING
    DEFINE g_c35        STRING
    DEFINE g_c36        LIKE abb_file.abb14 
    DEFINE g_c37        LIKE abb_file.abb04 
    DEFINE g_c38        LIKE abb_file.abb07 
    DEFINE g_c39        STRING 
    DEFINE gc33         LIKE abb_file.abb03 
    DEFINE gc39         STRING 
    DEFINE l_sum_amt1   LIKE abb_file.abb07 
    DEFINE l_sum_amt2   LIKE abb_file.abb07 
    DEFINE l_sum2_amt1  LIKE abb_file.abb07 
    DEFINE l_sum2_amt2  LIKE abb_file.abb07 
    DEFINE l_amtbal     LIKE abb_file.abb07 
    DEFINE l_amtbal2    LIKE abb_file.abb07 
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            CASE  sr1.l_n
               WHEN 1
                  IF sr1.abb11 ="" AND sr1.abb04 = "" THEN
                     LET g_str1 = NULL
                  ELSE
                     LET g_str1 = cl_gr_getmsg("gre-214",g_lang,1),"1: ",sr1.abb11,' ',sr1.abb04
                  END IF 
               WHEN 2
                  IF sr1.abb11 = "" AND sr1.abb12 = "" THEN
                     LET g_str1 = NULL
                  ELSE
                     LET g_str1 = cl_gr_getmsg("gre-214",g_lang,1),"1: ",sr1.abb11,' ',cl_gr_getmsg("gre-214",g_lang,1),"2: ",sr1.abb12 
                  END IF
               WHEN 3 
                  IF sr1.abb11 = "" AND sr1.abb12 = "" AND sr1.abb13 = "" THEN
                     LET g_str1 = NULL
                  ELSE
                     LET g_str1 = cl_gr_getmsg("gre-214",g_lang,1),"1: ",sr1.abb11,' ',cl_gr_getmsg("gre-214",g_lang,1),"2: ",sr1.abb12,  
                                  ' ',cl_gr_getmsg("gre-214",g_lang,1),"3: ",sr1.abb13
                  END IF
               WHEN 4 
                  IF sr1.abb11 = "" AND sr1.abb12 = "" AND sr1.abb13 = "" AND sr1.abb14 = "" THEN
                     LET g_str1 = NULL
                  ELSE
                     LET g_str1 = cl_gr_getmsg("gre-214",g_lang,1),"1: ",sr1.abb11,' ',cl_gr_getmsg("gre-214",g_lang,1),"2: ",           
                                  ' ',cl_gr_getmsg("gre-214",g_lang,1),"3: ",sr1.abb13,' ',cl_gr_getmsg("gre-214",g_lang,1),"4: ", 
                                  sr1.abb14
                  END IF
               WHEN 5 
                  IF sr1.abb11 = "" AND sr1.abb12 = "" AND sr1.abb13 = "" AND sr1.abb14 = "" AND sr1.abb31 = "" THEN
                     LET g_str1 = NULL
                  ELSE
                     LET g_str1 = cl_gr_getmsg("gre-214",g_lang,1),"1: ",sr1.abb11,' ',cl_gr_getmsg("gre-214",g_lang,1),"2: ", 
                                  sr1.abb12,' ',cl_gr_getmsg("gre-214",g_lang,1),"3: ",sr1.abb13,' ',cl_gr_getmsg("gre-214",g_lang,1),"4: ",sr1.abb14,
                                  ' ',cl_gr_getmsg("gre-214",g_lang,1),"5: ",sr1.abb31
                  END IF
               WHEN 6 
                  IF sr1.abb11 = "" AND sr1.abb12 = "" AND sr1.abb13 = "" AND sr1.abb31 = "" AND sr1.abb32 = "" THEN
                     LET g_str1 = NULL
                  ELSE
                     LET g_str1 = cl_gr_getmsg("gre-214",g_lang,1),"1: ",sr1.abb11, ' ',cl_gr_getmsg("gre-214",g_lang,1),"2: ",
                                  sr1.abb12,' ',cl_gr_getmsg("gre-214",g_lang,1),"3: ",sr1.abb13,' ',cl_gr_getmsg("gre-214",g_lang,1),"4: ",    
                                  sr1.abb14,' ',cl_gr_getmsg("gre-214",g_lang,1),"5: ",sr1.abb31,' ',cl_gr_getmsg("gre214",g_lang,1),"6: ",sr1.abb32
                  END IF
               WHEN 7 
                  IF sr1.abb11 = "" AND sr1.abb12 = "" AND sr1.abb13 = "" AND sr1.abb14 = "" AND sr1.abb31 = "" AND sr1.abb32 = "" AND sr1.abb33 = "" THEN
                     LET g_str1 = NULL
                  ELSE
                     LET g_str1 = cl_gr_getmsg("gre-214",g_lang,1),"1: ",sr1.abb11,' ',cl_gr_getmsg("gre-214",g_lang,1),"2: ", sr1.abb12,
                                  ' ',cl_gr_getmsg("gre-214",g_lang,1),"3: ",sr1.abb13,' ',cl_gr_getmsg("gre-214",g_lang,1),"4:",
                                  sr1.abb14,' ',cl_gr_getmsg("gre-214",g_lang,1),"5: ",sr1.abb31,' ',cl_gr_getmsg("gre-214",g_lang,1),
                                  "6: ",sr1.abb32,' ',cl_gr_getmsg("gre-214",g_lang,1),"7: ",sr1.abb33
                  END IF
               WHEN 8 
                  IF sr1.abb11 = "" AND sr1.abb12 = "" AND sr1.abb13 = "" AND sr1.abb14 = "" AND sr1.abb31 = "" AND sr1.abb32 = "" AND sr1.abb33 = "" AND sr1.abb34 = "" THEN
                     LET g_str1 = NULL
                  ELSE
                     LET g_str1 = cl_gr_getmsg("gre-214",g_lang,1),"1: ",sr1.abb11,' ',cl_gr_getmsg("gre-214",g_lang,1),"2: ",sr1.abb12,' ',
                                  cl_gr_getmsg("gre-214",g_lang,1),"3: ",sr1.abb13,' ',cl_gr_getmsg("gre-214",g_lang,1),"4: ",
                                  sr1.abb14,' ',cl_gr_getmsg("gre-214",g_lang,1),"5: ", sr1.abb31,' ',cl_gr_getmsg("gre-214",g_lang,1),"6: ",
                                  sr1.abb32,' ',cl_gr_getmsg("gre-214",g_lang,1),"7: ",sr1.abb33,' ',cl_gr_getmsg("gre-214",g_lang,1),"8: ", 
                                  sr1.abb34 
                  END IF
              WHEN 9
                  IF sr1.abb11 = "" AND sr1.abb12 = "" AND sr1.abb13 = "" AND sr1.abb14 = "" AND sr1.abb31 = "" AND sr1.abb32 = "" AND sr1.abb33 = "" AND sr1.abb34 = "" AND sr1.abb35 = "" THEN
                     LET g_str1 = NULL
                  ELSE
                     LET g_str1 = cl_gr_getmsg("gre-214",g_lang,1),"1: ",sr1.abb11,' ',cl_gr_getmsg("gre-214",g_lang,1),"2: ",
                                  sr1.abb12,' ',cl_gr_getmsg("gre-214",g_lang,1),"3: ",sr1.abb13,' ',cl_gr_getmsg("gre-214",g_lang,1),"4: ",
                                  sr1.abb14,' ',cl_gr_getmsg("gre-214",g_lang,1),"5: ",sr1.abb31,' ',cl_gr_getmsg("gre-214",g_lang,1),"6: ",
                                  sr1.abb32,' ',cl_gr_getmsg("gre-214",g_lang,1),"7: ",sr1.abb33,' ',cl_gr_getmsg("gre-214",g_lang,1),"8: ",      
                                  sr1.abb34,' ',cl_gr_getmsg("gre-214",g_lang,1),"9: ",sr1.abb35
                  END IF
              WHEN 10 
                  IF sr1.abb11 = "" AND sr1.abb12 = "" AND sr1.abb13 = "" AND sr1.abb14 = "" AND sr1.abb31 = "" AND sr1.abb32 = "" AND sr1.abb33 = "" AND sr1.abb34 = "" AND sr1.abb35 = "" AND sr1.abb36 = "" THEN
                     LET g_str1 = NULL
                  ELSE
                     LET g_str1 = cl_gr_getmsg("gre-214",g_lang,1),"1: ",sr1.abb11, ' ',cl_gr_getmsg("gre-214",g_lang,1),"2: ",
                                  sr1.abb12,' ',cl_gr_getmsg("gre-214",g_lang,1),"3: ",sr1.abb13,' ',cl_gr_getmsg("gre-214",g_lang,1),"4: ",
                                  sr1.abb14,' ',cl_gr_getmsg("gre-214",g_lang,1),"5: ", sr1.abb31,' ',cl_gr_getmsg("gre-214",g_lang,1),"6: ",
                                  sr1.abb32,' ',cl_gr_getmsg("gre-214",g_lang,1),"7: ",sr1.abb33,' ',cl_gr_getmsg("gre-214",g_lang,1),"8: ", 
                                  sr1.abb34,' ',cl_gr_getmsg("gre-214",g_lang,1),"9: ",sr1.abb35,' ',cl_gr_getmsg("gre-214",g_lang,1),"10:",sr1.abb36 
                  END IF
               OTHERWISE 
                  LET g_str1 = sr1.abb04
            END CASE
            PRINTX g_str1
            ...
            CASE sr1.l_n
              ...
              WHEN 3 
                 ...
                 END IF
              WHEN 4 
                 LET g_c35 = cl_gr_getmsg("gre-214",g_lang,1),'8'
            END CASE
            PRINTX g_c35
            ...
            CASE sr1.l_n
               WHEN 0 
                  IF sr1.abb06 = '1' THEN LET g_c39 = "DB" ELSE LET g_c39 = "CR" END IF
               WHEN 1 
                  IF cl_null(sr1.abb04) THEN 
                     IF sr1.abb06 = '1' THEN  LET g_c39 = "DB" ELSE LET g_c39 = "CR" END IF
                  END IF
            ...
            IF sr1.abb06 = '1' THEN LET gc39 = "DB" ELSE LET gc39 = "CR" END IF
            PRINTX gc39
     
            LET l_amt_fmt = cl_gr_numfmt("abb_file","abb07",t_azi04)
            PRINTX l_amt_fmt

            LET l_sql = "SELECT A.*,B.t_azi04 FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,"A LEFT OUTER JOIN ",
                        g_cr_db_str CLIPPED,l_table CLIPPED," B ON A.aba00 = B.aba00 ",
                        " WHERE aba00 = '",sr1.aba00 CLIPPED,"'",
                        " AND aba01 = '",sr1.aba01 CLIPPED,"'",
                        " AND aba02 = ",sr1.aba02,
                        " AND l_abc04 = '",sr1.l_abc04 CLIPPED ,"'"
            START REPORT aglg750_subrep01
            DECLARE aglg750_subrep01 CURSOR FROM l_sql
            FOREACH aglg750_subrep01 INTO sr2.*,l_azi04
                OUTPUT TO REPORT aglg750_subrep01 (sr2.*,l_azi04)
            END FOREACH
            FINISH REPORT aglg750_subrep01
        ...
        AFTER GROUP OF sr1.abb05
            #FUN-B80158-------add-----str-----
            LET l_sum2_amt1 = GROUP SUM(sr1.amt1)
            PRINTX l_sum2_amt1
            LET l_sum2_amt2 = GROUP SUM(sr1.amt2)
            PRINTX l_sum2_amt2
            LET l_amtbal2 = l_sum2_amt1 - l_sum2_amt2
            PRINTX l_amtbal2
            #FUN-B80158-------add-----end-----
        ...
        AFTER GROUP OF sr1.abb03
            ...
            PRINTX l_sum_amt1
            LET l_sum_amt2 = GROUP SUM(sr1.amt2)
            PRINTX l_sum_amt2
            LET l_amtbal = l_sum_amt1 - l_sum_amt2
            PRINTX l_amtbal
            #FUN-B80158-------add-----end-----
...
#FUN-B80158-------add-----str-----
REPORT aglg750_subrep01(sr2,l_azi04)
    DEFINE sr2 sr2_t
    DEFINE l_str     STRING
    DEFINE l_azi04   LIKE azi_file.azi04
    DEFINE g_c35     STRING
    DEFINE g_c36     LIKE abc_file.abc04
    DEFINE g_c38     LIKE abb_file.abb07
    DEFINE g_c39     STRING
    DEFINE g_c38_fmt STRING

    FORMAT
        ON EVERY ROW
            IF sr2.g_cnt = 0 AND sr2.l_no = 0 AND cl_null(sr2.abb04) THEN
               LET l_str = sr2.l_abc04
            ELSE 
               LET l_str = g_str1
            END IF
            PRINTX l_str
            ...
            IF sr2.g_cnt = 0 THEN  
               CASE sr2.l_no 
                  WHEN 0 
                     IF NOT cl_null(sr2.abb04) THEN 
                        LET g_c35 = l_str
                        LET g_c36 = sr2.l_abc04
                        LET g_c38 = sr2.abb07  
                        IF sr2.abb06 = '1' THEN LET g_c39 = "DB" ELSE LET g_c39 = "CR" END IF
                     END IF
                  WHEN 1 
                     IF cl_null(sr2.abb04) THEN 
                        LET g_c35 = l_str
                        LET g_c36 = sr2.l_abc04
                        LET g_c38 = sr2.abb04  
                        IF sr2.abb06 = '1' THEN LET g_c39 = "DB" ELSE LET g_c39 = "CR" END IF
                     ELSE
                        LET g_c35 = sr2.l_abc04
                     END IF
                  WHEN 2
                     IF NOT cl_null(sr2.abb04) THEN
                        LET g_c35 = sr2.abb04
               ...
               END CASE       
            END IF
            IF sr2.g_cnt = 1 AND sr2.l_no = 0 AND cl_null(sr2.abb04) THEN
               LET g_c35 = g_str1
               LET g_c36 = sr2.l_abc04
               LET g_c38 = sr2.abb07  
               IF sr2.abb06 = '1' THEN LET g_c39 = "DB" ELSE LET g_c39 = "CR" END IF
            ...
            LET g_c38_fmt = cl_gr_numfmt("abb_file","abb07",l_azi04)
            PRINTX g_c38_fmt
