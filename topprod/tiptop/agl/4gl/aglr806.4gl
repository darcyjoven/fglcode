...
MAIN
 ...
 #--str FUN-780059 add--#                                                                                                           
   ...
   LET l_table = cl_prt_temptable('aglr806',g_sql) CLIPPED   # 產生Temp Table                                                       
...
#No.FUN-740020 ---end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'  # If background job sw is off
      THEN CALL aglr806_tm(0,0)          # Input print condition
      ELSE CALL aglr806()                # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
END MAIN
 
FUNCTION aglr806_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,          #No.FUN-680098 SMALLINT
   ...
   END IF
   OPEN WINDOW aglr806_w AT p_row,p_col
        WITH FORM "agl/42f/aglr806"
################################################################################
...
# 
#   IF INT_FLAG THEN
#      LET INT_FLAG = 0 CLOSE WINDOW aglr806_w 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
#      EXIT PROGRAM
#         
#   END IF
...
#                        " '",g_bookno CLIPPED,"'",   #TQC-610056        #No.FUN-740020
         ...
         CALL cl_cmdat('aglr806',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW aglr806_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#    LET l_sql = "SELECT aed01, aag02, aag13, aag15, aag16, aag17, aag18,",  #FUN-6C0012
     LET l_sql = "SELECT aed00,aed01, aag02, aag13, aag15, aag16, aag17, aag18,",   #NO.TQC-760105
		 ...
		 "  FROM aed_file LEFT OUTER JOIN aag_file ON aed00 = aag_file.aag00 AND aed01 = aag_file.aag01  LEFT OUTER JOIN aee_file ON aed00 = aee_file.aee00 AND aed01 = aee_file.aee01 AND aed011 = aee_file.aee02 AND aed02 = aee_file.aee03 ",       #MOD-A70200
                ...
                #"          '','','','','','','','','','',",        #MOD-A60138 mark 
                 "          aed02,aee04"
                 # FUN-5C0015 add (e)
     PREPARE aglr806_prepare1 FROM l_sql
     ...
     END IF
     DECLARE aglr806_curs1 CURSOR FOR aglr806_prepare1
 
#    CALL cl_outnam('aglr806') RETURNING l_name                     #FUN-780059--mark
 
     #FUN-6C0012.....begin
     IF tm.e = 'Y' THEN
         LET g_zaa[35].zaa06 ='Y'                                               
         LET g_zaa[36].zaa06 ='N'
         LET l_name1 = 'aglr806_1'                                                
      ELSE                                                                      
      ...
      CALL cl_prt_pos_len()
     #FUN-6C0012.....end
#    START REPORT aglr806_rep TO l_name                             #FUN-780059--mark
 
     LET g_pageno = 0
     FOREACH aglr806_curs1 INTO sr.*
        IF SQLCA.sqlcode != 0 THEN
...
#--end-FUN-780059-add--#
#       OUTPUT TO REPORT aglr806_rep(sr.*)                       #FUN-780059--mark
...
#    FINISH REPORT aglr806_rep                                   #FUN-780059--mark
...
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)                 #FUN-780059--mark
 ## **** 與 Crystal Reports 串聯段 - <<<< CALL cs3() >>>> FUN-770052 **** ##                                                        
    ...
    LET g_str = ''                                                                                                                  
    LET g_str = tm.wc,";",tm.aed03,";",tm.aed04_b,";",tm.aed04_e,";",          
                tm.t,";",t_azi04,";",t_azi05                                                                               
    CALL cl_prt_cs3('aglr806',l_name1,l_sql,g_str)                                                                                
    #------------------------------ CR (4) ------------------------------# 
END FUNCTION
 
#--str-FUN-780059--mark--#
#REPORT aglr806_rep(sr)
