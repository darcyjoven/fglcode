...
TYPE sr1_t RECORD
    ...
    a     LIKE aao_file.aao05,    #FUN-B800158 add
    a1    LIKE aao_file.aao05,    #FUN-B800158 add
    a2    LIKE aao_file.aao05,    #FUN-B800158 add
    lastcharge1 LIKE aao_file.aao05     #FUN-B800158 add
END RECORD
###GENGRE###END
...
#No.FUN-780060--start--add                                                                                                          
   LET g_sql="aao00.aao_file.aao00,",                                                                                               
             ...
             "a.aao_file.aao05,",              #FUN-B80158 add
             "a1.aao_file.aao05,",             #FUN-B80158 add
             "a2.aao_file.aao05,",             #FUN-B80158 add
             "lastcharge1.aao_file.aao05"      #FUN-B80158 add
   LET l_table = cl_prt_temptable('aglg183',g_sql) CLIPPED                                                                          
   IF l_table = -1 THEN 
...
FUNCTION g183_tm()
   ...
   OPEN WINDOW g183_w AT p_row,p_col
     WITH FORM "agl/42f/aglg183"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
  ...
  #FUN-B20054--add--end 
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglg183',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0073
          ...
          sr        RECORD
                       ...
                       a           LIKE aao_file.aao05,   #FUN-B80158 add  
                       a1          LIKE aao_file.aao05,   #FUN-B80158 add
                       a2          LIKE aao_file.aao05,   #FUN-B80158 add
                       lastcharge1 LIKE aao_file.aao05    #FUN-B80158 add
                    END RECORD
#No.FUN-780060--start--add                                                                                                          
...
#No.FUN-780060--end--
   ...
   IF STATUS THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g183_caag CURSOR FOR g183_paag
   ...
   IF STATUS THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g183_cgem CURSOR FOR g183_pgem
   ...
   PREPARE g183_paaodef FROM l_sql
   IF STATUS THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g183_caaodef CURSOR FOR g183_paaodef
   ...
   PREPARE g183_paaocom FROM l_sql
   IF STATUS THEN
      CALL cl_err('prepare1:',STATUS,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)               #FUN-B80158 add
      EXIT PROGRAM
   END IF
   DECLARE g183_caaocom CURSOR FOR g183_paaocom
#No.FUN-780060--start--mark
#  CALL cl_outnam('aglg183') RETURNING l_name
#  START REPORT aglg183_rep TO l_name
...
#  END CASE
#No.FUN-780060--end--
   FOREACH g183_caag INTO l_aag01,sr.aag02,sr.aag13       #FUN-6C0012
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
 
      FOREACH g183_cgem INTO l_gem01
         LET sr.aao00 = bookno     #No.FUN-740020
         LET sr.aao01 = l_aag01
         LET sr.aao02 = l_gem01
 
         #-->(1)基準:實際費用
         OPEN g183_caaodef USING bookno,l_aag01,l_gem01   #No.FUN-740020
         FETCH g183_caaodef INTO sr.aao05,sr.aao06
         ...
         #-->(1)比較:實際費用
         OPEN g183_caaocom USING bookno,l_aag01,l_gem01    #No.FUN-740020
         FETCH g183_caaocom INTO l_aao05,l_aao06
         IF SQLCA.sqlcode THEN
            LET l_aao05 = 0
            LET l_aao06 = 0
         END IF
         ...
         #FUN-B80158----add----str-----
         LET sr.a = sr.aao05 - sr.aao06
         IF sr.aao00 = bookno THEN 
            LET sr.a1 = sr.a
            LET sr.lastcharge1 = sr.lastcharge
         END IF
...
#No.FUN-780060--start--add
         ...
         EXECUTE insert_prep USING 
                ...
                ,sr.a,sr.a1,sr.lastcharge1,sr.a2        #FUN-B80158 add
#        OUTPUT TO REPORT aglg183_rep(sr.*)
 
      END FOREACH
   END FOREACH
 
#  FINISH REPORT aglg183_rep
...
#   CALL cl_prt(l_name,g_prtway,g_copies,g_len)                                                                                     
    ...
    LET l_tit3 = tm.defyy USING '####','/',tm.defbm USING '&#','-',
                 tm.defem USING '&#' 
###GENGRE###    LET g_str=g_str,";",tm.type,";",l_tit1,";",l_tit2,";",l_tit3,";",
###GENGRE###              l_flag,";",bookno,";",tm.diff1,";",tm.diff2,";",g_azi04
###GENGRE###              ,";",g_azi05
###GENGRE###    LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                                
###GENGRE###    CALL cl_prt_cs3('aglg183','aglg183',g_sql,g_str)
    CALL aglg183_grdata()    ###GENGRE###
#No.FUN-780060--end--
END FUNCTION
#No.FUN-780060--start--mark
{REPORT aglg183_rep(sr)
   DEFINE sr        RECORD
                      aao00       LIKE aao_file.aao00,   #帳別
                      aao01       LIKE aao_file.aao01,   #科目
                      aao02       LIKE aao_file.aao02,   #部門
...
FUNCTION aglg183_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg183")
        IF handler IS NOT NULL THEN
            START REPORT aglg183_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY aao01"    #FUN-B80158 add
          
            DECLARE aglg183_datacur1 CURSOR FROM l_sql
            FOREACH aglg183_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg183_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg183_rep
        END IF
...
REPORT aglg183_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B80158-------add-----str-----
    DEFINE l_b            STRING
    DEFINE l_a1           LIKE aao_file.aao05
    DEFINE l_a2           LIKE aao_file.aao05
    DEFINE l_difperyy     LIKE aao_file.aao05
    DEFINE l_defcharge1   LIKE aao_file.aao05
    DEFINE l_difpercent   LIKE aao_file.aao05
    DEFINE l_defpercent1  LIKE aao_file.aao05
    DEFINE l_diff1        LIKE aao_file.aao05
    DEFINE l_diff2        LIKE aao_file.aao05
    DEFINE l_st           LIKE type_file.chr1
    DEFINE l_RTotal0      LIKE aao_file.aao05
    DEFINE l_RTotal1      LIKE aao_file.aao05
    DEFINE l_lastcharge   LIKE aao_file.aao05
    DEFINE l_tit1    LIKE type_file.chr1000                                                                                          
    DEFINE l_tit2    LIKE type_file.chr1000 
    DEFINE l_tit3    LIKE type_file.chr1000 
    DEFINE l_aao05_1_fmt   STRING
    DEFINE l_aao05_2_fmt   STRING
    #FUN-B80158-------add-----end-----

    
    ORDER EXTERNAL BY sr1.aao01
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
            #FUN-B80158-------add-----str-----
            IF tm.type = 'M' OR tm.type = 'P' OR tm.type = 'R' OR tm.type = 'S' THEN
               LET l_b = cl_gr_getmsg("gre-200",g_lang,tm.type)
            ELSE
               LET l_b = cl_gr_getmsg("gre-200",g_lang,'O')
            END IF
            PRINTX l_b
            LET l_tit1 = tm.defyy USING '####','/',tm.defbm USING '&#','-',
                         tm.defem USING '&#',cl_gr_getmsg("gre-201",g_lang,1)
            LET l_tit2 = tm.comyy USING '####','/',tm.combm USING '&#','-',
                         tm.comem USING '&#',cl_gr_getmsg("gre-201",g_lang,1)
            PRINTX l_tit1
            PRINTX l_tit2
            LET l_tit3 = tm.defyy USING '####','/',tm.defbm USING '&#','-',
                         tm.defem USING '&#'
        ...
        AFTER GROUP OF sr1.aao01
            #FUN-B80158-------add-----str-----
            LET  l_aao05_1_fmt = cl_gr_numfmt("aao_file","aao05",g_azi04)
            PRINTX l_aao05_1_fmt

            LET l_defcharge1 = GROUP SUM(sr1.a2)
            PRINTX l_defcharge1
 
#           IF sr1.aao00 = bookno THEN 
#              LET l_a2 = l_a
#              LET l_lastcharge = sr1.lastcharge
#           END IF
            LET l_RTotal0 = GROUP SUM(sr1.a1)
            IF cl_null(l_RTotal0) THEN
               LET l_RTotal0 = 0
            END IF
            LET l_RTotal1 = GROUP SUM(sr1.lastcharge1)
            IF cl_null(l_RTotal1) THEN
               LET l_RTotal1 = 0
            END IF
            PRINTX l_RTotal0
            PRINTX l_RTotal1
           
            LET l_diff1 = l_RTotal0 - l_RTotal1
            PRINTX l_diff1
  
            IF l_RTotal1 != 0 THEN
               LET l_difpercent = (l_diff1 / l_RTotal1) * 100 
            ELSE
               LET l_difpercent = 0
            END IF
            PRINTX l_difpercent

            IF l_defcharge1 != 0 THEN
               LET l_difperyy = (l_RTotal0 / l_defcharge1) * 100
            ELSE 
               LET l_difperyy = 0
            END IF
            PRINTX l_difperyy
        ...
        ON LAST ROW
            #FUN-B80158-------add-----str-----
            LET l_aao05_2_fmt = cl_gr_numfmt("aao_file","aao05",g_azi05)
            PRINTX l_aao05_2_fmt
   
            LET l_diff2 = SUM(sr1.a) - SUM(sr1.lastcharge)
            PRINTX l_diff2
 
            IF SUM(sr1.lastcharge) != 0 THEN
               LET l_defpercent1 = (l_diff2 /  SUM(sr1.lastcharge)) * 100
            ELSE 
               LET l_defpercent1 = 0 
            END IF
            PRINTX l_defpercent1 
            #FUN-B80158-------add-----end-----
