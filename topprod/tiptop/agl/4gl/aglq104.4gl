...
# Modify.........: NO.TQC-C70089 12/07/13 By lujh 只列印損益表資料，則“報表編號”欄位開窗應該篩選挑到mai03='3'損益類的該帳套資料
...
# Modify.........: No:MOD-DA0182 13/10/28 By fengmy 打印報表失敗
...
MAIN
   ...
   LET tm.axa01 = ARG_VAL(32)
   LET tm.axa02 = ARG_VAL(33)

   IF cl_null(tm.b) THEN
      SELECT aaw01 INTO g_aaw01 FROM aaw_file
      LET tm.b = g_aaw01
   END IF
  
   OPEN WINDOW q104_w AT 5,10
        WITH FORM "agl/42f/aglq104" ATTRIBUTE(STYLE = g_win_style)

   CALL cl_ui_init()

   DROP TABLE aglq104_tmp;
   CREATE TEMP TABLE aglq104_tmp(
   maj20   LIKE maj_file.maj20,
   maj02   LIKE maj_file.maj02,
   maj03   LIKE maj_file.maj03,
   maj26   LIKE maj_file.maj26,
...
FUNCTION q104_tm()
   ...
   OPEN WINDOW q104_w1 AT p_row,p_col WITH FORM "agl/42f/aglq104_1"
...
WHILE TRUE
    ...
    INPUT BY NAME tm.b,tm.a,  #No.FUN-740032
      ...
      AFTER FIELD a
       ...
       #FUN-6C0068--begin
         CALL s_chkmai(tm.a,'AGL') RETURNING li_result
      ...
      ON ACTION CONTROLP
         CASE
            WHEN INFIELD(axa01) OR INFIELD(axa02)
               ...
               LET g_qryparam.form = 'q_axa4'
               LET g_qryparam.default1 = tm.axa01
               LET g_qryparam.default2 = tm.axa02
               CALL cl_create_qry() RETURNING tm.axa01,tm.axa02
...
#              CALL FGL_DIALOG_SETBUFFER( tm.a )
               ...
               #LET g_qryparam.where = " mai00 = '",tm.b,"'"  #No.FUN-740032      #TQC-C70089  mark
               LET g_qryparam.where = " mai00 = '",tm.b,"' AND mai03 = '3' "      #TQC-C70089  add
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",tm.axa01 CLIPPED,"'",
                         " '",tm.axa02 CLIPPED,"'"
         CALL cl_cmdat('aglq104',g_time,l_cmd)    # Execute cmd at later time
      END IF
...
#     DEFINE   l_time LIKE type_file.chr8        #No.FUN-6A0097
   ...
   #No.FUN-B80057--mark--End-----
     ...
     DELETE FROM aglq104_tmp;   #No.FUN-850030
    ...
    #CALL cl_outnam('aglq104') RETURNING l_name    #No:FUN-780057
    #START REPORT q104_rep TO l_name               #No:FUN-780057
     ...
     FOREACH q104_c INTO maj.*
          ...
          IF NOT cl_null(maj.maj21) THEN
	        ...
	        #按照公式計算上月相關金額
                IF tm.em1 = 1 OR tm.bm1 != tm.em1 THEN
                   LET lamt1 = 0
                ELSE
                   SELECT SUM(axh08) INTO lamt1 FROM axh_file
                    WHERE axh00 = g_aaw01     #帐别
                      AND axh01 = tm.axa01     #族群
                    #Mark by sam 20101204 
                    # AND axh02 = tm.axa02     #上层公司
                    #End Mark sam 20101204
                      AND axh05 BETWEEN maj.maj21 AND maj.maj22
                      AND axh06 = tm.yy1   #年度
                      AND axh07 = tm.em1-1        #期别
                END IF
             END IF
             IF STATUS THEN
                CALL cl_err('sel axh8:',STATUS,1)
                EXIT FOREACH
             END IF
       ...
       IF maj.maj06='5' THEN      ## 貸方金額
          IF NOT cl_null(maj.maj21) THEN
             IF maj.maj24 IS NULL THEN
                SELECT SUM(axh09) INTO amt1 FROM axh_file
                 WHERE axh00 = g_aaw01     #帐别
                   AND axh01 = tm.axa01     #族群
                  #Mark by sam 20101204 
                  #AND axh02 = tm.axa02     #上层公司
                  #End Mark sam 20101204 
                   ...
                   AND axh07 = tm.em1        #期别
                #luttb 101227--add--str-
                SELECT SUM(axh09) INTO amt4 FROM axh_file
                 WHERE axh00 = g_aaw01     #帐别
                   AND axh01 = tm.axa01     #族群
                   AND axh05 BETWEEN maj.maj21 AND maj.maj22
                   AND axh06 = tm.yy1   #年度
                ...
                IF tm.em1 = 1 OR tm.bm1 != tm.em1 THEN
                   LET lamt1 = 0
                ELSE
                   SELECT SUM(axh09) INTO lamt1 FROM axh_file
                    WHERE axh00 = g_aaw01     #帐别
                      AND axh01 = tm.axa01     #族群
                     #Mark by sam 20101204 
                     #AND axh02 = tm.axa02     #上层公司
       ...
       IF maj.maj06<'4' THEN     #No.FUN-570087  --add
          IF NOT cl_null(maj.maj21) THEN
             IF maj.maj24 IS NULL THEN
                SELECT SUM(axh08-axh09) INTO amt1 FROM axh_file
                 WHERE axh00 = g_aaw01     #帐别
                   AND axh01 = tm.axa01     #族群
                  #Mark by sam 20101204
                  #AND axh02 = tm.axa02     #上层公司
                  #End Mark
                   ...
                   AND axh07 = tm.em1
                #luttb 101227--add--str-
                SELECT SUM(axh08-axh09) INTO amt4 FROM axh_file
                 WHERE axh00 = g_aaw01     #帐别
                   AND axh01 = tm.axa01     #族群
                   AND axh05 BETWEEN maj.maj21 AND maj.maj22
                   AND axh06 = tm.yy1   #年度
                ...
                IF tm.em1 = 1 OR tm.bm1 != tm.em1 THEN
                   LET lamt1 = 0
                ELSE
                   SELECT SUM(axh08-axh09) INTO lamt1 FROM axh_file
                    WHERE axh00 = g_aaw01     #帐别
                      AND axh01 = tm.axa01     #族群
                    #Mark by sam 20101204
                    # AND axh02 = tm.axa02     #上层公司
                ...
                END IF
             END IF
             IF STATUS THEN CALL cl_err('sel axh1:',STATUS,1) EXIT FOREACH END IF
             IF amt1 IS NULL THEN LET amt1 = 0 END IF
       ...
       IF tm.yy2 IS NOT NULL THEN
          IF maj.maj06 = '4' THEN
            IF NOT cl_null(maj.maj21) THEN
               IF maj.maj24 IS NULL THEN
                  SELECT SUM(axh08) INTO amt2 FROM axh_file
                    WHERE axh00 = g_aaw01     #帐别
                      AND axh01 = tm.axa01     #族群
                    #Mark by sam 20101204
                    # AND axh02 = tm.axa02     #上层公司
                  ...
                  IF tm.em2 = 1 OR tm.bm2 != tm.em2 THEN
                     LET lamt2 = 0
                  ELSE
                     SELECT SUM(axh08) INTO lamt2 FROM axh_file
                      WHERE axh00 = g_aaw01     #帐别
                        AND axh01 = tm.axa01     #族群
                       #Mark by sam 20101204
                       #AND axh02 = tm.axa02     #上层公司
          ...
          IF maj.maj06 = '5' THEN
             IF NOT cl_null(maj.maj21) THEN
                IF maj.maj24 IS NULL THEN
                   SELECT SUM(axh09) INTO amt2 FROM axh_file
                    WHERE axh00 = g_aaw01     #帐别
                      AND axh01 = tm.axa01     #族群
                     #Mark by  sam 20101204 
                     #AND axh02 = tm.axa02     #上层公司
                    ...
                    #按照公式計算上月相關金額 -5B0299
                     IF tm.em2 = 1 OR tm.bm2 != tm.em2 THEN
                        LET lamt2 = 0
                     ELSE
                        SELECT SUM(axh09) INTO lamt2 FROM axh_file
                      WHERE axh00 = g_aaw01     #帐别
                        AND axh01 = tm.axa01     #族群
                       #Mark by sam 20101204
                       # AND axh02 = tm.axa02     #上层公司
        ...
        IF maj.maj06<'4' THEN
           IF NOT cl_null(maj.maj21) THEN
              IF maj.maj24 IS NULL THEN
                        SELECT SUM(axh08-axh09) INTO amt2 FROM axh_file
                         WHERE axh00 = g_aaw01
                           AND axh01 = tm.axa01  #No.FUN-740032
                          #Mark by sam 20101204
                          #AND axh02 = tm.axa02  #No.FUN-740032
                          #End Mark sam 20101204
                           AND axh05 BETWEEN maj.maj21 AND maj.maj22
                           AND axh06 = tm.yy2
                           AND axh07 = tm.em2
                  ...
                  IF tm.em2 = 1 OR tm.bm2 != tm.em2 THEN
                     LET lamt2 = 0
                  ELSE
                           SELECT SUM(axh08-axh09) INTO lamt2 FROM axh_file
                            WHERE axh00 = g_aaw01
                              AND axh01 = tm.axa01  #No.FUN-740032
                             #Mark by sam 20101204
                             #AND axh02 = tm.axa02  #No.FUN-740032
       ...
       #新增按公式打印,累計已計算完成科目金額
        ...
        IF maj.maj04=0 THEN
           ...
           INSERT INTO aglq104_tmp VALUES(
             maj.maj20,maj.maj02,maj.maj03,maj.maj26,sr.bal1,sr.bal2,sr.lbal1,sr.lbal2,
             '2',maj.maj06)
           #No.FUN-850030  --End
        ELSE
     ...
     #CALL cl_prt_cs3('aglq104','aglq104',g_sql,g_str)
     #No.FUN-850030  --End
     #No:FUN-780057 --END
     ...
     #No.FUN-850030  --Begin
     DECLARE aglq104_cur CURSOR FOR SELECT * FROM aglq104_tmp
                                     ORDER BY maj02,line
     CALL g_aag.clear()
     LET l_i = 1
     FOREACH aglq104_cur INTO g_pr.*
  ...
  DEFINE l_i     LIKE type_file.num10
   ...
   LET g_sql="maj20.maj_file.maj20,",
         ...
         #   "zo12.zo_file.zo12"       #FUN-920034
            ...
            #"pja02.pja_file.pja02,",  #DFUN-950014   #MOD-DA0182 mark
            #"gem02.gem_file.gem02,",  #MOD-DA0182 mark
             "zx02.zx_file.zx02"       #DFUN-960015
   LET l_table=cl_prt_temptable('aglq104',g_sql) CLIPPED
   IF  l_table=-1 THEN
       CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80057--add--
       EXIT PROGRAM
   END IF
   LET g_sql="INSERT INTO ds_report.",l_table CLIPPED,
            #" VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)"     #No.TQC-810068  #FUN-920034 #DFUN-950014  #DFUN-960015  #MOD-DA0182 mark
             " VALUES(?,?,?,?,?,?,?,?,?,?,?,?)"         #MOD-DA0182
...
DEFINE sr   DYNAMIC ARRAY OF RECORD
   ...
   LET l_sql="SELECT abd02",
             "  FROM abd_file ",
             " WHERE abd01 = '",p_group,"' "
   PREPARE aglq104_prepare FROM l_sql
   DECLARE aglq104_curs CURSOR FOR aglq104_prepare

   LET l_i = 1
   FOREACH aglq104_curs INTO sr[l_i].*  
      LET l_i = l_i + 1    
   END FOREACH   
