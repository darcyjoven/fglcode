...
# Modify.........: No.FUN-C50139 12/06/05 By chenying rep段修改 
# Modify.........: No.FUN-CB0051 12/12/07 By chenying 去除單身定位點處理 
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglg113',g_sql) CLIPPED   # 產生Temp Table   
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN    # If background job sw is off
      CALL g113_tm()                            # Input print condition
   ELSE 
      CALL g113()                               # Read data and create out-file
   END IF
...
FUNCTION g113_tm()
   ...
   OPEN WINDOW aglg113 AT p_row,p_col WITH FORM "agl/42f/aglg113"
   ...
   WHILE TRUE
      ...
      END INPUT
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW aglg113 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time
         EXIT PROGRAM
      END IF
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
          LET l_cmd = l_cmd CLIPPED,          #(at time fglgo xxxx p1 p2 p3)
                      ...
                      " '",g_template CLIPPED,"'",
                         " '",g_rpt_name CLIPPED,"'"
          CALL cl_cmdat('aglg113',g_time,l_cmd)
         END IF
         CLOSE WINDOW aglg113
         CALL cl_used(g_prog,g_time,2) RETURNING g_time
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
FUNCTION g113()
   ...
   DROP TABLE aglg113_tmp
   CREATE TEMP TABLE aglg113_tmp(
      maj23    LIKE maj_file.maj23,
      maj31  LIKE maj_file.maj31, #FUN-B90140 add maj31
      maj20    LIKE maj_file.maj20,
      bal      LIKE type_file.num20_6,
   ...
   FOREACH g113_c INTO maj.*
      ...
      IF maj.maj03='3' OR maj.maj03='4' THEN
         IF NOT cl_null(maj.maj20) THEN
            INSERT INTO aglg113_tmp VALUES
   ...
   #抓報表結構裡帳戶左邊最大序號
   SELECT MAX(maj02) INTO l_maj02_l FROM aglg113_tmp WHERE maj23='11'
   #抓報表結構裡帳戶右邊最大序號
   SELECT MAX(maj02) INTO l_maj02_r FROM aglg113_tmp WHERE maj23='12'
   IF cl_null(l_maj02_l) THEN LET l_maj02_l = 0 END IF
   IF cl_null(l_maj02_r) THEN LET l_maj02_r = 0 END IF
   LET r_row = 0  LET l_row = 0
   DECLARE g113_c1 CURSOR FOR SELECT * FROM aglg113_tmp ORDER BY maj23,maj02,line
   FOREACH g113_c1 INTO tmp1.*
...
###GENGRE###               tm.bm,";",tm.e,";",l_lastday,";",g_basetot1,";",tm.yy,";",
###GENGRE####                p11        p12
###GENGRE###               tm.yy1,";",tm.bm
###GENGRE###               ,";",tm.acc_code   #FUN-B90140   Add
###GENGRE###   CALL cl_prt_cs3('aglg113','aglg113',g_sql,g_str) 
    CALL aglg113_grdata()    ###GENGRE###
END FUNCTION

###GENGRE###START
FUNCTION aglg113_grdata()
    DEFINE l_sql    STRING
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg113")
        IF handler IS NOT NULL THEN
            START REPORT aglg113_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg113_datacur1 CURSOR FROM l_sql
            FOREACH aglg113_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg113_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg113_rep
        END IF
    ...
    FORMAT
        ...
        ON EVERY ROW
           ...
           #PRINTX l_per_l
            ...
            IF g_basetot1 != "" THEN
               LET l_per_l_1 = (sr1.bal_l_1/g_basetot1) * 100
            ELSE
               LET l_per_l_1 = ""
            END IF
           #PRINTX l_per_l_1
          ...
          #PRINTX l_per_r
           ...
           IF g_basetot1 != "" THEN
              LET l_per_r_1 = (sr1.bal_r_1/g_basetot1) * 100
           ELSE
              LET l_per_r_1 = ""
               LET sr1.maj20_l = " "
               LET sr1.bal_l = " "
...
#       AFTER GROUP OF sr1.maj31_l     #FUN-C50139 mark
