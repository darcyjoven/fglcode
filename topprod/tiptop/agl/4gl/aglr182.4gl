...
#            "merge.aao_file.aao05,",       #TQC-950103                                                                                             
   ...
   LET l_table = cl_prt_temptable('aglr182',g_sql) CLIPPED                                                                          
...
#   CALL s_dsmark(bookno)   #No.FUN-740020
   ...
   OPEN WINDOW r182_w AT p_row,p_col
     WITH FORM "agl/42f/aglr182"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
  ...
  #FUN-B20054--add--end 
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglr182',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#No.FUN-780060--end--
   ...
   PREPARE r182_paag FROM l_sql
   IF STATUS THEN
      CALL cl_err('prepare:',STATUS,1)   
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE r182_caag CURSOR FOR r182_paag
   ...
   PREPARE r182_pgem FROM l_sql
   IF STATUS THEN
      CALL cl_err('pre gem:',STATUS,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE r182_cgem CURSOR FOR r182_pgem
   ...
   PREPARE r182_paao FROM l_sql
   IF STATUS THEN
      CALL cl_err('prepare aao:',STATUS,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE r182_caao CURSOR FOR r182_paao
 
#  CALL cl_outnam('aglr182') RETURNING l_name               #No.FUN-780060
#  START REPORT aglr182_rep TO l_name                       #No.FUN-780060  
...
#FUN-D70095--------------------------end 
   FOREACH r182_caag INTO l_aag01,l_aag02,l_aag13,l_aag221  #FUN-6C0012
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
 
      FOREACH r182_cgem INTO l_gem01
         INITIALIZE sr.* TO NULL            # Default condition
         #-->取基準月份
         OPEN r182_caao USING l_aag01,l_gem01
         FETCH r182_caao INTO sr.*
         ...
         END IF
#FUN-D70095--------------------------begin         
         OPEN r121_cesumc USING l_aag01,l_gem01,'1',tm.defyy,tm.defmm
         ...
         OPEN r121_cesumc USING l_aag01,l_gem01,'2',tm.defyy,tm.defmm
         FETCH r121_cesumc INTO l_CE_sum2   
         IF STATUS THEN CALL cl_err('sel abb:',STATUS,1) EXIT FOREACH END IF
         IF l_CE_sum2 IS NULL THEN LET l_CE_sum2 = 0 END IF
         LET sr.aao.aao05 = sr.aao.aao05 - l_CE_sum1
         LET sr.aao.aao06 = sr.aao.aao06 - l_CE_sum2
#FUN-D70095--------------------------end          
         #-->抓基準:  預算
#FUN-AB0020 --------------------Begin--------------         
         IF NOT cl_null(tm.afc01) THEN
...
#FUN-AB0020 --------------------End----------------            
         ...
         END IF
#FUN-D70095--------------------------begin         
         OPEN r121_cesumc USING l_aag01,l_gem01,'1',tm.comyy,tm.commm
         FETCH r121_cesumc INTO l_CE_sum1   
         IF STATUS THEN CALL cl_err('sel abb:',STATUS,1) EXIT FOREACH END IF
         IF l_CE_sum1 IS NULL THEN LET l_CE_sum1 = 0 END IF
         
         OPEN r121_cesumc USING l_aag01,l_gem01,'2',tm.comyy,tm.commm
         FETCH r121_cesumc INTO l_CE_sum2   
         IF STATUS THEN CALL cl_err('sel abb:',STATUS,1) EXIT FOREACH END IF
         IF l_CE_sum2 IS NULL THEN LET l_CE_sum2 = 0 END IF
...
#No.FUN-780060--end--
#        OUTPUT TO REPORT aglr182_rep(sr.*)
 
      END FOREACH
   END FOREACH
 
#   FINISH REPORT aglr182_rep
#No.FUN-780060--start--add
#   CALL cl_prt(l_name,g_prtway,g_copies,g_len)
    ...
    LET g_str=g_str,";'';",tm.defyy,";",tm.defmm,";",tm.comyy,";",tm.commm,";", #No.FUN-830139 tm.budget -> ''
              tm.merge,";",tm.bookno,";",tm.zero_sw,";",g_azi05,";",l_tit1,";",l_tit2
    LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                               
    CALL cl_prt_cs3('aglr182','aglr182',g_sql,g_str)
#No.FUN-780060--end--
END FUNCTION
#No.FUN-780060--start--mark
{REPORT aglr182_rep(sr)
   DEFINE sr        RECORD
                       aao  RECORD LIKE aao_file.*,
                       aag02       LIKE aag_file.aag02,   #科目名稱
                       aag13       LIKE aag_file.aag13,   #額外名稱 #FUN-6C0012
