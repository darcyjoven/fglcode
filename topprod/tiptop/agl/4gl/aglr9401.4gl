# Prog. Version..: '5.30.06-13.03.21(00009)'     #
#
# Pattern name...: aglr9401.4gl
...
# Modify.........: No:MOD-C90017 12/09/21 By Elise 取消r9401_str處理
# Modify.........: No:CHI-C60016 12/11/08 By Belle 修正累計盈虧的計算方式
# Modify.........: No.CHI-C90015 13/01/30 by Lori 合併現金流量表的本期損益BS期初數應含入"少數股權淨利"科餘
...
DEFINE g_aaz131    LIKE aaz_file.aaz131   #CHI-C60016
MAIN
   OPTIONS
       INPUT NO WRAP
   ...
   IF g_flag = 'N' THEN
      ...
      IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN 
         CALL r9401_tm()                         # Input print condition
      ELSE
         CALL r9401()                            # Read data and create out-file
      END IF
   ELSE
      CALL r9401()                               # Read data and create out-file
   END IF
...
FUNCTION r9401_tm()
   ...
   END IF
   OPEN WINDOW r9401_w AT p_row,p_col WITH FORM "agl/42f/aglr9401"
  ...
  #            "str4.type_file.chr50"
   ...
   LET g_sql = "type.type_file.chr1,",           
               "str1.type_file.num20_6,",
               "giqq02.type_file.chr1000,",
               "str2.type_file.num20_6,",        
               "giqq04.giqq_file.giqq04,",
               "str3.type_file.num20_6,",
               "str4.type_file.num20_6"
  #MOD-C90017---mod---E---

   LET l_table = cl_prt_temptable('aglr9401',g_sql) CLIPPED
   IF l_table = -1 THEN 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      EXIT PROGRAM 
   END IF
   ...
   WHILE TRUE
      ...
      INPUT BY NAME tm.axa01,tm.axa02,tm.b,tm.title
        ...
        #CHI-C40011---
         ...
         AFTER FIELD axa01
            ...
            CALL r9401_set_entry()
            CALL r9401_set_no_entry()
            IF tm.axa06 = '1' THEN
               LET tm.q1 = '' 
               LET tm.h1 = '' 
	       LET tm.q2 = ''
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW r9401_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
FUNCTION r9401()
   ...
   SELECT aaf03 INTO g_company FROM aaf_file
    ...
    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aglr9401'
...
FUNCTION r9401_cr()
  #DEFINE l_sql       LIKE type_file.chr1000,       # RDSQL STATEMENT  #MOD-C90017 mark
   DEFINE l_sql       STRING,                       #MOD-C90017
  ...
  #               END RECORD 
   DEFINE tmp1        RECORD
          type        LIKE type_file.chr1,
          str1        LIKE type_file.num20_6,
          giqq02      LIKE type_file.chr1000,
          str2        LIKE type_file.num20_6,
          giqq04      LIKE giqq_file.giqq04,
          str3        LIKE type_file.num20_6,
          str4        LIKE type_file.num20_6
                  END RECORD
  #MOD-C90017---mod---E
   DEFINE l_n         LIKE type_file.num10        #FUN-C30098
   DEFINE l_n1        LIKE type_file.num10        #FUN-C30098
   DEFINE l_giyy03    LIKE giyy_file.giyy03       #MOD-C90017 add
   DEFINE l_axh081    LIKE axh_file.axh08         #CHI-C60016
   DEFINE l_axh06     LIKE axh_file.axh06,        #CHI-C60016
          l_axh07     LIKE axh_file.axh07         #CHI-C60016
   CALL r9401_create_temp_table()
  ...
  #營業活動之現金流量
   ...
   LET l_flag = 'N'
  #=========================本期淨利(損)=========================#
   CALL r9401_axh5(g_aaz.aaz114,tm.y1 ,tm.m2) RETURNING l_amt            #CHI-C40004  r9401_axh-->r9401_axh5
   IF cl_null(l_amt) THEN LET l_amt = 0 END IF
   CALL r9401_axh5(g_aaz.aaz114,tm.y11,tm.m2) RETURNING l_amt_1          #CHI-C40004  r9401_axh-->r9401_axh5
  ...
  #將本期淨利(損)金額加至 營業活動產生之淨現金l_sub_amt
   ...
   LET l_tot_amt = l_amt       #MOD-C90017
   LET l_tot_amt_1 = l_amt_1   #MOD-C90017
  #MOD-C90017 mark---S---
  #IF l_amt >= 0 THEN
  #   LET l_str1 = cl_numfor(l_amt,20,tm.e)
  #ELSE
  #   CALL r9401_str(l_amt,l_str1) RETURNING l_str1
  #   LET l_str1 = l_str1 CLIPPED,')'
  #END IF
  #IF l_amt_1 >= 0 THEN
  #   LET l_str3 = cl_numfor(l_amt_1,20,tm.e)
  #ELSE
  #   CALL r9401_str(l_amt_1,l_str3) RETURNING l_str3
  ...
  #=========================營業科目=========================#
   ...
   PREPARE r9401_giqq_p1 FROM l_sql
   DECLARE r9401_giqq_c1 CURSOR FOR r9401_giqq_p1
   LET l_sql= "SELECT aag01,aag02,giww03,giww04 "
             ...
             ,"   AND giww05 = '",tm.axa01,"'"
             ,"   AND giww06 = '",tm.axa02,"'"
   PREPARE r9401_giww_p1 FROM l_sql
   DECLARE r9401_giww_c1 CURSOR FOR r9401_giww_p1
   FOREACH r9401_giqq_c1 INTO giqq.*
      LET l_i_1 = 1
      LET l_amt_s = 0
      LET l_amt_s_1 = 0
      FOREACH r9401_giww_c1 USING giqq.giqq01 INTO tmp.*
         LET l_amt = 0
         LET l_amt_1 = 0
         CASE tmp.giww04  
            WHEN '1'
              #CALL r9401_axh(tmp.aag01,tm.y2 ,tm.m2) RETURNING l_amt                            #FUN-C30098 mark
              #CALL r9401_axh(tmp.aag01,tm.y21,tm.m2) RETURNING l_amt_1                          #FUN-C30098 mark
               CALL r9401_axh3(tmp.aag01,tm.y2 ,tm.m2,giqq.giqq01) RETURNING l_amt               #FUN-C30098
               CALL r9401_axh3(tmp.aag01,tm.y21,tm.m2,giqq.giqq01) RETURNING l_amt_1             #FUN-C30098
            WHEN '2'
              #CALL r9401_axh2(tmp.aag01,l_last_y ,l_last_m) RETURNING l_amt                     #FUN-C30098 mark
              #CALL r9401_axh2(tmp.aag01,l_last_y1,l_last_m) RETURNING l_amt_1                   #FUN-C30098 mark
               CALL r9401_axh4(tmp.aag01,l_last_y ,l_last_m,giqq.giqq01) RETURNING l_amt         #FUN-C30098
               CALL r9401_axh4(tmp.aag01,l_last_y1,l_last_m,giqq.giqq01) RETURNING l_amt_1       #FUN-C30098
            WHEN '3'
              #CALL r9401_axh2(tmp.aag01,tm.y2 ,tm.m2) RETURNING l_amt                           #FUN-C30098 mark
              #CALL r9401_axh2(tmp.aag01,tm.y21,tm.m2) RETURNING l_amt_1                         #FUN-C30098 mark
               CALL r9401_axh4(tmp.aag01,tm.y2 ,tm.m2,giqq.giqq01) RETURNING l_amt               #FUN-C30098
               CALL r9401_axh4(tmp.aag01,tm.y21,tm.m2,giqq.giqq01) RETURNING l_amt_1             #FUN-C30098
            WHEN '4'
               ...
               LET l_i_1 = l_i_1 + 1
            WHEN '5'  #借方異動
              #CALL r9401_axh08(tmp.aag01,tm.y1 ,tm.m2,l_last_y ,l_last_m) RETURNING l_amt                 #FUN-C30098 mark 
              #CALL r9401_axh08(tmp.aag01,tm.y11,tm.m2,l_last_y1,l_last_m) RETURNING l_amt_1               #FUN-C30098 mark
               CALL r9401_axh081(tmp.aag01,tm.y1 ,tm.m2,l_last_y ,l_last_m,giqq.giqq01) RETURNING l_amt    #FUN-C30098
               CALL r9401_axh081(tmp.aag01,tm.y11,tm.m2,l_last_y1,l_last_m,giqq.giqq01) RETURNING l_amt_1  #FUN-C30098
            WHEN '6'  #貸方異動
              #CALL r9401_axh09(tmp.aag01,tm.y1 ,tm.m2,l_last_y ,l_last_m) RETURNING l_amt                 #FUN-C30098 mark 
              #CALL r9401_axh09(tmp.aag01,tm.y11,tm.m2,l_last_y1,l_last_m) RETURNING l_amt_1               #FUN-C30098 mark
               CALL r9401_axh091(tmp.aag01,tm.y1 ,tm.m2,l_last_y ,l_last_m,giqq.giqq01) RETURNING l_amt    #FUN-C30098 
               CALL r9401_axh091(tmp.aag01,tm.y11,tm.m2,l_last_y1,l_last_m,giqq.giqq01) RETURNING l_amt_1  #FUN-C30098
         END CASE
         IF cl_null(l_amt)   THEN LET l_amt   = 0 END IF
         IF cl_null(l_amt_1) THEN LET l_amt_1 = 0 END IF
     ...
     #ELSE
     #   CALL r9401_str(l_amt_s,l_str2) RETURNING l_str2
     #   LET l_str2 = l_str2 CLIPPED,')'
     #END IF
     #IF l_amt_s_1 >= 0 THEN
     #   LET l_str4 = cl_numfor(l_amt_s_1,20,tm.e)
     #ELSE
     #   CALL r9401_str(l_amt_s_1,l_str4) RETURNING l_str4
     #   LET l_str4 = l_str4 CLIPPED,')'
     #END IF
     #MOD-C90017 mark---E---
      LET l_flag = 'Y'
      IF tm.t = 'N' THEN 
        #INSERT INTO r9401_tmp VALUES (l_type,l_str1,giqq.giqq02,l_str2,giqq.giqq04,l_str3,l_str4)              #MOD-C90017 mark
         INSERT INTO r9401_tmp VALUES (l_type,l_tot_amt,giqq.giqq02,l_amt_s,giqq.giqq04,l_tot_amt_1,l_amt_s_1)  #MOD-C90017
      ELSE 
           ...
           #ELSE
           #   CALL r9401_str(l_gixx[l_i_2].gixx05,l_str2) RETURNING l_str2
           #   LET l_str2 = l_str2 CLIPPED,')'
           #END IF
           #IF l_gixx[l_i_2].gixx051 >= 0 THEN
           #   LET l_str4 = cl_numfor(l_gixx[l_i_2].gixx051,20,tm.e)
           #ELSE
           #   CALL r9401_str(l_gixx[l_i_2].gixx051,l_str4) RETURNING l_str4
           #   LET l_str4 = l_str4 CLIPPED,')'
           #END IF
           #MOD-C90017 mark---E---
           #INSERT INTO r9401_tmp VALUES (l_type,l_str1,l_gixx[l_i_2].giqq02,l_str2,giqq.giqq04,l_str3,l_str4)                                       #MOD-C90017 mark
            INSERT INTO r9401_tmp VALUES (l_type,l_tot_amt,l_gixx[l_i_2].giqq02,l_gixx[l_i_2].gixx05,giqq.giqq04,l_tot_amt_1,l_gixx[l_i_2].gixx051)  #MOD-C90017
      	 END FOR     
      END IF
   END FOREACH
   IF l_flag = 'N' THEN
     #INSERT INTO r9401_tmp VALUES (l_type,l_str1,'','',l_giqq04,l_str3,'')  #MOD-C90017 mark
      INSERT INTO r9401_tmp VALUES (l_type,l_tot_amt,'','',l_giqq04,l_tot_amt_1,'')  #MOD-C90017
   END IF
   LET l_tot_amt = 0     #MOD-C90017
   LET l_tot_amt_1 = 0   #MOD-C90017
  #=========================營業活動產生之淨現金=========================#
   LET l_type = '2' 
  ...
  #ELSE
  #   CALL r9401_str(l_sub_amt,l_str1) RETURNING l_str1
  #   LET l_str1 = l_str1 CLIPPED,')'
  #END IF
  #IF l_sub_amt_1 >= 0 THEN
  #   LET l_str3 = cl_numfor(l_sub_amt_1,20,tm.e)
  #ELSE
  #   CALL r9401_str(l_sub_amt_1,l_str3) RETURNING l_str3
  #   LET l_str3 = l_str3 CLIPPED,')'
  #END IF
  #MOD-C90017 mark---S---
  #INSERT INTO r9401_tmp VALUES (l_type,l_str1,'','',l_giqq04,l_str3,'')  #MOD-C90017 mark
   INSERT INTO r9401_tmp VALUES (l_type,l_sub_amt,'','',l_giqq04,l_sub_amt_1,'')  #MOD-C90017
   LET l_sub_amt = 0
   LET l_sub_amt_1 = 0
  ...
  #=========================投資活動之現金流量=========================#
   ...
   LET l_sql= "SELECT giqq01,giqq02,giqq04 FROM giqq_file "
             ," WHERE giqq03 = '2'"
             ," ORDER BY giqq04,giqq02"
   PREPARE r9401_giqq_p2 FROM l_sql
   DECLARE r9401_giqq_c2 CURSOR FOR r9401_giqq_p2
   LET l_sql= "SELECT aag01,aag02,giww03,giww04 "
             ,"  FROM aag_file,giww_file,giqq_file"
             ," WHERE aag00=giww00 AND aag01=giww02 AND giqq01=giww01 "
             ,"   AND aag00 = '",tm.b,"' "
             ...
             ,"   AND giww05 = '",tm.axa01,"'"
             ,"   AND giww06 = '",tm.axa02,"'"
   PREPARE r9401_giww_p2 FROM l_sql
   DECLARE r9401_giww_c2 CURSOR FOR r9401_giww_p2
   FOREACH r9401_giqq_c2 INTO giqq.*
      LET l_i_1 = 1
      LET l_amt_s = 0
      LET l_amt_s_1 = 0
      FOREACH r9401_giww_c2 USING giqq.giqq01 INTO tmp.*
         LET l_amt = 0
         LET l_amt_1 = 0
         CASE tmp.giww04
            WHEN '1'
     ...
     #MOD-C90017 mark---E---
      LET l_flag = 'Y'
      IF tm.t = 'N' THEN 
        #INSERT INTO r9401_tmp VALUES (l_type,'',giqq.giqq02,l_str2,giqq.giqq04,'',l_str4)  #MOD-C90017 mark
         INSERT INTO r9401_tmp VALUES (l_type,'',giqq.giqq02,l_amt_s,giqq.giqq04,'',l_amt_s_1)  #MOD-C90017
      ELSE 
      	 FOR l_i_2 = 1 TO l_i_1
            IF cl_null(l_gixx[l_i_2].gixx05) THEN
               LET l_gixx[l_i_2].gixx05 = 0
           ...
           #MOD-C90017 mark---E---
           #INSERT INTO r9401_tmp VALUES (l_type,l_str1,l_gixx[l_i_2].giqq02,l_str2,giqq.giqq04,l_str3,l_str4)  #MOD-C90017 mark
            INSERT INTO r9401_tmp VALUES (l_type,l_amt,l_gixx[l_i_2].giqq02,l_gixx[l_i_2].gixx05,giqq.giqq04,l_amt_1,l_gixx[l_i_2].gixx051)  #MOD-C90017
     	 END FOR
      END IF
   END FOREACH
   IF l_flag = 'N' THEN
      INSERT INTO r9401_tmp VALUES (l_type,'','','',l_giqq04,'','')
   END IF
  ...
  #理財活動之現金流量
   ...
   LET l_sql = "SELECT giqq01,giqq02,giqq04 FROM giqq_file "
              ," WHERE giqq03 = '3'"
              ," ORDER BY giqq04,giqq02"
   PREPARE r9401_giqq_p3 FROM l_sql
   DECLARE r9401_giqq_c3 CURSOR FOR r9401_giqq_p3
   LET l_sql= "SELECT aag01,aag02,giww03,giww04 "
             ,"  FROM aag_file,giww_file,giqq_file"
             ," WHERE aag00=giww00 AND aag01=giww02 AND giqq01=giww01 "
             ,"   AND aag00 = '",tm.b,"' "
             ...
             ,"   AND giww05 = '",tm.axa01,"'"
             ,"   AND giww06 = '",tm.axa02,"'"
   PREPARE r9401_giww_p3 FROM l_sql
   DECLARE r9401_giww_c3 CURSOR FOR r9401_giww_p3
   FOREACH r9401_giqq_c3 INTO giqq.*
      LET l_i_1 = 1
      LET l_amt_s = 0
      LET l_amt_s_1 = 0
      FOREACH r9401_giww_c3 USING giqq.giqq01 INTO tmp.*
         LET l_amt = 0
         LET l_amt_1 = 0
         CASE tmp.giww04  
            WHEN '1'
     ...
     #CHI-C60016---S---
      ...
      LET l_sql = l_sql CLIPPED
                 ...
                 ,"                    AND axh06 = '",l_axh06,"'"
                 ,"                    AND axh07 <= '",l_axh07,"')"
      PREPARE i9401_axh5_p5 FROM l_sql
      DECLARE i9401_axh5_c5 SCROLL CURSOR FOR i9401_axh5_p5
      OPEN i9401_axh5_c5
      FETCH i9401_axh5_c5 INTO l_axh081
      IF cl_null(l_axh081) THEN LET l_axh081 = 0 END IF  
      SELECT aaz131 INTO g_aaz131 FROM aaz_file
      IF giqq.giqq01 = g_aaz131 THEN 
         IF tm.m1 = 1 THEN
            LET l_amt_s = l_amt_s - l_axh081
         END IF
      END IF
     #CHI-C60016---E---
     #計算理財活動產生之淨現金
     ...
     #CHI-C60016---S---
      IF tm.m11=1 THEN 
         LET l_axh06 = tm.y11 - 1
         LET l_axh07 = 12
      ELSE
         LET l_axh06 = tm.y11 
         LET l_axh07 = tm.m11 - 1
      END IF
      IF l_type = '1' THEN
         LET l_sql = " SELECT SUM(axh08-axh09) FROM axh_file"
      ELSE
      ...
      LET l_sql = l_sql CLIPPED
                 ...
                 ,"                    AND axh06 = '",l_axh06,"'"
                 ,"                    AND axh07 <= '",l_axh07,"')"
      PREPARE i9401_axh5_p6 FROM l_sql
      DECLARE i9401_axh5_c6 SCROLL CURSOR FOR i9401_axh5_p6
      OPEN i9401_axh5_c6
      FETCH i9401_axh5_c6 INTO l_axh081
      IF cl_null(l_axh081) THEN LET l_axh081 = 0 END IF
      SELECT aaz131 INTO g_aaz131 FROM aaz_file
      IF giqq.giqq01 = g_aaz131 THEN
         IF tm.m1 = 1  THEN   
            LET l_amt_s_1 = l_amt_s_1 - l_axh081
         END IF                  
      END IF
     #CHI-C60016---E---
      LET l_sub_amt_1 = l_sub_amt_1 + l_amt_s_1
     ...
     #MOD-C90017 mark---E---
      ...
      ELSE 
         FOR l_i_2 = 1 TO l_i_1
           ...
           #MOD-C90017 mark---E---
           #INSERT INTO r9401_tmp VALUES (l_type,l_str1,l_gixx[l_i_2].giqq02,l_str2,giqq.giqq04,l_str3,l_str4)  #MOD-C90017 mark
            INSERT INTO r9401_tmp VALUES (l_type,l_sub_amt,l_gixx[l_i_2].giqq02,l_gixx[l_i_2].gixx05,giqq.giqq04,l_sub_amt_1,l_gixx[l_i_2].gixx051)  #MOD-C90017
    	 END FOR     
      END IF
   END FOREACH
   IF l_flag = 'N' THEN
  ...
  #   LET l_str3 = cl_numfor(l_amt_1,20,tm.e)
  #ELSE
  #  #CALL r9401_str(l_amt,l_str3) RETURNING l_str3           #FUN-C30098 mark
  #   CALL r9401_str(l_amt_1,l_str3) RETURNING l_str3         #FUN-C30098
  #   LET l_str3 = l_str3 CLIPPED,')'
  #END IF
  #MOD-C90017---E---
  #INSERT INTO r9401_tmp VALUES (l_type,l_str1,'','',l_giqq04,l_str3,'')  #MOD-C90017 mark
   INSERT INTO r9401_tmp VALUES (l_type,l_amt,'','',l_giqq04,l_amt_1,'')  #MOD-C90017
  ...
  #FUN-C30098---End Mark---
   ...
   LET l_sql = "SELECT giqq01,giqq02,giqq04 FROM giqq_file "
              ," WHERE giqq03 = '4'"
              ," ORDER BY giqq04,giqq02"
   PREPARE r9401_giqq_p4 FROM l_sql
   DECLARE r9401_giqq_c4 CURSOR FOR r9401_giqq_p4
   LET l_sql= "SELECT aag01,aag02,giww03,giww04 "
             ,"  FROM aag_file,giww_file,giqq_file"
             ," WHERE aag00 = giww00 AND aag01 = giww02 AND giqq01 = giww01 "
             ,"   AND aag00 = '",tm.b,"' "
             ...
             ,"   AND giww05 = '",tm.axa01,"'"
             ,"   AND giww06 = '",tm.axa02,"'"
   PREPARE r9401_giww_p4 FROM l_sql
   DECLARE r9401_giww_c4 CURSOR FOR r9401_giww_p4
   FOREACH r9401_giqq_c4 INTO giqq.*
      LET l_i_1 = 1
      LET l_amt_s = 0
      LET l_amt_s_1 = 0
      FOREACH r9401_giww_c4 USING giqq.giqq01 INTO tmp.*
         LET l_amt = 0
         LET l_amt_1 = 0
        #FUN-C30098--Begin--
         LET g_sql = " SELECT gizz05 FROM gizz_file"
        ...
        #FUN-C30098---End---
         CASE tmp.giww04
            WHEN '1'
              #IF l_last = 0 THEN        #FUN-C30098 mark
               IF l_n = 0 THEN           #FUN-C30098
                  CALL r9401_axh(tmp.aag01,tm.y2,tm.m2) RETURNING l_amt
               END IF
              #IF l_last_1 = 0 THEN      #FUN-C30098 mark
               IF l_n1 = 0 THEN          #FUN-C30098
                  CALL r9401_axh(tmp.aag01,tm.y21,tm.m2) RETURNING l_amt_1
               END IF
            WHEN '2'
              #IF l_last = 0 THEN        #FUN-C30098 mark
               IF l_n = 0 THEN           #FUN-C30098
                  CALL r9401_axh2(tmp.aag01,l_last_y,l_last_m) RETURNING l_amt
               END IF
              #IF l_last_1 = 0 THEN      #FUN-C30098 mark
               IF l_n1 = 0 THEN          #FUN-C30098
                  CALL r9401_axh2(tmp.aag01,l_last_y1,l_last_m) RETURNING l_amt_1
               END IF
            WHEN '3'
              #IF l_last = 0 THEN        #FUN-C30098 mark
               IF l_n = 0 THEN           #FUN-C30098
                  CALL r9401_axh2(tmp.aag01,tm.y2,tm.m2) RETURNING l_amt
               END IF
              #IF l_last_1 = 0 THEN      #FUN-C30098 mark
               IF l_n1 = 0 THEN          #FUN-C30098
                  CALL r9401_axh2(tmp.aag01,tm.y21,tm.m2) RETURNING l_amt_1
               END IF
            WHEN '4'
               LET l_str_1 = tmp.aag01
               LET l_len = LENGTH(l_str_1)
            ...
            WHEN '5'  #借方異動
              #IF l_last = 0 THEN        #FUN-C30098 mark
               IF l_n = 0 THEN           #FUN-C30098
                  CALL r9401_axh08(tmp.aag01,tm.y1 ,tm.m2,l_last_y ,l_last_m) RETURNING l_amt 
               END IF
              #IF l_last_1 = 0 THEN      #FUN-C30098 mark
               IF l_n1 = 0 THEN          #FUN-C30098
                  CALL r9401_axh08(tmp.aag01,tm.y11,tm.m2,l_last_y1,l_last_m) RETURNING l_amt_1
               END IF
            WHEN '6'  #貸方異動
              #IF l_last = 0 THEN        #FUN-C30098 mark
               IF l_n = 0 THEN           #FUN-C30098
                  CALL r9401_axh09(tmp.aag01,tm.y1 ,tm.m2,l_last_y ,l_last_m) RETURNING l_amt   
               END IF
              #IF l_last_1 = 0 THEN      #FUN-C30098 mark
               IF l_n1 = 0 THEN          #FUN-C30098
                  CALL r9401_axh09(tmp.aag01,tm.y11,tm.m2,l_last_y1,l_last_m) RETURNING l_amt_1
               END IF
         END CASE
         IF cl_null(l_amt)   THEN LET l_amt   = 0 END IF
         IF cl_null(l_amt_1) THEN LET l_amt_1 = 0 END IF
     ...
     #IF tm.t = 'N' THEN #TQC-C50139 mark
        #INSERT INTO r9401_tmp VALUES (l_type,l_str1,giqq.giqq02,l_str2,giqq.giqq04,l_str3,l_str4)  #MOD-C90017 mark
         INSERT INTO r9401_tmp VALUES (l_type,l_this,giqq.giqq02,l_amt_s,giqq.giqq04,l_this_1,l_amt_s_1)  #MOD-C90017
     #TQC-C50139--mark--str
     #ELSE
     #   FOR l_i_2 = 1 TO l_i_1
     #      IF cl_null(l_gixx[l_i_2].gixx05) THEN
     ...
     #         LET l_str4 = l_str4 CLIPPED,')'
     #      END IF
     #      INSERT INTO r9401_tmp VALUES (l_type,l_str1,l_gixx[l_i_2].giqq02,l_str2,giqq.giqq04,l_str3,l_str4)
     #   END FOR     
     #END IF 
     #TQC-C50139--mark--end
   END FOREACH
   IF l_flag = 'N' THEN
      INSERT INTO r9401_tmp VALUES (l_type,'0','','',l_giqq04,'0','')
   END IF
  #=========================期末現金及約當現金餘額=========================#
   LET l_type ='A'
   LET l_flag = 'N'
   ...
   LET l_sql = "SELECT giqq01,giqq02,giqq04 FROM giqq_file "
              ," WHERE giqq03 = '5'"
              ," ORDER BY giqq04,giqq02"
   PREPARE r9401_giqq_p5 FROM l_sql
   DECLARE r9401_giqq_c5 CURSOR FOR r9401_giqq_p5
   LET l_sql= "SELECT aag01,aag02,giww03,giww04 "
             ,"  FROM aag_file,giww_file,giqq_file"
             ," WHERE aag00=giww00 AND aag01=giww02 AND giqq01=giww01 "
             ,"   AND aag00 = '",tm.b,"' "
             ...
             ,"   AND giww05 = '",tm.axa01,"'"
             ,"   AND giww06 = '",tm.axa02,"'"
   PREPARE r9401_giww_p5 FROM l_sql
   DECLARE r9401_giww_c5 CURSOR FOR r9401_giww_p5
   FOREACH r9401_giqq_c5 INTO giqq.*
      LET l_i_1 = 1
      LET l_amt_s = 0
      LET l_amt_s_1 = 0
      FOREACH r9401_giww_c5 USING giqq.giqq01 INTO tmp.*
         LET l_amt = 0
         LET l_amt_1 = 0      
         CASE tmp.giww04 
            WHEN '1'
     ...
     #IF tm.t = 'N' THEN #TQC-C50139 mark
        #INSERT INTO r9401_tmp VALUES (l_type,l_str1,giqq.giqq02,l_str2,giqq.giqq04,l_str3,l_str4)  #MOD-C90017 mark
         INSERT INTO r9401_tmp VALUES (l_type,l_last,giqq.giqq02,l_amt_s,giqq.giqq04,l_last_1,l_amt_s_1)  #MOD-C90017
     #TQC-C50139--mark--str
     #ELSE 
     #	 FOR l_i_2 = 1 TO l_i_1
     #      IF cl_null(l_gixx[l_i_2].gixx05) THEN
  ...
  #   LET l_diff = l_last - (l_this  + l_amt)          #FUN-C30098 mark
   ...
   END IF    #TQC-C50218   add
    ...
    #    LET l_str1 =  cl_numfor(l_diff,20,tm.e)
    # ELSE
    #    CALL r9401_str(l_diff,l_str1) RETURNING l_str1
    #    LET l_str1 = l_str1 CLIPPED,")"
    # END IF 
    ##FUN-C30098---End---
    #MOD-C90017 mark---E---
  ...
  #   LET l_diff_1 = l_last_1 - (l_this_1  + l_amt_1)     #FUN-C30098 mark
   ...
   END IF   #TQC-C50218   add
    ...
    #    LET l_str3 =  cl_numfor(l_diff_1,20,tm.e)
    # ELSE
    #    CALL r9401_str(l_diff_1,l_str3) RETURNING l_str3
    #    LET l_str3 = l_str3 CLIPPED,")"
    # END IF 
    ##FUN-C30098---End---
    #MOD-C90017 mark---E---
   #END IF    #TQC-C50218   mark
  #INSERT INTO r9401_tmp VALUES (l_type,l_str1,'','',l_giqq04,l_str3,'')  #MOD-C90017 mark
   INSERT INTO r9401_tmp VALUES (l_type,l_diff,'','',l_giqq04,l_diff_1,'')  #MOD-C90017
   ...
   IF tm.s = 'Y' THEN
     ...
     #MOD-C90017 修正揭露事項 ---(S)
      LET g_sql = "SELECT giyy03 FROM giyy_file",
          " WHERE giyy06 = '",tm.axa01,"' AND giyy07 = '",tm.axa02,"' ",
          "  AND giyy04 = ",tm.y11," AND giyy05 BETWEEN ",tm.m11," AND ",tm.m21,
         #"  AND giyy01=? AND giyy02=? "   #CHI-D10047 mark
          "  AND giyy02=? "                #CHI-D10047
      PREPARE r9401_fgiyy_p FROM g_sql
      DECLARE r9401_fgiyy_c SCROLL CURSOR FOR r9401_fgiyy_p
     #MOD-C90017 修正揭露事項 ---(E)

      DECLARE r9401_giyy CURSOR FOR
      ...
      FOREACH r9401_giyy INTO g_giyy[g_cnt].*
         IF SQLCA.sqlcode THEN
        ...
        #MOD-C90017---add---S---
         LET l_giyy03=0
        #OPEN r9401_fgiyy_c USING g_giyy[g_cnt].giyy01,g_giyy[g_cnt].giyy02   #CHI-D10047 mark
         OPEN r9401_fgiyy_c USING g_giyy[g_cnt].giyy02                        #CHI-D10047
         FETCH FIRST r9401_fgiyy_c INTO l_giyy03
         IF cl_null(l_giyy03) THEN LET l_giyy03 = 0 END IF
         INSERT INTO r9401_tmp VALUES (l_type,'',g_giyy[g_cnt].giyy02,g_giyy[g_cnt].giyy03,g_giyy[g_cnt].giyy01,'',l_giyy03)
         LET l_flag = 'Y'
        #MOD-C90017---add---E---
         LET g_cnt = g_cnt + 1
     ...
     #   LET l_flag = 'Y'
     #   INSERT INTO r9401_tmp VALUES (l_type,'',g_giyy[i].giyy02,l_str2,g_giyy[i].giyy01,'',l_str4)
     #END FOR 
     #MOD-C90017 mark---E
     #CHI-D10047--str
      LET l_giyy03= 0
      DECLARE r9401_giyy_1 CURSOR FOR
         SELECT giyy01,giyy02,0,giyy03 FROM giyy_file a
          WHERE giyy06 = tm.axa01 AND giyy07 = tm.axa02
            AND giyy04 = tm.y1-1 AND giyy05 BETWEEN tm.m1 AND tm.m2
            AND NOT EXISTS(SELECT * FROM giyy_file
                            WHERE giyy06 = tm.axa01 AND giyy07 = tm.axa02
                              AND giyy04 = tm.y1 AND giyy05 BETWEEN tm.m1 AND tm.m2 AND giyy02 = a.giyy02)
          ORDER BY giyy04,giyy05,giyy01                                          
      FOREACH r9401_giyy_1 INTO g_giyy[g_cnt].*,l_giyy03
         IF SQLCA.sqlcode THEN
             CALL cl_err('foreach:',SQLCA.sqlcode,1)  
             EXIT FOREACH
         END IF
  ...
  #FUN-C30098--
   ...
   LET l_sql = " SELECT * FROM r9401_tmp "
              ," ORDER BY type,giqq04,giqq02 "
   PREPARE r9401_p1 FROM l_sql
   DECLARE r9401_c1 CURSOR FOR r9401_p1
   FOREACH r9401_c1 INTO tmp1.*
     #MOD-C90017 mark---S---
     #CASE tmp1.type
     #   WHEN '9'
     #     #FUN-C30098--
     #      SELECT SUM(str2) INTO l_last   FROM r9401_tmp WHERE l_type ='9'
     #      SELECT SUM(str4) INTO l_last_1 FROM r9401_tmp WHERE l_type ='9'
     #      IF cl_null(l_last) THEN LET l_last = 0 END IF
     #      IF cl_null(l_last_1) THEN LET l_last_1 = 0 END IF
     #     #FUN-C30098--
     #      IF l_last >=0 THEN
     ...
     #      ELSE
     #         CALL r9401_str(l_last,tmp1.str1) RETURNING tmp1.str1
     #         LET tmp1.str1 = tmp1.str1 CLIPPED,')'                   #FUN-C40004 mod
     #      END IF
     #      IF l_last_1 >=0 THEN
     #         LET tmp1.str3 = cl_numfor(l_last_1,20,tm.e)
     #      ELSE
     #         CALL r9401_str(l_last_1,tmp1.str3) RETURNING tmp1.str3
     #         LET tmp1.str3 = tmp1.str3 CLIPPED,')'                   #FUN-C40004 mod
     #      END IF
     #   WHEN 'A'
     #     #FUN-C30098--
     #     #SELECT SUM(str2) INTO l_amt   FROM r9401_tmp WHERE l_type ='9'
     #     #SELECT SUM(str4) INTO l_amt_1 FROM r9401_tmp WHERE l_type ='9'
     #     #FUN-C30098--
     #      SELECT SUM(str2) INTO l_amt   FROM r9401_tmp WHERE l_type ='A'  #FUN-C30098
     #      SELECT SUM(str4) INTO l_amt_1 FROM r9401_tmp WHERE l_type ='A'  #FUN-C30098
     #      IF cl_null(l_amt) THEN LET l_amt = 0 END IF
     #      IF cl_null(l_amt_1) THEN LET l_amt_1 = 0 END IF
     #      IF l_amt >=0 THEN
     #         LET tmp1.str1 = cl_numfor(l_amt,20,tm.e)
     #      ELSE
     #         CALL r9401_str(l_amt,tmp1.str1) RETURNING tmp1.str1
     #         LET tmp1.str1 = tmp1.str1 CLIPPED,')'                   #FUN-C40004 mod
     #      END IF
     #      IF l_amt_1 >=0 THEN
     #         LET tmp1.str3 = cl_numfor(l_amt_1,20,tm.e)
     #      ELSE
     #         CALL r9401_str(l_amt_1,tmp1.str3) RETURNING tmp1.str3
     #         LET tmp1.str3 = tmp1.str3 CLIPPED,')'                   #FUN-C40004 mod
     #      END IF
     #END CASE
     #MOD-C90017 mark---E---
  ...
  #LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                           #FUN-C30098 mark
   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED," ORDER BY type,giqq04"   #FUN-C30098
   LET g_str = ''
   LET g_str = tm.d,";",tm.title,";",tm.p,";","",";",tm.y1,";",tm.m1,";",
               l_d1,";",tm.y2,";",tm.m2,";",l_d2,";",tm.e   #MOD-C90017 add tm.e
   CALL cl_prt_cs3('aglr9401','aglr9401',g_sql,g_str)
        
END FUNCTION

FUNCTION r9401_axh(p_axh05,p_axh06,p_axh07)
  ...
  #DEFINE l_sql       LIKE type_file.chr1000      #MOD-C90017 mark
   DEFINE l_sql       STRING                      #MOD-C90017
...
FUNCTION r9401_axh2(p_axh05,p_axh06,p_axh07)
   DEFINE p_axh05     LIKE axh_file.axh05,
          p_axh06     LIKE axh_file.axh06,
          p_axh07     LIKE axh_file.axh07,
          l_axh08     LIKE axh_file.axh08, 
  ...
  #FUN-C30098--
   ...
   RETURN l_axh08
END FUNCTION
FUNCTION r9401_axh08(p_axh05,p_axh06,p_axh07,p_axh061,p_axh071)
   DEFINE p_axh05     LIKE axh_file.axh05,
          p_axh06     LIKE axh_file.axh06,
          p_axh07     LIKE axh_file.axh07,
...
FUNCTION r9401_axh09(p_axh05,p_axh06,p_axh07,p_axh061,p_axh071)
   DEFINE p_axh05     LIKE axh_file.axh05,
          p_axh06     LIKE axh_file.axh06,
          p_axh07     LIKE axh_file.axh07,
...
#MOD-C90017 mark-----S-----
#FUNCTION r9401_str(p_amt,p_str)
...
FUNCTION r9401_set_entry()
   CALL cl_set_comp_entry("q1,m1,h1,q2,m2,h2",TRUE)
END FUNCTION
FUNCTION r9401_set_no_entry()
...
#MOD-C90017----mark----S
#FUNCTION r9401_create_temp_table()
#   DROP TABLE r9401_tmp
#   CREATE TEMP TABLE r9401_tmp(
#   type   LIKE type_file.chr1,
#   str1   LIKE type_file.chr50,
#   giqq02 LIKE type_file.chr1000,
#   str2   LIKE type_file.chr50,
...
#FUN-C30098--Begin--
FUNCTION r9401_axh3(p_axh05,p_axh06,p_axh07,p_gizz07)
   DEFINE p_axh05     LIKE axh_file.axh05,
          p_axh06     LIKE axh_file.axh06,
          p_axh07     LIKE axh_file.axh07,
...
FUNCTION r9401_axh4(p_axh05,p_axh06,p_axh07,p_gizz07)
   DEFINE p_axh05     LIKE axh_file.axh05,
          p_axh06     LIKE axh_file.axh06,
          p_axh07     LIKE axh_file.axh07,
          p_gizz07    LIKE gizz_file.gizz07,
  ...
  #DEFINE l_sql       LIKE type_file.chr1000  #MOD-C90017 mark
   ...
   RETURN l_axh08
END FUNCTION
FUNCTION r9401_axh081(p_axh05,p_axh06,p_axh07,p_axh061,p_axh071,p_gizz07)
   DEFINE p_axh05     LIKE axh_file.axh05,
          p_axh06     LIKE axh_file.axh06,
          p_axh07     LIKE axh_file.axh07,
          p_axh08     LIKE axh_file.axh08,
  ...
  #DEFINE l_sql       LIKE type_file.chr1000  #MOD-C90017 mark
   ...
   ELSE
      LET l_sql = " SELECT axh08 FROM axh_file"
                 ...
                 ,"    AND axh02 = '",tm.axa02,"'"
                 ,"    AND axh05 = '",p_axh05,"'"
                #,"    AND axh06 = '",p_axh061,"'"    #CHI-C60016 mark
                 ,"    AND axh06 = '",l_axh06,"'"     #CHI-C60016
                 ,"    AND axh07 = (SELECT MAX(axh07) FROM axh_file "
                 ,"                  WHERE axh00 = '",g_aaz641,"'"
                 ,"                    AND axh01 = '",tm.axa01,"'"
                 ,"                    AND axh02 = '",tm.axa02,"'"
...
FUNCTION r9401_axh091(p_axh05,p_axh06,p_axh07,p_axh061,p_axh071,p_gizz07)
   DEFINE p_axh05     LIKE axh_file.axh05,
          p_axh06     LIKE axh_file.axh06,
          p_axh07     LIKE axh_file.axh07,
          p_axh09     LIKE axh_file.axh09,
...
#FUN-C30098---ENd---
#CHI-C40004--Begin--
FUNCTION r9401_axh5(p_axh05,p_axh06,p_axh07)
   DEFINE p_axh05     LIKE axh_file.axh05,
          p_axh06     LIKE axh_file.axh06,
          p_axh07     LIKE axh_file.axh07,
          l_axh08     LIKE axh_file.axh08, 
  ...
  #DEFINE l_sql       LIKE type_file.chr1000  #MOD-C90017 mark
   DEFINE l_sql       STRING                  #MOD-C90017
   DEFINE l_n         LIKE type_file.num10
  #DEFINE l_aaz129    LIKE axh_file.axh08     #CHI-C90015
  #DEFINE l_aaz129_1  LIKE axh_file.axh08     #CHI-C90015
  #DEFINE l_type1     LIKE type_file.chr1     #CHI-C90015

   SELECT aag06 INTO l_type FROM aag_file WHERE aag01 = p_axh05
                                            AND aag00 = tm.b 

  ##CHI-C90015 start--
  #SELECT aag06 INTO l_type1 FROM aag_file WHERE aag01 = g_aaz.aaz129
  #                                         AND aag00 = tm.b
  ##CHI-C90015 end---
  ...
  #   #--CHI-C90015--start--此處要取出少數股權淨利(aaz129)科餘，與本期損益BS(aaz114)科目相加--
  #   LET l_aaz129 = 0
  #   IF l_type1 = '1' THEN
  #      LET l_sql = " SELECT SUM(axh08-axh09) FROM axh_file"
  #   ELSE
  ...
  #              ,"                    AND axh06 = '",l_axh06,"'"
  #              ,"                    AND axh07 <= '",l_axh07,"')"
  #   PREPARE i9401_axh5_p3 FROM l_sql
  #   DECLARE i9401_axh5_c3 SCROLL CURSOR FOR i9401_axh5_p3
  #   OPEN i9401_axh5_c3
  #   FETCH i9401_axh5_c3 INTO l_aaz129
  #   IF cl_null(l_aaz129) THEN LET l_aaz129 = 0 END IF
  #   LET l_axh081 = l_axh081 + l_aaz129
   END IF
  ##--CHI-C90015 end------------------------------------------------
   IF l_type = '1' THEN
      SELECT SUM(axh08-axh09) INTO l_axh08 FROM axh_file
  ...
  #LET l_aaz129_1 = 0
  #IF l_type1 = '1' THEN
  #   SELECT SUM(axh08-axh09) INTO l_aaz129_1 FROM axh_file
  #    WHERE axh00 = tm.b AND axh05=g_aaz.aaz129
  #      AND axh01 = tm.axa01 AND axh02 = tm.axa02
  #      AND axh06 = p_axh06 AND axh07 = tm.m2
  #ELSE
  #   SELECT SUM(axh09-axh08) INTO l_aaz129_1 FROM axh_file
  #    WHERE axh00 = tm.b AND axh05=g_aaz.aaz129
  #      AND axh01 = tm.axa01 AND axh02 = tm.axa02
  #      AND axh06 = p_axh06 AND axh07 = tm.m2
  #END IF
  #IF cl_null(l_aaz129_1) THEN LET l_aaz129_1 = 0 END IF
  #LET l_axh08 = l_axh08 + l_aaz129_1
  ##--CHI-C90015 end------
