...
#
# Pattern name...: aglu813.4gl
# Descriptions...: 分類帳資料清除作業
# Input parameter: 
# Return code....: 
# Date & Author..: 92/06/01 By Nora
  ...
  INPUT NO WRAP
   ...
   CALL cl_used(g_prog,g_time,1) RETURNING g_time #No.FUN-690114
	...
	IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN	# If background job sw is off
		CALL u813_tm()						# Input condition
	ELSE
		CALL aglu813()						# Read data and process it
	END IF
...
FUNCTION u813_tm()
DEFINE
	p_row,p_col	LIKE type_file.num5,          #No.FUN-680098 SMASLLINT
   ...
   ELSE LET p_row = 4 
   END IF
	OPEN WINDOW u813_w AT p_row,p_col
		WITH FORM "agl/42f/aglu813" 
################################################################################
      ...
      ON ACTION help          #MOD-4C0121
		...
		IF g_bgjob = 'Y' THEN
			...
			ELSE
				...
				CALL cl_cmdat('aglu813',g_time,l_cmd)
			END IF
			CLOSE WINDOW u813_w
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
			EXIT PROGRAM
...
DEFINE
	...
	IF tm.download='Y' THEN
		LET l_sql=
			...
			" select * from aea_file",
			' where aea00="',g_bookno,'" and ',tm.wc CLIPPED,
			"'"
	...
	IF tm.cleanup='Y' THEN
		MESSAGE "delete data ..."
		LET l_sql="DELETE FROM aea_file ",
			" WHERE aea00='",g_bookno,"' AND ",tm.wc CLIPPED
		PREPARE u813_del FROM l_sql
