...
# Modify.........: No.TQC-760105 07/06/12 By dxfwo aglr907(總分類日計帳) 科目層級無法輸入!
...
#No.FUN-790008---Begin                                                                                                              
   ...
   LET l_table = cl_prt_temptable('aglr907',g_sql) CLIPPED                                                                          
   IF l_table = -1 THEN EXIT PROGRAM END IF                                                                                         
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,                                                                  
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?)"   #MOD-850130 add ?
...
DEFINE li_chk_bookno  LIKE type_file.num5             #No.FUN-B20054
   ...
   OPEN WINDOW aglr907_w AT p_row,p_col
     WITH FORM "agl/42f/aglr907"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
#No.FUN-B20054--add--end-- 
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglr907',g_time,l_cmd)      # Execute cmd at later time
         END IF
...
#       l_time          LIKE type_file.chr8          #No.FUN-6A0073
   ...
   #No.A056
   LET l_sql1= "SELECT aag01,aag02,aag13,aag07,aag24,'',0,0,0,0,0,0,0",  #FUN-6C0012
               "  FROM aag_file ",
   ...
   PREPARE aglr907_prepare2 FROM l_sql1
   IF SQLCA.sqlcode THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE aglr907_curs2 CURSOR FOR aglr907_prepare2
 
#  CALL cl_outnam('aglr907') RETURNING l_name   #No.FUN-790008
#  START REPORT aglr907_rep TO l_name           #No.FUN-790008
   CALL cl_del_data(l_table)                    #No.FUN-790008
 
   LET g_pageno = 0
 
   FOREACH aglr907_curs2 INTO sr.*
      IF STATUS THEN
...
#No.FUN-790008---End 
#No.FUN-790008---Begin
#        OUTPUT TO REPORT aglr907_rep(sr.*)
   ...
   END FOREACH
    ...
    IF g_zz05 = 'Y' THEN                                                                                                            
       ...
       LET g_str = tm.wc                                                                                                            
    END IF                                                                                                                          
     LET g_str = tm.wc,";",tm.e,";",g_azi04                                                                                         
     LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED                                                             
     CALL cl_prt_cs3('aglr907','aglr907',l_sql,g_str)
 
#  FINISH REPORT aglr907_rep
