...
#                                                 請依照4gl里的CR Temptable欄位,重新產生aglg725.xml
...
# Modify.........: NO:FUN-B90028 11/09/10 By qirl  統計分析CR轉GR
# Modify.........: No.TQC-C50042 12/05/23 By zhangweib 修改開窗q_mai去除報表性質為5\6的資料
...
TYPE sr1_t RECORD
    ...
    per3 LIKE type_file.num20_6,
    per4 LIKE type_file.num20_6,
...
#MOD-960232   ---end                                                                         
   ...
   LET l_table = cl_prt_temptable('aglg725',g_sql) CLIPPED   # 產生Temp Table                                                       
...
FUNCTION g725_tm()
   ...
   OPEN WINDOW g725_w AT p_row,p_col
     WITH FORM "agl/42f/aglg725"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
#              CALL cl_err(tm.p,'agl-109',0)    #No.FUN-660123
         ...
         ON ACTION CONTROLP
            IF INFIELD(a) THEN
              ...
              #LET g_qryparam.where = " mai00 = '",tm.b,"'"     #No.FUN-740020  #No.TQC-C50042   Add
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglg725',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#MOD-960232   ---end
   ...
   IF STATUS THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g725_c CURSOR FOR g725_p
 ...
 # CALL cl_outnam('aglg725') RETURNING l_name        #No.FUN-780059--mark
 # START REPORT g725_rep TO l_name                   #No.FUN-780059--mark
...
#           CALL cl_err('sel aef2:',STATUS,1)   #No.FUN-660123
    ...
    # OUTPUT TO REPORT g725_rep(maj.*, sr.*)               #No.FUN-780059--mark
   END FOREACH
   #--str FUN-780059 add--#
   ## **** 與 Crystal Reports 串聯段 - <<<< CALL cs3() >>>> CR11 **** ##                                                            
###GENGRE###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                                 
###GENGRE###   LET g_str = g_mai02,";",tm.a,";",tm.d,";",tm.p,";",tm.x,";",tm.title1,";",tm.title2,";",                                                                   
###GENGRE###               l_tit1,";",l_tit2,";",tm.e,";",g_basetot1,";",g_basetot2  #CHI-A70046 add g_basetot1,g_basetot2                                                                              
###GENGRE###   CALL cl_prt_cs3('aglg725','aglg725',g_sql,g_str)                                                                                 
    CALL aglg725_grdata()    ###GENGRE###
   #------------------------------ CR (4) ------------------------------#
   #--end FUN-780059 add--#
 ...
 # FINISH REPORT g725_rep                                 #No.FUN-780059--mark
...
FUNCTION aglg725_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg725")
        IF handler IS NOT NULL THEN
            START REPORT aglg725_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg725_datacur1 CURSOR FROM l_sql
            FOREACH aglg725_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg725_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg725_rep
        END IF
...
#FUN-B90028------------END--------
        ...
        ON EVERY ROW
#FUN-B90028--------STAR-----
            LET l_per3_fmt = cl_gr_numfmt('type_file','num20_6',tm.e)
            PRINTX l_per3_fmt  
            LET l_per4_fmt = cl_gr_numfmt('type_file','num20_6',tm.e)
            PRINTX l_per4_fmt  
...
#FUN-B90028------STAR----
        IF NOT cl_null(g_basetot1) THEN 
            ...
            LET l_per1 = (sr1.per3/l_per2)*100 USING '###.&&'
        ELSE 
            LET l_per1 = " "
        END IF
            PRINTX l_per1

        IF NOT cl_null(g_basetot2) THEN
            LET l_per4 = g_basetot2
            LET l_per2 = (sr1.per4/l_per4)*100 USING '###.&&'
        ELSE 	
            LET l_per2 = " "
        END IF  
            PRINTX l_per2
