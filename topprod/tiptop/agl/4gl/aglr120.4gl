...
# Modify.........: No.MOD-CB0178 12/11/20 By Polly 修正無法轉換語言別
...
FUNCTION r120_tm()
   ...
   OPEN WINDOW r120_w AT p_row,p_col WITH FORM "agl/42f/aglr120"
   ...
   WHILE TRUE
      ...
      INPUT BY NAME tm.rtype,tm.b,tm.a,tm.dept,tm.title1,tm.yy1,
         ...
         ON ACTION locale
            ...
            CALL cl_dynamic_locale()                  #MOD-CB0178 add
            CALL cl_show_fld_cont()                   #MOD-CB0178 add
...
FUNCTION r120()
  ...
  #tm.s-列印下階部門
   ...
   FOREACH r120_c INTO maj.*
      ...
      LET l_CE_sum1 = 0   #No.FUN-D70095 add
      LET l_CE_sum2 = 0   #No.FUN-D70095 add     
...
#FUN-AB0020 ---------------------Begin---------------------------
            ...
            END IF
#FUN-AB0020 ---------------------End-----------------------------            
            PREPARE r120_p3 FROM l_sql
            IF STATUS THEN
               CALL cl_err('prepare3:',STATUS,1)
               EXIT FOREACH
            END IF
            DECLARE r120_c3 CURSOR FOR r120_p3
            IF STATUS THEN
               CALL cl_err('r120_c3',STATUS,1)
               EXIT FOREACH
            END IF
            OPEN r120_c3
            FETCH r120_c3 INTO amt1_1   
#--------------------------------精簡程式-----------------------------------------            
            IF STATUS THEN
      ...
      ELSE
         IF NOT cl_null(maj.maj21) THEN
            ...
            IF STATUS THEN
               CALL cl_err('r120_c1',STATUS,1)
               EXIT FOREACH
            END IF
            OPEN r120_c1
            FETCH r120_c1 INTO amt1
            IF STATUS THEN
...
#FUN-D70095--------------------------begin
            LET l_sql = "SELECT SUM(abb07) FROM abb_file,aba_file " ,   
                       ...
                       "   AND aba06 = 'CE'  AND abb06 = ? AND aba03 ='",tm.yy1,"'",
...
#FUN-AB0020 ---------------------End-----------------------------                        
            ...
            IF STATUS THEN
               CALL cl_err('r120_c2',STATUS,1)
               EXIT FOREACH
            END IF
            OPEN r120_c2
            FETCH r120_c2 INTO amt1_1
            IF STATUS THEN
...
#      END IF
 #---------------FUN-D70095---mark-------(E)
      ...
      IF maj.maj07 = '2' THEN                                                                                                    
        #LET sr.bal1  = sr.bal1  * -1   #FUN-D70095 mark
   ...
   LET g_str= g_mai02,";",tm.d,";",tm.a,";",tm.p,";",tm.yy1,";",tm.bm1,";",                #No.FUN-830139
              tm.em1,";",tm.title1,";",tm.e,";",m_gem02
   CALL cl_prt_cs3('aglr120','aglr120',g_sql,g_str)
END FUNCTION
 
FUNCTION r120_bom(l_dept,l_sw)
