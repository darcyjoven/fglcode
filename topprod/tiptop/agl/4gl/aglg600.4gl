...
# Modify.........: No.FUN-B80161 11/09/05 By qiruile 明細CR報表轉GR
# Modify.........: No.FUN-B80161 12/01/06 By qirl MOD-B90171追單
# Modify.........: No.FUN-C50005 12/05/02 By qirl GR程式優化
...
TYPE sr1_t RECORD
    ...
    desc1 LIKE aff_file.aff05,
    abb03 LIKE abb_file.abb03,
    aag02 LIKE aag_file.aag02,
    desc2 LIKE aff_file.aff06,
    amt_a LIKE aff_file.aff09,
    amt_b LIKE aff_file.aff09,
    amt_c LIKE aff_file.aff09
END RECORD
###GENGRE###END
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglg600',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL g600_tm()	        	# Input print condition
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,		#(at time fglgo xxxx p1 p2 p3)
			 " '",tm.bookno CLIPPED,"'",  
                         ...
                         " '",g_template CLIPPED,"'",
                         " '",g_rpt_name CLIPPED,"'"  
         CALL cl_cmdat('aglg600',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW g600_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)           #FUN-B80161
...
DEFINE l_sum2          LIKE abb_file.abb07
 ...
 #  LET l_sql = "SELECT abb36,'',aff05,abb03,'',aff06,abb07,abb11,abb12,abb13,abb14,abb31,abb32,abb33,abb34,aba00,aba03,aba04",      # No.FUN-C50005 mark
    LET l_sql = "SELECT abb36,azf03,aff05,abb03,'',aff06,abb07,abb11,abb12,abb13,abb14,abb31,abb32,abb33,abb34,aba00,aba03,aba04",# No.FUN-C50005 add
            #   " FROM aba_file,abb_file,aff_file",             # No.FUN-C50005 mark
                 " FROM aba_file ,abb_file LEFT OUTER JOIN azf_file ON  (abb36=azf01 and azf02='2'),aff_file ",# No.FUN-C50005 add
     ...
     IF SQLCA.sqlcode THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE g600_c1 CURSOR FOR g600_p1
 
     FOREACH g600_c1 INTO sr.*
     ...
     #   SELECT azf03 INTO l_azf03 FROM azf_file WHERE azf01=sr.abb36 AND azf02='2'         # No.FUN-C50005 mark 
     ...
     #       sr.abb36,l_azf03,sr.aff05,sr.abb03,               # No.FUN-C50005 mark
            sr.abb36,sr.azf03,sr.aff05,sr.abb03,# No.FUN-C50005 add
             l_aag02,l_abbx,sr.abb07,l_afg08,l_dec      #MOD-B80161 sr.abb11改l_abb
            #l_aag02,sr.abb11,sr.abb07,l_afg08,l_dec    #MOD-B80161 mark
...
###GENGRE###     CALL cl_prt_cs3('aglg600','aglg600',l_sql,g_str)
    CALL aglg600_grdata()    ###GENGRE###
END FUNCTION
#No.FUN-850027

###GENGRE###START
FUNCTION aglg600_grdata()
    DEFINE l_sql    STRING
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg600")
        IF handler IS NOT NULL THEN
            START REPORT aglg600_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg600_datacur1 CURSOR FROM l_sql
            FOREACH aglg600_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg600_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg600_rep
        END IF
...
REPORT aglg600_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B80161----add-----str-------------
    DEFINE l_amt_c  LIKE aff_file.aff09
    DEFINE l_amt_a  LIKE aff_file.aff09
    DEFINE l_amt_b  LIKE aff_file.aff09
    #FUN-B80161----add-----end-------------
    ...
    FORMAT
        ...
        ON EVERY ROW
            LET l_amt_a =sr1.amt_a
            LET l_amt_b =sr1.amt_b
            LET l_amt_c =l_amt_a -l_amt_b
            PRINTX l_amt_c
