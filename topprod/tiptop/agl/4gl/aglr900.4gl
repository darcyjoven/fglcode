...
MAIN
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL aglr900_tm()	        	# Input print condition
      ELSE CALL aglr900()			# Read data and create out-file
   END IF
...
FUNCTION aglr900_tm()
   ...
   END IF
   OPEN WINDOW aglr900_w AT p_row,p_col
        WITH FORM "agl/42f/aglr900"
...
################################################################################
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW aglr900_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglr900',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW aglr900_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8	    #No.FUN-6A0073
     ...
     #CALL cl_outnam('aglr900') RETURNING l_name
     #START REPORT aglr900_rep TO l_name
     #LET g_pageno = 0
     #No.FUN-750129  --End
     LET g_cnt    = 1
   #----> 取得"起始期別"的起始與截止日期
  ...
  #IF g_aza.aza26 = '2' THEN
      IF tm.a1 = 'Y' THEN CALL aglr900_1_2() END IF
      IF tm.a2 = 'Y' THEN CALL aglr900_2_2() END IF
  #ELSE
  #   IF tm.a1 = 'Y' THEN CALL aglr900_1() END IF
  #   IF tm.a2 = 'Y' THEN CALL aglr900_2() END IF
  #END IF
   #CHI-710005 MARK END
   #No.MOD-5B0097  --end
   IF tm.a3 = 'Y' THEN CALL aglr900_3() END IF
   IF tm.a4 = 'Y' THEN CALL aglr900_4() END IF
   IF tm.a5 = 'Y' THEN CALL aglr900_5() END IF
   #End
 
   #No.FUN-750129  --Begin
   #FINISH REPORT aglr900_rep
   #CALL cl_prt(l_name,g_prtway,g_copies,g_len)
   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
   LET g_str = tm.yy,";",tm.bm,";",tm.yy,";",tm.em,";",t_azi04
#   CALL cl_prt_cs3('aglr900','aglr900',g_sql,g_str)#TQC-AB0194
#TQC-AB0194--add--str--
   IF g_cnt >1 THEN
      CALL cl_prt_cs3('aglr900','aglr900',g_sql,g_str)
   ELSE
...
FUNCTION aglr900_1()
   DEFINE    l_sql 	LIKE type_file.chr1000,	# RDSQL STATEMENT        #No.FUN-680098  VARCHAR(600)
    ...
    DELETE FROM t;
     ...
     #No.A116  取得明細所屬統制科目
    DECLARE gglr900_1_1 CURSOR FOR
     SELECT UNIQUE aag08
    ...
    FOREACH gglr900_1_1 INTO l_aag08
       IF STATUS THEN CALL cl_err('gglr900_1_1',STATUS,1) EXIT FOREACH END IF
       IF NOT cl_null(l_aag08) THEN
          DECLARE aglr900_1 CURSOR FOR
...
#        CALL cl_err('ins #2:',STATUS,1) # NO.FUN-660123
   ...
   DECLARE aglr900_curs2 CURSOR FOR
      SELECT '2' ,account,SUM(amt_1),SUM(amt_2),0
          FROM t
          GROUP BY account
          HAVING SUM(amt_1) != SUM(amt_2)
 
   FOREACH aglr900_curs2 INTO sr.*
...
#        CALL cl_err('ins #3:',STATUS,1) # NO.FUN-660123 
    ...
    DECLARE aglr900_curs3 CURSOR FOR
      SELECT '3',account,amt_1,amt_2,0
          FROM y
          WHERE amt_1 != amt_2
 
    FOREACH aglr900_curs3 INTO sr.*
...
#        CALL cl_err('ins #4:',STATUS,1) # NO.FUN-660123 
    ...
    DECLARE aglr900_curs4 CURSOR FOR
      SELECT '4',account,amt_1,amt_2,0
          FROM y
          WHERE amt_1 != amt_2
 
    FOREACH aglr900_curs4 INTO sr.*
...
#        CALL cl_err('ins #5:',STATUS,1) # NO.FUN-660123
    ...
    END FOREACH
     ...
     DECLARE aglr900_curs6 CURSOR FOR
        SELECT '5' , b, a,0,0
            FROM v
            GROUP BY b,a
 
     FOREACH aglr900_curs6 INTO sr.*
...
FUNCTION aglr900_1_2()
   DEFINE    l_sql     LIKE type_file.chr1000,     # RDSQL STATEMENT        #No.FUN-680098  VARCHAR(600)
    ...
    DELETE FROM t;
     ...
     #No.A116  取得明細所屬統制科目
    DECLARE gglr900_1_1_2 CURSOR FOR
     SELECT UNIQUE aag08
       FROM aah_file,aag_file
      WHERE aah01 = aag01 AND aag07 = '2'       #明細帳戶
        AND aah00 = tm.bookno                  #No.FUN-740020
     ...
     SELECT UNIQUE aag08
      ...
      WHERE aah01 = aag01 AND aag07 = '1'       #統治科目
        ...
        AND aah02 = tm.yy                          #年度
        AND aah03 BETWEEN tm.bm AND tm.em          #期別
    FOREACH gglr900_1_1_2 INTO l_aag08
       IF STATUS THEN CALL cl_err('gglr900_1_1_2',STATUS,1) EXIT FOREACH END IF
       IF NOT cl_null(l_aag08) THEN
          DECLARE aglr900_1_2 CURSOR FOR
           SELECT '1' a, aah01 account,(aah04-aah05) amt_1,0 amt_2,0 amt_3
             FROM aag_file, aah_file
            WHERE aag01 = aah01 AND aag07 = '1'        #統制帳戶
              AND aah00 = tm.bookno                      #No.FUN-740020                  
...
#               CALL cl_err('ins #1_2:',STATUS,1) # NO.FUN-660123
    ...
    END FOREACH
     ...
     DECLARE aglr900_curs1_2 CURSOR FOR
         SELECT '1' ,account,SUM(amt_1),SUM(amt_2),0
          FROM t
          GROUP BY account
          HAVING SUM(amt_1) != SUM(amt_2)
     FOREACH aglr900_curs1_2 INTO sr.*
...
FUNCTION aglr900_2_2()
   ...
   DECLARE aglr900_2_2 CURSOR FOR
   SELECT '1' a, aas01 account,(aas04-aas05) amt_1,0 amt_2,0 amt_3
          FROM aag_file, aas_file
          WHERE aag01 = aas01 AND aag07 = '1'        #統制帳戶
            AND aas00 = tm.bookno                    #No.FUN-740020 
...
#        CALL cl_err('ins #2_2:',STATUS,1) # NO.FUN-660123
   ...
   DECLARE aglr900_curs2_2 CURSOR FOR
      SELECT '2' ,account,SUM(amt_1),SUM(amt_2),0
          FROM t
          GROUP BY account
          HAVING SUM(amt_1) != SUM(amt_2)
 
   FOREACH aglr900_curs2_2 INTO sr.*
