...
FUNCTION r100_page()
   ...
   FOR l_i = 10 TO l_cnt STEP 10
      ...
      IF l_i <= g_no THEN
         LET l_no = l_i - 10
         LET g_sql ="INSERT INTO dept1_tmp ",
...
FUNCTION q011_process1()
    ...
    LET g_sql = " UPDATE q011_tmp a SET d_ce = (SELECT NVL(SUM(abb07),0) FROM abb_tmp,maj_file b,mai_file ",
                "                 WHERE abb06 = '1'  ",
                "                   AND mai01 = b.maj01 ",
                "                   AND mai01 = '",tm.a,"'",
                "                   AND mai00 = '",tm.b,"'",
    ...
    LET g_sql = " UPDATE q011_tmp a SET c_ce = (SELECT NVL(SUM(abb07),0) FROM abb_tmp,maj_file b,mai_file ",
                "                  WHERE abb06 = '2'   ",
                "                    AND mai01 = b.maj01",
                "                    AND mai01 = '",tm.a,"'",
                "                    AND mai00 = '",tm.b,"'",
   ...
   IF tm.d MATCHES '[23]' THEN
      ...
      END IF
      LET g_sql = " UPDATE q011_tmp SET d    = d    / ",l_factor,",",
                                    "   c    = c    / ",l_factor,",",
                                    "   d_ce = d_ce / ",l_factor,",",
                                    "   c_ce = c_ce / ",l_factor,",",
                                    "   sum  = sum  / ",l_factor,",",
...
FUNCTION q011_process2()
   ...
   FOREACH q011_maj_cs INTO l_maj.* 
        ...
        IF l_maj.maj06 MATCHES '[579]' AND l_maj.maj07 = '1' THEN
          #LET l_sql = " UPDATE q011_tmp SET sum = -1 * sum   WHERE maj02 = ",l_maj.maj02   #fengmy mark 130828
           LET g_sql = " UPDATE q011_tmp SET sum = -1 * sum , amt = -1 * amt  WHERE maj02 = ",l_maj.maj02  #fengmy add 130828
   ...
   IF l_max_maj08 > 1 THEN 
      LET g_sql = " UPDATE q011_tmp a SET a.amt  = (SELECT SUM(b.amt * d.sw) FROM q011_tmp b ,speed_tmp c ,sw_tmp d",
...
FUNCTION q011_fill_arr(l_i) 
   ...
   LET g_sql = "SELECT amt FROM q011_tmp ",
               " WHERE maj02 = ? AND dept_m = ? "
   PREPARE q011_sel_amt_cs FROM g_sql   
#   CALL g_maj.clear()
   LET l_cnt = 1
