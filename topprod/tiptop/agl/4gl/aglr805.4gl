...
# Modify.........: No.TQC-810066 08/01/21 By lumxa aglr805褪醴瞄呾砐祑雄�惆桶湖荂釬珛ㄛ枑尨拸惆桶囀�
...
MAIN
 ...
 #--str FUN-780059 add--#                                                                                                           
   ...
   LET l_table = cl_prt_temptable('aglr805',g_sql) CLIPPED   # 產生Temp Table                                                       
...
#  LET g_bookno = ARG_VAL(1)   #TQC-610056 #No.FUN-740020
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL aglr805_tm(0,0)        # Input print condition
      ELSE CALL aglr805()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
END MAIN
 
FUNCTION aglr805_tm(p_row,p_col)
...
DEFINE li_chk_bookno    LIKE type_file.num5           #No.FUN-B20054
   ...
   END IF
   OPEN WINDOW aglr805_w AT p_row,p_col
        WITH FORM "agl/42f/aglr805"
################################################################################
...
# 
#   IF INT_FLAG THEN
#      LET INT_FLAG = 0 CLOSE WINDOW aglr805_w 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
#      EXIT PROGRAM
#         
#   END IF
...
#No.FUn-B20054--add-end--
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglr805',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW aglr805_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#                " GROUP BY aec01,aag02,aag13,aag15,aag16,aag17,aag18,aag06,",  #No.MOD-650121   #FUN-6C0012 #TQC-810066
     ...
     PREPARE aglr805_prepare1 FROM l_sql
     ...
     END IF
     DECLARE aglr805_curs1 CURSOR FOR aglr805_prepare1
 
#    CALL cl_outnam('aglr805') RETURNING l_name                     #FUN-780059--mark
     ...
     ELSE                                                                       
        ...
        LET l_name1 = 'aglr805_1'                                                
     END IF
     CALL cl_prt_pos_len() 
     #FUN-6C0012.....end
#    START REPORT aglr805_rep TO l_name                             #FUN-780059--mark
 
     LET g_pageno = 0
     FOREACH aglr805_curs1 INTO sr.*
        IF SQLCA.sqlcode != 0 THEN
...
#       OUTPUT TO REPORT aglr805_rep(sr.*)                         #FUN-780059--mark
        ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-770052 *** ##                                                   
           EXECUTE insert_prep USING                                                                                                
                      sr.aec01,sr.aag02,sr.aag06,sr.aec05,sr.aee04,sr.abb07,l_buff                                                    
          #------------------------------ CR (3) ------------------------------#
     END FOREACH
 
#    FINISH REPORT aglr805_rep                                     #FUN-780059--mark
...
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)                   #FUN-780059--mark
 ## **** 與 Crystal Reports 串聯段 - <<<< CALL cs3() >>>> FUN-770052 **** ##                                                        
    ...
    LET g_str = ''                                                                                                                  
    LET g_str = tm.wc,";",tm.aec02_b,";",tm.aec02_e,";",t_azi04,";",                                                               
                tm.t,";",t_azi05                                                                         
    CALL cl_prt_cs3('aglr805',l_name1,l_sql,g_str)                                                                                
    #------------------------------ CR (4) ------------------------------#
END FUNCTION
 
#--str-FUN-780059--mark--#
#REPORT aglr805_rep(sr)
