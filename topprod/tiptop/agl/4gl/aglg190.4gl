...
#No.FUN-780060--end--add
  ...
  #str MOD-990078 add                                                                                                               
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL g190_tm()                        # Input print condition
   ELSE
      CALL g190()         
   END IF
...
FUNCTION g190_tm()
   ...
   OPEN WINDOW g190_w AT p_row,p_col WITH FORM "agl/42f/aglg190" 
...
#            CALL FGL_DIALOG_SETBUFFER( tm.p )
   ...
   END INPUT
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW g190_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)               #FUN-B80158 add
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglg190',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW g190_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)               #FUN-B80158 add
...
#---------------------------------------------------
   ...
   LET rep_cnt=0
#---------------------------------------------------
#  FINISH REPORT g190_rep
#  CALL cl_prt(l_name,g_prtway,g_copies,g_len)
   LET l_tit=tm.yy USING '<<<<','/',tm.bm USING'&&','-',tm.em USING'&&'
###GENGRE###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED  
###GENGRE###   LET g_str= g_mai02,";",tm.a,";",tm.d,";",tm.p,";",tm.e,";",l_tit,";",g_basetot1 #CHI-A70046 add g_basetot1
###GENGRE###   CALL cl_prt_cs3('aglg190','aglg190',g_sql,g_str) 
    CALL aglg190_grdata()    ###GENGRE###
END FUNCTION
 
FUNCTION g190_process(l_dept)
...
### 98/03/04 by connie modify where conditions
   ...
   LET l_sql = l_sql CLIPPED," AND aag38= '",tm.aag38,"'"      #FUN-A30022 ADD     
    PREPARE g190_sum FROM l_sql
    DECLARE g190_sumc CURSOR FOR g190_sum
    #-----------------------------------------------------------------------
    FOREACH g190_c INTO maj.*
       IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
       LET amt1 = 0
       IF NOT cl_null(maj.maj21) THEN
          OPEN g190_sumc USING maj.maj21,maj.maj22
          FETCH g190_sumc INTO amt1   
...
#No.FUN-780060--start--add
      ...
      #LET per1 = (sr.bal1 / g_basetot1[rep_cnt]) * 100 #MOD-990078                                                                
       ...
       END IF
#No.FUN-780060--end--
#      OUTPUT TO REPORT g190_rep(maj.*, sr.*)
    END FOREACH
  #str MOD-990078 add                                                                                                               
   IF NOT cl_null(g_basetot1) THEN                                                                                                  
      LET l_sql = "SELECT maj02,bal1,line",                                                                                         
                  ...
                  " WHERE dept='",l_dept,"'"                                                                                        
      PREPARE g190_crtmp_p FROM l_sql                                                                                               
      IF STATUS THEN CALL cl_err('prepare:',STATUS,1)                                                                               
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114                                                              
      END IF                                                                                                                        
      DECLARE g190_crtmp_c CURSOR FOR g190_crtmp_p                                                                                  
      FOREACH g190_crtmp_c INTO cr.*                                                                                                
         IF cr.bal1 != 0 THEN                                                                                                       
...
FUNCTION aglg190_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg190")
        IF handler IS NOT NULL THEN
            START REPORT aglg190_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg190_datacur1 CURSOR FROM l_sql
            FOREACH aglg190_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg190_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg190_rep
        END IF
...
REPORT aglg190_rep(sr1)
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            #FUN-B80158-----add-----str------
            IF sr1.maj03 = "4" THEN 
               IF NOT cl_null(sr1_o.maj20) THEN 
                  IF sr1_o.maj20 != sr1.maj20 AND sr1_o.maj02 != sr1.maj02 THEN 
                     LET l_display1 = 'Y'
                  ELSE
                     LET l_display1 = 'N'
                  END IF
