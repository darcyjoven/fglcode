...
# Modify.........: No.MOD-BC0280 11/12/30 By Polly 科目開窗改開q_aag16
...
MAIN
 ...
 #--str FUN-780059 add--#                                                                                                           
   ...
   LET l_table = cl_prt_temptable('aglr740',g_sql) CLIPPED   # 產生Temp Table                                                       
...
DEFINE li_chk_bookno  LIKE type_file.num5          #FUN-B20054 
   ...
   OPEN WINDOW aglr740_w AT p_row,p_col
     WITH FORM "agl/42f/aglr740"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
    ...
    #FUN-B20054--add--str--
      ON ACTION CONTROLP                                                                                                         
          CASE                                                                                                                      
            ...
            WHEN INFIELD(aag01)
                CALL cl_init_qry_var()
               #LET g_qryparam.form = "q_aag02"                    #MOD-BC0280 mark
                LET g_qryparam.form = "q_aag16"                    #MOD-BC0280 add
 ...
 #FUN-B20054--mark--end
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglr740',g_time,l_cmd)      # Execute cmd at later time
         END IF
...
#       l_time          LIKE type_file.chr8               #No.FUN-6A0073
   ...
   PREPARE aglr740_prepare2 FROM l_sql1
   IF STATUS != 0 THEN
      CALL cl_err('prepare:',STATUS,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE aglr740_curs2 CURSOR FOR aglr740_prepare2
 
#  CALL cl_outnam('aglr740') RETURNING l_name                      #FUN-780059-mark
#  START REPORT aglr740_rep TO l_name                              #FUN-780059-mark
 
   LET g_pageno = 0
 
   FOREACH aglr740_curs2 INTO sr1.*
      IF STATUS THEN
...
#                 "       '',0,azi04 ",
      ...
      PREPARE aglr740_prepare1 FROM l_sql
      IF STATUS != 0 THEN
         CALL cl_err('prepare:',STATUS,1)
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
         EXIT PROGRAM
      END IF
      DECLARE aglr740_curs1 CURSOR FOR aglr740_prepare1
 
      LET l_flag='N'
 
      FOREACH aglr740_curs1 INTO sr.*
...
#        OUTPUT TO REPORT aglr740_rep(sr.*)                       #FUN-780059-mark
        ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-770052 *** ##                                                   
           EXECUTE insert_prep USING      
                   sr.aea01,sr.aea02,sr.aea03,sr.abb04,sr.abb05,sr.abb06,sr.abb07,sr.aag02,sr.aag13,
                   sr.azi04,m_gem02,l_bal,l_bal_1,l_buf,l_chr,l_abc04,l_t_d,l_t_c,dept_d,dept_c                                                                                          
          #------------------------------ CR (3) ------------------------------#
        ...
        ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-770052 *** ##                                                   
           EXECUTE insert_prep USING                                                                                                
                  ...
                  #sr.azi04,m_gem02,l_bal,l_buf,l_chr,l_abc04,l_t_d,l_t_c,dept_d,dept_c          #MOD-B20028 mark 
                   sr.azi04,m_gem02,l_bal,l_bal_1,l_buf,l_chr,l_abc04,l_t_d,l_t_c,dept_d,dept_c  #MOD-B20028 
          #------------------------------ CR (3) ------------------------------#
...
#  FINISH REPORT aglr740_rep                                      #FUN-780059-mark
#-str-FUN-780059--add--#
         IF g_zz05 = 'Y' THEN                                                                                                       
...
#  CALL cl_prt(l_name,g_prtway,g_copies,g_len)                    #FUN-780059-mark
 ## **** 與 Crystal Reports 串聯段 - <<<< CALL cs3() >>>> FUN-770052 **** ##                                                        
    ...
    LET g_str = ''                                                                                                                  
    LET g_str = tm.wc,";",bdate,";",edate,";",tm.u,";",tm.e,";",                                                              
                tm.s,";",tm.v                                                                        
    CALL cl_prt_cs3('aglr740','aglr740',l_sql,g_str)                                                                                
    #------------------------------ CR (4) ------------------------------#
 
END FUNCTION
 
#-str-FUN-780059--mark--#
#REPORT aglr740_rep(sr)
#  DEFINE sr       RECORD
