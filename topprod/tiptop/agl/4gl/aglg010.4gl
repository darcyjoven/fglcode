...
# Modify.........: No.FUn-B90028 11/09/16 By qirl明細CR報表轉GR
...
DEFINE tm  RECORD                      # Print condition RECORD
           ...
           b     LIKE aaz_file.aaz641, #合并帳別   #FUN-B50001 #FUN-B90028 (FUN-B50001) rollback
           #b     LIKE aaw_file.aaw01, #合并帳別 #FUN-B90028 (FUN-B50001) rollback
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglg010',g_sql) CLIPPED    
...
#--FUN-A90032 mark-------------   
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'            # If background job sw is off
      THEN CALL aglg010_tm()                    # Input print condition
      ELSE CALL aglg010()                       # Read data and create out-file
   END IF
...
FUNCTION aglg010_tm()
   ...
   OPEN WINDOW aglg010_w AT p_row,p_col WITH FORM "agl/42f/aglg010"
...
#FUN-B50001--mod--end#FUN-B90028 (FUN-B50001) rollback
             ...
             NEXT FIELD axa02
          ELSE
             LET tm.b = g_aaz641    #FUN-B50001#FUN-B90028 (FUN-B50001) rollback
             #LET tm.b = g_aaw01#FUN-B90028 (FUN-B50001) rollback
...
#FUN-A90032 --End          
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW aglg010_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)   #FUN-B90028 add
      EXIT PROGRAM
   END IF
...
#---------------------FUN-A90032 mark----------------
                     ...
                     " '",g_template CLIPPED,"'",        
                     " '",g_rpt_name CLIPPED,"'"     
         CALL cl_cmdat('aglg010',g_time,l_cmd)  # Execute cmd at later time
      END IF
      CLOSE WINDOW aglg010_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)   #FUN-B90028 add
      EXIT PROGRAM
   END IF
...
#--FUN-A90032 end---
   ...
   FOR g_i = 1 TO 5
      ...
      LET l_sql = "SELECT aag06,aag19,SUM(axh08-axh09)",
                # " FROM axh_file,",g_dbs_axz03,"aag_file",  #FUN-A30122 mark
                  ...
                  "   AND axh01 = '",tm.axa01,"'",
                  "   AND axh02 = '",tm.axa02,"'",
                  #"   AND axh03 = '",tm.axa03,"'",     #FUN-A90032 mark
                  "   AND axh03 = '",tm.b,"'",          #FUN-A90032 mod   
                  "   AND axh05 = aag01",
                  "   AND axh06 = ? ",
      ...
      DECLARE aglg010_c1 CURSOR FOR aglg010_p1
      FOREACH aglg010_c1 USING l_yy[g_i],l_em[g_i] INTO l_aag06,l_aag19,l_tot
         IF SQLCA.sqlcode != 0 THEN
      ...
      IF l_bm1< 1 THEN
         ...
         LET l_yy1= l_yy1 -1
      END IF
      FOREACH aglg010_c1 USING l_yy1,l_bm1 INTO l_aag06,l_aag19,l_tot
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
   ...
   FOR g_i = 1 TO 5
      ...
      DECLARE aglg010_c2 CURSOR FOR aglg010_p2
      FOREACH aglg010_c2 USING l_yy[g_i] INTO l_aag06,l_aag19,l_tot
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
      ...
      FOREACH aglg010_c2 USING l_yyb INTO l_aag06,l_aag19,l_tot
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
...
###GENGRE###               tm.m10,";",tm.n1,";",tm.n2,";",tm.n3,";",tm.n4
###GENGRE###   LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
###GENGRE###   CALL cl_prt_cs3('aglg010','aglg010',l_sql,g_str)
    CALL aglg010_grdata()    ###GENGRE###
END FUNCTION

FUNCTION aglg010_ins_temp()
...
FUNCTION aglg010_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg010")
        IF handler IS NOT NULL THEN
            START REPORT aglg010_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg010_datacur1 CURSOR FROM l_sql
            FOREACH aglg010_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg010_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg010_rep
        END IF
...
REPORT aglg010_rep(sr1)
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.sort
            #FUN-B90028----add-----str----------
            IF NOT cl_null(sr1.sort2) THEN
               IF sr1_o.sort2 != sr1.sort2 THEN
                  LET l_display1 = 'Y'
               ELSE
                  LET l_display1 = 'N'
               END IF
            ...
            PRINTX l_date04
            LET l_date05 = l_tmyy5.trim(),'/',l_tmm8.trim(),'-',l_tmm10.trim()
        ...
        ON EVERY ROW
            ...
            DECLARE aglg0101_reordcur1 CURSOR FROM g_sql1
            FOREACH aglg0101_reordcur1 INTO l_cnt END FOREACH

            LET g_sql2 = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                         " WHERE sort2 = '",sr1.sort2,"'"
            DECLARE aglg0101_reordcur2 CURSOR FROM g_sql1
            FOREACH aglg0101_reordcur2 INTO sr1_o.*
