...
TYPE sr1_t RECORD
    ...
    bal1_1 LIKE aed_file.aed05,
    amt1 LIKE aed_file.aed05,
    bal2 LIKE aed_file.aed05,
    bal1_2 LIKE aed_file.aed05,
    amt2 LIKE aed_file.aed05,
    per1 LIKE aed_file.aed05,
    per2 LIKE aed_file.aed05,
...
MAIN
   ...
   LET g_sql="maj20.maj_file.maj20,maj20e.maj_file.maj20e,",                    
             ...
             "line.type_file.num5"                                              
   LET l_table = cl_prt_temptable('aglg325',g_sql) CLIPPED                      
   IF l_table = -1 THEN 
...
FUNCTION g325_tm()
   ...
   OPEN WINDOW g325_w AT p_row,p_col
     WITH FORM "agl/42f/aglg325"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
#                       " '",tm.budget CLIPPED,"'",   #No.FUN-830139
            ...
            CALL cl_cmdat('aglg325',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#     DEFINE   l_time LIKE type_file.chr8            #No.FUN-6A0073
   ...
   IF STATUS THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g325_c CURSOR FOR g325_p
...
#   CALL cl_outnam('aglg325') RETURNING l_name       #No.FUN-810009
#   START REPORT g325_rep TO l_name                  #No.FUN-810009
 
   FOREACH g325_c INTO maj.*
      IF STATUS THEN
...
#      OUTPUT TO REPORT g325_rep(maj.*, sr.*)    #No.FUN-810009
      #No.FUN-810009 --start--                                                  
      IF maj.maj07 = '2' THEN                                                   
         LET sr.bal1 = sr.bal1 * -1                                             
         LET sr.bal2 = sr.bal2 * -1                                             
...
#   FINISH REPORT g325_rep                      #No.FUN-810009
...
###GENGRE###               tm.title1,";",tm.title2,";",l_tit1,";",l_tit2,";",tm.e,";",tm.afc01  #No.FUN-830139    #FUN-AB0020 add tm.afc01 
#              tm.title1,";",tm.title2,";",l_tit1,";",l_tit2,";",tm.e,";",    #No.FUN-830139   
#              tm.budget                                                      #No.FUN-830139   
###GENGRE###   LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED             
###GENGRE###   CALL cl_prt_cs3('aglg325','aglg325',l_sql,g_str)                             
    CALL aglg325_grdata()    ###GENGRE###
   #No.FUN-810009 --end-- 
END FUNCTION
#No.FUN-810009 --start-- mark
#REPORT g325_rep(maj, sr)
...
FUNCTION aglg325_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg325")
        IF handler IS NOT NULL THEN
            START REPORT aglg325_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg325_datacur1 CURSOR FROM l_sql
            FOREACH aglg325_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg325_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg325_rep
        END IF
...
REPORT aglg325_rep(sr1)
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B80158--------add-----str----
            LET l_aed05_fmt = cl_gr_numfmt("aed_file","aed05",tm.e)
            PRINTX l_aed05_fmt
            #FUN-B80158--------add-----end---- 
