...
#
# Pattern name...: aglg810.4gl
# Descriptions...: 科目異動碼明細表(aglg810)
...
# Modify.........: No:MOD-B80100 11/08/18 By johung g810_cr_cur排序條件加上balance
# Modify.........: No:FUN-B90027 11/09/16 By Wangning  明細CR報表轉GR
# Modify.........: No.FUN-B90027 12/01/05 By qirl FUN-BB0047追单
# Modify.........: No.FUN-B90027 12/01/06 By qirl MOD-B80100追單
...
MAIN
   ...
   IF (NOT cl_setup("AGL")) THEN
      EXIT PROGRAM
   END IF
  # CALL cl_used(g_prog,g_time,1) RETURNING g_time #No.FUN-690114#FUN-B90027
...
#No.FUN-7C0038-------------start-------------
   ...
   LET l_table = cl_prt_temptable('aglg810',g_sql) CLIPPED
   IF l_table = -1 THEN 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time          #FUN-B90027 add
      CALL cl_gre_drop_temptable(l_table1)                     #FUN-B90027
      EXIT PROGRAM
    END IF
   ...
   LET l_table1 = cl_prt_temptable('aglg8101',g_sql) CLIPPED
   IF l_table1 = -1 THEN 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time          #FUN-B90027 add
      CALL cl_gre_drop_temptable(l_table1)                     #FUN-B90027
      EXIT PROGRAM
   END IF
#No.FUN-7C0038-------------end-------------
    CALL cl_used(g_prog,g_time,1) RETURNING g_time 	#FUN-B90027
#   SELECT azi04,azi05 INTO g_azi04,g_azi05 FROM azi_file WHERE azi01=g_aza.aza17                 #No.CHI-6A0004 mark
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'  THEN # If background job sw is off
      CALL aglg810_tm(0,0)                    # Input print condition
   ELSE
      CALL aglg810()                          # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
   CALL cl_gre_drop_temptable(l_table1)      #No.FUN-B90027
END MAIN
 
FUNCTION aglg810_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,          #No.FUN-680098  SMALLINT
   ...
   END IF
   OPEN WINDOW aglg810_w AT p_row,p_col
        WITH FORM "agl/42f/aglg810"
...
# 
#   IF INT_FLAG THEN
#      LET INT_FLAG = 0 CLOSE WINDOW aglg810_w 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
...
#No.FUN-B20054-add-end--
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglg810',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW aglg810_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table1)                     #FUN-B90027
      EXIT PROGRAM
   END IF
...
#NO.FUN-7C0038--------------------END------
     ...
     PREPARE aglg810_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM 
        END IF
     DECLARE aglg810_curs1 CURSOR FOR aglg810_prepare1
...
#NO.FUN-7C0038--------------------END------
     ...
     PREPARE g810_abg_pre FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM 
        END IF
     DECLARE g810_abg_cur CURSOR FOR g810_abg_pre
...
#NO.FUN-7C0038--------------------END------
     ...
     PREPARE g810_abh_pre FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM 
        END IF
     DECLARE g810_abh_cur CURSOR FOR g810_abh_pre
     ...
     PREPARE g810_abhsum_pre FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM 
        END IF
     DECLARE g810_abhsum_cur CURSOR FOR g810_abhsum_pre
#NO.FUN-7C0038--------------------START------
#     CALL cl_outnam('aglg810') RETURNING l_name
#     START REPORT aglg810_rep TO l_name
#
#     LET g_pageno = 0
#NO.FUN-7C0038--------------------END------
     INITIALIZE sr.* TO NULL
     #--->期初資料(有可能科目只有期初但當月無立帳或沖帳)
     FOREACH aglg810_curs1 INTO sr.*
         IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('foreach g810_curs1:',SQLCA.sqlcode,1)
          EXIT FOREACH
         END IF
         # FUN-5C0015 add(s)
...
#NO.FUN-7C0038--------------------START------
#         LET p_order = 1
#         OUTPUT TO REPORT aglg810_rep(sr.*,p_order)
      EXECUTE insert_prep USING sr.*
      IF STATUS THEN
         CALL cl_err("execute insert_prep:",STATUS,1)
         EXIT FOREACH
      END IF
#      OPEN g810_abhsum_cur USING sr.abi05,sr.abi11
#      FETCH g810_abhsum_cur INTO l_rem
#      IF cl_null(l_rem) THEN LET l_rem = 0 END IF
#      CLOSE g810_abhsum_cur
#NO.FUN-7C0038--------------------END------
 
     END FOREACH
     #--->當月立帳
     FOREACH g810_abg_cur INTO sr.*
         IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('foreach g810_curs1:',SQLCA.sqlcode,1)   
          EXIT FOREACH
         END IF
...
#               " ORDER BY abi05,abi11,abi06,type"           #MOD-B90027 mark
                " ORDER BY abi05,abi11,abi06,type,balance"   #MOD-B90027
    PREPARE g810_cr_pre FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('pre abhsum_pre:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
       CALL cl_gre_drop_temptable(l_table1)                     #FUN-B90027
       EXIT PROGRAM 
    END IF
   DECLARE g810_cr_cur CURSOR FOR g810_cr_pre
   FOREACH g810_cr_cur INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach g810_curs1:',SQLCA.sqlcode,1)   
         EXIT FOREACH
      END IF
      ...
      IF l_chg_abi11 = '1' THEN
         OPEN g810_abhsum_cur USING sr.abi05,sr.abi11
         FETCH g810_abhsum_cur INTO sr1.l_rem
...
#----------FUN-7C0038-----------START------
      ...
      CALL cl_getmsg('aglg810',g_lang) RETURNING l_gx
#----------FUN-7C0038-----------END--------
...
###GENGRE###   LET g_str = tm.wc,";",g_zz05,";",g_azi03,";",g_azi04,";",g_azi05,";",tm.e
###GENGRE###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED
###GENGRE###   CALL cl_prt_cs3('aglg810','aglg810',g_sql,g_str)
    CALL aglg810_grdata()    ###GENGRE###
#NO.FUN-7C0038--------------------END------
END FUNCTION
 
#NO.FUN-7C0038--------------------START------
#REPORT aglg810_rep(sr,p_order)
...
FUNCTION aglg810_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg810")
        IF handler IS NOT NULL THEN
            START REPORT aglg810_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED
                        ," ORDER BY abi05,abi11,abi06,type,balance"   #FUN-B90027
          
            DECLARE aglg810_datacur1 CURSOR FROM l_sql
            FOREACH aglg810_datacur1 INTO sr2.*
                OUTPUT TO REPORT aglg810_rep(sr2.*)
            END FOREACH
            FINISH REPORT aglg810_rep
        END IF
...
REPORT aglg810_rep(sr2)
    DEFINE sr2 sr2_t
    DEFINE l_lineno LIKE type_file.num5

    #FUN-B90027----add----str---------
    DEFINE l_sw            LIKE type_file.chr1
    DEFINE g_tot           LIKE abi_file.abi08
    DEFINE l_str_t         STRING
    DEFINE l_abc_t         STRING
    DEFINE l_length        LIKE type_file.num5
    DEFINE l_length_a      LIKE type_file.num5
    DEFINE l_account_name  STRING
    #FUN-B90027----add----end---------
    

    ORDER EXTERNAL BY sr2.abi05,sr2.abi11
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            #FUN-B90027----add----str---------
#           IF tm.e = 'Y' THEN 
#              LET l_account_name = sr2.aag13,'(',sr2.abi05,')'
#           ELSE
#              LET l_account_name = sr2.aag02,'(',sr2.abi05,')'
#           END IF 
#           PRINTX l_account_name
            #FUN-B90027----add----end---------
            
        BEFORE GROUP OF sr2.abi05
        BEFORE GROUP OF sr2.abi11
        ...
        ON EVERY ROW
            ...
            LET l_str_t = sr2.l_str
            LET l_str_t = l_str_t.trim()
            LET l_length = LENGTH(l_str_t)
            PRINTX l_length

            LET l_abc_t = sr2.abc04
            LET l_abc_t = l_abc_t.trim()
            LET l_length_a = LENGTH(l_abc_t)
            PRINTX l_length_a
            #FUN-B90027----add----end---------

            PRINTX sr2.*

        AFTER GROUP OF sr2.abi05
            #FUN-B90027----add----str---------
            LET g_tot = GROUP SUM(sr2.l_diff)
            PRINTX g_tot
            #FUN-B90027----add----end--------- 
        AFTER GROUP OF sr2.abi11
