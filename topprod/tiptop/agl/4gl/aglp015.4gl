...
#
# Pattern name...: aglp015.4gl (FUN-770086 copy from aglp143)
# Descriptions...: 合併報表長期投資認列傳票拋轉還原作業
# Date & Author..: 07/07/27 By kim
# Modify.........: No.MOD-870231 08/07/29 By Sarah 考慮到所在資料庫帳別的關帳日，輸入的總帳傳票編號日期不可在關帳日之內
...
#     DEFINE   l_time LIKE type_file.chr8           #No.FUN-6A0073
   ...
   WHILE TRUE
      ...
      ELSE
         ...
         CALL p015_existno_chk()  #FUN-D40105  add
         CALL p015_t()
         CALL s_showmsg()   #FUN-D40105  add
         IF g_success = "Y" THEN
            COMMIT WORK
...
FUNCTION p015_ask()
   ...
   DEFINE l_aaa07            LIKE aaa_file.aaa07       #MOD-870231 add
 
   OPEN WINDOW p015_w AT p_row,p_col WITH FORM "agl/42f/aglp015" 
   ...
   INPUT BY NAME p_plant,p_bookno ATTRIBUTE(WITHOUT DEFAULTS=TRUE)    #FUN-D40105 add
      ...
      #                " WHERE aba01 = ? AND aba00 = ? AND aba06='CD'"
 	  #      CALL cl_replace_sqldb(g_sql) RETURNING g_sql        #FUN-920032
      #      CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql  #FUN-A50102
      #      PREPARE p015_t_p1 FROM g_sql
      #      DECLARE p015_t_c1 CURSOR FOR p015_t_p1
      #      IF STATUS THEN
      #         CALL cl_err('decl aba_cursor:',STATUS,0) 
      #         NEXT FIELD g_existno
      #      END IF
 
      #      OPEN p015_t_c1 USING g_existno,g_aaz.aaz64
      #      FETCH p015_t_c1 INTO gl_date,gl_yy,gl_mm,l_abapost,l_aba19,
      #                           l_abaacti 
      #      IF STATUS THEN
      ...
      ON ACTION CONTROLP
         CASE 
            WHEN INFIELD(g_existno)
              LET g_existno_str = ''
              CALL q_aba01_1( TRUE, TRUE, p_plant,p_bookno,' ','CD')
   ...
   END IF
   IF g_bgjob = "Y" THEN
      SELECT zz08 INTO lc_cmd FROM zz_file WHERE zz01 = "aglp015"
      IF SQLCA.sqlcode OR lc_cmd IS NULL THEN
         #CALL cl_err('aglp015','9031',1)          #FUN-D40105 mark
         CALL s_errmsg('','aglp015','','9031',1)   #FUN-D40105 add  
      ELSE
         ...
         LET lc_cmd = lc_cmd CLIPPED,
                      ...
                      " '",g_existno CLIPPED ,"'",
                      " '",g_bgjob CLIPPED,"'"
         CALL cl_cmdat('aglp015',g_time,lc_cmd CLIPPED)
      END IF
      CLOSE WINDOW p015_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
...
FUNCTION p015_t()
   ...
   IF g_bgjob = 'N' THEN  
      OPEN WINDOW p015_t_w9 AT 19,4 WITH 3 ROWS, 70 COLUMNS 
      LET g_success = 'Y'
      LET g_plant_new = p_plant
      CALL s_getdbs()
      LET g_dbs_gl = g_dbs_new 
   ...
   IF g_aaz84 = '2' THEN   #還原方式為作廢 
     #LET g_sql="UPDATE ",g_dbs_gl," aba_file  SET abaacti = 'N' ",  #FUN-A50102
 	 ...
 	 CALL cl_replace_sqldb(g_sql) RETURNING g_sql        #FUN-920032
      CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql  #FUN-A50102
      PREPARE p015_updaba_p FROM g_sql
      #EXECUTE p015_updaba_p USING g_existno,g_aaz.aaz64  #FUN-D40105  mark 
      EXECUTE p015_updaba_p USING g_aaz.aaz64             #FUN-D40105 add 
   ...
   ELSE
     ...
     #LET g_sql="DELETE FROM ",g_dbs_gl,"abb_file WHERE abb01=? AND abb00=?"   #FUN-A50102
      ...
      LET g_sql="DELETE FROM ",cl_get_target_table(g_plant_new,'abb_file'),    #FUN-A50102
 	  ...
 	  CALL cl_replace_sqldb(g_sql) RETURNING g_sql        #FUN-920032
      CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql  #FUN-A50102
      PREPARE p015_2_p3 FROM g_sql
      #EXECUTE p015_2_p3 USING g_existno,g_aaz.aaz64     #FUN-D40105  mark
      EXECUTE p015_2_p3 USING g_aaz.aaz64                #FUN-D40105 add
      IF SQLCA.sqlcode THEN
     ...
     #FUN-A50102--mod--end
 	 CALL cl_replace_sqldb(g_sql) RETURNING g_sql        #FUN-920032
     CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql  #FUN-A50102
      PREPARE p015_2_p4 FROM g_sql
      #EXECUTE p015_2_p4 USING g_existno,g_aaz.aaz64   #FUN-D40105  mark
      EXECUTE p015_2_p4 USING g_aaz.aaz64              #FUN-D40105  add
      IF SQLCA.sqlcode THEN
     ...
     #FUN-A50102--mod--end
 	  CALL cl_replace_sqldb(g_sql) RETURNING g_sql        #FUN-920032
      CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql  #FUN-A50102
      PREPARE p015_2_p5 FROM g_sql
      #EXECUTE p015_2_p5 USING g_existno,g_aaz.aaz64   #FUN-D40105 mark
      EXECUTE p015_2_p5 USING g_aaz.aaz64              #FUN-D40105 add
      IF SQLCA.sqlcode THEN
...
#FUN-B40056 -begin
      ...
      PREPARE p015_2_p9 FROM g_sql
      #EXECUTE p015_2_p9 USING g_existno,g_aaz.aaz64   #FUN-D40105 mark
      EXECUTE p015_2_p9 USING g_aaz.aaz64              #FUN-D40105 add
      IF SQLCA.sqlcode THEN
...
#FUN-B40056 -end
   ...
   INSERT INTO azo_file (azo01,azo02,azo03,azo04,azo05,azo06,azoplant,azolegal) #FUN-980003 add azoplant,azolegal
                 VALUES ('aglp015',g_user,g_today,g_msg,g_existno,'delete',g_plant,g_legal) #FUN-980003 add g_plant,g_legal
   #----------------------------------------------------------------------
...
FUNCTION p015_existno_chk()
   ...
   LET g_sql="SELECT aba01,aba02,aba03,aba04,abapost,aba19,abaacti,aba06 ",   #MOD-G30031 add aba06
            ...
            #" WHERE aba00 = ? AND aba06='CD'",   #MOD-G30031 mark
   ...
   FOREACH p015_t_c1 USING g_aaz.aaz64
     ...
     #MOD-G30031---add---end--
      ...
      #輸入的總帳傳票其日期不可小於帳別的關帳日
      IF gl_date <= l_aaa07 THEN
         CALL s_errmsg(gl_date,l_aba01,'','axm-164',1)
         LET g_success = 'N'
      END IF
      IF l_aba19 ='Y' THEN 
