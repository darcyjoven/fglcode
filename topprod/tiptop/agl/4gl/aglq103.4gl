...
# Modify.........: No.MOD-C70165 12/07/20 By yinhy 去除報表類型為5的資料
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm         RECORD
	             ...
	             bm    LIKE type_file.num5,  #Begin 期別
                     ...
                     axa02 LIKE axa_file.axa02,  #上层公司
...
MAIN
   ...
   LET tm.axa01 = ARG_VAL(25)  
   LET tm.axa02 = ARG_VAL(26)
   ...
   OPEN WINDOW q103_w AT 5,10
        WITH FORM "agl/42f/aglq103" ATTRIBUTE(STYLE = g_win_style)              
...
FUNCTION q103_tm()
   ...
   OPEN WINDOW q103_w1 AT p_row,p_col WITH FORM "agl/42f/aglq103_1" 
...
WHILE TRUE
    ...
    LET r_row=0
    LET l_sw = 1
    INPUT BY NAME tm.b,tm.a,tm.yy,tm.bm,tm.axa01,tm.axa02,
	  ...
	  SELECT mai02,mai03 INTO g_mai02,g_mai03 FROM mai_file
	   WHERE mai01 = tm.a AND maiacti IN ('Y','y')
             AND mai00 = tm.b
             #AND mai03 IN ('2','5')   #No.TQC-B70088  #MOD-C70165
             AND mai03 = '2'           #MOD-C70165
	  IF STATUS THEN 
	 ...
	 CASE
            ...
            WHEN INFIELD(a) 
               ...
               LET g_qryparam.where = " mai00 = '",tm.b,"' AND mai03 IN ('2')"  #MOD-C70165
   ...
   IF g_bgjob = 'Y' THEN
	 ...
	 LET l_cmd = l_cmd CLIPPED,          #(at time fglgo xxxx p1 p2 p3)
			 ...
			 " '",tm.a CLIPPED,"'",
                         ...
                         " '",tm.axa01 CLIPPED,"'",
                         " '",tm.axa02 CLIPPED,"'"
	 CALL cl_cmdat('aglq103',g_time,l_cmd)    # Execute cmd at later time
      END IF
   ...
   CREATE TEMP TABLE tmp_file(                                                                                                       
     ...
     FOREACH q103_c INTO maj.*
       ...
       IF NOT cl_null(maj.maj21) THEN
          SELECT SUM(axh08-axh09) INTO amt0 FROM axh_file
...
FUNCTION q103_out()
   ...
   LET l_table = cl_prt_temptable('aglq103',g_sql)  CLIPPED   #產生Temp Table
   IF l_table = -1 THEN
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80057--add--
      EXIT PROGRAM
   END IF
   ...
   END FOR
     ...
     CALL cl_prt_cs3('aglq103','aglq103',g_sql,g_str)
     #----------------------------- CR (4) --------------------------------------------------#
