...
#
# Pattern name...: aglg811.4gl
# Descriptions...: 科目異動碼明細表(aglg811)
# Input parameter:
# Return code....:
...
# Modify.........: No.FUN-B80158 12/01/06 By qirl MOD-B80029追單
#                                12/01/13 By chenying MOD-BC0172追單
# Modify.........: No.FUN-C30061 12/03/13 By yangtt GR調整      
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD                             # Print condition RECORD
             #wc      LIKE type_file.chr1000,    # Where condition   #No.FUN-680098    VARCHAR(300) #FUN-B80158 mark(MOD-BC0172追單) 
              wc      STRING,                    #FUN-B80158 add (MOD-BC0172追單) 
...
MAIN
 ...
 #  CALL cl_used(g_prog,g_time,1) RETURNING g_time #No.FUN-690114#FUN-B80158
   ...
   LET l_table1 = cl_prt_temptable('aglg8111',g_sql) CLIPPED                                                                  
   IF  l_table1 = -1 THEN 
   ...
   LET l_table2 = cl_prt_temptable('aglg8112',g_sql) CLIPPED                    
   IF  l_table2 = -1 THEN 
       CALL cl_gre_drop_temptable(l_table1||"|"||l_table2)    #FUN-B80158 add
       EXIT PROGRAM 
   END IF    
...
#   SELECT azi04,azi05 INTO g_azi04,g_azi05 FROM azi_file WHERE azi01=g_aza.aza17          #No.CHI-6A0004 mark
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'  THEN    # If background job sw is off
      CALL aglg811_tm(0,0)                    # Input print condition
   ELSE
      CALL aglg811()                          # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
   CALL cl_gre_drop_temptable(l_table1||"|"||l_table2)    #FUN-B80158 add
END MAIN
 
FUNCTION aglg811_tm(p_row,p_col)
...
DEFINE li_chk_bookno  LIKE type_file.num5             #No.FUN-B20054
   ...
   OPEN WINDOW aglg811_w AT p_row,p_col
     WITH FORM "agl/42f/aglg811"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
#No.FUN-B20054--add-end-- 
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglg811',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW aglg811_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
         CALL cl_gre_drop_temptable(l_table1||"|"||l_table2)    #FUN-B80158 add
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0073
         #l_sql     LIKE type_file.chr1000,         # RDSQL STATEMENT        #No.FUN-680098 VARCHAR(1000) #FUN-B80158 mark(MOD-BC0172追單)
          l_sql     STRING,                         #FUN-B80158 add (MOD-BC0172追單)  
...
#No.FUN-830104---Begin                                                                                                              
 ...
 #No.FUN-830104---End 
     ...
     DECLARE aglg811_za_cur CURSOR FOR
      SELECT za02,za05 FROM za_file WHERE za01 = "aglg811" AND za03 = g_rlang
 
     FOREACH aglg811_za_cur INTO g_i,l_za05
        LET g_x[g_i] = l_za05
     END FOREACH
 
#No.FUN-580010--start
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aglg811'
...
#No.FUN-580010--end
     ...
     PREPARE g811_abh_pre FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
     ...
     DECLARE g811_abh_cur CURSOR FOR g811_abh_pre
     ...
     PREPARE aglg811_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
     ...
     DECLARE aglg811_curs1 CURSOR FOR aglg811_prepare1
    ...
    #END IF
     ...
     PREPARE g811_abg_pre FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
     ...
     DECLARE g811_abg_cur CURSOR FOR g811_abg_pre       #No.FUN-830104 
 
#    CALL cl_outnam('aglg811') RETURNING l_name         #No.FUN-830104 
 
     CALL g811_show_field()     # FUN-5C0015 add
     CALL cl_prt_pos_len()      # FUN-5C0015 重算g_len
 
#    START REPORT aglg811_rep TO l_name                 #No.FUN-830104 
 
     LET g_pageno = 0
     INITIALIZE sr.* TO NULL
 
     #--->期初立帳尚有餘額者
     FOREACH aglg811_curs1 INTO sr.*
        IF SQLCA.sqlcode != 0 THEN
           CALL cl_err('foreach g811_curs1:',SQLCA.sqlcode,1)
           EXIT FOREACH
        END IF
 
        #---->扣除當月沖帳
        FOREACH g811_abh_cur USING sr.abi01,sr.abi02 INTO l_abh.*
           IF SQLCA.sqlcode != 0 THEN
...
#FUN-B40026   ---end     Mark 
       # FUN-5C0015 add(e)-------
#No.FUN-830104 ----Begin---- 
#       OUTPUT TO REPORT aglg811_rep(sr.*)
     ...
     FOREACH g811_abg_cur INTO sr.*
        ...
        FOREACH g811_abh_cur USING sr.abi01,sr.abi02 INTO l_abh.*
           ...
           LET sr.abi08 = sr.abi08 - l_abh.abh09
        END FOREACH
        IF tm.zero = "N" THEN    #MOD-B80158 add
           IF sr.abi08 = 0 THEN
              CONTINUE FOREACH
           END IF
        END IF                   #MOD-B80158 add
...
###GENGRE###                    "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED                                                           
        ...
        END IF                                                                                                                      
#                     p1       p2         p3         p4                                                                             
###GENGRE###        LET g_str = g_str,";",tm.e,";",g_azi04,";",g_azi05                                                                          
###GENGRE###        CALL cl_prt_cs3('aglg811','aglg811',g_sql,g_str)                                                                            
    CALL aglg811_grdata()    ###GENGRE###
#No.FUN-830104 ----End----                    
END FUNCTION
#No.FUN-830104 ----Begin---- 
#REPORT aglg811_rep(sr)
...
FUNCTION aglg811_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg811")
        IF handler IS NOT NULL THEN
            START REPORT aglg811_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED
          
            DECLARE aglg811_datacur1 CURSOR FROM l_sql
            FOREACH aglg811_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg811_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg811_rep
        END IF
...
REPORT aglg811_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE sr2 sr2_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_abi08_1_subtot LIKE abi_file.abi08
    DEFINE l_abi08_2_subtot LIKE abi_file.abi08
    #FUN-B80158-------add----str----
    DEFINE l_sql            STRING
    DEFINE l_title2         STRING
    DEFINE l_abi11          LIKE abi_file.abi11
    DEFINE l_abi12          LIKE abi_file.abi12
    DEFINE l_abi13          LIKE abi_file.abi13
    DEFINE l_abi14          LIKE abi_file.abi14
    DEFINE l_abi08_fmt      STRING
    DEFINE l_abi08_1_fmt    STRING
    DEFINE l_abi08_2_fmt    STRING
    #FUN-B80158-------add----end----
    DEFINE l_skip           LIKE type_file.chr1    #FUN-C30061 add                                                                
    DEFINE sr1_o            sr1_t                  #FUN-C30061 add
    
    ORDER EXTERNAL BY sr1.abi05,sr1.abi06,sr1.abi11,sr1.abi12,sr1.abi13  
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.abi05
            #FUN-B80158-------add----str----
            IF tm.e = 'Y' THEN
               LET l_title2 = sr1.aag13,'(',sr1.abi05,')',cl_gr_getmsg("gre-263",g_lang,1)   #FUN-C30061 add gre-263
            ELSE 
               LET l_title2 = sr1.aag02,'(',sr1.abi05,')',cl_gr_getmsg("gre-263",g_lang,1)   #FUN-C30061 add gre-263
            END IF
            PRINTX l_title2
            #FUN-B80158-------add----end----
            #FUN-C30061-------add---str---
            IF NOT cl_null(sr1_o.abi05) THEN
               IF sr1_o.abi05 != sr1.abi05 THEN
                  LET l_skip = 'Y'
               ELSE
                  LET l_skip = 'N'
               END IF
            ...
            PRINTX l_skip
            #FUN-C30061-------add---end---
        AFTER GROUP OF sr1.abi06
        BEFORE GROUP OF sr1.abi06
        BEFORE GROUP OF sr1.abi11
        BEFORE GROUP OF sr1.abi12
        BEFORE GROUP OF sr1.abi13
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B80158-------add----str----
            LET l_abi08_fmt = cl_gr_numfmt("abi_file","abi08",g_azi05)
            PRINTX l_abi08_fmt

            IF NOT cl_null(sr1.abi11) THEN
               LET l_abi11 = sr1.abi11
            END IF
            IF NOT cl_null(sr1.abi12) THEN
               LET l_abi12 = sr1.abi12
            END IF
            IF NOT cl_null(sr1.abi13) THEN
               LET l_abi13 = sr1.abi13
            END IF
            IF NOT cl_null(sr1.abi14) THEN
               LET l_abi14 = sr1.abi14
            END IF
            PRINTX l_abi11
            PRINTX l_abi12
            PRINTX l_abi13
            ...
            LET l_sql = "SELECT DISTINCT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,
                        ...
                        " AND abc01 = '",sr1.abi01 CLIPPED,"'",
                        " AND abc02 = ",sr1.abi02
            START REPORT aglg811_subrep01
            DECLARE aglg811_subrep01 CURSOR FROM l_sql
            FOREACH aglg811_subrep01 INTO sr2.*
                OUTPUT TO REPORT aglg811_subrep01 (sr2.*)
            END FOREACH
            FINISH REPORT aglg811_subrep01
            #FUN-B80158-------add----end----

            LET sr1_o.* = sr1.*    #FUN-C30061 add
            PRINTX sr1.*

        AFTER GROUP OF sr1.abi05
            #FUN-B80158-----add---str-----
            LET l_abi08_1_fmt = cl_gr_numfmt("abi_file","abi08",g_azi05)
            PRINTX l_abi08_1_fmt
            #FUN-B80158-----add---end-----
            LET l_abi08_1_subtot = GROUP SUM(sr1.abi08)
            PRINTX l_abi08_1_subtot
        AFTER GROUP OF sr1.abi11
            #FUN-B80158-----add---str-----
            LET l_abi08_2_fmt = cl_gr_numfmt("abi_file","abi08",g_azi05)
            PRINTX l_abi08_2_fmt
            #FUN-B80158-----add---end-----
            LET l_abi08_2_subtot = GROUP SUM(sr1.abi08)
            PRINTX l_abi08_2_subtot
        AFTER GROUP OF sr1.abi12
        AFTER GROUP OF sr1.abi13
...
#FUN-B80158-----add---str-----
REPORT aglg811_subrep01(sr2)
    DEFINE sr2 sr2_t
    DEFINE l_str   STRING

    FORMAT
        ON EVERY ROW
            LET l_str = sr2.l_chr,sr2.abc04
            PRINTX l_str
