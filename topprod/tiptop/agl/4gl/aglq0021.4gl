...
# Pattern name...: aglq0021.4gl
# Descriptions...: 子公司股本查詢作業
# Date & Author..: 13/02/26 By Lori(FUN-D20045)
# Modify.........: No:FUN-D20049 13/03/20 By Lori 1.aglq0021的資料來源一律由aglt002維護,不能有新增及單身的權限
#                                                 2.取消列印功能
...
MAIN
   ...
   OPEN WINDOW q0021_w AT p_row,p_col
     WITH FORM "agl/42f/aglq0021"  ATTRIBUTE (STYLE = g_win_style CLIPPED)
...
DEFINE l_sql STRING
    ...
    PREPARE q0021_prepare FROM g_sql      #預備一下
    DECLARE q0021_bcs                     #宣告成可捲動的
    SCROLL CURSOR WITH HOLD FOR q0021_prepare
    ...
    PREPARE q0021_precount FROM g_sql    
    DECLARE q0021_count CURSOR FOR q0021_precount
...
FUNCTION q0021_menu()
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         #      LET l_cmd = 'p_query "aglq0021" "',g_wc CLIPPED,'"'               
...
FUNCTION q0021_q()
   ...
   CALL q0021_cs()                      #取得查詢條件
   IF INT_FLAG THEN                     #使用者不玩了
      LET INT_FLAG = 0
      INITIALIZE g_axbb01,g_axbb02,g_axbb03,g_axbb04,g_axbb05 TO NULL
      RETURN
   END IF
   OPEN q0021_bcs                       #從DB產生合乎條件TEMP(0-30秒)
   IF SQLCA.sqlcode THEN                #有問題
      CALL cl_err('',SQLCA.sqlcode,0)
      INITIALIZE g_axbb01,g_axbb02 TO NULL
   ELSE
      OPEN q0021_count
      FETCH q0021_count INTO g_row_count
      DISPLAY g_row_count TO FORMONLY.cnt
      CALL q0021_fetch('F')            #讀出TEMP第一筆並顯示
   END IF
END FUNCTION

#處理資料的讀取
FUNCTION q0021_fetch(p_flag)
DEFINE
   p_flag          LIKE type_file.chr1,   #處理方式
   l_abso          LIKE type_file.num10   #絕對的筆數
   MESSAGE ""
   CASE p_flag
       WHEN 'N' FETCH NEXT     q0021_bcs INTO g_axbb01,g_axbb02,g_axbb03,g_axbb04,g_axbb05
       WHEN 'P' FETCH PREVIOUS q0021_bcs INTO g_axbb01,g_axbb02,g_axbb03,g_axbb04,g_axbb05
       WHEN 'F' FETCH FIRST    q0021_bcs INTO g_axbb01,g_axbb02,g_axbb03,g_axbb04,g_axbb05
       WHEN 'L' FETCH LAST     q0021_bcs INTO g_axbb01,g_axbb02,g_axbb03,g_axbb04,g_axbb05
       WHEN '/'
            ...
            FETCH ABSOLUTE l_abso q0021_bcs INTO g_axbb01,g_axbb02
   END CASE
...
#   DECLARE q0021_bcl CURSOR FROM g_forupd_sql      # LOCK CURSOR
#
...
#            BEGIN WORK
#            OPEN q0021_bcl USING g_axbb01,g_axbb02,g_axbb03,g_axbb04,g_axbb05,g_axbb[l_ac].axbb06
#            IF STATUS THEN
#               CALL cl_err("OPEN q0021_bcl:", STATUS, 1)
#               LET l_lock_sw = "Y"
#            ELSE
#               FETCH q0021_bcl INTO g_axbb[l_ac].*
#               IF STATUS THEN
#                  CALL cl_err("OPEN q0021_bcl:", STATUS, 1)
#                  LET l_lock_sw = "Y"
#               ELSE
#                  CALL q0021_set_axbb10(g_axbb[l_ac].axbb10) RETURNING g_axbb[l_ac].aag02
...
#            PREPARE q0021_pre_axbb10_1 FROM l_sql
#            DECLARE q0021_cur_axbb10_1 CURSOR FOR q0021_pre_axbb10_1
#            OPEN q0021_cur_axbb10_1
#            FETCH q0021_cur_axbb10_1 INTO l_n
#            IF l_n=0 THEN
...
#         LET l_axbb07 = 0
#         LET l_sum_axbb07 = 0
#         DECLARE q0021_cs1 CURSOR FOR
#            SELECT DISTINCT axbb01,axbb02,axbb03,axbb04,axbb05 FROM axbb_file
#             WHERE axbb04 = g_axbb04 
#               AND axbb05 = g_axbb05
#               AND axbb01 = g_axbb01
#         FOREACH q0021_cs1 INTO l_axbb.axbb01,l_axbb.axbb02,l_axbb.axbb03,l_axbb.axbb04,l_axbb.axbb05
...
#            LET l_axbb07 = 0
#            LET l_sum_axbb07 = 0
#            DECLARE q0021_cs2 CURSOR FOR
#               SELECT DISTINCT axbb01,axbb02,axbb03,axbb04,axbb05 FROM axbb_file
#                WHERE axbb04 = g_axbb04 and axbb05 = g_axbb05
#            FOREACH q0021_cs2 INTO l_axbb.axbb01,l_axbb.axbb02,l_axbb.axbb03,l_axbb.axbb04,l_axbb.axbb05
#               SELECT axbb07 INTO l_axbb07
#                 FROM axbb_file
#                WHERE axbb01 = l_axbb.axbb01 AND axbb02 = l_axbb.axbb02 AND axbb03 = l_axbb.axbb03
#                  AND axbb04 = l_axbb.axbb04 AND axbb05 = l_axbb.axbb05
...
FUNCTION q0021_b_fill(p_wc)                     #BODY FILL UP
DEFINE p_wc STRING
   ...
   LET g_sql = "SELECT axbb07,axbb08,axbb09,axbb10,'',axbb06",
               ...
               "   AND ",p_wc CLIPPED ,
               " ORDER BY axbb06"
   PREPARE q0021_prepare2 FROM g_sql       #預備一下
   DECLARE axbb_cs CURSOR FOR q0021_prepare2
   ...
   FOREACH axbb_cs INTO g_axbb[g_cnt].*     #單身 ARRAY 填充
      IF SQLCA.sqlcode THEN
         ...
         EXIT FOREACH
      END IF
      CALL q0021_set_axbb10(g_axbb[g_cnt].axbb10) RETURNING g_axbb[g_cnt].aag02
      LET g_cnt = g_cnt + 1
...
FUNCTION q0021_set_axbb10(p_axbb10)
DEFINE p_axbb10 LIKE axbb_file.axbb10,
   ...
   PREPARE q0021_pre_axbb10_2 FROM g_sql
   DECLARE q0021_cur_axbb10_2 CURSOR FOR q0021_pre_axbb10_2
   OPEN q0021_cur_axbb10_2
   FETCH q0021_cur_axbb10_2 INTO l_aag02
