...
FUNCTION aglp011_tm(p_row,p_col)
   ...
   OPEN WINDOW aglp011_w AT p_row,p_col WITH FORM "agl/42f/aglp011" 
   ...
   WHILE TRUE 
     ...
     #FUN-570145 --start--
      ...
      IF g_bgjob = 'Y' THEN
         SELECT zz08 INTO lc_cmd FROM zz_file WHERE zz01= 'aglp011'
         IF SQLCA.sqlcode OR lc_cmd IS NULL THEN
            CALL cl_err('aglp011','9031',1)   
         ELSE
            LET lc_cmd = lc_cmd CLIPPED,
                         ...
                         " '",tm.aac01 CLIPPED,"'",  
                         " '",g_bgjob CLIPPED,"'"
            CALL cl_cmdat('aglp011',g_time,lc_cmd CLIPPED)
         END IF
         CLOSE WINDOW aglp011_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
         EXIT PROGRAM
...
DEFINE l_axa02   LIKE axa_file.axa02
   ######INSERT 前先删除
   ######指定[年度],[期别],[族群]只对应一笔axi08='4' 的调整分录
   ...
   DELETE FROM axj_file WHERE axj00 = g_aaw01 AND axj01 = l_axi01
      AND axjconf = 'Y'    #FUN-B60134
   ####step1  insert axi_file
   INITIALIZE l_axi.* TO NULL
   LET l_axi.axi00 = g_aaw01
   CALL s_auto_assign_no("AGL",tm.aac01,g_today,"","axi_file","axi01",g_plant,2,l_axi.axi00)
   ...
   #FUN-B80135--add--str--
      IF l_axi.axi03 <g_year OR (l_axi.axi03=g_year AND l_axi.axi04<=g_month) THEN 
         LET g_showmsg=l_axi.axi03,"/",l_axi.axi04
         CALL s_errmsg('axi03,axi04',g_showmsg,'','axm-164',1)
      END IF 
   #FUN-B80135--add--end
   ...
   ELSE 
      SELECT aaw04 INTO l_axj.axj03 FROM aaw_file WHERE aaw00 = '0'
      ...
      #LET l_axj.axj05 = ' '  
      SELECT axa02 INTO l_axj.axj05 FROM axa_file 
       WHERE axa01 = tm.axi05 AND axa04 = 'Y'
