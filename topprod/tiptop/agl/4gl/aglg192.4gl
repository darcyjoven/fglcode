...
# Modify.........: No.MOD-850160 08/05/19 By Sarah g192_file裡的maj02改成LIKE maj_file.maj02
...
# Modify.........: No.FUN-B90027 11/09/05 By wangning 明細CR報表轉GR
...
TYPE sr1_t RECORD
    ...
    l_base1 LIKE type_file.num20_6,
    l_diff1 LIKE type_file.num20_6,
    l_per1 LIKE con_file.con06,
    l_amount2 LIKE aah_file.aah04,
    l_base2 LIKE type_file.num20_6,
    l_diff2 LIKE type_file.num20_6,
    l_per2 LIKE con_file.con06,
    l_amount3 LIKE aah_file.aah04,
    l_base3 LIKE type_file.num20_6,
    l_diff3 LIKE type_file.num20_6,
    l_per3 LIKE con_file.con06,
    l_amount4 LIKE aah_file.aah04,
    l_base4 LIKE type_file.num20_6,
    l_diff4 LIKE type_file.num20_6,
    l_per4 LIKE con_file.con06,
    l_amount5 LIKE aah_file.aah04,
    l_base5 LIKE type_file.num20_6,
    l_diff5 LIKE type_file.num20_6,
    l_per5 LIKE con_file.con06,
    l_amount6 LIKE aah_file.aah04,
    l_base6 LIKE type_file.num20_6,
    l_diff6 LIKE type_file.num20_6,
    l_per6 LIKE con_file.con06
END RECORD
###GENGRE###END
...
MAIN
 ...
 #  CALL cl_used(g_prog,g_time,1) RETURNING g_time #No.FUN-690114#FUN-B90027
   ...
   LET l_table = cl_prt_temptable('aglg192',g_sql) CLIPPED                                                                          
   IF l_table = -1 THEN
      CALL cl_gre_drop_temptable(l_table)      #No.FUN-B90027
      EXIT PROGRAM
   END IF                                                                                         
 
   DROP TABLE g192_file
   CREATE TEMP TABLE g192_file(
   ...
   CALL cl_used(g_prog,g_time,1) RETURNING g_time  #FUN-B900027
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL g192_tm()                        # Input print condition
   ELSE
      CALL g192()         
   END IF
...
FUNCTION g192_tm()
   ...
   END IF
   OPEN WINDOW g192_w AT p_row,p_col
        WITH FORM "agl/42f/aglg192" 
...
#FUN-AB0020 ---------------End---------------------          
   ...
   END INPUT
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW g192_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)      #No.FUN-B90027
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglg192',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW g192_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)      #No.FUN-B90027
...
FUNCTION g192()
   ...
   SELECT aaf03 INTO g_company FROM aaf_file WHERE aaf01 = tm.b
      AND aaf02 = g_rlang
   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aglg192'
   ...
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      ...
      EXIT PROGRAM 
   END IF
   DECLARE g192_c CURSOR FOR g192_p
...
#START FUN-570021  #新版寫法，部門要拆開，不要一次把六個部門寫入一個字串裡
       ...
       DECLARE tmp_curs CURSOR FOR
          SELECT * FROM g192_file ORDER BY maj02,no
  ...
  #             " ",os.Path.join(FGL_GETENV("TEMPDIR"),g_xml_rep CLIPPED)       #FUN-A30049
   ...
   RUN l_cmd
###GENGRE###   LET g_str = g_aaz.aaz77,";",g_mai02,";",tm.a,";",tm.p,";",tm.yy USING '<<<<',";",                                                
###GENGRE###               tm.bm USING'&&',";",tm.em USING'&&',";",tm.d,";",tm.h,";",g_azi05                                                    
###GENGRE###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                                 
###GENGRE###   CALL cl_prt_cs3('aglg192','aglg192',g_sql,g_str)                                                                                 
    CALL aglg192_grdata()    ###GENGRE###
END FUNCTION
 
FUNCTION g192_process(l_cn)
...
#FUN-AB0020 ---------------------Begin---------------------------
    ...
    END IF
#FUN-AB0020 ---------------------End-----------------------------                  
    PREPARE g192_sum FROM l_sql
    DECLARE g192_sumc CURSOR FOR g192_sum
...
#FUN-AB0020 ---------------------Begin---------------------------
    ...
    END IF
#FUN-AB0020 ---------------------End-----------------------------                 
    PREPARE g192_sum1 FROM l_sql1
    DECLARE g192_sumc1 CURSOR FOR g192_sum1
    IF STATUS THEN CALL cl_err('sum1 prepare',STATUS,1)
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
       CALL cl_gre_drop_temptable(l_table)      #No.FUN-B90027
       EXIT PROGRAM 
    END IF
    FOREACH g192_c INTO maj.*
        IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
        LET amt1 = 0 LET amt2 = 0 LET amt  = 0
        IF NOT cl_null(maj.maj21) THEN
           OPEN g192_sumc USING maj.maj21,maj.maj22
           FETCH g192_sumc INTO amt1   
           IF cl_null(amt1) THEN LET amt1 = 0 END IF
           OPEN g192_sumc1 USING maj.maj21,maj.maj22
           FETCH g192_sumc1 INTO amt2
       ...
       #CHI-A70050---mark---end---
        ...
        END IF
        INSERT INTO g192_file VALUES(l_cn,maj.maj02,maj.maj03,maj.maj04,
                                     maj.maj05,maj.maj07,maj.maj20,maj.maj20e,
                                     m_bal1,0,m_bal2,0)
        IF STATUS OR SQLCA.SQLERRD[3] = 0 THEN
           CALL cl_err3("ins","g192_file",l_cn,maj.maj02,SQLCA.sqlcode,"","ins g192_file",1)   #No.FUN-660123
           EXIT FOREACH
        END IF
    END FOREACH
END FUNCTION
...
FUNCTION aglg192_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg192")
        IF handler IS NOT NULL THEN
            START REPORT aglg192_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg192_datacur1 CURSOR FROM l_sql
            FOREACH aglg192_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg192_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg192_rep
        END IF
...
REPORT aglg192_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno      LIKE type_file.num5
    DEFINE g_used_name   STRING
    #FUN-B90027----add----str--------
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX g_user,g_pdate,g_prog,g_company,g_ptime,g_user_name    #FUN-B90027 add g_ptime,g_used_name
            #FUN-B90027----add----str--------
            LET l_yy = tm.yy      
            LET l_yy = l_yy.trim() 
            PRINTX l_yy             
        ...
        ON EVERY ROW
            ...
            LET l_unit = cl_gr_getmsg("gre-247",g_lang,tm.d)
            PRINTX l_unit

            LET l_fmt = cl_gr_numfmt("type_file","num20_6",g_azi05)
            PRINTX l_fmt
            #FUN-B90027----add----end------------
