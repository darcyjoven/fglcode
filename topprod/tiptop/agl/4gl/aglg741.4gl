...
# Modify.........: No.FUN-B80158 12/01/06 By yangtt MOD-BC0203追單
...
TYPE sr1_t RECORD
    ...
    #FUN-B80158----add----str---
    abb07_c  LIKE abb_file.abb07,
    abb07_d  LIKE abb_file.abb07,
...
MAIN
 ...
 #--str FUN-780059 add--#                                                                                                           
   ...
   LET g_sql = "aea05.aea_file.aea05,",                                                                                             
               ...
               "l_c.abb_file.abb07,",
               #FUN-B80158----add----str---
               "abb07_c.abb_file.abb07,",
               "abb07_b.abb_file.abb07,",
               "abb07_1.abb_file.abb07,",
               "abb07_2.abb_file.abb07"
               #FUN-B80158----add----end---
 
   LET l_table = cl_prt_temptable('aglg741',g_sql) CLIPPED   # 產生Temp Table                                                       
...
DEFINE li_chk_bookno     LIKE type_file.num5        #FUN-B20054
   ...
   OPEN WINDOW aglg741_w AT p_row,p_col
     WITH FORM "agl/42f/aglg741"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
# 
#    IF INT_FLAG THEN
#        LET INT_FLAG = 0 CLOSE WINDOW aglg741_w 
#        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
#        EXIT PROGRAM
#           
#    END IF
    ...
    IF g_bgjob = 'Y' THEN
       ...
       ELSE
         LET tm.wc=cl_replace_str(tm.wc, "'", "\"")
          ...
          CALL cl_cmdat('aglg741',g_time,l_cmd)      # Execute cmd at later time
       END IF
       CLOSE WINDOW aglg741_w
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
       CALL cl_gre_drop_temptable(l_table)   #FUN-B80158 add
       EXIT PROGRAM
    END IF
...
#       l_time          LIKE type_file.chr8               #No.FUN-6A0073
          l_sql         STRING,           # RDSQL STATEMENT        #No.FUN-680098 VARCHAR(1000)  #NO.FUN-B80158 MOD 1000  --> STRING
          l_sql1        STRING,           # RDSQL STATEMENT        #No.FUN-680098 VARCHAR(1000)  #NO.FUN-B80158 MOD 1000  --> STRING
          ...
          sr     RECORD
                    ...
                    azi04 LIKE azi_file.azi04,
             #      gem02 LIKE gem_file.gem02,      #FUN-C50005 ADD   #FUN-CB0058 mark
                    l_bal      LIKE abb_file.abb07,
                    l_d        LIKE abb_file.abb07,
                    l_c        LIKE abb_file.abb07,
                    #FUN-B80158----add----str---
   ...
   #--end FUN-780059 add--#
     ...
     PREPARE aglg741_prepare2 FROM l_sql1
        IF SQLCA.sqlcode != 0 THEN
         ...
         EXIT PROGRAM
      END IF
        DECLARE aglg741_curs2 CURSOR FOR aglg741_prepare2
#       CALL cl_outnam('aglg741') RETURNING l_name              #FUN-780059-mark
#       START REPORT aglg741_rep TO l_name                      #FUN-780059-mark
     ...
     LET l_sql = "SELECT ",
            ...
            # "  '',0,azi04,gem02,0,0,0,0,0,0,0",     #FUN-CB0058 mark
              "  '',0,azi04,0,0,0,0,0,0,0",    #FUN-CB0058 add 
           ...
           PREPARE aglg741_prepare1 FROM l_sql
           IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('prepare:',SQLCA.sqlcode,1)
            CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
            CALL cl_gre_drop_temptable(l_table)   #FUN-B80158 add
            EXIT PROGRAM
         END IF
           DECLARE aglg741_curs1 CURSOR FOR aglg741_prepare1
...
# No.FUN-C50005 mark--------------------star----------
#    LET l_sql=" SELECT abc04 FROM abc_file ",
#              " WHERE abc01='",tm.bookno,"' ",
#              "   AND abc01= ?  ",
#              "   AND abc02= ? "
#    PREPARE aglg741_prep1 FROM l_sql
#   DECLARE abc_curs CURSOR FOR aglg741_prep1
# No.FUN-C50005 mark--------------------end------------
    

      LET g_pageno = 0
        FOREACH aglg741_curs2 INTO sr1.*
           IF SQLCA.sqlcode != 0 THEN
...
#             "  abb11,abb12,abb13,abb14,",
#             "  abb31,abb32,abb33,abb34,abb35,abb36,",
#             "  '',0,azi04,0,0,0,0,0,0,0",   #FUN-B80158  add 4 0
#            #FUN-5C0015--------------------------------------------(E)
#             "  FROM aea_file,aba_file,abb_file,azi_file,aaa_file ",
...
#          #FUN-C50005--MARK--END
           LET l_flag='N'
           LET l_abb05_old='---'
    #      FOREACH aglg741_curs1 INTO sr.*  #FUN-C50005--MARK
           FOREACH aglg741_curs1 USING sr1.aag01 INTO sr.*            # No.FUN-C50005 add
             #FUN-CB0058----mark---str--
...
#-end--FUN-780059--add--#
#             OUTPUT TO REPORT aglg741_rep(sr.*)                #FUN-780059-mark
        ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-770052 *** ##                                                   
          #FUN-B80158-----add----str----
           IF sr.abb06 != '1' THEN
              LET sr.abb07_c = sr.abb07
           ELSE
              LET sr.abb07_c = 0
           END IF

           IF sr.abb06 = '1' THEN
              LET sr.abb07_d = sr.abb07
           ELSE
              LET sr.abb07_d = 0
           END IF

           IF sr.abb07 != 0 AND sr.abb06  = '1' THEN
           ...
           EXECUTE insert_prep USING                                                                                                
                   ...
                   sr.azi04,m_gem02,l_chr,'0',l_bal_1,l_buf,l_abc04,l_t_d,l_t_c,dept_d,      #TQC-810009 #FUN-C50005 MOD m_gem02-->sr.gem02  #FUN-CB0058 mod sr.gem02 ->m_gem02
                   dept_c,'0','0',sr.abb07_c,sr.abb07_d,sr.abb07_1,sr.abb07_2                #TQC-810009  #FUN-B80158 add 4'0'
          #------------------------------ CR (3) ------------------------------#
...
#         END FOREACH
         ...
         IF sr.l_bal IS NULL THEN LET sr.l_bal = 0 END IF
           ...
           LET sr.abb07=0
#          OUTPUT TO REPORT aglg741_rep(sr.*)                       #FUN-780059_mark
        ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-770052 *** ##                                                   
           ...
           EXECUTE insert_prep USING                                                                                                
                   ...
                   sr.azi04,m_gem02,l_chr,sr.l_bal,l_bal_1,l_buf,l_abc04,l_t_d,l_t_c,dept_d,      #TQC-810009  #FUN-C50005 MOD m_gem02-->sr.gem02  #FUN-CB0058 mod sr.gem02 ->m_gem02
                   dept_c,sr.l_d,sr.l_c,                                    
                   sr.abb07_c,sr.abb07_d,sr.abb07_1,sr.abb07_2        #FUN-B80158 add
          #------------------------------ CR (3) ------------------------------#
         END IF
           END FOREACH
        END FOREACH
#       FINISH REPORT aglg741_rep                                   #FUN-780059_mark
...
###GENGRE###    LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                                
    LET g_str = ''                                                                                                                  
###GENGRE###    LET g_str = tm.wc,";",bdate,";",edate,";",tm.u,";",'',";",                                                                    
###GENGRE###                tm.s                                                                                                                
###GENGRE###    CALL cl_prt_cs3('aglg741','aglg741',l_sql,g_str)                                                                                
    CALL aglg741_grdata()    ###GENGRE###
    #------------------------------ CR (4) ------------------------------#
END FUNCTION
 
#-str-FUN-780059--mark--#
#REPORT aglg741_rep(sr)
#  DEFINE
#         sr     RECORD
#                   aea05 LIKE aea_file.aea05,
#                   aea02 LIKE aea_file.aea02,
...
FUNCTION aglg741_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg741")
        IF handler IS NOT NULL THEN
            START REPORT aglg741_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY abb05 asc nulls first,aea05,aea02,aea03 desc"  #FUN-B80158 add
          
            DECLARE aglg741_datacur1 CURSOR FROM l_sql
            FOREACH aglg741_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg741_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg741_rep
        END IF
...
REPORT aglg741_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_abb07_1_subtot LIKE abb_file.abb07
    #FUN-B80158----add----str------
    DEFINE l_riqi   STRING
    DEFINE g_c37    LIKE type_file.chr1
    DEFINE g_c37_1  LIKE type_file.chr1
    DEFINE l_bal  LIKE abb_file.abb07
    DEFINE l_bal1   LIKE abb_file.abb07
    DEFINE l_bal_1   LIKE abb_file.abb07   #NO.FUN-B80158 add
    DEFINE l_RTotal_totsum   LIKE abb_file.abb07
    DEFINE l_RTotal_csum   LIKE abb_file.abb07
    DEFINE l_RTotal_dsum   LIKE abb_file.abb07
    DEFINE l_RTotal_depcsum   LIKE abb_file.abb07
    DEFINE l_RTotal_depdsum   LIKE abb_file.abb07
    DEFINE l_RTotal_totcsum   LIKE abb_file.abb07
    DEFINE l_RTotal_totdsum   LIKE abb_file.abb07
    DEFINE l_abb07_fmt        STRING
    DEFINE l_bal1_fmt         STRING
    DEFINE l_depsum_fmt       STRING
    DEFINE l_totsum_fmt       STRING
    ...
    DEFINE l_dsum             LIKE abb_file.abb07
    DEFINE l_csum             LIKE abb_file.abb07
    #FUN-B80158----add----end------
    DEFINE l_abb07_1          LIKE abb_file.abb07   #FUN-CB0058
    DEFINE l_abb07_2          LIKE abb_file.abb07   #FUN-CB0058
    
    ORDER EXTERNAL BY sr1.abb05,sr1.aea05
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            LET l_cnt3 = 0                  #FUN-B80158 add
              
        BEFORE GROUP OF sr1.abb05
        BEFORE GROUP OF sr1.aea05
            #FUN-B80158-----add----str----
            IF sr1.l_bal_1 < 0 THEN
               LET sr1.l_bal_1 = sr1.l_bal_1 * (-1) 
            ELSE
               LET sr1.l_bal_1 = sr1.l_bal_1
            END IF
            #FUN-B80158 
            PRINTX sr1.l_bal_1            
            IF sr1.l_bal_1 > = 0 THEN
            ...
            END IF
            PRINTX g_c37_1           
            LET l_bal1_fmt = cl_gr_numfmt("abb_file","abb07",sr1.azi04)
            PRINTX l_bal1_fmt
            #FUN-B80158-----add----end----
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B80158-----add----str----
            LET l_sql1 = "SELECT COUNT(aea05) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," WHERE abb05 = '",sr1.abb05,"'",
                         " AND abb07 != 0 "

            DECLARE aglg741_datacur2 CURSOR FROM l_sql1
            FOREACH aglg741_datacur2 INTO l_cnt2 
            END FOREACH
            IF sr1.abb07 != 0 THEN
               IF l_cnt2 = 0 THEN
                  LET l_cnt3 = 0
            ...
            IF sr1.abb07 != 0 AND (NOT cl_null(sr1.abb04) AND sr1.abb04 <> '') OR (NOT cl_null(sr1.l_abc04) AND sr1.l_abc04 <> '') THEN
               LET l_display = "Y"
            ELSE 
               LET l_display = "N"
            END IF
           ...
           #LET l_bal = l_RTotal_totsum + l_RTotal_dsum - l_RTotal_csum  #FUN-B80158
           #PRINTX l_bal                                                 #FUN-B80158
           #FUN-B80158
           IF cl_null(l_RTotal_totsum) THEN LET l_RTotal_totsum = 0 END IF
           IF cl_null(l_dsum) THEN LET l_dsum = 0 END IF
           IF cl_null(l_csum) THEN LET l_csum = 0 END IF
           LET l_RTotal_totsum = sr1.l_bal_1 + l_RTotal_totsum
           LET l_dsum = sr1.abb07_1 + l_dsum
           LET l_csum = sr1.abb07_2 + l_csum
           LET l_balance = l_RTotal_totsum + l_dsum - l_csum
            IF l_balance > = 0 THEN
               LET g_c37 = 'D'
            ELSE
               LET g_c37 = 'C'
            ...
            PRINTX g_c37           
            IF l_balance <0 THEN
               LET l_balance = l_balance * (-1)
            ELSE
               LET l_balance = l_balance
            END IF
            PRINTX l_balance
            #FUN-B80158----add---end--
        ...
        AFTER GROUP OF sr1.abb05
            #FUN-B80158-----add----str----
            LET l_depsum_fmt = cl_gr_numfmt("abb_file","abb07",sr1.azi04)
            PRINTX l_depsum_fmt
            LET l_RTotal_totdsum = GROUP SUM(sr1.abb07_1)
            PRINTX l_RTotal_totdsum                  
            LET l_RTotal_totcsum = GROUP SUM(sr1.abb07_2)
            PRINTX l_RTotal_totcsum
            #FUN-B80158-----add----end----
        ...
        AFTER GROUP OF sr1.aea05
         ...
         #  LET l_cnt3 = l_cnt3 + 1
            ...
            LET l_RTotal_totsum = 0 
            LET l_dsum = 0
            LET l_csum = 0

            LET l_totsum_fmt = cl_gr_numfmt("abb_file","abb07",sr1.azi04)
            PRINTX l_totsum_fmt
            LET l_RTotal_dsum = GROUP SUM(sr1.abb07_c)            
            PRINTX l_RTotal_dsum 
            LET l_RTotal_csum = GROUP SUM(sr1.abb07_d)            
            PRINTX l_RTotal_csum 
            LET l_RTotal_depdsum = GROUP SUM(sr1.abb07_1)
            PRINTX l_RTotal_depdsum
            LET l_RTotal_depcsum = GROUP SUM(sr1.abb07_2)
            PRINTX l_RTotal_depcsum
            #FUN-B80158-----add----end----
