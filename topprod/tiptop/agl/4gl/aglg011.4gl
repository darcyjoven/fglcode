...
# Modify.........: No.FUN-B90028 11/09/16 By qirl  明細CR報表轉GR
# Modify.........: No.FUN-BA0048 11/10/09 By zhangweib 給tm.aaw01赋值
# Modify.........: No.FUN-B90028 12/01/12 By xuxz FUN-B50001還原
# Modify.........: No.FUN-B90028 12/01/12 By qirl FUN-BA0048還原
...
DEFINE tm  RECORD
            ...
            aaz641 LIKE aaz_file.aaz641,       #合并帳別 #FUN-B50001 #FUN-B90028 not mark
            #aaw01 LIKE aaw_file.aaw01,       #合并帳別 #FUN-B90028 mark
...
DEFINE g_sql      STRING             
DEFINE g_str      STRING            
DEFINE g_aaz641   LIKE aaz_file.aaz641   #FUN-B50001 #FUN-B90028 not mark
#DEFINE g_aaw01   LIKE aaw_file.aaw01 #FUN-B90028 mark
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglg011',g_sql) CLIPPED   # 產生Temp Table   
   IF l_table = -1 THEN 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B90028 add 
...
#  LET tm.bm = ARG_VAL(11)   #FUN-A90032
   ...
   IF cl_null(tm.aaz641) THEN LET tm.aaz641 = g_aza.aza81 END IF   #FUN-B50001  #FUN-B90028 not mark
   #IF cl_null(tm.aaw01) THEN LET tm.aaw01 = g_aza.aza81 END IF    #FUN-B90028 mark
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL g011_tm()                   # Input print condition
      ELSE CALL g011()                      # Read data and create out-file
   END IF
...
FUNCTION g011_tm()
   ...
   OPEN WINDOW g011_w AT p_row,p_col WITH FORM "agl/42f/aglg011"
   ...
   #使用預設帳別之幣別       
   #SELECT aaw01 INTO tm.aaw01 FROM aaw_file   #FUN-BA0048   Add              #FUN-B90028 mark 
   #DISPLAY BY NAME tm.aaw01                   #FUN-BA0048   Add              #FUN-B90028 mark                                                                                         
   ...
   WHILE TRUE
      ...
      LET r_row=0
      LET l_sw = 1
      INPUT BY NAME tm.axa01,tm.axa02,tm.b,tm.aaz641,   #FUN-B50001 #FUN-B90028 not mark
      #INPUT BY NAME tm.axa01,tm.axa02,tm.b,tm.aaw01,  #FUN-B90028 mark
...
#           CALL g011_getdbs()
           ...
           #CALL s_get_aaz641(g_dbs_axz03) RETURNING g_aaz641             #MOD-A70060 #FUN-A30122  mark by vealxu
            CALL s_get_aaz641(g_plant_axz03) RETURNING g_aaz641           #FUN-A30122 add by velaxu #FUN-B50001 #FUN-B90028 not mark
            #CALL s_get_aaz641(g_plant_axz03) RETURNING g_aaw01 #FUN-B90028 mark
  
#FUN-A30122 --End
            IF cl_null(g_aaz641) THEN   #FUN-B50001 #FUN-B90028 not mark
            #IF cl_null(g_aaw01) THEN #FUN-B90028 mark
               CALL cl_err(g_axz03,'agl-601',1)
               NEXT FIELD axa02
            ELSE
               LET tm.aaz641 = g_aaz641 #FUN-B90028 add
               DISPLAY BY NAME tm.aaz641 #FUN-B90028 add
             #FUN-B50001--mod--str--
         ...
         AFTER FIELD a
            ...
            SELECT mai02,mai03 INTO g_mai02,g_mai03 FROM mai_file
             WHERE mai01 = tm.a 
               AND mai00 = tm.aaz641   #FUN-B50001 #FUN-B90028 not mark
               #AND mai00 = tm.aaw01#FUN-B90028 mark
...
#FUN-B50001--mod--end
            ...
            PREPARE g011_cnt_p FROM g_sql
            DECLARE g011_cnt_c CURSOR FOR g011_cnt_p
            OPEN g011_cnt_c
            FETCH g011_cnt_c INTO l_cnt
            IF cl_null(l_cnt) OR l_cnt=0 THEN
               CALL cl_err('','agl-965',1)
               NEXT FIELD aaz641   #FUN-B50001 #FUN-B90028 not mark
               #NEXT FIELD aaw01#FUN-B90028 mark
            END IF   
         ...
         ON ACTION CONTROLP
            CASE
               ...
               WHEN INFIELD(a)
                  ...
                  LET g_qryparam.where = " mai00 = '",tm.aaz641,"'" #FUN-B50001 AAZ641-AAW01 #FUN-B90028 aaw01-->aaz641
...
#FUN-B50001--mod--end
      ...
      END INPUT
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW g011_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)   #FUN-B90028 add
         EXIT PROGRAM
      END IF
...
#                     " '",tm.bm CLIPPED,"'",    #FUN-A90032 mark
          ...
          CALL cl_cmdat('aglg011',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW g011_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)   #FUN-B90028 add
...
FUNCTION g011()
   ...
   DROP TABLE aglg011_tmp
   CREATE TEMP TABLE aglg011_tmp(
      maj23    LIKE maj_file.maj23,
      maj20    LIKE type_file.chr50,
      bal      LIKE type_file.num20_6,
      per      LIKE type_file.num20_6,
   ...
   SELECT aaf03 INTO g_company FROM aaf_file
    WHERE aaf01=tm.aaz641 AND aaf02=g_rlang    #FUN-B50001 #FUN-B90028 not mark
    #WHERE aaf01=tm.aaw01 AND aaf02=g_rlang#FUN-B90028 mark
   ...
   LET l_sql = "SELECT * FROM maj_file ",
               ...
               "   AND maj23[1,1]='1' ",
               " ORDER BY maj02"
   PREPARE g011_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)   #FUN-B90028 add
      EXIT PROGRAM 
   END IF
   DECLARE g011_c CURSOR FOR g011_p
   ...
   #會計科目各期余額檔
         LET g_sql="SELECT SUM(axh08-axh09) ",
                   ...
                   "   AND axh06 = '",tm.yy,"' AND axh07 = '",tm.em,"'", 
   ...
   FOREACH g011_c INTO maj.*
      ...
      IF NOT cl_null(maj.maj21) THEN
        ...
        #          " WHERE axh00 = '",tm.aaz641,"'",    #FUN-B50001 #FUN-B90028 not mark
        #          #" WHERE axh00 = '",tm.aaw01,"'",#FUN-B90028 mark
        #          "   AND axh00 = aag00 ",
        #          "   AND axh01 = '",tm.axa01,"'",
        #          "   AND axh02 = '",tm.axa02,"'",
        #          "   AND axh03 = '",tm.b,"'",    
...
#       #          "   AND axh06 = '",tm.yy,"' AND axh07 <= '",tm.bm,"'", #FUN-A90032
        ...
        #OPEN sel_amt1_cs 
        #FUN-C50007--MARK--END
         OPEN sel_amt1_cs USING maj.maj21,maj.maj22         #FUN-C50007 ADD
      ...
      IF maj.maj03='3' OR maj.maj03='4' THEN
         IF NOT cl_null(maj.maj20) THEN
            INSERT INTO aglg011_tmp VALUES
   ...
   #抓報表結構裡帳戶左邊最大序號
   SELECT MAX(maj02) INTO l_maj02_l FROM aglg011_tmp WHERE maj23='11'
   #抓報表結構裡帳戶右邊最大序號
   SELECT MAX(maj02) INTO l_maj02_r FROM aglg011_tmp WHERE maj23='12'
   IF cl_null(l_maj02_l) THEN LET l_maj02_l = 0 END IF
   IF cl_null(l_maj02_r) THEN LET l_maj02_r = 0 END IF
   LET r_row = 0  LET l_row = 0
   DECLARE g011_c1 CURSOR FOR SELECT * FROM aglg011_tmp ORDER BY maj23,maj02,line #CHI-A70061 add line
   FOREACH g011_c1 INTO tmp1.*
...
###GENGRE###               tm.bm,";",tm.e,";",                             
###GENGRE###               l_lastday,";",g_basetot1  #CHI-A70046 add g_basetot1          
###GENGRE###   CALL cl_prt_cs3('aglg011','aglg011',g_sql,g_str) 
    CALL aglg011_grdata()    ###GENGRE###
END FUNCTION

#FUN-A90032 --Begin
FUNCTION g011_set_entry()                                                                                                           
   CALL cl_set_comp_entry("q1,em,h1",TRUE)#FUN-A90032 add 
END FUNCTION                                                                                                                        
                                                                                                                                    
FUNCTION g011_set_no_entry()                                                                                                        
   IF tm.axa06 ="1" THEN
...
#              " WHERE aaz00 = '0'"
#  PREPARE g011_pre_11 FROM g_sql
#  DECLARE g011_cur_11 CURSOR FOR g011_pre_11
#  OPEN g011_cur_11
#  FETCH g011_cur_11 INTO g_aaz641
#END FUNCTION	　
#FUN-A30122 --End


###GENGRE###START
FUNCTION aglg011_grdata()
    DEFINE l_sql    STRING
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg011")
        IF handler IS NOT NULL THEN
            START REPORT aglg011_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg011_datacur1 CURSOR FROM l_sql
            FOREACH aglg011_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg011_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg011_rep
        END IF
...
REPORT aglg011_rep(sr1)
    ...
    DEFINE sr1_o      sr1_t
    #FUN-B90028----add----end--------
    DEFINE l_display2 LIKE type_file.chr1   #FUN-C50007
    ...
    FORMAT
        ...
        ON EVERY ROW
            #FUN-B90028----add----str--------
            IF Not cl_null(sr1_o.maj20_l) AND sr1_o.maj20_l = sr1.maj20_l then
            ...
            IF NOT cl_null(g_basetot1) THEN
               ...
               LET l_per_l = (sr1.bal_l/l_per1)*100 USING '###.&&'
            ELSE
               LET l_per_l = " "
            END IF
               PRINTX l_per_l
            ...
            IF NOT cl_null(g_basetot1) THEN
               LET l_per2 = g_basetot1
               LET l_per_r = (sr1.bal_r/l_per2)*100 USING '###.&&'
            ELSE
               LET l_per_r = " "
            END IF
              PRINTX l_per_r
