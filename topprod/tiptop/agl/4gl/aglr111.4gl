...
# Modify.........: No.CHI-940060 09/08/21 By Sarah 當左右只有其中一方列印碼是3時,該行仍會顯示(FUNCTION r111()重寫)
...
# Modify.........: NO.MOD-BB0042 11/11/03 By Dido 左右列印問題調整 
# Modify.........: No:MOD-C30617 12/03/13 By Polly 查詢條件後需再重新給予 INPUT 欄位變數
# Modify.........: No:MOD-C40079 12/04/12 By Polly 增加帳別欄位檢查
# Modify.........: No:MOD-C40163 12/04/20 By Elise 當為空白行時，maj.maj03應塞欄位為1,不然列印某欄位選3且空白行數>0時,會造成此欄位的上一個科目的金額也會有底線
# Modify.........: No.TQC-C50042 12/05/07 By zhangweib 修改開窗q_mai去除報表性質為5\6的資料
# Modify.........: No.TQC-C90047 12/09/07 By lujh  “報表編號”開窗時應只挑到mai03='2'資產負債類對應帳套資料。 
# Modify.........: No.TQC-C90041 12/09/10 By lujh  合計計算有誤 
# Modify.........: No.MOD-C80152 12/10/23 By Polly 異動帳別時，將新帳別的aaa03預設給予tm.r/tm.p
# Modify.........: No.MOD-D30040 13/03/06 By fengmy 抓取的邏輯按gglq811修改
...
MAIN
   ...
   LET g_sql = "maj31_l.maj_file.maj31,",    #FUN-B90140   Add
               ...
               "maj31_r.maj_file.maj31,",    #FUN-B90140   Add
   ...
   LET l_table = cl_prt_temptable('aglr111',g_sql) CLIPPED   # 產生Temp Table   
   IF l_table = -1 THEN EXIT PROGRAM END IF                  # Temp Table產生   
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,                        
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?)"   #MOD-890121 add 2?  #MOD-A10023 add ?  #FUN-B90140   Add ?,?
...
FUNCTION r111_tm()
   ...
   DEFINE l_cnt          LIKE type_file.num5      #MOD-C40079 add
   ...
   OPEN WINDOW r111_w AT p_row,p_col WITH FORM "agl/42f/aglr111"
   ...
   WHILE TRUE
      ...
      INPUT BY NAME tm.b,tm.a,tm.yy,tm.bm,tm.e,tm.f,      #No.FUN-740020
         ...
         AFTER FIELD a
           ...
           #No.TQC-C50042   ---start---   Add
            ELSE
               #IF g_mai03 = '5' OR g_mai03 = '6' THEN     #TQC-C90047  mark
               IF g_mai03 <> '2' THEN                      #TQC-C90047  add
                  #CALL cl_err('','agl-268',0)             #TQC-C90047  mark
                  CALL cl_err('','agl-270',0)              #TQC-C90047  add                          
                  NEXT FIELD a
               END IF
            ...
            CASE
               WHEN INFIELD(a)
                 ...
                 #LET g_qryparam.where = " mai00 = '",tm.b,"'"  #No.FUN-740020   #No.TQC-C50042   Mark
                  #LET g_qryparam.where = " mai00 = '",tm.b,"' AND mai03 NOT IN ('5','6')"  #No.TQC-C50042   Add   #TQC-C90047 mark
                  LET g_qryparam.where = " mai00 = '",tm.b,"' AND mai03 = '2'"      #TQC-C90047  add
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
          ...
          CALL cl_cmdat('aglr111',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW r111_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
         EXIT PROGRAM
...
FUNCTION r111()
   ...
   DEFINE prt_l     DYNAMIC ARRAY OF RECORD                   #--- 陣列 for 資產類 (左)
                       maj31    LIKE maj_file.maj31,          #FUN-B90140  add
   ...
   DROP TABLE aglr111_tmp
   CREATE TEMP TABLE aglr111_tmp(
      maj31    LIKE maj_file.maj31,          #FUN-B90140 add
      maj23    LIKE maj_file.maj23,
      maj20    LIKE maj_file.maj20,        #MOD-A10167 modify  chr50->maj20
      bal      LIKE type_file.num20_6,
   ...
   FOREACH r111_c INTO maj.*
      ...
      IF NOT cl_null(maj.maj21) THEN
         ...
         #TQC-C90041--add--str--
         IF maj.maj09 = '-' OR maj.maj07 = '2' THEN   #MOD-D30040 add maj07
            LET  amt1= amt1 *-1
         END IF
         #TQC-C90041--add--end--
      END IF
      ...
      IF maj.maj03='3' OR maj.maj03='4' THEN
         IF NOT cl_null(maj.maj20) THEN
            INSERT INTO aglr111_tmp VALUES
             (maj.maj31,maj.maj23,maj.maj20,sr.bal1,per1,'1',maj.maj02,2)  #FUN-B90140  add  maj.maj31
         END IF
         INSERT INTO aglr111_tmp VALUES
          (maj.maj31,maj.maj23,maj.maj20,sr.bal1,per1,maj.maj03,maj.maj02,2)  #FUN-B90140  add  maj.maj31
      ELSE
         INSERT INTO aglr111_tmp VALUES
          (maj.maj31,maj.maj23,maj.maj20,sr.bal1,per1,maj.maj03,maj.maj02,2)  #FUN-B90140  add  maj.maj31
      END IF
      IF maj.maj04 > 0 THEN
         ...
         FOR i = 1 TO maj.maj04
            INSERT INTO aglr111_tmp VALUES
            #('',maj.maj23,maj.maj20,0,0,maj.maj03,maj.maj02,1)  #FUN-B90140  add  ''  #MOD-C40163 mark
             ('',maj.maj23,maj.maj20,0,0,'1',maj.maj02,1)          #MOD-C40163 maj03->1
         END FOR
      END IF
   END FOREACH
 
   #抓報表結構裡帳戶左邊最大序號
   SELECT MAX(maj02) INTO l_maj02_l FROM aglr111_tmp WHERE maj23='11'
   #抓報表結構裡帳戶右邊最大序號
   SELECT MAX(maj02) INTO l_maj02_r FROM aglr111_tmp WHERE maj23='12'
   IF cl_null(l_maj02_l) THEN LET l_maj02_l = 0 END IF
   IF cl_null(l_maj02_r) THEN LET l_maj02_r = 0 END IF
   LET r_row = 0  LET l_row = 0
   DECLARE r111_c1 CURSOR FOR SELECT * FROM aglr111_tmp ORDER BY maj23,maj02,line     #CHI-A40035 add line
   FOREACH r111_c1 INTO tmp1.*
      ...
      IF l_row_l > l_row_r THEN
         ...
         IF prt_l[l_row_l].maj03='3' OR prt_l[l_row_l].maj03='4' THEN
            IF prt_r[l_row_r].maj03='3' OR prt_r[l_row_r].maj03='4' THEN
              ...
              #INITIALIZE prt_l[l_row_l].* TO NULL            #MOD-AB0198 #MOD-BB0042 mark 
               INITIALIZE prt_r[l_row_r].* TO NULL                        #MOD-BB0042
               LET prt_r[l_row_l-1].* = prt_r[l_row_r-1].*
      ...
      IF l_row_l < l_row_r THEN
         ...
         ELSE
            IF prt_l[l_row_l].maj03='3' OR prt_l[l_row_l].maj03='4' THEN
              ...
              #LET prt_l[l_row_l+1].maj31=''    #FUN-B90140  add  #MOD-BB0042 mark
               LET prt_r[l_row_r+1].maj31=''                      #MOD-BB0042 
   ...
   LET g_str = g_mai02,";",tm.a,";",tm.p,";",tm.d,";",tm.yy,";",                
               tm.bm,";",tm.e,";",
               l_lastday,";",g_basetot1    #CHI-A40035 add         #MOD-920278 add
                ,";",tm.acc_code            #FUN-B90140   Add
   CALL cl_prt_cs3('aglr111','aglr111',g_sql,g_str) 
END FUNCTION
#No.FUN-9C0072 精簡程式碼 
