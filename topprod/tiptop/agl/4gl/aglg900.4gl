...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglg900',g_sql) CLIPPED
   IF l_table = -1 THEN 
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL aglg900_tm()	        	# Input print condition
      ELSE CALL aglg900()			# Read data and create out-file
   END IF
...
FUNCTION aglg900_tm()
   ...
   END IF
   OPEN WINDOW aglg900_w AT p_row,p_col
        WITH FORM "agl/42f/aglg900"
...
################################################################################
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW aglg900_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)               #FUN-B80158 add
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglg900',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW aglg900_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)               #FUN-B80158 add
      EXIT PROGRAM
   END IF
...
#       l_time          LIKE type_file.chr8	    #No.FUN-6A0073
     ...
     #CALL cl_outnam('aglg900') RETURNING l_name
     #START REPORT aglg900_rep TO l_name
     #LET g_pageno = 0
     #No.FUN-750129  --End
     LET g_cnt    = 1
   #----> 取得"起始期別"的起始與截止日期
  ...
  #IF g_aza.aza26 = '2' THEN
      IF tm.a1 = 'Y' THEN CALL aglg900_1_2() END IF
      IF tm.a2 = 'Y' THEN CALL aglg900_2_2() END IF
  #ELSE
  #   IF tm.a1 = 'Y' THEN CALL aglg900_1() END IF
  #   IF tm.a2 = 'Y' THEN CALL aglg900_2() END IF
  #END IF
   #CHI-710005 MARK END
   #No.MOD-5B0097  --end
   IF tm.a3 = 'Y' THEN CALL aglg900_3() END IF
   IF tm.a4 = 'Y' THEN CALL aglg900_4() END IF
   IF tm.a5 = 'Y' THEN CALL aglg900_5() END IF
   #End
 
   #No.FUN-750129  --Begin
   #FINISH REPORT aglg900_rep
   #CALL cl_prt(l_name,g_prtway,g_copies,g_len)
###GENGRE###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
###GENGRE###   LET g_str = tm.yy,";",tm.bm,";",tm.yy,";",tm.em,";",t_azi04
#   CALL cl_prt_cs3('aglg900','aglg900',g_sql,g_str)#TQC-AB0194
#TQC-AB0194--add--str--
   IF g_cnt >1 THEN
###GENGRE###      CALL cl_prt_cs3('aglg900','aglg900',g_sql,g_str)
    CALL aglg900_grdata()    ###GENGRE###
   ELSE
...
FUNCTION aglg900_1()
   DEFINE    l_sql 	LIKE type_file.chr1000,	# RDSQL STATEMENT        #No.FUN-680098  VARCHAR(600)
    ...
    DELETE FROM t;
     ...
     #No.A116  取得明細所屬統制科目
    DECLARE gglg900_1_1 CURSOR FOR
     SELECT UNIQUE aag08
    ...
    FOREACH gglg900_1_1 INTO l_aag08
       IF STATUS THEN CALL cl_err('gglg900_1_1',STATUS,1) EXIT FOREACH END IF
       IF NOT cl_null(l_aag08) THEN
          DECLARE aglg900_1 CURSOR FOR
...
#               CALL cl_err('ins #1:',STATUS,1) # NO.FUN-660123
    ...
    END FOREACH
     ...
     DECLARE aglg900_curs1 CURSOR FOR
         SELECT '1' ,account,SUM(amt_1),SUM(amt_2),0
          FROM t
          GROUP BY account
          HAVING SUM(amt_1) != SUM(amt_2)
     FOREACH aglg900_curs1 INTO sr.*
        IF STATUS THEN CALL cl_err('for-1:',STATUS,1) EXIT FOREACH END IF
        #No.FUN-750129  --Begin
        #OUTPUT TO REPORT aglg900_rep(sr.*)
        EXECUTE insert_prep 
...
#        CALL cl_err('ins #2:',STATUS,1) # NO.FUN-660123
   ...
   DECLARE aglg900_curs2 CURSOR FOR
      SELECT '2' ,account,SUM(amt_1),SUM(amt_2),0
          FROM t
          GROUP BY account
          HAVING SUM(amt_1) != SUM(amt_2)
 
   FOREACH aglg900_curs2 INTO sr.*
...
#        CALL cl_err('ins #3:',STATUS,1) # NO.FUN-660123 
    ...
    DECLARE aglg900_curs3 CURSOR FOR
      SELECT '3',account,amt_1,amt_2,0
          FROM y
          WHERE amt_1 != amt_2
 
    FOREACH aglg900_curs3 INTO sr.*
...
#        CALL cl_err('ins #4:',STATUS,1) # NO.FUN-660123 
    ...
    DECLARE aglg900_curs4 CURSOR FOR
      SELECT '4',account,amt_1,amt_2,0
          FROM y
          WHERE amt_1 != amt_2
 
    FOREACH aglg900_curs4 INTO sr.*
...
#        CALL cl_err('ins #5:',STATUS,1) # NO.FUN-660123
    ...
    END FOREACH
     ...
     DECLARE aglg900_curs6 CURSOR FOR
        SELECT '5' , b, a,0,0
            FROM v
            GROUP BY b,a
 
     FOREACH aglg900_curs6 INTO sr.*
...
FUNCTION aglg900_1_2()
   DEFINE    l_sql     LIKE type_file.chr1000,     # RDSQL STATEMENT        #No.FUN-680098  VARCHAR(600)
    ...
    DELETE FROM t;
     ...
     #No.A116  取得明細所屬統制科目
    DECLARE gglg900_1_1_2 CURSOR FOR
     SELECT UNIQUE aag08
       FROM aah_file,aag_file
      WHERE aah01 = aag01 AND aag07 = '2'       #明細帳戶
        AND aah00 = tm.bookno                  #No.FUN-740020
     ...
     SELECT UNIQUE aag08
      ...
      WHERE aah01 = aag01 AND aag07 = '1'       #統治科目
        ...
        AND aah02 = tm.yy                          #年度
        AND aah03 BETWEEN tm.bm AND tm.em          #期別
    FOREACH gglg900_1_1_2 INTO l_aag08
       IF STATUS THEN CALL cl_err('gglg900_1_1_2',STATUS,1) EXIT FOREACH END IF
       IF NOT cl_null(l_aag08) THEN
          DECLARE aglg900_1_2 CURSOR FOR
           SELECT '1' a, aah01 account,(aah04-aah05) amt_1,0 amt_2,0 amt_3
             FROM aag_file, aah_file
            WHERE aag01 = aah01 AND aag07 = '1'        #統制帳戶
              AND aah00 = tm.bookno                      #No.FUN-740020                  
...
#               CALL cl_err('ins #1_2:',STATUS,1) # NO.FUN-660123
    ...
    END FOREACH
     ...
     DECLARE aglg900_curs1_2 CURSOR FOR
         SELECT '1' ,account,SUM(amt_1),SUM(amt_2),0
          FROM t
          GROUP BY account
          HAVING SUM(amt_1) != SUM(amt_2)
     FOREACH aglg900_curs1_2 INTO sr.*
...
FUNCTION aglg900_2_2()
   ...
   DECLARE aglg900_2_2 CURSOR FOR
   SELECT '1' a, aas01 account,(aas04-aas05) amt_1,0 amt_2,0 amt_3
          FROM aag_file, aas_file
          WHERE aag01 = aas01 AND aag07 = '1'        #統制帳戶
            AND aas00 = tm.bookno                    #No.FUN-740020 
...
#        CALL cl_err('ins #2_2:',STATUS,1) # NO.FUN-660123
   ...
   DECLARE aglg900_curs2_2 CURSOR FOR
      SELECT '2' ,account,SUM(amt_1),SUM(amt_2),0
          FROM t
          GROUP BY account
          HAVING SUM(amt_1) != SUM(amt_2)
 
   FOREACH aglg900_curs2_2 INTO sr.*
...
FUNCTION aglg900_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg900")
        IF handler IS NOT NULL THEN
            START REPORT aglg900_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY name1,name2"           #FUN-B80158 add
          
            DECLARE aglg900_datacur1 CURSOR FROM l_sql
            FOREACH aglg900_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg900_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg900_rep
        END IF
...
REPORT aglg900_rep(sr1)
    ...
    DEFINE l_name3_fmt    STRING
    DEFINE l_name4_fmt    STRING
    ...
    DEFINE l_em           STRING
    #FUN-B80158-------add-----end-----
    DEFINE l_name4       LIKE aah_file.aah04   #FUN-CB0058
    DEFINE l_name3       LIKE aah_file.aah04   #FUN-CB0058
    DEFINE l_str1_2 STRING     #FUN-CB0058
    DEFINE l_str2_2 STRING     #FUN-CB0058
    DEFINE l_str3_2 STRING     #FUN-CB0058
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B80158-------add-----str-----
            IF sr1.name1 = '1' OR sr1.name1 = '2' THEN
               LET l_str1 = sr1.name1,' ',cl_gr_getmsg("gre-217",g_lang,1),':'
            ELSE
               IF sr1.name1 = '3' OR sr1.name1 = '4' THEN
                  LET l_str1 = sr1.name1,' ',cl_gr_getmsg("gre-217",g_lang,2),':'
               ELSE
                  IF sr1.name1 = '5' THEN
                     IF sr1.name1 = '0' THEN
                        LET l_str1 = sr1.name1,' ',cl_gr_getmsg("gre-217",g_lang,3),':'
                     ELSE
                        LET l_str1 = sr1.name1,' ',cl_gr_getmsg("gre-217",g_lang,4),':'
                     END IF
                  END IF
               END IF
            END IF
        #   PRINTX l_str1     #FUN-CB0058 mark

            IF sr1.name1 = '1' THEN
               LET l_str2 = cl_gr_getmsg("gre-218",g_lang,1),':'
            ELSE
               IF sr1.name1 = '2' THEN 
                  LET l_str2 = cl_gr_getmsg("gre-218",g_lang,2),':'
               ELSE
                 IF sr1.name1 = '3' THEN
                    LET l_str2 = cl_gr_getmsg("gre-218",g_lang,3),':'
                 ELSE
                    IF sr1.name1 = '4' THEN
                       LET l_str2 = cl_gr_getmsg("gre-218",g_lang,4),':'
                    ELSE
                       IF sr1.name1 = '5' THEN
                          IF sr1.name3 = 0 THEN 
                             LET l_str2 = cl_gr_getmsg("gre-218",g_lang,5)
                          ELSE
                             LET l_str2 = cl_gr_getmsg("gre-218",g_lang,6)
                          END IF
                       END IF
                    END IF
                 END IF
               END IF
            END IF
           #PRINTX l_str2   #FUN-CB0058 mark 

            IF sr1.name1 = '1' THEN
               LET l_str3 = cl_gr_getmsg("gre-219",g_lang,1),':'
            ELSE
               IF sr1.name1 = '2' THEN
                  LET l_str3 = cl_gr_getmsg("gre-219",g_lang,2),':'
               ELSE
                 IF sr1.name1 = '3' THEN
                    LET l_str3 = cl_gr_getmsg("gre-219",g_lang,3),':'
                 ELSE
                    IF sr1.name1 = '4' THEN
                       LET l_str3 = cl_gr_getmsg("gre-219",g_lang,4),':'
                    ELSE
                       IF sr1.name1 = '5' THEN
                          LET l_str3 = "" 
                       END IF
               ...
               END IF
            END IF
           #PRINTX l_str3   #FUN-CB0058 mark

            LET l_str1_2 = l_str1    #FUN-CB0058
            LET l_str2_2 = l_str2    #FUN-CB0058
            LET l_str3_2 = l_str3    #FUN-CB0058

            IF l_str1_1 = l_str1_2 THEN
               LET l_display1 = 'N'
               LET l_str1 = "  "      #FUN-CB0058
            ELSE
               LET l_display1 = 'Y'
               LET l_str1 = l_str1    #FUN-CB0058
            END IF
            PRINTX l_display1

            IF l_str2_1 = l_str2_2 THEN
               LET l_display2 = 'N'
               LET l_str2 = "  "      #FUN-CB0058
            ELSE
               LET l_display2 = 'Y'
               LET l_str2 = l_str2    #FUN-CB0058
            END IF
            PRINTX l_display2


            IF l_str3_1 = l_str3_2 OR sr1.name1 = '5' THEN    #FUN-CB0058 add OR sr1.name1 = '5'
               LET l_display3 = 'N'
               LET l_str3 = "  "      #FUN-CB0058
            ELSE
               LET l_display3 = 'Y'
               LET l_str3 = l_str3    #FUN-CB0058
            END IF
            PRINTX l_display3

            #FUN-CB0058----add---str--
            IF sr1.name1 != '5' THEN
               LET l_name4 = sr1.name4 
               LET l_name3 = sr1.name3 
            ELSE
               LET l_name4 = NULL      
               LET l_name3 = NULL      
            END IF
            ...
            LET l_name3_fmt = cl_gr_numfmt("aah_file","aah01",t_azi04)
            PRINTX l_name3_fmt
            LET l_name4_fmt = cl_gr_numfmt("aah_file","aah04",t_azi04)
            PRINTX l_name4_fmt
 
            LET l_str1_1 = l_str1_2
            LET l_str2_1 = l_str2_2
            LET l_str3_1 = l_str3_2
            #FUN-B80158-------add-----end-----

            PRINTX l_str1    #FUN-CB0058
            PRINTX l_str2    #FUN-CB0058
            PRINTX l_str3    #FUN-CB0058
