...
# Modify.........: No:FUN-D40020 12/11/02 By wangrr CR轉XtraGrid,財務報表改善，起始日期截止日期默認為當期的第一天和最後一天，人員、傳票編號開窗
...
MAIN
   ...
   LET g_sql = " aba02.aba_file.aba02,",
              ...
              #" err.type_file.chr1   "  #FUN-D40020 mark
               " err.type_file.chr1000"  #FUN-D40020 add

   LET l_table = cl_prt_temptable('aglx931',g_sql) CLIPPED
...
FUNCTION x931_tm(p_row,p_col)
   ...
   OPEN WINDOW x931_w AT p_row,p_col
     WITH FORM "agl/42f/aglx931"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
#FUN-D40020--MOD--str--
   ...
   CALL getDate() RETURNING tm.b_aba02,tm.e_aba02
#FUN-D40020--MOD--end
   LET g_pdate = g_today
   LET g_rlang = g_lang
   LET g_bgjob = 'N'
      ...
      INPUT BY NAME tm.o_aba00,tm.o_aba01,tm.n_aba00,tm.n_aba01,tm.b_aba02,
         ...
         AFTER FIELD o_aba00
           ...
           #No.FUN-D40020 ---end  --- Mark
            IF NOT cl_null(tm.o_aba00) THEN   #No.FUN-D40020   Add
            #No.FUN-670005--begin
...
#              CALL cl_err(tm.o_aba00,'agl-095',0)   #No.FUN-660123
         ...
         AFTER FIELD o_aba01
           ...
           #END IF
           #No.FUN-D40020 ---end  --- Mark
            IF NOT cl_null(tm.o_aba01) THEN   #No.FUN-D40020   Add 
...
#              CALL cl_err(tm.o_aba01,'agl-095',0)   #No.FUN-660123
         ...
         AFTER FIELD n_aba00
           ...
           #END IF
           #No.FUN-D40020 ---end  --- Mark
            IF NOT cl_null(tm.n_aba00) THEN   #No.FUN-D40020   Add
            #No.FUN-670005--begin
...
#              CALL cl_err(tm.n_aba00,'agl-095',0)   #No.FUN-660123
         ...
         AFTER FIELD n_aba01
           ...
           #END IF
           #No.FUN-D40020 ---end  --- Mark
            IF NOT cl_null(tm.n_aba01) THEN   #No.FUN-D40020   Add 
            SELECT aac01 FROM aac_file
...
#              CALL cl_err(tm.n_aba01,'agl-095',0)   #No.FUN-660123
         ...
         AFTER FIELD e_aba02
           ...
           #END IF
           #IF tm.e_aba02 < tm.b_aba02 THEN
           #   CALL cl_err('','agl-031',0)   #FUN-D40020 add
           #   NEXT FIELD e_aba02
           #END IF
           #No.FUN-D40020 ---end  --- Mark
         ...
         #FUN-D40020--MOD--str--
            IF NOT cl_null(tm.b_aba01) THEN
               IF cl_null(tm.e_aba01) THEN
                  LET tm.e_aba01=tm.b_aba01
                  DISPLAY tm.e_aba01 TO FORMONLY.e_aba01
               ELSE
                  IF tm.b_aba01>tm.e_aba01 THEN
                     CALL cl_err('','agl-278',0) 
                     NEXT FIELD b_aba01
                  END IF 
               END IF
            END IF
         ...
         AFTER FIELD e_aba01
            IF tm.e_aba01 < tm.b_aba01 THEN
               CALL cl_err('','agl-277',0)   #FUN-D40020 add
               NEXT FIELD e_aba01
            END IF
               ...
               WHEN INFIELD(n_aba01) #單據性質
                 #CALL q_aac(FALSE,TRUE,tm.n_aba01,' ',' ',' ',g_sys)  RETURNING tm.n_aba01  #TQC-670008
                  CALL q_aac(FALSE,TRUE,tm.n_aba01,' ',' ',' ','AGL')  RETURNING tm.n_aba01  #TQC-670008
                   ...
                   NEXT FIELD n_aba01
               #FUN-D40020--add--str--
               WHEN INFIELD(b_aba01)
                  CALL cl_init_qry_var()
                  LET g_qryparam.form = 'q_aba01'
                  LET g_qryparam.default1 = tm.b_aba01
                  LET g_qryparam.where="aba01 LIKE '",tm.o_aba01,"-%'"
                  CALL cl_create_qry() RETURNING tm.b_aba01
                  DISPLAY tm.b_aba01 TO FORMONLY.b_aba01
                  NEXT FIELD b_aba01
               WHEN INFIELD(e_aba01)
                  CALL cl_init_qry_var()
                  LET g_qryparam.form = 'q_aba01'
                  LET g_qryparam.default1 = tm.e_aba01
                  LET g_qryparam.where="aba01 LIKE '",tm.n_aba01,"-%'"
                  CALL cl_create_qry() RETURNING tm.e_aba01
                  DISPLAY tm.e_aba01 TO FORMONLY.e_aba01
                  NEXT FIELD e_aba01
               WHEN INFIELD(pers)
                  CALL cl_init_qry_var()
                  LET g_qryparam.form = 'q_abauser'
                  LET g_qryparam.default1 = tm.pers
                  CALL cl_create_qry() RETURNING tm.pers
                  DISPLAY tm.pers TO FORMONLY.pers
                  NEXT FIELD pers
               #FUN-D40020--add--end
            END CASE
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglx931',g_time,l_cmd)      # Execute cmd at later time
         END IF
...
#         e_aba01     VARCHAR(12),
   ...
   LET l_sql = "SELECT *",
               "  FROM aba_file, abb_file, abm_file",
   ...
   PREPARE x931_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE x931_cs1 CURSOR FOR x931_prepare1
 
   #No.FUN-750129  --Begin
   #CALL cl_outnam('aglx931') RETURNING l_name
   #START REPORT x931_rep TO l_name
   #No.FUN-750129  --End
 
   FOREACH x931_cs1 INTO l_aba.*,l_abb.*,l_abm.*      #FUN-C40112 add l_abm.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
      #No.FUN-750129  --Begin
      #OUTPUT TO REPORT x931_rep(l_aba.*,l_abb.*)
      #CALL x931_insert(l_aba.*,l_abb.*)              #FUN-C40112 mark
      CALL x931_insert(l_aba.*,l_abb.*,l_abm.*)      #FUN-C40112 add
      #No.FUN-750129  --End
...
#      LET e_aba01=tm.n_aba01,'-',tm.e_aba01[5,12]
   ...
   PREPARE x931_prepare2 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare2:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE x931_cs2 CURSOR FOR x931_prepare2
 
   FOREACH x931_cs2 INTO l_aba.*,l_abb.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
   ...
   #No.FUN-750129  --Begin
   #FINISH REPORT x931_rep
   #CALL cl_prt(l_name,g_prtway,g_copies,g_len)
###XtraGrid###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
###XtraGrid###   LET g_str = ''
###XtraGrid###   CALL cl_prt_cs3('aglx931','aglx931',g_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
    LET g_xgrid.order_field='aba02'  #FUN-D40020 add
...
#No.FUN-750129  --Begin
FUNCTION x931_insert(p_aba,p_abb,p_abm)        #FUN-C40112 add p_abm
#FUNCTION x931_insert(p_aba,p_abb)             #FUN-C40112 mark
...
DEFINE l_str    LIKE type_file.chr1000   #FUN-D40020 add
      ...
      IF p_abb.abb00!=' ' AND p_abb.abb00 IS NOT NULL AND
        ...
        #FUN-C40112-Add End-----
         ...
         IF STATUS=100 THEN
            LET l_str=cl_gr_getmsg('gre-216',g_lang,'1') #FUN-D40020 add
            EXECUTE insert_prep USING
                    p_aba.aba02,p_aba.aba01,p_abb.abb02,p_aba.aba06,
                   #p_aba.aba16,p_aba.aba17,'1'       #FUN-C40112 Mark     #g_x[11]
                   #p_abm.abm05,p_abm.abm06,'1'       #FUN-C40112 Add   #FUN-D40020 mark
                    p_abm.abm05,p_abm.abm06,l_str     #FUN-D40020 add
            LET flag=1
         ELSE
            IF p_aba.aba08!=m_aba.aba08 OR p_aba.aba09!=m_aba.aba09 THEN
               LET l_str=cl_gr_getmsg('gre-216',g_lang,'2') #FUN-D40020 add
               EXECUTE insert_prep USING
                       p_aba.aba02,p_aba.aba01,p_abb.abb02,p_aba.aba06,
                      #p_aba.aba16,p_aba.aba17,'2'     #FUN-C40112 Mark     #g_x[9]
                      #p_abm.abm05,p_abm.abm06,'2'     #FUN-C40112 Add   #FUN-D40020 mark
                       p_abm.abm05,p_abm.abm06,l_str   #FUN-D40020 add
            END IF
         END IF
      END IF
      IF (p_abb.abb00=' ' OR p_abb.abb00 IS NULL) AND
         ...
         LET l_str=cl_gr_getmsg('gre-216',g_lang,'3') #FUN-D40020 add
         EXECUTE insert_prep USING
                 p_aba.aba02,p_aba.aba01,'',p_aba.aba06,
                # '','','3'  #g_x[17]    #FUN-D40020 mark
                  '','',l_str   #FUN-D40020 add
      ELSE
         IF flag=0 THEN
            ...
            IF STATUS=100 THEN
               LET l_str=cl_gr_getmsg('gre-216',g_lang,'4') #FUN-D40020 add
               EXECUTE insert_prep USING
                       p_aba.aba02,p_aba.aba01,p_abb.abb02,p_aba.aba06,
                      #p_aba.aba16,p_aba.aba17,'4'       #FUN-C40112 Mark      #g_x[10]
                      #p_abm.abm05,p_abm.abm06,'4'       #FUN-C40112 Add   #FUN-D40020 mark
                       p_abm.abm05,p_abm.abm06,l_str     #FUN-D40020 add
            ELSE
               IF p_abb.abb03!=m_abb.abb03 THEN
                  LET l_str=cl_gr_getmsg('gre-216',g_lang,'5') #FUN-D40020 add
                  EXECUTE insert_prep USING
                          p_aba.aba02,p_aba.aba01,p_abb.abb02,p_aba.aba06,
                         #p_aba.aba16,p_aba.aba17,'5'     #FUN-C40112 Mark      #g_x[12]
                         #p_abm.abm05,p_abm.abm06,'5'     #FUN-C40112 Add  #FUN-D40020 mark
                          p_abm.abm05,p_abm.abm06,l_str   #FUN-D40020 add
               END IF
               IF p_abb.abb11!=m_abb.abb11 OR p_abb.abb12!=m_abb.abb12 OR
                  ...
                  LET l_str=cl_gr_getmsg('gre-216',g_lang,'6') #FUN-D40020 add
                  EXECUTE insert_prep USING
                          p_aba.aba02,p_aba.aba01,p_abb.abb02,p_aba.aba06,
                         #p_aba.aba16,p_aba.aba17,'6'    #FUN-C40112 Mark      #g_x[13]
                         #p_abm.abm05,p_abm.abm06,'6'    #FUN-C40112 Add  #FUN-D40020 mark
                          p_abm.abm05,p_abm.abm06,l_str  #FUN-D40020 add
               END IF
               IF p_abb.abb05!=m_abb.abb05 THEN
                  LET l_str=cl_gr_getmsg('gre-216',g_lang,'7') #FUN-D40020 add
                  EXECUTE insert_prep USING
                          p_aba.aba02,p_aba.aba01,p_abb.abb02,p_aba.aba06,
                         #p_aba.aba16,p_aba.aba17,'7'     #FUN-C40112 Mark      #g_x[14]
                         #p_abm.abm05,p_abm.abm06,'7'     #FUN-C40112 Add   #FUN-D40020 mark
                          p_abm.abm05,p_abm.abm06,l_str   #FUN-D40020 add
               END IF
               IF p_abb.abb06!=m_abb.abb06 THEN
                  LET l_str=cl_gr_getmsg('gre-216',g_lang,'8') #FUN-D40020 add
                  EXECUTE insert_prep USING
                          p_aba.aba02,p_aba.aba01,p_abb.abb02,p_aba.aba06,
                         #p_aba.aba16,p_aba.aba17,'8'    #FUN-C40112 Mark      #g_x[15]
                         #p_abm.abm05,p_abm.abm06,'8'    #FUN-C40112 Add  #FUN-D40020 mark
                          p_abm.abm05,p_abm.abm06,l_str  #FUN-D40020 add
               END IF
               IF p_abb.abb07!=m_abb.abb07 THEN
                  LET l_str=cl_gr_getmsg('gre-216',g_lang,'9') #FUN-D40020 add
                  EXECUTE insert_prep USING
                          p_aba.aba02,p_aba.aba01,p_abb.abb02,p_aba.aba06,
                         #p_aba.aba16,p_aba.aba17,'9'   #FUN-C40112 Mark      #g_x[16]
                         #p_abm.abm05,p_abm.abm06,'9'   #FUN-C40112 Add  #FUN-D40020 mark
                          p_abm.abm05,p_abm.abm06,l_str #FUN-D40020 add
               END IF
            END IF
         END IF
...
DEFINE l_mm   LIKE type_file.num5,
       ...
       l_bm   LIKE type_file.chr20,
       l_em   LIKE type_file.chr20

   LET l_mm = MONTH(g_today)
   LET l_yy = YEAR(g_today)
   LET l_bm = l_yy,"/",l_mm,"/01"
   CASE
      WHEN (l_mm=1 OR l_mm=3 OR l_mm=5 OR l_mm=7 OR l_mm=8 OR l_mm=10 OR l_mm=12)
         LET l_em = l_yy,"/",l_mm,"/31"
      WHEN (l_mm=4 OR l_mm=6 OR l_mm=9 OR l_mm=11)
         LET l_em = l_yy,"/",l_mm,"/30"
      WHEN (l_mm = 2)
         IF (l_yy MOD 4 = 0 AND l_yy MOD 100 !=0) OR (l_yy MOD 400 = 0) THEN
            LET l_em = l_yy,"/",l_mm,"/29"
         ELSE
            LET l_em = l_yy,"/",l_mm,"/28"
         END IF
   END CASE
   RETURN l_bm,l_em
END FUNCTION
