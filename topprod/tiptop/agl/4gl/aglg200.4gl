...
# Modify.........: No.TQC-740093 07/04/18 By johnray aglg200_p1的SQL語句有錯
...
# Modify.........: No:FUN-B80158 11/08/30 By yangtt 明細類CR轉換成GRW
...
DEFINE g_sql2   STRING                              #FUN-B80158
...
###GENGRE###START
TYPE sr1_t RECORD
    order1 LIKE type_file.chr100,     #FUN-B80158 add
    order2 LIKE type_file.chr100,     #FUN-B80158 add
    ...
    amt1 LIKE npr_file.npr06,
    amt1_1 LIKE npr_file.npr06,
    amt2_d LIKE npr_file.npr06,
    amt2_f LIKE npr_file.npr06,
    amt3_d LIKE npr_file.npr06,
    amt3_f LIKE npr_file.npr06,
...
MAIN
   ...
   #No.FUN-780061 --start--
   LET g_sql="order1.type_file.chr100,order2.type_file.chr100,",      #FUN-B80158 add
             "npr00.npr_file.npr00,npr01.npr_file.npr01,npr02.npr_file.npr02,",
   ...
   LET l_table = cl_prt_temptable('aglg200',g_sql) CLIPPED
   IF l_table = -1 THEN 
   ...
   LET g_sql = "INSERT INTO ", g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?)"  #MOD-870261 add ?  #FUN-B80158 add 2?
...
FUNCTION aglg200_tm(p_row,p_col)
...
DEFINE l_combo        ui.ComboBox             #FUN-B80158 add
   ...
   OPEN WINDOW aglg200_w AT p_row,p_col
     WITH FORM "agl/42f/aglg200"  ATTRIBUTE (STYLE = g_win_style CLIPPED) 
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                        ...
                        " '",g_rep_clas CLIPPED,"'",     
                        " '",g_template CLIPPED,"'"     
            CALL cl_cmdat('aglg200',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0073
   ...
   DEFINE l_order   ARRAY[3] OF LIKE type_file.chr100   #FUN-B80158  add
 
   CALL cl_del_data(l_table)                         #No.FUN-780061  
   SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
   
#   CALL cl_outnam('aglg200') RETURNING l_name       #No.FUN-780061
...
#
#   CALL cl_prt_pos_len()
#   START REPORT aglg200_rep TO l_name
   #No.FUN-780061 --end--
...
#                 "       '',''   ",     #No.TQC-760154 mark
   ...
   IF STATUS THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE aglg200_c1 CURSOR FOR aglg200_p1
 
   FOREACH aglg200_c1 INTO sr.*
      IF STATUS != 0 THEN
...
#      OUTPUT TO REPORT aglg200_rep(sr.*)    #No.FUN-780061
      #No.FUN-780061 --start--
      IF sr.npr11 != '-' THEN
      ...
      FOR g_i = 1 TO 2
          CASE WHEN  tm.s[g_i,g_i] = '1' LET l_order[g_i] = sr.npr01
               WHEN  tm.s[g_i,g_i] = '2' LET l_order[g_i] = sr.npr03 CLIPPED
               OTHERWISE                 LET l_order[g_i] = ' ' 
          END CASE
      END FOR
      ...
      EXECUTE insert_prep USING l_order[1],l_order[2],    #FUN-B80158 add
                                sr.npr00,sr.npr01,sr.npr02,sr.npr03,sr.npr11,
...
#   FINISH REPORT aglg200_rep                   #No.FUN-780061        
#
#   CALL cl_prt(l_name,g_prtway,g_copies,g_len) #No.FUN-780061
   #No.FUN-780061 --start--
   IF tm.a = 'N' THEN 
#     LET l_name = 'aglg200'    #FUN-B80158 mark
      LET g_template = 'aglg200'  #FUN-B80158 add
      CALL aglg200_grdata()          #FUN-B80158 add 
   ELSE 
#     LET l_name = 'aglg200_1'   #FUN-B80158 mark
      LET g_template = 'aglg200_1'   #FUN-B80158 add
      CALL aglg200_grdata()          #FUN-B80158 add 
   END IF
...
###GENGRE###   LET g_str = g_str,";",l_str,";",tm.o,";",l_aza17,";",
###GENGRE###               tm.s[1,1],";",tm.s[2,2],";",tm.t[1,1],";",
###GENGRE###               tm.t[2,2],";",tm.u[1,1],";",tm.u[2,2],";",
###GENGRE###               tm.c,";",g_azi04,";",g_azi05   #MOD-870261 add g_azi05
###GENGRE###   LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED
###GENGRE###   CALL cl_prt_cs3('aglg200',l_name,l_sql,g_str)
   #No.FUN-780061 --end--
END FUNCTION
#No.FUN-780061 --start-- mark
#REPORT aglg200_rep(sr)
...
#                     "   FROM curr_tmp "
#        END IF
#        PREPARE g200_prepare_tot FROM l_sql
#        IF SQLCA.sqlcode THEN
#           CALL cl_err('prepare_tot:',SQLCA.sqlcode,1)
#           RETURN
#        END IF
#        DECLARE curr_temp_tot CURSOR FOR g200_prepare_tot
...
FUNCTION aglg200_grdata()
    ...
    #FUN-B80158-----add---str---
   #LET g_sql1 = "SELECT COUNT(DISTINCT order1) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
    LET g_sql1 = "SELECT COUNT(*) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                 " WHERE npr00 = ?"
    DECLARE aglg200_repcur01 CURSOR FROM g_sql1 
    LET g_sql2 = "SELECT COUNT(*) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
    IF tm.t[1,1] = 'Y' OR tm.u[1,1] = 'Y' THEN
           LET g_sql2 = g_sql2," WHERE (order1=? OR order1 IS NULL) AND npr00 = ?"
    ELSE
           LET g_sql2 = g_sql2," WHERE npr00 = ?" 
    END IF
    DECLARE aglg200_repcur02 CURSOR FROM g_sql2 
    #FUN-B80158---add---end---

    WHILE TRUE
        CALL cl_gre_init_pageheader()            
        LET handler = cl_gre_outnam("aglg200")
        IF handler IS NOT NULL THEN
            START REPORT aglg200_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY npr00,order1,order2,npr01,npr02,npr03,npr11"   #FUN-B80158
          
            DECLARE aglg200_datacur1 CURSOR FROM l_sql
            FOREACH aglg200_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg200_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg200_rep
        END IF
...
#FUN-B80158----------add----str---
PRIVATE FUNCTION aglg200_replace_ord_desc(p_desc)   
...
REPORT aglg200_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B80158----------add----str---
    DEFINE l_cnt1          LIKE type_file.num10  #FUN-B80158
    DEFINE l_cnt2          LIKE type_file.num10  #FUN-B80158
    DEFINE l_ord1_cnt      LIKE type_file.num10  #FUN-B80158
    DEFINE l_ord2_cnt      LIKE type_file.num10  #FUN-B80158
    DEFINE l_skip_ord1     LIKE type_file.chr1   #FUN-B80158
    DEFINE l_skip_ord2     LIKE type_file.chr1   #FUN-B80158
    DEFINE l_sum_amt1      LIKE npr_file.npr06
    DEFINE l_sum_amt1_1    LIKE npr_file.npr06    
    DEFINE l_sumamt11      LIKE npr_file.npr06
    DEFINE l_sumamt1_1     LIKE npr_file.npr06    
    DEFINE l_sumamt4       LIKE npr_file.npr06 
    DEFINE l_sumamt4_1      LIKE npr_file.npr06 
    DEFINE l_amt4          LIKE npr_file.npr06 
    DEFINE l_amt4_1        LIKE npr_file.npr06 
    DEFINE l_sumamt1_lorder1  LIKE npr_file.npr06
    DEFINE l_sumamt1_lorder2  LIKE npr_file.npr06
    DEFINE l_sumamt4_lorder1  LIKE npr_file.npr06
    DEFINE l_sumamt4_lorder2  LIKE npr_file.npr06
    DEFINE l_sumamt11order1  LIKE npr_file.npr06
    DEFINE l_sumamt11order2  LIKE npr_file.npr06
    DEFINE l_sumamt4order1  LIKE npr_file.npr06
    DEFINE l_sumamt4order2  LIKE npr_file.npr06
    DEFINE l_amt4_lorder1   LIKE npr_file.npr06
    DEFINE l_amt4_lorder2   LIKE npr_file.npr06
    DEFINE l_amt4order1     LIKE npr_file.npr06
    DEFINE l_sumamt1sub     LIKE npr_file.npr06
    DEFINE l_sumamt2_dsub    LIKE npr_file.npr06
    DEFINE l_sumamt3_dsub    LIKE npr_file.npr06
    DEFINE l_sumamt4sub    LIKE npr_file.npr06
    DEFINE l_amt4order2     LIKE npr_file.npr06
    DEFINE l_CorD           LIKE type_file.chr1
    DEFINE l_CorD1            LIKE type_file.chr1
    DEFINE l_CorDlorder1      LIKE type_file.chr1 
    DEFINE l_CorDlorder2      LIKE type_file.chr1 
    DEFINE l_Corder1          LIKE type_file.chr1 
    DEFINE l_Corder2          LIKE type_file.chr1 
    DEFINE l_npr00             STRING
    DEFINE l_npr06_11_fmt      STRING
    DEFINE l_npr06_11_1_fmt    STRING
    DEFINE l_npr06_1_fmt       STRING
    DEFINE l_npr06_2_fmt       STRING
    DEFINE l_amt_f_11_fmt      STRING
    DEFINE l_amt_f_1_fmt       STRING
    DEFINE l_amt_f_2_fmt       STRING
    DEFINE l_amt_f1_11_fmt     STRING
    DEFINE l_subp_fmt          STRING
    DEFINE l_subp_1_fmt        STRING
    DEFINE l_ord1_show         STRING
    DEFINE l_ord2_show         STRING
    DEFINE l_ord1_skip         STRING
    DEFINE l_ord2_skip         STRING
    DEFINE l_str               LIKE type_file.chr1000
    DEFINE l_aza19             LIKE aza_file.aza19
    DEFINE l_CorD1sub          LIKE type_file.chr1
    DEFINE l_CorDsub           LIKE type_file.chr1
    DEFINE l_sumamt4_11        LIKE npr_file.npr06
    DEFINE l_sumamt4_1sub      LIKE npr_file.npr06
    DEFINE l_sumamt41          LIKE npr_file.npr06
    DEFINE l_flag          LIKE type_file.chr1
    #FUN-B80158----------add----end---

    
    ORDER EXTERNAL BY sr1.npr00,sr1.order1,sr1.order2,sr1.npr01,sr1.npr02,sr1.npr03,sr1.npr11   #FUN-B80158 add
    ...
    FORMAT
        FIRST PAGE HEADER
           ...
           IF sr1.aag04 = "1" THEN
              IF tm.c= "N" THEN
                 LET l_npr00 = sr1.npr00,' ',sr1.aag02,' ',cl_gr_getmsg("gre-119",g_lang,1)
              ELSE 
                 LET l_npr00 = sr1.npr00,' ',sr1.aag13,' ',cl_gr_getmsg("gre-119",g_lang,1)
              END IF                 
           ELSE
              IF tm.c= "N" THEN
                 LET l_npr00 = sr1.npr00,' ',sr1.aag02,' ',cl_gr_getmsg("gre-119",g_lang,2)
              ELSE 
                 LET l_npr00 = sr1.npr00,' ',sr1.aag13,' ',cl_gr_getmsg("gre-119",g_lang,2)
              END IF                 
           END IF
           PRINTX l_npr00
            #FUN-B80158----------add----end---
              
        BEFORE GROUP OF sr1.npr00
        BEFORE GROUP OF sr1.order1
           FOREACH aglg200_repcur01 USING sr1.npr00 INTO l_cnt1 END FOREACH  #FUN-B80158
 
        BEFORE GROUP OF sr1.order2
           IF l_ord1_skip = 'Y' OR l_ord1_show = 'Y' THEN
               FOREACH aglg200_repcur02 USING sr1.order1,sr1.npr00 INTO l_cnt2 END FOREACH #FUN-B80158
           ELSE
               FOREACH aglg200_repcur02 USING sr1.npr00 INTO l_cnt2 END FOREACH #FUN-B80158
           END IF
        BEFORE GROUP OF sr1.npr01
        BEFORE GROUP OF sr1.npr02
        BEFORE GROUP OF sr1.npr03
        BEFORE GROUP OF sr1.npr11

        
        ON EVERY ROW
            LET l_lineno = l_lineno + 1
            PRINTX l_lineno
            LET l_ord2_cnt = l_ord2_cnt + 1   #FUN-B80158
            LET l_ord1_cnt = l_ord1_cnt + 1   #FUN-B80158

            PRINTX sr1.*

        AFTER GROUP OF sr1.npr00
           #FUN-B80158----------add----str---
           LET l_subp_fmt = cl_gr_numfmt("npr_file","npr06",g_azi05)
           PRINTX l_subp_fmt
           LET l_subp_1_fmt = cl_gr_numfmt("npr_file","npr06",sr1.azi05)
           PRINTX l_subp_1_fmt
           LET l_sum_amt1 = GROUP SUM(sr1.amt1)
           IF l_sum_amt1 > 0 THEN
              LET l_sumamt1sub = l_sum_amt1
           ELSE
              LET l_sumamt1sub = l_sum_amt1 * (-1)
           END IF
           PRINTX l_sumamt1sub

           LET l_sumamt2_dsub = GROUP SUM(sr1.amt2_d)
           PRINTX l_sumamt2_dsub
           LET l_sumamt3_dsub = GROUP SUM(sr1.amt3_d)
           PRINTX l_sumamt3_dsub
           LET l_sumamt4sub = l_sum_amt1 + GROUP SUM(sr1.amt2_d) - GROUP SUM(sr1.amt3_d)
           PRINTX l_sumamt4sub
           IF l_sumamt4sub > 0 THEN
              LET l_sumamt4_1 = l_sumamt4sub 
           ELSE   
              LET l_sumamt4_1 = l_sumamt4sub * (-1)
           END IF
           PRINTX l_sumamt4_1 
 
           LET l_sumamt4_1sub = GROUP SUM(sr1.amt1_1) + GROUP SUM(sr1.amt2_f) - GROUP SUM(sr1.amt3_f)
           PRINTX l_sumamt4_1sub
           IF l_sumamt4_1sub > 0 THEN
              LET l_sumamt4_11 = l_sumamt4_1sub
           ELSE
              LET l_sumamt4_11 = l_sumamt4_1sub * (-1)
           END IF
           PRINTX l_sumamt4_11

           LET l_sumamt4sub =  GROUP SUM(sr1.amt1_1) + GROUP SUM(sr1.amt2_f) - GROUP SUM(sr1.amt3_f)
           PRINTX l_sumamt4sub
           IF l_sumamt4sub > 0 THEN
              LET l_sumamt41 = l_sumamt4sub
           ELSE
              LET l_sumamt41 = l_sumamt4sub * (-1)
           END IF
           PRINTX l_sumamt41
        ...
        AFTER GROUP OF sr1.order1
           ...
           LET l_amt_f_1_fmt = cl_gr_numfmt("npr_file","npr06",sr1.azi05)
           PRINTX l_amt_f_1_fmt
           LET l_npr06_1_fmt = cl_gr_numfmt("npr_file","npr06",g_azi05)
           PRINTX l_npr06_1_fmt
           LET g_ord1_desc = aglg200_replace_ord_desc(g_ord1_desc)
           PRINTX g_ord1_desc

           LET l_sum_amt1 = GROUP SUM(sr1.amt1)
           LET l_sum_amt1_1 = GROUP SUM(sr1.amt1_1)
           IF l_sum_amt1_1 > 0 THEN
              LET l_sumamt1_lorder1 = l_sum_amt1_1
           ELSE
              LET l_sumamt1_lorder1 = l_sum_amt1_1 * (-1)
           END IF
           PRINTX l_sumamt1_lorder1

           IF l_sum_amt1 > 0 THEN
              LET l_sumamt11order1 = l_sum_amt1
           ELSE
              LET l_sumamt11order1 = l_sum_amt1 * (-1)
           END IF
           PRINTX l_sumamt11order1
           ...
           LET l_sumamt4_lorder1 = l_sum_amt1_1 + GROUP SUM(sr1.amt2_f) - GROUP SUM(sr1.amt3_f)
           PRINTX l_sumamt4_lorder1
           LET l_sumamt4order1 = l_sum_amt1 + GROUP SUM(sr1.amt2_d) - GROUP SUM(sr1.amt3_d)
           PRINTX l_sumamt4order1
  
           IF l_sumamt4order1 > 0 THEN
              LET l_CorDlorder1 = 'D'
           ELSE
              LET l_CorDlorder1 = 'C'
           END IF
           PRINTX l_CorDlorder1

           IF l_sumamt4order1 > 0 THEN
              LET l_amt4order1 = l_sumamt4order1
           ELSE
              LET l_amt4order1 = l_sumamt4order1 * (-1)
           END IF
           PRINTX l_amt4order1

           IF l_sumamt4order1 > 0 THEN
              LET l_amt4_lorder1 = l_sumamt4order1
           ELSE
              LET l_amt4_lorder1 = l_sumamt4order1 * (-1)
           END IF
           PRINTX l_amt4_lorder1
           #FUN-B80158----------add----end---
...
#          LET l_ord2_cnt = l_ord2_cnt + 1
           ...
           END IF
           PRINTX l_skip_ord2 
           LET l_npr06_2_fmt = cl_gr_numfmt("npr_file","npr06",g_azi05)
           PRINTX l_npr06_2_fmt
           LET l_amt_f_2_fmt = cl_gr_numfmt("npr_file","npr06",sr1.azi05)
           PRINTX l_amt_f_2_fmt

           LET g_ord2_desc = aglg200_replace_ord_desc(g_ord2_desc)
           PRINTX g_ord2_desc
      
           LET l_sum_amt1 = GROUP SUM(sr1.amt1)
           LET l_sum_amt1_1 = GROUP SUM(sr1.amt1_1)
           IF l_sum_amt1_1 > 0 THEN
              LET l_sumamt1_lorder2 = l_sumamt1_lorder2
           ELSE
              LET l_sumamt1_lorder2 = l_sumamt1_lorder2 * (-1)
           END IF
           PRINTX l_sumamt1_lorder2

           IF l_sum_amt1 > 0 THEN
              LET l_sumamt11order2 = l_sum_amt1
           ELSE
              LET l_sumamt11order2 = l_sum_amt1 * (-1)
           END IF
           PRINTX l_sumamt11order2

           LET l_sumamt4_lorder2 = l_sum_amt1_1 + GROUP SUM(sr1.amt2_f) - GROUP SUM(sr1.amt3_f)
           PRINTX l_sumamt4_lorder2
           LET l_sumamt4order2 = l_sum_amt1 + GROUP SUM(sr1.amt2_d) - GROUP SUM(sr1.amt3_d)
           PRINTX l_sumamt4order2

           IF l_sumamt4order2 > 0 THEN
              LET l_CorDlorder2 = 'D'
           ELSE
              LET l_CorDlorder2 = 'C'
           END IF
           PRINTX l_CorDlorder2

           IF l_sumamt4order2 > 0 THEN
              LET l_amt4order2 = l_sumamt4order2
           ELSE
              LET l_amt4order2 = l_sumamt4order2 * (-1)
           END IF
           PRINTX l_amt4order2

           IF l_sumamt4order2 > 0 THEN
              LET l_amt4_lorder2 = l_sumamt4order2
           ELSE
              LET l_amt4_lorder2 = l_sumamt4order2 * (-1)
           END IF
           PRINTX l_amt4_lorder2
           #FUN-B80158----------add----end---
        AFTER GROUP OF sr1.npr01
        AFTER GROUP OF sr1.npr02
        AFTER GROUP OF sr1.npr03
        AFTER GROUP OF sr1.npr11
           #FUN-B80158----------add----str---
           LET l_npr06_11_fmt = cl_gr_numfmt("npr_file","npr06",g_azi05)
           PRINTX l_npr06_11_fmt
           LET l_npr06_11_1_fmt = cl_gr_numfmt("npr_file","npr06",sr1.azi05)
           PRINTX l_npr06_11_1_fmt
           LET l_amt_f_11_fmt = cl_gr_numfmt("npr_file","npr06",g_azi04)
           PRINTX l_amt_f_11_fmt
           LET l_amt_f1_11_fmt = cl_gr_numfmt("npr_file","npr06",sr1.azi04)
           PRINTX l_amt_f1_11_fmt
           
           LET l_sum_amt1 = GROUP SUM(sr1.amt1)
           LET l_sum_amt1_1 = GROUP SUM(sr1.amt1_1)
           IF l_sum_amt1 > 0 THEN
              LET l_sumamt1_1 = GROUP SUM(sr1.amt1_1)
           ELSE
              LET l_sumamt1_1 = GROUP SUM(sr1.amt1_1) * (-1)
           END IF 
           PRINTX l_sumamt1_1

           IF l_sum_amt1 > 0 THEN
              LET l_sumamt11 = GROUP SUM(sr1.amt1)
           ELSE
              LET l_sumamt11 = GROUP SUM(sr1.amt1) * (-1)
           END IF 
           PRINTX l_sumamt11
           ...
           LET l_sumamt4 = l_sum_amt1 + GROUP SUM(sr1.amt2_d) - GROUP SUM(sr1.amt3_d) 
           LET l_sumamt4_1 = l_sum_amt1_1 + GROUP SUM(sr1.amt2_f) - GROUP SUM(sr1.amt3_f) 
           IF l_sumamt4 > 0 THEN
              LET l_amt4 = l_sumamt4
           ELSE 
              LET l_amt4 = l_sumamt4 * (-1)
           END IF  
           PRINTX l_amt4
             
           IF l_sumamt4_1 > 0 THEN
              LET l_amt4_1 = l_sumamt4_1
           ELSE 
              LET l_amt4_1 = l_sumamt4_1 * (-1)
           END IF 
           PRINTX l_amt4_1
