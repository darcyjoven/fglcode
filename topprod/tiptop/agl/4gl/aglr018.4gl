...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglr018',g_sql) CLIPPED   # 產生Temp Table   
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL r018_tm()                      # Input print condition
      ELSE CALL r018()                         # Read data and create out-file
   END IF
...
FUNCTION r018_tm()
   ...
   OPEN WINDOW r018_w AT p_row,p_col WITH FORM "agl/42f/aglr018"
   ...
   WHILE TRUE
      ...
      INPUT BY NAME tm.axa01,tm.axa02,tm.b,tm.aaz641,tm.a,
        ...
        AFTER FIELD axa02
         ...
         AFTER FIELD aaz641
            ...
            PREPARE r018_cnt_p FROM g_sql
            DECLARE r018_cnt_c CURSOR FOR r018_cnt_p
            OPEN r018_cnt_c
            FETCH r018_cnt_c INTO l_cnt
      ...
      END INPUT
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW r018_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            LET l_cmd = l_cmd CLIPPED,          #(at time fglgo xxxx p1 p2 p3)
                   ...
                   " '",tm.q1 CLIPPED,"'",
                   " '",tm.h1 CLIPPED,"'"
            CALL cl_cmdat('aglr018',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW r018_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
...
FUNCTION r018()
   ...
   DROP TABLE aglr018_tmp
   CREATE TEMP TABLE aglr018_tmp(
      maj23    LIKE maj_file.maj23,
      maj20    LIKE type_file.chr50,
      bal      LIKE type_file.num20_6,
      bal_1    LIKE type_file.num20_6,
   ...
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      ...
      EXIT PROGRAM 
   END IF
   DECLARE r018_c CURSOR FOR r018_p

   FOR g_i = 1 TO 100 LET g_tot1[g_i] = 0 END FOR

   FOREACH r018_c INTO maj.*
      ...
      IF maj.maj03='3' OR maj.maj03='4' THEN
         IF NOT cl_null(maj.maj20) THEN
            INSERT INTO aglr018_tmp VALUES
   ...
   #抓報表結構裡帳戶左邊最大序號
   SELECT MAX(maj02) INTO l_maj02_l FROM aglr018_tmp WHERE maj23='11'
   #抓報表結構裡帳戶右邊最大序號
   SELECT MAX(maj02) INTO l_maj02_r FROM aglr018_tmp WHERE maj23='12'
   IF cl_null(l_maj02_l) THEN LET l_maj02_l = 0 END IF
   IF cl_null(l_maj02_r) THEN LET l_maj02_r = 0 END IF
   LET r_row = 0  LET l_row = 0
   DECLARE r018_c1 CURSOR FOR SELECT * FROM aglr018_tmp ORDER BY maj23,maj02,line
   FOREACH r018_c1 INTO tmp1.*
   ...
   LET g_str = g_mai02,";",tm.a,";",tm.p,";",tm.d,";",tm.yy1,";",
               ...
               ";",tm.yy2,";",tm.title1,";",tm.title2
   CALL cl_prt_cs3('aglr018','aglr018',g_sql,g_str) 
END FUNCTION

FUNCTION r018_set_entry()                                                                                                           
   CALL cl_set_comp_entry("q1,em1,h1",TRUE)
END FUNCTION                                                                                                                        
                                                                                                                                    
FUNCTION r018_set_no_entry()                                                                                                        
