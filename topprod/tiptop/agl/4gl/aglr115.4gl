...
# Modify.........: No:MOD-D10076 13/01/10 By apo 合計階數判斷式調整、line1與2寫入temp table調整
# Modify.........: No.TQC-CC0122 13/01/11 By lujh “报表编号”栏位开窗应依据报表类型查詢資料
# Modify.........: No:MOD-D10186 13/01/21 By apo 修改合計階加總的時候,借與貸會相加的問題
# Modify.........: No:MOD-D30112 13/03/12 By apo 增加判斷maj03狀況為5的情況
# Modify.........: No:MOD-D40007 13/04/02 By apo 當選擇報表為損益表類型,單一期顯示當期損益
# Modify.........: No:FUN-D40044 13/04/25 By zhangweib 增加選項：是否包含結轉憑證.若=N,則扣除CE/CA（axcq310）的金額
# Modify.........: No:TQC-D50035 13/05/06 By apo amt_o計算前需清空
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglr115',g_sql) CLIPPED   # 產生Temp Table   
   ...
   #No.FUN-570264 ---end---
   LET tm.ce   = ARG_VAL(26)   #No.FUN-D40044  Add
...
FUNCTION r115_tm()
   ...
   OPEN WINDOW r115_w AT p_row,p_col WITH FORM "agl/42f/aglr115"
...
#     INPUT BY NAME tm.rtype,tm.a,tm.b,tm.yy,tm.em,tm.e,tm.f,  #No.FUN-740020
      INPUT BY NAME tm.rtype,tm.b,tm.a,tm.yy,tm.em,tm.e,tm.f,  #No.FUN-740020 
                    tm.d,tm.acc_code,tm.c,tm.h,tm.ce,tm.o,tm.r,      #FUN-B90140 add tm.acc_code   #No.FUN-D40044   Add tm.ce
...
#              CALL cl_err(tm.p,'agl-109',0)    #No.FUN-660123
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglr115',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#   DEFINE l_dbs     LIKE azp_file.azp03          #MOD-930148 add                  #No.FUN-A10098 ----mark
   ...
   DEFINE sr        RECORD
                    ...
                    END RECORD
   DEFINE amt_o     LIKE aah_file.aah04   #MOD-D40007
   DEFINE l_flag    LIKE type_file.chr1   #是否為損益表   #MOD-D40007 
...
#No.FUN-A10098 ----mark end
   ...
   #CALL cl_outnam('aglr115') RETURNING l_name        #No.FUN-780058
   ...
   FOREACH r115_c INTO maj.*
      ...
      FOR i = 1 TO 12
         LET bal[i] = 0
      END FOR
      LET amt_o = 0   #TQC-D50035 
     #--MOD-D40007--(S)
      IF g_mai03 = '3' THEN   #性質為損益表
         LET l_flag = 'Y'
      ELSE
         LET l_flag = 'N'
      END IF
...
#MOD-D10186--mark--end
       ...
       #No.MOD-C60112  --remark End
         ...
         #IF maj.maj03 MATCHES "[012]" AND maj.maj08 > 0 THEN     #合計階數處理   #MOD-D10076 mark
         IF maj.maj03 MATCHES "[012349]" AND maj.maj08 > 0 THEN   #合計階數處理   #MOD-D10076
            FOR j=1 TO 100
            ...
            LET k = maj.maj08
           #--MOD-D40007--(S)
            IF cl_null(amt_o) THEN LET amt_o = 0 END IF   
            IF l_flag = 'Y' THEN 
               LET bal[i] = g_tot[k,i] - amt_o
               LET amt_o = g_tot[k,i]   
            ELSE
           #--MOD-D40007--(E)
            LET bal[i] = g_tot[k,i]
     ...
     #IF maj.maj06 NOT MATCHES "[13]" THEN                         #MOD-D40007 mark  #MOD-B80132 add
      IF (maj.maj06 NOT MATCHES "[13]") OR l_flag ='Y' THEN        #MOD-D40007  
    ...
    #----------------MOD-C60128--------------------------remark
     ...
     ##No.FUN-780058---Begin
      ...
      #IF maj.maj07='2' AND maj.maj09='-' THEN     #MOD-D30112 mark   #MOD-C60112                                              
       IF maj.maj07='2' AND maj.maj09='-' AND maj.maj03 MATCHES "[5]" THEN     #MOD-D30112                                              
    ...
    #----------------TQC-C70061--------------------------remark
      ...
      IF maj.maj03='3' OR maj.maj03='4' THEN
         IF NOT cl_null(maj.maj20) THEN
           EXECUTE insert_prep USING         
              maj.maj31,maj.maj20,maj.maj20e,maj.maj02,'1', 
              sr.bal01,sr.bal02,sr.bal03,sr.bal04,sr.bal05, 
              sr.bal06,sr.bal07,sr.bal08,sr.bal09,sr.bal10,
              sr.bal11,sr.bal12,sr.bal13,'2'
         END IF
         EXECUTE insert_prep USING         
            maj.maj31,maj.maj20,maj.maj20e,maj.maj02,maj.maj03, 
            sr.bal01,sr.bal02,sr.bal03,sr.bal04,sr.bal05, 
            sr.bal06,sr.bal07,sr.bal08,sr.bal09,sr.bal10,
            sr.bal11,sr.bal12,sr.bal13,'2'
      ELSE
 ...
 CALL cl_prt_cs3('aglr115',l_name,g_sql,g_str)
   #No.FUN-780058---End
END FUNCTION
