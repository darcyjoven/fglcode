...
# Modify.........: No:FUN-B80161 11/08/26 By chenying 明細類CR報表轉GR
...
TYPE sr2_t RECORD
    ...
    amt_d LIKE abb_file.abb07,
    amt_c LIKE abb_file.abb07,
    l_begin LIKE abb_file.abb07,
    s_count LIKE type_file.num5,
...
#No.FUN-760083   --begin--
  ...
  LET l_table = cl_prt_temptable("aglg159",g_sql) CLIPPED
  IF l_table=-1 THEN 
     CALL cl_used(g_prog,g_time,2) RETURNING g_time      #FUN-B80161 
  ...
  LET l_table1 = cl_prt_temptable("aglg1591",g_sql) CLIPPED                                                                          
  IF l_table1=-1 THEN 
     CALL cl_used(g_prog,g_time,2) RETURNING g_time      #FUN-B80161 
     CALL cl_gre_drop_temptable(l_table||"|"||l_table1)  #FUN-B80161     
     EXIT PROGRAM
...
FUNCTION aglg159_tm()
   ...
   OPEN WINDOW aglg159_w AT p_row,p_col
     WITH FORM "agl/42f/aglg159"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
    ...
    #FUN-B20054--add--end
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglg159',g_time,l_cmd)      # Execute cmd at later time
         END IF
...
DEFINE l_gem02         LIKE gem_file.gem02    #No.FUN-760083
 ...
 #END MOD-570354
   ...
   PREPARE aglg159_prepare1 FROM l_sql
   IF STATUS THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE aglg159_curs1 CURSOR FOR aglg159_prepare1
 
   #CALL cl_outnam('aglg159') RETURNING l_name   #No.FUN-760083
   #START REPORT aglg159_rep TO l_name           #No.FUN-760083
...
#No.FUN-760083  --end--
   ...
   FOREACH aglg159_curs1 INTO sr.*
      ...
      #OUTPUT TO REPORT aglg159_rep(sr.*)                                #No.FUN-760083
  ...
  #-MOD-A80152-end-
   ...
   PREPARE aglg159_prepare2 FROM l_sql                                        
   IF STATUS THEN                                                             
      CALL cl_err('aglg159_prepare2',STATUS,0)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1)  #FUN-B80161     
      EXIT PROGRAM
   END IF
   DECLARE aglg159_curs2 CURSOR FOR aglg159_prepare2
 
   INITIALIZE sr.* TO NULL
   LET sr.amt_d = 0
   LET sr.amt_c = 0
 
   FOREACH aglg159_curs2 INTO sr.aag01,sr.aag02,sr.aag13,sr.abb05  #FUN-6C0012
   ...
   #FINISH REPORT aglg159_rep   #No.FUN-760083
 
   #CALL cl_prt(l_name,g_prtway,g_copies,g_len)   #No.FUN-760083
#No.FUN-760083  --begin--
   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
   PREPARE aglg159_prepare3 FROM g_sql 
   IF STATUS THEN                                                                                                                   
      CALL cl_err('aglg159_prepare3',STATUS,0)                                                                                      
      CALL cl_used(g_prog,g_time,2) RETURNING g_time                                                             
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1)  #FUN-B80161     
      EXIT PROGRAM                                                                                                                  
   END IF                                                                                                                           
   DECLARE aglg159_curs3 CURSOR FOR aglg159_prepare3
 
   FOREACH aglg159_curs3 INTO sr.*
...
###GENGRE###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1  CLIPPED
   IF g_zz05 ='Y' THEN
     ...
     RETURNING  tm.wc
   END IF
###GENGRE###   LET g_str=tm.wc,';',tm.yy1,';',tm.m1,';',tm.yy2,';',tm.m2,';',tm.e,';',t_azi05
###GENGRE###   CALL cl_prt_cs3("aglg159","aglg159",g_sql,g_str)
    CALL aglg159_grdata()    ###GENGRE###
#No.FUN-760083  --end--
END FUNCTION
 
#No.FUN-760083 --begin--
{
REPORT aglg159_rep(sr)
...
FUNCTION aglg159_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg159")
        IF handler IS NOT NULL THEN
            START REPORT aglg159_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " ORDER BY aag01,abb05,aba02,aba14"
          
            DECLARE aglg159_datacur1 CURSOR FROM l_sql
            FOREACH aglg159_datacur1 INTO sr2.*
                OUTPUT TO REPORT aglg159_rep(sr2.*)
            END FOREACH
            FINISH REPORT aglg159_rep
        END IF
...
REPORT aglg159_rep(sr2)
    DEFINE sr2 sr2_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B80161---add-----str---------  
    DEFINE l_aag13_aag02 LIKE aag_file.aag02
    DEFINE l_abb05       STRING
    DEFINE l_t_amt_d     LIKE  abb_file.abb07
    DEFINE l_sum_t_amt_d LIKE  abb_file.abb07
    DEFINE l_t_amt_c     LIKE  abb_file.abb07
    DEFINE l_sum_t_amt_c LIKE  abb_file.abb07
    DEFINE l_sum         LIKE  abb_file.abb07
    DEFINE l_sum1        LIKE  abb_file.abb07
    DEFINE l_sum_l_begin LIKE  abb_file.abb07
    DEFINE l_fmt         STRING
    ...
    ORDER EXTERNAL BY sr2.aag01,sr2.abb05
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            #FUN-B80161---add---str--------
            PRINTX t_azi05  
            LET l_date = cl_gr_getmsg("gre-118",g_lang,'1'),':',tm.yy1,
                         cl_gr_getmsg("gre-118",g_lang,'3'),':',tm.m1,' ',
                         cl_gr_getmsg("gre-118",g_lang,'2'),':',tm.yy2,
                         cl_gr_getmsg("gre-118",g_lang,'3'),':',tm.m2
            PRINTX l_date   
            #FUN-B80161---add---end--------
              
        BEFORE GROUP OF sr2.aag01
        BEFORE GROUP OF sr2.abb05
            #FUN-B80161---add------str----------
            IF tm.e = 'Y' THEN 
               LET l_aag13_aag02 = sr2.aag13
            ELSE
               LET l_aag13_aag02 = sr2.aag02
            END IF 
            PRINTX l_aag13_aag02

            LET l_abb05 = cl_gr_getmsg("gre-117",g_lang,'1'),':',sr2.abb05
            PRINTX l_abb05
            #FUN-B80161---add------end----------
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B80161---add------str----------
            LET l_fmt = cl_gr_numfmt('abb_file','abb07',t_azi05)
            PRINTX l_fmt
            IF NOT cl_null(sr2.aba02) THEN LET l_display = 'Y' ELSE LET l_display = 'N' END IF
            PRINTX l_display
        ...
        AFTER GROUP OF sr2.aag01
        AFTER GROUP OF sr2.abb05
            #FUN-B80161---add------str----------
            LET l_t_amt_d = GROUP SUM(sr2.amt_d)
            IF cl_null(l_t_amt_d) THEN 
               LET l_t_amt_d = 0
            END IF
            PRINTX l_t_amt_d
            LET l_t_amt_c = GROUP SUM(sr2.amt_c)
            IF cl_null(l_t_amt_c) THEN 
               LET l_t_amt_c = 0
            END IF
            PRINTX l_t_amt_c
            LET l_sum = sr2.l_begin + l_t_amt_d - l_t_amt_c
            PRINTX l_sum
            #FUN-B80161---add------end----------
        ...
        ON LAST ROW
            #FUN-B80161---add------str----------
            LET l_sum_l_begin = SUM(sr2.l_begin)
            LET l_sum_t_amt_d = SUM(sr2.amt_d)
            LET l_sum_t_amt_c = SUM(sr2.amt_c)
            LET l_sum1 = l_sum_l_begin + l_sum_t_amt_d - l_sum_t_amt_c
            PRINTX l_sum_l_begin
            PRINTX l_sum_t_amt_d
            PRINTX l_sum_t_amt_c
            PRINTX l_sum1
            #FUN-B80161---add------end----------
