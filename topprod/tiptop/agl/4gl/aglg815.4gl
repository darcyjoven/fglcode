...
# Modify.........: FUN-C40071 12/04/25 BY wangrr CR轉GR
# Modify.........: TQC-CB0007 12/11/05 By yinhy s_minus_ce增加參數
...
DEFINE tm          RECORD
         ...
         #e        LIKE type_file.num5,            #小數位數  #FUN-C40071 Mark
          e        LIKE type_file.num10,            #小數位數  #FUN-C40071 num5-->num10
...
TYPE sr1_t RECORD
    ...
    amt1 LIKE axh_file.axh08,
    amt2 LIKE axh_file.axh08,
    azi03 LIKE azi_file.azi03,
    azi04 LIKE azi_file.azi04,
    azi05 LIKE azi_file.azi05,
    aeh31 LIKE aeh_file.aeh31,
    yy1 LIKE type_file.chr5,
    yy2 LIKE type_file.chr5,
    title1 LIKE type_file.chr50,
    title2 LIKE type_file.chr50
END RECORD
###GENGRE###END
...
MAIN       
   ...
   LET l_table = cl_prt_temptable('aglg815',g_sql) CLIPPED  # 產生Temp Table
...
FUNCTION g815_tm()
   ...
   OPEN WINDOW g815_w AT p_row,p_col WITH FORM "agl/42f/aglg815"
   ...
   WHILE TRUE
      ...
      AFTER INPUT 
        ...
        END IF
         ...
         IF tm.d = '2' THEN LET g_unit = 1000 END IF
         IF tm.d = '3' THEN LET g_unit = 1000000 END IF
         CALL g815_getmsg(tm.bm1) RETURNING tm.title1_1
         CALL g815_getmsg(tm.bm2) RETURNING tm.title2_1
         CALL g815_getmsg(tm.em1) RETURNING tm.title1  
         CALL g815_getmsg(tm.em2) RETURNING tm.title2  
...
################################################################################
      ...
      ON ACTION CONTROLP
         IF INFIELD(a) THEN
           ...
           #LET g_qryparam.where = " mai00 = '",tm.aeh00,"'"   #No.FUN-C40071 Mark
            LET g_qryparam.where = " mai00 = '",tm.aeh00,"' AND mai03 NOT IN ('5','6')"  #No.FUN-C40071 Add
   ...
   END INPUT
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW g815_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(l_table)  #FUN-C40071 Add
      EXIT PROGRAM
   END IF
      IF g_bgjob = 'Y' THEN
         ...
         ELSE 
            ...
            CALL cl_cmdat('aglg815',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW g815_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time
         CALL cl_gre_drop_temptable(l_table)  #FUN-C40071 Add
...
FUNCTION g815()
  ...
  #DEFINE l_i_cnt05 LIKE type_file.num5     #FUN-BC0004 Mark
   ...
   IF STATUS THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g815_c CURSOR FOR g815_p
...
#FUN-C40071--add--str
   ...
   LET g_sql="SELECT SUM(aeh11-aeh12)",
             ...
             "   AND aeh01 BETWEEN ? AND ? ",
             "   AND aeh09 = '",tm.yy1,"'",
             "   AND aeh10 BETWEEN '",tm.bm1,"' AND '",tm.em1,"'"
   CASE
      WHEN tm.s = '1'
         LET g_sql = g_sql CLIPPED," AND aeh31 = '",tm.aeh31,"'"
      WHEN tm.s = '2'
         LET g_sql = g_sql CLIPPED," AND aeh32 = '",tm.aeh32,"'"
      WHEN tm.s = '3'
         LET g_sql = g_sql CLIPPED," AND aeh33 = '",tm.aeh33,"'"
      WHEN tm.s = '4'
         LET g_sql = g_sql CLIPPED," AND aeh34 = '",tm.aeh34,"'"
   END CASE
...
#FUN-C40071--add--end
   ...
   FOREACH g815_c INTO maj.*
     ...
     END IF
        ...
        #    OPEN sel_aeh_cs1 
        #FUN-C40071--mark--end
            OPEN sel_aeh_cs1 USING maj.maj21,maj.maj22  #FUN-C40071 Add
            LET amt1 = 0
            ...
            CALL s_minus_ce(tm.aeh00, maj.maj21, maj.maj22, NULL,    NULL,    NULL,
                            ...
                            l_aeh34,  NULL,      NULL,      NULL,    g_plant, l_aaa09,'0')   #TQC-CB0007 
        ...
        #    OPEN sel_aeh_cs4 
        #FUN-C40071--mark--end
            OPEN sel_aeh_cs4 USING maj.maj21,maj.maj22  #FUN-C40071 Add
            LET amt2 = 0
...
###GENGRE###   LET g_str = g_mai02,";",tm.a,";",tm.e                                                         #FUN-BC0004   Add
 
###GENGRE###   CALL cl_prt_cs3('aglg815','aglg815',g_sql,g_str)
    CALL aglg815_grdata()    ###GENGRE###
     
END FUNCTION

FUNCTION g815_getmsg(p_tmp)
...
FUNCTION aglg815_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg815")
        IF handler IS NOT NULL THEN
            START REPORT aglg815_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY maj02"   #FUN-C40071 Add

            DECLARE aglg815_datacur1 CURSOR FROM l_sql
            FOREACH aglg815_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg815_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg815_rep
        END IF
 ...
 #FUN-C40071--add--end    
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
        #FUN-C40071--add--str
            LET l_amt1_fmt = cl_gr_numfmt('aeh_file','aeh11',tm.e)
            LET l_amt2_fmt = cl_gr_numfmt('aeh_file','aeh11',tm.e)
            PRINTX l_amt1_fmt
            PRINTX l_amt2_fmt
        #FUN-C40071--add--end    
            PRINTX sr1.*
