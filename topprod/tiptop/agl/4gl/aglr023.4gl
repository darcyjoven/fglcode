...
#
# Pattern name...: aglr023.4gl
# Descriptions...: 合併後差異調節表(IFRS) FUN-B60143
# Modify.........: No.MOD-C40219 12/04/30 By Elise aglr023抓取tm.em變數應參考aglr002中tm.em的抓法
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglr023',g_sql) CLIPPED   # 產生Temp Table   
...
DEFINE l_aaa05           LIKE aaa_file.aaa05     #MOD-C40219
DEFINE l_axz03           LIKE axz_file.axz03     #MOD-C40219
 
   CALL s_dsmark(g_bookno)
 
   LET p_row = 2 LET p_col = 20
 
   OPEN WINDOW aglr023 AT p_row,p_col
     WITH FORM "agl/42f/aglr023"  ATTRIBUTE (STYLE = g_win_style CLIPPED)
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                        ...
                        " '",g_template CLIPPED,"'",
                        " '",g_rpt_name CLIPPED,"'"
            CALL cl_cmdat('aglr023',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#------------------------------ CR (2) ------------------------------#
   ...
   PREPARE r023_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      EXIT PROGRAM 
   END IF
   DECLARE r023_c CURSOR FOR r023_p
   FOR g_i = 1 TO 100 
      LET g_tot1[g_i] = 0
      LET g_tot2[g_i] = 0
      LET g_tot3[g_i] = 0
   ...
   FOREACH r023_c INTO maj.*
      ...
      IF NOT cl_null(maj.maj21) THEN
         ...
         LET l_sql = "SELECT SUM(axh08-axh09) ",
                     ...
                     "   AND axh12 = '",x_aaa03,"'"
         CALL cl_replace_sqldb(l_sql) RETURNING l_sql
         PREPARE r023_sum_p FROM l_sql
         DECLARE r023_sum_c CURSOR FOR r023_sum_p
         OPEN r023_sum_c
         FETCH r023_sum_c INTO amt1
         IF SQLCA.sqlcode THEN
      ...
      IF NOT cl_null(maj.maj34) THEN
         ...
         LET l_sql = "SELECT SUM(axh08-axh09) ",
                     ...
                     "   AND axh12 = '",l_aaa03,"'"
         CALL cl_replace_sqldb(l_sql) RETURNING l_sql
         PREPARE r023_sum_p1 FROM l_sql
         DECLARE r023_sum_c1 CURSOR FOR r023_sum_p1
         OPEN r023_sum_c1
         FETCH r023_sum_c1 INTO amt2
         IF SQLCA.sqlcode THEN
            CALL cl_err3("sel","axh_file,aag_file",tm.yy,"",STATUS,"","sel axh:",1)
            EXIT FOREACH 
         END IF
...
#     END IF
   ...
   LET g_str = g_mai02,";",tm.a,";",tm.p,";",tm.d,";",tm.yy,";",tm.bm,";",
             #    p7         p8           p9        p10       p11
               tm.e,";",l_lastday,";",tm.rtype,";",tm.em,";",l_e
 
   CALL cl_prt_cs3('aglr023','aglr023',l_sql,g_str)
   #------------------------------ CR (4) ------------------------------#
   
END FUNCTION

FUNCTION r023_set_entry()
  CALL cl_set_comp_entry("q1,em,h1",TRUE)
END FUNCTION
 
FUNCTION r023_set_no_entry()
   IF tm.axa06 ="1" THEN  
