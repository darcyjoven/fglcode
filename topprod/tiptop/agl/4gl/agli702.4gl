...
#
# Pattern name...: agli702.4gl
# Descriptions...: 專案預算維護作業
# Date & Author..: 01/09/21 BY Wiky
# Modify.........: No.MOD-470041 04/07/19 By Nicola 修改INSERT INTO 語法
# Modify.........: No.MOD-480429 04/08/19 By Nicola 調整功能按"取消"會出現-201
# Modify.........: No.MOD-470515 04/10/05 By Nicola 加入"相關文件"功能
# Modify.........: No.MOD-4B0089 04/11/12 By Nicola 專案編號改call q_gja
...
# Modify.........: No.FUN-680092 06/08/28 By yjkhero  欄位類型轉換為 LIKE型
...
# Modify.........: No.TQC-750022 07/05/09 By Lynn 打印時,FROM位置在報表名之上
# Modify.........: No.FUN-750051 07/05/22 By johnray 連續二次查詢key值時,若第二次查詢不到key值時,會顯示錯誤key值
# Modify.........: No.TQC-760105 07/06/18 By hongmei 處理出現-400 error,列印出現-284 error(子查詢...不止一列)
# Modify.........: No.TQC-7C0006 07/12/04 By dxfwo   agli702(科目異動碼預算)打印異常問題
#                                                    第二次打印直接跳出問題 
# Modify.........: No.FUN-7C0050 08/01/15 By johnray 串查程序代碼添加共用 ACTION 的引用
# Modify.........: No.FUN-810045 08/03/31 By rainy 專案管理,專案table gja_file->pja_file
# Modify.........: No.FUN-830095 08/04/21 By lutingting 報表改為使用Crystal Report 輸出
# Modify.........: No.TQC-860018 08/06/09 By Smapmin 增加on idle控管
# Modify.........: No.FUN-980030 09/08/31 By Hiko 加上GP5.2的相關設定
# Modify.........: No.FUN-960038 09/07/29 By chenmoyan 增加"專案未結案"的判斷
...
#   g_wc,g_sql          VARCHAR(300)    #No.FUN-680098
...
DEFINE   mi_no_ask       LIKE type_file.num5           #No.FUN-680098 INTEGER
...
#No.FUN-830095--------start--
    LET l_sql="afb00.afb_file.afb00,",
              "afb01.afb_file.afb01,", 
              "l_afa02.afa_file.afa02,",
              ...
              "l_x.type_file.chr1000"
    LET l_table = cl_prt_temptable('agli702',l_sql) CLIPPED
...
#No.FUN-830095---------end
    ...
    OPEN WINDOW i702_w AT p_row,p_col
         WITH FORM "agl/42f/agli702"
...
#       afb01   ,afb02  ,afb03  ,afb04,  afb07   , afb08  , afb09  ,
        afb00   ,afb01   ,afb02  ,afb03  ,afb04,  afb07   , afb09  ,  #No.FUN-730070
        afb05   ,afb06  , afb10 ,
...
#         CALL q_aag(0,0,g_afb.afb02,' ','24',' ') RETURNING g_afb.afb02
          ...
          CALL i702_afb02('d')
       END IF
       IF INFIELD(afb04) THEN #專案代號
#         CALL q_pja(0,0,g_afb.afb04) RETURNING g_afb.afb04
          CALL cl_init_qry_var()
          LET g_qryparam.state = "c"
          #LET g_qryparam.form ="q_gja"    #No.MOD-4B0089  #FUN-810045
    ...
    LET g_sql="SELECT afb00,afb01,afb02,afb03,afb04 FROM afb_file ", # 組合出 SQL 指令
              " WHERE ", g_wc CLIPPED, " AND afb15 = '4'",
    ...
    LET g_sql=
        ...
        g_wc CLIPPED," AND afb15 = '4'"  #No.FUN-730070
       ,"   AND afbacti = 'Y' "                        #FUN-D70090 add 
...
#           l_sql     VARCHAR(200)
    ...
    IF p_cmd = 'u' THEN
       DELETE FROM afc_file WHERE afc00 = g_afb.afb00 AND afc01 = g_afb.afb01  #No.FUN-730070
                              AND afc02 = g_afb.afb02 AND afc03 = g_afb.afb03
                              AND afc04 = g_afb.afb04
        IF SQLCA.sqlcode THEN  
#           CALL cl_err('delete:',SQLCA.sqlcode,0)  #No.FUN-660123
#           CALL cl_err3("del","afc_file",g_afb.afb01,g_afb.afb02,SQLCA.sqlcode,"","delete:",1)  #No.FUN-660123  #TQC-760105
    ...
    FOR l_n = 1 TO g_cnt
        ...
        PUT i702_c3 FROM l_n,l_afc06[l_n],l_afc07[l_n]
        IF SQLCA.sqlcode THEN
    ...
    INPUT BY NAME g_afb.afboriu,g_afb.afborig,
        ...
        AFTER FIELD afb04
            IF g_afb.afb04 IS NOT NULL THEN
              #FUN-810045 begin
               #SELECT COUNT(*) INTO l_n FROM gja_file  #是否存在專案名稱檔
               # WHERE gja01=g_afb.afb04
               SELECT COUNT(*) INTO l_n FROM pja_file  #是否存在專案名稱檔
                WHERE pja01=g_afb.afb04
                  AND pjaacti = 'Y'
                  AND pjaclose = 'N'                   #No.FUN-960038
              #FUN-810045 end
               IF l_n =0 THEN
                   LET g_msg = g_afb.afb04 CLIPPED
                   CALL cl_err(g_msg,'agl-007',0)
                   NEXT FIELD afb04
               END IF
                  ...
                  THEN
                   ...
                   IF l_n > 0 THEN                  # Duplicated
                       ...
                       DISPLAY BY NAME g_afb.afb01,g_afb.afb02,
                                       g_afb.afb03,g_afb.afb04,g_afb.afb00
                       NEXT FIELD afb00
                   END IF
                  #No.FUN-730070  --End   
               END IF
...
DEFINE   p_cmd     LIKE type_file.chr1          #No.FUN-680098   VARCHAR(1)
     ...
     IF p_cmd = 'a'  AND (NOT g_before_input_done) THEN
         CALL cl_set_comp_entry("afb00,afb01,afb02,afb03,afb04",TRUE)  #No.FUN-730070
     END IF
...
FUNCTION i702_set_no_entry(p_cmd)
    ...
    IF p_cmd = 'u' AND g_chkey = 'N' AND (NOT g_before_input_done) THEN
       CALL cl_set_comp_entry("afb00,afb01,afb02,afb03,afb04",FALSE)  #No.FUN-730070
    END IF
...
#       l_sql            VARCHAR(200),
    ...
    CASE p_flafb
        WHEN 'N' FETCH NEXT     i702_cs INTO g_afb.afb00,g_afb.afb01,  #No.FUN-730070
                                          g_afb.afb02,g_afb.afb03,g_afb.afb04
        WHEN 'P' FETCH PREVIOUS i702_cs INTO g_afb.afb00,g_afb.afb01,  #No.FUN-730070
                                          g_afb.afb02,g_afb.afb03,g_afb.afb04
        WHEN 'F' FETCH FIRST    i702_cs INTO g_afb.afb00,g_afb.afb01,  #No.FUN-730070
                                          g_afb.afb02,g_afb.afb03,g_afb.afb04
        WHEN 'L' FETCH LAST     i702_cs INTO g_afb.afb00,g_afb.afb01,  #No.FUN-730070
                                          g_afb.afb02,g_afb.afb03,g_afb.afb04
        WHEN '/'
...
#                      CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
                ...
                END IF
            END IF
            FETCH ABSOLUTE  g_jump i702_cs INTO g_afb.afb00,g_afb.afb01,  #No.FUN-730070
                                                 g_afb.afb02,g_afb.afb03,g_afb.afb04
            LET mi_no_ask = FALSE
    END CASE
...
#      CALL cl_err(g_afb.afb01,SQLCA.sqlcode,0)   #No.FUN-660123
    ...
    ELSE
       ...
       LET l_sql = "SELECT afc06,afc07 FROM afc_file",
                   ...
                   " AND afc04 ='",g_afb.afb04,"'",
                   " AND afc05 = ?"
       PREPARE i702_p2 FROM l_sql
       DECLARE i702_c2 CURSOR FOR i702_p2
       FOR g_cnt = 1 TO 13
           OPEN i702_c2 USING g_cnt
           FETCH i702_c2 INTO l_afc06[g_cnt],l_afc07[g_cnt]
...
#       g_afb.afb07   ,g_afb.afb08   ,g_afb.afb09   ,
    ...
    CALL i702_afb00('d')  #No.FUN-730070
    CALL i702_afb01('d')
    CALL i702_afb02('d')
    CALL cl_show_fld_cont()                   #No.FUN-550037 hmf
END FUNCTION
...
FUNCTION i702_u()
    ...
    OPEN i702_cl USING g_afb.afb00,g_afb.afb01,g_afb.afb02,g_afb.afb03,g_afb.afb04
    IF STATUS THEN
       ...
       RETURN
    END IF
    FETCH i702_cl INTO g_afb.*               # 對DB鎖定
    IF SQLCA.sqlcode THEN
        CALL cl_err(g_afb.afb01,SQLCA.sqlcode,0)
        RETURN
    END IF
...
#No.FUN-730070  --End  
    ...
    LET g_before_input_done=FALSE
    CALL i702_set_entry('a')
    LET g_before_input_done = TRUE
 
    LET o_afb.* = g_afb.*  #No.FUN-730070
    INPUT t_afb.afb00,t_afb.afb01,t_afb.afb02,t_afb.afb03,t_afb.afb04  #No.FUN-730070
     FROM afb00,afb01,afb02,afb03,afb04  #No.FUN-730070
        ...
        AFTER FIELD afb02
         ...
         ON ACTION controlp
            ...
            #No.FUN-730070  --End   
            IF INFIELD(afb04) THEN #專案代號
#              CALL q_pja(5,7,t_afb.afb04) RETURNING t_afb.afb04
               CALL cl_init_qry_var()
               #LET g_qryparam.form ="q_gja"    #No.MOD-4B0089  #FUN-810045
               LET g_qryparam.form ="q_pja2"    #FUN-810045 
    ...
    IF INT_FLAG THEN
        ...
        DISPLAY BY NAME g_afb.afb01,g_afb.afb02,g_afb.afb00,g_afb.afb03,g_afb.afb04  #No.FUN-730070
        RETURN
    END IF
    LET g_afb.* = o_afb.*   #No.FUN-730070
    LET l_afb.* = g_afb.*
...
#No.FUN-830095-----start--
    ...
    DEFINE l_gx228   LIKE zaa_file.zaa08
#No.FUN-830095-----end    
    ...
    IF g_wc IS NULL THEN
     #  CALL cl_err('',-400,0)
        ...
        RETURN
    END IF
    #CALL cl_wait()                                #No.FUN-830095
    #CALL cl_outnam('agli702') RETURNING l_name    #No.FUN-830095
    CALL cl_del_data(l_table)                           #No.FUN-830095
    SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'agli702'   #No.FUN-830095       
	#金額小數位數
    #No.FUN-730070  --Begin
    ...
    #START REPORT i702_rep TO l_name  #No.FUN-830095
 
    FOREACH i702_co INTO l_afb.*
        IF SQLCA.sqlcode THEN
            CALL cl_err('Foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
            END IF
        #OUTPUT TO REPORT i702_rep(l_afb.*)   #No.FUN-830095
         ...
         IF SQLCA.sqlcode THEN 
            ...
            CALL cl_getmsg('agl-228',g_lang) RETURNING l_gx228
            LET l_x = l_gx228 CLIPPED,' ',l_afb.afb00,'    '
         END IF
         ...
         PREPARE i702_p4 FROM l_sql 
         IF SQLCA.sqlcode                                   
            THEN CALL cl_err('p4:',SQLCA.sqlcode,1)       
            CALL cl_used(g_prog,g_time,2) RETURNING g_time
            EXIT PROGRAM                             
         END IF                                        
         DECLARE i702_c4 CURSOR FOR i702_p4            
         IF g_aza.aza02 = '1' THEN                     
            LET g_i = 12                               
         ELSE                                          
            LET g_i = 13                               
         ...
         LET g_cnt1 = ''                                 
         FOR g_cnt = 1 TO g_i                            
              OPEN i702_c4 USING g_cnt                  
              FETCH i702_c4 INTO l_afc06[g_cnt]         
                 IF SQLCA.sqlcode THEN                  
         ...
         EXECUTE insert_prep USING
                 l_afb.afb00,l_afb.afb01,l_afa02,l_afb.afbacti,l_afb.afb02,
                 l_afb.afb03,l_afb.afb05,l_afb.afb04,l_aag02, l_afc06[1],
                 l_afc06[2],l_afc06[3],l_afc06[4],l_afc06[5],l_afc06[6],
                 l_afc06[7],  l_afc06[8],l_afc06[9],l_afc06[10],l_afc06[11],
                 l_afc06[12],l_afc06[13],l_afb.afb11,
                 l_afb.afb12,l_afb.afb13,l_afb.afb14,
                 l_afb.afb10,t_azi04,t_azi05,l_x                           
         #No.FUN-830095--------------------end
                 
#No.TQC-760105----------------Begin 由于在rep函數里當有一筆資料取不到值的時候，
#                              直接返回到這里，在FINISH REPORT 之前跳出
    IF g_cnt1 = '0' THEN                                                                                                        
          EXIT FOREACH                                                                                                              
        END IF                                                                                                                      
#         RETURN               #No.TQC-7C0006                                                                                                     
    END FOREACH                                                                                                                     
#       IF g_cnt1 ='0' THEN                                                                                                         
#         ERROR  ""                                                                                                                 
#         RETURN                                                                                                                    
#       END IF
#No.TQC-760105---------------End
    #FINISH REPORT i702_rep       #No.FUN-830095        
...
#No.FUN-830095---------------start--
    ...
    IF g_zz05='Y' THEN
       CALL cl_wcchp(g_wc,'afb00,afb01,afb02,afb03,afb04,afb07, afb09,  
                           afb05,afb06,afb10,afc06_1,afc06_2,afc06_3,
                           afc06_4,afc06_5,afc06_6,afc06_7,afc06_8,
                           afc06_9,afc06_10,afc06_11,afc06_12,afc06_13,
                           afb11,afb12,afb13,afb14,afbuser ,afbgrup,
                           afbmodu,afbdate,afbacti')
           RETURNING l_str
           LET g_str = l_str
     END IF
     ...
     CALL cl_prt_cs3('agli702','agli702',l_sql,g_str)                       
#No.FUN-830095---------------end    
END FUNCTION
...
#                COLUMN (g_len-FGL_WIDTH(g_user)-15),'FROM:',g_user CLIPPED, COLUMN g_len-8,g_x[3] CLIPPED,PAGENO USING '<<<'    # No.TQC-750022
#            SELECT afa02 INTO l_afa02 FROM afa_file
#                   WHERE afa01 = sr.afb01 AND afa00 = sr.afb00  #No.FUN-730070
#            IF SQLCA.sqlcode THEN 
##              CALL cl_err('1:',SQLCA.sqlcode,1) # NO.FUN-660123
#               CALL cl_err3("sel","afa_file",sr.afb00,sr.afb01,SQLCA.sqlcode,"","1:",1)  #NO.FUN-660123  #No.FUN-730070
#            PRINT g_x[21] CLIPPED,' ',sr.afb00,'    ';  #No.FUN-730070
#            END IF
...
#            PRINT g_x[11],'        ',g_x[12] CLIPPED,' ',g_x[20] CLIPPED
#            PRINT '  ------------------------  ------------',
##FUN-590124
#                  '-------------------------------------- ----  --  ---------------'
##                 '------------------ ----  --  ------------'
##           SELECT aag02 INTO l_aag02 FROM aag_file WHERE aag01 = sr.afb02        #No.TQC-760105
...
#                    " AND afc01 ='",sr.afb01,"'",
#                    " AND afc02 ='",sr.afb02,"'",
#            #       " AND afc03 ='",sr.afb03,"'",    #No.TQC-760105
#                    " AND afc03 =",sr.afb03,         #No.TQC-760105
#                    " AND afc04 ='",sr.afb04,"'",
#                    " AND afc05 = ?"
#
...
#            FETCH i702_c4 INTO l_afc06[g_cnt]
#               IF SQLCA.sqlcode THEN
##                 CALL cl_err('3:',SQLCA.sqlcode,1) #No.TQC-7C0006
#                  CALL cl_err('3:',SQLCA.sqlcode,0) #No.TQC-7C0006
##No.TQC-760105----------------Begin 出現無資料還會打印的錯誤,
##                                   直接讓出現錯誤的時候跳出rep函數
##                 LET g_cnt1 = '0'                  #No.TQC-7C0006                                                                                 
#                  EXIT FOR
#               END IF
#        END FOR
##       IF g_cnt1 = '0' THEN                                                                                                        
...
FUNCTION i702_m()
DEFINE  l_percent  LIKE type_file.num15_3,          #LIKE cpf_file.cpf112,            #No.FUN-680098   DECIMAL(6,2)   #TQC-B90211
        l_percent1 LIKE type_file.num5,             #No.FUN-680098   SMALLLINT
#       l_sql      VARCHAR(200)
        l_sql      STRING        #TQC-630166     
    ...
    END INPUT
    CLOSE WINDOW i702_w2
     IF INT_FLAG THEN       #No.MOD-480429
       RETURN
    END IF
...
{ckp#1}UPDATE afb_file SET afb10 = afb10 * l_percent,
           ...
           WHERE afb00 = g_afb.afb00    AND afb01 = g_afb.afb01 AND  #No.FUN-730070
                 ...
                 afb04 = g_afb.afb04 AND afbacti = 'Y'               #FUN-D70090 add  afbacti
              
    IF SQLCA.sqlcode THEN
       LET g_success = 'N'
#      CALL cl_err('ckp#1',SQLCA.sqlcode,1)  # NO.FUN-660123 
       CALL cl_err3("upd","afb_file",g_afb.afb01,g_afb.afb02,SQLCA.sqlcode,"","ckp#1",1) # NO.FUN-660123
    END IF
    SELECT afb_file.* INTO g_afb.* FROM afb_file
                   WHERE afb00 = g_afb.afb00 AND afb01 = g_afb.afb01  #No.FUN-730070
                         AND afb02 = g_afb.afb02 AND afb03 = g_afb.afb03
                         AND afb04 = g_afb.afb04 AND afbacti = 'Y'                        #FUN-D70090 add afbacti
   FOR g_cnt = 1 TO 13
      OPEN i702_c2 USING g_cnt
      FETCH i702_c2 INTO l_afc06[g_cnt]
      IF SQLCA.sqlcode != 100 AND SQLCA.sqlcode != 0 THEN
         CALL cl_err('fetch:',SQLCA.sqlcode,0)   
      END IF
   END FOR
