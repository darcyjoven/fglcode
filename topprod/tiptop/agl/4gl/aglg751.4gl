...
# Modify.........: No.FUN-B80161 11/09/15 By chenying 明細類CR轉GR
# Modify.........: No.FUN-B80161 12/01/05 By qirl FUN-B80057,FUN-BB0047追单
...
TYPE sr2_t RECORD
    abb03 LIKE abb_file.abb03,
    f_dep_d LIKE aao_file.aao05,
    f_dep_c LIKE aao_file.aao05,
    f_dep_bal LIKE aao_file.aao05,
    l_dep_d LIKE aao_file.aao05,
    l_dep_c LIKE aao_file.aao05,
    l_dep_bal LIKE aao_file.aao05,
    t_d LIKE aao_file.aao05,
    t_c LIKE aao_file.aao05,
    ...
    aag13 LIKE aag_file.aag13,
    abb06 LIKE abb_file.abb06,
    dep_d LIKE abb_file.abb07,
    dep_c LIKE abb_file.abb07,
    dep_bal LIKE abb_file.abb07,
    aba02 LIKE aba_file.aba02,
    abb05 LIKE abb_file.abb05,
...
TYPE sr3_t RECORD
    ...
    l_gem03 LIKE gem_file.gem03,
    l_pred LIKE aao_file.aao05,
    l_prec LIKE aao_file.aao05,
    l_prebal LIKE aao_file.aao05
END RECORD
...
MAIN
   ...
   IF (NOT cl_setup("AGL")) THEN
      EXIT PROGRAM
   END IF
  # CALL cl_used(g_prog,g_time,1) RETURNING g_time #No.FUN-690114#FUN-B80161
...
#NO.FUN-810091 -- begin --
   ...
   LET l_table = cl_prt_temptable('aglg751',g_sql) CLIPPED
   ...
   LET g_sql = "abb03.abb_file.abb03,",
               ...
               "abb07.abb_file.abb07"
   LET l_table1 = cl_prt_temptable('aglg7511',g_sql) CLIPPED
   ...
   LET l_table2 = cl_prt_temptable('aglg7512',g_sql) CLIPPED
   ...
   LET l_table3 = cl_prt_temptable('aglg7513',g_sql) CLIPPED
   IF l_table3 = -1 THEN EXIT PROGRAM END IF
#NO.FUN-810091 -- end --
   CALL cl_used(g_prog,g_time,1) RETURNING g_time  #FUN-B80161 
...
#No.FUN-740020  ---end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'            # If background job sw is off
      THEN CALL aglg751_tm()                    # Input print condition
      ELSE CALL aglg751()                       # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
   CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)               #FUN-B80161 add
END MAIN
...
DEFINE li_chk_bookno  LIKE type_file.num5     #FUN-B20054 
   ...
   END IF
   OPEN WINDOW aglg751_w AT p_row,p_col
        WITH FORM "agl/42f/aglg751"
################################################################################
...
# 
#    IF INT_FLAG THEN
#        LET INT_FLAG = 0 CLOSE WINDOW aglg751_w 
#        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
#        EXIT PROGRAM
#           
#    END IF
    ...
    IF g_bgjob = 'Y' THEN
       ...
       ELSE
         LET tm.wc=cl_replace_str(tm.wc, "'", "\"")
          ...
          CALL cl_cmdat('aglg751',g_time,l_cmd) # Execute cmd at later time
       END IF
       CLOSE WINDOW aglg751_w
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
       CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)               #FUN-B80161 add
       EXIT PROGRAM
    END IF
...
#NO.FUN-810091 -- begin --
   ...
   IF STATUS THEN
      ...
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)               #FUN-B80161 add
      EXIT PROGRAM
   END IF
...
#NO.FUN-810091 --end--
     ...
     PREPARE g751_preaao FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('pre g751_preaao:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
        CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)               #FUN-B80161 add
        EXIT PROGRAM
     END IF
     DECLARE g751_cursaao CURSOR FOR g751_preaao
     ...
     PREPARE g751_preabb FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('g751_preabb:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
        CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)               #FUN-B80161 add
        EXIT PROGRAM
     END IF
     DECLARE g751_cursabb CURSOR FOR g751_preabb
    ...
    #-->取要列印的科目
     ...
     PREPARE aglg751_prepare2 FROM l_sql1
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE aglg751_curs2 CURSOR FOR aglg751_prepare2
 
#    CALL cl_outnam('aglg751') RETURNING l_name          #NO.FUN-810091 Mark
#NO.FUN-810091 -- begin --
#     START REPORT aglg751_rep TO l_name
#     LET g_pageno = 0
#NO.FUN-810091 -- end --
     #-->取科目
     FOREACH aglg751_curs2 INTO sr1.*
        IF SQLCA.sqlcode != 0 THEN
        ...
        #-->取當期傳票資料
        LET l_cnt = 0
        FOREACH g751_cursabb USING sr1.aag01 INTO sr.*
           IF SQLCA.sqlcode != 0 THEN
           ...
           END IF
#NO.FUN-810091 -- begin --
#        OUTPUT TO REPORT aglg751_rep(sr.*)
...
#NO.FUN-810091 -- end --
     ...
     END FOREACH
#NO.FUN-810091 -- begin --
#   FINISH REPORT aglg751_rep
#   CALL cl_prt(l_name,g_prtway,g_copies,g_len)
   LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED," ORDER BY abb03,order1,order2,abb01,abb02"
   PREPARE g751_prep1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('g751_prep1:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)               #FUN-B80161 add
      EXIT PROGRAM
   END IF
   DECLARE g751_curs1 CURSOR FOR g751_prep1
   LET l_sql = "SELECT SUM(abb07) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " WHERE abb03=? AND abb05=? AND abb06='1'"
   PREPARE g751_prep_l_dep_d FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('g751_prep_l_dep_d:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)               #FUN-B80161 add
      EXIT PROGRAM
   END IF
   DECLARE g751_curs_l_dep_d CURSOR FOR g751_prep_l_dep_d
   LET l_sql = "SELECT SUM(abb07) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " WHERE abb03=? AND abb05=? AND abb06='2'"
   PREPARE g751_prep_l_dep_c FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('g751_prep_l_dep_c:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)               #FUN-B80161 add
      EXIT PROGRAM
   END IF
   DECLARE g751_curs_l_dep_c CURSOR FOR g751_prep_l_dep_c
   LET l_first = 1
   LET l_abb03 = NULL
   LET l_order1= NULL
   LET l_chg_abb03=0
   LET l_chg_order1=0
   FOREACH g751_curs1 INTO sr.abb03,sr.order1,sr.order2,sr.abb01,sr.abb02,
                           sr.aag02,sr.aag13,sr.abb11,sr.abb12,sr.abb13,
                           sr.abb14,sr.abb31,sr.abb32,sr.abb33,sr.abb34,
                           sr.abb35,sr.abb36,sr.abb06,sr.aba02,sr.abb05,
                           sr.abb07,sr.abb04
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach g751_curs1:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
      ...
      IF l_chg_abb03=1 THEN              # GROUP abb03 判斷
         ...
         FOREACH g751_cursaao USING sr.abb03 INTO l_aao02,l_pred,l_prec,l_prebal
            IF SQLCA.sqlcode != 0 THEN
               CALL cl_err('foreach g751_cursaao:',SQLCA.sqlcode,1)
               EXIT FOREACH
            END IF
      ...
      IF l_chg_order1 = 1 THEN   # GROUP order1 判斷
         ...
         LET l_dep_d = 0
         LET l_dep_c = 0
         OPEN g751_curs_l_dep_d USING sr.abb03,sr.abb05
         FETCH g751_curs_l_dep_d INTO l_dep_d
         IF l_dep_d IS NULL THEN LET l_dep_d = 0 END IF
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('fetch g751_curs_l_dep_d:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
         CLOSE g751_curs_l_dep_d
         OPEN g751_curs_l_dep_c USING sr.abb03,sr.abb05
         FETCH g751_curs_l_dep_c INTO l_dep_c
         IF l_dep_c IS NULL THEN LET l_dep_c = 0 END IF
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('fetch g751_curs_l_dep_c:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
         CLOSE g751_curs_l_dep_c
   ...
   END FOREACH
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aglg751'
   IF g_zz05 = 'Y' THEN 
...
###GENGRE###   LET g_str = tm.wc CLIPPED,";",tm.bdate,";",tm.edate,";",tm.babb05,";",
###GENGRE###               tm.eabb05,";",tm.e,";",tm.s,";",t_azi04,";",t_azi05
...
###GENGRE###   CALL cl_prt_cs3('aglg751','aglg751',g_sql,g_str)
    CALL aglg751_grdata()    ###GENGRE###
#NO.FUN-810091 -- end --
END FUNCTION
 
#NO.FUN-810091 -- begin --
#REPORT aglg751_rep(sr)
#   DEFINE
#          sr               RECORD
#                    order1 LIKE abb_file.abb05,  #No.FUN-680098 VARCHAR(20)
#                    order2 LIKE aba_file.aba02,  #No.FUN-680098 VARCHAR(20)
...
FUNCTION aglg751_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg751")
        IF handler IS NOT NULL THEN
            START REPORT aglg751_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED
                  #     ," ORDER BY order2,abb01,abb02"   #FUN-B80161 add
          
            DECLARE aglg751_datacur1 CURSOR FROM l_sql
            FOREACH aglg751_datacur1 INTO sr2.*
                OUTPUT TO REPORT aglg751_rep(sr2.*)
            END FOREACH
            FINISH REPORT aglg751_rep
        END IF
...
REPORT aglg751_rep(sr2)
    ...
    #FUN-B80161---add----str--------- 
    DEFINE l_account_name  LIKE aag_file.aag02
    DEFINE l_balance       LIKE abb_file.abb07
    DEFINE l_sql           STRING
    DEFINE l_abb07_fmt     STRING
    DEFINE l_fmt           STRING
    #FUN-B80161---add----end--------- 
    DEFINE l_abb07_1          LIKE abb_file.abb07   #FUN-CB0058
    DEFINE l_abb07_2          LIKE abb_file.abb07   #FUN-CB0058

    
    ORDER EXTERNAL BY sr2.abb03,sr2.order1
    ...
    FORMAT
	    ...
	    #FUN-B80161---add---str-----------
#           IF tm.e = 'N' THEN 
#              LET l_account_name = sr2.aag02 
#           ELSE 
#              LET l_account_name = sr2.aag13 
#           END IF
#           PRINTX l_account_name
            #FUN-B80161---add---end-----------

              
        BEFORE GROUP OF sr2.abb03
            #FUN-B80161---add---str-----------
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,
                        " WHERE aao01 = '",sr2.abb03 CLIPPED,"'"
            START REPORT aglg751_subrep01
            DECLARE aglg751_subrep01 CURSOR FROM l_sql
            FOREACH aglg751_subrep01 INTO sr3.*
                OUTPUT TO REPORT aglg751_subrep01 (sr3.*)
            END FOREACH 
            FINISH REPORT aglg751_subrep01
            #FUN-B80161---add---end-----------
	    ...
	    #FUN-B80161---add---str-----------
            ...
            END IF
            PRINTX l_account_name
            LET l_abb07_fmt = cl_gr_numfmt("abb_file","abb07",t_azi05)
            PRINTX l_abb07_fmt
            LET l_fmt = cl_gr_numfmt("abb_file","abb07",t_azi04)
            PRINTX l_fmt

            IF sr2.abb06 = '1' THEN LET l_balance = sr2.abb07 ELSE LET l_balance = sr2.abb07 * (-1) END IF
            PRINTX l_balance

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        " WHERE abc01 = '",sr2.abb01 CLIPPED,"'",
                        " AND abc02 = ",sr2.abb02 CLIPPED
            START REPORT aglg751_subrep02
            DECLARE aglg751_subrep02 CURSOR FROM l_sql
            FOREACH aglg751_subrep02 INTO sr4.*
                OUTPUT TO REPORT aglg751_subrep02(sr4.*)
            END FOREACH
            FINISH REPORT aglg751_subrep02
            #FUN-B80161---add---end-----------

            #FUN-CB0058---add---str--
            IF sr1.abb06 != 1 THEN
               LET l_abb07_1 = sr2.abb07
               LET l_abb07_2 = NULL     
            ELSE 
               LET l_abb07_2 = sr2.abb07
               LET l_abb07_1 = NULL     
            END IF 
            PRINTX l_abb07_1
            PRINTX l_abb07_2
