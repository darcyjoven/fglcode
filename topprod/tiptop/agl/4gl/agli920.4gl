...
DEFINE 
    g_gim           DYNAMIC ARRAY OF RECORD   #程式變數(Program Variables)
...
#     DEFINEl_time LIKE type_file.chr8            #No.FUN-6A0073
   ...
   OPEN WINDOW i920_w AT p_row,p_col 
     WITH FORM "agl/42f/agli920"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
      ...
      CASE g_action_choice
         ...
         WHEN "output"
            IF cl_chk_act_auth() THEN
               ...
               LET l_cmd = 'p_query "agli920" "',g_wc2 CLIPPED,'"'               
...
DEFINE l_ac_t          LIKE type_file.num5,                #未取消的ARRAY CNT  #No.FUN-680098 SMALLINT
   ...
   LET g_forupd_sql = " SELECT gim00,gim01,gim02,gim03 ",
                      "   FROM gim_file ",
                      "   WHERE gim00 = ? ", 
                      "    AND gim01 = ? FOR UPDATE "
...
#No.FUN-570108 --end--             
            ...
            OPEN i920_bcl USING g_gim_t.gim00,g_gim_t.gim01
            IF STATUS THEN
...
#No.FUN-570108 --start--                                                        
         ...
         LET  g_before_input_done = TRUE                                         
#No.FUN-570108 --end--          
         INITIALIZE g_gim[l_ac].* TO NULL      #900423
         LET g_gim[l_ac].gim00 = 'N'           #default
         LET g_gim_t.* = g_gim[l_ac].*         #新輸入資料
         CALL cl_show_fld_cont()     #FUN-550037(smin)
         NEXT FIELD gim00
 
      AFTER FIELD gim00                        #check 編號是否重複
         IF NOT cl_null(g_gim[l_ac].gim00) THEN 
            IF g_gim[l_ac].gim00 NOT MATCHES '[YN]' THEN 
               NEXT FIELD gim00
            END IF
         END IF
         IF NOT cl_null(g_gim[l_ac].gim01) THEN
            IF g_gim[l_ac].gim00 != g_gim_t.gim00 THEN
               SELECT count(*) INTO l_n FROM gim_file
                WHERE gim00 = g_gim[l_ac].gim00
                  AND gim01 = g_gim[l_ac].gim01
               IF l_n > 0 THEN
                  CALL cl_err('',-239,0)
                  LET g_gim[l_ac].gim00 = g_gim_t.gim00
      ...
      BEFORE DELETE                            #是否取消單身
         IF g_gim_t.gim01 IS NOT NULL AND g_gim_t.gim01 IS NOT NULL THEN
            ...
            LET g_doc.column1 = "gim00"               #No.FUN-9B0098 10/02/24
            LET g_doc.value1 = g_gim[l_ac].gim00      #No.FUN-9B0098 10/02/24
            LET g_doc.column2 = "gim01"               #No.FUN-9B0098 10/02/24
            LET g_doc.value2 = g_gim[l_ac].gim01      #No.FUN-9B0098 10/02/24
            ...
            DELETE FROM gim_file 
             WHERE gim00 = g_gim_t.gim00 
               AND gim01 = g_gim_t.gim01
            IF SQLCA.sqlcode THEN
#              CALL cl_err(g_gim_t.gim01,SQLCA.sqlcode,0)   #No.FUN-660123
               CALL cl_err3("del","gim_file",g_gim_t.gim00,g_gim_t.gim01,SQLCA.sqlcode,"","",1)  #No.FUN-660123
               ROLLBACK WORK
               CANCEL DELETE
            END IF
      ...
      ON ROW CHANGE
         ...
         IF l_lock_sw = 'Y' THEN
            ...
            LET g_gim[l_ac].* = g_gim_t.*
         ELSE
            UPDATE gim_file SET gim00 = g_gim[l_ac].gim00, 
             ...
             WHERE gim00 = g_gim_t.gim00
               AND gim01 = g_gim_t.gim01
            IF SQLCA.sqlcode THEN
#              CALL cl_err(g_gim[l_ac].gim01,SQLCA.sqlcode,0)   #No.FUN-660123
               CALL cl_err3("upd","gim_file",g_gim_t.gim00,g_gim_t.gim01,SQLCA.sqlcode,"","",1)  #No.FUN-660123
               LET g_gim[l_ac].* = g_gim_t.*
               ROLLBACK WORK
            ELSE
               MESSAGE 'UPDATE O.K'
      ...
      AFTER INSERT
         ...
         INSERT INTO gim_file(gim00,gim01,gim02,gim03)
                       VALUES(g_gim[l_ac].gim00,g_gim[l_ac].gim01,
                              g_gim[l_ac].gim02,g_gim[l_ac].gim03)
          IF SQLCA.sqlcode THEN
#            CALL cl_err(g_gim[l_ac].gim01,SQLCA.sqlcode,0)   #No.FUN-660123
             CALL cl_err3("ins","gim_file",g_gim[l_ac].gim00,g_gim[l_ac].gim01,SQLCA.sqlcode,"","",1)  #No.FUN-660123
             LET g_gim[l_ac].* = g_gim_t.*
          ELSE
             MESSAGE 'INSERT O.K'
             LET g_rec_b = g_rec_b + 1
...
FUNCTION i920_b_askkey()
   ...
   CONSTRUCT g_wc2 ON gim00,gim01,gim02,gim03
        FROM s_gim[1].gim00,s_gim[1].gim01,s_gim[1].gim02,s_gim[1].gim03 
...
FUNCTION i920_out()
   ...
   CALL cl_wait()
   CALL cl_outnam('agli920') RETURNING l_name
   SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_lang
   
   LET g_sql="SELECT * FROM gim_file ",          # 組合出 SQL 指令
             " WHERE ",g_wc2 CLIPPED,
             " ORDER BY gim01 "
   ...
   FOREACH i920_co INTO l_gim.*
      IF SQLCA.sqlcode THEN
         CALL cl_err('Foreach:',SQLCA.sqlcode,1)   
         EXIT FOREACH
      END IF
 
      OUTPUT TO REPORT i920_rep(l_gim.*)
...
REPORT i920_rep(sr)
   ...
   FORMAT
      ...
      ON EVERY ROW
         PRINT COLUMN g_c[31],sr.gim00 CLIPPED,
               COLUMN g_c[32],sr.gim01 USING '###&', #FUN-590118
               COLUMN g_c[33],sr.gim02 CLIPPED,
               COLUMN g_c[34],sr.gim03 USING '###,###,###,##&.&&'
 
      ON LAST ROW
         IF g_zz05 = 'Y' THEN     # 80:70,140,210      132:120,240
            CALL cl_wcchp(g_wc2,'gim00,gim01,gim02,gim03') RETURNING g_sql
            PRINT g_dash[1,g_len]
...
FUNCTION i920_set_entry(p_cmd)                                                  
  DEFINE p_cmd   LIKE type_file.chr1          #No.FUN-680098 VARCHAR(1)
   IF p_cmd = 'a' AND ( NOT g_before_input_done ) THEN                          
      CALL cl_set_comp_entry("gim00,gim01",TRUE)                                
   END IF                                                                       
END FUNCTION                                                                    
...
FUNCTION i920_set_no_entry(p_cmd)                                               
  DEFINE p_cmd   LIKE type_file.chr1          #No.FUN-680098 VARCHAR(1)
   IF p_cmd = 'u' AND ( NOT g_before_input_done ) AND g_chkey='N' THEN          
      CALL cl_set_comp_entry("gim00,gim01",FALSE)                               
   END IF                                                                       
END FUNCTION                                                                    
