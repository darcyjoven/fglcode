...
MAIN
   ...
   LET g_sql = "maj20_l.type_file.chr50,",
               ...
               "bal_l_3.type_file.num20_6,",   
               "per_l.type_file.num20_6,",
               "per_l_2.type_file.num20_6,",
               "per_l_3.type_file.num20_6,",
               ...
               "bal_r_3.type_file.num20_6," ,  
               "per_r.type_file.num20_6,", 
               "per_r_2.type_file.num20_6,",
               "per_r_3.type_file.num20_6,",
               "maj03_r.maj_file.maj03,",
               "maj02_r.maj_file.maj02,",
               "line_r.type_file.num5,",
               "line.type_file.num5"    
               
   LET l_table = cl_prt_temptable('aglr025',g_sql) CLIPPED   # 產生Temp Table   
   IF l_table = -1 THEN EXIT PROGRAM END IF                  # Temp Table產生   
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,  #FUN-B60140   Add
               " VALUES(?,?,?,?,?,   ?,?,?,?,?,   ",
               "        ?,?,?,?,?,   ?,?,?,?,?,    ?)"      #FUN-B60140   Add
   ...
   LET tm.b = ARG_VAL(11)
   LET tm.a = ARG_VAL(12)
   LET tm.aaz641  = ARG_VAL(13)
   LET tm.yy1 = ARG_VAL(14)
   LET tm.em1 = ARG_VAL(15)
   LET tm.q1 = ARG_VAL(16)
   LET tm.h1 = ARG_VAL(17)
   LET tm.yy2 = ARG_VAL(18)
   LET tm.em2 = ARG_VAL(19)
   LET tm.q2 = ARG_VAL(20)
   LET tm.h2 = ARG_VAL(21)
   LET tm.yy3 = ARG_VAL(22)
   LET tm.em3 = ARG_VAL(23)
   LET tm.q3 = ARG_VAL(24)
   LET tm.h3 = ARG_VAL(25)
   LET tm.c  = ARG_VAL(26)
   LET tm.d  = ARG_VAL(27)
   LET tm.e  = ARG_VAL(28)
   LET tm.f  = ARG_VAL(29)
   ...
   LET tm.q  = ARG_VAL(33)
   LET tm.r  = ARG_VAL(34)
   LET g_rep_user = ARG_VAL(35)
   LET g_rep_clas = ARG_VAL(36)
   LET g_template = ARG_VAL(37)
   LET g_rpt_name = ARG_VAL(38)
 
   IF cl_null(g_bookno) THEN LET g_bookno = g_aaz.aaz64 END IF
   IF cl_null(tm.aaz641) THEN LET tm.aaz641 = g_aza.aza81 END IF    
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL r025_tm()                      # Input print condition
      ELSE CALL r025()                         # Read data and create out-file
   END IF
...
FUNCTION r025_tm()
   ...
   OPEN WINDOW r025_w AT p_row,p_col WITH FORM "agl/42f/aglr025"
   ...
   WHILE TRUE
      ...
      INPUT BY NAME tm.axa01,tm.axa02,tm.b,tm.aaz641,tm.a,
         ...
         AFTER FIELD axa01
	       ...
	       LET tm.h2 = ''
            ...
            IF tm.axa06 = '4' THEN
               ...
               LET tm.h3 = ''
            END IF
            DISPLAY BY NAME tm.em1,tm.em2,tm.em3,
                            tm.q1,tm.q2,tm.q3,tm.h1,tm.h2,tm.h3
        ...
        AFTER FIELD em2
           IF cl_null(tm.em2) THEN NEXT FIELD em2 END IF
       ...
       AFTER FIELD q3
          IF NOT cl_null(tm.q3) AND tm.q3 NOT MATCHES '[1234]' THEN
             NEXT FIELD q1
          END IF
          CASE tm.q3
             WHEN 1  CALL cl_getmsg('agl-304',g_lang) RETURNING tm.title3
       ...
       AFTER FIELD h3
          IF (tm.h3>2 OR tm.h3<0) AND tm.axa06='4' THEN
             NEXT FIELD h1
          END IF
          CASE tm.h3
             WHEN 1  CALL cl_getmsg('agl-308',g_lang) RETURNING tm.title3
        ...
        AFTER FIELD axa02
         ...
         AFTER FIELD aaz641
            ...
            PREPARE r025_cnt_p FROM g_sql
            DECLARE r025_cnt_c CURSOR FOR r025_cnt_p
            OPEN r025_cnt_c
            FETCH r025_cnt_c INTO l_cnt
            ...
            IF NOT cl_null(tm.axa06) THEN
               CASE
                  WHEN tm.axa06 = '1'  #月 
                     ...
                     LET tm.bm2 = 0
                  OTHERWISE      
                     CALL s_get_aznn01(g_dbs_axz03,tm.axa06,tm.b,tm.yy1,tm.q1,tm.h1)    
                          RETURNING tm.em1
                     CALL s_get_aznn01(g_dbs_axz03,tm.axa06,tm.b,tm.yy2,tm.q2,tm.h2)   
                          RETURNING tm.em2
                     CALL s_get_aznn01(g_dbs_axz03,tm.axa06,tm.b,tm.yy3,tm.q3,tm.h3)  
                          RETURNING tm.em3
               END CASE
            END IF
      ...
      END INPUT
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW r025_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            LET l_cmd = l_cmd CLIPPED,          #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_template CLIPPED,"'",
                         " '",g_rpt_name CLIPPED,"'"
            CALL cl_cmdat('aglr025',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW r025_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
...
FUNCTION r025()
   ...
   DEFINE per3      LIKE fid_file.fid03
   ...
   DROP TABLE aglr025_tmp
   CREATE TEMP TABLE aglr025_tmp(
      maj23    LIKE maj_file.maj23,
      maj20    LIKE type_file.chr50,
      bal      LIKE type_file.num20_6,
      bal_1    LIKE type_file.num20_6,    
   ...
   LET l_sql = "SELECT * FROM maj_file ",
               ...
               "   AND maj23[1,1]='1' ",
               " ORDER BY maj02"
   PREPARE r025_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      EXIT PROGRAM
   END IF

   DECLARE r025_c CURSOR FOR r025_p
   ...
   FOREACH r025_c INTO maj.*
      ...
      IF NOT cl_null(maj.maj21) THEN
        #會計科目各期余額檔
         LET g_sql="SELECT SUM(axh08-axh09) ",
                   "  FROM axh_file,",cl_get_target_table(g_plant,'aag_file'),    #g_dbs_axz03,"aag_file",
      ...
      IF NOT cl_null(maj.maj21) AND NOT cl_null(tm.yy3) THEN
         LET g_sql="SELECT SUM(axh08-axh09)",
                   ...
                   "   AND axh05 = aag01 AND aag07 !='1' "
         CALL cl_replace_sqldb(g_sql) RETURNING g_sql
         PREPARE sel_amt3_pre FROM g_sql
         DECLARE sel_amt3_cs CURSOR FOR sel_amt3_pre
         OPEN sel_amt3_cs
         FETCH sel_amt3_cs INTO amt3
      ...
      IF maj.maj03 MATCHES "[012349]" AND maj.maj08 > 0 THEN    #合計階數處理
         ...
         LET k=maj.maj08  LET sr.bal3=g_tot3[k]
      ...
      IF maj.maj11 = 'Y' THEN                        #百分比基準科目
         ...
         IF tm.d MATCHES '[23]' THEN LET g_basetot2=g_basetot2/g_unit END IF   
         IF tm.d MATCHES '[23]' THEN LET g_basetot3=g_basetot3/g_unit END IF
      END IF
      ...
      IF (tm.c='N' OR maj.maj03='2') AND             #餘額為 0 者不列印
          maj.maj03 MATCHES "[012345]" AND sr.bal1=0 AND sr.bal2=0 AND sr.bal3 = 0 THEN   
         CONTINUE FOREACH
      END IF
      ...
      IF maj.maj07='2' THEN LET sr.bal3=sr.bal3*-1 END IF
      LET sr.bal1=cl_numfor(sr.bal1,17,tm.e)
      LET sr.bal2=cl_numfor(sr.bal2,17,tm.e)
      LET sr.bal3=cl_numfor(sr.bal3,17,tm.e)
      #列印額外名稱
      ...
      IF tm.d MATCHES '[23]' THEN LET sr.bal3=sr.bal3/g_unit END IF
      LET maj.maj20=maj.maj05 SPACES,maj.maj20

      IF maj.maj03='3' OR maj.maj03='4' THEN
         IF NOT cl_null(maj.maj20) THEN
            INSERT INTO aglr025_tmp VALUES
             (maj.maj23,maj.maj20,sr.bal1,sr.bal2,sr.bal3,per1,per2,per3,'1',maj.maj02,2)
         END IF
         INSERT INTO aglr025_tmp VALUES
          (maj.maj23,maj.maj20,sr.bal1,sr.bal2,sr.bal3,per1,per2,per3,maj.maj03,maj.maj02,2)
      ELSE
         INSERT INTO aglr025_tmp VALUES
          (maj.maj23,maj.maj20,sr.bal1,sr.bal2,sr.bal3,per1,per2,per3,maj.maj03,maj.maj02,2)
      END IF
      IF maj.maj04 > 0 THEN
         ...
         FOR i = 1 TO maj.maj04
            INSERT INTO aglr025_tmp VALUES
             (maj.maj23,maj.maj20,'','','','','','',maj.maj03,maj.maj02,1)  
         END FOR
      END IF
   END FOREACH

   #抓報表結構裡帳戶左邊最大序號
   SELECT MAX(maj02) INTO l_maj02_l FROM aglr025_tmp WHERE maj23='11'
   #抓報表結構裡帳戶右邊最大序號
   SELECT MAX(maj02) INTO l_maj02_r FROM aglr025_tmp WHERE maj23='12'
   IF cl_null(l_maj02_l) THEN LET l_maj02_l = 0 END IF
   IF cl_null(l_maj02_r) THEN LET l_maj02_r = 0 END IF
   LET r_row = 0  LET l_row = 0
   DECLARE r025_c1 CURSOR FOR SELECT * FROM aglr025_tmp ORDER BY maj02

   FOREACH r025_c1 INTO tmp1.*
      IF tmp1.maj23[2,2]='2' THEN     #--- 右邊(負債&業主權益)
         ...
         LET prt_r[r_row].bal_2=tmp1.bal_2
         LET prt_r[r_row].per  =tmp1.per
         LET prt_r[r_row].per_1=tmp1.per_1    
         LET prt_r[r_row].per_2=tmp1.per_2
      ...
      ELSE                           #--- 左邊(資產)
         ...
         LET prt_l[l_row].bal_2=tmp1.bal_2
         LET prt_l[l_row].per  =tmp1.per
         LET prt_l[l_row].per_1=tmp1.per_1   
         LET prt_l[l_row].per_2=tmp1.per_2
   ...
   FOR i=1 TO l_last
      EXECUTE insert_prep USING
         prt_l[i].maj20,prt_l[i].bal,    prt_l[i].bal_1,  prt_l[i].bal_2,  
                        prt_l[i].per,    prt_l[i].per_1,  prt_l[i].per_2,
         prt_l[i].maj03,prt_l[i].maj02,  prt_l[i].line,
         prt_r[i].maj20,prt_r[i].bal,    prt_r[i].bal_1,  prt_r[i].bal_2,  
                        prt_r[i].per,    prt_r[i].per_1,  prt_r[i].per_2,
         prt_r[i].maj03,prt_r[i].maj02,  prt_r[i].line,   i
   END FOR

   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED," ORDER BY line"
   LET l_lastday = MDY(tm.em1,'1',tm.yy1)
   LET l_lastday = s_last(l_lastday)
   LET g_str = g_mai02,";",tm.a,";",tm.p,";",tm.d,";",tm.yy1,";",
               tm.bm1,";",tm.e,";",l_lastday,";",g_basetot1,
               ";",tm.yy2,";",tm.title1,";",tm.title2,";",tm.title3,";",tm.yy3
   CALL cl_prt_cs3('aglr025','aglr025',g_sql,g_str)

END FUNCTION

FUNCTION r025_set_entry()                                                                                                           
   CALL cl_set_comp_entry("q1,em1,h1,q2,em2,h2,q3,em3,h3",TRUE)
END FUNCTION                                                                                                                        
                                                                                                                                    
FUNCTION r025_set_no_entry()                                                                                                        
   IF tm.axa06 ="1" THEN  #月
      CALL cl_set_comp_entry("q1,h1,q2,h2,q3,h3",FALSE)  
   END IF
   IF tm.axa06 ="2" THEN  #季
      CALL cl_set_comp_entry("em1,h1,em2,h2,em3,h3",FALSE)
   END IF
   IF tm.axa06 ="3" THEN  #半年
      CALL cl_set_comp_entry("em1,q1,em2,q2,em3,q3",FALSE)
   END IF
   IF tm.axa06 ="4" THEN  #年
      CALL cl_set_comp_entry("q1,em1,h1,q2,em2,g2,q3,em3,h3",FALSE)
   END IF
END FUNCTION
#FUN-BC0083
