  ...
  INPUT NO WRAP
   ...
   CALL cl_used(g_prog,g_time,1) RETURNING g_time #No.FUN-690114
	...
	IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN	# If background job sw is off
		CALL u811_tm()						# Input condition
	ELSE
		CALL aglu811()						# Read data and process it
	END IF
...
DEFINE
   ...
   ELSE LET p_row = 4
   END IF
	OPEN WINDOW u811_w AT p_row,p_col
		WITH FORM "agl/42f/aglu811"
################################################################################
...
################################################################################
	...
	WHILE TRUE
		CONSTRUCT tm.wc
			ON aec02,aec01,aec05
			FROM transaction_date,account_no,transaction_code
       ON ACTION locale
      ...
      ON ACTION help          #MOD-4C0121
		...
		IF g_bgjob = 'Y' THEN
			...
			ELSE
				...
				CALL cl_cmdat('aglu811',g_time,l_cmd)
			END IF
			CLOSE WINDOW u811_w
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
			EXIT PROGRAM
...
DEFINE
	...
	IF tm.download='Y' THEN
		LET l_sql=
			...
			" select * from aec_file",
			' where aec00="',g_bookno,'" and ',tm.wc CLIPPED,
			"'"
	...
	IF tm.cleanup='Y' THEN
		MESSAGE "delete data ..."
		LET l_sql="DELETE FROM aec_file ",
			" WHERE aec00='",g_bookno,"' AND ",tm.wc CLIPPED
		PREPARE u811_del FROM l_sql
