...
# Modify.........: No.FUN-C50004 12/05/04 By nanbing GR 優化
DATABASE ds
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD
                ...
                aaz641  LIKE aaz_file.aaz641,    #合并帳別編號    #FUN-B50001#FUN-B90028(FUN-B50001 rollback)
                #aaw01  LIKE aaw_file.aaw01,    #合并帳別編號#FUN-B90028(FUN-B50001 rollback)
...
TYPE sr1_t RECORD
    ...
    bal01 LIKE axh_file.axh08,
    bal02 LIKE axh_file.axh08,
    bal03 LIKE axh_file.axh08,
    bal04 LIKE axh_file.axh08,
    bal05 LIKE axh_file.axh08,
    bal06 LIKE axh_file.axh08,
    bal07 LIKE axh_file.axh08,
    bal08 LIKE axh_file.axh08,
    bal09 LIKE axh_file.axh08,
    bal10 LIKE axh_file.axh08,
    bal11 LIKE axh_file.axh08,
    bal12 LIKE axh_file.axh08,
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglg013',g_sql) CLIPPED   # 產生Temp Table   
...
FUNCTION g013_tm()
   ...
   OPEN WINDOW g013_w AT p_row,p_col WITH FORM "agl/42f/aglg013"
   ...
   WHILE TRUE
     ...
     # INPUT BY NAME tm.rtype,tm.axa01,tm.axa02,tm.b,tm.aaz641,tm.a,tm.yy,tm.em,tm.e,tm.f,  
      INPUT BY NAME tm.rtype,tm.axa01,tm.axa02,tm.b,tm.aaz641,tm.a,tm.yy,tm.e,tm.f, #FUN-AB0085   #FUN-B50001#FUN-B90028(FUN-B50001 rollback)
      #INPUT BY NAME tm.rtype,tm.axa01,tm.axa02,tm.b,tm.aaw01,tm.a,tm.yy,tm.e,tm.f, #FUN-B90028(FUN-B50001 rollback)
                    tm.d,tm.c,tm.h,tm.o,tm.r,
                    tm.p,tm.q,tm.more WITHOUT DEFAULTS  
         BEFORE INPUT
            CALL cl_qbe_init()
            CALL g013_set_entry()    #FUN-A90032
            CALL g013_set_no_entry() #FUN-A90032
...
#FUN-B50001--mod--end#FUN-B90028(FUN-B50001 rollback)
            ...
            PREPARE g013_cnt_p FROM g_sql
            DECLARE g013_cnt_c CURSOR FOR g013_cnt_p
            OPEN g013_cnt_c
            FETCH g013_cnt_c INTO l_cnt
         ...
         AFTER FIELD yy
            ...
            SELECT azmm02 INTO l_azmm02
              FROM azmm_file
             WHERE azmm00 = tm.aaz641   #FUN-B50001#FUN-B90028(FUN-B50001 rollback)
             #WHERE azmm00 = tm.aaw01#FUN-B90028(FUN-B50001 rollback)
               AND azmm01 = tm.yy
...
#FUN-A90032 --End
         ...
         ON ACTION CONTROLP
            ...
            IF INFIELD(a) THEN
               ...
               LET g_qryparam.where = " mai00 = '",tm.aaz641,"'" #FUN-B50001 aaz641->aaw01 #FUN-B90028(FUN-B50001 rollback)
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                        ...
                        " '",g_template CLIPPED,"'",       
                        " '",g_rpt_name CLIPPED,"'"       
            CALL cl_cmdat('aglg013',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
FUNCTION g013()
   ...
   SELECT aaf03 INTO g_company FROM aaf_file
    WHERE aaf01 = tm.aaz641 AND aaf02 = g_rlang   #FUN-B50001#FUN-B90028(FUN-B50001 rollback)
    #WHERE aaf01 = tm.aaw01 AND aaf02 = g_rlang#FUN-B90028(FUN-B50001 rollback)
   ...
   IF STATUS THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g013_c CURSOR FOR g013_p
   ...
   IF STATUS THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g013_axh_c CURSOR FOR g013_axh_p   
   #FUN-C50004 end
   
   FOREACH g013_c INTO maj.*
      IF STATUS THEN
      ...
      #--FUN-AB0085 end---------------------
         ...
         IF NOT cl_null(maj.maj21) THEN
            ...
            #OPEN sel_amt_cs
            #FUN-C50004 mark end
            OPEN sel_amt_cs USING maj.maj21,maj.maj22,i #FUN-C50004 add
   ...
   IF tm.rtype = "1" THEN                                                       
#      LET l_name = 'aglg013_1'                                                  
         LET g_template = 'aglg013_1'        #FUN-B90028 ad
   ELSE                                                                         
#      LET l_name = 'aglg013' 
         LET g_template = 'aglg013'        #FUN-B90028 ad
   END IF 
###GENGRE###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED             
###GENGRE###   LET g_str = g_mai02,";",tm.a,";",tm.p,";",tm.d,";",tm.yy,";",
###GENGRE###               tm.rtype,";",tm.e      
###GENGRE###   CALL cl_prt_cs3('aglg013',l_name,g_sql,g_str)
    CALL aglg013_grdata()    ###GENGRE###
END FUNCTION
　
#FUN-A30122 --Begin
#FUNCTION g013_getdbs()
...
#              " WHERE aaz00 = '0'"
#  PREPARE g013_pre_11 FROM g_sql
#  DECLARE g013_cur_11 CURSOR FOR g013_pre_11
#  OPEN g013_cur_11
#  FETCH g013_cur_11 INTO g_aaz641
#END FUNCTION	　
#FUN-A30122 --End
#FUN-9A0036　	　	　
#FUN-A90032 --Begin
FUNCTION g013_set_entry()                                                                                                           
   CALL cl_set_comp_entry("q1,em,h1",TRUE)#FUN-A90032 add
END FUNCTION                                                                                                                        
                                                                                                                                    
FUNCTION g013_set_no_entry()                                                                                                        
...
FUNCTION aglg013_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg013")
        IF handler IS NOT NULL THEN
            START REPORT aglg013_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg013_datacur1 CURSOR FROM l_sql
            FOREACH aglg013_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg013_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg013_rep
        END IF
