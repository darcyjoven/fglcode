...
TYPE sr1_t RECORD
    ...
    bug1 LIKE afc_file.afc06,
    bug2 LIKE afc_file.afc06,
    bug3 LIKE afc_file.afc06,
    bug4 LIKE afc_file.afc06,
    bug5 LIKE afc_file.afc06,
    #FUN-B80158-------add----str-----
    l_bug1 LIKE afc_file.afc06,
    l_bug2 LIKE afc_file.afc06,
    l_bug3 LIKE afc_file.afc06,
    l_bug4 LIKE afc_file.afc06,
    l_bug5 LIKE afc_file.afc06
    #FUN-B80158-------add----end-----
END RECORD
###GENGRE###END
...
MAIN
   ...
   LET g_sql="aag01.aag_file.aag01,aag02.aag_file.aag02,",
             ...
             "bug5.afc_file.afc06,",
             #FUN-B80158-------add----str-----
             "l_bug1.afc_file.afc06, l_bug2.afc_file.afc06,",
             "l_bug3.afc_file.afc06, l_bug4.afc_file.afc06,",
             "l_bug5.afc_file.afc06"
             #FUN-B80158-------add----end-----
   LET l_table = cl_prt_temptable('aglg602',g_sql) CLIPPED
   IF l_table = -1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?)"  #MOD-8C0064 add ?  #FUN-B80158 add 5?
...
#No.FUN-740020 ---end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL g602_tm()	        	# Input print condition
      ELSE CALL g602()			# Read data and create out-file
   END IF
...
DEFINE li_chk_bookno    LIKE type_file.num5   #FUN-B20054 
   ...
   END IF
   OPEN WINDOW g602_w AT p_row,p_col
        WITH FORM "agl/42f/aglg602"
################################################################################
...
# 
#   IF INT_FLAG THEN
#      LET INT_FLAG = 0 CLOSE WINDOW g602_w 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
#      EXIT PROGRAM
#         
#   END IF
 ...
 #FUN-B20054--add--end
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglg602',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW g602_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)   #FUN-B80158 add
...
DEFINE l_bookno2       LIKE aaa_file.aaa01
     ...
     IF SQLCA.sqlcode THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE g602_c1 CURSOR FOR g602_p1
     ...
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE g602_c2 CURSOR FOR g602_p2
 
     FOREACH g602_c1 INTO sr.aag01,sr.aag02,sr.aag13,sr.aag06,sr.aag07,
     ...
     #No.FUN-840227  --End
        ...
        #No.FUN-840227  --Begin
        #本期預計消耗
        OPEN g602_c2 USING sr.aag01,sr.afb04,sr.afb041,sr.afb042,'1'
        FETCH g602_c2 INTO l_debit
        IF STATUS THEN CALL cl_err('c2:',STATUS,0) EXIT FOREACH END IF
        IF l_debit IS NULL THEN LET l_debit = 0 END IF
        OPEN g602_c2 USING sr.aag01,sr.afb04,sr.afb041,sr.afb042,'2'
        FETCH g602_c2 INTO l_credit
        IF STATUS THEN CALL cl_err('c2:',STATUS,0) EXIT FOREACH END IF
...
#       OUTPUT TO REPORT g602_rep(sr.*)              #No.FUN-780061
        #No.FUN-780061 --add--
 
        #FUN-B80158-----add-----str----
        IF sr.aag07 != 0 THEN
           LET sr.l_bug1 = sr.bug1
           LET sr.l_bug2 = sr.bug2
           LET sr.l_bug3 = sr.bug3
           LET sr.l_bug4 = sr.bug4
           LET sr.l_bug5 = sr.bug5
        END IF 
        #FUN-B80158-----add-----end----
        ...
        EXECUTE insert_prep USING
           ...
           ,sr.l_bug1,sr.l_bug2,sr.l_bug3,sr.l_bug4,sr.l_bug5    #FUN-B80158 ADD
        #No.FUN-780061 --end--
     END FOREACH
 
#     FINISH REPORT g602_rep                                 #No.FUN-780061
...
#     CALL cl_prt(l_name,g_prtway,g_copies,g_len)            #No.FUN-780061
     ...
     IF g_zz05 = 'Y' THEN
        ...
        LET g_str = tm.wc
     END IF
###GENGRE###     LET g_str = g_str,";",tm.bugno,";",tm.yy,";",tm.mm,";",tm.e,";",
###GENGRE###                 g_azi04,";",g_azi05
###GENGRE###     LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED
###GENGRE###     CALL cl_prt_cs3('aglg602','aglg602',l_sql,g_str)
    CALL aglg602_grdata()    ###GENGRE###
     #No.FUN-780061 --end--
END FUNCTION
...
FUNCTION aglg602_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg602")
        IF handler IS NOT NULL THEN
            START REPORT aglg602_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg602_datacur1 CURSOR FROM l_sql
            FOREACH aglg602_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg602_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg602_rep
        END IF
...
REPORT aglg602_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B80158--------add-----str----
    DEFINE l_totalbug1   LIKE afc_file.afc06
    DEFINE l_totalbug2   LIKE afc_file.afc06
    DEFINE l_totalbug3   LIKE afc_file.afc06
    DEFINE l_totalbug4   LIKE afc_file.afc06
    DEFINE l_totalbug5   LIKE afc_file.afc06
    DEFINE l_aag07_desc  STRING
    DEFINE l_bug_fmt     STRING
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B80158--------add-----str----
            LET l_bug_fmt = cl_gr_numfmt("afc_file","afc06",g_azi04)
            PRINTX l_bug_fmt
            LET l_aag07_desc = cl_gr_getmsg("gre-202",g_lang,sr1.aag07)
            PRINTX l_aag07_desc
            #FUN-B80158--------add-----end----
        ...
        ON LAST ROW
            #FUN-B80158--------add-----str----
            LET l_total_fmt = cl_gr_numfmt("afc_file","afc06",g_azi05)
            PRINTX l_total_fmt
            LET l_totalbug1 = SUM(sr1.l_bug1)
            LET l_totalbug2 = SUM(sr1.l_bug2)
            LET l_totalbug3 = SUM(sr1.l_bug3)
            LET l_totalbug4 = SUM(sr1.l_bug4)
            LET l_totalbug5 = SUM(sr1.l_bug5)
            PRINTX l_totalbug1
            PRINTX l_totalbug2
            PRINTX l_totalbug3
            PRINTX l_totalbug4
