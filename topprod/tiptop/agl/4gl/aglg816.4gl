...
MAIN
   ...
   LET g_sql = "order1.type_file.num5,",
               ...
               "abg00.abg_file.abg00,",    
               "abg01.abg_file.abg01,",   
               "abg06.abg_file.abg06,",   
               "aag01.aag_file.aag01,",   
               "aag02.aag_file.aag02,", 
               "msg0.type_file.chr100,",  
               "msg1.type_file.chr100,", 
               "msg2.type_file.chr100" 
   LET l_table = cl_prt_temptable('aglg816',g_sql) CLIPPED
...
FUNCTION g816_tm()
   ...
   OPEN WINDOW g816_w AT p_row,p_col WITH FORM "agl/42f/aglg816" 
      ...
      IF tm.more = 'Y' THEN
         ...
         ELSE
            LET l_cmd = l_cmd CLIPPED,          #(at time fglgo xxxx p1 p2 p3)
                      ...
                      " '",tm.b_abg06 CLIPPED, "'",
                      " '",tm.e_abg06 CLIPPED, "'"
            CALL cl_cmdat('aglg816',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW g816_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time
         CALL cl_gre_drop_temptable(l_table)            #FUN-C40071 add
...
FUNCTION g816()
  ...
  #傳票日期
   ...
   PREPARE g816_p1 FROM l_sql
   DECLARE g816_c1 CURSOR WITH HOLD FOR g816_p1

  #FUN-C40071------add-----str--
   LET l_sql = " SELECT ze02,ze03 FROM ze_file"
               ,"  WHERE ze01 = 'agl-518'"
               ,"  ORDER BY ze02"
   PREPARE g816_p_518 FROM l_sql
   DECLARE g816_c_518 CURSOR FOR g816_p_518

   LET l_sql = " SELECT ze02,ze03 FROM ze_file"
               ,"  WHERE ze01 = 'agl-229'"
               ,"  ORDER BY ze02"
   PREPARE g816_p_229 FROM l_sql
   DECLARE g816_c_229 CURSOR FOR g816_p_229

   LET l_sql = " SELECT ze02,ze03 FROM ze_file"
               ,"  WHERE ze01 = 'agl-519'"
               ,"  ORDER BY ze02"
   PREPARE g816_p_519 FROM l_sql
   DECLARE g816_c_519 CURSOR FOR g816_p_519

   LET l_sql = " SELECT gaq03 FROM gaq_file"
               ,"  WHERE gaq01 = ? AND gaq02 = ?"
   PREPARE g816_p_gaq03 FROM l_sql
   DECLARE g816_c_gaq03 CURSOR FOR g816_p_gaq03

   LET l_sql = " SELECT ze02,ze03 FROM ze_file"
               ,"  WHERE ze01 = 'agl1020'"
               ,"  ORDER BY ze02"
   PREPARE g816_p_1020 FROM l_sql
   DECLARE g816_c_1020 CURSOR FOR g816_p_1020

   LET l_sql = " SELECT ze02,ze03 FROM ze_file"
               ,"  WHERE ze01 = 'agl-520'"
               ,"  ORDER BY ze02"
   PREPARE g816_p_520 FROM l_sql
   DECLARE g816_c_520 CURSOR FOR g816_p_520

   LET l_sql = " SELECT ze02,ze03 FROM ze_file"
               ,"  WHERE ze01 = 'agl-521'"
               ,"  ORDER BY ze02"
   PREPARE g816_p_521 FROM l_sql
   DECLARE g816_c_521 CURSOR FOR g816_p_521
  #FUN-C40071------add-----end--

   LET j = 0
   FOREACH g816_c1 INTO g_abg.*
      LET j = j+1
      LET l_row = 0
      CALL tmp1.clear()
    ...
    #FUN-C40071-----mark-----end--
      IF l_n = 0 THEN
         ...
         FOREACH g816_c_518 INTO l_ze02,l_ze03
            CASE l_ze02
               WHEN "0"
    ...
    #FUN-C40071-----mark-----end--
      IF l_n = 0 THEN
         ...
         FOREACH g816_c_229 INTO l_ze02,l_ze03
            CASE l_ze02
               WHEN "0"
                  LET tmp1[l_row].msg0 = l_ze03
               WHEN "1"
    ...
    # DECLARE g816_c_519 CURSOR FOR g816_p_519
    #FUN-C40071-----mark-----end--
      FOREACH g816_c_519 INTO l_ze02,l_ze03
         CASE l_ze02
            WHEN "0"
               LET l_ze03_0 = l_ze03
            WHEN "1"
     ...
     #FUN-C40071-----mark-----end--
      ...
      IF l_aag15_1 <> l_aag15_2 THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag15",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
         FETCH g816_c_gaq03 INTO l_gaq03
      ...
      IF l_aag151_1 <> l_aag151_2 THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag151",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag16_1 <> l_aag16_2 THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag16",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag161_1 <> l_aag161_2 THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag161",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag17_1 <> l_aag17_2  THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag17",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag171_1 <> l_aag171_2 THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag171",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag18_1 <> l_aag18_2  THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag18",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag181_1 <> l_aag181_2  THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag181",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag31_1 <> l_aag31_2  THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag31",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag311_1 <> l_aag311_2 THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag311",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag32_1 <> l_aag32_2  THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag32",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag321_1 <> l_aag321_2 THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag321",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag33_1 <> l_aag33_2  THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag33",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag331_1 <> l_aag331_2 THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag331",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag34_1 <> l_aag34_2 THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag34",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
      ...
      IF l_aag341_1 <> l_aag341_2 THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag341",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
     ...
     #   LET l_gaq03 = " "
     #   OPEN g816_c_gaq03 USING "aag35",g_lang
     #   IF STATUS THEN
     #      CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
     #      CLOSE g816_c_gaq03
     #   END IF
     ...
     #   LET l_gaq03 = " "
     #   OPEN g816_c_gaq03 USING "aag351",g_lang
     #   IF STATUS THEN
     #      CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
     #      CLOSE g816_c_gaq03
     #   END IF
     ...
     #   LET l_gaq03 = " "
     #   OPEN g816_c_gaq03 USING "aag36",g_lang
     #   IF STATUS THEN
     #      CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
     #      CLOSE g816_c_gaq03
     #   END IF
     ...
     #   LET l_gaq03 = " "
     #   OPEN g816_c_gaq03 USING "aag361",g_lang
     #   IF STATUS THEN
     #      CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
     #      CLOSE g816_c_gaq03
     #   END IF
     ...
     #FUN-C40071----mark---end--
      IF l_aag05_1 <> l_aag05_2  THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag05",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
         ...
         LET tmp1[l_row].aag01 = g_abg.abg03
         LET tmp1[l_row].aag02 = l_aag02 
         FOREACH g816_c_1020 INTO l_ze02,l_ze03
            CASE l_ze02
               WHEN "0"
     ...
     #FUN-C40071----mark---end--
      IF l_aag20_1 <> l_aag20_2  THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag20",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
         ...
         LET tmp1[l_row].aag01 = g_abg.abg03
         LET tmp1[l_row].aag02 = l_aag02 
         FOREACH g816_c_520 INTO l_ze02,l_ze03
            CASE l_ze02
               WHEN "0"
                  LET tmp1[l_row].msg0  = l_gaq03,"-",l_ze03
               WHEN "1"
     ...
     #FUN-C40071----mark---end--
      IF l_aag222_1 <> l_aag222_2 THEN
         LET l_gaq03 = " "
         OPEN g816_c_gaq03 USING "aag222",g_lang
         IF STATUS THEN
            CALL cl_err("OPEN g816_c_gaq03:", STATUS, 1)
            CLOSE g816_c_gaq03
         END IF
         ...
         LET tmp1[l_row].aag01 = g_abg.abg03
         LET tmp1[l_row].aag02 = l_aag02 
         FOREACH g816_c_521 INTO l_ze02,l_ze03
            CASE l_ze02
               WHEN "0"
                  LET tmp1[l_row].msg0  = l_gaq03,"-",l_ze03
               WHEN "1"
  ...
  #傳票日期
   ...
   PREPARE g816_p2 FROM l_sql
   DECLARE g816_c2 CURSOR WITH HOLD FOR g816_p2

   FOREACH g816_c2 INTO g_abg.*,g_abh.*
      LET j = j+1
      LET l_row = 0
      CALL tmp1.clear()
     #檢核拋轉帳中，是否有存在傳票，傳票未拋轉者不可拋立沖資料
...
###GENGRE###   LET g_str = ""
###GENGRE###   CALL cl_prt_cs3('aglg816','aglg816',g_sql,g_str) 
    CALL aglg816_grdata()    ###GENGRE###
END FUNCTION



###GENGRE###START
FUNCTION aglg816_grdata()
    DEFINE l_sql    STRING
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg816")
        IF handler IS NOT NULL THEN
            START REPORT aglg816_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg816_datacur1 CURSOR FROM l_sql
            FOREACH aglg816_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg816_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg816_rep
        END IF
...
REPORT aglg816_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5

    
  #  ORDER EXTERNAL BY sr1.abg00
