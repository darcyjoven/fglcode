...
# Modify.........: No.MOD-B90188 11/09/23 By Polly 計算合計階段需增加maj09的控制
# Modify.........: No.FUN-B90140 11/10/11 By minpp 增加checkbox"列印會計科目"
# Modify.........: No.MOD-BB0296 11/11/28 By Dido 合計階計算需包含 maj09 判斷處理
# Modify.........: No.FUN-BB0047 11/12/30 By fengrui  調整時間函數問題
# Modify.........: No.MOD-C10221 12/02/02 By Polly 查詢條件後需再重新給予 INPUT 欄位變數
# Modify.........: No.TQC-C50042 12/05/07 By zhangweib 修改開窗q_mai去除報表性質為5\6的資料
# Modify.........: No.TQC-C80006 12/08/01 By lujh 匯率欄位增加控管
# Modify.........: No.MOD-C80152 12/10/23 By Polly 異動帳別時，將新帳別的aaa03預設給予tm.r/tm.p
...
GLOBALS "../../config/top.global"
   ...
   DEFINE g_b_o          LIKE aaa_file.aaa01         #MOD-C80152 add
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglr110',g_sql) CLIPPED   # 產生Temp Table   
   IF l_table = -1 THEN EXIT PROGRAM END IF                  # Temp Table產生   
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,#No.TQC-820008 
               " VALUES(?,?,?,?,?,?, ?,? )"             #FUN-8C0018      #FUN-A10098 去掉1個 ?  #FUN-B90140   Add ?               
 ...
 #NO.FUN-A10098  ----end---
   ...
   IF cl_null(g_bookno) THEN
      ...
      LET g_b_o = g_bookno           #MOD-C80152 add
   END IF
...
FUNCTION r110_tm()
   ...
   DEFINE lc_qbe_sn       LIKE gbm_file.gbm01       #MOD-C10221 add
   DEFINE l_azi05         LIKE azi_file.azi05       #MOD-C80152 add
   ...
   OPEN WINDOW r110_w AT p_row,p_col
     WITH FORM "agl/42f/aglr110"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
  ...
  #LET tm.p1=g_plant
  #NO.FUN-A10098 ----end---
   CALL r110_set_entry_1()               
   CALL r110_set_no_entry_1()    
   ...
   WHILE TRUE
      ...
      INPUT BY NAME tm.rtype,
                  ...
                  # tm.p4,tm.p5,tm.p6,tm.p7,tm.p8,     #FUN-8C0018  #FUN-A10098 mark
                    ...
                    tm.e,tm.f,tm.d,tm.acc_code,tm.c,tm.h,  #FUN-B90140   Add tm.acc_code
   ...
   #NO.FUN-A10098  ----end---
         ...
         #TQC-C80006--add--str--
         AFTER FIELD q
            IF NOT cl_null(tm.q) THEN
               IF tm.r = tm.p THEN
                  IF tm.q != 1 THEN
                     CALL cl_err('','agl-453',0)
                     NEXT FIELD q
                  END IF
               END IF
            END IF
         ...
         ON ACTION qbe_select
           ...
           #---------------------------MOD-C10221-------------------------start
            ...
            LET tm.bm = GET_FLDBUF(bm)
            LET tm.em = GET_FLDBUF(em)
            LET tm.dd = GET_FLDBUF(dd)
            ...
            LET tm.xbrl = GET_FLDBUF(xbrl)
            LET tm.exp = GET_FLDBUF(exp)
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       #NO.FUN-A10098  ---end----
            CALL cl_cmdat('aglr110',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
FUNCTION r110()
   ...
   FOREACH r110_c INTO maj.*
      ...
      IF NOT cl_null(maj.maj21) THEN
         ...
         IF maj.maj24 IS NULL THEN
            ...
            DECLARE r110_c1  CURSOR FOR r110_prepare1                                                                                 
            OPEN r110_c1                                                                                    
            FETCH r110_c1 INTO amt1
            IF SQLCA.sqlcode THEN 
         ...
         ELSE
            ...
            DECLARE r110_c2  CURSOR FOR r110_prepare2                                                                                 
            OPEN r110_c2                                                                                    
            FETCH r110_c2 INTO amt1
            IF SQLCA.sqlcode THEN 
        ...
        #NO.FUN-A10098  ----end----
         ...
         PREPARE r110_prepare6 FROM l_sql                                                                                          
         DECLARE r110_c6  CURSOR FOR r110_prepare6                                                                                
         OPEN r110_c6                                                                                  
         FETCH r110_c6 INTO g_aaa09
         ...
         IF g_aaa09 = '2' THEN
           ...
           #NO.FUN-A10098  ----start---
                        ...
                        "   AND abb03 = aag01 ", 
                        "   AND aag03 <> '4' "   
            PREPARE r110_prepare3 FROM l_sql                                                                                          
            DECLARE r110_c3  CURSOR FOR r110_prepare3                                                                                 
            OPEN r110_c3                                                                                    
            FETCH r110_c3 INTO l_endy1
          #NO.FUN-A10098  ----start---
          # LET l_sql = "SELECT SUM(abb07) FROM ",l_dbs CLIPPED," abb_file,",l_dbs CLIPPED," aba_file,",l_dbs CLIPPED," aag_file ", 
            LET l_sql = "SELECT SUM(abb07) FROM abb_file,aba_file,aag_file ",
          #NO.FUN-A10098  ----end---      
                        ...
                        " AND abb03 = aag01 ",  
                        " AND aag03 <> '4' "    
            PREPARE r110_prepare4 FROM l_sql                                                                                          
            DECLARE r110_c4  CURSOR FOR r110_prepare4                                                                                 
            OPEN r110_c4                                                                                    
            FETCH r110_c4 INTO l_endy2
         ...
         IF NOT cl_null(tm.dd) THEN
            LET l_sql = "SELECT SUM(aas04-aas05) ",
                       ...
                       #NO.FUN-A10098   ----end---
                        ...
                        " AND aas01 = aag01 ",
                        " AND aag07 IN ('2','3') "
            PREPARE r110_prepare5 FROM l_sql                                                                                          
            DECLARE r110_c5  CURSOR FOR r110_prepare5                                                                                 
            OPEN r110_c5                                                                                    
            FETCH r110_c5 INTO l_tmp
            IF STATUS THEN 
      ...
      IF maj.maj03 MATCHES "[012]" AND maj.maj08 > 0 THEN
         FOR i = 1 TO 100
            IF maj.maj09 = '-' THEN               #MOD-BB0296
               LET g_tot1[i] = g_tot1[i] - amt1   #MOD-BB0296 
            ELSE                                  #MOD-BB0296
         ...
         IF maj.maj07 = '1' AND maj.maj09 = '-' THEN        #MOD-B90188 add
            LET sr.bal1 = sr.bal1 * -1                      #MOD-B90188 add
         END IF                                             #MOD-B90188 add
      ...
      ELSE                                                                      
         ...
         FOR i = 1 TO maj.maj04                                                 
            EXECUTE insert_prep USING                                           
               maj.maj31,maj.maj20,maj.maj20e,maj.maj02,'',    #FUN-B90140   Add maj.maj31                             
               '0','0',                                                 
 ...
 # END FOR                              #FUN-8C0018    #FUN-A10098
   ...
   LET g_str = g_mai02,";",tm.a,";",tm.p,";",tm.d,";",tm.yy,";",tm.bm,";",               
               tm.em,";",tm.e,";",g_basetot1,";",   #MOD-850004 add g_basetot1
              #NO.FUN-A10098  ---start--- 
              #tm.rtype,";",tm.dd,";",              #FUN-8B0098 add
              #tm.bb,";",tm.b_1                     #No.FUN-8C0018 #by jan add tm.b_1
               tm.rtype,";",tm.dd,";",tm.acc_code   #FUN-B90140   Add  ,";",tm.acc_code
   CALL cl_prt_cs3('aglr110','aglr110',g_sql,g_str)
  #IF tm.bb = 'N' THEN                                                  #No.FUN-8C0018
  #   CALL cl_prt_cs3('aglr110','aglr110',g_sql,g_str)
  #ELSE                                                                #No.FUN-8C0018
  #   CALL cl_prt_cs3('aglr110','aglr110_1',g_sql,g_str)               #No.FUN-8C0018
  #END IF                
  #NO.FUN-A10098  ---end---
...
#---------取报表类别                                                                                                                
   ...
   IF tm.exp = '1' THEN
      ...
      END IF
      LET l_name = "aglr110",l_dt CLIPPED,".txt"
   ...
   ELSE
    ...
    # LET sheet1 = l_ze03            #FUN-A60039
      ...
      IF g_mai03 <> '3' THEN
        #LET l_unixpath = "$TOP/tool/report/aglr110_1.xls"   #FUN-A30049
         LET l_unixpath = os.Path.join( os.Path.join( os.Path.join(FGL_GETENV("TOP"),"tool"),"report"),"aglr110_1.xls")   #FUN-A30049
         LET l_winpath  = "C:\\TIPTOP\\aglr1101",l_dt CLIPPED,".xls"
         SELECT ze03 INTO l_ze03 FROM ze_file WHERE ze01='agl-513' AND ze02=g_lang
      ELSE
        #LET l_unixpath = "$TOP/tool/report/aglr110_2.xls"   #FUN-A30049
         LET l_unixpath = os.Path.join( os.Path.join( os.Path.join(FGL_GETENV("TOP"),"tool"),"report"),"aglr110_2.xls")   #FUN-A30049
         LET l_winpath  = "C:\\TIPTOP\\aglr1102",l_dt CLIPPED,".xls"
    ...
    # LET sheet2 = l_ze03     #FUN-A60039 mark
      ...
      CALL r110_checkError(li_result,"Connect DDE Sheet1")
      CALL r110_exceldata(1,' ',' ',' ',' ',' ')
      ...
      CALL r110_checkError(li_result,"Connect DDE Sheet2")
   END IF
   CASE WHEN tm.rtype='0' LET g_msg=" 1=1"
        WHEN tm.rtype='1'
...
#NO.FUN-A10098  ----end--- 
                  ...
                  "   AND ",g_msg CLIPPED,
                  " ORDER BY maj02"
      PREPARE r110_pre1 FROM l_sql
      IF STATUS THEN        
         CALL cl_err('prepare:',STATUS,1)   
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM  
      END IF
      DECLARE r110_curs1 CURSOR FOR r110_pre1
      FOR g_i = 1 TO 100
         LET g_tot1[g_i] = 0
      END FOR
      #--MOD-A70192 start--
      ...
      FOREACH r110_curs1 INTO l_maj.*
         IF STATUS THEN
            CALL cl_err('foreach:',STATUS,1)
            EXIT FOREACH
         END IF
         #--将计算金额的部份独立出来，以供後续要计算前一年金额时可以重复CALL FUNCTION---#                                           
         #CALL r110_xbrl_bal(tm.yy,l_maj.*) RETURNING sr.bal1    #今年金额    #MOD-A70192                                                          
         #CALL r110_xbrl_bal(tm.yy-1,l_maj.*) RETURNING sr.bal2  #去年金额    #MOD-A70192

         #---MOD-A70192 start--
         LET l_amt1 = 0
         LET l_amt2 = 0
         CALL r110_xbrl_bal(tm.yy,l_maj.*) RETURNING l_amt1      #今年金額  
         CALL r110_xbrl_bal(tm.yy-1,l_maj.*) RETURNING l_amt2    #去年金額 
         ...
         IF l_maj.maj03 MATCHES "[012]" AND l_maj.maj08 > 0 THEN
            ...
            LET l_bal1= g_tot1[k]
            LET l_bal2= g_tot2[k]
            IF l_maj.maj07 = '1' AND l_maj.maj09 = '-' THEN    #MOD-B90188 add
               LET l_bal1 = l_bal1 * -1                        #MOD-B90188 add
               LET l_bal2 = l_bal2 * -1                        #MOD-B90188 add
            END IF                                             #MOD-B90188 add
        ...
        #-MOD-B40063-end-
         ...
         #CALL r110_tofile(l_r,l_name,l_maj.maj21,l_maj.maj20,sr.bal1,sr.bal2)  #FUN-A50069 mark
         #CALL r110_tofile(g_r,l_name,l_maj.maj21,l_maj.maj20,sr.bal1,sr.bal2)  #FUN-A50069 add
         #CALL r110_tofile(g_r,l_name,l_maj.maj21,l_maj.maj20,l_bal1,l_bal2)    #MOD-A70192
         CALL r110_tofile(g_r,l_name,l_maj.maj31,l_maj.maj20,l_bal1,l_bal2)     #FUN-B60077
      END FOREACH
 # END FOR              #FUN-A10098  
   IF cl_null(tm.dd) THEN
      #CALL s_azm(tm.yy,tm.em) RETURNING l_flag,l_bdate,l_edate #CHI-A70007 mark
...
FUNCTION r110_xbrl_bal(p_yy,l_maj)
...
#                    " AND aag07 MATCHES '[23]' "           #No.TQC-B30100 Mark
         ...
         PREPARE r110_pre2 FROM l_sql
         DECLARE r110_curs2  CURSOR FOR r110_pre2
         OPEN r110_curs2
         FETCH r110_curs2 INTO l_amt1
         IF SQLCA.sqlcode THEN
...
#                    " AND aag07 MATCHES '[23]' "           #No.TQC-B30100 Mark
                     " AND aag07 IN ('2','3')  "            #No.TQC-B30100 add
         PREPARE r110_pre3 FROM l_sql
         DECLARE r110_curs3  CURSOR FOR r110_pre3
         OPEN r110_curs3
         FETCH r110_curs3 INTO l_amt1
         IF SQLCA.sqlcode THEN 
      ...
      IF g_aaa09 = '2' THEN     
        ...
        #NO.FUN-A10098  ---end---
                     ...
                     " AND abb03 = aag01 ", 
                     " AND aag03 <> '4' "    
         PREPARE r110_pre4 FROM l_sql
         DECLARE r110_curs4  CURSOR FOR r110_pre4
         OPEN r110_curs4
         FETCH r110_curs4 INTO l_endy1
        #NO.FUN-A10098   -----start---
        #LET l_sql = "SELECT SUM(abb07) FROM ",l_dbs CLIPPED," abb_file,",l_dbs CLIPPED," aba_file,",l_dbs CLIPPED," aag_file ",   
         LET l_sql = "SELECT SUM(abb07) FROM abb_file,aba_file,aag_file ",
        #NO.FUN-A10098   ----end---
                     ...
                     " AND abb03 = aag01 ",  
                     " AND aag03 <> '4' "    
         PREPARE r110_pre5 FROM l_sql

         DECLARE r110_curs5  CURSOR FOR r110_pre5
         OPEN r110_curs5
         FETCH r110_curs5 INTO l_endy2
         IF l_endy1 IS NULL THEN LET l_endy1 = 0 END IF
         IF l_endy2 IS NULL THEN LET l_endy2 = 0 END IF
...
#                    " AND aag07 MATCHES '[23]' "           #No.TQC-B30100 Mark
                     " AND aag07 IN ('2','3')  "            #No.TQC-B30100 add
         PREPARE r110_pre6 FROM l_sql                                         
         DECLARE r110_curs6  CURSOR FOR r110_pre6
         OPEN r110_curs6                      
         FETCH r110_curs6 INTO l_tmp
         IF STATUS THEN 
...
#FUNCTION r110_tofile(p_r,p_name,p_maj21,p_maj20,p_bal1,p_bal2)  #FUN-B60077 mark
FUNCTION r110_tofile(p_r,p_name,p_maj31,p_maj20,p_bal1,p_bal2)   #FUN-B60077
...
DEFINE l_azn05      LIKE azn_file.azn05
   ...
   IF tm.exp = '1' THEN           #TXT
     #CALL r110_totxt(p_maj21,p_maj20,p_bal1,p_bal2)   #FUN-B60077 mark
      CALL r110_totxt(p_maj31,p_maj20,p_bal1,p_bal2)   #FUN-B60077
   ELSE
     #CALL r110_toexcel('2',p_r,p_maj21,p_maj20,p_bal1,p_bal2)   #FUN-B60077 mark
      CALL r110_toexcel('2',p_r,p_maj31,p_maj20,p_bal1,p_bal2)   #FUN-B60077
   END IF
END FUNCTION

#FUNCTION r110_totxt(p_maj21,p_maj20,p_bal1,p_bal2)     #FUN-B60077 mark
#DEFINE p_maj21    LIKE maj_file.maj21                  #FUN-B60077 mark
FUNCTION r110_totxt(p_maj31,p_maj20,p_bal1,p_bal2)      #FUN-B60077
DEFINE p_maj31    LIKE maj_file.maj31                   #FUN-B60077
DEFINE p_maj20    LIKE maj_file.maj20
DEFINE p_bal1     LIKE aah_file.aah04
DEFINE p_bal2     LIKE aah_file.aah04
...
#FUNCTION r110_toexcel(p_s,p_r,p_maj21,p_maj20,p_bal1,p_bal2)  #FUN-B60077 mark
FUNCTION r110_toexcel(p_s,p_r,p_maj31,p_maj20,p_bal1,p_bal2)   #FUN-B60077
...
DEFINE li_result  LIKE type_file.num5
  ...
  #CALL r110_exceldata(p_s,p_r,p_maj21,p_maj20,p_bal1,p_bal2)   #FUN-B60077 mark
   CALL r110_exceldata(p_s,p_r,p_maj31,p_maj20,p_bal1,p_bal2)   #FUN-B60077
   
END FUNCTION

FUNCTION r110_lastday()
...
#FUNCTION r110_exceldata(p_s,p_r,p_maj21,p_maj20,p_bal1,p_bal2)   #FUN-B60077 mark
FUNCTION r110_exceldata(p_s,p_r,p_maj31,p_maj20,p_bal1,p_bal2)    #FUN-B60077
...
#公司代号
     ...
     #LET l_str = l_ze03             #FUN-A60039 mark
      ...
      CALL r110_checkError(li_result,"DDEPoke Sheet1 R1C1")
      LET l_str = g_zo16
      LET ls_cell = "R1C2"
      CALL ui.Interface.frontCall("WINDDE","DDEPoke",[g_cmd,sheet1,ls_cell,l_str],[li_result])
      CALL r110_checkError(li_result,"DDEPoke Sheet1 R1C2")
#年度
      SELECT ze03 INTO l_ze03 
        FROM ze_file 
       WHERE ze01 = 'agl-506' 
     ...
     #LET l_str = l_ze03         #FUN-A60039
      ...
      LET ls_cell = "R2C1"
      CALL ui.Interface.frontCall("WINDDE","DDEPoke",[g_cmd,sheet1,ls_cell,l_str],[li_result])
      CALL r110_checkError(li_result,"DDEPoke Sheet1 R2C1")
      LET l_str = g_yy
      LET ls_cell = "R2C2"
      CALL ui.Interface.frontCall("WINDDE","DDEPoke",[g_cmd,sheet1,ls_cell,l_str],[li_result])
      CALL r110_checkError(li_result,"DDEPoke Sheet1 R2C2")
#季别
      SELECT ze03 INTO l_ze03 
        FROM ze_file 
       WHERE ze01 = 'agl-507' 
     ...
     #LET l_str = l_ze03       #FUN-A60039
      ...
      LET ls_cell = "R3C1"
      CALL ui.Interface.frontCall("WINDDE","DDEPoke",[g_cmd,sheet1,ls_cell,l_str],[li_result])
      CALL r110_checkError(li_result,"DDEPoke Sheet1 R3C1")
       LET l_str = g_ss
      LET ls_cell = "R3C2"
      CALL ui.Interface.frontCall("WINDDE","DDEPoke",[g_cmd,sheet1,ls_cell,l_str],[li_result])
      CALL r110_checkError(li_result,"DDEPoke Sheet1 R3C2")
...
#科目代号
       ...
       # LET l_str = l_ze03       #FUN-A60039
         ...
         CALL r110_checkError(li_result,"DDEPoke Sheet2 R1C1")
#科目名称
         SELECT ze03 INTO l_ze03 
           FROM ze_file 
          WHERE ze01 = 'agl-509' 
       ...
       # LET l_str = l_ze03          #FUN-A60039
         ...
         CALL r110_checkError(li_result,"DDEPoke Sheet2 R1C2")
#今年数值
         SELECT ze03 INTO l_ze03 
           FROM ze_file 
          WHERE ze01 = 'agl-510'
       ...
       # LET l_str = l_ze03           #FUN-A60039 
         ...
         CALL r110_checkError(li_result,"DDEPoke Sheet2 R1C3")
#去年数值
         SELECT ze03 INTO l_ze03 
           FROM ze_file 
          WHERE ze01 = 'agl-511' 
       ...
       # LET l_str = l_ze03          #FUN-A60039
         ...
         CALL r110_checkError(li_result,"DDEPoke Sheet2 R1C4")
         LET g_r = g_r + 1    #FUN-A50069 add
               ...
               WHEN 1
                 ...
                 #   LET l_str = p_maj21         #FUN-B60077 mark
                  IF NOT cl_null(p_maj31) THEN   #FUN-B60077
                     ...
                     CALL r110_checkError(li_result,"DDEPoke Sheet2 ")
                  END IF
               WHEN 2
...
#   END IF       #FUN-A50069 mark
END FUNCTION
FUNCTION r110_checkError(p_result,p_msg)
