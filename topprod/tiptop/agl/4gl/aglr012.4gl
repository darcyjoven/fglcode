...
# Modify.........: No.MOD-C10038 12/01/08 By Polly CALL s_get_aznn01屬於OTHERWISE時才需要使用
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglr012',g_sql) CLIPPED  # 產生Temp Table
...
#  LET tm.bm1 = ARG_VAL(13) #FUN-A90032
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL r012_tm()                # Input print condition
      ELSE CALL r012()            # Read data and create out-file
   END IF
...
FUNCTION r012_tm()
   ...
   END IF
   OPEN WINDOW r012_w AT p_row,p_col
        WITH FORM "agl/42f/aglr012" 
################################################################################
...
#                 tm.title2,tm.yy2,tm.bm2,tm.em2, #FUN-A90032
       ...
       BEFORE INPUT
          CALL cl_qbe_init()
          CALL r012_set_entry()       #FUN-A90032
          CALL r012_set_no_entry()    #FUN-A90032
       ON ACTION locale
...
#FUN-A30122 --End
      ...
      AFTER FIELD aaz641
       # LET g_sql ="SELECT COUNT(*) FROM ",g_dbs_axz03,"aaz_file",    #FUN-A30122 mark by vealxu
         ...
         PREPARE r012_cnt_p FROM g_sql
         DECLARE r012_cnt_c CURSOR FOR r012_cnt_p
         OPEN r012_cnt_c
         FETCH r012_cnt_c INTO l_cnt
...
################################################################################
   ...
   END INPUT
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW r012_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_template CLIPPED,"'",         
                         " '",g_rpt_name CLIPPED,"'"         
         CALL cl_cmdat('aglr012',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW r012_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM
...
FUNCTION r012()
   ...
   LET l_sql = "SELECT * FROM maj_file",
               ...
               "   AND ",g_msg CLIPPED,
               " ORDER BY maj02"
   PREPARE r012_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM 
   END IF
   DECLARE r012_c CURSOR FOR r012_p
...
#                 "   AND axh06 = '",tm.yy2,"' AND axh07 BETWEEN '",tm.bm2,"' AND '",tm.em2,"'", #FUN-A90032
   ...
   CALL cl_prt_cs3('aglr012','aglr012',l_sql,g_str)   


END FUNCTION

FUNCTION r012_set_entry() 
#FUN-A90032 --Begin
...
#              " WHERE aaz00 = '0'"
#  PREPARE r012_pre_11 FROM g_sql
#  DECLARE r012_cur_11 CURSOR FOR r012_pre_11
#  OPEN r012_cur_11
#  FETCH r012_cur_11 INTO g_aaz641
#END FUNCTION	　
#FUN-A30122 --End
#FUN-9A0036  
