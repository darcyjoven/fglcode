...
#No.FUN-830100  --BEGIN                                                                                                             
   ...
   LET l_table = cl_prt_temptable("aglr930",g_sql) CLIPPED                                                                          
...
DEFINE l_azfacti     LIKE azf_file.azfacti     #FUN-AB0020  
   ...
   OPEN WINDOW r930_w AT p_row,p_col
     WITH FORM "agl/42f/aglr930"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN LET INT_FLAG = 0 CLOSE WINDOW r930_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM 
   END IF
...
#                        " '",tm.h CLIPPED,"'",                 #No.FUN-830139 
         ...
         CALL cl_cmdat('aglr930',g_time,l_cmd)
      END IF
      CLOSE WINDOW r930_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
...
#                "   AND afc04 = '",tm.dept CLIPPED,"'",   #MOD-570228     #No.TQC-840049
                 "   AND afc041= '",tm.dept CLIPPED,"'",   #No.TQC-840049
                 "   AND afc05 != 0",
...
#FUN-AB0020------add---------str------------------
     ...
     END IF     
#FUN-AB0020------add---------end------------------
     PREPARE r930_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
        EXIT PROGRAM
     END IF
     DECLARE r930_curs1 CURSOR FOR r930_prepare1
#    LET l_name = 'aglr930.out'
#No.FUN-830100  --BEGIN                                                                                                             
{               
     CALL cl_outnam('aglr930') RETURNING l_name
     ...
     CALL cl_prt_pos_len()
     #FUN-6C0012.....end
     START REPORT r930_rep TO l_name
     LET g_pageno = 0
}                                                                                                                                   
#No.FUN-830100  --END 
     FOREACH r930_curs1 INTO sr.*
          IF SQLCA.sqlcode != 0 THEN
     ...
     IF g_zz05='Y' THEN                                                                                                             
        ...
        LET g_str=g_wc                                                                                                              
     END IF                                                                                                                         
     LET g_str=g_str CLIPPED,';',tm.s[1,1],';',tm.s[2,2],';',tm.azh02,';',                                                          
                                 tm.t[1,1],';',tm.t[2,2],';',tm.yy,';',tm.k,';',                                                    
                                 tm.dept,';',g_base,';',tm.detail                                                                   
     LET g_sql="SELECT * FROM ", g_cr_db_str CLIPPED,l_table CLIPPED                                                                
     CALL cl_prt_cs3('aglr930','aglr930',g_sql,g_str)                                                                               
     #No.FUN-830100  --end                              
END FUNCTION
