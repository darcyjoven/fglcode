...
# Modify.........: No:FUN-C30085 12/07/04 By lixiang CR報表改成GR報表
# Modify.........: No:MOD-D30008 13/03/04 By apo 條件增加按照type排序
...
MAIN
   ...
   IF g_flag = 'N' THEN
      ...
      IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
         THEN CALL g940_tm()         # Input print condition
         ELSE CALL g940()            # Read data and create out-file
      END IF
   ELSE
      CALL g940()                    # Read data and create out-file
   END IF
...
FUNCTION g940_tm()
   ...
   END IF
   OPEN WINDOW g940_w AT p_row,p_col WITH FORM "agl/42f/aglg940"
   ...
   LET l_table = cl_prt_temptable('aglg940',g_sql) CLIPPED
   IF l_table = -1 THEN 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(l_table)    
      EXIT PROGRAM 
   ...
   WHILE TRUE
     ...
     #              tm.b,tm.e,tm.d,tm.c,tm.s,tm.t,        #FUN-C30098 mark
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW g940_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)     
         EXIT PROGRAM
      END IF
...
FUNCTION g940()
   ...
   SELECT aaf03 INTO g_company FROM aaf_file
    ...
    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aglg940'

   #IF g_len = 0 OR g_len IS NULL THEN LET g_len = 132 END IF   #FUN-C30085
   #FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR      #FUN-C30085
   #CALL cl_outnam('aglg940') RETURNING l_name                  #FUN-C30085
...
FUNCTION g940_cr()
  ...
  #---------------------本期淨利(損)------------------------------
   ...
   CALL g940_aah(g_aaa15,tm.y1,tm.m2) RETURNING l_amt
   IF cl_null(l_amt) THEN LET l_amt = 0 END IF
   CALL g940_aah(g_aaa15,tm.y11,tm.m2) RETURNING l_amt_1
  ...
  #FUN-C30098--
   ...
   ELSE
      CALL g940_str(l_amt,l_str1) RETURNING l_str1
      LET l_str1 = l_str1 CLIPPED,')'
   END IF
   IF l_amt_1 >= 0 THEN
      LET l_str3 = cl_numfor(l_amt_1,20,tm.e)
   ELSE
      CALL g940_str(l_amt_1,l_str3) RETURNING l_str3
   ...
   PREPARE g940_giq_p1 FROM l_sql
   DECLARE g940_giq_c1 CURSOR FOR g940_giq_p1
   ...
   LET l_sql = "SELECT aag01,aag02,giw03,giw04 "
              ...
              ,"   AND giw01 = ?"
   PREPARE g940_giw_p1 FROM l_sql
   DECLARE g940_giw_c1 CURSOR FOR g940_giw_p1
   FOREACH g940_giq_c1 INTO giq.*
      LET l_amt_s = 0
      LET l_amt_s_1 = 0
      LET l_i_1 = 1
      FOREACH g940_giw_c1 USING giq.giq01 INTO tmp.*
         LET l_amt = 0
         LET l_amt_1 = 0
         CASE tmp.giw04  
           WHEN '1'
              CALL g940_aah(tmp.aag01,tm.y2,tm.m2) RETURNING l_amt
              CALL g940_aah(tmp.aag01,tm.y21,tm.m2) RETURNING l_amt_1
           WHEN '2'
              CALL g940_aah2(tmp.aag01,l_last_y,l_last_m) RETURNING l_amt
              CALL g940_aah2(tmp.aag01,l_last_y1,l_last_m) RETURNING l_amt_1
           WHEN '3'
              CALL g940_aah2(tmp.aag01,tm.y2,tm.m2) RETURNING l_amt
              CALL g940_aah2(tmp.aag01,tm.y21,tm.m2) RETURNING l_amt_1
           WHEN '4'
              ...
              SELECT SUM(gix05) INTO l_amt FROM gix_file,giq_file
               WHERE gix01 = giq.giq01 AND gix02 = tmp.aag01
                 AND gix06 = tm.y1 AND gix07 BETWEEN tm.m1 AND tm.m2
                 AND gix01 = giq01
              LET l_gix[l_i_1].gix05=l_amt
              SELECT SUM(gix05) INTO l_amt_1 FROM gix_file,giq_file
               WHERE gix01 = giq.giq01 AND gix02 = tmp.aag01
                 AND gix06 = tm.y11 AND gix07 BETWEEN tm.m1 AND tm.m2
                 AND gix01 = giq01
     ...
     #FUN-C30098--
      ...
      ELSE
         CALL g940_str(l_amt_s,l_str2) RETURNING l_str2
         LET l_str2 = l_str2 CLIPPED,')'
      END IF
      IF l_amt_s_1 >= 0 THEN
         LET l_str4 = cl_numfor(l_amt_s_1,20,tm.e)
      ELSE
         CALL g940_str(l_amt_s_1,l_str4) RETURNING l_str4
         LET l_str4 = l_str4 CLIPPED,')'
      END IF
      LET l_flag = 'Y'
      IF tm.t = 'N' THEN 
         INSERT INTO g940_tmp VALUES (l_type,l_str1,giq.giq02,l_str2,giq.giq04,l_str3,l_str4)
      ELSE 
      	 FOR l_i_2 = 1 TO l_i_1
            ...
            ELSE
               CALL g940_str(l_gix[l_i_2].gix05,l_str2) RETURNING l_str2
               LET l_str2 = l_str2 CLIPPED,')'
            END IF
            IF l_gix[l_i_2].gix051 >= 0 THEN
               LET l_str4 = cl_numfor(l_gix[l_i_2].gix051,20,tm.e)
            ELSE
               CALL g940_str(l_gix[l_i_2].gix051,l_str4) RETURNING l_str4
               LET l_str4 = l_str4 CLIPPED,')'
            END IF
            INSERT INTO g940_tmp VALUES (l_type,l_str1,l_gix[l_i_2].giq02,l_str2,giq.giq04,l_str3,l_str4)
      	 END FOR     
      END IF
   END FOREACH
   IF l_flag = 'N' THEN
      INSERT INTO g940_tmp VALUES (l_type,l_str1,'','',l_giq04,l_str3,'')
   END IF
  ...
  #FUN-C30098--
   ...
   ELSE
      CALL g940_str(l_sub_amt,l_str1) RETURNING l_str1
      LET l_str1 = l_str1 CLIPPED,')'
   END IF
   IF l_sub_amt_1 >= 0 THEN
      LET l_str3 = cl_numfor(l_sub_amt_1,20,tm.e)
   ELSE
      CALL g940_str(l_sub_amt_1,l_str3) RETURNING l_str3
      LET l_str3 = l_str3 CLIPPED,')'
   END IF
   INSERT INTO g940_tmp VALUES (l_type,l_str1,'','',l_giq04,l_str3,'')
   LET l_sub_amt = 0
   ...
   LET l_sql = "SELECT giq01,giq02,giq04 FROM giq_file "
              ," WHERE giq03 = '2'"
              ," ORDER BY giq04,giq02"
   PREPARE g940_giq_p2 FROM l_sql
   DECLARE g940_giq_c2 CURSOR FOR g940_giq_p2
   LET l_sql = "SELECT aag01,aag02,giw03,giw04 "
              ,"  FROM aag_file,giw_file,giq_file"
              ," WHERE aag01=giw02 AND giw01=giq01 "
              ,"   AND aag00=giw00 AND aag00 = '",tm.b,"' "
              ,"   AND giw01 = ?" 
   PREPARE g940_giw_p2 FROM l_sql
   DECLARE g940_giw_c2 CURSOR FOR g940_giw_p2

   FOREACH g940_giq_c2 INTO giq.*
      LET l_amt_s = 0
      LET l_amt_s_1 = 0
      LET l_i_1 = 1
      FOREACH g940_giw_c2 USING giq.giq01 INTO tmp.*
        LET l_amt = 0
        LET l_amt_1 = 0
        CASE tmp.giw04
          WHEN '1'
     ...
     #FUN-C30098--
      ...
      LET l_flag = 'Y'
      IF tm.t = 'N' THEN 
         INSERT INTO g940_tmp VALUES (l_type,'',giq.giq02,l_str2,giq.giq04,'',l_str4)
      ELSE 
      	 FOR l_i_2 = 1 TO l_i_1
            IF cl_null(l_gix[l_i_2].gix05) THEN
               LET l_gix[l_i_2].gix05 = 0
            ...
            END IF
     	    IF l_gix[l_i_2].gix051 >= 0 THEN
               LET l_str4 = cl_numfor(l_gix[l_i_2].gix051,20,tm.e)
            ELSE
               CALL g940_str(l_gix[l_i_2].gix051,l_str2) RETURNING l_str4
               LET l_str4 = l_str4 CLIPPED,')'
            END IF
      	    INSERT INTO g940_tmp VALUES (l_type,'',l_gix[l_i_2].giq02,l_str2,giq.giq04,'',l_str4)
     	 END FOR     
      END IF
   END FOREACH
   IF l_flag = 'N' THEN
      INSERT INTO g940_tmp VALUES (l_type,'','','',l_giq04,'','')
   END IF
  ...
  #FUN-C30098--
   ...
   PREPARE g940_giq_p3 FROM l_sql
   DECLARE g940_giq_c3 CURSOR FOR g940_giq_p3
   LET l_sql = "SELECT aag01,aag02,giw03,giw04 "
              ," FROM aag_file,giw_file,giq_file"
              ," WHERE aag01=giw02 AND giw01=giq01 "
              ,"   AND aag00=giw00 AND aag00 = '",tm.b,"' "
              ,"   AND giw01 = ?"
   PREPARE g940_giw_p3 FROM l_sql
   DECLARE g940_giw_c3 CURSOR FOR g940_giw_p3
   FOREACH g940_giq_c3 INTO giq.*
      LET l_amt_s = 0
      LET l_amt_s_1 = 0
      LET l_i_1 = 1
      FOREACH g940_giw_c3 USING giq.giq01 INTO tmp.*
        LET l_amt = 0
        LET l_amt_1 = 0
        CASE tmp.giw04  
          WHEN '1'
  ...
  #FUN-C30098--
   ...
   IF l_amt_1 >=0 THEN
      LET l_str3 = cl_numfor(l_amt_1,20,tm.e)
   ELSE
      CALL g940_str(l_amt_1,l_str3) RETURNING l_str3  #MOD-C60050 l_amt -> l_amt_1
      LET l_str3 = l_str3 CLIPPED,')'
   END IF
   INSERT INTO g940_tmp VALUES (l_type,l_str1,'','',l_giq04,l_str3,'')
  ...
  #FUN-C30098--Mark--
   ...
   LET l_sql = "SELECT giq01,giq02,giq04 FROM giq_file "
              ," WHERE giq03 = '4'"
              ," ORDER BY giq04,giq04"
   PREPARE g940_giq_p4 FROM l_sql
   DECLARE g940_giq_c4 CURSOR FOR g940_giq_p4
   LET l_sql = "SELECT aag01,aag02,giw03,giw04 "
              ,"  FROM aag_file,giw_file,giq_file"
              ," WHERE aag01=giw02 AND giq01=giw01 "
              ,"   AND aag00=giw00 AND aag00 = '",tm.b,"' "
              ,"   AND giw01 = ?"
   PREPARE g940_giw_p4 FROM l_sql
   DECLARE g940_giw_c4 CURSOR FOR g940_giw_p4
   FOREACH g940_giq_c4 INTO giq.*
      LET l_amt_s = 0
      LET l_amt_s_1 = 0
      LET l_i_1 = 1
      FOREACH g940_giw_c4 USING giq.giq01 INTO tmp.*
         LET l_amt = 0
         LET l_amt_1 = 0
         CASE tmp.giw04
           WHEN '1'
   ...
   IF tm.s = 'Y' THEN
      ...
      FOREACH g940_giy INTO g_giy[g_cnt].*
         IF SQLCA.sqlcode THEN
      ...
      FOR i = 1 TO g_cnt
         ...
         LET l_str4= cl_numfor(g_giy[i].giy03,20,tm.e)
         LET l_flag = 'Y'
         INSERT INTO g940_tmp VALUES (l_type,'',g_giy[i].giy02,l_str2,g_giy[i].giy01,'',l_str4)
      END FOR
      IF l_flag = 'N' THEN
         INSERT INTO g940_tmp VALUES (l_type,'','','',l_giq04,'','')
      END IF
   END IF
   IF l_last = 0 THEN
      SELECT SUM(str2) INTO l_last  FROM g940_tmp WHERE l_type ='8'
   END IF
   IF l_last_1 = 0 THEN
      SELECT SUM(str4) INTO l_last_1  FROM g940_tmp WHERE l_type ='8'
   END IF
   IF cl_null(l_last) THEN LET l_last = 0 END IF
   IF cl_null(l_last_1) THEN LET l_last_1 = 0 END IF
   LET l_flag = 'N'

   LET l_sql = " SELECT * FROM g940_tmp "
              ," ORDER BY type,giq04,giq02 "
   PREPARE g940_p1 FROM l_sql
   DECLARE g940_c1 CURSOR FOR g940_p1
   FOREACH g940_c1 INTO tmp1.*
      CASE tmp1.type
         WHEN '8'
            IF l_last >=0 THEN
               LET tmp1.str1 = cl_numfor(l_last,20,tm.e)
            ELSE
               CALL g940_str(l_last,tmp1.str1) RETURNING tmp1.str1
               LET tmp1.str1 = tmp1.str1 CLIPPED,')'                   #FUN-C40004 l_str-->tmp1.str1
            END IF
            IF l_last_1 >=0 THEN
               LET tmp1.str3 = cl_numfor(l_last_1,20,tm.e)
            ELSE
               CALL g940_str(l_last_1,tmp1.str3) RETURNING tmp1.str3 
         ...
         WHEN '9'
            ...
            ELSE
               CALL g940_str(l_amt,tmp1.str1) RETURNING tmp1.str1
               LET tmp1.str1 = tmp1.str1 CLIPPED,')'                    #FUN-C40004 l_str-->tmp1.str1
            END IF
            IF l_amt_1 >=0 THEN
               LET tmp1.str3 = cl_numfor(l_amt_1,20,tm.e)
            ELSE
               CALL g940_str(l_amt_1,tmp1.str3) RETURNING tmp1.str3 
               LET tmp1.str3 = tmp1.str3 CLIPPED,')'                    #FUN-C40004 l_str-->tmp1.str3
            END IF
      END CASE
      IF (tmp1.type = '8') THEN
...
###GENGRE###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED," ORDER BY type,giq04"                     #FUN-C30098 mod 
   LET g_str = ''
###GENGRE###   LET g_str = tm.d,";",tm.title,";",tm.p,";","",";",tm.y1,";",tm.m1,";",
###GENGRE###               l_d1,";",tm.y2,";",tm.m2,";",l_d2
###GENGRE###   CALL cl_prt_cs3('aglg940','aglg940',g_sql,g_str)
    CALL aglg940_grdata()    ###GENGRE###
        
END FUNCTION

FUNCTION g940_aah(p_aah01,p_aah02,p_aah03)
...
FUNCTION g940_aah2(p_aah01,p_aah02,p_aah03)
   DEFINE p_aah01     LIKE aah_file.aah01,
          p_aah02     LIKE aah_file.aah02,
          p_aah03     LIKE aah_file.aah03,
          p_value     LIKE aah_file.aah04   
...
FUNCTION g940_numfor(p_value,p_n)
...
FUNCTION g940_create_temp_table()
   DROP TABLE g940_tmp
   CREATE TEMP TABLE g940_tmp(
   type   LIKE type_file.chr1,
   str1   LIKE type_file.chr50,
   giq02  LIKE type_file.chr1000,
   str2   LIKE type_file.chr50,
...
FUNCTION aglg940_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg940")
        IF handler IS NOT NULL THEN
            START REPORT aglg940_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY type"   #MOD-D30008
            DECLARE aglg940_datacur1 CURSOR FROM l_sql
            FOREACH aglg940_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg940_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg940_rep
        END IF
...
REPORT aglg940_rep(sr1)
 ...
 #FUN-C30085--add--end--
    ...
    FORMAT
        FIRST PAGE HEADER
            LET l_str1 = NULL    #FUN-C30085 add
            LET l_str2 = NULL    #FUN-C30085 add
            LET l_str3 = NULL    #FUN-C30085 add
            LET l_str4 = NULL    #FUN-C30085 add
