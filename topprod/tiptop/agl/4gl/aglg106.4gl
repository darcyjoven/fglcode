...
# Modify.........: No.FUN-B80161 11/08/25 By chenying 明細類報表轉GR
...
DEFINE g_cnt1    LIKE type_file.num10    #No.FUN-B80161
...
#No.FUN-760083   --begin--
   ...
   LET l_table=cl_prt_temptable("aglg106",g_sql) CLIPPED
   IF l_table=-1 THEN
...
DEFINE li_chk_bookno      LIKE type_file.num5      #FUN-B20054
   ...
   OPEN WINDOW aglg106_w AT p_row,p_col
     WITH FORM "agl/42f/aglg106"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
   ...
   #FUN-B20054--add--end
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglg106',g_time,l_cmd)      # Execute cmd at later time
         END IF
...
#  LET l_sql = l_sql," GROUP BY aag01,aag02,aag13,aao02"   #FUN-6C0012 #No.MOD-930094
   ...
   PREPARE aglg106_prepare1 FROM l_sql
   IF STATUS THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE aglg106_curs1 CURSOR FOR aglg106_prepare1
  ...
  #No.MOD-920135--begin--
   LET l_sql = " SELECT SUM(aao05-aao06) FROM aao_file ",
               ...
               "    AND aao01 = ? AND aao02 = ? " 
   PREPARE aglg106_prepare2 FROM l_sql
   IF STATUS THEN
      CALL cl_err('prepare:',STATUS,1)
      ...
      EXIT PROGRAM
   END IF
   DECLARE aglg106_curs2 CURSOR FOR aglg106_prepare2
  #No.MOD-920135---end---
 
#No.FUN-760083  --begin--
   #CALL cl_outnam('aglg106') RETURNING l_name               
   ...
   #START REPORT aglg106_rep TO l_name   
   LET g_str = ''                        
   CALL cl_del_data(l_table)             
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01=g_prog
   IF tm.e='Y' THEN 
#     LET l_name='aglg106_1'         #FUN-B80161 mark  
      LET g_template = 'aglg106_1'   #FUN-B80161 add
   ELSE 
#     LET l_name='aglg106'
      LET g_template = 'aglg106'     #FUN-B80161 add
   END IF
   ...
   FOREACH aglg106_curs1 INTO sr.*
     ...
     #No.MOD-920135--begin--
      OPEN aglg106_curs2 USING sr.aag01,sr.aao02
      FETCH aglg106_curs2 INTO sr.aao05
      CLOSE aglg106_curs2
     #No.MOD-920135---end---
      ...
      #OUTPUT TO REPORT aglg106_rep(sr.*)    #No.FUN-760083 
  ...
  #END IF
   ...
   #FINISH REPORT aglg106_rep                #No.FUN-760083 
...
###GENGRE###   LET g_str=tm.wc,';',g_tot,';',tm.e,';',tm.yy,';',tm.mm,';',t_azi04,';',t_azi05,';',tm.f             #MOD-AC0168 mod m2 -> mm 
###GENGRE###   LET g_sql="SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED        
###GENGRE###   CALL cl_prt_cs3("aglg106",l_name,g_sql,g_str)     
    CALL aglg106_grdata()    ###GENGRE###
#No.FUN-760083 --end--
                   
END FUNCTION
#No.FUN-760083 --begin--
{
REPORT aglg106_rep(sr)
...
FUNCTION aglg106_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg106")
        IF handler IS NOT NULL THEN
            START REPORT aglg106_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY aag01" #FUN-B80161
          
            DECLARE aglg106_datacur1 CURSOR FROM l_sql
            FOREACH aglg106_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg106_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg106_rep
        END IF
...
REPORT aglg106_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_aao05_1_subtot LIKE aao_file.aao05
    #FUN-B80161---add-------str----------
    DEFINE l_head1  STRING             
    DEFINE l_per    LIKE type_file.num20_6
    DEFINE l_amt1   LIKE aao_file.aao05  
    DEFINE l_dc     LIKE type_file.chr2 
    DEFINE l_amt2   LIKE aao_file.aao05  
    DEFINE l_dc1    LIKE type_file.chr2
    DEFINE l_dc2    LIKE type_file.chr2
    DEFINE l_peramt1 LIKE type_file.num20_6 
    DEFINE l_peramt2 LIKE type_file.num20_6 
    DEFINE l_pertot  LIKE type_file.num20_6 
    DEFINE l_amt_tot LIKE aao_file.aao05  
    DEFINE l_fmt     STRING
    #FUN-B80161---add-------str----------

    ORDER EXTERNAL BY sr1.order1,sr1.aag01,sr1.aao02
    
    FORMAT
        FIRST PAGE HEADER
            LET l_head1 = cl_gr_getmsg("gre-113",g_lang,'1'),':',tm.yy,' ',cl_gr_getmsg("gre-113",g_lang,'2'),':',tm.mm  #FUN-B80161
            PRINTX l_head1   #FUN-B80161 
        ...
        ON EVERY ROW
            ...
            IF g_tot = 0 THEN
                LET l_per = 0   
            ELSE
                LET l_per = (sr1.aao05 / g_tot) * 100   
            END IF
            PRINTX l_per

            LET l_fmt = cl_gr_numfmt("aao_file","aao05",t_azi04)
            PRINTX l_fmt
            #FUN-B80161--add-----end---------  
            PRINTX sr1.*
        ...
        AFTER GROUP OF sr1.order1
            #FUN-B80161---add------str-----   
            LET g_cnt1 = GROUP COUNT(*) 
            IF tm.f > 0 AND g_cnt1 > 1 THEN 
               LET l_amt2 = GROUP SUM(sr1.aao05) 
               IF l_amt2 >= 0 THEN 
                  LET l_dc1 = 'D'
               ELSE
                  LET l_dc1 = 'C'
        ...
        AFTER GROUP OF sr1.aag01
            ...
            LET g_cnt = GROUP COUNT(*)
            IF g_cnt > 1 THEN       
               LET l_amt1 = GROUP SUM(sr1.aao05)
               IF l_amt1 >= 0 THEN
                  LET l_dc = 'D'
               ELSE
                  LET l_dc = 'C'
