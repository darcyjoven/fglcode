...
# Modify.........: No.FUN-B90140 11/10/08 By zhangweib 增加checkbox"列印會計科目"
# Modify.........: NO.TQC-AA0098 11/10/14 by Yiting 開始報表代號時，應使用合併帳別
# Modify.........: NO.FUN-BA0099 11/10/26 By lujh  在空行的部分，把maj.maj31换成''
...
MAIN
  ...
  #LET g_sql = "maj20.maj_file.maj20,maj20e.maj_file.maj20e,",                     #FUN-B90140   Mark
   LET g_sql = "maj31.maj_file.maj31,maj20.maj_file.maj20,maj20e.maj_file.maj20e,",      #FUN-B90140   Add
   ...
   LET l_table = cl_prt_temptable('aglr003',g_sql) CLIPPED # 產生TEMP TABLE
   IF l_table = -1 THEN EXIT PROGRAM END IF
   LET g_sql="INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,      # TQC-780054
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?)"   #TQC-830012 add ?   #MOD-860109 add ? #CHI-A70061 add ?   #FUN-B90140   Add ?
...
#  LET tm.bm    = ARG_VAL(15)   #FUN-A90032
 ...
 # LET g_rpt_name = ARG_VAL(29)  #No.FUN-7C0078            #FUN-A50076 mark
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL r003_tm()                # Input print condition
   ELSE
      IF tm.xbrl = 'N' THEN   #FUN-A50076
         CALL r003()            # Read data and create out-file
      ELSE                    #FUN-A50076
         CALL r003_xbrl()     #FUN-A50076
      END IF                  #FUN-A50076
   END IF
...
FUNCTION r003_tm()
   ...
   OPEN WINDOW r003_w AT p_row,p_col WITH FORM "agl/42f/aglr003" 
...
#                 tm.a,tm.b,tm.yy,tm.bm,tm.em,          #FUN-A90032
                  ...
                  tm.e,tm.f,tm.d,tm.acc_code,tm.c,tm.h,tm.xbrl,tm.exp,tm.o,tm.r,     #FUN-A50076 add tm.xbrl,tm.exp #FUN-B90140   Add tm.acc_code
...
#         ELSE
#            #抓取上層公司所屬合併帳別
#            CALL r003_get_bookno() RETURNING tm.b
       ...
       ON CHANGE a                #FUN-A50076 add
         ...
         #LET g_sql = "SELECT mai02,mai03 FROM ",l_dbs_axz03,"mai_file",  #FUN-A50102
          LET g_sql = "SELECT mai02,mai03 FROM ",cl_get_target_table(g_plant_new,'mai_file'), #FUN-A50102
                      ...
                      "   AND mai00 = '",tm.b,"'",          #MOD-A40008  #TQC-AA0098
                      "   AND maiacti IN ('Y','y')"
          CALL cl_replace_sqldb(g_sql) RETURNING g_sql   #FUN-A50102
          CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql   #FUN-A50102
          PREPARE r003_pre FROM g_sql
          DECLARE r003_cur CURSOR FOR r003_pre
          OPEN r003_cur
          FETCH r003_cur INTO g_mai02,g_mai03
          IF STATUS THEN 
...
#          IF tm.bm > tm.em THEN CALL cl_err('','9011',0) NEXT FIELD bm END IF   #FUN-A90032 mark
       ...
       ON CHANGE xbrl
           ...
           CALL r003_set_entry_1()
           CALL r003_set_no_entry_1()          
       #FUN-A50076 -----------------add end------------------------------ 
...
#FUN-A90032 --End mark          
       ...
       ON ACTION CONTROLP
          CASE
             ...
             WHEN INFIELD(a) 
               ...
               #LET g_qryparam.where = "mai00 = '",tm.axa03,"'"      #MOD-A40008 
               #LET g_qryparam.where = "mai00 = '",tm.b,"'"      #MOD-A40008   #TQC-AA0098   #No.TQC-C50042   Mark
    ...
    IF INT_FLAG THEN
       LET INT_FLAG = 0 CLOSE WINDOW r003_w 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
       EXIT PROGRAM
    END IF
    IF g_bgjob = 'Y' THEN
...
#                         " '",tm.bm CLIPPED,"'",    #FUN-A90032 mark
          ...
          CALL cl_cmdat('aglr003',g_time,l_cmd)    # Execute cmd at later time
       END IF
       CLOSE WINDOW r003_w
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
       EXIT PROGRAM
...
FUNCTION r003()
  ...
  #DEFINE l_sql2    STRING                    #MOD-A50167 add        #MOD-A70189 mark
   ...
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      ...
      EXIT PROGRAM 
   END IF
   DECLARE r003_c CURSOR FOR r003_p
  ...
  #PREPARE r003_p1 FROM l_sql2
  #EXECUTE r003_p1 INTO l_maj11
  #MOD-A50167---add---end--- 
  #-MOD-A70189-end-
   FOREACH r003_c INTO maj.*
...
#                   "   AND axh07 BETWEEN ",tm.bm," AND ",tm.em, #FUN-A90032 mark
        ...
        CALL cl_replace_sqldb(l_sql) RETURNING l_sql   #FUN-A50102
        CALL cl_parse_qry_sql(l_sql,g_plant_new) RETURNING l_sql   #FUN-A50102
        PREPARE r003_sum_p FROM l_sql
        DECLARE r003_sum_c CURSOR FOR r003_sum_p
        OPEN r003_sum_c
        FETCH r003_sum_c INTO amt1
        IF SQLCA.sqlcode THEN 
     ...
     EXECUTE insert_prep USING maj.maj31,maj.maj20,maj.maj20e,maj.maj03,   #FUN-B90140   Add maj.maj31
     ...
     IF maj.maj04 > 0 THEN
        ...
        FOR i = 1 TO maj.maj04
           EXECUTE insert_prep USING '',maj.maj20,maj.maj20e,maj.maj03,    #FUN-B90140   Add maj.maj31   #FUN-BA0099   把maj.maj31 换成''
  ...
  #列印公司編號,名稱
   ...
   LET g_str = g_basetot1,";",tm.rtype,";",tm.axa01,";",l_str,";",tm.axa03,";", #FUN-770069      10/19  #FUN-770069 10/19
               tm.a,";",tm.b,";",tm.yy,";",tm.bm,";",tm.em,";",
               tm.e,";",tm.f,";",tm.d,";",tm.c,";",tm.h,";",
               tm.o,";",tm.r,";",tm.p,";",tm.q  #,";",tm.ver  #NO.FUN-750076  #FUN-950048 拿掉tm.ver
               ,";",tm.acc_code   #FUN-B90140   Add
 
   CALL cl_prt_cs3('aglr003','aglr003',l_sql,g_str)   #FUN-710080 modify
   #------------------------------ CR (4) ------------------------------#
   
END FUNCTION
 
FUNCTION r003_set_entry()
...
#FUN-A50076 --------------------------add start---------------------------
FUNCTION r003_set_entry_1()
   CALL cl_set_comp_entry("exp,d",TRUE)
END FUNCTION

FUNCTION r003_set_no_entry_1()
   IF tm.xbrl = 'N' THEN
...
FUNCTION r003_get_bookno()
  ...
  #LET g_sql = "SELECT aaz641 FROM ",g_dbs_new,"aaz_file",   #FUN-A50102
   ...
   CALL cl_replace_sqldb(g_sql) RETURNING g_sql   #FUN-A50102
   CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql   #FUN-A50102
   PREPARE r003_pre_01 FROM g_sql
   DECLARE r003_cur_01 CURSOR FOR r003_pre_01
   OPEN r003_cur_01
   FETCH r003_cur_01 INTO g_aaz641   #合併帳別
   CLOSE r003_cur_01
   RETURN g_aaz641
...
FUNCTION r003_chk_bookno()
  ...
  #LET g_sql = "SELECT COUNT(*) FROM ",g_dbs_new,"aaz_file",   #FUN-A50102
   ...
   CALL cl_replace_sqldb(g_sql) RETURNING g_sql   #FUN-A50102
   CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql   #FUN-A50102
   PREPARE r003_pre_02 FROM g_sql
   DECLARE r003_cur_02 CURSOR FOR r003_pre_02
   OPEN r003_cur_02
   FETCH r003_cur_02 INTO l_cnt
...
FUNCTION r003_xbrl()
   ...
   IF tm.exp = '1' THEN
      ...
      LET l_name = "aglr003",l_dt CLIPPED,".txt"
   ...
   ELSE
     SELECT ze03 INTO l_ze03 FROM ze_file WHERE ze01='agl-512' AND ze02=g_lang
      ...
      CALL r003_checkError(li_result,"Connect DDE Sheet1")
      CALL r003_exceldata(1,' ',' ',' ',' ',' ')
      ...
      CALL r003_checkError(li_result,"Connect DDE Sheet2")
   END IF
   CASE WHEN tm.rtype='0' LET g_msg=" 1=1"
        WHEN tm.rtype='1' 
   ...
   LET g_basetot1 = NULL
    ...
    # LET l_sql = "SELECT * FROM ",g_dbs_axz03 CLIPPED," maj_file",               #FUN-A50076 mark
      ...
      PREPARE r003_pre1 FROM l_sql
      IF STATUS THEN
         CALL cl_err('prepare:',STATUS,1)
         CALL cl_used(g_prog,g_time,2) RETURNING g_time
         EXIT PROGRAM
      END IF
      DECLARE r003_curs1 CURSOR FOR r003_pre1
      FOR l_i = 1 TO 100
        LET g_tot1[l_i] = 0
      END FOR
      ...
      FOREACH r003_curs1 INTO l_maj.*
         ...
         LET l_amt1 = 0
         LET l_amt2 = 0
         #CALL r003_xbrl_bal(tm.yy,l_maj.*) RETURNING sr.bal1    #今年金額  #MOD-A70192 mark
         #CALL r003_xbrl_bal(tm.yy-1,l_maj.*) RETURNING sr.bal2  #去年金額  #MOD-A70192 mark

         #--MOD-A70192 start--
         LET l_amt1 = 0
         LET l_amt2 = 0
         CALL r003_xbrl_bal(tm.yy,l_maj.*) RETURNING l_amt1    #今年金額
         CALL r003_xbrl_bal(tm.yy-1,l_maj.*) RETURNING l_amt2  #去年金額
         #--MOD-A70192 end--
         ...
         LET g_r = g_r +1
        #CALL r003_tofile(g_r,l_name,l_maj.maj21,l_maj.maj20,l_bal1,l_bal2)   #FUN-B60077 mark
         CALL r003_tofile(g_r,l_name,l_maj.maj31,l_maj.maj20,l_bal1,l_bal2)   #FUN-B60077 add
      END FOREACH  
...
#FUN-A50076 -----------------------------------------add start----------------------------------
FUNCTION r003_xbrl_bal(p_yy,l_maj)
...
#                 "   AND axh05 = aag01 AND aag07 MATCHES '[23]'",       #No.TQC-B30100 Mark
      ...
      CALL cl_replace_sqldb(l_sql) RETURNING l_sql
      CALL cl_parse_qry_sql(l_sql,g_plant_axz03) RETURNING l_sql              #FUN-A50076 add
      PREPARE r003_sum_p1 FROM l_sql
      DECLARE r003_sum_c1 CURSOR FOR r003_sum_p1
      OPEN r003_sum_c1
      FETCH r003_sum_c1 INTO l_amt1
      IF SQLCA.sqlcode THEN
...
#FUN-A50076 -------------------------add start-------------------------------------
#FUNCTION r003_tofile(p_r,p_name,p_maj21,p_maj20,p_bal1,p_bal2)  #FUN-B60077 mark
FUNCTION r003_tofile(p_r,p_name,p_maj31,p_maj20,p_bal1,p_bal2)   #FUN-B60077 modify 
...
DEFINE l_azn05      LIKE azn_file.azn05
     ...
     #CALL r003_totxt(p_maj21,p_maj20,p_bal1,p_bal2)   #FUN-B60077 mark
      CALL r003_totxt(p_maj31,p_maj20,p_bal1,p_bal2)   #FUN-B60077 add 
   ELSE
     #CALL r003_exceldata('2',p_r,p_maj21,p_maj20,p_bal1,p_bal2)  #FUN-B60077 mark
      CALL r003_exceldata('2',p_r,p_maj31,p_maj20,p_bal1,p_bal2)  #FUN-B60077 add
   END IF
END FUNCTION
#FUN-A50076 -------------------------add end-------------------------------

#FUN-A50076 -------------------------add start-----------------------------
#FUNCTION r003_totxt(p_maj21,p_maj20,p_bal1,p_bal2)  #FUN-B60077 mark
#DEFINE p_maj21    LIKE maj_file.maj21               #FUN-B60077 mark
FUNCTION r003_totxt(p_maj31,p_maj20,p_bal1,p_bal2)   #FUN-B60077 add 
DEFINE p_maj31    LIKE maj_file.maj31                #FUN-B60077 add
DEFINE p_maj20    LIKE maj_file.maj20
DEFINE p_bal1     LIKE aah_file.aah04
DEFINE p_bal2     LIKE aah_file.aah04
...
#FUNCTION r003_exceldata(p_s,p_r,p_maj21,p_maj20,p_bal1,p_bal2)  #FUN-B60077 mark
FUNCTION r003_exceldata(p_s,p_r,p_maj31,p_maj20,p_bal1,p_bal2)   #FUN-B60077
...
#公司代號
      ...
      CALL r003_checkError(li_result,"DDEPoke Sheet1 R1C1")
      LET l_str = g_axz09
      LET ls_cell = "R1C2"
      CALL ui.Interface.frontCall("WINDDE","DDEPoke",[g_cmd,sheet1,ls_cell,l_str],[li_result])
      CALL r003_checkError(li_result,"DDEPoke Sheet1 R1C2")
#年度
     SELECT ze03 INTO l_ze03
        FROM ze_file
       WHERE ze01 = 'agl-506'
      ...
      LET ls_cell = "R2C1"
      CALL ui.Interface.frontCall("WINDDE","DDEPoke",[g_cmd,sheet1,ls_cell,l_str],[li_result])
      CALL r003_checkError(li_result,"DDEPoke Sheet1 R2C1")
      LET l_str = g_yy
      LET ls_cell = "R2C2"
      CALL ui.Interface.frontCall("WINDDE","DDEPoke",[g_cmd,sheet1,ls_cell,l_str],[li_result])
      CALL r003_checkError(li_result,"DDEPoke Sheet1 R2C2")
#季别
     SELECT ze03 INTO l_ze03
        FROM ze_file
       WHERE ze01 = 'agl-507'
      ...
      LET ls_cell = "R3C1"
      CALL ui.Interface.frontCall("WINDDE","DDEPoke",[g_cmd,sheet1,ls_cell,l_str],[li_result])
      CALL r003_checkError(li_result,"DDEPoke Sheet1 R3C1")
      LET l_str = g_ss
      LET ls_cell = "R3C2"
      CALL ui.Interface.frontCall("WINDDE","DDEPoke",[g_cmd,sheet1,ls_cell,l_str],[li_result])
      CALL r003_checkError(li_result,"DDEPoke Sheet1 R3C2")
   END IF
   IF p_r = 1 THEN
#科目代号
      SELECT ze03 INTO l_ze03
      ...
      CALL r003_checkError(li_result,"DDEPoke Sheet2 R1C1")
...
#科目名称
      ...
      CALL r003_checkError(li_result,"DDEPoke Sheet2 R1C2")
#今年数值
     SELECT ze03 INTO l_ze03
       FROM ze_file
      WHERE ze01 = 'agl-510'
     ...
     CALL r003_checkError(li_result,"DDEPoke Sheet2 R1C3")   
#去年数值     
     SELECT ze03 INTO l_ze03
       FROM ze_file
      WHERE ze01 = 'agl-511'
     ...
     CALL r003_checkError(li_result,"DDEPoke Sheet2 R1C4")
     LET g_r = g_r + 1
     LET p_r = g_r
   END IF
             ...
             WHEN 1
               ...
               #   LET l_str = p_maj21         #FUN-B60077 mark
                IF NOT cl_null(p_maj31) THEN   #FUN-B60077
                   ...
                   CALL r003_checkError(li_result,"DDEPoke Sheet2 ")
                END IF
             WHEN 2
...
FUNCTION r003_checkError(p_result,p_msg)
