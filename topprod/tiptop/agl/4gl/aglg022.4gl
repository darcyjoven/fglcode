...
# Modify.........: NO:FUN-C40071 12/04/20 By yangtt CR報表轉成GRW
...
TYPE sr1_t RECORD
    ...
    aeii05 LIKE aeii_file.aeii05,
    yy1 LIKE aeii_file.aeii09,
    yy2 LIKE aeii_file.aeii09,
...
MAIN       
   ...
   LET l_table = cl_prt_temptable('aglg022',g_sql) CLIPPED  # 產生Temp Table
...
FUNCTION g022_tm()
   ...
   OPEN WINDOW g022_w AT p_row,p_col WITH FORM "agl/42f/aglg022"
...
#luttb
      ...
      AFTER FIELD b
         ...
         PREPARE g022_cnt_p FROM g_sql
         DECLARE g022_cnt_c CURSOR FOR g022_cnt_p
         OPEN g022_cnt_c
         FETCH g022_cnt_c INTO l_cnt
...
################################################################################
      ...
      ON ACTION CONTROLP
         ...
         IF INFIELD(a) THEN
           ...
           #LET g_qryparam.where = " mai00 = '",tm.b,"'"   #No.FUN-C40071   Mark
            LET g_qryparam.where = " mai00 = '",tm.b,"' AND mai03 NOT IN ('5','6')"  #No.FUN-C40071   Add
   ...
   END INPUT
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW g022_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      EXIT PROGRAM
   ...
   END IF
      IF g_bgjob = 'Y' THEN
         ...
         ELSE 
            ...
            CALL cl_cmdat('aglg022',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW g022_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time
         EXIT PROGRAM
...
###GENGRE###   LET g_str = g_mai02,";",tm.a,";",tm.e,";",l_i_cnt05,";",l_i_cnt05,";",l_i_cnt05,";",l_i_cnt05
 
###GENGRE###   CALL cl_prt_cs3('aglg022','aglg022',g_sql,g_str)
    CALL aglg022_grdata()    ###GENGRE###
     
END FUNCTION

FUNCTION g022_ahe02(p_giu)
...
FUNCTION aglg022_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg022")
        IF handler IS NOT NULL THEN
            START REPORT aglg022_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg022_datacur1 CURSOR FROM l_sql
            FOREACH aglg022_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg022_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg022_rep
        END IF
...
REPORT aglg022_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_fmt1   STRING                #FUN-C40071  
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            LET l_fmt1 = cl_gr_numfmt("axh_file","axh08",sr1.azi04)    #FUN-C40071
            PRINTX l_fmt1    #FUN-C40071
