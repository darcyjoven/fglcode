...
# Modify.........: No.TQC-760105 07/06/12 By dxfwo aglr905(總分類帳) 帳別 tm.bookno 未 display or deafult!
...
# Modify.........: No.MOD-BA0205 11/10/31 By Polly 1.修改aglr905_prepare1條件
#                                                  2.FUNCTION aglr905_tm()將 bdate 與 edate 給予今天的預設值
# Modify.........: No.FUN-BB0047 11/12/30 By fengrui  調整時間函數問題
# Modify.........: No.MOD-C30912 12/03/30 By wujie  科目开窗限定aag07 in 1，3
...
MAIN
   ...
   LET l_table1 = cl_prt_temptable('aglr9051',l_sql) CLIPPED                                                                        
   IF l_table1 = -1 THEN EXIT PROGRAM END IF                                                                                        
                                                                                                                                    
   LET l_sql = "abc01.abc_file.abc01,",                                                                                             
               "abc02.abc_file.abc02,",                                                                                             
               "abc04.abc_file.abc04"                                                                                               
   LET l_table2 = cl_prt_temptable('aglr9052',l_sql) CLIPPED                                                                        
...
#  CALL s_dsmark(g_bookno)   #No.FUN-740020
   ...
   OPEN WINDOW aglr905_w AT p_row,p_col
     WITH FORM "agl/42f/aglr905"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
         ...
         ON ACTION CONTROLP
          CASE
           ...
           WHEN INFIELD(aag01)
              ...
              LET g_qryparam.state    = "c"
              LET g_qryparam.form = "q_aag"
              LET g_qryparam.where = " aag00 = '",tm.bookno CLIPPED,"' AND aag07 IN ('1','3')"   #No.MOD-C30912
              CALL cl_create_qry() RETURNING g_qryparam.multiret
              DISPLAY g_qryparam.multiret TO aag01
              NEXT FIELD aag01
           OTHERWISE EXIT CASE
...
#                        " '",g_bookno CLIPPED,"' ",
             ...
             CALL cl_cmdat('aglr905',g_time,l_cmd)      # Execute cmd at later time
          END IF
...
#   WHERE aaf01 = g_bookno     #No.FUN-740020
   ...
   PREPARE aglr905_prepare2 FROM l_sql1
   IF STATUS != 0 THEN
      CALL cl_err('prepare:',STATUS,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE aglr905_curs2 CURSOR FOR aglr905_prepare2
...
#              "   AND aba00 = '",g_bookno,"' ",    #No.FUN-740020
               ...
               "   AND aba00 = abb00 ",
              #"   AND aba00 = abb00 ",             #MOD-BA0205 mark
               "   AND aea00 = abb00 ",             #MOD-BA0205 add
               "   AND aea02 BETWEEN '",bdate,"' AND '",edate,"' ",
               "   AND aba01 = aea03 AND abb01 = aea03 AND abb02 = aea04 ",
               "   ORDER BY aea01,aea02,aea03"
 
   PREPARE aglr905_prepare1 FROM l_sql
   IF STATUS != 0 THEN
      CALL cl_err('prepare:',STATUS,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE aglr905_curs1 CURSOR FOR aglr905_prepare1
 
   #CALL cl_outnam('aglr905') RETURNING l_name       #No.FUN-830100
   #START REPORT aglr905_rep TO l_name               #No.FUN-830100 
 
   #LET g_pageno = 0                                 #No.FUN-830100  
 
   FOREACH aglr905_curs2 INTO sr1.*
      IF STATUS THEN
    ...
    # IF g_aza.aza26 = '2' AND NOT cl_null(tm.y) THEN  #CHI-710005
     ...
     #LET l_flag='N'    #MOD-A50010 mark 
      ...
      FOREACH aglr905_curs1 USING sr1.aag01 INTO sr.*
        ...
        #LET l_flag='Y'   #MOD-850186 mark   #將l_flag改變值段搬到後面
         ...
         #No.FUN-830100   --begin
         #OUTPUT TO REPORT aglr905_rep(sr.*)
         LET l_bal = 0                                                                                                              
         LET l_bal0=0                                                                                                               
         LET l_d=0                                                                                                                  
         LET l_c=0                                                                                                                  
  ...
  #是否列印選擇條件                                                                                                                 
   IF g_zz05 = 'Y' THEN                                                                                                             
      ...
      RETURNING  tm.wc                                                                                                              
   END IF                                                                                                                           
   LET g_str = tm.wc,';',bdate,';',edate,';',tm.e,';',tm.u,';',tm.s,';',                                                            
               g_azi04,';',tm.v,';',tm.bookno                                                                                       
   CALL cl_prt_cs3('aglr905','aglr905',l_sql,g_str)                                                                                 
   #No.FUN-830100  --end                             
   
END FUNCTION
#No.FUN-830100  --BEGIN 
{
REPORT aglr905_rep(sr)
   DEFINE
          sr   RECORD
