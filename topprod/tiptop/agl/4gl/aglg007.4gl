...
# Modify.........: NO:FUN-B70066 11/07/21 bt belle 增加累換數科目,增加欄位:調整金額及合計欄位
...
# Modify.........: No.FUN-B90028 12/01/12 By xuxz FUN-BA0012追單 ,FUN-B50001還原,MOD-BB0128,MOD-BB0244,MOD-BC0144 
...
DEFINE g_amt41       LIKE type_file.num20_6       #FUN-B90028(MOD-BC0144) add
DEFINE g_amt42       LIKE type_file.num20_6       #FUN-B90028(MOD-BC0144) add
...
DEFINE g_aaz641      LIKE aaz_file.aaz641         #MOD-940202 add   #FUN-B50001 #FUN-B90028 not mark FROM FUN-B50001
#DEFINE g_aaw01      LIKE aaw_file.aaw01          #FUN-B90028 mark FROM FUN-B50001
DEFINE l_length      LIKE type_file.num5          #No.FUN-930076
DEFINE g_axa05       LIKE axa_file.axa05          #FUN-A90032
DEFINE g_axb02       LIKE axb_file.axb02          #FUN-B90028(MOD-BB0128) add
DEFINE g_axb03       LIKE axb_file.axb03          #FUN-B90028(MOD-BB0128) add
...
MAIN
  ...
  LET l_table=cl_prt_temptable("aglg007",g_sql) CLIPPED 
  ...
  LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,     #TQC-A40133 mod
              " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?)"  #FUN-B70066 add ? #No:FUN-870151 #CHI-A70061 add ?
...
#  LET tm.bm   = ARG_VAL(11)  #FUN-A90032
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL g007_tm()                           # Input print condition
   ELSE
      CALL g007()         
   END IF
...
FUNCTION g007_tm()
   ...
   OPEN WINDOW g007_w AT p_row,p_col WITH FORM "agl/42f/aglg007" 
...
WHILE TRUE
    LET l_sw = 1
    INPUT BY NAME tm.rtype,tm.axb01,tm.axb04,tm.axb05,tm.a,tm.b,  #FUN-930076 mod   #NO.FUN-750076 #FUN-950048 拿掉tm.ver
#                 tm.yy,tm.bm,tm.em,tm.f,tm.d,tm.c,tm.e,tm.more          #FUN-6C0012 #FUN-A90032 mark
       ...
       BEFORE INPUT
          CALL cl_qbe_init()
          CALL g007_set_entry()      #FUN-A90032
          CALL g007_set_no_entry()   #FUN-A90032
...
#         ELSE
#            #抓取上層公司所屬合併帳別
#            CALL g007_get_bookno() RETURNING tm.b
...
#FUN-A30122 -------------------------------add start------------------------------------------
          ...
          CALL s_get_aaz641(g_plant_axz03) RETURNING g_aaz641   #FUN-B50001 #FUN-B90028 not mark
          #CALL s_get_aaz641(g_plant_axz03) RETURNING g_aaw01#FUN-B90028 mark
          LET g_plant_new = g_plant_axz03
          LET tm.b = g_aaz641  #FUN-B90028 not mark
          #LET tm.b = g_aaw01  #FUN-B90028 mark
          DISPLAY BY NAME tm.b
       ...
       AFTER FIELD a
         ...
         #LET g_sql = "SELECT mai02,mai03 FROM ",g_dbs_axz03,"mai_file",  #FUN-A50102
          LET g_sql = "SELECT mai02,mai03 FROM ",cl_get_target_table(g_plant_new,'mai_file'), #FUN-A50102   
                      " WHERE mai01 = '",tm.a,"'",
                      #"   AND mai00 = '",tm.axb05,"'",     #MOD-A40008 #FUN-B90028 from FUN-B0012(TQC-B90053)
                      "   AND mai00 = '",tm.b,"'",    #FUN-B90028 from FUN-B0012(TQC-B90053)
                      "   AND maiacti IN ('Y','y')"
          CALL cl_replace_sqldb(g_sql) RETURNING g_sql   #FUN-A50102
          CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql   #FUN-A50102
          PREPARE g007_pre FROM g_sql
          DECLARE g007_cur CURSOR FOR g007_pre
          OPEN g007_cur
          FETCH g007_cur INTO g_mai02,g_mai03
          IF STATUS THEN
...
#FUN-A30122 --------------------------mod start---------------------------------
          IF tm.b <> g_aaz641 THEN      #FUN-B50001       #FUN-B90028 not mark                                 
          #IF tm.b <> g_aaw01 THEN     # FUN-B90028 mark                                        
             CALL cl_err('','agl-965',0)                                        
             NEXT FIELD b
          END IF 
#         LET g_errno = ''
#         CALL g007_chk_bookno()
...
#FUN-A90032 --End   
       ...
       ON ACTION CONTROLP
          CASE
             WHEN INFIELD(a) 
               ...
               #LET g_qryparam.where = "mai00 = '",tm.axb05,"'"      #MOD-A40008 #FUN-B90028 from FUN-B0012(TQC-B90053) mark
               #LET g_qryparam.where = "mai00 = '",tm.b,"'"   #FUN-B90028 from FUN-B0012(TQC-B90053) add   #No.TQC-C50042   Add
    ...
    END INPUT
    IF INT_FLAG THEN
       LET INT_FLAG = 0 CLOSE WINDOW g007_w 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
       CALL cl_gre_drop_temptable(l_table)   #FUN-B90028 add
       EXIT PROGRAM
    END IF
...
#                         " '",tm.bm CLIPPED,"'",    #FUN-A90032
          ...
          CALL cl_cmdat('aglg007',g_time,l_cmd)    # Execute cmd at later time
       END IF
       CLOSE WINDOW g007_w
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
       CALL cl_gre_drop_temptable(l_table)   #FUN-B90028 add
...
FUNCTION g007()
   ...
   SELECT aaf03 INTO g_company FROM aaf_file WHERE aaf01 = tm.b
      AND aaf02 = g_rlang
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aglg007'
  ...
  #     WHEN tm.rtype='2' LET g_msg=" SUBSTRING(maj23,1,1)='2'"  #NO.FUN-750076                    #CHI-A70061 mark
   ...
   END CASE
   DROP TABLE g007_file
   CREATE TEMP TABLE g007_file(
    maj02 LIKE maj_file.maj02,     
    maj03 LIKE maj_file.maj03,     
    maj04 LIKE maj_file.maj04,     
    maj05 LIKE maj_file.maj05,     
  ...
  #LET l_sql = "SELECT * FROM ",g_dbs_axz03 CLIPPED,"maj_file",   #No.FUN-920199  #FUN-A50102
   ...
   PREPARE g007_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)   #FUN-B90028 add
      EXIT PROGRAM 
   END IF
   DECLARE g007_c CURSOR FOR g007_p
   ...
   #SELECT axb02 INTO l_axb02#FUN-B90028(MOD-BB0128) mark
   SELECT axb02,axb03 INTO g_axb02,g_axb03 #FUN-B90028(MOD-BB0128) add
     FROM axb_file
    WHERE axb01 = tm.axb01
      AND axb04 = tm.axb04
      AND axb05 = tm.axb05
 
   #SELECT * INTO g_axz_u.* FROM axz_file WHERE axz01= l_axb02   #上層公司#FUN-B90028(MOD-BB0244) mark
   SELECT * INTO g_axz_u.* FROM axz_file WHERE axz01= g_axb02   #上層公司 #FUN-B90028(MOD-BB0244) add
   IF SQLCA.sqlcode THEN
      #CALL cl_err3("sel","axz_file",l_axb02,"",SQLCA.sqlcode,"","",0)   #No.FUN-660123  #FUN-B90028(MOD-BB0244) mark
      CALL cl_err3("sel","axz_file",g_axb02,"",SQLCA.sqlcode,"","",0)   #FUN-B90028(MOD-BB0244) add
   ...
   DECLARE tmp_curs CURSOR FOR
      SELECT * FROM g007_file ORDER BY maj02
   ...
   FOREACH tmp_curs INTO sr.*
      ...
      EXECUTE insert_prep USING
        ...
        #sr.maj21,sr.maj22,'2',sr.bal1,sr.axe11,sr.rate,sr.bal2,sr.axe12,                 #FUN-B70066 mark #CHI-A70061 add '2'
         sr.maj21,sr.maj22,'2',sr.bal1,sr.bal4,sr.bal5,sr.axe11,sr.rate,sr.bal2,sr.axe12, #FUN-B70066 
         sr.rate1,sr.bal3
        ...
        FOR i = 1 TO sr.maj04
           EXECUTE insert_prep USING
                  ...
                  #sr.maj21,sr.maj22,'1',sr.bal1,sr.axe11,sr.rate,sr.bal2,sr.axe12,                  #MOD-B80279 mark
                   sr.maj21,sr.maj22,'1',sr.bal1,sr.bal4,sr.bal5,sr.axe11,sr.rate,sr.bal2,sr.axe12,  #MOD-B80279
...
#        LET l_name = 'aglg007_1'                      #No.FUN-760083#FUN-B90028 mark
         LET g_template = 'aglg007_1'      #FUN-B90028 add
   ELSE                                           #No.FUN-760083
#      LET l_name = 'aglg007'                    #No.FUN-760083 #FUN-B90028 mark
       LET g_template = 'aglg007'      #FUN-B90028 add
   END IF                                         #No.FUN-760083
 
###GENGRE###   LET g_sql="SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED   #No.FUN-760083
###GENGRE###   LET g_str=tm.d,';',g_aaz.aaz77,';',g_mai02,';',tm.a,';',tm.yy,';',
###GENGRE###             tm.bm,';',tm.em,';',g_azi02_1,';',g_azi02_2,';',g_azi02_3,';',
###GENGRE###             tm.e,";",'',";",         #FUN-770069 add tm.ver #FUN-950048 
###GENGRE###             tm.axb04,";",g_axz_d.axz02   #FUN-770069 add 10/19
###GENGRE###   CALL cl_prt_cs3("aglg007",l_name,g_sql,g_str)      #No.FUN-760083
    CALL aglg007_grdata()    ###GENGRE###
END FUNCTION
 
FUNCTION g007_process()
   DEFINE l_sql         LIKE type_file.chr1000 #RDSQL STATEMENT        #No.FUN-680098  VARCHAR(1000) 
   ...
   #MOD-A50018---add---end---
    LET l_sql = "SELECT '',SUM(axg08-axg09),SUM(axg13-axg14),",
               ...
               #"   AND axg07 BETWEEN ? AND ? ", #FUN-A90032 mark
                ...
                "   AND axg02 = '",g_axb02,"'",  #FUN-B90028 (MOD-BB0128) add
                "   AND axg03 = '",g_axb03,"'"   #FUN-B90028 (MOD-BB0128) add
 
    PREPARE g007_axg_p FROM l_sql
    DECLARE g007_axg_c CURSOR FOR g007_axg_p
   ...
   #FUN-B70066--begin--
    LET l_sql1= " SELECT SUM(axj07)",
                ...
                "   AND axj06 = ? ",  #FUN-B90028(MOD-BC0144) add
                "   AND axi03 = '",tm.yy,"'",
                "   AND axi04 = '",tm.em,"'",
                "   AND axi08 = '1'",
                "   AND axi05 = '",tm.axb01,"'",
                ...
                "   AND aag00 = axj00 ",
                "   AND aag01 = axj03 "
    PREPARE g007_axg_p1 FROM l_sql1
    #DECLARE g007_axg_c1 CURSOR FOR g007_axg_p1 #FUN-B90028(MOD-BC0144) mark
    DECLARE g007_axg_c1 SCROLL CURSOR FOR g007_axg_p1  #FUN-B90028(MOD-BC0144) add
    IF STATUS THEN CALL cl_err('axg prepare',STATUS,1)
   ...
   #-MOD-A40008-add- 
    ...
    END IF
   #-MOD-A40008-end- 
    FOREACH g007_c INTO maj.*
       IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
       LET g_amt1 = 0
       LET g_amt2 = 0
       LET g_amt3 = 0
       LET g_amt4 = 0  #FUN-B70066
       LET g_amt41 = 0 #FUN-B90028(MOD-BC0144) add
       LET g_amt42 = 0 #FUN-B90028(MOD-BC0144) add
       ...
       IF NOT cl_null(maj.maj21) THEN
          ...
          LET g_amt42 = 0 #FUN-B90028(MOD-BC0144) add
          LET g_amt5 = 0  #FUN-B70066
#         OPEN g007_axg_c USING tm.b,maj.maj21,maj.maj22,tm.yy,tm.bm,tm.em #FUN-A90032 mark
          OPEN g007_axg_c USING tm.b,maj.maj21,maj.maj22,tm.yy,tm.em #FUN-A90032
         #FETCH g007_axg_c INTO l_axg05,g_amt3,g_amt1,g_amt2  #FUN-B70066
          FETCH g007_axg_c INTO l_axg05,g_amt3,g_amt5,g_amt2  #FUN-B70066
          IF STATUS THEN
             LET l_axg05=''
             LET l_axg06=''
             LET l_axg07=''
          ...
          END IF                     #FUN-770069
         #FUN-B70066--begin--
          OPEN g007_axg_c1 USING tm.b,maj.maj21,maj.maj22,'1' #FUN-B90028(MOD-BC0144) add
          FETCH g007_axg_c1 INTO g_amt41  #FUN-B90028(MOD-BC0144)mod g_amt4 -> g_amt41 
          #FUN-B90028(MOD-BC0144) add--str
          CLOSE g007_axg_c1
          IF cl_null(g_amt41) THEN LET g_amt41 = 0 END IF 
          OPEN g007_axg_c1 USING tm.b,maj.maj21,maj.maj22,'2' 
          FETCH g007_axg_c1 INTO g_amt42                    
          CLOSE g007_axg_c1
          IF cl_null(g_amt42) THEN LET g_amt42 = 0 END IF 
          LET g_amt4 = g_amt41 - g_amt42
          #FUN-B90028(MOD-BC0144) add--end
          IF STATUS THEN
             LET g_amt1=0
             LET g_amt4=0
          END IF
...
#         AND axg07 BETWEEN tm.bm AND tm.em #FUN-A90032
     ...
     #累積換算調整數(g_aaz.aaz87)要另外抓分錄(axj_file)裡的金額來show
       IF maj.maj21 = g_aaz.aaz87 OR maj.maj22 = g_aaz.aaz87 THEN
        ...
        #--FUN-B90028 from FUN-BA0012(MOD-B90085)--mark--str
        ...
        #--FUN-B90028 from FUN-BA0012(MOD-B90085)--mark--end
       END IF
       #FUN-B70066---end---
       #-->合計階數處理
      ...
      #   sr.bal1=0 THEN                                                           #MOD-B80279 mark
          ...
          CONTINUE FOREACH
       END IF
       INSERT INTO g007_file VALUES(maj.maj02,maj.maj03,maj.maj04,
                                    maj.maj05,maj.maj07,maj.maj09,   #FUN-770069 add maj.maj09 10/19
                                    maj.maj20,maj.maj20e,
                                    maj.maj21,maj.maj22,sr.*)
       IF STATUS OR SQLCA.SQLERRD[3] = 0 THEN
          CALL cl_err3("ins","g007_file",maj.maj02,maj.maj03,SQLCA.sqlcode,"","ins g007_file",1)   #No.FUN-660123
          CALL ui.Interface.refresh()           #MOD-AB0200
          EXIT FOREACH  
       END IF
    END FOREACH
...
#FUN-B50001--mod--end
   CALL cl_replace_sqldb(g_sql) RETURNING g_sql   #FUN-A50102
   CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql   #FUN-A50102
   PREPARE g007_pre_01 FROM g_sql
   DECLARE g007_cur_01 CURSOR FOR g007_pre_01
   OPEN g007_cur_01
   FETCH g007_cur_01 INTO g_aaz641   #合併帳別   #FUN-B50001#FUN-B90028 not mark
   #FETCH g007_cur_01 INTO g_aaw01   #合併帳別 #FUN-B90028 mark
   CLOSE g007_cur_01
   RETURN g_aaz641   #FUN-B50001 #FUN-B90028 not mark
   #RETURN g_aaw01 #FUN-B90028 mark
 
END FUNCTION
 
FUNCTION g007_chk_bookno()
   DEFINE l_axa02        LIKE axa_file.axa02
...
#FUN-B50001--mod--end
   CALL cl_replace_sqldb(g_sql) RETURNING g_sql   #FUN-A50102
   CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql   #FUN-A50102
   PREPARE g007_pre_02 FROM g_sql
   DECLARE g007_cur_02 CURSOR FOR g007_pre_02
   OPEN g007_cur_02
   FETCH g007_cur_02 INTO l_cnt
...
FUNCTION aglg007_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg007")
        IF handler IS NOT NULL THEN
            START REPORT aglg007_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg007_datacur1 CURSOR FROM l_sql
            FOREACH aglg007_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg007_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg007_rep
        END IF
...
#FUN-B90028-----------STAR-----------
    ...
    DEFINE l_unit1       STRING
    DEFINE l_unit2       STRING
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            IF g_aaz.aaz77 = 'Y' THEN
               LET l_g_x = g_mai02
            ELSE 
               LET l_g_x = g_grPageHeader.title2 
            END IF
            PRINTX l_g_x

            LET l_sub = tm.axb04,"(",g_axz_d.axz02,")"
            PRINTX l_sub
        ...
        ON EVERY ROW
            ...
            LET l_unit1 = cl_gr_getmsg("gre-233",g_lang,sr1.axe11)
            PRINTX l_unit1
            LET l_unit2 = cl_gr_getmsg("gre-233",g_lang,sr1.axe12)
            PRINTX l_unit2

            LET l_fmt1 = cl_gr_numfmt("axg_file","axg18",sr1.azi07)
            PRINTX l_fmt1
            LET l_fmt2 = cl_gr_numfmt("axg_file","axg19",sr1.azi07_1)
            PRINTX l_fmt2
