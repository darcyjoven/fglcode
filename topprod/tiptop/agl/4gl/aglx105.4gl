...
# Modify.........: No.FUN-D40020 12/11/05 By wangrr CR轉XtraGrid
...
MAIN
   ...
   LET g_sql = "maj20.maj_file.maj20,",
              ...
              ,",e.type_file.num5 "       #FUN-D40020 add #小數位數
 
   LET l_table = cl_prt_temptable('aglx105',g_sql) CLIPPED   # 產生Temp Table
   ...
   #LET g_sql = "INSERT INTO ds_report.",l_table CLIPPED,  #TQC-A40133 mark
    LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,   #TQC-A40133 mod
               " VALUES(?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?)"              #FUN-8B0059 Add ?,?  #FUN-D40020 add 1 '?'
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL x105_tm()                   # Input print condition
      ELSE CALL x105()                      # Read data and create out-file
   END IF
...
FUNCTION x105_tm()
   ...
   OPEN WINDOW x105_w AT p_row,p_col WITH FORM "agl/42f/aglx105" 
...
#      INPUT BY NAME tm.a,tm.b,tm.yy,tm.bm,tm.em,tm.dd,
       ...
       ON ACTION locale
         ...
         AFTER FIELD a
           #IF tm.a IS NULL THEN NEXT FIELD a END IF   #No.FUN-D40020   Mark
          #FUN-6C0068--begin
           IF NOT cl_null(tm.a) THEN   #No.FUN-D40020   Add
...
#              CALL cl_err('sel mai:',STATUS,0) #No.FUN-660123
          ...
          AFTER FIELD b
           #IF tm.b IS NULL THEN NEXT FIELD b END IF   #No.FUN-D40020   Mark
            #No.FUN-670005--begin
...
#              CALL cl_err('sel aaa:',STATUS,0)  #No.FUN-660123 
         ...
         AFTER FIELD c
           #IF tm.c IS NULL OR tm.c NOT MATCHES "[YN]" THEN NEXT FIELD c END IF   #No.FUN-D40020   Mark
            IF tm.c NOT MATCHES "[YN]" THEN NEXT FIELD c END IF
 
         AFTER FIELD yy
# genero  script marked             IF cl_ku() THEN NEXT FIELD PREVIOUS END IF
           #IF tm.yy IS NULL OR tm.yy = 0 THEN   #No.FUN-D40020   Mark
            IF tm.yy = 0 THEN   #No.FUN-D40020   Mark
               NEXT FIELD yy
            END IF
...
#No.TQC-720032 -- end --
# genero  script marked             IF cl_ku() THEN NEXT FIELD PREVIOUS END IF
           #IF tm.bm IS NULL THEN NEXT FIELD bm END IF     #No.FUN-D40020   Mark
#No.TQC-720032 -- begin --
...
#No.TQC-720032 -- begin --
         ...
         END IF
#No.TQC-720032 -- end --
           #IF tm.em IS NULL THEN NEXT FIELD em END IF   #No.FUN-D40020   Mark
#No.TQC-720032 -- begin --
...
#No.TQC-720032 -- end --
         ...
         AFTER FIELD d
           #IF tm.d IS NULL OR tm.d NOT MATCHES'[123]'  THEN    #No.FUN-D40020   Mark
            IF tm.d NOT MATCHES'[123]'  THEN                    #No.FUN-D40020   Mark
               NEXT FIELD d
            END IF
         ...
         AFTER FIELD f
           #IF tm.f IS NULL OR tm.f < 0  THEN   #No.FUN-D40020   Mark
            IF tm.f < 0 THEN                    #No.FUN-D40020   Mark
               LET tm.f = 0
         ...
         AFTER FIELD o
           #IF tm.o IS NULL OR tm.o NOT MATCHES'[YN]' THEN NEXT FIELD o END IF   #No.FUN-D40020   Mark
            IF tm.o NOT MATCHES'[YN]' THEN NEXT FIELD o END IF                   #No.FUN-D40020   Mark
            IF tm.o = 'N' THEN 
               LET tm.p = g_aaa03 
               LET tm.q = 1
               CALL cl_set_comp_entry('p,q',FALSE)   #No.FUN-D40020   Add
         ...
         AFTER FIELD p
            IF NOT cl_null(tm.p) THEN   #No.FUN-D40020   Add
...
#               CALL FGL_DIALOG_SETBUFFER( tm.p )
      ...
      ON ACTION help          #MOD-4C0121
       ...
       END INPUT
       IF INT_FLAG THEN
          LET INT_FLAG = 0 CLOSE WINDOW x105_w 
          CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
          EXIT PROGRAM
       ...
       IF g_bgjob = 'Y' THEN
          ...
          ELSE
            ...
            CALL cl_cmdat('aglx105',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW x105_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
         EXIT PROGRAM
...
#-----END TQC-620081-----
   ...
   PREPARE x105_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM 
   END IF
   DECLARE x105_c CURSOR FOR x105_p
   ...
   END FOR
  #CALL cl_outnam('aglx105') RETURNING l_name   #FUN-770044 mark
  #START REPORT x105_rep TO l_name              #FUN-770044 mark
   #-----MOD-850040---------
   LET l_maj02=''
   SELECT MAX(maj02) INTO l_maj02 FROM maj_file 
     WHERE maj01 = tm.a
   #-----END MOD-850040-----
   FOREACH x105_c INTO maj.*
      IF SQLCA.sqlcode THEN 
...
#-----END MOD-850040-----
      ...
      #FUN-D40020--add--str--
      LET sr.bal1_1=cl_digcut(sr.bal1_1,tm.e)
      LET sr.bal1_2=cl_digcut(sr.bal1_2,tm.e)
      LET sr.bal2_1=cl_digcut(sr.bal2_1,tm.e)
      LET sr.bal2_2=cl_digcut(sr.bal2_2,tm.e)
      IF NOT ((cl_null(maj.maj21) OR maj.maj21=' ') 
         AND  (cl_null(maj.maj22) OR maj.maj22=' ')) THEN
         IF tm.m='Y' THEN
            IF tm.h = 'N' THEN
               LET maj.maj20=maj.maj21,'-',maj.maj22,'  ',maj.maj20
            ELSE
               LET maj.maj20=maj.maj21,'-',maj.maj22,'  ',maj.maj20e
            END IF
         END IF
      END IF
      #FUN-D40020--add--end
     ...
     #OUTPUT TO REPORT x105_rep(maj.*, sr.*)   #FUN-770044 mark
   END FOREACH
...
###XtraGrid###   LET g_str = g_mai02,";",tm.a,";",tm.d,";",tm.yy,";",tm.bm,";",
###XtraGrid###               tm.em,";",tm.dd,";",tm.e,";",tm.m,";",tm.p                   #FUN-8B0059 Add tm.m,tm.p
###XtraGrid###   CALL cl_prt_cs3('aglx105','aglx105',g_sql,g_str)
...
#FUN-D40020--add--str--
   ...
   END IF
   LET g_str=cl_getmsg('aem-002',g_lang),':'
   IF cl_null(tm.dd) THEN
      LET g_str=g_str,tm.yy USING '<<<<','/',tm.bm USING'&&','-',
                      tm.yy USING '<<<<','/',tm.em USING'&&'
   ELSE
      LET g_str=g_str,tm.yy USING '<<<<','/',tm.bm USING'&&','/01-',
                      tm.yy USING '<<<<','/',tm.em USING'&&','/',tm.dd USING'&&'
   END IF
   LET g_str=g_str,cl_getmsg('azz1150',g_lang),':',tm.p
   ...
   LET g_xgrid.footerinfo2=cl_getmsg('agl-276',g_lang)
#FUN-D40020--add--end
    CALL cl_xg_view()    ###XtraGrid###
   #------------------------------ CR (4) ------------------------------#
   #end FUN-770044 add
 
  #FINISH REPORT x105_rep
 
  #CALL cl_prt(l_name,g_prtway,g_copies,g_len)
END FUNCTION
 
#str FUN-770044 mark
#REPORT x105_rep(maj, sr)
