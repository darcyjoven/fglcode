...
MAIN
   ...
   LET l_table = cl_prt_temptable("aglg911",g_sql) CLIPPED                                                                          
...
#No.FUN-830100  --END          
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      THEN CALL aglg911_tm()               # Input print condition
      ELSE CALL aglg911()                  # Read data and create out-file
   END IF
...
DEFINE l_combo        ui.ComboBox     #FUN-B80161 add 
   ...
   END IF
   OPEN WINDOW aglg911_w AT p_row,p_col
        WITH FORM "agl/42f/aglg911"
################################################################################
...
# 
#   IF INT_FLAG THEN
#      LET INT_FLAG = 0 CLOSE WINDOW aglg911_w 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
#      EXIT PROGRAM
#         
#   END IF
...
#No.FUN-B20054--add--end--
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglg911',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW aglg911_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)   #FUN-B80161
      EXIT PROGRAM
   END IF
...
#  LET l_sql = "SELECT '','','', aba00,aba01,aba02,",           #FUN-B80161 mark
   ...
   PREPARE aglg911_prepare1 FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)   #FUN-B80161
      EXIT PROGRAM 
   END IF
   DECLARE aglg911_curs1 CURSOR FOR aglg911_prepare1
   #CALL cl_outnam('aglg911') RETURNING l_name          #No.FUN-830100 
...
{         
   ...
   START REPORT aglg911_rep TO l_name
}                                                                                                                                   
#No.FUN-830100  --END                
   FOREACH aglg911_curs1 INTO sr.*
...
#No.FUN-830100  --end  
    ...
    #EXECUTE insert_prep USING sr.aba00, sr.aba01, #FUN-B80161 mark                                                                                 
     ...
     #OUTPUT TO REPORT aglg911_rep(sr.*)
     #No.FUN-830100  --END   
   END FOREACH
   #No.FUN-830100  --begin   
   #FINISH REPORT aglg911_rep
...
###GENGRE###   CALL cl_prt_cs3('aglg911',l_name,g_sql,g_str)                                                                                    
   IF tm.e = 'Y' AND tm.k = '1' THEN                                                                                                
#     LET l_name = 'aglg911'       #FUN-B80161 mark                                                                                                 
      LET g_template = 'aglg911'   #FUN-B80161  add
      CALL aglg911_grdata()        #FUN-B80161  add
   END IF                                                                                                                           
   IF tm.e = 'Y' AND tm.k = '2' THEN                                                                                                
#     LET l_name = 'aglg911'       #FUN-B80161 mark                                                                                                 
      LET g_template = 'aglg911'   #FUN-B80161  add
   ...
   END IF                                                                                                                           
   IF tm.e = 'N' THEN                                                                                                               
#     LET l_name = 'aglg911_1'     #FUN-B80161 mark                                                                                                  
      LET g_template = 'aglg911_1' #FUN-B80161  add
      CALL aglg911_grdata()        #FUN-B80161  add
   END IF                                                                                                                           
   #No.FUN-830100  --end                              
END FUNCTION
...
FUNCTION aglg911_grdata()
    ...
    LET g_sql1 = "SELECT COUNT(*) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                 " WHERE order1=?"
    DECLARE aglg911_repcur01 CURSOR FROM g_sql1
    LET g_sql2 = "SELECT COUNT(*) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                 " WHERE order1=? AND order2=?"
    DECLARE aglg911_repcur02 CURSOR FROM g_sql2
    LET g_sql2 = "SELECT COUNT(*) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                 " WHERE order1=? AND order2=? AND order3=?"
    DECLARE aglg911_repcur03 CURSOR FROM g_sql3
    #FUN-B80161------add---end---

    WHILE TRUE
        CALL cl_gre_init_pageheader()            
        LET handler = cl_gre_outnam("aglg911")
        IF handler IS NOT NULL THEN
            START REPORT aglg911_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY order1,order2,order3,aba01"   #FUN-B80161  add
          
            DECLARE aglg911_datacur1 CURSOR FROM l_sql
            FOREACH aglg911_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg911_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg911_rep
        END IF
...
#FUN-B80161---add-----str--------
PRIVATE FUNCTION aglg911_replace_ord_desc(p_desc)
   ...
   ORDER EXTERNAL BY sr1.order1,sr1.order2,sr1.order3    #FUN-B80161 add    
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            LET l_orderA = aglg911_replace_ord_desc(g_ord1_desc),'-',aglg911_replace_ord_desc(g_ord2_desc),'-',aglg911_replace_ord_desc(g_ord3_desc)  
            LET g_ord1_desc = aglg911_replace_ord_desc(g_ord1_desc),cl_gr_getmsg("gre-245",g_lang,1),':'
            LET g_ord2_desc = aglg911_replace_ord_desc(g_ord2_desc),cl_gr_getmsg("gre-245",g_lang,1),':'
            LET g_ord3_desc = aglg911_replace_ord_desc(g_ord3_desc),cl_gr_getmsg("gre-245",g_lang,1),':'
        ...
        BEFORE GROUP OF sr1.order1
            #FUN-B80161---add----str-------
            FOREACH aglg911_repcur01 USING sr1.order1 INTO l_cnt1 END FOREACH
            LET l_ord1_cnt = 0
            #FUN-B80161---add----end-------
        BEFORE GROUP OF sr1.order2
            #FUN-B80161---add----str-------
            FOREACH aglg911_repcur02 USING sr1.order2 INTO l_cnt2 END FOREACH
            LET l_ord2_cnt = 0
            #FUN-B80161---add----end-------
        BEFORE GROUP OF sr1.order3
            #FUN-B80161---add----str-------
            FOREACH aglg911_repcur03 USING sr1.order3 INTO l_cnt3 END FOREACH
            LET l_ord3_cnt = 0
            #FUN-B80161---add----end-------
