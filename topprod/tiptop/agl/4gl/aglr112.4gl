...
# Modify.........: No.MOD-BB0296 11/11/28 By Dido 合計階計算需包含 maj07 判斷處理
...
# Modify.........: No.TQC-C90046 12/09/07 By lujh 1.報表類型選擇“資產負債表”時，報表編號開窗時應只選擇報表性質是“2:資產負債表” 
#                                                 2.報表類型選擇“損益表”時報表編號開窗時應只選擇報表性質是mai03=3:損益表
# Modify.........: No.TQC-C90051 12/09/10 By lujh 合計計算有誤
# Modify.........: No.MOD-C80045 12/09/28 By wangwei 資產負責類型報表不需要減損益類科目金額
# Modify.........: No.MOD-CA0166 12/10/12 By yinhy 更改CE憑證抓取方式
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglr112',g_sql) CLIPPED  # 產生Temp Table
...
#  LET tm.b = g_bookno     #No.FUN-740020
   ...
   CALL cl_set_comp_visible("ce",TRUE)   #No.FUN-D40044   Add
WHILE TRUE
    LET l_sw = 1
#   INPUT BY NAME tm.rtype,tm.a,tm.b,
    INPUT BY NAME tm.rtype,tm.b,tm.a,    #No.FUN-740020
                  ...
                  tm.e,tm.f,tm.d,tm.acc_code,tm.c,tm.h,tm.ce,tm.o,tm.r,   #FUN-B90140 add tm.acc_code   #No.FUN-D40044   Add tm.ce
     ...
     #No.FUN-D40044 ---add--- str
      ON CHANGE rtype
         IF tm.rtype = '1' THEN
            CALL cl_set_comp_visible("ce",FALSE)
         ELSE
            CALL cl_set_comp_visible("ce",TRUE)
         END IF
       ...
       #FUN-6C0068--end
         SELECT mai02,mai03 INTO g_mai02,g_mai03 FROM mai_file
                WHERE mai01 = tm.a 
                  ...
                  AND maiacti IN ('Y','y')
                  AND mai03 IN ('2','3')     #TQC-C90046  add
         IF STATUS THEN 
#           CALL cl_err('sel mai:',STATUS,0)  #No.FUN-660123
        ...
        #No.TQC-C50042   ---start---   Add
         ELSE
            ...
            IF tm.rtype='1' THEN
               IF g_mai03 = '3' THEN
                  CALL cl_err('','agl-270',0)
                  NEXT FIELD a
               END IF
            END IF
            IF tm.rtype='2' THEN
               IF g_mai03 = '2' THEN
                  CALL cl_err('','agl-271',0)
                  NEXT FIELD a
               END IF
            END IF
            #TQC-C90046--add--end--
...
################################################################################
      ...
      ON ACTION CONTROLP
         IF INFIELD(a) THEN
            IF tm.rtype='1' THEN     #TQC-C90046  add
#              CALL q_mai(0,0,tm.a,tm.a) RETURNING tm.a
              ...
              #LET g_qryparam.where = " mai00 = '",tm.b,"'"  #No.FUN-740020   #No.TQC-C50042   Mark
               #LET g_qryparam.where = " mai00 = '",tm.b,"' AND mai03 NOT IN ('5','6')"  #No.TQC-C50042   Add   #TQC-C90046  mark
               LET g_qryparam.where = " mai00 = '",tm.b,"' AND mai03 = '2'" #TQC-C90046  add
            ...
            ELSE
               ...
               LET g_qryparam.form = 'q_mai'
               LET g_qryparam.default1 = tm.a
               LET g_qryparam.where = " mai00 = '",tm.b,"' AND mai03 = '3'" 
               CALL cl_create_qry() RETURNING tm.a
               #No.BUG_480213 (end)
               DISPLAY BY NAME tm.a
               NEXT FIELD a
...
#           CALL FGL_DIALOG_SETBUFFER( tm.p )
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglr112',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW r112_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
...
#     DEFINE   l_time LIKE type_file.chr8         #No.FUN-6A0073
   ...
   DEFINE l_aeh15   LIKE aeh_file.aeh15   #MOD-CA0166
   DEFINE l_aeh16   LIKE aeh_file.aeh16   #MOD-CA0166
...
#  CALL cl_outnam('aglr112') RETURNING l_name
     ...
     IF NOT cl_null(maj.maj21) AND tm.yy2 IS NOT NULL THEN
        ...
        IF tm.rtype='2' THEN   #MOD-C80045
           ...
           CALL s_minus_ce(tm.b, maj.maj21, maj.maj22, NULL,     NULL,     NULL,
                           ...
                           tm.bm2,    tm.em2,    NULL,      NULL,     NULL,     NULL,
                           NULL,  NULL,      NULL,      NULL,     g_plant,  g_aaa09,'0')
                RETURNING  l_endy1,l_endy2,l_aeh15,l_aeh16
           #No.MOD-CA0166  --End
            ...
            #-MOD-A60135-add-
             FOR i = 1 TO 100 
                 IF maj.maj09 = '-' THEN
                    ...
                    LET g_tot2[i] = g_tot2[i] - amt2   #MOD-BB0296
                    LET g_tot3[i] = g_tot3[i] - amt3   #MOD-BB0296
                 ELSE
                    LET g_tot1[i] = g_tot1[i] + amt1
                    LET g_tot2[i] = g_tot2[i] + amt2   #MOD-BB0296
                    LET g_tot3[i] = g_tot3[i] + amt3   #MOD-BB0296
...
#    OUTPUT TO REPORT r112_rep(maj.*, sr.*)
#MOD-720040 By TSD.Jin--end
   END FOREACH
   #-----TQC-6B0171--------- 
   ...
   CALL cl_prt_cs3('aglr112','aglr112',l_sql,g_str)   #FUN-710080 modify
