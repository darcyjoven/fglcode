...
#NO.FUN-7C0038-------------start----------
   ...
   LET l_table = cl_prt_temptable('aglr801',g_sql) CLIPPED
...
FUNCTION aglr801_tm(p_row,p_col)
...
#   CALL s_dsmark(g_bookno)   #No.FUN-740020
   ...
   OPEN WINDOW aglr801_w AT p_row,p_col
     WITH FORM "agl/42f/aglr801"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
#                        " '",g_bookno CLIPPED,"'", #No.FUN-740020
            ...
            CALL cl_cmdat('aglr801',g_time,l_cmd)      # Execute cmd at later time
         END IF
...
#    WHERE aaf01 = g_bookno       #No.FUN-740020
   ...
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aglr801' #NO.FUN-7C0038 
...
#No.FUN-740020 -- begin --
   LET l_sql = "SELECT aec00,aec01,aec02,aec03,aec051,aec05,abb06,abb07,azi04,'' ",
...
#     LET l_sql =l_sql CLIPPED," AND aag01 =aec01 aag00 =aec00 AND aag07 MATCHES '[2]' "   #No.FUN-740020
   ...
   PREPARE aglr801_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE aglr801_curs1 CURSOR FOR aglr801_prepare1
 
#   CALL cl_outnam('aglr801') RETURNING l_name             #NO.FUN-7C0038
#   START REPORT aglr801_rep TO l_name                     #NO.FUN-7C0038
 
#   LET g_pageno = 0                                       #NO.FUN-7C0038
 
   FOREACH aglr801_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
...
#      OUTPUT TO REPORT aglr801_rep(sr.*)                  #NO.FUN-7C0038
#NO.FUN-7C0038-------------start---------
...
#   FINISH REPORT aglr801_rep                              #NO.FUN-7C0038
...
#NO.FUN-7C0038------------start---------
    ...
    IF g_zz05 = 'Y' THEN
       CALL cl_wcchp(tm.wc,'aec01,aec051,aec052,aec05,aec02')
          RETURNING tm.wc
    END IF
    LET g_str = tm.wc,";",tm.t,";",tm.e
    CALL cl_prt_cs3('aglr801','aglr801',g_sql,g_str)
#NO.FUN-7C0038---------------end-------
END FUNCTION
#NO.FUN-7C0038---------------start-----mark---------
#REPORT aglr801_rep(sr)
