...
#No.FUN-760083  --BEGIN--
   ...
   LET l_table=cl_prt_temptable("aglr108",g_sql)  CLIPPED
...
FUNCTION aglr108_tm()
 ...
 DEFINE li_chk_bookno LIKE type_file.num5      #FUN-B20054
   ...
   OPEN WINDOW aglr108_w AT p_row,p_col
     WITH FORM "agl/42f/aglr108"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
    ...
    #FUN-B20054--add--end
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglr108',g_time,l_cmd)  # Execute cmd at later time
         END IF
...
#  LET l_sql = l_sql CLIPPED," AND aag38 = '",tm.aag38,"'"   #TQC-6C0098 add  #TQC-770047 mark
   ...
   PREPARE aglr108_prepare1 FROM l_sql
   IF STATUS THEN
      CALL cl_err('prepare:',STATUS,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE aglr108_curs1 CURSOR FOR aglr108_prepare1
#No.FUN-760083 --BEGIN--
  {
   CALL cl_outnam('aglr108') RETURNING l_name     
   ...
   START REPORT aglr108_rep TO l_name
  }
   IF tm.f = 'Y' THEN
      LET l_name = 'aglr108_1'
   ELSE
      LET l_name = 'aglr108'
   END IF
   LET g_str=''
...
#No.FUN-760083  --end--
   ...
   FOREACH aglr108_curs1 INTO sr.*
      ...
      #OUTPUT TO REPORT aglr108_rep(sr.*)   #No.FUN-760083
   ...
   #FINISH REPORT aglr108_rep  #No.FUN-760083
   ...
   LET g_str = tm.wc,';',tm.b_date,';',tm.e_date,';',t_azi04,';',tm.e,';',t_azi05  #No.FUN-760083
   CALL cl_prt_cs3("aglr108",l_name,g_sql,g_str)    #No.FUN-760083
END FUNCTION
 
#No.FUN-760083  --begin--
{
REPORT aglr108_rep(sr)
