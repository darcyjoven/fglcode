...
# Modify.........: No.FUN-B80180 11/09/02 By xuxz 程序中涉及到gim_file的sql加上
#                                    條件:AND gim04 = tm.y1 AND gim05 = tm.m2 AND gim06 = tm.axa01
# Modify.........: No.FUN-B80158 11/09/20 By yangtt 明細類CR轉換成GRW
# Modify.........: No.FUN-B80158 12/01/05 By qirl FUN-B80180追單
...
#  LET tm.m1 = ARG_VAL(10)    #FUN-A90032
  ...
  #LET l_table = cl_prt_temptable('aglg006',g_sql) CLIPPED      
  #IF l_table = -1 THEN EXIT PROGRAM END IF                  
 
  #LET g_sql = "title.type_file.chr20,",
  #            "gim02.gim_file.gim02,",
  #            "gim03.gim_file.gim03"
  #LET l_table1 = cl_prt_temptable('aglg0061',g_sql) CLIPPED     
...
# genero  script marked    LET g_gim_arrno = 200
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL g006_tm()         # Input print condition
   ELSE
      CALL g006()            # Read data and create out-file
   END IF
...
FUNCTION g006_tm()
   ...
   OPEN WINDOW g006_w AT p_row,p_col WITH FORM "agl/42f/aglg006"
...
#               CALL FGL_DIALOG_SETBUFFER( tm.b )
   ...
   END INPUT
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW g006_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)   #FUN-B80158 add
      EXIT PROGRAM
...
FUNCTION g006()   #整段改寫
  ...
  #LET l_tot_amt = l_tot_amt + l_amt        #計算本期現金淨增數
   ...
   ELSE
      CALL g006_str(l_amt,l_str1) RETURNING l_str1
   ...
   PREPARE g006_gir_p1 FROM l_sql
   DECLARE g006_gir_cs1 CURSOR FOR g006_gir_p1
   #FUN-950111--mod--str--                                                                           
   ...
   PREPARE g006_p31 FROM l_sql                                                
   DECLARE g006_cu31 CURSOR FOR g006_p31                                 
   FOREACH g006_gir_cs1 INTO gir.*                                           
     LET l_amt_s = 0                                                         
     LET l_i_1 = 1  #FUN-940035 add
     FOREACH g006_cu31 USING gir.gir01 INTO tmp.*
   ...
   #FUN-950111--mod--end
         ...
         CASE tmp.gis04
           WHEN '1'
             CALL g006_axh(tmp.aag01,tm.y2,tm.m2,l_last_y,l_last_m,l_amt)
                  RETURNING l_amt
           WHEN '2'
             CALL g006_axh2(tmp.aag01,l_last_y,l_last_m) RETURNING l_amt
           WHEN '3'
             CALL g006_axh2(tmp.aag01,tm.y2,tm.m2) RETURNING l_amt
           WHEN '4'
           ...
           WHEN '5'  #借方異動
            #str FUN-780068 mod 09/20                                         
             CALL g006_axh_dc(tmp.aag01,'5',l_last_y,l_last_m)                
                  RETURNING l_amt       
           WHEN '6'  #貸方異動                                                
             CALL g006_axh_dc(tmp.aag01,'6',l_last_y,l_last_m)                
                  RETURNING l_amt
         END CASE
         IF cl_null(l_amt) THEN LET l_amt = 0 END IF
        #-MOD-AC0013-add-
      ...
      ELSE
         CALL g006_str(l_amt_s,l_str2) RETURNING l_str2
      ...
      ELSE 
      	 FOR l_i_2 = 1 TO l_i_1
            ...
            ELSE
               CALL g006_str(l_git[l_i_2].git05,l_str2) RETURNING l_str2
               LET l_str2 = l_str2 CLIPPED,')'
   ...
   ELSE
      CALL g006_str(l_sub_amt,l_str1) RETURNING l_str1
      LET l_str1 = l_str1 CLIPPED,')'
   END IF
   EXECUTE insert_prep USING l_type,l_str1,'','',l_gir05   #CHI-990066 add l_gir05
   LET l_sub_amt = 0
   ...
   PREPARE g006_gir_p2 FROM l_sql
   DECLARE g006_gir_cs2 CURSOR FOR g006_gir_p2
   ...
   LET l_sql = "SELECT '','',gis02,gis03,gis04,gis05,gis06,axz03,axz04 ",                                                        
               "  FROM gis_file,gir_file,axz_file",                                                                              
               " WHERE gis01 = gir01 AND axz01 = gis06 AND gir04='Y' AND gis01 = ?"                                              
   PREPARE g006_p4 FROM l_sql                                                
   DECLARE g006_cu4 CURSOR FOR g006_p4                                       
   FOREACH g006_gir_cs2 INTO gir.*                                           
      LET l_amt_s = 0                                                        
      LET l_i_1 = 1  #FUN-940035 add
      FOREACH g006_cu4 USING gir.gir01 INTO tmp.*
        CALL s_aaz641_dbs(tmp.gis05,tmp.gis06) RETURNING g_dbs_axz03
        #CALL s_get_aaz641(g_dbs_axz03) RETURNING g_aaz641   #FUN-B50001
        CALL s_get_aaz641(g_dbs_axz03) RETURNING g_aaw01
        LET l_sql = "SELECT aag01,aag02 FROM ",g_dbs_axz03 CLIPPED,"aag_file ",
   ...
   PREPARE g006_gir_p3 FROM l_sql
   DECLARE g006_gir_cs3 CURSOR FOR g006_gir_p3
   ...
   LET l_sql = "SELECT '','',gis02,gis03,gis04,gis05,gis06,axz03,axz04 ",                                                        
               "  FROM gis_file,gir_file,axz_file",                                                                              
               " WHERE gis01 = gir01 AND axz01 = gis06 AND gir04='Y' AND gis01 = ?"                                              
   PREPARE g006_p5 FROM l_sql                                                
   DECLARE g006_cu5 CURSOR FOR g006_p5                                       
   FOREACH g006_gir_cs3 INTO gir.*                                           
      LET l_amt_s = 0                                                        
      LET l_i_1 = 1  #FUN-940035 add
      FOREACH g006_cu5 USING gir.gir01 INTO tmp.*
        CALL s_aaz641_dbs(tmp.gis05,tmp.gis06) RETURNING g_dbs_axz03
        #CALL s_get_aaz641(g_dbs_axz03) RETURNING g_aaz641   #FUN-B50001
   ...
   PREPARE g006_gir_p4 FROM l_sql
   DECLARE g006_gir_cs4 CURSOR FOR g006_gir_p4
   ...
   LET l_sql = "SELECT '','',gis02,gis03,gis04,gis05,gis06,axz03,axz04 ",                                                        
               "  FROM gis_file,gir_file,axz_file",                                                                              
               " WHERE gis01 = gir01 AND axz01 = gis06 AND gir04='Y' AND gis01 = ?"                                              
   PREPARE g006_p6 FROM l_sql                                                
   DECLARE g006_cu6 CURSOR FOR g006_p6                                       
   FOREACH g006_gir_cs4 INTO gir.*                                                   
      LET l_amt_s = 0                                                        
      LET l_i_1 = 1  #FUN-940035 add
      FOREACH g006_cu6 USING gir.gir01 INTO tmp.*
         CALL s_aaz641_dbs(tmp.gis05,tmp.gis06) RETURNING g_dbs_axz03
         #CALL s_get_aaz641(g_dbs_axz03) RETURNING g_aaz641   #FUN-B50001
         CALL s_get_aaz641(g_dbs_axz03) RETURNING g_aaw01
         LET l_sql = "SELECT aag01,aag02 FROM ",g_dbs_axz03 CLIPPED,"aag_file ",
   ...
   PREPARE g006_gir_p5 FROM l_sql
   DECLARE g006_gir_cs5 CURSOR FOR g006_gir_p5
   ...
   LET l_sql = "SELECT '','',gis02,gis03,gis04,gis05,gis06,axz03,axz04 ",                                                        
               "  FROM gis_file,gir_file,axz_file",                                                                              
               " WHERE gis01 = gir01 AND axz01 = gis06 AND gir04='Y' AND gis01 = ?"                                              
   PREPARE g006_p7 FROM l_sql                                                
   DECLARE g006_cu7 CURSOR FOR g006_p7                                       
   FOREACH g006_gir_cs5 INTO gir.*                                                    
      LET l_amt_s = 0                                                        
      LET l_i_1 = 1  #FUN-940035 add
      FOREACH g006_cu7 USING gir.gir01 INTO tmp.*
         CALL s_aaz641_dbs(tmp.gis05,tmp.gis06) RETURNING g_dbs_axz03
         #CALL s_get_aaz641(g_dbs_axz03) RETURNING g_aaz641   #FUN-B50001
         CALL s_get_aaz641(g_dbs_axz03) RETURNING g_aaw01
         LET l_sql = "SELECT aag01,aag02 FROM ",g_dbs_axz03 CLIPPED,"aag_file ",
   ...
   IF tm.s = 'Y' THEN
      ...
      LET l_flag = 'N'
      LET g_cnt = 1
      DECLARE g006_gim CURSOR FOR
         SELECT gim01,gim02,gim03 FROM gim_file 
          WHERE gim00='N'
            AND gim04 = tm.y1  #No:FUN-B80158 add
            AND gim05 = tm.m2  #No:FUN-B80158 add
            AND gim06 = tm.axa01  #No:FUN-B80158 add
      FOREACH g006_gim INTO g_gim[g_cnt].*
         IF SQLCA.sqlcode THEN
...
###GENGRE###   CALL cl_prt_cs3('aglg006','aglg006',g_sql,g_str)
    CALL aglg006_grdata()    ###GENGRE###
...
#            AND aaf02 = g_rlang
#     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aglg006'
#     IF g_len = 0 OR g_len IS NULL THEN LET g_len = 80 END IF
#     FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR
#     FOR g_i = 1 TO 70 LET g_dash2[g_i,g_i] = '-' END FOR
# 
#     #CALL cl_outnam('aglg006') RETURNING l_name      #No.FUN-780058
# 
#     ## TQC-650058 By Joe -------------------------------------
#     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aglg006'
...
# 
#     #No.FUN-780058---Begin
#     #START REPORT g006_rep TO l_name
#     #LET g_pageno = 0
#         
#     #--------CR----------------------------------------------------
#     #制表日期,期間,頁次                                                       
...
#           CASE tmp.gis04                                                       
#             WHEN '1'   #異動                                                   
#              #CALL g006_axh(tmp.aag01,tm.y2,tm.m2,l_amt)                    #FU
#               CALL g006_axh(tmp.aag01,tm.y2,tm.m2,l_last_y,l_last_m,l_amt)  #FU
#                    RETURNING l_amt    
#               IF cl_null(l_amt) THEN LET l_amt = 0 END IF                      
#             WHEN '2'   #期初                                                   
#               CALL g006_axh2(tmp.aag01,l_last_y,l_last_m) RETURNING l_amt      
...
#      LET l_sub_amt = l_sub_amt*tm.q/g_unit  #依匯率及單位換算                  
#      CALL g006_str(l_sub_amt,l_str3) RETURNING l_str3                         
#      LET l_sub_amt = 0                                                         
#      LET l_amt_s = 0       
# 
#      #----------------------投資活動之現金流量-----------------------
...
#         LET l_sub_amt = l_sub_amt + l_amt_s                                    
#         LET l_amt_s = l_amt_s*tm.q/g_unit       #依匯率及單位換算              
#            CALL g006_str(l_amt_s,l_str4) RETURNING l_str4                      
#      END FOREACH                                                               
#      LET l_tot_amt = l_tot_amt + l_sub_amt  #計算本期現金凈增數                
#      LET l_sub_amt = l_sub_amt*tm.q/g_unit  #依匯率及單位換算                  
#      CALL g006_str(l_sub_amt,l_str5) RETURNING l_str5                         
#      LET l_sub_amt = 0                                                         
#      LET l_amt_s = 0                                                           
#                                                                                
#      #-------------------理財活動之現金流量------------------------------      
...
#         LET l_sub_amt = l_sub_amt + l_amt_s                                    
#         LET l_amt_s= l_amt_s*tm.q/g_unit       #依匯率及單位換算               
#         CALL g006_str(l_amt_s,l_str6) RETURNING l_str6                        
#      END FOREACH                                                               
#      LET l_tot_amt = l_tot_amt + l_sub_amt  #計算本期現金凈增數                
#      LET l_sub_amt = l_sub_amt*tm.q/g_unit  #依匯率及單位換算                  
#         CALL g006_str(l_sub_amt,l_str7) RETURNING l_str7                         
#                                                                                
###########################################################################      
#      LET l_amt = l_tot_amt                  #本期現金凈增數                    
#      LET l_amt = l_amt*tm.q/g_unit          #依匯率及單位換算                  
#      CALL g006_str(l_amt,l_str8) RETURNING l_str8                             
# 
#      #-------------------期初現金及約當現金餘額 -------------------------      
#      #SKIP 1 LINE                                                               
#      LET l_amt = 0                                                             
...
#         LET l_amt_s= l_amt_s*tm.q/g_unit       #依匯率及單位換算               
#         LET l_this  = l_amt_s                                                  
#         CALL g006_str(l_amt_s,l_str9) RETURNING l_str9                       
#      END FOREACH      
#      #--------------------期未現金及約當現金餘額---------------------          
#      INITIALIZE gir.* TO NULL                                                  
#      #SKIP 1 LINE                                                               
...
#         LET l_amt_s= l_amt_s*tm.q/g_unit       #依匯率及單位換算               
#         LET l_last = l_amt_s                                                   
#         CALL g006_str(l_amt_s,l_str10) RETURNING l_str10                        
#      END FOREACH                                                               
#      LET l_amt = l_tot_amt                  #本期現金凈增數                    
#      LET l_amt = l_amt*tm.q/g_unit          #依匯率及單位換算                  
#                                                                                
...
#     #OUTPUT TO REPORT g006_rep()
# 
#     #FINISH REPORT g006_rep
# 
...
#                 " ON A.title=B.title" 
#     CALL cl_prt_cs3('aglg006','aglg006',l_sql,g_str)
#     #No.FUN-780058---End 
#END FUNCTION
#end CHI-AC0033 mark
...
#     #---------------------本期淨利(損)------------------------------
#    #str FUN-780068 mod 09/20
#    #CALL g006_axh(g_aaz.aaz31,tm.y1,tm.m2,l_amt) RETURNING l_amt
#     SELECT SUM(axh09) INTO l_amt FROM axh_file
#      WHERE axh00=tm.b                           #帳別  
...
#     ELSE
#        CALL g006_str(l_amt,l_str) RETURNING l_str
...
#     LET l_sql="SELECT gir01,gir02 FROM gir_file ",
#               " WHERE gir03 = '1' AND gir04='Y'"
#     PREPARE g006_gir_p11 FROM l_sql
#     DECLARE g006_gir_cs11 CURSOR FOR g006_gir_p11
 
#     LET l_sql="SELECT aag01,aag02,gis03,gis04 ",
#               "  FROM aag_file,gis_file,gir_file",
#               " WHERE aag01=gis02 AND gis01=gir01 AND aag00='",tm.b,"' ",  #No.FUN-740020
#               " AND gir04='Y' AND gis01 = ? "
#     PREPARE g006_p3 FROM l_sql
#     DECLARE g006_cu3 CURSOR FOR g006_p3
#     FOREACH g006_gir_cs11 INTO gir.*
#       LET l_amt_s = 0
#       FOREACH g006_cu3 USING gir.gir01 INTO tmp.*
#          CASE tmp.gis04
#            WHEN '1'   #異動
#             #CALL g006_axh(tmp.aag01,tm.y2,tm.m2,l_amt)                    #FUN-780068 mark 09/20
#              CALL g006_axh(tmp.aag01,tm.y2,tm.m2,l_last_y,l_last_m,l_amt)  #FUN-780068      09/20
#                   RETURNING l_amt
#              IF cl_null(l_amt) THEN LET l_amt = 0 END IF
#            WHEN '2'   #期初
#              CALL g006_axh2(tmp.aag01,l_last_y,l_last_m) RETURNING l_amt
...
#       ELSE
#          CALL g006_str(l_amt_s,l_str) RETURNING l_str
...
#     ELSE
#        CALL g006_str(l_sub_amt,l_str) RETURNING l_str
#        PRINT COLUMN 59,l_str,')'
#     END IF
#     LET l_sub_amt = 0
#     LET l_amt_s = 0
...
#     LET l_sql="SELECT gir01,gir02 FROM gir_file ",
#               " WHERE gir03 = '2' AND gir04='Y' "
#     PREPARE g006_gir_p21 FROM l_sql
#     DECLARE g006_gir_cs21 CURSOR FOR g006_gir_p21
 
#     LET l_sql="SELECT aag01,aag02,gis03,gis04 ",
#             " FROM aag_file,gis_file,gir_file",
#             " WHERE aag01=gis02 AND gis01=gir01 AND gir04='Y' AND gis01 = ?  AND aag00='",tm.b,"' " #No.FUN-740020
#     PREPARE g006_p41 FROM l_sql
#     DECLARE g006_cu41 CURSOR FOR g006_p41
#     FOREACH g006_gir_cs21 INTO gir.*
#        LET l_amt_s = 0
#        FOREACH g006_cu41 USING gir.gir01 INTO tmp.*
#          CASE tmp.gis04
#            WHEN '1'
#             #CALL g006_axh(tmp.aag01,tm.y2,tm.m2,l_amt)                    #FUN-780068 mark 09/20
#              CALL g006_axh(tmp.aag01,tm.y2,tm.m2,l_last_y,l_last_m,l_amt)  #FUN-780068      09/20
...
#     LET l_sql="SELECT gir01,gir02 FROM gir_file ",
#               " WHERE gir03 = '3' AND gir04='Y' "
#     PREPARE g006_gir_p31 FROM l_sql
#     DECLARE g006_gir_cs31 CURSOR FOR g006_gir_p31
 
#     LET l_sql="SELECT aag01,aag02,gis03,gis04 ",
#            " FROM aag_file,gis_file,gir_file",
#            " WHERE aag01=gis02 AND gir01=gis01 AND gir04='Y' AND gis01 = ?  AND aag00='",tm.b,"'" #No.FUN-740020
#     PREPARE g006_p51 FROM l_sql
#     DECLARE g006_cu51 CURSOR FOR g006_p51
#     FOREACH g006_gir_cs31 INTO gir.*
#        LET l_amt_s = 0
#        FOREACH g006_cu51 USING gir.gir01 INTO tmp.*
#          CASE tmp.gis04
#            WHEN '1'
#             #CALL g006_axh(tmp.aag01,tm.y2,tm.m2,l_amt)                    #FUN-780068 mark 09/20
#              CALL g006_axh(tmp.aag01,tm.y2,tm.m2,l_last_y,l_last_m,l_amt)  #FUN-780068      09/20
...
#     LET l_sql="SELECT gir01,gir02 FROM gir_file ",
#               " WHERE gir03 = '4' AND gir04='Y' "
#     PREPARE g006_gir_p41 FROM l_sql
#     DECLARE g006_gir_cs41 CURSOR FOR g006_gir_p41
 
#     LET l_sql="SELECT aag01,aag02,gis03,gis04 ",
#              " FROM aag_file,gis_file,gir_file",
#              " WHERE aag01=gis02 AND gir01=gis01 AND gir04='Y' AND gis01 = ? AND aag00='",tm.b,"'"  #No.FUN-740020
#     PREPARE g006_p61 FROM l_sql
#     DECLARE g006_cu61 CURSOR FOR g006_p61
#     FOREACH g006_gir_cs41 INTO gir.*
#        LET l_amt_s = 0
#        FOREACH g006_cu6 USING gir.gir01 INTO tmp.*
#          CASE tmp.gis04
#            WHEN '1'   #異動
...
#     LET l_sql="SELECT gir01,gir02 FROM gir_file ",
#               " WHERE gir03 = '5' AND gir04='Y' "
#     PREPARE g006_gir_p51 FROM l_sql
#     DECLARE g006_gir_cs51 CURSOR FOR g006_gir_p51
 
#     LET l_sql="SELECT aag01,aag02,gis03,gis04 ",
#             " FROM aag_file,gis_file,gir_file",
#             " WHERE aag01=gis02 AND gis01=gir01 AND gir04='Y' AND gis01 = ? AND aag00='",tm.b,"'"  #No.FUN-740020
#     PREPARE g006_p71 FROM l_sql
#     DECLARE g006_cu71 CURSOR FOR g006_p71
#     FOREACH g006_gir_cs51 INTO gir.*
#        LET l_amt_s = 0
#        FOREACH g006_cu71 USING gir.gir01 INTO tmp.*
#          CASE tmp.gis04
#            WHEN '1'
#             #CALL g006_axh(tmp.aag01,tm.y2,tm.m2,l_amt)                    #FUN-780068 mark 09/20
#              CALL g006_axh(tmp.aag01,tm.y2,tm.m2,l_last_y,l_last_m,l_amt)  #FUN-780068      09/20
...
#        SKIP 1 LINE
#        LET g_cnt = 1
#        DECLARE g006_gim1 CURSOR FOR
#         SELECT gim02,gim03 FROM gim_file WHERE gim00 = 'Y'
#        FOREACH g006_gim1 INTO g_gim[g_cnt].*
#          IF SQLCA.sqlcode THEN
#              CALL cl_err('foreach:',SQLCA.sqlcode,1)  
#              EXIT FOREACH
#          END IF
...
FUNCTION g006_axh(p_axh05,p_axh06,p_axh07,p_last_y,p_last_m,p_value)   #FUN-780068 mod 09/20
...
FUNCTION g006_axh2(p_axh05,p_axh06,p_axh07)
   DEFINE p_axh05     LIKE axh_file.axh05,
          p_axh06     LIKE axh_file.axh06,
          p_axh07     LIKE axh_file.axh07,
          p_value     LIKE axh_file.axh08     #No.FUN-680098  decimal(20,6)
...
FUNCTION g006_axh_dc(p_axh05,p_dc,p_last_y,p_last_m)
   DEFINE p_axh05     LIKE axh_file.axh05,
...
#FUN-A90032 --Begin
FUNCTION g006_set_entry()
  CALL cl_set_comp_entry("q1,m2,h1",TRUE)
END FUNCTION

FUNCTION g006_set_no_entry()
   IF tm.axa06 ="1" THEN
...
FUNCTION aglg006_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg006")
        IF handler IS NOT NULL THEN
            START REPORT aglg006_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg006_datacur1 CURSOR FROM l_sql
            FOREACH aglg006_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg006_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg006_rep
        END IF
...
REPORT aglg006_rep(sr1)
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX g_user,g_pdate,g_prog,g_company,g_user_name,g_ptime  #FUN-B80158  add _user_name,g_ptime
            PRINTX tm.*
            #FUN-B80158-----add----str---
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B80158-------add----str----
            IF cl_null(sr1.gir02) AND cl_null(sr1.str2) THEN
               LET l_display = 'Y'
            ELSE
               LET l_display = 'N'
            END IF
