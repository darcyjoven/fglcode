...
# Modify.........: No.MOD-970236 09/07/27 By mike 將aglg101_file里的maj20e改為定義成LIKE maj_file.maj20e                            
...
TYPE sr1_t RECORD
    ...
    l_str1 LIKE type_file.chr20,
    l_str2 LIKE type_file.chr20,
    l_str3 LIKE type_file.chr20,
    l_str4 LIKE type_file.chr20,
    l_str5 LIKE type_file.chr20,
    l_str6 LIKE type_file.chr20,
    l_str7 LIKE type_file.chr20,
    l_str8 LIKE type_file.chr20,
    l_str9 LIKE type_file.chr20,
    l_str10 LIKE type_file.chr20,
...
#No.FUN-830053 -- begin --
   LET g_sql = "maj02.maj_file.maj02,",
               ...
               "line.type_file.num5"
   LET l_table = cl_prt_temptable('aglg101',g_sql) CLIPPED
   IF l_table = -1 THEN 
...
#No.FUN-830053 -- end --
#FUN-680098-BEGIN
   CREATE TEMP TABLE aglg101_file(
...
#FUN-680098-END
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL g101_tm()                        # Input print condition
   ELSE
      CALL g101()
   END IF
...
FUNCTION g101_tm()
   ...
   OPEN WINDOW g101_w AT p_row,p_col WITH FORM "agl/42f/aglg101"
...
#               CALL FGL_DIALOG_SETBUFFER( tm.b )
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglg101',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW g101_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)               #FUN-B80158 add
...
#   DEFINE l_amt         LIKE type_file.num20_6                 #No.FUN-680098    dec(20,6)  #No.FUN-830053 mark
   ...
   SELECT aaf03 INTO g_company FROM aaf_file WHERE aaf01 = tm.b
      AND aaf02 = g_rlang
   #FUN-6C0012.....begin
#   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aglg101'
#   IF g_len = 0 OR g_len IS NULL THEN LET g_len = 223 END IF
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aglg101'
#No.FUN-830053 -- begin --
...
#No.FUN-830053 -- end --
   ...
   LET l_sql = "SELECT * FROM maj_file",
               ...
               " ORDER BY maj02"
   PREPARE g101_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)               #FUN-B80158 add
      EXIT PROGRAM 
   END IF
   DECLARE g101_c CURSOR FOR g101_p
...
#   CALL cl_outnam('aglg101') RETURNING l_name  #No.FUN-830053
...
#控制一次印十個部門---------------------------------
   ...
   FOR l_i = 10 TO l_cnt STEP 10
      LET g_flag = 'n'
     #LET l_name1='g101_',l_i/10 USING '&&&','.out' #MOD-970245
#     START REPORT g101_rep TO l_name1   #No.FUN-830053 mark
      LET g_cn = 0
      LET g_pageno = 0
      DELETE FROM aglg101_file
...
#                  LET l_str = NULL                                           #No.FUN-830053 mark
               ...
               END IF
#No.FUN-830053 -- begin --
#              OUTPUT TO REPORT g101_rep(sr1.*,l_str)
               IF sr1.maj04 = 0 THEN
...
#No.FUN-830053 -- end --
      ...
      ELSE
         ...
         DECLARE tmp_curs CURSOR FOR
          SELECT * FROM aglg101_file ORDER BY maj02,no
...
#                  LET l_str =NULL                                     #No.FUN-830053
               ...
               END IF
#No.FUN-830053 -- begin --
#              OUTPUT TO REPORT g101_rep(sr.maj02,sr.maj03,sr.maj04,sr.maj05,
...
#     FINISH REPORT g101_rep   #No.FUN-830053
#MOD-970245   ---start       
...
###GENGRE###               tm.bm USING'&&',";",tm.em USING'&&',";",tm.d,";",tm.e
###GENGRE###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
###GENGRE###   CALL cl_prt_cs3('aglg101','aglg101',g_sql,g_str)
    CALL aglg101_grdata()    ###GENGRE###
#No.FUN-830053 -- end --
#---------------------------------------------------
END FUNCTION
 
FUNCTION g101_process(l_cn)
...
#No.MOD-860310 --end
    ...
    PREPARE g101_sum FROM l_sql
    DECLARE g101_sumc CURSOR FOR g101_sum
...
#No.MOD-860310 --end
                "   AND aao02 IN (",g_buf CLIPPED,")"   #---- g_buf 部門族群
    PREPARE g101_sum1 FROM l_sql
    DECLARE g101_sumc1 CURSOR FOR g101_sum1
    IF STATUS THEN CALL cl_err('sum prepare',STATUS,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
       CALL cl_gre_drop_temptable(l_table)               #FUN-B80158 add
       EXIT PROGRAM 
    END IF
    FOREACH g101_c INTO maj.*
       ...
       IF NOT cl_null(maj.maj21) THEN
          IF maj.maj06 = '1' OR maj.maj06='3' THEN # Thomas99/01/11 取前期餘額
             ...
             OPEN g101_sumc1 USING tm.b,maj.maj21,maj.maj22,tm.yy,l_mm
             FETCH g101_sumc1 INTO amt1
          ELSE
             OPEN g101_sumc USING tm.b,maj.maj21,maj.maj22,tm.yy,tm.bm,g_mm
             FETCH g101_sumc INTO amt1
          END IF
...
# Thomas 98/11/15 GL014
	  IF maj.maj03 = '%' THEN
	     ...
	     SELECT bal1 INTO l_sun FROM aglg101_file WHERE no=l_cn
	            AND maj02=l_temp
	     LET l_temp = maj.maj22
	     SELECT bal1 INTO l_mon FROM aglg101_file WHERE no=l_cn
	            AND maj02=l_temp
...
# Thomas 99/01/11
       ...
       END IF
       INSERT INTO aglg101_file VALUES(l_cn,maj.maj02,maj.maj03,maj.maj04,
                                    maj.maj05,maj.maj07,maj.maj20,maj.maj20e,
                                    m_bal1)
       IF STATUS OR SQLCA.SQLERRD[3] = 0 THEN
#         CALL cl_err('ins g101_file',STATUS,1)  #No.FUN-660123
          CALL cl_err3("ins","aglg101_file",l_cn,maj.maj02,STATUS,"","ins g101_file",1)   #No.FUN-660123
          EXIT FOREACH 
       END IF
    END FOREACH
END FUNCTION
...
FUNCTION g101_total()
    ...
    DECLARE tot_curs CURSOR FOR
      SELECT maj02,SUM(bal1) FROM aglg101_file #WHERE maj20 IS NOT NULL
...
FUNCTION aglg101_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg101")
        IF handler IS NOT NULL THEN
            START REPORT aglg101_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY page,maj02,line"     #FUN-B80158 add
          
            DECLARE aglg101_datacur1 CURSOR FROM l_sql
            FOREACH aglg101_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg101_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg101_rep
        END IF
...
REPORT aglg101_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B80158-------add----str---
    DEFINE l_maj20     STRING
    DEFINE l_title2    LIKE mai_file.mai02
    DEFINE l_unit      STRING
    DEFINE l_maj20_1   STRING 
    #FUN-B80158-------add----end---
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            #FUN-B80158------add-----str----- 
            IF tm.e = 'Y' THEN 
               LET l_maj20_1 = sr1.maj20e
               LET l_maj20_1 = l_maj20_1.trimright()
               LET l_maj20 = sr1.maj05 spaces,l_maj20_1
            ELSE 
               LET l_maj20_1 = sr1.maj20
               LET l_maj20_1 = l_maj20_1.trimright()
               LET l_maj20 = sr1.maj05 spaces,l_maj20_1
            END IF
            PRINTX l_maj20
