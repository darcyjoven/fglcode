...
# Modify.........: No.MOD-C30895 12/04/02 By Polly 取消重復減1的動作
# Modify.........: No.MOD-C50009 12/05/02 By Elise 固定資產週轉率 = 銷貨淨額 / 平均固定資產
#                                                      資產週轉率 = 銷貨淨額 / 平均資產總額
# Modify.........: No.TQC-C50076 12/05/09 By xuxz 流動資產週轉率（次）=主營業務收入淨額/平均流動資產總額*100%
#                                                 資產報酬率=(淨利潤+利息費用+所得稅)/平均資產總額*100%
# Modify.........: No.MOD-C80102 12/08/14 By Polly 調整營業利益=(23類*-1)+(24類*-1)-25類-26類-27類
# Modify.........: No.MOD-C90050 12/09/10 By Polly 流動資產週轉率、固定資產週轉率、資產週轉率取消 *100
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglr401',g_sql) CLIPPED    
   ...
   LET g_bookno = ARG_VAL(34)    #CHI-B70028 add
   LET g_rep_user = ARG_VAL(35)
   LET g_rep_clas = ARG_VAL(36)
   LET g_template = ARG_VAL(37)
   LET g_rpt_name = ARG_VAL(38)  #No.FUN-7C0078
   IF cl_null(g_bookno) THEN
      LET g_bookno = g_aaz.aaz64
   END IF
   IF cl_null(g_bgjob) OR g_bgjob = 'N'            # If background job sw is off
      THEN CALL aglr401_tm()                    # Input print condition
      ELSE CALL aglr401()                       # Read data and create out-file
   END IF
...
FUNCTION aglr401_tm()
   ...
   END IF
   OPEN WINDOW aglr401_w AT p_row,p_col
        WITH FORM "agl/42f/aglr401"
...
#------------------------------CHI-B70028----------------------------------------end
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW aglr401_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         LET l_cmd = l_cmd CLIPPED,             #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_bookno CLIPPED,"'",             #CHI-B70028 add
                         " '",g_rep_user CLIPPED,"'",           #No.FUN-570264
                         " '",g_rep_clas CLIPPED,"'",           #No.FUN-570264
                         " '",g_template CLIPPED,"'",           #No.FUN-570264
                         " '",g_rpt_name CLIPPED,"'"            #No.FUN-7C0078
         CALL cl_cmdat('aglr401',g_time,l_cmd)  # Execute cmd at later time
      END IF
      CLOSE WINDOW aglr401_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#  DEFINE l_aaz31       LIKE aaz_file.aaz31               #CHI-A40061 add  #FUN-BC0027
   DEFINE l_aaa14       LIKE aaa_file.aaa14                #FUN-BC0027
...
#     SELECT aaz31 INTO l_aaz31 FROM aaz_file  #CHI-A40061 add #FUN-BC0027
      SELECT aaa14 INTO l_aaa14 FROM aaa_file WHERE aaa01 = g_bookno  #FUN-BC0027     
...
#                   "   AND aah01 != '",l_aaz31,"'",              #MOD-A60008  #FUN-BC0027
                    "   AND aah01 != '",l_aaa14,"'",              #FUN-BC0027
                    "   AND aah02 = ? ",
                    "   AND aah03 <=  ? ",
                    "   AND aag04 = '1'",
        ...
        DECLARE aglr401_c1 CURSOR FOR aglr401_p1
        FOREACH aglr401_c1 USING l_yy[g_i],l_em[g_i] INTO l_aag06,l_aag19,l_tot
          IF SQLCA.sqlcode != 0 THEN
        ...
        END IF
        # 期初
        FOREACH aglr401_c1 USING l_yy1,l_bm1 INTO l_aag06,l_aag19,l_tot
          IF SQLCA.sqlcode != 0 THEN
                 CALL cl_err('foreach:',SQLCA.sqlcode,1)
                 EXIT FOREACH
          END IF
...
#                   "   AND aah01 != '",l_aaz31,"'",      #MOD-AB0115  #FUN-BC0027
                    "   AND aah01 != '",l_aaa14,"'",      #FUN-BC0027
                    "   AND aah02 = ? ",
                    "   AND aah03 >= ",l_bm[g_i],
                    "   AND aah03 <= ",l_em[g_i],
        ...
        DECLARE aglr401_c2 CURSOR FOR aglr401_p2
        FOREACH aglr401_c2 USING l_yy[g_i] INTO l_aag06,l_aag19,l_tot
          IF SQLCA.sqlcode != 0 THEN
             CALL cl_err('foreach:',SQLCA.sqlcode,1)
             EXIT FOREACH
          END IF
        ...
        #去年同期
        FOREACH aglr401_c2 USING l_yyb INTO l_aag06,l_aag19,l_tot
          IF SQLCA.sqlcode != 0 THEN
             CALL cl_err('foreach:',SQLCA.sqlcode,1)
             EXIT FOREACH
          END IF
     ...
     CALL cl_prt_cs3('aglr401','aglr401',l_sql,g_str)
END FUNCTION
 
FUNCTION aglr401_ins_temp()
   ...
   DEFINE sr    DYNAMIC ARRAY OF RECORD
                 ...
                 v      LIKE type_file.num20_6     #上期流動資產#TQC-C50076 add
                END RECORD 
   DEFINE  sr1  RECORD                                                          
   ...
   #TQC-C50076--add--str
   #上期流動資產的運算=1類+2類+3類+4類+5類+6類+7類+8類
   FOR g_i = 1 TO 8
       LET sr[1].v = sr[1].v + g_tot_b[1,g_i]   #第一期
       LET sr[2].v = sr[2].v + g_tot_b[2,g_i]   #第二期
       LET sr[3].v = sr[3].v + g_tot_b[3,g_i]   #第三期
       LET sr[4].v = sr[4].v + g_tot_b[4,g_i]   #第四期
       LET sr[5].v = sr[5].v + g_tot_b[5,g_i]   #第五期
   END FOR
   #TQC-C50076--add--end
  ...
  #-MOD-AB0186-end-
   ...
   FOR g_i = 1 TO 5
       ...
       IF sr[g_i].e != 0 THEN
         ...
         #資產報酬率=(淨利潤+利息費用+所得稅)/平均資產總額*100%
          LET l_tot[g_i,25] = (sr[g_i].p+g_tot[g_i,31]+g_tot[g_i,29])/
                              ((sr[g_i].e+sr[g_i].t)/2) * 100
         #TQC-C50076--mod--end

                      #資產週轉率 (銷貨淨額/資產總額)
         #LET l_tot[g_i,27] = (sr[g_i].f/sr[g_i].e)                        #MOD-C50009 mark
         #LET l_tot[g_i,27] = (sr[g_i].f/((sr[g_i].e+sr[g_i].t)/2)) * 100  #MOD-C50009 #MOD-C90050 mark
          LET l_tot[g_i,27] = (sr[g_i].f/((sr[g_i].e+sr[g_i].t)/2))        #MOD-C90050 add
       END IF
       #流動資產不為零
       IF sr[g_i].a != 0 THEN
         #TQC-C50076--mod--str
         ...
         #            #流動資產週轉率（次）=主營業務收入淨額/平均流動資產總額*100%
         #LET l_tot[g_i,6] = (sr[g_i].f/((sr[g_i].a+sr[g_i].v)/2))*100  #MOD-C90050 mark
          LET l_tot[g_i,6] = (sr[g_i].f/((sr[g_i].a+sr[g_i].v)/2))      #MOD-C90050 add
         #TQC-C50076--mod--end
                      #現金佔流動資產比率 (現金/流動資產)
       ...
       IF g_tot[g_i,12] != 0 THEN
              #固定資產週轉率 (銷貨淨額/固定資產)
         #LET l_tot[g_i,28] = (sr[g_i].f/g_tot[g_i,12])                              #MOD-C50009 mark
         #LET l_tot[g_i,28] = (sr[g_i].f/((g_tot[g_i,12]+g_tot_b[g_i,12])/2)) * 100  #MOD-C50009 #MOD-C90050 mark
          LET l_tot[g_i,28] = (sr[g_i].f/((g_tot[g_i,12]+g_tot_b[g_i,12])/2))        #MOD-C90050 add
  ...
  #決定金額取位的FORMAT
   ...
   LET sr1.num01 = sr[1].f USING l_format1  #'##,###,###,###,##&'
  #LET sr1.num02 = sr[1].a USING l_format2  #'<<,<<<,<<<,<<<,<<&'              #MOD-C90050 mark
   LET sr1.num02 = (sr[1].a+sr[1].v)/2 USING l_format2  #'<<,<<<,<<<,<<<,<<&'  #MOD-C90050
   ...
   LET sr1.num01 = sr[1].f USING l_format1      #'##,###,###,###,##&'
  #LET sr1.num02 = g_tot[1,12] USING l_format2  #'<<,<<<,<<<,<<<,<<&'                   #MOD-C90050 mark
   LET sr1.num02 = (g_tot[1,12]+g_tot_b[1,12])/2 USING l_format2  #'<<,<<<,<<<,<<<,<<&' #MOD-C90050
   ...
   LET sr1.num01 = sr[1].f USING l_format1  #'##,###,###,###,##&'
  #LET sr1.num02 = sr[1].e USING l_format2  #'<<,<<<,<<<,<<<,<<&'             #MOD-C90050 mark
   LET sr1.num02 = (sr[1].e+sr[1].t)/2 USING l_format2  #'<<,<<<,<<<,<<<,<<&' #MOD-C90050
