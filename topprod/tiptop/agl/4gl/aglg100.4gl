...
# Modify.........: No.FUN-B90027 11/09/23 By Wangning 明細CR報表轉GR
# Modify.........: No.FUN-B90027 12/01/05 By qirl FUN-BB0047追单
# Modify.........: No.FUN-B90027 12/01/17 By yangtt MOD-BC0237追单
...
DEFINE tm  RECORD
      ...
      #g_buf      LIKE type_file.chg1000,   #No.FUN-680098     char(600)  #MOD-B40245 mark 
...
TYPE sr1_t RECORD
    ...
    m_dept1 LIKE gem_file.gem02,
    m_dept2 LIKE gem_file.gem02,
    m_dept3 LIKE gem_file.gem02,
    m_dept4 LIKE gem_file.gem02,
    m_dept5 LIKE gem_file.gem02,
    m_dept6 LIKE gem_file.gem02,
    m_dept7 LIKE gem_file.gem02,
    m_dept8 LIKE gem_file.gem02,
    m_dept9 LIKE gem_file.gem02,
    m_dept10 LIKE gem_file.gem02,
    l_amt1 LIKE type_file.num20_6,
    l_amt2 LIKE type_file.num20_6,
    l_amt3 LIKE type_file.num20_6,
    l_amt4 LIKE type_file.num20_6,
    l_amt5 LIKE type_file.num20_6,
    l_amt6 LIKE type_file.num20_6,
    l_amt7 LIKE type_file.num20_6,
    l_amt8 LIKE type_file.num20_6,
    l_amt9 LIKE type_file.num20_6,
...
MAIN
   ...
   IF (NOT cl_setup("AGL")) THEN
      EXIT PROGRAM
   END IF
 #  CALL cl_used(g_prog,g_time,1) RETURNING g_time #No.FUN-690114#FUN-B900027
   ...
   LET g_sql="maj02.maj_file.maj02,maj03.maj_file.maj03,",
             ...
             "line.type_file.num5"
   LET l_table = cl_prt_temptable('aglg100',g_sql) CLIPPED
   IF l_table = -1 THEN
...
#CHI-B50007 -- begin --
   LET g_sql="maj02.maj_file.maj02,maj03.maj_file.maj03,",
             ...
             "line.type_file.num5"
   LET l_table1 = cl_prt_temptable('aglg1001',g_sql) CLIPPED
   IF l_table1 = -1 THEN 
      CALL cl_gre_drop_temptable(l_table)                 #FUN-B90027
      EXIT PROGRAM 
   END IF
#CHI-B50007 -- end --
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL g100_tm()                        # Input print condition
   ELSE
      CALL g100()
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
   CALL cl_gre_drop_temptable(l_table||"|"||l_table1)                 #FUN-B90027
END MAIN
 
FUNCTION g100_tm()
   DEFINE p_row,p_col    LIKE type_file.num5,          #No.FUN-680098 smallint
          l_sw           LIKE type_file.chr1,          #重要欄位是否空白  #No.FUN-680098  VARCHAR(1)   
          l_cmd          LIKE type_file.chr1000        #No.FUN-680098   VARCHAR(400)   #FUN-B90027  chg1000-->chr1000
   ...
   OPEN WINDOW g100_w AT p_row,p_col WITH FORM "agl/42f/aglg100"
...
WHILE TRUE
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglg100',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW g100_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1)                 #FUN-B90027
...
FUNCTION g100()
  ...
  #DEFINE l_sql     LIKE type_file.chg1000        # RDSQL STATEMENT        #No.FUN-680098   char(1000)  #MOD-B40245 mark 
   ...
   DEFINE l_cmd,l_cmd1,l_cmd2  LIKE type_file.chr1000       #No.FUN-680098  VARCHAR(400)    #FUN-B90027  chg1000-->chr1000
   ...
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aglg100'
...
#CHI-B50007 -- end --
   ...
   LET l_sql = "SELECT * FROM maj_file",
               ...
               " ORDER BY maj02"
   PREPARE g100_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1)                 #FUN-B90027
      EXIT PROGRAM 
   END IF
   DECLARE g100_c CURSOR FOR g100_p
...
#控制一次印十個部門---------------------------------
   ...
   FOR l_i = 10 TO l_cnt STEP 10
      LET g_flag = 'n'
      LET l_name1='g100_',l_i/10 USING '&&&','.out'
 
      LET g_pageno = 0
 
      LET g_cn = 0
      DELETE FROM g100_file
      ...
      ELSE
         ...
         DECLARE tmp_curs CURSOR FOR
            SELECT * FROM g100_file ORDER BY maj02,no
      ...
      END IF
   END FOR
###GENGRE###   LET g_str = g_aaz.aaz77,";",g_mai02,";",tm.yy USING '<<<<',";",
###GENGRE###               tm.bm USING'&&',";",tm.em USING'&&',";",tm.d,";",tm.e,";",g_azi04   #NO.FUN-B90027
###GENGRE###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
###GENGRE###   CALL cl_prt_cs3('aglg100','aglg100',g_sql,g_str)
    CALL aglg100_grdata()    ###GENGRE###
END FUNCTION


#CHI-B50007
FUNCTION g100_not_page()
   DEFINE l_name1   LIKE type_file.chr20
   DEFINE l_chr     LIKE type_file.chr1  
   DEFINE l_leng,l_leng2  LIKE type_file.num5
   DEFINE l_gem02   LIKE gem_file.gem02
   ...
   LET g_str = g_aaz.aaz77,";",g_mai02,";",tm.yy USING '<<<<',";",
               tm.bm USING'&&',";",tm.em USING'&&',";",tm.d,";",tm.e,";",g_no
   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED
###GENGRE###   CALL cl_prt_cs3('aglg100','aglg100_1',g_sql,g_str)
    CALL aglg100_1_grdata()    ###GENGRE###
END FUNCTION

FUNCTION g100_process(l_cn)
  #DEFINE l_sql,l_sql1   LIKE type_file.chg1000   #No.FUN-680098 char(1000) #MOD-B40245 mark
   ...
   DEFINE l_mm            LIKE type_file.num5         #No.FUN-680098 smallint
    ...
    PREPARE g100_sum FROM l_sql
    DECLARE g100_sumc CURSOR FOR g100_sum
    IF STATUS THEN CALL cl_err('sum prepare',STATUS,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
       CALL cl_gre_drop_temptable(l_table||"|"||l_table1)                 #FUN-B90027
       EXIT PROGRAM 
    END IF
    FOREACH g100_c INTO maj.*
       IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
       LET amt1 = 0 LET amt  = 0
       IF NOT cl_null(maj.maj21) THEN
          OPEN g100_sumc USING tm.b,maj.maj21,maj.maj22,tm.yy,tm.bm,g_mm
          FETCH g100_sumc INTO amt1
	  ...
	  IF maj.maj03 = '%' THEN
	     ...
	     SELECT bal1 INTO l_sun FROM g100_file WHERE no=l_cn
	            AND maj02=l_temp
	     LET l_temp = maj.maj22
	     SELECT bal1 INTO l_mon FROM g100_file WHERE no=l_cn
	            AND maj02=l_temp
	     ...
	     ELSE
             ...
             END IF
          ELSE
          #  LET m_bal1=NULL     #FUN-B90027 mark
          #NO.FUN-B90027-----add--str---
             IF maj.maj03 = '5' THEN
                LET m_bal1 = amt1
             ELSE
                LET m_bal1 = NULL
             END IF
          #NO.FUN-B90027-----add--end---
          END IF
       END IF
       ...
       END IF
       INSERT INTO g100_file VALUES(l_cn,maj.maj02,maj.maj03,maj.maj04,
                                    maj.maj05,maj.maj07,maj.maj20,maj.maj20e,
                                    maj.maj21,maj.maj22,m_bal1)
       IF STATUS OR SQLCA.SQLERRD[3] = 0 THEN
          CALL cl_err3("ins","g100_file",l_cn,maj.maj02,SQLCA.sqlcode,"","ins g100_file",1)   #No.FUN-660123
          EXIT FOREACH
       END IF
    END FOREACH
END FUNCTION
 
FUNCTION g100_bom(l_dept,l_sw)
...
FUNCTION aglg100_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg100")
        IF handler IS NOT NULL THEN
            START REPORT aglg100_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg100_datacur1 CURSOR FROM l_sql
            FOREACH aglg100_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg100_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg100_rep
        END IF
...
FUNCTION aglg100_1_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg100")
        IF handler IS NOT NULL THEN
            START REPORT aglg100_1_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED
          
            DECLARE aglg100_datacur2 CURSOR FROM l_sql
            FOREACH aglg100_datacur2 INTO sr2.*
                OUTPUT TO REPORT aglg100_1_rep(sr2.*)
            END FOREACH
            FINISH REPORT aglg100_1_rep
        END IF
        IF INT_FLAG = TRUE THEN
            LET INT_FLAG = FALSE
            EXIT WHILE
...
REPORT aglg100_rep(sr1)
    ...
    DEFINE l_str         STRING
    DEFINE l_amt1        STRING
    DEFINE l_amt2        STRING
    DEFINE l_amt3        STRING
    DEFINE l_amt4        STRING
    DEFINE l_amt5        STRING
    DEFINE l_amt6        STRING
    DEFINE l_amt7        STRING
    DEFINE l_amt8        STRING
    DEFINE l_amt9        STRING
    DEFINE l_amt10       STRING
    DEFINE l_t_amt1      STRING
    DEFINE l_t_amt2      STRING
    DEFINE l_t_amt3      STRING
    DEFINE l_t_amt4      STRING
    DEFINE l_t_amt5      STRING
    DEFINE l_t_amt6      STRING
    DEFINE l_t_amt7      STRING
    DEFINE l_t_amt8      STRING
    DEFINE l_t_amt9      STRING
    DEFINE l_t_amt10     STRING
    DEFINE l_unit        STRING
    ...
    DEFINE l_display9    LIKE type_file.chr1
    DEFINE l_display10   LIKE type_file.chr1
    DEFINE l_length1     LIKE type_file.num5
    DEFINE l_length2     LIKE type_file.num5
    DEFINE l_length3     LIKE type_file.num5
    DEFINE l_length4     LIKE type_file.num5
    DEFINE l_length5     LIKE type_file.num5
    DEFINE l_length6     LIKE type_file.num5
    DEFINE l_length7     LIKE type_file.num5
    DEFINE l_length8     LIKE type_file.num5
    DEFINE l_length9     LIKE type_file.num5
    DEFINE l_length10    LIKE type_file.num5
    #FUN-B90027----add----end----------


    
    ORDER EXTERNAL BY sr1.page,sr1.maj02,sr1.line
    
    FORMAT
        FIRST PAGE HEADER
            PRINTX g_grPageHeader.*    
            PRINTX g_user,g_pdate,g_prog,g_company,g_ptime,g_user_name              #FUN-B90027
            #FUN-B90027----add----str--------
        ...
        ON EVERY ROW
            ...
            IF sr1.maj03 = '%' THEN
               LET l_amt1 = l_amt1 USING '---,---,--&.&&'
               LET l_amt1 = l_amt1.trim()
               LET l_t_amt1 = l_amt1,'%'
            ELSE
           #   LET l_amt1 = sr1.l_amt1 USING '---,---,--&'
               LET l_amt1 = cl_numfor(sr1.l_amt1,26,g_azi04)   #NO.FUN-B90027 
               LET l_amt1 = l_amt1.trim()
               LET l_t_amt1 = l_amt1
            END IF
            PRINTX l_t_amt1
            ...
            IF sr1.maj03 = '%' THEN
               LET l_amt2 = sr1.l_amt2 USING '---,---,--&.&&'
               LET l_amt2 = l_amt2.trim()
               LET l_t_amt2 = l_amt2,'%'
            ELSE
               LET l_amt2 = cl_numfor(sr1.l_amt2,26,g_azi04)   #NO.FUN-B90027 
               LET l_amt2 = l_amt2.trim()
               LET l_t_amt2 = l_amt2
            END IF
            PRINTX l_t_amt2
            ...
            IF sr1.maj03 = '%' THEN
               LET l_amt3 = sr1.l_amt3 USING '---,---,--&.&&'
               LET l_amt3 = l_amt3.trim()
               LET l_t_amt3 = l_amt3,'%'
            ELSE
            #  LET l_amt3 = sr1.l_amt3 USING '---,---,--&'
               LET l_amt3 = cl_numfor(sr1.l_amt3,26,g_azi04)  #NO.FUN-B90027 
               LET l_amt3 = l_amt3.trim()
               LET l_t_amt3 = l_amt3
            END IF
            PRINTX l_t_amt3
            ...
            IF sr1.maj03 = '%' THEN
               LET l_amt4 = sr1.l_amt4 USING '---,---,--&.&&'
               LET l_amt4 = l_amt4.trim()
               LET l_t_amt4 = l_amt4,'%'
            ELSE
            #  LET l_amt4 = sr1.l_amt4 USING '---,---,--&'
               LET l_amt4 = cl_numfor(sr1.l_amt4,26,g_azi04)  #NO.FUN-B90027 
               LET l_amt4 = l_amt4.trim()
               LET l_t_amt4 = l_amt4
            END IF
            PRINTX l_t_amt4
            ...
            IF sr1.maj03 = '%' THEN
               LET l_amt5 = sr1.l_amt5 USING '---,---,--&.&&'
               LET l_amt5 = l_amt5.trim()
               LET l_t_amt5 = l_amt5,'%'
            ELSE
       #       LET l_amt5 = sr1.l_amt5 USING '---,---,--&'
               LET l_amt5 = cl_numfor(sr1.l_amt5,26,g_azi04)    #NO.FUN-B90027 
               LET l_amt5 = l_amt5.trim()
               LET l_t_amt5 = l_amt5
            END IF
            PRINTX l_t_amt5
            ...
            IF sr1.maj03 = '%' THEN
               LET l_amt6 = sr1.l_amt6 USING '---,---,--&.&&'
               LET l_amt6 = l_amt6.trim()
               LET l_t_amt6 = l_amt6,'%'
            ELSE
       #       LET l_amt6 = sr1.l_amt6 USING '---,---,--&'
               LET l_amt6 = cl_numfor(sr1.l_amt6,26,g_azi04)    #NO.FUN-B90027 
               LET l_amt6 = l_amt6.trim()
               LET l_t_amt6 = l_amt6
            END IF
            PRINTX l_t_amt6
            ...
            IF sr1.maj03 = '%' THEN
               LET l_amt7 = sr1.l_amt7 USING '---,---,--&.&&'
               LET l_amt7 = l_amt7.trim()
               LET l_t_amt7 = l_amt7,'%'
            ELSE
  #            LET l_amt7 = sr1.l_amt7 USING '---,---,--&'
               LET l_amt7 = cl_numfor(sr1.l_amt7,26,g_azi04)    #NO.FUN-B90027 
               LET l_amt7 = l_amt7.trim()
               LET l_t_amt7 = l_amt7
            END IF
            PRINTX l_t_amt7
            ...
            IF sr1.maj03 = '%' THEN
               LET l_amt8 = sr1.l_amt8 USING '---,---,--&.&&'
               LET l_amt8 = l_amt8.trim()
               LET l_t_amt8 = l_amt8,'%'
            ELSE
  #            LET l_amt8 = sr1.l_amt8 USING '---,---,--&'
               LET l_amt8 = cl_numfor(sr1.l_amt8,26,g_azi04)   #NO.FUN-B90027 
               LET l_amt8 = l_amt8.trim()
               LET l_t_amt8 = l_amt8
            END IF
            PRINTX l_t_amt8
            ...
            IF sr1.maj03 = '%' THEN
               LET l_amt9 = sr1.l_amt9 USING '---,---,--&.&&'
               LET l_amt9 = l_amt9.trim()
               LET l_t_amt9 = l_amt9,'%'
            ELSE
   #           LET l_amt9 = sr1.l_amt9 USING '---,---,--&'
               LET l_amt9 = cl_numfor(sr1.l_amt9,26,g_azi04)    #NO.FUN-B90027 
               LET l_amt9 = l_amt9.trim()
               LET l_t_amt9 = l_amt9
            END IF
            PRINTX l_t_amt9
            ...
            IF sr1.maj03 = '%' THEN
               LET l_amt10 = sr1.l_amt10 USING '---,---,--&.&&'
               LET l_amt10 = l_amt10.trim()
               LET l_t_amt10 = l_amt10,'%'
            ELSE
               LET l_amt10 = cl_numfor(sr1.l_amt10,26,g_azi04)     #NO.FUN-B90027 add
               LET l_amt10 = l_amt10.trim()
               LET l_t_amt10 = l_amt10
            END IF
            PRINTX l_t_amt10
            ...
            LET l_length1 = LENGTH(sr1.m_dept1)
            PRINTX l_length1

            LET l_length2 = LENGTH(sr1.m_dept2)
            PRINTX l_length2
            
            LET l_length3 = LENGTH(sr1.m_dept3)
            PRINTX l_length3
            
            LET l_length4 = LENGTH(sr1.m_dept4)
            PRINTX l_length4
          
            LET l_length5 = LENGTH(sr1.m_dept5)
            PRINTX l_length5

            LET l_length6 = LENGTH(sr1.m_dept6)
            PRINTX l_length6

            LET l_length7 = LENGTH(sr1.m_dept7)
            PRINTX l_length7

            LET l_length8 = LENGTH(sr1.m_dept8)
            PRINTX l_length8

            LET l_length9 = LENGTH(sr1.m_dept9)
            PRINTX l_length9

            LET l_length10 = LENGTH(sr1.m_dept10)
            PRINTX l_length10

            IF cl_null(sr1.m_dept1) OR cl_null(l_t_amt1) THEN
               LET l_display1 = 'N'
            ELSE 
               LET l_display1 = 'Y'
            END IF
            ...
            IF cl_null(sr1.m_dept2) OR cl_null(l_t_amt2)THEN
               LET l_display2 = 'N'
            ELSE 
               LET l_display2 = 'Y'
            END IF
            ...
            IF cl_null(sr1.m_dept3) OR cl_null(l_t_amt3) THEN
               LET l_display3 = 'N'
            ELSE 
               LET l_display3 = 'Y'
            END IF
            ...
            IF cl_null(sr1.m_dept4)  OR cl_null(l_t_amt4) THEN
               LET l_display4 = 'N'
            ELSE 
               LET l_display4 = 'Y'
            END IF
            ...
            IF cl_null(sr1.m_dept5) OR cl_null(l_t_amt5) THEN
               LET l_display5 = 'N'
            ELSE 
               LET l_display5 = 'Y'
            END IF
            ...
            IF cl_null(sr1.m_dept6) OR cl_null(l_t_amt6) THEN
               LET l_display6 = 'N'
            ELSE 
               LET l_display6 = 'Y'
            END IF
            ...
            IF cl_null(sr1.m_dept7) OR cl_null(l_t_amt7) THEN
               LET l_display7 = 'N'
            ELSE 
               LET l_display7 = 'Y'
            END IF
            ...
            IF cl_null(sr1.m_dept8) OR cl_null(l_t_amt8) THEN
               LET l_display8 = 'N'
            ELSE 
               LET l_display8 = 'Y'
            END IF
            ...
            IF cl_null(sr1.m_dept9) OR cl_null(l_t_amt9) THEN
               LET l_display9 = 'N'
            ELSE 
               LET l_display9 = 'Y'
            END IF
        ...
        #   IF cl_null(sr1.m_dept10) AND cl_null(l_t_amt10) THEN
            IF cl_null(sr1.m_dept10) THEN
               LET l_display10 = 'N'
            ELSE 
               LET l_display10 = 'Y'
            END IF
