...
# Modify.........: No:FUN-B80161 11/08/31 By chenying 明細類CR轉GR
...
###GENGRE###START
TYPE sr1_t RECORD
    order1 LIKE type_file.chr100,   #FUN-B80161 add
    order2 LIKE type_file.chr100,   #FUN-B80161 add
    ...
    m_a LIKE type_file.num20_6,
    m_b LIKE type_file.num20_6,
    y_a LIKE type_file.num20_6,
    y_b LIKE type_file.num20_6,
    azi04 LIKE azi_file.azi04,
    azi05 LIKE azi_file.azi05
END RECORD
###GENGRE###END
DEFINE g_ord1_desc      STRING    #FUN-B80161 add
DEFINE g_ord2_desc      STRING    #FUN-B80161 add
...
MAIN
   ...
   #No.FUN-790008---Begin                                                                                                              
   LET g_sql = " order1.type_file.chr100,",   #FUN-B80161 add
               " order2.type_file.chr100,",   #FUN-B80161 add
           ...
           LET l_table = cl_prt_temptable('aglg809',g_sql) CLIPPED                                                                  
...
#NO.FUN-740020     --End
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL aglg809_tm(0,0)        # Input print condition
      ELSE CALL aglg809()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
   CALL cl_gre_drop_temptable(l_table)               #FUN-B80158 add
END MAIN
 
FUNCTION aglg809_tm(p_row,p_col)
...
#   CALL s_dsmark(g_bookno)               #NO.FUN-740020
   ...
   END IF
   OPEN WINDOW aglg809_w AT p_row,p_col
        WITH FORM "agl/42f/aglg809"
################################################################################
...
# 
#   IF INT_FLAG THEN
#      LET INT_FLAG = 0 CLOSE WINDOW aglg809_w 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
#      EXIT PROGRAM
#         
#   END IF
...
##     ON ACTION CONTROLP CALL aglg809_wc()   # Input detail Where Condition
#No.FUN-B20054--mark--end--
      AFTER INPUT
...
#                        " '",tm.afc01 CLIPPED,"'",          #No.FUN-830139
         ...
         CALL cl_cmdat('aglg809',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW aglg809_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)               #FUN-B80158 add
      EXIT PROGRAM
   END IF
...
#         l_order ARRAY[2] OF LIKE aed_file.aed02,#No.FUN-680098 VARCHAR(20)  #FUN-B80161 mark
          l_order   ARRAY[2] OF LIKE type_file.chr100,  #FUN-B80161 add 
...
#    LET l_sql = "SELECT aee01, aag02, aag13, aag06, aee03,aed03,aed04,aee04",   #FUN-6C0012  #09/10/20 xiaofeizhu Add aed03,aed04   #FUN-B80161
     LET l_sql = "SELECT aee01, aag02, aag13, aag06, aee03,aee04",   #FUN-6C0012  #09/10/20 xiaofeizhu Add aed03,aed04   #FUN-B80161
...
#                "   AND aee_file.aee00 = aag_file.aag00"            #NO.FUN-740020 #09/10/20 xiaofeizhu Mark
     ...
     PREPARE aglg809_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0
        ...
        EXIT PROGRAM 
        END IF
     DECLARE aglg809_curs1 CURSOR FOR aglg809_prepare1
 
#    CALL cl_outnam('aglg809') RETURNING l_name           #No.FUN-790008 
#    START REPORT aglg809_rep TO l_name                   #No.FUN-790008 
 
     LET g_pageno = 0
     FOREACH aglg809_curs1 INTO sr.*
...
#FUN-B80161---mod----end-----------
       ...
       #EXECUTE insert_prep USING sr.aed01,l_aed03,sr.aag02,sr.aag13,sr.aag06,sr.aed02,sr.aee04,    #MOD-A60139 #FUN-B80161 mark 
        EXECUTE insert_prep USING l_order[1],l_order[2],sr.aed01,l_aed03,sr.aag02,sr.aag13,sr.aag06,sr.aed02,sr.aee04,    #MOD-A60139 #FUN-B80161 add 
                            l_m_a,l_m_b,l_y_a,l_y_b,g_azi04,g_azi05                                 #MOD-A60139  
#       OUTPUT TO REPORT aglg809_rep(l_order[1],l_oname[1],
#                                    l_order[2],l_oname[2],
...
###GENGRE###     LET g_str = tm.wc,";",tm.afc01,";",tm.yyyy,";",tm.bmm,";",tm.emm,";",         #FUN-AB0020 add
###GENGRE###                 tm.st[1,1],";",tm.st[2,2],";",g_azi04,";",g_azi05                                                                 
###GENGRE###     LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED                                              
###GENGRE###     CALL cl_prt_cs3('aglg809','aglg809',l_sql,g_str)  
    CALL aglg809_grdata()    ###GENGRE###
#    FINISH REPORT aglg809_rep
 
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
END FUNCTION
{                               #No.FUN-790008
REPORT aglg809_rep(sr)
...
FUNCTION aglg809_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg809")
        IF handler IS NOT NULL THEN
            START REPORT aglg809_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg809_datacur1 CURSOR FROM l_sql
            FOREACH aglg809_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg809_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg809_rep
        END IF
...
REPORT aglg809_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B80161----add----str---------------
    DEFINE l_m_a      LIKE type_file.num20_6    
    DEFINE l_m_b      LIKE type_file.num20_6    
    DEFINE l_m_dif    LIKE type_file.num20_6    
    DEFINE l_y_a      LIKE type_file.num20_6    
    DEFINE l_y_b      LIKE type_file.num20_6    
    DEFINE l_y_dif    LIKE type_file.num20_6    
    DEFINE l_m_a_1    LIKE type_file.num20_6    
    DEFINE l_m_b_1    LIKE type_file.num20_6    
    DEFINE l_m_diff   LIKE type_file.num20_6    
    DEFINE l_y_a_1    LIKE type_file.num20_6    
    DEFINE l_y_b_1    LIKE type_file.num20_6    
    DEFINE l_y_diff   LIKE type_file.num20_6   
    ...
    DEFINE l_sum4     LIKE type_file.num20_6 
    DEFINE l_sum5     LIKE type_file.num20_6 
    DEFINE l_sum6     LIKE type_file.num20_6 
    DEFINE l_a        STRING
    DEFINE l_b        STRING
    DEFINE l_fmt      STRING
    #FUN-B80161----add----end---------------

    ORDER EXTERNAL BY sr1.order1,sr1.order2   #FUN-B80161 add 
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            LET l_qibie = cl_gr_getmsg("gre-113",g_lang,'2'),':',tm.bmm,'-',tm.emm
            PRINTX l_qibie

            CASE tm.st[1,1]
               WHEN '1' LET l_a = cl_gr_getmsg("gre-236",g_lang,1)
               WHEN '2' LET l_a = cl_gr_getmsg("gre-236",g_lang,2)
               WHEN '3' LET l_a = cl_gr_getmsg("gre-236",g_lang,3)
               OTHERWISE LET l_a = " "
           END CASE
           PRINTX l_a
      
            CASE tm.st[2,2]
               WHEN '1' LET l_b = cl_gr_getmsg("gre-236",g_lang,1)
               WHEN '2' LET l_b = cl_gr_getmsg("gre-236",g_lang,2)
               WHEN '3' LET l_b = cl_gr_getmsg("gre-236",g_lang,3)
               OTHERWISE LET l_b = " "
           END CASE
           PRINTX l_b
        ...
        AFTER GROUP OF sr1.order1
            #FUN-B80161---add---str------------  
            LET l_m_a_1 = GROUP SUM(sr1.m_a)
            LET l_m_b_1 = GROUP SUM(sr1.m_b)
            LET l_m_diff = l_m_a_1 - l_m_b_1
            PRINTX l_m_a_1
            PRINTX l_m_b_1
            PRINTX l_m_diff
 
            LET l_y_a_1 = GROUP SUM(sr1.y_a)
            LET l_y_b_1 = GROUP SUM(sr1.y_b)
            LET l_y_diff = l_y_a_1 - l_y_b_1
            PRINTX l_y_a_1
            PRINTX l_y_b_1
            PRINTX l_y_diff
            #FUN-B80161---add---end------------  
        ...
        AFTER GROUP OF sr1.order2
            #FUN-B80161---add---str------------  
            LET l_m_a = GROUP SUM(sr1.m_a)
            LET l_m_b = GROUP SUM(sr1.m_b)
            LET l_m_dif = l_m_a - l_m_b
            PRINTX l_m_a
            PRINTX l_m_b
            PRINTX l_m_dif

            LET l_y_a = GROUP SUM(sr1.y_a)
            LET l_y_b = GROUP SUM(sr1.y_b)
            LET l_y_dif = l_y_a = l_y_b
            PRINTX l_y_a
            PRINTX l_y_b
            PRINTX l_y_dif
        ...
        ON LAST ROW
            #FUN-B80161----add----str-----------    
            LET l_sum1 = SUM(sr1.m_a)      
            LET l_sum2 = SUM(sr1.m_b)    
            LET l_sum3 =  l_sum1 - l_sum2
            PRINTX l_sum1  
            PRINTX l_sum2  
            PRINTX l_sum3
 
            LET l_sum4 = SUM(sr1.y_a)
            LET l_sum5 = SUM(sr1.y_b)
