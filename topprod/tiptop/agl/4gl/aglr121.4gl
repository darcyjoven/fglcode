...
# Modify.........: No.MOD-CA0088 12/10/16 By Polly 增加預算變數調整
...
#No.FUN-780060--start--add  
   LET g_sql="maj20.maj_file.maj20,", 
             ...
             "line.type_file.num5"      
   LET l_table = cl_prt_temptable('aglr121',g_sql) CLIPPED 
...
#No.FUN-780060--end--add
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL r121_tm()                # Input print condition
      ELSE CALL r121()            # Read data and create out-file
   END IF
...
FUNCTION r121_tm()
   ...
   OPEN WINDOW r121_w AT p_row,p_col WITH FORM "agl/42f/aglr121" 
...
#FUN-AB0020 ---------------End---------------------         
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aglr121',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW r121_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
...
#No.FUN-780060--end--
   ...
   LET l_sql = "SELECT * FROM maj_file",
               ...
               "   AND ",g_msg CLIPPED,
               " ORDER BY maj02"
   PREPARE r121_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM 
   END IF
   DECLARE r121_c CURSOR FOR r121_p
   FOR g_i = 1 TO 100
       LET g_tot1[g_i] = 0 LET g_tot2[g_i] = 0
       LET g_tot1_1[g_i] = 0 LET g_tot2_1[g_i] = 0
   END FOR
#   CALL cl_outnam('aglr121') RETURNING l_name  #No.FUN-780060
#   START REPORT r121_rep TO l_name             #No.FUN-780060
#   LET g_pageno = 0                            #No.FUN-780060
 
        IF g_abe01=' ' THEN                     #--- 部門
           DECLARE r121_curs10 CURSOR FOR 
            SELECT abd02,gem05 FROM abd_file,gem_file WHERE abd01=tm.abe01
                                                        AND gem01=abd02
             ORDER BY 1
           FOREACH r121_curs10 INTO m_abd02,l_chr
               IF SQLCA.SQLCODE THEN EXIT FOREACH END IF
               CALL r121_bom(m_abd02,l_chr)
           END FOREACH
         # IF g_buf IS NULL THEN LET g_buf="'",tm.abe01 CLIPPED,"'," END IF
          ...
          #LET g_buf=g_buf[1,l_leng-1] CLIPPED             #MOD-B60232 mark
           ...
           CALL r121_process(tm.abe01)
        ELSE                                    #--- 族群
           DECLARE r121_bom CURSOR FOR
            SELECT abe03,gem05 FROM abe_file,gem_file WHERE abe01=tm.abe01
                                                        AND gem01=abe03
             ORDER BY 1
           FOREACH r121_bom INTO l_abe03,l_chr
            IF SQLCA.SQLCODE THEN EXIT FOREACH END IF
            CALL r121_bom(l_abe03,l_chr)
            IF g_buf IS NULL THEN LET g_buf="'",l_abe03 CLIPPED,"'," END IF
           #LET l_leng=LENGTH(g_buf)                        #MOD-B60232 mark
           #LET g_buf=g_buf[1,l_leng-1] CLIPPED             #MOD-B60232 mark
            LET l_leng= g_buf.getlength()                   #MOD-B60232
            LET g_buf = g_buf.substring(1,l_leng-1) CLIPPED #MOD-B60232
            CALL r121_process(l_abe03)
            LET g_buf=''
           END FOREACH
        END IF
...
#No.FUN-780060--start--add
   ...
   LET g_str=g_mai02,";",tm.a,";",tm.d,";",tm.p,";",tm.e,";",tm.title1,";",tm.title2,";", 
               l_tit1,";",l_tit2  #No.FUN-830139
   CALL cl_prt_cs3('aglr121','aglr121',g_sql,g_str)
#No.FUN-780060--end-- 
END FUNCTION
 
FUNCTION r121_process(l_dept)
...
#No.FUN-D70095--start
   ...
   DEFINE l_CE_sum3    LIKE abb_file.abb07
   DEFINE l_CE_sum4    LIKE abb_file.abb07
   DEFINE l_CE1        LIKE abb_file.abb07
   DEFINE l_CE2        LIKE abb_file.abb07
...
#    PREPARE r121_sum FROM l_sql
#    DECLARE r121_sumc CURSOR FOR r121_sum
# 
...
#    END IF
##FUN-AB0020 ---------------------End----------------------------- 
#    PREPARE r121_sumbug FROM l_sql
#    DECLARE r121_sumcbug CURSOR FOR r121_sumbug
##No.FUN-D70095--end--mark------------ 
#    #-----------------------------------------------------------------------
##FUN-D70095--------------------------begin
...
#          OPEN r121_sumc USING maj.maj21,maj.maj22,tm.yy1,tm.bm1,tm.em1
#          FETCH r121_sumc INTO amt1   
#          IF STATUS THEN CALL cl_err('sel aah:',STATUS,1) EXIT FOREACH END IF
#          IF amt1 IS NULL THEN LET amt1 = 0 END IF          
#          OPEN r121_sumcbug USING maj.maj21,maj.maj22,tm.yy1,tm.bm1,tm.em1
#          FETCH r121_sumcbug INTO amt1_1   
...
#          OPEN r121_sumc USING maj.maj21,maj.maj22,tm.yy2,tm.bm2,tm.em2
#          FETCH r121_sumc INTO amt2
#          IF STATUS THEN CALL cl_err('sel aah:',STATUS,1) EXIT FOREACH END IF
#          IF amt2 IS NULL THEN LET amt2 = 0 END IF
#          OPEN r121_sumcbug USING maj.maj21,maj.maj22,tm.yy2,tm.bm2,tm.em2
#          FETCH r121_sumcbug INTO amt2_1
#          IF STATUS THEN CALL cl_err('sel afc:',STATUS,1) EXIT FOREACH END IF
#          IF cl_null(amt2_1) THEN LET amt2_1 = 0 END IF
#       END IF
#No.FUN-D70095--mark--end------------ 
...
#No.FUN-D70095--start--mark------------       
#     IF NOT cl_null(maj.maj21) THEN
#          OPEN r121_cesumc USING maj.maj21,maj.maj22,'1',tm.yy1,tm.bm1,tm.em1
...
#          
#          OPEN r121_cesumc USING maj.maj21,maj.maj22,'2',tm.yy1,tm.bm1,tm.em1
#          FETCH r121_cesumc INTO l_CE_sum2   
#          IF STATUS THEN CALL cl_err('sel abb:',STATUS,1) EXIT FOREACH END IF
#          IF l_CE_sum2 IS NULL THEN LET l_CE_sum2 = 0 END IF
#          
#          OPEN r121_cesumc USING maj.maj21,maj.maj22,'1',tm.yy2,tm.bm2,tm.em2
#          FETCH r121_cesumc INTO l_CE_sum3   
#          IF STATUS THEN CALL cl_err('sel abb:',STATUS,1) EXIT FOREACH END IF
#          IF l_CE_sum3 IS NULL THEN LET l_CE_sum3 = 0 END IF
#          
#          OPEN r121_cesumc USING maj.maj21,maj.maj22,'2',tm.yy2,tm.bm2,tm.em2
#          FETCH r121_cesumc INTO l_CE_sum4  
#          IF STATUS THEN CALL cl_err('sel abb:',STATUS,1) EXIT FOREACH END IF
#          IF l_CE_sum4 IS NULL THEN LET l_CE_sum4 = 0 END IF 
#          LET amt1 = amt1 - l_CE_sum1 + l_CE_sum2
#          LET amt2 = amt2 - l_CE_sum3 + l_CE_sum4            
#       END IF 
#No.FUN-D70095--end--mark------------ 
      #---------------FUN-D70095----------(S)
      #期別 yy1/bm1 - yy1/em1
      IF NOT cl_null(maj.maj21) THEN
         CALL s_aao_amt(tm.b,maj.maj21,maj.maj22,g_buf,tm.yy1,tm.bm1,tm.em1,maj.maj06,g_plant)
              RETURNING amt1
         CALL s_ce_abb07(tm.b,maj.maj21,maj.maj22,g_buf,tm.yy1,tm.bm1,tm.em1,maj.maj06,g_plant)
                RETURNING l_CE1
         LET amt1 = amt1 - l_CE1
         CALL s_afc_amt(tm.b,maj.maj21,maj.maj22,g_buf,tm.yy1,tm.bm1,tm.em1,tm.afc01,maj.maj06,g_plant)
              RETURNING amt1_1
      END IF 
      #期別 yy2/bm2 - yy2/em2
      IF NOT cl_null(maj.maj21) AND tm.yy2 IS NOT NULL THEN
         CALL s_aao_amt(tm.b,maj.maj21,maj.maj22,g_buf,tm.yy2,tm.bm2,tm.em2,maj.maj06,g_plant)
              RETURNING amt2
         CALL s_ce_abb07(tm.b,maj.maj21,maj.maj22,g_buf,tm.yy2,tm.bm2,tm.em2,maj.maj06,g_plant)
              RETURNING l_CE2          
         LET amt2 = amt2 - l_CE2 
         CALL s_aao_amt(tm.b,maj.maj21,maj.maj22,g_buf,tm.yy2,tm.bm2,tm.em2,tm.afc01,maj.maj06,g_plant)
              RETURNING amt2_1 
      END IF
      #---------------FUN-D70095----------(E)      
       IF tm.o = 'Y' THEN
       ...
       IF NOT cl_null(maj.maj21) THEN
          IF maj.maj06 MATCHES '[123468]' AND maj.maj07 = '2' THEN
             ...
             LET amt2_1 = amt2_1 * -1
          END IF          
          IF maj.maj06 = '[579]' AND maj.maj07 = '1' THEN
             LET amt1 = amt1 * -1
             LET amt2 = amt2 * -1
             LET amt1_1 = amt1_1 * -1
             LET amt2_1 = amt2_1 * -1
       ...
       IF maj.maj03 MATCHES "[012]" AND maj.maj08 > 0  THEN      #合計階數處理
          #---------------FUN-D70095----------(S)
               IF maj.maj08 = '1' THEN
                  ...
                  LET g_bal_a[g_cnt].lbal1  = amt1_1
                  LET g_bal_a[g_cnt].lbal2  = amt2_1
       ...
       LET sr.bal1_1 = g_bal_a[g_cnt].lbal1
       LET sr.bal2_1 = g_bal_a[g_cnt].lbal2
       LET g_cnt = g_cnt + 1
       #---------------FUN-D70095----------(E)
       #---------------FUN-D70095---mark-------(S)
#          FOR i = 1 TO 100
...
#         END IF
        ...
        IF sr.bal2_1<0 THEN LET sr.bal2_1=sr.bal2_1*-1 END IF #MOD-CA0088 add
        ...
        END IF 
#No.FUN-780060--end--
#       OUTPUT TO REPORT r121_rep(maj.*, sr.*)              #No.FUN-780060  
    END FOREACH
END FUNCTION
#No.FUN-780060--start--add
{REPORT r121_rep(maj, sr)
