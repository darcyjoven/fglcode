...
# Modify.........: No.TQC-760105 07/06/12 By dxfwo aglr107(科目余額表) 無法印出
...
#No.FUN-760083  --BEGIN--
   ...
   LET l_table=cl_prt_temptable("aglr107",g_sql) CLIPPED
   IF l_table=-1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?)"   #MOD-930187 add 3?
...
DEFINE li_chk_bookno  LIKE type_file.num5         #FUN-B20054
   ...
   OPEN WINDOW aglr107_w AT p_row,p_col
     WITH FORM "agl/42f/aglr107"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
 ...
 #FUN-B20054--add--end
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglr107',g_time,l_cmd)      # Execute cmd at later time
         END IF
...
#         l_time    LIKE type_file.chr8       #No.FUN-6A0073
   ...
   LET l_sql = "SELECT '',aag01,aag02,aag13,aag06,aag07,aag08,aag24,'',",   #MOD-930187 add aag08
               "       SUM(aah04-aah05),0,0", #FUN-560211  #FUN-6C0012   #MOD-930187 add 0,0
 ...
 #             "   AND aag00 = b", #No.FUN-740020   #No.TQC-760105 
   ...
   PREPARE aglr107_prepare1 FROM l_sql
   IF STATUS THEN
      CALL cl_err('prepare:',STATUS,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE aglr107_curs1 CURSOR FOR aglr107_prepare1
#No.FUN-760083  --begin--
{
   CALL cl_outnam('aglr107') RETURNING l_name     
   ...
   START REPORT aglr107_rep TO l_name
}
   LET g_str=''
...
#No.FUN-760083  -END--
   ...
   FOREACH aglr107_curs1 INTO sr.*
     ...
     ##-----END MOD-770122-----
      ...
      #OUTPUT TO REPORT aglr107_rep(sr.*)    #No.FUN-760083
  ...
  #str MOD-930187 add
   ...
   PREPARE aglr107_pre_0 FROM g_sql
   IF STATUS THEN CALL cl_err('aglr107_pre_0',STATUS,1) END IF
   DECLARE aglr107_cs_0 CURSOR FOR aglr107_pre_0
   LET g_sql="SELECT DISTINCT aag01 FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
             " WHERE aag07='1'",
             " ORDER BY aag01"
   PREPARE aglr107_pre_1 FROM g_sql
   DECLARE aglr107_cs_1 CURSOR FOR aglr107_pre_1
   FOREACH aglr107_cs_1 INTO l_aag01
      IF STATUS THEN
         CALL cl_err('foreach:',STATUS,1)
         EXIT FOREACH
      END IF
      ...
      LET l_cnt = 0
      OPEN  aglr107_cs_0 USING l_aag01
      FETCH aglr107_cs_0 INTO l_cnt
      IF l_cnt = 0 THEN
   ...
   ELSE
      LET g_sql="SELECT aag01,SUM(aah04),SUM(aah04_1)",
                ...
                " GROUP BY aag01"
   END IF
   PREPARE aglr107_pre_2 FROM g_sql
   DECLARE aglr107_cs_2 CURSOR FOR aglr107_pre_2
   FOREACH aglr107_cs_2 INTO l_order1,s_aah04,s_aah04_1
      IF STATUS THEN
         CALL cl_err('foreach:',STATUS,1)
         EXIT FOREACH
      END IF
   ...
   LET g_sql="SELECT SUM(aah04_1) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
             " WHERE dc=?"
   PREPARE aglr107_pre_sum FROM g_sql
   IF STATUS THEN CALL cl_err('aglr107_pre_sum',STATUS,1) END IF
   DECLARE aglr107_cs_sum CURSOR FOR aglr107_pre_sum
   LET l_dc ='D'
   OPEN  aglr107_cs_sum USING l_dc
   FETCH aglr107_cs_sum INTO g_tot
   IF cl_null(g_tot) THEN LET g_tot = 0 END IF   #TQC-930135 add
   LET l_dc ='C'
   OPEN  aglr107_cs_sum USING l_dc
   FETCH aglr107_cs_sum INTO g_tot1
...
#No.FUN-760083  --begin--
   ...
   LET g_str=tm.wc,';',t_azi05,';',g_tot,';',t_azi04,';',tm.yy,';',tm.m1,';',tm.m2,';',tm.f,';',tm.e,';',g_tot1   #MOD-770122
   LET g_sql="SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
   CALL cl_prt_cs3("aglr107","aglr107",g_sql,g_str)
#No.FUN-760083  --end--
END FUNCTION
 
#No.FUN-760083  --begin--
{
REPORT aglr107_rep(sr)
