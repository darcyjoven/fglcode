...
MAIN
 ...
 #--str FUN-780059 add--#                                                                                                           
   ...
   LET g_sql = "srorder1.aag_file.aag223,",    #FUN-B80158 add
   ...
   LET l_table = cl_prt_temptable('aglg804',g_sql) CLIPPED   # 產生Temp Table
   IF l_table = -1 THEN 
...
FUNCTION aglg804_tm(p_row,p_col)
...
#  CALL s_dsmark(g_bookno)       #No.FUN-740020
   ...
   OPEN WINDOW aglg804_w AT p_row,p_col
     WITH FORM "agl/42f/aglg804"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
#No.FUN-B20054--add--end--
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglg804',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0073
          ...
          sr        RECORD srorder1 LIKE aag_file.aag223,    #FUN-B80158 add
...
#  LET l_sql = "SELECT '',aed01, aed011, aed02, SUM(aed05-aed06)",               #NO.FUN-780059
   LET l_sql = "SELECT '','','',aed01, aed011, aed02, SUM(aed05-aed06)",            #NO.FUN-780059   #FUN-B80158 add ''
   ...
   PREPARE aglg804_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE aglg804_curs1 CURSOR FOR aglg804_prepare1
 
#  CALL cl_outnam('aglg804') RETURNING l_name               #FUN-780059--mark
#  START REPORT aglg804_rep TO l_name                       #FUN-780059--mark
 
   LET g_pageno = 0
   INITIALIZE sr.* TO NULL
 
   FOREACH aglg804_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
  ...
  #           AND aed00 = g_bookno    #No.FUN-740020                                                                                
      ...
      #FUN-B80158--------add-----str----
      CASE WHEN tm.w = '1' LET sr.srorder1 = sr.aag223
           WHEN tm.w = '2' LET sr.srorder1 = sr.aag224
           WHEN tm.w = '3' LET sr.srorder1 = sr.aag225
           WHEN tm.w = '4' LET sr.srorder1 = sr.aag226
      END CASE      
      #FUN-B80158--------add-----end----
...
#--str-FUN-780059--add--#
         ...
         END IF
#--end-FUN-780059--add--#
#     OUTPUT TO REPORT aglg804_rep(sr.*)                 #FUN-780059--mark
        ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-770052 *** ##                                                   
           EXECUTE insert_prep USING              
                      sr.srorder1,                   #FUN-B80158 add
...
#  FINISH REPORT aglg804_rep                             #FUN-780059--mark
...
###GENGRE###    LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                                
    LET g_str = ''                                                                                                                  
###GENGRE###    LET g_str = tm.wc,";",tm.w,";",tm.aed03,";",tm.aed04,";",tm.e,";",                                                          
###GENGRE###                tm.x,";",tm.t,";",t_azi04,";",t_azi05                                                                                        
###GENGRE###    CALL cl_prt_cs3('aglg804','aglg804',l_sql,g_str)                                                                                  
    CALL aglg804_grdata()    ###GENGRE###
    #------------------------------ CR (4) ------------------------------#
 
END FUNCTION
 
#--str-FUN-780059--mark--#
#REPORT aglg804_rep(sr)
...
FUNCTION aglg804_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg804")
        IF handler IS NOT NULL THEN
            START REPORT aglg804_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY srorder1 asc nulls first,aed01,aed02"     #FUN-B80158 add
          
            DECLARE aglg804_datacur1 CURSOR FROM l_sql
            FOREACH aglg804_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg804_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg804_rep
        END IF
...
REPORT aglg804_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B80158-------add-------str---
    DEFINE l_RTotal0     LIKE aed_file.aed05
    DEFINE l_RTotal1     LIKE aed_file.aed05
    DEFINE l_RTotal2     LIKE aed_file.aed05
    DEFINE l_sum_c       LIKE aed_file.aed06
    DEFINE l_sum_d       LIKE aed_file.aed05
    DEFINE l_sum_d_c2    LIKE aed_file.aed05
    DEFINE g_c38         STRING
    DEFINE g_c41         LIKE aag_file.aag02
    DEFINE g_c41_1       LIKE aed_file.aed05
    DEFINE g_c41_2       LIKE aed_file.aed05
    DEFINE g_c41_3       LIKE aed_file.aed05
    DEFINE g_c42_1       LIKE type_file.chr1
    DEFINE g_c42_2       LIKE type_file.chr1
    DEFINE g_c42_3       LIKE type_file.chr1
    DEFINE l_c_fmt       STRING
    DEFINE l_d_fmt       STRING
    DEFINE l_c41_2_fmt   STRING
    DEFINE l_c41_3_fmt   STRING
    DEFINE l_aed01_cnt   LIKE type_file.num10
    ...
    ORDER EXTERNAL BY sr1.srorder1,sr1.aed01,sr1.aed02      #FUN-B80158  add
    
    FORMAT
        FIRST PAGE HEADER
            PRINTX g_grPageHeader.*    
            PRINTX g_user,g_pdate,g_prog,g_company,g_ptime,g_user_name   #FUN-B80158 addg_ptime,g_user_name
            PRINTX tm.*
            LET l_aed01_cnt = 0                               #FUN-B80158 add
            LET l_order1_cnt = 0                              #FUN-B80158 add
              
        BEFORE GROUP OF sr1.srorder1      #FUN-B80158  add
           #FUN-B80158-------add----str----
           LET g_sql2 = "SELECT COUNT(*) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                       " WHERE srorder1 IS NULL OR srorder1 = '",sr1.srorder1,"'"
           LET l_cnt2 = 0

           DECLARE aglg804_repcur2 CURSOR FROM g_sql2
           FOREACH aglg804_repcur2  INTO l_cnt2 END FOREACH
           #FUN-B80158-------add-------end---
        BEFORE GROUP OF sr1.aed01
            #FUN-B80158-------add----str----
            LET g_c38 = sr1.srorder1,sr1.aae02
            PRINTX g_c38
            IF tm.e = 'N' THEN
               LET g_c41 = sr1.aag02
            ELSE
        ...
        ON EVERY ROW
            ...
            LET g_sql1 = "SELECT COUNT(DISTINCT aed01) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
            DECLARE aglg804_repcur1 CURSOR FROM g_sql1
            FOREACH aglg804_repcur1  INTO l_cnt1 END FOREACH   #FUN-B80158 add
            #FUN-B80158-------add----end----

            PRINTX sr1.*

        AFTER GROUP OF sr1.srorder1       #FUN-B80158  add
            #FUN-B80158-------add----str----
            LET l_c41_3_fmt = cl_gr_numfmt("aed_file","aed05",t_azi05)
            PRINTX l_c41_3_fmt

            LET l_RTotal2 = GROUP SUM(sr1.d_c)
            PRINTX l_RTotal2

            IF l_RTotal2 < 0 THEN
               LET g_c41_3 = l_RTotal2 * (-1)
            ELSE
               LET g_c41_3 = l_RTotal2 
            END IF
            PRINTX g_c41_3 
        ...
        AFTER GROUP OF sr1.aed01
            #FUN-B80158-------add----str----
            LET l_c41_2_fmt = cl_gr_numfmt("aed_file","aed05",t_azi05)
            PRINTX l_c41_2_fmt
            LET l_RTotal1 = GROUP SUM(sr1.d_c)
            PRINTX l_RTotal1

            IF l_RTotal1 < 0 THEN
               LET g_c41_2 = l_RTotal1 * (-1)
            ELSE
               LET g_c41_2 = l_RTotal1 
            END IF
            PRINTX g_c41_2 
           ...
           LET l_aed01_cnt = l_aed01_cnt + 1
           IF l_aed01_cnt = l_cnt1 THEN
              LET l_skip = 'N'
           ELSE
              LET l_skip = 'Y'
           END IF
           ...
           PRINTX l_skip1
            #FUN-B80158-------add----end----
        AFTER GROUP OF sr1.aed02
            #FUN-B80158-------add----str----
            LET l_RTotal0 = GROUP SUM(sr1.d_c)
            PRINTX l_RTotal0
            LET l_sum_c = GROUP SUM(sr1.c)
            PRINTX l_sum_c 
            LET l_sum_d = GROUP SUM(sr1.d)
            PRINTX l_sum_d 
            LET l_sum_d_c2 = GROUP SUM(sr1.d_c2)
            PRINTX l_sum_d_c2 
            
            IF l_RTotal0 < 0 THEN
               LET g_c41_1 = l_RTotal0 * (-1)
            ELSE
               LET g_c41_1 = l_RTotal0 
            END IF
            PRINTX g_c41_1 
            ...
            LET l_c_fmt = cl_gr_numfmt("aed_file","aed06",t_azi04)
            PRINTX l_c_fmt
            LET l_d_fmt = cl_gr_numfmt("aed_file","aed05",t_azi04)
            PRINTX l_d_fmt
            #FUN-B80158-------add----end----
