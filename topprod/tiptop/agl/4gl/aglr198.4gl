...
# Modify.........: No.MOD-850162 08/05/19 By Sarah r198_file裡的maj02改成LIKE maj_file.maj02
...
# Modify.........: No.MOD-930020 09/03/03 By Sarah 在r198_sum與r198_sum1前,先判斷若maj.maj06為Null則清空l_sql,l_sql1,CONTINUE FOREACH
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aglr198',g_sql) CLIPPED   # 產生Temp Table
   ...
   LET l_table1 = cl_prt_temptable('aglr1981',g_sql) CLIPPED   # 產生Temp Table   
   ...
   DROP TABLE r198_file
   CREATE TEMP TABLE r198_file(
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL r198_tm()       # Input print condition
   ELSE
      CALL r198()         
   END IF
...
FUNCTION r198_tm()
   ...
   END IF
   OPEN WINDOW r198_w AT p_row,p_col WITH FORM "agl/42f/aglr198" 
...
#FUN-AB0020 ---------------End---------------------        
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW r198_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglr198',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW r198_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
         EXIT PROGRAM
...
FUNCTION r198()
  ...
  DEFINE sr4 RECORD
   ...
   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aglr198'
   ...
   LET l_sql = "SELECT * FROM maj_file",
               ...
               "   AND ",g_msg CLIPPED,
               " ORDER BY maj02"
   PREPARE r198_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM 
   END IF
   DECLARE r198_c CURSOR FOR r198_p
...
#---------98-06-18 Modify控制一次印十個部門
   ...
   FOR l_i = 10 TO l_cnt STEP 10
       ...
       DECLARE tmp_curs CURSOR FOR
          SELECT * FROM r198_file ORDER BY maj02,no
...
#---------------FUN-D70095---mark----(E) 
  ...
  LET g_sql = "SELECT A.*,B.dept_01,B.dept_02,B.dept_03,B.dept_04,B.dept_05,",
  ...
  #p1~p7
  LET g_str = tm.a,";",tm.yy,";",tm.bm,";",tm.em,";",tm.d,";",
              tm.p,";",g_azi05
  CALL cl_prt_cs3('aglr198','aglr198',g_sql,g_str)
END FUNCTION
 
FUNCTION r198_process(l_cn)
   ...
   DEFINE l_CE           LIKE abb_file.abb07         #FUN-D70095 add
   ...
   FOREACH r198_c INTO maj.*
      ...
      IF cl_null(maj.maj06) THEN
         ...
         CONTINUE FOREACH
      END IF
      PREPARE r198_sum FROM l_sql
      DECLARE r198_sumc CURSOR FOR r198_sum
...
#FUN-AB0020 ---------------------End-----------------------------       
      ...
      DECLARE r198_sumc1 CURSOR FOR r198_sum1
 
      LET amt1 = 0 LET amt2 = 0 LET amt  = 0
      IF NOT cl_null(maj.maj21) THEN
         OPEN r198_sumc USING maj.maj21,maj.maj22
         FETCH r198_sumc INTO amt1   
         IF cl_null(amt1) THEN LET amt1 = 0 END IF
         #---------------FUN-D70095------------(S)
         IF NOT cl_null(maj.maj06) THEN 
            IF maj.maj06 < '4' THEN
            ...
            ELSE
         	     OPEN r121_cesumc USING maj.maj21,maj.maj22
         	     FETCH r121_cesumc INTO l_CE
        ...
        #END IF                     #MOD-B70110 mark
         #CHI-A50013 add --end--
         OPEN r198_sumc1 USING maj.maj21,maj.maj22
         FETCH r198_sumc1 INTO amt2
...
#      END IF
      ...
      END IF
      INSERT INTO r198_file VALUES(l_cn,maj.maj02,maj.maj03,maj.maj04,
                                   maj.maj05,maj.maj07,maj.maj20,maj.maj20e,
                                   m_bal1,0,m_bal2,0)
      IF STATUS OR SQLCA.SQLERRD[3] = 0 THEN
         CALL cl_err3("ins","r198_file",l_cn,maj.maj02,STATUS,"","ins r198_file",1)   #No.FUN-660123
         EXIT FOREACH 
      END IF
   END FOREACH
END FUNCTION
 
REPORT r198_rep(sr,sr2,sr3,sr4)  #FUN-560242
