...
# Modify.........: No.FUN-B50001 11/05/10 By lutingting 參數檔agls101改為aaw_file
# Modify.........: No.MOD-B90005 11/09/05 By Polly 調整第二期範圍的判斷
# Modify.........: No.FUN-B80158 11/09/08 By yangtt 明細類CR轉換成GRW
# Modify.........: No.FUN-B80158 12/01/05 By yangtt FUN-B90140追單
# Modify.........: No.FUN-B80158 12/01/12 By xuxz FUN-BA0012追單,FUN-B50001 rollback
# Modify.........: No.FUN-B80158 12/01/17 By xuxz g_sql="insert into ds_report: “問題修改
# Modify.........: No.FUN-C50004 12/05/08 By nanbing GR 優化
...
GLOBALS "../../config/top.global" 
   ...
   DEFINE tm  RECORD    #FUN-AB0091
                 ...
                 aaz641 LIKE aaz_file.aaz641,            #合并帳別 #FUN-B50001  #FUN-B80158 not mark
                 #aaw01 LIKE aaw_file.aaw01,             #合并帳別    #FUN-B50001#FUN-B80158 mark
   ...
   DEFINE g_aaz641   LIKE aaz_file.aaz641   #FUN-B50001 #FUN-B80158 not mark
   #DEFINE g_aaw01   LIKE aaw_file.aaw01 #FUN-B80158 mark
...
MAIN
   ...
   IF (NOT cl_setup("AGL")) THEN
      EXIT PROGRAM
   END IF
   #CALL cl_used(g_prog,g_time,1) RETURNING g_time  #FUN-B80158 mark
   ...
   LET l_table = cl_prt_temptable('aglg019',g_sql) CLIPPED  # 產生Temp Table
   IF l_table = -1 THEN EXIT PROGRAM END IF                 # Temp Table產生有錯
   #LET g_sql = "INSERT INTO ds_report:",l_table CLIPPED,#FUN-B80158(2012.1.17) mark
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,#FUN-B80158(2012.1.17) add
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?) " #CHI-A70061 add ?   #NO.FUN-B80158 add ?
   ...
   LET tm.a  = ARG_VAL(9)
   LET tm.aaz641  = ARG_VAL(10)  #FUN-B50001  #FUN-B80158not mark
   #LET tm.aaw01  = ARG_VAL(10)  #FUN-B80158 mark
...
#FUN-B50001--mod--end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL g019_tm()                # Input print condition
      ELSE CALL g019()            # Read data and create out-file
   END IF
...
FUNCTION g019_tm()
   ...
   END IF
   OPEN WINDOW g019_w AT p_row,p_col
        WITH FORM "agl/42f/aglg019" 
################################################################################
...
WHILE TRUE
    LET l_sw = 1
    INPUT BY NAME tm.rtype,tm.axa01,tm.axa02,tm.axa03,tm.aaz641,tm.a,  #FUN-B50001 #FUN-B80158 not mark
    #INPUT BY NAME tm.rtype,tm.axa01,tm.axa02,tm.axa03,tm.aaw01,tm.a,   #FUN-B80158 mark
                  ...
                  tm.e,tm.f,tm.d,tm.acc_code,tm.c,tm.h,tm.o,tm.r,    #No.FUN-B80158 Add tm.acc_code
                  tm.p,tm.q,tm.more WITHOUT DEFAULTS  
       BEFORE INPUT
          CALL cl_qbe_init()
          CALL g019_set_entry()      
          CALL g019_set_no_entry()  
...
#FUN-B50001--mod--end
       ...
       AFTER FIELD a
          ...
          SELECT mai02,mai03 INTO g_mai02,g_mai03 FROM mai_file
           WHERE mai01 = tm.a 
             AND mai00 = tm.aaz641   #FUN-B50001  #FUN-B80158 not mark
             #AND mai00 = tm.aaw01#FUN-B80158 mark
             AND maiacti IN ('Y','y')
          IF STATUS THEN 
          ...
          CALL g019_set_entry()        #FUN-AC0021
          CALL g019_set_no_entry()     #FUN-AC0021      

      AFTER FIELD aaz641    #FUN-B50001#FUN-B80158 not mark
     # AFTER FIELD aaw01#FUN-B80158 mark
...
#         LET g_sql ="SELECT COUNT(*) FROM ",cl_get_target_table(g_dbs_axz03,'aaw_file'), 
...
#FUN-B50001--mod--end
         ...
         PREPARE g019_cnt_p FROM g_sql
         DECLARE g019_cnt_c CURSOR FOR g019_cnt_p
         OPEN g019_cnt_c
         FETCH g019_cnt_c INTO l_cnt
         IF cl_null(l_cnt) OR l_cnt=0 THEN
            CALL cl_err('','agl-965',1)
            NEXT FIELD aaz641   #FUN-B50001 #FUN-B80158 not mark
            #NEXT FIELD aaw01 #FUN-B80158 mark
         END IF         
...
################################################################################
      ...
      ON ACTION CONTROLP
         ...
         IF INFIELD(a) THEN
           ...
           #LET g_qryparam.where = " mai00 = '",tm.aaz641,"'"  #FUN-B80158 aaw01-->aaz641   #No.TQC-C50042   Mark
...
#FUN-B50001--mod--end
   ...
   END INPUT
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW g019_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)               #FUN-80158 add
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",tm.rtype CLIPPED,"'",
                         " '",tm.a CLIPPED,"'",
                         " '",tm.aaz641 CLIPPED,"'", #FUN-B50001  #FUN-B80158 not mark
                         #" '",tm.aaw01 CLIPPED,"'",  #FUN-B80158 mark
                         " '",tm.title1 CLIPPED,"'",   
                         " '",tm.yy1 CLIPPED,"'",
                         " '",tm.bm1 CLIPPED,"'",
                         " '",tm.em1 CLIPPED,"'",
                         ...
                         " '",g_template CLIPPED,"'",         
                         " '",g_rpt_name CLIPPED,"'"         
         CALL cl_cmdat('aglg019',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW g019_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)               #FUN-80158 add
...
FUNCTION g019()
   ...
   SELECT aaf03 INTO g_company FROM aaf_file
    WHERE aaf01 = tm.aaz641   #FUN-B50001 #FUN-B80158 not mark
    #WHERE aaf01 = tm.aaw01 #FUN-B80158 mark
      AND aaf02 = g_rlang
   ...
   LET l_sql = "SELECT * FROM maj_file",
               ...
               "   AND ",g_msg CLIPPED,
               " ORDER BY maj02"
   PREPARE g019_p FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)               #FUN-80158 add
      EXIT PROGRAM 
   END IF
   DECLARE g019_c CURSOR FOR g019_p
   ...
   FOREACH g019_c INTO maj.*
     ...
     IF NOT cl_null(maj.maj21) THEN
        ...
        #LET g_sql="SELECT SUM(axh08-axh09)",
        #          "  FROM axh_file",
        #          " WHERE axh00 = '",tm.aaz641,"'",   #FUN-B50001#FUN-B80158 not mark
        #          #" WHERE axh00 = '",tm.aaw01,"'",#FUN-B80158 mark
        #          "   AND axh01 = '",tm.axa01,"'",
        #          "   AND axh02 = '",tm.axa02,"'",
        #          "   AND axh03 = '",tm.axa03,"'",     
        ...
        IF tm.rtype = '2' THEN
             #FUN-AC0021 start--
             #取出起始期別的前一期
             #IF tm.bm1 > 1 THEN  #FUN-B80158 from FUN-BA0012(MOD-B90005) mark
             IF tm.bm2 > 1 THEN  #FUN-B80158 from FUN-BA0012(MOD-B90005) add
     ...
     EXECUTE insert_prep USING
        maj.maj31,maj.maj01,maj.maj02,maj.maj03,maj.maj04,maj.maj05,    #NO.FUN-B80158 add
...
FUNCTION aglg019_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg019")
        IF handler IS NOT NULL THEN
            START REPORT aglg019_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg019_datacur1 CURSOR FROM l_sql
            FOREACH aglg019_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg019_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg019_rep
        END IF
...
REPORT aglg019_rep(sr1)
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B80158------add-----str-----
            LET l_bal_fmt = cl_gr_numfmt("axh_file","axh08",tm.e)
            PRINTX l_bal_fmt

            CASE tm.d 
               WHEN '1'  LET l_bal1 = sr1.bal1 / 1
               WHEN '2'  LET l_bal1 = sr1.bal1 / 1000
               WHEN '3'  LET l_bal1 = sr1.bal1 / 1000000
            END CASE
            PRINTX l_bal1

            CASE tm.d 
               WHEN '1'  LET l_bal2 = sr1.bal2 / 1
               WHEN '2'  LET l_bal2 = sr1.bal2 / 1000
               WHEN '3'  LET l_bal2 = sr1.bal2 / 1000000
            END CASE
            PRINTX l_bal2 #FUN-B80158 

            CASE tm.d 
               WHEN '1'  LET l_bal3 = sr1.bal3 / 1
               WHEN '2'  LET l_bal3 = sr1.bal3 / 1000
               WHEN '3'  LET l_bal3 = sr1.bal3 / 1000000
            END CASE
            PRINTX l_bal3
            ...
            IF sr1.bal1 != 0 AND g_basetot1 != 0 AND NOT cl_null(sr1.bal1) AND NOT cl_null(g_basetot1) THEN
               LET l_per1 = (sr1.bal1 / g_basetot1 ) * 100
            ELSE
               LET l_per1 = 0
            END IF
            PRINTX l_per1

            IF sr1.bal2 != 0 AND g_basetot2 != 0 AND NOT cl_null(sr1.bal2) AND NOT cl_null(g_basetot2) THEN
               LET l_per2 = (sr1.bal2 / g_basetot2 ) * 100
            ELSE
               LET l_per2 = 0
            END IF
            PRINTX l_per2
