...
# Modify.........: No:TQC-D40025 13/04/22 by lixiang 修正FUN-D30032 BUG
...
FUNCTION t004_a()
   ...
   SELECT aaw01 INTO g_aen11 FROM aaw_file WHERE aaw00 = '0'  
   DISPLAY g_aen11 TO aen11
   CALL cl_opmsg('a')
...
FUNCTION t004_r()
    ...
    IF NOT cl_null(g_aen10) THEN CALL cl_err('','agl-353',0) RETURN END IF 
...
DEFINE l_axz01         LIKE axz_file.axz01   
    ...
    DIALOG ATTRIBUTES(UNBUFFERED)  
     ...
     #FUN-D30032--add--begin--
      BEFORE DIALOG
         CASE g_b_flag
              WHEN '1' NEXT FIELD aen06
              WHEN '2' NEXT FIELD aeo06
         END CASE
     #FUN-D30032--add--end---
      ...
      ON ACTION cancel
         ...
         IF p_cmd = 'a' THEN
           ...
           #TQC-D40025--add---begin--- 
            IF g_b_flag ='1' AND g_rec_b1 != 0 THEN 
...
#FUN-B90057--add-end
   ...
   FOREACH sel_aeo_curs INTO l_amt,l_aeo06,l_aeo15 
       ...
       SELECT aen08 INTO l_aen08 FROM aen_file WHERE aen01 = g_aen01
          ...
          AND aen05 = g_aen05 AND aen06 = l_aeo15 AND aen07 = l_aeo06
       ...
       IF cl_null(l_axj.axj02) THEN LET l_axj.axj02 = 1 END IF
       LET l_axj.axj06 = '2'
       LET l_axj.axj03 = g_aaw.aaw14    ###投資收益
       INSERT INTO axj_file VALUES(l_axj.*)    ##借方
       IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
          CALL s_errmsg('axj_file','insert',l_axi.axi01,SQLCA.sqlcode,1)
          LET g_success = 'N'
...
DEFINE l_axa07 LIKE axa_file.axa07      # No.FUN-B90018 
 ...
 #  FUN-B60134--add--end  
        ...
        FOREACH sel_aag01_pre1 INTO g_aag01_1[l_cnt2].aag01,g_aag01_1[l_cnt2].aag19  
           ...
           LET g_aeo[l_cnt4].aeo17 = g_aaw.aaw06
           ...
           CALL t004_get_aeo16(g_aeo[l_cnt4].aeo13,g_aeo[l_cnt4].aeo20,g_aeo[l_cnt4].aeo14,g_aeo[l_cnt4].aeo14_1)
                RETURNING g_aeo[l_cnt4].aeo16,g_aeo[l_cnt4].aeo21  
           LET g_aeo[l_cnt4].aeo15 = g_aaw.aaw13
          #FUN-B90009--mod--str--
