...
# Modify.......... NO.TQC-AA0098 11/10/14 BY yiting CREATE TEMP TABLE r008_file(版本欄位不可移除 
...
DEFINE g_aaz641       LIKE aaz_file.aaz641        #MOD-950214 add
...
MAIN
 ...
 CREATE TEMP TABLE r008_file(
       ...
       axk15  LIKE axk_file.axk15)       #NO.FUN-750076   #FUN-950048   #TQC-AA0098
 CREATE TEMP TABLE r008_axa_file(
...
#  寫死后導致程序無資料，無結果顯示
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL r008_tm()                           # Input print condition
   ELSE
      CALL r008()         
   END IF
   DROP TABLE r008_file
   DROP TABLE r008_axa_file     #FUN-930138
...
FUNCTION r008_tm()
   ...
   OPEN WINDOW r008_w AT p_row,p_col WITH FORM "agl/42f/aglr008" 
   ...
   WHILE TRUE
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglr008',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW r008_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
         EXIT PROGRAM
...
FUNCTION r008()
   ...
   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aglr008'
   ...
   WHILE g_flag = 'N'
     IF p_i = 1 THEN  #找第一階
         ...
         IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
            ...
            EXIT PROGRAM 
         END IF
         DECLARE r008_c1 CURSOR FOR r008_p1
         LET l_axb04 = ''
         FOREACH r008_c1 INTO l_axb04  #抓出第一層子公司
            LET l_axb02 = l_axb04
            LET l_cnt = 0
                INSERT INTO r008_axa_file VALUES(tm.axa02,l_axb04,g_j)
     ...
     ELSE
         LET l_axb04 = ''
         LET l_j = g_j - 1
         LET l_sql = "SELECT axb04 FROM r008_axa_file ",
                     " WHERE count = '",l_j,"'"
         ...
         IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
            ...
            EXIT PROGRAM 
         END IF
         DECLARE r008_c2 CURSOR FOR r008_p2
         FOREACH r008_c2 INTO l_axb02
             LET l_sql = "SELECT axb04 FROM axb_file",
                         " WHERE axb01 = '",tm.axa01,"'",
                         "   AND axb02 = '",l_axb02,"'",
                         " ORDER BY axb04"
             PREPARE r008_p3 FROM l_sql
             IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
                CALL cl_used(g_prog,g_time,2) RETURNING g_time 
                EXIT PROGRAM 
             END IF
             DECLARE r008_c3 CURSOR FOR r008_p3
             LET l_axb04 = ''
             FOREACH r008_c3 INTO l_axb04  #抓出第一層子公司
                   INSERT INTO r008_axa_file VALUES (l_axb02,l_axb04,g_j)
...
#  PREPARE r008_pre_0 FROM g_sql
#  DECLARE r008_cur_0 CURSOR FOR r008_pre_0
#  OPEN r008_cur_0
#  FETCH r008_cur_0 INTO g_aaz641   #合併帳別
#  CLOSE r008_cur_0
#FUN-A30122 --------------------------mark end--------------------------------
   ...
   FOR i = 1 TO l_tmp_count
     ...
     LET l_sql = "SELECT * FROM axf_file ",
                 ...
                 "   AND axf13 = '",tm.axa01,"'",
                 " ORDER BY axf01"
     PREPARE r008_p6 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
        EXIT PROGRAM
     END IF
     DECLARE r008_axf_c1 CURSOR FOR r008_p6
     FOREACH r008_axf_c1 INTO l_axf.*
        #--抓上層對下層 axk_file---
        SELECT axz06 INTO x_aaa03 FROM axz_file where axz01 = g_axa2[i].axb02
        IF l_axf.axf01[1,4] != 'MISC' THEN   #MOD-950214 add
           LET l_sql = "INSERT INTO r008_file ",
   ...
   PREPARE r008_prepare FROM l_sql
   DECLARE r008_curs CURSOR FOR r008_prepare
   LET g_pageno = 0
   FOREACH r008_curs INTO sr.axk02,sr.axk04,sr.axk07,sr.axk05,
...
#FUN-A30122 -----------------------------mark end--------------------------
    ...
    # CALL cl_parse_qry_sql(g_sql,l_axz03) RETURNING g_sql   #FUN-A70084        #FUN-A30122  mark
      ...
      PREPARE r008_pre FROM g_sql
      DECLARE r008_cur CURSOR FOR r008_pre
      OPEN r008_cur 
      FETCH r008_cur INTO l_aag02 
   ...
   LET g_str = '',";",tm.yy,";",tm.bm,";",tm.em,";",tm.d,";",  #No.FUN-750110  #FUN-950048
               tm.e,";",tm.axa01,";",tm.axa02,";",tm.axa03         #No.FUN-750110
   CALL cl_prt_cs3('aglr008','aglr008',l_sql,g_str)                #No.FUN-750110 
