...
# Modify.........: No.MOD-880170 08/08/26 By Sarah 增加r803_subchr(),r803_subchr1(),修正字串替換時程序當出的問題
...
# Modify.........: No:MOD-B30715 11/03/30 By Dido l_wc 轉換 aec052 位置對調;aglr803_prepare1語法調整 
...
#NO.FUN-830094-----START----
   ...
   LET l_table = cl_prt_temptable('aglr803',g_sql) CLIPPED
...
FUNCTION aglr803_tm()
   ...
   OPEN WINDOW aglr803_w AT p_row,p_col
     WITH FORM "agl/42f/aglr803"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
#      END IF
# 
#      LET tm.wc = r803_subchr(tm.wc,'"',"'")   #MOD-880170 add
#No.FUN-B20054--mark--end-- 
...
#No.FUN-B20054--add--end--
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aglr803',g_time,l_cmd)      # Execute cmd at later time
         END IF
...
DEFINE       l_bamt,l_d,l_c                 LIKE aed_file.aed05
   ...
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aglr803' 
#NO.FUN-830094---END-----
...
#  CALL cl_outnam('aglr803') RETURNING l_name             #NO.FUN-830094
#  START REPORT aglr803_rep TO l_name                     #NO.FUN-830094
  ...
  #   # FUN-5C0015  (e)
  #END FOR
   LET l_wc = r803_subchr1(l_wc,'aec01','aed01')
   LET l_wc = r803_subchr1(l_wc,'aec051','aed011')
  #LET l_wc = r803_subchr1(l_wc,'aec05','aed02')   #MOD-B30715 mark
   LET l_wc = r803_subchr1(l_wc,'aec052','aed012')
   LET l_wc = r803_subchr1(l_wc,'aec05','aed02')   #MOD-B30715
  #end MOD-880170 mod
   ...
   PREPARE aglr803_prepare2 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare2:',SQLCA.sqlcode,1)
#     CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE aglr803_curs2 CURSOR FOR aglr803_prepare2
 
   FOREACH aglr803_curs2 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
      ...
      LET l_flag='Y'
#     OUTPUT TO REPORT aglr803_rep(sr.*)           #NO.FUN-830094
#NO.FUN-830094---START-----
...
#NO.FUN-830094---END-----
   ...
   PREPARE aglr803_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare1:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      EXIT PROGRAM
   END IF
   DECLARE aglr803_curs1 CURSOR FOR aglr803_prepare1
 
   FOREACH aglr803_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
...
#  FINISH REPORT aglr803_rep                             #NO.FUN-830094
...
#NO.FUN-830094--------start------------
   ...
   IF g_zz05 = 'Y' THEN 
      CALL cl_wcchp(tm.wc,'aec01,aec051,aec052,aec05')
         RETURNING tm.wc
   END IF
   LET g_str = tm.wc,";",bdate,";",edate,";",tm.x,";",
               tm.t,";",tm.w,";",tm.e,";",g_azi04,";",
               tm.y             
   CALL cl_prt_cs3('aglr803','aglr803',g_sql,g_str)                      
#NO.FUN-7830094--------end------------
END FUNCTION
#NO.FUN-830094--START-----
#REPORT aglr803_rep(sr)
#DEFINE sr     RECORD
...
#str MOD-880170 add
FUNCTION r803_subchr(p_str,p_chr1,p_chr2)
...
FUNCTION r803_subchr1(p_str,p_chr1,p_chr2)
DEFINE p_str          LIKE type_file.chr1000
