...
DEFINE g_aed    RECORD
                   ...
                   axz01_a     LIKE axz_file.axz01,
                   axz01_b     LIKE axz_file.axz01,
                   axz02_a     LIKE axz_file.axz02,
                   axz02_b     LIKE axz_file.axz02,
       ...
       g_dept   DYNAMIC ARRAY OF RECORD
                   ...
                   axz05_a     LIKE axz_file.axz05,
                   axz01_b     LIKE axz_file.axz01,
                   azp03_b     LIKE azp_file.azp03,
                   axz05_b     LIKE axz_file.axz05
                END RECORD,
...
MAIN
   ...
   OPEN WINDOW q008_w AT p_row,p_col WITH FORM "agl/42f/aglq008"
...
DEFINE l_dept1  DYNAMIC ARRAY OF RECORD
   ...
   DIALOG ATTRIBUTES(UNBUFFERED)
      ...
      CONSTRUCT BY NAME g_wc2 ON axz01_a

         BEFORE CONSTRUCT
            CALL cl_qbe_init()
      END CONSTRUCT

      CONSTRUCT BY NAME g_wc3 ON axz01_b
      ...
      ON ACTION CONTROLP
         CASE
            WHEN INFIELD(axz01_a) #公司编号
                 CALL cl_init_qry_var()
                 LET g_qryparam.form = "q_axz"
                 LET g_qryparam.state = "c"
                 CALL cl_create_qry() RETURNING g_qryparam.multiret
                 DISPLAY g_qryparam.multiret TO axz01_a
                 NEXT FIELD axz01_a
            WHEN INFIELD(axz01_b) #公司编号
                 CALL cl_init_qry_var()
                 LET g_qryparam.form = "q_axz"
                 LET g_qryparam.state = "c"
                 CALL cl_create_qry() RETURNING g_qryparam.multiret
                 DISPLAY g_qryparam.multiret TO axz01_b
                 NEXT FIELD axz01_b
   ...
   LET g_wc2= cl_replace_str(g_wc2,'axz01_a','axz01')
   LET g_wc3= cl_replace_str(g_wc3,'axz01_b','axz01')
   ...
   LET i = 1
   LET g_sql = "SELECT axz01 FROM axz_file ",
               " WHERE ",g_wc2 CLIPPED
   PREPARE sel_axz01_a_pre FROM g_sql
   DECLARE sel_axz01_a_cur CURSOR FOR sel_axz01_a_pre
   FOREACH sel_axz01_a_cur INTO l_dept1[i].*
       LET i = i+1
   END FOREACH
   ...
   LET g_sql = "SELECT axz01 FROM axz_file ",
               " WHERE ",g_wc3 CLIPPED
   PREPARE sel_axz01_b_pre FROM g_sql
   DECLARE sel_axz01_b_cur CURSOR FOR sel_axz01_b_pre
   FOREACH sel_axz01_b_cur INTO l_dept2[j].*
       LET j = j+1
   END FOREACH
   ...
   CREATE TEMP TABLE q008_temp(
      axz01_a  LIKE type_file.chr10,
      axz01_b  LIKE type_file.chr10)
       
   FOR i = 1 TO l_dept1.getLength()   #A公司B公司排列组合
       FOR j =1 TO l_dept2.getLength()
           INSERT INTO q008_temp VALUES(l_dept1[i].axz01,l_dept2[j].axz01)
       END FOR
   END FOR

   LET g_sql=
       " SELECT * FROM q008_temp",
       " ORDER BY axz01_a,axz01_b"
...
DEFINE
    ...
    CASE p_flag
       WHEN 'N' FETCH NEXT     q008_cs INTO g_aed.axz01_a,g_aed.axz01_b
       WHEN 'P' FETCH PREVIOUS q008_cs INTO g_aed.axz01_a,g_aed.axz01_b 
       WHEN 'F' FETCH FIRST    q008_cs INTO g_aed.axz01_a,g_aed.axz01_b
       WHEN 'L' FETCH LAST     q008_cs INTO g_aed.axz01_a,g_aed.axz01_b
       WHEN '/'
           IF (NOT mi_no_ask) THEN
              ...
              END IF
           END IF
           FETCH ABSOLUTE g_jump q008_cs INTO g_aed.axz01_a,g_aed.axz01_b 
           LET mi_no_ask = FALSE
    END CASE
    IF SQLCA.sqlcode THEN
       CALL cl_err(g_aed.axz01_a,SQLCA.sqlcode,0)
       INITIALIZE g_aed.* TO NULL
       RETURN
    ELSE
       CASE p_flag
...
FUNCTION q008_show()
   ...
   DISPLAY g_aed.axz01_a TO FORMONLY.axz01_a
   DISPLAY g_aed.axz01_b TO FORMONLY.axz01_b
   DISPLAY g_qbe_aag01_a TO FORMONLY.aag01_a
   DISPLAY g_qbe_aag01_b TO FORMONLY.aag01_b
 
   SELECT axz02 INTO g_aed.axz02_a FROM axz_file WHERE axz01 = g_aed.axz01_a
   SELECT axz02 INTO g_aed.axz02_b FROM axz_file WHERE axz01 = g_aed.axz01_b
   DISPLAY g_aed.axz02_a TO FORMONLY.axz02_a
   DISPLAY g_aed.axz02_b TO FORMONLY.axz02_b
   CALL q008_b_fill_1()
   CALL q008_b_fill_2()
...
FUNCTION q008_b_fill_1()              #BODY FILL UP
   ...
   SELECT azp03,axz05 INTO l_azp03,l_axz05 FROM azp_file,axz_file
    WHERE azp01 = axz03 AND axz01 = g_aed.axz01_a
   LET l_azp03 = s_dbstring(l_azp03 CLIPPED)

  #LET l_sql = " SELECT aed01,aag02,aed02,'',aed05,aed06,'' ",                      #MOD-D30069  mark
   LET l_sql = " SELECT DISTINCT aed01,aag02,aed02,'','','','',aag06 ",             #MOD-D30069
               "   FROM ",l_azp03 CLIPPED,"aed_file,",l_azp03 CLIPPED,"aag_file",
               "  WHERE aag01 = aed01 ",
               "    AND aag00 = aed00 ",
               "    AND aed00 = '",l_axz05,"' AND aed011 = '99'",
               "    AND aed02 = '",g_aed.axz01_b,"'",     #异动码
              #"    AND aed03 = '",g_qbe_aed03,"' AND aed04 = '",g_qbe_aed04,"'",   #MOD-D30069  mark
               "    AND aed03 = '",g_qbe_aed03,"' ",                                #MOD-D30069
   ...
   FOREACH q008_bcs1 INTO g_aed_1[g_cnt].*,l_aag06       #MOD-D30069 add l_aag06
      IF STATUS THEN
         CALL cl_err('foreach:',STATUS,1)
         EXIT FOREACH
      END IF
     #MOD-D30069--begin
      LET l_sql = "SELECT aed05,aed06 FROM ",l_azp03 CLIPPED,"aed_file", 
                  " WHERE aed01 = '",g_aed_1[g_cnt].aag01_1,"'",   #科目
                  "   AND aed02 = '",g_aed.axz01_b,"'",            #异动码
                  "   AND aed03 = '",g_qbe_aed03,"'",              #年度
                  "   AND aed04 = '",g_qbe_aed04,"'",              #期别
                  "   AND aed00 = '",l_axz05,"'",                  #帐套
                  "   AND aed011='99'"
      PREPARE sel_amt2_pre FROM l_sql
      DECLARE sel_amt2_cur CURSOR FOR sel_amt2_pre
      EXECUTE sel_amt2_cur INTO g_aed_1[g_cnt].amt2_1d,g_aed_1[g_cnt].amt2_1c #期间
     ...
     #MOD-D30069--end
      ...
      #MOD-D30069--begin
      IF g_aed_1[g_cnt].amt1_1 = 0 AND g_aed_1[g_cnt].amt3_1 = 0 AND g_aed_1[g_cnt].amt2_1d = 0 AND g_aed_1[g_cnt].amt2_1c = 0 THEN
         CONTINUE FOREACH
      END IF
      #MOD-D30069--end
      LET g_cnt = g_cnt+1
...
FUNCTION q008_b_fill_2()              #BODY FILL UP
   ...
   SELECT azp03,axz05 INTO l_azp03,l_axz05 FROM azp_file,axz_file
    WHERE azp01 = axz03 AND axz01 = g_aed.axz01_b
   LET l_azp03 = s_dbstring(l_azp03 CLIPPED)
  ...
  #LET l_sql = " SELECT aed01,aag02,aed02,'',aed05,aed06,'' ",                     #MOD-D30069 mark
   ...
   FOREACH q008_bcs2 INTO g_aed_2[g_cnt].*,l_aag06       #MOD-D30069 add l_aag06
      IF STATUS THEN
         CALL cl_err('foreach:',STATUS,1)
         EXIT FOREACH
      END IF
     #MOD-D30069--begin
      LET l_sql = "SELECT aed05,aed06 FROM ",l_azp03 CLIPPED,"aed_file",
                  ...
                  "   AND aed00 = '",l_axz05,"'",                  #帐套
                  "   AND aed011='99'"
      PREPARE sel_amt2_pre_2 FROM l_sql
      DECLARE sel_amt2_cur_2 CURSOR FOR sel_amt2_pre_2
      EXECUTE sel_amt2_cur_2 INTO g_aed_2[g_cnt].amt2_2d,g_aed_2[g_cnt].amt2_2c #期间
     ...
     #MOD-D30069--end
      ...
      #MOD-D30069--begin
      IF g_aed_2[g_cnt].amt1_2 = 0 AND g_aed_2[g_cnt].amt3_2 = 0 AND g_aed_2[g_cnt].amt2_2d = 0 AND g_aed_2[g_cnt].amt2_2c = 0 THEN
         CONTINUE FOREACH
      END IF
      #MOD-D30069--end
      LET g_cnt = g_cnt+1
