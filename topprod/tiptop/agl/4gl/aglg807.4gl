...
# Modify.........: NO.FUN-B90028 11/09/13 By qirl明細CR報表轉GR 
DATABASE ds
...
TYPE sr1_t RECORD
    ...
    buff LIKE type_file.chr1000,
    d_c LIKE aed_file.aed05,
    afc06 LIKE afc_file.afc06,
    l_dif LIKE afc_file.afc06  #cy add 
END RECORD
###GENGRE###END
...
MAIN
   ...
   LET g_sql = " aed01.aed_file.aed01,",  # 科目編號
               ...
               " l_dif.afc_file.afc06 "  #cy add  
 
   LET l_table = cl_prt_temptable('aglg807',g_sql) CLIPPED
...
#  LET g_bookno = ARG_VAL(1)       #TQC-610056     #No.FUN-740020
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL aglg807_tm(0,0)                # Input print condition
      ELSE CALL aglg807()                      # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
   CALL cl_gre_drop_temptable(l_table)   #FUN-B90028 add
END MAIN
 
FUNCTION aglg807_tm(p_row,p_col)
...
DEFINE li_chk_bookno  LIKE type_file.num5        #No.FUN-B20054
   ...
   END IF
   OPEN WINDOW aglg807_w AT p_row,p_col
        WITH FORM "agl/42f/aglg807"
################################################################################
...
# 
#   IF INT_FLAG THEN
#      LET INT_FLAG = 0 CLOSE WINDOW aglg807_w 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
#      EXIT PROGRAM
#         
#   END IF
...
##     ON ACTION CONTROLP CALL aglg807_wc()   # Input detail Where Condition
#      ON IDLE g_idle_seconds
#         CALL cl_on_idle()
#         CONTINUE INPUT
# 
...
#                         " '",g_bookno CLIPPED,"'",   #TQC-610056     #No.FUN-740020
         ...
         CALL cl_cmdat('aglg807',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW aglg807_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690114
      CALL cl_gre_drop_temptable(l_table)   #FUN-B90028 add
      EXIT PROGRAM
   END IF
...
#     aed02xxxxxxxxxx aed05-06xxxx.xxx   afc06xxxxxxx.xxx   aed-afcxxxxx.xxx ax%
     ...
     LET l_sql = "SELECT aed00, aed01, aag02, aag13, aag06, aag15, aag16, aag17, aag18,",  #FUN-6C0012  #FUN-AB0020 add aed00
                ...
                #"       '','','','','','','','','','',",          #MOD-A60138 mark 
                 # FUN-5C0015 add (e)
                 "       aed02, afc06, SUM(aed05 - aed06),'' ",  #cy add '' 
...
#    LET l_name = 'aglr807.out'
     ...
     FOREACH aglr807_curs1 INTO sr.*
        ...
        IF sr.d_c < 0 THEN 
           IF sr.aag06 = '2' THEN 
              LET sr.l_dif = sr.d_c * (-1) - sr.afc06
           ELSE
              LET sr.l_dif = sr.d_c - sr.afc06
           END IF
        ELSE
           IF sr.aag06 = '1' THEN 
              LET sr.l_dif = sr.d_c - sr.afc06
        ...
        EXECUTE insert_prep USING sr.aed01,sr.aed02,sr.aag02,sr.aag06,
                l_buff,sr.d_c,sr.afc06,sr.l_dif   #cy add sr.l_dif  
        #No.FUN-780031  --End  
     END FOREACH
 
     #No.FUN-780031  --Begin
     #FINISH REPORT aglg807_rep
...
###GENGRE###     LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     ...
     IF g_zz05 = 'Y' THEN
        CALL cl_wcchp(tm.wc,'aed01,aed011,aed012,aed02')
             RETURNING g_str
     END IF
###GENGRE###     LET g_str = g_str,";",tm.afc01,";",tm.aed03,";",tm.aed04,";",tm.t,";",
###GENGRE###                 t_azi04,";",t_azi05,";",tm.e
###GENGRE###     CALL cl_prt_cs3('aglg807','aglg807',g_sql,g_str)
    CALL aglg807_grdata()    ###GENGRE###
     #No.FUN-780031  --End  
END FUNCTION
 
#No.FUN-780031  --Begin
#REPORT aglg807_rep(sr)
...
FUNCTION aglg807_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aglg807")
        IF handler IS NOT NULL THEN
            START REPORT aglg807_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE aglg807_datacur1 CURSOR FROM l_sql
            FOREACH aglg807_datacur1 INTO sr1.*
                OUTPUT TO REPORT aglg807_rep(sr1.*)
            END FOREACH
            FINISH REPORT aglg807_rep
        END IF
...
#FUN-B90028-----star-------
    ...
    DEFINE l_unit3   STRING
    DEFINE l_unit4   STRING
    DEFINE l_h1      STRING
    ...
    DEFINE l_dif     STRING			
    DEFINE l_per     STRING
    DEFINE l_amt1_print  LIKE type_file.num20_6
    DEFINE l_tmaed03     STRING
    DEFINE l_tmaed04     STRING

    DEFINE l_chr_aed01  LIKE type_file.num20_6
    DEFINE l_per_aed01  LIKE type_file.num20_6 
    DEFINE l_tmafc01     STRING
...
#FUN-B90028-----star-------
        IF tm.e = 'N' THEN 
           LET l_aag02 = cl_gr_getmsg("gre-213",g_lang,'1')
        ELSE 
           LET l_aag02= cl_gr_getmsg("gre-212",g_lang,'1')
        END IF 
        PRINTX l_aag02
        LET l_tmafc01 = tm.afc01
        LET l_tmaed03 = tm.aed03
        LET l_tmaed04 = tm.aed04 USING'&&'
        LET l_unit2 = cl_gr_getmsg("gre-224",g_lang,'1')
        LET l_unit3 = cl_gr_getmsg("gre-225",g_lang,'1')
        LET l_unit4 = cl_gr_getmsg("gre-226",g_lang,'1')
        LET l_h1 = l_unit2,l_tmafc01,' ',l_unit3,l_tmaed03.trim(),' ',l_unit4,l_tmaed04
        PRINTX l_h1 
#FUN-B90028------end--------
        ON EVERY ROW
#FUN-B90028-----star-------
            ...
            IF sr1.d_C < 0 THEN
               IF sr1.aag06 = '2' THEN
                  LET l_dif = l_amt - sr1.afc06
               ELSE 
                  LET l_dif = sr1.d_c - sr1.afc06
               END IF
            ELSE 
               IF sr1.aag06 = 1 THEN
                   LET l_dif = sr1.d_c - sr1.afc06
               ELSE 
                   LET l_dif = sr1.d_c*(-1) - sr1.afc06
               END IF 
            END IF 
            PRINTX l_dif
            
            LET l_per = l_dif/sr1.afc06*100
            PRINTX l_per
         

            LET l_fmt1 = cl_gr_numfmt("aed_file","aed05",t_azi04)
            PRINTX l_fmt1

            LET l_fmt2 = cl_gr_numfmt("aed_file","aed05",t_azi05)
            PRINTX l_fmt2
#FUN-B90028------end--------
...
#FUN-B90028-----star-------
            LET l_amt1 = SUM(sr1.d_c)
            IF l_amt1 < 0 THEN
               LET l_amt1_print = l_amt1*(-1)
            ELSE 
               LET l_amt1_print = l_amt1   
            END IF
            ...
            LET l_amt2 = GROUP SUM(sr1.afc06)
            PRINTX l_amt2            
            LET l_amt3 = GROUP SUM(sr1.l_dif)  #cy add
            PRINTX l_amt3                      #cy add

            LET l_per_aed01 = l_amt2/l_amt3*100
            PRINTX l_per_aed01
#FUN-B90028------end--------
        ON LAST ROW
