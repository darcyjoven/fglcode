...
#
# Pattern name...: acsp101.4gl
# Descriptions...: single-part-cost roll up-proposed cost
# Date & Author..: 92/02/10 By pin
...
# Modify.........: No.FUN-570142 06/02/27 By yiting 批次背景執行
...
# Modify.........: No.FUN-590003 06/12/05 By Carrier INPUT字段與PER不符,程序RUN不起來
# Modify.........: No.TQC-790177 07/09/29 By Carrier 去掉全型字符
# Modify.........: No.FUN-8B0035 08/11/12 By jan 下階料展BOM時，特性代碼抓ima910 
# Modify.........: No.FUN-980002 09/08/10 By TSD.SusanWu GP5.2架構重整，修改 INSERT INTO 語法
...
GLOBALS "../../config/top.global"
    ...
    DEFINE tm  RECORD			     	# Print condition RECORD
               item_no   LIKE ima_file.ima01,      # item no.  NO.MOD-490217
          ...
          l_msg     LIKE type_file.chr1000, #No.FUN-680071 VARCHAR(50)
...
MAIN
   ...
   CALL cl_used(g_prog,g_time,1) RETURNING g_time    #No.FUN-6A0064
...
FUNCTION p101_tm()
  DEFINE lc_cmd        LIKE type_file.chr1000 #No.FUN-680071 VARCHAR(500) #No.FUN-570142
      ...
      OPEN WINDOW p101_w WITH FORM "acs/42f/acsp101"
...
#     LET tm.p_cost      = 'N'            # default purchase use least price "No"  #No.FUN-590003
            ...
            END IF
{
         ...
         ON ACTION locale
         # CALL cl_dynamic_locale()   #NO.FUN-570142 MARK
      ...
      IF g_bgjob = "Y" THEN
          ...
          LET lc_cmd = lc_cmd CLIPPED,
                   ...
                   " '",tm.h CLIPPED,"'",
                   " '",g_bgjob CLIPPED,"'"
         CALL cl_cmdat('acsp101',g_time,lc_cmd CLIPPED)
       END IF
         CLOSE WINDOW p101_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time      #FUN-B30211
         EXIT PROGRAM
     END IF
     EXIT WHILE #FUN-980002 debug
 END WHILE
 ...
 DEFINE l_st     LIKE type_file.chr1      #No.FUN-680071 VARCHAR(01)
{
    ...
    CALL s_psdm('acsp101',tm.version) RETURNING l_st
    IF l_st ='Y' THEN
     	RETURN 0
    ELSE
        RETURN 1
...
#============================================================#
FUNCTION p101_unlock2()
  DEFINE l_st     LIKE type_file.chr1      #No.FUN-680071 VARCHAR(01)
 
    CALL p101_unwaitlock()
{
	UPDATE sma_file
		SET sma01='Y'
		WHERE sma00='0'
...
#     4.產生每料件成本要素模擬資料檔, 料件成本項目檔, 料件成本項目結構檔
 ...
 FUNCTION cost_roll_up()
   DEFINE g_material     LIKE type_file.num20_6,   #No.FUN-680071 decimal(20,6)
		  ...
		  g_add_val      LIKE type_file.num20_6,   #No.FUN-680071 DECIMAL(20,6)     #add values
...
#---->處理來源碼非P/V/W/Z/A 無產品結構料件成本逆算,假設其來源碼有誤
  SELECT ' ',' ',1,1,0,0,' ',
         ...
         ima58,ima873,ima874,imb311,imb318,imb316,imb3171,imb3172
 ...
 IF s_aa.ima08 NOT MATCHES '[PpVvWwZz]'   #檢查其是否有產品結構
    THEN SELECT count(*) into l_n FROM bmb_file
		...
		IF l_n=0 THEN
           ...
           IF NOT cl_prompt(0,0,l_msg)
              THEN CALL p101_unlock2()                 #UNLOCK
                   CALL p101_restore()
                   CALL cl_batch_bg_javamail("N")  #NO.FUN-570142
		   CALL cl_used(g_prog,g_time,2) RETURNING g_time      #FUN-B30211		   
                    EXIT PROGRAM
			 ELSE CALL p101_pvwz('P',s_aa.ima44,s_aa.ima86, #以最下階計算
                              s_aa.bmb03,s_aa.ima53,s_aa.imb318,s_aa.imb311,
                              s_aa.ima872,s_aa.ima871,s_aa.ima87)
     					    returning pur_cost, l_add_val,l_ind_m
			 END IF
        ELSE    #------為一般自製料-------#
           CALL acsp101_bom(0,tm.item_no,l_ima910)   #FUN-550100  #level,item no.
           ...
           CALL s_in_csa(tm.item_no,     #料件編號
						   ...
						   '3',            #standard cost
                            ...
                            0,tm.version  )       #附加成本
           IF g_sma.sma58 MATCHES '[Yy]'
           THEN CALL insert_cote2(tm.item_no,'3')   #複製下階成本項目資料
           END IF
...
FUNCTION acsp101_bom(p_level,p_key,p_key2)  #FUN-550100
...
#組完後的句子, 將準備成可以用的查詢命令
        PREPARE acsp101_ppp FROM l_cmd
        IF SQLCA.sqlcode THEN CALL cl_err('Prepare:',SQLCA.sqlcode,1)
                       ...
                       EXIT PROGRAM
        END IF
        DECLARE acsp101_cur CURSOR FOR acsp101_ppp
		...
		LET l_ind_m_t  =0
        LET l_ac = 1
        FOREACH acsp101_cur INTO sr[l_ac].*           #將資料丟入陣列中
{------>a. "NULL" INITIALIZE TO ZERO }
...
FOR i = 1 TO l_ac-1
    IF sr[i].bma01 IS NOT NULL  AND
	   sr[i].ima08 NOT MATCHES '[pPVvWwzZ]'
         THEN           #表非最下階元件
               #CALL acsp101_bom(p_level,sr[i].bmb03,' ') #再往下展  #FUN-550100#FUN-8B0035
                CALL acsp101_bom(p_level,sr[i].bmb03,l_ima910[i]) #再往下展  #FUN-8B0035
				...
				CALL s_in_csa (sr[i].bmb03,sr[i].ima871,l_material,'3',
...
#                    算至上階.
                IF tm.m_pur MATCHES '[Yy]'  #自製以採購計
 		     	  THEN IF sr[i].imb318 !=0 THEN
                          LET l_material=sr[i].imb318
                       END IF
				END IF
            ...
            IF tm.m_pur MATCHES '[Nn]' OR
              (tm.m_pur MATCHES '[yY]' AND sr[i].imb318=0)
             THEN
           ...
           END IF
                 ...
                 IF g_bgjob = 'Y' THEN  #NO.FUN-570142 
                     MESSAGE '料件編號:',sr[i].ima08 clipped,' '
                 END IF
     ELSE #無下階之元件
        CALL p101_pvwz(sr[i].ima08,sr[i].ima44,sr[i].ima86,
                       sr[i].bmb03,sr[i].ima53,sr[i].imb318,
                       sr[i].imb311,sr[i].ima872,sr[i].ima871,sr[i].ima87)
					    returning pur_cost, l_add_val,l_ind_m
        IF tm.p_mak MATCHES '[Yy]'  #採購以自製計(將上/下階非採購成本之值相加)
                                    #當上階之下階材料成本
           THEN  SELECT imb311+imb321+imb312+imb212+imb3131+imb213+
                        imb3132+imb2132+imb314+imb214+imb315+imb215+
						imb316+imb216+imb3171+imb2171+imb3172+imb2172+
						imb319+imb219+imb320+imb330
                   INTO pur_cost
...
#92/07/31 by pin
  ...
  SELECT count(*) INTO l_n FROM csb_file   WHERE csb01=part_no
											 ...
											 AND csb04=cost_cote
 IF l_n = 0
	THEN INSERT INTO csb_file (csb01 , csb02  ,   csb03,   csb04 ,   csb05, csbplant, csblegal) #FUN-980002 add csbplant,csblegal
...
#                     ELSE
#                          MESSAGE '料件編號:',part_no  clipped,' 新增完成'
#No.MOD-580325-end
		 END IF
        ...
        IF l_n=0
           THEN
              INSERT INTO csb_file (csb01, csb02  ,  csb03,   csb04 ,csb05 ,csbplant,csblegal) #FUN-980002 add csbplant,csblegal
	     	         VALUES  (part_no, tm.version,l_status  ,  l_csb04,t_cost,g_plant,g_legal) #FUN-980002 add g_plant,g_legal
...
FUNCTION lab_burden(l_aa,m_material)
 DEFINE l_aa RECORD
              ...
              imb3172 LIKE imb_file.imb3172   #廠外加工變動製費
          END RECORD,
         m_material LIKE type_file.num20_6,   #No.FUN-680071 DECIMAL(20,6)
         ...
         c10 LIKE type_file.num20_6   #No.FUN-680071 DECIMAL(20,6)
...
DEFINE p_aa  RECORD
	...
	p_material    LIKE type_file.num20_6,   #No.FUN-680071 DECIMAL(20,6)
...
#---->本階 fixed/variable burden cost
   CASE l_smh.smh305
        ...
        LET l_csa0305= l_csa0301 * l_smh.smh306 #本階材料成本*製費分攤率
        LET l_csa0306= l_csa0301 * l_smh.smh308 #本階材料成本*製費分攤率
...
FUNCTION  use_rout_wc(w_item,w_material,w_ima873,w_ima874,l_flag)
   DEFINE w_item LIKE ima_file.ima01,          #料件編號  NO.MOD-490217
