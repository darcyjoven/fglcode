...
#
# Pattern name...: acsp200.4gl
# Descriptions...: all-part-cost roll up-standard cost
...
GLOBALS "../../config/top.global"
    ...
    DEFINE tm  RECORD			     	# Print condition RECORD
			  ...
			  i          LIKE type_file.chr1000,   #No.FUN-680071 CHAR(50)      # simulation version description
          ...
          p_time    LIKE type_file.chr8,    #No.FUN-680071 char(08)
          l_chr     LIKE type_file.chr1,    #No.FUN-680071 CHAR(1)
          l_msg     LIKE type_file.chr1000,   #No.FUN-680071 char(70)
          l_name          LIKE type_file.chr20,      # External(Disk) file name  #No.FUN-680071 CHAR(20)
          l_n      LIKE type_file.num5,    #No.FUN-680071 SMALLINT
          l_za05   LIKE type_file.chr1000,   #  #No.FUN-680071 CHAR(40)
          l_cmd    LIKE type_file.chr1000,   #No.FUN-680071 CHAR(70)
          l_lock   LIKE type_file.num5,      #No.FUN-680071 smallint
          g_err    LIKE type_file.num5,      #No.FUN-680071 SMALLINT           # message no.
          m_err    ARRAY[15] OF  LIKE ima_file.ima02    #No.FUN-680071 CHAR(60)
DEFINE   g_change_lang   LIKE type_file.chr1      #No.FUN-680071 CHAR(1)    #No:FUN-570142
DEFINE   g_i             LIKE type_file.num5     #count/index for any purpose  #No.FUN-680071 SMALLINT
DEFINE   g_msg           LIKE type_file.chr1000 #No.FUN-680071 CHAR(72)

MAIN
  DEFINE l_flag        LIKE type_file.chr1      #No:FUN-570142  #No.FUN-680071 CHAR(1)
...
#NO.FUN-570142 START-------------------------
FUNCTION p200_tm()
  DEFINE lc_cmd        LIKE type_file.chr1000 #No.FUN-680071 CHAR(500) #No:FUN-570142
 
  OPEN WINDOW acsp200_w WITH FORM "acs/42f/acsp200"
      ...
      INPUT BY NAME tm.*,g_bgjob WITHOUT DEFAULTS  #input condition
       ...
       ON ACTION locale
         ...
         #---> The version is running ?
            SELECT csc01 FROM csc_file WHERE csc01 =tm.version AND csc14 matches '[Yy]'
...
#              CALL cl_err(tm.version,'mfg9181',0)   #No.FUN-660089
         ...
         #---> The  execute environment setup ?
            SELECT COUNT(*) INTO l_n FROM csc_file WHERE csc14 MATCHES '[yY]' AND csc15 ='2'
            IF l_n >0 THEN
      ...
      IF g_bgjob = "Y" THEN
          ...
          ELSE
             LET lc_cmd = lc_cmd CLIPPED,
                      ...
                      " '",tm.f CLIPPED,"'",
                      " '",g_bgjob CLIPPED,"'"
             CALL cl_cmdat('acsp200',g_time,lc_cmd CLIPPED)
          END IF
          CLOSE WINDOW p200_w
          CALL cl_used(g_prog,g_time,2) RETURNING g_time      #FUN-B30211
       EXIT PROGRAM
...
#               CONTINUE PROMPT
      ...
      CALL cl_wait()
#報表設定
     #LET g_len=79                                              #TQC-650044
      LET g_rlang=g_lang
      SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
      CALL cl_outnam('acsp200') RETURNING l_name
      IF g_len = 0 OR g_len IS NULL THEN LET g_len = 78 END IF  #TQC-650044
      FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR
      START REPORT p200_log TO l_name
 
#     SET LOCK MODE TO WAIT
      BEGIN WORK  #94/04/12 modify by pin
      LOCK TABLE csa_file IN EXCLUSIVE MODE
      DELETE FROM csa_file WHERE csa02=tm.version
      LOCK TABLE csb_file IN EXCLUSIVE MODE
      DELETE FROM csb_file WHERE csb02=tm.version
      LOCK TABLE csc_file IN EXCLUSIVE MODE
      DELETE FROM csc_file WHERE csc01=tm.version
      COMMIT WORK
      IF tm.x='2' THEN #不允許別人使用時,將系統 shutdown,且lock 相關檔案
         CALL p200_lock2() returning l_lock  #將系統shutdown
         IF l_lock THEN call cl_err(' ','mfg6027',0)
            CALL cl_used(g_prog,g_time,2) RETURNING g_time   # FUN-B80022
            exit program
         END IF
         IF p200_waitlock() THEN
            CALL p200_unlock2()
            EXIT WHILE
         END IF
      END IF
...
FUNCTION p200_defcur()
DEFINE l_sql  LIKE type_file.chr1000 #No.FUN-680071 CHAR(600)
...
FUNCTION p200_prt()
    ...
    OPEN WINDOW p200_prt WITH 4 ROWS, 50 COLUMNS
...
#               CONTINUE PROMPT
     ...
     END CASE
    END WHILE
     CLOSE WINDOW p200_prt
END FUNCTION
 
 
FUNCTION p200_lock2()
  DEFINE  l_st           LIKE type_file.chr1      #No.FUN-680071 CHAR(01)
{
	UPDATE sma_file
		SET sma01='N'
		WHERE sma00='0'
   ...
   CALL s_psdm('acsp200',tm.version) RETURNING l_st
   IF l_st ='Y' THEN
      RETURN 0
   ELSE
      RETURN 1
...
#============================================================#
FUNCTION p200_unlock2()
  DEFINE l_st          LIKE type_file.chr1      #No.FUN-680071 CHAR(01)
   CALL p200_unwaitlock()
   SET LOCK MODE TO WAIT
{
   UPDATE sma_file
		SET sma01='Y'
		WHERE sma00='0'
	IF SQLCA.SQLERRD[3]=0 THEN CALL cl_err('','mfg9183',0) END IF
}
   CALL s_prsm()
END FUNCTION
...
#     4.產生每料件成本要素模擬資料檔, 料件成本項目檔, 料件成本項目結構檔
 ...
 FUNCTION cost_roll_up()
   DEFINE g_material     LIKE oeb_file.oeb13,    #No.FUN-680071 decimal(20,6)
       ...
       s_aa RECORD
          ...
          END RECORD,
          l_name LIKE type_file.chr20,   #No.FUN-680071 CHAR(20)
  ...
  DECLARE p200_cur CURSOR FOR
       SELECT  ima01,' ',1,1,0,0,' ',ima08,ima53,ima87,ima871,
               ...
               imb111,imb118,imb116,imb1171,imb1172
                  INTO s_aa.* FROM ima_file, OUTER imb_file
                  WHERE ima08 NOT MATCHES '[A]'
                    AND imaacti NOT MATCHES '[Nn]'
                    AND ima01=imb01        #主件編號
                    AND ima16=0            #從最高階抓 93/04/01 pin
 
    OPEN WINDOW p200_wsg WITH 4 ROWS, 66 COLUMNS
                    					
    LET g_err=0
 
#----->讀取資料
 FOREACH p200_cur INTO s_aa.*
 IF SQLCA.sqlcode !=0
 ...
 ELSE
  ...
  IF s_aa.ima08 NOT MATCHES '[PpVvWwZz]'   #檢查其是否有產品結構
     THEN SELECT count(*) into l_n FROM bmb_file
		...
		IF l_n=0 THEN
		   CALL p200_err(5,s_aa.bmb03)
		   CALL p200_pvwz('P',s_aa.ima44,s_aa.ima86, #以最下階計算
                           s_aa.bmb03,s_aa.ima53,s_aa.imb118,s_aa.imb111,
                           s_aa.ima872,s_aa.ima871,s_aa.ima87)
     					    returning pur_cost, l_add_val,l_ind_m
                #DISPLAY  'part no.:',s_aa.bmb03 clipped,' ' AT 2,1     #CHI-A70049 mark
        ELSE    #------為一般自製料-------#
           CALL acsp200_bom(0,s_aa.bmb03,l_ima910) #FUN-550100 #level,item no.
           ...
           IF g_sma.sma58 MATCHES '[Yy]'
           THEN CALL insert_cote2(s_aa.bmb03,'1')   #複製下階成本項目資料
           END IF
...
#--->更新料件上/下階成本要素
  ...
  ELSE #---->處理來源碼為 P/V/W/Z
    IF s_aa.ima08 MATCHES '[PVWZ]'     #當為最下階料件時
       THEN CALL p200_pvwz('P',s_aa.ima44,s_aa.ima86, #以最下階計算
                           s_aa.bmb03,s_aa.ima53,s_aa.imb118,s_aa.imb111,
                           s_aa.ima872,s_aa.ima871,s_aa.ima87)
     					    returning pur_cost, l_add_val,l_ind_m
     END IF
...
END IF
 END FOREACH
 CLOSE WINDOW p200_wsg
END FUNCTION
 
FUNCTION acsp200_bom(p_level,p_key,p_key2)   #FUN-550100
...
#-->labor/burden
          labor_cost    LIKE oeb_file.oeb13,    #No.FUN-680071 DECIMAL(20,6)       #labor cost
          ind_labor     LIKE oeb_file.oeb13,    #No.FUN-680071 DECIMAL(20,6)       #indirect labor cost
          ...
          l_status  LIKE type_file.chr1,      #No.FUN-680071 CHAR(01)
          l_cmd		LIKE type_file.chr1000 #No.FUN-680071 CHAR(600)
...
#組完後的句子, 將準備成可以用的查詢命令
        PREPARE acsp200_ppp FROM l_cmd
        IF SQLCA.sqlcode THEN
           ...
           EXIT PROGRAM 
        END IF
        DECLARE acsp200_cur CURSOR FOR acsp200_ppp
    WHILE TRUE
		...
		LET l_ind_m_t  =0
        LET l_ac = 1
        FOREACH acsp200_cur INTO sr[l_ac].*           #將資料丟入陣列中
{------>a. "NULL" INITIALIZE TO ZERO }
              ...
              SELECT * INTO l_csa.*
				...
				IF SQLCA.sqlcode =0    #表此料件之前已做過, 不再做一次
				   THEN LET l_material= l_csa.csa0321  #直接抓取下階各成本
                        ...
                        LET l_status ='N'
				  #ELSE CALL acsp200_bom(p_level,sr[i].bmb03,' ') #再往下展   #FUN-550100#FUN-8B0035
				   ELSE CALL acsp200_bom(p_level,sr[i].bmb03,l_ima910[i]) #再往下展   #FUN-8B0035
...
#------>累計下階材料成本
                ...
                IF l_status ='Y'
                 THEN CALL s_in_csa (sr[i].bmb03,sr[i].ima871,l_material,'1',
                                       0,0,sr[i].ima872,0,0,tm.version)
#------->計算本階各成本
                      IF g_sma.sma58 MATCHES '[Yy]'    #有使用成本項目結構
			               THEN CALL insert_cote2(sr[i].bmb03,'1')
                       END IF
...
#---->毛需求量=發料單位, 需轉成成本單位
                ...
                #DISPLAY  'part no.:','                    ' AT 2,1  #CHI-A70049 mark
                #DISPLAY  'part no.:',sr[i].bmb03 clipped,' ' AT 2,1 #CHI-A70049 mark
            ...
            IF SQLCA.sqlcode =0
               THEN IF sr[i].ima08 MATCHES '[pP]'
                       ...
                       ELSE  LET pur_cost = l_csa.csa0301
                             ...
                             LET l_status ='N'
                     END IF
                ELSE CALL p200_pvwz(sr[i].ima08,sr[i].ima44,sr[i].ima86,
...
#92/07/31 by pin
 ...
 IF l_n = 0
	THEN INSERT INTO csb_file (csb01 , csb02  ,   csb03,   csb04 ,   csb05,
		 ...
		 IF SQLCA.sqlcode !=0
            THEN ERROR 'insert',part_no clipped,'part fail!!!',
...
{------>e.是否考慮損耗率}
	...
	FOREACH insert_cote_c USING l_item ,tm.version,l_status
	  ...
	  IF SQLCA.sqlcode !=0 THEN EXIT FOREACH END IF
          IF l_csb04 IS NULL THEN LET l_csb04= ' ' END IF
        #92/03/19 by pin
...
#--->下階成本項目成本*下階QPA/底數*(1+SCRAP RATE)
        ...
        IF l_n=0
           THEN
		      ...
		      IF SQLCA.sqlcode !=0
		           THEN ERROR 'insert',part_no clipped,'part fail!!!' sleep 2
              END IF
...
#---->本階 fixed/variable burden cost
   CASE l_smh.smh105
        ...
        LET l_csa0305= l_csa0301 * l_smh.smh106 #本階材料成本*製費分攤率
        LET l_csa0306= l_csa0301 * l_smh.smh108 #本階材料成本*製費分攤率
...
FUNCTION  use_rout_wc(w_item,w_material,w_ima873,w_ima874,l_flag)
   DEFINE w_item     LIKE ima_file.ima01,        #part no. NO:MOD-490217
		 ...
		 l_flag     LIKE type_file.chr1,                    #'c':cost center 'r':routing  #No.FUN-680071 CHAR(1)
         ...
         yn        LIKE type_file.chr1,      #No.FUN-680071 char(01)
         ...
         s_cmd    LIKE type_file.chr1000   #No.FUN-680071 char(500)
...
#CHI-A70049---mark---start---
 #IF SQLCA.sqlcode !=0
	#THEN CALL p200_err(14,tm.version)
	#ELSE DISPLAY ' ' AT 2,1
        #MESSAGE  '成本模擬新增完成!!'
 #END IF
#CHI-A70049---mark---end---
...
FUNCTION p200_err(p_code,p_part)
DEFINE
	p_code LIKE type_file.num5,      #No.FUN-680071 SMALLINT
#p_status SMALLINT,
	p_part LIKE ima_file.ima01,
#p_ord LIKE rpc_file.rpc02,
    g_msg   LIKE ze_file.ze03,          #No.FUN-680071 CHAR(60)
	l_which  LIKE type_file.chr8      #No.FUN-680071 CHAR(08)
...
#CHI-A70049---mark---start---
	...
	#END IF
#CHI-A70049---mark---end---
 	OUTPUT TO REPORT p200_log(p_part,m_err[p_code])
	LET g_err=g_err+1
END FUNCTION
...
REPORT p200_log(sr)
DEFINE
	l_trailer_sw  LIKE type_file.chr1,      #No.FUN-680071 CHAR(1)
	sr RECORD
		part LIKE ima_file.ima01,
		msg  LIKE ze_file.ze03    #No.FUN-680071 CHAR(60)
	END RECORD
