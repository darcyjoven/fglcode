...
# Modify.........: No.FUN-D10098 13/02/01 By lujh 新增gapg313
...
DEFINE tm  RECORD                             # Print condition RECORD                 #TQC-750048 add
           #wc      LIKE type_file.chr1000,   # Where condition                        #No.FUN-690028 VARCHAR(600) #FUN-CC0093 mark
...
#   CALL cl_used(g_prog,g_time,1) RETURNING g_time    #FUN-B80105 MARK	
   ...
   LET l_table = cl_prt_temptable('gapg313',g_sql) CLIPPED 
   IF l_table = -1 THEN
   ...
   LET l_table1 = cl_prt_temptable('gapg3131',g_sql) CLIPPED 
   IF l_table1 = -1 THEN 
     #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40097  #FUN-BB0047 mark     
     #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3) #FUN-B40097#FUN-C10036 mark  
      EXIT PROGRAM 
   ...
   LET l_table2 = cl_prt_temptable('gapg3132',g_sql) CLIPPED 
   IF l_table2 = -1 THEN 
     #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40097  #FUN-BB0047 mark    
     #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3) #FUN-B40097#FUN-C10036 mark 
      EXIT PROGRAM 
   ...
   LET l_table3 = cl_prt_temptable('gapg3133',g_sql) CLIPPED 
   IF l_table3 = -1 THEN 
     #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40097  #FUN-BB0047 mark    
     #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3) #FUN-B40097#FUN-C10036 mark   
      EXIT PROGRAM 
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01,    #No.FUN-580031
   ...
   OPEN WINDOW g313_w AT p_row,p_col WITH FORM "gap/42f/gapg313"
        ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
 
   CALL cl_ui_init()
   CALL cl_set_comp_visible('c',FALSE) #FUN-CC0093 add
   CALL cl_opmsg('p')
   ...
   WHILE TRUE
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('gapg313',g_time,l_cmd)    #Execute cmd at later time
         END IF
         CLOSE WINDOW g313_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40097
         CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3) #FUN-B40097
...
#          l_dbs_anm,l_dbs LIKE type_file.chr21       # No.FUN-690028 VARCHAR(20)  #No.FUN-7B0012     #FUN-A10098 ----mark
 ...
 DEFINE l_plant     LIKE type_file.chr10       #FUN-980020
   ...
   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'gapg313'
...
###GENGRE###   CALL cl_prt_cs3('gapg313','gapg313',g_sql,g_str)  
    CALL gapg313_grdata()    ###GENGRE###
...
FUNCTION gapg313_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("gapg313")
        IF handler IS NOT NULL THEN
            START REPORT gapg313_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                       ," ORDER BY apf01"      
            DECLARE gapg313_datacur1 CURSOR FROM l_sql
            FOREACH gapg313_datacur1 INTO sr1.*
                OUTPUT TO REPORT gapg313_rep(sr1.*)
            END FOREACH
            FINISH REPORT gapg313_rep
        END IF
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.apf01
            ...
            LET l_sql = "SELECT DISTINCT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,  
                       ...
                       "  ORDER BY apg02"
            START REPORT gapg313_subrep02
            DECLARE gapg313_repcur2 CURSOR FROM l_sql
            FOREACH gapg313_repcur2 INTO sr3.*
                OUTPUT TO REPORT gapg313_subrep02(sr3.*)
            END FOREACH
            FINISH REPORT gapg313_subrep02  
            #FUN-CC0093--add--end
            #付款明細
            ...
            START REPORT gapg313_subrep01
            DECLARE gapg313_repcur1 CURSOR FROM l_sql
            FOREACH gapg313_repcur1 INTO sr2.*,l_azi05
                OUTPUT TO REPORT gapg313_subrep01(sr2.*,l_azi05)
            END FOREACH
            FINISH REPORT gapg313_subrep01
            #付款總計
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE aph01 = '",sr1.apf01 CLIPPED,"'"
            START REPORT gapg313_subrep04
            DECLARE gapg313_repcur4 CURSOR FROM l_sql
            FOREACH gapg313_repcur4 INTO sr2.*
                OUTPUT TO REPORT gapg313_subrep04(sr2.*)
            END FOREACH
            FINISH REPORT gapg313_subrep04
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        ...
                        " ORDER BY aph03"                    
            START REPORT gapg313_subrep05
            DECLARE gapg313_repcur5 CURSOR FROM l_sql
            FOREACH gapg313_repcur5 INTO sr2.*
                OUTPUT TO REPORT gapg313_subrep05(sr2.*)
            END FOREACH
            FINISH REPORT gapg313_subrep05
            #FUN-CC0093--add--end
        ...
        #    START REPORT gapg313_subrep03
        #    DECLARE gapg313_repcur3 CURSOR FROM l_sql
        #    FOREACH gapg313_repcur3 INTO sr4.*
        #        OUTPUT TO REPORT gapg313_subrep03(sr4.*)
        #    END FOREACH
        #    FINISH REPORT gapg313_subrep03
        #FUN-CC0093--mark--end 
            #FUN-B40097----add-----end---------
...
#FUN-B40097----add-----str---------
REPORT gapg313_subrep01(sr2,l_azi05)
    DEFINE sr2 sr2_t
    DEFINE sr3 sr3_t
        ...
        AFTER GROUP OF sr3.apg01
          ...
          #LET l_apg05f = GROUP SUM(sr3.apg05f)  #FUN-C20053 mark
           ...
           LET l_sql = " SELECT SUM(apg05),SUM(apg05f) FROM ",
             ...
             DECLARE gapg313_subrep02_cs CURSOR FROM l_sql
             FOREACH gapg313_subrep02_cs INTO l_apg05,l_apg05f
             END FOREACH 
           #FUN-C20053----add----end------------------
           PRINTX l_apg05
           PRINTX l_apg05f            
