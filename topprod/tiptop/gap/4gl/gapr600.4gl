...
#No.FUN-7C0034 -- begin --
   ...
   LET l_table = cl_prt_temptable('gapr600',g_sql) CLIPPED
...
FUNCTION gapr600_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,    #NO FUN-690009 SMALLINT
   ...
   OPEN WINDOW gapr600_w AT p_row,p_col
        WITH FORM "gap/42f/gapr600"
################################################################################
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('gapr600',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW gapr600_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time            #FUN-B80049   ADD
      EXIT PROGRAM
...
#             " ORDER BY 1,2,3,5 "
#
#   PREPARE gapr600_prepare1 FROM l_sql
#   IF SQLCA.sqlcode != 0 THEN
#      CALL cl_err('prepare1:',SQLCA.sqlcode,1)
#      EXIT PROGRAM
#   END IF
#   DECLARE gapr600_curs1 CURSOR FOR gapr600_prepare1
#
...
#             " GROUP BY oox05",
#             " ORDER BY 1"
#   PREPARE gapr600_prepare3 FROM l_sql
#   IF SQLCA.sqlcode != 0 THEN
#      CALL cl_err('prepare3:',SQLCA.sqlcode,1)
#      EXIT PROGRAM
#   END IF
#   DECLARE gapr600_curs3 CURSOR FOR gapr600_prepare3
#
...
#             " ORDER BY 1,2,6,4 "
#
#   PREPARE gapr600_prepare2 FROM l_sql
#   IF SQLCA.sqlcode != 0 THEN
#      CALL cl_err('prepare2:',SQLCA.sqlcode,1)
#      EXIT PROGRAM
#   END IF
#   DECLARE gapr600_curs2 CURSOR FOR gapr600_prepare2
#
#   LET l_sql="SELECT oox05,SUM(oox10) FROM oox_file ",
#             " WHERE oox00 = 'AP'",
#             "   AND oox01 = ?",
...
#             " GROUP BY oox05",
#             " ORDER BY 1"
#   PREPARE gapr600_prepare4 FROM l_sql
#   IF SQLCA.sqlcode != 0 THEN
#      CALL cl_err('prepare4:',SQLCA.sqlcode,1)
#      EXIT PROGRAM
#   END IF
#   DECLARE gapr600_curs4 CURSOR FOR gapr600_prepare4
#
#   CALL cl_outnam('gapr600') RETURNING l_name
## NO.FUN-670107 --start--
##     #-----TQC-630195---------
##     IF tm.b = '2' THEN            #明細報表
##        LET g_len = 99
...
#      START REPORT gapr600_rep TO l_name
#   ELSE                          #明細報表
#      START REPORT gapr600_rep1 TO l_name
#   END IF
#
#   LET g_pageno     = 0
#   IF tm.b = '1' THEN              #統計報表 #modify NO.A080 030616
#      FOREACH gapr600_curs1 INTO sr.*
#         IF SQLCA.sqlcode != 0 THEN
...
#            LET sr.oox03v = '2'
#         END IF
#         OUTPUT TO REPORT gapr600_rep(sr.*)
#      END FOREACH
#      FINISH REPORT gapr600_rep
#   ELSE                          #明細報表
#      FOREACH gapr600_curs2 INTO sr1.*
#         IF SQLCA.sqlcode != 0 THEN
#            CALL cl_err('foreach:',SQLCA.sqlcode,1)
#            EXIT FOREACH
#         END IF
#         OUTPUT TO REPORT gapr600_rep1(sr1.*)
#      END FOREACH
#      FINISH REPORT gapr600_rep1
#   END IF
#
...
#      FOREACH gapr600_curs3 USING sr.oox01 INTO gum.*
...
#      FOREACH gapr600_curs4 USING sr.oox01,sr.oox02 INTO gum.*
#         SELECT azi05 INTO l_azi05 FROM azi_file    
#          WHERE azi01=gum.oox05
#         IF SQLCA.sqlcode THEN
#            LET l_azi05 = t_azi05    #使用本國幣別   #No.CHI-6A0004  
...
#END REPORT
   ...
   END IF
   DECLARE gapr600_curs1 CURSOR FOR gapr600_prepare1
   FOREACH gapr600_curs1 INTO sr1.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
   ...
   CALL cl_prt_cs3('gapr600',l_name,g_sql,g_str)                                                                                    
   #No.FUN-BB0047--mark--Begin---
