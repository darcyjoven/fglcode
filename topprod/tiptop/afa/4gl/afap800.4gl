...
#
# Pattern name...: afap800.4gl
# Descriptions...: 開工核准作業  
# Date & Author..: 96/05/24 By Star
# Modify.........: No.MOD-490483 04/09/30 By Kitty 執行程式會down出來
...
DEFINE g_fch RECORD
             ...
             fch04  LIKE fch_file.fch04,   #核准日期
...
#       l_time	    LIKE type_file.chr8	      #No.FUN-6A0069
   ...
   OPEN WINDOW p800_w AT p_row,p_col WITH FORM "afa/42f/afap800" 
...
#         CALL cl_dynamic_locale()
         ...
         AFTER FIELD fch01
            IF cl_null(g_fch.fch01) THEN
               NEXT FIELD fch01
            END IF
## No: 2343 modify 98/07/06  ---------------
            SELECT fch02,fch04,fch05,fch07 INTO 
                   g_fch.fch02,g_fch.fch04,g_fch.fch05,g_fch07
              FROM fch_file 
...
#              CALL cl_err('sel fch',STATUS,1)    #No.FUN-660136
               ...
               NEXT FIELD fch01
            END IF
            DISPLAY g_fch.fch01,g_fch.fch02,g_fch.fch04,g_fch.fch05 TO fch01,fch02,fch04,fch05 
##------------------------------------------
            CALL p800_show()
## No: 2345 modify 98/07/06  ---------------
            IF NOT cl_null(g_fch07) THEN
               CALL cl_err(g_fch07,'afa-135',0)
               NEXT FIELD fch01
            END IF
##------------------------------------------
 
## No: 2344 modify 98/07/06  ---------------
         AFTER FIELD fch04
            IF cl_null(g_fch.fch04) THEN
               NEXT FIELD fch04  
            END IF
...
#                CALL q_fch(7,3,g_fch.fch01,g_fch.fch02) 
#                RETURNING g_fch.fch01,g_fch.fch02,g_fch.fch04,g_fch.fch05
#                CALL FGL_DIALOG_SETBUFFER( g_fch.fch01 )
#                CALL FGL_DIALOG_SETBUFFER( g_fch.fch02 )
#                CALL FGL_DIALOG_SETBUFFER( g_fch.fch04 )
#                CALL FGL_DIALOG_SETBUFFER( g_fch.fch05 )
                 CALL cl_init_qry_var()
                 LET g_qryparam.form = "q_fch"
                 LET g_qryparam.default1 = g_fch.fch01
                 CALL cl_create_qry() RETURNING g_fch.fch01,g_fch.fch02,g_fch.fch04
#                 CALL FGL_DIALOG_SETBUFFER( g_fch.fch01 )
#                 CALL FGL_DIALOG_SETBUFFER( g_fch.fch02 )
#                 CALL FGL_DIALOG_SETBUFFER( g_fch.fch04 )
 
                 SELECT fch05 INTO g_fch.fch05 FROM fch_file
                  WHERE fch01 = g_fch.fch01
                 DISPLAY g_fch.fch01,g_fch.fch02,g_fch.fch04,g_fch.fch05 
                      TO fch01,fch02,fch04,fch05 
                 NEXT FIELD fch01
              OTHERWISE EXIT CASE
           END CASE
...
##No.2896 modify 1998/12/07
      ...
      END FOR
      IF l_flag = 'Y' THEN     
         UPDATE fch_file SET fch04 = g_fch.fch04, fch05 = g_fch.fch05 
          WHERE fch01 = g_fch.fch01
      ELSE
         UPDATE fch_file SET fch04 = NULL, fch05 = NULL
          WHERE fch01 = g_fch.fch01
      END IF
##------------------------------------
   END WHILE
...
FUNCTION p800_show()
    CALL p800_b_fill(1=1) 
    CALL cl_show_fld_cont()                   #No.FUN-550037 hmf
END FUNCTION
 
 
FUNCTION p800_b_fill(p_wc2)              #BODY FILL UP
DEFINE
    ...
    FOREACH fci_curs INTO g_fci[g_cnt].*   #單身 ARRAY 填充
     ...
     #  IF l_fci19 MATCHES '[13]' THEN LET g_fci[g_cnt].sure = 'N' END IF 
...
DEFINE     
  ...
  # l_flag          LIKE type_file.num10,        #No.FUN-680070 INTEGER
    ...
    DECLARE p800_bcl CURSOR FROM g_forupd_sql # LOCK CURSOR
    ...
    #LET l_allow_delete = cl_detail_input_auth("delete")
       ...
       #    CALL fgl_set_arr_curr(l_ac)
	    ...
	    BEGIN WORK
            ...
            IF g_rec_b >= l_ac THEN
                LET p_cmd='u'
                LET g_fci_t.* = g_fci[l_ac].*  #BACKUP
                 OPEN p800_bcl USING g_fch.fch01,g_fci_t.fci03,g_fci_t.fci031    #No.MOD-490483
                IF SQLCA.sqlcode THEN
                   CALL cl_err(g_fci[l_ac].fci02,SQLCA.sqlcode,1)   
                   LET l_lock_sw = "Y"
                ELSE
                   FETCH p800_bcl INTO g_fci[l_ac].* 
                   IF SQLCA.sqlcode THEN
                       CALL cl_err(g_fci[l_ac].fci02,SQLCA.sqlcode,1)  
                       LET l_lock_sw = "Y"
                   END IF
       ...
       AFTER ROW 
         ...
         ON ACTION accept
          FOR l_ac = 1 TO g_rec_b 
            IF g_fci[l_ac].sure = 'Y' THEN 
                #4.選擇「Y」者,
                #  ぇ更新對應免稅單身檔之狀態碼為「2.開工已核准」
                     UPDATE fci_file SET fci19 = '2'
                      WHERE fci01 = g_fch.fch01 
                        AND fci03 = g_fci[l_ac].fci03
                        AND fci031 = g_fci[l_ac].fci031 
                #  え同時更新合併編號為此財產編號之狀態碼為「2.開工已核准」
                     ...
                     FOREACH p800_bcy INTO l_fci.*
                       ...
                       END IF
                       UPDATE fci_file SET fci19 = '2' 
                        WHERE fci03 = l_fci.fci03  
                          AND fci031= l_fci.fci031  
                     END FOREACH 
               # ぉ更新對應資產主檔之免稅碼[faj40]為「6.開工核准」
                    UPDATE faj_file SET faj40 = '6'
                     WHERE faj02  = g_fci[l_ac].fci03
                       AND faj022 = g_fci[l_ac].fci031 
            ELSE 
                #5.選擇「N」者,
                # ぇ更新對應免稅單身檔之狀態碼為「3.開工刪除」
                    UPDATE fci_file SET fci19 = '3'
                     WHERE fci01 = g_fch.fch01 
                       AND fci03 = g_fci[l_ac].fci03
                       AND fci031 = g_fci[l_ac].fci031 
                # え同時更新合併編號為此財產編號之狀態碼為「3.開工刪除」
                     ...
                     FOREACH p800_bcn INTO l_fci.*
                       ...
                       UPDATE fci_file SET fci19 = '3' 
                        WHERE fci03 = l_fci.fci03  
                          AND fci031= l_fci.fci031  
                     END FOREACH 
               # ぉ更新對應資產主檔之免稅碼[faj40]為「4.開工刪除」
                    UPDATE faj_file SET faj40 = '4'
                     WHERE faj02  = g_fci[l_ac].fci03
                       AND faj022 = g_fci[l_ac].fci031 
               # お更新合併編號為此財產編號之資產主檔之
               #   免稅碼[faj40]為「4.開工刪除」
                     DECLARE p800_bca CURSOR  FOR      # LOCK CURSOR
                      SELECT fci01,fci03,fci031,fci07,fci071 
                        FROM fci_file 
                       WHERE fci01 = g_fch.fch01
                         AND fci07 = g_fci[l_ac].fci03  
                         AND fci071= g_fci[l_ac].fci031  
                     FOREACH p800_bca INTO l_fci.*
                       IF SQLCA.sqlcode != 0 THEN 
                          CALL cl_err('foreach:',SQLCA.sqlcode,1)   
                          EXIT FOREACH 
                       END IF
        ...
        #End 
         ...
         # 2.在單身每一筆資料可按[ENTER] 鍵切換「Y」「N」選定是否處理。
