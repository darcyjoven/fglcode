...
# Modify.........: No.FUN-B50140 11/05/30 By lixiang 列印時加上財簽二選項
...
#FUN-710083.....begin
   LET g_sql="fbg01.fbg_file.fbg01,",
     ...
     LET l_table = cl_prt_temptable('afar108',g_sql) CLIPPED
...
#FUN-710083.....end 
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'	# If background job sw is off
      THEN CALL r108_tm(0,0)		# Input print condition
      ELSE CALL afar108()		# Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW r108_w AT p_row,p_col WITH FORM "afa/42f/afar108"
 ...
 END CONSTRUCT
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW r108_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         EXIT PROGRAM
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('afar108',g_time,l_cmd)      # Execute cmd at later time
         END IF
         CLOSE WINDOW r108_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         EXIT PROGRAM
...
#       l_time        LIKE type_file.chr8       #No.FUN-6A0069
   ...
   #TQC-C10034--add--end  
    ...
    #FUN-920036---mark---end---
     ...
     PREPARE r108_p1 FROM l_sql                # RUNTIME 編譯
        IF STATUS THEN CALL cl_err('prepare:',STATUS,0) RETURN END IF
     DECLARE r108_co  CURSOR FOR r108_p1
#FUN-710083....mark
#     SELECT * INTO g_zo.* FROM zo_file WHERE zo01=g_rlang
#     CALL cl_outnam('afar108') RETURNING l_name
#No.FUN-580010 --start
...
#      CALL cl_prt_pos_len()
#No.FUN-580010 --end
#     START REPORT r108_rep TO l_name
#     LET g_pageno = 0
#FUN-710083....mark
...
#FUN-710083.....begin
     ...
     #CHI-C60010--add--str--#財簽二
        IF tm.b='3' THEN
           EXECUTE insert_prep USING sr.*,l_gem02a,m_fbh05,l_gen02a,l_gen02,m_faj20,l_azi04,l_azi05,
                                     "",  l_img_blob,   "N","", l_faj06 
        ELSE
     #CHI-C60010--add--end
        EXECUTE insert_prep USING sr.*,l_gem02a,m_fbh05,l_gen02a,l_gen02,m_faj20,g_azi04,g_azi05,
                                  "",  l_img_blob,   "N","",   #TQC-C10034  add
                                  l_faj06                      #TQC-BC0033 add l_faj06
        END IF  #CHI-C60010 add
#        OUTPUT TO REPORT r108_rep(sr.*,sr.tot_dep,yyyymm)  
    END FOREACH
  ...
  # CALL cl_prt_cs3('afar108',l_sql,g_str)
    #TQC-C10034--add--begin
    LET g_cr_table = l_table
    LET g_cr_apr_key_f = "fbg01" 
    #TQC-C10034--add--end
    LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
    CALL cl_prt_cs3('afar108','afar108',l_sql,g_str)
#FUN-710083.....end
 
#    FINISH REPORT r108_rep   #FUN-710083 mark
 
#    CALL cl_prt(l_name,' ','1',g_len)  #FUN-710083 mark
END FUNCTION
#FUN-710083.....begin mark
#REPORT r108_rep(sr,p_faj32,p_yyyymm)
#    DEFINE
