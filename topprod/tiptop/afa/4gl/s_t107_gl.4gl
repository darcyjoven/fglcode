...
# Modify.........: No.MOD-560181 05/07/27 By Smapmin 異動後累折<累折時, 產生的分錄借貸相反了
...
# Modify.........: No.MOD-6C0140 06/12/25 By Sarah s_def_npq()時,p_key2的地方改成傳fbd02
...
FUNCTION t107_gl(p_no,p_date,p_npptype)
...
#   DEFINE l_t1,l_fahdmy3  LIKE type_file.chr3         #No.FUN-680070 VARCHAR(03)
   DEFINE l_t1,l_fahdmy3  LIKE type_file.chr5     #No.FUN-550034          #No.FUN-680070 VARCHAR(05)
...
#     CALL cl_err('sel fah:',STATUS,0) #No.FUN-660136
   ...
   #-----MOD-690080---------
   #SELECT COUNT(*) INTO l_n FROM npp_file WHERE nppsys = 'FA' AND npp00= 9
   #                                         AND npp01 = g_no
   #                                         AND npp011 = 1
   SELECT COUNT(*) INTO l_n FROM npq_file WHERE npqsys = 'FA' AND npq00= 9
                                            AND npq01 = g_no
                                            AND npq011 = 1
   #-----END MOD-690080-----
   ...
   #FUN-B40056--add--end--
   CALL s_t107(p_npptype)     #No.FUN-680028
   DELETE FROM npq_file WHERE npq01=g_no AND npq03='-'
                          AND npqtype = p_npptype     #No.FUN-680028
   CALL s_t107_diff()                                 #FUN-A40033
...
FUNCTION s_t107(p_npptype)      #No.FUN-680028
DEFINE p_npptype  LIKE npp_file.npptype     #No.FUN-680028 
DEFINE l_fbz07    LIKE fbz_file.fbz07,  
       l_fbz071   LIKE fbz_file.fbz071,     #No.FUN-680028  
       l_fbd02    LIKE fbd_file.fbd02,      #MOD-6C0140 add
       l_fbd04    LIKE fbd_file.fbd04,  
       l_fbd05    LIKE fbd_file.fbd05,  
       l_fbd09    LIKE fbd_file.fbd09,  
       l_fbd10    LIKE fbd_file.fbd10,  
       ...
       l_diff,l_diff2  LIKE fbd_file.fbd04,
...
#FUN-AB0088---add---end---
   ...
   #-------------(單頭)-----------------------------------
   LET g_npp.nppsys = 'FA'        LET g_npp.npp00 = 9   
...
#     CALL cl_err('ins npp',STATUS,1)    #No.FUN-660136
   ...
   #-------------(單身)-----------------------------------
   LET g_npq.npqsys = 'FA'        LET g_npq.npq00 = 9   
   ...
   SELECT fbz07,fbz071 INTO l_fbz07,l_fbz071 FROM fbz_file WHERE fbz00 = '0' 
   IF SQLCA.sqlcode THEN LET l_fbz07 = ' ' END IF
 
   LET l_sql = "SELECT fbd02,fbd04,fbd05,fbd09,fbd10,",   #MOD-6C0140 add fbd02
               " faj23,faj24,faj53,faj531,faj54,faj541,faj55,faj551, ",
               " fbd042,fbd052,fbd092,fbd102,faj232,faj242 ",     ##FUN-AB0088 add
               "  FROM fbd_file LEFT OUTER JOIN faj_file ON fbd03=faj02 AND fbd031=faj022",  #No.TQC-9B0030 mod
               #"  FROM fbd_file LEFT OUTER faj_file ON fbd03=faj02 AND fbd031=faj022",  #No.TQC-9B0030 mark
               " WHERE fbd01 = '",g_no,"'"
   PREPARE t107_p FROM l_sql
   DECLARE t107_c CURSOR WITH HOLD FOR t107_p
   FOREACH t107_c INTO l_fbd02,l_fbd04,l_fbd05,l_fbd09,l_fbd10,   #MOD-6C0140 add l_fbd02
                       l_faj23,l_faj24,l_faj53,l_faj531,l_faj54,l_faj541,l_faj55,l_faj551,     #No.FUN-680028                         
                       l_fbd042,l_fbd052,l_fbd092,l_fbd102,l_faj232,l_faj242   #FUN-AB0088 add
#No.FUN-710028 --begin                                                                                                              
      IF g_success='N' THEN                                                                                                         
         LET g_totsuccess='N'                                                                                                       
         LET g_success="Y"                                                                                                          
...
#No.FUN-710028 -begin
         IF g_bgerr THEN
            CALL s_errmsg('fbd01',g_no,'t107_c',SQLCA.sqlcode,0)   #No.FUN-710028
         ELSE
            CALL cl_err('t107_c',SQLCA.sqlcode,0)  
         END IF
#No.FUN-710028 --end
         EXIT FOREACH 
      END IF
      #-------Dr.: 資產之資產科目(faj53) (負值:fbz07)--------
      IF g_npq.npqtype = '0' THEN    #FUN-AB0088 add
         LET l_diff = l_fbd09 - l_fbd04
      #FUN-AB0088---add---str---
      ELSE
         LET l_diff = l_fbd092 - l_fbd042
      END IF
      #FUN-AB0088---add---end---  
...
#           LET g_npq.npq03 = l_faj53   
            ...
            END IF
            #No.FUN-680028 --end
            LET g_npq.npq07f= l_diff 
            LET g_npq.npq07 = l_diff  
       ELSE LET g_npq.npq06 = '2'
            #No.FUN-680028 --begin
#           LET g_npq.npq03 = l_faj53  #no:5975(原LET g_npq.npq03 = l_fbz07) 
            IF g_npq.npqtype = '0' THEN
               LET g_npq.npq03 = l_faj53    
            ELSE
               LET g_npq.npq03 = l_faj531
            END IF
            #No.FUN-680028 --end
            LET g_npq.npq07f= l_diff * (-1)
            LET g_npq.npq07 = l_diff * (-1) 
         END IF
       # LET g_npq.npq07f= l_diff 
       # LET g_npq.npq07 = l_diff  
##---------------------------------
        ...
        #CALL s_def_npq(g_npq.npq03,g_prog,g_npq.*,g_npq.npq01,'','')        #MOD-6C0140 mark
         CALL s_def_npq(g_npq.npq03,g_prog,g_npq.*,g_npq.npq01,l_fbd02,'',g_bookno)   #MOD-6C0140  #No.FUN-740033
         RETURNING g_npq.*
...
#           LET g_success='N' EXIT FOREACH     #no.5573 #No.FUN-710028
         ...
         END IF
      #-------Cr.: 調整之對方科目(正值:fbz07) (負值:faj53)-----
         LET g_npq.npq02 = g_npq.npq02 + 1
##No.2882 modify 1998/12/2
         IF l_diff > 0 THEN 
            LET g_npq.npq06 = '2'
            #No.FUN-680028 --begin
#           LET g_npq.npq03 = l_fbz07 
            IF g_npq.npqtype = '0' THEN
               LET g_npq.npq03 = l_fbz07
            ELSE
               LET g_npq.npq03 = l_fbz071
            END IF
            #No.FUN-680028 --end
            LET g_npq.npq07f=l_diff  
            LET g_npq.npq07 =l_diff    
         ELSE 
            LET g_npq.npq06 = '1'
            #No.FUN-680028 --begin
#           LET g_npq.npq03 = l_fbz07  #no:5975(原LET g_npq.npq03 = l_faj53)
            IF g_npq.npqtype = '0' THEN
               LET g_npq.npq03 = l_fbz07
            ELSE
               LET g_npq.npq03 = l_fbz071
...
#           LET g_success='N' EXIT FOREACH     #no.5573 #No.FUN-710028
      ...
      #-------Dr.: 資產之累折科目(faj55) (負值:faj54)--------
      #NO:4614若累折增加,正確分錄應為借:折舊 貸:累折
      IF g_npq.npqtype = '0' THEN    #FUN-AB0088 add
         LET l_diff2 = l_fbd10 - l_fbd05
      #FUN-AB0088---add---str---
      ELSE
          LET l_diff2 = l_fbd102 - l_fbd052
      END IF  
      #FUN-AB0088---add---end---
      IF l_diff2 <> 0 THEN
         LET g_npq.npq02 = g_npq.npq02 + 1
##No.2882 modify 1998/12/2
        IF l_diff2 > 0 THEN 
           LET g_npq.npq06 = '1'
         #No.FUN-680028 --begin
#          LET g_npq.npq03 = l_faj55    #原l_faj54
         IF g_npq.npqtype = '0' THEN
            LET g_npq.npq03 = l_faj55
         ELSE
            LET g_npq.npq03 = l_faj551
         END IF
         #No.FUN-680028 --end
         LET g_npq.npq07f= l_diff2 
         LET g_npq.npq07 = l_diff2 
      ELSE 
     #   LET g_npq.npq06 = '2'   #MOD-560181
         LET g_npq.npq06 = '1'   #MOD-560181
         #No.FUN-680028 --begin
#        LET g_npq.npq03 = l_faj54    #原l_faj55
         IF g_npq.npqtype = '0' THEN
         ...
         END IF
         #No.FUN-680028 --end
         LET g_npq.npq07f= l_diff2 * (-1)
         LET g_npq.npq07 = l_diff2 * (-1) 
      END IF
      LET g_npq.npq04 = NULL   #MOD-6C0140 add
   #  LET g_npq.npq07f= l_diff2 
   #  LET g_npq.npq07 = l_diff2 
