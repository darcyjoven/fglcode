...
# Modify.........: No.FUN-C10036 12/01/11 By xuxz FUN-B80081追單
# Modify.........: No.FUN-C40020 12/04/10 By qirl  GR報表列印TIPTOP與EasyFlow簽核圖片
# Modify.........: No.FUN-C50008 12/05/14 By wangrr MOD-C30044 追單1.l_sql/tm.wc 改為 STRING 2.需抓取已確認的資產
# Modify.........: No.FUN-C40020 12/07/31 By qirl   MARKGR報表列印TIPTOP與EasyFlow簽核圖片 
...
GLOBALS "../../config/top.global"
    ...
    DEFINE tm  RECORD				# Print condition RECORD
          #wc      LIKE type_file.chr1000,		# Where condition       #No.FUN-680070 VARCHAR(1000) #FUN-C50008 mark--
           wc      STRING,               #FUN-C50008 add
...
#No.FUN-710083 --begin
   ...
   LET l_table = cl_prt_temptable('afag217',g_sql) CLIPPED
   IF l_table = -1 THEN 
   ...
   LET g_sql = "INSERT INTO ds_report.",l_table CLIPPED,
               " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?,",
               "        ?, ?, ?, ?, ?, ? )"   #?, ?, ?, ?)"   # No.FUN-C40020 add   # No.FUN-C40020 mark
   ...
   END IF
#No.FUN-710083 --end
#FUN-B50018----remark ---end----------
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN    # If background job sw is off
      CALL afag217_tm(0,0)		     # Input print condition
   ELSE
      CALL afag217()		             # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
   CALL cl_gre_drop_temptable(l_table)
END MAIN
 
FUNCTION afag217_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW afag217_w AT p_row,p_col WITH FORM "afa/42f/afag217"
 ...
 END CONSTRUCT
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW afag217_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         CALL cl_gre_drop_temptable(l_table)
         EXIT PROGRAM
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('afag217',g_time,l_cmd)      # Execute cmd at later time
         END IF
         CLOSE WINDOW afag217_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         CALL cl_gre_drop_temptable(l_table)
         EXIT PROGRAM
      END IF
...
#       l_time          LIKE type_file.chr8	    #No.FUN-6A0069
        #  l_sql 	LIKE type_file.chr1000,		# RDSQL STATEMENT       #No.FUN-680070 VARCHAR(1000) #FUN-C50008 mark--
          l_sql         STRING,                      #FUN-C50008 add 
...
#  LOCATE l_img_blob        IN MEMORY            # No.FUN-C40020 add    # No.FUN-C40020 mark
     CALL cl_del_data(l_table)     #No.FUN-710083   #MOD-880175 mark  #FUN-B50018
     SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
#No.FUN-710083 --begin
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'afag217'
...
#No.FUN-710083 --end
     ...
     LET l_sql = "SELECT faj01,faj02,faj022,faj20,gem02,faj19,gen02,faj06,",
                 ...
                 " WHERE ",tm.wc CLIPPED
                ,"   AND fajconf = 'Y' "     #FUN-C50008 add
     #No.CHI-480001
     IF tm.d = '1' THEN    #停用
       # LET l_sql = l_sql CLIPPED," AND faj105 = 'Y' "#FUN-C10036 mark
       LET l_sql = l_sql CLIPPED," AND faj43 = 'Z' " #FUN-C10036 add
     END IF
     IF tm.d = '0' THEN    #正常使用
        LET l_sql = l_sql CLIPPED,
                   # " AND (faj105 = 'N' OR faj105 IS NULL OR faj105 = ' ') ",#FUN-C10036 mark
                    " AND faj43<>'Z' ", #No.FUN-C10036 add
                    " AND faj43!='5' AND (faj43 !='6' OR faj43 ='6' AND faj17 >faj58)"   #No.MOD-950224
     END IF
     #end
     #FUN-B50018---add---str-----------
     PREPARE afag217_prepare1 FROM l_sql
     DECLARE afag217_curs1 CURSOR FOR afag217_prepare1 
     FOREACH afag217_curs1 INTO sr.*
       IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF  
       EXECUTE insert_prep USING sr.*   #,"",  l_img_blob,"N",""  # No.FUN-C40020 add   # No.FUN-C40020 mark
     END FOREACH
     #FUN-B50018---add---end----------
 
#No.FUN-710083 --begin
   # CALL cl_prt_cs1('afag217',l_sql,'')   #TQC-730088
   # CALL cl_prt_cs1('afag217','afag217',l_sql,'')   #FUN-B50018
#   LET g_cr_table = l_table                    # No.FUN-C40020 add    # No.FUN-C40020 mark
#   LET g_cr_apr_key_f = "faj01"          # No.FUN-C40020 add          # No.FUN-C40020 mark
     CALL afag217_grdata()   #FUN-B50018
#    LET l_rec = 0
#    LET i     = 0
#    LET j     = 0
#    PREPARE afag217_prepare1 FROM l_sql
...
#    END IF
#    DECLARE afag217_curs1 CURSOR FOR afag217_prepare1
#    CALL cl_outnam('afag217') RETURNING l_name
#    START REPORT afag217_rep TO l_name
#    LET g_cnt = 0
#    FOREACH afag217_curs1 INTO sr.*
#      IF SQLCA.sqlcode != 0 THEN
...
#      END IF
#      LET g_cnt = g_cnt + 1
#      OUTPUT TO REPORT afag217_rep(sr.*)
#    END FOREACH
 
#    FINISH REPORT afag217_rep
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
#No.FUN-710083 --end
END FUNCTION
 
#No.FUN-710083 --begin
#REPORT afag217_rep(sr)
#   DEFINE
...
#   CALL cl_gre_init_apr()             # No.FUN-C40020 add    # No.FUN-C40020 mark
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("afag217")
        IF handler IS NOT NULL THEN
            START REPORT afag217_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED

            DECLARE afag217_datacur1 CURSOR FROM l_sql
            FOREACH afag217_datacur1 INTO sr1.*
                OUTPUT TO REPORT afag217_rep(sr1.*)
            END FOREACH
            FINISH REPORT afag217_rep
        END IF
...
REPORT afag217_rep(sr1)
    DEFINE  l_lineno       LIKE type_file.num5
    DEFINE  l_faj10_pmc03  STRING
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            LET   l_faj10_pmc03 = sr1.faj10,' ',sr1.pmc03
            PRINTX  l_faj10_pmc03

            LET   l_faj19_gen02 = sr1.faj19,' ',sr1.gen02
            PRINTX  l_faj19_gen02
           
            LET   l_faj20_gem02 = sr1.faj20,' ',sr1.gem02
