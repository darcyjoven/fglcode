...
# Modify.........: No.FUN-C20052 12/02/23 By chenying GR調整
...
#No.FUN-710083 --begin
   ...
   LET l_table = cl_prt_temptable('afag750',g_sql) CLIPPED
   IF l_table = -1 THEN 
...
#No.FUN-710083 --end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'       # If background job sw is off
      THEN CALL g750_tm(0,0)                  # Input print condition
      ELSE CALL afag750()                     # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
   CALL cl_gre_drop_temptable(l_table)
END MAIN
 
FUNCTION g750_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,    #No.FUN-680070 SMALLINT
   ...
   OPEN WINDOW g750_w AT p_row,p_col WITH FORM "afa/42f/afag750"
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW g750_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         CALL cl_gre_drop_temptable(l_table)
         EXIT PROGRAM
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('afag750',g_time,l_cmd)
         END IF
         CLOSE WINDOW g750_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         CALL cl_gre_drop_temptable(l_table)
...
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'afag750'
...
#No.FUN-710083 --end
     ...
     LET l_sql = "SELECT fcc01,fcc02,fcb03,fcb04,fcc06,fcc07,fcc11,fcc08,fcc17,fcc18, ",
                ...
                #"  FROM fcb_file,fcc_file,OUTER faj_file  ",  #FUN-C50008 mark--
                 "  FROM fcb_file,fcc_file LEFT OUTER JOIN faj_file ON (faj02=fcc03 AND faj022=fcc031) ",  #FUN-C50008 add LEFT OUTER
                 " WHERE fcb01 = fcc01 ",
                 "   AND fcbconf != 'X' ",   #010802增
                 "   AND fcc04 != '0' ",
               # "   AND fcc09 != 0 ",
                 "   AND fcc21 != 0 ",
               # "   AND faj_file.faj02=fcc_file.fcc03 ",   #FUN-C50008 mark--
               # "   AND faj_file.faj022=fcc_file.fcc031 ", #FUN-C50008 mark--
                 "   AND ",tm.wc,
                 "   ORDER BY fcc02,faj25"
 
     PREPARE g750_prepare FROM l_sql
     ...
     END IF
     DECLARE g750_cs CURSOR FOR g750_prepare
 
#No.FUN-710083 --begin
#    CALL cl_outnam('afag750') RETURNING l_name
 
#    START REPORT g750_rep TO l_name
#    LET g_pageno=0
#No.FUN-710083 --end
 
#    CALL g750_create_temp1()   #FUN-B40087
     LET g_line = 0
     FOREACH g750_cs INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#No.FUN-710083 --begin
  ...
  #  CALL cl_prt_cs3('afag750',l_sql,'')
###GENGRE###     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
###GENGRE###     CALL cl_prt_cs3('afag750','afag750',l_sql,'')

    CALL afag750_grdata()    ###GENGRE###
#    FINISH REPORT g750_rep
...
FUNCTION afag750_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("afag750")
        IF handler IS NOT NULL THEN
            START REPORT afag750_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY fcc02"   #FUN-C20052  
          
            DECLARE afag750_datacur1 CURSOR FROM l_sql
            FOREACH afag750_datacur1 INTO sr1.*
                OUTPUT TO REPORT afag750_rep(sr1.*)
            END FOREACH
            FINISH REPORT afag750_rep
        END IF
...
REPORT afag750_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno       LIKE type_file.num5
    #FUN-B40087  ADD --------STR -----
    DEFINE l_fcc06        LIKE fcc_file.fcc06
    DEFINE l_fcc07        LIKE fcc_file.fcc07
    DEFINE l_fcc08        LIKE fcc_file.fcc08
    DEFINE l_fcc09        LIKE fcc_file.fcc09
    DEFINE l_fcc11        LIKE fcc_file.fcc11
    DEFINE l_fcc12        LIKE fcc_file.fcc12
    DEFINE l_fcc14        LIKE fcc_file.fcc14
    DEFINE l_fcc09_fcc13  LIKE type_file.num26_10
    #FUN-B40087  ADD --------END ----- 

#   ORDER EXTERNAL BY sr1.fcc02
...
#       BEFORE GROUP OF sr1.fcc02
        ...
        ON EVERY ROW
            ...
            #FUN-B40087  ADD --------STR -----
            LET l_fcc06 = sr1.fcc06[1,15]
            PRINTX l_fcc06
            LET l_fcc07 = sr1.fcc07[1,12]
            PRINTX l_fcc07
            LET l_fcc08 = sr1.fcc08
            PRINTX l_fcc08
            IF cl_null(sr1.fcc09) THEN 
               LET l_fcc09 = sr1.fcc09
            ELSE
               LET l_fcc09 = 0 
            END IF 
            PRINTX l_fcc09
            LET l_fcc09_fcc13 = sr1.fcc09 * sr1.fcc13
            PRINTX l_fcc09_fcc13
            IF cl_null(sr1.fcc12) THEN 
               LET l_fcc12 = 0
            ELSE
               LET l_fcc12 = sr1.fcc12
            END IF 
            PRINTX l_fcc12
            LET l_fcc11 = sr1.fcc11[1,10]
            PRINTX l_fcc11
            LET l_fcc14 = sr1.fcc14[1,4]
            PRINTX l_fcc14
            #FUN-B40087  ADD --------END -----

            PRINTX sr1.*

#       AFTER GROUP OF sr1.fcc02
