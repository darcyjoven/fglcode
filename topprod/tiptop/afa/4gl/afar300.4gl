...
# Modify.........: No.MOD-820130 08/02/22 By dxfwo   afar300 當某一資產在一年內提完折舊以后，
#                                                    次年在打印的時候 前期折舊為0，
#                                                    應該是有折舊的。
# Modify.........: No.MOD-870156 08/07/17 By Sarah 當aza26='2'時,l_name='afar300',aza26!='2'時,l_name='afar300_1'
...
# Modify.........: No.MOD-980136 09/09/03 By sabrina r300_presum的SQL條件下錯了。
...
# Modify.........: No:MOD-CC0255 13/04/03 By apo 調整折畢後金額錯誤問題
# Modify.........: No:TQC-DA0034 13/10/23 By yangxf “資產類型”，“次類型”，“財產編號”，“財產附號”，“中文名稱”,“部門範圍”欄位增加開窗
...
#NO.FUN-7B0139-------start-------
   ...
   LET l_table = cl_prt_temptable('afar300',g_sql) CLIPPED                                                                          
...
#NO.FUN-7B0139-------end-------
  ...
  #LET tm.b  = ARG_VAL(15)   #CHI-A50027 add#TQC-C50106 mark
   #No:FUN-570264 --start--
   LET g_rep_user = ARG_VAL(16)  #CHI-A50027 mod 15->16
   LET g_rep_clas = ARG_VAL(17)  #CHI-A50027 mod 16->17
   LET g_template = ARG_VAL(18)  #CHI-A50027 mod 17->18
   LET g_rpt_name = ARG_VAL(19)  #No:FUN-7C0078  #CHI-A50027 mod 18->19
   #No.FUN-570264 ---end---
   #Polly
#No.CHI-6A0004--begin
...
#No.CHI-6A0004--end
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      ...
      ELSE CALL afar300()               # Read data and create out-file
   END IF
...
FUNCTION r300_tm(p_row,p_col)
   ...
   OPEN WINDOW r300_w AT p_row,p_col WITH FORM "afa/42f/afar300"
...
#TQC-DA0034 add begin ---
         ON ACTION controlp
           CASE
              WHEN INFIELD(fan06)   #財產編號
                 CALL cl_init_qry_var()
                 LET g_qryparam.state = "c"
...
#TQC-DA0034 add end ---
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('afar300',g_time,l_cmd)      # Execute cmd at later time
         END IF
         CLOSE WINDOW r300_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         EXIT PROGRAM
...
#                "        AND fan07 > 0 )) "
     ...
     CASE
        ...
        OTHERWISE EXIT CASE
     END CASE
     PREPARE r300_prefan  FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
        EXIT PROGRAM
     END IF
     DECLARE r300_csfan CURSOR FOR r300_prefan
     ...
     CASE
        ...
        OTHERWISE EXIT CASE
     END CASE
     PREPARE r300_presum FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('presum:',SQLCA.sqlcode,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
        EXIT PROGRAM
     END IF
     DECLARE r300_cssum CURSOR FOR r300_presum
 
#NO.FUN-7B0139---------mark---start-----
#    CALL cl_outnam('afar300') RETURNING l_name
 
#No.FUN-580010 --start
     IF g_aza.aza26 = '2' THEN
#       LET g_zaa[44].zaa06 = "N"   #MOD-620069
       #LET l_name = 'afar300_1'    #NO.FUN-7B0139   #MOD-870156 mark
        LET l_name = 'afar300'      #NO.FUN-7B0139   #MOD-870156
     ELSE
#       LET g_zaa[44].zaa06 = "Y"   #MOD-620069
       #LET l_name = 'afar300'      #NO.FUN-7B0139   #MOD-870156 mark
        LET l_name = 'afar300_1'    #NO.FUN-7B0139   #MOD-870156
     END IF
#    CALL cl_prt_pos_len()
#No.FUN-580010 --end
#
...
#+NO.FUN-7B0139-------end----------------
      ...
      #MOD-A40056---add---end---
       ...
       FOREACH r300_csfan USING sr.faj02,sr.faj022
          INTO sr2.*
          IF SQLCA.sqlcode != 0 THEN
             CALL cl_err('r300_csfan:',SQLCA.sqlcode,1) EXIT FOREACH
          END IF
          LET l_flag = 'N'
          OPEN r300_cssum USING sr.faj02,sr.faj022,sr2.fan06
          FETCH r300_cssum INTO sr2.fan15,sr2.fan17        #No.CHI-480001
         ...
         #LET l_adj_fan07 = 0
         #LET l_adj_fap54 = 0
         #CALL r300_chk_adj(sr.faj02,sr.faj022,sr2.fan15)           #MOD-C40130 mark
         #   RETURNING l_adj_fan07,l_adj_fan07_1,l_adj_fap54        #MOD-C40130 mark
         #CALL r300_chk_adj(sr.faj02,sr.faj022,sr2.fan15,tm.mm)               #CHI-CA0063 mark #MOD-C40130 add
          CALL r300_chk_adj(sr.faj02,sr.faj022,sr2.fan15,tm.mm,sr2.fan06)     #CHI-CA0063
...
#         LET sr.order3 = l_order[3]
          #No.+364 end---
#         OUTPUT TO REPORT r300_rep(sr.*,sr2.*)
#NO.FUN-7B0139------------------end-------------
#NO.FUN-7B0139------start-------
       ...
       IF l_flag ='Y' THEN
         ...
         #            " fan09, fan14, fan15,0,fan08,0,0,0,fan17 "
          ...
          #-----END TQC-630111-----
             ...
             LET l_adj_fap54 = 0
            #CALL r300_chk_adj(sr.faj02,sr.faj022,sr2.fan15)           #MOD-C40130 mark
            #CALL r300_chk_adj(sr.faj02,sr.faj022,sr2.fan15,l_fan04)      #CHI-CA0063 mark #MOD-C40130 add
             CALL r300_chk_adj(sr.faj02,sr.faj022,sr2.fan15,l_fan04,'')   #CHI-CA0063
                RETURNING l_adj_fan07,l_adj_fan07_1,l_adj_fap54
            #LET sr2.fan15 = sr2.fan15 + l_adj_fan07                      #MOD-CC0255 mark
             LET sr2.fan07 = sr2.fan07 + l_adj_fan07_1
            #LET sr2.curr_d = sr2.curr_d + l_adj_fan07                    #MOD-CC0255 mark
            #LET sr2.fan14 = sr2.fan14 + l_adj_fap54                      #MOD-CC0255 mark
             LET sr2.fan15 = sr2.fan15                                    #MOD-CC0255 add
             LET sr2.curr_d = sr2.curr_d                                  #MOD-CC0255 add
             LET sr2.fan14 = sr2.fan14                                    #MOD-CC0255 add
             #CHI-B70033 -- end --
...
#               END IF                                      #MOD-820130
             ...
             LET sr2.cost = sr2.curr_val - sr2.fan17
             #end
#            OUTPUT TO REPORT r300_rep(sr.*,sr2.*)     #NO.FUN-7B0139
...
#   FINISH REPORT r300_rep                            #NO.FUN-7B0139
...
#NO.FUN-7B0139-----------start-----
    ...
    LET g_str = tm.wc,";",tm.yyyy,";",tm.mm,";",tm.s[1,1],";",                                                 
                ...
                tm.t[1,1],";",tm.t[2,2],";",tm.t[3,3],";",tm.a   #CHI-A50027 add tm.b #TQC-C50106 mod  tm.b---->tm.a                                            
                                                                                                             
    LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                               
    CALL cl_prt_cs3('afar300',l_name,g_sql,g_str)
#NO.FUN-7B0139----------end----
END FUNCTION
 
#CHI-B70033 -- begin --
#判斷是否要回傳調整金額
#FUNCTION r300_chk_adj(p_faj02,p_faj022,p_fan15)          #MOD-C40130 mark
#FUNCTION r300_chk_adj(p_faj02,p_faj022,p_fan15,p_fan04)          #CHI-CA0063 mark #MOD-C40130 add
FUNCTION r300_chk_adj(p_faj02,p_faj022,p_fan15,p_fan04,p_fan06)   #CHI-CA0063
