...
# Modify.........: No.FUN-CC0093 12/11/05 By zhangweib CR轉XtraGrid
# Modify.........: No.FUN-D30070 13/03/21 By yangtt 群組CALL lib函數錯誤
# Modify.........: No.FUN-D40129 13/05/15 By yangtt 盤點編號、資產類型、財產編號、序號、保管人、保管部門、存放位置栏位增加開窗
...
GLOBALS "../../config/top.global"
    ...
    DEFINE tm  RECORD                    # Print condition RECORD
              ...
              s        LIKE type_file.chr5,           # Order by sequence       #No.FUN-680070 VARCHAR(4)   #No.FUN-CC0093   Mod chr4 --> chr5
              t        LIKE type_file.chr5,           # Eject sw       #No.FUN-680070 VARCHAR(3)            #No.FUN-CC0093   Mod chr3 --> chr5
...
### *** FUN-770052 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>>--*** ##                                                     
    LET g_sql = "fca01.fca_file.fca01,",
                "fca01_1.type_file.chr50,",    #No.FUN-CC0093   Add
                "fca02.fca_file.fca02,",                                                                                            
                "fca03.fca_file.fca03,",
                "fca03_1.type_file.chr50,",    #No.FUN-CC0093   Add                                                                    
                ...
                "num51.type_file.num5,",
                "fca051.fca_file.fca05,",      #No.FUN-CC0093   Add
                "fca061.fca_file.fca06,",      #No.FUN-CC0093   Add
                "fca071.fca_file.fca07"        #No.FUN-CC0093   Add
    LET l_table = cl_prt_temptable('afax610',g_sql) CLIPPED   # 產生Temp Table                                                      
    IF l_table = -1 THEN EXIT PROGRAM END IF                  # Temp Table產生                                                      
    LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,                                                                           
                " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,? ,?,?,?,?)"  #CHI-AB0020 mod #No.FUN-CC0093  ?,?,?,?,?,?          
...
#----------------------------------------------------------CR (1) ------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' # If background job sw is off
      THEN CALL x610_tm(0,0)         # Input print condition
      ELSE CALL x610()               # Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW x610_w AT p_row,p_col WITH FORM "afa/42f/afax610"
       ...
       ON ACTION CONTROLP
        CASE
           ...
           WHEN INFIELD(fca04)
                CALL cl_init_qry_var()
                LET g_qryparam.state = "c"
                LET g_qryparam.form = "q_fca04"
                CALL cl_create_qry() RETURNING g_qryparam.multiret
                DISPLAY g_qryparam.multiret TO fca04
                NEXT FIELD fca04
           WHEN INFIELD(fca05)
                CALL cl_init_qry_var()
                LET g_qryparam.state = "c"
...
## --------------------------------------
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW x610_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         EXIT PROGRAM
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('afax610',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW x610_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0069
     ...
     DEFINE l_fca01   LIKE type_file.chr50   #No.FUN-CC0093   Add
     DEFINE l_fca03   LIKE type_file.chr50   #No.FUN-CC0093   Add
...
#      OUTPUT TO REPORT x610_rep(sr.*)}                                #FUN-770052
        ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-770052 *** ##                                                   
           LET l_fca01 = sr.fca01," ",sr.fca02 USING "<<<<<" CLIPPED   #No.FUN-CC0093   Add
           LET l_fca03 = sr.fca03," ",sr.fca031 USING "<<<<" CLIPPED   #No.FUN-CC0093   Add
           EXECUTE insert_prep USING                                                                                                
                  #sr.fca01,sr.fca02,sr.fca03,sr.fca031,sr.fca05,sr.gem02,sr.fca06,sr.gen02,sr.fca07,sr.faf02,sr.faj26, #CHI-AB0020 add gem02,gen02,faf02   #No.FUN-CC0093   Mark
                   sr.fca01,l_fca01,sr.fca02,sr.fca03,l_fca03,sr.fca031,sr.fca05,sr.gem02,sr.fca06,sr.gen02,sr.fca07,sr.faf02,sr.faj26, #No.FUN-CC0093   Add
                   sr.faj07,sr.faj06,sr.faj08,sr.fcacnt,sr.fca04,sr.faj04,sr.fca21               #FUN9A0036 add fca21
          #------------------------------ CR (3) ------------------------------#
                  ,'','','',''   #No.FUN-CC0093  Add
     END FOREACH
 
#    FINISH REPORT x610_rep                                            #FUN-770052
...
###XtraGrid###    LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                                
    LET g_str = ''                                                                                                                  
###XtraGrid###    LET g_str = g_head1,";",'',";",tm.wc,";",g_azi04,";",tm.s[1,1],";",                                                          
###XtraGrid###                tm.s[2,2],";",tm.s[3,3],";",                                                                                        
###XtraGrid###                tm.t[1,1],";",tm.t[2,2],";",tm.t[3,3]                                    
###XtraGrid###    CALL cl_prt_cs3('afax610','afax610',l_sql,g_str)                                                                                  
    LET g_xgrid.table = l_table    ###XtraGrid###
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"fca01,faj04,fca03,fca04,fca06,fca05,fca07,faj26,fca21")
    IF tm.s[1] = '1' OR tm.s[2] = '1' OR tm.s[3] = '1' THEN 
       LET g_xgrid.order_field = cl_replace_str(g_xgrid.order_field,'fca01','fca01,fca02')
    END IF
   #LET g_xgrid.grup_field = cl_get_sum_field(tm.s,tm.t,"fca01,faj04,fca03,fca04,fca06,fca05,fca07,faj26,fca21")    #FUN-D30070 mark
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"fca01,faj04,fca03,fca04,fca06,fca05,fca07,faj26,fca21")  #FUN-D30070 add
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"fca01,faj04,fca03,fca04,fca06,fca05,fca07,faj26,fca21")
   #LET g_xgrid.condition= tm.wc   #FUN-D40129 mark
    LET g_xgrid.condition= cl_getmsg('lib-160',g_lang),tm.wc    #FUN-D40129 add 
    LET g_xgrid.footerinfo1=cl_getmsg('afa-197',g_lang)
    CALL cl_xg_view()    ###XtraGrid###
    #------------------------------ CR (4) ------------------------------# 
END FUNCTION
 
{REPORT x610_rep(sr)                                     #FUN-770052
