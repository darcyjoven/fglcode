...
#
# Modify.........: No.MOD-490624 04/09/01 By Yuna 輸入負數金額時,金額帶負數
...
# Modify.........: No.MOD-6C0140 06/12/25 By Sarah s_def_npq()時,p_key2的地方改成傳fbb02
...
FUNCTION t106_gl(p_no,p_date,p_npptype)
...
#     CALL cl_err('sel fah:',STATUS,0)   #No.FUN-660136
   ...
   #-----MOD-690080---------
   #SELECT COUNT(*) INTO l_n FROM npp_file WHERE nppsys = 'FA' AND npp00= 8
   #                                         AND npp01 = g_no
   #                                         AND npp011 = 1
   SELECT COUNT(*) INTO l_n FROM npq_file WHERE npqsys = 'FA' AND npq00= 8
                                            AND npq01 = g_no
                                            AND npq011 = 1
   #-----END MOD-690080-----
   ...
   #FUN-B40056--add--end--
   CALL s_t106(p_npptype)
   DELETE FROM npq_file WHERE npq01=g_no  AND npq03='-'
                          AND npqsys='FA' AND npq011 = 1
                          AND npq00=8
                          AND npqtype = p_npptype     #No.FUN-680028
   CALL s_t106_diff() #FUN-A40033
...
FUNCTION s_t106(p_npptype)      #No.FUN-680028
DEFINE p_npptype  LIKE npp_file.npptype     #No.FUN-680028 
DEFINE l_fbz06    LIKE fbz_file.fbz06,  
       l_fbz061   LIKE fbz_file.fbz061,     #No.FUN-680028  
       l_npq07_dr LIKE fbb_file.fbb08,  
       l_npq07_cr LIKE fbb_file.fbb08,  
       l_fbb02    LIKE fbb_file.fbb02,      #MOD-6C0140 add
       l_fbb08    LIKE fbb_file.fbb08,  
...
## No:2386 ---
   ...
   LET l_npq07_cr=0
   #-------------(單頭)-----------------------------------
   LET g_npp.nppsys = 'FA'        LET g_npp.npp00 = 8   
   LET g_npp.npp01  = g_no        LET g_npp.npp011= 1
   LET g_npp.npp02  = g_date      LET g_npp.npp03 = ' '
...
#     CALL cl_err('ins npp',STATUS,1)    #No.FUN-660136
  ...
  #固資已不和二套掛鉤，所以原本利用s_get_bookno取到賬別一及賬別二是用是否使用二套賬來判斷
   ...
   #-------------(單身)-----------------------------------
   #-------------(單身)-----------------------------------
   LET g_npq.npqsys = 'FA'        LET g_npq.npq00 = 8   
   ...
   SELECT fbz06,fbz061 INTO l_fbz06,l_fbz061 FROM fbz_file WHERE fbz00 = '0'      #No.FUN-680028
   IF SQLCA.sqlcode THEN LET l_fbz06 = ' ' END IF
 
   LET l_sql = "SELECT fbb02,fbb08,faj23,faj24,faj53,faj531, ",     #No.FUN-680028   #MOD-6C0140 add fbb02
               "       fbb082,faj232,faj242 ",                      #FUN-AB0088 add
               "  FROM fbb_file LEFT OUTER JOIN faj_file ON fbb03=faj02 AND fbb031=faj022 ",
               " WHERE fbb01 = '",g_no,"'"
   PREPARE t106_p FROM l_sql
   DECLARE t106_c CURSOR WITH HOLD FOR t106_p
   FOREACH t106_c INTO l_fbb02,l_fbb08,l_faj23,l_faj24,l_faj53,l_faj531,    #No.FUN-680028   #MOD-6C0140 add l_fbb02
                       l_fbb082,l_faj232,l_faj242                           #FUN-AB0088 add
#No.FUN-710028 --begin                                                                                                              
      IF g_success='N' THEN                                                                                                         
         LET g_totsuccess='N'                                                                                                       
         LET g_success="Y"                                                                                                          
...
#No.FUN-710028 -begin
         IF g_bgerr THEN
            CALL s_errmsg('fbb01',g_no,'t106_c',SQLCA.sqlcode,0)  #No.FUN-710028
         ELSE
            CALL cl_err('t106_c',SQLCA.sqlcode,0)    
         END IF
#No.FUN-710028 --end
         EXIT FOREACH 
      END IF
      #-------Dr.: 資產之資產科目(faj53) (負值:fbz06)--------
      ...
      LET l_s = 1
      IF g_npq.npqtype <> '0' THEN LET l_fbb08 = l_fbb082 END IF    ##FUN-AB0088 add
      IF l_fbb08 > 0 THEN 
           LET g_npq.npq06 = '1'
           #No.FUN-680028 --begin
#          LET g_npq.npq03 = l_faj53   
      ...
      ELSE LET g_npq.npq06 = '1'  #調整成本為負
           #No.FUN-680028 --begin
#          LET g_npq.npq03 = l_fbz06
           IF g_npq.npqtype = '0' THEN
              LET g_npq.npq03 = l_fbz06
           ELSE
              LET g_npq.npq03 = l_fbz061
           END IF
           #No.FUN-680028 --end
           LET l_s= -1 
      END IF
       LET g_npq.npq07f= l_fbb08 * l_s #No.MOD-490624
      LET g_npq.npq07 = l_fbb08 * l_s
## No:2386 ---
      LET l_npq07_dr=l_npq07_dr+l_fbb08
      LET g_npq.npq23 = ' ' 
      MESSAGE '>',g_npq.npq02,' ',g_npq.npq03
      #NO.FUN-5C0015 ---start
     #CALL s_def_npq(g_npq.npq03,g_prog,g_npq.*,g_npq.npq01,'','')        #MOD-6C0140 mark
      CALL s_def_npq(g_npq.npq03,g_prog,g_npq.*,g_npq.npq01,l_fbb02,'',g_bookno)   #MOD-6C0140  #No.FUN-740033
      RETURNING g_npq.*
      #No.FUN-5C0015 ---end
      CALL s_def_npq31_npq34(g_npq.*,g_bookno)                  #FUN-AA0087                
...
#          LET g_npq.npq03 = l_faj53
      ...
      LET g_npq.npq07 =l_fbb08 * l_s  
## No:2386 ---
      LET l_npq07_cr=l_npq07_cr+l_fbb08
      LET g_npq.npq23 = ' '
      MESSAGE '>',g_npq.npq02,' ',g_npq.npq03
      #NO.FUN-5C0015 ---start
     #CALL s_def_npq(g_npq.npq03,g_prog,g_npq.*,g_npq.npq01,'','')        #MOD-6C0140 mark
...
## No:2386 modify 1998/07/27 ----------------------------
## 若所產生之分錄貸方金額=0 and 借方金額=0 then 不產生分錄
   IF l_npq07_dr=0 AND l_npq07_cr=0 THEN
      DELETE FROM npp_file WHERE nppsys = 'FA' AND npp00= 8
                             AND npp01 = g_no  
                             AND npp011 = 1
                             AND npptype = p_npptype     #No.FUN-680028
