...
#No.FUN-710083 --begin
   ...
   LET l_table = cl_prt_temptable('afag103',g_sql) CLIPPED
   IF l_table = -1 THEN
...
#No.FUN-710083 --end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'	# If background job sw is off
      ...
      ELSE CALL afag103()		# Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW g103_w AT p_row,p_col WITH FORM "afa/42f/afag103"
 ...
 END CONSTRUCT
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('afag103',g_time,l_cmd)      # Execute cmd at later time
         END IF
         CLOSE WINDOW g103_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         CALL cl_gre_drop_temptable(l_table)
...
#    CALL cl_outnam('afag103') RETURNING l_name
...
###GENGRE###     LET g_str = tm.wc,";",g_zz05
   ...
   # CALL cl_prt_cs3('afag103',l_sql,g_str)
...
###GENGRE###     LET l_sql_2 = "SELECT DISTINCT fau01 FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     ...
     END FOREACH
     ###FUN-940042 END ###
###GENGRE###     CALL cl_prt_cs3('afag103','afag103',l_sql,g_str)
    CALL afag103_grdata()    ###GENGRE###
...
FUNCTION afag103_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("afag103")
        IF handler IS NOT NULL THEN
            START REPORT afag103_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY fau01,fav02"
          
            DECLARE afag103_datacur1 CURSOR FROM l_sql
            FOREACH afag103_datacur1 INTO sr1.*
                OUTPUT TO REPORT afag103_rep(sr1.*)
            END FOREACH
            FINISH REPORT afag103_rep
        END IF
