...
#
# Pattern name...: s_chart_sale2_s4.4gl
# Descriptions...: 查看某一年度各營運中心的銷售業績
...
#                  p_area     銷售城區
#                  p_loc      圖表位置
# Usage..........: CALL s_chart_sale2_s4(p_year,p_month,p_itemclass,p_area,p_loc)
# Modify.........: 
...
FUNCTION s_chart_sale2_s4(p_year,p_month,p_itemclass,p_area,p_loc)
...
DEFINE l_sale       LIKE ogb_file.ogb14t #銷售金額
DEFINE l_sale1      LIKE ogb_file.ogb14t #去年銷售金額
DEFINE l_return     LIKE ogb_file.ogb14t #銷退金額
DEFINE l_return1     LIKE ogb_file.ogb14t #去年銷退金額
...
DEFINE l_chk_auth        STRING
   ...
   IF NOT s_chart_auth("s_chart_sale2_s4",g_user) THEN
   ...
   FOREACH s_chart_sale2_s3 INTO l_azw01 
      IF SQLCA.sqlcode THEN
         ...
         EXIT FOREACH
      END IF
      INSERT INTO sale_per_s4_temp VALUES (l_azw01)
      INITIALIZE l_azw01 TO NULL
   END FOREACH
   LET l_sql4 = "SELECT UNIQUE azw01 FROM sale_per_s4_temp"
   ...
   #抓取銷售金額
      ...
      LET l_sql = "SELECT SUM(a.ogb14t*oga24) FROM ",cl_get_target_table(l_azw01,'ogb_file')," a ",
   ...
   #抓取销退金额
      ...
      LET l_sql = "SELECT SUM(a.ohb14t*oha24)*(-1) FROM ",cl_get_target_table(l_azw01,'ohb_file')," a ",
      ...
      IF cl_null(l_azw08) THEN LET l_str = l_azw01 CLIPPED 
      ELSE
   ...
   END IF
   CALL cl_chart_create(p_loc,"s_chart_sale2_s4")  
