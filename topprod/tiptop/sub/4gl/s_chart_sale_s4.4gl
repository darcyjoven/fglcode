...
#
# Pattern name...: s_chart_sale_s4.4gl
...
#                  p_loc       圖表位置
#                  p_grup      部門編號
# Usage..........: CALL s_chart_sale_s4(p_year,p_month,p_itemclass,p_cusclass,p_salclass,p_loc,p_grup)
# Modify.........: 
...
FUNCTION s_chart_sale_s4(p_year,p_month,p_itemclass,p_cusclass,p_salclass,p_loc,p_grup)
...
DEFINE l_geo02      LIKE geo_file.geo02   #地區名稱
DEFINE l_geo_str    STRING                #存放地區信息
...
DEFINE l_gem02      LIKE gem_file.gem02 
   ...
   IF NOT s_chart_auth("s_chart_sale_s4",g_user) AND cl_null(p_grup) THEN 
   ...
   DROP TABLE sale_per_s4_temp
   CREATE TEMP TABLE sale_per_s4_temp(
   occ22  LIKE occ_file.occ22,
   sale   LIKE ogb_file.ogb14t)

   #抓取銷售金額
   INITIALIZE l_sale.* TO NULL
   LET l_sql = "SELECT e.occ22,SUM(a.ogb14t*oga24) ",
   ...
   LET l_sql = l_sql CLIPPED," GROUP BY e.occ22"
   ...
   FOREACH sel_sale_cs INTO l_sale.*
      ...
      INSERT INTO sale_per_s4_temp values(l_sale.occ22,l_sale.sale)
      INITIALIZE l_sale.* TO NULL
   END FOREACH

   #抓取销退金额
   INITIALIZE l_sale.* TO NULL
   LET l_sql = "SELECT e.occ22,SUM(a.ohb14t *oha24)*(-1) ",
   ...
   LET l_sql = "SELECT occ22,SUM(sale) FROM sale_per_s4_temp GROUP BY occ22 "
   ...
   FOREACH sel_sale_perfor_cs INTO l_sale.*
      ...
      END IF
      IF cl_null(l_sale.sale) THEN LET l_sale.sale = 0 END IF
      SELECT geo02 INTO l_geo02 FROM geo_file WHERE geo01 = l_sale.occ22
      IF NOT cl_null(l_geo02) THEN
         LET l_geo_str = l_sale.occ22 CLIPPED,"(",l_geo02 CLIPPED,")"
      ELSE
         LET l_geo_str = l_sale.occ22 CLIPPED
      END IF
      CALL cl_chart_array_data(p_loc,"dataset",l_geo_str,l_sale.sale)
   END FOREACH
   ...
   IF NOT cl_null( p_itemclass) THEN 
      ...
      LET l_substr = l_substr CLIPPED," / ",cl_getmsg('azz-167',g_lang),":",p_itemclass CLIPPED,"(",l_oba02,")" #產品分類說明
   END IF
   IF NOT cl_null( p_cusclass) THEN 
      SELECT oca02 INTO l_oca02 FROM oca_file WHERE oca01 = p_cusclass
      LET l_substr = l_substr CLIPPED," / ",cl_getmsg('azz-163',g_lang),":",p_cusclass CLIPPED,"(",l_oca02,")" #客戶分類說明
   END IF
   IF NOT cl_null( p_salclass ) THEN 
      ...
      END CASE
      LET l_substr = l_substr CLIPPED," / ",cl_getmsg('azz-165',g_lang),":",p_salclass CLIPPED,"(",l_oga08,")" #出貨類別說明
   END IF
   ...
   END IF
   IF NOT cl_null(p_grup) THEN
      CALL cl_chart_create(p_loc,"s_chart_sale3_s4")
   ELSE
      CALL cl_chart_create(p_loc,"s_chart_sale_s4")  
   END IF
