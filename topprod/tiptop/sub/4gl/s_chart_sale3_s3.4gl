...
#
# Pattern name...: s_chart_sale3_s3.4gl
...
#                  p_salcalss  出貨類別
#                  p_loc       圖表位置
# Usage..........: CALL s_chart_sale3_s3(p_year,p_month,p_itemclass,p_salarea,p_salclass,p_loc)
# Modify.........: 
...
FUNCTION s_chart_sale3_s3(p_year,p_month,p_itemclass,p_salarea,p_salclass,p_loc)
...
DEFINE l_chk_auth   STRING
   ...
   IF NOT s_chart_auth("s_chart_sale3_s3",g_user) THEN 
      LET l_chk_auth = cl_getmsg('azz1135',g_lang) CLIPPED
      CALL cl_chart_set_empty(p_loc, l_chk_auth)  
      RETURN 
   END IF                                #判斷權限

   #聯動至銷售業績4gl程式(s_chart_sale_s3)并傳參當前部門編號g_grup
   CALL s_chart_sale_s3(p_year,p_month,p_itemclass,p_salarea,p_salclass,p_loc,g_grup)
END FUNCTION
