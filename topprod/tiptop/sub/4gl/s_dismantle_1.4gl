...
FUNCTION s_dismantle_1(p_no,p_lineno,p_date,p_item,p_ware,p_loc,p_lot,p_unit_arr,p_imm01)  #No.FUN-610090
...
#  CALL s_lck_imgg()   #No.FUN-610090 Mark
   ...
   CALL s_pre_work_1(p_unit_arr) RETURNING tot_o
   ...
   #在此處檢查總量是否滿足這次的全部需求
   CALL s_chk_tot_qty_1(tot_o) RETURNING l_flag
   IF l_flag = '0' THEN
      #-----No.FUN-6C0083-----
      IF g_bgerr THEN
      ...
      ELSE                   #要拆
         ...
         IF l_i <> 1 THEN   #第一次拆,當然沒有零頭表羅
            ...
            FOREACH odd_cur INTO l_unit,l_qty  #零頭表的數量本身就是for ima25的
               ...
               IF tot >= l_reqqty THEN
                  CALL s_set_compute_1(1,l_reqqty-tot_o)
         ...
         FOREACH imgg_cur INTO l_imgg.*
            ...
            IF l_qty >= l_reqqty THEN   #這個單位就可以滿足需求了
               CALL s_set_compute_1(l_imgg.imgg21,l_reqqty)
                        RETURNING l_set,l_odd
               ...
               IF l_set <> 0 THEN
                  CALL s_ins_dis_table_1(l_i,'2',l_imgg.imgg09,l_imgg.imgg21,l_set)
                  IF g_success = 'N' THEN
                     RETURN p_imm01 #TQC-630079
                  END IF
               END IF
               ...
               IF l_odd <> 0 THEN
                  CALL s_ins_odd_table_1(l_imgg.imgg09,l_odd)
                  IF g_success = 'N' THEN
                     RETURN p_imm01 #TQC-630079
                  END IF
               END IF
            ...
            ELSE  #不夠拆
               ...
               IF g_sma.sma903[2,2] = 'Y' THEN
                  ...
                  IF tot > l_reqqty THEN   #夠拆
                     CALL s_set_compute_1(l_imgg.imgg21,l_reqqty-tot_o)
                          RETURNING l_set,l_odd
                     IF l_set <> 0 THEN
                        CALL s_ins_dis_table_1(l_i,'2',l_imgg.imgg09,
                                             l_imgg.imgg21,l_set)
                        IF g_success = 'N' THEN
                           RETURN p_imm01 #TQC-630079
                        END IF
   ...
   #其他調整單/異動明細檔等的處理
   IF g_success = 'Y' THEN
      CALL s_o_process_1(p_no,p_lineno,p_date) RETURNING g_imm01  #No.FUN-610090
      #-----No.FUN-610090-----
...
FUNCTION s_chk_dismantle_1(p_unit,p_reqty)
...
FUNCTION s_o_process_1(p_no,p_lineno,p_date)
   DEFINE p_no     LIKE inb_file.inb01
   DEFINE p_lineno LIKE inb_file.inb03
   DEFINE p_date   LIKE type_file.dat           #No.FUN-680147 DATE
   DEFINE l_flag   LIKE type_file.chr1          #No.FUN-680147 VARCHAR(01)
...
#NO.TQC-670045 end--
  ...
  FOREACH deduct_imgg INTO l_imgg.*
     ...
     IF l_imgg.stat = '1' THEN
        ...
        CALL s_ins_tlff_1(p_no,p_lineno,p_date,g_item,g_ware,
...
FUNCTION s_set_compute_1(p_fac,p_reqqty)
...
#                  p_flag   +1 撥入  -1 撥出
# Return.........: NULL
# Usage..........: CALL s_ins_tlff_1()
 
FUNCTION s_ins_tlff_1(p_no,p_lineno,p_date,p_item,p_ware,
...
#                  p_qty         數量
# Return.........: NULL
# Usage..........: CALL s_ins_dis_table_1(1,'0','PCS',1,21)
 
FUNCTION s_ins_dis_table_1(p_hierarchy,p_stat,p_unit,p_fac,p_qty)
...
#                  p_qty         數量
# Return.........: NULL
# Usage..........: CALL s_ins_odd_table_1('PCS',1)
 
FUNCTION s_ins_odd_table_1(p_unit,p_qty)
   DEFINE p_unit    LIKE ima_file.ima25
   DEFINE p_qty     LIKE img_file.img10
...
#                  0.不滿足,勿須拆箱
# Usage..........: CALL s_chk_tot_qty_1(p_reqqty)
#                  RETURNING 0  不要拆箱了.總庫存都不滿足
 
FUNCTION s_chk_tot_qty_1(p_reqqty)
   DEFINE p_reqqty LIKE img_file.img10
...
FUNCTION s_hold_qty_1()
  ...
  FOR l_i = 1 TO g_unit_arr.getLength()
     #檢查是否要拆箱
     CALL s_chk_dismantle_1(g_unit_arr[l_i].unit,g_unit_arr[l_i].qty)
                RETURNING l_do
     IF l_do = '0' THEN   #不用拆箱,自身數量就夠了
        CALL s_ins_dis_table_1(l_i,'0',g_unit_arr[l_i].unit,
     ...
     ELSE                 #要拆
       ...
       #IF l_imgg10 <> 0 THEN
           CALL s_ins_dis_table_1(l_i,'1',g_unit_arr[l_i].unit,
...
# Input parameter: NULL
# Return.........: NULL
# Usage..........: CALL s_pre_work_1(p_unit_arr)
 
FUNCTION s_pre_work_1(p_unit_arr)
   DEFINE p_unit_arr DYNAMIC ARRAY OF RECORD
                        unit   LIKE ima_file.ima25,
                        fac    LIKE img_file.img21,
                        qty    LIKE img_file.img10
...
FUNCTION s_undo_dismantle_1(p_no,p_lineno)
   DEFINE p_no     LIKE inb_file.inb01
   DEFINE p_lineno LIKE inb_file.inb03
   DEFINE l_flag   LIKE type_file.chr1          #No.FUN-680147 VARCHAR(01)
   DEFINE l_unit   LIKE ima_file.ima25
...
# Input parameter: NULL
# Return.........: NULL
# Usage..........: CALL s_aft_work_1()
 
FUNCTION s_aft_work_1()
...
# Input parameter: NULL
# Return.........: NULL
# Usage..........: CALL s_cre_tmp_tab_1()
 
FUNCTION s_cre_tmp_tab_1()
  #用于拆箱的臨時表
