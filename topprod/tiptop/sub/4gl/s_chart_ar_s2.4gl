...
#
# Pattern name...: s_chart_ar_s2.4gl
# Descriptions...: 內銷/外銷應收帳款比例
...
#                  p_empno      收款人員
#                  p_custno     收款客戶
# Usage..........: CALL s_chart_ar_s2(p_loc)
# Modify.........: 
...
FUNCTION s_chart_ar_s2(p_ym, p_tranclass, p_deptno, p_empno, p_custno,p_loc)
DEFINE p_loc                  LIKE type_file.chr10
DEFINE ar_s2                  LIKE oma_file.oma56t #帳款金額
...
DEFINE l_chk_auth             STRING
   ...
   END IF
   LET l_sql = l_sql CLIPPED," GROUP BY oma08 "
   PREPARE sel_oma_pre FROM l_sql
   DECLARE sel_oma_cs CURSOR FOR sel_oma_pre
   FOREACH sel_oma_cs INTO l_oma08,ar_s2
      ...
      END IF
   #內銷/外銷應收帳款比例
   SELECT oma08 INTO l_oma08 FROM oma_file WHERE oma01 = l_oma08
 #  LET l_str1 = l_oma08 CLIPPED,"(",l_oma08='1' CLIPPED,")" ,"(",l_oma08='2' CLIPPED,")","(",l_oma08='3' CLIPPED,")" #1.內銷 2.外銷 3.視同外銷 金額
   CASE l_oma08
         WHEN '1'
            LET l_str1 = l_oma08 CLIPPED,"(",cl_getmsg('aws-023',g_lang),")"
         WHEN '2'
            LET l_str1 = l_oma08 CLIPPED,"(",cl_getmsg('aws-024',g_lang),")"
         WHEN '3'
            LET l_str1 = l_oma08 CLIPPED,"(",cl_getmsg('aws-025',g_lang),")"
      END CASE
   CALL cl_chart_array_data(p_loc,"dataset",l_str1,ar_s2)
   INITIALIZE l_oma08,ar_s2 TO NULL
   END FOREACH
   ...
   END IF
   CALL cl_chart_create(p_loc,"s_chart_ar_s2")
