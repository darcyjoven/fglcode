...
#
# Pattern name...: s_chart_sale_s3.4gl
...
#                  p_loc       圖表位置
#                  p_grup      部門編號
# Usage..........: CALL s_chart_sale_s3(p_year,p_month,p_itemclass,p_salarea,p_salclass,p_loc,p_grup)
# Modify.........: 
...
FUNCTION s_chart_sale_s3(p_year,p_month,p_itemclass,p_salarea,p_salclass,p_loc,p_grup)
...
DEFINE l_oca02      LIKE oca_file.oca02  #客戶分類名稱
DEFINE l_oca_str    STRING               #存放客戶分類信息
...
DEFINE l_gem02      LIKE gem_file.gem02
   ...
   IF NOT s_chart_auth("s_chart_sale_s3",g_user) AND cl_null(p_grup) THEN 
   ...
   LET l_sql = "SELECT d.occ03,SUM(a.ogb14t *oga24) ",
   ...
   LET l_sql = l_sql CLIPPED," GROUP BY d.occ03"
   ...
   FOREACH sel_sale_cs INTO l_sale.*
      ...
      INSERT INTO sale_per_s3_temp values(l_sale.occ03,l_sale.sale)
      INITIALIZE l_sale.* TO NULL
   END FOREACH

   #抓取销退金额
   INITIALIZE l_sale.* TO NULL
   LET l_sql = "SELECT d.occ03,SUM(a.ohb14t *oha24)*(-1) ",
   ...
   LET l_sql = "SELECT occ03,SUM(sale) FROM sale_per_s3_temp GROUP BY occ03 "
   ...
   FOREACH sel_sale_perfor_cs INTO l_sale.*
      ...
      IF NOT cl_null(l_oca02) THEN
         LET l_oca_str = l_sale.occ03 CLIPPED,"(",l_oca02 CLIPPED,")"
      ELSE
         LET l_oca_str = l_sale.occ03 CLIPPED
      END IF
      CALL cl_chart_array_data(p_loc,"dataset",l_oca_str,l_sale.sale)
   END FOREACH
   ...
   END IF
   IF NOT cl_null(p_grup) THEN
      CALL cl_chart_create(p_loc,"s_chart_sale3_s3")
   ELSE
      CALL cl_chart_create(p_loc,"s_chart_sale_s3")  
   END IF
