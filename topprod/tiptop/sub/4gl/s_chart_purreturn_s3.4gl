...
#
# Pattern name...: s_chart_purreturn_s3.4gl
...
#                  p_vendorno  廠商名稱
#                  p_grup      部門編號
# Usage..........: CALL s_chart_purreturn_s3(p_year,p_month,p_rtnreason,p_vendclass,p_vendorno,p_loc,p_grup)
# Modify.........: 
...
FUNCTION s_chart_purreturn_s3(p_year,p_month,p_rtnreason,p_vendclass,p_vendorno,p_loc,p_grup)
...
DEFINE l_gem02                 LIKE gem_file.gem02
   ...
   IF NOT s_chart_auth("s_chart_purreturn_s3",g_user) AND cl_null(p_grup) THEN 
      LET l_chk_auth = cl_getmsg('azz1135',g_lang) CLIPPED
      CALL cl_chart_set_empty(p_loc, l_chk_auth)  
      RETURN 
   END IF                                            #判斷權限
   #抓取資料
   LET l_sql = "SELECT ima131,SUM(apb10) ", 
               "  FROM rvu_file ", 
   ...
   FOREACH sel_rvv_cs INTO l_ima131,purreturn_sum
      ...
      LET l_str1 = l_ima131 CLIPPED,"(",l_oba02 CLIPPED,")" #產品分類(分類說明)
      CALL cl_chart_array_data(p_loc,"dataset",l_str1,purreturn_sum) #
      INITIALIZE l_ima131,purreturn_sum TO NULL
   END FOREACH
   ...
   IF NOT cl_null( p_vendorno ) THEN                      #廠商名稱
      SELECT pmc03 INTO l_pmc03 FROM pmc_file WHERE pmc01 = p_vendorno
      LET l_substr = l_substr CLIPPED,"/",cl_getmsg('sub-103',g_lang),":",p_vendorno CLIPPED,"(",l_pmc03,")"
   END IF
   ...
   END IF
   IF NOT cl_null(p_grup) THEN
      CALL cl_chart_create(p_loc,"s_chart_purreturn2_s3")
   ELSE
      CALL cl_chart_create(p_loc,"s_chart_purreturn_s3")
   END IF
