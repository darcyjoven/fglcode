...
# Modify.........: No.FUN-7B0055 07/11/14 By Carrier s_g_np是by非多帳期的角度來取得未衝金額,故取消apc/omc的內容
...
FUNCTION s_g_np(p_sw,p_sw2,p_no,p_seq)
...
#  SELECT azi04 INTO g_azi04 FROM azi_file WHERE azi01 = g_aza.aza17   #No.CHI-6A0004
  ...
  PREPARE s_g_np_p1 FROM l_sql
  IF STATUS THEN 
# CALL cl_err('s_g_np_p1',STATUS,1)
#No.FUN-720003--begin                                                                                                               
      IF g_bgerr THEN                                                                                                               
         LET g_showmsg = l_oox00,"/",p_no,"/",p_seq      
         CALL s_errmsg('oox00,oox03,oox04',g_showmsg,'s_g_np_p1',STATUS,1)                                                                     
      ELSE                                                                                                                          
         CALL cl_err('s_g_np_p1',STATUS,1)                                                                             
      END IF                                                                                                                        
#No.FUN-720003--end 
  RETURN 0 END IF
  DECLARE s_g_np_c1 CURSOR FOR s_g_np_p1
 
  OPEN s_g_np_c1 
  IF STATUS THEN 
# CALL cl_err('s_g_np_c1',STATUS,1) 
#No.FUN-720003--begin                                                                                                               
      IF g_bgerr THEN                                                                                                               
         CALL s_errmsg('','','s_g_np_c1',STATUS,1)                                                                                  
      ELSE                                                                                                                          
         CALL cl_err('s_g_np_c1',STATUS,1)                                                                                          
      END IF                                                                                                                        
#No.FUN-720003--end           
  RETURN 0 END IF
  FETCH s_g_np_c1 INTO l_oox10
