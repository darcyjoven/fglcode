...
# Usage..........: CALL s_cralc(p_wo,p_wotype,p_part,p_btflg,p_woq,p_date,p_mps,
...
# Modify.........: No:MOD-A40136 10/04/22 By Sarah cralc_sou()在判斷庫存量夠不夠時,忘了乘單位換算率
...
# Modify.........: No:MOD-A50131 10/05/24 By liuxqa cralc_sou()在計算應發數量時，沒有考慮最小發料量和最小發料倍數.
...
FUNCTION s_cralc(p_wo,p_wotype,p_part,p_btflg,p_woq,p_date,p_mps,p_yld,
...
DEFINE
   ...
   DEFINE l_n        LIKE type_file.num5       #NO.TQC-A80180
    ...
    ELSE
        #CALL cralc_bom(0,p_part,p_altno,p_woq,1) #FUN-550112  #由BOM產生備料檔  #No.TQC-610003
        IF g_sma.sma542 = 'Y' AND g_sfb.sfb93 = 'Y' THEN
           ...
           FOREACH ecm_cur INTO l_ecm03_par,l_ecm11,l_ecm03,l_ecm012
                   CALL cralc_brb_bom(0,p_part,p_altno,p_woq,1,p_woq,l_ecm03_par,l_ecm11,l_ecm03,l_ecm012)
           END FOREACH
           #NO.TQC-A80180--begin
           SELECT COUNT(*) INTO l_n FROM sfa_file WHERE sfa01=g_sfb.sfb01
           IF l_n=0 THEN  
              CALL cralc_bom(0,p_part,p_altno,p_woq,1)
           END IF     
           #NO.TQC-A80180--end
        ELSE
        #FUN-A50066--end--add--------------------------
...
#   2. 工單型態為再加工工單(5) 及拆件式工單(11)者, 其備料的料件為再加工
#      / 拆件料件本身的料件, 其產品結構不展開, 損耗率不計算
FUNCTION cralc_rd(p_part,p_woq)
...
#MOD-990002 --begin-- 
    ...
    IF l_cnt > 0 THEN 
       ...
       IF s_industry('icd') THEN                
          CALL cralc_upd_sfaa(g_sfa.*,g_sfa.sfa01,g_sfa.sfa03,g_sfa.sfa08,g_sfa.sfa12,
...
#MOD-990002 --end--      
    ...
    ELSE
       IF NOT s_industry('std') THEN
          ...
          IF l_flag THEN 
             IF s_industry('icd') THEN
                IF NOT cralc_ins_sfaa(g_sfa.*,l_sfai.*) THEN
...
FUNCTION cralc_bom(p_level,p_key,p_key2,p_total,p_QPA)  #FUN-550112
DEFINE
...
DEFINE g_tc_ecm RECORD LIKE tc_ecm_file.*  #add by huanglf161031
DEFINE l_num       LIKE type_file.num5   #add by huanglf161031
DEFINE l_num1      LIKE type_file.num5   #add by huanglf161031
DEFINE l_sql       STRING                #add by huanglf161031
...
#            LET l_total=sr[l_i].bmb06*p_total*((100+sr[l_i].bmb08))/100
           ...
           LET l_bmb06=sr[l_i].bmb06/sr[l_i].bmb07
           CALL cralc_rate(p_key,sr[l_i].bmb03,p_total,sr[l_i].bmb081,sr[l_i].bmb08,sr[l_i].bmb082,l_bmb06,p_QPA) 
...
## add for top40:(bmb19):'1.不展開 2.不展開但自動開立工單 3.展開'
                   CASE
                      ...
                      WHEN sr[l_i].bmb19='3'     #展開
                          ...
                          #No.FUN-A60031--end
                          #CALL cralc_bom(p_level,sr[l_i].bmb03,' ',   #FUN-550112
                          ...
                          #CHI-8A0002-begin-add
                           ...
                           ELSE 
                             #CHI-8A0002-end-add
                             # CALL cralc_bom(p_level,sr[l_i].bmb03,' ',  #FUN-8B0015
                             # l_total,l_ActualQPA)
                           ...
                           IF g_bmb19 IS NOT NULL AND g_bmb19 != '  ' THEN
                              ...
                              IF g_bmb19 = 'Y' THEN ##-展下階
                                 ...
                                 ELSE
                                    ...
                                    CALL cralc_bom(p_level,sr[l_i].bmb03,l_ima910[l_i],l_total,l_QPA)
                                    LET fs_insert = 'N'
                                 END IF
                              ELSE
                                 LET fs_insert = 'Y'
...
#       原來庫存不足才找取代, 現在改為只要有取代, 先備取代料, 再備新料
        CASE
         WHEN g_sfa.sfa26='1' AND g_sma.sma66='Y'	## UTE:先備舊料,再備新料
              ...
              LET l_unaloc=0
              LET l_s=0                             	##取替代是否成功#
              CALL cralc_sou(g_sfa.sfa03,g_sfa.sfa26,
...
FUNCTION cralc_oeo(p_part,p_woq)
...
FUNCTION cralc_brb_bom(p_level,p_key,p_key2,p_total,p_QPA,p_total2,p_ecm03_par,p_ecm11,p_ecm03,p_ecm012)  
DEFINE
    ...
    WHILE TRUE
        ...
        FOR l_i = 1 TO l_x
           ...
           #No.FUN-870117 --begin--                                                                                                
            ...
            CALL cralc_rate(p_key,sr[l_i].brb03,p_total,sr[l_i].brb081,sr[l_i].brb08,sr[l_i].brb082,l_brb06,p_QPA) 
            RETURNING l_total,l_QPA,l_ActualQPA
            LET l_total2=l_total
            LET l_sfa11='N'
            IF sr[l_i].ima08='R' THEN #routable part
...
FUNCTION cralc_rate(p_bmb01,p_bmb03,p_total,p_bmb081,p_bmb08,p_bmb082,p_bmb06,p_QPA)
...
#返回值 备料量，标准QPA,实际QPA
FUNCTION cralc_eck_rate(p_sfb01,p_ecm03,p_ecm012,p_total,p_bmb081,p_bmb08,p_bmb082,p_bmb06,p_QPA)
DEFINE  l_n         LIKE type_file.num5,
        sr          DYNAMIC ARRAY OF RECORD 
...
#FUN-C30274 --START--
FUNCTION cralc_ins_sfaa(l_sfa,l_sfai)
DEFINE l_sfa    RECORD LIKE sfa_file.*
DEFINE l_sfai   RECORD LIKE sfai_file.*
...
FUNCTION cralc_upd_sfaa(l_sfa,l_sfa01,l_sfa03,l_sfa08,l_sfa12,l_sfa27,l_sfa012,l_sfa013)
DEFINE l_sfa    RECORD LIKE sfa_file.*
DEFINE l_sfai   RECORD LIKE sfai_file.*
DEFINE l_sfaa   RECORD LIKE sfaa_file.*
