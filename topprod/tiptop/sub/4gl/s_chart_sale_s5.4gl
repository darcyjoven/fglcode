...
#
# Pattern name...: s_chart_sale_s5.4gl
...
#                  p_loc       圖表位置
#                  p_grup      部門編號
# Usage..........: CALL s_chart_sale_s5(p_year,p_month,p_itemclass,p_cusclass,p_salarea,p_loc,p_grup)
# Modify.........: 
...
FUNCTION s_chart_sale_s5(p_year,p_month,p_itemclass,p_cusclass,p_salarea,p_loc,p_grup)
...
DEFINE l_gem02      LIKE gem_file.gem02 
   ...
   IF NOT s_chart_auth("s_chart_sale_s5",g_user) AND cl_null(p_grup) THEN 
   ...
   LET l_sql = "SELECT b.oga08,SUM(a.ogb14t*oga24) ",
   ...
   LET l_sql = l_sql CLIPPED," GROUP BY b.oga08"
   ...
   FOREACH sel_sale_cs INTO l_sale.*
      ...
      INSERT INTO sale_per_s5_temp values(l_sale.oga08,l_sale.sale)
      INITIALIZE l_sale.* TO NULL
   END FOREACH

   #抓取销退金额
   INITIALIZE l_sale.* TO NULL
   LET l_sql = "SELECT b.oha08,SUM(a.ohb14t *oha24)*(-1) ",
   ...
   LET l_sql = l_sql CLIPPED," GROUP BY b.oha08"
   ...
   LET l_sql = "SELECT oga08,SUM(sale) FROM sale_per_s5_temp GROUP BY oga08 "
   ...
   FOREACH sel_sale_perfor_cs INTO l_sale.*
      ...
      CASE l_sale.oga08
         WHEN '1'
            LET l_oga_str = l_sale.oga08 CLIPPED,"(",cl_getmsg('aws-023',g_lang),")"
         WHEN '2'
            LET l_oga_str = l_sale.oga08 CLIPPED,"(",cl_getmsg('aws-024',g_lang),")"
         WHEN '3'
            LET l_oga_str = l_sale.oga08 CLIPPED,"(",cl_getmsg('aws-025',g_lang),")"
      END CASE
      CALL cl_chart_array_data(p_loc,"dataset",l_oga_str,l_sale.sale)
   END FOREACH
   ...
   IF NOT cl_null( p_salarea ) THEN 
      ...
      LET l_substr = l_substr CLIPPED," / ",cl_getmsg('azz-164',g_lang),":",p_salarea CLIPPED,"(",l_geo02,")" #銷售地區說明
   END IF
   ...
   END IF
   IF NOT cl_null(p_grup) THEN
      CALL cl_chart_create(p_loc,"s_chart_sale3_s5")
   ELSE
      CALL cl_chart_create(p_loc,"s_chart_sale_s5")  
   END IF
