...
#
# Pattern name...: s_chart_salereturn_s3.4gl
# Descriptions...: 各產品分類銷退余額
...
#                  p_loc      圖表位置
#                  p_grup     部門編號
# Usage..........: CALL s_chart_salereturn_s3(p_year,p_month,p_rtnreason,p_cusclass,p_custno,p_loc,p_grup)
# Modify.........: 
...
FUNCTION s_chart_salereturn_s3(p_year,p_month,p_rtnreason,p_cusclass,p_custno,p_loc,p_grup)
...
DEFINE l_sale       RECORD               #產品分類
             ima131 LIKE ima_file.ima131,
             sale   LIKE ogb_file.ogb14t
                    END RECORD
DEFINE l_oca02      LIKE oca_file.oca02  #客戶分類
DEFINE l_oga08      STRING
DEFINE l_oak02      LIKE oak_file.oak02  #銷退原因說明
DEFINE l_oba_str    STRING               #分類說明
...
DEFINE l_gem02      LIKE gem_file.gem02
   ...
   IF NOT s_chart_auth("s_chart_salereturn_s3",g_user) AND cl_null(p_grup) THEN
   ...
   LET l_sale.sale=0
   LET l_sql = "SELECT c.ima131,SUM(a.ogb14t*oga24)*(-1) ",
   ...
   FOREACH sel_sale_cs INTO l_sale.*
      ...
      INSERT INTO sale_per_s3_temp values(l_sale.ima131,l_sale.sale)
      INITIALIZE l_sale.ima131 TO NULL
      LET l_sale.sale=0
   END FOREACH
   ...
   INITIALIZE l_sale.ima131 TO NULL
   LET l_sale.sale=0
   LET l_sql = "SELECT c.ima131,SUM(a.ohb14t*oha24) ",
   ...
   LET l_sql = "SELECT ima131,SUM(sale) FROM sale_per_s3_temp GROUP BY ima131 "
   ...
   END IF
   IF NOT cl_null(p_grup) THEN
      CALL cl_chart_create(p_loc,"s_chart_salereturn2_s3")
   ELSE
      CALL cl_chart_create(p_loc,"s_chart_salereturn_s3")  
   END IF
