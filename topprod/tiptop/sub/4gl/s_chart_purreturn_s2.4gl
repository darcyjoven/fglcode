...
#
# Pattern name...: s_chart_purreturn_s2.4gl
...
#                  p_vendorno  廠商名稱
#                  p_grup      部門編號
# Usage..........: CALL s_chart_purreturn_s2(p_year,p_month,p_itemclass,p_vendclass,p_vendorno,p_loc,p_grup)
# Modify.........: 
...
FUNCTION s_chart_purreturn_s2(p_year,p_month,p_itemclass,p_vendclass,p_vendorno,p_loc,p_grup)
...
DEFINE l_rvv26                 LIKE rvv_file.rvv26  #退貨原因
DEFINE l_oba02                 LIKE oba_file.oba02
DEFINE l_rvu05                 LIKE rvu_file.rvu05  #廠商名稱
DEFINE l_pmc02                 LIKE pmc_file.pmc02  #廠商分類
DEFINE l_azf03                 LIKE azf_file.azf03  #原因說明
...
DEFINE l_gem02                 LIKE gem_file.gem02
   ...
   IF NOT s_chart_auth("s_chart_purreturn_s2",g_user) AND cl_null(p_grup) THEN 
      LET l_chk_auth = cl_getmsg('azz1135',g_lang) CLIPPED
      CALL cl_chart_set_empty(p_loc, l_chk_auth)  
      RETURN 
   END IF                                            #判斷權限
   #抓取資料
   LET l_sql = "SELECT rvv26,SUM(apb10) ",
               "  FROM rvu_file", 
   ...
   END IF
   IF NOT cl_null(p_vendorno) THEN 
      LET l_sql = l_sql CLIPPED, " AND rvu04 = '",p_vendorno CLIPPED,"'"
   END IF
   ...
   LET l_sql = l_sql CLIPPED," GROUP BY rvv26"
   PREPARE sel_rvv_pre FROM l_sql
   DECLARE sel_rvv_cs CURSOR FOR sel_rvv_pre
   FOREACH sel_rvv_cs INTO l_rvv01,purreturn_sum
      IF SQLCA.sqlcode THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
      IF cl_null(purreturn_sum) THEN LET purreturn_sum = 0 END IF
      SELECT azf03 INTO l_azf03 FROM azf_file WHERE azf01 = l_rvv01 AND azf02 ='2'
      LET l_str1 = l_rvv01 CLIPPED,"(",l_azf03 CLIPPED,")" #退貨原因(原因說明)
      CALL cl_chart_array_data(p_loc,"dataset",l_str1,purreturn_sum) #
      INITIALIZE l_rvv01,purreturn_sum TO NULL
   END FOREACH
   ...
   END IF 
   IF NOT cl_null( p_vendorno ) THEN                      #廠商名稱
      SELECT rvu05 INTO l_rvu05 FROM rvu_file WHERE rvu04 = p_vendorno
      LET l_substr = l_substr CLIPPED, " / ",cl_getmsg('sub-103',g_lang),":",p_vendorno  CLIPPED,"(",l_rvu05,")"
   END IF
   ...
   END IF
   IF NOT cl_null(p_grup) THEN
      CALL cl_chart_create(p_loc,"s_chart_purreturn2_s2")
   ELSE
      CALL cl_chart_create(p_loc,"s_chart_purreturn_s2")
   END IF
