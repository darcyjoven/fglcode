...
FUNCTION s_runcard(p_runcard,p_wono,p_primary,p_part,p_woq) 
DEFINE
    ...
    IF (l_sfb93='Y' AND g_sma.sma28='2')	        #使用製程排程
        ...
        LET g_cnt=0
       SELECT COUNT(*) INTO g_cnt FROM sgm_file WHERE sgm01=g_runcard
        IF SQLCA.sqlcode OR g_cnt=0 OR g_cnt IS NULL THEN #record not exist
             #--> 產生製程追蹤檔
                ...
                CALL s_put2(l_ima571) RETURNING g_cnt 
 
                IF g_cnt = 0 THEN 
                   CALL s_put2(g_part) RETURNING g_cnt 
                END IF 
        END IF
    END IF
    MESSAGE ""
...
#FUN-B10056 ---------------------Begin----------------------
#檢查製程段是否存在
FUNCTION s_runcard_sgm012(p_sgm01,p_sgm012)
...
END FUNCTION
#抓取制程段号说明
FUNCTION s_runcard_sgm014(p_sgm01,p_sgm012)
   DEFINE p_sgm01     LIKE sgm_file.sgm01
   DEFINE p_sgm012    LIKE sgm_file.sgm012
