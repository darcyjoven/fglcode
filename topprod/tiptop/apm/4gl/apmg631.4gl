...
# Modify.........: No.MOD-580395 05/09/08 By echo apmr631,apmr632列印時串apmg631
...
# Modify.........: No.TQC-740294 07/06/04 By Sarah apmg631/apmr631/apmr632共用程式,630以外的報表出表有問題
...
# Modify.........: NO.TQC-960231 09/06/19 By lilingyu apmg631畫面rvu01字段名稱顯示錯誤
...
# Modify.........: No.FUN-C70081 12/07/20 By xjll   添加服飾流通業款式款式明細選項
...
END GLOBALS
  ...
  DEFINE tm  RECORD                                                           # Print condition RECORD
             wc        STRING,   #No.FUN-680136 VARCHAR(600)     # Where condition#FUN-C70081--STRING---
             a         LIKE type_file.chr1,      #No.FUN-680136 VARCHAR(1)       # 選擇(1)已列印 (2)未列印 (3)全部
             b         LIKE type_file.chr1,      #No.FUN-860026 
             c         LIKE type_file.chr1,      #No.FUN-C70081
  ...
  DEFINE l_table2     STRING     #FUN-C70081---
  DEFINE l_table3     STRING     #FUN-C70081---
...
MAIN
   ...
   LET tm.c = ARG_VAL(16)        #FUN-C70081---ADD---
   ...
   LET l_table = cl_prt_temptable('apmg631',g_sql) CLIPPED
   IF l_table = -1 THEN 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time         #FUN-B40087
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)     #FUN-B40087 #FUN-C70081--ADD L_TABLE2,L_TABLE3
      EXIT PROGRAM END IF
   LET g_sql = "rvbs01.rvbs_file.rvbs01,",                                                                                          
               ...
               "rvv17.rvv_file.rvv17"  
   LET l_table1 = cl_prt_temptable('apmg6311',g_sql) CLIPPED                                                                        
   IF  l_table1 = -1 THEN 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time         #FUN-B40087
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)     #FUN-B40087 #FUN-C70081--ADD L_TABLE2,L_TABLE3
       EXIT PROGRAM 
...
#FUN-C70081-----ADD----STR-----
   ...
   LET l_table2 = cl_prt_temptable('apmg6312',g_sql) CLIPPED 
   IF l_table2 = -1 THEN 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time         #FUN-B40087
   ...
   LET l_table3 = cl_prt_temptable('apmg6313',g_sql) CLIPPED 
   IF l_table3 = -1 THEN 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time         #FUN-B40087
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)
      EXIT PROGRAM 
...
#FUN-C70081-----ADD----END-----   
   IF cl_null(g_bgjob) OR g_bgjob = 'N'   # If background job sw is off
      THEN CALL g631_tm(0,0)        # Input print condition
   ELSE 
#FUN-C70081---add---begin---------
      ...
      END IF  
#FUN-C70081---add---end-----------
      CALL g631()              # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
   CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)  #FUN-C70081--ADD L_TABLE2,L_TABLE3
END MAIN
 
FUNCTION g631_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
  ...
  DEFINE l_msg1         STRING                      #TQC-960354 
   ...
   OPEN WINDOW g631_w AT p_row,p_col WITH FORM "apm/42f/apmg631"
   ATTRIBUTE (STYLE = g_win_style CLIPPED)
 
   #2004/06/02共用程式時呼叫
   CALL cl_set_locale_frm_name("apmg631")
   CALL cl_ui_init()
   #FUN-C70081------ADD---STR---
   IF s_industry('slk') AND g_azw.azw04='2' THEN
   ...
   IF s_industry('slk') AND g_azw.azw04='2' THEN
      LET tm.c = 'Y'   #FUN-C70081---ADD---
   ELSE
      LET tm.c = 'N'   #FUN-C70081---ADD---
   END IF
   LET tm.more = 'N'
   LET g_pdate = g_today
   LET g_rlang = g_lang
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW g631_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3) #FUN-C70081 ADD
      EXIT PROGRAM
         
   END IF
   INPUT BY NAME tm.a,tm.b,tm.c,tm.more WITHOUT DEFAULTS   #No.FUN-860026 add tm.b#FUN-C70081 ADD TM.C
         BEFORE INPUT
      ...
      AFTER FIELD more
         IF tm.more = 'Y' THEN
            ...
            LET g_prog='apmg631'     #MOD-760106 add
   ...
   IF g_bgjob = 'Y' THEN
      ...
      IF SQLCA.sqlcode OR l_cmd IS NULL THEN
         CALL cl_err('apmg631','9031',1)   #No.FUN-660129
      ELSE
         ...
         CALL cl_cmdat('apmg631',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW g631_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3) #FUN-C70081--ADD--L_TABLE2,L_TABLE3
...
#FUN-C70081-----ADD-----END-----     
     ...
     CALL cl_del_data(l_table2)   #FUN-C70081---ADD--
     CALL cl_del_data(l_table3)   #FUN-C70081---ADD---
...
#FUN-C70081-----ADD----END-------     
     ...
     PREPARE g631_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE g631_curs1 CURSOR FOR g631_prepare1
     ...
     FOREACH g631_curs1 INTO sr.*,l_ima021,l_ima906,l_img09     #FUN-C50003 add l_ima021,l_ima906,l_img09
          IF SQLCA.sqlcode != 0 THEN
...
#FUN-C70051-----ADD----STR----
      ...
      IF l_ima151='Y' AND tm.c='Y' AND sr.rvv17>0 THEN
         LET l_sql = "SELECT DISTINCT(imx02),agd04 FROM imx_file,agd_file",
               ...
               " ORDER BY agd04"
           PREPARE g631_sr2_pre FROM l_sql
           DECLARE g631_sr2_cs CURSOR FOR g631_sr2_pre 
           LET l_imx02 = NULL 
           ...
           END FOR
           LET l_i = 1
           FOREACH g631_sr2_cs INTO l_imx02,l_agd04
              LET l_imx[l_i].imx01=' '  
...
#子報表2
           LET l_sql = "SELECT DISTINCT(imx01),agd04 FROM imx_file,agd_file",
               ...
               " ORDER BY agd04"
           PREPARE g631_colslk_pre FROM l_sql
           DECLARE g631_colslk_cs CURSOR FOR g631_colslk_pre
           LET l_imx01 = NULL

           FOREACH g631_colslk_cs INTO l_imx01,l_agd04
              ...
              LET l_sql2 = "SELECT DISTINCT(imx02),agd04 FROM imx_file,agd_file",
                   ...
                   " (SELECT ima941 FROM ima_file WHERE ima01='",sr.rvv31,"')",
                   " ORDER BY agd04"
              PREPARE g631_sr3_pre FROM l_sql2
              DECLARE g631_sr3_cs CURSOR FOR g631_sr3_pre
              LET l_imx02 = NULL
              LET l_i = 1
              FOR l_i = 1 TO 15
              ...
              END FOR
              LET l_i = 1
              FOREACH g631_sr3_cs INTO l_imx02,l_agd04
                 LET l_imx[l_i].imx01=' '
                 SELECT agd03 INTO l_imx[l_i].imx01  FROM agd_file,ima_file
                  WHERE agd01 = ima941 AND agd02 = l_imx02
                    AND ima01 = sr.rvv31
...
###GENGRE###     CALL cl_prt_cs3('apmg631','apmg631',l_sql,g_str)
#FUN-C70081-----ADD----STR---
     IF s_industry('slk') AND g_azw.azw04='2' AND tm.c='Y' THEN
        LET g_template="apmg631_slk"
        CALL apmg631_slk_grdata()
     ELSE 
        LET g_template="apmg631"  
#FUN-C70081-----ADD----END---
        CALL apmg631_grdata()    ###GENGRE###
     END IF                    #FUN-C70081-----ADD
...
FUNCTION apmg631_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("apmg631")
        IF handler IS NOT NULL THEN
            START REPORT apmg631_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY rvu01,rvv02"
            DECLARE apmg631_datacur1 CURSOR FROM l_sql
            FOREACH apmg631_datacur1 INTO sr1.*
                OUTPUT TO REPORT apmg631_rep(sr1.*)
            END FOREACH
            FINISH REPORT apmg631_rep
        END IF
...
REPORT apmg631_rep(sr1)
    ...
    FORMAT
        ...
        AFTER GROUP OF sr1.rvu01
            ...
            START REPORT apmg631_subrep01
            DECLARE apmg631_repcur1 CURSOR FROM l_sql
            FOREACH apmg631_repcur1 INTO sr2.*
                OUTPUT TO REPORT apmg631_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT apmg631_subrep01
            #FUN-B40087---------add------end-----
...
FUNCTION apmg631_slk_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("apmg631")
        IF handler IS NOT NULL THEN
            START REPORT apmg631_slk_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY rvu01,rvv02"
            DECLARE apmg631_slk_datacur1 CURSOR FROM l_sql
            FOREACH apmg631_slk_datacur1 INTO sr1.*
                OUTPUT TO REPORT apmg631_slk_rep(sr1.*)
            END FOREACH
            FINISH REPORT apmg631_slk_rep
        END IF
        IF INT_FLAG = TRUE THEN
            LET INT_FLAG = FALSE
            EXIT WHILE
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            LET l_sql = "SELECT distinct * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,
               ...
               START REPORT apmg631_subrep02
               DECLARE apmg631_repcur2 CURSOR FROM l_sql
               FOREACH apmg631_repcur2 INTO sr3.*
                  OUTPUT TO REPORT apmg631_subrep02(sr3.*)
               END FOREACH
               FINISH REPORT apmg631_subrep02

#子報表2
               LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        " WHERE rvvslk01 = '",sr1.rvu01 CLIPPED,"'",
                        " AND rvvslk02 = ",sr1.rvv02 CLIPPED
               START REPORT apmg631_subrep03
               DECLARE apmg631_repcur3 CURSOR FROM l_sql
               FOREACH apmg631_repcur3 INTO sr4.*
                   OUTPUT TO REPORT apmg631_subrep03(sr4.*)
               END FOREACH
               FINISH REPORT apmg631_subrep03
        ...
        AFTER GROUP OF sr1.rvu01
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        ...
                        " AND rvbs02 = ",sr1.rvv02 CLIPPED
            START REPORT apmg631_subrep01
            DECLARE apmg631_slkrepcur1 CURSOR FROM l_sql
            FOREACH apmg631_slkrepcur1 INTO sr2.*
                OUTPUT TO REPORT apmg631_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT apmg631_subrep01
