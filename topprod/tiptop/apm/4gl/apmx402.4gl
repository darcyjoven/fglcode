...
# Modify.........: No.MOD-550035 05/06/16 By kim 已結案的,不show在報表上,修改apmx402.ora
...
# Modify.........: No.TQC-820008 08/02/16 By baofei 修改INSERT NTO temptable語法
...
# Modify.........: No.FUN-CB0001 12/11/05 By yangtt CR轉XtraGrid
...
# Modify.........: No.FUN-D40129 13/05/08 By yangtt pml16新增開窗
...
## *** FUN-750093  與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>> *** ##
    ...
    LET l_table = cl_prt_temptable('apmx402',g_sql) CLIPPED   # 產生Temp Table                                                      
...
#----------------No.TQC-610085 end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL x402_tm(0,0)		# Input print condition
      ELSE CALL x402()			# Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
END MAIN
 
FUNCTION x402_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,      #No.FUN-680136 SMALLINT
   ...
   OPEN WINDOW x402_w AT p_row,p_col WITH FORM "apm/42f/apmx402"
 ...
 WHILE TRUE
      ...
      ON ACTION CONTROLP
         CASE WHEN INFIELD(pmk01)      #請購單號
             ...
             #FUN-D40129---add---str--
              WHEN INFIELD(pml16) 
                   CALL cl_init_qry_var()
                   LET g_qryparam.form = "q_pml16"
                   LET g_qryparam.state= "c"
                   CALL cl_create_qry() RETURNING g_qryparam.multiret
                   DISPLAY g_qryparam.multiret TO pml16
                   NEXT FIELD pml16
             #FUN-D40129---add---end--
              OTHERWISE
                   EXIT CASE
   ...
   IF INT_FLAG THEN LET INT_FLAG = 0 CLOSE WINDOW x402_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      EXIT PROGRAM 
   END IF
   IF tm.wc=" 1=1 " THEN
...
################################################################################
   ...
   END INPUT
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW x402_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('apmx402',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW x402_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      EXIT PROGRAM
...
#         l_sql      LIKE type_file.chr1000,	# RDSQL STATEMENT                  #No.FUN-680136 VARCHAR(1000)  #TQC-B40074
   ...
   DEFINE sr         RECORD
                    ...
                    #rvb08     LIKE    rvb_file.rvb08,   #數量	#MOD-970250 mark
                     ...
                     END RECORD
     DEFINE  l_str   STRING    #FUN-CB0001 add
...
LET l_sql = 
 ...
 " ORDER BY rvb01,rvb02 "
     ...
     END IF
     DECLARE x402_c1 CURSOR FOR x402_p1
 
#     CALL cl_outnam('apmx402') RETURNING l_name      #No.FUN-750093
#     START REPORT x402_rep TO l_name                 #No.FUN-750093
 
#     LET g_pageno = 0                                #No.FUN-750093
     FOREACH x402_c1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
 ...
 # -- No.FUN-750093 -- begin --
     ...
     EXECUTE insert_prep USING                                                                                                      
                  ...
                  #sr.rva06,sr.rvb08,sr.l_order1,sr.order2		#MOD-970250 mark 
                   sr.rva06,sr.rvb07,l_num,l_num1,sr.l_order1,sr.order2	#MOD-970250   #FUN-CB0001 add l_num,l_num1
 
 
# -- No.FUN-750093 -- end --
#       OUTPUT TO REPORT x402_rep(sr.*)              # FUN-750093
     END FOREACH
 
#     FINISH REPORT x402_rep                         # FUN-750093
...
###XtraGrid###     LET l_sql= "SELECT *  FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                                
     LET g_str=''                                                                                                                   
###XtraGrid###     LET g_str=tm.wc,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",tm.t
###XtraGrid###     CALL cl_prt_cs3('apmx402','apmx402',l_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
    #FUN-CB0001-----add---str---
     CALL cl_wcchp(tm.wc,'pmk01,pmk04,pmk12,pmk13,pml04')   #FUN-CB0001 
           RETURNING tm.wc             #FUN-CB0001
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"pmk01,pmk04,pmk12,pmk13,pml04")
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"pmk01,pmk04,pmk12,pmk13,pml04")  #FUN-D30053 mark
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"pmk01,pmk04,pmk12,pmk13,pml04")
   #LET l_str = cl_wcchp(g_xgrid.order_field,"pmk01,pmk04,pmk12,pmk13,pml04") #FUN-D30070 mark
