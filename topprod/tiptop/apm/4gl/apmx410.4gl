...
#--------------------------CR(1)--------------------------------#                                                                   
   ...
   LET l_table1 = cl_prt_temptable('apmx4101',g_sql) CLIPPED                                                                       
   ...
   #
   #LET l_table2 = cl_prt_temptable('apmx4102',g_sql) CLIPPED                                                                        
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW x410_w AT p_row,p_col WITH FORM "apm/42f/apmx410"
...
END CONSTRUCT
   ...
   IF g_bgjob = 'Y' THEN
      ...
      IF SQLCA.sqlcode OR l_cmd IS NULL THEN
         CALL cl_err('apmx410','9031',1)   #No.FUN-660129
      ELSE
         ...
         CALL cl_cmdat('apmx410',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW x410_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      EXIT PROGRAM
...
FUNCTION x410()
   DEFINE l_name     LIKE type_file.chr20, 		 # External(Disk) file name      #No.FUN-680136 VARCHAR(20)
          ...
          #l_sql      LIKE type_file.chr1000,		 # RDSQL STATEMENT               #No.FUN-680136 VARCHAR(1000)   #FUN-CB0002  mark
          l_sql      STRING,                              #FUN-CB0002 add 		 
...
#No.CHI-6A0004------End-------------
    ...
    #LET l_sql = l_sql CLIPPED,cl_get_extra_cond('pmkuser', 'pmkgrup')   #MOD-C90195 mark
     ...
     PREPARE x410_p1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
        EXIT PROGRAM
     END IF
     DECLARE x410_c1 CURSOR FOR x410_p1
...
#    CALL cl_outnam('apmx410') RETURNING l_name
#    #MOD-540173 add
#    FOR g_i = 1 TO 3
#       CASE
...
#No.FUN-840021 --Add--Begin--
     ...
     ELSE
        IF g_sma115 = "Y" THEN
           LET l_name = 'apmx410_1'
        ELSE
           LET l_name = 'apmx410_2'
        END IF
     END IF
#No.FUN-840021 --Add--End--  
 
     LET g_pageno = 0
     FOREACH x410_c1 INTO sr.*
...
#      OUTPUT TO REPORT x410_rep(sr.*)                                                       #FUN-840021--Mark
...
#    FINISH REPORT x410_rep                                                                  #FUN-840021--Mark
...
###XtraGrid###     LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED                                                         
     #-----END CHI-950035-----
     LET g_str = ''                                                                                                                 
     #是否列印選擇條件 
     #FUN-CB0002--mark--str--
...
###XtraGrid###                 g_azi03,";",g_azi04    #CHI-950035                                                                           
###XtraGrid###     CALL cl_prt_cs3('apmx410',l_name,g_sql,g_str)                                                                               
    ...
    IF g_zz05 = 'Y' THEN                                                                                                           
       CALL cl_wcchp(tm.wc,'pmk01,pmk04,pmk07,pmk12,pmk09,pmn01')                                                                                    
             RETURNING tm.wc                                                                                                        
    END IF  
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"pmk01,pmk04,pmk07,pmk12,pmk09,pmn01")

   #LET l_str = cl_wcchp(g_xgrid.order_field,"pmk01,pmk04,pmk07,pmk12,pmk09,pmn01") #FUN-D30070 mark
