...
# Modify.........: No.FUN-CB0001 12/11/12 By yangtt CR轉XtraGrid	
# Modify.........: No.FUN-D40129 13/05/08 By yangtt 【採購單號】【料號範圍】開窗增加料件規格欄位的顯示
...
MAIN
   ...
   #FUN-C30285---begin
#  IF s_industry("icd") THEN   #FUN-CB0001 mark
      LET g_sql = "rva05.rva_file.rva05,",
                  ...
                  "print.type_file.chr1,",     #FUN-CB0001 add
                  "sum1.type_file.num20_6,",   #FUN-CB0001 add
                  "sum2.type_file.num20_6,",   #FUN-CB0001 add
                  "l_num1.type_file.num5"      #FUN-CB0001 add
#  ELSE   #FUN-CB0001 mark 
   #FUN-C30285---end
  #FUN-CB0001----mark---str---
  ##No.FUN-7C0034---Begin
  #   LET g_sql = "rva05.rva_file.rva05,",
...
#  IF s_industry("icd") THEN    #FUN-CB0001 mark 
  ...
  #FUN-CB0001----mark---end---
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'
      THEN CALL x515_tm(0,0)
      ELSE CALL x515()
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
END MAIN
 
FUNCTION x515_tm(p_row,p_col)
   ...
   OPEN WINDOW x515_w AT p_row,p_col WITH FORM "apm/42f/apmx515"
   ...
   ON ACTION CONTROLP
     CASE WHEN INFIELD(rva05)      #廠商編號
          ...
          WHEN INFIELD(rvb04)      #採購單號
               CALL cl_init_qry_var()
               LET g_qryparam.state= "c"
              #LET g_qryparam.form = "q_pmm602"    #FUN-D40129 mark
               LET g_qryparam.form = "q_pmm602_1"  #FUN-D40129 add
               CALL cl_create_qry() RETURNING g_qryparam.multiret
          ...
          WHEN INFIELD(rvb05)      #料件編號
               CALL cl_init_qry_var()
               LET g_qryparam.state= "c"
              #LET g_qryparam.form = "q_ima1"     #FUN-D40129 mark 
               LET g_qryparam.form = "q_ima1_1"   #FUN-D40129 add
               CALL cl_create_qry() RETURNING g_qryparam.multiret
...
#UI
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('apmx515',g_time,l_cmd)
      END IF
      CLOSE WINDOW x515_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      EXIT PROGRAM
...
FUNCTION x515()
   DEFINE l_name    LIKE type_file.chr20,       # External(Disk) file name        #No.FUN-680136 VARCHAR(20)
     ...
     DEFINE l_print1     LIKE type_file.chr1      #FUN-CB0001
     DEFINE l_sum1       LIKE type_file.num20_6   #FUN-CB0001
     DEFINE l_sum2       LIKE type_file.num20_6   #FUN-CB0001
     DEFINE l_num1       LIKE type_file.num5      #FUN-CB0001
     DEFINE l_str1       STRING                   #FUN-CB0001
     DEFINE l_str2       STRING                   #FUN-CB0001
...
#No.CHI-6A0004--------------------------------------End---------------
     ...
     END IF
     PREPARE x515_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
        EXIT PROGRAM
     END IF
     DECLARE x515_curs1 CURSOR FOR x515_prepare1
     #No.FUN-7C0034---Begin
     #CALL cl_outnam('apmx515') RETURNING l_name
     #FUN-C30285---begin
     IF s_industry("icd") THEN 
        IF g_sma.sma116 MATCHES '[13]' THEN
           #LET l_name = 'apmx515_2'            #FUN-CB0001 mark
            LET l_name = 'apmx515_2|apmx515_3'  #FUN-CB0001 add
        ELSE
           #LET l_name = 'apmx515_3'            #FUN-CB0001 mark
            LET l_name = 'apmx515_2|apmx515_4'  #FUN-CB0001 add
        END IF
     ELSE 
     #FUN-C30285---end
      ##TQC-6C0124---add---str---
      ...
      #     LET g_zaa[50].zaa06 = "N" #計價數量
           #LET l_name = 'apmx515'              #FUN-CB0001 mark
            LET l_name = 'apmx515|apmx515_2'    #FUN-CB0001 add
        ELSE
      #     LET g_zaa[46].zaa06 = "Y" #入庫單位
      #     LET g_zaa[49].zaa06 = "Y" #計價單位
      #     LET g_zaa[50].zaa06 = "Y" #計價數量
           #LET l_name = 'apmx515_1'            #FUN-CB0001 mark
            LET l_name = 'apmx515_1|apmx515_2'  #FUN-CB0001 add
        END IF
      ##TQC-6C0124---add---end---
     END IF   #FUN-C30285
     #CALL cl_prt_pos_len()    #TQC-6C0124  add
 
     #START REPORT x515_rep TO l_name
     #No.FUN-7C0034---End
     FOREACH x515_curs1 INTO sr.*,l_rvb38,l_rvbiicd03   #FUN-C30285
          IF SQLCA.sqlcode != 0 THEN
...
#         IF NOT cl_prichk('$') THEN LET sr.rvb10 = NULL END IF
          ...
          ELSE 
             LET l_print1 = ' ' 
          END IF 
          LET l_sum1 = sr.rvb87 * sr.rvb10
          LET l_sum2 = sr.rvb87 * sr.rvb10 * sr.pmm42
          #FUN-CB0001---add---end---
          #FUN-C30285---begin
          LET l_num1 = 0 #FUN-CB0001 add
          IF s_industry("icd") THEN
             EXECUTE insert_prep USING sr.rva05,sr.pmc03,sr.rvb19,sr.rva01,
                                       ...
                                       l_rvb38,l_rvbiicd03,
                                       l_print1,l_sum1,l_sum2,l_num1  #FUN-CB0001 add
          ELSE 
          #FUN-C30285---end
             EXECUTE insert_prep USING sr.rva05,sr.pmc03,sr.rvb19,sr.rva01,
                                       sr.rvb02,sr.rvb22,sr.rvb34,sr.rva06,
                                       ...
                                       '','',l_print1,l_sum1,l_sum2,l_num1   #FUN-CB0001 add
          END IF   #FUN-C30285 
          #OUTPUT TO REPORT x515_rep(sr.*)
          #No.FUN-7C0034---End
     END FOREACH
     #No.FUN-7C0034---Begin
     #FINISH REPORT x515_rep
     #CALL cl_prt(l_name,g_prtway,g_copies,g_len)
###XtraGrid###     LET g_str = g_str,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",           
###XtraGrid###                 g_sma.sma116,";",g_aza.aza17,";",g_azi05,";",tm.sub,";",tm.b          
###XtraGrid###     LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED         
###XtraGrid###     CALL cl_prt_cs3('apmx515',l_name,l_sql,g_str) 
...
#FUN-CB0001---add---str--
    ...
    IF g_zz05 = 'Y' THEN                                                       
       ...
       RETURNING g_str                                                         
    END IF                                                                     
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"rva05,rva06,rva01,rvb04,rvb05")   
    LET g_xgrid.grup_field = 'rva05' 
    CASE tm.sub
       WHEN '1' LET l_str1 = cl_getmsg('apm-384',g_lang),':',cl_getmsg('apm-382',g_lang)
       WHEN '2' LET l_str1 = cl_getmsg('apm-384',g_lang),':',cl_getmsg('apm-383',g_lang)
       WHEN '3' LET l_str1 = cl_getmsg('apm-384',g_lang),':',cl_getmsg('rpt-016',g_lang)
    END CASE
    CASE tm.b
       WHEN '1' LET l_str2 = cl_getmsg('apm-385',g_lang),':',cl_getmsg('apm-380',g_lang)
       WHEN '2' LET l_str2 = cl_getmsg('apm-385',g_lang),':',cl_getmsg('apm-381',g_lang)
       WHEN '3' LET l_str2 = cl_getmsg('apm-385',g_lang),':',cl_getmsg('rpt-016',g_lang)
    END CASE
    LET g_xgrid.footerinfo1 = l_str1 
    LET g_xgrid.footerinfo2 = l_str2 
