...
# Modify.........: No:FUN-D60130 13/06/28 By lixh1 委外倉退改善:既能退數量也能退金額,委外工單結案后也能進行委外倉退
...
DEFINE g_rvv38t_t         LIKE rvv_file.rvv38t   #TQC-D80008 add
...
#TQC-AA0132 --end-- 
   ...
   ELSE
      IF g_bgjob='N' OR cl_null(g_bgjob) THEN               #FUN-A50001 add
        CASE
          WHEN (g_argv1='1' AND g_argv3='ICD')
               OPEN WINDOW t720_w WITH FORM "aic/42f/aict042"
                ATTRIBUTE (STYLE = g_win_style CLIPPED) 
          WHEN (g_argv1='2' AND g_argv3='ICD')
               OPEN WINDOW t720_w WITH FORM "aic/42f/aict042"
                ATTRIBUTE (STYLE = g_win_style CLIPPED) 
          WHEN (g_argv1='3' AND g_argv3='ICD')
               OPEN WINDOW t720_w WITH FORM "aic/42f/aict042"
                ATTRIBUTE (STYLE = g_win_style CLIPPED) 
...
#FUN-B90060 --START--
         IF g_prog = 'aict042' OR g_prog[1,7] = 'apmt730' THEN
            CALL cl_set_act_visible("apm_next_wo",TRUE)
         ELSE    
            CALL cl_set_act_visible("apm_next_wo",FALSE)
         END IF  
#FUN-B90060 --END--
   ...
   ELSE
    ...
    IF g_argv2<> ' ' OR g_icq='Y'  OR g_argv5 <> ' ' THEN   #MOD-C90169 add argv5
       IF NOT cl_null(g_argv3) THEN
          ...
          ELSE
             IF (NOT cl_null(g_prog)) AND (g_prog[1,3]='aic') THEN
                LET g_wc =" (rvu02='",g_argv2 CLIPPED,"' OR rvu01='",g_argv2 CLIPPED,"')",
                          " AND rvu08 LIKE 'WB_'"   
             ELSE
                LET g_wc =" (rvu02='",g_argv2 CLIPPED,"' OR rvu01='",g_argv2 CLIPPED,"')",
                          " AND rvu08 ='",g_argv3,"'"   
        ...
        ON ACTION CONTROLP
           CASE
               ...
               WHEN INFIELD(rvu02)  #驗收單
                    ...
                    #MOD-B60049 add --end--
                    IF g_argv3='ICD' THEN
                       LET g_qryparam.where=g_qryparam.where, " AND rva10 LIKE 'WB%'"       #TQC-C50103 add
                    ELSE      
                       LET g_qryparam.where=g_qryparam.where, " AND rva10 NOT LIKE 'WB%'"   #TQC-C50103 add
                    END IF
                    CALL cl_create_qry() RETURNING g_qryparam.multiret
   ...
   #FUN-B40031 End-----
       ...
       LET g_wc = g_wc CLIPPED,cl_get_extra_cond('rvuuser', 'rvugrup')
        ...
        CONSTRUCT g_wc2 ON rvv02,rvv05,rvv45,rvv46,qcl02,rvv47,rvv31,rvv031, #FUN-BC0104 add rvv45,rvv46,qcl02,rvv47
                          rvviicd03,rvviicd04,   #No.FUN-7C0017
                          rvv35,rvv35_fac, #MOD-530073
                          rvv10,rvv11,rvv12,rvv13,   #NO.FUN-960130---add---
                          rvv18,rvv36,rvv37,rvv919,pmm909,pmn24,pmn25,rvv25,rvv89,rvv17,rvv32,rvv33,rvv34,   #FUN-9A0068 add rvv89   #FUN-A80150 add rvv919
                          rvviicd05,rvviicd01,rvviicd02,rvviicd06,#No.FUN-7C0017
                     ...
                     FROM s_rvv[1].rvv02, s_rvv[1].rvv05,
                          ...
                          s_rvv[1].rvv31,
                          s_rvv[1].rvv031,
                          s_rvv[1].rvviicd03,s_rvv[1].rvviicd04,  #No.FUN-7C0017
                          s_rvv[1].rvv35, s_rvv[1].rvv35_fac, #MOD-530073
                          s_rvv[1].rvv10,s_rvv[1].rvv11,s_rvv[1].rvv12,s_rvv[1].rvv13,  #NO.FUN-960130----add---
                          s_rvv[1].rvv18, s_rvv[1].rvv36, s_rvv[1].rvv37,s_rvv[1].rvv919,     #FUN-A80150 add rvv919
                          ...
                          s_rvv[1].rvviicd05,s_rvv[1].rvviicd01,s_rvv[1].rvviicd02,s_rvv[1].rvviicd06, #No.FUN-7C0017
                          s_rvv[1].rvv83,
...
#FUN-AA0059---------mod------------end-----------------
                 ...
                 WHEN INFIELD(rvv05) #查詢驗收單項次
                      IF g_rvu.rvu00='3' THEN #-- 倉退
                          CALL q_rvb7(TRUE,TRUE,g_rvu.rvu02,g_rvv[1].rvv05,g_rvu.rvu01) RETURNING g_qryparam.multiret
                      ELSE
                          CALL cl_init_qry_var()
               ...
               WHEN INFIELD(pmn24) #請購單號
                ...
                WHEN INFIELD(rvvud06)
                   ...
                   NEXT FIELD rvvud06
                #CHI-A70035 add --end--
                WHEN INFIELD(rvviicd01)
                   CALL cl_init_qry_var()
                   LET g_qryparam.form  = "q_ecd_icd"
                   LET g_qryparam.state = "c"   #多選
                   #MOD-B90102 ---- add start -----
                   CALL cl_create_qry() RETURNING g_qryparam.multiret
                   DISPLAY g_qryparam.multiret TO rvviicd01
                   NEXT FIELD rvviicd01
                   #MOD-B90102 ---- add end -----
                   #MOD-B90102 ---- mark start -----
                   #CALL cl_create_qry() RETURNING g_rvv[l_ac].rvviicd01
                   #DISPLAY BY NAME g_rvv[l_ac].rvviicd01
                   #NEXT FIELD rvbiicd01
                   #MOD-B90102 ---- mark end -----
                WHEN INFIELD(rvviicd03)
                   CALL cl_init_qry_var()
                   ...
                   #MOD-B90102 ---- add end -----
                   #MOD-B90102 ---- mark start -----
                   #CALL cl_create_qry() RETURNING g_rvv[l_ac].rvviicd03
                   #DISPLAY BY NAME g_rvv[l_ac].rvviicd03
                   #MOD-B90102 ---- mark end -----
                   NEXT FIELD rvviicd03
                WHEN INFIELD(rvviicd04)
#FUN-AA0059---------mod------------str-----------------               
#                   CALL cl_init_qry_var()
#                   LET g_qryparam.form = "q_ima"
#                   LET g_qryparam.state = 'c'
#                   CALL cl_create_qry() RETURNING g_rvv[l_ac].rvviicd04
                    #CALL q_sel_ima(TRUE, "q_ima","","","","","","","",'')  RETURNING  g_rvv[l_ac].rvviicd04  #MOD-B90102 mark
                    CALL q_sel_ima(TRUE, "q_ima","","","","","","","",'')  RETURNING  g_qryparam.multiret     #MOD-B90102 add
#FUN-AA0059---------mod------------end-----------------
                   #DISPLAY BY NAME g_rvv[l_ac].rvviicd04   #MOD-B90102 mark
                   DISPLAY g_qryparam.multiret TO rvviicd04 #MOD-B90102 add
                   NEXT FIELD rvviicd04
           END CASE
          ON IDLE g_idle_seconds
             CALL cl_on_idle()
             CONTINUE CONSTRUCT
...
#TQC-A60046 --End
          ...
          IF g_wc2.getindexof('rvvi',1)>0 THEN
             LET g_sql = "SELECT UNIQUE rvu01,rvu00 ",
                         "  FROM rvu_file, rvv_file,rvvi_file",
                         #"      ,pmm_file, pmn_file",            #No.FUN-940083
                         ",OUTER (pmn_file, OUTER pmm_file) ",    #No.FUN-940083
                         " WHERE rvu01 = rvv01 ",
                         #"   AND pmm01 = pmn01 ",                  #No.FUN-940083
                         "   AND pmm_file.pmm01 = pmn_file.pmn01 ", #No.FUN-940083
                         "   AND pmn_file.pmn01 = rvv36 AND pmn_file.pmn02 = rvv37 ",   #No.FUN-940083]
                         "   AND rvv01 = rvvi01 ",
                         "   AND rvv02 = rvvi02 ",
    ...
    IF NOT cl_null(g_argv3) THEN
       IF g_argv3 !='TAP' THEN
          IF g_argv3='ICD' THEN
             LET g_sql=g_sql clipped," AND rvu08 LIKE 'WB%'"
          ELSE
    ...
    ELSE
       ...
       LET g_sql=g_sql clipped," AND rvu08 NOT LIKE 'WB%'"
    END IF    
    ...
    ELSE
       IF g_prog = 'apmt720' OR g_prog = 'aict042' THEN  #MOD-D30083 add aict042
          LET g_sql="SELECT COUNT(DISTINCT rvu01) ",
                    ...
                    "   AND ",g_wc CLIPPED," AND ",g_wc2 CLIPPED
          IF g_wc2.getindexof('rvvi',1)>0 THEN
             LET g_sql="SELECT COUNT(DISTINCT rvu01) ",             #No.FUN-940083
                       "   FROM rvu_file,rvv_file,rvvi_file,",      #No.FUN-940083
                       "        OUTER (pmn_file,OUTER pmm_file )",  #No.FUN-940083
                       " WHERE rvv01=rvu01 ",
                       "   AND pmm_file.pmm01 = pmn_file.pmn01 ",   #No.FUN-940083
                       "   AND rvv36 = pmn_file.pmn01 ",            #No.FUN-940083
       ...
       ELSE
          LET g_sql="SELECT COUNT(DISTINCT rvu01) FROM rvu_file,rvv_file ",
                    ...
                    "   AND ",g_wc CLIPPED," AND ",g_wc2 CLIPPED
          IF g_wc2.getindexof('rvvi',1)>0 THEN
             LET g_sql="SELECT COUNT(DISTINCT rvu01) FROM rvu_file,rvv_file,rvvi_file ",
                       " WHERE rvv01 = rvu01 ",
                       "   AND rvv01 = rvvi01 ",
                       "   AND rvv02 = rvvi02 ",
                       "   AND ",g_wc CLIPPED," AND ",g_wc2 CLIPPED
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
       ...
       #@WHEN "拋轉還原"
         ...
         WHEN "aic_qry_back"
            IF cl_chk_act_auth() THEN
               IF NOT cl_null(g_rvu.rvu01) AND
                  NOT cl_null(l_ac) AND l_ac <> 0 THEN
                  LET g_cmd = "aicq021 '",g_rvu.rvu02,"' ",
                              "'",g_rvv[l_ac].rvv05,"'"
         ...
         WHEN "aic_s_icdqry"
            IF cl_chk_act_auth() THEN
               IF NOT cl_null(g_rvu.rvu01) THEN
                  CALL s_icdqry(1,g_rvu.rvu01,'',g_rvu.rvuconf,'')
               END IF
            END IF
               ...
               IF NOT cl_null(g_rvu.rvu02) THEN
                  ...
                  ELSE 
                     IF cl_confirm('abx-080') THEN 
                        LET g_msg="aicp047 'Y' '",g_rvu.rvu02,"'"
                        CALL cl_cmdrun_wait(g_msg CLIPPED)
                     END IF 
                  END IF
               END IF      
           ...
           IF NOT cl_null(l_rvu08) THEN
              CASE
                 WHEN g_argv3 !='SUB' AND g_argv3 !='TAP'
                    IF g_prog[1,3] = 'aic' THEN
                       IF l_rvu08 <> 'WB0' AND l_rvu08 <> 'WB2' THEN
...
#FUN-D50103 ----------- add -------------- begin ---------------
                 CASE
                    WHEN g_argv3 !='SUB' AND g_argv3 !='TAP'
                       IF g_prog[1,3] = 'aic' THEN
                          LET g_sql = " (smy72 in('WB0','WB2') OR smy72 is null ) "
                       ELSE
...
#                      LET g_qryparam.where    = " rvb31 >0 "  #MOD-8A0070   #MOD-B60049 mark
                       ...
                       #MOD-B60049 add --end--
                       IF g_argv3='ICD' THEN
                          LET g_qryparam.where    = g_qryparam.where, " AND rvb31 >0 AND rva10 LIKE 'WB%'" #MOD-8A0070     #TQC-C50103 add
                       ELSE
                          LET g_qryparam.where    = g_qryparam.where, " AND rvb31 >0 AND rva10 NOT LIKE 'WB%'" #MOD-8A0070 #TQC-C50103 add
                       END IF
...
#                      LET g_qryparam.where    = " rvb18 <='30' "  #MOD-8A0070    #MOD-B60049 mark
                       ...
                       #MOD-B60049 add --end--
                       IF g_argv3='ICD' THEN
                          LET g_qryparam.where    = g_qryparam.where, " AND rvb18 <='30' AND rva10 LIKE 'WB%'"  #MOD-8A0070    #TQC-C50103 add
                       ELSE
                          LET g_qryparam.where    = g_qryparam.where, " AND rvb18 <='30' AND rva10 NOT LIKE 'WB%'" #MOD-8A0070 #TQC-C50103 add
                       END IF
                       CALL cl_create_qry() RETURNING g_rvu.rvu02
                       DISPLAY BY NAME g_rvu.rvu02        
                  WHEN g_rvu.rvu00='3'
...
#                      LET g_qryparam.where    = " rvb18 <='30' "   #MOD-B60049 mark
                       ...
                       #MOD-B60049 add --end--
                       IF g_argv3='ICD' THEN
                          LET g_qryparam.where    = g_qryparam.where, " AND rvb18 <='30' AND rva10 LIKE 'WB%'"     #TQC-C50103 add
                       ELSE
                          LET g_qryparam.where    = g_qryparam.where, " AND rvb18 <='30' AND rva10 NOT LIKE 'WB%'" #TQC-C50103 add
                       END IF
                       CALL cl_create_qry() RETURNING g_rvu.rvu02
                        DISPLAY BY NAME g_rvu.rvu02              #No.MOD-490371
                 END CASE
...
FUNCTION t720_r()  #刪除
    ...
    DEFINE l_type          LIKE type_file.num5  #FUN-810038
          ...
          ELSE
             CASE g_argv1
                ...
                WHEN "3"
                   LET l_type = -1
             END CASE
             CALL s_icdinout_del(l_type,g_rvu.rvu01,'','') RETURNING l_flag  #FUN-B80119--傳入p_plant參數''---
             IF l_flag = 0 THEN
                CLOSE t720_cl
                ROLLBACK WORK
                RETURN
...
DEFINE l_type       LIKE type_file.num5     #FUN-810038
DEFINE l_rvbiicd06  LIKE rvbi_file.rvbiicd06  #FUN-A20017
...
#FUN-A60035 ---MARK END 
               ...
               ELSE
                  ...
                  IF SQLCA.sqlcode THEN
                     ...
                     LET l_lock_sw = "Y"
                  ELSE
                     OPEN t720_bcl_ind USING g_rvu.rvu01,g_rvv_t.rvv02
                     IF STATUS THEN
                        CALL cl_err("OPEN t720_bcl_ind:", STATUS, 1)
                     ELSE
                        FETCH t720_bcl_ind INTO b_rvvi.*
                        IF SQLCA.sqlcode THEN
                           CALL cl_err('lock rvvi',SQLCA.sqlcode,1)
                           LET l_lock_sw = "Y"
                        END IF
                     END IF
                     SELECT ima021 INTO g_rvv[l_ac].ima021 FROM ima_file
...
#### Modify:98/11/15 ----單位換算率抓不到 ---####
            ...
            LET b_rvv.rvv88 = 0  #No.TQC-7B0083
            INITIALIZE b_rvvi.* TO NULL  #FUN-810038
            #檢查入庫/驗退數量與刻號入庫否數量是否相同
            IF NOT t720sub_ind_icd_chk_icdin_yn(b_rvv.*,b_rvvi.*,g_rvu.rvu00,g_rvu.rvu08,
                                              g_rvu.rvu02,g_argv1,g_argv3) THEN   #FUN-A10130
               INITIALIZE g_rvv[l_ac].* TO NULL
               CALL g_rvv.deleteElement(l_ac)
               CANCEL INSERT
            END IF
            # 料號為參考單位,單一單位,且狀態為其它段生產料號(imaicd04=3,4)時,
            # 將單位一的資料給單位二
            CALL t720_ind_icd_set_value()
           #-----MOD-AC0024---------
...
#FUN-A60035 ---MARK END
        ...
        #若已有ida_file存在不可修改
        AFTER FIELD rvv02
           IF p_cmd = 'u' AND NOT cl_null(g_rvv[l_ac].rvv02) THEN
              IF g_rvv_t.rvv02 <> g_rvv[l_ac].rvv02 THEN
                 CALL t720_ind_icd_chk_icd()
                 IF NOT cl_null(g_errno) THEN
                    CALL cl_err('',g_errno,0)
                    LET g_rvv[l_ac].rvv02 = g_rvv_t.rvv02
                    DISPLAY BY NAME g_rvv[l_ac].rvv02
                    NEXT FIELD rvv02
                 END IF
              END IF
           END IF
        ...
        AFTER FIELD rvv05             #驗收項次        
           #若已有ida_file存在不可修改
           IF p_cmd = 'u' AND NOT cl_null(g_rvv[l_ac].rvv05) THEN
              IF g_rvv_t.rvv05 <> g_rvv[l_ac].rvv05 THEN
                 CALL t720_ind_icd_chk_icd()
                 IF NOT cl_null(g_errno) THEN
                    CALL cl_err('',g_errno,0)
                    LET g_rvv[l_ac].rvv05 = g_rvv_t.rvv05
                    DISPLAY BY NAME g_rvv[l_ac].rvv05
                    NEXT FIELD rvv05
                 END IF
              END IF
           END IF
           IF NOT cl_null(g_rvu.rvu02) AND NOT cl_null(g_rvv[l_ac].rvv05) THEN
              ...
              CALL t720_b_move_back()
              #檢查入庫/驗退數量與刻號入庫否數量是否相同
              #在此僅先警告不拒絕,AFTER INSERT,ON ROW CHANGE時再總檢
              LET l_flag=t720sub_ind_icd_chk_icdin_yn(b_rvv.*,b_rvvi.*,g_rvu.rvu00,g_rvu.rvu08,
                                                    g_rvu.rvu02,g_argv1,g_argv3)  #FUN-A10130
              IF g_sma.sma115 = 'Y' THEN
...
#&endif       #FUN-B50096
        ...
        AFTER FIELD rvv31
           #若已有ida_file存在不可修改
           IF p_cmd = 'u' AND NOT cl_null(g_rvv[l_ac].rvv31) THEN
              IF g_rvv_t.rvv05 <> g_rvv[l_ac].rvv31 THEN
                 CALL t720_ind_icd_chk_icd()
                 IF NOT cl_null(g_errno) THEN
                    CALL cl_err('',g_errno,0)
...
#&endif      #FUN-B50096
        ...
        AFTER FIELD rvv35             #單位
           #若已有ida_file存在不可修改
           IF p_cmd = 'u' AND NOT cl_null(g_rvv[l_ac].rvv35) THEN
              IF g_rvv_t.rvv35 <> g_rvv[l_ac].rvv35 THEN
                 CALL t720_ind_icd_chk_icd()
                 IF NOT cl_null(g_errno) THEN
                    CALL cl_err('',g_errno,0)
                    LET g_rvv[l_ac].rvv35 = g_rvv_t.rvv35
                    NEXT FIELD rvv35
                 END IF
              END IF
           END IF
        ...
        AFTER FIELD rvv32              #倉庫
           ...
           END IF
           #若已有ida存在不可修改
           IF p_cmd = 'u' AND NOT cl_null(g_rvv[l_ac].rvv32) THEN IF g_rvv_t.rvv32 <> g_rvv[l_ac].rvv32 THEN CALL t720_ind_icd_chk_icd() IF NOT cl_null(g_errno) THEN CALL cl_err('',g_errno,0) LET g_rvv[l_ac].rvv32 = g_rvv_t.rvv32
                    NEXT FIELD rvv32
                 END IF
              END IF
           END IF
                 ...
                 #IF sn1=1 AND g_rvv[l_ac].rvv32 != g_rvv_t.rvv32 THEN  #MOD-D30080 mark
                  IF sn1=1 THEN           #MOD-D30080 add
                     IF NOT cl_null(g_rvv[l_ac].rvv32) OR g_rvv[l_ac].rvv32 != g_rvv_t.rvv32 THEN  #MOD-D30080 add
                        ...
                        CALL cl_err(g_rvv[l_ac].rvv32,'mfg6080',0) NEXT FIELD rvv32
                        NEXT FIELD rvv32  #MOD-D30080 add
        ...
        AFTER FIELD rvv33              #儲位
           ...
           IF p_cmd = 'u' AND NOT cl_null(g_rvv[l_ac].rvv33) THEN
              IF g_rvv_t.rvv33 <> g_rvv[l_ac].rvv33 THEN
                 CALL t720_ind_icd_chk_icd()
                 IF NOT cl_null(g_errno) THEN
                    CALL cl_err('',g_errno,0)
                    LET g_rvv[l_ac].rvv33 = g_rvv_t.rvv33
                    NEXT FIELD rvv33
                 END IF
              END IF
           END IF
          ...
          #IF g_rvv[l_ac].rvv31[1,4] != 'MISC' AND g_rvu.rvu00!='2' AND l_ec_sw='N'  THEN   #CHI-CC0014
           ...
           #------------------------------------ 檢查料號預設倉儲及單別預設倉儲
              ...
              END IF
              IF g_rvv[l_ac].rvv33 IS NULL THEN LET g_rvv[l_ac].rvv33=' ' END IF
             #NEXT FIELD rvv34         #MOD-940128    #TQC-C70207 mark
           END IF
#FUN-B50096 ------------Begin------------
           IF NOT cl_null(g_rvv[l_ac].rvv31) THEN
   ...
   #No.TQC-B90236--------add----------end-------------
           ...
           IF p_cmd = 'u' AND g_sma.sma115 = 'Y' THEN  #修改且多單位
              IF NOT cl_null(g_rvv[l_ac].rvv80)  THEN
                 ...
                 IF NOT cl_null(g_ima906) AND g_ima906 MATCHES '[13]' THEN
                    IF g_rvv_t.rvv80 <> g_rvv[l_ac].rvv80 THEN
                       CALL t720_ind_icd_chk_icd()
                       IF NOT cl_null(g_errno) THEN
                          CALL cl_err('',g_errno,0)
        ...
        AFTER FIELD rvv82  #第一數量
           ...
           IF g_rvu.rvu00='1' THEN
             #no.7143 檢驗碼改單身
              ...
              #本張單據,其他屬於此收貨項次的入庫數量
               ...
               SELECT COALESCE(rvbiicd06,0) INTO l_rvbiicd06 FROM rvbi_file
                WHERE rvbi01=g_rvu.rvu02 AND rvbi02=g_rvv[l_ac].rvv05
               IF l_rvv17_1_other > (l_rvb33_tol+l_rvbiicd06) THEN
               #FUN-A20017--end--add---
                  CALL cl_err(g_rvv[l_ac].rvv31,'apm-306',0)
                  NEXT FIELD rvv82
               END IF
          ...
          #MOD-C40047 end mark----
           ...
           IF NOT t720sub_ind_icd_chk_icdin_yn(b_rvv.*,b_rvvi.*,
                                            g_rvu.rvu00,g_rvu.rvu08,g_rvu.rvu02,g_argv1,g_argv3) THEN   #FUN-A10130
              LET g_rvv[l_ac].rvv82 = g_rvv_t.rvv82
              DISPLAY BY NAME g_rvv[l_ac].rvv82
              NEXT FIELD rvv82 
           END IF
           
           IF p_cmd = 'u' AND NOT cl_null(g_rvv_t.rvv82) AND
              g_rvv_t.rvv82 <> g_rvv[l_ac].rvv82 THEN
              #修改數量,若料號有展明細(imaicd08) ='Y',
              #則需檢查發料量與所維護之數量是否一致,如果不一致,出錯誤訊息
              CALL t720_ind_icd_chk_icdqty()
...
#MOD-B70231 -- end --
         ...
         AFTER FIELD rvv38t
          ...
          #IF g_rvv[l_ac].rvv87>0 THEN #MOD-B30033 mark
           IF g_rvv[l_ac].rvv87>0 OR g_rvu.rvu00 ='1' THEN #MOD-B30033
            ...
            #TQC-D80008 --------Begin-----------
              IF g_azw.azw04 = '2' AND g_prog='apmt742' THEN
                 IF g_rvv[l_ac].rvv38t <> g_rvv38t_t OR cl_null(g_rvv38t_t)  THEN
        ...
        BEFORE DELETE                            #是否取消單身
           IF g_rvv_t.rvv02 > 0 AND NOT cl_null(g_rvv_t.rvv02) THEN
              ...
              END IF
              #若已有ida資料要一併刪除
              #IF g_argv3 <> "TAP"  THEN  #FUN-B90012--mark--
              LET g_cnt = 0
              IF g_argv1 MATCHES '[12]' THEN
                 SELECT COUNT(*) INTO g_cnt FROM ida_file
                    WHERE ida07 = g_rvu.rvu01
                      AND ida08 = g_rvv_t.rvv02
              ELSE
                 SELECT COUNT(*) INTO g_cnt FROM idb_file
                    WHERE idb07 = g_rvu.rvu01
                      AND idb08 = g_rvv_t.rvv02
              END IF
              IF g_cnt > 0 THEN
                 IF NOT cl_confirm('aic-112') THEN
                    CANCEL DELETE
                 ELSE
                    IF g_argv1='2' THEN #驗退 (ICD的驗退要打倉儲批)
                       LET l_type = 0
                    END IF
                    IF g_argv1='1' THEN #入庫
                       LET l_type = 1
                    END IF
                    IF g_argv1='3' THEN #倉退
                       LET l_type = -1
                    END IF
                    CALL s_icdinout_del(l_type,g_rvu.rvu01,g_rvv_t.rvv02,'')  #FUN-B80119--傳入p_plant參數''---
                         RETURNING l_flag
                    IF l_flag = 0 THEN ROLLBACK WORK CANCEL DELETE END IF
                 END IF
              END IF
              #END IF   #FUN-B90012--mark--
             ...
             #FUN-A60035 ---MARK END
                ...
                ELSE  #FUN-BC0104 add
               #ELSE  #FUN-BC0104 mark 
                  #IF NOT s_del_rvvi(g_rvu.rvu01,g_rvv_t.rvv02,'') THEN       #No.FUN-A50054
                   IF NOT s_del_rvvi(g_rvu.rvu01,g_rvv_t.rvv02,g_plant) THEN  #No.FUN-A50054
                      ROLLBACK WORK
                      CANCEL DELETE
                   END IF
                   #FUN-BC0104---add---str
        ...
        ON ROW CHANGE
            IF INT_FLAG THEN
               ...
               EXIT INPUT
            END IF
            IF cl_null(g_rvv_t.rvv33) THEN LET g_rvv_t.rvv33 = ' ' END IF
            IF cl_null(g_rvv_t.rvv34) THEN LET g_rvv_t.rvv34 = ' ' END IF
            ...
            IF cl_null(g_rvv[l_ac].rvv34) THEN
               LET g_rvv[l_ac].rvv34 = ' '
            END IF
            IF g_rvv_t.rvv02 <> g_rvv[l_ac].rvv02 OR
               g_rvv_t.rvv05 <> g_rvv[l_ac].rvv05 OR
               g_rvv_t.rvv31 <> g_rvv[l_ac].rvv31 OR
               g_rvv_t.rvv32 <> g_rvv[l_ac].rvv32 OR
               g_rvv_t.rvv33 <> g_rvv[l_ac].rvv33 OR
               g_rvv_t.rvv34 <> g_rvv[l_ac].rvv34 OR
               g_rvv_t.rvv35 <> g_rvv[l_ac].rvv35 THEN
               CALL t720_ind_icd_chk_icd()
               IF NOT cl_null(g_errno) THEN
                  CALL cl_err('',g_errno,0)
                  NEXT FIELD rvv05
            ...
            ELSE
               ...
               #MOD-D30016 add end  -----
                ...
                IF cl_null(g_rvv[l_ac].rvv34) THEN
                   LET g_rvv[l_ac].rvv34=' '
                END IF
                #檢查入庫/驗退數量與刻號入庫否數量是否相同,不同則err
                IF NOT t720sub_ind_icd_chk_icdin_yn(b_rvv.*,b_rvvi.*,
                                         g_rvu.rvu00,g_rvu.rvu08,g_rvu.rvu02,g_argv1,g_argv3) THEN   #FUN-A10130
                   NEXT FIELD rvv05
                END IF
                #料號為參考單位,單一單位,且狀態為其它段生產料號(imaicd04=3,4)
                #時,將單位一的資料給單位二
                CALL t720_ind_icd_set_value()
...
#FUN-A60035 ---MARK END
               ...
               #MOD-D30016 mark end  -----
                ...
                IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
                   ...
                   LET g_rvv[l_ac].* = g_rvv_t.*
                ELSE
                   UPDATE rvvi_file SET * = b_rvvi.*
                    WHERE rvvi01=g_rvu.rvu01
                      AND rvvi02=g_rvv_t.rvv02
                    IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
                       CALL cl_err3("upd","rvvi_file",g_rvu.rvu01,g_rvv_t.rvv02,SQLCA.sqlcode,"","upd rvvi",1)
                       LET g_rvv[l_ac].* = g_rvv_t.*
                    END IF
...
#CHI-B90021 -----------------End-------------------
         ...
         AFTER INPUT
             ...
             FOR l_ac = 1 TO g_cnt
                 ...
                 IF g_ima918 = "Y" OR g_ima921 = "Y" THEN
                    UPDATE rvbs_file SET rvbs021 = g_rvv[l_ac].rvv31
                      WHERE rvbs00 = g_prog
                        AND rvbs01 = g_rvu.rvu01
                        AND rvbs02 = g_rvv[l_ac].rvv02
                 END IF
             ...
             WHEN INFIELD(rvv05) #查詢驗收單項次
               IF g_rvu.rvu00='3' THEN #-- 倉退
                   CALL q_rvb7(FALSE,TRUE,
                               g_rvu.rvu02,g_rvv[l_ac].rvv05,g_rvu.rvu01)
...
#                    CALL cl_dynamic_qry() RETURNING g_qryparam.multiret
                     ...
                     #RETURNING  g_qryparam.multiret           #MOD-B90102 mark
                     RETURNING g_rvv[l_ac].rvviicd04           #MOD-B90102 add
#FUN-AA0059---------mod------------end-----------------
                    #DISPLAY g_qryparam.multiret TO rvviicd04  #MOD-B90102 mark
                    DISPLAY BY NAME g_rvv[l_ac].rvviicd04      #MOD-B90102 add
                    NEXT FIELD rvviicd04
              OTHERWISE EXIT CASE
           END CASE
       ...
       ON ACTION default_w_h
        ...
        ON ACTION modi_lot
           ...
           IF g_ima918 = "Y" OR g_ima921 = "Y" THEN
              ...
              WHERE img01=g_rvv[l_ac].rvv31 AND img02=g_rvv[l_ac].rvv32
                AND img03=g_rvv[l_ac].rvv33 AND img04=g_rvv[l_ac].rvv34
              #IF g_sma.sma90 = 'Y' THEN                            #MOD-C40098 mark
              IF NOT (g_sma.sma90 = 'N' AND g_rvu.rvu00 = '2') THEN #MOD-C40098 add
                 SELECT COUNT(*) INTO l_n FROM rvbs_file
                  WHERE rvbs00 = g_prog
                    AND rvbs01 = g_rvu.rvu01
                    AND rvbs02 = g_rvv[l_ac].rvv02
...
FUNCTION t720_b_move_to()
   ...
   LET g_rvv[l_ac].rvviicd03 = b_rvvi.rvviicd03
   LET g_rvv[l_ac].rvviicd04 = b_rvvi.rvviicd04
   LET g_rvv[l_ac].rvviicd05 = b_rvvi.rvviicd05
   LET g_rvv[l_ac].rvviicd01 = b_rvvi.rvviicd01
   LET g_rvv[l_ac].rvviicd02 = b_rvvi.rvviicd02
   LET g_rvv[l_ac].rvviicd06 = b_rvvi.rvviicd06
    ...
    CALL t720sub_rvv39(b_rvv.rvv36,b_rvv.rvv39,b_rvv.rvv39t,g_rvu.rvu04,g_rvu.rvu01)  #MOD-BC0262 add
     ...
     #MOD-D30016 add start -----
      ...
      CALL cl_digcut(b_rvv.rvv39t,t_azi04)
                        RETURNING b_rvv.rvv39t
     #MOD-D30016 add end   -----
   LET b_rvvi.rvvi01 = g_rvu.rvu01      #TQC-950054
   LET b_rvvi.rvvi02 = g_rvv[l_ac].rvv02#TQC-950054
   LET b_rvvi.rvviicd03 = g_rvv[l_ac].rvviicd03
   LET b_rvvi.rvviicd04 = g_rvv[l_ac].rvviicd04
   LET b_rvvi.rvviicd05 = g_rvv[l_ac].rvviicd05
   LET b_rvvi.rvviicd01 = g_rvv[l_ac].rvviicd01
   LET b_rvvi.rvviicd02 = g_rvv[l_ac].rvviicd02
   LET b_rvvi.rvviicd06 = g_rvv[l_ac].rvviicd06
   IF cl_null(b_rvvi.rvviicd07) THEN
      LET b_rvvi.rvviicd07='N'
   END IF
...
#        END IF  #No.FUN-A50054 Add   #FUN-A60035 ---MARK
        #取得母體料號之品名規格
        IF NOT cl_null(g_rvv[g_cnt].rvviicd03) THEN
           SELECT ima02 INTO g_rvv[g_cnt].rvviicd03_desc
              FROM ima_file
             WHERE ima01 = g_rvv[g_cnt].rvviicd03
        END IF 
        IF NOT cl_null(g_rvv[g_cnt].rvviicd04) THEN
           SELECT ima02 INTO g_rvv[g_cnt].rvviicd04_desc
              FROM ima_file
             WHERE ima01 = g_rvv[g_cnt].rvviicd04
        END IF 
        IF NOT cl_null(g_rvv[g_cnt].rvviicd01) THEN
           SELECT ecd02 INTO g_rvv[g_cnt].ecd02
              FROM ecd_file
             WHERE ecd01 = g_rvv[g_cnt].rvviicd01
        END IF 
        LET g_cnt = g_cnt + 1
...
#@  ON ACTION 刻號BIN維護作業
    ...
    #FUN-B90060 --START--      
    ON ACTION apm_next_wo
       LET g_action_choice="apm_next_wo"
       EXIT DIALOG
    #FUN-B90060 --END--          
...
FUNCTION t720_rvu02()      #驗收單號
   ...
   DEFINE l_rva10   LIKE rva_file.rva10     #FUN-810038
...
#TQC-A60132 --End
   ...
   IF g_argv3='ICD' THEN
      IF cl_null(g_rvu.rvu08) THEN
         ...
         RETURN
      ELSE
         SELECT rva10 INTO l_rva10
           FROM rva_file
          WHERE rva01=g_rvu.rvu02
         IF cl_null(l_rva10) OR (l_rva10<>g_rvu.rvu08) THEN
            LET g_errno = 'aic-117'
            RETURN
         END IF
      END IF
...
FUNCTION t720_rvv05(p_cmd)     # 驗收單項次
  ...
  IF g_rvu.rvu02 != g_rvu_t.rvu02 OR g_rvv[l_ac].rvv05 != g_rvv_t.rvv05 OR
     cl_null(g_rvv_t.rvv05) THEN
       #檢查是否為委外tky非最終站之收貨單據
       #IF g_rvu.rvu00 != '1' AND g_argv3<>'TAP' THEN   #FUN-B90012--mark--
       IF g_rvu.rvu00 != '1' THEN                       #FUN-B90012---add--
          LET g_cnt = 0
          SELECT COUNT(*) INTO g_cnt FROM rvb_file,rvbi_file
            WHERE rvb01 = g_rvu.rvu02
              AND rvb02 = g_rvv[l_ac].rvv05
              AND rvbi01=rvb01
              AND rvbi02=rvb02
       ...
       CASE
        WHEN g_rvu.rvu00='1' OR g_rvu.rvu00='3' #入庫
          ...
          SELECT rvbiicd14,rvbiicd16,rvb331,rvb332,
                 rvbiicd03,rvbiicd08,rvbiicd15,rvbiicd11,rvbiicd13,
             #             內編母體 ,            母批
               g_rvv[l_ac].rvviicd03,g_rvv[l_ac].rvviicd05,l_rvb331,l_rvb332,
             #          ICD作業編號,            Datecode
              g_rvv[l_ac].rvviicd01,g_rvv[l_ac].rvviicd02,
             #             產品型號,            Tape Reel,委外tky非最終站否
              g_rvv[l_ac].rvviicd04,g_rvv[l_ac].rvviicd06,b_rvvi.rvviicd07   #TQC-930128  mark
       ...
       #FUN-B40083 --end
          ...
          IF g_rvu.rvu00='1' THEN
             ...
             IF l_rvb39 = 'Y' THEN      #-->檢驗  no.7143
                ...
                IF l_rvv82 > l_rvb331 THEN
                   LET l_rvv82 = l_rvb331
                END IF
                IF l_rvv85 > l_rvb332 THEN
                   LET l_rvv85 = l_rvb332
                END IF
             ELSE
        ...
        WHEN g_rvu.rvu00='2'     #驗退
         ...
         SELECT rvb03,rvb04,rvb05,rvb919,rvb30,rvb34,rvb35,rvb89,    #FUN-9A0068 add rvb89  #FUN-A80150 add rvb919
                #驗退應為實收(rvb07)-允收(rvb33)
                rvb07 - rvb33,
                rvb10,rvb25,
                rvb89,               #FUN-9A0068 add
                #作業編號,datecode, 母體料號, 產品型號,
                rvbiicd03,rvbiicd08,rvbiicd14,rvbiicd15,
                #母批,   Tap Reel
                rvbiicd16,rvbiicd11,
                rvb80,rvb81,
                #驗退應為實收(rvb82)-允收(rvb331)
                rvb82 - rvb331,
                rvb83,rvb84,
                #驗退應為實收(rvb85)-允收(rvb332)
                rvb85 - rvb332,
                rvb86,rvb87,
                rvb10t,rvb39,rvb40,rvbiicd13
          INTO g_rvv[l_ac].rvv37,g_rvv[l_ac].rvv36,
               g_rvv[l_ac].rvv31,g_rvv[l_ac].rvv919,g_rvv[l_ac].rvv17,g_rvv[l_ac].rvv18, #FUN-A80150 add rvv919
               g_rvv[l_ac].rvv25,g_rvv[l_ac].rvv89,g_inqty,b_rvv.rvv38,    #FUN-9A0068 add rvv89
               g_rvv[l_ac].rvv41,
               g_rvv[l_ac].rvviicd01, g_rvv[l_ac].rvviicd02,
               g_rvv[l_ac].rvviicd03, g_rvv[l_ac].rvviicd04,
               g_rvv[l_ac].rvviicd05, g_rvv[l_ac].rvviicd06,
               l_rvv80,l_rvv81,l_rvv82,l_rvv83,l_rvv84,l_rvv85,
               l_rvv86,l_rvv87,b_rvv.rvv38t,l_rvb39,l_rvb40,b_rvvi.rvviicd07
          FROM rvb_file,rvbi_file
         WHERE rvb01=g_rvu.rvu02 AND rvb02=g_rvv[l_ac].rvv05
           AND rvbi01=rvb01 AND rvbi02=rvb02
         IF SQLCA.sqlcode THEN LET g_errno='mfg3070' RETURN END IF
...
#MOD-B90019 -- end --
        ...
        #ICD作業編號
        IF NOT cl_null(g_rvv[l_ac].rvviicd01) THEN
           SELECT ecd02 INTO g_rvv[l_ac].ecd02 FROM ecd_file
           WHERE ecd01 = g_rvv[l_ac].rvviicd01
        END IF
        #內編母體
        IF NOT cl_null(g_rvv[l_ac].rvviicd03) THEN
           SELECT ima02 INTO g_rvv[l_ac].rvviicd03_desc FROM ima_file
            WHERE ima01 = g_rvv[l_ac].rvviicd03
        END IF
        #產品型號
        IF NOT cl_null(g_rvv[l_ac].rvviicd04) THEN
           SELECT ima02 INTO g_rvv[l_ac].rvviicd04_desc FROM ima_file
            WHERE ima01 = g_rvv[l_ac].rvviicd04
        END IF
      ...
      LET b_rvv.rvv87 = g_rvv[l_ac].rvv87
       ...
       END IF                                                                   
   END IF
   #計算rvv82,rvv85,rvv87,rvv17
   IF g_rvu.rvu00='1' THEN
      CALL t720_ind_icd_def_rvv8x('Y',g_rvu.rvu00)
   ELSE
      CALL t720_ind_icd_def_rvv8x('N',g_rvu.rvu00)
   END IF
   LET g_rvv[l_ac].rvv82 = b_rvv.rvv82
   LET g_rvv[l_ac].rvv85 = b_rvv.rvv85
   LET g_rvv[l_ac].rvv87 = b_rvv.rvv87
   LET g_rvv[l_ac].rvv17 = b_rvv.rvv17
    ...
    ELSE
       ...
       END IF   #MOD-A40166
    END IF
   IF g_argv3 !='SUB' AND g_argv3 !='TAP' THEN    #一般和Wafer的
      #計價數量(rvv87)=  rvv17
      LET g_rvv[l_ac].rvv87 = g_rvv[l_ac].rvv17
      LET b_rvv.rvv87 = g_rvv[l_ac].rvv87
   END IF
     ...
     #END IF
     #-----END MOD-AA0165-----
     IF g_rvu.rvu02 != g_rvu_t.rvu02 OR 
        g_rvv[l_ac].rvv05 != g_rvv_t.rvv05 OR
        cl_null(g_rvv_t.rvv05) THEN
        #委外入庫,重新計算計價數量,金額,單價等值
        CALL t720_ind_icd_upd_rvvxx('b')
     END IF
     ...
     IF g_rvu.rvu00='2' THEN
        ...
        #配合刻號維護要有倉儲批資訊,所以驗退時不需把它們清空
        #直接default收貨項次之倉儲批
        #IF g_argv1='2' AND g_argv3<>'TAP' THEN  #FUN-B90012--mark--
        IF g_argv1='2' THEN                      #FUN-B90012---add--
            SELECT rvb36,rvb37,rvb38 
               INTO b_rvv.rvv32,b_rvv.rvv33,b_rvv.rvv34          
              FROM rvb_file
             WHERE rvb01=g_rvu.rvu02 
               AND rvb02=g_rvv[l_ac].rvv05
...
FUNCTION t720_x(p_type)   #FUN-D20025
  ...
  #IF cl_void(0,0,g_rvu.rvuconf) THEN  #FUN-D20025
   ...
   IF cl_void(0,0,l_flag) THEN         #FUN-D20025
        #MOD-D40137 add start -----
         LET g_cnt = 0
         SELECT COUNT(*) INTO g_cnt FROM ida_file WHERE ida07 = g_rvu.rvu01
         IF g_cnt > 0 THEN
            IF NOT cl_confirm('aic-112') THEN
               CLOSE t720_cl
               ROLLBACK WORK
               RETURN
            ELSE
               CALL s_icdinout_del(0,g_rvu.rvu01,'','') RETURNING l_flag  #FUN-B80119--傳入p_plant參數''---
               IF l_flag = 0 THEN
                  CLOSE t720_cl
                  ROLLBACK WORK
                  RETURN
...
#------- UPDATE -------
   ...
   IF g_success = 'Y' THEN
     ...
     #MESSAGE ""                    #FUN-A50001 mark
      ...
      IF g_success = 'Y' THEN
         #委外tky非最終站收貨項次的數量更新
         CALL t720sub_ind_icd_tky_upd_rvb('z',g_argv3,g_rvu.*,g_argv1) #FUN-A10130
      END IF
      IF g_success = 'Y' THEN
      #FUN-D20059--str--
      ...
      #FUN-D20059--end--
         ...
         FOREACH t720_z_c INTO b_rvv.*
            SELECT * INTO b_rvvi.*
              FROM rvvi_file
             WHERE rvvi01 = b_rvv.rvv01
               AND rvvi02 = b_rvv.rvv02
            #排除委外TKY非最終站的資料
            IF b_rvvi.rvviicd07 = 'Y' THEN CONTINUE FOREACH END IF
 
            LET l_type = NULL
            IF g_argv1='2' THEN #驗退
               LET l_type = 0
            END IF
            IF g_argv1='1' THEN #入庫
               LET l_type = 1
            ...
            IF g_argv1='3' THEN #倉退
               LET l_type = -1
            END IF
            IF l_type = 0 OR l_type = 1  OR l_type = -1 THEN
              #MOD-CC0212---add---S
              #退貨方式為價格折讓，則不做檢核
               IF g_rvu.rvu116='3' THEN
                  LET l_flag = 1
               ...
               ELSE
              #MOD-CC0212---add---E
                  CALL s_icdpost(l_type,b_rvv.rvv31,b_rvv.rvv32,b_rvv.rvv33,
                                        b_rvv.rvv34,b_rvv.rvv35,b_rvv.rvv17,
                                        b_rvv.rvv01,b_rvv.rvv02,
                                        g_rvu.rvu03,'N',b_rvv.rvv04,b_rvv.rvv05
                                        ,b_rvvi.rvviicd05,b_rvvi.rvviicd02,'') #FUN-B30187 #TQC-B80005  #FUN-B80119--傳入p_plant參數''---
                       RETURNING l_flag
               END IF      #MOD-CC0212 add
               IF l_flag = 0 THEN
                  LET g_success = 'N'
                  EXIT FOREACH
               END IF
...
#退貨方式為價格折讓，則不做異動
               IF l_flag2 != 'N' THEN           #MOD-CC0212 add
...
#FUN-A60035 ---MARK END
      ...
      LET g_rvv[l_ac].rvv02=b_rvv.rvv02
      LET g_rvv[l_ac].rvv05=b_rvv.rvv05
      INITIALIZE b_rvvi.* TO NULL
          #  ICD作業編號,Datecode ,內編母體 ,產品型號 ,母批
      SELECT   rvbiicd03,rvbiicd08,rvbiicd14,rvbiicd15,rvbiicd16,
               rvbiicd11,rvbiicd13
        INTO b_rvvi.rvviicd01,b_rvvi.rvviicd02,b_rvvi.rvviicd03,
             b_rvvi.rvviicd04,b_rvvi.rvviicd05,b_rvvi.rvviicd06,
             b_rvvi.rvviicd07                
        FROM rvbi_file
       WHERE rvbi01=b_rvv.rvv04
         AND rvbi02=b_rvv.rvv05
      LET g_rvv[l_ac].rvviicd01 = b_rvvi.rvviicd01
      LET g_rvv[l_ac].rvviicd02 = b_rvvi.rvviicd02
      LET g_rvv[l_ac].rvviicd03 = b_rvvi.rvviicd03
      LET g_rvv[l_ac].rvviicd04 = b_rvvi.rvviicd04
      ...
      IF l_rvb39 = 'Y' THEN          #-->檢驗 #no.7143
         ...
         IF NOT cl_null(l_rvb33) AND l_rvb33 > 0 THEN
           ...
           END IF
           IF b_rvv.rvv82 > l_rvb331 THEN
              LET b_rvv.rvv82 = l_rvb331
           END IF
           IF b_rvv.rvv85 > l_rvb332 THEN
              LET b_rvv.rvv85=l_rvb332
           END IF
         ELSE
            CONTINUE FOREACH
         END IF         
      END IF
      #計算數量rvv82,rvv85,rvv87,rvv18
      CALL t720_ind_icd_def_rvv8x('Y',g_rvu.rvu00)
...
#FUN-AB0066 --end--
    ...
    #當不使用多單位時，單位一的換算率也不default值
      ...
      CALL t720sub_ind_icd_ins_icin(b_rvv.rvv01,b_rvv.rvv02,b_rvv.rvv04, #FUN-A10130
                                    b_rvv.rvv05,g_rvu.rvu00,g_rvu.rvu03)
         
      #委外入庫,重新計算計價數量,金額,單價等值
      CALL t720_ind_icd_upd_rvvxx('g')
      #END IF                    #FUN-B90012--mark--
...
DEFINE   l_ima903  LIKE ima_file.ima903   #MOD-D40048 add
   ...
   CALL cl_set_comp_entry("rvv17,rvv82,rvv85",TRUE)
  #MOD-D40048 add
   IF g_before_input_done THEN
      SELECT ima903 INTO l_ima903 FROM ima_file WHERE ima01 = g_rvv[l_ac].rvv31
      IF g_rvv[l_ac].rvv31[1,4] = 'MISC' OR l_ima903 = 'Y' THEN
...
#FUN-A20017--end---
   ...
   END IF
    ...
    IF b_rvvi.rvviicd07 = 'Y' THEN
       CALL cl_set_comp_entry("rvv32,rvv33,rvv34",FALSE)
       CALL cl_set_comp_entry("rvv17,rvv82,rvv85",FALSE)
    END IF
...
FUNCTION t720_def_form()
  ...
  #FUN-A60009 add end ----
   ...
   #IF g_argv3 = "TAP" THEN
   #   CALL cl_set_act_visible("aic_qry_back,aic_s_icdin,aic_s_icdqry",FALSE)
   #ELSE
   #FUN-B90012--mark---end---
   CALL cl_set_act_visible("aic_qry_back,aic_s_icdin,aic_s_icdqry",TRUE)
   #END IF   #FUN-B90012--mark--
...
FUNCTION t720_s_icdin(p_cmd,p_cmd1)
   ...
   IF g_argv1='3' THEN #倉退
      LET l_type = -1
   END IF
   IF cl_null(l_type) THEN RETURN END IF
 
   IF cl_null(g_rvu.rvu01) THEN RETURN END IF
   IF g_rvu.rvuconf != 'N' THEN RETURN END IF
   IF cl_null(l_ac) OR l_ac = 0 THEN LET l_ac = 1 END IF
   IF p_cmd = 'b' AND (cl_null(p_cmd1) OR p_cmd1 NOT MATCHES '[au]') THEN
      RETURN
   END IF
 
   #若為委外tky非最終站的資料,不需做刻號維護
   SELECT * INTO l_rvvi.* FROM rvvi_file
     WHERE rvv01 = g_rvu.rvu01
       AND rvv02 = g_rvv[l_ac].rvv02
   IF l_rvvi.rvviicd07 = 'Y' THEN RETURN END IF
   ...
   IF p_cmd = 'b' AND
      ...
      #FUN-BA0051 --END mark--
      IF s_icdbin(g_rvv[l_ac].rvv31) THEN   #FUN-BA0051   
         IF NOT cl_confirm('aic-312') THEN  #No.FUN-830078
            RETURN
         END IF
      ELSE
         RETURN
   ...
   IF l_type = 0 OR l_type = 1 THEN
      ##若是未測wafer imaicd04,母批=單身中的批號
      LET l_dies = 0
      CALL s_icdin(l_type,l_rvv.rvv31,l_rvv.rvv32,
                          l_rvv.rvv33,l_rvv.rvv34,
                          l_rvv.rvv35,l_rvv.rvv17,
                          g_rvu.rvu01,l_rvv.rvv02,
                          g_rvu.rvu03,'',
                          l_rvvi.rvviicd03,l_rvv.rvv04,l_rvv.rvv05)
           RETURNING l_dies,l_r,l_qty   #FUN-C30302
           #FUN-C30302---begin
           IF l_r = 'Y' THEN
              LET l_qty = s_digqty(l_qty,l_rvv.rvv35) 
              LET g_rvv[l_ac].rvv17 = l_qty
              LET g_rvv[l_ac].rvv82 = l_qty
              LET g_rvv[l_ac].rvv87 = l_qty
              LET g_rvv[l_ac].rvv39 = cl_digcut(g_rvv[l_ac].rvv87 * l_rvv.rvv38, t_azi04)
              LET g_rvv[l_ac].rvv39t = cl_digcut(g_rvv[l_ac].rvv87 * l_rvv.rvv38t, t_azi04)
              UPDATE rvv_file SET rvv17 = g_rvv[l_ac].rvv17,
                                  rvv82 = g_rvv[l_ac].rvv82,
                                  rvv87 = g_rvv[l_ac].rvv87,
                                  rvv39 = g_rvv[l_ac].rvv39,
                                  rvv39t = g_rvv[l_ac].rvv39t
               WHERE rvv01=g_rvu.rvu01 AND rvv02=g_rvv[l_ac].rvv02
              IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
                 LET g_rvv[l_ac].rvv17 = b_rvv.rvv17
                 LET g_rvv[l_ac].rvv82 = b_rvv.rvv82
                 LET g_rvv[l_ac].rvv87 = b_rvv.rvv87
                 LET g_rvv[l_ac].rvv39 = b_rvv.rvv39
                 LET g_rvv[l_ac].rvv39t = b_rvv.rvv39t
                 LET g_success  = 'N'   
              ELSE
                 LET b_rvv.rvv17  = g_rvv[l_ac].rvv17
                 LET b_rvv.rvv82  = g_rvv[l_ac].rvv82
                 ...
                 LET b_rvv.rvv39t  = g_rvv[l_ac].rvv39t
              END IF
              DISPLAY BY NAME g_rvv[l_ac].rvv17, g_rvv[l_ac].rvv82, g_rvv[l_ac].rvv87,
                              g_rvv[l_ac].rvv39, g_rvv[l_ac].rvv39t
           END IF 
           #FUN-C30302---end
   END IF
   IF l_type = -1 THEN
     CALL s_icdout(l_rvv.rvv31,l_rvv.rvv32,
                   l_rvv.rvv33,l_rvv.rvv34,
                   l_rvv.rvv35,l_rvv.rvv17,
                   g_rvu.rvu01,l_rvv.rvv02,
                   g_rvu.rvu03,
          ...
          #FUN-C30302---end
   END IF
   CALL t720_ind_icd_upd_dies(l_dies)
END FUNCTION
 
#更新die數
#當料號為wafer段時imaicd04=1,用dice數量加總給原將單據的第二單位數量。
#當料號為wafer段時imaicd04=2,用pass bin = 'Y'數量加總給原將單據的第二單位數量。
FUNCTION t720_ind_icd_upd_dies(l_dies)
   DEFINE l_ima906    LIKE ima_file.ima906,
          l_imaicd04  LIKE imaicd_file.imaicd04,
          l_dies      LIKE rvv_file.rvv17,
          l_rvv83     LIKE rvv_file.rvv83   #No.FUN-BB0086
   ...
   IF g_sma.sma115 = 'Y' THEN
      ...
      SELECT ima906,imaicd04 INTO l_ima906,l_imaicd04 
       ...
       WHERE ima01 = imaicd00
         AND ima01 = g_rvv[l_ac].rvv31
      IF NOT cl_null(l_ima906) AND l_ima906 MATCHES '[13]' THEN
         IF l_imaicd04 MATCHES '[012]' THEN
            LET g_rvv[l_ac].rvv85 = l_dies             #數量
            LET g_rvv[l_ac].rvv84 = g_rvv[l_ac].rvv82/g_rvv[l_ac].rvv85
            #No.FUN-BB0086--add--begin--
            SELECT rvv83 INTO l_rvv83 FROM rvv_file 
             WHERE rvv01=g_rvu.rvu01 AND rvv02=g_rvv[l_ac].rvv02
            LET g_rvv[l_ac].rvv85 = s_digqty(g_rvv[l_ac].rvv85,l_rvv83)  
            #No.FUN-BB0086--add--end--
            UPDATE rvv_file set rvv84 = g_rvv[l_ac].rvv84,
                                rvv85 = g_rvv[l_ac].rvv85
             WHERE rvv01=g_rvu.rvu01 AND rvv02=g_rvv[l_ac].rvv02
            IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
               LET g_rvv[l_ac].rvv84 = b_rvv.rvv84
               LET g_rvv[l_ac].rvv85 = b_rvv.rvv85
            ...
            ELSE
               ...
               LET b_rvv.rvv85  = g_rvv[l_ac].rvv85
            END IF
            DISPLAY BY NAME g_rvv[l_ac].rvv84, g_rvv[l_ac].rvv85
         END IF
      END IF
   END IF

   #FUN-BC0109 --START--
   IF s_icdbin(g_rvv[l_ac].rvv31) THEN   
       LET g_sql = "SELECT ida15 FROM ida_file",   
                   " WHERE ida07 = '", g_rvu.rvu01, "'",
                    " AND idb08 =", g_rvv[l_ac].rvv02      
       DECLARE t720_upd_dia_c CURSOR FROM g_sql            
       OPEN t720_upd_dia_c                                       
       FETCH t720_upd_dia_c INTO g_rvv[l_ac].rvviicd05   
       
       #串接Date Code值
       CALL s_icdfun_datecode('2',g_rvu.rvu01,g_rvv[l_ac].rvv02) 
                                RETURNING g_rvv[l_ac].rvviicd02  
       
       
       UPDATE rvvi_file set rvviicd05 = g_rvv[l_ac].rvviicd05,
        rvviicd02 = g_rvv[l_ac].rvviicd02 
        WHERE rvvi01=g_rvu.rvu01 AND rvvi02=g_rvv[l_ac].rvv02
       IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
          LET g_rvv[l_ac].rvviicd02 = b_rvvi.rvviicd02
          LET g_rvv[l_ac].rvviicd05 = b_rvvi.rvviicd05
          LET g_success  = 'N'
       ELSE
          ...
          LET b_rvvi.rvviicd05 = g_rvv[l_ac].rvviicd05          
       END IF 
       DISPLAY BY NAME g_rvv[l_ac].rvviicd02, g_rvv[l_ac].rvviicd05       
    END IF
...
#   料號為參考單位,單一單位,且狀態為其它段生產料號(imaicd04=3,4)時,
#   將單位一的資料給單位二
FUNCTION t720_ind_icd_set_value()
   DEFINE l_ima906    LIKE ima_file.ima906,
          l_imaicd04  LIKE imaicd_file.imaicd04
     ...
     #若料號為單一單位, 如IC料號, 出入庫不回寫單位二級單位二數量
      IF NOT cl_null(l_ima906) AND l_ima906 MATCHES '[3]' THEN
         IF l_imaicd04 MATCHES '[34]' THEN
            LET g_rvv[l_ac].rvv83 = g_rvv[l_ac].rvv80 #單位
            LET g_rvv[l_ac].rvv85 = g_rvv[l_ac].rvv82 #數量
            LET g_rvv[l_ac].rvv84 = g_rvv[l_ac].rvv82/g_rvv[l_ac].rvv85
...
#檢查發料量與所維護之數量是否一致,如果不一致,出錯誤訊息
FUNCTION t720_ind_icd_chk_icdqty()
   #DEFINE l_imaicd04   LIKE imaicd_file.imaicd04,   #FUN-BA0051 mark
   #       l_imaicd08   LIKE imaicd_file.imaicd08    #FUN-BA0051 mark
   DEFINE l_flag       LIKE type_file.num5
 
   #委外tky非最終站的不用檢查
   IF b_rvvi.rvviicd07 = 'Y' THEN RETURN END IF
 
   IF g_argv3='TAP' THEN RETURN END IF
   #FUN-BA0051 --START mark--
   #SELECT imaicd08,imaicd04 INTO l_imaicd08,l_imaicd04
   #  FROM imaicd_file
   # WHERE imaicd00 = g_rvv[l_ac].rvv31
   ...
   #FUN-BA0051 --END mark-- 
   IF s_icdbin(g_rvv[l_ac].rvv31) THEN   #FUN-BA0051
      CALL s_icdchk(1,g_rvv[l_ac].rvv31,g_rvv[l_ac].rvv32,
                       g_rvv[l_ac].rvv33 ,g_rvv[l_ac].rvv34,
                       g_rvv[l_ac].rvv17,g_rvu.rvu01,
                       g_rvv[l_ac].rvv02,g_rvu.rvu03,'')  #FUN-B80119--傳入p_plant參數''---
           RETURNING l_flag
   END IF
END FUNCTION
...
#3-2.其他單價: 比照Package
FUNCTION t720_ind_icd_upd_rvvxx(p_cmd)
   DEFINE l_ecdicd01  LIKE ecd_file.ecdicd01
   DEFINE l_icg27     LIKE icg_file.icg27
   DEFINE l_rvbiicd06 LIKE rvbi_file.rvbiicd06
   DEFINE l_n         LIKE type_file.num5
   DEFINE p_cmd       LIKE type_file.chr1 
   DEFINE l_rvbi      RECORD LIKE rvbi_file.*
 
   IF g_argv3 != 'SUB' THEN RETURN END IF
   IF b_rvv.rvv03 != '1' THEN RETURN END IF
 
   CALL t720_ind_icd_upd_rvv87(p_cmd)
 
   #取得該收貨項次的不良品數
   #若該收貨項次尚未產生過入庫單,則全數納入本張入庫單項次之計價數量
   LET l_n = 0
   LET l_rvbiicd06 = 0
   SELECT COUNT(*) INTO l_n FROM rvv_file,rvu_file
      WHERE rvv01 = rvu01
        AND rvuconf != 'X'
        AND rvv04 = b_rvv.rvv04
        AND rvv05 = b_rvv.rvv05
        AND (rvv01 != g_rvu.rvu01
        OR rvv02 != b_rvv.rvv02)
   IF l_n > 0 THEN
      LET l_rvbiicd06 = 0
   ELSE
      SELECT rvbiicd06 INTO l_rvbiicd06
         FROM rvb_file
        WHERE rvb01 = b_rvv.rvv04
          AND rvb02 = b_rvv.rvv05
   END IF
   IF cl_null(l_rvbiicd06) THEN LET l_rvbiicd06 = 0 END IF
 
   #取得收貨單作業編號之作業群組
   LET l_ecdicd01 = ''
   SELECT ecdicd01,rvbi_file.* INTO l_ecdicd01,l_rvbi.*
     FROM ecd_file,rvb_file,rvbi_file
    WHERE ecd01 = rvbiicd03
      AND rvbi01=rvb01
      AND rvbi02=rvb02
      AND rvbiicd03 IS NOT NULL
      AND rvbiicd03 != ' '
      AND rvb01 = b_rvv.rvv04
      AND rvb02 = b_rvv.rvv05
   ...
   IF l_ecdicd01 = '5' THEN
      ...
      LET b_rvv.rvv39 = (b_rvv.rvv87 + l_rvbiicd06)*b_rvv.rvv38 +   #MOD-AA0165 rvv17-->rvv87
                        (b_rvv.rvv87 * l_icg27)   #MOD-AA0165 rvv17-->rvv87
 
      LET b_rvv.rvv38 = b_rvv.rvv39 / b_rvv.rvv87
   END IF
...
#8.1 CP: 計價數量= 可允收數(rvv17)
#8.2 DS: 計價數量= 可允收數(rvv17)
#8.3 ASS: 計價數量= 可允收數(rvv17)
#8.4 FT: 計價數量= 可允收數(rvv17) + 不良品數(rvbiicd06)
#8.5 TKY:
#8.5.1若作業群組'4.ASS': 可允收數(rvv17) + 不良品數(rvbiicd06)
#8.5.2若作業群組'5.FT': 可允收數(rvv17) + 不良品數(rvbiicd06)
FUNCTION t720_ind_icd_upd_rvv87(p_cmd)
   DEFINE l_ecdicd01     LIKE ecd_file.ecdicd01
   ...
   SELECT ecdicd01 INTO l_ecdicd01_tky
    ...
    WHERE pmni01 = b_rvv.rvv36
      AND pmni02 = b_rvv.rvv37
      AND pmniicd03 IS NOT NULL
      AND pmniicd03 != ' '
      AND pmniicd03 = ecdicd01
   ...
   SELECT ecdicd01 INTO l_ecdicd01
     FROM ecd_file,rvbi_file
    WHERE ecdicd01 = rvbiicd03
      AND rvbiicd03 IS NOT NULL 
      AND rvbiicd03 != ' '
      AND rvbi01 = b_rvv.rvv04
      AND rvbi02 = b_rvv.rvv05
 
   CASE l_ecdicd01
        WHEN '2'  #CP: 計價數量= 可允收數(rvb33)
             LET b_rvv.rvv87 = b_rvv.rvv17
        WHEN '3'  #DS: 計價數量= 可允收數(rvb33)
             LET b_rvv.rvv87 = b_rvv.rvv17
        WHEN '4'  #ASS: 計價數量= 可允收數(rvb33)
             LET b_rvv.rvv87 = b_rvv.rvv17
        WHEN '5'  # FT: 計價數量= 可允收數(rvb33) + 不良品數(rvbiicd06)
             LET b_rvv.rvv87 = b_rvv.rvv17 + l_rvbiicd06
   END CASE
...
FUNCTION t720_ind_icd_def_rvv8x(p_yn,p_type)
   DEFINE l_rvv82_1,l_rvv82_2   LIKE rvv_file.rvv82,
          l_rvv85_1,l_rvv85_2   LIKE rvv_file.rvv85,
          l_rvv87_1,l_rvv87_2   LIKE rvv_file.rvv87,
          l_rvv17_1,l_rvv17_2   LIKE rvv_file.rvv17,
          l_rvb331              LIKE rvb_file.rvb331,
          l_rvb332              LIKE rvb_file.rvb332
   DEFINE p_type                LIKE rvu_file.rvu00    #入庫類別
   DEFINE p_yn                  LIKE type_file.chr1
 #p_type='1':表示rvv17 default值為rvb33(允收數量,不會扣掉已存在其他入庫或退貨的數量),
 #  所以需要在此扣除已存在其他入庫或退貨單項次的數量
 #p_type<>'1':表示rvv17 default值為rvb31(可入庫數量,已扣除存在其他入庫或退貨單項次的數量)
 
   SELECT SUM(rvv82),SUM(rvv85),SUM(rvv87)
     INTO l_rvv82_1,l_rvv85_1,l_rvv87_1
   ...
   SELECT SUM(rvv82),SUM(rvv85),SUM(rvv87)
     INTO l_rvv82_2,l_rvv85_2,l_rvv87_2
     FROM rvv_file,rvu_file
    WHERE rvv01 = g_rvu.rvu01
      AND rvv02 != b_rvv.rvv02
      AND rvv04 = g_rvu.rvu02
      AND rvv05 = b_rvv.rvv05
      AND rvuconf != 'X'
      AND rvu01 = rvv01
   ...
   SELECT SUM(rvv17) INTO l_rvv17_1
     FROM rvv_file,rvu_file
    WHERE rvv01 != g_rvu.rvu01
      AND rvv04 = g_rvu.rvu02
      AND rvv05 = b_rvv.rvv05
   ...
   SELECT SUM(rvv17) INTO l_rvv17_2
     FROM rvv_file,rvu_file
    WHERE rvv01 = g_rvu.rvu01
      AND rvv02 != b_rvv.rvv02
      AND rvv04 = g_rvu.rvu02
   ...
   LET b_rvv.rvv82 = b_rvv.rvv82 - l_rvv82_1 - l_rvv82_2
   LET b_rvv.rvv85 = b_rvv.rvv85 - l_rvv85_1 - l_rvv85_2
   LET b_rvv.rvv87 = b_rvv.rvv87 - l_rvv87_1 - l_rvv87_2
   #Y:表示rvv17 default值為rvb33(允收數量,不會扣掉已存在其他入庫或退貨的數量),
   #  所以需要在此扣除已存在其他入庫或退貨單項次的數量
   IF p_yn='Y' THEN
      LET b_rvv.rvv17 = b_rvv.rvv17 - l_rvv17_1 - l_rvv17_2
   END IF
 
   LET b_rvv.rvv84 = b_rvv.rvv82 / b_rvv.rvv85  #第二單位換算率
 
   #--------計價數量-----------
   #  單據單位        計價單位
   IF b_rvv.rvv35 != b_rvv.rvv86 THEN
      #單位換算
      LET g_factor = 1
      CALL s_umfchk(b_rvv.rvv31,b_rvv.rvv35,b_rvv.rvv86)
           RETURNING g_cnt,g_factor
      IF g_cnt = 1 THEN
         LET g_factor = 1
      END IF
      LET b_rvv.rvv87 = b_rvv.rvv17 * g_factor
      LET b_rvv.rvv87 = s_digqty(b_rvv.rvv87,b_rvv.rvv86)   #No.FUN-BB0086
   ELSE
      LET b_rvv.rvv87 = b_rvv.rvv17
   END IF
END FUNCTION
