...
#--------------No.TQC-610085 end
   ...
   LET l_table = cl_prt_temptable('apmr200',g_sql) CLIPPED
  ...
  #LET g_sql ="pmc01.pmc_file.pmc01,",   #CHI-830006 mark
   ...
   LET l_table1 = cl_prt_temptable('apmr2001',g_sql) CLIPPED
   IF  l_table1 = -1 THEN EXIT PROGRAM END IF
  #LET g_sql ="pmc01.pmc_file.pmc01,",   #CHI-830006 mark
   LET g_sql ="pmg01.pmg_file.pmg01,",   #CHI-830006
              "pmg02.pmg_file.pmg02,",   #CHI-830006 add
              "pmg03.pmg_file.pmg03"
   LET l_table2 = cl_prt_temptable('apmr2002',g_sql) CLIPPED
   IF  l_table2 = -1 THEN EXIT PROGRAM END IF
   #No.FUN-710091  --end  
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL apmr200_tm(0,0)		        # Input print condition
      ELSE CALL apmr200()			# Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
END MAIN
 
FUNCTION apmr200_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01       #No.FUN-580031
   ...
   OPEN WINDOW apmr200_w AT p_row,p_col WITH FORM "apm/42f/apmr200"
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW apmr200_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('apmr200',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW apmr200_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#No.CHI-6A0004-----------End------ 
     ...
     LET l_sql = "SELECT pmc01,pmc03,pmc02,pmc30,pmc05,' ',pmc06, ",
                 ...
                 " FROM pmc_file ,OUTER gea_file, OUTER geb_file",
                 " WHERE pmc_file.pmc06=gea_file.gea01 AND pmc_file.pmc07=geb_file.geb01 AND ",
                 "       (pmcacti = 'Y' OR pmcacti IS NULL) ",
                 " AND ",tm.wc ,
                 " ORDER BY 3,1 "
 
     PREPARE apmr200_prepare  FROM l_sql
     IF SQLCA.sqlcode != 0 THEN CALL cl_err('prepare:',SQLCA.sqlcode,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
        EXIT PROGRAM 
     END IF
     DECLARE apmr200_cs  CURSOR FOR apmr200_prepare
 
    #CALL cl_outnam('apmr200') RETURNING l_name #No.FUN-710091
     #-----No.MOD-650056-----
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'apmr200'
     IF g_len = 0 OR g_len IS NULL THEN LET g_len = 80 END IF
     FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR
     #-----No.MOD-650056 END-----
    #START REPORT apmr200_rep TO l_name  #No.FUN-710091
     LET g_pageno = 0
 
     FOREACH apmr200_cs INTO sr.*
       ...
       END IF
      #    OUTPUT TO REPORT apmr200_rep(sr.*)   #No.FUN-710091  
      #No.FUN-710091  --begin
      ...
      LET l_sqlr1 = "SELECT pmc081,pmc082,pmc091,pmc092,pmc093, ",
                    ...
                    " WHERE pmc01= ? AND pmc_file.pmc17=pma_file.pma01 ",
                    "   AND pmc_file.pmc55=nmt_file.nmt01"
      ...
      LET l_sqlr3 = "SELECT pme031,pme032,pme033,pme034,pme035 ",
                    "  FROM pme_file ",
                    " WHERE pme01= ? "
      PREPARE apmr200_psr1   FROM l_sqlr1
      IF SQLCA.sqlcode != 0  AND SQLCA.sqlcode !=100
           THEN CALL cl_err('prepare:',SQLCA.sqlcode,1) 
           CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
           EXIT PROGRAM 
           END IF
      DECLARE apmr200_psrt1  CURSOR FOR apmr200_psr1
 
       OPEN apmr200_psrt1  USING sr.pmc01
       FETCH apmr200_psrt1  INTO t1.*
      ...
      PREPARE apmr200_pr3  FROM l_sqlr31
      IF SQLCA.sqlcode != 0 AND SQLCA.sqlcode !=100
          THEN CALL cl_err('prepare:',SQLCA.sqlcode,1) 
          CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
          EXIT PROGRAM 
          END IF
      DECLARE apmr200_prt3  CURSOR FOR apmr200_pr3
      IF t1.pmc15 IS NOT NULL THEN
       OPEN apmr200_prt3  USING t1.pmc15
       FETCH apmr200_prt3  INTO t2.*
      END IF
      CLOSE apmr200_prt3
 
      #帳單地址
      LET l_sqlr32=l_sqlr3 CLIPPED," AND (pme02='1' OR pme02='2') "
      PREPARE apmr200_pr4   FROM l_sqlr32
      IF SQLCA.sqlcode != 0 AND SQLCA.sqlcode != 100
          THEN CALL cl_err('prepare:',SQLCA.sqlcode,1) 
          CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
          EXIT PROGRAM 
          END IF
      DECLARE apmr200_prt4  CURSOR FOR apmr200_pr4
      IF t1.pmc15 IS NOT NULL THEN
       OPEN apmr200_prt4  USING t1.pmc16
       FETCH apmr200_prt4  INTO t3.*
      END IF
      CLOSE apmr200_prt4
  ...
  #   聯絡人
      ...
      PREPARE apmr200_pr5   FROM l_sqlr4
      IF SQLCA.sqlcode != 0  AND SQLCA.sqlcode != 100
          THEN CALL cl_err('prepare:',SQLCA.sqlcode,1) 
          CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
          EXIT PROGRAM 
          END IF
      DECLARE apmr200_prt5  CURSOR FOR apmr200_pr5
      FOREACH apmr200_prt5 INTO t4.*
           EXECUTE insert_prep1 USING sr.pmc01,t4.*
      END FOREACH
  
      DECLARE apmr200_prt6  CURSOR FOR
         SELECT pmg03,pmg02                       #CHI-830006 add pmg02
           FROM pmg_file  WHERE pmg01=sr.pmc01
      FOREACH apmr200_prt6 INTO l_pmg03,l_pmg02   #CHI-830006 add l_pmg02
           EXECUTE insert_prep2 USING sr.pmc01,l_pmg02,l_pmg03  #CHI-830006 add l_pmg02
      END FOREACH
      EXECUTE insert_prep USING sr.*,t1.*,t2.*,t3.*,"",l_img_blob, "N",""      #No.FUN-710091  #TQC-C10039 ADD "",l_img_blob, "N",""
      #No.FUN-710091  --end  
   ...
   ##TQC-730088# "  FROM ",l_table CLIPPED," A,",
     ...
     LET g_str = tm.wc,";",tm.state
   # CALL cl_prt_cs3("apmr200",g_sql,g_str)   #TQC-730088
     LET g_cr_table = l_table                 #主報表的temp table名稱  #TQC-C10039 
     LET g_cr_apr_key_f = "pmc01"       #報表主鍵欄位名稱  #TQC-C10039

     CALL cl_prt_cs3("apmr200",'apmr200',g_sql,g_str)
     #No.FUN-710091  --end  
 
    #FINISH REPORT apmr200_rep   #No.FUN-710091  
 
    #CALL cl_prt(l_name,g_prtway,g_copies,g_len)  #No.FUN-710091 
END FUNCTION
#No.FUN-710091  --begin
#REPORT apmr200_rep(sr)
...
#      FOREACH apmr200_prt6 INTO l_pmg03
