...
# Modify.........: No.FUN-B40092 11/06/06 By xujing 憑證報表轉GRW
...
#No.FUN-830152---Begin                                                                                                              
   ...
   LET l_table = cl_prt_temptable('apmg650',g_sql) CLIPPED                                                                          
   IF l_table = -1 THEN
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW g650_w AT p_row,p_col WITH FORM "apm/42f/apmg650"
...
#   IF INT_FLAG THEN
#      LET INT_FLAG = 0 CLOSE WINDOW g650_w 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
#      EXIT PROGRAM
#         
#   END IF
...
#No.FUN-B20054--addend--
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('apmg650',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW g650_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      CALL cl_gre_drop_temptable(l_table)
...
#                 "   AND ",tm.wc CLIPPED
     LET l_sql = " SELECT afb_file.*,afc_file.*,afa02  FROM afc_file,afb_file ",             #FUN-C50003 add afa02
                 "   LEFT OUTER JOIN afa_file ON afa01=afb01 AND afa00 ='",g_aza.aza81,"'",  #FUN-C50003 add
     ...
     PREPARE g650_prepare  FROM l_sql
     IF SQLCA.sqlcode != 0 THEN CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
        CALL cl_gre_drop_temptable(l_table)
       EXIT PROGRAM
     END IF
     DECLARE g650_cs  CURSOR FOR g650_prepare
#     CALL cl_outnam('apmg650') RETURNING l_name      #No.FUN-830152  
#     START REPORT g650_rep TO l_name                 #No.FUN-830152  
     FOREACH g650_cs INTO sr.*,l_afa02                #FUN-C50003 add l_afa02
...
#No.FUN-830152---Begin   
     ...
     #FUN-C50003---mark----end----
      ...
      EXECUTE insert_prep  USING   sr.afb.afb00,sr.afb.afb01,sr.afb.afb02,sr.afb.afb03,
                                   ...
                                   l_afa02,l_aag02,l_gem02,l_pja02,                 
                                   "",        l_img_blob,    "N",           ""    #FUN-C40019 add
#No.FUN-830152---End 
#       OUTPUT TO REPORT g650_rep(sr.*)
     END FOREACH
#No.FUN-830152---Begin  
      ...
      IF g_zz05 = 'Y' THEN                                                                                                          
         CALL cl_wcchp(tm.wc,'afb00,afb03,afb01,afb02,afb041,afb042,afb04')                                                                             
              RETURNING tm.wc                                                                                                       
      END IF                                                                                                                        
###GENGRE###     LET g_str=g_azi04,";",g_azi05,";",tm.wc 
###GENGRE###     LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED                                                             
###GENGRE###     CALL cl_prt_cs3('apmg650','apmg650',l_sql,g_str)  
    LET g_cr_table = l_table                   #主報表的temp table名稱          #FUN-C40019 add
    LET g_cr_apr_key_f = "afb00"         #報表主鍵欄位名稱，用"|"隔開     #FUN-C40019 add
    CALL apmg650_grdata()    ###GENGRE###
#     FINISH REPORT g650_rep
#     ERROR ' '
...
#           COLUMN g_c[33],cl_numfor(GROUP SUM(sr.afc.afc08),33,g_azi05),
#           COLUMN g_c[34],cl_numfor(GROUP SUM(sr.afc.afc09),34,g_azi05),
#           COLUMN g_c[35],cl_numfor(GROUP SUM(sr.afc.afc07),35,g_azi05)
#  ON LAST ROW
#     PRINT g_dash
#     LET l_last_sw = 'y'
##     PRINT g_x[4],g_x[5] CLIPPED, COLUMN g_c[35], g_x[7] CLIPPED      #No.TQC-6B0095
#     PRINT g_x[4],g_x[5] CLIPPED, COLUMN g_len-9, g_x[7] CLIPPED      #No.TQC-6B0095
 
#  PAGE TRAILER
#     IF l_last_sw = 'n'
#        THEN PRINT g_dash
##             PRINT g_x[4],g_x[5] CLIPPED, COLUMN g_c[35], g_x[6] CLIPPED      #No.TQC-6B0095
...
FUNCTION apmg650_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("apmg650")
        IF handler IS NOT NULL THEN
            START REPORT apmg650_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY afb00,afb03,afb01,afb02,afb041,afb042,afb04,afc05"    #FUN-B40092    
            DECLARE apmg650_datacur1 CURSOR FROM l_sql
            FOREACH apmg650_datacur1 INTO sr1.*
                OUTPUT TO REPORT apmg650_rep(sr1.*)
            END FOREACH
            FINISH REPORT apmg650_rep
        END IF
...
REPORT apmg650_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B40092------add------str
    DEFINE l_afc06_fmt      STRING
    DEFINE l_afc07_fmt      STRING
    DEFINE l_afc08_fmt      STRING
    DEFINE l_afc09_fmt      STRING
    DEFINE l_afc06_sum_fmt      STRING
    DEFINE l_afc07_sum_fmt      STRING
    DEFINE l_afc08_sum_fmt      STRING
    DEFINE l_afc09_sum_fmt      STRING
    DEFINE l_afc06_sum LIKE afc_file.afc06
    DEFINE l_afc07_sum LIKE afc_file.afc07
    DEFINE l_afc08_sum LIKE afc_file.afc08
    DEFINE l_afc09_sum LIKE afc_file.afc09
    #FUN-B40092------add------end

    
    ORDER EXTERNAL BY sr1.afb00,sr1.afb03,sr1.afb01,sr1.afb02,sr1.afb041,sr1.afb042,sr1.afb04,sr1.afc05
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.afb00
        BEFORE GROUP OF sr1.afb03
        BEFORE GROUP OF sr1.afb01
        BEFORE GROUP OF sr1.afb02
        BEFORE GROUP OF sr1.afb041
        BEFORE GROUP OF sr1.afb042      
        BEFORE GROUP OF sr1.afb04
            LET l_lineno = 0
        BEFORE GROUP OF sr1.afc05

        
        ON EVERY ROW
            LET l_afc06_fmt = cl_gr_numfmt('afc_file','afc06',g_azi04)
            PRINTX l_afc06_fmt
            LET l_afc07_fmt = cl_gr_numfmt('afc_file','afc07',g_azi04)
            PRINTX l_afc07_fmt
            LET l_afc08_fmt = cl_gr_numfmt('afc_file','afc08',g_azi04)
            PRINTX l_afc08_fmt
            LET l_afc09_fmt = cl_gr_numfmt('afc_file','afc09',g_azi04)
            PRINTX l_afc09_fmt
        ...
        AFTER GROUP OF sr1.afb00
        AFTER GROUP OF sr1.afb03
        AFTER GROUP OF sr1.afb01
        AFTER GROUP OF sr1.afb02
        AFTER GROUP OF sr1.afb041
        AFTER GROUP OF sr1.afb042
        AFTER GROUP OF sr1.afb04
        #FUN-B40092------add------str
        LET l_afc06_sum = GROUP SUM(sr1.afc06)
        LET l_afc07_sum = GROUP SUM(sr1.afc07)
        LET l_afc08_sum = GROUP SUM(sr1.afc08)
        LET l_afc09_sum = GROUP SUM(sr1.afc09)
        PRINTX l_afc06_sum
        PRINTX l_afc07_sum
        PRINTX l_afc08_sum
        PRINTX l_afc09_sum
        LET l_afc09_sum_fmt = cl_gr_numfmt('afc_file','afc09',g_azi05)
            PRINTX l_afc09_sum_fmt
        LET l_afc08_sum_fmt = cl_gr_numfmt('afc_file','afc08',g_azi05)
            PRINTX l_afc08_sum_fmt
        LET l_afc07_sum_fmt = cl_gr_numfmt('afc_file','afc07',g_azi05)
            PRINTX l_afc07_sum_fmt
        LET l_afc06_sum_fmt = cl_gr_numfmt('afc_file','afc06',g_azi05)
            PRINTX l_afc06_sum_fmt
        #FUN-B40092------add------end
        AFTER GROUP OF sr1.afc05
