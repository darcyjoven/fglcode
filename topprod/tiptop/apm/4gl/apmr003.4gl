...
# Modify.........: No.TQC-D20019 13/02/18 By xuxz 添加幫助按鈕的ACTION段代碼
...
MAIN
   ...
   LET l_table = cl_prt_temptable('apmr003',g_sql) CLIPPED
  ...
  #變動表頭 
   ...
   LET l_table1 = cl_prt_temptable('apmr0031',g_sql) CLIPPED
  ...
  #總計銷貨
   ...
   LET l_table2 = cl_prt_temptable('apmr0032',g_sql) CLIPPED
   IF  l_table2 = -1 THEN EXIT PROGRAM END IF
 
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      THEN CALL apmr003_tm(0,0)            # Input print condition
      ELSE CALL apmr003()                  # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80088--add--
END MAIN
 
FUNCTION apmr003_tm(p_row,p_col)
   ...
   OPEN WINDOW apmr003_w AT p_row,p_col
        WITH FORM "apm/42f/apmr003"
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_rep_clas CLIPPED,"'",                         
                         " '",g_template CLIPPED,"'"                          
         CALL cl_cmdat('apmr003',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW apmr003_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80088--add--
      EXIT PROGRAM
...
FUNCTION apmr003()
   ...
   DEFINE l_poz02      LIKE poz_file.poz02 
     ...
     DROP TABLE apmr003_tmp
#No.FUN-A10098 -----mark start
#     CREATE TEMP TABLE apmr003_tmp
...
#No.FUN-A10098 -----add end
     ...
     DELETE FROM apmr003_tmp
 
     #紀錄AFTER GROUP OF 流程代碼的總計值 STR
     DROP TABLE apmr003_tmp2
#No.FUN-A10098 -----mark start
#     CREATE TEMP TABLE apmr003_tmp2
#     (
#      flow       VARCHAR(10),    #流程
...
#No.FUN-A10098 -----add end
    ...
    #CR
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'apmr003'
...
#            DECLARE poz02_get CURSOR FROM l_sql                             #FUN-A10098--mark
        ...
        #佣金第二筆 END
         ...
         FOR l_i = 1 TO g_rec_plant
            #merits add
             ...
             END CASE
            #寫入CR
            EXECUTE insert_prep USING sr_dy[l_i].* 
 
            #總計在AFTER GROUP OF flow STR
            IF l_i = g_rec_plant THEN
 
            INSERT INTO apmr003_tmp2 VALUES (
               sr_dy[l_i].flow,
               sr_dy[l_i].flow_sn,
               sr_dy[l_i].ds_sn,
         ...
         FOR g_rec_b1 = 1 TO g_rec_plant           #最多只有六個拋轉db
             IF g_rec_b1=1 THEN 
                ...
                FOREACH r003_cs4 INTO l_flow,l_ds_sn,l_desc,l_oma23,
                  ...
                  SELECT DISTINCT azi01 INTO l_aza17 FROM apmr003_tmp2 WHERE flow=l_poz01
   ...
   #CR
    ...
    LET g_str =l_wc,";",tm.bdate,";",tm.edate        
    
    CALL cl_prt_cs3('apmr003','apmr003',g_sql,g_str)
