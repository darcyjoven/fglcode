...
# Modify.........: No.FUN-CB0002 12/11/08 By lujh CR轉XtraGrid
...
END GLOBALS
   ...
   DEFINE tm  RECORD                      # Print condition RECORD
       ...
       #u       LIKE type_file.chr3,      # Group total sw    #No.FUN-680136 VARCHAR(3) #FUN-D30070 mark
...
MAIN
   ...
   LET g_sql = " l_str.type_file.chr1000,",
               ...
               " l_str3.type_file.chr1000,",   #FUN-CB0002  add
               " l_str4.type_file.chr1000,",   #FUN-CB0002  add
               " l_str5.type_file.chr1000"     #FUN-CB0002  add
   LET l_table = cl_prt_temptable('apmx899',g_sql) CLIPPED                                                                          
   IF l_table = -1 THEN EXIT PROGRAM END IF                                                                                         
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,#No.TQC-820008 
               " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"    #FUN-CB0002  add  ?,?,?
  ...
  #LET tm.u  = ARG_VAL(10) #FUN-D30070 mark
   LET tm.exm= ARG_VAL(10) #FUN-D30070 mod 11->10
   LET tm.d  = ARG_VAL(11) #FUN-D30070 mod 12->11
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      THEN CALL x899_tm(0,0)        # Input print condition
      ELSE CALL apmx899()        # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
END MAIN
 
FUNCTION x899_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01      #No.FUN-580031
   ...
   OPEN WINDOW x899_w AT p_row,p_col WITH FORM "apm/42f/apmx899"
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW x899_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      EXIT PROGRAM
...
#UI
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('apmx899',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW x899_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      EXIT PROGRAM
...
---------------}
     ...
     PREPARE x899_prepare1 FROM l_sql
     IF STATUS != 0 THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
        EXIT PROGRAM 
     END IF
     DECLARE x899_cs1 CURSOR FOR x899_prepare1
     IF STATUS != 0 THEN CALL cl_err('declare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
        EXIT PROGRAM 
     END IF
     LET l_name = 'apmx899.out'
     ...
     LET  l_total1 = 0
    #FOREACH x899_cs1 INTO sr.*,l_pmn18,l_pmn41,l_pmn43,l_pmn012  #CHI-8C0017 Add l_pmn18,l_pmn41,l_pmn43  #FUN-A60027 add l_pmn012 #MOD-BA0046 mark
     FOREACH x899_cs1 INTO sr.*,l_pmn18,l_pmn41,l_pmn43,l_pmn012,l_rvb051  #MOD-BA0046
      ...
      END IF
       ...
       EXECUTE insert_prep USING l_str,sr.rva01,sr.rva06,sr.rva05,
                                 ...
                                 sr.rvb04,sr.rvb03,'','',''  #FUN-CB0002  add '','',''
     END FOREACH
    #FUN-CB0002--mark--str--
    #IF g_zz05 = 'Y' THEN                                                        
    ...
    #END IF
    #FUN-CB0002--mark--end--
###XtraGrid###   LET g_str = tm.wc,";",tm.t,";",g_sma115,";",tm.u[1,1],";",tm.u[2,2],";",tm.u[3,3],";",
###XtraGrid###               tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",tm.exm
###XtraGrid###   LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED   
###XtraGrid###   CALL cl_prt_cs3('apmx899','apmx899',l_sql,g_str)   
    LET g_xgrid.table = l_table    ###XtraGrid###
    IF g_zz05 = 'Y' THEN                                                        
       CALL cl_wcchp(tm.wc,'rva01,rva06,rva05,rvb04,rvb05')         
            RETURNING tm.wc                                                                                                        
    END IF
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"rva01,rva06,rva05,rvb04,rvb05")
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"rva01,rva06,rva05,rvb04,rvb05")
   #LET g_xgrid.grup_sum_field = cl_get_sum_field(tm.s,tm.u,"rva01,rva06,rva05,rvb04,rvb05") #FUN-D30070 mark
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"rva01,rva06,rva05,rvb04,rvb05")
   #LET l_str = cl_wcchp(g_xgrid.order_field,"rva01,rva06,rva05,rvb04,rvb05") #FUN-D30070 mark
