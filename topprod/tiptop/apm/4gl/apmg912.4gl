...
# Modify.........: No:FUN-CB0073 12/11/22 By chenjing CR轉GR
...
###GENGRE###START
TYPE sr1_t RECORD
    order1 LIKE type_file.chr20,    #FUN-CB0073
    order2 LIKE type_file.chr20,    #FUN-CB0073
    order3 LIKE type_file.chr20,    #FUN-CB0073
    rva05 LIKE rva_file.rva05,
    pmc03 LIKE pmc_file.pmc03,
    pmc091 LIKE pmc_file.pmc091,
    endday LIKE rva_file.rva06,
...
MAIN
   ...
   ELSE 
   #FUN-C30285---end
      LET g_sql = "order1.type_file.chr20,",       #FUN-CB0073
                  "order2.type_file.chr20,",       #FUN-CB0073
                  "order3.type_file.chr20,",       #FUN-CB0073
                  ...
                  "l_count.type_file.num5"   #No.FUN-A80030
   END IF        #FUN-C30285
   LET l_table = cl_prt_temptable('apmg912',g_sql) CLIPPED                                                                          
   ...
   LET l_table1 = cl_prt_temptable('apmg9121',g_sql) CLIPPED
   ...
   CALL cl_gre_drop_temptable(l_table||"|"||l_table1)    #FUN-CB0073
END MAIN
 
FUNCTION g912_tm(p_row,p_col)
DEFINE p_row,p_col    LIKE type_file.num5          
DEFINE l_cmd          LIKE type_file.chr1000       
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   
 
   LET p_row = 3 LET p_col = 11
   OPEN WINDOW g912_w AT p_row,p_col WITH FORM "apm/42f/apmg912"
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         ...
         CLOSE WINDOW g912_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table||"|"||l_table1)          #FUN_CB0073
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,
                        ...
                        " '",g_rep_clas CLIPPED,"'", 
                        " '",g_template CLIPPED,"'" 
            CALL cl_cmdat('apmg912',g_time,l_cmd)
         END IF
...
DEFINE l_order       ARRAY[3] OF LIKE type_file.chr20   #FUN-CB0073
DEFINE order1    LIKE type_file.chr20,   #FUN-CB0073
       order2    LIKE type_file.chr20,   #FUN-CB0073
       order3    LIKE type_file.chr20    #FUN-CB0073
DEFINE sr        RECORD
...
DEFINE l_rvviicd01 LIKE rvvi_file.rvviicd01  #FUN-C30285
   ...
   #FUN-C30285---end
      LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,                                                                  
                  " VALUES(?,?,?,?,?, ?,?,?,?,?, ",
                  "        ?,?,?,?,?, ?,?,?,?,?, ",
                  "        ?,?,?,?,?, ?,?,?,?,?, ?,?,? ,?,?,?) "    #No.FUN-A80030 加两個?   #FUN-AA0081 add ? #FUN-CB0073 add 3?
   END IF #FUN-C30285
   ...
   #FUN-C30285----begin
   IF s_industry("icd") THEN
      LET l_sql = "SELECT rvu00,rva05,pmc03,pmc901,'',pmm20,pma02,pmm21,gec02,pmm22, ",
   ...
   IF s_industry("icd") THEN
      LET l_sql = l_sql,
                  "UNION SELECT rvu00,rva05,pmc03,pmc901,'',rvu111,pma02,rvu115,gec02,rvu113, ",
                  "       rva06,rvv04,rvv01,rvb22,rvv36,rvv31,ima021,rvv031,rvb07,rvv17, ",
                  "       rvv35,rvv23,rvv38,rvv38t,rvv39,rvv39t,0,0,0,rvv02,'N',rvv34,rvviicd01 ",  #FUN-C30285
                  "  FROM rva_file ", 
                  "  INNER JOIN rvb_file ON rva_file.rva01 = rvb_file.rvb01",
   ...
   PREPARE g912_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g912_curs1 CURSOR FOR g912_prepare1
   #FUN-C30285---begin
   IF s_industry("icd") THEN  
      LET l_name='apmg912_1' 
   ELSE 
   #FUN-C30285---end 
      LET l_name='apmg912'
   END IF   #FUN-C30285

   FOREACH g912_curs1 INTO sr.*,l_flag,l_rvv34,l_rvviicd01   #CHI-AC0007 add l_flag #FUN-C30285 add l_rvv34,l_rvviicd01
...
#FUN-CB0073--add--str--
         FOR g_i = 1 TO 3
            CASE WHEN tm.s[g_i,g_i] = '1' LET l_order[g_i] = sr.rvv04
                 WHEN tm.s[g_i,g_i] = '2' LET l_order[g_i] = sr.rva05
                 WHEN tm.s[g_i,g_i] = '3' LET l_order[g_i] = sr.rva06 USING "yyyy/mm/dd" 
                 WHEN tm.s[g_i,g_i] = '4' LET l_order[g_i] = sr.rvv36
                 WHEN tm.s[g_i,g_i] = '5' LET l_order[g_i] = sr.rvv31
                 WHEN tm.s[g_i,g_i] = '6' LET l_order[g_i] = sr.rvv01
                 WHEN tm.s[g_i,g_i] = '7' LET l_order[g_i] = sr.rvb22
                 OTHERWISE LET l_order[g_i] = '-'
            END CASE
         END FOR          
         LET order1 = l_order[1]
         LET order2 = l_order[2]
         LET order3 = l_order[3]
#FUN-CB0073--add--end--
      #FUN-C30285---begin
      IF s_industry("icd") THEN
      ...
      ELSE 
      #FUN-C30285---end
         EXECUTE insert_prep USING order1,order2,order3,    #FUN-CB0073 
                 sr.rva05,sr.pmc03,sr.pmc091,tm.endday,sr.pmm20,
                 sr.pma02,sr.pmm21,sr.gec02,sr.pmm22,sr.rva06,
                 sr.rvv04,sr.rvv01,sr.rvb22,sr.rvv36,sr.rvv31,sr.ima021,	
                 sr.rvv031,sr.rvb07,sr.rvv17,sr.rvb80,sr.rvv23,sr.rvv38,   #FUN-AA0081 add sr.rvv17
   ...
   END FOREACH
      IF g_zz05 = 'Y' THEN 
         CALL cl_wcchp(tm.wc,'rva01,rva05,rva06,rvb04,rvb05,rvv01,rvb22') #CHI-AC0007 mod rvw01->rvb22
              RETURNING tm.wc    
      END IF
###GENGRE###      LET g_str=tm.wc ,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",
###GENGRE###                           tm.endday,";",tm.c
###GENGRE###   LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED,"|",
###GENGRE###               "SELECT DISTINCT * FROM ", g_cr_db_str CLIPPED,l_table1 CLIPPED

###GENGRE###   CALL cl_prt_cs3('apmg912',l_name,l_sql,g_str) 
    CALL apmg912_grdata()    ###GENGRE###
...
FUNCTION apmg912_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("apmg912")
        IF handler IS NOT NULL THEN
            START REPORT apmg912_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY rva05 asc nulls first,pmm22,pmm21,order1,order2,order3,rvv31" #FUN-CB0073 by yangtt 
          
            DECLARE apmg912_datacur1 CURSOR FROM l_sql
            FOREACH apmg912_datacur1 INTO sr1.*
                OUTPUT TO REPORT apmg912_rep(sr1.*)
            END FOREACH
            FINISH REPORT apmg912_rep
        END IF
...
#FUN-CB0073--add--str--
    DEFINE l_rvv39        LIKE rvv_file.rvv39,
           ...
           l_rvv39_fmt    STRING,
           l_rvv39t_fmt   STRING,
           l_rvv39_1      LIKE rvv_file.rvv39,
           l_rvv39t_1     LIKE rvv_file.rvv39t,
           l_rvv39_1_fmt  STRING,
           l_rvv39t_1_fmt STRING
    DEFINE l_rvv38_fmt    STRING    
    DEFINE l_rvv38t_fmt   STRING    
    DEFINE l_display     LIKE type_file.chr1,
           l_display1    LIKE type_file.chr1,
           l_display2    LIKE type_file.chr2,
           l_order       STRING
    DEFINE l_amt       LIKE rvv_file.rvv38,
           l_no_amt    LIKE rvv_file.rvv38
#FUN-CB0073--add--end--

    
    ORDER EXTERNAL BY sr1.rva05,sr1.pmm22,sr1.pmm20,sr1.pmm21,sr1.order1,sr1.order2,sr1.order3,sr1.rvv31
    ...
    FORMAT
        ...
        #FUN-CB0073--add--str--
           LET l_rvv38_fmt = cl_gr_numfmt('rvv_file','rvv38',sr1.azi03)
           LET l_rvv38t_fmt = cl_gr_numfmt('rvv_file','rvv38t',sr1.azi03)
           PRINTX l_rvv38_fmt
           PRINTX l_rvv38t_fmt
        #FUN-CB0073--add--end--
            PRINTX sr1.*
        ...
        #FUN-CB0073--add--str--
            ...
            START REPORT apmg912_subrep01
            DECLARE apmg912_repcur1 CURSOR FROM l_sql
            FOREACH apmg912_repcur1 INTO sr2.*
                OUTPUT TO REPORT apmg912_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT apmg912_subrep01

        #FUN-CB0073--add--end--
        AFTER GROUP OF sr1.pmm21
    #FUN-CB0073--add--str--
         LET l_rvv39 = 0
         LET l_rvv39_1 = 0
         LET l_rvv39t = 0
         LET l_rvv39t_1 = 0
         IF cl_null(sr1.rvb22) THEN 
            LET l_rvv39 = GROUP SUM(sr1.rvv39) 
            LET l_rvv39t = GROUP SUM(sr1.rvv39t) 
            LET l_rvv39_fmt = cl_gr_numfmt('rvv_file','rvv39',sr1.azi05)
            LET l_rvv39_fmt = cl_replace_str(l_rvv39_fmt,"&.","#.")
            LET l_rvv39t_fmt = cl_gr_numfmt('rvv_file','rvv39t',sr1.azi05)
            LET l_rvv39t_fmt = cl_replace_str(l_rvv39t_fmt,"&.","#.")
            LET l_no_amt = GROUP SUM(sr1.amt)
         ELSE
            LET l_rvv39_1 = GROUP SUM(sr1.rvv39)
            LET l_rvv39t_1 = GROUP SUM(sr1.rvv39t)
            LET l_rvv39_1_fmt = cl_gr_numfmt('rvv_file','rvv39',sr1.azi05)
            LET l_rvv39_1_fmt = cl_replace_str(l_rvv39_1_fmt,"&.","#.")
            LET l_rvv39t_1_fmt = cl_gr_numfmt('rvv_file','rvv39t',sr1.azi05)
            LET l_rvv39t_1_fmt = cl_replace_str(l_rvv39t_1_fmt,"&.","#.")
            LET l_amt = GROUP SUM(sr1.amt)
         END IF
            PRINTX l_rvv39,l_rvv39t,l_rvv39_fmt,l_rvv39t_fmt
            PRINTX l_rvv39_1,l_rvv39t_1,l_rvv39_1_fmt,l_rvv39t_1_fmt
            PRINTX l_no_amt
            PRINTX l_amt
         ...
         ELSE
            LET l_display1 = 'N'
         END IF 
         IF NOT cl_null(l_no_amt) THEN 
            LET l_display2 = 'Y'
         ELSE
            LET l_display2 = 'N'
         END IF
         PRINTX l_display1,l_display2
         
         LET l_order = cl_gr_getmsg('gre-319',g_lang,tm.s[1,1]),'-',cl_gr_getmsg('gre-319',g_lang,tm.s[2,2]),
                                    '-',cl_gr_getmsg('gre-319',g_lang,tm.s[3,3])
         PRINTX l_order
