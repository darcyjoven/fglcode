...
#No.FUN-710091  --begin
   ...
   DEFINE g_sql         STRING
   DEFINE g_str         STRING
   DEFINE g_tot         LIKE type_file.num5   #FUN-B50018 
#No.FUN-710091  --end  
###GENGRE###START
TYPE sr1_t RECORD
    ...
    g_pmn LIKE pmn_file.pmn31,
    pmm43 LIKE pmm_file.pmm43,
    pmmprno LIKE pmm_file.pmmprno,
    l_pmn31_pmn20 LIKE pmn_file.pmn31,        #FUN-B50018
    ...
    l_pmm13_gem02 LIKE type_file.chr50,
    l_pmm15_gen02 LIKE type_file.chr50,
    l_pmm12_gen02 LIKE type_file.chr50,
...
TYPE sr2_t RECORD
    pmm01 LIKE pmm_file.pmm01,
    pmo06_1 LIKE pmo_file.pmo06
END RECORD

TYPE sr3_t RECORD
    pmm01 LIKE pmm_file.pmm01,
    pmn02 LIKE pmn_file.pmn02,
    pmo06_2 LIKE pmo_file.pmo06
END RECORD

TYPE sr4_t RECORD
    pmm01 LIKE pmm_file.pmm01,
    pmn02 LIKE pmn_file.pmn02,
    pmo06_3 LIKE pmo_file.pmo06
END RECORD

TYPE sr5_t RECORD
    pmm01 LIKE pmm_file.pmm01,
    pmo06_4 LIKE pmo_file.pmo06
END RECORD
###GENGRE###END
...
MAIN
  ...
  #LET tm.b  = ARG_VAL(9)   #FUN-740223 mark
   ...
   LET g_sql ="pmm01.pmm_file.pmm01,",
              ...
              "l_pmn31_pmn20.pmn_file.pmn31,",       #FUN-B50018
              "l_n.type_file.num5,",
              "zo041.zo_file.zo041,",
              "zo042.zo_file.zo042,",
   ...
   LET l_table = cl_prt_temptable('apmg902',g_sql) CLIPPED
   IF  l_table = -1 THEN 
  ...
  END IF
   ...
   LET l_table1 = cl_prt_temptable('apmg9021',g_sql) CLIPPED
   IF  l_table1 = -1 THEN
       CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add
       CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)   #FUN-B50018  add 
       EXIT PROGRAM 
   ...
   LET l_table2 = cl_prt_temptable('apmg9022',g_sql) CLIPPED
   IF  l_table2 = -1 THEN 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add
       CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)   #FUN-B50018  add
       EXIT PROGRAM
   ...
   LET l_table3 = cl_prt_temptable('apmg9023',g_sql) CLIPPED
   IF  l_table3 = -1 THEN 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add
       CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)   #FUN-B50018  add
       EXIT PROGRAM 
   ...
   LET l_table4 = cl_prt_temptable('apmg9024',g_sql) CLIPPED
   IF  l_table4 = -1 THEN 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add
       CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)   #FUN-B50018  add
       EXIT PROGRAM
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      THEN CALL g902_tm(0,0)        # Input print condition
      ELSE CALL apmg902()        # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
   CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)
END MAIN
 
FUNCTION g902_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,     #No.FUN-680136 SMALLINT
   ...
   OPEN WINDOW g902_w AT p_row,p_col WITH FORM "apm/42f/apmg902"
...
WHILE TRUE
  #DISPLAY BY NAME tm.a,tm.b,tm.d,tm.e,tm.f,tm.more    #FUN-740223 mark
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW g902_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)
      EXIT PROGRAM
   END IF
  ...
  #INPUT BY NAME tm.a,tm.b,tm.d,tm.e,tm.f,tm.more      #FUN-740223 mark
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('apmg902',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW g902_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)
...
#          DEFINE   l_sql6   LIKE  type_file.chr1000
     ...
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'apmg902'
...
#No.CHI-6A0004----End------
     ...
     LET g_sql ="INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,  #TQC-780049
                " VALUES(?,?,?,?,?,",
                "?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ",
                "?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?)"       #FUN-870151 Add "?"  #MOD-9C0315 add ?   #FUN-B50018  add ?
     ...
     PREPARE g902_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN CALL cl_err('prepare:',SQLCA.sqlcode,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
        CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)
        EXIT PROGRAM 
     END IF
     DECLARE g902_cs1 CURSOR FOR g902_prepare1
    ...
    #CALL cl_outnam('apmg902') RETURNING l_name   #No.FUN-710091 
    #TQC-650044
    ...
    #START REPORT g902_rep TO l_name  #No.FUN-710091 
 
     LET g_pageno = 0
     INITIALIZE g_pmm01 TO NULL  #No.FUN-710091
     FOREACH g902_cs1 INTO sr.*
     ...
     #TQC-6C0136 --end
#      OUTPUT TO REPORT g902_rep(sr.*)  #No.FUN-710091 
      ...
      #    PREPARE g902_prepare5 FROM l_sql5
      #    DECLARE g902_note_cur5 CURSOR FOR g902_prepare5
      #    FOREACH g902_note_cur5 INTO l_pmo06
      #        IF SQLCA.sqlcode != 0 AND SQLCA.sqlcode !=100
      ...
      #END IF
       ...
       END IF
       LET sr.l_pmn31_pmn20 = sr.pmn31 * sr.pmn20              #FUN-B50018 add 
  ...
  #str CHI-7A0018 add
   ...
   LET l_sql = "SELECT pmm01 FROM pmm_file ",
               " WHERE ",tm.wc CLIPPED,
               "   ORDER BY pmm01"
   PREPARE g902_prepare2 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g902_cs2 CURSOR FOR g902_prepare2
 
   FOREACH g902_cs2 INTO sr.pmm01
      #額外備註-單據前備註 
...
#    FINISH REPORT g902_rep #No.FUN-710091
#No.FUN-710091  --begin
...
###GENGRE###     LET g_str = tm.wc,";;;",tm.d,";",tm.e,";",tm.f
  #end FUN-740223 modify
   # CALL cl_prt_cs3("apmg902",g_sql,g_str)  #TQC-730088
###GENGRE###     CALL cl_prt_cs3('apmg902','apmg902',g_sql,g_str)
    CALL apmg902_grdata()    ###GENGRE###
#No.FUN-710091  --end  
...
FUNCTION apmg902_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("apmg902")
        IF handler IS NOT NULL THEN
            START REPORT apmg902_rep TO XML HANDLER handler
            LET l_sql = "SELECT COUNT(DISTINCT(pmm01)) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
            DECLARE apmg902_cur1 CURSOR FROM l_sql
            EXECUTE apmg902_cur1 INTO g_tot 

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY pmm01,pmn02" 
            DECLARE apmg902_datacur1 CURSOR FROM l_sql
            FOREACH apmg902_datacur1 INTO sr1.*
                OUTPUT TO REPORT apmg902_rep(sr1.*)
            END FOREACH
            FINISH REPORT apmg902_rep
        END IF
...
REPORT apmg902_rep(sr1)
    ...
    DEFINE l_pmn31_fmt            STRING                      #FUN-B50018----add
    DEFINE l_pmn31_pmn20_fmt      STRING
    DEFINE l_pmn31_pmn20_sum_fmt  STRING                      #FUN-B50018----add
    DEFINE l_pmn31_pmn20_sum    LIKE pmn_file.pmn31           #FUN-B50018----add
    DEFINE l_pmn02_sum          LIKE pmn_file.pmn02           #FUN-B50018----add
    DEFINE l_pmm42_fmt            STRING                      #FUN-B50018----add
    DEFINE l_pmn20_fmt            STRING                      #FUN-B50018----add
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
            PRINTX g_tot                                       #FUN-B50018----add
        BEFORE GROUP OF sr1.pmm01
            LET l_lineno = 0
            #FUN-B50018----add-----str-----------------
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE pmm01 = '",sr1.pmm01 CLIPPED,"'"
            START REPORT apmg902_subrep01
            DECLARE apmg902_repcur1 CURSOR FROM l_sql
            FOREACH apmg902_repcur1 INTO sr2.*
                OUTPUT TO REPORT apmg902_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT apmg902_subrep01
            #FUN-B50018----add-----end-----------------
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B50018----add-----str-----------------
            LET l_pmn31_fmt = cl_gr_numfmt('pmn_file','pmn31',sr1.t_azi03)
            PRINTX l_pmn31_fmt
            LET l_pmn31_pmn20_fmt = cl_gr_numfmt('pmn_file','pmn31',sr1.t_azi04)
            PRINTX l_pmn31_pmn20_fmt
            LET l_pmn20_fmt = cl_gr_numfmt('pmn_file','pmn20',sr1.t_azi04)
            PRINTX l_pmn20_fmt
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,
                        " WHERE pmm01 = '",sr1.pmm01 CLIPPED,"'",
                        " AND pmn02 = ",sr1.pmn02 CLIPPED
            START REPORT apmg902_subrep02
            DECLARE apmg902_repcur2 CURSOR FROM l_sql
            FOREACH apmg902_repcur2 INTO sr3.*
                OUTPUT TO REPORT apmg902_subrep02(sr3.*)
            END FOREACH
            FINISH REPORT apmg902_subrep02


            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        " WHERE pmm01 = '",sr1.pmm01 CLIPPED,"'",
                        " AND pmn02 = ",sr1.pmn02 CLIPPED
            START REPORT apmg902_subrep03
            DECLARE apmg902_repcur3 CURSOR FROM l_sql
            FOREACH apmg902_repcur3 INTO sr4.*
                OUTPUT TO REPORT apmg902_subrep03(sr4.*)
            END FOREACH
            FINISH REPORT apmg902_subrep03
            #FUN-B50018----add-----end-----------------
        ...
        AFTER GROUP OF sr1.pmm01
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table4 CLIPPED,
                        " WHERE pmm01 = '",sr1.pmm01 CLIPPED,"'"
            START REPORT apmg902_subrep04
            DECLARE apmg902_repcur4 CURSOR FROM l_sql
            FOREACH apmg902_repcur4 INTO sr5.*
                OUTPUT TO REPORT apmg902_subrep04(sr5.*)
            END FOREACH
            FINISH REPORT apmg902_subrep04

            LET l_pmn02_sum = GROUP SUM(sr1.pmn02)
            PRINTX l_pmn02_sum

            LET l_pmn31_pmn20_sum = GROUP SUM(sr1.l_pmn31_pmn20)
            PRINTX l_pmn31_pmn20_sum
            LET l_pmn31_pmn20_sum_fmt = cl_gr_numfmt('pmn_file','pmn31',sr1.t_azi04)
            PRINTX l_pmn31_pmn20_sum_fmt
            LET l_pmm42_fmt = cl_gr_numfmt('pmm_file','pmm42',sr1.t_azi07)
            PRINTX l_pmm42_fmt
            #FUN-B50018----add-----end-----------------
