...
#
# Pattern name...: apmp550.4gl
# Descriptions...: 採購單凍結作業
# Date & Author..: 92/02/25 By Carol
# Modify ........: 99/04/15 By Carol747:update s_pmmsta() 
...
FUNCTION p550_tm()
   ...
   DEFINE l_no          LIKE type_file.num5       #No.FUN-680136 VARCHAR(1)
 
   LET p_row = 2 LET p_col = 19
 
   OPEN WINDOW p550_w AT p_row,p_col WITH FORM "apm/42f/apmp550" 
       ...
       ON ACTION CONTROLP
          CASE
             WHEN INFIELD(pmm01) #單號
                CALL q_pmm4(FALSE,TRUE,tm.pmm01,'012') RETURNING tm.pmm01
...
#No.FUN-710030 -- end --
           IF g_pmn[l_no].sure = 'Y' THEN
              UPDATE pmn_file SET pmn11 = 'Y' 
               WHERE pmn01 = tm.pmm01 AND pmn02 = g_pmn[l_no].pmn02 
              IF SQLCA.sqlcode THEN 
                 LET g_success='N' 
#                CALL cl_err('(p550:ckp#2)',SQLCA.sqlcode,1)   #No.FUN-660129
#No.FUN-710030 -- begin --
#                 CALL cl_err3("upd","pmn_file",tm.pmm01,g_pmn[l_no].pmn02,SQLCA.sqlcode,"","(p550:ckp#2)",1)  #No.FUN-660129
#                 EXIT FOR 
                 IF g_bgerr THEN
                    LET g_showmsg = tm.pmm01,"/",g_pmn[l_no].pmn02
                    CALL s_errmsg("pmn01,pmn02",g_showmsg,"(p550:ckp#2)",SQLCA.sqlcode,1)
                    CONTINUE FOR
                 ELSE
                    CALL cl_err3("upd","pmn_file",tm.pmm01,g_pmn[l_no].pmn02,SQLCA.sqlcode,"","(p550:ckp#2)",1)
                    EXIT FOR
                 END IF
#No.FUN-710030 -- end --
              END IF
...
FUNCTION p550_pmm01()
   ...
   SELECT UNIQUE(pmm09) INTO l_pmm09
          ...
          WHERE pmm01 = tm.pmm01  AND pmm01 = pmn01
            AND pmm25 IN ('0','1','2') AND pmn11 NOT IN ('y','Y')
   IF SQLCA.sqlcode THEN
       LET g_error='N'
       LET l_pmm09=NULL
       LET l_pmc03=NULL
...
FUNCTION p550_b_fill()
   ...
   DEFINE l_pmmmksg     LIKE pmm_file.pmmmksg
     ...
     LET l_sql = " SELECT 'N',pmn02,pmn04,pmn16,pmn20,pmm18,pmmmksg",
                 ...
                 "    AND pmn16 IN ('0','1','2') ",
                 "    AND pmn11 NOT IN ('Y','y')",
                 "    AND pmn01=pmm01 ",
                 "  ORDER BY pmn02 "
     LET l_sql = l_sql clipped,l_wc clipped
     PREPARE p550_prepare FROM l_sql
     IF SQLCA.sqlcode THEN 
        CALL cl_err('cannot prepare ',SQLCA.sqlcode,1) 
        LET g_success = 'N'
        RETURN
     END IF
     DECLARE p550_cur CURSOR FOR p550_prepare
     IF SQLCA.sqlcode THEN 
     ...
     LET g_rec_b = 0 
     LET l_cnt   = 0 
     FOREACH p550_cur INTO g_pmn[l_ac].*,l_pmm18,l_pmmmksg 
        IF SQLCA.sqlcode THEN 
