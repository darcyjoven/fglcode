...
# Modify.........: No.FUN-CB0072 12/11/28 By dongsz CR轉GR
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD				# Print condition RECORD
         #     wc   VARCHAR(700),                  # Where Condition
              ...
              s    LIKE type_file.chr3,         #No.FUN-680136 VARCHAR(03) # 排列項目
              t    LIKE type_file.chr3,         #No.FUN-680136 VARCHAR(03) # 同項目是否跳頁
              c    LIKE type_file.chr1,         #No.FUN-680136 VARCHAR(01) # 是否列印交期確認資料
              more LIKE type_file.chr1          #No.FUN-680136 VARCHAR(01) # 特殊列印條件
              END RECORD
   #       g_dash1    VARCHAR(200),
...
TYPE sr1_t RECORD
    ...
    pmz12 LIKE pmz_file.pmz12,               #FUN-CB0072 add
    pmz13 LIKE pmz_file.pmz13,               #FUN-CB0072 add
    g_cnt1 LIKE type_file.num5,
    g_cnt LIKE type_file.num5,
    order1 LIKE pmn_file.pmn04,              #FUN-CB0072 add
    order2 LIKE pmn_file.pmn04,              #FUN-CB0072 add
    order3 LIKE pmn_file.pmn04               #FUN-CB0072 add 
END RECORD
###GENGRE###END
...
#NO.FUN-750098-----Begin 
   LET g_sql ="pmn33.pmn_file.pmn33,", 
              ...
              "pmz12.pmz_file.pmz12,",             #FUN-CB0072 add
              "pmz13.pmz_file.pmz13,",             #FUN-CB0072 add
              "g_cnt1.type_file.num5,",
              "g_cnt.type_file.num5,",
              "order1.pmn_file.pmn04,",            #FUN-CB0072 add
              "order2.pmn_file.pmn04,",            #FUN-CB0072 add
              "order3.pmn_file.pmn04"              #FUN-CB0072 add
      
   LET l_table = cl_prt_temptable('apmg503',g_sql) CLIPPED        # 產生Temp Table                                                       
...
DEFINE p_row,p_col    LIKE type_file.num5,    #No.FUN-680136 SMALLINT
   ...
   OPEN WINDOW r503_w AT p_row,p_col WITH FORM "apm/42f/apmg503"
...
WHILE TRUE
   CONSTRUCT BY NAME  tm.wc ON pmn33,pmn36,pmn34,pmn04,pmm09,pmm01,pmm12    #FUN-CB0072 add pmm01
     #--No.FUN-4A0010--------
         #No.FUN-580031 --start--
         BEFORE CONSTRUCT
             CALL cl_qbe_init()
 ...
 #        CALL cl_about()      #MOD-4C0121
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('apmg503',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW r503_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      CALL cl_gre_drop_temptable(l_table)                  #FUN-CB0072 add
...
FUNCTION r503()
   DEFINE l_name     LIKE type_file.chr20, 	   # External(Disk) file name  #No.FUN-680136 VARCHAR(20)
         ...
         #l_sql      LIKE type_file.chr1000,       # RDSQL STATEMENT  #No.FUN-680136 VARCHAR(1000) #MOD-B10019 mark
          ...
          l_pmd03    LIKE pmd_file.pmd03,          #FUN-CB0072 add
          ...
          sr         RECORD
                     ...
                     pmz12     LIKE    pmz_file.pmz12,   #備註二          #FUN-CB0072 ADD
                     pmz13     LIKE    pmz_file.pmz13    #備註三          #FUN-CB0072 ADD
                     END RECORD
      CALL cl_del_data(l_table)   #No.FUN-750098
      SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_lang
      SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog    #No.FUN-750098
#     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'apmg503'
...
#                   " pmn33,pmn04,pmn20,(pmn20-pmn50+pmn55),pmm09,pmc03,",       #No.FUN-940083
                    ...
                    " pmn16,pmm12,gen02,pmn01,pmn02,pmn14,pmn36,pmn34,pmn06,",
                     " pmn50-pmn55,pmc10,pmm04,pmn15,pmn041, ",   #No.MOD-570114
                    " pmz03,pmz05,pmz06,pmz07,pmz08,pmz09,pmz10,pmz11,pmz12,pmz13 ",   #FUN-CB0072 add pmz12,pmz13
                    " FROM pmm_file,pmn_file, OUTER pmc_file, OUTER gen_file,",
...
#    CALL cl_outnam('apmg503') RETURNING l_name    #No.FUN-750098
...
#      OUTPUT TO REPORT r503_rep(sr.*)
      ...
      #FUN-CB0072--add--str---
       IF cl_null(sr.pmc10) THEN
          SELECT pmd03 INTO l_pmd03 FROM pmd_file 
           WHERE pmd01 = sr.pmm09
          LET sr.pmc10 = l_pmd03
       END IF
      #FUN-CB0072--add--end--- 
       IF tm.c='Y' THEN
          ...
          SELECT gen02 INTO l_gen02 FROM gen_file
           WHERE gen01=sr.pmz10 
             EXECUTE insert_prep USING                                                                                              
                     ...
                     sr.pmz08,sr.pmz09,sr.pmz10,sr.pmz11,sr.pmz12,sr.pmz13,g_cnt1,g_cnt,  #FUN-CB0072 add sr.pmz12,sr.pmz13
                     sr.order1,sr.order2,sr.order3                           #FUN-CB0072 add
           LET g_cnt1=g_cnt1+1   
          ELSE  
              EXECUTE insert_prep USING                                                                                              
                     sr.pmn33,sr.pmn04,sr.pmn20_n,sr.pmn041,sr.pmn20,                                                               
...
###GENGRE###     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                               
     #是否列印選擇條件                                                                                                              
     IF g_zz05 = 'Y' THEN
        CALL cl_wcchp(tm.wc,'pmn33,pmn36,pmn34,pmn04,pmm09,pmm01,pmm12') RETURNING tm.wc   #FUN-CB0072 add pmm01
        LET g_str = tm.wc                                                                                                           
     END IF
###GENGRE###     LET g_str = g_str,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",tm.t,";",tm.c
###GENGRE###     CALL  cl_prt_cs3('apmg503','apmg503',l_sql,g_str) 
    CALL apmg503_grdata()    ###GENGRE###
#NO.FUN-750098-------End  
END FUNCTION
...
FUNCTION apmg503_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("apmg503")
        IF handler IS NOT NULL THEN
            START REPORT apmg503_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                       ," ORDER BY order1,order2,order3,pmn01,pmn02,pmz03"              #FUN-CB0072 add
          
            DECLARE apmg503_datacur1 CURSOR FROM l_sql
            FOREACH apmg503_datacur1 INTO sr1.*
                OUTPUT TO REPORT apmg503_rep(sr1.*)
            END FOREACH
            FINISH REPORT apmg503_rep
        END IF
...
REPORT apmg503_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_order_fld    STRING             #FUN-CB0072 add
   ...
   #ORDER EXTERNAL BY sr1.pmn01,sr1.pmn02,sr1.pmz03           #FUN-CB0072 mark
    ORDER EXTERNAL BY sr1.order1,sr1.order2,sr1.order3,sr1.pmn01,sr1.pmn02,sr1.pmz03        #FUN-CB0072 add
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
           #FUN-CB0072--add--str---
            LET l_order_fld = cl_gr_getmsg('gre-324',g_lang,tm.s[1,1]),'-',cl_gr_getmsg('gre-324',g_lang,tm.s[2,2]),  
                                       '-',cl_gr_getmsg('gre-324',g_lang,tm.s[3,3])                                  
            PRINTX l_order_fld                                                                                      
        ...
        BEFORE GROUP OF sr1.order1               #FUN-CB0072 add
        BEFORE GROUP OF sr1.order2               #FUN-CB0072 add
        BEFORE GROUP OF sr1.order3               #FUN-CB0072 add
        BEFORE GROUP OF sr1.pmn01
        BEFORE GROUP OF sr1.pmn02
        BEFORE GROUP OF sr1.pmz03
        ...
        AFTER GROUP OF sr1.order1                #FUN-CB0072 add
        AFTER GROUP OF sr1.order2                #FUN-CB0072 add
        AFTER GROUP OF sr1.order3                #FUN-CB0072 add
        AFTER GROUP OF sr1.pmn01
        AFTER GROUP OF sr1.pmn02
        AFTER GROUP OF sr1.pmz03
