...
MAIN
   ...
   LET l_table = cl_prt_temptable('apmr220',g_sql) CLIPPED   # 產生Temp Table
...
FUNCTION apmr220_tm(p_row,p_col)
   ...
   OPEN WINDOW apmr220_w AT p_row,p_col WITH FORM "apm/42f/apmr220"
   ...
   WHILE TRUE
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('apmr220',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#No.FUN-BA0053---End-----add
   ...
   DECLARE apmr220_c1 CURSOR FOR apmr220_p1
   ...
   FOREACH apmr220_c1 INTO l_rva05,l_pmc03,l_pmc02,l_rva06,l_rvb10,
                          #-------------No.MOD-720060 modify
...
#              CALL cl_err('insert:',SQLCA.SQLCODE,1)   #No.FUN-660129
   ...
   DECLARE apmr220_c2 CURSOR FOR apmr220_p2
...
#           CALL cl_err('update:',SQLCA.SQLCODE,1)   #No.FUN-660129
   ...
   IF g_zz05 = 'Y' THEN
      ...
      LET g_str = tm.wc
   END IF
   LET g_str = g_str,";",g_azi04                      #FUN-710080 add
   CALL cl_prt_cs3('apmr220','apmr220',l_sql,g_str)   #FUN-710080 modify
   #------------------------------ CR (4) ------------------------------#
   #end FUN-720010 add 
