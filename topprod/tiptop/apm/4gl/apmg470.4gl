...
# Modify.........: No:FUN-CB0073 12/11/16 By chenjing CR轉GR
# Modify.........: No:FUN-D10098 13/01/22 By chenjing 增加替代說明
...
TYPE sr1_t RECORD
    ...
    lstat LIKE type_file.chr1,
    ldate LIKE pml_file.pml35,
    lno LIKE pml_file.pml01,
    lseq LIKE pml_file.pml02,
    lqty LIKE pml_file.pml20,
    ltype LIKE type_file.chr1,
    lscode LIKE type_file.chr1,
    pnl11 LIKE pnl_file.pnl11,
    pnl10 LIKE pnl_file.pnl10,
    pnl09 LIKE pnl_file.pnl09,
    pnl08 LIKE pnl_file.pnl08,
    pnl07 LIKE pnl_file.pnl07,
    pnl06 LIKE pnl_file.pnl06,
    pnl05 LIKE pnl_file.pnl05,
    pnl04 LIKE pnl_file.pnl04,
    pnl03 LIKE pnl_file.pnl03,
    pnl02 LIKE pnl_file.pnl02,
...
#                "ima262.ima_file.ima262,", #FUN-A20044
    ...
    LET l_table = cl_prt_temptable('apmg470',g_sql) CLIPPED   # 產生Temp Table                                                      
...
#----------------------------------------------------------CR (1) ------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      ...
      ELSE CALL apmg470()		        # Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW r470_w AT p_row,p_col WITH FORM "apm/42f/apmg470"
...
END CONSTRUCT
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('apmg470',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW r470_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
      CALL cl_gre_drop_temptable(l_table)            #FUN-CB0073
...
#                  ima262 LIKE ima_file.ima262,	# 可用量 #FUN-A20044
     ...
     #MOD-640356
     #SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'apmg470'
...
#    CALL cl_outnam('apmg470') RETURNING l_name
#    IF tm.choice = '3' THEN
#   	LET l_name2=l_name
#    	LET l_name2[11,11]='x'
#    ELSE
...
#                   '1','','','','','','','','',       #TQC-960245                                                                      
                   ...
                   '','','','','','','','','','','',''                                                                              
          #------------------------------ CR (3) ------------------------------#
       #      LET g_template = 'apmg470_1'                           #FUN-CB0073
...
#                   '2','','','','','','','','',      #TQC-960245
                   ...
                   '','','','','','','','','','','',''                                                                              
          #------------------------------ CR (3) ------------------------------#
       #      LET g_template = 'apmg470'                         #FUN-CB0073
           END IF                                                  #No.FUN-7C0034
         END FOREACH
       END IF
       #--->產生備料數量
...
###GENGRE###    LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                                
    LET g_str = ''                                                                                                                  
###GENGRE###    LET g_str = tm.wc,";",g_towhom,";",tm.choice                                                         
###GENGRE###    CALL cl_prt_cs3('apmg470','apmg470',l_sql,g_str)                                                                                  
    CALL apmg470_grdata()    ###GENGRE###
    #------------------------------ CR (4) ------------------------------# 
END FUNCTION
...
FUNCTION apmg470_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("apmg470")
        IF handler IS NOT NULL THEN
            START REPORT apmg470_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        "  ORDER BY chr1,pmk01,pml02,pml04 asc nulls first "
                     #  "  ORDER BY pmk01,pml02,pml04,lstat desc,lseq asc nulls first,ldate"
          
            DECLARE apmg470_datacur1 CURSOR FROM l_sql
            FOREACH apmg470_datacur1 INTO sr1.*
                OUTPUT TO REPORT apmg470_rep(sr1.*)
            END FOREACH
            FINISH REPORT apmg470_rep
        END IF
...
REPORT apmg470_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
#FUN-CB0073--add--str---
    DEFINE l_rem      LIKE ima_file.ima262   
    ...
    DEFINE l_str      STRING    #FUN-D10098

#FUN-CB0073--add--end---
    
    ORDER EXTERNAL BY sr1.chr1,sr1.pmk01,sr1.pml02      #FUN-CB0073 add pml02
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
            PRINTX g_towhom    #FUN-CB0073 add
    #FUN-CB0073--add--str---
          LET l_title2 = g_grPageHeader.title2,cl_gr_getmsg('gre-315',g_lang,'2')
          LET l_title1 = g_grPageHeader.title2,cl_gr_getmsg('gre-315',g_lang,'1')
          PRINTX l_title2,l_title1
    #FUN-CB0073--add--end---
              
        BEFORE GROUP OF sr1.chr1 
        BEFORE GROUP OF sr1.pmk01
        BEFORE GROUP OF sr1.pml02   #FUN-CB0073 add 
      ...
      #FUN-CB0073--add--start--
          #IF sr1.pnl05 = 'N' THEN 
          #   LET l_ima262 = 0 
          #ELSE
          #   LET l_ima262 = sr1.avl_stk
          #END IF
           PRINTX l_ima262
        
        
           IF sr1.ltype = 'D' THEN 
              LET l_rem = sr1.avl_stk - sr1.lqty
           ELSE
              LET l_rem = sr1.avl_stk + sr1.lqty
           END IF 
           PRINTX l_rem
           ...
           IF tm.choice = '2' THEN 
              LET l_display = 'Y'
           ELSE
              IF tm.choice = '3' AND sr1.chr1 = '2' THEN 
                 LET l_display = 'Y'
              ELSE
                 LET l_display = 'N'
              END IF
           ...
           IF tm.choice = '1' THEN
              LET l_display1 = 'Y'
           ELSE
              IF tm.choice = '3' AND sr1.chr1 = '1' THEN
                 LET l_display1 = 'Y'
              ELSE
                 LET l_display1 = 'N'
              END IF
           END IF
           PRINTX l_display1
           LET l_str = cl_gr_getmsg("gre-337",g_lang,sr1.lstat)    #FUN-D10098
           PRINTX l_str                                            #FUN-D10098
      #FUN-CB0073--add--end---
            PRINTX sr1.*
    ...
    #FUN-CB0073--add--str--
        AFTER GROUP OF sr1.pml02
           IF sr1.lscode = 'R' THEN
              LET req_qty = GROUP SUM(sr1.lqty)
           ELSE 
              LET req_qty = 0
           END IF
           PRINTX req_qty
           ...
           IF sr1.pnl10 = 'N' THEN 
              LET al_qty = 0
           ELSE 
              IF sr1.lscode = '1' THEN 
                 LET al_qty = GROUP SUM(sr1.lqty)
              ELSE
                 LET al_qty = 0
              END IF
           END IF
           ...
           IF sr1.pnl07 = 'N' THEN
              LET pr_qty = 0
           ELSE
              IF sr1.lscode = '2' THEN
                 LET pr_qty = GROUP SUM(sr1.lqty)
              ELSE
                 LET pr_qty = 0
              END IF
           END IF
           ...
           IF sr1.pnl08 = 'N' THEN
              LET po_qty = 0
           ELSE
              IF sr1.lscode = '3' THEN
                 LET po_qty = GROUP SUM(sr1.lqty)
              ELSE
                 LET po_qty = 0
              END IF
           END IF
           ...
           IF sr1.pnl06 = 'N' THEN
              LET qc_qty = 0
           ELSE
              IF sr1.lscode = '4' THEN
                 LET qc_qty = GROUP SUM(sr1.lqty)
              ELSE
                 LET qc_qty = 0
              END IF
           END IF
           ...
           IF sr1.pnl09 = 'N' THEN
              LET wo_qty = 0
           ELSE
              IF sr1.lscode = '5' THEN
                 LET wo_qty = GROUP SUM(sr1.lqty)
              ELSE
                 LET wo_qty = 0
              END IF
           END IF
           ...
           LET l_supply= l_ima262 + qc_qty + po_qty + pr_qty + wo_qty
           LET l_demand= req_qty + al_qty
           LET sh_qty = l_demand - l_supply
           PRINTX sh_qty
    #FUN-CB0073--add--end-- 
