...
# Modify.........: No.DEV-D40015 13/04/17 By Nina (1)aimi100有勾選使用條碼&條碼產生時機='E'，確認前，
...
# Modify.........: No.MOD-D60109 13/06/13 By SunLM 如果是misc料件，则不进行判断仓库
# Modify.........: No.FUN-D60103 13/06/28 By lixh1 已經結案的委外工單也可以進行委外倉退
# Modify.........: No:FUN-D40103 13/05/08 By lixh1 增加儲位有效性檢查
# Modify.........: No:MOD-D60078 13/06/13 By SunLM 理由码检查,将after field rvv26的那段检查逻辑复制到审核段检查段。
# Modify.........: No.TQC-D80008 13/08/07 By lixh1 調整單價計算邏輯
# Modify.........: No.TQC-D80017 13/08/12 By lixh1 apmt742 多角銷退時使用進項稅率
# Modify.........: No.FUN-D70021 13/08/26 By yangtt aapp115增加传入参数"MISC料号是否立帳"g_a='N'
# Modify.........: No.FUN-D60083 13/08/26 By yangtt aapp115增加传入参数"非成本倉是否立賬"g_b='N'
...
GLOBALS "../../aba/4gl/barcode.global" #No:DEV-D30026--add
...
FUNCTION t720sub_u_rvb(p_cmd,l_rvu,l_rvv,p_argv1) # Update rvb_file
  ...
  DEFINE l_rvbslk30  LIKE rvbslk_file.rvbslk30 #FUN-B90104 add
      ...
      IF l_rvu.rvu00='3' AND l_rvu.rvu08='SUB' THEN RETURN END IF    #Add by zm 131121  委外仓退不需更新rvb相关数量
      ...
      IF l_qty<(l_rvb30+l_rvb29) AND p_argv1 <>'2' THEN
         IF l_rvv.rvv01[1,3] = 'PRO' THEN 
           #add by donghy 期初单据暂不检查
         ELSE
   ...
   IF l_n=0 THEN RETURN END IF           #如果全部入庫
     ...
     #FUN-C20064---add---end---
       ...
       FOREACH t110_y2 INTO la_rvv.*
          ...
          SELECT rvb07-rvb30 INTO l_qty FROM rvb_file    #未入庫量
  ...
  ELSE
   #不使用單價*數量=金額, 改以金額回推稅率, 以避免小數位差的問題
    ...
    IF NOT cl_null(l_rvu02) THEN  #MOD-BC0262 add
       ...
       IF l_rva00 = '2' THEN
          SELECT gec07,gec05 INTO l_gec07,l_gec05 FROM gec_file WHERE gec01 = l_rva115   #CHI-AC0016 add gec05
       ELSE
    ...
    END IF
  END IF
  #add by liy190430 s #应客户要求，解决金额与apmt540计算逻辑不同导致的尾差
              CALL cl_digcut(p_rvv39,t_azi04) 
                                RETURNING p_rvv39
              CALL cl_digcut(p_rvv39t,t_azi04)   
                                RETURNING p_rvv39t 
  #add by liy190430 e
...
DEFINE l_ima930      LIKE ima_file.ima930       #DEV-D30040 add
  CALL s_showmsg_init()   
   ...
   #TQC-B90236  ----add---begin
            ...
            END IF 
   #TQC-B90236  ----end--
            #IF g_sma.sma90 = "Y" THEN #MOD-C30074 add #MOD-C40098 mark
            IF NOT (g_sma.sma90 = 'N' AND l_rvu.rvu00 = '2') THEN  #MOD-C40098 add
   ...
   #              AND rvbs02 = l_rvv.rvv05       #MOD-B40007   #MOD-B80041
           ...
           #   IF (l_rvv.rvv17 * l_rvv.rvv35_fac) <> l_rvbs06 THEN
               IF (l_ima930 = 'Y' and l_rvbs06 <> 0) OR l_ima930 = 'N' THEN  #DEV-D30040
                  IF (l_rvv.rvv17 * l_rvv35_fac) <> l_rvbs06 THEN #CHI-C30064
                     LET g_success = "N"
      ...
      IF l_rvv.rvv31[1,4] <> 'MISC' THEN 
         ...
         IF l_cnt > 0 THEN    #大於有效日期
            ...
            #LET g_success = 'N' #donghy 临时取消管控 --160817
            #RETURN
         END IF
      END IF 
      ...
      IF l_rvu.rvu08='SUB' THEN
         SELECT pmn41,pmn43,pmn18,pmn32
           INTO l_pmn41,l_pmn43,l_pmn18,l_pmn32
           FROM pmn_file
...
FUNCTION t720sub_u_pmn(l_rvv,p_rvviicd07)                           #更新P/O已交貨量
 ...
 DEFINE l_rvu08   LIKE rvu_file.rvu08
   ...
   #Add by zm 131121
   SELECT rvu00,rvu08 INTO l_rvu00,l_rvu08 FROM rvu_file 
    WHERE rvu01=l_rvv.rvv01
   IF l_rvu00='3' AND l_rvu08='SUB' THEN RETURN END IF  
   #End by zm 131121
...
FUNCTION t720sub_t1(l_rvv,l_rvu,p_argv1,p_rva00)
   ...
   DEFINE l_rvvud13       LIKE rvv_file.rvvud13   #add by guanyao160621
	   #------>多倉管理時更新庫存明細檔
	   IF l_rvv.rvv31[1,4] = 'MISC' THEN
	      CALL t720sub_u_rvb('y',l_rvu.*,l_rvv.*,p_argv1)  #->更新驗收單單身檔
	      IF p_rva00 = '1' THEN
...
#        	 LET l_forupd_sql="SELECT ima FROM ima_file",    #TQC-A50034 
         ...
         IF l_rvv.rvv32 IS NOT NULL AND l_pmn65='1' THEN
            ...
            IF NOT cl_null(l_rvv.rvvud13) AND l_rvv.rvvud01 NOT MATCHES 'WE1*' THEN     #add by shawn rvuud02 
               LET l_ima71 = ''
               SELECT ima71 INTO l_ima71 FROM ima_file WHERE ima01 = l_rvv.rvv31
               IF l_ima71>0 THEN
                  LET l_rvvud13 = '' 
                  SELECT to_date(l_rvv.rvvud13,'yy/mm/dd')+l_ima71 INTO l_rvvud13 FROM dual 
                  UPDATE img_file SET img18 = l_rvvud13
                                WHERE img01 = l_rvv.rvv31
                                  AND img02 = l_rvv.rvv32
                                  AND img03 = l_rvv.rvv33
                                  AND img04 = l_rvv.rvv34
                  IF STATUS  THEN
                     CALL cl_err('up_img18:','9050',0)
                     LET g_success='N'
                     RETURN
                  END IF
               END IF 
...
#--------產生異動記錄
  ...
  #TQC-B40100 mark --end--
   ...
   IF l_rvu.rvu00='3' AND l_rvu.rvu08='SUB' THEN RETURN END IF    #Add by zm 131121  委外仓退不需更新rvb/pmn相关数量
...
### ---委外倉退確認應update sfb09=sfb09-倉退量*轉換率
  #Mark by zm 131121 委外仓退不更新sfb09
...
FUNCTION t720sub_chk_sfb(l_rvv,p_rvu08)
  ...
  IF l_sfb39 != '2' THEN
     IF  p_rvu08<>'SUB' and  l_sfb05 = l_rvv.rvv31 THEN    #ly170901  #委外暂不检查发料套数
...
FUNCTION t720sub_minp(p_rvv18,p_rvv31)
  ...
  # sma74 工單完工量容許差異百分比
   ...
   IF g_sma.sma73='Y' THEN
      LET l_over_qty=((l_min_set_t*l_ima153)/100)  
   ELSE
      LET l_over_qty=0
   END IF
...
FUNCTION t720sub_IQC_return(l_rvu,p_argv1,p_argv3,p_chr,p_inTransaction,p_ask_post,p_rva00)
   ...
   END IF
       ...
       IF la_rvu.rvu900 IS NULL THEN LET la_rvu.rvu900 = ' ' END IF
      ...
      #FUN-A60009 add end -------
       ...
       FOREACH t110_y1 INTO lb_rvv.*,lb_rvvi.*
          ...
          IF l_rvb39='Y' THEN
             ...
             IF la_rvv.rvv17<=0 THEN CONTINUE FOREACH END IF   #MOD-CA0181 add
       ...
       CASE g_prog
         ...
         WHEN "apmt740_icd"
          #LET l_cmd = "apmt741_icd ","'",l_rvu.rvu02 CLIPPED,"' '' '2'"             #FUN-A60009 mark
           LET l_cmd = "apmt741_icd ","'' '' '",l_rvu.rvu02 CLIPPED,"' '2'"          #FUN-A60009 add
         WHEN "aict042"
       ...
       CASE g_prog
         ...
         WHEN "apmt740"
          #LET l_cmd = "apmt741 ","'",l_rvu.rvu02 CLIPPED,"' '' '2'"                 #FUN-A60009 mark
           LET l_cmd = "apmt741 ","'' '' '",l_rvu.rvu02 CLIPPED,"' '2'"              #FUN-A60009 add
       END CASE
    END IF
...
DEFINE l_x         LIKE type_file.num5
   ...
   #str-----add by guanyao160824
   LET l_x = 0
   SELECT COUNT(*) INTO l_x FROM tc_shb_file WHERE tc_shbud03 = p_rvu01
   IF l_x >0 THEN 
      CALL cl_err('','cpm-052',0)
      RETURN 
   END IF 
   #end-----add by guanyao160824
#str--------mark by guanyao160825
 ...
 --
   ...
   --ELSE
      --LET l_rvv02 = 1
   --END IF
#end--------mark by guanyao160825
   #str-----add by guanyao160824
   DECLARE t720_sub_ecm CURSOR FOR SELECT UNIQUE pmn18 
                                     FROM pmn_file 
                                    WHERE pmn01 IN (SELECT rvv36 FROM rvv_file WHERE rvv01 = p_rvu01)
   FOREACH t720_sub_ecm INTO l_pmn18 
  # SELECT UNIQUE pmn18
  #   INTO l_pmn18
  #   FROM pmn_file
  ...
  #                   WHERE rvv01 = p_rvu01)
   ...
   OPEN WINDOW t720_sub_tc_shb AT 5,10 WITH FORM "cpm/42f/cpmt720_sub"
        ATTRIBUTE (STYLE = g_win_style CLIPPED)
 
   CALL cl_ui_locale("cpmt720_sub")
   
   LET tm_1.gen01 = g_user
   SELECT gen02 INTO l_gen02 FROM gen_file WHERE gen01 = g_user
   DISPLAY l_gen02 TO gen02 
 
   INPUT BY NAME tm_1.gen01,tm_1.ecg01 WITHOUT DEFAULTS
      AFTER FIELD gen01
         IF NOT cl_null(tm_1.gen01) THEN
            LET l_x = 0
            SELECT COUNT(*) INTO l_x FROM gen_file WHERE gen01 = tm_1.gen01
            IF l_x= 0 OR cl_null(l_x) THEN 
               CALL cl_err('','mfg3096',0)
               NEXT FIELD gen01
            END IF  
            SELECT gen02 INTO l_gen02 FROM gen_file WHERE gen01 = tm_1.gen01
            DISPLAY l_gen02 TO gen02
         END IF

      AFTER FIELD ecg01
         IF NOT cl_null(tm_1.ecg01) THEN
            LET l_x = 0
            SELECT COUNT(*) INTO l_x FROM ecg_file WHERE ecg01 =tm_1.ecg01
            IF l_x= 0 OR cl_null(l_x) THEN 
               CALL cl_err('','cec-008',0)
               NEXT FIELD ecg01
            END IF  
            SELECT ecg02 INTO l_ecg02 FROM ecg_file WHERE ecg01 = tm_1.ecg01
      ...
      ON ACTION controlp
         CASE
            WHEN INFIELD(gen01)
               ...
               LET g_qryparam.default1 = tm_1.gen01
               CALL cl_create_qry() RETURNING tm_1.gen01
               DISPLAY BY NAME tm_1.gen01
            WHEN INFIELD(ecg01)
               CALL cl_init_qry_var()
               LET g_qryparam.form = "cq_ecg"
               LET g_qryparam.default1 = tm_1.ecg01
               CALL cl_create_qry() RETURNING tm_1.ecg01
               DISPLAY BY NAME tm_1.ecg01
         END CASE
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0
      CLOSE WINDOW t720_sub_tc_shb
      RETURN
   END IF 
   CLOSE WINDOW t720_sub_tc_shb
   
   CALL t720_sub_tc_shb(p_rvu01)
...
#  DEFINE l_sma92     LIKE sma_file.sma92   #FUN-B30161   #TQC-B40020   #TQC-B60252
  ...
  #FUN-A60009 add end --------
   ...
   DEFINE l_tc_sfpconf   LIKE tc_sfp_file.tc_sfpconf     #add by guanyao160707
   DEFINE l_rvv05_1      LIKE rvv_file.rvv05             #add by guanyao160707
   DEFINE l_count        LIKE type_file.num5             #add by guanyao160801
...
#CHI-C40016---mark---END-----------------------------------------
   ...
   IF l_rvu.rvu08 = 'SUB' THEN 
      DECLARE t720sub_pre CURSOR FOR SELECT rvv05 FROM rvv_file WHERE rvv01 = l_rvu.rvu01
      LET l_rvv05_1 = ''
      FOREACH  t720sub_pre INTO l_rvv05_1
         #str-----add by guanyao160801
         LET l_count = 0
         SELECT COUNT(*) INTO l_count FROM pmn_file,sgm_file,ecd_file,rvb_file 
          WHERE pmn18=sgm01 
            AND pmn43=sgm03 
            AND sgm04=ecd01 
            AND ecd02 LIKE '%SMT%'        
         ...
         LET l_tc_sfpconf =''
         #SELECT tc_sfpconf INTO l_tc_sfpconf FROM tc_sfp_file WHERE tc_sfp11 = l_rvu.rvu02 AND tc_sfp12 = l_rvv05_1 
   {      
   SELECT tc_sfpconf INTO l_tc_sfpconf FROM tc_sfp_file WHERE tc_sfp11 = l_rvu.rvu02 
           ---- AND (tc_sfp13 = 0 OR tc_sfp13 IS NULL)  #mark ly0816
         IF l_tc_sfpconf !='Y' OR cl_null(l_tc_sfpconf) THEN 
            CALL cl_err(l_rvu.rvu04||l_rvv05_1,'cpm-041',0)
            LET g_success = 'N' 
            RETURN 
         END IF
} #mark  ly170806 
...
#TQC-B60085   ---end     Add
   ...
   IF p_argv1='6' THEN
      SELECT rvu_file.* INTO l_rvu.*
        FROM rvu_file
      # WHERE rvu01 = p_argv2
       WHERE rvu01 = p_rvu01  #No.18010101
      IF SQLCA.SQLCODE THEN
         CALL cl_err3("sel","rvu_file","","",SQLCA.sqlcode,"","sel rvu",1)
         LET g_success = 'N' RETURN
      END IF
   ...
   DECLARE t720_rvv36 CURSOR FOR
       ...
       SELECT rvv36,rvv37,rvv02,rvv17,rvv32,rvv31,rvv05 FROM rvv_file WHERE rvv01 = l_rvu.rvu01   #MOD-BC0057 add

   #FOREACH t720_rvv36 INTO l_rvv36,l_rvv37   #MOD-A40166
   #FOREACH t720_rvv36 INTO l_rvv36,l_rvv37,l_rvv02,l_rvv17,l_rvv32,l_rvv31   #MOD-A40166   #MOD-AA0164 add rvv31 #MOD-BC0057 mark
   FOREACH t720_rvv36 INTO l_rvv36,l_rvv37,l_rvv02,l_rvv17,l_rvv32,l_rvv31,l_rvv05   #MOD-BC0057 add
      #-----MOD-A40166---------
      IF l_rvu.rvu00='3' AND l_rvv17>0 AND cl_null(l_rvv32) AND 
...
#MOD-C30507----add---end---  
            ...
            IF s_icdbin(l_rvv31) THEN   #FUN-BA0051
              #CALL s_icdchk(1,l_rvv31,l_rvv32,l_rvv33,l_rvv34,      #MOD-B90265 mark
               CALL s_icdchk(l_type,l_rvv31,l_rvv32,l_rvv33,l_rvv34, #MOD-B90265
                               l_rvv17,l_rvu.rvu01,l_rvv02,l_rvu.rvu03,'')  #FUN-B80119--傳入p_plant參數''---
                    RETURNING l_flag
               IF l_flag = 0 THEN
                  LET g_success = 'N'
                  RETURN
   ...
   FOREACH t720_rvv26 INTO l_rvv02,l_rvv31,l_rvv32,l_rvv26 
      ...
      END IF 
   END FOREACH 
   IF g_prog<>'csfp001' THEN   #add by jixf 160801
      CALL s_showmsg() 
   END IF 
   IF g_success = 'N' THEN
      RETURN 
...
#FUN-D60130 -----End--------
  ...
  #DEV-D40015 add str-------------
  #若aimi100[條碼使用否]=Y且有勾選製造批號/製造序號，需控卡不可直接確認or過帳
   IF g_aza.aza131 = 'Y' AND (g_prog = 'apmt720' OR g_prog = 'apmt721') THEN
     #確認是否有符合條件的料件
      LET l_cnt = 0
      SELECT COUNT(*) INTO l_cnt
        FROM ima_file
     ...
     #IF l_cnt > 0 THEN                       #DEV-D40020 mark
      IF l_cnt > 0 AND l_scan_flag = 'N' THEN #DEV-D40020 add
...
#CHI-C40016---remark---END-----------------------------------------   
  ...
  #LET g_success = 'Y'   #FUN-A60009 mark   
   ...
   IF g_success = 'Y' THEN
     #2021112901 add----begin----
     IF cl_getscmparameter() AND l_rvu.rvu00 = '3' THEN 
       INITIALIZE l_ret TO NULL
       CALL cjc_zmx_json_task('PT1',l_rvu.rvu01) RETURNING l_ret.*
       IF l_ret.success = 'Y' THEN
       ELSE
          IF cl_null(l_ret.msg) THEN
              LET l_ret.msg = "仓退单(",l_rvu.rvu01 CLIPPED,")同步失败"
          END IF
   ...
   IF l_rva00 = '1' THEN
      ...
      IF l_pmm02 = 'SUB' AND l_sma131 = 'Y' AND (g_sma.sma1431='N' OR cl_null(g_sma.sma1431)) THEN #為委外入庫單且入庫后自動生成移轉資料  #FUN-A80102
         ...
         IF l_sfb93='Y' AND p_auto = 'Y' THEN #MOD-C80049 add
            CALL t720sub_ecm(l_rvu.rvu01)
         END IF
      END IF
   END IF
...
DEFINE l_rvu   RECORD LIKE rvu_file.*
    ...
    CALL t720sub_azp(l_rvu.rvu04) RETURNING l_dbs 
    LET l_dbs = s_dbstring(l_dbs CLIPPED)
    LET l_sql = "SELECT * FROM ",cl_get_target_table(l_rvu.rvu04,'pov_file'),  #FUN-A50102
                 " WHERE pov01 = '1'",
     ...
     LET l_sql = "SELECT * FROM ",cl_get_target_table(l_rvu.rvu04,'occ_file'), #FUN-A50102 
                " WHERE pmc01 = '",l_rvu.rvu04 CLIPPED,"'"
     CALL cl_replace_sqldb(l_sql) RETURNING l_sql               
     CALL cl_parse_qry_sql(l_sql,l_rvu.rvu04) RETURNING l_sql  
...
#DEV-D30045--add--begin
FUNCTION t720sub_barcode_gen(p_rvu01,p_ask)
   DEFINE p_rvu01   LIKE rvu_file.rvu01
   ...
   IF NOT s_tlfb_chk(l_rvu.rvu01) THEN
      LET g_success = 'N'
      RETURN
   END IF
...
FUNCTION t720sub_barcode2(p_rvu01,p_type,p_argv3)
   ...
   LET l_sql = "SELECT rvv_file.* ",
               ...
               "  FROM rvv_file,ima_file ",
               " WHERE rvv31 = ima01 ",
               "   AND rvv01 = '",p_rvu01,"'",           #條碼產生時機點(ima932)
               "   AND ima932 IN ('F','G','K','L','E') " #F:採購單(apmt540)
                                                         #G:委外採購單(apmt590)
                                                         #K:採購收貨(apmt110)
                                                         #L:IQC(aqct110)
                                                         #E:採購入庫(apmt720)
...
#str------add by guanyao160824
FUNCTION t720_sub_tc_shb(p_rvu01)
DEFINE p_rvu01        LIKE rvu_file.rvu01
DEFINE l_temp         RECORD 
        pmn18         LIKE pmn_file.pmn18,
        pmn32         LIKE pmn_file.pmn32,
...
DEFINE l_tmp          LIKE type_file.chr20
    ...
    BEGIN WORK 
    LET g_success = 'Y'
    LET l_sql = "SELECT pmn18,pmn32,SUM(rvv17),rvv35,rvv18", 
                "  FROM pmn_file,rvv_file ",
                " WHERE rvv36 = pmn01 AND rvv37 = pmn02",
                "   AND rvv01 = '",p_rvu01,"'",
                " GROUP BY pmn18,pmn32,rvv35,rvv18",
                " ORDER BY pmn18,pmn32"
    PREPARE t720_sub_tc_shb_p FROM l_sql
    DECLARE t720_sub_tc_shb_c CURSOR WITH HOLD FOR t720_sub_tc_shb_p
    INITIALIZE l_tc_shb.* TO NULL 
    INITIALIZE l_tc_shb.* TO NULL
    FOREACH t720_sub_tc_shb_c INTO l_temp.*
       IF STATUS THEN
          CALL cl_err('foreach rvbs200:',STATUS,1)
          EXIT FOREACH
       END IF
       SELECT sgm03_par,shm06,sgm04,sgm06,sgm58,CASE ta_ecd03 WHEN 'N' THEN 1 ELSE nvl(imaud10,1) END IF  
         INTO l_tc_shb.tc_shb05,l_tc_shb.tc_shb07,l_tc_shb.tc_shb08,  
              l_tc_shb.tc_shb09,l_tc_shb.tc_shb16,l_imaud10 
         FROM sgm_file,shm_file,ima_file,ecd_file                      
        WHERE sgm01 = l_temp.pmn18
          AND sgm03 = l_temp.pmn32
          AND sgm01 = shm01
          AND sgm03_par = ima01
          AND sgm04 = ecd01
       LET l_tc_shb.tc_shb01 = '1'
       LET l_tc_shb.tc_shb03 = l_temp.pmn18
       LET l_tc_shb.tc_shb04 = l_temp.rvv18
       LET l_tc_shb.tc_shb06 = l_temp.pmn32
       LET l_tc_shb.tc_shb10 = 'Y'
       LET l_tc_shb.tc_shb11 = tm_1.gen01
       LET l_tc_shb.tc_shb12 = l_temp.rvv17
       LET l_tc_shb.tc_shb13 = tm_1.ecg01
       LET l_tc_shb.tc_shb14 = ''
       LET l_tc_shb.tc_shb15 = ''
       LET l_tc_shb.tc_shb16 = l_temp.rvv35
       LET l_tc_shb.tc_shb17 = ''
       LET l_tc_shb.tc_shbud01 = ''
       LET l_tc_shb.tc_shbud03 = p_rvu01
       LET l_tc_shb.tc_shbud07 = ''
       LET l_tc_shb.tc_shbud08 = '' 
       LET l_tc_shb.tc_shbud09 = l_tc_shb.tc_shb12/l_imaud10
       ...
       IF STATUS THEN
          CALL cl_err3("ins","rvv_file",l_tc_shb.tc_shb01,l_tc_shb.tc_shb02,STATUS,"","ins rvv:",1)  #No.FUN-660129
          LET g_success='N'
          EXIT FOREACH
       END IF
       LET l_tc_shb.tc_shb01 = 2
       LET l_tc_shb.tc_shbud02 = l_tc_shb.tc_shb02
       LET l_y = '' LET l_m = '' LET l_d = '' LET l_str = '' LET l_tmp = ''
       LET l_y =YEAR(g_today)
       LET l_y = l_y[3,4] USING '&&' 
       LET l_m =MONTH(g_today)
       LET l_m = l_m USING '&&' 
       ...
       IF STATUS THEN
          ...
          EXIT FOREACH
       END IF
       LET l_tc_shc.tc_shc01 = '2'
       LET l_tc_shc.tc_shc03 = l_tc_shb.tc_shb03 
       LET l_tc_shc.tc_shc04 = l_tc_shb.tc_shb04 
       LET l_tc_shc.tc_shc05 = l_tc_shb.tc_shb05 
       LET l_tc_shc.tc_shc06 = l_tc_shb.tc_shb06
       LET l_tc_shc.tc_shc07 = l_tc_shb.tc_shb07
       LET l_tc_shc.tc_shc08 = l_tc_shb.tc_shb08
       LET l_tc_shc.tc_shc09 = l_tc_shb.tc_shb09
       LET l_tc_shc.tc_shc10 = l_tc_shb.tc_shb10
       LET l_tc_shc.tc_shc11 = l_tc_shb.tc_shb11
       LET l_tc_shc.tc_shc12 = l_tc_shb.tc_shb12
       LET l_tc_shc.tc_shc13 = l_tc_shb.tc_shb13
       LET l_tc_shc.tc_shc14 = l_tc_shb.tc_shb14
       LET l_tc_shc.tc_shc15 = l_tc_shb.tc_shb15
       LET l_tc_shc.tc_shc16 = l_tc_shb.tc_shb16
       LET l_tc_shc.tc_shcud09 = l_tc_shb.tc_shbud09
       LET l_y = '' LET l_m = '' LET l_d = '' LET l_str = '' LET l_tmp = ''
       LET l_y =YEAR(g_today)
       LET l_y = l_y[3,4] USING '&&' 
       LET l_m =MONTH(g_today)
       ...
       IF STATUS THEN
          CALL cl_err3("ins","rvv_file",l_tc_shc.tc_shc01,l_tc_shc.tc_shc02,STATUS,"","ins rvv:",1)  #No.FUN-660129
          LET g_success='N'
          EXIT FOREACH
       END IF
    ...
    IF g_success = 'Y' THEN 
       COMMIT WORK 
       CALL cl_err('','abm-019',1)      #批次作業正確結束
    ELSE 
       ROLLBACK WORK 
       CALL cl_err('','abm-020',1)      #批次作業失敗
    END IF 
