...
# Modify.........: No:130902     13/09/02 By SunLM 調整MOD-D30186的問題導致採購數量與請購數量差異百分比問題
# Modify.........: No:160523     16/05/23 By guanyao 增加发料单，或者收料
         ...
         WHEN "transf2scm"
            IF cl_chk_act_auth() THEN
                ...
                IF cl_getscmparameter() THEN
                    ...
                    CALL cjc_zmx_json_pnaorder(g_pna.pna01,g_pna.pna02)  RETURNING l_ret.*
                    IF l_ret.success = 'Y' THEN
                    ELSE
                       IF cl_null(l_ret.msg) THEN
                           LET l_ret.msg = "采购单变更(",g_pna.pna01 CLIPPED,")同步失败"
                       END IF
                    END IF
   ...
   DEFINE l_pmm906 LIKE pmm_file.pmm906 #CHI-D30023 add
      ...
      SELECT count(*) INTO l_n
        ...
        IF l_poz00 = '2' AND l_poz011 = '2' THEN 
           ...
           END IF
           IF l_cnt>0 THEN 
              CALL cl_set_comp_entry("pnb04a",FALSE) #131122
           END IF    
        END IF
        #-----END MOD-A90171-----
        IF g_sma.sma32='Y' THEN
...
#str-----add by guanyao160523
      ON ACTION fa_issue
         LET g_action_choice="fa_issue"  #MOD-580129
         EXIT DISPLAY
...
DEFINE l_psf    LIKE pna_file.pna01     #add by jiangrh 210712
   ...
   #FUN-C40089---end
      SELECT COUNT(*) INTO g_cnt FROM pnb_file
       WHERE pnb01 = g_pna.pna01
         ...
         AND pnb31a IS NOT NULL
         AND pnb31a <=0
      LET l_psf = g_pna.pna01[1,3] #add by jiangrh 210712
      IF g_cnt > 0 AND l_psf <> 'PSF' THEN  #add AND l_psf='PSF' by jiangrh 210712
...
FUNCTION t910_g()
  ...
  DEFINE l_pnb03_1      LIKE pnb_file.pnb03            #add by guanyao160707
  DEFINE l_pmm02        LIKE pmm_file.pmm02            #add by guanyao160707
 #No.18010101--begin--
   DEFINE l_ret        RECORD
             success   LIKE type_file.chr1,
             code      LIKE type_file.chr10,
...
#str------add by guanyao160707
  ...
  IF cl_getscmparameter() THEN
   INITIALIZE l_ret TO NULL
    ...
    ELSE
         IF cl_null(l_ret.msg) THEN
               LET l_ret.msg = "采购变更单(",g_pna.pna01 CLIPPED ,g_pna.pna02 CLIPPED,")同步失败"
         END IF
    END IF
    IF NOT cl_null(l_ret.msg) THEN 
     CALL cl_err(l_ret.msg,'!',1)  
     RETURN 
    END IF 
  END IF 
...
# IF l_pmm909 = '3' THEN                                                                 #MOD-B60122 mark
    ...
    SELECT pnb04b,pnb04a,pnb07b,pnb07a,pnb20a,pnb20b,pnb90,pnb91 #MOD-D30186 add
     FROM pna_file,pnb_file
      WHERE pna01 = pnb01 
       ...
       AND pnb90 = l_pnb90   #MOD-8C0196 #130902
       AND pnb91 = l_pnb91   #MOD-8C0196 #130902
       #MOD-D30186 mark end   -----
  FOREACH pnb_curs INTO 
    ...
    #MOD-D30186 add start -----130902 mark
...
#     END IF
  ...
  IF g_cnt = 1 THEN
     LET g_factor = 1
  END IF
 #MOD-D30186 mark start -----130902 add remove #
 ...
 #MOD-D30186 mark end   -----130902 add remove #
  
  IF g_retype = 0 THEN   #MOD-BB0334 add
    IF g_pnb[l_ac].pnb20a * g_factor > (l_old*(1+l_rate)-l_new)+l_old1 THEN  #MOD-D30186 mark #130902 add remove #
     #IF g_pnb[l_ac].pnb20a * g_factor > (l_old*(1+l_rate)-l_new) THEN         #MOD-D30186 add 130902 mark
        RETURN FALSE 
     ELSE
        RETURN TRUE
     END IF
  #MOD-BB0334 ----- modify start -----
  ELSE
    LET l_pnb20a = ((l_old-l_new)+l_old1)/g_factor #MOD-D30186 mark 130902 add remove #
    # LET l_pnb20a = ((l_old-l_new))/g_factor        #MOD-D30186 add 130902 add add #
...
DEFINE l_cmd   STRING 
   ...
   IF g_pna.pna05 = 'N' THEN 
      CALL cl_err('pna05=N','aap-717',0) 
      RETURN 
   END IF
   IF g_pna.pna05 = 'X' THEN 
      CALL cl_err('','9024',0) 
   ...
   LET l_cmd = ''
      LET l_cmd = "csft001",
                 ...
                 # " '",g_pnb[l_x].pnb03 CLIPPED,"'",  #mark by guanyao160810
                 '',
                   " '",g_pna.pna02,"'"  #add by guanyao160523   增加版本号
