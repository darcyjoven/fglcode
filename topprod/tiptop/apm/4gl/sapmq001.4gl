...
FUNCTION sapmq001(p_prog)
   DEFINE p_prog LIKE type_file.chr20

   OPEN WINDOW q001_w AT 5,10                        #str---add by huanglf160806
        WITH FORM "apm/42f/apmq001" ATTRIBUTE(STYLE = g_win_style)  
   CASE p_prog
      WHEN 'apmq001' 
         LET g_prog = p_prog
         LET g_pname = '1'
         CALL cl_set_locale_frm_name("apmq001")
      WHEN 'apmq002'
         LET g_prog = p_prog
         LET g_pname = '2'
         CALL cl_set_locale_frm_name("apmq002")
      WHEN 'apmq003'
         LET g_prog = p_prog
         LET g_pname = '3'
   ...
   CALL q001_table()   #free
   DELETE FROM apmq001_tmp
   DELETE FROM apmq001_tmp1
   CALL q001_additem()
   CALL q001_q()   
   CALL q001_menu()

   DROP TABLE apmq001_tmp;
   CLOSE WINDOW q001_w  
END FUNCTION 
...
FUNCTION q001_menu()
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         WHEN "pml"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_pml()
            END IF
            LET g_action_choice = " "
         WHEN "pmm"
            IF cl_chk_act_auth() THEN
               IF NOT cl_null(g_head.pmk01_2) THEN
                 CALL q001_b_fill_pmm()
               ELSE
                  CALL g_pmm.clear()
               END IF
            END IF
         ...
         WHEN "pmn"
            IF cl_chk_act_auth() THEN
               #待处理
               CALL q001_b_fill_pmn() 
            END IF
            LET g_action_choice = " "
         WHEN "rva"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_rva()
            END IF
            LET g_action_choice = " "   
         WHEN "rvb"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_rvb()
            END IF
            LET g_action_choice = " "  
         WHEN "qcs"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_qcs()
            END IF
            LET g_action_choice = " "
         WHEN "qct"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_qct()
            END IF
            LET g_action_choice = " "
         WHEN "rvu"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_rvu()
            END IF
            LET g_action_choice = " "
         WHEN "rvv"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_rvv()
            END IF
            LET g_action_choice = " "
         WHEN "rvu_y"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_rvu_y()
            END IF
            LET g_action_choice = " "
         WHEN "rvv_y"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_rvv_y()
            END IF
            LET g_action_choice = " "
         WHEN "rvu_c"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_rvu_c()
            END IF
            LET g_action_choice = " "
         WHEN "rvv_c"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_rvv_c()
            END IF
            LET g_action_choice = " "
         WHEN "apa"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_apa()
            END IF
            LET g_action_choice = " "
         WHEN "apb"
            IF cl_chk_act_auth() THEN
               CALL q001_b_fill_apb()
            END IF
            LET g_action_choice = " "
      END CASE
   END WHILE
...
DEFINE l_wc1   STRING 
   ...
   LET g_sql = "SELECT speed,state,pmk01,pmk02,pmk04,pmk12,gen02,pmk13,gem02,pmk09, ", 
               ...
               "  FROM apmq001_tmp WHERE 1=1 AND ",l_wc1
      
   LET g_sql = g_sql," ORDER BY pmk01,pmk02,pmk04,pmk12,pmk13,pmk09,pml04,pml12 "
  
   
   PREPARE apmq001_pb_cs FROM g_sql
   DECLARE pmk_curs  CURSOR FOR apmq001_pb_cs        #CURSOR
...
FUNCTION q001_b_fill_pmk()              #BODY FILL UP
    ...
    PREPARE q001_pmk1 FROM g_sql
    DECLARE pmk_curs1 CURSOR FOR q001_pmk1
...
FUNCTION q001_b_fill_pml()              #BODY FILL UP
...
FUNCTION q001_b_fill_pmm()              #BODY FILL UP
...
FUNCTION q001_b_fill_pmn()              #BODY FILL UP
...
FUNCTION q001_b_fill_rva()              #BODY FILL UP
    ...
    PREPARE q001_rva FROM g_sql
    DECLARE rva_curs CURSOR FOR q001_rva
...
FUNCTION q001_b_fill_rvb()              #BODY FILL UP
    ...
    PREPARE q001_rvb FROM g_sql
    DECLARE rvb_curs CURSOR FOR q001_rvb
...
FUNCTION q001_b_fill_qcs()              #BODY FILL UP
    ...
    PREPARE q001_qcs FROM g_sql
    DECLARE qcs_curs CURSOR FOR q001_qcs
...
FUNCTION q001_b_fill_qct()              #BODY FILL UP
    ...
    PREPARE q001_qct FROM g_sql
    DECLARE qct_curs CURSOR FOR q001_qct
...
FUNCTION q001_b_fill_rvu()              #BODY FILL UP
    ...
    PREPARE q001_rvu FROM g_sql
    DECLARE rvu_curs CURSOR FOR q001_rvu
...
FUNCTION q001_b_fill_rvv()              #BODY FILL UP
    ...
    PREPARE q001_rvv FROM g_sql
    DECLARE rvv_curs CURSOR FOR q001_rvv
...
#add str
FUNCTION q001_b_fill_rvu_y()              #BODY FILL UP
    ...
    PREPARE q001_rvu_y FROM g_sql
    DECLARE rvu_y_curs CURSOR FOR q001_rvu_y
...
FUNCTION q001_b_fill_rvv_y()              #BODY FILL UP
    ...
    PREPARE q001_rvv_y FROM g_sql
    DECLARE rvv_y_curs CURSOR FOR q001_rvv_y
...
FUNCTION q001_b_fill_rvu_c()              #BODY FILL UP
    ...
    PREPARE q001_rvu_c FROM g_sql
    DECLARE rvu_c_curs CURSOR FOR q001_rvu_c
...
FUNCTION q001_b_fill_rvv_c()              #BODY FILL UP
    ...
    PREPARE q001_rvv_c FROM g_sql
    DECLARE rvv_c_curs CURSOR FOR q001_rvv_c
...
END FUNCTION
#add end
FUNCTION q001_b_fill_apa()              #BODY FILL UP
    ...
    PREPARE q001_apa FROM g_sql
    DECLARE apa_curs CURSOR FOR q001_apa
...
FUNCTION q001_b_fill_apb()              #BODY FILL UP
    ...
    CASE g_pname 
       ...
       OTHERWISE
          ...
          END IF 
    END CASE    
    PREPARE q001_apb FROM g_sql
    DECLARE apb_curs CURSOR FOR q001_apb
...
FUNCTION q001_table()
   ...
   PREPARE q001_tab FROM l_sql
   EXECUTE q001_tab

   LET l_sql = 
   "CREATE TEMP TABLE apmq001_tmp1( ",
             "   speed     LIKE type_file.chr1, ",
             "   state     LIKE type_file.chr1, ",
             "   pmk01     LIKE pmk_file.pmk01, ",
             "   pmk02     LIKE pmk_file.pmk02, ",
...
FUNCTION q001()
   ...
   LET l_sql = " INSERT INTO apmq001_tmp ",l_sql CLIPPED
   ...
   #   IF cl_null(sr.apb09) THEN LET sr.apb09=0 END IF 
   #   IF cl_null(sr.apb24) THEN LET sr.apb24=0 END IF 
   #   INSERT INTO apmq001_tmp VALUES(sr.*)
   #END FOREACH 
   #free--mark--end--
   IF g_pname='1' THEN 
      ...
      LET l_sql = " INSERT INTO apmq001_tmp1 ",l_sql CLIPPED
      PREPARE q001_ins1 FROM l_sql
      EXECUTE q001_ins1

      #rvv39,39t
      LET l_sql = 
          " MERGE INTO apmq001_tmp1 o ",
      ...
      LET l_sql = " INSERT INTO apmq001_tmp ",
                  " SELECT * FROM apmq001_tmp1 "
      ...
      END IF
   #free--mark--str--
   #   PREPARE q001_prepare_sfb FROM l_sql
   #   DECLARE q001_curs_sfb CURSOR FOR q001_prepare_sfb
               
   #   DECLARE q001_cs_sfb                         #SCROLL CURSOR
   #           SCROLL CURSOR FOR q001_prepare_sfb
   #   OPEN q001_cs_sfb
   #   FETCH FIRST q001_cs_sfb INTO sr.*
   #   IF SQLCA.SQLCODE THEN 
   #      LET g_int_flag = '1'
   #   END IF   
   #   INITIALIZE sr.* TO NULL 
   #   FOREACH q001_curs_sfb INTO sr.*
   #      IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
   #         IF cl_null(sr.pmk01) THEN LET sr.pmk01='' END IF 
   #         IF cl_null(sr.pmk02) THEN LET sr.pmk02='' END IF 
   #         IF cl_null(sr.pmk04) THEN LET sr.pmk04='' END IF 
   ...
   END IF 
      IF tm.h <> '2' THEN
         LET l_sql = " DELETE FROM apmq001_tmp o ",
                     "  WHERE o.speed != '",tm.h,"' "
...
DEFINE     l_sql    STRING
   ...
   FOREACH q001_curs1 INTO g_pmk_1[g_cnt].*
      IF SQLCA.sqlcode THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
   ...
   CASE tm.a 
      WHEN "1" 
         ...
         ELSE 
            LET l_tmp2 = ''
         END IF 
         LET l_sql = "SELECT * FROM apmq001_tmp WHERE (pmk01='",g_pmk_1[p_ac].pmk01_1,"' ",l_tmp2," )",
      ...
      WHEN "2"
         ...
         ELSE 
            LET l_tmp2 = ''
         END IF 
         LET l_sql = "SELECT * FROM apmq001_tmp WHERE (pmk02='",g_pmk_1[p_ac].pmk02_1,"' ",l_tmp2," )",
                     " AND (pmm22 = '",g_pmk_1[p_ac].pmm22_1 CLIPPED ,"'",l_tmp," )",
                     " ORDER BY pmk02,pmm22"
      WHEN "3"
         ...
         ELSE 
            LET l_tmp2 = ''
         END IF 
         LET l_sql = "SELECT * FROM apmq001_tmp WHERE (pmk04='",g_pmk_1[p_ac].pmk04_1,"' ",l_tmp2," )",
                     " AND (pmm22 = '",g_pmk_1[p_ac].pmm22_1 CLIPPED ,"'",l_tmp," )",
                     " ORDER BY pmk04,pmm22"
      WHEN "4"
         ...
         ELSE 
            LET l_tmp2 = ''
         END IF 
         LET l_sql = "SELECT * FROM apmq001_tmp WHERE (pmk12='",g_pmk_1[p_ac].pmk12_1,"' ",l_tmp2," )",
                     " AND (pmm22 = '",g_pmk_1[p_ac].pmm22_1 CLIPPED ,"'",l_tmp," )",
      ...
      WHEN "5"
         ...
         ELSE 
            LET l_tmp2 = ''
         END IF 
         LET l_sql = "SELECT * FROM apmq001_tmp WHERE (pmk13 = '",g_pmk_1[p_ac].pmk13_1,"' ",l_tmp2," )",
                     " AND (pmm22 = '",g_pmk_1[p_ac].pmm22_1 CLIPPED ,"'",l_tmp," )",
      ...
      WHEN "6"
         ...
         ELSE 
            LET l_tmp2 = ''
         END IF 
         LET l_sql = "SELECT * FROM apmq001_tmp WHERE (pmk09 = '",g_pmk_1[p_ac].pmk09_1,"' ",l_tmp2," )",
                     " AND (pmm22 = '",g_pmk_1[p_ac].pmm22_1 CLIPPED ,"'",l_tmp," )",
      ...
      WHEN "7"
         ...
         ELSE 
            LET l_tmp2 = ''
         END IF 
         LET l_sql = "SELECT * FROM apmq001_tmp WHERE (pml04 = '",g_pmk_1[p_ac].pml04_1,"' ",l_tmp2," )",
                     " AND (pmm22 = '",g_pmk_1[p_ac].pmm22_1 CLIPPED ,"'",l_tmp," )",
      ...
      WHEN "8"
         ...
         ELSE 
            LET l_tmp2 = ''
         END IF 
         LET l_sql = "SELECT * FROM apmq001_tmp WHERE (pml12 = '",g_pmk_1[p_ac].pml12_1,"' ",l_tmp2," )",
                     " AND (pmm22 = '",g_pmk_1[p_ac].pmm22_1 CLIPPED ,"'",l_tmp," )",
                     " ORDER BY pml12,pmm22"
   END CASE

      PREPARE apmq001_pb_detail FROM l_sql
      DECLARE pmk_curs_detail  CURSOR FOR apmq001_pb_detail        #CURSOR
      CALL g_pmk.clear()
      CALL g_pmk_excel.clear()
      LET g_cnt = 1
      LET g_rec_b = 0
...
FUNCTION q001_bp3()
   ...
   CASE g_b_flag2
      ...
      OTHERWISE
         CASE g_pname 
            WHEN '2'  CALL q001_bp_po()
            WHEN '3'  CALL q001_bp_chu()
            OTHERWISE CALL q001_bp_pr()
         END CASE 
   END CASE
