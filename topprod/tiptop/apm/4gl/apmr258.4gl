...
# Modify.........: No.MOD-840621 08/04/23 By Carol 修正r258_cs2 end foreach 的位置
...
GLOBALS "../../config/top.global"
    ...
    DEFINE tm  RECORD				# Print condition RECORD
                 ...
                 u    	LIKE type_file.chr3,    #判斷同項目資料列印完則 "合計"   #No.FUN-680136 VARCHAR(3) chr4  
...
## ***  與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>> TSD.Martin  *** ##
    ...
    LET l_table = cl_prt_temptable('apmr258',g_sql) CLIPPED   # 產生Temp Table
...
#----------------------------------------------------------CR (1) ------------#
    ...
    LET tm.mm = ARG_VAL(16)
    LET tm.type = ARG_VAL(17)  #CHI-9C0025
   ...
   LET g_rpt_name = ARG_VAL(21)  #No.FUN-7C0078
    ...
    ELSE
        CALL apmr258()		# Read data and create out-file
    END IF
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
    ...
    OPEN WINDOW r258_w AT p_row,p_col WITH FORM "apm/42f/apmr258"
...
END CONSTRUCT
    ...
    IF tm.wc = " 1=1 " THEN
       ...
       CONTINUE WHILE
    END IF
    DISPLAY BY NAME tm.pmm04a,tm.pmm04b,tm.s,tm.t,tm.u,tm.a,tm.b,
    ...
    IF g_bgjob = 'Y' THEN
        ...
        ELSE
         LET tm.wc=cl_replace_str(tm.wc, "'", "\"")
            ...
            CALL cl_cmdat('apmr258',g_time,l_cmd)      # Execute cmd at later time
        END IF
        CLOSE WINDOW r258_w
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690119
        EXIT PROGRAM
...
#          l_sql 	LIKE type_file.chr1000,	      # RDSQL STATEMENT                 #No.FUN-680136 VARCHAR(1000)   #TQC-B30073 
    ...
    LET l_sql = " SELECT '','', ",
                "   pmm09,pmc03,pmn04,ima02,'','',0,0,0,0,0,0,'','' ",                          ##NO.FUN-610092 #CHI-8C0017 Add pmm02,pmn18,pmn41,pmn43 #MOD-940403 move to cs2
    ...
    #-----END MOD-A50199-----
       LET l_sql =
                  ...
                  " ORDER BY pmm04 DESC "
    END IF   #MOD-A50199
    PREPARE r258_prepare2 FROM l_sql
    ...
    END IF
    DECLARE r258_cs2 CURSOR FOR r258_prepare2
 
    LET g_pageno = 0
    FOREACH r258_cs1 INTO sr.*                                             #CHI-8C0017 Add l_pmm02,l_pmn18,l_pmn41,l_pmn43  #MOD-940403 move to cs2
        IF SQLCA.sqlcode THEN
        ...
        FOREACH r258_cs2  USING sr.pmm09,sr.pmn04
        ...
        IF tm.a = 'N' OR (tm.a='Y' AND
           ...
           EXECUTE insert_prep USING 
                   sr.pmm09,sr.pmc03,sr.pmn04,sr.ima02,sr.pmm01,
    ...
    IF g_zz05 = 'Y' THEN
       ...
       LET g_str = tm.wc
    END IF
    LET g_str = g_str,";",tm.s[1,1],";",tm.s[2,2],";",tm.t,";",tm.u,";",tm.b
                     ,";",g_azi03,";",g_azi04,";",g_azi05   #FUN-710080 add
    CALL cl_prt_cs3('apmr258','apmr258',l_sql,g_str)        #FUN-710080 modify
    #------------------------------ CR (4) ------------------------------#
