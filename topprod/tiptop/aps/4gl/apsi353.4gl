...
#
# Pattern name...: apsi353.4gl
# Descriptions...: MDS沖銷-業務員優先序設定作業
...
DEFINE p_row,p_col   LIKE type_file.num5      
   ...
   OPEN WINDOW i353_w AT p_row,p_col WITH FORM "aps/42f/apsi353"  ATTRIBUTE (STYLE = g_win_style CLIPPED) 
   CALL cl_ui_init()
   CALL cl_set_comp_visible("vlpacti",FALSE)
   LET g_vlp01 = '3'
   LET g_wc2 = '1=1' CALL i353_b_fill(g_wc2)
   CALL i353_menu()
   CLOSE WINDOW i353_w                 #結束畫面
...
FUNCTION i353_menu()
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         WHEN "auto_gen_sales"
            IF cl_chk_act_auth() THEN 
               CALL cl_cmdrun_wait("apsp353")
               CALL i353_b_fill(' 1=1')
            END IF
...
DEFINE
    ...
    DECLARE i353_bcl CURSOR FROM g_forupd_sql      # LOCK CURSOR
      ...
      ATTRIBUTE (COUNT=g_rec_b,MAXCOUNT=g_max_rec,UNBUFFERED,
        ...
        BEFORE ROW
            ...
            IF g_rec_b>=l_ac THEN
               ...
               OPEN i353_bcl USING g_vlp01,g_vlp_t.vlp03
               IF STATUS THEN
                  CALL cl_err("OPEN i353_bcl:", STATUS, 1)
                  LET l_lock_sw = "Y"
               ELSE 
                  FETCH i353_bcl INTO g_vlp[l_ac].* 
...
FUNCTION i353_vlp03(p_cmd)
   ...
   SELECT   gen02,  gen03,  genacti,  gem02
     INTO l_gen02,l_gen03,l_genacti,l_gem02
     FROM gen_file,OUTER gem_file
    WHERE gen01 = g_vlp[l_ac].vlp03
   ...
   CASE WHEN SQLCA.SQLCODE = 100       LET g_errno = 'mfg1312'
        ...
        WHEN l_genacti MATCHES '[PH]'  LET g_errno = '9038'
        OTHERWISE                      LET g_errno = SQLCA.SQLCODE USING '-------'
   END CASE

   IF cl_null(g_errno) OR p_cmd = 'd' THEN
      LET g_vlp[l_ac].gen02  = l_gen02
      LET g_vlp[l_ac].gen03 = l_gen03
      LET g_vlp[l_ac].gem02  = l_gem02
      DISPLAY BY NAME g_vlp[l_ac].gen02
      DISPLAY BY NAME g_vlp[l_ac].gen03
      DISPLAY BY NAME g_vlp[l_ac].gem02
   END IF

END FUNCTION

FUNCTION i353_b_askkey()
   ...
   END IF
    ...
    CALL i353_b_fill(g_wc2)

END FUNCTION

FUNCTION i353_b_fill(p_wc2)              #BODY FILL UP
DEFINE
    p_wc2           LIKE type_file.chr1000   #No:FUN-680102 VARCHAR(200)

    LET g_sql =
        "SELECT vlp02,vlp03,gen02,gen03,gem02,vlpacti",
        " FROM vlp_file,OUTER gen_file,OUTER gem_file",
        " WHERE vlp03 = gen_file.gen01 ",
        "   AND vlp01 = '",g_vlp01,"'",
        "   AND gen03 = gem_file.gem01 ",
        "   AND ", p_wc2 CLIPPED,                     #單身
        " ORDER BY vlp02,vlp03"
    PREPARE i353_pb FROM g_sql
    DECLARE vlp_curs CURSOR FOR i353_pb
...
FUNCTION i353_bp(p_ud)
   ...
   DISPLAY ARRAY g_vlp TO s_vlp.* ATTRIBUTE(COUNT=g_rec_b)
      ...
      #匯入員工
      ON ACTION auto_gen_sales
         LET g_action_choice="auto_gen_sales"
         EXIT DISPLAY
      ...
      #依員工編號排序
      ON ACTION reorder_vlp_employee
         LET g_action_choice="reorder_vlp_employee"
         EXIT DISPLAY

      #依業務排序
      ON ACTION reorder_vlp_5
         LET g_action_choice="reorder_vlp_5"
         EXIT DISPLAY
