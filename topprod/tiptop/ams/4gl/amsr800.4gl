...
#                "plan.mps_file.mps065 "      #No.TQC-950116
    ...
    LET l_table = cl_prt_temptable('amsr800',g_sql) CLIPPED   # 產生Temp Table
...
#----------------------------------------------------------CR (1) ------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amsr800_tm(0,0)        # Input print condition
      ELSE CALL amsr800()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amsr800_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01    #No.FUN-580031
   ...
   END IF
   OPEN WINDOW amsr800_w AT p_row,p_col WITH FORM "ams/42f/amsr800" 
...
#No.FUN-570240 --end-- 
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amsr800_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amsr800',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amsr800_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
# ------------------------------------------------------ CR (2) ------- ##
    ...
    SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog   ### FUN-740078 add ###
     ...
     LET g_pageno = 0
     PREPARE amsr800_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amsr800_curs1 CURSOR FOR amsr800_prepare1
     FOREACH amsr800_curs1 INTO l_mps01
   ...
   LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED   #FUN-740078 modify
   #是否列印選擇條件
   IF g_zz05 = 'Y' THEN
   ...
   CALL cl_prt_cs3('amsr800','amsr800',l_sql,g_str)   
   #------------------------------ CR (4) ------------------------------#
   #end FUN-740078 add 
END FUNCTION
 
REPORT amsr800_rep(sr)
