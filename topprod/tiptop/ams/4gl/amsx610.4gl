...
MAIN
   ...
   LET g_sql  = "rqa01.rqa_file.rqa01,",
            ...
            #FUN-CB0003--add--str--
                "l_rpf.type_file.chr1000,",
                "diff.rqa_file.rqa05,",
                "diffpr.rqa_file.rqa05,",
                "l_num.type_file.num5,",
                "flag1.type_file.chr1"
           #FUN-CB0003--add--end--
   LET l_table = cl_prt_temptable('amsx610',g_sql) CLIPPED          # 產生Temp Table
...
#  LET g_sql = "INSERT INTO ds_report.",l_table CLIPPED,            # TQC-780054
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'    # If background job sw is off
      ...
      ELSE CALL amsx610()        # Read data and create out-file
   END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         LET tm.wc=cl_replace_str(tm.wc, "'", "\"")
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
             ...
             CALL cl_cmdat('amsx610',g_time,l_cmd)   # Execute cmd at later time
         END IF
         CLOSE WINDOW x610_w #後來自己加的
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
         EXIT PROGRAM        #後來自己加的
      END IF
      CALL cl_wait()
...
#       l_time      LIKE type_file.chr8        #No.FUN-6A0081
      ...
      sr        RECORD #要列印出來的資料放在sr
        ...
        rpf03   LIKE rpf_file.rpf03 #產能單位
            END RECORD
#FUN-CB0003--add--str--
    DEFINE l_rpf     LIKE type_file.chr1000,
           l_diff    LIKE rqa_file.rqa05,
           l_diffpr  LIKE rqa_file.rqa05,
           l_num     LIKE type_file.num5,
           l_flag    LIKE type_file.chr1
    DEFINE rqa01_t  LIKE rqa_file.rqa01
#FUN-CB0003--add--end--
   #str FUN-740078 add
   ...
   IF SQLCA.sqlcode THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE x610_cs1 CURSOR FOR x610_prepare1
#   CALL cl_outnam('amsx610') RETURNING l_name #FUN-8B0002
   LET g_pageno = 0
   FOREACH x610_cs1 INTO sr.*
      IF SQLCA.sqlcode THEN
...
#FUN-CB0003--add--str--
      IF sr.rpf03 = '1' THEN 
         LET l_rpf = cl_getmsg('lib-071',g_lang)
      ELSE
         LET l_rpf = cl_getmsg('lib-072',g_lang)
      END IF
      ...
      END IF
      LET l_num = 0
      IF NOT cl_null(rqa01_t) THEN
         IF sr.rqa01 != rqa01_t THEN 
            LET l_flag = "Y"
         ELSE
            LET l_flag = "N"
         END IF
...
###XtraGrid###   LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED  
   ...
   IF g_zz05 = 'Y' THEN
      ...
      LET g_str = tm.wc
   END IF
###XtraGrid###   LET g_str = g_str ,";",tm.ver_no
###XtraGrid###   CALL cl_prt_cs3('amsx610','amsx610',l_sql,g_str)  
    LET g_xgrid.table = l_table    ###XtraGrid###
#FUN-CB0003--add--str--
    LET g_xgrid.grup_field = 'rqa01,rqa03'
    LET g_xgrid.order_field = 'rqa01,rqa03'
