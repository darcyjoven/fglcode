...
# Modify.........: No.TQC-D20032 13/02/20 By xuxz wc，l_sql定義為string
...
DEFINE  tm RECORD                           # Print condition RECORD
              #wc      LIKE type_file.chr1000,  #NO.FUN-680101 VARCHAR(600)     # Where condition#TQC-D20032 mark
               wc      STRING,#TQC-D20032 add
...
#No.FUN-770012 -- begin --
   ...
   LET l_table = cl_prt_temptable('amsr512',g_sql) CLIPPED
...
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW amsr512_w AT p_row,p_col
        WITH FORM "ams/42f/amsr512"
...
#No.FUN-570240 --end--
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amsr512',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amsr512_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0081
         #l_sql     LIKE type_file.chr1000, #NO.FUN-680101 VARCHAR(1000)    # RDSQL STATEMENT#TQC-D20032 mark
          l_sql     STRING,#TQC-D20032 add
...
#----No.MOD-580294 mark
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'amsr512'
...
#-----end
     ...
     END CASE
     DISPLAY 'l_sql:',l_sql
     PREPARE amsr512_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM
     END IF
     DECLARE amsr512_curs1 CURSOR FOR amsr512_prepare1
 
#No.FUN-770012 -- begin --
#     CALL cl_outnam('amsr512') RETURNING l_name
#     START REPORT amsr512_rep TO l_name
#     LET g_pageno = 0
#No.FUN-770012 -- end --
     FOREACH amsr512_curs1 INTO mps.*, ima.*
       ...
       DECLARE amsr512_curs2 CURSOR FOR
          SELECT * FROM mpt_file
            WHERE mpt01=mps.mps01 AND mpt03=mps.mps03
              AND mpt_v=mps.mps_v
       FOREACH amsr512_curs2 INTO mpt.*
         ...
         END CASE
#No.FUN-770012 -- begin --
#         OUTPUT TO REPORT amsr512_rep(mps.*, mpt.*, sr.*)
...
#     CALL cl_prt(l_name,g_prtway,g_copies,g_len)
       ...
       #--------------No.MOD-940413 end
        ...
        IF (mpt.mpt05 MATCHES '4*') OR (mpt.mpt05 = '53' AND mpt.mpt08 < 0) OR
          #(mpt.mpt05 = '71' AND mpt.mpt08 < 0) THEN                          #No.MOD-940413 mark
           ...
           LET demand.mpt05=mpt.mpt05,' ',l_buf
  #         LET demand.mpt06=mpt.mpt06[1,10],' ',mpt.mpt061 USING '###'        #TQC-AB0408 mark
        ...
        ELSE
           ...
           LET supply.mpt05=mpt.mpt05,' ',l_buf
  #         LET supply.mpt06=mpt.mpt06[1,10],' ',mpt.mpt061 USING '###'        #TQC-AB0408 mark
        ...
        EXECUTE insert_prep1 USING mps.mps01,sr.ima02,sr.ima25,mps.mps00,mps.mps03,sr.order1,
                                   ...
                                   supply.mpt05,supply.mpt06,supply.mpt07,supply.mpt08
        IF STATUS THEN
     ...
     CALL cl_wcchp(tm.wc,'mps01,ima08,ima67,ima43,mps03')
          RETURNING tm.wc
     LET g_str = tm.wc,";",g_zz05,";",g_azi03,";",g_azi04,";",g_azi05
     LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED
     CALL cl_prt_cs3('amsr512','amsr512',g_sql,g_str)
#No.FUN-770012 -- end --
END FUNCTION
...
FUNCTION r512_bom1(p_level,p_key,p_key2)       #FUN-550110
   DEFINE p_level	LIKE type_file.num5,   #NO.FUN-680101 SMALLINT
          ...
          END RECORD,
         #l_sql		LIKE type_file.chr1000 #NO.FUN-680101 VARCHAR(1000)#TQC-D20032 mark
          l_sql         STRING #TQC-D20032 add
...
FUNCTION r512_bom2(p_level,p_key,p_key2)        #FUN-550110
   DEFINE p_level	LIKE type_file.num5,    #NO.FUN-680101
          ...
          END RECORD,
         #l_sql		LIKE type_file.chr1000  #NO.FUN-680101#TQC-D20032 mark
          l_sql         STRING #TQC-D20032 add
   DEFINE l_ima910    DYNAMIC ARRAY OF LIKE ima_file.ima910          #No.FUN-8B0035 
