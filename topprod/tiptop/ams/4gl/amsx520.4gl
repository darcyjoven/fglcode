...
#
# Pattern name...: amsx520.4gl
# Descriptions...: MPS 採購/工單建議表(依料號)
# Input parameter: 
# Return code....: 
...
# Modify.........: No.MOD-640366 06/04/010 By pengu PML/PLP量轉"拆成"PML/PLP"與"轉"兩個欄位
...
# Modify.........: No.FUN-CB0001 12/11/02 By yangtt CR轉XtraGrid
# Modify.........: No.FUN-D40129 13/05/06 By yangtt 新增規格欄位ima021
...
### *** FUN-850082 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>>--*** ##                                                     
    ...
    LET l_table = cl_prt_temptable('amsx520',g_sql) CLIPPED   # 產生Temp Table                                                      
...
#----------------------------------------------------------CR (1) ------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amsx520_tm(0,0)        # Input print condition
      ELSE CALL amsx520()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amsx520_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01     #No.FUN-580031
   ...
   OPEN WINDOW amsx520_w AT p_row,p_col
        WITH FORM "ams/42f/amsx520" 
################################################################################
...
WHILE TRUE
   CONSTRUCT BY NAME tm.wc ON mps01,ima08,ima67,ima43,mps03,mps11 
#No.FUN-570240 --start--                                                                                    
         #No.FUN-580031 --start--
         BEFORE CONSTRUCT
             CALL cl_qbe_init()
      ...
      ON ACTION CONTROLP                                                                                              
           #IF INFIELD(mps01) THEN        #FUN-CB0001 mark                                                                                          
         CASE   #FUN-CB0001
            WHEN INFIELD(mps01)    #FUN-CB0001
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amsx520_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amsx520',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amsx520_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0081
          ...
          l_za05    LIKE type_file.chr1000,#NO.FUN-680101 VARCHAR(400)
   ...
   #------------------------------ CR (2) ------------------------------#
     ...
     PREPARE amsx520_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amsx520_curs1 CURSOR FOR amsx520_prepare1
 
#    CALL cl_outnam('amsx520') RETURNING l_name                                           #No.FUN-850082
#    START REPORT amsx520_rep TO l_name                                                   #No.FUN-850082
#    LET g_pageno = 0                                                                     #No.FUN-850082
     FOREACH amsx520_curs1 INTO mps.*, ima.*
...
#                                                        USING 'yyyymmdd'
#          WHEN tm.s[g_cnt,g_cnt]='6' LET l_order[g_cnt]=mps.mps11
#                                                        USING 'yyyymmdd'
...
#      OUTPUT TO REPORT amsx520_rep(sr.*,mps.*, ima.*)
     ...
     ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-850082 *** ##                                                      
         EXECUTE insert_prep USING                                                                                                  
                 ...
                 ima.ima02,ima.ima021,ima.ima25,ima.ima08,ima.ima67,ima.ima43    #FUN-D40129 add ima.ima021
     #------------------------------ CR (3) ------------------------------#
 
     END FOREACH
 
#    FINISH REPORT amsx520_rep                                                             #No.FUN-850082  
 
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)                                           #No.FUN-850082     
 
#No.FUN-850082--begin                                                                                                               
      IF g_zz05 = 'Y' THEN                                                                                                          
         CALL cl_wcchp(tm.wc,'mps01,ima08,ima67,ima43,mps03,mps11')                                               
              RETURNING tm.wc                                                                                                       
      END IF                                                                                                                        
#No.FUN-850082--end                                                                                                                 
...
###XtraGrid###                tm.t[1,1],";",tm.t[2,2],";",tm.t[3,3],";",tm.ver_no                                                                               
###XtraGrid###    CALL cl_prt_cs3('amsx520','amsx520',g_sql,g_str)                                                                                
    ...
    IF g_zz05 = 'Y' THEN
         CALL cl_wcchp(tm.wc,'mps01,ima08,ima67,ima43,mps03,mps11')
              RETURNING tm.wc
      END IF
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"mps01,ima08,ima67,ima43,mps03,mps11")
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"mps01,ima08,ima67,ima43,mps03,mps11")
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"mps01,ima08,ima67,ima43,mps03,mps11")
    LET g_xgrid.footerinfo1 = cl_getmsg('azz-211',g_lang),':',tm.ver_no
...
#           g_x[35] CLIPPED,g_x[36] CLIPPED,g_x[37] CLIPPED,g_x[38] CLIPPED  #No.MOD-640366 add
#     PRINT g_dash1
#     LET l_last_sw = 'n'
#  
...
#  ON EVERY ROW
#    #LET l_date=mps.mps10,' ',mps.mps11    #No.MOD-640366 mark
#     PRINT COLUMN g_c[34],mps.mps00 USING '###&',#FUN-590118
#           COLUMN g_c[35],mps.mps03 CLIPPED,
#         #----------------No.MOD-640366 modify
#           COLUMN g_c[36],cl_numfor(mps.mps09,36,2),
#           COLUMN g_c[37],mps.mps10,
#           COLUMN g_c[38],mps.mps11
#         #----------------No.MOD-640366 end
