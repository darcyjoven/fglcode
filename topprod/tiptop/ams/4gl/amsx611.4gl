...
#NO.FUN-850045--start---
   LET g_sql ="rqa01.rqa_file.rqa01,",
              ...
              "diff.rqa_file.rqa05,",    #FUN-CB0001 add
              "diffpr.rqa_file.rqa05,",  #FUN-CB0001 add
              "l_num.type_file.num5"    #FUN-CB0001 add
   LET l_table = cl_prt_temptable('amsx611',g_sql) CLIPPED
...
#NO.FUN-850045--end-----
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'    # If background job sw is off
      ...
      ELSE CALL amsx611()        # Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn        LIKE gbm_file.gbm01     #No.FUN-580031
   ...
   END IF
   OPEN WINDOW x611_w AT p_row,p_col
        WITH FORM "ams/42f/amsx611"
################################################################################
      ...
      IF g_bgjob = 'Y' THEN
         ...
         LET tm.wc=cl_replace_str(tm.wc, "'", "\"")
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
             ...
             CALL cl_cmdat('amsx611',g_time,l_cmd)   # Execute cmd at later time
         END IF
         CLOSE WINDOW x611_w #後來自己加的
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
         EXIT PROGRAM        #後來自己加的
      END IF
      CALL cl_wait()
...
#       l_time      LIKE type_file.chr8        #No.FUN-6A0081
      ...
      END RECORD
   DEFINE diff     LIKE rqa_file.rqa05    #FUN-CB0001 add
   DEFINE diffpr   LIKE rqa_file.rqa05    #FUN-CB0001 add
...
#   CALL cl_outnam('amsx611') RETURNING l_name              #NO.FUN-850045
#   START REPORT x611_rep TO l_name                         #NO.FUN-850045
#   LET g_pageno = 0                                        #NO.FUN-850045
   FOREACH x611_cs1 INTO sr.*
      IF SQLCA.sqlcode THEN
         ...
         EXIT FOREACH
      END IF
#      OUTPUT TO REPORT x611_rep(sr.*)                      #NO.FUN-850045
#NO.FUN-850045--start---
      LET l_num = 0  #FUN-CB0001
      LET diff = sr.rqa05 - sr.rqa06  #FUN-CB0001
      LET diffpr = sr.rqa06 / sr.rqa05 * 100  #FUN-CB0001
      EXECUTE insert_prep USING
        sr.rqa01,sr.rqa03,sr.rqa04,sr.rqa05,sr.rqa06,sr.rpf02,sr.rpf03,
        sr.rqb06,sr.rqb08,sr.rqb09,diff,diffpr,l_num   #FUN-CB0001 diff,diffpr,l_num
#NO.FUN-850045--end-----
   END FOREACH
#   FINISH REPORT x611_rep                                  #NO.FUN-850045
...
###XtraGrid###     LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog     #FUN-CB0001
     IF g_zz05 = 'Y' THEN 
        CALL cl_wcchp(tm.wc,'rqa01')
           RETURNING tm.wc
     END IF
###XtraGrid###     LET g_str = tm.wc
###XtraGrid###     CALL cl_prt_cs3('amsx611','amsx611',g_sql,g_str) 
    ...
    SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog     #FUN-CB0001
     IF g_zz05 = 'Y' THEN
        CALL cl_wcchp(tm.wc,'rqa01')
           RETURNING tm.wc
     END IF
    LET g_xgrid.order_field = "rqa01,rqa03,rqb06"
    LET g_xgrid.grup_field = 'rqa01'
