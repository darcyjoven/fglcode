...
# Modify.........: No.FUN-CB0001 12/11/04 By yangtt CR轉XtraGrid
...
### *** FUN-850082 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>>--*** ##                                                     
    ...
    LET l_table = cl_prt_temptable('amsx522',g_sql) CLIPPED   # 產生Temp Table                                                      
...
#----------------------------------------------------------CR (1) ------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amsx522_tm(0,0)        # Input print condition
      ELSE CALL amsx522()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amsx522_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW amsx522_w AT p_row,p_col
        WITH FORM "ams/42f/amsx522" 
################################################################################
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amsx522_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amsx522',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amsx522_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0081
   ...
   #------------------------------ CR (2) ------------------------------#
     ...
     LET l_sql = "SELECT *",
                 ...
                 " AND mps_v='",tm.ver_no,"'",
                 " AND ",tm.wc clipped
     PREPARE amsx522_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amsx522_curs1 CURSOR FOR amsx522_prepare1
 
#    CALL cl_outnam('amsx522') RETURNING l_name                                       #No.FUN-850082
#    START REPORT amsx522_rep TO l_name                                               #No.FUN-850082
#    LET g_pageno = 0                                                                 #No.FUN-850082
 
     FOREACH amsx522_curs1 INTO mps.*, ima.*
...
#      OUTPUT TO REPORT amsx522_rep(sr.*,mps.*, ima.*)
...
#    FINISH REPORT amsx522_rep                                                           #No.FUN-850082
...
###XtraGrid###    CALL cl_prt_cs3('amsx522','amsx522',g_sql,g_str)                                                                                
    ...
    IF g_zz05 = 'Y' THEN
         CALL cl_wcchp(tm.wc,'mps11,mps01,ima08,ima67,ima43,mps03')
              RETURNING tm.wc
      END IF
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"mps11,mps01,ima08,ima67,ima43,mps03")
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"mps11,mps01,ima08,ima67,ima43,mps03")  #FUN-D30053 mark
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"mps11,mps01,ima08,ima67,ima43,mps03")
    LET g_xgrid.footerinfo1 = cl_getmsg('azz-211',g_lang),':',tm.ver_no
   #LET l_str = cl_wcchp(g_xgrid.order_field,"mps11,mps01,ima08,ima67,ima43,mps03") #FUn-D30070 mark
