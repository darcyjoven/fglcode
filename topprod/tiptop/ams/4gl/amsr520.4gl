...
### *** FUN-850082 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>>--*** ##                                                     
    ...
    LET l_table = cl_prt_temptable('amsr520',g_sql) CLIPPED   # 產生Temp Table                                                      
...
#----------------------------------------------------------CR (1) ------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amsr520_tm(0,0)        # Input print condition
      ELSE CALL amsr520()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amsr520_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01     #No.FUN-580031
   ...
   OPEN WINDOW amsr520_w AT p_row,p_col
        WITH FORM "ams/42f/amsr520" 
################################################################################
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amsr520_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amsr520',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amsr520_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0081
   ...
   #------------------------------ CR (2) ------------------------------#
     ...
     PREPARE amsr520_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amsr520_curs1 CURSOR FOR amsr520_prepare1
 
#    CALL cl_outnam('amsr520') RETURNING l_name                                           #No.FUN-850082
#    START REPORT amsr520_rep TO l_name                                                   #No.FUN-850082
#    LET g_pageno = 0                                                                     #No.FUN-850082
     FOREACH amsr520_curs1 INTO mps.*, ima.*
...
#      OUTPUT TO REPORT amsr520_rep(sr.*,mps.*, ima.*)
     ...
     ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-850082 *** ##                                                      
         EXECUTE insert_prep USING                                                                                                  
                 ...
                 ima.ima02,ima.ima25,ima.ima08,ima.ima67,ima.ima43   
     #------------------------------ CR (3) ------------------------------#
 
     END FOREACH
 
#    FINISH REPORT amsr520_rep                                                             #No.FUN-850082  
...
#No.FUN-850082--end                                                                                                                 
 ## **** 與 Crystal Reports 串聯段 - <<<< CALL cs3() >>>> FUN-850082 **** ##                                                        
    ...
    CALL cl_prt_cs3('amsr520','amsr520',g_sql,g_str)                                                                                
    #------------------------------ CR (4) ------------------------------#
 
END FUNCTION
 
#No.FUN-850082--Mark--Begin--#
#REPORT amsr520_rep(sr,mps, ima)
