...
# Modify.........: No:FUN-C40026 12/04/09 By xumm GR動態簽核
...
TYPE sr1_t RECORD
    ...
    bnb11_1 LIKE type_file.chr4,
    bnb11_2 LIKE type_file.chr2,
    bnb11_3 LIKE type_file.chr2,
    ...
    amt_68   LIKE type_file.num20_6,  #FUN-B40097
    l_money  LIKE type_file.num20_6,  #FUN-B40097  #FUN-C40026 add,
#FUN-C40026----add---str---
...
MAIN
   ...
   LET g_sql = "bna06.bna_file.bna06,bnb01.bnb_file.bnb01,",
               ...
               "amt_68.type_file.num20_6,l_money.type_file.num20_6,",   #FUN-C40026 add 2,
               #FUN-C40026----add---str----
               "sign_type.type_file.chr1,sign_img.type_file.blob,",   #簽核方式, 簽核圖檔
               "sign_show.type_file.chr1,sign_str.type_file.chr1000"
               #FUN-C40026----add---end----
   LET l_table = cl_prt_temptable('abxg100',g_sql) CLIPPED
   IF l_table = -1 THEN 
   ...
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,   #MOD-A80001 mod
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?)"   #FUN-B40097 2?   #FUN-C40026 add 4?
   ...
   IF cl_null(tm.wc)
      ...
      ELSE 
        #LET tm.wc = "bnb01 ='",tm.wc CLIPPED,"'"    #No.TQC-610081 mark
         CALL abxg100()            # Read data and create out-file
   END IF
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('abxg100',g_time,l_cmd)    # Execute cmd at later time
      END IF
...
#         l_sql     LIKE type_file.chg1000,       # RDSQL STATEMENT #No.FUN-680062 VARCHAR(1000) #FUN-B40097 mark     
          l_sql     STRING,                       #FUN-B40097 ADD 
          ...
          sr               RECORD
                                  ...
                                  amt_68    LIKE type_file.num20_6,    #FUN-B40097
                                  l_money    LIKE type_file.num20_6,    #FUN-B40097
  ...
  #LET g_len = 67
   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'abxg100'
  #----------------No.TQC-660042 modfiy
   ...
   IF tm.type = 'Y' THEN   #依保稅群組列印   #MOD-A50206 add
      LET l_sql = "SELECT bnb01,bnb02,bnb03,bnb04,bnb05,bnb06, ",
                  ...
                  "       bnc10,bna06,' ',' ',bnb16,           ",
                  " ima1916,'','', bxe02, bxe03, bxe04 ",    #FUN-6A0007  #FUN-B40097 add 2''
  ...
  #str MOD-A50206 add
   ELSE
      ...
      LET l_sql = "SELECT bnb01,bnb02,bnb03,bnb04,bnb05,bnb06, ",
                  ...
                  "       bnc04,bnc05,bnc06,bnc07,bnc08,bnc09, ",
                  "       bnc10,bna06,' ',' ',bnb16,           ",
                  "       ima1916,'','', '', '', '' ",  #FUN-6A0007   #FUN-B40097 add 2''
                  "  FROM bnb_file, bnc_file, bna_file,",
                  "       ima_file  ",  #FUN-6A0007
                  " WHERE bnb01 = bnc01 ",
                  "   AND bnc03 = ima01 ",           #FUN-6A0007
...
#  CALL cl_outnam('abxg100') RETURNING l_name
#  IF g_len = 0 OR g_len IS NULL THEN LET g_len = 100 END IF
#  FOR g_i = 1 TO g_len LET g_dash1_1[g_i,g_i] = '=' END FOR
# 
#  IF tm.type = 'Y' THEN
#     START REPORT abxg100_rep2 TO l_name
#  ELSE
#     START REPORT abxg100_rep TO l_name
#  END IF
# 
#  LET g_pageno = 0
#  LET g_mount = 0
#No.FUN-710085 --end--
   FOREACH abxg100_curs1 INTO sr.*
      ...
      IF tm.type = 'Y' THEN 
         ...
         LET sr.amt_68    = sr.bnc06 * sr.bnc08   #FUN-B40097 
         LET sr.l_money  = sr.bnc06 * sr.bnc08 * sr.bnc09   #FUN-B40097 
         ...
         EXECUTE insert_prep USING sr.bna06,sr.bnb01,sr.bnb02,sr.bnb03,
                                   ...
                                   sr.bnc09,sr.ima1916,sr.amt_68,sr.l_money,   #FUN-B40097 add sr.amt sr.money   #FUN-C40026 add ,
                                   "",l_img_blob,"N",""                        #FUN-C40026 add
         #No.FUN-710085 --end--
 
#        OUTPUT TO REPORT abxg100_rep2(sr.*)  #No.FUN-710085 mark
      ELSE 
         #No.FUN-710085 --start--
         LET l_bnb15=sr.bnb15
         LET l_bnb16=sr.bnb16
         ...
         EXECUTE insert_prep USING sr.bna06,sr.bnb01,sr.bnb02,sr.bnb03,
                                   ...
                                   l_bnb15,sr.bnb16,sr.bnc03,sr.bnc04,
                                   sr.bnc05,sr.bnc06,sr.bnc07,sr.bnc08,
                                   sr.bnc09,sr.ima1916,sr.amt_68,sr.l_money,   #FUN-B40097 add sr.amt_68 sr.l_money   #FUN-C40026 add ,
                                   "",l_img_blob,"N",""                        #FUN-C40026 add
         #No.FUN-710085 --end--
 
#        OUTPUT TO REPORT abxg100_rep(sr.*)   #No.FUN-710085 mark
      END IF
   END FOREACH
   ...
   IF tm.type = 'Y' THEN 
    ...
    # CALL cl_prt_cs3('abxg1002',l_sql,'')
###GENGRE###      LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
###GENGRE###      CALL cl_prt_cs3('abxg100','abxg1002',l_sql,'')
    CALL abxg100_grdata()    ###GENGRE###
      #No.FUN-710085 --end--
 
#     FINISH REPORT abxg100_rep2              #No.FUN-710085 mark
   ELSE  
      #No.FUN-710085 --start--                                                                                                       
    # LET l_sql = "SELECT * FROM ",l_table CLIPPED   #TQC-730113
    # CALL cl_prt_cs3('abxg1001',l_sql,'')
      LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
###GENGRE###      CALL cl_prt_cs3('abxg100','abxg1001',l_sql,'')
    CALL abxg100_grdata()    ###GENGRE###
      #No.FUN-710085 --end--
 
#     FINISH REPORT abxg100_rep               #No.FUN-710085 mark
   END IF
...
REPORT abxg100_rep2(sr)
   DEFINE l_last_sw    LIKE type_file.chr1,
          ...
          l_amt_68,l_amt_89 LIKE bnc_file.bnc10,
          sr               RECORD
                                  bnb01  LIKE bnb_file.bnb01,
                                  bnb02  LIKE bnb_file.bnb02,
                                  bnb03  LIKE bnb_file.bnb03,
  ...
  FORMAT
   ...
   AFTER GROUP OF sr.ima1916  
      ...
      #-->做替換動作
        ...
        LET l_amt_68_68  = GROUP SUM(sr.bnc06*sr.bnc08) #數量*單價
...
FUNCTION abxg100_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("abxg100")
        IF handler IS NOT NULL THEN
            START REPORT abxg100_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY bnb01"
                         
            DECLARE abxg100_datacur1 CURSOR FROM l_sql
            FOREACH abxg100_datacur1 INTO sr1.*
                OUTPUT TO REPORT abxg100_rep(sr1.*)
            END FOREACH
            FINISH REPORT abxg100_rep
        END IF
...
REPORT abxg100_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_bnc06  LIKE bnc_file.bnc06   #FUN-B40097
    DEFINE l_bnc06_bnc08        LIKE bnc_file.bnc06   #FUN-B40097
    DEFINE l_bnc06_bnc08_bnc09  LIKE bnc_file.bnc06   #FUN-B40097
    DEFINE l_head               STRING                #FUN-B40097
 
    ORDER EXTERNAL BY sr1.bnb01,sr1.ima1916
    ...
    FORMAT
        ...
        AFTER GROUP OF sr1.bnb01
            #FUN-B40097-----add----str-----------------
            LET l_bnc06_bnc08_bnc09 = GROUP SUM(sr1.l_money)  
            PRINTX l_bnc06_bnc08_bnc09
            #FUN-B40097-----add----end-------------------

        AFTER GROUP OF sr1.ima1916
            #FUN-B40097-----add----str-----------------
            LET l_bnc06 = GROUP SUM(sr1.bnc06)
            PRINTX l_bnc06 

            LET l_bnc06_bnc08 = GROUP SUM(sr1.amt_68)
            PRINTX l_bnc06_bnc08 
            #FUN-B40097-----add----end-------------------
        
        ON LAST ROW
            LET l_head = cl_gr_getmsg("gre-049",g_lang,sr1.bna06)
            PRINTX l_head   
