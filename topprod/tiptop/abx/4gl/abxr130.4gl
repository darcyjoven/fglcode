...
#
# Pattern name...: abxr130.4gl
# Descriptions...: 內銷報稅明細表(By料號)
...
# Modify.........: No.FUN-850089 08/05/26 By TSD.sar2436 報表改由CR產出
...
## *** 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>> CR11 *** ##
 ...
 LET l_table = cl_prt_temptable('abxr130',g_sql) CLIPPED   # 產生Temp Table
...
#------------------------------ CR (1) ------------------------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL abxr130_tm(4,15)        # Input print condition
      ELSE CALL abxr130()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690108
END MAIN
 
FUNCTION abxr130_tm(p_row,p_col)
   ...
   END IF
   OPEN WINDOW abxr130_w AT p_row,p_col
        WITH FORM "abx/42f/abxr130" 
################################################################################
...
################################################################################
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW abxr130_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690108
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('abxr130',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW abxr130_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690108
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0062
          ...
          l_ima02_2   LIKE ima_file.ima02,        #FUN-850089 add
          l_ima021_2  LIKE ima_file.ima021,       #FUN-850089 add
          sr               RECORD bng05  LIKE bng_file.bng05, #主件料件編號
                                  bng07  LIKE bng_file.bng07,
                                  bng08  LIKE bng_file.bng08,
                                  bng01  LIKE bng_file.bng01,
...
#FUN-850089 add---END
     ...
     LET l_sql = "SELECT bng05,bng07,bng08,bng01,bnf10 ",
                 "  FROM bng_file,bnf_file,ima_file ",
                 " WHERE bng01 = bnf01 ",
                 "   AND bng02 = bnf02 ",
                 "   AND bng03 = bnf03 ",
                 "   AND bng04 = bnf04 ",
               # "   AND bng05 = ima01 AND ima15 MATCHES '[13]' ",
                 "   AND bng05 = ima01 ",
                 "   AND bng03 = ",tm.yea,
                 "   AND bng04 = ",tm.mo  
 
     PREPARE abxr130_prepare1 FROM l_sql
     ...
     END IF
     DECLARE abxr130_curs1 CURSOR FOR abxr130_prepare1
 
     LET g_pageno = 0
     FOREACH abxr130_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN 
     ...
     #---FUN-850089 add---START
       ...
       LET l_ima02_2 = ''
       LET l_ima021_2 = ''
       SELECT ima02,ima021 INTO l_ima02_2,l_ima021_2 
        FROM ima_file WHERE ima01 = sr.bng05
          
       EXECUTE insert_prep USING sr.bng01,   l_ima02,    l_ima021,  sr.bnf10,    sr.bng05,
                                 sr.bng07,   sr.bng08,  l_ima02_2,  l_ima021_2
 ...
 #FUN-850089  ---start---
   ...
   CALL cl_prt_cs3('abxr130','abxr130',l_sql2,g_str)
   #------------------------------ CR (4) ------------------------------#
 #FUN-850089  ----end---
END FUNCTION
#No.FUN-870144
