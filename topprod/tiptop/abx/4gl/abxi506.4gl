 ...
 ## *** 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>> CR11 *** ##
  LET g_sql = "bzl01.bzl_file.bzl01,",
              ...
              "bzm06.bzm_file.bzm06"         #備註
                                          #21 items
  LET l_table = cl_prt_temptable('abxi506',g_sql) CLIPPED   # 產生Temp Table
...
#---FUN-850089 add ---end
   ...
   #CALL cl_used('abxi506',g_time,1) RETURNING g_time      #計算使用時間 (進入時間) #FUN-B30211
   CALL cl_used(g_prog,g_time,1) RETURNING g_time  #計算使用時間 (進入時間)    #FUN-B30211
   LET g_forupd_sql = "SELECT * FROM bzl_file WHERE bzl01 = ? FOR UPDATE"
   LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
   DECLARE i506_cl CURSOR FROM g_forupd_sql
 
   LET p_row = 2 LET p_col = 9
 
   OPEN WINDOW i506_w AT p_row,p_col              #顯示畫面
        WITH FORM "abx/42f/abxi506" ATTRIBUTE (STYLE = g_win_style)
 
   CALL cl_ui_init()
 
   CALL i506_menu()
   CLOSE WINDOW i506_w                 #結束畫面
   #CALL cl_used('abxi506',g_time,2) RETURNING g_time      #計算使用時間 (退出時間)  #FUN-B30211
...
FUNCTION i506_cs()
   ...
   CONSTRUCT BY NAME g_wc ON bzl01,bzl02,bzl03,bzl04,bzl05,
   ...
   PREPARE i506_prepare FROM g_sql
   DECLARE i506_cs                         #SCROLL CURSOR
       SCROLL CURSOR WITH HOLD FOR i506_prepare
 
   IF g_wc2 = " 1=1" THEN                  # 取合乎條件筆數
      LET g_sql="SELECT COUNT(*) FROM bzl_file WHERE ",g_wc CLIPPED,
                "   AND bzl00 ='1' "
   ELSE
...
FUNCTION i506_menu()
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         WHEN "void"
            IF cl_chk_act_auth() THEN
               #CALL i506_x() #FUN-D20025 mark
               CALL i506_x(1) #FUN-D20025 add
               CALL i506_pic()
            END IF
         #FUN-D20025 add
         WHEN "undo_void"          #"取消作廢"
...
FUNCTION i506_a()
   ...
   WHILE TRUE
      LET g_bzl.bzl00 = '1'   #報廢
      LET g_bzl.bzl02 = g_today
      LET g_bzl.bzl04 = NULL
      LET g_bzl.bzl05 = ' '
      LET g_bzl.bzl06 = 'N'
      ...
      DISPLAY '1:報廢' TO FORMONLY.bzl00
      CALL i506_i("a")                   #輸入單頭
      ...
      CALL s_auto_assign_no("abx",g_t1,g_bzl.bzl02,"C","","","","","")
      ...
      SELECT bzl01 INTO g_bzl.bzl01 FROM bzl_file
...
FUNCTION i506_u()
   ...
   SELECT * INTO g_bzl.* FROM bzl_file
    WHERE bzl01=g_bzl.bzl01
   #檢查資料是否為無效
   IF g_bzl.bzlacti = 'N' THEN 
      CALL cl_err(g_bzl.bzl01,'mfg1000',0) 
      RETURN 
   END IF
   IF g_bzl.bzl06 = 'Y'  THEN CALL cl_err('',9023,0) RETURN END IF
   IF g_bzl.bzl06 = 'X'  THEN CALL cl_err('',9024,0) RETURN END IF
   ...
   OPEN i506_cl USING g_bzl.bzl01
   IF STATUS THEN
      CALL cl_err("OPEN i506_cl:", STATUS, 1)
      CLOSE i506_cl
      ROLLBACK WORK
      RETURN
   END IF
 
   FETCH i506_cl INTO g_bzl.*                      # 鎖住將被更改或取消的資料
   ...
   WHILE TRUE
      ...
      # 更改單號(當g_chkey='Y'可修改key值)
      IF g_bzl.bzl01 != g_bzl01_t THEN
...
DEFINE
   ...
   p_cmd     LIKE type_file.chr1,               #a:輸入 u:更改 y:確認
   ...
   DISPLAY BY NAME g_bzl.bzl00,g_bzl.bzluser,g_bzl.bzlmodu,
                   g_bzl.bzlgrup,g_bzl.bzldate,g_bzl.bzlacti
 
   CALL cl_set_head_visible("","YES")         #No.FUN-6B0033			
 
   INPUT BY NAME g_bzl.bzl01,g_bzl.bzl02,g_bzl.bzl03,
      ...
      AFTER FIELD bzl01
         IF NOT cl_null(g_bzl.bzl01) THEN
            IF g_bzl.bzl01 != g_bzl01_t OR g_bzl01_t IS NULL THEN
               #進行輸入之單號檢查
#              CALL s_check_no(g_sys,g_bzl.bzl01,g_bzl01_t,"C","bzl_file","bzl01","")
               CALL s_check_no("abx",g_bzl.bzl01,g_bzl01_t,"C","bzl_file","bzl01","")   #No.FUN-A40041
                    RETURNING li_result,g_bzl.bzl01
               DISPLAY BY NAME g_bzl.bzl01
               IF (NOT li_result) THEN
                  NEXT FIELD bzl01
      ...
      AFTER FIELD bzl04   #確認人
         IF (p_cmd='y') AND NOT cl_null(g_bzl.bzl04) THEN
            LET l_n = 0
            SELECT COUNT(*) INTO l_n FROM gen_file
              WHERE gen01 = g_bzl.bzl04
            IF l_n IS NULL THEN LET l_n = 0 END IF
            IF l_n = 0 THEN
              CALL cl_err(g_bzl.bzl04,'aap-038',0)
              NEXT FIELD bzl04
            END IF
            CALL i506_bzl04('p_cmd')
         END IF
 
      AFTER FIELD bzl05     
         IF (p_cmd='y') AND NOT cl_null(g_bzl.bzl05) THEN
      ...
      ON ACTION controlp
         CASE
            WHEN INFIELD(bzl01) #單據編號
                 LET g_t1 = s_get_doc_no(g_bzl.bzl01)
#                CALL q_bna(FALSE,TRUE,g_t1,'C',g_sys) RETURNING g_t1
                 CALL q_bna(FALSE,TRUE,g_t1,'C','abx') RETURNING g_t1   #No.FUN-A40041
                 IF INT_FLAG THEN
                    LET INT_FLAG = 0
                 END IF
...
FUNCTION i506_q()
   ...
   OPEN i506_cs                            # 從DB產生合乎條件TEMP(0-30秒)
   IF SQLCA.sqlcode THEN
...
DEFINE
   ...
   CASE p_flag
      WHEN 'N' FETCH NEXT     i506_cs INTO g_bzl.bzl01
      WHEN 'P' FETCH PREVIOUS i506_cs INTO g_bzl.bzl01
      WHEN 'F' FETCH FIRST    i506_cs INTO g_bzl.bzl01
      WHEN 'L' FETCH LAST     i506_cs INTO g_bzl.bzl01
      WHEN '/'
            IF (NOT mi_no_ask) THEN
                ...
                END IF
            END IF
            FETCH ABSOLUTE g_jump i506_cs INTO g_bzl.bzl01
            LET mi_no_ask = FALSE
   END CASE
...
FUNCTION i506_show()
   ...
   CALL i506_bzl04('d')
   CALL i506_b_fill(g_wc2)                 #單身
   CALL i506_pic()
END FUNCTION
 
FUNCTION i506_y()    #確認
   DEFINE     l_cnt     LIKE type_file.num5
   DEFINE     l_bzm03   LIKE bzm_file.bzm03,
              l_bzm04   LIKE bzm_file.bzm04,
              l_bzm05   LIKE bzm_file.bzm05,
              l_bzm02   LIKE bzm_file.bzm02,
              l_bzb07   LIKE bzb_file.bzb07,  #帳面結餘數量
   ...
   IF cl_null(g_bzl.bzl01) THEN
      ...
      RETURN
   END IF
   IF g_bzl.bzl06 = 'Y' THEN CALL cl_err('',9023,0) RETURN END IF     #CHI-C30107 add
   IF g_bzl.bzl06 = 'X' THEN CALL cl_err('',9024,0) RETURN END IF     #CHI-C30107 add
   IF NOT cl_confirm('axm-108') THEN RETURN END IF #取消確認此張單據  #CHI-C30107 add
   SELECT * INTO g_bzl.* FROM bzl_file WHERE bzl01 = g_bzl.bzl01
   IF g_bzl.bzl06 = 'Y' THEN CALL cl_err('',9023,0) RETURN END IF
   IF g_bzl.bzl06 = 'X' THEN CALL cl_err('',9024,0) RETURN END IF
   ...
   #無單身資料不可確認
   LET l_cnt=0
   SELECT COUNT(*) INTO l_cnt FROM bzm_file
...
#  IF NOT cl_confirm('axm-108') THEN RETURN END IF #取消確認此張單據  #CHI-C30107 mark
   MESSAGE ""
   LET g_bzl01_t = g_bzl.bzl01
   ...
   WHILE TRUE
     ...
     #IF cl_null(g_bzl.bzl05) THEN           #確認日期不可空白
     #   CONTINUE WHILE
     #END IF
     #CHI-C80072--mark--end--
     CLOSE i506_cl
     ...
     #更新bzb_file
     DECLARE i506_dy1 CURSOR FOR
       SELECT bzm02,bzm03,bzm04,bzm05
         FROM bzm_file 
        WHERE bzm01 = g_bzl.bzl01 
        ORDER BY bzm03,bzm04
     FOREACH i506_dy1 INTO l_bzm02,l_bzm03,l_bzm04,l_bzm05
       IF STATUS THEN
          CALL cl_err('foreach i506_dy1:',STATUS,0)
          LET g_success = 'N'
          EXIT FOREACH
       END IF
       LET l_bzb07 = 0
       ...
       IF l_bzb07 IS NULL THEN
          LET l_bzb07 = 0
       END IF
       IF l_bzm05 > l_bzb07 THEN    #不可大於帳面結餘數量
          CALL cl_err(g_bzm[l_ac].bzm05,'abx-031',1)
          LET g_success = 'N'
          EXIT FOREACH
       END IF
       UPDATE bzb_file SET bzb12 = bzb12+l_bzm05,   #報廢數量
                              bzb08 = '2',                     #狀態碼
                              bzb09 = g_bzl.bzl05        #報廢日期
        WHERE bzb01 = l_bzm03 AND bzb02 = l_bzm04
       IF SQLCA.sqlcode OR SQLCA.sqlerrd[3]=0 THEN
          CALL cl_err('upd1 bzb_file:',SQLCA.sqlcode,0)
          LET g_success = 'N'
          EXIT FOREACH
       END IF
 
       UPDATE bzb_file 
          SET bzb07 = bzb05-bzb10+bzb11-bzb12-bzb13 #帳面結餘數量
        WHERE bzb01 = l_bzm03 AND bzb02 = l_bzm04
       IF SQLCA.sqlcode OR SQLCA.sqlerrd[3]=0 THEN
          CALL cl_err('upd2 bzb_file:',SQLCA.sqlcode,0)
          LET g_success = 'N'
          EXIT FOREACH
       END IF
       #更新bza_file帳面結餘數量
       UPDATE bza_file
          SET bza16=(SELECT SUM(bzb07) FROM bzb_file WHERE bzb01=l_bzm03)
         WHERE bza01 = l_bzm03
       IF SQLCA.sqlcode OR SQLCA.sqlerrd[3]=0 THEN
          CALL cl_err('upd bza_file:',SQLCA.sqlcode,0)
          LET g_success = 'N'
          EXIT FOREACH
       END IF
       #新增bzp_file記錄
       INSERT INTO bzp_file(bzp01,bzp02,bzp03,bzp04,bzp05,
                        ...
                        VALUES(g_bzl.bzl01,l_bzm02,g_today,'C',-1,
                               l_bzm03,l_bzm04,l_bzm05,
                               g_bzl.bzl05,
                               g_plant,g_legal)  #FUN-980001 add
       IF SQLCA.sqlcode OR SQLCA.sqlerrd[3]=0 THEN
          CALL cl_err('ins bzp_file:',SQLCA.sqlcode,0)
          LET g_success = 'N'
          EXIT FOREACH
       END IF
     END FOREACH
...
FUNCTION i506_z()  #取消確認
  DEFINE l_bzm03 LIKE bzm_file.bzm03,
   ...
   END IF
   SELECT * INTO g_bzl.* FROM bzl_file WHERE bzl01 = g_bzl.bzl01
   IF g_bzl.bzl06 = 'N' THEN CALL cl_err('',9025,0) RETURN END IF
   IF g_bzl.bzl06 = 'X' THEN CALL cl_err('',9024,0) RETURN END IF
   ...
   IF SQLCA.sqlcode THEN
      CALL cl_err(g_bzl.bzl01,SQLCA.sqlcode,0)      #資料被他人LOCK
      CLOSE i506_cl ROLLBACK WORK RETURN
   END IF
   CLOSE i506_cl
   ...
   DISPLAY l_gen02 TO gen02_1                                           #CHI-C80072 add
   #更新bzb_file
   DECLARE i506_dz1 CURSOR FOR
     SELECT bzm03,bzm04,bzm05
       FROM bzm_file
      WHERE bzm01 = g_bzl.bzl01 
      ORDER BY bzm03,bzm04
   FOREACH i506_dz1 INTO l_bzm03,l_bzm04,l_bzm05
     IF SQLCA.sqlcode THEN
        CALL cl_err('foreach i506_dz1:',SQLCA.sqlcode,0)
        LET g_success = 'N'
        EXIT FOREACH
     END IF
     UPDATE bzb_file SET bzb12 = bzb12 - l_bzm05,  #報廢數量
      ...
      WHERE bzb01 = l_bzm03 AND bzb02 = l_bzm04
     IF SQLCA.sqlcode OR SQLCA.sqlerrd[3]=0 THEN
        CALL cl_err('upd1 bzb_file2:',SQLCA.sqlcode,0)
        LET g_success = 'N'
        EXIT FOREACH
     END IF
     #帳面結餘數量
     UPDATE bzb_file 
      ...
      WHERE bzb01 = l_bzm03 AND bzb02 = l_bzm04
     IF SQLCA.sqlcode OR SQLCA.sqlerrd[3]=0 THEN
        CALL cl_err('upd2 bzb_file2:',SQLCA.sqlcode,0)
        LET g_success = 'N'
        EXIT FOREACH
     END IF
     #更新bza_file帳面結餘數量
     UPDATE bza_file 
      ...
      WHERE bza01 = l_bzm03
     IF SQLCA.sqlcode OR SQLCA.sqlerrd[3]=0 THEN
        CALL cl_err('upd bza_file2:',SQLCA.sqlcode,0)
        LET g_success = 'N'
        EXIT FOREACH
     END IF
   END FOREACH
   #刪除bzp_file記錄
   DELETE FROM bzp_file WHERE bzp01 = g_bzl.bzl01
   IF SQLCA.sqlcode OR SQLCA.sqlerrd[3]=0 THEN
      CALL cl_err('del bzp_file:',SQLCA.sqlcode,0)
      LET g_success = 'N'
   END IF
   IF g_success = 'Y' THEN
      COMMIT WORK
...
#FUNCTION i506_x()  #FUN-D20025 mark
FUNCTION i506_x(p_type) #FUN-D20025 add
   ...
   IF cl_void(0,0,g_bzl.bzl06) THEN
      IF g_bzl.bzl06 ='N' THEN
         LET g_bzl.bzl06='X'
      ELSE
         LET g_bzl.bzl06='N'
      END IF
      ...
      UPDATE bzl_file SET bzl06   = g_bzl.bzl06,
       ...
       WHERE bzl01 = g_bzl.bzl01
      IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
         CALL cl_err('upd bzl06:',SQLCA.sqlcode,1)
         LET g_success='N'
      END IF
   END IF
   CLOSE i506_cl
...
DEFINE
    ...
    LET g_forupd_sql = "SELECT bzm02,bzm03,bzm04,",
                       ...
                       #FUN-840202 --end--
                       "  FROM bzm_file",
                       " WHERE bzm01=? AND bzm02=? FOR UPDATE"
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    ...
    INPUT ARRAY g_bzm WITHOUT DEFAULTS FROM s_bzm.*
        ...
        BEFORE ROW
           ...
           IF g_rec_b >= l_ac THEN
              ...
              OPEN i506_bcl USING g_bzl.bzl01,g_bzm_t.bzm02
              IF STATUS THEN
                 CALL cl_err("OPEN i506_bcl:", STATUS, 1)
                 LET l_lock_sw = "Y"
              ELSE
                 FETCH i506_bcl INTO g_bzm[l_ac].*
                 IF SQLCA.sqlcode THEN
        ...
        BEFORE INSERT
           ...
           LET g_bzm[l_ac].bzm05 =  0            #Body default
        ...
        BEFORE FIELD bzm02                        #default 序號
           IF g_bzm[l_ac].bzm02 IS NULL OR g_bzm[l_ac].bzm02 = 0 THEN
              SELECT max(bzm02)+1
                INTO g_bzm[l_ac].bzm02
                FROM bzm_file
               WHERE bzm01 = g_bzl.bzl01
              IF g_bzm[l_ac].bzm02 IS NULL THEN
                 LET g_bzm[l_ac].bzm02 = 1
              END IF
           END IF
 
        AFTER FIELD bzm02                        #check 序號是否重複
           IF NOT cl_null(g_bzm[l_ac].bzm02) THEN
              IF g_bzm[l_ac].bzm02 != g_bzm_t.bzm02
                 OR g_bzm_t.bzm02 IS NULL THEN
                 SELECT count(*) INTO l_n FROM bzm_file
                  WHERE bzm01 = g_bzl.bzl01
              ...
              END IF
           END IF
           DISPLAY BY NAME g_bzm[l_ac].bzm02   #No.TQC-740062 
 
        AFTER FIELD bzm03   #保稅機器設備編號
           IF NOT cl_null(g_bzm[l_ac].bzm03) THEN
              CALL i506_bzm03_chk(p_cmd)
              IF NOT cl_null(g_errno) THEN
              ...
              END IF   
           END IF
           DISPLAY BY NAME g_bzm[l_ac].bzm03   #No.TQC-740062 
 
        AFTER FIELD bzm04   #序號
           IF NOT cl_null(g_bzm[l_ac].bzm03) AND
              ...
              CALL i506_bzm03('d')
           END IF
           DISPLAY BY NAME g_bzm[l_ac].bzm04   #No.TQC-740062 
 
        AFTER FIELD bzm05   #報廢數量
        ...
        BEFORE DELETE                      #是否取消單身
           IF g_bzm_t.bzm02 > 0 AND g_bzm_t.bzm02 IS NOT NULL THEN
    ...
    CLOSE i506_bcl
    COMMIT WORK
#   CALL i506_delall()  #CHI-C30002 mark
    CALL i506_delHeader()     #CHI-C30002 add
 
END FUNCTION
 
#CHI-C30002 -------- add -------- begin
FUNCTION i506_delHeader()
   ...
   IF g_rec_b = 0 THEN
      ...
      PREPARE i506_pb1 FROM l_sql 
      EXECUTE i506_pb1 INTO l_cnt      
...
FUNCTION i506_bzm05_chk(p_cmd)
   ...
   END IF
   LET l_bzb07 = 0
   SELECT bzb07 INTO l_bzb07 FROM bzb_file
    WHERE bzb01=g_bzm[l_ac].bzm03 
      AND bzb02=g_bzm[l_ac].bzm04
   IF l_bzb07 IS NULL THEN LET l_bzb07 = 0 END IF
   IF l_bzb07 < g_bzm[l_ac].bzm05 THEN #不可大於帳面結餘數量
      LET g_errno = 'abx-031'
   END IF
END FUNCTION
 
FUNCTION i506_bzm03(p_cmd)  #機器設備編號
    ...
    LET g_errno = ' '
    SELECT bza02,bza03,bza04,bzaacti,bzb03,bzb04,gem02,gen02
      INTO l_bza02,l_bza03,l_bza04,l_bzaacti,l_bzb03,l_bzb04,
           l_gem02,l_gen02
      FROM bza_file,bzb_file,
           OUTER gen_file,OUTER gem_file
     WHERE bzb01 = g_bzm[l_ac].bzm03 AND
           bzb01 = bza01 AND
           bzb02 = g_bzm[l_ac].bzm04 AND
           bzb_file.bzb03 = gem_file.gem01 AND
           bzb_file.bzb04 = gen_file.gen01
...
FUNCTION i506_b_askkey()
DEFINE
    l_wc2     STRING
 
    CONSTRUCT l_wc2 ON bzm02,bzm03,bzm04,bzm05,bzm06 # 螢幕上取單身條件
           #No.FUN-840202 --start--
           ,bzmud01,bzmud02,bzmud03,bzmud04,bzmud05
           ,bzmud06,bzmud07,bzmud08,bzmud09,bzmud10
           ,bzmud11,bzmud12,bzmud13,bzmud14,bzmud15
           #No.FUN-840202 ---end---
            FROM s_bzm[1].bzm02,s_bzm[1].bzm03,s_bzm[1].bzm04,
                 s_bzm[1].bzm05,s_bzm[1].bzm06
           #No.FUN-840202 --start--
           ,s_bzm[1].bzmud01,s_bzm[1].bzmud02,s_bzm[1].bzmud03,s_bzm[1].bzmud04,s_bzm[1].bzmud05
           ,s_bzm[1].bzmud06,s_bzm[1].bzmud07,s_bzm[1].bzmud08,s_bzm[1].bzmud09,s_bzm[1].bzmud10
           ,s_bzm[1].bzmud11,s_bzm[1].bzmud12,s_bzm[1].bzmud13,s_bzm[1].bzmud14,s_bzm[1].bzmud15
           #No.FUN-840202 ---end---
...
DEFINE
    ...
    #-------MOD-850191-------------
   #LET g_sql = "SELECT bzm02,bzm03,bzm04,bza02,bza03,bza04,bzb03,gem02,",
   #            "       bzb04,gen02,bzm05,bzm06 ",
   #            #No.FUN-840202 --start--
   #            ",bzmud01,bzmud02,bzmud03,bzmud04,bzmud05,",
   #            "bzmud06,bzmud07,bzmud08,bzmud09,bzmud10,",
   #            "bzmud11,bzmud12,bzmud13,bzmud14,bzmud15", 
   #            #No.FUN-840202 ---end---
   #            " FROM bzm_file,OUTER bza_file,",
   #            "      OUTER (bzb_file,OUTER gem_file,OUTER gen_file) ",
   #            " WHERE bzm01 ='",g_bzl.bzl01,"' ",   #單頭
   #            "   AND bzb_file.bzb01 = bzm_file.bzm03 AND ",
   #            "       bzb_file.bzb02 = bzm_file.bzm04 AND ",
   #            "       bzm03 = bza01 AND ",
   #            "       bzb_file.bzb03 = gem_file.gem01 AND ",
   #            "       bzb04 = gen_file.gen01 "
    LET g_sql = "SELECT bzm02,bzm03,bzm04,bza02,bza03,bza04,bzb03,'',",
                "       bzb04,'',bzm05,bzm06 ",
                #No.FUN-840202 --start--
                ",bzmud01,bzmud02,bzmud03,bzmud04,bzmud05,",
                "bzmud06,bzmud07,bzmud08,bzmud09,bzmud10,",
                ...
                "   AND bzb_file.bzb01 = bzm_file.bzm03 AND ",
                "       bzb_file.bzb02 = bzm_file.bzm04 AND ",
                "       bzm_file.bzm03 = bza_file.bza01 "
    #-------END MOD-850191---------
 
    IF NOT cl_null(p_wc2) THEN
       LET g_sql=g_sql CLIPPED," AND ",p_wc2 CLIPPED
    END IF
    LET g_sql=g_sql CLIPPED," ORDER BY bzm01,bzm02 "
 
    PREPARE i506_pb FROM g_sql
    DECLARE bzm_cs                       #CURSOR
        CURSOR FOR i506_pb
    ...
    FOREACH bzm_cs INTO g_bzm[g_cnt].*   #單身 ARRAY 填充
      ...
      END IF
      #-------MOD-850191-------------
      LET g_bzm[g_cnt].gem02 = ''
      SELECT gem02 INTO g_bzm[g_cnt].gem02
        FROM gem_file
       WHERE g_bzm[g_cnt].bzb03 = gem01
 
      LET g_bzm[g_cnt].gen02 = ''
      SELECT gen02 INTO g_bzm[g_cnt].gen02
        FROM gen_file
       WHERE g_bzm[g_cnt].bzb04 = gen_file.gen01
     #-------END MOD-850191---------
      LET g_cnt = g_cnt + 1
...
FUNCTION i506_set_entry(p_cmd)
  DEFINE p_cmd   LIKE type_file.chr1
    ...
    IF (p_cmd = 'a' OR p_cmd = 'u') AND ( NOT g_before_input_done ) THEN
      CALL cl_set_comp_entry("bzl01,bzl02,bzl03",TRUE)
    END IF
    IF p_cmd = 'y' AND ( NOT g_before_input_done ) THEN
      CALL cl_set_comp_entry("bzl04,bzl05",TRUE)
    END IF
END FUNCTION
...
FUNCTION i506_set_no_entry(p_cmd)
  DEFINE p_cmd   LIKE type_file.chr1
    ...
    END IF
    IF (p_cmd = 'a' OR p_cmd = 'u') AND ( NOT g_before_input_done ) THEN
       CALL cl_set_comp_entry("bzl04,bzl05",FALSE)
    END IF
    IF p_cmd = 'y' AND ( NOT g_before_input_done ) THEN
       CALL cl_set_comp_entry("bzl01,bzl02,bzl03",FALSE)
    END IF
END FUNCTION
...
#FUN-850089 add---END
   ...
   CALL cl_wait()
#   CALL cl_outnam('abxi506') RETURNING l_name          #FUN-850089
...
LET g_sql="SELECT bzl01,bzl02,bzl03,bzl04,b.gen02,bzl05,bzl06,",
          ...
          "  LEFT OUTER JOIN gen_file c ON e.bzb04=c.gen01) ",
          "  ON bzm_file.bzm03=e.bzb01 AND bzm_file.bzm04=e.bzb02 ",
          "  WHERE bzm01=bzl01 ",
          "    AND bzl00='1'",
          "    AND ",g_wc CLIPPED," AND ",g_wc2 CLIPPED         
 
   PREPARE i506_p1 FROM g_sql                # RUNTIME 編譯
   DECLARE i506_co CURSOR FOR i506_p1        # SCROLL CURSOR
 
#  START REPORT i506_rep TO l_name          #FUN-850089
 
   FOREACH i506_co INTO sr.*
      IF SQLCA.sqlcode THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)                 
         EXIT FOREACH
      END IF
#      OUTPUT TO REPORT i506_rep(sr.*)        #FUN-850089
      #---FUN-850089 add---START
...
#   FINISH REPORT i506_rep                   #FUN-850089
  #FUN-850089  ---start---
    ...
    END IF
    LET g_str = g_str
    CALL cl_prt_cs3('abxi506','abxi506',l_sql,g_str)
    #------------------------------ CR (4) ------------------------------#
  #FUN-850089  ----end---
   CLOSE i506_co
   ERROR ""
