...
#
# Pattern name...: abxr013.4gl
# Descriptions...: 區內事業交易申報書買入憑證
# Date & Author..: 2006/10/18 By kim
# Modify.........: No.FUN-6A0062 06/10/27 By hellen l_time轉g_time
# Modify.........: No.FUN-850089 08/05/20 By TSD.hoho 傳統報表轉Crystal Report
...
LET g_sql = "bxi03.bxi_file.bxi03,",
            ...
            "pmc1914.pmc_file.pmc1914,",
            "pmc24.pmc_file.pmc24,",
            "pmc1915.pmc_file.pmc1915,",
...
LET l_table = cl_prt_temptable('abxr013',g_sql) CLIPPED   # 產生Temp Table
...
#FUN-850089  ----end---
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL abxr013_tm(4,17)                    # Input print condition
   ELSE
      CALL abxr013()                           # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80082--add--
END MAIN
 
FUNCTION abxr013_tm(p_row,p_col)
   DEFINE p_row,p_col    LIKE type_file.num5,
          l_cmd          LIKE type_file.chr1000
 
   OPEN WINDOW abxr013_w AT p_row,p_col
        WITH FORM "abx/42f/abxr013"
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,      #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_rep_clas CLIPPED,"'",
                         " '",g_template CLIPPED,"'" 
            CALL cl_cmdat('abxr013',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW abxr013_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80082--add--
         EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0062
          ...
          sr        RECORD
                       ...
                       pmc1914  LIKE pmc_file.pmc1914,
                       pmc24      LIKE pmc_file.pmc24,
                       pmc1915  LIKE pmc_file.pmc1915,
                       pmc1916  LIKE pmc_file.pmc1916,
...
#FUN-850089 add---END
   ...
   #   END IF
   LET l_sql = "SELECT bxi03,ima1916,pmc03,pmc1914,pmc24, ",
               "       pmc1915,pmc1916,bxi02,bxj04,ima02,bxj05,bxj06, ",
               "       bxj15,'',ima1914,ima021,bxj20,bxi04,bxj10,",
               "       bxe02,bxe03,bxi06 ",
               "  FROM bxi_file, bxj_file, ima_file, ",
               "       OUTER pmc_file, OUTER bxe_file ",
               " WHERE bxi01 = bxj01 ",
               "   AND bxj04 = ima01 ",
               "   AND bxi_file.bxi03 = pmc_file.pmc01 ",
               "   AND ima_file.ima1916 = bxe_file.bxe01 ",
               "   AND ",tm.wc CLIPPED
 
   IF tm.aa = "1" THEN 
      LET l_sql = l_sql CLIPPED, " AND bxi06='1' "   #進貨(入庫單)
   END IF 
   IF tm.aa = "2" THEN 
      LET l_sql = l_sql CLIPPED, " AND bxi06='2' "   #倉退(倉退單)
   END IF 
   ...
   PREPARE abxr013_prepare1 FROM l_sql
   IF SQLCA.sqlcode THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80082--add--
      EXIT PROGRAM
   END IF
   DECLARE abxr013_curs1 CURSOR FOR abxr013_prepare1
 
   LET g_pageno = 0
 
   FOREACH abxr013_curs1 INTO sr.*
      IF SQLCA.sqlcode THEN
    ...
    #---FUN-850089 add---START
     EXECUTE insert_prep USING sr.bxi03, sr.ima1916,sr.pmc03,  sr.pmc1914,
                               sr.pmc24, sr.pmc1915,sr.pmc1916,sr.bxi02, 
                               sr.bxj04, sr.ima02,  sr.bxj05,  sr.bxj06,
                               sr.bxj15, sr.bnd04,  sr.ima1914,sr.ima021,
                               sr.bxj20, sr.bxi04,  sr.bxj10,  sr.bxe02,
                               sr.bxe03, sr.bxi06
     IF SQLCA.sqlcode  THEN
...
#FUN-850089  ---start---
  ...
  LET g_str = g_str,";",        tm.s[1,1],";",    tm.u[1,1],";",
              ...
              #P13              #P14
              tm.name1,";",     tm.kk
  CALL cl_prt_cs3('abxr013','abxr013',l_sql2,g_str)
  #------------------------------ CR (4) ------------------------------#
#FUN-850089  ----end---
   #No.FUN-B80082--mark--Begin--- 
