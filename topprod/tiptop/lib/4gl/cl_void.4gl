...
#
# Library name...: cl_void
# Descriptions...: 詢問 "此筆資料是否確定作廢(Y/N):"
#                    或 "此筆資料是否取消作廢(Y/N):"
# Input parameter: p_row,p_col,p_old_acticode
# RETURN code....: 1 FOR TRUE  : 是
#                  0 FOR FALSE : 否
# Usage .........: if cl_void(p_row,p_col,p_old_acticode)
...
FUNCTION cl_void(p_row,p_col,p_old_acticode)
   ...
   IF p_old_acticode = 'N' THEN
      SELECT ze03 INTO ls_msg FROM ze_file WHERE ze01 = 'lib-016' AND ze02 = g_lang
   ELSE
      SELECT ze03 INTO ls_msg FROM ze_file WHERE ze01 = 'lib-017' AND ze02 = g_lang
   END IF
