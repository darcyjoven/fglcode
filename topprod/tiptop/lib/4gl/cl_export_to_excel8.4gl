# Prog. Version..: '5.25.07-12.03.07(00000)'     #
#
...
# Modify.........: No.13032901   13/03/29 By chenl  针对资产负债表和损益表调整导出格式模板
...
#FUNCTION cl_export_to_excel8(n,t,t1,t2)       #No.13032901 mark
FUNCTION cl_export_to_excel8(n,t,t1,t2,p_aaa01,p_mai01,p_type,p_yy,p_mm)   #No.13032901
#p_aaa01    账套编号，得到企业名称   
#p_mai01    报表编号，得到报表名称
#p_type     单位：元，千元，万元等
#p_yy       年度
...
### FUN-570216 ### 
 ...
 DEFINE  prog             LIKE type_file.chr100    #No.13032901
 DEFINE  p_aaa01          LIKE aaa_file.aaa01      #No.13032901
 DEFINE  l_aaf03          LIKE aaf_file.aaf03      #No.13032901
 DEFINE  p_mai01          LIKE mai_file.mai01      #No.13032901
 DEFINE  l_mai02          LIKE mai_file.mai02      #No.13032901
 DEFINE  p_type             LIKE type_file.chr1      #No.13032901
 DEFINE  l_chr            LIKE type_file.chr100    #No.13032901
 DEFINE  p_yy             LIKE type_file.num5      #No.13032901
 DEFINE  p_mm             LIKE type_file.num5      #No.13032901
 DEFINE  l_colspan        LIKE type_file.num5      #No.13032901
   ...
   CALL cl_get_cell_cnt8(t) RETURNING tmp_cnt
   LET cnt_cell = tmp_cnt 
 
   IF t1 IS NOT NULL then
      LET cnt_table = 2 
      CALL cl_get_cell_cnt8(t1) RETURNING tmp_cnt
      LET cnt_cell = cnt_cell + tmp_cnt 
   END IF   
 
   IF t2 IS NOT NULL then
      LET cnt_table = 3 
      CALL cl_get_cell_cnt8(t2) RETURNING tmp_cnt
      LET cnt_cell = cnt_cell + tmp_cnt 
   END IF 
...
### FUN-570216 ### 
   IF cnt_table = 1 THEN
     ...
     #RUN l_cmd
      ...
      ### FUN-570128 ###
     #CALL l_channel.write("<body><table border=0 cellpadding=0 cellspacing=0 width=432 style='border-collapse: collapse;table-layout:fixed;width:324pt'>")   #No.13032901 mark
      CALL l_channel.write("<body>")
      CALL l_channel.write("<table border=0 cellpadding=0 cellspacing=0 width=432 style='border-collapse: collapse;table-layout:fixed;width:324pt'>")
     #No.13032901--begin--
      SELECT mai02 INTO l_mai02 FROM mai_file WHERE mai01 = p_mai01 
      LET l_colspan = 0
      IF g_prog = "gglq811" THEN 
          LET l_colspan = 8
      END IF 
      IF g_prog = "gglq812" THEN 
          LET l_colspan = 4
      END IF 
      LET l_str = "<tr height=22><th colspan = ",l_colspan," >",l_mai02 CLIPPED,"</th></tr>"    #FUN-A20036
      CALL l_channel.write(l_str CLIPPED)
      INITIALIZE l_str TO NULL
      LET l_str = p_yy USING '<<<<'," 年 ",p_mm USING '&&'," 月"
      LET l_str = "<tr height=22><th colspan = ",l_colspan," >",l_str,"</th></tr>"    #FUN-A20036
      CALL l_channel.write(l_str CLIPPED)
      INITIALIZE l_str TO NULL
      IF g_prog = "gglq811" THEN 
          LET l_str = "会企01表"
          LET l_str = "<tr height=22><td></td><td></td><td></td><td></td>",
                                    "<td></td><td></td><td></td><td>",l_str,"</td></tr>"
      END IF 
      IF g_prog = "gglq812" THEN 
          LET l_str = "会企02表"
          LET l_str = "<tr height=22><td></td><td></td><td></td><td>",l_str,"</td></tr>"
      END IF 
      CALL l_channel.write(l_str CLIPPED)
      ...
      CASE p_type
          ...
          WHEN 4
              LET l_chr = "百万"
      END CASE
      INITIALIZE l_aaf03 TO NULL
      SELECT aaf03 INTO l_aaf03 FROM aaf_file WHERE aaf01 = p_aaa01 AND aaf02 = g_lang
     #LET l_str = "<tr height=22><th colspan = 8 >",l_aaf03 CLIPPED,"</th></tr>"    #FUN-A20036
      LET l_str = "<tr height=22><td>","公司名称:",l_aaf03 CLIPPED,"</td><td></td><td></td>"
      IF g_prog = "gglq811" THEN 
          LET l_str = l_str,"<td></td><td></td><td></td><td></td>"
      END IF
      LET l_str = l_str,"<td>","单位:",l_chr,"</td>"
      CALL l_channel.write(l_str CLIPPED)
     #No.13032901---end---
      CALL l_channel.write("<table border=1 cellpadding=0 cellspacing=0 width=432 style='border-collapse: collapse;table-layout:fixed;width:324pt'>")
...
#TQC-790109---end-----                  
      ...
      #END TQC-690124
         ...
         #### FUN-570128 ####
          ...
          #### CHI-9C0051 START ####
#            IF chk_mask8(field_name) THEN   #FUN-A10046  mask by tommas
            IF chk_mask8(n1_text) THEN       #FUN-A10046  add by tommas
               LET g_mask[i] = "1"
            END IF
          #### CHI-9C0051 END   ###
      ...
      END FOR
      ##TQC-690124
      IF h=1 THEN CALL cl_get_body8(h,cnt_header,t,combo_arr,l_seq) END IF
      IF h=2 THEN CALL cl_get_body8(h,cnt_header,t1,combo_arr,l_seq) END IF
      IF h=3 THEN CALL cl_get_body8(h,cnt_header,t2,combo_arr,l_seq) END IF
      ##END TQC-690124
...
# Modify ........: 
##################################################
FUNCTION chk_mask8(n1_text)   #No.FUN-A10046
...
# Input parameter: 
# Return code....:            
FUNCTION cl_get_body8(p_h,p_cnt_header,s,s_combo_arr,p_seq)  ##TQC-690124
...
### FUN-570216 ###
   IF cnt_table = 1 THEN
     #CALL l_channel.write("</tr></table></body></html>")    #No.13032901 mark
     #No.13032901--begin--
      CALL l_channel.write("</tr></table>")                  
      CALL l_channel.write("<table border=0 cellpadding=0 cellspacing=0 width=432 style='border-collapse: collapse;table-layout:fixed;width:324pt'>")
      LET l_str = "<tr></tr><tr><td>","企业负责人:","</td>"
      IF g_prog = "gglq811" THEN 
          LET l_str = l_str,"<td></td><td></td><td></td>"
      END IF 
      LET l_str = l_str,"<td>","财务负责人:","</td><td></td><td>","制表人:","</td><td></td>",
                  "</tr></table>"
      CALL l_channel.write(l_str)                
      CALL l_channel.write("</body></html>")                
     #No.13032901---end---
...
# Input parameter:                                                                              
# Return code....:                                                                              
FUNCTION cl_body_set_sort8(p_cnt_header,s)                                                       
   DEFINE s                    om.DomNode               #報表資料                               
   DEFINE p_cnt_header         LIKE type_file.num10     #欄位個數                               
   DEFINE n_field              om.DomNode,                                                      
          n_record             om.DomNode,                                                      
