...
# Modify.........: No.FUN-980097 09/08/21 By alex 合併WOS至4gl內
...
FUNCTION cl_sqlserver(p_gcf,p_gch)                             #FUN-740207
   ...
   DEFINE l_gch        RECORD LIKE gch_file.* ,
          ...
          l_cnt,i      LIKE type_file.num10,               # 計算資料筆數 #No.FUN-690005
          l_gci_cnt    LIKE type_file.num10,               # 計算資料筆數 #No.FUN-690005
          l_msg,l_err  STRING,                # 存儲成功查詢結果
          l_cmd        STRING,                # 組合查詢SQL Server資料參數
          l_tsql_cmd   STRING,                # 組合查詢SQL Server資料參數
          l_where      STRING,                # 組合查詢SQL Server資料條件
          l_status     STRING                 # 返回通訊SQL Server結果狀態
   DEFINE l_channel    base.Channel,          # 存取SQL Server查詢返回結果
          l_data       STRING,                # 獲取查詢返回數據，按行
          l_chr        LIKE type_file.chr1000,  # 字串轉換字符   #No.FUN-690005
          l_token      base.StringTokenizer,  #
          l_array      ARRAY[5] OF LIKE gci_file.gci03,# 暫存字串 #No.FUN-690005
          l_gci03      LIKE gci_file.gci03,   #No.FUN-690005
          l_gci04      LIKE gci_file.gci04,   #No.FUN-690005
          l_gci        RECORD LIKE gci_file.* # 截取字串
#         l_gci        DYNAMIC ARRAY OF RECORD LIKE gci_file.*
                                              # 程式變數(Program Variables)
   DEFINE l_zz01       LIKE zz_file.zz01,
          l_zz02       LIKE zz_file.zz02,
          l_zz08       LIKE zz_file.zz08
   DEFINE l_zm04       LIKE zm_file.zm04
   DEFINE l_i          LIKE type_file.num10,   #No.FUN-690005    
          l_gaz_cmd    STRING,
          l_zm_tag     LIKE type_file.num5,    #No.FUN-690005
  ...
  #  LET l_where = " ",l_gch.gch08 CLIPPED,"..A.M_DOC_C_NAME NOT LIKE '%ds%' "
  #ELSE
  #   LET l_where = " SUBSTRING(",l_gch.gch08 CLIPPED,"..A.M_DOC_C_NAME,1,",
  #                 l_where.getLength() USING "&",")='",l_gcf.gcf06 CLIPPED,"'"
  #END IF
  #END FUN-6B0004
   LET l_tsql_cmd = "tsql -H ",l_gch.gch06 CLIPPED,
               " -p ",l_gch.gch07 CLIPPED,
               " -U ",l_gch.gch09 CLIPPED,
               " -P ",l_gch.gch10 CLIPPED," << %%\n" 
  #LET l_cmd = l_tsql_cmd CLIPPED,
  #            " SELECT COUNT(*),'^' ",
  #            "   FROM OBJ_M_DOCUMENTS ",
  #            "  WHERE ",l_where CLIPPED," USE ",l_gch.gch08 CLIPPED,";\n",
  #            "   go; \n",
  #            "   %%\n"
  #FUN-6B0004
  #LET l_cmd = l_tsql_cmd CLIPPED,
  #            " SELECT COUNT(*),'^' ",
  #            "   FROM OBJ_M_DOCUMENTS,OBJ_M_CATEG,OBJ_M_DOCCATEG     ",
  #            "  WHERE OBJ_M_DOCCATEG.M_CATEG_N_ID = OBJ_M_CATEG.M_CATEG_N_ID",
  #            "    AND OBJ_M_DOCCATEG.M_DOC_N_ID = OBJ_M_DOCUMENTS.M_DOC_N_ID",
  #            "    AND ",l_where CLIPPED,
  #            "    USE ",l_gch.gch08 CLIPPED,";\n",
  #            "   go; \n",
  #            "%%"
  #判斷 Express 是否與 BI 整合,因為sql語法不同
   IF l_gcf.gcf09 = 'Y' THEN
      LET l_cmd = l_tsql_cmd CLIPPED,
                  "SELECT COUNT(*),'^' ", 
                  "FROM  ",l_gch.gch08 CLIPPED,"..OBJ_M_DOCUMENTS, ",
                  "      ",l_gch.gch08 CLIPPED,"..OBJ_M_DOCCATEG, ",
                  "      ",l_gch.gch08 CLIPPED,"..OBJ_M_CATEG ",
                  "WHERE ",l_gch.gch08 CLIPPED,"..OBJ_M_DOCCATEG.M_DOC_N_ID = ",
                  "      ",l_gch.gch08 CLIPPED,"..OBJ_M_DOCUMENTS.M_DOC_N_ID ",
                  "AND   ",l_gch.gch08 CLIPPED,"..OBJ_M_CATEG.M_CATEG_N_ID= ",
                  "      ",l_gch.gch08 CLIPPED,"..OBJ_M_DOCCATEG.M_CATEG_N_ID ",
                  "AND  upper( substring(",l_gch.gch08 CLIPPED,"..OBJ_M_CATEG.M_CATEG_C_NAME,1,7))='EXPRESS';\n",
                  "   go; \n",
                  "%%"
   ELSE
      LET l_cmd = l_tsql_cmd CLIPPED,
  ...
  #END FUN-6B0004
   ...
   WHILE l_channel.read(l_data)
      IF l_data.getIndexOf("^",1) > 0 THEN 
         LET l_chr = NULL
         LET l_chr = l_data.subString(1,l_data.getIndexOf("^",1)-1)
        #LET l_cmd = "SELECT TO_NUMBER(",l_chr CLIPPED,") FROM DUAL"
        #PREPARE gci_cnt FROM l_cmd
        #EXECUTE gci_cnt INTO l_gci_cnt
         CALL cl_replace_str(l_chr,"\t","") RETURNING l_chr
         LET l_gci_cnt = l_chr CLIPPED
         #FUN-6B0004
         #IF l_gci_cnt = 0 THEN 
   ...
   LET l_cmd = NULL
   #LET l_cmd = l_tsql_cmd CLIPPED,
   #           " SELECT LTRIM(RTRIM(OBJ_M_DOCUMENTS.M_DOC_N_TYPE)),'^',",
   #           "        LTRIM(RTRIM(OBJ_M_DOCUMENTS.M_DOC_N_ID)),'^',  ",
   #           "        LTRIM(RTRIM(OBJ_M_DOCUMENTS.M_DOC_C_NAME)),'^',",
   #           "        LTRIM(RTRIM(OBJ_M_CATEG.M_CATEG_C_NAME))       ",
   #           "   FROM OBJ_M_DOCUMENTS,OBJ_M_CATEG,OBJ_M_DOCCATEG     ",
   #           "  WHERE OBJ_M_DOCCATEG.M_CATEG_N_ID = OBJ_M_CATEG.M_CATEG_N_ID",
   #           "    AND OBJ_M_DOCCATEG.M_DOC_N_ID = OBJ_M_DOCUMENTS.M_DOC_N_ID",
   #           "    AND ",l_where CLIPPED,
   ...
   IF l_gcf.gcf09 = 'Y' THEN
      LET l_cmd = l_tsql_cmd CLIPPED,
             " SELECT LTRIM(RTRIM(",l_gch.gch08 CLIPPED, "..OBJ_M_DOCUMENTS.M_DOC_N_TYPE)),'*#*',",
             "        LTRIM(RTRIM(",l_gch.gch08 CLIPPED, "..OBJ_M_DOCUMENTS.M_DOC_N_ID)),'*#*',  ",
             "        LTRIM(RTRIM(",l_gch.gch08 CLIPPED, "..OBJ_M_DOCUMENTS.M_DOC_C_NAME)),'*#*',",
             "        LTRIM(RTRIM(",l_gch.gch08 CLIPPED, "..OBJ_M_CATEG.M_CATEG_C_NAME))       ",
             "FROM  ",l_gch.gch08 CLIPPED,"..OBJ_M_DOCUMENTS, ",
             "      ",l_gch.gch08 CLIPPED,"..OBJ_M_DOCCATEG, ",
             "      ",l_gch.gch08 CLIPPED,"..OBJ_M_CATEG ",
             "WHERE ",l_gch.gch08 CLIPPED,"..OBJ_M_DOCCATEG.M_DOC_N_ID = ",
             ...
             "      ",l_gch.gch08 CLIPPED,"..OBJ_M_DOCCATEG.M_CATEG_N_ID ",
             "AND  upper( substring(",l_gch.gch08 CLIPPED,"..OBJ_M_CATEG.M_CATEG_C_NAME,1,7))='EXPRESS' ",
             " ORDER BY  ",l_gch.gch08 CLIPPED,"..OBJ_M_CATEG.M_CATEG_C_NAME ;\n",
             "   go; \n",
             "%%"
   ELSE
      LET l_cmd = l_tsql_cmd CLIPPED,
   ...
   END IF
   #END FUN-6B0004
   # LET l_cmd = "sqlserver.qry ",l_gch.gch01 CLIPPED," ",
   #                              l_gch.gch02 CLIPPED," ",
   #                              l_gch.gch03 CLIPPED," ",
   #                              l_gch.gch04 CLIPPED," ",
   #                              l_gch.gch05 CLIPPED," " 
   ...
   #    CALL cl_err('','SQL Server Communication Error!',1)
   #    RETURN
   # END IF
  ...
  #LET l_err = "REP Fail Import List:\n"
   ...
   WHILE l_channel.read(l_data)
      ...
      IF l_data.getIndexOf("*#*",1) > 0 AND (l_data NOT MATCHES '*TRIM*') THEN  #FUN-6B0004
  
 
         # 處理字串中間變數
         CALL cl_replace_str(l_data,"\t","") RETURNING l_data
 
         display l_data
         CALL l_array.clear()
         LET l_token = base.StringTokenizer.create(l_data,"*#*")   #FUN-6B0004
         LET i = 1
 
         # 按行讀取內容 ，分隔符為 *#* 
         WHILE l_token.hasMoreTokens()
            LET l_array[i] = l_token.nextToken()
            LET i = i + 1
         END WHILE
 
         CALL cl_replace_str(l_array[1],"\t","") RETURNING l_array[1]
         CASE l_array[1]
         ...
         # Marked by Leagh 2006.01.03 No Need Convertaion
         # LET l_cmd = "SELECT TO_NUMBER(",l_array[2] CLIPPED,") FROM DUAL"
         # PREPARE gci02_pre FROM l_cmd
         # EXECUTE gci02_pre INTO l_gci.gci02
         LET l_gci.gci02 = l_array[2] CLIPPED
         # End MOD.
 
         LET l_gci03 = l_array[3]
         LET l_gci04 = l_array[4]
         CALL cl_replace_str(l_gci03,"\t","") RETURNING l_gci03
         CALL cl_replace_str(l_gci04,"\t","") RETURNING l_gci04
 
         INSERT INTO gci_file(gci01,gci02,gci03,gci04) 
                       VALUES(l_gci.gci01,l_gci.gci02,
                              l_gci03,l_gci04)
                            # l_gci.gci03,l_gci.gci04)
 
         # 未成功導入報表清單
         IF STATUS THEN
            #CALL cl_err(l_gci03,SQLCA.sqlcode,0)
            CALL cl_err3("ins","gci_file","","",SQLCA.sqlcode,"","",0)   #No.FUN-640161
            LET l_err = l_err CLIPPED,l_gci03 CLIPPED,"\n"
         ELSE 
            LET l_zz01 = NULL LET l_zz02 = NULL LET l_zz08 = NULL
            CALL cl_replace_str(l_array[2],"\t","") RETURNING l_zz01
   ...
   IF NOT cl_null(l_msg) THEN
    ...
    #RUN l_cmd
     IF os.Path.chrwx("p_bi.out",511) THEN
     END IF
