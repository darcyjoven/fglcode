...
#
# Pattern name...: aoou012.4gl
# Descriptions...: 請購資料簽核作業
# Date & Author..: 91/04/22 By Lee
# Modify.........: 92/09/24 By Pin
#                  簽核處理修正(順序以簽核剩餘天數,簽核完成天數來排)
#                  因劃面編幅有限,無法顯示剩餘天數與簽核完成天數
# Modify.........: 92/09/29 By Pin
#                  劃面刪除(gka07,gka36),而加上(gkadays,gkaprit)
# Modify.........: 99/04/15 By Carol:SQL加pmk18='Y' 
...
# genero  script marked     l_arrno      SMALLINT,                  #program array no
    ...
    l_pmk03 ARRAY[48] OF LIKE type_file.chr2,      # Prog. Version..: '5.30.06-13.03.12(02),     #更動序號
    l_needs ARRAY[48] OF LIKE type_file.chr1,           #No.FUN-680102CHAR(01),
    l_pmk DYNAMIC ARRAY OF RECORD               #結果
        choice      LIKE type_file.chr1,           # Prog. Version..: '5.30.06-13.03.12(01),            #選擇碼
        pmk01       LIKE bxi_file.bxi01,     #No.FUN-680102 VARCHAR(16),            #報價單號  #No.FUN-550058
        pmk02       LIKE pmk_file.pmk02,    #報價日期
        pmk04       LIKE pmk_file.pmk04,    #有效日期
        pmc03       LIKE pmc_file.pmc03,    #帳款客戶
        pmk22       LIKE pmk_file.pmk22,    #總金額
 ...
 DEFINE g_i             LIKE type_file.num5,                #可新增否        #No.FUN-680102 SMALLINT
    ...
    OPEN WINDOW u012_w AT p_row,p_col                    #開窗讓使用者選擇
        WITH FORM "aoo/42f/aoou012" 
       ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
    
    CALL cl_ui_locale("aoou012")
    ...
    WHILE TRUE
        ...
        CONSTRUCT l_wc ON pmk01,pmk02,pmk04,pmc03,pmk22,
                 pmksign,pmkdays,pmkprit 
            FROM s_pmk[1].pmk01,s_pmk[1].pmk02,s_pmk[1].pmk04,
                 s_pmk[1].pmc03,s_pmk[1].pmk22,s_pmk[1].pmksign,
                 s_pmk[1].pmkdays,s_pmk[1].pmkprit 
      ...
      ON ACTION controlg      #MOD-4C0121
		...
		#No.FUN-580031 --end--       HCN
        END CONSTRUCT
        LET l_wc = l_wc CLIPPED,cl_get_extra_cond('pmkuser', 'pmkgrup') #FUN-980030
        ...
        LET l_sql = "SELECT ",                      #組合查詢句子
            " 'N', pmk01,pmk02,pmk04,pmc03,",
            " pmk22,pmksign,pmkdays,pmkprit ",
            " FROM pmk_file LEFT OUTER JOIN pmc_file ON pmk09 = pmc_file.pmc01 ",
            " WHERE pmksign IS NOT NULL AND",
            " pmkmksg='Y' AND",
            " pmk18='Y' AND",
            " pmk25='S' AND pmkacti='Y' AND pmk18 !='X' ",
            
            " AND ",l_wc CLIPPED,
            " AND pmksseq=(SELECT (azc02 - 1)",
            " FROM azc_file WHERE azc01=pmksign AND azc03='",g_pass,"')",
            " ORDER BY 8,9 "
        PREPARE u012_prepare FROM l_sql      #預備之
        IF SQLCA.sqlcode THEN                          #有問題了
            CALL cl_err('PREPARE:',SQLCA.sqlcode,1)
            EXIT WHILE
        END IF
        DECLARE u012_curs CURSOR FOR u012_prepare     #宣告之
 
        CALL l_pmk.clear()
#       FOR g_cnt=1 TO l.getLength()
#           INITIALIZE l_pmk[g_cnt].* TO NULL
#       END FOR
        LET g_cnt=1                                         #總選取筆數
        FOREACH u012_curs # USING g_today
            INTO l_pmk[g_cnt].* # , l_pmk03[g_cnt],l_needs[g_cnt],l_days,l_prit
            LET l_pmk[g_cnt].pmkdays = l_pmk[g_cnt].pmk04 - g_today + l_pmk[g_cnt].pmkdays
            IF SQLCA.sqlcode THEN                                  #有問題
                CALL cl_err('FOREACH:',SQLCA.sqlcode,1)
                EXIT FOREACH
            END IF
          # error 'l_days,l_prit,l_pmk[g_cnt].pmk01',
          #        l_days,l_prit,l_pmk[g_cnt].pmk01 sleep 3
#取得廠商簡稱
          {
            SELECT pmc03 INTO l_pmk[g_cnt].pmc03
                FROM pmc_file
                WHERE pmc01=l_pmk[g_cnt].pmc03
            IF SQLCA.sqlcode THEN
                LET l_pmk[g_cnt].pmc03=''
            END IF
          }
#取得是否需要閱後方可簽核
            SELECT aze07 INTO l_needs[g_cnt]   #簽核等級單頭檔
                FROM aze_file
                WHERE aze01=l_pmk[g_cnt].pmksign
            IF SQLCA.sqlcode THEN
                LET l_needs[g_cnt]=''
            END IF
            LET l_pmk[g_cnt].pmk01=l_pmk[g_cnt].pmk01 CLIPPED
#               '-',l_pmk03[g_cnt] CLIPPED
        ...
        IF g_cnt=1 THEN                                     #沒有抓到
            ...
            CONTINUE WHILE
        END IF
        CALL l_pmk.deleteElement(g_cnt)
...
#        DISPLAY ARRAY l_pmk TO s_pmk.*  #顯示並進行選擇
        LET l_allow_insert = FALSE
        LET l_allow_delete = FALSE
       
        LET l_ac = 1
        CALL cl_set_comp_visible("select_all,cancel_all,cmt",TRUE)
        INPUT ARRAY l_pmk WITHOUT DEFAULTS FROM s_pmk.*
          ...
          ON CHANGE choice
             IF NOT cl_null(l_pmk[l_ac].choice) THEN
                IF l_pmk[l_ac].choice NOT MATCHES "[YN]" THEN
                     NEXT FIELD choice
                END IF
                LET l_cnt = 0
                FOR g_i =1 TO g_cnt
                 IF l_pmk[g_i].choice = 'Y' THEN
                   LET l_cnt = l_cnt + 1
                 END IF
                END FOR
                DISPLAY l_cnt TO FORMONLY.cmt
            ...
            ON ACTION select_all
               LET l_cnt = 0
               FOR g_i = 1 to g_cnt
                   LET l_pmk[g_i].choice = 'Y'
                   DISPLAY BY NAME l_pmk[g_i].choice 
                   LET l_cnt = l_cnt +1
               END FOR 
               display l_cnt TO FORMONLY.cmt
            ...
            #   LET l_sql = "apmr903 '",l_pmk[l_ac].pmk01[1,10],"'" 
            #   LET l_sql = l_sql clipped," '",g_today,"' '' '' " 
            #   LET l_sql = l_sql CLIPPED, "'N' 'Y' 'N'"
            #   LET l_sql = l_sql clipped
 
                LET l_sql="apmr903 '",g_today,"' '' '",g_lang,"'",
                    " 'Y' 'D' '1' ' pmk01="
         ## 若在 GUI 的環境下將 'D':pg 改為 'V':dsview
          ...
          --# END IF
            ...
            #   LET l_sql=l_sql CLIPPED,'"',l_pmk[l_ac].pmk01[1,10],'"'
                LET l_sql=l_sql CLIPPED,'"',l_pmk[l_ac].pmk01[1,g_no_ep],'"'   #No.FUN-550058
                LET l_sql=l_sql CLIPPED, "' 'N' 'Y' 'N'"
      ...
      #  CALL cl_set_act_visible("accept,cancel", TRUE)
        ...
        LET l_sl=0
        FOR l_ac=1 TO g_cnt
            IF l_pmk[l_ac].choice='Y' THEN          #該單據要簽核
                #寫入簽核過程檔
                LET g_success = 'Y'
                BEGIN WORK
    #           LET l_pmk[l_ac].pmk01=l_pmk[l_ac].pmk01[1,10] 
                LET l_pmk[l_ac].pmk01=l_pmk[l_ac].pmk01[1,g_no_ep]    #No.FUN-550058
                INSERT INTO azd_file(azd01,azd02,azd03,azd04)
                    VALUES(l_pmk[l_ac].pmk01,2,g_pass,g_today)
                IF SQLCA.sqlcode THEN
                   LET g_success = 'N' 
#                  CALL cl_err('ckp#2',SQLCA.sqlcode,1)   #No.FUN-660131
                   CALL cl_err3("ins","azd_file",l_pmk[l_ac].pmk01,g_pass,SQLCA.sqlcode,"","ckp#2",1)    #No.FUN-660131
                END IF
                #請記得要改單據性質 ----------+
                #更新已簽核順序
{chkp#1}        UPDATE pmk_file
                    SET pmksseq=pmksseq+1
                    WHERE pmk01=l_pmk[l_ac].pmk01
                UPDATE pmk_file
                    SET pmk25='1'
                    WHERE pmk01=l_pmk[l_ac].pmk01 AND
                          pmksseq=pmksmax
                
                IF SQLCA.sqlcode THEN
                   LET g_success = 'N' 
#                  CALL cl_err('ckp#1',SQLCA.sqlcode,1)   #No.FUN-660131
                   CALL cl_err3("upd","pmk_file",l_pmk[l_ac].pmk01,"",SQLCA.sqlcode,"","ckp#1",1)    #No.FUN-660131
                END IF
                #更改單身之狀況碼
                IF SQLCA.SQLERRD[3]=1 THEN
  {chk#3}           UPDATE pml_file
                        SET pml16='1'
                        WHERE pml01=l_pmk[l_ac].pmk01
                    IF SQLCA.sqlcode THEN
                       LET g_success = 'N' 
#                      CALL cl_err('ckp#3',SQLCA.sqlcode,1)   #No.FUN-660131
                       CALL cl_err3("upd","pml_file",l_pmk[l_ac].pmk01,"",SQLCA.sqlcode,"","ckp#3",1)  #No.FUN-660131
                    END IF
                END IF
                LET l_sl=l_sl+1
