...
#
# Pattern name...: admr102.4gl
# Descriptions...: 訂單追蹤延遲警訊表(業務員別)
# Input parameter:
...
# Modify.........: No.MOD-810087 08/01/11 By Carol 過濾條件未考慮到tm.dd
# Modify.........: No.FUN-850107 08/05/21 By Cockroach 報表轉CR
# Modify.........: No.FUN-870144 08/07/29 By lutingting  報表轉CR追到31區
# Modify.........: No.MOD-960254 09/06/30 By Smapmin 應依訂單項次抓取最後出貨日 
...
#No.FUN-850107 --ADD START--  
 LET g_sql =         "oea14.oea_file.oea14,",
   ...
   LET l_table = cl_prt_temptable('admr102',g_sql) CLIPPED
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('admr102',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW r102_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690111
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8         #No.FUN-6A0100
          ...
          sr         RECORD
                     oea14     LIKE    oea_file.oea14,    #業務員編號
...
#No.FUN-850107 --ADD END-- 
     #-----MOD-960254---------
     #LET l_sql = " SELECT oea14,oea03,oea032,oea01,oeb03,oeb15,max(oga02),oeb12,oeb24,oeb13,",
     ...
     #            " AND (TODAY - oeb15) > ",tm.dd ,  #MOD-810087 #CHI-710043 add
     #            " GROUP BY oea14,oea03,oea032,oea01,oeb03,oeb15,oeb12,oeb24,",
     #            #" oeb13,ima02,ima021,ogb01,oea23" #TQC-5B0141 mark
     #            " oeb13,ima02,ima021,ogb01,oea23,oea24,azi04" #TQC-5B0141 add
     LET l_sql = " SELECT oea14,oea03,oea032,oea01,oeb03,oeb15,'',oeb12,oeb24,oeb13,",
     ...
     PREPARE r102_p1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690111
        EXIT PROGRAM
     END IF
     DECLARE r102_c1 CURSOR FOR r102_p1
     IF SQLCA.sqlcode != 0 THEN
...
#    CALL cl_outnam('admr102') RETURNING l_name
...
#No.FUN-850107 --ADD START--  
    ...
    sr.oea14,sr.oea03,sr.oea032,sr.oea01,sr.oeb03,sr.oeb15,
   ...
   CALL cl_prt_cs3('admr102','admr102',l_sql,g_str)   
#No.FUN-850107 --ADD END--
...
# ORDER BY sr.oea14,sr.oea03,sr.oea01,sr.oeb03
...
#  BEFORE GROUP OF sr.oea14
#     LET l_amt=0
#     PRINT COLUMN g_c[31],sr.oea14,
#           COLUMN g_c[32],sr.oea032;
 
#  BEFORE GROUP OF sr.oea01
#     PRINT COLUMN g_c[33],sr.oea01;
...
#           COLUMN g_c[36],(g_today - sr.oeb15) CLIPPED USING '#####&',
#           COLUMN g_c[37],cl_numfor(l_aa,37,sr.azi04),  #金額
#           COLUMN g_c[38],sr.ima02 CLIPPED,
#           COLUMN g_c[39],sr.ima021 CLIPPED
 
#  AFTER GROUP OF sr.oea14
#     PRINT g_dash2
#     PRINT COLUMN g_c[35],g_x[9] CLIPPED,
#           COLUMN g_c[37],cl_numfor(l_amt,37,sr.azi04)
#     PRINT
 
#  ON LAST ROW
#     PRINT g_dash2
#     PRINT COLUMN g_c[35],g_x[10] CLIPPED,
#           COLUMN g_c[37],cl_numfor(l_amt1,37,sr.azi04)
