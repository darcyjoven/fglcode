...
# Modify.........: No.MOD-BC0160 11/12/19 By Polly 此報表為共用 axrt400/axrt401/axrt410 的部分,不應限制 ooa37 條件 
# Modify.........: No.TQC-C10039 12/01/17 By JinJJ  CR報表列印TIPTOP與EasyFlow簽核圖片
# Modify.........: No.FUN-C40001 12/04/13 By yinhy 增加開窗功能
# Modify.........: No.TQC-C70130 12/07/20 By lujh  去掉ooa37=1的限制條件
...
MAIN
   ...
   LET l_table3 = cl_prt_temptable('axrr4003',g_sql) CLIPPED                    
   ...
   LET l_table4 = cl_prt_temptable('axrr4004',g_sql) CLIPPED                    
   ...
   CREATE TEMP TABLE r400_tmp
   (tmp01 LIKE azi_file.azi01,
    tmp02 LIKE type_file.chr1,  
    tmp03 LIKE type_file.num20_6,
    tmp04 LIKE type_file.num20_6)
          #No.FUN-680123 end
   create unique index r400_tmp_01 on r400_tmp(tmp01,tmp02);
   IF cl_null(tm.wc) THEN
      CALL axrr400_tm(0,0)             # Input print condition
   ELSE 
      CALL axrr400()                   # Read data and create out-file
   END IF
   DROP TABLE r400_tmp
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
END MAIN
 
FUNCTION axrr400_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01        #No.FUN-580031
   ...
   OPEN WINDOW axrr400_w AT p_row,p_col
        WITH FORM "axr/42f/axrr400"
################################################################################
...
WHILE TRUE
        ...
        ON ACTION CONTROLP
           CASE
              WHEN INFIELD(ooa01)
                 ...
                 #LET g_qryparam.form = "q_ooa"     #TQC-C70130  mark
                 LET g_qryparam.form = "q_ooa3"     #TQC-C70130  add
                 #LET g_qryparam.arg1 = "1"         #TQC-C70130  mark
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axrr400_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axrr400',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axrr400_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#  DEFINE l_ii           INTEGER                                            #TQC-C10039 mark
#  DEFINE l_sql_2        LIKE type_file.chr1000        # RDSQL STATEMENT    #TQC-C10039 mark
#  DEFINE l_key          RECORD                  #主鍵                      #TQC-C10039 mark
#             v1          LIKE ooa_file.ooa01                               #TQC-C10039 mark
#                         END RECORD                                        #TQC-C10039 mark
   ###FUN-940042 END ###
...
#              "       oob11,aag02,oob12,",       #No.FUN-740009                                                                    
              ...
              #"   AND ooa37 = '1'",        #FUN-B20014  #MOD-BC0160 mark
               "   AND ",tm.wc CLIPPED,
               " ORDER BY ooa01 "
     PREPARE axrr400_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
        EXIT PROGRAM
     END IF
     DECLARE axrr400_curs1 CURSOR FOR axrr400_prepare1
 
#     CALL cl_outnam('axrr400') RETURNING l_name       #No.FUN-7A0025
#     START REPORT axrr400_rep TO l_name               #No.FUN-7A0025
#
#     LET g_pageno = 0                                 #No.FUN-7A0025
     FOREACH axrr400_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#       OUTPUT TO REPORT axrr400_rep(sr.*)                  #No.FUN-7A0025
       #-----MOD-810076---------
       #No.FUN-7A0025 --start--
       ...
       EXECUTE insert_prep USING sr.ooa01,sr.ooa02,sr.ooa03,sr.ooa032,sr.ooa15,
                                 ...
                                 sr.azi07,
                                 "",l_img_blob,"N",""    #FUN-940042  #TQC-C10039 add ""
     #No.FUN-7A0025 --end--
     END FOREACH
     #-----TQC-810052---------
     LET l_sql = "SELECT DISTINCT ooa01 FROM ",
                  g_cr_db_str CLIPPED,l_table CLIPPED
     PREPARE r400_p FROM l_sql
     DECLARE r400_curs CURSOR FOR r400_p
     FOREACH r400_curs INTO sr.ooa01
        DECLARE memo_c1 CURSOR FOR 
         SELECT oao06 FROM oao_file                 
           WHERE oao01=sr.ooa01                     
             AND oao03=0 AND oao05='1'                
...
#     FINISH REPORT axrr400_rep                   #No.FUN-7A0025
...
#     END FOREACH
     ###FUN-940042 END ###
#TQC-C10039--END MARK--
     CALL cl_prt_cs3('axrr400','axrr400',l_sql,g_str)
     #No.FUN-7A0025 --end--
END FUNCTION
#No.FUN-7A0025 --start-- mark
{REPORT axrr400_rep(sr)
...
#     PRINT g_head CLIPPED, pageno_total  #No.TQC-770028 mark
   ...
   ON EVERY ROW
      SELECT COUNT(*) INTO l_n FROM r400_tmp
       WHERE tmp01=sr.oob07 AND tmp02=sr.oob03
      IF NOT cl_null(sr.oob07) THEN
         IF l_n > 0 THEN
            UPDATE r400_tmp SET tmp03=tmp03+sr.oob09,
                                tmp04=tmp04+sr.oob10
             WHERE tmp01=sr.oob07 AND tmp02=sr.oob03
         ELSE
           INSERT INTO r400_tmp VALUES(sr.oob07,sr.oob03,sr.oob09,sr.oob10)
         END IF
      END IF
   ...
   AFTER GROUP OF sr.ooa01
      ...
      #依幣別總計
      DECLARE r400_tmp_cs CURSOR FOR SELECT * FROM r400_tmp ORDER BY tmp01,tmp02
      FOREACH r400_tmp_cs INTO st.*
