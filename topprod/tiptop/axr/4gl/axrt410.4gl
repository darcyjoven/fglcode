...
# Modify.........: No.MOD-B70162 11/07/17 By Polly 調整拋轉傳票(axrp590)使用g_ooa.oaauser 
# Modify.........: No:TQC-B70021 11/07/18 By elva 现金流量表修正
# Modify.........: No:MOD-B70234 11/08/25 By yinhy 貸方單身參考單號欄位應可以錄入15，17類型
# Modify.........: No:MOD-B70242 11/08/26 By yinhy 單身參考單號欄位應可以錄入單據性質為28單據
# Modify.........: No:FUN-B80072 11/08/08 By Lujh 模組程序撰寫規範修正
# Modify.........: No:TQC-B90100 11/09/15 By yinhy 查詢時，資料建立者，資料建立部門欄位無法下查詢條件
# Modify.........: No.FUN-B90062 11/09/16 By wujie 产生nme_file时同时产生tic_file   
# Modify.........: No.MOD-BC0031 11/12/05 By yinhy 自動產生單身時，從T/T取資料時會將已經衝過帳的單據重復抓出
# Modify.........: No.MOD-C10172 12/01/19 By yinhy 單據取消審核時報錯
# Modify.........: No:FUN-C10039 12/02/02 by Hiko 整批修改資料歸屬設定
# Modify.........: No.TQC-C20430 12/02/27 By wangrr 刪除操作時更新財務編號luc28=NULL
# Modify.........: No:MOD-C20207 12/03/01 By Polly 拋轉傳票判斷修改
# Modify.........: No.FUN-C10024 12/05/21 By minpp 帳套取歷年主會計帳別檔tna_file
# Modify.........: No.CHI-C30107 12/06/12 By yuhuabao  整批修改將確認的詢問窗口放到chk段的前面
# Modify.........: No.TQC-C60155 12/06/20 By lujh 没有参考单号的情况下输入银行编号，应带入相应的科目
# Modify.........: No.TQC-C60212 12/07/02 By yuhuabao 調整CHI-C30107修改的bug
# Modify.........: No.FUN-C30085 12/07/06 By lixiang 改CR報表串GR報表
# Modify.........: No.MOD-C70125 12/07/12 By Polly 將借貸方9的選項功能mark，取消貸方T/T來源選項
...
# Modify.........: No:FUN-C90092 12/10/17 by minpp 1.画面栏位oob04贷方增加Z 2.修改贷方oob06的开窗及栏位检查
#..................................................3.审核向nmh_file 给值，取消审核删除
# Modify.........: No.MOD-CA0070 12/10/11 By Polly 取消sw2在畫面上顯示
# Modify.........: No.MOD-CC0005 12/12/03 By yinhy 點"列印"按鈕后，增加退出功能
# Modify.........: No:FUN-CB0080 13/01/10 By wangrr 9主機追單到30主機,新增資料清單頁簽
# Modify.........: No.FUN-D10098 13/02/04 By lujh 大陸版本用gxrg420
# Modify.........: No.FUN-D20035 13/02/19 By nanbing 將作廢功能分成作廢與取消作廢2個action
# Modify.........: No:FUN-CB0044 13/02/21 By minpp 重整科目类型，借方拿掉：Q:券，E:联盟卡，贷方拿掉：B:转收入，转费用
# Modify.........: No.FUN-D10058 13/03/07 By lujh aza69不勾選時，隱藏ooa992
...
#FUN-A90003--Add--End--#
       ...
       l_ac1            LIKE type_file.num5,              #No.FUN-CB0080             #目前處理的ARRAY CNT
...
DEFINE g_ooa_1          DYNAMIC ARRAY OF RECORD
          ...
          ooa35         LIKE ooa_file.ooa35,
          ooa36         LIKE ooa_file.ooa36
                        END RECORD
...
#FUN-580154
   ...
   #END FUN-640246
    ...
    CALL aws_efapp_flowaction("insert, modify, delete, reproduce, detail, query,locale, void, undo_void,confirm, undo_confirm, easyflow_approval,gen_entry,entry_sheet,carry_voucher,undo_carry_voucher")  #TQC-740156 modify #FUN-D20035 add undo_void
...
DEFINE  lc_qbe_sn       LIKE    gbm_file.gbm01   
   ...
   DECLARE t410_count CURSOR FOR t410_precount
   #No.FUN-CB0080 ---start--- Add
   LET g_sql = "SELECT ooa01,ooa02,ooa03,ooa032,ooa14,'',ooa15,'',",
               "       ooa23,ooa33,ooaconf,ooa34,ooa35,ooa36      "
   IF g_wc3 = " 1=1" THEN
      IF g_wc2 = " 1=1" THEN
         LET g_sql = g_sql,"  FROM ooa_file ",
                           " WHERE ooa00 = '1' AND ooa37 = '2' AND ",g_wc CLIPPED,
                           " ORDER BY ooa01"
      ELSE
         LET g_sql = g_sql,"  FROM ooa_file,oob_file ",
                           " WHERE ooa00 = '1' AND ooa37 = '2' AND ",g_wc CLIPPED,
                           "   AND (",g_wc2 CLIPPED," AND oob03='1')",
                           " ORDER BY ooa01"
      END IF
   ...
   PREPARE axrt410_prepare FROM g_sql
   DECLARE axrt410_list_cur CURSOR FOR axrt410_prepare
  #No.FUN-CB0080 ---end  --- Add
...
#FUN-580154
               ...
               IF g_success = "Y" THEN
                  CALL t410_y_upd()       #CALL 原確認的 update 段
                  CALL t410_b_fill_1()   #No.FUN-CB0080   Add
               END IF
#FUN-580154 End
            END IF
         ...
         WHEN "void"
            IF cl_chk_act_auth() THEN
               #CALL t410_x()  #FUN-D20035 mark
               CALL t410_x(1)  #FUN-D20035 add
            END IF
#FUN-D20035 add
         WHEN "undo_void"
            IF cl_chk_act_auth() THEN
...
#     LET g_ooa.ooa37 = 'Y'              #FUN-960141    #FUN-A40076 Mark
      ...
      IF SQLCA.SQLCODE OR SQLCA.SQLERRD[3]=0 THEN
         CALL cl_err3("ins","ooa_file",g_ooa.ooa01,"",SQLCA.sqlcode,"","ins ooa",1)  #No.FUN-660116      #FUN-B80072   ADD
         ROLLBACK WORK  #No:7837
        # CALL cl_err3("ins","ooa_file",g_ooa.ooa01,"",SQLCA.sqlcode,"","ins ooa",1)  #No.FUN-660116     #FUN-B80072   MARK  
         CONTINUE WHILE
      ELSE
         COMMIT WORK   #No:7837
...
FUNCTION t410_g_b1()
   ...
   CALL cl_set_comp_visible("sw2",FALSE)   #TQC-A20043 #MOD-CA0070 remark
   CALL cl_set_comp_visible("sw4",FALSE)
   #FUN-960141 end
   CALL cl_ui_locale("axrt4003")

  #LET sw2 = 'Y'    #TQC-A20043  #MOD-C70125 mark
   LET sw3 = 'Y'
 
  #INPUT BY NAME sw2, sw3 WITHOUT DEFAULTS    #TQC-A20043 add sw2 #MOD-C70125 mark sw2
   INPUT BY NAME sw3 WITHOUT DEFAULTS         #MOD-C70125 add
...
DEFINE l_tic02     LIKE tic_file.tic02   #FUN-E80012 add
   IF cl_null(g_ooa.ooa33) THEN
      ...
      IF g_nmz.nmz70 = '3' THEN
         LET l_tic01=s_get_aznn(g_plant,g_bookno1,g_ooa.ooa02,1)
         LET l_tic02=s_get_aznn(g_plant,g_bookno1,g_ooa.ooa02,3)
         UPDATE tic_file SET tic01=l_tic01,
                             tic02=l_tic02
         WHERE tic04=g_ooa.ooa01
         IF STATUS THEN
            CALL cl_err3("upd","tic_file",g_ooa.ooa01,"",STATUS,"","upd tic01 tic02",1)
         END IF
      END IF
      #FUN-E80012---add---end---
   END IF
...
FUNCTION t410_show()
    ...
    DISPLAY BY NAME g_ooa.ooa01,g_ooa.ooa02,g_ooa.ooa03,g_ooa.ooa032, g_ooa.ooaoriu,g_ooa.ooaorig,
                   ...
                   #FUN-850038     ---start---
                    ...
                    g_ooa.ooaud13,g_ooa.ooaud14,g_ooa.ooaud15,
                   #FUN-850038     ----end----
                    g_ooa.ooaoriu,g_ooa.ooaorig            #TQC-B90100
...
#MOD-950129 --begin--
           IF g_ooz.ooz62 = 'Y' THEN  
              ...
              END IF
           END IF
          #IF g_oob[l_ac].oob03='2' AND g_oob[l_ac].oob04 MATCHES "[19]" THEN      #MOD-C70125 mark
           IF g_oob[l_ac].oob03='2' AND g_oob[l_ac].oob04 MATCHES "[1]" THEN       #MOD-C70125 add
                 ...
                 IF NOT cl_null(g_oob[l_ac].oob04) THEN                                                                             
                    ...
                    IF l_cnt = 0 THEN                                                                                               
                       IF g_oob[l_ac].oob03='1' THEN                                                                                
                          ...
                          #IF g_oob[l_ac].oob04 NOT MATCHES '[1-9,A,F]' THEN   #FUN-A40001 #MOD-C70125 mark
                           IF g_oob[l_ac].oob04 NOT MATCHES '[1-8,A,F]' THEN   #MOD-C70125 add
                          #End
                       ...
                       ELSE                                                                                                         
                          ...
                          #IF g_oob[l_ac].oob04 NOT MATCHES '[1,2,4,7,9,A,C,D,E,Q]' THEN       #FUN-A40001 #MOD-C70125 mark
                           IF g_oob[l_ac].oob04 NOT MATCHES '[1,2,4,7,A,C,D,E,Q]' THEN         #MOD-C70125 add
                          #End
                             CALL cl_err('','axr-917',0)                                                                            
                             NEXT FIELD oob04                                                                                       
                          END IF                                                                                                    
           ...
           IF NOT cl_null(g_oob[l_ac].oob04) THEN
              ...
              IF l_cnt = 0 THEN
                 IF g_oob[l_ac].oob03='1' THEN
                   #FUN-960141 modify
                   #IF g_oob[l_ac].oob04 NOT MATCHES '[1-9]' THEN
                   #IF g_oob[l_ac].oob04 NOT MATCHES '[1-9,A,E,F,Q]' THEN   #MOD-C70125 mark
                   #IF g_oob[l_ac].oob04 NOT MATCHES '[1-8,A,E,F,Q]' THEN   #MOD-C70125 add    #FUN-CB0044
                    IF g_oob[l_ac].oob04 NOT MATCHES '[1-8,A,F]' THEN       #FUN-CB0044 del-E,Q
                   #End
                       CALL cl_err('','axr-917',0)
                       NEXT FIELD oob04
                    END IF
                 ...
                 ELSE
                   ...
                   #IF g_oob[l_ac].oob04 NOT MATCHES '[1,2,4,7,9]' THEN
                   #IF g_oob[l_ac].oob04 NOT MATCHES '[1,2,4,7,9,A,B,C,D,E,F,Q]' THEN   #MOD-9B0043
                   #IF g_oob[l_ac].oob04 NOT MATCHES '[1,2,4,7,9,A,B,C,D,E,Q]' THEN     #MOD-9B0043 #MOD-C70125 mark
                   #End 
                   #IF g_oob[l_ac].oob04 NOT MATCHES '[1,2,4,7,A,B,C,D,E,Q]' THEN       #MOD-C70125 add   #FUN-CB0044 mark
                    IF g_oob[l_ac].oob04 NOT MATCHES '[1,2,4,7,A,B,C,D]' THEN           #FUN-CB0044 del-E,Q
                      CALL cl_err('','axr-917',0)
                      NEXT FIELD oob04
                    END IF
                 END IF
...
#MOD-940420  add --begin
            #IF g_oob[l_ac].oob03='1' AND g_oob[l_ac].oob04 MATCHES "[39]" THEN      #MOD-C70125 mark
             IF g_oob[l_ac].oob03='1' AND g_oob[l_ac].oob04 MATCHES "[3]" THEN       #MOD-C70125 add
             ...
             #         IF (l_oma00!='21') AND (l_oma00!='22') AND (l_oma00!='23') AND (l_oma00!='24') AND (l_oma00!='27') THEN    #TQC-AC0397 
                       IF (l_oma00!='21') AND (l_oma00!='22') AND (l_oma00!='23') AND 
                          (l_oma00!='24') AND (l_oma00!='26') AND (l_oma00!='27') AND  #TQC-B1005
                          (l_oma00!='28') THEN  #MOD-B70242 add 28
...
#MOD-940420  add  --begin
                   IF g_oob[l_ac].oob04 = '1' THEN 
                      ...
                      IF (l_oma00!='11') AND (l_oma00!='12') AND
                         (l_oma00!='13') AND (l_oma00!='14') AND
                         (l_oma00!='15') AND (l_oma00!='17') THEN  #MOD-B70234 add 15,17
...
#MOD-940420  add --end                                      
       ...
       #FUN-680022 --end--
        ...
        AFTER FIELD oob15
           ...
           #No.TQC-810034--begin--
           #IF g_oob[l_ac].oob03='2' AND g_oob[l_ac].oob04 MATCHES "[19]" THEN   #MOD-850006 #MOD-C70125 mark
            IF g_oob[l_ac].oob03='2' AND g_oob[l_ac].oob04 MATCHES "[1]" THEN    #MOD-C70125 add
               IF NOT cl_null(g_oob[l_ac].oob15) THEN 
                  CALL t410_oob06_chk(g_oob[l_ac].oob06,g_oob[l_ac].oob15,l_ac)
                  IF NOT cl_null(g_errno) THEN 
...
#MOD-740395.....end
        ...
        BEFORE FIELD oob07
          ...
          #IF (g_oob[l_ac].oob03='1' AND g_oob[l_ac].oob04 MATCHES "[1239]") OR    #MOD-C70125 mark
           IF (g_oob[l_ac].oob03='1' AND g_oob[l_ac].oob04 MATCHES "[123]") OR     #MOD-C70125 add
...
#      IF g_oob[l_ac].oob04 NOT MATCHES '[34]' THEN                  #No.MOD-740406
      ...
      #IF g_oob[l_ac].oob04 NOT MATCHES '[349]' AND   #MOD-8B0210 #MOD-C70125 mark
       IF g_oob[l_ac].oob04 NOT MATCHES '[34]' AND    #MOD-C70125 add
    ...
    IF g_oob[l_ac].oob03='2' THEN
      ...
      #END IF
      #--------------------MOD-C70125-------------mark
      #IF g_oob[l_ac].oob04 NOT MATCHES '[19]' THEN     #MOD-C70125 mark
       IF g_oob[l_ac].oob04 NOT MATCHES '[1]' THEN      #MOD-C70125 add
          CALL cl_set_comp_entry("oob15",TRUE)
       END IF
    END IF
...
#    IF g_oob[l_ac].oob04='2' THEN CALL t410_oob06_12() END IF   #MOD-630069
    ...
    #IF g_oob[l_ac].oob04='9' THEN CALL t410_oob06_19('1') END IF   #MOD-750063 #MOD-C70125 mark
  END IF
  IF g_oob[l_ac].oob03='2' THEN
     IF g_oob[l_ac].oob04='1' THEN CALL t410_oob06_13('1') END IF
    #IF g_oob[l_ac].oob04='9' THEN CALL t410_oob06_19() END IF   #MOD-750063
    #IF g_oob[l_ac].oob04='9' THEN CALL t410_oob06_19('2') END IF   #MOD-750063 #MOD-C70125 mark
  END IF
       ...
       # IF g_ooz.ooz62 = 'Y' THEN  #No.MOD-8A0111
          ...
          #No.FUN-680022--end-- mark
           #IF (tot6+g_oob[l_ac].oob10) > tot3 THEN  #No.FUN-680022 --mark
            IF (tot8+g_oob[l_ac].oob10) > l_omc.omc13 THEN  #No.FUN-680022 --add
               CALL cl_err('','axr-185',1)
           ...
           #原幣沖完但本幣未沖完
            IF (tot1+g_oob[l_ac].oob09) = (l_oma.oma54t-l_oma.oma55)   AND
               (tot8+g_oob[l_ac].oob10 - g_net)!= (l_omc.omc09  - l_omc.omc11) THEN  #yinhy130801 add g_net
               CALL cl_err('','axr-193',1)
            END IF
           #end MOD-940262 add
    ...
    MENU ""
       ...
       ON ACTION print_this_pay_slip
         ...
         #LET g_msg = "axrr400 '' '' '",g_lang,"' 'Y' '' '' ",  #FUN-C30085
          #LET g_msg = "axrg400 '' '' '",g_lang,"' 'Y' '' '' ",  #FUN-C30085   #TQC-C70127  mark
          #            "'ooa01 =\"",g_ooa.ooa01,"\"' 'g_ooa.ooaconf' "         #TQC-C70127  mark 
          #TQC-C70127--add--str--
          IF cl_null(g_ooa.ooa35) THEN 
             LET g_ooa.ooa35 = 6
          END IF
          #FUN-D10098--add--str--
          IF g_aza.aza26 = '2' THEN
             LET g_msg = "gxrg420 '' '' '",g_lang,"' 'Y' '' '' ",      
                         "'ooa01 =\"",g_ooa.ooa01,"\"' '3' '' '' '' '' '",g_ooa.ooa35,"' "   
          ELSE
          #FUN-D10098--add--end--
             LET g_msg = "axrg420 '' '' '",g_lang,"' 'Y' '' '' ",
                         "'ooa01 =\"",g_ooa.ooa01,"\"' '3' '' '' '' '' '",g_ooa.ooa35,"' "
          END IF
          #TQC-C70127--add--end--
          #-----END TQC-660010-----
...
#MOD-940420  --begin   
   LET l_sql = "SELECT oob03,oob04,oob06 FROM oob_file",
               " WHERE oob01 = '",g_ooa.ooa01,"'",
              #"   AND ((oob03 = '1' AND (oob04='3' OR oob04='9'))",     #MOD-C70125 mark
              #"    OR  (oob03 = '2' AND (oob04='1' OR oob04='9')))"     #MOD-C70125 mark
               "   AND ((oob03 = '1' AND oob04='3' )",                   #MOD-C70125 add
               "    OR  (oob03 = '2' AND oob04='1' ))"                   #MOD-C70125 add
  ...
  #          IF (l_oma00 != '21') AND (l_oma00 != '22') AND (l_oma00 !='23') AND (l_oma00 != '24') THEN   #MOD-9B0043 add 23     #TQC-B10005 mark     
             IF (l_oma00 != '21') AND (l_oma00 != '22') AND (l_oma00 !='23') AND 
                (l_oma00 != '24') AND (l_oma00 != '26') AND (l_oma00 !='27') AND      #TQC-B10005
                (l_oma00 != '28') THEN  #MOD-B70242 add 28
...
#      CALL s_showmsg()                    #NO.FUN-710050  #No.TQC-740094
  ...
  IF g_ooy.ooydmy1 = 'Y' AND g_ooy.ooyglcr = 'Y' AND g_success = 'Y' THEN
    ...
    #LET g_str="axrp590 '",g_wc_gl CLIPPED,"' '",g_user,"' '",g_user,"' '",g_ooz.ooz02p,"' '",g_ooz.ooz02b,"' '",g_ooy.ooygslp,"' '",g_ooa.ooa02,"' 'Y' '1' 'Y' '",g_ooz.ooz02c,"' '",g_ooy.ooygslp1,"'"   #No.FUN-670047  #No.MOD-860075  #No.MOD-B70162 mark
     LET g_str="axrp590 '",g_wc_gl CLIPPED,"' '",g_ooa.ooauser,"' '",g_ooa.ooauser,"' '",g_ooz.ooz02p,"' '",g_ooz.ooz02b,"' '",g_ooy.ooygslp,"' '",g_ooa.ooa02,"' 'Y' '1' 'Y' '",g_ooz.ooz02c,"' '",g_ooy.ooygslp1,"'"  #No.MOD-B70162 add
...
#     LET g_plant_new = b_oob.oob05 CALL s_getdbs()
     ...
     #----------------------MOD-C70125---------------------------------------mark
      ...
      #FUN-960141 add end
      #FUN-C90092--add--str
      IF b_oob.oob03 = '2' AND b_oob.oob04 = 'Z' THEN
         CALL t410_bu_2Z('+')
      END IF
      #FUN-C90092--add--end 
      LET l_cnt = l_cnt + 1
...
#   LET g_plant_new = b_oob.oob05 CALL s_getdbs()
   ...
   #IF b_oob.oob03 = '1' AND b_oob.oob04 = '9' THEN CALL t410_bu_19('-') END IF   #MOD-C70125 mark
    IF b_oob.oob03 = '2' AND b_oob.oob04 = '1' THEN CALL t410_bu_21('-') END IF
   #IF b_oob.oob03 = '2' AND b_oob.oob04 = '9' THEN CALL t410_bu_19('-') END IF   #MOD-C70125 mark
    IF b_oob.oob03 = '2' AND b_oob.oob04 = 'Z' THEN CALL t410_bu_2Z('-') END IF    #FUN-C90092
    IF b_oob.oob03 = '2' AND b_oob.oob04 = '2' THEN
       CALL t410_bu_22('-',l_cnt)
    END IF
   #FUN-960141 add begin
...
#-------------------------MOD-C70125------------------mark
#FUN-C90092--add--str
FUNCTION t410_bu_2Z(p_sw)
DEFINE  l_oob         RECORD LIKE oob_file.*
DEFINE  p_sw          LIKE type_file.chr1

DECLARE t410_bu_cd CURSOR FOR
    SELECT * FROM oob_file WHERE oob01 = g_ooa.ooa01
                             AND oob04 = 'Z'

         FOREACH t410_bu_cd INTO l_oob.*
            IF p_sw='+' THEN
               CALL t410_ins_nmh(l_oob.*)
            ELSE
               CALL t300_del_nmh(l_oob.*)
            END IF
            IF g_success='N' THEN
               CONTINUE FOREACH
            END IF
...
FUNCTION t410_ins_nmh(l_oob)
DEFINE l_nmh42    LIKE  nmh_file.nmh42
DEFINE l_oob      RECORD LIKE oob_file.*

   IF l_oob.oob04 != 'Z' THEN RETURN END IF
   ...
   UPDATE nmh_file SET nmh42 =l_oob.oob09 +l_nmh42
    WHERE nmh01 = l_oob.oob06
     IF STATUS THEN
        CALL s_errmsg('nmh01','l_oob.oob06','upd nmh42',STATUS,1)              #NO.FUN-710050
        LET g_success = 'N'
        RETURN
     END IF
     IF SQLCA.SQLERRD[3]=0 THEN
        CALL s_errmsg('nmh01','l_oob.oob06','upd nmh42','axr-198',1) LET g_success = 'N' RETURN      #NO.FUN-710050
     END IF

END FUNCTION

FUNCTION t300_del_nmh(l_oob)
DEFINE l_oob      RECORD LIKE oob_file.*
DEFINE l_nmh42    LIKE nmh_file.nmh42
...
IF l_oob.oob04 != 'Z' THEN RETURN END IF
   ...
   UPDATE nmh_file SET nmh42 = l_nmh42 - l_oob.oob09
    WHERE nmh01 = l_oob.oob06
     IF STATUS THEN
        CALL s_errmsg('nmh01',l_oob.oob06,'upd nmh42',STATUS,1)              #NO.FUN-710050
        LET g_success = 'N'
...
#FUNCTION t410_x() #FUN-D20035 mark
FUNCTION t410_x(p_type) #FUN-D20035 add
...
FUNCTION t410_carry_voucher()
  ...
  DEFINE l_n          LIKE type_file.num5      #No.FUN-680123 SMALLINT 
   ...
   #IF g_ooy.ooyglcr = 'Y' THEN                                                           #MOD-C20207 mark
    IF g_ooy.ooyglcr = 'Y' OR (g_ooy.ooyglcr ='N' AND NOT cl_null(g_ooy.ooygslp)) THEN    #MOD-C20207 add
    ...
    ELSE
      #CALL cl_err('','aap-992',1)     #MOD-C20207 mark
       CALL cl_err('','aap-936',1)     #MOD-C20207 add
       RETURN
       #開窗作業
#      LET g_plant_new= g_ooz.ooz02p
#      CALL s_getdbs()
...
#      END INPUT
   ...
   #LET g_str="axrp590 '",g_wc_gl CLIPPED,"' '",g_user,"' '",g_user,"' '",g_ooz.ooz02p,"' '",g_ooz.ooz02b,"' '",l_ooygslp,"' '",g_ooa.ooa02,"' 'Y' '1' 'Y' '",g_ooz.ooz02c,"' '",g_ooy.ooygslp1,"'"  #No.FUN-670047  #No.MOD-860075 #No.MOD-B70162 mark
    LET g_str="axrp590 '",g_wc_gl CLIPPED,"' '",g_ooa.ooauser,"' '",g_ooa.ooauser,"' '",g_ooz.ooz02p,"' '",g_ooz.ooz02b,"' '",l_ooygslp,"' '",g_ooa.ooa02,"' 'Y' '1' 'Y' '",g_ooz.ooz02c,"' '",g_ooy.ooygslp1,"'"    #No.MOD-B70162 add 
    CALL cl_cmdrun_wait(g_str)
    SELECT ooa33 INTO g_ooa.ooa33 FROM ooa_file
     WHERE ooa01 = g_ooa.ooa01
    DISPLAY BY NAME g_ooa.ooa33
...
FUNCTION t410_oob17(p_code)
   ...
   DEFINE l_nma05      LIKE  nma_file.nma05   #TQC-C60155  add 
   DEFINE l_nma051     LIKE  nma_file.nma051  #TQC-C60155  add

   LET g_errno = ''
   SELECT nma01,nmaacti,nma05,nma051 INTO l_nma01,l_nmaacti,l_nma05,l_nma051 FROM nma_file     #TQC-C60155  add  nma05,nma051
    WHERE nma01 = p_code
   CASE WHEN SQLCA.sqlcode = 100     LET g_errno = 'aap-007'
    ...
    END CASE 
    #TQC-C60155--add--str--
    IF cl_null(g_errno) AND cl_null(g_oob_d[l_ac2].oob06_1) THEN
      LET g_oob_d[l_ac2].oob11_1 = l_nma05
      DISPLAY BY NAME g_oob_d[l_ac2].oob11_1
      IF g_aza.aza63 = 'Y' THEN
         LET g_oob_d[l_ac2].oob111_1 = l_nma051
         DISPLAY BY NAME g_oob_d[l_ac2].oob111_1
      END IF
    END IF
    #TQC-C60155--add--end--
...
#FUN-B30166--add--end
   ...
   IF p_sw = '-' THEN
      DELETE FROM nme_file WHERE nme12 = g_ooa.ooa01
                             AND nme21 = b_oob.oob02  #MOD-C10172
      IF SQLCA.sqlcode OR SQLCA.SQLERRD[3] = 0 THEN
...
FUNCTION t410_set_no_entry_b_2()
    ...
    END IF
   #IF (g_oob_d[l_ac2].oob03_d='1' AND g_oob_d[l_ac2].oob04_1 MATCHES "[1239]") OR   #MOD-C70125 mark
    IF (g_oob_d[l_ac2].oob03_d='1' AND g_oob_d[l_ac2].oob04_1 MATCHES "[123]") OR    #MOD-C70125 add
       (g_oob_d[l_ac2].oob03_d='2' AND g_oob_d[l_ac2].oob04_1 MATCHES "[1]") THEN
...
FUNCTION t410_set_entry_b1_2()
    ...
    IF g_oob_d[l_ac2].oob03_d='1' THEN
       ...
       IF g_oob_d[l_ac2].oob04_1='4' THEN
          ...
          END IF
       END IF
      #IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[349]' AND                         #MOD-C70125 mark
       IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[34]' AND                          #MOD-C70125 add
    ...
    IF g_oob_d[l_ac2].oob03_d='2' THEN
      ...
      #END IF
      #-----------------------MOD-C70125-------------mark
      #IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[19]' THEN    #MOD-C70125 mark
       IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[1]' THEN     #MOD-C70125 add
          CALL cl_set_comp_entry("oob15_1",TRUE)
       END IF
    END IF
...
DEFINE l_nmg  RECORD LIKE nmg_file.*  
  ...
  IF g_oob_d[l_ac2].oob03_d='1' THEN
    ...
    #IF g_oob_d[l_ac2].oob04_1='9' THEN CALL t410_oob06_1_19('1') END IF  #MOD-C70125 mark
  END IF
  IF g_oob_d[l_ac2].oob03_d='2' THEN
     IF g_oob_d[l_ac2].oob04_1='1' THEN CALL t410_oob06_1_13('1') END IF
    #IF g_oob_d[l_ac2].oob04_1='9' THEN CALL t410_oob06_1_19('2') END IF  #MOD-C70125 mark
     IF g_oob_d[l_ac2].oob04_1='Z' THEN CALL t410_oob06_1_2Z('2') END IF  #FUN-C90092
  END IF
...
#FUN-C90092--add--str
FUNCTION t410_oob06_1_2Z(p_code)
DEFINE l_nmh     RECORD LIKE nmh_file.*
DEFINE l_nmydmy3 LIKE nmy_file.nmydmy3
DEFINE l_nmh02   LIKE nmh_file.nmh02
DEFINE p_code    LIKE type_file.chr1
DEFINE tot       LIKE type_file.num5

      LET l_sql = "SELECT nmh_file.*,nmydmy3 FROM nmh_file,nmy_file ",
                  " WHERE nmh01= '",g_oob_d[l_ac2].oob06_1,"'",
                  "   AND SUBSTR(nmh01,1,",g_doc_len,")=nmyslip"
      PREPARE nmh_p2_1 FROM l_sql
      DECLARE nmh_c2_1 CURSOR FOR nmh_p2_1
      OPEN nmh_c2_1

      FETCH nmh_c2_1 INTO l_nmh.*,l_nmydmy3
      IF STATUS THEN
         CALL cl_err('sel nmh',STATUS,1)
         LET g_errno=STATUS
         RETURN
      END IF
      ...
      IF l_nmh.nmh38 = 'X' THEN
        ...
        RETURN
      END IF
      IF l_nmh.nmh22!=g_ooa.ooa03 THEN
         LET g_errno='axr-145'
         RETURN
      END IF

      LET tot1 = 0
      SELECT SUM(oob09) INTO tot1
        FROM oob_file,ooa_file
       WHERE oob04 = 'Z'
         AND oob06 = g_oob_d[l_ac2].oob06_1
         AND oob01 = ooa01 AND ooaconf != 'X'   #MOD-9C0147 add
      IF cl_null(tot1) THEN LET tot1 = 0 END IF
      SELECT nmh02 INTO l_nmh02
        FROM nmh_file
       WHERE nmh01 = g_oob_d[l_ac2].oob06_1
         AND nmh24='5'
         AND nmh38='Y'
      IF cl_null(l_nmh02) THEN LET l_nmh02 = 0 END IF

      LET g_oob_d[l_ac2].oob07_1=l_nmh.nmh03
      LET g_oob_d[l_ac2].oob08_1=l_nmh.nmh32/l_nmh.nmh02
      LET g_oob_d[l_ac2].oob09_1=l_nmh02 - tot1
      LET g_oob_d[l_ac2].oob10_1=g_oob_d[l_ac2].oob09_1*g_oob_d[l_ac2].oob08_1
      LET g_oob_d[l_ac2].oob23_1= l_nmh.nmh05
      LET g_oob_d[l_ac2].oob13_1=l_nmh.nmh15
      LET g_oob_d[l_ac2].oob17_1=l_nmh.nmh06
...
FUNCTION t410_oob09_1_13_2(l_sw,l_cmd)  # 檢查 :3:取待抵 1:取應收
   ...
   ELSE
      ...
      IF g_ooz.ooz07 = 'Y' THEN   #有做月底重評時, 需判斷不可超過未沖金額
           ...
           #原幣沖完但本幣未沖完
            IF (tot1+g_oob_d[l_ac2].oob09_1) = (l_oma.oma54t-l_oma.oma55)   AND
               (tot8+g_oob_d[l_ac2].oob10_1 - g_net)!= (l_omc.omc09  - l_omc.omc11) THEN    #yinhy130801
               CALL cl_err('','axr-193',1)
            END IF
      END IF
   END IF
...
#   LET comb_value = '1,2,3,4,5,6,7,8,9,A,E,F,Q'                #No.MOD-B30227 Mark                                                                       
   #LET comb_value = '3,4,5,6,7,8,9,A,E,F,Q'                    #No.MOD-B30227 add #MOD-C70125 mark
   #LET comb_value = '3,4,5,6,7,8,A,E,F,Q'                      #MOD-C70125 add    #FUN-CB0044 mark
    LET comb_value = '3,4,5,6,7,8,A,F'                          #FUN-CB0044 del-E,Q 
...
FUNCTION t410_set_comb_oob04_1()
  ...
  DEFINE comb_item  LIKE type_file.chr1000    
    ...
    LET comb_value = '1,2,3,4,7,A,C,D,E,Q,Z'  #FUN-C90092 add--Z  #FUN-CB0044 del--B,F                                                                                        
...
DEFINE l_apa00         LIKE apa_file.apa00   #MOD-940420
DEFINE l_ac2_t         LIKE type_file.num5   #FUN-A90003 Add
DEFINE l_oob06_o       LIKE oob_file.oob06   #FUN-C90092
DEFINE l_sum_oob09     LIKE oob_file.oob09   #FUN-C90092
DEFINE l_sum_oob10     LIKE oob_file.oob10   #FUN-C90092
DEFINE l_nmh02         LIKE nmh_file.nmh17   #FUN-C90092
      ...
      INPUT ARRAY g_oob_d WITHOUT DEFAULTS FROM s_oob_d.*
        ...
        AFTER INSERT
           ...
           IF g_ooz.ooz62 = 'Y' THEN  
              ...
              END IF
           END IF
          #IF g_oob_d[l_ac2].oob03_d='2' AND g_oob_d[l_ac2].oob04_1 MATCHES "[19]" THEN   #MOD-C70125 mark
           IF g_oob_d[l_ac2].oob03_d='2' AND g_oob_d[l_ac2].oob04_1 MATCHES "[1]" THEN    #MOD-C70125 add
               IF NOT cl_null(g_oob_d[l_ac2].oob15_1) THEN 
                  CALL t410_oob06_1_chk_2(g_oob_d[l_ac2].oob06_1,g_oob_d[l_ac2].oob15_1,l_ac2)
                  IF NOT cl_null(g_errno) THEN 
                     CALL cl_err(g_oob_d[l_ac2].oob06_1,g_errno,0)
           ...
           IF NOT cl_null(g_oob_d[l_ac2].oob03_d) THEN
              ...
              CALL t410_need_2()                                                                                             
                 IF NOT cl_null(g_oob_d[l_ac2].oob04_1) THEN                                                                             
                    ...
                    IF l_cnt = 0 THEN                                                                                               
                       IF g_oob_d[l_ac2].oob03_d='1' THEN
                          #IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[1-9,A,F]' THEN    #MOD-C70125 mark
                           IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[1-8,A,F]' THEN    #MOD-C70125 add
                             CALL cl_err('','axr-917',0)                                                                            
                             NEXT FIELD oob04_1                                                                                       
                          END IF                                                                                                    
                       ELSE                                                                                                         
                         #IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[1,2,4,7,9,A,C,D,E,Q]' THEN    #MOD-C70125 mark
                          IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[1,2,4,7,A,C,D,E,Q]' THEN      #MOD-C70125 add
                             CALL cl_err('','axr-917',0)                                                                            
                             NEXT FIELD oob04_1                                                                                       
                          END IF                                                                                                    
                       END IF                                                                                                       
        ...
        AFTER FIELD oob04_1
           IF NOT cl_null(g_oob_d[l_ac2].oob04_1) THEN
              ...
              IF l_cnt = 0 THEN
                 IF g_oob_d[l_ac2].oob03_d='1' THEN
                   #IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[1-9,A,E,F,Q]' THEN     #MOD-C70125 mark
                   #IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[1-8,A,E,F,Q,Z]' THEN     #MOD-C70125 add #FUN-C90092 add--Z  #FUN-CB0044
                    IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[1-8,A,E,Q,Z]' THEN     #FUN-CB0044 del-F
                       CALL cl_err('','axr-917',0)
                       NEXT FIELD oob04_1
                    END IF
                 ELSE
                  #IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[1,2,4,7,9,A,B,C,D,E,Q]' THEN   #MOD-C70125 mark
                  #IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[1,2,4,7,A,B,C,D,E,Q,Z]' THEN     #MOD-C70125 add #FUN-C90092 add--Z  #FUN-CB0044
                   IF g_oob_d[l_ac2].oob04_1 NOT MATCHES '[1,2,4,7,A,C,D,E,Q,Z]' THEN     #FUN-CB0044 del-B
                       CALL cl_err('','axr-917',0)
                       NEXT FIELD oob04_1
                    END IF
                 END IF
            ...
            #-------------------MOD-C70125----------------mark
              ...
              #MOD-B30240--add--end
              #FUN-C90092--add--str
              IF g_oob_d[l_ac2].oob04_1 ='Z' THEN
                 IF cl_null(g_oob_d[l_ac2].oob06_1) THEN
                    NEXT FIELD oob06_1
                 END IF
              END IF
              #FUN-C90092--add--end
        ...
        AFTER FIELD oob06_1
           IF NOT cl_null(g_oob_d[l_ac2].oob06_1) THEN
            ...
            #IF g_oob_d[l_ac2].oob03_d='1' AND g_oob_d[l_ac2].oob04_1 MATCHES "[39]" THEN       #MOD-C70125 mark
             IF g_oob_d[l_ac2].oob03_d='1' AND g_oob_d[l_ac2].oob04_1 MATCHES "[3]" THEN        #MOD-C70125 add
                IF p_cmd= 'a' OR g_oob_d_t.oob06_1 != g_oob_d[l_ac2].oob06_1  THEN 
                   IF g_oob_d[l_ac2].oob04_1 = '3' THEN 
                      SELECT oma00 INTO l_oma00 FROM oma_file
                       WHERE oma01 = g_oob_d[l_ac2].oob06_1
            ...
            #IF g_oob_d[l_ac2].oob03_d='2' AND g_oob_d[l_ac2].oob04_1 MATCHES "[19]" THEN   #MOD-C70125 mark
             IF g_oob_d[l_ac2].oob03_d='2' AND g_oob_d[l_ac2].oob04_1 MATCHES "[1]" THEN    #MOD-C70125 add
              ...
              END IF
              #FUN-C90092--add--str
              IF g_oob_d[l_ac2].oob03_d='2' AND g_oob_d[l_ac2].oob04_1='Z' THEN
                IF ( g_oob_d[l_ac2].oob06_1!=g_oob_d_t.oob06_1 OR g_oob_d_t.oob06_1 IS NULL) THEN
                   IF cl_null(l_oob06_o) THEN
                      LET l_oob06_o = g_oob_d_t.oob06_1
                   END IF
                   CALL t410_oob06_1_2()
                   IF NOT cl_null(g_errno) THEN
                      LET g_oob_d[l_ac2].oob06_1 = g_oob_d_t.oob06_1
                      CALL cl_err('',g_errno,1)
                      NEXT FIELD oob06_1
                   END IF
                   IF g_oob_d[l_ac2].oob06_1 != l_oob06_o OR l_oob06_o IS NULL THEN
                      LET l_oob06_o = g_oob_d[l_ac2].oob06_1
                   END IF
                   IF cl_null(g_oob_d[l_ac2].oob11_1) THEN
                      LET g_oob_d[l_ac2].oob11_1 =''
                      LET g_oob_d[l_ac2].oob111_1=''
                   END IF
                   DISPLAY BY NAME g_oob_d[l_ac2].oob11_1
                   DISPLAY BY NAME g_oob_d[l_ac2].oob111_1
                   DISPLAY BY NAME g_oob_d[l_ac2].oob07_1
                   ...
                   DISPLAY BY NAME g_oob_d[l_ac2].oob09_1
                   DISPLAY BY NAME g_oob_d[l_ac2].oob10_1
                   DISPLAY BY NAME g_oob_d[l_ac2].oob23_1
                   DISPLAY BY NAME g_oob_d[l_ac2].oob15_1
                   DISPLAY BY NAME g_oob_d[l_ac2].oob17_1
                END IF
             END IF
        ...
        AFTER FIELD oob19_1
           IF cl_null(g_oob_d[l_ac2].oob19_1) AND g_oob_d[l_ac2].oob04_1<>'Z' AND  #FUN-C90092 add--oob04_1<>z
              NOT (g_oob_d[l_ac2].oob03_d='1' AND g_oob_d[l_ac2].oob04_1='4') THEN   
              CALL cl_err("","axr-411",1)
              NEXT FIELD oob06_1 
           END IF 
           IF NOT cl_null(g_oob_d[l_ac2].oob19_1) THEN
             #若為待扺，或為貸方
              ...
              END IF
              #FUN-C90092--add--str
              IF g_oob_d[l_ac2].oob04_1='Z' AND g_oob_d[l_ac2].oob03_d = '2' THEN
                 LET l_sum_oob09=0
                 LET l_sum_oob10=0
                 SELECT SUM(oob09),SUM(oob10)  INTO l_sum_oob09,l_sum_oob10 FROM ooa_file,oob_file
                  WHERE oob04 = 'Z'
                    AND oob06 = g_oob_d[l_ac2].oob06_1
                    AND oob01 = ooa01
                    AND ooaconf !='X'
                  IF cl_null(l_sum_oob09) THEN LET l_sum_oob09=0 END IF
                  IF cl_null(l_sum_oob10) THEN LET l_sum_oob10=0 END IF
                  IF cl_null(g_oob_d_t.oob09_1) THEN LET g_oob_d_t.oob09_1=0 END IF
                  LET l_sum_oob09 = l_sum_oob09-g_oob_d_t.oob09_1
                  ...
                  IF l_sum_oob09+g_oob_d[l_ac2].oob09_1>l_nmh02 THEN
                     CALL cl_err(g_oob_d[l_ac2].oob09_1,'aap-670',1)
                     NEXT FIELD oob09_1
                  END IF
              END IF
              #FUN-C90092---add--end
   ...
   WHILE TRUE
      ...
      IF NOT cl_null(g_action_choice) AND l_ac1>0 THEN #將清單的資料回傳到主畫面
         SELECT ooa_file.*
           INTO g_ooa.*
           FROM ooa_file
          WHERE ooa01 = g_ooa_1[l_ac1].ooa01
      END IF
                   ...
                   IF cl_confirm('aws-081') THEN  #詢問是否繼續下一筆資料的簽核
                     IF aws_efapp_getnextforminfo() THEN #取得下一筆簽核單號
                        ...
                        IF NOT cl_null(g_argv1) THEN     #自動 query 帶出資料
                          CALL t410_q()
                         #設定簽核功能及哪些 action 在簽核狀態時是不可被執行的
                          CALL aws_efapp_flowaction("insert, modify, delete, reproduce, detail, query,locale, void, undo_void,confirm, undo_confirm, easyflow_approval,gen_entry,entry_sheet,carry_voucher,undo_carry_voucher") #FUN-D20035 add undo_void
                               RETURNING g_laststage
                        ELSE
                            EXIT WHILE
                        END IF
                  ...
                  IF cl_confirm('aws-081') THEN     #詢問是否繼續下一筆資料的簽>
                     IF aws_efapp_getnextforminfo() THEN #取得下一筆簽核單號
                       ...
                       IF NOT cl_null(g_argv1) THEN      #自動 query 帶出資料
                          CALL t410_q()
                        #設定簽核功能及哪些 action 在簽核狀態時是不可被執行的
                          CALL aws_efapp_flowaction("insert, modify, delete, reproduce, detail, query,locale, void, confirm, undo_confirm, easyflow_approval,gen_entry,entry_sheet,carry_voucher,undo_carry_voucher")
                              RETURNING g_laststage
                       ELSE
                             EXIT WHILE
                       END IF
...
FUNCTION t410_bp_1(p_ud)
   ...
   LET g_action_choice = " "
   CALL cl_set_act_visible("accept,cancel", FALSE)
   DISPLAY ARRAY g_ooa_1 TO s_ooa_1.* ATTRIBUTE(COUNT=g_rec_b1,UNBUFFERED)
	 ...
	 CONTINUE DISPLAY
      ...
      ON ACTION locale
         ...
         IF g_ooa.ooa34 = '1' THEN LET g_chr2='Y' ELSE LET g_chr2='N' END IF
         CALL cl_set_field_pic(g_ooa.ooaconf,g_chr2,"","",g_chr,"")
         CALL t410_set_comb_oob04()
         CALL t410_set_comb_oob04_1()
         EXIT DISPLAY
...
FUNCTION t410_b_fill_1()
  DEFINE l_i             LIKE type_file.num10

    CALL g_ooa_1.clear()
    LET l_i = 1
    FOREACH axrt410_list_cur INTO g_ooa_1[l_i].*
       IF SQLCA.sqlcode THEN
          CALL cl_err('foreach item_cur',SQLCA.sqlcode,1)
          CONTINUE FOREACH
       END IF
       SELECT gen02 INTO g_ooa_1[l_i].gen02 FROM gen_file
        WHERE gen01 = g_ooa_1[l_i].ooa14
       SELECT gem02 INTO g_ooa_1[l_i].gem02 FROM gem_file
        WHERE gem01 = g_ooa_1[l_i].ooa15
       LET l_i = l_i + 1
    END FOREACH
    LET g_rec_b1 = l_i - 1
    DISPLAY ARRAY g_ooa_1 TO s_ooa_1.* ATTRIBUTE(COUNT=g_rec_b1,UNBUFFERED)
