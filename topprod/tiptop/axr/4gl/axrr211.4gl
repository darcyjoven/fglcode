...
#
# Pattern name...: axrr211.4gl
# Descriptions...: 修改信用狀申請書(axrr211)
...
# Modify.........: No.MOD-C30688 12/03/14 By wangrr axrr211_prepare1增加條件ole02>0
...
#No.FUN-720053--begin
   LET g_sql = "ole01.ole_file.ole01,",
   ...
   LET l_table = cl_prt_temptable('axrr211',g_sql) CLIPPED  
...
#No.FUN-720053--end
   ...
   IF cl_null(tm.wc) THEN
      CALL axrr211_tm(0,0)             # Input print condition
   ELSE
      CALL axrr211()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
END MAIN
 
FUNCTION axrr211_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01      #No.FUN-580031
   ...
   OPEN WINDOW axrr211_w AT p_row,p_col
        WITH FORM "axr/42f/axrr211"
################################################################################
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axrr211_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axrr211',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axrr211_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#No.FUN-720053--begin
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'axrr211'
#No.FUN-720053--end
     ...
     LET l_sql = "SELECT ole01, ole02, ole03, ole04, ole05, ole07,ole09,",
                 ...
                 "   AND ", tm.wc CLIPPED,
                 "   AND ole02>0"    #MOD-C30688 add--
 
     PREPARE axrr211_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
        EXIT PROGRAM
     END IF
     DECLARE axrr211_curs1 CURSOR FOR axrr211_prepare1
 
#No.FUN-720053--begin
#     CALL cl_outnam('axrr211') RETURNING l_name
#     START REPORT axrr211_rep TO l_name
#
#     LET g_pageno = 0
#No.FUN-720053--end
     FOREACH axrr211_curs1 INTO sr.*,l_ole31     #FUN-A60056 add ole31
       IF SQLCA.sqlcode != 0 THEN
...
#      OUTPUT TO REPORT axrr211_rep(sr.*)
#No.FUN-720053--end
     END FOREACH
 
#No.FUN-720053--begin
#    FINISH REPORT axrr211_rep
     CALL cl_wcchp(tm.wc,'ole01,ole02,ola04') 
          RETURNING tm.wc 
   # LET g_sql = "SELECT * FROM ",l_table CLIPPED     #TQC-730113
     LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     LET g_str = tm.wc
   # CALL cl_prt_cs3('axrr211',g_sql,g_str)           #TQC-730113
     LET g_cr_table = l_table                 #主報表的temp table名稱   #TQC-C100399
     LET g_cr_apr_key_f = "ole01"             #報表主鍵欄位名稱  #TQC-C10039
     CALL cl_prt_cs3('axrr211','axrr211',g_sql,g_str)            
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
#No.FUN-720053--end
END FUNCTION
 
#No.FUN-720053--begin
#REPORT axrr211_rep(sr)
#   DEFINE 
