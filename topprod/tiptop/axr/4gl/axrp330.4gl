...
DEFINE g_omf10    LIKE omf_file.omf10  #add by huanglf160923
#FUN-C60036 add--end
...
FUNCTION p330_process()
  ...
  IF g_oaz92 = 'Y' AND g_aza.aza26 = '2'  THEN
    ...
    #LET g_sql= "SELECT DISTINCT omf01,omf02,oga94,ogb31,'','',oga01,ogaplant,oga011,oga03,oga18, ",     #minpp mark
     IF ls_n > 0 THEN 
        LET g_sql = " SELECT DISTINCT omf00,omf01,omf02,oga94,ogb31,'','',oga01,omf09,oga011,oga03,oga18, ",#FUN-CA0084 ogaplant--->omf09
                    "       oga05,oga21,oga15,oga14,oga23,oga02,oga11,oga27,oga08,'2' type ",
    ...
    #IF g_aza.aza26 = '2' AND g_ooz.ooz65 = 'Y' THEN    #CHI-C10018 mark
     IF g_ooz.ooz65 = 'Y' THEN                          #CHI-C10018 add
        LET g_sql = g_sql CLIPPED," UNION ",
          #   "SELECT DISTINCT omf01,omf02,oha94,ohb33,'','',oha01,ohaplant,''    ,oha03,oha03, ",   #minpp--mark
              ...
              "       ''   ,oha21,oha15,oha14,oha23,oha02,CAST('' AS DATE),'',oha08,'3' type ",    #MOD-DA0161 add
     ...
     END IF
     LET g_sql = g_sql CLIPPED,
              " ORDER BY omf09,omf00,omf01,omf02,oga03,oga27,oga21,oga15,oga14,oga23,type,oga05 "  #minpp--add--omf00
  ELSE   
  #FUN-C60036--add-end
   ...
   #IF g_aza.aza26 = '2' AND g_ooz.ooz65 = 'Y' THEN    #CHI-C10018 mark 
    ...
    LET g_sql = g_sql CLIPPED,
           ...
           #  " ORDER BY ogaplant,oga03,oga27,oga05,oga21,oga15,oga14,oga23,type" #MOD-AA0156 #MOD-C30721 mark
              " ORDER BY ogaplant,oga03,oga27,oga21,oga15,oga14,oga23,type,oga05 " #MOD-C30721 add
   END IF #FUN-C60036
...
#No.MOD-DA0161 --end
   ...
   FOREACH p330_ogb_cs INTO l_ogb03                                             
     ...
     IF ls_n  > 0 AND  cl_null(sr.oga01) AND g_oaz.oaz92 = 'Y' THEN
        ...
        SELECT omf11,omf12,omf13,omf916,omf14,' ',omf917,0,0,omf28,omf29,omf29t,' ',' ',' ',omf09,omf10 #add by huanglf160923
  ...
  #生成分錄底稿
   ...
   FOREACH p330_cs01 INTO g_oma.*
    ...
    #-CHI-CB0016-add-
     ...
     #IF g_oma.oma54 <=0 OR g_oma.oma56 <= 0 OR g_oma.oma54t<0 OR g_oma.oma56<0 THEN  #yinhy131216 #mark by dengsy170321
     IF g_oma.oma54 <0 OR g_oma.oma56 < 0 OR g_oma.oma54t<0 OR g_oma.oma56<0 THEN   #add by dengsy170321
        ...
        FOREACH p330_err INTO l_omb31
           ...
           END IF
        END FOREACH
        UPDATE oma_file SET oma00='22' WHERE oma01=g_oma.oma01
        #CALL s_errmsg('oga01|oha01',l_msg,'','axr-899',1)
        #LET g_success = 'N'  #mark by dengsy170321
     END IF
     #FUN-D90048---add---end--
   ...
   # IF l_sum > 0 THEN  #表示產生了oob_file  #MOD-AA0156
    ...
    # IF g_oaz92 != 'Y' AND g_aza.aza26 != '2' THEN #FUN-C60036 add  MOD-D90014 mark
    #add by huanglf160923
    IF g_omf10 != '9' THEN  
        CALL s_up_omb(g_oma.oma01)  #MOD-AB0006
    END IF 
    # END IF #FUN-C60036 add MOD-D90014 mark
   END FOREACH
...
FUNCTION p330_g_oma()
   ...
   #str------ add by dengsy170331
      #IF cl_null(g_oma.oma08) THEN 
         IF NOT cl_null(g_oma.oma03) THEN 
            SELECT (CASE WHEN occ43='X01' THEN '1' ELSE '2' END),occ67
            INTO g_oma.oma08,g_oma.oma13
            FROM occ_file WHERE occ01=g_oma.oma03
         END IF 
      #END IF  #mark by dengsy170502 
      #end------ add by dengsy170331
   #No.MOD-C40096  --Begin
