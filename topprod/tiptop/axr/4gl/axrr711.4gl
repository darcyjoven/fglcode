# Prog. Version..: '5.30.06-13.03.12(00010)'     #
#
# Pattern name...: axrr711.4gl
# Descriptions...: 客戶業務彙總帳列印
# Date & Author..: 03/05/21 by Carrier
# Modify.........: No.FUN-4C0100 05/03/01 By Smapmin 放寬金額欄位
# Modify.........: No.MOD-530866 05/03/30 By Smapmin 將VARCHAR轉為CHAR
# Modify.........: NO.TQC-650054 06/05/12 by yiting 報表錯誤
# Modify.........: No.FUN-660116 06/06/16 By ice cl_err --> cl_err3
# Modify.........: No.TQC-630155 06/06/22 By Smapmin 將報表轉為XML格式
# Modify.........: No.TQC-610059 06/06/05 By Smapmin 修改外部參數接收
# Modify.........: No.FUN-660060 06/06/26 By Rainy 表頭期間置於中間
# Modify.........: No.FUN-670006 06/07/10 By Jackho 帳套權限修改
# Modify.........: No.TQC-670076 06/07/21 By cl ooo10和npp06對應g_ooz.ooz02p
# Modify.........: No.FUN-680123 06/08/29 By hongmei 欄位類型轉換
# Modify.........: No.FUN-690127 06/10/16 By baogui cl_used位置調整及EXIT PROGRAM后加cl_used
# Modify.........: No.CHI-6A0004 06/10/23 By hongmei g_azixx(本幣取位)與t_azixx(原幣取位)變數定義問題修改
 
# Modify.........: No.FUN-6A0095 06/10/27 By Xumin l_time轉g_time
# Modify.........: No.TQC-6B0051 06/12/13 By xufeng 去除多余的制表日期和頁次
# Modify.........: No.FUN-740009 07/04/03 By Judy 會計科目加帳套
# Modify.........: No.TQC-740093 07/04/16 By Lynn 會計科目加帳套
# Modify.........: No.FUN-7B0026 07/11/19 By baofei  報表輸出至Crystal Reports功能
# Modify.........: No.FUN-980030 09/08/31 By Hiko 加上GP5.2的相關設定
# Modify.........: No.FUN-B10053 11/01/20 By yinhy 科目查询自动过滤,調整帳套位置
 
DATABASE ds
 
GLOBALS "../../config/top.global"
 
DEFINE tm        RECORD                                # Print condition RECORD
		 wc      LIKE type_file.chr1000,       #No.FUN-680123 VARCHAR(1000) # Where condition
                 a       LIKE type_file.chr20,         #No.FUN-680123 VARCHAR(20)
                 yy      LIKE type_file.num5,          #No.FUN-680123 SMALLINT
                 m1      LIKE type_file.num5,          #No.FUN-680123 SMALLINT
                 m2      LIKE type_file.num5,          #No.FUN-680123 SMALLINT
                 o       LIKE aaa_file.aaa01,
                 b       LIKE type_file.chr1,          #No.FUN-680123 VARCHAR(01)
                 c       LIKE azi_file.azi01,          #No.FUN-680123 VARCHAR(4) 
                 d       LIKE type_file.chr1,          #No.FUN-680123 VARCHAR(01)
                 e       LIKE type_file.chr1,          #No.FUN-680123 VARCHAR(01)
 		 more    LIKE type_file.chr1           # Prog. Version..: '5.30.06-13.03.12(01) # Input more condition(Y/N)
                 END RECORD,
       g_d       LIKE type_file.chr1,          #No.FUN-680123 VARCHAR(01)
       g_null    LIKE type_file.chr1,          #No.FUN-680123 VARCHAR(01)
       g_print   LIKE type_file.num5,          #No.FUN-680123 SMALLINT
       g_aza17   LIKE aza_file.aza17,
       l_aza17   LIKE aza_file.aza17,
       l_aag02   LIKE aag_file.aag02,
       g_qcyef   LIKE ooo_file.ooo08d,
       g_qcye    LIKE ooo_file.ooo08d,
       g_ooo08_l LIKE ooo_file.ooo08d,
       g_ooo08_r LIKE ooo_file.ooo08d,
       g_ooo09_l LIKE ooo_file.ooo08d,
       g_ooo09_r LIKE ooo_file.ooo08d,
       t_qcyef   LIKE ooo_file.ooo08d,
       t_qcye    LIKE ooo_file.ooo08d,
       t_ooo08_l LIKE ooo_file.ooo08d,
       t_ooo08_r LIKE ooo_file.ooo08d,
       t_ooo09_l LIKE ooo_file.ooo08d,
       t_ooo09_r LIKE ooo_file.ooo08d
 
DEFINE   g_i         LIKE type_file.num5     #count/index for any purpose    #No.FUN-680123 SMALLINT
#No.FUN-7B0026---Begin                                                                                                              
DEFINE l_table        STRING,                                                                                                       
       g_str          STRING,
       g_sql          STRING                                                                                                       
 
#No.FUN-7B0026---End 
MAIN
   OPTIONS
       INPUT NO WRAP
   DEFER INTERRUPT                        # Supress DEL key function
 
   IF (NOT cl_user()) THEN
      EXIT PROGRAM
   END IF
 
   WHENEVER ERROR CALL cl_err_msg_log
 
   IF (NOT cl_setup("AXR")) THEN
      EXIT PROGRAM
   END IF
   CALL cl_used(g_prog,g_time,1) RETURNING g_time #No.FUN-690127
 
#No.FUN-7B0026---Begin                                                                                                              
   LET g_sql = "ooo01.ooo_file.ooo01,",                                                                                             
               "ooo02.ooo_file.ooo02,",                                                                                             
               "ooo06.ooo_file.ooo06,",                                                                                             
               "ooo07.ooo_file.ooo07,",                                                                                           
               "ooo08c.ooo_file.ooo08c,",                                                                                             
               "ooo08d.ooo_file.ooo08d,",                                                                                             
               "ooo09c.ooo_file.ooo09c,",                                                                                             
               "ooo09d.ooo_file.ooo09d,",                                                                                             
               "qcyef.ooo_file.ooo08d,",                                                                                             
               "qcye.ooo_file.ooo08d,",                                                                                    
               "prt.type_file.chr1"                                                                                            
   LET l_table = cl_prt_temptable('axrr711',g_sql) CLIPPED                                                                          
   IF l_table = -1 THEN EXIT PROGRAM END IF                                                                                         
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,                                                                  
               " VALUES(?,?,?,?,?,?,?,?,?,?,?) "                                                                  
                                                                                                   
   PREPARE insert_prep FROM g_sql                                                                                                   
   IF STATUS THEN                                                                                                                   
      CALL cl_err('insert_prep:',status,1) EXIT PROGRAM                                                                             
   END IF 
#No.FUN-7B0026---End                  
   #-----TQC-610059---------
   LET g_pdate=ARG_VAL(1)
   LET g_towhom=ARG_VAL(2)
   LET g_rlang=ARG_VAL(3)
   LET g_bgjob=ARG_VAL(4)
   LET g_prtway=ARG_VAL(5)
   LET g_copies=ARG_VAL(6)
   LET tm.wc = ARG_VAL(7)
   LET tm.a = ARG_VAL(8)
   LET tm.yy = ARG_VAL(9)
   LET tm.m1 = ARG_VAL(10)
   LET tm.m2 = ARG_VAL(11)
   LET tm.o = ARG_VAL(12)
   LET tm.b = ARG_VAL(13)
   LET tm.c = ARG_VAL(14)
   LET tm.d = ARG_VAL(15)
   LET tm.e = ARG_VAL(16)
   #-----END TQC-610059-----
   #No.FUN-570264 --start--
   LET g_rep_user = ARG_VAL(17)
   LET g_rep_clas = ARG_VAL(18)
   LET g_template = ARG_VAL(19)
   LET g_rpt_name = ARG_VAL(20)  #No.FUN-7C0078
   #No.FUN-570264 ---end---
   IF cl_null(tm.wc) THEN
       CALL axrr711_tm(0,0)             # Input print condition
   ELSE 
       CALL axrr711()
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
END MAIN

FUNCTION axrr711_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01           #No.FUN-580031
   DEFINE p_row,p_col    LIKE type_file.num5,          #No.FUN-680123 SMALLINT
          l_n            LIKE type_file.num5,          #No.FUN-680123 SMALLINT
          l_flag         LIKE type_file.num5,          #No.FUN-680123 SMALLINT
          l_cmd          LIKE type_file.chr1000        #No.FUN-680123 VARCHAR(1000)
   DEFINE li_chk_bookno  LIKE type_file.num5           #No.FUN-680123 SMALLINT  #No.FUN-670006 
   LET p_row = 4 LET p_col =25
 
   OPEN WINDOW axrr711_w AT p_row,p_col WITH FORM "axr/42f/axrr711"
       ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
 
    CALL cl_ui_init()
 
 
   CALL cl_opmsg('p')
   INITIALIZE tm.* TO NULL            # Default condition
   SELECT ool11 INTO tm.a FROM ooz_file,ool_file
    WHERE ooz00 = '0' AND ool01 = ooz08
   LET tm.yy = YEAR(g_today)
   LET tm.m1 = MONTH(g_today)
   LET tm.m2 = MONTH(g_today)
   LET tm.o  = g_ooz.ooz02b
   LET tm.b = 'N'
   LET tm.c = g_aza.aza17
   LET tm.d = 'N'
   LET tm.e = 'N'
   LET tm.more = 'N'
   LET g_pdate = g_today
   LET g_rlang = g_lang
   LET g_bgjob = 'N'
   LET g_copies= '1'
WHILE TRUE
   CONSTRUCT BY NAME tm.wc ON ooo01
         #No.FUN-580031 --start--
         BEFORE CONSTRUCT
             CALL cl_qbe_init()
         #No.FUN-580031 ---end---
 
       ON ACTION locale
           #CALL cl_dynamic_locale()
          CALL cl_show_fld_cont()                   #No.FUN-550037 hmf
         LET g_action_choice = "locale"
         EXIT CONSTRUCT
 
     ON IDLE g_idle_seconds
        CALL cl_on_idle()
        CONTINUE CONSTRUCT
 
      ON ACTION about         #MOD-4C0121
         CALL cl_about()      #MOD-4C0121
 
      ON ACTION help          #MOD-4C0121
         CALL cl_show_help()  #MOD-4C0121
 
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
 
 
           ON ACTION exit
           LET INT_FLAG = 1
           EXIT CONSTRUCT
         #No.FUN-580031 --start--
         ON ACTION qbe_select
            CALL cl_qbe_select()
         #No.FUN-580031 ---end---
 
  END CONSTRUCT
  LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
       IF g_action_choice = "locale" THEN
          LET g_action_choice = ""
          CALL cl_dynamic_locale()
          CONTINUE WHILE
       END IF
 
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axrr711_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
      EXIT PROGRAM
         
   END IF
   IF tm.wc = ' 1=1' THEN
      CALL cl_err('','9046',0) CONTINUE WHILE
   END IF
   #INPUT BY NAME tm.a,tm.yy,tm.m1,tm.m2,tm.o,tm.b,tm.c,tm.d,tm.e,tm.more   #FUN-B10053 mark
   INPUT BY NAME tm.o,tm.a,tm.yy,tm.m1,tm.m2,tm.b,tm.c,tm.d,tm.e,tm.more    #FUN-B10053 mod
   WITHOUT DEFAULTS
         #No.FUN-580031 --start--
         BEFORE INPUT
             CALL cl_qbe_display_condition(lc_qbe_sn)
         #No.FUN-580031 ---end---
 
     AFTER FIELD a
        LET l_flag = 0
        #No.FUN-B10053  --Begin
        #IF cl_null(tm.a) THEN
        #   NEXT FIELD a
        IF NOT cl_null(tm.a) THEN
           CALL axrr711_a()
           IF NOT cl_null(g_errno) THEN
              CALL cl_err(tm.a,g_errno,0)
              CALL cl_init_qry_var()
              LET g_qryparam.form ="q_aag"
              LET g_qryparam.construct = 'N'
              LET g_qryparam.default1 = tm.a
              LET g_qryparam.arg1 = tm.o
              LET g_qryparam.where = " aag07 IN ('2','3')  AND aagacti='Y' AND aag01 LIKE '",tm.a CLIPPED,"%'"
              CALL cl_create_qry() RETURNING tm.a
              DISPLAY BY NAME tm.a
              NEXT FIELD a             
            END IF  
        #No.FUN-B10053  --End
#No.TQC-740093 --begin
#       ELSE
#          SELECT aag02 INTO l_aag02 FROM aag_file
#           WHERE aag01 = tm.a AND aag07 IN ('2','3')
#             AND aag00 = tm.o    #FUN-740009
#          IF STATUS THEN
#             CALL cl_err(tm.a,'mfg1004',0)   #No.FUN-660116
#             CALL cl_err3("sel","aag_file",tm.a,"","mfg1004","","",0)   #No.FUN-660116
#             NEXT FIELD a 
#          END IF
#No.TQC-740093 --end
        END IF
 
     AFTER FIELD yy
        IF cl_null(tm.yy) THEN NEXT FIELD yy END IF
 
     AFTER FIELD m1
        IF cl_null(tm.m1) OR tm.m1 > 13 OR tm.m1 < 1 THEN
           NEXT FIELD m1
        END IF
 
     AFTER FIELD m2
        IF cl_null(tm.m2) OR tm.m2 > 13 OR tm.m2 < 1 OR tm.m2 < tm.m1 THEN
           NEXT FIELD m2
        END IF
 
     AFTER FIELD o
        IF cl_null(tm.o) THEN NEXT FIELD o END IF
        #No.FUN-670006--begin
             CALL s_check_bookno(tm.o,g_user,g_plant) 
                  RETURNING li_chk_bookno
             IF (NOT li_chk_bookno) THEN
                  NEXT FIELD o
             END IF 
             #No.FUN-670006--end
        SELECT * FROM aaa_file WHERE aaa01 = tm.o
        IF SQLCA.sqlcode THEN
#          CALL cl_err(tm.o,'aap-229',0)   #No.FUN-660116
           CALL cl_err3("sel","aaa_file",tm.o,"","aap-229","","",0)   #No.FUN-660116
           NEXT FIELD o 
        END IF
#No.FUN-B10053 --Begin
#No.TQC-740093 ---begin
#           SELECT aag02 INTO l_aag02 FROM aag_file
#            WHERE aag01 = tm.a AND aag07 IN ('2','3')
#              AND aag00 = tm.o  
#           IF STATUS THEN
#              CALL cl_err(tm.a,'mfg1004',0)   #No.FUN-660116
#              CALL cl_err3("sel","aag_file",tm.a,"","mfg1004","","",0)   #No.FUN-660116
#              NEXT FIELD a 
#           END IF
#No.TQC-740093 ---end
        CALL axrr711_a()
        IF NOT cl_null(g_errno) THEN 
           CALL cl_err(tm.a,g_errno,0)
           NEXT FIELD a
        END IF
#No.FUN-B10053 --Begin 
        SELECT aaa03 INTO l_aza17 FROM aaa_file WHERE aaa01 = tm.o
        IF SQLCA.sqlcode THEN LET l_aza17 = g_aza.aza17 END IF   #使用本國幣別
 
      AFTER FIELD b
        LET l_flag = 1
# genero  script marked         IF cl_ku() THEN NEXT FIELD o END IF
        IF cl_null(tm.b) OR tm.b NOT MATCHES '[YN]' THEN
           NEXT FIELD b
        END IF
        IF tm.b = 'N' THEN
           LET tm.c = ''
           DISPLAY BY NAME tm.c
        END IF
 
      BEFORE FIELD c
        IF l_flag = 1 THEN
           IF tm.b = 'N' THEN NEXT FIELD d END IF
        ELSE IF l_flag = 2 THEN
                IF tm.b = 'N' THEN NEXT FIELD b END IF
             END IF
        END IF
 
      AFTER FIELD c
        IF tm.b = 'Y' THEN
           IF cl_null(tm.c) THEN NEXT FIELD c END IF
           SELECT azi01 FROM azi_file WHERE azi01 = tm.c
           IF STATUS THEN 
#             CALL cl_err(tm.c,'mfg3008',0)     #No.FUN-660116
              CALL cl_err3("sel","azi_file",tm.c,"","mfg3008","","",0)   #No.FUN-660116
              NEXT FIELD c END IF
        END IF
 
      AFTER FIELD d
        LET l_flag = 2
# genero  script marked         IF cl_ku() THEN
# genero  script marked            IF tm.b = 'N' THEN NEXT FIELD b ELSE NEXT FIELD c END IF
# genero  script marked         END IF
        IF cl_null(tm.d) OR tm.d NOT MATCHES '[YN]' THEN NEXT FIELD d END IF
 
      AFTER FIELD e
        IF cl_null(tm.e) OR tm.e NOT MATCHES '[YN]' THEN NEXT FIELD e END IF
 
      AFTER FIELD more
         IF tm.more = 'Y'
            THEN CALL cl_repcon(0,0,g_pdate,g_towhom,g_rlang,
                                g_bgjob,g_time,g_prtway,g_copies)
                      RETURNING g_pdate,g_towhom,g_rlang,
                                g_bgjob,g_time,g_prtway,g_copies
         END IF
      AFTER INPUT
         IF tm.b = 'N' THEN LET tm.c = '' DISPLAY BY NAME tm.c END IF
         IF tm.b = 'Y' AND cl_null(tm.c) THEN NEXT FIELD c END IF
      ON ACTION CONTROLP
         CASE
            WHEN INFIELD(a)     #科目代號
#              CALL q_aag(8,5,tm.a,'23','','') RETURNING tm.a
#              CALL FGL_DIALOG_SETBUFFER( tm.a )
               CALL cl_init_qry_var()
               LET g_qryparam.form = 'q_aag'
               LET g_qryparam.default1 = tm.a
               LET g_qryparam.arg1 = tm.o       #FUN-740009
               CALL cl_create_qry() RETURNING tm.a
#               CALL FGL_DIALOG_SETBUFFER( tm.a )
               DISPLAY BY NAME tm.a
               NEXT FIELD a
 
            WHEN INFIELD(c)     #幣別代號
#              CALL q_azi(8,5,tm.c) RETURNING tm.c
#              CALL FGL_DIALOG_SETBUFFER( tm.c )
               CALL cl_init_qry_var()
               LET g_qryparam.form = 'q_azi'
               LET g_qryparam.default1 = tm.c
               CALL cl_create_qry() RETURNING tm.c
#               CALL FGL_DIALOG_SETBUFFER( tm.c )
               DISPLAY BY NAME tm.c
               NEXT FIELD c
 
            OTHERWISE
               EXIT CASE
         END CASE
 
      ON ACTION CONTROLR
         CALL cl_show_req_fields()
 
      ON ACTION CONTROLG
         CALL cl_cmdask()    # Command execution
 
      ON IDLE g_idle_seconds
         CALL cl_on_idle()
         CONTINUE INPUT
 
      ON ACTION about         #MOD-4C0121
         CALL cl_about()      #MOD-4C0121
 
      ON ACTION help          #MOD-4C0121
         CALL cl_show_help()  #MOD-4C0121
 
 
          ON ACTION exit
          LET INT_FLAG = 1
          EXIT INPUT
         #No.FUN-580031 --start--
         ON ACTION qbe_save
            CALL cl_qbe_save()
         #No.FUN-580031 ---end---
 
   END INPUT
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axrr711_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
      EXIT PROGRAM
         
   END IF
   IF g_bgjob = 'Y' THEN
      SELECT zz08 INTO l_cmd FROM zz_file    #get exec cmd (fglgo xxxx)
             WHERE zz01='axrr711'
      IF SQLCA.sqlcode OR l_cmd IS NULL THEN
         CALL cl_err('axrr711','9031',1)
      ELSE
         LET tm.wc=cl_replace_str(tm.wc, "'", "\"")
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         " '",g_pdate  CLIPPED,"'",
                         " '",g_towhom CLIPPED,"'",
                         #" '",g_lang   CLIPPED,"'", #No.FUN-7C0078
                         " '",g_rlang CLIPPED,"'", #No.FUN-7C0078
                         " '",g_bgjob  CLIPPED,"'",
                         " '",g_prtway CLIPPED,"'",
                         " '",g_copies CLIPPED,"'",
                         " '",tm.wc    CLIPPED,"'" ,
                         " '",tm.a     CLIPPED,"'",
                         " '",tm.yy    CLIPPED,"'" ,
                         " '",tm.m1    CLIPPED,"'" ,
                         " '",tm.m2    CLIPPED,"'" ,
                         " '",tm.o     CLIPPED,"'",
                         " '",tm.b     CLIPPED,"'",
                         " '",tm.c     CLIPPED,"'",
                         " '",tm.d     CLIPPED,"'",
                         " '",tm.e     CLIPPED,"'",
                         " '",g_rep_user CLIPPED,"'",           #No.FUN-570264
                         " '",g_rep_clas CLIPPED,"'",           #No.FUN-570264
                         " '",g_template CLIPPED,"'",           #No.FUN-570264
                         " '",g_rpt_name CLIPPED,"'"            #No.FUN-7C0078
         CALL cl_cmdat('axrr711',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axrr711_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
      EXIT PROGRAM
   END IF
   CALL cl_wait()
   CALL axrr711()
   ERROR ""
END WHILE
   CLOSE WINDOW axrr711_w
END FUNCTION
         
FUNCTION axrr711()
   DEFINE l_name    LIKE type_file.chr20,         #No.FUN-680123 VARCHAR(20) # External(Disk) file name
#       l_time          LIKE type_file.chr8        #No.FUN-6A0095
          l_sql     LIKE type_file.chr1000,       #No.FUN-680123 VARCHAR(1000)
          l_sql1    LIKE type_file.chr1000,       #No.FUN-680123 VARCHAR(1000)
          l_za05    LIKE type_file.chr1000,       #No.FUN-680123 VARCHAR(40)
          l_i       LIKE type_file.num5,          #No.FUN-680123 SMALLINT
          l_str     STRING,                       #No.FUN-7B0026
          l_qcyef   LIKE ooo_file.ooo08d,
          l_qcye    LIKE ooo_file.ooo08d,
          m_ooo08d  LIKE ooo_file.ooo08d,
          m_ooo08c  LIKE ooo_file.ooo08c,
          m_ooo09d  LIKE ooo_file.ooo08d,
          m_ooo09c  LIKE ooo_file.ooo08c,
          sr1       RECORD
                    ooo01    LIKE ooo_file.ooo01,
                    ooo02    LIKE ooo_file.ooo02
                    END RECORD,
          sr        RECORD
                    ooo01    LIKE ooo_file.ooo01,
                    ooo02    LIKE ooo_file.ooo02,
                    ooo06    LIKE ooo_file.ooo06,
                    ooo07    LIKE ooo_file.ooo07,
                    ooo08d   LIKE ooo_file.ooo08d,
                    ooo08c   LIKE ooo_file.ooo08c,
                    ooo09d   LIKE ooo_file.ooo09d,
                    ooo09c   LIKE ooo_file.ooo09c,
                    qcyef    LIKE ooo_file.ooo08d,
                    qcye     LIKE ooo_file.ooo08d,
                    prt      LIKE type_file.chr1          #No.FUN-680123 VARCHAR(01)
                    END RECORD
#No.FUN-7B0026---Begin
 
   CALL cl_del_data(l_table)                                                                                                                                                                                                         
   SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang                                                          
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog   
{ 
#     CALL cl_outnam('axrr711') RETURNING l_name   #NO.TQC-650054
    #-----TQC-630155---------
    IF tm.b = 'N' THEN
       LET g_zaa[39].zaa06='Y'
       LET g_zaa[40].zaa06='Y'
       LET g_zaa[41].zaa06='Y'
       LET g_zaa[42].zaa06='Y'
       LET g_zaa[43].zaa06='Y'
       LET g_zaa[44].zaa06='Y'
       LET g_zaa[45].zaa06='Y'
    ELSE
       LET g_zaa[35].zaa06='Y'
       LET g_zaa[36].zaa06='Y'
       LET g_zaa[37].zaa06='Y'
       LET g_zaa[38].zaa06='Y'
    END IF
    CALL cl_prt_pos_len()
    #-----END TQC-630155-----
    FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR
}
    IF tm.b = 'N' THEN                                                                                                              
       LET l_name = 'axrr711'
    ELSE                                                                                                                            
       LET l_name = 'axrr711_1'
    END IF                 
#No.FUN-7B0026---End
#No.TQC-6B0051  --begin
     SELECT aaf03 INTO g_company FROM aaf_file
      WHERE aaf01 = tm.o AND aaf02 = g_rlang
     SELECT zo02 INTO g_company FROM zo_file
      WHERE zo01 = g_rlang
#No.TQC-6B0051  --end
     IF tm.b = 'Y' THEN
        LET g_aza17 = tm.c
     ELSE
        LET g_aza17 = l_aza17
     END IF
   # SELECT azi04 INTO g_azi04 FROM azi_file WHERE azi01=g_aza17      #No.CHI-6A0004
 
     LET l_sql = " SELECT UNIQUE ooo01,ooo02 FROM ooo_file ",
                 "  WHERE ooo03 = '",tm.a,"'",
               # "    AND ooo10 = '",g_plant,"'", #TQC-670076 
                 "    AND ooo10 = '",g_ooz.ooz02p,"'", #TQC-670076  
                 "    AND ooo11 = '",tm.o,"'",
                 "    AND ",tm.wc CLIPPED
     IF tm.b = 'Y' THEN
        LET l_sql = l_sql CLIPPED," AND ooo05 = '",g_aza17,"'"
     END IF
     PREPARE axrr711_pr1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
        EXIT PROGRAM
           
     END IF
     DECLARE axrr711_curs1 CURSOR FOR axrr711_pr1
 
     LET l_sql1="SELECT ooo01,ooo02,ooo06,ooo07,SUM(ooo08d),",
                "       SUM(ooo08c),SUM(ooo09d),SUM(ooo09c),0,0,'' ",
                "  FROM ooo_file ",
                " WHERE ooo03 = '",tm.a,"'",
              # "   AND ooo10 = '",g_plant,"'",#TQC-670076
                "   AND ooo10 = '",g_ooz.ooz02p,"'",  #TQC-670076 
                "   AND ooo11 = '",tm.o,"'",
                "   AND ooo01 = ?  AND ooo02 = ? ",
                "   AND ooo06 = ",tm.yy,
                "   AND ooo07 BETWEEN ",tm.m1," AND ",tm.m2,
                "   AND ",tm.wc CLIPPED
     IF tm.b = 'Y' THEN
        LET l_sql1 = l_sql1 CLIPPED," AND ooo05 = '",g_aza17,"'"
     END IF
     LET l_sql1 = l_sql1 CLIPPED,
                " GROUP BY ooo01,ooo02,ooo06,ooo07 ",
                " ORDER BY ooo01,ooo02,ooo06,ooo07 "
     PREPARE axrr711_prepare1 FROM l_sql1
     IF SQLCA.sqlcode THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,0) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
        EXIT PROGRAM
           
     END IF
     DECLARE axrr711_curs CURSOR FOR axrr711_prepare1
 
#     CALL cl_outnam('axrr711') RETURNING l_name  #NO.TQC-650054 MARK
#No.FUN-7B0026---Begin
#     IF tm.b = 'N' THEN   #本幣
#        START REPORT axrr711_rep TO l_name
#     ELSE                 #原幣
#        START REPORT axrr711_rep1 TO l_name
#     END IF
#No.FUN-7B0026---End
     LET g_pageno  = 0
     LET t_qcyef   = 0
     LET t_qcye    = 0
     LET t_ooo08_l = 0
     LET t_ooo08_r = 0
     LET t_ooo09_l = 0
     LET t_ooo09_r = 0
 
     FOREACH axrr711_curs1 INTO sr1.*
       IF SQLCA.sqlcode THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,0) EXIT FOREACH
       END IF
       LET g_null = 'N'
       IF tm.b = 'Y' THEN
          SELECT SUM(ooo08d-ooo08c),SUM(ooo09d-ooo09c)  #期初余額
            INTO l_qcyef,l_qcye    FROM ooo_file
           WHERE ooo01 = sr1.ooo01 AND ooo02 = sr1.ooo02
             AND ooo03 = tm.a      AND ooo05 = g_aza17
             AND ooo06 = tm.yy     AND ooo07 < tm.m1
          #  AND ooo10 = g_plant   AND ooo11 = tm.o   #TQC-670076
             AND ooo10 = g_ooz.ooz02p  AND ooo11 = tm.o   #TQC-670076
          SELECT SUM(ooo08d),SUM(ooo08c),SUM(ooo09d),SUM(ooo09c)  #期間余額
            INTO m_ooo08d,m_ooo08c,m_ooo09d,m_ooo09c
            FROM ooo_file
           WHERE ooo01 = sr1.ooo01 AND ooo02 = sr1.ooo02
             AND ooo03 = tm.a      AND ooo05 = g_aza17
             AND ooo06 = tm.yy     AND ooo07 BETWEEN tm.m1 AND tm.m2
          #  AND ooo10 = g_plant   AND ooo11 = tm.o  #TQC-670076
             AND ooo10 = g_ooz.ooz02p  AND ooo11 = tm.o  #TQC-670076
       ELSE
          SELECT SUM(ooo09d-ooo09c)  #期初余額
            INTO l_qcye    FROM ooo_file
           WHERE ooo01 = sr1.ooo01 AND ooo02 = sr1.ooo02
             AND ooo03 = tm.a
             AND ooo06 = tm.yy     AND ooo07 < tm.m1
           # AND ooo10 = g_plant   AND ooo11 = tm.o  #TQC-670076
             AND ooo10 = g_ooz.ooz02p  AND ooo11 = tm.o  #TQC-670076
          SELECT SUM(ooo09d),SUM(ooo09c)  #期間余額
            INTO m_ooo09d,m_ooo09c
            FROM ooo_file
           WHERE ooo01 = sr1.ooo01 AND ooo02 = sr1.ooo02
             AND ooo03 = tm.a
             AND ooo06 = tm.yy     AND ooo07 BETWEEN tm.m1 AND tm.m2
           # AND ooo10 = g_plant   AND ooo11 = tm.o  #TQC-670076
             AND ooo10 = g_ooz.ooz02p  AND ooo11 = tm.o  #TQC-670076
       END IF
       IF cl_null(l_qcyef)  THEN LET l_qcyef = 0 END IF
       IF cl_null(l_qcye )  THEN LET l_qcye  = 0 END IF
       IF cl_null(m_ooo08d) THEN LET m_ooo08d = 0 END IF
       IF cl_null(m_ooo08c) THEN LET m_ooo08c = 0 END IF
       IF cl_null(m_ooo09d) THEN LET m_ooo09d = 0 END IF
       IF cl_null(m_ooo09c) THEN LET m_ooo09c = 0 END IF
 
       IF tm.b = 'N' AND l_qcye = 0 AND      #本幣
          m_ooo09d = 0 AND m_ooo09c = 0 THEN
          LET g_null = 'Y'   #期初為零且無異動
       END IF
       IF tm.b='Y' AND l_qcyef=0 AND l_qcye=0 AND m_ooo08d=0   #外幣
          AND m_ooo08c=0  AND m_ooo09d=0 AND m_ooo09c=0 THEN
          LET g_null = 'Y'
       END IF
       IF tm.d = 'N' THEN  #期初為零且無異動不打印
          IF g_null = 'Y' THEN
             CONTINUE FOREACH
          END IF
       END IF
       IF tm.d = 'Y' AND g_null = 'Y' OR
          tm.b = 'N' AND m_ooo09d = 0 AND m_ooo09c = 0  OR
          tm.b='Y' AND m_ooo08d=0 AND m_ooo08c=0 AND m_ooo09d=0 AND m_ooo09c=0
          THEN
             LET sr.ooo01  = sr1.ooo01
             LET sr.ooo02  = sr1.ooo02
             LET sr.ooo06  = tm.yy
             LET sr.ooo08d = 0
             LET sr.ooo08c = 0
             LET sr.ooo09d = 0
             LET sr.ooo09c = 0
             LET sr.qcye   = l_qcye
             LET sr.qcyef  = l_qcyef
             LET sr.prt    = '0'
 
             IF tm.b = 'N' THEN   #本幣
#No.FUN-7B0026---Begin
             LET sr.ooo08c = ''                                                                                                      
             LET sr.ooo08d = ''                                                                                                      
             LET sr.qcyef = ''
             EXECUTE insert_prep USING                                                                                              
                           sr.ooo01,sr.ooo02,sr.ooo06,sr.ooo07,sr.ooo08c,                                                           
                           sr.ooo08d,sr.ooo09c,sr.ooo09d,sr.qcyef,sr.qcye,sr.prt 
#                OUTPUT TO REPORT axrr711_rep(sr.*)
#No.FUN-7B0026---End           
             ELSE                 #原幣
#No.FUN-7B0026---Begin
             EXECUTE insert_prep USING 
                           sr.ooo01,sr.ooo02,sr.ooo06,sr.ooo07,sr.ooo08c,
                           sr.ooo08d,sr.ooo09c,sr.ooo09d,sr.qcyef,sr.qcye,sr.prt
#                OUTPUT TO REPORT axrr711_rep1(sr.*)
#No.FUN-7B0026---End
             END IF
             CONTINUE FOREACH
       END IF
       FOREACH axrr711_curs USING sr1.ooo01,sr1.ooo02 INTO sr.*
         IF SQLCA.sqlcode THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,0)
            EXIT FOREACH
         END IF
         IF tm.b = 'N' THEN
            IF sr.ooo09d = 0 AND sr.ooo09c = 0 THEN
               CONTINUE FOREACH
            END IF
         ELSE
            IF sr.ooo09d=0 AND sr.ooo09c=0 AND sr.ooo08d=0 AND sr.ooo08c=0 THEN
               CONTINUE FOREACH
            END IF
         END IF
         LET sr.qcyef = l_qcyef
         LET sr.qcye  = l_qcye
         LET sr.prt   = '1'
         IF tm.b = 'N' THEN   #本幣
#No.FUn-7B0026---Begin  
             LET sr.ooo08c = ''                                                                                                     
             LET sr.ooo08d = ''                                                                                                     
             LET sr.qcyef = ''                                                                                                      
             EXECUTE insert_prep USING                                                                                              
                           sr.ooo01,sr.ooo02,sr.ooo06,sr.ooo07,sr.ooo08c,                                                           
                           sr.ooo08d,sr.ooo09c,sr.ooo09d,sr.qcyef,sr.qcye,sr.prt 
#            OUTPUT TO REPORT axrr711_rep(sr.*)
#No.FUN-7B0026---End   
         ELSE                 #原幣
#No.FUN-7B0026---Begin
         EXECUTE insert_prep USING  sr.ooo01,sr.ooo02,sr.ooo06,sr.ooo07,sr.ooo08c,
                     sr.ooo08d,sr.ooo09c,sr.ooo09d,sr.qcyef,sr.qcye,sr.prt
#            OUTPUT TO REPORT axrr711_rep1(sr.*)
#No.FUN-7B0026---End 
         END IF
         #LET g_print = g_print + 1    #No.FUN-7B0026
       END FOREACH
     END FOREACH
#No.FUN-7B0026---Begin
{
     IF tm.b = 'N' THEN   #本幣
        FINISH REPORT axrr711_rep
     ELSE                 #原幣
        FINISH REPORT axrr711_rep1
     END IF
 
     CALL cl_prt(l_name,g_prtway,g_copies,g_len)
}
      IF g_zz05 = 'Y' THEN                                                                                                          
         CALL cl_wcchp(tm.wc,'ooo01')                                                                       
              RETURNING tm.wc                                                                                                       
      END IF             
      LET l_str=tm.yy USING '&&&&','/',tm.m1 USING '&&','-',tm.yy USING '&&&&','/',tm.m2 USING '&&'               
      LET g_str=tm.wc ,";",tm.a,";",tm.e,";",l_str,";",g_azi04,";",tm.o,";",tm.c                                                 
                                                                                                                                    
                                                                                                                                    
                                                                                                                                    
   LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED                                                               
                                                                                                                                    
   CALL cl_prt_cs3('axrr711',l_name,l_sql,g_str) 
#No.FUN-7B0026---End 
END FUNCTION
#No.FUN-B10053  --Begin
FUNCTION axrr711_a()
    DEFINE l_aag02      LIKE aag_file.aag02  
    DEFINE l_aag07      LIKE aag_file.aag07
    DEFINE l_acti       LIKE aag_file.aagacti

    LET g_errno = ' '
    SELECT aag02,aag07,aagacti 
      INTO l_aag02,l_aag07,l_acti 
      FROM aag_file
     WHERE aag01 = tm.a AND aag07 IN ('2','3')
       AND aag00 = tm.o
    
    CASE WHEN SQLCA.SQLCODE = 100  LET g_errno = 'anm-027'
         WHEN l_acti  ='N'         LET g_errno = '9028'
         WHEN l_aag07  = '1'       LET g_errno = 'agl-015'
         OTHERWISE          LET g_errno = SQLCA.SQLCODE USING '-------'
    END CASE    
END FUNCTION 
#No.FUN-B10053  --End
#No.FUn-7B0026---Begin
{
 
REPORT axrr711_rep(sr)
   DEFINE l_last_sw LIKE type_file.chr1,            #No.FUN-680123 VARCHAR(1)
          sr        RECORD
                        ooo01    LIKE ooo_file.ooo01,
                        ooo02    LIKE ooo_file.ooo02,
                        ooo06    LIKE ooo_file.ooo06,
                        ooo07    LIKE ooo_file.ooo07,
                        ooo08d   LIKE ooo_file.ooo08d,
                        ooo08c   LIKE ooo_file.ooo08c,
                        ooo09d   LIKE ooo_file.ooo09d,
                        ooo09c   LIKE ooo_file.ooo09c,
                        qcyef    LIKE ooo_file.ooo08d,
                        qcye     LIKE ooo_file.ooo08d,
                        prt      LIKE type_file.chr1           #No.FUN-680123 VARCHAR(01)
                    END RECORD
 
  OUTPUT TOP MARGIN g_top_margin LEFT MARGIN g_left_margin BOTTOM MARGIN g_bottom_margin PAGE LENGTH g_page_line
  ORDER BY sr.ooo01,sr.ooo02,sr.ooo06,sr.ooo07
  FORMAT
   PAGE HEADER
#-----TQC-630155---------
        PRINT COLUMN ((g_len-FGL_WIDTH(g_company CLIPPED))/2)-1,g_company CLIPPED
#       PRINT COLUMN ((g_len-FGL_WIDTH(g_x[1]))/2)-1,g_x[1]   #No.TQC-6B0051
        LET g_pageno = g_pageno + 1
        LET pageno_total = PAGENO USING '<<<',"/pageno"
        PRINT g_head CLIPPED,pageno_total
        PRINT COLUMN ((g_len-FGL_WIDTH(g_x[1]))/2)-1,g_x[1]   #No.TQC-6B0051
        PRINT g_x[11] CLIPPED,l_aag02 CLIPPED,'(',tm.a CLIPPED,')'
#No.TQC-6B0051 --begin  --mark--
#        PRINT g_x[2] CLIPPED,g_pdate ,' ',TIME,
#              #COLUMN 33,g_x[27] CLIPPED,                #FUN-660060 remark
#              COLUMN (g_len-25)/2+1, g_x[27] CLIPPED,    #FUN-660060
#              tm.yy USING '&&&&','/',tm.m1 USING '&&','-',
#              tm.yy USING '&&&&','/',tm.m2 USING '&&',
#              COLUMN 65,g_x[26] CLIPPED,tm.o CLIPPED,
#              COLUMN g_len-7,g_x[3] CLIPPED,PAGENO USING '<<<'
#No.TQC-6B0051 --begin  --mark--
        #No.TQC-6B0051  --begin
        PRINT g_x[27] CLIPPED,
              tm.yy USING '&&&&','/',tm.m1 USING '&&','-',
              tm.yy USING '&&&&','/',tm.m2 USING '&&',
              COLUMN 30,g_x[26] CLIPPED,tm.o CLIPPED 
         #No.TQC-6B0051  --end
        PRINT g_dash
        PRINT g_x[30],g_x[31],g_x[32],g_x[33],g_x[34],g_x[35],g_x[36],g_x[37],
              g_x[38]
        PRINT g_dash1
        LET l_last_sw = 'n'
 
 
#     PRINT (g_len-FGL_WIDTH(g_company CLIPPED))/2 SPACES,g_company CLIPPED
#     IF g_towhom IS NULL OR g_towhom = ' '
#        THEN PRINT '';
#        ELSE PRINT 'TO:',g_towhom;
#     END IF
#     PRINT COLUMN (g_len-FGL_WIDTH(g_user)-5),'FROM:',g_user CLIPPED
#     PRINT (g_len-FGL_WIDTH(g_x[1] CLIPPED))/2 SPACES,g_x[1]
#     PRINT ' '
#     LET g_pageno = g_pageno + 1
#     PRINT g_x[11] CLIPPED,l_aag02 CLIPPED,'(',tm.a CLIPPED,')'
#     PRINT g_x[2] CLIPPED,g_pdate ,' ',TIME,
#           COLUMN 33,g_x[27] CLIPPED,
#           tm.yy USING '&&&&','/',tm.m1 USING '&&','-',
#           tm.yy USING '&&&&','/',tm.m2 USING '&&',
#           COLUMN 65,g_x[26] CLIPPED,tm.o CLIPPED,
#           COLUMN g_len-7,g_x[3] CLIPPED,PAGENO USING '<<<'
#     PRINT g_dash[1,g_len]
#     PRINT COLUMN  1,g_x[13] CLIPPED,
#           COLUMN 43,g_x[14] CLIPPED,
#           COLUMN 81,g_x[15] CLIPPED
#     PRINT '---------- -------- ---- -- -------- ------------------ ',
#           '------------------  --  ------------------'
#     LET l_last_sw ='n'
#-----END TQC-630155-----
 
   BEFORE GROUP OF sr.ooo02
     IF tm.e = 'Y' THEN
        SKIP TO TOP OF PAGE
     END IF
     LET g_qcye = sr.qcye
     LET g_ooo09_l = 0
     LET g_ooo09_r = 0
#-----TQC-630155---------
     PRINT COLUMN g_c[30],sr.ooo01 CLIPPED,
           COLUMN g_c[31],sr.ooo02 CLIPPED,
           COLUMN g_c[32],sr.ooo06 USING "&&&&",
           COLUMN g_c[34],g_x[21] CLIPPED;
     IF sr.qcye > 0 THEN
        PRINT COLUMN g_c[37],g_x[22],
              COLUMN g_c[38],cl_numfor(sr.qcye,38,g_azi04)
     ELSE
        IF sr.qcye = 0 THEN
           PRINT COLUMN g_c[37],g_x[29],
                 COLUMN g_c[38],cl_numfor(sr.qcye,38,g_azi04)
        ELSE
           PRINT COLUMN g_c[37],g_x[28],
                 COLUMN g_c[38],cl_numfor(sr.qcye*-1,38,g_azi04)
        END IF
     END IF
 
#    PRINT COLUMN 1,sr.ooo01 CLIPPED,
#          COLUMN 12,sr.ooo02 CLIPPED,
#          COLUMN 21,sr.ooo06 USING "&&&&",
#          COLUMN 29,g_x[21] CLIPPED;
#    IF sr.qcye > 0 THEN
#       PRINT COLUMN 77,g_x[22],
#             COLUMN 80,cl_numfor(sr.qcye,18,g_azi04)
#    ELSE
#       IF sr.qcye = 0 THEN
#          PRINT COLUMN 77,g_x[29],
#                COLUMN 80,cl_numfor(sr.qcye,18,g_azi04)
#       ELSE
#          PRINT COLUMN 77,g_x[28],
#                COLUMN 80,cl_numfor(sr.qcye*-1,18,g_azi04)
#       END IF
#    END IF
#-----END TQC-630155-----
   ON EVERY ROW
#-----TQC-630155---------
     IF sr.prt = '1' THEN
        PRINT COLUMN  g_c[33],sr.ooo07 USING "#&",
              COLUMN  g_c[34],g_x[23] CLIPPED,
              COLUMN  g_c[35],cl_numfor(sr.ooo09d,35,g_azi04),
              COLUMN  g_c[36],cl_numfor(sr.ooo09c,36,g_azi04);
        LET g_ooo09_l = g_ooo09_l + sr.ooo09d
        LET g_ooo09_r = g_ooo09_r + sr.ooo09c
        LET g_qcye = g_qcye + sr.ooo09d - sr.ooo09c
        IF g_qcye > 0 THEN
           PRINT COLUMN g_c[37],g_x[22],
                 COLUMN g_c[38],cl_numfor(g_qcye,38,g_azi04)
        ELSE
           IF g_qcye = 0 THEN
              PRINT COLUMN g_c[37],g_x[29],
                    COLUMN g_c[38],cl_numfor(g_qcye,38,g_azi04)
           ELSE
              PRINT COLUMN g_c[37],g_x[28],
                    COLUMN g_c[38],cl_numfor(g_qcye*-1,38,g_azi04)
           END IF
        END IF
     END IF
 
#    IF sr.prt = '1' THEN
#       PRINT COLUMN  26,sr.ooo07 USING "#&",
#             COLUMN  29,g_x[23] CLIPPED,
#             COLUMN  37,cl_numfor(sr.ooo09d,18,g_azi04),
#             COLUMN  53,cl_numfor(sr.ooo09c,18,g_azi04);
#       LET g_ooo09_l = g_ooo09_l + sr.ooo09d
#       LET g_ooo09_r = g_ooo09_r + sr.ooo09c
#       LET g_qcye = g_qcye + sr.ooo09d - sr.ooo09c
#       IF g_qcye > 0 THEN
#          PRINT COLUMN 77,g_x[22],
#                COLUMN 80,cl_numfor(g_qcye,18,g_azi04)
#       ELSE
#          IF g_qcye = 0 THEN
#             PRINT COLUMN 77,g_x[29],
#                   COLUMN 80,cl_numfor(g_qcye,18,g_azi04)
#          ELSE
#             PRINT COLUMN 77,g_x[28],
#                   COLUMN 80,cl_numfor(g_qcye*-1,18,g_azi04)
#          END IF
#       END IF
#    END IF
#-----END TQC-630155-----
 
   AFTER GROUP OF sr.ooo02
#-----TQC-630155---------
     PRINT COLUMN g_c[34],g_x[24] CLIPPED,
           COLUMN g_c[35],cl_numfor(g_ooo09_l,35,g_azi04),
           COLUMN g_c[36],cl_numfor(g_ooo09_r,36,g_azi04);
     IF g_qcye > 0 THEN
        PRINT COLUMN g_c[37],g_x[22],
              COLUMN g_c[38],cl_numfor(g_qcye,38,g_azi04)
     ELSE
        IF g_qcye = 0 THEN
           PRINT COLUMN g_c[37],g_x[29],
                 COLUMN g_c[38],cl_numfor(g_qcye,38,g_azi04)
        ELSE
           PRINT COLUMN g_c[37],g_x[28],
                 COLUMN g_c[38],cl_numfor(g_qcye*-1,38,g_azi04)
        END IF
     END IF
     LET t_qcye = t_qcye + g_qcye
     LET t_ooo09_l = t_ooo09_l + g_ooo09_l
     LET t_ooo09_r = t_ooo09_r + g_ooo09_r
     PRINT
 
#    PRINT COLUMN 29,g_x[24] CLIPPED,
#          COLUMN 37,cl_numfor(g_ooo09_l,18,g_azi04),
#          COLUMN 53,cl_numfor(g_ooo09_r,18,g_azi04);
#    IF g_qcye > 0 THEN
#       PRINT COLUMN 77,g_x[22],
#             COLUMN 80,cl_numfor(g_qcye,18,g_azi04)
#    ELSE
#       IF g_qcye = 0 THEN
#          PRINT COLUMN 77,g_x[29],
#                COLUMN 80,cl_numfor(g_qcye,18,g_azi04)
#       ELSE
#          PRINT COLUMN 77,g_x[28],
#                COLUMN 80,cl_numfor(g_qcye*-1,18,g_azi04)
#       END IF
#    END IF
#    LET t_qcye = t_qcye + g_qcye
#    LET t_ooo09_l = t_ooo09_l + g_ooo09_l
#    LET t_ooo09_r = t_ooo09_r + g_ooo09_r
#    PRINT
#-----END TQC-630155-----
 
   ON LAST ROW
#-----TQC-630155---------
      PRINT COLUMN g_c[34],g_x[25] CLIPPED,
            COLUMN g_c[35],cl_numfor(t_ooo09_l,35,g_azi04),
            COLUMN g_c[36],cl_numfor(t_ooo09_r,36,g_azi04);
      IF t_qcye > 0 THEN
         PRINT COLUMN g_c[37],g_x[22],
               COLUMN g_c[38],cl_numfor(t_qcye,38,g_azi04)
      ELSE
         IF t_qcye = 0 THEN
            PRINT COLUMN g_c[37],g_x[29],
                  COLUMN g_c[38],cl_numfor(t_qcye,38,g_azi04)
         ELSE
            PRINT COLUMN g_c[37],g_x[28],
                  COLUMN g_c[38],cl_numfor(t_qcye*-1,38,g_azi04)
         END IF
      END IF
 
#     PRINT COLUMN 29,g_x[25] CLIPPED,
#           COLUMN 37,cl_numfor(t_ooo09_l,18,g_azi04),
#           COLUMN 53,cl_numfor(t_ooo09_r,18,g_azi04);
#     IF t_qcye > 0 THEN
#        PRINT COLUMN 77,g_x[22],
#              COLUMN 80,cl_numfor(t_qcye,18,g_azi04)
#     ELSE
#        IF t_qcye = 0 THEN
#           PRINT COLUMN 77,g_x[29],
#                 COLUMN 80,cl_numfor(t_qcye,18,g_azi04)
#        ELSE
#           PRINT COLUMN 77,g_x[28],
#                 COLUMN 80,cl_numfor(t_qcye*-1,18,g_azi04)
#        END IF
#     END IF
#-----END TQC-630155-----
      PRINT g_dash[1,g_len]
      LET l_last_sw = 'y'
      PRINT g_x[4],g_x[5] CLIPPED, COLUMN (g_len-9), g_x[7] CLIPPED
 
   PAGE TRAILER
      IF l_last_sw = 'n' THEN
         PRINT g_dash[1,g_len]
         PRINT g_x[4],g_x[5] CLIPPED, COLUMN (g_len-9), g_x[6] CLIPPED
      ELSE SKIP 2 LINE
      END IF
END REPORT
 
REPORT axrr711_rep1(sr)
   DEFINE l_last_sw LIKE type_file.chr1,          #No.FUN-680123 VARCHAR(1)
          sr        RECORD
                        ooo01    LIKE ooo_file.ooo01,
                        ooo02    LIKE ooo_file.ooo02,
                        ooo06    LIKE ooo_file.ooo06,
                        ooo07    LIKE ooo_file.ooo07,
                        ooo08d   LIKE ooo_file.ooo08d,
                        ooo08c   LIKE ooo_file.ooo08c,
                        ooo09d   LIKE ooo_file.ooo09d,
                        ooo09c   LIKE ooo_file.ooo09c,
                        qcyef    LIKE ooo_file.ooo08d,
                        qcye     LIKE ooo_file.ooo08d,
                        prt      LIKE type_file.chr1          #No.FUN-680123 VARCHAR(01)
                    END RECORD
 
  OUTPUT TOP MARGIN g_top_margin LEFT MARGIN g_left_margin BOTTOM MARGIN g_bottom_margin PAGE LENGTH g_page_line
  ORDER BY sr.ooo01,sr.ooo02,sr.ooo06,sr.ooo07
  FORMAT
   PAGE HEADER
#-----TQC-630155---------
        PRINT COLUMN ((g_len-FGL_WIDTH(g_company CLIPPED))/2)-1,g_company CLIPPED
        PRINT COLUMN ((g_len-FGL_WIDTH(g_x[1]))/2)-1,g_x[1]
        LET g_pageno = g_pageno + 1
        LET pageno_total = PAGENO USING '<<<',"/pageno"
        PRINT g_head CLIPPED,pageno_total
        PRINT g_x[11] CLIPPED,l_aag02 CLIPPED,'(',tm.a CLIPPED,')'
        PRINT g_x[2] CLIPPED,g_pdate ,' ',TIME,
              COLUMN 51,g_x[27] CLIPPED,
              tm.yy USING '&&&&','/',tm.m1 USING '&&','-',
              tm.yy USING '&&&&','/',tm.m2 USING '&&',
              COLUMN 87,g_x[26] CLIPPED,tm.o CLIPPED,
              COLUMN g_len-7,g_x[3] CLIPPED,PAGENO USING '<<<'
        PRINT g_dash
        PRINT COLUMN 54,g_x[16],
              COLUMN 92,g_x[17], 
              COLUMN 135,g_x[18]
        PRINT COLUMN 38,g_x[19],
              COLUMN 76,g_x[19],
              COLUMN 119,g_x[19]
        PRINT g_x[30],g_x[31],g_x[32],g_x[33],g_x[34],g_x[39],g_x[40],g_x[41],
              g_x[42],g_x[43],g_x[44],g_x[45]
        PRINT g_dash1
        LET l_last_sw = 'n'
 
#     PRINT (g_len-FGL_WIDTH(g_company CLIPPED))/2 SPACES,g_company CLIPPED
#     IF g_towhom IS NULL OR g_towhom = ' '
#        THEN PRINT '';
#        ELSE PRINT 'TO:',g_towhom;
#     END IF
#     PRINT COLUMN (g_len-FGL_WIDTH(g_user)-5),'FROM:',g_user CLIPPED
#     PRINT (g_len-FGL_WIDTH(g_x[1] CLIPPED))/2 SPACES,g_x[1]
#     PRINT ' '
#     LET g_pageno = g_pageno + 1
#     PRINT g_x[11] CLIPPED,l_aag02 CLIPPED,'(',tm.a CLIPPED,')'
#     PRINT g_x[2] CLIPPED,g_pdate ,' ',TIME,
#           COLUMN 51,g_x[27] CLIPPED,
#           tm.yy USING '&&&&','/',tm.m1 USING '&&','-',
#           tm.yy USING '&&&&','/',tm.m2 USING '&&',
#           COLUMN 87,g_x[26] CLIPPED,tm.o CLIPPED,
#           COLUMN g_len-7,g_x[3] CLIPPED,PAGENO USING '<<<'
#     PRINT g_dash[1,g_len]
#     PRINT COLUMN 43,g_x[16] CLIPPED,COLUMN 81,g_x[17] CLIPPED,
#           COLUMN 124,g_x[18] CLIPPED
#     PRINT '                                    ',
#           ' ------------------------------------- -------------------------------------',
#           '      -------------------------------------'
#     PRINT COLUMN  1,g_x[13] CLIPPED, COLUMN 41,g_x[19] CLIPPED,
#           COLUMN 79,g_x[19] CLIPPED, COLUMN 114,g_x[20] CLIPPED,
#           COLUMN 122,g_x[19] CLIPPED
#     PRINT '---------- -------- ---- -- -------- ------------------ ',
#           '------------------ ------------------ ------------------',
#           '  --  ------------------ ------------------'
#     LET l_last_sw ='n'
#-----END TQC-630155-----
 
   BEFORE GROUP OF sr.ooo02
     IF tm.e = 'Y' THEN
        SKIP TO TOP OF PAGE
     END IF
     LET g_qcyef = sr.qcyef
     LET g_qcye  = sr.qcye
     LET g_ooo08_l = 0
     LET g_ooo08_r = 0
     LET g_ooo09_l = 0
     LET g_ooo09_r = 0
#-----TQC-630155---------
     PRINT COLUMN g_c[30],sr.ooo01 CLIPPED,
           COLUMN g_c[31],sr.ooo02 CLIPPED,
           COLUMN g_c[32],sr.ooo06 USING "&&&&",
           COLUMN g_c[34],g_x[21];
     IF sr.qcyef > 0 THEN
        PRINT COLUMN g_c[43],g_x[22],
              COLUMN g_c[44],cl_numfor(sr.qcyef,44,g_azi04),      #No.CHI-6A0004
              COLUMN g_c[45],cl_numfor(sr.qcye,45,g_azi04)
     ELSE
        IF sr.qcyef = 0 THEN
           PRINT COLUMN g_c[43],g_x[29],
                 COLUMN g_c[44],cl_numfor(sr.qcyef,44,g_azi04),      #No.CHI-6A0004
                 COLUMN g_c[45],cl_numfor(sr.qcye,45,g_azi04)
        ELSE
           PRINT COLUMN g_c[43],g_x[28],
                 COLUMN g_c[44],cl_numfor(sr.qcyef*-1,44,g_azi04),      #No.CHI-6A0004
                 COLUMN g_c[45],cl_numfor(sr.qcye*-1,45,g_azi04)
        END IF
     END IF
 
#    PRINT COLUMN 1,sr.ooo01 CLIPPED,
#          COLUMN 12,sr.ooo02 CLIPPED,
#          COLUMN 21,sr.ooo06 USING "&&&&",
#          COLUMN 29,g_x[21];
#    IF sr.qcyef > 0 THEN
#       PRINT COLUMN 115,g_x[22],
#             COLUMN 118,cl_numfor(sr.qcyef,18,g_azi04),      #No.CHI-6A0004
#             COLUMN 136,cl_numfor(sr.qcye,18,g_azi04)
#    ELSE
#       IF sr.qcyef = 0 THEN
#          PRINT COLUMN 115,g_x[29],
#                COLUMN 118,cl_numfor(sr.qcyef,18,g_azi04),      #No.CHI-6A0004
#                COLUMN 136,cl_numfor(sr.qcye,18,g_azi04)
#       ELSE
#          PRINT COLUMN 115,g_x[28],
#                COLUMN 118,cl_numfor(sr.qcyef*-1,18,g_azi04),      #No.CHI-6A0004
#                COLUMN 136,cl_numfor(sr.qcye*-1,18,g_azi04)
#       END IF
#    END IF
#-----END TQC-630155-----
 
   ON EVERY ROW
#-----TQC-630155---------
     IF sr.prt = '1' THEN
        PRINT COLUMN  g_c[33],sr.ooo07 USING "#&",
              COLUMN  g_c[34],g_x[23] CLIPPED,
              COLUMN  g_c[39],cl_numfor(sr.ooo08d,39,g_azi04),      #No.CHI-6A0004
              COLUMN  g_c[40],cl_numfor(sr.ooo09d,40,g_azi04),
              COLUMN  g_c[41],cl_numfor(sr.ooo08c,41,g_azi04),      #No.CHI-6A0004
              COLUMN  g_c[42],cl_numfor(sr.ooo09c,42,g_azi04);
        LET g_ooo08_l = g_ooo08_l + sr.ooo08d
        LET g_ooo08_r = g_ooo08_r + sr.ooo08c
        LET g_qcyef = g_qcyef + sr.ooo08d - sr.ooo08c
        LET g_ooo09_l = g_ooo09_l + sr.ooo09d
        LET g_ooo09_r = g_ooo09_r + sr.ooo09c
        LET g_qcye  = g_qcye  + sr.ooo09d - sr.ooo09c
        IF g_qcyef > 0 THEN
           PRINT COLUMN g_c[43],g_x[22],
                 COLUMN g_c[44],cl_numfor(g_qcyef,44,g_azi04),      #No.CHI-6A0004
                 COLUMN g_c[45],cl_numfor(g_qcye,45,g_azi04)
        ELSE
           IF g_qcyef = 0 THEN
              PRINT COLUMN g_c[43],g_x[29],
                    COLUMN g_c[44],cl_numfor(g_qcyef,44,g_azi04),      #No.CHI-6A0004
                    COLUMN g_c[45],cl_numfor(g_qcye,45,g_azi04)
           ELSE
              PRINT COLUMN g_c[43],g_x[28],
                    COLUMN g_c[44],cl_numfor(g_qcyef*-1,44,g_azi04),      #No.CHI-6A0004
                    COLUMN g_c[45],cl_numfor(g_qcye*-1,45,g_azi04)
           END IF
        END IF
     END IF
 
#    IF sr.prt = '1' THEN
#       PRINT COLUMN  26,sr.ooo07 USING "#&",
#             COLUMN  29,g_x[23] CLIPPED,
#             COLUMN  37,cl_numfor(sr.ooo08d,18,g_azi04),      #No.CHI-6A0004
#             COLUMN  55,cl_numfor(sr.ooo09d,18,g_azi04),
#             COLUMN  73,cl_numfor(sr.ooo08c,18,g_azi04),      #No.CHI-6A0004
#             COLUMN  91,cl_numfor(sr.ooo09c,18,g_azi04);
#       LET g_ooo08_l = g_ooo08_l + sr.ooo08d
#       LET g_ooo08_r = g_ooo08_r + sr.ooo08c
#       LET g_qcyef = g_qcyef + sr.ooo08d - sr.ooo08c
#       LET g_ooo09_l = g_ooo09_l + sr.ooo09d
#       LET g_ooo09_r = g_ooo09_r + sr.ooo09c
#       LET g_qcye  = g_qcye  + sr.ooo09d - sr.ooo09c
#       IF g_qcyef > 0 THEN
#          PRINT COLUMN 115,g_x[22],
#                COLUMN 118,cl_numfor(g_qcyef,18,g_azi04),      #No.CHI-6A0004
#                COLUMN 136,cl_numfor(g_qcye,18,g_azi04)
#       ELSE
#          IF g_qcyef = 0 THEN
#             PRINT COLUMN 115,g_x[29],
#                   COLUMN 118,cl_numfor(g_qcyef,18,g_azi04),      #No.CHI-6A0004
#                   COLUMN 136,cl_numfor(g_qcye,18,g_azi04)
#          ELSE
#             PRINT COLUMN 115,g_x[28],
#                   COLUMN 118,cl_numfor(g_qcyef*-1,18,g_azi04),      #No.CHI-6A0004
#                   COLUMN 136,cl_numfor(g_qcye*-1,18,g_azi04)
#          END IF
#       END IF
#    END IF
#-----END TQC-630155-----
 
   AFTER GROUP OF sr.ooo02
#-----TQC-630155---------
     PRINT COLUMN g_c[34],g_x[24] CLIPPED,
           COLUMN g_c[39],cl_numfor(g_ooo08_l,39,g_azi04),      #No.CHI-6A0004
           COLUMN g_c[40],cl_numfor(g_ooo09_l,40,g_azi04),
           COLUMN g_c[41],cl_numfor(g_ooo08_r,41,g_azi04),      #No.CHI-6A0004
           COLUMN g_c[42],cl_numfor(g_ooo09_r,42,g_azi04);
     IF g_qcyef > 0 THEN
        PRINT COLUMN g_c[43],g_x[22],
              COLUMN g_c[44],cl_numfor(g_qcyef,44,g_azi04),      #No.CHI-6A0004
              COLUMN g_c[45],cl_numfor(g_qcye,45,g_azi04)
     ELSE
        IF g_qcyef = 0 THEN
           PRINT COLUMN g_c[43],g_x[29],
                 COLUMN g_c[44],cl_numfor(g_qcyef,44,g_azi04),      #No.CHI-6A0004
                 COLUMN g_c[45],cl_numfor(g_qcye,45,g_azi04)
        ELSE
           PRINT COLUMN g_c[43],g_x[28],
                 COLUMN g_c[44],cl_numfor(g_qcyef*-1,44,g_azi04),      #No.CHI-6A0004
                 COLUMN g_c[45],cl_numfor(g_qcye*-1,45,g_azi04)
        END IF
     END IF
 
#    PRINT COLUMN 29,g_x[24] CLIPPED,
#          COLUMN 37,cl_numfor(g_ooo08_l,18,g_azi04),      #No.CHI-6A0004
#          COLUMN 55,cl_numfor(g_ooo09_l,18,g_azi04),
#          COLUMN 73,cl_numfor(g_ooo08_r,18,g_azi04),      #No.CHI-6A0004
#          COLUMN 91,cl_numfor(g_ooo09_r,18,g_azi04);
#    IF g_qcyef > 0 THEN
#       PRINT COLUMN 115,g_x[22],
#             COLUMN 118,cl_numfor(g_qcyef,18,g_azi04),      #No.CHI-6A0004
#             COLUMN 136,cl_numfor(g_qcye,18,g_azi04)
#    ELSE
#       IF g_qcyef = 0 THEN
#          PRINT COLUMN 115,g_x[29],
#                COLUMN 118,cl_numfor(g_qcyef,18,g_azi04),      #No.CHI-6A0004
#                COLUMN 136,cl_numfor(g_qcye,18,g_azi04)
#       ELSE
#          PRINT COLUMN 115,g_x[28],
#                COLUMN 118,cl_numfor(g_qcyef*-1,18,g_azi04),      #No.CHI-6A0004
#                COLUMN 136,cl_numfor(g_qcye*-1,18,g_azi04)
#       END IF
#    END IF
#-----END TQC-630155-----
     LET t_qcyef = t_qcyef + g_qcyef
     LET t_ooo08_l = t_ooo08_l + g_ooo08_l
     LET t_ooo08_r = t_ooo08_r + g_ooo08_r
     LET t_qcye  = t_qcye  + g_qcye
     LET t_ooo09_l = t_ooo09_l + g_ooo09_l
     LET t_ooo09_r = t_ooo09_r + g_ooo09_r
     PRINT
 
   ON LAST ROW
#-----TQC-630155---------
      PRINT COLUMN g_c[34],g_x[25] CLIPPED,
            COLUMN g_c[39],cl_numfor(t_ooo08_l,39,g_azi04),      #No.CHI-6A0004
            COLUMN g_c[40],cl_numfor(t_ooo09_l,40,g_azi04),
            COLUMN g_c[41],cl_numfor(t_ooo08_r,41,g_azi04),      #No.CHI-6A0004
            COLUMN g_c[42],cl_numfor(t_ooo09_r,42,g_azi04);
      IF t_qcyef > 0 THEN
         PRINT COLUMN g_c[43],g_x[22],
               COLUMN g_c[44],cl_numfor(t_qcyef,44,g_azi04),      #No.CHI-6A0004
               COLUMN g_c[45],cl_numfor(t_qcye,45,g_azi04)
      ELSE
         IF t_qcyef = 0 THEN
            PRINT COLUMN g_c[43],g_x[29],
                  COLUMN g_c[44],cl_numfor(t_qcyef,44,g_azi04),      #No.CHI-6A0004
                  COLUMN g_c[45],cl_numfor(t_qcye,45,g_azi04)
         ELSE
            PRINT COLUMN g_c[43],g_x[28],
                  COLUMN g_c[44],cl_numfor(t_qcyef*-1,44,g_azi04),      #No.CHI-6A0004
                  COLUMN g_c[45],cl_numfor(t_qcye*-1,45,g_azi04)
         END IF
      END IF
 
#     PRINT COLUMN 29,g_x[25] CLIPPED,
#           COLUMN 37,cl_numfor(t_ooo08_l,18,g_azi04),      #No.CHI-6A0004
#           COLUMN 55,cl_numfor(t_ooo09_l,18,g_azi04),
#           COLUMN 73,cl_numfor(t_ooo08_r,18,g_azi04),      #No.CHI-6A0004
#           COLUMN 91,cl_numfor(t_ooo09_r,18,g_azi04);
#     IF t_qcyef > 0 THEN
#        PRINT COLUMN 115,g_x[22],
#              COLUMN 118,cl_numfor(t_qcyef,18,g_azi04),      #No.CHI-6A0004
#              COLUMN 136,cl_numfor(t_qcye,18,g_azi04)
#     ELSE
#        IF t_qcyef = 0 THEN
#           PRINT COLUMN 115,g_x[29],
#                 COLUMN 118,cl_numfor(t_qcyef,18,g_azi04),      #No.CHI-6A0004
#                 COLUMN 136,cl_numfor(t_qcye,18,g_azi04)
#        ELSE
#           PRINT COLUMN 115,g_x[28],
#                 COLUMN 118,cl_numfor(t_qcyef*-1,18,g_azi04),      #No.CHI-6A0004
#                 COLUMN 136,cl_numfor(t_qcye*-1,18,g_azi04)
#        END IF
#     END IF
#-----END TQC-630155-----
      PRINT g_dash[1,g_len]
      LET l_last_sw = 'y'
      PRINT g_x[4],g_x[5] CLIPPED, COLUMN (g_len-9), g_x[7] CLIPPED
 
   PAGE TRAILER
      IF l_last_sw = 'n' THEN
         PRINT g_dash[1,g_len]
         PRINT g_x[4],g_x[5] CLIPPED, COLUMN (g_len-9), g_x[6] CLIPPED
      ELSE SKIP 2 LINE
      END IF
 
END REPORT
 
#Patch....NO.TQC-610037 <001> #
}
#No.FUN-7B0026---End
