...
# Modify.........: No.MOD-C50056 12/05/09 By Elise PREPARE r330_prepare2的aza26 = '2'的SQL取消aza34條件 
...
DEFINE tm  RECORD                            # Print condition RECORD
  	    ...
  	    n        LIKE type_file.chr1,     # 是否列印產品明細 #No.FUN-680123 VARCHAR(01)
           ...
           #x        LIKE type_file.chr1,     # 是否將客戶對帳單輸出至Word Process Console #No.FUN-680123 VARCHAR(01)   #MOD-850306 mark
...
MAIN
   ...
   LET l_table = cl_prt_temptable('axrr3301',g_sql) CLIPPED   # 產生Temp Table   #MOD-810074
   ...
   LET g_sql = "oma01.oma_file.oma01,omb03.omb_file.omb03,",
               ...
               "azi03.azi_file.azi03,azi04.azi_file.azi04,",
               "azi05.azi_file.azi05,azi07.azi_file.azi07"
   LET l_table2 = cl_prt_temptable('axrr3302',g_sql) CLIPPED   # 產生Temp Table
  ...
  #LET tm.x    =ARG_VAL(14)   #MOD-850306 mark
   ...
   IF cl_null(tm.wc)
      THEN CALL axrr330_tm(0,0)                 # Input print condition
   ELSE
      CALL axrr330()                            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
END MAIN
 
FUNCTION axrr330_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01    #No.FUN-580031
   ...
   OPEN WINDOW axrr330_w AT p_row,p_col WITH FORM "axr/42f/axrr330"
...
WHILE TRUE
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axrr330_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
      EXIT PROGRAM
   END IF
   ...
   INPUT BY NAME tm2.s1,tm2.s2,tm2.s3,
                 ...
                 tm2.u1,tm2.u2,tm2.u3,tm.deadline,tm.n,tm.y,   #tm.x,   #MOD-850306 mark tm.x
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axrr330',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axrr330_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0095
  ...
  ##no.5196(end)
   ...
   PREPARE axrr330_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
      EXIT PROGRAM
   END IF
   DECLARE axrr330_curs1 CURSOR FOR axrr330_prepare1
...
### ******** Word Process Console 2002/07/31 ******** ###
  ...
  #IF tm.x = 'N' THEN
  #   START REPORT axrr330_rep1 TO l_name
  #ELSE
  #   START REPORT axrr330_rep2 TO l_name
  #END IF
  #end FUN-710080 mark
### ************************************************* ###
 
  #LET headtag = '0'   #FUN-710080 mark
   FOREACH axrr330_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
         ...
         WHILE cl_null(l_oma24)
            IF g_ooz.ooz62 = 'N' THEN
               LET l_sql_2 = "SELECT COUNT(*) FROM oox_file",
                             ...
                             "   AND oox03 = '",sr.oma01,"'",
                             "   AND oox04 = '0'"
               PREPARE r330_prepare7 FROM l_sql_2
               DECLARE r330_oox7 CURSOR FOR r330_prepare7
               OPEN r330_oox7
               FETCH r330_oox7 INTO l_count
               CLOSE r330_oox7                       
               IF l_count = 0 THEN
            ...
            ELSE
               LET l_sql_2 = "SELECT COUNT(*) FROM oox_file",
                             ...
                             "   AND oox03 = '",sr.oma01,"'",
                             "   AND oox04 <> '0'"
               PREPARE r330_prepare8 FROM l_sql_2
               DECLARE r330_oox8 CURSOR FOR r330_prepare8
               OPEN r330_oox8
               FETCH r330_oox8 INTO l_count
               CLOSE r330_oox8                       
               IF l_count = 0 THEN
                  #LET l_oma24 = '1'   #TQC-B10083 mark 
                  EXIT WHILE           #TQC-B10083 add
            ...
            IF l_count <> 0 THEN        
               PREPARE r330_prepare07 FROM l_sql_1
               DECLARE r330_oox07 CURSOR FOR r330_prepare07
               OPEN r330_oox07
               FETCH r330_oox07 INTO l_oma24
               CLOSE r330_oox07
            END IF              
         END WHILE                       
      END IF
      ...
      CALL axmr330_getdesc(sr.oma00) RETURNING sr.oma00			
      #no.5196
      #--modi by kitty Bug NO:A057
      ...
      END IF
      DECLARE axrr330_cur2 CURSOR FOR r330_prepare2
      #No.FUN-5B0139 --end--
 
      LET l_flag = 'N'
      FOREACH axrr330_cur2 INTO sr2.*
         #-----MOD-810074---------
         IF l_flag = 'N' THEN 
...
### ******** Word Process Console 2002/07/31 ******** ###
  #str FUN-710080 mark
  #IF tm.x='N' then
  #   FINISH REPORT axrr330_rep1
  #   CALL cl_prt(l_name,g_prtway,g_copies,g_len)
  #ELSE
  #   FINISH REPORT axrr330_rep2
...
## ************************************************* ###
   ...
   LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
              #" ORDER BY oma03,oma032 nulls first,oma02 |",  #FUN-C30037 add  ORDER BY 'oma03,oma032 nulls first,oma02' #TQC-CA0058 mark
               " ORDER BY oma032 nulls first,oma02 |",  #TQC-CA0058 add
               "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED
   #-----END MOD-810074-----
   #傳per檔的排序、跳頁、小計、帳款截止日等參數
   LET g_str = tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",
               tm.t,";",tm.u,";",tm.deadline,";",tm.n,";",tm.y,";",
               t_azi05,";",g_azi07
   #是否列印選擇條件
   IF g_zz05 = 'Y' THEN
   ...
   END IF
   IF tm.y = 'Y' THEN
      CALL cl_prt_cs3('axrr330','axrr330_1',l_sql,g_str)
   ELSE
      CALL cl_prt_cs3('axrr330','axrr330_2',l_sql,g_str)
   END IF
   #------------------------------ CR (4) ------------------------------#
   #end FUN-710080 add
...
FUNCTION axmr330_getdesc(l_oma00)
...
#         LET l_cmd = 'echo -n "">',xml_name CLIPPED      #TQC-630185 #MOD-650098
          ...
          CALL l_channel.write( "<wordProcess reportName=""axrr330"">" )
