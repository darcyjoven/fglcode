# Prog. Version..: '5.30.06-13.03.18(00007)'     #
#
# Pattern name...: axrg440.4gl
...
MAIN
   ...
   LET l_table = cl_prt_temptable('axrg440',g_sql) CLIPPED
   IF l_table = -1 THEN 
  ...
  LET l_table1 = cl_prt_temptable('axrg4401',g_sql) CLIPPED                      
  IF l_table1 = -1 THEN 
     #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add    #FUN-C10036   mark
     #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)    #FUN-B50018  add   #FUN-C10036   mark
     EXIT PROGRAM
  ...
  LET l_table2 = cl_prt_temptable('axrg4402',g_sql) CLIPPED                    
  IF l_table2 = -1 THEN 
  ...
  LET l_table3 = cl_prt_temptable('axrg4403',g_sql) CLIPPED                    
  IF l_table3 = -1 THEN 
     CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add
     CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)    #FUN-B50018  add
     EXIT PROGRAM 
  ...
  LET l_table4 = cl_prt_temptable('axrg4404',g_sql) CLIPPED                    
  IF l_table4 = -1 THEN 
     CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add
     CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)    #FUN-B50018  add
     EXIT PROGRAM END IF 
...
#   tmp04 LIKE type_file.num20_6)             #FUN-B50018--------mark-------
   ...
   IF cl_null(tm.wc) THEN
      CALL axrg440_tm(0,0)             # Input print condition
   ELSE 
      CALL axrg440()                   # Read data and create out-file
   END IF
...
FUNCTION axrg440_tm(p_row,p_col)
   ...
   OPEN WINDOW axrg440_w AT p_row,p_col
        WITH FORM "axr/42f/axrg440"
################################################################################
...
WHILE TRUE
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axrg440_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_template CLIPPED,"'",
                         " '",g_rpt_name CLIPPED,"'" 
         CALL cl_cmdat('axrg440',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axrg440_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4) 
      EXIT PROGRAM
   END IF
...
FUNCTION axrg440()
   ...
   DEFINE l_ooz29   LIKE ooz_file.ooz29
    ...
    LET l_sql="SELECT ooa01,ooa02,ooa03,ooa032,ooa15,gem02,ooa13,ooaconf, ",
               ...
               "  FROM ooa_file LEFT OUTER JOIN gem_file ON ooa_file.ooa15=gem_file.gem01,", #FUN-CC0093 mark      #FUN-D10098  unmark
               #"  FROM ooa_file LEFT OUTER JOIN gem_file ON ooa_file.ooa15=gem_file.gem01 ",  #FUN-CC0093 add     #FUN-D10098  mark
               #"                LEFT OUTER JOIN gen_file ON ooa_file.ooa14=gen_file.gen01,",  #FUN-CC0093 add     #FUN-D10098  mark
               ...
               "   AND ",tm.wc CLIPPED,
               " ORDER BY ooa01 "
     PREPARE axrg440_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE axrg440_curs1 CURSOR FOR axrg440_prepare1

     FOREACH axrg440_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
###GENGRE###     CALL cl_prt_cs3('axrg440','axrg440',l_sql,g_str)
    CALL axrg440_grdata()    ###GENGRE###

END FUNCTION

###GENGRE###START
FUNCTION axrg440_grdata()
    DEFINE l_sql    STRING
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("axrg440")
        IF handler IS NOT NULL THEN
            START REPORT axrg440_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY ooa01,oob03"
            DECLARE axrg440_datacur1 CURSOR FROM l_sql
            FOREACH axrg440_datacur1 INTO sr1.*
                OUTPUT TO REPORT axrg440_rep(sr1.*)
            END FOREACH
            FINISH REPORT axrg440_rep
        END IF
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.ooa01
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        ...
                        " AND oao03 = 0 AND oao05='1'"
            START REPORT axrg440_subrep01
            DECLARE axrg440_repcur1 CURSOR FROM l_sql
            FOREACH axrg440_repcur1 INTO sr2.*
                OUTPUT TO REPORT axrg440_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT axrg440_subrep01
            #FUN-CC0093--mark--end
            #FUN-D10098--unmark--end--
            #FUN-B50018----add-----end-----------------
        ...
        ON EVERY ROW
            ...
            START REPORT axrg440_subrep02
            DECLARE axrg440_repcur2 CURSOR FROM l_sql
            FOREACH axrg440_repcur2 INTO sr3.*
                OUTPUT TO REPORT axrg440_subrep02(sr3.*)
            END FOREACH
            FINISH REPORT axrg440_subrep02
            ...
            START REPORT axrg440_subrep03
            DECLARE axrg440_repcur3 CURSOR FROM l_sql
            FOREACH axrg440_repcur3 INTO sr4.*
                OUTPUT TO REPORT axrg440_subrep03(sr4.*)
            END FOREACH
            FINISH REPORT axrg440_subrep03
            #FUN-CC0093--mark--end
            #FUN-D10098--unmark--end--
            #FUN-B50018----add-----end-----------------
        ...
        AFTER GROUP OF sr1.ooa01
           ...
           #START REPORT axrg440_subrep01
           #DECLARE axrg440_repcur1 CURSOR FROM l_sql
           #FOREACH axrg440_repcur1 INTO sr1.*
           #    OUTPUT TO REPORT axrg440_subrep01(sr1.*)
           #END FOREACH
           #FINISH REPORT axrg440_subrep01
           ...
           #START REPORT axrg440_subrep02               
           #DECLARE axrg440_repcur2 CURSOR FROM l_sql
           #FOREACH axrg440_repcur2 INTO sr1.*
           #    OUTPUT TO REPORT axrg440_subrep02(sr1.*)
           #END FOREACH
           #FINISH REPORT axrg440_subrep02  
           #
           #LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
           ...
           #START REPORT axrg440_subrep03
           #DECLARE axrg440_repcur3 CURSOR FROM l_sql
           #FOREACH axrg440_repcur3 INTO sr1.*
           #    OUTPUT TO REPORT axrg440_subrep03(sr1.*)
           #END FOREACH
           #FINISH REPORT axrg440_subrep03
           ...
           #            "ORDER BY oob04"
           #START REPORT axrg440_subrep04
           #DECLARE axrg440_repcur4 CURSOR FROM l_sql
           #FOREACH axrg440_repcur4 INTO sr1.*
           #    OUTPUT TO REPORT axrg440_subrep04(sr1.*)
           #END FOREACH
           #FINISH REPORT axrg440_subrep04
          #FUN-CC0093--add--end
          #FUN-D10098--mark--end--
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        ...
                        " ORDER BY oob03" 
            START REPORT axrg440_subrep05
            DECLARE axrg440_repcur5 CURSOR FROM l_sql
            FOREACH axrg440_repcur5 INTO sr1.*
                OUTPUT TO REPORT axrg440_subrep05(sr1.*)
            END FOREACH
            FINISH REPORT axrg440_subrep05

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table4 CLIPPED,
                        " WHERE oao01_3 = '",sr1.ooa01 CLIPPED,"' AND oao03_3=0 AND oao05_3='2'"
            START REPORT axrg440_subrep04
            DECLARE axrg440_repcur4 CURSOR FROM l_sql
            FOREACH axrg440_repcur4 INTO sr5.*
                OUTPUT TO REPORT axrg440_subrep04(sr5.*)
            END FOREACH
            FINISH REPORT axrg440_subrep04
           #FUN-CC0093--mark--end
           #FUN-D10098--unmark--end--
