...
MAIN
   ...
   LET l_table1 = cl_prt_temptable('axrg3011',g_sql) CLIPPED                      
   IF l_table1 = -1 THEN 
   ...
   LET l_table2 = cl_prt_temptable('axrg3012',g_sql) CLIPPED                      
   IF l_table2 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add   #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5||"|"||l_table6)   #FUN-B50018  add   #FUN-C10036   mark
      EXIT PROGRAM
   ...
   LET l_table3 = cl_prt_temptable('axrg3013',g_sql) CLIPPED                    
   IF l_table3 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add   #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5||"|"||l_table6)   #FUN-B50018  add   #FUN-C10036   mark
      EXIT PROGRAM 
   ...
   LET l_table4 = cl_prt_temptable('axrg3014',g_sql) CLIPPED                    
   IF l_table4 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add   #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5||"|"||l_table6)   #FUN-B50018  add   #FUN-C10036   mark 
      EXIT PROGRAM 
   ...
   LET l_table6 = cl_prt_temptable('axrg3016',g_sql) CLIPPED                    
   IF l_table6 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add   #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5||"|"||l_table6)   #FUN-B50018  add   #FUN-C10036   mark
      EXIT PROGRAM
   ...
   LET g_sql = "npq01.npq_file.npq01,",
               ...
               "npq07.npq_file.npq07,",
               "azi04.azi_file.azi04"     
   LET l_table5 = cl_prt_temptable('axrg3015',g_sql) CLIPPED
   IF l_table5 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add    #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5||"|"||l_table6)   #FUN-B50018  add   #FUN-C10036   mark 
      EXIT PROGRAM
 ...
 # SELECT azi04 INTO g_azi04 FROM azi_file WHERE azi01=g_aza.aza17   #No.CHI-6A0004
   ...
   END IF
    ...
    CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5||"|"||l_table6)
END MAIN
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axrg301',g_time,l_cmd) # Execute cmd at later time
      END IF
      CLOSE WINDOW g301_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0095
  ...
  #     LET l1_sql = l1_sql CLIPPED," AND oma00 = '",tm.f2[1,2],"'"    #FUN-C10036 mark
     ...
     #No.TQC-7A0013---Begin
     #display "prog:",g_prog
     #CALL cl_outnam('axrg301') RETURNING l_name
...
FUNCTION axrg301_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("axrg301")
        IF handler IS NOT NULL THEN
            START REPORT axrg301_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " ORDER BY oma01,omb03"      #FUN-B50018
          
            DECLARE axrg301_datacur1 CURSOR FROM l_sql
            FOREACH axrg301_datacur1 INTO sr1.*
                OUTPUT TO REPORT axrg301_rep(sr1.*)
            END FOREACH
            FINISH REPORT axrg301_rep
        END IF
...
REPORT axrg301_rep(sr1)
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.oma01
            ...
            START REPORT axrg301_subrep01
            DECLARE axrg301_repcur1 CURSOR FROM l_sql
            FOREACH axrg301_repcur1 INTO sr2.*
                OUTPUT TO REPORT axrg301_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT axrg301_subrep01
            #FUN-B50018----add-----end-----------------
        ...
        ON EVERY ROW
            ...
            START REPORT axrg301_subrep02
            DECLARE axrg301_repcur2 CURSOR FROM l_sql
            FOREACH axrg301_repcur2 INTO sr3.*
                OUTPUT TO REPORT axrg301_subrep02(sr3.*)
            END FOREACH
            FINISH REPORT axrg301_subrep02
            ...
            START REPORT axrg301_subrep03
            DECLARE axrg301_repcur3 CURSOR FROM l_sql
            FOREACH axrg301_repcur3 INTO sr4.*
                OUTPUT TO REPORT axrg301_subrep03(sr4.*)
            END FOREACH
            FINISH REPORT axrg301_subrep03
            #FUN-B50018----add-----end-----------------
        ...
        AFTER GROUP OF sr1.oma01
            ...
            START REPORT axrg301_subrep05
            DECLARE axrg301_repcur5 CURSOR FROM l_sql
            FOREACH axrg301_repcur5 INTO sr5.*
                OUTPUT TO REPORT  axrg301_subrep05(sr5.*)
            END FOREACH
            FINISH REPORT axrg301_subrep05

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table5 CLIPPED,
                        " WHERE npq01 = '",sr1.oma01 CLIPPED,"'"
            START REPORT axrg301_subrep04
            DECLARE axrg301_subrep04 CURSOR FROM l_sql
            FOREACH axrg301_subrep04 INTO sr6.*
                OUTPUT TO REPORT axrg301_subrep04 (sr6.*)
            END FOREACH
            FINISH REPORT axrg301_subrep04
            #FUN-B50018----add-----end-----------------        
