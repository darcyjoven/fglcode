...
# Modify.........: No:MOD-B50168 11/05/19 By Dido 抓取資料排序調整 
# Modify.........: No:FUN-B80072 11/08/08 By Lujh 模組程序撰寫規範修正
# Modify.........: No:MOD-B80247 11/08/23 By Dido 若 oma213 與 oea213 不同,則以 oea213 為主 
# Modify.........: No.CHI-B80052 11/09/09 By Polly 修正若二聯式(oma212='2')發票含稅(oma213='Y')時，未稅金額應與含稅金額相同，稅額為 0
# Modify.........: No.MOD-BB0201 11/11/18 By Polly 調整axrr300_prepare1 sql寫法，改用OUTER JOIN 
...
#              "sign_show.type_file.chr1,sign_str.type_file.chr1000"    #是否顯示簽核資料(Y/N)  #TQC-C10039  #TQC-C20049 Mark TQC-C10039
   ...
   LET l_table1= cl_prt_temptable('axrr3001',g_sql) CLIPPED  
   ...
   LET l_table2= cl_prt_temptable('axrr3002',g_sql) CLIPPED  
   ...
   CREATE TEMP TABLE r300_tmp
     (ome01   LIKE ome_file.ome01,
      ...
      oma212  LIKE oma_file.oma212,  #CHI-B80052 add
      oma213  LIKE oma_file.oma213)  #CHI-B80052 add
   CREATE unique index r300_tmp_01 on r300_temp(ome01,omb06);
...
FUNCTION axrr300_tm(p_row,p_col)
   ...
   OPEN WINDOW axrr300_w AT p_row,p_col WITH FORM "axr/42f/axrr300"
   ...
   WHILE TRUE
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('axrr300',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW axrr300_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
         EXIT PROGRAM
...
FUNCTION axrr300()
   ...
   DEFINE l_n       LIKE type_file.num5,    
        ...
        # l_per     LIKE oma_file.oma161,     #MOD-A50054 add
          ...
          l_oma212  LIKE oma_file.oma212,                    #CHI-B80052 add
          l_oma213  LIKE oma_file.oma213,      #含稅否       #MOD-890027 add
          l_oea213  LIKE oea_file.oea213,      #含稅否       #MOD-B80247
...
#   DEFINE l_img_blob     LIKE type_file.blob    #TQC-C10039  #TQC-C20049 Mark TQC-C10039
   ...
   DELETE FROM r300_tmp   #MOD-890122 add
...
#   LOCATE l_img_blob IN MEMORY    #TQC-C10039  #TQC-C20049 Mark TQC-C10039
   ...
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
#               "        ?,?,?,?,?, ?,?,?,?, ?,?,?,?)"  #FUN-8C0083 add 2?  #TQC-C10039 add 4?  #TQC-C20049 Mark TQC-C10039
              "        ?,?,?,?,?, ?,?,?,?)"  #FUN-8C0083 add 2?
   ...
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'axrr300'
...
#將FOREACH中對oma00,oma161,oma59,oma59x,oma59t的撈取，放在這個sql里，防止有多筆
            ...
            #"       oma00,oma161,ome59,ome59x,ome59t,oma01,oma213,",        #MOD-890027 add oma01,oma213 #MOD-B50168 #CHI-B80052 mark
             "       oma00,oma161,ome59,ome59x,ome59t,oma01,oma212,oma213,", #CHI-B80052 add oma212 
            ...
            #----------------------------------------------MOD-BB0201--------------------------------------end
            #" ORDER BY ome01,omb06,oma01"   #MOD-890027 add oma01 #MOD-B50168 mark
             " ORDER BY ome01,omb01,omb03"                         #MOD-B50168
 
   PREPARE axrr300_prepare1 FROM l_sql
   ...
   DECLARE axrr300_curs1 CURSOR FOR axrr300_prepare1
 
  #DISPLAY "prog:",g_prog  #MOD-B50168
 
   LET l_sum59  = 0   LET l_sum59x = 0   LET l_sum59t = 0   #add by hellen
   LET l_page   = 0      #MOD-B50168 mod 1 -> 0
    LET l_cntp   = 1                      #MOD-890122 add
   LET l_oma01_o = ' '   #MOD-890027 add
   LET l_ome01_o = ' '   #MOD-A50054 add
   FOREACH axrr300_curs1 INTO sr.*,l_oma00,l_oma161,l_oma59,l_oma59x,l_oma59t
                                  ,l_oma01,l_oma212,l_oma213,l_oma162,l_oma163,l_oma16   #MOD-890027 add  #MOD-A50054 add l_oma162,l_oma163    #No:FUN-A50103 #CHI-B80052 add oma212
      IF SQLCA.sqlcode != 0 THEN
         ...
         IF g_aza.aza26 = '2' THEN
            ...
            IF l1_n = l_n + 1 THEN
               LET l_sql = "SELECT omb06,-omb12,omb17,-omb18 ",
                           ...
                           "  FROM oma_file ",
                           " WHERE oma10 = '",sr.ome01,"')) ",
                          #" ORDER BY omb06 "       #MOD-B50168 mark
                           " ORDER BY omb01,omb03"  #MOD-B50168
               PREPARE r300_prepare_omb FROM l_sql
               IF SQLCA.sqlcode THEN
                  CALL cl_err('r300_prepare_omb:',SQLCA.sqlcode,1) RETURN
               END IF
               DECLARE axrr300_cur2 CURSOR FOR r300_prepare_omb
               FOREACH axrr300_cur2 INTO sr2.*
                  IF SQLCA.sqlcode THEN
                     CALL cl_err('foreach:',SQLCA.sqlcode,1)
                     EXIT FOREACH
                  END IF
     ...
     #LET sr.omb12 = (sr.omb12 * l_per) / 100  
      ...
      IF l_j = l_cnt THEN
        ...
        #LET sr.omb18x = l_oma59 - t_omb18x    #MOD-B50168 mark
         LET sr.omb18x = l_oma59x - t_omb18x   #MOD-B50168
         CALL cl_digcut(sr.omb18x,g_azi04) RETURNING sr.omb18x
        #LET sr.omb18t = l_oma59 - t_omb18t    #MOD-B50168 mark
         LET sr.omb18t = l_oma59t - t_omb18t   #MOD-B50168
     ...
     #MOD-A50054---add---end---
      ...
      INSERT INTO r300_tmp VALUES
         (sr.ome01, sr.ome042, sr.ome043,sr.ome044,sr.omb06,
          ...
          l_oma212,l_oma213)                         #CHI-B80052 add 
   ...
   DECLARE r300_tmp_cs CURSOR FOR
      SELECT * FROM r300_tmp ORDER BY ome01,omb01,omb03   #FUN-8C0083
   FOREACH r300_tmp_cs INTO sr1.*
      SELECT MAX(page) INTO l_maxpage FROM r300_tmp
...
#         "",l_img_blob,"N",""    #TQC-C10039 add "",l_img_blob,"N",""   "        ?,?,?,?,?, ?,?,?,?)"  #FUN-8C0083 add 2?
...
#   LET g_cr_apr_key_f = "ome01"             #報表主鍵欄位名稱  #TQC-C10039  #TQC-C20049 Mark TQC-C10039
   CALL cl_prt_cs3('axrr300','axrr300',g_sql,g_str)
