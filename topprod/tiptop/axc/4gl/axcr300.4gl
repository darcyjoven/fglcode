...
# Modify.........: No.MOD-970190 09/07/22 By mike 1.調整UPDATE r300_tmp的程式段，c5與c10兩個欄位update的值應該是sr.ccc23e才對，此程>
#                                                   所以只須調整ora檔即可。                                                         
...
MAIN
  ...
  #NO.MOD-720042 07/02/07 By TSD.Sideny --start--
   ...
   LET l_table = cl_prt_temptable('axcr300',g_sql) CLIPPED   # 產生Temp Table
...
#CREATE TEMP TABLE
   DROP TABLE r300_tmp
#No.FUN-680122 -- begin --
#     CREATE TEMP TABLE r300_tmp(pn VARCHAR(40),yy1 smallint,mm1 smallint,      #FUN-560011
...
#No.FUN-680122 -- end --
   CREATE UNIQUE INDEX r300_01 ON r300_tmp(pn);
   ...
   #        LET tm.mm2=g_argv5
   #        LET tm.x  =g_argv6
   #        CALL axcr300()            # Read data and create out-file
   #END IF
   IF cl_null(g_bgjob) OR g_bgjob = 'N' # If background job sw is off
      THEN CALL axcr300_tm(0,0)         # Input print condition
      ELSE CALL axcr300()               # Read data and create out-file
   END IF
   #TQC-610051-end
   DROP TABLE r300_tmp
...
FUNCTION axcr300_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   END IF
   OPEN WINDOW axcr300_w AT p_row,p_col
        WITH FORM "axc/42f/axcr300" 
################################################################################
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond('cccuser', 'cccgrup') #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axcr300_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690125 BY dxfwo 
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axcr300',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axcr300_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690125 BY dxfwo 
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0146
    ...
    #NO.MOD-720042 07/02/07 By TSD.Sideny --start--
    #CALL cl_outnam('axcr300') RETURNING l_name
    #START REPORT axcr300_rep TO l_name
    #LET g_pageno = 0
    #NO.MOD-720042 07/02/07 By TSD.Sideny ---end---
...
#--------------------------------------------------------------- 取 ccg_file
     ...
     PREPARE axcr300_prepare1 FROM l_sql
     DECLARE axcr300_curs1 CURSOR WITH HOLD FOR axcr300_prepare1
     FOREACH axcr300_curs1 INTO sr.*
       IF STATUS THEN CALL cl_err('foreach1:',STATUS,1) EXIT FOREACH END IF
       IF tm.yy1=sr.ccc02 AND tm.mm1=sr.ccc03 THEN
         UPDATE r300_tmp SET pn=sr.ccc01,yy1=sr.ccc02,mm1=sr.ccc03,ccc08=sr.ccc08,c1=sr.ccc23a,c2=sr.ccc23b,c3=sr.ccc23c,c4=sr.ccc23d,c5=sr.ccc23e,c51=sr.ccc23f,c52=sr.ccc23g,c53=sr.ccc23h   #No.FUN-7C0101 modify #MOD-970190 modify ora
           WHERE pn=sr.ccc01
          IF STATUS OR SQLCA.SQLERRD[3]=0 THEN
             INSERT INTO r300_tmp(pn,yy1,mm1,ccc08,c1,c2,c3,c4,c5,c51,c52,c53) VALUES(sr.*) #No.FUN-7C0101 modify
          END IF
       ELSE
         UPDATE r300_tmp SET pn=sr.ccc01,yy2=sr.ccc02,mm2=sr.ccc03,ccc08=sr.ccc08,c6=sr.ccc23a,c7=sr.ccc23b,c8=sr.ccc23c,c9=sr.ccc23d,c10=sr.ccc23e,c101=sr.ccc23f,c102=sr.ccc23g,c103=sr.ccc23h   #No.FUN-7C0101 modify #MOD-970190 modify ora
           WHERE pn=sr.ccc01
          IF STATUS OR SQLCA.SQLERRD[3]=0 THEN
             INSERT INTO r300_tmp(pn,yy2,mm2,ccc08,c6,c7,c8,c9,c10,c101,c102,c103) VALUES(sr.*) #No.FUN-7C0101 modify
          END IF
       END IF
     END FOREACH
 
     DECLARE r300_curs2 CURSOR WITH HOLD FOR SELECT * FROM r300_tmp
     FOREACH r300_curs2 INTO sr1.*
       IF STATUS THEN CALL cl_err('foreach1:',STATUS,1) EXIT FOREACH END IF
    ...
    ## **** 與 Crystal Reports 串聯段 - <<<< CALL cs3() >>>> CR11 **** ##
     ...
     ELSE
        LET g_str = " "
     END IF
     LET g_str = g_str,";",tm.yy1 USING '####',";",tm.mm1 USING '##'   #FUN-710080 modify
                      ,";",tm.yy2 USING '####',";",tm.mm2 USING '##'   #FUN-710080 modify 
                      ,";",tm.x,";",tm.type
#FUN-840016-start-
      IF tm.type MATCHES '[12]' THEN                                                                                   
        CALL cl_prt_cs3('axcr300','axcr300',l_sql,g_str)                                                                            
     #No.FUN-7C0101--start--                                                                                                        
     END IF                                                                                                                         
     IF tm.type MATCHES '[345]' THEN                                                                                                
        CALL cl_prt_cs3('axcr300','axcr300_1',l_sql,g_str)                                                                          
     END IF
#FUN-840016-end-
#     CALL cl_prt_cs3('axcr300','axcr300',l_sql,g_str)   #FUN-710080 modify
    #NO.MOD-720042 07/02/07 By TSD.Sideny ---end---
 
END FUNCTION
 
{
REPORT axcr300_rep(sr)
