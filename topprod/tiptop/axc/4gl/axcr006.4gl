...
# Modify.........: No:MOD-AB0084 10/11/10 By sabrina 在call axcr006()時應將sr1/sr2 array歸0 
...
MAIN
  ...
  # LET g_sql = "azf06.type_file.chr1,",     #CHI-B90043 mark  
   LET g_sql = "azf06.type_file.chr2,",      #CHI-B90043 add
   ...
   LET l_table = cl_prt_temptable('axcr006',g_sql)
...
FUNCTION axcr006_tm(p_row,p_col)
   DEFINE p_row,p_col LIKE type_file.num5,
          l_cmd       LIKE type_file.chr1000,
          g_za05      LIKE type_file.chr50
          
   IF p_row = 0 THEN LET p_row = 5 LET p_col = 15 END IF
   OPEN WINDOW axcr006_w AT p_row,p_col
        WITH FORM "axc/42f/axcr006" 
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         LET l_cmd = l_cmd CLIPPED,         
                         ...
                         " '",g_rep_clas CLIPPED,"'",            
                         " '",g_template CLIPPED,"'"             
         CALL cl_cmdat('axcr006',g_time,l_cmd)     
      END IF
      CLOSE WINDOW axcr006_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time            #FUN-B80056   ADD
      EXIT PROGRAM
...
FUNCTION axcr006()
   ...
   DEFINE l_sql3   STRING                #CHI-B90043 add
     ...
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'axcr006'
     ...
     DECLARE axcr006_curs1 CURSOR FOR 
      SELECT azf06,azf01,azf03 FROM azf_file 
       WHERE azf02 = 'G'     #成本分群碼
         AND azfacti = 'Y'
       ORDER BY azf06 DESC, azf01
     FOREACH axcr006_curs1 INTO l_azf06,l_azf01,l_azf03
     ...
     CALL r002_wip('9','ZZZZZ',g_msg)

     CALL cl_getmsg('axc-301',g_lang) RETURNING g_msg 
     EXECUTE insert_prep USING 'y','ZZZZZ','90','',g_msg,'','71',''	
     CALL cl_getmsg('axc-302',g_lang) RETURNING g_msg
     #-----------CHI-B80085 modify 
     EXECUTE insert_prep USING 'z','ZZZZZ','99','',g_msg,g_ccc62,'72',''
     EXECUTE insert_prep USING 'z','ZZZZZ','98','',g_msg,g_ccc62,'72',''   
     #-----------CHI-B80085 end
     #------------------CHI-B80085 add
     CALL cl_getmsg('axc-304',g_lang) RETURNING g_msg 
     EXECUTE insert_prep USING 'z','ZZZZZ','99','',g_msg,g_ccc82,'73',''  
     #------------------CHI-B80085 end
    ...
    #CHI-B90043---add---end---
     ...
     ELSE
        LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED," ORDER BY azf06"
     END IF
     #FUN-B20053-add-end--
     ...
     IF g_zz05 = 'Y' THEN
        ...
        RETURNING tm.wc
     END IF
     LET g_str = tm.wc,";",tm.azh01,";",tm.azh02,";",tm.yy,";",tm.m1,";",
                #tm.m2,";",tm.c,";",tm.b,";",g_azi04  #MOD-A50196 mark
                 #tm.m2,";",tm.c,";",tm.b,";",g_azi03  #MOD-A50196 #CHI-C30012 mark
                 tm.m2,";",tm.c,";",tm.b,";",g_ccz.ccz26  #CHI-C30012
     CALL cl_prt_cs3('axcr006','axcr006',g_sql,g_str)
     #No.FUN-BB0047--mark--Begin---
...
FUNCTION r002_stand(l_azf06,l_azf01,l_title)   #庫存成本計算模組
  ...
  DEFINE l_azf06       LIKE type_file.chr1
 #DEFINE l_azf01       LIKE type_file.chr4     #MOD-C10094 mark
  ...
  DEFINE sr            RECORD azf06  LIKE type_file.chr1,
...
# LET sr.azf01 = l_azf01,'1'      #MOD-B70063 mark
 ...
 LET l_sql = "SELECT SUM(tlf21*tlf907)",
              ...
              "   AND tlf902 NOT IN (SELECT jce02 FROM jce_file)"         #CHI-B80021 add
 #---------------CHI-B80085 end
  PREPARE r006_p1 FROM l_sql
  EXECUTE r006_p1 INTO l_amt3
 #end MOD-A50164 mod
 ...
 #   AND ima12=l_azf01
  LET l_sql = "SELECT SUM(tlf21*tlf907)",
              ...
              "   AND ima12='",l_azf01,"'",
              "   AND tlf902 NOT IN (SELECT jce02 FROM jce_file)"         #CHI-B80021 add
  PREPARE r006_p2 FROM l_sql
  EXECUTE r006_p2 INTO sr.amount
 #end MOD-A50164 mod
  IF cl_null(sr.amount) THEN LET sr.amount = 0 END IF
 ...
 #MOD-AB0223---mark---end---
  ...
  LET sr.order1 = '91'
  EXECUTE insert_prep USING 'y','ZZZZZ','91','',sr.title,sr.amount,sr.order1,''
  #---------------CHI-B80085 modify
  LET sr.order1 = '92'
  EXECUTE insert_prep USING 'y','ZZZZZ','92','',sr.title,l_amt3,sr.order1,''   
 #---------------CHI-B80085 end
  LET sr.order = '21'
  LET sr.order1 = '21'
