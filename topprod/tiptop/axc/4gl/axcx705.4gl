...
# Modify.........: No:FUN-D30060 13/03/22 By wangrr CR转为XtraGrid報表
# Modify.........: No:FUN-D40128 13/05/16 By wangrr 欄位"成本分群,入退庫單編號,廠商編號,傳票編號"增加開窗,來源碼改為下拉框
...
DEFINE   l_table1        STRING  #FUN-D30060
DEFINE   g_sql           STRING
DEFINE   g_str           STRING
...
MAIN
  ...
  #MOD-720042 TSD.miki-----------------------------------------------------------(S)
   ...
   LET l_table = cl_prt_temptable('axcx705',g_sql) CLIPPED   # 產生Temp Table
   IF l_table  = -1 THEN EXIT PROGRAM END IF                 # Temp Table產生
   LET l_table1 = cl_prt_temptable('axcx705_1',g_sql) CLIPPED   # 產生Temp Table #FUN-D30060
   IF l_table1  = -1 THEN EXIT PROGRAM END IF                   # Temp Table產生 #FUN-D30060
#  LET g_sql = "SELECT * FROM ds_report.",l_table CLIPPED,        #No.TQC-780054
  ...
  #CHI-B30087---mark---end---  
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      THEN CALL axcx705_tm(0,0)         # Input print condition
      ELSE CALL axcx705()               # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690125 BY dxfwo 
END MAIN
 
FUNCTION axcx705_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW axcx705_w AT p_row,p_col
        WITH FORM "axc/42f/axcx705" 
         ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
    
   CALL cl_ui_init()
 
   CALL cl_opmsg('p')
   CALL x705_set_visible() RETURNING l_cnt    #FUN-A70084
   ...
   LET tm.b ='N'
   LET tm.p1=g_plant
   CALL x705_set_entry_1()               
   CALL x705_set_no_entry_1()
  #CALL x705_set_comb()        #CHI-B30087 mark    
   #FUN-8B0026-End--#   
...
#No.FUN-570240 --start                                                          
     ON ACTION controlp                                                      
        ...
        IF INFIELD(ima12) THEN
           ...
           NEXT FIELD ima12
        END IF
        IF INFIELD(tlf905) THEN
           CALL cl_init_qry_var()
           LET g_qryparam.form = "q_tlf905"
           LET g_qryparam.state = "c"
           CALL cl_create_qry() RETURNING g_qryparam.multiret
           DISPLAY g_qryparam.multiret TO tlf905
           NEXT FIELD tlf905
        END IF
        IF INFIELD(tlf19) THEN
           CALL cl_init_qry_var()
           LET g_qryparam.form = "q_tlf19"
           LET g_qryparam.state = "c"
           CALL cl_create_qry() RETURNING g_qryparam.multiret
           DISPLAY g_qryparam.multiret TO tlf19
           NEXT FIELD tlf19
        END IF
        IF INFIELD(tlf65) THEN
           CALL cl_init_qry_var()
           LET g_qryparam.form = "q_tlf65"
           LET g_qryparam.state = "c"
           CALL cl_create_qry() RETURNING g_qryparam.multiret
           DISPLAY g_qryparam.multiret TO tlf65
           NEXT FIELD tlf65
        END IF
        #FUN-D40128--add--end                                                              
#No.FUN-570240 --end     
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond('imauser', 'imagrup') #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axcx705_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690125 BY dxfwo 
      EXIT PROGRAM
   ...
   WITHOUT DEFAULTS 
      ...
      AFTER FIELD p2
         IF NOT cl_null(tm.p2) THEN
            ...
            IF NOT x705_chklegal(m_legal[2],1) THEN
      ...
      AFTER FIELD p3
         IF NOT cl_null(tm.p3) THEN
            ...
            IF NOT x705_chklegal(m_legal[3],2) THEN
      ...
      AFTER FIELD p4
         IF NOT cl_null(tm.p4) THEN
            ...
            IF NOT x705_chklegal(m_legal[4],3) THEN
      ...
      AFTER FIELD p5
         IF NOT cl_null(tm.p5) THEN
            ...
            IF NOT x705_chklegal(m_legal[5],4) THEN
      ...
      AFTER FIELD p6
         IF NOT cl_null(tm.p6) THEN
            ...
            IF NOT x705_chklegal(m_legal[6],5) THEN
      ...
      AFTER FIELD p7
         IF NOT cl_null(tm.p7) THEN
            ...
            IF NOT x705_chklegal(m_legal[7],6) THEN
      ...
      AFTER FIELD p8
         IF NOT cl_null(tm.p8) THEN
            ...
            IF NOT x705_chklegal(m_legal[8],7) THEN
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axcx705',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axcx705_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690125 BY dxfwo 
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
DEFINE     l_tlf65    LIKE tlf_file.tlf65                 #No.FUN-8B0026                         
    #MOD-720042 TSD.miki---------------------------------------------------------(S)
     ...
     CALL cl_del_data(l_table1)  #FUN-D30060
#     DELETE FROM x
...
#                 "  AND tlf902 NOT IN (SELECT jce02 FROM ",l_dbs CLIPPED,"jce_file)",#FUN-A10098 #JIT除外   #FUN-8B0026 Add ",l_dbs CLIPPED,"
    ...
    #CALL cl_parse_qry_sql(l_sql,m_plant[l_i]) RETURNING l_sql  #FUN-A10098    #FUN-A70084 #TQC-BB0182 mark 
     PREPARE axcx705_prepare1 FROM l_sql
     ...
     END IF
     DECLARE axcx705_curs1 CURSOR FOR axcx705_prepare1
 
    #CALL cl_outnam('axcx705') RETURNING l_name
    #START REPORT axcx705_rep TO l_name   #MOD-720042 TSD.miki mark
 
     LET g_pageno = 0  
     FOREACH axcx705_curs1 INTO sr.*,l_pmc903,l_tlf905,l_tlf01,l_tlf65              #FUN-8B0026 Add l_pmc903,l_tlf905,l_tlf01,l_tlf65
        IF SQLCA.sqlcode != 0 THEN 
       ...
       #OUTPUT TO REPORT axcx705_rep(sr.*)   #MOD-720042 TSD.miki mark
        CALL axcx705_data(sr.*,m_plant[l_i],l_ima12,l_ima57,l_ima08,l_tlf905,l_tlf01,l_tlf65)   #MOD-720042 TSD.miki #FUN-8B0026 Add m_dbs[l_i],l_ima12,l_ima57,l_ima08,l_tlf905,l_tlf65    #FUN-A70084 m_dbs-->m_plant
     END FOREACH
   END FOR                                                                      #FUN-8B0026     
    #MOD-720042 TSD.miki---------------------------------------------------------(S)
    #FINISH REPORT axcx705_rep
...
###XtraGrid###     LET l_sql = "SELECT tlf19,pmc03,tlfccost,SUM(tot01) tot01,SUM(tot02) tot02,",     #No.FUN-7C0101
###XtraGrid###                 "       SUM(tot03) tot03,SUM(tot04) tot04,SUM(tot05) tot05,",
###XtraGrid###                 "       SUM(tot06) tot06,SUM(tot07) tot07,SUM(tot08) tot08,",
###XtraGrid###                 "       SUM(tot09) tot09,SUM(tot10) tot10,SUM(tot11) tot11,",
###XtraGrid###                 "       SUM(tot12) tot12,SUM(tot13) tot13,azi03,azi04,azi05,",
###XtraGrid###                #"       plant,ima12,ima57,ima08,tlf905,tlf01,tlf65",                  #FUN-8B0026  #CHI-B30087 mark
###XtraGrid###                 "       plant",                                         #CHI-B30087 add
###XtraGrid###                 "  FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
###XtraGrid###                #" GROUP BY tlf19,pmc03,tlfccost,azi03,azi04,azi05,plant,ima12,ima57,ima08,tlf905,tlf01,tlf65" #No.FUN-7C0101 #FUN-8B0026 Add plant,ima12,ima57,ima08,tlf905,tlf01,tlf65 #CHI-B30087 mark
###XtraGrid###                 " GROUP BY tlf19,pmc03,tlfccost,azi03,azi04,azi05,plant"   #CHI-B30087 add
    #end TQC-790019 mod
     #是否列印選擇條件
...
#FUN-D30060--add--str--
   LET g_sql="INSERT INTO ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
             " SELECT tlf19,pmc03,tlfccost,SUM(tot01) tot01,SUM(tot02) tot02,",
             ...
             "  GROUP BY tlf19,pmc03,tlfccost,azi03,azi04,azi05,plant",
             "  ORDER BY plant,tlf19,pmc03,tlfccost"
...
#FUN-D30060--add--end
     ...
     IF g_zz05 = 'Y' THEN
        ...
        LET g_str = tm.wc
     END IF
###XtraGrid###     LET g_str = g_str,";",tm.bdate,";",tm.edate,";",
###XtraGrid###                #tm.s[1,1],";",tm.s[2,2],";",                             #FUN-8B0026  #CHI-B30087 mark
###XtraGrid###                #tm.s[3,3],";",tm.t,";",tm.u,";",tm.b                     #FUN-8B0026  #CHI-B30087 mark
###XtraGrid###                tm.b       #CHI-B30087 add
     #No.FUN-7C0101--start--
     IF tm.type MATCHES '[12]' THEN
###XtraGrid###     CALL cl_prt_cs3('axcx705','axcx705_1',l_sql,g_str)   
        LET g_xgrid.visible_column = cl_xg_column_visible(g_xgrid.visible_column,"tlfccost","N")
     END IF
     IF tm.type MATCHES '[345]' THEN
###XtraGrid###     CALL cl_prt_cs3('axcx705','axcx705',l_sql,g_str)   #FUN-710080 modify
        LET g_xgrid.visible_column = cl_xg_column_visible(g_xgrid.visible_column,"tlfccost","Y")
     END IF
     #No.FUN-7C0101---end---
    #MOD-720042 TSD.miki---------------------------------------------------------(E)
    
    #FUN-D30060--add--str--
    LET g_xgrid.table = l_table1
    LET g_xgrid.order_field='tlf19'
    IF tm.b='Y' THEN
       LET g_xgrid.visible_column = cl_xg_column_visible(g_xgrid.visible_column,"plant","Y")
    ELSE
       LET g_xgrid.visible_column = cl_xg_column_visible(g_xgrid.visible_column,"plant","N")
    END IF
    LET g_xgrid.footerinfo1=cl_getmsg('axc-138',g_lang),':',tm.bdate,'-',tm.edate
...
#FUN-8B0026--Begin--#
FUNCTION x705_set_entry_1()
    CALL cl_set_comp_entry("p1,p2,p3,p4,p5,p6,p7,p8",TRUE)
END FUNCTION
FUNCTION x705_set_no_entry_1()
    IF tm.b = 'N' THEN
...
#MOD-720042 TSD.miki-------------------------------------------------------------(S)
FUNCTION axcx705_data(sr,l_azp01,t_ima12,t_ima57,t_ima08,t_tlf905,t_tlf01,t_tlf65)      #FUN-8B0026 Add l_azp01,t_ima12,t_ima57,t_ima08,t_tlf905,t_tlf01,t_tlf65
   DEFINE sr           RECORD 
                         tlf19  LIKE tlf_file.tlf19,   #廠商
                         pmc03  LIKE pmc_file.pmc03,   #廠商簡稱
                         tlf06  LIKE tlf_file.tlf06,   #異動日期
...
FUNCTION x705_chklegal(l_legal,n)
