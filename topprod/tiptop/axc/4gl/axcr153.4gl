...
#
# Pattern name...: axcr153.4gl
# Descriptions...: 凈變現價值取價明細表
# Date & Author..: No.FUN-930100 09/04/03 By jan
# Modify.........: NO.FUN-950058 09/05/18 By kim for 十號公報-將取單價的所有單據/項次,記錄於cmg_file,在axcr153列印出來
# Modify.........: NO.CHI-970034 09/07/24 By jan 報表打印增加cmg10/cmg11
# Modify.........: NO.TQC-980005 09/08/04 By jan 打印時，是否列印料號明細 不勾選時，程序出錯
# Modify.........: No.FUN-980030 09/08/31 By Hiko 加上GP5.2的相關設定
# Modify.........: No.FUN-9C0170 10/01/07 By jan 新增ctype欄位及相關處理
# Modify.........: No.FUN-A10098 10/01/20 By lutingting GP5.2跨DB處理 
# Modify.........: No.TQC-A50166 10/06/01 By Carrier FUN-9A0067追单
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD                         # Print condition RECORD
             #wc1     STRING,                # Where condition   #FUN-A10098
              ...
              b       LIKE type_file.chr1,   # 是否列印取價單據明細(有列印料號明細時)  #FUN-950058
  ...
  DEFINE l_table1    STRING  #FUN-950058
  DEFINE g_sql       STRING
  DEFINE g_str       STRING
...
MAIN
  ...
  #LET tm.a     = ARG_VAL(11)
  #LET tm.b     = ARG_VAL(12) #FUN-950058
  ...
  #FUN-A10098--mod--end 
   ...
   LET g_sql = "cma021.cma_file.cma021,",
               ...
               "cma271.cma_file.cma271,",
               "cma27.cma_file.cma27,",
               "cma28.cma_file.cma28,",
               "cma29.cma_file.cma29,",
               "cma26.cma_file.cma26,",
               "cme04.cme_file.cme04,",
               "cma25.cma_file.cma25,",
               "imz02.imz_file.imz02,",
               "oba02.oba_file.oba02,",              
               "cma08.cma_file.cma08,", #FUN-9C0170
               "l_desc.imz_file.imz02,",
               "l_cnt.type_file.num10"  #FUN-950058
               
   LET l_table = cl_prt_temptable('axcr153',g_sql) CLIPPED   # 產生Temp Table
   IF l_table = -1 THEN EXIT PROGRAM END IF                  # Temp Table產生
   
   #FUN-950058...................mark begin #移到下面去做
   ...
   #FUN-950058................begin
   LET g_sql="cmg01.cmg_file.cmg01,",
             "cmg02.cmg_file.cmg02,",
             "cmg03.cmg_file.cmg03,",
             "cmg04.cmg_file.cmg04,",
   ...
   LET l_table1 = cl_prt_temptable('axcr1531',g_sql) CLIPPED
   IF l_table1 = -1 THEN EXIT PROGRAM END IF
 
 
  #IF cl_null(tm.wc1)   #FUN-A10098
   IF cl_null(tm.wc2)   #FUN-A10098
      THEN CALL r153_tm(0,0)             # Input print condition
      ELSE CALL r153()                   # Read data and create out-file
   END IF
...
FUNCTION r153_tm(p_row,p_col)
   ...
   OPEN WINDOW r153_w AT p_row,p_col WITH FORM "axc/42f/axcr153"
        ATTRIBUTE (STYLE = g_win_style CLIPPED)
   CALL cl_ui_init()
 
   CALL cl_opmsg('p')
 
   CALL r153_set_visible() RETURNING l_cnt    #FUN-A70084
...
WHILE TRUE
 ...
 #FUN-A10098--mark--end 
   ...
   INPUT BY NAME tm.yy,tm.mm,tm.ctype,tm.a,tm.b,  #FUN-950058 add tm.b#FUN-9C0170 add ctype
                 tm.more
                 #FUN-A10098--add--str--
      ...
      AFTER FIELD plant_1
         IF NOT cl_null(tm.plant_1) THEN
            IF NOT r153_chkplant(tm.plant_1) THEN
      ...
      AFTER FIELD plant_2
             ...
             ELSE
                IF NOT cl_null(tm.plant_2) THEN
                   IF NOT r153_chkplant(tm.plant_2) THEN
             ...
             IF NOT cl_null(tm.plant_2) THEN
                IF NOT r153_chklegal(m_legal[2],1) THEN
                   CALL cl_err(tm.plant_2,g_errno,0)
                   NEXT FIELD plant_2
                END IF
             END IF
         ...
         AFTER FIELD plant_3
             ...
             ELSE
                IF NOT cl_null(tm.plant_3) THEN
                   IF NOT r153_chkplant(tm.plant_3) THEN
             ...
             #FUN-A70084--add--str--檢查所錄入PLANT在同一法人下
             IF NOT cl_null(tm.plant_3) THEN
                IF NOT r153_chklegal(m_legal[3],2) THEN
                   CALL cl_err(tm.plant_3,g_errno,0)
                   NEXT FIELD plant_3
                END IF
             END IF
      ...
      AFTER FIELD plant_4
             ...
             ELSE
                IF NOT cl_null(tm.plant_4) THEN
                   IF NOT r153_chkplant(tm.plant_4) THEN
             ...
             #FUN-A70084--add--str--檢查所錄入PLANT在同一法人下
             IF NOT cl_null(tm.plant_4) THEN
                IF NOT r153_chklegal(m_legal[4],3) THEN
                   CALL cl_err(tm.plant_4,g_errno,0)
                   NEXT FIELD plant_4
                END IF
             END IF
         ...
         AFTER FIELD plant_5
             ...
             ELSE
                IF NOT cl_null(tm.plant_5) THEN
                   IF NOT r153_chkplant(tm.plant_5) THEN
             ...
             #FUN-A70084--add--str--檢查所錄入PLANT在同一法人下
             IF NOT cl_null(tm.plant_5) THEN
                IF NOT r153_chklegal(m_legal[5],4) THEN
                   CALL cl_err(tm.plant_5,g_errno,0)
                   NEXT FIELD plant_5
                END IF
             END IF
      ...
      AFTER FIELD plant_6
             ...
             ELSE
                IF NOT cl_null(tm.plant_6) THEN
                   IF NOT r153_chkplant(tm.plant_6) THEN
             ...
             #FUN-A70084--add--str--檢查所錄入PLANT在同一法人下
             IF NOT cl_null(tm.plant_6) THEN
                IF NOT r153_chklegal(m_legal[6],5) THEN
                   CALL cl_err(tm.plant_6,g_errno,0)
                   NEXT FIELD plant_6
                END IF
             END IF
         ...
         AFTER FIELD plant_7
             ...
             ELSE
                IF NOT cl_null(tm.plant_7) THEN
                   IF NOT r153_chkplant(tm.plant_7) THEN
             ...
             #FUN-A70084--add--str--檢查所錄入PLANT在同一法人下
             IF NOT cl_null(tm.plant_7) THEN
                IF NOT r153_chklegal(m_legal[7],6) THEN
                   CALL cl_err(tm.plant_7,g_errno,0)
                   NEXT FIELD plant_7
                END IF
             END IF
      ...
      AFTER FIELD plant_8
             ...
             ELSE
                IF NOT cl_null(tm.plant_8) THEN
                   IF NOT r153_chkplant(tm.plant_8) THEN
             ...
             #FUN-A70084--add--str--檢查所錄入PLANT在同一法人下
             IF NOT cl_null(tm.plant_8) THEN
                IF NOT r153_chklegal(m_legal[8],7) THEN
                   CALL cl_err(tm.plant_8,g_errno,0)
                   NEXT FIELD plant_8
                END IF
             END IF
        ...
        #LET tm.wc1=cl_replace_str(tm.wc1, "'", "\"")   #FUN-A10098
         ...
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                        ...
                        #" '",tm.wc1 CLIPPED,"'" ,   #FUN-A10098
                         ...
                         " '",tm.mm CLIPPED,"'" ,
                         " '",tm.a CLIPPED,"'" ,
                         " '",tm.b CLIPPED,"'" ,   #FUN-950058
                         " '",g_rep_user CLIPPED,"'", 
                         " '",g_rep_clas CLIPPED,"'",
                         " '",g_template CLIPPED,"'", #FUN-9C0170
                        ...
                        ," '",l_plant[1] CLIPPED,"'",
                         ...
                         " '",l_plant[8] CLIPPED,"'"
                         #FUN-A10098--add--end
         CALL cl_cmdat('axcr153',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW r153_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      EXIT PROGRAM
...
FUNCTION r153()
   DEFINE l_name      LIKE type_file.chr20,        # External(Disk) file name
         ...
         #l_azp03     LIKE azp_file.azp03,        #資料庫代碼      #FUN-A10098
          ...
          sr          RECORD
            ...
            cma271    LIKE cma_file.cma271, 
            cma27     LIKE cma_file.cma27,
            cma28     LIKE cma_file.cma28, 
            cma29     LIKE cma_file.cma29, 
            cma26     LIKE cma_file.cma26,   
            cme04     LIKE cme_file.cme04,
          ...
          #FUN-950058................begin
          sr2         RECORD
            cmg01     LIKE cmg_file.cmg01,        #年度
            cmg02     LIKE cmg_file.cmg02,        #期別
            cmg03     LIKE cmg_file.cmg03,        #料號
            cmg04     LIKE cmg_file.cmg04,        #項次
            cmg05     LIKE cmg_file.cmg05,        #類別
            cmg06     LIKE cmg_file.cmg06,        #單據編碼
            cmg07     LIKE cmg_file.cmg07,        #單據項次
            cmg08     LIKE cmg_file.cmg08,        #價格
            cmg09     LIKE cmg_file.cmg09,        #單據日期
            cmg10     LIKE cmg_file.cmg10,        #數量 #CHI-970034
            cmg11     LIKE cmg_file.cmg11,        #單位 #CHI-970034
     ...
     DEFINE l_cnt      LIKE type_file.num10       #FUN-950058
     DEFINE l_i       LIKE type_file.num5        #No.FUN-A10098
 
     #FUN-950058...................begin
     LET g_sql = "INSERT INTO ",g_cr_db_str,l_table CLIPPED,
                 " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,? ,?,?,?)"   #FUN-950058 #FUN-9C0170
     PREPARE insert_prep FROM g_sql
     IF STATUS THEN
     ...
     CALL cl_del_data(l_table1)  #FUN-950058
    ...
    # CALL cl_used(g_prog,l_time,1) RETURNING l_time       #FUN-B80056   MARK
     ...
     IF tm.a = "Y" THEN
        LET l_name = 'axcr153'
     ELSE
        LET l_name = 'axcr153_1'
     END IF
     
    #FUN-A10098--mod--str--
    #LET l_sql = "SELECT azp01,azp03 FROM azp_file ",
    #            " WHERE ", tm.wc1 CLIPPED
    #PREPARE r153_pre1 FROM l_sql
    #IF SQLCA.sqlcode != 0 THEN
    #   CALL cl_err('r153_pre1:',SQLCA.sqlcode,1)
    #   EXIT PROGRAM
    #END IF
    #DECLARE r153_cs1 CURSOR FOR r153_pre1
    #FOREACH r153_cs1 INTO l_azp01,l_azp03
    #   IF SQLCA.SQLCODE THEN
    #      CALL cl_err('FOREACH r153_cs1:',SQLCA.SQLCODE,0)
    #      EXIT FOREACH
    #   END IF
    ...
    #FUN-A10098--mod--end
        LET l_sql = "SELECT cma021,cma022,cma31,'',cma24,cma01,cma271,",
                    "       cma27,cma28,cma29,cma26,cma32,cma25,imz02,oba02,", 
                    "       cma08,",   #FUN-9C0170
                   ...
                   #FUN-A10098--mod--end
                    ...
                    "   AND ", tm.wc2 CLIPPED
        CALL cl_replace_sqldb(l_sql) RETURNING l_sql  #FUN-950083
       #CALL cl_parse_qry_sql(l_sql,l_plant[l_i]) RETURNING l_sql   #FUN-A10098  #TQC-BB0182 mark
        PREPARE r153_pre2 FROM l_sql
        IF SQLCA.sqlcode != 0 THEN
           CALL cl_err('r153_pre2:',SQLCA.sqlcode,1)
          #EXIT FOREACH   #FUN-A10098
           EXIT FOR       #FUN-A10098
        END IF
        DECLARE r153_cs2 CURSOR FOR r153_pre2
        FOREACH r153_cs2 INTO sr.*,sr1.*
           IF SQLCA.sqlcode != 0 THEN
              CALL cl_err('FOREACH r153_cs2:',SQLCA.sqlcode,1)
              EXIT FOREACH
           END IF
          ...
          CASE sr.cma31
               ...
               WHEN '2'
                   ...
                   #CALL r153_get_azf03(l_dbs_n,sr1.ima09,'D') RETURNING sr1.desc        #FUN-A10098
                    CALL r153_get_azf03(l_plant[l_i],sr1.ima09,'D') RETURNING sr1.desc   #FUN-A10098
               WHEN '3'
                    LET sr.cme03=sr1.ima10
                   #CALL r153_get_azf03(l_dbs_n,sr1.ima10,'E') RETURNING sr1.desc        #FUN-A10098
                    CALL r153_get_azf03(l_plant[l_i],sr1.ima10,'E') RETURNING sr1.desc   #FUN-A10098
               WHEN '4'
                    LET sr.cme03=sr1.ima11
                   #CALL r153_get_azf03(l_dbs_n,sr1.ima11,'F') RETURNING sr1.desc        #FUN-A10098
                    CALL r153_get_azf03(l_plant[l_i],sr1.ima11,'F') RETURNING sr1.desc   #FUN-A10098
               WHEN '5'
                    LET sr.cme03=sr1.ima12
                   #CALL r153_get_azf03(l_dbs_n,sr1.ima12,'G') RETURNING sr1.desc        #FUN-A10098
                    CALL r153_get_azf03(l_plant[l_i],sr1.ima12,'G') RETURNING sr1.desc   #FUN-A10098
               WHEN '6'
                    LET sr.cme03=sr1.ima131
                    LET sr1.desc = sr.oba02
           END CASE
 
          #EXECUTE insert_prep USING sr.*,sr1.desc  #FUN-950058 移到下去做
           #FUN-950058................begin
           LET l_cnt=0
           IF (tm.b='Y') AND (sr.cma01 IS NOT NULL) THEN
             #LET l_dbs_n = l_azp03 CLIPPED, "."   #FUN-A10098
              LET l_sql = "SELECT cmg01,cmg02,cmg03,cmg04,cmg05,cmg06,cmg07,",
                          "       cmg08,cmg09,cmg10,cmg11,cmg11_fac ", #CHI-970034 add cmg10,cmg11,cmg11_fac
                         #"  FROM ", l_dbs_n CLIPPED, "cmg_file ",   #FUN-A10098
                          "  FROM ",cl_get_target_table(l_plant[l_i],'cmg_file'),
                          " WHERE cmg01= ", tm.yy,
                          "   AND cmg02= ", tm.mm,
                          "   AND cmg03='", sr.cma01 CLIPPED ,"'",
                          " ORDER BY cmg01,cmg02,cmg03,cmg04 "
              CALL cl_replace_sqldb(l_sql) RETURNING l_sql  #FUN-950083
             #CALL cl_parse_qry_sql(l_sql,l_plant[l_i]) RETURNING l_sql   #FUN-A10098  #TQC-BB0182 mark
              PREPARE r153_pre3 FROM l_sql
              IF SQLCA.sqlcode != 0 THEN
                 CALL cl_err('r153_pre3:',SQLCA.sqlcode,1)
                 EXIT FOREACH
              END IF
              DECLARE r153_cs3 CURSOR FOR r153_pre3
              FOREACH r153_cs3 INTO sr2.*
                 IF SQLCA.sqlcode != 0 THEN
                    CALL cl_err('FOREACH r153_cs3:',SQLCA.sqlcode,1)
                    EXIT FOREACH
                 END IF
                 LET l_cnt = l_cnt + 1
              ...
              END FOREACH
           END IF
           EXECUTE insert_prep USING sr.*,sr1.desc,l_cnt
           #FUN-950058................end
    END FOREACH
   ...
   #END FOREACH    #FUN-A10098
    ...
    IF tm.a = "Y" THEN   #TQC-980005
    LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table  CLIPPED,"|",  #FUN-950058
                "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED       #FUN-950058
    ELSE    #TQC-980005
       LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table  CLIPPED #TQC-980005
    END IF  #TQC-980005
   #是否列印選擇條件
    IF g_zz05 = 'Y' THEN
    ...
    LET g_str = g_str,";",tm.yy,";",tm.mm,";",tm.a,
                #";",g_azi03,";",g_azi04,";",g_azi05   #FUN-950058 #CHI-C30012
                ";",g_ccz.ccz26,";",g_ccz.ccz26,";",g_ccz.ccz26     #CHI-C30012
    CALL cl_prt_cs3('axcr153',l_name,l_sql,g_str)   
...
# 取得azf03
#====================================
#FUNCTION r153_get_azf03(p_dbs,p_azf01,p_azf02)   #FUN-A10098
FUNCTION r153_get_azf03(p_plant,p_azf01,p_azf02)   #FUN-A10098
  ...
  #CALL cl_parse_qry_sql(l_sql,p_plant) RETURNING l_sql   #FUN-A10098  #TQC-BB0182 mark
   PREPARE r153_azf_p1 FROM l_sql
   DECLARE r153_azf_cs1 CURSOR FOR r153_azf_p1
   OPEN r153_azf_cs1
   FETCH r153_azf_cs1 INTO l_azf03
   RETURN l_azf03
END FUNCTION
#FUN-930100
#FUN-A10098--add--str--
FUNCTION r153_chkplant(l_plant)
...
DEFINE l_azw05  LIKE azw_file.azw05
  ...
  IF l_cnt > 1 THEN
     CALL cl_set_comp_visible("gb18",FALSE)
  END IF
  RETURN l_cnt
END FUNCTION

FUNCTION r153_chklegal(l_legal,n)
