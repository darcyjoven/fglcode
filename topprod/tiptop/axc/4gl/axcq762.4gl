...
# Modify.........: No.MOD-E20136 14/02/27 By SunLM 根據oaz95判斷tlf920為Y或者N
# Modify.........: No.2015040230 15/09/15 By renjj 回收p_sser-流水号:2015040227
# Modify.........: No........... 17/01/11 By QianYuan 考虑发出商品不纳入成本计算的逻辑
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   LET tm.a    ='3'
   LET tm.b    ='3'
  #LET tm.c    ='Y'   #mark by lifang 190625
   LET tm.c    ='N'   #add by lifang 190625
   LET tm.d    ='N'
   LET g_pdate= g_today
   LET g_rlang= g_lang
   LET g_bgjob= 'N'
...
#                  "   ORDER BY tlf01,tlf905,tlf906 "  
      ...
      LET l_sql = "SELECT tlf905,tlf906,tlf01,ima02,ima021,tlf19,occ02,NVL(SUM(tlf10*tlf60*tlf907),0),NVL(SUM(tlfc21*tlf907),0),",
                  "        0 wsaleamt,0 l_wsale_tlf21,0 l_rate,1 salerat,1 raterat,occ04,'' gen02,'' gem01,'' gem02,",
                  "        '' oma08,'' oab01,occ03,'' warea_name,ima131,tlf036,NVL(occ37,'N'),",
                #  "       NVL(SUM(tlfc221*tlf907),0),NVL(SUM(tlfc222*tlf907),0),NVL(SUM(tlfc2231*tlf907),0),NVL(SUM(tlfc2232*tlf907),0),",   # mark by renjj 150915
                #  "       NVL(SUM(tlfc224*tlf907),0),NVL(SUM(tlfc2241*tlf907),0),NVL(SUM(tlfc2242*tlf907),0),NVL(SUM(tlfc2243*tlf907),0),tlf13,tlf66,NVL(azf08,'N'),' ' ogb01,0 ogb03,tlf907 ",      #FUN-CC0157 add ogb01,ogb03  #MOD-D90159 add tlf907  # mark by renjj 150915
                  "       NVL(SUM(tlfc221*tlf907),0),NVL(SUM(tlfc222*tlf907),0),NVL(SUM(tlfc2231*tlf907),0),",                       #add by renjj 150915
                  "       NVL(SUM(tlfc224*tlf907),0),NVL(SUM(tlfc2241*tlf907),0),NVL(SUM(tlfc2242*tlf907),0),NVL(SUM(tlfc2243*tlf907),0),NVL(SUM(tlfc2232*tlf907),0),tlf13,tlf66,NVL(azf08,'N'),' ' ogb01,0 ogb03,tlf907 ",      #FUN-CC0157 add ogb01,ogb03  #MOD-D90159 add tlf907 #add by renjj 150915
                  "  FROM ",cl_get_target_table(m_plant[l_i],'ima_file'),",",cl_get_target_table(m_plant[l_i],'tlf_file'),
                  "  LEFT OUTER JOIN ",cl_get_target_table(m_plant[l_i],'occ_file')," ON tlf19=occ01 ",
                  "  LEFT OUTER JOIN ",cl_get_target_table(m_plant[l_i],'azf_file')," ON tlf14=azf01 AND azf02='2' ",   
                  "  LEFT OUTER JOIN ",cl_get_target_table(m_plant[l_i],'tlfc_file')," ON tlfc01 = tlf01 ",
     ...
     # END IF
     # IF g_oaz.oaz92='N' THEN  #MOD-E20136 add begin----- 
...
#                  "      SET o.wsaleamt = o.wsaleamt+ x.omb16 ",      #No.TQC-D90012
      ...
      LET l_sql = "MERGE INTO axcq761_tmp o",
                 #"  USING( SELECT DISTINCT oma00,NVL(omb16,0) omb16,omb38,omb31,omb32 ",         #MOD-D90121 mark
                  ...
                  "           FROM ",cl_get_target_table(m_plant[l_i],'oma_file'),",", 
                                     cl_get_target_table(m_plant[l_i],'omb_file'),
                  "          WHERE oma01=omb01 AND (oma02 BETWEEN '",tm.bdate,"' AND '",tm.edate,"')",
             #    "                AND omaconf = 'Y' AND omavoid != 'Y' AND (oma00  LIKE '1%' OR omb38 <> '3') group by omb31,omb32 ) x ",  #mark by lifang 190416
                  "                AND omaconf = 'Y' AND omavoid != 'Y' AND (oma00  LIKE '1%' AND omb38 <> '3') group by omb31,omb32 ) x ",  #add by lifang 190416 
                 #"  ON (o.tlf905=x.omb31 AND o.tlf906=x.omb32 )",  
                 #"  ON (o.ogb01 =x.omb31 AND o.ogb03 =x.omb32 )",  #FUN-CC0157 mod                           #MOD-D90121 mark
      ...
      #  LET l_sql = "MERGE INTO axcq761_tmp o",
      #              "  USING(SELECT ogb01,ogb03,ogb04,(ogb14*oga24) ogb14_1 ",   #MOD-D90121 add 
      ...
      #            #  "                AND ogb01 NOT IN (SELECT omb31 from omb_file LEFT JOIN oma_file ON oma01=omb01 where omaconf = 'Y' AND omavoid != 'Y' AND omb38='2' ) ) x ",
      #              "          AND (omb31 IS NuLL OR omb31=' ') AND (omb32 IS NULL OR omb32 = ' ') ) x ",
      #              "  ON (o.ogb01 =x.ogb01 AND o.ogb03=x.ogb03 )", 
      #              " WHEN MATCHED ",
      #              " THEN ",
      #              "   UPDATE ",
      #              "      SET o.wsaleamt = o.wsaleamt+ x.ogb14_1 ",
      #              "    WHERE o.ogb01 =x.ogb01 AND o.ogb03=x.ogb03  "
      #   PREPARE q761_wsaleamt05 FROM l_sql
      #   EXECUTE q761_wsaleamt05
         ...
         LET l_sql = "MERGE INTO axcq761_tmp o",
                   ...
                   # "                AND ohb01 NOT IN (SELECT omb31 from omb_file LEFT JOIN oma_file ON oma01=omb01 where omaconf = 'Y' AND omavoid != 'Y' AND omb38='3' ) ) x ",
                    ...
                    " THEN ",
                    "   UPDATE ",
                    "      SET o.wsaleamt = o.wsaleamt + x.ohb14_1 ",
      ...
      #--add by lifang 190625 begin#
       LET l_sql = "MERGE INTO axcq761_tmp o",
                  "  USING(SELECT DISTINCT ogb01,ogb03,ogb04,(ogb14*oga24) ogb14_1 ",   #MOD-D90121 add
                  "           FROM ",cl_get_target_table(m_plant[l_i],'ogb_file'),",",
                                   cl_get_target_table(m_plant[l_i],'oga_file'),",",
                                  "( select omb31,omb32,sum(omb12) omb12 FROM ",cl_get_target_table(m_plant[l_i],'omb_file'),",",cl_get_target_table(m_plant[l_i],'oma_file'),
                                  "  WHERE oma01 = omb01 and oma02 between '",tm.bdate,"' AND '",tm.edate,"' AND omaconf='Y' AND omavoid='N'  group by omb31,omb32) a",                             
                  "          WHERE ogb01=oga01 AND ogapost='Y' ",
                  "                AND oga02 between '",tm.bdate,"' AND '",tm.edate,"'",
                  "                AND ogb01 = a.omb31 and ogb03 = a.omb32 and ogb12 <> a.omb12 ) x",
                  "  ON (o.ogb01 =x.ogb01 AND o.ogb03=x.ogb03 )", 
                  " WHEN MATCHED ",
                  " THEN ",
                  "   UPDATE ",
                  "      SET o.wsaleamt = x.ogb14_1 " 
      PREPARE q761_wsaleamt07_1 FROM l_sql
      EXECUTE q761_wsaleamt07_1

      #--add by lifang 190625 end#
       
      #--add by maoyy20190711 begin#
      LET l_sql = "MERGE INTO axcq761_tmp o",
                  "  USING(SELECT DISTINCT ohb01,ohb03,ohb04,(ohb14*oha24) ohb14_1 ",   #MOD-D90121 add 
                  "           FROM ",cl_get_target_table(m_plant[l_i],'ohb_file'),",",
              
                                         cl_get_target_table(m_plant[l_i],'oha_file'),
                  "          WHERE ohb01=oha01 AND ohapost='Y' ",
                  "                AND oha02 >= '",tm.bdate,"'",
                  "                AND oha02 <= '",tm.edate,"' ) x ",
                  ...
                  "      SET o.wsaleamt = o.wsaleamt+ x.ohb14_1 ",
                  "    WHERE o.wsaleamt = 0 "
      PREPARE q761_wsaleamt08 FROM l_sql
      EXECUTE q761_wsaleamt08

      #---add by maoyy20190711 end#
      #add by xujw200609--begin--
      LET l_sql = "MERGE INTO axcq761_tmp o",
                  "  USING(SELECT DISTINCT ohb01,ohb03,ohb04,(ohb14*oha24) ohb14_1 ",   #MOD-D90121 add 
                  "           FROM ",cl_get_target_table(m_plant[l_i],'ohb_file'),",",        
                                         cl_get_target_table(m_plant[l_i],'oha_file'),",",
                  ...
                  "          WHERE ohb01=oha01 AND ohapost='Y' ",
                  "                AND oha02 >= '",tm.bdate,"'",
                  "                AND oha02 <= '",tm.edate,"'  AND ohb01 = a.omb31 and ohb03 = a.omb32 and ohb12 <> a.omb12 ) x ",
                  "  ON (o.ogb01 =x.ohb01 AND o.ogb03=x.ohb03 )", 
                  " WHEN MATCHED ",
                  " THEN ",
                  "   UPDATE ",
                  "      SET o.wsaleamt = x.ohb14_1 "
      PREPARE q761_wsaleamt09 FROM l_sql
      EXECUTE q761_wsaleamt09
      #add by xujw200609--end--
