...
# Modify.........: No.FUN-D10098 13/02/01 By lujh 新增gfag600
...
MAIN
   ...
   LET l_table = cl_prt_temptable('gfag600',g_sql) CLIPPED
   IF l_table = -1 THEN 
...
FUNCTION g600_tm(p_row,p_col)
   ...
   OPEN WINDOW g600_w AT p_row,p_col WITH FORM "gfa/42f/gfag600"
    ...
    #FUN-CC0093--add--end
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('gfag600',g_time,l_cmd)
         END IF
         CLOSE WINDOW g600_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         CALL cl_gre_drop_temptable(l_table)
...
#       l_time          LIKE type_file.chr8	    #No.FUN-6A0069
     ...
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'gfag600'
#No.FUN-710083 --begin
...
#No.FUN-710083 --begin
   # CALL cl_prt_cs1('gfag600',l_sql,'')             #TQC-730088
   # CALL cl_prt_cs1('gfag600','gfag600',l_sql,'')   #FUN-B40097 mark
#    CALL cl_outnam('gfag600') RETURNING l_name
...
#      OUTPUT TO REPORT g600_rep(sr.*)
    ...
    LET g_cr_apr_key_f = "fca01"          # No.FUN-C40020 add
     CALL gfag600_grdata()                                       #FUN-B40097 add 
...
FUNCTION gfag600_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("gfag600")
        IF handler IS NOT NULL THEN
            START REPORT gfag600_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY fca01,fca02"
                 
            DECLARE gfag600_datacur1 CURSOR FROM l_sql
            FOREACH gfag600_datacur1 INTO sr1.*
                OUTPUT TO REPORT gfag600_rep(sr1.*)
            END FOREACH
            FINISH REPORT gfag600_rep
        END IF
