...
# Modify.........: No.FUN-D10098 13/02/01 By lujh 新增gfag103
...
#No.FUN-710083 --begin
   ...
   LET l_table = cl_prt_temptable('gfag103',g_sql) CLIPPED
   IF l_table = -1 THEN
...
#No.FUN-710083 --end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'	# If background job sw is off
      THEN CALL g103_tm(0,0)		# Input print condition
      ELSE CALL gfag103()		# Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
   CALL cl_gre_drop_temptable(l_table)
END MAIN
 
FUNCTION g103_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW g103_w AT p_row,p_col WITH FORM "gfa/42f/gfag103"
 ...
 END CONSTRUCT
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW g103_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         CALL cl_gre_drop_temptable(l_table)
         EXIT PROGRAM
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('gfag103',g_time,l_cmd)      # Execute cmd at later time
         END IF
         CLOSE WINDOW g103_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690113
         CALL cl_gre_drop_temptable(l_table)
  ...
  # LOCATE l_img_blob        IN MEMORY            # No.FUN-C40020 add
   ...
   LOCATE l_img_blob        IN MEMORY            # No.FUN-C40020 add
     ...
     LET l_sql = "SELECT fau01,fau02,fau04,fav02,fav03,fav031,",
                ...
                #"  FROM fau_file,fav_file, OUTER faj_file,OUTER fag_file ", #FUN-C50008 mark--
                 "  FROM fau_file,fav_file LEFT OUTER JOIN faj_file ON (fav03=faj02 AND fav031=faj022) LEFT OUTER JOIN fag_file ON (fav06=fag01) ", #FUN-C50008 add LEFT JOIN
                 " WHERE fau01 = fav01 ",
                 "   AND fauconf <> 'X' ",
                #"   AND fav_file.fav03=faj_file.faj02 ",   #FUN-C50008 mark--
                #"   AND fav_file.fav031=faj_file.faj022 ", #FUN-C50008 mark--
                # "   AND fav_file.fav06 = fag_file.fag01 ",#FUN-C50008 mark--
                 "   AND ",tm.wc CLIPPED
     ...
     PREPARE g103_prepare1 FROM l_sql
     ...
     END IF
     DECLARE g103_cs1 CURSOR FOR g103_prepare1
#No.FUN-710083 --begin
#    SELECT * INTO g_zo.* FROM zo_file WHERE zo01=g_rlang
#    CALL cl_outnam('gfag103') RETURNING l_name
#    START REPORT g103_rep TO l_name
#    LET g_pageno = 0
#No.FUN-710083 --end
     FOREACH g103_cs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#No.FUN-710083 --begin
       ...
       EXECUTE insert_prep USING sr.*,l_gem02,
                                 "",l_img_blob,"N",""    #FUN-940042 #No.FUN-C40020
#      OUTPUT TO REPORT g103_rep(sr.*)
#No.FUN-710083 --end
     END FOREACH
#No.FUN-710083 --begin
...
###GENGRE###     LET g_str = tm.wc,";",g_zz05
   ...
   # CALL cl_prt_cs3('gfag103',l_sql,g_str)
     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     ###FUN-940042 START ###
     LET g_cr_table = l_table                 #主報表的temp table名稱
     LET g_cr_gcx01 = "afai060"               #單別維護程式
     LET g_cr_apr_key_f = "fau01"             #報表主鍵欄位名稱，用"|"隔開 
###GENGRE###     LET l_sql_2 = "SELECT DISTINCT fau01 FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     ...
     END FOREACH
     ###FUN-940042 END ###
###GENGRE###     CALL cl_prt_cs3('gfag103','gfag103',l_sql,g_str)
    CALL gfag103_grdata()    ###GENGRE###
 
#    FINISH REPORT g103_rep
...
FUNCTION gfag103_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("gfag103")
        IF handler IS NOT NULL THEN
            START REPORT gfag103_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY fau01,fav02"
          
            DECLARE gfag103_datacur1 CURSOR FROM l_sql
            FOREACH gfag103_datacur1 INTO sr1.*
                OUTPUT TO REPORT gfag103_rep(sr1.*)
            END FOREACH
            FINISH REPORT gfag103_rep
        END IF
...
REPORT gfag103_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_fau04_gem02   STRING
    
    ORDER EXTERNAL BY sr1.fau01
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.fau01
            #FUN-B40092------add------str
            LET l_fau04_gem02 = sr1.fau04,' ',sr1.gem02
            PRINTX l_fau04_gem02
            #FUN-B40092------add------end
            LET l_lineno = 0
