...
#
# Pattern name...: almt6103.4gl
# Descriptions...: 儲值卡支票付款維護作業
...
MAIN
   ...
   CALL cl_used(g_prog,g_time,1) RETURNING g_time    
    ...
    OPEN WINDOW t6103_w WITH FORM "alm/42f/almt6103" 
         ATTRIBUTE (STYLE = g_win_style CLIPPED) 
    CALL cl_ui_init()    
 
    IF NOT cl_null(g_argv1) THEN
       LET g_wc2 = "rxy01 = '",g_argv1,"' "                   
    ELSE
    	 LET g_wc2 = " 1=1"
    END IF  
    SELECT lps07,lps13,lpsplant,lps09 INTO g_lps07,g_lps13,g_lpsplant,g_lps09 FROM lps_file
     WHERE lps01=g_argv1 
    ...
    CALL t6103_b_fill(g_wc2)
    
    CALL t6103_menu()
    CLOSE WINDOW t6103_w    
         ...
         WHEN "controlg"
          ...
          WHEN "related_document" 
            IF cl_chk_act_auth() AND l_ac != 0 THEN 
               IF g_rxy[l_ac].rxy06 IS NOT NULL THEN
                  LET g_doc.column1 = "rxy06"
                  LET g_doc.value1 = g_rxy[l_ac].rxy06
                  CALL cl_doc()
               END IF
            END IF
...
DEFINE l_nmtacti   LIKE nmt_file.nmtacti #No.FUN-C30038
   ...
   LET g_forupd_sql = "SELECT rxy02,rxy06,rxy09,rxy10,rxy11,rxy05", 
                      "  FROM rxy_file WHERE rxy01= '",g_argv1,"' and ",
                      "  rxy02 = ? and rxy00='20' ",
#                      "and rxyplant='",g_lpsplant,"' ",
                      "  FOR UPDATE "
   LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
   DECLARE t6103_bcl CURSOR FROM g_forupd_sql      # LOCK CURSOR
   ...
   INPUT ARRAY g_rxy WITHOUT DEFAULTS FROM s_rxy.*
     ATTRIBUTE (COUNT=g_rec_b,MAXCOUNT=g_max_rec,UNBUFFERED,
       ...
       BEFORE ROW
          ...
          IF g_rec_b>=l_ac THEN 
             ...
             CALL t6103_set_entry(p_cmd)                                         
             CALL t6103_set_no_entry(p_cmd) 
             CALL t6103_set_entry_b()
             LET g_before_input_done = TRUE   
             LET g_rxy_t.* = g_rxy[l_ac].*  
             OPEN t6103_bcl USING g_rxy_t.rxy02
             IF STATUS THEN
                CALL cl_err("OPEN t6103_bcl:", STATUS, 1)
                LET l_lock_sw = "Y"
             ELSE
                FETCH t6103_bcl INTO g_rxy[l_ac].* 
     ...
     #    INSERT INTO rxy_file(rxy00,rxy01,rxy02,rxy03,rxy04,rxy05,rxy06,rxy09,rxy10,rxy11,rxyplant,rxy21,rxy22,rxy17)
     #                  VALUES('20',g_argv1,g_rxy[l_ac].rxy02,'03','1',g_rxy[l_ac].rxy05,g_rxy[l_ac].rxy06,
     #                         g_rxy[l_ac].rxy09,g_rxy[l_ac].rxy10,g_rxy[l_ac].rxy11,g_lpsplant,g_today,
     #                         to_char(sysdate,'HH24:MI'),g_rxy[l_ac].rxy17)
          INSERT INTO rxy_file(rxy00,rxy01,rxy02,rxy03,rxy04,rxy05,rxy06,rxy09,rxy10,rxy11,rxyplant,rxy21,rxy22,rxy17,rxylegal)
                        VALUES('20',g_argv1,g_rxy[l_ac].rxy02,'03','1',g_rxy[l_ac].rxy05,g_rxy[l_ac].rxy06,
                               g_rxy[l_ac].rxy09,g_rxy[l_ac].rxy10,g_rxy[l_ac].rxy11,g_lpsplant,g_today,
                              #to_char(sysdate,'HH24:MI'),g_rxy[l_ac].rxy17,g_legal)    #TQC-A70053houlia rxylagel
                               l_time,g_rxy[l_ac].rxy17,g_legal) #No.FUN-A80008
 
          IF SQLCA.sqlcode THEN
             CALL cl_err3("ins","rxy_file",g_rxy[l_ac].rxy02,"",SQLCA.sqlcode,"","",1) 
             CANCEL INSERT
          ELSE
             LET g_lps13=g_lps13+g_rxy[l_ac].rxy05
             LET g_amt=g_lps07-g_lps13
             IF g_amt<0 THEN 
                LET g_amt=0
             END IF 
       ...
       BEFORE FIELD rxy02   
         IF cl_null(g_rxy[l_ac].rxy02) OR cl_null(g_rxy_t.rxy02) THEN 
            SELECT max(rxy02)+1 INTO g_rxy[l_ac].rxy02 FROM rxy_file      
             WHERE rxy00='20' AND rxy01= g_argv1   
       ...
       AFTER FIELD rxy09
          IF NOT cl_null(g_rxy[l_ac].rxy09) THEN
             IF g_rxy[l_ac].rxy09<0 THEN 
                CALL cl_err('','alm-198',1)
                LET g_rxy[l_ac].rxy09=g_rxy_t.rxy09
                NEXT FIELD rxy09
             ELSE
                IF g_rxy[l_ac].rxy09+g_lps13>g_lps07 THEN
#                   CALL cl_err('','alm-209',1)
#                   NEXT FIELD rxy09
                   LET g_rxy[l_ac].rxy17=g_rxy[l_ac].rxy09+g_lps13-g_lps07
                   LET g_rxy[l_ac].rxy05 = g_rxy[l_ac].rxy09-g_rxy[l_ac].rxy17
                   DISPLAY BY NAME g_rxy[l_ac].rxy05
                   DISPLAY BY NAME g_rxy[l_ac].rxy17
                ELSE
...
#No.FUN-C30038---END---
       ...
       BEFORE DELETE                            #是否取消單身
          IF (g_rxy_t.rxy02 IS NOT NULL) THEN
             ...
             LET g_doc.column1 = "rxy06"               #No.FUN-9B0098 10/02/24
             LET g_doc.value1 = g_rxy[l_ac].rxy06      #No.FUN-9B0098 10/02/24
             ...
             IF l_lock_sw = "Y" THEN 
                ...
                CANCEL DELETE 
             END IF 
             DELETE FROM rxy_file WHERE rxy01 = g_argv1
       ...
       ON ROW CHANGE
          ...
          IF l_lock_sw="Y" THEN
             ...
             LET g_rxy[l_ac].* = g_rxy_t.*
          ELSE
             UPDATE rxy_file SET rxy06=g_rxy[l_ac].rxy06,
                                 rxy09=g_rxy[l_ac].rxy09,
                                 rxy10=g_rxy[l_ac].rxy10,
                                 rxy13=g_rxy[l_ac].rxy11,
...
#               " AND rxyplant='",g_lpsplant,"' ", 
               " ORDER BY rxy02"
    PREPARE t6103_pb FROM g_sql
    DECLARE rxy_curs CURSOR FOR t6103_pb
...
FUNCTION t6103_set_entry(p_cmd)                                                  
...
FUNCTION t6103_set_no_entry(p_cmd)                                               
  DEFINE p_cmd   LIKE type_file.chr1                               
...
FUNCTION t6103_set_entry_b()
     CALL cl_set_comp_entry("rxy02,rxy06,rxy09,rxy10,rxy11",TRUE) 
END FUNCTION 
 
FUNCTION t6103_set_no_entry_b()
     CALL cl_set_comp_entry("rxy02,rxy06,rxy09,rxy10,rxy11,rxy05",FALSE) 
END FUNCTION                                           
