...
MAIN
   ...
   LET l_table = cl_prt_temptable('almr131',g_sql) CLIPPED
...
FUNCTION r131_tm(p_row,p_col)
   ...
   OPEN WINDOW almr131_w AT p_row,p_col WITH FORM "alm/42f/almr131" 
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW almr131_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,               #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('almr131',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW almr131_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
DEFINE sr    RECORD 
    ...
    PREPARE almr131_prepare1 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       EXIT PROGRAM
    END IF
    DECLARE almr131_curs1 CURSOR FOR almr131_prepare1

    FOREACH almr131_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
    ...
    IF g_zz05 = 'Y' THEN
       CALL cl_wcchp(tm.wc,'lmdstore,lmd03,lmd04,lmd14,lmd01,lmd07') RETURNING tm.wc
       LET g_str = tm.wc
   ...
   CALL cl_prt_cs1('almr131','almr131',l_sql,g_str) 
END FUNCTION 
