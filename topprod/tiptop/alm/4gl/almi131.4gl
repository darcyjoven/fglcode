...
# Modify.........: No.FUN-B80141 11/08/24 By baogc 將原來程序名almi130更新為almi131,并做相應的修改

# Modify.........: No:FUN-C10039 12/02/02 by Hiko 整批修改資料歸屬設定
# Modify.........: No:TQC-C30111 12/03/08 By yangxf lme_file 已不再使用,因此mark掉与lme_file 有关的代码
...
MAIN
#  DEFINE cb ui.ComboBox   #FUN-B80141 MARK
   ...
   DECLARE i131_cl CURSOR FROM g_forupd_sql                 # LOCK CURSOR
 
   OPEN WINDOW i131_w WITH FORM "alm/42f/almi131"
     ATTRIBUTE (STYLE = g_win_style CLIPPED) 
 
   CALL cl_ui_init()   
 
#  LET cb = ui.ComboBox.forname("lmd10") #FUN-B80141 MARK 
...
FUNCTION i131_curs()
   ...
   CONSTRUCT BY NAME g_wc ON  
                    #lmd01,lmdstore,lmdlegal,lmd03,lmd04,lmd05,lmd06,lmd07,                    #FUN-B80141 MARK
                     lmd01,lmd16,lmdstore,lmdlegal,lmd03,lmd04,lmd14,lmd06,lmd15,lmd05,lmd07,  #FUN-B80141 ADD
#                    lmd08,lmd09,lmd10,lmd11,lmd12,lmd13,lmduser,lmdgrup,     #FUN-A80148 mark
   ...
   DECLARE i131_cs                                # SCROLL CURSOR
      SCROLL CURSOR WITH HOLD FOR i131_prepare
...
#本程序只供查詢,其他FUNCTION&ACTION都Mark
#       ON ACTION insert
...
#DEFINE   l_lme04    LIKE lme_file.lme04  #TQC-C30111 mark
DEFINE   l_count    LIKE type_file.num5 
   ...
   INPUT BY NAME   g_lmd.lmd01,g_lmd.lmdstore,g_lmd.lmd03,g_lmd.lmd04, g_lmd.lmdoriu,g_lmd.lmdorig,
      ...
      AFTER FIELD lmd01
          IF NOT cl_null(g_lmd.lmd01) THEN
            IF p_cmd = 'u' THEN
            ELSE          	         
               CALL i131_check_lmd01(g_lmd.lmd01) 
               IF g_success = 'N' THEN                                                             
...
FUNCTION i131_fetch(p_icb)
 DEFINE p_icb LIKE type_file.chr1 
 
    CASE p_icb
        WHEN 'N' FETCH NEXT     i131_cs INTO g_lmd.lmd01
        WHEN 'P' FETCH PREVIOUS i131_cs INTO g_lmd.lmd01
        WHEN 'F' FETCH FIRST    i131_cs INTO g_lmd.lmd01
        WHEN 'L' FETCH LAST     i131_cs INTO g_lmd.lmd01
        WHEN '/'
            IF (NOT g_no_ask) THEN
               ...
               END IF
            END IF
            FETCH ABSOLUTE g_jump i131_cs INTO g_lmd.lmd01
            LET g_no_ask = FALSE
    END CASE
...
#FUN-B80141 Mark End -----
#FUN-B80141 Add Begin ---
    DISPLAY BY NAME g_lmd.lmd01,g_lmd.lmd16,g_lmd.lmdstore,g_lmd.lmdlegal,g_lmd.lmd03,
                    g_lmd.lmd04,g_lmd.lmd14,g_lmd.lmd06,g_lmd.lmd15,g_lmd.lmd05,
                    g_lmd.lmd07,g_lmd.lmd10,g_lmd.lmd11,g_lmd.lmd12,g_lmd.lmd13,
                    g_lmd.lmduser,g_lmd.lmdgrup,g_lmd.lmdoriu,g_lmd.lmdorig,
                    g_lmd.lmdmodu,g_lmd.lmddate,g_lmd.lmdacti,g_lmd.lmdcrat
#FUN-B80141 Add End -----
    CALL i131_pic()
    CALL cl_show_fld_cont()   
...
#   CALL i131_lmd06()    
   CALL i131_rtz13() 
   CALL i131_lmd14()    #FUN-B80141 ADD
END FUNCTION
...
DEFINE l_lmy04   LIKE lmy_file.lmy04
   ...
   SELECT lmy04 INTO l_lmy04 
     FROM lmy_file 
    WHERE lmy01 = g_lmd.lmd03 AND lmy02 = g_lmd.lmd04 AND lmy03 = g_lmd.lmd14 AND lmystore = g_lmd.lmdstore
...
DEFINE  p_w   LIKE type_file.chr1
    ...
    OPEN i131_cl USING g_lmd.lmd01
    IF STATUS THEN
...
FUNCTION i131_r()
   ...
   END IF 
    ...
    OPEN i131_cl USING g_lmd.lmd01
    IF STATUS THEN
       CALL cl_err("OPEN i131_cl:",STATUS,0)
       CLOSE i131_cl
       ROLLBACK WORK
       RETURN
    END IF
...
DEFINE l_count   LIKE type_file.num5 
    ...
    ELSE
       ...
       #FUN-C30027---end
    END IF
    #LET g_lmd.lmd01 = l_oldno  #FUN-C30027
    CALL i131_show()
END FUNCTION
...
FUNCTION i131_check_lmd01(p_cmd)
