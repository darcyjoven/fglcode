...
# Modify.........: No.FUN-B80060 11/08/05 By fanbj FOR UPDATE 下一行調用cl_forupd_sql 函式
# Modify.........: No.FUN-910088 11/12/21 By chenjing 增加數量欄位小數取位
# Modify.........: No.TQC-C30070 12/03/05 By pauline 新增出貨單時一併新增ogi_file&ogh_file 
# Modify.........: No:FUN-C50047 12/05/14 By pauline 寫入實際交易稅別明細檔調整
# Modify.........: No:FUN-C50097 12/06/13 By SunLM  對非空字段進行判斷ogb50,51,52
# Modify.........: No:MOD-C70031 12/07/11 By Elise ogi04,ogi05抓不到值時給預設值
...
# Modify.........: No.FUN-CB0087 12/12/20 By xujing 倉庫單據理由碼改善  xianghui:ogb_file
...
FUNCTION t605_ins_oga(p_no)
 ...
 DEFINE li_result   LIKE type_file.num5
    ...
    CALL t605_sel_rye('1') RETURNING g_oga.oga01
    ...
    LET g_prog = 'axmt620'
    CALL t605_ins_ogb()   
   #LET g_prog = 'almt605'  #MOD-C80258 mark
  ...
  #FUN-C50047 add
   CALL t605_ins_ogh(g_oga.oga01)  #新增實際交易稅別檔
   IF g_success = 'N' THEN
      RETURN
   END IF
  #FUN-C50047 add
    ...
    IF g_success = 'Y' THEN
       CALL t605_oga_post(g_oga.oga01,g_lrl.lrlplant)
    END IF
    RETURN g_oga.oga01
END FUNCTION

#產生出貨單身
FUNCTION t605_ins_ogb()   
 ...
 DEFINE l_ima908    LIKE ima_file.ima908
   ...
   FOREACH oeb_cs INTO l_lrg.*
      ...
      CALL s_get_coststore(g_lrl.lrlplant,l_ogb.ogb04) RETURNING l_ogb.ogb09    #FUN-C90049 add     
     ...
     #CALL cl_digcut(l_ogb.ogb14t,t_azi04)RETURNING l_ogb.ogb14t
      ...
      # #CALL s_inv_shrt_by_warehouse(g_sma.sma894[2,2],l_ogb.ogb09) RETURNING g_sma894  #FUN-D30024 mark
      #  IF g_sma894 = 'N' THEN
         INITIALIZE g_imd23 TO NULL
         CALL s_inv_shrt_by_warehouse(l_ogb.ogb09,g_lrl.lrlplant) RETURNING g_imd23  #TQC-D40078 g_lrl.lrlplant
      ...
      #FUN-D30024 ---------End-----------
         ...
         ELSE
            IF g_sma.sma892[3,3] = 'Y' THEN
               ...
               END IF
            END IF
            CALL s_add_img(l_lrg.lrg02,l_ogb.ogb09,l_ogb.ogb091,l_ogb.ogb092,
      ...
      SELECT obk11 INTO l_ogb.ogb19
       ...
       WHERE obk01 = l_ogb.ogb04
         ...
         AND obk05 = g_oga.oga23  #TQC-C30070 add
     ...
     #LET l_ogb.ogb19   = l_ima24              #檢驗否obk11
      ...
      IF g_aza.aza115 = 'Y' THEN 
         CALL s_reason_code(l_ogb.ogb01,l_ogb.ogb31,'',l_ogb.ogb04,
                            l_ogb.ogb09,g_oga.oga14,g_oga.oga15) RETURNING l_ogb.ogb1001
      ...
      CALL t605_ins_ogi(l_ogb.ogb01,l_ogb.ogb03,l_ogb.ogb04)  #TQC-C30070 add  新增單身稅別明細
     #CALL t605_ins_ogh(l_ogb.ogb01,l_ogb.ogb03,l_ogb.ogb04)  #TQC-C30070 add  新增單身實際交易稅額  #FUN-C50047 mark 
...
#出貨單過帳
FUNCTION t605_oga_post(p_oga01,p_plant)
...
FUNCTION t605_s1()
   ...
   CALL cl_replace_sqldb(g_sql) RETURNING g_sql          
   CALL cl_parse_qry_sql(g_sql,l_plant) RETURNING g_sql
   PREPARE t605_s2_pl FROM g_sql
   DECLARE t605_s2_c CURSOR FOR t605_s2_pl
   FOREACH t605_s2_c INTO g_ogb.*
      IF STATUS THEN
      ...
      CALL t605_chk_avl_stk(g_ogb.ogb16)
      CALL t605_update(g_ogb.ogb09,g_ogb.ogb091,g_ogb.ogb092,
...
FUNCTION t605_update(p_ware,p_loca,p_lot,p_qty,p_uom,p_factor,p_qty2,p_flag,p_type)
   ...
   #CALL cl_replace_sqldb(g_forupd_sql) RETURNING g_forupd_sql         #FUN-B80060  mark
   CALL cl_forupd_sql(g_forupd_sql) RETURNING g_forupd_sql             #FUN-B80060  add
   ...
   IF g_success='Y' AND p_type = '-' THEN
      CALL t605_tlf(p_ware,p_loca,p_lot,g_ima25,p_qty,l_qty,p_uom,p_factor,p_flag)
   END IF

END FUNCTION

FUNCTION t605_tlf(p_ware,p_loca,p_lot,p_unit,p_qty,p_img10,p_uom,p_factor,p_flag)
   DEFINE p_ware     LIKE ogb_file.ogb09        ##倉庫
   DEFINE p_loca     LIKE ogb_file.ogb091       ##儲位
   DEFINE p_lot      LIKE ogb_file.ogb092       ##批號
   ...
   IF g_ima906 = '2' THEN  #子母單位
      IF NOT cl_null(g_ogb.ogb913) THEN
         ...
         IF g_flag = 1 THEN
          ...
          #FUN-D30024 ----------Begin--------
          # INITIALIZE g_sma894 TO NULL
          # CALL s_inv_shrt_by_warehouse(g_sma.sma894[2,2],g_ogb.ogb09) RETURNING g_sma894  #FUN-D30024 mark
          # IF g_sma894 = 'N' OR g_sma894 IS NULL THEN
            INITIALIZE g_imd23 TO NULL
            CALL s_inv_shrt_by_warehouse(g_ogb.ogb09,l_azp03) RETURNING g_imd23                    #FUN-D30024 #TQC-D40078 l_azp03
            IF g_imd23 = 'N' OR g_imd23 IS NULL THEN
          #FUN-D30024 ----------End----------
               LET g_showmsg = g_ogb.ogb04,'/',g_ogb.ogb09,'/',g_ogb.ogb091,'/',g_ogb.ogb092
               CALL s_errmsg('img01,img02,img03,img04',g_showmsg,'sel imgg:','axm-244',1)
            ...
            ELSE
               IF g_sma.sma892[3,3] = 'Y' THEN
                  ...
                  END IF
               END IF
               CALL s_add_imgg(g_ogb.ogb04,g_ogb.ogb09,
                               g_ogb.ogb091,g_ogb.ogb092,
                               g_ogb.ogb913,g_ogb.ogb914,
                               g_oga.oga01,g_ogb.ogb03,0)
                     RETURNING g_flag
               IF g_flag = 1 THEN
                  LET g_success = 'N'
                  RETURN
           ...
           #FUN-C80107 modify end-----------------------------------------
         END IF
         CALL t605_upd_imgg('1',g_ogb.ogb04,g_ogb.ogb09,g_ogb.ogb091,g_ogb.ogb092,
                             g_ogb.ogb913,g_ogb.ogb914,g_ogb.ogb915,-1,'2')
         IF g_success='N' THEN RETURN END IF
         IF NOT cl_null(g_ogb.ogb915) THEN
            CALL t605_tlff(g_ogb.ogb09,g_ogb.ogb091,g_ogb.ogb092,g_ima25,
      ...
      IF NOT cl_null(g_ogb.ogb913) THEN
         ...
         IF g_flag = 1 THEN
          ...
          #FUN-D30024 -----------End----------
           ...
           #FUN-C80107 modify end-----------------------------------------
         END IF
         CALL t605_upd_imgg('2',g_ogb.ogb04,g_ogb.ogb09,g_ogb.ogb091,g_ogb.ogb092,
                             g_ogb.ogb913,g_ogb.ogb914,g_ogb.ogb915,-1,'2')
         IF g_success='N' THEN RETURN END IF
         IF NOT cl_null(g_ogb.ogb915) THEN 
            CALL t605_tlff(g_ogb.ogb09,g_ogb.ogb091,g_ogb.ogb092,g_ima25,
...
FUNCTION t605_tlff(p_ware,p_loca,p_lot,p_unit,p_qty,p_img10,p_uom,p_factor,
...
FUNCTION t605_du_2(p_item,p_ware,p_loc,p_lot,p_unit2,p_fac2,p_qty2,p_unit1,p_fac1,p_qty1,p_flag)
  ...
  IF g_ima906 = '3' THEN
     IF NOT cl_null(p_unit2) THEN
        CALL t605_upd_imgg('2',p_item,p_ware,p_loc,p_lot,
                           p_unit2,p_fac2,p_qty2,+1,'2')
        IF g_success='N' THEN RETURN END IF
     END IF
     IF p_flag = '2' THEN
...
FUNCTION t605_upd_imgg(p_imgg00,p_imgg01,p_imgg02,p_imgg03,p_imgg04,
...
FUNCTION t605_chk_avl_stk(p_qty2)
   DEFINE p_qty2    LIKE ogc_file.ogc16    ##銷售數量(img 單位)
   ...
   #--訂單備置為'N',須check(庫存量ima262-sum(備置量oeb12-oeb24))>出貨量
    IF NOT cl_null(g_ogb.ogb31) AND NOT cl_null(g_ogb.ogb32) THEN  
       ...
       #  CALL s_inv_shrt_by_warehouse(g_sma.sma894[2,2],g_ogb.ogb09) RETURNING g_sma894            #FUN-C80107  #FUN-D30024
       ...
       #  IF l_qoh < p_qty2 AND g_sma894 = 'N' THEN  #量不足時,Fail                                 #FUN-C80107
          INITIALIZE g_imd23 TO NULL  
          CALL s_inv_shrt_by_warehouse(g_ogb.ogb09,l_plant) RETURNING g_imd23                              #FUN-D30024   #TQC-D40078 l_plant
...
FUNCTION t605_upd_oeb(p_type)                  #更新訂單已出貨量
...
FUNCTION t605_upd_oea()                 #更新訂單出貨金額
...
#过账还原
FUNCTION t605_s_2(p_oga01,l_plant)
   ...
   FOREACH ogb2_s1_c INTO g_ogb.*
      ...
      CALL t605_du_2(g_ogb.ogb04,g_ogb.ogb09,g_ogb.ogb091,g_ogb.ogb092,
...
FUNCTION t605_u_tlf()#------------------------------------------ Update tlf_file
   ...
   LET g_sql = "SELECT * FROM ",cl_get_target_table(l_plant,'tlf_file'),
    ...
    CALL cl_replace_sqldb(g_sql) RETURNING g_sql
    CALL cl_parse_qry_sql(g_sql,l_plant) RETURNING g_sql  
    DECLARE t605_u_tlf_c CURSOR FROM g_sql
    LET l_i = 0 
    CALL la_tlf.clear()
    FOREACH t605_u_tlf_c INTO g_tlf.*
       LET l_i = l_i + 1
...
FUNCTION t605_tlff_2()#------------------------------------------ Update tlf_file
...
FUNCTION t605_ins_ina(p_no)
 ...
 DEFINE l_inbi       RECORD LIKE inbi_file.*  #FUN-B70074 add
   ...
   CALL t605_sel_rye('2') RETURNING g_ina.ina01
   ...
   FOREACH lrg_curs2 INTO l_lrg.*
      ...
      CALL s_get_coststore(g_plant,l_inb.inb04) RETURNING l_inb.inb05     #FUN-C90049 add
...
FUNCTION t370_update(p_ware,p_loca,p_lot,p_qty,p_uom,p_factor)
  ...
  DEFINE l_msg     STRING
  DEFINE l_n       LIKE type_file.num5  #FUN-C80107 add 121024
    ...
    SELECT COUNT(*) INTO l_n FROM img_file
     WHERE img01= g_inb.inb04 AND img02= p_ware
       AND img03= p_loca AND img04= p_lot
    IF l_n = 0 THEN
    #FUN-D30024 ----------Begin-----------
      #INITIALIZE g_sma894 TO NULL
      #CALL s_inv_shrt_by_warehouse(g_sma.sma894[1,1],p_ware) RETURNING g_sma894   #FUN-D30024
      #IF g_sma894 = 'Y' THEN
       INITIALIZE g_imd23 TO NULL
       CALL s_inv_shrt_by_warehouse(p_ware,g_plant) RETURNING g_imd23                     #FUN-D30024 #TQC-D40078 g_plant
       IF g_imd23 = 'Y' THEN
    #FUN-D30024 ----------End-------------
          IF g_sma.sma892[3,3] = 'Y' THEN
             IF NOT cl_confirm('mfg1401') THEN
                CALL s_errmsg('img01',g_inb.inb04,'sel img fail:',STATUS,1)
                LET g_success = 'N'
                RETURN
             END IF
          END IF
          CALL s_add_img(g_inb.inb04,p_ware,p_loca,p_lot,
                         g_ina.ina01,g_inb.inb03,g_ina.ina02)
          IF g_errno='N' THEN
             CALL s_errmsg('img01',g_inb.inb04,'add img fail:',STATUS,1)
             LET g_success = 'N'
             RETURN
          END IF
       END IF
...
FUNCTION t605_sel_rye(p_cmd)
 DEFINE p_cmd       LIKE type_file.chr1
 DEFINE l_oga01     LIKE oga_file.oga01
 DEFINE l_ina01     LIKE ina_file.ina01
 DEFINE li_result   LIKE type_file.num5

   OPEN WINDOW t6051_w WITH FORM "alm/42f/almt6051"
...
FUNCTION t605_ins_ogi(p_ogi01,p_ogi02,p_ogb04)
...
DEFINE l_rtz06             LIKE rtz_file.rtz06  #MOD-C70031 add   

   SELECT rtz04,rtz06 INTO l_rtz04,l_rtz06      #MOD-C70031 add rtz06
     FROM rtz_file 
    WHERE rtz01 = g_plant
  ...
  #MOD-C70031---S---
   IF cl_null(l_ogi.ogi04) THEN
      SELECT occ41 INTO l_ogi.ogi04
        FROM occ_file
       WHERE occ01 = l_rtz06 
   END IF                   
  #MOD-C70031---E---
...
#FUNCTION t605_ins_ogh(p_ogh01,p_ogh02,p_ogb04)  #FUN-C50047 mark
FUNCTION t605_ins_ogh(p_ogh01)  #FUN-C50047  add
...
#FUN-C50047 add END
  ...
  #LET l_ogh.ogh01 = p_ogh01
  #LET l_ogh.ogh02 = p_ogh02
  ...
  #FUN-C50047 add START
   ...
   FOREACH t620_sel_ogi_cs INTO l_ogi04,l_ogi05,l_ogi06,l_ogi07,l_ogi08,l_ogi08t,l_ogi09
      IF cl_null(l_ogi06) THEN LET l_ogi06 = 0 END IF
      IF cl_null(l_ogi08) THEN LET l_ogi08 = 0 END IF
      IF cl_null(l_ogi08t) THEN LET l_ogi08t = 0 END IF
      IF cl_null(l_ogi09) THEN LET l_ogi09 = 0 END IF
      LET l_ogh.ogh01 = p_ogh01
   ...
   #FUN-C50047add END
     ...
     #FUN-C50047 mark END
      ...
      LET l_ogh.oghmodu = g_user
      LET l_ogh.oghuser = g_user
      LET l_ogh.oghoriu = g_user
