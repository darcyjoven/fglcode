...
# Date & Author..: No:FUN-B90121 11/09/26 By  yangxf
# Modify.........: No:FUN-C10039 12/02/02 by Hiko 整批修改資料歸屬設定
# Modify.........: No:TQC-C20578 12/03/01 By yangxf 添加lip17查询/新增开窗
# Modify.........: No:TQC-C30239 12/03/20 By fanbj 修改時帶出輸入人員名稱
# Modify.........: No.CHI-C30107 12/06/11 By yuhuabao  整批修改將確認的詢問窗口放到chk段的前面
# Modify.........: No.FUN-CA0081 12/10/18 By xumm 比例单身增加 上限和下限栏位
...
DEFINE g_lip           RECORD LIKE lip_file.*,
       ...
       g_liq1       DYNAMIC ARRAY OF RECORD
           ...
           liq081      LIKE liq_file.liq081,   #FUN-CA0081 add
           liq082      LIKE liq_file.liq082,   #FUN-CA0081 add
...
DEFINE  l_where         LIKE    type_file.chr1000
            ...
            ON ACTION controlp
               CASE
                  ...
                  WHEN INFIELD(lip11)
                     ...
                     NEXT FIELD lip11
                  #TQC-C20578 add begin --------
                  WHEN INFIELD(lip17)
                     CALL cl_init_qry_var()
                     LET g_qryparam.state = 'c'
                     LET g_qryparam.form ="q_lip17"
                     CALL cl_create_qry() RETURNING g_qryparam.multiret
                     DISPLAY g_qryparam.multiret TO lip17
                     NEXT FIELD lip17
                  #TQC-C20578 add end -----------
      ...
      DIALOG ATTRIBUTES(UNBUFFERED)
       ...
       CONSTRUCT g_wc2 ON liq02_1,liq03_1,liq04_1,
                          liq05_1,liq07,liq081_1,liq082_1,liq071,liq072,liq073,liq074,liq09_1   #FUN-CA0081 add liq081,liq082
            FROM s_liq1[1].liq02_1,s_liq1[1].liq03_1,
                 s_liq1[1].liq04_1,s_liq1[1].liq05_1,
                 s_liq1[1].liq07,s_liq1[1].liq081_1,s_liq1[1].liq082_1,s_liq1[1].liq071,        #FUN-CA0081 add s_liq1[1].liq081,s_liq1[1].liq082
...
DEFINE
   ...
   INPUT BY NAME   g_lip.lip01,g_lip.lip02,g_lip.lip03,
      ...
      AFTER FIELD lip17
         IF NOT cl_null(g_lip.lip17) THEN
            CALL i365_lip17(p_cmd) 
            IF NOT cl_null(g_errno) THEN
               CALL cl_err('',g_errno,0)
               LET g_lip.lip17 = g_lip_t.lip17
               DISPLAY BY NAME g_lip.lip17
               NEXT FIELD lip17
            END IF 
         END IF 
      #TQC-C30239--end add--------------------------------------------         
...
#TQC-C20578 add begin --------
            WHEN INFIELD(lip17)
               ...
               CALL cl_create_qry() RETURNING g_lip.lip17
               DISPLAY g_lip.lip17 TO lip17
               CALL i365_desc()
               NEXT FIELD lip17
#TQC-C20578 add end ----------
         END CASE    
...
DEFINE l_n          LIKE type_file.num5,
       l_n1         LIKE type_file.num5,  #FUN-CA0081 add
       ...
       p_cmd        LIKE type_file.chr1
   LET g_errno =''
   LET l_n1 = 0 #FUN-CA0081 add
   LET l_n = 0  #FUN-CA0081 add
   IF p_cmd = 'u' THEN
      #FUN-CA0081-----add---str
      IF g_lip.lip04 = '2' THEN
         SELECT COUNT(*) INTO l_n1 FROM liq_file
          WHERE liq01 = g_lip.lip01
            AND liq03 = l_liq03
            AND liq02 <> l_liq02
            AND liq04 = p_stardate
            AND liq05 = p_enddate
      END IF
      IF l_n1 = 0 THEN
      #FUN-CA0081-----add---end
...
DEFINE
    ...
    IF g_lip.lipconf='S' THEN
       ...
       RETURN
    END IF
    LET g_forupd_sql = "SELECT liq02,liq03,liq04,liq05,liq07,liq081,liq082,liq071,liq072,liq073,liq074,liq09",   #FUN-CA0081 add liq081,liq082
                       "  FROM liq_file",
                       "  WHERE liq01=? AND liq02=? ",
                       "  FOR UPDATE "
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    ...
    INPUT ARRAY g_liq1 WITHOUT DEFAULTS FROM s_liq1.*
        ...
        BEFORE ROW
           ...
           IF g_rec_b1 >= l_ac THEN
              ...
              ELSE
                 ...
                 END IF
              END IF
              CALL i365_set_required(g_liq1[l_ac].liq04,g_liq1[l_ac].liq05,g_liq1_t.liq02,g_liq1[l_ac].liq081,g_liq1[l_ac].liq082,p_cmd)  #FUN-CA0081 add
              CALL cl_show_fld_cont()
           END IF
        ...
        AFTER INSERT
           ...
           END IF 
           INSERT INTO liq_file(liq01,liq02,liq03,liq04,liq05,liq07,liq081,liq082,       #FUN-CA0081 add liq081,liq082              
           ...
           VALUES(g_lip.lip01,g_liq1[l_ac].liq02,
                  ...
                  g_liq1[l_ac].liq081,g_liq1[l_ac].liq082,                               #FUN-CA0081 add
      ...
      AFTER FIELD liq04_1 
           IF NOT cl_null(g_liq1[l_ac].liq04) THEN
              ...
              IF NOT cl_null(g_liq1[l_ac].liq05) THEN
                 ...
                 #FUN-CA0081---add----str
                 IF cl_null(g_liq1[l_ac].liq081) AND cl_null(g_liq1[l_ac].liq082)  THEN
                    CALL i365_chk_liq04(g_liq1[l_ac].liq04,g_liq1[l_ac].liq05,g_liq1_t.liq02,p_cmd)
                    IF NOT cl_null(g_errno) THEN
                       CALL cl_err('',g_errno,0)
                       NEXT FIELD liq04_1
                    END IF
                 END IF
                 IF NOT cl_null(g_liq1[l_ac].liq081) AND NOT cl_null(g_liq1[l_ac].liq082)  THEN
                    CALL i365_chk_liq081(g_liq1[l_ac].liq04,g_liq1[l_ac].liq05,g_liq1_t.liq02,g_liq1[l_ac].liq081,g_liq1[l_ac].liq082,p_cmd)
                    IF NOT cl_null(g_errno) THEN
                       CALL cl_err('',g_errno,0)
                       NEXT FIELD liq04_1
                    END IF   
      ...
      #FUN-CA0081-----add---str
      AFTER FIELD liq081_1
         IF NOT cl_null(g_liq1[l_ac].liq081) THEN
            IF g_liq1[l_ac].liq081 < 0 THEN
               CALL cl_err('','alm1089',0)
               LET g_liq1[l_ac].liq081 = g_liq1_t.liq081
               NEXT FIELD liq081_1
            END IF
         END IF 
         IF NOT cl_null(g_liq1[l_ac].liq081) AND NOT cl_null(g_liq1[l_ac].liq082) THEN
            IF g_liq1[l_ac].liq081 > g_liq1[l_ac].liq082 THEN
               CALL cl_err('','alm-161',0)
               NEXT FIELD liq081_1
            END IF
            IF NOT cl_null(g_liq1[l_ac].liq04) AND NOT cl_null(g_liq1[l_ac].liq05)  THEN
               CALL i365_chk_liq081(g_liq1[l_ac].liq04,g_liq1[l_ac].liq05,g_liq1_t.liq02,g_liq1[l_ac].liq081,g_liq1[l_ac].liq082,p_cmd)
               IF NOT cl_null(g_errno) THEN
                  CALL cl_err('',g_errno,0)
                  NEXT FIELD liq081_1
            ...
            END IF
         END IF
         IF (NOT cl_null(g_liq1[l_ac].liq082) AND cl_null(g_liq1[l_ac].liq081))
            OR (cl_null(g_liq1[l_ac].liq082) AND NOT cl_null(g_liq1[l_ac].liq081)) THEN
            CALL cl_err('','alm1383',0)
         END IF
         IF cl_null(g_liq1[l_ac].liq081) AND cl_null(g_liq1[l_ac].liq082) THEN
            IF NOT cl_null(g_liq1[l_ac].liq04) AND NOT cl_null(g_liq1[l_ac].liq05)  THEN
               CALL i365_chk_liq082(g_liq1[l_ac].liq04,g_liq1[l_ac].liq05,g_liq1_t.liq02,p_cmd)
               IF NOT cl_null(g_errno) THEN
                  CALL cl_err('',g_errno,0)
                  NEXT FIELD liq081_1
               END IF
      ...
      AFTER FIELD liq082_1
         IF NOT cl_null(g_liq1[l_ac].liq082) THEN
            IF g_liq1[l_ac].liq082 < 0 THEN
               CALL cl_err('','alm1089',0)
               LET g_liq1[l_ac].liq082 = g_liq1_t.liq082
               NEXT FIELD liq082_1
            END IF
         END IF
         IF NOT cl_null(g_liq1[l_ac].liq081) AND NOT cl_null(g_liq1[l_ac].liq082) THEN
      ...
      ON CHANGE liq04_1,liq05_1,liq081_1,liq082_1 
         CALL i365_set_required(g_liq1[l_ac].liq04,g_liq1[l_ac].liq05,g_liq1_t.liq02,g_liq1[l_ac].liq081,g_liq1[l_ac].liq082,p_cmd)  #FUN-CA0081 add
      #FUN-CA0081-----add---end
        ...
        ON ROW CHANGE
           ...
           ELSE
              UPDATE liq_file SET liq02=g_liq1[l_ac].liq02,
                                  ...
                                  liq081=g_liq1[l_ac].liq081,      #FUN-CA0081 add
                                  liq082=g_liq1[l_ac].liq082,      #FUN-CA0081 add
...
DEFINE p_wc2      STRING
   ...
   LET g_sql = "SELECT liq02,liq03,liq04,liq05,liq07,liq081,liq082,liq071,liq072,liq073,liq074,liq09",    #FUN-CA0081 add liq081,liq082
               "  FROM liq_file",
               " WHERE liq01 ='",g_lip.lip01,"' "
...
#FUN-CA0081-----------add------str
FUNCTION i365_chk_liq04(startdate,enddate,p_liq02,p_cmd)
   DEFINE p_liq02          LIKE liq_file.liq02
   DEFINE p_cmd            LIKE type_file.chr1
   DEFINE l_n              LIKE type_file.num5
   ...
   END IF
END FUNCTION
FUNCTION i365_chk_liq082(startdate,enddate,p_liq02,p_cmd)
   DEFINE p_liq02          LIKE liq_file.liq02
   DEFINE p_cmd            LIKE type_file.chr1
   DEFINE l_n              LIKE type_file.num5
   DEFINE startdate        LIKE type_file.dat
   ...
   IF p_cmd= 'a' THEN
      SELECT COUNT(*) INTO l_n FROM liq_file
       WHERE liq01 = g_lip.lip01
         AND liq04 = startdate
         AND liq05 = enddate
         AND liq081 IS NOT NULL
         AND liq082 IS NOT NULL
   ELSE
      SELECT COUNT(*) INTO l_n FROM liq_file
       WHERE liq01 = g_lip.lip01
         AND liq02 <> p_liq02
...
END FUNCTION
#检查同一时间段内上下限不重复
FUNCTION i365_chk_liq081(startdate,enddate,p_liq02,p_liq081,p_liq082,p_cmd)
   DEFINE p_liq02          LIKE liq_file.liq02
   DEFINE p_liq081         LIKE liq_file.liq081
   DEFINE p_liq082         LIKE liq_file.liq082
   DEFINE p_cmd            LIKE type_file.chr1
   DEFINE l_n              LIKE type_file.num5
   DEFINE startdate        LIKE type_file.dat
   DEFINE enddate          LIKE type_file.dat
   ...
   IF p_cmd= 'a' THEN
      SELECT COUNT(*) INTO l_n FROM liq_file
       WHERE liq01 = g_lip.lip01
         AND liq04 = startdate
         AND liq05 = enddate 
         AND ((p_liq081 BETWEEN liq081 AND liq082
              OR p_liq082 BETWEEN liq081 AND liq082)
          OR (liq081 BETWEEN p_liq081 AND p_liq082
              OR liq082 BETWEEN p_liq081 AND p_liq082))
   ELSE
      SELECT COUNT(*) INTO l_n FROM liq_file
       WHERE liq01 = g_lip.lip01
         AND liq02 <> p_liq02
   ...
   END IF
END FUNCTION
FUNCTION i365_set_required(startdate,enddate,p_liq02,p_liq081,p_liq082,p_cmd)
   DEFINE p_liq02          LIKE liq_file.liq02
   DEFINE p_liq081         LIKE liq_file.liq081
   DEFINE p_liq082         LIKE liq_file.liq082
   DEFINE p_cmd            LIKE type_file.chr1
   ...
   IF NOT cl_null(startdate) AND NOT cl_null(enddate) THEN
      ...
      END IF
   END IF
   IF NOT cl_null(p_liq081) OR NOT cl_null(p_liq082) OR l_n >0 THEN
      CALL cl_set_comp_required("liq081_1",TRUE)
      CALL cl_set_comp_required("liq082_1",TRUE)
   ELSE
      CALL cl_set_comp_required("liq081_1",FALSE)
      CALL cl_set_comp_required("liq082_1",FALSE)
   END IF
END FUNCTION
#FUN-CA0081-----------add------end
