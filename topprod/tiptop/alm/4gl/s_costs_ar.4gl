...
#
# Pattern name...: s_costs_ar.4gl
# Descriptions...: 產生費用單時直接拋AR副程序(適用于手動錄費用單和系統自動生成費用單)
# Date & Author..: FUN-960140 2009/06/09 By lutingting
...
# Modify.........: NO.FUN-9B0147 09/11/26 By lutingting 給ooa37默認值N 
# Modify.........: No:FUN-9C0014 09/12/09 By shiwuying s_t300_gl,s_t300_rgl增加一參數
# Modify.........: No.FUN-9C0080 09/12/16 By lutingting產生應收時金額要四舍五入
...
# Modify.........: No:FUN-A60064 10/06/24 By chenls 插入oma_file時omalegal賦值,插入omb_file時omblegal賦值
#                                                   插入omc_file時omclegal賦值
# Modify.........: No.FUN-A40076 10/07/02 By xiaofeizhu 更改ooa37的默認值，如果為N改為1
# Modify.........: No:FUN-A70118 10/07/29 BY shiwuying xxxlegal賦值
# Modify.........: No:FUN-A80008 10/08/02 By shiwuying SQL中的to_char改成BDL語法
# Modify.........: No:TQC-AC0113 10/12/15 By huangtao 自動編號傳參錯誤
...
# Modify.........: No:TQC-B20151 11/02/22 By lilingyu artt610資料審核時程式當出
# Modify.........: NO.FUN-B30166 11/03/29 By zhangweib nme_file add nme27
# Modify.........: NO.FUN-B40011 11/04/20 By guoch  nmh_file add nmh42
# Modify.........: No:FUN-B40056 11/05/11 By lixia 刪除資料時一併刪除tic_file的資料
# Modify.........: No:TQC-B70021 11/07/19 By wujie 抛转tic_file资料
# Modify.........: No:TQC-B80169 11/08/23 By lixia omamksg賦值
# Modify.........: No.FUN-B90062 11/09/15 By wujie 产生nme_file时同时产生tic_file    
# Modify.........: No.MOD-C10091 12/01/10 By wujie oma00=17时，oma61没有取得值 
...
DEFINE  l_sum        LIKE type_file.num15_3,       #TQC-B20151 num5->num15_3
   ...
   CASE g_lua.lua02 
      ...
      WHEN '02'
         ...
         END IF 
      WHEN '10'
         LET g_oma.oma00 = '22'   #產生待扺
         IF cl_null(g_oow.oow26) THEN                                                                                                
            IF g_bgerr THEN                                                                                                          
               CALL s_errmsg('oow26',g_oow.oow26,'','axr-149',1)                                                                     
            ELSE                                                                                                                     
               CALL cl_err('oow26','axr-149',1)                                                                                      
            END IF                                                                                                                   
            LET g_success = 'N' 
         ELSE
            LET l_slip=g_oow.oow26
         END IF 
   END CASE
 
      #####自動編號 (ps:lua02='0'時，取單別oow02自動編號﹔lua02='1'時，取單別oow03自動編號)
      CALL s_auto_assign_no("AXR",l_slip,g_lua.lua09,g_oma.oma00,"oma_file","oma01","","","")
      ...
      LET g_oma.oma02 = g_lua.lua09 LET g_oma.oma03 = g_lua.lua06
...
#     CALL s_rdatem(g_oma.oma03,g_oma.oma32,g_oma.oma02,g_oma.oma09,g_oma.oma02,g_dbs2)    #FUN-980020 mark
      ...
      #LET g_oma.omaplant = g_lua.luaplant   #FUN-960140 mark 090824
      LET g_oma.oma66 = g_lua.luaplant      #FUN-960140 add  090824
      LET g_oma.oma70 = '1'
      LET g_oma.oma50 = 0 
      LET g_oma.oma50t = 0
      LET g_oma.oma52 = 0
      ...
      LET g_oma.oma54 = g_lua.lua08
      LET g_oma.oma56 = g_lua.lua08*g_oma.oma24
      CALL cl_digcut(g_oma.oma56,g_azi04) RETURNING g_oma.oma56
      LET g_oma.oma54t=g_lua.lua08t
      LET g_oma.oma56t=g_lua.lua08t*g_oma.oma24
      LET g_oma.oma54x=g_lua.lua08t - g_lua.lua08
...
#No.MOD-C10091 --end 
      ...
      LET g_oma.omamksg='N'   #TQC-B80169
      LET g_oma.oma64 = '0'
      LET g_oma.omalegal = g_legal    #FUN-A60064 add
#No.FUN-AB0034 --begin
...
FUNCTION s_ar_gen_omb()
   DEFINE i LIKE type_file.num5,
          l_lub  RECORD LIKE lub_file.*
 
   LET i = 1
   INITIALIZE g_omb.* TO NULL
 
   LET g_sql1 = "SELECT * FROM lub_file WHERE lubplant = ? AND lub01 = ? "
   PREPARE s_costs_pb1 FROM g_sql1
   DECLARE s_costs_cs1 CURSOR FOR s_costs_pb1
   FOREACH s_costs_cs1 USING g_plant,g_oma.oma16 INTO l_lub.* 　
       IF SQLCA.sqlcode THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,1)
          EXIT FOREACH
       END IF
       ...
       IF cl_null(l_lub.lub04) THEN
          LET l_lub.lub04 = 0
       END IF
       IF cl_null(l_lub.lub04t) THEN
          LET l_lub.lub04t = 0
       END IF
       IF g_lua.lua23 = 'Y' THEN
          LET g_omb.omb13 = l_lub.lub04t
       ELSE
          LET g_omb.omb13 = l_lub.lub04
       END IF
 
       CALL cl_digcut(g_omb.omb13,t_azi03) RETURNING g_omb.omb13
 
 
       LET g_omb.omb14 = l_lub.lub04
       LET g_omb.omb14t = l_lub.lub04t
       ...
       LET g_omb.omb31 = l_lub.lub01
       LET g_omb.omb32 = l_lub.lub02
 
       SELECT oaj04,oaj041 INTO g_omb.omb33,g_omb.omb331
         FROM oaj_file
        WHERE oaj01=l_lub.lub03
       ...
       LET g_omb.omb38 = '06'
       LET g_omb.omb39 = 'N'
       LET g_omb.omblegal = g_legal     #FUN-A60064 add
...
FUNCTION s_ar_gen_omc()
   ...
   LET g_omc.omclegal = g_legal   #FUN-A60064 add
...
DEFINE l_cnt        LIKE type_file.num5    #TQC-B20151
 
 
#a:借方明細單身  (根據不同的付款方式)
   LET l_ac = 1
   LET g_sql1 = "SELECT * FROM rxx_file WHERE rxx01 = '",g_lua.lua01,
  ...
  END CASE
   ...
   #LET g_oob.oob17 = l_ryd05          #銀行編號   #FUN-9C0168
   ...
   CASE
      WHEN l_rxx.rxx02 = '04'      #券
           ...
           FOREACH s_q_cs INTO l_rxy05,l_rxy12,l_rxy17
            ...
            #  WHERE lpx01 = l_rxy12
              SELECT oog02,oog03 INTO g_oob.oob11,g_oob.oob111 FROM oog_file
               WHERE oog01 = l_rxy12
            #FUN-A60052--mod--end
              IF g_aza.aza63 = 'N' THEN LET g_oob.oob111 = '' END IF
              IF (cl_null(g_oob.oob11)) 
                 OR (g_aza.aza63 = 'Y' AND cl_null(g_oob.oob111)) THEN     #券種沒有抓到科目
...
###產生anmt200的資料
           IF cl_null(g_oow.oow22) THEN                                                                                                
              ...
              LET g_success = 'N' 
           END IF 
#           CALL s_auto_assign_no("ANM",g_oow.oow22,l_rxy.rxy09,"2","nmh_file","nmh01","","","")    #TQC-AC0113  mark   
            CALL s_auto_assign_no("ANM",g_oow.oow22,l_rxy.rxy10,"2","nmh_file","nmh01","","","")    #TQC-AC0113
           ...
           LET g_nmh.nmh21 = l_ooe02_1   #TQC-B10270
           #No.TQC-B10270  --END
           LET g_nmh.nmh24 = '1'
           ...
           #LET g_nmh.nmh41 = ' '           #TQC-AC0113 add   #TQC-B10270 mark
...
DEFINE l_cnt           LIKE type_file.num5       #TQC-B20151
  ...
  #LET g_ooa.ooaconf = 'N' #TQC-AC0127
   ...
   #LET g_ooa.ooa37 = 'N'    #FUN-9B0147   #FUN-A40076 mark
   LET g_ooa.ooa37 = '1'    #FUN-40076 add
...
FUNCTION s_yz_hu1(p_sw)
  ...
  IF p_sw = '+' THEN
    ...
    CALL s_auto_assign_no("axr",l_oma.oma01,g_lua.lua09,"26","oma_file","oma01","","","")
    ...
    LET l_oma.omamksg='N'   #TQC-B80169
    LET l_oma.oma65 = '1'
    ...
    IF g_aaz.aaz90='Y' THEN
       ...
       END IF
       #LET l_oma.omaplant=s_costcenter(l_oma.oma15)   #FUN-960140 mark 090824
       LET l_oma.oma66 = s_costcenter(l_oma.oma15)    #FUN-960140 add  090824
    END IF
 
    LET l_oma.omaoriu = g_user      #No.FUN-980030 10/01/04
    LET l_oma.omaorig = g_grup      #No.FUN-980030 10/01/04
    LET l_oma.omalegal = g_legal    #FUN-A60064 add
#No.FUN-AB0034 --begin
...
FUNCTION s_ar_y_chk()
   ...
   IF g_ooy.ooydmy1 = 'Y' THEN  
      ...
      #    AND ( to_char(oma02,'YYYY') != to_char(npp02,'YYYY') OR          #FUN-A60064 add
      #      (to_char(oma02,'YYYY')  = to_char(npp02,'YYYY') AND to_char(oma02,'MM') != to_char(npp02,'MM')))    #FUN-A60064 add
           AND ( YEAR(oma02) != YEAR(npp02) OR
               (YEAR(oma02)  = YEAR(npp02) AND MONTH(oma02) != MONTH(npp02)))
      #No.FUN-A80008 -END-------
         IF g_cnt >0 THEN
