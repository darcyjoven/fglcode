# Prog. Version..: '5.30.06-13.04.17(00002)'     #
#
# Pattern name...: amdg401.4gl
# Descriptions...:
...
# Modify.........: No.MOD-970199 09/07/22 By mike  amdg401營業人申報書,選擇合并稅籍,月份:5,6,報表表頭沒有印出統一編號,稅籍編號..    
...
TYPE sr1_t RECORD
    ...
    amd173_b LIKE type_file.chr3,
    amd174_b LIKE type_file.chr2,
    amd174_e LIKE type_file.chr2,
    ame04 LIKE ame_file.ame04,
    ame05 LIKE ame_file.ame05,
    ame06 LIKE ame_file.ame06,
    ame08 LIKE ame_file.ame08,
    inv_all LIKE type_file.num5,
    pdate_1 LIKE type_file.chr3,
    pdate_2 LIKE type_file.chr2,
    pdate_3 LIKE type_file.chr2,
    tot1 LIKE type_file.num10,
    tot10 LIKE type_file.num10,
    tot101 LIKE type_file.num10,
    tot107 LIKE type_file.num10,
    tot108 LIKE type_file.num10,
    tot110 LIKE type_file.num10,
    tot111 LIKE type_file.num10,
    tot112 LIKE type_file.num10,
    tot113 LIKE type_file.num10,
    tot114 LIKE type_file.num10,
    tot115 LIKE type_file.num10,
    tot13 LIKE type_file.num10,
    tot14 LIKE type_file.num10,
    tot15 LIKE type_file.num10,
    tot171 LIKE type_file.num10,
    tot172 LIKE type_file.num10,
    tot181 LIKE type_file.num10,
    tot182 LIKE type_file.num10,
    tot19 LIKE type_file.num10,
    tot2 LIKE type_file.num10,
    tot21 LIKE type_file.num10,
    tot22 LIKE type_file.num10,
    tot23 LIKE type_file.num10,
    tot25 LIKE type_file.num10,
    tot28 LIKE type_file.num10,
    tot29 LIKE type_file.num10,
    tot30 LIKE type_file.num10,
    tot31 LIKE type_file.num10,
    tot32 LIKE type_file.num10,
    tot33 LIKE type_file.num10,
    tot34 LIKE type_file.num10,
    tot35 LIKE type_file.num10,
    tot36 LIKE type_file.num10,
    tot37 LIKE type_file.num10,
    tot38 LIKE type_file.num10,
    tot39 LIKE type_file.num10,
    tot401 LIKE type_file.num10,
    tot402 LIKE type_file.num10,
    tot403 LIKE type_file.num10,
    tot411 LIKE type_file.num10,
    tot412 LIKE type_file.num10,
    tot413 LIKE type_file.num10,
    tot421 LIKE type_file.num10,
    tot422 LIKE type_file.num10,
    tot423 LIKE type_file.num10,
    tot431 LIKE type_file.num10,
    tot432 LIKE type_file.num10,
    tot433 LIKE type_file.num10,
    tot44 LIKE type_file.num10,
    tot45 LIKE type_file.num10,
    tot46 LIKE type_file.num10,
    tot47 LIKE type_file.num10,
    tot48 LIKE type_file.num10,
    tot49 LIKE type_file.num10,
    tot5 LIKE type_file.num10,
    tot6 LIKE type_file.num10,
    tot7 LIKE type_file.num10,
    tot78 LIKE type_file.num10,
    tot79 LIKE type_file.num10,
    tot80 LIKE type_file.num10,
    tot81 LIKE type_file.num10,
    tot9 LIKE type_file.num10,
...
MAIN
   ...
   LET g_sql = "abx.type_file.num10,ama02.ama_file.ama02,",
               ...
               "tot27.type_file.num10 "                       #FUN-8B0081 add
 
   LET l_table = cl_prt_temptable('amdg401',g_sql) CLIPPED
   IF l_table = -1 THEN 
...
FUNCTION g401_tm(p_row,p_col)
   ...
   END IF
   OPEN WINDOW g401_w AT p_row,p_col
        WITH FORM "amd/42f/amdg401"
################################################################################
...
################################################################################
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW g401_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      CALL cl_gre_drop_temptable(l_table)
      EXIT PROGRAM
   END IF
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amdg401',g_time,l_cmd)
      END IF
      CLOSE WINDOW g401_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      CALL cl_gre_drop_temptable(l_table)
...
FUNCTION g401()
   DEFINE l_name    LIKE type_file.chr20,      #No.FUN-680074 VARCHAR(20) # External(Disk) file name
          ...
          sr        RECORD
                    ...
                    amd031 LIKE amd_file.amd03,   #發票聯數       #MOD-AC0006
     ...
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'amdg401'
    ...
    #註記欄(總機構彙總報繳l_flag='Y'/分單位分別申報l_flag='N')
     ...
     END IF
###GENGRE###     LET g_str = l_flag

     #-->進銷項
     LET l_sql = "SELECT amd17,amd171,amd172,amd07,amd08,amd03,amd031,amd04,amd06,'',amd25, ",   #MOD-650091 #FUN-8B0081 add '' #MOD-AC0006 add amd031
     ...
     PREPARE g401_prepare  FROM l_sql
     IF SQLCA.SQLCODE THEN CALL cl_err('prepare',STATUS,0) END IF
     DECLARE g401_curs CURSOR FOR g401_prepare
    ...
    #媒體申報其他資料
     ...
     LET g_tot19 = 0
     LET g_abx =0   #保稅
     CALL g401_sum1()  #計算發票數
     FOREACH g401_curs INTO sr.*,l_amd25,l_amd35,l_amd36,l_amd37,
                            l_amd38,l_amd39
    ...
    #-MOD-AC0006-end-
     ...
     SELECT SUM(amd07) INTO l_tot35 FROM x WHERE amd171='25' AND amd17 ='2'
                                             AND amd172='1'
     #37:載有稅額其他憑證(包括二聯式收銀)進貨及費用稅額
    #SELECT SUM(amd07) INTO l_tot37 FROM x WHERE (amd171='22' OR amd171='27') #MOD-AC0006 mark
    #                                        AND  amd17 ='1' AND amd172='1'   #MOD-AC0006 mark
     #39:載有稅額其他憑證(包括二聯式收銀)固定資產稅額
    #SELECT SUM(amd07) INTO l_tot39 FROM x WHERE amd171='22' AND amd17='2'    #MOD-AC0006 mark
    #                                        AND amd172='1'                   #MOD-AC0006 mark
     ...
     PREPARE g401_prepare_27  FROM l_sql_27
     IF SQLCA.SQLCODE THEN CALL cl_err('prepare',STATUS,0) END IF
     DECLARE g401_curs_27 CURSOR FOR g401_prepare_27
     ...
     FOREACH g401_curs_27 INTO sr27.*,l_amd38_27,l_amd39_27
       IF SQLCA.sqlcode != 0 THEN
           CALL cl_err('foreach:',SQLCA.sqlcode,1)
           EXIT FOREACH
       END IF
...
###GENGRE###     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
    IF tm.t = "1" THEN
#     LET l_name = "amdg401"            #FUN-B40087 mark
       LET g_template = "amdg401"       #FUN-B40087 add       
       CALL amdg401_grdata()            #FUN-B40087 add
    ELSE
#     LET l_name = "amdg401_2"          #FUN-B40087 mark
       LET g_template = "amdg401_1"     #FUN-B40087 add
       CALL amdg401_grdata()            #FUN-B40087 add
    END IF
###GENGRE###     CALL cl_prt_cs3('amdg401',l_name,l_sql,g_str)  #MOD-9C0151 mod
 
 
END FUNCTION
 
FUNCTION g401_sum1()  # 計算發票數
...
FUNCTION amdg401_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("amdg401")
        IF handler IS NOT NULL THEN
            START REPORT amdg401_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE amdg401_datacur1 CURSOR FROM l_sql
            FOREACH amdg401_datacur1 INTO sr1.*
                OUTPUT TO REPORT amdg401_rep(sr1.*)
            END FOREACH
            FINISH REPORT amdg401_rep
        END IF
...
REPORT amdg401_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B40087-----------------add--------str----
    DEFINE l_ame08_04_05        LIKE ame_file.ame08
    DEFINE l_tot171_tot172      LIKE type_file.num10
    DEFINE l_tot181_tot182      LIKE type_file.num10
    DEFINE l_tot401_402_403     LIKE type_file.num10
    DEFINE l_tot411_412_413     LIKE type_file.num10
    DEFINE l_tot421_422_423     LIKE type_file.num10
    DEFINE l_tot431_432_433     LIKE type_file.num10 
    DEFINE l_flag               LIKE type_file.chr1
    DEFINE l_ama15              LIKE ama_file.ama15 
    #FUN-B40087----------------add---------end----
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.ama02
            ...
            END IF
            PRINTX l_flag
            LET sr1.amd173_b = tm.amd173_b - 1911 USING '&&&'
            LET sr1.amd174_b = tm.amd174_b
            LET sr1.amd174_e = tm.amd174_e
            LET sr1.pdate_1 = YEAR(tm.pdate)-1911 USING '&&&'
            LET sr1.pdate_2 = MONTH(tm.pdate)
            LET sr1.pdate_3 = DAY(tm.pdate)
            LET l_ame08_04_05 =sr1.ame08 + sr1.ame04 + sr1.ame05
            PRINTX l_ame08_04_05
            LET l_tot171_tot172 = sr1.tot171 + sr1.tot172
            PRINTX l_tot171_tot172
            LET l_tot181_tot182 = sr1.tot181 + sr1.tot182
            PRINTX l_tot181_tot182
            LET l_tot401_402_403 = sr1.tot401 + sr1.tot402 + sr1.tot403
            PRINTX l_tot401_402_403
            LET l_tot411_412_413 = sr1.tot411 + sr1.tot412 + sr1.tot413
            PRINTX l_tot411_412_413
            LET l_tot421_422_423 = sr1.tot421 + sr1.tot422 + sr1.tot423
            PRINTX l_tot421_422_423
            LET l_tot431_432_433 = sr1.tot431 + sr1.tot432 + sr1.tot433
            PRINTX l_tot431_432_433
            #FUN-B40087----------------add---------end----
