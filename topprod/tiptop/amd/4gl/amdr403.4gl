...
# Modify.........: No.MOD-B50229 11/05/26 By Dido 不得扣抵比例欄位應無條件捨去 
# Modify.........: No.TQC-B60182 11/06/17 By zhangweib tm.rep选择1时要可以选择格式一/格式二
# Modify.........: No.FUN-B80050 11/08/03 By minpp 程序撰写规范修改
# Modify.........: No.FUN-BA0021 11/10/21 By Belle 將原來撈ama25~ama105的欄位修改為amk04~amk84
...
# Modify.........: No:MOD-C20071 12/02/14 By Polly 取消GROUP BY的方式
# Modify.........: No.MOD-C20233 12/03/03 By Polly 1.計算發票份數條件修正，
#                                                  2.若採用直接扣抵法，「應比例計算之進項稅額」，不需處理
# Modify.........: No:TQC-CB0052 12/11/16 By zhangweib 得退稅額合計 g_tot113 如為負數,應改為 0 呈現
# Modify.........: No:TQC-CB0073 12/11/16 By zhangweib TQC-CB0052 修改有誤,998行缺少#
...
DEFINE tm  RECORD
       ...
       g_amk RECORD  LIKE amk_file.*,           #FUN-BA0021 
...
MAIN
   ...
   LET g_sql = "ama02.ama_file.ama02,ama03.ama_file.ama03,",
	       ...
	       "ama18.ama_file.ama18,ama19.type_file.chr20,",      #FUN-980024
               ...
               "tot26.type_file.num10,tot27.type_file.num10"     #FUN-970101 #No.TQC-C10034 add, , #NO.TQC-C20047 del ,,
...
#No.TQC-C20047----- mark ----- end
   ...
   LET l_table = cl_prt_temptable('amdr403',g_sql) CLIPPED
   ...
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               ...
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,? )"     #FUN-970101 #CHI-8B0031   #MOD-9A0080 add ? #No.TQC-C10034 add 4? #No.TQC-C20047 del 4?
...
FUNCTION r403_tm(p_row,p_col)
   ...
   END IF
   OPEN WINDOW r403_w AT p_row,p_col
        WITH FORM "amd/42f/amdr403"
################################################################################
...
################################################################################
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW r403_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amdr403',g_time,l_cmd)
      END IF
      CLOSE WINDOW r403_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
#    LOCATE l_img_blob IN MEMORY   #blob初始化   #Genero Version 2.21.02之後要先初始化 #No.TQC-C10034 add #No.TQC-C20047 mark
    ...
    #注记栏(总机构汇总报缴l_flag='Y'/分单位分别申报l_flag='N')
     ...
     PREPARE r403_prepare  FROM l_sql
     IF SQLCA.SQLCODE THEN CALL cl_err('prepare',STATUS,0) END IF
     DECLARE r403_curs CURSOR FOR r403_prepare
 
    #媒體申報其他資料
     IF tm.a <> 'Y' THEN   #TQC-630076
       #SELECT ame01,ame02,ame03,sum(ame04),sum(ame05),sum(ame06),  #MOD-C20071 mark
        SELECT sum(ame04),sum(ame05),sum(ame06),                    #MOD-C20071 add
               sum(ame07),sum(ame08),                               #TQC-620014 #MOD-C20071 remark
              #sum(ame07),sum(ame08),'','','','','',                #TQC-620014 #MOD-C20071 mark
               sum(ame09),sum(ame10),sum(ame11)                     #TQC-620014
         #INTO g_ame.* FROM ame_file                                #MOD-C20071 mark
          INTO g_ame.ame04,g_ame.ame05,g_ame.ame06,                 #MOD-C20071 add
               g_ame.ame07,g_ame.ame08,g_ame.ame09,                 #MOD-C20071 add
               g_ame.ame10,g_ame.ame11                              #MOD-C20071 add
          FROM ame_file                                             #MOD-C20071 add
         WHERE ame01=tm.amd22 AND  ame02 = tm.amd173_b
           AND ame03  BETWEEN tm.amd174_b AND tm.amd174_e
        #GROUP BY ame01,ame02,ame03                                 #MOD-C20071 mark
     ELSE
       #SELECT ame01,ame02,ame03,sum(ame04),sum(ame05),sum(ame06), #MOD-C20071 mark
        SELECT sum(ame04),sum(ame05),sum(ame06),                   #MOD-C20071 add
               sum(ame07),sum(ame08),                              #TQC-620014 #MOD-C20071 remark
     ...
     LET g_tot19 = 0
     LET g_abx =0   #保稅
     CALL r403_sum1()  #計算發票數
     FOREACH r403_curs INTO sr.*,l_amd25,l_amd35,l_amd36,l_amd37,
                            l_amd38,l_amd39
    ...
    #SELECT SUM(amd07) INTO l_b22b FROM x WHERE amd171='22' AND amd17 ='2'             #MOD-A30080 mark
     ...
     #LET l_tot11 = cl_digcut(l_tot11,2)                #MOD-920065 add #MOD-B50229 mark
      LET l_tot11 = s_trunc(l_tot11,2)                  #MOD-B50229
...
#FUN-B40054--add-Begin--
      ...
      ELSE
         #LET l_tot12 = g_ama.ama97    #FUN-BA0021
          LET l_tot12 = g_amk.amk76    #FUN-BA0021
      END IF
#FUN-B40054--add-End----
...
#FUN-B40054--add-End----
    ...
    #IF l_flag ='Y' THEN   #MOD-970063    #MOD-B10117 mark           
     IF l_flag ='Y' OR l_flag =' ' THEN   #MOD-B10117
       ...
       #LET g_tot107 = l_ab+l_bb   #MOD-970063 mark
        ...
        IF g_tot112 <0 THEN LET g_tot112=0 END IF
        LET g_tot113 = g_tot23 * 0.05 + l_bb
       #IF g_tot113 < 0 THEN LET g_tot113 = 0 END IF   TQC-CB0052   Add   #No.TQC-CB0073  Mark
        IF g_tot113 < 0 THEN LET g_tot113 = 0 END IF  #TQC-CB0052   Add   #No.TQC-CB0052   Add
        IF g_tot112 > g_tot113 THEN
           LET g_tot114 = g_tot113
        ELSE
           LET g_tot114 = g_tot112
    ...
    #str MOD-970063 add
     ...
     PREPARE r403_pre_26  FROM l_sql26
     IF SQLCA.SQLCODE THEN CALL cl_err('prepare',STATUS,0) END IF
     DECLARE r403_curs_26 CURSOR FOR r403_pre_26
 
     LET g_tot7_26 = 0
     LET g_tot15_26  = 0
     LET g_tot19_26  = 0
 
     FOREACH r403_curs_26 INTO sr26.*,l_amd25_26,l_amd35_26,l_amd36_26,
     ...
     PREPARE r403_pre_27  FROM l_sql27
     IF SQLCA.SQLCODE THEN CALL cl_err('prepare',STATUS,0) END IF
     DECLARE r403_curs_27 CURSOR FOR r403_pre_27
 
     LET g_tot7_27 = 0
     LET g_tot15_27  = 0
     LET g_tot19_27  = 0
 
     FOREACH r403_curs_27 INTO sr27.*,l_amd25_27,l_amd35_27,l_amd36_27,
...
#                              ,"",  l_img_blob,   "N",""      #No.TQC-C10034 add #No.TQC-C20047 mark
     ...
     IF tm.t = "1" THEN 
        LET l_name = "amdr403"     #無格     
     ELSE  
        LET l_name = "amdr403_2"   #有格  
     END IF
#No.TQC-C20047 ----mark---begin
#    LET g_cr_table = l_table                 #主報表的temp table名稱        #No.TQC-C10034 add
#    LET g_cr_apr_key_f = "ama02"       #報表主鍵欄位名稱，用"|"隔開   #No.TQC-C10034 add       
#No.TQC-C20047 ----mark---end
     CALL cl_prt_cs3('amdr403',l_name,l_sql,g_str)   #CHI-8B0031
 
 
END FUNCTION
 
FUNCTION r403_sum1()  # 計算發票數
  IF tm.a <> 'Y' THEN   #TQC-630076
  ...
  ELSE
     SELECT count(*) INTO g_inv_all FROM amd_file
            WHERE amd173 = tm.amd173_b
              AND amd174 BETWEEN tm.amd174_b  
              AND tm.amd174_e
              AND amd171 MATCHES '3*'
              AND amd171<>'33'              #MOD-C20233 add
              AND amd171<>'34'              #MOD-C20233 add
              AND amd171<>'36' 
              AND amd30='Y'    #不含虛擬發票,已確認資料no:7393
     IF SQLCA.SQLCODE THEN
       CALL cl_err3("sel","amd_file","","",STATUS,"","sel oma",0)   #No.FUN-660093
     END IF
...
FUNCTION r403_set_entry()
  CALL cl_set_comp_entry("amd22",TRUE)
END FUNCTION
 
FUNCTION r403_set_no_entry()
...
# LOCATE l_img_blob IN MEMORY   #blob初始化   #Genero Version 2.21.02之後要先初始化 #No.TQC-C10034 add #No.TQC-C20047 mark
   ...
   LET l_sql = "amk04.amk_file.amk04,amk05.amk_file.amk05,",
               ...
               "amk76.amk_file.amk76"                 #No.TQC-C10034 add , , #No.TQC-C20047 del ,,
#No.TQC-C20047 ----mark---begin
#              "sign_type.type_file.chr1,",   #簽核方式                   #No.TQC-C10034 add
#              "sign_img.type_file.blob,",    #簽核圖檔                   #No.TQC-C10034 add
#              "sign_show.type_file.chr1,",   #是否顯示簽核資料(Y/N)      #No.TQC-C10034 add
...
#No.TQC-C20047 ----mark---end
#FUN-BA0021---End---
   LET l_table = cl_prt_temptable('amdr403',l_sql) CLIPPED
   IF l_table = -1 THEN 
   ...
   LET l_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               ...
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,? )"  #No.TQC-C10034 add4? #No.TQC-C20047 del 4?
  ...
  #EXECUTE insert_prep_1 USING g_ama_1.*     #FUN-BA0021 mark
   EXECUTE insert_prep_1 USING g_amk_1.*     #FUN-BA0021
#                              ,"",  l_img_blob,   "N",""      #No.TQC-C10034 add #No.TQC-C20047 mark

   LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
#  LET g_cr_table = l_table                 #主報表的temp table名稱        #No.TQC-C10034 add #No.TQC-C20047 mark
#  LET g_cr_apr_key_f = "amk04"       #報表主鍵欄位名稱，用"|"隔開   #No.TQC-C10034 add       #No.TQC-C20047 mark
   CALL cl_prt_cs3('amdr403','amdr403_3',l_sql,'')
END FUNCTION
...
# LOCATE l_img_blob IN MEMORY   #blob初始化   #Genero Version 2.21.02之後要先初始化 #No.TQC-C10034 add #No.TQC-C20047 mark
   ...
   LET l_sql = "amk78.amk_file.amk78,amk79.amk_file.amk79,",
               ...
               "amk84.amk_file.amk84"    #No.TQC-C10034 add, , #No.TQC-C20047 del ,,
#No.TQC-C20047----- mark ----- begin
#              "sign_type.type_file.chr1,",   #簽核方式                   #No.TQC-C10034 add
#              "sign_img.type_file.blob,",    #簽核圖檔                   #No.TQC-C10034 add
#              "sign_show.type_file.chr1,",   #是否顯示簽核資料(Y/N)      #No.TQC-C10034 add
...
#FUN-BA0021---End---
   ...
   END IF
   LET l_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,? )"  #No.TQC-C10034 add 4? #No.TQC-C20047 del 4?
  ...
  #EXECUTE insert_prep_2 USING g_ama_2.*     #FUN-BA0021 mark
   EXECUTE insert_prep_2 USING g_amk_2.*     #FUN-BA0021
#                              ,"",  l_img_blob,   "N",""      #No.TQC-C10034 add #No.TQC-C20047 mark

   LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
#  LET g_cr_table = l_table                 #主報表的temp table名稱        #No.TQC-C10034 add #No.TQC-C20047 mark
#  LET g_cr_apr_key_f = "amk78"       #報表主鍵欄位名稱，用"|"隔開   #No.TQC-C10034 add       #No.TQC-C20047 mark
   CALL cl_prt_cs3('amdr403','amdr403_4',l_sql,'')
