...
# Modify.........: No.FUN-690077 06/10/11 By Sarah 複製amdr311改成amdr310,PRINT段遇到l時增加CLIPPED
...
# Modify.........: No:MOD-C30852 12/03/26 By Polly 調整EXIT WHILE，在EXECUTE時再判斷是否要離開
...
MAIN
   ...
   IF (NOT cl_setup("AMD")) THEN
      EXIT PROGRAM
   END IF
   #CALL cl_used(g_prog,g_time,1) RETURNING g_time  #FUN-B80050    ADD #FUN-BB0047 mark
   ...
   LET g_sql =
      "ama02.ama_file.ama02,     ama03.ama_file.ama03,",
      ...
      "total5.type_file.num20_6"                             #No.TQC-C10034 add , , #No.TQC-C20047 del , ,
...
#No.TQC-C20047 ----- mark ----- end
   LET l_table = cl_prt_temptable('amdr310',g_sql) CLIPPED                                                                          
   ...
   LET l_table1 = cl_prt_temptable('amdr3101',g_sql) CLIPPED 
   IF l_table1=-1 THEN EXIT PROGRAM END IF                                                                                          
 
   CALL cl_used(g_prog,g_time,1) RETURNING g_time  #FUN-BB0047 add
   IF cl_null(g_bgjob) OR g_bgjob = 'N'
      THEN CALL r310_tm(0,0)
      ELSE CALL r310()
   END IF
...
FUNCTION r310_tm(p_row,p_col)
   ...
   OPEN WINDOW r310_w AT p_row,p_col WITH FORM "amd/42f/amdr310"
   ...
   WHILE TRUE
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('amdr310',g_time,l_cmd)
         END IF
         CLOSE WINDOW r310_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80050    ADD
         EXIT PROGRAM
...
#No.TQC-C20047 ----- mark ----- end
   ...
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,? )"   #MOD-870262 add 4? #No.TQC-C10034 add 4? #No.TQC-C20047 del 4?
   ...
   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'amdr310'
   ...
   FOREACH oom_curs INTO sr.*
      ...
      WHILE TRUE
         ...
         IF k_oom07>=0  AND k_oom07<=49 THEN   #MOD-610086   
            CALL r310_oom11(sr.oom11,sr.oom12)
         ...
         IF k_oom07>=50 AND k_oom07<=99 THEN   #MOD-610086   
            CALL r310_oom11(sr.oom11,sr.oom12)   #MOD-610086
            LET j_oom07=m_oom07[sr.oom11,sr.oom12]
         ...
         ELSE
            LET m_oom07=l_oom07
         END IF
        #IF m_oom07 >= sr.oom08 THEN EXIT WHILE END IF   #MOD-C30852 mark
...
DEFINE    l_sql     STRING,                        #No.FUN-680074 VARCHAR(1000) # RDSQL STATEMENT
   ...
   FOREACH oom_curs INTO sr.*
      ...
      WHILE TRUE
         ...
         END IF
         CALL r310_cal_tot2(m_oom07,sr.oom11,sr.oom12)
         LET l_oom07=l_oom07 + 50 USING g_format
