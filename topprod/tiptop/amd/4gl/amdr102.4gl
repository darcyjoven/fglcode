...
# Modify.........: No.TQC-C20047 12/02/10 By yuhuabao 套表無需簽核内容
...
#DEFINE g_inv_all    LIKE type_file.num5    #使用發票份數  #MOD-D40106 mark   #MOD-690155
...
#-----TQC-610057---------
   ...
   LET g_sql ="a311.amd_file.amd08,",
              ...
              "amd174e.type_file.num10"   #No.TQC-C10034 add , ,       #No.TQC-C20047 del , ,
...
#No.TQC-C20047 ----- mark ----- end
   ...
   LET l_table = cl_prt_temptable('amdr102',g_sql) CLIPPED
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amdr102',g_time,l_cmd)
      END IF
      CLOSE WINDOW r102_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
#No.TQC-C20047 ----- mark ----- end
     ...
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'amdr102'
...
#              "   AND amd22 ='",tm.ama01,"'"
#    PREPARE r102_prepare  FROM l_sql
#    IF SQLCA.SQLCODE THEN CALL cl_err('prepare',STATUS,0) END IF
#    DECLARE r102_curs CURSOR FOR r102_prepare
 
     CALL cl_del_data(l_table) 
 
#    CALL cl_outnam('amdr102') RETURNING l_name
#    START REPORT r102_rep TO l_name
#    LET g_pageno = 0
 
     CALL r102_sum()  #計算發票數   #MOD-690155
 
#    FOREACH r102_curs INTO sr.*
...
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
   ...
   # LET l_sql = "SELECT * FROM ",l_table CLIPPED   #TQC-730013                   
     ...
     LET l_str = g_ama02 CLIPPED,";",g_ama03 CLIPPED,";",
                 g_ama05[1,30] CLIPPED,";",g_company CLIPPED,";",
                 tm.amd173_b CLIPPED,";",
                 tm.amd174_b CLIPPED,";",tm.amd174_e CLIPPED,";",
                 g_inv_all CLIPPED
 
#No.TQC-C20047 ----- mark ----- begin
#  LET g_cr_table = l_table                 #主報表的temp table名稱        #No.TQC-C10034 add
#  LET g_cr_apr_key_f = "a311"       #報表主鍵欄位名稱，用"|"隔開   #No.TQC-C10034 add
#No.TQC-C20047 ----- mark ----- end
   # CALL cl_prt_cs3(g_prog,l_sql,l_str)   #TQC-730113
     CALL cl_prt_cs3(g_prog,'amdr102',l_sql,l_str) 
     #No.FUN-710082--end
END FUNCTION
