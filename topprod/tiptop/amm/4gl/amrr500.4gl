...
#       l_time       LIKE type_file.chr8        #No.FUN-6A0076
     ...
     LET l_sql = "SELECT mmg_file.*,mmh_file.*,ima08,mmi02 ",
                 "  FROM mmg_file,mmh_file,OUTER ima_file,OUTER mmi_file ",
                 " WHERE ima01=mmg04 AND mmi01=mmg05 AND mmi03='1' ",
                 "   AND mmg01=mmh01 AND mmg02=mmh011 ",
                 "   AND ", tm.wc CLIPPED,
   ...
   #LET l_sql =" SELECT mmb01,mmb02,mmb18,mmb131,mmb132 FROM mma_file,mmb_file",
    ...
    #LET l_sql =" SELECT inb01,inb03,inb09 FROM inb_file",
     ...
     CALL cl_outnam('ammr500') RETURNING l_name
...
#                         qty        LIKE ima_file.ima26         #No.FUN-680100 DEC(15,3)#數量#FUN-A20044
                         qty        LIKE type_file.num15_3      #FUN-A20044
                        END RECORD,
                l_desc      LIKE ade_file.ade04,        #No.FUN-680100 DEC(15,3)
