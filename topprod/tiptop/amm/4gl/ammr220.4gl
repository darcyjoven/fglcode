...
MAIN
   ...
   LET g_sql = "mma01.mma_file.mma01,",
               ...
               "azi03.azi_file.azi03,",  #No.TQC-C10034 add
   ...
   LET l_table = cl_prt_temptable('ammr220',g_sql) CLIPPED
   IF  l_table = -1 THEN EXIT PROGRAM END IF
   #No.FUN-710091  --end  
   CALL cl_used(g_prog,g_time,1) RETURNING g_time  #No.FUN-B80065--add--
   IF cl_null(g_bgjob) OR g_bgjob = 'N'	# If background job sw is off
      THEN CALL r220_tm(0,0)		# Input print condition
      ELSE CALL ammr220()		# Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80065--add--
END MAIN
 
FUNCTION r220_tm(p_row,p_col)
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond('mmauser', 'mmagrup') #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW r220_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80065--add--
      EXIT PROGRAM
   END IF
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('ammr220',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW r220_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80065--add--
      EXIT PROGRAM
...
LOCATE l_img_blob IN MEMORY   #blob初始化   #Genero Version 2.21.02之後要先初始化 #No.TQC-C10034 add
     ...
     PREPARE r220_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80065--add--
        EXIT PROGRAM
     END IF
     DECLARE r220_cs1 CURSOR FOR r220_prepare1
 #   CALL cl_outnam('ammr220') RETURNING l_name  #No.FUN-710091 
 #   START REPORT r220_rep TO l_name   #No.FUN-710091 
...
#       SELECT mmc02 INTO sr.mmc02 FROM mmc_file WHERE mmc01 = sr.mmb05
       ...
       #--
    #  OUTPUT TO REPORT r220_rep(sr.*)  #No.FUN-710091
...
#       EXECUTE insert_prep USING sr.*,g_azi03    #No.FUN-850062
    EXECUTE insert_prep USING sr.mma.mma01,sr.mma.mma02,sr.mma.mma03,sr.mma.mma021,
                              ...
                              sr.mmi02,sr.mmc02,g_azi03        #No.FUN-850062
   ...
   # CALL cl_prt_cs3("ammr220",g_sql,g_str)        #TQC-730113
     CALL cl_prt_cs3('ammr220','ammr220',g_sql,g_str)
     #No.FUN-710091  --end  
 
   # FINISH REPORT r220_rep  #No.FUN-710091 
