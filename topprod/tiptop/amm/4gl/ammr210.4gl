...
MAIN
   ...
   LET l_table = cl_prt_temptable('ammr210',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'	# If background job sw is off
      ...
      ELSE CALL ammr210()		# Read data and create out-file
   END IF
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('ammr210',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW r210_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80065--add--
      EXIT PROGRAM
...
LOCATE l_img_blob IN MEMORY   #blob初始化   #Genero Version 2.21.02之後要先初始化 #No.TQC-C10034 add
   ...
   # LET l_sql = "SELECT mmg10,mme01,mme02,mme03,mme10,mme14,mmeprno,",
     LET l_sql = "SELECT mmf10,mme01,mme02,mme03,mme10,mme14,mmeprno,",
                 ...
                 "       pmc081,pmc091,gec02,azi02,pma02,pnz02 ", #FUN-930113  oah-->pnz
                 " FROM mme_file , mmf_file , OUTER pmc_file, OUTER gec_file ,",
                 "       OUTER azi_file , OUTER pma_file , OUTER pnz_file ", #FUN-930113  oah-->pnz
    ...
    #LET l_name = 'ammr210.out'   #No.FUN-710091
    #CALL cl_outnam('ammr210') RETURNING l_name  #No.FUN-710091
    #START REPORT r210_rep TO l_name  #No.FUN-710091
     LET g_pageno = 0
...
#           CALL cl_err(sr.mme03,'mme1216',1) #No.FUN-660094
           ...
           END IF
       END IF  
      #OUTPUT TO REPORT r210_rep(sr.*)  #No.FUN-710091
       EXECUTE insert_prep USING sr.*,g_zo.zo041,g_zo.zo042,g_zo.zo05,g_zo.zo09,g_azi03,g_azi05  #No.FUN-710091
   ...
   # LET g_sql ="SELECT * FROM ",l_table CLIPPED   #TQC-730113
     ...
     LET g_cr_apr_key_f = "mme01"             #報表主鍵欄位名稱，用"|"隔開   #No.TQC-C10034 add
   # CALL cl_prt_cs3("ammr210",g_sql,g_str)        #TQC-730113
     CALL cl_prt_cs3('ammr210','ammr210',g_sql,g_str)
     #No.FUN-710091  --end  
 
    #FINISH REPORT r210_rep  #No.FUN-710091
