...
# Modify.........: No:FUN-C10036 12/01/19 By yangtt CR轉成GRW
...
MAIN
   ...
   LET g_sql=  "azw01.azw_file.azw01,",
    ...
    LET l_table = cl_prt_temptable('artg523',g_sql) CLIPPED
    ...
    IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN 
       CALL artg523_tm()        # Input print condition
    ELSE 
       CALL artg523() 
    END IF
...
DEFINE l_sql          STRING
   ...
   OPEN WINDOW artg523_w AT p_row,p_col WITH FORM "art/42f/artg523" 
   ...
   WHILE TRUE
      CONSTRUCT BY NAME tm.wc ON ogaplant,rtz10,oga02,oga01,ima131
        BEFORE CONSTRUCT
         ...
         AFTER CONSTRUCT
            IF INT_FLAG THEN
               LET INT_FLAG = 0 CLOSE WINDOW artg523_w 
               CALL cl_used(g_prog,g_time,2) RETURNING g_time 
               EXIT PROGRAM
            END IF  
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd1 CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artg523',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artg523_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
DEFINE l_azi04   LIKE  azi_file.azi04
    ...
    FOREACH sel_azp01_cs INTO l_plant,l_azp02 
     ...
     LET l_sql = l_sql," UNION ALL SELECT oha01,oha02,ohb03,ohb04,ima02,(-1)*ohb12,ohb05,(-1)*ohb14t,ohb37,rxc03,(-1)*NVL(rxc06,0) FROM ",
      ...
      PREPARE artg523_prepare1 FROM l_sql                  
      IF SQLCA.sqlcode != 0 THEN                           
         CALL cl_err('prepare:',SQLCA.sqlcode,1)           
         CALL cl_used(g_prog,g_time,2) RETURNING g_time    
         EXIT PROGRAM                                      
      END IF                                               
      DECLARE artg523_curs1 CURSOR FOR artg523_prepare1 
      FOREACH artg523_curs1 INTO sr.*  
         IF SQLCA.sqlcode != 0 THEN
...
###GENGRE###   LET g_str = g_str,";",tm.cb1,";",tm.cb2,";",l_azi04,";"
###GENGRE###   CALL cl_prt_cs3('artg523','artg523',l_sql,g_str)  
    CALL artg523_grdata()    ###GENGRE###
...
FUNCTION artg523_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg523")
        IF handler IS NOT NULL THEN
            START REPORT artg523_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY azw01,oga02,oga01,ogb03"    #FUN-C10036 add
          
            DECLARE artg523_datacur1 CURSOR FROM l_sql
            FOREACH artg523_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg523_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg523_rep
        END IF
...
REPORT artg523_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_rxc06_1_subtot LIKE rxc_file.rxc06
    DEFINE l_ogb14t_2_subtot LIKE ogb_file.ogb14t
    #FUN-C10036----add----str---
    DEFINE l_rxc03_name    STRING   
    DEFINE l_ogb14t_fmt    STRING
    ...
    ORDER EXTERNAL BY sr1.azw01,sr1.oga02,sr1.oga01,sr1.ogb03
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.oga02
            #FUN-C10036----add----str---
            IF sr1.azw01 != sr1_o.azw01 OR sr1.oga02 != sr1_o.oga02 THEN
               LET l_skip = 'Y'
            ELSE
               LET l_skip = 'N'
            END IF
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            LET l_rxc03_name = cl_gr_getmsg("gre-254",g_lang,sr1.rxc03)    #FUN-C10036 add
            PRINTX l_rxc03_name                                     #FUN-C10036 add

            #FUN-C10036----add----str---
            SELECT azi04 INTO l_azi04 FROM azi_file WHERE azi01 = g_aza.aza17
            LET l_ogb14t_fmt = cl_gr_numfmt("ogb_file","ogb14",l_azi04)
            PRINTX l_ogb14t_fmt
            LET l_rxc06_fmt = cl_gr_numfmt("rxc_file","rxc06",l_azi04) 
            PRINTX l_rxc06_fmt

            IF sr1.oga01 = sr1_o.oga01 THEN
               LET l_display = 'N'
            ELSE
               LET l_display = 'Y'
            END IF
        ...
        AFTER GROUP OF sr1.oga02
            LET l_ogb14t_2_subtot = GROUP SUM(sr1.ogb14t)
            PRINTX l_ogb14t_2_subtot
            LET l_rxc06_1_subtot = GROUP SUM(sr1.rxc06)
            PRINTX l_rxc06_1_subtot
