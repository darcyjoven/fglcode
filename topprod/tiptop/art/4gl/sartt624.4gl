...
#
# Pattern name...: sartt624.4gl
# Descriptions...: 押金收取/返還單
# Date & Author..: No:FUN-870100 09/09/27 By Cockroach
# Modify.........: No:FUN-9B0025 09/12/09 By cockroach PASS NO.
# Modify.........: No:TQC-A10085 10/01/10 By Cockroach 新增邏輯
# Modify.........: No:TQC-A20025 10/02/09 By Cockroach BUG處理
# Modify.........: No:TQC-A20047 10/02/23 By Cockroach BUG處理
# Modify.........: No:TQC-A20048 10/02/23 By Cockroach BUG處理
# Modify.........: No:TQC-A30002 10/03/01 By Cockroach BUG處理
# Modify.........: No:TQC-A30056 10/03/17 By Cockroach PASS No.
# Modify.........: No:FUN-A70130 10/08/06 By shaoyong  ART單據性質調整,q_smy改为q_oay
# Modify.........: No:TQC-AC0124 10/12/17 By huangtao 預設押金金額
# Modify.........: No:TQC-AC0223 10/12/17 By wangxin 借出單號帶出相對應的押金單號及退還金額 
# Modify.........: No:TQC-AC0127 10/12/22 By chenying add rxr16
# Modify.........: No:TQC-AC0127 10/12/28 By wuxj    當rxr16不為空的時候，不可取消審核，刪除時相對應的收款退款資料一併刪除
#                                                 增加產生賬款按鈕，調用axrp700
# Modify.........: No:TQC-B20065 11/02/16 By elva 增加接收参数rxr01
# Modify.........: No:CHI-B40058 11/05/13 By JoHung 修改有使用到apy/gpy模組p_ze資料的程式
# Modify.........: No:FUN-B50064 11/06/02 By xianghui BUG修改，刪除時提取資料報400錯誤
# Modify.........: No:FUN-BB0117 11/11/24 By fanbj 將t624替換為t624
# Modify.........: No:FUN-C10039 12/02/02 by Hiko 整批修改資料歸屬設定
# Modify.........: No:TQC-C50169 12/05/21 by fanbj rxr02新增開窗調整
...
#FUNCTION t624(p_argv1,p_argv2)  #TQC-B20065
FUNCTION t624(p_argv1,p_argv2,p_argv3)  #TQC-B20065 #FUN-BB0117
...
DEFINE     l_n       LIKE type_file.num5 
    ...
    DECLARE t624_cl CURSOR FROM g_forupd_sql
    
    LET p_row = 4 LET p_col = 10
    OPEN WINDOW t624_w AT p_row,p_col WITH FORM "art/42f/artt624"
...
DEFINE  lc_qbe_sn   LIKE    gbm_file.gbm01
    ...
    PREPARE t624_prepare FROM g_sql
    DECLARE t624_cs SCROLL CURSOR WITH HOLD FOR t624_prepare

    LET g_sql="SELECT COUNT(*) FROM rxr_file ",
              " WHERE rxr00='",g_rxr00,"' AND ",g_wc CLIPPED                
    PREPARE t624_precount FROM g_sql
    DECLARE t624_count CURSOR FOR t624_precount
...
#TQC-AC0127   ---end---
      ...
      ON ACTION locale
         ...
         CALL t624_g_form()              #TQC-A20047 ADD
...
FUNCTION t624_q()
    ...
    ELSE
       OPEN t624_count
       FETCH t624_count INTO g_row_count
...
FUNCTION t624_fetch(p_flrxr)
DEFINE p_flrxr         LIKE type_file.chr1    
       
    CASE p_flrxr
        WHEN 'N' FETCH NEXT     t624_cs INTO g_rxr.rxr01
        WHEN 'P' FETCH PREVIOUS t624_cs INTO g_rxr.rxr01
        WHEN 'F' FETCH FIRST    t624_cs INTO g_rxr.rxr01
        WHEN 'L' FETCH LAST     t624_cs INTO g_rxr.rxr01
        WHEN '/'
            IF (NOT mi_no_ask) THEN                   
               ...
               END IF
            END IF
            FETCH ABSOLUTE g_jump t624_cs INTO g_rxr.rxr01
            LET mi_no_ask = FALSE        
    END CASE
...
DEFINE     l_rur15   LIKE rur_file.rur15    #add    
   ...
   INPUT BY NAME
      ...
      BEFORE INPUT
          ...
          CALL t624_set_entry(p_cmd)
          CALL t624_set_no_entry(p_cmd)
...
FUNCTION t624_u()
   ...
   OPEN t624_cl USING g_rxr.rxr01
   IF STATUS THEN
      CALL cl_err("OPEN t624_cl:", STATUS, 1)
      CLOSE t624_cl
      ROLLBACK WORK
      RETURN
   END IF

   FETCH t624_cl INTO g_rxr.*                      
   IF SQLCA.sqlcode THEN
       CALL cl_err(g_rxr.rxr01,SQLCA.sqlcode,0)    
       CLOSE t624_cl
       ROLLBACK WORK
...
FUNCTION t624_set_no_entry(p_cmd)
DEFINE p_cmd LIKE type_file.chr1         
...
#     CALL s_auto_assign_no("art",g_rxr.rxr01,g_today,"S","rxr_file","rxr00,rxr01","","","") #FUN-A70130 mark
   ...
   CALL t624_u()
   #SELECT rxr_file.* INTO g_rxr.* FROM rxr_file WHERE rxr01 = l_oldno  #FUN-C80046
   CALL t624_show()
   ...
   IF g_success = 'Y' THEN
      ...
      SELECT gen02 INTO l_gen02 FROM gen_file
       WHERE gen01 = g_rxr.rxrconu
         AND genacti='Y'
      IF cl_null(l_gen02) THEN LET l_gen02=' ' END IF 
      DISPLAY l_gen02 TO rxrconu_desc             #CHI-D20015
...
##TQC-A20025 ------------------------------------------
   ...
   IF cl_upsw(0,0,g_rxr.rxrconf) THEN 
      LET g_rxr.rxrconf = 'N'
     #LET g_rxr.rxrconu = ''        #CHI-D20015
     #LET g_rxr.rxrcond = ''        #CHI-D20015
      LET g_rxr.rxrconu = g_user    #CHI-D20015
      LET g_rxr.rxrcond = g_today   #CHI-D20015
...
DEFINE l_n LIKE type_file.num5
   ...
   #FUN-D20039 ----------sta
   IF p_type = 1 THEN
      IF g_rxr.rxrconf='X' THEN RETURN END IF
   ELSE
      IF g_rxr.rxrconf<>'X' THEN RETURN END IF
   END IF
   #FUN-D20039 ----------end
...
FUNCTION t624_rxr04(p_cmd,p_type)
DEFINE p_cmd LIKE type_file.chr1
