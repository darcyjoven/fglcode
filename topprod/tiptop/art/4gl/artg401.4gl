...
MAIN
   ...
   LET l_table = cl_prt_temptable('artg401',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artg401_tm(0,0)        # Input print condition
      ELSE CALL artg401()            # Read data and create out-file
   END IF
...
FUNCTION artg401_tm(p_row,p_col)
   ...
   OPEN WINDOW artg401_w AT p_row,p_col WITH FORM "art/42f/artg401" 
    ...
    CONSTRUCT BY NAME tm.wc1 ON azp01
         ...
         BEFORE CONSTRUCT
          ...
          AFTER FIELD azp01 
            ...
            END IF 
            IF g_chk_azp01 THEN
               DECLARE g401_zxy_cs1 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH g401_zxy_cs1 INTO l_zxy03 
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
      ...
      END IF
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artg401_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)               #FUN-BA0063 add
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artg401',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artg401_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)               #FUN-BA0063 add
         EXIT PROGRAM
      END IF
...
FUNCTION artg401()
   ...
   DEFINE l_name    LIKE type_file.chr20,         # External(Disk) file name        
    ...
    IF STATUS THEN                                                                                                                   
       ...
       EXIT PROGRAM                                                                                                                 
    END IF
    DROP TABLE artg401_tmp    
    CREATE TEMP TABLE artg401_tmp( 
           oeaplant LIKE oea_file.oeaplant, 
           azp02    LIKE azp_file.azp02, 
           ima131   LIKE ima_file.ima131,
           oba02    LIKE oba_file.oba02, 
 ...
 CASE tm.a
     WHEN '1'   
      ...
      IF l_lpk01_find = 0 THEN 
            #LET l_sql = "SELECT oeaplant,'',ima131,'',ima01,ima02,",
             LET l_sql = "SELECT oeaplant,'',ima131,'',ima01,ima02,SUM(oeb12),",  #FUN-BA0063 FROM FUN-BB0032
      ...
      OTHERWISE EXIT CASE
     END CASE  
    PREPARE artg401_prepare1 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       ...
       EXIT PROGRAM
    END IF
    DECLARE artg401_curs1 CURSOR FOR artg401_prepare1

    FOREACH artg401_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
    ...
    LET l_sql="SELECT azi10 FROM ",cl_get_target_table(l_plant,'azi_file'),
         ...
         CASE l_azi10
            ...
            WHEN '2'  
               LET sr.oeb14t = sr.oeb14t/sr.oea24 
         END CASE
         INSERT INTO artg401_tmp
         VALUES(sr.oeaplant,l_azp02,sr.ima131,l_oba02,sr.ima01,
        #sr.ima02,sr.oeb14t,sr.lpk01,sr.lpk04)
         sr.ima02,sr.oeb12,sr.oeb14t,sr.lpk01,sr.lpk04)   #FUN-BA0063 FROM FUN-BB0032
         LET l_oba02 =''          
      END FOREACH 
  END FOREACH 
     CASE
   WHEN tm.b='1'
         #LET l_sql = " SELECT '','',ima131,oba02,'','',",
          LET l_sql = " SELECT '','',ima131,oba02,'','',SUM(oeb12),",  #FUN-BA0063 FROM FUN-BB0032
    ...
    END CASE
    PREPARE artg401_prepare2 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       CALL cl_gre_drop_temptable(l_table)               #FUN-BA0063 add
       EXIT PROGRAM
    END IF
    DECLARE artg401_curs2 CURSOR FOR artg401_prepare2

    FOREACH artg401_curs2 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
      ...
      #sr.oba02,sr.ima01,sr.ima02,sr.oeb14t,sr.lpk01,sr.lpk04   #FUN-BA0063 FROM FUN-BB0032  mark
      sr.oba02,sr.ima01,sr.ima02,sr.oeb12,sr.oeb14t,sr.lpk01,sr.lpk04    #FUN-BA0063 FROM FUN-BB0032  add
...
###GENGRE###   LET g_str = tm.wc,";",tm.w,";",tm.x   #FUN-BA0063 FROM FUN-BB0032  add tm.w,tm.x 
   CASE
      WHEN tm.b='1'
###GENGRE###         CALL cl_prt_cs3('artg401','artg401_1',l_sql,g_str)  
         LET g_template = 'artg401'     #FUN-BA0063  add
         CALL artg401_1_grdata()    ###GENGRE###
      WHEN tm.b='2'
###GENGRE###         CALL cl_prt_cs3('artg401','artg401_2',l_sql,g_str) 
         LET g_template = 'artg401_1'     #FUN-BA0063  add
         CALL artg401_grdata()    ###GENGRE###
   END CASE
END FUNCTION

###GENGRE###START
FUNCTION artg401_grdata()
    DEFINE l_sql    STRING
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg401")
        IF handler IS NOT NULL THEN
            START REPORT artg401_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                       ," ORDER BY lower(ima131),lower(oba02)"
            DECLARE artg401_datacur1 CURSOR FROM l_sql
            FOREACH artg401_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg401_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg401_rep
        END IF
...
FUNCTION artg401_1_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg401")
        IF handler IS NOT NULL THEN
            START REPORT artg401_1_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                       ," ORDER BY ima131,oba02,oeaplant,azp02"
            DECLARE artg401_datacur2 CURSOR FROM l_sql
            FOREACH artg401_datacur2 INTO sr1.*
                OUTPUT TO REPORT artg401_1_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg401_1_rep
        END IF
        IF INT_FLAG = TRUE THEN
            LET INT_FLAG = FALSE
            EXIT WHILE
...
REPORT artg401_1_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_display  LIKE type_file.chr1
    DEFINE l_display1 LIKE type_file.chr1
    DEFINE sr1_o      sr1_t
    DEFINE l_oeb12_sum LIKE oeb_file.oeb12      #FUN-BA0063 FROM FUN-BB0032   add
    DEFINE l_oeb14t_sum LIKE oeb_file.oeb14t    #FUN-BA0063 FROM FUN-BB0032   add
    ...
    ON EVERY ROW
        ...
        IF NOT cl_null(sr1_o.ima131) THEN
            ...
            IF NOT cl_null(sr1_o.oba02) AND tm.w = 'N' THEN
        ...
        ON LAST ROW
           #FUN-BA0063 FROM FUN-BB0032---------add------str-----
           LET l_oeb12_sum = SUM(sr1.oeb12)
           PRINTX l_oeb12_sum
           LET l_oeb14t_sum = SUM(sr1.oeb14t)
           PRINTX l_oeb14t_sum
           #FUN-BA0063 FROM FUN-BB0032---------add------end-----
END REPORT

#FUN-BA0063---add----end------------

REPORT artg401_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-BA0063------add----str--
    DEFINE l_display  LIKE type_file.chr1
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            IF NOT cl_null(sr1_o.oba02) THEN
               IF sr1_o.oba02 = sr1.oba02 AND tm.w = 'N' THEN
                  LET l_display1 = 'N'
               LET l_oba02 = "   "         #FUN-CB0058
               ELSE
                  LET l_display1 = 'Y'
