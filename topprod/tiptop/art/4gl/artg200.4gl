...
DEFINE tm          RECORD                          # Print condition RECORD
             ...
             #      wc3     LIKE type_file.chr1000, # Where condition   #TQC-B70068 mark
                   ...
                   w       LIKE type_file.chr1,    #FUN-BA0061(FUN-BB0018) add
                   x       LIKE type_file.chr1,    #FUN-BA0061(FUN-BB0018) add
...
MAIN
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artg200_tm(0,0)        # Input print condition
      ELSE CALL artg200()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
   CALL cl_gre_drop_temptable(l_table)  #FUN-BA0061
END MAIN

FUNCTION artg200_tm(p_row,p_col)
   ...
   OPEN WINDOW artg200_w AT p_row,p_col WITH FORM "art/42f/artg200" 
   ...
   LET tm.wc3 = ' 1=1'
   LET tm.type = '1'
   LET tm.w = 'Y'   #FUN-BA0061(FUN-BB0018) add 
   LET tm.x = 'Y'   #FUN-BA0061(FUN-BB0018) add
   LET tm.a = '1' 
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artg200_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)  #FUN-BA0061
         EXIT PROGRAM
      END IF
      ...
      DISPLAY BY NAME  tm.edate,tm.type,tm.w,tm.x,tm.a,tm.more #FUN-BA0061(FUN-BB0018) add tm.x,tm.w
      
      #INPUT BY NAME tm.edate,tm.type,tm.a,tm.more WITHOUT DEFAULTS
      INPUT BY NAME tm.edate,tm.type,tm.w,tm.x,tm.a,tm.more WITHOUT DEFAULTS  #FUN-BA0061(FUN-BB0018) add tm.x,tm.w
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_copies CLIPPED,"'",
                       " '",tm.wc1 CLIPPED,"'" ,
                       " '",tm.w CLIPPED,"'" , #FUN-BA0061
                       " '",tm.a CLIPPED,"'" , #FUN-BA0061               
                       " '",g_rep_user CLIPPED,"'",           
                       " '",g_rep_clas CLIPPED,"'",           
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'",        
                       " '",tm.x CLIPPED,"'" #FUN-BA0061    
            CALL cl_cmdat('artg200',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artg200_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)  #FUN-BA0061
         EXIT PROGRAM
      END IF
...
#   DEFINE l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT     #TQC-B70068 mark   
   DEFINE l_sql     STRING,               
          sr        RECORD 
                    ...
                    img09 LIKE img_file.img09,   #--單位 #FUN-BA0061
   ...
   LET l_table = cl_prt_temptable('artg200',g_sql) CLIPPED
   IF  l_table = -1 THEN 
   ...
   IF STATUS THEN                                                                                                                   
      ...
      EXIT PROGRAM                                                                                                                 
   END IF
   DROP TABLE artg200_tmp

   CREATE TEMP TABLE artg200_tmp(
          azp01  LIKE azp_file.azp01,
          azp02  LIKE azp_file.azp02,
          ima01  LIKE ima_file.ima01,
          ima02  LIKE ima_file.ima02,
   ...
   FOREACH sel_azp01_cs INTO l_plant,l_azp02  
     ...
     #mark by suncx 20100908 NO.FUN-A80082 end--------
      ...
      IF SQLCA.sqlcode THEN CALL cl_err('prepare1:',SQLCA.sqlcode,1) 
         ...
         EXIT PROGRAM 
      END IF
      DECLARE g200_curs1 CURSOR FOR g200_prepare1
      FOREACH g200_curs1 INTO sr.azw01,sr.img01,sr.img02,sr.img03,sr.img04,
                              sr.tmp01,sr.tmp02,sr.ima01,sr.ima02,sr.ima131,
                              sr.img09  #FUN-BA0061 add
         SELECT COUNT(*) INTO l_cnt FROM artg200_tmp  #--己存在不覆蓋
         ...
         IF SQLCA.sqlcode THEN                                                     
            LET l_oba02 = ''                                                                                                        
         END IF
         INSERT INTO artg200_tmp 
         VALUES(sr.azw01,l_azp02,sr.ima01,sr.ima02,sr.ima131,l_oba02,sr.img02,sr.img09,l_img10,l_amt)#FUN-BA0061
      END FOREACH
   END FOREACH 
   CASE
      WHEN tm.a='1'
         LET l_sql = "SELECT azp01,azp02,'','','','',img02,'',SUM(img10),SUM(amt) ",#FUN-BA0061 add after img02,''
                     " FROM artg200_tmp ",
                     " GROUP BY azp01,azp02,img02"
      WHEN tm.a='2'
         LET l_sql = "SELECT azp01,azp02,'','',ima131,oba02,img02,'',SUM(img10),SUM(amt) ",#FUN-BA0061 add after img02,''
                     " FROM artg200_tmp ",
   ...
   PREPARE artg200_prepare2 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE artg200_curs2 CURSOR FOR artg200_prepare2

   FOREACH artg200_curs2 INTO sr.azw01,l_azp02,sr.ima01,sr.ima02,
                              sr.ima131,l_oba02,sr.img02,sr.img09,l_img10,l_amt#FUN-BA0061 add img09
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
      SELECT SUM(amt) INTO l_amts FROM artg200_tmp WHERE azp01 = sr.azw01
      IF l_amts  <> 0 THEN
...
#              LET g_str = g_str,";",tm.w,";",tm.x  #FUN-BA0061 mark
   CASE
      WHEN tm.a='1'
###GENGRE###         CALL cl_prt_cs3('artg200','artg200_1',l_sql,g_str)  
    LET g_template = 'artg200'
    CALL artg200_grdata()    ###GENGRE###
      WHEN tm.a='2'
###GENGRE###         CALL cl_prt_cs3('artg200','artg200_2',l_sql,g_str) 
    LET g_template = 'artg200_1'
    CALL artg200_grdata()    ###GENGRE###
      WHEN tm.a='3'
###GENGRE###         CALL cl_prt_cs3('artg200','artg200_3',l_sql,g_str) 
    LET g_template = 'artg200_2'
    CALL artg200_grdata()    ###GENGRE###
   END CASE
END FUNCTION 
#FUN-A80082
...
FUNCTION artg200_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg200")
        IF handler IS NOT NULL THEN
            START REPORT artg200_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
            #FUN-BA0061 add start----------------------
            CASE
               WHEN tm.a='1'
            ...
            DECLARE artg200_datacur1 CURSOR FROM l_sql
            FOREACH artg200_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg200_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg200_rep
        END IF
...
#FUN-BA0061----------STAR--------
    ...
    DEFINE l_per         LIKE type_file.chr4 
    DEFINE l_per1        STRING 
    DEFINE l_img10_sum   LIKE img_file.img10
    DEFINE l_amt_sum     LIKE type_file.num15_3
    DEFINE l_percent_sum STRING 
    DEFINE l_img10_tot   LIKE img_file.img10 
    DEFINE l_amt_tot     LIKE type_file.num15_3
    DEFINE l_percent_tot STRING 
#FUN-BA0061-----------END------
    DEFINE l_azp02   LIKE azp_file.azp02   #FUN-CB0051
    DEFINE l_azp01   LIKE azp_file.azp01   #FUN-CB0051
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
            PRINTX g_str       #FUN-BA0061
            LET sr1_o.azp01 = NULL    #FUN-B80161  
            LET sr1_o.azp02 = NULL    #FUN-B80161  
        ...
        ON EVERY ROW
            ...
            IF NOT cl_null(sr1_o.azp01) THEN
               IF sr1_o.azp01 = sr1.azp01 THEN
                  LET l_display = 'N'
                  LET l_azp01 = ' '  #FUN-CB0051 
               ELSE
                  LET l_display = 'Y'
                  LET l_azp01 = sr1.azp01  #FUN-CB0051 
               END IF
            ELSE
               LET l_display = 'Y'
               LET l_azp01 = sr1.azp01  #FUN-CB0051 
            ...
            IF NOT cl_null(sr1_o.azp02) THEN
               IF sr1_o.azp02 = sr1.azp02 THEN
                  LET l_display1 = 'N'
                  LET l_azp02 = ' '  #FUN-CB0051 
               ELSE
                  LET l_display1 = 'Y'
                  LET l_azp02 = sr1.azp02  #FUN-CB0051 
               END IF
            ELSE
               LET l_display1 = 'Y'
               LET l_azp02 = sr1.azp02  #FUN-CB0051 
            END IF
            PRINTX l_display1
            PRINTX l_azp01   #FUN-CB0051
            PRINTX l_azp02   #FUN-CB0051
            LET sr1_o.* = sr1.*    
            LET l_per = sr1.percent
            LET l_per1 =l_per,'%'
            PRINTX l_per1
        ...
        AFTER GROUP OF sr1.azp01
            ...
            LET l_percent_sum = GROUP SUM(sr1.percent),'%'
            PRINT l_percent_sum
        ON LAST ROW
            ...
            LET l_percent_tot = SUM(sr1.percent),'%'
            PRINT l_percent_tot
END REPORT
###GENGRE###END
