...
MAIN
   ...
   LET l_table = cl_prt_temptable('artg520',g_sql) CLIPPED
   IF l_table = -1 THEN
   ...
   LET g_chk_auth = ""
   DECLARE g520_zxy_cs CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
   FOREACH g520_zxy_cs INTO l_zxy03
...
DEFINE p_row,p_col    LIKE type_file.num5,   
   ...
   END IF
   OPEN WINDOW g520_w AT p_row,p_col
        WITH FORM "art/42f/artg520"
   ...
   LET g_bgjob = 'N'
   LET g_copies = '1'
   LET g_pdate = g_today  #FUN-BA0062 add
WHILE TRUE
   WHILE TRUE
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,
                         ...
                         " '",g_template CLIPPED,"'",           
                         " '",g_rpt_name CLIPPED,"'"            
         CALL cl_cmdat('artg520',g_time,l_cmd)
      END IF
      CLOSE WINDOW g520_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)   #FUN-BA0062
...
DEFINE l_db_type  STRING       #FUN-B40029
     ...
     DROP TABLE artg520_tmp
     CREATE TEMP TABLE artg520_tmp(
                    azp01 LIKE azp_file.azp01,         #--營運中心編號
                    azp02 LIKE azp_file.azp02,         #--營運中心名稱
                    ima131 LIKE ima_file.ima131,       #--產品分類
                    oba02 LIKE oba_file.oba02,         #--分類名稱
     ...
     FOREACH sel_azp01_cs INTO l_azp01,l_azp02      
        ...
        PREPARE g520_prepare1 FROM l_sql
        IF SQLCA.sqlcode THEN CALL cl_err('prepare1:',SQLCA.sqlcode,1) 
           CALL cl_used(g_prog,g_time,2) RETURNING g_time 
           CALL cl_gre_drop_temptable(l_table)   #FUN-BA0062
           EXIT PROGRAM 
        END IF
        DECLARE g520_curs1 CURSOR FOR g520_prepare1
...
#                      "UNION SELECT ohb09,ohb092,(CASE WHEN ohb12 IS NULL THEN 0 ELSE ohb12 END)*(-1),(CASE WHEN ohb14t IS NULL THEN 0 ELSE ohb14t END)*(-1),oha02,'',ohb01,ohb03,oha24 ", #TQC-B70204 mark
        ...
        CALL cl_replace_sqldb(l_sql) RETURNING l_sql
        CALL cl_parse_qry_sql(l_sql,l_azp01) RETURNING l_sql 
        PREPARE g520_prepare2 FROM l_sql
        IF SQLCA.sqlcode THEN CALL cl_err('prepare2:',SQLCA.sqlcode,1) 
           CALL cl_used(g_prog,g_time,2) RETURNING g_time 
           CALL cl_gre_drop_temptable(l_table)   #FUN-BA0062
           EXIT PROGRAM 
        END IF
        DECLARE g520_curs2 CURSOR FOR g520_prepare2

      LET l_n =0 
      FOREACH g520_curs1 INTO sr.ima01,sr.ima02,sr.ima131,sr.oba02
        LET sr.azp01 = l_azp01
        LET sr.azp02 = l_azp02
        
        LET start_qty = 0
        LET end_qty = 0
        CALL g520_avg(g_yy1,g_mm1,l_azp01,sr.ima01,tm.date1) RETURNING start_qty
        CALL g520_avg(g_yy2,g_mm2,l_azp01,sr.ima01,tm.date2) RETURNING end_qty
        ...
        LET l_ccc23 = 0
        LET l_ogb14t = 0
        OPEN g520_curs2 USING sr.ima01,sr.ima01
        FOREACH g520_curs2 INTO l_ogb09,l_ogb092,l_ogb12,l_ogb14t,l_oga02,l_ogb41,l_ogb01,l_ogb03,l_oga24
        ...
        INSERT  INTO artg520_tmp VALUES(sr.*)
    ...
    CASE
      ...
      END CASE
      PREPARE g520_prepare3 FROM l_sql
      IF SQLCA.sqlcode != 0 THEN
         ...
         EXIT PROGRAM
      END IF
      DECLARE g520_curs3 CURSOR FOR g520_prepare3

      FOREACH g520_curs3 INTO sr.*
        #周轉率=銷售成本/平均庫存
...
###GENGRE###     LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED
     CASE
        WHEN tm.style='1'
###GENGRE###           CALL cl_prt_cs3('artg520','artg520_1',l_sql,g_str)  
    LET g_template = 'artg520'
    CALL artg520_grdata()    ###GENGRE###
        WHEN tm.style='2'
###GENGRE###           CALL cl_prt_cs3('artg520','artg520_2',l_sql,g_str)
    LET g_template = 'artg520_1'
    CALL artg520_1_grdata()    ###GENGRE###
        WHEN tm.style='3'
###GENGRE###           CALL cl_prt_cs3('artg520','artg520_3',l_sql,g_str)         
    LET g_template = 'artg520_2'
    CALL artg520_2_grdata()    ###GENGRE###
     END CASE
END FUNCTION


FUNCTION g520_avg(l_yy_t,l_mm_t,l_azp01,l_ima01,l_date) 
   DEFINE l_azp01  LIKE azp_file.azp01
   DEFINE l_ima01  LIKE ima_file.ima01
   DEFINE l_imk09  LIKE imk_file.imk09
   DEFINE l_tlf10  LIKE tlf_file.tlf10
...
FUNCTION artg520_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg520")
        IF handler IS NOT NULL THEN
            START REPORT artg520_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY azp01" 
          
            DECLARE artg520_datacur1 CURSOR FROM l_sql
            FOREACH artg520_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg520_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg520_rep
        END IF
...
FUNCTION artg520_1_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg520")
        IF handler IS NOT NULL THEN
            START REPORT artg520_1_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY azp01,ima131"

            DECLARE artg520_datacur2 CURSOR FROM l_sql
            FOREACH artg520_datacur2 INTO sr1.*
                OUTPUT TO REPORT artg520_1_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg520_1_rep
        END IF
        IF INT_FLAG = TRUE THEN
            LET INT_FLAG = FALSE
            EXIT WHILE
...
FUNCTION artg520_2_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg520")
        IF handler IS NOT NULL THEN
            START REPORT artg520_2_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY azp01,ima01"

            DECLARE artg520_datacur3 CURSOR FROM l_sql
            FOREACH artg520_datacur3 INTO sr1.*
                OUTPUT TO REPORT artg520_2_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg520_2_rep
        END IF
        IF INT_FLAG = TRUE THEN
            LET INT_FLAG = FALSE
            EXIT WHILE
...
REPORT artg520_1_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE sr1_o sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_display   STRING          #FUN-BA0062  
    DEFINE l_display1  STRING          #FUN-BA0062  
    DEFINE l_display2  STRING          #FUN-BA0062  
    DEFINE l_azp01      LIKE azp_file.azp01        #FUN-CB0058
    ...
    ORDER EXTERNAL BY sr1.azp01,sr1.ima131
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
            LET sr1_o.ima131 = NULL #FUN-BA0062 
            LET sr1_o.azp01 = NULL #FUN-BA0062 

        BEFORE GROUP OF sr1.azp01
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-BA0062----add----str--------
            IF NOT cl_null(sr1_o.azp01) THEN
               IF sr1_o.azp01 != sr1.azp01 THEN
                  LET l_display = 'Y'
                  LET l_azp01 = sr1.azp01        #FUN-CB0058
                  LET l_azp02 = sr1.azp02        #FUN-CB0058
               ELSE
                  LET l_display = 'N'
                  LET l_azp01 = "    "           #FUN-CB0058
            ...
            IF NOT cl_null(sr1_o.ima131) AND NOT cl_null(sr1_o.azp01) THEN
               IF sr1_o.ima131 = sr1.ima131 AND sr1_o.azp01 = sr1.azp01 THEN
                  LET l_display1 = 'N'
            ...
            IF cl_null(tm.wc) OR tm.wc = "" THEN
               LET l_display2 = 'N'
            ELSE
               LET l_display2 = 'Y'
            END IF
...
REPORT artg520_2_rep(sr1)
    ...
    DEFINE l_azp01      LIKE azp_file.azp01        #FUN-CB0058
    DEFINE l_azp02      LIKE azp_file.azp02        #FUN-CB0058
    DEFINE l_ima01      LIKE ima_file.ima02        #FUN-CB0058
    DEFINE l_ima02      LIKE ima_file.ima02        #FUN-CB0058

    ORDER EXTERNAL BY sr1.azp01,sr1.ima01
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX g_user,g_pdate,g_prog,g_company,g_ptime,g_user_name          #FUN-BA0062 add g_ptime,g_user_name
            PRINTX tm.*
            LET sr1_o.ima01 = NULL #FUN-BA0062 
            LET sr1_o.ima02 = NULL #FUN-BA0062 
            LET sr1_o.azp01 = NULL #FUN-BA0062 
            LET sr1_o.azp02 = NULL #FUN-BA0062 
        ...
        ON EVERY ROW
            ...
            IF NOT cl_null(sr1_o.ima01) AND NOT cl_null(sr1_o.azp01) THEN
               IF sr1_o.ima01 = sr1.ima01 AND sr1_o.azp01 = sr1.azp01 THEN
                  LET l_display2 = 'N'
            ...
            IF NOT cl_null(sr1_o.ima02) AND NOT cl_null(sr1_o.azp01) THEN
               IF sr1_o.ima02 = sr1.ima02 AND sr1_o.azp01 = sr1.azp01 THEN
                  LET l_display3 = 'N'
...
REPORT artg520_rep(sr1)
    ...
    FORMAT
        ...
        ON EVERY ROW
            ...
            IF NOT cl_null(sr1_o.azp02) AND NOT cl_null(sr1_o.azp01) THEN
               IF sr1_o.azp02 = sr1.azp02 AND sr1_o.azp01 = sr1.azp01 THEN
                  LET l_display1 = 'N'
                  LET l_azp02 = "   "      #FUN-CB0058
               ELSE
                  LET l_display1 = 'Y'
