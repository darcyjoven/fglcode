...
# Modify.........: No.FUN-D30098 13/03/29 By xumm 逻辑调整
...
MAIN
   ...
   LET l_table = cl_prt_temptable('artr492',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artr492_tm(0,0)        # Input print condition
      ELSE CALL artr492()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN

FUNCTION artr492_tm(p_row,p_col)
   DEFINE lc_qbe_sn      LIKE gbm_file.gbm01
   DEFINE p_row,p_col    LIKE type_file.num5,
          l_cmd          LIKE type_file.chr1000 
   LET p_row = 6 LET p_col = 16
 
   OPEN WINDOW artr492_w AT p_row,p_col WITH FORM "art/42f/artr492" 
...
#FUN-C70007 add end ---
     #TQC-B60017---
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artr492_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
...
#        CALL cl_err('','9046',0) CONTINUE WHILE          #FUN-C70007 MARK
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('artr492',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr492_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
#TQC-B60017 ADD - BEGIN ---------------------------------------------------
    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'artr492'
...
#FUN-C70007 MARK END ----
    END CASE
    PREPARE artr492_prepare1 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       EXIT PROGRAM
    END IF
    DECLARE artr492_curs1 CURSOR FOR artr492_prepare1

    FOREACH artr492_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
...
#FUN-C70007 add begin ---
   CASE
      WHEN tm.a = '1'
           LET l_sql = " SELECT lpx01,lpx02,SUM(lpx10) AS lpx10",
                       "   FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                       "  GROUP BY lpx01,lpx02 ORDER BY lpx01 "
      WHEN tm.a = '2'
           LET l_sql = " SELECT lpx01,lpx02,azw01,azw08,SUM(lpx10) AS lpx10",
                       "   FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                       "  GROUP BY lpx01,lpx02,azw01,azw08 ORDER BY lpx01 "
      WHEN tm.a = '3'
           LET l_sql = " SELECT lpx01,lpx02,gen01,gen02,SUM(lpx10) AS lpx10 ",
                       "   FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                       "  GROUP BY lpx01,lpx02,gen01,gen02 ORDER BY lpx01 "
   END CASE 
#FUN-C70007 add end -----
#TQC-B60017 ADD - BEGIN ---------------------------------------------------
   IF g_zz05 = 'Y' THEN
      CALL cl_wcchp(tm.wc,'azp01')
           RETURNING tm.wc
      LET g_str = tm.wc
   END IF
#TQC-B60017 ADD -  END  ---------------------------------------------------
#  LET g_str = tm.wc   #TQC-B60017 MARK
   CASE
      WHEN tm.a = '1'
         CALL cl_prt_cs3('artr492','artr492_1',l_sql,g_str) 
      WHEN tm.a = '2'
         CALL cl_prt_cs3('artr492','artr492_2',l_sql,g_str)
      WHEN tm.a = '3'
         CALL cl_prt_cs3('artr492','artr492_3',l_sql,g_str)
   END CASE
END FUNCTION
