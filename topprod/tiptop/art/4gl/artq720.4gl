...
# Modify.........: No.TQC-A10045 10/01/07 By destiny DB取值请取实体DB 
...
DEFINE g_zxy03        LIKE zxy_file.zxy03    #No.TQC-A10045
...
MAIN
   ...
   SELECT ruwplant,azp02,ruw02,ruw01,ruw03,ruw04,ruw05,imd02,jce02,
       ...
       FROM ruw_file,azp_file,gen_file,imd_file,jce_file
        WHERE 1=0  INTO TEMP temp_q720
 
   LET p_row = 2 LET p_col = 3
 
   OPEN WINDOW q720_w AT p_row,p_col WITH FORM "art/42f/artq720"
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         WHEN "pancha_detail"
            IF cl_chk_act_auth() THEN
               IF g_rec_b != 0 THEN
                  LET l_cmd = "artq730 '",g_ruw[l_ac].ruw03,"' '",g_ruw[l_ac].ruwplant,"'"
...
FUNCTION q720_q()
   ...
   DELETE FROM temp_q720
 
   CALL q720_b_askkey()
 
END FUNCTION
 
FUNCTION q720_b_askkey()
   ...
   ELSE
      CONSTRUCT g_wc ON ruwplant,ruw02,ruw01,ruw03,ruw04,ruw05,ruw06, 
                     ...
                     FROM s_ruw[1].ruwplant,s_ruw[1].ruw02,s_ruw[1].ruw01,
                        ...
                        s_ruw[1].ruwcont,s_ruw[1].ruwconu,s_ruw[1].ruw07   #FUN-B50042 remvoe POS                     
         BEFORE CONSTRUCT
            CALL cl_qbe_init()
   ...
   ELSE
      CONSTRUCT g_wc2 ON rux02,rux03,rux04,rux05,rux06,rux07,rux08,rux09
       ...
       IF g_chk_ruwplant THEN
          DECLARE q720_zxy_cs CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
          FOREACH q720_zxy_cs INTO l_zxy03
...
#TQC-B90175 add END--------------------------------
    ...
    END IF
    LET l_ac = 1 
    CALL q720_b_fill(g_wc)
    CALL q720_b2_fill(g_wc2)
   
END FUNCTION
 
FUNCTION q720_b_fill(p_wc)              
DEFINE p_wc STRING        
...
#       LET g_sql = g_sql," AND azp01 = '",g_argv2,"'"
    ...
    END IF
    PREPARE q720_pre FROM g_sql
    DECLARE q720_DB_cs CURSOR FOR q720_pre
    LET g_sql2 = ''
    #FOREACH q720_DB_cs INTO g_dbs
     FOREACH q720_DB_cs INTO g_zxy03
       IF SQLCA.sqlcode THEN
           CALL cl_err('foreach:q720_DB_cs',SQLCA.sqlcode,1)
           EXIT FOREACH
       END IF
#TQC-B90175 add START------------------------------
...
#TQC-B90175 add END--------------------------------
    ...
    #NO.TQC-A10043--end
       ...
       #FUN-A50102 ---mark---end---
       LET g_sql = "INSERT INTO temp_q720 ",
                   ...
                   " ON azp01 = ruwplant) LEFT JOIN ",cl_get_target_table(g_zxy03, 'gen_file'), #FUN-A50102
                   " B ON ruwconu = B.gen01 ) LEFT JOIN ",
       ...
       #IF SQLCA.SQLCODE THEN                       #FUN-870100
       #   CALL cl_err('',SQLCA.SQLCODE,1)          #FUN-870100
       #   RETURN                                   #FUN-870100
       #END IF                                      #FUN-870100
    END FOREACH
    #去掉表中重复的（只保留一)    
    DELETE FROM temp_q720 WHERE ruw01 IN (
       SELECT a.ruw01 FROM temp_q720 a
         WHERE a.ruw01 != (
          (SELECT max(b.ruw01) FROM temp_q720 b
              WHERE a.ruw01 = b.ruw01 and 
                a.ruwplant = b.ruwplant)))
 
    LET g_sql = "SELECT * FROM temp_q720 "
    PREPARE q720_pb FROM g_sql
    DECLARE q720_cs CURSOR FOR q720_pb
 
    CALL g_ruw.clear()
    LET g_cnt = 1
    MESSAGE "Searching!"
    FOREACH q720_cs INTO g_ruw[g_cnt].*  
        IF STATUS THEN 
           CALL cl_err('foreach q720_cs:',STATUS,1) 
           EXIT FOREACH
        END IF
        LET g_cnt = g_cnt + 1
...
DEFINE l_dbs LIKE azp_file.azp03
    ...
    PREPARE q720_pb2 FROM g_sql
    DECLARE q720_cs2 CURSOR FOR q720_pb2
        
    CALL g_rux.clear()
    LET g_cnt = 1
    MESSAGE "Searching!"
    FOREACH q720_cs2 INTO g_rux[g_cnt].*  
        IF STATUS THEN 
           CALL cl_err('foreach q720_cs2:',STATUS,1) 
           EXIT FOREACH
        END IF
        
        #LET g_sql = "SELECT ima02 FROM ",l_dbs,".ima_file ",     #No.TQC-A10096
        #LET g_sql = "SELECT ima02 FROM ",l_dbs,"ima_file ",     #No.TQC-A10096 #FUN-A50102
