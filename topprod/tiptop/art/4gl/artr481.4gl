...
MAIN
   ...
   LET l_table = cl_prt_temptable('artr481',g_sql) CLIPPED
   IF  l_table = -1 THEN  EXIT PROGRAM  END IF           
   CALL cl_used(g_prog,g_time,1) RETURNING g_time

   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artr481_tm(0,0)        # Input print condition
      ELSE CALL artr481()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN   
FUNCTION artr481_tm(p_row,p_col)
   ...
   OPEN WINDOW artr481_w AT p_row,p_col WITH FORM "art/42f/artr481" 
...
#               END IF 
#               IF g_chk_shop THEN
#                  DECLARE r481_zxy_cs1 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
#                  FOREACH r481_zxy_cs1 INTO l_zxy03 
#                    IF g_chk_auth IS NULL THEN
#                       LET g_chk_auth = "'",l_zxy03,"'"
#                    ELSE
#                       LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
...
#         END IF
#         IF INT_FLAG THEN
#            LET INT_FLAG = 0 CLOSE WINDOW artr481_w 
#            CALL cl_used(g_prog,g_time,2) RETURNING g_time 
#            EXIT PROGRAM
#         END IF
#   
...
#FUN-C70007 add begin ---
         ...
         ON ACTION accept
            ...
            END IF
            IF g_chk_shop THEN
               DECLARE r481_zxy_cs2 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH r481_zxy_cs2 INTO l_zxy03
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artr481',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr481_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
...
END CASE
  ...
  #FUN-BC0026 add START
   ...
   END IF
  #FUN-BC0026 add END
  CALL cl_prt_cs1('artr481','artr481',l_sql,g_str)  
END FUNCTION
...
DEFINE l_cnt      LIKE type_file.num5 #FUN-CA0087 Add
  ...
  #FUN-AA0024 add ###end###
     ...
     #         ELSE
     #         #FUN-B40029-add-end--
     #      LET l_sql = "SELECT SHOP,SNO,'','','',SUM(PRICE) ",
     #                  " FROM ",g_posdb,"POSDB",g_posdb_link,
     ...
     #FUN-C70007 add begin ---
        ...
        #FUN-CA0087 Mark End -----
         IF l_db_type='MSV' THEN
            ...
            #LET l_sql = "SELECT SHOP,ima131,'','','', SUM(PRICE)",
            ...
            #            " AND ",tm.wc CLIPPED,
            #            " GROUP BY SHOP,ima131"    
             LET l_sql = "SELECT td_Sale_Detail.SHOP,ima131,'','','',SUM(CASE Type WHEN 0 THEN AMT ELSE AMT*(-1) END) ",
                         " FROM ",g_posdb,"td_Sale_Detail",g_posdb_link,
                         ...
                         "   AND Type IN ('0','1','2')",
                         "   AND ",tm.wc CLIPPED,
                         " GROUP BY td_Sale_Detail.SHOP,ima131 "
     ...
     #FUN-C70007 add end  ---
         ...
         PREPARE artr481_prepare1 FROM l_sql
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('prepare:',SQLCA.sqlcode,1) 
            CALL cl_used(g_prog,g_time,2) RETURNING g_time 
            EXIT PROGRAM
         END IF
         DECLARE artr481_curs1 CURSOR FOR artr481_prepare1
         
         #FUN-AA0024  add  -------------------begin---------------------    
         IF tm.d = 'Y' THEN
            LET l_i = 1
            FOREACH artr481_curs1 INTO l_sr[l_i].*
               IF SQLCA.sqlcode != 0 THEN
           ...
           #       LET l_sr[l_i].SHOP = l_azp01
           #       LET l_sr[l_i].PRICE = 0
           #     
           # END IF   
           #FUN-C70007 mark end ----- 
        ...
        #FUN-CA0087 Add Begin ---
         IF tm.d = 'Y' THEN
            ...
            IF l_cnt > 0 AND l_cnt <  l_n THEN
               ...
               LET l_sql = "SELECT SNO FROM ",g_cr_db_str CLIPPED,l_table CLIPPED," WHERE SHOP = '",l_azp01,"' "
               PREPARE sel_sno_pre FROM l_sql
               EXECUTE sel_sno_pre INTO sr.sno
               LET t = 0
               LET btime = 0
               LET etime = 0
               WHILE t < 24
