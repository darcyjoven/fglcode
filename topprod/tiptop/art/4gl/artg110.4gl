...
MAIN
   ...
   CALL cl_used(g_prog,g_time,1) RETURNING g_time   #FUN-BA0061--add--
...
###GENGRE###   CALL cl_prt_cs3('artr110','artr110',l_sql,g_str) 
    CALL artg110_grdata()    ###GENGRE###
...
FUNCTION artg110_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg110")
        IF handler IS NOT NULL THEN
            START REPORT artg110_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY oba13"    #FUN-BA0061
          
            DECLARE artg110_datacur1 CURSOR FROM l_sql
            FOREACH artg110_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg110_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg110_rep
        END IF
...
REPORT artg110_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
#FUN-BA0061----STAR------
    DEFINE l_oba01_1     STRING
    DEFINE l_oba01       STRING
    DEFINE l_oba13       STRING
    DEFINE l_oba13_1     STRING
    DEFINE l_oba01oba01  STRING
    DEFINE l_p_level     STRING
    DEFINE l_p_level1    STRING
    DEFINE l_str         STRING
    DEFINE l_cnt         LIKE type_file.chr1 
    DEFINE l_sql         STRING
#FUN-BA0061------END-------
    ORDER EXTERNAL BY sr1.l_no
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.l_no

            LET l_sql = "SELECT COUNT(*) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
            DECLARE artg110_repcur1 CURSOR FROM l_sql
            FOREACH artg110_repcur1 INTO l_cnt END FOREACH
        ...
        ON EVERY ROW
            ...
            LET l_p_level = sr1.p_level
            LET l_oba01 = sr1.oba01,l_p_level
            PRINTX l_oba01
      
            LET l_p_level1 = sr1.p_level - 1
            LET l_oba13 = sr1.oba13,l_p_level1
            PRINTX l_oba13
            
            IF sr1.p_level = 0 THEN
               LET l_oba01oba01 = sr1.oba01
            ELSE
               IF l_cnt = l_lineno  THEN
                  LET l_str = sr1.p_level * 3
                  LET l_oba01oba01 =l_str spaces,'└',sr1.oba01
               ELSE 
                  IF l_oba13_1 = l_oba13 THEN
                     LET l_str = sr1.p_level * 3
                     LET l_oba01oba01 = l_str spaces,'├',sr1.oba01
                  ELSE 
                     LET l_str = sr1.p_level * 3
            ...
            LET l_oba13_1 = l_oba13
            PRINTX sr1.*

        AFTER GROUP OF sr1.l_no
