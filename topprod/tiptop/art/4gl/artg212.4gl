...
# Modify.........: No.FUN-C40071 12/04/23 By qirl  明細類CR轉換成GRW
...
MAIN
   ...
   LET g_sql=  "order1.type_file.num10,",
    ...
    LET l_table = cl_prt_temptable('artg212',g_sql) CLIPPED
    ...
    IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN 
       CALL artg212_tm()        # Input print condition
    ELSE 
       CALL artg212() 
    END IF
...
DEFINE l_sql          STRING
   ...
   OPEN WINDOW artg212_w AT p_row,p_col WITH FORM "art/42f/artg212" 
   ...
   WHILE TRUE
      CONSTRUCT BY NAME tm.wc ON ruw02,ruw04,ruwplant,rtz10,ima131,rty05
        BEFORE CONSTRUCT
         ...
         AFTER CONSTRUCT
            IF INT_FLAG THEN
               LET INT_FLAG = 0 CLOSE WINDOW artg212_w 
               CALL cl_used(g_prog,g_time,2) RETURNING g_time 
               CALL cl_gre_drop_temptable(l_table)   #FUN-C40071 add      
               EXIT PROGRAM
            END IF  
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd1 CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artg212',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artg212_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)   #FUN-C40071 add
         EXIT PROGRAM
      END IF
...
DEFINE l_cost    LIKE type_file.num10
    DROP TABLE artg212_tmp
    CREATE TEMP TABLE artg212_tmp(
                order1 LIKE type_file.num10,
                azw01 LIKE azw_file.azw01,
                azw08 LIKE azw_file.azw08, 
                rux03 LIKE rux_file.rux03, 
...
#       LET l_sql = l_sql,"   UNION SELECT ruwplant,ruw05,rux03,ima02,ima25,ima131,rux01,rux02,rux04,rux05,rux06,rux07,rux08 FROM ", #TQC-B70204 mark
      ...
      PREPARE artg212_prepare1 FROM l_sql                  
      IF SQLCA.sqlcode != 0 THEN                           
         ...
         EXIT PROGRAM                                      
      END IF                                               
      DECLARE artg212_curs1 CURSOR FOR artg212_prepare1 
      FOREACH artg212_curs1 INTO sr.*  
         IF SQLCA.sqlcode != 0 THEN
         ...
         INSERT INTO artg212_tmp VALUES(0,sr.ruwplant,l_azw08,sr.rux03,sr.ima02,sr.ima131,l_oba02,sr.rux01,sr.rux02,
   ...
   IF tm.type1='1' THEN 
      IF tm.type2='1' THEN
         LET l_sql = "SELECT rux03,rux04,SUM(income) FROM artg212_tmp "
      END IF
      IF tm.type2='2' THEN
         LET l_sql = "SELECT rux03,rux04,SUM(cost) FROM artg212_tmp "
      END IF
      IF tm.type2='3' THEN
         LET l_sql = "SELECT rux03,rux04,SUM(rux08) FROM artg212_tmp "
      END IF
      IF tm.cb1 = 'Y' THEN
      ...
      IF tm.type2 = '1' OR tm.type2 = '2' OR tm.type2='3' THEN
         ...
         DECLARE g212_sum_curs CURSOR FOR sel_sum_pre
         FOREACH g212_sum_curs INTO l_rux03,l_rux04,l_order1
            UPDATE artg212_tmp SET order1 = l_order1
             WHERE rux03 = l_rux03
               AND rux04 = l_rux04
         END FOREACH
      END IF
   ...
   IF tm.type1='2' THEN
      IF tm.type2='1' THEN
         LET l_sql = "SELECT ima131,rux04,SUM(income) FROM artg212_tmp "
      END IF
      IF tm.type2='2' THEN
         LET l_sql = "SELECT ima131,rux04,SUM(cost) FROM artg212_tmp "
      END IF
      IF tm.type2='3' THEN
         LET l_sql = "SELECT ima131,rux04,SUM(rux08) FROM artg212_tmp "
      END IF
      IF tm.cb1 = 'Y' THEN
         LET l_sql = l_sql," WHERE rux08 <> 0 "
      END IF
      ...
      IF tm.type2 = '1' OR tm.type2 = '2' OR tm.type2='3' THEN
         ...
         DECLARE g212_sum_curs1 CURSOR FOR sel_sum_pre1
         FOREACH g212_sum_curs1 INTO l_ima131,l_rux04,l_order1
   ...
   DECLARE g212_curs CURSOR FOR SELECT * FROM artg212_tmp
   FOREACH g212_curs INTO l_order1,sr.ruwplant,l_azw08,sr.rux03,sr.ima02,sr.ima131,l_oba02,sr.rux01,sr.rux02,
                 sr.rux04,sr.rux05,sr.rux06,sr.rux07,sr.rux08,l_rtg05,l_income,l_ccc23,l_cost
...
###GENGRE###   LET g_str = g_str,";",tm.type1,";",tm.type2,';',tm.cb1,";",tm.cb2,";"
    CASE
      WHEN tm.type1='1'
###GENGRE###         CALL cl_prt_cs3('artg212','artg212_1',l_sql,g_str)  
      LET g_template = 'artg212_1' #FUN-C40071 add
    CALL artg212_1_grdata(l_sql)    ###GENGRE###
      WHEN tm.type1='2'
###GENGRE###         CALL cl_prt_cs3('artg212','artg212_2',l_sql,g_str) 
      LET g_template = 'artg212_2' #FUN-C40071 add
    CALL artg212_2_grdata(l_sql)    ###GENGRE###
    END CASE
...
###GENGRE###START
FUNCTION artg212_1_grdata(l_sql)
    DEFINE l_sql    STRING
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg212")
        IF handler IS NOT NULL THEN
            START REPORT artg212_1_rep TO XML HANDLER handler
            #FUN-C40071---add--str--
            IF tm.type2 = '1' OR tm.type2 = '2' OR tm.type2 = '3' THEN 
               LET l_sql = l_sql," ,rux03,rux04"
            END IF
            #FUN-C40071---add--end--
            DECLARE artg212_datacur1 CURSOR FROM l_sql
            FOREACH artg212_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg212_1_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg212_1_rep
        END IF
...
FUNCTION artg212_2_grdata(l_sql)
    DEFINE l_sql    STRING
    DEFINE handler  om.SaxDocumentHandler
    DEFINE sr1      sr1_t
    DEFINE l_cnt    LIKE type_file.num10
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg212")
        IF handler IS NOT NULL THEN
            START REPORT artg212_2_rep TO XML HANDLER handler
            #FUN-C40071---add--str--
            IF tm.type2 = '1' OR tm.type2 = '2' OR tm.type2 = '3' THEN 
               LET l_sql = l_sql," ,ima131,rux04"
            END IF
            #FUN-C40071---add--end--
            DECLARE artg212_datacur2 CURSOR FROM l_sql
            FOREACH artg212_datacur2 INTO sr1.*
                OUTPUT TO REPORT artg212_2_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg212_2_rep
        END IF
        IF INT_FLAG = TRUE THEN
            LET INT_FLAG = FALSE
            EXIT WHILE
...
REPORT artg212_2_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE sr3 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-C40071--add---star----
    DEFINE l_rux04_rux05_sum       LIKE type_file.num20_6
    DEFINE l_rux04_rux06_07_sum    LIKE type_file.num20_6
    DEFINE l_rux04_rux08_sum       LIKE type_file.num20_6
    DEFINE l_rux04_income_sum      LIKE type_file.num20_6
    DEFINE l_rux04_cost_sum        LIKE type_file.num20_6
    DEFINE l_sql             STRING
    DEFINE l_subrep01_2_show LIKE type_file.chr1 
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.rux04
            ...
            #FUN-C40071 add --str
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " WHERE rux04 = '",sr1.rux04,"' AND ima131 = '",sr1.ima131,"'"
            IF tm.cb1 = 'Y' THEN
            ...
            IF tm.type2 = '5' THEN
               LET l_sql = l_sql," ORDER BY ima131"
            END IF
            START REPORT artg212_subrep02
            DECLARE artg212_repcur2 CURSOR FROM l_sql
            FOREACH artg212_repcur2 INTO sr3.*
                OUTPUT TO REPORT artg212_subrep02(sr3.*)
            END FOREACH
            FINISH REPORT artg212_subrep02
            #FUN-C40071 add --end
        ...
        AFTER GROUP OF sr1.rux04
           ...
           #FUN-C40071 --add--str
           LET l_rux04_rux05_sum = GROUP SUM(sr1.rux05)
           PRINTX l_rux04_rux05_sum
           LET l_rux04_rux06_07_sum = GROUP SUM(sr1.rux06) + GROUP SUM(sr1.rux07)
           PRINTX l_rux04_rux06_07_sum
           LET l_rux04_rux08_sum = GROUP SUM(sr1.rux08)
           PRINTX l_rux04_rux08_sum
           LET l_rux04_income_sum = GROUP SUM(sr1.income)
           PRINTX l_rux04_income_sum
           LET l_rux04_cost_sum = GROUP SUM(sr1.cost)
           PRINTX l_rux04_cost_sum
           IF tm.cb2 = 'N' THEN
               LET l_subrep01_2_show = 'N'
           ELSE
              IF l_rux04_rux08_sum = 0 AND tm.cb1 = 'Y' THEN
                 LET l_subrep01_2_show = 'N'
              ELSE
                 LET l_subrep01_2_show = 'Y'
              END IF
           END IF
           PRINTX l_subrep01_2_show

           IF tm.cb1 = 'N' THEN
               LET l_show = 'Y'
           ELSE
              IF l_rux04_rux08_sum = 0  THEN
                 LET l_show = 'N'
              ELSE
                 LET l_show = 'Y'
              END IF
...
REPORT artg212_1_rep(sr1)
    ...
    DEFINE l_rux04_cost_sum        LIKE type_file.num20_6
    DEFINE l_sql             STRING
    DEFINE l_subrep01_1_show LIKE type_file.chr1 
    #FUN-C40071------add----END----
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.order1
        BEFORE GROUP OF sr1.rux03
        BEFORE GROUP OF sr1.rux04

            ##FUN-C40071 --add--str
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " WHERE rux04 = '",sr1.rux04,"'  AND rux03 = '",sr1.rux03,"'"
            IF tm.cb1 = 'Y' THEN
               LET l_sql = l_sql," AND rux08 <> 0 "
            END IF
            ...
            START REPORT artg212_subrep01
            DECLARE artg212_repcur1 CURSOR FROM l_sql
            FOREACH artg212_repcur1 INTO sr2.*
                OUTPUT TO REPORT artg212_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT artg212_subrep01
            ##FUN-C40071 --add--end
        ...
        AFTER GROUP OF sr1.rux04
           ...
           IF tm.cb2 = 'N' THEN
              LET l_subrep01_1_show = 'N'
           ELSE
              LET l_subrep01_1_show = 'Y'
           END IF
           PRINTX l_subrep01_1_show 
 
        ON LAST ROW

END REPORT
#FUN-C40071 --add--str
REPORT artg212_subrep01(sr2)
    DEFINE sr2 sr1_t
    DEFINE l_rux06_rux07_sum LIKE rux_file.rux06

    FORMAT
        ON EVERY ROW
            LET l_rux06_rux07_sum = sr2.rux06+sr2.rux07
            PRINTX l_rux06_rux07_sum
            PRINTX sr2.*
END REPORT

REPORT artg212_subrep02(sr3)
    DEFINE sr3 sr1_t
    DEFINE l_azw01_rux05_sum       LIKE type_file.num20_6
    DEFINE l_azw01_rux06_07_sum    LIKE type_file.num20_6
    DEFINE l_azw01_rux08_sum       LIKE type_file.num20_6
    DEFINE l_azw01_income_sum      LIKE type_file.num20_6
    DEFINE l_azw01_cost_sum         LIKE type_file.num20_6

    ORDER EXTERNAL BY sr3.azw01

    FORMAT
        ON EVERY ROW
            PRINTX sr3.*

        AFTER GROUP OF sr3.azw01
           #FUN-C40071 --add--str
           LET l_azw01_rux05_sum = GROUP SUM(sr3.rux05)
           PRINTX l_azw01_rux05_sum
           LET l_azw01_rux06_07_sum = GROUP SUM(sr3.rux06) + GROUP SUM(sr3.rux07)
           PRINTX l_azw01_rux06_07_sum
           LET l_azw01_rux08_sum = GROUP SUM(sr3.rux08)
           PRINTX l_azw01_rux08_sum
           LET l_azw01_income_sum = GROUP SUM(sr3.income)
           PRINTX l_azw01_income_sum
           LET l_azw01_cost_sum = GROUP SUM(sr3.cost)
           PRINTX l_azw01_cost_sum
END REPORT
###GENGRE###END
