...
MAIN
   ...
   END IF
    ...
    LET l_table = cl_prt_temptable('artr511',g_sql) CLIPPED
    ...
    IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artr511_tm()        # Input print condition
      ELSE CALL artr511() 
    END IF
...
DEFINE l_zxy03        LIKE zxy_file.zxy03 
   ...
   OPEN WINDOW artr511_w AT p_row,p_col
...
WHILE TRUE
   CONSTRUCT BY NAME tm.wc1 ON azp01
         ...
         AFTER FIELD azp01 
            ...
            END IF 
            IF g_chk_azp01 THEN
               DECLARE r511_zxy_cs1 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH r511_zxy_cs1 INTO l_zxy03 
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artr511_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_template CLIPPED,"'",          
                         " '",g_rpt_name CLIPPED,"'"           
         CALL cl_cmdat('artr511',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW artr511_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
FUNCTION artr511()
   ...
   DEFINE l_name    LIKE type_file.chr20,            #External(Disk) file name  
      ...
      DEFINE l_db_type  STRING       #FUN-B40029
        ...
        DROP TABLE artr511_tmp2
     
     CREATE TEMP TABLE artr511_tmp2(
        ...
        DELETE FROM artr511_tmp2
        
        DROP TABLE artr511_tmp3
     CREATE TEMP TABLE artr511_tmp3(
            rvv31       LIKE rvv_file.rvv31,
            rvv17       LIKE rvv_file.rvv17,
            rvv39t      LIKE rvv_file.rvv39t,
            pmm22       LIKE pmm_file.pmm22,
   ...
   FOREACH sel_azp01_cs INTO l_plant,l_azp02  
     ...
     #               " ORDER BY ima131"
      ...
      PREPARE artr511_p1 FROM l_sql
      DECLARE artr511_p1_curs CURSOR FOR artr511_p1
      FOREACH artr511_p1_curs INTO sr1.* 
         ...
         ELSE     
            ...
            END CASE              
         END IF  
         INSERT INTO artr511_tmp3 VALUES (sr1.rvv31,sr1.rvv17,sr1.rvv39t,sr1.pmm22,sr1.pmm42,sr1.rva113,sr1.rva114,sr1.ima131,sr1.oba02)
         END FOREACH
...
#                     "UNION ",          #TQC-B70204 mark
     ...
     PREPARE artr511_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
     ...
     END IF
     DECLARE artr511_curs1 CURSOR FOR artr511_prepare1
     FOREACH artr511_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,1)
          EXIT FOREACH
       END IF
     ...
     END CASE
       ...
       LET l_sql =" SELECT oba02 FROM ",cl_get_target_table(l_plant,'oba_file'),
        ...
        INSERT INTO artr511_tmp2 VALUES( sr.ogbplant, l_azp02,sr.ima131,l_oba02,sr.ima02,
                                        '','',sr.ogb04,sr.ogb12,l_sales,l_maori)
                 
      END FOREACH 
     END FOREACH
      LET l_sql = " SELECT ima131,oba02, SUM(rvv17),SUM(rvv39t) FROM artr511_tmp3 GROUP BY ima131,oba02 "
      ...
      FOREACH sel_sum_curs INTO l_ima131,l_oba02,l_rvv17,l_rvv39t 
        IF l_ima131 = '' THEN
          UPDATE artr511_tmp2 SET rvv17 = l_rvv17,income = l_rvv39t
      ...
      END FOREACH
       ...
       DECLARE r511_curs3 CURSOR FOR 
        SELECT ima131,oba02,rvv17,income,SUM(ogb12),SUM(sales),SUM(maori) 
        FROM artr511_tmp2 GROUP BY ima131,oba02,rvv17,income

        
     FOREACH r511_curs3 INTO l_ima131,l_oba02,l_rvv17,l_income,l_ogb12,l_sales,l_maori
     IF SQLCA.sqlcode != 0 THEN
     ...
     END IF
      ...
      #FUN-AA0024 add --------begin--------
         ...
         END IF    
      #FUN-AA0024 add --------end--------
      EXECUTE insert_prep USING l_ima131,l_oba02,l_rvv17,l_income,l_ogb12,l_sales,l_maori,l_gross
     ...
     CALL cl_prt_cs3('artr511','artr511',l_sql,g_str)
