...
MAIN
   ...
   CALL cl_used(g_prog,g_time,1) RETURNING g_time 
    ...
    LET l_table = cl_prt_temptable('artr301',g_sql) CLIPPED   # 產生Temp Table                                                      
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      THEN CALL artr301_tm(0,0)            # Input print condition
      ELSE CALL artr301()               # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN 

FUNCTION artr301_tm(p_row,p_col)
   ...
   OPEN WINDOW artr301_w AT p_row,p_col WITH FORM "art/42f/artr301"
   ...
   CALL r301_set_entry()       #TQC-BB0034
   CALL r301_setS() RETURNING tm2.s1,tm2.s2,tm2.s3  #TQC-BB0034
...
WHILE TRUE
   ...
   CONSTRUCT BY NAME tm.wc1 ON azp01 
      ...
      AFTER FIELD azp01            
         ...
         IF NOT cl_null(l_azp01_str) AND l_azp01_str <> "*" THEN
            ...
            END WHILE
         ELSE
            DECLARE r301_zxy_cs  CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
            FOREACH r301_zxy_cs  INTO l_zxy03
               IF g_chk_auth IS NULL THEN
                  LET g_chk_auth = "'",l_zxy03,"'"
               ELSE
                  LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
   ...
   END IF 
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW artr301_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM
   END IF 
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,  #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_template CLIPPED,"'",           
                         " '",g_rpt_name CLIPPED,"'"            
         CALL cl_cmdat('artr301',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW artr301_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
FUNCTION artr301()
   ...
   DROP TABLE artr301_tmp
   CREATE TEMP TABLE artr301_tmp(
   ...
   FOREACH sel_azp01_cs INTO l_plant,l_azp02                                 ##--最外围的FOREACH--
     ...
     END IF
     DECLARE artr301_curs1 CURSOR FOR artr301_p1
     ...
     FOREACH artr301_curs1 INTO sr.*
        ...
        INSERT INTO artr301_tmp VALUES(sr.azp01,sr.azp02,sr.rvv39t,sr.ima131,sr.oba02,sr.rvv31,
  ...
  END FOREACH
   ...
   END CASE
      PREPARE r301_prepare3 FROM l_sql
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('prepare3:',SQLCA.sqlcode,1) 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      DECLARE r301_curs3 CURSOR FOR r301_prepare3  
      FOREACH r301_curs3 INTO sr.azp01,sr.azp02,sr.rvv39t,sr.ima131,sr.oba02,sr.rvv31,
                              sr.ima02,sr.rvv17,sr.rvu04,sr.rvu05,sr.rvu03 
   ...
   CASE
      WHEN tm.t='1'
         CALL cl_prt_cs3('artr301','artr301',l_sql,g_str)  
      WHEN tm.t='2'
         CALL cl_prt_cs3('artr301','artr301_1',l_sql,g_str) 
   END CASE
END FUNCTION       
