...
# Modify.........: No.FUN-BA0063 11/10/20 By yangtt CR轉換成GRW
...
###GENGRE###START
TYPE sr1_t RECORD
    order1 LIKE type_file.num15_3,       #FUN-BA0063 add
    order2 LIKE type_file.num15_3,       #FUN-BA0063 add
...
MAIN
   ...
   END IF
    ...
    LET g_sql=  "order1.type_file.num15_3,",       #FUN-BA0063 add
                "order2.type_file.num15_3,",       #FUN-BA0063 add
                "rvvplant.rvv_file.rvvplant,",      #營運中心
    ...
    IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artg510_tm()        # Input print condition
      ELSE CALL artg510() 
    END IF
...
DEFINE l_combo        ui.ComboBox        #FUN-BA0063  add
   ...
   OPEN WINDOW artg510_w AT p_row,p_col
...
WHILE TRUE
  CONSTRUCT BY NAME tm.wc1 ON azp01
         ...
         AFTER FIELD azp01 
            ...
            END IF 
            IF g_chk_azp01 THEN
               DECLARE g510_zxy_cs1 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH g510_zxy_cs1 INTO l_zxy03 
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artg510_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)               #FUN-BA0063 add
         EXIT PROGRAM
      END IF
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_template CLIPPED,"'",          
                         " '",g_rpt_name CLIPPED,"'"           
         CALL cl_cmdat('artg510',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW artg510_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)               #FUN-BA0063 add
      EXIT PROGRAM
   END IF
...
FUNCTION artg510()
   ...
   DEFINE l_name    LIKE type_file.chr20,            #External(Disk) file name  
          ...
          l_order1  LIKE type_file.num15_3,       #FUN-BA0063 add
          l_order2  LIKE type_file.num15_3,       #FUN-BA0063 add
      ...
      DEFINE l_db_type  STRING       #FUN-B40029
        ...
        DROP TABLE artg510_tmp2
     CREATE TEMP TABLE artg510_tmp2(
        ...
        DROP TABLE artg510_tmp3
     CREATE TEMP TABLE artg510_tmp3(
            rvv17       LIKE rvv_file.rvv17,
            rvv39t      LIKE rvv_file.rvv39t,
            pmm22       LIKE pmm_file.pmm22,
            pmm42       LIKE pmm_file.pmm42,
   ...
   FOREACH sel_azp01_cs INTO l_plant,l_azp02  
     ...
     #FUN-AA0024  ----------------------end------------------------------            
      ...
      PREPARE artg510_p1 FROM l_sql
      DECLARE artg510_p1_curs CURSOR FOR artg510_p1
      FOREACH artg510_p1_curs INTO sr1.* 
         ...
         ELSE     
            ...
            END CASE              
         END IF  
         INSERT INTO artg510_tmp3 VALUES (sr1.rvv17,sr1.rvv39t,sr1.pmm22,sr1.pmm42,sr1.rva113,sr1.rva114,sr1.rvvplant)
      END FOREACH
      
      
      LET l_sql = " SELECT  SUM(rvv17),SUM(rvv39t) FROM artg510_tmp3 GROUP BY rvvplant "
...
#                  " UNION ",    #TQC-B70204 mark
     ...
     CALL cl_replace_sqldb(l_sql) RETURNING l_sql
     #CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql  #FUN-AA0024 mark
     PREPARE artg510_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
     ...
     END IF
     DECLARE artg510_curs1 CURSOR FOR artg510_prepare1
     FOREACH artg510_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,1)
          EXIT FOREACH
       END IF
     ...
     END CASE
       ...
       LET l_sql =" SELECT oba02 FROM ",cl_get_target_table(l_plant,'oba_file'),
        ...
        INSERT INTO artg510_tmp2 VALUES( sr.ogbplant, l_azp02,sr.ima131,l_oba02,sr.ima02,
      ...
      END FOREACH
       ...
       DECLARE g510_curs3 CURSOR FOR 
        SELECT rvvplant,azp02,rvv17,income,SUM(ogb12),SUM(sales),SUM(maori)    #FUN-BA0063 add 2 ''
        FROM artg510_tmp2 GROUP BY rvvplant,azp02,rvv17,income

        
     FOREACH g510_curs3 INTO l_rvvplant,l_azp02,l_rvv17,l_income,l_ogb12,l_sales,l_maori  
     IF SQLCA.sqlcode != 0 THEN
     ...
     END IF
      ...
      EXECUTE insert_prep USING 
                               l_order1,l_order2,      #FUN-BA0063 add
                               l_rvvplant,l_azp02,l_rvv17,l_income,l_ogb12,l_sales,l_maori,l_gross
...
###GENGRE###     CALL cl_prt_cs3('artg510','artg510',l_sql,g_str)
    CALL artg510_grdata()    ###GENGRE###
...
#FUN-BA0063-------add-----str----
PRIVATE FUNCTION artg510_replace_ord_desc(p_desc)
...
FUNCTION artg510_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg510")
        IF handler IS NOT NULL THEN
            START REPORT artg510_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY order1 desc"    #FUN-BA0063 add
          
            DECLARE artg510_datacur1 CURSOR FROM l_sql
            FOREACH artg510_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg510_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg510_rep
        END IF
...
REPORT artg510_rep(sr1)
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            #FUN-BA0063-------add---str-----
            LET l_lineno = 0
            LET g_ord1_desc = artg510_replace_ord_desc(g_ord1_desc)
            LET g_ord2_desc = artg510_replace_ord_desc(g_ord2_desc)
