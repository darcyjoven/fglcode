...
MAIN
   ...
   LET l_table = cl_prt_temptable('artr400',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artr400_tm(0,0)        # Input print condition
      ELSE CALL artr400()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN


FUNCTION artr400_tm(p_row,p_col) 
   ...
   OPEN WINDOW artr400_w AT p_row,p_col WITH FORM "art/42f/artr400" 
    ...
    WHILE TRUE
     CONSTRUCT BY NAME tm.wc1 ON azp01
         ...
         AFTER FIELD azp01 
            ...
            END IF 
            IF g_chk_azp01 THEN
               DECLARE r400_zxy_cs1 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH r400_zxy_cs1 INTO l_zxy03 
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
      ...
      END IF
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artr400_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artr400',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr400_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
FUNCTION artr400()
   ...
   DEFINE l_name    LIKE type_file.chr20,         # External(Disk) file name        
    ...
    DROP TABLE artr400_tmp    
    CREATE TEMP TABLE artr400_tmp( 
           oeaplant LIKE oea_file.oeaplant, 
           azp02    LIKE azp_file.azp02, 
           ima131   LIKE ima_file.ima131,
           oba02    LIKE oba_file.oba02, 
...
LET l_sql = "SELECT DISTINCT azp01,azp02 FROM azp_file,azw_file ",
 ...
 WHEN '1' 
     ...
     WHEN '3'
      ...
      END IF   
       ...
       CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql  
        ...
        OTHERWISE EXIT CASE
     END CASE           
    PREPARE artr400_prepare1 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       EXIT PROGRAM
    END IF
    DECLARE artr400_curs1 CURSOR FOR artr400_prepare1

    FOREACH artr400_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
  ...
  CASE
   WHEN tm.b='1'
        #LET l_sql = " SELECT oeaplant,azp02,'','','','',",
         LET l_sql = " SELECT oeaplant,azp02,'','','','','',SUM(oeb12),",  #FUN-BB0032
       ...
       WHEN tm.b='2'
         #LET l_sql = " SELECT oeaplant,azp02,'','',ima01,ima02,",
          LET l_sql = " SELECT oeaplant,azp02,'','',ima01,ima02,oeb05,SUM(oeb12),",  #FUN-BB0032
                ...
                " ORDER BY oeaplant,azp02,ima01,ima02,oeb05"
       WHEN tm.b='3'
    ...
    PREPARE artr400_prepare2 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       EXIT PROGRAM
    END IF
    DECLARE artr400_curs2 CURSOR FOR artr400_prepare2

    FOREACH artr400_curs2 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
  ...
  #FUN-BC0026 add END
   CASE
      WHEN tm.b='1'
         CALL cl_prt_cs3('artr400','artr400_1',l_sql,g_str)  
      WHEN tm.b='2'
         CALL cl_prt_cs3('artr400','artr400_2',l_sql,g_str) 
      WHEN tm.b='3'
         CALL cl_prt_cs3('artr400','artr400_3',l_sql,g_str) 
   END CASE
