...
#
# Pattern name...: artg511.4gl
# Descriptions...: 產品分類排行榜明細表
# Input parameter:
# Return code....:
...
# Modify.........: No:FUN-B80161 11/10/20 By chenying 去除特殊字符，以便可轉GR
#                                         By chenying 明細CR轉GR
...
###GENGRE###START
TYPE sr1_t RECORD
    order1 LIKE type_file.num15_3,    #FUN-B80161 add
    order2 LIKE type_file.num15_3,    #FUN-B80161 add
...
MAIN
   ...
   END IF
    ...
    LET g_sql = "order1.type_file.num15_3,",    #FUN-B80161 add
                "order2.type_file.num15_3,",    #FUN-B80161 add
    ...
    LET l_table = cl_prt_temptable('artg511',g_sql) CLIPPED
    IF l_table = -1 THEN 
    ...
    IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artg511_tm()        # Input print condition
      ELSE CALL artg511() 
    END IF
...
DEFINE l_combo        ui.ComboBox     #FUN-B80161 add
   ...
   OPEN WINDOW artg511_w AT p_row,p_col
...
WHILE TRUE
   CONSTRUCT BY NAME tm.wc1 ON azp01
         ...
         AFTER FIELD azp01 
            ...
            END IF 
            IF g_chk_azp01 THEN
               DECLARE g511_zxy_cs1 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH g511_zxy_cs1 INTO l_zxy03 
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artg511_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)   #FUN-B80161
         EXIT PROGRAM
      END IF
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_template CLIPPED,"'",          
                         " '",g_rpt_name CLIPPED,"'"           
         CALL cl_cmdat('artg511',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW artg511_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)   #FUN-B80161
      EXIT PROGRAM
   END IF
...
FUNCTION artg511()
   ...
   DEFINE l_name    LIKE type_file.chr20,            #External(Disk) file name  
          ...
          l_ogb12   LIKE ogb_file.ogb12,
           ...
           l_ima131_t LIKE ima_file.ima131,
           l_ima131_o LIKE ima_file.ima131,
           l_flag     LIKE type_file.chr1,
          l_order1  LIKE type_file.num15_3,    #FUN-B80161 add
          l_order2  LIKE type_file.num15_3,    #FUN-B80161 add
      ...
      DEFINE l_db_type  STRING       #FUN-B40029
        ...
        DROP TABLE artg511_tmp2
     
     CREATE TEMP TABLE artg511_tmp2(
        ...
        DELETE FROM artg511_tmp2
        
        DROP TABLE artg511_tmp3
     CREATE TEMP TABLE artg511_tmp3(
            rvv31       LIKE rvv_file.rvv31,
            rvv17       LIKE rvv_file.rvv17,
            rvv39t      LIKE rvv_file.rvv39t,
            pmm22       LIKE pmm_file.pmm22,
   ...
   FOREACH sel_azp01_cs INTO l_plant,l_azp02  
     ...
     #FUN-AA0024  ----------------------begin----------------------------
     #LET l_sql = " SELECT rvv31,rvv17,rvv39t,pmm22,pmm42,rva113,rva114,ima131,oba02 ",
     ...
     #               " ORDER BY ima131"
      ...
      LET l_sql = " SELECT rvv31,rvv17-(SELECT CASE WHEN SUM(rvv17) IS NULL THEN 0 ELSE SUM(rvv17) END ",
     	            " FROM ",cl_get_target_table(l_plant,'rvu_file'),
     	            "     ,",cl_get_target_table(l_plant,'ima_file'),
     	            "     ,",cl_get_target_table(l_plant,'oba_file'),
     	            "     ,",cl_get_target_table(l_plant,'rvv_file'),
                  " LEFT OUTER JOIN ",cl_get_target_table(l_plant,'pmm_file')," ON pmm01 = rvv36 ", 
                  " LEFT OUTER JOIN ",cl_get_target_table(l_plant,'rva_file')," ON rva01 = rvv04 ",
                  " WHERE rvv31 = ima01 AND oba01 = ima131 AND rvv01 = rvu01 AND rvu00 = '3' AND rvuconf = 'Y' AND rvu02 = u.rvu02 AND rvv02 = v.rvv02 ",
                  " AND (rvu03 BETWEEN '",tm.yy1,"' AND '",tm.yy2,"') ",
                  " AND rvvplant = '",l_plant,"'),",
                  ...
                  " AND (rvu03 BETWEEN '",tm.yy1,"' AND '",tm.yy2,"') ",
                  " AND rvvplant = '",l_plant,"'),",
                  " pmm22,pmm42,rva113,rva114,ima131,oba02 ",
      ...
      PREPARE artg511_p1 FROM l_sql
      DECLARE artg511_p1_curs CURSOR FOR artg511_p1
      FOREACH artg511_p1_curs INTO sr1.* 
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF  
         #FUN-AA0024 add ------------------begin-----------------
            IF cl_null(sr1.rvv17) THEN 
               LET sr1.rvv17 = 0
            END IF
            IF cl_null(sr1.rvv39t) THEN 
               LET sr1.rvv39t = 0
            END IF   
         #FUN-AA0024 add ------------------end-------------------
         ...
         ELSE     
            ...
            END CASE              
         END IF  
         INSERT INTO artg511_tmp3 VALUES (sr1.rvv31,sr1.rvv17,sr1.rvv39t,sr1.pmm22,sr1.pmm42,sr1.rva113,sr1.rva114,sr1.ima131,sr1.oba02)
         END FOREACH
...
#                     "UNION ",          #TQC-B70204 mark
     ...
     PREPARE artg511_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
     ...
     END IF
     DECLARE artg511_curs1 CURSOR FOR artg511_prepare1
     FOREACH artg511_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,1)
          EXIT FOREACH
       END IF
       ...
       ##FUN-AA0024 add ---------------begin------------------
       # IF NOT cl_null(sr.oha24) THEN
       #       LET l_sql = "SELECT azi10 FROM ",cl_get_target_table(l_plant,'azi_file'),
       #                   " WHERE azi01 = '",sr.oha23,"'"
       #       CALL cl_replace_sqldb(l_sql) RETURNING l_sql
       #       CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql
       #       PREPARE sel_azi10_pre3 FROM l_sql
       #       EXECUTE sel_azi10_pre3 INTO l_azi10
       # ELSE
       #       LET sr.oha24 = 1
       # END IF
       ...
       # CASE l_azi10
       #     WHEN '1'
       #          LET sr.ohb14t = sr.ohb14t * sr.oha24
       #     WHEN '2'
       #          LET sr.ohb14t = sr.ohb14t / sr.oha24  
       # END CASE  
       ##FUN-AA0024 add ---------------end--------------------
     ...
     END CASE
       ...
       LET l_sql =" SELECT oba02 FROM ",cl_get_target_table(l_plant,'oba_file'),
        ...
        INSERT INTO artg511_tmp2 VALUES( sr.ogbplant, l_azp02,sr.ima131,l_oba02,sr.ima02,
                                        '','',sr.ogb04,sr.ogb12,l_sales,l_maori)
                 
      END FOREACH 
     END FOREACH
      LET l_sql = " SELECT ima131,oba02, SUM(rvv17),SUM(rvv39t) FROM artg511_tmp3 GROUP BY ima131,oba02 "
      PREPARE sel_sum_pre FROM l_sql
      DECLARE sel_sum_curs CURSOR FOR sel_sum_pre
      FOREACH sel_sum_curs INTO l_ima131,l_oba02,l_rvv17,l_rvv39t 
        IF l_ima131 = '' THEN
          UPDATE artg511_tmp2 SET rvv17 = l_rvv17,income = l_rvv39t
           WHERE ima131 IS NULL AND oba02 IS NULL
        ELSE 
           IF l_oba02 = '' THEN
                UPDATE artg511_tmp2 SET rvv17 = l_rvv17,income = l_rvv39t
                WHERE ima131 = l_ima131 AND oba02 IS NULL
           ELSE
                UPDATE artg511_tmp2 SET rvv17 = l_rvv17,income = l_rvv39t
                 WHERE ima131 = l_ima131 AND oba02 = l_oba02
           END IF
        END IF
      END FOREACH

       DECLARE g511_curs3 CURSOR FOR 
        SELECT ima131,oba02,rvv17,income,SUM(ogb12),SUM(sales),SUM(maori) 
        FROM artg511_tmp2 GROUP BY ima131,oba02,rvv17,income

        
     FOREACH g511_curs3 INTO l_ima131,l_oba02,l_rvv17,l_income,l_ogb12,l_sales,l_maori  
     IF SQLCA.sqlcode != 0 THEN
     ...
     END IF
      ...
      EXECUTE insert_prep USING 
                               l_order1,l_order2,      #FUN-B80161 add
                               l_ima131,l_oba02,l_rvv17,l_income,l_ogb12,l_sales,l_maori,l_gross
...
###GENGRE###     CALL cl_prt_cs3('artg511','artg511',l_sql,g_str)
    CALL artg511_grdata()    ###GENGRE###
...
#FUN-BA0063-------add-----str----
PRIVATE FUNCTION artg511_replace_ord_desc(p_desc)
...
FUNCTION artg511_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg511")
        IF handler IS NOT NULL THEN
            START REPORT artg511_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY order1 desc,order2"    #FUN-B80161 add
          
            DECLARE artg511_datacur1 CURSOR FROM l_sql
            FOREACH artg511_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg511_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg511_rep
        END IF
...
REPORT artg511_rep(sr1)
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            #FUN-B80161-------add---str-----
            LET l_lineno = 0
            LET g_ord1_desc = artg511_replace_ord_desc(g_ord1_desc)
            LET g_ord2_desc = artg511_replace_ord_desc(g_ord2_desc)
        ...
        ON EVERY ROW
            ...
            LET l_gross = sr1.gross USING '---,---,---,---,--&.&&'
            LET l_gross = l_gross.trim()
            LET l_gross = l_gross,'%'
            PRINTX l_gross
