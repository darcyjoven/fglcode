...
MAIN
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artr402_tm(0,0)        # Input print condition
      ELSE CALL artr402()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN


FUNCTION artr402_tm(p_row,p_col)
   ...
   OPEN WINDOW artr402_w AT p_row,p_col WITH FORM "art/42f/artr402" 
    ...
    WHILE TRUE
        CONSTRUCT BY NAME tm.wc1 ON azp01
         ...
         AFTER FIELD azp01 
            ...
            END IF 
            IF g_chk_azp01 THEN
               DECLARE r402_zxy_cs1 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH r402_zxy_cs1 INTO l_zxy03 
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
      ...
      END IF
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artr402_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artr402',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr402_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
   ...
   DEFINE l_name    LIKE type_file.chr20,         # External(Disk) file name        
    #      l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT          #TQC-B70066 mark
          ...
          sr        RECORD 
                    ...
                    oeb05    LIKE oeb_file.oeb05,  #FUN-BB0032
    ...
    IF STATUS THEN                                                                                                                   
       ...
       EXIT PROGRAM
    END IF
    DROP TABLE artr402_tmp    
    CREATE TEMP TABLE artr402_tmp( 
           oeaplant LIKE oea_file.oeaplant, 
           azp02    LIKE azp_file.azp02, 
           ima131   LIKE ima_file.ima131,
           oba02    LIKE oba_file.oba02, 
...
CASE tm.a
     WHEN '1'  
      ...
      IF l_lpk01_find = 0 THEN 
            #LET l_sql = "SELECT oeaplant,'',ima131,'',ima01,ima02,SUM(oeb12),",
             LET l_sql = "SELECT oeaplant,'',ima131,'',ima01,ima02,oeb05,SUM(oeb12),",  #FUN-BB0032
                        ...
                        " GROUP BY oeaplant,ima131,ima01,ima02,oeb05,oea23,oea24"
      ELSE 
        #LET l_sql = "SELECT oeaplant,'',ima131,'',ima01,ima02,SUM(oeb12),",
         LET l_sql = "SELECT oeaplant,'',ima131,'',ima01,ima02,oeb05,SUM(oeb12),",  #FUN-BB0032
                     ...
                     " AND oeb04 = ima01 AND ",tm.wc CLIPPED, 
                     " GROUP BY oeaplant,ima131,ima01,ima02,oeb05,lpk01,lpk04,oea23,oea24"
    ...
    PREPARE artr402_prepare1 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       EXIT PROGRAM
    END IF
    DECLARE artr402_curs1 CURSOR FOR artr402_prepare1

    FOREACH artr402_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
...
CASE
   WHEN tm.b='1'
         #LET l_sql = " SELECT '','','','',ima01,ima02,SUM(oeb12),",
          LET l_sql = " SELECT '','','','',ima01,ima02,oeb05,SUM(oeb12),",  #FUN-BB0032 
                " SUM(oeb14t),lpk01,lpk04,'','' ", 
                " FROM artr402_tmp", 
                " GROUP BY ima01,ima02,oeb05,lpk01,lpk04",
                " ORDER BY ima01,ima02"
       
       WHEN tm.b='2'
          
         #LET l_sql =" SELECT oeaplant,azp02,'','',ima01,ima02,SUM(oeb12),",
          LET l_sql =" SELECT oeaplant,azp02,'','',ima01,ima02,oeb05,SUM(oeb12),",  #FUN-BB0032 
                " SUM(oeb14t),lpk01,lpk04,'','' ",
                " FROM artr402_tmp", 
                " GROUP BY oeaplant,azp02,ima01,ima02,oeb05,lpk01,lpk04",
                " ORDER BY ima01,ima02,oeaplant,azp02"
    END CASE
    PREPARE artr402_prepare2 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       EXIT PROGRAM
    END IF
    DECLARE artr402_curs2 CURSOR FOR artr402_prepare2

    FOREACH artr402_curs2 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
     ...
     #sr.oba02,sr.ima01,sr.ima02,sr.oeb12,sr.oeb14t,sr.lpk01,sr.lpk04  #FUN-BB0032
      sr.oba02,sr.ima01,sr.ima02,sr.oeb05,sr.oeb12,sr.oeb14t,sr.lpk01,sr.lpk04  #FUN-BB0032
...
END FOREACH
  ...
  #FUN-BC0026 add END
   CASE
      WHEN tm.b='1'
         CALL cl_prt_cs3('artr402','artr402_1',l_sql,g_str)  
      WHEN tm.b='2'
         CALL cl_prt_cs3('artr402','artr402_2',l_sql,g_str) 
   END CASE
END FUNCTION
