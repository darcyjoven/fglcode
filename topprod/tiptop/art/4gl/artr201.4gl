...
MAIN
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artr201_tm(0,0)        # Input print condition
      ELSE CALL artr201()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN

FUNCTION artr201_tm(p_row,p_col)
   ...
   OPEN WINDOW artr201_w AT p_row,p_col WITH FORM "art/42f/artr201" 
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artr201_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
     ...
     #INPUT BY NAME tm.edate,tm.type,tm.a,tm.more WITHOUT DEFAULTS
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('artr201',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr201_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
FUNCTION artr201()
 #  DEFINE l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT      #TQC-B70068
   ...
   LET l_table = cl_prt_temptable('artr201',g_sql) CLIPPED
   IF  l_table = -1 THEN
   ...
   IF STATUS THEN                                                                                                                   
      ...
      EXIT PROGRAM                                                                                                                 
   END IF
   DROP TABLE artr201_tmp

   CREATE TEMP TABLE artr201_tmp(
   ...
   FOREACH sel_azp01_cs INTO l_plant,l_azp02  
      ...
      FOREACH r200_curs1 INTO sr.azw01,sr.img01,sr.img02,sr.img03,sr.img04,
         ...
         SELECT COUNT(*) INTO l_cnt FROM artr201_tmp  #--己存在不覆蓋
   ...
   PREPARE artr201_prepare2 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM
   END IF
   DECLARE artr201_curs2 CURSOR FOR artr201_prepare2

   FOREACH artr201_curs2 INTO sr.azw01,l_azp02,sr.ima01,sr.ima02,
                              sr.ima131,l_oba02,sr.img02,l_img10,l_amt
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
      SELECT SUM(amt) INTO l_amts FROM artr201_tmp WHERE azp01 = sr.azw01
      IF l_amts  <> 0 THEN
  ...
  #FUN-BC0026 add END
   ...
   CASE
      WHEN tm.a='1'
         CALL cl_prt_cs3('artr201','artr201_1',l_sql,g_str)  
      WHEN tm.a='2'
         CALL cl_prt_cs3('artr201','artr201_2',l_sql,g_str) 
   END CASE
END FUNCTION 
#FUN-A80082
