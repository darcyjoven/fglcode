...
MAIN
   ...
   END IF
    ...
    CALL cl_set_comp_visible("rvn08",FALSE)
...
DEFINE l_rvmplant_desc  LIKE azp_file.azp02
    ...
    DISPLAY BY NAME g_rvm.rvm01,g_rvm.rvm02,g_rvm.rvm03,g_rvm.rvm04, g_rvm.rvmoriu,g_rvm.rvmorig,
                    ...
                    g_rvm.rvmcond,g_rvm.rvmconu,g_rvm.rvmmksg,g_rvm.rvm900,g_rvm.rvmplant,
...
DEFINE   p_wc2       STRING        
    ...
    LET g_sql = "SELECT rvn02,rvn03,rvn04,rvn05,rvn06,rvn06_desc,",
                "       rvn07,rvn07_desc,rvn08,rvn09,ima02,ima131,",
                "       ruc16,gfe02,ruc18,ruc18_01,rvn10,rvn11,rvn11_desc,",
                "       rvn12,rvn12_desc,ruc29,rvn13",
                "  FROM cs_rvn_temp ORDER BY rvn02"
...
DEFINE l_rvmplant_desc LIKE azp_file.azp02  #機構名稱
      ...
      DISPLAY BY NAME g_rvm.rvmuser,g_rvm.rvmgrup,g_rvm.rvmcrat,
                      g_rvm.rvmacti,g_rvm.rvmconf,g_rvm.rvm900,
...
DEFINE l_n LIKE type_file.num5
   ...
   LET l_sql = "INSERT INTO rvn_temp ",
               ...
               "ruc06 rvn11,ruc06 rvn12,ruc06 rvn13,azw02 rvnlegal,ruc01 rvnplant,",
...
DEFINE     p_cmd     LIKE type_file.chr1,                
   ...
   INPUT BY NAME g_rvm.rvm01,g_rvm.rvm02,g_rvm.rvm03, g_rvm.rvmoriu,g_rvm.rvmorig,
                 ...
                 g_rvm.rvm07,g_rvm.rvmmksg,g_rvm.rvm08
     WITHOUT DEFAULTS
      ...
      AFTER FIELD rvn13 #多角貿易代碼
        ...
        ON ROW CHANGE
           ...
           IF l_lock_sw = 'Y' THEN
              ...
              LET g_rvn[l_ac].* = g_rvn_t.*
           ELSE
              UPDATE rvn_file SET rvn13 = g_rvn[l_ac].rvn13
                 WHERE rvn01 = g_rvm.rvm01
                   AND rvn02 = g_rvn_t.rvn02
...
DEFINE l_ruo01 LIKE ruo_file.ruo01
    ...
    LET l_sql  = "SELECT rvn03,rvn04,rvn09,rvn10,rvn11 ",
                 "  FROM rvn_temp2",
                 " WHERE rvn06 = ? ",
                 "   AND rvn07 = ? ",
                 "   AND rvn12 = ? "
...
#TQC-A70088 --end  add ruolegal
      ...
      UPDATE ruo_temp SET ruo02 = '3',
       ...
       FOREACH rvn_cs2 USING l_rvn.rvn06,l_rvn.rvn07,l_rvn.rvn12
                        INTO l_rvn.rvn03,l_rvn.rvn04,l_rvn.rvn09,
...
#TQC-A70088 --begin
         #  INSERT INTO rup_temp (rup01,rup02,rupplant)VALUES(l_ruo.ruo01,g_cnt,l_rvn.rvn07)
            ...
            INSERT INTO rup_temp (rup01,rup02,rupplant,ruplegal)VALUES(l_ruo.ruo01,g_cnt,l_rvn.rvn07,l_ruo.ruolegal)  #No.FUN-A70123
...
#稅種為空    
    ...
    LET l_sql = "INSERT INTO oeb_temp",
                ...
                " oeb44,oeb47,oeb48,",
                " oeb916,oeb917,oeb15,oeb72,oeb910,",  #FUN-B20060 add oeb72
                ...
                "       ' ',0,'1',",
                "       pmn86,pmn87,pmn33,pmn33,pmn80,",  #FUN-B20060 add pmn33
...
#如果是最后一站取出貨機構，否則取機構預設倉庫對應機構    
    ...
    ELSE
       ...
       LET l_sql = "UPDATE ogb_temp a ",
                   "   SET ogb09 = (SELECT rtz07 FROM rtz_file WHERE rtz01 = a.ogbplant),",
                   "       ogb091 = ' ',",
                   "       ogb092 = ' '",             
                   " WHERE ogb44 = '1'"
       EXECUTE IMMEDIATE l_sql
       #取預設非成本倉 
       LET l_sql = "UPDATE ogb_temp a ",
                   "   SET ogb09 = (SELECT rtz08 FROM rtz_file WHERE rtz01 = a.ogbplant),",
                   "       ogb091 = ' ',",
                   "       ogb092 = ' '",
                   " WHERE ogb44 <> '1'"
       EXECUTE IMMEDIATE l_sql
...
#取機構預設倉庫      
      IF l_pmm.pmm51 = '1' THEN
         UPDATE rvb_temp
            SET rvb36 = (SELECT rtz07 FROM rtz_file WHERE rtz01 = l_pmm.pmmplant)
      ELSE
         UPDATE rvb_temp
            SET rvb36 = (SELECT rtz08 FROM rtz_file WHERE rtz01 = l_pmm.pmmplant)
      END IF
...
#     CALL s_auto_assign_no("axm",l_no,g_today,"19","adk_file","adk01","","","") #FUN-A70130 mark
      CALL s_auto_assign_no("art",l_no,g_today,"U8","adk_file","adk01","","","") #FUN-A70130 mod
