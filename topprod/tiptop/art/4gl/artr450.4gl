...
# Modify.........: No.MOD-D10215 12/01/23 By Sakura 新增ARG_VAL變數處理 
DATABASE ds
...
MAIN
   ...
   LET tm.a = ARG_VAL(8)        #MOD-D10215 add
   LET tm.wc1 = ARG_VAL(9)      #MOD-D10215 add
   LET g_rep_user = ARG_VAL(10) #MOD-D10215 8->10
   LET g_rep_clas = ARG_VAL(11) #MOD-D10215 9->11
   LET g_template = ARG_VAL(12) #MOD-D10215 10->12
   LET g_rpt_name = ARG_VAL(13) #MOD-D10215 11->13
   #FUN-AA0024 add ###end###
   ...
   LET g_sql = "ogaplant.oga_file.ogaplant,",
    ...
    LET l_table = cl_prt_temptable('artr450',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artr450_tm(0,0)        # Input print condition
      ELSE CALL artr450()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN

FUNCTION artr450_tm(p_row,p_col)
   ...
   OPEN WINDOW artr450_w AT p_row,p_col WITH FORM "art/42f/artr450" 
   ...
   WHILE TRUE
      ...
      IF g_bgjob = 'Y' THEN
         ...
         IF SQLCA.sqlcode OR l_cmd IS NULL THEN
            CALL cl_err('artr450','9031',1)
         ELSE
            LET tm.wc2=cl_replace_str(tm.wc2, "'", "\"")
            LET tm.wc1=cl_replace_str(tm.wc1, "'", "\"") #MOD-D10215 add
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_copies CLIPPED,"'",
                       " '",tm.wc2 CLIPPED,"'" ,
                       " '",tm.a CLIPPED,"'" ,       #MOD-D10215 add
                       " '",tm.wc1 CLIPPED,"'" ,     #MOD-D10215 add
                       " '",g_rep_user CLIPPED,"'",           
                       " '",g_rep_clas CLIPPED,"'",           
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artr450',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr450_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
FUNCTION artr450() 
   ...
   DEFINE l_wc         STRING                   #FUN-BC0026 add
    ...
    DROP TABLE artr450_tmp    
    CREATE TEMP TABLE artr450_tmp( 
           ogaplant LIKE oga_file.ogaplant, 
           azp02    LIKE azp_file.azp02, 
           ima131   LIKE ima_file.ima131,
           oba02    LIKE oba_file.oba02, 
           ...
           lpk01    LIKE lpk_file.lpk01,
           lpk04    LIKE lpk_file.lpk04)      
    DELETE FROM artr450_tmp 
    #FUN-AA0024 add ------------------------------begin-------------------------
    DROP TABLE artr450_tmp1    
    CREATE TEMP TABLE artr450_tmp1( 
           ohaplant LIKE oha_file.ohaplant, 
           ima131   LIKE ima_file.ima131,
           ima01    LIKE ima_file.ima01,
           ohb04    LIKE ohb_file.ohb04,
...
#MOD-D10215---add-----STR
   ...
   IF g_bgjob = 'Y' THEN
      LET l_sql = "SELECT DISTINCT azp01,azp02 FROM azp_file,azw_file ",
                  " WHERE azw01 = azp01 AND ",tm.wc1 CLIPPED,
                  " ORDER BY azp01 "
      ELSE
#MOD-D10215---add-----END
      LET l_sql = "SELECT DISTINCT azp01,azp02 FROM azp_file,azw_file ",
...
#FUN-C90045 add end ---
      ...
      PREPARE artr450_prepare1 FROM l_sql                  
      IF SQLCA.sqlcode != 0 THEN                           
         CALL cl_err('prepare:',SQLCA.sqlcode,1)           
         CALL cl_used(g_prog,g_time,2) RETURNING g_time    
         EXIT PROGRAM                                      
      END IF                                               
      DECLARE artr450_curs1 CURSOR FOR artr450_prepare1    

      #MOD-B60025 add --start--
      CALL cl_replace_sqldb(l_sql2) RETURNING l_sql2         
      CALL cl_parse_qry_sql(l_sql2,l_plant) RETURNING l_sql2   
      PREPARE artr450_prepare3 FROM l_sql2                  
      DECLARE artr450_curs3 CURSOR FOR artr450_prepare3    
      #MOD-B60025 add --end--
 
      LET sr.ogb14t_1 = 0 #MOD-B60025 add 
      LET l_type = NULL
      FOREACH artr450_curs1 INTO l_type,sr.*  #TQC-B60299 add l_type
         IF SQLCA.sqlcode != 0 THEN
         ...
         LET l_ogb14_1 = 0 
         LET l_ogb14t_1 = 0 
#        IF l_type = '1' THEN  #TQC-B60299 ADD   #FUN-C90045 MARK
#        FOREACH artr450_curs3 USING sr.ima01,sr.oga23,sr.oga24,'','','' INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 add '','',''  #FUN-C9004
         FOREACH artr450_curs3 USING sr.ima01,sr.oga23,sr.oga24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #FUN-C90045 add
...
#         ELSE
#            FOREACH artr450_curs3 USING '','','',sr.ima01,sr.oga23,sr.oga24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 add '','',''
#              SELECT azi04 INTO t_azi04 FROM azi_file
#               WHERE azi01=l_oga23
#               IF l_oga213 = 'N' THEN
#                  LET l_ogb14_1 =l_ogb917*l_ogb37
...
#FUN-C90045 add end ---
         CALL cl_replace_sqldb(l_sql) RETURNING l_sql         
         #CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql    
         PREPARE artr450_prepare6 FROM l_sql                  
         IF SQLCA.sqlcode != 0 THEN                           
            CALL cl_err('prepare:',SQLCA.sqlcode,1)           
            CALL cl_used(g_prog,g_time,2) RETURNING g_time    
            EXIT PROGRAM                                      
         END IF                                               
         PREPARE artr450_prepare11 FROM l_sql                  
         IF SQLCA.sqlcode != 0 THEN                           
            CALL cl_err('prepare:',SQLCA.sqlcode,1)           
            CALL cl_used(g_prog,g_time,2) RETURNING g_time    
            EXIT PROGRAM                                      
         END IF                                               
         DECLARE artr450_curs11 CURSOR FOR artr450_prepare11    

         #MOD-B60025 add --start--
         CALL cl_replace_sqldb(l_sql2) RETURNING l_sql2         
         CALL cl_parse_qry_sql(l_sql2,l_plant) RETURNING l_sql2   
         PREPARE artr450_prepare4 FROM l_sql2                  
         DECLARE artr450_curs4 CURSOR FOR artr450_prepare4    
         #MOD-B60025 add --end--
 
         LET sr1.ohb14t_1 = 0 #MOD-B60025 add
         LET l_type = NULL #TQC-B60299 ADD
         FOREACH artr450_curs11 INTO l_type,sr1.*  #TQC-B60299 add l_type
            IF SQLCA.sqlcode != 0 THEN
               CALL cl_err('foreach:',SQLCA.sqlcode,1)
               EXIT FOREACH
            END IF
...
#               FOREACH artr450_curs4 USING sr1.ima01,sr1.oha23,sr1.oha24,'','','' INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 add '','',''
#                 SELECT azi04 INTO t_azi04 FROM azi_file 
#                  WHERE azi01=l_oga23
#                  IF l_oga213 = 'N' THEN
...
##TQC-B60299 ADD -BEGIN -----------------------------------------
#            ELSE
#              FOREACH artr450_curs4 USING '','','',sr1.ima01,sr1.oha23,sr1.oha24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37
#FUN-C90045 mark end ---
           FOREACH artr450_curs4 USING sr1.ima01,sr1.oha23,sr1.oha24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37     #FUN-C90045 add
             SELECT azi04 INTO t_azi04 FROM azi_file
              WHERE azi01=l_oga23
              IF l_oga213 = 'N' THEN
                 LET l_ohb14_1 =l_ogb917*l_ogb37
...
#TQC-B90112 add START----------------------
   CASE
      WHEN tm.a = '1'
         LET  l_sql = " SELECT ogaplant,'','','','','' FROM artr450_tmp WHERE ogaplant IS NOT NULL",
                      " UNION SELECT ohaplant,'','','','','' FROM artr450_tmp1 WHERE ohaplant IS NOT NULL "
      WHEN tm.a = '2'
         LET  l_sql = " SELECT ogaplant,'',ima131,'','','' FROM artr450_tmp ",
                      " UNION SELECT ohaplant,'',ima131,'','','' FROM artr450_tmp1"
      WHEN tm.a = '3'
         LET  l_sql = " SELECT ogaplant,'','','',ima01,'' FROM artr450_tmp ",
                      " UNION SELECT ohaplant,'','','',ima01,'' FROM artr450_tmp1"
   END CASE

   PREPARE artr450_prepare5 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      EXIT PROGRAM
   END IF
   DECLARE artr450_curs5 CURSOR FOR artr450_prepare5 
   FOREACH artr450_curs5 INTO   sr.ogaplant, sr.azp02, sr.ima131,sr.oba02, sr.ima01,sr.ima02
   ...
   CASE
      WHEN tm.a = '1'
         SELECT SUM(ogb14)    INTO sr.ogb14     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant
         SELECT SUM(ogb47)    INTO sr.ogb47     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant
         SELECT SUM(ogb14t)   INTO sr.ogb14t    FROM artr450_tmp  WHERE ogaplant = sr.ogaplant
         SELECT SUM(ogb14t_1) INTO sr.ogb14t_1  FROM artr450_tmp  WHERE ogaplant = sr.ogaplant
         SELECT lpk01         INTO sr.lpk01     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant
         SELECT lpk04         INTO sr.lpk04     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant
         SELECT oga23         INTO sr.oga23     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant
         SELECT oga24         INTO sr.oga24     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant
         SELECT SUM(ohb14t)   INTO sr.ohb14t    FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant
         SELECT SUM(ohb14t_1) INTO sr.ohb14t_1  FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant
         SELECT SUM(ohb67)    INTO sr.ohb67     FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant
         SELECT SUM(oha23)    INTO sr.oha23     FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant
         SELECT SUM(oha24)    INTO sr.oha24     FROM artr450_tm1p WHERE ohaplant = sr.ogaplant

      WHEN tm.a = '2'
         SELECT SUM(ogb14)    INTO sr.ogb14     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ogb47)    INTO sr.ogb47     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ogb14t)   INTO sr.ogb14t    FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ogb14t_1) INTO sr.ogb14t_1  FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT lpk01         INTO sr.lpk01     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT lpk04         INTO sr.lpk04     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT oga23         INTO sr.oga23     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT oga24         INTO sr.oga24     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ohb14t)   INTO sr.ohb14t    FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ohb14t_1) INTO sr.ohb14t_1  FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ohb67)    INTO sr.ohb67     FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(oha23)    INTO sr.oha23     FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(oha24)    INTO sr.oha24     FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131

      WHEN tm.a = '3'
         SELECT SUM(ogb14)    INTO sr.ogb14     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT SUM(ogb47)    INTO sr.ogb47     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT SUM(ogb14t)   INTO sr.ogb14t    FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT SUM(ogb14t_1) INTO sr.ogb14t_1  FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT lpk01         INTO sr.lpk01     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT lpk04         INTO sr.lpk04     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT oga23         INTO sr.oga23     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT oga24         INTO sr.oga24     FROM artr450_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT SUM(ohb14t)   INTO sr.ohb14t    FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT SUM(ohb14t_1) INTO sr.ohb14t_1  FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT SUM(ohb67)    INTO sr.ohb67     FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT SUM(oha23)    INTO sr.oha23     FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT SUM(oha24)    INTO sr.oha24     FROM artr450_tmp1 WHERE ohaplant = sr.ogaplant AND ima01 = sr.ima01
     END CASE
#TQC-B90112 add END------------------------
...
##                     " ,(select SUM(ohb14t) from artr450_tmp1 b)",              #FUN-AA0024 add       #FUN-B80009 mark
##                     " ,(select SUM(ohb14t_1) from artr450_tmp1 b)",            #MOD-B60025 add       #FUN-B80009 mark
##                     " ,(select SUM(ohb67)  from artr450_tmp1 b),'',''",        #FUN-AA0024 add       #FUN-B80009 mark
#                     " ,(SELECT SUM(ohb14t) FROM artr450_tmp1 b WHERE  b.ohaplant = a.ogaplant)",              #FUN-B80009 add
#                     " ,(SELECT SUM(ohb14t_1) FROM artr450_tmp1 b WHERE b.ohaplant = a.ogaplant)",             #FUN-B80009 add
#                     " ,(SELECT SUM(ohb67) FROM artr450_tmp1 b WHERE b.ohaplant = a.ogaplant),'',''",          #FUN-B80009 add
#                     " FROM artr450_tmp a",
...
#                     "SUM(ogb47),SUM(ogb14t),SUM(ogb14t_1),lpk01,lpk04,'','' ", #MOD-B60025 add ogb14t_1
#                     " ,(select SUM(ohb14t) from artr450_tmp1 b where a.ima131=b.ima131)",              #FUN-AA0024 add       #FUN-B80009 mark
#                     " ,(select SUM(ohb14t_1) from artr450_tmp1 b where a.ima131=b.ima131)",            #MOD-B60025 add       #FUN-B80009 mark
#                     " ,(select SUM(ohb67)  from artr450_tmp1 b where a.ima131=b.ima131),'',''",        #FUN-AA0024 add       #FUN-B80009 mark
#                     " ,(SELECT SUM(ohb14t) FROM artr450_tmp1 b WHERE a.ima131=b.ima131 and b.ohaplant = a.ogaplant)",              #FUN-B80009 add
#                     " ,(SELECT SUM(ohb14t_1) FROM artr450_tmp1 b WHERE a.ima131=b.ima131 and b.ohaplant = a.ogaplant)",            #FUN-B80009 add
#                     " ,(SELECT SUM(ohb67) FROM artr450_tmp1 b WHERE a.ima131=b.ima131 and b.ohaplant = a.ogaplant),'',''",         #FUN-B80009 add
#                     " FROM artr450_tmp a ",
...
#                     "SUM(ogb47),SUM(ogb14t),SUM(ogb14t_1),lpk01,lpk04,'','' ",  #MOD-B60025 add ogb14t_1
##                     " ,(select SUM(ohb14t) from artr450_tmp1 b where a.ima01=b.ima01)",              #FUN-AA0024 add         #FUN-B80009 mark
##                     " ,(select SUM(ohb14t_1) from artr450_tmp1 b where a.ima01=b.ima01)",            #MOD-B60025 add         #FUN-B80009 mark
##                     " ,(select SUM(ohb67)  from artr450_tmp1 b where a.ima01=b.ima01),'',''",        #FUN-AA0024 add         #FUN-B80009 marki
#                     " ,(SELECT SUM(ohb14t) FROM artr450_tmp1 b WHERE a.ima01=b.ima01 and b.ohaplant = a.ogaplant)",              #FUN-B80009 add
#                     " ,(SELECT SUM(ohb14t_1) FROM artr450_tmp1 b WHERE a.ima01=b.ima01 and b.ohaplant = a.ogaplant)",            #FUN-B80009 add
#                     " ,(SELECT SUM(ohb67) FROM artr450_tmp1 b WHERE a.ima01=b.ima01 and b.ohaplant = a.ogaplant),'',''",         #FUN-B80009 add
#                     " FROM artr450_tmp a  ",     
#                     " GROUP BY ogaplant,azp02,a.ima01,ima02,lpk01,lpk04", 
#                     " ORDER BY ogaplant,azp02,a.ima01,ima02" 
#   END CASE        
#   PREPARE artr450_prepare2 FROM l_sql                                                
#   IF SQLCA.sqlcode != 0 THEN
#      CALL cl_err('prepare:',SQLCA.sqlcode,1) 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
#      EXIT PROGRAM
#   END IF
#   DECLARE artr450_curs2 CURSOR FOR artr450_prepare2   
#   FOREACH artr450_curs2 INTO sr.*
#      IF SQLCA.sqlcode != 0 THEN
#         CALL cl_err('foreach:',SQLCA.sqlcode,1)
#         EXIT FOREACH
#      END IF   
#TQC-B90112 mark END----------------------
  ...
  #LET g_str = tm.wc2  #FUN-BC0026 mark
   CASE
      WHEN tm.a='1'
         CALL cl_prt_cs3('artr450','artr450_1',l_sql,g_str)  
      WHEN tm.a='2'
         CALL cl_prt_cs3('artr450','artr450_2',l_sql,g_str) 
      WHEN tm.a='3'
         CALL cl_prt_cs3('artr450','artr450_3',l_sql,g_str) 
   END CASE
END FUNCTION
#FUN-A60075
