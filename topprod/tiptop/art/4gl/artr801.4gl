...
MAIN
   ...
   LET g_sql=  "rch01.rch_file.rch01,",
    ...
    LET l_table = cl_prt_temptable('artr801',g_sql) CLIPPED
    IF l_table = -1 THEN EXIT PROGRAM END IF
    LET g_sql = "rch01.rch_file.rch01,",
                "rci10.rci_file.rci10,",
                "oaj02.oaj_file.oaj02,",
                "feiy.rci_file.rci11"
    LET l_table1 = cl_prt_temptable('artr8011',g_sql) CLIPPED
    IF l_table1=-1 THEN EXIT PROGRAM END IF  
    IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN 
       CALL artr801_tm()        # Input print condition
    ELSE 
       CALL artr801() 
    END IF
...
DEFINE l_sql          STRING
   ...
   OPEN WINDOW artr801_w AT p_row,p_col WITH FORM "art/42f/artr800" 
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artr801_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd1 CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artr801',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr801_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
DEFINE  l_occ231 LIKE occ_file.occ18
    ...
    CREATE TEMP TABLE artr801_tmp(
                    rch01 LIKE rch_file.rch01,
                    rci10 LIKE rci_file.rci10,
                    feiy  LIKE rci_file.rci11)
    DELETE FROM artr801_tmp
...
#FUN-B60054 ---------------END 
    ...
    END IF
    PREPARE artr801_prepare1 FROM l_sql 
    DECLARE artr801_curs1 CURSOR FOR artr801_prepare1
    FOREACH artr801_curs1 INTO sr.* 
       ...
       IF tm.cb = 'Y' THEN
          LET l_sql = " SELECT rci10,rci11,rci12 FROM rci_file ",
                      ...
                      " AND rci02 = '2'"
          PREPARE artr801_prepare2 FROM l_sql 
          DECLARE artr801_curs2 CURSOR FOR artr801_prepare2
          DELETE FROM artr801_tmp
          FOREACH artr801_curs2 INTO l_rci10,l_rci11,l_rci12 
             IF sr.rci06 = 'N' THEN
                LET l_feiy = l_rci11
             ELSE
                LET l_feiy = l_rci12
             ...
             INSERT INTO artr801_tmp VALUES(sr.rch01,l_rci10,l_feiy)
          END FOREACH  
          SELECT SUM(feiy) INTO l_feiys FROM  artr801_tmp
       END IF 
       ...
       EXECUTE insert_prep USING sr.rch01,sr.rch05,l_azw08,sr.rch071,sr.rci04,
                               ...
                               sr.rci09, l_occ18,l_occ11,l_occ231,l_feiys,l_azi04
   ...
   CALL cl_prt_cs3('artr801','artr801',l_sql,g_str)  
