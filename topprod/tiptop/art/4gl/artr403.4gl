...
MAIN
   ...
   LET l_table = cl_prt_temptable('artr403',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artr403_tm(0,0)        # Input print condition
      ELSE CALL artr403()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN


FUNCTION artr403_tm(p_row,p_col)
   ...
   OPEN WINDOW artr403_w AT p_row,p_col WITH FORM "art/42f/artr403" 
   ...
   LET tm.b = '1'  
    ...
    CONSTRUCT BY NAME tm.wc1 ON azp01
         ...
         AFTER FIELD azp01 
            ...
            END IF 
            IF g_chk_azp01 THEN
               DECLARE r403_zxy_cs1 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH r403_zxy_cs1 INTO l_zxy03 
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
      ...
      END IF
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artr403_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artr403',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr403_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
FUNCTION artr403()
   ...
   #       l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT             #TQC-B70066 mark    
    ...
    IF STATUS THEN                                                                                                                   
       ...
       EXIT PROGRAM                                                                                                                 
    END IF
  DROP TABLE artr403_tmp    
    CREATE TEMP TABLE artr403_tmp( 
           oeaplant LIKE oea_file.oeaplant, 
           azp02    LIKE azp_file.azp02, 
           ima131   LIKE ima_file.ima131,
           oba02    LIKE oba_file.oba02, 
...
CASE tm.a
   ...
   CALL cl_replace_sqldb(l_sql) RETURNING l_sql          
    ...
    PREPARE artr403_prepare1 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       EXIT PROGRAM
    END IF
    DECLARE artr403_curs1 CURSOR FOR artr403_prepare1

    FOREACH artr403_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
...
LET l_sql="SELECT oba02 FROM ",cl_get_target_table(l_plant,'oba_file'),
  ...
  CASE
   WHEN tm.b='1'
    ...
    END CASE 
     PREPARE artr403_prepare2 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       EXIT PROGRAM
    END IF
    DECLARE artr403_curs2 CURSOR FOR artr403_prepare2

    FOREACH artr403_curs2 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
  ...
  #FUN-BC0026 add END
   CASE
      WHEN tm.b='1'
         CALL cl_prt_cs3('artr403','artr403_1',l_sql,g_str)  
      WHEN tm.b='2'
         CALL cl_prt_cs3('artr403','artr403_2',l_sql,g_str) 
      WHEN tm.b='3'
         CALL cl_prt_cs3('artr403','artr403_3',l_sql,g_str) 
   END CASE
END FUNCTION
