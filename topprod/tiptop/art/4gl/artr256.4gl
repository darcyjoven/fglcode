...
MAIN
   ...
   LET g_sql=  "ruo01.ruo_file.ruo01,",
    ...
    LET l_table = cl_prt_temptable('artr256',g_sql) CLIPPED
    ...
    IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN 
       CALL artr256_tm()        # Input print condition
    ELSE 
       CALL artr256() 
    END IF
...
DEFINE l_sql          STRING
   ...
   OPEN WINDOW artr256_w AT p_row,p_col WITH FORM "art/42f/artr256" 
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artr256_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF 
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            LET l_cmd = l_cmd1 CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artr256',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr256_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
DEFINE l_ima02      LIKE ima_file.ima02
   ...
   CALL cl_prt_cs1('artr256','artr256',l_sql,g_str)   
