...
MAIN
   ...
   LET g_sql = "ogaplant.oga_file.ogaplant,",
    ...
    LET l_table = cl_prt_temptable('artr451',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artr451_tm(0,0)        # Input print condition
      ELSE CALL artr451()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN

FUNCTION artr451_tm(p_row,p_col)
   ...
   OPEN WINDOW artr451_w AT p_row,p_col WITH FORM "art/42f/artr451" 
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artr451_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artr451',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr451_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
#TQC-B60299 - ADD -  END  ------------------------------------
    ...
    IF STATUS THEN                                                                                                                   
       ...
       EXIT PROGRAM                                                                                                                 
    END IF  
    DROP TABLE artr451_tmp    
    CREATE TEMP TABLE artr451_tmp( 
           ogaplant LIKE oga_file.ogaplant, 
           azp02    LIKE azp_file.azp02, 
           ima131   LIKE ima_file.ima131,
           oba02    LIKE oba_file.oba02, 
           ...
           lpk01    LIKE lpk_file.lpk01,
           lpk04    LIKE lpk_file.lpk04) 
    DELETE FROM artr451_tmp
    
    #FUN-AA0024 add ------------------------------begin-------------------------
    DROP TABLE artr451_tmp1    
    CREATE TEMP TABLE artr451_tmp1( 
           ohaplant LIKE oha_file.ohaplant, 
           ima131   LIKE ima_file.ima131, 
           ima01    LIKE ima_file.ima01,
           ohb04    LIKE ohb_file.ohb04,
...
#FUN-C90045 add end ---
      ...
      PREPARE artr451_prepare1 FROM l_sql                  
      IF SQLCA.sqlcode != 0 THEN                           
         CALL cl_err('prepare:',SQLCA.sqlcode,1)           
         CALL cl_used(g_prog,g_time,2) RETURNING g_time    
         EXIT PROGRAM                                      
      END IF                                               
      DECLARE artr451_curs1 CURSOR FOR artr451_prepare1    
 
      #TQC-B60299 add --start--
      CALL cl_replace_sqldb(l_sql2) RETURNING l_sql2
      CALL cl_parse_qry_sql(l_sql2,l_plant) RETURNING l_sql2
      PREPARE artr451_prepare3 FROM l_sql2
      DECLARE artr451_curs3 CURSOR FOR artr451_prepare3
      #TQC-B60299 add --end--

      LET sr.ogb14t_1 = 0  #TQC-B60299 add
      LET l_type = NULL    #TQC-B60299 add

      FOREACH artr451_curs1 INTO l_type,sr.*  #TQC-B60299 add l_type
         IF SQLCA.sqlcode != 0 THEN
...
#        FOREACH artr451_curs3 USING sr.ima01,sr.oga23,sr.oga24,'','','' INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 MOD  #FUN-C90045 mark
         FOREACH artr451_curs3 USING sr.ima01,sr.oga23,sr.oga24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #FUN-C90045 add
...
##TQC-B60299 ADD - BEGIN ------------------------------
#         ELSE
#            FOREACH artr451_curs3 USING '','','',sr.ima01,sr.oga23,sr.oga24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 MOD
#              SELECT azi04 INTO t_azi04 FROM azi_file
#               WHERE azi01=l_oga23
#               IF l_oga213 = 'N' THEN
#                  LET l_ogb14_1 =l_ogb917*l_ogb37
...
#FUN-C90045 add end ---
         CALL cl_replace_sqldb(l_sql) RETURNING l_sql         
         #CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql    
         PREPARE artr451_prepare6 FROM l_sql                  
         IF SQLCA.sqlcode != 0 THEN                           
            CALL cl_err('prepare:',SQLCA.sqlcode,1)           
            CALL cl_used(g_prog,g_time,2) RETURNING g_time    
            EXIT PROGRAM                                      
         END IF                                               
         PREPARE artr451_prepare11 FROM l_sql                  
         IF SQLCA.sqlcode != 0 THEN                           
            CALL cl_err('prepare:',SQLCA.sqlcode,1)           
            CALL cl_used(g_prog,g_time,2) RETURNING g_time    
            EXIT PROGRAM                                      
         END IF                                               
         DECLARE artr451_curs11 CURSOR FOR artr451_prepare11    

         #TQC-B60299 add --start--
         CALL cl_replace_sqldb(l_sql2) RETURNING l_sql2
         CALL cl_parse_qry_sql(l_sql2,l_plant) RETURNING l_sql2
         PREPARE artr451_prepare4 FROM l_sql2
         DECLARE artr451_curs4 CURSOR FOR artr451_prepare4
         #TQC-B60299 add --end--

         LET sr1.ohb14t_1 = 0  #TQC-B60299 add
         LET l_type = NULL     #TQC-B60299 add

         FOREACH artr451_curs11 INTO l_type,sr1.*  #TQC-B60299 add l_type
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
...
#           FOREACH artr451_curs4 USING sr1.ima01,sr1.oha23,sr1.oha24,'','','' INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 MOD #FUN-C90045 MARK
            FOREACH artr451_curs4 USING sr1.ima01,sr1.oha23,sr1.oha24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #FUN-C90045  add
              SELECT azi04 INTO t_azi04 FROM azi_file
               WHERE azi01=l_oga23
               IF l_oga213 = 'N' THEN
...
#FUN-C90045 mark begin ---
#         ELSE
#            FOREACH artr451_curs4 USING '','','',sr1.ima01,sr1.oha23,sr1.oha24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299
#              SELECT azi04 INTO t_azi04 FROM azi_file
#               WHERE azi01=l_oga23
#               IF l_oga213 = 'N' THEN
#                  LET l_ohb14_1 =l_ogb917*l_ogb37
...
#TQC-B90113 add START-----------------------------------
   CASE 
      WHEN tm.a = '1'  
         LET l_sql = " SELECT '',ima131,'','','','' FROM artr451_tmp WHERE ima131 IS NOT NULL",
                     " UNION SELECT '',ima131,'','','','' FROM artr451_tmp1 WHERE ima131 IS NOT NULL"   
      WHEN tm.a = '2'
         LET l_sql = " SELECT ogaplant, ima131,'','','','' FROM artr451_tmp ",
                     " UNION SELECT ohaplant , ima131,'','','','' FROM artr451_tmp1 "
   END CASE

   PREPARE artr451_prepare5 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      EXIT PROGRAM
   END IF
   DECLARE artr451_curs5 CURSOR FOR artr451_prepare5
   FOREACH artr451_curs5 INTO   sr.ogaplant,  sr.ima131, sr.azp02, sr.oba02, sr.ima01, sr.ima02
   ...
   CASE
      WHEN tm.a = '1'
         SELECT SUM(ogb14)    INTO sr.ogb14     FROM artr451_tmp  WHERE ima131 = sr.ima131
         SELECT SUM(ogb47)    INTO sr.ogb47     FROM artr451_tmp  WHERE ima131 = sr.ima131
         SELECT SUM(ogb14t)   INTO sr.ogb14t    FROM artr451_tmp  WHERE ima131 = sr.ima131
         SELECT SUM(ogb14t_1) INTO sr.ogb14t_1  FROM artr451_tmp  WHERE ima131 = sr.ima131
         SELECT lpk01         INTO sr.lpk01     FROM artr451_tmp  WHERE ima131 = sr.ima131
         SELECT lpk04         INTO sr.lpk04     FROM artr451_tmp  WHERE ima131 = sr.ima131
         SELECT oga23         INTO sr.oga23     FROM artr451_tmp  WHERE ima131 = sr.ima131
         SELECT oga24         INTO sr.oga24     FROM artr451_tmp  WHERE ima131 = sr.ima131
         SELECT SUM(ohb14t)   INTO sr.ohb14t    FROM artr451_tmp1 WHERE ima131 = sr.ima131
         SELECT SUM(ohb14t_1) INTO sr.ohb14t_1  FROM artr451_tmp1 WHERE ima131 = sr.ima131
         SELECT SUM(ohb67)    INTO sr.ohb67     FROM artr451_tmp1 WHERE ima131 = sr.ima131
         SELECT SUM(oha23)    INTO sr.oha23     FROM artr451_tmp1 WHERE ima131 = sr.ima131
         SELECT SUM(oha24)    INTO sr.oha24     FROM artr451_tmp1 WHERE ima131 = sr.ima131
      WHEN tm.a = '2'
         SELECT SUM(ogb14)    INTO sr.ogb14     FROM artr451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ogb47)    INTO sr.ogb47     FROM artr451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ogb14t)   INTO sr.ogb14t    FROM artr451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ogb14t_1) INTO sr.ogb14t_1  FROM artr451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT lpk01         INTO sr.lpk01     FROM artr451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT lpk04         INTO sr.lpk04     FROM artr451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT oga23         INTO sr.oga23     FROM artr451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT oga24         INTO sr.oga24     FROM artr451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ohb14t)   INTO sr.ohb14t    FROM artr451_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ohb14t_1) INTO sr.ohb14t_1  FROM artr451_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ohb67)    INTO sr.ohb67     FROM artr451_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(oha23)    INTO sr.oha23     FROM artr451_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(oha24)    INTO sr.oha24     FROM artr451_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
   END CASE
...
#                     " ,(SELECT SUM(ohb14t) FROM artr451_tmp1 b WHERE a.ima131=b.ima131)",              #FUN-AA0024 add
#               #      " ,(SELECT SUM(ohb14t_1) FROM artr451_tmp1 b)",                                    #TQC-B60299 add     #TQC-B80077 mark
#                     ", (SELECT SUM(ohb14t_1) FROM artr451_tmp1 b WHERE a.ima131=b.ima131)",            #TQC-B80077 add
#                     " ,(SELECT SUM(ohb67)  FROM artr451_tmp1 b WHERE a.ima131=b.ima131),'',''",        #FUN-AA0024 add
#                     " FROM artr451_tmp a ",
...
#                     "SUM(ogb47),SUM(ogb14t),SUM(ogb14t_1),lpk01,lpk04,'',''",                          #TQC-B60299 add SUM(ogb14t_1)
#               #      " ,(select SUM(ohb14t) from artr451_tmp1 b where a.ima131=b.ima131)",              #FUN-AA0024 add      #TQC-B80077 mark
#               #      " ,(select SUM(ohb14t_1) from artr451_tmp1 b)",                                    #TQC-B60299 add      #TQC-B80077 mark
#               #      " ,(select SUM(ohb67)  from artr451_tmp1 b where a.ima131=b.ima131),'',''",        #FUN-AA0024 add      #TQC-B80077 mark
#                     " ,(SELECT SUM(ohb14t) FROM artr451_tmp1 b WHERE a.ima131=b.ima131 AND b.ohaplant = a.ogaplant)",              #FUN-AA0024 add      #TQC-B80077 add
#                     " ,(SELECT SUM(ohb14t_1) FROM artr451_tmp1 b WHERE a.ima131=b.ima131 AND b.ohaplant = a.ogaplant)",            #TQC-B60299 add      #TQC-B80077 add
#                     " ,(SELECT SUM(ohb67)  FROM artr451_tmp1 b WHERE a.ima131=b.ima131 AND b.ohaplant = a.ogaplant),'',''",        #FUN-AA0024 add      #TQC-B80077 add
#                     " FROM artr451_tmp a",
#                     " GROUP BY a.ima131,oba02,ogaplant,azp02,lpk01,lpk04",
#                     " ORDER BY a.ima131,oba02,ogaplant,azp02"              
#   END CASE                                                
#   PREPARE artr451_prepare2 FROM l_sql                                                
#   IF SQLCA.sqlcode != 0 THEN
#      CALL cl_err('prepare:',SQLCA.sqlcode,1) 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
#      EXIT PROGRAM
#   END IF
#   DECLARE artr451_curs2 CURSOR FOR artr451_prepare2   
#   FOREACH artr451_curs2 INTO sr.*
#      IF SQLCA.sqlcode != 0 THEN
#         CALL cl_err('foreach:',SQLCA.sqlcode,1)
#         EXIT FOREACH
#      END IF
...
#TQC-B90113 add END--------------------------------------
  ...
  #LET g_str = tm.wc2   #FUN-BC0026 mark
   CASE
      WHEN tm.a='1'
         CALL cl_prt_cs3('artr451','artr451_1',l_sql,g_str)  
      WHEN tm.a='2'
         CALL cl_prt_cs3('artr451','artr451_2',l_sql,g_str) 
   END CASE
END FUNCTION
#FUN-A60075
