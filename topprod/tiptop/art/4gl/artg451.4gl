...
# Modify.........: No:FUN-B80161 11/10/27 by chenying 明細CR轉GR
...
MAIN
   ...
   LET g_sql = "ogaplant.oga_file.ogaplant,",
    ...
    LET l_table = cl_prt_temptable('artg451',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artg451_tm(0,0)        # Input print condition
      ELSE CALL artg451()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time
   CALL cl_gre_drop_temptable(l_table)   #FUN-B80161 
END MAIN

FUNCTION artg451_tm(p_row,p_col)
   ...
   OPEN WINDOW artg451_w AT p_row,p_col WITH FORM "art/42f/artg451" 
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artg451_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)   #FUN-B80161 
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artg451',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artg451_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)   #FUN-B80161 
         EXIT PROGRAM
      END IF
...
#TQC-B60299 - ADD -  END  ------------------------------------
    ...
    IF STATUS THEN                                                                                                                   
       ...
       EXIT PROGRAM                                                                                                                 
    END IF  
    DROP TABLE artg451_tmp    
    CREATE TEMP TABLE artg451_tmp( 
           ogaplant LIKE oga_file.ogaplant, 
           azp02    LIKE azp_file.azp02, 
           ima131   LIKE ima_file.ima131,
           oba02    LIKE oba_file.oba02, 
           ...
           lpk01    LIKE lpk_file.lpk01,
           lpk04    LIKE lpk_file.lpk04) 
    DELETE FROM artg451_tmp
    
    #FUN-AA0024 add ------------------------------begin-------------------------
    DROP TABLE artg451_tmp1    
    CREATE TEMP TABLE artg451_tmp1( 
           ohaplant LIKE oha_file.ohaplant, 
           ima131   LIKE ima_file.ima131, 
           ima01    LIKE ima_file.ima01,
           ohb04    LIKE ohb_file.ohb04,
...
#                  #  " SUM(ohb67),SUM(ohb14t)*(-1),0,'','',oha23,oha24 ",  #TQC-B60299 ADD 0
...
#FUN-C90116--------add-----end
      ...
      PREPARE artg451_prepare1 FROM l_sql                  
      IF SQLCA.sqlcode != 0 THEN                           
         ...
         EXIT PROGRAM                                      
      END IF                                               
      DECLARE artg451_curs1 CURSOR FOR artg451_prepare1    
 
      #TQC-B60299 add --start--
      CALL cl_replace_sqldb(l_sql2) RETURNING l_sql2
      CALL cl_parse_qry_sql(l_sql2,l_plant) RETURNING l_sql2
      PREPARE artg451_prepare3 FROM l_sql2
      DECLARE artg451_curs3 CURSOR FOR artg451_prepare3
      #TQC-B60299 add --end--

      LET sr.ogb14t_1 = 0  #TQC-B60299 add
      LET l_type = NULL    #TQC-B60299 add

      FOREACH artg451_curs1 INTO l_type,sr.*  #TQC-B60299 add l_type
         IF SQLCA.sqlcode != 0 THEN
         ...
         #IF l_type = '1' THEN #TQC-B60299 ADD  #FUN-C90116 mark
            #FOREACH artg451_curs3 USING sr.ima01,sr.oga23,sr.oga24,'','','' INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 MOD #FUN-C90116 mark
            FOREACH artg451_curs3 USING sr.ima01,sr.oga23,sr.oga24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #FUN-C90116 add
...
#FUN-C90116-----mark-----str
#        ELSE
#           FOREACH artg451_curs3 USING '','','',sr.ima01,sr.oga23,sr.oga24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 MOD
#             SELECT azi04 INTO t_azi04 FROM azi_file
#              WHERE azi01=l_oga23
#              IF l_oga213 = 'N' THEN
#                 LET l_ogb14_1 =l_ogb917*l_ogb37
...
#TQC-B60299 ADD -  END  ------------------------------
         ...
         CASE l_azi10
            ...
            WHEN '2'  
               ...
               LET sr.ogb14t_1 = sr.ogb14t_1/sr.oga24 #TQC-B60299 ADD
         END CASE
         INSERT INTO artg451_tmp
            VALUES(sr.ogaplant,l_azp02,sr.ima131,l_oba02,
            sr.ogb14,sr.ogb47,sr.ogb14t,sr.ogb14t_1,sr.lpk01,sr.lpk04)  #TQC-B60299 ADD sr.ogb14t_1
            LET l_oba02 =''          
         END FOREACH 
         #FUN-AA0024 add --------------------------------begin--------------------------------
#FUN-C90116-----mark----str
...
#           LET tm.wc2 = cl_replace_str(tm.wc2,"oga03","oha03")
#           LET tm.wc2 = cl_replace_str(tm.wc2,"oga02","oha02")
#           LET l_sql = "SELECT '1',ohaplant,ima131,ima01,ohb04,SUM(ohb14t),0,SUM(ohb67),oha23,oha24", #TQC-B60299 ADD 0,'1'
#                       " FROM ",cl_get_target_table(l_plant,'ima_file'),
#                       " ,",cl_get_target_table(l_plant,'ohb_file'),		
#                       " ,",cl_get_target_table(l_plant,'oha_file'),	
#                       " WHERE oha01=ohb01",
...
#           LET tm.wc2 = cl_replace_str(tm.wc2,"oha03","oga03")
#           LET tm.wc2 = cl_replace_str(tm.wc2,"oha02","oga02")
#        #   LET l_sql = l_sql," UNION SELECT '2',ogaplant,ima131,ima01,ogb04,SUM(ogb14t)*(-1),0,SUM(ogb47),oga23,oga24 ", #TQC-B60299 ADD 0,'2' #FUN-B80117 mark
#           LET l_sql = l_sql," UNION SELECT '2',ogaplant,ima131,ima01,ogb04,SUM(ogb14t)*(-1),0,SUM(ogb47)*(-1),oga23,oga24 ", #TQC-B60299 ADD 0,'2'  #FUN-B80117 add
#                       " FROM ",cl_get_target_table(l_plant,'ima_file'),
#                       " ,",cl_get_target_table(l_plant,'ogb_file'),
#                       " ,",cl_get_target_table(l_plant,'oga_file'),
#                       " WHERE oga01=ogb01",
...
#FUN-C90116-----add-----end
         CALL cl_replace_sqldb(l_sql) RETURNING l_sql         
         #CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql    
         PREPARE artg451_prepare6 FROM l_sql                  
         IF SQLCA.sqlcode != 0 THEN                           
            CALL cl_err('prepare:',SQLCA.sqlcode,1)           
            CALL cl_used(g_prog,g_time,2) RETURNING g_time    
            CALL cl_gre_drop_temptable(l_table)   #FUN-B80161 
            EXIT PROGRAM                                      
         END IF                                               
         PREPARE artg451_prepare11 FROM l_sql                  
         IF SQLCA.sqlcode != 0 THEN                           
            CALL cl_err('prepare:',SQLCA.sqlcode,1)           
            CALL cl_used(g_prog,g_time,2) RETURNING g_time    
            CALL cl_gre_drop_temptable(l_table)   #FUN-B80161 
            EXIT PROGRAM                                      
         END IF                                               
         DECLARE artg451_curs11 CURSOR FOR artg451_prepare11    

         #TQC-B60299 add --start--
         CALL cl_replace_sqldb(l_sql2) RETURNING l_sql2
         CALL cl_parse_qry_sql(l_sql2,l_plant) RETURNING l_sql2
         PREPARE artg451_prepare4 FROM l_sql2
         DECLARE artg451_curs4 CURSOR FOR artg451_prepare4
         #TQC-B60299 add --end--

         LET sr1.ohb14t_1 = 0  #TQC-B60299 add
         LET l_type = NULL     #TQC-B60299 add

         FOREACH artg451_curs11 INTO l_type,sr1.*  #TQC-B60299 add l_type
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
         ...
         #IF l_type = '1' THEN   #FUN-C90116 mark
            #FOREACH artg451_curs4 USING sr1.ima01,sr1.oha23,sr1.oha24,'','','' INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 MOD  #FUN-C90116 mark
            FOREACH artg451_curs4 USING sr1.ima01,sr1.oha23,sr1.oha24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #FUN-C90116 add
              SELECT azi04 INTO t_azi04 FROM azi_file
               WHERE azi01=l_oga23
               IF l_oga213 = 'N' THEN
...
#FUN-C90116----mark---str
#        ELSE
#           FOREACH artg451_curs4 USING '','','',sr1.ima01,sr1.oha23,sr1.oha24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299
#             SELECT azi04 INTO t_azi04 FROM azi_file
#              WHERE azi01=l_oga23
#              IF l_oga213 = 'N' THEN
#                 LET l_ohb14_1 =l_ogb917*l_ogb37
...
#FUN-C90116----mark---end
         ...
         CASE l_azi10
            ...
            WHEN '2'  
               ...
               LET sr1.ohb14t_1 = sr1.ohb14t_1/sr1.oha24 #TQC-B60299 add
         END CASE
         INSERT INTO artg451_tmp1
         VALUES(sr1.ohaplant,sr1.ima131,sr1.ima01,sr1.ohb04,sr1.ohb14t,sr1.ohb14t_1,sr1.ohb67)  #TQC-B60299 add sr1.ohb14t_1
         END FOREACH
         #FUN-AA0024 add --------------------------------end----------------------------------
...
#TQC-B90113 add START-----------------------------------
   CASE 
      WHEN tm.a = '1'  
         LET l_sql = " SELECT '',ima131,'','','','' FROM artg451_tmp WHERE ima131 IS NOT NULL",
                     " UNION SELECT '',ima131,'','','','' FROM artg451_tmp1 WHERE ima131 IS NOT NULL"   
      WHEN tm.a = '2'
         LET l_sql = " SELECT ogaplant, ima131,'','','','' FROM artg451_tmp ",
                     " UNION SELECT ohaplant , ima131,'','','','' FROM artg451_tmp1 "
   END CASE

   PREPARE artg451_prepare5 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(l_table)   #FUN-B80161 
      EXIT PROGRAM
   END IF
   DECLARE artg451_curs5 CURSOR FOR artg451_prepare5
   FOREACH artg451_curs5 INTO   sr.ogaplant,  sr.ima131, sr.azp02, sr.oba02, sr.ima01, sr.ima02
   ...
   CASE
      WHEN tm.a = '1'
         SELECT SUM(ogb14)    INTO sr.ogb14     FROM artg451_tmp  WHERE ima131 = sr.ima131
         SELECT SUM(ogb47)    INTO sr.ogb47     FROM artg451_tmp  WHERE ima131 = sr.ima131
         SELECT SUM(ogb14t)   INTO sr.ogb14t    FROM artg451_tmp  WHERE ima131 = sr.ima131
         SELECT SUM(ogb14t_1) INTO sr.ogb14t_1  FROM artg451_tmp  WHERE ima131 = sr.ima131
         SELECT lpk01         INTO sr.lpk01     FROM artg451_tmp  WHERE ima131 = sr.ima131
         SELECT lpk04         INTO sr.lpk04     FROM artg451_tmp  WHERE ima131 = sr.ima131
         SELECT oga23         INTO sr.oga23     FROM artg451_tmp  WHERE ima131 = sr.ima131
         SELECT oga24         INTO sr.oga24     FROM artg451_tmp  WHERE ima131 = sr.ima131
         SELECT SUM(ohb14t)   INTO sr.ohb14t    FROM artg451_tmp1 WHERE ima131 = sr.ima131
         SELECT SUM(ohb14t_1) INTO sr.ohb14t_1  FROM artg451_tmp1 WHERE ima131 = sr.ima131
         SELECT SUM(ohb67)    INTO sr.ohb67     FROM artg451_tmp1 WHERE ima131 = sr.ima131
         SELECT SUM(oha23)    INTO sr.oha23     FROM artg451_tmp1 WHERE ima131 = sr.ima131
         SELECT SUM(oha24)    INTO sr.oha24     FROM artg451_tmp1 WHERE ima131 = sr.ima131
      WHEN tm.a = '2'
         SELECT SUM(ogb14)    INTO sr.ogb14     FROM artg451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ogb47)    INTO sr.ogb47     FROM artg451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ogb14t)   INTO sr.ogb14t    FROM artg451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ogb14t_1) INTO sr.ogb14t_1  FROM artg451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT lpk01         INTO sr.lpk01     FROM artg451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT lpk04         INTO sr.lpk04     FROM artg451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT oga23         INTO sr.oga23     FROM artg451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT oga24         INTO sr.oga24     FROM artg451_tmp  WHERE ogaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ohb14t)   INTO sr.ohb14t    FROM artg451_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ohb14t_1) INTO sr.ohb14t_1  FROM artg451_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(ohb67)    INTO sr.ohb67     FROM artg451_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(oha23)    INTO sr.oha23     FROM artg451_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
         SELECT SUM(oha24)    INTO sr.oha24     FROM artg451_tmp1 WHERE ohaplant = sr.ogaplant AND ima131 = sr.ima131
   END CASE
...
#   CASE
#      WHEN tm.a='1' 
#         LET l_sql = "SELECT '','',a.ima131,oba02,'','',SUM(ogb14),",
#                     "SUM(ogb47),SUM(ogb14t),SUM(ogb14t_1),lpk01,lpk04,'',''",                          #TQC-B60299 add SUM(ogb14t_1)
#                     " ,(SELECT SUM(ohb14t) FROM artg451_tmp1 b WHERE a.ima131=b.ima131)",              #FUN-AA0024 add
#               #      " ,(SELECT SUM(ohb14t_1) FROM artg451_tmp1 b)",                                    #TQC-B60299 add     #TQC-B80077 mark
#                     ", (SELECT SUM(ohb14t_1) FROM artg451_tmp1 b WHERE a.ima131=b.ima131)",            #TQC-B80077 add
#                     " ,(SELECT SUM(ohb67)  FROM artg451_tmp1 b WHERE a.ima131=b.ima131),'',''",        #FUN-AA0024 add
#                     " FROM artg451_tmp a ",
#                     " GROUP BY a.ima131,oba02,lpk01,lpk04",
#                     " ORDER BY a.ima131,oba02"
#      WHEN tm.a='2'
#         LET l_sql = "SELECT ogaplant,azp02,a.ima131,oba02,'','',SUM(ogb14),",
#                     "SUM(ogb47),SUM(ogb14t),SUM(ogb14t_1),lpk01,lpk04,'',''",                          #TQC-B60299 add SUM(ogb14t_1)
#               #      " ,(select SUM(ohb14t) from artg451_tmp1 b where a.ima131=b.ima131)",              #FUN-AA0024 add      #TQC-B80077 mark
#               #      " ,(select SUM(ohb14t_1) from artg451_tmp1 b)",                                    #TQC-B60299 add      #TQC-B80077 mark
#               #      " ,(select SUM(ohb67)  from artg451_tmp1 b where a.ima131=b.ima131),'',''",        #FUN-AA0024 add      #TQC-B80077 mark
#                     " ,(SELECT SUM(ohb14t) FROM artg451_tmp1 b WHERE a.ima131=b.ima131 AND b.ohaplant = a.ogaplant)",              #FUN-AA0024 add      #TQC-B80077 add
#                     " ,(SELECT SUM(ohb14t_1) FROM artg451_tmp1 b WHERE a.ima131=b.ima131 AND b.ohaplant = a.ogaplant)",            #TQC-B60299 add      #TQC-B80077 add
#                     " ,(SELECT SUM(ohb67)  FROM artg451_tmp1 b WHERE a.ima131=b.ima131 AND b.ohaplant = a.ogaplant),'',''",        #FUN-AA0024 add      #TQC-B80077 add
#                     " FROM artg451_tmp a",
#                     " GROUP BY a.ima131,oba02,ogaplant,azp02,lpk01,lpk04",
#                     " ORDER BY a.ima131,oba02,ogaplant,azp02"              
#   END CASE                                                
#   PREPARE artg451_prepare2 FROM l_sql                                                
#   IF SQLCA.sqlcode != 0 THEN
#      CALL cl_err('prepare:',SQLCA.sqlcode,1) 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
#      EXIT PROGRAM
#   END IF
#   DECLARE artg451_curs2 CURSOR FOR artg451_prepare2   
#   FOREACH artg451_curs2 INTO sr.*
#      IF SQLCA.sqlcode != 0 THEN
#         CALL cl_err('foreach:',SQLCA.sqlcode,1)
#         EXIT FOREACH
#      END IF
...
###GENGRE###   LET g_str = tm.wc2
   CASE
      WHEN tm.a='1'
###GENGRE###         CALL cl_prt_cs3('artg451','artg451_1',l_sql,g_str) 
    LET g_template = 'artg451'   #FUN-B80161 
    CALL artg451_grdata()    ###GENGRE###
      WHEN tm.a='2'
###GENGRE###         CALL cl_prt_cs3('artg451','artg451_2',l_sql,g_str) 
    LET g_template = 'artg451_1'   #FUN-B80161 
    CALL artg451_grdata()    ###GENGRE###
   END CASE
END FUNCTION
#FUN-A60075
...
FUNCTION artg451_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg451")
        IF handler IS NOT NULL THEN
            START REPORT artg451_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE artg451_datacur1 CURSOR FROM l_sql
            FOREACH artg451_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg451_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg451_rep
        END IF
...
REPORT artg451_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE sr1_o sr1_t   #FUN-B80161
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_ogb14t LIKE ogb_file.ogb14t   #FUN-B80161
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
            LET sr1_o.ima131 = NULL   #FUN-B80161              
        ...
        ON EVERY ROW
            ...
            LET l_ogb14t = sr1.ogb14t - sr1.ohb14t   #FUN-B80161 add
            PRINTX l_ogb14t
            LET sr1_o.*  = sr1.* 
            #FUN-B80161---add---end---------
            PRINTX sr1.*
