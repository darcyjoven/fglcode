...
# Modify.........: No.TQC-B90113 11/08/10 By pauline 銷退的數值有誤 增加選取營運中心條件
...
MAIN
   ...
   LET l_table = cl_prt_temptable('artg452',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artg452_tm(0,0)        # Input print condition
      ELSE CALL artg452()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
   CALL cl_gre_drop_temptable(l_table)  #FUN-BA0061
END MAIN

FUNCTION artg452_tm(p_row,p_col)
   ...
   OPEN WINDOW artg452_w AT p_row,p_col WITH FORM "art/42f/artg452" 
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artg452_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)  #FUN-BA0061
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artg452',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artg452_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)  #FUN-BA0061
         EXIT PROGRAM
      END IF
...
#TQC-B60299 - ADD -  END  ------------------------------------
    ...
    IF STATUS THEN                                                                                                                   
       ...
       EXIT PROGRAM                                                                                                                 
    END IF  
    DROP TABLE artg452_tmp    
    CREATE TEMP TABLE artg452_tmp( 
           ogaplant LIKE oga_file.ogaplant, 
           azp02    LIKE azp_file.azp02, 
           ima131   LIKE ima_file.ima131,
           oba02    LIKE oba_file.oba02, 
    ...
    DELETE FROM artg452_tmp
    
    #FUN-AA0024 add ------------------------------begin-------------------------
    DROP TABLE artg452_tmp1    
    CREATE TEMP TABLE artg452_tmp1( 
           ohaplant LIKE oha_file.ohaplant, 
           ohb04    LIKE ohb_file.ohb04, 
           ohb12    LIKE ohb_file.ohb12,
           ohb14t   LIKE ohb_file.ohb14t, 
...
#FUN-C90116-------add-----end
      ...
      PREPARE artg452_prepare1 FROM l_sql                  
      IF SQLCA.sqlcode != 0 THEN                           
         ...
         EXIT PROGRAM                                      
      END IF                                               
      DECLARE artg452_curs1 CURSOR FOR artg452_prepare1    
 
      #TQC-B60299 add --start--
      CALL cl_replace_sqldb(l_sql2) RETURNING l_sql2
      CALL cl_parse_qry_sql(l_sql2,l_plant) RETURNING l_sql2
      PREPARE artg452_prepare3 FROM l_sql2
      DECLARE artg452_curs3 CURSOR FOR artg452_prepare3
      #TQC-B60299 add --end--

      LET sr.ogb14t_1 = 0  #TQC-B60299 add
      LET l_type = NULL    #TQC-B60299 add

      FOREACH artg452_curs1 INTO l_type,sr.* #TQC-B60299 add l_type
         IF SQLCA.sqlcode != 0 THEN
         ...
         #IF l_type = '1' THEN   #FUN-C90116 mark
            #FOREACH artg452_curs3 USING sr.ima01,sr.oga23,sr.oga24,'','','' INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 MOD #FUN-C90116 mark
            FOREACH artg452_curs3 USING sr.ima01,sr.oga23,sr.oga24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #FUN-C90116 add
...
#FUN-C90116-------mark----str
#        ELSE
#           FOREACH artg452_curs3 USING '','','',sr.ima01,sr.oga23,sr.oga24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 MOD
#             SELECT azi04 INTO t_azi04 FROM azi_file
#              WHERE azi01=l_oga23
#              IF l_oga213 = 'N' THEN
#                 LET l_ogb14_1 =l_ogb917*l_ogb37
...
#               #     " GROUP BY ohaplant,ima131,ohb04,oha23,oha24"         #FUN-BA0061 mark
#                    " GROUP BY ohaplant,ima131,ima01,ohb04,oha23,oha24"    #FUN-BA0061 add
#        #TQC-B60299 add --start--
...
#                       " AND ogb04 = ima01 AND ",tm.wc2 CLIPPED,
#                       " AND ogb14 < 0 ",
#                   #    " GROUP BY ogaplant,ima131,ogb04,oga23,oga24"                #FUN-BA0061 mark 
#                       " GROUP BY ogaplant,ima131,ima01,ogb04,oga23,oga24"          #FUN-BA0061 add
#        #TQC-B60299 add --start--
...
#FUN-C90116--------add-----end
      CALL cl_replace_sqldb(l_sql) RETURNING l_sql         
      #CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql        #FUN-AA0024 mark
      PREPARE artg452_prepare11 FROM l_sql                  
      IF SQLCA.sqlcode != 0 THEN                           
         CALL cl_err('prepare:',SQLCA.sqlcode,1)           
         CALL cl_used(g_prog,g_time,2) RETURNING g_time    
         CALL cl_gre_drop_temptable(l_table)  #FUN-BA0061
         EXIT PROGRAM                                      
      END IF                                               
      DECLARE artg452_curs11 CURSOR FOR artg452_prepare11    
 
      #TQC-B60299 add --start--
      CALL cl_replace_sqldb(l_sql2) RETURNING l_sql2
      CALL cl_parse_qry_sql(l_sql2,l_plant) RETURNING l_sql2
      PREPARE artg452_prepare4 FROM l_sql2
      DECLARE artg452_curs4 CURSOR FOR artg452_prepare4
      #TQC-B60299 add --end--

      LET sr1.ohb14t_1 = 0  #TQC-B60299 add
      LET l_type = NULL     #TQC-B60299 add

      FOREACH artg452_curs11 INTO l_type,l_ima01,sr1.* #TQC-B60299 add l_type,l_ima01
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
         ...
         LET l_ohb14t_1 = 0
         #IF l_type = '1' THEN   #FUN-C90116 mark
            #FOREACH artg452_curs4 USING l_ima01,sr1.oha23,sr1.oha24,'','','' INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 MOD #FUN-C90116 mark
            FOREACH artg452_curs4 USING l_ima01,sr1.oha23,sr1.oha24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #FUN-C90116 add
              SELECT azi04 INTO t_azi04 FROM azi_file
               WHERE azi01=l_oga23
               IF l_oga213 = 'N' THEN
...
#FUN-C90116---------mark----str
#        ELSE
#           FOREACH artg452_curs4 USING '','','',l_ima01,sr1.oha23,sr1.oha24 INTO l_oga23,l_oga211,l_oga213,l_ogb917,l_ogb37 #TQC-B60299 MOD
#             SELECT azi04 INTO t_azi04 FROM azi_file
#              WHERE azi01=l_oga23
#              IF l_oga213 = 'N' THEN
#                 LET l_ohb14_1 =l_ogb917*l_ogb37
...
#TQC-B90113 add START---------------------------------------
   CASE 
      WHEN tm.a = '1'
         LET  l_sql = " SELECT '','','','',ima01,'' FROM artg452_tmp ",
                      " UNION SELECT '','','','',ohb04,'' FROM artg452_tmp1"
      WHEN tm.a = '2'
         LET  l_sql = " SELECT ogaplant,'','','',ima01,'' FROM artg452_tmp ",
                      " UNION SELECT ohaplant,'','','',ohb04,'' FROM artg452_tmp1"
   END CASE
   PREPARE artg452_prepare5 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      CALL cl_gre_drop_temptable(l_table)  #FUN-BA0061
      EXIT PROGRAM
   END IF
   DECLARE artg452_curs5 CURSOR FOR artg452_prepare5
   FOREACH artg452_curs5 INTO   sr.ogaplant, sr.azp02, sr.ima131,sr.oba02, sr.ima01,sr.ima02
   ...
   CASE
      WHEN tm.a = '1'
         SELECT SUM(ogb12)    INTO sr.ogb12     FROM artg452_tmp  WHERE ima01 = sr.ima01
         SELECT SUM(ogb47)    INTO sr.ogb47     FROM artg452_tmp  WHERE ima01 = sr.ima01
         SELECT SUM(ogb14t)   INTO sr.ogb14t    FROM artg452_tmp  WHERE ima01 = sr.ima01
         SELECT SUM(ogb14t_1) INTO sr.ogb14t_1  FROM artg452_tmp  WHERE ima01 = sr.ima01
         SELECT lpk01         INTO sr.lpk01     FROM artg452_tmp  WHERE ima01 = sr.ima01
         SELECT lpk04         INTO sr.lpk04     FROM artg452_tmp  WHERE ima01 = sr.ima01
         SELECT oga23         INTO sr.oga23     FROM artg452_tmp  WHERE ima01 = sr.ima01
         SELECT oga24         INTO sr.oga24     FROM artg452_tmp  WHERE ima01 = sr.ima01
         SELECT SUM(ohb14t)   INTO sr.ohb14t    FROM artg452_tmp1 WHERE ohb04 = sr.ima01
         SELECT SUM(ohb14t_1) INTO sr.ohb14t_1  FROM artg452_tmp1 WHERE ohb04 = sr.ima01
         SELECT SUM(ohb67)    INTO sr.ohb67     FROM artg452_tmp1 WHERE ohb04 = sr.ima01
         SELECT SUM(ohb12)    INTO sr.ohb12     FROM artg452_tmp1 WHERE ohb04 = sr.ima01
         SELECT SUM(oha23)    INTO sr.oha23     FROM artg452_tmp1 WHERE ohb04 = sr.ima01
         SELECT SUM(oha24)    INTO sr.oha24     FROM artg452_tmp1 WHERE ohb04 = sr.ima01
      WHEN tm.a = '2'
         SELECT SUM(ogb12)    INTO sr.ogb12     FROM artg452_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT SUM(ogb47)    INTO sr.ogb47     FROM artg452_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT SUM(ogb14t)   INTO sr.ogb14t    FROM artg452_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT SUM(ogb14t_1) INTO sr.ogb14t_1  FROM artg452_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT lpk01         INTO sr.lpk01     FROM artg452_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT lpk04         INTO sr.lpk04     FROM artg452_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT oga23         INTO sr.oga23     FROM artg452_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT oga24         INTO sr.oga24     FROM artg452_tmp  WHERE ogaplant = sr.ogaplant AND ima01 = sr.ima01
         SELECT SUM(ohb14t)   INTO sr.ohb14t    FROM artg452_tmp1 WHERE ohaplant = sr.ogaplant AND ohb04 = sr.ima01
         SELECT SUM(ohb14t_1) INTO sr.ohb14t_1  FROM artg452_tmp1 WHERE ohaplant = sr.ogaplant AND ohb04 = sr.ima01
         SELECT SUM(ohb67)    INTO sr.ohb67     FROM artg452_tmp1 WHERE ohaplant = sr.ogaplant AND ohb04 = sr.ima01
         SELECT SUM(ohb12)    INTO sr.ohb12     FROM artg452_tmp1 WHERE ohaplant = sr.ogaplant AND ohb04 = sr.ima01
         SELECT SUM(oha23)    INTO sr.oha23     FROM artg452_tmp1 WHERE ohaplant = sr.ogaplant AND ohb04 = sr.ima01
         SELECT SUM(oha24)    INTO sr.oha24     FROM artg452_tmp1 WHERE ohaplant = sr.ogaplant AND ohb04 = sr.ima01
   END CASE
...
#               #      ",(select SUM(ohb14t) from artg452_tmp1 where ima01=ohb04)",            #FUN-AA0024 add    #FUN-BA0061 mark
#               #      ",(select SUM(ohb14t_1) from artg452_tmp1)",                            #TQC-B60299 add    #FUN-BA0061 mark
#               #      ",(select SUM(ohb67)  from artg452_tmp1 where ima01=ohb04)",            #FUN-AA0024 add    #FUN-BA0061 mark
#               #      ",(select SUM(ohb12)  from artg452_tmp1 where ima01=ohb04),'',''",      #FUN-AA0024 add    #FUN-BA0061 mark
#               #      " FROM artg452_tmp  ",                                                                     #FUN-BA0061 mark
#                     ",(SELECT SUM(ohb14t) FROM artg452_tmp1 b WHERE a.ima01=b.ohb04 )",           #FUN-BA0061 add
#                     ",(SELECT SUM(ohb14t_1) FROM artg452_tmp1 b WHERE a.ima01=b.ohb04 )",         #FUN-BA0061 add
#                     ",(SELECT SUM(ohb67)  FROM artg452_tmp1 b WHERE a.ima01=b.ohb04 )",           #FUN-BA0061 add
#                     ",(SELECT SUM(ohb12)  FROM artg452_tmp1 b WHERE a.ima01=b.ohb04 ),'',''",     #FUN-BA0061 add
#                     " FROM artg452_tmp a",
...
#                #     ",(select SUM(ohb12)  from artg452_tmp1 where ima01=ohb04),'',''",      #FUN-AA0024 add    #FUN-BA0061 mark
#                #     " FROM artg452_tmp",                                                                       #FUN-BA0061 mark
#                     ",(SELECT SUM(ohb14t) FROM artg452_tmp1 b WHERE ima01=ohb04 AND b.ohaplant = a.ogaplant)",           #FUN-BA0061 add
#                     ",(SELECT SUM(ohb14t_1) FROM artg452_tmp1 b WHERE ima01=ohb04 AND b.ohaplant = a.ogaplant)",         #FUN-BA0061 add
#                     ",(SELECT SUM(ohb67)  FROM artg452_tmp1 b WHERE ima01=ohb04 AND b.ohaplant = a.ogaplant)",           #FUN-BA0061 add
#                     ",(SELECT SUM(ohb12)  FROM artg452_tmp1 b WHERE ima01=ohb04 AND b.ohaplant = a.ogaplant),'',''",     #FUN-BA0061 add
#                     " FROM artg452_tmp a",                                                                               #FUN-BA0061 add
#                     " GROUP BY ima01,ima02,ogaplant,azp02,lpk01,lpk04", 
#                     " ORDER BY ima01,ima02,ogaplant,azp02"
#   END CASE                                                
#   PREPARE artg452_prepare2 FROM l_sql                                                
#   IF SQLCA.sqlcode != 0 THEN
#      CALL cl_err('prepare:',SQLCA.sqlcode,1) 
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
#      EXIT PROGRAM
#   END IF
#   DECLARE artg452_curs2 CURSOR FOR artg452_prepare2   
#   FOREACH artg452_curs2 INTO sr.*
#      IF SQLCA.sqlcode != 0 THEN
#         CALL cl_err('foreach:',SQLCA.sqlcode,1)
#         EXIT FOREACH
#      END IF
...
###GENGRE###   LET g_str = tm.wc2
   CASE
      WHEN tm.a='1'
###GENGRE###         CALL cl_prt_cs3('artg452','artg452_1',l_sql,g_str)  
    LET g_template = 'artg452'
    CALL artg452_grdata()    ###GENGRE###
      WHEN tm.a='2'
###GENGRE###         CALL cl_prt_cs3('artg452','artg452_2',l_sql,g_str) 
    LET g_template = 'artg452_1'
    CALL artg452_grdata()    ###GENGRE###
   END CASE
END FUNCTION
...
FUNCTION artg452_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg452")
        IF handler IS NOT NULL THEN
            START REPORT artg452_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY ogaplant,ima01"       #FUN-BA0061 add
          
            DECLARE artg452_datacur1 CURSOR FROM l_sql
            FOREACH artg452_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg452_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg452_rep
        END IF
