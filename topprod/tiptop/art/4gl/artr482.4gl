...
MAIN
   ...
   LET l_table = cl_prt_temptable('artr482',g_sql) CLIPPED
   IF  l_table = -1 THEN  EXIT PROGRAM  END IF           
   CALL cl_used(g_prog,g_time,1) RETURNING g_time

   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artr482_tm(0,0)        # Input print condition
      ELSE CALL artr482()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN   
FUNCTION artr482_tm(p_row,p_col)
   ...
   OPEN WINDOW artr482_w AT p_row,p_col WITH FORM "art/42f/artr482" 
...
#               END IF 
#               IF g_chk_shop THEN
#                  DECLARE r482_zxy_cs1 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
#                  FOREACH r482_zxy_cs1 INTO l_zxy03 
#                    IF g_chk_auth IS NULL THEN
#                       LET g_chk_auth = "'",l_zxy03,"'"
#                    ELSE
#                       LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
...
#         END IF
#         IF INT_FLAG THEN
#            LET INT_FLAG = 0 CLOSE WINDOW artr482_w 
#            CALL cl_used(g_prog,g_time,2) RETURNING g_time 
#            EXIT PROGRAM
#         END IF
#   
...
#FUN-C70007 add begin ---
         ...
         ON ACTION accept
            ...
            END IF
            IF g_chk_shop THEN
               DECLARE r482_zxy_cs2 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH r482_zxy_cs2 INTO l_zxy03
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artr482',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr482_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
...
END CASE
  ...
  #FUN-BC0026 add START
   ...
   IF g_zz05 = 'Y' THEN
      ...
      RETURNING tm.wc
      CALL cl_wcchp(tm.wc1,'shop') RETURNING tm.wc1 #FUN-CA0087 Add
     #LET g_str =tm.wc                 #FUN-CA0087 Mark
      LET g_str = tm.wc1," AND ",tm.wc #FUN-CA0087 Add
   ...
   END IF
  #FUN-BC0026 add END
  CALL cl_prt_cs1('artr482','artr482',l_sql,g_str)  
END FUNCTION
...
DEFINE  l_sr  DYNAMIC ARRAY OF RECORD
           SHOP       LIKE oga_file.ogaplant,
           SNO        LIKE ima_file.ima131,
           PROD       LIKE ima_file.ima01,
           CNFDATE    LIKE type_file.chr8,
...
DEFINE l_cnt      LIKE type_file.num5 #FUN-CA0087 Add
  ...
  #FUN-AA0024 add ###begin###
    ...
    FOREACH sel_azp01_cs INTO l_azp01        
     ...
     #         ELSE
     #         #FUN-B40029-add-end--
     #      LET l_sql = "SELECT SHOP,'',PROD,'','',SUM(PRICE)",
     #                  " FROM ",g_posdb,"POSDB",g_posdb_link,
     ...
     #FUN-C70007 add begin ---
        ...
        #FUN-CA0087 Mark End -----
         IF l_db_type='MSV' THEN
           ...
           #LET l_sql = "SELECT SHOP,'',ima01,'','', SUM(PRICE)",
           ...
           #            " AND ",tm.wc CLIPPED,
           #            " GROUP BY SHOP,ima01 "
            LET l_sql = "SELECT td_Sale_Detail.SHOP,'',ima01,'','',SUM(CASE Type WHEN 0 THEN AMT ELSE AMT*(-1) END)",
                        "  FROM ",g_posdb,"td_Sale_Detail",g_posdb_link,
                        ...
                        "   AND Type IN ('0','1','2')",
                        "   AND ",tm.wc CLIPPED,
                        " GROUP BY td_Sale_Detail.SHOP,ima01 "
     ...
     #FUN-C70007 add end  --- 
         ...
         PREPARE artr482_prepare1 FROM l_sql
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('prepare:',SQLCA.sqlcode,1) 
            CALL cl_used(g_prog,g_time,2) RETURNING g_time 
            EXIT PROGRAM
         END IF
         DECLARE artr482_curs1 CURSOR FOR artr482_prepare1
         ...
         IF tm.d = 'Y' THEN
           ...
           #IF cl_null(tm.bdate) AND cl_null(tm.edate) THEN
           #  #FUN-CA0087 Mark&Add Begin ---
           #  #LET l_sql = "SELECT PROD ",
           #  #            "  FROM ",g_posdb,"POSDB",g_posdb_link,                                            
           #  #            " WHERE ",
           #  #            #" type = '0' AND ", #FUN-AA0024 mark
           #  #            "       (trans_type is null or trans_type ='0') AND  ",tm.wc CLIPPED, #FUN-CA0087 Mod
           #  #            "   AND SHOP = '",l_azp01 CLIPPED,"' ", #FUN-C70007 Add
           #  #            " GROUP BY PROD "
           #   LET l_sql = "SELECT DISTINCT(CASE WHEN FEATURENO IS NULL THEN PLUNO WHEN FEATURENO = ' ' THEN PLUNO ELSE FEATURENO) END ",
           #               "  FROM ",g_posdb,"td_Sale_Detail",g_posdb_link,
           #               "      ,",cl_get_target_table(l_azp01,'ima_file'),
           #               " WHERE ",tm.wc CLIPPED,
           #               " AND (EXISTS (SELECT 1 FROM ",cl_get_target_table(l_azp01,'imx_file'),
           ...
           #               "                       WHERE imx000 = FeatureNO AND imx00 = PLUNO) AND PLUNO = ima01)) ",
           #               "   AND SHOP = '",l_azp01 CLIPPED,"' ",
           #               "   AND (SDATE BETWEEN '",l_bdate,"' AND '",l_edate,"') "
           #   #FUN-CA0087 Mark&Add End -----
           # END IF                 
           # CALL cl_replace_sqldb(l_sql) RETURNING l_sql            
           # #CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql    
           # PREPARE prod_prepare1 FROM l_sql
           # IF SQLCA.sqlcode != 0 THEN
           #    CALL cl_err('prepare:',SQLCA.sqlcode,1) 
           #    CALL cl_used(g_prog,g_time,2) RETURNING g_time 
           #    EXIT PROGRAM
           # END IF
           # DECLARE prod_curs1 CURSOR FOR prod_prepare1
             
             
             FOREACH artr482_curs1 INTO l_sr[l_i].*
               IF SQLCA.sqlcode != 0 THEN
                  CALL cl_err('foreach:',SQLCA.sqlcode,1)
                  EXIT FOREACH
               END IF  
               #IF cl_null(l_sr[l_i].ohb14t) THEN 
               #   LET l_sr[l_i].ohb14t = 0 
               #END IF 
               LET l_i = l_i + 1
             END FOREACH
            
            CALL l_sr.deleteElement(l_i)
         
              
           #FOREACH prod_curs1 INTO l_prod[l_p].PROD
           #   IF SQLCA.sqlcode != 0 THEN
           #      CALL cl_err('foreach:',SQLCA.sqlcode,1)
           #      EXIT FOREACH
           #   END IF 
           #   IF l_sr.getLength() = 0 THEN
           #      LET l_sr[l_p].SHOP = l_azp01
           #      LET l_sr[l_p].PROD = l_prod[l_p].PROD
           #      LET l_sr[l_p].PRICE = 0
           #   END IF  
           #   LET l_p = l_p + 1
           #END FOREACH
            
            FOR l_i =1 to l_sr.getLength()
               #LET sr.*= l_sr[l_i].*
               EXECUTE  insert_prep  USING  
               l_sr[l_i].SHOP,l_sr[l_i].SNO,l_sr[l_i].PROD,
               l_sr[l_i].CNFDATE,l_sr[l_i].CNFTIME,l_sr[l_i].PRICE,s[i].str
     ...
     #FUN-CA0087 Add Begin ---
      IF tm.d = 'Y' THEN
         LET l_sql = "SELECT COUNT(DISTINCT STR) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED," WHERE SHOP = '",l_azp01,"' "
         PREPARE sel_count_pre FROM l_sql
         EXECUTE sel_count_pre INTO l_cnt
         IF l_cnt > 0 AND l_cnt <  l_n THEN
            INITIALIZE sr.* TO NULL
            LET l_sql = "SELECT PROD FROM ",g_cr_db_str CLIPPED,l_table CLIPPED," WHERE SHOP = '",l_azp01,"' "
            PREPARE sel_sno_pre FROM l_sql
            EXECUTE sel_sno_pre INTO sr.prod
            LET t = 0
            LET btime = 0
            LET etime = 0
            WHILE t < 24
               ...
               END IF
               IF etime < 10 THEN
                  LET str = str||"0"||etime||":00"
               ELSE
                  LET str = str||etime||":00"
               END IF
               LET l_cnt = 0
               LET l_sql = "SELECT COUNT(*) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED," WHERE SHOP = '",l_azp01,"' AND STR = '",str CLIPPED,"' "
               PREPARE sel_count_pre2 FROM l_sql
               EXECUTE sel_count_pre2 INTO l_cnt
               IF l_cnt = 0 THEN
                  EXECUTE  insert_prep  USING
                  l_azp01,sr.SNO,sr.PROD,
                  sr.CNFDATE,sr.CNFTIME,'0',str
               END IF
            END WHILE
         END IF
      END IF
