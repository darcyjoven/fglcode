...
MAIN
   ...
   LET g_sql=  "order1.type_file.num10,",
    ...
    LET l_table = cl_prt_temptable('artr211',g_sql) CLIPPED
    ...
    IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN 
       CALL artr211_tm()        # Input print condition
    ELSE 
       CALL artr211() 
    END IF
...
DEFINE l_sql          STRING
   ...
   OPEN WINDOW artr211_w AT p_row,p_col WITH FORM "art/42f/artr211" 
   ...
   WHILE TRUE
      CONSTRUCT BY NAME tm.wc ON ruw02,ruw04,ruwplant,rtz10,ima131,rty05
        BEFORE CONSTRUCT
         ...
         AFTER CONSTRUCT
            IF INT_FLAG THEN
               LET INT_FLAG = 0 CLOSE WINDOW artr211_w 
               CALL cl_used(g_prog,g_time,2) RETURNING g_time 
               EXIT PROGRAM
            END IF  
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd1 CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artr211',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr211_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
DEFINE l_order1  LIKE type_file.num10
    ...
    DROP TABLE artr211_tmp
    CREATE TEMP TABLE artr211_tmp(
...
#       LET l_sql = l_sql,"   UNION SELECT ruwplant,rux03,ima02,ima131,rux01,rux02,rux04,rux05,rux06,rux07,rux08 FROM ",    #TQC-B70204 mark
      ...
      PREPARE artr211_prepare1 FROM l_sql                  
      IF SQLCA.sqlcode != 0 THEN                           
         CALL cl_err('prepare:',SQLCA.sqlcode,1)           
         CALL cl_used(g_prog,g_time,2) RETURNING g_time    
         EXIT PROGRAM                                      
      END IF                                               
      DECLARE artr211_curs1 CURSOR FOR artr211_prepare1 
      FOREACH artr211_curs1 INTO sr.*  
         IF SQLCA.sqlcode != 0 THEN
         ...
         INSERT INTO artr211_tmp VALUES(0,sr.ruwplant,l_azw08,sr.rux03,sr.ima02,sr.ima131,l_oba02,sr.rux01,sr.rux02,
   ...
   IF tm.type1='1' THEN 
      IF tm.type2='1' THEN
         LET l_sql = "SELECT rux03,rux04,SUM(income) FROM artr211_tmp "
      END IF
      IF tm.type2='2' THEN
         LET l_sql = "SELECT rux03,rux04,SUM(rux08) FROM artr211_tmp "
      END IF
      IF tm.cb1  = 'Y' THEN
      ...
      IF tm.type2 = '1' OR tm.type2 = '2' THEN
         ...
         DECLARE r211_sum_curs CURSOR FOR sel_sum_pre
         FOREACH r211_sum_curs INTO l_rux03,l_rux04,l_order1
            UPDATE artr211_tmp SET order1 = l_order1
             WHERE rux03 = l_rux03
               AND rux04 = l_rux04
         END FOREACH
      END IF
   ...
   IF tm.type1='2' THEN
      IF tm.type2='1' THEN
         LET l_sql = "SELECT ima131,rux04,SUM(income) FROM artr211_tmp "
      END IF
      IF tm.type2='2' THEN
         LET l_sql = "SELECT ima131,rux04,SUM(rux08) FROM artr211_tmp "
      END IF
      IF tm.cb1 = 'Y' THEN
         LET l_sql = l_sql," WHERE rux08 <> 0 "
      END IF
      ...
      IF tm.type2 = '1' OR tm.type2 = '2' THEN
         ...
         DECLARE r211_sum_curs1 CURSOR FOR sel_sum_pre1
         FOREACH r211_sum_curs1 INTO l_ima131,l_rux04,l_order1
   ...
   DECLARE r211_curs CURSOR FOR SELECT * FROM artr211_tmp
   FOREACH r211_curs INTO l_order1,sr.ruwplant,l_azw08,sr.rux03,sr.ima02,sr.ima131,l_oba02,sr.rux01,sr.rux02,
                 sr.rux04,sr.rux05,sr.rux06,sr.rux07,sr.rux08,l_rtg05,l_income
   ...
   LET g_str = g_str,";",tm.type1,";",tm.type2,';',tm.cb1,";",tm.cb2,";"
    CASE
      WHEN tm.type1='1'
         CALL cl_prt_cs3('artr211','artr211_1',l_sql,g_str)  
      WHEN tm.type1='2'
         CALL cl_prt_cs3('artr211','artr211_2',l_sql,g_str) 
    END CASE
