...
MAIN
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artr202_tm(0,0)        # Input print condition
      ELSE CALL artr202()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN

FUNCTION artr202_tm(p_row,p_col)
   ...
   OPEN WINDOW artr202_w AT p_row,p_col WITH FORM "art/42f/artr202" 
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artr202_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
     ...
     #INPUT BY NAME tm.edate,tm.type,tm.a,tm.more WITHOUT DEFAULTS
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('artr202',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr202_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
#   DEFINE l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT         #TQC-B70068 mark
   ...
   LET l_table = cl_prt_temptable('artr202',g_sql) CLIPPED
   IF  l_table = -1 THEN 
   ...
   IF STATUS THEN                                                                                                                   
      ...
      EXIT PROGRAM                                                                                                                 
   END IF
   DROP TABLE artr202_tmp

   CREATE TEMP TABLE artr202_tmp(
   ...
   FOREACH sel_azp01_cs INTO l_plant,l_azp02  
      ...
      FOREACH r200_curs1 INTO sr.azw01,sr.img01,sr.img02,sr.img03,sr.img04,
                              ...
                              sr.img09   #FUN-BB0018
         SELECT COUNT(*) INTO l_cnt FROM artr202_tmp  #--己存在不覆蓋
   ...
   CASE
      WHEN tm.a='1'
        #LET l_sql = "SELECT '','',ima01,ima02,'','',img02,SUM(img10),SUM(amt) ",
         LET l_sql = "SELECT '','',ima01,ima02,'','',img02,img09,SUM(img10),SUM(amt) ",   #FUN-BB0018 add img09
                     " FROM artr202_tmp ",
                    #" GROUP BY ima01,ima02,img02"
                     " GROUP BY ima01,ima02,img02,img09"  #FUN-BB0018 add img09
      WHEN tm.a='2'
        #LET l_sql = "SELECT azp01,azp02,ima01,ima02,'','',img02,SUM(img10),SUM(amt) ",
         LET l_sql = "SELECT azp01,azp02,ima01,ima02,'','',img02,img09,SUM(img10),SUM(amt) ",   #FUN-BB0018 add img09
                     " FROM artr202_tmp ",
                    #" GROUP BY ima01,ima02,azp01,azp02,img02"
                     " GROUP BY ima01,ima02,azp01,azp02,img02,img09"   #FUN-BB0018 add img09
   END CASE

   PREPARE artr202_prepare2 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM
   END IF
   DECLARE artr202_curs2 CURSOR FOR artr202_prepare2

   FOREACH artr202_curs2 INTO sr.azw01,l_azp02,sr.ima01,sr.ima02,
      ...
      IF SQLCA.sqlcode != 0 THEN
         ...
         EXIT FOREACH
      END IF
      SELECT SUM(amt) INTO l_amts FROM artr202_tmp WHERE azp01 = sr.azw01
      IF l_amts  <> 0 THEN
  ...
  #FUN-BC0026 add END
   ...
   CASE
      WHEN tm.a='1'
         CALL cl_prt_cs3('artr202','artr202_1',l_sql,g_str)  
      WHEN tm.a='2'
         CALL cl_prt_cs3('artr202','artr202_2',l_sql,g_str) 
   END CASE
END FUNCTION 
#FUN-A80082
