...
# Modify.........: No.FUN-BA0062 11/10/18 By wangning 明細CR報表轉GR
# Modify.........: No.FUN-BA0062 12/01/18 By xuxz FUN-BB0018 追單
...
DEFINE tm          RECORD                          # Print condition RECORD
               #    wc1     LIKE type_file.chr1000, # Where condition   #TQC-B70068
               #    wc2     LIKE type_file.chr1000, # Where condition   #TQC-B70068
               #    wc3     LIKE type_file.chr1000, # Where condition   #TQC-B70068
                   wc1     STRING,     #TQC-B70068
                   wc2     STRING,     #TQC-B70068
                   wc3     STRING,     #TQC-B70068
                   edate   LIKE type_file.dat,    
                   type    LIKE type_file.chr1,    
                   w       LIKE type_file.chr1,#FUN-BA0062
                   x       LIKE type_file.chr1,#FUN-BA0062
...
MAIN
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artg201_tm(0,0)        # Input print condition
      ELSE CALL artg201()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
   CALL cl_gre_drop_temptable(l_table)   #FUN-BA0062
END MAIN

FUNCTION artg201_tm(p_row,p_col)
   ...
   OPEN WINDOW artg201_w AT p_row,p_col WITH FORM "art/42f/artg201" 
   ...
   LET tm.wc3 = ' 1=1'
   LET tm.type = '1'
   LET tm.w = 'Y' #FUN-BA0062
   LET tm.x = 'Y' #FUN-BA0062
   LET tm.a = '1' 
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artg201_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)   #FUN-BA0062
         EXIT PROGRAM
      END IF
     ...
     DISPLAY BY NAME  tm.edate,tm.type,tm.w,tm.x,tm.a,tm.more #FUN-BA0062 add tm.x,tm.w
      
     #INPUT BY NAME tm.edate,tm.type,tm.a,tm.more WITHOUT DEFAULTS
      INPUT BY NAME tm.edate,tm.type,tm.w,tm.x,tm.a,tm.more WITHOUT DEFAULTS  #FUN-BA0062 add tm.x,tm.w
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_copies CLIPPED,"'",
                       " '",tm.wc1 CLIPPED,"'" ,
                       " '",tm.wc2 CLIPPED,"'" ,#FUN-BA0062 add
                       " '",tm.wc3 CLIPPED,"'" ,#FUN-BA0062 add
                       " '",tm.w CLIPPED,"'" ,    #FUN-BA0062 add
                       " '",tm.a CLIPPED,"'" ,#FUN-BA0062 add
                       " '",g_rep_user CLIPPED,"'",           
                       " '",g_rep_clas CLIPPED,"'",           
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'",           
                       " '",tm.x CLIPPED,"'"     #FUN-BA0062 add 
            CALL cl_cmdat('artg201',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artg201_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)   #FUN-BA0062
         EXIT PROGRAM
      END IF
...
FUNCTION artg201()
 #  DEFINE l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT      #TQC-B70068
   ...
   LET l_table = cl_prt_temptable('artg201',g_sql) CLIPPED
   IF  l_table = -1 THEN 
   ...
   IF STATUS THEN                                                                                                                   
      ...
      EXIT PROGRAM                                                                                                                 
   END IF
   DROP TABLE artg201_tmp

   CREATE TEMP TABLE artg201_tmp(
          azp01  LIKE azp_file.azp01,
          azp02  LIKE azp_file.azp02,
          ima01  LIKE ima_file.ima01,
          ima02  LIKE ima_file.ima02,
   ...
   FOREACH sel_azp01_cs INTO l_plant,l_azp02  
      ...
      FOREACH r200_curs1 INTO sr.azw01,sr.img01,sr.img02,sr.img03,sr.img04,
         ...
         SELECT COUNT(*) INTO l_cnt FROM artg201_tmp  #--己存在不覆蓋
         ...
         IF SQLCA.sqlcode THEN                                                     
            LET l_oba02 = ''                                                                                                        
         END IF
         INSERT INTO artg201_tmp 
         VALUES(sr.azw01,l_azp02,sr.ima01,sr.ima02,sr.ima131,l_oba02,sr.img02,l_img10,l_amt)
      END FOREACH
   END FOREACH 
   CASE
      WHEN tm.a='1'
         LET l_sql = "SELECT '','','','',ima131,oba02,img02,SUM(img10),SUM(amt) ",
                     " FROM artg201_tmp ",
                     " GROUP BY ima131,oba02,img02"
      WHEN tm.a='2'
         LET l_sql = "SELECT azp01,azp02,'','',ima131,oba02,img02,SUM(img10),SUM(amt) ",
                     " FROM artg201_tmp ",
                     " GROUP BY ima131,oba02,azp01,azp02,img02"
   END CASE

   PREPARE artg201_prepare2 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE artg201_curs2 CURSOR FOR artg201_prepare2

   FOREACH artg201_curs2 INTO sr.azw01,l_azp02,sr.ima01,sr.ima02,
                              sr.ima131,l_oba02,sr.img02,l_img10,l_amt
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
      SELECT SUM(amt) INTO l_amts FROM artg201_tmp WHERE azp01 = sr.azw01
      IF l_amts  <> 0 THEN
...
###GENGRE###   LET g_str = tm.wc1," AND ",tm.wc2," AND ",tm.wc3
   CASE
      WHEN tm.a='1'
###GENGRE###         CALL cl_prt_cs3('artg201','artg201_1',l_sql,g_str)  
    LET g_template = 'artg201'
    CALL artg201_grdata()    ###GENGRE###
      WHEN tm.a='2'
###GENGRE###         CALL cl_prt_cs3('artg201','artg201_2',l_sql,g_str) 
    LET g_template = 'artg201_1'
    CALL artg201_grdata()    ###GENGRE###
   END CASE
END FUNCTION 
#FUN-A80082
...
FUNCTION artg201_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg201")
        IF handler IS NOT NULL THEN
            START REPORT artg201_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            #FUN-BA0062 add  start----------------------
            CASE
               WHEN tm.a='1'
                  LET l_sql = l_sql CLIPPED," ORDER BY ima131,img02"
               WHEN tm.a='2'
                  LET l_sql = l_sql CLIPPED," ORDER BY ima131,azp01,img02"
            END CASE
            #FUN-BA0062 add  end------------------------
            DECLARE artg201_datacur1 CURSOR FROM l_sql
            FOREACH artg201_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg201_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg201_rep
        END IF
...
REPORT artg201_rep(sr1)
    ...
    DEFINE l_img10_sum LIKE img_file.img10  #FUN-BA0062 xuxz add 2012.1.19
    DEFINE l_amt_sum   LIKE type_file.num15_3 #FUN-BA0062 xuxz add 2012.1.19
    DEFINE l_img10_tot LIKE img_file.img10  #FUN-BA0062 xuxz add 2012.1.19
    DEFINE l_amt_tot   LIKE type_file.num15_3 #FUN-BA0062 xuxz add 2012.1.19
    ...
    FORMAT
        ...
        AFTER GROUP OF sr1.ima131
           LET l_img10_sum = GROUP SUM(sr1.img10)
           PRINTX l_img10_sum
