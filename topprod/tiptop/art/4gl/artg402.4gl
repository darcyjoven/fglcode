...
# Modify.........: No.FUN-B80161 12/01/16 By yangtt FUN-BB0032追單
# Modify.........: NO.FUN-CB0058 12/11/21 By yangtt 4rp中的visibility condition在4gl中實現，達到單身無定位點
...
DEFINE tm  RECORD                # Print condition RECORD
              ...
              w       LIKE type_file.chr1,      #FUN-B80161 FROM FUN-BB0032
              x       LIKE type_file.chr1,      #FUN-B80161 FROM FUN-BB0032
...
TYPE sr1_t RECORD
    ...
    oeb05 LIKE oeb_file.oeb05, #FUN-B80161 FROM FUN-BB0032
...
MAIN
   OPTIONS
       INPUT NO WRAP
   DEFER INTERRUPT                # FUN-A80101 Supress DEL key function
   ...
   LET tm.a = ARG_VAL(8)
   LET tm.b = ARG_VAL(9)
   LET tm.w = ARG_VAL(10)   #FUN-B80161 FROM FUN-BB0032
   LET tm.x = ARG_VAL(11)   #FUN-B80161 FROM FUN-BB0032
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artg402_tm(0,0)        # Input print condition
      ELSE CALL artg402()            # Read data and create out-file
   END IF
...
FUNCTION artg402_tm(p_row,p_col)
   ...
   OPEN WINDOW artg402_w AT p_row,p_col WITH FORM "art/42f/artg402" 
   ...
   LET tm.a = '1' 
   LET tm.b = '1'  
   LET tm.w = 'Y'    #FUN-B80161 FROM FUN-BB0032
   LET tm.x = 'Y'    #FUN-B80161 FROM FUN-BB0032
    ...
    WHILE TRUE
        CONSTRUCT BY NAME tm.wc1 ON azp01
         ...
         AFTER FIELD azp01 
            ...
            END IF 
            IF g_chk_azp01 THEN
               DECLARE g402_zxy_cs1 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH g402_zxy_cs1 INTO l_zxy03 
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
      ...
      END IF
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artg402_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)   #FUN-B80161 
         EXIT PROGRAM
      END IF
     ...
     #DISPLAY BY NAME tm.a,tm.b,tm.more
      DISPLAY BY NAME tm.a,tm.b,tm.w,tm.x,tm.more   #FUN-B80161 FROM FUN-BB0032

     #INPUT BY NAME tm.a,tm.b,tm.more WITHOUT DEFAULTS
      INPUT BY NAME tm.a,tm.b,tm.w,tm.x,tm.more WITHOUT DEFAULTS  #FUN-B80161 FROM FUN-BB0032
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",tm.a CLIPPED,"'" ,
                       " '",tm.b CLIPPED,"'" ,
                       " '",tm.w CLIPPED,"'" ,   #FUN-B80161 FROM FUN-BB0032
                       " '",tm.x CLIPPED,"'" ,   #FUN-B80161 FROM FUN-BB0032
                       " '",g_rep_user CLIPPED,"'",           
                       " '",g_rep_clas CLIPPED,"'",           
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artg402',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artg402_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)   #FUN-B80161 
         EXIT PROGRAM
      END IF
...
FUNCTION artg402()
   ...
   DEFINE l_name    LIKE type_file.chr20,         # External(Disk) file name        
    ...
    LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,                                                                 
                " VALUES(?, ?, ?, ?, ?, ?,  ?, ?, ?, ?, ?) "     #FUN-B80161 FROM FUN-BB0032 add ?                                                                                                       
    ...
    IF STATUS THEN                                                                                                                   
       ...
       EXIT PROGRAM
    END IF
    DROP TABLE artg402_tmp    
    CREATE TEMP TABLE artg402_tmp( 
           oeaplant LIKE oea_file.oeaplant, 
           azp02    LIKE azp_file.azp02, 
           ima131   LIKE ima_file.ima131,
           oba02    LIKE oba_file.oba02, 
...
CASE tm.a
     WHEN '1'  
      ...
      IF l_lpk01_find = 0 THEN 
            #LET l_sql = "SELECT oeaplant,'',ima131,'',ima01,ima02,SUM(oeb12),",
             LET l_sql = "SELECT oeaplant,'',ima131,'',ima01,ima02,oeb05,SUM(oeb12),",  #FUN-B80161 FROM FUN-BB0032
                        ...
                        " GROUP BY oeaplant,ima131,ima01,ima02,oeb05,oea23,oea24"   #FUN-B80161 FROM FUN-BB0032  add   oeb05
      ELSE 
        #LET l_sql = "SELECT oeaplant,'',ima131,'',ima01,ima02,SUM(oeb12),",
         LET l_sql = "SELECT oeaplant,'',ima131,'',ima01,ima02,oeb05,SUM(oeb12),",  #FUN-B80161 FROM FUN-BB0032
                     ...
                     " AND oeb04 = ima01 AND ",tm.wc CLIPPED, 
                     " GROUP BY oeaplant,ima131,ima01,ima02,oeb05,lpk01,lpk04,oea23,oea24"  #FUN-B80161 FROM FUN-BB0032  add   oeb05
    ...
    PREPARE artg402_prepare1 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       ...
       EXIT PROGRAM
    END IF
    DECLARE artg402_curs1 CURSOR FOR artg402_prepare1

    FOREACH artg402_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
    ...
    LET l_sql="SELECT azi10 FROM ",cl_get_target_table(l_plant,'azi_file'),
         ...
         CASE l_azi10
            ...
            WHEN '2'  
               LET sr.oeb14t = sr.oeb14t/sr.oea24 
         END CASE
         INSERT INTO artg402_tmp
         VALUES(sr.oeaplant,l_azp02,sr.ima131,l_oba02,sr.ima01,
        #sr.ima02,sr.oeb12,sr.oeb14t,sr.lpk01,sr.lpk04)
         sr.ima02,sr.oeb05,sr.oeb12,sr.oeb14t,sr.lpk01,sr.lpk04)  #FUN-B80161 FROM FUN-BB0032
         LET l_oba02 =''          
      END FOREACH 
  END FOREACH 
CASE
   WHEN tm.b='1'
         #LET l_sql = " SELECT '','','','',ima01,ima02,SUM(oeb12),",
          LET l_sql = " SELECT '','','','',ima01,ima02,oeb05,SUM(oeb12),",  #FUN-B80161 FROM FUN-BB0032
                " SUM(oeb14t),lpk01,lpk04,'','' ", 
                " FROM artg402_tmp", 
                " GROUP BY ima01,ima02,oeb05,lpk01,lpk04",    #FUN-B80161 add oeb05  FROM FUN-BB0032
                " ORDER BY ima01,ima02,oeb05"                 #FUN-B80161 add oeb05  FROM FUN-BB0032
       
       WHEN tm.b='2'
         #LET l_sql =" SELECT oeaplant,azp02,'','',ima01,ima02,SUM(oeb12),",
          LET l_sql =" SELECT oeaplant,azp02,'','',ima01,ima02,oeb05,SUM(oeb12),",  #FUN-B80161 FROM FUN-BB0032 
                " SUM(oeb14t),lpk01,lpk04,'','' ",
                " FROM artg402_tmp", 
                " GROUP BY oeaplant,azp02,ima01,ima02,oeb05,lpk01,lpk04",  #FUN-B80161 add oeb05  FROM FUN-BB0032
                " ORDER BY ima01,ima02,oeaplant,azp02,oeb05"               #FUN-B80161 add oeb05  FROM FUN-BB0032
    END CASE
    PREPARE artg402_prepare2 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       CALL cl_gre_drop_temptable(l_table)   #FUN-B80161 
       EXIT PROGRAM
    END IF
    DECLARE artg402_curs2 CURSOR FOR artg402_prepare2

    FOREACH artg402_curs2 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
     ...
     #sr.oba02,sr.ima01,sr.ima02,sr.oeb12,sr.oeb14t,sr.lpk01,sr.lpk04  #FUN-B80161 FROM FUN-BB0032
      sr.oba02,sr.ima01,sr.ima02,sr.oeb05,sr.oeb12,sr.oeb14t,sr.lpk01,sr.lpk04  #FUN-B80161 FROM FUN-BB0032
      
    
END FOREACH
###GENGRE###   LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
###GENGRE###   LET g_str = tm.wc,";",tm.w,";",tm.x   #FUN-B80161 FROM FUN-BB0032 add tm.w,tm.x
   CASE
      WHEN tm.b='1'
###GENGRE###         CALL cl_prt_cs3('artg402','artg402_1',l_sql,g_str)  
    LET g_template = 'artg402'   #FUN-B80161 add
    CALL artg402_grdata()    ###GENGRE###
      WHEN tm.b='2'
###GENGRE###         CALL cl_prt_cs3('artg402','artg402_2',l_sql,g_str) 
    LET g_template = 'artg402_1'   #FUN-B80161 add
    CALL artg402_grdata()    ###GENGRE###
   END CASE
END FUNCTION

###GENGRE###START
FUNCTION artg402_grdata()
    DEFINE l_sql    STRING
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg402")
        IF handler IS NOT NULL THEN
            START REPORT artg402_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                        ," ORDER BY lower(ima01)"   #FUN-B80161 add
          
            DECLARE artg402_datacur1 CURSOR FROM l_sql
            FOREACH artg402_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg402_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg402_rep
        END IF
...
REPORT artg402_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE sr1_o sr1_t    #FUN-B80161 add
    DEFINE l_display LIKE type_file.chr1   #FUN-B80161 add
    DEFINE l_display1 LIKE type_file.chr1   #FUN-B80161 add
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B80161 FROM FUN-BB0032---------add------str-----
    ...
    #FUN-B80161 FROM FUN-BB0032---------add------end-----
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
            LET sr1_o.ima01 = NULL #FUN-B80161
            LET sr1_o.ima02 = NULL #FUN-B80161
        ...
        ON EVERY ROW
            ...
            IF NOT cl_null(sr1_o.ima01) THEN
               IF sr1_o.ima01 != sr1.ima01 OR tm.w='Y'THEN
                  LET l_display = 'Y'
            ...
            IF NOT cl_null(sr1_o.ima02) THEN
               IF sr1_o.ima02 != sr1.ima02 OR tm.w='Y'THEN
                  LET l_display1 = 'Y'
