...
#                  p_no       單據編號  
#                  p_no1      提案單號
#                  p_cmd      用戶動作 a:新增 b:修改 d:刪除 e:結束錄入  
#                  p_count    數量                                                                                                  
  ...
  #FUN-9C0068    ---end   
   ...
   IF p_cmd = 'd' THEN
      ...
      IF SQLCA.SQLCODE THEN
         ...
         RETURN 0,0               #FUN-AA0065
      END IF
      RETURN 0,0  #No.FUN-A10016  #FUN-AA0065
   END IF
   LET g_term = p_term
   LET g_cmd = p_cmd
   ...
   FOREACH ohi1_cur INTO l_ohi02
      ...
      FOREACH cur_ohi03 INTO l_ohi03
                 ...
                 #MOD-9C0173   ---start
                  WHEN l_ohi04 = 'A3' 
                     CALL s_fetch_price_A3(p_item,p_no,p_plant,p_type)
                        RETURNING l_price
                  WHEN l_ohi04 = 'A4' 
                     CALL s_fetch_price_A4(p_unit,p_plant,p_line,p_no,p_item,p_cust,p_term,p_curr)
                        RETURNING l_price
                  WHEN l_ohi04 = 'A5'
        ...
        #IF l_ohi04 = 'A1' OR l_ohi04 = 'A2' THEN #MOD-9C0173
         IF l_ohi04 = 'A1' OR l_ohi04 = 'A2' OR l_ohi04 = 'A3' OR l_ohi04 = 'A4' OR l_ohi04 = 'A5' OR l_ohi04 = 'A6' THEN #MOD-9C0173 
...
#FUN-AA0065-----add-- begin--   
   ...
   IF l_n1 >0 THEN 
      ...
      CASE p_type
        WHEN '1'
           LET l_sql = "SELECT no,line,item,price,qty,qty2,oeb917 FROM s_price_c2,   ",
                       cl_get_target_table(g_plant_new,'oeb_file'),
                       "   WHERE oeb01 = '",p_no,"' ",
                       "   AND oeb01 = no AND line = oeb03 "
        WHEN '2'
           LET l_sql = "SELECT no,line,item,price,qty,qty2,ogb917 FROM s_price_c2,   ",
                       cl_get_target_table(g_plant_new,'ogb_file'),
                       "   WHERE ogb01 = '",p_no,"' ",
                       "   AND ogb01 = no AND line = ogb03 "
        WHEN '3'
           LET l_sql = "SELECT no,line,item,price,qty,qty2,ohb917 FROM s_price_c2,   ",
      ...
      FOREACH cur_scale_1 INTO l_tmp[l_cnt].* ,g_oeb[l_cnt].oeb917
         ...
         IF l_tmp[l_cnt].qty > l_tmp[l_cnt].qty2 THEN
              CASE p_type
                 WHEN '1'
                    ...
                    ELSE
                       ...
                       CALL cl_digcut(d_oeb[l_cnt].oeb14,t_azi04) RETURNING d_oeb[l_cnt].oeb14     
                    END IF  
                       CALL cl_digcut(l_price_1[l_cnt],t_azi03)RETURNING l_price_1[l_cnt]      
...
DEFINE l_rth04      LIKE rth_file.rth04
  ...
  #FUN-9C0068   ---END
   ...
   IF cl_null(l_rtz05) THEN RETURN 0 END IF
   SELECT rtg05,rtg08 INTO l_rtg05,l_rtg08 FROM rtg_file
   ...
   IF l_rtg08 = 'Y' THEN
      SELECT rth04 INTO l_rth04 FROM rth_file WHERE rth01 = p_item AND rth02 = p_unit
          AND rthacti = 'Y'
...
FUNCTION s_fetch_price_A4(p_unit,p_plant,p_line,p_no,p_item,p_cust,p_term,p_curr)
DEFINE l_price  LIKE oeb_file.oeb13
...
DEFINE l_oea87      LIKE oea_file.oea87
  ...
  #FUN-9C0068   ---END
   ...
   LET l_rxc.rxc04 = ' '
   LET l_rxc.rxc05 = ''
   LET l_rxc.rxc06 = p_price*(100-l_xmh03*l_occ32/100)/100
   LET l_rxc.rxc07 = 0
   LET l_rxc.rxc08 = ''
   LET l_rxc.rxc09 = '0'
   LET l_rxc.rxc10 = ''
...
#參數設定需要重新審核，以當前機構活動代碼為准;否則以拋轉資料為主
   ...
   CALL s_fetch_price_ins_temp_table(p_type,p_cust,p_no,p_line,p_count,p_plant,p_price,p_item,p_unit,p_cmd)
   CASE p_type
        WHEN '1'
           LET l_sql = "DELETE FROM ",cl_get_target_table(g_plant_new,'rxc_file'), 
                       " WHERE rxc03 IN ('01','02','03') ",
                       " AND   rxc00 = '01' AND rxc01 = '",p_no,"' "
       WHEN '2'                
           LET l_sql = "DELETE FROM ",cl_get_target_table(g_plant_new,'rxc_file'), 
                       " WHERE rxc03 IN ('02','02','03') ",
                       " AND   rxc00 = '01' AND rxc01 = '",p_no,"' "
       WHEN '3'                 
           LET l_sql = "DELETE FROM ",cl_get_target_table(g_plant_new,'rxc_file'), 
                       " WHERE rxc03 IN ('01','02','03') ",
...
DEFINE l_tmp         RECORD
   ...
   LET l_time = TIME
   LET l_sql1 = "SELECT DISTINCT rac_file.* ",
...
#更新本次消耗数量为0
   ...
   FOREACH s_fetch_price_c2_curs3 INTO l_tmp.*               
      ...
      IF NOT s_fetch_price_chk('1',l_rac.rac01,l_rac.rac02,l_rac.rac03,g_plant_new,l_tmp.item) THEN
         CONTINUE FOREACH
      END IF
      UPDATE　 s_price_c2 SET qty1 = l_tmp.qty2,qty2 = 0  WHERE item = l_tmp.item AND line = l_tmp.line   #本次消耗数量，剩余数量
      CASE l_rac.rac04	
         WHEN '1'
...
#计算rxc_file相关值，INSERT INTO rxc_file
       ...
       IF p_type = '2' THEN LET l_rxc.rxc00 = '02' END IF
       IF p_type = '3' THEN LET l_rxc.rxc00 = '03' END IF
       LET l_sql = "SELECT rtu01,rtv02,rtv13,rtu05 FROM ",cl_get_target_table(g_plant_new,'rtu_file'),",", 
       ...
       EXECUTE pre_sel3_rtv13 INTO l_rtu01,l_rtv02,l_rtv13,l_rtu05 
...
DEFINE l_sum_qty2    LIKE type_file.num5 
DEFINE l_rap06       LIKE rap_file.rap06
DEFINE l_rap07       LIKE rap_file.rap07
DEFINE l_rap08       LIKE rap_file.rap08 
DEFINE l_price       LIKE oeb_file.oeb13
...
#更新本次消耗数量为0
    ...
    END IF
    LET l_time = TIME
    LET l_sql1 = "SELECT DISTINCT rae_file.* ",
    ...
    FOREACH s_sel_rae_cs INTO l_rae.*
        ...
        IF l_rae.rae21 = 0 OR l_rae.rae21 = l_raf05_1 THEN	
           LET l_zu = l_zu1	
        ELSE
        ...
        LET g_sql = "SELECT * FROM s_price_c2 WHERE qty2 > 0 AND raf03 IS NOT NULL "	  
    ...
    #           WHERE raf03= l_zu
            ...
            IF l_raf04 = '1' THEN  
               ...
               IF l_tmp1.qty2 >= l_zuzo THEN 
                  LET l_tmp1.qty1 = l_zuzo                #該項次料件本次消耗數量           
               ELSE
        ...
        FOREACH s_fetch_price_c2_curs6 INTO l_tmp[l_cnt].*   #by 每筆單身算本次消耗數量
           ...
           EXECUTE pre_sel4_rtv13 INTO l_rtu01,l_rtv02,l_rtv13,l_rtu05 
           IF cl_null(l_rtv13) OR l_rtv13 = 0 THEN
               LET l_sql = "SELECT A.rto01,A.rto03,A.rto05,A.rto12 FROM ",cl_get_target_table(g_plant_new,'rto_file A'),
                           "   WHERE A.rto05 = '",l_rtu05,"'",
                           "     AND '",p_date,"' BETWEEN A.rto08 AND A.rto09 ",
           ...
           END IF
           LET l_cnt = l_cnt+1
           UPDATE s_price_c2 SET qty1 = 0  WHERE item = l_tmp[l_cnt].item AND line=l_tmp[l_cnt].line
        END FOREACH
    END FOREACH
#   RETURN l_price
END FUNCTION
...
#剩余数量>0，才可以走下面的逻辑
    ...
    IF l_cnt = 0 THEN RETURN END IF
    LET l_time = TIME
    LET l_sql1 = "SELECT DISTINCT rah_file.* ",
...
#按照会员等级设置	
  ...
  #           LET i = i + 1
       ...
       LET l_sql = "SELECT count(*) ",
                   ...
                   " WHERE rai01 = '",l_rah.rah01,"' AND rai02 = '",l_rah.rah02,"'",
                   "   AND raiplant = '",g_plant_new,"'",
                   "   AND rai04 <= '",l_amt,"'  order by rai04 "
       CALL cl_replace_sqldb(l_sql) RETURNING l_sql              							
       CALL cl_parse_qry_sql(l_sql,g_plant_new) RETURNING l_sql          
       ...
       FOREACH s_fetch_price_c2_curs5 INTO l_fg,l_line,l_item,l_price,l_qty1,l_qty2,l_flg1,l_type,l_cx_no
          ...
          IF  l_rah.rah12 = 'Y' THEN
            ...
            ELSE  
               ...
               EXECUTE pre_sel5_rtv13 INTO l_rtu01,l_rtv02,l_rtv13,l_rtu05 
               IF cl_null(l_rtv13) OR l_rtv13 = 0 THEN
                  LET l_sql = "SELECT A.rto01,A.rto03,A.rto05,A.rto12 FROM ",cl_get_target_table(g_plant_new,'rto_file A'),
                              "   WHERE A.rto05 = '",l_rtu05,"'",
                              "     AND '",p_date,"' BETWEEN A.rto08 AND A.rto09 ",
     ...
     IF NOT cl_null(l_oea87)  THEN
        ...
        FOREACH s_fetch_price_c2_curs7 INTO l_no,l_line,l_cx_no,l_item,l_price,l_qty,l_qty2,l_flg1,l_type
           IF l_qty2 >0 AND l_flg1 = 'Y' THEN
              ...
              UPDATE rxc_file SET rxc06 = rxc06 + l_rxc06 
              WHERE rxc01 = l_no AND rxc02 = l_line 
           END IF
       END FOREACH 
    END IF  
 #   RETURN l_price
...
FUNCTION s_fetch_price_ins_temp_table(p_type,p_cust,p_no,p_line,p_count,p_plant,p_price,p_item,p_unit,p_cmd)
DEFINE p_type       LIKE type_file.chr1
DEFINE l_sql        STRING
DEFINE p_cmd        LIKE type_file.chr1
DEFINE p_cust       LIKE occ_file.occ01
...
FUNCTION s_base_price(p_cust,p_item,p_unit,p_date,p_type,p_plant,p_curr,p_team,p_term,p_line,p_no) #MOD-9C0173
DEFINE p_cust       LIKE occ_file.occ01
DEFINE p_item       LIKE ima_file.ima01
DEFINE p_unit       LIKE ima_file.ima25
DEFINE p_date       LIKE type_file.dat
...
FUNCTION s_dynamic(p_cust,p_item,p_unit,p_date,p_curr,p_no,p_type,p_plant,p_rwf,p_team,p_price,p_term,p_line) #MOD-9C0173
DEFINE p_team       LIKE ohi_file.ohi02
DEFINE p_cust       LIKE occ_file.occ01
DEFINE p_item       LIKE ima_file.ima01
DEFINE p_unit       LIKE ima_file.ima25
   ...
   IF p_rwf.rwf10 = '1' THEN   
      ...
      FOREACH cur_tmp8 INTO l_tmp.*
         #CALL s_base_price(p_cust,l_tmp.oeb04,l_tmp.oeb05,p_date,p_type,p_plant,p_curr,p_team) #MOD-9C0173
          CALL s_base_price(p_cust,l_tmp.oeb04,l_tmp.oeb05,p_date,p_type,p_plant,p_curr,p_team,p_term,p_line,p_no) #MOD-9C0173 
      ...
      FOREACH cur_ohb1 INTO l_oeb03,l_oeb04,l_oeb05,l_oeb12
         ...
         IF l_use_sum > 0 THEN
           #CALL s_base_price(p_cust,l_oeb04,l_oeb05,p_date,p_type,p_plant,p_curr,p_team) #MOD-9C0173
            CALL s_base_price(p_cust,l_oeb04,l_oeb05,p_date,p_type,p_plant,p_curr,p_team,p_term,p_line,p_no) #MOD-9C0173 
...
DEFINE l_oea87      LIKE oea_file.oea87
  ...
  #FUN-9C0068   ---END
   ...
   LET l_sql = "SELECT xmc08,xmc09 FROM ",cl_get_target_table(g_plant_new,'xmb_file'),",", #FUN-A50102
                                          cl_get_target_table(g_plant_new,'xmc_file'),     #FUN-A50102
               "   WHERE xmb01 = xmc01 AND xmb02 = xmc02 ",
               "     AND xmb03 = xmc03 AND xmb04 = xmc04 ",
