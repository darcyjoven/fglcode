...
#
# Pattern name...: artg492.4gl
# Descriptions...: 券種收銀收款統計表
...
MAIN
   ...
   LET g_sql = "azw01.azw_file.azw01,",
               ...
               "lpx10.lpx_file.lpx10"
                         
 
   LET l_table = cl_prt_temptable('artg492',g_sql) CLIPPED
   IF  l_table = -1 THEN
       EXIT PROGRAM
   END IF
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artg492_tm(0,0)        # Input print condition
      ELSE CALL artg492()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
   CALL cl_gre_drop_temptable(l_table)     #FUN-BA0062
END MAIN

FUNCTION artg492_tm(p_row,p_col)
   DEFINE lc_qbe_sn      LIKE gbm_file.gbm01
   DEFINE p_row,p_col    LIKE type_file.num5,
          l_cmd          LIKE type_file.chr1000 
   LET p_row = 6 LET p_col = 16
 
   OPEN WINDOW artg492_w AT p_row,p_col WITH FORM "art/42f/artg492" 
...
#FUN-C80043 add end ---
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artg492_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)     #FUN-BA0062
         EXIT PROGRAM
      END IF
...
#        CALL cl_err('','9046',0) CONTINUE WHILE          #FUN-C80043 MARK
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('artg492',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artg492_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)     #FUN-BA0062
         EXIT PROGRAM
      END IF
   ...
   DEFINE l_name    LIKE type_file.chr20,         # External(Disk) file name        
         #l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT   #FUN-D30098 Mark
          ...
          sr        RECORD 
                    ...
                    GIFTCTF    LIKE lpx_file.lpx01,
                    lpx02      LIKE lpx_file.lpx02,
                    PAY        LIKE lpx_file.lpx10
                    END RECORD
...
#TQC-B60017 ADD - BEGIN ---------------------------------------------------
    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'artg492'
...
#FUN-C80043 add begin ---
         #LET l_sql = " SELECT '','',CTTYPE,'',SUM(PAY-CHANGED)",         #FUN-D30098 Mark
          LET l_sql = " SELECT '','',CTTYPE,'',SUM(CASE Type WHEN 0 THEN PAY-CHANGED WHEN 3 THEN PAY-CHANGED ELSE (PAY-CHANGED)*(-1) END)", #FUN-D30098 Add 
                      ...
                      "   AND ryd01 = '04' AND td_Sale_Pay.CNFFLG='Y'",
...
#FUN-C80043 MARK END ----
    END CASE
    PREPARE artg492_prepare1 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       ...
       EXIT PROGRAM
    END IF
    DECLARE artg492_curs1 CURSOR FOR artg492_prepare1

    FOREACH artg492_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
     ...
     #TQC-B60017 MARK&ADD   END------------------------------------------
      ...
      SELECT lpx02 INTO l_lpx02 FROM lpx_file WHERE lpx01 = sr.GIFTCTF  
     ...
     #sr.SHOP,l_azw08,sr.OPNO,l_gen02,
     #sr.GIFTCTF,l_lpx02,sr.PAY

      EXECUTE  insert_prep  USING
      sr.SHOP,l_azp02,sr.OPNO,l_gen02,
      sr.GIFTCTF,l_lpx02,sr.PAY
     #TQC-B60017 MARK&ADD   END------------------------------------------
      LET l_gen02 = ''
      LET l_ryi02 = '' #TQC-B50083 add
      LET l_lpx02 = '' #TQC-B60017 ADD
...
#TQC-B60017 ADD - BEGIN ---------------------------------------------------
   IF g_zz05 = 'Y' THEN
     ...
     #CALL cl_wcchp(tm.wc,'azp01')        #FUN-C80043 mark
      CALL cl_wcchp(tm.wc,'azp01,ryi01')  #FUN-C80043 add
           RETURNING tm.wc
...
#  LET g_str = tm.wc   #TQC-B60017 MARK
   CASE
      WHEN tm.a = '1'
###GENGRE###         CALL cl_prt_cs3('artg492','artg492_1',l_sql,g_str) 
    LET g_template = 'artg492_1'
    CALL artg492_grdata()    ###GENGRE###
      WHEN tm.a = '2'
###GENGRE###         CALL cl_prt_cs3('artg492','artg492_2',l_sql,g_str)
    LET g_template = 'artg492_2'
    CALL artg492_grdata()    ###GENGRE###
      WHEN tm.a = '3'
###GENGRE###         CALL cl_prt_cs3('artg492','artg492_3',l_sql,g_str)
    LET g_template = 'artg492_3'
    CALL artg492_grdata()    ###GENGRE###
   END CASE
END FUNCTION
...
FUNCTION artg492_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg492")
        IF handler IS NOT NULL THEN
            START REPORT artg492_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
           #FUN-C80043 By shi Begin---
            CASE
               WHEN tm.a = '1'
                  LET l_sql = l_sql CLIPPED," ORDER BY lpx01 "
               WHEN tm.a = '2'
                  LET l_sql = l_sql CLIPPED," ORDER BY lpx01,azw01 "
               WHEN tm.a = '3'
                  LET l_sql = l_sql CLIPPED," ORDER BY lpx01,gen01 "
            END CASE
           #FUN-C80043 By shi End-----
          
            DECLARE artg492_datacur1 CURSOR FROM l_sql
            FOREACH artg492_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg492_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg492_rep
        END IF
...
REPORT artg492_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_lpx10_tot   LIKE type_file.num20_6  #FUN-D30098 Add
    ...
    FORMAT
        ...
        ON LAST ROW
            LET l_lpx10_tot = SUM(sr1.lpx10)     #FUN-D30098 Add
            PRINTX l_lpx10_tot                   #FUN-D30098 Add
