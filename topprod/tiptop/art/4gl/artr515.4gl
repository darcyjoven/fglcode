...
MAIN
   ...
   LET g_sql="azw01.azw_file.azw01,",
             ...
             "ave.img_file.img20,",
             "rate.img_file.img21"
   LET l_table = cl_prt_temptable('artr515',g_sql) CLIPPED
...
DEFINE tok            base.StringTokenizer        
   ...
   END IF
   OPEN WINDOW r515_w AT p_row,p_col
        WITH FORM "art/42f/artr515"
      ...
      CONSTRUCT BY NAME tm.wc1 ON azp01 
         ...
         AFTER FIELD azp01            
            ...
            IF NOT cl_null(l_azp01_str) AND l_azp01_str <> "*" THEN
               ...
               END WHILE
            ELSE
               DECLARE r515_zxy_cs  CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH r515_zxy_cs  INTO l_zxy03
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,
                         ...
                         " '",g_template CLIPPED,"'",           
                         " '",g_rpt_name CLIPPED,"'"            
         CALL cl_cmdat('artr515',g_time,l_cmd)
      END IF
      CLOSE WINDOW r515_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM
...
FUNCTION r515()
   DEFINE l_plant   LIKE azp_file.azp01,
     ...
     DROP TABLE artr515_tmp
     CREATE TEMP TABLE artr515_tmp(
...
#                   " UNION SELECT oha01,oha02,ohb03,ohb09,ohb092,(CASE WHEN ohb12 IS NULL THEN 0 ELSE ohb12 END)*(-1),'' ",   #TQC-B70204 mark
        ...
        PREPARE r515_prepare2 FROM l_sql
        IF SQLCA.sqlcode THEN CALL cl_err('prepare2:',SQLCA.sqlcode,1) 
           CALL cl_used(g_prog,g_time,2) RETURNING g_time 
           EXIT PROGRAM 
        END IF
        DECLARE r515_curs2 CURSOR FOR r515_prepare2          
        FOREACH r515_curs1 INTO sr.ima01,sr.ima02,sr.ima131,sr.oba02               # 营运中心的产品信息
        ...
        LET sr.azp02 = l_azp02
         ...
         #--计算起始日期的库存 
           ...
           CALL r515_avg(g_yy1,g_mm1,sr.azp01,sr.ima01,tm.bdate) RETURNING sr.imk09  
         #--计算截止日期的库存 
           CALL s_yp(tm.edate) RETURNING g_yy1,g_mm1
           CALL r515_avg(g_yy1,g_mm1,sr.azp01,sr.ima01,tm.edate) RETURNING sr.img10 
         #--
          LET sr.ave = (sr.imk09 + sr.img10)/2    # --每笔资料的平均库存
          LET sr.ccc23 = 0
          LET sr.rate = 0
         #--计算销售资料    
          FOREACH r515_curs2 USING sr.ima01,sr.azp01,sr.ima01,sr.azp01   
                              INTO sr1.* 

              #计算单位销售成本 
              CALL r515_cost(sr.azp01,sr.ima01,tm.type,sr1.*)  RETURNING l_cost
              LET sr.ccc23 = sr.ccc23 + l_cost 
              INITIALIZE sr1 TO NULL
          END FOREACH           
   
          INSERT INTO artr515_tmp VALUES(sr.*) 
  ...
  END FOREACH       
     ...
     CASE
      ...
      END CASE
      PREPARE r515_prepare3 FROM l_sql
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('prepare3:',SQLCA.sqlcode,1) 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      DECLARE r515_curs3 CURSOR FOR r515_prepare3

      FOREACH r515_curs3 INTO sr.* 
   ...
   CASE
      WHEN tm.t='1'
         CALL cl_prt_cs3('artr515','artr515',l_sql,g_str)  
      WHEN tm.t='2'
        CALL cl_prt_cs3('artr515','artr515_1',l_sql,g_str)
      WHEN tm.t='3'
        CALL cl_prt_cs3('artr515','artr515_2',l_sql,g_str)         
   END CASE
END FUNCTION  
FUNCTION r515_avg(l_yy_t,l_mm_t,l_azp01,l_ima01,l_date)
   DEFINE l_azp01  LIKE azp_file.azp01
