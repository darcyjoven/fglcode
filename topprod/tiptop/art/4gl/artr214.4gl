...
MAIN
   ...
   LET g_sql=  "order1.type_file.num10,",
    ...
    LET l_table = cl_prt_temptable('artr214',g_sql) CLIPPED
    ...
    IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN 
       CALL artr214_tm()        # Input print condition
    ELSE 
       CALL artr214() 
    END IF
...
DEFINE l_sql          STRING
   ...
   OPEN WINDOW artr214_w AT p_row,p_col WITH FORM "art/42f/artr214" 
   ...
   WHILE TRUE
      CONSTRUCT BY NAME tm.wc ON ruw02,ruw04,ruwplant,rtz10,ima131,rty05
        BEFORE CONSTRUCT
         ...
         AFTER CONSTRUCT
            IF INT_FLAG THEN
               LET INT_FLAG = 0 CLOSE WINDOW artr214_w 
               CALL cl_used(g_prog,g_time,2) RETURNING g_time 
               EXIT PROGRAM
            END IF  
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd1 CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artr214',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artr214_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
DEFINE l_cost    LIKE type_file.num10
    ...
    DROP TABLE artr214_tmp
    CREATE TEMP TABLE artr214_tmp(
...
#       LET l_sql = l_sql,"   UNION SELECT rtz10,ruwplant,ruw05,rux03,ima02,ima25,ima131,rux01,rux02,rux04,rux05,rux06,rux07,rux08 FROM ",  #TQC-B70204 mark
      ...
      PREPARE artr214_prepare1 FROM l_sql                  
      IF SQLCA.sqlcode != 0 THEN                           
         CALL cl_err('prepare:',SQLCA.sqlcode,1)           
         CALL cl_used(g_prog,g_time,2) RETURNING g_time    
         EXIT PROGRAM                                      
      END IF                                               
      DECLARE artr214_curs1 CURSOR FOR artr214_prepare1 
      FOREACH artr214_curs1 INTO sr.*  
         IF SQLCA.sqlcode != 0 THEN
         ...
         INSERT INTO artr214_tmp VALUES(0,sr.rtz10,l_ryf02,sr.ruwplant,l_azw08,sr.rux03,sr.ima02,sr.ima131,l_oba02,sr.rux01,sr.rux02,
   ...
   IF tm.cb2='Y' THEN
      IF tm.type1='1' THEN
         LET l_sql = "SELECT ima131,rux04,SUM(income) FROM artr214_tmp "
      END IF
      IF tm.type1='2' THEN
         LET l_sql = "SELECT ima131,rux04,SUM(cost) FROM artr214_tmp "
      END IF
      IF tm.type1='3' THEN
         LET l_sql = "SELECT ima131,rux04,SUM(rux08) FROM artr214_tmp "
      END IF
      IF tm.cb1 = 'Y' THEN
      ...
      IF tm.type1 = '1' OR tm.type1 = '2' OR tm.type1='3' THEN
         ...
         FOREACH r214_sum_curs1 INTO l_ima131,l_rux04,l_order1
            IF l_ima131 IS NOT NULL AND l_rux04 IS NOT NULL THEN
              UPDATE artr214_tmp SET order1 = l_order1
   ...
   DECLARE r214_curs CURSOR FOR SELECT * FROM artr214_tmp
   ...
   LET g_str = g_str,";",tm.type1,";",tm.cb1,';',tm.cb2,";",tm.cb3,";"
    CASE
      WHEN tm.cb2='N'
         CALL cl_prt_cs3('artr214','artr214_1',l_sql,g_str)  
      WHEN tm.cb2='Y'
         CALL cl_prt_cs3('artr214','artr214_2',l_sql,g_str) 
    END CASE
