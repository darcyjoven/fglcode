...
# Pattern name...: artg301.4gl
# Descriptions...: 產品分類統計表       
# Date & Author..: #FUN-A70005 10/07/12 By lixh1 
# Modify.........: No.FUN-BA0063 11/10/19 By yangtt CR轉換成GRW
# Modify.........: No.FUN-BA0063 12/01/13 By yangtt 追單 TQC-BB0034
...
MAIN
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      THEN CALL artg301_tm(0,0)            # Input print condition
      ELSE CALL artg301()               # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
   CALL cl_gre_drop_temptable(l_table)               #FUN-BA0063 add
END MAIN 

FUNCTION artg301_tm(p_row,p_col)
   ...
   OPEN WINDOW artg301_w AT p_row,p_col WITH FORM "art/42f/artg301"
   ...
   LET g_copies = '1' 
   #genero版本default 排序
#  LET tm2.s1   = tm.s[1,1]     #FUN-BA0036 FROM FUN-BA0063 FROM TQC-BB0034 mark
#  LET tm2.s2   = tm.s[2,2]     #FUN-BA0036 FROM FUN-BA0063 FROM TQC-BB0034 mark
#  LET tm2.s3   = tm.s[3,3]     #FUN-BA0036 FROM FUN-BA0063 FROM TQC-BB0034 mark
   CALL r301_set_entry()       #FUN-BA0063 FROM TQC-BB0034
   CALL r301_setS() RETURNING tm2.s1,tm2.s2,tm2.s3  #FUN-BA0063 FROM TQC-BB0034
...
WHILE TRUE
   ...
   CONSTRUCT BY NAME tm.wc1 ON azp01 
      ...
      AFTER FIELD azp01            
         ...
         IF NOT cl_null(l_azp01_str) AND l_azp01_str <> "*" THEN
            ...
            END WHILE
         ELSE
            DECLARE g301_zxy_cs  CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
            FOREACH g301_zxy_cs  INTO l_zxy03
               IF g_chk_auth IS NULL THEN
                  LET g_chk_auth = "'",l_zxy03,"'"
               ELSE
                  LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
   ...
   END IF 
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW artg301_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)               #FUN-BA0063 add
      EXIT PROGRAM
   END IF 
...
#  INPUT BY NAME tm2.s1,tm2.s2,tm2.s3,tm.t,tm.more  WITHOUT DEFAULTS
   INPUT BY NAME tm.w,tm.x,tm.t,tm2.s1,tm2.s2,tm2.s3,tm.more  WITHOUT DEFAULTS   #FUN-BA0063 FROM TQC-BB0034
      ...
      #FUN-BA0063 FROM FUN-BA0063 FROM TQC-BB0034 add-----str----
      ON CHANGE t                 
         CALL r301_set_entry()     
         CALL r301_setS() RETURNING tm2.s1,tm2.s2,tm2.s3 
         DISPLAY tm2.s1,tm2.s2,tm2.s3 TO s1,s2,s3 
      #FUN-BA0063 FROM FUN-BA0063 FROM TQC-BB0034 add-----end----
      AFTER FIELD t
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,  #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_template CLIPPED,"'",           
                         " '",g_rpt_name CLIPPED,"'"            
         CALL cl_cmdat('artg301',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW artg301_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)               #FUN-BA0063 add
      EXIT PROGRAM
   END IF
...
FUNCTION artg301()
   ...
   DROP TABLE artg301_tmp
   CREATE TEMP TABLE artg301_tmp(
   ...
   FOREACH sel_azp01_cs INTO l_plant,l_azp02                                 ##--最外围的FOREACH--
     ...
     END IF
     DECLARE artg301_curs1 CURSOR FOR artg301_p1
     ...
     FOREACH artg301_curs1 INTO sr.*
        ...
        INSERT INTO artg301_tmp VALUES(sr.azp01,sr.azp02,sr.rvv39t,sr.ima131,sr.oba02,sr.rvv31,
  ...
  END FOREACH
     CASE 
        WHEN tm.t='1'
           #LET l_sql = "SELECT '','',SUM(rvv39t),ima131,oba02,'','','','','','' ",
           LET l_sql = "SELECT '','',SUM(rvv39t),ima131,oba02,'','',SUM(rvv17),'','','' ",
                       "  FROM artg301_tmp ",
                       " GROUP BY ima131,oba02 "
        WHEN tm.t='2' 
           #LET l_sql = "SELECT azp01,azp02,SUM(rvv39t),ima131,oba02,'','','','','','' ",
           LET l_sql = "SELECT azp01,azp02,SUM(rvv39t),ima131,oba02,'','',SUM(rvv17),'','','' ",
                       "  FROM artg301_tmp ",
                       " GROUP BY azp01,azp02,ima131,oba02 "
   END CASE
      PREPARE g301_prepare3 FROM l_sql
      IF SQLCA.sqlcode != 0 THEN
         ...
         EXIT PROGRAM
      END IF
      DECLARE g301_curs3 CURSOR FOR g301_prepare3  
      FOREACH g301_curs3 INTO sr.azp01,sr.azp02,sr.rvv39t,sr.ima131,sr.oba02,sr.rvv31,
                              sr.ima02,sr.rvv17,sr.rvu04,sr.rvu05,sr.rvu03 
...
###GENGRE###     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     ...
     LET tm.wc = tm.wc1," AND ",tm.wc      
        ...
        LET g_str = tm.wc
     END IF
###GENGRE###     LET g_str = g_str,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",tm.t,";",tm.w,";",tm.x   #FUN-BA0063 FROM FUN-BA0063 FROM TQC-BB0034 add tm.w,tm.x
   CASE
      WHEN tm.t='1'
###GENGRE###         CALL cl_prt_cs3('artg301','artg301',l_sql,g_str)  
         LET g_template = 'artg301'    #FUN-BA0063 add
    CALL artg301_grdata()    ###GENGRE###
      WHEN tm.t='2'
###GENGRE###         CALL cl_prt_cs3('artg301','artg301_1',l_sql,g_str) 
         LET g_template = 'artg301_1'    #FUN-BA0063 add
    CALL artg301_grdata()    ###GENGRE###
   END CASE
END FUNCTION       

#FUN-BA0063 FROM TQC-BB0034 add begin-----------
FUNCTION r301_set_entry()
   CASE
      WHEN tm.t='1'
...
#FUN-BA0063-------add-----str----
PRIVATE FUNCTION artg301_replace_ord_desc(p_desc)
...
FUNCTION artg301_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg301")
        IF handler IS NOT NULL THEN
            START REPORT artg301_rep TO XML HANDLER handler
            ...
            DECLARE artg301_datacur1 CURSOR FROM l_sql
            FOREACH artg301_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg301_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg301_rep
        END IF
...
REPORT artg301_rep(sr1)
    ...
    ORDER EXTERNAL BY sr1.ima131,sr1.order1,sr1.order2,sr1.order3
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            #FUN-BA0063-------add---str-----
            LET g_ord1_desc = artg301_replace_ord_desc(g_ord1_desc)
            LET g_ord2_desc = artg301_replace_ord_desc(g_ord2_desc)
            LET g_ord3_desc = artg301_replace_ord_desc(g_ord3_desc)
        ...
        ON EVERY ROW
            ...
            #  IF sr1_o.ima131 = sr1.ima131 THEN     #FUN-CB0058 mark
               IF sr1_o.ima131 = sr1.ima131 AND tm.w = 'N' THEN  #FUN-CB0058 add
            #     LET l_display = 'N'   #FUN-CB0058 mark
                  LET l_ima131 = "  "   #FUN-CB0058 add
               ELSE
            ...
            END IF
            PRINTX l_display
            PRINTX l_ima131    #FUN-CB0058 add

            IF NOT cl_null(sr1_o.oba02) THEN
            #  IF sr1_o.oba02 = sr1.oba02 THEN   #FUN-CB0058 mark
               IF sr1_o.oba02 = sr1.oba02 AND tm.w = 'N' THEN   #FUN-CB0058 add
            #     LET l_display1 = 'N'   #FUN-CB0058 mark
                  LET l_oba02 = "  "     #FUN-CB0058 add
               ELSE
            ...
            END IF
            PRINTX l_display2
            PRINTX l_rvuplant   #FUN-CB0058 add
