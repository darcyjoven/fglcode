...
# Modify.........: No.FUN-C30298 12/03/28 By Pauline 請將artg144加上zxy的控管，與artr450一致
# Modify.........: No.FUN-C40071 12/04/11 By lixiang CR 转GR
...
MAIN
   ...
   LET l_table = cl_prt_temptable('artg144',g_sql) CLIPPED
...
#FUN-C30298 add END       
   ...
   OPEN WINDOW g144_w AT p_row,p_col WITH FORM "art/42f/artg144"
   ...
   LET g_ptime = g_time    #FUN-C40071 add
   WHILE TRUE
   ...
   DEFINE sr1         RECORD
     ...
     LET l_sql = "SELECT DISTINCT azw01,azw08,rtz10 FROM azw_file,rtz_file ",
               ...
               #" AND azw01 IN ",g_auth,  #FUN-C30298 mark
                ...
                " ORDER BY azw01 "
     PREPARE g144_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE g144_cs1 CURSOR FOR g144_prepare1
     FOREACH g144_cs1 INTO sr1.*
...
###GENGRE###               " ORDER BY operating,oga02,ogb01,ogb03"
     LET g_str = ''
  ...
  #FUN-BC0026 add START
     ...
     IF g_zz05 = 'Y' THEN
        ...
        END IF
     END IF
###GENGRE###     LET g_str = g_str,";",l_azi04,";"
  #FUN-BC0026 add END
###GENGRE###     CALL cl_prt_cs3('artg144','artg144',l_sql,g_str)
    CALL artg144_grdata()    ###GENGRE###
END FUNCTION
...
FUNCTION artg144_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg144")
        IF handler IS NOT NULL THEN
            START REPORT artg144_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY operating,oga02  asc nulls first "    #FUN-C40071 add
               
            DECLARE artg144_datacur1 CURSOR FROM l_sql
            FOREACH artg144_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg144_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg144_rep
        END IF
...
REPORT artg144_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_ogb12_subtot  LIKE ogb_file.ogb12
    DEFINE l_ogb47_subtot  LIKE ogb_file.ogb47
    DEFINE l_ogb14_subtot  LIKE ogb_file.ogb14
    DEFINE l_ogb14t_subtot LIKE ogb_file.ogb14t
    DEFINE l_fmt_ogb37         STRING            #FUN-C40071 add
    DEFINE l_fmt_ogb47         STRING            #FUN-C40071 add
    DEFINE l_fmt_ogb14         STRING            #FUN-C40071 add
    DEFINE l_fmt_ogb14t        STRING            #FUN-C40071 add
    DEFINE l_fmt_ogb47_total   STRING            #FUN-C40071 add
    DEFINE l_fmt_ogb14_total   STRING            #FUN-C40071 add
    DEFINE l_fmt_ogb14t_total  STRING            #FUN-C40071 add
    DEFINE l_azi04          LIKE azi_file.azi04  #FUN-C40071 add
    ORDER EXTERNAL BY sr1.operating,sr1.oga02
    ...
    FORMAT
        ...
        ON EVERY ROW
         ...
         #FUN-C40071--add--begin---
            SELECT azi04 INTO l_azi04 FROM azi_file WHERE azi01 = g_aza.aza17
            LET l_fmt_ogb37 = cl_gr_numfmt('ogb_file','ogb37',l_azi04)
            PRINTX l_fmt_ogb37
            LET l_fmt_ogb47 = cl_gr_numfmt('ogb_file','ogb47',l_azi04)
            PRINTX l_fmt_ogb47
            LET l_fmt_ogb14 = cl_gr_numfmt('ogb_file','ogb14',l_azi04)
            PRINTX l_fmt_ogb14
            LET l_fmt_ogb14t = cl_gr_numfmt('ogb_file','ogb14t',l_azi04)
            PRINTX l_fmt_ogb14t 
            LET l_fmt_ogb47_total = cl_gr_numfmt('ogb_file','ogb47',l_azi04)
            PRINTX l_fmt_ogb47_total
            LET l_fmt_ogb14_total = cl_gr_numfmt('ogb_file','ogb14',l_azi04)
            PRINTX l_fmt_ogb14_total
            LET l_fmt_ogb14t_total = cl_gr_numfmt('ogb_file','ogb14t',l_azi04)
            PRINTX l_fmt_ogb14t_total
         #FUN-C40071--add--end--
        ...
        AFTER GROUP OF sr1.oga02
            LET l_ogb14t_subtot = GROUP SUM(sr1.ogb14t)
            PRINTX l_ogb14t_subtot
            LET l_ogb14_subtot = GROUP SUM(sr1.ogb14)
            PRINTX l_ogb14_subtot
            LET l_ogb47_subtot = GROUP SUM(sr1.ogb47)
            PRINTX l_ogb47_subtot
            LET l_ogb12_subtot = GROUP SUM(sr1.ogb12)
            PRINTX l_ogb12_subtot
