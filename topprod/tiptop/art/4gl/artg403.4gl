...
MAIN
   ...
   LET l_table = cl_prt_temptable('artg403',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL artg403_tm(0,0)        # Input print condition
      ELSE CALL artg403()            # Read data and create out-file
   END IF
...
FUNCTION artg403_tm(p_row,p_col)
   ...
   OPEN WINDOW artg403_w AT p_row,p_col WITH FORM "art/42f/artg403" 
   ...
   LET tm.b = '1'  
    ...
    CONSTRUCT BY NAME tm.wc1 ON azp01
         ...
         AFTER FIELD azp01 
            ...
            END IF 
            IF g_chk_azp01 THEN
               DECLARE g403_zxy_cs1 CURSOR FOR SELECT zxy03 FROM zxy_file WHERE zxy01 = g_user
               FOREACH g403_zxy_cs1 INTO l_zxy03 
                 IF g_chk_auth IS NULL THEN
                    LET g_chk_auth = "'",l_zxy03,"'"
                 ELSE
                    LET g_chk_auth = g_chk_auth,",'",l_zxy03,"'"
      ...
      END IF
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW artg403_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)  #FUN-BA0061
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('artg403',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW artg403_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         CALL cl_gre_drop_temptable(l_table)  #FUN-BA0061
         EXIT PROGRAM
      END IF
...
FUNCTION artg403()
   ...
   #       l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT             #TQC-B70066 mark    
    ...
    IF STATUS THEN                                                                                                                   
       ...
       EXIT PROGRAM                                                                                                                 
    END IF
  DROP TABLE artg403_tmp    
    CREATE TEMP TABLE artg403_tmp( 
           oeaplant LIKE oea_file.oeaplant, 
           azp02    LIKE azp_file.azp02, 
           ima131   LIKE ima_file.ima131,
           oba02    LIKE oba_file.oba02, 
...
CASE tm.a
   ...
   CALL cl_replace_sqldb(l_sql) RETURNING l_sql          
    ...
    PREPARE artg403_prepare1 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       ...
       EXIT PROGRAM
    END IF
    DECLARE artg403_curs1 CURSOR FOR artg403_prepare1

    FOREACH artg403_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
   ...
   WHEN tm.b='1'
       ...
       WHEN tm.b='2'
         LET l_sql = " SELECT oeaplant,azp02,'','','','',",
                ...
                " ORDER BY lpk01,lpk04,oeaplant,azp02"
       WHEN tm.b='3'
          
                 LET l_sql =" SELECT oeaplant,azp02,'','',ima01,ima02,",
                " SUM(oeb14t),lpk01,lpk04,'','' ",
                " FROM artg403_tmp", 
                " GROUP BY oeaplant,azp02,ima01,ima02,lpk01,lpk04",
                " ORDER BY lpk01,lpk04,oeaplant,azp02,ima01,ima02"
    END CASE 
     PREPARE artg403_prepare2 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       CALL cl_gre_drop_temptable(l_table)  #FUN-BA0061
       EXIT PROGRAM
    END IF
    DECLARE artg403_curs2 CURSOR FOR artg403_prepare2

    FOREACH artg403_curs2 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
...
###GENGRE###   LET g_str = ''
   CASE
      WHEN tm.b='1'
###GENGRE###         CALL cl_prt_cs3('artg403','artg403',l_sql,g_str)  
    LET g_template = 'artg403'
    CALL artg403_grdata()    ###GENGRE###
      WHEN tm.b='2'
###GENGRE###         CALL cl_prt_cs3('artg403','artg403_1',l_sql,g_str) 
    LET g_template = 'artg403_1'
    CALL artg403_grdata()    ###GENGRE###
      WHEN tm.b='3'
###GENGRE###         CALL cl_prt_cs3('artg403','artg403_2',l_sql,g_str) 
    LET g_template = 'artg403_2'
    CALL artg403_grdata()    ###GENGRE###
   END CASE
END FUNCTION

###GENGRE###START
FUNCTION artg403_grdata()
    DEFINE l_sql    STRING
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("artg403")
        IF handler IS NOT NULL THEN
            START REPORT artg403_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
          
            DECLARE artg403_datacur1 CURSOR FROM l_sql
            FOREACH artg403_datacur1 INTO sr1.*
                OUTPUT TO REPORT artg403_rep(sr1.*)
            END FOREACH
            FINISH REPORT artg403_rep
        END IF
...
#FUN-BA0061------END------
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            LET sr1_o.oeaplant = NULL #FUN-BA0061        
        ...
        ON EVERY ROW
            ...
            IF NOT cl_null(sr1_o.oeaplant) THEN
               IF sr1_o.oeaplant = sr1.oeaplant THEN
                  LET l_display2 = 'N'
