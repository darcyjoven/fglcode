...
# Modify.........: No.FUN-850046 08/05/14 By lutingting報表轉為使用CR
# Modify.........: No.FUN-870144 08/07/29 By chenmoyan 追單到31區
# Modify.........: No.TQC-950135 09/06/05 By chenmoyan tm.t,tm.s與畫面欄位名稱不一致
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aqcr450',g_sql) CLIPPED
...
#------------No.TQC-610086 modify
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL aqcr450_tm(0,0)        # Input print condition
      ELSE CALL aqcr450()            # Read data and create out-file
   END IF
   CALL aqcr450()
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
END MAIN
 
FUNCTION aqcr450_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW aqcr450_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcr450"
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW aqcr450_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcr450',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW aqcr450_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0085
         #l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT        #No.FUN-680104 VARCHAR(1100)
          ...
          #No.FUN-550063  --end
                        ...
                        END RECORD
  DEFINE l_ima021      LIKE ima_file.ima021    #No.FUN-850046
     CALL cl_del_data(l_table)   #No.FUN-850046
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aqcr450'   #No.FUN-850046
     ...
     PREPARE aqcr450_prepare1 FROM l_sql
     ...
     END IF
     DECLARE aqcr450_curs1 CURSOR FOR aqcr450_prepare1
 
      #CALL cl_outnam('aqcr450') RETURNING l_name   #No.FUN-850046
 
     #START REPORT aqcr450_rep TO l_name   #No.FUN-850046
     LET g_pageno = 0
     FOREACH aqcr450_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
       ...
       SELECT COUNT(*) INTO sr.qty1 FROM qcf_file,sfb_file
        WHERE qcf02 = sfb01 AND sfb02 <> 7 AND qcf14='Y' AND qcf18='2'
       ...
       EXECUTE insert_prep USING
           sr.qcf04,sr.sfb22,sr.qcf03,sr.qcf02,sr.qcf01,sr.qcf021,sr.ima02,
           l_ima021,sr.gem02,sr.qty1,sr.qty2,sr.code
       #OUTPUT TO REPORT aqcr450_rep(sr.*)
       #No.FUN-850046--end
     END FOREACH
     ...
     LET g_str = tm.wc,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",
                 tm.bdate,";",tm.edate 
     CALL cl_prt_cs3('aqcr450','aqcr450',g_sql,g_str)
     #FINISH REPORT aqcr450_rep
...
##     PRINT g_x[11] CLIPPED,g_x[19] CLIPPED, 8 SPACES ,g_x[12] CLIPPED,14 SPACES,
##           g_x[13] CLIPPED
##     PRINT "-------- ---------- ---------------- ---------- ---------- -------------------- ",
##           "------------------------------ ",
##           "--------"
...
#                 COLUMN g_c[37],g_x[13] CLIPPED,cl_numfor(sr.qty1+sr.qty2,15,0)
#      PRINT g_dash
#      LET l_last_sw = 'y'
