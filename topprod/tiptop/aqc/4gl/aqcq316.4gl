...
#
# Pattern name...: aqcq316.4gl
# Descriptions...: 廠商品質履歷查詢
# Date & Author..: 01/05/15 By Mandy
...
# Modify.........: No.FUN-850013 08/05/05 By jan 報表改CR輸出
...
#     DEFINE   l_time LIKE type_file.chr8             #No.FUN-6A0085
   ...
   END IF
#No.FUN-850013--BEGIN--                                                                                                             
   LET g_sql1="qcs03.qcs_file.qcs03,",                                                                                               
              ...
              "rate.ima_file.ima18,", 
              "qct08.ze_file.ze03"
    LET l_table=cl_prt_temptable("aqcq316",g_sql1) CLIPPED                                                                           
...
#No.FUN-850013--END--   
    ...
    OPEN WINDOW q316_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcq316"
...
#       CALL q316_q()
#    END IF
IF NOT cl_null(g_argv1) THEN CALL q316_q() END IF
    CALL q316_menu()
    CLOSE WINDOW q316_w
...
#bugno:6062 end..............................
   ...
   PREPARE q316_prepare FROM g_sql
   DECLARE q316_cs SCROLL CURSOR WITH HOLD FOR q316_prepare
   LET g_sql=" SELECT UNIQUE qcs03 FROM qcs_file ",
             " WHERE ",tm.wc CLIPPED,
             " AND qcs14='Y' ",
             " AND qcs04 BETWEEN '",tm.bdate,"' AND '",tm.edate,"'",
   ...
   PREPARE q316_precount FROM g_sql
   DECLARE q316_count SCROLL CURSOR FOR q316_precount
END FUNCTION
...
FUNCTION q316_q()
  ...
  DEFINE l_qcs03 LIKE qcs_file.qcs03
    ...
    CALL q316_cs()
    IF INT_FLAG THEN LET INT_FLAG = 0 RETURN END IF
    OPEN q316_cs                            # 從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
       CALL cl_err('',SQLCA.sqlcode,0)
    ELSE
       LET m_cnt=0
       FOREACH q316_count INTO l_qcs03
          IF SQLCA.sqlcode THEN EXIT FOREACH END IF
          LET m_cnt=m_cnt+1
       END FOREACH
       DISPLAY m_cnt TO FORMONLY.cnt
       LET g_row_count = m_cnt
       CALL q316_fetch('F')                  # 讀出TEMP第一筆並顯示
    END IF
    MESSAGE ''
END FUNCTION
 
 
FUNCTION q316_fetch(p_flag)
DEFINE
    p_flag          LIKE type_file.chr1,                 #處理方式        #No.FUN-680104 VARCHAR(1)
    l_abso          LIKE type_file.num10                 #絕對的筆數        #No.FUN-680104 INTEGER
 
    CASE p_flag
        WHEN 'N' FETCH NEXT     q316_cs INTO g_qcs.qcs03
        WHEN 'P' FETCH PREVIOUS q316_cs INTO g_qcs.qcs03
        WHEN 'F' FETCH FIRST    q316_cs INTO g_qcs.qcs03
        WHEN 'L' FETCH LAST     q316_cs INTO g_qcs.qcs03
        WHEN '/'
...
#                    CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
              ...
              END IF
          END IF
          FETCH ABSOLUTE g_jump q316_cs INTO g_qcs.qcs03
          LET mi_no_ask = FALSE
    END CASE
...
FUNCTION q316_b_fill()              #BODY FILL UP
   ...
   LET l_sql =
     "SELECT qcs01,qcs04,qcs021,ima02,qct04,azf03,qct11,qct07,0,qct08 ",
     " FROM  qcs_file LEFT OUTER JOIN ima_file ON ima01=qcs021,qct_file LEFT OUTER JOIN azf_file ON qct04=azf01 AND azf02 = '6'",
     "   WHERE qct01 = qcs01 ",
     "   AND qct02 = qcs02 ",
     "   AND qct021= qcs05 ",
     "   AND qcs04 BETWEEN '",tm.bdate,"' AND '",tm.edate,"'",
     ...
     "   AND qcs14='Y' ",
     "   AND qcs00<'5' ",  #No.FUN-5C0078
     " ORDER BY qcs01,qcs04,qcs021,ima02,qct04 "
    PREPARE q316_pb FROM l_sql
    DECLARE q316_bcs CURSOR FOR q316_pb
    CALL g_qct.clear()
    LET g_rec_b=0 LET g_cnt = 1
    FOREACH q316_bcs INTO g_qct[g_cnt].*,m_qcs01,m_qcs02,m_qcs05
...
#      IF g_cnt > g_qct_arrno THEN CALL cl_err('',9035,0) EXIT FOREACH END IF
    ...
    CALL g_qct.deleteElement(g_cnt)   #No.MOD-5A0169 add
    LET g_cnt = 0                      #No.MOD-5A0169 add
 
END FUNCTION
 
 
FUNCTION q316_bp(p_ud)
   ...
   #DISPLAY ARRAY g_qct TO s_qct.* #MOD-5A0169 mark
   DISPLAY ARRAY g_qct TO s_qct.* ATTRIBUTE(COUNT=g_rec_b,UNBUFFERED) #MOD-5A0169 add
...
DEFINE l_ima021     LIKE ima_file.ima021                               #No.FUN-850013
...
#   CALL cl_outnam('aqcq316') RETURNING l_name
#No.FUN-850013--END--
    SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_lang
     LET g_sql =
     "SELECT qcs03,pmc03,qcs01,qcs04,qcs021,ima02,qct04,azf03,qct11,qct07,qct08,0 ",
    ...
    PREPARE q316_p1 FROM g_sql                # RUNTIME 編譯
    DECLARE q316_co                         # SCROLL CURSOR
        CURSOR FOR q316_p1
 
# genero  script marked     LET g_pageno = 0
#   START REPORT q316_rep TO l_name         #No.FUN-850013
 
    FOREACH q316_co INTO sr.*,m_qcs01,m_qcs02,m_qcs05
        IF SQLCA.sqlcode THEN
...
#       OUTPUT TO REPORT q316_rep(sr.*)
        EXECUTE insert_prep USING sr.qcs03,sr.pmc03,sr.qcs01,sr.qcs04,sr.qcs021,
                                  sr.ima02,l_ima021,sr.qct04,sr.azf03,sr.qct11,
                                  sr.qct07,sr.rate,sr.qct08
 
    END FOREACH
 
#   FINISH REPORT q316_rep
    ...
    CALL cl_prt_cs3('aqcq316','aqcq316',g_sql1,g_str)
    CLOSE q316_co
    ERROR ""
...
#
#    ORDER BY sr.qcs03,sr.qcs01,sr.qcs04,sr.qcs021,sr.qct04
#
#    FORMAT
#        PAGE HEADER
...
#                LET l_ima021 = NULL
#            END IF
#            PRINT COLUMN g_c[33],sr.qcs021,
#                   COLUMN g_c[34],sr.ima02, #MOD-4A0238
#                  COLUMN g_c[35],l_ima021;
#        ON EVERY ROW
#
#            PRINT COLUMN g_c[36],sr.qct04,
#                  COLUMN g_c[37],sr.azf03,
#                  COLUMN g_c[38],cl_numfor(sr.qct11,38,0),
#                  COLUMN g_c[39],cl_numfor(sr.qct07,39,0),
#                  COLUMN g_c[40],cl_numfor(sr.rate,40,3),
#                  COLUMN g_c[41],sr.qct08
#
#        ON LAST ROW
#            PRINT g_dash
#            LET l_trailer_sw = 'y'
