...
#
# Pattern name...: aqcq515.4gl
# Descriptions...: aqcq515 FQC品質記錄彙總查詢(BY 產品)
# Date & Author..: 99/05/19 By Iceman
# Modify.........: No.FUN-530065 05/03/29 By Will 增加料件的開窗
# Modify.........: No.TQC-5A0043 05/10/14 By Carrier 報表格式修改
...
# Modify.........: No.FUN-7B0134 07/11/28 By claire 不以料號跳頁
# Modify.........: No.FUN-850073 08/05/16 By dxfwo  CR報表
...
DEFINE
    ...
    g_qcm  RECORD
            ...
            qcm22               LIKE qcm_file.qcm06,       #No.FUN-680104 DECIMAL(10,3)      #送驗量
...
#     DEFINE   l_time LIKE type_file.chr8            #No.FUN-6A0085
 ...
 #No.FUN-850073---Begin 
   ...
   LET l_table = cl_prt_temptable('aqcq515',g_sql) CLIPPED                                                                          
...
#No.FUN-850073---End
   ...
   END IF
    ...
    OPEN WINDOW q515_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcq515"
...
#bugno:6062 end..............................
   ...
   MESSAGE ' WAIT '
   LET g_sql=" SELECT UNIQUE qcm021,ima02",         
             " FROM qcm_file,ima_file ",
             " WHERE ",tm.wc CLIPPED,
             " AND ima01 = qcm021 AND qcm14='Y' AND qcm18='1' ",
   ...
   PREPARE q515_prepare FROM g_sql
   DECLARE q515_cs SCROLL CURSOR FOR q515_prepare
   ...
   PREPARE q515_prepare1 FROM g_sql
   DECLARE q515_count CURSOR FOR q515_prepare1
END FUNCTION
...
FUNCTION q515_q()
    ...
    OPEN q515_cs                            # 從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
       CALL cl_err('',SQLCA.sqlcode,0)
    ELSE
       OPEN q515_count
       FETCH q515_count INTO g_row_count
       DISPLAY g_row_count TO FORMONLY.cnt
       CALL q515_fetch('F')                  # 讀出TEMP第一筆並顯示
    END IF
    MESSAGE ''
END FUNCTION
 
FUNCTION q515_fetch(p_flag)
DEFINE
    p_flag          LIKE type_file.chr1,         #No.FUN-680104 VARCHAR(1)     #處理方式
    l_abso          LIKE type_file.num10         #No.FUN-680104 INTEGER         #絕對的筆數
    CASE p_flag
        WHEN 'N' FETCH NEXT     q515_cs INTO g_qcm.qcm021,g_qcm.ima02
        WHEN 'P' FETCH PREVIOUS q515_cs INTO g_qcm.qcm021,g_qcm.ima02
        WHEN 'F' FETCH FIRST    q515_cs INTO g_qcm.qcm021,g_qcm.ima02
        WHEN 'L' FETCH LAST     q515_cs INTO g_qcm.qcm021,g_qcm.ima02
        WHEN '/'
...
#                    CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
              ...
              END IF
          END IF
          FETCH ABSOLUTE g_jump q515_cs INTO g_qcm.qcm021,g_qcm.ima02
          LET mi_no_ask = FALSE
    END CASE
    IF SQLCA.SQLCODE THEN
...
# 計算送驗批數
   SELECT COUNT(*) INTO g_qcm.lot FROM qcm_file, ima_file
        WHERE qcm021 = g_qcm.qcm021 AND ima01 = g_qcm.qcm021
           AND qcm04 BETWEEN g_qcm.bdate AND g_qcm.edate AND qcm14='Y' AND qcm18='1'
...
# 計算不合格批數
   SELECT count(*) INTO g_qcm.rejqty FROM qcm_file,sfb_file
	...
	AND qcm04 BETWEEN g_qcm.bdate AND g_qcm.edate
          AND qcm09 != '1' AND qcm14='Y' AND qcm18='1'
...
DEFINE
    ...
    CALL cl_wait()
#   CALL cl_outnam('aqcq515') RETURNING l_name  #No.FUN-850073
    SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_lang
#FUN-670106--begin
#   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aqcq515'
#   IF g_len = 0 OR g_len IS NULL THEN LET g_len = 100 END IF #No.TQC-5A0043
#   FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR
#   FOR g_i = 1 TO g_len LET g_dash_1[g_i,g_i] = '-' END FOR
#FUN-670106--end
#   START REPORT q515_rep TO l_name             #No.FUN-850073
    CALL cl_del_data(l_table)                   #No.FUN-850073
    FOREACH q515_cs INTO g_qcm.qcm021,g_qcm.ima02
        IF SQLCA.SQLCODE THEN
...
#              DISPLAY BY NAME g_qcm.*                                                                                                          
        CALL q515_calc()
#No.FUN-850073---Begin
#       OUTPUT TO REPORT q515_rep(g_qcm.*)
        EXECUTE insert_prep USING g_qcm.qcm021, g_qcm.lot,    g_qcm.qcm22,  g_qcm.stkqty, 
                                  g_qcm.rejqty, g_qcm.ima02,  g_qcm.crqty,  g_qcm.crrate,
                                  g_qcm.rejrate,g_qcm.ima021 
    END FOREACH
    ...
    END IF                                                                                                                          
     ...
     CALL cl_prt_cs3('aqcq515','aqcq515',l_sql,g_str)  
#   FINISH REPORT q515_rep
#   ERROR ""
