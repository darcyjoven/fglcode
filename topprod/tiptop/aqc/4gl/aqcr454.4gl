...
# Modify.........: NO.FUN-740013 07/04/20 By TSD.c123k 改為 crystal report
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW r454_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcr454"
################################################################################
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcr454',g_time,l_cmd)
      END IF
      CLOSE WINDOW r454_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
#FUN-740013 - END
     ...
     PREPARE r454_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
        EXIT PROGRAM
     END IF
     DECLARE r454_curs1 CURSOR FOR r454_prepare1
#     CALL cl_outnam('aqcr454') RETURNING l_name       #NO.CHI-910058 mark
#     START REPORT r454_rep TO l_name                  #NO.CHI-910058 mark
     LET g_pageno = 0
     ...
     DELETE FROM aqcr454_tmp
 
     FOREACH r454_curs1 INTO sr1.*
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
         INSERT INTO aqcr454_tmp VALUES (sr1.*)
     ...
     LET l_i = 1
     DECLARE r454_head1 CURSOR FOR                    #No.FUN-580013
      SELECT qcu04,SUM(qcu05) qcu05 FROM aqcr454_tmp
       GROUP BY qcu04
       ORDER BY qcu05 DESC
     FOREACH r454_head1 INTO g_head1[l_i].*               #No.FUN-580013
     ...
     #------單身資料-----------------
#NO.CHI-910058 start -----------------------------------------------
     DECLARE r454_body CURSOR FOR
     # SELECT * FROM aqcr454_tmp                         #NO.CHI-910058 mark
       SELECT DISTINCT qcf021,qcf01,qcf22 FROM aqcr454_tmp
       ORDER BY qcf021
    #  FOREACH r454_body INTO sr.*                          #NO.CHI-910058 mark   
      FOREACH r454_body INTO sr.qcf021,sr.qcf01,sr.qcf22 
       IF STATUS THEN CALL cl_err('foreach body',STATUS,0) EXIT FOREACH END IF
 
     #  OUTPUT TO REPORT r454_rep(sr.*)                   #NO.CHI-910058 mark
       ...
       LET l_i =1
       DECLARE r454_head11 CURSOR FOR 
        SELECT DISTINCT qcu04,SUM(qcu05) qcu05 FROM aqcr454_tmp
         GROUP BY qcu04
         ORDER BY qcu05 DESC
       FOREACH r454_head11 INTO sr.qcu04,sr.qcu05 
          IF STATUS THEN CALL cl_err('foreach head',STATUS,0) EXIT FOREACH END IF
 
          SELECT SUM(qcu05) INTO l_amt FROM aqcr454_tmp
    ...
    #  LET amt = GROUP SUM(sr.qcf22)  #
      SELECT SUM(qcf22) INTO amt
        FROM aqcr454_tmp
       WHERE qcf021 = sr.qcf021
 
      DROP TABLE x
#     SELECT DISTINCT qcf01 INTO cnt FROM aqcr454_tmp  #No.TQC-950136
      SELECT DISTINCT qcf01 FROM aqcr454_tmp           #No.TQC-950136
       WHERE qcf021 = sr.qcf021
       GROUP BY qcf01
       INTO TEMP x
      ...
      CALL r454_qce(g_head1[1].qcu04) RETURNING l_str1
      CALL r454_qce(g_head1[2].qcu04) RETURNING l_str2
      CALL r454_qce(g_head1[3].qcu04) RETURNING l_str3
      CALL r454_qce(g_head1[4].qcu04) RETURNING l_str4
      CALL r454_qce(g_head1[5].qcu04) RETURNING l_str5
      CALL r454_qce(g_head1[6].qcu04) RETURNING l_str6
      CALL r454_qce(g_head1[7].qcu04) RETURNING l_str7
      CALL r454_qce(g_head1[8].qcu04) RETURNING l_str8
      CALL r454_qce(g_head1[9].qcu04) RETURNING l_str9
      CALL r454_qce(g_head1[10].qcu04) RETURNING l_str10
...
#NO.CHI-910058 END---------------------------------------------------------
    ...
    # FINISH REPORT r454_rep                             #NO.CHI-910058 mark
    ...
    CALL cl_prt_cs3('aqcr454','aqcr454',l_sql,g_str)
    #------------------------------ CR (4) ------------------------------#
    # FUN-740013 end
...
#     DROP TABLE x
#     SELECT DISTINCT qcf01 INTO cnt FROM aqcr454_tmp
#      WHERE qcf021 = sr.qcf021
#      GROUP BY qcf01
#      INTO TEMP x
#     SELECT COUNT(*) INTO cnt FROM x
...
FUNCTION r454_cre_tmp()
#No.FUN-680104-begin
  CREATE TEMP TABLE aqcr454_tmp(
    sfb82     LIKE sfb_file.sfb82,
    qcu04     LIKE qcu_file.qcu04,
    qcu05     LIKE qcu_file.qcu05,
    qcf01     LIKE qcf_file.qcf01,
