...
#No.FUN-830131  --BEGIN
       #CONSTRUCT g_wc ON qcu01, qcu02, qcu021, qcu03, qcu04, qcu05 
        ...
        CONSTRUCT g_wc ON qcu01, qcu02, qcu021, qcu03,qcuicd01,qcuicd02,qcu04, qcu05
             FROM qcu01,qcu02,qcu021,s_qcu[1].qcu03,s_qcu[1].qcuicd01,s_qcu[1].qcuicd02,
                  s_qcu[1].qcu04,s_qcu[1].qcu05
         ON ACTION controlp                                                                                                          
         CASE                                                                                                                       
...
#FUN-830131  --BEGIN
        AFTER FIELD qcuicd01
           IF NOT cl_null(g_qcu[l_ac].qcuicd01) THEN
              LET l_n = 0
              SELECT COUNT(DISTINCT ida05) INTO l_n FROM ida_file
               WHERE ida30 = g_qcu01 AND ida31 = g_qcu02
              IF l_n = 0 THEN
                 CALL cl_err(g_qcu[l_ac].qcuicd01,'aic-212',0)
                 NEXT FIELD qcuicd01
              END IF
           END IF
    
        AFTER FIELD qcuicd02
           IF NOT cl_null(g_qcu[l_ac].qcuicd02) THEN
              LET l_n = 0
              SELECT COUNT(icd01) INTO l_n FROM icd_file 
               WHERE icd02 = 'T' 
              IF l_n = 0 THEN
                 CALL cl_err(g_qcu[l_ac].qcuicd02,'aic-213',0)
                 NEXT FIELD qcuicd02
              END IF
              CALL i111_icd03('d')
           END IF
...
DEFINE
    ...
    FOREACH qcu_curs INTO g_qcu[g_cnt].*   #單身 ARRAY 填充
        ...
        END CASE
#No.FUN-830131  --begin
        SELECT icd03 INTO g_qcu[g_cnt].icd03 FROM icd_file  
         WHERE icd01 = g_qcu[g_cnt].qcuicd02                
#No.FUN-830131  --end
        LET g_cnt = g_cnt + 1
