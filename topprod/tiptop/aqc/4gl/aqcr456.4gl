...
#
# Pattern name...: aqcr456.4gl
# Descriptions...: Run Card FQC料件品質履歷報告
...
# Modify.........: No.FUN-850054 08/05/12 By ChenMoyan
...
#No.FUN-850054 --Begin
   LET g_sql = "qcf01.qcf_file.qcf01,",
               ...
               "l_qcf13.gen_file.gen02"
LET l_table=cl_prt_temptable('aqcr456',g_sql) CLIPPED
IF l_table=-1 THEN EXIT PROGRAM END IF
LET g_sql1="qcf01.qcf_file.qcf01,",
           "qcg03.qcg_file.qcg03,",
           "l_str.type_file.chr1000"
LET l_table1=cl_prt_temptable('aqcr456_sub',g_sql1) CLIPPED
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW r456_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcr456"
################################################################################
...
WHILE TRUE
   WHILE TRUE
      CONSTRUCT BY NAME tm.wc ON qcf021,qcf03,qcf13
         #No.FUN-580031 --start--
         BEFORE CONSTRUCT
             CALL cl_qbe_init()
         #No.FUN-580031 ---end---
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcr456',g_time,l_cmd)
      END IF
      CLOSE WINDOW r456_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
#No.FUN-850054 --Begin
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='aqcr456'
...
#No.FUN-850054 --End
     ...
     LET l_sql = "SELECT qcf01,qcf02,qcf05,qcf03,' ',qcf021,' ',",
                 ...
                 " WHERE  qcf01 = qcg01 ",
                 "   AND  qcf18 = '2' ",  #Run Card
                 "   AND  qcf14='Y' ",
                 "   AND  qcf04 BETWEEN '",tm.bdate,"' AND '",tm.edate,"' ",
                 "   AND ", tm.wc CLIPPED
 
 
     PREPARE r456_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
        EXIT PROGRAM
     END IF
     DECLARE r456_curs1 CURSOR FOR r456_prepare1
     CALL cl_outnam('aqcr456') RETURNING l_name
#    START REPORT r456_rep TO l_name                  #No.FUN-850054 --Begin
     LET g_pageno = 0
     FOREACH r456_curs1 INTO sr.*
         IF SQLCA.sqlcode != 0 THEN
         ...
         IF tm.ans = 'Y' THEN
               ...
               LET l_n1 = l_n1 +1                                                                                                       
                  ...
                  ELSE                                                                                                                   
                     LET l_n1=0
                     LET l_str1 = l_str1 CLIPPED,' ', cl_numfor(sr.qcgg04,6,2)                                                            
                     EXECUTE insert_prep1 USING sr.qcf01,sr.qcg03,l_str1
                     LET l_flag = 'Y'
                     LET l_str1 = NULL                                                                                                    
                  END IF                                                                                                                 
              END FOREACH    
              ...
              END IF
         END IF
         EXECUTE insert_prep USING sr.qcf01,sr.qcf02,sr.qcf05,sr.qcf03,sr.pmc03,
                                   sr.qcf021,sr.ima02,sr.qcf04,sr.qcf22,sr.qcf091,
                                   sr.qcf09,sr.qcf13,sr.qcg03,sr.qcg04,sr.azf03,
                                   sr.qcg11,sr.qcg07,sr.qcg08,sr.qcg131,sr.qcg132,
                                   l_qcf13
#        OUTPUT TO REPORT r456_rep(sr.*)
     END FOREACH
 
#    FINISH REPORT r456_rep
 
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
     LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,'|',
                 "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED
     IF g_zz05='Y' THEN
         CALL cl_wcchp(tm.wc,'qcf021,qcf03,qcf13')
               RETURNING tm.wc
     ELSE
         LET tm.wc=""
     END IF
     LET g_str=tm.wc,';',tm.bdate,';',tm.edate,';',tm.ans
     CALL cl_prt_cs3('aqcr456','aqcr456',g_sql,g_str)
...
#         l_qcf09      LIKE ze_file.ze04,       #No.FUN-680104 VARCHAR(04)
...
#           COLUMN g_c[35], cl_numfor(sr.qcf22,35,0),
#           COLUMN g_c[36], cl_numfor(sr.qcf091,36,0),
#           COLUMN g_c[37], l_qcf09 CLIPPED,
#           COLUMN g_c[38], sr.qcf13 CLIPPED,
#           COLUMN g_c[39], l_qcf13 CLIPPED;
...
#     END CASE
#     PRINT COLUMN g_c[40],sr.qcg04,
#           COLUMN g_c[41],sr.azf03,
#           COLUMN g_c[42],cl_numfor(sr.qcg11,42,0),
#           COLUMN g_c[43],cl_numfor(sr.qcg07,43,0),
#           COLUMN g_c[44],l_str,
#           COLUMN g_c[45],cl_numfor(sr.qcg131,45,2),   #No.FUN-620041 modify 
#           COLUMN g_c[46],cl_numfor(sr.qcg132,46,2)    #No.FUN-620041 modify
#     IF tm.ans = 'Y' THEN
#        LET l_n = 0
#        DECLARE qcgg04_cs CURSOR FOR
#         SELECT qcgg04 FROM qcgg_file
