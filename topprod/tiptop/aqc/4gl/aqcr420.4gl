...
#FUN-740013 - START
   ...
   LET l_table = cl_prt_temptable('aqcr420',g_sql) CLIPPED   # 產生Temp Table
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW r420_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcr420"
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcr420',g_time,l_cmd)
      END IF
      CLOSE WINDOW r420_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
#FUN-740013 - END
     ...
     LET l_sql = "SELECT sfb82,qcu04,qcu05,qcf01,qcf02,qcf021,qcf22",
                 ...
                 "   AND qcf14='Y' AND qcf09='2' AND qcf18='1' ",
     ...
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE r420_curs1 CURSOR FOR r420_prepare1
#    CALL cl_outnam('aqcr420') RETURNING l_name                                #CHI-910058 Mark
#    START REPORT r420_rep TO l_name                                           #CHI-910058 Mark
#    LET g_pageno = 0                                                          #CHI-910058 Mark
     ...
     DELETE FROM aqcr420_tmp
 
     FOREACH r420_curs1 INTO sr1.*
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
 
         INSERT INTO aqcr420_tmp VALUES (sr1.*)
     END FOREACH
     #------表頭前十名不良原因資料----------
     FOR l_i = 1 TO 10
     ...
     LET l_i = 1
     DECLARE r420_head CURSOR FOR
      SELECT qcu04,SUM(qcu05) qcu05 FROM aqcr420_tmp
       GROUP BY qcu04
       ORDER BY qcu05 DESC
     FOREACH r420_head INTO g_head1[l_i].*
       IF STATUS THEN CALL cl_err('foreach head',STATUS,0) EXIT FOREACH END IF
       IF l_i >= 10 THEN  EXIT FOREACH END IF
       LET l_i = l_i + 1
     END FOREACH
     #------單身資料-----------------
     DECLARE r420_sfb82 CURSOR FOR                                                      #CHI-910058 Add
       SELECT DISTINCT sfb82 FROM aqcr420_tmp                                           #CHI-910058 Add 
     FOREACH r420_sfb82 INTO sr.sfb82                                                   #CHI-910058 Add
       FOR l_n = 1 TO 10                                                                #CHI-910058 Add     
           LET l_qcu05[l_n] = 0                                                         #CHI-910058 Add
       END FOR                                                                          #CHI-910058 Add     
     
     DECLARE r420_body CURSOR FOR
#     SELECT * FROM aqcr420_tmp                                                         #CHI-910058 Mark
      SELECT DISTINCT qcu04,SUM(qcu05) qcu05 FROM aqcr420_tmp                           #CHI-910058 Add
       GROUP BY qcu04                                                                   #CHI-910058 Add
       ORDER BY qcu05 DESC                                                              #CHI-910058 Add
#    FOREACH r420_body INTO sr.*                                                        #CHI-910058 Mark
     FOREACH r420_body INTO sr.qcu04                                                    #CHI-910058 Add
       IF STATUS THEN CALL cl_err('foreach body',STATUS,0) EXIT FOREACH END IF
 
#      OUTPUT TO REPORT r420_rep(sr.*)                                                  #CHI-910058 Mark
     #CHI-910058 --Begin--#
      SELECT SUM(qcu05) INTO l_amt FROM aqcr420_tmp
     ...
     #SELECT SUM(qcf22) INTO amt FROM aqcr420_tmp
     # WHERE sfb82 = sr.sfb82                
      LET g_sql = "SELECT SUM(A.qcf22) FROM ",                                                                                      
                  " (SELECT DISTINCT sfb82,qcf22 FROM aqcr420_tmp WHERE sfb82 = '",sr.sfb82,"' ) A"                                 
      PREPARE pre_sel_sum FROM g_sql                                                                                                
      EXECUTE pre_sel_sum INTO amt       
     #CHI-910058 END
 
      DROP TABLE x
      #SELECT DISTINCT qcf01 INTO cnt FROM aqcr420_tmp  #FUN-A50053 mark
      SELECT DISTINCT qcf01  FROM aqcr420_tmp   #FUN-A50053 mod
       WHERE sfb82 = sr.sfb82
       GROUP BY qcf01
       INTO TEMP x
     ...
     #CHI-910058--Begin--#
      CALL r420_qce(g_head1[1].qcu04) RETURNING l_str1
      CALL r420_qce(g_head1[2].qcu04) RETURNING l_str2
      CALL r420_qce(g_head1[3].qcu04) RETURNING l_str3
      CALL r420_qce(g_head1[4].qcu04) RETURNING l_str4
      CALL r420_qce(g_head1[5].qcu04) RETURNING l_str5
      CALL r420_qce(g_head1[6].qcu04) RETURNING l_str6
      CALL r420_qce(g_head1[7].qcu04) RETURNING l_str7
      CALL r420_qce(g_head1[8].qcu04) RETURNING l_str8
      CALL r420_qce(g_head1[9].qcu04) RETURNING l_str9
      CALL r420_qce(g_head1[10].qcu04) RETURNING l_str10
...
#    FINISH REPORT r420_rep                                                             #CHI-910058 Mark
    ...
    CALL cl_prt_cs3('aqcr420','aqcr420',l_sql,g_str)
    #------------------------------ CR (4) ------------------------------#
    # FUN-740013 end
...
#     DROP TABLE x
#     SELECT DISTINCT qcf01 INTO cnt FROM aqcr420_tmp
#      WHERE sfb82 = sr.sfb82
#      GROUP BY qcf01
#      INTO TEMP x
#     SELECT COUNT(*) INTO cnt FROM x
...
FUNCTION r420_cre_tmp()
 
  DROP TABLE aqcr420_tmp
#No.FUN-680104-begin
  CREATE TEMP TABLE aqcr420_tmp(
    sfb82     LIKE sfb_file.sfb82,
    qcu04     LIKE qcu_file.qcu04,
    qcu05     LIKE qcu_file.qcu05,
    qcf01     LIKE qcf_file.qcf01,
