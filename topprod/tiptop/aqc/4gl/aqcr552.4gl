...
# Modify.........: No.FUN-5B0059 06/01/03 By Sarah 當(cl_null(g_bgjob) OR g_bgjob = 'N') AND cl_null(tm.wc)時才CALL r552_tm()
...
#FUN-730009.....begin
   ...
   LET l_table = cl_prt_temptable('aqcr552',g_sql) CLIPPED
...
#FUN-730009.....end
   ...
   LET l_table1 = cl_prt_temptable('aqcr5521',g_sql) CLIPPED
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01           #No.FUN-580031
   ...
   OPEN WINDOW r552_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcr552"
################################################################################
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcr552',g_time,l_cmd)
      END IF
      CLOSE WINDOW r552_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
#bugno:6010 --> SQL:qcm14 = 'Y' 移除
     ...
     PREPARE r552_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
        EXIT PROGRAM
     END IF
     DECLARE r552_curs1 CURSOR FOR r552_prepare1
#FUN-730009.....begin mark
#    CALL cl_outnam('aqcr552') RETURNING l_name
#    START REPORT r552_rep TO l_name
#FUN-730009.....end mark
     LET g_pageno = 0
     FOREACH r552_curs1 INTO sr.*
         IF SQLCA.sqlcode != 0 THEN
...
#FUN-730009.....begin
  ...
  #  LET l_sql = "SELECT * FROM ",l_table CLIPPED   #TQC-730113
    ...
    #CHI-8A0040-end-add
  #  CALL cl_prt_cs3('aqcr552',l_sql,g_str)         #TQC-730113
     LET g_cr_table = l_table                 #主報表的temp table名稱  #TQC-C10039
     LET g_cr_apr_key_f = "qcm01"       #報表主鍵欄位名稱  #TQC-C10039
     CALL cl_prt_cs3('aqcr552','aqcr552',l_sql,g_str)
#    FINISH REPORT r552_rep
