...
#
# Pattern name...: aqcq311.4gl
# Descriptions...: 料件品質履歷查詢
...
# Modify.........: No.MOD-590003 05/09/05 By jackie 修正報表中表頭遺漏g_x[41]的情況
...
# Modify.........: No.FUN-850016 08/05/07 By mike 報表輸出方式轉為Crystal Reports
# Modify.........: No.FUN-980030 09/08/31 By Hiko 加上GP5.2的相關設定
# Modify.........: No:MOD-A30001 10/03/31 By Summer 將aqc-005改成apm-244
# Modify.........: No:TQC-B90211 11/10/21 By Smapmin 人事table drop
# Modify.........: No:TQC-B90211 11/10/21 By Smapmin 人事table drop
# Modify.........: No:CHI-C30045 12/03/16 By bart 增加規格欄位
...
DEFINE
    ...
    g_qcs  RECORD
            ...
            ima021              LIKE ima_file.ima021,        #CHI-C30045
            bdate               LIKE type_file.dat,          #No.FUN-680104 DATE
            edate               LIKE type_file.dat           #No.FUN-680104 DATE
        END RECORD,
...
#No.FUN-850016  --BEGIN                                                                                                             
   LET g_sql = "qcs021.qcs_file.qcs021,",                                                                                           
               ...
               "ma.qcs_file.qcs06,",                                                                                                
               "mi.qcs_file.qcs06"                                                                                                  
   LET l_table = cl_prt_temptable("aqcq311",g_sql) CLIPPED                                                                          
...
#No.FUN-850016  --END                    
    ...
    OPEN WINDOW q311_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcq311"
...
#       CALL q311_q()
#    END IF
IF NOT cl_null(g_argv1) THEN CALL q311_q() END IF
    CALL q311_menu()
    CLOSE WINDOW q311_w
...
#bugno:6062 end..............................
   ...
   LET g_sql=" SELECT UNIQUE qcs021 FROM qcs_file,ima_file ",
             " WHERE ",tm.wc CLIPPED,
             " AND qcs021=ima01 ",
   ...
   DECLARE q311_cs SCROLL CURSOR FOR q311_prepare
   ...
   DECLARE q311_count SCROLL CURSOR FOR q311_precount
END FUNCTION
...
FUNCTION q311_q()
 ...
 DEFINE l_qcs021  LIKE qcs_file.qcs021
    ...
    ELSE
       LET m_cnt = 0
       FOREACH q311_count INTO l_qcs021
...
DEFINE
    ...
    CASE p_flag
        WHEN 'N' FETCH NEXT     q311_cs INTO g_qcs.qcs021
        WHEN 'P' FETCH PREVIOUS q311_cs INTO g_qcs.qcs021
        WHEN 'F' FETCH FIRST    q311_cs INTO g_qcs.qcs021
        WHEN 'L' FETCH LAST     q311_cs INTO g_qcs.qcs021
        WHEN '/'
...
#                    CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
              ...
              END IF
          END IF
          FETCH ABSOLUTE g_jump q311_cs INTO g_qcs.qcs021
          LET mi_no_ask = FALSE
    END CASE
    ...
    #SELECT ima02 INTO g_qcs.ima02 FROM ima_file WHERE ima01=g_qcs.qcs021   #CHI-C30045 mark
    #IF SQLCA.sqlcode THEN LET g_qcs.ima02=' ' END IF                       #CHI-C30045 mark
    SELECT ima02, ima021 INTO g_qcs.ima02, g_qcs.ima021 FROM ima_file WHERE ima01=g_qcs.qcs021
    IF SQLCA.sqlcode THEN LET g_qcs.ima02=' ', g_qcs.ima021=' ' END IF
...
FUNCTION q311_b_fill()              #BODY FILL UP
   ...
   LET l_sql =
      #------------------No.TQC-750064 modify
      #"SELECT qcs04,qcs01,qcs03,pmc03,qcs06,qcs22,0,0,qcs09,",
       "SELECT qcs04,qcs01,qcs03,pmc03,qcs22,qcs091,0,0,qcs09,",
      #------------------No.TQC-750064 end
    ...
    DECLARE q311_bcs CURSOR FOR q311_pb
    FOR g_cnt = 1 TO g_qcs1.getLength()           #單身 ARRAY 乾洗
       INITIALIZE g_qcs1[g_cnt].* TO NULL
    END FOR
    LET g_rec_b=0 LET g_cnt = 1
    FOREACH q311_bcs INTO g_qcs1[g_cnt].*,m_qcs01,m_qcs02,m_qcs05
       #-----------No.TQC-750064 modify
...
DEFINE
    ...
    CALL cl_wait()
    #CALL cl_outnam('aqcq311') RETURNING l_name  #No.FUN-850016 
    SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_lang
 
   #--------------------No.TQC-750064 modify
   #LET g_sql="SELECT qcs021,ima02,ima021,qcs04,qcs03,pmc03,qcs22,qcs06,0,0,",
    LET g_sql="SELECT qcs021,ima02,ima021,qcs04,qcs03,pmc03,qcs22,qcs091,0,0,",
   #--------------------No.TQC-750064 end
              "       qcs09,0,0,0,qcs01,qcs02,qcs05",
              " FROM qcs_file LEFT OUTER JOIN ima_file ON ima01 = qcs021 LEFT OUTER JOIN pmc_file ON qcs03 = pmc01",
              " WHERE ",tm.wc CLIPPED,
              " AND qcs04 BETWEEN '",tm.bdate,"' AND '",tm.edate,"'",
              " AND qcs14='Y' ",
              " AND qcs00<'5' ",  #No.FUN-5C0078
    ...
    PREPARE q311_p1 FROM g_sql                # RUNTIME 編譯
    DECLARE q311_co                         # SCROLL CURSOR
        CURSOR FOR q311_p1
 
# genero  script marked     LET g_pageno = 0
    #START REPORT q311_rep TO l_name  #No.FUN-850016 
 
    FOREACH q311_co INTO sr.*,m_qcs01,m_qcs02,m_qcs05
        IF SQLCA.sqlcode THEN
   ...
   END CASE
        #No.FUN-850016  --BEGIN
        #OUTPUT TO REPORT q311_rep(sr.*)
        EXECUTE insert_prep USING sr.qcs021,sr.ima02,sr.ima021,sr.qcs04,sr.qcs03,                                                   
                                  sr.pmc03, sr.qcs22,sr.qcs091,sr.qty,  sr.rate,                                                    
                                  sr.qcs09, sr.cr,   sr.ma,    sr.mi                                                                
        #No.FUN-850016  --END         
    END FOREACH
 
    #FINISH REPORT q311_rep  #No.FUN-850016 
    ...
    CALL cl_prt_cs3('aqcq311','aqcq311',g_sql,g_str)                                                                                
    #No.FUN-850016   --end           
END FUNCTION
   ...
   OUTPUT
    ...
    FORMAT
        PAGE HEADER
            ...
            PRINT COLUMN 11,sr.ima021     #No.MOD-590003
            #END TQC-5B0109&051112
            PRINT g_x[31],g_x[32],g_x[33],g_x[34],g_x[35],
                  g_x[36],g_x[37],g_x[38],g_x[39],g_x[40],g_x[41]  #No.MOD-590003
            PRINT g_dash1
            LET l_trailer_sw = 'n'
        ...
        ON EVERY ROW
            PRINT COLUMN g_c[31],sr.qcs04,
                  ...
                  COLUMN g_c[34],cl_numfor(sr.qcs22,34,0),
                  COLUMN g_c[35],cl_numfor(sr.qcs091,35,0),    #No.TQC-750064 modify
                  COLUMN g_c[36],cl_numfor(sr.qty,36,2),
                  COLUMN g_c[37],cl_numfor(sr.rate,37,2),
                  COLUMN g_c[38],sr.qcs09,
                  COLUMN g_c[39],cl_numfor(sr.cr,39,0),
                  COLUMN g_c[40],cl_numfor(sr.ma,40,0),
                  COLUMN g_c[41],cl_numfor(sr.mi,41,0)
