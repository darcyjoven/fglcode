...
#
# Pattern name...: aqcr553.4gl
# Descriptions...: Run Card PQC料件品質履歷報告
...
# Modify.........: No.TQC-5B0034 05/11/08 By Rosayu 修改畫面欄位置設
# Modify.........: No.TQC-5B0106 05/11/12 By Dido 修改報表料件編號及品名欄位置設
...
# Modify.........: No.FUN-850054 08/05/15 By ChenMoyan 老報表轉CR
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD
              ...
              ans     LIKE type_file.chr1,         #No.FUN-680104 VARCHAR(1) 
...
#No.FUN-850054 --Begin
   ...
   LET l_table = cl_prt_temptable('aqcr553',g_sql)
   IF l_table = -1 THEN EXIT PROGRAM END IF
   LET g_sql1="qcm01.qcm_file.qcm01,",
              "qcn03.qcn_file.qcn03,",
              "l_str.type_file.chr1000"
   LET l_table1=cl_prt_temptable('aqcr553_sub',g_sql1)
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01           #No.FUN-580031
   ...
   OPEN WINDOW r553_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcr553"
################################################################################
...
WHILE TRUE
   WHILE TRUE
      CONSTRUCT BY NAME tm.wc ON qcm021,qcm03,qcm13
         #No.FUN-580031 --start--
         BEFORE CONSTRUCT
             CALL cl_qbe_init()
         #No.FUN-580031 ---end---
...
END CONSTRUCT
   ...
   INPUT BY NAME tm.bdate,tm.edate,tm.ans,
                 tm.more WITHOUT DEFAULTS #TQC-5B0034
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcr553',g_time,l_cmd)
      END IF
      CLOSE WINDOW r553_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
#No.FUN-850054 --Begin
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='aqcr553'
#No.FUN-850054 --End
     ...
     LET l_sql = "SELECT qcm01,qcm02,qcm05,qcm03,' ',qcm021,' ',",
                 ...
                 " WHERE  qcm01 = qcn01 ",
                 "   AND  qcm18 = '2' ",  #Run Card
                 "   AND  qcm14='Y' ",
                 "   AND  qcm04 BETWEEN '",tm.bdate,"' AND '",tm.edate,"' ",
                 "   AND ", tm.wc CLIPPED
 
 
     PREPARE r553_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
        EXIT PROGRAM
     END IF
     DECLARE r553_curs1 CURSOR FOR r553_prepare1
#No.FUN-850054 --Begin
#    CALL cl_outnam('aqcr553') RETURNING l_name
#    START REPORT r553_rep TO l_name
#    LET g_pageno = 0
     DECLARE qcnn04_cs CURSOR FOR                                                                                                   
               SELECT qcnn04 FROM qcnn_file                                                                                         
                WHERE qcnn01 = sr.qcm01 AND qcnn03 = sr.qcn03
#No.FUN-850054 --End
     FOREACH r553_curs1 INTO sr.*
         IF SQLCA.sqlcode != 0 THEN
         ...
         SELECT azf03 INTO sr.azf03 FROM azf_file
          WHERE azf01 = sr.qcn04 AND azf02='6' #6818
#No.FUN-850054 --Begin
#        OUTPUT TO REPORT r553_rep(sr.*)
     SELECT gen02 INTO l_qcm13 FROM gen_file WHERE gen01=sr.qcm13                                                               
         ...
         IF tm.ans = 'Y' THEN                                                                                                       
              ...
              END IF                                                                                                                
         END IF                                                                                                                     
         EXECUTE insert_prep USING sr.qcm01,sr.qcm02,sr.qcm03,sr.qcm021,sr.ima02,
                                   sr.qcm04,sr.qcm22,sr.qcm091,sr.qcm09,sr.qcm13,                                                   
                                   sr.qcn03,sr.qcn04,sr.azf03,sr.qcn11,sr.qcn07,
                                   sr.qcn08,sr.qcn131,sr.qcn132,l_qcm13
#No.FUN-850054 --End
     END FOREACH
#No.FUN-850054 --Bgein
#    FINISH REPORT r553_rep
 
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
     LET g_sql="SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,'|',
               "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED
     IF g_zz05 = 'Y' THEN
         CALL cl_wcchp(tm.wc,'qcm021,qcm03,qcm13')
                 RETURNING tm.wc
     ELSE
         LET tm.wc=""
     END IF
     LET g_str = tm.wc,';',tm.bdate,';',tm.edate,';',tm.ans
     CALL cl_prt_cs3('aqcr553','aqcr553',g_sql,g_str)
#No.FUN-850054 --End
END FUNCTION
...
#         l_num        LIKE ecb_file.ecb18,       #No.FUN-680104 DEC(15,2)
...
#         l_str        LIKE type_file.chr1000,     #No.FUN-680104 VARCHAR(4)
...
#     PRINT g_dash
##TQC-5B0106
#     PRINT g_x[15] CLIPPED,sr.qcm021
#     PRINT g_x[16] CLIPPED,sr.ima02
##    PRINT g_x[15] CLIPPED,sr.qcm021,' ',
##          g_x[16] CLIPPED,sr.ima02
##TQC-5B0106 End
...
##    PRINT ''
##    PRINT g_x[15] CLIPPED,COLUMN 36,g_x[16] CLIPPED,
##          COLUMN 75,g_x[17] CLIPPED,COLUMN 114,g_x[22] CLIPPED
##    PRINT g_x[23] CLIPPED,COLUMN 38,g_x[24] CLIPPED,
##          COLUMN 76,g_x[25] CLIPPED,COLUMN 115,g_x[26] CLIPPED
...
#           COLUMN g_c[33], sr.qcm03,
#           COLUMN g_c[34], sr.qcm02,
#           COLUMN g_c[35], cl_numfor(sr.qcm22,35,0),
#           COLUMN g_c[36], cl_numfor(sr.qcm091,36,0),
#           COLUMN g_c[37], l_qcm09 CLIPPED,
#           COLUMN g_c[38], sr.qcm13 CLIPPED,
#           COLUMN g_c[39], l_qcm13 CLIPPED;
...
#     END CASE
#     PRINT COLUMN g_c[40],sr.qcn04,
#           COLUMN g_c[41],sr.azf03,
#           COLUMN g_c[42],cl_numfor(sr.qcn11,42,0),
#           COLUMN g_c[43],cl_numfor(sr.qcn07,43,0),
#           COLUMN g_c[44],l_str,
#           COLUMN g_c[45],cl_numfor(sr.qcn131,45,2),     #No.FUN-620041 modify
#           COLUMN g_c[46],cl_numfor(sr.qcn132,46,2)      #No.FUN-620041 modify
#     IF tm.ans = 'Y' THEN
#        LET l_n = 0
#        DECLARE qcnn04_cs CURSOR FOR
#         SELECT qcnn04 FROM qcnn_file
