...
#
# Pattern name...: aqcq415.4gl
# Descriptions...: aqcq415 FQC品質記錄彙總查詢(BY 產品)
...
#No.FUN-850061 --End
   ...
   END IF
    OPEN WINDOW q415_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcq415"
...
#bugno:6062 end..............................
   ...
   PREPARE q415_prepare FROM g_sql
   DECLARE q415_cs SCROLL CURSOR FOR q415_prepare
   ...
   PREPARE q415_prepare1 FROM g_sql
   DECLARE q415_count CURSOR FOR q415_prepare1
END FUNCTION
...
FUNCTION q415_q()
    ...
    OPEN q415_cs                            # 從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
       CALL cl_err('',SQLCA.sqlcode,0)
    ELSE
       OPEN q415_count
       FETCH q415_count INTO g_row_count
       DISPLAY g_row_count TO FORMONLY.cnt
       CALL q415_fetch('F')                  # 讀出TEMP第一筆並顯示
    END IF
    MESSAGE ''
END FUNCTION
 
 
FUNCTION q415_fetch(p_flag)
DEFINE
    p_flag          LIKE type_file.chr1,         #No.FUN-680104              #處理方式
    l_abso          LIKE type_file.num10         #No.FUN-680104          #絕對的筆數
    CASE p_flag
        WHEN 'N' FETCH NEXT     q415_cs INTO g_qcf.ima02,g_qcf.qcf021
        WHEN 'P' FETCH PREVIOUS q415_cs INTO g_qcf.ima02,g_qcf.qcf021
        WHEN 'F' FETCH FIRST    q415_cs INTO g_qcf.ima02,g_qcf.qcf021
        WHEN 'L' FETCH LAST     q415_cs INTO g_qcf.ima02,g_qcf.qcf021
        WHEN '/'
...
#                    CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
              ...
              END IF
          END IF
          FETCH ABSOLUTE g_jump q415_cs INTO g_qcf.ima02,g_qcf.qcf021
          LET mi_no_ask = FALSE
    END CASE
    IF SQLCA.SQLCODE THEN
...
# 計算送驗批數
   SELECT COUNT(*) INTO g_qcf.lot FROM qcf_file, ima_file
        WHERE qcf021 = g_qcf.qcf021 AND ima01 = g_qcf.qcf021 AND qcf18='1'
...
# 計算送驗量
   SELECT SUM(qcf22) INTO g_qcf.qcf22 FROM qcf_file
       WHERE qcf021 = g_qcf.qcf021 AND qcf18='1'
         AND qcf04 BETWEEN g_qcf.bdate AND g_qcf.edate AND qcf14='Y'
...
DEFINE
    ...
    l_za05          LIKE ima_file.ima01          #No.FUN-680104 VARCHAR(40)
#No.FUN-850061 --Begin
...
#   CALL cl_wait()
#   CALL cl_outnam('aqcq415') RETURNING l_name
#No.FUN-850061 --End
    SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_lang
#   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aqcq415'        #No.FUN-670106
#   IF g_len = 0 OR g_len IS NULL THEN LET g_len = 100 END IF #No.TQC-5A0043
#   FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR                        #No.FUN-670106 
#   FOR g_i = 1 TO g_len LET g_dash_1[g_i,g_i] = '-' END FOR
#No.FUN-850061 --Begin
#   START REPORT q415_rep TO l_name
    SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='aqcq415'
#No.FUN-850061 --End
    FOREACH q415_cs INTO g_qcf.ima02,g_qcf.qcf021
        IF SQLCA.SQLCODE THEN
        ...
        CALL q415_calc()
#No.FUN-850061 --Begin
#       OUTPUT TO REPORT q415_rep(g_qcf.*)                     
        EXECUTE insert_prep USING g_qcf.* 
#No.FUN-850061 --End
    END FOREACH
#No.FUN-850061 --Begin
#   FINISH REPORT q415_rep
#   ERROR ""
#   CALL cl_prt(l_name,' ','1',g_len)
    ...
    CALL cl_prt_cs3('aqcq415','aqcq415',g_sql1,g_str)         
#No.FUN-850061 --End
    OPEN q415_cs                            # 從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
       CALL cl_err('',SQLCA.sqlcode,0)
...
{ START REPORT q415_rep TO l_name                                                                                                
     LET g_pageno=0                                                                                                                 
     FOREACH q415_calc INTO sr.*                                                                                                       
       IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF                                                          
       OUTPUT TO REPORT q415_rep(sr.*)                                                                                              
     END FOREACH                                                                                                                    
...
#                 COLUMN 36,g_x[12] CLIPPED, sr.qcf22  USING '###########&.&&&',
