...
#
# Pattern name...: aqcr451.4gl
# Descriptions...: FQC 不合格明細表(BY 客戶)
# Date & Author..: 99/05/19 By Iceman
# Modify.........: No.FUN-4C0099 05/01/28 By kim 報表轉XML功能
# Modify.........: No.FUN-550063 05/05/19 By Will 單據編號放大
...
MAIN
   ...
   LET g_sql = "occ01.occ_file.occ01,", 
               ...
               "aba18.aba_file.aba18" 
   LET l_table = cl_prt_temptable('aqcr451',g_sql) CLIPPED
...
#------------No.TQC-610086 modify
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL aqcr451_tm(0,0)        # Input print condition
      ELSE CALL aqcr451()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
END MAIN
 
FUNCTION aqcr451_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,         #No.FUN-680104 SMALLINT
   ...
   OPEN WINDOW aqcr451_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcr451"
...
WHILE TRUE
   CONSTRUCT BY NAME tm.wc ON occ01,qcf01,qcf03,qcf02,qcf021
         #No.FUN-580031 --start--
         BEFORE CONSTRUCT
             CALL cl_qbe_init()
         #No.FUN-580031 ---end---
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW aqcr451_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcr451',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW aqcr451_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0085
         #l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT        #No.FUN-680104 VARCHAR(1100)
          ...
          #No.FUN-550063  --start
          l_order   ARRAY[4] OF LIKE qcf_file.qcf021,       #No.FUN-680104 VARCHAR(40) #FUN-5B0105 20->40
           sr               RECORD order1 LIKE qcf_file.qcf021,       #No.FUN-680104 VARCHAR(40) #FUN-5B0105 20->40
                                   order2 LIKE qcf_file.qcf021,       #No.FUN-680104 VARCHAR(40) #FUN-5B0105 20->40
                                   order3 LIKE qcf_file.qcf021,       #No.FUN-680104 VARCHAR(40) #FUN-5B0105 20->40
          #No.FUN-550063  --end
                                  ...
                                  code   LIKE aba_file.aba18        #No.FUN-680104 VARCHAR(02)
                        END RECORD
 
     CALL cl_del_data(l_table)    #No.FUN-850046
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aqcr451'  #No.FUN-850046
     ...
     LET l_sql = "SELECT '','','',occ01,occ02,",
                 "     qcf04,sfb22,qcf03,qcf02,qcf01,qcf021,ima02,ima021,     ",
                 "     '',sfb82, gem02,0,0,0,'' ",
                 "  FROM qcf_file,sfb_file,ima_file,occ_file,oea_file,",
                 "       OUTER gem_file",
     ...
     PREPARE aqcr451_prepare1 FROM l_sql
     ...
     END IF
     DECLARE aqcr451_curs1 CURSOR FOR aqcr451_prepare1
 
      #CALL cl_outnam('aqcr451') RETURNING l_name       #No.FUN-850046
 
     #START REPORT aqcr451_rep TO l_name               #No.FUN-850046
     LET g_pageno = 0
     FOREACH aqcr451_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
       ...
       #        WHEN tm.s[g_i,g_i] = '3' LET l_order[g_i] = sr.qcf04
       #        WHEN tm.s[g_i,g_i] = '4' LET l_order[g_i] = sr.qcf021
       ...
       #OUTPUT TO REPORT aqcr451_rep(sr.*)
       EXECUTE insert_prep USING
           sr.occ01,sr.occ02,sr.qcf04,sr.sfb22,sr.qcf03,sr.qcf02,sr.qcf01,
           sr.qcf021,sr.ima02,sr.ima021,sr.sfb82,sr.gem02,sr.code           
       #No.FUN-850046--end       
     END FOREACH
     ...
     IF g_zz05 = 'Y'  THEN 
        CALL cl_wcchp(tm.wc,'occ01,qcf01,qcf03,qcf02,qcf021')
        RETURNING tm.wc
     END IF
     LET g_str = tm.wc,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",
                 tm.t[1,1],";",tm.t[2,2],";",tm.t[3,3],";",tm.bdate,";",tm.edate
     CALL cl_prt_cs3('aqcr451','aqcr451',g_sql,g_str)
     #FINISH REPORT aqcr451_rep
...
#  ORDER BY sr.occ01,sr.code ,sr.order1,sr.order2,sr.order3
#  FORMAT
#   PAGE HEADER
...
#      PRINT g_x[11] CLIPPED, tm.bdate ,'-',tm.edate CLIPPED
#      PRINT g_dash
#      PRINT g_x[15] CLIPPED,sr.occ01,' ',sr.occ02
...
##        PRINT g_x[15] CLIPPED,sr.occ01,' ',sr.occ02,'    ';
##     SKIP 1 LINE
##     PRINT g_x[11] CLIPPED,g_x[17] CLIPPED,'   ',g_x[12] CLIPPED,20 SPACES,
##           g_x[13] CLIPPED
##     PRINT "-------- ---------- ---------------- ---------- ---------- -------------------- ",
##           "------------------------------ ",
##           "--------"
...
#            COLUMN g_c[33],sr.qcf03,
#            COLUMN g_c[34],sr.qcf02,
#            COLUMN g_c[35],sr.qcf01,
#            COLUMN g_c[36],sr.qcf021,
#            COLUMN g_c[37],sr.ima02,
#            COLUMN g_c[38],sr.ima021,
#            COLUMN g_c[39],sr.sfb82,
