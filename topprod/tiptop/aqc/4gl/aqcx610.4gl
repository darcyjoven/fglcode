...
# Modify.........: No.FUN-CB0001 13/03/15 By yangtt CR轉XGrid     
# Modify.........: No.FUN-D40129 13/05/15 By yangtt qcs01新增開窗    
# Modify.........: No.FUN-D50099 13/05/29 By yangtt 新增規格(ima021)欄位顯示 
...
#No.FUN-A20051 ---BEGIN
   LET g_sql="qcs01.qcs_file.qcs01,",
            ...
            #"ima02.ima_file.ima02,",       #FUN-D50099 mark
             "ima02.type_file.chr300,",     #FUN-D50099 add
             "pmc03.pmc_file.pmc03,",
             "qce03.qce_file.qce03,",
             "per.type_file.num15_3,",
             "l_per.type_file.chr100"   #FUN-CB0001

   LET l_table = cl_prt_temptable('aqcx610',g_sql)
...
#No.FUN-570243 --start
     ON ACTION CONTROLP
        ...
        END IF
        #FUN-D40129----add---str--
        IF INFIELD(qcs01) THEN
           CALL cl_init_qry_var()
           LET g_qryparam.form = "q_qcs01_1"
           LET g_qryparam.state = "c"
           CALL cl_create_qry() RETURNING g_qryparam.multiret
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcx610',g_time,l_cmd)
      END IF
      CLOSE WINDOW x610_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
#FUN-A20051 ---add begin
                   #ima02      LIKE ima_file.ima02,      #FUN-D50099 mark
                    ima02      LIKE type_file.chr300,    #FUN-D50099 add
                    pmc03      LIKE pmc_file.pmc03,
                    qce03      LIKE qce_file.qce03,
                    per        LIKE qcu_file.qcu05,     #每一筆所占比率
#FUN-A20051 ---add end
                    l_per      LIKE type_file.chr100    #FUN-CB0001
                    END RECORD
#FUN-A20051 ---add begin
     DEFINE l_ima021      LIKE ima_file.ima021   #FUN-D50099

     LET g_sql="INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?)"    #FUN-CB0001 add 1?
...
#FUN-A20051 ---add end
     ...
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aqcx610'
...
#FUN-A20051 ---BEGIN
#     CALL cl_outnam('aqcx610') RETURNING l_name
#     IF tm.ch='1' THEN
#        START REPORT x610_rep1 TO l_name
#     ELSE
#        START REPORT x610_rep2 TO l_name
#     END IF
#     LET g_pageno = 0
#FUN-A20051 ---END
...
#FUN-A20051 ---BEGIN
         ...
         SELECT ima02,ima021 INTO sr.ima02,l_ima021 FROM ima_file    #FUN-D50099 add ima021
          WHERE ima01 = sr.qcs021
         LET sr.ima02 = sr.ima02,"  ",l_ima021   #FUN-D50099
         ...
         LET sr.l_per = sr.per USING '#&&.&&'   #FUN-CB0001
         LET sr.l_per = sr.l_per,'%'   #FUN-CB0001
         ...
         EXECUTE insert_prep USING sr.qcs01,sr.qcs02,sr.qcs05,sr.qcs021,
                                   ...
                                   sr.l_per   #FUN-CB0001
#         IF tm.ch='1' THEN
#            OUTPUT TO REPORT x610_rep1(sr.*)
#         ELSE
#            OUTPUT TO REPORT x610_rep2(sr.*)
#         END IF
#FUN-A20051 ---END
     END FOREACH
...
###XtraGrid###     LET g_str = tm.wc
     IF tm.ch='1' THEN
###XtraGrid###        CALL cl_prt_cs3('aqcx610','aqcx610',g_sql,g_str)
        LET g_xgrid.template = 'aqcx610'  #FUN-CB0001
     ELSE
###XtraGrid###        CALL cl_prt_cs3('aqcx610','aqcx610_1',g_sql,g_str)
        LET g_xgrid.template = 'aqcx610_1' #FUN-CB0001
     END IF
    LET g_xgrid.table = l_table    ###XtraGrid###
    LET g_xgrid.skippage_field = 'qcs01'   #FUN-CB0001
    CALL cl_xg_view()    ###XtraGrid###
#FUN-A20051 ---END

#FUN-A20051 ---BEGIN 
#    IF tm.ch='1' THEN
#       FINISH REPORT x610_rep1
#    ELSE
#       FINISH REPORT x610_rep2
#    END IF
#
#    IF tm.ch='1' THEN
