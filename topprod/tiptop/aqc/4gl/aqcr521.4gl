...
# Modify.........: No.FUN-580013 05/08/17 By yoyo 憑証類報表原則修改
...
# Modify.........: NO.CHI-910058 09/02/06 by shiwuying CR段調整
#                                09/07/02 by Cockroach 5.1-->5.2
 
# Modify.........: No.FUN-980030 09/08/31 By Hiko 加上GP5.2的相關設定
# Modify.........: No.TQC-950136 10/03/04 By vealxu 去掉SELECT語句後的 INTO cnt
# Modify.........: No.FUN-A50053 10/05/25 By liuxqa 追单MOD-9A0002
...
MAIN
  ...
  LET l_table = cl_prt_temptable('aqcr521',g_sql) CLIPPED   # 產生Temp Table
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW r521_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcr521"
################################################################################
...
WHILE TRUE
   WHILE TRUE
      CONSTRUCT BY NAME tm.wc ON qcm021,qcm02,qcu04
         #No.FUN-580031 --start--
         BEFORE CONSTRUCT
             CALL cl_qbe_init()
         #No.FUN-580031 ---end---
...
END CONSTRUCT
      ...
      CALL cl_err('',9046,0)
   END WHILE
   INPUT BY NAME tm.bdate,tm.edate,tm.more  WITHOUT  DEFAULTS #FUN-5B0014 add without defaults
      #No.FUN-580031 --start--
      BEFORE INPUT
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcr521',g_time,l_cmd)
      END IF
      CLOSE WINDOW r521_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
#NO.CHI-910058 start -----------------------------------------------
   DEFINE
          l_qcu05      ARRAY[10] of LIKE qcu_file.qcu05,
...
#NO.CHI-910058 END--------------------------------------------------
    ...
    #------------------------------ CR (2) ----------------------------------#
     ...
     LET l_sql = "SELECT qcm021,qcu04,qcu05,qcm01,qcm02,qcm22",
     ...
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE r521_curs1 CURSOR FOR r521_prepare1
#     CALL cl_outnam('aqcr521') RETURNING l_name       #NO.CHI-910058 mark 
#     START REPORT r521_rep TO l_name                  #NO.CHI-910058 mark 
     LET g_pageno = 0
     ...
     DELETE FROM aqcr521_tmp
 
     FOREACH r521_curs1 INTO sr1.*
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
         INSERT INTO aqcr521_tmp VALUES (sr1.*)
     ...
     LET l_i = 1
     DECLARE r521_head1 CURSOR FOR                #No.FUN-580013
      SELECT qcu04,SUM(qcu05) qcu05 FROM aqcr521_tmp
       GROUP BY qcu04
       ORDER BY qcu05 DESC
     FOREACH r521_head1 INTO g_head1[l_i].*                #No.FUN-580013
...
#NO.CHI-910058 start -----------------------------------------------
     ...
     DECLARE r521_body CURSOR FOR
#      SELECT * FROM aqcr521_tmp                        #NO.CHI-910058 mark 
#     FOREACH r521_body INTO sr.*                       #NO.CHI-910058 mark 
      SELECT DISTINCT qcm021 FROM aqcr521_tmp
     FOREACH r521_body INTO sr.qcm021
       IF STATUS THEN CALL cl_err('foreach body',STATUS,0) EXIT FOREACH END IF
 
#       OUTPUT TO REPORT r521_rep(sr.*)                 #NO.CHI-910058 mark 
        ...
        LET l_i = 1
        DECLARE r521_head2 CURSOR FOR
         SELECT qcu04,SUM(qcu05) qcu05 FROM aqcr521_tmp
          GROUP BY qcu04
          ORDER BY qcu05 DESC
        FOREACH r521_head2 INTO sr.qcu04,sr.qcu05
           IF STATUS THEN CALL cl_err('foreach head2',STATUS,0) EXIT FOREACH END IF
     
           SELECT SUM(qcu05) INTO l_amt FROM aqcr521_tmp
  ...
  #    LET amt = GROUP SUM(sr.qcm22)  #總送驗量
        ...
        DROP TABLE x
#       SELECT DISTINCT qcm01 INTO cnt FROM aqcr521_tmp #No.TQC-950136
        SELECT DISTINCT qcm01 FROM aqcr521_tmp          #No.TQC-950136
         WHERE qcm021 = sr.qcm021
         GROUP BY qcm01
          INTO TEMP x
     ...
     CALL r521_qce(g_head1[1].qcu04) RETURNING l_str1
      CALL r521_qce(g_head1[2].qcu04) RETURNING l_str2
      CALL r521_qce(g_head1[3].qcu04) RETURNING l_str3
      CALL r521_qce(g_head1[4].qcu04) RETURNING l_str4
      CALL r521_qce(g_head1[5].qcu04) RETURNING l_str5
      CALL r521_qce(g_head1[6].qcu04) RETURNING l_str6
      CALL r521_qce(g_head1[7].qcu04) RETURNING l_str7
      CALL r521_qce(g_head1[8].qcu04) RETURNING l_str8
      CALL r521_qce(g_head1[9].qcu04) RETURNING l_str9
      CALL r521_qce(g_head1[10].qcu04) RETURNING l_str10
#NO.CHI-910058 END---------------------------------------------------------
#     CALL cl_prt(l_name,g_prtway,g_copies,g_len)
   ...
   #是否列印選擇條件
   IF g_zz05 = 'Y' THEN
      CALL cl_wcchp(tm.wc,'qcm021,qcm02,qcu04')
           RETURNING tm.wc
      LET g_str = tm.wc
   END IF
...
#   FINISH REPORT r521_rep                         #NO.CHI-910058 mark
   CALL cl_prt_cs3('aqcr521','aqcr521',l_sql,g_str)
   #------------------------------ CR (4) ------------------------------#
...
#NO.CHI-910058 start -----------------------------------------------
#REPORT r521_rep(sr)
#  DEFINE l_last_sw    LIKE type_file.chr1,         #No.FUN-680104 VARCHAR(2)
...
#     LET amt = GROUP SUM(sr.qcm22)  #總送驗量
#     DROP TABLE x
#     SELECT DISTINCT qcm01 INTO cnt FROM aqcr521_tmp
#      WHERE qcm021 = sr.qcm021
#      GROUP BY qcm01
#      INTO TEMP x
#     SELECT COUNT(*) INTO cnt FROM x
...
FUNCTION r521_cre_tmp()
 
  DROP TABLE aqcr521_tmp
#No.FUN-680104-begin
  CREATE TEMP TABLE aqcr521_tmp(
    qcm021    LIKE qcm_file.qcm021,
    qcu04     LIKE qcu_file.qcu04,
    qcu05     LIKE qcu_file.qcu05,
    qcm01     LIKE qcm_file.qcm01,
