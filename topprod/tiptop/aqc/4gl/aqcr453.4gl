...
#
# Pattern name...: aqcr453.4gl
# Descriptions...: FQC不良原因分析(By 廠商)
...
# Modify.........: No.FUN-660152 06/06/27 By rainy CREATE TEMP TABLE 單號部份改為char(16)
...
#FUN-740013 - START
   ...
   LET g_sql = "sfb82.sfb_file.sfb82,",
   ...
   LET l_table = cl_prt_temptable('aqcr453',g_sql) CLIPPED   # 產生Temp Table
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW r453_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcr453"
...
END CONSTRUCT
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcr453',g_time,l_cmd)
      END IF
      CLOSE WINDOW r453_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
#FUN-740013 - END
     ...
     LET l_sql = "SELECT sfb82,qcu04,qcu05,qcf01,qcf02,qcf021,qcf22",
                 "  FROM qcf_file,qcu_file,sfb_file ",
                 " WHERE qcf01=qcu01 ",
                 "   AND qcf02 = sfb01",     #工單編號
         #       "   AND qcf14='Y' AND qcf09='2' AND qcf18='2' ",
         #       "   AND qcf04 BETWEEN '",tm.bdate,"' AND '",tm.edate,"'",
                 "   AND ", tm.wc CLIPPED
     DISPLAY "l_sql:",l_sql
 
     PREPARE r453_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
        EXIT PROGRAM
     END IF
     DECLARE r453_curs1 CURSOR FOR r453_prepare1
#    CALL cl_outnam('aqcr453') RETURNING l_name                                           #CHI-910058 Mark
#    START REPORT r453_rep TO l_name                                                      #CHI-910058 Mark
#    LET g_pageno = 0                                                                     #CHI-910058 Mark
     ...
     LET g_tot9 = 0
     LET g_tot10= 0
     DELETE FROM aqcr453_tmp
 
     FOREACH r453_curs1 INTO sr1.*
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
 
         INSERT INTO aqcr453_tmp VALUES (sr1.*)
     END FOREACH
     #------表頭前十名不良原因資料----------
     FOR l_i = 1 TO 10
     ...
     LET l_i = 1
     DECLARE r453_head1 CURSOR FOR                             #No.FUN-580013
      SELECT qcu04,SUM(qcu05) qcu05 FROM aqcr453_tmp
       GROUP BY qcu04
       ORDER BY qcu05 DESC
     FOREACH r453_head1 INTO g_head1[l_i].*
       IF STATUS THEN CALL cl_err('foreach head',STATUS,0) EXIT FOREACH END IF
       IF l_i >= 10 THEN  EXIT FOREACH END IF
       LET l_i = l_i + 1
     END FOREACH
     #------單身資料-----------------
     DECLARE r453_sfb82 CURSOR FOR                                                      #CHI-910058 Add
       SELECT DISTINCT sfb82 FROM aqcr453_tmp                                           #CHI-910058 Add 
     FOREACH r453_sfb82 INTO sr.sfb82                                                   #CHI-910058 Add
       FOR l_n = 1 TO 10                                                                #CHI-910058 Add     
           LET l_qcu05[l_n] = 0                                                         #CHI-910058 Add
       END FOR                                                                          #CHI-910058 Add     
     
     DECLARE r453_body CURSOR FOR
#     SELECT * FROM aqcr453_tmp                                                         #CHI-910058 Mark
      SELECT DISTINCT qcu04,SUM(qcu05) qcu05 FROM aqcr453_tmp                           #CHI-910058 Add
       GROUP BY qcu04                                                                   #CHI-910058 Add 
       ORDER BY qcu05 DESC                                                              #CHI-910058 Add
#    FOREACH r453_body INTO sr.*                                                        #CHI-910058 Mark
     FOREACH r453_body INTO sr.qcu04                                                    #CHI-910058 Add
       IF STATUS THEN CALL cl_err('foreach body',STATUS,0) EXIT FOREACH END IF
 
#      OUTPUT TO REPORT r453_rep(sr.*)                                                  #CHI-910058 Mark
     #CHI-910058 --Begin--#
      SELECT SUM(qcu05) INTO l_amt FROM aqcr453_tmp
       WHERE sfb82 = sr.sfb82
         AND qcu04 = sr.qcu04
      ...
      SELECT gem02 INTO l_gem02 FROM gem_file WHERE gem01 = sr.sfb82
       SELECT SUM(qcf22) INTO amt FROM aqcr453_tmp
        WHERE sfb82 = sr.sfb82        
      DROP TABLE x
#     SELECT DISTINCT qcf01 INTO cnt FROM aqcr453_tmp    #No.TQC-950136 mark
      SELECT DISTINCT qcf01 FROM aqcr453_tmp             #No.TQC-950136 add 
       WHERE sfb82 = sr.sfb82
       GROUP BY qcf01
       INTO TEMP x
     ...
     #CHI-910058 --Begin--#
      CALL r453_qce(g_head1[1].qcu04) RETURNING l_str1
      CALL r453_qce(g_head1[2].qcu04) RETURNING l_str2
      CALL r453_qce(g_head1[3].qcu04) RETURNING l_str3
      CALL r453_qce(g_head1[4].qcu04) RETURNING l_str4
      CALL r453_qce(g_head1[5].qcu04) RETURNING l_str5
      CALL r453_qce(g_head1[6].qcu04) RETURNING l_str6
      CALL r453_qce(g_head1[7].qcu04) RETURNING l_str7
      CALL r453_qce(g_head1[8].qcu04) RETURNING l_str8
      CALL r453_qce(g_head1[9].qcu04) RETURNING l_str9
      CALL r453_qce(g_head1[10].qcu04) RETURNING l_str10
      ...
      EXECUTE insert_prep USING
         sr.sfb82,    l_gem02,    sr.qcu04,   sr.qcu05,      amt,      cnt,  
         g_head1[1].qcu04,
...
#    FINISH REPORT r453_rep                                                             #CHI-910058 Mark
    ...
    CALL cl_prt_cs3('aqcr453','aqcr453',l_sql,g_str)
    #------------------------------ CR (4) ------------------------------#
    # FUN-740013 end
END FUNCTION
...
# ORDER BY sr.sfb82,sr.qcu04
# FORMAT
#  PAGE HEADER
 
#  BEFORE GROUP OF sr.sfb82
#     FOR l_n = 1 TO 10
#         LET l_qcu05[l_n] = 0
#     END FOR
...
#  AFTER GROUP OF sr.sfb82   #部門
#     SELECT gem02 INTO l_gem02 FROM gem_file WHERE gem01 = sr.sfb82
#     LET amt = GROUP SUM(sr.qcf22)  #總送驗量
#     DROP TABLE x
#     SELECT DISTINCT qcf01 INTO cnt FROM aqcr453_tmp
#      WHERE sfb82 = sr.sfb82
#      GROUP BY qcf01
#      INTO TEMP x
#     SELECT COUNT(*) INTO cnt FROM x
...
FUNCTION r453_cre_tmp()
#No.FUN-680104-begin
  CREATE TEMP TABLE aqcr453_tmp(
    sfb82     LIKE sfb_file.sfb82,
    qcu04     LIKE qcu_file.qcu04,
    qcu05     LIKE qcu_file.qcu05,
    qcf01     LIKE qcf_file.qcf01,
