...
# Modify.........: NO.CHI-910058 09/02/12 by shiwuying CR段調整
...
# Modify.........: No.FUN-A50053 10/05/25 By liuxqa 追单MOD-9A0003
...
MAIN
  ...
  LET g_sql =
   ...
   "ecm03.ecm_file.ecm03,",     #FUN-A60027
 ...
 LET l_table = cl_prt_temptable('aqcr520',g_sql) CLIPPED   # 產生Temp Table
...
WHILE TRUE
   WHILE TRUE
      CONSTRUCT BY NAME tm.wc ON ecm04,qcm021,qcu04
         #No.FUN-580031 --start--
         BEFORE CONSTRUCT
             CALL cl_qbe_init()
         #No.FUN-580031 ---end---
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcr520',g_time,l_cmd)
      END IF
      CLOSE WINDOW r520_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0085
          ...
          sr1       RECORD
                    ...
                    ecm03     LIKE ecm_file.ecm03,    #FUN-A60027 
...
#NO.CHI-910058 END--------------------------------------------------
   ...
   #------------------------------ CR (2) ----------------------------------#
     ...
     LET l_sql = "SELECT ecm04,ecm012,ecm03,ecm45,qcu04,qcu05,qcm01,qcm02,qcm021,qcm22",   #FUN-A60027 add ecm012,ecm03
                 "  FROM qcm_file,qcu_file, ecm_file, sfb_file ",
                 " WHERE qcm01=qcu01 ",
...
#     CALL cl_outnam('aqcr520') RETURNING l_name        #NO.CHI-910058 mark
#     START REPORT r520_rep TO l_name                   #NO.CHI-910058 mark
     LET g_pageno = 0
     ...
     FOREACH r520_curs1 INTO sr1.*
         ...
         INSERT INTO aqcr520_tmp VALUES (sr1.*)
     END FOREACH
     #------表頭前十名不良原因資料----------
     FOR l_i = 1 TO 10
     ...
     LET l_i = 1
     DECLARE r520_head1 CURSOR FOR                     #NO.FUN-580013
      SELECT qcu04,SUM(qcu05) qcu05 FROM aqcr520_tmp
       GROUP BY qcu04
       ORDER BY qcu05 DESC
     FOREACH r520_head1 INTO g_head1[l_i].*                     #NO.FUN-580013
     ...
     #------單身資料-----------------
#NO.CHI-910058 START--------------------------------------------------
     DECLARE r520_body CURSOR FOR
#      SELECT * FROM aqcr520_tmp                   #NO.CHI-910058 mark
#     FOREACH r520_body INTO sr.*                  #NO.CHI-910058 mark
#     SELECT DISTINCT ecm04 FROM aqcr520_tmp       #No.FUN-A60027 mark
      SELECT DISTINCT ecm04,ecm012,ecm03 FROM aqcr520_tmp   #No.FUN-A60027 
     FOREACH r520_body INTO sr.ecm04,sr.ecm012,sr.ecm03
       IF STATUS THEN CALL cl_err('foreach body',STATUS,0) EXIT FOREACH END IF
 
#       OUTPUT TO REPORT r520_rep(sr.*)            #NO.CHI-910058 mark
       FOR l_n = 1 TO 10
          LET l_qcu05[l_n] = 0
       END FOR
 
       LET l_i = 1
       DECLARE r520_head2 CURSOR FOR 
        SELECT DISTINCT qcu04,SUM(qcu05) qcu05 FROM aqcr520_tmp
         GROUP BY qcu04
         ORDER BY qcu05 DESC
       FOREACH r520_head2 INTO sr.qcu04,sr.qcu05
          IF STATUS THEN CALL cl_err('foreach head2',STATUS,0) EXIT FOREACH END IF
 
          SELECT SUM(qcu05) INTO l_amt FROM aqcr520_tmp
           WHERE ecm04 = sr.ecm04
             AND qcu04 = sr.qcu04
   ...
   #   LET amt = GROUP SUM(sr.qcm22)  #總送驗量
       ...
       DROP TABLE x
#      SELECT DISTINCT qcm01 INTO cnt FROM aqcr520_tmp  #No.TQC-950136
       SELECT DISTINCT qcm01 FROM aqcr520_tmp           #No.TQC-950136
        WHERE ecm04 = sr.ecm04
        GROUP BY qcm01
         INTO TEMP x
       ...
       ###TQC-9C0179 START ###
       LET l_ecm04 = sr.ecm04[1,6]
       LET l_ecm45 = sr.ecm45[1,16]
       ###TQC-9C0179 END ### 
       #EXECUTE insert_prep USING sr.ecm04[1,6],sr.qcu04,sr.ecm45[1,16],amt,cnt, #TQC-9C0179 mark
       EXECUTE insert_prep USING l_ecm04,sr.ecm012,sr.ecm03,sr.qcu04,l_ecm45,amt,cnt,               #TQC-9C0179   #FUN-A60027 add sr.ecm012,sr.ecm03
        l_qcu05[1], l_qcu05[2] , l_qcu05[3] , l_qcu05[4] , l_qcu05[5] ,
        l_qcu05[6] , l_qcu05[7] , l_qcu05[8] , l_qcu05[9] , l_qcu05[10]
     END FOREACH
     CALL r520_qce(g_head1[1].qcu04) RETURNING l_str1
     CALL r520_qce(g_head1[2].qcu04) RETURNING l_str2
     CALL r520_qce(g_head1[3].qcu04) RETURNING l_str3
     CALL r520_qce(g_head1[4].qcu04) RETURNING l_str4
     CALL r520_qce(g_head1[5].qcu04) RETURNING l_str5
     CALL r520_qce(g_head1[6].qcu04) RETURNING l_str6
     CALL r520_qce(g_head1[7].qcu04) RETURNING l_str7
     CALL r520_qce(g_head1[8].qcu04) RETURNING l_str8
     CALL r520_qce(g_head1[9].qcu04) RETURNING l_str9
     CALL r520_qce(g_head1[10].qcu04) RETURNING l_str10
#NO.CHI-910058 END--------------------------------------------------
  ...
  #是否列印選擇條件
  IF g_zz05 = 'Y' THEN
     CALL cl_wcchp(tm.wc,'ecm04,qcm021,qcu04')
          RETURNING tm.wc
     LET g_str = tm.wc
  END IF
...
#  FINISH REPORT r520_rep                           #NO.CHI-910058 MARK 
 #CALL cl_prt_cs3('aqcr520','aqcr520',l_sql,g_str)   #FUN-A60027
 #No.FUN-A60027 ------------------start-------------------
  IF g_sma.sma541 = 'Y' THEN
     CALL cl_prt_cs3('aqcr520','aqcr520_1',l_sql,g_str)
  ELSE
     CALL cl_prt_cs3('aqcr520','aqcr520',l_sql,g_str)
  END IF 
 #No.FUN-A60027 ------------------end-----------------------
...
#         l_amt        LIKE qcu_file.qcu05,
#         amt          LIKE qcu_file.qcu05,       #No.FUN-680104 DEC(12,3)
...
#  AFTER GROUP OF sr.ecm04   #部門
#     LET amt = GROUP SUM(sr.qcm22)  #總送驗量
#     DROP TABLE x
#     SELECT DISTINCT qcm01 INTO cnt FROM aqcr520_tmp
#      WHERE ecm04 = sr.ecm04
#      GROUP BY qcm01
#      INTO TEMP x
#     SELECT COUNT(*) INTO cnt FROM x
...
#     IF cl_null(cnt) THEN LET cnt = 0 END IF
##No.FUN-580013--start
##     PRINT COLUMN 01,sr.ecm04[1,6],
##           COLUMN 08,sr.ecm45[1,16],
##           COLUMN 26,amt USING '#########&',' ',
...
##           l_qcu05[10] USING '########'
#     PRINTX name=D1
#           COLUMN g_c[31],sr.ecm04[1,6],
#           COLUMN g_c[32],sr.ecm45[1,16],
...
#       EXECUTE insert_prep USING sr.ecm04[1,6],sr.qcu04,sr.ecm45[1,16],amt,cnt,
#       l_qcu05[1], l_qcu05[2] , l_qcu05[3] , l_qcu05[4] , l_qcu05[5] ,
#       l_qcu05[6] , l_qcu05[7] , l_qcu05[8] , l_qcu05[9] , l_qcu05[10]
...
#     CALL r520_qce(g_head1[9].qcu04) RETURNING l_str9
#     CALL r520_qce(g_head1[10].qcu04) RETURNING l_str10
#     PRINT g_head1[1].qcu04,':',l_str1,' ',g_head1[2].qcu04,':',l_str2,' ',
#           g_head1[3].qcu04,':',l_str3,' ',g_head1[4].qcu04,':',l_str4
#     PRINT g_head1[5].qcu04,':',l_str5,' ',g_head1[6].qcu04,':',l_str6,' ',
#           g_head1[7].qcu04,':',l_str7,' ',g_head1[8].qcu04,':',l_str8
#     PRINT g_head1[9].qcu04,':',l_str9,' ',g_head1[10].qcu04,':',l_str10
#     PRINT g_dash[1,g_len]
#     LET l_last_sw = 'y'
...
FUNCTION r520_cre_tmp()
#No.FUN-680104-begin
  CREATE TEMP TABLE aqcr520_tmp(
    ecm04     LIKE ecm_file.ecm04,
    ecm012    LIKE ecm_file.ecm012,    #FUN-A60027
    ecm03     LIKE ecm_file.ecm03,     #FUN-A60027
    ecm45     LIKE ecm_file.ecm45,
