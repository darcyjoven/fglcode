...
#
# Pattern name...: aqcr320.4gl
# Descriptions...: IQC不良原因分析(By 廠商)
# Date & Author..: 99/05/14 By Sophia
# Modify.........: No.FUN-4B0001 04/11/02 By Smapmin 廠商編號開窗
...
# Modify.........: No.MOD-710161 07/02/13 By pengu  送批數量欄位列印不出資料
...
# Modify.........: NO.MOD-AB0234 10/11/29 By sabrina 總送驗量計算有誤
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD
              ...
              END RECORD,
          g_head320  DYNAMIC ARRAY OF RECORD
...
#FUN-740013 - START
   ...
   LET l_table = cl_prt_temptable('aqcr320',g_sql) CLIPPED   # 產生Temp Table
...
WHILE TRUE
   WHILE TRUE
      CONSTRUCT BY NAME tm.wc ON qcs03,qcs021,qcu04
         #No.FUN-580031 --start--
         BEFORE CONSTRUCT
             CALL cl_qbe_init()
         #No.FUN-580031 ---end---
...
END CONSTRUCT
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcr320',g_time,l_cmd)
      END IF
      CLOSE WINDOW r320_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
#    CALL cl_outnam('aqcr320') RETURNING l_name        #CHI-910058 Mark               
#    START REPORT r320_rep TO l_name                   #CHI-910058 Mark
#    LET g_pageno = 0                                  #CHI-910058 Mark
     ...
     DELETE FROM aqcr320_tmp
 
     FOREACH r320_curs1 INTO l_qcs.*,l_qcu.*
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
 
         INSERT INTO aqcr320_tmp VALUES (l_qcs.qcs03,l_qcs.qcs01,l_qcs.qcs02,l_qcs.qcs05,l_qcs.qcs22,    #MOD-AB0234 add qcs02,qcs05
                                         l_qcu.qcu04,l_qcu.qcu05)
     END FOREACH
     #------表頭前十名不良原因資料----------
     FOR l_i = 1 TO 10
         INITIALIZE g_head320[l_i].* TO NULL
     END FOR
 
     LET l_i = 1
     DECLARE r320_head CURSOR FOR
      SELECT qcu04,SUM(qcu05) qcu05 FROM aqcr320_tmp
       GROUP BY qcu04
       ORDER BY qcu05 DESC
     FOREACH r320_head INTO g_head320[l_i].*
       IF STATUS THEN CALL cl_err('foreach head',STATUS,0) EXIT FOREACH END IF
       IF l_i >= 10 THEN  EXIT FOREACH END IF
       LET l_i = l_i + 1
     END FOREACH
     #------單身資料-----------------
     DECLARE r320_qcs03 CURSOR FOR                                                      #CHI-910058 Add
       SELECT DISTINCT qcs03 FROM aqcr320_tmp                                           #CHI-910058 Add 
     FOREACH r320_qcs03 INTO sr.qcs03                                                   #CHI-910058 Add
       FOR l_n = 1 TO 10                                                                #CHI-910058 Add     
           LET l_qcu05[l_n] = 0                                                         #CHI-910058 Add
       END FOR                                                                          #CHI-910058 Add     
     
     DECLARE r320_body CURSOR FOR
#      SELECT * FROM aqcr320_tmp                                                        #CHI-910058 Mark
       SELECT DISTINCT qcu04,SUM(qcu05) qcu05 FROM aqcr320_tmp                          #CHI-910058 Add
        WHERE qcs03 = sr.qcs03                                                          #CHI-910058 Add
        GROUP BY qcu04                                                                  #CHI-910058 Add 
        ORDER BY qcu05 DESC                                                             #CHI-910058 Add
 
#    FOREACH r320_body INTO sr.*                                                        #CHI-910058 Mark
     FOREACH r320_body INTO sr.qcu04                                                    #CHI-910058 Add
       IF STATUS THEN CALL cl_err('foreach body',STATUS,0) EXIT FOREACH END IF
 
#      OUTPUT TO REPORT r320_rep(sr.*)                                                  #CHI-910058 Mark
       #CHI-910058 --Begin--#                      
       SELECT SUM(qcu05) INTO l_amt FROM aqcr320_tmp
        WHERE qcs03 = sr.qcs03
          AND qcu04 = sr.qcu04
       IF cl_null(l_amt) THEN LET l_amt = 0 END IF
       
       FOR l_n = 1 TO 10                     
           IF g_head320[l_n].qcu04 = sr.qcu04 THEN
      ...
      #SELECT SUM(qcs22) INTO amt FROM aqcr320_tmp
      # WHERE qcs03 = sr.qcs03
        LET g_sql = "SELECT SUM(A.qcs22) FROM ",                                                                                    
                   #" (SELECT DISTINCT qcs03,qcs22 FROM aqcr320_tmp WHERE qcs03 = '",sr.qcs03,"' ) A"             #MOD-AB0234 mark                                
                    " (SELECT DISTINCT qcs01,qcs02,qcs03,qcs05,qcs22 FROM aqcr320_tmp WHERE qcs03 = '",sr.qcs03,"' ) A"       #MOD-AB0234 add 
        PREPARE pre_sel_sum FROM g_sql                                                                                              
        EXECUTE pre_sel_sum INTO amt       
      #TQC-910058 BY COCKROACH 
   
       DROP TABLE x
       SELECT DISTINCT qcs01,qcs02,qcs05 FROM aqcr320_tmp     #MOD-AB0234 add qcs02,qcs05
        WHERE qcs03 = sr.qcs03
        GROUP BY qcs01,qcs02,qcs05      #MOD-AB0234 add qcs02,qcs05
        INTO TEMP x
       SELECT COUNT(*) INTO cnt FROM x
       IF cl_null(amt) THEN LET amt = 0 END IF
       IF cl_null(cnt) THEN LET cnt = 0 END IF
       FOR l_n = 1 TO 10 
           IF cl_null(g_head320[l_n].qcu04) THEN
              LET g_head320[l_n].qcu04 = ' '
           END IF 
       END FOR
       #CHI-910058 --End--#
     ...
     END FOREACH
      ...
      #CHI-910058 --Begin--#
      CALL r320_qce(g_head320[1].qcu04) RETURNING l_str1
      CALL r320_qce(g_head320[2].qcu04) RETURNING l_str2
      CALL r320_qce(g_head320[3].qcu04) RETURNING l_str3
      CALL r320_qce(g_head320[4].qcu04) RETURNING l_str4
      CALL r320_qce(g_head320[5].qcu04) RETURNING l_str5
      CALL r320_qce(g_head320[6].qcu04) RETURNING l_str6
      CALL r320_qce(g_head320[7].qcu04) RETURNING l_str7
      CALL r320_qce(g_head320[8].qcu04) RETURNING l_str8
      CALL r320_qce(g_head320[9].qcu04) RETURNING l_str9
      CALL r320_qce(g_head320[10].qcu04) RETURNING l_str10
      ...
      EXECUTE insert_prep USING
         ...
         sr.qcs03,    l_pmc03,     amt,     cnt,   g_head320[1].qcu04, 
         g_head320[2].qcu04,  g_head320[3].qcu04,  g_head320[4].qcu04, 
         g_head320[5].qcu04,  g_head320[6].qcu04,  g_head320[7].qcu04,
         g_head320[8].qcu04,  g_head320[9].qcu04,  g_head320[10].qcu04,
...
#    FINISH REPORT r320_rep                       #CHI-910058 Mark
    ...
    #是否列印選擇條件
    IF g_zz05 = 'Y' THEN
       CALL cl_wcchp(tm.wc,'qcs03,qcs021,qcu04')
            RETURNING tm.wc
       LET g_str = tm.wc
    END IF
    LET g_str = g_str,";",tm.bdate,";",tm.edate
 
    CALL cl_prt_cs3('aqcr320','aqcr320',l_sql,g_str)
    #------------------------------ CR (4) ------------------------------#
    # FUN-740013 end 
...
#  AFTER GROUP OF sr.qcs03   #廠商
#     SELECT pmc03 INTO l_pmc03 FROM pmc_file WHERE pmc01 = sr.qcs03
#     LET amt = GROUP SUM(sr.qcs22)   #總送驗量
#     DROP TABLE x
#    #--------------No.MOD-710161 modify
#    #SELECT DISTINCT qcs01 INTO cnt FROM aqcr320_tmp
#     SELECT DISTINCT qcs01 FROM aqcr320_tmp
#    #--------------No.MOD-710161 end
#      WHERE qcs03 = sr.qcs03
#      GROUP BY qcs01
#      INTO TEMP x
...
FUNCTION r320_cre_tmp()
#No.FUN-680104-begin
  CREATE TEMP TABLE aqcr320_tmp(
    qcs03     LIKE qcs_file.qcs03,
    qcs01     LIKE qcs_file.qcs01,
    qcs02     LIKE qcs_file.qcs02,      #MOD-AB0234 add
    qcs05     LIKE qcs_file.qcs05,      #MOD-AB0234 add
    qcs22     LIKE qcs_file.qcs22,
    qcu04     LIKE qcu_file.qcu04,
    qcu05     LIKE qcu_file.qcu05)
#No.FUN-680104-end
