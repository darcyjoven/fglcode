...
# Modify.........: No.FUN-850054 08/05/12 By ChenMoyan 老報表轉CR
# Modify.........: No.FUN-910082 09/02/02 By ve007 wc,sql 定義為STRING
# Modify.........: No.TQC-950135 09/06/05 By chenmoyan tm.s,tm.t與畫面欄位不符
# Modify.........: No.TQC-960252 09/07/14 By baofei 修改insert_prep參數與?不符 
...
#No.FUN-850054 --Begin
   LET g_sql="qcf04.qcf_file.qcf04,",
             ...
             "code.aba_file.aba18"
   LET l_table=cl_prt_temptable('aqcr452',g_sql) CLIPPED
...
#------------No.TQC-610086 modify
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL aqcr452_tm(0,0)        # Input print condition
      ELSE CALL aqcr452()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
END MAIN
 
FUNCTION aqcr452_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW aqcr452_w AT p_row,p_col
        WITH FORM "aqc/42f/aqcr452"
...
WHILE TRUE
   CONSTRUCT BY NAME tm.wc ON qcf01,qcf03,qcf02,qcf021
         #No.FUN-580031 --start--
         BEFORE CONSTRUCT
             CALL cl_qbe_init()
         #No.FUN-580031 ---end---
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW aqcr452_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aqcr452',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW aqcr452_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690121
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0085
          ...
          #No.FUN-550063  --start
          l_order   ARRAY[4] OF LIKE cob_file.cob01,       #No.FUN-680104 VARCHAR(40) #FUN-5B0105 20->40
#No.FUN-850054 --Begin
...
#                                 qcf04  LIKE qcf_file.qcf04,
          sr               RECORD qcf04  LIKE qcf_file.qcf04,
#No.FUN-850054 --End
...
#                                 azf03  LIKE azf_file.azf03,      #No.FUN-850054
...
#No.FUN-850054 --Begin
     ...
     IF STATUS THEN                                                                                                                   
        ...
        EXIT PROGRAM                                                                             
     END IF                       
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='aqcr452'
#No.FUN-850054 --End    
     SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
     CALL cl_del_data(l_table)                                     #No.FUN-850054 
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aqcr452'   #No.FUN-850054
     #Begin:FUN-980030
...
#No.FUN-850054 --Begin
#    LET l_sql = "SELECT '','','',",
#                "       qcf04,sfb22,qcf03,qcf02,qcf01,qcf021,ima02,",
     LET l_sql = "SELECT qcf04,sfb22,qcf03,qcf02,qcf01,qcf021,ima02,",
#                "       '', oea04,occ02,'','' ",
                 "       oea04,occ02,''",
#No.FUN-850054 --End
                 ...
                 "        AND qcf09 = '2' ",
                 "        AND ima_file.ima01 = qcf_file.qcf021 ",
                 "        AND qcf02 = sfb01 ",
                 "        AND sfb22 = oea01 ",
                 "        AND oea04 = occ01 ",
     ...
     PREPARE aqcr452_prepare1 FROM l_sql
     ...
     END IF
     DECLARE aqcr452_curs1 CURSOR FOR aqcr452_prepare1
#No.FUN-850054 --Begin
#     CALL cl_outnam('aqcr452') RETURNING l_name
#    START REPORT aqcr452_rep TO l_name
#    LET g_pageno = 0
#No.FUN-850054 --End
     FOREACH aqcr452_curs1 INTO sr.*
       IF SQLCA.sqlcode THEN
...
#      OUTPUT TO REPORT aqcr452_rep(sr.*)
       EXECUTE insert_prep USING sr.qcf04,sr.sfb22,sr.qcf03,sr.qcf02,sr.qcf01,sr.qcf021,sr.ima02,sr.oea04,sr.occ02,sr.code
     END FOREACH
#    FINISH REPORT aqcr452_rep
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
     LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     IF g_zz05='Y' THEN
        CALL cl_wcchp(tm.wc,'qcf01,qcf03,qcf02,qcf021')
             RETURNING tm.wc
     ELSE
        LET tm.wc=""
     END IF 
     LET g_str=tm.wc,';',tm.bdate,';',tm.edate
     CALL cl_prt_cs3('aqcr452','aqcr452',g_sql,g_str)
#No.FUN-850054 --End
END FUNCTION
 
#No.FUN-850054 --Begin
#REPORT aqcr452_rep(sr)
...
#  	SKIP 1 LINE
##    PRINT COLUMN 10,g_x[11] CLIPPED,g_x[16] CLIPPED,'  ',g_x[13] CLIPPED
##    PRINT COLUMN 10,"-------- ---------- ---------------- ---------- ---------- --------"
#  ON EVERY ROW
