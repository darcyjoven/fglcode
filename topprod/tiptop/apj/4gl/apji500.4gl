...
#
# Pattern name...: apji500.4gl
# Descriptions...: 活動分類維護
# Date & Author..: No.FUN-790025 07/10/19 By Cockroach
...
DEFINE 
    g_pjr           DYNAMIC ARRAY OF RECORD    #
        pjr01       LIKE pjr_file.pjr01,       #分類代碼
...
#    END IF 
    ...
    OPEN WINDOW i500_w AT p_row,p_col WITH FORM "apj/42f/apji500"
      ...
      CASE g_action_choice
         ...
         WHEN "output" 
            IF cl_chk_act_auth() THEN
               ...
               LET g_msg = 'p_query "apji500" "',g_wc2 CLIPPED,'"'
...
DEFINE
    ...
    l_lock_sw       LIKE type_file.chr1,      #  VARCHAR(1)
    p_cmd           LIKE type_file.chr1,      #  VARCHAR(1)
    l_allow_insert  LIKE type_file.chr1,      #  VARCHAR(01)
    l_allow_delete  LIKE type_file.chr1       #  VARCHAR(01)
    ...
    LET g_forupd_sql = "SELECT pjr01,pjr02,pjracti FROM pjr_file WHERE pjr00='1' AND pjr01=?  FOR UPDATE"
    ...
    DECLARE i500_bcl CURSOR FROM g_forupd_sql      # LOCK CURSOR
      ...
      BEFORE INPUT                                                              
        ...
        BEFORE ROW
            ...
            IF g_rec_b>=l_ac THEN
               ...
               OPEN i500_bcl USING g_pjr_t.pjr01
               IF STATUS THEN
                  CALL cl_err("OPEN i500_bcl:", STATUS, 1)
                  LET l_lock_sw = "Y"
               ELSE   
                  FETCH i500_bcl INTO g_pjr[l_ac].* 
      ...
      AFTER INSERT
         ...
         END IF
          ...
          INSERT INTO pjr_file(pjr00,pjr01,pjr02,pjracti,
                              pjruser,pjrgrup,pjrdate,pjroriu,pjrorig)
           VALUES('1',g_pjr[l_ac].pjr01,g_pjr[l_ac].pjr02,
            ...
            IF g_pjr[l_ac].pjr01 != g_pjr_t.pjr01 OR
               (g_pjr[l_ac].pjr01 IS NOT NULL AND g_pjr_t.pjr01 IS NULL) THEN
                SELECT count(*) INTO l_n FROM pjr_file
                    WHERE pjr00 = '1' AND pjr01 = g_pjr[l_ac].pjr01
                IF l_n > 0 THEN      
                    CALL cl_err('',-239,0)
...
DEFINE p_wc2  STRING     #NO.FUN-910082 
    ...
    LET g_sql =
        "SELECT pjr01,pjr02,pjracti",
        " FROM pjr_file",
        " WHERE ", p_wc2 CLIPPED, 
        " AND pjr_file.pjr00='1'"    
    PREPARE i500_pb FROM g_sql
    DECLARE pjr_curs CURSOR FOR i500_pb
...
FUNCTION i500_set_entry(p_cmd)                                                                                                      
  DEFINE p_cmd   LIKE type_file.chr1        # VARCHAR(01)                                                                                                             #No.FUN-680103
