...
#
#Pattern name..:"apjt501.4gl"
#Descriptions..:專案多階段收款設定作業 
#Date & Author..:No.FUN-790025 07/11/20 By zhangyajun
#Modify.........:No.TQC-840018 08/04/08 By Zhangyajun 1:報表SQL修改
#                                                     2:比例範圍1-100
# Modify.........: No.CHI-8A0001 08/11/04 By tsai_yen 寫ora取代"只能使用相同群的資料"的MATCHES字串
# Modify.........: No.TQC-950123 09/06/05 By chenmoyan 402行  AND",g_wc要多加一個空格
...
# Modify.........: No.TQC-B70188 11/07/25 By lixia 錄入時程序和畫面欄位不對應
...
DEFINE  g_str           STRING
DEFINE  g_check         LIKE type_file.chr1  #判斷復制時是否按取消
...
MAIN
   ...
   END IF
    ...
    LET g_forupd_sql="SELECT * FROM pjo_file WHERE pjo01=? AND pjo02='1' FOR UPDATE "
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    DECLARE t501_cl    CURSOR FROM g_forupd_sql
    
    LET p_row = 4 LET p_col = 10
    OPEN WINDOW t501_w AT p_row,p_col WITH FORM "apj/42f/apjt501"
       ...
       ON ACTION controlp
           CASE
              WHEN INFIELD(pjp06)
                 ...
                 LET g_qryparam.state = "c"
#                 LET g_qryparam.arg1=g_pjo.pjo01    #TQC-840018
                 CALL cl_create_qry() RETURNING g_qryparam.multiret
    ...
    IF  g_wc2="1=1" THEN       
        ...
        " WHERE pjo02='1' AND ",g_wc CLIPPED, " ORDER BY pjo01"
    ELSE                                 
    LET g_sql=
        "SELECT UNIQUE pjo01",
        " FROM pjo_file,pjp_file",
        " WHERE pjo01=pjp01 AND pjo02='1' AND pjp02='1'",
        " AND ",g_wc CLIPPED," AND ",g_wc2 CLIPPED,
        " ORDER BY pjo01"
    END IF
    PREPARE t501_prepare FROM g_sql
    DECLARE t501_cs SCROLL CURSOR WITH HOLD FOR t501_prepare
    IF g_wc2=" 1=1" THEN
        LET g_sql="SELECT COUNT(*) FROM pjo_file WHERE pjo02='1' AND ",g_wc CLIPPED
    ELSE
        LET g_sql="SELECT COUNT(DISTINCT pjo01) FROM pjo_file,pjp_file WHERE",
#               " pjo01=pjp01 AND pjo02='1' AND pjp02='1' AND",g_wc CLIPPED," AND ",g_wc2 CLIPPED  #No.TQC-950123
                " pjo01=pjp01 AND pjo02='1' AND pjp02='1' AND ",g_wc CLIPPED," AND ",g_wc2 CLIPPED #No.TQC-950123
    END IF 
...
FUNCTION t501_fetch(p_flpjo)
    DEFINE
        p_flpjo         LIKE type_file.chr1           
    CASE p_flpjo
        WHEN 'N' FETCH NEXT     t501_cs INTO g_pjo.pjo01
        WHEN 'P' FETCH PREVIOUS t501_cs INTO g_pjo.pjo01
        WHEN 'F' FETCH FIRST    t501_cs INTO g_pjo.pjo01
        WHEN 'L' FETCH LAST     t501_cs INTO g_pjo.pjo01
        WHEN '/'
            IF (NOT mi_no_ask) THEN                   
               ...
               END IF
            END IF
            FETCH ABSOLUTE g_jump t501_cs INTO g_pjo.pjo01
            LET mi_no_ask = FALSE        
    END CASE
    ...
    SELECT * INTO g_pjo.* FROM pjo_file    
       WHERE pjo01 = g_pjo.pjo01 AND pjo02 = '1'
    IF SQLCA.sqlcode THEN
...
FUNCTION t501_show()
    ...
    #CALL cl_set_field_pic(g_pjo.pjoconf,"","","","",g_pjo.pjoacti)  #CHI-C80041
    IF g_pjo.pjoconf = 'X' THEN LET g_void = 'Y' ELSE LET g_void = 'N' END IF  #CHI-C80041
    CALL cl_set_field_pic(g_pjo.pjoconf,"","","",g_void,g_pjo.pjoacti)  #CHI-C80041
    CALL t501_pjo01('d')
...
DEFINE
    ...
    LET g_sql =
        ...
        " WHERE pjp02='1' AND pjp01='",g_pjo.pjo01,"'"
    IF NOT cl_null(p_wc2) THEN
      LET g_sql=g_sql CLIPPED," AND ",p_wc2 CLIPPED
   END IF
    PREPARE t501_pb FROM g_sql
    DECLARE pjp_cs CURSOR FOR t501_pb
...
FUNCTION t501_i(p_cmd)
   ...
   #INPUT BY NAME g_pjo.pjo01,g_pjo.pjooriu,g_pjo.pjoorig  #TQC-B70188 mark
   INPUT BY NAME g_pjo.pjo01                               #TQC-B70188 add
      WITHOUT DEFAULTS
...
#No.FUN-960038 --End
        ...
        LET g_forupd_sql="SELECT  pjp03,pjp04,pjp05,pjp06,'','',pjp07,pjp08,pjp09",
                        " FROM pjp_file",
                        "  WHERE pjp01=?  AND pjp03=? AND pjp02='1'",
                        " FOR UPDATE "
        LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
        DECLARE t501_bcl CURSOR FROM g_forupd_sql
        ...
        BEFORE ROW
                ...
                BEGIN WORK 
                OPEN t501_cl USING g_pjo.pjo01
                IF STATUS THEN
                ...
                IF g_rec_b>=l_ac THEN 
                        ...
                        OPEN t501_bcl USING g_pjo.pjo01,g_pjp_t.pjp03
                        IF STATUS THEN
                                CALL cl_err("OPEN t501_bcl:",STATUS,1)
                                LET l_lock_sw='Y'
                        ELSE
                                FETCH t501_bcl INTO g_pjp[l_ac].*
                                IF SQLCA.sqlcode THEN
...
#TQC-840018-add-end-
       ...
       AFTER INSERT
                ...
                VALUES(g_pjo.pjo01,'1',g_pjp[l_ac].pjp03,
      ...
      AFTER FIELD pjp03
        IF NOT cl_null(g_pjp[l_ac].pjp03) THEN 
                IF g_pjp[l_ac].pjp03!=g_pjp_t.pjp03
                        ...
                        WHERE pjp01= g_pjo.pjo01 AND pjp03=g_pjp[l_ac].pjp03 AND pjp02='1'
                       IF l_n>0 THEN
                           CALL cl_err('',-239,0)
...
#TQC-840018-mod-start-
               CASE g_pjp[l_ac].pjp07
                   WHEN '2'
...
#TQC-840018-mod-end-
        ...
        ON ROW CHANGE
           ...
           ELSE                        
              UPDATE pjp_file SET pjp03=g_pjp[l_ac].pjp03,
                 ...
                 WHERE pjp01=g_pjo.pjo01 AND pjp02 = '1'
                   AND pjp03=g_pjp_t.pjp03
    ...
    CLOSE t501_bcl
    COMMIT WORK
#   CALL t501_delall()   #CHI-C30002 mark
    CALL t501_delHeader()     #CHI-C30002 add
    CALL t501_show()
END FUNCTION                 
...
FUNCTION t501_delHeader()
   ...
   IF g_rec_b = 0 THEN
      ...
      #IF cl_confirm("9042") THEN  #CHI-C80041
         DELETE FROM pjo_file WHERE pjo01 = g_pjo.pjo01 AND pjo02 = '1'
         INITIALIZE g_pjo.* TO NULL
         CLEAR FORM
      END IF
   END IF
...
#No.FUN-960038 --End
   ...
   IF cl_delh(0,0) THEN                   
      DELETE FROM pjo_file WHERE pjo01 = g_pjo.pjo01 AND pjo02='1'
      DELETE FROM pjp_file WHERE pjp01 = g_pjo.pjo01 AND pjp02='1'
      CLEAR FORM
      CALL g_pjp.clear()
      OPEN t501_count
...
FUNCTION t501_copy()
       ...
       AFTER FIELD pjo01
          IF l_newno IS NOT NULL THEN                                          
              SELECT count(*) INTO l_cnt FROM pjo_file                          
                  WHERE pjo01 = l_newno AND pjo02 ='1'                                        
              IF l_cnt > 0 THEN                                                 
   ...
   SELECT * FROM pjo_file         
       WHERE pjo01=g_pjo01_t AND pjo02='1'
       INTO TEMP y
   ...
   SELECT * FROM pjp_file         
       WHERE pjp01=g_pjo01_t AND pjp02='1'
       INTO TEMP x
   IF SQLCA.sqlcode THEN
   ...
   SELECT pjo_file.* INTO g_pjo.* FROM pjo_file WHERE pjo01 = l_newno AND pjo02='1'
#  CALL t501_u()
   LET g_check='N'
   CALL t501_b()  
   IF g_check='Y' THEN
      DELETE FROM pjo_file WHERE pjo01=l_newno AND pjo02='1'
      IF SQLCA.sqlcode THEN   
         CALL cl_err3("del","pjp_file",l_newno,"",SQLCA.sqlcode,"","",1)        
      END IF
      DELETE FROM pjp_file WHERE pjp01=l_newno AND pjp02='1'
      IF SQLCA.sqlcode THEN   
         CALL cl_err3("del","pjp_file",l_newno,"",SQLCA.sqlcode,"","",1)        
      END IF
      CALL cl_err('','9001',0)       
   END IF
   #SELECT pjo_file.* INTO g_pjo.* FROM pjo_file WHERE pjo01 = l_oldno AND pjo02='1'  #FUN-C80046
   #CALL t501_show()  #FUN-C80046
...
#CHI-C30107 ------------- mark ------------ end
   ...
   FETCH t501_cl INTO g_pjo.*    
      ...
      RETURN
   END IF
   UPDATE pjo_file SET(pjoconf)=('Y') WHERE pjo01 = g_pjo.pjo01 AND pjo02 ='1'
   IF SQLCA.sqlcode THEN
...
#No.FUN-960038 --End
   ...
   UPDATE pjo_file SET(pjoconf)=('N') WHERE pjo01 = g_pjo.pjo01 AND pjo02='1'
   IF SQLCA.sqlcode  THEN
...
##             " FROM pjo_file left join pjn_file on pjo01=pjn01,pja_file,gen_file,gem_file,pjp_file",               #TQC-840018 mark
#             " FROM pjo_file,pjp_file left join pjn_file on pjp01=pjn01 AND pjp06=pjn02,pja_file,gen_file,gem_file",  #TQC-840018
...
##             " AND pjn_file.pjn02=pjp_file.pjp06 ",   #TQC-840018
#             " AND pjo_file.pjo02='1' AND pjp_file.pjp02='1'",                           
#             " AND ",g_wc CLIPPED," AND ",g_wc2 CLIPPED
#TQC-840018-mod
   LET g_sql="SELECT pjo01,pjoacti,pja02,pja08,pja09,pja13,gen02,gem02,",
             " pjp03,pjp04,pjp05,pjp06,pjp07,pjp08,pjp09,pjn03,pjn05",             
             " FROM ((((pjo_file left join pjp_file ON pjo01=pjp01) left join pjn_file on pjp01=pjn01 AND pjp06=pjn02)",
             " LEFT JOIN pja_file ON pjo01=pja01) LEFT JOIN gem_file ON pja09=gem01) LEFT JOIN gen_file ON pja08=gen01", 
             " WHERE pjo_file.pjo02='1' AND pjp_file.pjp02='1'",                           
             " AND ",g_wc CLIPPED," AND ",g_wc2 CLIPPED
 
   LET g_wc=g_wc CLIPPED," AND ",g_wc2 CLIPPED   #TQC-840018
   IF g_zz05 = 'Y' THEN
   ...
   CALL cl_prt_cs1('apjt501','apjt501',g_sql,l_wc)        
...
DEFINE l_chr LIKE type_file.chr1
   ...
   IF STATUS THEN
      ...
      RETURN
   END IF
   FETCH t501_cl INTO g_pjo.*          #鎖住將被更改或取消的資料
   IF SQLCA.sqlcode THEN
