...
#
# Pattern name...: apjp203.4gl
# Descriptions...: 活動預計材料消耗用維護 BOM 展開
# Date & Author..: 08/04/23 By shiwuying
# Modify.........: No.MOD-840322 08/04/23 By shiwuying apjt203增加依BOM表整批產生功能
...
FUNCTION p203(p_argv1,p_argv2)
 
   DEFINE l_time  LIKE type_file.chr8,
          #l_sql   LIKE type_file.chr1000,
          l_sql   STRING,      #NO.FUN-910082
          p_argv1 LIKE pjfb_file.pjfb01,
          p_argv2 LIKE pjfb_file.pjfb02 
 
   WHENEVER ERROR CONTINUE
   IF p_argv1 IS NULL OR p_argv1 = ' ' THEN 
      CALL cl_err(p_argv1,'apj-087',0) 
      RETURN
   END IF
   IF p_argv2 IS NULL OR p_argv2 = ' ' THEN 
      CALL cl_err(p_argv2,'apj-088',0) 
      RETURN
   END IF
   LET g_ccc=0
   LET g_pjfb1.pjfb01  = p_argv1
   LET g_pjfb1.pjfb02  = p_argv2
   LET tm.qty=0
   ...
   WHILE TRUE 
    #-->條件畫面輸入
      OPEN WINDOW p203_w AT 6,30 WITH FORM "apm/42f/apmp480" 
      ...
      END INPUT
      IF INT_FLAG THEN LET INT_FLAG=0 CLOSE WINDOW p203_w RETURN END IF
      CALL p203_init()
      CALL p203_bom()
      CLOSE WINDOW p203_w
      EXIT WHILE
...
FUNCTION p203_init()
   ...
   LET g_pjfb2.pjfb03 = 0
   LET g_pjfb2.pjfb06 = 0                                           
   LET g_pjfb2.pjfb07 = g_today 
   LET g_pjfb2.pjfbacti='Y'
END FUNCTION
...
END FUNCTION
 ...
 FUNCTION p203_bom()
   ...
   CALL p203_bom2(0,tm.part,l_ima910,tm.qty,1)
      IF g_ccc=0 THEN
         LET g_errno='asf-014'
      END IF
...
FUNCTION p203_bom2(p_level,p_key,p_key2,p_total,p_QPA)
...
#TQC-9A0187-Add-Begin
             ...
             "                 LEFT OUTER JOIN bma_file ON bmb03 = bma01 AND bmb29 = bma06 ",
             "  WHERE bmb01='",p_key,"' AND bmb02 > ?",
             "    AND bmb29='",p_key2,"'",
#TQC-9A0187-Add-End
      ...
      FOR l_i = 1 TO l_x
          IF sr[l_i].ima08='X' THEN
             IF sr[l_i].bma01 IS NOT NULL THEN 
               #CALL p203_bom2(p_level,sr[l_i].bmb03,' ',           #FUN-8B0035
                CALL p203_bom2(p_level,sr[l_i].bmb03,l_ima910[l_i], #FUN-8B0035
          ...
          IF NOT(sr[l_i].ima08='X' OR sr[l_i].ima08='M' OR
                 sr[l_i].ima08='S') THEN  
             LET g_ccc=g_ccc+1
             LET g_pjfb2.pjfb04=sr[l_i].bmb03
             LET g_pjfb2.pjfb05=sr[l_i].ima02
               
             SELECT COUNT(*) INTO l_cnt FROM pjfb_file
              WHERE pjfb01=g_pjfb1.pjfb01  AND pjfb02=g_pjfb1.pjfb02
                                           AND pjfb04=g_pjfb2.pjfb04
             IF l_cnt > 0 THEN
 
             ELSE
                LET g_pjfb2.pjfb03=g_pjfb2.pjfb03+1
                LET g_pjfb2.pjfb01=g_pjfb1.pjfb01
                LET g_pjfb2.pjfb02=g_pjfb1.pjfb02
                LET g_pjfb2.pjfboriu = g_user      #No.FUN-980030 10/01/04
                LET g_pjfb2.pjfborig = g_grup      #No.FUN-980030 10/01/04
                INSERT INTO pjfb_file VALUES(g_pjfb2.*)
                IF SQLCA.SQLCODE THEN 
