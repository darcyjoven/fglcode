...
# Modify.........: No.TQC-A50151 10/05/26 By Carrier main中结构调整 & MOD-9A0183追单
# Modify.........: No:CHI-AB0013 10/11/19 By Summer 報表的最前面加"期初現金及約當現金餘額"
...
DEFINE tm  RECORD
       ...
       g_unit         LIKE type_file.num10   #NO FUN-680145 INTEGER #TQC-840066
...
## *** CR11 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>>
   ...
   LET g_sql = " amt0.type_file.num5, ",
               " type.type_file.chr1, ",
               " item1.nml_file.nml02,",
               " amt1.nme_file.nme08, ",
...
FUNCTION r940_tm()
   ...
   OPEN WINDOW r940_w AT p_row,p_col WITH FORM "gnm/42f/gnmr940"
...
#     DEFINE     l_time LIKE type_file.chr8        #No.FUN-6A0098
     ...
     #No.FUN-720013 --END --
     DEFINE l_yy,l_mm LIKE type_file.num5 #CHI-AB0013 add
     DEFINE l_nmp09   LIKE nmp_file.nmp09 #CHI-AB0013 add
 
#     No.FUN-720013   Start CR11
      DROP TABLE r940_temp
      CREATE TEMP TABLE r940_temp(
         r940_amt0  LIKE type_file.num5,
         r940_type  LIKE type_file.chr1,
         r940_item1 LIKE nml_file.nml02,
         r940_amt1  LIKE nme_file.nme08,
         r940_azi04 LIKE azi_file.azi04)
#     No.FUN-720013   END CR11
    ...
    LET l_sql = "SELECT '1' ,'' ,nml01  ,nml02 ,nml03 ,SUM(nme08) ",
                "  FROM nml_file LEFT OUTER JOIN nme_file ",
                   ...
                   "   AND nme16 BETWEEN '",bdate,"' AND '",edate,"'",          #No.TQC-A50151
    ...
    #No.FUN-710056  --End   
     ...
     #CHI-AB0013 add --start--
     LET l_yy=YEAR(bdate) USING '&&&&' 
     LET l_mm=MONTH(bdate)
     LET l_mm=l_mm-1   
     IF l_mm=0 THEN 
        LET l_yy=l_yy-1 
        LET l_mm=12 
     ...
     ELSE
        ...
        LET l_sql = "SELECT * FROM r940_temp "
        DECLARE r940_temp_c CURSOR FROM l_sql
        FOREACH r940_temp_c INTO l_cnt, l_type, l_item1, l_amt1, l_azi04
              EXECUTE insert_prep USING l_cnt, l_type, l_item1, l_amt1, l_azi04
        END FOREACH
        #------------------------------ CR (3) ------------------------------#
        ...
        IF g_zz05 = 'Y' THEN
           ...
           #LET g_str = tm.wc
        END IF
        LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED, " Order By amt0 "
        LET g_str = g_str,";",tm.p,";",tm.d,";",l_nmp09 #CHI-AB0013 add l_nmp09
  ...
  FORMAT
   ...
   AFTER GROUP OF sr.nml01
      ...
      IF tm.c = 'Y' OR sr.amt != 0 THEN
         ...
         PRINT g_dash2
         LET sr.nml02 = '    ',sr.nml02   #FUN-750144 add
         # No.FUN-720013 07/03/01 By TSD.Ken 
         INSERT INTO r940_temp VALUES(g_cnt,sr.type,sr.nml02,l_amt,t_azi04)
         LET g_cnt = g_cnt + 1
         # No.FUN-720013 07/03/01 By TSD.Ken 
      END IF
...
#No.TQC-720034--begin
       CASE
         WHEN sr.nml03='1' OR sr.nml03='3'
            PRINT COLUMN g_c[31],g_x[18];
             LET l_tmpstr = g_x[18]
         WHEN sr.nml03='2'
            PRINT COLUMN g_c[31],g_x[17];
            LET l_tmpstr = g_x[17] 
         OTHERWISE 
            PRINT COLUMN g_c[31],g_x[sr.nml03 MOD 10 + 17];
            LET l_tmpstr = g_x[sr.nml03 MOD 10 + 17]
       END CASE
#          PRINT COLUMN g_c[31],g_x[sr.nml03 MOD 10 + 17];#No.FUN-580110
#          LET l_tmpstr = g_x[sr.nml03 MOD 10 + 17] #No.FUN-720013 
#No.TQC-720034--end
          IF sr.nml03[2,2] = '0' THEN
              ...
              PRINT g_dash2
              LET l_tmpstr = '    ',l_tmpstr   #FUN-750144 add
              # No.FUN-720013 07/03/01 By TSD.Ken #CR11
              INSERT INTO r940_temp VALUES(g_cnt,sr.type,l_tmpstr,cash_in,t_azi04)
              LET g_cnt = g_cnt + 1
              # No.FUN-720013 07/03/01 By TSD.Ken #CR11
          ELSE
              ...
              LET l_tmpstr = '    ',l_tmpstr   #FUN-750144 add
              # No.FUN-720013 07/03/01 By TSD.Ken #CR11
              INSERT INTO r940_temp VALUES(g_cnt,sr.type,l_tmpstr,cash_out,t_azi04)
              LET g_cnt = g_cnt + 1
              # No.FUN-720013 07/03/01 By TSD.Ken #CR11
          END IF
      END IF
      ...
      CASE sr.type
        WHEN '1'
             ...
             PRINT COLUMN g_c[31],g_x[10] CLIPPED,
              ...
              LET l_tmpstr = g_x[10] 
              LET l_tmpstr = '    ',l_tmpstr   #FUN-750144 add
              INSERT INTO r940_temp VALUES(g_cnt,sr.type,l_tmpstr,l_cash,t_azi04)
              LET g_cnt = g_cnt + 1
              # No.FUN-720013 07/03/01 By TSD.Ken #CR11
   ...
   ON LAST ROW
      ...
      # No.FUN-720013
      LET l_tmpstr=g_x[16]
      INSERT INTO r940_temp VALUES(g_cnt,sr.type,l_tmpstr,l_count,t_azi04)
      LET g_cnt = g_cnt + 1
      # No.FUN-720013 
      PRINT g_dash
   ...
   AFTER GROUP OF sr1.type
      ...
      CASE sr1.type
         WHEN '1'
            ...
            LET l_cash1 = cash_in1 - cash_out1
            #No.TQC-A50151  --End  
            PRINT '\"',g_x[5] CLIPPED,'\"',                    #報表編號
                        '\t',
                  '\"',g_company CLIPPED,'\"',  #編制單位
