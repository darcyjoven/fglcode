...
#
# Pattern name...: gnmp621.4gl
# Descriptions...: 應付票據月底重評價還原作業
# Date & Author..: 07/11/19 By Carrier  #No.FUN-7B0092
# Modify.........: No.FUN-980030 09/08/31 By Hiko 加上GP5.2的相關設定
# Modify.........: No:MOD-9C0072 09/12/08 By Smapmin 更新月底重評價年度月份nmz64,nmz65
...
FUNCTION p621()
   ...
   OPEN WINDOW p621 WITH FORM "gnm/42f/gnmp621" 
   ...
   WHILE TRUE
      ...
      IF g_bgjob = "Y" THEN
         ...
         IF SQLCA.sqlcode OR lc_cmd IS NULL THEN
             CALL cl_err('gnmp621','9031',1)   
         ELSE
            LET lc_cmd = lc_cmd CLIPPED,
                         " '",g_yy CLIPPED,"'",
                         " '",g_mm CLIPPED,"'",
                         " '",g_bgjob CLIPPED,"'"
            CALL cl_cmdat('gnmp621',g_time,lc_cmd CLIPPED)
         END IF
         CLOSE WINDOW p621
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-B30211
         EXIT PROGRAM
...
FUNCTION p621_t()
   ...
   DECLARE p621_del_curs1 CURSOR FOR 
     SELECT oox_file.* FROM oox_file 
      WHERE oox00 = 'NP' AND oox01 = g_yy AND oox02 = g_mm
   FOREACH p621_del_curs1 INTO l_oox.*
      IF STATUS THEN
         CALL cl_err('p621_del_curs1',STATUS,1) LET g_success='N' RETURN  
      END IF
   ...
   FOREACH upd_nmz_cs INTO l_yy,l_mm                                                                                                
      EXIT FOREACH                                                                                                                  
   END FOREACH                                                                                                                      
   UPDATE nmz_file SET nmz64 = l_yy,nmz65 = l_mm                                                                                    
    WHERE nmz00 = '0'                                                                                                               
   IF STATUS OR SQLCA.SQLERRD[3]=0 THEN
      CALL cl_err3("upd","nmz_file","","",STATUS,"","upd nmz64/65",1) 
      LET g_success='N'
   END IF
   #-----END MOD-9C0072-----
