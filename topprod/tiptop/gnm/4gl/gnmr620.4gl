...
# Modify.........: No.FUN-CC0093 13/01/21 By zhangweib 異動單號、類型增加開窗
#                                                      打印明細時，移動單號顯示為幣別，對此進行修改
...
MAIN
   ...
   LET l_table = cl_prt_temptable('gnmr620',g_sql)
   IF l_table =-1 THEN EXIT PROGRAM END IF
 
   LET l_table1 = cl_prt_temptable('gnmr620_1',g_sql)
   ...
   LET g_sql = "oox01.oox_file.oox01,",
               ...
               "oox10.oox_file.oox10,",
               "azi05.azi_file.azi05"
   LET l_table2 = cl_prt_temptable('gnmr620_2',g_sql)                              
   IF l_table2 = -1 THEN EXIT PROGRAM END IF                                      
                                                                                
   LET l_table3 = cl_prt_temptable('gnmr620_3',g_sql)                           
...
FUNCTION gnmr620_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW gnmr620_w AT p_row,p_col WITH FORM "gnm/42f/gnmr620"
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('gnmr620',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW gnmr620_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80038--add--
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#     CALL  cl_used(g_prog,g_time,1) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6A0098 #No.FUN-B80038---mark---
     ...
     PREPARE gnmr620_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare1:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80038--add--
        EXIT PROGRAM
     END IF
     DECLARE gnmr620_curs1 CURSOR FOR gnmr620_prepare1
     ...
     PREPARE gnmr620_prepare3 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare3:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80038--add--
        EXIT PROGRAM
     END IF
     DECLARE gnmr620_curs3 CURSOR FOR gnmr620_prepare3
     ...
     PREPARE gnmr620_prepare2 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare2:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80038--add-- 
        EXIT PROGRAM
     END IF
     DECLARE gnmr620_curs2 CURSOR FOR gnmr620_prepare2
     ...
     END IF
     LET l_sql = l_sql CLIPPED," GROUP BY oox05 ORDER BY oox05"
     PREPARE gnmr620_prepare4 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare4:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80038--add--
        EXIT PROGRAM
     END IF
     DECLARE gnmr620_curs4 CURSOR FOR gnmr620_prepare4
 
     CALL cl_outnam('gnmr620') RETURNING l_name
#NO.FUN-850005----BEGIN---------- 
#No.FUN-580110 --start
#    IF tm.b = '1' THEN              #統計報表
...
#NO.FUN-850005----END-----
#No.FUN-580110 --start
#        START REPORT gnmr620_rep TO l_name
 
#    LET g_pageno = 0
     IF tm.b='1' THEN
        FOREACH gnmr620_curs1 INTO sr.*
           IF SQLCA.sqlcode != 0 THEN
           ...
           IF SQLCA.sqlcode THEN
               ...
               LET t_azi07 = g_azi07
           END IF
           FOREACH gnmr620_curs3 USING sr.oox01 INTO gum.*
              ...
              EXECUTE insert_prep2 USING sr.oox01,sr.oox02,gum.oox05,
                                         gum.oox10,t_azi05_1
           END FOREACH
          #EXECUTE insert_prep USING sr.oox01,sr.oox02,sr.oox05,sr.oox05,   #No.FUN-CC0093   Mark
           EXECUTE insert_prep USING sr.oox01,sr.oox02,sr.oox03,sr.oox05,   #No.FUN-CC0093   Add
                                     sr.oox07,sr.oox08,sr.oox09,sr.oox10,
                                     t_azi07,t_azi05
           #OUTPUT TO REPORT gnmr620_rep(sr.*)
           #NO.FUN-850005-----BEGIN-------
        END FOREACH
     ELSE
        FOREACH gnmr620_curs2 INTO sr.*
           IF SQLCA.sqlcode != 0 THEN
              CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
           END IF
           SELECT azi04,azi05,azi07 INTO t_azi04,t_azi05,t_azi07 FROM azi_file  #No.CHI-6A0004
           ...
           IF SQLCA.sqlcode THEN
              ...
              LET t_azi07 = g_azi07
          END IF
          FOREACH gnmr620_curs4 USING sr.oox01,sr.oox02 INTO gum.*
             SELECT azi05 INTO t_azi05_1 FROM azi_file
                 WHERE azi01=gum.oox05
             IF SQLCA.sqlcode THEN
                LET t_azi05 = g_azi05 
             ...
             EXECUTE insert_prep3 USING sr.oox01,sr.oox02,gum.oox05,           
                                        gum.oox10,t_azi05_1
          END FOREACH
         #EXECUTE insert_prep1 USING sr.oox01,sr.oox02,sr.oox05,sr.oox05,   #No.FUN-CC0093   Mark
          EXECUTE insert_prep1 USING sr.oox01,sr.oox02,sr.oox03,sr.oox05,   #No.FUN-CC0093   Add
                                     sr.oox07,sr.oox08,sr.oox09,sr.oox10,       
                                     t_azi07,t_azi05
          #OUTPUT TO REPORT gnmr620_rep(sr.*)
        END FOREACH
     ...
     ELSE
        LET tm.wc=""
     END IF
     LET g_str = tm.wc,";",tm.b,";",tm.a
     IF tm.b = '1' THEN
        LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,"|",
                    "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED
        CALL cl_prt_cs3('gnmr620','gnmr620',g_sql,g_str)
     ELSE
        LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,"|",
                    "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED
        IF tm.a = '1' THEN
           CALL cl_prt_cs3('gnmr620','gnmr620_1',g_sql,g_str)
        ELSE
           CALL cl_prt_cs3('gnmr620','gnmr620_2',g_sql,g_str)
        END IF
     END IF
     
     #FINISH REPORT gnmr620_rep
#No.FUN-580110 -end
