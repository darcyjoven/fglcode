...
# Modify.........: No.FUN-D40020 12/12/24 By wangrr xtraGrid修改
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD                         # Print condition RECORD
	      ...
	      wc      STRING,
              a       LIKE type_file.chr1,   # Prog. Version..: '5.30.07-13.05.16(01)  # No:A071,A080又用回
...
#No.FUN-7A0036---Begin                                                          
   ...
   LET l_table = cl_prt_temptable('gnmx610',g_sql) CLIPPED                      
   IF l_table = -1 THEN
...
FUNCTION gnmx610_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW gnmx610_w AT p_row,p_col WITH FORM "gnm/42f/gnmx610"
...
#No.TQC-720032 -- end --
     ...
     IF g_bgjob = 'Y' THEN
        ...
        ELSE
           ...
           CALL cl_cmdat('gnmx610',g_time,l_cmd)    # Execute cmd at later time
        END IF
        CLOSE WINDOW gnmx610_w
        CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80038--add--
        EXIT PROGRAM
     END IF
...
#       CALL  cl_used(g_prog,g_time,1) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6A0098  #No.FUN-B80038---mark---
     ...
     DECLARE gnmx610_za_cur CURSOR FOR
             SELECT za02,za05 FROM za_file
              WHERE za01 = "gnmx610" AND za03 = g_lang
#No.FUN-580110  --begin
#    FOREACH gnmx610_za_cur INTO g_i,l_za05
#       LET g_x[g_i] = l_za05
#    END FOREACH
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'gnmx610'
#  #No.FUN-550057  --start--
...
#No.FUN-580110  --end
     ...
     PREPARE gnmx610_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare1:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80038--add--
        EXIT PROGRAM
     END IF
     DECLARE gnmx610_curs1 CURSOR FOR gnmx610_prepare1
     ...
     LET l_sql="SELECT oox05,SUM(oox10) FROM oox_file ",
               ...
               " GROUP BY oox05",
               " ORDER BY 1"
     PREPARE gnmx610_prepare3 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare3:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80038--add--
        EXIT PROGRAM
     END IF
     DECLARE gnmx610_curs3 CURSOR FOR gnmx610_prepare3
     ...
     PREPARE gnmx610_prepare2 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare2:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80038--add--
        EXIT PROGRAM
     END IF
     DECLARE gnmx610_curs2 CURSOR FOR gnmx610_prepare2
     ...
     LET l_sql="SELECT oox05,SUM(oox10) FROM oox_file ",
               ...
               " GROUP BY oox05",
               " ORDER BY 1"
     PREPARE gnmx610_prepare4 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare4:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-B80038--add--
        EXIT PROGRAM
     END IF
     DECLARE gnmx610_curs4 CURSOR FOR gnmx610_prepare4
 
    #CALL cl_outnam('gnmx610') RETURNING l_name   #No.FUN-CA0132
...
#     END IF
      IF tm.b = '1' THEN
         LET l_name ='gnmx610_1'
      ELSE
         LET l_name ='gnmx610'
      END IF
#     CALL cl_prt_pos_len()           
#     START REPORT gnmx610_rep TO l_name   
#No.FUN-580110  --end
 
#     LET g_pageno     = 0          
#No.FUN-7C0007---End
     IF tm.b = '1' THEN              #統計報表 #modify NO.A080 030616
        FOREACH gnmx610_curs1 INTO sr.*
           IF SQLCA.sqlcode != 0 THEN
...
#           OUTPUT TO REPORT gnmx610_rep(sr.*)
#No.FUN-7C0007---End
        END FOREACH
     ELSE                          #明細報表
        FOREACH gnmx610_curs2 INTO sr1.*
           IF SQLCA.sqlcode != 0 THEN
              CALL cl_err('foreach:',SQLCA.sqlcode,1)
           EXIT FOREACH
           END IF
#          OUTPUT TO REPORT gnmx610_rep1(sr1.*)
#No.FUN-7C0007---Begin
         ...
         EXECUTE insert_prep USING sr1.oox01,sr1.oox02,sr1.oox03,sr1.oox05,
                                   ...
                                   t_azi04,t_azi05,t_azi07
#           OUTPUT TO REPORT gnmx610_rep(sr1.*)    #No.FUN-580110
#No.FUN-7C0007---End 
        END FOREACH
     END IF
#No.FUN-7C0007---Begin
#     FINISH REPORT gnmx610_rep
      IF g_zz05 = 'Y' THEN                                                      
         CALL cl_wcchp(tm.wc,'oox03')                         
              RETURNING tm.wc                                                   
      END IF                                                                    
###XtraGrid###      LET g_str=tm.wc ,";",tm.a
###XtraGrid###   LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED
###XtraGrid###   CALL cl_prt_cs3('gnmx610',l_name,l_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
    LET g_xgrid.condition = cl_getmsg('lib-160',g_lang),tm.wc #FUN-D40020
    IF tm.b = '1' THEN
       LET g_xgrid.template = 'gnmx610'
       LET g_xgrid.grup_field = "oox02,oox05"  #FUN-D40020 add
    ELSE
       LET g_xgrid.template = 'gnmx610_1'
       #LET g_xgrid.skippage_field = "oox02"   #FUN-D40020 mark
       LET g_xgrid.grup_field = "oox05"        #FUN-D40020 add
    END IF
   #LET g_xgrid.grup_field = "oox02"     #FUN-D40020 mark
...
#        FOREACH gnmx610_curs4 USING sr.oox01,sr.oox02 INTO gum.*
...
#        FOREACH gnmx610_curs3 USING sr.oox01 INTO gum.*
#           SELECT azi05 INTO t_azi05 FROM azi_file                   #No.CHI-6A0004
#            WHERE azi01=gum.oox05
#           IF SQLCA.sqlcode THEN
#              LET t_azi05 = g_azi05    #使用本國幣別   #No.CHI-6A0004
