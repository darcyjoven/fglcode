...
#
# Pattern name...: aapr324.4gl
# Descriptions...: 应付余额调整明细表打印
# Date & Author..: No.FUN-B20041 11/02/17  By zhangll
# Memo...........: base on aapr320 cancel tm.a and add apf00='36'，增加转入厂商
# Modify ........: No.8263 03/09/24 By Wiky aapr324.4gl 第153行那一段AFTER INPUT應該要移到124行
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aapr324',g_sql) CLIPPED
   ...
   LET l_table1 = cl_prt_temptable('aapr3241',g_sql) CLIPPED
   IF l_table1 = -1 THEN EXIT PROGRAM END IF
 
   LET g_sql="aph01.aph_file.aph01,aph02.aph_file.aph02,aph03.aph_file.aph03,", #MOD-990210 add aph02
             "apw02.apw_file.apw02,",   #zLynn
             ...
             "aph21.aph_file.aph21,pmc03.pmc_file.pmc03,",
             "l_n2.type_file.num5"   #MOD-990210 add
   LET l_table2 = cl_prt_temptable('aapr3242',g_sql) CLIPPED
   ...
   LET l_table3 = cl_prt_temptable('aapr3243',g_sql) CLIPPED
   ...
   LET l_table4 = cl_prt_temptable('aapr3244',g_sql) CLIPPED
   ...
   LET l_table5 = cl_prt_temptable('aapr3245',g_sql) CLIPPED
...
FUNCTION r324_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   DEFINE p_row,p_col    LIKE type_file.num5,    #No.FUN-690028 SMALLINT
          l_cmd        LIKE type_file.chr1000 #No.FUN-690028 VARCHAR(400)
 
   LET p_row = 2 LET p_col = 18
   OPEN WINDOW r324_w AT p_row,p_col
     WITH FORM "aap/42f/aapr324"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
   ...
   WHILE TRUE
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aapr324',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
FUNCTION r324()
   ...
   PREPARE r324_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
      EXIT PROGRAM
   END IF
   DECLARE r324_curs1 CURSOR FOR r324_prepare1
   ...
   FOREACH r324_curs1 INTO sr.*
      ...
      DECLARE sr1_curs CURSOR FOR
         SELECT apg02,apg03,apg04,apg05f,apa08,apa13  #MOD-990210 add apg02
...
#            LET p_sql="SELECT apa08 FROM ",m_dbs CLIPPED,"apa_file ",         #No.FUN-A10098 ----------mark
      ...
      DECLARE sr2_curs CURSOR FOR
         SELECT aph02,aph03,'',aph04,aph05,aph05f,aph07,aph13,aph21   #MOD-820063  #MOD-990210 add aph02
           FROM aph_file
      ...
      FOREACH sr2_curs INTO sr2[l_idx].*
         ...
         END IF   #MOD-820063
         SELECT apw02 INTO sr2[l_idx].apw02 FROM apw_file
          WHERE apw01 = sr2[l_idx].aph03
         SELECT pmc03 INTO sr2[l_idx].pmc03 FROM pmc_file
          WHERE pmc01 = sr2[l_idx].aph21
         LET l_idx = l_idx + 1
         LET l_n = l_n + 1   #MOD-990210 add
      END FOREACH
      ...
      FOR l_idx = 1 TO 600
         ...
         ELSE
            ...
            EXECUTE insert_prep2 USING
               sr.apf01,sr2[l_idx].aph02,sr2[l_idx].aph03,sr2[l_idx].apw02,  #MOD-990210 add aph02
               sr2[l_idx].aph05,sr2[l_idx].aph04,sr2[l_idx].aph07,t_azi04,
               sr2[l_idx].aph13,sr2[l_idx].aph05f,g_azi04,  #MOD-820063
               sr2[l_idx].aph21,sr2[l_idx].pmc03,
   ...
   LET g_str=g_str,";",tm.s[1,1],";",tm.s[2,2],";",tm.t[1,1],";",
             tm.t[2,2],";",tm.u[1,1],";",tm.u[2,2],";'3';",tm.g
   ...
   CALL cl_prt_cs3('aapr324','aapr324',l_sql,g_str)
