...
DEFINE tm  RECORD
           ...
           rate_op LIKE type_file.chr1,        # Prog. Version..: '5.30.06-13.03.12(01),   #FUN-550015
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aapr165',g_sql) CLIPPED
...
FUNCTION r165_tm()
 ...
 OPEN WINDOW r165_w WITH FORM "aap/42f/aapr165"
...
WHILE TRUE
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW r165_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
      EXIT PROGRAM
   END IF
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         IF g_trace = 'Y' THEN
            ERROR l_cmd
         END IF
         CALL cl_cmdat('aapr165',g_time,l_cmd)
      END IF
      CLOSE WINDOW r165_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
      EXIT PROGRAM
...
FUNCTION r165()
 ...
 #NO.FUN-A10098   ----end---
     ...
     #-MOD-B10078-end-
      ...
      PREPARE r165_prepare1 FROM l_sql
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('prepare:',SQLCA.sqlcode,1)
         CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
         EXIT PROGRAM
      END IF
      DECLARE r165_curs1 CURSOR FOR r165_prepare1
    ...
    #NO.FUN-A10098 ----end---
                 "  WHERE apk01 = ? "
	CALL cl_replace_sqldb(l_sql) RETURNING l_sql		#FUN-920032
      PREPARE r165_prepare_apk FROM l_sql
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('prepare:',SQLCA.sqlcode,1)
         CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
         EXIT PROGRAM
      END IF
      DECLARE r165_curs_apk CURSOR FOR r165_prepare_apk
     ...
     #-MOD-B10078-end-
      PREPARE r165_prepare_azj FROM l_sql
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('prepare:',SQLCA.sqlcode,1)
         CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
         EXIT PROGRAM
      END IF
      LET l_j=1
      DECLARE r165_curs_azj CURSOR FOR r165_prepare_azj
      FOREACH r165_curs_azj INTO l_azj01[l_j],l_azj07[l_j]
...
#     LET l_apa01 = sr.apa01[1,g_doc_len]  #No.TQC-A40136  #No.TQC-A50157
      FOREACH r165_curs1 INTO sr.*
        ...
        #-MOD-BA0207-end-
         ...
         PREPARE r165_pre_apyslip FROM l_sql
#        EXECUTE r165_pre_apyslip INTO l_apyslip USING sr.apa01[1,g_doc_len]#No.TQC-A40136

         LET l_apa01 = sr.apa01[1,g_doc_len]  #No.TQC-A40136  #No.TQC-A50157
         EXECUTE r165_pre_apyslip INTO l_apyslip USING l_apa01              #No.TQC-A40136     
      ...
      IF g_apz.apz27 = 'Y' THEN
         ...
         WHILE cl_null(l_apa14)
               ...
               LET l_sql_2 = "SELECT COUNT(*) FROM oox_file",
                             ...
                             "   AND oox03 = '",sr.apa01,"'",
                             "   AND oox04 = '0'"
               PREPARE r165_prepare7 FROM l_sql_2
               DECLARE r165_oox7 CURSOR FOR r165_prepare7
               OPEN r165_oox7
               FETCH r165_oox7 INTO l_count
               CLOSE r165_oox7                       
               IF l_count = 0 THEN
            ...
            IF l_count <> 0 THEN        
               PREPARE r165_prepare07 FROM l_sql_1
               DECLARE r165_oox07 CURSOR FOR r165_prepare07
               OPEN r165_oox07
               FETCH r165_oox07 INTO l_apa14
               CLOSE r165_oox07
            END IF              
         END WHILE                       
      END IF
...
#--------------------------------------No.MOD-B70265------------------------------end
  ...
  #END FOR              #NO.FUN-A10098  ----mark----
   ...
   CALL cl_prt_cs3('aapr165','aapr165',l_sql,g_str)
