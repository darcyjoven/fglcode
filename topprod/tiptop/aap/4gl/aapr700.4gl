...
# Modify.........: No:FUN-BB0157 11/12/01 By janethuang CR與TIPTOP整合列印簽核圖片
# Modify.........: No.TQC-BC0013 12/01/16 By Elise 加入付款條件、價格條件說明，結匯應付
# Modify.........: NO.TQC-BB0145 12/02/03 By yinhy ala02，ala03開窗
...
#No.FUN-710086--begin 
   LET g_sql = "ala01.ala_file.ala01,ala02.ala_file.ala02,",
               ...
               "t_azi07.azi_file.azi07,",                          #FUN-BB0157 ADD ","
               "sign_type.type_file.chr1, sign_img.type_file.blob,", # FUN-BB0157 ADD
               "sign_show.type_file.chr1,",                             # FUN-BB0157 ADD
               "sign_str.type_file.chr1000,",                           # FUN-BB0157 ADD
               "l_ala02.pma_file.pma02,l_ala36.pnz_file.pnz02"   #TQC-BC0013
 
   LET l_table = cl_prt_temptable('aapr700',g_sql) CLIPPED   # 產生Temp Table
   IF l_table = -1 THEN EXIT PROGRAM END IF 
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED, 
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?)"   #TQC-750033 add ?   #modify TQC-AB0163 add ? FUN-BB0157 add ?  #TQC-BC0013 add ?,? 
...
#No.FUN-710086--end 
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'   # If background job sw is off
      ...
      ELSE CALL r700()                 # Read data and create out-file
   END IF
...
FUNCTION r700_tm(p_row,p_col)
   ...
   OPEN WINDOW r700_w AT p_row,p_col
     WITH FORM "aap/42f/aapr700"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
...
WHILE TRUE
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aapr700',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW r700_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
      EXIT PROGRAM
...
FUNCTION r700()
   DEFINE l_img_blob     LIKE type_file.blob      #FUN-BB0157 ADD
#   DEFINE l_name    LIKE type_file.chr20,         # External(Disk) file name  #No.FUN-690028 VARCHAR(20) #FUN-BB0157 mark
#         l_time    LIKE type_file.chr8,          # Used time for running the job  #No.FUN-690028 VARCHAR(8)
   DEFINE l_sql     LIKE type_file.chr1000,       # RDSQL STATEMENT  #No.FUN-690028 VARCHAR(1200)
...
#No.FUN-710086--begin
   ...
   DEFINE l_pmc03_1 LIKE pmc_file.pmc03          #出貨廠商名稱   #TQC-750033 add
     ...
     LOCATE l_img_blob IN MEMORY   #blob初始化  FUN-BB0157
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aapr700'
#No.FUN-710086--end
...
#      CALL  cl_used(g_prog,g_time,1) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6A0055
     ...
     LET l_sql = "SELECT ",
                ...
                #" alb80,alb81,alb82,alb83,alb84,alb85,alb86,alb87,pmn20 ",           #FUN-710029  #FUN-A60056
                 " alb80,alb81,alb82,alb83,alb84,alb85,alb86,alb87,'',ala97 ",              #FUN-A60056
    ...
    #CALL cl_replace_sqldb(l_sql) RETURNING l_sql   #FUN-A60056    #FUN-A70139
     ...
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE r700_curs1 CURSOR FOR r700_prepare1

#    CALL cl_outnam('aapr700') RETURNING l_name    #No.FUN-710086
...
#     START REPORT r700_rep TO l_name
#     LET g_pageno = 0
#No.FUN-710086--end

 
     FOREACH r700_curs1 INTO sr.*
          IF SQLCA.sqlcode != 0 THEN
...
#          IF cl_null(sr.azi05) THEN LET sr.azi05 = 0 END IF
         ...
         # LET g_cr_gcx01 = "aapr700"              #單別維護程式
          LET g_cr_apr_key_f = "ala01"           #報表主鍵欄位名稱，用"|"隔開
          #FUN-BB0157 add end ------
          #str TQC-750033 add
          ...
          EXECUTE insert_prep USING sr.*,l_pmc03_1,l_str2,l_flag,t_azi07,"",l_img_blob,"N","",l_ala02,l_ala36  #TQC-750033 add l_pmc03_1 #FUN-BB0157 ADD ,"",l_img_blob,"N",""
                                                                                                               #TQC-BC0013 add l_ala02, l_ala36
#         OUTPUT TO REPORT r700_rep(sr.*)
#No.FUN-710086--end
     END FOREACH   #FUN-BB0157 MARK
#No.FUN-710086--begin
#     FINISH REPORT r700_rep
#     CALL cl_prt(l_name,g_prtway,g_copies,g_len)
   ...
   # LET g_sql = "SELECT * FROM ",l_table CLIPPED          # TQC-730088
     ...
     LET g_str = tm.wc         
   # CALL cl_prt_cs3('aapr700',g_sql,g_str)                # TQC-730088 
     CALL cl_prt_cs3('aapr700','aapr700',g_sql,g_str)  
