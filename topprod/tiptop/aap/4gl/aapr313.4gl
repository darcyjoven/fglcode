...
# Modify.........: No.TQC-C10034 12/01/18 By zhuhao 報表列印簽核欄
DATABASE ds
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aapr313',g_sql) CLIPPED 
   ...
   LET l_table1 = cl_prt_temptable('aapr3131',g_sql) CLIPPED 
   ...
   LET l_table2 = cl_prt_temptable('aapr3132',g_sql) CLIPPED 
   IF l_table2 = -1 THEN EXIT PROGRAM END IF 
 
   LET g_sql = "apd01.apd_file.apd01,",    
               "apd02.apd_file.apd02,",   #MOD-7A0137 add    
               "apd03.apd_file.apd03"    
   LET l_table3 = cl_prt_temptable('aapr3133',g_sql) CLIPPED 
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'   # If background job sw is off
      THEN CALL r313_tm(0,0)        # Input print condition
      ELSE CALL r313()              # Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01,    #No.FUN-580031
   ...
   OPEN WINDOW r313_w AT p_row,p_col WITH FORM "aap/42f/aapr313"
   ...
   WHILE TRUE
      ...
      END IF
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW r313_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aapr313',g_time,l_cmd)    #Execute cmd at later time
         END IF
         CLOSE WINDOW r313_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
         EXIT PROGRAM
...
#TQC-C10034--mark--end
 DEFINE l_plant     LIKE type_file.chr10       #FUN-980020
   ...
   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aapr313'
...
#No.FUN-A10098 -----mark end
   ...
   LET l_sql = " SELECT aph_file.aph01, aph_file.aph02,",
               ...
               "        aag02,nma02 ",
               "   FROM aph_file LEFT OUTER JOIN aag_file ON aag01 = aph04 AND aag_file.aag00 = '",g_bookno1,"' LEFT OUTER JOIN nma_file ON nma01 = aph08 ", 
...
#                   " FROM  ",g_dbs_new CLIPPED," apk_file ",        #No.FUN-A10098 -----mark
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
         PREPARE r313_premisc FROM l_sql
         DECLARE r313_misc CURSOR FOR r313_premisc
...
#FUN-A60056--mod--end
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
         ...
         PREPARE r313_ppay FROM l_sql
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('prepare:',SQLCA.sqlcode,1)
            CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
            EXIT PROGRAM
         END IF
         DECLARE r313_cpay CURSOR FOR r313_ppay
         FOREACH r313_cpay INTO sr12.*,l_azi03,l_azi04,l_azi05
            IF SQLCA.sqlcode THEN
               CALL cl_err('foreac r313_cpay',SQLCA.sqlcode,0)
               EXIT FOREACH
            END IF
            ...
            IF sr12.apa08 = 'MISC' THEN   #表示是多發票,需抓apk_file
               LET l_flag =0
               FOREACH r313_misc USING srapg.apg04   #MOD-7A0137
      ...
      IF tm.c = 'Y' THEN
         ...
         END FOREACH
      END IF
      CALL r313_upd(sr1.apf01)       #更改"列印次數"
   END FOREACH
