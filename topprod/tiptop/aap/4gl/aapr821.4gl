...
#
# Pattern name...: aapr821.4gl
# Descriptions...: 信用狀償還預估彙總表列印
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD
                 ...
                 n       LIKE type_file.chr1,        # No.FUN-690028  VARCHAR(1),
...
MAIN
   ...
   LET tm.t  = ARG_VAL(9)
   LET tm.u  = ARG_VAL(10)
   LET tm.n  = ARG_VAL(11)   #TQC-610053
   #No.FUN-570264 --start--
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW r821_w AT p_row,p_col
     WITH FORM "aap/42f/aapr821"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
   ...
   WHILE TRUE
      ...
      CONSTRUCT BY NAME tm.wc ON alh06,alh05,alh11,alh08,alh01,alh10,alh04
      ...
      INPUT BY NAME tm2.s1,tm2.s2,tm2.s3,tm2.t1,tm2.t2,tm2.t3,
                    tm2.u1,tm2.u2,tm2.u3,tm.n,tm.more WITHOUT DEFAULTS
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                        ...
                        " '",tm.t CLIPPED,"'",
                        " '",tm.u CLIPPED,"'",
                        " '",tm.n CLIPPED,"'",   #TQC-610053
                         " '",g_rep_user CLIPPED,"'",           #No.FUN-570264
                         " '",g_rep_clas CLIPPED,"'",           #No.FUN-570264
                         " '",g_template CLIPPED,"'"            #No.FUN-570264
            CALL cl_cmdat('aapr821',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#    CALL  cl_used(g_prog,g_time,1) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6A0055
   ...
   LET l_sql = "SELECT '','','', ",
               " alh_file.*, alg_file.*, pmc03, ala79, 0",
               " FROM alh_file,OUTER alg_file,OUTER pmc_file,OUTER ala_file",
                " WHERE alh_file.alh06 = alg_file.alg01 AND alh_file.alh05=pmc_file.pmc01 AND alh_file.alh03=ala_file.ala01",
               "   AND alh14>alh76",
               "   AND ", tm.wc CLIPPED
   ...
   IF SQLCA.sqlcode != 0 THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE r821_curs1 CURSOR FOR r821_prepare1
 
   CALL cl_outnam('aapr821') RETURNING l_name
   START REPORT r821_rep TO l_name
 
   LET g_pageno = 0
 
   FOREACH r821_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
      ...
      IF tm.n='Y' THEN
         LET sr.alh.alh06 = sr.alh.alh05
         LET sr.alg.alg02 = sr.pmc03
      END IF
 
      IF sr.ala79 IS NULL THEN
         LET sr.ala79 = 0
      END IF
 
      IF sr.alh.alh00 = '2' THEN
         SELECT alh12,alh14 INTO l_alh12,l_alh14
           FROM alh_file
          WHERE alh01=sr.alh.alh30
      ELSE
         LET l_alh12 = sr.alh.alh12
         LET l_alh14 = sr.alh.alh14
      END IF
 
      LET sr.used_amt = l_alh14*sr.alh.alh18
      LET sr.alh.alh14=sr.alh.alh14-sr.alh.alh76
      LET sr.alh.alh16=sr.alh.alh16-sr.alh.alh77
 
      FOR g_i = 1 TO 3
         CASE WHEN tm.s[g_i,g_i] = '1' LET l_order[g_i] = sr.alh.alh06
                                       LET g_orderA[g_i]= g_x[10]
              WHEN tm.s[g_i,g_i] = '2' LET l_order[g_i] = sr.alh.alh05
                                       LET g_orderA[g_i]= g_x[11]
              WHEN tm.s[g_i,g_i] = '3' LET l_order[g_i] = sr.alh.alh11
                                       LET g_orderA[g_i]= g_x[12]
              WHEN tm.s[g_i,g_i] = '4' LET l_order[g_i] = sr.alh.alh08 USING 'YYYYMMDD'
...
REPORT r821_rep(sr)
   ...
   ORDER BY sr.order1,sr.alh.alh11,sr.order2,sr.order3,
            sr.alh.alh06,sr.alh.alh05,sr.alh.alh11,sr.alh.alh08
...
#          INTO l_azi03,l_azi04,l_azi05   #NO.CHI-6A0004
         ...
         PRINT COLUMN g_c[31],sr.alh.alh06,
               COLUMN g_c[32],sr.alg.alg02,
               COLUMN g_c[33],sr.alh.alh11,
               COLUMN g_c[34],sr.alh.alh08,
#              COLUMN g_c[35],cl_numfor(sr.alh.alh14,35,l_azi04),   #NO.CHI-6A0004
               COLUMN g_c[35],cl_numfor(sr.alh.alh14,35,t_azi04),   #NO.CHI-6A0004
               COLUMN g_c[36],cl_numfor(sr.alh.alh16,36,g_azi04),
               COLUMN g_c[37],cl_numfor(sr.used_amt,37,g_azi04)
 
      AFTER GROUP OF sr.alh.alh11
         IF tm.u[1,1] = 'Y' THEN
#            PRINT COLUMN g_c[35],"---------------- "   #TQC-5B0043
            PRINT COLUMN g_c[35],"------------------ "   #TQC-5B0043
            PRINT COLUMN g_c[33],g_x[16] CLIPPED,
                  COLUMN g_c[34],sr.alh.alh11,
#                 COLUMN g_c[35],cl_numfor(GROUP SUM(sr.alh.alh14),35,l_azi05)   #NO.CHI-6A0004
                  COLUMN g_c[35],cl_numfor(GROUP SUM(sr.alh.alh14),35,t_azi05)   #NO.CHI-6A0004
            PRINT
         END IF
      ...
      AFTER GROUP OF sr.order1
         IF tm.u[1,1] = 'Y' THEN
            PRINT g_dash2
            PRINT COLUMN g_c[31],g_orderA[1] CLIPPED,
                  COLUMN g_c[32],sr.order1 CLIPPED,
                  COLUMN g_c[33],g_x[16] CLIPPED,
                  COLUMN g_c[36],cl_numfor(GROUP SUM(sr.alh.alh16),36,g_azi05),
                  COLUMN g_c[37],cl_numfor(GROUP SUM(sr.used_amt ),37,g_azi05)
            PRINT
         END IF
 
      ON LAST ROW
#            PRINT COLUMN g_c[33],g_dash2[40,g_len]   #TQC-5B0043
            PRINT COLUMN g_c[33],g_dash2   #TQC-5B0043    #TQC-6A0081
            PRINT COLUMN g_c[33],g_x[17] CLIPPED,
                  COLUMN g_c[36],cl_numfor(SUM(sr.alh.alh16),36,g_azi05),
                  COLUMN g_c[37],cl_numfor(SUM(sr.used_amt ),37,g_azi05)
         ...
         PRINT g_dash[1,g_len]
         LET l_last_sw = 'y'
  #      PRINT g_x[4],g_x[5] CLIPPED,COLUMN g_c[37],g_x[7] CLIPPED   #TQC-6A0081
         PRINT g_x[4],g_x[5] CLIPPED,COLUMN g_len-9,g_x[7] CLIPPED   #TQC-6A0081
 
      PAGE TRAILER
         IF l_last_sw = 'n' THEN
            PRINT g_dash[1,g_len]
  #         PRINT g_x[4],g_x[5] CLIPPED,COLUMN g_c[37],g_x[6] CLIPPED    #TQC-6A0081
