...
# Modify.........: No:TQC-BC0013 12/02/10 By Elise 加入價格條件說明
...
#No.FUN-710086--begin 
   LET g_sql = "ala01.ala_file.ala01,",  
               ...
               "ala97.ala_file.ala97,",       #TQC-BC0013 add
               ...
               #TQC-C10034---add---end
               "l_ala36.pnz_file.pnz02"       #TQC-BC0013 add l_ala36
 
   LET l_table = cl_prt_temptable('aapr701',g_sql) CLIPPED 
   ...
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED, 
               ...
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?)"   #TQC-C10034 add 4?    #TQC-BC0013 add 2 ?          
...
#No.FUN-710086--end 
   ...
   CALL cl_used(g_prog,g_time,1) RETURNING g_time  #FUN-B80105    ADD
...
WHILE TRUE
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aapr701',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW r701_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
      EXIT PROGRAM
...
#         l_time          LIKE type_file.chr8        #No.FUN-6A0055
   ...
   sr         RECORD              alc01 LIKE alc_file.alc01,
                        ...
                        END RECORD
   DEFINE l_ala36 LIKE pnz_file.pnz02  #TQC-BC0013 add
#No.FUN-710086--begin
   ...
   #TQC-C10034--add--end
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'aapr701'
#No.FUN-710086--end      
...
#     CALL  cl_used(g_prog,g_time,1) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6A0055    #FUN-B80105  MARK
     ...
     LET l_sql = "SELECT ",
                ...
                #" FROM alc_file,OUTER (all_file, OUTER pmn_file),",   #FUN-A60056
                 " FROM alc_file,OUTER all_file, ",                    #FUN-A60056
                 "      ala_file",   #FUN-850002
                 " WHERE alc_file.alc01=all_file.all01 ",
                 " AND alc_file.alc02=all_file.all00 ",
     ...
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE r701_curs1 CURSOR FOR r701_prepare1
#No.FUN-710086--begin
#     CALL cl_outnam('aapr701') RETURNING l_name
...
#No.FUN-710086--begin
      ...
      SELECT pnz02 INTO l_ala36 FROM pnz_file
       WHERE pnz01 = sr_1.ala36
      #TQC-BC0013---add---end---

      EXECUTE insert_prep USING  sr_1.*,sr.*,l_pmc03,l_str2,l_flag,g_azi04,
                                 "",  l_img_blob,   "N","" ,l_ala36          #TQC-C10034  add      #TQC-BC0013 add l_ala36 
...
#     CALL cl_prt(l_name,g_prtway,g_copies,g_len)
   ...
   # LET g_sql = "SELECT * FROM ",l_table CLIPPED  #TQC-730088
     ...
     LET g_str = tm.wc        
   # CALL cl_prt_cs3('aapr701',g_sql,g_str)  #TQC-730088
     #TQC-C10034--add--begin
     LET g_cr_table = l_table
     LET g_cr_apr_key_f = "alc01|alc02" 
     #TQC-C10034--add--end
     CALL cl_prt_cs3('aapr701','aapr701',g_sql,g_str)  
