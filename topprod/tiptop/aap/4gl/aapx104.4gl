...
# Modify.........: No.MOD-880150 08/08/21 By Sarah aapx104_aqc_prep CURSOR的SQL,條件apb08=apb081取消,將apb03=aqc03改為apb02=aqc03
...
# Modify.........: No.MOD-BA0019 11/10/11 By Polly aapx104_aqc_prep排除作廢的單據aqaconf <> 'X'，不能納入計算
...
# Modify.........: No.FUN-D30059 13/03/19 By yangtt CR報表轉XGrid                     
...
END GLOBALS
   ...
   DEFINE tm  RECORD
             ...
             #t,u     LIKE type_file.chr3,    # No.FUN-690028  #FUN-D30070 mark
              t       LIKE type_file.chr3,    # FUN-D30070 add 
...
MAIN
   ...
   LET g_sql = "rvu00.rvu_file.rvu00,  rvu01.rvu_file.rvu01,",
               ...
               "rvu00_desc.type_file.chr100,  l_n1.type_file.num5,",   #FUN-D30059
               "t_azi04.azi_file.azi04, t_azi05.azi_file.azi05"   #FUN-D30059
   LET l_table = cl_prt_temptable('aapx104',g_sql) CLIPPED
   IF l_table = -1 THEN EXIT PROGRAM END IF
 
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?)"   #FUN-750117 add ? #MOD-9A0042 add ?? #CHI-B30020 add ?  #FUN-D30059 add 4?
 ...
 # LET g_rep_user = ARG_VAL(15)  #FUN-D70020 mark
 # LET g_rep_clas = ARG_VAL(16)  #FUN-D70020 mark
 # LET g_template = ARG_VAL(17)  #FUN-D70020 mark
 # LET g_rpt_name = ARG_VAL(18)  #No.FUN-7C0078  #FUN-D70020 mark
   LET g_rep_user = ARG_VAL(14)  #FUN-D70020 add
   LET g_rep_clas = ARG_VAL(15)  #FUN-D70020 add
   LET g_template = ARG_VAL(16)  #FUN-D70020 add
   LET g_rpt_name = ARG_VAL(17)  #No.FUN-7C0078  #FUN-D70020 add
  ...
  CREATE TEMP TABLE curr_tmp(
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL aapx104_tm(0,0)                     # Input print condition
   ELSE
      CALL aapx104()                           # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
END MAIN
 
FUNCTION aapx104_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW aapx104_w AT p_row,p_col
     WITH FORM "aap/42f/aapx104"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
   ...
   LET tm2.t2   = tm.t[2,2]
   LET tm2.t3   = tm.t[3,3]
 # LET tm2.u1   = tm.u[1,1]  #FUN-D70020 mark
 # LET tm2.u2   = tm.u[2,2]  #FUN-D70020 mark
 # LET tm2.u3   = tm.u[3,3]  #FUN-D70020 mark
...
#  IF cl_null(tm2.u1) THEN LET tm2.u1 = "N" END IF  #FUN-D70020 mark
#  IF cl_null(tm2.u2) THEN LET tm2.u2 = "N" END IF  #FUN-D70020 mark
#  IF cl_null(tm2.u3) THEN LET tm2.u3 = "N" END IF  #FUN-D70020 mark
...
#No.FUN-A10098 -----mark end
      ...
      INPUT BY NAME tm.misc,tm.amt_sw,tm.LC_sw,tm.allo_sw,   #FUN-750117 add tm.misc,tm.allo_sw
                   ...
                   #tm2.u1,tm2.u2,tm2.u3,tm.more WITHOUT DEFAULTS  #FUN-D70020 mark
                    tm.more WITHOUT DEFAULTS   #FUN-D70020 mark
         ...
         AFTER INPUT
          ...
          # LET tm.u = tm2.u1,tm2.u2,tm2.u3  #FUN-D70020 mark
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_rlang CLIPPED,"'", #No.FUN-7C0078
                            ...
                            " '",tm.s CLIPPED,"'" ,
                            " '",tm.t CLIPPED,"'" ,
                          # " '",tm.u CLIPPED,"'" ,  #FUN-D70020 mark
                         " '",g_rep_user CLIPPED,"'",           #No.FUN-570264
                         " '",g_rep_clas CLIPPED,"'",           #No.FUN-570264
                         " '",g_template CLIPPED,"'",           #No.FUN-570264
                         " '",g_rpt_name CLIPPED,"'"            #No.FUN-7C0078
            CALL cl_cmdat('aapx104',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
FUNCTION aapx104()
   ...
   DEFINE l_aqc_amt_title    STRING                #FUN-D30059 add
   DEFINE l_rvu00_desc       LIKE type_file.chr100 #FUN-D30059 add
...
#FUN-A60056--mod--end
      ...
      PREPARE aapx104_prepare1 FROM l_sql
      IF STATUS THEN
         CALL cl_err('prepare:',STATUS,1)
         CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
         EXIT PROGRAM
      END IF
      DECLARE aapx104_curs1 CURSOR FOR aapx104_prepare1
 
      LET g_pageno = 0
      FOREACH aapx104_curs1 INTO rvu.*,rvv.*,sr.*
...
#No.FUN-A10098 -----add end
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
         PREPARE aapx104_aqc_prep FROM l_sql
         IF STATUS THEN
            CALL cl_err('aapx104_aqc_prep:',STATUS,1)
            CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
            EXIT PROGRAM
         END IF
         DECLARE aapx104_aqc_cs CURSOR FOR aapx104_aqc_prep
 
         LET l_flag='N'
         #FUN-D30059----add---str--
         CASE rvu.rvu00 
            WHEN '1' LET l_rvu00_desc = "1.",cl_getmsg('apm-243',g_lang)
            WHEN '2' LET l_rvu00_desc = "2.",cl_getmsg('apm-244',g_lang)
            WHEN '3' LET l_rvu00_desc = "3.",cl_getmsg('apm-245',g_lang)
         END CASE

         LET l_n1 = 2
         #FUN-D30059----add---end--
         FOREACH aapx104_aqc_cs INTO l_aqc06,l_aqc08
            LET l_flag='Y'
            ...
            EXECUTE insert_prep USING 
              ...
              #rvv.rvv87, rvv.rvv86, rvv.rvv38,  rvv.rvv39, l_aqc_amt,   #FUN-750117 add l_aqc_amt #CHI-B30020 mark
               ...
               l_rvu00_desc, l_n1, g_azi04, g_azi05   #FUN-D30059
         END FOREACH
...
###XtraGrid###      LET g_str = tm.wc,";",g_zz05,";",t_azi03,";",g_azi04,";",g_azi05,";",     #CHI-B30020
###XtraGrid###                  tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",tm.t,";",tm.u,";",
###XtraGrid###                  tm.allo_sw   #FUN-750117 add
 
      IF g_sma115 = "Y" THEN
         LET l_name = "aapx104"
      ELSE
         LET l_name = "aapx104_1"
      END IF
###XtraGrid###      CALL cl_prt_cs3('aapx104',l_name,g_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
    #FUN-D30059----add--str--
    LET g_xgrid.template = l_name
    CASE tm.allo_sw 
       WHEN '1' LET l_aqc_amt_title = cl_getmsg('aap-783',g_lang)
       WHEN '2' LET l_aqc_amt_title = cl_getmsg('aap-784',g_lang)
    END CASE
    LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'aqc_amt',l_aqc_amt_title,'')
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"rvu00,rvu01,rvu03,rvu04,rvv31")
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"rvu00,rvu01,rvu03,rvu04,rvv31")  
    IF cl_null(g_xgrid.grup_field) THEN
       LET g_xgrid.grup_field = "pmm22"
    ELSE
       LET g_xgrid.grup_field = g_xgrid.grup_field,",pmm22"
    END IF
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"rvu00,rvu01,rvu03,rvu04,rvv31")
#   LET g_xgrid.grup_sum_field = cl_get_sum_field(tm.s,tm.u,"rvu00,rvu01,rvu03,rvu04,rvv31")  #FUN-D70020 mark
    LET l_str = cl_wcchp(g_xgrid.order_field,"rvu00,rvu01,rvu03,rvu04,rvv31")
