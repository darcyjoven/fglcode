...
MAIN
   ...
   LET g_sql = " apa01.apa_file.apa01,",   #帳款編號
               ...
               " apb21a.type_file.chr21,",
               " apb06a.type_file.chr21 "
   LET l_table = cl_prt_temptable('aapr107',g_sql) CLIPPED
   IF l_table = -1 THEN 
      EXIT PROGRAM
   END IF
...
FUNCTION aapr107_tm(p_row,p_col)
   DEFINE p_row,p_col    LIKE type_file.num5,    #No.FUN-690028 SMALLINT
          l_cmd        LIKE type_file.chr1000 #No.FUN-690028 VARCHAR(400)
 
   LET p_row = 3 LET p_col = 13
   OPEN WINDOW aapr107_w AT p_row,p_col
     WITH FORM "aap/42f/aapr107"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
   ...
   WHILE TRUE
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aapr107',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#    CALL  cl_used(g_prog,g_time,1) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6A0055
   ...
   PREPARE aapr107_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80105    ADD
      EXIT PROGRAM
   END IF
   DECLARE aapr107_curs1 CURSOR FOR aapr107_prepare1
 
   #No.FUN-750129  --Begin
#  CALL cl_outnam('aapr107') RETURNING l_name
#  START REPORT aapr107_rep TO l_name
   #No.FUN-750129  --End
 
   FOREACH aapr107_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
...
#        PREPARE r107_p2 FROM l_sql
#        OPEN r107_p2
#        FETCH r107_p2 INTO sr.apb12,sr.pmn31
#     END IF
...
#FUN-A60056--mod--end
      ...
      #No.FUN-750129  --Begin
#     OUTPUT TO REPORT aapr107_rep(sr.*)
   ...
   #  CALL  cl_used(g_prog,g_time,2) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6A0055
    ...
    CALL cl_prt_cs3('aapr107','aapr107',g_sql,g_str)
   #No.FUN-750129  --End
