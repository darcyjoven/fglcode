...
#                                                 aapg111.rpt單身資料里的部門名稱改顯示l_gem02
...
# Modify.........: No.TQC-DA0038 13/10/24 By yangxf “賬款編號”，“賬款人員”，“賬款部門”，“賬款類型”，“付款廠商”欄位添加开窗
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD                  # Print condition RECORD
             #wc      LIKE type_file.chr1000,     # Where condition  #No.FUN-690028 VARCHAR(700)                  #TQC-DA0038 mark
              wc      STRING,                                                                                     #TQC-DA0038 add
...
MAIN
   ...
   LET l_table = cl_prt_temptable('aapg111',g_sql) CLIPPED
   IF l_table = -1 THEN 
   ...
   LET l_table1 = cl_prt_temptable('aapg1111',g_sql) CLIPPED
   IF l_table1 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-BB0047 mark 
   ...
   LET l_table2 = cl_prt_temptable('aapg1112',g_sql) CLIPPED
   IF l_table2 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-BB0047 mark 
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)#FUN-C10036 mark
      EXIT PROGRAM 
   ...
   LET l_table3 = cl_prt_temptable('aapg1113',g_sql) CLIPPED
   IF l_table3 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-BB0047 mark 
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)#FUN-C10036 mark
      EXIT PROGRAM 
   ...
   LET l_table4 = cl_prt_temptable('aapg1114',g_sql) CLIPPED
   IF l_table4 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time  #No.FUN-BB0047 mark 
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)#FUN-C10036 mark
      EXIT PROGRAM 
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW g111_w AT p_row,p_col WITH FORM "aap/42f/aapg111"
...
END CONSTRUCT
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aapg111',g_time,l_cmd) # Execute cmd at later time
      END IF
      CLOSE WINDOW g111_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B40097
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)
...
FUNCTION g111()
   ...
   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aapg111'
...
###GENGRE###     CALL cl_prt_cs3('aapg111','aapg111',l_sql,g_str)
    CALL aapg111_grdata()    ###GENGRE###
...
FUNCTION aapg111_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aapg111")
        IF handler IS NOT NULL THEN
            START REPORT aapg111_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY apa01,apb02"
          
            DECLARE aapg111_datacur1 CURSOR FROM l_sql
            FOREACH aapg111_datacur1 INTO sr1.*
                OUTPUT TO REPORT aapg111_rep(sr1.*)
            END FOREACH
            FINISH REPORT aapg111_rep
        END IF
...
REPORT aapg111_rep(sr1)
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.apa01
           ...
           START REPORT aapg111_subrep01
           DECLARE aapg111_repcur1 CURSOR FROM l_sql
           FOREACH aapg111_repcur1 INTO sr2.*
                OUTPUT TO REPORT aapg111_subrep01(sr2.*)
           END FOREACH
           FINISH REPORT aapg111_subrep01
        ...
        AFTER GROUP OF sr1.apa01
           ...
           START REPORT aapg111_subrep02
           DECLARE aapg111_repcur2 CURSOR FROM l_sql
           FOREACH aapg111_repcur2 INTO sr3.*
                OUTPUT TO REPORT aapg111_subrep02(sr3.*)
           END FOREACH
           FINISH REPORT aapg111_subrep02

           LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        " WHERE npq01 = '",sr1.apa01 CLIPPED,"'"
           START REPORT aapg111_subrep03
           DECLARE aapg111_repcur3 CURSOR FROM l_sql
           FOREACH aapg111_repcur3 INTO sr4.*
                OUTPUT TO REPORT aapg111_subrep03(sr4.*)
           END FOREACH
           FINISH REPORT aapg111_subrep03

           LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table4 CLIPPED,
                        " WHERE apd01 = '",sr1.apa01 CLIPPED,"'"
           START REPORT aapg111_subrep04
           DECLARE aapg111_repcur4 CURSOR FROM l_sql
           FOREACH aapg111_repcur4 INTO sr5.*
                OUTPUT TO REPORT aapg111_subrep04(sr5.*)
           END FOREACH
           FINISH REPORT aapg111_subrep04
           ...
           LET l_sql = "SELECT COUNT(*) FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        " WHERE npq01 = '",sr1.apa01 CLIPPED,"'"
           DECLARE aapg111_repcur5 CURSOR FROM l_sql
           FOREACH aapg111_repcur5 INTO l_cnt2 
           END FOREACH
