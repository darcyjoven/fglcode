...
MAIN
   ...
   LET l_table = cl_prt_temptable('aapg312',g_sql) CLIPPED 
   IF l_table = -1 THEN 
   ...
   LET l_table1 = cl_prt_temptable('aapg3121',g_sql) CLIPPED 
   IF l_table1 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-BB0047 mark
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)    #FUN-B90058#FUN-C10036 mark
      EXIT PROGRAM 
   ...
   LET l_table2 = cl_prt_temptable('aapg3122',g_sql) CLIPPED 
   IF l_table2 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-BB0047 mark 
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)    #FUN-B90058#FUN-C10036 mark
      EXIT PROGRAM 
   ...
   LET l_table3 = cl_prt_temptable('aapg3123',g_sql) CLIPPED 
   IF l_table3 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-BB0047 mark  
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)    #FUN-B90058#FUN-C10036 mark
      EXIT PROGRAM 
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01,    #No.FUN-580031
   ...
   OPEN WINDOW g312_w AT p_row,p_col WITH FORM "aap/42f/aapg312"
   ...
   WHILE TRUE
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aapg312',g_time,l_cmd)    #Execute cmd at later time
         END IF
         CLOSE WINDOW g312_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time
         CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)    #FUN-B90058
...
#          l_dbs_anm,l_dbs LIKE type_file.chr21       # No.FUN-690028 VARCHAR(20)  #No.FUN-7B0012     #FUN-A10098 ----mark
 ...
 DEFINE l_plant     LIKE type_file.chr10       #FUN-980020
   ...
   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aapg312'
...
###GENGRE###   CALL cl_prt_cs3('aapg312','aapg312',g_sql,g_str)  
    CALL aapg312_grdata()    ###GENGRE###
...
FUNCTION aapg312_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aapg312")
        IF handler IS NOT NULL THEN
            START REPORT aapg312_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY apf01" 
            DECLARE aapg312_datacur1 CURSOR FROM l_sql
            FOREACH aapg312_datacur1 INTO sr1.*
                OUTPUT TO REPORT aapg312_rep(sr1.*)
            END FOREACH
            FINISH REPORT aapg312_rep
        END IF
...
REPORT aapg312_rep(sr1)
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.apf01
            ...
            #TART REPORT aapg312_subrep02
            #ECLARE aapg312_repcur2 CURSOR FROM l_sql
            #OREACH aapg312_repcur2 INTO sr3.*
            #   OUTPUT TO REPORT aapg312_subrep02(sr3.*)
            #ND FOREACH
            #INISH REPORT aapg312_subrep02
            #FUN-CC0093--add--end
            #FUN-D10098--mark--end--
            #付款明細
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE aph01 = '",sr1.apf01 CLIPPED,"'"                         
            START REPORT aapg312_subrep01
            DECLARE aapg312_repcur1 CURSOR FROM l_sql
            FOREACH aapg312_repcur1 INTO sr2.*
                OUTPUT TO REPORT aapg312_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT aapg312_subrep01
            ...
            LET l_sql = "SELECT  A.*,B.azi05_g,B.apf01 FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED, " A LEFT OUTER JOIN ",
                        ...
                        " WHERE aph01 = '",sr1.apf01 CLIPPED,"'"
            START REPORT aapg312_subrep04
            DECLARE aapg312_repcur4 CURSOR FROM l_sql
            FOREACH aapg312_repcur4 INTO sr2.*,l_azi05
                OUTPUT TO REPORT aapg312_subrep04(sr2.*,l_azi05)
            END FOREACH
            FINISH REPORT aapg312_subrep04
            #實際應付金額
            #FUN-D10098--mark--str--
            #FUN-CC0093--add--str----
            #LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
            ...
            #            " ORDER BY aph03"                    
            #START REPORT aapg312_subrep05
            #DECLARE aapg312_repcur5 CURSOR FROM l_sql
            #FOREACH aapg312_repcur5 INTO sr2.*
            #    OUTPUT TO REPORT aapg312_subrep05(sr2.*)
            #END FOREACH
            #FINISH REPORT aapg312_subrep05
            #FUN-CC0093--add--end
            #FUN-D10098--mark--end--
            ...
            LET l_sql = "SELECT DISTINCT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,
                        " WHERE apg01 = '",sr1.apf01 CLIPPED,"'"    
            START REPORT aapg312_subrep02
            DECLARE aapg312_repcur2 CURSOR FROM l_sql
            FOREACH aapg312_repcur2 INTO sr3.*
               OUTPUT TO REPORT aapg312_subrep02(sr3.*)
            END FOREACH
            FINISH REPORT aapg312_subrep02
            #FUN-CC0093--mark--end
        ...
        AFTER GROUP OF sr1.apf01
          #FUN-B40097----add-----str---------
            ...
            START REPORT aapg312_subrep03
            DECLARE aapg312_repcur3 CURSOR FROM l_sql
            FOREACH aapg312_repcur3 INTO sr4.*
                OUTPUT TO REPORT aapg312_subrep03(sr4.*)
            END FOREACH
            FINISH REPORT aapg312_subrep03
        #FUN-CC0093--mark--end
        #FUN-D10098--unmark--end-- 
            #FUN-B40097----add-----end---------
        ...
        AFTER GROUP OF sr3.apg01
          ...
          #LET l_apg05f = GROUP SUM(sr3.apg05f)
           LET l_sql = " SELECT SUM(apg05),SUM(apg05f) FROM ",
             ...
             DECLARE aapg312_subrep02_cs CURSOR FROM l_sql
             FOREACH aapg312_subrep02_cs INTO l_apg05,l_apg05f
             END FOREACH 
           PRINTX l_apg05  
           PRINTX l_apg05f 
...
REPORT aapg312_subrep04(sr2,l_azi05)
    DEFINE sr2 sr2_t
