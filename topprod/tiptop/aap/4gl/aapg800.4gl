...
# Modify.........: No.FUN-C30085 12/06/25 By nanbing 新增付款條件、部門編號欄位說明
...
TYPE sr1_t RECORD
    ...
    als22 LIKE als_file.als22,
    als23 LIKE als_file.als23,
    ...
    ,l_als04 LIKE gem_file.gem02, #FUN-C30085 
    l_als10 LIKE pma_file.pma02  #FUN-C30085 add als04,als10
END RECORD
###GENGRE###END
...
MAIN
   ...
   LET g_sql = "als05.als_file.als05,pmc03_1.pmc_file.pmc03,ala03.ala_file.ala03,",
               ...
               "str.type_file.chr1000,azi04.azi_file.azi04,azi03.azi_file.azi03,",
               ...
               ,",l_als04.gem_file.gem02,l_als10.pma_file.pma02"  #FUN-C30085 add als04,als10
   LET l_table = cl_prt_temptable('aapg800',g_sql) CLIPPED
   IF l_table = -1 THEN 
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN
      CALL g800_tm(0,0)        # Input print condition
   ELSE
      CALL g800()              # Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW g800_w WITH FORM "aap/42f/aapg800"
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aapg800',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW g800_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table)
...
#     FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR
     ...
     LET l_sql= "SELECT als_file.*, alt_file.*, ala_file.*, ",
                "       pmc03, pmn041, pmn07,azi04,azi03,azi05,pmn80,pmn82,pmn83,pmn85, ",  #No.FUN-580003
                "       pmn20 ",  #FUN-710029
     ...
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE g800_curs1 CURSOR FOR g800_prepare1
 
#No.FUN-710086 --start-- mark
#    CALL cl_outnam('aapg800') RETURNING l_name
...
#No.FUN-710086 --end--
     ...
     FOREACH g800_curs1 INTO als.*, alt.*, ala.*, sr.*
          IF SQLCA.sqlcode != 0 THEN
         ...
         #SELECT ima906 INTO l_ima906 FROM ima_file WHERE ima01=sr.pmn04  #TQC-920069
          ...
          EXECUTE insert_prep USING als.als05,sr.pmc03,ala.ala03,als.als06,
                                    ...
                                    l_str2,sr.azi04,sr.azi03,sr.azi05,
                                    "",        l_img_blob,    "N",           ""    #FUN-C40019 add
                                    ,l_als04,l_als10                       #FUN-C30085 add l_als04, l_als10
          #No.FUN-710086 --end--
 
#         OUTPUT TO REPORT g800_rep(als.*, alt.*, ala.*, sr.*)       #No.FUN-710086 mark
     END FOREACH
 
#    FINISH REPORT g800_rep                        #No.FUN-710086
 
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)   #No.FUN-710086
 

     LET g_cr_table = l_table                   #主報表的temp table名稱          #FUN-C40019 add
     LET g_cr_apr_key_f = "als01"               #報表主鍵欄位名稱，用"|"隔開     #FUN-C40019 add
...
###GENGRE###     LET l_sql = "SELECT * FROM ", g_cr_db_str CLIPPED, l_table CLIPPED
   # CALL cl_prt_cs3('aapg800',l_sql,g_str)        #TQC-730088
###GENGRE###     CALL cl_prt_cs3('aapg800','aapg800',l_sql,g_str)
    CALL aapg800_grdata()    ###GENGRE###
     #No.FUN-710086 --end--
 
END FUNCTION
 
#No.FUN-710086 --start-- mark
{REPORT g800_rep(als, alt, ala, sr)
...
FUNCTION aapg800_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aapg800")
        IF handler IS NOT NULL THEN
            START REPORT aapg800_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY als01,alt02 " #FUN-B40087 add 
          
            DECLARE aapg800_datacur1 CURSOR FROM l_sql
            FOREACH aapg800_datacur1 INTO sr1.*
                OUTPUT TO REPORT aapg800_rep(sr1.*)
            END FOREACH
            FINISH REPORT aapg800_rep
        END IF
...
REPORT aapg800_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno           LIKE type_file.num5
    #FUN-B40087  ADD --------str -----
    DEFINE l_als05_pmc03_1    STRING 
    DEFINE l_als06_pmc03_2    STRING
    DEFINE l_als08_als09      STRING
    DEFINE l_als22_als23      STRING
    DEFINE l_als31_als36      LIKE als_file.als31 
    DEFINE l_als22            STRING
    DEFINE l_sum_als13        LIKE als_file.als13 
    DEFINE l_display          LIKE sma_file.sma116  
    DEFINE l_display1         LIKE sma_file.sma115 
    DEFINE l_alt87_fmt        STRING
    DEFINE l_alt06_fmt        STRING
    DEFINE l_alt05_fmt        STRING
    DEFINE l_alt07_fmt        STRING
    DEFINE l_als13_fmt        STRING
    ...
    DEFINE l_als31_als36_fmt  STRING
    #FUN-B40087  ADD --------end -----

    ORDER EXTERNAL BY sr1.als01,sr1.alt02
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.als01
            LET l_lineno = 0
            #FUN-B40087  ADD --------str -----
               LET l_display1 = g_sma115
               PRINTX l_display1
            LET l_als13_fmt = cl_gr_numfmt('als_file','als13',sr1.azi05)
            PRINTX l_als13_fmt
            LET l_als31_fmt = cl_gr_numfmt('als_file','als31',sr1.azi04)
            PRINTX l_als31_fmt
            LET l_als32_fmt = cl_gr_numfmt('als_file','als32',sr1.azi04)
            PRINTX l_als32_fmt
            LET l_als33_fmt = cl_gr_numfmt('als_file','als33',sr1.azi04)
            PRINTX l_als33_fmt
            LET l_als34_fmt = cl_gr_numfmt('als_file','als34',sr1.azi04)
            PRINTX l_als34_fmt
            LET l_als35_fmt = cl_gr_numfmt('als_file','als35',sr1.azi04)
            PRINTX l_als35_fmt
            LET l_als36_fmt = cl_gr_numfmt('als_file','als36',sr1.azi04)
            PRINTX l_als36_fmt
            LET l_als31_als36_fmt = cl_gr_numfmt('als_file','als31',sr1.azi04)
            PRINTX l_als31_als36_fmt
            LET l_als05_pmc03_1 = sr1.als05,' ',sr1.pmc03_1
            LET l_als06_pmc03_2 = sr1.als06,' ',sr1.pmc03_2
            LET l_als08_als09 = sr1.als08,' ',sr1.als09
            LET l_als22 = sr1.als22 USING '--,---,---,---,---,--&'
            LET l_als22_als23 = l_als22,sr1.als23
            PRINTX l_als05_pmc03_1
            PRINTX l_als06_pmc03_2
            PRINTX l_als08_als09
            PRINTX l_als22_als23           
            LET l_display = g_sma116
            PRINTX l_display
            #FUN-B40087  ADD --------end -----
        BEFORE GROUP OF sr1.alt02
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B40087  ADD --------str-----
            LET l_alt87_fmt = cl_gr_numfmt('alt_file','alt87',sr1.azi04)
            PRINTX l_alt87_fmt
            LET l_alt06_fmt = cl_gr_numfmt('alt_file','alt06',sr1.azi04)
            PRINTX l_alt06_fmt
            LET l_alt07_fmt = cl_gr_numfmt('alt_file','alt07',sr1.azi04)
            PRINTX l_alt07_fmt
            LET l_alt05_fmt = cl_gr_numfmt('alt_file','alt05',sr1.azi03)
            PRINTX l_alt05_fmt
            #FUN-B40087  ADD --------end------
            PRINTX sr1.*

        AFTER GROUP OF sr1.als01
            #FUN-B40087  ADD --------str -----
            LET l_als31_als36 = sr1.als31 + sr1.als32 + sr1.als33 + sr1.als34 + sr1.als35 + sr1.als36
            PRINTX l_als31_als36
            LET l_sum_als13 = GROUP SUM(sr1.als13)
            PRINTX l_sum_als13
            #FUN-B40087  ADD --------end -----
        AFTER GROUP OF sr1.alt02
