# Prog. Version..: '5.30.06-13.03.18(00009)'     #
#
# Pattern name...: aapg313.4gl
# Descriptions...: 應付餘額調整單列印
# Date & Author..: No.FUN-B20018 11/02/11 By wujie 
...
# Modify.........: No.FUN-B40097 11/05/23 By chenying 憑證類CR轉GR
...
# Modify.........: No.FUN-C10024 12/05/15 By minpp 帳套取歷年主會計帳別檔tna_file 
...
#   CALL cl_used(g_prog,g_time,1) RETURNING g_time    #FUN-B80105 MARK	
   ...
   LET l_table = cl_prt_temptable('aapg313',g_sql) CLIPPED 
   IF l_table = -1 THEN
   ...
   LET l_table1 = cl_prt_temptable('aapg3131',g_sql) CLIPPED 
   IF l_table1 = -1 THEN 
     #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40097  #FUN-BB0047 mark     
     #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3) #FUN-B40097#FUN-C10036 mark  
      EXIT PROGRAM 
   ...
   LET l_table2 = cl_prt_temptable('aapg3132',g_sql) CLIPPED 
   IF l_table2 = -1 THEN 
     #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40097  #FUN-BB0047 mark    
     #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3) #FUN-B40097#FUN-C10036 mark 
      EXIT PROGRAM 
   ...
   LET l_table3 = cl_prt_temptable('aapg3133',g_sql) CLIPPED 
   IF l_table3 = -1 THEN 
     #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40097  #FUN-BB0047 mark    
     #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3) #FUN-B40097#FUN-C10036 mark   
      EXIT PROGRAM 
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'   # If background job sw is off
      THEN CALL g313_tm(0,0)        # Input print condition
      ELSE CALL g313()              # Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01,    #No.FUN-580031
   ...
   OPEN WINDOW g313_w AT p_row,p_col WITH FORM "aap/42f/aapg313"
        ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
 
   CALL cl_ui_init()
   #CALL cl_set_comp_visible('c',FALSE) #FUN-CC0093 add   #FUN-D10098 mark
   CALL cl_opmsg('p')
   ...
   WHILE TRUE
      ...
      END IF
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW g313_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40097
         CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3) #FUN-B40097
         EXIT PROGRAM
      END IF
 
      INPUT BY NAME tm.h, tm.b, tm.c, tm.more
            WITHOUT DEFAULTS
         BEFORE INPUT
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('aapg313',g_time,l_cmd)    #Execute cmd at later time
         END IF
         CLOSE WINDOW g313_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40097
         CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3) #FUN-B40097
...
#          l_dbs_anm,l_dbs LIKE type_file.chr21       # No.FUN-690028 VARCHAR(20)  #No.FUN-7B0012     #FUN-A10098 ----mark
 ...
 DEFINE l_plant     LIKE type_file.chr10       #FUN-980020
   ...
   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'aapg313'
   ...
   LET l_sql = "SELECT ",
              ...
              #" apf01, apf02, apf03, apf04,  '',     apf05, apf06,",   #NO.TQC-750115 #MOD-40204 mark
               ...
               " azi03, azi04, azi05 ,'','','' ",                    #FUN-C50003 add   #minpp add 3''
...
#No.FUN-A10098 -----mark end
   ...
   LET l_sql = " SELECT aph_file.aph01, aph_file.aph02,",
               ...
               "        aph_file.aph06, aph_file.aph24,pmc03,",
               "        aph_file.aph23, aph_file.aph09,", 
               "        aph_file.aph13, aph_file.aph14,", 
               "        aag02,nma02 ,",
               "        azi03,azi04,azi05,azi07",          #FUN-C50003 add
             # "   FROM aph_file LEFT OUTER JOIN aag_file ON aag01 = aph04 AND aag_file.aag00 = '",g_bookno1,"' LEFT OUTER JOIN nma_file ON nma01 = aph08 ", #FUN-C10024 MARK
               "   FROM aph_file LEFT OUTER JOIN aag_file ON aag01 = aph04 AND aag_file.aag00 = ? LEFT OUTER JOIN nma_file ON nma01 = aph08 ", #FUN-C10024 ADD
               "        LEFT OUTER JOIN pmc_file ON pmc01 = aph21 ",
...
#FUN-C50003-----aad-----str--
         ...
         PREPARE g313_premisc FROM l_sql
         DECLARE g313_misc CURSOR FOR g313_premisc
         ...
         IF SQLCA.sqlcode != 0 THEN
            ...
            EXIT PROGRAM
         END IF
         DECLARE g313_cpay CURSOR FOR g313_ppay
#FUN-C50003-----add-----end--

   FOREACH g313_curs1 INTO sr1.*
      IF SQLCA.sqlcode != 0 THEN
...
#        DECLARE g313_cpay CURSOR FOR g313_ppay
#FUN-C50003-----mark---end----
         FOREACH g313_cpay USING srapg.apg04 INTO sr12.*,l_azi03,l_azi04,l_azi05   #FUN-C50003 add USING srapg.apg04
            IF SQLCA.sqlcode THEN
               CALL cl_err('foreac g313_cpay',SQLCA.sqlcode,0)
               EXIT FOREACH
            END IF
            ...
            IF sr12.apa08 = 'MISC' THEN   #表示是多發票,需抓apk_file
               LET l_flag =0
               FOREACH g313_misc USING srapg.apg04   #MOD-7A0137
      ...
      IF tm.c = 'Y' THEN
         ...
         END FOREACH
      END IF
      CALL g313_upd(sr1.apf01)       #更改"列印次數"
   END FOREACH
...
FUNCTION aapg313_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("aapg313")
        IF handler IS NOT NULL THEN
            START REPORT aapg313_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                       ," ORDER BY apf01"      
            DECLARE aapg313_datacur1 CURSOR FROM l_sql
            FOREACH aapg313_datacur1 INTO sr1.*
                OUTPUT TO REPORT aapg313_rep(sr1.*)
            END FOREACH
            FINISH REPORT aapg313_rep
        END IF
...
REPORT aapg313_rep(sr1)
    ...
    DEFINE l_apf04_gen02    STRING        #FUN-B40097
    DEFINE l_apf08_fmt      STRING        #FUN-B40097
    DEFINE l_apf08f_fmt     STRING        #FUN-B40097
    DEFINE l_apf10_fmt      STRING        #FUN-B40097
    DEFINE l_apf10f_fmt     STRING        #FUN-B40097
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.apf01
            ...
            #           "  ORDER BY apg02"
            #START REPORT aapg313_subrep02
            #DECLARE aapg313_repcur2 CURSOR FROM l_sql
            #FOREACH aapg313_repcur2 INTO sr3.*
            #    OUTPUT TO REPORT aapg313_subrep02(sr3.*)
            #END FOREACH
            #FINISH REPORT aapg313_subrep02  
            #FUN-CC0093--add--end
            #FUN-D10098--mark--end--
            #付款明細
            ...
            START REPORT aapg313_subrep01
            DECLARE aapg313_repcur1 CURSOR FROM l_sql
            FOREACH aapg313_repcur1 INTO sr2.*,l_azi05
                OUTPUT TO REPORT aapg313_subrep01(sr2.*,l_azi05)
            END FOREACH
            FINISH REPORT aapg313_subrep01
            #付款總計
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE aph01 = '",sr1.apf01 CLIPPED,"'"
            START REPORT aapg313_subrep04
            DECLARE aapg313_repcur4 CURSOR FROM l_sql
            FOREACH aapg313_repcur4 INTO sr2.*
                OUTPUT TO REPORT aapg313_subrep04(sr2.*)
            END FOREACH
            FINISH REPORT aapg313_subrep04
            ...
            #            " ORDER BY aph03"                    
            #START REPORT aapg313_subrep05
            #DECLARE aapg313_repcur5 CURSOR FROM l_sql
            #FOREACH aapg313_repcur5 INTO sr2.*
            #    OUTPUT TO REPORT aapg313_subrep05(sr2.*)
            #END FOREACH
            #FINISH REPORT aapg313_subrep05
            #FUN-CC0093--add--end
            #FUN-D10098--mark--str--
            
           #LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,           #FUN-C20053 mark
           #FUN-D10098--unmark--str--
           #FUN-CC0093--mark--str--
           LET l_sql = "SELECT DISTINCT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,  #FUN-C20053 add
                       "  WHERE apg01 = '",sr1.apf01 CLIPPED,"'",
                       "  ORDER BY apg02"
            START REPORT aapg313_subrep02
            DECLARE aapg313_repcur2 CURSOR FROM l_sql
        ...
        AFTER GROUP OF sr1.apf01
           #FUN-B40097----add-----str---------
            ...
            START REPORT aapg313_subrep03
            DECLARE aapg313_repcur3 CURSOR FROM l_sql
            FOREACH aapg313_repcur3 INTO sr4.*
                OUTPUT TO REPORT aapg313_subrep03(sr4.*)
            END FOREACH
            FINISH REPORT aapg313_subrep03
        #FUN-CC0093--mark--end 
        #FUN-D10098--unmark--end--
            #FUN-B40097----add-----end---------
...
#FUN-B40097----add-----str---------
REPORT aapg313_subrep01(sr2,l_azi05)
    DEFINE sr2 sr2_t
    DEFINE sr3 sr3_t
    ...
    DEFINE l_azi05           LIKE azi_file.azi05
    #DEFINE l_aph04          STRING  #FUN-CC0093   #FUN-D10098 mark
    ...
    FORMAT
        ON EVERY ROW
           ...
           LET l_option = cl_gr_getmsg("gre-101",g_lang,sr2.aph03)
           LET l_aph03  = sr2.aph03,'.',l_option 
           IF cl_null(l_option) THEN
              LET l_aph03  = sr2.apw02
           END IF
           PRINTX l_aph03
           #LET l_aph04 = sr2.aph04,' ',sr2.aag02     #FUN-CC0093   #FUN-D10098 mark
           #PRINTX l_aph04                            #FUN-CC0093   #FUN-D10098 mark
           PRINTX sr2.*
...
REPORT aapg313_subrep02(sr3)
    ...
    DEFINE l_apb13_fmt     STRING
    DEFINE l_apb14_fmt     STRING
    DEFINE l_apb15_fmt     STRING
    DEFINE l_apg05_sum_fmt     STRING
    DEFINE l_apg05f_sum_fmt    STRING
    DEFINE l_sql           STRING #FUN-C20053

    ORDER EXTERNAL BY sr3.apg01
    ...
    FORMAT
        ON EVERY ROW
           ...
           PRINTX l_apg05f_fmt
           LET l_apb08_fmt = cl_gr_numfmt('apb_file','apb08',sr3.azi03_1)
           PRINTX l_apb08_fmt
           LET l_apb13_fmt = cl_gr_numfmt('apb_file','apb13',sr3.azi03_1)
           PRINTX l_apb13_fmt
           LET l_apb09_fmt = cl_gr_numfmt('apb_file','apb09',sr3.azi05_1)
           PRINTX l_apb09_fmt
           LET l_apb15_fmt = cl_gr_numfmt('apb_file','apb15',sr3.azi05_1)
           PRINTX l_apb15_fmt
           LET l_apb14_fmt = cl_gr_numfmt('apg_file','apg14',sr3.azi04_1)
           PRINTX l_apb14_fmt
           LET l_apb10_fmt = cl_gr_numfmt('apg_file','apg10',sr3.azi04_1)
           PRINTX l_apb10_fmt
           PRINTX sr3.*
        AFTER GROUP OF sr3.apg01
          ...
          #LET l_apg05 = GROUP SUM(sr3.apg05)    #FUN-C20053 mark
          #LET l_apg05f = GROUP SUM(sr3.apg05f)  #FUN-C20053 mark
           #FUN-C20053----add----str------------------
           LET l_sql = " SELECT SUM(apg05),SUM(apg05f) FROM ",
                       " (SELECT  DISTINCT apg01,apg02,apg04,apg05,apg05f FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,
                       " WHERE apg01 = '",sr3.apg01 CLIPPED,"' AND apg04 = '",sr3.apg04 CLIPPED,"' )"
             DECLARE aapg313_subrep02_cs CURSOR FROM l_sql
             FOREACH aapg313_subrep02_cs INTO l_apg05,l_apg05f
             END FOREACH 
           #FUN-C20053----add----end------------------
           PRINTX l_apg05
           PRINTX l_apg05f            
