...
#
# Pattern name...: acot500.4gl
# Descriptions...: 進口集中報關貨物申報單維護作業
# Date & Author..: 00/09/26 By Kammy
# Modify.........: No.MOD-490371 04/09/23 By Kitty Controlp 未加display
# Modify.........: No.FUN-4B0023 04/11/02 By ching add '轉Excel檔' action
# Modify.........: No.MOD-490398 04/11/22 By Carrier
#                  增加海關編號,修改確認及確認還原中量的計算
#                  cno10修改成手冊編號
#                  將原來的acot500拆成三支..本作業用于進口
#                  cno031新增4.耗用
...
# Modify.........: No.MOD-940032 09/04/06 By Smapmin 交易對象應抓取廠商編號
# Modify.........: No.FUN-980002 09/08/05 By TSD.SusanWu GP5.2架構重整，修改 INSERT INTO 語法
 
# Modify.........: No.FUN-980030 09/08/31 By Hiko 加上GP5.2的相關設定
# Modify.........: No.FUN-9B0098 10/02/24 by tommas delete cl_doc
# Modify.........: No.TQC-AB0124 10/11/28 By chenying 交易對象cno08開窗修改為cno03=1.成品時,開 q_occ 客戶;cno03=2.材料時,開q_pmc 廠商
# Modify.........: No.MOD-B30264 11/03/12 By sabrina 報關單號應允許使用者隨意輸入不需像單別一樣中間有一碼"-"
...
DEFINE
    ...
    g_cno031        LIKE cno_file.cno031,         #No.MOD-490398
...
#       l_time    LIKE type_file.chr8              #No.FUN-6A0063
    ...
    OPEN WINDOW t500_w AT p_row,p_col WITH FORM "aco/42f/acot500"
          ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
 
    CALL cl_ui_init()
 
    LET g_cno031 = '2'                     #進口  No.MOD-490398
...
DEFINE l_type       LIKE type_file.chr1         #No.FUN-680069 VARCHAR(1)
  ...
  CALL g_cnp.clear()
     ...
     #No.MOD-490398  --begin
  CALL t500_fill_cno04('0')
#FUN-630014-start 
  IF NOT cl_null(g_argv1) THEN
     LET g_wc = " cno01 = '",g_argv1,"'"
     LET g_wc2= ' 1=1'
  ELSE
#FUN-630014-end
...
#                  CALL q_coy(10,3,g_cno.cno01,'14',g_sys) RETURNING g_cno.cno01
              ...
              WHEN INFIELD(cno05)
                IF g_cno.cno04 = '2' THEN     #轉廠資料查詢
                   IF g_cno.cno03 = '1' THEN   #成品
                      IF g_cno.cno031 = '1' THEN LET l_type = '4' END IF #轉出
                      IF g_cno.cno031 = '2' THEN LET l_type = '2' END IF #轉入
                   ELSE                        #材料
                      IF g_cno.cno031 = '1' THEN LET l_type = '3' END IF #轉出
                      IF g_cno.cno031 = '2' THEN LET l_type = '1' END IF #轉入
                   END IF
#                  CALL q_cnm(0,0,g_cno.cno05,l_type) RETURNING g_cno.cno05
...
#                  CALL q_cnu(0,0,g_cno.cno05,g_cno.cno03) RETURNING g_cno.cno05
                   CALL cl_init_qry_var()
                ...
                NEXT FIELD cno05
              WHEN INFIELD(cno08)
#               CALL q_cnb(10,3,g_cno.cno08) RETURNING g_cno.cno08
                CALL cl_init_qry_var()
                #LET g_qryparam.form = "q_cnb"   #MOD-940032
                LET g_qryparam.form = "q_pmc"    #MOD-940032
                LET g_qryparam.state = 'c'
                CALL cl_create_qry() RETURNING g_qryparam.multiret
               ...
               #No.MOD-490398  --begin
              WHEN INFIELD(cno10)
#               CALL q_coc(10,3,g_coc01) RETURNING g_cno.cno10
              ...
              WHEN INFIELD(cno20)
               ...
               #No.MOD-490398  --end
              WHEN INFIELD(cno13)
#               CALL q_geb(10,3,g_cno.cno13) RETURNING g_cno.cno13
                CALL cl_init_qry_var()
    ...
    CONSTRUCT g_wc2 ON cnp012,cnp02,cnp03,cnp05,     #螢幕上取單身條件
            ...
            FROM s_cnp[1].cnp012,s_cnp[1].cnp02,s_cnp[1].cnp03,
                 ...
                 #No.FUN-840202 --start--
                 ,s_cnp[1].cnpud01,s_cnp[1].cnpud02,s_cnp[1].cnpud03,
                 s_cnp[1].cnpud04,s_cnp[1].cnpud05, s_cnp[1].cnpud06,
                 s_cnp[1].cnpud07,s_cnp[1].cnpud08,s_cnp[1].cnpud09,
                 s_cnp[1].cnpud10, s_cnp[1].cnpud11,s_cnp[1].cnpud12,
                 s_cnp[1].cnpud13,s_cnp[1].cnpud14,s_cnp[1].cnpud15
                 #No.FUN-840202 ---end---
...
#                          CALL q_cod(4,2,g_coc01,g_cnp[1].cnp02) RETURNING g_coc01,g_cnp[1].cnp02
                           CALL cl_init_qry_var()
                           LET g_qryparam.form = "q_cod"
                           LET g_qryparam.state = 'c'
                           CALL cl_create_qry() RETURNING g_qryparam.multiret
                           DISPLAY g_qryparam.multiret TO cnp02
                           NEXT FIELD cnp02
                        ELSE
#                          CALL q_coe(4,2,g_coc01,g_cnp[1].cnp02) RETURNING g_coc01,g_cnp[1].cnp02
  ...
  END IF    #FUN-630014 add
    ...
    IF g_wc2 = " 1=1" THEN			# 若單身未輸入條件
       LET g_sql = "SELECT cno01 FROM cno_file ",
                   " WHERE ", g_wc CLIPPED,
                   "   AND (cno031='2' AND cno04 <> '5' OR (cno031='4' ",
                   "   AND cno18 IN (SELECT cno01 FROM cno_file ",
                   "       WHERE cno03 ='1' AND cno031='2')))",
                   " ORDER BY cno01"
     ELSE					# 若單身有輸入條件
...
FUNCTION t500_a()
    ...
    CALL cl_opmsg('a')
    WHILE TRUE
        LET g_cno.cno03 = '2'           #No.MOD-490398
        LET g_cno.cno031= g_cno031      #No.MOD-490398
        LET g_cno.cno15 = 0
        LET g_cno.cno16 = 0
        LET g_cno.cno19 = 0
...
FUNCTION t500_u()
    ...
    OPEN t500_cl USING g_cno.cno01
    IF STATUS THEN
       ...
       RETURN
    END IF
    FETCH t500_cl INTO g_cno.*            # 鎖住將被更改或撤銷的資料
...
DEFINE
    ...
    INPUT BY NAME
        ...
        #CALL cl_set_docno_format("cno06")   #MOD-B30264 mark
...
#No.FUN-550036 --start--
          IF g_cno.cno01 != g_cno01_t OR g_cno01_t IS NULL THEN
            ...
            IF (NOT li_result) THEN
               ...
               NEXT FIELD cno01
            END IF
#           DISPLAY g_smy.smydesc TO smydesc     #No.FUN-560106
...
#No.FUN-550036 ---end---
       ...
       BEFORE FIELD cno04
            CALL t500_fill_cno04('1')
            CALL t500_set_no_required()
 
        AFTER FIELD cno04
            IF g_cno.cno04 MATCHES '[0-6]' THEN
               CALL t500_cno04()
            END IF
            CALL t500_set_required()
         #No.MOD-490398  --end
            ...
            IF NOT cl_null(g_cno.cno08) THEN
               ...
               IF NOT cl_null(g_errno) THEN
                  ...
                  DISPLAY BY NAME g_cno.cno08
                   NEXT FIELD cno08   #No.MOD-490398
               END IF
            END IF
            ...
            IF NOT cl_null(g_cno.cno10) THEN
               ...
               IF NOT cl_null(g_errno) THEN
                  ...
                  DISPLAY BY NAME g_cno.cno10
                   NEXT FIELD cno10   #No.MOD-490398
               END IF
            END IF
            ...
            IF NOT cl_null(g_cno.cno13) THEN
                ...
                IF NOT cl_null(g_errno) THEN
                   ...
                   DISPLAY BY NAME g_cno.cno13
                    NEXT FIELD cno13   #No.MOD-490398
                END IF
            END IF
        ...
        #FUN-840202     ----end----
         ...
         AFTER INPUT
            ...
            IF g_cno.cno04 MATCHES '[24]' THEN   #轉廠/內銷要輸入[預申請單號]
               ...
               END IF
            END IF
            CALL t500_valid_check()  RETURNING  g_i
...
#                   CALL FGL_DIALOG_SETBUFFER( g_cno.cno01 )
                   DISPLAY BY NAME g_cno.cno01
                   NEXT FIELD cno01
              ...
              WHEN INFIELD(cno05)
                IF g_cno.cno04 = '2' THEN     #need modify because source changed
                   ...
                   END CASE
                    #No.MOD-490398  --end
#                  CALL q_cnm(0,0,g_cno.cno05,l_type) RETURNING g_cno.cno05
#                  CALL FGL_DIALOG_SETBUFFER( g_cno.cno05 )
                   CALL cl_init_qry_var()
                   LET g_qryparam.form = "q_cnm"
...
#                   CALL FGL_DIALOG_SETBUFFER( g_cno.cno05 )
              ...
              WHEN INFIELD(cno08)
#               CALL q_cnb(10,3,g_cno.cno08) RETURNING g_cno.cno08
#               CALL FGL_DIALOG_SETBUFFER( g_cno.cno08 )
                CALL cl_init_qry_var()
                #LET g_qryparam.form = "q_cnb"   #MOD-940032
                #TQC-AB0124-----add--------str--------------
                IF g_cno.cno03 = '1' THEN
...
#               CALL q_coc(10,3,g_coc01) RETURNING g_cno.cno10
              ...
              WHEN INFIELD(cno13)
#               CALL q_geb(10,3,g_cno.cno13) RETURNING g_cno.cno13
#               CALL FGL_DIALOG_SETBUFFER( g_cno.cno13 )
                CALL cl_init_qry_var()
                LET g_qryparam.form = "q_geb"
...
FUNCTION t500_cno04()       #類別
    DEFINE l_chr     LIKE zaa_file.zaa08       #No.FUN-680069  VARCHAR(4) 
     #No.MOD-490398   --begin
    CASE g_cno.cno04
         WHEN '0'    LET l_chr = cl_getmsg('aco-071',g_lang)  #報廢
         WHEN '1'    LET l_chr = cl_getmsg('aco-041',g_lang)  #直接
...
FUNCTION t500_cno05_2()  #檢查轉廠/內銷申請單
    DEFINE l_cnm04   LIKE cnm_file.cnm04,
     ...
     #No.MOD-490398  --begin   #cno05欄位不僅只限于轉廠和內銷..
...
FUNCTION t500_cno08(p_cmd)  #交易對象
    #-----MOD-940032---------
    ...
    DEFINE  l_occ02    LIKE occ_file.occ02   #TQC-AB0124 add
    DEFINE  l_occacti  LIKE occ_file.occacti #TQC-AB0124 add

    LET g_errno = ' '
    IF g_cno.cno03 = '2' THEN    #TQC-AB0124 add
      ...
      END CASE
#     IF cl_null(g_cno.cno09) THEN
         LET g_cno.cno09 = l_pmc03             #TQC-AB0124 mark 
         DISPLAY BY NAME g_cno.cno09 
...
FUNCTION t500_cno10(p_cmd)  #手冊編號
...
FUNCTION t500_cno13(p_cmd)  #貿易國別
...
FUNCTION t500_q()
    ...
    IF INT_FLAG THEN
        ...
        RETURN
    END IF
    OPEN t500_cs                            # 從DB生成合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
...
DEFINE
    ...
    CASE p_flag
        WHEN 'N' FETCH NEXT     t500_cs INTO g_cno.cno01
        WHEN 'P' FETCH PREVIOUS t500_cs INTO g_cno.cno01
        WHEN 'F' FETCH FIRST    t500_cs INTO g_cno.cno01
        WHEN 'L' FETCH LAST     t500_cs INTO g_cno.cno01
        WHEN '/'
      ...
      ON ACTION controlg      #MOD-4C0121
         ...
         END IF
         FETCH ABSOLUTE g_jump t500_cs INTO g_cno.cno01
         LET mi_no_ask = FALSE
    END CASE
...
FUNCTION t500_show()
    ...
    DISPLAY BY NAME
        g_cno.cno01,g_cno.cno03,g_cno.cno031,g_cno.cno04,
        g_cno.cno02,g_cno.cno05,g_cno.cno06,g_cno.cno07,
        g_cno.cno08,g_cno.cno09,g_cno.cno10,
        g_cno.cno12,g_cno.cno13,g_cno.cno14,g_cno.cno15,
        g_cno.cno16,g_cno.cno17,g_cno.cno18,g_cno.cno19,
        g_cno.cno20,g_cno.cno21,   #No.MOD-530224
        g_cno.cnoconf,g_cno.cnouser,
        g_cno.cnogrup,g_cno.cnomodu,g_cno.cnodate,g_cno.cnoacti
        #FUN-840202     ---start---
        ,g_cno.cnoud01,g_cno.cnoud02,g_cno.cnoud03,g_cno.cnoud04,
        g_cno.cnoud05,g_cno.cnoud06,g_cno.cnoud07,g_cno.cnoud08,
...
FUNCTION t500_r()
    ...
    IF cl_delh(0,0) THEN                   #審核一下
       ...
       INSERT INTO azo_file(azo01,azo02,azo03,azo04,azo05,azo06,azoplant,azolegal) #FUN-980002 add azoplant,azolegal
          VALUES ('acot500',g_user,g_today,g_msg,g_cno.cno01,'delete',g_plant,g_legal) #FUN-980002 add g_plant,g_legal
        #No.MOD-490398 end
            CLEAR FORM
            CALL g_cnp.clear()
...
DEFINE
    ...
    LET l_ac_t = 0
     ...
     #No.MOD-490398  --begin  #目前進口的流程都有走底稿
    #
    IF g_status = '0' THEN
        ...
        BEFORE ROW
            ...
            IF g_rec_b >= l_ac THEN
               ...
               LET g_cnp_o.* = g_cnp[l_ac].*  #BACKUP
                OPEN t500_bcl USING g_cno.cno01,g_cnp_t.cnp012
                IF STATUS THEN
                    CALL cl_err("OPEN t500_bcl:", STATUS, 1)
                    LET l_lock_sw = "Y"
                ELSE
                    FETCH t500_bcl INTO g_cnp[l_ac].*
                    IF SQLCA.sqlcode THEN
                        CALL cl_err(g_cnp_t.cnp012,SQLCA.sqlcode,1)
                        LET l_lock_sw = "Y"
                    END IF
                END IF
                CALL t500_set_entry_b()
                CALL t500_set_no_entry_b()
        ...
        AFTER INSERT
            ...
            INSERT INTO cnp_file(cnp01,cnp012,cnp02,cnp03,
                                 cnp05,cnp06,cnp07,cnp08,cnp09,
                                  cnp10,cnp11,cnp14,cnp15  #No.MOD-490398
                                #No.FUN-840202 --start--
                                ,cnpud01,cnpud02,cnpud03,cnpud04,cnpud05,
                                 cnpud06,cnpud07,cnpud08,cnpud09,cnpud10,
            ...
            VALUES(g_cno.cno01,g_cnp[l_ac].cnp012,g_cnp[l_ac].cnp02,
                   ...
                   g_cnp[l_ac].cnp14,g_cno.cno20  #No.MOD-490398
                   #No.FUN-840202 --start--
...
#              CALL cl_err(g_cnp[l_ac].cnp012,SQLCA.sqlcode,0) #No.TQC-660045
               CALL cl_err3("ins","cnp_file",g_cno.cno01,g_cnp[l_ac].cnp02,SQLCA.sqlcode,"","",1) #TQC-660045
               ROLLBACK WORK
               CANCEL INSERT
               ...
               IF p_cmd = 'a' OR
                 (p_cmd = 'u' AND g_cnp[l_ac].cnp02 != g_cnp_o.cnp02 ) THEN
                  ...
                  IF NOT cl_null(g_errno) THEN
                     ...
                     #------MOD-5A0095 START----------
                     DISPLAY BY NAME g_cnp[l_ac].cnp02
                     #------MOD-5A0095 END------------
                     NEXT FIELD cnp02
                  END IF
                  #備案序號不可重復
        ...
        AFTER FIELD cnp05                        #數量
            #FUN-BB0084 ---------Begin----------
            IF NOT cl_null(g_cnp[l_ac].cnp05) AND NOT cl_null(g_cnp[l_ac].cnp06) THEN
      ...
      #     EXIT INPUT
        ...
        ON ACTION CONTROLP
            CASE
               WHEN INFIELD(cnp02)              #備案序號
                        IF g_cno.cno03 = '1' THEN
#                          CALL q_cod(4,2,g_coc01,g_cnp[l_ac].cnp02) RETURNING g_coc01,g_cnp[l_ac].cnp02
#                          CALL FGL_DIALOG_SETBUFFER( g_coc01 )
#                          CALL FGL_DIALOG_SETBUFFER( g_cnp[l_ac].cnp02 )
                           CALL cl_init_qry_var()
                           LET g_qryparam.form = "q_cod"
                           LET g_qryparam.default1 = g_coc01
                           LET g_qryparam.default2 = g_cnp[l_ac].cnp02
                           CALL cl_create_qry() RETURNING g_coc01,g_cnp[l_ac].cnp02
#                           CALL FGL_DIALOG_SETBUFFER( g_coc01 )
#                           CALL FGL_DIALOG_SETBUFFER( g_cnp[l_ac].cnp02 )
                           NEXT FIELD cnp02
                        ELSE
#                          CALL q_coe(4,2,g_coc01,g_cnp[l_ac].cnp02) RETURNING g_coc01,g_cnp[l_ac].cnp02
                           CALL q_coe(FALSE,TRUE,g_coc01,g_cnp[l_ac].cnp02) RETURNING g_coc01,g_cnp[l_ac].cnp02
#                           CALL FGL_DIALOG_SETBUFFER( g_coc01 )
#                           CALL FGL_DIALOG_SETBUFFER( g_cnp[l_ac].cnp02 )
                           NEXT FIELD cnp02
                        END IF
                         #No.MOD-490398
                         #DISPLAY BY NAME g_cnp[l_ac].cnp02          #No.MOD-490371
               OTHERWISE EXIT CASE
            END CASE
...
FUNCTION t500_cnp02(p_cmd)
    ...
    ELSE                     #料件
       ...
       CALL s_coeqty(g_cno.cno10,g_cnp[l_ac].cnp02) RETURNING l_qty  #剩餘量
    END IF
...
DEFINE
    ...
    #   " ORDER BY cnp012,cnp02"   #No.MOD-490398
...
#  IF NOT cl_confirm('axm-108') THEN RETURN END IF #CHI-C30107 mark
   ...
   IF STATUS THEN
      ...
      RETURN
   END IF
   FETCH t500_cl INTO g_cno.*               # 對DB鎖定
   IF SQLCA.sqlcode THEN
       CALL cl_err(g_cno.cno01,SQLCA.sqlcode,0)
       CLOSE t500_cl
       ROLLBACK WORK
...
#     CALL cl_err('upd cofconf',STATUS,0)  #No.TQC-660045
   ...
   END IF
    #No.MOD-490398  --begin
   CALL t500_yz('0',g_cno.cno01,g_cno.cno03,g_cno.cno031,g_cno.cno04,'y')
   #成品-出口/進口/報廢,自動生成折合原料單---此版本就成品而言,沒有退港一說
   IF g_cno.cno03 = '1' THEN
#     IF cl_confirm('aco-066') THEN
         CALL t500_gen()
         IF g_success = 'Y' THEN
...
#     END IF
   ...
   #CALL cl_set_field_pic(g_cno.cnoconf,"","","","",g_cno.cnoacti)
   IF g_cno.cnoconf='X' THEN LET g_void='Y' ELSE LET g_void='N' END IF  #CHI-C80041
   CALL cl_set_field_pic(g_cno.cnoconf,"","","",g_void,g_cno.cnoacti)  #CHI-C80041
END FUNCTION
 
 FUNCTION t500_yz(p_flag,p_no,p_type,p_type1,p_type2,p_chr) #No.MOD-490398
   ...
   FOREACH firm_cs INTO l_cnp.*
      ...
      IF p_type='1' THEN               #型態:合同成品
         IF p_type1 = '2' THEN       #狀況:進口
            ...
            CASE p_type2
                 WHEN '1' #直接
                       CALL s_upd_codcoe(g_cno.cno10,l_cnp.cnp02,'1','2','1')
                 WHEN '2' #轉廠
                       CALL s_upd_codcoe(g_cno.cno10,l_cnp.cnp02,'1','2','2')
                 WHEN '4' #內銷
                       CALL s_upd_codcoe(g_cno.cno10,l_cnp.cnp02,'1','2','4')
            END CASE
         END IF
      END IF
      IF p_type='2' THEN               #型態:合同材料
         SELECT cno18 INTO g_cno.cno18 FROM cno_file WHERE cno01 = g_cno.cno01
         #若不為出口成品折合原料單, 才需檢查剩餘量
         IF p_chr = 'y' AND p_flag = '0' THEN
            ##檢查合同剩餘量
            IF p_type1='2' THEN   #只在進口的時候檢查還可以進口的材料量
         ...
         IF p_chr = 'z' AND p_flag = '0' THEN
            ...
            IF p_type1 = '2' THEN
               IF l_qty - l_cnp.cnp05 < 0 THEN
                  CALL cl_err(l_cnp.cnp02,'aco-024',1)
                  LET g_success='N' EXIT FOREACH
               END IF
            END IF
         END IF
         ...
         IF p_type1 = '2' THEN         #狀況:進口
            CASE p_type2
                 WHEN '1' #直接
                       CALL s_upd_codcoe(g_cno.cno10,l_cnp.cnp02,'2','2','1')
                 WHEN '2' #轉廠
                       CALL s_upd_codcoe(g_cno.cno10,l_cnp.cnp02,'2','2','2')
                 WHEN '6' #內購
                       CALL s_upd_codcoe(g_cno.cno10,l_cnp.cnp02,'2','2','6')
             END CASE
         END IF
      ...
      IF p_flag = '0' AND g_cno.cno04 = '2' THEN
          #No.MOD-490398  要判斷還有多少沒轉量...不是申請量
...
#              CALL cl_err('upd cnn051:',STATUS,0) #No.TQC-660045
               CALL cl_err3("upd","cnn_file",g_cno.cno05,l_cnp.cnp02,STATUS,"","upd cnn_file",1) #TQC-660045
               LET g_success = 'N' EXIT FOREACH
            END IF
         END IF
...
FUNCTION t500_z()
   ...
   #IF g_cno.cno03 = '2' AND g_cno.cno031 MATCHES '[13]' AND     #折合單
   ...
   IF STATUS THEN
#     CALL cl_err('upd cofconf',STATUS,0)  #No.TQC-660045
      CALL cl_err3("upd","coc_file",g_cno.cno01,"",STATUS,"","upd cofconf",1) #TQC-660045
      LET g_success='N'
   END IF
    #No.MOD-490398  --begin
   CALL t500_yz('0',g_cno.cno01,g_cno.cno03,g_cno.cno031,g_cno.cno04,'z')
   #成品-出口/進口/報廢,自動生成折合原料單---此版本就成品而言,沒有退港一說
...
#        CALL cl_err('upd cnoconf #2',STATUS,0)  #No.TQC-660045
         CALL cl_err3("upd","cno_file",g_cno.cno18,"",STATUS,"","upd cnoconf #2",1) #TQC-660045
         LET g_success='N'
      END IF
      CALL t500_yz('1',g_cno.cno18,'2','4',g_cno.cno04,'z')
 
      DELETE FROM cno_file WHERE cno01 = g_cno.cno18
      IF STATUS THEN
#         CALL cl_err('del cno',STATUS,0)  #No.TQC-660045
          CALL cl_err3("del","cno_file",g_cno.cno18,"",STATUS,"","del cno",1) #TQC-660045
          LET g_success='N'
      END IF
...
# IF NOT cl_confirm('aco-061') THEN RETURN END IF
  ...
  IF g_cno.cno03 = '1' THEN
     CASE g_cno.cno04
        WHEN '1' LET l_coo10 = '6'    #直接進口
        WHEN '2' LET l_coo10 = '7'    #轉廠進口
        WHEN '4' LET l_coo10 = '5'    #內銷進口
     END CASE
     LET l_sql = "SELECT * FROM coo_file ",
                 " WHERE coo07 = '",g_cno.cno06,"'",
                  "   AND coo18 = '",g_cno.cno10,"'", #No.MOD-490398
                 "   AND cooconf = 'Y' ",
                 "   AND coo20   = 'N' ",         #未報關
                 "   AND coo10 = '",l_coo10,"'"
     PREPARE t500_g_p FROM l_sql
     IF STATUS THEN CALL cl_err('t500_g_p',STATUS,0) RETURN END IF
     DECLARE t500_g_cs CURSOR FOR t500_g_p
 
     FOREACH t500_g_cs INTO l_coo.*
        LET l_sql ="SELECT cod02,cod06,cod07,cod11,coc_file.* ",
                   "  FROM cod_file,coc_file ",
                    " WHERE coc03 = '",g_cno.cno10,"'", #No.MOD-490398
                   "   AND cod03 = '",l_coo.coo11,"'",
                   "   AND coc01 = cod01 "
        PREPARE t500_g_pr FROM l_sql
        IF STATUS THEN CALL cl_err('t500_g_pr',STATUS,0) RETURN END IF
        DECLARE t500_g_cs_1 CURSOR FOR t500_g_pr
        FOREACH t500_g_cs_1 INTO l_cod.*
           ...
           LET l_cnp.cnp03  = l_coo.coo11
           LET l_cnp.cnp05  = l_coo.coo16
...
#              CALL cl_err('ins cnp:',STATUS,1) #No.TQC-660045
  ...
  IF g_cno.cno03 = '2' THEN
     CASE g_cno.cno04
        WHEN '1' LET l_coo10 = '1'    #直接進口
        WHEN '2' LET l_coo10 = '2'    #轉廠進口
        WHEN '6' LET l_coo10 = '3'    #內購進口
     END CASE
     DECLARE t500_g_cs1 CURSOR FOR        # 資料來源
     ...
     FOREACH t500_g_cs1 INTO l_cop.*
         DECLARE t500_g_cs_2 CURSOR FOR
          SELECT *  FROM coe_file,coc_file
           WHERE coc03 = g_cno.cno10
             AND coe03 = l_cop.cop11
             AND coc01 = coe01
         FOREACH t500_g_cs_2 INTO l_coe.*
             LET l_count = l_count + 1
             SELECT MAX(cnp012) INTO l_cnp.cnp012
               FROM cnp_file
              WHERE cnp01 = g_cno.cno01
...
#    CALL cl_err(m_cno.cno01[1,3],'mfg0014',1) 
#    CALL cl_err(s_get_doc_no(m_cno.cno01),'mfg0014',1)    #No.FUN-550036 #No.TQC-660045
     CALL cl_err3("sel","coy_file",l_slip,"","mfg0014","","",1)   #TQC-660045
     LET g_success='N' RETURN
  END IF
# LET m_cno.cno01 = m_cno.cno01[1,3],g_cno.cno01[4,10]
  LET m_cno.cno01 = s_get_doc_no(m_cno.cno01),g_cno.cno01[g_no_sp-1,g_no_ep]    #No.FUN-550036
  LET m_cno.cno03 = '2'
   #No.MOD-490398  --begin
  LET m_cno.cno031= '4'
...
#    CALL cl_err('ins cno',STATUS,0)  #No.TQC-660045
  ...
  IF g_status = '0' THEN
     ...
     FOREACH cnp_curs INTO l_col04,l_col06,l_coe02,l_coe07,l_qty
        ...
        LET m_cnp.cnp05 = l_qty * (-1)  #No.MOD-490398
...
#          CALL cl_err('ins cnp',STATUS,0)  #No.TQC-660045
  ...
  ELSE
     CALL t500_ins_coe()
     DECLARE t500_ins_cnp_a CURSOR FOR
      SELECT con03,con04,coe02,coe07,SUM(qty) FROM coe_tmp
       GROUP BY con03,con04,coe02,coe07
       ORDER BY coe02
     IF STATUS THEN
        CALL cl_err('t500_ins_cnp_a',STATUS,0) LET g_success='N' RETURN
     END IF
     FOREACH t500_ins_cnp_a INTO l_col04,l_col06,l_coe02,l_coe07,l_qty
        IF STATUS THEN
           CALL cl_err('cnp_curs',STATUS,0) LET g_success='N' RETURN
        END IF
        LET m_cnp.cnp012 = m_cnp.cnp012 + 1
...
#       CALL cl_err('upd cno',STATUS,0)  #No.TQC-660045
        ...
        LET g_success='N' RETURN
     END IF
      CALL t500_yz('1',m_cno.cno01,m_cno.cno03,m_cno.cno031,m_cno.cno04,'y') #No.MOD-490398
  END IF
END FUNCTION
...
FUNCTION t500_set_entry_b()
 
    CALL cl_set_comp_entry("cnp02,cnp05",TRUE)
 
END FUNCTION
 
FUNCTION t500_set_no_entry_b()
...
END FUNCTION
 ...
 #No.MOD-490398  --begin
FUNCTION t500_fill_cno031(p_status)
...
#                        6.內購
    ...
    IF p_status = '0' THEN
       CALL lsb_item.append("2,4")
       LET l_chr1= "2:" || cl_getmsg('aco-040',g_lang)   #進口
       LET l_chr2= "4:" || cl_getmsg('aco-207',g_lang)   #耗用
       CALL lsb_item_value.append(l_chr1 CLIPPED || "," || l_chr2 CLIPPED)
    ELSE
       CALL lsb_item.append("2")
       LET l_chr1= "2:" || cl_getmsg('aco-040',g_lang)   #進口
...
FUNCTION t500_fill_cno04(p_status) #0.ALL 1.Product 2.Material
  ...
  DEFINE l_chr4           LIKE zaa_file.zaa08       #No.FUN-680069 VARCHAR(6)
    ...
    IF p_status = '0' OR cl_null(g_cno.cno03) THEN
          CALL lsb_item.append("1,2,4,6")
          LET l_chr1= "1:" || cl_getmsg('aco-041',g_lang)  #直接
          LET l_chr2= "2:" || cl_getmsg('aco-042',g_lang)  #轉廠
          LET l_chr3= "4:" || cl_getmsg('aco-044',g_lang)  #內銷
          LET l_chr4= "6:" || cl_getmsg('aco-182',g_lang)  #內購
          CALL lsb_item_value.append(l_chr1 CLIPPED || "," || l_chr2 CLIPPED || "," || l_chr3 CLIPPED || "," || l_chr4 CLIPPED )
    ELSE
       IF g_cno.cno03 = '1' THEN
...
FUNCTION t500_valid_check()
  DEFINE l_i   LIKE type_file.num5          #No.FUN-680069 SMALLINT
      ...
      #                     4.內銷
      LET l_i = 0
      IF g_cno.cno03 = '1' AND g_cno.cno031='2'
         AND g_cno.cno04 MATCHES '[124]' THEN
         LET l_i = 1
      END IF
 
      IF g_cno.cno03='2' AND g_cno.cno031='2'
         AND g_cno.cno04 MATCHES '[126]' THEN
         LET l_i = 1
      END IF
...
DEFINE   l_chr              LIKE type_file.chr1
   ...
   IF STATUS THEN
      ...
      RETURN
   END IF
   FETCH t500_cl INTO g_cno.*          #鎖住將被更改或取消的資料
   IF SQLCA.sqlcode THEN
      CALL cl_err(g_cno.cno01,SQLCA.sqlcode,0)      #資料被他人LOCK
