...
#
# Pattern name...: acor101.4gl
# Descriptions...: 出口成品申請備案清單列印
# Date & Author..: 00/07/26 By Carol
# Modify         : No.8964 04/07/07 By ching 品名規格長度未修改
...
#No.FUN-770006 --start--
   LET g_sql="coc10.coc_file.coc10,cna02.cna_file.cna02,coc03.coc_file.coc03,",
             "cod02.cod_file.cod02,cod03.cod_file.cod03,cob02.cob_file.cob02,",
             "cob021.cob_file.cob021,geb02.geb_file.geb02,",
             "cod05.cod_file.cod05,cod06.cod_file.cod06,cod07.cod_file.cod07,",
             "cod08.cod_file.cod08,cnc02.cnc_file.cnc02,coc01.coc_file.coc01,",
             "azi03.azi_file.azi03,azi04.azi_file.azi04"
   LET l_table = cl_prt_temptable('acor101',g_sql) CLIPPED
...
FUNCTION acor101_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,       #No.FUN-680069 SMALLINT
   ...
   OPEN WINDOW acor101_w AT p_row,p_col WITH FORM "aco/42f/acor101"
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW acor101_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('acor101',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW acor101_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0063
   ...
   DEFINE  l_cna02      LIKE cna_file.cna02                  #No.FUN-770006
     ...
     #End:FUN-980030
      ...
      LET l_sql = "SELECT coc01,coc02,coc10,coc03,cod02,cod03,cod05,cod06,cod07,",  #NO.MOD-490398
                 "  cod08,cob02,cob021,geb02,cnc02  FROM coc_file,cod_file,",
                 "OUTER geb_file,OUTER cob_file,OUTER cnc_file ",
                 " WHERE ",tm.wc CLIPPED," AND coc01=cod01 AND cod_file.cod03 = cob_file.cob01 ",
                 "   AND cocacti = 'Y' ",
                 "   AND cod_file.cod11 = geb_file.geb01 AND cod_file.cod12 = cnc_file.cnc01" CLIPPED
 
     PREPARE acor101_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
        EXIT PROGRAM 
     END IF
     DECLARE acor101_curs1 CURSOR FOR acor101_prepare1
 
#    CALL cl_outnam('acor101') RETURNING l_name        #No.FUN-770006 
#    START REPORT acor101_rep TO l_name                #No.FUN-770006
#    LET g_pageno = 0                                  #No.FUN-770006
     FOREACH acor101_curs1 INTO sr.*
       IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
 
#       OUTPUT TO REPORT acor101_rep(sr.*,'')  #NO.MOD-490398#No.FUN-770006
#No.FUN-770006 --start-- 
       ...
       IF tm.y='Y' THEN                                                        
          SELECT cob09 INTO sr.cod03 FROM cob_file                             
                 WHERE cob01=sr.cod03                                          
       END IF                                                                  
       EXECUTE insert_prep USING sr.coc10,l_cna02,sr.coc03,sr.cod02,sr.cod03,  
                                 sr.cob02,sr.cob021,sr.geb02,sr.cod05,sr.cod06,
                                 sr.cod07,sr.cod08,sr.cnc02,sr.coc01,t_azi03,  
                                 t_azi04 
#No.FUN-770006 --end--  
     END FOREACH
 
#    FINISH REPORT acor101_rep                            #No.FUN-770006
...
#No.FUN-770006 --start-- 
     ...
     CALL cl_prt_cs3('acor101','acor101',l_sql,g_str) 
#No.FUN-770006 --end-- 
END FUNCTION
#No.FUN-770006 --start--
{REPORT acor101_rep(sr)
  ...
  ORDER BY sr.coc03,sr.coc01,sr.cod02
...
#No.CHI-6A0004--end
    ...
    ON EVERY ROW
       PRINT COLUMN g_c[31],sr.cod02 USING '###&';#FUN-590118
 #NO.MOD-490398--begin
       IF tm.y = 'N' THEN
          PRINT COLUMN g_c[32],sr.cod03;
       ELSE
          SELECT cob09 INTO sr.cob09 FROM cob_file WHERE cob01=sr.cod03
          PRINT COLUMN g_c[32],sr.cob09;
       END IF
       PRINT COLUMN g_c[33],sr.cob02, #No.8964
             COLUMN g_c[40],sr.cob021,
             COLUMN g_c[34],sr.geb02[1,10],
             COLUMN g_c[35],sr.cod05 USING '##########&.&&&',
             COLUMN g_c[36],sr.cod06,
             COLUMN g_c[37],cl_numfor(sr.cod07,37,t_azi03),                  #No.CHI-6A0004
             COLUMN g_c[38],cl_numfor(sr.cod08,38,t_azi04),                  #No.CHI-6A0004
