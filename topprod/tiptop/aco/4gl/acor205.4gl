...
#
# Pattern name...: acor205.4gl
# Descriptions...: 年度出口成品報關單明細表
# Date & Author..: 05/01/13 by day
...
# Modify.........: No.FUN-770006 07/07/20 By zhoufeng 報表輸出改為Crystal Reports
...
#No.FUN-770006 --start--
   ...
   LET l_table = cl_prt_temptable('acor205',g_sql) CLIPPED 
...
FUNCTION acor205_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,         #No.FUN-680069 SMALLINT
   ...
   OPEN WINDOW acor205_w AT p_row,p_col WITH FORM "aco/42f/acor205"
 ...
 WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW acor205_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('acor205',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW acor205_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0063
          ...
          sr            RECORD
                               ...
                               msg    LIKE cno_file.cno08,        #No.FUN-680069 VARCHAR(20)
   ...
   DEFINE l_cnp05sum   LIKE cnp_file.cnp05,                      #No.FUN-770006
     ...
     LET l_sql = "SELECT '','','',cno01,cno07,cno031,cno04,cno06,cno08,cno10,",
                 "       cnp03,cnp04,cnp05,'',cno09 ",
                 " FROM cno_file,cnp_file WHERE cno01=cnp01 ",
                 "  AND cno03 = '1' AND ",tm.wc CLIPPED,
                 "  AND cno07 BETWEEN '",tm.y1,"' AND '",tm.y2,"' ",
                 "  AND cnoconf <> 'X' ",  #CHI-C80041
                 "ORDER BY cnp03,cno07 "
     PREPARE acor205_pre1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
        EXIT PROGRAM 
     END IF
     DECLARE acor205_cs1 CURSOR FOR acor205_pre1
 
#    CALL cl_outnam('acor205') RETURNING l_name        #No.FUN-770006
#    START REPORT r205_rep TO l_name                   #No.FUN-770006
#    LET g_pageno = 0                                  #No.FUN-770006
     FOREACH acor205_cs1 INTO sr.*
        IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
        END IF
        IF sr.cno031 = '2' AND sr.cno04 = '1' THEN
           CALL cl_getmsg('aco-191',g_lang) RETURNING sr.msg
        END IF
        IF sr.cno031 = '2' AND sr.cno04 = '2' THEN
           CALL cl_getmsg('aco-192',g_lang) RETURNING sr.msg
        END IF
        IF sr.cno031 = '2' AND sr.cno04 = '4' THEN
           CALL cl_getmsg('aco-206',g_lang) RETURNING sr.msg
        END IF
        IF sr.cno031 = '1' AND sr.cno04 = '1' THEN
           CALL cl_getmsg('aco-195',g_lang) RETURNING sr.msg
        END IF
        IF sr.cno031 = '1' AND sr.cno04 = '2' THEN
           CALL cl_getmsg('aco-196',g_lang) RETURNING sr.msg
        END IF
        IF sr.cno031 = '1' AND sr.cno04 = '4' THEN
           CALL cl_getmsg('aco-205',g_lang) RETURNING sr.msg
        END IF
        IF sr.cno031 = '3' THEN
           CALL cl_getmsg('aco-202',g_lang) RETURNING sr.msg
        END IF
...
#No.FUN-770006 --end--
     END FOREACH
#    FINISH REPORT r205_rep                      #No.FUN-770006
...
#No.FUN-770006 --start--                                                        
     ...
     CALL cl_prt_cs3('acor205','acor205',l_sql,g_str)                           
#No.FUN-770006 --end-- 
END FUNCTION
#No.FUN-770006 --start-- mark
{REPORT r205_rep(sr)
  ...
  FORMAT
   PAGE HEADER
      ...
      PRINT COLUMN 1,g_x[17] CLIPPED,l_cob021 CLIPPED,
   ...
   ON EVERY ROW
       ...
       ELSE
         LET l_cnp05sum = l_cnp05sum - sr.cnp05
       END IF
       PRINT COLUMN g_c[31], sr.cno07 CLIPPED,
             COLUMN g_c[32], sr.msg CLIPPED,
             COLUMN g_c[33], sr.cno06,
             COLUMN g_c[34], sr.cno09 CLIPPED,
             COLUMN g_c[35], sr.cnp05 USING '-----------&.&&',
