...
#
# Pattern name...: acoq510.4gl
# Descriptions...: 成品進出口各期異動統計量查詢
# Date & Author..: 05/01/17 By wujie
# Modify.........: 05/03/25 By Carrier No.MOD-530224 去掉cnk11
...
DEFINE
     tm  RECORD
        	...
        	yy   LIKE cnk_file.cnk03,         #No.FUN-680069 VARCHAR(4)
                mm   LIKE cnk_file.cnk04          #No.FUN-680069 VARCHAR(4)
        END RECORD,
    g_cod   RECORD
        ...
        END RECORD,
    g_cnk DYNAMIC ARRAY OF RECORD
                cnk03  LIKE cnk_file.cnk03,
                cnk04  LIKE cnk_file.cnk04,
                cnk05  LIKE cnk_file.cnk05,
                cnk06  LIKE cnk_file.cnk06,
                cnk07  LIKE cnk_file.cnk07,
                cnk08  LIKE cnk_file.cnk08,
                cnk09  LIKE cnk_file.cnk09,
                cnk10  LIKE cnk_file.cnk10,
               #cnk11  LIKE cnk_file.cnk11, #No.MOD-530224
                cnk12  LIKE cnk_file.cnk12
        END RECORD,
...
FUNCTION q510_cs()
   ...
   LET g_sql = g_sql clipped," ORDER BY cod03"
    ...
    DECLARE q510_cs                         #SCROLL CURSOR
            SCROLL CURSOR FOR q510_prepare
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         WHEN "exporttoexcel" #FUN-4B0002
            CALL cl_export_to_excel(ui.Interface.getRootNode(),base.TypeInfo.create(g_cnk),'','')
      END CASE
   END WHILE
END FUNCTION
...
DEFINE
    ...
    CASE p_flag
        WHEN 'N' FETCH NEXT     q510_cs INTO g_cod01,g_cod02,g_cod.cod03
        WHEN 'P' FETCH PREVIOUS q510_cs INTO g_cod01,g_cod02,g_cod.cod03
        WHEN 'F' FETCH FIRST    q510_cs INTO g_cod01,g_cod02,g_cod.cod03
        WHEN 'L' FETCH LAST     q510_cs INTO g_cod01,g_cod02,g_cod.cod03
        WHEN '/'
            IF (NOT mi_no_ask) THEN
               ...
               END IF
            END IF
            FETCH ABSOLUTE g_jump q510_cs INTO g_cod01,g_cod02,g_cod.cod03
            LET mi_no_ask = FALSE
    END CASE
...
FUNCTION q510_b_fill()              #BODY FILL UP
   ...
   LET l_sql=" SELECT cnk03,cnk04,cnk05,cnk06,cnk07,cnk08,cnk09,",
              "        cnk10,cnk12",   #No.MOD-530224
             "  FROM cnk_file ",
             " WHERE cnk01 ='",g_cod.coc03,"'",
             "   AND cnk02 ='",g_cod.cod03,"'",
             "   AND cnk03 >='",tm.yy,"'",
             "   AND cnk04 >='",tm.mm,"'",
             " ORDER BY cnk03,cnk04"
   PREPARE q510_pb FROM l_sql
   DECLARE q510_bcs
       CURSOR FOR q510_pb
   FOR g_cnt = 1 TO g_cnk.getLength()           #單身 ARRAY 乾洗
      INITIALIZE g_cnk[g_cnt].* TO NULL
   END FOR
   LET g_rec_b=0
   LET g_cnt = 1
   FOREACH q510_bcs INTO g_cnk[g_cnt].*
       IF g_cnt=1 THEN
	 ...
	 EXIT FOREACH
      END IF
   END FOREACH
   CALL g_cnk.deleteElement(g_cnt)
   LET g_rec_b = g_cnt- 1
...
FUNCTION q510_bp(p_ud)
   ...
   CALL cl_set_act_visible("accept,cancel", FALSE)
    DISPLAY ARRAY g_cnk TO s_cnk.* ATTRIBUTE(COUNT=g_rec_b,UNBUFFERED)  #No.MOD-480051
