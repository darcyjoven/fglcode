...
#
# Pattern name...: acor308.4gl
# Descriptions...: 手冊料件進口數量表
# Date & Author..: 03/01/16 By Carrier
#                : modified by carrier 03/02/21
# Modify.........: No:6887 03/08/06 By Wiky cno06 放寬欄位24
# Modify.........: NO.MOD-490398 04/11/22 BY DAY  add cno20,Customs No.,去cnp04
...
DEFINE   g_i       LIKE type_file.num5         #No.FUN-680069 SMALLINT #CKP
...
#目的:對于不同的手冊它要求打印的cob02,cnp06的內容是不同的(變動的打印頭)
          ...
          g_line_1    LIKE type_file.chr1000,      #No.FUN-680069 VARCHAR(300)
          g_cob02     LIKE type_file.chr1000,      #No.FUN-680069 VARCHAR(112)
          g_cnp06     LIKE type_file.chr1000,      #No.FUN-680069 VARCHAR(112)
          g_cnp05     LIKE type_file.chr1000,      #No.FUN-680069 VARCHAR(112)
          g_no        LIKE type_file.num5,         #No.FUN-680069 SMALLINT
          g_no1       LIKE type_file.num5,         #No.FUN-680069 SMALLINT
          g_t         LIKE type_file.num5,         #No.FUN-680069 SMALLINT #7 mutiple
          g_cnp ARRAY[300] OF RECORD
                cob02 LIKE cob_file.cob02,         #No.FUN-680069 VARCHAR(40)
                cnp06 LIKE cnp_file.cnp06,         #No.FUN-680069 VARCHAR(4)
                cnptot LIKE crb_file.crb10         #No.FUN-680069 DEC(12,3)
                END RECORD
MAIN
   OPTIONS
       INPUT NO WRAP
...
FUNCTION acor308_tm(p_row,p_col)
...
IF p_row = 0 THEN LET p_row = 5 LET p_col = 12 END IF
   ...
   OPEN WINDOW acor308_w AT p_row,p_col
      #CKP
      WITH FORM "aco/42f/acor308"
 ...
 CONSTRUCT BY NAME tm.wc ON cno06,cnp03  #MOD-490398
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW acor308_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
      EXIT PROGRAM
...
INPUT BY NAME tm.a,tm.more WITHOUT DEFAULTS HELP 1
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('acor308',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW acor308_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
FUNCTION acor308()
   DEFINE l_name    LIKE type_file.chr20,        #No.FUN-680069 VARCHAR(40) # External(Disk) file name
          l_name1   LIKE type_file.chr20,        #No.FUN-680069 VARCHAR(40)
#       l_time          LIKE type_file.chr8        #No.FUN-6A0063
       ...
       sr        RECORD
                            ...
                            cob02_s LIKE zaa_file.zaa08,         #No.FUN-680069 VARCHAR(112) #No.TQC-6A0085
     ...
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'acor308'
...
#####sr
     ...
     IF tm.a = '3' THEN
        LET l_sql = l_temp CLIPPED," AND (cno04 = '1' OR cno04 = '2')"
     END IF
 #     LET l_sql = "SELECT unique cno20,cno11,cno06,SUM(cnp05),0,'','','',0,0,0 ", #MOD-490398
      LET l_sql = "SELECT unique cno20,cno10,cno06,SUM(cnp05),0,'','','',0,0,0 ", #MOD-490398
                 "  FROM cno_file,cnp_file,cob_file  ",
                 " WHERE cno01 = cnp01 ",
                 "   AND cnoconf = 'Y' AND cnoacti = 'Y' ",
                  "   AND cob01 = cnp03 ",     #MOD-490398
                 "   AND cno03 = '2' AND cno031 = '2' ",
 ...
 #                 " ORDER BY cno20,cno11,cno06"     #MOD-490398
     ...
     PREPARE acor308_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
        EXIT PROGRAM 
     END IF
     DECLARE acor308_curs1 CURSOR FOR acor308_prepare1
######################
 #     LET l_sql = "SELECT unique cno11 ", #No.MOD-490398
      LET l_sql = "SELECT unique cno10 ", #No.MOD-490398
                 ...
                 " WHERE cno01 = cnp01 ",
                 "   AND cnoconf = 'Y' AND cnoacti = 'Y' ",
                  "   AND cnp03 = cob01 ",   #MOD-490398
                 "   AND cno03 = '2' AND cno031 = '2' ",
                 "   AND ",tm.wc CLIPPED
     LET l_sql = l_sql CLIPPED,' ',l_temp
     PREPARE acor308_prepare5 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
        EXIT PROGRAM 
     END IF
     DECLARE acor308_curs2 CURSOR FOR acor308_prepare5
#####cnp03
     LET l_sql = "SELECT unique cob02,cnp06,SUM(cnp05) ",
                 "  FROM cno_file,cnp_file,cob_file  ",
                 " WHERE cno01 = cnp01 ",
                 "   AND cnoconf = 'Y' AND cnoacti = 'Y' ",
                 "   AND cno03 = '2' AND cno031 = '2' ",
 ...
 #                 "   AND cno11 = ? ", #No.MOD-490398
                 ...
                 "   AND ",tm.wc CLIPPED
     LET l_sql = l_sql CLIPPED,' ',l_temp CLIPPED,
                 " GROUP BY cob02,cnp06 ",
                 " ORDER BY cob02,cnp06 "
     PREPARE acor308_prepare2 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
        EXIT PROGRAM 
     END IF
     DECLARE acor308_cno1 CURSOR FOR acor308_prepare2
#####當前cnp03,cno10下的cnp05
 ...
 #                 " WHERE cno01 = cnp01 AND cno11 = ? ", #No.MOD-490398
                 ...
                 "   AND ",tm.wc CLIPPED
     LET l_sql = l_sql CLIPPED,' ',l_temp
     PREPARE acor308_prepare4 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
        EXIT PROGRAM 
     END IF
     DECLARE acor308_cno3 CURSOR FOR acor308_prepare4
#####cnp05
     LET l_sql = "SELECT SUM(cnp05) ",
                 "  FROM cno_file,cnp_file,cob_file  ",
 #                 " WHERE cno01 = cnp01 AND cno11 = ? ", #No.MOD-490398
                  " WHERE cno01 = cnp01 AND cno10 = ? ", #No.MOD-490398
                 "   AND cno06 = ? AND cob02 = ? AND cnp06 = ?",
                 "   AND cnoconf = 'Y' AND cnoacti = 'Y' ",
                 "   AND cno03 = '2' AND cno031 = '2' ",
                  "   AND cnp03 = cob01 ",   #MOD-490398
                 "   AND ",tm.wc CLIPPED
     LET l_sql = l_sql CLIPPED,' ',l_temp
     PREPARE acor308_prepare3 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690109
        EXIT PROGRAM 
     END IF
     DECLARE acor308_cno2 CURSOR FOR acor308_prepare3
     CALL cl_outnam('acor308') RETURNING l_name
     LET g_number = 0
     FOR l_i = 1 TO 300
         INITIALIZE g_cnp[l_i] TO NULL
     END FOR
     LET l_i = 1
 START REPORT acor308_rep TO l_name
 # FOREACH acor308_curs2 INTO l_cno11 #No.MOD-490398
  FOREACH acor308_curs2 INTO l_cno10 #No.MOD-490398
   IF SQLCA.sqlcode  THEN
      CALL cl_err('foreach:',STATUS,1) EXIT FOREACH
   END IF
   LET l_i = g_number + 1
 #     FOREACH acor308_cno1 USING l_cno11 INTO g_cnp[l_i].* #No.MOD-490398
      FOREACH acor308_cno1 USING l_cno10 INTO g_cnp[l_i].* #No.MOD-490398
       IF SQLCA.sqlcode  THEN
          CALL cl_err('foreach:',STATUS,1) EXIT FOREACH
       END IF
       LET l_i = l_i + 1
     END FOREACH
     LET g_cnt = l_i - 1-g_number   #cnp03總數
     LET g_no = 1
     LET g_no1 = 1
     LET g_pageno = 0
     LET l_i = (g_cnt + 1)/ 7
   ...
   FOR l_i = 1 TO g_t STEP 7
         ...
         LET l_space2 = 0
         FOR l_j = 0 TO 6
             LET g_cob02=g_cob02 CLIPPED,' ',l_space1 SPACES,
                         g_cnp[l_i+l_j+g_number].cob02 CLIPPED
             LET g_cnp06=g_cnp06 CLIPPED,' ',l_space2 SPACES,
                         g_cnp[l_i+l_j+g_number].cnp06 CLIPPED
             LET l_len = 0
             LET l_space1 = 0
             LET l_len  = LENGTH(g_cnp[l_i+l_j+g_number].cob02[1,15])
             LET l_space1 = 15 - l_len
             IF l_space1 <> 15 THEN LET l_space3 = l_space1 END IF
             LET l_len1 = LENGTH(g_cnp[l_i+l_j+g_number].cnp06)
             LET l_space2 = 15 - l_len1
             IF l_len1 <>0 OR l_len <>0 THEN
                LET g_line_1 = g_line_1 CLIPPED,' ---------------'
             END IF
         END FOR
 #     FOREACH acor308_curs1 USING l_cno11 INTO sr.* #No.MOD-490398
      FOREACH acor308_curs1 USING l_cno10 INTO sr.* #No.MOD-490398
       IF SQLCA.sqlcode  THEN
          CALL cl_err('foreach:',STATUS,1) EXIT FOREACH
       END IF
       LET g_cnp05 = ''
       LET l_cnp05 = 0
       FOR l_j = 0 TO 6
 #           FOREACH acor308_cno2 USING sr.cno11,sr.cno06, #No.MOD-490398
            FOREACH acor308_cno2 USING sr.cno10,sr.cno06, #No.MOD-490398
                             g_cnp[l_i+l_j+g_number].cob02,g_cnp[l_i+l_j+g_number].cnp06
                  INTO l_cnp05
              IF SQLCA.sqlcode  THEN
                 CALL cl_err('foreach:',STATUS,1) EXIT FOREACH
              END IF
       ...
       LET sr.cob02_s = g_cob02
       LET sr.cnp06_s = g_cnp06
       LET sr.line = g_line_1
       IF l_i > (g_cnt+1) - 7 THEN
          IF l_i = g_cnt + 1 THEN
             LET sr.cob02_s = sr.cob02_s CLIPPED,' ',g_x[12] CLIPPED
          ELSE
             LET sr.cob02_s = sr.cob02_s CLIPPED,' ',l_space3 SPACES,g_x[12] CLIPPED
          END IF
          LET sr.cnp05_s = sr.cnp05_s CLIPPED,sr.l_amt USING " ---,---,--&.&&&"
          LET sr.line = sr.line CLIPPED,' ---------------'
       END IF
       OUTPUT TO REPORT acor308_rep(sr.*)
     END FOREACH
 END FOR
 LET g_number = g_number + g_cnt
 END FOREACH
 FINISH REPORT acor308_rep
   CALL cl_prt(l_name,g_prtway,g_copies,g_len)
END FUNCTION
 
REPORT acor308_rep(sr)
 ...
 #  ORDER BY sr.cno11,sr.cob02_s #No.MOD-490398
   ORDER BY sr.cno10,sr.cob02_s #No.MOD-490398
  ...
  FORMAT
   PAGE HEADER
         ...
         ELSE PRINT 'TO:',g_towhom CLIPPED;    #No.TQC-6A0085
         END IF
 ...
 #MOD-490398(BEGIN)
         ...
         PRINT g_x[2] CLIPPED,g_pdate ,' ',TIME,
               COLUMN g_len/2-(FGL_WIDTH(sr.cno20 CLIPPED)+9)/2,g_x[16] CLIPPED,sr.cno20 CLIPPED,' ',l_cna02 CLIPPED,'   ',   #No.TQC-6A0085
 #               g_x[15] CLIPPED,sr.cno11, #No.MOD-490398
                g_x[15] CLIPPED,sr.cno10 CLIPPED, #No.MOD-490398 #No.TQC-6A0085
               COLUMN g_len-7,g_x[3] CLIPPED,PAGENO USING '<<<'
#bug-490398(END)
         PRINT g_dash[1,g_len]
         PRINT COLUMN 1,g_x[11] CLIPPED,COLUMN 30,sr.cob02_s CLIPPED  #No:6887 #No.TQC-6A0085
         PRINT COLUMN 1,g_x[13] CLIPPED,COLUMN 30,sr.cnp06_s CLIPPED #No:6887 #No.TQC-6A0085
 ...
 #   BEFORE GROUP OF sr.cno11 #No.MOD-490398
    BEFORE GROUP OF sr.cno10 #No.MOD-490398
      ...
      LET n_cnp05 = 0
      SKIP TO TOP OF PAGE
   BEFORE GROUP OF sr.cob02_s
      LET g_no1 = 1
      SKIP TO TOP OF PAGE
 
   ON EVERY ROW
        PRINT COLUMN 1,g_no1 USING "###&",
              COLUMN 6,sr.cno06 CLIPPED,        #No.TQC-6A0085
              COLUMN 30,sr.cnp05_s CLIPPED      #No:6887        #No.TQC-6A0085
        LET g_no1 = g_no1 + 1
#打印合計內容
 #   AFTER GROUP OF sr.cno11   #No.MOD-490398
    AFTER GROUP OF sr.cno10   #No.MOD-490398
       LET t_cnp05 = ''
       LET l_cnp05 = 0
       FOR l_j = 0 TO 6
       #sr.g_t等于g_number表示此手冊的g_cnp開始的下標
           LET l_tmp = sr.l_no + l_j +sr.g_t #計算所要用到的g_cnp陣列的下標
           LET l_cnp05 = 0
 #           FOREACH acor308_cno3 USING sr.cno11, #No.MOD-490398
            FOREACH acor308_cno3 USING sr.cno10, #No.MOD-490398
                             g_cnp[l_tmp].cob02,g_cnp[l_tmp].cnp06
                  INTO l_cnp05
              IF SQLCA.sqlcode  THEN
                 CALL cl_err('foreach:',STATUS,1) EXIT FOREACH
              END IF
           END FOREACH
           IF cl_null(l_cnp05) THEN LET l_cnp05 = 0 END IF
           LET n_cnp05 = n_cnp05 + l_cnp05  #計算小計的合計
 ...
 #這所以要減去一個l_cnp05,因為存在著不同手冊之間有相同的cnp03，多
