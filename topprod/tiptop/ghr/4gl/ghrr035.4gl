...
#
# Pattern name...: ghrr035.4gl
# Descriptions...: 用人单位用工备案与社会保险增员申报花名册
# Date & Author..: 13/10/10   by shenran
...
MAIN
   ...
   LET g_sql ="hrat03.type_file.chr100,",
              "hrat02.hrat_file.hrat02,hrat13.hrat_file.hrat13,",
              "hrbf08.hrbf_file.hrbf08,",
              "hrbf09.hrbf_file.hrbf09,hrdxa08.hrdxa_file.hrdxa08 "

   LET l_table = cl_prt_temptable('ghrr035',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN        # If background job sw is off
      CALL ghrr035_tm(0,0)        # Input print condition
   ELSE
      CALL ghrr035()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
END MAIN
 
FUNCTION ghrr035_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,        #No.FUN-680121 SMALLINT
  ...
  OPEN WINDOW ghrr035_w AT p_row,p_col WITH FORM "ghr/42f/ghrr035"
  ...
  WHILE TRUE
    ...
    IF g_bgjob = 'Y' THEN
       ...
       ELSE
          ...
          CALL cl_cmdat('ghrr035',g_time,l_cmd)    # Execute cmd at later time
       END IF
       CLOSE WINDOW ghrr035_w
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
       EXIT PROGRAM
    END IF
...
FUNCTION ghrr035()
   ...
   LET tm.wc = cl_replace_str(tm.wc,"bmonth","hrct05")
       ...
       LET l_sql = " SELECT DISTINCT hrat03,hrat02,hrat13,hrbf08, ",
                                    " hrbf09,hrdxa08 FROM hrat_file,hrbf_file,hrdxa_file ",
                   "  WHERE hratid = hrbf02   ",
                   "  AND   hratid = hrdxa02  ",
                   "  AND   YEAR(hrbf08)<=  ",g_byear," ",
                   "  AND   MONTH(hrbf08)<= ",g_bmonth," ",
                   "  AND   YEAR(hrbf09)>=  ",g_byear," ",
                   "  AND   MONTH(hrbf09)>= ",g_bmonth," ",
                   "  AND   hrdxa01 IN (select hrct11 from hrct_file where ",tm.wc CLIPPED, ") "
       LET l_sql = l_sql," ORDER BY hrat03 "
   
      PREPARE r035_p FROM l_sql 
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('ghrr035_p:',SQLCA.sqlcode,1)
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
         EXIT PROGRAM
      END IF
      DECLARE r035_curs CURSOR FOR r035_p
      
      FOREACH r035_curs INTO sr.*
           IF SQLCA.sqlcode != 0 THEN
    ...
    CALL cl_prt_cs3('ghrr035','ghrr035',l_sql,g_str)
