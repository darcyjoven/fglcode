...
#
# Pattern name...: ghri0224.4gl
# Descriptions...: 下月末待签合同 
...
DEFINE 
     ...
     g_hrat           DYNAMIC ARRAY OF RECORD    #程式變數(Program Variables)
        ...
        hrbf08       LIKE type_file.dat,        #合同生效日期
        hrbf09       LIKE type_file.dat         #合同失效日期
                    END RECORD,
...
DEFINE p_row,p_col   LIKE type_file.num5         #No.FUN-680102 SMALLINT
   ...
   END IF
    ...
    OPEN WINDOW i0224_w AT p_row,p_col WITH FORM "ghr/42f/ghri022_a"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
    
    CALL cl_ui_init()
    CALL cl_set_comp_visible("hratid",FALSE)
    
    LET g_wc2 = '1=1' CALL i0224_b_fill(g_wc2,'')
    CALL i0224_menu()
    CLOSE WINDOW i0224_w                 #結束畫面
...
FUNCTION i0224_b_askkey()
   ...
   CALL g_hrat.clear()
    CONSTRUCT g_wc2 ON hrat03,hrat01,hrat02,hrat17,hrat04,hrat05,hrat25,hrat19,
         ...
         FROM s_hrat[1].hrat03,s_hrat[1].hrat01,s_hrat[1].hrat02,s_hrat[1].hrat17,
              ...
              s_hrat[1].hrat22,s_hrat[1].hrat42,s_hrbf[1].hrbf08,s_hrbf[1].hrbf09
   ...
   IF INT_FLAG THEN
      ...
      RETURN
   END IF
    CALL i0224_b_fill(g_wc2,'')
END FUNCTION
 
FUNCTION i0224_b_fill(p_wc2,p_start)              #BODY FILL UP
DEFINE
 ...
 IF cl_null(p_start) THEN 
    LET g_sql =
        ...
        "         GROUP BY hrbf02 ",
        "         HAVING MAX(hrbf09) <= LAST_DAY(ADD_MONTHS(SYSDATE,+1))",
        "        ) file1 ",
        " WHERE ", p_wc2 CLIPPED,                     #單身
        "   AND hratconf = 'Y' AND hrad02=hrat19 AND hrad01<>'003'",
        "   AND hratacti = 'Y' ",
        "   AND hratid = file1.hrbf02 ",
        " ORDER BY hrat01 "

     
    PREPARE i0224_pb FROM g_sql
    DECLARE hrat_curs CURSOR FOR i0224_pb
    ...
    FOREACH hrat_curs INTO g_hrat_t[g_cnt].*   #單身 ARRAY 填充
        ...
        SELECT hrbf08,hrbf09 INTO g_hrat_t[g_cnt].hrbf08,g_hrat_t[g_cnt].hrbf09 FROM hrat_file,hrbf_file
        WHERE hrat01 = g_hrat_t[g_cnt].hrat01 AND hrbf02=hratid  
    ...
    CALL g_hrat.deleteElement(g_cnt)
      # add by shenran 	start      
        ...
        CALL i0224_set_data(0,g_turn)
        DISPLAY g_turn TO cnt1 
      ELSE 
        CALL i0224_set_data(p_start,g_turn)
        DISPLAY g_turn TO cnt1  
 END IF  
    LET g_cnt = 0
...
FUNCTION i0224_assign()
 ...
 DEFINE l_err    LIKE  type_file.chr1000
   ...
   OPEN WINDOW i0224_w1 WITH FORM "ghr/42f/ghri0221"
 ...
 #  LET g_hrbf.hrbf10 = g_today
   ...
   WHILE TRUE
      ...
      INPUT BY NAME g_hrbf.hrbf03,g_hrbf.hrbf04,g_hrbf.hrbf05,
        ...
        AFTER FIELD hrbf04
           IF NOT cl_null(g_hrbf.hrbf04) THEN
           	  CALL i0224_hrbf04() RETURNING l_des
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW i0224_w1 
         RETURN
      END IF  
      EXIT WHILE  
   END WHILE
...
#      	 END IF 
   ...
   CLOSE WINDOW i0224_w1
   CALL i0224_b_fill(g_wc2,'')
END FUNCTION


FUNCTION i0224_hrbf04()
   ...
   RETURN l_hrbe02
END FUNCTION
FUNCTION i0224_set_data(l_cnt,l_turn)
...
FUNCTION i0224_tz()  #add by shenran NO.130707
