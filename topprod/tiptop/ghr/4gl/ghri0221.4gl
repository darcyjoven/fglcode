...
DEFINE p_row,p_col   LIKE type_file.num5         #No.FUN-680102 SMALLINT
   ...
   END IF
    ...
    OPEN WINDOW i0221_w AT p_row,p_col WITH FORM "ghr/42f/ghri022_a"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
    
    CALL cl_ui_init()
    CALL cl_set_comp_visible("hratid",FALSE)
    
    LET g_wc2 = '1=1' CALL i0221_b_fill(g_wc2,'')
    CALL i0221_menu()
    CLOSE WINDOW i0221_w                 #結束畫面
...
FUNCTION i0221_b_askkey()
   ...
   IF INT_FLAG THEN
      ...
      RETURN
   END IF
    CALL i0221_b_fill(g_wc2,'')
END FUNCTION
 
FUNCTION i0221_b_fill(p_wc2,p_start)              #BODY FILL UP
DEFINE
 ...
 IF cl_null(p_start) THEN 
    LET g_sql =
        ...
        "   AND hrad01 <> '003'",
        "   AND hratacti = 'Y' ",
        "   AND hratid NOT IN(SELECT hrbf02 FROM hrbf_file,hrbe_file ",
        "                      WHERE hrbf04 = hrbe01 ",
        "                        AND hrbe03 = '001') ",
        "   AND hrat19 NOT LIKE '3%'                 ",
        " ORDER BY hrat01 "
        
    PREPARE i0221_pb FROM g_sql
    DECLARE hrat_curs CURSOR FOR i0221_pb
    ...
    CALL g_hrat_t.deleteElement(g_cnt)
       ...
       CALL i0221_set_data(0,g_turn)
       DISPLAY g_turn TO cnt1 
      ELSE 
        CALL i0221_set_data(p_start,g_turn)
        DISPLAY g_turn TO cnt1 
 END IF 
    LET g_cnt = 0
      ...
      INPUT BY NAME g_hrbf.hrbf03,g_hrbf.hrbf04,g_hrbf.hrbf05,
        ...
        AFTER FIELD hrbf04
           IF NOT cl_null(g_hrbf.hrbf04) THEN
           	  CALL i0221_hrbf04() RETURNING l_des
   ...
   FOR l_n=1 TO g_turn 
      IF g_hrat[l_n].chk = 'Y' THEN 
         ...
         IF cl_null(g_hrbf.hrbf08) THEN
            SELECT hrat25 INTO g_hrbf.hrbf08 FROM hrat_file 
             WHERE hratid = g_hrbf.hrbf02 
         END IF 
         IF NOT cl_null(g_hrbf.hrbf05) THEN
...
FUNCTION i0221_hrbf04()
   ...
   RETURN l_hrbe02
END FUNCTION
FUNCTION i0221_set_data(l_cnt,l_turn)
...
FUNCTION i0221_tz()  #add by shenran NO.130707
