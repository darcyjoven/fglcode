...
MAIN
  ...
  DEFINE i,l_i,l_j,l_k      INT                  #add by zhangbo for test
     ...
     IF Formula.getLength()>1 THEN
        ...
        IF g_success='Y' THEN
           ...
           FOR l_i=1 TO Formula.getLength()-1
              INSERT INTO hrdka_file VALUES (g_argv1,l_i,Formula[l_i].id,Formula[l_i].mark,Formula[l_i].context,
                                             Formula[l_i].lineNumber,Formula[l_i].length,Formula[l_i].beginX,
        ...
        IF g_success='Y' THEN
         ...
         IF g_hrdk.hrdk05 = '004' THEN LET g_hrdk.hrdk17 = g_hrdk.hrdk17," SELECT  CEIL(",g_hrdk.hrdk16,"*POWER(10,",g_hrdk.hrdk06,"))/POWER(10,",g_hrdk.hrdk06,") INTO ",g_hrdk.hrdk16," FROM dual;" END IF 
           UPDATE hrdk_file SET hrdk14=g_hrdk.hrdk14,hrdk15=g_hrdk.hrdk15,hrdk17=g_hrdk.hrdk17,hrdkud01=g_hrdk.hrdkud01,hrdkud02 = ''
...
DEFINE l_ta_hrdl01   LIKE  hrdl_file.ta_hrdl01
DEFINE l_ta_hrdl02   LIKE hrdl_file.ta_hrdl02
...
DEFINE l_desc            STRING         #add by zhangbo131115
       ...
       LET l_dec[6].name="FN_TONUMBER"
       LET l_dec[6].des="字符转数值:FN_TONUMBER(字符):将数值文本转为数值"	
       LET l_dec[7].name="FN_GETTURNOVER"
       LET l_dec[7].des="取部门营业额:FN_GETTURNOVER(部门编码):取部门营业额"	
...
DEFINE l_desc            STRING
       ...
       LET l_date[16].des="DIFFDATE(开始日期,结束日期,返回类型,参照行事例):根据返回类型,在日期区间内取值\n",
                          "返回类型=1:返回两个日期中间隔天数;\n",
       ...
       #add by yinbq 20150120 for 添加获取日期对应薪资月份数对应数值函数
       LET l_date[21].name="FN_DateOFXZY"
       LET l_date[21].des="FN_DateOFXZY(日期)"
       #add by yinbq 20150120 for 添加获取日期对应薪资月份数对应数值函数
...
FUNCTION Init()
  ...
  LET DW.s_ln   = 1 
  #20150229 add by yinbq for 重新设定窗体显示内容行数
  IF Formula.getLength() = 0 THEN 
    SELECT MAX(hrdka06) INTO DW.height FROM hrdka_file WHERE hrdka01=g_argv1
  ELSE
    LET DW.height = Formula[Formula.getLength()].lineNumber
  END IF 
...
DEFINE l_str1,l_str2 STRING 
  ...
  LET ln=myCursor.Ln    #光标行数
  LET pos=myCursor.Pos  #光标位置
  LET g_str=''          #公式全文本
  LET l_str1=''         #光标前的文本
  LET l_str2=''         #光标后的文本
  ...
  IF Formula.getLength() > 1 THEN
    #游标在中间位置
    IF Formula.getLength() - 1 >= pos THEN  #如果公式文本内容数大于光标位置，即光标前后都有文本
      FOR l_i=1 TO pos-1
      ...
      FOR l_i=pos TO Formula.getLength()-1
