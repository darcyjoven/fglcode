...
#
# Pattern name...: ghrr025.4gl
# Descriptions...: 员工津贴信息明细表打印
...
#DEFINE g_hrao10    LIKE hrao_file.hrao10  #modified by yeap NO.130929 
...
MAIN
   ...
   LET g_sql ="hrat01.hrat_file.hrat01,hrat02.hrat_file.hrat02,",
              "hrat03.type_file.chr100,hrat04.hrat_file.hrat04,",
              "hrat04_1.type_file.chr100,hrat05.type_file.chr100,",
              "hrat21.type_file.chr100,hrcq03.hrcq_file.hrcq03,",
              "hrcq04.type_file.chr100,hrcq05.type_file.chr100,",
              "hrcq06.hrcq_file.hrcq06,hrcq07.hrcq_file.hrcq07,",
              "year.type_file.num5,month.type_file.num5"

   LET l_table = cl_prt_temptable('ghrr025',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN        # If background job sw is off
      CALL ghrr025_tm(0,0)        # Input print condition
   ELSE
      CALL ghrr025()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
END MAIN
 
FUNCTION ghrr025_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,        #No.FUN-680121 SMALLINT
  ...
  OPEN WINDOW ghrr025_w AT p_row,p_col WITH FORM "ghr/42f/ghrr025"
  ...
  WHILE TRUE
    CONSTRUCT BY NAME tm.wc ON hrat03,hrat04,byear,bmonth
       ...
       BEFORE CONSTRUCT
          CALL cl_qbe_init()
          DISPLAY YEAR(g_today),MONTH(g_today) TO FORMONLY.byear,FORMONLY.bmonth
    ...
    IF g_bgjob = 'Y' THEN
       ...
       ELSE
          ...
          CALL cl_cmdat('ghrr025',g_time,l_cmd)    # Execute cmd at later time
       END IF
       CLOSE WINDOW ghrr025_w
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
       EXIT PROGRAM
    END IF
...
FUNCTION ghrr025()
   DEFINE l_name    LIKE type_file.chr20,             #No.FUN-680121 VARCHAR(20)# External(Disk) file name
          ...
          sr        RECORD
                     ...
                     hrcq04      LIKE type_file.chr100,
                     hrcq05      LIKE type_file.chr100,
   ...
   IF tm.wc.getIndexOf("hrat04",1) THEN 
      ...
      LET l_sql = " SELECT DISTINCT hrat01,hrat02,hraa12,hrat04,hrao02,hras04,hrat21,hrcq03,hrbo03,hrbm04,hrcq06,hrcq07,YEAR(hrcq03),MONTH(hrcq03) ",
       	          ...
       	          "        hrcq_file LEFT JOIN hrbm_file ON hrbm03 = hrcq05   LEFT JOIN hrbo_file ON hrbo02 = hrcq04  ", 
       	          "  WHERE hratid = hrcq02 AND hratacti = 'Y' ",
       	          "    AND hratconf = 'Y' AND hrcqacti = 'Y' ",
   	              "    AND hrcqconf = 'Y'  ", 
       	          "    AND hrat03 = '",g_hrao00 CLIPPED,"' AND hrat04 IN ('",g_hrao10 CLIPPED,
       	          "')  AND year(hrcq03) ='",g_byear CLIPPED,"' AND MONTH(hrcq03) ='",g_bmonth CLIPPED,"' ",
       	          "    AND hrat19 <> '3001'"
       	              
      LET l_sql = l_sql," ORDER BY hrat01,YEAR(hrcq03),MONTH(hrcq03) "
      PREPARE r018_p FROM l_sql 
           IF SQLCA.sqlcode != 0 THEN
              CALL cl_err('ghrr025_p:',SQLCA.sqlcode,1)
              CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
              EXIT PROGRAM
              END IF
      ...
      PREPARE r025_p1 FROM l_sql 
           IF SQLCA.sqlcode != 0 THEN
              CALL cl_err('ghrr025_p1:',SQLCA.sqlcode,1)
              CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
              EXIT PROGRAM
           END IF
      DECLARE r025_curs1 CURSOR FOR r025_p1 
     
      FOREACH r025_curs1 INTO sr1.*
           IF SQLCA.sqlcode != 0 THEN
              CALL cl_err('foreach:',SQLCA.sqlcode,1)
              EXIT FOREACH
           END IF
           ...
           LET l_sql = " SELECT DISTINCT hrat01,hrat02,hraa12,hrat04,hrao02,hras04,hrat21,hrcq03,hrbo03,hrbm04,hrcq06,hrcq07,YEAR(hrcq03),MONTH(hrcq03) ",
       	   ...
       	   PREPARE r018_p2 FROM l_sql 
                IF SQLCA.sqlcode != 0 THEN
                   CALL cl_err('ghrr025_p2:',SQLCA.sqlcode,1)
                   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
                   EXIT PROGRAM
                END IF
  ...
  ELSE
      ...
      PREPARE r018_p3 FROM l_sql 
           IF SQLCA.sqlcode != 0 THEN
              CALL cl_err('ghrr025_p3:',SQLCA.sqlcode,1)
              CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
              EXIT PROGRAM
              END IF
   ...
   END IF 
    ...
    CALL cl_prt_cs3('ghrr025','ghrr025',l_sql,g_str)
