...
#
# Pattern name...: ghrr202.4gl
# Descriptions...: 员工司龄分布统计表
...
MAIN
   ...
   LET g_sql ="tc_hrat01.tc_hrat_file.tc_hrat01,",
              ...
              "hj.type_file.num15_3,",
              "zj.type_file.num15_3"
             
              
   LET l_table = cl_prt_temptable('ghrr202',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN        # If background job sw is off
      CALL ghrr202_tm(0,0)        # Input print condition
   ELSE
      CALL ghrr202()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
END MAIN
 
FUNCTION ghrr202_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,        #No.FUN-680121 SMALLINT
  ...
  OPEN WINDOW ghrr201_w AT p_row,p_col WITH FORM "ghr/42f/ghrr202"
...
FUNCTION ghrr202()
   DEFINE l_name    LIKE type_file.chr20,             #No.FUN-680121 VARCHAR(20)# External(Disk) file name
          ...
          sr        RECORD
                     ...
                     hj LIKE type_file.num15_3,                            
                     zj LIKE type_file.num15_3                                                 
                    END RECORD

   DEFINE l_tc_hrat01 LIKE tc_hrat_file.tc_hrat01
   DEFINE l_tc_hrat02 LIKE tc_hrat_file.tc_hrat02
...
--
   ...
   LET l_sql = "select tc_hrat01,tc_hrat02,hrat17,hrat21,sum(c1),sum(c2),sum(c3),sum(c4),sum(c5),sum(c6),0,0 from ", 
               " (select tc_hrat01,tc_hrat02,hrat17,hrat21,",
               " case when hrat25>=add_months(to_date(to_char(tc_hrat01)||'/'||to_char(tc_hrat02)||'/01','YYYY/MM/DD'),-3) then 1 else 0 end as c1,",
               " case when hrat25>=add_months(to_date(to_char(tc_hrat01)||'/'||to_char(tc_hrat02)||'/01','YYYY/MM/DD'),-12) ", 
               " and  hrat25<add_months(to_date(to_char(tc_hrat01)||'/'||to_char(tc_hrat02)||'/01','YYYY/MM/DD'),-3)  then 1 else 0 end as c2,",
               " case when hrat25>=add_months(to_date(to_char(tc_hrat01)||'/'||to_char(tc_hrat02)||'/01','YYYY/MM/DD'),-36) ", 
               " and  hrat25< add_months(to_date(to_char(tc_hrat01)||'/'||to_char(tc_hrat02)||'/01','YYYY/MM/DD'),-12) then 1 else 0 end as c3,",
               " case when hrat25>=add_months(to_date(to_char(tc_hrat01)||'/'||to_char(tc_hrat02)||'/01','YYYY/MM/DD'),-60)",
               " and  hrat25< add_months(to_date(to_char(tc_hrat01)||'/'||to_char(tc_hrat02)||'/01','YYYY/MM/DD'),-36)  then 1 else 0 end as c4,",
               " case when hrat25>=add_months(to_date(to_char(tc_hrat01)||'/'||to_char(tc_hrat02)||'/01','YYYY/MM/DD'),-120)",
               " and  hrat25< add_months(to_date(to_char(tc_hrat01)||'/'||to_char(tc_hrat02)||'/01','YYYY/MM/DD'),-60) then 1 else 0 end as c5,",
               " case when hrat25< add_months(to_date(to_char(tc_hrat01)||'/'||to_char(tc_hrat02)||'/01','YYYY/MM/DD'),-120) then 1 else 0 end as c6", 
      ...
      FOREACH r100_curs INTO sr.*
          ...
          LET sr.hj=sr.c1+sr.c2+sr.c3+sr.c4+sr.c5+sr.c6

          IF l_tc_hrat01<>sr.tc_hrat01 and l_tc_hrat02<>sr.tc_hrat01 THEN
            LET l_tc_hrat01=sr.tc_hrat01
            LET l_tc_hrat02=sr.tc_hrat02
            LET sr.zj=0
          END IF 
    ...
    LET l_rep_name="ghr/ghrr202.cpt&p2=",tm.s_year,"&p3=",tm.s_month  
