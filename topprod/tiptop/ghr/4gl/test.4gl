...
#No.TQC-5C0013 --end--  
 DEFINE  l_msg    STRING     #add by zhangbo130607
 DEFINE  l_n,l_n1      LIKE  type_file.num5     #add by zhangbo130608
 DEFINE  l_sql         STRING                   #add by zhangbo130608
    ...
    ON ACTION choose_itemno  #Control + H 開料件編號窗,只對元件料號欄位有效
       CASE 
         WHEN INFIELD(bmb03) 
           ...
           #CALL FGL_DIALOG_SETSELECTION(1,l_pos)   #add by zhangbo130607
#FUN-AA0059 --Begin--
...
#FUN-AA0059 --End--
           ...
           #add by zhangbo130608---begin
           LET l_n=0
           LET l_n1=0
           LET l_sql="SELECT LENGTH('",l_str CLIPPED,"') FROM dual "
           PREPARE test_ln FROM l_sql
           EXECUTE test_ln INTO l_n
           LET l_sql="SELECT LENGTH('",l_ima01 CLIPPED,"') FROM dual "
           PREPARE test_ln1 FROM l_sql
           EXECUTE test_ln1 INTO l_n1
           IF l_n < l_pos THEN LET l_n = l_pos END IF
           IF l_pos = 1 THEN
              LET g_input.bmb03 = l_ima01,l_str
           ELSE
             LET l_sql="SELECT substr('",l_str CLIPPED,"',1,",l_pos-1,")||'",l_ima01 CLIPPED,
                       "'||substr('",l_str,"',",l_pos,",",l_n,")",
                       " FROM dual"
              PREPARE test_result FROM l_sql
              EXECUTE test_result INTO g_input.bmb03
              #LET g_input.bmb03 = l_str[1,l_pos-1],l_ima01,l_str[l_pos,l_n]
           END IF
           CALL FGL_DIALOG_SETCURSOR(l_pos+l_n1)
           #add by zhangbo130608  
...
DEFINE l_length  LIKE type_file.num10          #No.FUN-680102INTEGER 
DEFINE l_length1 LIKE type_file.num5           #add by zhangbo130607
...
#      FROM sma_file                                                                              #TQC-9B0022 Mark
      IF l_pos <= l_length THEN                                      #add by zhangbo光标不在txt字串最后
         LET l_str = l_str[1,l_pos-1],l_return,l_str[l_pos,l_length]                                  #TQC-9B0022 Add
      ELSE                                                           #add by zhangbo130607
         LET l_str = l_str[1,l_pos-1],l_return                       #add by zhangbo130607
      END IF                                                         #add by zhangbo130607 
