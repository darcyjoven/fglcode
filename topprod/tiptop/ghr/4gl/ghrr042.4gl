...
#
# Pattern name...: ghrr042.4gl
# Descriptions...: 社保专员信息提醒
...
{   display "l_hraa  =",l_hraa
   ...
   LET g_sql =" CR01.hrao_file.hrao02,",
              ...
              " CR08.hrao_file.hraoud02"
   LET l_table = cl_prt_temptable('ghrr042',g_sql) CLIPPED
   ...
   LET l_table1 = cl_prt_temptable('ghrr042_a',g_sql) CLIPPED
   ...
   LET l_table2 = cl_prt_temptable('ghrr042_b',g_sql) CLIPPED
   ...
   LET g_sql =" CR31.hrao_file.hrao02,",
              ...
              " CR37.hrao_file.hraoud02"

   LET l_table3 = cl_prt_temptable('ghrr042_c',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN        # If background job sw is off
      CALL ghrr042_tm(0,0)        # Input print condition
   ELSE
      ...
      END IF}
      CALL ghrr042()            # Read data and create out-file
   END IF
   
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
END MAIN
 
FUNCTION ghrr042_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,        
  ...
  LET p_row = 6 
  LET p_col = 20
  OPEN WINDOW ghrr042_w AT p_row,p_col WITH FORM "ghr/42f/ghrr042"
  ...
  WHILE TRUE
    ...
    IF g_bgjob = 'Y' THEN
       ...
       ELSE
          ...
          CALL cl_cmdat('ghrr042',g_time,l_cmd)    # Execute cmd at later time
       END IF
       CLOSE WINDOW ghrr042_w
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
       EXIT PROGRAM
    END IF
...
DEFINE l_img_blob     LIKE type_file.blob
   ...
   LET l_sql = " SELECT hrao02,hrat01,hrat02,hras04,a.hrag07,hrat25,b.hrag07,case hraoud02 when '001' then '萧山' when '002' then '海宁' when '003' then '非基地' end",
               " FROM hrat_file",
               " LEFT JOIN hrao_file ON hrao01=hrat04",
               " LEFT JOIN hras_file ON hras01=hrat05",
               " LEFT JOIN hrag_file a ON a.hrag01='337' AND a.hrag06=hrat21",
               " LEFT JOIN hrag_file b ON b.hrag01='340' AND b.hrag06=hrat68 where ",tm.wc CLIPPED
 
   LET l_sql=l_sql CLIPPED,"  ORDER BY hrat01 "   
   PREPARE ghrr042_p FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('ghrr042_p:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM
   END IF
   DECLARE ghrr042_curs CURSOR FOR ghrr042_p
   ...
   PREPARE ghrr042_p1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('ghrr042_p1:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM
   END IF
   DECLARE ghrr042_curs1 CURSOR FOR ghrr042_p1 
   ...
   PREPARE ghrr042_p2 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('ghrr042_p2:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM
   END IF
   DECLARE ghrr042_curs2 CURSOR FOR ghrr042_p2
   
   LET tm.wc = cl_replace_str(tm.wc,'hrazdate', 'hrbhdate')
   
   LET l_sql = " SELECT hrao02,hrat01,hrat02,hras04,a.hrag07,hrat77,case hraoud02 when '001' then '萧山' when '002' then '海宁' when '003' then '非基地' end",
               " FROM hrbh_file",
   ...
   LET l_sql=l_sql CLIPPED,"  ORDER BY hrat01 "   
   PREPARE ghrr042_p3 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('ghrr042_p3:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM
   END IF
   DECLARE ghrr042_curs3 CURSOR FOR ghrr042_p3
   
   INITIALIZE sr.* TO NULL
   FOREACH ghrr042_curs INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
   ...
   FOREACH ghrr042_curs1 INTO sr1.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
   ...
   FOREACH ghrr042_curs2 INTO sr2.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
   ...
   FOREACH ghrr042_curs3 INTO sr3.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
  ...
  {           
   ...
   CALL cl_prt_cs3('ghrr042','ghrr042',l_sql,g_str)}
    LET l_rep_name="ghr/SheBao.cpt" #&p2=",tm.s_year,"&p3=",tm.s_month  
    LET l_sql= " SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED 
    ,"|"," SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,"|",
              " SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,"|",
              " SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED
