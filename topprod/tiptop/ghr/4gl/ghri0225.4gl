...
#
# Pattern name...: ghri0225.4gl
# Descriptions...: 待新签合同 
# Date & Author..: 14/11/17 By zhuzw
...
DEFINE p_row,p_col   LIKE type_file.num5         #No.FUN-680102 SMALLINT
   ...
   END IF
    ...
    OPEN WINDOW i0225_w AT p_row,p_col WITH FORM "ghr/42f/ghri022_e"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
    
    CALL cl_ui_init()
    CALL cl_set_comp_visible("hratid",FALSE)
    
    LET g_wc2 = '1=1' CALL i0225_b_fill(g_wc2,'')
    CALL i0225_menu()
    CLOSE WINDOW i0225_w                 #結束畫面
...
FUNCTION i0225_b_askkey()
   ...
   IF INT_FLAG THEN
      ...
      RETURN
   END IF
    CALL i0225_b_fill(g_wc2,'')
END FUNCTION
 
FUNCTION i0225_b_fill(p_wc2,p_start)              #BODY FILL UP
DEFINE
 ...
 IF cl_null(p_start) THEN 
    LET g_sql =
      ...
      "       hrat19,hrat22,hrat42,hrbf04,hrbf10,hrbf08,hrbf09,hrbf11 ",
      "  FROM hrbf_file, hrat_file,hrad_file ",
      " WHERE ", p_wc2 CLIPPED,                     #單身
      "   AND hrbf02=hratid AND hrad02=hrat19 AND hrad01<>'003'",
      "   and hrbf01 in (select hrbf01 from hrbf_file,(select hrbf02,max(hrbf08) hrbf08 from hrbf_file ",
      " group by hrbf02) a                  ",
      "  WHERE  a.hrbf02 = hrbf_file.hrbf02  ",
      "    AND  a.hrbf08 = hrbf_file.hrbf08) ",
      " ORDER BY hrat01,hrbf10 "
        
    PREPARE i0225_pb FROM g_sql
    DECLARE hrat_curs CURSOR FOR i0225_pb
    ...
    CALL g_hrat_t.deleteElement(g_cnt)
       ...
       CALL i0225_set_data(0,g_turn)
       DISPLAY g_turn TO cnt1 
      ELSE 
        CALL i0225_set_data(p_start,g_turn)
        DISPLAY g_turn TO cnt1 
 END IF 
    LET g_cnt = 0
...
FUNCTION i0225_bp(p_ud)
   ...
   DISPLAY ARRAY g_hrat TO s_hrat.* ATTRIBUTE(COUNT=g_turn)
      ...
      ON ACTION bght
         LET g_action_choice = 'bght'
         EXIT DISPLAY
      ON ACTION zzht
         LET g_action_choice = 'zzht'
         EXIT DISPLAY                                    
      # No.FUN-530067 --start--
      AFTER DISPLAY
         CONTINUE DISPLAY
...
FUNCTION i0225_assign()
 ...
 DEFINE l_str   STRING
   ...
   OPEN WINDOW i0225_w1 WITH FORM "ghr/42f/ghri0221"
      ...
      INPUT BY NAME g_hrbf.hrbf03,g_hrbf.hrbf04,g_hrbf.hrbf05,
        ...
        AFTER FIELD hrbf04
           IF NOT cl_null(g_hrbf.hrbf04) THEN
           	  CALL i0225_hrbf04() RETURNING l_des
...
#           	  DISPLAY BY NAME g_hrbf.hrbf09
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW i0225_w1 
         RETURN
      END IF  
      EXIT WHILE  
   END WHILE
         ...
         #move by zhuzw 20141117 逻辑下移至此  start
      	 IF l_chk = 'Y' THEN 
      	   SELECT * INTO l_hrbf.* FROM ( SELECT hrbf_file.*  FROM hrbf_file
      	     ...
      	     WHERE rownum = 1
      	  #新劳动合同的生效日期不能小于原劳动合同的生效日期
      	    IF g_hrbf.hrbf08 <= l_hrbf.hrbf08 THEN
      	    	 ...
      	    	 LET l_des = cl_getmsg('ghr-053',g_lang)||l_hrat.hrat01||' '||l_hrat.hrat02
      	 	     ...
      	 	     CONTINUE FOR      	 	     
      	    END IF
      	   UPDATE hrbf_file SET hrbf09 = g_hrbf.hrbf08 -1 
      	    WHERE hrbf01 = l_hrbf.hrbf01
      	 END IF
         ...
         #add by zhuzw 20141117 end 
      	...
      	# MESSAGE "Insert "||g_hrbf.hrbf01
      	 ...
      	 IF SQLCA.sqlcode THEN
            #LET l_infor = l_infor||g_hrbf.hrbf01||'  ins error'||'\n'
            CONTINUE FOR
         ELSE 
            #LET l_infor = l_infor||g_hrbf.hrbf01||'  ins success'||'\n'
            CONTINUE FOR
         END IF
      END IF 
      LET g_hrat[l_n].chk = 'N'
   ...
   CLOSE WINDOW i0225_w1
   CALL i0225_b_fill(g_wc2,'')
END FUNCTION


FUNCTION i0225_hrbf04()
   ...
   RETURN l_hrbe02
END FUNCTION
FUNCTION i0225_set_data(l_cnt,l_turn)
  ...
  FOR li_i = l_cnt+1 TO g_rec_b 
      ...
      LET l_hrat.hrat03 = g_hrat_t[li_i].hrat03
        ...
        CALL i022_hrbf04(g_hrat_t[li_i].hrbf04) RETURNING g_hrat[li_j].hrbf04         
        CALL s_code('339',g_hrat_t[li_i].hrbf11) RETURNING l_hrag.*
        LET g_hrat[li_j].hrbf11 = l_hrag.hrag07       
      LET li_j = li_j + 1
  END FOR
 
END FUNCTION
	
FUNCTION i0225_tz()  #add by shenran NO.130707
...
FUNCTION i0225_zzht()
 ...
 DEFINE l_infor STRING
 DEFINE l_err    LIKE  type_file.chr1000 
 DEFINE l_sxdate LIKE  type_file.dat
   IF g_rec_b = 0 THEN RETURN END IF 
   ...
   IF NOT cl_confirm('abx-080') THEN RETURN END IF 
   OPEN WINDOW i0225_w1 WITH FORM "ghr/42f/ghri022_f"
       ATTRIBUTE (STYLE = g_win_style CLIPPED)              #介面風格透過p_zz設定
   CALL cl_ui_init()
   LET l_sxdate = g_today
   DISPLAY l_sxdate TO sxdate
   INPUT l_sxdate WITHOUT DEFAULTS FROM sxdate
   BEFORE INPUT  
#   AFTER FIELD sxdate
#   IF NOT cl_null(l_sxdate) THEN
#
#   END IF 
   END INPUT 
   FOR l_n = 1 TO g_turn 
      IF g_hrat[l_n].chk = 'Y' THEN 
         UPDATE hrbf_file SET hrbf09 = l_sxdate
          WHERE hrbf02 = g_hrat[l_n].hratid
            AND hrbf08 = g_hrat[l_n].hrbf08
            AND hrbf09 = g_hrat[l_n].hrbf09
            LET g_hrat[l_n].chk = 'N' 
      END IF 
   END FOR  
