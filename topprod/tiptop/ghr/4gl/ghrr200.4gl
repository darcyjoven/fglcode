...
MAIN
   ...
   LET g_sql ="hrat01.hrat_file.hrat01,",
              ...
              "hratid.hrat_file.hratid,",
              "ischange.type_file.num5"

   LET l_table = cl_prt_temptable('ghrr200',g_sql) CLIPPED
...
--
   ...
   LET l_sql = "select a.hrat01,a.hrat02,a.hrat04,bm1.hrao02,a.hraoud02,bm02.hrao02,bm03.hrao02,bm04.hrao02,as1.hras04,",
               ...
               " ag2.hrag07,az2.hraz05,as2.hras04,az3.n1,bf1.hrbf08,bf1.hrbf09,ag3.hrag07,bf2.n2,",
               ...
               " dw2.dw04,dw2.dw05,",
               "case when du2.hrdu03='002' then du2.du04 else '' end,", 
               "a.hratid,az5.n1 ",
               ...
               " hraz05 >=to_date('",tm.l_last_day,"','YYYY/MM/DD') and hraz05 <=to_date('",tm.l_day,"','YYYY/MM/DD') group by hraz03) az3 on az3.hraz03=a.hratid ",
               " left join (select hraz03,max(hraz01) as hraz01,count(*) as n1 from hraz_file where hraz04='001' and ",
               " hraz05 >=to_date('",tm.l_last_day,"','YYYY/MM/DD') and hraz05<=to_date('",tm.l_day,"','YYYY/MM/DD') group by hraz03) az4 on az4.hraz03=a.hratid ",
               " left join (select hraz03,min(hraz01) as hraz01,count(*) as n1 from hraz_file where hraz04<>'001' and ",
               " hraz05 >=to_date('",tm.l_last_day,"','YYYY/MM/DD') and hraz05>to_date('",tm.l_day,"','YYYY/MM/DD') group by hraz03) az5 on az5.hraz03=a.hratid ",
               ...
               " left join hrag_file ag3 on bf3.hrbf11=ag3.hrag06 and ag3.hrag01='339' ",
               ...
               " hrdt_file where hrdu02=hrdt01 and hrdu05<>'007' and hrdu03='001' group by hrdu01,hrdt04,hrdu03) du1 on du1.hrdu01=a.hratid ", #xie150410  增加hrdu03='001'，去除一个人多行的问题。  
               " left join  (select hrdu01,hrdt04,min(hrdu04) as du04,hrdu03 from hrdu_file, ",  
 ...
 --              " (hrbh04 >= to_date('",tm.l_last_day,"','YYYY/MM/DD') )) ",
               " order by hrat01 "
      ...
      FOREACH r100_curs INTO sr.*
           ...
           IF (NOT cl_null(sr.AZ1_HRAZ05))  AND sr.AZ1_HRAZ05>=tm.l_last_day AND sr.AZ1_HRAZ05<=tm.l_day  THEN
              IF NOT cl_null(sr.hr_status) THEN
                LET sr.hr_status=sr.hr_status,'|'  
              END IF 
           ...
           IF (NOT cl_null(sr.AZ2_HRAZ05))  AND sr.AZ2_HRAZ05>=tm.l_last_day AND sr.AZ2_HRAZ05<=tm.l_day THEN
              IF NOT cl_null(sr.hr_status) THEN
                LET sr.hr_status=sr.hr_status,'|'  
              END IF 
          ...
          #IF l_du_year>tm.l_year OR l_du_month>tm.l_month THEN      #xie150410
          #   LET sr.du1_hrdu04='' 
          #END IF 
        ...
        #     LET sr.du2_hrdu04_1='' 
         # END IF  
          ...
          IF(sr.hrad03='离职员工') THEN
             SELECT count(*) INTO l_hrbh_n FROM hrbh_file WHERE hrbh01=sr.hratid 
                AND hrbh04>tm.l_day
          ...
          IF sr.ischange>0 THEN
             select hrao02 INTO sr.bm1_hrao02 from hraz_file,hrao_file where hraz07=hrao01 and  hraz43=sr.hratid  AND
                    hraz05>tm.l_day order by hraz05 desc   
          END IF 
          

          IF sr.hrat25<=tm.l_day THEN
            EXECUTE insert_prep USING sr.*
          END IF 
      END FOREACH

    LET l_rep_name="ghr/ghrr200.cpt&p2=",tm.s_year,"&p3=",tm.s_month  
