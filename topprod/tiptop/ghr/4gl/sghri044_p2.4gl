...
#
# Pattern name...: ghri044_p2.4gl
# Descriptions...: 批量特休
# Date & Author..: 13/07/12 By yangjian
...
DEFINE  g_hrat_o     RECORD
           ...
           hrat19   LIKE  hrat_file.hrat19
             END  RECORD  ,
     g_hrcc   DYNAMIC ARRAY OF RECORD
...
FUNCTION sghri044_p2()
 ...
 DEFINE l_msg        LIKE  type_file.chr1000
    ...
    OPEN WINDOW i044_w_p2 AT p_row,p_col WITH FORM "ghr/42f/ghri044_p2"  ATTRIBUTE (STYLE = g_win_style CLIPPED) 
    CALL cl_ui_init()
    CALL cl_set_label_justify("i044_w_p2","right")
   ...
   DIALOG ATTRIBUTES(UNBUFFERED)
         ...
         BEFORE ROW
            LET l_ac = ARR_CURR()
            IF l_ac > 0 THEN 
            	 CALL sghri044_p2_fill(g_hrcd.hrcd01,g_hrat[l_ac].hrat01)
            END IF 
            CALL cl_show_fld_cont()                  
         
      END DISPLAY
      
      DISPLAY ARRAY g_hrcc TO s_hrcc.* ATTRIBUTE(COUNT=g_rec_b)
      ...
      ON ACTION group_insert
         ...
         IF cl_chk_act_auth() THEN
            CALL sghri044_p2_group_insert()
         END IF 
      ON ACTION done_exit
         LET g_action_choice="done_exit"
         CALL sghri044_p2_insert()
         EXIT DIALOG
   ...
   END IF 
   END WHILE
   CLOSE WINDOW i044_w_p2 

END FUNCTION 

FUNCTION sghri044_p2_a()
 ...
 WHILE TRUE 
   ...
   INPUT BY NAME
      ...
      BEFORE INPUT
          CALL sghri044_p2_set_entry('a')
          CALL sghri044_p2_set_no_entry('a')
          
      AFTER FIELD hrcd01
            IF NOT cl_null(g_hrcd.hrcd01) THEN
               IF g_hrcd.hrcd01 != g_hrcd_t.hrcd01 OR
                  g_hrcd_t.hrcd01 IS NULL THEN
                  CALL sghri044_p2_hrcd01('a') RETURNING g_hrcd.hrcd11,l_hrcd01_name,g_hrcd.hrcd07
...
#          	 	     WHEN '002'
#          	 	        LET g_hrcd.hrcd06 = 2*(g_hrcd.hrcd04 - g_hrcd.hrcd02 + 1)
#          	 	     OTHERWISE 
#          	 	        LET g_hrcd.hrcd06 = g_hrcd.hrcd04 - g_hrcd.hrcd02 + 1 
#          	 	  END CASE 
      ...
      ATTRIBUTE (COUNT=g_rec_b,MAXCOUNT=g_max_rec,UNBUFFERED,
        ...
        BEFORE ROW
            ...
            IF g_rec_b>=l_ac THEN
               LET p_cmd='u'
               CALL sghri044_p2_set_b_entry(p_cmd)                                       
               CALL sghri044_p2_set_b_no_entry(p_cmd)                                    
       ...
       AFTER FIELD hrat01
            IF NOT cl_null(g_hrat[l_ac].hrat01) THEN
               IF g_hrat[l_ac].hrat01 != g_hrat_t.hrat01 OR
                  ...
                  CALL sghri044_p2_hrat01('a') RETURNING g_hrat[l_ac].hrat02,g_hrat[l_ac].hrat04,
              ...
              CASE 
                WHEN INFIELD(hrat01)
                   IF p_cmd = 'a' THEN 
                      ...
                      CALL sghri044_p2_mulit(g_mulit_hrat)
                   ELSE 
                      CALL cl_init_qry_var()
                      LET g_qryparam.form = "q_hrat"
...
FUNCTION sghri044_p2_fill(p_hrcda03,p_hrcda04)
 ...
 DEFINE p_hrcda04  LIKE  hrcda_file.hrcda04
   ...
   LET g_sql = "SELECT hrcc04,hrcc05,hrcc03,hrcc08,hrcc09,hrcc10,hrcc06,hrcc11 FROM hrcc_file ",
               " WHERE hrcc07 = '",p_hrcda04,"' ",
               "   AND hrcc02 = '",p_hrcda03,"' ",
               " ORDER BY hrcc05 DESC ,hrcc04 DESC "
   PREPARE sghri044_p2_fill_prep FROM g_sql
   DECLARE sghri044_p2_fill_cs CURSOR FOR sghri044_p2_fill_prep

   LET g_cnt = 1 
   FOREACH sghri044_p2_fill_cs INTO g_hrcc[g_cnt].*
      IF SQLCA.sqlcode THEN EXIT FOREACH END IF 
      
      LET g_cnt = g_cnt + 1 
   END FOREACH
   CALL g_hrcc.deleteElement(g_cnt)
   
END FUNCTION

FUNCTION sghri044_p2_set_entry(p_cmd)
  DEFINE  p_cmd  LIKE  type_file.chr1
  
     CALL cl_set_comp_entry("hrcd01,hrcd02,hrcd03,hrcd04,hrcd05,hrcd06,hrcd12",TRUE)                                     
END FUNCTION 

FUNCTION sghri044_p2_set_no_entry(p_cmd)
  DEFINE  p_cmd  LIKE  type_file.chr1
  
END FUNCTION 

FUNCTION sghri044_p2_set_b_entry(p_cmd)
  DEFINE  p_cmd  LIKE  type_file.chr1
  
  CALL cl_set_comp_entry("hrat01",TRUE)                                     
END FUNCTION 

FUNCTION sghri044_p2_set_b_no_entry(p_cmd)
  DEFINE  p_cmd  LIKE  type_file.chr1
...
FUNCTION sghri044_p2_hrcd01(p_cmd)
 DEFINE p_cmd         LIKE type_file.chr1
...
FUNCTION sghri044_p2_hrat01(p_cmd)
 DEFINE p_cmd         LIKE type_file.chr1
   ...
   FOR i =1 TO l_n
       ...
       IF SQLCA.sqlcode THEN
         #CALL cl_err3("ins","hrcd_file",l_hrcd.hrcd10,"",SQLCA.sqlcode,"","",1) 
          CALL s_errmsg('','',"ins hrcd_file err",SQLCA.sqlcode,1)
          LET g_success = 'N'
          CONTINUE FOR
       END IF
       CALL sghri044_p2_insert_chk(l_hrcd.*)
       IF g_success = 'N' THEN 
          CONTINUE FOR
       END IF 
   END FOR
...
FUNCTION sghri044_p2_mulit(p_mulit_hrat)
...
FUNCTION  sghri044_p2_group_insert()
  ...
  DEFINE  l_hrcka    RECORD LIKE hrcka_file.*
    ...
    IF NOT cl_null(l_hrcka01) AND NOT cl_null(l_hrcka02) THEN 
    	 ...
    	 PREPARE sghri044_p2_group_insert_prep FROM g_sql
    	 DECLARE sghri044_p2_group_insert_cs CURSOR  FOR sghri044_p2_group_insert_prep
    	 LET g_cnt = 1 
    	 FOREACH sghri044_p2_group_insert_cs INTO g_hrat[g_cnt].*
    	    IF SQLCA.sqlcode THEN EXIT FOREACH END IF 
...
FUNCTION sghri044_p2_insert_chk(p_hrcd)
...
#  DEFINE l_hrcc  RECORD  LIKE hrcc_file.*
  DEFINE  l_hrcc    DYNAMIC ARRAY OF RECORD 
          ...
          END RECORD  
  DEFINE l_hrcc_1  DYNAMIC ARRAY OF RECORD 
            hrcc02   LIKE  hrcc_file.hrcc02,
            hrcc04   LIKE  hrcc_file.hrcc04,
            hrcc07   LIKE  hrcc_file.hrcc07
             END RECORD
  ...
  DEFINE l_umf_day  LIKE hrcd_file.hrcd06
   ...
   FOR l_n = 0 TO l_end STEP 1
      ...
      CALL sghri044_p2_insertday_chk(p_hrcd.hrcd01,p_hrcd.hrcd09,l_day,p_hrcd.hrcd06,p_hrcd.hrcd07,'') RETURNING l_res,l_hrcc
      IF NOT l_res THEN 
      ...
      END IF 
      CALL l_no.clear()
      FOR i =1  TO l_hrcc.getLength()
         IF l_hrcc[i].hrcc01 IS NOT NULL THEN 
            LET l_no[i].no = l_hrcc[i].hrcc01
            LET l_no[i].day = l_hrcc[i].l_day_umf
            LET l_no[i].unit = l_hrcc[i].l_day_unit
      ...
      FOR i =1 TO l_hrcc.getLength()
         IF l_hrcc[i].hrcc01 IS NOT NULL THEN 
            CALL sghri044__umf(l_hrcc[i].l_day_umf,l_hrcc[i].l_day_unit,l_hrcc[i].hrcc10) RETURNING l_umf_day
            UPDATE hrcc_file SET hrcc08=hrcc08+l_umf_day,hrcc09=hrcc09-l_umf_day
             WHERE hrcc02 = l_hrcc[i].hrcc02
               AND hrcc04 = l_hrcc[i].hrcc04
               AND hrcc07 = l_hrcc[i].hrcc07
            IF l_hrcc[i].hrcc06 = 'Y' THEN 
               CALL l_hrcc_1.appendElement() 
               LET l_hrcc_1[l_hrcc_1.getLength()].hrcc02 = l_hrcc[i].hrcc02
               LET l_hrcc_1[l_hrcc_1.getLength()].hrcc04 = l_hrcc[i].hrcc04
               LET l_hrcc_1[l_hrcc_1.getLength()].hrcc07 = l_hrcc[i].hrcc07
            END IF 
         END IF       
      END FOR
#      UPDATE hrcc_file SET hrcc08=hrcc08+1,hrcc09=hrcc09-1
#       WHERE hrcc02 = l_hrcc.hrcc02
#         AND hrcc04 = l_hrcc.hrcc04
#         AND hrcc07 = l_hrcc.hrcc07
#      IF l_hrcc.hrcc06 = 'Y' THEN 
#         CALL l_hrcc_1.appendElement() 
#         LET l_hrcc_1[l_hrcc_1.getLength()].hrcc02 = l_hrcc.hrcc02
#         LET l_hrcc_1[l_hrcc_1.getLength()].hrcc04 = l_hrcc.hrcc04
#         LET l_hrcc_1[l_hrcc_1.getLength()].hrcc07 = l_hrcc.hrcc07
#      END IF 
   END FOR
   FOR l_n = 1 TO l_hrcc_1.getLength()
       UPDATE hrcc_file SET hrcc09 = 0
        WHERE hrcc02 = l_hrcc_1[l_n].hrcc02
          AND hrcc04 = l_hrcc_1[l_n].hrcc04
          AND hrcc07 = l_hrcc_1[l_n].hrcc07
   END FOR
END FUNCTION
