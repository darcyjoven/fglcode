...
MAIN
   ...
   LET g_sql = "title_name.type_file.chr50, hrat25.hrat_file.hrat25,      ",
               "hrat19.hrat_file.hrat19,    hrat19_name.type_file.chr50, ",
               "l_sum.type_file.num20,      sum.type_file.num20           "

                             
   LET l_table = cl_prt_temptable('ghrr010',g_sql) CLIPPED   # 产生Temp Table
   ...
   IF (cl_null(g_bgjob) OR g_bgjob = 'N') THEN  
      CALL ghrr010_tm(0,0)             # Input print condition
   ELSE 
      CALL ghrr010()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN
 
FUNCTION ghrr010_tm(p_row,p_col)
   ...
   OPEN WINDOW ghrr010_w AT p_row,p_col WITH FORM "ghr/42f/ghrr010"
...
WHILE TRUE
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 
      CLOSE WINDOW ghrr010_w  
      EXIT WHILE        
   END IF
   CONSTRUCT BY NAME tm.wc ON hrat03,hrat04,hrat25
    ...
    INPUT BY NAME tm.qj WITHOUT DEFAULTS  
       ...
       AFTER INPUT
         IF NOT cl_null(tm.hrat04) THEN
            LET tm.wc=" hrat03='",tm.hrat03,"'" CLIPPED," AND "," hrat04='",tm.hrat04,"'" CLIPPED," AND "," hrat25<='",tm.hrat25,"'" CLIPPED
         END IF
   ...
   IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW ghrr010_w
   ...
   END IF
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,     
                         ...
                         " '",g_template CLIPPED,"'",       
                         " '",g_rpt_name CLIPPED,"'"          
         CALL cl_cmdat('ghrr010',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW ghrr010_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time    #No.FUN-690126
      EXIT PROGRAM
...
FUNCTION ghrr010()        # Read data and create out-file
  DEFINE l_cmd      LIKE     type_file.chr1000, 
   ...
   SELECT hrao02 INTO l_hrat04 FROM hrao_file WHERE hrao01=tm.hrat04
   SELECT hraa12 INTO l_hraa12 FROM hraa_file WHERE hraa01=tm.hrat03
   
 
   LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond('oeauser', 'oeagrup')
   LET l_sql=" SELECT '','',hrad02,'',count(hrat01),''   ", 
             " FROM hrat_file,hrad_file                ",
             " WHERE hrat19=hrad02 AND hrad01<>'003' AND      ",
             tm.wc CLIPPED,
             " GROUP BY hrad02 "

    
   PREPARE ghrr010_prepare1 FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM 
   END IF
   DECLARE ghrr010_curs1 CURSOR FOR ghrr010_prepare1
   IF STATUS THEN CALL cl_err('declare:',STATUS,1) RETURN END IF
 
   FOREACH ghrr010_curs1 INTO sr.*
      IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
      IF cl_null(l_hrat04) THEN
        LET sr.title_name=l_hraa12
       SELECT count(hrat01) INTO sr.sum FROM hrat_file,hrad_file
       WHERE hrat19=hrad02 AND hrad01<>'003' AND hrat03=tm.hrat03 AND hrat25<=tm.hrat25
      END IF
      IF NOT cl_null(tm.hrat04) THEN
        LET sr.title_name=l_hraa12,'-',l_hrat04
      SELECT count(hrat01) INTO sr.sum FROM hrat_file,hrad_file
      WHERE hrat19=hrad02 AND hrad01<>'003' AND hrat03=tm.hrat03
            AND hrat04=tm.hrat04 AND hrat25<=tm.hrat25
      END IF
      LET sr.hrat25=tm.hrat25
      IF NOT cl_null(sr.hrat19) THEN
         SELECT hrad03 INTO sr.hrat19_name  FROM hrad_file WHERE hrad02=sr.hrat19
      END IF      
       EXECUTE insert_prep USING sr.*   
   END FOREACH  
   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED    
   LET g_str = tm.wc
   CALL cl_prt_cs3('ghrr010','ghrr010',g_sql,g_str)     #第一个ghrr010是程序名字  第二是模板名字 p_zaw
END FUNCTION
