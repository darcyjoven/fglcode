...
MAIN
   ...
   LET g_sql = " month.type_file.num10,",
               " wshu.hrag_file.hrag07,",
               " hrag07.hrag_file.hrag07,",
               " flje.hrdxa_file.hrdxa24, ",
               " renshu.type_file.num10 "
               
                                              
   LET l_table = cl_prt_temptable('ghrr030',g_sql) CLIPPED
   ...
   IF (cl_null(g_bgjob) OR g_bgjob = 'N') AND cl_null(tm.wc) THEN
      CALL ghrr030_tm(0,0)                     # Input print condition
   ELSE                                   
      CALL ghrr030()                                # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
END MAIN 

FUNCTION ghrr030_tm(p_row,p_col)
   ...
   OPEN WINDOW r030_w AT p_row,p_col WITH FORM "ghr/42f/ghrr030"
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW r030_w
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,
                            ...
                            " '",g_template CLIPPED,"'",
                            " '",g_rpt_name CLIPPED,"'"
            CALL cl_cmdat('ghrr030',g_time,l_cmd)      # Execute cmd at later time
         END IF
         CLOSE WINDOW r030_w
...
DEFINE l_sum,l_sum1,l_sum2       LIKE     type_file.num10           
       ...
       PREPARE ghrr030_prepare1 FROM l_sql
       IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('prepare:',SQLCA.sqlcode,1)
          CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690127
          EXIT PROGRAM
       END IF
       DECLARE ghrr030_curs1 CURSOR FOR ghrr030_prepare1        
       FOREACH ghrr030_curs1 INTO sr1.*
         CALL r030_renshu(sr1.hrar02)  RETURNING sr.renshu           
       ...
       CALL cl_prt_cs3('ghrr030','ghrr030',g_sql,g_str)
                                                           
END FUNCTION 
FUNCTION r030_renshu(p_hrar02)
DEFINE l_sum,l_sum1,l_sum2       LIKE     type_file.num10  
DEFINE l_date    LIKE type_file.dat
DEFINE p_hrar02  LIKE hrar_file.hrar02
        SELECT  MDY('12','31',tm.hrct04) INTO l_date    FROM  dual
        SELECT  COUNT(*) INTO l_sum1  FROM  hrat_file,hras_file,hrar_file,hrad_file,hrag_file
         WHERE  hrat25 <= to_date(l_date,'yyyy/mm/dd')
           ...
           AND  hrat19 = hrad02          
           AND  hrad01 != '003'                    
        SELECT  COUNT(*) INTO l_sum2  FROM  hrat_file,hras_file,hrar_file,hrad_file,hrag_file
         WHERE  hrat25 <= to_date(l_date,'yyyy/mm/dd')
           AND  hrat05=hras01 
           AND  hras03=hrar03 
           AND  hrar02 = hrag06
