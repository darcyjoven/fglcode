...
#{
# Program name...: aws_get_po_receiving_in_data.4gl
# Descriptions...: 提供取得 ERP 採購收貨單(可入庫) 資料服務
...
#[
# Description....: 提供取得 ERP 採購收貨單(可入庫) 資料服務(入口 function)
# Date & Author..: 2008/04/08 by kim  (FUN-840012)
# Parameter......: none
# Return.........: none
# Memo...........:
...
#
#]
FUNCTION aws_get_po_receiving_in_data()
    ...
    IF g_status.code = "0" THEN
       CALL aws_get_po_receiving_in_data_process()
    END IF
...
#
#]
FUNCTION aws_get_po_receiving_in_data_process()
    ...
    LET l_sql = "SELECT * FROM rvb_file WHERE rvb01 = ? ",
                " AND (rvb07+rvb29-rvb30>0) ORDER BY rvb01,rvb02"
    ...
    FOREACH rva_curs INTO l_rva.*
       ...
       IF l_rvb.getlength()>0 THEN  #避免帶出有單頭,無單身的資料
          LET l_node = aws_ttsrv_addMasterRecord(base.TypeInfo.create(l_rva), "rva_file")   #加入此筆單頭資料至 Respo_receiving_innse 中
          CALL aws_ttsrv_addDetailRecord(l_node, base.TypeInfo.create(l_rvb), "rvb_file")   #加入此筆單頭的單身資料至 Respo_receiving_innse 中
       END IF
    END FOREACH
