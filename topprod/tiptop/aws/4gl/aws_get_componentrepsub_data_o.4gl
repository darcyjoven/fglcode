...
#
#]
FUNCTION aws_getComponentRepSubData_g()
    ...
    # 檢查登入資訊                                                             #
    #--------------------------------------------------------------------------#
    IF NOT aws_ttsrv_checkSignIn(g_getComponentRepSubData_in.signIn) THEN    
    ...
    IF g_status.code = "0" THEN
       CALL aws_getComponentRepSubData_get()
    END IF
    
    LET g_getComponentRepSubData_out.status = aws_ttsrv_getStatus()
...
#
#]
FUNCTION aws_getComponentRepSubData_get()
    DEFINE l_bmd       RECORD LIKE bmd_file.*
    DEFINE l_node      om.DomNode
    DEFINE l_sql       STRING
 
    
    LET g_table = "bmd_file"
    ...
    #--------------------------------------------------------------------------#
    IF cl_null(g_getComponentRepSubData_in.condition) THEN
       LET g_getComponentRepSubData_in.condition = " 1=1" 
    END IF
    LET l_sql = "SELECT * FROM bmd_file WHERE ",
                 g_getComponentRepSubData_in.condition,
    ...
    FOREACH bmd_curs INTO l_bmd.*
        ...
        #------------------------------------------------------------------#
        LET l_node = aws_ttsrv_setDataSetRecord(base.TypeInfo.create(l_bmd), l_node, g_table)
    END FOREACH
 
    #--------------------------------------------------------------------------#
    # Response Xml 文件改成字串                                                #
    #--------------------------------------------------------------------------#
    LET g_getComponentRepSubData_out.bmd = aws_ttsrv_xmlToString(l_node)
END FUNCTION
