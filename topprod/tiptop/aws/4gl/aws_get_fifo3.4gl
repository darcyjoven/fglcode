...
#{
# Program name...: aws_get_fifo3.4gl
# Descriptions...: 获取销售出货fifo建议
...
FUNCTION aws_get_fifo3_process()
   ...
   #FUN-B90089 add str---
    ...
    DEFINE l_tc_spy01 LIKE tc_spy_file.tc_spy01
    DEFINE l_fifo3    RECORD 
    ...
    LET l_tc_spy01 = aws_ttsrv_getParameter("tc_spy01")   #取由呼叫端呼叫時給予的 SQL Condition
   
    IF cl_null(l_tc_spy01) THEN 
       LET g_status.code=-1
       LET g_status.description="配货计划单号不能为空"
       RETURN
    END IF
   LET l_sql="select a.ibb01,a.ima02,a.imgb03,a.imgb05,a.sumimgb05,a.tc_spy05",
             " from",
    ...
    FOREACH occ_cur INTO l_fifo3.*
       LET l_node = aws_ttsrv_addMasterRecord(base.TypeInfo.create(l_fifo3), "Master")   #加入此筆單檔資料至 Response 中
