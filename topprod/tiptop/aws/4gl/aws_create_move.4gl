...
#
# Descriptions...: 移库条码异动(tlfb_file)
# Date & Author..: 2016-06-05 9:34:17 shenran
...
GLOBALS
DEFINE g_ime02        LIKE ime_file.ime02         #移入库位编码
DEFINE g_imd01        LIKE imd_file.imd01         #移入仓库编码
DEFINE g_ime02a       LIKE ime_file.ime02         #移出库位编码
DEFINE g_imd01a       LIKE imd_file.imd01         #移出仓库编码
DEFINE g_barcode      LIKE tlfb_file.tlfb01       #料件批次
...
#
#]
FUNCTION aws_create_move_process()
   ...
   DEFINE l_rvb07      LIKE rvb_file.rvb07
    ...
    FOR l_i = 1 TO l_cnt1
        ...
        LET g_ime02    = aws_ttsrv_getRecordField(l_node1,"ime02")
        LET g_imd01    = aws_ttsrv_getRecordField(l_node1,"imd01")
        LET g_ime02a   = aws_ttsrv_getRecordField(l_node1,"ime02a")
        LET g_imd01a   = aws_ttsrv_getRecordField(l_node1,"imd01a")
        LET g_barcode  = aws_ttsrv_getRecordField(l_node1,"barcode")
...
FUNCTION move_load()
   ...
   END IF
END FUNCTION
FUNCTION move_ins_tlfb()   #No.MOD-8A0112 add
   ...
   DEFINE l_sql       STRING
      ...
      SELECT COUNT(*) INTO l_n FROM iba_file WHERE iba01=g_barcode
      SELECT COUNT(*) INTO l_t FROM ibb_file WHERE ibb01=g_barcode
      IF cl_null(l_n) THEN LET l_n=0 END IF
      IF cl_null(l_t) THEN LET l_t=0 END IF
      IF l_n=0 AND l_t=0 THEN
         LET l_iba.iba01=g_barcode
         ...
         IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
             ...
             RETURN
         END IF
         LET l_ibb.ibb01=g_barcode                #条码编号
   ...
   #移入仓库从处理
      ...
      LET g_tlfb.tlfb01 = g_barcode                    #条码编号
      LET g_tlfb.tlfb02 = g_imd01                      #仓库
      LET g_tlfb.tlfb03 = g_ime02                      #库位
      LET g_tlfb.tlfb04 = g_batch                      #批号  ibb17
...
#移出仓库处理
      LET g_tlfb.tlfb01 = g_barcode                    #条码编号
      LET g_tlfb.tlfb02 = g_imd01a                      #仓库
      LET g_tlfb.tlfb03 = g_ime02a                      #库位
      LET g_tlfb.tlfb04 = g_batch                      #批号  ibb17
      LET g_tlfb.tlfb05 = g_tlfb05                     #数量
     ...
     LET g_sql =" SELECT COUNT(*) FROM imgb_file ",
      ...
      PREPARE t018_imgb_pre1 FROM g_sql
      EXECUTE t018_imgb_pre1 INTO l_n
      IF l_n = 0 THEN #没有imgb_file，新增imgb_file
        CALL s_ins_imgb(g_tlfb.tlfb01,
              g_tlfb.tlfb02,g_tlfb.tlfb03,g_tlfb.tlfb04,g_tlfb.tlfb05,g_tlfb.tlfb06,'')
      ELSE
