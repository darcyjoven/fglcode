...
#
#]
FUNCTION aws_getEmployeeList_g()
    DEFINE l_str  STRING,
    ...
    # 檢查登入資訊                                                             #
    #--------------------------------------------------------------------------#
    IF NOT aws_ttsrv_checkSignIn(g_getEmployeeList_in.signIn) THEN    
    ...
    # 查詢 ERP 員工編號                                                        #
    #--------------------------------------------------------------------------#
    CALL g_getEmployeeList_out.gen.clear()   #清空回覆參數的 ARRAY
    IF g_status.code = "0" THEN
       CALL aws_getEmployeeList_get()
    END IF
    
    LET g_getEmployeeList_out.status = aws_ttsrv_getStatus()
END FUNCTION
...
#
#]
FUNCTION aws_getEmployeeList_get()
    ...
    FOREACH gen_cur INTO l_gen.*
        LET g_getEmployeeList_out.gen[l_i].gen01 = l_gen.gen01 CLIPPED
        LET g_getEmployeeList_out.gen[l_i].gen02 = l_gen.gen02 CLIPPED
        LET l_i = l_i + 1
    END FOREACH
END FUNCTION
