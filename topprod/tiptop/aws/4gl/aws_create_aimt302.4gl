...
#
# Pattern name...: aws_create_aimt302.4gl
# Descriptions...: 提供产生杂项收料单据
...
DEFINE g_ina01   RECORD
                 ina01 LIKE ina_file.ina01
                 END RECORD                      
                      
END GLOBALS                                                          

FUNCTION  aws_create_aimt302()                                                                      
 ...
 WHENEVER ERROR CONTINUE                                                                           
    ...
    IF g_status.code = "0" THEN                                                                    
       CALL aws_create_aimt302_process()                                                           
    END IF                                                                                         
                                                                                                   
    CALL aws_ttsrv_postprocess()   #呼叫服務後置處理程序                                           
     DROP TABLE i302_file                                                                   
     DROP TABLE i302_file1                                                                         
END FUNCTION                                                                                       
                                                                                                                                                                                              
FUNCTION aws_create_aimt302_process() 
  ...
  DEFINE  l_i302_file1     RECORD               
   ...
   CREATE TEMP TABLE i302_file(                                                                  
                 inb15     LIKE inb_file.inb15,      #理由码编码       
                 inb04     LIKE inb_file.inb04,      #料件    
                 inb16     LIKE inb_file.inb16,      #数量     
                 inb05     LIKE inb_file.inb05)      #仓库 
                                                 
   CREATE TEMP TABLE i302_file1(                                                                   
                  barcode LIKE type_file.chr50,     #批次条码   
                  ima01   LIKE ima_file.ima01,      #料件编号   
                  batch   LIKE type_file.chr50,     #批次 
                  inb06   LIKE inb_file.inb06,      #库位         
  ...
  #--------------------------------------------------------------------------#
    ...
    FOR l_i = 1 TO l_cnt1
        ...
        # 處理第一單身資料                                                         #
        #----------------------------------------------------------------------#
        LET l_cnt2 = aws_ttsrv_getDetailRecordLength(l_node1, "i302_file")       #取得目前單頭共有幾筆單身資料
        IF l_cnt2 = 0 THEN
        ...
        FOR l_k = 1 TO l_cnt2
            INITIALIZE l_i302_file.* TO NULL
            LET l_node2 = aws_ttsrv_getDetailRecord(l_node1, l_k,"i302_file")   #目前單身的 XML 節點

            LET l_i302_file.inb15 = aws_ttsrv_getRecordField(l_node2, "inb15")
            LET l_i302_file.inb04 = aws_ttsrv_getRecordField(l_node2, "inb04")                        
            LET l_i302_file.inb16 = aws_ttsrv_getRecordField(l_node2, "inb16")                        
            LET l_i302_file.inb05 = aws_ttsrv_getRecordField(l_node2, "inb05")                                            
                                                                                                      
            INSERT INTO i302_file VALUES (l_i302_file.*)    
        END FOR
        #----------------------------------------------------------------------#
        # 處理第二單身資料                                                         #
        #----------------------------------------------------------------------#        
        LET l_cnt3 = aws_ttsrv_getDetailRecordLength(l_node1, "i302_file1")       #取得目前單頭共有幾筆單身資料
        IF l_cnt3 = 0 THEN
           LET g_status.code = "mfg-009"   #必須有單身資料
           EXIT FOR
        END IF
        ...
        FOR l_j = 1 TO l_cnt3                                                                      
        INITIALIZE l_i302_file1.* TO NULL                                                       
        LET l_node3 = aws_ttsrv_getDetailRecord(l_node1, l_j,"i302_file1")   #目前                 

            LET l_i302_file1.barcode = aws_ttsrv_getRecordField(l_node3, "barcode")                                              
            LET l_i302_file1.ima01   = aws_ttsrv_getRecordField(l_node3, "ima01")                    
            LET l_i302_file1.batch   = aws_ttsrv_getRecordField(l_node3, "batch")
            LET l_i302_file1.inb06   = aws_ttsrv_getRecordField(l_node3, "inb06")             
            LET l_i302_file1.inb16a  = aws_ttsrv_getRecordField(l_node3, "inb16a")                       
            INSERT INTO i302_file1 VALUES (l_i302_file1.*)                                                   
        END FOR    
    END FOR
    IF g_status.code='0' THEN
       CALL i302_load()
       ...
       IF g_success = 'Y' THEN
          CALL aws_ttsrv_addParameterRecord(base.TypeInfo.create(g_ina01))
       END IF
    END IF 
END FUNCTION
   ...
   DEFINE l_ibb       RECORD LIKE ibb_file.*
    ...
    LET g_success = "Y"
     ...
     IF g_success = "Y" THEN
        ...
        LET g_sql ="select barcode,ima01,batch,inb06,inb16a from i302_file1"
     ...
     IF g_success = 'Y' THEN   #审核检核
       CALL t370sub_y_chk(g_ina.ina01,'3','N') #FUN-B50138  #TQC-C40079 add Y
     END IF
     ...
     IF g_success = "Y" THEN   #过账
        CALL t370sub_s_upd(g_ina.ina01,'3',TRUE)       #CALL 原確認的 update 段#FUN-B50138
     END IF
...
FUNCTION ins_aimt302()
   ...
   DEFINE l_sfa03    LIKE sfa_file.sfa03,
     ...
     IF (NOT li_result) THEN
        LET g_success = 'N'
        LET g_status.code = "-1"
        LET g_status.description = "产生杂收单号有误!"
        RETURN
     END IF
     ...
     IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
        ...
        RETURN
     ELSE 
     	  LET g_ina01.ina01=g_ina.ina01
     END IF

     LET l_sql=" select inb15,inb04,inb16,inb05 from i302_file"
