...
#
#]
FUNCTION aws_getEmployeeData_g()
    ...
    # 檢查登入資訊                                                             #
    #--------------------------------------------------------------------------#
    IF NOT aws_ttsrv_checkSignIn(g_getEmployeeData_in.signIn) THEN    
    ...
    IF g_status.code = "0" THEN
       CALL aws_getEmployeeData_get()
    END IF
    
    LET g_getEmployeeData_out.status = aws_ttsrv_getStatus()
...
#
#]
FUNCTION aws_getEmployeeData_get()
    DEFINE l_gen       RECORD LIKE gen_file.*
    DEFINE l_node      om.DomNode
    DEFINE l_sql       STRING
 
 
    LET g_table = "gen_file"
    ...
    #--------------------------------------------------------------------------#
    #依據資料條件(condition),抓取員工資料                                         #
    #--------------------------------------------------------------------------#
    IF cl_null(g_getEmployeeData_in.condition) THEN
        LET g_getEmployeeData_in.condition = " 1=1"
    END IF   
    LET l_sql="SELECT * FROM gen_file WHERE genacti = 'Y' AND ",g_getEmployeeData_in.condition,
              " ORDER BY gen01"
    ...
    FOREACH gen_curs INTO l_gen.*
        ...
        #------------------------------------------------------------------#
        LET l_node = aws_ttsrv_setDataSetRecord(base.TypeInfo.create(l_gen), l_node, g_table)
    ...
    # Response Xml 文件改成字串                                                #
    #--------------------------------------------------------------------------#
    LET g_getEmployeeData_out.gen = aws_ttsrv_xmlToString(l_node)
END FUNCTION
