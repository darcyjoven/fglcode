...
# Date & Author..: 2016/4/19 19:52:13 shenran
...
#
#]
FUNCTION aws_create_apmt720mei()
 ...
 WHENEVER ERROR CONTINUE
    ...
    IF g_status.code = "0" THEN
       CALL aws_create_apmt720mei_process()
    END IF
...
#
#]
FUNCTION aws_create_apmt720mei_process()
   ...
   DEFINE l_t003_file1      RECORD                          #单身
                    ...
                    rvb07a     LIKE rvb_file.rvb07          #本次数量
                 END RECORD
   DROP TABLE t003_file1
   DROP TABLE tmp 
   ...
   CREATE TEMP TABLE tmp(                                                                                                                 
    ...
    FOR l_i = 1 TO l_cnt1
        ...
        LET l_t003_file1.rvb01  = aws_ttsrv_getRecordField(l_node1, "rvb01")
        LET l_t003_file1.rvb02  = aws_ttsrv_getRecordField(l_node1, "rvb02")
        LET l_t003_file1.rvb05  = aws_ttsrv_getRecordField(l_node1, "rvb05")
        LET l_t003_file1.ima35  = aws_ttsrv_getRecordField(l_node1, "ima35")         
        LET l_t003_file1.rvb07  = aws_ttsrv_getRecordField(l_node1, "rvb07")   
        LET l_t003_file1.rvb07a = aws_ttsrv_getRecordField(l_node1, "rvb07a")
        INSERT INTO t003_file1 VALUES (l_t003_file1.*)
    END FOR
    IF g_success = 'Y' THEN 
       CALL t003_loadmei()
       IF g_success = 'Y' THEN
    ...
    END IF
END FUNCTION
FUNCTION t003_loadmei()
   DEFINE l_n         LIKE type_file.num5
   DEFINE l_img09     LIKE img_file.img09

   LET g_success = "Y"
   BEGIN WORK
   CALL t003_ins_rvumei()
   IF g_success = "Y" THEN
      COMMIT WORK
   ELSE 
   	  ROLLBACK WORK
   END IF
END FUNCTION
FUNCTION t003_ins_rvumei()   #No.MOD-8A0112 add
  ...
  #用傳入的參數來判斷
   ...
   #SELECT gen03 INTO l_rvu.rvu06 FROM gen_file WHERE gen01=l_rvu.rvu07 #部门
...
#  LET l_rvu.rvudate=g_today  #TQC-A60021 -mark
   ...
   END IF
   IF g_success='Y' THEN
      CALL t003_ins_rvvmei(l_rvu.rvu01) #NO:7143單身  #FUN-810038
   END IF
   IF g_success='Y' THEN
   	  ...
   	  ELSE 
   	  	 LET g_prog="apmt720"
   	  END IF
      CALL t720sub_y_upd(l_rvu.rvu01,'1','',l_rvu.rvu08,'N',TRUE,'N','Y')
         LET g_prog="aws_ttsrv2"
   END IF
   IF g_success='Y' THEN
      IF l_rvu.rvu08='SUB' THEN
   	      CALL t003_mmei(l_rvu.rvu01)
      END IF
   END IF
 
END FUNCTION

FUNCTION t003_ins_rvvmei(p_rvv01)     #FUN-810038
 ...
 FOREACH t003_file_rvv_curs INTO l_rvb01,l_rvb02,l_rvb05,l_ima35,l_rvb07,l_rvv17a
     IF STATUS THEN
       EXIT FOREACH
     END IF
     ...
     IF cl_null(l_rvv.rvv02) THEN LET l_rvv.rvv02=1 END IF
      ...
      END IF
      #SELECT ima35 INTO l_rvv.rvv32 FROM ima_file WHERE ima01=l_rvv.rvv31 #抓取默认仓库
      
      LET l_rvv.rvv32 = l_ima35       #仓库
      LET l_rvv.rvv33=b_rvb.rvb37     #儲位
      LET l_rvv.rvv34=b_rvb.rvb38     #批號
      IF cl_null(l_rvv.rvv32) THEN
      	  LET g_status.code = -1
          LET g_status.sqlcode = SQLCA.SQLCODE
          LET g_status.description="aimi100中主要仓库为空,请维护!"
          LET g_success = 'N'
          RETURN
      END IF
...
#不須檢查img_file的合理性
   IF NOT cl_null(l_rvv.rvv32) THEN  #FUN-810038  AND (g_sma.sma886[7,7] = 'Y')
      ...
      IF l_cnt=0 THEN
      	         	   LET g_status.code = -1
             LET g_status.sqlcode = SQLCA.SQLCODE
             LET g_status.description="IMG_FILE不存在!"
             LET g_success='N'
             RETURN
      END IF
      LET g_ima906 = NULL
...
#領料單月份以完工入庫日期該月的最後一天為領料日                                                                                     
         ...
         #IF l_sfa.sfa26 MATCHES '[SUT]' THEN      #FUN-A20037 
          ...
          ELSE                                                                                                                      
             LET l_sfs.sfs27 = l_sfa.sfa27                                                                                          
          END IF                                                                                                                    
          CALL t720_chk_ima64mei(l_sfs.sfs04, l_sfs.sfs05) RETURNING l_sfs.sfs05                                                       
...
FUNCTION t720_chk_ima64mei(p_part, p_qty)                                                                                              
