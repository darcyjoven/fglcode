...
FUNCTION cws_webservice_demo_process()
    ...
    FOR l_i = 1 TO l_cnt1
        LET l_node1 = aws_ttsrv_getMasterRecord(l_i, "Master")
        #TAG:当前处理的节点

        LET g_bma.bma01 = aws_ttsrv_getRecordField(l_node1,"bma01")
        #TAG: 单头取值

        LET l_cnt3 = aws_ttsrv_getDetailRecordLength(l_node1, "Detail")
        #TAG:单身笔数

        CALL g_bmb.clear()
        FOR l_k = 1 TO l_cnt3 
            LET l_node3 = aws_ttsrv_getDetailRecord(l_node1, l_k,"Detail")
            #TAG:当前处理的单身节点
