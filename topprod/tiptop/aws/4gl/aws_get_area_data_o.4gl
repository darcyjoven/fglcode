...
FUNCTION aws_getAreaData_g()
    ...
    # 檢查登入資訊                                                             #
    #--------------------------------------------------------------------------#
    IF NOT aws_ttsrv_checkSignIn(g_getAreaData_in.signIn) THEN    
    ...
    IF g_status.code = "0" THEN
       CALL aws_getAreaData_get()
    END IF
    
    LET g_getAreaData_out.status = aws_ttsrv_getStatus()
...
#
#]
FUNCTION aws_getAreaData_get()
    DEFINE l_gea       RECORD LIKE gea_file.*
    DEFINE l_node      om.DomNode
    DEFINE l_sql       STRING
 
 
    LET g_table = "gea_file"
    ...
    #--------------------------------------------------------------------------#
    IF cl_null(g_getAreaData_in.condition) THEN
       LET g_getAreaData_in.condition = " 1=1" 
    END IF
    LET l_sql = "SELECT * FROM gea_file WHERE geaacti = 'Y' AND ",
                 g_getAreaData_in.condition," ORDER BY gea01 "
    ...
    FOREACH gea_curs INTO l_gea.*
        ...
        #------------------------------------------------------------------#
        LET l_node = aws_ttsrv_setDataSetRecord(base.TypeInfo.create(l_gea), l_node, g_table)
    END FOREACH
 
    #--------------------------------------------------------------------------#
    # Response Xml 文件改成字串                                                #
    #--------------------------------------------------------------------------#
    LET g_getAreaData_out.gea = aws_ttsrv_xmlToString(l_node)
END FUNCTION
