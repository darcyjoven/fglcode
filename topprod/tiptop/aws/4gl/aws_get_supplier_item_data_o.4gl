...
#
#]
FUNCTION aws_getSupplierItemData_g()
    ...
    # 檢查登入資訊                                                             #
    #--------------------------------------------------------------------------#
    IF NOT aws_ttsrv_checkSignIn(g_getSupplierItemData_in.signIn) THEN    
    ...
    IF g_status.code = "0" THEN
       CALL aws_getSupplierItemData_get()
    END IF
    
    LET g_getSupplierItemData_out.status = aws_ttsrv_getStatus()
...
#
#]
FUNCTION aws_getSupplierItemData_get()
    DEFINE l_pmh       RECORD LIKE pmh_file.*
    DEFINE l_node      om.DomNode
    DEFINE l_sql       STRING
 
 
    LET g_table = "pmh_file"
    ...
    #--------------------------------------------------------------------------#
    IF cl_null(g_getSupplierItemData_in.condition) THEN
       LET g_getSupplierItemData_in.condition = " 1=1" 
    END IF
    LET l_sql = "SELECT * FROM pmh_file WHERE pmhacti = 'Y' AND ",
                 g_getSupplierItemData_in.condition," ORDER BY pmh01,pmh02,pmh13 "
    ...
    FOREACH pmh_curs INTO l_pmh.*
        ...
        #------------------------------------------------------------------#
        LET l_node = aws_ttsrv_setDataSetRecord(base.TypeInfo.create(l_pmh), l_node, g_table)
    ...
    # Response Xml 文件改成字串                                                #
    #--------------------------------------------------------------------------#
    LET g_getSupplierItemData_out.pmh = aws_ttsrv_xmltoString(l_node)
END FUNCTION
