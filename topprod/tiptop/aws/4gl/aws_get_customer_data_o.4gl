...
#
#]
FUNCTION aws_getCustomerData_g()
    ...
    # 檢查登入資訊                                                             #
    #--------------------------------------------------------------------------#
    IF NOT aws_ttsrv_checkSignIn(g_getCustomerData_in.signIn) THEN    
    ...
    IF g_status.code = "0" THEN
       CALL aws_getCustomerData_get()
    END IF
    
    LET g_getCustomerData_out.status = aws_ttsrv_getStatus()
...
#
#]
FUNCTION aws_getCustomerData_get()
    ...
    #--------------------------------------------------------------------------#
    IF cl_null(g_getCustomerData_in.condition) THEN
       LET g_getCustomerData_in.condition = " 1=1" 
    END IF
    LET l_sql = "SELECT * FROM occ_file WHERE occacti = 'Y' AND ",
                 g_getCustomerData_in.condition," ORDER BY occ01 "
    ...
    # Response Xml 文件改成字串                                                #
    #--------------------------------------------------------------------------#
    LET g_getCustomerData_out.occ = aws_ttsrv_xmlToString(l_node)
END FUNCTION
