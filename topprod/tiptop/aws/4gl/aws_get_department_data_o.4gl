...
#
#]
FUNCTION aws_getDepartmentData_g()
    ...
    # 檢查登入資訊                                                             #
    #--------------------------------------------------------------------------#
    IF NOT aws_ttsrv_checkSignIn(g_getDepartmentData_in.signIn) THEN    
    ...
    IF g_status.code = "0" THEN
       CALL aws_getDepartmentData_get()
    END IF
    
    LET g_getDepartmentData_out.status = aws_ttsrv_getStatus()
...
#
#]
FUNCTION aws_getDepartmentData_get()
    DEFINE l_gem       RECORD LIKE gem_file.*
    DEFINE l_node      om.DomNode
    DEFINE l_sql       STRING
 
 
    LET g_table = "gem_file"
    ...
    #--------------------------------------------------------------------------#
    #依據資料條件(condition),抓取部門資料                                         #
    #--------------------------------------------------------------------------#
    IF cl_null(g_getDepartmentData_in.condition) THEN
        LET g_getDepartmentData_in.condition = " 1=1"
    END IF
    
    LET l_sql="SELECT * FROM gem_file WHERE gemacti='Y' AND ",g_getDepartmentData_in.condition,
              " ORDER BY gem01"
    ...
    FOREACH gem_curs INTO l_gem.*
        ...
        #------------------------------------------------------------------#
        LET l_node = aws_ttsrv_setDataSetRecord(base.TypeInfo.create(l_gem), l_node, g_table)
    ...
    # Response Xml 文件改成字串                                                #
    #--------------------------------------------------------------------------#
    LET g_getDepartmentData_out.gem = aws_ttsrv_xmlToString(l_node)
END FUNCTION
