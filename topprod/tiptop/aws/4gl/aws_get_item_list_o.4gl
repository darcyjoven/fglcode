...
FUNCTION aws_getItemList_g()
    ...
    # 檢查登入資訊                                                             #
    #--------------------------------------------------------------------------#
    IF NOT aws_ttsrv_checkSignIn(g_getItemList_in.signIn) THEN    
    ...
    # 查詢 ERP 料件編號                                                        #
    #--------------------------------------------------------------------------#
    CALL g_getItemList_out.ima.clear()   #清空回覆參數的 ARRAY
    IF g_status.code = "0" THEN
       CALL aws_getItemList_get()
    END IF
    
    LET g_getItemList_out.status = aws_ttsrv_getStatus()
END FUNCTION
...
FUNCTION aws_getItemList_get()
    ...
    LET l_i = 1
    FOREACH ima_cur INTO l_ima.*
        LET g_getItemList_out.ima[l_i].ima01 = l_ima.ima01 CLIPPED
        LET g_getItemList_out.ima[l_i].ima02 = l_ima.ima02 CLIPPED
        LET l_i = l_i + 1
    END FOREACH
END FUNCTION
