...
#
#]
FUNCTION aws_getCustomerProductData_g()
    ...
    # 檢查登入資訊                                                             #
    #--------------------------------------------------------------------------#
    IF NOT aws_ttsrv_checkSignIn(g_getCustomerProductData_in.signIn) THEN    
    ...
    IF g_status.code = "0" THEN
       CALL aws_getCustomerProductData_get()
    END IF
    
    LET g_getCustomerProductData_out.status = aws_ttsrv_getStatus()
...
#
#]
FUNCTION aws_getCustomerProductData_get()
    DEFINE l_obk       RECORD LIKE obk_file.*
    DEFINE l_node      om.DomNode
    DEFINE l_sql       STRING
 
 
    LET g_table = "obk_file"
    ...
    #--------------------------------------------------------------------------#
    IF cl_null(g_getCustomerProductData_in.condition) THEN
       LET g_getCustomerProductData_in.condition = " 1=1" 
    END IF
    LET l_sql = "SELECT * FROM obk_file WHERE ",
                 g_getCustomerProductData_in.condition," ORDER BY obk01,obk02 "
    ...
    FOREACH obk_curs INTO l_obk.*
        ...
        #------------------------------------------------------------------#
        LET l_node = aws_ttsrv_setDataSetRecord(base.TypeInfo.create(l_obk), l_node, g_table)
    END FOREACH
 
    #--------------------------------------------------------------------------#
    # Response Xml 文件改成字串                                                #
    #--------------------------------------------------------------------------#
    LET g_getCustomerProductData_out.obk = aws_ttsrv_xmlToString(l_node)
END FUNCTION
