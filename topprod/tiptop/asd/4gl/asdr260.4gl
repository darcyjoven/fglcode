...
#
# Pattern name...: asdr260.4gl
# Descriptions...: 製造費用明細表二
...
#No.FUN-850087 --Begin
   ...
   LET l_table=cl_prt_temptable('asdr260',g_sql)
...
#   
##No.FUN-690010------End------
#    create unique index r260_tmp on r260_tmp(ima12)
 
#  #-->依成本分群+主製程小計
##No.FUN-690010-----Begin------
##   CREATE TEMP TABLE r260_tmp2
##   (
##    ima12     VARCHAR(04),          
##    sta07     VARCHAR(06),          
##    ste15     decimal(20,6)
...
#   
##No.FUN-690010-----End-----
#    create unique index r260_tmp2 on r260_tmp2(ima12,sta07) 
 
#  #-->依主製程小計
##No.FUN-690010-----Begin-----
##   CREATE TEMP TABLE r260_tmp3
##   (
##    sta07     VARCHAR(06),          
##    ste15     decimal(20,6)
##   );
...
##No.FUN-690010-----End------
#   create unique index r260_tmp3 on r260_tmp3(sta07) 
#No.FUN-850087 --End
   CALL cl_used(g_prog,g_time,1) RETURNING g_time  #FUN-BB0047 add
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN        # If background job sw is off
      CALL asdr260_tm()        # Input print condition
   ELSE 
      CALL asdr260()           # Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW asdr260_w AT p_row,p_col WITH FORM "asd/42f/asdr260" 
   ...
   WHILE TRUE
     ...
     IF g_bgjob = 'Y' THEN
        ...
        ELSE
           ...
           CALL cl_cmdat('asdr260',g_time,l_cmd)    # Execute cmd at later time
           EXIT WHILE   
        END IF
     END IF
...
#No.FUN-850087 --Begin
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='asdr260'
#No.FUN-850087 --End
     SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
#No.FUN-850087 --Begin
#    DELETE FROM r260_tmp
#    DELETE FROM r260_tmp2
#    DELETE FROM r260_tmp3
#No.FUN-850087 --End
     ...
     PREPARE asdr260_prepare1 FROM l_sql
     ...
     END IF
     DECLARE asdr260_curs1 CURSOR FOR asdr260_prepare1
 
#No.FUN-850087 --Begin
#     CALL cl_outnam('asdr260') RETURNING l_name
 
#    START REPORT asdr260_rep TO l_name
#    LET g_pageno = 0
#No.FUN-850087 --End
 
     FOREACH asdr260_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#      #-->依成本分群小計
#       INSERT INTO r260_tmp  VALUES(l_ima12,sr.ste15)
#         IF SQLCA.sqlcode THEN 
#            UPDATE r260_tmp  SET ste15=ste15+sr.ste15
#                           WHERE ima12 = l_ima12 
#         END IF
 
#      #-->依成本分群+主製程小計
#       INSERT INTO r260_tmp2 VALUES(l_ima12,l_sta07,sr.ste15)
#         IF SQLCA.sqlcode THEN 
#            UPDATE r260_tmp2 SET ste15=ste15+sr.ste15
#                           WHERE ima12 = l_ima12 AND sta07=l_sta07 
#         END IF
 
#      #-->依據主製程小計
#       INSERT INTO r260_tmp3 VALUES(l_sta07,sr.ste15)
#         IF SQLCA.sqlcode THEN 
#            UPDATE r260_tmp3 SET ste15=ste15+sr.ste15
#                           WHERE sta07=l_sta07 
#         END IF
 
#      OUTPUT TO REPORT asdr260_rep(l_ima12,l_ima131,l_sta07,sr.*)
       EXECUTE insert_prep USING l_ima12,l_sta07,sr.ste05,l_ima02,l_ima021,
                                 sr.ste04,l_sfb82,l_gem02,sr.ste15
#No.FUN-850087 --End
     END FOREACH
#No.FUN-850087 --Begin
#    FINISH REPORT asdr260_rep
#      CALL  cl_used(g_prog,g_time,2) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6A0089
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
     LET g_sql=" SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
               '|'," SELECT ima12,sta07,ste15,ste05,ste04 FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
               '|'," SELECT ima12,sta07,ste15,ste05,ste04 FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
               '|'," SELECT ima12,sta07,ste15,ste05,ste04 FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     IF g_zz05='Y' THEN
     ...
     CALL cl_prt_cs3('asdr260','asdr260',g_sql,g_str)
#No.FUN-850087 --End
END FUNCTION
 
REPORT asdr260_rep(l_ima12,l_ima131,l_sta07,sr)
    ...
    #SELECT ima02 INTO l_ima02                   #FUN-5A0059 mark
     ...
     PRINT COLUMN g_c[33],sr.ste05,                       #料號
          ...
          #start FUN-5A0059
           ...
           COLUMN g_c[37],l_sfb82,                        #部門廠商
           COLUMN g_c[38],l_gem02,
           COLUMN g_c[39],cl_numfor(sr.ste15,39,g_azi04)  #製費
          #end FUN-5A0059
 
   AFTER GROUP OF l_sta07   #主製程
      LET l_ste15=GROUP SUM(sr.ste15)
      PRINT COLUMN g_c[34],g_x[13] CLIPPED,
            COLUMN g_c[39],cl_numfor(l_ste15,39,g_azi04)  #製費   #FUN-5A0059
      PRINT ' '
 
   ON LAST ROW
      PRINT g_dash[1,g_len]
      LET l_ste15=SUM(sr.ste15)
      ...
      #-->依主製程小計
      LET l_cnt = 0 
      DECLARE r260_cur3 CURSOR FOR
        SELECT * FROM r260_tmp3 ORDER BY sta07
      FOREACH r260_cur3 INTO l_sta07,l_ste15
        IF STATUS <> 0 THEN EXIT FOREACH END IF
        IF l_cnt =0 THEN PRINT column g_c[34],g_x[15] CLIPPED END IF
       #start FUN-5A0059
        PRINT COLUMN g_c[37],l_sta07,
              COLUMN g_c[39],cl_numfor(l_ste15,39,g_azi04)  #製費
       #end FUN-5A0059
        LET l_cnt = l_cnt + 1
      END FOREACH
      PRINT g_dash2[1,g_len]
      ...
      #-->依成本分群小計
      LET l_cnt = 0
      DECLARE r260_cur CURSOR FOR
        SELECT * FROM r260_tmp ORDER BY ima12
      FOREACH r260_cur INTO l_ima12,l_ste15
        IF STATUS <> 0 THEN EXIT FOREACH END IF
      ...
      #-->依成本分群+主製程小計
      LET l_cnt = 0
      DECLARE r260_cur2 CURSOR FOR
        SELECT * FROM r260_tmp2 ORDER BY ima12,sta07
      FOREACH r260_cur2 INTO l_ima12,l_sta07,l_ste15
        IF STATUS <> 0 THEN EXIT FOREACH END IF
        IF l_cnt =0 THEN PRINT column g_c[34],g_x[17] CLIPPED END IF
       #start FUN-5A0059
        PRINT COLUMN g_c[37],l_ima12,
              COLUMN g_c[38],l_sta07,
              COLUMN g_c[39],cl_numfor(l_ste15,39,g_azi04)  #製費
       #end FUN-5A0059
        LET l_cnt = l_cnt + 1
      END FOREACH
