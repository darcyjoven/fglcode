...
# Modify.........: No.TQC-960342 09/06/23 By hongmei mark NEXT OPTION "next"
...
DEFINE
    g_stp   RECORD LIKE stp_file.*,
    g_gem   RECORD LIKE gem_file.*,
    g_stp_t RECORD LIKE stp_file.*,
    g_stp02_t LIKE stp_file.stp02,
...
#FUN-AA0059---------mod------------end-----------------
    ...
    LET g_sql="SELECT stp02 FROM stp_file ",
              " WHERE ",g_wc CLIPPED, " ORDER BY stp02 "
    PREPARE i160_prepare FROM g_sql           # RUNTIME 編譯
    DECLARE i160_cs                         # SCROLL CURSOR
        SCROLL CURSOR WITH HOLD FOR i160_prepare
    LET g_sql=
        "SELECT COUNT(*) FROM stp_file WHERE ",g_wc CLIPPED
     DISPLAY g_sql
    PREPARE i160_cnt FROM g_sql
    DECLARE i160_count                         # SCROLL CURSOR
        SCROLL CURSOR WITH HOLD FOR i160_cnt
END FUNCTION
    ...
    MENU ""
        ...
        ON ACTION query
            LET g_action_choice="query"
                 CALL i160_q()
        #   NEXT OPTION "next"   #TQC-960342 mark
        ON ACTION next
            CALL i160_fetch('N')
        ON ACTION previous
            CALL i160_fetch('P')
...
FUNCTION i160_a()
    ...
    CLEAR FORM                                   # 清螢墓欄位內容
    INITIALIZE g_stp.* LIKE stp_file.*
    LET g_stp02_t = NULL
    CALL cl_opmsg('a')
    WHILE TRUE
        #NO.FUN-590002-start----
        ...
        LET g_stp.stpplant = g_plant #FUN-980008 add
        LET g_stp.stplegal = g_legal #FUN-980008 add
        ...
        IF g_stp.stp02 IS NULL THEN                # KEY 不可空白
            CONTINUE WHILE
        END IF
        INSERT INTO stp_file VALUES(g_stp.*)       # DISK WRITE
        IF SQLCA.sqlcode THEN
#           CALL cl_err(g_stp.stp02,SQLCA.sqlcode,0)   #No.FUN-660120
            CALL cl_err3("ins","stp_file",g_stp.stp02,"",SQLCA.sqlcode,"","",1)  #No.FUN-660120
            CONTINUE WHILE
        ELSE
            LET g_stp_t.* = g_stp.*                # 保存上筆資料
            SELECT stp02 INTO g_stp.stp02 FROM stp_file
...
FUNCTION i160_i(p_cmd)
    ...
    DISPLAY BY NAME
        g_stp.stp02, g_stp.stp021,g_stp.stp01, g_stp.stp03, g_stp.stp04,
        g_stp.stp05, g_stp.stp06, g_stp.stp07, g_stp.stp08, g_stp.stp09,
        g_stp.stp10, g_stp.stp11, g_stp.stp12, g_stp.stp13, g_stp.stp14,
        g_stp.stp15
    INPUT BY NAME
        g_stp.stp02, g_stp.stp021,g_stp.stp01, g_stp.stp03, g_stp.stp04,
        g_stp.stp05, g_stp.stp06, g_stp.stp07, g_stp.stp08, g_stp.stp09,
...
#              IF NOT cl_null(g_stp.stp02) THEN
...
#FUN-AB0025 ----------mark end-------------
               IF p_cmd = "a" OR                    # 若輸入或更改且改KEY
                 (p_cmd = "u" AND g_stp.stp02 != g_stp02_t ) THEN
                  SELECT count(*) INTO l_n FROM stp_file
                   WHERE stp02 = g_stp.stp02
        ...
        AFTER  FIELD stp07
            IF l_stp07 <> g_stp.stp07 then
               ...
               ## 採購件
               IF g_stp.stp01 = '1'  then
                  LET g_stp.stp08 = g_stp.stp07 * g_stp.stp03
               ELSE
                  LET g_stp.stp08 = g_stp.stp07 * g_stp.stp03*
                                    (100- l_sto05 )/100
               END IF
               # 重置成本= 重置單價* 期末數量 * sto_file.sto05(推銷費用率%)
...
FUNCTION i160_fetch(p_flstp)
    DEFINE
        p_flstp         LIKE type_file.chr1,         #No.FUN-690010 VARCHAR(1),
        l_abso          LIKE type_file.num10   #No.FUN-690010 INTEGER
 
    CASE p_flstp
        WHEN 'N' FETCH NEXT     i160_cs INTO g_stp.stp02
        WHEN 'P' FETCH PREVIOUS i160_cs INTO g_stp.stp02
        WHEN 'F' FETCH FIRST    i160_cs INTO g_stp.stp02
        WHEN 'L' FETCH LAST     i160_cs INTO g_stp.stp02
        WHEN '/'
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
               ...
               END IF
            END IF
            FETCH ABSOLUTE g_jump i160_cs INTO g_stp.stp02
            LET mi_no_ask = FALSE
    END CASE
    ...
    SELECT * INTO g_stp.* FROM stp_file            # 重讀DB,因TEMP有不被更新特性
       WHERE stp02 = g_stp.stp02
    IF SQLCA.sqlcode THEN
#       CALL cl_err(g_stp.stp01,SQLCA.sqlcode,0)   #No.FUN-660120
        CALL cl_err3("sel","stp_file",g_stp.stp02,"",SQLCA.sqlcode,"","",1)  #No.FUN-660120
    ELSE
        CALL i160_show()                      # 重新顯示
    END IF
...
#   DISPLAY BY NAME g_stp.*    #FUN-980008 mod
    DISPLAY BY NAME g_stp.stp01,  g_stp.stp02, g_stp.stp021, g_stp.stp03,
         g_stp.stp04, g_stp.stp05, g_stp.stp06, g_stp.stp07, g_stp.stp08,
         g_stp.stp09, g_stp.stp10, g_stp.stp11, g_stp.stp12, g_stp.stp13,
         g_stp.stp14, g_stp.stp15
    CALL cl_show_fld_cont()                   #No.FUN-550037 hmf
END FUNCTION
...
FUNCTION i160_u()
    ...
    BEGIN WORK
    OPEN i160_cl USING g_stp.stp02
    IF STATUS THEN
       ...
       RETURN
    END IF
    FETCH i160_cl INTO g_stp.*               # 對DB鎖定
    IF SQLCA.sqlcode THEN
        CALL cl_err(g_stp.stp01,SQLCA.sqlcode,0)
        RETURN
    END IF
    ...
    WHILE TRUE
        ...
        IF INT_FLAG THEN
            ...
            EXIT WHILE
        END IF
        UPDATE stp_file SET stp_file.* = g_stp.*    # 更新DB
            WHERE stp02 = g_stp.stp02             # COLAUTH?
        IF SQLCA.sqlcode THEN
#           CALL cl_err(g_stp.stp01,SQLCA.sqlcode,0)   #No.FUN-660120
            CALL cl_err3("upd","stp_file",g_stp_t.stp02,"",SQLCA.sqlcode,"","",1)  #No.FUN-660120
            CONTINUE WHILE
        END IF
        EXIT WHILE
    END WHILE
...
FUNCTION i160_r()
    ...
    IF cl_delete() THEN
       ...
       DELETE FROM stp_file WHERE stp02=g_stp.stp02
       CLEAR FORM
       OPEN i160_count
       FETCH i160_count INTO g_row_count
       DISPLAY g_row_count TO FORMONLY.cnt
