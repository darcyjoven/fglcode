...
#
# Pattern name...: asdr410.4gl
# Descriptions...: 材料成本與巿價孰低比較表
# Date & Author..: 99/10/10 By Eric
...
MAIN
   ...
   LET l_table = cl_prt_temptable('asdr410',g_sql) CLIPPED   # 產生Temp Table
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW asdr11_w AT p_row,p_col WITH FORM "asd/42f/asdr410" 
   ...
   LET g_copies = '1'
   WHILE TRUE
     CONSTRUCT BY NAME tm.wc ON stp02,stp09 
...
#No.FUN-570244 --end  
     ...
     IF g_bgjob = 'Y' THEN
        ...
        ELSE
           ...
           CALL cl_cmdat('asdr410',g_time,l_cmd)    # Execute cmd at later time
        END IF
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0089
  ...
  SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog   ### FUN-750031 add ###
     ...
     DELETE FROM r410tmp
     LET l_sql = "SELECT * FROM stp_file",
                 " WHERE stp01='1' AND ",tm.wc CLIPPED
     ...
     #CALL cl_outnam('asdr410') RETURNING l_name
     #START REPORT asdr11_rep TO l_name
     LET g_pageno = 0
     LET l_cnt=1
     FOREACH asdr11_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
       ...
       ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-750031 *** ##
         EXECUTE insert_prep USING sr.stp02,
                                   l_ima02,
                                   l_ima021,
                                   sr.stp03,
                                   sr.stp04,
       ...
       SELECT * FROM r410tmp WHERE stp021=sort.stp021
       IF STATUS <> 0 THEN
          INSERT INTO r410tmp VALUES(sort.*)
       ELSE
          UPDATE r410tmp
       ...
       LET l_cnt=l_cnt+1
     END FOREACH
     # FINISH REPORT asdr11_rep  CALL  cl_used(g_prog,g_time,2) RETURNING g_time #No.MOD-580088  HCN 20050818  #No.FUN-6A0089
     ...
     #是否列印選擇條件
     IF g_zz05 = 'Y' THEN
        CALL cl_wcchp(tm.wc,'stp02,stp09')
             RETURNING tm.wc
        LET g_str = tm.wc
     END IF
     LET g_str = g_str,";"
     CALL cl_prt_cs3('asdr410','asdr410',l_sql,g_str)   
     #------------------------------ CR (4) ------------------------------#
END FUNCTION
