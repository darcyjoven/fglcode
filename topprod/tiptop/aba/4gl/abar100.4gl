...
#
# Pattern name...: abar100.4gl
# Descriptions...: 条码打印
# Date & Author..: No:DEV-CB0011 2012/11/14 By TSD.JIE
# Modify.........: No.DEV-D30025 2013/03/11 By Nina---GP5.3 追版:以上為GP5.25 的單號---
# Modify.........: No.DEV-D30032 2013/03/19 By TSD.JIE 列印報表類型增加3.純條碼
# Modify.........: No.DEV-D30041 2013/03/21 By TSD.JIE 增加記錄列印次數(ibb12)
...
DEFINE tm   RECORD
            wc       STRING,
            iba02    LIKE iba_file.iba02,  #條碼類型 F:條碼批號-料號 G:條碼批號-工單
                                           #         H:包號 D:訂單包裝單
                                           #         5:製造批號 6:序號編號
            ibb02    LIKE ibb_file.ibb02,  #條碼產生時機點 A:工單(asfi301) F:採購單(apmt540)
                                           #               G:委外採購單(apmt590) H:訂單包裝單(abai140)
                                           #               I:雜收單(aimt302)
            type     LIKE type_file.chr1,  #印表(rpt樣版)類型：1:一般條碼2:包裝單條碼
...
MAIN
   ...
   LET l_table = cl_prt_temptable ('abar100',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN
      CALL abar100_tm(0,0)        # Input print condition
   ELSE
      CALL abar100()              # Read data and create out-file
   END IF

   CALL cl_used(g_prog,g_time,2) RETURNING g_time
END MAIN

FUNCTION abar100_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01
DEFINE p_row,p_col    LIKE type_file.num5,
       l_cmd          LIKE type_file.chr1000

   LET p_row = 4 LET p_col = 16
   OPEN WINDOW abar100_w AT p_row,p_col WITH FORM "aba/42f/abar100"
   ...
   WHILE TRUE
      ...
      END INPUT
      IF INT_FLAG THEN
          LET INT_FLAG = 0 CLOSE WINDOW abar100_w
          CALL cl_used(g_prog,g_time,2) RETURNING g_time
          EXIT PROGRAM
      END IF
      CALL cl_wait()
...
FUNCTION abar100()
   ...
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'abar100'
   ...
   LET l_sql = "SELECT ibb_file.*, iba01, iba02, ",  #條碼資訊
               ...
               " WHERE ",tm.wc 
              ,"   AND ibb11 = 'Y' "    #有效 #No.DEV-D30041 add
   IF NOT cl_null(tm.iba02) THEN
   ...
   PREPARE abar100_prepare FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      EXIT PROGRAM
   END IF
   DECLARE abar100_curs CURSOR FOR abar100_prepare

   FOREACH abar100_curs INTO l_ibb.*,l_iba01,l_iba02,l_ima02,l_ima021
      IF SQLCA.sqlcode != 0 THEN
   ...
   LET g_str = tm.wc,";",tm.type
   #CALL cl_prt_cs3('abar100','abar100',g_sql,g_str) #DEV-D30032--mark

   #DEV-D30032--begin
   IF tm.type = '3' THEN
      CALL cl_prt_cs3('abar100','abar100_3',g_sql,g_str)
   ELSE 
      CALL cl_prt_cs3('abar100','abar100',g_sql,g_str)
   END IF
   #DEV-D30032--end
