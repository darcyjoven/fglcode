...
#
# Pattern name...: abai104.4gl
# Descriptions...: 销售退货待入库物料条码生成作业
...
FUNCTION i104_tm(p_row,p_col)
   ...
   OPEN WINDOW i104_w AT p_row,p_col WITH FORM "aba/42f/abai102"
   ...
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      AFTER FIELD tc_ibb03
         IF NOT cl_null(g_tm.tc_ibb03) THEN
            SELECT COUNT(*) INTO l_n FROM oha_file WHERE oha01 = g_tm.tc_ibb03
            IF l_n = 0 THEN
               CALL cl_err('','单号不存在！',0)
               NEXT FIELD tc_ibb03
            ELSE
      ...
      AFTER FIELD tc_ibb04 
         IF NOT cl_null(g_tm.tc_ibb04) AND NOT cl_null(g_tm.tc_ibb03) THEN
            SELECT COUNT(*) INTO l_n FROM ohb_file WHERE ohb01 = g_tm.tc_ibb03 
            AND ohb03=g_tm.tc_ibb04
            IF l_n = 0 THEN
               CALL cl_err('','单号项次不匹配！',0)
               NEXT FIELD ibb01
            ELSE
               SELECT ohb04,ohb12 INTO g_tm.tc_ibb06,g_tm.tc_ibb07,g_tm.tc_ibb10 
               FROM ohb_file WHERE ohb01 = g_tm.tc_ibb03 
                  AND ohb03=g_tm.tc_ibb04
               ...
               IF cl_null(g_tm.tc_ibb10) THEN 
               	  LET g_tm.tc_ibb10=1
               END IF 
               SELECT oha03,oha032 INTO g_tm.tc_ibb22,g_tm.tc_ibb22_n 
               FROM oha_file WHERE oha01=g_tm.tc_ibb03
