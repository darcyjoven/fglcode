...
#
# Pattern name...: abai105.4gl
# Descriptions...: 杂收单待入库物料条码生成作业
...
FUNCTION i105_tm(p_row,p_col)
   ...
   OPEN WINDOW i105_w AT p_row,p_col WITH FORM "aba/42f/abai102"
   ...
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      AFTER FIELD tc_ibb03
         IF NOT cl_null(g_tm.tc_ibb03) THEN
            SELECT COUNT(*) INTO l_n FROM ina_file WHERE ina01 = g_tm.tc_ibb03
            IF l_n = 0 THEN
               CALL cl_err('','单号不存在！',0)
               NEXT FIELD tc_ibb03
            ELSE
      ...
      AFTER FIELD tc_ibb04 
         IF NOT cl_null(g_tm.tc_ibb04) AND NOT cl_null(g_tm.tc_ibb03) THEN
            SELECT COUNT(*) INTO l_n FROM inb_file WHERE inb01 = g_tm.tc_ibb03 
            AND inb03=g_tm.tc_ibb04
            IF l_n = 0 THEN
               CALL cl_err('','单号项次不匹配！',0)
               NEXT FIELD ibb01
            ELSE
               SELECT inb04,inb09 INTO g_tm.tc_ibb06,g_tm.tc_ibb07 
               FROM inb_file WHERE inb01 = g_tm.tc_ibb03 
                  AND inb03=g_tm.tc_ibb04
               ...
               IF cl_null(g_tm.tc_ibb10) THEN 
               	  LET g_tm.tc_ibb10=1
               END IF 
               SELECT ina1001 INTO g_tm.tc_ibb22 FROM ina_file WHERE ina01=g_tm.tc_ibb03
               SELECT occ02 INTO g_tm.tc_ibb22_n  FROM occ_file WHERE occ01=g_tm.tc_ibb22
