# Prog. Version..: '5.30.06-13.03.12(00000)'     #
#
# Pattern name...: aict040.4gl
# Descriptions...: WAFER採購單維護作業
# Date & Author..: 08/01/08 By hellen 
# Modify.........: No.FUN-810038 08/01/25 By kim GP5.1 ICD
 
# Modify.........: No.FUN-980030 09/08/31 By Hiko 加上GP5.2的相關設定
 
DATABASE ds
 
GLOBALS "../../config/top.global"
GLOBALS "../../apm/4gl/sapmt540.global"
 
  #DEFINE g_argv1  LIKE pmm_file.pmm01                     #採購單號
  #DEFINE g_argv2  LIKE type_file.chr1                     #狀況碼
  #DEFINE g_argv3  LIKE pmm_file.pmm02                     #採購單性質 
  #DEFINE g_argv4  LIKE sfb_file.sfb01                     #工單單號
  #DEFINE g_argv5  LIKE sfb_file.sfb05                     #part no
  #DEFINE g_argv6  LIKE sfb_file.sfb15                     #finishdate
  #DEFINE g_argv7  LIKE sfb_file.sfb08                     #qty
  #DEFINE g_argv8  LIKE type_file.num5                     #本次作業序號
  #DEFINE g_argv9  LIKE type_file.num5                     #下次作業序號
  #DEFINE g_argv10 LIKE sfb_file.sfb08                     #轉入下一作業數量
  #DEFINE g_argv12 STRING                                  #執行功能
  DEFINE l_time   LIKE type_file.chr8       
 
MAIN
 
   IF FGL_GETENV("FGLGUI") <> "0" THEN
      OPTIONS
          INPUT NO WRAP,
          FIELD ORDER FORM                                #整個畫面會依照p_per所設定的欄位順序(忽略4gl寫的順序)
   END IF
 
   DEFER INTERRUPT
 
   #移位
   LET g_argv1 = ARG_VAL(1)                               #參數-1(採購單號)
   LET g_argv12 = ARG_VAL(2)                              #執行功能   
   LET g_argv2 = "0"                                      #參數-2(狀況碼)(已核准)
   LET g_argv3 = ARG_VAL(3)                               #參數-3(性質)
   LET g_argv4 = ARG_VAL(4)                               #參數-4(工單單號)
   LET g_argv5 = ARG_VAL(5)                               #參數-5 Parts No     
   LET g_argv6 = ARG_VAL(6)                               #參數-6 Finish date  
   LET g_argv7 = ARG_VAL(7)                               #     7 qty      
   LET g_argv8 = ARG_VAL(8)                               #參數-8(本次作業序號)
   LET g_argv9 = ARG_VAL(9)                               #參數-9(下次作業序號)
   LET g_argv10= ARG_VAL(10)                              #轉入下一作業數量
      
   IF (NOT cl_user()) THEN
      EXIT PROGRAM
   END IF
  
   IF (NOT s_industry('icd')) THEN
      CALL cl_err('','aic-999',1)
      EXIT PROGRAM
   END IF
 
   WHENEVER ERROR CALL cl_err_msg_log
  
   IF (NOT cl_setup("AIC")) THEN
      EXIT PROGRAM
   END IF
 
 
   CALL cl_used(g_prog,l_time,1) RETURNING l_time
   CALL t540(g_argv1,g_argv2,g_argv3,g_argv4,g_argv5,g_argv6,g_argv7,
             g_argv8,g_argv9,g_argv10,g_argv12)
   CALL cl_used(g_prog,l_time,2) RETURNING l_time
END MAIN
#FUN-810038
