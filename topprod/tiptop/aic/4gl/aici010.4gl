...
# Descriptions...: ICD料件FT維護作業                                                                                                
# Date & Author..: No.FUN-7B0075 07/11/15 By Sunyanchun
# Modify.........: No.TQC-840008 08/04/02 By Sunyanchun  查詢出資料，再查詢，然后退出，畫面上保留上一次查詢的值
# Modify.........: NO.FUN-860108 08/06/26 By Sunyanchun 多用戶訪問同一條數據的時候，資料沒有被鎖定。 
...
DEFINE g_ickd       RECORD                      #單頭
       ...
       g_ick         DYNAMIC ARRAY OF RECORD      #單身
...
FUNCTION i010_curs()
   ...
   ELSE                                      
      ...
      INITIALIZE g_ickd.* TO NULL     
      CONSTRUCT g_wc ON ick01,ick02,ick09,ick03,ick05,
                        ick06,ick07,ick08,ick10,ick04                               
                FROM ick01,s_ick[1].ick02,s_ick[1].ick09,s_ick[1].ick03,   
   ...
   DECLARE i010_count CURSOR FOR i1010_precount    #計算查詢到資料的筆數杅         
...
FUNCTION i010_b()
    ...
    LET g_sql= "SELECT ick02,'',ick09,'',ick03,ick05,ick06,ick07,ick08,ick10,ick04 ",
               " FROM ick_file  WHERE ick01 = ? AND ick02 = ? AND ick09 = ? ",
               "FOR UPDATE "         #NO.FUN-860108
    ...
    BEFORE ROW
       LET p_cmd = ''
       LET l_ac = ARR_CURR()   #得到單身當前行俴
       LET l_lock_sw = 'N'
       LET l_n = ARR_COUNT()   #得到單身總行數軞俴
               
       IF g_rec_b>=l_ac THEN   #單身處于修改狀態
          SELECT ickacti INTO g_ickd.ickacti FROM ick_file 
            WHERE ick01=g_ickd.ick01 AND ick02=g_ick_t.ick02 AND ick03=g_ick_t.ick03
          IF g_ickd.ickacti = 'N' THEN
          ...
          BEGIN WORK 
          LET p_cmd = 'u'
          LET g_ick_t.* = g_ick[l_ac].*    #備份舊值
          LET g_ick_o.* = g_ick[l_ac].*    #備份舊值 
          OPEN i010_clt USING g_ickd.ick01,g_ick_t.ick02,g_ick_t.ick09
          IF SQLCA.sqlcode THEN
             CALL cl_err("OPEN i010_clt:",SQLCA.sqlcode,1)
             CLOSE i010_clt
             ROLLBACK WORK
             #RETURN         #NO.FUN-860108
          END IF
    ...
    AFTER INSERT
       ...
       INSERT INTO ick_file(ick01,ick02,ick03,ick04,ick05,ick06,
         ...
         VALUES(g_ickd.ick01,g_ick[l_ac].ick02,g_ick[l_ac].ick03,
                ...
                cast(g_ick[l_ac].ick07 as date),g_ick[l_ac].ick08,g_ick[l_ac].ick09,
                g_ick[l_ac].ick10,
       ...
       AFTER FIELD ick02
           IF NOT cl_null(g_ick[l_ac].ick02) THEN
              #FUN-AA0059 ------------------add start-------------
               ...
               END IF 
              #FUN-AA0059 -------- -- add end------------------------  
               IF g_ick[l_ac].ick02 = g_ickd.ick01 THEN
                  CALL cl_err('ick02','aic010',0)
                  NEXT FIELD ick02
               END IF
       ...
       AFTER FIELD ick09
           ...
           END IF
       AFTER FIELD ick10 
           IF g_ick[l_ac].ick10<0 THEN
              CALL cl_err('ick10','aic-005',0)
              NEXT FIELD ick10
           END IF      
       BEFORE DELETE 
           ...
           ELSE
              UPDATE ick_file                                                                                                      
                  SET ick02=g_ick[l_ac].ick02,                                                                                         
                      ...
                      ickgrup=g_grup,                                                                                               
                      ickdate=g_today
              WHERE ick02=g_ick_t.ick02 AND ick01=g_ickd.ick01 AND ick09=g_ick_t.ick09
      ...
      AFTER ROW
       ...
       ON ACTION CONTROLO                 #沿用所有欄位                                                                     
           IF INFIELD(ick02) AND l_ac > 1 THEN                                                                                      
              LET g_ick[l_ac].* = g_ick[l_ac-1].*                                                                                   
              NEXT FIELD ick02                                                                                                      
           END IF     
       ON ACTION CONTROLR
...
END FUNCTION
#檢查一個值是否在表里，返回的l_count的值就是根據條件查詢到的結果
#p_field         檢查的闌位
#p_value         當前闌位的值
#p_table         查詢的表
#p_condition     附加條件
...
#FUN-AA0059 --End--
    ...
    CALL i010_b()
    #LET g_ickd.ick01 = l_oldno  #FUN-C30027
    #CALL i010_show()            #FUN-C30027
END FUNCTION
...
#      RETURN
#   END IF
   IF cl_null(g_wc) AND NOT cl_null(g_ickd.ick01) THEN
   ...
   LET g_sql="SELECT distinct ick01,ick02,ick03,ick04,ick05,ick06,ick07,ick08,",
             ...
             " FROM ick_file LEFT OUTER JOIN ima_file A ON ick01=A.ima01 ",
             " LEFT OUTER JOIN ima_file B ON ick02=B.ima01 ",
             " LEFT OUTER JOIN pmc_file ON ick09=pmc01 ",  
             " WHERE ",g_wc CLIPPED,
