...
# Modify.........: No.TQC-5A0109 05/10/28 By Mandy No.MOD-570253 oeb08=>no use
# Modify.........: No.TQC-5A0131 05/11/08 By Nicola 單身不提供查詢
# Modify.........: No.FUN-610020 06/01/06 By Carrier 出貨驗收功能 -- 單身新增驗收欄位acc_q*/acd_q*
...
# Modify.........: No:MOD-B30059 11/03/09 By Summer 單身要納入借貨出貨單的資料
# Modify.........: No:TQC-C90088 12/09/20 By dongsz 訂單單號欄位增加開窗
...
DEFINE
    tm  RECORD
               #wc      LIKE type_file.chr1000,# Head Where condition  #No.FUN-680137 VARCHAR(500) #TQC-C90088 mark                                            
               #wc2     LIKE type_file.chr1000 # Body Where condition  #No.FUN-680137 VARCHAR(500) #TQC-C90088 mark
                wc      STRING,                #TQC-C90088 add
                wc2     STRING                 #TQC-C90088 add
        END RECORD,
    g_oea   RECORD
...
#       CALL q410_q()
#    END IF
IF NOT cl_null(g_argv1) THEN CALL q410_q() END IF
    CALL q410_menu()
    CLOSE WINDOW q410_w
...
FUNCTION q410_cs()
   ...
   INITIALIZE g_oea.* TO NULL    #No.FUN-750051
           CONSTRUCT BY NAME tm.wc ON
                    ...
                    #oeb08,oeb09,oeb091,oeb092, #TQC-5A0109
      ...
      ON ACTION controlg      #MOD-4C0121
		...
		#No.FUN-580031 --end--       HCN
          ...
          #CALL q410_b_askkey()   #No.MOD-5A0131
           IF INT_FLAG THEN RETURN END IF
   END IF
   ...
   LET g_sql = "SELECT COUNT(*) FROM oea_file,oeb_file",
               " WHERE ",tm.wc CLIPPED,
               "   AND oeaconf != 'X' ", #01/08/15 mandy
               "   AND oeb01 = oea01 "
   ...
   DECLARE q410_cnt   CURSOR FOR q410_pp
END FUNCTION
...
FUNCTION q410_q()
    ...
    ELSE
        OPEN q410_cnt
        FETCH q410_cnt INTO g_row_count
...
DEFINE
    ...
    CASE p_flag
       WHEN 'N' FETCH NEXT     q410_cs INTO g_oea.oea01,
                                            g_oea.oeb03  #CHI-930019 
       WHEN 'P' FETCH PREVIOUS q410_cs INTO g_oea.oea01,
                                            g_oea.oeb03  #CHI-930019 
       WHEN 'F' FETCH FIRST    q410_cs INTO g_oea.oea01,
                                            g_oea.oeb03  #CHI-930019 
       WHEN 'L' FETCH LAST     q410_cs INTO g_oea.oea01,
                                            g_oea.oeb03  #CHI-930019 
       WHEN '/'
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
            ...
            END IF
            LET mi_no_ask = FALSE
            FETCH ABSOLUTE g_jump q410_cs INTO g_oea.oea01,g_oea.oeb03  #CHI-930019
    END CASE
	...
	SELECT oea01,oea02,oea03,oea04,oea032,'',oea14,oea15,oeaconf,oeahold,
        ...
        #FUN-570175  --end
          #oeb15,oeb70,oeb06,oeb08,oeb09,oeb091,oeb16 #TQC-5A0109
           oeb15,oeb70,oeb06,      oeb09,oeb091,oeb16 #TQC-5A0109
	  INTO g_oea.*
...
FUNCTION q410_b_fill()              #BODY FILL UP
   ...
   LET l_sql =
   #FUN-570175  --begin
        "SELECT oga02,ogb01,ogb03,oga10,0,ogb12,ogb913,ogb915,0,ogb910,", #No.FUN-610020
   ...
   #FUN-570175  --end
        ...
        "   AND oga01=ogb01 ",
        "   AND oga09 IN ('2','4','6','A') ",  #No.FUN-610020 #MOD-B30059 add 'A'
        "   AND ogapost='Y'",
        " ORDER BY oga02"   #No.FUN-610020
    PREPARE q410_pb FROM l_sql
    DECLARE q410_bcs                       #BODY CURSOR
        CURSOR WITH HOLD FOR q410_pb
#TQC-790065--start--
...
#TQC-790065--end--
    LET g_cnt = 1
    FOREACH q410_bcs INTO g_oga[g_cnt].*,l_oga09  #No.FUN-610020
        IF SQLCA.sqlcode THEN
        ...
        #No.FUN-610020  --Begin
           SELECT SUM(ogb12),SUM(ogb915),SUM(ogb912)
             INTO g_oga[g_cnt].acc_q,g_oga[g_cnt].acc_q2,g_oga[g_cnt].acc_q1
             FROM oga_file,ogb_file
            WHERE oga01 = ogb01
              AND ogaconf = 'Y' AND ogapost = 'Y'
              AND oga09 = '8'   
