...
#
# Pattern name...: axmp895.4gl
# Descriptions...: 三角貿易銷退應收/應付帳款拋轉還原作業(逆拋)
# Date & Author..: NO.FUN-710019 07/01/05 BY yiting 
...
FUNCTION p895_p1()
    ...
    OPEN WINDOW p895_w WITH FORM "axm/42f/axmp895" 
 ...
 WHILE TRUE
    ...
    LET tm.oha05 = '2'
    INPUT BY NAME tm.oha01,tm.oha05,g_bgjob   WITHOUT DEFAULTS   #NO.FUN-570155 
         AFTER FIELD oha01
            ...
            IF g_oha.oha10 IS NULL THEN    #帳單編號
               CALL cl_err(g_oha.oha10,'axm-077',0)
               NEXT FIELD oha01 
            END IF
   ...
   END IF
   IF g_bgjob = 'Y' THEN
      SELECT zz08 INTO lc_cmd FROM zz_file WHERE zz01 = 'axmp895'
      IF SQLCA.sqlcode OR lc_cmd IS NULL THEN
          CALL cl_err('axmp895','9031',1)   
      ELSE
         LET lc_cmd = lc_cmd CLIPPED,
                      " '",tm.oha01 CLIPPED,"'",
                      " '",g_bgjob CLIPPED,"'"
         CALL cl_cmdat('axmp895',g_time,lc_cmd CLIPPED)
      END IF
      CLOSE WINDOW p895_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-B30211
      EXIT PROGRAM
...
FUNCTION p895_p2()
  ...
  DEFINE l_cnt  LIKE type_file.num5           #FUN-710019
     ...
     IF g_poz.poz011 = '2' THEN
        IF g_poz.poz19 = 'N' OR (g_poz.poz19 = 'Y' AND l_cnt = 0 ) THEN
...
DEFINE l_j    LIKE type_file.num5,          #No.FUN-680137 SMALLINT
       l_msg  LIKE type_file.chr1000        #No.FUN-680137 VARCHAR(60)
           ...
           IF i != p_last THEN   ##最後一家工廠不必拋AP
               ...
               CLOSE apy_c1
           END IF
           IF tm.oha05 = '2' OR (tm.oha05='3' AND i <> 0) THEN
               #判斷應收帳款是否已拋轉至總帳
           ...
           IF tm.oha05 = '2' OR (tm.oha05='3' AND i <> 0) THEN
               ...
               IF p_ooy.ooydmy1 = 'Y' THEN
                  ...
                  IF l_npp.nppglno IS NOT NULL THEN
                     CALL cl_err('nppglno(AR):','axm-898',1)
                     LET g_success='N' RETURN
                  END IF
               END IF
           END IF
           IF i != p_last THEN    ##最後一家工廠不必拋AP
             IF p_apy.apydmy3 = 'Y' THEN
              ...
              IF l_npp.nppglno IS NOT NULL THEN
                 CALL cl_err('nppglno(AP):','axm-898',1)
                 LET g_success='N' RETURN
              END IF
             END IF
           END IF
           #****************************************************#
           #還原各廠之應收/應付資料
           #CALL p895_pro(l_dbs_new,i)
           CALL p895_pro(l_plant_new,i) #FUN-A50102
END FUNCTION
 
FUNCTION p895_azp(l_i)
...
#FUNCTION p895_pro(p_dbs,i) 
FUNCTION p895_pro(l_plant,i) #FUN-A50102
 ...
 DEFINE l_cnt  LIKE type_file.num5     #CHI-B40063 add
     ...
     IF tm.oha05 = '2' OR (tm.oha05='3' AND i <> 0) THEN
        ...
        #LET l_sql="UPDATE ",p_dbs CLIPPED,"oha_file",
         ...
         IF SQLCA.sqlcode<>0 THEN
            CALL cl_err('upd oha10:',SQLCA.sqlcode,1)
            LET g_success = 'N'
         END IF
         #更新出貨單單身檔之已開折讓數量
        ...
        LET l_sql="UPDATE ",cl_get_target_table(l_plant,'ohb_file'), #FUN-A50102
         ...
         IF SQLCA.sqlcode<>0 THEN
            CALL cl_err('upd ohb60:',SQLCA.sqlcode,1)
            LET g_success = 'N'
         END IF
         IF p_ooy.ooydmy1 = 'Y' THEN
             ...
             EXECUTE del_tic_ar
             IF SQLCA.SQLCODE THEN
                CALL cl_err('del tic(AR):',SQLCA.SQLCODE,1)
                LET g_success='N' 
             END IF 
             #FUN-B40056--add--end--
   ...
   IF i <> p_last THEN
      ...
      IF p_apy.apydmy3 = 'Y' THEN
       ...
       EXECUTE del_tic_ap
       IF SQLCA.SQLCODE THEN
          CALL cl_err('del tic(AP):',SQLCA.SQLCODE,1)
          LET g_success='N'
       END IF
       #FUN-B40056--add--end--
      END IF
...
#     LET AR_t1=g_oma01[1,3]
     LET AR_t1=g_oma01                       #No.FUN-550070
END FUNCTION
