...
# Modify.........: No.FUN-A70068 10/07/21 By shenyang 加上營運中心代碼  并跨庫處理
...
# Modify.........: No.CHI-CA0003 12/12/25 By pauline 新增"部分已交"選項,並將原本"已交"改為"全部已交"
# Modify.........: No.MOD-D40063 13/04/11 By Vampire 當2:未出貨，oeb12=0 不產生報表
...
MAIN
   ...
   LET g_sql = " azp01.azp_file.azp01,",         #No.FUN-A70068
               ...
               " oeb24.oeb_file.oeb24,",   #CHI-CA0003 add
   ...
   LET l_table = cl_prt_temptable('axmr450',g_sql) CLIPPED
...
#No.FUN-A70068  ..end
   ...
   OPEN WINDOW r450_w AT p_row,p_col WITH FORM "axm/42f/axmr450"
...
#  IF tm.wc = ' 1=1' THEN CALL cl_err('','9046',0) CONTINUE WHILE END IF
   INPUT BY NAME tm2.s1,tm2.s2,tm2.s3,
      ...
      AFTER FIELD a
        #IF tm.a NOT MATCHES '[123]' THEN NEXT FIELD a END IF   #CHI-CA0003 mark
         IF tm.a NOT MATCHES '[1234]' THEN NEXT FIELD a END IF   #CHI-CA0003 add
      AFTER FIELD more
         IF tm.more = 'Y' THEN
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmr450',g_time,l_cmd)
      END IF
      CLOSE WINDOW r450_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
...
#DEFINE l_za05    LIKE type_file.chr1000        #No.FUN-680137 VARCHAR(40)                                      #TQC-C50101 mark
 ...
 DEFINE sr        RECORD azp01 LIKE azp_file.azp01,          #No.FUN-A70068
                        ...
                        oeb24 LIKE oeb_file.oeb24               #CHI-CA0003 add
                        END RECORD,
...
#      WHERE azi01=g_aza.aza17               #No.CHI-6A0004
     ...
     LET l_sql = "SELECT azp01,'','','',",
                 ...
                 "oeb13, oeb14, oeb15,oeb910,oeb912,oeb913,oeb915,oeb916,oeb917,oea14, oea15,  ",       #No.FUN-580004
                 "oeb12-oeb24+oeb25-oeb26 oeb24 ",   #CHI-CA0003 add
     ...
     CASE tm.a
         WHEN '1'        #已交   #全部已交
           #LET l_sql = l_sql CLIPPED," AND oeb24 > 0 "   #CHI-CA0003 mark
            LET l_sql = l_sql CLIPPED," AND oeb24  = oeb12 "  #CHI-CA0003 add
         WHEN '2'        #未交 BugNo:4038 已結案訂單不可納入
            ...
            LET l_sql = l_sql CLIPPED," AND oeb12 <> 0 " #MOD-D40063 add 
        #CHI-CA0003 add START
         WHEN '4'       #部分已交
            LET l_sql = l_sql CLIPPED," AND oeb24 > 0 AND oeb24 < oeb12 "
        #CHI-CA0003 add END
...
display 'l_sql:',l_sql
     ...
     #No.FUN-750129  --Begin
     #CALL cl_outnam('axmr450') RETURNING l_name
...
#FUN-580004--begin
   ...
   LET  l_sql="SELECT sma115,sma116 FROM ",cl_get_target_table(l_plant,'sma_file')
     ...
     # CALL cl_prt_pos_len()
#No.FUN-580004--end
     #START REPORT r450_rep TO l_name
     #LET g_pageno = 0
    ...
    LET l_sql="SELECT COUNT(*)  FROM ",cl_get_target_table(l_plant,'oga_file'),
         ...
         CALL cl_replace_sqldb(l_sql) RETURNING l_sql
         CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql
             PREPARE r450_c1 from l_sql
             DECLARE r450_c1_curs CURSOR FOR  r450_c1 
              FOREACH r450_c1_curs INTO  l_oga.*
                IF STATUS THEN
         ...
         CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql
                  ...
                  PREPARE r450_c2 from l_sql
                  DECLARE r450_c2_curs CURSOR FOR  r450_c2 
                 FOREACH r450_c2_curs INTO l_oma.oma01,l_oma.oma10,l_oma.omb03
 ...
 END FOREACH
     ...
     CALL cl_prt_cs3('axmr450',l_name,g_sql,g_str)
     #No.FUN-750129  --End  
END FUNCTION
...
#               LET l_oma.oma10=''
#               LET l_oma.omb03=''
#               DECLARE r450_c2 CURSOR FOR
#                  SELECT oma01,oma10,omb03 FROM oma_file,omb_file
#                   WHERE oma01=omb01 AND omb31=l_oga.oga01
#                     AND omb32=l_oga.ogb03 AND omaconf='Y'
#                     ORDER BY oma01,omb03
#               FOREACH r450_c2 INTO l_oma.oma01,l_oma.oma10,l_oma.omb03
#                  IF SQLCA.sqlcode THEN
#                     PRINT ''
#                     EXIT FOREACH
#                  END IF
