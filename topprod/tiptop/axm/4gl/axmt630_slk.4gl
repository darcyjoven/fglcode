...
DEFINE p_row,p_col   LIKE type_file.num5          #No.FUN-680137 SMALLINT
    ...
    IF g_argv1 = '1' THEN
	     LET g_prog = 'axmt630_slk' 
    ELSE
	     LET g_prog = 'axmt632_slk' 
    END IF
...
#CHI-CC0034 add --end--
   ...
   # 若有新增的出貨項次                                                                                                             
    IF l_n > 0 THEN                                                                                                                 
          LET p_row = 2 LET p_col = 8
          OPEN WINDOW t630_2_w AT p_row,p_col WITH FORM "axm/42f/axmt630_2"
             ATTRIBUTE (STYLE = g_win_style CLIPPED)
 
          CALL cl_ui_locale("axmt630_2")
    ...
    INPUT ARRAY g_ogd WITHOUT DEFAULTS FROM s_ogd.*
       ...
       AFTER FIELD ogd03
        ...
        BEFORE FIELD ogd04                            #default 序號
          ...
          #CHI-CC0034 add --end--
            WHERE ogb01=g_oga.oga01 AND ogb03=g_ogd[l_ac].ogd03
            IF STATUS THEN LET g_ogd[l_ac].ogb04=''  LET l_ogb12=0 END IF
             ...
             IF g_ogd[l_ac].ogd17='2' THEN          #No.FUN-820046
                ...
                IF l_ogb31 IS NULL THEN
                   SELECT COUNT(*)INTO l_n FROM obj_file,obi_file 
                     ...
                     AND obj_file.obj05=g_ogd[l_ac].ogb04
        ...
        ON ACTION controlp
           CASE
              WHEN INFIELD(ogd08)
                   ...
                   IF g_ogd[l_ac].ogd17='2' THEN             #No.FUN-820046
                      IF NOT cl_null(g_ogd[l_ac].ogb04) THEN
                       LET g_qryparam.form = "q_skb08_a" 
                       LET g_qryparam.arg1=g_ogd[l_ac].ogb04
                      ELSE
...
DEFINE
    p_wc2           LIKE type_file.chr1000       #No.FUN-680137 VARCHAR(200)
        ...
        "SELECT ogd03,ogb04,ogb06,'',ogd04,ogd17,ogd08,ogd09,ogd10,ogd11,ogd12b,ogd12e,",       #No.FUN-820046   #FUN-9B0121 add ogb06,''
        "       ogd13,ogd14,ogd14t,ogd15,ogd15t,ogd16,ogd16t, ",
        "       ogdud01,ogdud02,ogdud03,ogdud04,ogdud05,",
        "       ogdud06,ogdud07,ogdud08,ogdud09,ogdud10,",
        "       ogdud11,ogdud12,ogdud13,ogdud14,ogdud15", 
        " FROM ogd_file LEFT OUTER JOIN ogb_file ON ogd_file.ogd01=ogb_file.ogb01 AND ogd_file.ogd03=ogb_file.ogb03",          #No.FUN-820046
  ...
  IF l_sma128!='Y' THEN
   FOREACH ogb_curs INTO l_ogb.*
      ...
      SELECT obl03 INTO l_ogd.ogd08 FROM obl_file
       WHERE obl01 = l_ogb.ogb04 AND obl02 = g_oga.oga04
 
      #-->包裝方式,單位淨重
     #SELECT ima134,ima18 INTO l_ogd08,l_ogd14  #CHI-CC0034 mark
      SELECT ima134,ima18,ima31 INTO l_ogd08,l_ima18,l_ima31  #CHI-CC0034
        FROM ima_file    WHERE ima01=l_ogb.ogb04
     #IF STATUS THEN LET l_ogd08='' LET l_ogd14=0 END IF #CHI-CC0034 mark
      IF STATUS THEN LET l_ogd08='' LET l_ima18=0 END IF #CHI-CC0034
...
DEFINE  l_ogb04       LIKE ogb_file.ogb04    #料號     #No.FUN-870124
DEFINE  l_ogb12       LIKE ogb_file.ogb12    #出貨數量 #No.FUN-870124
DEFINE  l_skb14       LIKE skb_file.skb14    #包裝數量 #No.FUN-870124
    ...
    OPEN WINDOW t630_1_w AT p_row,p_col WITH FORM "axm/42f/axmt630_1"
      ATTRIBUTE (STYLE = g_win_style CLIPPED)
 
    CALL cl_set_locale_frm_name("axmt630_1")
    CALL cl_ui_locale("axmt630_1")
 
    INPUT l_ska01 FROM ska01
     
       AFTER FIELD ska01
          IF NOT cl_null(l_ska01) THEN
             SELECT COUNT(*) INTO l_n FROM ska_file
              WHERE ska01 = l_ska01
                AND ska03 = g_oga.oga16
             IF l_n = 0 THEN
       ...
       ON ACTION controlp
          CASE 
            WHEN INFIELD (ska01)
              CALL cl_init_qry_var()
              LET g_qryparam.form="q_ska"
              LET g_qryparam.arg1=g_oga.oga16
              CALL cl_create_qry() RETURNING l_ska01
              DISPLAY l_ska01 TO FORMONLY.ska01
              NEXT FIELD ska01
          END CASE
    ...
    IF INT_FLAG THEN
       LET INT_FLAG = 0
       CLOSE WINDOW t630_1_w
       RETURN
    END IF
 
    LET l_sql = "SELECT skb04,skb07,skb08,skb09,skb10,skb11,skb12,skb13,",
                "       skb14,skb15,skb18,skb16,skb19,skb17,skb20",
                "  FROM skb_file,ska_file",
                " WHERE ska01 = skb01",
                "   AND skb01 = '",l_ska01,"'",
                "   AND ska03 = '",g_oga.oga16,"'"
    PREPARE t630_ypp FROM l_sql
    DECLARE t630_ypd CURSOR FOR t630_ypp
 
    LET l_ogd.ogd01 = g_oga.oga01
    FOREACH t630_ypd INTO l_ogd.ogd04,l_ogd.ogd17,l_ogd.ogd08,l_ogd.ogd09,
                          l_ogd.ogd10,l_ogd.ogd11,l_ogd.ogd12b,l_ogd.ogd12e,l_ogd.ogd13,
                          l_ogd.ogd14,l_ogd.ogd14t,l_ogd.ogd15,l_ogd.ogd15t,l_ogd.ogd16,l_ogd.ogd16t
       IF SQLCA.sqlcode THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,1)
          EXIT FOREACH
       END IF
        
       LET l_ogd.ogd03 = l_ogd.ogd04
       LET l_ogd.ogd04 = '' 
       #判斷一下這個料號包裝單里面的數量和出貨單中的數量是否相同，
       #包裝單里面的數量應該小于等于出貨單中的數量
       SELECT ogb04,ogb12 INTO l_ogb04,l_ogb12 FROM ogb_file
        WHERE ogb01 = g_oga.oga01  #出貨單號
          AND ogb03 = l_ogd.ogd03  #項次
       SELECT SUM(skb14) INTO l_skb14 FROM skb_file
        WHERE skb01 = l_ska01    #包裝單號
          AND skb05 = l_ogb04    #料號
       IF l_skb14 > l_ogb12 THEN
          CALL cl_err(l_ogb04,'axm_601',1)
          LET g_success = 'N'
       END IF
       SELECT MAX(ogd04)+1 INTO l_ogd.ogd04 FROM ogd_file
        WHERE ogd01 = l_ogd.ogd01
          AND ogd03 = l_ogd.ogd03 
       IF cl_null(l_ogd.ogd04 ) THEN
          LET l_ogd.ogd04  = 1
       END IF
       SELECT COUNT(*) INTO l_n FROM ogd_file
        WHERE ogd01 = l_ogd.ogd01
          AND ogd03 = l_ogd.ogd03
          AND ogd04 = l_ogd.ogd04
       IF l_n = 0 THEN
          LET l_ogd.ogdplant = g_plant 
          LET l_ogd.ogdlegal = g_legal 
          INSERT INTO ogd_file VALUES(l_ogd.*)
              IF SQLCA.sqlcode THEN
                 CALL cl_err3("ins","ogd_file",l_ogd.ogd01,l_ogd.ogd03,SQLCA.sqlcode,"","",1)
                 LET g_success = 'N'
                 EXIT FOREACH
              END IF
       END IF
