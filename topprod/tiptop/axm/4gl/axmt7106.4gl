...
#DEFINE       l_time    LIKE type_file.chr8            #No.FUN-6A0094
   ...
   END IF
    ...
    DECLARE t7106_cl CURSOR FROM g_forupd_sql
 
    LET p_row = 5 LET p_col = 17
 
    OPEN WINDOW t7106_w AT p_row,p_col              #顯示畫面
         WITH FORM "axm/42f/axmt7106"
...
DEFINE  lc_qbe_sn       LIKE    gbm_file.gbm01    #No.FUN-580031  HCN
   ...
   INITIALIZE g_ohf.* TO NULL    #No.FUN-750051
        ...
        ON ACTION controlp
           CASE
              WHEN INFIELD(ohf03)
#                CALL q_gen(05,11,g_ohf.ohf03) RETURNING g_ohf.ohf03
################################################################################
...
################################################################################
                 ...
                 NEXT FIELD ohf03
              WHEN INFIELD(ohf04)
#                CALL q_gen(05,11,g_ohf.ohf04) RETURNING g_ohf.ohf04
################################################################################
# START genero shell script ADD
                 CALL cl_init_qry_var()
                 LET g_qryparam.state = "c"
...
################################################################################
      ...
      ON ACTION controlg      #MOD-4C0121
        ...
        ON ACTION controlp
           CASE
              WHEN INFIELD(ohg03)
#                CALL q_ock(0,0,g_ohg[1].ohg03) RETURNING g_ohg[1].ohg03
################################################################################
# START genero shell script ADD
                 CALL cl_init_qry_var()
                 LET g_qryparam.state = "c"
                 LET g_qryparam.form ="q_ock"
                 LET g_qryparam.default1 = g_ohg[1].ohg03
                 CALL cl_create_qry() RETURNING g_ohg[1].ohg03
# END genero shell script ADD
################################################################################
                 NEXT FIELD ohg03
              OTHERWISE
    ...
    PREPARE t7106_prepare FROM g_sql
    DECLARE t7106_cs                         #SCROLL CURSOR
        SCROLL CURSOR WITH HOLD FOR t7106_prepare
    ...
    DROP TABLE x
    PREPARE t7106_precount_x FROM g_sql
    EXECUTE t7106_precount_x
 
        LET g_sql="SELECT COUNT(*) FROM x "
 
    PREPARE t7106_precount FROM g_sql
    DECLARE t7106_count CURSOR FOR t7106_precount
...
FUNCTION t7106_u()
    ...
    OPEN t7106_cl USING g_ohf.ohf01,g_ohf.ohf02
    IF STATUS THEN
       CALL cl_err("OPEN t7106_cl:", STATUS, 1) 
       CLOSE t7106_cl
       ROLLBACK WORK
       RETURN
    END IF
 
    FETCH t7106_cl INTO g_ohf.*            # 鎖住將被更改或取消的資料
...
FUNCTION t7106_ohf03(p_cmd)  #主辦
    DEFINE p_cmd     LIKE type_file.chr1,          #No.FUN-680137 VARCHAR(1)
           l_genacti LIKE gen_file.genacti,
           l_gen02_1 LIKE gen_file.gen02
 
    LET g_errno = " "
    SELECT gen02,genacti INTO l_gen02_1,l_genacti FROM gen_file
...
FUNCTION t7106_q()
    ...
    OPEN t7106_cs                            # 從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
       CALL cl_err('',SQLCA.sqlcode,0)
       INITIALIZE g_ohf.* TO NULL
    ELSE
       OPEN t7106_count
       FETCH t7106_count INTO g_row_count
       DISPLAY g_row_count TO FORMONLY.cnt
       CALL t7106_fetch('F')                  # 讀出TEMP第一筆並顯示
    END IF
 
END FUNCTION
 
#處理資料的讀取
FUNCTION t7106_fetch(p_flag)
DEFINE
    p_flag          LIKE type_file.chr1                  #處理方式        #No.FUN-680137 VARCHAR(1)
 
    CASE p_flag
        WHEN 'N' FETCH NEXT     t7106_cs INTO g_ohf.ohf01,g_ohf.ohf02
        WHEN 'P' FETCH PREVIOUS t7106_cs INTO g_ohf.ohf01,g_ohf.ohf02
        WHEN 'F' FETCH FIRST    t7106_cs INTO g_ohf.ohf01,g_ohf.ohf02
        WHEN 'L' FETCH LAST     t7106_cs INTO g_ohf.ohf01,g_ohf.ohf02
        WHEN '/'
...
#                       CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
            ...
            END IF
            LET mi_no_ask = FALSE
            FETCH ABSOLUTE g_jump t7106_cs INTO g_ohf.ohf01,g_ohf.ohf02
    END CASE
...
DEFINE l_ohf02_t LIKE type_file.chr20       # No.FUN-680137 VARCHAR(20)
    ...
    DISPLAY BY NAME                              # 顯示單頭值
        g_ohf.ohf01,g_ohf.ohf02,
        g_ohf.ohf03,g_ohf.ohf04
    CALL t7106_ohf03('d')
    CALL t7106_ohf04('d')
 
    CALL t7106_b_fill(g_wc2)                 #單身
...
DEFINE
    ...
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    DECLARE t7106_bcl CURSOR FROM g_forupd_sql      # LOCK CURSOR
    ...
    INPUT ARRAY g_ohg WITHOUT DEFAULTS FROM s_ohg.*
        ...
        BEFORE ROW
            ...
            IF g_rec_b >= l_ac THEN
               ...
               OPEN t7106_bcl USING g_ohf.ohf01, g_ohf.ohf02, g_ohg_t.ohg03
               IF STATUS THEN
                  CALL cl_err("OPEN t7106_bcl:", STATUS, 1)
                  LET l_lock_sw = "Y"
               ELSE
                  FETCH t7106_bcl INTO g_ohg[l_ac].*
                  IF SQLCA.sqlcode THEN
...
FUNCTION t7106_b_askkey()
DEFINE
    l_wc2           LIKE type_file.chr1000       #No.FUN-680137  VARCHAR(200)
 
    CONSTRUCT l_wc2 ON ohg03,ohg04
            FROM s_ohg[1].ohg03,s_ohg[1].ohg04
              #No.FUN-580031 --start--     HCN
              BEFORE CONSTRUCT
                 CALL cl_qbe_init()
              #No.FUN-580031 --end--       HCN
    ...
    IF INT_FLAG THEN
        ...
        RETURN
    END IF
    CALL t7106_b_fill(l_wc2)
END FUNCTION
 
FUNCTION t7106_b_fill(p_wc2)              #BODY FILL UP
DEFINE
