...
# Modify.........: No.TQC-630212 06/03/22 By pengu 3x列印時出現「ins x411_tmp查無此錯誤訊息-11023」
...
# Modify.........: No.MOD-890060 08/09/04 By wujie  x411_1()的時候，第一期漏加了occ22的條件
...
# Modify.........: No.FUN-CB0003 12/11/07 By chenjing CR轉XtraGrid
# Modify.........: No.FUN-D40095 13/04/23 By chenjing 修改欄位說明動態顯示問題
# Modify.........: No.FUN-D40128 13/05/08 BY wangrr 更改"國家""地區別"開窗
...
DEFINE tm  RECORD
        #  wc      VARCHAR(300),
          #wc2     VARCHAR(300),   #FUN-650095 mark
           ...
           a       LIKE type_file.chr1,        # No.FUN-680137 VARCHAR(01)        #'1'本幣 '2'原幣
...
#     DEFINE   l_time    LIKE type_file.chr8       #No.FUN-6A0094
   ...
   LET g_sql="occ20.occ_file.occ20,occ21.occ_file.occ21,geb02.geb_file.geb02,",
     ...
     #FUN-CB0003--add--str---
             "country.type_file.chr1000,",
             "geo02.geo_file.geo02,",
             "azi04.azi_file.azi04"
     #FUN-CB0003--add--end---
   LET l_table = cl_prt_temptable('axmx411',g_sql) CLIPPED
...
FUNCTION x411_tm(p_row,p_col)
   ...
   OPEN WINDOW x411_w AT p_row,p_col WITH FORM "axm/42f/axmx411"
   ...
   CONSTRUCT BY NAME tm.wc ON occ20,occ21,occ22,oea03,oea25,oeb04,oeb15,oea01    #FUN-550091   #FUN-650095 add oea01
        ...
        ON ACTION CONTROLP    #FUN-4B0043
           ...
           IF INFIELD(occ21) THEN
              CALL cl_init_qry_var()
             #LET g_qryparam.form = "q_geb"  #FUN-D40128 mark
              LET g_qryparam.form = "q_geb2" #FUN-D40128
              LET g_qryparam.state = "c"
              CALL cl_create_qry() RETURNING g_qryparam.multiret
...
#FUN-550091
           IF INFIELD(occ22) THEN
              CALL cl_init_qry_var()
             #LET g_qryparam.form = "q_geo"   #FUN-D40128 mark
              LET g_qryparam.form = "q_geo2"  #FUN-D40128
              LET g_qryparam.state = "c"
              CALL cl_create_qry() RETURNING g_qryparam.multiret
  ...
  #IF tm.wc2[1,5]='oea01' THEN LET wc3=tm.wc2[6,300] CLIPPED END IF
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmx411',g_time,l_cmd)
      END IF
      CLOSE WINDOW x411_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
...
DEFINE sr        RECORD
       ...
       sr2       RECORD
                 ...
                 END RECORD
     DEFINE l_country LIKE type_file.chr1000       #FUN-CB0003 
     DEFINE l_curr STRING                          #FUN-CB0003
     DEFINE l_geo02  LIKE geo_file.geo02           #FUN-CB0003
     DEFINE l_str   STRING                        #FUN-CB0003
     DEFINE l_str1  STRING                        #FUN-D40095
     DEFINE l_str2  STRING                        #FUN-D40095
     CALL cl_del_data(l_table)                          #No.FUN-7A0025
 
     SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
#No.FUN-580013  --begin
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'axmx411'
...
#FUN-CB0003--add--str--
     IF tm.a='1' THEN 
        LET l_curr = '(',cl_getmsg("lib-075",g_lang),tm.cdate,')'
     ELSE
        LET l_curr = '(',cl_getmsg("afa-332",g_lang),')'
     END IF
#FUN-CB0003--add--end--
     #取三期
...
#No.FUN-550070--end
     ...
     PREPARE x411_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
        EXIT PROGRAM
     END IF
     DECLARE x411_curs1 CURSOR FOR x411_prepare1
     FOREACH x411_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#       IF cl_null(sr.occ22) THEN LET sr.occ22='' END IF
       #MOD-810069-end-add
        ...
        IF cl_null(sr.occ22) THEN LET sr.occ22=' ' END IF
#No.MOD-890060 --end
       INSERT INTO x411_tmp VALUES(sr.*)
       IF SQLCA.SQLCODE THEN
#         CALL cl_err('ins x411_tmp',SQLCA.SQLCODE,1)   #No.FUN-660167
          CALL cl_err3("ins","x411_tmp","","",SQLCA.SQLCODE,"","ins x411_tmp",1)   #No.FUN-660167
          CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
          EXIT PROGRAM
       END IF
     END FOREACH
     ...
     #No.FUN-7A0025 --mark--
#     CALL cl_outnam('axmx411') RETURNING l_name
##No.FUN-580013  -begin
...
##No.FUN-580013  -end
#     START REPORT x411_rep1 TO l_name
#     LET g_pageno = 0
     #No.FUN-7A0025 --end--
     ...
     LET l_sql = "SELECT DISTINCT occ20,occ21,occ22,geb02,oea03,oea032",   #FUN-550091
                 #"  FROM x411_tmp ORDER BY occ21,oea03" CLIPPED
                 #"  FROM x411_tmp ORDER BY occ20,occ21,oea03" CLIPPED   #FUN-550091
                 "  FROM x411_tmp ORDER BY occ20,occ21,occ22,oea03" CLIPPED   #FUN-550091
     PREPARE x411_p1pre FROM l_sql
     DECLARE x411_cur1 CURSOR FOR x411_p1pre
     #Modify By Snow..020810
     #FOREACH x411_cur1 INTO sr1.occ21,sr1.geb02,sr1.oea03,sr1.oea032
     #FOREACH x411_cur1 INTO sr1.occ20,sr1.occ21,sr1.geb02,sr1.oea03,sr1.oea032   #FUN-550091
     FOREACH x411_cur1 INTO sr1.occ20,sr1.occ21,sr1.occ22,sr1.geb02,sr1.oea03,sr1.oea032   #FUN-550091
       #第一期
...
#       OUTPUT TO REPORT x411_rep1(sr1.*,sr2.*)      #No.FUN-7A0025
       #No.FUN-7A0025 --start--
    #FUN-CB0003--add--str--
       LET l_country = sr1.occ21,' ',sr1.geb02
       SELECT geo02 INTO l_geo02 FROM geo_file WHERE geo01 = sr1.occ22
    #FUN-CB0003--add--end--
       EXECUTE insert_prep USING sr1.occ20,sr1.occ21,sr1.geb02,sr1.occ22,sr1.oea03,
                                 ...
                                 sr2.ogb014,sr.oea23,l_country,l_geo02,g_azi04   #FUN-CB0003 add l_country,l_geo02,g_azi04
       #No.FUN-7A0025 --end--
     END FOREACH
 
#     FINISH REPORT x411_rep1                      #No.FUN-7A0025
#
#     CALL cl_prt(l_name,g_prtway,g_copies,g_len)  #No.FUN-7A0025
     ...
     IF g_zz05 = 'Y' THEN 
        ...
        LET g_str = tm.wc
     END IF
###XtraGrid###     LET g_str = g_str,";",tm.cdate,";",g_so,";",g_date,";",tm.a,";",
###XtraGrid###                 g_d1,";",g_d3,";",g_d5,";",g_azi04,";",g_aza.aza17   #MOD-920350 add aza17
###XtraGrid###     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
###XtraGrid###     CALL cl_prt_cs3('axmx411','axmx411',l_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
#FUN-CB0003--add--str---
    LET l_str = cl_getmsg('azz1296',g_lang),'(',g_d1,')'
    LET l_str1 = cl_getmsg('azz1296',g_lang),'(',g_d3,')'    #FUN-D40095
    LET l_str2 = cl_getmsg('azz1296',g_lang),'(',g_d5,')'    #FUN-D40095
    LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'oeb112',l_str,'')
    LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'oeb212',l_str1,'')    #FUN-D40095
    LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'oeb312',l_str2,'')    #FUN-D40095
    LET g_xgrid.grup_field = 'occ20,occ21,occ22,oea23'
    LET g_xgrid.order_field = 'occ20,occ21,occ22,oea23'
    LET g_xgrid.footerinfo1 = cl_getmsg("lib-073",g_lang),g_so
    LET g_xgrid.footerinfo2 = cl_getmsg("lib-074",g_lang),g_date
    LET g_xgrid.condition = cl_getmsg('lib-160',g_lang),tm.wc 
    LET g_xgrid.headerinfo1 = l_curr
#FUN-CB0003--add--end---
...
FUNCTION x411_cretmp()
  DROP TABLE x411_tmp
# No.FUN-680137 --start--
  CREATE TEMP TABLE x411_tmp(
...
#No.FUN-550070--end
     ...
     PREPARE x411_prepare12 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare2:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
        EXIT PROGRAM
     END IF
     DECLARE x411_curs12 CURSOR FOR x411_prepare12
     FOREACH x411_curs12 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#No.MOD-890060 --end
       INSERT INTO x411_tmp VALUES(sr.*)
       IF SQLCA.SQLCODE THEN
#         CALL cl_err('ins x411_tmp2',SQLCA.SQLCODE,1)   #No.FUN-660167
          CALL cl_err3("ins","x411_tmp","","",SQLCA.SQLCODE,"","ins x411_tmp2",1)   #No.FUN-660167
          CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
          EXIT PROGRAM
       END IF
     END FOREACH
...
#     LET g_zaa[48].zaa08 = g_zaa[48].zaa08,'(',g_x[20],')'
##No.FUN-580013  -end
#     START REPORT x411_rep2 TO l_name
#     LET g_pageno = 0
     #No.FUN-7A0025 --end--
     ...
     LET l_sql = "SELECT DISTINCT occ20,occ21,occ22,geb02,oea03,oea032,oea23",   #FUN-550091
                 #"  FROM x411_tmp ORDER BY occ21,oea03,oea23" CLIPPED
                 #"  FROM x411_tmp ORDER BY occ20,occ21,oea03,oea23" CLIPPED   #FUN-550091
                 "  FROM x411_tmp ORDER BY occ20,occ21,occ22,oea03,oea23" CLIPPED   #FUN-550091
     PREPARE x411_p1pre2 FROM l_sql
     DECLARE x411_cur12 CURSOR FOR x411_p1pre2
     #Modify By Snow..020810
     #FOREACH x411_cur12 INTO sr1.occ21,sr1.geb02,sr1.oea03,sr1.oea032,sr1.oea23
     #FOREACH x411_cur12 INTO sr1.occ20,sr1.occ21,sr1.geb02,sr1.oea03,sr1.oea032,sr1.oea23      #FUN-550091
     FOREACH x411_cur12 INTO sr1.occ20,sr1.occ21,sr1.occ22,sr1.geb02,sr1.oea03,sr1.oea032,sr1.oea23     #FUN-550091
...
#       OUTPUT TO REPORT x411_rep2(sr1.*,sr2.*)      #No.FUN-7A0025
       #No.FUN-7A0025 --start--                                                 
    #FUN-CB0003--add--str--
       LET l_country = sr1.occ21,' ',sr1.geb02
       SELECT geo02 INTO l_geo02 FROM geo_file WHERE geo01 = sr1.occ22
    #FUN-CB0003--add--end--
       EXECUTE insert_prep USING sr1.occ20,sr1.occ21,sr1.geb02,sr1.occ22,sr1.oea03,  
                                 ...
                                 sr2.ogb214,sr2.oeb312,sr2.oeb314,sr2.ogb312,       
                                 sr2.ogb314,sr2.oeb012,sr2.oeb014,sr2.ogb012,       
                                 sr2.ogb014,sr1.oea23,l_country,l_geo02,g_azi04    #FUN-CB0003 add l_country,l_geo02,g_azi04                             
       #No.FUN-7A0025 --end-- 
 
     END FOREACH
 
#     FINISH REPORT x411_rep2                     #No.FUN-7A0025
#
#     CALL cl_prt(l_name,g_prtway,g_copies,g_len) #No.FUN-7A0025
     #No.FUN-7A0025 --start--                                                   
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog                   
