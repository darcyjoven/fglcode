...
#
# Pattern name...: axmr830.4gl
# Descriptions...: 三角貿易訂單明細表(axmr830)
# Input parameter:
# Return code....:
...
# Modify.........: No.TQC-660087 06/06/20 By Pengu Saxmt810 多角貿易拋單時會產生axmr830,可是過程中無資料產生,lib-216
...
#No.TQC-A50044  --Begin
  ...
  #No.FUN-750013--begin--
   ...
   LET  l_table = cl_prt_temptable('axmr830',g_sql) CLIPPED
   IF l_table=-1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,?,",
               "        ?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,?,",
               "        ?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,?,",
               "        ?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?, ?,?,?,?)"       #MOD-830174 modify  #TQC-C10039 add 4?
...
#No.TQC-A50044  --End  
 ...
 # IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
   IF cl_null(tm.wc)
      THEN CALL axmr830_tm(0,0)        # Input print condition
      ELSE
           LET tm.more = 'N'
           LET g_pdate = g_today
           LET g_rlang = g_lang
          ...
          #LET tm.wc=" oea01='",tm.wc CLIPPED,"' "     #No.TQC-660087 mark
           CALL axmr830()            # Read data and create out-file
   END IF
 
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
END MAIN
 
FUNCTION axmr830_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,      #No.FUN-680137 SMALLINT
   ...
   OPEN WINDOW axmr830_w AT p_row,p_col WITH FORM "axm/42f/axmr830"
...
WHILE TRUE
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmr830_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmr830',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmr830_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
FUNCTION axmr830()
   DEFINE l_name     LIKE type_file.chr20,               # External(Disk) file name        #No.FUN-680137 VARCHAR(20)
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='axmr830'    #No.FUN-750013 add 
     ...
     PREPARE axmr830_prepare1 FROM l_sql
    ...
    #CALL cl_outnam('axmr830') RETURNING l_name  #FUN-750013 mark
    #START REPORT axmr830_rep TO l_name          #FUN-750013 mark
     DECLARE axmr830_curs1 CURSOR FOR axmr830_prepare1
     FOREACH axmr830_curs1 INTO sr.*,l_oea.*        
        IF SQLCA.sqlcode != 0 THEN
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
            ...
            PREPARE axmr830_prepare2 FROM l_sql
            IF STATUS THEN CALL cl_err('axmr830_prepare2',STATUS,1) END IF
            DECLARE axmr830_curs2 CURSOR FOR axmr830_prepare2
            FOREACH axmr830_curs2 INTO sr1.*
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_plant2[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr830_prepare3 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr830_prepare3',STATUS,1) END IF
               DECLARE axmr830_curs3 CURSOR FOR axmr830_prepare3
               OPEN axmr830_curs3
               FETCH axmr830_curs3 INTO sr1.oap041, sr1.oap042, sr1.oap043
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_plant2[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr830_prepare4 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr830_prepare4',STATUS,1) END IF
               DECLARE axmr830_curs4 CURSOR FOR axmr830_prepare4
               OPEN axmr830_curs4
               FETCH axmr830_curs4 INTO sr1.occ01, sr1.occ02, sr1.occ18, sr1.occ61
              #IF STATUS THEN CALL cl_err('fetch',STATUS,1) END IF
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_plant2[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr830_pre14 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr830_prepare4',STATUS,1) END IF
               DECLARE axmr830_curs14 CURSOR FOR axmr830_pre14
               OPEN axmr830_curs14
               FETCH axmr830_curs14 INTO sr1.occ02_2, sr1.occ18_2
              #IF STATUS THEN CALL cl_err('fetch',STATUS,1) END IF
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_plant2[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr830_prepare5 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr830_prepare5',STATUS,1) END IF
               DECLARE axmr830_curs5 CURSOR FOR axmr830_prepare5
               OPEN axmr830_curs5
               FETCH axmr830_curs5 INTO sr1.occg03, sr1.occg04
              # IF STATUS THEN CALL cl_err('fetch',STATUS,1) END IF
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_plant2[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr830_zo6 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr830_zo6',STATUS,1) END IF
               
               DECLARE axmr830_czo6 CURSOR FOR axmr830_zo6
               OPEN axmr830_czo6
              #FETCH axmr830_czo6 INTO l_company        #FUN-750013 mark
               FETCH axmr830_czo6 INTO sr1.company      #FUN-750013 mod
              #IF STATUS THEN CALL cl_err('fetch',STATUS,1) END IF
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_plant2[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr830_prepare7 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr830_prepare7',STATUS,1) END IF
               DECLARE axmr830_curs7 CURSOR FOR axmr830_prepare7
               OPEN axmr830_curs7
               FETCH axmr830_curs7 INTO sr1.oah02
              #IF STATUS THEN CALL cl_err('fetch',STATUS,1) END IF
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_plant2[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr830_prepare9 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr830_prepare9',STATUS,1) END IF
               DECLARE axmr830_curs9 CURSOR FOR axmr830_prepare9
               OPEN axmr830_curs9
               FETCH axmr830_curs9 INTO sr1.oao06
              #IF STATUS THEN CALL cl_err('fetch',STATUS,1) END IF
              ...
              #PREPARE axmr830_prepare11 FROM l_sql
              #IF STATUS THEN CALL cl_err('axmr830_prepare11',STATUS,1) END IF
              #DECLARE axmr830_curs11 CURSOR FOR axmr830_prepare11
              #OPEN axmr830_curs11
              #FETCH axmr830_curs11 INTO g_ocg.ocg48, g_ocg.ocg49,
...
#              LET sr1.amn = sr1.oeb12 * sr1.oeb13    #CHI-B70039 mark
              ...
              #OUTPUT TO REPORT axmr830_rep(l_company,sr1.*,  #FUN-750013 mark
              ...
              EXECUTE insert_prep USING 
                ...
                t_azi.azi05 ,  l_oea.oea23,                                  #MOD-830174  
    ...
    #No.FUN-750013--add---str---
     ...
     LET g_str = tm.wc,";",g_azi03,";",g_azi04,";",g_azi05,";",l_gaz03  
     LET g_cr_table = l_table                 #主報表的temp table名稱   #TQC-C100399
     LET g_cr_apr_key_f = "oea01"             #報表主鍵欄位名稱  #TQC-C10039 
     CALL cl_prt_cs3('axmr830','axmr830',g_sql,g_str)
    #No.FUN-750013--add---end---
 
    #FINISH REPORT axmr830_rep                   #FUN-750013 mark
    #CALL cl_prt(l_name,g_prtway,g_copies,g_len) #FUN-750013 mark
 
END FUNCTION
 
#FUN-750013---mark---str---
#REPORT axmr830_rep(sr1,l_oea,l_oed,l_occ02,l_occ18)
