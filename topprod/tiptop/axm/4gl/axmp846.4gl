...
#
# Pattern name...: axmp846.4gl    
# Descriptions...: 自動產生應收帳款副程式(多工廠)(逆拋,由axmp845串)
# Date & Author..: FUN-670007 06/12/27 BY Yiting 
...
# Modify.........: No.TQC-7C0132 07/12/10 By Rayven 無法拋轉，拋轉報錯，-391 ins npq:無法將null插入欄的'欄-名稱'
# Modify.........: No.TQC-7C0129 07/12/10 By claire omc08,omc09應給值
# Modify.........: No.TQC-7C0146 07/12/18 By (1)claire apa31,apa32,apa31f,apa32f應給值不可以null
#                                            (2)取不到p_pmm的值
...
# Modify.........: No.CHI-820013 08/02/27 By claire (1)關係人取值應先取設定檔(agli120)若空值時再判斷aag371的設定
#                                                   (2)增加分錄及異動碼產生功能(CHI-760003)
# Modify.........: No.MOD-860188 08/06/19 By claire oma50,oma50t未依原幣取位
# Modify.........: No.MOD-870216 08/07/17 By claire apa57應由apa31給值
# Modify.........: No.MOD-8A0002 08/10/01 By claire 中間站的應付單身沒有回寫採購單號(apb06)
...
# Modify.........: No.FUN-980059 09/09/08 By arman GP5.2架構重整，修改sub相關傳參
# Modify.........: No.FUN-980020 09/09/08 By douzh GP5.2架構重整，修改sub相關傳參
# Modify.........: No.FUN-980030 09/08/31 By Hiko 加上GP5.2的相關設定
# Modify.........: No.FUN-980092 09/09/18 By TSD.apple GP5.2 跨資料庫語法修改
# Modify.........: No.MOD-9A0097 09/10/16 By Dido 增加異動碼 5-10 
# Modify.........: No:CHI-9A0017 09/10/22 By Dido 尾差問題調整
# Modify.........: No:MOD-9A0163 09/10/28 By Dido 調整不申報時比對發票金額檢核問題 
# Modify.........: No:TQC-9A0154 09/10/28 By Dido 未稅金額計算判斷調整 
# Modify.........: No:MOD-9B0076 09/11/12 By Dido oma24回寫oga24需考慮ooz10的參數設定
...
# Modify.........: No:MOD-A50208 10/06/08 By Smapmin 修改出口報單號碼抓取
...
# Modify.........: No.TQC-AB0061 10/11/17 By wujie  omb15提前截位，与出货单金额保持一致 
...
# Modify.........: No:MOD-B30315 11/03/12 By chenying 中斷點拋轉AR/AP失敗
# Modify.........: No:MOD-B30560 11/03/18 By chenying p_plant變數重覆定義
...
# Modify.........: No:MOD-B90141 11/09/26 By Smapmin 不應再重算omb16/omb16t，應與應收算法一致直接用omb14/omb14t乘上oma24即可
...
DEFINE p_plant     LIKE azp_file.azp01    #AR廠別 VARCHAR(10)
...
DEFINE p_next_plant LIKE azp_file.azp01   #CHI-AC0043
...
FUNCTION axmp846(t_oga01,p_oma02,l_poy20,l_poy16,l_poy17,l_plant,
                 #next_dbs,l_poy12,l_last_plant,   #CHI-AC0043
                 next_plant,l_poy12,l_last_plant,   #CHI-AC0043
                 l_poy18,l_poy19,l_flow99,AR_t1,AP_t1,
                 l_rvu01,l_oga01,l_rva01,l_i,l_oga09,l_poy45,l_poy46)
   ...
   DEFINE l_poy45      LIKE poy_file.poy45
   DEFINE l_poy46      LIKE poy_file.poy46 
   DEFINE l_i          LIKE type_file.num5
   DEFINE l_oga09      LIKE oga_file.oga09
#  DEFINE p_plant    LIKE azp_file.azp01          #FUN-980092   #MOD-B30560 mark
   DEFINE l_plant      LIKE type_file.chr10       #FUN-980092 
   DEFINE next_plant   LIKE type_file.chr10      #CHI-AC0043
...
#   CALL s_gettrandbs()        ##取得TRANS DB ->存在 g_dbs_new Global變數中     #FUN-A50102
   ...
   LET p_next_plant = next_plant   #CHI-AC0043
   ...
   IF SQLCA.SQLCODE THEN 
      CALL cl_err('apz_p1',SQLCA.SQLCODE,1) 
   END IF
   ...
   IF SQLCA.SQLCODE THEN
      CALL cl_err('ooz_p1',SQLCA.SQLCODE,1)
   END IF
   ...
   IF SQLCA.SQLCODE THEN 
      CALL cl_err('oga_p1',SQLCA.SQLCODE,1)
   END IF
   ...
   IF SQLCA.SQLCODE THEN
      CALL cl_err('azi_p1',SQLCA.SQLCODE,1)
   END IF
   ...
   IF p_oga09 = '4' OR (p_oga09 = '6' AND p_i <> 0 ) THEN
   ...
   IF SQLCA.SQLCODE THEN
      CALL cl_err('ogb_p1',SQLCA.SQLCODE,1) 
   END IF
   ...
   FOREACH ogb_c1 INTO p_ogb.* 
     ...
     IF p_last_plant <> p_plant  THEN   #FUN-980092 
      #讀取採購單單頭檔資料(pmm_file)
      LET l_sql = "SELECT * ",
                #  " FROM ",p_dbs_tra CLIPPED,"pmm_file,", p_dbs_tra CLIPPED,"oea_file ",  #No.TQC-690065 add oea_file  #FUN-980092   #FUN-A50102
     ...
     CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql   #FUN-A50102
      ...
      #新增至應付帳款單身檔
      IF p_last_plant != p_plant THEN  #FUN-980092
         CALL s_auto_assign_no("aap",AP_t1,p_oma.oma02,'11',"","",p_plant,"","")
  ...
  IF p_oga09 = '4' OR (p_oga09 = '6' AND p_i <> 0 ) THEN
      ...
      #調整尾差(會將差異捲至單身金額最大筆)
      CALL p846_up_omb(p_oma.oma01)  #CHI-9A0017

      #新增應收多帳期資料
      CALL p846_ins_omc()      #No: TQC-7C0019 
      IF p_ooy.ooydmy1 = 'Y' THEN    #No.CHI-920010
      #新增至分錄底稿單頭檔
        CALL p846_ar_npp('0')          #No.FUN-6A0026 add '0'
  ...
  IF p_last_plant != p_plant THEN  #FUN-980092
     ...
     #調整尾差(會將差異捲至單身金額最大筆)
     CALL p846_up_apb(p_apa.apa01)  #CHI-9A0017

     #新增應付多帳期資料
     CALL p846_ins_apc()      #No: TQC-7C0019 
...
FUNCTION p846_oma(p_oma01)
  ...
  DEFINE l_dbs     LIKE type_file.chr21   #FUN-670007 #No.FUN-680137 CHAR(21)     
  ...
  DEFINE l_oea24   LIKE oea_file.oea24    #CHI-9B0023
   ...
   LET p_oma.oma66 =  p_plant      #FUN-980092 直接給不用再重取plant
...
#   LET l_sql = "SELECT gec08,gec06 FROM ",p_dbs CLIPPED,"gec_file",  #FUN-A50102
   ...
   IF SQLCA.SQLCODE THEN
      CALL cl_err('ool_p1',SQLCA.SQLCODE,1)
   END IF
   ...
   LET l_sql = "SELECT UNIQUE oea18,oea24 ",
              ...
              # "      ",p_dbs CLIPPED,"ogb_file ",   #FUN-A50102
   ...
   #LET p_oma.oma39 = p_oga.oga27   #INVOICE NO#  99.01.27 add   #MOD-A50208
   LET p_oma.oma39 = p_oga.oga39   #INVOICE NO#  99.01.27 add   #MOD-A50208
...
FUNCTION p846_omb()
  ...
  CLOSE rvv_c1
     ...
     CALL cl_digcut(p_omb.omb15,s_azi.azi03) RETURNING p_omb.omb15  #No.TQC-AB0061
     LET p_omb.omb16 =p_omb.omb14 *p_oma.oma24  #本幣未稅金額   #MOD-B90141
     LET p_omb.omb16t=p_omb.omb14t*p_oma.oma24  #本幣含稅金額   #MOD-B90141
     ...
     #若為負數則給0
     IF p_omb.omb17 <= 0 THEN	#CHI-9A0017
        LET p_omb.omb17=0
        LET p_omb.omb18=0
        LET p_omb.omb18t=0
     END IF
    ...
    END IF
     ...
     LET p_omb.omb930 = p_ogb.ogb930  #出貨單身成本中心  #MOD-920266 add
 
     #單頭之發票金額
     LET p_oma.oma50 = p_oma.oma50  + p_omb.omb14   #原幣應收未稅金額
     LET p_oma.oma50t= p_oma.oma50t + p_omb.omb14t  #原幣應收未稅金額
     CALL cl_digcut(p_oma.oma50 ,p_azi.azi04) RETURNING p_oma.oma50    #MOD-860188 modify s->p
     CALL cl_digcut(p_oma.oma50t,p_azi.azi04) RETURNING p_oma.oma50t   #MOD-860188 modify s->p
     #新增至單身檔
     CALL p846_ins_omb()
END FUNCTION
 
FUNCTION p846_ins_omb()
    #新增應收帳款單身檔
 #   LET l_sql1="INSERT INTO ",p_dbs CLIPPED,"omb_file",   #FUN-A50102
    LET l_sql1="INSERT INTO ",cl_get_target_table( p_plant, 'omb_file' ),   #FUN-A50102
               ...
               " omb35,omb36,",
               " omb930,omb331,omblegal) ",                #FUN-960141 add 090824 #FUN-980010  
               " VALUES( ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
...
FUNCTION p846_apb()
  ...
  CLOSE pmn_c2
     ...
     LET p_apb.apb06 = p_rvv.rvv36   #採購單號
     LET p_apb.apb07 = p_rvv.rvv37   #採購項次
 
     #讀取應付帳款單身檔資料(apb_file)
     LET l_sql = "SELECT * ",
                 #" FROM ",p_dbs_tra CLIPPED,"pmn_file ",   #FUN-980092   #FUN-A50102
                 " FROM ",cl_get_target_table( p_plant, 'pmn_file' ),   #FUN-A50102
                 " WHERE pmn01 = '",p_rvv.rvv36,"' ",     
                 "  AND pmn02 =",p_rvv.rvv37
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
         CALL cl_parse_qry_sql(l_sql,p_plant) RETURNING l_sql #FUN-980092
     PREPARE pmn_p21 FROM l_sql 
     IF SQLCA.SQLCODE THEN CALL cl_err('pmn_p21',SQLCA.SQLCODE,1) END IF
     DECLARE pmn_c21 CURSOR FOR pmn_p21
     OPEN pmn_c21
     FETCH pmn_c21 INTO p_pmn.* 
     CLOSE pmn_c21
...
FUNCTION p846_ins_apb()
   ...
   # LET l_sql1="INSERT INTO ",p_dbs CLIPPED,"apb_file",   #FUN-A50102
    LET l_sql1="INSERT INTO ", cl_get_target_table( p_plant, 'apb_file' ),   #FUN-A50102
               ...
               " apb29,",
               " apb930,apb251,apb34,apblegal) ",            #FUN-960141 add 090824   #FUN-980010
               " VALUES( ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
               "         ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,?)"          #FUN-960141 add 090824  #FUN-980010
 	 CALL cl_replace_sqldb(l_sql1) RETURNING l_sql1        #FUN-920032
     CALL cl_parse_qry_sql( l_sql1, p_plant ) RETURNING l_sql1   #FUN-A50102
...
FUNCTION p846_ins_oma()
DEFINE l_oga53 LIKE oga_file.oga53
DEFINE l_oga24 LIKE oga_file.oga24    #MOD-9B0076
  ...
  END IF
  LET p_oma.oma66 =  p_oga.ogaplant        #FUN-960141 add 090824
#No.FUN-AB0034 --begin
...
#No.FUN-AB0034 --end
   ...
   # LET l_sql1="INSERT INTO ",p_dbs CLIPPED,"oma_file",   #FUN-A50102
    LET l_sql1="INSERT INTO ",cl_get_target_table( p_plant, 'oma_file' ),   #FUN-A50102
               ...
               " omagrup,omamodu,omadate,oma181,oma64,oma66,omalegal,omaoriu,omaorig,oma73,oma73f,oma74)",  #FUN-A10036  #omaplant)",  #MOD-940111 #FUN-960141 add omaplant  #FUN-960141 090824 omaplant-->oma66  #FUN-980010  FUN-AB0034 add oma73,oma73f,oma74
               " VALUES( ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?) "                 #FUN-Z10036 #No.FUN-670026 #MOD-940111 #MOD-950030 
                                                                #FUN-960141 add ? #FUN-980010  FUN-AB0034 add ???
                                                                #MOD-C50058 add ?
 	 CALL cl_replace_sqldb(l_sql1) RETURNING l_sql1        #FUN-920032
     CALL cl_parse_qry_sql( l_sql1, p_plant ) RETURNING l_sql1   #FUN-A50102
           ...
           EXECUTE ins_oma USING 
                ...
                p_oma.oma61,p_oma.oma65,p_oma.oma67,               #MOD-C50058 add oma65  #MOD-950030 add oma67
                ...
                p_oma.omauser,p_oma.omagrup,p_oma.omamodu,p_oma.omadate,p_oma.oma181,p_oma.oma64 
                ,p_oma.oma66,      #FUN-960141 add 090824
                p_legal,g_user,g_grup,p_oma.oma73,p_oma.oma73f,p_oma.oma74    #FUN-A10039    #FUN-980010   FUN-AB0034 add oma73,oma73f,oma74
              IF SQLCA.sqlcode<>0 THEN
               CALL cl_err('ins oma:',SQLCA.sqlcode,1)
               LET g_success = 'N'
              END IF
 
   LET l_oga24 = p_oga.oga24
   IF p_oga.oga23 <> p_aza.aza17  THEN  #同為本國幣別不UPDATE
      IF p_ooz.ooz10='Y' AND (p_oga.oga23 = p_oma.oma23)  THEN 
         LET l_oga24 = p_oma.oma24
      END IF 
   END IF 
   #更新出貨單頭檔
 #  LET l_sql1="UPDATE ",p_dbs_tra CLIPPED,"oga_file",  #FUN-980092  #FUN-A50102
   ...
   EXECUTE upd_oga USING p_oma.oma01,l_oga24,p_oga01  #MOD-9B0076 
   IF SQLCA.SQLCODE OR SQLCA.SQLERRD[3]=0 THEN
...
FUNCTION p846_ar_npp(p_npptype)
DEFINE  p_npptype   LIKE npp_file.npptype    #No.FUN-6A0026
    ...
    CALL s_get_bookno1(YEAR(p_oma.oma02),p_plant) RETURNING g_flag,g_bookno1,g_bookno2   #FUN-980092                                                
...
FUNCTION p846_def_npq()
        ...
        LET p_npq.npq02 = 0
       #CALL p846_azp()     	         #MOD-B30315 mark
        LET p_npq.npq30=g_plant          #MOD-B30315 add
END FUNCTION
 
#----新增 A/R 分錄底稿單身檔(npq_file)-------------
FUNCTION p846_ar_npq(p_npqtype)
...
#MOD-B30012 add --end--
     ...
     CALL p846_def_npq()   #給分錄底稿單身檔預設值(每一單身共同之部份欄位)
     #讀取客戶簡稱
     LET l_occ02=''
  ...
  IF p_npqtype=0 THEN
     ...
     CALL s_def_npq_m(p_npq.npq03,'axrt300',p_npq.*,p_npq.npq01,'','',g_bookno1,p_plant)   #No.FUN-980059
     RETURNING p_npq.*  
   ...
   ## 判斷應收客戶是否為關係人
#   CALL p846_occ37(p_npq.npq03,g_bookno1,p_oma.oma03,p_npq.npq37,p_dbs)       #FUN-A50102
   CALL p846_occ37(p_npq.npq03,g_bookno1,p_oma.oma03,p_npq.npq37,p_plant)      #FUN-A50102
   RETURNING p_npq.npq37
     #新增一筆分錄底稿單身檔
     CALL p846_ins_npq()                         #CHI-8C0034 mod
     ...
     #MOD-B30012 add --end--
       ...
       #LET p_npq.npq03 = p_ool.ool41    #科目 #MOD-B30012 mark
        ...
        LET p_npq.npq04 = NULL           #CHI-820013 
        #讀會計科目
        OPEN aag_c3 USING p_npq.npq03,g_bookno1         #No.FUN-730057
        FETCH aag_c3 INTO l_aag.* 
        IF SQLCA.SQLCODE THEN 
...
#       CALL p846_occ37(p_npq.npq03,g_bookno1,p_oma.oma03,p_npq.npq37,p_dbs)    #FUN-A50102
  ...
  IF p_npqtype=1 THEN
     ...
     LET p_npq.npq07f=p_oma.oma54t    #原幣金額
     LET p_npq.npq07 =p_oma.oma56t    #本幣金額
      CALL s_def_npq_m(p_npq.npq03,'axrt300',p_npq.*,p_npq.npq01,'','',g_bookno2,p_plant)  #No.FUN-980059
      RETURNING p_npq.*  
     CALL s_def_npq31_npq34_m(p_npq.*,g_bookno2,p_plant)         #FUN-AA0087
     RETURNING p_npq.npq31,p_npq.npq32,p_npq.npq33,p_npq.npq34 #FUN-AA0087
    ## 判斷應收客戶是否為關係人
#       CALL p846_occ37(p_npq.npq03,g_bookno2,p_oma.oma03,p_npq.npq37,p_dbs)    #FUN-A50102
       CALL p846_occ37(p_npq.npq03,g_bookno2,p_oma.oma03,p_npq.npq37,p_plant)   #FUN-A50102
       RETURNING p_npq.npq37
     #新增一筆分錄底稿單身檔
     CALL p846_ins_npq()                        #CHI-8C0034 mod
     LET p_npq.npq37 = NULL           #No.MOD-660043 add
...
#新增分錄底稿單身檔
FUNCTION p846_ins_npq()                      #CHI-8C0034 mod
 ...
 #   LET l_sql1="INSERT INTO ",p_dbs CLIPPED,"npq_file",   #FUN-A50102
    LET l_sql1="INSERT INTO ",cl_get_target_table( p_plant, 'npq_file' ),   #FUN-A50102
               ...
               " npq36,npq37,npqtype,npqlegal) ",       #No.FUN-6A0026	#FUN-980010	#MOD-9A0097 add npq36
               " VALUES( ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
               "         ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
     ...
     CALL cl_parse_qry_sql( l_sql1, p_plant ) RETURNING l_sql1   #FUN-A50102
           ...
           EXECUTE ins_npq USING 
                ...
                p_npq.npq36,p_npq.npq37,p_npq.npqtype,p_legal           #No.FUN-6A0026	#FUN-980010	#MOD-9A0097 
              IF SQLCA.sqlcode<>0 THEN
...
FUNCTION p846_ap_apa()
  ...
  #讀取 
       ...
       # LET l_sql = " SELECT oga27 FROM ",p_dbs_tra," oga_file ", #FUN-980092    #FUN-A50102
        ...
        #            "  WHERE oga01 = '",p_oga01,"'"
        LET l_sql = " SELECT oga27 FROM ",cl_get_target_table( p_next_plant, 'oga_file' ),   #FUN-A50102
	...
	CALL cl_replace_sqldb(l_sql) RETURNING l_sql		#FUN-920032
       ...
       #MOD-BC0086 mark --end--
        ...
        IF cl_null(p_apa.apa31f) THEN LET p_apa.apa31f=0 END IF
        IF cl_null(p_apa.apa32f) THEN LET p_apa.apa32f=0 END IF
        IF cl_null(p_apa.apa31)  THEN LET p_apa.apa31=0  END IF
        IF cl_null(p_apa.apa32)  THEN LET p_apa.apa32=0  END IF
...
#    LET l_sql1="INSERT INTO ",p_dbs CLIPPED,"apa_file",     #FUN-A50102
    LET l_sql1="INSERT INTO ",cl_get_target_table( p_plant, 'apa_file' ),   #FUN-A50102
               ...
               " apamodu,apadate,apa511,apa521,apa531,apa541,apa100,apalegal,apaoriu,apaorig) ", #FUN-Z10036  #apaplant) ",  #No.FUN-6A0026 #FUN-960141 add apaplant  #FUN-960141 090824 apaplant-->apa100 #FUN-980010
               " VALUES( ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,?,?) "  #FUN-A10036 #FUN-960141 add ? #FUN-980010  #No.FUN-A60024 add ? #CHI-C60004 add apa77
 	 CALL cl_replace_sqldb(l_sql1) RETURNING l_sql1        #FUN-920032
     CALL cl_parse_qry_sql( l_sql1, p_plant ) RETURNING l_sql1   #FUN-A50102
           PREPARE inp846_apa FROM l_sql1
           EXECUTE inp846_apa USING 
                ...
                p_apa.apa75,p_apa.apa77,p_apa.apa79,p_apa.apa99,p_apa.apainpd,p_apa.apamksg, #No.8166    #No.FUN-A60024 add apa79 #CHI-C60004 add apa77
                p_apa.apasign,
...
FUNCTION p846_ap_npp(p_npptype)               #No.FUN-6A0026   
DEFINE  p_npptype   LIKE npp_file.npptype     #No.FUN-6A0026
     ...
     LET p_npp.nppglno =null         #已拋轉傳票編號
     LET p_npp.npptype = p_npptype   #No.FUN-6A0026
     CALL s_get_bookno1(YEAR(p_apa.apa02),p_plant) RETURNING g_flag,g_bookno1,g_bookno2   #FUN-980092                                              
     IF g_flag =  '1' THEN  #抓不到帳別                                                                                             
...
FUNCTION p846_ap_npq(p_npqtype)            #No.FUN-6A0026
  DEFINE p_npqtype  LIKE npq_file.npqtype   #No.FUN-6A0026
  ...
  DEFINE l_pmc03       LIKE pmc_file.pmc03    #No.CHI-820013 add
     ...
     CALL p846_apdef_npq()   #給分錄底稿單身檔預設值(每一單身共同之部份欄位)
     #讀取會計科目
  IF p_npqtype=0 THEN
    LET p_npq.npqtype= p_npqtype 
     LET l_sql = "SELECT * ",
     ...
     RETURNING p_npq.npq31,p_npq.npq32,p_npq.npq33,p_npq.npq34 #FUN-AA0087
     ## 判斷應收廠商是否為關係人
#     CALL p846_pmc903(p_npq.npq03,g_bookno1,p_apa.apa06,p_npq.npq37,p_dbs)     #FUN-A50102
     CALL p846_pmc903(p_npq.npq03,g_bookno1,p_apa.apa06,p_npq.npq37,p_plant)    #FUN-A50102
     RETURNING p_npq.npq37
        #新增一筆分錄底稿單身檔
        CALL p846_ins_npq()                       #CHI-8C0034 mod
     ELSE #NO.MOD-610100 add
        CALL p846_stock(p_apa.apa01,p_apa.apa66,p_npqtype)          #No.TQC-7C0132
  ...
  IF p_npqtype=1 THEN
     ...
     IF p_apa.apa51 <> 'STOCK' THEN #NO.MOD-610100 add
        ...
        ELSE
           LET p_npq.npq05 = ''           
        END IF
     CALL s_def_npq_m(p_npq.npq03,'aapt110',p_npq.*,p_npq.npq01,'','',g_bookno2,p_plant)   #No.FUN-980059
     RETURNING p_npq.*  
     CALL s_def_npq31_npq34_m(p_npq.*,g_bookno2,p_plant)         #FUN-AA0087
     RETURNING p_npq.npq31,p_npq.npq32,p_npq.npq33,p_npq.npq34 #FUN-AA0087
     ## 判斷應收廠商是否為關係人
#     CALL p846_pmc903(p_npq.npq03,g_bookno2,p_apa.apa06,p_npq.npq37,p_dbs)     #FUN-A50102
     CALL p846_pmc903(p_npq.npq03,g_bookno2,p_apa.apa06,p_npq.npq37,p_plant)    #FUN-A50102  
     RETURNING p_npq.npq37
        #新增一筆分錄底稿單身檔
        CALL p846_ins_npq()                        #CHI-8C0034 mod
     ELSE #NO.MOD-610100 add
...
FUNCTION p846_stock(p_apa01,p_apa66,p_npqtype)  #No.TQC-7C0132
  ...
  DEFINE p_npqtype     LIKE npq_file.npqtype #No.TQC-7C0132
  ...
  LET l_npq.npqtype = p_npqtype  #No.TQC-7C0132
  LET l_npq.npq02 = 0
  LET l_npq.npq11 = ''
  LET l_npq.npq15 = ''
  LET l_sql1 =" SELECT apb10,apb29,'',apb26,apb12,apb09,apb08,apb24,rvv32,rvv33",
 	 ...
 	 CALL cl_replace_sqldb(l_sql1) RETURNING l_sql1        #FUN-920032
         CALL cl_parse_qry_sql(l_sql,p_plant) RETURNING l_sql #FUN-980092
  PREPARE p846_gl_p4 FROM l_sql1
  DECLARE p846_gl_c4 CURSOR FOR p846_gl_p4
  FOREACH p846_gl_c4 INTO l_amt,l_apb29,l_actno,l_deptno,
                          l_apb12,l_apb09,l_apb08,l_amt_f,l_ware,l_loc
     IF STATUS THEN EXIT FOREACH END IF
     IF cl_null(l_actno) THEN
        LET l_actno = p846_stock_act(l_apb12,l_ware,l_loc) #no.6903
     END IF
     LET l_npq.npqsys = 'AP'
     LET l_npq.npq00 = '1'
     ...
     ELSE
        LET l_npq.npq05 = ''
     END IF
     CALL s_def_npq_m(l_npq.npq03,'aapt110',l_npq.*,l_npq.npq01,'','',g_bookno,p_plant)  #No.FUN-980059
     RETURNING l_npq.*  
     CALL s_def_npq31_npq34_m(l_npq.*,g_bookno,p_plant)         #FUN-AA0087
     RETURNING p_npq.npq31,p_npq.npq32,p_npq.npq33,p_npq.npq34 #FUN-AA0087
     ## 判斷應收廠商是否為關係人
#     CALL p846_pmc903(l_npq.npq03,g_bookno,p_apa.apa06,l_npq.npq37,p_dbs)      #FUN-A50102
     CALL p846_pmc903(l_npq.npq03,g_bookno,p_apa.apa06,l_npq.npq37,p_plant)     #FUN-A50102 
     ...
     IF l_npq.npq07!=0 THEN
      #  LET l_sql1="INSERT INTO ",p_dbs CLIPPED,"npq_file",   #FUN-A50102
        LET l_sql1="INSERT INTO ",cl_get_target_table( p_plant, 'npq_file' ),   #FUN-A50102
               ...
               " npq15,npq21,npq22,npq23,",
               " npq24,npq25,npq26,npq27,",
               " npq28,npq29,npq30,npq37,npqtype,npqlegal) ",    #No.FUN-6A0026 #FUN-980010
               " VALUES( ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
               "         ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,? ) "        #No.FUN-6A0026 #FUN-980010
 	 CALL cl_replace_sqldb(l_sql1) RETURNING l_sql1        #FUN-920032
     CALL cl_parse_qry_sql( l_sql1, p_plant ) RETURNING l_sql1   #FUN-A50102
           ...
           EXECUTE ins_npq2 USING
                ...
                l_npq.npq28,l_npq.npq29,l_npq.npq30,l_npq.npq37,
                l_npq.npqtype,  #No.FUN-6A0026
                p_legal  #FUN-980010
              IF SQLCA.sqlcode<>0 THEN
               CALL cl_err('ins npq:',SQLCA.sqlcode,1)
               LET g_success = 'N'
        ...
        MESSAGE '>',l_npq.npq02,' ',l_npq.npq03,' ',l_npq.npq06,' ',
                 l_npq.npq07
        IF STATUS THEN
           CALL cl_err('p846_g_gl(ckp#1)',STATUS,1)
  ...
  CALL s_flows('3','',p_npq.npq01,p_npp.npp02,'N',p_npq.npqtype,TRUE)   #No.TQC-B70021   
END FUNCTION
FUNCTION p846_stock_act(p_item,p_ware,p_loc)
...
#  LET l_sql="INSERT INTO ",p_dbs CLIPPED,"omc_file",     #FUN-A50102
  LET l_sql="INSERT INTO ",cl_get_target_table( p_plant, 'omc_file' ),   #FUN-A50102
            ...
            " omc15,omclegal)",  #FUN-960141 090824 add  #FUN-980010
            " VALUES( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) "  #FUN-960141 090824 #FUN-980010
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
     CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql   #FUN-A50102
     ...
     CALL cl_parse_qry_sql( ll_sql, p_plant ) RETURNING ll_sql   #FUN-A50102
            ...
            FOREACH p110_c1 INTO l_pmb.pmb01,l_pmb.pmb03,
               ...
               EXECUTE ins_apc USING l_apc.apc01,l_apc.apc02,l_apc.apc03,
                                     ...
                                     l_apc.apc16,  #,l_apc.apcplant #FUN-960141 add apcplant   #FUN-960141 del apcplant
                                     p_legal    #FUN-980010
               IF STATUS THEN
                  LET g_success = 'N'
...
#FUNCTION p846_occ37(l_npq03,l_bookno,l_oma03,l_npq37,l_dbs)   #FUN-A50102
FUNCTION p846_occ37(l_npq03,l_bookno,l_oma03,l_npq37,p_plant)  #FUN-A50102
   DEFINE l_npq03  LIKE npq_file.npq03,   #科目 
         ...
         # l_dbs    LIKE type_file.chr21,  #資料庫名稱     #FUN-A50102  
          p_plant  LIKE azp_file.azp01,   #營運中心        #FUN-A50102 
...
## 判斷應收廠商是否為關係人
#FUNCTION p846_pmc903(l_npq03,l_bookno,l_apa06,l_npq37,l_dbs)   #FUN-A50102
FUNCTION p846_pmc903(l_npq03,l_bookno,l_apa06,l_npq37,p_plant)  #FUN-A50102
   DEFINE l_npq03  LIKE npq_file.npq03,   #科目 
          l_bookno LIKE aza_file.aza81,   #帳別
...
FUNCTION p846_up_omb(p_oma01) 
 ...
 # LET l_sql="SELECT * FROM ",p_dbs," oma_file ",   #FUN-A50102
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql   #FUN-A50102
  PREPARE p846_prep91 FROM l_sql
  DECLARE p846_curs91 CURSOR FOR p846_prep91
  OPEN p846_curs91
  FETCH p846_curs91 INTO m_oma.*
  IF STATUS THEN 
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql	
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql   #FUN-A50102 
  PREPARE p846_prep93 FROM l_sql
  DECLARE p846_curs93 CURSOR FOR p846_prep93
  OPEN p846_curs93
  FETCH p846_curs93 INTO l_omb14,l_omb16,l_omb18,l_omb18t 				#TQC-9A0154 add omb18t
  ...
  IF m_oma.omaconf = 'Y' THEN
     ...
     CALL cl_replace_sqldb(l_sql) RETURNING l_sql
     CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql   #FUN-A50102  
     PREPARE p846_prep94 FROM l_sql
     DECLARE p846_curs94 CURSOR FOR p846_prep94
     OPEN p846_curs94
     FETCH p846_curs94 INTO l_cnt,l_oot05,l_oot05t
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql	
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql   #FUN-A50102
  PREPARE p846_prep95 FROM l_sql
  DECLARE p846_curs95 CURSOR FOR p846_prep95
  OPEN p846_curs95
  FETCH p846_curs95 INTO l_omb14,l_omb14t,l_omb16,l_omb16t,l_omb18,l_omb18t
...
#  LET l_sql="SELECT * FROM ",p_dbs," omb_file ",    #FUN-A50102
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql	
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql   #FUN-A50102
  PREPARE p846_prep96 FROM l_sql
  DECLARE p846_curs96 CURSOR FOR p846_prep96
  FOREACH p846_curs96 INTO l_omb.* 
      IF SQLCA.SQLCODE <> 0 THEN EXIT FOREACH END IF
      LET l_flag = 'N'    
...
#  LET l_sql="SELECT * FROM ",p_dbs," apa_file ",   #FUN-A50102
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql   #FUN-A50102
  PREPARE p846_prep82 FROM l_sql
  DECLARE p846_curs82 CURSOR FOR p846_prep82
  OPEN p846_curs82
  FETCH p846_curs82 INTO m_apa.*
  IF STATUS THEN 
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql	
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql   #FUN-A50102
  PREPARE p846_prep83 FROM l_sql
  DECLARE p846_curs83 CURSOR FOR p846_prep83
  OPEN p846_curs83
  FETCH p846_curs83 INTO l_apb24,l_apb10
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql   #FUN-A50102
  PREPARE p846_prep84 FROM l_sql
  DECLARE p846_curs84 CURSOR FOR p846_prep84
  OPEN p846_curs84
  FETCH p846_curs84 INTO s_apa31f,s_apa31
  ...
  IF diff_apb24 <> 0  OR diff_apb10 <> 0 THEN 
     ...
     CALL cl_replace_sqldb(l_sql) RETURNING l_sql	
     CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql   #FUN-A50102
     PREPARE p846_prep86 FROM l_sql
     DECLARE p846_curs86 CURSOR FOR p846_prep86
     FOREACH p846_curs86 INTO m_apb.* 
       IF SQLCA.SQLCODE <> 0 THEN EXIT FOREACH END IF

       #更新差異至單身最大筆
      #  LET l_sql = " UPDATE ",p_dbs," apb_file SET apb24 = apb24+ ? ",  #FUN-50102
