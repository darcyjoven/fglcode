               ...
               #MOD-C70061 add end   -----
                  ...
                  IF l_oea11 = '3' THEN #是否由合約轉入
                     ...
                     IF cl_null(l_oeb71) THEN
                        IF cl_null(l_oea12) THEN
                           LET l_cnt = 0
                           LET g_sql="SELECT DISTINCT oebud02 FROM oeb_file WHERE oeb01='",g_oep.oep01 CLIPPED,"'"
                           PREPARE t800_oeb12_sel FROM g_sql
                           DECLARE t800_oeb12_cs SCROLL CURSOR WITH HOLD FOR t800_oeb12_sel
...
END FUNCTION
#MOD-D10109 add end   -----
#add by donghy 17/03/15
FUNCTION t800_ogb_chk(p_oeb01,p_oeb03)
...
FUNCTION t800_oeb23(p_oeb01,p_oeb03)
  DEFINE p_oeb01    LIKE oeb_file.oeb01
  DEFINE p_oeb03    LIKE oeb_file.oeb03
  DEFINE l_tz       LIKE oeb_file.oeb12
  DEFINE l_ch       LIKE oeb_file.oeb12
  DEFINE l_sl       LIKE oeb_file.oeb12

  IF cl_null(p_oeb01) THEN RETURN 0 END IF
  IF cl_null(p_oeb03) THEN RETURN 0 END IF

  #取所有已确认出货通知单数量
  SELECT SUM(ogb16) INTO l_tz FROM ogb_file,oga_file
    WHERE oga01=ogb01 AND ogb31=p_oeb01 AND ogb32=p_oeb03
    AND oga09='1' AND ogaconf='Y'

  SELECT SUM(ogb16) INTO l_ch FROM ogb_file,oga_file
    WHERE oga01=ogb01 AND ogb31=p_oeb01 AND ogb32=p_oeb03
