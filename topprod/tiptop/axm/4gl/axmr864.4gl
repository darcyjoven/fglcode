...
MAIN
   ...
   LET l_table = cl_prt_temptable('axmr864',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL axmr864_tm(0,0)        # Input print condition
      ELSE CALL axmr864()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN

FUNCTION axmr864_tm(p_row,p_col)
   ...
   OPEN WINDOW axmr864_w AT p_row,p_col WITH FORM "axm/42f/axmr864" 
   ...
   WHILE TRUE
     ...
     CONSTRUCT BY NAME tm.wc ON ima131,ima1005
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW axmr864_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('axmr864',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW axmr864_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
FUNCTION axmr864_1(l_plant,l_ima01,l_ccc08,l_oga02)          #抓取金額和成本
...
FUNCTION axmr864()
   ...
   DROP TABLE axmr864_tmp
   CREATE TEMP TABLE axmr864_tmp(
          ima131     LIKE ima_file.ima131,
   ...
   DELETE FROM axmr864_tmp
   
   #FUN-AA0024 add --------------------------------begin----------------------------------
    DROP TABLE axmr864_tmp1
    CREATE TEMP TABLE axmr864_tmp1(
          ima131     LIKE ima_file.ima131,
          oba02      LIKE oba_file.oba02,
          ohb14t     LIKE ohb_file.ohb14t,
          ccc23      LIKE ccc_file.ccc23,
    ...
    CALL cl_replace_sqldb(l_sql) RETURNING l_sql        
    CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql               
    PREPARE axmr864_prepare1 FROM l_sql
    ...
    DECLARE axmr864_curs1 CURSOR FOR axmr864_prepare1
    FOREACH axmr864_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
    ...
    CASE tm.type01                     #根據不同類型抓取銷售成本
       WHEN '1'           
         CALL axmr864_1(l_plant,sr.ima01,'',sr.oga02) 
            RETURNING sr.ccc23
       ...
       WHEN '2' 
         ...
         LET sr.ccc23 = sr.ccc23*sr.ogb12                 #計算銷售成本
       WHEN '3'     
          CALL axmr864_1(l_plant,sr.ima01,sr.ogb092,sr.oga02) 
             RETURNING sr.ccc23   
             LET sr.ccc23 = sr.ccc23*sr.ogb12             #計算銷售成本
       WHEN '4'     
          CALL axmr864_1(l_plant,sr.ima01,sr.ogb41,sr.oga02) 
             RETURNING sr.ccc23 
             LET sr.ccc23 = sr.ccc23*sr.ogb12             #計算銷售成本
       WHEN '5'     
...
#FUN-AA0024 add --------------------------------begin----------------------------------
    ...
    CALL cl_replace_sqldb(l_sql) RETURNING l_sql        
    CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql               
    PREPARE axmr864_prepare11 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       EXIT PROGRAM
    ...
    DECLARE axmr864_curs11 CURSOR FOR axmr864_prepare11
    FOREACH axmr864_curs11 INTO sr1.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
    ...
    CASE tm.type01                     #根據不同類型抓取銷售成本
       WHEN '1'           
         CALL axmr864_1(l_plant,sr1.ima01,'',sr1.oha02) 
            RETURNING sr1.ccc23
       ...
       WHEN '2' 
         ...
         END IF                                             #計算銷售成本
       WHEN '3'     
          CALL axmr864_1(l_plant,sr1.ima01,sr1.ohb092,sr1.oha02) 
             RETURNING sr1.ccc23   
             LET sr1.ccc23 = sr1.ccc23*sr1.ohb12             #計算銷售成本
       WHEN '4'     
          CALL axmr864_1(l_plant,sr1.ima01,'',sr1.oha02) 
...
#FUN-AA0024 add --------------------------------end------------------------------------
   ...
   END FOREACH
      LET l_sql = " SELECT a.ima131,'',0,",
                  "SUM(a.ccc23)-(SELECT CASE WHEN SUM(b.ccc23) IS NULL THEN 0 ELSE SUM(b.ccc23) END FROM axmr864_tmp1 b WHERE a.ima131=b.ima131),",
                  "SUM(a.ogb12),",
                  "SUM(a.ogb14t_2)-(SELECT CASE WHEN SUM(b.ohb14t_2) IS NULL THEN 0 ELSE SUM(b.ohb14t_2) END FROM axmr864_tmp1 b WHERE a.ima131=b.ima131),",
                  "SUM(a.ogb14t_3)-(SELECT CASE WHEN SUM(b.ohb14t_3) IS NULL THEN 0 ELSE SUM(b.ohb14t_3) END FROM axmr864_tmp1 b WHERE a.ima131=b.ima131) ",
                  "   FROM axmr864_tmp a",
                  " GROUP BY a.ima131"
      PREPARE axmr864_prepare2 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM
   END IF
   DECLARE axmr864_curs2 CURSOR FOR axmr864_prepare2

   FOREACH axmr864_curs2 INTO sr.ima131,l_oba02,sr.ogb14t_2,
   ...
   LET g_str = tm.wc1," and ",tm.wc
         CALL cl_prt_cs3('axmr864','axmr864',l_sql,g_str)  
END FUNCTION
#FUN-A70025
