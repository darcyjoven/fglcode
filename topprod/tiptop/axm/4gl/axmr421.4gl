...
MAIN
   ...
   #
   #DROP TABLE r421_tmp1
   #CREATE TEMP TABLE r421_tmp1(
...
#TQC-730022 end
   ...
   IF (cl_null(g_bgjob) OR g_bgjob = 'N') THEN  # If background   #FUN-570069
      CALL axmr421_tm(0,0)             # Input print condition
   ELSE 
      CALL axmr421()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
END MAIN
 
FUNCTION axmr421_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   
   ...
   OPEN WINDOW axmr421_w AT p_row,p_col WITH FORM "axm/42f/axmr421"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmr421_w 
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_rep_clas CLIPPED,"'",           
                         " '",g_template CLIPPED,"'"            
         CALL cl_cmdat('axmr421',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmr421_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#TQC-C10039--add--end---
   ...
   #DELETE FROM r421_tmp
   #DELETE FROM r421_tmp1
   #
   #LET g_sql = "INSERT INTO r421_tmp ",
   #            " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
   #            "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
   #            "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
   #            "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
   ...
   #END IF
   #
   #LET g_sql = "INSERT INTO r421_tmp1 VALUES(?,?,?,?,?, ?,?,?)"
   #PREPARE insert_prep1 FROM g_sql
   #IF STATUS THEN
   ...
   LET l_sql="SELECT oea00,oea01,oea02,oea10,oea11,oea12,",   #No.MOD-570395
            ...
            #"      ,occ55,",      #FUN-7B0142 mark  #FUN-570069 add
             ...
             "  FROM oea_file,oeb_file,occ_file,OUTER ima_file,OUTER azf_file,OUTER ged_file ",
             " WHERE oea01=oeb01 ",
             "   AND oea_file.oea43=ged_file.ged01 ",
             "   AND oea04=occ01 ",
             "   AND oeb_file.oeb1001=azf_file.azf01 ",
             "   AND oeaconf != 'X' ", 
             "   AND ima_file.ima01=oeb_file.oeb04 ",
             "   AND oea00 IN ('8','9') ",
             "   AND ",tm.wc CLIPPED,
             " ORDER BY oea01,oea02,oeb03 "
   PREPARE axmr421_prepare1 FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM 
   END IF
   DECLARE axmr421_curs1 CURSOR FOR axmr421_prepare1
   IF STATUS THEN CALL cl_err('declare:',STATUS,1) RETURN END IF
 
   LET l_i = 0  
   CALL g_show_msg.clear() #FUN-650020
   FOREACH axmr421_curs1 INTO sr.*
      ...
      IF g_oaz.oaz121 = "1" THEN
         ...
         #FUN-650020...............begin
         IF r421_err_ana(g_showmsg) THEN
      ...
      IF NOT cl_null(l_ocf.ocf01) THEN
         ...
         LET g_ctn_no2=""
         #-----No.MOD-570395 END-----
         LET l_ocf.ocf101=r421_ocf_c(l_ocf.ocf101)
         LET l_ocf.ocf102=r421_ocf_c(l_ocf.ocf102)
         LET l_ocf.ocf103=r421_ocf_c(l_ocf.ocf103)
         LET l_ocf.ocf104=r421_ocf_c(l_ocf.ocf104)
         LET l_ocf.ocf105=r421_ocf_c(l_ocf.ocf105)
         LET l_ocf.ocf106=r421_ocf_c(l_ocf.ocf106)
         LET l_ocf.ocf107=r421_ocf_c(l_ocf.ocf107)
         LET l_ocf.ocf108=r421_ocf_c(l_ocf.ocf108)
         LET l_ocf.ocf109=r421_ocf_c(l_ocf.ocf109)
         LET l_ocf.ocf110=r421_ocf_c(l_ocf.ocf110)
         LET l_ocf.ocf111=r421_ocf_c(l_ocf.ocf111)
         LET l_ocf.ocf112=r421_ocf_c(l_ocf.ocf112)
         LET l_ocf.ocf201=r421_ocf_c(l_ocf.ocf201)
         LET l_ocf.ocf202=r421_ocf_c(l_ocf.ocf202)
         LET l_ocf.ocf203=r421_ocf_c(l_ocf.ocf203)
         LET l_ocf.ocf204=r421_ocf_c(l_ocf.ocf204)
         LET l_ocf.ocf205=r421_ocf_c(l_ocf.ocf205)
         LET l_ocf.ocf206=r421_ocf_c(l_ocf.ocf206)
         LET l_ocf.ocf207=r421_ocf_c(l_ocf.ocf207)
         LET l_ocf.ocf208=r421_ocf_c(l_ocf.ocf208)
         LET l_ocf.ocf209=r421_ocf_c(l_ocf.ocf209)
         LET l_ocf.ocf210=r421_ocf_c(l_ocf.ocf210)
         LET l_ocf.ocf211=r421_ocf_c(l_ocf.ocf211)
         LET l_ocf.ocf212=r421_ocf_c(l_ocf.ocf212)
      ELSE
         LET l_ocf.ocf01=' '
         LET l_ocf.ocf02=' '
         LET l_ocf.ocf101=' '
      ...
      EXECUTE insert_prep USING    #MOD-870006
        ...
        #sr.oeb20    ,sr.oeb21    ,sr.occ55    ,sr.oeb916   ,sr.oeb917   ,  #FUN-7B0142 mod
         ...
         l_ocf.ocf210,l_ocf.ocf211,l_ocf.ocf212,"",l_img_blob,"N",""       #TQC-C10039 ADD "",l_img_blob,"N",""
   ...
   PREPARE r421_p FROM l_sql
   DECLARE r421_curs CURSOR FOR r421_p
   FOREACH r421_curs INTO sr.oea01
     IF SQLCA.sqlcode THEN
   ...
   #   
   #PREPARE axmr421_prepare0 FROM l_sql
   #IF SQLCA.sqlcode != 0 THEN
   #   CALL cl_err('prepare0:',SQLCA.sqlcode,1)
   #END IF
   #DECLARE axmr421_curs0 CURSOR FOR axmr421_prepare0
   #FOREACH axmr421_curs0 INTO cr.*
...
#TQC-C10039--add--start---
     ...
     LET g_cr_apr_key_f = "oea01"  #報表主鍵欄位名稱
#TQC-C10039--add--end---
   CALL cl_prt_cs3('axmr421','axmr421',g_sql,g_str)
   #------------------------------ CR (4) ------------------------------#
...
FUNCTION r421_err_ana(ls_showmsg)    #FUN-650020
