...
# Modify.........: No.MOD-980205 09/08/25 By Smapmin 加入留置碼為NULL的條件
 
# Modify.........: No.FUN-980030 09/08/31 By Hiko 加上GP5.2的相關設定
# Modify.........: No:CHI-A70044 10/08/11 By Summer 1.在axmq220銷退量(oeb25)的後面加上償價數量(oeb29)
#                                                   2.未出貨量(unqty)要多扣oeb29的數量
...
DEFINE
    ...
    g_oeb DYNAMIC ARRAY OF RECORD
            ...
            oeb29   LIKE oeb_file.oeb29,   #CHI-A70044 add
...
DEFINE          l_sl		LIKE type_file.num5          #No.FUN-680137 SMALLINT
   ...
   END IF
    ...
    OPEN WINDOW q220_w AT p_row,p_col
         WITH FORM "axm/42f/axmq220"
...
#       CALL q220_q()
#    END IF
IF NOT cl_null(g_argv1) THEN CALL q220_q() END IF
    CALL q220_menu()
    CLOSE WINDOW q220_w
...
FUNCTION q220_cs()
   ...
   ELSE
      LET g_sql=" SELECT occ01 FROM occ_file ",
                ...
                "     WHERE oea01=oeb01 AND oeb12>(oeb24-oeb25) ",
                "       AND ",tm.wc2 CLIPPED,
                "       AND oeb70='N' ",
                "       AND oeahold IS NULL ",   #MOD-980205
                "       AND oeaconf = 'Y') "
   END IF
   #-----END MOD-8B0192-----
...
FUNCTION q220_b_askkey()
   CONSTRUCT tm.wc2 ON oea01,oea04,oeb04,oeb092,oeb06,oeb12,oeb23,
                       oeb24,oeb25,oeb29,oeb15,oeb16 #CHI-A70044 add oeb29
                  FROM s_oeb[1].oea01, s_oeb[1].oea04,s_oeb[1].oeb04,
                       ...
                       s_oeb[1].oeb29, s_oeb[1].oeb15, s_oeb[1].oeb16 #CHI-A70044 add oeb29
              #No.FUN-580031 --start--     HCN
              BEFORE CONSTRUCT
                 CALL cl_qbe_init()
              #No.FUN-580031 --end--       HCN
...
FUNCTION q220_q()
    ...
    ELSE
        OPEN q220_cnt
        FETCH q220_cnt INTO g_row_count
...
DEFINE
    ...
    CASE p_flag
        WHEN 'N' FETCH NEXT     q220_cs INTO g_occ.occ01
        WHEN 'P' FETCH PREVIOUS q220_cs INTO g_occ.occ01
        WHEN 'F' FETCH FIRST    q220_cs INTO g_occ.occ01
        WHEN 'L' FETCH LAST     q220_cs INTO g_occ.occ01
        WHEN '/'
...
#                       CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
            ...
            END IF
            LET mi_no_ask = FALSE
            FETCH ABSOLUTE g_jump q220_cs INTO g_occ.occ01
    END CASE
...
FUNCTION q220_b_fill()              #BODY FILL UP
   ...
   LET l_sql =
        "SELECT oea01,oea04,oeb04,oeb092,oeb06,oeb12,oeb23,0,oeb24,oeb25,oeb29,", #No.FUN-610020 #CHI-A70044 add oeb29
        "       0,oeb15,oeb16,oeb03 ",  #No.FUN-610020
        ...
        " ORDER BY oeb15"
    PREPARE q220_pb FROM l_sql
    DECLARE q220_bcs                       #BODY CURSOR
        CURSOR WITH HOLD FOR q220_pb
#TQC-790065---start--
...
#TQC-790065---end---
    LET g_cnt = 1
    FOREACH q220_bcs INTO g_oeb[g_cnt].*,l_oeb03  #No.FUN-610020
        IF SQLCA.sqlcode THEN
        ...
        IF g_oeb[g_cnt].oeb29 IS NULL THEN LET g_oeb[g_cnt].oeb29 = 0 END IF #CHI-A70044 add
        LET g_oeb[g_cnt].unqty = g_oeb[g_cnt].oeb12 -
                                 g_oeb[g_cnt].oeb24 + g_oeb[g_cnt].oeb25
                                 - g_oeb[g_cnt].oeb29 #CHI-A70044 add
        #No.FUN-610020  --Begin
        SELECT SUM(ogb12) INTO g_oeb[g_cnt].acc_q
          FROM ogb_file,oga_file,oea_file,oeb_file
         WHERE ogb31 = g_oeb[g_cnt].oea01 AND ogb32 = l_oeb03
           AND ogb01 = oga01
           AND oeb01 = oea01
           ...
           #AND oga09 = '8'   AND oga65 = 'Y'   #MOD-980205
           AND oga09 = '8'     #MOD-980205
