# Prog. Version..: '5.30.06-13.04.03(00008)'     #
#
# Pattern name...: axmr430.4gl
...
# Modify.........: No.MOD-D30255 13/03/28 By Vampire 會出現「參數欄位的類型和參數欄位的目前值不相容」訊息
...
#No.FUN-7B0026---Begin                                                                                                              
   ...
   #LET l_table = cl_prt_temptable('anmr430',g_sql) CLIPPED    #FUN-940063 mark
   LET l_table = cl_prt_temptable('axmr430',g_sql) CLIPPED     #FUN-940063 add                           
   IF l_table = -1 THEN EXIT PROGRAM END IF                                                                                         
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,                                                                  
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?) " #FUN-840166 add1?                                                                 
...
#-------------No.TQC-610089 end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL axmr430_tm(0,0)             # Input print condition
      ELSE CALL axmr430()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
END MAIN
 
FUNCTION axmr430_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW axmr430_w AT p_row,p_col WITH FORM "axm/42f/axmr430"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmr430_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmr430',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmr430_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#     WHERE azi01=g_aza.aza17               #No.CHI-6A0004 
     ...
     LET l_sql ="SELECT '','','',oeb15,oeb16,oea01,oea14,gen02,oea15,gem02,",
                ...
                "       oeb910,oeb912,oeb913,oeb915,oeb03 ", #FUN-860144 add oeb03  #No.FUN-580004
                "  FROM oea_file,oeb_file,",
                "       OUTER occ_file,OUTER gen_file,OUTER gem_file ",
                " WHERE oea01=oeb01 ",
     ...
     PREPARE axmr430_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
        EXIT PROGRAM
     END IF
     DECLARE axmr430_curs1 CURSOR FOR axmr430_prepare1
 
#     CALL cl_outnam('axmr430') RETURNING l_name      #No.FUN-7B0026
 
#FUN-580004--begin
     SELECT sma115 INTO g_sma115 FROM sma_file
#     IF g_sma115 = "Y"  THEN                  #No.FUN-7B0026 
#             LET l_name='axmr430'             #No.FUN-7B0026
#            LET g_zaa[47].zaa06 = "N"         #No.FUN-7B0026
 
#     ELSE                                     #No.FUN-7B0026 
#             LET l_name='axmr430_1'           #No.FUN-7B0026
#            LET g_zaa[47].zaa06 = "Y"         #No.FUN-7B0026
#     END IF                                   #No.FUN-7B0026 
#      CALL cl_prt_pos_len()                   #No.FUN-7B0026
#No.FUN-580004--end
#     START REPORT axmr430_rep TO l_name       #No.FUN-7B0026
 
#     LET g_pageno = 0                         #No.FUN-7B0026
     FOREACH axmr430_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#       LET sr.order3 = l_order[3]
      ...
      ELSE                                                                                                                          
      END IF                     
#       OUTPUT TO REPORT axmr430_rep(sr.*)
      EXECUTE insert_prep USING sr.oea01,sr.oea03,sr.oea04,sr.oea032,sr.oea14,sr.oea15,sr.oeb04,
                                ...
                                sr.gen02,sr.gem02,sr.oeb03   #FUN-840166 add sr.oeb03
 
#No.FUN-7B0026---End
     END FOREACH
#No.FUN-7B0026---Begin 
#     FINISH REPORT axmr430_rep
   IF g_zz05 = 'Y' THEN                                                                                                          
      CALL cl_wcchp(tm.wc,'oeb15,oeb16,oea01,oea14,oea15,oea03,oea04,oeb04,oeb09')       
           RETURNING tm.wc                                                                                                       
  #MOD-D30255 add start -----
   ELSE
      LET tm.wc=''
  #MOD-D30255 add end   -----
   END IF                                                                                                                        
   ...
   CALL cl_prt_cs3('axmr430','axmr430',l_sql,g_str)    
#     CALL cl_prt(l_name,g_prtway,g_copies,g_len)
#No.FUN-7B0026---End 
END FUNCTION
#No.FUN-7B0026---Begin
  
#REPORT axmr430_rep(sr)
