...
#DEFINE   g_sql           STRING              #TQC-C10039   #No.TQC-C20203 mark
#DEFINE 	 l_table         STRING              #TQC-C10039   #No.TQC-C20203 mark
MAIN
   OPTIONS
       INPUT NO WRAP
...
#No.TQC-C20203 --- mark --- start
#  LET g_sql="occ01.occ_file.occ01,occ03.occ_file.occ03,",
#            "occ22.occ_file.occ22,occ21.occ_file.occ21,",
#            "occ20.occ_file.occ20,occ241.occ_file.occ241,",
#            "occ242.occ_file.occ242,occ243.occ_file.occ243,",
#            "occ244.occ_file.occ244,occ245.occ_file.occ245,",
#            "occ231.occ_file.occ231,occ232.occ_file.occ232,",
#            "occ233.occ_file.occ233,occ234.occ_file.occ234,",
#            "occ235.occ_file.occ235,oce02.oce_file.oce02,",
#            "oce03.oce_file.oce03,occ18.occ_file.occ18,",
#            "occ19.occ_file.occ19,sign_type.type_file.chr1,",
#            "sign_img.type_file.blob,sign_show.type_file.chr1,",
#            "sign_str.type_file.chr1000"
#  LET l_table = cl_prt_temptable('axmr201',g_sql) CLIPPED
#  IF l_table = -1 THEN EXIT PROGRAM END IF
#No.TQC-C20203 --- mark ---end
#TQC-C10039--add--end-----
...
#------------No.TQC-610089 end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'	# If background job sw is off
      THEN CALL axmr201_tm(0,0)		# Input print condition
      ELSE CALL axmr201()		# Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
END MAIN
 
FUNCTION axmr201_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW axmr201_w AT p_row,p_col WITH FORM "axm/42f/axmr201"
...
#FUN-A20005 ---end---       
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmr201_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
  ...
  INPUT BY NAME tm.b,tm.c,tm.d,tm.e,tm.more WITHOUT DEFAULTS       #No.FUN-710090 
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmr201',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW axmr201_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#                                 occ244 LIKE occ_file.occ244,
#                                 occ245 LIKE occ_file.occ245,
                                  #No.TQC-C20203 --- mark --- end
                                  #TQC-C10039--add--end-----
...
#   CALL cl_del_data(l_table)                      #No.TQC-C20203 mark
#TQC-C10039--add--end-----

     SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
   # SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'axmr201'   #No.FUN-710090 mark
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'axmr201'              #No.FUN-710090
...
#      END IF             
     ...
     LET l_sql = "SELECT  ",
                 ...
                 "   occ01,  occ03,  occ22,  occ21,  occ20,",
                 "  occ241, occ242, occ243, occ244, occ245,",     #FUN-550091  #MOD-790170 modify
                 " occ231 , occ232, occ233, occ234, occ235, " ,   #FUN-550091  #MOD-790170 modify
   ...
   ### FUN-710090 先判斷有無資料或其他問題，再串到CR ###
     PREPARE axmr201_prepare1 FROM l_sql
    ...
    #TQC-C10039--add--start------
    #No.TQC-C20203---mark---start
#    DECLARE r201_cs1 CURSOR FOR axmr201_prepare1
#    FOREACH r201_cs1 INTO sr.*
#       IF SQLCA.sqlcode != 0 THEN
#          CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
#       END IF
#       EXECUTE insert_prep USING 
#          sr.occ01 , sr.occ03 , sr.occ22 , sr.occ21 , sr.occ20 ,
#          sr.occ241, sr.occ242, sr.occ243, sr.occ244, sr.occ245,
#          sr.occ231, sr.occ232, sr.occ233, sr.occ234, sr.occ235,
#          sr.oce02 , sr.oce03 , sr.occ18 , sr.occ19 , ""       ,
#          l_img_blob,"N"      ,"" 
#    END FOREACH
...
#     LET g_cr_apr_key_f = "occ01"  #報表主鍵欄位名稱        #No.TQC-C20203 mark
     #TQC-C10039--add--end-----
      CASE
        WHEN tm.c='1'
         #  CALL cl_prt_cs1('axmr201_1',l_sql,g_str)    #TQC-730088
            CALL cl_prt_cs1('axmr201','axmr201_1',l_sql,g_str) 
#         CALL cl_prt_cs3('axmr201','axmr201_1',g_sql,g_str) #TQC-C10039  #No.TQC-C20203 mark
        WHEN tm.c='2'
         #  CALL cl_prt_cs1('axmr201_2',l_sql,g_str)    #TQC-730088
            CALL cl_prt_cs1('axmr201','axmr201_2',l_sql,g_str) 
#         CALL cl_prt_cs3('axmr201','axmr201_2',g_sql,g_str) #TQC-C10039   #No.TQC-C20203 mark
        WHEN tm.c='3'
         #  CALL cl_prt_cs1('axmr201_3',l_sql,g_str)    #TQC-730088
            CALL cl_prt_cs1('axmr201','axmr201_3',l_sql,g_str)
#         CALL cl_prt_cs3('axmr201','axmr201_3',g_sql,g_str) #TQC-C10039   #No.TQC-C20203 mark
      END CASE
...
#  ##No.FUN-710090--begin-- mark #####
#       DECLARE axmr201_curs1 CURSOR FOR axmr201_prepare1
#       LET l_name = 'axmr201.out'
#       CALL cl_outnam('axmr201') RETURNING l_name
#       #MOD-6C0071.................begin
#       CASE tm.c
#          WHEN "1"
#             LET g_len_201=38
...
#       END CASE
#       #MOD-6C0071.................end
#       START REPORT axmr201_rep TO l_name
#       LET g_cnt = 0
#       FOREACH axmr201_curs1 INTO sr.*
#         IF SQLCA.sqlcode != 0 THEN
#            CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
#         END IF
#         LET g_cnt = g_cnt + 1
#         OUTPUT TO REPORT axmr201_rep(sr.*)
#       END FOREACH
#  
#       FINISH REPORT axmr201_rep
#       CALL cl_prt(l_name,g_prtway,g_copies,g_len)
#  END FUNCTION
#  
#  REPORT axmr201_rep(sr)
#     DEFINE
