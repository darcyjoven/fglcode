...
# Modify.........: No:TQC-D10036 13/01/08 By xuxz oea01添加開窗
DATABASE ds
...
MAIN
   ...
   LET l_table = cl_prt_temptable('axmr420',g_sql) CLIPPED   # 產生Temp Table
...
DEFINE l_cmd          LIKE type_file.chr1000       #No.FUN-680137 VARCHAR(1000)
   ...
   OPEN WINDOW r420_w AT p_row,p_col WITH FORM "axm/42f/axmr420"
  ...
  END CONSTRUCT
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmr420',g_time,l_cmd)
      END IF
      CLOSE WINDOW r420_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
  ...
  {  SELECT azi03,azi04,azi05
     ...
     LET l_sql = "SELECT",
                ...
                #"  oef05,oea08,ima021,0,0", #FUN-720004 modify  #No.TQC-A40055
                 "  0,oea08,ima021,0,0",     #No.TQC-A50044
...
#            LET sr.tot2=(sr.tot1/sr.oeb14)            #TQC-C20014
          ...
          EXECUTE insert_prep USING
             ...
             #sr.oea14, sr.oea15,   sr.oea23,  sr.oea21,  sr.oea31, sr.oea32,	#MOD-970080 mark
              ...
              sr.oea08, sr.ima021,  sr.tot1,   sr.tot2
          #------------------------------ CR (3) ------------------------------#
     ...
     LET g_str = g_str,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",
                 tm.t,";",tm.u,";",t_azi04,";",t_azi05  #FUN-710080 modify
     CALL cl_prt_cs3('axmr420','axmr420',l_sql,g_str)   #FUN-710080 modify
     #------------------------------ CR (4) ------------------------------#
