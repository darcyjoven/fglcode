...
DEFINE tm  RECORD
           wc      STRING,
           c       LIKE type_file.chr1,        # Prog. Version..: '5.30.06-13.03.12(01)    #NO.FUN-650013     
...
#No.FUN-990073 --End
   LET l_table = cl_prt_temptable('axmr440',g_sql)
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'
      THEN CALL r440_tm(0,0)
      ELSE CALL r440()
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
END MAIN
 
FUNCTION r440_tm(p_row,p_col)
...
DEFINE l_cmd          LIKE type_file.chr1000       #No.FUN-680137 VARCHAR(400)
   ...
   OPEN WINDOW r440_w AT p_row,p_col WITH FORM "axm/42f/axmr440"
...
#                  CALL FGL_DIALOG_SETBUFFER( tm.rpg01 )
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmr440',g_time,l_cmd)
      END IF
      CLOSE WINDOW r440_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
...
#No.FUN-580013  --begin
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'axmr440'
...
#     LET l_sql = "SELECT oeb04,oeb12,oeb15,oeb16,oeahold,oeaconf ",  #No.MOD-890099 mark by liuxqa
      LET l_sql = "SELECT oeb04,oeb12-oeb24+oeb25,oeb15,oeb16,oeahold,oeaconf", #No.MOD-890099 modify by liuxqa
     ...
     PREPARE r440_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
        EXIT PROGRAM
     END IF
     DECLARE r440_curs1 CURSOR FOR r440_prepare1
 
     SELECT * INTO g_rpg.* FROM rpg_file WHERE rpg01=tm.rpg01
 
#     CALL cl_outnam('axmr440') RETURNING l_name            #NO.FUN-840039
 
#     START REPORT r440_rep TO l_name                       #NO.FUN-840039
...
#         OUTPUT TO REPORT r440_rep(sr.*)             #NO.FUN-840039--
         #NO.FUN-840039-------------BEGIN-------------------
...
#No.FUN-990073 --End
         ...
         EXECUTE insert_prep USING l_betot,l_tot1,l_tot2,l_tot3,l_tot4,
             ...
             ,l_ima02,l_ima021                            #No.FUN-990073
     ...
     ELSE
        LET tm.wc=""
     END IF
     LET g_str = tm.wc,";",l_title1,";",l_title2,";",l_title3,";",l_title4,";",
                 l_title5,";",l_title6,";",l_title7,";",l_title8,";",
                 l_title9,";",l_title10,";",l_title11,";",tm.c,";",
                 tm.date,";",tm.a,";",tm.b,";",tm.o
     CALL cl_prt_cs3('axmr440','axmr440',g_sql,g_str)
         #NO.FUN-840039-------------END----------------------     
#     FINISH REPORT r440_rep                               #NO.FUN-840039
