...
# Modify.........: No:TQC-B50074 11/05/16 By lixia 開窗全選報錯修改
# Modify.........: No:MOD-B60064 11/06/15 By Summer 將FOREACH axmr553_curs1多的ogd.*拿掉
...
MAIN
   ...
   LET l_table = cl_prt_temptable('axmr553',g_sql) CLIPPED   # 產生Temp Table
  ...
  #MOD-7A0121---add---str---
   ...
   LET l_table1 = cl_prt_temptable('axmr5531',g_sql) CLIPPED   # 產生Temp Table
  ...
  #No.FUN-A80091 --start--
    ...
    LET l_table2 = cl_prt_temptable('axmr5532',g_sql) CLIPPED
 ...
 #--------------No.TQC-610089 end
   IF cl_null(tm.wc)
      THEN CALL axmr553_tm(0,0)             # Input print condition
      ELSE 
         #LET tm.wc="ofa01 ='",tm.wc CLIPPED,"'" #CHI-AB0029 mark
         CALL axmr553()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
END MAIN
 
FUNCTION axmr553_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW axmr553_w AT p_row,p_col WITH FORM "axm/42f/axmr553"
...
WHILE TRUE
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmr553_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmr553',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmr553_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#TQC-C20051-----MARK---END----
  ...
  #l_table
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,             
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?, ?,?)"   #FUN-810029 38?->41?  #FUN-A80091 add 2? #TQC-C10039 add 4?#No.TQC-C20051 MARK TQC-C10039
   PREPARE insert_prep FROM g_sql                                              
   IF STATUS THEN                                                               
  ...
  #No.FUN-A80091---END
  #MOD-7A0121---mark---str---
  #DELETE FROM r553_tmp
  #DELETE FROM r553_tmp1
  #DELETE FROM r553_tmp2
 
  #LET g_sql = "INSERT INTO r553_tmp ",
  #            " VALUES(?,?,?,?,?, ?,?,?,?,?,",
  #            "        ?,?,?,?,?, ?,?,?,?,?,",
  #            "        ?,?,?,?,?)"
  ...
  #LET g_sql = "INSERT INTO r553_tmp1 ",
  #            " VALUES(?,?,?,?,?, ?,?,?,?,?,",
  #            "        ?,?,?)"
  #PREPARE insert_prep1 FROM g_sql
  #IF STATUS THEN
  ...
  #LET g_sql = "INSERT INTO r553_tmp2 VALUES(?,?,?,?,?, ?,?)"
  #PREPARE insert_prep2 FROM g_sql
  #IF STATUS THEN
...
#No.FUN-580013  --begin
#   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'axmr553'
...
#No.FUN-580013  --end
   ...
   LET l_sql="SELECT ofa_file.*, ofb_file.* ",
             ...
             "   AND ",tm.wc CLIPPED,
             "   AND ofaconf !='X' "
   PREPARE axmr553_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare1:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   DECLARE axmr553_curs1 CURSOR FOR axmr553_prepare1
 
  #FOREACH axmr553_curs1 INTO ofa.*, ofb.*,ogd.* #MOD-B60064 mark
   FOREACH axmr553_curs1 INTO ofa.*, ofb.*       #MOD-B60064
      IF SQLCA.sqlcode != 0 THEN
  ...
  #MOD-7A0121---add---str---                                                               #MOD-7A0121---mark---str--
   ...
   IF tm.b='Y' THEN   #是否列印備註
      LET l_sql = "SELECT ofa01 FROM ofa_file ",
                  " WHERE ",tm.wc CLIPPED,      
                  "   ORDER BY ofa01"           
      PREPARE r553_prepare2 FROM l_sql
      IF SQLCA.sqlcode != 0 THEN      
         CALL cl_err('prepare2:',SQLCA.sqlcode,1)
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM  
      END IF           
      DECLARE r553_cs2 CURSOR FOR r553_prepare2
                                                                                 
      FOREACH r553_cs2 INTO ofa.ofa01
         #整張前備註
         DECLARE oao_c2 CURSOR FOR  
          SELECT * FROM oao_file
  ...
  #    "  FROM r553_tmp A,",
  #    "       r553_tmp1 B,r553_tmp2 C",
  #    " WHERE A.ofa01=B.ofa01_1(+)",
  #    "   AND A.ofa01=C.ofa01_2(+)"
  #PREPARE axmr553_prepare0 FROM l_sql
  #IF SQLCA.sqlcode != 0 THEN
  #   CALL cl_err('prepare0:',SQLCA.sqlcode,1)
  #END IF
  #DECLARE axmr553_curs0 CURSOR FOR axmr553_prepare0
  #FOREACH axmr553_curs0 INTO cr.*
  ...
  #MOD-7A0121---mark---end--
   ...
   ELSE
      LET tm.wc = ''
   END IF                   
   LET g_str = tm.d,";",tm.b,";",tm.wc,";",
               tm.a,";",tm.c,";",tm.e               #FUN-740057 add tm.a,tm.c  #FUN-A80091 add tm.e
#TQC-C20051-----MARK---STAR-----
   #TQC-C10039--add--start---
...
#    LET g_cr_apr_key_f = "ofa01"  #報表主鍵欄位名稱，用"|"隔開
    #TQC-C10039--add--end---
#TQC-C20051-----MARK---END----
   CALL cl_prt_cs3('axmr553','axmr553',g_sql,g_str)
   #------------------------------ CR (4) ------------------------------#
   #end FUN-710080 add
...
{
REPORT axmr553_rep(ofa, ofb,ogd, sr)
