...
# Modify.........: No.FUN-D40128 13/05/08 By wangrr 更改"額度客戶""業務員編號""部門編號"開窗
...
MAIN
   ...
   LET  g_sql = "occ01.occ_file.occ01,",
                ...
                "g_head1.type_file.chr100"      #FUN-CB0002  add
   LET l_table = cl_prt_temptable('axmx210',g_sql) CLIPPED
   ...
   CREATE TEMP TABLE x210_ccc(
    ...
    DELETE FROM x210_ccc WHERE 1=1
   ...
   IF cl_null(tm.wc) THEN
      CALL axmx210_tm(0,0)             # Input print condition
   ELSE
      CALL axmx210()                   # Read data and create out-file
   END IF
 
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
END MAIN
 
FUNCTION axmx210_tm(p_row,p_col)
 
   DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   DEFINE p_row,p_col    LIKE type_file.num5,       #No.FUN-680137 SMALLINT
          l_cmd          LIKE type_file.chr1000       #No.FUN-680137 VARCHAR(1000)
 
   OPEN WINDOW axmx210_w WITH FORM "axm/42f/axmx210"
      ...
      ON ACTION CONTROLP
        CASE
          WHEN INFIELD(occ33)
            ...
            #LET g_qryparam.form ="q_occ33"    #FUN-D40128 mark
             LET g_qryparam.form ="q_occ33_1"  #FUN-D40128
          ...
          WHEN INFIELD(occ04)
             CALL cl_init_qry_var()
             LET g_qryparam.state = 'c'
            #LET g_qryparam.form ="q_occ04"     #FUN-D40128 mark
             LET g_qryparam.form ="q_occ04_1"   #FUN-D40128 
             CALL cl_create_qry() RETURNING g_qryparam.multiret
          ...
          WHEN INFIELD(occgrup)
             CALL cl_init_qry_var()
             LET g_qryparam.state = 'c'
            #LET g_qryparam.form ="q_occgrup"    #FUN-D40128 mark
             LET g_qryparam.form ="q_occgrup_1"  #FUN-D40128
             CALL cl_create_qry() RETURNING g_qryparam.multiret
...
#TQC-C60125 --end-- 
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmx210_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
  ...
  #UI
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmx210',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmx210_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
FUNCTION axmx210()
   ...
   #FUN-CB0002--add--str--
   DEFINE l_msg1 LIKE type_file.chr100
   DEFINE l_msg2 LIKE type_file.chr100
   DEFINE l_msg3 LIKE type_file.chr100
   DEFINE l_msg4 LIKE type_file.chr100
   DEFINE l_tot  LIKE type_file.chr20
   #FUN-CB0002--add--end--
         
   CALL cl_del_data(l_table)                     #NO.FUN-850009
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'axmx210'  #NO.FUN-850009
   
     SELECT zo02 INTO g_company FROM zo_file WHERE zo01 = g_rlang
     DECLARE axmx210_za_cur CURSOR FOR
             SELECT za02,za05 FROM za_file
              WHERE za01 = "axmx210" AND za03 = g_rlang
     FOREACH axmx210_za_cur INTO g_i,l_za05
        LET g_x[g_i] = l_za05
     END FOREACH
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'axmx210'
     ...
     LET l_sql="SELECT UNIQUE occ33,'',''",
               ...
               " ORDER BY occ33 "
     PREPARE axmx210_prepare  FROM l_sql
     IF SQLCA.sqlcode THEN
        CALL cl_err('prepare0:',SQLCA.sqlcode,1)
        RETURN
     END IF
     DECLARE axmx210_curs CURSOR FOR axmx210_prepare
     IF SQLCA.sqlcode THEN
     ...
     LET l_sql=" SELECT occ01,occ02,occ61,occ63,occ64,occ36,occ631 ",
               ...
               "   AND occ62 = 'Y'",
               " ORDER BY occ01 "
     PREPARE axmx210_prepare1 FROM l_sql
     IF SQLCA.sqlcode THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        RETURN
     END IF
     DECLARE axmx210_curs1 CURSOR FOR axmx210_prepare1
     IF SQLCA.sqlcode THEN
     ...
     LET s65=0  LET s70=0
     FOREACH axmx210_curs  INTO g_occ33
       IF SQLCA.sqlcode THEN
       ...
       LET stot=0 LET sbal=0
       LET s65=0  LET s70=0
       FOREACH axmx210_curs1 USING g_occ33,g_occ33
         ...
         CALL x210_b_get(sr.occ01,l_occ61)
       ...
       LET l_bal=stot
       #計算信用餘額
       DECLARE x210_b CURSOR FOR
          SELECT * FROM x210_ccc
           WHERE oma50 <> 0
             AND kind <> 'A' AND  kind<> 'C'
            ORDER BY oma11,kind,plant
        FOREACH x210_b INTO sr.plant,sr.tradetype,sr.oma11,
      ...
      LET sr.occ02 = sr.occ02[1,10]  #FUN-CB0002  add
      LET l_msg4 = sr.occ01,' ',sr.occ02_o   #FUN-CB0002  add
      EXECUTE insert_prep USING
        ...
        t_azi04h,t_azi04,l_occ631,l_msg4
            LET i = i +1
        END FOREACH
       ...
       LET old_kind='1'
       #計算逾期票據/逾期帳款
       DECLARE x210_b2 CURSOR FOR
          SELECT * FROM x210_ccc
           WHERE oma50 <> 0
             AND (kind = 'A' OR  kind= 'C')
            ORDER BY kind,oma11,plant
        FOREACH x210_b2 INTO sr.plant,sr.tradetype,sr.oma11,
                            sr.oma01,sr.oma23,sr.oma50,sr.weight,sr.balance,
                            sr.kind,sr.occ02
            IF SQLCA.sqlcode THEN
      ...
      EXECUTE insert_prep USING
        sr.occ01,sr.occ02,sr.occ02_o,sr.tot,sr.plant,sr.tradetype,sr.oma11,
        sr.oma01,sr.oma23,sr.oma50,sr.weight,sr.balance,sr.kind,sr.num,
        t_azi04h,t_azi04,l_occ631,l_msg4    #FUN-CB0002  add
            LET i = i +1
...
###XtraGrid###     CALL cl_prt_cs3('axmx210','axmx210',g_sql,g_str) 
    LET g_xgrid.table = l_table    ###XtraGrid###
    IF g_zz05 = 'Y' THEN 
        CALL cl_wcchp(tm.wc,'occ33,occ04,occgrup')
           RETURNING tm.wc
    END IF
    LET g_xgrid.skippage_field = "occ01"
    #LET l_msg1 = cl_getmsg('axm-546',g_lang)
    #LET l_msg2 = cl_getmsg('axm-547',g_lang)
    #LET l_msg3 = cl_getmsg('axm-548',g_lang)
    #LET l_msg4 = sr.occ01,' ',sr.occ02_o
    #LET l_tot = sr.tot
    #LET g_xgrid.footerinfo1 = l_msg1,l_msg4,'|',l_msg2,l_tot,'|',l_msg3,l_occ631
    LET g_xgrid.condition = cl_getmsg('lib-160',g_lang),tm.wc
    
    CALL cl_xg_view()    ###XtraGrid###
END FUNCTION
 
FUNCTION x210_b_get(p_cus_no,l_occ61)
...
#              LET g_exrate=s_exrate_m(l_aza17,l_occ631,l_oaz212,l_azp01) #FUN-980020
         ...
         END IF
        #換算額度客戶幣別匯率
        CALL x210_check_exrate(l_occ631) RETURNING g_exrate
        #換算額度客戶幣別金額
        LET l_tmp = l_tmp*g_exrate
...
#       INSERT INTO x210_ccc VALUES(l_plant[l_i],l_type,l_date,l_no,g_curr,
        INSERT INTO x210_ccc VALUES('',l_type,l_date,l_no,g_curr,
                                    l_tmp,g_ocg.ocg02,0,'1',g_occ02)
        IF SQLCA.SQLCODE OR STATUS=100 THEN
           CALL cl_err3("ins","x210_ccc","","",SQLCA.SQLCODE,"","ins x210_ccc",0)   #No.FUN-660167
           EXIT FOREACH
        END IF
      ...
      LET l_sql=" SELECT oea01,oea02,oea23,oea24,oea211,",
       ...
       FOREACH t67_curs INTO l_oea01,l_date,g_curr,l_oea24,l_oea211,
        ...
        IF SQLCA.SQLCODE OR SQLCA.SQLERRD[3]=0 THEN
           CALL cl_err3("ins","x210_ccc",l_plant[l_i],"",SQLCA.SQLCODE,"","ins x210_cccD",0)
           EXIT FOREACH
        END IF
...
#                   "      ",s_dbstring(l_azp03 CLIPPED),"oeb_file ",  #FUN-930006      
       ...
       #LET l_amt2 = l_amt2*g_exrate #MOD-B20126 mark
        ...
        INSERT INTO x210_ccc VALUES(l_plant[l_i],l_type,l_date,l_no,l_curr,
                                   #l_amt2,g_ocg.ocg09,0,'9',g_occ02) #MOD-B20126 mark 
                                    l_amt,g_ocg.ocg09,0,'9',g_occ02)  #MOD-B20126
        IF SQLCA.SQLCODE OR STATUS=100 THEN
           CALL cl_err3("ins","x210_ccc","","",SQLCA.SQLCODE,"","ins x210_ccc",0)  
           EXIT FOREACH
        END IF
...
FUNCTION x210_check_exrate(p_occ631)
