...
#DEFINE       l_time    LIKE type_file.chr8            #No.FUN-6A0094
   ...
   END IF
    ...
    DECLARE t7105_cl CURSOR FROM g_forupd_sql
 
    LET p_row = 5 LET p_col = 17
 
    OPEN WINDOW t7105_w AT p_row,p_col              #顯示畫面
         WITH FORM "axm/42f/axmt7105"
...
DEFINE  lc_qbe_sn       LIKE    gbm_file.gbm01    #No.FUN-580031  HCN
   ...
   INITIALIZE g_ohf.* TO NULL    #No.FUN-750051
    ...
    PREPARE t7105_prepare FROM g_sql
    DECLARE t7105_cs                         #SCROLL CURSOR
        SCROLL CURSOR WITH HOLD FOR t7105_prepare
    ...
    DROP TABLE x
    PREPARE t7105_precount_x FROM g_sql
    EXECUTE t7105_precount_x
 
        LET g_sql="SELECT COUNT(*) FROM x "
 
    PREPARE t7105_precount FROM g_sql
    DECLARE t7105_count CURSOR FOR t7105_precount
...
FUNCTION t7105_u()
    ...
    OPEN t7105_cl USING g_ohf.ohf01,g_ohf.ohf02
    IF STATUS THEN
       CALL cl_err("OPEN t7105_cl:", STATUS, 1) 
       CLOSE t7105_cl
       ROLLBACK WORK
       RETURN
    END IF
 
    FETCH t7105_cl INTO g_ohf.*            # 鎖住將被更改或取消的資料
...
DEFINE
    ...
    INPUT BY NAME g_ohf.ohf04 WITHOUT DEFAULTS
...
FUNCTION t7105_ohf04(p_cmd)  #核示
    DEFINE p_cmd     LIKE type_file.chr1,          #No.FUN-680137 VARCHAR(1)
           l_genacti LIKE gen_file.genacti,
           l_gen02_2 LIKE gen_file.gen02
 
    LET g_errno = " "
    SELECT gen02,genacti INTO l_gen02_2,l_genacti FROM gen_file
...
FUNCTION t7105_q()
    ...
    OPEN t7105_cs                            # 從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
       CALL cl_err('',SQLCA.sqlcode,0)
       INITIALIZE g_ohf.* TO NULL
    ELSE
       OPEN t7105_count
       FETCH t7105_count INTO g_row_count
       DISPLAY g_row_count TO FORMONLY.cnt
       CALL t7105_fetch('F')                  # 讀出TEMP第一筆並顯示
    END IF
 
END FUNCTION
 
#處理資料的讀取
FUNCTION t7105_fetch(p_flag)
DEFINE
    p_flag          LIKE type_file.chr1                  #處理方式        #No.FUN-680137 VARCHAR(1)
 
    CASE p_flag
        WHEN 'N' FETCH NEXT     t7105_cs INTO g_ohf.ohf01,g_ohf.ohf02
        WHEN 'P' FETCH PREVIOUS t7105_cs INTO g_ohf.ohf01,g_ohf.ohf02
        WHEN 'F' FETCH FIRST    t7105_cs INTO g_ohf.ohf01,g_ohf.ohf02
        WHEN 'L' FETCH LAST     t7105_cs INTO g_ohf.ohf01,g_ohf.ohf02
        WHEN '/'
...
#                       CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
            ...
            END IF
            LET mi_no_ask = FALSE
            FETCH ABSOLUTE g_jump t7105_cs INTO g_ohf.ohf01,g_ohf.ohf02
    END CASE
...
DEFINE
    ...
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    DECLARE t7105_bcl CURSOR FROM g_forupd_sql      # LOCK CURSOR
    ...
    INPUT ARRAY g_ohg WITHOUT DEFAULTS FROM s_ohg.*
        ...
        BEFORE ROW
            ...
            IF g_rec_b >= l_ac THEN
               ...
               OPEN t7105_bcl USING g_ohf.ohf01, g_ohf.ohf02, g_ohg_t.ohg03
               IF STATUS THEN
                  CALL cl_err("OPEN t7105_bcl:", STATUS, 1)
                  LET l_lock_sw = "Y"
               ELSE
                  FETCH t7105_bcl INTO g_ohg[l_ac].*
                  IF SQLCA.sqlcode THEN
...
DEFINE
    ...
    CALL t7105_b_fill(l_wc2)
 
END FUNCTION
 
FUNCTION t7105_b_fill(p_wc2)              #BODY FILL UP
DEFINE
