...
# 
# Pattern name...: axmr820.4gl
# Descriptions...: 三角貿易採購單明細表(axmr820)
...
# Modify.........: No.FUN-B40092 11/06/01 By xujing \t特殊字元導致轉GR的時候p_gengre出錯
...
#---------------No.TQC-610089 modify
  ...
  #No.FUN-750012--begin--
   ...
   LET  l_table = cl_prt_temptable('axmr820',g_sql) CLIPPED
 ...
 # IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
   IF cl_null(tm.wc)
      THEN CALL axmr820_tm(0,0)                # Input print condition
      ELSE
          LET tm.more = 'N'
          LET g_pdate = g_today
          LET g_rlang = g_lang
         ...
         #LET tm.wc=" pmm01='",tm.wc CLIPPED,"' "   #No.TQC-610089 mark
           CALL axmr820()                           # Read data and create out-file
   END IF
 
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
END MAIN
 
FUNCTION axmr820_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,        #No.FUN-680137 SMALLINT
   ...
   OPEN WINDOW axmr820_w AT p_row,p_col WITH FORM "axm/42f/axmr820"
...
WHILE TRUE
 ...
 END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmr820_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
...
#UI
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmr820',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmr820_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL axmr820()
...
FUNCTION axmr820()
   DEFINE l_name    LIKE type_file.chr20,                # External(Disk) file name        #No.FUN-680137 VARCHAR(20)
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='axmr820'    #No.FUN-750012 add 
     ...
     PREPARE axmr820_prepare1 FROM l_sql
     ...
     END IF 
    #FUN-750012---mark---str---
    #CALL cl_outnam('axmr820') RETURNING l_name
    #START REPORT axmr820_rep TO l_name
    #FUN-750012---mark---end---
     DECLARE axmr820_curs1 CURSOR FOR axmr820_prepare1
     FOREACH axmr820_curs1 INTO sr.*,l_oea.*,l_pmm22  #MOD-830175
        IF SQLCA.sqlcode != 0 THEN
     ...
     CALL s_mtrade_last_plant(l_oea.oea904) RETURNING l_last,g_msg
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
            ...
            PREPARE axmr820_prepare2 FROM l_sql
            IF STATUS THEN CALL cl_err('axmr820_prepare2',STATUS,1) END IF
            DECLARE axmr820_curs2 CURSOR FOR axmr820_prepare2
               FOREACH axmr820_curs2 INTO sr1.*
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_azp01[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr820_prepare3 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr820_prepare3',STATUS,1) END IF
               DECLARE axmr820_curs3 CURSOR FOR axmr820_prepare3
               OPEN axmr820_curs3
               FETCH axmr820_curs3 INTO sr1.oap041, sr1.oap042, sr1.oap043
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_azp01[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr820_prepare4 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr820_prepare4',STATUS,1) END IF
               DECLARE axmr820_curs4 CURSOR FOR axmr820_prepare4
               OPEN axmr820_curs4
               FETCH axmr820_curs4 INTO sr1.occ01, sr1.occ02, sr1.occ18, sr1.occ61
              #IF STATUS THEN CALL cl_err('fetch',STATUS,1) END IF
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_azp01[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr820_prepare5 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr820_prepare5',STATUS,1) END IF
               DECLARE axmr820_curs5 CURSOR FOR axmr820_prepare5
               OPEN axmr820_curs5
               FETCH axmr820_curs5 INTO sr1.occg03, sr1.occg04
              #IF STATUS THEN CALL cl_err('fetch',STATUS,1) END IF
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_azp01[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr820_prepare6 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr820_prepare6',STATUS,1) END IF
               DECLARE axmr820_curs6 CURSOR FOR axmr820_prepare6
               OPEN axmr820_curs6
               FETCH axmr820_curs6 INTO sr1.pmc03
              #IF STATUS THEN CALL cl_err('fetch',STATUS,1) END IF
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_azp01[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr820_zo6 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr820_zo6',STATUS,1) END IF
               DECLARE axmr820_czo6 CURSOR FOR axmr820_zo6
               OPEN axmr820_czo6
               FETCH axmr820_czo6 INTO l_company
              #IF STATUS THEN CALL cl_err('fetch',STATUS,1) END IF
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
               CALL cl_parse_qry_sql(l_sql,l_azp01[i]) RETURNING l_sql    #FUN-A10056
               PREPARE axmr820_prepare9 FROM l_sql
               IF STATUS THEN CALL cl_err('axmr820_prepare9',STATUS,1) END IF
               DECLARE axmr820_curs9 CURSOR FOR axmr820_prepare9
               OPEN axmr820_curs9
               FETCH axmr820_curs9 INTO sr1.oao06
              #IF STATUS THEN CALL cl_err('fetch',STATUS,1) END IF
               IF sr1.oao06 IS NULL THEN LET sr1.oao06 = ' '  END IF
                  ...
                  #PREPARE axmr820_prepare11 FROM l_sql
                  #IF STATUS THEN CALL cl_err('axmr820_prepare11',STATUS,1) END IF
                  #DECLARE axmr820_curs11 CURSOR FOR axmr820_prepare11
                  #OPEN axmr820_curs11
                  #FETCH axmr820_curs11 INTO g_ocg.ocg48, g_ocg.ocg49,
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
                CALL cl_parse_qry_sql(l_sql,l_azp01[i]) RETURNING l_sql    #FUN-A10056
                PREPARE axmr820_pre14 FROM l_sql
                IF STATUS THEN CALL cl_err('axmr820_prepare4',STATUS,1) END IF
                DECLARE axmr820_curs14 CURSOR FOR axmr820_pre14
                OPEN axmr820_curs14
                FETCH axmr820_curs14 INTO sr1.occ02_2, sr1.occ18_2
               #IF STATUS THEN CALL cl_err('fetch',STATUS,1) END IF
...
#               CALL s_addrm(sr1.oea01,sr1.oea04,sr1.oea044,l_dbs[i])       #FUN-980020 mark
               ...
               #OUTPUT TO REPORT axmr820_rep(l_company,sr1.*,  #FUN-750012 mark
               ...
               EXECUTE insert_prep USING 
                       ...
                       t_azi.azi05 ,l_pmm22,"",l_img_blob,"N",""    #TQC-C10039 add "",l_img_blob,"N",""                                  #MOD-830175  
               #FUN-750012---add---end---
               END FOREACH 
         END FOR
     END FOREACH  
    ...
    #FINISH REPORT axmr310_rep
     ...
     CALL cl_prt_cs3('axmr820','axmr820',g_sql,g_str)
    #No.FUN-750012--add---end---
 
    #FINISH REPORT axmr820_rep                         #FUN-750012 mark
    #CALL cl_prt(l_name,g_prtway,g_copies,g_len)       #FUN-750012 mark
END FUNCTION
 
#FUN-750012---mark---str---
#REPORT axmr820_rep(sr1,l_oea,l_oed,l_occ02,l_occ18)
