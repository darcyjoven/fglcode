...
# Modify.........: No.FUN-B40087 11/06/07 By yangtt  憑證報表轉GRW 
# Modify.........: No.FUN-C10036 12/01/11 By qirl TQC-B90225追單 FUN-B80089追單
# Modify.........: No:FUN-C40026 12/04/11 By xumm GR動態簽核
# Modify.........: No:FUN-C50008 12/04/28 By wangrr GR程式優化
# Modify.........: No:CHI-C30024 12/06/07 By bart 增加第五地址欄位
# Modify.........: No:FUN-C30085 12/07/02 By lixiang 修改欄位顯示的BUG
...
TYPE sr1_t RECORD
    ...
    oqt151 LIKE oqt_file.oqt151,
    oqt152 LIKE oqt_file.oqt152,
    oqt161 LIKE oqt_file.oqt161,
    oqt162 LIKE oqt_file.oqt162,
    oqt171 LIKE oqt_file.oqt171,
    oqt172 LIKE oqt_file.oqt172,
    oqt173 LIKE oqt_file.oqt173,
    oqt181 LIKE oqt_file.oqt181,
    oqt182 LIKE oqt_file.oqt182,
    oqt191 LIKE oqt_file.oqt191,
    ...
    oac02_1 LIKE oac_file.oac02,   #FUN-C40026 add,
#FUN-C40026----add---str---
...
TYPE sr5_t RECORD
    oqv01 LIKE oqv_file.oqv01,
    oqv02 LIKE oqv_file.oqv02,
...
MAIN
  ...
  #No.FUN-710090--begin-- 
   LET g_sql="oqt12.oqt_file.oqt12,  oqt01.oqt_file.oqt01,",
            ...
            #"azi04.azi_file.azi04,  occ55.occ_file.occ55 ,",  #FUN-7C0036 mark  
             ...
             "oac02_1.oac_file.oac02, ",                         #NO.FUN-A80056   #FUN-C40026 add 2,
             #FUN-C40026----add---str----
   ...
   LET l_table = cl_prt_temptable('axmg360',g_sql) CLIPPED
   IF l_table = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time     #FUN-B40087   #FUN-C10036  mark                                                      
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5)         #FUN-B40087  #FUN-C10036  mark
      EXIT PROGRAM 
   END IF
  ...
  #l_table2   額外品名規格類別
   ...
   LET l_table1 = cl_prt_temptable('axmg3601',g_sql) CLIPPED
   IF l_table1 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time     #FUN-B40087  #FUN-C10036  mark                                                       
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5)         #FUN-B40087  #FUN-C10036  mark
      EXIT PROGRAM 
  ...
  #l_table2  費用資料
   ...
   LET l_table2 = cl_prt_temptable('axmg3602',g_sql) CLIPPED
   IF l_table2 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time     #FUN-B40087    #FUN-C10036  mark                                                     
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5)         #FUN-B40087  #FUN-C10036  mark
      EXIT PROGRAM 
  ...
  #l_table3  備註
   ...
   LET l_table3 = cl_prt_temptable('axmg3603',g_sql) CLIPPED   # 產生Temp Table
   IF l_table3 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time     #FUN-B40087    #FUN-C10036  mark                                                      
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5)         #FUN-B40087  #FUN-C10036  mark
      EXIT PROGRAM 
  ...
  #l_table4  分量計價
   ...
   LET l_table4 = cl_prt_temptable('axmg3604',g_sql) CLIPPED   # 產生Temp Table
   IF l_table4 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time     #FUN-B40087   #FUN-C10036  mark                                                      
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5)         #FUN-B40087 #FUN-C10036  mark
      EXIT PROGRAM 
  ...
  #MOD-820057-end-add
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN   # If background job sw is off
      CALL g360_tm(0,0)                        # Input print condition
   ELSE
      CALL g360()                              # Read data and create out-file
   END IF
...
DEFINE p_row,p_col       LIKE type_file.num5,         #No.FUN-680137 SMALLINT
   ...
   OPEN WINDOW g360_w AT p_row,p_col WITH FORM "axm/42f/axmg360"
   ...
   WHILE TRUE
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('axmg360',g_time,l_cmd)	# Execute cmd at later time
         END IF
...
DEFINE    l_img_blob LIKE    type_file.blob            #FUN-C40026 add
   ...
   LOCATE l_img_blob      IN MEMORY            #FUN-C40026 ad
 #No.FUN-710090--begin--
  #FUN-7C0073---mod---str---
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?)"  #FUN-7C0036 mark 1個? #FUN-7C0073 35->45個  #FUN-810029 44?->47?  #No.TQC-A50044 #NO.FUN-A80056Q     #FUN-C40026 add 4? #CHI-C30024 ?
  ...
  #公司地址zo041、公司電話zo05、公司傳真zo09
   ...
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='axmg360'
 ...
 #No.FUN-710090--end--
  ...
  #FUN-7C0073--mod---str---
   LET l_sql1 = " SELECT oqt12,",
                " oqt01 ,oqt03 ,oqt04,oqt041,oqt051,oqt052,oqt053,oqt054,oqt055,oqt13,",  #FUN-690032 add oqt04 #CHI-C30024 add oqt055
       ...
       #         " FROM oqt_file ",        
               #" FROM oqt_file LEFT OUTER JOIN oqu_file ON oqt01=oqu01 ", #FUN-C10036 ADD  #FUN-C50008 mark--在單頭對應單身時抓取的單頭重複
                " FROM oqt_file ",  #FUN-C50008 add
                " WHERE oqt12 = ? ",
   ...
   LET l_sql2  = " SELECT oqu01,oqu02,oqu03,oqu04,oqu031,oqu032,oqu06,oqu05,",   #MOD-A90107 add oqu04
                ...
                #" FROM  oqt_file,OUTER oqu_file",   #MOD-4A0256  #FUN-C50008 mark--
                 " FROM  oqt_file LEFT OUTER JOIN oqu_file ON (oqt01=oqu01) ",  #FUN-C50008 add  LEFT OUTER
                 " WHERE oqt_file.oqt01 = ? ",       #MOD-560215
                #"   AND oqt_file.oqt01 = oqu_file.oqu01 ",   #MOD-4A0256  #FUN-C50008 mark--
                 " ORDER BY oqu02"
 
   PREPARE g360_p1 FROM l_sql1
   IF SQLCA.sqlcode THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g360_c1 CURSOR FOR g360_p1
 
   PREPARE g360_p2 FROM l_sql2
   IF SQLCA.sqlcode THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g360_c2 CURSOR FOR g360_p2
  ...
  #FUN-C50008--add--str
   ...
   IF SQLCA.sqlcode THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g360_c7 CURSOR FOR g360_p7
 
  LET l_sql4 = "SELECT oqt01 FROM oqt_file ",
               " WHERE ",tm.wc CLIPPED,
               "   ORDER BY oqt01"
  PREPARE g360_p5 FROM l_sql4
  IF SQLCA.sqlcode != 0 THEN
     ...
     EXIT PROGRAM
  END IF
  DECLARE g360_c5 CURSOR FOR g360_p5

 #單頭費用資料
  DECLARE g360_c6 CURSOR FOR
     SELECT oqx01,oqx03,oqx04,oaj02,oqx041,oqx05,oqx06
       FROM oqx_file LEFT OUTER JOIN  oaj_file ON oqx04=oaj01
           WHERE oqx01 = ? AND oqx03 = '0' ORDER BY 1
   ...
   #單頭備註
    DECLARE oao_c2 CURSOR FOR
       SELECT * FROM oao_file
        WHERE oao01=? AND oao03='0'
          AND (oao05='1' OR oao05='2') ORDER BY oao04
  #FUN-C50008--add--end
 
  ## Modify Bug No.:7252
  #CALL cl_outnam('axmg360') RETURNING l_name  #No.FUN-710090 mark
 
   OPEN g360_c1 USING tm.b
   FOREACH g360_c1 USING tm.b INTO sr1.*
      IF SQLCA.sqlcode THEN
     ...
     #FUN-C50008--mark--end
      ...
      #-MOD-970263-add- 
      #FOREACH g360_c4 INTO sr3.*   #FUN-C50008 mark--
      FOREACH g360_c4 USING sr1.oqt01 INTO sr3.*  #FUN-C50008 add        
        IF SQLCA.sqlcode THEN
           EXIT FOREACH
        END IF
       #-MOD-970263-add- 
     ...
     #抓單頭與單身資料
      FOREACH g360_c2 USING sr1.oqt01 INTO sr2.*
         IF SQLCA.sqlcode THEN
        ...
        #額外品名規格 
         ...
         IF NOT cl_null(tm.a) AND sr2.oqu03[1,4] !='MISC'  THEN  #讀取額外品名規格
            ...
            FOREACH g360_c3 USING sr2.oqu03,tm.a INTO l_imc03,l_imc04[i]   #MOD-790032 modify
       ...
       #FUN-C50008--mark--end
         FOREACH oao_c1 USING sr1.oqt01,sr2.oqu02 INTO oao.*  #FUN-C50008 add                                               
         ...
         IF sr1.oqt12 = 'N' AND tm.b ='N' THEN    #N：一般
            ...
            EXECUTE insert1 USING sr1.oqt12 ,sr1.oqt01 ,sr1.oqt03 ,sr1.oqt04 ,sr1.oqt041,sr1.oqt051,
                                  ...
                                  #sr1.oqt27,l_oac02_1                                             #NO.FUN-A80056   #FUN-C40026 mark
                                  sr1.oqt27,l_oac02_1,"",l_img_blob,"N","",                    #FUN-C40026 add
         ...
         IF sr1.oqt12 = 'Y' AND tm.b ='Y' THEN    #Y:分量計價
           ...
           #FUN-C50008--mark--end
            LET i = 1
            FOREACH g360_c7 USING sr1.oqt01,sr2.oqu02 INTO l_oqv[i].*
 ...
 #FUN-C50008--mark--str   
   ...
   # DECLARE g360_c5 CURSOR FOR g360_p5
 #FUN-C50008--mark--end 
    FOREACH g360_c5 INTO sr1.oqt01
      #單頭費用資料
      #FUN-C50008--mark--str
      # DECLARE g360_c6 CURSOR FOR
      #    SELECT oqx01,oqx03,oqx04,oaj02,oqx041,oqx05,oqx06 
      ...
      # FOREACH g360_c6 INTO sr3.*
      #FUN-C50008--mark--end
      FOREACH g360_c6 USING sr1.oqt01 INTO sr3.* #FUN-C50008 add         
         IF SQLCA.sqlcode THEN
            EXIT FOREACH
         END IF
       #-MOD-970263-add- 
      ...
      #FUN-C50008--mark--end                                                         
       FOREACH oao_c2 USING sr1.oqt01 INTO oao.* #FUN-C50008 add                                              
          IF NOT cl_null(oao.oao06) THEN
             EXECUTE insert4 USING 
    ...
    LET g_cr_apr_key_f = "oqt01"             #FUN-C40026 add
  #FUN-7C0073---add---str--- 
###GENGRE###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,"|",
###GENGRE###               "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,"|",
###GENGRE###               "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,      #單身費用明細
###GENGRE###               " WHERE oqx03 != 0 ","|",
###GENGRE###               "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,"|",  #費用總計
###GENGRE###               "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,      #單頭費用明細
###GENGRE###               " WHERE oqx03 = 0 ","|",
...
###GENGRE###               "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
###GENGRE###               " WHERE oao03 =0 AND oao05='2'","|",                         #MOD-820057 modify add "|"
###GENGRE###               "SELECT * FROM ",g_cr_db_str CLIPPED,l_table4 CLIPPED,"|",   #MOD-820057 add #分量計價明細
###GENGRE###               "SELECT * FROM ",g_cr_db_str CLIPPED,l_table4 CLIPPED        #MOD-820057 add #分量計價明細
...
###GENGRE###       LET g_str = g_str,';',tm.d  
    END IF 
  # CALL cl_prt_cs3('axmg360',g_sql,g_str)
###GENGRE###    CALL cl_prt_cs3('axmg360','axmg360',g_sql,g_str)
    CALL axmg360_grdata()    ###GENGRE###
...
#   DECLARE g360_c3 CURSOR FOR g360_p3
#
#   PREPARE g360_p4 FROM l_sql4
#   IF SQLCA.sqlcode THEN
#      CALL cl_err('prepare:',SQLCA.sqlcode,1)
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
#      EXIT PROGRAM
#   END IF
#   DECLARE g360_c4 CURSOR FOR g360_p4
#
#   PREPARE g360_p5 FROM l_sql5
#   IF SQLCA.sqlcode THEN
#      CALL cl_err('prepare:',SQLCA.sqlcode,1)
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
#      EXIT PROGRAM
...
#   DECLARE g360_c5 CURSOR FOR g360_p5
#
#   PREPARE g360_p6 FROM l_sql6
#   IF SQLCA.sqlcode THEN
#      CALL cl_err('prepare:',SQLCA.sqlcode,1)
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
#      EXIT PROGRAM
#   END IF
#   DECLARE g360_c6 CURSOR FOR g360_p6
#
#   PREPARE g360_p7 FROM l_sql7
#   IF SQLCA.sqlcode THEN
#      CALL cl_err('prepare:',SQLCA.sqlcode,1)
#      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
#      EXIT PROGRAM
...
# #     #START REPORT g360_rep TO l_name  #FUN-690132
# #  ELSE
# #     LET g_zaa[38].zaa06='Y'
# #     #START REPORT g360_rep TO l_name   #FUN-690032
# #  END IF
...
#         LET l_oao03 = 0
#         LET l_oao05 = '1'
#         FOREACH g360_c3 USING sr1.oqt01,l_oao03,l_oao05
...
#      IF l_n > 0 THEN
#         LET l_oqx03_1=0
#         FOREACH g360_c6 USING sr1.oqt01,l_oqx03_1,l_oqx03_1 INTO sr3.*
#            IF SQLCA.sqlcode THEN
#               EXIT FOREACH
#            END IF
...
#        #   OUTPUT TO REPORT g360_rep(sr1.*,sr2.*,l_oqt01_t)
#        #   LET g_type= 'N'
#        #END IF
#
...
#            LET l_oao05='1'
#            LET l_oao06_2=''
#            FOREACH g360_c3 USING sr1.oqt01,l_oao03,l_oao05 INTO l_oao04,l_oao06
#               IF SQLCA.sqlcode THEN
#                  EXIT FOREACH
#               END IF
#               IF cl_null(l_oao06) THEN
...
#            LET i=1
#            
#            FOREACH g360_c4 USING sr2.oqu03,tm.a INTO l_imc04[i],l_imc03   #MOD-790032 modify
#              IF SQLCA.sqlcode THEN EXIT FOREACH END IF
...
#         END IF
#         LET i = 1
#         FOREACH g360_c5 USING sr1.oqt01,sr2.oqu02 INTO l_oqv[i].*
#            IF SQLCA.sqlcode THEN
#               EXIT FOREACH
#            END IF
#            LET i = i +1
...
#      LET l_oqx03_2=100
#      LET l_n=0
#      FOREACH g360_c6 USING sr1.oqt01,l_oqx03_1,l_oqx03_2 INTO sr3.*
#         IF SQLCA.sqlcode THEN
#            EXIT FOREACH
#         END IF
#         IF cl_null(sr3.oqx05_1) OR sr3.oqx05_1 = 0 THEN
...
#      LET l_oqx03_2=100
#      LET l_n=0
#      FOREACH g360_c7 USING sr1.oqt01,l_oqx03_1,l_oqx03_2 INTO sr4.*
#         IF SQLCA.sqlcode THEN
#            EXIT FOREACH
#         END IF
...
#    END IF 
#  # CALL cl_prt_cs3('axmg360',g_sql,g_str)
#    CALL cl_prt_cs3('axmg360','axmg360',g_sql,g_str)
#  #CALL cl_prt(l_name,g_prtway,g_copies,g_len)
#  #No.FUN-710090--end--
#
#END FUNCTION
...
##No.FUN-710090--begin--
#REPORT g360_rep(sr1,sr2,l_oqt01_t)
...
FUNCTION axmg360_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("axmg360")
        IF handler IS NOT NULL THEN
            START REPORT axmg360_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY oqt01"
          
            DECLARE axmg360_datacur1 CURSOR FROM l_sql
            FOREACH axmg360_datacur1 INTO sr1.*
                OUTPUT TO REPORT axmg360_rep(sr1.*)
            END FOREACH
            FINISH REPORT axmg360_rep
        END IF
...
REPORT axmg360_rep(sr1)
    ...
    #FUN-B40087------------add-------str------
    DEFINE l_oqt08_oqt11_oqt10     STRING
    DEFINE l_oqu11_oqu13           STRING
    DEFINE l_oqt25_oag02           STRING
    DEFINE l_sql                   STRING
    DEFINE l_oqu11                 STRING
    DEFINE l_oqu13                 STRING
    DEFINE l_oqx05_fmt             STRING
    DEFINE l_oqu06_fmt             STRING
    DEFINE l_oqu07_fmt             STRING
    DEFINE l_oqu09_fmt             STRING
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.oqt01
            LET l_lineno = 0
            #FUN-B40087------------add-------str------
            SELECT azi03 INTO l_azi03 FROM azi_file WHERE azi01 = sr1.oqt09
            PRINTX g_lang
            IF tm.d = "Y" AND (sr1.obk03 != " " OR NOT cl_null(sr1.obk03)) THEN
               LET l_display = "Y"
            ELSE 
               LET l_display = "N"
            END IF
            ...
            END IF
            PRINTX l_display1
            LET l_oqu06_fmt = cl_gr_numfmt('oqu_file','oqu06',sr1.azi04)
            PRINTX l_oqu06_fmt
            LET l_oqu07_fmt = cl_gr_numfmt('oqu_file','oqu07',l_azi03)
            PRINTX l_oqu07_fmt
            LET l_oqu09_fmt = cl_gr_numfmt('oqu_file','oqu09',sr1.azi04)
            PRINTX l_oqu09_fmt
            LET l_oqt08_oqt11_oqt10 = sr1.oqt10,' ',sr1.oqt08,'(',sr1.oqt11,')'
            PRINTX l_oqt08_oqt11_oqt10
            LET l_oqt25_oag02 = sr1.oqt25,sr1.oag02
            PRINTX l_oqt25_oag02

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        " WHERE oao01 = '",sr1.oqt01 CLIPPED,"'",
                        " AND oao05='1' AND oao03= 0"
            START REPORT axmg360_subrep05
            DECLARE axmg360_repcur5 CURSOR FROM l_sql
            FOREACH axmg360_repcur5 INTO sr4.*
                OUTPUT TO REPORT axmg360_subrep05(sr4.*)
            END FOREACH
            FINISH REPORT axmg360_subrep05

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED," A LEFT OUTER JOIN ",
                        " oaj_file B ON A.oqx04=B.oaj01",
                        " WHERE oqx01 = '",sr1.oqt01 CLIPPED,"'",
                        " AND oqx03='0'"
            START REPORT axmg360_subrep04
            DECLARE axmg360_repcur4 CURSOR FROM l_sql
            FOREACH axmg360_repcur4 INTO sr3.*
                OUTPUT TO REPORT axmg360_subrep04(sr3.*)
            END FOREACH
            FINISH REPORT axmg360_subrep04
            #FUN-B40087------------add-------end------
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B40087------------add-------str------
            LET l_oqu11 = sr1.oqu11 USING '-,---,--&.&&'
            LET l_oqu13 = sr1.oqu13 USING '-,---,--&.&&'
           #LET l_oqu11_oqu13 = l_oqu11,"'/",l_oqu13,"KG" #FUN-C30085 mark
            LET l_oqu11_oqu13 = l_oqu11,"/",l_oqu13,"KG"  #FUN-C30085 add
            PRINTX l_oqu11_oqu13

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        " WHERE oao01 = '",sr1.oqt01 CLIPPED,"'",
                        " AND oao05='1' AND oao03= ",sr1.oqu02 CLIPPED 
            START REPORT axmg360_subrep06
            DECLARE axmg360_repcur6 CURSOR FROM l_sql
            FOREACH axmg360_repcur6 INTO sr4.*
                OUTPUT TO REPORT axmg360_subrep06(sr4.*)
            END FOREACH
            FINISH REPORT axmg360_subrep06
      
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table4 CLIPPED,
                        " WHERE oqv01 = '",sr1.oqt01 CLIPPED,"'",
                        " AND oqv02= ",sr1.oqu02 CLIPPED
            START REPORT axmg360_subrep09
            DECLARE axmg360_repcur9 CURSOR FROM l_sql
            FOREACH axmg360_repcur9 INTO sr5.*
                OUTPUT TO REPORT axmg360_subrep09(sr5.*)
            END FOREACH
            FINISH REPORT axmg360_subrep09

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table4 CLIPPED,
                        " WHERE oqv01 = '",sr1.oqt01 CLIPPED,"'",
                        " AND oqv02= ",sr1.oqu02 CLIPPED
            START REPORT axmg360_subrep10
            DECLARE axmg360_repcur10 CURSOR FROM l_sql
            FOREACH axmg360_repcur10 INTO sr5.*
                OUTPUT TO REPORT axmg360_subrep10(sr5.*)
            END FOREACH
            FINISH REPORT axmg360_subrep10
        
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE imc01 = '",sr1.oqu03 CLIPPED,"'",
                        " AND imc02= '",tm.a CLIPPED,"'"
            START REPORT axmg360_subrep01
            DECLARE axmg360_repcur1 CURSOR FROM l_sql
            FOREACH axmg360_repcur1 INTO sr2.*
                OUTPUT TO REPORT axmg360_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT axmg360_subrep01

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        " WHERE oao01 = '",sr1.oqt01 CLIPPED,"'",
                        " AND oao05='2' AND oao03= ",sr1.oqu02 CLIPPED
            START REPORT axmg360_subrep07
            DECLARE axmg360_repcur7 CURSOR FROM l_sql
            FOREACH axmg360_repcur7 INTO sr4.*
                OUTPUT TO REPORT axmg360_subrep07(sr4.*)
            END FOREACH
            FINISH REPORT axmg360_subrep07
            #FUN-B40087------------add-------end------
        ...
        AFTER GROUP OF sr1.oqt01
            ...
            START REPORT axmg360_subrep08
            DECLARE axmg360_repcur8 CURSOR FROM l_sql
            FOREACH axmg360_repcur8 INTO sr4.*
                OUTPUT TO REPORT axmg360_subrep08(sr4.*)
            END FOREACH
            FINISH REPORT axmg360_subrep08

            LET l_sql = "SELECT A.*,B.* FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED," A LEFT OUTER JOIN ",
                        " oaj_file B ON A.oqx04=B.oaj01",
                        " WHERE oqx01 = '",sr1.oqt01 CLIPPED,"'",
                        " AND oqx03<>'0'"
            START REPORT axmg360_subrep02
            DECLARE axmg360_repcur2 CURSOR FROM l_sql
            FOREACH axmg360_repcur2 INTO sr3.*
                OUTPUT TO REPORT axmg360_subrep02(sr3.*)
            END FOREACH
            FINISH REPORT axmg360_subrep02
 
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED," A LEFT OUTER JOIN ",
                        " oaj_file B ON A.oqx04=B.oaj01",
                        " WHERE oqx01 = '",sr1.oqt01 CLIPPED,"'",
                        " AND oqx03<>'0'"
            START REPORT axmg360_subrep03
            DECLARE axmg360_repcur3 CURSOR FROM l_sql
            FOREACH axmg360_repcur3 INTO sr3.*
                OUTPUT TO REPORT axmg360_subrep03(sr3.*)
            END FOREACH
            FINISH REPORT axmg360_subrep03
            #FUN-B40087------------add-------end------
...
REPORT axmg360_subrep01(sr2)
    ...
    FORMAT
        ON EVERY ROW
            LET l_oqu06_fmt = cl_gr_numfmt('oqu_file','oqu06',sr1.azi04) 
            PRINTX l_oqu06_fmt
            LET l_oqu07_fmt = cl_gr_numfmt('oqu_file','oqu07',sr1.azi04)
            PRINTX l_oqu07_fmt
            LET l_oqu09_fmt = cl_gr_numfmt('oqu_file','oqu09',sr1.azi04)
            PRINTX l_oqu09_fmt
            PRINTX sr2.*
...
REPORT axmg360_subrep02(sr3)
    ...
    FORMAT
        ON EVERY ROW
            LET l_oqx05_fmt = cl_gr_numfmt('oqx_file','oqx05',sr3.azi04)
            PRINTX l_oqx05_fmt
            PRINTX sr3.*
END REPORT

REPORT axmg360_subrep03(sr3)
    DEFINE sr3 sr3_t
    DEFINE l_oqx05_sum_fmt  STRING
    DEFINE l_oqx05_sum  LIKE oqx_file.oqx05

    ORDER EXTERNAL BY sr3.oqx01

    FORMAT
        ON EVERY ROW
            LET l_oqx05_sum_fmt = cl_gr_numfmt('oqx_file','oqx05',sr3.azi04)
            PRINTX l_oqx05_sum_fmt
            PRINTX sr3.*

        AFTER GROUP OF sr3.oqx01
           LET l_oqx05_sum = GROUP SUM(sr3.oqx05)
           PRINTX l_oqx05_sum
END REPORT

REPORT axmg360_subrep04(sr3)
    DEFINE sr3 sr3_t
    DEFINE l_oqx05_fmt  STRING
