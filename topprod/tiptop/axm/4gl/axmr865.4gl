...
#
# Pattern name...: axmr865.4gl
# Descriptions...: 品牌同期績效分析表
...
MAIN
   ...
   LET g_sql =  "ima1005.ima_file.ima1005,",
   ...
   LET l_table = cl_prt_temptable('axmr865',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL axmr865_tm(0,0)        # Input print condition
      ELSE CALL axmr865()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time 
END MAIN

FUNCTION axmr865_tm(p_row,p_col)
   ...
   OPEN WINDOW axmr865_w AT p_row,p_col WITH FORM "axm/42f/axmr865" 
   ...
   WHILE TRUE
     ...
     CONSTRUCT BY NAME tm.wc ON ima131,ima1005
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW axmr865_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                       ...
                       " '",g_template CLIPPED,"'",           
                       " '",g_rpt_name CLIPPED,"'"            
            CALL cl_cmdat('axmr865',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW axmr865_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time 
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
FUNCTION axmr865_1(l_plant,l_ima01,l_ccc08,l_oga02)          #抓取金額和成本
...
FUNCTION axmr865()
   ...
   DROP TABLE axmr865_tmp
   CREATE TEMP TABLE axmr865_tmp(
          ima1005    LIKE ima_file.ima1005,
          oga02      LIKE oga_file.oga02,
   ...
   DELETE FROM axmr865_tmp
   
   #FUN-AA0024 add --------------------------------begin----------------------------------
    DROP TABLE axmr865_tmp1
    CREATE TEMP TABLE axmr865_tmp1(
          ima1005    LIKE ima_file.ima1005,
          oga02      LIKE oga_file.oga02,
          tqa02      LIKE tqa_file.tqa02,
          ohb14t     LIKE ohb_file.ohb14t,
    ...
    IF cl_null(l_rtz04) THEN              
       LET l_sql = " SELECT ima1005,ogb01,oga02,ogb03,ogb09,ogb41,ogb092,ima01,0,0,ogb12,0,0",
    ...
    CALL cl_replace_sqldb(l_sql) RETURNING l_sql        
    CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql               
    PREPARE axmr865_prepare1 FROM l_sql
    ...
    DECLARE axmr865_curs1 CURSOR FOR axmr865_prepare1
    FOREACH axmr865_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
    ...
    IF tm.type02 = '1' THEN 
         LET l_sql = " SELECT SUM(ogb14t) FROM ",cl_get_target_table(l_plant,'ogb_file'),",",
                     ...
                     "    AND oga01 = ogb01 ", 
                     "    AND ogaconf = 'Y' ", 
                     "    AND (ima1005 = '",sr.ima1005,"'"," OR ima1005 IS NULL)",
    ...
    CASE tm.type01                     #根據不同類型抓取銷售成本
       WHEN '1'           
         CALL axmr865_1(l_plant,sr.ima01,'',sr.oga02) 
            RETURNING sr.ccc23
       ...
       WHEN '2' 
         ...
         LET sr.ccc23 = sr.ccc23*sr.ogb12                 #計算銷售成本
       WHEN '3'     
          CALL axmr865_1(l_plant,sr.ima01,sr.ogb092,sr.oga02) 
             RETURNING sr.ccc23   
             LET sr.ccc23 = sr.ccc23*sr.ogb12             #計算銷售成本
       WHEN '4'     
          CALL axmr865_1(l_plant,sr.ima01,sr.ogb41,sr.oga02) 
             RETURNING sr.ccc23 
             LET sr.ccc23 = sr.ccc23*sr.ogb12             #計算銷售成本
       WHEN '5'     
          ...
          LET sr.ccc23 = sr.ccc23*sr.ogb12                  #計算銷售成本
       END CASE
      INSERT INTO axmr865_tmp 
         VALUES(sr.ima1005,sr.oga02,l_tqa02,sr.ogb14t_2,sr.ccc23,sr.ogb12,sr.ogb14t_2,sr.ogb14t_3)
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
...
#FUN-AA0024 add --------------------------------begin----------------------------------
IF cl_null(l_rtz04) THEN              
       LET l_sql = " SELECT ima1005,ohb01,oha02,ohb03,ohb09,ohb092,ima01,0,0,ohb12,0,0",
    ...
    CALL cl_replace_sqldb(l_sql) RETURNING l_sql        
    CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql               
    PREPARE axmr865_prepare11 FROM l_sql
    IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time 
       EXIT PROGRAM
    ...
    DECLARE axmr865_curs11 CURSOR FOR axmr865_prepare11
    FOREACH axmr865_curs11 INTO sr1.*
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
    ...
    IF tm.type02 = '1' THEN 
         LET l_sql = " SELECT SUM(ohb14t) FROM ",cl_get_target_table(l_plant,'ohb_file'),",",
                     ...
                     "    AND oha01 = ohb01 ", 
                     "    AND ohaconf = 'Y' ", 
                     "    AND (ima1005 = '",sr1.ima1005,"'"," OR ima1005 IS NULL)",
    ...
    CASE tm.type01                     #根據不同類型抓取銷售成本
       WHEN '1'           
         CALL axmr865_1(l_plant,sr1.ima01,'',sr1.oha02) 
            RETURNING sr1.ccc23
       ...
       WHEN '2' 
         ...
         LET sr1.ccc23 = sr1.ccc23*sr1.ohb12                 #計算銷售成本
       WHEN '3'     
          CALL axmr865_1(l_plant,sr1.ima01,sr1.ohb092,sr1.oha02) 
             RETURNING sr1.ccc23   
             LET sr1.ccc23 = sr1.ccc23*sr1.ohb12             #計算銷售成本
       WHEN '4'     
          CALL axmr865_1(l_plant,sr1.ima01,'',sr1.oha02) 
       ...
       WHEN '5'     
          ...
          LET sr1.ccc23 = sr1.ccc23*sr1.ohb12                  #計算銷售成本
       END CASE
      INSERT INTO axmr865_tmp1 
         VALUES(sr1.ima1005,sr1.oha02,l_tqa02,sr1.ohb14t_2,sr1.ccc23,sr1.ohb12,sr1.ohb14t_2,sr1.ohb14t_3)
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
...
#FUN-AA0024 add --------------------------------end------------------------------------
   END FOREACH
      IF tm.type02 = '1' THEN
      LET l_sql = " SELECT a.ima1005,'',0,",
                  "SUM(a.ccc23)-(SELECT CASE WHEN SUM(b.ccc23) IS NULL THEN 0 ELSE SUM(b.ccc23) END FROM axmr865_tmp1 b WHERE (a.ima1005=b.ima1005 ) OR (b.ima1005 IS NULL AND a.ima1005 IS NULL)),",
                  "SUM(ogb12),",
                  "SUM(a.ogb14t_2)-(SELECT CASE WHEN SUM(b.ohb14t_2) IS NULL THEN 0 ELSE SUM(b.ohb14t_2) END FROM axmr865_tmp1 b WHERE (a.ima1005=b.ima1005 ) OR (b.ima1005 IS NULL AND a.ima1005 IS NULL)),'' ",
                  "   FROM axmr865_tmp a WHERE YEAR(oga02) = '",tm.yy01,"'",
                  " GROUP BY a.ima1005"
      ELSE
      LET l_sql = " SELECT a.ima1005,'',0,",
                  "SUM(a.ccc23)-(SELECT CASE WHEN SUM(b.ccc23) IS NULL THEN 0 ELSE SUM(b.ccc23) END FROM axmr865_tmp1 b WHERE (a.ima1005=b.ima1005 ) OR (b.ima1005 IS NULL AND a.ima1005 IS NULL)),",
                  ...
                  " GROUP BY a.ima1005"
      END IF             
      PREPARE axmr865_prepare2 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1) 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      EXIT PROGRAM
   END IF
   DECLARE axmr865_curs2 CURSOR FOR axmr865_prepare2
   
   FOREACH axmr865_curs2 INTO sr.ima1005,l_tqa02,sr.ogb14t_2,
                              sr.ccc23,sr.ogb12,sr.ogb14t_2,sr.ogb14t_3 
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF 
      LET l_sql = " SELECT tqa02 FROM ",cl_get_target_table(l_plant,'tqa_file'), 
                  "  WHERE tqa01 = '",sr.ima1005,"'"
      CALL cl_replace_sqldb(l_sql) RETURNING l_sql
      CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql
      PREPARE sel_tqa02_pre1 FROM l_sql
      EXECUTE sel_tqa02_pre1 INTO l_tqa02
      
      IF tm.type02 = '1' THEN
         IF cl_null(sr.ima1005) THEN 
            SELECT SUM(ogb14t_3) INTO sr.ogb14t_3 FROM axmr865_tmp WHERE YEAR(oga02)=tm.yy02 AND ima1005 IS NULL
         ELSE 
            SELECT SUM(ogb14t_3) INTO sr.ogb14t_3 FROM axmr865_tmp WHERE YEAR(oga02)=tm.yy02 AND ima1005=sr.ima1005   
         END IF   
      ELSE
         IF cl_null(sr.ima1005) THEN
            SELECT SUM(ogb14t_3) INTO sr.ogb14t_3 FROM axmr865_tmp WHERE YEAR(oga02)=tm.yy02 AND MONTH(oga02)=tm.mm02 
            AND ima1005 IS NULL
         ELSE 
            SELECT SUM(ogb14t_3) INTO sr.ogb14t_3 FROM axmr865_tmp WHERE YEAR(oga02)=tm.yy02 AND MONTH(oga02)=tm.mm02
            AND ima1005=sr.ima1005 
         END IF    
      END IF          
      EXECUTE  insert_prep  USING  
      sr.ima1005,l_tqa02,sr.ogb14t_2,
      sr.ccc23,sr.ogb12,sr.ogb14t_2,sr.ogb14t_3 
   END FOREACH
   LET l_sql = " SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
   LET g_str = tm.wc1," and ",tm.wc
         CALL cl_prt_cs3('axmr865','axmr865',l_sql,g_str)  
END FUNCTION
#FUN-A70025
