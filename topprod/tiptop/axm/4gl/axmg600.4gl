...
# Pattern name...: axmg600.4gl
# Descriptions...: 出貨單
# Date & Author..: 95/01/17 by Nick
# Modify.........: 99/06/21 By Carol:add 備註列印   g600_oao()
...
# Modify.........: No.FUN-B50018 11/05/11 By mei  CR 轉GRW
# Modify.........: No.FUN-B50018 11/08/11 By xumm 程式規範修改
# Modify.........: No.FUN-C10036 12/01/11 By qirl FUN-B80089,MOD-B90173追單
# Modify.........: No.FUN-C40019 12/04/11 By yangtt GR報表列印TIPTOP與EasyFlow簽核圖片
# Modify.........: No:FUN-C50003 12/05/14 By yangtt GR程式優化
# Modify.........: No:FUN-C30085 12/07/03 By chenying GR修改-l_str3在給值前先清空
...
#               "zo09.zo_file.zo09"                                #FUN-810029 add  #No.FUN-860026
   ...
   LET l_table = cl_prt_temptable('axmg600',g_sql) CLIPPED
   IF l_table = -1 THEN 
...
#  LET g_sql ="ogc01.ogc_file.ogc01,",   #FUN-B50018  mark
   LET g_sql ="ogc01.ogb_file.ogb04,",   #FUN-B50018
              "ogb03.ogb_file.ogb03,",   #CHI-7A0011 add
              "i1.type_file.num5,",
              "loc.type_file.chr37,",
              "ogc16.ogc_file.ogc16"
   LET l_table1 = cl_prt_temptable('axmg6001',g_sql) CLIPPED
   IF l_table1 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add  #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5)   #FUN-B50018  add  #FUN-C10036   mark
      EXIT PROGRAM
   ...
   LET l_table2 = cl_prt_temptable('axmg6002',g_sql) CLIPPED
   IF l_table2 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add  #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5)   #FUN-B50018  add  #FUN-C10036   mark
      EXIT PROGRAM
  ...
  #str CHI-7A0011 add
   ...
   LET l_table3 = cl_prt_temptable('axmg6003',g_sql) CLIPPED
   IF l_table3 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add  #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5)   #FUN-B50018  add  #FUN-C10036   mark
      EXIT PROGRAM
...
#No.FUN-860026---begin                                                                                                              
   ...
   LET l_table4 = cl_prt_temptable('axmg6004',g_sql) CLIPPED                                                                        
   IF  l_table4 = -1 THEN 
       #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add  #FUN-C10036   mark
       #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5)   #FUN-B50018  add  #FUN-C10036   mark
       EXIT PROGRAM 
...
#No.FUN-860026---end  
   ...
   LET g_sql = "imc01.imc_file.imc01,",
    ...
    LET l_table5 = cl_prt_temptable('axmg6005',g_sql) CLIPPED
    IF  l_table5 = -1 THEN 
        #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add  #FUN-C10036   mark
        #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4||"|"||l_table5)   #FUN-B50018  add  #FUN-C10036   mark
        EXIT PROGRAM 
   ...
   IF cl_null(tm.wc) THEN
      CALL axmg600_tm(0,0)             # Input print condition
   ELSE
     #LET tm.wc="oga01 ='",tm.wc CLIPPED,"' " CLIPPED    #No.TQC-610089 mark
      LET tm.a = "1"    #No.MOD-580084
      CALL axmg600()                   # Read data and create out-file
   END IF
...
FUNCTION axmg600_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW axmg600_w AT p_row,p_col WITH FORM "axm/42f/axmg600"
...
#No.FUN-860026---END   
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('axmg600',g_time,l_cmd)    # Execute cmd at later time
         END IF
...
#No.FUN-860026---end      
  ...
  #公司全名zo02、公司地址zo041、公司電話zo05、公司傳真zo09
   ...
   #MOD-540174..................begin
#  DECLARE axmg600_za_cur CURSOR FOR
#          SELECT za02,za05 FROM za_file
#           WHERE za01 = "axmg600" AND za03 = g_rlang
#  FOREACH axmg600_za_cur INTO g_i,l_za05
#     LET g_x[g_i] = l_za05
#  END FOREACH
   #MOD-540174..................end
#FUN-710081--start
#   SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'axmg600' 
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'axmg600'  
 
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " values(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
#               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?)"  #FUN-720014 add 2欄位   #FUN-810029 32?->35?   #No.FUN-860026 
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?)"   #No.FUN-860026   #MOD-8A0255 37->38  #FUN-C40019 add 4?
...
##No.FUN-B10026 ---end---
   ...
   LET l_sql="SELECT oga01,oaydesc,oga02,oga021,oga011,oga14,oga15,oga16,",
             ...
             "       ogb17,ogb19,ogb910,ogb912,ogb913,ogb915,ogb916,ima18,ogb07",  #No.FUN-580004 #TQC-5B0127 add ogb916 AND FUN-5C0075  #FUN-A80083 add ogb07  
              " FROM oga_file LEFT OUTER JOIN oay_file ON oga01 LIKE ltrim(rtrim(oay_file.oayslip)) || '-%' ",     #FUN-C50003 oga_file.oga01-->oga01
                            " LEFT OUTER JOIN occ_file ON oga04 = occ01,ogb_file ",   #FUN-C50003 oga_file.oga04,occ_file.occ01-->oga04,occ01
                            " LEFT OUTER JOIN ima_file ON ogb04 = ima01 ",      #FUN-C50003 ogb_file.ogb04,ima_file.ima01-->ogb04,ima01
             " WHERE oga01 = ogb01 ", 
#No.FUN-550070--end
...
#No.FUN-B10026 ---end---
   ...
   PREPARE axmg600_prepare1 FROM l_sql
...
#FUN-710081--start
   DECLARE axmg600_curs1 CURSOR FOR axmg600_prepare1
 
#   CALL cl_outnam('axmg600') RETURNING l_name
...
#   START REPORT axmg600_rep TO l_name
  ...
  #FUN-C50003------mark---end---
   ...
   FOREACH axmg600_curs1 INTO sr.*
     ...
     IF g_oaz.oaz141 = "1" THEN
        ...
        IF g600_err_ana(g_showmsg) THEN
        END IF
        IF g_errno = 'N' THEN
...
#TQC-5B0127--add
     ...
     #str CHI-7A0011 mod
      ...
      #FUN-C50003---mark---end--
        FOREACH oao_c2 USING sr.oga01,sr.ogb03 INTO oao.*     #FUN-C50003 add USING sr.oga01,sr.ogb03
...
#TQC-5B0127--end
      ...
      PREPARE g600_p2 FROM l_sql
      DECLARE g600_c2 CURSOR FOR g600_p2
      LET i=1
      FOREACH g600_c2 INTO l_ogc.*
...
#No.FUN-5C0075--begin
      ...
      IF l_oaz23 = 'Y'  AND tm.c = 'Y' THEN
        ...
        #FUN-C50003----mark-----end---
         LET i = 1
         FOREACH ogc_cs USING sr.oga01 INTO g_ogc.*    #FUN-C50003 add USING sr.oga01
...
#No.FUN-860026---begin                                                                                                              
   ...
   #              ORDER BY  rvbs04                                                                                                  
   #FUN-C50003----mark-----str---
    FOREACH  r920_c USING sr.oga01,sr.ogb03 INTO l_rvbs.*       #FUN-C50003 add USING sr.oga01,sr.ogb03              
         LET flag = 1                                                                                
    ...
    END FOREACH                                                                                                                     
#No.FUN-860026---end    
#      OUTPUT TO REPORT axmg600_rep(sr.*)
      ...
      IF tm.f = 'Y' THEN
          ...
          IF l_count !=0  THEN
           ...
           #ORDER BY imc03                                        
           #FUN-C50003------mark---end---
            FOREACH imc_cur USING sr.ogb04,sr.ogb07 INTO sr1.*        #FUN-C50003 add USING sr.ogb04,sr.ogb07                    
  ...
  #str CHI-7A0010 add
   #列印整張備註
      LET l_sql = "SELECT oga01 FROM oga_file ",
                  " WHERE ",tm.wc CLIPPED,
                  "   ORDER BY oga01"
      PREPARE g600_prepare2 FROM l_sql
      IF SQLCA.sqlcode != 0 THEN
         ...
         EXIT PROGRAM
      END IF
      DECLARE g600_cs2 CURSOR FOR g600_prepare2
     ...
     #FUN-C50003----add----end--
      ...
      FOREACH g600_cs2 INTO sr.oga01
        #FUN-C50003----mark---str--
        #DECLARE oao_c1 CURSOR FOR
        # SELECT * FROM oao_file
        #  WHERE oao01=sr.oga01 AND oao03=0 AND (oao05='1' OR oao05='2')
        #FUN-C50003----mark---end--
         FOREACH oao_c1 USING sr.oga01 INTO oao.*    #FUN-C50003 add USING sr.oga01
            IF NOT cl_null(oao.oao06) THEN
               EXECUTE insert_prep3 USING
                  sr.oga01,oao.oao03,oao.oao04,oao.oao05,oao.oao06
            END IF
...
#      LET l_str = l_str CLIPPED,";",tm.wc  #No.FUN-860026
   ...
   #p4~p8
###GENGRE###   LET l_str = l_str CLIPPED,";",l_str1,";",tm.c,";",tm.d,
###GENGRE###                             ";",l_oaz23 CLIPPED,";",l_oga09,
###GENGRE###                             ";",tm.a,";",tm.e,";",tm.f   #CHI-7A0011 add  #No.FUN-860026 add tm.e #FUN-A80083 add tm.f
###GENGRE###                             ,";",g_oaz.oaz141 #CHI-A90032 add
   #-----MOD-8A0255---------
   #LET g_msg=NULL
   #IF g_oaz.oaz141 = "1" THEN
...
#No.FUN-860026---end
 ...
 # CALL cl_prt_cs3('axmg600',l_sql,l_str)        #TQC-730088   
###GENGRE###   CALL cl_prt_cs3('axmg600','axmg600',l_sql,l_str)    
    LET g_cr_table = l_table                   #主報表的temp table名稱          #FUN-C40019 add
    LET g_cr_apr_key_f = "oga01"               #報表主鍵欄位名稱，用"|"隔開     #FUN-C40019 add
    CALL axmg600_grdata()    ###GENGRE###
 
#   FINISH REPORT axmg600_rep
#FUN-710081--end
      ...
      ON EVERY ROW
        ...
        #MOD-680081-end-add
         ...
         IF tm.b = 'Y'  THEN     #列印備註於單身前
            CALL g600_oao(sr.oga01,sr.ogb03,'1')
            FOR l_n = 1 TO 40
               IF NOT cl_null(g_m[l_n]) THEN
                  PRINT g_m[l_n]  CLIPPED
               ELSE
...
#no.FUN-580004--end
         ...
         IF l_oga09 = '8' THEN
            SELECT ogb_file.* INTO l_ogb.* FROM oga_file,ogb_file
             WHERE oga01 = ogb01 AND oga011 = sr.oga011
               AND ogb03 = sr.ogb03 AND oga09 = '9'
        #   LET l_str3 = ''       #FUN-B50018  #FUN-C30085
            IF SQLCA.sqlcode = 0 THEN
               IF g_sma115 = "Y" THEN
                  CASE l_ima906
                     WHEN "2"
...
#no.FUN-570176--end--
         IF tm.b = 'Y'  THEN     #列印備註於單身後
            CALL g600_oao(sr.oga01,sr.ogb03,'2')
            FOR l_n = 1 TO 40
               IF NOT cl_null(g_m[l_n]) THEN
                  PRINT g_m[l_n] CLIPPED
               ELSE
...
FUNCTION g600_oao(l_p1,l_p3,l_p5)
   ...
   DECLARE g600_c5 CURSOR FOR SELECT oao06 FROM oao_file
                               WHERE oao01 = l_p1
                                 AND oao03 = l_p3
                                 AND oao05 = l_p5
   ...
   FOREACH g600_c5 INTO g_m[l_n]
      LET l_n = l_n + 1
   END FOREACH
 
END FUNCTION
 
FUNCTION g600_err_ana(ls_showmsg)    #FUN-650020
...
FUNCTION axmg600_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("axmg600")
        IF handler IS NOT NULL THEN
            START REPORT axmg600_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY oga01,ogb03"
            DECLARE axmg600_datacur1 CURSOR FROM l_sql
            FOREACH axmg600_datacur1 INTO sr1.*
                OUTPUT TO REPORT axmg600_rep(sr1.*)
            END FOREACH
            FINISH REPORT axmg600_rep
        END IF
...
REPORT axmg600_rep(sr1)
    ...
    #FUN-B50018----add-----str-----------------
    DEFINE l_flag         LIKE oga_file.oga01
    DEFINE l_flag1        LIKE oga_file.oga01
    DEFINE l_message      STRING
    DEFINE l_message1     STRING
    DEFINE l_oga01_ogb04  LIKE  ogb_file.ogb04
    DEFINE l_titlenew     STRING
    DEFINE l_weight_sum   LIKE ogb_file.ogb12
    ...
    #FUN-B50018----add-----end-----------------
    DEFINE l_display7     LIKE type_file.chr1 #DEV-D40005
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
            #FUN-B50018----add-----str-----------------              
            LET l_display = g_oaz.oaz141
            PRINTX l_display
            SELECT oga09 INTO l_oga09 FROM oga_file WHERE oga01=sr1.oga01
            PRINTX l_oga09
            IF  NOT cl_null(l_oaz23) THEN 
                LET l_titlenew = cl_gr_getmsg("gre-003",g_lang,l_oaz23)
            ELSE
                LET l_titlenew = cl_gr_getmsg("gre-003",g_lang,'0')
            END IF  
            PRINTX   l_titlenew
            #FUN-B50018----add-----end-----------------
        BEFORE GROUP OF sr1.oga01
            ...
            START REPORT axmg600_subrep03
            DECLARE axmg600_repcur3 CURSOR FROM l_sql
            FOREACH axmg600_repcur3 INTO sr4.*
                OUTPUT TO REPORT axmg600_subrep03(sr4.*)
            END FOREACH
            FINISH REPORT axmg600_subrep03

            IF g_oaz.oaz141 = '0' THEN 
               LET l_display1 = 'N'
            ELSE
               LET l_display1 = 'Y'
            END IF
            ...
            IF g_oaz.oaz141 = 'Y' AND sr1.l_count > 0  THEN
               LET l_display3 = 'Y'
            ELSE
               LET l_display3 = 'N'
            END IF
        ...
        ON EVERY ROW
            ...
            PRINTX l_lineno
            #FUN-B50018----add-----str-----------------
            IF sr1.flag > sr1.oga01  THEN
               LET  l_flag = sr1.flag
            ELSE
               LET  l_flag = sr1.oga01
            END IF
            PRINTX  l_flag
     
            IF sr1.flag1 > sr1.oga01  THEN
               LET  l_flag1 = sr1.flag
            ELSE
               LET  l_flag1 = sr1.oga01
            END IF
            PRINTX  l_flag1

            IF tm.a = '1' THEN 
               LET l_oga01_ogb04 = sr1.oga01
            ELSE 
               LET l_oga01_ogb04 = sr1.ogb04
            END IF
            PRINTX  l_oga01_ogb04
      

            IF cl_null(sr1.note) THEN
               LET l_display5 = 'N'
            ELSE
               LET l_display5 = 'Y'
            END IF
            ...
            EXECUTE l_prep1 INTO l_flag2
            PRINTX l_flag2

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        " WHERE oao01 = '",sr1.oga01 CLIPPED,"'",
                        " AND oao03 = ",sr1.ogb03 CLIPPED,        
                        " AND oao05='1'"                        
            START REPORT axmg600_subrep04
            DECLARE axmg600_repcur4 CURSOR FROM l_sql
            FOREACH axmg600_repcur4 INTO sr4.*
                OUTPUT TO REPORT axmg600_subrep04(sr4.*)
            END FOREACH
            FINISH REPORT axmg600_subrep04
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table5 CLIPPED,
                        ...
                        " AND oga01 = '",sr1.oga01 ,"'",
                        " AND ogb03 = ",sr1.ogb03                      
            START REPORT axmg600_subrep08
            DECLARE axmg600_subrep08 CURSOR FROM l_sql
            FOREACH axmg600_subrep08 INTO sr6.*
                OUTPUT TO REPORT axmg600_subrep08 (sr6.*)
            END FOREACH
            FINISH REPORT axmg600_subrep08
            ...
            ELSE
               LET l_sql = "SELECT DISTINCT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                           " WHERE ogc01 = '",sr1.ogb04 CLIPPED,"'",
                           "   AND ogb03 = ",sr1.ogb03,
                           " ORDER BY i1"
            END IF       
            START REPORT axmg600_subrep01
            DECLARE axmg600_subrep01 CURSOR FROM l_sql
            FOREACH axmg600_subrep01 INTO sr2.*
                OUTPUT TO REPORT axmg600_subrep01 (sr2.*)
            END FOREACH
            FINISH REPORT axmg600_subrep01

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,
                        " WHERE ogc01 = '",sr1.oga01 CLIPPED,"'"     
            START REPORT axmg600_subrep02
            DECLARE axmg600_subrep02 CURSOR FROM l_sql
            FOREACH axmg600_subrep02 INTO sr3.*
                OUTPUT TO REPORT axmg600_subrep02 (sr3.*)
            END FOREACH
            FINISH REPORT axmg600_subrep02
            #FUN-B50018----add-----end-----------------
            PRINTX sr1.*
        ...
        AFTER GROUP OF sr1.oga01
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        ...
                        " AND oao03 = ",sr1.ogb03 CLIPPED,        
                        " AND oao05='2'"
            START REPORT axmg600_subrep05
            DECLARE axmg600_repcur5 CURSOR FROM l_sql
            FOREACH axmg600_repcur5 INTO sr4.*
                OUTPUT TO REPORT axmg600_subrep05(sr4.*)
            END FOREACH
            FINISH REPORT axmg600_subrep05
            
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                        " WHERE oao01 = '",sr1.oga01 CLIPPED,"'",  
                        " AND oao03 = 0 AND oao05='2'"
            START REPORT axmg600_subrep06
            DECLARE axmg600_repcur6 CURSOR FROM l_sql
            FOREACH axmg600_repcur6 INTO sr4.*

                OUTPUT TO REPORT axmg600_subrep06(sr4.*)
            END FOREACH
            FINISH REPORT axmg600_subrep06

            
            LET l_weight_sum = GROUP SUM(sr1.weight)
            PRINTX l_weight_sum
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table4 CLIPPED,
                        " WHERE rvbs01 = '",sr1.oga01 CLIPPED,"' AND rvbs02 = ",sr1.ogb03
            START REPORT axmg600_subrep07
            DECLARE axmg600_subrep07 CURSOR FROM l_sql
            FOREACH axmg600_subrep07 INTO sr5.*
                OUTPUT TO REPORT axmg600_subrep07 (sr5.*)
            END FOREACH
            FINISH REPORT axmg600_subrep07
...
REPORT axmg600_subrep01(sr2)
    DEFINE sr2 sr2_t

    ORDER EXTERNAL BY sr2.i1
...
REPORT axmg600_subrep02(sr3)
    DEFINE sr3 sr3_t
    
    ORDER EXTERNAL BY sr3.ogc01
 
    FORMAT
        BEFORE GROUP OF sr3.ogc01
 
        ON EVERY ROW
            PRINTX sr3.*
END REPORT

REPORT axmg600_subrep03(sr4)
    DEFINE sr4 sr4_t
...
REPORT axmg600_subrep07(sr5)
    DEFINE sr5 sr5_t
    DEFINE l_rvbs06_sum     LIKE rvbs_file.rvbs06
  
    ORDER EXTERNAL BY sr5.rvbs01

    FORMAT
        ON EVERY ROW
            PRINTX sr5.*

        AFTER GROUP OF sr5.rvbs01
            LET l_rvbs06_sum  = GROUP SUM(sr5.rvbs06)
            PRINTX l_rvbs06_sum

END REPORT

REPORT axmg600_subrep08(sr6)
    DEFINE sr6 sr6_t
