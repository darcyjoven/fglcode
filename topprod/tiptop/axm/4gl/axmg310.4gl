...
# Modify.........: No.FUN-C50008 12/04/27 By wangrr GR程式優化
...
TYPE sr1_t RECORD
    ...
    oqa02 LIKE oqa_file.oqa02,
    oqa03 LIKE oqa_file.oqa03,
    oqa031 LIKE oqa_file.oqa031,
    oqa032 LIKE oqa_file.oqa032,
    oqa04 LIKE oqa_file.oqa04,
    oqa041 LIKE oqa_file.oqa041,
    oqa042 LIKE oqa_file.oqa042,
    oqa05 LIKE oqa_file.oqa05,
    oqa06 LIKE oqa_file.oqa06,
    oqa07 LIKE oqa_file.oqa07,
    oqa08 LIKE oqa_file.oqa08,
    oqa09 LIKE oqa_file.oqa09,
    oqa10 LIKE oqa_file.oqa10,
    oqa11 LIKE oqa_file.oqa11,
    oqa12 LIKE oqa_file.oqa12,
    oqa13 LIKE oqa_file.oqa13,
    oqa14 LIKE oqa_file.oqa14,
    oqa15 LIKE oqa_file.oqa15,
    oqa16 LIKE oqa_file.oqa16,
    oqa17 LIKE oqa_file.oqa17,
    oqaconf LIKE oqa_file.oqaconf,
    oqb01 LIKE oqb_file.oqb01,
    ...
    oqe04_1 LIKE oqe_file.oqe04,
    oqe04_0 LIKE oqe_file.oqe04,
...
MAIN
  ...
  #No.FUN-710090--begin--
   ...
   LET  l_table = cl_prt_temptable('axmg310',g_sql) CLIPPED
   IF l_table=-1 THEN 
...
#------------No.TQC-610089 end
   IF cl_null(tm.wc)
      THEN CALL axmg310_tm(0,0)             # Input print condition
      ELSE
          #LET tm.wc="oqa01= '",tm.wc CLIPPED,"'"   #MOD-650024 mark
           CALL axmg310()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
   CALL cl_gre_drop_temptable(l_table)
END MAIN
 
FUNCTION axmg310_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW axmg310_w AT p_row,p_col WITH FORM "axm/42f/axmg310"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmg310_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      CALL cl_gre_drop_temptable(l_table)
      EXIT PROGRAM
  ...
  #UI
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmg310',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmg310_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      CALL cl_gre_drop_temptable(l_table)
      EXIT PROGRAM
   END IF
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0094
  ...
  #No.FUN-710090--end--
   ...
   DEFINE  l_img_blob     LIKE type_file.blob    #FUN-C40036 add
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='axmg310' #No.FUN-710090
 
#No.FUN-580013  --begin
#    SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'axmg310'
...
#No.FUN-580013  --end
     ...
     PREPARE axmg310_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
        CALL cl_gre_drop_temptable(l_table)
        EXIT PROGRAM 
     END IF
     DECLARE axmg310_curs1 CURSOR FOR axmg310_prepare1
     
    #FUN-C50008--add-str
    DECLARE oqe_cur1 CURSOR FOR
       SELECT * FROM oqe_file WHERE oqe01=? AND oqe02=0 ORDER BY oqe03
    DECLARE oqe_cur2 CURSOR FOR
       SELECT * FROM oqe_file WHERE oqe01=? AND oqe02=? ORDER BY oqe03
    #FUN-C50008--add-end 

    #No.FUN-710090--begin-- mark
    #CALL cl_outnam('axmg310') RETURNING l_name
    #FOR g_i = 1 TO g_len LET g_dash2[g_i,g_i] = '-' END FOR  #No.FUN-580013
    #START REPORT axmg310_rep TO l_name
 
    #LET g_pageno = 0
    #No.FUN-710090--end-- mark
     FOREACH axmg310_curs1 INTO sr.*
...
#      SELECT occ02 INTO l_occ02 FROM occ_file WHERE occ01=sr.oqa.oqa06            #FUN-890011 Mark       
     ...
     #FUN-C50008--mark--end
       LET l_oqe04_0 = ""
       #FOREACH oqe_cur1 INTO l_oqe.*   #FUN-C50008 mark--
       FOREACH oqe_cur1 USING sr.oqa.oqa01 INTO l_oqe.*    #FUN-C50008程式優化
     ...
     #FUN-C50008--mark--end
       LET l_oqe04_1 = ""
      #FOREACH oqe_cur2 INTO l_oqe.*   #FUN-C50008 mark--
      FOREACH oqe_cur2 USING sr.oqa.oqa01,sr.oqb.oqb02 INTO l_oqe.*   #FUN-C50008程式優化
       ...
       EXECUTE insert_prep USING sr.oqa.oqa01,sr.oqa.oqa02,sr.oqa.oqa03,sr.oqa.oqa031,sr.oqa.oqa032,
                                 ...
                                 l_azi03,l_azi04,l_azi05,l_azi07
                                 ,"",l_img_blob,"N",""                        #FUN-C40036 add
       DISPLAY "EXECUTE :",STATUS  #odyliao add
      #No.FUN-710090--end-- 
     # OUTPUT TO REPORT axmg310_rep(sr.*)   #No.FUN-710090 mark
     END FOREACH
 
    #No.FUN-710090--begin--
    #FINISH REPORT axmg310_rep
...
###GENGRE###     LET g_str = tm.wc
   # CALL cl_prt_cs3('axmg310',g_sql,g_str)      #TQC-730088
###GENGRE###     CALL cl_prt_cs3('axmg310','axmg310',g_sql,g_str)
    LET g_cr_table = l_table                   #主報表的temp table名稱         #FUN-C40036 add
    LET g_cr_apr_key_f = "oqa01"       #報表主鍵欄位名稱，用"|"隔開  #FUN-C40036 add
    DISPLAY "BEFORE grdata()"
    CALL axmg310_grdata()    ###GENGRE###
    DISPLAY "AFTER grdata()"
    #CALL cl_prt(l_name,g_prtway,g_copies,g_len)
    #No.FUN-710090--end--
END FUNCTION
 
#No.FUN-710090--begin-- mark
#REPORT axmg310_rep(sr)
...
FUNCTION axmg310_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("axmg310")
        IF handler IS NOT NULL THEN
            START REPORT axmg310_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY oqa01,oqb02"
            DECLARE axmg310_datacur1 CURSOR FROM l_sql
            FOREACH axmg310_datacur1 INTO sr1.*
                OUTPUT TO REPORT axmg310_rep(sr1.*)
            END FOREACH
            FINISH REPORT axmg310_rep
        END IF
...
REPORT axmg310_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B40092------add------str
    DEFINE l_oqa05_gem02 STRING               
    DEFINE l_oqa06_occ02 STRING               
    DEFINE l_oqa07_gen02 STRING              
    DEFINE l_oqb05_sum LIKE oqb_file.oqb05
    DEFINE l_oqb11_sum LIKE oqb_file.oqb11
    DEFINE l_oqa09_fmt STRING
    DEFINE l_oqa10_fmt STRING
    DEFINE l_oqa17_fmt STRING
    DEFINE l_oqa13_fmt STRING
    DEFINE l_oqa14_fmt STRING
    DEFINE l_oqa15_fmt STRING
    DEFINE l_oqa16_fmt STRING
    DEFINE l_oqb10_fmt STRING
    DEFINE l_oqb05_fmt STRING
    DEFINE l_oqb11_fmt STRING
    DEFINE l_oqb05_sum_fmt STRING
    DEFINE l_oqb11_sum_fmt STRING
    #FUN-B40092------add------end
    ORDER EXTERNAL BY sr1.oqa01,sr1.oqb01
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.oqa01
            LET l_lineno = 0
            IF NOT cl_null(sr1.oqa05) THEN                      # NO.FUN-B40092----add start        
               LET l_oqa05_gem02 = sr1.oqa05,' ',sr1.gem02
            END IF
            
            IF NOT cl_null(sr1.oqa06) OR NOT cl_null(sr1.occ02) THEN 
               LET l_oqa06_occ02 = sr1.oqa06,' ',sr1.occ02
            END IF

            IF NOT cl_null(sr1.oqa07) OR NOT cl_null(sr1.gen02) THEN
               LET l_oqa07_gen02 = sr1.oqa07,' ',sr1.gen02
            END IF
            PRINTX l_oqa05_gem02
            PRINTX l_oqa06_occ02
            PRINTX l_oqa07_gen02                            # NO.FUN-B40092-----add end
        BEFORE GROUP OF sr1.oqb01

        
        ON EVERY ROW
            #FUN-B40092------add------str
            LET l_oqa09_fmt = cl_gr_numfmt('oqa_file','oqa09',sr1.azi07)
            PRINTX l_oqa09_fmt

            LET l_oqa10_fmt = cl_gr_numfmt('oqa_file','oqa10',sr1.azi04)
            PRINTX l_oqa10_fmt

            LET l_oqa13_fmt = cl_gr_numfmt('oqa_file','oqa13',sr1.azi04)
            PRINTX l_oqa13_fmt

            LET l_oqa14_fmt = cl_gr_numfmt('oqa_file','oqa14',sr1.azi04)
            PRINTX l_oqa14_fmt

            LET l_oqa15_fmt = cl_gr_numfmt('oqa_file','oqa15',sr1.azi04)
            PRINTX l_oqa15_fmt

            LET l_oqa16_fmt = cl_gr_numfmt('oqa_file','oqa16',sr1.azi04)
            PRINTX l_oqa16_fmt

            LET l_oqa17_fmt = cl_gr_numfmt('oqa_file','oqa17',sr1.azi05)
            PRINTX l_oqa17_fmt

            LET l_oqb05_fmt = cl_gr_numfmt('oqb_file','oqb05',sr1.azi04)
            PRINTX l_oqb05_fmt

            LET l_oqb10_fmt = cl_gr_numfmt('oqb_file','oqb10',sr1.azi03)
            PRINTX l_oqb10_fmt

            LET l_oqb11_fmt = cl_gr_numfmt('oqb_file','oqb11',sr1.azi04)
            PRINTX l_oqb11_fmt

            LET l_oqb05_sum_fmt = cl_gr_numfmt('oqb_file','oqb05',sr1.azi05)
            PRINTX l_oqb05_sum_fmt

            LET l_oqb11_sum_fmt = cl_gr_numfmt('oqb_file','oqb11',sr1.azi05)
            PRINTX l_oqb11_sum_fmt
        ...
        AFTER GROUP OF sr1.oqa01
        AFTER GROUP OF sr1.oqb01
           LET l_oqb05_sum = GROUP SUM(sr1.oqb05)
           LET l_oqb11_SUM = GROUP SUM(sr1.oqb11)
           PRINTX l_oqb05_sum
           PRINTX l_oqb11_sum
        ON LAST ROW
