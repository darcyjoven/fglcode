...
#DEFINE       l_time    LIKE type_file.chr8            #No.FUN-6A0094
   ...
   END IF
    ...
    DECLARE t7104_cl CURSOR FROM g_forupd_sql
 
    LET p_row = 5 LET p_col = 17
 
    OPEN WINDOW t7104_w AT p_row,p_col              #顯示畫面
         WITH FORM "axm/42f/axmt7104"
...
DEFINE  lc_qbe_sn       LIKE    gbm_file.gbm01    #No.FUN-580031  HCN
   ...
   INITIALIZE g_ohf.* TO NULL    #No.FUN-750051
    ...
    PREPARE t7104_prepare FROM g_sql
    DECLARE t7104_cs                         #SCROLL CURSOR
        SCROLL CURSOR WITH HOLD FOR t7104_prepare
    ...
    DROP TABLE x
    PREPARE t7104_precount_x FROM g_sql
    EXECUTE t7104_precount_x
 
        LET g_sql="SELECT COUNT(*) FROM x "
 
    PREPARE t7104_precount FROM g_sql
    DECLARE t7104_count CURSOR FOR t7104_precount
...
FUNCTION t7104_u()
    ...
    OPEN t7104_cl USING g_ohf.ohf01,g_ohf.ohf02
    IF STATUS THEN
       CALL cl_err("OPEN t7104_cl:", STATUS, 1) 
       CLOSE t7104_cl
       ROLLBACK WORK
       RETURN
    END IF
 
    FETCH t7104_cl INTO g_ohf.*            # 鎖住將被更改或取消的資料
...
FUNCTION t7104_q()
    ...
    OPEN t7104_cs                            # 從DB產生合乎條件TEMP(0-30秒)
    IF SQLCA.sqlcode THEN
       CALL cl_err('',SQLCA.sqlcode,0)
       INITIALIZE g_ohf.* TO NULL
    ELSE
       OPEN t7104_count
       FETCH t7104_count INTO g_row_count
       DISPLAY g_row_count TO FORMONLY.cnt
       CALL t7104_fetch('F')                  # 讀出TEMP第一筆並顯示
    END IF
 
END FUNCTION
 
#處理資料的讀取
FUNCTION t7104_fetch(p_flag)
DEFINE
    p_flag          LIKE type_file.chr1                  #處理方式        #No.FUN-680137 VARCHAR(1)
 
    CASE p_flag
        WHEN 'N' FETCH NEXT     t7104_cs INTO g_ohf.ohf01,g_ohf.ohf02
        WHEN 'P' FETCH PREVIOUS t7104_cs INTO g_ohf.ohf01,g_ohf.ohf02
        WHEN 'F' FETCH FIRST    t7104_cs INTO g_ohf.ohf01,g_ohf.ohf02
        WHEN 'L' FETCH LAST     t7104_cs INTO g_ohf.ohf01,g_ohf.ohf02
        WHEN '/'
...
#                       CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
            ...
            END IF
            LET mi_no_ask = FALSE
            FETCH ABSOLUTE g_jump t7104_cs INTO g_ohf.ohf01,g_ohf.ohf02
    END CASE
...
DEFINE
    ...
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    DECLARE t7104_bcl CURSOR FROM g_forupd_sql      # LOCK CURSOR
    ...
    INPUT ARRAY g_ohg WITHOUT DEFAULTS FROM s_ohg.*
        ...
        BEFORE ROW
            ...
            IF g_rec_b >= l_ac THEN
               ...
               OPEN t7104_bcl USING g_ohf.ohf01, g_ohf.ohf02, g_ohg_t.ohg03
               IF STATUS THEN
                  CALL cl_err("OPEN t7104_bcl:", STATUS, 1)
                  LET l_lock_sw = "Y"
               ELSE
                  FETCH t7104_bcl INTO g_ohg[l_ac].*
                  IF SQLCA.sqlcode THEN
...
DEFINE
    ...
    IF INT_FLAG THEN
        ...
        RETURN
    END IF
    CALL t7104_b_fill(l_wc2)
END FUNCTION
 
FUNCTION t7104_b_fill(p_wc2)              #BODY FILL UP
DEFINE
