...
# Modify.........: No.FUN-D30070 13/03/21 By chenying XtraGrid修改
...
MAIN
   ...
   LET g_sql = " ofd10.ofd_file.ofd10,",  
               ...
               " l_e.type_file.num20_6"
               #FUN-CB0002--add--end--
               
   LET l_table = cl_prt_temptable('axmx707',g_sql) CLIPPED   # 產生Temp Table                                                      
 ...
 #--------------No.TQC-610089 end
   IF cl_null(g_bgjob) OR g_bgjob = 'N'
      THEN CALL axmx707_tm(0,0)             # Input print condition
      ELSE CALL axmx707()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
END MAIN
 
FUNCTION axmx707_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW axmx707_w AT p_row,p_col WITH FORM "axm/42f/axmx707"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmx707_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmx707',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmx707_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0094
          ...
          l_test    LIKE type_file.dat,
          #FUN-CB0002--add--str--
          l_a       LIKE type_file.chr20,
          l_b       LIKE type_file.chr20,
          l_b1      LIKE ofc_file.ofc06,
          l_c       LIKE ofd_file.ofd26,
          l_d       LIKE type_file.num20_6,
          l_e       LIKE type_file.num20_6,
          l_msg1    LIKE type_file.chr20,
          l_msg2    LIKE type_file.chr20,
          l_msg3    LIKE type_file.chr20,
          #FUN-CB0002--add--end--
          sr        RECORD
...
#No.FUN-750097-----------------end----------------
     ...
     LET l_sql="SELECT ofd10,ofd01,ofd23,ofd26,ofd27,",                         
             ...
             # "   AND (CAST('",g_today,"' as DATETIME) -ofd26) >= ",tm.b,   #FUN-CB0002 mark       
               "   AND ",tm.wc CLIPPED                           
#No.FUN-750097-----------------------start-----------------------
     PREPARE axmx707_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare1:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
        EXIT PROGRAM
     END IF
     DECLARE axmx707_curs1 CURSOR FOR axmx707_prepare1
 
#    CALL cl_outnam('axmx707') RETURNING l_name
#    START REPORT axmx707_rep TO l_name
 
#    LET g_pageno = 0
     FOREACH axmx707_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,1)
          EXIT FOREACH
       END IF
#      OUTPUT TO REPORT axmx707_rep(sr.*)
       #FUN-CB0002--add--str--
       LET l_msg1 = cl_getmsg('anm-157',g_lang) 
       LET l_msg2 = cl_getmsg('apj-025',g_lang)
       LET l_msg1 = cl_getmsg('abg-131',g_lang) 
       IF cl_null(sr.ofc04) OR cl_null(sr.ofc05) THEN 
          LET l_a = ' '
       ELSE 
          LET l_a = sr.ofc04,l_msg1,sr.ofc05,l_msg2
       END IF 
       IF cl_null(sr.ofc06) THEN 
          LET l_b = ' ' 
       ELSE 
          LET l_b1 = cl_digcut(sr.ofc06,0)
          LET l_b = l_b1,l_msg1
       END IF 
       ...
       ELSE  
          IF sr.ofc06> 0 THEN
             #cstr(Date({axmr707.ofd26} + ({axmr707.ofc04}*30+{axmr707.ofc05})/{axmr707.ofc06}),"yyyy/MM/dd")
             LET l_c = sr.ofd26 + (sr.ofc04 * 30 + sr.ofc05)/sr.ofc06
          ELSE                                                                                                                        
             #cstr(Date({axmr707.ofd26}),"yyyy/MM/dd")
             LET l_c = sr.ofd26
          END IF 
       END IF 
       IF cl_null(sr.ofd26) THEN
          LET l_d = ' '  
       ELSE
          #CSTR(CurrentDate - {axmr707.ofd26},0)
          LET l_d = g_today - sr.ofd26
          LET l_d = cl_digcut(l_d,0)
       END IF 
       IF sr.ofc06 > 0 THEN
          IF (g_today - (sr.ofd26 + (sr.ofc04 * 30 + sr.ofc05)/sr.ofc06)) > 0 THEN
             #CSTR(CurrentDate - ({axmr707.ofd26} + ({axmr707.ofc04}*30+{axmr707.ofc05})/{axmr707.ofc06}),0)
             LET l_e = g_today - (sr.ofd26 + (sr.ofc04 * 30 + sr.ofc05)/sr.ofc06)
             LET l_e = cl_digcut(l_e,0)
          ELSE 
             LET l_e = '0'
          END IF 
       ELSE
          IF (g_today - sr.ofd26) > 0 THEN
             #CSTR(CurrentDate - {axmr707.ofd26},0)
             LET l_e = g_today - sr.ofd26
             LET l_e = cl_digcut(l_e,0)
          ELSE
             LET l_e = '0'
          END IF 
       END IF 
       LET l_e = g_today - sr.ofd26 
       IF g_today - sr.ofd26 >= tm.b  THEN
       #FUN-CB0002--add--end--
          EXECUTE insert_prep USING
               sr.ofd10,sr.ofd01,sr.ofd23,sr.ofd26,sr.ofd27,sr.ofd31,
               sr.ofc04,sr.ofc05,sr.ofc06,sr.ofd02,sr.gen02,sr.ofd33,l_a,l_b,l_c,l_d,l_e   #cy add l_a,l_b,l_c,l_d,l_e
       END IF  #FUN-CB0002  add
     END FOREACH
 
#    FINISH REPORT axmx707_rep
...
###XtraGrid###     CALL  cl_prt_cs3('axmx707','axmx707',l_sql,g_str) #NO.FUN-750097
    LET g_xgrid.table = l_table    ###XtraGrid###
    LET g_xgrid.order_field = "ofd10"   #FUN-D30070
    LET g_xgrid.grup_field = "ofd10"   #FUN-D30070
   #LET g_xgrid.grup_sum_field = "ofd10" #FUN-D30070
    IF g_zz05 = 'Y' THEN                                                                                                           
        CALL cl_wcchp(tm.wc,'ofd01')                                                                                                
             RETURNING tm.wc                                                                                                                                                                                                               
    END IF  
