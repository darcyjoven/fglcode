...
# Modify.........: No:MOD-A60176 10/06/29 By Smapmin 訂單修改記錄不應加上版本條件
...
DEFINE
    ...
    g_oem DYNAMIC ARRAY OF RECORD
            ...
            oem13n  LIKE oem_file.oem13n,
            oem15n  LIKE oem_file.oem15n
        END RECORD,
...
#     DEFINE   l_time LIKE type_file.chr8	    #No.FUN-6A0094
 DEFINE         l_sl		LIKE type_file.num5          #No.FUN-680137 SMALLINT
   ...
   END IF
    ...
    OPEN WINDOW q430_w AT p_row,p_col
        WITH FORM "axm/42f/axmq430"
...
#       CALL q430_q()
#    END IF
IF NOT cl_null(g_argv1) THEN CALL q430_q() END IF
    CALL q430_menu()
    CLOSE WINDOW q430_w
...
FUNCTION q430_cs()
   ...
   INITIALIZE g_oea.* TO NULL    #No.FUN-750051
           CONSTRUCT BY NAME tm.wc ON oea01,oea02,oea06,oea03,oea04,oea032,
...
FUNCTION q430_b_askkey()
   CONSTRUCT tm.wc2 ON oem03,oem08,oem04,oem06,oem07,
                       oem10n,oem11n,oem12n,oem13n,oem15n
                  FROM s_oem[1].oem03,s_oem[1].oem08,s_oem[1].oem04,
                       s_oem[1].oem06,s_oem[1].oem07,
                       s_oem[1].oem10n,s_oem[1].oem11n,
                       s_oem[1].oem12n,s_oem[1].oem13n,s_oem[1].oem15n
              #No.FUN-580031 --start--     HCN
              BEFORE CONSTRUCT
                 CALL cl_qbe_init()
              #No.FUN-580031 --end--       HCN
...
DEFINE
    ...
    CASE p_flag
        WHEN 'N' FETCH NEXT     q430_cs INTO g_oea.oea01
        WHEN 'P' FETCH PREVIOUS q430_cs INTO g_oea.oea01
        WHEN 'F' FETCH FIRST    q430_cs INTO g_oea.oea01
        WHEN 'L' FETCH LAST     q430_cs INTO g_oea.oea01
        WHEN '/'
...
#                       CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
            ...
            END IF
            LET mi_no_ask = FALSE
            FETCH ABSOLUTE g_jump q430_cs INTO g_oea.oea01
    END CASE
    ...
    ELSE
       ...
       CALL cl_navigator_setting( g_curs_index, g_row_count )
    END IF
	SELECT oea01,oea02,oea03,oea04,oea06,oea032,'',
...
FUNCTION q430_b_fill()              #BODY FILL UP
   ...
   IF NOT cl_null(g_argv1) THEN
      LET tm.wc2 = tm.wc2 CLIPPED," AND oem08 = ",g_oea.oea06," "
   END IF
   #-----END MOD-A60176-----
   LET l_sql =
        ...
        "       oem10n,oem11n,oem12n,oem13n,oem15n ",
        "  FROM oem_file ",
        " WHERE oem01 = '",g_oea.oea01,"'"," AND ", tm.wc2 CLIPPED,
        " ORDER BY oem03,oem08,oem04"
    PREPARE q430_pb FROM l_sql
    DECLARE q430_bcs                       #BODY CURSOR
        CURSOR WITH HOLD FOR q430_pb
#TQC-790065---start--
...
#TQC-790065---end--
    LET g_cnt = 1
    FOREACH q430_bcs INTO g_oem[g_cnt].*
        IF SQLCA.sqlcode THEN
...
#       IF NOT cl_chk_act_auth() THEN LET g_oem[g_cnt].oem13n = NULL END IF
    ...
    END FOREACH
    CALL g_oem.deleteElement(g_cnt)   #TQC-790065
    LET g_rec_b=g_cnt-1
