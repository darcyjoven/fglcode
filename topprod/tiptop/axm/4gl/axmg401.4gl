...
# Prog. Version..: '5.30.06-13.03.12(01),               # Language #NO.TQC-5B0061
...
MAIN
  ...
  #str MOD-7C0033 mod
   LET g_sql="oea00.oea_file.oea00  ,oea01.oea_file.oea01,",
             ...
             "oeb07.oeb_file.oeb07  ,l_count.type_file.num5,",      #FUN-A80079 add   #FUN-C40026 add 2,
             #FUN-C40026----add---str----
             "sign_type.type_file.chr1,sign_img.type_file.blob,",   #簽核方式, 簽核圖檔
             "sign_show.type_file.chr1,sign_str.type_file.chr1000"
             #FUN-C40026----add---end----
  #end MD-7C0033 mod
   LET l_table = cl_prt_temptable('axmg401',g_sql) CLIPPED 
   IF l_table = -1 THEN 
  ...
  #str MOD-7C0033 add
   ...
   LET l_table1 = cl_prt_temptable('axmg4011',g_sql) CLIPPED
   IF l_table1 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time                        #FUN-B40087  #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)     #FUN-B40087  #FUN-C10036   mark
      EXIT PROGRAM 
  ...
  #No.FUN-A80079 --start--
   LET g_sql = "imc01.imc_file.imc01,",
    ...
    LET l_table2 = cl_prt_temptable('axmg4012',g_sql) CLIPPED
    IF l_table2 = -1 THEN 
       #CALL cl_used(g_prog,g_time,2) RETURNING g_time                        #FUN-B40087  #FUN-C10036   mark
       #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)     #FUN-B40087  #FUN-C10036   mark
       EXIT PROGRAM
 ...
 #--------------No.TQC-610089 end
   IF cl_null(tm.wc) THEN
      CALL axmg401_tm(0,0)             # Input print condition
   ELSE 
     #LET tm.wc="oea01= '",tm.wc CLIPPED,"'"    #No.TQC-610089 mark
      CALL axmg401()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
   CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)
END MAIN
 
FUNCTION axmg401_tm(p_row,p_col)
...
DEFINE l_cmd          LIKE type_file.chr1000 #No.FUN-680137 VARCHAR(1000)
   ...
   OPEN WINDOW axmg401_w AT p_row,p_col WITH FORM "axm/42f/axmg401"
...
#  LET tm.o ='1'   #NO.TQC-5B0061
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW axmg401_w 
       #  CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)   #FUN-B80089 MARK
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
         CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)   #FUN-B80089  ADD
         EXIT PROGRAM
      END IF
      IF tm.wc=" 1=1" THEN
...
#                            " '",tm.o CLIPPED,"'",                #NO.TQC-5B0061
            ...
            CALL cl_cmdat('axmg401',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW axmg401_w
         #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)    #FUN-B80089 MARK
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
         CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)    #FUN-B80089  ADD
         EXIT PROGRAM
...
DEFINE l_img_blob LIKE type_file.blob      #FUN-C40026 add
  ...
  #No.FUN-710090--begin--
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               ...
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?)"   #TQC-740103 多了一個?,拿掉   #MOD-7C0033 mod #FUN-810029 69?->72?     #FUN-C40026 add 4?
  ...
  #No.FUN-A80079 --start--
   ...
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='axmg401'   #NO.FUN-710090
   #str FUN-740057 add
...
#NO.TQC-5B0061
 ...
 " FROM oeb_file,oea_file LEFT OUTER JOIN occ_file ON oea_file.oea04 = occ_file.occ01  LEFT OUTER JOIN oag_file ON oea_file.oea32 = oag_file.oag01 LEFT OUTER JOIN oah_file ON oea_file.oea31 = oah_file.oah01 ",
             ...
             "   AND ",tm.wc CLIPPED,
             " ORDER BY oea01 "
   PREPARE axmg401_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
    #  CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)    #FUN-B80089 MARK
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)    #FUN-B8008   ADD
      EXIT PROGRAM
   END IF
   DECLARE axmg401_curs1 CURSOR FOR axmg401_prepare1
 ...
 " FROM oeb_file,oea_file LEFT OUTER JOIN occ_file ON oea_file.oea04 = occ_file.occ01  LEFT OUTER JOIN oag_file ON oea_file.oea32 = oag_file.oag01 LEFT OUTER JOIN oah_file ON oea_file.oea31 = oah_file.oah01 ",
   ...
   PREPARE axmg401_prepare2 FROM l_sql
   IF SQLCA.sqlcode THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE axmg401_curs2 CURSOR FOR axmg401_prepare2
#FUN-C50008--add--str
   #列印單身備註
...
#FUN-C50008--add--end
  #CALL cl_outnam('axmg401') RETURNING l_name  #No.FUN-710090 mark
  ...
  #START REPORT axmg401_rep TO l_name
  #LET g_pageno = 0
  #No.FUN-710090--end-- mark
 
   FOREACH axmg401_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0 THEN
    ...
    #FUN-C50008--mark--end
     ...
     #str MOD-7C0033 mod
      EXECUTE insert_prep USING 
         ...
         l_zo041     ,l_zo05      ,l_zo09,sr.oeb07,l_count,   #FUN-810029 add  #FUN-A80079 add sr_oeb07,l_count    #FUN-C40026 add ,
         "",l_img_blob,"N",""                    #FUN-C40026 add
     #end MOD-7C0033 mod
      #No.FUN-710090--end--
     # LET g_pageno = g_pageno + 1              #No.FUN-710090 mark   
     # OUTPUT TO REPORT axmg401_rep(sr.*)       #No.FUN-710090 mark  
   END FOREACH
 
   LET g_cr_table = l_table                 #FUN-C40026 add
   LET g_cr_apr_key_f = "oea01"             #FUN-C40026 add
  #No.FUN-710090--begin--
  #FINISH REPORT axmg401_rep
   #
   FOREACH axmg401_curs2 INTO sr2.*
      #幣別說明
  ...
  #str MOD-7C0033 add
   ...
   LET l_sql = "SELECT oea01 FROM oea_file ",
               " WHERE ",tm.wc CLIPPED,
               "   ORDER BY oea01"
   PREPARE g401_prepare3 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g401_cs3 CURSOR FOR g401_prepare3
...
#FUN-C50008--add-end 
   ...
   FOREACH g401_cs3 INTO sr.oea01
   #FUN-C50008--mark--str
      #DECLARE oao_c1 CURSOR FOR
      #SELECT * FROM oao_file
...
###GENGRE###               "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
###GENGRE###                " WHERE oao03 =0 AND oao05='2'","|",   
###GENGRE###               "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED     #FUN-A80079 add
  #end MOD-7C0033 mod
   IF g_zz05='Y' THEN
   ...
   ELSE 
      LET tm.wc = ''
   END IF
###GENGRE###   LET g_str = tm.wc,';',tm.n
###GENGRE###                    ,";",tm.b,";",tm.c,";",l_zo12,";",tm.e   #FUN-740057 add  #FUN-A80079 add tm.e
  #CALL cl_prt_cs3('axmg401',g_sql,g_str)   #TQC-730088
###GENGRE###   CALL cl_prt_cs3('axmg401','axmg401',g_sql,g_str)
    CALL axmg401_grdata()    ###GENGRE###
...
FUNCTION axmg401_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("axmg401")
        IF handler IS NOT NULL THEN
            START REPORT axmg401_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED," ORDER BY oea01,oeb03"
          
            DECLARE axmg401_datacur1 CURSOR FROM l_sql
            FOREACH axmg401_datacur1 INTO sr1.*
                OUTPUT TO REPORT axmg401_rep(sr1.*)
            END FOREACH
            FINISH REPORT axmg401_rep
        END IF
...
REPORT axmg401_rep(sr1)
    ...
    DEFINE l_oeb13            STRING
    DEFINE l_Bank_Swift       STRING
    DEFINE l_coin_no          STRING
    DEFINE l_sql              STRING
    DEFINE l_oeb14_sum        LIKE oeb_file.oeb14
    DEFINE l_oeb14_tot        STRING
    DEFINE l_title            STRING
    DEFINE l_oeb14            STRING
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.oea01
            ...
            CASE tm.n
               WHEN "1"
                  LET l_title = "Sales Confirmation"
               WHEN "2"
                  LET l_title = "Proforma Invoice"
               WHEN "3"
                  LET l_title = "Sales Contract" 
            END CASE
            PRINTX l_title

            CASE sr1.oea03
               WHEN "BNUSA"
                  LET l_Bank_Swift = "TPBKTWTP671"
               WHEN "EEUA01"
                  LET l_Bank_Swift = "TPBKTWTP671"
               WHEN "EEBY01"
                  LET l_Bank_Swift = "TPBKTWTP671"
               OTHERWISE
                  LET l_Bank_Swift = "SCSBTWTP041"
            END CASE 
            PRINTX  l_Bank_Swift

            IF NOT cl_null(sr1.oea01) AND NOT cl_null(sr1.oea00) THEN
               LET l_oea01_oea00 = sr1.oea01,'(',sr1.oea00,')'
            ELSE IF NOT cl_null(sr1.oea01) AND cl_null(sr1.oea00) THEN
            ...
            ELSE IF NOT cl_null(sr1.oea01) AND cl_null(sr1.oea06) THEN
                    LET l_oea01_oea06 = sr1.oea01
                 END IF
                 IF cl_null(sr1.oea01) AND NOT cl_null(sr1.oea06) THEN
                    LET l_oea01_oea06 = '(',sr1.oea06,')'
                 END IF
            END IF
            PRINTX l_oea01_oea06

            IF NOT cl_null(sr1.oea03) AND NOT cl_null(sr1.occ02_1) THEN
               LET l_oea03_occ02_1 = '[',sr1.oea03,']',sr1.occ02_1
            ELSE IF NOT cl_null(sr1.oea03) AND cl_null(sr1.occ02_1) THEN
                    LET l_oea03_occ02_1 = '[',sr1.oea03,']'
                 END IF
                 IF cl_null(sr1.oea01) AND NOT cl_null(sr1.occ02_1) THEN
                    LET l_oea03_occ02_1 = sr1.occ02_1
                 END IF
            END IF
            PRINTX l_oea03_occ02_1

            IF NOT cl_null(sr1.oea04) AND NOT cl_null(sr1.occ02) THEN
               LET l_oea04_occ02 = '[',sr1.oea04,']',sr1.occ02
            ELSE IF NOT cl_null(sr1.oea04) AND cl_null(sr1.occ02) THEN
            ...
            ELSE IF NOT cl_null(sr1.oea14) AND cl_null(sr1.gen02) THEN
            ...
            ELSE IF NOT cl_null(sr1.oea14) AND cl_null(sr1.gem02) THEN
            ...
            ELSE IF NOT cl_null(sr1.oea21) AND cl_null(sr1.gec02) THEN
            ...
            IF NOT cl_null(sr1.oea25) AND NOT cl_null(sr1.oab02) THEN
               LET l_oea25_oab02 = sr1.oea21,' ',sr1.oab02
            ELSE IF NOT cl_null(sr1.oea25) AND cl_null(sr1.oab02) THEN
            ...
            ELSE IF NOT cl_null(sr1.oea31) AND cl_null(sr1.oah02) THEN
            ...
            IF NOT cl_null(sr1.oea32) AND NOT cl_null(sr1.oag02) THEN
               LET l_oea32_oag02 = '[',sr1.oea04,']',sr1.oag02
            ELSE IF NOT cl_null(sr1.oea32) AND cl_null(sr1.oag02) THEN
                    LET l_oea32_oag02 = '[',sr1.oea04,']'
                 END IF
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        ...
                        " AND oao03 = 0",
                        " AND oao05= '1'"
            START REPORT axmg401_subrep01
            DECLARE axmg401_repcur1 CURSOR FROM l_sql
            FOREACH axmg401_repcur1 INTO sr2.*
                OUTPUT TO REPORT axmg401_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT axmg401_subrep01
        ...
        BEFORE GROUP OF sr1.oeb03
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE oao01 = '",sr1.oea01 CLIPPED,"'",
                        " AND oao03 = ",sr1.oeb03 CLIPPED,
                        " AND oao05= '1'"
            START REPORT axmg401_subrep02
            DECLARE axmg401_repcur2 CURSOR FROM l_sql
            FOREACH axmg401_repcur2 INTO sr2.*
                OUTPUT TO REPORT axmg401_subrep02(sr2.*)
            END FOREACH
            FINISH REPORT axmg401_subrep02
            #FUN-B40087----------add------end-------
        ...
        ON EVERY ROW
            ...
            #FUN-B40087----------add------str-------
            CALL cl_remove_zero(sr1.oeb13)  RETURNING l_oeb13
            CALL cl_remove_zero(sr1.oeb14)  RETURNING l_oeb14
            PRINTX l_oeb14
            PRINTX l_oeb13
            ...
            START REPORT axmg401_subrep05
            DECLARE axmg401_repcur5 CURSOR FROM l_sql
            FOREACH axmg401_repcur5 INTO sr3.*
                OUTPUT TO REPORT axmg401_subrep05(sr3.*)
            END FOREACH
            FINISH REPORT axmg401_subrep05
            #FUN-B40087----------add------end-------
        ...
        AFTER GROUP OF sr1.oea01
           ...
           CALL cl_remove_zero(l_oeb14_sum) RETURNING l_oeb14_tot
           PRINTX l_oeb14_tot
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        ...
                        " AND oao03 = 1",
                        " AND oao05= '2'"
            START REPORT axmg401_subrep04
            DECLARE axmg401_repcur4 CURSOR FROM l_sql
            FOREACH axmg401_repcur4 INTO sr2.*
                OUTPUT TO REPORT axmg401_subrep04(sr2.*)
            END FOREACH
            FINISH REPORT axmg401_subrep04
           #FUN-B40087----------add------end-------
        ...
        AFTER GROUP OF sr1.oeb03
           ...
           LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        ...
                        " AND oao03 = ",sr1.oeb03 CLIPPED,
                        " AND oao05= '2'"
            START REPORT axmg401_subrep03
            DECLARE axmg401_repcur3 CURSOR FROM l_sql
            FOREACH axmg401_repcur3 INTO sr2.*
                OUTPUT TO REPORT axmg401_subrep03(sr2.*)
            END FOREACH
            FINISH REPORT axmg401_subrep03
            #FUN-B40087----------add------end-------
