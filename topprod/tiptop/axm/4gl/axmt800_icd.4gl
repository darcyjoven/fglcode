...
# Modify.........: No.MOD-D40089 13/04/15 By Vampire 若有存在較大版本的變更單，不可以取消作廢
# Modify.........: No:FUN-C40079 13/04/16 By Sakura 調整axmt800,三個比率欄位為no entry
# Modify.........: No:TQC-D40051 13/04/16 By xumm 修改單身新增時按下放棄鍵未執行AFTER INSERT的問題
...
DEFINE
    ...
    b_oeqi          RECORD LIKE oeqi_file.*,   #No:FUN-7C0017
    b_oebi          RECORD LIKE oebi_file.*,   #No:FUN-7C0017
   ...
   INITIALIZE g_oep.* TO NULL    #No.FUN-750051
	...
	#No:FUN-580031 --end--       HCN
      ...
      CONSTRUCT g_wc2 ON oeq03, oeq04b,oeq041b,oeq30b,oeq05b,oeq12b,oeq23b,oeq24b,   #No.FUN-990030 add oeq30b #CHI-C50063 add oeq041b,oeq05b
                         ...
                         ,oeqiicd01b,oeqiicd02b,oeqiicd03b,     #No:FUN-7C0017
                          oeqiicd04b                            #No:FUN-7C0017                         
                    ...
                    FROM s_oeq[1].oeq03 ,s_oeq[1].oeq04b,s_oeq[1].oeq041b,s_oeq[1].oeq30b,s_oeq[1].oeq05b,s_oeq[1].oeq12b, #No.FUN-990030 add oeq30b #CHI-C50063 add oeq041b,oeq05b
                         ...
                         ,s_oeq[1].oeqiicd01b,s_oeq[1].oeqiicd02b, #No:FUN-7C0017
                          s_oeq[1].oeqiicd03b,s_oeq[1].oeqiicd04b  #No:FUN-7C0017
		...
		BEFORE CONSTRUCT
         ...
         ON ACTION CONTROLP
            CASE
               WHEN INFIELD(oeqiicd01b)
#FUN-AA0059---------mod------------str-----------------               
#                  CALL cl_init_qry_var()
#                  LET g_qryparam.state = 'c'
#                  LET g_qryparam.form = "q_imaicd00"
#                  CALL cl_create_qry() RETURNING g_qryparam.multiret
                   CALL q_sel_ima(TRUE, "q_imaicd00","","","","","","","",'')  RETURNING  g_qryparam.multiret
#FUN-AA0059---------mod------------end-----------------
                  DISPLAY g_qryparam.multiret TO oeqiicd01b
                  NEXT FIELD oeqiicd01b
               WHEN INFIELD(oeqiicd04b)
                  CALL cl_init_qry_var()                                                                                            
                  LET g_qryparam.state = 'c'                                                                                        
                  LET g_qryparam.form = "q_icd3"                                                                                
                  CALL cl_create_qry() RETURNING g_qryparam.multiret                                                                
                  DISPLAY g_qryparam.multiret TO oeqiicd04b                                                                         
                  NEXT FIELD oeqiicd04b
               WHEN INFIELD(oeq29b)                                                                                                 
                  CALL cl_init_qry_var()                                                                                            
                  LET g_qryparam.state = 'c'                                                                                        
...
#FUN-A80118--Add--End                  
   ...
   ELSE
      LET g_sql= "SELECT UNIQUE oep01,oep02 ",    #No.TQC-9A0114  #MOD-C50097 add UNIQUE
                 ...
                 " ORDER BY oep01 "    #TQC-9A0168 mod
        IF g_wc2.getindexof('oeqi',1) > 0 THEN
            LET g_sql = "SELECT UNIQUE oep01,oep02 ",   #No.TQC-9A0114  #MOD-C50097 add UNIQUE
                         "  FROM oep_file, oeq_file, oea_file,oeqi_file",
                         " WHERE oep01 = oeq01 AND oep01 IS NOT NULL ",
                         "   AND oep02 = oeq02 ",
                         "   AND oep01 = oea01 ",
                         "   AND oeqi01 = oeq01 ",
   ...
   ELSE
      LET g_sql="SELECT oep01,oep02 FROM oep_file,oeq_file,oea_file ",
                ...
                " INTO TEMP count_tmp"
       IF g_wc2.getindexof('oeqi',1) > 0 THEN
         LET g_sql="SELECT oep01,oep02 FROM oep_file,oeq_file,oea_file,oeqi_file ",
                " WHERE ", g_wc CLIPPED," AND ",g_wc2 CLIPPED,
                "   AND oeq01 = oep01 ",
                "   AND oep01 = oea01 ",
                "   AND oeqi01 = oeq01 ",
...
FUNCTION t800_set_entry(p_cmd)
 DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-680137 VARCHAR(1)
   ...
   #CALL cl_set_comp_entry("oep161b,oep162b,oep163b",TRUE)    #No:FUN-A50103 #FUN-C40079 mark
    CALL cl_set_comp_entry("oep161b,oep162b,oep163b",FALSE)   #No:FUN-A50103 #FUN-C40079 add
...
##FUN-CC0007---add-----END
       ...
       IF SQLCA.sqlcode THEN
          ...
          LET g_success = 'N'
       ELSE
          IF NOT s_del_oeqi(g_oep.oep01,g_oep.oep02,'','') THEN
             LET g_success = 'N'
          END IF
       END IF
...
DEFINE l_count         LIKE type_file.num5
DEFINE l_oeqi          RECORD LIKE oeqi_file.*   #No.FUN-7B0018
...
DEFINE l_ima130        LIKE ima_file.ima130      #MOD-D20152 add
   ...
   #FUN-CA0009 add END
    ...
    DECLARE t800_bcl CURSOR FROM g_forupd_sql      # LOCK CURSOR
       ...
       LET g_forupd_sql = "SELECT * FROM oeqi_file ",
                          " WHERE oeqi01= ? AND oeqi02= ? AND oeqi03= ?  FOR UPDATE "
       LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
       DECLARE t800_bcl_ind CURSOR FROM g_forupd_sql      # LOCK CURSOR
          ...
          #FUN-910088-add--edd--
               ...
               ELSE
                  ...
                  IF SQLCA.sqlcode THEN
                     ...
                     LET l_lock_sw = "Y"
                  ELSE
                       OPEN t800_bcl_ind USING g_oep.oep01,g_oep.oep02,g_oeq_t.oeq03
                       IF SQLCA.sqlcode THEN
                          CALL cl_err('lock oeqi',SQLCA.sqlcode,1)
                          LET l_lock_sw = "Y"
                       ELSE
                          FETCH t800_bcl_ind INTO b_oeqi.*
                          IF SQLCA.sqlcode THEN
                             CALL cl_err('lock oeqi',SQLCA.sqlcode,1)
                             LET l_lock_sw = "Y"
                          END IF
...
#MOD-B80196 -- end --
               ...
               #IF b_oeq.oeq37a=0 OR cl_null(b_oeq.oeq37a) THEN LET b_oeq.oeq37a=b_oeq.oeq13a END IF #FUN-AB0082 #MOD-B60080 mark
                ...
                IF SQLCA.sqlcode THEN
                   ...
                   CANCEL INSERT
                ELSE
                LET b_oeqi.oeqi01=g_oep.oep01
                LET b_oeqi.oeqi02=g_oep.oep02
                LET b_oeqi.oeqi03=g_oeq[l_ac].oeq03
                IF NOT s_ins_oeqi(b_oeqi.*,'') THEN
                   CANCEL INSERT
                END IF
                   MESSAGE 'INSERT O.K'
               ...
               #---->當已交量>0時,不可變更單位
                ...
                END IF
                  ...
                  IF g_cnt = 0 THEN
                     ...
                     LET g_oeq[l_ac].oeqiicd01b=''  #FUN-C30289
                     LET g_oeq[l_ac].oeqiicd04b=''  #FUN-C30289
                     ...
                     END IF   #MOD-910144 
                     LET g_oeq[l_ac].oeqiicd01a=''  #FUN-C30289
                     LET g_oeq[l_ac].oeqiicd04a=''  #FUN-C30289
                     LET g_oeb1006 = 100                                                                                            
                     LET g_oeb1012 = 'N'                                                                                            
                  ELSE
...
#                                     g_oep.oep07b,g_oep.oep01,g_oeq[l_ac].oeq03,        #MOD-AC0117    
            ...
            CALL t800_oeqiicd01a()   #No.FUN-7B0077 
        #---> 輸入數量後,是否大於已交量
        AFTER FIELD oeq30a
...
#             IF cl_null(g_oeq[l_ac].oeq13a) OR g_oeq[l_ac].oeq13a = 0 THEN                             #MOD-B80192 mark
      ...
      #TQC-C20183--add--start--
       ...
       #TQC-C20183--add--edd-- 
        ...
        AFTER FIELD oeqiicd04a
           IF NOT cl_null(g_oeq[l_ac].oeqiicd04a) THEN                                                                              
              LET l_n=0                                                                                                             
              SELECT count(*) INTO l_n FROM icd_file                                                                                
               WHERE icd02='R' and icdacti='Y' and icd01=g_oeq[l_ac].oeqiicd04a                                                     
              IF l_n=0 THEN                                                                                                         
                 CALL cl_err('','aic-004',0)                                                                                        
                 NEXT FIELD oeqiicd04a                                                                                              
              END IF                                                                                                                
           END IF                          
        ...
        BEFORE DELETE                            #是否取消單身
           ...
           #    END IF
           # #FUN-CC0007---add-----END
               IF NOT s_del_oeqi(g_oep.oep01,g_oep.oep02,g_oeq_t.oeq03,'') THEN
                  ROLLBACK WORK
                  CANCEL DELETE
               END IF
...
#MOD-B80196 -- end --
               ...
               IF SQLCA.sqlcode THEN
                   ...
                   LET g_oeq[l_ac].* = g_oeq_t.*
               ELSE
                      UPDATE oeqi_file SET *=b_oeqi.*
                        WHERE oeqi01=g_oep.oep01
                          AND oeqi02=g_oep.oep02
                          AND oeqi03=g_oeq_t.oeq03 
                    IF SQLCA.sqlcode THEN
                       CALL cl_err3("upd","oeqi_file",g_oep.oep01,'',SQLCA.sqlcode,"","upd oeqi",1)  
                    END IF
                   MESSAGE 'UPDATE O.K'
                  LET l_oep09 = '0'          #FUN-550031   
                   IF g_oaz.oaz06 = 'Y' THEN
...
#FUN-AA0059---------mod------------end-----------------
              ...
              WHEN INFIELD(oeq30a)
                 ...
                 NEXT FIELD oeq30a
              WHEN INFIELD(oeqiicd04a)
                 CALL cl_init_qry_var()
                 LET g_qryparam.form ="q_icd3" 
                 LET g_qryparam.default1 = g_oeq[l_ac].oeqiicd04a                                                                                      
                 CALL cl_create_qry() RETURNING g_oeq[l_ac].oeqiicd04a 
                 DISPLAY BY NAME g_oeq[l_ac].oeqiicd04a                                                                
                 NEXT FIELD oeqiicd04a
...
FUNCTION t800_oeqiicd01a()
   DEFINE l_imaacti LIKE ima_file.imaacti

   LET g_errno = " "
   SELECT ima01,imaacti INTO g_oeq[l_ac].oeqiicd01a,l_imaacti
    FROM ima_file,imaicd_file
   WHERE ima01=imaicd00 AND imaacti='Y' #AND imaicd05='1' #FUN-C30278
   AND imaicd04='0' OR imaicd04='1'  #FUN-C30278
      AND ima01=g_oeq[l_ac].oeq04a  
   CASE WHEN SQLCA.SQLCODE = 100 LET g_errno = 'mfg0002'
                                 LET g_oeq[l_ac].oeqiicd01a=NULL
        WHEN l_imaacti='N' LET g_errno = '9028'
        OTHERWISE          LET g_errno=SQLCA.SQLCODE USING '-------'
   END CASE
   DISPLAY BY NAME g_oeq[l_ac].oeqiicd01a
END FUNCTION
...
DEFINE
    ...
    IF cl_null(g_oep.oep02) THEN LET g_oep.oep02 = 0 END IF
     ...
     "       oeq37b,",  #FUN-AB0082
     "       oeq13b,oeq14b,oeq14tb,oeq31b,oeq32b,oeq33b,oeq15b,oeq28b,oeq29b,oeqiicd01b,oeqiicd02b,oeqiicd03b,oeqiicd04b,",      #MOD-A70005 add oeq14tb #FUN-A80118 Add oeq31b,oeq32b,oeq33b
     ...
     "       oeq37a, ", #FUN-AB0082
     "       oeq13a,oeq14a,oeq14ta,oeq31a,oeq32a,oeq33a,oeq15a,oeq28a,oeq29a,oeqiicd01a,oeqiicd02a,oeqiicd03a,oeqiicd04a,",   #No:FUN-740016   #MOD-A70005 add oeq14ta  #FUN-A80118 Add oeq31a,oeq32a,oeq33a
                  "oeq50",
     " FROM oeq_file LEFT OUTER JOIN oeqi_file ON oeq01 = oeqi01 AND oeq02 = oeqi02 AND oeq03 = oeqi03 ",
     " WHERE oeq01 = '",g_oep.oep01, "'",
#    "   AND oeq02 = '",g_oep.oep02,"'",   #TQC-AB0025 mark 
     "   AND oeq02 = ",g_oep.oep02 ,       #TQC-AB0025 add
     "   AND ",p_wc CLIPPED
...
FUNCTION t800_g1()
   ...
   FOREACH t800_cur2 INTO g_oeq05a,g_oeq05b,g_oeq04a,g_oeq04b,g_oeq041a,
      ...
      INITIALIZE  b_oeqi.* TO NULL
      SELECT * INTO b_oeqi.* FROM oeqi_file WHERE oeqi01 = g_oep.oep01 
                                              AND oeqi02 = g_oep.oep02
                                              AND oeqi03 = g_oeq03
  ...
  IF g_cnt=0 THEN
    ...
    INSERT INTO oeb_file(oeb01,oeb03,oeb04,oeb05,oeb05_fac,
     ...
     IF SQLCA.SQLCODE OR SQLCA.SQLERRD[3]=0 THEN
        ...
        RETURN
     ELSE
        LET b_oebi.oebi01 = g_oep.oep01
        LET b_oebi.oebi03 = g_oeq03
        LET b_oebi.oebiicd01=b_oeqi.oeqiicd01a  
        LET b_oebi.oebiicd03=b_oeqi.oeqiicd02a  
        LET b_oebi.oebiicd05=b_oeqi.oeqiicd03a  
        LET b_oebi.oebiicd07=b_oeqi.oeqiicd04a  
        IF NOT s_ins_oebi(b_oebi.*,'') THEN
           LET g_success = 'N'
           RETURN
        END IF
     END IF
...
{ckp#15}
 ...
 END IF
     ...
     IF NOT cl_null(b_oeqi.oeqiicd01a) AND 
       (b_oeqi.oeqiicd01a<>b_oeqi.oeqiicd01b OR b_oeqi.oeqiicd01b IS NULL)
       THEN
       UPDATE oebi_file SET oebiicd01=b_oeqi.oeqiicd01a
        WHERE oebi01=g_oep.oep01
          AND oebi03=g_oeq03
#    END IF   #MOD-B80105 mark
       IF SQLCA.SQLERRD[3] = 0 THEN
          LET g_success = 'N'
          LET g_showmsg=g_oep.oep01,"/",g_oeq03 
          CALL s_errmsg("oebi01,oebi03",g_showmsg,"(axmt800:#b_oebi.oebiicd01=b_oeqi.oeqiicd01a)","mfg9328",1) 
          RETURN
       END IF
     END IF   #MOD-B80105
     IF NOT cl_null(b_oeqi.oeqiicd02a) AND 
       (b_oeqi.oeqiicd02a<>b_oeqi.oeqiicd02b OR b_oeqi.oeqiicd02b IS NULL)
       THEN
       UPDATE oebi_file SET oebiicd03=b_oeqi.oeqiicd02a
        WHERE oebi01=g_oep.oep01
          AND oebi03=g_oeq03
#    END IF   #MOD-B80105 mark
       IF SQLCA.SQLERRD[3] = 0 THEN
          LET g_success = 'N'
          LET g_showmsg=g_oep.oep01,"/",g_oeq03 
          CALL s_errmsg("oebi01,oebi03",g_showmsg,"(axmt800:#b_oebi.oebiicd03=b_oeqi.oeqiicd02a)","mfg9328",1) 
          RETURN
       END IF
     END IF   #MOD-B80105
     IF NOT cl_null(b_oeqi.oeqiicd03a) AND 
       (b_oeqi.oeqiicd03a<>b_oeqi.oeqiicd03b OR b_oeqi.oeqiicd03b IS NULL)
       THEN
       UPDATE oebi_file SET oebiicd05=b_oeqi.oeqiicd03a
        WHERE oebi01=g_oep.oep01
          AND oebi03=g_oeq03
#    END IF   #MOD-B80105 mark
       IF SQLCA.SQLERRD[3] = 0 THEN
          LET g_success = 'N'
          LET g_showmsg=g_oep.oep01,"/",g_oeq03 
          CALL s_errmsg("oebi01,oebi03",g_showmsg,"(axmt800:#b_oebi.oebiicd05=b_oeqi.oeqiicd03a)","mfg9328",1) 
          RETURN
       END IF
     END IF   #MOD-B80105
     IF NOT cl_null(b_oeqi.oeqiicd04a) AND 
       (b_oeqi.oeqiicd04a<>b_oeqi.oeqiicd04b OR b_oeqi.oeqiicd04b IS NULL)
       THEN
       UPDATE oebi_file SET oebiicd07=b_oeqi.oeqiicd04a
        WHERE oebi01=g_oep.oep01
          AND oebi03=g_oeq03
#    END IF   #MOD-B80105 mark
       IF SQLCA.SQLERRD[3] = 0 THEN
          LET g_success = 'N'
          LET g_showmsg=g_oep.oep01,"/",g_oeq03 
          CALL s_errmsg("oebi01,oebi03",g_showmsg,"(axmt800:#b_oebi.oebiicd07=b_oeqi.oeqiicd04a)","mfg9328",1) 
          RETURN
       END IF
     END IF   #MOD-B80105
END FUNCTION
...
#--- 產生本張單據之報表檔
  # LET l_prog='axmr800' #FUN-C30085 mark
   LET l_prog='axmg800' #FUN-C30085 add
...
FUNCTION t800_b_move_to()
   ...
   #FUN-A80118-- Add--End   
   LET g_oeq[l_ac].oeqiicd01b=b_oeqi.oeqiicd01b
   LET g_oeq[l_ac].oeqiicd02b=b_oeqi.oeqiicd02b
   LET g_oeq[l_ac].oeqiicd03b=b_oeqi.oeqiicd03b
   LET g_oeq[l_ac].oeqiicd04b=b_oeqi.oeqiicd04b
   LET g_oeq[l_ac].oeqiicd01a=b_oeqi.oeqiicd01a
   LET g_oeq[l_ac].oeqiicd02a=b_oeqi.oeqiicd02a
   LET g_oeq[l_ac].oeqiicd03a=b_oeqi.oeqiicd03a
   LET g_oeq[l_ac].oeqiicd04a=b_oeqi.oeqiicd04a
END FUNCTION
...
FUNCTION t800_b_move_back()
  ...
  #LET b_oeq.oeq14ta=t_oeq14ta   #MOD-A70005
   ...
   #FUN-A80118-- Add--End    
   LET b_oeqi.oeqi01=g_oep.oep01   #TQC-950054
   LET b_oeqi.oeqi02=g_oep.oep02   #TQC-950054 
   LET b_oeqi.oeqi03=g_oeq[l_ac].oeq03 #TQC-950054
   LET b_oeqi.oeqiicd01a=g_oeq[l_ac].oeqiicd01a
   LET b_oeqi.oeqiicd01b=g_oeq[l_ac].oeqiicd01b
   LET b_oeqi.oeqiicd02a=g_oeq[l_ac].oeqiicd02a
   LET b_oeqi.oeqiicd02b=g_oeq[l_ac].oeqiicd02b
   LET b_oeqi.oeqiicd03a=g_oeq[l_ac].oeqiicd03a
   LET b_oeqi.oeqiicd03b=g_oeq[l_ac].oeqiicd03b
   LET b_oeqi.oeqiicd04a=g_oeq[l_ac].oeqiicd04a
   LET b_oeqi.oeqiicd04b=g_oeq[l_ac].oeqiicd04b   
END FUNCTION
