...
# Modify.........: No.FUN-CB0072 12/12/18 By dongsz CR轉GR
...
TYPE sr2_t RECORD
    ofa01 LIKE ofa_file.ofa01,
    ofb01 LIKE ofb_file.ofb01,        #FUN-CB0072 add
    ofb06 LIKE ofb_file.ofb06,
    ima02 LIKE ima_file.ima02,        #FUN-CB0072 add
    ofa23 LIKE ofa_file.ofa23,
...
MAIN
   ...
   LET l_table = cl_prt_temptable('axmg558',g_sql) CLIPPED   # 產生Temp Table
   IF l_table = -1 THEN 
   ...
   LET g_sql = "ofa01.ofa_file.ofa01,ofb01.ofb_file.ofb01,ofb06.ofb_file.ofb06,ima02.ima_file.ima02,     ofa23.ofa_file.ofa23,", #FUN-CB0072 add ofb01.ofb_file.ofb01,ima02.ima_file.ima02
               "ofb13.ofb_file.ofb13,     ofb14t.ofb_file.ofb14t,",             
               "ofb916.ofb_file.ofb916,   ofb917.ofb_file.ofb917,",
               "str1.type_file.chr1000,   str2.type_file.chr1000"
   LET l_table1 = cl_prt_temptable('axmg5581',g_sql) CLIPPED   # 產生Temp Table
   IF l_table1 = -1 THEN 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time                                                        
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1)          #FUN-CB0072 add
      EXIT PROGRAM 
   ...
   IF cl_null(tm.wc) THEN
      CALL axmg558_tm(0,0)             # Input print condition
   ELSE 
      CALL axmg558()                        # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time
END MAIN
 
FUNCTION axmg558_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,      
   ...
   OPEN WINDOW axmg558_w AT p_row,p_col WITH FORM "axm/42f/axmg558"
...
WHILE TRUE
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmg558_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1)          #FUN-CB0072 add
      EXIT PROGRAM
   END IF
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                         ...
                         " '",g_template CLIPPED,"'",   
                         " '",g_rpt_name CLIPPED,"'"    
         CALL cl_cmdat('axmg558',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmg558_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time 
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1)          #FUN-CB0072 add
      EXIT PROGRAM
   END IF
...
FUNCTION axmg558()
   ...
   DEFINE l_ima02    LIKE ima_file.ima02      #FUN-CB0072 add
   ...
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='axmg558'
...
#            "                ofa0455,ofa41,ofa42,ofa43,ofa45,ofa45,ofa49,ofa61,ocf_file.*",
             ...
             " WHERE ofaconf !='X' ",
             "   AND ",tm.wc CLIPPED
   PREPARE axmg558_prepare1 FROM l_sql
   ...
   LET l_sql="SELECT SUM(ofb12*ofb13) FROM ofa_file,ofb_file",
             ...
             "   AND ofaconf !='X' ",
             " GROUP BY ofa01"
   PREPARE axmg558_chk_say_sql FROM l_sql
   DECLARE axmg558_chk_say_cur CURSOR FOR axmg558_chk_say_sql
   OPEN axmg558_chk_say_cur
   IF SQLCA.sqlcode THEN
      LET l_ofb12=0
   ELSE
      FOREACH axmg558_chk_say_cur INTO l_ofb12
      ...
      END FOREACH
   END IF
   DECLARE axmg558_curs1 CURSOR FOR axmg558_prepare1
 
   FOREACH axmg558_curs1 INTO l_ofa.ofa01,l_ofa.ofa02,l_ofa.ofa10,l_ofa.ofa16,l_ofa.ofa23,l_ofa.ofa32,
...
#     SELECT ged02 INTO l_ged02   FROM ged_file WHERE ged01 = l_ofa.ofa43
   ...
   END FOREACH
      ...
      PREPARE axmg558_prepare2 FROM l_sql
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('prepare1:',SQLCA.sqlcode,1)
         CALL cl_used(g_prog,g_time,2) RETURNING g_time
         CALL cl_gre_drop_temptable(l_table||"|"||l_table1)          #FUN-CB0072 add
         EXIT PROGRAM
      END IF
      DECLARE axmg558_curs2 CURSOR FOR axmg558_prepare2
      FOREACH axmg558_curs2 INTO l_ofa.ofa01,l_ofb.*
         LET t_ofb14 = 0
         SELECT SUM(ofb14t) INTO t_ofb14 FROM ofb_file WHERE ofb01=l_ofa.ofa01
         IF cl_null(t_ofb14) THEN LET t_ofb14 = 0 END IF 
         SELECT ima02 INTO l_ima02 FROM ima_file WHERE ima021 = l_ofb.ofb06    #FUN-CB0072 add
         IF SQLCA.sqlcode THEN LET l_ima02 = NULL END IF                       #FUN-CB0072 add
         CALL cl_say(t_ofb14,80) RETURNING l_str1,l_str2
 
        EXECUTE insert_prep1 USING
           l_ofa.ofa01,l_ofb.ofb01,l_ofb.ofb06,l_ima02,l_ofa.ofa23,l_ofb.ofb13,l_ofb.ofb14t,l_ofb.ofb916,    #FUN-CB0072 add l_ofb.ofb01,l_ima02
...
###GENGRE###               "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED
   ...
   ELSE
      LET tm.wc = ''
   END IF
###GENGRE###   LET g_str = tm.wc,";",g_sma.sma115,";",g_sma.sma116
###GENGRE###   CALL cl_prt_cs3('axmg558','axmg558',g_sql,g_str)
    CALL axmg558_grdata()    ###GENGRE###
   #------------------------------ CR (4) ------------------------------#
...
FUNCTION axmg558_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("axmg558")
        IF handler IS NOT NULL THEN
            START REPORT axmg558_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                       ," ORDER BY ofa01"                    #FUN-CB0072 add
          
            DECLARE axmg558_datacur1 CURSOR FROM l_sql
            FOREACH axmg558_datacur1 INTO sr1.*
                OUTPUT TO REPORT axmg558_rep(sr1.*)
            END FOREACH
            FINISH REPORT axmg558_rep
        END IF
...
REPORT axmg558_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE sr2 sr2_t
    DEFINE l_lineno LIKE type_file.num5
    DEFINE l_sql    STRING                #FUN-CB0072 add  
    DEFINE l_say_total_1   STRING         #FUN-CB0072 add
    DEFINE l_sum_ofb14t_1  STRING         #FUN-CB0072 add
        ...
        ON EVERY ROW
           ...
           #FUN-CB0072--add--str---
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED
                       ," WHERE ofb01 = '",sr1.ofa01 CLIPPED,"'"
 
            START REPORT axmg558_subrep01
            DECLARE axmg558_repcur1 CURSOR FROM l_sql
            FOREACH axmg558_repcur1 INTO sr2.*
               OUTPUT TO REPORT axmg558_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT axmg558_subrep01
           #FUN-CB0072--add--end---
        ...
        AFTER GROUP OF sr1.ofa01
          #FUN-CB0072--add--str---
           IF cl_null(sr1.str1) AND cl_null(sr1.str2) THEN
              LET l_say_total_1 = sr1.ofa23
           ELSE
              IF cl_null(sr1.str1) AND NOT cl_null(sr1.str2) THEN
                 LET l_say_total_1 = sr1.ofa23,'  ',sr1.str2
              ELSE
                 IF NOT cl_null(sr1.str1) AND cl_null(sr1.str2) THEN
                    LET l_say_total_1 = sr1.ofa23,'  ',sr1.str1
                 ELSE
                    LET l_say_total_1 = sr1.ofa23,'  ',sr1.str1,'  ',sr1.str2
                 END IF
              END IF
           END IF 
           PRINTX l_say_total_1

           LET l_sum_ofb14t_1 = sr1.ofa23,sr1.ofb14t
           PRINTX l_sum_ofb14t_1
          #FUN-CB0072--add--end---
...
#FUN-CB0072--add--str---
REPORT axmg558_subrep01(sr2)
   DEFINE sr2 sr2_t
   DEFINE l_ofb917_ofb916   STRING
   DEFINE l_ofb13           STRING
   DEFINE l_ofb14t          STRING
   DEFINE l_ofb14t_sum  LIKE ofb_file.ofb14t
   DEFINE l_sum_ofb14t      STRING
   DEFINE l_ofb917_sum  LIKE ofb_file.ofb917
   DEFINE l_sum_qty         STRING
   ...
   FORMAT
      ON EVERY ROW
         ...
         LET l_ofb917_ofb916 = sr2.ofb917,sr2.ofb916
         PRINTX l_ofb917_ofb916
         LET l_ofb13 = sr2.ofa23,sr2.ofb13,"/",sr2.ofb916
         PRINTX l_ofb13
         LET l_ofb14t = sr2.ofa23,sr2.ofb14t
         PRINTX l_ofb14t
         
         PRINTX sr2.*

      AFTER GROUP OF sr2.ofb01
         LET l_ofb14t_sum = SUM(sr2.ofb14t)
         PRINTX l_ofb14t_sum
         LET l_sum_ofb14t = sr2.ofa23,l_ofb14t_sum
         PRINTX l_sum_ofb14t
         LET l_ofb917_sum = SUM(sr2.ofb917)
         PRINTX l_ofb917_sum
         LET l_sum_qty = l_ofb917_sum,sr2.ofb916
         PRINTX l_sum_qty
         IF cl_null(sr2.str1) AND cl_null(sr2.str2) THEN
            LET l_say_total = sr2.ofa23
         ELSE  
            IF cl_null(sr2.str1) AND NOT cl_null(sr2.str2) THEN
               LET l_say_total = sr2.ofa23,'  ',sr2.str2
            ELSE
               IF NOT cl_null(sr2.str1) AND cl_null(sr2.str2) THEN
                  LET l_say_total = sr2.ofa23,'  ',sr2.str1
               ELSE
                  LET l_say_total = sr2.ofa23,'  ',sr2.str1,'  ',sr2.str2
               END IF
            END IF
         END IF
         PRINTX l_say_total
