...
#
# Pattern name...: axmp886.4gl    
# Descriptions...: 銷貨折讓自動產生應收帳款副程式(多工廠)(逆拋)
...
# Modify.........: NO.MOD-610111 06/01/25 By p886_ap_apa()抓取ap帳款類別時,組sql少括孤造成分錄產生錯誤
...
# Modify.........: NO.FUN-710019 06/09/11 by Yiting 1.依apmi000設定站別取資料
...
# Modify.........: NO.TQC-7C0157 07/12/21 BY yiting 資料庫有誤 
...
# Modify.........: No.MOD-830039 08/03/06 By claire g_apz應改為p_apz才有值
...
# Modify.........: No.TQC-960404 09/06/29 By lilingyu oma64 給予預設值為'1'
...
# Modify.........: No:MOD-AC0007 10/12/01 By lilingyu axmp855拋轉過來的單據,原幣和本幣相差一分錢
...
# Modify.........: No:MOD-BC0020 11/12/10 By Vampire DECLARE aag_c1 CURSOR段不應該包在IF p_apa.apa51 <> 'STOCK' THEN的條件裡
...
# Modify.........: No:MOD-C40063 12/06/05 By Vampire 調整 CALL _up_omb後,再CALL _ins_omc()、CALL __up_apb後,再CALL _ins_apc()
# Modify.........: No:CHI-C60004 12/11/07 By Sakura 拋轉新增apa77欄位處理
# Modify.........: No:TQC-D30046 13/03/20 By SunLM 多角銷退的收入科目需要考慮oba11
# Modify.........: No:MOD-D40090 13/04/15 By Vampire 將 nppqlegal 改為 npplegal
...
DEFINE p_plant     LIKE type_file.chr21       #No.FUN-680137 VARCHAR(21)                   #資料庫名稱
...
DEFINE p_i         LIKE type_file.num5     #No.FUN-680137 SMALLINT                     #FUN-710019
DEFINE p_poz01     LIKE poz_file.poz01     #FUN-710019 
DEFINE g_npq       RECORD LIKE npq_file.*  # No.MOD-670069 add 
DEFINE g_ccz07     LIKE ccz_file.ccz17     # No.MOD-670069 add
DEFINE p_omb930    LIKE omb_file.omb930    #FUN-710019
DEFINE p_apa930    LIKE apa_file.apa930    #FUN-710019
DEFINE p_oha05     LIKE oha_file.oha05
...
FUNCTION axmp886(t_oha01,p_oma02,l_poy20,l_poy16,l_poy17,l_plant,   #FUN-980092
                next_dbs,l_poy12,l_last_plant,
                l_poy18,l_poy19,l_flow99,AR_t1,AP_t1,
                l_rvu01,l_oha01,l_rva01,l_apa930,l_omb930,l_oha05,l_i) 
 ...
 DEFINE l_oha05     LIKE oha_file.oha05
 ...
 DEFINE l_apa34f     LIKE apa_file.apa34f    #CHI-B40063 add
  ...
  #給予應收單頭值
  IF p_oha05 = '2' OR (p_oha05='3' AND p_i <> 0) THEN
      CALL p886_oma(p_oha01)
  END IF 
  LET p_apa31f = 0
  LET p_apa31  = 0
  #讀取出貨單身檔(ogb_file)
  ...
  FOREACH ohb_c1 INTO p_ohb.* 
      ...
      LET l_sql = "SELECT * ",
                  ...
                  " FROM ",cl_get_target_table(p_plant,'pmm_file'), #FUN-A50102
                   " WHERE pmm99 = '",p_oea.oea99,"' AND pmm18 <> 'X' " #訂單單號  #no.TQC-7C0157 add
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
         CALL cl_parse_qry_sql(l_sql,p_plant) RETURNING l_sql #FUN-980092
  ...
  IF p_oha05 = '2' OR (p_oha05='3' AND p_i <> 0) THEN
      ...
      #調整尾差(會將差異捲至單身金額最大筆)
      CALL p886_up_omb(p_oma.oma01)  #CHI-BC0008 add
      CALL p886_ins_omc()  #MOD-C40063 add
  ...
  IF p_last_plant != p_plant THEN  #FUN-980092
       ...
       #調整尾差(會將差異捲至單身金額最大筆)
       CALL p886_up_apb(p_apa.apa01)  #CHI-BC0008 add
       CALL p886_ins_apc()  #MOD-C40063 add
...
FUNCTION p886_oma(p_oma01)
  ...
  DEFINE l_dbs     LIKE type_file.chr21    #No.FUN-680137 VARCHAR(21)             #FUN-710019 
...
FUNCTION p886_omb()
  ...
  CLOSE rvv_c5
     ...
     IF p_oma.oma213 = 'N' THEN
       ...
       CALL cl_digcut(p_omb.omb16,s_azi.azi04) RETURNING p_omb.omb16         #MOD-AC0007
#      LET p_omb.omb16t=p_omb.omb15*p_omb.omb12*(1+p_oma.oma211/100)         #MOD-AC0007
       LET p_omb.omb16t=p_omb.omb16*(1+p_oma.oma211/100)                     #MOD-AC0007
     ELSE
#MOD-AC0007 --begin--
...
#       LET p_omb.omb16t= p_omb.omb15*p_omb.omb12
        ...
        LET p_omb.omb16=p_omb.omb16t/(1+p_oma.oma211/100)
#MOD-AC0007 --end--
     END IF
...
FUNCTION p886_ins_omb()
    ...
    LET l_sql1="INSERT INTO ",cl_get_target_table(p_plant,'omb_file'), #FUN-A50102
               ...
               " omb35,omb36,omb930,omblegal)",   #,ombplant) ",   #MOD-920266 #FUN-960141 add ombplant  #FUN-960141 del ombplant 090824   #FUN-980010  
               " VALUES( ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
               "         ?,?,?,?, ?,?,?, ?,?,?,? ) "     #No.FUN-670026 #MOD-920266 #FUN-960141 add 1 ?  #FUN-960141 del 1 ? 090824  #FUN-980010  
...
FUNCTION p886_ins_apb()
    ...
    LET l_sql1="INSERT INTO ",cl_get_target_table(p_plant,'apb_file'), #FUN-A50102
               ...
               " apb29,apb251,apb34,apb930,apblegal) ",   #,apbplant) ",   #No.TQC-7B0083 #MOD-920266 #FUN-960141 add apbplant  #FUN-960141 del apbplant 090824  #FUN-980010
               " VALUES( ?,?,?,?, ?,?,?,?, ?, ?,?, ?, ?,?,?, ",
               "         ?,?,?,?, ?,?,?, ?,?, ?,?,?,?, ?,?,?,?,?)"  #No.TQC-7B0083 #MOD-920266 #FUN-960141 add 1 ?  #FUN-960141 del 1 ? 090824 #FUN-980010
 	 CALL cl_replace_sqldb(l_sql1) RETURNING l_sql1        #FUN-920032
     CALL cl_parse_qry_sql(l_sql1,p_plant) RETURNING l_sql1 #FUN-A50102
...
#                  EXECUTE del_omc 
# 
#           LET l_omc13 =p_oma.oma56t              #MOD-7C0183 
...
#                      "omc08,omc09,omc10,omc11,omc12,omc13,omc14,omc15,omclegal)",  #,omcplant)", #FUN-960141 add omcplant   #FUN-960141 del omcplant 090824   #FUN-980010
#                      " VALUES( ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?, ?)"  #FUN-960141 add  1 ? #FUN-960141 090824 del 1 ?  #FU-980010   #FUN-980010
# 	 CALL cl_replace_sqldb(l_sql1) RETURNING l_sql1        #FUN-920032
#     CALL cl_parse_qry_sql(l_sql1,p_plant) RETURNING l_sql1 #FUN-A50102
...
#                p_oma.oma10,l_omc13,'0','0', p_legal   #FUN-980010  add
# 
#           IF SQLCA.sqlcode<>0 THEN
...
FUNCTION p886_ar_npp(p_npptype)          #No.FUN-6A0026
...
FUNCTION p886_ins_npp()
    ...
    LET l_sql1="INSERT INTO ",cl_get_target_table(p_plant,'npp_file'), #FUN-A50102
              ...
              #" npp06,npp07,nppglno,npptype,nppqlegal ) ",      #No.FUN-6A0026  #FUN-980010 #MOD-D40090 mark
               " npp06,npp07,nppglno,npptype,npplegal ) ",      #No.FUN-6A0026  #FUN-980010 #MOD-D40090 add
...
FUNCTION p886_ar_npq(p_npqtype)
DEFINE p_npqtype LIKE npq_file.npqtype
DEFINE l_occ37   LIKE occ_file.occ37     #FUN-710019 
DEFINE l_occ02   LIKE occ_file.occ02     #CHI-820013
 
   DEFINE l_aag   RECORD LIKE aag_file.*
     CALL p886_def_npq()   #給分錄底稿單身檔預設值(每一單身共同之部份欄位)
     #讀取客戶簡稱
     LET l_occ37=NULL    
     ...
     ## 判斷應收客戶是否為關係人
     #CALL p886_occ37(p_npq.npq03,g_bookno,p_oma.oma03,p_npq.npq37,p_dbs)
    CALL p886_occ37(p_npq.npq03,g_bookno,p_oma.oma03,p_npq.npq37,p_dbs,p_plant)  #FUN-A50102 
     RETURNING p_npq.npq37
     #新增一筆分錄底稿單身檔
     CALL p886_ins_npq()                      #CHI-8C0034 mod
     #----- (Cr:銷貨收入) ----
      LET p_npq.npq02 = p_npq.npq02+1  #項次
      IF p_npqtype = '0' THEN
...
#新增分錄底稿單身檔
FUNCTION p886_ins_npq()                  #CHI-8C0034 mod
    ...
    LET l_sql1="INSERT INTO ",cl_get_target_table(p_plant,'npq_file'), #FUN-A50102
               ...
               " npq36,npq37,npqtype,npqlegal) ",       #FUN-980010	#MOD-9A0097 add npq36
               " VALUES( ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
               "         ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
     ...
     CALL cl_parse_qry_sql(l_sql1,p_plant) RETURNING l_sql1 #FUN-A50102
           ...
           EXECUTE ins_npq USING 
                ...
                p_npq.npq36,p_npq.npq37,p_npq.npqtype,p_legal           #FUN-980010	#MOD-9A0097 
           IF cl_sql_dup_value(SQLCA.SQLCODE) THEN
               #更新出貨單頭檔
               ...
               " npq15=?,npq21=?,npq22=?,npq23=?,",
               " npq24=?,npq25=?,npq26=?,npq27=?,",
               " npq28=?,npq29=?,npq30=?,npq37=?,npqtype=? ",
  ...
  CLOSE gec_c1
     ...
     CALL cl_parse_qry_sql(l_sql,p_plant) RETURNING l_sql #FUN-A50102
        ...
        CLOSE apt_c1
         ...
         LET p_apa.apa79  = 0               #No.FUN-A60024 add apa79
    ...
    LET l_sql1="INSERT INTO ",cl_get_target_table(p_plant,'apa_file'), #FUN-A50102
               ...
               " apamodu,apadate,apa100,apalegal,apaoriu,apaorig ) ", #FUN-A10036 #FUN-960141 add apaplant   #FUN-960141 090824  apaplant-->apa100 #FUN-980010
               " VALUES( ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
 	 ...
 	 CALL cl_replace_sqldb(l_sql1) RETURNING l_sql1        #FUN-920032
     CALL cl_parse_qry_sql(l_sql1,p_plant) RETURNING l_sql1 #FUN-A50102
           PREPARE inp886_apa FROM l_sql1
           EXECUTE inp886_apa USING 
                ...
                p_apa.apa79,       #No.FUN-A60024 add apa79
                p_apa.apa99,
...
#                      "(apc01,apc02,apc03,apc04,apc05,apc06,apc07,",   #no.381-07120003
#                      "apc08,apc09,apc10,apc11,apc12,apc13,apc14,apc15,apc16,apclegal)",  #,apcplant)", #FUN-960141 add apcplant   #FUN-960141 del apcplant #FUN-980010
...
FUNCTION p886_ap_npp(p_npptype)
...
FUNCTION p886_ap_npq(p_npqtype)
  ...
  DEFINE p_aag  RECORD LIKE aag_file.*
     ...
     #讀取會計科目
   IF p_apa.apa51 <> 'STOCK' THEN   #MOD-920185 add
     #MOD-BC0020 ----- mark start -----
     #LET l_sql = "SELECT * ",
     #            #" FROM ",p_dbs CLIPPED,"aag_file ",
     #            " FROM ",cl_get_target_table(p_plant,'aag_file'), #FUN-A50102
     #            " WHERE aag01 = ? AND aag00 = ? "      #No.FUN-730057
     ...
     END IF
        ...
        CALL s_def_npq_m(p_npq.npq03,'aapt210',p_npq.*,p_npq.npq01,'','',g_bookno,p_plant)   #No.FUN-980059
        RETURNING p_npq.*  
        CALL s_def_npq31_npq34_m(p_npq.*,g_bookno,p_plant)          #FUN-AA0087
        RETURNING p_npq.npq31,p_npq.npq32,p_npq.npq33,p_npq.npq34 #FUN-AA0087
        #新增一筆分錄底稿單身檔 
        CALL p886_ins_npq()  
   ELSE
     CALL p886_stock(p_apa.apa01,p_apa.apa66,p_npqtype) 
     LET p_npq.npq37 = NULL    
   END IF 
     #------( Dr: VAT tax        )---------------------
     ...
     RETURNING p_npq.npq31,p_npq.npq32,p_npq.npq33,p_npq.npq34 #FUN-AA0087
     ## 判斷應收廠商是否為關係人
     #CALL p886_pmc903(p_npq.npq03,g_bookno,p_apa.apa06,p_npq.npq37,p_dbs) 
     CALL p886_pmc903(p_npq.npq03,g_bookno,p_apa.apa06,p_npq.npq37,p_dbs,p_plant)  #FUN-A50102
     RETURNING p_npq.npq37
     #新增一筆分錄底稿單身檔
     CALL p886_ins_npq()                      #CHI-8C0034 mod
     LET p_npq.npq37 = NULL     
...
FUNCTION p886_stock(p_apa01,p_apa66,p_type) 
  ...
  DEFINE p_type        LIKE npq_file.npqtype   #FUN-710019
  DEFINE l_pmc903  LIKE pmc_file.pmc903        #FUN-710019 
  DEFINE l_pmc03       LIKE pmc_file.pmc03     #CHI-820013
 ...
 LET l_sql1 =" SELECT apb10,apb29,'',apb26,apb12,apb09,apb08,apb24,rvv32,rvv33",
 	 ...
 	 CALL cl_replace_sqldb(l_sql1) RETURNING l_sql1        #FUN-920032
         CALL cl_parse_qry_sql(l_sql1,p_plant) RETURNING l_sql1 #FUN-980092
  PREPARE p886_gl_p4 FROM l_sql1
  DECLARE p886_gl_c4 CURSOR FOR p886_gl_p4
  FOREACH p886_gl_c4 INTO l_amt,l_apb29,l_actno,l_deptno,
                          l_apb12,l_apb09,l_apb08,l_amt_f,l_ware,l_loc
     IF STATUS THEN EXIT FOREACH END IF
     IF cl_null(l_actno) THEN
        LET l_actno = p886_stock_act(l_apb12,l_ware,l_loc) 
     END IF
     LET g_npq.npqsys = 'AP'
     LET g_npq.npq00 = '1'
     ...
     ELSE
        LET g_npq.npq05 = ''
     END IF
     CALL s_def_npq_m(g_npq.npq03,'aapt210',g_npq.*,g_npq.npq01,'','',g_bookno,p_plant)  #MOD-820013 modify  #No.FUN-980059
     RETURNING g_npq.*  
     CALL s_def_npq31_npq34_m(p_npq.*,g_bookno,p_plant)          #FUN-AA0087
     RETURNING p_npq.npq31,p_npq.npq32,p_npq.npq33,p_npq.npq34 #FUN-AA0087
     ## 判斷應收廠商是否為關係人
     #CALL p886_pmc903(g_npq.npq03,g_bookno,p_apa.apa06,g_npq.npq37,p_dbs)
    CALL p886_pmc903(g_npq.npq03,g_bookno,p_apa.apa06,g_npq.npq37,p_dbs,p_plant)    #FUN-A50102
     RETURNING g_npq.npq37
     LET g_npq.npqtype = p_type   #FUN-710019
     IF g_npq.npq07!=0 THEN
         ...
         LET l_sql1="INSERT INTO ",cl_get_target_table(p_plant,'npq_file'), #FUN-A50102
                   ...
                   " npq15,npq21,npq22,npq23,",
                   " npq24,npq25,npq26,npq27,",
                   " npq28,npq29,npq30,npq37,npqtype,npqlegal) ",   #FUN-980010
                   " VALUES( ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                   "         ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,? ) "     #FUN-980010
 	 CALL cl_replace_sqldb(l_sql1) RETURNING l_sql1        #FUN-920032
     CALL cl_parse_qry_sql(l_sql1,p_plant) RETURNING l_sql1 #FUN-A50102
         ...
         EXECUTE ins_npq2 USING
              ...
              g_npq.npq28,g_npq.npq29,g_npq.npq30,g_npq.npq37,
              g_npq.npqtype,
              p_legal       #FUN-980010
         IF SQLCA.sqlcode<>0 THEN
          CALL cl_err('ins npq:',SQLCA.sqlcode,1)
         ...
         MESSAGE '>',g_npq.npq02,' ',g_npq.npq03,' ',g_npq.npq06,' ',
                  g_npq.npq07
         IF STATUS THEN
            CALL cl_err('p886_g_gl(ckp#1)',STATUS,1)
...
FUNCTION p886_stock_act(p_item,p_ware,p_loc)
...
#FUNCTION p886_occ37(l_npq03,l_bookno,l_oma03,l_npq37,l_dbs)
FUNCTION p886_occ37(l_npq03,l_bookno,l_oma03,l_npq37,l_dbs,l_plant)  #FUN-A50102
...
## 判斷應收廠商是否為關係人
#FUNCTION p886_pmc903(l_npq03,l_bookno,l_apa06,l_npq37,l_dbs)
FUNCTION p886_pmc903(l_npq03,l_bookno,l_apa06,l_npq37,l_dbs,l_plant)  #FUN-A50102
   DEFINE l_npq03  LIKE npq_file.npq03,   #科目 
          l_bookno LIKE aza_file.aza81,   #帳別
...
FUNCTION p886_up_omb(p_oma01) 
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql      
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql 
  PREPARE p886_prep91 FROM l_sql
  DECLARE p886_curs91 CURSOR FOR p886_prep91
  OPEN p886_curs91
  FETCH p886_curs91 INTO m_oma.*
  IF STATUS THEN 
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql        
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql 
  PREPARE p886_prep93 FROM l_sql
  DECLARE p886_curs93 CURSOR FOR p886_prep93
  OPEN p886_curs93
  FETCH p886_curs93 INTO l_omb14,l_omb16,l_omb18,l_omb18t
  ...
  IF m_oma.omaconf = 'Y' THEN
     ...
     CALL cl_replace_sqldb(l_sql) RETURNING l_sql       
     CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql 
     PREPARE p886_prep94 FROM l_sql
     DECLARE p886_curs94 CURSOR FOR p886_prep94
     OPEN p886_curs94
     FETCH p886_curs94 INTO l_cnt,l_oot05,l_oot05t
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql        
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql  
  PREPARE p886_prep95 FROM l_sql
  DECLARE p886_curs95 CURSOR FOR p886_prep95
  OPEN p886_curs95
  FETCH p886_curs95 INTO l_omb14,l_omb14t,l_omb16,l_omb16t,l_omb18,l_omb18t
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql     
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql  
  PREPARE p886_prep96 FROM l_sql
  DECLARE p886_curs96 CURSOR FOR p886_prep96
  FOREACH p886_curs96 INTO l_omb.* 
...
FUNCTION p886_up_apb(p_apa01) 
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql      
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql 
  PREPARE p886_prep82 FROM l_sql
  DECLARE p886_curs82 CURSOR FOR p886_prep82
  OPEN p886_curs82
  FETCH p886_curs82 INTO m_apa.*
  IF STATUS THEN 
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql        
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql 
  PREPARE p886_prep83 FROM l_sql
  DECLARE p886_curs83 CURSOR FOR p886_prep83
  OPEN p886_curs83
  FETCH p886_curs83 INTO l_apb24,l_apb10
  ...
  CALL cl_replace_sqldb(l_sql) RETURNING l_sql       
  CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql  
  PREPARE p886_prep84 FROM l_sql
  DECLARE p886_curs84 CURSOR FOR p886_prep84
  OPEN p886_curs84
  FETCH p886_curs84 INTO s_apa31f,s_apa31
  ...
  IF diff_apb24 <> 0  OR diff_apb10 <> 0 THEN 
     ...
     CALL cl_replace_sqldb(l_sql) RETURNING l_sql        
     CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql 
     PREPARE p886_prep86 FROM l_sql
     DECLARE p886_curs86 CURSOR FOR p886_prep86
     FOREACH p886_curs86 INTO m_apb.* 
       IF SQLCA.SQLCODE <> 0 THEN EXIT FOREACH END IF
