...
#FUN-AA0059---------mod------------end-----------------
                     ...
                     WHEN INFIELD(opc08)
                       ...
                       NEXT FIELD opc08
                     WHEN INFIELD(opcslk01)
                       CALL cl_init_qry_var()
                       LET g_qryparam.form = "q_opcslk01"
                       LET g_qryparam.state = 'c'
                       CALL cl_create_qry() RETURNING g_qryparam.multiret
                       DISPLAY g_qryparam.multiret TO opcslk01
                       NEXT FIELD opcslk01 
                END CASE
...
#FUN-AA0059 ---------------------end-------------------------------
        ...
        AFTER FIELD opc09
          IF NOT cl_null(g_opc.opc08) THEN
              ...
              END IF
          END IF
        AFTER FIELD opcslk01
          IF NOT cl_null(g_opc.opcslk01) THEN
              SELECT count(*) INTO l_n1 FROM skd_file 
                WHERE skd01=g_opc.opcslk01 AND skd04='Y'
              IF l_n1 = 0  THEN 
...
#FUN-AA0059---------mod------------end-----------------
                ...
                WHEN INFIELD(opcslk01)
                  CALL cl_init_qry_var()
                  LET g_qryparam.form = "q_opcslk01"
                  CALL cl_create_qry() RETURNING g_opc.opcslk01
                  DISPLAY g_opc.opcslk01 TO opcslk01
                  NEXT FIELD opcslk01
           END CASE
        ON ACTION CONTROLR
