...
#
# Pattern name...: axmr559.4gl
# Descriptions...: Packing List
# Date & Author..: 12/09/26 NO.FUN-C90100 By xujing
...
MAIN
   ...
   LET l_table = cl_prt_temptable('axmr559',g_sql) CLIPPED   # 產生Temp Table
   ...
   LET l_table1 = cl_prt_temptable('axmr5591',g_sql) CLIPPED   # 產生Temp Table
   ...
   IF cl_null(tm.wc) THEN
      CALL axmr559_tm(0,0)             # Input print condition
   ELSE 
      LET tm.wc = " ofa011='",tm.wc CLIPPED,"'"
      CALL axmr559()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
END MAIN
 
FUNCTION axmr559_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,         #No.FUN-680137 SMALLINT
   ...
   OPEN WINDOW axmr559_w AT p_row,p_col WITH FORM "axm/42f/axmr559"
   ...
   WHILE TRUE
      ...
      IF INT_FLAG THEN
         LET INT_FLAG = 0 CLOSE WINDOW axmr559_w 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
         EXIT PROGRAM
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('axmr559',g_time,l_cmd)    # Execute cmd at later time
         END IF
         CLOSE WINDOW axmr559_w
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
         EXIT PROGRAM
      END IF
      CALL cl_wait()
...
FUNCTION axmr559()
   ...
   PREPARE axmr559_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare1:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   DECLARE axmr559_curs1 CURSOR FOR axmr559_prepare1

display time 
   FOREACH axmr559_curs1 INTO  sr.*,l_ofa10,l_ofa45,l_ofa46
      IF SQLCA.sqlcode != 0 THEN
...
#      LET sr.zo042 = g_zo042
      ...
      PREPARE  axmr559_prepare2 FROM l_sql
#      DECLARE  axmr559_curs2 CURSOR FOR axmr559_prepare2
      LET l_sql1 = "INSERT INTO ",g_cr_db_str CLIPPED,l_table1 CLIPPED, 
                   " ",l_sql
      PREPARE  axmr559_prepare3 FROM l_sql1
      EXECUTE  axmr559_prepare3
{      FOREACH axmr559_curs2 INTO sr1.*
         IF SQLCA.sqlcode != 0 THEN
            CALL cl_err('foreach:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
...
display time 
   ...
   END IF
   LET g_str = tm.wc
   CALL cl_prt_cs3('axmr559','axmr559',g_sql,g_str)
   #------------------------------ CR (4) ------------------------------#
