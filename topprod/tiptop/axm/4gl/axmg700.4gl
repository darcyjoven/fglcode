...
# Modify.........: No.TQC-C90072 12/09/13 By dongsz 傳參g_zz05并漢化列印條件
...
TYPE sr1_t RECORD
    ...
    oak02 LIKE azf_file.azf03,
    occ01 LIKE occ_file.occ01,
    occ02 LIKE occ_file.occ02,
    occ18_oha03 LIKE occ_file.occ18,
    occ18_oha04 LIKE occ_file.occ18,
...
#             "ima021.ima_file.ima021"    #No.FUN-860026
    ...
    LET l_table = cl_prt_temptable('axmg700',g_sql) CLIPPED
    IF l_table = -1 THEN 
  ...
  #CHI-7B0002---str---add---
   ...
   LET l_table1 = cl_prt_temptable('axmg7001',g_sql) CLIPPED   # 產生Temp Table
   IF l_table1 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-C10036  mark
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)   #FUN-B50018  add  #FUN-C10036  mark
      EXIT PROGRAM 
...
#No.FUN-860026---begin                                                                                                              
   ...
   LET l_table2 = cl_prt_temptable('axmg7002',g_sql) CLIPPED                                                                        
   IF  l_table2 = -1 THEN 
       #CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-C10036  mark
       #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)   #FUN-B50018  add  #FUN-C10036  mark
       EXIT PROGRAM 
...
#No.FUN-860026---end 
   #No.FUN-A80103 --start--
    LET g_sql = "imc01.imc_file.imc01,",
                ...
                "ohb03.ohb_file.ohb03"
    LET l_table3 = cl_prt_temptable('axmg7003',g_sql) CLIPPED
    IF  l_table3 = -1 THEN 
        #CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-C10036  mark
        #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)   #FUN-B50018  add  #FUN-C10036  mark
        EXIT PROGRAM 
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW axmg700_w AT p_row,p_col WITH FORM "axm/42f/axmg700"
...
#FUN-C10036----END----- 
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmg700_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)
      EXIT PROGRAM
...
#No.FUN-A80103---END  
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmg700',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmg700_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2||"|"||l_table3)
      EXIT PROGRAM
   END IF
...
FUNCTION axmg700()
   ...
   DEFINE    l_lang_t    LIKE type_file.chr1   #TQC-C90072 add
...
#No.FUN-860026---end    
   ...
   #l_table1
    LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,             
                ...
                "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
#                "       ?,?,?,?)"  #No.FUN-860026
                "        ?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?)"# No.FUN-C40020 add4?   #No.FUN-860026  #FUN-A80103 add 2? #CHI-AB0003 add 3? #CHI-C30127
     PREPARE insert_prep FROM g_sql                                                                                                  
     IF STATUS THEN        
...
#No.FUN-860026---END   
    ...
    #SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'axmg700'    #No.FUN-710090 mark
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'axmg700'    #No.FUN-710090  
     #Begin:FUN-980030
   ...
   # LET l_sql="SELECT oha01,oaydesc,oha02,oha14,oha15,oha03,oha032,oha04, ",
     ...
     PREPARE axmg700_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        ...
        EXIT PROGRAM
     END IF
     DECLARE axmg700_curs1 CURSOR FOR axmg700_prepare1
     ...
     DECLARE r920_d  CURSOR  FOR
        SELECT rvbs03,rvbs04,rvbs06,rvbs021  FROM rvbs_file
         WHERE rvbs01 =? AND rvbs02 = ? ORDER BY  rvbs04
     ...
     #FUN-C50008--add--end 
#No.FUN-710090--begin--
#     CALL cl_outnam('axmg700') RETURNING l_name
##No.FUN-580004-begin
...
#      CALL cl_prt_pos_len()
##No.FUN-580004-end
#     START REPORT axmg700_rep TO l_name
#    LET g_pageno = 0
#No.FUN-710090--end--
 
     FOREACH axmg700_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
      ...
      #FUN-C50008--mark--end
       FOREACH oao_c1 USING sr.oha01,sr.ohb03 INTO oao.*   #FUN-C50008 add                                               
...
#FUN-C50008--mark--end
    FOREACH  r920_d USING sr.oha01,sr.ohb03 INTO l_rvbs.*   #FUN-C50008 add
          LET flag =1                                                                                                  
...
#FUN-C50008--mark--end
          FOREACH imc_cur USING sr.ohb04,sr.ohb07 INTO sr1.*   #FUN-C50008 add                        
     ...
     #No.FUN-A80103  --end   
      ...
      #No.FUN-710090--end--
      #OUTPUT TO REPORT axmg700_rep(sr.*)    #No.FUN-710090 mark
     END FOREACH
    ...
    #CHI-7B0002---end---add---
     ...
     IF SQLCA.sqlcode != 0 THEN      
        ...
        EXIT PROGRAM  
     END IF           
     DECLARE g700_cs2 CURSOR FOR g700_prepare1
   ...
   #FUN-C50008--add--end
     ...
     FOREACH g700_cs2 INTO sr.oha01
        #整張前備註
    #FUN-C50008--mark--str
      #  DECLARE oao_c2 CURSOR FOR  
      #   SELECT * FROM oao_file
    ...
    #FUN-C50008--mark--end
        FOREACH oao_c2 USING sr.oha01 INTO oao.*   #FUN-C50008 add
           IF NOT cl_null(oao.oao06) THEN
              EXECUTE insert_prep1 USING
...
#No.FUN-860026---end
    #CHI-7B0002---end---add---
#No.TQC-C90072 add---sart
   IF g_zz05='Y' THEN
      LET l_lang_t  = g_lang  
      LET g_lang    = g_rlang
      CALL cl_wcchp(tm.wc,'oha01') RETURNING tm.wc
      LET g_lang    = l_lang_t 
   END IF
#No.TQC-C90072 add---end
 #FUN-C50008--mark--str
 #    IF g_zz05='Y' THEN
 ...
 #    END IF
 #FUN-C50008--mark--end
###GENGRE###     LET g_str = tm.wc,";",tm.b,";",tm.c   #No.FUN-860026  add tm.b #FUN-A80103 add tm.c
   # CALL cl_prt_cs3('axmg700',g_sql,g_str)    #TQC-730088
###GENGRE###     CALL cl_prt_cs3('axmg700','axmg700',g_sql,g_str)
    LET g_cr_table = l_table                    # No.FUN-C40020 add
    LET g_cr_apr_key_f = "oha01"                    # No.FUN-C40020 add
    CALL axmg700_grdata()    ###GENGRE###
...
FUNCTION axmg700_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("axmg700")
        IF handler IS NOT NULL THEN
            START REPORT axmg700_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " ORDER BY oha01,ohb03"
          
            DECLARE axmg700_datacur1 CURSOR FROM l_sql
            FOREACH axmg700_datacur1 INTO sr1.*
                OUTPUT TO REPORT axmg700_rep(sr1.*)
            END FOREACH
            FINISH REPORT axmg700_rep
        END IF
...
REPORT axmg700_rep(sr1)
    ...
    #FUN-B50018----add-----str-----------------
    DEFINE  l_sql                 STRING
    DEFINE  l_flag                LIKE oha_file.oha01
    DEFINE  l_oha03_occ18         STRING
    DEFINE  l_oha04_occ18         STRING
    DEFINE  l_oha14_gen02         STRING
    DEFINE  l_oha15_gem02         STRING
    DEFINE  l_ohb09_ohb091_ohb092 STRING
    DEFINE  l_oah09               STRING
    ...
    DEFINE  l_oah05               STRING  #CHI-C30127
    #FUN-B50018----add-----end-----------------
    DEFINE l_display5          LIKE type_file.chr1 #DEV-D40005

    ORDER EXTERNAL BY sr1.oha01
    ...
    FORMAT
        FIRST PAGE HEADER
            ...
            PRINTX tm.*
            PRINTX g_zz05                     #TQC-C90072 add
              
        BEFORE GROUP OF sr1.oha01
            LET l_lineno = 0

            #FUN-B50018----add-----str-----------------
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE oao01 = '",sr1.oha01 CLIPPED,"'",
                        " AND oao03 = 0 AND oao05='1'"
            START REPORT axmg700_subrep01
            DECLARE axmg700_repcur1 CURSOR FROM l_sql
            FOREACH axmg700_repcur1 INTO sr2.*
                OUTPUT TO REPORT axmg700_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT axmg700_subrep01
            #FUN-B50018----add-----end----------------- 

            #CHI-C30127---begin
            LET  l_oah05 = cl_gr_getmsg("gre-326",g_lang,sr1.oha05)  
            LET  l_oah05 = '(',l_oah05,')'
            PRINTX  l_oah05   
            #CHI-C30127---end
        ...
        ON EVERY ROW
            ...
            #FUN-B50018----add-----str-----------------
            IF  sr1.flag > sr1.oha01 THEN
                LET  l_flag = sr1.flag
            ELSE
                LET  l_flag = sr1.oha01
            END IF
            PRINTX  l_flag

            IF NOT cl_null(sr1.oha03) OR NOT cl_null(sr1.occ18_oha03) THEN
               LET l_oha03_occ18 = sr1.oha03,' ',sr1.occ18_oha03
            END IF
            PRINTX  l_oha03_occ18

            IF NOT cl_null(sr1.oha04) OR NOT cl_null(sr1.occ18_oha04) THEN
               LET l_oha04_occ18 = sr1.oha04,' ',sr1.occ18_oha04
            END IF
            PRINTX  l_oha04_occ18

            IF NOT cl_null(sr1.oha14) THEN
               LET l_oha14_gen02 = sr1.oha14,' ',sr1.gen02
            END IF
            PRINTX  l_oha14_gen02

            IF NOT cl_null(sr1.oha15) THEN
               LET l_oha15_gem02 = sr1.oha15,' ',sr1.gem02
            END IF
            PRINTX  l_oha15_gem02

            LET  l_ohb09_ohb091_ohb092 = '(',sr1.ohb09,'/',sr1.ohb091,'/',sr1.ohb092,')'
            PRINTX  l_ohb09_ohb091_ohb092
           
            IF NOT cl_null(sr1.oha09) THEN
               LET  l_oah09 = cl_gr_getmsg("gre-004",g_lang,sr1.oha09)
            END IF
            PRINTX  l_oah09
            
            LET l_unit_ep = cl_gr_getmsg("gre-005",g_lang,'0')
            IF NOT cl_null(sr1.unit_ep) THEN
               LET l_unit = l_unit_ep,sr1.unit_ep
            ELSE 
               LET l_unit = l_unit_ep
            END IF
            PRINTX  l_unit

            LET l_sql = "SELECT MAX(flag) FROM ",g_cr_db_str CLIPPED,l_table CLIPPED,
                        " WHERE oha01='",sr1.oha01 CLIPPED,"'"
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        ...
                        " AND oao03 = ",sr1.ohb03,
                        " AND oao05='1'"
            START REPORT axmg700_subrep02
            DECLARE axmg700_repcur2 CURSOR FROM l_sql
            FOREACH axmg700_repcur2 INTO sr2.*
                OUTPUT TO REPORT axmg700_subrep02(sr2.*)
            END FOREACH
            FINISH REPORT axmg700_subrep02

            

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE imc01 = '",sr1.ohb04 CLIPPED,"'",
                        " AND imc02 = '",sr1.ohb07,"'"
            START REPORT axmg700_subrep06
            DECLARE axmg700_repcur6 CURSOR FROM l_sql
            FOREACH axmg700_repcur6 INTO sr4.*
                OUTPUT TO REPORT axmg700_subrep06 (sr4.*)
            END FOREACH
            FINISH REPORT axmg700_subrep06

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE oao01 = '",sr1.oha01 CLIPPED,"'",
                        " AND oao03 = ",sr1.ohb03,
                        " AND oao05='2'"
            START REPORT axmg700_subrep03
            DECLARE axmg700_repcur3 CURSOR FROM l_sql
            FOREACH axmg700_repcur3 INTO sr2.*
                OUTPUT TO REPORT axmg700_subrep03 (sr2.*)
            END FOREACH
            FINISH REPORT axmg700_subrep03


            IF cl_null(sr1.ima021) AND  cl_null(sr1.unit_ep) THEN
               LET l_display1 = 'N'
            ELSE
               LET l_display1 = 'Y'
            END IF
        ...
        AFTER GROUP OF sr1.oha01
            ...
            START REPORT axmg700_subrep04
            DECLARE axmg700_repcur4 CURSOR FROM l_sql
            FOREACH axmg700_repcur4 INTO sr2.*

                OUTPUT TO REPORT axmg700_subrep04(sr2.*)
            END FOREACH
            FINISH REPORT axmg700_subrep04

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,
                        " WHERE rvbs01 = '",sr1.oha01 CLIPPED,"'",
                        " AND rvbs02 = ",sr1.ohb03
            START REPORT axmg700_subrep05
            DECLARE axmg700_repcur5 CURSOR FROM l_sql
            FOREACH axmg700_repcur5 INTO sr3.*
                OUTPUT TO REPORT axmg700_subrep05(sr3.*)
            END FOREACH
            FINISH REPORT axmg700_subrep05
            #FUN-B50018----add-----end-----------------      
...
REPORT axmg700_subrep05(sr3)
    DEFINE sr3 sr3_t
    DEFINE l_rvbs06_sum  LIKE rvbs_file.rvbs06
    ORDER EXTERNAL BY sr3.rvbs01
    ...
    FORMAT
        ...
        AFTER GROUP OF sr3.rvbs01
            LET l_rvbs06_sum = GROUP SUM(sr3.rvbs06)
            PRINTX l_rvbs06_sum

END REPORT


REPORT axmg700_subrep06(sr4)
    DEFINE sr4 sr4_t
