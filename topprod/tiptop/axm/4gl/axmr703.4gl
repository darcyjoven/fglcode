...
# Modify.........: No.TQC-D10014 13/01/05 By qirl QBE條件欄位增加開窗
...
#--------------------------CR(1)--------------------------------#                                                                   
   LET g_sql = " ofd23.ofd_file.ofd23,",                                                                                            
    ...
    LET l_table = cl_prt_temptable('axmr703',g_sql) CLIPPED   # 產生Temp Table                                                      
...
#--------------------------CR(1)--------------------------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'    # If background job sw is off
      ...
      ELSE CALL r703()          # Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW r703_w AT p_row,p_col WITH FORM "axm/42f/axmr703"
      ...
      ON ACTION CONTROLP
           CASE
              ...
              WHEN INFIELD(ofd22)
                 CALL cl_init_qry_var()
                 LET g_qryparam.state = "c"
                 LET g_qryparam.form = "q_ofd22"
                 CALL cl_create_qry() RETURNING g_qryparam.multiret
                 DISPLAY g_qryparam.multiret TO ofd22
                 NEXT FIELD ofd22
              WHEN INFIELD(ofd23)
                 CALL cl_init_qry_var()
                 LET g_qryparam.state = "c"
...
#--TQC-D10014--add--end---
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW r703_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmr703',g_time,l_cmd)      # Execute cmd at later time
      END IF
      CLOSE WINDOW r703_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
...
#    FOR g_i = 1 TO g_len LET g_dash[g_i,g_i] = '=' END FOR                      # No.FUN-850014
     ...
     PREPARE r703_prepare FROM l_sql
     ...
     END IF
     DECLARE r703_cs CURSOR FOR r703_prepare
 
#    CALL cl_outnam('axmr703') RETURNING l_name                                       #No.FUN-850014  
#    START REPORT r703_rep TO l_name                                                  #No.FUN-850014 
 
#    LET g_pageno = 0                                                                 #No.FUN-850014 
     FOREACH r703_cs INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
          CALL cl_err('foreach:',SQLCA.sqlcode,1) EXIT FOREACH
       END IF
#      OUTPUT TO REPORT r703_rep(sr.*)                                                #No.FUN-850014  
...
#    FINISH REPORT r703_rep                                                           #No.FUN-850014 
...
#No.FUN-850014--end                                                                                                                 
 ## **** 與 Crystal Reports 串聯段 - <<<< CALL cs3() >>>> FUN-850014 **** ##                                                        
    ...
    CALL cl_prt_cs3('axmr703','axmr703',l_sql,g_str)                                                                                
    #------------------------------ CR (4) ------------------------------#
