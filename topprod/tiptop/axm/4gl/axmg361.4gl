...
#
# Pattern name...: axmg361.4gl
# Descriptions...: Print QUOTATION 
# Input parameter:
# Return code....:
# Date & Author..: No.FUN-6C0005 06/12/05 By rainy  ref.axmg361
...
# Modify.........: No.FUN-CC0087 12/12/14 By yangtt 4rp中，oqt53,oqt54,oqt13,oqt141,oqt142,oqt143為空則抑制顯示
...
#            "oao06_1.oao_file.oao06,oao06_3.oao_file.oao06,",#FUN-810029 mark
   ...
   LET l_table1 = cl_prt_temptable('axmg361',g_sql) CLIPPED
   IF l_table1 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add   #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)    #FUN-B50018  add  #FUN-C10036   mark
      EXIT PROGRAM 
   END IF
...
#            "oao06_2.oao_file.oao06"   #FUN-810029 mark
   LET l_table2 = cl_prt_temptable('axmg3612',g_sql) CLIPPED
   IF l_table2 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add   #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)    #FUN-B50018  add   #FUN-C10036   mark
      EXIT PROGRAM 
   ...
   LET l_table3 = cl_prt_temptable('axmg3613',g_sql) CLIPPED
   IF l_table3 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add   #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)    #FUN-B50018  add   #FUN-C10036   mark
      EXIT PROGRAM
  ...
  #l_table4  備註
   ...
   LET l_table4 = cl_prt_temptable('axmg3614',g_sql) CLIPPED   # 產生Temp Table
   IF l_table4 = -1 THEN 
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time   #FUN-B50018  add   #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)    #FUN-B50018  add  #FUN-C10036   mark
      EXIT PROGRAM
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N' THEN    # If background job sw is off
      CALL g361_tm(0,0)	        	     # Input print condition
   ELSE
      CALL g361()			     # Read data and create out-file
   END IF
 
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126

   CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)
END MAIN
 
FUNCTION g361_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW g361_w AT p_row,p_col WITH FORM "axm/42f/axmg361"
   ...
   WHILE TRUE
      ...
      IF g_bgjob = 'Y' THEN
         ...
         ELSE
            ...
            CALL cl_cmdat('axmg361',g_time,l_cmd)	# Execute cmd at later time
         END IF
...
DEFINE    l_img_blob LIKE    type_file.blob        #FUN-C40036 add
  ...
  #公司地址zo041、公司電話zo05、公司傳真zo09
   ...
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='axmg361'
 ...
 #               " WHERE oqu01 = ? ",                #MOD-4A0256
  ...
  #LET l_sql3  = " SELECT oao04,oao06 FROM oao_file",   #FUN-810029 mark
   ...
   LET l_sql6  = " SELECT oqx01,oqx03,oqx04,oaj02,oqx041,oqx05,oqx06 ",
                #" FROM oqx_file,OUTER oaj_file ",   #FUN-C50008 mark--
                 " FROM oqx_file LEFT OUTER JOIN oaj_file ON oqx04=oaj01 ", #FUN-C50008 add LEFT OUTER 
                 " WHERE oqx01 = ? AND oqx03 BETWEEN ? AND ? ",
                #" AND oqx_file.oqx04=oaj_file.oaj01  ORDER BY 1" #FUN-C50008 mark--
                 "  ORDER BY 1"  #FUN-C50008 add
 
   PREPARE g361_p1 FROM l_sql1
   IF SQLCA.sqlcode THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g361_c1 CURSOR FOR g361_p1
 
   PREPARE g361_p2 FROM l_sql2
   IF SQLCA.sqlcode THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)
      EXIT PROGRAM
   END IF
   DECLARE g361_c2 CURSOR FOR g361_p2
 
   PREPARE g361_p3 FROM l_sql3
   IF SQLCA.sqlcode THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)
      EXIT PROGRAM
   END IF
   DECLARE g361_c3 CURSOR FOR g361_p3
 
   PREPARE g361_p4 FROM l_sql4
   IF SQLCA.sqlcode THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)
      EXIT PROGRAM
   END IF
   DECLARE g361_c4 CURSOR FOR g361_p4
 
   PREPARE g361_p5 FROM l_sql5
   IF SQLCA.sqlcode THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)
      EXIT PROGRAM
   END IF
   DECLARE g361_c5 CURSOR FOR g361_p5
 
   PREPARE g361_p6 FROM l_sql6
   IF SQLCA.sqlcode THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      CALL cl_gre_drop_temptable(l_table1||"|"||l_table2||"|"||l_table3||"|"||l_table4)
      EXIT PROGRAM
   END IF
   DECLARE g361_c6 CURSOR FOR g361_p6
   ## Modify Bug No.:7252
 
  #CALL cl_outnam('axmg361') RETURNING l_name  #No.FUN-710090 mark
   OPEN g361_c1 USING tm.b
  ...
  #    #START REPORT g361_rep TO l_name  #FUN-690132
  # ELSE
  #    LET g_zaa[38].zaa06='Y'
  #    #START REPORT g361_rep TO l_name   #FUN-690032
  # END IF
  ...
  # #No.FUN-710090--end-- mark
   ...
   FOREACH g361_c1 USING tm.b INTO sr1.*
      IF SQLCA.sqlcode THEN
     ...
     #No.FUN-710090--begin-- add
      ...
      IF l_n > 0 THEN
         LET l_oao03 = 0
         FOREACH g361_c3 USING sr1.oqt01,l_oao03 INTO oao.*
      ...
      IF l_n > 0 THEN
         LET l_oqx03_1=0
         FOREACH g361_c6 USING sr1.oqt01,l_oqx03_1,l_oqx03_1 INTO sr3.*
            IF SQLCA.sqlcode THEN EXIT FOREACH END IF
     ...
     #No.FUN-710090--end-- add
      ...
      FOREACH g361_c2 USING sr1.oqt01 INTO sr2.*
         IF SQLCA.sqlcode THEN
        ...
        #   OUTPUT TO REPORT g361_rep(sr1.*,sr2.*,l_oqt01_t)
        #   LET g_type= 'N'
        #END IF
        ...
        #END IF
         ...
         IF l_n > 0 THEN
            FOREACH g361_c3 USING sr1.oqt01,sr2.oqu02 INTO oao.*
               IF SQLCA.sqlcode THEN EXIT FOREACH END IF
               IF NOT cl_null(oao.oao06) THEN
                  EXECUTE insert4 USING
         ...
         IF NOT cl_null(tm.a) AND sr2.oqu03[1,4] !='MISC'  THEN  
            LET i=1
            FOREACH g361_c4 USING sr2.oqu03,tm.a INTO l_imc04[i]
               IF SQLCA.sqlcode THEN EXIT FOREACH END IF
               LET i = i +1
            END FOREACH
         END IF
         ...
         #讀取報價單-分量計價資料
         LET i = 1
         FOREACH g361_c5 USING sr1.oqt01,sr2.oqu02 INTO l_oqv[i].*
            IF SQLCA.sqlcode THEN
               EXIT FOREACH
            END IF
            LET i = i +1
     ...
     #No.FUN-710090--begin--
      ...
      LET l_oqx03_2=100
      LET l_n=0
      FOREACH g361_c6 USING sr1.oqt01,l_oqx03_1,l_oqx03_2 INTO sr3.*
         IF SQLCA.sqlcode THEN EXIT FOREACH END IF
         IF cl_null(sr3.oqx05_1) OR sr3.oqx05_1 = 0 THEN
            CONTINUE FOREACH
         ELSE
...
#            "   AND A.oqt01=C.oqx01(+) ",
#            "   AND A.oqt01=D.oqx01(+) "
###GENGRE###   LET g_sql="SELECT A.*,B.obk03,B.oqu02,B.oqu03,B.oqu031,B.oqu032,",
###GENGRE###             "       B.oqu05,B.oqu06,B.oqu07,B.oqu09,B.oqu10,B.oqu11,B.oqu13 ",
###GENGRE###             "  FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED," A",
###GENGRE###             "      ,",g_cr_db_str CLIPPED,l_table2 CLIPPED," B",
###GENGRE###             " WHERE A.oqt01=B.oqu01","|",
###GENGRE###             "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
###GENGRE###             " WHERE oqx03 =0","|",
###GENGRE###             "SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
###GENGRE###             " WHERE oqx03!=0","|",
###GENGRE###             "SELECT oqx01,oqx041,azi04,SUM(oqx05) oqx05 ",
###GENGRE###             "  FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
###GENGRE###             " WHERE oqx03!=0",
###GENGRE###             " GROUP BY oqx01,oqx041,azi04","|",
...
###GENGRE###             " WHERE oao03 =0 AND oao05='2'"
    ...
    ELSE
       LET tm.wc=''
    END IF
###GENGRE###    LET g_str = tm.wc,";",tm.d
  # CALL cl_prt_cs3('axmg361',g_sql,g_str)   #TQC-730088
###GENGRE###    CALL cl_prt_cs3('axmg361','axmg361',g_sql,g_str)

    LET g_cr_table = l_table1                   #主報表的temp table名稱    #FUN-C40036 add
    LET g_cr_apr_key_f = "oqt01"       #報表主鍵欄位名稱，用"|"隔開        #FUN-C40036 add

    CALL axmg361_grdata()    ###GENGRE###
...
#No.FUN-710090--begin-- mark
#REPORT g361_rep(sr1,sr2,l_oqt01_t)
...
#               LET l_oao03 = 0
#               LET l_oao05 = '1'
#               FOREACH g361_c3 USING sr1.oqt01,l_oao03,l_oao05
...
#            LET l_oao05='1'
#            FOREACH g361_c3 USING sr1.oqt01,l_oao03,l_oao05 INTO l_oao04,l_oao06
#               IF SQLCA.sqlcode THEN
#                  EXIT FOREACH
#               END IF
#               IF cl_null(l_oao06) THEN
...
#         LET l_oqx03_2=100
#         LET l_n=0
#         FOREACH g361_c7 USING sr1.oqt01,l_oqx03_1,l_oqx03_2 INTO sr4.*
#            IF SQLCA.sqlcode THEN
#               EXIT FOREACH
#            END IF
...
FUNCTION axmg361_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("axmg361")
        IF handler IS NOT NULL THEN
            START REPORT axmg361_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED," A LEFT OUTER JOIN ",
                        g_cr_db_str CLIPPED,l_table2 CLIPPED," B ON  A.oqt01 = B.oqu01 ",
                        " ORDER BY oqt01"
            DECLARE axmg361_datacur1 CURSOR FROM l_sql
            fOREACH axmg361_datacur1 INTO sr1.*,sr2.*
                OUTPUT TO REPORT axmg361_rep(sr1.*,sr2.*)
            END FOREACH
            FINISH REPORT axmg361_rep
        END IF
...
REPORT axmg361_rep(sr1,sr2)
    ...
    DEFINE l_oqx03_oqx041_oqx08_oqx06  STRING
    DEFINE l_sql                       STRING                         
    DEFINE l_oqu11_str                 STRING
    DEFINE l_oqu13_str                 STRING
    DEFINE l_oqu11_num                 LIKE oqu_file.oqu11
    DEFINE l_oqu13_num                 LIKE oqu_file.oqu13       
    DEFINE l_s                         STRING 
    DEFINE l_oqx041                    STRING                           
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.oqt01
            ...
            " WHERE oqx01 = '",sr1.oqt01,"' AND oqx03 =0 ",
            " AND oqx04=oaj01  ORDER BY 1"
            START REPORT axmg361_subrep01
            DECLARE axmg361_subrep01 CURSOR FROM l_sql
            FOREACH axmg361_subrep01 INTO sr3.*
                OUTPUT TO REPORT axmg361_subrep01 (sr3.*)
            END FOREACH
            FINISH REPORT axmg361_subrep01

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table4 CLIPPED,
                        " WHERE oao01 = '",sr1.oqt01 CLIPPED,"'",
                        " AND oao03 = 0 AND oao05='1'"
            START REPORT axmg361_subrep04
            DECLARE axmg361_repcur4 CURSOR FROM l_sql
            FOREACH axmg361_repcur4 INTO sr4.*
                OUTPUT TO REPORT axmg361_subrep04(sr4.*)
            END FOREACH
            FINISH REPORT axmg361_subrep04
            #FUN-B50018----add-----end---------------
        ...
        ON EVERY ROW
            ...
            #FUN-B50018----add-----str-----------------
            SELECT oqu11,oqu13 INTO l_oqu11_num,l_oqu13_num FROM oqu_file 
              WHERE oqu01 = sr1.oqt01 AND oqu02 = sr2.oqu02
            LET l_oqu11_str = l_oqu11_num   USING '--,---,---,---,---,--&.&&'
            LET l_oqu13_str = l_oqu13_num   USING '--,---,---,---,---,--&.&&'
            IF NOT cl_null(l_oqu11_num) AND NOT cl_null(l_oqu13_num) THEN
	       LET l_oqu11_oqu13 = l_oqu11_str,'/',l_oqu13_str     
            ELSE 
               LET l_oqu11_oqu13 = ' '
            END IF  
            ...
            PRINTX l_oqx03_oqx041_oqx08_oqx06                            
            PRINTX sr1.*

            IF NOT cl_null(sr1.oqt20) AND sr1.oqt20 != ' ' THEN
               LET l_display1 = 'Y' 
            ELSE 
               LET l_display1 = 'N'
            END IF
            ...
            IF tm.d = 'Y' AND NOT cl_null(sr2.obk03) THEN
               LET l_display2 = 'Y'
            ELSE
               LET l_display2 = 'N' 
            END IF
...
#                       " WHERE oqu01 ='",sr1.oqt01 CLIPPED,"'",
#                       " ORDER BY oqu02 "
#          
#           DECLARE axmg361_cur CURSOR FROM l_sql
#           FOREACH axmg361_cur INTO sr2.*
...
#           END FOREACH 
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table4 CLIPPED,
                        " WHERE oao01 = '",sr2.oqu01 CLIPPED,"'",
                        " AND oao03 = '",sr4.oao06 CLIPPED,"'",
                        " AND oao05='1'"
            START REPORT axmg361_subrep05
            DECLARE axmg361_repcur5 CURSOR FROM l_sql
            FOREACH axmg361_repcur5 INTO sr4.*
                OUTPUT TO REPORT axmg361_subrep05(sr4.*)
            END FOREACH
            FINISH REPORT axmg361_subrep05
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table4 CLIPPED,
                        ...
                        " AND oao03 = '",sr4.oao06 CLIPPED,"'",
                        " AND oao05='2'"
            START REPORT axmg361_subrep06
            DECLARE axmg361_repcur6 CURSOR FROM l_sql
            FOREACH axmg361_repcur6 INTO sr4.*
                OUTPUT TO REPORT axmg361_subrep06 (sr4.*)
            END FOREACH
            FINISH REPORT axmg361_subrep06
            #FUN-B50018----add-----end---------------
        ...
        AFTER GROUP OF sr1.oqt01
            ...
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table4 CLIPPED,
                        " WHERE oao01 = '",sr3.oqx01 CLIPPED,"'",
                        " AND oao03 = 0 AND oao05='2'"
            START REPORT axmg361_subrep07
            DECLARE axmg361_repcur7 CURSOR FROM l_sql
            FOREACH axmg361_repcur7 INTO sr4.*

                OUTPUT TO REPORT axmg361_subrep07(sr4.*)
            END FOREACH
            FINISH REPORT axmg361_subrep07


            LET l_sql = " SELECT * ",
                 " FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                 " WHERE oqx01 = '",sr1.oqt01 CLIPPED,"'", 
                 " AND oqx03 BETWEEN 1 AND 100 ",
                 " ORDER BY 1"
                 START REPORT axmg361_subrep02
                 DECLARE axmg361_subrep02 CURSOR FROM l_sql
                 FOREACH axmg361_subrep02 INTO sr3.*
                     OUTPUT TO REPORT axmg361_subrep02 (sr3.*)
                 END FOREACH
                 FINISH REPORT axmg361_subrep02

            LET l_sql = " SELECT oqx01,oqx03,oqx04,oaj02,oqx041,SUM(oqx05),oqx06,azi04 ",
                 " FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED,
                 " WHERE oqx01 = '",sr1.oqt01 CLIPPED,"'",
                 " AND oqx03 BETWEEN 1 AND 100 ",                #FUN0-C20051 mark
                 " GROUP BY oqx01,oqx03,oqx04,oaj02,oqx041,oqx06,azi04"
                 START REPORT axmg361_subrep03
                 DECLARE axmg361_subrep03 CURSOR FROM l_sql
                 FOREACH axmg361_subrep03 INTO sr3.*
                     OUTPUT TO REPORT axmg361_subrep03 (sr3.*)
                 END FOREACH
                 FINISH REPORT axmg361_subrep03
            #FUN-B50018----add-----end---------------        
        ON LAST ROW
