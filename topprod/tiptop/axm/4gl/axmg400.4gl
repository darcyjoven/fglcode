...
#           wc      LIKE type_file.chr1000,       #No.FUN-680137 VARCHAR(500)             # Where condition    #TQC-C80100 mark
            ...
            b       LIKE type_file.chr1,          # Prog. Version..: '5.30.06-13.03.12(01)              # 列印訂單嘜頭
...
DEFINE l_sql       STRING                #FUN-40092  add
...
MAIN
   ...
   LET g_sql = "oea00.oea_file.oea00  ,oea01.oea_file.oea01,",
              ...
              #"oeb21.oeb_file.oeb21  ,occ55.occ_file.occ55,",    #FUN-7B0142 mark
               ...
               "oeb07.oeb_file.oeb07,l_count.type_file.num5"
              ,",sign_type.type_file.chr1,sign_img.type_file.blob,",    #FUN-C40036 簽核方式,簽核圖檔
               "sign_show.type_file.chr1,sign_str.type_file.chr1000"  #FUN-C40036  是否顯示簽核資料(Y/N),簽核字串
              #str CHI-7A0010 mark
              #"oao04.oao_file.oao04,",
              #"oao06h.oao_file.oao06 ,oao06d1.oao_file.oao06,",
              #"oao06d2.oao_file.oao06,oao06t.oao_file.oao06"
              #end CHI-7A0010 mark
   LET l_table = cl_prt_temptable('axmg400',g_sql) CLIPPED   # 產生Temp Table
   IF l_table = -1 THEN
      #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40092  #FUN-C10036   mark
      #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2) #FUN-B40092  #FUN-C10036   mark
      EXIT PROGRAM 
   END IF                  # Temp Table產生
   ...
   #str CHI-7A0010 add
    ...
    LET l_table1 = cl_prt_temptable('axmg4001',g_sql) CLIPPED
    IF  l_table1 = -1 THEN 
        #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40092  #FUN-C10036   mark
        #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2) #FUN-B40092  #FUN-C10036   mark
        EXIT PROGRAM 
   ...
   LET g_sql = "imc01.imc_file.imc01,",
    ...
    LET l_table2 = cl_prt_temptable('axmg4002',g_sql) CLIPPED
    IF  l_table2 = -1 THEN
        #CALL cl_used(g_prog,g_time,2) RETURNING g_time #FUN-B40092  #FUN-C10036   mark
        #CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2) #FUN-B40092  #FUN-C10036   mark
        EXIT PROGRAM 
...
#TQC-730022 end
  ...
  #IF cl_null(tm.wc) THEN                                         #FUN-570069 mark
   IF (cl_null(g_bgjob) OR g_bgjob = 'N') THEN  # If background   #FUN-570069
      CALL axmg400_tm(0,0)             # Input print condition
   ELSE 
  #   LET tm.wc="oea01= '",tm.wc CLIPPED,"'"   #FUN-570069 mark
      CALL axmg400()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
   CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)
END MAIN
 
FUNCTION axmg400_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW axmg400_w AT p_row,p_col WITH FORM "axm/42f/axmg400"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmg400_w 
    #  CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)    #FUN-B80089  MARK
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)    #FUN-B80089
      EXIT PROGRAM
  ...
  #INPUT BY NAME tm.n,tm.b,tm.d,tm.e,tm.more WITHOUT DEFAULTS  #BugNo:6289  #FUN-690032 add tm.d  #TQC-730022 add tm.e   #TQC-740271 mark
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmg400',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmg400_w
   #   CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)       #FUN-B80089
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      CALL cl_gre_drop_temptable(l_table||"|"||l_table1||"|"||l_table2)       #FUN-B80089
      EXIT PROGRAM
...
#TQC-730022 end
   ...
   DEFINE l_name    LIKE type_file.chr20,         # External(Disk) file name        #No.FUN-680137 VARCHAR(20)
         ...
         #l_sql     LIKE type_file.chr1000,       #No.FUN-680137 VARCHAR(3000)      #TQC-C80100 mark
          l_sql     STRING,                       #TQC-C80100 add
...
DEFINE l_img_blob     LIKE type_file.blob        #FUN-C40036 add
  ...
  #str CHI-7A0010 add
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               ...
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?)"  #?  #FUN-7B0142 mod  #?,?,?,?,?  #071002 mod  #MOD-7B0236   #FUN-810029 84?->87?  #No.FUN-A80024 add ? #No.FUN-A80072 add 2?   #FUN-C40036 add 4 ?
  ...
  #公司全名zo02、公司地址zo041、公司電話zo05、公司傳真zo09
   ...
   LET l_sql="SELECT oea00,oea01,oea02,oea10,oea11,oea12,",   #No.MOD-570395
            ...
            #"  FROM oea_file,oeb_file LEFT OUTER JOIN ima_file ON oeb_file.oeb04 = ima_file.ima01,occ_file ",  #FUN-C50008 mark--
             "  FROM oea_file LEFT OUTER JOIN azi_file ON azi01=oea23,",             #FUN-C50008 add LEFT OUTER 
             "       oeb_file LEFT OUTER JOIN ima_file ON oeb04 = ima01,occ_file ",  #FUN-C50008 add LEFT OUTER
             " WHERE oea01=oeb01 ",
             "   AND oea04=occ01 ",  
             "   AND oeaconf != 'X' ", #01/08/01 mandy
             "   AND ",tm.wc CLIPPED,
             " ORDER BY oea01,oea02,oeb03 "
   PREPARE axmg400_prepare1 FROM l_sql
   IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
      ...
      EXIT PROGRAM 
   END IF
   DECLARE axmg400_curs1 CURSOR FOR axmg400_prepare1
...
#   PREPARE axmg400_prepare2 FROM l_sql
#   IF STATUS THEN CALL cl_err('prepare2:',STATUS,1) RETURN END IF
#   DECLARE axmg400_curs2 CURSOR FOR axmg400_prepare2
...
##
 #-----No.MOD-570395 Mark END-----
  ...
  #ELSE
  #   CALL cl_outnam('axmg400') RETURNING l_name
  #END IF
  ##MOD-6B0114--add---str---
  ##zaa06隱藏否
  ...
  #START REPORT axmg400_rep TO l_name
  #
  ##start FUN-570069 add
  ...
  #PREPARE g400_zaa_pre FROM l_sql
  #DECLARE g400_zaa_cur CURSOR FOR g400_zaa_pre
  #
  ...
  #LET g_pageno = 0
   ...
   FOREACH axmg400_curs1 INTO sr.*,t_azi03,t_azi04,t_azi05 #FUN-C50008 add ",t_azi03,t_azi04,t_azi05" 
      ...
      IF g_oaz.oaz121 = "1" THEN
         ...
         #FUN-650020...............begin
         IF g400_err_ana(g_showmsg) THEN
  ...
  #FUN-C50008--mark--end
      FOREACH oao_c2 USING sr.oea01,sr.oeb03 INTO oao.*  #FUN-C50008 add
      ...
      IF NOT cl_null(l_ocf.ocf01) THEN
         ...
         LET g_ctn_no2=""
         #-----No.MOD-570395 END-----
         LET l_ocf.ocf101=g400_ocf_c(l_ocf.ocf101)
         LET l_ocf.ocf102=g400_ocf_c(l_ocf.ocf102)
         LET l_ocf.ocf103=g400_ocf_c(l_ocf.ocf103)
         LET l_ocf.ocf104=g400_ocf_c(l_ocf.ocf104)
         LET l_ocf.ocf105=g400_ocf_c(l_ocf.ocf105)
         LET l_ocf.ocf106=g400_ocf_c(l_ocf.ocf106)
         LET l_ocf.ocf107=g400_ocf_c(l_ocf.ocf107)
         LET l_ocf.ocf108=g400_ocf_c(l_ocf.ocf108)
         LET l_ocf.ocf109=g400_ocf_c(l_ocf.ocf109)
         LET l_ocf.ocf110=g400_ocf_c(l_ocf.ocf110)
         LET l_ocf.ocf111=g400_ocf_c(l_ocf.ocf111)
         LET l_ocf.ocf112=g400_ocf_c(l_ocf.ocf112)
         LET l_ocf.ocf201=g400_ocf_c(l_ocf.ocf201)
         LET l_ocf.ocf202=g400_ocf_c(l_ocf.ocf202)
         LET l_ocf.ocf203=g400_ocf_c(l_ocf.ocf203)
         LET l_ocf.ocf204=g400_ocf_c(l_ocf.ocf204)
         LET l_ocf.ocf205=g400_ocf_c(l_ocf.ocf205)
         LET l_ocf.ocf206=g400_ocf_c(l_ocf.ocf206)
         LET l_ocf.ocf207=g400_ocf_c(l_ocf.ocf207)
         LET l_ocf.ocf208=g400_ocf_c(l_ocf.ocf208)
         LET l_ocf.ocf209=g400_ocf_c(l_ocf.ocf209)
         LET l_ocf.ocf210=g400_ocf_c(l_ocf.ocf210)
         LET l_ocf.ocf211=g400_ocf_c(l_ocf.ocf211)
         LET l_ocf.ocf212=g400_ocf_c(l_ocf.ocf212)
      ELSE
         LET l_ocf.ocf01=' '
         LET l_ocf.ocf02=' '
         LET l_ocf.ocf101=' '
    ...
    #FUN-C50008--mark--end
           FOREACH imc_cur USING sr.oeb04,sr.oeb07 INTO sr1.*   #FUN-C50008 add                          
 ...
 #----No.MOD-570395 Mark-----
#No:7674
#     FOREACH axmg400_curs2 USING sr.oea01
...
##
 #----No.MOD-570395 Mark END-----
    ...
    #str TQC-740271 mark
    # OUTPUT TO REPORT axmg400_rep(sr.*)
  ...
  #str CHI-7A0010 add
   ...
   LET l_sql = "SELECT oea01 FROM oea_file ",
               " WHERE ",tm.wc CLIPPED,
               "   ORDER BY oea01"
   PREPARE g400_prepare2 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      ...
      EXIT PROGRAM
   END IF
   DECLARE g400_cs2 CURSOR FOR g400_prepare2
...
#FUN-C50008--add--end
   ...
   FOREACH g400_cs2 INTO sr.oea01
      #FUN-C50008--mark--str
      #DECLARE oao_c1 CURSOR FOR
      # SELECT * FROM oao_file
     ...
     #FUN-C50008--mark--end
      FOREACH oao_c1 USING sr.oea01 INTO oao.*   #FUN-C50008 add
         IF NOT cl_null(oao.oao06) THEN
            EXECUTE insert_prep1 USING 
               sr.oea01,oao.oao03,oao.oao04,oao.oao05,oao.oao06
         END IF
  ...
  #FINISH REPORT axmg400_rep   #TQC-740271 mark
...
###GENGRE###               "SELECT * FROM ", g_cr_db_str CLIPPED,l_table2 CLIPPED
   ...
   ELSE
      LET tm.wc = ''
   END IF
###GENGRE###    LET g255 = g_str ,";",tm.wc,";",tm.c                #FUN-7B0142 mark 還原  #TQC-750114
   #LET g_str = g_str ,";",l_wc0 CLIPPED,";",l_wc1 CLIPPED,";",l_wc2 CLIPPED  #FUN-7B0142 mark#TQC-750114
###GENGRE###   CALL cl_prt_cs3('axmg400','axmg400',g_sql,g_str)
    LET g_cr_table = l_table                   #主報表的temp table名稱         #FUN-C40036 add
    LET g_cr_apr_key_f = "oea01"       #報表主鍵欄位名稱，用"|"隔開  #FUN-C40036 add

    CALL axmg400_grdata()    ###GENGRE###
   #------------------------------ CR (4) ------------------------------#
   #end TQC-740271 add
...
#            #OPEN g400_zaa_cur USING g_rlang
#            #FOREACH g400_zaa_cur INTO g_i,l_zaa08,l_zaa09,l_zaa14,l_zaa16
#             FOREACH g400_zaa_cur USING g_rlang INTO g_i,l_zaa08,l_zaa09,l_zaa14,l_zaa16
#            #TQC-720017-end-add
#              IF l_zaa09 = '1' THEN
...
#                #-----No.MOD-570395 END-----
# 
#               LET sr2.ocf.ocf101=g400_ocf_c(sr2.ocf.ocf101)
#               LET sr2.ocf.ocf102=g400_ocf_c(sr2.ocf.ocf102)
#               LET sr2.ocf.ocf103=g400_ocf_c(sr2.ocf.ocf103)
#               LET sr2.ocf.ocf104=g400_ocf_c(sr2.ocf.ocf104)
#               LET sr2.ocf.ocf105=g400_ocf_c(sr2.ocf.ocf105)
#               LET sr2.ocf.ocf106=g400_ocf_c(sr2.ocf.ocf106)
#               LET sr2.ocf.ocf107=g400_ocf_c(sr2.ocf.ocf107)
#               LET sr2.ocf.ocf108=g400_ocf_c(sr2.ocf.ocf108)
#               LET sr2.ocf.ocf109=g400_ocf_c(sr2.ocf.ocf109)
#               LET sr2.ocf.ocf110=g400_ocf_c(sr2.ocf.ocf110)
#               LET sr2.ocf.ocf111=g400_ocf_c(sr2.ocf.ocf111)
#               LET sr2.ocf.ocf112=g400_ocf_c(sr2.ocf.ocf112)
#               LET sr2.ocf.ocf201=g400_ocf_c(sr2.ocf.ocf201)
#               LET sr2.ocf.ocf202=g400_ocf_c(sr2.ocf.ocf202)
#               LET sr2.ocf.ocf203=g400_ocf_c(sr2.ocf.ocf203)
#               LET sr2.ocf.ocf204=g400_ocf_c(sr2.ocf.ocf204)
#               LET sr2.ocf.ocf205=g400_ocf_c(sr2.ocf.ocf205)
#               LET sr2.ocf.ocf206=g400_ocf_c(sr2.ocf.ocf206)
#               LET sr2.ocf.ocf207=g400_ocf_c(sr2.ocf.ocf207)
#               LET sr2.ocf.ocf208=g400_ocf_c(sr2.ocf.ocf208)
#               LET sr2.ocf.ocf209=g400_ocf_c(sr2.ocf.ocf209)
#               LET sr2.ocf.ocf210=g400_ocf_c(sr2.ocf.ocf210)
#               LET sr2.ocf.ocf211=g400_ocf_c(sr2.ocf.ocf211)
#               LET sr2.ocf.ocf212=g400_ocf_c(sr2.ocf.ocf212)
#               ##
...
FUNCTION g400_err_ana(ls_showmsg)    #FUN-650020
...
FUNCTION axmg400_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("axmg400")
        IF handler IS NOT NULL THEN
            START REPORT axmg400_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED," ORDER BY oea01,oeb03"
          
            DECLARE axmg400_datacur1 CURSOR FROM l_sql
            FOREACH axmg400_datacur1 INTO sr1.*
                OUTPUT TO REPORT axmg400_rep(sr1.*)
            END FOREACH
            FINISH REPORT axmg400_rep
        END IF
...
REPORT axmg400_rep(sr1)
    DEFINE sr1 sr1_t
    DEFINE sr2 sr2_t
    DEFINE sr3 sr3_t
    DEFINE l_lineno LIKE type_file.num5
    #FUN-B40092------add------str
    DEFINE l_data_qty  LIKE oeb_file.oeb917
    DEFINE l_data_unit STRING
    DEFINE l_oea01_oea00 STRING
    DEFINE l_oea01_oea06 STRING
    DEFINE l_oea03_oea032 STRING
    DEFINE l_oea04_occ02 STRING
    DEFINE l_oea14_gen02 STRING
    DEFINE l_oea15_gem02 STRING
    DEFINE l_oea21_gec02 STRING
    DEFINE l_oea25_oab02 STRING
    DEFINE l_oea31_oah02 STRING
    DEFINE l_oea32_oag02 STRING
    ...
    DEFINE l_oeb14_fmt        STRING
    DEFINE l_oeb14t_fmt       STRING
    DEFINE l_oeb14_sum_fmt    STRING
    DEFINE l_oeb14t_sum_fmt   STRING
    #FUN-B40092------add------end
    ...
    FORMAT
        ...
        BEFORE GROUP OF sr1.oea01
            LET l_lineno = 0
         #FUN-B40092------add------str
         LET l_oeb12_fmt = cl_gr_numfmt('oeb_file','oeb12',sr1.azi04)
         PRINTX l_oeb12_fmt
         LET l_oeb13_fmt = cl_gr_numfmt('oeb_file','oeb13',sr1.azi03)
         PRINTX l_oeb13_fmt
         LET l_oeb14_fmt = cl_gr_numfmt('oeb_file','oeb14',sr1.azi04)
         PRINTX l_oeb14_fmt
         LET l_oeb14t_fmt = cl_gr_numfmt('oeb_file','oeb14t',sr1.azi04)
         PRINTX l_oeb14t_fmt
         IF g_sma116 = '2' OR g_sma116 = '3' THEN
            LET l_display4 = 'Y'
         ELSE
            LET l_display4 = 'N'
         END IF
        ...
        END IF
         ...
         IF NOT cl_null(sr1.oea01) AND NOT cl_null(sr1.oea06) THEN
            LET l_oea01_oea06 = sr1.oea01,'(',sr1.oea06,')'
         ELSE
            IF NOT cl_null(sr1.oea01) AND NOT cl_null(sr1.oea06) THEN
               LET l_oea01_oea06 = sr1.oea01
            END IF
            IF NOT cl_null(sr1.oea01) AND NOT cl_null(sr1.oea06) THEN
               LET l_oea01_oea06 = '(',sr1.oea06,')'
            END IF
        END IF   
        
         IF NOT cl_null(sr1.oea03) AND NOT cl_null(sr1.oea032) THEN
            LET l_oea03_oea032 = sr1.oea03,' ',sr1.oea032
         ELSE
            IF NOT cl_null(sr1.oea03) AND  cl_null(sr1.oea032) THEN
               LET l_oea03_oea032 = sr1.oea03
            END IF
            IF  cl_null(sr1.oea03) AND NOT cl_null(sr1.oea032) THEN
               LET l_oea03_oea032 = sr1.oea032
            END IF
        END IF
        ...
        IF NOT cl_null(sr1.oea21) AND NOT cl_null(sr1.gec02) THEN
            LET l_oea21_gec02 = sr1.oea21,' ',sr1.gec02
         ELSE
            IF NOT cl_null(sr1.oea21) AND  cl_null(sr1.gec02) THEN
               LET l_oea21_gec02 = sr1.oea21
            END IF
            IF  cl_null(sr1.oea21) AND NOT cl_null(sr1.gec02) THEN
               LET l_oea21_gec02 = sr1.gec02
            END IF
        END IF
      
        IF NOT cl_null(sr1.oea25) AND NOT cl_null(sr1.oab02) THEN
            LET l_oea25_oab02 = sr1.oea25,' ',sr1.oab02
         ELSE
            IF NOT cl_null(sr1.oea25) AND  cl_null(sr1.oab02) THEN
               LET l_oea25_oab02 = sr1.oea25
            END IF
            IF  cl_null(sr1.oea25) AND NOT cl_null(sr1.oab02) THEN
               LET l_oea25_oab02 = sr1.oab02
            END IF
        END IF

         IF NOT cl_null(sr1.oea31) AND NOT cl_null(sr1.oah02) THEN
            LET l_oea31_oah02 = sr1.oea31,' ',sr1.oah02
         ELSE
            IF NOT cl_null(sr1.oea31) AND  cl_null(sr1.oah02) THEN
               LET l_oea31_oah02 = sr1.oea31
            END IF
            IF  cl_null(sr1.oea31) AND NOT cl_null(sr1.oah02) THEN
               LET l_oea31_oah02 = sr1.oah02
            END IF
         END IF 

         IF NOT cl_null(sr1.oea32) AND NOT cl_null(sr1.oag02) THEN
            LET l_oea32_oag02 = sr1.oea32,' ',sr1.oag02
         ELSE
            IF NOT cl_null(sr1.oea32) AND  cl_null(sr1.oag02) THEN
               LET l_oea32_oag02 = sr1.oea32
            END IF
            IF  cl_null(sr1.oea32) AND NOT cl_null(sr1.oag02) THEN
               LET l_oea32_oag02 = sr1.oag02
            END IF
         END IF

         #單身前單據備註
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE oao01 = '",sr1.oea01 CLIPPED,"'",
                        " AND oao03 = 0 AND oao05='1'"
            START REPORT axmg400_subrep01
            DECLARE axmg400_repcur1 CURSOR FROM l_sql
            FOREACH axmg400_repcur1 INTO sr2.*
                OUTPUT TO REPORT axmg400_subrep01(sr2.*)
            END FOREACH
            FINISH REPORT axmg400_subrep01
               
           PRINTX l_oea01_oea00
           PRINTX l_oea01_oea06
           PRINTX l_oea03_oea032
           PRINTX l_oea04_occ02
           PRINTX l_oea14_gen02 
           PRINTX l_oea15_gem02
           PRINTX l_oea21_gec02
           PRINTX l_oea25_oab02
           PRINTX l_oea31_oah02
           PRINTX l_oea32_oag02
           #FUN-B40092------add------end
        BEFORE GROUP OF sr1.oeb03
        ON EVERY ROW
           IF tm.d = 'Y' OR NOT cl_null(sr1.unit) THEN 
              LET l_display = 'Y'
           ELSE
              LET l_display = 'N'
           END IF
          
           IF tm.c = 'Y' AND sr1.l_count > 0 THEN
              LET l_display1 = 'Y'
           ELSE 
              LET l_display1 = 'N'
           END IF
          ...
          #單身前備註
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE oao01 = '",sr1.oea01 CLIPPED,"'",
                        " AND oao03 = ",sr1.oeb03 CLIPPED," AND oao05='1'"
            START REPORT axmg400_subrep02
            DECLARE axmg400_repcur2 CURSOR FROM l_sql
            FOREACH axmg400_repcur2 INTO sr2.*
                OUTPUT TO REPORT axmg400_subrep02(sr2.*)
            END FOREACH
            FINISH REPORT axmg400_subrep02

            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table2 CLIPPED,
                       " WHERE imc01 = '",sr1.oeb04 CLIPPED,"'",
                        " AND imc02 = '",sr1.oeb07 CLIPPED,"'",
                        " AND oea01 = ",sr1.oea01 CLIPPED
            START REPORT axmg400_subrep05
            DECLARE axmg400_repcur5 CURSOR FROM l_sql
            FOREACH axmg400_repcur5 INTO sr3.*
                OUTPUT TO REPORT axmg400_subrep05(sr3.*)
            END FOREACH
            FINISH REPORT axmg400_subrep05
                                        
          #單身後備註
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE oao01 = '",sr1.oea01 CLIPPED,"'",
                        " AND oao03 = ",sr1.oeb03 CLIPPED," AND oao05='2'"
            START REPORT axmg400_subrep03
            DECLARE axmg400_repcur3 CURSOR FROM l_sql
            FOREACH axmg400_repcur3 INTO sr2.*
                OUTPUT TO REPORT axmg400_subrep03(sr2.*)
            END FOREACH
            FINISH REPORT axmg400_subrep03

            LET l_lineno = l_lineno + 1
            PRINTX l_lineno

            IF g_sma116 = '2' OR g_sma116 = '3' THEN
               LET l_data_qty = sr1.oeb917
            ELSE   
               LET l_data_qty = sr1.oeb12
            END IF

            IF g_sma116 = '2' OR g_sma116 = '3' THEN
               LET l_data_unit = sr1.oeb916
            ELSE
               LET l_data_unit = sr1.oeb05
            END IF
            PRINTX sr1.*
            PRINTX l_data_qty
            PRINTX l_data_unit
        AFTER GROUP OF sr1.oea01
            ...
            ELSE
               LET l_display2 = 'Y'
            END IF
            IF tm.b ='Y' AND NOT cl_null(sr1.ocf01) THEN
               LET l_display3 = 'Y'
            ELSE
               LET l_display3 = 'N'
            END IF
            LET l_oeb14_sum = GROUP SUM(sr1.oeb14)
            LET l_oeb14t_sum = GROUP SUM(sr1.oeb14t)
            LET l_oeb14_sum_fmt = cl_gr_numfmt('oeb_file','oeb14',sr1.azi05)
            LET l_oeb14t_sum_fmt = cl_gr_numfmt('oeb_file','oeb14t',sr1.azi05)
            PRINTX l_oeb14_sum_fmt
            PRINTX l_oeb14t_sum_fmt
            PRINTX l_oeb14_sum
            PRINTX l_oeb14t_sum
            PRINTX l_display2
            PRINTX l_display3 
        #單身後單據備註
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table1 CLIPPED,
                        " WHERE oao01 = '",sr1.oea01 CLIPPED,"'",
                        " AND oao03 = 0 AND oao05='2'"
            START REPORT axmg400_subrep04
            DECLARE axmg400_repcur4 CURSOR FROM l_sql
            FOREACH axmg400_repcur4 INTO sr2.*

                OUTPUT TO REPORT axmg400_subrep04(sr2.*)
            END FOREACH
            FINISH REPORT axmg400_subrep04
