...
# Modify.........: No.FUN-D40129 13/05/20 By yangtt 報表顯示新增原幣/本幣含稅金額欄位(ogb14_1,ogb141_1)
...
#No.FUN-580004--end
   DEFINE tm  RECORD                  # Print condition RECORD
             ...
             #u      LIKE type_file.chr3,       # No.FUN-680137 VARCHAR(3)       # Group total sw   #FUN-D30070 mark 
...
MAIN
  ...
  #FUN-D30070---mod--end--
#NO.FUN-710081--start
      #       OUTPUT TO REPORT axmx630_rep(sr.*)
     LET g_sql ="oga01.oga_file.oga01,",
               ...
               " ogb14_1.ogb_file.ogb14,",  #FUN-D40129
               " ogb141_1.ogb_file.ogb14,", #FUN-D40129
               ...
               " l_n1.type_file.num5"      #FUN-D30059
 
   LET l_table = cl_prt_temptable('axmx630',g_sql) CLIPPED
   IF l_table = -1 THEN EXIT PROGRAM END IF  
 #No.FUN-710081--end    
   SELECT * INTO g_ooz.* FROM ooz_file WHERE ooz00='0' #MOD-B30057 add
 
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL axmx630_tm(0,0)        # Input print condition
      ELSE CALL axmx630()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
END MAIN
 
FUNCTION axmx630_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW axmx630_w AT p_row,p_col WITH FORM "axm/42f/axmx630"
...
#---TQC-CC0098--add--end--
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmx630_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
...
#     ON ACTION CONTROLP CALL axmx630_wc()   # Input detail Where Condition
      #UI
      AFTER INPUT
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axmx630',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmx630_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#No.FUN-550070 --end--
                                  ...
                                  ogb14_1 LIKE ogb_file.ogb14,   #FUN-D40129 add
                                  ogb141_1 LIKE ogb_file.ogb14,  #FUN-D40129 add
...
DEFINE       l_n1      LIKE type_file.num5     #FUN-D30059
...
#                "       oga15,ogb03,ogb31,ogb04,ogb06,ogb05, ",
                ...
                #"       ogb13,ogb917-ogb60,0,0,ogb910,ogb912,ogb913,ogb915,ogb916,ogb917,oga10, ",        #No.FUN-580004  #MOD-690107 ogb12->ogb917   #MOD-8C0209  #FUN-D40129 mark
                 "       ogb13,ogb917-ogb60,0,0,0,0,ogb910,ogb912,ogb913,ogb915,ogb916,ogb917,oga10, ",    #FUN-D40129 add 
                #"		 azi03,azi04,azi05 ",   #FUN-790019 mark(有\t\t)
                 ...
                 "   AND ogapost='Y'   ",
                 "   AND ",tm.wc
     PREPARE axmx630_prepare1 FROM l_sql
     ...
     END IF
     DECLARE axmx630_curs1 CURSOR FOR axmx630_prepare1
 
#    LET l_name = 'axmx630.out'
 
#     CALL cl_outnam('axmx630') RETURNING l_name
#FUN-580004--begin
...
##No.FUN-580004--end
#
#     START REPORT axmx630_rep TO l_name
##    LET l_name = 'cat > axmx630.out'
##    START REPORT axmx630_rep TO PIPE l_name
 
#     LET g_pageno = 0
     CALL cl_del_data(l_table)
     LET g_sql=" INSERT INTO ",g_cr_db_str CLIPPED,l_table clipped,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, 
                        ?,?,?,?,?, ?,?,?,?,?, ?,?,?)"  #No.MOD-890247 add 2 '?' by liuxqa  #TQC-940009 add ?   #MOD-A60015 add ?   #FUN-D30059 add 1?  #FUN-D40129 add 2?
     ...
     END IF   
#NO.FUN-710081--end
     LET l_n1 = 2   #FUN-D30059
     FOREACH axmx630_curs1 INTO sr.*,l_oga08 #MOD-B30057 add l_oga08
       IF SQLCA.sqlcode != 0 THEN
...
#          LET sr.ogb14	 = sr.balance*sr.ogb13
	   ...
	   #LET sr.ogb14  = sr.balance*sr.ogb13*sr.oga24
           ...
           IF sr.oga213 = 'N' THEN
             #LET sr.ogb141 = (sr.balance*sr.ogb13)  #原幣未稅金額    #FUN-D40129
             #LET sr.ogb14 = sr.ogb141 * sr.oga24  #本幣未稅金額      #FUN-D40129
              LET sr.ogb14 = (sr.balance*sr.ogb13)  #原幣未稅金額     #FUN-D40129
              LET sr.ogb141 = sr.ogb14 * sr.oga24   #本幣未稅金額     #FUN-D40129
              LET sr.ogb14_1 = sr.ogb14*(1+sr.oga211/100)  #原幣含稅金額     #FUN-D40129
              LET sr.ogb141_1 = sr.ogb141 * (1+sr.oga211/100)   #本幣含稅金額     #FUN-D40129
           ELSE
             #LET sr.ogb141 = (sr.balance*sr.ogb13)/(1+sr.oga211/100)  #原幣未稅金額  #FUN-D40129
             #LET sr.ogb14 = sr.ogb141 * sr.oga24  #本幣未稅金額                      #FUN-D40129
              LET sr.ogb14_1 = (sr.balance*sr.ogb13)  #原幣含稅金額   #FUN-D40129
              LET sr.ogb141_1 = sr.ogb14_1 * sr.oga24   #本幣含稅金額   #FUN-D40129
              LET sr.ogb14 = sr.ogb14_1/(1+sr.oga211/100)  #原幣未稅金額     #FUN-D40129
              LET sr.ogb141 = sr.ogb141_1/(1+sr.oga211/100)   #本未含稅金額     #FUN-D40129
           END IF   
          #LET sr.ogb141 = cl_digcut(sr.ogb141,t_azi04)   #MOD-C10175 #FUN-D40129 mark
          #LET sr.ogb14 = cl_digcut(sr.ogb14,g_azi04)     #MOD-C10175 #FUN-D40129 mark 
           LET sr.ogb141 = cl_digcut(sr.ogb141,g_azi04)   #FUN-D40129 ADD
           LET sr.ogb14 = cl_digcut(sr.ogb14,t_azi04)     #FUN-D40129 ADD
           LET sr.ogb141_1 = cl_digcut(sr.ogb141_1,g_azi04)   #FUN-D40129
           LET sr.ogb14_1 = cl_digcut(sr.ogb14_1,t_azi04)     #FUN-D40129
           #-----END MOD-A10020-----
## --
...
#FUN-580004--begin
      ...
      EXECUTE insert_prep USING sr.oga01,sr.oga02,
                                ...
                                sr.ogb14_1,sr.ogb141_1,    #FUN-D40129
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'axmx630'
...
###XtraGrid###     LET l_sql=" SELECT * FROM ",g_cr_db_str CLIPPED,l_table clipped
     PREPARE axmx630_preparet FROM l_sql
...
###XtraGrid###                 tm.wc CLIPPED,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3]
     ...
     IF g_sma.sma116 MATCHES '[23]' THEN  
      # CALL cl_prt_cs3('axmx630',l_sql,l_str)     #TQC-730113 
###XtraGrid###        CALL cl_prt_cs3('axmx630','axmx630',l_sql,l_str) 
        LET g_xgrid.template = 'axmx630'   #FUN-D30059
     ELSE
        IF g_sma115= 'Y' THEN
      ##   CALL cl_prt_cs3('axmx6301',l_sql,l_str) #TQC-730113  
###XtraGrid###           CALL cl_prt_cs3('axmx630','axmx630_1',l_sql,l_str)  
           LET g_xgrid.template = 'axmx630_1'   #FUN-D30059
           LET g_xgrid.visible_column = cl_xg_column_visible(g_xgrid.visible_column,"str2","Y")
        ELSE
      ##   CALL cl_prt_cs3('axmx6302',l_sql,l_str) #TQC-730113
###XtraGrid###           CALL cl_prt_cs3('axmx630','axmx630_2',l_sql,l_str) 
           LET g_xgrid.template = 'axmx630_1'   #FUN-D30059
           LET g_xgrid.visible_column = cl_xg_column_visible(g_xgrid.visible_column,"str2","N")
        END IF
     END IF 
    #FUN-D30059----add---str---
   ...
   #LET g_xgrid.grup_sum_field = cl_get_sum_field(tm.s,tm.u,"oga01,oga02,oga03,oga04,oga14,oga15")  #FUN-D30070 mark
