...
# Modify.........: No.TQC-C40098 12/04/16 By zhuhao 出貨單的銷退方式為3.不折讓不換貨的出貨單，不應再在axmx661裡列印出
...
# Modify.........: No.FUN-C80010 12/08/16 By suncx 新增axmx661
# Modify.........: No.FUN-CB0004 12/11/08 By dongsz CR轉XtraGrid
# Modify.........: No.FUN-D30070 13/03/21 By chenying 去除小計
# Modify.........: No.FUN-D40129 13/05/16 By yangtt 1、調整光標跳船順序
#                                                   2、追MOD-CC0188、MOD-D10030單
...
END GLOBALS
  ...
  DEFINE tm  RECORD                  # Print condition RECORD
             ...
             #u       LIKE type_file.chr3,    #No.FUN-680137  VARCHAR(3)  # Group total sw  #FUN-D30070
              oma02   LIKE oma_file.oma02,    #FUN-D40129 mod by MOD-CC0188
...
MAIN
  ...
  #MOD-B20018 mod +1 --start--
   LET tm.s  = ARG_VAL(10)
   LET tm.t  = ARG_VAL(11)
  #LET tm.u  = ARG_VAL(12)  #FUN-D30070
...
#No.FUN-750095 --begin
   LET g_sql = "oga01.oga_file.oga01,",
               ...
               "oha01.oha_file.oha01,",       #FUN-CB0004 add
               "ccc23.ccc_file.ccc23,",   #FUN-D40129 mod by MOD-CC0188
               "sum23.ccc_file.ccc23,",   #FUN-D40129 mod by MOD-CC0188
               "ogb12t.ogb_file.ogb12,",  #FUN-D40129 mod by MOD-CC0188
               "ima39.ima_file.ima39,",   #FUN-D40129 mod by MOD-CC0188
               "l_n1.type_file.num5"      #FUN-D40129
          
   LET l_table = cl_prt_temptable('axmx661',g_sql) CLIPPED
   IF l_table = -1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,",
               "        ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,  ?,?)"  ##MOD-C60216?     #FUN-CB0004 add 2?  #FUN-D40129 add 5?
...
#No.FUN-750095 --end
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'       # If background job sw is off
      THEN CALL axmx661_tm(0,0)               # Input print condition
      ELSE CALL axmx661()                     # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80089    ADD
END MAIN
 
FUNCTION axmx661_tm(p_row,p_col)
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01
   ...
   OPEN WINDOW axmx661_w AT p_row,p_col WITH FORM "axm/42f/axmx661"
...
WHILE TRUE
  ...
  #CONSTRUCT BY NAME tm.wc ON oga01,oga02,oga03,oga04,oga14,oga15,oha01   #FUN-C50129 oha01 #FUN-D40129 mark
   CONSTRUCT BY NAME tm.wc ON oga01,oha01,oga02,oga03,oga04,oga14,oga15   #FUN-D40129      
  ...
  INPUT BY NAME tm.a,tm.b,tm2.s1,tm2.s2,tm2.s3,   #FUN-C50129 add tm.b
                tm2.t1,tm2.t2,tm2.t3,tm.oma02,   #FUN-D40129 mod by MOD-CC0188 add tm.oma01
               #tm2.u1,tm2.u2,tm2.u3,  #FUN-D30070
                tm.more WITHOUT DEFAULTS
      BEFORE INPUT
             CALL cl_qbe_display_condition(lc_qbe_sn)
             LET tm.oma02=g_today #FUN-D40129 mod by MOD-CC0188
             DISPLAY tm.oma02 TO oma02 #FUN-D40129 mod by MOD-CC0188
   ...
   END INPUT
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axmx661_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80089    ADD
      EXIT PROGRAM
   END IF
      ...
      ELSE
         ...
         LET l_cmd = l_cmd CLIPPED,        #(at time fglgo xxxx p1 p2 p3)
                        ...
                        #" '",tm.u CLIPPED,"'",  #FUN-D30070
                         ...
                         " '",g_rep_clas CLIPPED,"'",
                         " '",g_template CLIPPED,"'"
         CALL cl_cmdat('axmx661',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axmx661_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80089    ADD
      EXIT PROGRAM
...
#         l_time    LIKE type_file.chr8         #No.FUN-6A0094
   ...
   DEFINE sr        RECORD order1  LIKE oga_file.oga01, #No.FUN-680137 VARCHAR(16)
                           ...
                           ogb12   LIKE ogb_file.ogb12, #FUN-D40129 mod by MOD-CC0188
                           ...
                           oha01   LIKE oha_file.oha01,  #FUN-CB0004 add
                           ogb09   LIKE ogb_file.ogb09, #FUN-D40129 mod by MOD-CC0188
                           ogb091  LIKE ogb_file.ogb091 #FUN-D40129 mod by MOD-CC0188
                    END RECORD
#No.FUN-750095 -- begin --
...
#No.FUN-750095 -- end --
   ...
   DEFINE l_ccc23    LIKE ccc_file.ccc23  #FUN-D40129 mod by MOD-CC0188
   DEFINE l_sum23    LIKE ccc_file.ccc23  #FUN-D40129 mod by MOD-CC0188
   DEFINE t_ogb12t   LIKE ogb_file.ogb12  #FUN-D40129 mod by MOD-CC0188
   DEFINE l_omb12    LIKE omb_file.omb12  #FUN-D40129 mod by MOD-CC0188
   DEFINE j_cnt     LIKE type_file.chr1   #FUN-D40129 mod by MOD-CC0188
   DEFINE l_ima39   LIKE ima_file.ima39   #FUN-D40129 mod by MOD-CC0188
   DEFINE l_ccz07   LIKE ccz_file.ccz07   #FUN-D40129 mod by MOD-CC0188
   DEFINE l_oga213  LIKE oga_file.oga213 #FUN-D40129 mod by MOD-CC0188
   DEFINE l_oga211  LIKE oga_file.oga211 #FUN-D40129 mod by MOD-CC0188
...
#FUN-D40129 mod by MOD-CC0188 mark begin-------------------------
...
#              "    AND ",tm.wc
#FUN-D40129 mod by MOD-CC0188 mark end----------------------------
#FUN-D40129 mod by MOD-CC0188 add begin----------------------------
   ...
   LET l_sql = "SELECT distinct '','','',",
               ...
               "    OR oga00 ='4')    ",
               "    AND ogapost='Y'   ",
               #"    AND ogb64 < ogb12 ",  #TQC-C70037 add #FUN-D40129 mark by MOD-D10030
               "    AND ",tm.wc
#FUN-D40129 mod by MOD-CC0188 add end----------------------------
   ...
   IF tm.b = 'Y' THEN 
      ...
      LET tm.wc=cl_replace_str(tm.wc, "oga01", "oha01")  #FUN-D40129 mod by MOD-CC0188 add
      LET l_sql = l_sql,
#FUN-D40129 mod by MOD-CC0188 mark begin-------------------------
#                 " UNION ALL ",
#                 "SELECT DISTINCT '','','',",
...
#FUN-D40129 mod by MOD-CC0188 add end----------------------------
   ...
   PREPARE axmx661_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('prepare:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80089    ADD
      EXIT PROGRAM
   END IF
   DECLARE axmx661_curs1 CURSOR FOR axmx661_prepare1
 
#No.FUN-750095 -- begin --
#   CALL cl_outnam('axmx661') RETURNING l_name
#
...
#   CALL cl_prt_pos_len()
#
#   START REPORT axmx661_rep TO l_name
#   LET g_pageno = 0
#No.FUN-750095 -- end --
 
   FOREACH axmx661_curs1 INTO sr.*,l_oha09
      IF SQLCA.sqlcode != 0 THEN
...
#       OUTPUT TO REPORT axmx661_rep(sr.*)
      #-----CHI-880035---------
#FUN-D40129 mod by MOD-CC0188 mark begin-------------------------
#     LET l_cnt = 0
...
#FUN-D40129 mod by MOD-CC0188 add begin-----------------------------------
      ...
      LET l_sql="SELECT omb12 FROM oma_file,omb_file ",
                ...
                " AND oma01=omb01 AND oma02<= '",tm.oma02,"'"
...
#FUN-D40129 mod by MOD-CC0188  add end-------------------------------------------
      ...
      #MOD-C60216 add beg
     #LET sr.ogb14t  = sr.ogb14t*sr.oga24  #FUN-D40129 mod by MOD-CC0188 mark
...
#FUN-D40129 mod by MOD-CC0188 add-start-----------------------
      ...
      EXECUTE insert_prep USING sr.oga01,sr.oga02,sr.oga03,sr.oga032,sr.oga04,
              ...
              sr.ogb13,sr.ogb14,sr.azi03,sr.azi04,sr.azi05,sr.ogb14t,l_pot,sr.oha01  ##MOD-C60216 add sr.ogb14t   #FUN-CB0004 add l_pot,sr.oha01
              ,l_ccc23,l_sum23,t_ogb12t,l_ima39,l_n1   #FUN-D40129 mod by MOD-CC0188
      IF STATUS THEN
...
#No.FUN-750095 -- begin --
#   FINISH REPORT axmx661_rep
#   CALL cl_prt(l_name,g_prtway,g_copies,g_len)
   LET g_xgrid.table = l_table      ###XtraGrid###
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = g_prog
   IF g_zz05 = 'Y'  THEN                                           #FUN-CB0004 add
      CALL cl_wcchp(tm.wc,'oga01,oga04,oga02,oga14,oga03,oga15,oha01') RETURNING tm.wc    #FUN-CB0004 add oha01
   END IF                                                          #FUN-CB0004 add
###XtraGrid###   LET g_str = tm.wc,";",g_zz05,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",tm.t,";",tm.u
###XtraGrid###   LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
   SELECT sma115,sma116 INTO g_sma115,g_sma116 FROM sma_file
   IF g_sma.sma116 MATCHES '[23]' THEN
###XtraGrid###      CALL cl_prt_cs3('axmx661','axmx661',g_sql,g_str)
      LET g_xgrid.template = 'axmx661'           #FUN-CB0004 add
 #  LET g_xgrid.table = l_table    ###XtraGrid###
 #  CALL cl_xg_view()    ###XtraGrid###
   ELSE
      IF g_sma115 = "Y" THEN
###XtraGrid###         CALL cl_prt_cs3('axmx661','axmx661_1',g_sql,g_str)
         LET g_xgrid.template = 'axmx661_1'      #FUN-CB0004 add
 #  LET g_xgrid.table = l_table    ###XtraGrid###
 #  CALL cl_xg_view()    ###XtraGrid###
      ELSE
###XtraGrid###         CALL cl_prt_cs3('axmx661','axmx661_2',g_sql,g_str)
         LET g_xgrid.template = 'axmx661_2'      #FUN-CB0004 add
 #  LET g_xgrid.table = l_table    ###XtraGrid###
 #  CALL cl_xg_view()    ###XtraGrid###
      END IF
   END IF
  #FUN-CB0004--add--str---
   LET g_xgrid.order_field = cl_get_order_field(tm.s,"oga01,oga02,oga03,oga04,oga14,oga15")
   LET g_xgrid.grup_field = cl_get_order_field(tm.s,"oga01,oga02,oga03,oga04,oga14,oga15")
  #LET g_xgrid.grup_sum_field = cl_get_sum_field(tm.s,tm.u,"oga01,oga02,oga03,oga04,oga14,oga15")  #FUN-D30070
   LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"oga01,oga02,oga03,oga04,oga14,oga15")
