...
# Modify.........: No.FUN-B30211 11/03/31 By yangtingting 離開時沒有cl_used(1)
...
DEFINE          
       ...
       boe78a        LIKE boe_file.boe07,        
          a3         LIKE boe_file.boe07                      
                     END RECORD,
       g_boe_t       RECORD                        
          a2         LIKE type_file.num10,                      
       boe78a        LIKE boe_file.boe07,         
...
DEFINE g_chr               LIKE boe_file.boe07   
...
DEFINE g_boe07             LIKE boe_file.boe07
...
FUNCTION i614_ylsfbl(p_boe07,p_boe01,p_boe02,p_lang,p_boe03,p_boe04,p_boe05,p_boe06)
DEFINE p_boe07             LIKE boe_file.boe07
...
DEFINE p_lang              LIKE type_file.chr1
   ...
   OPEN WINDOW i614_1_w WITH FORM "abm/42f/abmi614_1"
...
FUNCTION i614_1_menu() 
   ...
   CASE g_action_choice
         ...
         WHEN "qc_formula"
           IF cl_chk_act_auth() THEN
             IF NOT cl_null(l_a1) THEN
               IF NOT cl_null(g_boe07) THEN  
                IF  l_a1[1,LENGTH(l_a1)-1]!= g_boe07 THEN 
                  CALL i614_1_qc_formula()
                END IF 
               ELSE
               	  CALL i614_1_qc_formula()
               END IF 	   
         ...
         WHEN "fin_formula"
           IF cl_chk_act_auth() THEN
           IF NOT cl_null(l_a1) THEN 
            IF l_p = 'Y' OR l_a1[1,LENGTH(l_a1)-1] = g_boe07 THEN 
              CALL i614_1_fin_formula()
            ELSE
...
DEFINE 
       ...
       END RECORD
DEFINE ls_tmp     STRING                          #FUN-B30219
...
#FUN-B30219 --Begin
             LET ls_tmp = l_a1
             LET l_atest = ls_tmp.subString(cl_str_position(l_a1,'_',-1,2)+1,
                           cl_str_position(l_a1,'_',-1,1)-
                           cl_str_position(l_a1,'_',-1,2)-1)
...
#FUN-B30219 --Begin
              LET ls_tmp = l_a1
              LET l_atest = ls_tmp.subString(cl_str_position(l_a1,'_',-1,2)+1,
                     cl_str_position(l_a1,'_',-1,1)-cl_str_position(l_a1,'_',-1,2)-1)
...
#              SELECT replace(l_a1,'_') INTO l_adis  FROM dual #FUN-B30219 mark
      ...
      ON ACTION act03                      
          IF cl_null(l_a1) THEN 
              NEXT FIELD a5
          END IF    
#         SELECT instr(l_a1,'_',-1,2) INTO l_n FROM dual #FUN-B30219 mark
...
DEFINE  l_a1_1   STRING                 #FUN-B30219
   ...
   IF cl_null(l_a1) THEN
      ...
      RETURN      
   END IF     
#  SELECT TRIM(l_a1) INTO l_s FROM DUAL #FUN-B30219 mark
#FUN-B30219 --Begin
   LET l_a1_1 = l_a1
   LET l_s    = l_a1_1.trim()
#FUN-B30219 --End
...
DEFINE  l_re    LIKE type_file.chr1000
      ...
      END IF    
         SELECT COUNT(*) INTO l_int1  FROM boe_file WHERE boe01 = g_boe01
                                                      ...
                                                      AND boe06 = g_boe06
         IF l_int1 >0 THEN
           UPDATE boe_file SET boe07 = l_re
