...
#
# Pattern name...: abmp603.4gl
# Descriptions...: 低階碼更新巨集作業
# Input parameter: 
# Return code....: 
# Date & Author..: 08/08/08 By Davidzv
# Modify.........: No:FUN-D10080 13/04/11 By bart 回收程式
...
DEFINE l_logdate  LIKE type_file.chr100
DEFINE l_logfile  LIKE type_file.chr100
DEFINE l_datefile LIKE type_file.chr100
...
DEFINE   lc_cmd   VARCHAR(1000)   
   ...
   OPEN WINDOW p603_w AT p_row,p_col WITH FORM "abm/42f/abmp603" 
   ...
   WHILE TRUE        
     ...
     IF g_bgjob = "Y" THEN
        ...
        IF SQLCA.sqlcode OR lc_cmd IS NULL THEN
           CALL cl_err3('sel','zz_file','abmp603','',SQLCA.sqlcode,'','9031',1)
        ELSE
           LET lc_cmd = lc_cmd CLIPPED,
                        ...
                        " '",tm.sw CLIPPED,"'",
                        " '",g_bgjob CLIPPED,"'"
           CALL cl_cmdat('abmp603',g_time,lc_cmd CLIPPED)
        END IF
        CLOSE WINDOW p603_w
        CALL cl_used(g_prog,g_time,2) RETURNING g_time
        EXIT PROGRAM
...
DEFINE l_cmd  VARCHAR(200)
   ...
   PREPARE p603_bmb_ins_pr FROM l_sql
   EXECUTE p603_bmb_ins_pr
   IF SQLCA.sqlcode THEN
   ...
   LET l_sql = " INSERT INTO bmb_tmp ",
               ...
               " WHERE bmdacti='Y' ",
               "   AND bmd08 <> 'ALL'"
   PREPARE p603_bmd_ins_pr1 FROM l_sql
   EXECUTE p603_bmd_ins_pr1
   IF SQLCA.sqlcode THEN
   ...
   PREPARE p603_bmd_ins_pr2 FROM l_sql
   EXECUTE p603_bmd_ins_pr2   
   IF SQLCA.sqlcode THEN
      CALL cl_err3('INS','bmb_tmp','','',SQLCA.sqlcode,'','INSERT bmb_tmp FROM BMD <>ALL',1)
   END IF   
   ...
   #建立2个临时表,用来循环调用
   DROP TABLE ima16_tmp_f
   CREATE TEMP TABLE ima16_tmp_f        #作为每次进入循环时的初始料件表,在每次循环结束时用ima16_tmp_s刷新
   (ima01_f   LIKE ima_file.ima01)
   CREATE INDEX ima16_tmp_f01 on ima16_tmp_f(ima01_f)
   DROP TABLE ima16_tmp_s               #依据ima16_tmp_f,和bmb_file中主元件关系,存储剥离ima16_tmp_f中最低阶料后的下阶料
