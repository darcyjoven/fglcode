...
# Modify.........: No.FUN-850049 08/05/09 By xiaofeizhu 將報表輸出改為CR
...
### *** FUN-850049 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>>--*** ##                                                     
    ...
    LET l_table = cl_prt_temptable('abmr670',g_sql) CLIPPED   # 產生Temp Table                                                      
...
#----------------------------------------------------------CR (1) ------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL r670_tm(0,0)		# Input print condition
      ELSE CALL abmr670()			# Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690107
END MAIN
 
FUNCTION r670_tm(p_row,p_col)
   ...
   OPEN WINDOW r670_w AT p_row,p_col
        WITH FORM "abm/42f/abmr670"
################################################################################
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW r670_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690107
      EXIT PROGRAM
...
################################################################################
      ...
      ON ACTION CONTROLP CALL r670_wc()   # Input detail Where Condition
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('abmr670',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW r670_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690107
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8	    #No.FUN-6A0060
          ...
          sr RECORD  
             ...
             bmd07 LIKE bmd_file.bmd07         #組成用量
          END RECORD
...
##No.FUN-850049-Add--Begin--#
   DEFINE l_ima02 LIKE ima_file.ima02,        #品名規格                                                                             
          ...
          l_ima63 LIKE ima_file.ima63         #來源
##No.FUN-850049-Add--End--#
   ...
   #------------------------------ CR (2) ------------------------------#
     ...
     #                     " AND imagrup MATCHES '",g_grup CLIPPED,"*'"
     #     END IF
 
     #     IF g_priv3 MATCHES "[5678]" THEN    #TQC-5C0134群組權限
     #         LET tm.wc = tm.wc clipped,
     #                     " AND imagrup IN ",cl_chk_tgrup_list()
     #     END IF
     LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond('imauser', 'imagrup')
     #End:FUN-980030
 
     LET l_sql = "SELECT '','','',",
                 " ima01,ima02,ima021,ima05,ima06,", #FUN-510033 add ima021
                 "ima09,ima10,ima11,ima12,ima08,ima63,"
     IF tm.ww='1' THEN
         LET l_sql=l_sql CLIPPED, "bmd04,bmd05,bmd06,bmd07"
     ELSE
         LET l_sql=l_sql CLIPPED, "bmd01,bmd05,bmd06,bmd07"
     END IF
 
     LET l_sql=l_sql CLIPPED,
         " FROM ima_file,bmd_file",
         " WHERE bmd02='1'",
         " AND ima08!='Z'",
         " AND ",tm.wc,
     ...
     END IF
     #.PER中要求必不為雜項料件哦!?
     PREPARE r670_prepare1 FROM l_sql
     ...
     END IF
     DECLARE r670_cs1 CURSOR FOR r670_prepare1
 
#    CALL cl_outnam('abmr670') RETURNING l_name                                    #No.FUN-850049
#    START REPORT r670_rep TO l_name                                               #No.FUN-850049
 
#    LET g_pageno = 0                                                              #No.FUN-850049
     FOREACH r670_cs1 INTO sr.*
       IF SQLCA.sqlcode THEN
...
#      OUTPUT TO REPORT r670_rep(sr.*)
#No.FUN-850049--Mark--End--#
 
#No.FUN-850049--Add--Begin--#
      SELECT ima02,ima021,ima05,ima08,ima63                                                                                         
          INTO l_ima02,l_ima021,l_ima05,l_ima08,l_ima63                                                                             
          FROM ima_file                                                                                                             
          WHERE ima01=sr.item                                                                                                       
      IF SQLCA.sqlcode THEN                                                                                                         
         LET l_ima02=' ' LET l_ima05 = ' '                                                                                          
         LET l_ima08=' ' LET l_ima63 = ' '                                                                                          
         LET l_ima021=' '                                                                                                           
      END IF
     ...
     ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-850049 *** ##                                                      
         EXECUTE insert_prep USING                                                                                                  
                 ...
                 sr.ima11,sr.ima12,sr.ima08,sr.ima63,sr.item,sr.bmd05,sr.bmd06,
                 sr.bmd07,l_ima02,l_ima021,l_ima05,l_ima08,l_ima63                                                              
     #------------------------------ CR (3) ------------------------------#
#No.FUN-850049--Add--End--#
     ...
     END FOREACH
        ...
        END IF
    #DISPLAY '' AT 2,1
#    FINISH REPORT r670_rep                                                       #No.FUN-850049
...
#No.FUN-850049--end                                                                                                                 
 ## **** 與 Crystal Reports 串聯段 - <<<< CALL cs3() >>>> FUN-850049 **** ##                                                        
    ...
    CALL cl_prt_cs3('abmr670','abmr670',g_sql,g_str)                                                                                
    #------------------------------ CR (4) ------------------------------#
...
#           COLUMN g_c[34],g_dash2[1,g_w[34]+1+g_w[35]+1+g_w[36]+1+g_w[37]+1+g_w[38]+1+g_w[39]+1+g_w[40]]
#     PRINTX name=H1 g_x[31],g_x[32],g_x[33],g_x[34],g_x[35],g_x[36],g_x[37],g_x[38],g_x[39],g_x[40]
#     PRINTX name=H2 g_x[41],g_x[45],g_x[42]    #No.TQC-5B0030 modify
#     PRINTX name=H3 g_x[43],g_x[46],g_x[44]    #No.TQC-5B0030 modify
#     PRINT g_dash1
#     LET l_last_sw = 'n'
 
#  BEFORE GROUP OF sr.order1
#     IF tm.t[1,1] = 'Y' AND (PAGENO > 1 OR LINENO > 8)
#        THEN SKIP TO TOP OF PAGE
#     END IF
#  BEFORE GROUP OF sr.order2
#     IF tm.t[2,2] = 'Y' AND (PAGENO > 1 OR LINENO > 8)
#        THEN SKIP TO TOP OF PAGE
#     END IF
#  BEFORE GROUP OF sr.order3
#     IF tm.t[3,3] = 'Y' AND (PAGENO > 1 OR LINENO > 8)
#        THEN SKIP TO TOP OF PAGE
#     END IF
...
#                        COLUMN g_c[34],sr.item,
#                        COLUMN g_c[35],l_ima05,
#                        COLUMN g_c[36],l_ima08,
#                        COLUMN g_c[37],sr.bmd05,
#                        COLUMN g_c[38],sr.bmd06,
#                        COLUMN g_c[39],cl_numfor(sr.bmd07,39,3),
#                        COLUMN g_c[40],l_ima63
#         PRINTX name=D2 COLUMN g_c[41],sr.ima02,
#                        COLUMN g_c[42],l_ima02
#         PRINTX name=D3 COLUMN g_c[43],sr.ima021,
#                        COLUMN g_c[44],l_ima021
#     ELSE
#         PRINTX name=D1 COLUMN g_c[31],sr.item,
#                        COLUMN g_c[32],l_ima05,
#                        COLUMN g_c[33],l_ima08,
#                        COLUMN g_c[34],sr.ima01,
#                        COLUMN g_c[35],sr.ima05,
#                        COLUMN g_c[36],sr.ima08,
#                        COLUMN g_c[37],sr.bmd05,
#                        COLUMN g_c[38],sr.bmd06,
#                        COLUMN g_c[39],cl_numfor(sr.bmd07,39,3),
#                        COLUMN g_c[40],sr.ima63
#         PRINTX name=D2 COLUMN g_c[41],l_ima02,
#                        COLUMN g_c[42],sr.ima02
#         PRINTX name=D3 COLUMN g_c[43],l_ima021,
