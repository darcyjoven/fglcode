...
#
# Pattern name...: abmp999_b.4gl
# Descriptions...: 正式BOM底稿批次產生作業   
# Date & Author..: 07/09/13 By arman
# Modufy.........: No.FUN-8A0145 08/10/30 By arman 
# Modify.........: No.FUN-8A0151 08/11/01 By Carrier 去掉transaction內容
...
FUNCTION abmp999_bom(p_bmz02,p_a3)    
...
DEFINE   p_a3          LIKE type_file.chr20   
   ...
   LET g_a3 = p_a3
     ...
     CALL g_c.clear()
        ...
        CALL s_showmsg()
        IF g_success = 'Y' THEN
#          COMMIT WORK   #No.FUN-8A0151
        ELSE
#          ROLLBACK WORK   #No.FUN-8A0151
        END IF
...
FUNCTION p620()
  ...
  DEFINE #l_sql           LIKE type_file.chr1000 
         l_sql,ls_sql,l_sql5,l_sql6,l_sql7       STRING      #NO.FUN-910082
...
CALL p620_create_tbok_file()                          
CALL p620_create_tbmb_file()
   ...
   END IF
        IF g_a3 IS NULL THEN
         IF l_str IS NULL THEN
           LET l_sql = " SELECT bmb_file.* FROM bmb_file,bma_file ",
                       " WHERE (bmb09 IS NULL OR bmb09 = ' ') ",  
                       " AND bmb01=bma01 AND bma01='",g_bmz02,"' AND bmb04<='",g_today,"' ",
                       " AND (bmb05>'",g_today,"' OR bmb05 IS NULL OR bmb05='') ",     
                       " AND bmb29 = bma06 ",   #No.FUN-870127 
                       " AND bmb29 = ' ' ",   #No.FUN-870127 
                       " ORDER BY bmb02 "
         ELSE
           LET l_sql = " SELECT bmb_file.* FROM bmb_file,bma_file ",
        ...
        ELSE
         IF l_str IS NULL THEN
           LET l_sql = " SELECT bmb_file.* FROM bmb_file,bma_file ",
                       " WHERE (bmb09 IS NULL OR bmb09 = ' ') ",  
                       " AND bmb01=bma01 AND bma01='",g_bmz02,"' AND bmb04<='",g_today,"' AND (bmb05>'",g_today,"' ",
...
#     END IF
      ...
      IF NOT s_control(g_bmb.bmb01,g_bmb.bmb03,g_a3) THEN
      ...
      IF g_bmb.bmb30 = '1' THEN
          ...
          SELECT MAX(pbmb02)  INTO l_bok02 FROM pbmb_file      #NO.FUN-870127
           WHERE pbmb01 = g_c[z].c2                     
          ...
          INSERT INTO pbmb_file  
               VALUES  (g_c[z].c2,l_bok02,g_bmb.bmb03,g_bmb.bmb04,g_bmb.bmb05,
          ...
          DECLARE p620_bmt_curs
             ...
             FOREACH p620_bmt_curs INTO l_bmt.* 
               IF SQLCA.sqlcode THEN 
                   ...
                   CONTINUE FOREACH                                                       
               END IF 
               INSERT INTO tbmt_file VALUES(g_c[z].c2,l_bok02,g_bmb.bmb03,g_bmb.bmb04,l_bmt.bmt05,l_bmt.bmt06,l_bmt.bmt07,g_bmb.bmb29)
               IF SQLCA.sqlcode THEN
                    CALL cl_err(g_c[z].c2,SQLCA.sqlcode,0)
                    LET g_success = 'N'
                    RETURN                 
      ...
      ELSE
        IF g_bmb.bmb30 = '4' THEN
           ...
           DELETE FROM tbmb_file      
             ...
             FOREACH p620_boc_curs INTO l_boc.* 
                ...
                IF l_b = '1' THEN
                      SELECT imx01 INTO l_boj01a  FROM imx_file  WHERE  imx000=g_c[z].c2   
                END IF 
                IF l_b = '2' THEN
                        ...
                        INSERT INTO tbok_file VALUES (l_bod.bod02,l_a,l_bod.bod07,' ','2')
                          IF SQLCA.sqlcode THEN
                            CALL cl_err(l_bod.bod02,SQLCA.sqlcode,0)
                             ...
                             RETURN                 
                          END IF
                        #by arman 080702   --begin
                        ELSE
                            CALL s_errmsg('',g_c[z].c2 CLIPPED || "|" || l_boj01a CLIPPED || "|" || g_bmb.bmb03 CLIPPED,'','abm-027',1) 
                            LET g_totsuccess='N'
                            LET g_success = 'N'  
                        END IF
                        #by arman 080702   --end
             ...
             IF g_a3 IS NULL THEN             
               LET l_sql = " SELECT bmv04,MAX(bmv05) FROM bmv_file  ", 
                           ...
                           " AND boc02='",g_bmb.bmb03,"') GROUP BY bmv04 "
             ELSE
               LET l_sql = " SELECT bmv04,MAX(bmv05) FROM bmv_file  ", 
                           " WHERE bmv01='",g_bmb.bmb01,"' AND bmv02='",g_bmb.bmb03,"' AND bmv03='",g_bmb.bmb29,"' ",
                           " AND bmv05<='",g_today,"' AND (bmv06>'",g_today,"' OR bmv06 IS NULL OR bmv06='') ",
                           " AND bmv04 NOT IN(SELECT boc04 FROM boc_file WHERE boc01= '",g_bmb.bmb01,"' ", 
                           " AND boc02='",g_bmb.bmb03,"') AND bmv03='",g_a3,"'  GROUP BY bmv04 "
             END IF
        ...
        FOREACH p620_tbmb_curs INTO t_tbmb.*                           
          ...
          LET l_sql7 = " SELECT tbmb05 FROM tbmb_file WHERE tbmb01 = '",t_tbmb.tbmb01,"' ",
                       "                              AND (tbmb02 = '",t_tbmb.tbmb02,"' OR tbmb02 IS NULL)",      #No.FUN-870127
                       "                              AND (tbmb03 = '",t_tbmb.tbmb03,"' OR tbmb03 IS NULL) ",      #No.FUN-870127
                       "                              AND (tbmb04 = '",t_tbmb.tbmb04,"' OR tbmb04 IS NULL)  "    #NO.FUN-870127
           PREPARE p620_bod_3 FROM l_sql7 
            ...
            CURSOR WITH HOLD FOR p620_bod_3
             ...
             FOREACH p620_bod_curs_3 INTO l_tbmb05[l_p] 
              ...
              END IF
              LET l_boe07 = ''             #No.FUN-870127  by ve007
              LET l_boe08 = ''             #No.FUN-870127  by ve007 
                     ...
                     ELSE
                      ...
                      LET l_str1 = l_s[j+1,i]    
                       SELECT agb02 INTO l_f FROM agb_file WHERE agb01 = l_imaag AND agb03 = l_str1
                               IF l_f = '1' THEN
                                  SELECT imx01 INTO p_flag FROM imx_file WHERE imx000 = g_c[z].c2  
                               END IF
                               IF l_f = '2' THEN
                                  SELECT imx02 INTO p_flag FROM imx_file WHERE imx000 = g_c[z].c2  
                               END IF
                               IF l_f = '3' THEN
                                  SELECT imx03 INTO p_flag FROM imx_file WHERE imx000 = g_c[z].c2   
                               END IF
                      ELSE
                       LET l_str1 = l_s[j,i]    
                     ...
                     ELSE
                      IF l_s[j,j] = '$' THEN
                       ...
                       SELECT agb02 INTO l_f FROM agb_file WHERE agb01 = l_imaag  AND agb03 = l_str1
                               ...
                               END IF
                               IF l_f = '2' THEN
                                  SELECT imx02 INTO p_flag FROM imx_file WHERE imx000 = g_c[z].c2 END IF 
                               IF l_f = '3' THEN
                                  SELECT imx03 INTO p_flag FROM imx_file WHERE imx000 = g_c[z].c2 
                               END IF
                      ELSE
                    ...
                    IF i = length(l_s)  THEN
                     ...
                     ELSE
                      IF l_s[j,j] = '$' THEN
                       ...
                       SELECT agb02 INTO l_f FROM agb_file WHERE agb01 = l_imaag  AND agb03 = l_str1
                               IF l_f = '1' THEN
                                  SELECT imx01 INTO p_flag_1  FROM imx_file WHERE imx000 = g_c[z].c2 
                               END IF
                               IF l_f = '2' THEN
                                  SELECT imx02 INTO p_flag_1 FROM imx_file WHERE imx000 = g_c[z].c2  
                               END IF
                               IF l_f = '3' THEN
                                  SELECT imx03 INTO p_flag_1 FROM imx_file WHERE imx000 = g_c[z].c2  
                               END IF
                      ELSE
                       LET l_str1 = l_s[j,i]    
...
#FUN-B30219 --End
          ...
          SELECT sma46 INTO l_sma46 FROM sma_file                     
              ...
              INSERT INTO tbmt_file VALUES(g_c[z].c2,l_bok02,l_bok03,g_bmb.bmb04,l_p,l_tbmb05[l_p],t_tbmb.tbmb06,g_bmb.bmb29)
                        IF SQLCA.sqlcode THEN
                             CALL cl_err(g_c[z].c2,SQLCA.sqlcode,0)
                             LET g_success = 'N'
                             RETURN                 
             ...
             END FOREACH
              ...
              IF l_m <= 0 THEN
                 ...
                 SELECT * INTO l_ima.* FROM ima_file WHERE ima01 = g_bmb.bmb03             
              ...
              END IF
                   ...
                   INSERT INTO pbmb_file
                    VALUES(g_bok.bok01,g_bok.bok02,g_bok.bok03,g_bmb.bmb04,g_bmb.bmb05,g_bok.bok06,
