...
#
# Pattern name...: abmr671.4gl
# Descriptions...: 取代料件使用清單
# Input parameter:
# Return code....:
...
### *** FUN-850049 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>>--*** ##                                                     
    ...
    LET l_table = cl_prt_temptable('abmr671',g_sql) CLIPPED   # 產生Temp Table
...
#----------------------------------------------------------CR (1) ------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL r671_tm(0,0)		# Input print condition
      ELSE CALL abmr671()			# Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690107
END MAIN
 
FUNCTION r671_tm(p_row,p_col)
   ...
   OPEN WINDOW r671_w AT p_row,p_col
        WITH FORM "abm/42f/abmr671"
################################################################################
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW r671_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690107
      EXIT PROGRAM
...
################################################################################
      ...
      ON ACTION CONTROLP CALL r671_wc()   # Input detail Where Condition
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('abmr671',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW r671_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690107
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8	    #No.FUN-6A0060
          l_sql 	LIKE type_file.chr1000, # RDSQL STATEMENT    #No.FUN-680096
          ...
          sr RECORD
             ...
             bmd01 LIKE bmd_file.bmd01,        #被取代料件
             bmd05 LIKE bmd_file.bmd05,        #生效日
             bmd06 LIKE bmd_file.bmd06,        #失效日
...
##No.FUN-850049-Add--End--#
   ...
   #------------------------------ CR (2) ------------------------------#
     ...
     PREPARE r671_prepare1 FROM l_sql
     ...
     END IF
     DECLARE r671_cs1 CURSOR FOR r671_prepare1
 
#    CALL cl_outnam('abmr671') RETURNING l_name                                         #No.FUN-850049
#    START REPORT r671_rep TO l_name                                                    #No.FUN-850049
 
     LET g_pageno = 0
     FOREACH r671_cs1 INTO sr.*
       IF SQLCA.sqlcode THEN
...
#No.FUN-850049--Add--Begin--#
     ...
     ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-850049 *** ##                                                      
         EXECUTE insert_prep USING                                                                                                  
                 ...
                 sr.ima11,sr.ima12,sr.ima08,sr.ima63,sr.bmd01,sr.bmd05,sr.bmd06,                                                     
                 sr.bmd07,l_ima02,l_ima021,l_ima05,l_ima08                                                                 
     #------------------------------ CR (3) ------------------------------#                                                         
#No.FUN-850049--Add--End--#
     END FOREACH
     ...
     END IF
    #DISPLAY '' AT 2,1
#    FINISH REPORT r671_rep                                                            #No.FUN-850049
...
#No.FUN-850049--end                                                                                                                 
 ## **** 與 Crystal Reports 串聯段 - <<<< CALL cs3() >>>> FUN-850049 **** ##                                                        
    ...
    CALL cl_prt_cs3('abmr671','abmr671',g_sql,g_str)                                                                                
    #------------------------------ CR (4) ------------------------------#
...
#        LET l_ima021 = ' '
#     END IF
#     PRINTX name=D1 COLUMN g_c[34],sr.bmd01,
...
#     END IF
#     PRINTX name=D3 COLUMN g_c[44],l_ima021        #No.TQC-5B0030 modify
#     LET l_cnt = l_cnt + 1
