...
MAIN
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      ...
      ELSE CALL abmr001()			# Read data and create out-file
   END IF
...
################################################################################
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW r001_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690107
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('abmr001',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW r001_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690107
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8	    #No.FUN-6A0060
	...
	END RECORD
     ...
     SELECT zz17,zz05 INTO g_len,g_zz05 FROM zz_file WHERE zz01 = 'abmr001'
#     IF g_len = 0 OR cl_null(g_len) THEN LET g_len = 75 END IF
	...
	INITIALIZE sr_null.* TO NULL
     CALL cl_outnam('abmr001') RETURNING l_name
 START REPORT r001_rep TO l_name
     LET g_pageno = 1
     LET p_level = 0
     FOREACH r001_curs1 INTO sr.bmb03,sr.ima02,g_bma06 #FUN-550095 add
       IF SQLCA.sqlcode != 0 THEN
       ...
       LET g_pageno = 0
       OUTPUT TO REPORT r001_rep(p_level,sr.*)
       OUTPUT TO REPORT r001_rep(p_level+1,sr_null.*)
       LET g_end = 0
       IF cl_null(g_bma06) THEN LET g_bma06 = ' ' END IF #FUN-550095 add
       CALL r001_bom(0,sr.bmb03,g_bma06) #FUN-550095 add g_bma06
       LET g_end = 1	
     END FOREACH	
...
FUNCTION r001_bom(p_level,p_key,p_key2) #FUN-550014 add p_key2
	...
	FOR i = 1 TO l_count
          ...
          OUTPUT TO REPORT r001_rep(p_level,sr[i].*)
          SELECT bma01 FROM bma_file
                       WHERE bma01 = sr[i].bmb03
          IF status != NOTFOUND AND p_level < tm.x THEN
             OUTPUT TO REPORT r001_rep(p_level+1,sr[600].*)
             #No.TQC-740125 --start--
#            CALL r001_bom(p_level,sr[i].bmb03,' ') #FUN-550095 add #No.TQC-740125 mark
             SELECT bma06 INTO l_bma06 FROM bma_file
              WHERE bma01 = sr[i].bmb03
             IF cl_null(l_bma06) THEN LET l_bma06 = ' ' END IF
            #CALL r001_bom(p_level,sr[i].bmb03,l_bma06)  #TQC-8C0063
             CALL r001_bom(p_level,sr[i].bmb03,l_ima910[i])  #TQC-8C0063
             #No.TQC-740125 --end--
          END IF
	END FOR
END FUNCTION
