...
#
# Pattern name...: abmr681.4gl
# Descriptions...: 替代料件用途清單
# Input parameter:
# Return code....:
...
# Modify.........: No.FUN-850049 08/05/11 By xiaofeizhu 將報表輸出改為CR
...
#----------------------------------------------------------CR (1) ------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'		# If background job sw is off
      THEN CALL r681_tm(0,0)		# Input print condition
      ELSE CALL abmr681()			# Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690107
END MAIN
 
FUNCTION r681_tm(p_row,p_col)
   ...
   OPEN WINDOW r681_w AT p_row,p_col
        WITH FORM "abm/42f/abmr681"
################################################################################
...
END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW r681_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690107
      EXIT PROGRAM
...
################################################################################
      ...
      ON ACTION CONTROLP CALL r681_wc()   # Input detail Where Condition
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('abmr681',g_time,l_cmd)	# Execute cmd at later time
      END IF
      CLOSE WINDOW r681_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690107
      EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8	    #No.FUN-6A0060
          ...
          sr RECORD
             ...
             bmd03 LIKE bmd_file.bmd03,        #被替代順序
             bmd01 LIKE bmd_file.bmd01,        #被取代料件
             bmd05 LIKE bmd_file.bmd05,        #被取代料件
             bmd06 LIKE bmd_file.bmd06,        #被取代料件
             bmd07 LIKE bmd_file.bmd07,        #被取代料件 #CHI-A70008 add ,
  ...
  #CHI-A70008 add --end--
   ...
   #------------------------------ CR (2) ------------------------------#
     ...
     LET l_sql = "SELECT '','','',",
         ...
         " ima11,ima12,ima08,ima63,",
         " bmd03,bmd01,bmd05,bmd06,bmd07,bmd08 ", #CHI-A70008 add bmd08
     ...
     PREPARE r681_prepare1 FROM l_sql
     ...
     END IF
     DECLARE r681_cs1 CURSOR FOR r681_prepare1
 
#    CALL cl_outnam('abmr681') RETURNING l_name                                      #No.FUN-850049    
#    START REPORT r681_rep TO l_name                                                 #No.FUN-850049                                                
 
     LET g_pageno = 0
     FOREACH r681_cs1 INTO sr.*
       IF SQLCA.sqlcode THEN
...
#No.FUN-850049--Add--Begin--#
     ...
     ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-850049 *** ##                                                      
         EXECUTE insert_prep USING                                                                                                  
                 ...
                 sr.ima11,sr.ima12,sr.ima08,sr.ima63,sr.bmd03,sr.bmd01,sr.bmd05,                                                     
                 sr.bmd06,sr.bmd07,l_ima02,l_ima021,l_ima05,l_ima08,sr.bmd08,l_ima02_h2,l_ima021_h2 #CHI-A70008 add sr.bmd08,l_ima02_h2,l_ima021_h2                                                                 
     #------------------------------ CR (3) ------------------------------#                                                         
#No.FUN-850049--Add--End--#                                                                                                         
     ...
     END IF
    #DISPLAY '' AT 2,1
#    FINISH REPORT r681_rep                                                            #No.FUN-850049     
...
#No.FUN-850049--end                                                                                                                 
 ## **** 與 Crystal Reports 串聯段 - <<<< CALL cs3() >>>> FUN-850049 **** ##                                                        
    ...
    CALL cl_prt_cs3('abmr681','abmr681',g_sql,g_str)                                                                                
    #------------------------------ CR (4) ------------------------------#
