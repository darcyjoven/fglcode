...
# Modify.........: No.FUN-CB0072 12/11/16 By dongsz CR轉GR
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD                              # Print condition RECORD
             #wc      LIKE type_file.chr1000,     # Where condition          #NO.FUN-680082 VARCHAR(600)  #FUN-CB0072 mark
...
###GENGRE###START
TYPE sr1_t RECORD
    l_order1 LIKE mss_file.mss01,       #FUN-CB0072 add
    l_order2 LIKE mss_file.mss01,       #FUN-CB0072 add
    l_order3 LIKE mss_file.mss01,       #FUN-CB0072 add
...
# No.FUN-750095--begin--
  ...
  LET g_sql ="l_order1.mss_file.mss01,",    #FUN-CB0072 add
             "l_order2.mss_file.mss01,",    #FUN-CB0072 add
             "l_order3.mss_file.mss01,",    #FUN-CB0072 add
  ...
  LET l_table = cl_prt_temptable('amrg540',g_sql) CLIPPED
  IF l_table = -1 THEN 
...
# No.FUN-750095--end--
   IF cl_null(g_bgjob) OR g_bgjob='N' THEN    # If background job sw is off
      CALL amrg540_tm()        # Input print condition
   ELSE 
      CALL amrg540()           # Read data and create out-file
   END IF
...
DEFINE lc_qbe_sn          LIKE gbm_file.gbm01     #No.FUN-580031
   ...
   OPEN WINDOW amrg540_w AT p_row,p_col
        WITH FORM "amr/42f/amrg540" 
   CALL cl_ui_init()
...
#No.FUN-570240 --start
      ...
      ON ACTION controlp 
         CASE                                 #FUN-CB0072 add                                                                                                
       # IF INFIELD(mss01) THEN               #FUN-CB0072 mark
         WHEN INFIELD(mss01)                  #FUN-CB0072 add                                                                                   
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amrg540_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      CALL cl_gre_drop_temptable(l_table)                  #FUN-CB0072 add
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amrg540',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amrg540_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      CALL cl_gre_drop_temptable(l_table)                  #FUN-CB0072 add
      EXIT PROGRAM
   END IF
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0076
         #l_sql     LIKE type_file.chr1000, # RDSQL STATEMENT     #NO.FUN-680082 VARCHAR(1000)   #FUN-CB0072 mark
...
# No.FUN-750095--begin--
     LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED," values(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?) " #FUN-CB0072 add 3?
...
# No.FUN-750095--end--
     ...
     PREPARE amrg540_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        CALL cl_gre_drop_temptable(l_table)                  #FUN-CB0072 add
        EXIT PROGRAM 
     END IF
     DECLARE amrg540_curs1 CURSOR FOR amrg540_prepare1
      ...
      PREPARE amrg540_premss  FROM l_sql
      IF STATUS THEN CALL cl_err('pre mss:',STATUS,1) 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
         CALL cl_gre_drop_temptable(l_table)                  #FUN-CB0072 add
         EXIT PROGRAM 
      END IF
      DECLARE amrg540_curmss CURSOR FOR amrg540_premss   
      ...
      PREPARE amrg540_premst  FROM l_sql
      IF STATUS THEN CALL cl_err('pre mst:',STATUS,1) 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
         CALL cl_gre_drop_temptable(l_table)                  #FUN-CB0072 add
         EXIT PROGRAM 
      END IF
      DECLARE r540_c2        CURSOR FOR amrg540_premst   
      ...
      PREPARE amrg540_pretmp  FROM l_sql
      IF STATUS THEN CALL cl_err('pre tmp:',STATUS,1) 
         CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
         CALL cl_gre_drop_temptable(l_table)                  #FUN-CB0072 add
         EXIT PROGRAM 
      END IF
      DECLARE r540_c3        CURSOR FOR amrg540_pretmp   
 
#    CALL cl_outnam('amrg540') RETURNING l_name        # No.FUN-750095
#    START REPORT amrg540_rep TO l_name                # No.FUN-750095
     LET g_pageno = 0
     FOREACH amrg540_curs1 INTO mss.mss01,mss.mss02
      IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
      LET l_max = 'Y' 
 
      FOREACH amrg540_curmss  USING mss.mss01,mss.mss02
        INTO mss.* 
...
#         OUTPUT TO REPORT amrg540_rep(sr.*,mss.*, ima.*,mst.*,l_lastbal) 
          EXECUTE insert_prep USING sr.*,mss.mss00,mss.mss01,mss.mss02,l_lastbal,l_pmc03,mst.mst04,      #FUN-CB0072 add sr.*
                                    mst.mst06,mst.mst061,mst.mst07,mst.mst08,
                                    ima.ima01,ima.ima02,ima.ima08,ima.ima25,ima.ima43,ima.ima67,
                                    l_ima02,l_ima25,mss.mss03,mst.mst05
...
# No.FUN-750095--end
     ...
     END FOREACH
#    FINISH REPORT amrg540_rep                       # No.FUN-750095
     DROP TABLE r540_tmp
# No.FUN-750095--begin--
...
###GENGRE###     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                                 
     ...
     IF g_zz05 = 'Y' THEN                                                                                                             
        ...
        LET g_str = tm.wc                                                                                                             
     END IF                                                                                                                           
###GENGRE###     LET g_str = g_str,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",tm.t                                                    
###GENGRE###                      ,";",tm.ver_no,";",tm.edate                                                                            
###GENGRE###     CALL cl_prt_cs3('amrg540','amrg540',l_sql,g_str)
    CALL amrg540_grdata()    ###GENGRE###
# No.FUN-750095--end--
END FUNCTION
 
# No.FUN-750095--begin--
{
REPORT amrg540_rep(sr,mss, ima,mst,p_lastbal)
...
FUNCTION amrg540_grdata()
    ...
    WHILE TRUE
        ...
        LET handler = cl_gre_outnam("amrg540")
        IF handler IS NOT NULL THEN
            START REPORT amrg540_rep TO XML HANDLER handler
            LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
                       ," ORDER BY l_order1,l_order2,l_order3,mss01,mss02,mss03,mst04 DESC,mst05"    #FUN-CB0072 add
                       #"ORDER BY l_order1,l_order2,l_order3,mss01,mss02,mss03,mst04 DESC,mst05"    #FUN-CB0072 add
          
            DECLARE amrg540_datacur1 CURSOR FROM l_sql
            FOREACH amrg540_datacur1 INTO sr1.*
                OUTPUT TO REPORT amrg540_rep(sr1.*)
            END FOREACH
            FINISH REPORT amrg540_rep
        END IF
...
REPORT amrg540_rep(sr1)
    ...
    ORDER EXTERNAL BY sr1.l_order1,sr1.l_order2,sr1.l_order3,sr1.mss01,sr1.mss02      #FUN-CB0072 add sr1.l_order1,sr1.l_order2,sr1.l_order3
    ...
    FORMAT
       ...
       #FUN-CB0072--add--end---
        ...
        BEFORE GROUP OF sr1.mss01
        BEFORE GROUP OF sr1.mss02
        ...
        AFTER GROUP OF sr1.mss01
        AFTER GROUP OF sr1.mss02
       #AFTER GROUP OF sr1.mss03
       #AFTER GROUP OF sr1.mst04
       #AFTER GROUP OF sr1.mst05
