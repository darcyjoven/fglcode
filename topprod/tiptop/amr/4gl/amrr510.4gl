...
### *** FUN-830153 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>>--*** ##                                                     
    ...
    LET l_table = cl_prt_temptable('amrr510',g_sql) CLIPPED   # 產生Temp Table                                                      
...
#----------------------------------------------------------CR (1) ------------#  
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amrr510_tm(0,0)        # Input print condition
      ELSE CALL amrr510()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amrr510_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01     #No.FUN-580031
   ...
   OPEN WINDOW amrr510_w AT p_row,p_col
        WITH FORM "amr/42f/amrr510" 
################################################################################
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amrr510_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
#                   CALL FGL_DIALOG_SETBUFFER( tm.part2 )
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amrr510',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amrr510_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
   ...
   #------------------------------ CR (2) ------------------------------#  
     CASE WHEN tm.part1 IS NOT NULL
               ...
               LET l_sql = "SELECT mss_file.*, ima_file.*",
                           "  FROM r510_tmp, mss_file, ima_file",
          ...
          OTHERWISE
               LET l_sql = "SELECT mss_file.*, ima_file.*",
                           ...
                           "   AND mss_v='",tm.ver_no,"'"
     END CASE
     PREPARE amrr510_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amrr510_curs1 CURSOR FOR amrr510_prepare1 #No.FUN-830153
 
#     CALL cl_outnam('amrr510') RETURNING l_name     #No.FUN-830153 
#     START REPORT amrr510_rep TO l_name
#     LET g_pageno = 0                               #No.FUN-830153
     FOREACH amrr510_curs1 INTO mss.*, ima.*
...
#       END IF
#No.FUN-830153--Mark--Begin--  
       DECLARE amrr510_curs2 CURSOR FOR
          SELECT * FROM mst_file
            WHERE mst01=mss.mss01 AND mst02=mss.mss02 AND mst03=mss.mss03
              AND mst_v=mss.mss_v
       FOREACH amrr510_curs2 INTO mst.*
         #message 'mst:',mst.mst01 clipped
#         OUTPUT TO REPORT amrr510_rep(l_order,mss.*, mst.*, ima.*)   #No.FUN-830153
...
#No.FUN-830153--Add--End--  
    ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-830153 *** ##                                                       
        EXECUTE insert_prep USING                                                                                                   
               ...
               mst.mst06,mst.mst05,mst.mst04,ima.ima25,ima.ima02,ima.ima67,ima.ima43,l_buf                                          
    #------------------------------ CR (3) ------------------------------#     
...
#             OUTPUT TO REPORT amrr510_rep(l_order,mss.*, mst.*, ima.*)
#No.FUN-830153--Add--Begin--                                                                                                        
     LET l_buf=s_mst05(g_lang,mst.mst05)                                                                                            
     LET l_buf=mst.mst05,l_buf                                                                                                      
#No.FUN-830153--Add--End--                                                                                                          
     ...
     ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-830153 *** ##                                                      
         EXECUTE insert_prep USING                                                                                                  
               #mss.mss03,mss.mss02,mss.mss01,mss.mss00,l_str,mst.mst07,mst.mst061,          #MOD-B20029 mark                                        
                ...
                mst.mst06,'71',mss.mss03,ima.ima25,ima.ima02,ima.ima67,ima.ima43,l_buf                                              
     #------------------------------ CR (3) ------------------------------#  
          END IF
       END IF
...
#No.FUN-830153--Add--End--                                                                                                          
     ...
     ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-830153 *** ##                                                      
         EXECUTE insert_prep USING                                                                                                  
                ...
                mst.mst06,l_str1,mst.mst04,ima.ima25,ima.ima02,ima.ima67,ima.ima43,l_buf                                            
     #------------------------------ CR (3) ------------------------------#  
...
#No.FUN-830153--end                                                                                                                 
 ## **** 與 Crystal Reports 串聯段 - <<<< CALL cs3() >>>> FUN-830153 **** ##                                                        
    ...
    LET g_str = ''                                                                                                                  
    LET g_str = tm.wc,";",tm.order,";",tm.ver_no,";",tm.part1,";",tm.part2,";",tm.print_adj,";",tm.print_plp                        
    CALL cl_prt_cs3('amrr510','amrr510',g_sql,g_str)                                                                                
    #------------------------------ CR (4) ------------------------------#   
END FUNCTION
...
FUNCTION r510_bom1_main()
   ...
   IF STATUS THEN 
# Prog. Version..: '5.30.06-13.03.12(0) r510_tmp:',STATUS,1)  #No.FUN-660107
    CALL cl_err3("ins","r510_tmp","","",STATUS,"","ins(0) r510_tmp:",1)        #NO.FUN-660107
   END IF
   #FUN-550110
   LET l_ima910=''
...
END FUNCTION
#NO.FUN-830153 -Mark--Begin--#   
#REPORT amrr510_rep(l_order,mss, mst, ima)
