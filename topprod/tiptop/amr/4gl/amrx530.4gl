...
# Modify.........: No.FUN-CB0003 12/11/08 By chenjing CR轉XtraGrid
...
#--------FUN-750112--begin--CR(1)-----------#                                                                                       
    ...
    LET l_table = cl_prt_temptable('amrx530',g_sql) CLIPPED                                                                         
    IF l_table = -1 THEN EXIT PROGRAM END IF                                                                                        
    LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,                                                                           
                " VALUES(?,?,?,?,?,?,?,?,?,?,? ,?,?,?)"    #FUN-CB0003 add 3?                                                                                                   
...
#-------------------No.FUN-750112--end--CR(1)---------------#                                                                       
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amrx530_tm(0,0)        # Input print condition
      ELSE CALL amrx530()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amrx530_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW amrx530_w AT p_row,p_col
        WITH FORM "amr/42f/amrx530" 
################################################################################
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amrx530_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amrx530',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amrx530_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#         l_time    LIKE type_file.chr8     #No.FUN-6A0076
   ...
   DEFINE l_pmc03 LIKE pmc_file.pmc03    #FUN-CB0003 
...
#No.FUN-750112--end--CR(2)  
     ...
     LET l_sql = "SELECT *",
                 ...
                 "   AND mss_v='",tm.ver_no,"'"
     PREPARE amrx530_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amrx530_curs1 CURSOR FOR amrx530_prepare1
 
#No.FUN-750112--begin-----------------------------------------------#
#     CALL cl_outnam('amrx530') RETURNING l_name
#     START REPORT amrx530_rep TO l_name
#     LET g_pageno = 0
#No.FUN-750112--end-------------------------------------------------#
     FOREACH amrx530_curs1 INTO mss.*, ima.*
       IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
       LET l_num = 0                                #FUN-CB0003
       SELECT pmc03 INTO l_pmc03 FROM pmc_file WHERE pmc01 = mss.mss02   #FUN-CB0003     
#No.FUN-750112--begin-----------------------------------------------#
#       FOR g_cnt = 1 TO 3
#         CASE
...
## Crystal Reports insert temp table --CR(3) ## 
            EXECUTE insert_prep USING                                                                                                   
                  ...
                  ima.ima67,ima.ima43,ima.ima021,l_pmc03,l_num    #FUN-CB0003 add ima.ima021,l_pmc03,l_num
...
## Crystal Reports insert temp table --CR(3) ## 
          EXECUTE insert_prep USING                                                                                                   
             ...
             ima.ima43,ima.ima021,l_pmc03,l_num    #FUN-CB0003 add ima.ima021,l_pmc03,l_num
## insert temp table--end--CR(3)             ##
       END IF
#       OUTPUT TO REPORT amrx530_rep(sr.*,mss.*, ima.*)
     END FOREACH
 
#     FINISH REPORT amrx530_rep
...
###XtraGrid###     CALL cl_prt_cs3('amrx530','amrx530',l_sql,g_str)                                                                                
