...
#             "plan.mss_file.mss065,",    #Plan(計劃產)                      #No.TQC-970294                                         
   ...
   LET l_table = cl_prt_temptable('amrr800',g_sql) CLIPPED   # 產生Temp Table 
   IF l_table = -1 THEN 
      EXIT PROGRAM 
   END IF        
   # Temp Table產生   
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amrr800_tm(0,0)        # Input print condition
      ELSE CALL amrr800()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amrr800_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   END IF
   OPEN WINDOW amrr800_w AT p_row,p_col
        WITH FORM "amr/42f/amrr800" 
################################################################################
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amrr800_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amrr800',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amrr800_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0076
     ...
     CALL cl_outnam('amrr800') RETURNING l_name
     START REPORT amrr800_rep TO l_name
     LET g_pageno = 0
     PREPARE amrr800_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amrr800_curs1 CURSOR FOR amrr800_prepare1
     FOREACH amrr800_curs1 INTO l_mss01
       ...
       ELSE
          ...
          END IF
       END IF
       OUTPUT TO REPORT amrr800_rep(ss.*)
     END FOREACH
     FINISH REPORT amrr800_rep
     CALL cl_prt(l_name,g_prtway,g_copies,g_len)
END FUNCTION
 
REPORT amrr800_rep(sr)
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0076
   ...
   FOREACH amrr800_curs1 INTO l_mss01
      ...
      EXECUTE insert_prep USING ss.order1,ss.ima01,ss.ima02,ss.ima08,
                                ...
                                ss.plan,l_total,ss.ima23,ss.ima67
   ...
   IF cl_null(g_str) THEN
      LET g_str=' ' 
   END IF
   LET g_str = g_str,";",tm.ver_no,";",tm.edate,";",tm.maxqty,";",tm.l_sw,";",tm.order   
   CALL cl_prt_cs3('amrr800','amrr800',l_sql,g_str)   #FUN-710080 modify
   #NO.MOD-720041 07/02/08 BY TSD.GILL --END
