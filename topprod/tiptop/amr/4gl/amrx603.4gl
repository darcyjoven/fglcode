...
# Modify.........: No.FUN-D40128 13/05/14 By wangrr 報表增加"規格"欄位
...
MAIN
   ...
   LET g_sql = "tmp_g01.msu_file.msu01,",
               ...
               "tmp_g03_1.msu_file.msu03,",
               "l_num.type_file.num5"  #FUN-CB0001 add
   LET l_table = cl_prt_temptable('amrx603',g_sql) CLIPPED
...
DEFINE s_gplantv,d_gplantv  LIKE msv_file.msv032
   ...
   OPEN WINDOW x603_w AT p_row,p_col WITH FORM "amr/42f/amrx603"
...
WHILE TRUE
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amrx603',g_time,l_cmd)
      END IF
      CLOSE WINDOW x603_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
DEFINE sr        RECORD order1  LIKE msu_file.msu01,  #NO.FUN-680082 VARCHAR(20)
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'amrx603'    #No.FUN-850143
     ...
     PREPARE x603_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM
     END IF
     DECLARE lcur_tmpmrp CURSOR FOR x603_prepare1
     #CALL cl_outnam('amrx603') RETURNING l_name     #No.FUN-850143
     #START REPORT x603_rep TO l_name                #No.FUN-850143
     LET g_pageno = 0
     FOREACH lcur_tmpmrp INTO sr_tmpmrp.*
          ...
          LET l_ima02=NULL   #FUN-D40128
          LET l_ima021=NULL  #FUN-D40128
          SELECT ima02,ima021 INTO l_ima02,l_ima021 FROM ima_file  #FUN-D40128 add ima021
          ...
          EXECUTE insert_prep USING
               ...
               l_tmp_g01,l_ima02,l_ima021,sr_tmpmrp.tmp_g02,l_pmc03,       #TQC-9C0179  #FUN-D40128 add l_ima021
               sr_tmpmrp.tmp_g03,sr_tmpmrp.tmp_sgplant,sr_tmpmrp.tmp_sgplantv,
               sr_tmpmrp.tmp_dgplant,sr_tmpmrp.tmp_dgplantv,sr_tmpmrp.tmp_qty,sr_tmpmrp.tmp_g03,l_num  #FUN-CB0001 add sr_tmpmrp.tmp_g03,l_num                  
          #OUTPUT TO REPORT x603_rep(sr.*,sr_tmpmrp.*)
          #No.FUN-850143-----end
     END FOREACH
...
###XtraGrid###                 tm.t[1,1],";",tm.t[2,2],";",tm.t[3,3],";",tm.msu000
     
###XtraGrid###     CALL cl_prt_cs3('amrx603','amrx603',g_sql,g_str)
    ...
    IF g_zz05 = 'Y' THEN
        ...
        RETURNING tm.wc
     END IF
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"tmp_g01,tmp_g02,tmp_g03,tmp_sgplant,tmp_dgplant")   
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"tmp_g01,tmp_g02,tmp_g03,tmp_sgplant,tmp_dgplant")  #FUN-D30053 mark 
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"tmp_g01,tmp_g02,tmp_g03,tmp_sgplant,tmp_dgplant")   
    LET g_xgrid.footerinfo1 = cl_getmsg('amr-310',g_lang),':',tm.msu000
   #LET l_str = cl_wcchp(g_xgrid.order_field,"tmp_g01,tmp_g02,tmp_g03,tmp_sgplant,tmp_dgplant") #FUN-D30070 mark
