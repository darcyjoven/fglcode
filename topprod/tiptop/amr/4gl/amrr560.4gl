...
#No.FUN-750094 -- begin --
   ...
   LET l_table = cl_prt_temptable('amrr560',g_sql) CLIPPED
...
#No.FUN-750094 -- end --
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amrr560_tm(0,0)        # Input print condition
      ELSE CALL amrr560()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amrr560_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01     #No.FUN-580031
   ...
   OPEN WINDOW amrr560_w AT p_row,p_col
        WITH FORM "amr/42f/amrr560" 
################################################################################
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amrr560_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amrr560',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amrr560_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0076
   ...
   DEFINE l_ima25   LIKE ima_file.ima25     #No.FUN-750094
     ...
     LET l_sql = "SELECT mst_file.*, ima25, sfb05, msb03,ima08,ima67,ima43 ",
                 ...
                 "   AND mst05='53'",
                 "   AND mst_v='",tm.ver_no,"'"
     PREPARE amrr560_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amrr560_curs1 CURSOR FOR amrr560_prepare1
 
#No.FUN-750094 -- begin --
#     CALL cl_outnam('amrr560') RETURNING l_name
#     START REPORT amrr560_rep TO l_name
#     LET g_pageno = 0
#No.FUN-750094 -- end --
     FOREACH amrr560_curs1 INTO mst.*, sr.*
...
#       OUTPUT TO REPORT amrr560_rep(tr.*,mst.*, sr.*)
         EXECUTE insert_prep USING sr.ima08,sr.ima67,sr.ima43,mst.mst01,l_ima02_1,
                 sr.ima25,mst.mst02,l_pmc03,mst.mst03,mst.mst08,mst.mst07,
                 l_ima02_2,mst.mst06,sr.sfb05,l_ima02_3
         IF STATUS THEN
...
#No.FUN-750094 -- begin --
#     FINISH REPORT amrr560_rep
#     CALL cl_prt(l_name,g_prtway,g_copies,g_len)
     CALL cl_wcchp(tm.wc,'mst01,ima67,mst02,ima08,ima43,mst03')
          RETURNING tm.wc
     LET g_str = tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",tm.t,";",tm.wc,";",g_zz05
     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     CALL cl_prt_cs3('amrr560','amrr560',l_sql,g_str)
#No.FUN-750094 -- end --
END FUNCTION
 
#No.FUN-750094 -- begin --
#REPORT amrr560_rep(tr,mst, sr)
