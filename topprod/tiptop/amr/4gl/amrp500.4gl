...
# Modify.........: No:MOD-D60220 13/06/28 By bart 還原CHI-C50068|MOD-D20082修改
# Modify.........: No:CHI-D40027 13/07/08 By lixh1 還原FUN-B20060_4的修改
# Modify.........: No:MOD-D80141 13/08/22 By suncx 計算在制量過濾拆件式工單
...
#DEFINE m                LIKE type_file.num10    #CHI-C50068  #MOD-D60220
...
#DEFINE po_days1         LIKE type_file.num10    #CHI-C50068 #MOD-D60220
#DEFINE wo_days1         LIKE type_file.num10    #CHI-C50068 #MOD-D60220
...
#     DEFINE   l_time   LIKE type_file.chr8          #No.FUN-6A0076
  #DEFINE   p_row,p_col LIKE type_file.num5     #NO.FUN-680082  SMALLINT #FUN-940048
   ...
   #LET po_days1 = ARG_VAL(12)  #CHI-C50068 #MOD-D60220
   #LET wo_days1 = ARG_VAL(13)  #CHI-C50068 #MOD-D60220
...
FUNCTION p500_wc_default()
   ...
   IF STATUS=0
      THEN LET lot_type=msr.lot_type
           ...
           #LET po_days1 =msr.msr13  #CHI-C50068 #MOD-D60220
           #LET wo_days1 =msr.msr14  #CHI-C50068 #MOD-D60220
...
#TQC-A30021 --end--
      ...
      ELSE LET lot_type='1'
           ...
           LET po_days =0
           LET wo_days =0
           #LET po_days1 =0  #CHI-C50068 #MOD-D60220
           #LET wo_days1 =0  #CHI-C50068 #MOD-D60220
           LET incl_id='Y'
           LET incl_so='Y'
           LET msb_expl='Y'
           LET mss_expl='Y'
...
WHILE TRUE
  ...
  INPUT BY NAME ver_no,lot_type,lot_bm,lot_no1,lot_no2,bdate,edate,
                buk_type,buk_code,po_days,wo_days,#po_days1,wo_days1,  #CHI-C50068 #MOD-D60220
     ...
     AFTER FIELD ver_no
        ...
        #IF lot_type IS NULL THEN  #MOD-C80131 mark
           ...
           DISPLAY BY NAME lot_type, lot_bm,   lot_no1 ,lot_no2  ,
                           ...
                           buk_type, buk_code,
                           po_days, wo_days,
                           #po_days1, wo_days1,  #CHI-C50068 #MOD-D60220
  ...
  IF g_bgjob = "Y" THEN
      ...
      ELSE
         LET lc_cmd = lc_cmd CLIPPED,
                      ...
                      " '",po_days CLIPPED,"'",
                      " '",wo_days CLIPPED,"'",
                      #" '",po_days1 CLIPPED,"'",  #CHI-C50068 #MOD-D60220
                      #" '",wo_days1 CLIPPED,"'",  #CHI-C50068 #MOD-D60220
                      " '",incl_id CLIPPED,"'",
                      " '",incl_so CLIPPED,"'",
                      " '",msb_expl CLIPPED,"'",
                      " '",mss_expl CLIPPED,"'",
...
FUNCTION p500_ins_msr1()
  DEFINE l_msr02   LIKE msr_file.msr02   #NO.FUN-680082  VARCHAR(8)
   ...
   #LET msr.msr13  =po_days1  #CHI-C50068 #MOD-D60220
   #LET msr.msr14  =wo_days1  #CHI-C50068 #MOD-D60220
...
FUNCTION p500_mss061()  # 彙總 請購量
 ...
 #DEFINE l_sql          LIKE type_file.chr1000 #NO.FUN-680082  VARCHAR(3000)  #MOD-BC0226  mark
  ...
  LET l_sql = l_sql CLIPPED,
              ...
              "   AND pmk18 <> 'X'",  #CHI-C50068 #MOD-D60220
                          #"   AND pmk18 = 'Y'", #CHI-C50068  #MOD-D60220
...
REPORT p500_rep(p_mss_v,p_mss01,p_mss02,p_mss03, mss,l_chr)     #No.MOD-880201 add l_chr
  ...
  #DEFINE bal2           LIKE mst_file.mst08   #CHI-C50068 #MOD-D60220
  #DEFINE l_ima721       LIKE ima_file.ima721  #CHI-C50068 #MOD-D60220
  ...
  FORMAT
    BEFORE GROUP OF mss.mss01
      SELECT ima08,ima45,ima46,ima47,ima50+ima48+ima49+ima491,
                   ima56,ima561,ima562,
                   ima59,ima60,ima601,ima61,ima72,#ima721,  #FUN-840194 add ima601  #CHI-C50068 #MOD-D60220
                   ima25,ima44,ima55                   #No.TQC-740336 add
        INTO l_ima08,l_ima45,l_ima46,l_ima47,l_ima50,
                     l_ima56,l_ima561,l_ima562,
                     l_ima59,l_ima60,l_ima601,l_ima61,l_ima72,#l_ima721,  #FUN-840194 add l_ima601  #CHI-C50068 #MOD-D60220
      ...
      #IF l_ima721 IS NULL THEN LET l_ima721=0 END IF  #CHI-C50068 #MOD-D60220
      IF STATUS THEN
...
FUNCTION p500_mss045_ins(p_mss00,p_opendate,p_needdate,p_qty,p_id)
  ...
  #DEFINE l_cnt          LIKE type_file.num5  #MOD-D20082  #MOD-D60220
                                             ## 解決虛擬料件回溯追蹤
...
FUNCTION p500_plan_sub()
  ...
  FOREACH p500_plan_sub_c INTO g_mss01,g_mss03
     ...
     #   ORDER BY 1 DESC, 2, 5, 3
     #FOREACH p500_plan_sub_c2 INTO g_mst03,g_mst04,g_mst06,g_mst061,
     #CHI-C80002---end
...
#######################################      
       ...
       FOREACH p500_plan_sub_c3 USING g_mss01,g_mst07 INTO bmd.*  #CHI-C80002
        ...
        #------------ No:MOD-B30174  end
         ...
         #UPDATE sub_tmp SET sub_qty=sub_qty+g_sub_qty #CHI-C80002
         # WHERE sub_partno=g_mss01 AND sub_wo_no=g_mst06 AND sub_prodno=g_mst07 #CHI-C80002
