...
# Modify.........: No.FUN-CB0003 12/11/02 By chenjing 修改FOREACH報錯問題
...
#No.FUN-750097 -----------start-----------------                                                                                    
    ...
    LET l_table = cl_prt_temptable('amrr532',g_sql) CLIPPED   # 產生Temp Table                                                      
...
#No.FUN-750097---------------end------------
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amrr532_tm(0,0)              # Input print condition
      ELSE CALL amrr532()                    # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amrr532_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01           #No.FUN-580031
   ...
   OPEN WINDOW amrr532_w AT p_row,p_col
        WITH FORM "amr/42f/amrr532" 
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amrr532_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amrr532',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amrr532_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#No.FUN-750097-----------------end----------------
     ...
     PREPARE amrr532_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amrr532_curs1 CURSOR FOR amrr532_prepare1
 
#    CALL cl_outnam('amrr532') RETURNING l_name        #No.FUN-750097
#    START REPORT amrr532_rep TO l_name                #No.FUN-750097
     LET g_pageno = 0
     FOREACH amrr532_curs1 INTO mss.*,ima.*
	   ...
	   #     IF sss[l].s1 > 0 THEN  #如果有可延遲的采購單
           ...
           #   IF l_leave<=0 THEN 
           #     #No.FUN-750097-------start-----------------
           #     #OUTPUT TO REPORT amrr532_rep(sr.*,sss[l].*,ima.*,mst.*)  
...
#    CALL cl_prt(l_name,g_prtway,g_copies,g_len)
     ...
     END IF                                                                                                                         
     LET g_str = g_str                                        
     CALL cl_prt_cs3('amrr532','amrr532',l_sql,g_str)                                                                                 
#No.FUN-750097-------------end--------------------
END FUNCTION
 
#No.FUN-750097-------------end--------------------
{REPORT amrr532_rep(sr,mss, ima,mst)
