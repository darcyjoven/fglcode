...
#No.FUN-750093 --start--
   ...
   LET l_table = cl_prt_temptable('amrr580',g_sql) CLIPPED
...
#No.FUN-750093 --end--
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amrr580_tm(0,0)        # Input print condition
      ELSE CALL amrr580()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amrr580_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01    #No.FUN-580031
   ...
   OPEN WINDOW amrr580_w AT p_row,p_col
        WITH FORM "amr/42f/amrr580" 
################################################################################
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amrr580_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amrr580',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amrr580_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#No.FUN-750093 --end--
    ...
    #----------------No.MOD-8B0056 end
     ...
     PREPARE amrr580_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amrr580_curs1 CURSOR FOR amrr580_prepare1
 
#     CALL cl_outnam('amrr580') RETURNING l_name  #No.FUN-750093
#     START REPORT amrr580_rep TO l_name          #No.FUN-750093
#     LET g_pageno = 0                            #No.FUN-750093
     FOREACH amrr580_curs1 INTO sr.*            
...
#          OUTPUT TO REPORT amrr580_rep(sr.*)       #No.FUN-750093
           EXECUTE insert_prep USING sr.mss01,l_ima02,l_ima021,l_ima25,
                                     sr.mss09,sr.pml20,l_diff 
       END IF
     END FOREACH
 
#     FINISH REPORT amrr580_rep                     #No.FUN-750093
#
...
#No.FUN-750093 --start--
     ...
     CALL cl_wcchp(tm.wc,'mss01')
          RETURNING tm.wc
     LET g_str = tm.ver_no,";",tm.bdate,";",tm.edate,";",tm.wc
     LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED
     CALL cl_prt_cs3('amrr580','amrr580',l_sql,g_str)
#No.FUN-750093 --end--
END FUNCTION
#No.FUN-750093 --start--mark
{REPORT amrr580_rep(sr)
