...
MAIN
   ...
   LET l_table = cl_prt_temptable('amrr522',g_sql) CLIPPED
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amrr522_tm(0,0)        # Input print condition
      ELSE CALL amrr522()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amrr522_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01      #No.FUN-580031
   ...
   OPEN WINDOW amrr522_w AT p_row,p_col
        WITH FORM "amr/42f/amrr522" 
################################################################################
...
#No.FUN-570240 --end  
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amrr522_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amrr522',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amrr522_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0076
  ...
  SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'amrr522'
  ...
  #No.FUN-850143-----end
     ...
     LET l_sql = "SELECT *",
                 ...
                 " AND mss_v='",tm.ver_no,"'",
                 " AND ",tm.wc clipped
     PREPARE amrr522_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amrr522_curs1 CURSOR FOR amrr522_prepare1
 
     #CALL cl_outnam('amrr522') RETURNING l_name    #No.FUN-850143
     #START REPORT amrr522_rep TO l_name     #No.FUN-850143
     LET g_pageno = 0
     FOREACH amrr522_curs1 INTO mss.*, ima.*
       ...
       EXECUTE insert_prep USING
           ...
           mss.mss11,mss.mss01,l_ima02,ima.ima25,l_mss02,l_pmc03,          #TQC-9C0179 mark
           mss.mss00,mss.mss03,mss.mss09,mss.mss10,ima.ima08,ima.ima67,ima.ima43   
       #OUTPUT TO REPORT amrr522_rep(sr.*,mss.*, ima.*)
       #No.FUN-850143---------end
     END FOREACH
     ...
     LET g_str = tm.wc,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",
                 tm.t[1,1],";",tm.t[2,2],";",tm.t[3,3],";",tm.n,";",tm.ver_no
     
     CALL cl_prt_cs3('amrr522','amrr522',g_sql,g_str)
     #FINISH REPORT amrr522_rep
