...
#No.FUN-830153---End    
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amrx520_tm(0,0)        # Input print condition
      ELSE CALL amrx520()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amrx520_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01      #No.FUN-580031
   ...
   OPEN WINDOW amrx520_w AT p_row,p_col
        WITH FORM "amr/42f/amrx520" 
################################################################################
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amrx520_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amrx520',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amrx520_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0076
   ...
   DEFINE l_str       STRING   #FUN-D30044
   DEFINE l_str1      STRING   #FUN-D30044
...
#No.FUN-830153---End          
     ...
     PREPARE amrx520_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amrx520_curs1 CURSOR FOR amrx520_prepare1
 
#     CALL cl_outnam('amrx520') RETURNING l_name    #No.FUN-830153 
#     START REPORT amrx520_rep TO l_name            #No.FUN-830153 
     LET g_pageno = 0
     FOREACH amrx520_curs1 INTO mss.*, ima.*
         ...
         IF l_cnt > 0 THEN                                                                                                          
           ...
           SELECT pmh02,pmc03,pmh13,pmh12 FROM pmh_file,pmc_file                                                                    
            WHERE pmh02=pmc01 AND pmhacti='Y' AND pmh01= mss.mss01                                                                  
              ...
              AND pmh23 = ' '                                                            #CHI-960033
              AND pmhacti = 'Y'                                                        #CHI-910021                                                                    
           FOREACH x520_p INTO l_pmh02,l_pmc031,l_pmh13,l_pmh12                                                                     
              IF STATUS THEN                                                                                                        
                 EXIT FOREACH                                                                                                       
              END IF                                                                                                                
...
#       OUTPUT TO REPORT amrx520_rep(sr.*,mss.*, ima.*)
#No.FUN-830153---End  
     END FOREACH
#No.FUN-830153---Begin 
#     FINISH REPORT amrx520_rep
###XtraGrid###   LET g_str=tm.s[1,1],";", tm.s[2,2],";",                                                                                          
###XtraGrid###             tm.s[3,3],";",tm.t[1,1],";",tm.t[2,2],";",tm.t[3,3],";",tm.p,";",                                                      
###XtraGrid###             tm.ver_no,";",tm.n                                                                                                     
...
###XtraGrid###   CALL cl_prt_cs3('amrx520','amrx520',l_sql,g_str) 
    LET g_xgrid.table = l_table,"|",l_table1   #FUN-D30044 
    #FUN-D30044----add---str--
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"mss01,ima08,ima67,ima43,mss02,mss03,mss11")
    LET g_xgrid.grup_field = cl_get_order_field(tm.s,"mss01,ima08,ima67,ima43,mss02,mss03,mss11")  #FUN-D30053 mark
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"mss01,ima08,ima67,ima43,mss02,mss03,mss11")
   #FUN-D40129---remark---str--
    LET g_xgrid.headerinfo1 = cl_getmsg("azz-211",g_lang),":",tm.ver_no
    CASE tm.n
       WHEN '1'  LET l_str1 = cl_getmsg("amr-070",g_lang)
       WHEN '2'  LET l_str1 = cl_getmsg("amr-071",g_lang)
       WHEN '3'  LET l_str1 = cl_getmsg("amr-072",g_lang)
       OTHERWISE LET l_str1 = " "
    END CASE
    LET g_xgrid.footerinfo1 = cl_getmsg("amr-920",g_lang),":",l_str1,'|',cl_getmsg("amr-921",g_lang)
   #FUN-D40129---remark---end--
   #LET l_str = cl_wcchp(g_xgrid.order_field,"mss01,ima08,ima67,ima43,mss02,mss03,mss11")
   #LET l_str = cl_replace_str(l_str,',','-')
   #LET g_xgrid.footerinfo2 = cl_getmsg("lib-626",g_lang),l_str
    #FUN-D30044----add---end--
