...
# Modify.........: No.FUN-D40128 13/05/08 By wangrr "來源碼""採購員""主分群碼""計劃員""倉管員"增加開窗
...
MAIN
   ...
   LET l_table = cl_prt_temptable('amrx810',g_sql) CLIPPED  # 產生Temp Table
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amrx810_tm(0,0)        # Input print condition
      ELSE CALL amrx810()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amrx810_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW amrx810_w AT p_row,p_col
        WITH FORM "amr/42f/amrx810"
################################################################################
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amrx810_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amrx810',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amrx810_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0076
          ...
          sr  RECORD
              ...
              ima021 LIKE ima_file.ima021,  #FUN-D40128
    ...
    DEFINE l_flag   LIKE type_file.chr1  #FUN-D40128
...
#    CALL cl_outnam('amrx810') RETURNING l_name
#    START REPORT amrx810_rep TO l_name
#No.MOD-720041 By TSD.Jin--end
    
    #FUN-D40128--add--str--
    IF tm.order='1' OR tm.order='2' OR tm.order='3' THEN
       LET l_flag='Y'
    ELSE
       LET l_flag='N'
    END IF
    #FUN-D40128--add--end
     ...
     PREPARE amrx810_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amrx810_curs1 CURSOR FOR amrx810_prepare1
     FOREACH amrx810_curs1 INTO sr.ima01
       ...
       IF sr.qty0=0 AND sr.qty1=0 AND sr.qty2=0 AND sr.qty3=0 AND
          ...
          CONTINUE FOREACH
       END IF
       SELECT ima01,ima02,ima021,ima06,ima08,ima23,ima25,ima43,ima67  #FUN-D40128 add ima021
         INTO sr.ima01,sr.ima02,sr.ima021,sr.ima06,sr.ima08,sr.ima23,sr.ima25,sr.ima43,sr.ima67 #FUN-D40128 add sr.ima021
       ...
       END CASE
     #No.MOD-720041 By TSD.Jin--start
     # OUTPUT TO REPORT amrx810_rep(sr.*)
       ...
       END IF
       IF tm.order='2' THEN
          SELECT gen02 INTO l_gen02 FROM gen_file WHERE gen01=sr.ima67
       END IF
       IF tm.order='3' THEN
          SELECT gen02 INTO l_gen02 FROM gen_file WHERE gen01=sr.ima43
       END IF
       #FUN-D40128--add--end

       EXECUTE insert_prep USING 
          sr.order, sr.ima01, sr.ima02, sr.ima021,sr.ima06, sr.ima08, #FUN-D40128 add sr.ima021
          sr.ima23, l_gen02,  sr.ima25, sr.ima43, sr.ima67, sr.qty0,  #FUN-D40128 add l_gen02
...
###XtraGrid###                 YEAR(g_date0) USING '##&&','/',MONTH(g_date0) USING '&&',";",
###XtraGrid###   YEAR(g_date1) USING '##&&','/',MONTH(g_date1) USING '&&',";",
###XtraGrid###   YEAR(g_date2) USING '##&&','/',MONTH(g_date2) USING '&&',";",
###XtraGrid###   YEAR(g_date3) USING '##&&','/',MONTH(g_date3) USING '&&',";",
###XtraGrid###   YEAR(g_date4) USING '##&&','/',MONTH(g_date4) USING '&&',";",
###XtraGrid###   YEAR(g_date5) USING '##&&','/',MONTH(g_date5) USING '&&',";" 

     #FUN-CB0002--add--str--
     LET l_msg1 = YEAR(g_date0) USING '##&&','/',MONTH(g_date0) USING '&&'
     LET l_msg2 = YEAR(g_date1) USING '##&&','/',MONTH(g_date1) USING '&&'
     LET l_msg3 = YEAR(g_date2) USING '##&&','/',MONTH(g_date2) USING '&&'
     LET l_msg4 = YEAR(g_date3) USING '##&&','/',MONTH(g_date3) USING '&&'
     LET l_msg5 = YEAR(g_date4) USING '##&&','/',MONTH(g_date4) USING '&&'
     LET l_msg6 = YEAR(g_date5) USING '##&&','/',MONTH(g_date5) USING '&&'
     #FUN-CB0002--add--end--
     LET g_xgrid.visible_column = cl_xg_column_visible(g_xgrid.visible_column,"gen02",l_flag)  #FUN-D40128
     IF tm.order = '5' THEN
###XtraGrid###        CALL cl_prt_cs3('amrx810','amrx810_2',l_sql,g_str)   #FUN-710080 modify
        LET g_xgrid.table = l_table    ###XtraGrid###
        LET g_xgrid.template = 'amrx810_2'
       #LET g_xgrid.order_field ="ima06,ima08,ima67,ima43,ima23"  #FUN-D40128 mark
        LET g_xgrid.order_field ="order1"  #FUN-D40128
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'qty0',l_msg1,'')
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'amt0',l_msg1,'')
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'qty1',l_msg2,'')
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'amt1',l_msg2,'')
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'qty2',l_msg3,'')
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'amt2',l_msg3,'')
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'qty3',l_msg4,'')
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'amt3',l_msg4,'')
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'qty4',l_msg5,'')
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'amt4',l_msg5,'')
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'qty5',l_msg6,'')
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'amt5',l_msg6,'')
        IF g_zz05 = 'Y' THEN
           CALL cl_wcchp(tm.wc,'mss01,ima06,ima08,ima67,ima43,ima23')
               RETURNING tm.wc
        END IF
     ...
     ELSE
###XtraGrid###        CALL cl_prt_cs3('amrx810','amrx810_1',l_sql,g_str)   #FUN-710080 modify
        LET g_xgrid.table = l_table    ###XtraGrid###
        LET g_xgrid.template = 'amrx810_1'
       #LET g_xgrid.order_field ="ima06,ima08,ima67,ima43,ima23"  #FUN-D40128 mark
        LET g_xgrid.order_field ="order1"  #FUN-D40128 
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'qty0',l_msg1,'')
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'amt0',l_msg1,'')
        ...
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'amt5',l_msg6,'')
        IF tm.order = '1' THEN 
           LET l_msg7 = cl_getmsg("amr-266",g_lang)
        END IF 
        IF tm.order = '2' THEN 
           LET l_msg7 = cl_getmsg("ima-003",g_lang)
        END IF 
        IF tm.order = '3' THEN 
           LET l_msg7 = cl_getmsg("ima-004",g_lang)
        END IF 
        IF tm.order = '4' THEN 
           LET l_msg7 = cl_getmsg("amr-267",g_lang)
        END IF 
        LET g_xgrid.dynamic_title = cl_xg_dynamic_title(g_xgrid.dynamic_title,'order1',l_msg7,'')
        IF g_zz05 = 'Y' THEN
           CALL cl_wcchp(tm.wc,'mss01,ima06,ima08,ima67,ima43,ima23')
               RETURNING tm.wc
        END IF
