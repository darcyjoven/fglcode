...
# Modify.........: No.FUN-D40129 13/05/06 By yangtt ima08欄位新增開窗
...
#No.FUN-850048--begin--
   LET g_sql="mss01.mss_file.mss01,",
        ...
        #FUN-CB0003--str---
             ...
             "k_amt.mss_file.mss01,",
             "j_amt.mss_file.mss071,",
             "l_num.type_file.num5"
        #FUN-CB0003--end---
     LET l_table = cl_prt_temptable('amrx500',g_sql) CLIPPED                                                                        
     IF l_table= -1 THEN EXIT PROGRAM END IF                                                                                        
     LET g_sql="INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,                                                                            
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,? ,?,?,?,?,?)"   #FUN-BC0003 add 5?                                                                                         
#               "        ?,?,?,?,?, ?,?,?,?,?,",                                                                                         
#               "        ?,?,?,?,? )"                                                                                                                
...
#TQC-610074-begin
  ...
  #TQC-610074-end
   IF cl_null(g_bgjob) OR g_bgjob='N'        # If background job sw is off
      THEN CALL amrx500_tm(0,0)        # Input print condition
      ELSE CALL amrx500()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amrx500_tm(p_row,p_col)
DEFINE lc_qbe_sn         LIKE gbm_file.gbm01     #No.FUN-580031
   ...
   OPEN WINDOW amrx500_w AT p_row,p_col
        WITH FORM "amr/42f/amrx500" 
################################################################################
...
LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW amrx500_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
         
   END IF
 # INPUT BY NAME tm.edate,tm.ver_no,tm.n,      #FUN-CB0003
   INPUT BY NAME tm.edate,tm.ver_no,           #FUN-CB0003
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('amrx500',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW amrx500_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0076
   ...
   #FUN-CB0003--add--str--
   DEFINE k_amt   LIKE mss_file.mss01,
          j_amt   LIKE mss_file.mss071,
   ...
   #FUN-CB0003--add--end--
     ...
     LET k_amt = 0    #FUN-CB0003
     LET l_sql = "SELECT '','','',mss_file.*,ima02,ima25,ima08,ima67,ima43,ima021",   #FUN-CB0003 add ima021
    ...
    IF tm.edate IS NOT NULL THEN
       LET l_sql=l_sql CLIPPED,
                 "   AND mss03<='",tm.edate,"'"
    END IF
    PREPARE amrx500_prepare1 FROM l_sql
    IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
       EXIT PROGRAM 
    END IF
    DECLARE amrx500_curs1 CURSOR FOR amrx500_prepare1
#No.FUN-850048--begin--
#    CALL cl_outnam('amrx500') RETURNING l_name
#    START REPORT amrx500_rep TO l_name
#    LET g_pageno = 0
    FOREACH amrx500_curs1 INTO sr.*,mss.*, l_ima02,l_ima25,l_ima08,l_ima67,l_ima43,l_ima021     #FUN-CB0003 add l_ima021
#     FOR g_cnt = 1 TO 3 
#       CASE 
...
#       LET sr.l_order3 = l_order[3] 
     ...
     #No.FUN-920183 start -----
        ...
        END IF
     #No.FUN-920183 end -------
#  OUTPUT TO REPORT amrx500_rep(sr.*,mss.*, l_ima02,l_ima25,l_ima08,l_ima67,l_ima43)
 #FUN-CB0003--add--str---
   SELECT pmc03 INTO l_pmc03 FROM pmc_file WHERE pmc01 = mss.mss02
   LET l_num = 0
   LET j_amt = mss.mss072-mss.mss071
#FUN-CB0003--mark-
#  IF  tm.n ='2' THEN 
#      LET k_amt = k_amt+(mss.mss051+mss.mss052+mss.mss053
...
#  END IF
 #FUN-CB0003--add--end---
     EXECUTE insert_prep USING mss.mss01,l_ima08,l_ima67,l_ima43,mss.mss02,l_ima25,mss.mss00,mss.mss03,
                               ...
                               mss.mss09,g_msg,l_ima02,l_ima021,l_pmc03,k_amt,j_amt,l_num   #FUN-CB0003 add l_ima021,l_pmc03,l_num,k_amt,j_amt
     END FOREACH
 
#     FINISH REPORT amrx500_rep
     IF g_zz05 = 'Y' THEN                                                                                                           
       CALL cl_wcchp(tm.wc,'mss01,ima67,mss02,ima08,ima43') 
        RETURNING tm.wc                                                                                                             
        LET g_str = tm.wc                                                                                                           
     END IF                                                                                                                         
###XtraGrid###     LET g_str=g_str,";",tm.s[1,1],";",tm.s[2,2],";",tm.s[3,3],";",tm.t[1,1],";",tm.t[2,2],";",
###XtraGrid###               tm.t[3,3],";",tm.n,";",tm.edate,";",tm.ver_no                                 
###XtraGrid###     LET g_sql = "SELECT * FROM ",g_cr_db_str CLIPPED,l_table CLIPPED                                                               
###XtraGrid###     CALL cl_prt_cs3('amrx500','amrx500',g_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
#FUN-CB0003--add--sta---
   #LET g_xgrid.order_field = cl_get_order_field(tm.s,"mss01,ima67,mss02,ima08,ima43")  #FUN-D30053 mark
    LET g_xgrid.order_field = cl_get_order_field(tm.s,"mss01,ima08,ima67,ima43,mss02")  #FUN-D30053
    LET g_xgrid.grup_field = "mss01"
   #LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"mss01,ima67,mss02,ima08,ima43") #FUN-D30053 mark
    LET g_xgrid.skippage_field = cl_get_skip_field(tm.s,tm.t,"mss01,ima08,ima67,ima43,mss02") #FUN-D30053
    LET g_xgrid.footerinfo1 = cl_getmsg("lib-076",g_lang),tm.edate
    LET g_xgrid.footerinfo2 = cl_getmsg("lib-077",g_lang),tm.ver_no
   #LET l_str = cl_wcchp(g_xgrid.order_field,"mss01,ima67,mss02,ima08,ima43")  #FUN-D30053 mark
   #LET l_str = cl_wcchp(g_xgrid.order_field,"mss01,ima08,ima67,ima43,mss02")  #FUN-D30053  #FUN-D30070  mark
   #LET l_str = cl_replace_str(l_str,',','-')  #FUN-D30070  mark
   #LET g_xgrid.footerinfo4 = cl_getmsg("lib-626",g_lang),l_str  #FUN-D30070  mark
...
END FUNCTION
#No.FUN-850048--begin--
#REPORT amrx500_rep(sr,mss, l_ima02,l_ima25,l_ima08,l_ima67,l_ima43)
...
#        #CALL x500_seq(mss.mss01,mss.mss02,mss.mss03)
