...
# Modify.........: No:CHI-C50068 12/11/05 By bart 將amrx514與amrr532合併成一個報表
# Modify.........: No.FUN-CB0002 12/11/02 By lujh CR轉XtraGrid
# Modify.........: No.FUN-D40020 13/01/30 By chenjing 修改開窗全選報錯問題
...
MAIN
   ...
   LET g_sql = "ima43.ima_file.ima43,", 
               ...
               #FUN-CB0002--add--str--
               "diff.mss_file.mss072,",
               "l_reqqty.mss_file.mss041,",
               "l_othqty.mss_file.mss051,",
               "l_diff_desc.type_file.chr20,",
               "l_count.type_file.num10,",
               "ima021.ima_file.ima021"
               #FUN-CB0002--add--str-- 
   LET l_table = cl_prt_temptable('amrx514',g_sql) CLIPPED
   IF l_table =-1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,?,",
               "        ?,?,?,?,?,?,?,?,?,?,?,?,?)"	  #CHI-C50068  #FUN-CB0002  add  ?,?,?,?,?,?
   ...
   IF cl_null(g_bgjob) OR g_bgjob='N'    # If background job sw is off
      THEN CALL amrx514_tm(0,0)          # Input print condition
      ELSE CALL amrx514()                # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
END MAIN
 
FUNCTION amrx514_tm(p_row,p_col)
...
IF p_row = 0 THEN LET p_row = 4 LET p_col = 13 END IF
   ...
   OPEN WINDOW amrx514_w AT p_row,p_col WITH FORM "amr/42f/amrx514"
...
END CONSTRUCT
     ...
     IF INT_FLAG THEN
        LET INT_FLAG = 0 CLOSE WINDOW amrx514_w 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM
     ...
     IF g_bgjob = 'Y' THEN
        ...
        ELSE
           ...
           CALL cl_cmdat('amrx514',g_time,l_cmd)    # Execute cmd at later time
        END IF
        CLOSE WINDOW amrx514_w
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0076
    #     l_sql     LIKE type_file.chr1000,     # RDSQL STATEMENT               #NO.FUN-680082 VARCHAR(1000)  #FUN-D40020
          l_sql     STRING,                     #FUN-D40020 
          ...
          ima   RECORD      
                  ...
                  ima021 LIKE ima_file.ima021,  #FUN-CB0002 add
  ...
  #FUN-CB0002--add--str--  
  DEFINE l_diff   LIKE mss_file.mss072
  DEFINE l_reqqty LIKE mss_file.mss041
  DEFINE l_othqty LIKE mss_file.mss051
  DEFINE l_diff_desc LIKE type_file.chr20
  DEFINE l_count  LIKE type_file.num10
  #FUN-CB0002--add--end-- 

  DROP TABLE x514_tmp
  CREATE TEMP TABLE x514_tmp(
  ...
  SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'amrx514' 
  #No.FUN-850143------end
     ...
     PREPARE amrx514_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE amrx514_curs1 CURSOR FOR amrx514_prepare1
     ...
     PREPARE x514_premst  FROM l_sql
     IF STATUS THEN CALL cl_err('prepare2:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690116
        EXIT PROGRAM 
     END IF
     DECLARE x514_curmst CURSOR FOR x514_premst
     LET l_count = 3   #FUN-CB0002  add
 
     #CALL cl_outnam('amrx514') RETURNING l_name     #No.FUN-850143
  
     #START REPORT amrx514_rep TO l_name    #No.FUN-850143
     LET g_pageno = 0 LET mss_o.mss01 = ' '
     #-----MOD-A10180-----
     #FOREACH amrx514_curs1 INTO mss.*, ima.* 
     ...
     #           DECLARE x514_mstmin CURSOR FOR mstsql
     #           FOREACH x514_mstmin INTO mst.*
     ...
     #              #----------No:MOD-890230 end
     #               mss_o.mss02   
     #           #OUTPUT TO REPORT amrx514_rep(mss_o.*,ima_o.*,mst.*)
     #           #No.FUN-850143-------end
     ...
     #
     #        IF l_move >0 THEN 
     #           FOREACH x514_curmst USING mss.mss01,mss.mss02,mss.mss03,mss.mss_v
     #           INTO mst.*
     #           IF l_move<=0 THEN CONTINUE FOREACH END IF 
     #           #No.FUN-850143-------start--
     #           SELECT gen02 INTO l_gen02 FROM gen_file WHERE gen01 = ima_o.ima43     #MOD-A10162 ima.ima43-->ima_o.ima43
     ...
     #END FOREACH
     LET i = 1 
     FOREACH amrx514_curs1 INTO sss[i].* 
        LET i = i + 1
     END FOREACH
         ...
         IF l_bal > 0 THEN 
			...
			#CHI-C50068---end
                ...
                ELSE
                   ...
                   FOREACH x514_curs1 INTO l_mst08,l_mst05,l_mst06,l_mst061,l_mst04                                      
                      #CHI-C50068---begin
                      ...
                      #CHI-C50068---end                                    
                      LET l_mst081= 0 
                      SELECT SUM(mst08) INTO l_mst081 FROM x514_tmp
                        WHERE mst_v = sss[k].mss_v
                          AND mst01 = sss[k].mss01
                          AND mst02 = sss[k].mss02
                          AND mst03 = sss[k].mss03
                      ...
                      IF l_mst05 ='61' THEN                                                    
                         ...
                         IF SQLCA.sqlcode THEN LET l_vender = ' ' LET l_pmc03 = ' ' END IF       
                      END IF                                                                     
                      SELECT ima02,ima021,ima08,ima25,ima43 INTO ima.* FROM ima_file      #FUN-CB0002 add  ima021
                      WHERE ima01 = sss[j].mss01

                    CASE WHEN tm.type = '1'  #CHI-C50068                                              
                      IF l_bal <= l_mst08 THEN
                         #FUN-CB0002--add--str--
                         LET l_diff = sss[j].mss072 - sss[j].mss071
                         LET l_reqqty = sss[j].mss041 + sss[j].mss043 + sss[j].mss044
                         LET l_othqty = sss[j].mss051+sss[j].mss052+sss[j].mss053+sss[j].mss063+
                                        sss[j].mss064+sss[j].mss065+sss[j].mss09
                         IF l_difference = '1' THEN 
                            LET l_diff_desc = cl_getmsg('amr-081',g_lang)
                         ELSE
                            LET l_diff_desc = cl_getmsg('amr-082',g_lang)
                         END IF 
                         #FUN-CB0002--add--end--
                         EXECUTE insert_prep USING
                            ...
                            sss[j].mss02,l_diff,l_reqqty,l_othqty,l_diff_desc,l_count,ima.ima021          #FUN-CB0002--add    l_diff,l_reqqty,l_othqty,l_diff_desc,l_count,ima.ima021                
                         INSERT INTO x514_tmp VALUES(sss[k].mss_v,sss[k].mss01,sss[k].mss02,
                  ...
                  #------------------------------No:MOD-A40096 add
                   ...
                   IF l_bal > 0 THEN
                      ...
                      FOREACH x514_curs2 INTO l_mst08,l_mst05,l_mst06,l_mst061,l_mst04
...
###XtraGrid###     LET g_str = tm.wc,";",tm.ver_no,";",tm.sdate,";",tm.edate  #CHI-C50068
     
###XtraGrid###     CALL cl_prt_cs3('amrx514','amrx514',g_sql,g_str)
    LET g_xgrid.table = l_table    ###XtraGrid###
    LET g_xgrid.order_field = "mss01,mss02,ima43"
    IF g_zz05 = 'Y' THEN
        CALL cl_wcchp(tm.wc,'mss01,ima67,mss02,ima08,ima43')
        RETURNING tm.wc
    END IF
...
#No.FUN-850143------start--
#REPORT amrx514_rep(mss,ima,mst)
