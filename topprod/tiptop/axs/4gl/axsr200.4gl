...
#
# Pattern name...: axsr200.4gl
# Descriptions...: 兩期銷售分析表
...
# Modify.........: No.MOD-580016 05/08/04 By kevin QBE+開窗
...
# Modify.........: No.MOD-840675 08/07/09 By Pengu 調整CONSTRUCT語法
...
# Modify.........: No.TQC-A50147 10/05/25 By Carrier main中结构调整 & MOD-A10015/MOD-980231追单
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD                               # Print condition RECORD
             ...
             #wc2     LIKE type_file.chr1000,      # Where condition   #No.FUN-680130 VARCHAR(1000) #MOD-D30276 mark
              ...
              END RECORD,
          sr  RECORD order1 LIKE oab_file.oab02,   #No.FUN-680130 VARCHAR(40)  #FUN-5B0105 20->40
                     order2 LIKE oab_file.oab02,   #No.FUN-680130 VARCHAR(40)  #FUN-5B0105 20->40
                     order3 LIKE oab_file.oab02,   #No.FUN-680130 VARCHAR(40)  #FUN-5B0105 20->40
                     order4 LIKE oab_file.oab02,   #No.FUN-680130 VARCHAR(40)  #FUN-5B0105 20->40
              ...
              END RECORD,
          g_order1,g_order2 LIKE oab_file.oab02, #No.FUN-680130 VARCHAR(40) #FUN-5B0105 20->40
          g_order3,g_order4 LIKE oab_file.oab02, #No.FUN-680130 VARCHAR(40) #FUN-5B0105 20->40
          g_order           LIKE oab_file.oab02, #No.FUN-680130 VARCHAR(60) 
          ...
          g_str5,g_str6     LIKE zaa_file.zaa08,   #No.FUN-680130 VARCHAR(08) 
...
DEFINE   g_ima09         LIKE ima_file.ima09,
         ...
         g_azf03_1       LIKE azf_file.azf03,
         g_azf03_2       LIKE azf_file.azf03,
         g_azf03_3       LIKE azf_file.azf03
#FUN-B90090----------add-------end         
...
MAIN
   ...
   LET tm.op  = ARG_VAL(22)
   LET tm.ch  = ARG_VAL(23)
   LET tm.w   = ARG_VAL(24)
   LET tm.z   = ARG_VAL(25)
...
#FUN-B90090----------add-------end                 
    ...
    LET l_table = cl_prt_temptable('axsr200',g_sql) CLIPPED   # 產生Temp Table
...
#----------------------------------------------------------CR (1) ------------#
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'        # If background job sw is off
      THEN CALL axsr200_tm(0,0)        # Input print condition
      ELSE CALL axsr200()            # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80059    ADD
END MAIN
 
FUNCTION axsr200_tm(p_row,p_col)
   ...
   OPEN WINDOW axsr200_w AT p_row,p_col
        WITH FORM "axs/42f/axsr200"
...
WHILE TRUE
   CONSTRUCT BY NAME tm.wc ON azp01
     ON ACTION controlp
         ...
         DISPLAY g_qryparam.multiret TO FORMONLY.azp01
  ...
  LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond(null, null) #FUN-980030
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW axsr200_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80059    ADD
      EXIT PROGRAM
   END IF
   IF tm.wc = ' 1=1' THEN
   ...
   END IF
   CONSTRUCT tm.wc2 ON oea15,oea14,oea25,oea26,oea03,oca_file.oca01,oea04,oea23,
                               ima_file.ima131,ima_file.ima06,oeb04,occ20,occ21,occ22
                               ,ima_file.ima09,ima_file.ima10,ima_file.ima11 #FUN-B90090 add
             FROM oea15,oea14,oea25,oea26,oea03,oca01,oea04,oea23,ima131,
...
#FUN-B90090----------add-------end
   ...
   INPUT BY NAME
                 ...
                 tm.y2,tm.m2b,tm.m2e,tm.op,tm.ch,tm.w,tm.z,tm.more
                 WITHOUT DEFAULTS
      AFTER FIELD a1
      ...
      AFTER FIELD b1
         IF cl_null(tm.b1) OR tm.b1 NOT MATCHES '[1-4]' THEN
            NEXT FIELD b1
         END IF
         CASE WHEN tm.b1='1' LET g_str5=g_x[15]
              WHEN tm.b1='2' LET g_str5=g_x[16]
              WHEN tm.b1='3' LET g_str5=g_x[17]
              WHEN tm.b1='4' LET g_str5=g_x[18]
         END CASE
         DISPLAY g_str5 TO FORMONLY.c1
      AFTER FIELD y1
      ...
      AFTER FIELD b2
         IF cl_null(tm.b2) OR tm.b2 NOT MATCHES '[1-4]' THEN
            NEXT FIELD b2
         END IF
         CASE WHEN tm.b2='1' LET g_str6=g_x[15]
              WHEN tm.b2='2' LET g_str6=g_x[16]
              WHEN tm.b2='3' LET g_str6=g_x[17]
              WHEN tm.b2='4' LET g_str6=g_x[18]
         END CASE
         DISPLAY g_str6 TO FORMONLY.c2
      AFTER FIELD y2
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('axsr200',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW axsr200_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time  #FUN-B80059    ADD
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#FUNCTION r200_g_sql(p_azp03,p_b,p_y,p_mb,p_me,p_bdate,p_edate)  #FUN-A10098
FUNCTION r200_g_sql(p_azp01,p_b,p_y,p_mb,p_me,p_bdate,p_edate)  #FUN-A10098
...
#FUN-A10098---end
   ...
   IF p_b MATCHES "[24]" THEN #-->訂單/出貨目標(osa_file)
      ...
      CALL r200_wc2('2') RETURNING l_wc
      LET g_sql = "SELECT '','','','',",
...
#                  " OUTER ",s_dbstring(p_azp03 CLIPPED),"occ_file,",   
#                  " OUTER ",s_dbstring(p_azp03 CLIPPED),"oca_file,",   
#                  " OUTER ",s_dbstring(p_azp03 CLIPPED),"gem_file,",    
#                  " OUTER ",s_dbstring(p_azp03 CLIPPED),"ima_file,",   
#                  " OUTER ",s_dbstring(p_azp03 CLIPPED),"gen_file,", 
...
#FUN-A10098---END
           ...
           #       "   AND occ_file.occ01 = osa_file.osa03 ",
           #       "   AND oca_file.oca01 = occ_file.occ03 ",
   ...
   IF p_b='3' THEN #-->出貨實際(oga_file)
      CALL r200_wc2('3') RETURNING l_wc
      LET g_sql = "SELECT '','','','',",
...
#                  " OUTER ",s_dbstring(p_azp03 CLIPPED),"ima_file,",  
#                  " OUTER ",s_dbstring(p_azp03 CLIPPED),"oab_file ",   
                  "  FROM ",cl_get_target_table(p_azp01,'ogb_file'),
                  " LEFT OUTER JOIN ",cl_get_target_table(p_azp01,'ima_file')," ON(ima01 = ogb04),",
                            cl_get_target_table(p_azp01,'occ_file'),
                  " LEFT OUTER JOIN ",cl_get_target_table(p_azp01,'oca_file')," ON(oca01 = occ03),",
                            cl_get_target_table(p_azp01,'oga_file'),
...
#FUN-A10098---END
                  " WHERE ",l_wc CLIPPED,
                  "   AND oga09 IN ('2','3','4','6','8','A')", #NO:7459  #No.FUN-610020  #MOD-780043 add '6' #CHI-A60005 add 'A'
      ...
      IF tm.z='Y' THEN LET g_sql = g_sql CLIPPED," AND ogapost='Y' " END IF
      IF tm.w='Y' THEN
...
# ------------------------------------------------------ CR (2) ------- ##
     ...
     FOREACH azp_cur INTO l_azp01,l_azp03
         ...
         #---------------------------- 第一期 ---------------------------
#         CALL r200_g_sql(l_azp03,tm.b1,tm.y1,tm.m1b,tm.m1e,b_date1,e_date1) #FUN-A10098
         CALL r200_g_sql(l_azp01,tm.b1,tm.y1,tm.m1b,tm.m1e,b_date1,e_date1)  #FUN-A10098
         FOR i=1 TO 2
          IF tm.b1 !='3' AND i=2 THEN EXIT FOR END IF
          IF tm.b1 ='3' AND i=2  THEN
             LET l_buf=l_buf_tmp
             LET l_str1 = "   AND oga09 IN ('2','3','4','6','8','A')   AND (oga65 != 'Y' OR oga09 = '8')" #CHI-A60005 add 'A'
             LET l_str2 = "   AND 1=1 "
             CALL cl_replace_str(l_buf,l_str1,l_str2) RETURNING l_buf 
             LET l_str1 = "   AND oga00 <> 'A' "   #MOD-980231
             LET l_str2 = "   AND 1=1 "   #MOD-980231
             CALL cl_replace_str(l_buf,l_str1,l_str2) RETURNING l_buf    #No.TQC-A50147
          ...
          PREPARE r200_p1 FROM g_sql
          DECLARE r200_c1 CURSOR FOR r200_p1
         #FOREACH r200_c1 INTO sr.* #FUN-B90090 mark 
          FOREACH r200_c1 INTO sr.*,g_ima09,g_ima10,g_ima11 #FUN-B90090 add
              ...
              #CALL r200_case() #FUN-B90090 mark
               CALL r200_case(l_azp01) #FUN-B90090 add
               CALL r200_m(sr.oea23)
        ...
        ## *** 與 Crystal Reports 串聯段 - <<<< 寫入暫存檔 >>>> FUN-740015 *** ##
         ...
         sr.oeb04, sr.oea15, sr.oea14, sr.oea25, sr.oea03,
         sr.oca01, sr.ima131,sr.ima06, sr.azp01, l_oab02,
         l_occ02 , l_occ20 , l_occ21 , l_occ22 , l_azi05
        ,g_azf03_1,g_azf03_2,g_azf03_3 #FUN-B90090 add 
       #------------------------------ CR (3) ------------------------------#
 
          END FOREACH
         END FOR
         #---------------------------- 第二期 ---------------------------
#         CALL r200_g_sql(l_azp03,tm.b2,tm.y2,tm.m2b,tm.m2e,b_date2,e_date2)  #FUN-A10098
         CALL r200_g_sql(l_azp01,tm.b2,tm.y2,tm.m2b,tm.m2e,b_date2,e_date2)  #FUN-A10098
         FOR i=1 TO 2
          ...
          END IF
          CALL cl_parse_qry_sql(g_sql,l_azp01) RETURNING  g_sql   #FUN-A10098 
          PREPARE r200_p2 FROM g_sql
          DECLARE r200_c2 CURSOR FOR r200_p2
         #FOREACH r200_c2 INTO sr.* #FUN-B90090 mark
          FOREACH r200_c2 INTO sr.*,g_ima09,g_ima10,g_ima11 #FUN-B90090 add 
               IF SQLCA.sqlcode != 0 THEN
   ...
   END IF
   LET g_str = g_str,";",tm.s,";",tm.t,";",tm.u,";",tm.ch,";",b_date1,";",
               e_date1,";",b_date2,";",e_date2,";",tm.a1,";",tm.a2,";",
               tm.b1,";",tm.b2,";",tm.op
   CALL cl_prt_cs3('axsr200','axsr200',l_sql,g_str)   
   #------------------------------ CR (4) ------------------------------#
   #end FUN-740015 add 
...
#FUNCTION r200_case() #FUN-B90090 mark 
FUNCTION r200_case(p_azp01)             #FUN-B90090 add
  DEFINE p_azp01   LIKE azp_file.azp01, #FUN-B90090 add
...
#FUN-B90090----------add-------str
               WHEN tm.s[g_i,g_i] = 'g'
                    LET l_sql = "SELECT azf03 FROM ",cl_get_target_table(p_azp01,'azf_file'),
                                " WHERE azf01 = '",g_ima09,"'",
                                "   AND azf02 = 'D' AND azfacti = 'Y' "
                    PREPARE sel_azf03_pre1 FROM l_sql
                    DECLARE sel_azf03_cs1 CURSOR FOR sel_azf03_pre1
                    OPEN sel_azf03_cs1
                    FETCH sel_azf03_cs1 INTO g_azf03_1
                    CLOSE sel_azf03_cs1
                    IF STATUS THEN LET g_azf03_1 = '' END IF
                    LET l_order[g_i] = g_azf03_1
               WHEN tm.s[g_i,g_i] = 'h'
                    LET l_sql = "SELECT azf03 FROM ",cl_get_target_table(p_azp01,'azf_file'),
                                " WHERE azf01 = '",g_ima10,"'",
                    ...
                    OPEN sel_azf03_cs2
                    FETCH sel_azf03_cs2 INTO g_azf03_2
                    CLOSE sel_azf03_cs2
                    IF STATUS THEN LET g_azf03_2 = '' END IF
                    LET l_order[g_i] = g_azf03_2
               WHEN tm.s[g_i,g_i] = 'i'
                    LET l_sql = "SELECT azf03 FROM ",cl_get_target_table(p_azp01,'azf_file'),
                                " WHERE azf01 = '",g_ima11,"'",
                    ...
                    OPEN sel_azf03_cs3
                    FETCH sel_azf03_cs3 INTO g_azf03_3
                    CLOSE sel_azf03_cs3
                    IF STATUS THEN LET g_azf03_3 = '' END IF
                    LET l_order[g_i] = g_azf03_3
#FUN-B90090----------add-------end                    
