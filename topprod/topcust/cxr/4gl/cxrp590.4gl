...
FUNCTION p590_t(l_npptype)  #No.FUN-670047
  ...
  #DEFINE l_msg           LIKE type_file.chr8    #No.FUN-680123 VARCHAR(80)   #MOD-A70139 mark
   ...
   DEFINE l_hb_cnt        LIKE type_file.num5   
  ...
  END IF
   ...
   WHILE TRUE
     #CALL s_showmsg_init()    #No.FUN-710050    #MOD-AC0267 mark 
      DISPLAY '写临时表耗时开始:',TIME
      LET l_hb_cnt = 1
      FOREACH p590_1_c0 INTO l_npp.*,l_npq.*
         IF l_hb_cnt = 1 THEN
            DISPLAY '写一次临时表耗时开始:',TIME
         END IF
         IF STATUS THEN
        ...
        #INSERT INTO p590_tmp VALUES(l_order,l_npp.*,l_npq.*,l_remark)             #FUN-C50011 mark
         ...
         IF l_hb_cnt = 1 THEN
            DISPLAY '写一次临时表耗时结束:',TIME
         END IF
         LET l_hb_cnt = l_hb_cnt + 1
      END FOREACH
      DISPLAY '写临时表耗时结束:',TIME
...
DEFINE l_aba01 LIKE aba_file.aba01
    ...
    #先找出單號符合的應收單身資料
    LET g_sql = "UPDATE ",cl_get_target_table(p_plant,"oct_file"),
                "   SET oct08 = '",l_aba01 CLIPPED,"'",
                " WHERE oct00 = '",l_npp.npptype CLIPPED,"'",
                "   AND oct16 = '1'",
                "   AND oct01 = '",l_npp.npp01,"'",
                "   AND EXISTS(SELECT 1 FROM ",cl_get_target_table(p_plant," omb_file"),
                "                      WHERE omb01=oct01 AND omb03=oct02 AND omb00 = '12' ",
                "                        AND omb01='",l_npp.npp01,"')"
