...
DEFINE tm  RECORD
           ...
           e       LIKE type_file.chr1,          #add by zhangyu
...
DEFINE g_tlf       DYNAMIC ARRAY OF RECORD
                   ...
                   tlfc221        LIKE tlfc_file.tlfc221,       #材料成本      #add by wuls 160223
                   tlfc222        LIKE tlfc_file.tlfc222,       #人工成本      #add by wuls 160223
                   tlfc2231       LIKE tlfc_file.tlfc2231,      #制费成本      #add by wuls 160223
                   tlfc2232       LIKE tlfc_file.tlfc2232,      #加工成本      #add by wuls 160223
                   tlfc224        LIKE tlfc_file.tlfc224,       #其他成本      #add by wuls 160223
                   tlfc2241       LIKE tlfc_file.tlfc2241,      #制费三成本      #add by wuls 160223
                   tlfc2242       LIKE tlfc_file.tlfc2242,      #制费四成本      #add by wuls 160223
                   tlfc2243       LIKE tlfc_file.tlfc2243       #制费五成本      #add by wuls 160223
...
MAIN
   ...
   IF cl_null(g_bgjob) or g_bgjob = 'N'THEN
      OPEN WINDOW q761_w AT 5,10  #FUN-C80092 add
           WITH FORM "cxc/42f/axcq761" ATTRIBUTE(STYLE = g_win_style)  #FUN-C80092 add
      CALL cl_ui_init()           #FUN-C80092 add
      CALL cl_set_act_visible("revert_filter",FALSE)
      CALL axcq761_tm(0,0)
      CALL q761_menu()            #FUN-C80092 add
      DROP TABLE axcq761_ktmp;     #FUN-C80092 add
...
#wujie 130904 --end          
  ...
  SELECT oaz92,oaz93 INTO g_oaz.oaz92,g_oaz.oaz93 FROM oaz_file    #add by kuangxj170213
#FUN-C80092 -----------Begin-------------
...
#      INSERT INTO axcq761_ktmp VALUES (g_tlf_excel[l_n].*,g_tlfc[l_n].*)
#        
#        LET l_n = l_n + 1 
...
#                  "   ORDER BY tlf01,tlf905,tlf906 "  
      ...
      LET l_sql = "SELECT tlf905,tlf906,tlf01,ima02,ima021,tlf19,occ02,NVL(SUM(tlf10*tlf60*tlf907),0),NVL(SUM(tlfc21*tlf907),0),",
                  "        0 wsaleamt,0 l_wsale_tlf21,0 l_rate,1 salerat,1 raterat,occ04,'' gen02,'' gem01,'' gem02,",
                  "        '' oma08,'' oab01,occ03,'' warea_name,ima131,tlf036,NVL(occ37,'N'),",
                  "        SUM(tlfc221),SUM(tlfc222),SUM(tlfc2231),SUM(tlfc2232),SUM(tlfc224),SUM(tlfc2241),SUM(tlfc2242),SUM(tlfc2243),",             #add by wuls 160223
                  "       NVL(SUM(tlfc221*tlf907),0),NVL(SUM(tlfc222*tlf907),0),NVL(SUM(tlf2231*tlf907),0),NVL(SUM(tlf2232*tlf907),0),",
                  ...
                  "  ,",cl_get_target_table(m_plant[l_i],'smy_file'),      #No.MOD-D30160
                  " WHERE ima01 = tlf01" 
     # IF g_oaz.oaz92 ='N' THEN      #MARK BY kuangxj170213
      IF g_oaz.oaz92='N' OR (g_oaz.oaz92='Y' AND g_oaz.oaz93='N') THEN # add by kuangxj170213 
         LET l_sql1 ="   AND (tlf13 LIKE 'axmt%' OR tlf13 LIKE 'aomt%')"
      ELSE 
# mark by zhangyu ----str
#         LET l_sql1 ="   AND (tlf13 = 'axmt670' OR tlf99 IS NOT NULL )"
# mark by zhangyu ---end 
# add by zhangyu---str
      IF tm.e='Y' THEN 
         LET l_sql1 ="   AND ((tlf13 ='axmt620' AND tlf99 IS NOT NULL ) OR tlf13 = 'axmt670' )" #zhangyu 20150529
      ELSE 
         LET l_sql1 ="   AND (tlf13 = 'axmt670' )"
      END IF 
     END IF
# add by zhangyu--end
     #add by kuangxj170213 begin
      IF g_oaz.oaz92='Y' AND g_oaz.oaz93='N' THEN 
         LET l_sql2 ="   AND ",tm.wc CLIPPED,
                  ...
                  "   AND (tlf06 BETWEEN '",tm.bdate,"' AND '",tm.edate,"')",
                  "   AND tlf905 LIKE TRIM(smyslip)||'-%' AND smydmy1 ='Y' ",    #No.MOD-D30160
                  " GROUP BY tlf01,ima02,ima021,ima131,occ02,occ03,occ04,occ37,tlf036,tlf13,tlf19,tlf66,tlf905,tlf906,azf08,tlf907 ", 
#MOD-D90159 add tlf907
                  "   ORDER BY tlf01,tlf905,tlf906 "
      ELSE 
      #add by kuangxj170213 end 
         LET l_sql2 ="   AND ",tm.wc CLIPPED,
                  "   AND ",g_filter_wc CLIPPED,
                  "   AND tlf902 not in (SELECT jce02 from ",cl_get_target_table(m_plant[l_i],'jce_file'),")",  
                  "   AND tlf920='Y' ",    #FUN-CC0157 add
                  ...
                  "   ORDER BY tlf01,tlf905,tlf906 "   
     END IF #add by kuangxj170213
      LET l_sql = l_sql,l_sql1,l_sql2        
#wujie 130904 --end
      LET l_sql = " INSERT INTO axcq761_ktmp ",
      ...
      LET l_sql = " DELETE FROM axcq761_ktmp ",
      ...
      LET l_sql = "MERGE INTO axcq761_ktmp o",
      ...
      UPDATE axcq761_ktmp SET ogb01 = tlf905,
                             ogb03 = tlf906
      ...
      LET l_sql = "MERGE INTO axcq761_ktmp o",
                 ...
                 #"  ON (o.ogb01 =x.omb31 AND o.ogb03 =x.omb32 )",  #FUN-CC0157 mod                           #MOD-D90121 mark
                  "  ON (o.ogb01 =x.omb31 AND o.ogb03 =x.omb32 )",    #AND (o.tlf905 = x.oma76 OR x.oma76 IS NULL))", #MOD-D90121 add   #mark by kuangxj170214
                  " WHEN MATCHED ",
                  " THEN ",
                  "   UPDATE ",
...
#                  "      SET o.wsaleamt = o.wsaleamt+ x.omb16 ",      #No.TQC-D90012
      ...
      UPDATE axcq761_ktmp SET wsaleamt=wsaleamt*-1 WHERE tlf907='1'  #MOD-D90159 add

      UPDATE axcq761_ktmp SET tlf10 = tlf10 * -1,
                             tlfc21= tlfc21* -1   

      LET l_sql = "UPDATE axcq761_ktmp o",
      ...
      UPDATE axcq761_ktmp SET l_wsale_tlf21 = wsaleamt - tlfc21
      
      LET l_sql = "UPDATE axcq761_ktmp SET l_rate = l_wsale_tlf21/wsaleamt " ,
                  "WHERE wsaleamt <> 0"
      PREPARE upd_l_rate FROM l_sql
      EXECUTE upd_l_rate                   
 
      DELETE FROM axcq761_ktmp WHERE tlf10 = 0 AND wsaleamt= 0 AND (tlfc21=0 OR tlfc21 IS NULL)
 
      IF tm.b = '1' THEN
         DELETE FROM axcq761_ktmp WHERE occ37 ='N'
      END IF
      IF tm.b = '2' THEN   #非關係人
         DELETE FROM axcq761_ktmp WHERE occ37 ='N'
      END IF    
      ...
      IF tm.a = '1' THEN
         DELETE FROM axcq761_ktmp WHERE oma08 <>'1'
      END IF
      IF tm.a = '2' THEN  
         DELETE FROM axcq761_ktmp WHERE oma08 <>'2'
      END IF     
...
#FUN-D10022----------------add-------------end-----------------------        
   ...
   IF tm.c = 'Y' THEN
      ...
      SELECT SUM(tlf10),SUM(tlfc21),SUM(wsaleamt) 
       ...
       FROM axcq761_ktmp WHERE tlf13 LIKE 'axmt%'
       
      SELECT SUM(tlf10),SUM(tlfc21)
        INTO l_num2,l_cost2
       FROM axcq761_ktmp WHERE tlf13 LIKE 'axmt%' AND azf08 ='Y'   
       
      SELECT SUM(tlf10),SUM(tlfc21),SUM(wsaleamt) 
        INTO l_num3,l_cost3,l_wsaleamt2
       FROM axcq761_ktmp WHERE tlf13 NOT LIKE 'axmt%' 
...
FUNCTION axcq761_table()
     DROP TABLE axcq761_ktmp;
     CREATE TEMP TABLE axcq761_ktmp(
...
FUNCTION axcq761_show()
   DISPLAY tm.type,tm.bdate,tm.edate,tm.cb1,tm.cb2,
           tm.cb3,tm.cb4,tm.a,tm.b,tm.c,tm.d,tm.e
        TO type,bdate,edate,cb1,cb2,cb3,cb4,a,b,c,d,e
   LET g_tot = 0 
   LET g_sum1 = 0 
   LET g_sum2 = 0
   LET g_sum3 = 0 
...
DEFINE i     LIKE type_file.num5
   ...
   #            "  FROM axcq761_ktmp ",l_str,l_str1 
   #FUN-D10022--mark--end--
   #FUN-D10022--add--str--   
   LET g_sql = g_sql CLIPPED,",NVL(SUM(tlf10),0),NVL(SUM(tlfc21),0),",
               ...
               "  FROM axcq761_ktmp x ",l_str2,l_str,l_str1 
   #FUN-D10022--add--end--               
...
DEFINE l_tlf907        LIKE tlf_file.tlf907 #MOD-D90159 add
   ...
   LET g_sql = "SELECT * ",           #FUN-C80092 add  121225
               #"  FROM axcq761_ktmp ORDER BY tlf026 "  #FUN-D10022
               "  FROM axcq761_ktmp ORDER BY tlf905 "   #FUN-D10022
...
FUNCTION q761_set_visible()
   IF tm.d = 'Y' THEN 
      CALL cl_set_comp_visible("tlfc221a,tlfc222a,tlfc2231a,tlfc224a,
                                tlfc2241a,tlfc2242a,tlfc2243a,tlfc2232a,tlfc221,tlfc222,tlfc2231,tlfc2232,tlfc224,tlfc2241,tlfc2242,tlfc2243",TRUE)    #,tlf221,tlf222,tlf2231,tlf2232,tlf224,tlf2241,tlf2242,tlf2243  #add by wuls 160223
   ELSE
      CALL cl_set_comp_visible("tlfc221a,tlfc222a,tlfc2231a,tlfc224a,
                                tlfc2241a,tlfc2242a,tlfc2243a,tlfc2232a,tlfc221,tlfc222,tlfc2231,tlfc2232,tlfc224,tlfc2241,tlfc2242,tlfc2243",FALSE)   #,tlf221,tlf222,tlf2231,tlf2232,tlf224,tlf2241,tlf2242,tlf2243  #add by wuls 160223
   END IF
END FUNCTION
