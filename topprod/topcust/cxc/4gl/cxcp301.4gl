...
#->No.FUN-570153 ---start---
   LET p_row = 1 LET p_col = 1
    ...
    OPEN WINDOW p301 AT p_row,p_col WITH FORM "cxc/42f/cxcp301" 
...
REPORT axcp301_rep(l_order,l_npp,l_npq,l_remark)
 ...
 #FUN-C70093--ADD--END
  ...
  DEFINE   l_aba07            LIKE aba_file.aba01 
  ...
  FORMAT
   ...
   #查找作废单
  #  LET l_aba07 =  'A','800','-',g_yy USING '&&&&',g_mm CLIPPED USING '&&','0907'
    SELECT aba01 INTO l_aba01 FROM aba_file WHERE aba00= p_bookno  AND aba07 = l_order AND aba19='X'
  IF NOT cl_null(l_aba01) THEN 
     LET gl_no  = l_aba01
      DELETE FROM aba_file WHERE aba00=p_bookno  AND aba19='X'  AND aba01=l_aba01  
      DELETE FROM abb_file WHERE abb00=p_bookno  AND abb01=l_aba01 
  ELSE   
   #查找作废单                                            
   #缺號使用完，再在流水號最大的編號上增加       
   IF l_flag1='N' THEN  
