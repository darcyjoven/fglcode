# Prog. Version..: '5.25.04-11.09.15(00002)'     #
...
# Modify.........: NO.130806     13/08/06 by xiayan 修改aag19的值
# Modify.........: NO.130808     13/08/08 by tangyh 产生的抵消投资收益单据日期应为当月的最后一天
...
DEFINE tm  RECORD
             ...
             ,axa02   LIKE axa_file.axa02  #NO.130806 add
           END RECORD
DEFINE tm1 RECORD
              wc      LIKE type_file.chr1000
             ,wc1     LIKE type_file.chr1000  #NO.130806 add
           END RECORD
...
FUNCTION p018_tm()
   ...
   OPEN WINDOW p018_w1 AT p_row,p_col
     WITH FORM "cgl/42f/cglp018"  ATTRIBUTE (STYLE = g_win_style CLIPPED)                                     
   CALL cl_ui_init()
   #使用預設帳別之幣別
   ...
   WHILE TRUE
      ...
      #CONSTRUCT BY NAME tm1.wc ON axa02   #NO.130806 mark
       
      #BEFORE CONSTRUCT   #NO.130806 mark
      INPUT BY NAME tm.axa02 WITHOUT DEFAULTS  #NO.130806 modify
        BEFORE INPUT  #NO.130806 modiify
         CALL cl_qbe_init()
      ...
      ON ACTION CONTROLP
        CASE
            WHEN INFIELD(axa02) 
               ...
               #LET g_qryparam.state = "c"   #NO.130806 mark
               LET g_qryparam.arg1 = tm.c   #NO.130806 add
               #CALL cl_create_qry() RETURNING g_qryparam.multiret  #NO.130806 mark
               #DISPLAY g_qryparam.multiret TO axa02  #NO.130806 mark
               CALL cl_create_qry() RETURNING tm.axa02   #NO.130806 modify
               DISPLAY tm.axa02 TO axa02   #NO.130806 modify
               NEXT FIELD axa02   
         END CASE
      ...
      ON IDLE g_idle_seconds
         CALL cl_on_idle()
         #CONTINUE CONSTRUCT   #NO.130806 mark
         CONTINUE INPUT   #NO.130806 modify
      ...
      ON ACTION exit                            #加離開功能
         LET INT_FLAG = 1
         #EXIT CONSTRUCT  #NO.130806 mark
         EXIT INPUT   #NO.130806 modify

      ON ACTION qbe_select
         CALL cl_qbe_select()

     #END CONSTRUCT   #NO.130806 mark
     END INPUT   #NO.130806 modify
     IF INT_FLAG THEN
        LET INT_FLAG = 0
        CLOSE WINDOW p018_w1
        CALL cl_used(g_prog,g_time,2) RETURNING g_time 
...
DEFINE l_axi12 LIKE axi_file.axi12
   ...
   LET l_sql = "SELECT axa02 FROM axa_file ",
               " WHERE axa01 = '",tm.c,"'",
               #"   AND ",tm1.wc CLIPPED   #NO.130806 mark
               "   AND axa02 = '",tm.axa02,"'"  #NO.130806 modify
   ...
   FOREACH sel_axa02_cur INTO l_axa02   
      ...
      FOR i = 1 TO g_no 
          ...
          LET l_sql = "SELECT aag01 FROM aag_file ",
                      #" WHERE aag00 = '",g_aaz.aaz641,"' AND aag19 = '46'",  #NO.130806 mark
                      " WHERE aag00 = '",g_aaz.aaz641,"' AND aag19 IN ('21','22')", #NO.130806 modify
                      "   AND aag07 <>'1'"       
...
DEFINE g_axi01 LIKE axi_file.axi01
   ...
   SELECT COUNT(*) INTO l_cnt FROM axi_file
    WHERE axi00 = g_aaz.aaz641 AND axi03 = tm.y AND axi04 = tm.m
      ...
      AND axi06 = tm.axa02   #NO.130806 modify
 ...
 #NO.130806 modify-----------------------end
    ...
    #FUN-B80135--add--str--
      IF tm.y <g_year OR (tm.y=g_year AND tm.m<=g_month) THEN 
         ...
         CALL s_errmsg('y,m',g_showmsg,'','axm-164',1)
      END IF 
    #FUN-B80135--add--end
