# Prog. Version..: '5.25.04-11.09.23(00003)'     #
#
...
#   WHERE axz01 = g_axi.axi06
   SELECT axz06 INTO g_axz06 FROM axz_file,axa_file
    WHERE axz01 = axa02 AND axa04 = 'Y'
      AND axa01 = g_axi.axi05
#FUN-B90057--mod--end
       ...
       WITHOUT DEFAULTS
        ...
        AFTER FIELD axi02
           IF NOT cl_null(g_axi.axi02) THEN
              ...
              IF g_axi.axi03 < g_year THEN
                 CALL cl_err(g_axi.axi03,'axm-164',0)
                 NEXT FIELD axi03
              END IF
              IF g_axi.axi03=g_year AND g_axi.axi04<=g_month THEN
                 CALL cl_err(g_axi.axi04,'axm-164',0)
                 NEXT FIELD axi04
              END IF  
        ...
        AFTER FIELD axi03
           ...
           END IF
          #No.FUN-B80135--add--str--
          IF NOT cl_null(g_axi.axi03) THEN
             IF g_axi.axi03 < 0 THEN
                CALL cl_err(g_axi.axi03,'apj-035',0)
                NEXT FIELD axi03 
             END IF 
             IF g_axi.axi03 < g_year THEN
                CALL cl_err(g_axi.axi03,'axm-164',0)
        ...
        AFTER FIELD axi04
           ...
           END IF
            #FUN-B80135--add--str--
             IF NOT cl_null(g_axi.axi04) THEN
                SELECT azm02 INTO g_azm.azm02 FROM azm_file
                 WHERE azm01 = g_axq.axq06
                 IF g_azm.azm02 = 1 THEN
                    IF g_axi.axi04 > 12 OR g_axi.axi04 < 1 THEN
                       CALL cl_err('','agl-020',0)
                       NEXT FIELD axi04
                    END IF
                 ELSE
                    IF g_axi.axi04 > 13 OR g_axi.axi04 < 1 THEN
                       CALL cl_err('','agl-020',0)
                       NEXT FIELD axi04
                    END IF
                END IF
                IF NOT cl_null(g_axi.axi03) AND g_axi.axi03=g_year
                   AND g_axi.axi04<=g_month THEN
                   CALL cl_err(g_axi.axi04,'axm-164',0)
                   NEXT FIELD axi04
                END IF
            END IF
           ...
           IF NOT cl_null(g_axi.axi05) THEN
             ...
             #FUN-B90057--add--str--
              ...
              IF cl_null(g_axz06) THEN
                 CALL cl_err(g_axi.axi05,'afa-050',0)
                 NEXT FIELD axi05
              ELSE
...
FUNCTION t001_copy()
       ...
       AFTER FIELD axi05   #族群編號
           IF NOT cl_null(l_newaxi05) THEN
             ...
             #FUN-B90057--add-str--
              SELECT axz06 INTO g_axz06 FROM axz_file,axa_file
               WHERE axz01 = axa02 AND axa04 = 'Y'
                 AND axa01 = l_newaxi05
              IF cl_null(g_axz06) THEN
                 CALL cl_err(l_newaxi05,'afa-050',0)
                 NEXT FIELD axi05
              ELSE
                 DISPLAY  g_axz06 TO FORMONLY.axz06
              END IF
   ...
   CALL t001_u()
   CALL t001_b()
   SELECT axi_file.* INTO g_axi.* FROM axi_file WHERE axi01 = l_oldno AND axi00 = l_oldno1
   CALL t001_show()
...
FUNCTION t001_r()
    ...
    DEFINE l_cnt    LIKE type_file.num5  
    DEFINE l_aen10  LIKE aen_file.aen10
    DEFINE l_axv18  LIKE axv_file.axv18   
    ...
    IF cl_delete() THEN
       ...
       #-----add 130814 by lily   begin-----
        SELECT aen10 INTO l_aen10 FROM aen_file
        WHERE aen10 = g_axi.axi01
        IF l_aen10 IS NOT NULL THEN 
       #-----add 130814 by lily   end -----
       ...
       IF g_axi.axi08 = '2' AND g_axi.axi081 = '2' THEN
        #-----add 130814 by lily   begin-----
        SELECT axv18 INTO l_axv18 FROM axv_file 
           WHERE axv01 = g_axi.axi03
             AND axv02 = g_axi.axi04 
             AND axv03 = g_axi.axi05
             AND axv031 = g_axi.axi06
        IF l_axv18 IS NOT NULL THEN 
       #-----add 130814 by lily   end -----
