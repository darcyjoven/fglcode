...
FUNCTION q111_q()
  ...
  # DISPLAY '   ' TO FORMONLY.cnt
    ...
    MESSAGE " SEARCHING ! "
    #OPEN q111_cs                            # 從DB產生合乎條件TEMP(0-30秒)
#    IF SQLCA.sqlcode THEN
#        CALL cl_err('',SQLCA.sqlcode,0)
#    ELSE
    #    OPEN q111_count
    #    FETCH q111_count INTO g_row_count
       #CALL q111_fetch('F')                  # 讀出TEMP第一筆並顯示
        CALL q111_b_fill()
 #   END IF
    MESSAGE ""
END FUNCTION
...
DEFINE
    ...
    CASE p_flag
        WHEN 'N' FETCH NEXT     q111_cs INTO g_ima.ima01
        WHEN 'P' FETCH PREVIOUS q111_cs INTO g_ima.ima01
        WHEN 'F' FETCH FIRST    q111_cs INTO g_ima.ima01
        WHEN 'L' FETCH LAST     q111_cs INTO g_ima.ima01
        WHEN '/'
            IF (NOT mi_no_ask) THEN
               ...
               END IF
            END IF
            FETCH ABSOLUTE g_jump q111_cs INTO g_ima.ima01
            LET mi_no_ask = FALSE
    END CASE
...
FUNCTION q111_b_fill()              #BODY FILL UP
   DEFINE l_sql     LIKE type_file.chr1000,       #No.FUN-680121 VARCHAR(1000)
          l_sqlr     LIKE type_file.chr1000,
...
#         l_tt      LIKE ima_file.ima26           #No.FUN-680121 DECIMAL(12,3)
   ...
   LET l_sql =
        "SELECT DISTINCT  tlf06,tlf01,ima02,ima021,0 qichu,0 ruku,0 chuku,0 jiecun,0 price,tlf902,tlf903  ",
        " FROM   tlf_file ",
    ...
    PREPARE q111_pb FROM l_sql
    DECLARE q111_bcs                       #BODY CURSOR
        CURSOR FOR q111_pb
    ...
    FOREACH q111_bcs INTO g_tlf1[g_cnt].*
        IF SQLCA.sqlcode THEN
        ...
        ELSE
           SELECT SUM(tlf10*tlf907) INTO l_tlf10 FROM tlf_file WHERE tlf01=g_tlf1[g_cnt].tlf011 AND tlf902=g_tlf1[g_cnt].tlf902
        ...
        END IF 
        #  当天入库
        SELECT  SUM(tlf10) INTO g_tlf1[g_cnt].ruku FROM tlf_file WHERE tlf01=g_tlf1[g_cnt].tlf011 AND tlf902=g_tlf1[g_cnt].tlf902
        ...
        # 当天出库 
        SELECT  SUM(tlf10) INTO g_tlf1[g_cnt].chuku FROM tlf_file WHERE tlf01=g_tlf1[g_cnt].tlf011 AND tlf902=g_tlf1[g_cnt].tlf902
        AND tlf903=g_tlf1[g_cnt].tlf903 AND tlf06=g_tlf1[g_cnt].tlf06 AND tlf907=-1
        IF cl_null(g_tlf1[g_cnt].chuku) THEN LET g_tlf1[g_cnt].chuku=0 END IF 
        #当日结存  =上期期末+当月到当天的库存异动
        SELECT sum(tlf10*tlf907) INTO ll_tlf10 FROM tlf_file WHERE tlf01=g_tlf1[g_cnt].tlf011 AND tlf902=g_tlf1[g_cnt].tlf902
...
LET l_sqlr =
        "SELECT CASE WHEN pmj05='RMB' THEN pmj07 ELSE PMJ07*XS END PMJ07  FROM ",
        "(SELECT PMJ03,MAX(to_char(PMJ09,'yyyyMMdd')||pmi01||pmj02) AS PMJ09 FROM PMJ_FILE,pmi_file ",
        " WHERE  pmi01=pmj01  and pmiconf='Y' AND PMJ03='",g_tlf1[g_cnt].tlf011,"' GROUP BY PMJ03 ) a  ",
        "LEFT JOIN ",
        "( SELECT a.*,b.*,to_char(PMJ09,'yyyyMMdd')||pmi01||pmj02 pmj09a FROM PMJ_FILE a,pmi_file b ",
        " WHERE pmi01=pmj01  and pmiconf='Y' AND PMJ03='",g_tlf1[g_cnt].tlf011,"' ) b  ON  a.PMJ09=b.pmj09a  ",
        " LEFT JOIN ",
        "(SELECT azj01,azj04 AS xs FROM azj_file WHERE azj02=to_char(SYSDATE,'yyyymm') ) c ON PMJ05=AZJ01 "
         PREPARE q111_pc FROM l_sqlr
         DECLARE q111_bct  CURSOR FOR q111_pc
         FOREACH q111_bct INTO g_tlf1[g_cnt].price
         END FOREACH
