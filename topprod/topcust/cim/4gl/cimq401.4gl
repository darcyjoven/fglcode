...
DEFINE
    ...
    g_img       DYNAMIC ARRAY OF RECORD
                ...
                oeb13   LIKE img_file.img10,
                oea24   LIKE img_file.img10,
                oeb14   LIKE img_file.img10
...
#str----end by huanglf160927
          ...
          LET g_imd.chk1 = 'Y'
           ...
           CONSTRUCT tm.wc2 ON img01,img09,img10
                     FROM s_img[1].img01,s_img[1].img09,s_img[1].img10
...
FUNCTION q401_b_fill()              #BODY FILL UP
  #DEFINE l_sql     LIKE type_file.chr1000 #No.FUN-690026 VARCHAR(1000)   #MOD-B90146 mark
   ...
   DEFINE  l_cn1   LIKE oeb_file.oeb14 


 IF g_imd.chk1 = 'N' THEN 
   LET l_sql =
        "SELECT img01, ima02, ima021,imaud10, img09, img10 ",  #MOD-6A0130 modify  #add imaud10 by guanyao160913
...
ELSE
   ...
   LET l_cn1=0
   LET l_sql1 =
        "SELECT img01,ima02,ima021,imaud10, img09,img10,xmf07,XMF02,ROUND(img10*xmf07,2) TJ from  ",
        "(SELECT img01,ima02,ima021,imaud10,img09, sum(img10) img10  ",
        "FROM  img_file,ima_file WHERE img02 = '",g_imd.imd01,"' AND  1=1 AND   img_file.img01 = ima_file.ima01 AND img10!=0   ",
        "GROUP BY img01,img03,ima02,ima021,img09,imaud10 )a  ",
        "left join  ",
        "( select xmf03,xmf02,xmf07 from (  ",
        "SELECT xmf03,xmf05,xmf02,xmf07,rank()over(partition by xmf03 order by XMF05 DESC,XMF07 DESC) rk  FROM   ",
        "   (SELECT xmf03,XMF05,  ",
        "   case when xmf02='RMB' THEN 1 ELSE AZJ041 END xmf02,CASE WHEN XMF02='RMB' THEN xmf07*(xmf08/100) ELSE xmf07*(xmf08/100)*AZJ041 END XMF07  ",
         "      FROM xme_file,gec_file,xmf_file,azj_file WHERE xme01 = xmf01 AND xme02 = xmf02   ",
        "       AND ta_xme01 =ta_xmf02  ",
        "       AND ta_xmf02 = gec01  and azj01(+)=xmf02 AND AZJ02(+)=TO_CHAR(SYSDATE,'YYYYMM')   ",
        "       AND xme00 = '1' ) A  )  where rk=1  ",
        ") b on a.img01=b.xmf03 ORDER BY IMG01 "
        #"SELECT img01,ima02,ima021,imaud10,sum(img10) ",
    ...
    DECLARE q401_bcs1  CURSOR FOR q401_pb1
    FOREACH q401_bcs1 INTO g_img[g_cnt].*
      IF NOT cl_null(g_img[g_cnt].oeb14) THEN
         LET l_cn1=l_cn1+g_img[g_cnt].oeb14
      END IF 
    LET g_cnt = g_cnt + 1
    END FOREACH
    
    DISPLAY l_cn1 TO FORMONLY.cn1
END IF 
...
#IF g_imd.chk1 = 'Y' THEN 
  #  FOREACH q401_bcs INTO g_img[g_cnt].*
   ...
   #LET l_sql11 =
     #   "SELECT  XMF07,XMF05 FROM         "
      #  "(SELECT XMF05,CASE WHEN XMF02='RMB' THEN xmf07*(xmf08/100) ELSE xmf07*(xmf08/100)*AZJ041 END XMF07        ",
      #  "FROM xme_file,gec_file,xmf_file,azj_file WHERE xme01 = xmf01 AND xme02 = xmf02            ",
      #  "AND ta_xme01 =ta_xmf02   AND xmf03 ='",g_img[g_cnt].IMG01,"' ",
      #  "AND ta_xmf02 = gec01  and azj01(+)=xmf02 AND AZJ02(+)=TO_CHAR(SYSDATE,'YYYYMM')         ",
      #  "AND xme00 = '1' ) A ORDER BY xmf05 DESC,xmf07 DESC         "
         
      #       PREPARE q401_pb11 FROM l_sql11
     #        DECLARE q401_bcs11                       #BODY CURSOR
      #       CURSOR FOR q401_pb11
      #        FOREACH q401_bcs11 INTO g_img[g_cnt].oeb13
              
      #  DISPLAY g_img[g_cnt].oeb13 TO oeb13
#--TQC-CC0003--add----end----
     #   LET g_cnt = g_cnt + 1
...
#ELSE
   ...
   # PREPARE q401_pb2 FROM l_sql2
    #EXECUTE q401_pb2 INTO g_img[g_cnt].img09,g_img[g_cnt].img19,g_img[g_cnt].img36,g_img[g_cnt].img37
#--TQC-CC0003--add----star---
