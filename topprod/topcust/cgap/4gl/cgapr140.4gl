...
MAIN
   ...
   LET  l_table = cl_prt_temptable('cgapr140',g_sql) CLIPPED
   IF l_table=-1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?, ?,?)"   #add ?? by lifang 200422                     
...
FUNCTION cgapr140_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,        
   ...
   OPEN WINDOW cgapr140_w AT p_row,p_col WITH FORM "cgap/42f/cgapr140"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW cgapr140_w 
  ...
  #UI
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('cgapr140',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW cgapr140_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0094
  ...
  #--add by lifang 200422 end#
   ...
   LOCATE l_img_blob IN MEMORY             
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='cgapr140' 
...
LET l_sql="select rvv06,pmc17,rvw18,'','',rvw02,rvw01,pmc081,pmc24,pmc52,rvw11,",  #add rvw18 by liyjf161109 
     ...
     PREPARE cgapr140_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
        EXIT PROGRAM 
     END IF
     DECLARE cgapr140_curs1 CURSOR FOR cgapr140_prepare1
 LET l_num = 1  
     FOREACH cgapr140_curs1 INTO sr.*
     ...
     END IF
       ...
       LET sr.apa34 =0
       LET sr.apa34f =0
       SELECT apa34,apa34f INTO sr.apa34,sr.apa34f
         FROM apa_file
     ...
     CALL cl_prt_cs3('cgapr140','cgapr140',g_sql,g_str)
