...
#
#]
FUNCTION cws_get_aimt302_process()
   ...
   DEFINE l_statu2    RECORD                  #返回料件信息
    ...
    END IF  
     ...
     WHERE ina01=inb01 AND (ina00='3' OR ina00 = '4') AND inaconf='Y' AND inapost='N' AND ina01=l_barcode    #add wanjz by 170105 add ina00 = '4'
    IF l_n>0 THEN 
    	 LET l_sql="SELECT '3',ina01,inb04,ima02,ima021,inb08,SUM(inb09) ",
    		  	  	  ...
    		  	  	  "AND (ina00='3' OR ina00 = '4')  AND inaconf='Y' and inapost='N' ",
       	 ...
       	 IF l_n=1 THEN 
    	 	 ...
    	 	 ELSE 
    	 		 ...
    	 		 IF g_barcode_n.stat = '1' THEN 
		 		 	 ...
		 		 	 LET l_statu1.ime02=g_barcode_n.imd02
		 		 ...
		 		 ELSE 
		 		 	 IF g_barcode_n.stat MATCHES '[2345]' THEN 
		 		 	 	 ...
		 		 	 	 SELECT ima25 INTO l_statu2.inb17 FROM ima_file WHERE ima01 = l_statu2.ima01
#		 		 	 	 LET l_statu2.qty=g_return.b_num
		 		 	 	 CALL aws_ttsrv_addParameterRecord(base.TypeInfo.create(l_statu2))
		 		 	 ELSE 
		 		 	    LET g_status.code = -1
       		                            LET g_status.description = '条码有误,或单据已过账或未审核!'
       		                            RETURN
		 		 	 END IF 
  	 	 		 END IF 
       	 END IF 
