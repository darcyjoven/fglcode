...
FUNCTION cws_create_work_process()
    ...
    #tianry add 170315   新增管控runcard+工序存在于cpmi001中 不允许前台扫入报工
{    SELECT COUNT(*) INTO l_cnt FROM tc_pmm_file WHERE tc_pmm03=l_tc_shb03
    AND tc_pmm08=g_tc_shb.tc_shb06 AND tc_pmm11='Y' 
    IF l_cnt>0 THEN
       LET g_status.code = '-1'
       LET g_status.description = '此RUN-CARD+工序为委外申请工序不可扫描开工'
       LET l_return.code="-1"
       LET l_return.msg = "开工信息录入失败"
       CALL aws_ttsrv_addParameterRecord(base.TypeInfo.create(l_return))
       RETURN
   ...
   # END IF 
    ...
    PREPARE  cre_wor_pre FROM l_sql
    declare cre_wor_cur CURSOR FOR cre_wor_pre
    OPEN cre_wor_cur
    FETCH cre_wor_cur INTO l_str
    CLOSE cre_wor_cur
    ...
    #str----add by neil190520
    #end----add by neil190520
    ...
    END IF
      ...
      #str---黄光区 FIFO  ly180815
      {
    IF g_tc_shb.tc_shb09='G1007' THEN 
       LET l_cnt=0
       SELECT  COUNT(*) INTO  l_cnt
       FROM sgm_file,ecd_file 
       WHERE  sgm04=ecd01 AND ecd07='G1007' AND  sgm02=g_tc_shb.tc_shb04  
       AND sgm03=g_tc_shb.tc_shb06  AND sgm01<g_tc_shb.tc_shb03
       AND sgm301+sgm302+sgm303+sgm304-sgm311-sgm312-sgm313-sgm314-sgm315-sgm316-sgm317>0

      IF cl_null(l_cnt) THEN LET l_cnt=0 END IF 

       IF l_cnt > 0 THEN
          LET g_status.code = '-1'
          LET g_status.description = '违反黄光区FIFO开工失败'
          LET l_return.code="-1"
          LET l_return.msg = "该作业编码隶属黄光区LOT号小的需FIFO"
          CALL aws_ttsrv_addParameterRecord(base.TypeInfo.create(l_return))
           RETURN 
       END IF
    END IF 
    #end---黄光区 FITO  ly180815
    }
    ...
    IF l_tc_shb12 >0 THEN   
       SELECT SUM(tc_shb12) INTO l_tc_shb12_1 FROM tc_shb_file  #已开工的数量大于的时候需要判断此人开工量和完工量是否相同
        WHERE tc_shb01 = '1'
          AND tc_shb03 = g_tc_shb.tc_shb03
          AND tc_shb06 = g_tc_shb.tc_shb06
         AND tc_shb11 = g_tc_shb.tc_shb11      #同一工序可不同人报工  ly 180528
    ...
    ELSE#非第一道工艺
       ...
       IF l_ta_ecd04 ='Y' THEN 
          ...
          ELSE    #非期初，前一道工艺是包装
             ...
             #   SELECT SUM(tc_shb12+tc_shb121+tc_shb122) INTO l_tc_shc12 FROM tc_shb_file  #180103
                 SELECT SUM(tc_shb12+nvl(tc_shb121,0)+nvl(tc_shb122,0))  INTO  l_tc_shc12 FROM tc_shb_file
                  WHERE tc_shb01 = '2'  #1 改2 ly #180104
                   AND tc_shb03 = g_tc_shb.tc_shb03
                   AND tc_shb06 = l_sgm03
                IF cl_null(l_tc_shc12) OR l_tc_shc12 = 0 THEN 
                   LET g_status.code = '-1'
    ...
    DECLARE sel_sgm_cur CURSOR FOR
      ...
      FOREACH sel_sgm_cur  INTO l_sgm03,l_sgm04,l_ta_sgm06
        IF l_ta_sgm06='Y' THEN #表明下一站是报工的 不能自动生成报工单
           EXIT FOREACH
        END IF
        LET l_tttt=p_tc_shb.tc_shb02[1,11]
        ...
        WHERE substr(tc_shb02,1,11)=l_tttt
        ...
        ELSE
           ...
           LET l_tmp = l_tmp USING '&&&&&&&&&'
        END IF
        LET p_tc_shb.tc_shb02=p_tc_shb.tc_shb02[1,11] CLIPPED,l_tmp
        #ly180103 add
        IF cl_null(p_tc_shb.tc_shb121) THEN LET p_tc_shb.tc_shb121=0 END IF 
        IF cl_null(p_tc_shb.tc_shb122）THEN LET p_tc_shb.tc_shb122=0 END IF
        #ly180103 edd
