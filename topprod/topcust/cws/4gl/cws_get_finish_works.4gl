...
#{
# Program name...: cws_get_finish_works.4gl
# Descriptions...: 获取完工资料
# Date & Author..: 2016/06/06 by guanyao
...
FUNCTION cws_get_finish_works()
    ...
    IF g_status.code = "0" THEN
       CALL cws_get_finish_works_process()
    END IF
...
#
#]
FUNCTION cws_get_finish_works_process()
    ...
    ELSE 
       ...
       ELSE 
          ...
          ELSE 
             ...
             ELSE 
                #备注
                SELECT instr(l_barcode,'|') INTO l_n FROM dual
                IF l_n > 0 THEN 
                   LET lst_token = base.StringTokenizer.create(l_barcode,"|")
                   LET l_n  = 1
                   ...
                   IF l_err = 'N' THEN 
                      ...
                      RETURN
                   ELSE 
                      LET l_return5.code = 0
                      LET l_return5.msg = '备注有资料'
                ...
                ELSE 
                   ...
                   ELSE 
                      ...
                      IF l_n >0 THEN 
                         SELECT '5',eca01,eca02 INTO l_return6.* FROM eca_file WHERE eca01  = l_barcode
                         LET l_node = aws_ttsrv_addMasterRecord(base.TypeInfo.create(l_return6), "Master")
                      ELSE
                      #end-----add by guanyao160718
                         LET g_status.code = -1
                         LET g_status.description = '没有开工资料'
