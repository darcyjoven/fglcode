...
FUNCTION cws_create_finish_work()
   ...
   IF  TIME>'6:45'  AND TIME<'7:00'  THEN 
       LET g_status.code = '-1'
    ...
    END IF 
     ...
     IF  TIME>'18:45'  AND TIME<'19:00'  THEN 
        LET g_status.code = '-1'
       LET g_status.description = '没有资料性质'
       LET l_return.code="-1"
       LET l_return.msg = "完工信息录入失败"
...
FUNCTION cws_create_finish_work_process()
    ...
    DEFINE l_sfbud06     LIKE sfb_file.sfbud06
    DEFINE l_ta_shm06    LIKE shm_file.ta_shm06
    DEFINE l_ta_ecd03    LIKE ecd_file.ta_ecd03
    DEFINE l_cmd       STRING
    ...
    DEFINE l_extime1 DATETIME YEAR TO SECOND
    DEFINE l_extime2 DATETIME YEAR TO SECOND
    DEFINE l_extime3 DATETIME YEAR TO SECOND
    ...
    DEFINE l_str1    STRING  #add by zhangsba201111
    DEFINE l_str2    STRING  #add by zhangsba201111
    #--------------------------------------------------------------------------#
    # 處理呼叫方傳遞給 ERP 的订單資料                                    #
    #--------------------------------------------------------------------------#
...
LET l_str2 = " 看走不走cws_create_finish_work_process   ",l_tc_shb01,"\n"
...
#add by zhangsba201111--e     
    ...
    #LET g_tc_shb.tc_ = aws_ttsrv_getParameter("tc_shb06")  #add by guanyao160620
    #IF cl_null(g_tc_shb.tc_) THEN   #add by guanyao160620
       LET g_status.code = '-1'
    ...
    ELSE 
       ...
       #SELECT sgm03 INTO l_tc_shb06 FROM sgm_file WHERE sgm01 = l_tc_shb03 AND sgm04 = g_tc_shb.tc_
    ...
    #   AND tc_shb11 = l_tc_shb11    同一工序可不同人报工  ly 180528
       AND tc_shb01 = '1'
  ...
  LET l_cnt1=0 
   SELECT count(*) INTO l_cnt1
    ...
    IF l_cnt1> 0 THEN 
       LET g_status.code = '-1'
       LET g_status.description = '不允许同一人完工'
       LET l_return.code="-1"
       LET l_return.msg = "不允许同一人完工,完工信息录入失败"
       CALL aws_ttsrv_addParameterRecord(base.TypeInfo.create(l_return))
       RETURN
    END IF 
    ...
    FROM sgm_file 
    WHERE sgm01=l_tc_shb03 AND sgm03=l_tc_shb06

  {  IF l_sgm04='F21010'  OR  l_sgm04='F21011' OR l_sgm04='F21060' THEN 
       IF l_tc_shb11<>'WX030' THEN 
          LET g_status.code = '-1'
          LET g_status.description = '化金不允许非WX030报工'
          LET l_return.code="-1"
          LET l_return.msg = "化金不允许非WX030报工"
          CALL aws_ttsrv_addParameterRecord(base.TypeInfo.create(l_return))
           RETURN
       END IF 
    END IF 
    ...
    INITIALIZE g_tc_shb.* TO NULL
      ...
      #tc_shb121,tc_shc122 处理  ly170103
     IF cl_null(g_tc_shb.tc_shb121) THEN 
        LET  g_tc_shb.tc_shb121=0
     END IF    
      IF cl_null(g_tc_shb.tc_shb122) THEN    
        LET  g_tc_shb.tc_shb122=0
     END IF
    ...
    #str----add by guanyao160617
       ...
       IF g_tc_shb.tc_shb01 = '2' THEN 
        #  SELECT SUM(tc_shb12+tc_shb121+tc_shb122) INTO l_tc_shb12_2 FROM tc_shb_file  ly180103 
       SELECT SUM(tc_shb12+nvl(tc_shb121,0)+nvl(tc_shb122,0)) INTO l_tc_shb12_2 FROM tc_shb_file
       ...
       END IF
    ###190701
      SELECT sgm02 INTO g_tc_shb.tc_shb04 FROM sgm_file WHERE sgm01=g_tc_shb.tc_shb03 and sgm03=g_tc_shb.tc_shb06

      SELECT sfbud05,sfbud06 INTO l_sfbud05,l_sfbud06 FROM sfb_file WHERE sfb01 = g_tc_shb.tc_shb04

      SELECT ta_shm06 INTO l_ta_shm06 FROM shm_file WHERE shm01=g_tc_shb.tc_shb03

       IF cl_null(l_sfbud05)   THEN LET l_sfbud05 = 'N' END IF
       IF cl_null(l_sfbud06)   THEN LET l_sfbud06 = 'N' END IF
       IF cl_null(l_ta_shm06 ) THEN LET l_ta_shm06  = 'N' END IF
       # 工单hold
          IF l_sfbud06='Y'    THEN
              LET g_status.code = '-1'
              LET g_status.description = '工单已hold不允许报工'
              LET l_return.code="-1"
              LET l_return.msg = "开工信息录入失败"
              CALL aws_ttsrv_addParameterRecord(base.TypeInfo.create(l_return))
          END IF

          IF  l_ta_shm06='Y'  THEN
              LET g_status.code = '-1'
              LET g_status.description = 'LOT已hold不允许报工'
              LET l_return.code="-1"
              LET l_return.msg = "开工信息录入失败"
              CALL aws_ttsrv_addParameterRecord(base.TypeInfo.create(l_return))
          END IF
      ...
      #   INTO g_tc_shb.tc_shb04,g_tc_shb.tc_shb05,g_tc_shb.tc_shb07,g_tc_shb.tc_
...
#add by zhangsba201111--s
LET l_str1 = " 观察走不走cws_create_finish_work_ins_sfp 账套  ",g_plant," l_sfbud05 ",l_sfbud05," l_x ",l_x," smaud02 ",g_sma.smaud02,"\n"
RUN " echo '"||l_str1||"' >> /u1/topprod/topcust/cws/4gl/asfi514_zhangsba_if_ins_sfp.log"
#add by zhangsba201111--e  
       #----------------------------------------------------------------------#
       # 執行單頭 INSERT SQL                                                  #
       #----------------------------------------------------------------------#
      #ly 20180103
      IF  cl_null(g_tc_shb.tc_shb121) THEN LET g_tc_shb.tc_shb121=0 END IF 
      IF  cl_null(g_tc_shb.tc_shb122) THEN LET g_tc_shb.tc_shb122=0 END IF 
      #ly 20180103 end 
      
      #pcs报工 pnl=0 强制完工数量为0 ly 180622
      SELECT  ta_ecd03  INTO l_ta_ecd03 FROM  ta_ecd_file WHERE ta_ecd01=g_tc_shb.tc_shb08
      #IF l_ta_ecd03<>'Y'  AND 
      IF  g_tc_shb.tc_shbud09=0 AND  g_tc_shb.tc_shb12>0 THEN 
         LET  g_tc_shb.tc_shb12=0 
      END IF
      #记录到秒的完工时间
       LET   g_tc_shb.tc_shbud12=l_extime1  
      ...
      IF cl_null(l_sfbud11) THEN  #属于精简报工的工单 
         ...
         FOREACH sel_tc_shc_cc_cur  INTO l_sgm03,l_sgm04,l_ta_sgm06,l_ecd07
           IF l_ta_sgm06='Y' THEN
              EXIT FOREACH
           END IF 
           SELECT ta_ecd04 INTO l_ta_ecd04 FROM ecd_file WHERE ecd01=l_sgm04  #判断下一站是否只有开工完工
           IF g_ta_ecd04='Y' THEN  #没有扫入扫出的 情况  此站的完工产生下一站的 扫入 开工 扫出 完工信息
              ...
              IF l_cnt=0 THEN
                # SELECT ta_ecd04 INTO l_ta_ecd04 FROM ta_ecd_file WHERE ecd01=l_sgm04  #判断下一站是否只有开工完工
                 IF l_ta_ecd04='N' OR cl_null(l_ta_ecd04) THEN  #下一站不存在开工完工的 情况
                    ...
                    IF cl_null(l_tc_shc.tc_shc02) THEN  #报名本站没有扫入扫出资料
                       select * into p_tc_shb.* from tc_shb_file where tc_shb03=g_tc_shb.tc_shb03 AND tc_shb06=g_tc_shb.tc_shb06
                 ...
                 LET l_tc_shc.tc_shc03=p_tc_shb.tc_shb03
                 LET l_tc_shc.tc_shc04=p_tc_shb.tc_shb04
                 LET l_tc_shc.tc_shc05=p_tc_shb.tc_shb05
                 LET l_tc_shc.tc_shc07=p_tc_shb.tc_shb07 
                 LET l_tc_shc.tc_shc10=p_tc_shb.tc_shb10
                 LET l_tc_shc.tc_shc11=p_tc_shb.tc_shb11
                 LET l_tc_shc.tc_shc13=p_tc_shb.tc_shb13
                 LET l_tc_shc.tc_shc14=p_tc_shb.tc_shb14
                 LET l_tc_shc.tc_shc15=p_tc_shb.tc_shb15
                 LET l_tc_shc.tc_shc16=p_tc_shb.tc_shb16
                 LET l_tc_shc.tc_shcud09=p_tc_shb.tc_shbud09
                 
                 #tianry add 170302tianry

                 LET l_ima01=l_tc_shc.tc_shc03 CLIPPED,'|',l_tc_shc.tc_shc06
                 #tianry add end 
                 LET g_prog='tc_shc01'
                 INSERT INTO tc_shc_file VALUES (l_tc_shc.*) 
                 IF STATUS OR SQLCA.SQLERRD[3]=0 THEN 
                    LET l_str="'",l_ima01,"' '",STATUS,"'  '",l_ima01,"' '",l_tc_shc.tc_shc02,"'  "
                    CALL writeStringToFile(l_str,l_logfile)
                    LET g_status.code = '-1'
                    LET g_status.description = 'INS tc_shc err'
                    LET g_success = 'N'
                ...
                #直接产生扫出资料
                 ...
                 #tianry add 170302tianry
                 LET g_prog='tc_shc1'
                 LET l_ima01=l_tc_shc1.tc_shc03 CLIPPED,'|',l_tc_shc1.tc_shc06
                 #tianry add end
             
                 INSERT INTO tc_shc_file VALUES (l_tc_shc1.*)
                 IF STATUS OR SQLCA.SQLERRD[3]=0 THEN
                    LET l_str="'",l_ima01,"' '",STATUS,"'  '",l_ima01,"' '",l_tc_shc1.tc_shc02,"'  "
                    CALL writeStringToFile(l_str,l_logfile)
                    LET g_status.code = '-1'
                    LET g_status.description = 'INS tc_shc err'
                    LET g_success = 'N' 
               ...
               END IF   #有扫入扫出结束    
                 ...
                 LET l_tc_shb.tc_shb121=0 
                 #tianry add 170302tianry

                 LET l_ima01=l_tc_shb.tc_shb03 CLIPPED,'|',l_tc_shb.tc_shb06
                 #tianry add end
                 LET g_prog='tc_shb'
                #add ly 180103
                IF cl_null(l_tc_shb.tc_shb121) THEN LET l_tc_shb.tc_shb121=0 END IF 
                IF cl_null(l_tc_shb.tc_shb122) THEN LET l_tc_shb.tc_shb122=0 END IF 
                # ly  end 180103 

                
                INSERT INTO tc_shb_file VALUES (l_tc_shb.*)
                 IF STATUS OR SQLCA.SQLERRD[3]=0 THEN
                    LET l_str="'",l_ima01,"' '",STATUS,"'  '",l_ima01,"' '",l_tc_shb.tc_shb02,"'  "
                    CALL writeStringToFile(l_str,l_logfile)
                    LET g_status.code = '-1'
                    LET g_status.description = 'INS tc_shc err'
                    LET g_success = 'N'
                 ...
                 #ly180126
                  #add ly 180126
                 INSERT INTO tc_shf_file(tc_shf01,tc_shf07,tc_shf08,tc_shf02) VALUES (l_tc_shb.tc_shb02,l_tc_shb.tc_shb03,l_tc_shb.tc_shb06,l_extime1)
                 #ly180103 add
                 IF cl_null(l_tc_shb.tc_shb121) THEN LET l_tc_shb.tc_shb121=0 END IF
                 IF cl_null(l_tc_shb.tc_shb122) THEN LET l_tc_shb.tc_shb122=0 END IF
                 ...
                 LET l_ima01=l_tc_shb1.tc_shb03 CLIPPED,'|',l_tc_shb1.tc_shb06
                 #tianry add end
                 LET g_prog='tc_shb1'
                 #add ly180103
                 #IF cl_null(l_tc_shb1.tc_shb121) THEN l_tc_shb1.tc_shb121=0 END IF 
                 #IF cl_null(l_tc_shb1.tc_shb122) THEN l_tc_shb1.tc_shb122=0 END IF
                 #add ly180103 end
                 INSERT INTO tc_shb_file VALUES (l_tc_shb1.*)
                 IF STATUS OR SQLCA.SQLERRD[3]=0 THEN 
                    LET l_str="'",l_ima01,"' '",STATUS,"'  '",l_ima01,"' '",l_tc_shb1.tc_shb02,"'  "
                    CALL writeStringToFile(l_str,l_logfile)
                    LET g_status.code = '-1'
                    LET g_status.description = 'INS tc_shc err'
                    LET g_success = 'N'            
              ...
              ELSE  #拆单的情况新增 累加更新
               IF l_ta_ecd04='N' OR cl_null(l_ta_ecd04) THEN  #下一站不存在开工完工的 情况
                 ...
                 WHERE tc_shc03=g_tc_shb.tc_shb03
                 AND tc_shc06=l_sgm03 AND ROWNUM=1  AND tc_shc01='1'  #扫入数量的增加
                 IF STATUS OR SQLCA.SQLERRD[3]=0 THEN
                    LET g_status.code = SQLCA.SQLCODE
                    LET g_status.sqlcode = SQLCA.SQLCODE
                    LET l_return.code="-1"
                 ...
                 UPDATE tc_shc_file SET tc_shc12=tc_shc12+g_tc_shb.tc_shb12,tc_shcud09=tc_shcud09+g_tc_shb.tc_shbud09    
                 WHERE tc_shc03=g_tc_shb.tc_shb03
                 AND tc_shc06=l_sgm03 AND ROWNUM=1  AND tc_shc01='2'  #扫入数量的增加
                 IF STATUS OR SQLCA.SQLERRD[3]=0 THEN
                    LET g_status.code = SQLCA.SQLCODE
                    LET g_status.sqlcode = SQLCA.SQLCODE
                    LET l_return.code="-1"
               ...
               END IF 
                 ...
                 UPDATE tc_shb_file SET tc_shb12=tc_shb12+g_tc_shb.tc_shb12,tc_shbud09=tc_shbud09+g_tc_shb.tc_shbud09
                 WHERE tc_shb03=g_tc_shb.tc_shb03
                 AND tc_shb06=l_sgm03 AND ROWNUM=1  AND tc_shb01='2'  #开工数量的增加
                 IF STATUS OR SQLCA.SQLERRD[3]=0 THEN                    LET g_status.code = SQLCA.SQLCODE
                    ...
                    EXIT FOREACH
                 END IF
                 SELECT * INTO l_tc_shb1.* FROM tc_shb_file WHERE tc_shb03=g_tc_shb.tc_shb03
                 AND tc_shb06=l_sgm03 AND ROWNUM=1  AND tc_shb01='2'
              END IF 
              
              #下一站报工资料产生
              LET g_tc_shb03=l_tc_shb1.tc_shb03   #g_argv1
              LET g_tc_shb08=l_tc_shb1.tc_shb08    #g_argv2
              LET g_tc_shb02=l_tc_shb1.tc_shb02
              LET g_tc_shb12=g_tc_shb.tc_shb12      #g_argv4
              CALL p740_try()
           END IF  
      ...
      #     LET g_tc_shb03=g_tc_shb.tc_shb03   #g_argv1
      #     LET g_tc_shb08=g_tc_shb.tc_shb08    #g_argv2
      #     LET g_tc_shb02=g_tc_shb.tc_shb02
      #     LET g_tc_shb12=g_tc_shb.tc_shb12      #g_argv4
       #    CALL p740_try()
        END FOREACH 
      END IF 
       ...
       END IF 
    #add ly 180126
    INSERT INTO tc_shf_file(tc_shf01,tc_shf07,tc_shf08,tc_shf02) VALUES (g_tc_shb.tc_shb02,g_tc_shb.tc_shb03,g_tc_shb.tc_shb06,l_extime1)
        #  tianry add 161129
...
DEFINE pp_tc_shb12     LIKE tc_shb_file.tc_shb12   #tianry add 170208
DEFINE l_str    STRING  #add by zhangsba201111   
    LET pp_tc_shb12=p_tc_shb12    #初始值复制  记录下总的数量  tianry add 170208
    DROP TABLE tmp_700                                                                                                             
...
{
   SELECT smyslip INTO l_t FROM (SELECT 'N',smyslip,smydesc,smyauno,smydmy5  FROM smy_file 
WHERE smyslip in ('103','104')  and 1=1 AND smyslip NOT IN (SELECT rye04 FROM rye_file WHERE ryeacti = 'Y'
}      
SELECT smyslip INTO l_t FROM (SELECT 'N',smyslip,smydesc,smyauno,smydmy5  FROM smy_file WHERE  1=1 
AND smyslip NOT IN  ('103','104') 
AND smyslip NOT IN (SELECT rye04 FROM rye_file WHERE ryeacti = 'Y' 
 ...
 AND upper(rye01) = 'ASF' AND rye04 IS NOT NULL) AND smyacti='Y' AND upper(smysys) ='ASF' AND smykind ='3' AND  (smy73 <> 'Y' OR smy73 is null) 
   ...
   ELSE
    ...
    END IF
#add by zhangsba201111--s
LET l_str = " 观察有没有进入并插入数据 账套  ",g_plant," 是否成功 ",g_success," 程序名 ",g_prog," sfp01 发料单号 ",l_sfp.sfp01," sfp06 ",l_sfp.sfp06,"\n"
RUN " echo '"||l_str||"' >> /u1/topprod/topcust/cws/4gl/create_finish_work_zhangsba_INSERT_sfpfile.log"
#add by zhangsba201111--e  
    ...
    FOREACH p700sub_sfs_cur INTO l_sfa.*    
        LET p_tc_shb12=pp_tc_shb12    #tianry add 170208  
       ...
       FOREACH img5_curs INTO p_sfs09,p_img10
          ...
          #tianry add 170118  库存单位换算
          LET p_img10=p_img10*l_fac
          #tianry add end 
          LET l_sfs.sfs07 = 'XBC'
...
#add by zhangsba201111--s
LET l_str = " 观察g_i的值 账套  ",g_plant," g_i ",g_i," 是否成功 ",g_success," 程序名 ",g_prog," sfp01 发料单号 ",l_sfp.sfp01," sfp06 ",l_sfp.sfp06,"\n"
RUN " echo '"||l_str||"' >> /u1/topprod/topcust/cws/4gl/create_finish_work_zhangsba_DELETE_sfpfile.log"
#add by zhangsba201111--e          
      END IF  
...
RUN " echo '"||l_str||"' >> /u1/topprod/topcust/cws/4gl/create_finish_work_zhangsba_i501sub_y_upd.log"
#add by zhangsba201111--e               
          IF g_success = 'Y' THEN 
...
DEFINE p_row,p_col   LIKE type_file.num5          #No.FUN-680136 SMALLINT
 ...
 {   LET l_sql = "SELECT tc_shb03,tc_shb04,tc_shb05,tc_shb06,tc_shb07,tc_shb09,",
    ...
    PREPARE p730_prepare FROM l_sql
     ...
     #    FROM tc_shb_file WHERE tc_shb01 = '1'  AND tc_shb03=g_tc_shb03 AND  tc_shb08=g_tc_ AND  tc_shb17 IS NULL
   ...
   #      FOREACH p730_curs2 INTO l_tc_shb.*
         ...
         #   END IF 
            ...
            IF cl_null(l_shb.shb032) THEN LET l_shb.shb032=0 END IF
            IF cl_null(l_shb.shb033) THEN LET l_shb.shb033=0 END IF 
          ...
          #  LET l_shb.shb111  = l_tc_shb12
           ...
           END IF 
           #tianry add 170114
           SELECT tc_shb121 INTO l_shb.shb112 FROM tc_shb_file WHERE tc_shb02=g_tc_shb02
           ...
           #tianry add end 
            ...
            IF STATUS THEN 
               #tianry add 170213 
               IF STATUS=-268 THEN   #出现抢单号的情况  重新抓取一遍单号
                  LET l_sql=" select smyslip  from smy_file  sample(1) where smysys='asf' AND smykind='9' ",
                            " and   rownum=1   AND smyslip IS NOT NULL "
                  PREPARE  ss_ee_pre444 FROM l_sql
                  DECLARE ss_ee_cur444 CURSOR for ss_ee_pre4
                  OPEN ss_ee_cur444
                  FETCH ss_ee_cur444 INTO l_str
                  CLOSE ss_ee_cur444
                  IF cl_null(l_str) THEN LET l_str='MRZ'  END IF    #出现空的报工单别
                  LET l_str=l_str CLIPPED,'-'
                  LET l_str=l_str CLIPPED,l_year CLIPPED,l_month CLIPPED,l_day CLIPPED
                  ...
                  INSERT  INTO shb_file VALUES (l_shb.*)
                  IF STATUS THEN
                     LET l_str="'",g_prog,"' '",STATUS,"'  '",TIME,"' '",l_shb.shb01,"'  "
                     CALL writeStringToFile(l_str,l_logfile)
                     LET g_status.code = '-1'
     ...
     {   LET l_shb1.* =l_shb.*       
      ...
      FOREACH sel_sgm_cur  INTO l_sgm03,l_sgm04,l_ta_sgm06 
        ...
        LET l_shb1.1=l_sgm04
        LET l_shb1.shb03=l_shb1.shb02
        LET l_shb1.shb031=l_shb1.shb021
        LET l_shb.shb032 =0
        LET l_shb.shb033 = 0
        SELECT ecd07 INTO l_shb1.shb07 FROM ecd_file WHERE ecd01=l_shb1.1
        #tianry add 161225
        ...
        #tianry add end 
        #ly180103 add
        IF cl_null(l_shb1.shb112) THEN LET l_shb1.shb112=0 END IF 
        IF cl_null(l_shb1.shb113) THEN LET l_shb1.shb113=0 END IF
        #ly180103 end   
...
#         EXIT PROGRAM
      ...
      #      WHERE tc_shb01='1'   #tianry add 161212
         ...
         IF cl_null(l_tc_shb14_max) OR cl_null(l_tc_shb15_max) THEN 
            ...
            RETURN
         END IF 
        # SELECT SUM(tc_shb12+tc_shb121+tc_shb122) INTO l_tc_shb12_sum FROM tc_shb_file WHERE tc_shbud02 = l_tc_shb02  #ly180103
SELECT SUM(tc_shb12+nvl(tc_shb121,0)+nvl(tc_shb122,0)) INTO l_tc_shb12_sum FROM tc_shb_file WHERE tc_shbud02 = l_tc_shb02
...
#          CALL cl_err('','axm-774',0)
    ...
    DELETE FROM shb1_temp2
      LET l_sql=" SELECT distinct shb01,shb05,shb16,1,shb06 FROM shb1_temp ORDER BY shb05,shb16,shb06 ASC "
...
#            CALL cl_err('foreach:',SQLCA.sqlcode,1)
      ...
      #   BEGIN WORK  #mark by ly170915 事务重理
         CALL t730sub_y_chk(l_shb01,TRUE)
             ...
             IF g_success='Y' THEN
              #  BEGIN WORK  #mark by ly 170915 事务重理
                IF NOT cl_null(g_shb32) THEN
...
FUNCTION p002_b_fill_try()
    ...
    LET g_wc= " shb16='",g_tc_shb03,"' "   # AND 1='",g_tc_shb06,"' " 

   #tianry add end 
   LET g_sql = "SELECT distinct 'Y',shb01,shb16,shb05,shb04,gen02,shb02,shb03,1,2,shb06,shb10,ima02,ima021,shb111 ",
