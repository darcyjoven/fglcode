...
DEFINE
    ...
    g_tc_imb           DYNAMIC ARRAY OF RECORD    #程式變數(Program Variables)
        sgm03               LIKE sgm_file.sgm03,          #工序号     
        tc_imb03            LIKE tc_imb_file.tc_imb03,    #本站作业编号
        ecd02               LIKE ecd_file.ecd02,          #本站作业说明 add by gujq 20160905
        eca01               LIKE eca_file.eca01,          #本站工作中心
        eca02               LIKE eca_file.eca02,          #本站工作说明
        sgm03_1             LIKE sgm_file.sgm03,          #下站作业工序
        ecd01_1             LIKE ecd_file.ecd01,          #下站作业编码
        ecd02_1             LIKE ecd_file.ecd02,          #下站作业说明
        eca01_1             LIKE eca_file.eca01,          #下站工作编码
        eca02_1             LIKE eca_file.eca02,          #下站工作说明
        ...
        sfb06               LIKE sfb_file.sfb06,          #版本号
        ima02             LIKE ima_file.ima02,            #品名     add by gujq 20160905
        ima021            LIKE ima_file.ima021,           #规格     add by gujq 20160905
        tc_imb06            LIKE tc_imb_file.tc_imb06,    #昨日结存
        tc_imb07            LIKE tc_imb_file.tc_imb07,    #白班入站
        tc_imb08            LIKE tc_imb_file.tc_imb08,    #白班出站
        tc_imb09            LIKE tc_imb_file.tc_imb09,    #晚班入站
        tc_imb11            LIKE tc_imb_file.tc_imb11,    #晚班出站
        tc_imb12            LIKE tc_imb_file.tc_imb12,    #日报废量
        tc_imb10            LIKE tc_imb_file.tc_imb10     #WIP
...
# DEFINE      l_time    LIKE type_file.chr8            #No.FUN-6A0068
   ...
   # DROP TABLE cecq022_tmp #add by guanyao160930
...
#  ELSE
   ...
   CALL cl_set_head_visible("folder01","YES")    #No.FUN-6B0029
   INPUT BY NAME
        g_tc_ima.tc_imaud03 ,g_tc_ima.tc_imaud02  #add by guanyao160923
        WITHOUT DEFAULTS

    BEFORE INPUT
       LET g_before_input_done = TRUE
       LET g_tc_ima.tc_imaud03='3' 
       LET g_tc_ima.tc_imaud02='3'
       DISPLAY BY NAME g_tc_ima.tc_imaud03 
       DISPLAY BY NAME g_tc_ima.tc_imaud02 
...
IF g_time>'07:00:00' and g_time<'19:00:00'   then 
    LET g_sql=" SELECT sgm03,sgm04,ecd02,eca01,eca02,sgm03_1,'' ecd01_1,'' ecd01_2 , ",
              " '' eca01_1,'' eca02_1,sgm01,sgm02,sgm03_par,sfb06,ima02,ima021,",
              " l_wipqty,b11,b21,w11,w21,w33,wipqty,''  ",
            #  " FROM v_ddwip    WHERE wipqty!=0 AND ",g_wc CLIPPED
              " FROM v_ddwip    WHERE 1=1 AND  ",g_wc CLIPPED
             #  " FROM yang_tmp   WHERE 1=1 AND ",g_wc CLIPPED
else 
  LET g_sql=" SELECT sgm03,sgm04,ecd02,eca01,eca02,sgm03_1,'' ecd01_1,'' ecd01_2 , ",
              " '' eca01_1,'' eca02_1,sgm01,sgm02,sgm03_par,sfb06,ima02,ima021,",
              " l_wipqty,b11,b21,w11,w21,w33,wipqty,''  ",
            #  " FROM v_ddwip    WHERE wipqty!=0 AND ",g_wc CLIPPED
              " FROM v_ddwip1    WHERE 1=1 AND  ",g_wc CLIPPED
             #  " FROM yang_tmp   WHERE 1=1 AND ",g_wc CLIPPED
   ...
   END IF  
     ...
     IF g_tc_ima.tc_imaud02 = '1' THEN
      LET g_sql = g_sql CLIPPED," AND  substr(sgm03_par,7,1)='A' "
   END IF
   IF g_tc_ima.tc_imaud02 = '2' THEN
      LET g_sql = g_sql CLIPPED," AND substr(sgm03_par,7,1)='F'  "
   END IF  
   
   IF g_tc_ima.tc_imaud02 = '3' THEN
      LET g_sql = g_sql CLIPPED," AND substr(sgm03_par,7,1)='R'  "
   END IF
    ...
    FOREACH tc_imb_curs1 INTO g_tc_imb[l_ac].*   #單身 ARRAY 填充
       IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
         SELECT ecd01,ecd02,eca01,eca02
         INTO g_tc_imb[l_ac].ecd01_1 ,g_tc_imb[l_ac].ecd02_1 ,
              g_tc_imb[l_ac].eca01_1 ,g_tc_imb[l_ac].eca02_1 
         FROM ecd_file,eca_file,sgm_file  
         WHERE ecd01 =sgm04 AND ecd07=eca01
              AND sgm03=g_tc_imb[l_ac].sgm03_1 
              AND sgm01=g_tc_imb[l_ac].tc_imb04
...
DEFINE
   ...
   # LET g_sql = "SELECT sgm02,sgm04,sfb05,ima02,ima021,sum(l_sum7) FROM cecq022_tmp ",
...
DEFINE
  ...
  #  LET g_sql = "SELECT sfb05,sgm04,ima02,ima021,sum(l_sum7) FROM cecq022_tmp ",
      LET g_sql = "SELECT sgm03_par,sgm04,ima02,ima021,sum(wipqty) FROM yang_tmp ",
                " GROUP BY  sgm03_par,sgm04,ima02,ima021 ORDER BY 1"
...
DEFINE
  ...
  #  LET g_sql = "SELECT sfb05,ima02,ima021,sgm06,sum(l_sum7) FROM cecq022_tmp  ",
     LET g_sql = "SELECT sgm03_par,ima02,ima021,sgm06,sum(wipqty) FROM yang_tmp  ",
                " GROUP BY  sgm03_par,ima02,ima021,sgm06 "
...
DEFINE l_sql1 STRING
    DROP TABLE cecq022_tmp
    CREATE TEMP TABLE cecq022_tmp
...
select a.sgm01 ,a.sgm02,a.sgm03_par,a.sgm03,a.sgm03_1,sgm04,
----昨日结存 -2 wipqty
nvl( b.wipqty,0) l_wipqty,  
---白班入  
nvl(c.sgm301,0)-nvl(b.sgm301,0)  b11,
---白班出
nvl(c.sgm311,0)-nvl(b.sgm311,0)  b21,
---晚班入
nvl(d.sgm301,0)-nvl(c.sgm301,0)  w11,
---晚班出 
nvl(d.sgm311,0)-nvl(c.sgm311,0)  w21,
---报废
nvl(d.sgm313,0)-nvl(b.sgm313,0)  W33,
--- 今日结存
nvl(d.wipqty,0)  wipqty 
from 

(
select unique sgm01,sgm02,sgm03,sgm03_par,sgm03_1,sgm03_2,sgm04
from 
(
select * from wiphis
where l_day=to_char(sysdate-1,'yyyy-mm-dd')  
---and l_type1='02'
union
select * from yang_tmp
)  a
---- -2 D-1 晚    --wipqty  昨日结存  D-1  7:00 数据
left join 
(
select l_day,l_type1,sgm01,sgm03,sgm301,sgm302,sgm303,sgm304,sgm311,sgm312,sgm313,sgm314,sgm315,sgm316,sgm317 ,wipqty
from wiphis
where  l_day=to_char(sysdate-1,'yyyy-mm-dd')  and l_type1='02' 
) b on a.sgm01=b.sgm01 and a.sgm03=b.sgm03



----- -1  D-1 白班   D-1 19：00 数据
left join 
(
select l_day,l_type1,sgm01,sgm03,sgm301,sgm302,sgm303,sgm304,sgm311,sgm312,sgm313,sgm314,sgm315,sgm316,sgm317 ,wipqty
 from wiphis
where l_day=to_char(sysdate-1,'yyyy-mm-dd')  and l_type1='01' 
) c on a.sgm01=c.sgm01 and a.sgm03=c.sgm03
...
select  l_day,l_type1,sgm01,sgm03,sgm301,sgm302,sgm303,sgm304,sgm311,sgm312,sgm313,sgm314,sgm315,sgm316,sgm317 ,wipqty
 from yang_tmp
) d on a.sgm01=d.sgm01 and a.sgm03=d.sgm03
...
where d.wipqty>0
   ...
   IF g_tc_ima.tc_imaud03 = '2' THEN
      LET l_sql = l_sql CLIPPED," AND instr(sgm03_par, '-') >0"
   END IF  
   LET l_sql1 = " INSERT INTO cecq022_tmp ",l_sql CLIPPED
