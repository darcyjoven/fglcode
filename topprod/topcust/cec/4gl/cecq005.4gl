...
FUNCTION q005_table()
  ...
  CREATE TEMP TABLE cecq005_tmp_sum_1(
        shm01       LIKE shm_file.shm01,
        shm012      LIKE shm_file.shm012,
        sgm03_par   LIKE sgm_file.sgm03_par, 
        ima02_2     LIKE ima_file.ima02, 
...
#str---add by huanlf160810
FUNCTION q005_get_sum_1()
   DEFINE l_wc     STRING
   DEFINE l_sql    STRING
   DEFINE l_sql1   STRING 
   

   DELETE FROM cecq005_tmp_sum_1
   #CALL g_sfb_1_attr.clear()
#str----add by huanlf160810
   LET l_sql1 = "SELECT distinct shm01,shm012,sgm03_par,ima02,ima021,shm06,shm08,shm09,sgm03,sgm06,eca_file.eca02,sgm04,sgm45,sgm65,0,0,0,",
                ...
                "       NVL(ecm52,'N'),NVL(ecm53,'N'),ecm55,ecm56,imaud10 ",
                "  FROM ecm_file LEFT JOIN eca_file ON eca01 = ecm06,",
                "       sgm_file LEFT JOIN ima_file ON ima01 = sgm03_par,cecq005_tmp,shm_file",
                " WHERE ecm01 = shm012",    
                "   AND ecm04 = tc_shc08",
                "   AND ecm01 = tc_shc04", 
                "   AND sgm01 = shm01",
    ...
    LET l_sql = "INSERT INTO cecq005_tmp_sum_1 ",l_sql1 CLIPPED

    
    
    PREPARE q005_ins_sum_1 FROM l_sql
    EXECUTE q005_ins_sum_1

    #良品转入量
    LET l_sql = " MERGE INTO cecq005_tmp_sum_1 o ",
    ...
    PREPARE q005_sum1_1 FROM l_sql
    EXECUTE q005_sum1_1
    ...
    PREPARE q005_sum2_1 FROM l_sql
    EXECUTE q005_sum2_1
    ...
    PREPARE q005_sum3_1 FROM l_sql
    EXECUTE q005_sum3_1
    ...
    PREPARE q005_sum4_1 FROM l_sql
    EXECUTE q005_sum4_1

    LET l_sql = "update cecq005_tmp_sum_1 set wipqty_1 =NVL(ecm301_1-ecm311_1-ecm313_1,0) "
    PREPARE q005_sum5_1 FROM l_sql
    EXECUTE q005_sum5_1

    LET l_sql = "SELECT * FROM cecq005_tmp_sum_1 ORDER BY shm01,shm012,sgm03"

              
   PREPARE q005_pb_1 FROM l_sql
   DECLARE q005_curs1_1 CURSOR FOR q005_pb_1
   FOREACH q005_curs1_1 INTO g_shm_1[g_cnt].*
      IF SQLCA.sqlcode THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
...
FUNCTION q005_detail_fill_1(p_ac)
   DEFINE p_ac         LIKE type_file.num5,
          l_sfb24      LIKE sfb_file.sfb24,
          l_type       LIKE type_file.chr1,   
          l_sql        STRING, 
   ...
   PREPARE cecq005_pb_detail_1 FROM l_sql
   DECLARE sfb_curs_detail_1  CURSOR FOR cecq005_pb_detail_1        #CURSOR
   CALL g_sfb.clear()
   CALL g_sfb_attr.clear()
   LET g_cnt = 1
   LET g_rec_b = 0
