...
MAIN
   ...
   DISPLAY "g_rpt_name:",g_rpt_name
    ...
    LET g_sql ="ecu01.ecu_file.ecu01,",
               "ecu11.ecu_file.ecu11,",
               "l_chr.type_file.chr50,",
               ...
               "ecb40.ecb_file.ecb40,",
               "ecbud04.ecb_file.ecbud04,",

               "ecbud05.ecb_file.ecbud05,",
               "ecbud03.ecb_file.ecbud03,",
               "ecbud02.ecb_file.ecbud02,",
               ...
               "imaud09.ima_file.imaud09,",
               "ecbud17.ecb_file.ecbud17"
               
   LET l_table = cl_prt_temptable('cecr001',g_sql) CLIPPED
...
FUNCTION cecr001_tm(p_row,p_col)
   ...
   OPEN WINDOW cecr001_w AT p_row,p_col WITH FORM "cec/42f/cecr001"
...
#            LET tm.wc = cl_replace_str(tm.wc, "'", "\"")
            LET l_cmd = l_cmd CLIPPED,       
                        ...
                        " '",g_template CLIPPED,"'",           
                         " '",g_rpt_name CLIPPED,"'"           
            CALL cl_cmdat('cecr001',g_time,l_cmd)
         END IF
...
FUNCTION cecr001()
   DEFINE sr        RECORD
               ...
               ecu11    LIKE ecu_file.ecu11,
               l_chr    LIKE type_file.chr50,
               ...
               ecu02     LIKE ecu_file.ecu02,    #add by huanglf161018
  ...
  #公司全名zo02、公司地址zo041、公司電話zo05、公司傳真zo09
   ...
   SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'cecr001'  
   ...
   LET l_sql = "SELECT ecu01,'','',trim(imaud07),trim(imaud10),ecb03,ecb06,ecb17,ecb40,trim(ecbud04),trim(ecbud05),trim(ecbud03),trim(concat(concat(ecbud02,'    '),ecbud17)) as ecbud02,ima02,'','',imaud06,ecu02,imaud09,concat(ecbud02,ecbud17) as ecbud17",
               "  FROM ecb_file,ecu_file LEFT JOIN ima_file ON ima01=ecu01 ",
   ...
   FOREACH decl_aaa INTO sr.*
     ...
     IF l_x = 0 THEN 
      SELECT ima021 INTO sr.title FROM ima_file WHERE ima01=sr.ecu01
        LET sr.title = sr.title CLIPPED,'流程单'
        LET sr.title1 = '客户料号'
     END IF 
   ...
   CALL cl_prt_cs3('cecr001','cecr001',l_sql,l_str)
