...
#
# Pattern name...: cecq021.4gl
# Descriptions...: 定时执行WIP获取数据 查询
...
DEFINE
    ...
    g_tc_imb           DYNAMIC ARRAY OF RECORD    #程式變數(Program Variables)
        ...
        tc_imb14            LIKE tc_imb_file.tc_imb12,    #返修
...
# DEFINE      l_time    LIKE type_file.chr8            #No.FUN-6A0068
   ...
   LET g_argv2=ARG_VAL(2)   #執行功能   #FUN-7C0050
    ...
    OPEN WINDOW t001_w33 AT 2,2 WITH FORM "cec/42f/cecq021"
...
DEFINE
   #p_wc1           VARCHAR(200) #TQC-630166     
    ...
    DEFINE l_tc_imb14    LIKE tc_imb_file.tc_imb12
    DEFINE  l_tc_snb09a, l_tc_snb09b  LIKE tc_imb_file.tc_imb12
    #end----add by guanyao160923 
   ...
   # LET g_sql=" SELECT sgm03,sgm04,ecd02,sgm01,sgm02,sfb05,   '','',l_sum1,l_sum2,l_sum3,l_sum6,l_sum4,l_sum5,l_sum7,l_sum10 FROM cecq021_tmp  WHERE l_sum7!=0"
    ...
    #LET g_sql=" SELECT sgm03,sgm04,ecd02,sgm01,sgm02,   '','',
 
    LET g_sql=" SELECT sgm03,sgm04,ecd02,sgm03_1,'' ecd01_1,'' ecd01_2 , ",
              " eca01,eca02,sgm01,sgm02,sgm03_par,sfb06,a.ima02,a.ima021,",
              " l_sum1,l_sum2,l_sum3,l_sum6,l_sum4,l_sum5,0, wipqty,b.imaud10  ",
              " FROM yang_tmp a left join ima_file b on sgm03_par=ima01  WHERE wipqty!=0 AND ",g_wc CLIPPED
   ...
   END IF
    ...
    FOREACH tc_imb_curs1 INTO g_tc_imb[l_ac].*   #單身 ARRAY 填充
       IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
         SELECT ecd01,ecd02 INTO g_tc_imb[l_ac].ecd01_1 ,g_tc_imb[l_ac].ecd02_1 
         FROM ecd_file,sgm_file  
         WHERE ecd01 =sgm04 AND sgm03=g_tc_imb[l_ac].sgm03_1 AND sgm01=g_tc_imb[l_ac].tc_imb04

     ### cect324 
let l_tc_snb09a=0 
select sum(tc_snb09) into l_tc_snb09a
from tc_snb_file ,tc_sna_file 
where tc_sna01=tc_snb01  and tc_snapost='Y' 
### and  tc_snb06='维修站' 
and tc_snb20=g_tc_imb[l_ac].tc_imb04
and tc_snb05=g_tc_imb[l_ac].tc_imb03
if cl_null(l_tc_snb09a) then let l_tc_snb09a=0 end if 

### cect325
let l_tc_snb09b=0 
select sum(tc_snb09)  into l_tc_snb09b
from tc_snb_file ,tc_sna_file 
where tc_sna01=tc_snb01  and tc_snapost='Y' 
### and tc_snb05='维修站'
and tc_snb20=g_tc_imb[l_ac].tc_imb04
and tc_snb06=g_tc_imb[l_ac].tc_imb03

if cl_null(l_tc_snb09b) then let l_tc_snb09b=0 end IF 

 #      SELECT ima02,ima021 INTO g_tc_imb[l_ac].ima02,g_tc_imb[l_ac].ima021 FROM ima_file WHERE ima01 = g_tc_imb[l_ac].sfb05 
       LET g_tc_imb[l_ac].tc_imb14=l_tc_snb09b-l_tc_snb09a
...
DEFINE l_sql1 STRING
   ...
   IF cl_null(g_wc) THEN LET  g_wc=' 1=1 ' END IF
   LET l_sql ="SELECT sgm03_par,sgm01,sgm02,sgm03,sgm03_1,sgm03_2,sgm04,sgm65,wipqty,sgm06,ima02,ima021,ecd02, l_sum1,l_sum2,l_sum3,l_sum4,l_sum5,l_sum7,l_sum8,l_sum9,imaud10  ",   
              "  FROM yang_tmp left join ima_file on sgm03_par=ima01",
              "   WHERE   ",g_wc CLIPPED
