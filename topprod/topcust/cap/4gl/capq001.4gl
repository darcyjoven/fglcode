      ...
      END CONSTRUCT
         ...
         ON ACTION controlp
            CASE
                ...
                CALL cl_init_qry_var()
                  LET g_qryparam.state = 'c'
                  LET g_qryparam.form ="q_rvv4"   #MOD-920024 q_pmc2-->q_pmc1
                  CALL cl_create_qry() RETURNING g_qryparam.multiret
...
DEFINE l_apb09        LIKE apb_file.apb09
   ...
   LET g_sql = " SELECT rvw02,rvw01,rvv06,pmc081,rvw18,rvw08,rvw09, ",
               "        rvw10,rvw17*rvw12,rvw05,apb09,apb081,apb101,apa00 ",
               "   FROM rvw_file,rvv_file,pmc_file,apa_file,apb_file ",
               "  WHERE rvw08 = rvv01 ",
               "    AND rvw09 = rvv02 ",
               "    AND rvw18 = apa01(+) ",
               "    AND rvw18 = apb01(+) ",
   ...
   FOREACH rvw_cs INTO g_rvw[g_cnt].*   #單身 ARRAY 填充
       ...
       IF l_apa00 MATCHES '2*' THEN
          LET g_rvw[g_cnt].apb09 = g_rvw[g_cnt].apb09*-1
          LET g_rvw[g_cnt].apb101= g_rvw[g_cnt].apb101*-1
       END IF 
       ...
       IF cl_null(g_rvw[g_cnt].rvw18) THEN
          LET g_rvw[g_cnt].apb09 = ''
          LET g_rvw[g_cnt].apb101= ''
       END IF
   ...
   INITIALIZE g_rvw[g_cnt].* TO NULL
   IF g_cnt > 1 THEN
