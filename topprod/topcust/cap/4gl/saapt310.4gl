...
DEFINE l_n2      LIKE aeh_file.aeh11     #add by liyjf170330
 ...
 DEFINE l_g3       LIKE  aph_file.aph05f  # add by zhangsba191010
       ...
       #-----------------MOD-BC0103--------------------------------------start
            IF l_ac > 0 THEN
             ...
             #mark by zhangsba190904---e  
               ...
               IF l_g1 IS NULL THEN LET l_g1 = 0 END IF
               IF l_g2 IS NULL THEN LET l_g2 = 0 END IF
               #add by zhangsba191010---s
               SELECT aph05f INTO l_g3 FROM aph_file,apf_file   #当前项次的金额不计算
               WHERE aph01 = apf01 AND aph08 = g_aph[l_ac].aph08 
                  AND apf41 = 'N'  AND apf01 = g_apf.apf01
                  AND aph02 = g_aph[l_ac].aph02
               #add by zhangsba191010---e 
               IF l_mm =1 THEN
                  LET l_mm = 12 
                  LET l_yy =l_yy - 1
               ELSE 
               ...
               #IF l_nmp06 + l_d1 + l_f1 - l_c1 - l_e1 - l_g1 - g_aph[l_ac].aph05f < 0 THEN                     #mark by zhangsba190904
               #   CALL cl_err_msg("","cnm-001",g_aph[l_ac].aph05f||"|"||l_nmp06+l_d1+l_f1-l_c1-l_e1-l_g1,0)    #mark by zhangsba190904
               IF l_nmp06 + l_d1 - l_c1 - l_g1 + l_g3 - g_aph[l_ac].aph05f < 0 THEN                         #add by zhangsba190904  #add by zhangsba191010 add + l_g3
                  CALL cl_err_msg("","cnm-001",g_aph[l_ac].aph05f||"|"||l_nmp06+l_d1-l_c1-l_g1+l_g3,0)    #add by zhangsba190904       #add by zhangsba191010 add + l_g3
                  NEXT FIELD aph05f
               END IF   
               END IF # add by zhangsba190828
             END IF 
...
#No.FUN-A40003 --end
      ...
      # LET g_sql = "UPDATE ",g_dbs_new CLIPPED,"apa_file ",                   #FUN-A50102 mark
       ...
       #END IF
        ...
        END IF
        EXECUTE t310_str4 USING l_amtf2,l_amt2,l_amt3,m_apg.apg04
        IF SQLCA.SQLCODE OR SQLCA.SQLERRD[3] = 0  THEN   
       ...
       #更新apc_file
         # LET g_sql = "UPDATE ",g_dbs_new CLIPPED,"apc_file ",              #FUN-A50102 mark
          ...
          #yinhy131210   --Begin
           ...
           EXECUTE t310_str4_apc USING l_amtf4,l_amt4,l_apc13,m_apg.apg04,m_apg.apg06  
           IF SQLCA.sqlcode OR SQLCA.SQLERRD[3]=0  THEN   #CHI-850023    
             #CALL cl_err(m_apg.apg04,SQLCA.SQLCODE,1) #CHI-A60034 mark
              #CHI-A60034 add --start--
              IF u_sw = 1 THEN
      ...
      FOREACH t310_bu_c2 INTO m_aph.*
        ...
        #LET g_plant_new = m_aph.aph08   #MOD-B10147 mark 
         ...
         IF g_apz.apz27 = 'N' THEN
            LET g_sql = "SELECT apa34f,apa34,apa00",
                      # "  FROM ",g_dbs_new CLIPPED,"apa_file",                 #FUN-A50102 mark
                        "  FROM ",cl_get_target_table(g_plant_new,'apa_file'),  #FUN-A50102
                        " WHERE apa01=?  AND apa41 = 'Y'",
                        "   AND apa02 <= '",g_apf.apf02 CLIPPED,"'"         #MOD-A30146 add
         ELSE
            LET g_sql = "SELECT apa34f,apa35+apa73,apa00",
                      # "  FROM ",g_dbs_new CLIPPED,"apa_file",                 #FUN-A50102 mark
                        "  FROM ",cl_get_target_table(g_plant_new,'apa_file'),  #FUN-A50102
                        " WHERE apa01=?  AND apa41 = 'Y'",
                        "   AND apa02 <= '",g_apf.apf02 CLIPPED,"'"         #MOD-A30146 add
         ...
         FETCH s3_aph_cur INTO l_apa34f,l_apa34,l_apa00
         IF SQLCA.sqlcode THEN
            LET l_apa34f = 0 LET l_apa34 = 0
         ELSE
            IF u_sw=1 THEN
              #IF (l_apa34f < (l_amtf+t1+t3+t10) ) OR (l_apa34  < (l_amt +t2+t4+t9)) THEN   #TQC-6C0044 #MOD-C90082 mark
               IF (l_apa34f < (l_amtf + t1 + t3 + t10 + t11) )                              #MOD-C90082 add t11
                OR (l_apa34  < (l_amt + t2 + t4 + t9 + t12)) THEN                           #MOD-C90082 add t12
       ...
       # LET g_sql = "UPDATE ",g_dbs_new CLIPPED,"apa_file ",               #FUN-A50102 mark
         ...
         EXECUTE s4_aph_cur USING l_amtf,l_amt,l_amt3,m_aph.aph04
         IF SQLCA.sqlcode OR SQLCA.SQLERRD[3]=0 THEN   #CHI-850023
        ...
        #更新apc_file
         IF NOT cl_null(m_aph.aph17) THEN
            IF g_apz.apz27 = 'N' THEN
               LET g_sql = "SELECT apc08 ,apc09,apa00",
                         # "  FROM ",g_dbs_new CLIPPED,"apa_file ,",                    #FUN-A50102 mark     
                         #           g_dbs_new CLIPPED,"apc_file  ",                    #FUN-A50102 mark  
                           "  FROM ",cl_get_target_table(g_plant_new,'apa_file'),",",   #FUN-A50102
                                     cl_get_target_table(g_plant_new,'apc_file'),       #FUN-A50102
                           ...
                           "   AND apa02 <= '",g_apf.apf02 CLIPPED,"'"         #MOD-A30146 add
            ELSE
               LET g_sql = "SELECT apc08,apc11+apc13,apa00",
                         # "  FROM ",g_dbs_new CLIPPED,"apa_file ,",                  #FUN-A50102 mark
                         #           g_dbs_new CLIPPED,"apc_file  ",                  #FUN-A50102 mark
                           "  FROM ",cl_get_target_table(g_plant_new,'apa_file'),",", #FUN-A50102
                                     cl_get_target_table(g_plant_new,'apc_file'),     #FUN-A50102   
            ...
            FETCH s3_aph_cur2 INTO l_apc08 ,l_apc09,l_apa00  #No.MOD-740413
            IF SQLCA.sqlcode THEN
               LET l_apc08  = 0 LET l_apc09 = 0
            ELSE
               IF u_sw=1 THEN
                 #IF (l_apc08  < (l_amtf5+t5+t7+t10) ) OR (l_apc09  < (l_amt5+t6+t8+t9)) THEN  #No.FUN-7B0055 #MOD-C90082 mark 
                  IF (l_apc08  < (l_amtf5 + t5 + t7 + t10 + t11) )                       #MOD-C90082 add t11
                   OR (l_apc09  < (l_amt5 + t6 + t8 + t9 + t12)) THEN                    #MOD-C90082 add t12
                    #CALL cl_err('','aap-250',1) #CHI-A60034 mark
                     CALL s_errmsg('','','','aap-250',1) #CHI-A60034
                     LET g_success='N'
                  END IF
          ...
          # LET g_sql = "UPDATE ",cl_get_target_table(g_plant_new,'apc_file'),",", #FUN-A50102  #TQC-AC0292
            ...
            IF m_aph.aph03 != '8' THEN   #MOD-910184 add
               ...
               END IF
            END IF   #MOD-910184 add
            EXECUTE s4_aph_cur2 USING l_amtf5,l_amt5,l_apc13,m_aph.aph04,m_aph.aph17    #No.FUN-690080
            IF SQLCA.sqlcode OR SQLCA.SQLERRD[3]=0 THEN   #CHI-850023
...
FUNCTION t310_undo_carry_voucher() 
  ...
  DEFINE l_dbs      STRING
  #add by zhangsba191104---s
  DEFINE l_yy       LIKE  type_file.num5
  DEFINE l_mm       LIKE  type_file.num5
  DEFINE l_apg04    LIKE  apg_file.apg04
  DEFINE l_cnt      LIKE  type_file.num5
  #add by zhangsba191104---e
  #add by zhangsba191104---s
 SELECT apg04 INTO l_apg04 
   FROM apg_file,apf_file
   WHERE apf01 = g_apf.apf01 
     AND apf01 = apg01 
    ...
    SELECT COUNT(*) INTO l_cnt 
      FROM oox_file 
     WHERE oox01 = l_yy AND oox02 = l_mm AND oox05 = g_apf.apf06 AND oox03 = l_apg04 
    
    IF l_cnt > 0 THEN
    	 CALL cl_err(l_apg04,'cap-101',1)
       RETURN
    END IF
  #add by zhangsba191104---e
