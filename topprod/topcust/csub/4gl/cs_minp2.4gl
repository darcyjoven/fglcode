...
FUNCTION cs_minp2(p_wono,p_check,p_percent,p_opseq,p_process,p_sequence,p_date)  #FUN-C70037 add 'p_date'
   ...
   DEFINE  l_taoshu     LIKE type_file.num20_6,   #tianry add end 
           l_shifa      LIKE type_file.num20_6
      ...
      FOREACH s_minp_c INTO l_sfa.*
       ...
       # IF l_sfa.sfa26 MATCHES '[34]' THEN   #FUN-A20037
        ...
        #LET qty=(l_sfa.sfa06-l_sfa.sfa063+l_sfa.sfa062)/l_sfa.sfa161           #No.TQC-610132 mark
         ...
         #  tianry add 161115
         SELECT sfb081 INTO l_taoshu FROM sfb_file WHERE sfb01=l_sfa.sfa01
         LET l_shifa=l_taoshu*l_sfa.sfa161
         IF l_shifa>l_sfa.sfa06 THEN 
            IF l_shifa-l_sfa.sfa06<0.001 THEN
               LET l_shifa=l_shifa+0.0001
               #LET l_shifa=l_sfa.sfa06
            END IF 
         ELSE
            LET l_shifa=l_sfa.sfa06
         END IF 
         #tianry add end 
                   #tianry 161115  sfa.sfa06->l_shifa
         LET qty=((l_shifa+l_sfa.sfa065-l_sfa.sfa063+l_sfa.sfa062)/l_sfa.sfa05)*l_sfb08          #FUN-C30166 
         # ---------------------------
         # 已發 - 報費 / 實計單位用量
         # --------------------------- 
         # No.+114 Tommy 考慮發料誤差 
         IF cl_null(l_sfa.sfa100) THEN LET l_sfa.sfa100 = 0 END IF
        #IF cl_null(l_sfa.sfa100) THEN LET l_sfa.sfa100 =0.1 END IF  #ly20180108  若未维护默认给0.1 千分之一 解决小数误差
      ...
      #No.+437 010718 add by linda 依單位小數位數取位
         SELECT ima55 INTO l_ima55
          ...
          WHERE sfb01=p_wono
           ...
           LET l_min_set = s_trunc(min_set,l_gfe03)   #tianry mark 
           IF min_set - l_min_set > 0 THEN 
              LET min_set = l_min_set + 1
           ELSE
              LET min_set = l_min_set
           END IF
...
FUNCTION cs_minp_routing2(p_wono1,p_check1,p_percent1,p_operation,p_process,p_sequence)  
