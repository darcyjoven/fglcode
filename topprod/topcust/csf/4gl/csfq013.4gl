...
DEFINE	  l_sl          LIKE type_file.num5          #No.FUN-680137 SMALLINT
   ...
   END IF
    ...
    OPEN WINDOW q013_w AT p_row,p_col
        WITH FORM "csf/42f/csfq013"
...
FUNCTION q013_b_fill()              #BODY FILL UP
  DEFINE l_sql,l_sql1,l_sql2,l_sql3    STRING,        #No.FUN-680137  VARCHAR(1000)
  ...
  LET l_sql1 = " SELECT sfb01,sfb05,ima02,ima021,sfb08,NVL(imaud09,0) imaud09,NVL(sfbud09,0) sfbud09,",
               ...
               " AND  sfs04 = sfa03 AND sfa01 = sfs04 ",
               " AND sfpconf = 'Y' AND sfb87 = 'Y' ",
               " GROUP BY sfb01,sfb05,ima02,ima021,sfb08,imaud09,sfbud09,sfs04 "
  LET l_sql2 = l_sql1," UNION ",
               " SELECT sfb01,sfb05,ima02,ima021,sfb08,NVL(imaud09,0) imaud09,NVL(sfbud09,0) sfbud09,",
               " '' sjll,sfe07 sfs04,NVL(SUM(sfe16),0) sfa05,'1','' sfb01_b ",
               " FROM sfb_file LEFT JOIN sfa_file ON sfa01 = sfb01 ",
               "               LEFT JOIN ima_file ON ima01 = sfb05 ",
               " ,sfe_file ", 
               " WHERE sfb81 BETWEEN to_date( '",g_dat.date1,"','YY/MM/DD') AND to_date ( '",g_dat.date2,"','YY/MM/DD') ",
               " AND ",tm.wc2 CLIPPED," AND sfb04 ! = '8'"," AND sfe07 = sfa03 AND sfa01 = sfe01 ",
               " AND sfb87 = 'Y'",
               " GROUP BY sfb01,sfb05,ima02,ima021,sfb08,imaud09,sfbud09,sfe07 "
  LET l_sql3 = l_sql2," UNION ",
               " SELECT a.sfb01 sfb01,a.sfb05 sfb05,ima02,ima021,a.sfb08,NVL(imaud09,0) imaud09, NVL(a.sfbud09,0) sfbud09,",
               " '' sjll,sfa03 sfs04,NVL(SUM(sfa05),0) sfa05,'2',b.sfb01 sfb01_b ",
               " FROM sfb_file a LEFT JOIN sfa_file ON sfa01 = a.sfb01 ",
               "               LEFT JOIN ima_file ON ima01 = a.sfb05 ",
               "               LEFT JOIN sfb_file b ON a.sfb01 = b.sfbud04 ",
               " WHERE a.sfb81 BETWEEN to_date( '",g_dat.date1,"','YY/MM/DD') AND to_date ( '",g_dat.date2,"','YY/MM/DD') ",
               " AND ",tm.wc2 CLIPPED," AND a.sfb04 ! = '8'",
               " AND b.sfb01 LIKE '%MSA%'  AND b.sfb87 = 'Y'  AND a.sfb87 = 'Y'",
               " GROUP BY a.sfb01,a.sfb05,ima02,ima021,a.sfb08,imaud09,a.sfbud09,sfa03,b.sfb01"

   LET l_sql3 = l_sql3," ORDER BY sfb01,sfb05 "

   LET l_sql = " INSERT INTO csfq013_tmp ",
               "   SELECT x.* ",
               "     FROM (",l_sql3 CLIPPED,") x "
   PREPARE q013_ins FROM l_sql
   EXECUTE q013_ins

   LET l_sql  = " SELECT sfb01,sfb05,ima02,ima021,sfb08,imaud09,sfbud09,'',sfs04,SUM(sfa05),ctype,sfb01_b ",
                " FROM csfq013_tmp ",
                " GROUP BY sfb01,sfb05,ima02,ima021,sfb08,imaud09,sfbud09,sfs04,ctype,sfb01_b "
   PREPARE q013_pb FROM l_sql
   DECLARE q013_bcs CURSOR WITH HOLD FOR q013_pb
    CALL g_sfb.clear()
    LET g_cnt = 1
    FOREACH q013_bcs INTO g_sfb[g_cnt].*
  ...
  #  INSERT INTO csfq013_tmp VALUES (g_sfb[g_cnt].*)
    LET g_cnt = g_cnt + 1
...
FUNCTION q013_temp_table()
     DROP TABLE csfq013_tmp;
	 CREATE TEMP TABLE csfq013_tmp(
            sfb01   LIKE sfb_file.sfb01,
            sfb05   LIKE sfb_file.sfb05,
            ima02   LIKE ima_file.ima02,
            ima021  LIKE ima_file.ima021,
