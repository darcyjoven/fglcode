...
FUNCTION scsft325(p_argv,p_argv1,p_argv2)
           ...
           WHEN "void"
              IF cl_chk_act_auth() THEN
                #CALL t325_x()    #CHI-D20010
                 ...
                 CALL t325_show() #FUN-BC0104
              END IF
              CALL t325_pic() #圖形顯示 #FUN-660137
           #CHI-D20010---begin
           WHEN "undo_void"
              IF cl_chk_act_auth() THEN
                 CALL t325_x(2)
    ...
    WHILE TRUE
       ...
       # END IF
        ...
        IF g_cnt>0 THEN
           ...
           END IF
           IF g_smy.smydmy4='Y' THEN CALL t325_s() END IF
        END IF
        EXIT WHILE
    END WHILE
END FUNCTION
...
FUNCTION t325_u()
    ...
    OPEN t325_cl USING g_tc_sna.tc_sna01
    IF STATUS THEN
       ...
       RETURN
    END IF
    FETCH t325_cl INTO g_tc_sna.*          # 鎖住將被更改或取消的資料
    ...
    WHILE TRUE
        ...
        IF STATUS OR SQLCA.SQLERRD[3]=0  THEN
           ...
           CONTINUE WHILE
        END IF
        IF g_tc_sna.tc_sna01 != g_tc_sna_t.tc_sna01 THEN CALL t325_chkkey() END IF
        EXIT WHILE
    END WHILE
...
FUNCTION t325_chkkey()
    ...
    IF STATUS OR SQLCA.SQLERRD[3]=0 THEN
       ...
       LET g_tc_sna.*=g_tc_sna_t.* CALL t325_show() ROLLBACK WORK RETURN
    END IF
END FUNCTION
    ...
    DISPLAY BY NAME
       ...
       BEFORE INPUT
        ...
        #TQC-AC0294-----end--------------
          ...
          CALL t325_set_no_entry(p_cmd)    #FUN-550012
...
FUNCTION t325_set_entry_b(p_cmd)
   DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-680121 VARCHAR(1)
...
FUNCTION t325_set_no_entry_b(p_cmd)
   DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-680121 VARCHAR(1)
...
FUNCTION t325_set_entry_b1(p_cmd)
 DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-680121 VARCHAR(1)
    CALL cl_set_comp_entry("tc_snb04",TRUE)
 
END FUNCTION
 
FUNCTION t325_set_no_entry_b1(p_cmd)
  DEFINE p_cmd    LIKE type_file.chr1    #No.FUN-680121 VARCHAR(1)
...
DEFINE
    ...
    CASE p_flag
        WHEN 'N' FETCH NEXT     t325_cs INTO g_tc_sna.tc_sna01
        WHEN 'P' FETCH PREVIOUS t325_cs INTO g_tc_sna.tc_sna01
        WHEN 'F' FETCH FIRST    t325_cs INTO g_tc_sna.tc_sna01
        WHEN 'L' FETCH LAST     t325_cs INTO g_tc_sna.tc_sna01
        WHEN '/'
            IF (NOT mi_no_ask) THEN
                ...
                END IF
            END IF
            FETCH ABSOLUTE g_jump t325_cs INTO g_tc_sna.tc_sna01
            LET mi_no_ask = FALSE
    END CASE
...
#str----end by huanglf161014
    ...
    LET g_forupd_sql = cl_forupd_sql(g_forupd_sql)
    DECLARE t325_bcl CURSOR FROM g_forupd_sql      # LOCK CURSOR
    ...
    INPUT ARRAY g_tc_snb WITHOUT DEFAULTS FROM s_tc_snb.*
        ...
        BEFORE ROW
            ...
            IF g_rec_b >= l_ac THEN
                ...
                OPEN t325_bcl USING g_tc_sna.tc_sna01,g_tc_snb_t.tc_snb03
                IF STATUS THEN
                   CALL cl_err("OPEN t325_bcl:", STATUS, 1)
                   LET l_lock_sw = "Y"
                ELSE
                   FETCH t325_bcl INTO g_tc_snb_t.*
                   IF SQLCA.sqlcode THEN
                   ...
                   LET g_before_input_done = FALSE
                   CALL t325_set_entry_b('u')
                   CALL t325_set_no_entry_b('u')
                   LET g_before_input_done = TRUE
                 LET g_change='N'  #No.FUN-580029
                END IF
                #FUN-BC0104---add---str---
                CALL t325_set_noentry_tc_snb46()       
                IF NOT cl_null(g_tc_snb[l_ac].tc_snb46) THEN
                   CALL t325_qcl02_desc()
                END IF
                CALL t325_set_entry_b1(p_cmd)
                CALL t325_set_no_entry_b1(p_cmd)
                #FUN-BC0104---add---end---
              ...
              #IF由工單帶入，則不可修改tc_snb41-43
               ...
               END IF
                  CALL cl_set_comp_entry("tc_snb05",FALSE)  
            END IF
...
#str-----end by huanglf161014
       #   IF g_success = 'Y' THEN #FUN-810025
        ...
        BEFORE INSERT
            ...
            LET p_cmd='a'
            LET g_before_input_done = FALSE
            CALL t325_set_entry_b('a')
            CALL t325_set_no_entry_b('a')
            LET g_before_input_done = TRUE
            ...
            END IF
 {
        ...
        AFTER FIELD tc_snb20     #Run Card
         ...
         #IF cl_null(g_tc_snb_t.tc_snb20) OR g_tc_snb[l_ac].tc_snb20 ! = g_tc_snb_t.tc_snb20 THEN #FUN-BC0104
          ...
          #加入RUN CARD 結案碼  
           ...
           CALL t325_set_no_entry_b(p_cmd) #FUN-810045 end
          
           IF g_argv = '1' THEN
              CALL t325_sfb01(l_ac)
              IF NOT cl_null(g_errno) THEN
                 CALL cl_err('',g_errno,1)
                 NEXT FIELD tc_snb20
              END IF
          ...
          #FUN-D70008-add---end
           ...
           END IF
         #END IF #FUN-BC0104
         CALL t325_set_no_entry_b1(p_cmd)  #FUN-BC0104
 
        BEFORE FIELD tc_snb05     #业编码
          
          LET g_tc_snb[l_ac].tc_snb05 = '维修站'

      BEFORE FIELD tc_snb06    #-- 作业编码
          LET  g_tc_snb[l_ac].tc_snb06 = g_tc_sna.tc_sna08
        ...
        AFTER FIELD tc_snb06    #-- 作业编码
           ...
           IF g_argv MATCHES '[12]' THEN
              ...
              SELECT  ecd01 INTO g_buf FROM ecd_file
                ...
                WHERE ecd01=g_tc_snb[l_ac].tc_snb06 AND ecdacti='Y'  #MOD-B50097 add
           ...
           END IF
            ...
            IF cl_null(g_tc_snb[l_ac].tc_snb06 ) THEN
    ...
    CALL t325_delHeader()     #CHI-C30002 add
 
END FUNCTION
 
#CHI-C30002 -------- add -------- begin
FUNCTION t325_delHeader()
...
# check W/O 是否存在對應之FQC單中 --
    ...
    LET g_tc_snb[p_ac].tc_snb04 = g_sfb.sfb05
 {
   ...
   END IF
    ...
    IF g_sma.sma115='Y' THEN
       CALL t325_sel_ima()
       CALL t325_set_du_by_origin(g_tc_snb[l_ac].tc_snb04,g_tc_snb[l_ac].tc_snb08,
...
FUNCTION t325_y_b()
   ...
   IF g_tc_sna.tc_sna00 != '0' THEN
      ...
      OPEN WINDOW t325_y_w WITH FORM "asf/42f/cect324y"
            ATTRIBUTE (STYLE = g_win_style CLIPPED)
      CALL cl_ui_locale("cect324y")
 
      CALL t325_y_b1()
 
      CLOSE WINDOW t325_y_w
...
#FUN-AB0025--------mod---------------end----------------
   ...
   PREPARE t325_y_b1_p FROM l_sql
   DECLARE t325_y_b1_c CURSOR FOR t325_y_b1_p
   LET i=1
   LET qty2=tot_qty
 
   MESSAGE "Waiting..."
   FOREACH t325_y_b1_c INTO l_shm[i].*,l_shm012
      IF g_tc_sna.tc_sna00 = '1' THEN
   ...
   FOR k=1 TO l_shm.getLength()
      ...
      IF g_sma.sma115='Y' THEN
         CALL t325_sel_ima()
         CALL t325_set_du_by_origin(l_tc_snb.tc_snb04,l_tc_snb.tc_snb08,
...
DEFINE p_wc2           LIKE type_file.chr1000 #No.FUN-680121 VARCHAR(200)
 ...
 DEFINE l_x            LIKE type_file.num5   #add by guanyao160901
    ...
    DECLARE tc_snb_curs CURSOR FOR t325_pb
...
FUNCTION t325_s()       #過帳
 ...
 DEFINE l_tc_snb           RECORD LIKE tc_snb_file.*  #FUN-C70087
   ...
   LET g_flag = 'Y'
   DECLARE t325_s_cs1 CURSOR FOR SELECT tc_snb03,tc_snb05,tc_snb06 FROM tc_snb_file                                                                              
                                  WHERE tc_snb01 = g_tc_sna.tc_sna01
   CALL s_showmsg_init() 
  ...
  # CALL s_padd_img_init()  #FUN-CC0095
   ...
   DECLARE t325_s_c2 CURSOR FOR SELECT * FROM tc_snb_file
     WHERE tc_snb01 = g_tc_sna.tc_sna01 
...
FUNCTION t325_sel_ima()
   SELECT ima25,ima906,ima907 INTO g_ima25,g_ima906,g_ima907
     FROM ima_file
    WHERE ima01=g_tc_snb[l_ac].tc_snb04
END FUNCTION
 
FUNCTION t325_set_du_by_origin(p_tc_snb04,p_tc_snb08,p_tc_snb09,p_tc_snb05,p_tc_snb06,p_tc_snb07)
...
FUNCTION t325_du_default(p_cmd)
...
#TQC-C60207 ----------------End-----------------
   ...
   DECLARE t325_y_chk_c CURSOR FOR SELECT * FROM tc_snb_file
                                   WHERE tc_snb01=g_tc_sna.tc_sna01
   FOREACH t325_y_chk_c INTO b_tc_snb.*   
...
FUNCTION t325_tc_snb32_check(l_qcf091,l_sfb08,l_tc_snb09)
   ...
   CALL t325_du_data_to_correct()
   CALL t325_set_origin_field()
   CALL t325_unit(g_tc_snb[l_ac].tc_snb08)
   IF NOT cl_null(g_errno) THEN
...
FUNCTION t325_set_noentry_tc_snb46()
   CALL cl_set_comp_entry('tc_snb46,tc_snb47',FALSE)
END FUNCTION

FUNCTION t325_set_comp_required(p_tc_snb46,p_tc_snb47)
