...
DEFINE
    ...
    g_sfv DYNAMIC ARRAY OF RECORD
     ...
     g_sfv_1 DYNAMIC ARRAY OF RECORD       
            sfu01_1  LIKE sfu_file.sfu01,
            sfv11_1  LIKE sfv_file.sfv11,
            sfv04_1  LIKE sfv_file.sfv04,
            ima02_1  LIKE ima_file.ima02,
            sfv08_1  LIKE sfv_file.sfv08,
            sfv09_1  LIKE sfv_file.sfv09,
            oea23_1  LIKE oea_file.oea23,
            sfvud07_1 LIKE sfv_file.sfvud07,
            sfv_sum_1 LIKE sfv_file.sfvud07
        END RECORD,
...
#str----add by huanglf161027
   #CALL cl_set_comp_entry("sfv05",FALSE)
   LET g_sfu.sfv05 = 'P001'
   DISPLAY BY NAME g_sfu.sfv05
#str----end by huanglf161027   
          # CONSTRUCT BY NAME tm.wc ON sfu02,sfv05
           CONSTRUCT BY NAME tm.wc ON sfu02,sfupost
              BEFORE CONSTRUCT
                 CALL cl_qbe_init()
              AFTER FIELD sfu02
                LET g_date = GET_FLDBUF(sfu02)
...
#str----add by huanglf161027
      --ON ACTION CONTROLP
         ...
         --WHEN INFIELD(sfv05)
            ...
            --CALL cl_create_qry() RETURNING g_sfu.sfv05
            --DISPLAY g_sfu.sfv05 TO sfv05
            --NEXT FIELD sfv05
        --NEXT FIELD sfv05
...
#No.FUN-930009 --End
      ...
      ON ACTION controlg      #MOD-4C0121
		...
		#No.FUN-580031 --end--       HCN
         ...
         #資料權限的檢查
    LET tm.wc = tm.wc CLIPPED,cl_get_extra_cond('g_user', 'g_grup')
    #End:FUN-980030
    LET g_sql="SELECT sfu02 FROM sfu_file,sfv_file ", # 組合出 SQL 指令
        " WHERE ",tm.wc CLIPPED, " and sfv05 = 'P001' and sfuconf !='X' and sfu01 = sfv01 ORDER BY sfu02"  #add by huanglf161027
...
DEFINE
 ...
 DEFINE   row1            LIKE type_file.num5
    CASE p_flag
        WHEN 'N' FETCH NEXT     q001_cs INTO g_sfu.sfu02,g_sfu.sfv05
        WHEN 'P' FETCH PREVIOUS q001_cs INTO g_sfu.sfu02,g_sfu.sfv05
        WHEN 'F' FETCH FIRST    q001_cs INTO g_sfu.sfu02,g_sfu.sfv05
        WHEN 'L' FETCH LAST     q001_cs INTO g_sfu.sfu02,g_sfu.sfv05
        WHEN '/'
...
#                       CONTINUE PROMPT
      ...
      ON ACTION controlg      #MOD-4C0121
         CALL cl_cmdask()     #MOD-4C0121
            ...
            END IF
            LET mi_no_ask = FALSE
            FETCH ABSOLUTE g_jump q001_cs INTO g_sfu.sfu02
    END CASE
 
    IF SQLCA.sqlcode THEN
        CALL cl_err(g_sfu.sfu02,SQLCA.sqlcode,0)
...
#add by donghy 170313
   ...
   LET l_sql =
    ...
    LET l_sql =
        "SELECT sfp03,sfp01,'',sfq02,sfb05,ima02,ima25,sfq03,'',0,0 ",  #MOD-6A0130 modify
        " FROM  sfp_file,sfq_file,sfb_file,ima_file ",
        " WHERE ",m_wc CLIPPED," AND ",m_wc1 CLIPPED, " AND ",m_wc2 CLIPPED,
