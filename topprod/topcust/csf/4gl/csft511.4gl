...
# Modify.........: #No:190314    19/03/14 by pulf  调拨单拨出库位和拨入库位为空不可过账
...
DEFINE j_imn_l DYNAMIC	ARRAY OF RECORD		### add by huzhou 20170808 增加page_list
...
#str----end by huanglf160928
   ...
   LET g_wc3 = " 1=1"
   CONSTRUCT g_wc3 ON tc_imp02,tc_imp03,tc_imp04,tc_imp05,tc_imp06,tc_imp07,tc_imp08,tc_imp09,tc_imp10     #add tc_imp10 by guanyao160909
   ...
   WHILE TRUE
      CASE
         ...
         WHEN (l_action_flag = "Page2")
            CALL i511_bp3("G")
	 WHEN (l_action_flag = "page_list")        #add by huzhou
	    CALL t511_list_fill()
	    CALL i511_bp4("G")
      END CASE
...
DEFINE l_ecm04      LIKE ecm_file.ecm04 
   ...
   INPUT ARRAY g_tc_imn WITHOUT DEFAULTS FROM s_tc_imn.*
          ...
          IF NOT cl_null(g_tc_imn[l_ac].tc_imn03) THEN 
             ...
             IF g_tc_imn[l_ac].tc_imn03 != g_tc_imn_t.tc_imn03 OR
                ...
                IF cl_null(l_n) OR l_n = 0 THEN 
                   ...
                   NEXT FIELD tc_imn03
                END IF 
                CALL t511_check_sfa(g_tc_imn[l_ac].tc_imn03,g_tc_imm.tc_imm08)
                IF g_error = 'N' THEN 
       ...
       ON ACTION CONTROLP
          CASE
             WHEN INFIELD(tc_imn03)
                CALL cq_sfb(true,true,g_tc_imm.tc_imm08,'','','','') RETURNING tc_imn03_str
                #NEXT FIELD tc_imn03
...
DEFINE l_sfa056   LIKE  sfa_file.sfa05
   ...
   END IF      
    ...
    LET g_forupd_sql = "SELECT tc_imp02,tc_imp03,tc_imp04,tc_imp05,'','',tc_imp06,'',tc_imp07,'',tc_imp08,tc_imp09,tc_imp10 ",   #add tc_imp10 by guanyao160909
                       "  FROM tc_imp_file",
    ...
    INPUT ARRAY g_tc_imp WITHOUT DEFAULTS FROM s_tc_imp.*
        ...
        AFTER INSERT
           IF INT_FLAG THEN
              ...
              CANCEL INSERT
           END IF
           INSERT INTO tc_imp_file(tc_imp01,tc_imp02,tc_imp03,tc_imp04,tc_imp05,tc_imp06,tc_imp07,tc_imp08,tc_imp09,tc_imp10)  #add tc_imp10 by guanyao160909
            VALUES(g_tc_imm.tc_imm01,g_tc_imp[l_ac].tc_imp02,g_tc_imp[l_ac].tc_imp03,g_tc_imp[l_ac].tc_imp04,g_tc_imp[l_ac].tc_imp05,g_tc_imp[l_ac].tc_imp06,
                   g_tc_imp[l_ac].tc_imp07,g_tc_imp[l_ac].tc_imp08,g_tc_imp[l_ac].tc_imp09,g_tc_imp[l_ac].tc_imp10)  #add tc_imp10 by guanyao160909
           IF SQLCA.sqlcode THEN
...
DEFINE p_wc2           STRING
    ...
    LET g_sql = "SELECT tc_imp02,tc_imp03,tc_imp04,tc_imp05,'','',tc_imp06,'',tc_imp07,'',tc_imp08,tc_imp09,tc_imp10 ",  #add tc_imp10 by guanyao160909
                " FROM tc_imp_file",
                " WHERE tc_imp01 ='",g_tc_imm.tc_imm01,"'",
                "   AND ",p_wc2 CLIPPED,
                " ORDER BY 1"
...
DEFINE l_immud13_t LIKE type_file.dat
   ...
   DECLARE t511sub_s1_c CURSOR FOR
   ...
   FOREACH t511sub_s1_c INTO l_tc_imq.*
      ...
      #No:190314 add begin------
      #IF cl_null(l_tc_imq.tc_imq06)  OR cl_null(l_tc_imq.tc_imq18) THEN
       IF cl_null(l_tc_imq.tc_imq06) THEN       
	 LET g_success = 'N'
      	 LET g_showmsg = l_tc_imq.tc_imq02
         CALL s_errmsg("tc_imq02",g_showmsg,'','apm1033',1)
         CONTINUE FOREACH
      END IF 
      #No:190314 add end--------
--    
      ...
      IF l_imd11_1 = 'Y' AND (l_imd11 = 'N' OR l_imd11 IS NULL) THEN 
         CALL t511sub_chk_avl_stk(l_tc_imq.*)     
         IF g_success='N' THEN
            LET g_totsuccess="N"
            CONTINUE FOREACH   
         END IF    
      ...
      #-->撥出更新
      IF t511sub_t(g_tc_imm.*,l_tc_imq.*) THEN
         LET g_totsuccess="N"
         CONTINUE FOREACH
      END IF

      #IF g_sma.sma115 = 'Y' THEN#此处只有一个库存单位
      #   CALL t511sub_upd_s(l_tc_imq.*,g_tc_imm.*)
      #END IF
      ...
      #-->撥入更新
      IF t511sub_t2(g_tc_imm.*,l_tc_imq.*) THEN
         LET g_totsuccess="N"
         CONTINUE FOREACH
      END IF

      #IF g_sma.sma115 = 'Y' THEN
      #   CALL t511sub_upd_t(l_imn.*,l_imm.*)
      #END IF
...
DEFINE l_tc_imq     RECORD LIKE tc_imq_file.*
  ...
  DECLARE t511_s1_c CURSOR FOR SELECT * FROM tc_imq_file WHERE tc_imq01 = g_tc_imm.tc_imm01
  FOREACH t511_s1_c INTO l_tc_imq.*
      ...
      CALL t511_u_img(l_tc_imq14,+1,l_tc_imq.tc_imq05,l_tc_imq.tc_imq06,
      ...
      IF g_success='N' THEN
         ...
         CONTINUE FOREACH
      END IF
       CALL t511_u_img(l_tc_imq15,-1,l_tc_imq.tc_imq05,l_tc_imq.tc_imq10,
...
FUNCTION t511sub_chk_avl_stk(p_tc_imq)   
...
#-->撥出更新
FUNCTION t511sub_t(p_tc_imm,p_tc_imq)
   DEFINE p_tc_imq         RECORD LIKE tc_imq_file.*,
          l_img            RECORD
...
FUNCTION t511sub_t2(p_tc_imm,p_tc_imq)
   DEFINE p_tc_imq          RECORD LIKE tc_imq_file.*,
          l_img          RECORD
             img16          LIKE img_file.img16,
             img23          LIKE img_file.img23,
   ...
   CALL t511sub_log_2(1,0,'1',p_tc_imq.*,p_tc_imm.*) 
   CALL t511sub_log_2(1,0,'0',p_tc_imq.*,p_tc_imm.*) RETURN 0
END FUNCTION

#處理異動記錄
FUNCTION t511sub_log_2(p_stdc,p_reason,p_code,p_tc_imq,p_tc_imm)
   ...
   LET g_tlf.tlf13='csft511'                   #異動命令代號
...
FUNCTION t511_u_img(qty,p_type,p1,p2,p3,p4,p5,p_tc_imq) # Update img_file
...
FUNCTION t511_u_tlf(p_tc_imq) #------------------------------------ Update tlf_file
...
DEFINE p_tc_imq RECORD LIKE tc_imq_file.*                    
    ...
    DECLARE t511_u_tlf_c CURSOR FROM l_sql
    LET l_i = 0 
    CALL la_tlf.clear()
    FOREACH t511_u_tlf_c INTO g_tlf.*
       LET l_i = l_i + 1
...
FUNCTION t511_u_tlfs(p_tc_imq) #------------------------------------ Update tlfs_file
...
#str----add by guanyao160922
FUNCTION t511_check_sfa(p_tc_imn03,p_tc_imm08)
DEFINE p_tc_imn03     LIKE tc_imn_file.tc_imn03
