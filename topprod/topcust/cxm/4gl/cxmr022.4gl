...
MAIN
   ...
   LET  l_table = cl_prt_temptable('cxmr022',g_sql) CLIPPED
...
FUNCTION cxmr022_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,        
   ...
   OPEN WINDOW cxmr022_w AT p_row,p_col WITH FORM "cxm/42f/cxmr022"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW cxmr022_w 
  ...
  #UI
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('cxmr022',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW cxmr022_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0094
   ...
   LOCATE l_img_blob IN MEMORY             
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='cxmr022' 
...
--LET l_sql="select '','','','','',oga01,'','','',ogb04,'',sum(ogb12),'','','','','','','','','',ogbud02,'','','',''",    #3'' add by qianyuan170421
           --" from ogb_file,oga_file ",
           --" WHERE oga01 = ogb01 ", 
           --"   AND ",tm.wc CLIPPED,
           --"   AND ogaconf != 'X' ", #01/08/20 mandy 
...
LET l_sql="select '','','','','',oga01,'','','',ogb04,'',sum(ogb12),'','','','','','','','','',ogbud02,'','',to_date(substr(ogb092,-6),'YY/MM/DD'),''",    
           " from ogb_file,oga_file ",
           " WHERE oga01 = ogb01 ", 
           "   AND ",tm.wc CLIPPED,
           "   AND ogaconf != 'X' ", #01/08/20 mandy 
           "   GROUP BY oga01,ogb04,ogbud02,to_date(substr(ogb092,-6),'YY/MM/DD')"
#---end---add by qianyuan170421---
    
     PREPARE cxmr022_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
        EXIT PROGRAM 
     END IF 
     LET l_ogb03 = 1
     DECLARE cxmr022_curs1 CURSOR FOR cxmr022_prepare1
     FOREACH cxmr022_curs1 INTO sr.* 
     ...
     SELECT ima01,ima02,ima021 INTO sr.ima01,sr.ima02,sr.ima021 FROM ima_file WHERE ima01 = sr.ogb04  #add by guanyao161101
    ...
    LET l_ogb03 = l_ogb03 + 1
     ...
     CALL cl_prt_cs3('cxmr022','cxmr022',g_sql,g_str)
