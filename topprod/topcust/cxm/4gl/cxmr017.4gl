# Prog. Version..:
#
# Pattern name...: cxmr017.4gl
# Descriptions...: 小米送货单打印
...
MAIN
   ...
   LET g_sql="zo02.zo_file.zo02,",
             ...
             "occ02.occ_file.occ18,",  #modify by huanglf161102
             ...
             "ima021.ima_file.ima021"  #add by huanglf161207

   LET  l_table = cl_prt_temptable('cxmr017',g_sql) CLIPPED
...
FUNCTION cxmr017_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,        
   ...
   OPEN WINDOW cxmr017_w AT p_row,p_col WITH FORM "cxm/42f/cxmr017"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW cxmr017_w 
  ...
  #UI
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('cxmr017',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW cxmr017_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0094
          ...
          sr        RECORD
                    ...
                    obk03    LIKE obk_file.obk03,   #客户产品编号
                    oga23    LIKE oga_file.oga23,
                    ima021   LIKE ima_file.ima021  #add by huanglf161207
   ...
   LOCATE l_img_blob IN MEMORY             
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='cxmr017' 
...
LET l_sql="select '','','','','',oga01,'','','',ogb04,'',sum(ogb12),'','','','','','','','',ogbud02,'',''",
           " from ogb_file,oga_file ",
           " WHERE oga01 = ogb01 ", 
           "   AND ",tm.wc CLIPPED,
           "   AND ogaconf != 'X' ", #01/08/20 mandy 
     ...
     PREPARE cxmr017_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
        EXIT PROGRAM 
     END IF 
     LET l_ogb03 = 1
     DECLARE cxmr017_curs1 CURSOR FOR cxmr017_prepare1
     FOREACH cxmr017_curs1 INTO sr.* 
     IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
     LET sr.ogb03 = l_ogb03
     
     SELECT oga03,ogb11,ogb05,ogb31,ogbud01,ogaud03,ogaud04,ogaud05,oga02,oga23
     INTO sr.oga04,sr.ogb11,sr.ogb05,sr.ogb31,sr.ogbud01,sr.ogaud03,sr.ogaud04,
          sr.ogaud05,sr.oga02,sr.oga23
     ...
     #ima021 add by huanglf161207
     SELECT obk03 INTO sr.obk03 FROM obk_file 
     WHERE obk01 = sr.oga04 AND obk02= sr.ogb04 AND obk05 = sr.oga23 
    ...
    LET l_ogb03 = l_ogb03 + 1
     ...
     CALL cl_prt_cs3('cxmr017','cxmr017',g_sql,g_str)
