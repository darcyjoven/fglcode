...
MAIN
   ...
   LET  l_table = cl_prt_temptable('cxmr013',g_sql) CLIPPED
  ...
  --LET g_sql1="zo02.zo_file.zo02,",
             ...
             --"oga02.oga_file.oga02"
--
   --LET  l_table1 = cl_prt_temptable('cxmr0131',g_sql1) CLIPPED
...
FUNCTION cxmr013_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,        
   ...
   OPEN WINDOW cxmr013_w AT p_row,p_col WITH FORM "cxm/42f/cxmr013"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW cxmr013_w 
  ...
  #UI
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('cxmr013',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW cxmr013_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0094
   ...
   LOCATE l_img_blob IN MEMORY             
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='cxmr013' 
     LET i = 1
...
LET l_sql="select '','',oga01,'',ogbud02,'',ogb04,sum(ogb12),'',''",
     ...
     PREPARE cxmr013_prepare1 FROM l_sql
     ...
     IF l_oga01_t IS NOT NULL  THEN 
        ...
        END IF
      END IF 
     DECLARE cxmr013_curs1 CURSOR FOR cxmr013_prepare1
     FOREACH cxmr013_curs1 INTO sr.* 
     ...
     IF i=1 THEN 
      LET l_wc = 'oga01 in ("',sr.oga01,'"'
      LET i = i+1
     ELSE 
      LET l_wc = l_wc,',"',sr.oga01,'"'
     END IF 
  ...
  #   LET l_wc='oga01="',sr.oga01,'"'
    ...
    LET l_ogb03 = l_ogb03 + 1
     ...
     CALL cl_prt_cs3('cxmr013','cxmr013',g_sql,g_str)
