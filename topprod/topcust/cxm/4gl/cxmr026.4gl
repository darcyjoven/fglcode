...
FUNCTION cxmr026_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,        
   ...
   OPEN WINDOW cxmr026_w AT p_row,p_col WITH FORM "cxm/42f/cxmr026"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW cxmr026_w 
  ...
  #UI
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('cxmr026',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW cxmr026_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0094
   ...
   LOCATE l_img_blob IN MEMORY             
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='cxmr026' 
...
LET l_sql="select '','','','','',oga01,'','','',ogb04,'',sum(ogb12),'','','','','','','','','',ogbud02",
     ...
     PREPARE cxmr026_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
        EXIT PROGRAM 
     END IF 
     LET l_ogb03 = 1
     DECLARE cxmr026_curs1 CURSOR FOR cxmr026_prepare1
     FOREACH cxmr026_curs1 INTO sr.* 
    ...
    #ly0814  临时处理下备注带出通单备注 ogbud01 替换为ogbud06
     ...
     SELECT obk03 INTO sr.obk03 FROM obk_file WHERE obk01=sr.ogb04 AND  obk02=sr.oga04
    ...
    LET l_ogb03 = l_ogb03 + 1
     ...
     CALL cl_prt_cs3('cxmr026','cxmr026',g_sql,g_str)
