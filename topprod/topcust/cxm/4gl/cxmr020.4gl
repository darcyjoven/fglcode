...
MAIN
   ...
   LET g_sql="oha01.oha_file.oha01,",
             ...
             "ohauser.oha_file.ohauser,",
             "gen02.gen_file.gen02,",
             "occ18.occ_file.occ18,",
             "imd02.imd_file.imd02"
             

   LET  l_table = cl_prt_temptable('cxmr020',g_sql) CLIPPED
...
FUNCTION cxmr020_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,        
   ...
   OPEN WINDOW cxmr020_w AT p_row,p_col WITH FORM "cxm/42f/cxmr020"
...
WHILE TRUE
      ...
      ON ACTION controlp
           CASE
              WHEN INFIELD(oha01)
                 ...
                 LET g_qryparam.form = "q_oha"  #No.TQC-5B0095
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW cxmr020_w 
  ...
  #UI
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('cxmr020',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW cxmr020_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0094
   ...
   LOCATE l_img_blob IN MEMORY             
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='cxmr020' 
...
LET l_sql= "select oha01,oha02,oha03,ohb04,ohb06,ohb12,ohb05,ohb33,ohb09,oha48,ohauser,'','',''",
           " from oha_file,ohb_file ",
           " WHERE oha01 = ohb01 ", 
           "   AND ",tm.wc CLIPPED,
           "   AND ohaconf != 'X' "
    
     PREPARE cxmr020_prepare1 FROM l_sql
     DECLARE cxmr020_curs1 CURSOR FOR cxmr020_prepare1
     FOREACH cxmr020_curs1 INTO sr.* 
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
        EXIT PROGRAM 
     END IF 
     SELECT gen02 INTO sr.gen02 FROM gen_file WHERE gen01 = sr.ohauser
     SELECT occ18 INTO sr.occ18 FROM occ_file WHERE occ01 = sr.oha03
     SELECT imd02 INTO sr.imd02 FROM imd_file WHERE imd01 = sr.ohb09
     ...
     CALL cl_prt_cs3('cxmr020','cxmr020',g_sql,g_str)
