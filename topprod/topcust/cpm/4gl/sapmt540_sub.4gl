...
#GLOBALS "../../config/top.global"                #add by guanyao160707
GLOBALS "../../../tiptop/config/top.global"   #add by guanyao160707
...
FUNCTION t540sub_y_chk(l_pmm)
  ...
  DEFINE h_pmm01    LIKE pmm_file.pmm01       #add by hehw 210402
...
FUNCTION t540sub_y_upd(l_pmm01,p_action_choice)
  ...
  DEFINE l_pmmud06    LIKE pmm_file.pmmud06
  DEFINE l_tc_pmmud13 LIKE tc_pmm_file.tc_pmmud13
...
# Update ecm_file if exist
  ...
  END IF
  #add by donghy 补单的情况不考虑sgm 160827
  SELECT pmmud06 INTO l_pmmud06 FROM pmm_file WHERE pmm01 = l_pmm.pmm01
  IF NOT cl_null(l_pmmud06) THEN 
     SELECT tc_pmmud13 INTO l_tc_pmmud13 FROM tc_pmm_file WHERE tc_pmm01=l_pmmud06
  END IF 
  IF cl_null(l_tc_pmmud13) THEN LET l_tc_pmmud13 = 'N' END IF
  #add by donghy 补单的情况不考虑sgm 160827
  IF l_pmm.pmm02='SUB' AND g_prog<>'csfp001' AND l_tc_pmmud13='N' THEN  #mod by jixf 160801 csfp001根据报工倒推委外采购时，不需要更新sgm
...
DEFINE l_flag     LIKE type_file.num5    #No.FUN-A60011
     ...
     FOREACH t540sub_upd_ecm INTO l_pmn.*
        ...
        ELSE            #確認時
           ...
           #str-----mark by guanyao160909 #临时取消
...
FUNCTION t540sub_issue(p_pmm01,p_flag)
   ...
   DEFINE l_msg       LIKE type_file.chr1000   #CHI-CB0069 add   
   DEFINE l_pmn01     LIKE pmn_file.pmn01      #add by guanyao160707
   DEFINE l_pmn02     LIKE pmn_file.pmn02      #add by guanyao160707
   DEFINE l_x         LIKE type_file.num5      #add by guanyao160707
   DEFINE l_count     LIKE type_file.num5      #add by jixf 160801
 ...
 #IF l_pmm.pmm25 != '1' THEN CALL cl_err('pmm25 != 1','apm-299',1) RETURN END IF  #No.MOD-540007   #MOD-640492 0->1   #MOD-870002   #CHI-CB0069 mark
  ...
  IF l_pmm.pmm02  = 'SUB' THEN 
     SELECT COUNT(*) INTO l_count FROM pmn_file,sgm_file,ecd_file 
       WHERE pmn18=sgm01 AND pmn43=sgm03 AND sgm04=ecd01 AND ecd02 LIKE '%SMT%' AND pmn01=l_pmm.pmm01
     IF l_count > 0 THEN 
         LET l_pmn01 =''
         LET l_pmn02 =''
         DECLARE t540sub_pmm02 CURSOR FOR SELECT pmn01,pmn02 FROM pmn_file WHERE pmn01 = l_pmm.pmm01
         FOREACH t540sub_pmm02 INTO l_pmn01,l_pmn02
            LET l_x = 0
            #SELECT COUNT(*) INTO l_x FROM tc_sfp_file WHERE tc_sfp03 = l_pmn01 AND tc_sfp04=l_pmn02 AND tc_sfpconf = 'Y'
            SELECT COUNT(*) INTO l_x FROM tc_sfp_file WHERE tc_sfp03 = l_pmn01 AND tc_sfpconf = 'Y' 
               ---AND (tc_sfp13 = 0 OR tc_sfp13 IS NULL )  #mark  ly170816
{            
IF l_x =0 OR cl_null(l_x) THEN 
               CALL cl_err(l_pmn01||l_pmn02,'cpm-038',0)
               RETURN 
            END IF 
}   #mark ly170819 
         END FOREACH 
...
FUNCTION t540sub_z(p_pmm01,p_action_choice,p_call_transaction)       # when g_pmk.pmk18='Y' (Turn to 'N')
   ...
   DEFINE l_pmn01_1       LIKE pmn_file.pmn01    #add by guanyao160707
   DEFINE l_pmn02_1       LIKE pmn_file.pmn02    #add by guanyao160707
   DEFINE l_x             LIKE type_file.num5    #add by guanyao160707
...
#MOD-C10151 ----- mark end -----
  ...
  #str-----add by guanyao160707
   IF l_pmm.pmm02 = 'SUB' THEN 
      LET l_pmn01_1 =''
      LET l_pmn02_1 =''
      DECLARE t540sub_pmm02_1 CURSOR FOR SELECT pmn01,pmn02 FROM pmn_file WHERE pmn01 = l_pmm.pmm01
      FOREACH t540sub_pmm02_1 INTO l_pmn01_1,l_pmn02_1
         LET l_x = 0
         SELECT COUNT(*) INTO l_x FROM tc_sfp_file WHERE tc_sfp03 = l_pmn01_1 AND tc_sfp04=l_pmn02_1 AND tc_sfpconf != 'X' 
            AND (tc_sfp13 = 0 OR tc_sfp13 IS NULL)
         IF l_x >0 THEN 
            CALL cl_err(l_pmn01_1||l_pmn02_1,'cpm-045',0)
            RETURN 
         END IF 
      END FOREACH
   END IF 
...
#    DEFINE l_qty                LIKE ima_file.ima26    #No.FUN-680136 DECIMAL(15,3) #FUN-A20044
    ...
    DEFINE l_pml07      LIKE pml_file.pml07     #FUN-910088--add--
    DEFINE l_pml16              LIKE pml_file.pml16      #MOD-BC0168 add
    DEFINE l_tc_pmmud11         LIKE tc_pmm_file.tc_pmmud11   #add by guanyao160722
    DEFINE l_tc_pmmud06         LIKE tc_pmm_file.tc_pmmud06   #add by guanyao160722
    ...
    FOREACH t540_r_c INTO l_pmn.*
       ...
       IF l_pmm.pmm02  = 'SUB' THEN 
          LET l_tc_pmmud06 = ''
          LET l_tc_pmmud11 = ''
          SELECT tc_pmmud06,tc_pmmud11 INTO l_tc_pmmud06,l_tc_pmmud11
            FROM tc_pmm_file WHERE tc_pmmud06 = l_pmn.pmn01 AND tc_pmmud11 = l_pmn.pmn02
          IF NOT cl_null(l_tc_pmmud06) AND NOT cl_null(l_tc_pmmud11) THEN 
             UPDATE tc_pmm_file SET tc_pmmud06 =NULL ,tc_pmmud11 = NULL 
              WHERE tc_pmmud06 = l_pmn.pmn01 AND tc_pmmud11 = l_pmn.pmn02
             IF SQLCA.SQLERRD[3]=0 THEN 
                CALL cl_err3("UPDATE","tc_pmm_file",l_pmm.pmm01,"",SQLCA.sqlcode,"","upd tc_pmm_file",1)
                IF p_call_transaction THEN
                  ROLLBACK WORK
                END IF
                CLOSE t540sub_cl
