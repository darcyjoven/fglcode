...
# Modify.........: NO.160523 16/05/23 By guanyao 取消审核的时候删除生成的杂发单     


DATABASE ds

#GLOBALS "../../config/top.global"   #mark by guanyao160523
GLOBALS "../../../tiptop/config/top.global"   #mark by guanyao160523
GLOBALS "../../../tiptop/apm/4gl/sapmt720.global"#add by guanyao160523
#GLOBALS "../4gl/sapmt720.global"#mark by guanyao160523
GLOBALS
...
#str------add by guanyao160523
   IF g_success='Y' AND g_prog = 'apmt730' THEN 
      LET l_prog_1 = g_prog
      LET g_prog = 'aimt301'
      LET l_sql= "SELECT ina01,inaconf,inapost FROM ina_file,tc_sfp_file,rvu_file,rvv_file",
                  " WHERE ina10 = tc_sfp01 ",
                  "   AND tc_sfp11 = rvu02",
                  "   AND tc_sfp12 = rvv05",
                  "   AND rvv01 ='",g_rvu.rvu01,"'"

      PREPARE t720_ina FROM l_sql
      IF SQLCA.sqlcode THEN
      ...
      DECLARE t720_ina_cs CURSOR FOR t720_ina
      FOREACH t720_ina_cs INTO l_ina01,l_inaconf,l_inapost
         IF l_inaconf <> 'Y' THEN 
            CALL cl_err('','csf-010',0)
            LET g_success = 'N'  
         END IF 
         IF l_inapost <>'Y' THEN 
            CALL cl_err('','csf-011',0)
            LET g_success = 'N' 
         END IF 
         CALL t720_ina_undo_post(l_ina01)
         IF g_success = 'Y' THEN 
            CALL t370sub_z(l_ina01,'N',TRUE)
            IF g_success='Y' THEN 
               CALL t720_ina_r(l_ina01)
            END IF  
         END IF 
      END FOREACH
      LET g_prog = l_prog_1 
...
#str-----add by guanyao160523
FUNCTION t720_ina_undo_post(p_ina01)
