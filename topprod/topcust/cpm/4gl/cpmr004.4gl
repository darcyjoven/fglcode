...
MAIN
   ...
   LET  l_table = cl_prt_temptable('cpmr004',g_sql) CLIPPED
...
FUNCTION cpmr004_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,        
   ...
   OPEN WINDOW cpmr004_w AT p_row,p_col WITH FORM "cpm/42f/cpmr004"
...
WHILE TRUE
  ...
  END CONSTRUCT
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW cpmr004_w 
  ...
  #UI
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('cpmr004',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW cpmr004_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0094
          ...
          sr        RECORD
                    ...
                    rvv32    LIKE rvv_file.rvv32,     #收料仓库
                    rvv36    LIKE rvv_file.rvv36,     #订单单号
                    rvu07    LIKE rvu_file.rvu07,     #制单人编号
   ...
   LOCATE l_img_blob IN MEMORY             
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01='cpmr004' 
 
     LET tm.wc = tm.wc CLIPPED 
     
LET l_sql="select rvu04,rvu05,rvu03,rvu01,rvv02,rvv31,rvv031,ima021,rvv34,rvv17,rvv35,rvv32,rvv36,rvu07,''",
           " from rvv_file left join ima_file on rvv31 = ima01,rvu_file ",
           " where ",tm.wc ,
           "   AND rvu01=rvv01"
     PREPARE cpmr004_prepare1 FROM l_sql
     IF STATUS THEN CALL cl_err('prepare:',STATUS,1) 
        CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690126
        EXIT PROGRAM 
     END IF
     DECLARE cpmr004_curs1 CURSOR FOR cpmr004_prepare1
 
     FOREACH cpmr004_curs1 INTO sr.*
     ...
     CALL cl_prt_cs3('cpmr004','cpmr004',g_sql,g_str)
