...
DEFINE l_returnmsg      String
    ...
    END IF 
        ...
        IF l_pmm.pmm02 = 'SUB' THEN 
            ...
            #工艺报工不同步 begin ---- 
            SELECT count(*) INTO l_cnt FROM pmn_file WHERE pmn01= l_pmm.pmm01 AND pmn78 IS NOT NULL AND pmn78 <> ' '
            IF l_cnt > 0 THEN 
                LET l_ret.success = 'N'
                LET l_ret.msg = "工艺委外采购单",p_doc CLIPPED ,"不用同步SCM，请检查！"
               #LET l_ret.msg = cl_getmsg('',g_lang)
                RETURN l_ret.*
            END IF 
            #
...
DEFINE l_str_json       LIKE type_file.chr1000 
   ...
   #--发料需求--begin  --- 
    ...
    IF p_type MATCHES 'MR1' THEN   
       ...
       { IF l_tc_sfd.tc_sfd04 = 'Y' THEN
        ...
        FOREACH cs_get_tc_sfd_cs INTO l_tc_sff.*
                ...
                LET l_detail.oemCode                                  = ''
              #  #发料需求单 初始化取待发料量  -- 正式启用后注释掉 --- begin -- 
                select DFLL INTO l_detail.num from view_df where tc_sfd01 not in('SQA-21071666','SQA-21061203') AND tc_sfd01 = l_tc_sfd.tc_sfd01 
                AND tc_sff03 = l_tc_sff.tc_sff03 AND tc_sff07 = l_tc_sff.tc_sff07 AND tc_sff04 = l_tc_sff.tc_sff04
                IF cl_null(l_detail.num) OR STATUS  THEN 
                     CONTINUE FOREACH 
                END IF 
                #发料需求单 初始化取待发料量  -- 正式启用后注释掉 --- END  --
                LET l_json_detail = cjc_addWorktask_detail_json(l_detail.*)
    ...
    #过滤条件
        ...
        IF NOT cl_checkpostdate(l_oha.oha02) THEN 
            LET l_ret.success = 'N'
            LET l_ret.msg = "销退单(",p_doc CLIPPED,") 异动日期不能小于SCM开账日期或者对应参数tc_zsa03未设置，请检查！"
            LET l_ret.code = SQLCA.SQLCODE = '-----'
            RETURN l_ret.*
        END IF 
    ...
    IF NOT cl_checkpostdate(l_rvu.rvu03) THEN 
        LET l_ret.success = 'N'
        LET l_ret.msg = "仓退单(",p_doc CLIPPED,") 异动日期不能小于SCM开账日期或者对应参数tc_zsa03未设置，请检查！"
        LET l_ret.code = SQLCA.SQLCODE = '-----'
        RETURN l_ret.*
    END IF 
    ...
    # 倒扣发料同步------ begin ---- 
      IF p_type = 'DK1' OR p_type = 'OR1' THEN   
        ...
        END IF
        SELECT * INTO l_sfp.* FROM sfp_file
        WHERE sfp01 = p_doc AND sfp04='Y'
        IF SQLCA.SQLCODE THEN
            LET l_ret.success = 'N'
            LET l_ret.msg = "获取发料(退料)单(",p_doc CLIPPED,")失败或者未过账,SQLERR:",SQLCA.SQLCODE USING '-----'
            LET l_ret.code = SQLCA.SQLCODE = '-----'
            RETURN l_ret.*
        END IF

        IF NOT cl_checkpostdate(l_sfp.sfp03) THEN 
            LET l_ret.success = 'N'
            LET l_ret.msg = "发料(退料)单(",p_doc CLIPPED,") 异动日期不能小于SCM开账日期或者对应参数tc_zsa03未设置，请检查！"
            LET l_ret.code = SQLCA.SQLCODE = '-----'
            RETURN l_ret.*
        END IF 
    ...
    IF l_returnstatus = '0' AND (l_returnmsg = 'OK' OR  l_returnmsg MATCHES 'DO2*' OR  l_returnmsg MATCHES 'MR1*'  OR l_returnmsg MATCHES 'PT1*' OR  l_returnmsg MATCHES 'OR1*'
      OR  l_returnmsg MATCHES '*已同步*'  OR l_returnmsg MATCHES 'DK1*'
    )  THEN 
    	LET l_ret.success = 'Y' 
    	ELSE 
    	LET l_ret.success = 'N' 
   ...
   #          UPDATE tc_sft_file SET tc_sft06 = 'Y' WHERE tc_sft01 =  l_tc_sft.tc_sft01 
          ...
          WHEN 'DK1'
             LET l_ret.msg = "倒扣发料(",p_doc CLIPPED,")同步成功"
          WHEN 'MC1'
